...
flt AnimEditor::TimeEps() {
    flt w = AnimEdit.track.rect().w();
    w = (w ? 1 / w : 0);
    if (AnimEdit.anim)
        w *= AnimEdit.anim->length();
    return w * 0.025f;
}
void AnimEditor::Change::create(ptr user) {
    ...
    if (AnimEdit.skel)
        skel = *AnimEdit.skel;
    if (ElmAnim *anim_data = AnimEdit.data())
        T.anim_data = *anim_data;
    AnimEdit.undoVis();
}
void AnimEditor::Change::apply(ptr user) {
    if (AnimEdit.anim)
        if (ElmAnim *anim_data = AnimEdit.data()) {
            ...
            if (Elm *skel = Proj.findElm(T.anim_data.skel_id))
                if (ElmSkel *skel_data = skel->skelData())
                    pose = skel_data->transform; // if the undo used a different skeleton, then we have to set animation pose to match it, because we will use it now, so always when the animation is linked to a skeleton then use its pose
            *AnimEdit.anim = anim;
            AnimEdit.anim->transform(GetTransform(T.anim_data.transform(), pose()), skel, false); // transform from undo matrix to new matrix
            anim_data->undo(T.anim_data);
            anim_data->undoSrcFile(T.anim_data); // call 'undoSrcFile' separately because it may not be called in 'undo', however we need it in case speed adjustment was performed
...
void AnimEditor::Track::removedEvent(int index) {
    if (event_lit == index)
        event_lit = -1;
    else if (event_lit > index)
        event_lit--;
    if (event_sel == index)
        event_sel = -1;
    else if (event_sel > index)
        event_sel--;
}
...
void AnimEditor::Track::update(C GuiPC &gpc) {
    event_lit = -1;
    if (gpc.visible && visible() && Gui.ms() == this)
        if (Animation *anim = AnimEdit.anim) {
            ...
            if (events) {
                // get lit
                flt dist = 0;
                if ((always_draw_events || AnimEdit.preview.event_op() >= 0) && AnimEdit.preview.event_op() != EVENT_NEW)
                    REPA(anim->events) {
                        ...
                        if (event_lit < 0 || d < dist) {
                            ...
                            int j = i;
                            for (; j > 0 && Equal(anim->events[j - 1].time, event_time);)
                                j--; // same is j..i inclusive
                            int same = i - j + 1;
                            if (same > 1)                                       // if more than 1
                ...
                if (AnimEdit.preview.event_op() == EVENT_RENAME && Ms.bp(0))
                    RenameEvent.activate(event_lit);
                if (AnimEdit.preview.event_op() == EVENT_DEL && Ms.bp(0))
                    AnimEdit.delEvent(event_lit);
                if (AnimEdit.preview.event_op() == EVENT_NEW && Ms.bp(0))
                    AnimEdit.newEvent(time);
                if (AnimEdit.preview.event_op() == EVENT_MOVE && Ms.b(0)) {
                    if (Ms.bp(0))
                        event_sel = event_lit;
                    AnimEdit.moveEvent(event_sel, time);
                    event_lit = event_sel;
                }

                // set time
                if (AnimEdit.preview.event_op() < 0 && Ms.b(0))
                    AnimEdit.animTime(time);
            } else {
                // set time
                if (Ms.b(0))
                    AnimEdit.animTime(time);
                ...
                // move KeyFrame
                if (Ms.b(1))
                    if (AnimKeys *keys = AnimEdit.findKeys(AnimEdit.sel_bone)) {
                        ...
                        if (Kb.ctrlCmd()) // all
                        {
                            ...
                            {
                                if (Kb.alt())
                                    dt = -AnimEdit.animTime();
                                if (AnimEdit.sel_bone < 0) // entire animation
                                {
                                    AnimEdit.anim->offsetTime(dt);
                                } else {
                                    REPA(keys->orns)
                                    Slide(keys->orns[i].time, dt, anim->length());
                                    REPA(keys->poss)
                                    Slide(keys->poss[i].time, dt, anim->length());
                                    REPA(keys->scales)
                                    Slide(keys->scales[i].time, dt, anim->length());
                                    if (AnimEdit.op() == OP_ORN2 && keys_parent) {
                                        REPA(keys_parent->orns)
                                        Slide(keys_parent->orns[i].time, dt, anim->length());
                                        REPA(keys_parent->poss)
                                        Slide(keys_parent->poss[i].time, dt, anim->length());
                                        REPA(keys_parent->scales)
                                        Slide(keys_parent->scales[i].time, dt, anim->length());
                                    }
                                }
                            }
                            ...
                            case OP_ORN:
                                ...
                                break;
                            case OP_POS:
                                if (AnimKeys::Pos *key = FindPos(*keys, AnimEdit.key_time))
                                    AnimEdit.key_time = Slide(key->time, dt, anim->length());
                                else
                                    AnimEdit.key_time = AnimEdit.animTime();
                                break;
                            case OP_SCALE:
                                if (AnimKeys::Scale *key = FindScale(*keys, AnimEdit.key_time))
                                    AnimEdit.key_time = Slide(key->time, dt, anim->length());
                                else
                                    AnimEdit.key_time = AnimEdit.animTime();
                                break;
                            case OP_ORN2: {
                                if (AnimKeys::Orn *key = FindOrn(*keys, AnimEdit.key_time)) {
                                    if (keys_parent)
                                        if (AnimKeys::Orn *key = FindOrn(*keys_parent, AnimEdit.key_time))
                                            Slide(key->time, dt, anim->length()); // !! first process the parent without modifying the 'key_time' !!
                                    AnimEdit.key_time = Slide(key->time, dt, anim->length());
                                } else
                                    AnimEdit.key_time = AnimEdit.animTime();
                            } break;
                        ...
                        anim->setRootMatrix();
                        flt t = AnimEdit.animTime();
                        AnimEdit.animTime(Slide(t, dt, anim->length()));
                        AnimEdit.setChanged();
                    }
            }
        }
...
void AnimEditor::Track::DrawKeys(AnimKeys &keys, C Rect &rect, C Color &color, flt y, flt r, int row, bool lit) {
    ...
    case TRACK_ORN:
        REPA(keys.orns)
        DrawKey(keys.orns[i].time, rect, y, r);
        break;
    case TRACK_POS:
        REPA(keys.poss)
        DrawKey(keys.poss[i].time, rect, y, r);
        break;
    case TRACK_SCALE:
        REPA(keys.scales)
        DrawKey(keys.scales[i].time, rect, y, r);
        break;
    }
    VI.end();
    ...
    if (lit)
        switch (row) {
        case TRACK_ORN:
            if (AnimEdit.op() == OP_ORN || AnimEdit.op() == OP_ORN2)
                if (AnimKeys::Orn *key = FindOrn(keys, AnimEdit.animTime())) {
                    VI.color(WHITE);
                    DrawKey(key->time, rect, y, r);
                    VI.end();
                }
            break;
        case TRACK_POS:
            if (AnimEdit.op() == OP_POS)
                if (AnimKeys::Pos *key = FindPos(keys, AnimEdit.animTime())) {
                    VI.color(WHITE);
                    DrawKey(key->time, rect, y, r);
                    VI.end();
                }
            break;
        case TRACK_SCALE:
            if (AnimEdit.op() == OP_SCALE)
                if (AnimKeys::Scale *key = FindScale(keys, AnimEdit.animTime())) {
                    VI.color(WHITE);
                    DrawKey(key->time, rect, y, r);
                    VI.end();
                }
...
void AnimEditor::Track::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        ...
        if (C Animation *anim = AnimEdit.getVisAnim()) {
            ...
            if (events) {
                TextStyleParams ts;
                ts.size = (AnimEdit.preview.event_op() >= 0 ? 0.04f : 0.035f);
                ts.align.set(0, 1);
                ts.color = ColorAlpha(0.6f);
                TextStyleParams ts_lit = ts;
                ts_lit.color = LitColor;
                ts_lit.size *= 1.1f;
                ...
                FREPA(anim->events) // draw events, from start
                {
                    ...
                    e.draw((event_lit == i) ? LitColor : LitSelColor);
                    if ((always_draw_events || AnimEdit.preview.event_op() >= 0)) {
                        pos.x = e.centerX();
                ...
                FREP(TRACK_NUM) {
                    ...
                    if (lit_keys)
                        DrawKeys(*lit_keys, r, ColorAlpha(Lerp(color, WHITE, 0.6f), 0.5f), y, s * 1.1f, i);
                    if (sel_keys)
                        DrawKeys(*sel_keys, r, color, y, s * 0.5f, i, true);
                }

                D.clip();
                bool editing = false;
                if (Ms.b(1))
                    if (Edit::Viewport4::View *view = AnimEdit.v4.getView(Gui.ms()))
                        editing = true;
                ...
                flt frame, frames;
                Str frame_t;
                if (AnimEdit.timeToFrame(AnimEdit.animTime(), frame) && AnimEdit.timeToFrame(anim->length(), frames))
                    frame_t = S + ", " + Round(frame) + '/' + Round(frames) + "f";
                D.text(ObjEdit.ts, p, S + "Time: " + AnimEdit.animTime() + '/' + anim->length() + "s (" + Round(AnimEdit.timeToFrac(AnimEdit.animTime()) * 100) + '%' + frame_t + ')');
                p.y -= ObjEdit.ts.size.y;
                if (1) {
                    D.text(ObjEdit.ts, p, S + "Speed: " + anim->rootSpeed());
                    p.y -= ObjEdit.ts.size.y;
                }
                if (skel && InRange(bone, skel->bones)) {
                    D.text(ObjEdit.ts, p, S + "Bone \"" + skel->bones[bone].name + "\", Parent: " + (InRange(skel->bones[bone].parent, skel->bones) ? S + '"' + skel->bones[skel->bones[bone].parent].name + '"' : S + "none"));
                    p.y -= ObjEdit.ts.size.y;
                ...
                if (Gui.ms() == this) {
                    ...
                    if (AnimEdit.timeToFrame(time, frame))
                        frame_t = S + ", " + Round(frame) + "f";
                    else
                        frame_t.clear();
                    TextStyleParams ts;
                    ts.align.set(0, 1);
                    ...
                    flt w_2 = ts.textWidth(t) / 2, x = Ms.pos().x;
                    Clamp(x, r.min.x + w_2, r.max.x - w_2);
...
Str AnimEditor::Preview::Loop(C Preview &editor) {
    if (ElmAnim *d = AnimEdit.data())
        return d->loop();
    return false;
}
void AnimEditor::Preview::Linear(Preview &editor, C Str &t) { AnimEdit.setLinear(TextBool(t)); }
Str AnimEditor::Preview::Linear(C Preview &editor) {
    if (ElmAnim *d = AnimEdit.data())
        return d->linear();
    return false;
}
...
void AnimEditor::Preview::Render() {
    ...
    case RM_PREPARE: {
        if (AnimEdit.mesh)
            AnimEdit.mesh->draw(AnimEdit.anim_skel);

        if (AnimEdit.preview.draw_slots)
            REPAO(AnimEdit.slot_meshes).draw(AnimEdit.anim_skel);
...
void AnimEditor::Preview::draw() {
    ...
    if (AnimEdit.skel) {
        ...
        SetMatrix();
        if (draw_plane) {
            bool line_smooth = D.lineSmooth(false);
            Plane(VecZero, Vec(0, 1, 0)).drawInfiniteByResolution(Color(255, 128), 192);
        ...
        if (draw_bones || draw_slots) {
            ...
            if (draw_slots)
                FREPAO(anim_skel.slots).draw((sel_slot == i && lit_slot == i) ? LitSelColor : (sel_slot == i || lit_slot == i) ? SelColor
                                                                                                                               : Color(255, 128, 0, 255),
                                             SkelSlotSize);
            D.depthUnlock();
        }
...
void AnimEditor::Preview::create() {
    ...
    autoData(&AnimEdit);
    flt h = 0.043f;
    Rect r = super::create("Animation Editor", Vec2(0.02f, -0.02f), 0.036f, h, 0.35f);
    button[1].show();
...
void AnimEditor::Preview::toGui() {
    super::toGui();
    if (length)
        length->display(S + "Length: " + (anim() ? S + TextReal(anim()->length(), -2) + "s" : S));
}
::AnimEditor::Preview &AnimEditor::Preview::hide() {
    if (!AnimEdit.fullscreen)
        AnimEdit.set(null);
    super::hide();
    return T;
}
Rect AnimEditor::Preview::sizeLimit() C {
...
::AnimEditor::Preview &AnimEditor::Preview::rect(C Rect &rect) {
    ...
    if (event)
        event->text.pos(event_op.rect().lu() + Vec2(0, 0.025f));
    return T;
}
void AnimEditor::Preview::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        if (contains(Gui.kb())) {
            ...
            if (Gui.kb()->type() != GO_TEXTLINE && Gui.kb()->type() != GO_CHECKBOX) {
                if (Kb.bp(KB_INS)) {
                    Kb.eat(KB_INS);
                    AnimEdit.newEvent(AnimEdit.animTime());
                }
                if (Kb.bp(KB_DEL) && Kb.ctrlCmd()) {
                    Kb.eat(KB_DEL);
                    AnimEdit.delEvent(track.event_lit);
                }
                if (Kb.bp(KB_SPACE)) {
                    Kb.eat(KB_SPACE);
                    AnimEdit.playToggle();
                }
                if (Kb.bp(KB_HOME) && Kb.ctrlCmd()) {
                    Kb.eat(KB_HOME);
                    Start(AnimEdit);
                }
                if (Kb.bp(KB_END) && Kb.ctrlCmd()) {
                    Kb.eat(KB_END);
                    End(AnimEdit);
                }
            }
        ...
        if (Gui.ms() == &viewport)
            if (Ms.b(0) || Ms.b(MS_BACK)) {
                cam_yaw -= Ms.d().x;
                cam_pitch += Ms.d().y;
                Ms.freeze();
            }
        if (Gui.wheel() == &viewport)
            Clamp(cam_zoom *= ScaleFactor(Ms.wheel() * -0.2f), 0.1f, 32);
        REPA(Touches)
        if (Touches[i].guiObj() == &viewport && Touches[i].on()) {
            cam_yaw -= Touches[i].ad().x * 2.0f;
            cam_pitch += Touches[i].ad().y * 2.0f;
        }
        ...
        if (draw_slots && Gui.dragging()) {
            ...
            REPA(AnimEdit.anim_skel.slots) {
                ...
                flt d;
                if (Distance2D(Ms.pos(), Edge(slot.pos, slot.pos + slot.perp * SkelSlotSize), d, 0))
                    if (d < dist) {
                        dist = d;
                        lit_slot = i;
                    }
    ...
    if (AnimEdit.anim)                            // if we have animation
        ...
        {
            delta_t = Frac(AnimEdit.animTime() - old_time, length);
            if (delta_t >= length * 0.5f)
                delta_t -= length;
        } // use delta based on anim time delta
}
...
Animation *AnimEditor::OptimizeAnim::getAnim() {
    ...
    if (refresh_needed) {
        ...
        if (file_size) {
            ...
            file_size->display(S + "File Size: " + SizeKB(f.size()));
        }
        optimizeDo(anim);
        if (optimized_size) {
            anim.save(f.reset());
            optimized_size->display(S + "Optimized Size: " + SizeKB(f.size()));
        }
    }
...
::AnimEditor::OptimizeAnim &AnimEditor::OptimizeAnim::create() {
    ...
    file_size = &add();
    optimized_size = &add();
    Rect r = super::create("Reduce Keyframes", Vec2(0.02f, -0.02f), 0.040f, 0.046f);
    button[2].show();
    autoData(this);
    resize(Vec2(0, 0.1f));
...
::AnimEditor::ScalePosKeys &AnimEditor::ScalePosKeys::create() {
    ...
    Rect r = super::create("Scale Pos Keys", Vec2(0.02f, -0.02f), 0.040f, 0.046f);
    button[2].show();
    autoData(this);
    resize(Vec2(0, 0.1f));
...
::AnimEditor::TimeRangeSpeed &AnimEditor::TimeRangeSpeed::create() {
    ...
    Rect r = super::create("Time Range Speed", Vec2(0.02f, -0.02f), 0.040f, 0.046f);
    button[2].show();
    autoData(this);
    resize(Vec2(0, 0.1f));
...
void AnimEditor::Render() {
    switch (Renderer()) {
    case RM_PREPARE: {
        if (AnimEdit.draw_mesh() && AnimEdit.mesh)
            AnimEdit.mesh->draw(AnimEdit.anim_skel);
...
void AnimEditor::Draw(Viewport &viewport) {
    if (Edit::Viewport4::View *view = AnimEdit.v4.getView(&viewport))
        AnimEdit.draw(*view);
}
    ...
    if (draw_bones() || settings() == 0) // always draw axis for root when settings are visible
    {
        ...
        if (draw_bones()) {
            ...
            REPA(anim_skel.bones) {
                ...
                bool has_keys = false;
                if (AnimKeys *keys = findVisKeys(i))
                    switch (op()) {
                    case OP_ORN:
                    case OP_ORN2:
                        has_keys = (keys->orns.elms() > 0);
                        break;
                    case OP_POS:
                        has_keys = (keys->poss.elms() > 0);
                        break;
                    case OP_SCALE:
                        has_keys = (keys->scales.elms() > 0);
                        break;
                    }
            ...
            // if(skel && InRange(sel_bone, skel.bones)) don't check this so we can draw just for root too
            {
                ...
                if (op() == OP_POS || op() == OP_SCALE || (op() == OP_ORN || op() == OP_ORN2 || (anim && anim->keys.is()) || settings() == 0) && sel_bone < 0 // always draw axis for root
                )
                    DrawMatrix(transformedBoneAxis(drawn_matrix_for_bone = sel_bone), bone_axis);
            }
        }
        if (drawn_matrix_for_bone != -1 && settings() == 0)
            DrawMatrix(transformedBoneAxis(-1), -1); // always draw axis for root when (not yet drawn and settings are visible)
        D.depthUnlock();
    }
    D.lineSmooth(line_smooth);
}
...
void AnimEditor::End(AnimEditor &editor) {
    if (editor.anim)
        editor.animTime(editor.anim->length());
}
...
void AnimEditor::Mirror(AnimEditor &editor) {
    if (editor.anim) {
        editor.undos.set("mirror", true);
        Skeleton temp;
        editor.anim->mirror(editor.skel ? *editor.skel : temp);
        editor.setAnimSkel();
        editor.setOrnTarget();
        editor.setChanged();
        editor.toGui();
...
void AnimEditor::rotate(C Matrix3 &m) {
    if (anim) {
        ...
        if (ElmAnim *d = data())
            if (d->rootMove()) {
                Vec root_move = d->root_move * m;
                d->rootMove(root_move);
            } // !! don't do "d.root_move*=m" because we need to call 'rootMove' !!
        Skeleton temp;
        anim->transform(m, skel ? *skel : temp, true);
        setAnimSkel();
        setOrnTarget();
        setChanged();
        toGui();
...
void AnimEditor::transformObj(bool transform_anims, C UID &ignore_anim_id) {
    ...
    if (anim)
        if (Elm *obj = Proj.animToObjElm(elm)) {
            ...
            Gui.msgBox(S, "There's no Object associated with this Animation.");
}
...
void AnimEditor::RootDelPos(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        ...
        FlagSet(d->flag, ElmAnim::ROOT_DEL_POS, on); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (on) {
            Skeleton temp, &skel = editor.skel ? *editor.skel : temp;
            editor.anim->adjustForSameTransformWithDifferentSkeleton(skel, skel, -1, null, ROOT_DEL_POSITION);
            editor.setAnimSkel();
            editor.setOrnTarget();
            editor.toGui();
        }
...
void AnimEditor::RootDelRot(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        ...
        FlagSet(d->flag, ElmAnim::ROOT_DEL_ROT, on); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (on) {
            Skeleton temp, &skel = editor.skel ? *editor.skel : temp;
            editor.anim->adjustForSameTransformWithDifferentSkeleton(skel, skel, -1, null, ROOT_DEL_ROTATION);
            editor.setAnimSkel();
            editor.setOrnTarget();
            editor.toGui();
        }
...
void AnimEditor::RootDel(AnimEditor &editor) {
    if (ElmAnim *d = editor.data())
        if (editor.skel) {
            ...
            if (d->flag != flag) {
                editor.anim->adjustForSameTransformWithDifferentSkeleton(*editor.skel, *editor.skel, -1, null, ROOT_DEL_POSITION | ROOT_DEL_ROTATION);
                editor.setAnimSkel();
                editor.setOrnTarget();
                editor.toGui();
                editor.setChanged();
...
void AnimEditor::RootSmooth(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        ...
        FlagSet(d->flag, ElmAnim::ROOT_SMOOTH_ROT_POS, on); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (on) {
            Skeleton temp, &skel = editor.skel ? *editor.skel : temp;
            editor.anim->adjustForSameTransformWithDifferentSkeleton(skel, skel, -1, null, ROOT_SMOOTH_ROT_POS);
            editor.setAnimSkel();
            editor.setOrnTarget();
            editor.toGui();
        }
...
void AnimEditor::RootDelPosX(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootDelPos");
        FlagToggle(d->flag, ElmAnim::ROOT_DEL_POS_X); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (d->flag & ElmAnim::ROOT_DEL_POS_X) {
            Skeleton temp, &skel = editor.skel ? *editor.skel : temp;
            editor.anim->adjustForSameTransformWithDifferentSkeleton(skel, skel, -1, null, ROOT_DEL_POSITION_X);
            editor.setAnimSkel();
            editor.setOrnTarget();
            editor.toGui();
        }
...
void AnimEditor::RootDelPosY(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootDelPos");
        FlagToggle(d->flag, ElmAnim::ROOT_DEL_POS_Y); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (d->flag & ElmAnim::ROOT_DEL_POS_Y) {
            Skeleton temp, &skel = editor.skel ? *editor.skel : temp;
            editor.anim->adjustForSameTransformWithDifferentSkeleton(skel, skel, -1, null, ROOT_DEL_POSITION_Y);
            editor.setAnimSkel();
            editor.setOrnTarget();
            editor.toGui();
        }
...
void AnimEditor::RootDelPosZ(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootDelPos");
        FlagToggle(d->flag, ElmAnim::ROOT_DEL_POS_Z); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (d->flag & ElmAnim::ROOT_DEL_POS_Z) {
            Skeleton temp, &skel = editor.skel ? *editor.skel : temp;
            editor.anim->adjustForSameTransformWithDifferentSkeleton(skel, skel, -1, null, ROOT_DEL_POSITION_Z);
            editor.setAnimSkel();
            editor.setOrnTarget();
            editor.toGui();
        }
...
void AnimEditor::RootDelRotX(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootDelRot");
        FlagToggle(d->flag, ElmAnim::ROOT_DEL_ROT_X); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (d->flag & ElmAnim::ROOT_DEL_ROT_X) {
            Skeleton temp, &skel = editor.skel ? *editor.skel : temp;
            editor.anim->adjustForSameTransformWithDifferentSkeleton(skel, skel, -1, null, ROOT_DEL_ROTATION_X);
            editor.setAnimSkel();
            editor.setOrnTarget();
            editor.toGui();
        }
...
void AnimEditor::RootDelRotY(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootDelRot");
        FlagToggle(d->flag, ElmAnim::ROOT_DEL_ROT_Y); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (d->flag & ElmAnim::ROOT_DEL_ROT_Y) {
            Skeleton temp, &skel = editor.skel ? *editor.skel : temp;
            editor.anim->adjustForSameTransformWithDifferentSkeleton(skel, skel, -1, null, ROOT_DEL_ROTATION_Y);
            editor.setAnimSkel();
            editor.setOrnTarget();
            editor.toGui();
        }
...
void AnimEditor::RootDelRotZ(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootDelRot");
        FlagToggle(d->flag, ElmAnim::ROOT_DEL_ROT_Z); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (d->flag & ElmAnim::ROOT_DEL_ROT_Z) {
            Skeleton temp, &skel = editor.skel ? *editor.skel : temp;
            editor.anim->adjustForSameTransformWithDifferentSkeleton(skel, skel, -1, null, ROOT_DEL_ROTATION_Z);
            editor.setAnimSkel();
            editor.setOrnTarget();
            editor.toGui();
        }
...
void AnimEditor::RootSmoothRot(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootSmooth");
        FlagToggle(d->flag, ElmAnim::ROOT_SMOOTH_ROT); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (d->flag & ElmAnim::ROOT_SMOOTH_ROT) {
            Skeleton temp, &skel = editor.skel ? *editor.skel : temp;
            editor.anim->adjustForSameTransformWithDifferentSkeleton(skel, skel, -1, null, ROOT_SMOOTH_ROT);
            editor.setAnimSkel();
            editor.setOrnTarget();
            editor.toGui();
        }
...
void AnimEditor::RootSmoothPos(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootSmooth");
        FlagToggle(d->flag, ElmAnim::ROOT_SMOOTH_POS); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (d->flag & ElmAnim::ROOT_SMOOTH_POS) {
            Skeleton temp, &skel = editor.skel ? *editor.skel : temp;
            editor.anim->adjustForSameTransformWithDifferentSkeleton(skel, skel, -1, null, ROOT_SMOOTH_POS);
            editor.setAnimSkel();
            editor.setOrnTarget();
            editor.toGui();
        }
...
void AnimEditor::RootFromBody(AnimEditor &editor) {
    if (ElmAnim *d = editor.data()) {
        ...
        FlagToggle(d->flag, ElmAnim::ROOT_FROM_BODY); /*d.file_time.getUTC(); already changed in 'setChanged' */
        if (d->flag & ElmAnim::ROOT_FROM_BODY)
            if (editor.skel) {
                editor.anim->adjustForSameTransformWithDifferentSkeleton(*editor.skel, *editor.skel, Max(0, editor.skel->findBoneI(BONE_SPINE)), null, d->rootFlags());
                editor.setAnimSkel();
                editor.setOrnTarget();
                editor.toGui();
            }
...
Str AnimEditor::RootMoveX(C AnimEditor &editor) {
    if (ElmAnim *d = editor.data())
        if (d->rootMove())
            return d->root_move.x * d->transform.scale;
    if (editor.anim)
        return editor.anim->rootTransform().pos.x;
    return S;
}
Str AnimEditor::RootMoveY(C AnimEditor &editor) {
    if (ElmAnim *d = editor.data())
        if (d->rootMove())
            return d->root_move.y * d->transform.scale;
    if (editor.anim)
        return editor.anim->rootTransform().pos.y;
    return S;
}
Str AnimEditor::RootMoveZ(C AnimEditor &editor) {
    if (ElmAnim *d = editor.data())
        if (d->rootMove())
            return d->root_move.z * d->transform.scale;
    if (editor.anim)
        return editor.anim->rootTransform().pos.z;
    return S;
}
void AnimEditor::RootMoveX(AnimEditor &editor, C Str &t) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootMove");
        editor.root_set_move.set(true, QUIET);
        if (!d->rootMove())
            d->root_move = editor.anim->rootTransform().pos / d->transform.scale;
        d->root_move.x = TextFlt(t) / d->transform.scale;
        d->rootMove(d->root_move); /*d.file_time.getUTC(); already changed in 'setChanged' */
        d->setRoot(*editor.anim);
        editor.setAnimSkel();
        editor.setOrnTarget();
        editor.setChanged();
...
void AnimEditor::RootMoveY(AnimEditor &editor, C Str &t) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootMove");
        editor.root_set_move.set(true, QUIET);
        if (!d->rootMove())
            d->root_move = editor.anim->rootTransform().pos / d->transform.scale;
        d->root_move.y = TextFlt(t) / d->transform.scale;
        d->rootMove(d->root_move); /*d.file_time.getUTC(); already changed in 'setChanged' */
        d->setRoot(*editor.anim);
        editor.setAnimSkel();
        editor.setOrnTarget();
...
void AnimEditor::RootMoveZ(AnimEditor &editor, C Str &t) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootMove");
        editor.root_set_move.set(true, QUIET);
        if (!d->rootMove())
            d->root_move = editor.anim->rootTransform().pos / d->transform.scale;
        d->root_move.z = TextFlt(t) / d->transform.scale;
        d->rootMove(d->root_move); /*d.file_time.getUTC(); already changed in 'setChanged' */
        d->setRoot(*editor.anim);
        editor.setAnimSkel();
        editor.setOrnTarget();
...
Str AnimEditor::RootRotX(C AnimEditor &editor) {
    if (ElmAnim *d = editor.data())
        if (d->rootRot())
            return TextReal(RadToDeg(d->root_rot.x), 1);
    if (editor.anim)
        return TextReal(RadToDeg(editor.anim->rootTransform().axisAngle().x), 1);
    return S;
}
Str AnimEditor::RootRotY(C AnimEditor &editor) {
    if (ElmAnim *d = editor.data())
        if (d->rootRot())
            return TextReal(RadToDeg(d->root_rot.y), 1);
    if (editor.anim)
        return TextReal(RadToDeg(editor.anim->rootTransform().axisAngle().y), 1);
    return S;
}
Str AnimEditor::RootRotZ(C AnimEditor &editor) {
    if (ElmAnim *d = editor.data())
        if (d->rootRot())
            return TextReal(RadToDeg(d->root_rot.z), 1);
    if (editor.anim)
        return TextReal(RadToDeg(editor.anim->rootTransform().axisAngle().z), 1);
    return S;
}
void AnimEditor::RootRotX(AnimEditor &editor, C Str &t) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootRot");
        editor.root_set_rot.set(true, QUIET);
        if (!d->rootRot())
            d->root_rot = editor.anim->rootTransform().axisAngle();
        d->root_rot.x = DegToRad(TextFlt(t));
        d->rootRot(d->root_rot); /*d.file_time.getUTC(); already changed in 'setChanged' */
        d->setRoot(*editor.anim);
        editor.setAnimSkel();
        editor.setOrnTarget();
        editor.setChanged();
...
void AnimEditor::RootRotY(AnimEditor &editor, C Str &t) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootRot");
        editor.root_set_rot.set(true, QUIET);
        if (!d->rootRot())
            d->root_rot = editor.anim->rootTransform().axisAngle();
        d->root_rot.y = DegToRad(TextFlt(t));
        d->rootRot(d->root_rot); /*d.file_time.getUTC(); already changed in 'setChanged' */
        d->setRoot(*editor.anim);
        editor.setAnimSkel();
        editor.setOrnTarget();
...
void AnimEditor::RootRotZ(AnimEditor &editor, C Str &t) {
    if (ElmAnim *d = editor.data()) {
        editor.undos.set("rootRot");
        editor.root_set_rot.set(true, QUIET);
        if (!d->rootRot())
            d->root_rot = editor.anim->rootTransform().axisAngle();
        d->root_rot.z = DegToRad(TextFlt(t));
        d->rootRot(d->root_rot); /*d.file_time.getUTC(); already changed in 'setChanged' */
        d->setRoot(*editor.anim);
        editor.setAnimSkel();
        editor.setOrnTarget();
...
void AnimEditor::setSelMirror(bool set_other) {
    if (skel) {
        ...
        if (SkelBone *bone = skel->bones.addr(bone_i)) {
            ...
            REPA(skel->bones)
            if (i != bone_i && bone_name == BoneNeutralName(skel->bones[i].name)) {
...
void AnimEditor::skelBonePosPaste(C Vec &mask) {
    if (skel && anim) {
        ...
        Vec delta = (copied_bone_pos - pos) * mask;
        if (delta.any()) {
...
Animation *AnimEditor::getVisAnim() { return scale_pos_keys.visibleOnActiveDesktop() ? scale_pos_keys.getAnim() : optimize_anim.visibleOnActiveDesktop() ? optimize_anim.getAnim()
                                                                                                                                                         : anim; }
...
bool AnimEditor::timeToFrame(flt time, flt &frame) C {
    if (ElmAnim *d = data())
        if (d->fps > 0) {
            frame = time * d->fps;
            FileParams fps = d->imported_file_params;
...
void AnimEditor::setLoop(bool loop) {
    if (ElmAnim *d = data())
        if (d->loop() != loop) {
            ...
            setAnimSkel();
            setOrnTarget();
            setChanged(false); // set 'file' to false because the Editor will recreate the animation file upon receiving new data from server
        }
}
...
void AnimEditor::setOrnTarget() {
    ...
    if (sel_bone >= 0)
        bone_parent = transformedBone(boneParent(sel_bone)); // set 'bone_parent' only if we have a bone selected, otherwise (when root is selected) then keep it as identity

    orn_target = bone.to();

    orn_perp = bone.pos - NearestPointOnLine(bone.pos, bone_parent.pos, !(orn_target - bone_parent.pos));
    if (!orn_perp.normalize())
        orn_perp.set(0, 0, 1);
}
...
AnimEditor &AnimEditor::create() {
    ...
    Rect r = AddProperties(props, T, play.rect().ru() + Vec2(0.01f, 0), play.rect().h(), 0.15f, &ObjEdit.ts);
    REPAO(props).autoData(this);
    ...
    T += cmd.create(Rect_LU(op.rect().ru(), h), n).focusable(false);
    cmd.flag |= COMBOBOX_CONST_TEXT;
    ...
    settings.tab(0).desc("Keyboard Shortcut: Alt+S") += settings_region.create(Rect_LU(settings.rect().ru(), 0.333f + h * 3, 0.5f)).skin(&TransparentSkin, false);
    settings_region.kb_lit = false;
    {
        flt x = 0.01f, y = -0.005f, l = h * 1.1f, ll = l * 1.1f, w = 0.25f, ph = h * 0.95f;
        ts.reset().size = 0.041f;
        settings_region += t_settings.create(Vec2(settings_region.clientWidth() / 2, y - h / 2), "Settings also affect reloading", &ts);
        y -= ll;
        settings_region += loop.create(Rect_U(settings_region.clientWidth() * (1.0f / 3.5f), y, 0.15f, h), "Loop").func(Loop, T);
        loop.mode = BUTTON_TOGGLE;
        settings_region += linear.create(Rect_U(settings_region.clientWidth() * (2.5f / 3.5f), y, 0.15f, h), "Linear").func(Linear, T);
        linear.mode = BUTTON_TOGGLE;
        y -= ll;
        settings_region += t_settings_root.create(Vec2(x, y - h / 2), "Root:", &ObjEdit.ts);
        y -= l;
        settings_region += root_del_pos.create(Rect_LU(x, y, w, h), "Del Position").func(RootDelPos, T);
        settings_region += root_del_pos_x.create(Rect_LU(root_del_pos.rect().ru() + Vec2(0.01f, 0), h, h), "X").func(RootDelPosX, T).subType(BUTTON_TYPE_TAB_LEFT);
        root_del_pos_x.mode = BUTTON_TOGGLE;
        settings_region += root_del_pos_y.create(Rect_LU(root_del_pos_x.rect().ru(), h, h), "Y").func(RootDelPosY, T).subType(BUTTON_TYPE_TAB_HORIZONTAL);
        root_del_pos_y.mode = BUTTON_TOGGLE;
        settings_region += root_del_pos_z.create(Rect_LU(root_del_pos_y.rect().ru(), h, h), "Z").func(RootDelPosZ, T).subType(BUTTON_TYPE_TAB_RIGHT);
        root_del_pos_z.mode = BUTTON_TOGGLE;
        y -= l;
        settings_region += root_del_rot.create(Rect_LU(x, y, w, h), "Del Rotation").func(RootDelRot, T);
        settings_region += root_del_rot_x.create(Rect_LU(root_del_rot.rect().ru() + Vec2(0.01f, 0), h, h), "X").func(RootDelRotX, T).subType(BUTTON_TYPE_TAB_LEFT);
        root_del_rot_x.mode = BUTTON_TOGGLE;
        settings_region += root_del_rot_y.create(Rect_LU(root_del_rot_x.rect().ru(), h, h), "Y").func(RootDelRotY, T).subType(BUTTON_TYPE_TAB_HORIZONTAL);
        root_del_rot_y.mode = BUTTON_TOGGLE;
        ...
        root_smooth_rot.mode = BUTTON_TOGGLE;
        root_smooth_rot.desc("Set smooth root rotation with constant velocities");
        settings_region += root_smooth_pos.create(Rect_LU(root_smooth_rot.rect().ru(), h * 2, h), "Pos").func(RootSmoothPos, T).subType(BUTTON_TYPE_TAB_RIGHT);
        root_smooth_pos.mode = BUTTON_TOGGLE;
        root_smooth_pos.desc("Set smooth root movement with constant velocities");
        y -= l;
        settings_region += root_set_move.create(Rect_LU(x, y, w, h), "Set Movement").func(RootSetMove, T);
        root_set_move.mode = BUTTON_TOGGLE;
        root_set_move.desc("Override Animation's root movement with a custom value");
        ...
        Rect r = AddProperties(root_props, settings_region, Vec2(x + w + 0.01f, y), ph, 0.17f, &ObjEdit.ts);
        REPAO(root_props).autoData(this);
        y = Min(r.min.y, reload.rect().min.y);
...
int AnimEditor::getBone(GuiObj *go, C Vec2 &screen_pos) {
    ...
    if (Edit::Viewport4::View *view = v4.getView(go)) {
        ...
        REPA(anim_skel.bones) {
            SkelBone bone = transformedBone(i);
            flt d;
            if (Distance2D(screen_pos, Edge(bone.pos, bone.to()), d, 0))
                if (index < 0 || d < dist) {
                    index = i;
                    dist = d;
                }
...
AnimKeys *AnimEditor::findKeys(Animation *anim, int sbon_index, bool root) {
    if (anim) {
        if (skel)
            if (C SkelBone *sbon = skel->bones.addr(sbon_index))
                return anim->findBone(sbon->name, sbon->type, sbon->type_index, sbon->type_sub); // use types in case animation was from another skeleton and we haven't adjusted types
        if (root)
            return &anim->keys;
    }
    return null;
...
AnimKeys *AnimEditor::getKeys(int sbon_index, bool root) {
    if (anim) {
        if (skel)
            if (C SkelBone *sbon = skel->bones.addr(sbon_index))
                return &anim->getBone(sbon->name, sbon->type, sbon->type_index, sbon->type_sub); // use types in case animation was from another skeleton and we haven't adjusted types
        if (root)
            return &anim->keys;
    }
    return null;
...
AnimKeys::Orn *AnimEditor::FindOrn(AnimKeys &keys, flt time) {
    AnimKeys::Orn *key = null;
    flt dist;
    REPA(keys.orns) {
        flt d = Abs(keys.orns[i].time - time);
        if (!key || d < dist) {
            key = &keys.orns[i];
            dist = d;
        }
    }
    return (!key || dist > TimeEps()) ? null : key;
}
AnimKeys::Pos *AnimEditor::FindPos(AnimKeys &keys, flt time) {
    AnimKeys::Pos *key = null;
    flt dist;
    REPA(keys.poss) {
        flt d = Abs(keys.poss[i].time - time);
        if (!key || d < dist) {
            key = &keys.poss[i];
            dist = d;
        }
...
AnimKeys::Scale *AnimEditor::FindScale(AnimKeys &keys, flt time) {
    AnimKeys::Scale *key = null;
    flt dist;
    REPA(keys.scales) {
        flt d = Abs(keys.scales[i].time - time);
        if (!key || d < dist) {
            key = &keys.scales[i];
            dist = d;
        }
...
AnimKeys::Orn &AnimEditor::GetOrn(AnimKeys &keys, flt time, C Orient &Default) {
    if (AnimKeys::Orn *key = FindOrn(keys, time))
        return *key;
    int i;
    if (keys.orns.binarySearch(time, i, CompareKey))
        return keys.orns[i];
    AnimKeys::Orn &key = keys.orns.NewAt(i);
    key.time = time;
    key.orn = Default;
    return key;
}
AnimKeys::Pos &AnimEditor::GetPos(AnimKeys &keys, flt time, C Vec &Default) {
    if (AnimKeys::Pos *key = FindPos(keys, time))
        return *key;
    int i;
    if (keys.poss.binarySearch(time, i, CompareKey))
        return keys.poss[i];
    AnimKeys::Pos &key = keys.poss.NewAt(i);
    key.time = time;
    key.pos = Default;
    return key;
}
AnimKeys::Scale &AnimEditor::GetScale(AnimKeys &keys, flt time, C Vec &Default) {
    if (AnimKeys::Scale *key = FindScale(keys, time))
        return *key;
    int i;
    if (keys.scales.binarySearch(time, i, CompareKey))
        return keys.scales[i];
    AnimKeys::Scale &key = keys.scales.NewAt(i);
    key.time = time;
    key.scale = Default;
    return key;
}
    ...
    if (gpc.visible && visible()) {
        ...
        if (draw_bones()) {
            ...
            if ((op() == OP_POS || op() == OP_SCALE)
                ...
                MatrixAxis(v4, transformedBoneAxis(sel_bone), bone_axis);
            else
                bone_axis = -1;
                ...
                if (Ms.bp(0))
                    ...
                    if (AnimKeys *keys = getKeys(sel_bone)) {
                        ...
                        op_orn:
                            ...
                            if (rotate) {
                                ...
                                if (all) {
                                    if (use_blend)
                                        REPA(keys->orns)
                                    keys->orns[i].orn.rotateDir(getBlend(keys->orns[i]) * d);
                                ...
                                Vec d = ActiveCam.matrix.x * Ms.d().x * mul.x + ActiveCam.matrix.y * Ms.d().y * mul.y;
                                orn_target += d;

                                bool freeze = Kb.b(KB_H); // hold
                                bool freeze_pos = Kb.b(KB_F);
                                if (freeze || freeze_pos || all) {
                                    Vec p = orn_target - bone.pos;
                                    p /= Matrix3(bone_parent);
                                    p.normalize();
                                    Matrix3 transform;
                                    if (freeze || freeze_pos) {
                                        if (skel) {
                                            Orient next = bone_orn;
                                            next.rotateToDir(p);
                                            next.fixPerp();
                                        ...
                                        if (use_blend)
                                            REPA(keys->orns) {
                                                Orient next = bone_orn;
                                                next.rotateToDir(p, getBlend(keys->orns[i]));
                                                next.fixPerp();
                                                GetTransform(transform, bone_orn, next);
                                ...
                                } else // single
                                {
                                    ...
                                    orn->orn = GetAnimOrient(bone, &bone_parent);
                                    orn->orn.fix();
                                }
                            }
                            keys->setTangents(anim->loop(), anim->length());
                            ...
                            if (!rotate) {
                                ...
                                if (perp.normalize() <= EPS)
                                    ...
                                    case BONE_FINGER: {
                                        ...
                                        if (hand >= 0)
                                            perp = transformedBone(hand).perp;
                                        else
                                            perp.set(0, 1, 0);
                                    } break;
                                    ...
                                    case BONE_SPINE:
                                    case BONE_NECK:
                                        perp.set(0, 0, bone.pos.z - orn_target.z);
                                        break;

                                    case BONE_TOE:
                                        perp.set(0, bone.pos.y - orn_target.y, 0);
                                        break;
                                ...
                                Orient pose = GetAnimOrient(bone_parent, &NoTemp(transformedBone(sbon_parent->parent)));
                                pose.fix();
                                AnimKeys::Orn *orn = &GetOrn(*keys_parent, animTime(), pose);
                                orn->orn = pose;

                                pose = GetAnimOrient(bone, &bone_parent);
                                pose.fix();
                        ...
                        case OP_POS: {
                            ...
                            switch (bone_axis) {
                            case 0:
                                d = AlignDirToCam(m.x, Ms.d() * mul);
                                break;
                            case 1:
                                d = AlignDirToCam(m.y, Ms.d() * mul);
                                break;
                            case 2:
                                d = AlignDirToCam(m.z, Ms.d() * mul);
                                break;
                            default:
                                d = ActiveCam.matrix.x * Ms.d().x * mul.x + ActiveCam.matrix.y * Ms.d().y * mul.y;
                                break;
                            }
                            orn_target += d;
                            ...
                            if (Kb.b(KB_F)) {
                                if (skel)
                                    anim->freezeMove(*skel, sel_bone, all ? -1 : keys->poss.index(pos), d);
                            } else if (all) {
                                if (use_blend)
                                    REPA(keys->poss)
                                keys->poss[i].pos += d * getBlend(keys->poss[i]);
                        ...
                        case OP_SCALE: {
                            ...
                            switch (bone_axis) {
                            case 0:
                                d.x += AlignDirToCamEx(bone.cross(), Ms.d() * mul);
                                break;
                            case 1:
                                d.y += AlignDirToCamEx(bone.perp, Ms.d() * mul);
                                break;
                            case 2:
                                d.z += AlignDirToCamEx(bone.dir, Ms.d() * mul);
                                break;
                            default:
                                d += (Ms.d() * mul).sum();
                                break;
                            ...
                            if (Kb.ctrlCmd()) // all
                            {
                                if (use_blend)
                                    REPA(keys->scales)
                                ScaleScaleFactor(keys->scales[i].scale, ScaleFactor(d * getBlend(keys->scales[i])));
                                else REPA(keys->scales)
                                    ScaleScaleFactor(keys->scales[i].scale, mul);
                            } else // single
                            {
        ...
        {
            if (settings_region.contains(Gui.ms()))
                settings.set(-1);
        }
    }
    super::update(gpc); // process after adjusting 'animTime' so clicking on anim track will not be changed afterwards
    ...
    if (mesh)
        if (Edit::Viewport4::View *view = v4.getView(go)) {
            ...
            if (!(lod.parts[i].part_flag & MSHP_HIDDEN)) {
                ...
                if (Sweep(pos, dir, part, null, &f, &hp))
...
void AnimEditor::camCenter(bool zoom) {
    Vec hit_pos;
    bool hit = getHit(Gui.ms(), Ms.pos(), hit_pos);
    flt dist;
    v4.moveTo(hit ? hit_pos : (skel && InRange(lit_bone, skel->bones)) ? transformedBone(lit_bone).center()
...
void AnimEditor::frame(int d) {
    if (d)
        if (AnimKeys *keys = findKeys(sel_bone))
            switch (Kb.shift() ? -1 : op()) {
            case OP_ORN:
            case OP_ORN2:
                if (keys->orns.elms()) {
                    int i = 0;
                    for (; i < keys->orns.elms(); i++) {
                        flt t = keys->orns[i].time;
                        if (Equal(animTime(), t))
                            break;
                        if (t >= animTime()) {
                            if (d > 0)
                    ...
                    i += d;
                    i = Mod(i, keys->orns.elms());
                    animTime(keys->orns[i].time);
                }
                break;
            ...
            case OP_POS:
                if (keys->poss.elms()) {
                    int i = 0;
                    for (; i < keys->poss.elms(); i++) {
                        flt t = keys->poss[i].time;
                        if (Equal(animTime(), t))
                            break;
                        if (t >= animTime()) {
                            if (d > 0)
                    ...
                    i += d;
                    i = Mod(i, keys->poss.elms());
                    animTime(keys->poss[i].time);
                }
                break;
            ...
            case OP_SCALE:
                if (keys->scales.elms()) {
                    int i = 0;
                    for (; i < keys->scales.elms(); i++) {
                        flt t = keys->scales[i].time;
                        if (Equal(animTime(), t))
                            break;
                        if (t >= animTime()) {
                            if (d > 0)
                    ...
                    i += d;
                    i = Mod(i, keys->scales.elms());
                    animTime(keys->scales[i].time);
                }
                break;

            default: {
                Memt<flt, 16384> times;
                keys->includeTimes(times, times, times);
                if (times.elms()) {
                    int i = 0;
                    for (; i < times.elms(); i++) {
                        flt t = times[i];
...
bool AnimEditor::delFrameOrn(int bone) {
    if (AnimKeys *keys = findKeys(bone))
        if (AnimKeys::Orn *key = FindOrn(*keys, animTime())) {
            undos.set("del");
            keys->orns.removeData(key, true);
            if (keys->is())
                keys->setTangents(anim->loop(), anim->length());
            else
                anim->bones.removeData(static_cast<AnimBone *>(keys), true);
            return true;
        }
    return false;
}
bool AnimEditor::delFramePos(int bone) {
    if (AnimKeys *keys = findKeys(bone))
        if (AnimKeys::Pos *key = FindPos(*keys, animTime())) {
            undos.set("del");
            keys->poss.removeData(key, true);
            if (keys->is())
                keys->setTangents(anim->loop(), anim->length());
            else
                anim->bones.removeData(static_cast<AnimBone *>(keys), true);
...
bool AnimEditor::delFrameScale(int bone) {
    if (AnimKeys *keys = findKeys(bone))
        if (AnimKeys::Scale *key = FindScale(*keys, animTime())) {
            undos.set("del");
            keys->scales.removeData(key, true);
            if (keys->is())
                keys->setTangents(anim->loop(), anim->length());
            else
                anim->bones.removeData(static_cast<AnimBone *>(keys), true);
...
bool AnimEditor::freezeDelFramePos(int bone) {
    if (skel)
        if (AnimKeys *keys = findKeys(bone))
            if (AnimKeys::Pos *key = FindPos(*keys, animTime())) {
                undos.set("del");
                anim->freezeDelPos(*skel, bone, keys->poss.index(key));
                return true;
            }
    return false;
}
...
bool AnimEditor::freezeDelFrameOrn(int bone, bool pos) {
    if (skel)
        if (AnimKeys *keys = findKeys(bone))
            if (AnimKeys::Orn *key = FindOrn(*keys, animTime())) {
                undos.set("del");
                anim->freezeDelRot(*skel, bone, keys->orns.index(key), pos);
                return true;
            }
    return false;
}
bool AnimEditor::freezeDelFramesOrn(int bone, bool pos) {
    if (skel)
        if (AnimKeys *keys = findKeys(bone))
            if (keys->orns.elms()) {
                undos.set("delAll");
                anim->freezeDelRot(*skel, bone, -1, pos);
                return true;
            }
    return false;
}
void AnimEditor::delFrame() {
    bool changed = false;
    if (op() == OP_ORN || op() < 0)
        changed |= delFrameOrn(sel_bone);
    if (op() == OP_ORN2)
        changed |= delFrameOrn(sel_bone) | delFrameOrn(boneParent(sel_bone));
    if (op() == OP_POS || op() < 0)
        changed |= delFramePos(sel_bone);
    if (op() == OP_SCALE || op() < 0)
        changed |= delFrameScale(sel_bone);
    if (changed) {
        setAnimSkel();
        setOrnTarget();
        anim->setRootMatrix();
...
void AnimEditor::delFrames(int bone) {
    bool changed = false;
    if (op() == OP_ORN || op() < 0)
        changed |= delFramesOrn(bone);
    if (op() == OP_ORN2)
        changed |= delFramesOrn(bone) | delFramesOrn(boneParent(bone));
    if (op() == OP_POS || op() < 0)
        changed |= delFramesPos(bone);
    if (op() == OP_SCALE || op() < 0)
        changed |= delFramesScale(bone);
    if (changed) {
        setAnimSkel();
        setOrnTarget();
        anim->setRootMatrix();
...
void AnimEditor::freezeDelFrame(bool pos) {
    bool changed = false;
    if (op() == OP_ORN || op() < 0)
        changed |= freezeDelFrameOrn(sel_bone, pos);
    if (op() == OP_ORN2)
...
void AnimEditor::freezeDelFrames(bool pos) {
    bool changed = false;
    if (op() == OP_ORN || op() < 0)
        changed |= freezeDelFramesOrn(sel_bone, pos);
    if (op() == OP_ORN2)
...
void AnimEditor::freezeBone() {
    if (skel && anim) {
        if (!InRange(sel_bone, skel->bones)) {
...
void AnimEditor::playUpdate(flt multiplier) {
    bool play = (Kb.b(KB_W) && Kb.ctrlCmd() && !Kb.alt() && (fullscreen ? contains(Gui.kb()) : preview.contains(Gui.kb())));
    if (play)
        T.force_play.push();
    play |= T.force_play();
    if (play && Kb.shift())
    ...
    if (anim) {
        if (flt length = anim->length()) {
            if (play && !Ms.b(1))
                _anim_time += Time.ad() * anim_speed * multiplier;
            if (_anim_time >= length && _anim_time < length + EPS)
                _anim_time = length;
            else
                _anim_time = Frac(_anim_time, length); // allow being at the end
        } else
            _anim_time = 0;
    }
    if (play) {
...
void AnimEditor::setAnimSkel(bool force) {
    ...
    if (anim && skel) {
        if (anim_skel.skeleton() != skel || anim_skel.bones.elms() != skel->bones.elms() || anim_skel.slots.elms() != skel->slots.elms() || force)
            anim_skel.create(skel);
        ...
        Matrix matrix_prev;
        anim->getRootMatrixCumulative(matrix_prev, time_prev);
        anim_skel.updateBegin().clear().animateEx(skel_anim, time_prev, true, false, true).updateMatrix(matrix_prev).updateEnd(); // use 'animateEx' to allow editing/previewing exact last keyframes without wrapping to beginning with Frac
        Matrix matrix;
        anim->getRootMatrixCumulative(matrix, time);
        anim_skel.updateBegin().clear().animateEx(skel_anim, time, true, false, true).updateMatrix(matrix).updateEnd();
        anim_skel.root.orn = anim_skel.matrix();
        anim_skel.root.orn.fix();
        anim_skel.root.pos = anim_skel.pos();
        anim_skel.root.scale = ScaleFactorR(anim_skel.matrix().scale()); // because we don't animate 'root' but the main matrix, we need to setup root based on obtained matrix, this is needed in case we want to access root data (for example adding new keyframes for root positions needs current position)
    } else {
        anim_skel.del().updateBegin().updateMatrix().updateEnd();
    }
}
void AnimEditor::setMeshSkel(bool force) {
    if (ElmAnim *anim_data = data())
        if (Elm *skel_elm = Proj.findElm(anim_data->skel_id))
            if (ElmSkel *skel_data = skel_elm->skelData())
                if (skel_data->mesh_id.valid()) // get mesh from anim->skel->mesh
                {
                    if (mesh_id != skel_data->mesh_id || skel_id != skel_elm->id || force) {
            ...
            skel=mesh->skeleton();
                        ...
                        skel = &T.skel_data;
                        T.skel_data.load(Proj.gamePath(skel_id));
                        mesh = &mesh_data;
                        Load(mesh_data, Proj.editPath(mesh_id), Proj.game_path);
                        RemovePartsAndLods(mesh_data);
                        mesh_data.transform(skel_data->transform());
...
void AnimEditor::applySpeed() {
    if (anim && !Equal(anim_speed, 1) && !Equal(anim_speed, 0))
        if (ElmAnim *data = T.data()) {
            undos.set("speed");
            ...
            if (FileParams *fps = file_params.data()) {
                ...
                flt set_speed = anim_speed;
                if (flt cur_speed = speed.asFlt())
                    set_speed *= cur_speed;
                if (Equal(set_speed, 1))
                    fps->params.removeData(&speed, true);
                else
...
void AnimEditor::setEvent(AnimEvent &e, int i) C {
    if (anim && InRange(i, anim->events))
        e = anim->events[i];
}
int AnimEditor::findEvent(C AnimEvent &e) C {
    if (anim)
        REPA(anim->events)
    if (e == anim->events[i])
        return i;
...
void AnimEditor::moveEvent(int event, flt time) {
    if (anim && InRange(event, anim->events)) {
        ...
        AnimEvent s, ps, l, pl;
        setEvent(s, track.event_sel);
        setEvent(ps, preview.track.event_sel);
        setEvent(l, track.event_lit);
        setEvent(pl, preview.track.event_lit);
        anim->sortEvents();
        track.event_sel = findEvent(s);
        preview.track.event_sel = findEvent(ps);
        track.event_lit = findEvent(l);
        preview.track.event_lit = findEvent(pl);
        setChanged();
    }
}
void AnimEditor::newEvent(flt time) {
    if (anim) {
        ...
        if (!(edit_name && !RenameEvent.visibleOnActiveDesktop())) // always add if we're going to show rename window as long as it's hidden
            REPA(anim->events) {
                C AnimEvent &event = anim->events[i];
                if (Equal(event.name, name) && Abs(event.time - time) <= TimeEps())
                    return;
            }
        ...
        setChanged();
        if (edit_name)
            RenameEvent.activate(findEvent(event)); // activate window for renaming created event
    }
}
...
void AnimEditor::renameEvent(int index, C Str &old_name, C Str &new_name) {
    if (anim)
        FREPA(anim->events)
    if (old_name == anim->events[i].name)
        if (!index--) {
...
void AnimEditor::flush() {
    if (elm && changed) {
        ...
        if (anim) {
            Save(*anim, Proj.gamePath(elm->id));
            Proj.savedGame(*elm);
        }
...
void AnimEditor::toggle(Elm *elm) {
    if (elm == T.elm && (fullscreen ? selected() : true))
        elm = null;
    if (fullscreen || preview.maximized())
        activate(elm);
    else
        set(elm); // if editor is in fullscreen mode, or the window is maximized then activate
}
...
void AnimEditor::erasing(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        set(null);
}
void AnimEditor::setTarget(C Str &obj_id) {
    if (anim)
        if (ElmAnim *anim_data = data()) {
            if (obj_id.is()) {
                if (Elm *obj = Proj.findElm(obj_id))
                    if (ElmObj *obj_data = obj->objData()) {
                        bool has_skel = false;
                        if (Elm *mesh = Proj.findElm(obj_data->mesh_id))
                            if (ElmMesh *mesh_data = mesh->meshData())
                                if (Elm *skel = Proj.findElm(mesh_data->skel_id))
                                    if (ElmSkel *skel_data = skel->skelData()) {
                                        has_skel = true;
                      ...
                      C Skeleton *old_skel=(old_skel_elm ? Skeletons(Proj.gamePath(*old_skel_elm)) : null),
                        ...
                        anim.transform(GetTransform(anim_data.transform(), skel_data.transform()), old_skel ? *old_skel : *new_skel); // transform from current to target, if 'old_skel' is not available then try using 'new_skel'
                                            ...
                                            if (RenameAnimBonesOnSkelChange)
                                                if (Skeleton *new_skel = Skeletons(Proj.gamePath(skel->id))) {
                                                    if (ObjEdit.skel_elm && ObjEdit.skel_elm->id == skel->id)
                                                        ObjEdit.flushMeshSkel(); // !! if this skeleton is currently edited, then we have to flush it, for example, it could have been empty at the start and we've just added bones, so 'new_skel' is empty and bones are only in 'ObjEdit.mesh_skel_temp' memory, and before changing 'anim_data.skel_id' !!
