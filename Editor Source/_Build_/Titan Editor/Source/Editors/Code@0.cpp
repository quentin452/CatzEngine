...
UID CodeView::appID() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        return app->id;
    return super::appID();
}
Str CodeView::appName() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        return app->name;
    return super::appName();
}
Str CodeView::appDirsWindows() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->dirs_windows;
    return super::appDirsWindows();
}
Str CodeView::appDirsMac() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->dirs_mac;
    return super::appDirsMac();
}
Str CodeView::appDirsLinux() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->dirs_linux;
    return super::appDirsLinux();
}
Str CodeView::appDirsAndroid() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->dirs_android;
    return super::appDirsAndroid();
}
Str CodeView::appDirsiOS() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->dirs_ios;
    return super::appDirsiOS();
}
Str CodeView::appDirsNintendo() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->dirs_nintendo;
    return super::appDirsNintendo();
}
Str CodeView::appHeadersWindows() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->headers_windows;
    return super::appHeadersWindows();
}
Str CodeView::appHeadersMac() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->headers_mac;
    return super::appHeadersMac();
}
Str CodeView::appHeadersLinux() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->headers_linux;
    return super::appHeadersLinux();
}
Str CodeView::appHeadersAndroid() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->headers_android;
    return super::appHeadersAndroid();
}
Str CodeView::appHeadersiOS() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->headers_ios;
    return super::appHeadersiOS();
}
Str CodeView::appHeadersNintendo() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->headers_nintendo;
    return super::appHeadersNintendo();
}
Str CodeView::appLibsWindows() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->libs_windows;
    return super::appLibsWindows();
}
Str CodeView::appLibsMac() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->libs_mac;
    return super::appLibsMac();
}
Str CodeView::appLibsLinux() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->libs_linux;
    return super::appLibsLinux();
}
Str CodeView::appLibsAndroid() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->libs_android;
    return super::appLibsAndroid();
}
Str CodeView::appLibsiOS() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->libs_ios;
    return super::appLibsiOS();
}
Str CodeView::appLibsNintendo() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->libs_nintendo;
    return super::appLibsNintendo();
}
Str CodeView::appPackage() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->package;
    return super::appPackage();
}
UID CodeView::appMicrosoftPublisherID() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->ms_publisher_id;
    return super::appMicrosoftPublisherID();
}
Str CodeView::appMicrosoftPublisherName() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->ms_publisher_name;
    return super::appMicrosoftPublisherName();
}
Edit::XBOX_LIVE CodeView::appXboxLiveProgram() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->xbl_program;
    return super::appXboxLiveProgram();
}
ULong CodeView::appXboxLiveTitleID() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->xbl_title_id;
    return super::appXboxLiveTitleID();
}
UID CodeView::appXboxLiveSCID() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->xbl_scid;
    return super::appXboxLiveSCID();
}
Str CodeView::appGooglePlayLicenseKey() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->android_license_key;
    return super::appGooglePlayLicenseKey();
}
bool CodeView::appGooglePlayAssetDelivery() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->playAssetDelivery();
    return super::appGooglePlayAssetDelivery();
}
Str CodeView::appLocationUsageReason() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->location_usage_reason;
    return super::appLocationUsageReason();
}
Str CodeView::appNintendoInitialCode() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->nintendo_initial_code;
    return super::appNintendoInitialCode();
}
ULong CodeView::appNintendoAppID() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->nintendo_app_id;
    return super::appNintendoAppID();
}
Str CodeView::appNintendoPublisherName() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->nintendo_publisher_name;
    return super::appNintendoPublisherName();
}
Str CodeView::appNintendoLegalInformation() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->nintendo_legal_info;
    return super::appNintendoLegalInformation();
}
Int CodeView::appBuild() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->build;
    return super::appBuild();
}
Long CodeView::appSaveSize() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->save_size;
    return super::appSaveSize();
}
ulong CodeView::appFacebookAppID() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->fb_app_id;
    return super::appFacebookAppID();
}
Str CodeView::appAdMobAppIDiOS() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->am_app_id_ios;
    return super::appAdMobAppIDiOS();
}
Str CodeView::appAdMobAppIDGooglePlay() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->am_app_id_google;
    return super::appAdMobAppIDGooglePlay();
}
Str CodeView::appChartboostAppIDiOS() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->cb_app_id_ios;
    return super::appChartboostAppIDiOS();
}
Str CodeView::appChartboostAppSignatureiOS() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->cb_app_signature_ios;
    return super::appChartboostAppSignatureiOS();
}
Str CodeView::appChartboostAppIDGooglePlay() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->cb_app_id_google;
    return super::appChartboostAppIDGooglePlay();
}
Str CodeView::appChartboostAppSignatureGooglePlay() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->cb_app_signature_google;
    return super::appChartboostAppSignatureGooglePlay();
}
Edit::STORAGE_MODE CodeView::appPreferredStorage() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->storage;
    return super::appPreferredStorage();
}
UInt CodeView::appSupportedOrientations() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->supported_orientations;
    return super::appSupportedOrientations();
}
UID CodeView::appGuiSkin() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->gui_skin;
    return super::appGuiSkin();
}
int CodeView::appEmbedEngineData() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->embedEngineData();
    return super::appEmbedEngineData();
}
Cipher *CodeView::appEmbedCipher() {
    static ProjectCipher cipher; /*if(Elm *app=Proj.findElm(Proj.curApp()))if(ElmApp *app_data=app.appData())*/
    return cipher.set(Proj)();
    return super::appEmbedCipher();
}
Bool CodeView::appPublishProjData() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->publishProjData();
    return super::appPublishProjData();
}
Bool CodeView::appPublishPhysxDll() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->publishPhysxDll();
    return super::appPublishPhysxDll();
}
Bool CodeView::appPublishSteamDll() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->publishSteamDll();
    return super::appPublishSteamDll();
}
Bool CodeView::appPublishOpenVRDll() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->publishOpenVRDll();
    return super::appPublishOpenVRDll();
}
Bool CodeView::appPublishDataAsPak() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            return app_data->publishDataAsPak();
    return super::appPublishDataAsPak();
}
ImagePtr CodeView::appIcon() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            if (app_data->icon.valid())
                return ImagePtr().get(Proj.gamePath(app_data->icon));
    return super::appIcon();
}
ImagePtr CodeView::appImagePortrait() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            if (app_data->image_portrait.valid())
                return ImagePtr().get(Proj.gamePath(app_data->image_portrait));
    return super::appImagePortrait();
}
ImagePtr CodeView::appImageLandscape() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            if (app_data->image_landscape.valid())
                return ImagePtr().get(Proj.gamePath(app_data->image_landscape));
    return super::appImageLandscape();
}
ImagePtr CodeView::appNotificationIcon() {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData())
            if (app_data->notification_icon.valid())
                return ImagePtr().get(Proj.gamePath(app_data->notification_icon));
    return super::appNotificationIcon();
}
void CodeView::appLanguages(MemPtr<LANG_TYPE> langs) {
    if (Elm *app = Proj.findElm(Proj.curApp()))
        if (ElmApp *app_data = app->appData()) {
            langs = app_data->supported_languages;
            return;
        }
    return super::appLanguages(langs);
}
int CodeView::editorAddrPort() { return EditServer.port(); }
void CodeView::editorAddr(SockAddr &addr) { addr = EditServer.localAddress(); }
void CodeView::focus() {
    if (Mode.tabAvailable(MODE_CODE))
        Mode.set(MODE_CODE);
}
void CodeView::ImageGenerateProcess(ImageGenerate &generate, ptr user, int thread_index) {
    ThreadMayUseGPUData();
    generate.process();
}
COMPRESS_TYPE CodeView::appEmbedCompress(Edit::EXE_TYPE exe_type) { /*if(Elm *app=Proj.findElm(Proj.curApp()))if(ElmApp *app_data=app.appData())*/
    return Proj.compress_type[ProjCompres(exe_type)];
    return super::appEmbedCompress(exe_type);
}
int CodeView::appEmbedCompressLevel(Edit::EXE_TYPE exe_type) { /*if(Elm *app=Proj.findElm(Proj.curApp()))if(ElmApp *app_data=app.appData())*/
    return Proj.compress_level[ProjCompres(exe_type)];
    return super::appEmbedCompressLevel(exe_type);
}
DateTime CodeView::appEmbedSettingsTime(Edit::EXE_TYPE exe_type) { /*if(Elm *app=Proj.findElm(Proj.curApp()))if(ElmApp *app_data=app.appData())*/
    return Max(Proj.compress_time[ProjCompres(exe_type)], Max(Proj.cipher_time, Proj.cipher_key_time)).asDateTime();
    return super::appEmbedSettingsTime(exe_type);
}
void CodeView::appSpecificFiles(MemPtr<PakFileData> files, Edit::EXE_TYPE exe_type) {
    ...
    Memt<Elm *> app_elms;
    Proj.getActiveAppElms(app_elms, exe_type);
...
Rect CodeView::menuRect() {
    ...
    if (Misc.visible() && Misc.pos.y == (int)CodeMenuOnTop && !(Mode.visibleTabs() && Misc.pos.y)) {
        if (Misc.move_misc.visible()) {
            if (Misc.pos.x == 0)
                MAX(r.min.x, Misc.move_misc.rect().max.x);
            else
                MIN(r.max.x, Misc.move_misc.rect().min.x);
        } else {
            if (Misc.pos.x == 0)
                MAX(r.min.x, Misc.rect().max.x);
            else
                MIN(r.max.x, Misc.rect().min.x);
        }
    } else if (Proj.visible())
        MAX(r.min.x, Proj.rect().max.x);
...
Edit::ERROR_TYPE CodeView::sourceDataLoad(C UID &id, Str &data) {
    if (Elm *elm = Proj.findElm(id)) {
        if (ElmCode *code_data = elm->codeData()) {
            ...
            if (error == Edit::EE_ERR_FILE_NOT_FOUND && !code_data->ver)
                return Edit::EE_ERR_ELM_NOT_DOWNLOADED; // if file not found and ver not set yet, then report as not yet downloaded
            return error;
        }
...
Bool CodeView::sourceDataSave(C UID &id, C Str &data) {
    if (Elm *elm = Proj.findElm(id))
        if (ElmCode *code_data = elm->codeData())
            if (SaveCode(data, Proj.codePath(id))) {
                code_data->newVer();
                code_data->from(data);
                return true;
            }
...
void CodeView::sourceChanged(bool activate) {
    ...
    if (activate && sourceCurIs()) {
        Mode.set(MODE_CODE);
        HideBig();
    } // set mode in case Code Editor triggers source opening
    SetTitle();
    if (activate)
        Proj.refresh();
}
Bool CodeView::elmValid(C UID &id) { return Proj.findElm(id) != null; }
Str CodeView::elmBaseName(C UID &id) {
    if (Elm *elm = Proj.findElm(id))
...
void CodeView::elmPreview(C UID &id, C Vec2 &pos, bool mouse, C Rect &clip) {
    if (Elm *elm = Proj.findElm(id)) {
        Rect_LU r(pos, D.h() * 0.65f);
        if (mouse)
            r -= D.pixelToScreenSize(VecI2(0, 32)); // avg mouse cursor height
        if (r.max.x > clip.max.x) {
            r.min.x -= r.max.x - clip.max.x;
            r.max.x = clip.max.x;
        }
        if (r.min.x < clip.min.x) {
            r.max.x += clip.min.x - r.min.x;
            r.min.x = clip.min.x;
        }
        if (r.min.y < clip.min.y) {
            r += Vec2(0, pos.y - r.min.y + 0.01f);
        }
        if (r.max.y > clip.max.y) {
            r.min.y -= r.max.y - clip.max.y;
            r.max.y = clip.max.y;
        }
        Preview.draw(*elm, r);
    }
...
void CodeView::getProjPublishElms(Memc<ElmLink> &elms) {
    elms.clear();
    FREPA(Proj.elms) // process in order
    {
        Elm &elm = Proj.elms[i];
        if (elm.finalPublish() && ElmPublish(elm.type) && ElmVisible(elm.type))
            elms.New().set(elm.id, Proj.elmFullName(&elm), Proj.elmIcon(elm.type));
    }
}
    ...
    if (selected()) {
        ...
        if (Elm *elm = Proj.findElm(elms[i]))
            if (ElmPublish(elm->type))
                temp.add(elm->id); // filter out non-publishable elements
...
bool CodeView::sourceDataSet(C UID &id, C Str &data) {
    if (super::sourceDataSet(id, data)) {
        sourceTitleChanged(id);
        return true;
    }
    return false;
...
void CodeView::makeAuto(bool publish) {
    if (Proj.valid()) {
        Memt<Enum::Elm> obj_types;
        REPA(Proj.existing_obj_classes)
        if (Elm *elm = Proj.findElm(Proj.existing_obj_classes[i]))
            obj_types.New().set(NameToEnum(elm->name), elm->id);
        obj_types.sort(Compare);
        int max_length = 0;
        REPA(obj_types)
        MAX(max_length, Length(obj_types[i].name));
        ...
        if (appGuiSkin().valid())
            data += "   Gui.default_skin=APP_GUI_SKIN; // set default Gui Skin\n"; // override only if specified
        ...
        if (obj_types.elms()) {
            ...
            FREPA(obj_types) {
                ...
                data += S + "      {\"" + CString(obj_types[i].name) + '"';
                REP(max_length - length)
                data += ' ';
                data += ", ";
                data += obj_types[i].id.asCString();
                data += "},\n";
            }
            data += "   };\n";
        ...
        if (Enum *e = Enums.get(Proj.gamePath(Proj.existing_enums[i])))
            if (Elm *elm = Proj.findElm(Proj.existing_enums[i]))
                if (ElmEnum *enum_data = elm->enumData()) {
                    ...
                    case EditEnums::TYPE_1:
                        data += " : byte";
                        break;
                    case EditEnums::TYPE_2:
                        data += " : ushort";
                        break;
                    case EditEnums::TYPE_4:
                        data += " : uint";
                        break;
                    }
                    data += "\n{\n";
                    int max_length = 0;
                    REPA(*e)
                    MAX(max_length, Length((*e)[i].name));
                    FREPA(*e) {
...
void CodeView::codeDo(Edit::BUILD_MODE mode) {
    switch (mode) {
    case Edit::BUILD_BUILD:
        super::build();
        break;
    case Edit::BUILD_PUBLISH:
        makeAuto(true);
        super::publish();
        clearAuto();
        makeAuto(false);
        break; // 'clearAuto' so we already set correct codes with this 'makeAuto'
    case Edit::BUILD_PLAY:
        super::play();
        break;
    case Edit::BUILD_DEBUG:
        super::debug();
        break;
    }
}
void CodeView::buildDo(Edit::BUILD_MODE mode) {
    if (PublishDataNeeded(configEXE()))
        StartPublish(S, configEXE(), mode);
    else {
...
void CodeView::openIDE() {
    if (PublishDataNeeded(configEXE()))
        StartPublish(S, configEXE(), Edit::BUILD_IDE, true, S, true);
    else // we need to create project data pak first
    {
        Proj.flush(); // flush in case we want to play with latest data
        super::openIDE();
    }
...
bool CodeView::Export(Edit::EXPORT_MODE mode, bool data) {
    ...
    if (ok = super::Export(mode))
        if (data) {
            Edit::EXE_TYPE exe = Edit::EXE_NUM;
            switch (mode) {
            case Edit::EXPORT_EXE:
                exe = configEXE();
                break;
            case Edit::EXPORT_ANDROID:
                exe = Edit::EXE_APK;
                break;
            case Edit::EXPORT_XCODE:
                exe = Edit::EXE_IOS;
                break;
            ...
            if (exe != Edit::EXE_NUM && PublishDataNeeded(exe))
                StartPublish(S, exe, Edit::BUILD_PUBLISH, true);
        }
...
void CodeView::createItems(Memx<EEItem> &dest, C Memx<Edit::Item> &src, EEItem *parent) {
    FREPA(src) {
        ...
        d.parent = parent;
        if (d.full_name.is())
            items_sorted.binaryInclude(&d, CompareItem);
...
void CodeView::create(GuiObj &parent) {
    ...
    if (C TextNode *code = Settings.findNode("Code"))
        loadSettings(*code);
}
void CodeView::resize(bool full) {
    rect(EditRect());
    if (full)
        CodeEditorInterface::resize();
}
...
void CodeView::toggle(EEItem *item) {
    if (item && item->type == ELM_CODE && item->full_name.is()) {
        if (!EqualPath(sourceCurName(), item->full_name))
            sourceCur(item->full_name);
        else if (selected())
            close();
        else
            Mode.set(MODE_CODE);
...
::AppPropsEditor::ORIENT AppPropsEditor::FlagToOrient(uint flag) {
    ...
    if (flag == (DIRF_X | DIRF_UP))
        return ORIENT_ALL_NO_DOWN;
    if ((flag & DIRF_X) == DIRF_RIGHT)
        return ORIENT_LANDSCAPE_LOCKED;
    if ((flag & DIRF_Y) == DIRF_UP)
        return ORIENT_PORTRAIT_LOCKED;
...
uint AppPropsEditor::OrientToFlag(ORIENT orient) {
    switch (orient) {
    default:
        return DIRF_X | DIRF_Y; // ORIENT_ALL
    case ORIENT_ALL_NO_DOWN:
        return DIRF_X | DIRF_UP;
    case ORIENT_PORTRAIT:
        return DIRF_Y;
    case ORIENT_LANDSCAPE:
        return DIRF_X;
    case ORIENT_PORTRAIT_LOCKED:
        return DIRF_UP;
    case ORIENT_LANDSCAPE_LOCKED:
        return DIRF_RIGHT;
    }
}
...
void AppPropsEditor::AppImage::setImage() {
    ...
    if (image && image->is() && image->mode() != IMAGE_2D) {
        ...
        if (size.x > 256)
            size = size * 256 / size.x;
        if (size.y > 256)
            size = size * 256 / size.y;
...
void AppPropsEditor::AppImage::setImage(C UID &image_id) {
    if (ElmApp *app = (AppPropsEdit.elm ? AppPropsEdit.elm->appData() : null)) {
        md.fromUID(app, image_id);
        md_time.as<TimeStamp>(app).getUTC();
        AppPropsEdit.setChanged();
        setImage();
    }
}
::AppPropsEditor::AppImage &AppPropsEditor::AppImage::create(C MemberDesc &md, C MemberDesc &md_time, GuiObj &parent, C Rect &rect) {
    T.md = md;
    T.md_time = md_time;
    parent += super::create(rect);
    fit = FIT_FULL;
    parent += remove.create(Rect_RU(rect.ru(), 0.045f, 0.045f)).func(Remove, T);
...
void AppPropsEditor::DirsWin(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->dirs_windows = text;
            app_data->dirs_windows_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::DirsWin(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->dirs_windows;
    return S;
}
void AppPropsEditor::DirsMac(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->dirs_mac = text;
            app_data->dirs_mac_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::DirsMac(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->dirs_mac;
    return S;
}
void AppPropsEditor::DirsLinux(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->dirs_linux = text;
            app_data->dirs_linux_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::DirsLinux(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->dirs_linux;
    return S;
}
void AppPropsEditor::DirsAndroid(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->dirs_android = text;
            app_data->dirs_android_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::DirsAndroid(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->dirs_android;
    return S;
}
void AppPropsEditor::DirsiOS(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->dirs_ios = text;
            app_data->dirs_ios_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::DirsiOS(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->dirs_ios;
    return S;
}
void AppPropsEditor::DirsNintendo(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->dirs_nintendo = text;
            app_data->dirs_nintendo_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::DirsNintendo(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->dirs_nintendo;
    return S;
}
void AppPropsEditor::HeadersWin(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->headers_windows = text;
            app_data->headers_windows_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::HeadersWin(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->headers_windows;
    return S;
}
void AppPropsEditor::HeadersMac(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->headers_mac = text;
            app_data->headers_mac_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::HeadersMac(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->headers_mac;
    return S;
}
void AppPropsEditor::HeadersLinux(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->headers_linux = text;
            app_data->headers_linux_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::HeadersLinux(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->headers_linux;
    return S;
}
void AppPropsEditor::HeadersAndroid(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->headers_android = text;
            app_data->headers_android_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::HeadersAndroid(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->headers_android;
    return S;
}
void AppPropsEditor::HeadersiOS(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->headers_ios = text;
            app_data->headers_ios_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::HeadersiOS(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->headers_ios;
    return S;
}
void AppPropsEditor::HeadersNintendo(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->headers_nintendo = text;
            app_data->headers_nintendo_time.getUTC();
            ap.changed_headers = true;
        }
}
Str AppPropsEditor::HeadersNintendo(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->headers_nintendo;
    return S;
}
void AppPropsEditor::LibsWin(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->libs_windows = text;
            app_data->libs_windows_time.getUTC();
        }
}
Str AppPropsEditor::LibsWin(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->libs_windows;
    return S;
}
void AppPropsEditor::LibsMac(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->libs_mac = text;
            app_data->libs_mac_time.getUTC();
        }
}
Str AppPropsEditor::LibsMac(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->libs_mac;
    return S;
}
void AppPropsEditor::LibsLinux(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->libs_linux = text;
            app_data->libs_linux_time.getUTC();
        }
}
Str AppPropsEditor::LibsLinux(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->libs_linux;
    return S;
}
void AppPropsEditor::LibsAndroid(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->libs_android = text;
            app_data->libs_android_time.getUTC();
        }
}
Str AppPropsEditor::LibsAndroid(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->libs_android;
    return S;
}
void AppPropsEditor::LibsiOS(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->libs_ios = text;
            app_data->libs_ios_time.getUTC();
        }
}
Str AppPropsEditor::LibsiOS(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->libs_ios;
    return S;
}
void AppPropsEditor::LibsNintendo(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->libs_nintendo = text;
            app_data->libs_nintendo_time.getUTC();
        }
}
Str AppPropsEditor::LibsNintendo(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->libs_nintendo;
    return S;
}
void AppPropsEditor::Package(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->package = text;
            app_data->package_time.getUTC();
        }
}
Str AppPropsEditor::Package(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->package;
    return S;
}
void AppPropsEditor::MicrosoftPublisherID(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->ms_publisher_id.fromCanonical(SkipStart(text, "CN="));
            app_data->ms_publisher_id_time.getUTC();
        }
}
Str AppPropsEditor::MicrosoftPublisherID(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            if (app_data->ms_publisher_id.valid())
                return S + "CN=" + CaseUp(app_data->ms_publisher_id.asCanonical());
    return S;
}
void AppPropsEditor::MicrosoftPublisherName(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->ms_publisher_name = text;
            app_data->ms_publisher_name_time.getUTC();
        }
}
Str AppPropsEditor::MicrosoftPublisherName(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->ms_publisher_name;
    return S;
}
void AppPropsEditor::XboxLiveProgram(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->xbl_program = (Edit::XBOX_LIVE)TextInt(text);
            app_data->xbl_program_time.getUTC();
        }
}
Str AppPropsEditor::XboxLiveProgram(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->xbl_program;
    return S;
}
void AppPropsEditor::XboxLiveTitleID(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->xbl_title_id = TextULong(text);
            app_data->xbl_title_id_time.getUTC();
        }
}
Str AppPropsEditor::XboxLiveTitleID(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            if (app_data->xbl_title_id)
                return app_data->xbl_title_id;
    return S;
}
void AppPropsEditor::XboxLiveSCID(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->xbl_scid.fromCanonical(text);
            app_data->xbl_scid_time.getUTC();
        }
}
Str AppPropsEditor::XboxLiveSCID(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            if (app_data->xbl_scid.valid())
                return CaseDown(app_data->xbl_scid.asCanonical());
    return S;
}
void AppPropsEditor::NintendoInitialCode(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->nintendo_initial_code = text;
            app_data->nintendo_initial_code_time.getUTC();
        }
}
Str AppPropsEditor::NintendoInitialCode(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->nintendo_initial_code;
    return S;
}
void AppPropsEditor::NintendoAppID(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->nintendo_app_id = TextULong(text);
            app_data->nintendo_app_id_time.getUTC();
        }
}
Str AppPropsEditor::NintendoAppID(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            if (app_data->nintendo_app_id)
                return TextHex(app_data->nintendo_app_id, 16, 0, true);
    return S;
}
void AppPropsEditor::NintendoPublisherName(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->nintendo_publisher_name = text;
            app_data->nintendo_publisher_name_time.getUTC();
        }
}
Str AppPropsEditor::NintendoPublisherName(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->nintendo_publisher_name;
    return S;
}
void AppPropsEditor::NintendoLegalInfo(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->nintendo_legal_info = text;
            app_data->nintendo_legal_info_time.getUTC();
        }
}
Str AppPropsEditor::NintendoLegalInfo(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->nintendo_legal_info;
    return S;
}
void AppPropsEditor::AndroidLicenseKey(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->android_license_key = text;
            app_data->android_license_key_time.getUTC();
        }
}
Str AppPropsEditor::AndroidLicenseKey(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->android_license_key;
    return S;
}
void AppPropsEditor::PlayAssetDelivery(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->playAssetDelivery(TextBool(text));
            app_data->play_asset_delivery_time.getUTC();
            if (ap.elm_id == Proj.curApp())
                CodeEdit.makeAuto();
        }
}
Str AppPropsEditor::PlayAssetDelivery(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->playAssetDelivery();
    return S;
}
void AppPropsEditor::Build(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->build = TextInt(text);
            app_data->build_time.getUTC();
            if (ap.elm_id == Proj.curApp())
                CodeEdit.makeAuto();
        }
}
Str AppPropsEditor::Build(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->build;
    return S;
}
void AppPropsEditor::SaveSize(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            CalcValue cv;
            TextValue(text, cv, false);
            int v = (cv.type ? cv.asInt() : -1);
            app_data->save_size = ((v >= 0) ? v * 1024 * 1024 : -1);
            app_data->save_size_time.getUTC();
        }
}
Str AppPropsEditor::SaveSize(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            if (app_data->save_size >= 0)
                return DivRound(app_data->save_size, 1024 * 1024);
    return S;
}
void AppPropsEditor::LocationUsageReason(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->location_usage_reason = text;
            app_data->location_usage_reason_time.getUTC();
        }
}
Str AppPropsEditor::LocationUsageReason(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->location_usage_reason;
    return S;
}
void AppPropsEditor::FacebookAppID(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->fb_app_id = TextULong(text);
            app_data->fb_app_id_time.getUTC();
        }
}
Str AppPropsEditor::FacebookAppID(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            if (app_data->fb_app_id)
                return app_data->fb_app_id;
    return S;
}
void AppPropsEditor::AdMobAppIDiOS(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->am_app_id_ios = text;
            app_data->am_app_id_ios_time.getUTC();
        }
}
Str AppPropsEditor::AdMobAppIDiOS(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->am_app_id_ios;
    return S;
}
void AppPropsEditor::AdMobAppIDGoogle(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->am_app_id_google = text;
            app_data->am_app_id_google_time.getUTC();
        }
}
Str AppPropsEditor::AdMobAppIDGoogle(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->am_app_id_google;
    return S;
}
void AppPropsEditor::ChartboostAppIDiOS(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->cb_app_id_ios = text;
            app_data->cb_app_id_ios_time.getUTC();
        }
}
Str AppPropsEditor::ChartboostAppIDiOS(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->cb_app_id_ios;
    return S;
}
void AppPropsEditor::ChartboostAppSignatureiOS(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->cb_app_signature_ios = text;
            app_data->cb_app_signature_ios_time.getUTC();
        }
}
Str AppPropsEditor::ChartboostAppSignatureiOS(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->cb_app_signature_ios;
    return S;
}
void AppPropsEditor::ChartboostAppIDGoogle(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->cb_app_id_google = text;
            app_data->cb_app_id_google_time.getUTC();
        }
}
Str AppPropsEditor::ChartboostAppIDGoogle(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->cb_app_id_google;
    return S;
}
void AppPropsEditor::ChartboostAppSignatureGoogle(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->cb_app_signature_google = text;
            app_data->cb_app_signature_google_time.getUTC();
        }
}
Str AppPropsEditor::ChartboostAppSignatureGoogle(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->cb_app_signature_google;
    return S;
}
void AppPropsEditor::Storage(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->storage = Edit::STORAGE_MODE(TextInt(text));
            app_data->storage_time.getUTC();
        }
}
Str AppPropsEditor::Storage(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->storage;
    return S;
}
void AppPropsEditor::GuiSkin(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->gui_skin = Proj.findElmID(text, ELM_GUI_SKIN);
            app_data->gui_skin_time.getUTC();
            if (ap.elm_id == Proj.curApp())
                CodeEdit.makeAuto();
        }
}
Str AppPropsEditor::GuiSkin(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return Proj.elmFullName(app_data->gui_skin);
    return S;
}
void AppPropsEditor::EmbedEngineData(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->embedEngineData(TextInt(text)).embed_engine_data_time.getUTC();
            if (ap.elm_id == Proj.curApp())
                CodeEdit.makeAuto();
        }
}
Str AppPropsEditor::EmbedEngineData(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->embedEngineData();
    return S;
}
void AppPropsEditor::PublishProjData(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->publishProjData(TextBool(text)).publish_proj_data_time.getUTC();
            if (ap.elm_id == Proj.curApp())
                CodeEdit.makeAuto();
        }
}
Str AppPropsEditor::PublishProjData(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->publishProjData();
    return S;
}
void AppPropsEditor::PublishDataAsPak(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->publishDataAsPak(TextBool(text)).publish_data_as_pak_time.getUTC();
            if (ap.elm_id == Proj.curApp())
                CodeEdit.makeAuto();
        }
}
Str AppPropsEditor::PublishDataAsPak(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->publishDataAsPak();
    return S;
}
void AppPropsEditor::PublishPhysxDll(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->publishPhysxDll(TextBool(text)).publish_physx_dll_time.getUTC();
            if (ap.elm_id == Proj.curApp())
                CodeEdit.makeAuto();
        }
}
Str AppPropsEditor::PublishPhysxDll(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->publishPhysxDll();
    return S;
}
void AppPropsEditor::PublishSteamDll(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->publishSteamDll(TextBool(text)).publish_steam_dll_time.getUTC();
            if (ap.elm_id == Proj.curApp()) {
                CodeEdit.makeAuto();
                CodeEdit.rebuildSymbols();
            }
        }
}
Str AppPropsEditor::PublishSteamDll(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->publishSteamDll();
    return S;
}
void AppPropsEditor::PublishOpenVRDll(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->publishOpenVRDll(TextBool(text)).publish_open_vr_dll_time.getUTC();
            if (ap.elm_id == Proj.curApp()) {
                CodeEdit.makeAuto();
                CodeEdit.rebuildSymbols();
            }
...
Str AppPropsEditor::PublishOpenVRDll(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return app_data->publishOpenVRDll();
    return S;
}
void AppPropsEditor::Orientation(AppPropsEditor &ap, C Str &text) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData()) {
            app_data->supported_orientations = OrientToFlag(ORIENT(TextInt(text)));
            app_data->supported_orientations_time.getUTC();
        }
}
Str AppPropsEditor::Orientation(C AppPropsEditor &ap) {
    if (ap.elm)
        if (ElmApp *app_data = ap.elm->appData())
            return FlagToOrient(app_data->supported_orientations);
    return S;
}
void AppPropsEditor::Language(ptr lang_ptr) {
    if (AppPropsEdit.p_languages && AppPropsEdit.elm)
        if (ElmApp *app_data = AppPropsEdit.elm->appData()) {
            LANG_TYPE lang_type = LANG_TYPE((intptr)lang_ptr);
            if (AppPropsEdit.p_languages->combobox.menu(LanguageName(lang_type)))
                app_data->supported_languages.include(lang_type);
            else
                app_data->supported_languages.exclude(lang_type);
...
void AppPropsEditor::create() {
    ...
    p_languages = &add("Supported Languages", MemberDesc(DATA_INT)).setEnum();
    {
        Node<MenuElm> n;
        FREPA(Languages)
        n.New().create(LanguageName(Languages[i]), Language, ptr(Languages[i])).flag(MENU_TOGGLABLE);
    ...
    storage.combobox.setColumns(NameDescListColumn, Elms(NameDescListColumn)).setData(StorageName, Elms(StorageName)).menu.list.setElmDesc(MEMBER(NameDesc, desc));
    ...
    autoData(this);
    win_props.autoData(this);
    mac_props.autoData(this);
    linux_props.autoData(this);
    android_props.autoData(this);
    ios_props.autoData(this);
    nintendo_props.autoData(this);
    flt vw = 0.85f;
    Rect rect = super::create(S, Vec2(0.02f, -0.01f), 0.04f, h, vw);
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
    Vec2 pos = rect.min;
    pos.y -= h * 3.5f;
    flt th = fb_app_id.textline.rect().h(), cw = clientWidth(), pw = cw - 0.02f;
    vw = cw;
    T += platforms.create(platforms_t, Elms(platforms_t)).valid(true).set(PWIN).rect(Rect_LU(pos, cw - 0.04f, 0.05f));
    pos.y -= h;
    ...
    win_props.changed(Changed);
    mac_props.changed(Changed);
    linux_props.changed(Changed);
    android_props.changed(Changed);
    ios_props.changed(Changed);
    nintendo_props.changed(Changed);

    T += fb_app_id.button.create(Rect_RU(fb_app_id.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetFacebookAppID);
    fb_app_id.textline.rect(Rect(fb_app_id.textline.rect().ld(), fb_app_id.button.rect().lu()));

    platforms.tab(PWIN) += ms_pub_id.button.create(Rect_RU(ms_pub_id.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetMicrosoftPublisher);
    ms_pub_id.textline.rect(Rect(ms_pub_id.textline.rect().ld(), ms_pub_id.button.rect().lu()));
    platforms.tab(PWIN) += ms_pub_nm.button.create(Rect_RU(ms_pub_nm.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetMicrosoftPublisher);
    ms_pub_nm.textline.rect(Rect(ms_pub_nm.textline.rect().ld(), ms_pub_nm.button.rect().lu()));
    platforms.tab(PWIN) += xb_prog.button.create(Rect_RU(xb_prog.combobox.rect().ru() + Vec2(0, 0), th * 2, th), "Get").func(GetXboxLive);
    xb_prog.combobox.rect(Rect(xb_prog.combobox.rect().ld(), xb_prog.button.rect().lu()));
    platforms.tab(PWIN) += xb_tit_id.button.create(Rect_RU(xb_tit_id.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetXboxLive);
    xb_tit_id.textline.rect(Rect(xb_tit_id.textline.rect().ld(), xb_tit_id.button.rect().lu()));
    platforms.tab(PWIN) += xb_scid.button.create(Rect_RU(xb_scid.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetXboxLive);
    xb_scid.textline.rect(Rect(xb_scid.textline.rect().ld(), xb_scid.button.rect().lu()));

    platforms.tab(PAND) += am_ai_g.button.create(Rect_RU(am_ai_g.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetAdMobApp);
    am_ai_g.textline.rect(Rect(am_ai_g.textline.rect().ld(), am_ai_g.button.rect().lu()));
    platforms.tab(PAND) += cb_ai_g.button.create(Rect_RU(cb_ai_g.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetChartboostApp);
    cb_ai_g.textline.rect(Rect(cb_ai_g.textline.rect().ld(), cb_ai_g.button.rect().lu()));
    platforms.tab(PAND) += cb_as_g.button.create(Rect_RU(cb_as_g.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetChartboostApp);
    cb_as_g.textline.rect(Rect(cb_as_g.textline.rect().ld(), cb_as_g.button.rect().lu()));
    platforms.tab(PAND) += google_lk.button.create(Rect_RU(google_lk.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetAndroidLicenseKey);
    google_lk.textline.rect(Rect(google_lk.textline.rect().ld(), google_lk.button.rect().lu()));

    platforms.tab(PIOS) += am_ai_i.button.create(Rect_RU(am_ai_i.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetAdMobApp);
    am_ai_i.textline.rect(Rect(am_ai_i.textline.rect().ld(), am_ai_i.button.rect().lu()));
    platforms.tab(PIOS) += cb_ai_i.button.create(Rect_RU(cb_ai_i.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetChartboostApp);
    cb_ai_i.textline.rect(Rect(cb_ai_i.textline.rect().ld(), cb_ai_i.button.rect().lu()));
    platforms.tab(PIOS) += cb_as_i.button.create(Rect_RU(cb_as_i.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetChartboostApp);
    cb_as_i.textline.rect(Rect(cb_as_i.textline.rect().ld(), cb_as_i.button.rect().lu()));

    platforms.tab(PNIN) += nn_ini_cd.button.create(Rect_RU(nn_ini_cd.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetNintendo);
    nn_ini_cd.textline.rect(Rect(nn_ini_cd.textline.rect().ld(), nn_ini_cd.button.rect().lu()));
    platforms.tab(PNIN) += nn_app_id.button.create(Rect_RU(nn_app_id.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetNintendo);
    nn_app_id.textline.rect(Rect(nn_app_id.textline.rect().ld(), nn_app_id.button.rect().lu()));
    platforms.tab(PNIN) += nn_pub_nm.button.create(Rect_RU(nn_pub_nm.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetNintendo);
    nn_pub_nm.textline.rect(Rect(nn_pub_nm.textline.rect().ld(), nn_pub_nm.button.rect().lu()));
    platforms.tab(PNIN) += nn_legal.button.create(Rect_RU(nn_legal.textline.rect().ru() + Vec2(th, 0), th * 2, th), "Get").func(GetNintendoLegal);
    nn_legal.textline.rect(Rect(nn_legal.textline.rect().ld(), nn_legal.button.rect().lu()));
    ...
    if (p_languages) {
        ...
        if (elm)
            if (ElmApp *app_data = elm->appData())
                FREPA(app_data->supported_languages)
        p_languages->combobox.text.space() += CaseUp(LanguageCode(app_data->supported_languages[i]));
    }
}
void AppPropsEditor::toGui() {
    ...
    if (p_languages) {
        ...
        FREPA(Languages)
        p_languages->combobox.menu(LanguageName(Languages[i]), app_data && app_data->supported_languages.has(Languages[i]), QUIET);
    }
}
AppPropsEditor &AppPropsEditor::hide() {
    set(null);
    super::hide();
    return T;
}
...
void AppPropsEditor::set(Elm *elm) {
    ...
    if (T.elm != elm) {
        ...
        if (elm)
            setTitle(S + '"' + elm->name + "\" Settings");
    }
}
void AppPropsEditor::activate(Elm *elm) {
    set(elm);
    if (T.elm)
        super::activate();
}
void AppPropsEditor::toggle(Elm *elm) {
    if (elm == T.elm)
        elm = null;
    set(elm);
}
void AppPropsEditor::elmChanged(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        toGui();
    if (elm_id == Proj.curApp())
        CodeEdit.makeAuto();
}
void AppPropsEditor::erasing(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        set(null);
}
void AppPropsEditor::drag(Memc<UID> &elms, GuiObj *obj, C Vec2 &screen_pos) {
    ...
    if (Elm *elm = Proj.findElm(elms[i]))
        if (ElmImageLike(elm->type)) {
            if (obj == &icon)
                icon.setImage(elm->id);
            if (obj == &image_portrait)
                image_portrait.setImage(elm->id);
            if (obj == &image_landscape)
                image_landscape.setImage(elm->id);
            if (obj == &notification_icon)
                notification_icon.setImage(elm->id);
            break;
        }
}
