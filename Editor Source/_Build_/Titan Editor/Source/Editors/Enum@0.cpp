...
/******************************************************************************/
void EnumEditor::Enum::setColor() {
    color = GuiListTextColor();
    if (removed)
        color.a /= 2;
}
void EnumEditor::Enum::operator=(C EditEnum &src) {
    enum_id = src.id;
    name = src.name;
    removed = src.removed;
    setColor();
...
void EnumEditor::RenameWindow::create() {
    Gui += super ::create(Rect_C(0, 0, 1, 0.14f), "Rename Enum").hide();
    button[2].show();
...
void EnumEditor::RenameWindow::update(C GuiPC &gpc) {
    ...
    if (Gui.window() == this) {
        if (Kb.k(KB_ENTER)) {
            if (EnumEdit.rename(enum_id, textline()))
                hide();
            else
                Gui.msgBox(S, "Invalid enum name.\nEnum may contain only letters, digits and underline characters.\nIt may not contain spaces, symbols or start with a digit.");
            Kb.eatKey();
        }
    }
}
int EnumEditor::ListEnum::getPos(C Vec2 &pos) {
    int p = screenToVisY(pos.y - elmHeight() / 2);
    return (p >= 0) ? p : visibleElms();
}
void EnumEditor::ListEnum::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        GuiSkin skin;
        int elm = -1;
        if (Gui.dragging() && Gui.objAtPos(Gui.dragPos()) == this)
            elm = getPos(Gui.dragPos());
        if (elm >= 0) {
            if (Gui.skin)
                skin = *Gui.skin;
            skin.list.highlight_color.zero();
            T.skin(&skin);
        }
        super::draw(gpc);
        if (elm >= 0) {
...
void EnumEditor::newEnum() {
    if (elm)
        FREP(1000) {
            Str name = S + "ENUM_" + i;
            REPA(enums)
            if (Equal(enums[i].name, name, true))
                goto exists;

            {
                undos.set("new", true);
                EditEnum &e = enums.New().setName(name);
                e.order_time.getUTC();
                setChanged();
...
bool EnumEditor::rename(C UID &enum_id, C Str &name) {
    if (ValidEnum(name)) {
        ...
        if (enums[i].id == enum_id) {
            undos.set("rename", true);
            enums[i].setName(name);
            setChanged();
            break;
        }
        return true;
...
void EnumEditor::create() {
    ...
    Gui += super::create("Enum Editor").hide();
    button[1].show();
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
    flag |= WIN_RESIZABLE;
    T += undo.create(Rect_LU(0.01f, -0.01f, 0.055f, 0.055f)).func(Undo, T).focusable(false).desc("Undo");
    undo.image = "Gui/Misc/undo.img";
    T += redo.create(Rect_LU(undo.rect().ru(), 0.055f, 0.055f)).func(Redo, T).focusable(false).desc("Redo");
    ...
    T += show_removed.create().func(ShowRemoved, T).focusable(false).desc("Show removed elements");
    ...
    list.setElmOffset(MEMBER(Enum, offset)).setElmTextColor(MEMBER(Enum, color));
    rect(Rect_C(0, 0, 0.6f, 1));
    rename_window.create();
}
Rect EnumEditor::sizeLimit() C {
    Rect r = super::sizeLimit();
    r.min.set(0.61f, 0.35f);
    return r;
}
EnumEditor &EnumEditor::rect(C Rect &rect) {
    super::rect(rect);
    Rect_RU r(clientWidth() - 0.01f, -0.01f, 0.23f, 0.055f);
    if (r.min.x - 0.01f - type.rect().h() < type.rect().max.x + 0.01f)
        r -= Vec2(0, type.rect().h() + 0.01f);
    new_enum.rect(r);
...
void EnumEditor::toGui() {
    ...
    data.clear();
    FREPA(enums)
    if (!enums[i].removed || show_removed()) {
        Enum &e = data.New();
        e = enums[i];
...
void EnumEditor::set(Elm *elm) {
    ...
    if (T.elm != elm) {
        ...
        if (elm)
            enums.load(Proj.editPath(elm->id));
        else
            enums.del();
        toGui();
...
void EnumEditor::activate(Elm *elm) {
    set(elm);
    if (T.elm)
        super::activate();
}
void EnumEditor::toggle(Elm *elm) {
    if (elm == T.elm)
        elm = null;
    set(elm);
}
EnumEditor &EnumEditor::hide() {
    if (visible()) {
        super::hide();
        set(null);
    }
...
void EnumEditor::dragEnums(GuiObj *obj, C Vec2 &screen_pos) {
    if (elm && obj == &list) {
        ...
        if (list.sel.elms() && target > list.sel.first())
            target--;
        ...
        int ti = list.sel.find(target);
        if (ti >= 0)
            for (; InRange(ti - 1, list.sel);) {
                int prev_target = list.sel[ti - 1];
                if (list.absToVis(prev_target) == list.absToVis(target) - 1) {
                    ti--;
                    target = prev_target;
                } else
                    break;
        ...
        Memt<UID> enum_id;
        FREPA(list.sel)
        if (Enum *e = list.absToData(list.sel[i]))
            enum_id.add(e->enum_id);
        if (enum_id.elms()) {
            undos.set("move", true);
            int tar = -1;
            if (InRange(target, list.totalElms()))
                if (Enum *e = list.absToData(target))
                    tar = enums.findI(e->enum_id);
            if (!InRange(tar, enums))
                tar = enums.elms(); // move to end
            FREPA(enum_id)
            tar = enums.move(enum_id[i], tar);
            setChanged();
            REPD(v, list.visibleElms())
            if (Enum *e = list.visToData(v))
                if (e->enum_id == list_cur) {
                    list.cur = v;
                    break;
                }
            FREPA(enum_id)
...
void EnumEditor::update(C GuiPC &gpc) {
    ...
    if (Gui.window() == this)
        REPA(MT)
    if (MT.bp(i) && MT.guiObj(i) == &list)
        if (Enum *e = list()) {
            if (list.selMode() == LSM_SET)
                Gui.drag(DragEnums, T, MT.touch(i));
            if (MT.bd(i))
                rename_window.activate(e->enum_id, e->name);
        }
}
...
void EnumEditor::erasing(C UID &elm_id) {
