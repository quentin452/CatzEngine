...
void EnvEditor::PropWin::Hide(PropWin &pw) {
    if (&pw == &EnvEdit.ambient)
        EnvEdit.bambient.push();
    if (&pw == &EnvEdit.bloom)
        EnvEdit.bbloom.push();
    if (&pw == &EnvEdit.clouds)
        EnvEdit.bclouds.push();
    if (&pw == &EnvEdit.fog)
        EnvEdit.bfog.push();
    if (&pw == &EnvEdit.sky)
        EnvEdit.bsky.push();
...
::EnvEditor::PropWin &EnvEditor::PropWin::create(C Str &name, flt value_width) {
    super::create(name, Vec2(0.02f, -0.02f), 0.036f, 0.043f, value_width);
    changed(Changed, PreChanged);
    button[2].func(Hide, T).show();
    return T;
}
...
void EnvEditor::AmbientColor(EditEnv &env, C Str &text) {
    env.ambient.color_s = TextVec(text);
    env.ambient_color_time.getUTC();
}
Str EnvEditor::AmbientColor(C EditEnv &env) { return Vec4(env.ambient.color_s, 1); }
void EnvEditor::NightShadeColor(EditEnv &env, C Str &text) {
    env.ambient.night_shade_color_s = TextVec(text);
    env.night_shade_color_time.getUTC();
}
Str EnvEditor::NightShadeColor(C EditEnv &env) { return Vec4(env.ambient.night_shade_color_s, 1); }
void EnvEditor::BloomOriginal(EditEnv &env, C Str &text) {
    env.bloom.original = TextFlt(text);
    env.bloom_original_time.getUTC();
}
Str EnvEditor::BloomOriginal(C EditEnv &env) { return env.bloom.original; }
void EnvEditor::BloomScale(EditEnv &env, C Str &text) {
    env.bloom.scale = TextFlt(text);
    env.bloom_scale_time.getUTC();
}
Str EnvEditor::BloomScale(C EditEnv &env) { return env.bloom.scale; }
void EnvEditor::BloomCut(EditEnv &env, C Str &text) {
    env.bloom.cut = TextFlt(text);
    env.bloom_cut_time.getUTC();
}
Str EnvEditor::BloomCut(C EditEnv &env) { return env.bloom.cut; }
void EnvEditor::BloomGlow(EditEnv &env, C Str &text) {
    env.bloom.glow = TextFlt(text);
    env.bloom_glow_time.getUTC();
}
Str EnvEditor::BloomGlow(C EditEnv &env) { return env.bloom.glow; }
void EnvEditor::CloudsScaleY(EditEnv &env, C Str &text) {
    env.clouds.vertical_scale = TextFlt(text);
    env.clouds_vertical_scale_time.getUTC();
}
Str EnvEditor::CloudsScaleY(C EditEnv &env) { return env.clouds.vertical_scale; }
void EnvEditor::FogSky(EditEnv &env, C Str &text) {
    env.fog.affect_sky = TextBool(text);
    env.fog_affect_sky_time.getUTC();
}
Str EnvEditor::FogSky(C EditEnv &env) { return env.fog.affect_sky; }
void EnvEditor::FogDensity(EditEnv &env, C Str &text) {
    env.fog.density = TextFlt(text);
    env.fog_density_time.getUTC();
}
Str EnvEditor::FogDensity(C EditEnv &env) { return (dbl)env.fog.density; }
void EnvEditor::FogColor(EditEnv &env, C Str &text) {
    env.fog.color_s = TextVec(text);
    env.fog_color_time.getUTC();
}
Str EnvEditor::FogColor(C EditEnv &env) { return Vec4(env.fog.color_s, 1); }
void EnvEditor::SkyFrac(EditEnv &env, C Str &text) {
    env.sky.frac = TextFlt(text);
    env.sky_frac_time.getUTC();
}
Str EnvEditor::SkyFrac(C EditEnv &env) { return env.sky.frac; }
void EnvEditor::SkyDnsExp(EditEnv &env, C Str &text) {
    env.sky.atmospheric_density_exponent = TextFlt(text);
    env.sky_atmospheric_density_exponent_time.getUTC();
}
Str EnvEditor::SkyDnsExp(C EditEnv &env) { return (dbl)env.sky.atmospheric_density_exponent; }
void EnvEditor::SkyHorExp(EditEnv &env, C Str &text) {
    env.sky.atmospheric_horizon_exponent = TextFlt(text);
    env.sky_atmospheric_horizon_exponent_time.getUTC();
}
Str EnvEditor::SkyHorExp(C EditEnv &env) { return env.sky.atmospheric_horizon_exponent; }
void EnvEditor::SkyHorCol(EditEnv &env, C Str &text) {
    env.sky.atmospheric_horizon_color_s = TextVec4(text);
    env.sky_atmospheric_horizon_color_time.getUTC();
}
Str EnvEditor::SkyHorCol(C EditEnv &env) { return env.sky.atmospheric_horizon_color_s; }
void EnvEditor::SkySkyCol(EditEnv &env, C Str &text) {
    env.sky.atmospheric_sky_color_s = TextVec4(text);
    env.sky_atmospheric_sky_color_time.getUTC();
}
Str EnvEditor::SkySkyCol(C EditEnv &env) { return env.sky.atmospheric_sky_color_s; }
void EnvEditor::SkyStars(EditEnv &env, C Str &text) {
    env.star_id = Proj.findElmImageID(text);
    env.sky_atmospheric_stars_time.getUTC();
}
Str EnvEditor::SkyStars(C EditEnv &env) { return Proj.elmFullName(env.star_id); }
void EnvEditor::SkyBox(EditEnv &env, C Str &text) {
    env.skybox_id = Proj.findElmImageID(text);
    env.sky_skybox_time.getUTC();
}
Str EnvEditor::SkyBox(C EditEnv &env) { return Proj.elmFullName(env.skybox_id); }
void EnvEditor::SunGlow(EditEnv &env, C Str &text) {
    env.sun.glow = TextInt(text);
    env.sun_glow_time.getUTC();
}
Str EnvEditor::SunGlow(C EditEnv &env) { return env.sun.glow; }
void EnvEditor::SunSize(EditEnv &env, C Str &text) {
    env.sun.size = TextFlt(text);
    env.sun_size_time.getUTC();
}
Str EnvEditor::SunSize(C EditEnv &env) { return env.sun.size; }
void EnvEditor::SunImage(EditEnv &env, C Str &text) {
    env.sun_id = Proj.findElmImageID(text);
    env.sun_image_time.getUTC();
}
Str EnvEditor::SunImage(C EditEnv &env) { return Proj.elmFullName(env.sun_id); }
void EnvEditor::SunImageC(EditEnv &env, C Str &text) {
    env.sun.image_color = TextVec4(text);
    env.sun_image_color_time.getUTC();
}
Str EnvEditor::SunImageC(C EditEnv &env) { return env.sun.image_color; }
void EnvEditor::SunLight(EditEnv &env, C Str &text) {
    env.sun.light_color_s = TextVec(text);
    env.sun_light_color_time.getUTC();
}
Str EnvEditor::SunLight(C EditEnv &env) { return Vec4(env.sun.light_color_s, 1); }
void EnvEditor::SunLitF(EditEnv &env, C Str &text) {
    env.sun.highlight_front = TextFlt(text);
    env.sun_highlight_front_time.getUTC();
}
Str EnvEditor::SunLitF(C EditEnv &env) { return env.sun.highlight_front; }
void EnvEditor::SunLitB(EditEnv &env, C Str &text) {
    env.sun.highlight_back = TextFlt(text);
    env.sun_highlight_back_time.getUTC();
}
Str EnvEditor::SunLitB(C EditEnv &env) { return env.sun.highlight_back; }
void EnvEditor::SunRays(EditEnv &env, C Str &text) {
    env.sun.rays_color = TextVec(text);
    env.sun_rays_color_time.getUTC();
}
...
void EnvEditor::CAmbient(EnvEditor &ee) {
    EnvEdit.undos.set(null, true);
    ee.edit.ambient.on ^= 1;
    ee.edit.ambient_on_time.getUTC();
    ee.setChanged();
}
void EnvEditor::CBloom(EnvEditor &ee) {
    EnvEdit.undos.set(null, true);
    ee.edit.bloom.on ^= 1;
    ee.edit.bloom_on_time.getUTC();
    ee.setChanged();
}
void EnvEditor::CClouds(EnvEditor &ee) {
    EnvEdit.undos.set(null, true);
    ee.edit.clouds.on ^= 1;
    ee.edit.clouds_on_time.getUTC();
    ee.setChanged();
}
void EnvEditor::CFog(EnvEditor &ee) {
    EnvEdit.undos.set(null, true);
    ee.edit.fog.on ^= 1;
    ee.edit.fog_on_time.getUTC();
    ee.setChanged();
}
void EnvEditor::CSun(EnvEditor &ee) {
    EnvEdit.undos.set(null, true);
    ee.edit.sun.on ^= 1;
    ee.edit.sun_on_time.getUTC();
    ee.setChanged();
}
void EnvEditor::CSky(EnvEditor &ee) {
    EnvEdit.undos.set(null, true);
    ee.edit.sky.on ^= 1;
    ee.edit.sky_on_time.getUTC();
    ee.setChanged();
}
...
Rect EnvEditor::getRect(C Rect &rect) {
    ...
    rects.add(T.rect());
    if (ambient.visible())
        rects.add(ambient.rect());
    if (bloom.visible())
        rects.add(bloom.rect());
    if (clouds.visible())
        rects.add(clouds.rect());
    if (fog.visible())
        rects.add(fog.rect());
    if (sun.visible())
        rects.add(sun.rect());
...
void EnvEditor::toggleWin(GuiObj &obj) {
    if (obj.hidden())
        obj.rect(getRect(obj.rect()));
    obj.visibleToggleActivate();
}
void EnvEditor::setSunAngle(flt x, flt y) {
    sun_angle.set(x, y);
    edit.sun.pos = Matrix3().setRotateXY(-y, x).z;
    edit.sun_pos_time.getUTC();
}
void EnvEditor::create() {
    Gui += super::create("Environment").hide();
    ...
    T += cambient.create(Rect_L(p, y, c, c)).func(CAmbient, T);
    T += bambient.create(Rect_L(x, y, w, h), "Ambient").func(BAmbient, T).focusable(false);
    bambient.mode = BUTTON_TOGGLE;
    y -= h;
    T += cbloom.create(Rect_L(p, y, c, c)).func(CBloom, T);
    T += bbloom.create(Rect_L(x, y, w, h), "Bloom").func(BBloom, T).focusable(false);
    bbloom.mode = BUTTON_TOGGLE;
    y -= h;
    T += cclouds.create(Rect_L(p, y, c, c)).func(CClouds, T);
    T += bclouds.create(Rect_L(x, y, w, h), "Clouds").func(BClouds, T).focusable(false);
    bclouds.mode = BUTTON_TOGGLE;
    y -= h;
    T += cfog.create(Rect_L(p, y, c, c)).func(CFog, T);
    T += bfog.create(Rect_L(x, y, w, h), "Fog").func(BFog, T).focusable(false);
    bfog.mode = BUTTON_TOGGLE;
    y -= h;
    T += csun.create(Rect_L(p, y, c, c)).func(CSun, T);
    T += bsun.create(Rect_L(x, y, w, h), "Sun").func(BSun, T).focusable(false);
    bsun.mode = BUTTON_TOGGLE;
    y -= h;
    T += csky.create(Rect_L(p, y, c, c)).func(CSky, T);
    T += bsky.create(Rect_L(x, y, w, h), "Sky").func(BSky, T).focusable(false);
    bsky.mode = BUTTON_TOGGLE;
    y -= h;
    ...
    ambient.autoData(&edit);
    ambient.create("Ambient", 0.2f);
    ...
    clouds.autoData(&edit);
    clouds.create("Clouds", PropElmNameWidth);
    ...
    sun.autoData(&edit);
    sun.create("Sun", PropElmNameWidth);
}
void EnvEditor::toGame() { edit.copyTo(*game, Proj); }
void EnvEditor::toGui() {
    sun_angle = Matrix3().setDir(edit.sun.pos).angles().yx();
    CHS(sun_angle.y);
    cambient.set(edit.ambient.on, QUIET);
    ambient.toGui();
    cbloom.set(edit.bloom.on, QUIET);
    bloom.toGui();
    cclouds.set(edit.clouds.on, QUIET);
    clouds.toGui();
    cfog.set(edit.fog.on, QUIET);
    fog.toGui();
    csky.set(edit.sky.on, QUIET);
    sky.toGui();
    csun.set(edit.sun.on, QUIET);
    sun.toGui();
    toGame();
}
EnvEditor &EnvEditor::hide() {
    set(null);
    ambient.hide();
    bloom.hide();
    clouds.hide();
...
void EnvEditor::activate(Elm *elm) {
    set(elm);
    if (T.elm)
        super::activate();
}
void EnvEditor::toggle(Elm *elm) {
    if (elm == T.elm)
        elm = null;
    set(elm);
}
...
void EnvEditor::erasing(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        set(null);
}
