...
void FontEditor::ParamsFont(Params &p, C Str &t) {
    p.font = t;
    p.font_time.getUTC();
}
void FontEditor::ParamsSize(Params &p, C Str &t) {
    p.size = TextInt(t);
    p.size_time.getUTC();
}
void FontEditor::ParamsScale(Params &p, C Str &t) {
    p.scale = TextFlt(t);
    p.scale_time.getUTC();
}
void FontEditor::ParamsClearType(Params &p, C Str &t) {
    p.clear_type = TextBool(t);
    p.clear_type_time.getUTC();
}
void FontEditor::ParamsSoftware(Params &p, C Str &t) {
    p.software = TextBool(t);
    p.software_time.getUTC();
}
void FontEditor::ParamsWeight(Params &p, C Str &t) {
    p.weight = TextFlt(t);
    p.weight_time.getUTC();
}
void FontEditor::ParamsDiagShadow(Params &p, C Str &t) {
    p.diagonal_shadow = TextBool(t);
    p.diagonal_shadow_time.getUTC();
}
void FontEditor::ParamsMipMaps(Params &p, C Str &t) {
    p.mip_maps = TextInt(t);
    p.mip_maps_time.getUTC();
}
void FontEditor::ParamsShdBlur(Params &p, C Str &t) {
    p.shadow_blur = TextFlt(t);
    p.shadow_blur_time.getUTC();
}
void FontEditor::ParamsShdOpacity(Params &p, C Str &t) {
    p.shadow_opacity = TextFlt(t);
    p.shadow_opacity_time.getUTC();
}
void FontEditor::ParamsShdSpread(Params &p, C Str &t) {
    p.shadow_spread = TextFlt(t);
    p.shadow_spread_time.getUTC();
}
void FontEditor::ParamsAscii(Params &p, C Str &t) {
    p.ascii = TextBool(t);
    p.ascii_time.getUTC();
}
void FontEditor::ParamsGerman(Params &p, C Str &t) {
    p.german = TextBool(t);
    p.german_time.getUTC();
}
void FontEditor::ParamsFrench(Params &p, C Str &t) {
    p.french = TextBool(t);
    p.french_time.getUTC();
}
void FontEditor::ParamsPolish(Params &p, C Str &t) {
    p.polish = TextBool(t);
    p.polish_time.getUTC();
}
void FontEditor::ParamsRussian(Params &p, C Str &t) {
    p.russian = TextBool(t);
    p.russian_time.getUTC();
}
void FontEditor::ParamsChinese(Params &p, C Str &t) {
    p.chinese = TextBool(t);
    p.chinese_time.getUTC();
}
void FontEditor::ParamsJapanese(Params &p, C Str &t) {
    p.japanese = TextBool(t);
    p.japanese_time.getUTC();
}
void FontEditor::ParamsKorean(Params &p, C Str &t) {
    p.korean = TextBool(t);
    p.korean_time.getUTC();
}
void FontEditor::ParamsCustomChar(Params &p, C Str &t) {
    p.custom_chars = t;
    p.custom_chars_time.getUTC();
}
void FontEditor::ParamsSampleText(Params &p, C Str &t) {
    p.sample_text = t;
    FontEdit.refresh();
}
...
void FontEditor::create() {
    ...
    super::create("Font Editor", Vec2(0.02f, -0.07f), 0.036f, 0.043f, 0.3f);
    super::changed(Changed, PreChanged);
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
    text.changed(null, null);
...
void FontEditor::flush() {
    if (elm && changed) {
        ...
        Proj.elmReload(elm_id, false, false);
    }
    changed = false;
}
...
void FontEditor::activate(Elm *elm) {
    set(elm);
    if (T.elm)
        super::activate();
}
void FontEditor::toggle(Elm *elm) {
    if (elm == T.elm)
        elm = null;
    set(elm);
}
...
void FontEditor::erasing(C UID &elm_id) {
