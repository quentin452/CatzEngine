...
void GuiView::CheckBox2::update(C GuiPC &gpc) {
    GuiPC temp = gpc;
    temp.enabled = false;
    super::update(temp);
}
GuiView::ComboBox2::~ComboBox2() { GuiEdit.removed(this); }
GuiView::ComboBox2::ComboBox2() {
    create();
    size(Vec2(0.3f, 0.05f));
}
...
void GuiView::ComboBox2::update(C GuiPC &gpc) {
    GuiPC temp = gpc;
    temp.enabled = false;
    super::update(temp);
}
GuiView::GuiCustom2::~GuiCustom2() { GuiEdit.removed(this); }
GuiView::GuiCustom2::GuiCustom2() {
    create();
    size(Vec2(0.3f));
}
...
void GuiView::GuiCustom2::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        D.clip(gpc.clip);
        (rect() + gpc.offset).draw(BLACK, false);
    }
}
...
GuiView::Tabs2::Tabs2() {
    cchar8 *t[] = {"Tab 0", "Tab 1", "Tab 2"};
    create(t, Elms(t));
    size(Vec2(0.6f, 0.06f));
}
...
void GuiView::Text2::draw(C GuiPC &gpc) {
    super::draw(gpc);
    if (gpc.visible && visible() && TextRectColor.a) {
        D.clip(gpc.clip);
        (rect() + gpc.offset).draw(TextRectColor, false);
    }
}
...
void GuiView::TextBox2::update(C GuiPC &gpc) {
    GuiPC temp = gpc;
    temp.enabled = false;
    super::update(temp);
}
void GuiView::TextBox2::draw(C GuiPC &gpc) {
    GuiPC temp = gpc;
    temp.enabled = false;
    super::draw(temp);
}
...
void GuiView::TextLine2::update(C GuiPC &gpc) {
    GuiPC temp = gpc;
    temp.enabled = false;
    super::update(temp);
}
void GuiView::TextLine2::draw(C GuiPC &gpc) {
    GuiPC temp = gpc;
    temp.enabled = false;
    super::draw(temp);
}
...
void GuiView::Viewport2::draw(C GuiPC &gpc) {
    draw_func = ViewportDraw;
    super::draw(gpc);
}
GuiView::Window2::~Window2() { GuiEdit.removed(this); }
GuiView::Window2::Window2() {
    create();
    size(Vec2(0.3f));
}
...
void GuiView::Window2::update(C GuiPC &gpc) {
    uint flag = T.flag;
    FlagDisable(T.flag, WIN_MOVABLE | WIN_RESIZABLE);
    super::update(gpc);
    T.flag = flag;
}
Memx<GuiObj> *GuiView::Objects::container(GUI_OBJ_TYPE type) {
    ...
    default:
        return null;
    case GO_BUTTON:
        return &SCAST(Memx<GuiObj>, button);
    case GO_CHECKBOX:
        return &SCAST(Memx<GuiObj>, checkbox);
    case GO_COMBOBOX:
        return &SCAST(Memx<GuiObj>, combobox);
    case GO_CUSTOM:
        return &SCAST(Memx<GuiObj>, custom);
    case GO_DESKTOP:
        return &SCAST(Memx<GuiObj>, desktop);
    case GO_IMAGE:
        return &SCAST(Memx<GuiObj>, image);
    case GO_LIST:
        return &SCAST(Memx<GuiObj>, list);
    case GO_MENU:
        return &SCAST(Memx<GuiObj>, menu);
    case GO_MENU_BAR:
        return &SCAST(Memx<GuiObj>, menubar);
    case GO_PROGRESS:
        return &SCAST(Memx<GuiObj>, progress);
    case GO_REGION:
        return &SCAST(Memx<GuiObj>, region);
    case GO_SLIDEBAR:
        return &SCAST(Memx<GuiObj>, slidebar);
    case GO_SLIDER:
        return &SCAST(Memx<GuiObj>, slider);
    case GO_TABS:
        return &SCAST(Memx<GuiObj>, tabs);
    case GO_TEXT:
        return &SCAST(Memx<GuiObj>, text);
    case GO_TEXTBOX:
        return &SCAST(Memx<GuiObj>, textbox);
    case GO_TEXTLINE:
        return &SCAST(Memx<GuiObj>, textline);
    case GO_VIEWPORT:
        return &SCAST(Memx<GuiObj>, viewport);
    case GO_WINDOW:
        return &SCAST(Memx<GuiObj>, window);
    }
}
...
bool GuiView::Objects::contains(GuiObj *go) // if 'go' is stored in this objects
{
    if (go)
        REP(GO_NUM)
    if (Memx<GuiObj> *container = T.container(GUI_OBJ_TYPE(i)))
        if (container->contains(go))
            return true;
    return false;
}
void GuiView::Objects::del() {
    REP(GO_NUM)
    if (Memx<GuiObj> *memx = container(GUI_OBJ_TYPE(i)))
        memx->del();
}
...
GuiObj *GuiView::Objects::New(GUI_OBJ_TYPE type) {
    if (Memx<GuiObj> *container = T.container(type))
...
void GuiView::Objects::remove(GuiObj *go) {
    ...
    if (go && contains(go)) {
        REP(go->childNum())
        remove(go->child(i)); // go from end
        if (Memx<GuiObj> *container = T.container(go->type()))
            container->removeData(go);
    }
}
bool GuiView::Objects::save(File &f, cchar *path) C {
    GuiObjs objs;
    copyTo(objs);
    return objs.save(f, path);
}
...
bool GuiView::Objects::save(C Str &name) C {
    File f;
    if (save(f.writeMem(), GetPath(name))) {
        f.pos(0);
...
void GuiView::ObjEdit::PreChanged(C Property &prop) {
    Skin = Gui.skin;
    if (GuiSkinPtr app_skin = Proj.appGuiSkin())
        Gui.skin = app_skin;
    GuiEdit.undo.set(&prop);
}
void GuiView::ObjEdit::Changed(C Property &prop) {
    GuiEdit.setChanged();
    Gui.skin = Skin;
    Skin = null;
}
Rect GuiView::ObjEdit::create(GUI_OBJ_TYPE type, int level) {
    if (props.elms()) {
        ...
        Rect rect = super::create(S + GuiObjTypeName(type) + " Properties", Vec2(0.02f, -0.02f), 0.036f, 0.043f, PropElmNameWidth);
...
void GuiView::ObjEdit::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        ...
        Gui.skin = Mode.Gui_skin; // restore Editor's Gui.skin before drawing this Window
        super::draw(gpc);
        Gui.skin = temp;
    }
}
GuiObj *GuiView::MainObject(GuiObj *go) {
    if (go) {
        if (go->parent())
            switch (go->type()) {
            case GO_BUTTON:
                if (go->parent()->type() == GO_SLIDEBAR)
                    go = go->parent();
                break; // one of slidebar buttons
            case GO_TAB:
                go = go->parent();
                break;
            }
        if (go->parent() && go->parent()->type() == GO_REGION) {
            ...
            if (go == &region.slidebar[0] || go == &region.slidebar[1] || go == &region.view)
                go = &region;
        }
    }
    return go;
...
GuiObj *GuiView::MainParent(GuiObj *parent, bool allow_tabs) {
    if (!allow_tabs && parent && parent->type() == GO_TAB)
        if (GuiObj *tabs = parent->parent())
            if (tabs->type() == GO_TABS)
                return MainParent(tabs->parent(), allow_tabs);
...
GuiObj *GuiView::FirstContainer(GuiObj *go, bool allow_tabs) {
    for (; go; go = go->parent())
        ...
        case GO_WINDOW:
            return go;
        case GO_TAB:
            return allow_tabs ? go : FirstContainer(go->parent(), allow_tabs);
            break;
        }
    return null;
}
...
GUI_OBJ_TYPE GuiView::Panel::objType() C {
    if (InRange(obj_type(), obj_new))
        return obj_new[obj_type()];
    return GO_NONE;
}
void GuiView::Panel::align(Vec2 &pos, bool force) {
    if ((grid_align() || force) && grid_size > EPS)
        pos.set(AlignRound(pos.x, grid_size), AlignRound(pos.y, grid_size));
}
void GuiView::Panel::alignTrunc(Vec2 &pos) {
    if (grid_align() && grid_size > EPS)
        pos.set(AlignTrunc(pos.x, grid_size), AlignTrunc(pos.y, grid_size));
}
::GuiView::Panel &GuiView::Panel::create() {
    ...
    T += draw_proportions.create(Rect_LU(b, -b, h, h)).focusable(false).desc("Draw screen proportions:\nPurple : X Range    -1 .. 1   , Aspect Ratio  1:1\nBlue   : X Range -1.25 .. 1.25, Aspect Ratio 5:4 (1280x1024)\nGreen : X Range -1.33 .. 1.33, Aspect Ratio 4:3 (1024x768)\nRed   : X Range -1.60 .. 1.60, Aspect Ratio 16:10 (1680x1050)\nCyan  : X Range -1.77 .. 1.77, Aspect Ratio 16:9 (1920x1080)");
    draw_proportions.mode = BUTTON_TOGGLE;
    draw_proportions.image = "Gui/Misc/grid_small.img";
    T += grid_align.create(Rect_LU(draw_proportions.rect().ru(), h, h)).focusable(false).desc("Align objects to grid");
    grid_align.mode = BUTTON_TOGGLE;
    grid_align.set(true);
    grid_align.image = "Gui/Misc/grid.img";
    T += op.create(Rect_LU(draw_proportions.rect().ld() + Vec2(0, -p), h * 5, h * 2), 0, (cchar **)null, OP_NUM);
    REPA(op)
    op.tab(i).size(Vec2(h, h)).pos(op.rect().lu());
...
void GuiView::set(Elm *elm) {
    ...
    if (T.elm != elm) {
        ...
        if (elm) {
            GuiSkinPtr temp = Gui.skin;
            if (GuiSkinPtr skin = Proj.appGuiSkin())
                Gui.skin = skin; // set app skin before loading objects because that may affect some of their properties (such as slidebar side button visibility)
...
void GuiView::erasing(C UID &elm_id) {
...
void GuiView::DelTab(ptr user) {
    if (Tabs *tabs = CAST(Tabs, GuiEdit.cur())) {
        ...
        if (InRange(i, *tabs)) {
            ...
            Tab &tab = tabs->tab(i);
            if (GuiObj *parent = tabs->parent())
                REP(tab.childNum()) *parent += *tab.child(i); // move tab children to tabs parent
            tabs->remove(i);
            if (tabs->tabs())
                GuiEdit.createPropsTabs(tabs).toGui();
            else
                GuiEdit.objs.remove(tabs); // if there are no tabs left then remove the object
            GuiEdit.setChanged();
        }
    }
}
...
GuiView &GuiView::createPropsTabs(Tabs *tabs) {
    ...
    if (t >= 1) {
        Button &b = oe.add("Tab 1", MemberDesc(DATA_STR).setFunc(TabsTabText<0>, TabsTabText<0>)).button;
        b.create().func(DelTab, (Ptr)0).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 2) {
        Button &b = oe.add("Tab 2", MemberDesc(DATA_STR).setFunc(TabsTabText<1>, TabsTabText<1>)).button;
        b.create().func(DelTab, (Ptr)1).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 3) {
        Button &b = oe.add("Tab 3", MemberDesc(DATA_STR).setFunc(TabsTabText<2>, TabsTabText<2>)).button;
        b.create().func(DelTab, (Ptr)2).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 4) {
        Button &b = oe.add("Tab 4", MemberDesc(DATA_STR).setFunc(TabsTabText<3>, TabsTabText<3>)).button;
        b.create().func(DelTab, (Ptr)3).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 5) {
        Button &b = oe.add("Tab 5", MemberDesc(DATA_STR).setFunc(TabsTabText<4>, TabsTabText<4>)).button;
        b.create().func(DelTab, (Ptr)4).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 6) {
        Button &b = oe.add("Tab 6", MemberDesc(DATA_STR).setFunc(TabsTabText<5>, TabsTabText<5>)).button;
        b.create().func(DelTab, (Ptr)5).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 7) {
        Button &b = oe.add("Tab 7", MemberDesc(DATA_STR).setFunc(TabsTabText<6>, TabsTabText<6>)).button;
        b.create().func(DelTab, (Ptr)6).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 8) {
        Button &b = oe.add("Tab 8", MemberDesc(DATA_STR).setFunc(TabsTabText<7>, TabsTabText<7>)).button;
        b.create().func(DelTab, (Ptr)7).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 9) {
        Button &b = oe.add("Tab 9", MemberDesc(DATA_STR).setFunc(TabsTabText<8>, TabsTabText<8>)).button;
        b.create().func(DelTab, (Ptr)8).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 10) {
        Button &b = oe.add("Tab 10", MemberDesc(DATA_STR).setFunc(TabsTabText<9>, TabsTabText<9>)).button;
        b.create().func(DelTab, (Ptr)9).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 11) {
        Button &b = oe.add("Tab 11", MemberDesc(DATA_STR).setFunc(TabsTabText<10>, TabsTabText<10>)).button;
        b.create().func(DelTab, (Ptr)10).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 12) {
        Button &b = oe.add("Tab 12", MemberDesc(DATA_STR).setFunc(TabsTabText<11>, TabsTabText<11>)).button;
        b.create().func(DelTab, (Ptr)11).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 13) {
        Button &b = oe.add("Tab 13", MemberDesc(DATA_STR).setFunc(TabsTabText<12>, TabsTabText<12>)).button;
        b.create().func(DelTab, (Ptr)11).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 14) {
        Button &b = oe.add("Tab 14", MemberDesc(DATA_STR).setFunc(TabsTabText<13>, TabsTabText<13>)).button;
        b.create().func(DelTab, (Ptr)11).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 15) {
        Button &b = oe.add("Tab 15", MemberDesc(DATA_STR).setFunc(TabsTabText<14>, TabsTabText<14>)).button;
        b.create().func(DelTab, (Ptr)11).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
    if (t >= 16) {
        Button &b = oe.add("Tab 16", MemberDesc(DATA_STR).setFunc(TabsTabText<15>, TabsTabText<15>)).button;
        b.create().func(DelTab, (Ptr)11).desc("Remove this tab");
        b.image = "Gui/close.img";
    }
...
GuiObj *GuiView::cur(GUI_OBJ_TYPE type) {
    if (GuiObj *obj = cur())
        if (obj->type() == type)
            return obj;
    return null;
}
bool GuiView::selectedParents(GuiObj *obj) {
...
bool GuiView::hasKbFocus() {
    if (editable(Gui.kb()))
        return true;
    if (panel.contains(Gui.kb()) || Gui.kb() == &Mode)
        if (Gui.kb()->type() != GO_TEXTLINE && Gui.kb()->type() != GO_TEXTBOX)
            return true;
    return false;
}
void GuiView::removed(GuiObj *obj) {
    if (obj) {
        sel.exclude(obj, true);
        lit.exclude(obj, true);
        if (obj->contains(sel_parent))
            sel_parent = null;
    }
    REPA(obj_edit)
    if (GuiObj *data = (GuiObj *)obj_edit[i].autoData())
        if (obj->contains(data))
            obj_edit[i].autoData(null).hide();
}
...
void GuiView::alignSel() {
    ...
    if (sel.elms()) {
        REPA(sel) {
            GuiObj &obj = *sel[i];
            Vec2 pos = obj.pos();
            panel.align(pos, true);
            obj.pos(pos);
        }
        setChanged(true);
    }
}
void GuiView::moveUpSel() {
    undo.set("moveUpDown", true);
    if (sel.elms()) {
        sort();
        REPA(sel)
        sel[i]->moveUp();
        for (int i = 0; i < sel.elms() - 1; i++)
            sel[i + 1]->moveAbove(*sel[i]);
        setChanged();
    }
}
void GuiView::moveDownSel() {
    undo.set("moveUpDown", true);
    if (sel.elms()) {
        sort();
        FREPA(sel)
...
void GuiView::resetAspect() {
    ...
    if (sel.elms()) {
        REPA(sel) {
            GuiObj &obj = *sel[i];
            Vec2 size = obj.size();
            if (obj.type() == GO_BUTTON)
                if (C ImagePtr &image = obj.asButton().image)
                    if (image->is())
                        size.y = size.x / image->aspect();
            if (obj.type() == GO_COMBOBOX)
                if (C ImagePtr &image = obj.asComboBox().image)
                    if (image->is())
                        size.y = size.x / image->aspect();
            if (obj.type() == GO_CHECKBOX)
                if (Gui.skin)
                    if (C PanelImagePtr &image = Gui.skin->checkbox.off)
                        if (image->image.is())
                            size.y = size.x / image->image.aspect();
            if (obj.type() == GO_IMAGE)
                if (C ImagePtr &image = obj.asImage().image)
                    if (image->is())
                        size.y = size.x / image->aspect();
            MAX(size.x, MinSize);
            MAX(size.y, MinSize);
...
void GuiView::processSel() {
    SelectionClass::SEL_MODE mode = ((Kb.ctrlCmd() && Kb.shift()) ? SelectionClass::SM_KEEP : Kb.ctrlCmd() ? SelectionClass::SM_TOGGLE
                                                                                          : Kb.shift()     ? SelectionClass::SM_INCLUDE
                                                                                          : Kb.alt()       ? SelectionClass::SM_EXCLUDE
                                                                                                           : SelectionClass::SM_SET);
    if (mode == SelectionClass::SM_KEEP) {
        REPA(sel)
        if (!lit.has(sel[i]))
            sel.remove(i);
    } else {
        if (mode == SelectionClass::SM_SET)
            sel.clear();
        REPA(lit)
        switch (mode) {
        case SelectionClass::SM_INCLUDE:
        case SelectionClass::SM_SET:
            sel.include(lit[i]);
            break;
        case SelectionClass::SM_TOGGLE:
            sel.toggle(lit[i]);
            break;
        case SelectionClass::SM_EXCLUDE:
            sel.exclude(lit[i]);
            break;
        }
    }
...
GuiView &GuiView::toGui() {
    if (GuiObj *obj = cur())
        if (InRange(obj->type(), obj_edit))
            obj_edit[obj->type()].toGui(obj);
    return T;
}
        ...
        if (!obj.contains(parent))
            if (GuiObj *dest = objs.New(obj.type())) {
                *parent += dest->create(obj);
                if (screen_pos) {
                    Vec2 screen = dest->pos(Vec2(0)).screenPos(), pos = *screen_pos - screen;
                    panel.align(pos);
                    dest->pos(pos);
                }
                if (offset)
                ...
                {
                    if (Tabs *dest_tabs = CAST(Tabs, dest))
                        FREP(Min(src_tabs->tabs(), dest_tabs->tabs())) {
                            Tab &src_tab = src_tabs->tab(i),
                                &dest_tab = dest_tabs->tab(i);
                            FREP(src_tab.childNum())
                            copy(*src_tab.child(i), &dest_tab, null, null);
                        }
                } else
                    FREP(obj.childNum())
                copy(*obj.child(i), dest, null, null);
...
void GuiView::update(C GuiPC &gpc) {
    ...
    lit.clear();
    if (gpc.visible && visible()) {
        GuiSkinPtr temp_skin = Gui.skin;
        if (GuiSkinPtr skin = Proj.appGuiSkin())
            Gui.skin = skin; // set skin because that may affect object properties (for example slidebar button visibility, window client rectangle)
            ...
            if (Memx<GuiObj> *container = objs.container(GUI_OBJ_TYPE(i)))
                REPA(*container) {
                    ...
                    if (MainParent(go.parent(), false) == sel_parent && go.visibleFull())
                        if (Cuts(sel_rect, go.screenRect()))
                            lit.include(&go);
                }
        ...
        if (!selecting) {
            if (!disable_selection)
                if (GuiObj *go = MainObject(Gui.ms()))
                    if (editable(go))
        ...
        if (!disable_selection && !selecting)
            if (!Ms.selecting() && (Ms.br(0) || Ms.bd(0)))
                if (editable(Gui.ms())) {
                    disable_selection |= Ms.bd(0);
                    processSel();
                }
        if (!((Ms.b(0) || Ms.br(0)) && editable(Gui.ms())))
            disable_selection = false; // if there are no presses and releases then re-enable selection
        ...
        // add new object
        if (!selecting)
            if (Ms.bp(1) && op == OP_NEW || Kb.kf(KB_INS) && Kb.k.ctrlCmd() && hasKbFocus())
                if (GuiObj *parent = FirstContainer(Gui.ms(), false))
                    if (editable(parent)) {
                        if (parent == Gui.desktop())
                            parent = T.parent(); // don't add to the desktop but to the Mode Gui Tab
                        if (parent) {
                            undo.set("new", true);
        ...
        if (!selecting && sel.elms())
            if (Ms.bp(1) && op == OP_COPY || Kb.kf(KB_D) && Kb.k.ctrlCmd() && hasKbFocus())
                if (GuiObj *parent = FirstContainer(Gui.ms(), true)) // allow tabs so we can copy a group at once to target tab
                    if (editable(parent)) {
                        ...
                        if (parent) {
                            ...
                            temp.clear();
                            FREPA(sel)
                            if (!selectedParents(sel[i]->parent()))
                                temp.add(sel[i]); // add root selected objects to temp (keep order)
                            sel.clear();
                            for (; temp.elms();) {
                                GuiObj *temp_parent = temp.first()->parent();
                                Vec2 pos = temp.first()->pos();
                                REPA(temp)
                                if (temp[i]->parent() == temp_parent) {
                                    Vec2 p = temp[i]->pos();
                                    MIN(pos.x, p.x);
                                    MAX(pos.y, p.y);
                                } // get top-left corner
                                for (int i = 0; i < temp.elms();)
                                    if (temp[i]->parent() == temp_parent) // keep order
                                    {
                                        if (GuiObj *obj = copy(*temp[i], parent, &Ms.pos(), &NoTemp(temp[i]->pos() - pos)))
                                            sel.add(obj);
        ...
        if (hasKbFocus() && Kb.kf(KB_DEL) && Kb.k.ctrlCmd() && sel.elms()) // remove selected on Ctrl+Del
        {
            ...
            temp = sel;
            REPA(temp)
            objs.remove(temp[i]); // copy to 'temp' container because removing changes 'sel'
            Kb.eatKey();
            setChanged();
        }

        // undo
        if (hasKbFocus() && Kb.k.ctrlCmd() && Kb.k(KB_Z)) {
            undo.undo();
            Kb.eatKey();
        }
        if (hasKbFocus() && Kb.k.ctrlCmd() && Kb.k(KB_Y)) {
            undo.redo();
            Kb.eatKey();
        }
        ...
        if (sel.elms()) {
            ...
            if (editable(Gui.ms()) && Ms.b(1) && op == OP_MOVE) // move objects
            {
                ...
                if (!selectedParents(sel[i]->parent()))
                    sel[i]->move(d);
                setChanged(true);
            } else if (editable(Gui.ms()) && Ms.b(1) && op == OP_SIZE) // resize objects
            {
                helper.x += Ms.dc().x;
                helper.y -= Ms.dc().y;
                Vec2 d = helper;
                ...
                helper -= d;
                cchar8 *type = "size";
                if (undo.lastChangeType() == type || d.any()) // this is to prevent creating a dummy Undo level right away after creating new gui objects
                {
                    undo.set(type);
                    REPA(sel) {
                        GuiObj &go = *sel[i];
            ...
            } else if (editable(Gui.ms()) && Ms.b(1) && op == OP_SEPARATE) // separate objects
            {
                undo.set("move");
                helper.x += Ms.dc().x;
                helper.y -= Ms.dc().y;
                ...
                for (temp = sel; temp.elms();) {
                    ...
                    REPA(temp)
                    if (temp[i]->parent() == parent)
                        rect |= temp[i]->rect(); // get rect covering all objects for this parent
                    Vec2 scale = 1;
                    if (rect.size().all()) {
                        scale = (rect.size() + d) / rect.size();
                        MAX(scale.x, 0);
                        MAX(scale.y, 0);
                    }
                    REPA(temp)
                    if (temp[i]->parent() == parent) // go from back because we're removing elements
                    {
                        GuiObj &obj = *temp[i];
                        temp.remove(i); // this object was processed so remove it
                        obj.pos((obj.pos() - rect.lu()) * scale + rect.lu());
                    }
                }
                setChanged(true);
            } else if (editable(Gui.ms()) && Ms.b(1) && op == OP_SCALE) // scale objects
            {
                undo.set("scale");
                helper.x += Ms.dc().x;
                helper.y -= Ms.dc().y;
                ...
                setChanged(true);
            } else if (editable(Gui.ms()) && Ms.b(1) && op == OP_PARENT) // parent objects
            {
                undo.set("move");
                IgnoreSelected = true;
                GuiObj *target = Gui.objAtPos(Ms.pos()); // don't put objects into selected ones
                IgnoreSelected = false;
                target = MainParent(FirstContainer(target, true), true);
                if (!editable(target))
                    target = null;
                if (target == Gui.desktop())
                    target = T.parent(); // don't add to the desktop but to the Mode Gui Tab
                lit.clear();
                if (target)
                    lit.add(MainParent(target, true)); // highlight target for silky smooth user experience
                helper += Ms.dc();
                Vec2 d = helper;
                panel.alignTrunc(d);
                helper -= d;
                sort();
                FREPA(sel)
                if (!selectedParents(sel[i]->parent())) // keep order
                {
                    sel[i]->move(d);
            ...
            if (hasKbFocus() && Kb.k.k) {
                ...
                Vec2 d = D.pixelToScreenSize();
                if (panel.grid_align())
                    d = panel.grid_size;
                Vec2 m(0, 0);
                if (Kb.k(KB_LEFT))
                    m.x -= d.x;
                else if (Kb.k(KB_RIGHT))
                    m.x += d.x;
                ...
                if (op == OP_SIZE) {
                    if (m.any()) {
                        ...
                        REPA(sel) {
                            GuiObj &obj = *sel[i];
                            Vec2 size = obj.size();
                            MAX(size.x += m.x, MinSize);
                            MAX(size.y -= m.y, MinSize);
                            obj.size(size);
                        }
                        setChanged(true);
                    }
        ...
        if (last != cur) {
            last = cur;
            if (cur && cur->type() == GO_TABS)
                createPropsTabs(&cur->asTabs());
...
void GuiView::drawPreview(GuiObj &obj, C GuiPC &gpc, GuiObj &parent) {
    parent += obj;
    Vec2 screen = obj.screenPos(), pos = Ms.pos() - screen;
    panel.align(pos);
    obj.pos(pos);
    parent -= obj;
    obj.move(screen);
...
void GuiView::drawPre() {
    if (GuiSkinPtr temp = Proj.appGuiSkin())
        D.clear(temp->background_color);
    else
...
void GuiView::draw() {
    if (selected()) {
        // highlight objects
        REPA(lit)
        Highlight(*lit[i], Color(128, 255, 255), (op == OP_PARENT || op == OP_COPY || op == OP_NEW) ? 1 : 0.6f);
        REPA(sel)
        Highlight(*sel[i], Color(255, 255, 0), 0.6f);

        // draw preview of object "to-be created"
        GuiPC gpc;
        gpc.clip = gpc.client_rect = Gui.desktop()->rect();
        ...
        if (op == OP_NEW && !Ms.b(1))
            if (GuiObj *parent = FirstContainer(Gui.ms(), false))
                if (editable(parent)) {
                    GuiSkinPtr temp = Gui.skin;
                    if (GuiSkinPtr skin = Proj.appGuiSkin())
                        Gui.skin = skin; // set current skin so object is previewed with correct skin
        ...
        if (panel.draw_proportions()) {
            flt y = 1,
                a = 16.0f / 9;
            Rect(-a, -y, a, y).draw(CYAN, false);
            a = 16.0f / 10;
            Rect(-a, -y, a, y).draw(RED, false);
            a = 4.0f / 3;
            Rect(-a, -y, a, y).draw(GREEN, false);
            a = 5.0f / 4;
            Rect(-a, -y, a, y).draw(BLUE, false);
            a = 1.0f / 1;
            Rect(-a, -y, a, y).draw(PURPLE, false);
            ...
            if (objs.contains(main)) {
                ...
                FREPA(props)                                        // order important (set first property of elm type in case there are many)
                {
                    ...
                    if (Elm *elm = Proj.findElm(elms[i]))
                        if (prop.compatible(elm->type)) // if dropping element of type that matches property type
                        {
                            undo.set(null, true);
