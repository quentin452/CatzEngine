...
void ImageAtlasEditor::ListElm::setColor() {
    color = GuiListTextColor();
    if (removed)
        color.a /= 2;
}
void ImageAtlasEditor::ListElm::operator=(C ElmImageAtlas::Img &src) {
    img_id = src.id;
    removed = src.removed;
    setColor();
}
...
Str ImageAtlasEditor::ElmName(C ListElm &le) {
    if (Elm *elm = Proj.findElm(le.img_id))
        return elm->name;
    return UnknownName;
}
int ImageAtlasEditor::CompareSource(C ImageAtlas::Source &a, C ImageAtlas::Source &b) { return ComparePathNumber(a.name, b.name); }
void ImageAtlasEditor::MipMaps(ImageAtlasEditor &iae, C Str &t) {
    if (ElmImageAtlas *d = iae.data()) {
        iae.undos.set("mms");
...
Str ImageAtlasEditor::MipMaps(C ImageAtlasEditor &iae) {
    if (ElmImageAtlas *d = iae.data())
        return d->mipMaps();
    return S;
}
void ImageAtlasEditor::Compress(ImageAtlasEditor &iae, C Str &t) {
    if (ElmImageAtlas *d = iae.data()) {
        iae.undos.set("cmp");
        d->compress(TextBool(t));
        d->compress_time.getUTC();
        iae.setChanged();
    }
}
Str ImageAtlasEditor::Compress(C ImageAtlasEditor &iae) {
    if (ElmImageAtlas *d = iae.data())
        return d->compress();
    return S;
}
...
void ImageAtlasEditor::makeDo() {
    if (ElmImageAtlas *data = T.data()) {
        ...
        FREPA(data->images) {
            ElmImageAtlas::Img &img = data->images[i];
            if (!img.removed)
                if (Elm *elm = Proj.findElm(img.id)) {
        ...
        if (!atlas.create(source, data->compress() ? (SupportBC7 ? IMAGE_BC7_SRGB : IMAGE_BC3_SRGB) : IMAGE_R8G8B8A8_SRGB, data->mipMaps() ? 0 : 1))
            Gui.msgBox(S, "Error creating Image Atlas");
        else {
            Save(atlas, Proj.gamePath(*elm));
            Proj.savedGame(*elm);
            setChanged(true);
        }
    }
...
void ImageAtlasEditor::create() {
    ...
    super::create("Image Atlas Editor");
    button[1].show();
    ...
    T += undo.create().func(Undo, T).focusable(false).desc("Undo");
    undo.image = "Gui/Misc/undo.img";
    T += redo.create().func(Redo, T).focusable(false).desc("Redo");
    ...
    list.setElmTextColor(MEMBER(ListElm, color));
    list.flag |= LIST_RESIZABLE_COLUMNS;
    FlagDisable(list.flag, LIST_SORTABLE); // disable sorting because of remove buttons
    list.elmHeight(0.043f).textSize(0, 1);
    flt w = 0.02f + region.slidebarSize();
    REPA(lc)
    w += lc[i].width;
    rect(Rect_C(0, 0, w, 1));
    desc("Drag and drop an image here");
}
Rect ImageAtlasEditor::sizeLimit() C {
    Rect r = super::sizeLimit();
    r.min.set(0.85f, 0.4f);
    return r;
}
...
void ImageAtlasEditor::remove(ListElm &le) {
    if (ElmImageAtlas *data = T.data())
        if (ElmImageAtlas::Img *img = data->find(le.img_id)) {
            undos.set(null, true);
            img->removed ^= 1;
            img->removed_time.getUTC();
            setChanged();
        }
}
void ImageAtlasEditor::toGui() {
    ...
    if (ElmImageAtlas *data = T.data())
        FREPA(data->images)
    if (!data->images[i].removed || show_removed()) {
        ListElm &le = list_data.New();
        le = data->images[i];
        ...
        if (data->images[i].removed)
            ...
            le.remove.image = "Gui/close.img";
    }
    list_data.sort(ListElm::ComparePath);
    FREPA(list_data)
    region += list_data[i].remove.rect(Rect_LU(0, -(v++) * h - list.columnHeight(), h, h));
    list.setData(list_data);
}
void ImageAtlasEditor::set(Elm *elm) {
    if (elm && elm->type != ELM_IMAGE_ATLAS)
        elm = null;
    if (T.elm != elm) {
        flush();
        undos.del();
...
void ImageAtlasEditor::activate(Elm *elm) {
    set(elm);
    if (T.elm)
        super::activate();
}
void ImageAtlasEditor::toggle(Elm *elm) {
    if (elm == T.elm)
        elm = null;
    set(elm);
}
ImageAtlasEditor &ImageAtlasEditor::hide() {
    if (visible()) {
        super::hide();
        set(null);
    }
...
void ImageAtlasEditor::drag(Memc<UID> &elms, GuiObj *obj, C Vec2 &screen_pos) {
    if (elm && contains(obj))
        REPA(elms)
    if (Elm *image = Proj.findElm(elms[i]))
        if (ElmImageLike(image->type)) // add image element to the atlas
        {
...
void ImageAtlasEditor::erasing(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        set(null);
}
