    ...
    if (gpc.visible && visible()) {
        ...
        if (image) {
            ...
            if (image->mode() == IMAGE_CUBE) {
                int clouds = Clouds.layered.layers();
                Clouds.layered.set(0);
        ...
        if (ImageEdit.elm && ImageEdit.elm->importing()) {
            TextStyleParams ts;
            ts.size *= 0.6f;
            ts.align.set(0, 1);
            D.text(ts, rect.down(), "Reloading");
        }
    }
}
...
void ImageEditor::Type(ImageEditor &ie, C Str &t) {
    if (ElmImage *d = ie.data()) {
        int i = TextInt(t);
        if (InRange(i, ElmImage::NUM))
            d->type = ElmImage::TYPE(i);
        d->type_time.getUTC();
    }
}
Str ImageEditor::Type(C ImageEditor &ie) {
    if (ElmImage *d = ie.data())
        return d->type;
    return -1;
}
void ImageEditor::Mode(ImageEditor &ie, C Str &t) {
    if (ElmImage *d = ie.data()) {
        int i = TextInt(t);
        if (InRange(i, ImageModes)) {
            d->mode = ImageModes[i].mode;
            d->mode_time.getUTC();
            d->env(ImageModes[i].env);
            d->env_time.getUTC();
        }
    }
}
Str ImageEditor::Mode(C ImageEditor &ie) {
    if (ElmImage *d = ie.data())
        REPA(ImageModes)
    if (d->mode == ImageModes[i].mode && d->envActual() == ImageModes[i].env)
        return i;
    return -1;
}
void ImageEditor::MipMaps(ImageEditor &ie, C Str &t) {
    if (ElmImage *d = ie.data()) {
        d->mipMaps(TextBool(t));
        d->mip_maps_time.getUTC();
    }
}
Str ImageEditor::MipMaps(C ImageEditor &ie) {
    if (ElmImage *d = ie.data())
        return d->mipMaps();
    return S;
}
void ImageEditor::Pow2(ImageEditor &ie, C Str &t) {
    if (ElmImage *d = ie.data()) {
        d->pow2(TextBool(t));
        d->pow2_time.getUTC();
    }
}
Str ImageEditor::Pow2(C ImageEditor &ie) {
    if (ElmImage *d = ie.data())
        return d->pow2();
    return S;
}
void ImageEditor::SRGB(ImageEditor &ie, C Str &t) {
    if (ElmImage *d = ie.data()) {
        d->sRGB(TextBool(t));
        d->srgb_time.getUTC();
    }
}
Str ImageEditor::SRGB(C ImageEditor &ie) {
    if (ElmImage *d = ie.data())
        return d->sRGB();
    return S;
}
void ImageEditor::Width(ImageEditor &ie, C Str &t) {
    if (ElmImage *d = ie.data()) {
        d->size.x = TextInt(t);
        d->size_time.getUTC();
    }
}
Str ImageEditor::Width(C ImageEditor &ie) {
    if (ElmImage *d = ie.data())
        return d->size.x;
    return S;
}
void ImageEditor::Height(ImageEditor &ie, C Str &t) {
    if (ElmImage *d = ie.data()) {
        d->size.y = TextInt(t);
        d->size_time.getUTC();
    }
}
Str ImageEditor::Height(C ImageEditor &ie) {
    if (ElmImage *d = ie.data())
        return d->size.y;
    return S;
}
void ImageEditor::AlphaLum(ImageEditor &ie, C Str &t) {
    if (ElmImage *d = ie.data()) {
        d->alphaLum(TextBool(t));
        d->alpha_lum_time.getUTC();
    }
}
Str ImageEditor::AlphaLum(C ImageEditor &ie) {
    if (ElmImage *d = ie.data())
        return d->alphaLum();
    return S;
}
...
void ImageEditor::setMipMap(bool on) {
    undos.set("mipMap");
    MipMaps(T, on);
    setChanged();
    toGui();
}
void ImageEditor::setSize(C VecI2 &size) {
    if (ElmImage *d = data()) {
        undos.set("size");
        d->size = size;
        d->size_time.getUTC();
...
void ImageEditor::create() {
    ...
    flt h = 0.043f;
    super::create("Image Editor", Vec2(0.02f, -0.07f), 0.036f, h, 0.28f);
    super::changed(Changed, PreChanged);
    chn->changed(null, null);
    nos->changed(null, null);
    src->changed(null, null);
    button[1].show();
...
void ImageEditor::make2D() {
    gui_image.image = ((data() && data()->mode == IMAGE_SOFT) ? &image_2d : game_image);
    REPAO(cube_faces).image = gui_image.image;
    image_2d.del();
    if (gui_image.image == &image_2d && game_image)
        game_image->copy(image_2d, -1, -1, 1, IMAGE_R8G8B8A8_SRGB, IMAGE_2D);
}
void ImageEditor::setInfo() {
    if (mip_maps)
        mip_maps->visible(!data() || !data()->envActual());
    if (width)
        width->display(S + "Width: " + (game_image ? game_image->w() : 0));
    if (height)
        height->display(S + "Height: " + (game_image ? game_image->h() : 0));
    if (type)
        type->display(S + "Type: " + (game_image ? game_image->typeInfo().name : null));
    if (i_mip_maps)
        i_mip_maps->display(S + "Mip Maps: " + (game_image ? game_image->mipMaps() : 0));
    if (mem_size)
        mem_size->display(S + "Size: " + SizeBytes(game_image ? game_image->typeMemUsage() : 0)); // use 'typeMemUsage' because we need this only for stats
    if (chn)
        chn->visible(data() && !IsCube(data()->mode));
    if (nos)
        nos->visible(data() && !IsCube(data()->mode));
    if (src)
        src->visible(data() && IsCube(data()->mode));
...
ImageEditor &ImageEditor::hide() {
    set(null);
    super::hide();
    return T;
}
Rect ImageEditor::sizeLimit() C {
    Rect r = super::sizeLimit();
    r.min.set(0.7f, 0.26f);
    return r;
}
...
void ImageEditor::flush() {
    if (elm && changed) {
        ...
        if (game_image) {
            Save(*game_image, Proj.gamePath(*elm));
            Proj.savedGame(*elm);
        }
...
void ImageEditor::setChanged() {
    if (elm) {
        ...
        if (ElmImage *data = T.data()) {
            data->newVer();
            if (!edit_image.is())
                edit_image.Import(Proj.editPath(*elm));
            if (game_image)
                EditToGameImage(edit_image, *game_image, *data);
            make2D();
        }
    }
}
void ImageEditor::set(Elm *elm) {
    if (elm && elm->type != ELM_IMAGE)
        elm = null;
    if (T.elm != elm) {
        flush();
        undos.del();
...
void ImageEditor::activate(Elm *elm) {
    set(elm);
    if (T.elm)
        super::activate();
}
void ImageEditor::toggle(Elm *elm) {
    if (elm == T.elm)
        elm = null;
    set(elm);
}
void ImageEditor::elmChanged(C UID &elm_id) {
    if (elm && elm->id == elm_id) {
        ...
        toGui();
        make2D();
        setInfo(); // delete 'edit_image' so it will be reloaded when needed
    }
}
void ImageEditor::ImageName::set(C Str &name, C Str &file) {
    T.name = name;
    T.file = file;
}
int ImageEditor::FaceToIndex(DIR_ENUM dir) {
    REPA(cube_faces)
    if (cube_faces[i].face == dir)
        return i;
    return -1;
}
            ...
            if (IsCube(d->mode)) {
                if (images.elms() > 1) // multi
                {
                    ...
                    REPA(images) {
                        ...
                        if (Contains(name, "front") || Contains(name, "forward"))
                            dir = DIR_FORWARD;
                        else if (Contains(name, "back"))
                            dir = DIR_BACK;
                        else if (Contains(name, "top") || Contains(name, "up"))
                            dir = DIR_UP;
                        else if (Contains(name, "bottom") || Contains(name, "down"))
                            dir = DIR_DOWN;
                        else if (Contains(name, "left"))
                            dir = DIR_LEFT;
                        else if (Contains(name, "right"))
                            dir = DIR_RIGHT;
                        import = SetCubeFile(import, FaceToIndex(dir), images[i].file);
                    }
                } else
                    REPA(cube_faces)
                if (cube_faces[i].contains(obj)) // check if cube face is selected
                {
                    import = SetCubeFile(elm->srcFile(), i, images[0].file);
                    break;
                }
            }
            elm->setSrcFile(import);
            Server.setElmShort(elm_id);
            Proj.elmReload(elm_id);
        }
}
void ImageEditor::drop(Memc<Str> &names, GuiObj *obj, C Vec2 &screen_pos) {
    if (contains(obj)) {
        ...
        if (ExtType(GetExt(names[i])) == EXT_IMAGE)
            images.New().set(GetBaseNoExt(names[i]), names[i]);
        setImage(images, obj);
    }
}
void ImageEditor::drag(Memc<UID> &elms, GuiObj *obj, C Vec2 &screen_pos) {
    if (contains(obj)) {
        Memc<ImageName> images;
        REPA(elms)
        if (Elm *image = Proj.findElm(elms[i], ELM_IMAGE))
            if (elm != image)
                images.New().set(image->name, EncodeFileName(image->id));
        setImage(images, obj);
    }
}
void ImageEditor::erasing(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        set(null);
}
