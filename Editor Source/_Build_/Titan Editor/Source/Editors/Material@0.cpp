    ...
    if (mr) {
        ...
        if (!mr->water())
            ...
            case TEX_COLOR:
                ...
                break;
            case TEX_ALPHA:
                if (em.color_map.is() || em.alpha_map.is())
                    return base_0;
                break;
            case TEX_BUMP:
                if (em.hasBumpMap())
            ...
            case TEX_MACRO:
                ...
                break;
            case TEX_DET_COLOR:
                if (em.detail_color.is())
                    return detail;
                break; // #MaterialTextureLayoutDetail
            case TEX_DET_BUMP:
                if (em.detail_bump.is())
                    return detail;
                break;
            case TEX_DET_NORMAL:
                if (em.detail_normal.is() || em.detail_bump.is())
                    return detail;
                break;
            case TEX_DET_SMOOTH:
                if (em.detail_smooth.is())
                    return detail;
                break;
            }
        else
            switch (type) // #MaterialTextureLayoutWater
            {
            case TEX_COLOR:
                if (em.color_map.is())
                    return base_0;
...
void MaterialRegion::Texture::ReplaceElmNames(Mems<FileParams> &files) {
    ...
    REPA(files) {
        ...
        if (DecodeFileName(fp.name, image_id))
            fp.name = Proj.elmFullName(image_id);
        ReplaceElmNames(fp.nodes);
    }
}
void MaterialRegion::Texture::setDesc() {
    Mems<FileParams> files = FileParams::Decode(file);
    ReplaceElmNames(files);
    Str desc = Replace(text, '\n', ' ');
    if (C ImagePtr &image = getImage())
        desc += S + ", " + image->w() + 'x' + image->h();
    if (type == TEX_MACRO)
        desc.line() += "Can be set for heightmap materials to decrease repetitiveness of textures.\nBecomes visible at distance of around 100 meters.";
    FREPA(files) {
        desc += '\n';
        desc += files[i].encode();
    }
...
void MaterialRegion::Texture::FixPath(Mems<FileParams> &fps) {
    REPA(fps) {
        FileParams &fp = fps[i];
        if (Contains(fp.name, '|'))
            fp.name = GetBaseNoExt(fp.name); // we want to support things like "|bump|" but when entering that into 'WindowIO', it may append the path, so when this command is detected, remove the path (and possible extension too)
        FixPath(fp.nodes);
    }
}
void MaterialRegion::Texture::setFile(Str file, bool set_undo) {
    ...
    if (mr) {
        ...
        if (!file.is()) // if removing texture
        {
            ...
            if (name.is())
                REPA(mr->texs) {
                    Texture &tex = mr->texs[i];
                    if (tex.type >= TEX_BASE_BEGIN && tex.type <= TEX_BASE_END) // iterate all base textures
                    {
        ...
        if (type >= TEX_BASE_BEGIN && type <= TEX_BASE_END)
            mr->rebuildBase(textures, 1 << type);
        else if (type >= TEX_DET_BEGIN && type <= TEX_DET_END)
            mr->rebuildDetail();
        else if (type == TEX_MACRO)
            mr->rebuildMacro();
        else if (type == TEX_EMISSIVE)
            mr->rebuildEmissive(textures);
        else
            mr->setChanged();
...
::MaterialRegion::Texture &MaterialRegion::Texture::create(TEX_TYPE type, C MemberDesc &md_file, C MemberDesc &md_time, Rect rect, C Str &text, MaterialRegion &mr) {
    ...
    T.md_file = md_file;
    T.md_time = md_time;
    super::create().rect(rect.extend(-0.003f));
    T.text = text;
    win_io.create().ext(SUPPORTED_IMAGE_EXT, Replace(text, '\n', ' ')).io(Load, Load, T);
    remove.create(Rect_RU(rect.ru(), 0.035f, 0.035f)).func(Remove, T);
...
bool MaterialRegion::Texture::ExploreFiles(C Mems<FileParams> &fps) {
    FREPA(fps) {
        ...
        if (fp.name.is()) {
            UID id;
            if (id.fromFileName(fp.name))
                Proj.elmLocate(id, true);
            else
                Explore(FFirstUp(fp.name));
            return true;
        }
        if (ExploreFiles(fp.nodes))
            return true;
    }
    return false;
}
    ...
    if (gpc.visible && visible()) {
        ...
        if (MT.bp(i) && MT.guiObj(i) == this) {
            if (file.is()) // texture path has anything
            {
                ...
                Str mtrl_path = Proj.elmSrcFileFirst(mr->elm);
                if (FileInfoSystem(mtrl_path).type == FSTD_FILE)
                    mtrl_path = GetPath(mtrl_path);
                Str color_path;
                if (fps.elms())
                    color_path = FFirstUp(GetPath(fps[0].name));
    ...
    if (mr) {
        ...
        if (!mr->water())
            ...
            case TEX_ALPHA:
                if (em.color_map.is() || em.alpha_map.is())
                    if (base_0) {
                        VI.shader(ShaderFiles("Main")->get("DrawTexWG"));
                        base_0->drawFit(rect);
                        tex = true;
                    }
                break;
            case TEX_BUMP:
                if (em.hasBumpMap())
                    if (base_2) {
                        VI.shader(ShaderFiles("Main")->get("DrawTexZG"));
                        base_2->drawFit(rect);
                        tex = true;
                    }
                break;
            case TEX_NORMAL:
                if (em.hasNormalMap())
                    if (base_1) {
                        VI.shader(ShaderFiles("Main")->get("DrawTexNrm"));
                        base_1->drawFit(rect);
                        tex = true;
                    }
                break;
            case TEX_SMOOTH:
                if (em.smooth_map.is())
                    if (base_2) {
                        VI.shader(ShaderFiles("Main")->get(em.smooth_is_rough ? "DrawTexYG" : "DrawTexYIG"));
                        base_2->drawFit(rect);
                        tex = true;
                    }
                break;
            case TEX_METAL:
                if (em.metal_map.is())
                    if (base_2) {
                        VI.shader(ShaderFiles("Main")->get("DrawTexXG"));
                        base_2->drawFit(rect);
                        tex = true;
                    }
                break;
            ...
            case TEX_DET_BUMP:
                if (em.detail_bump.is())
                    if (detail) {
                        if (Image *bump = mr->getDetailBump(em.detail_bump))
                            bump->drawFit(rect);
                        tex = true;
                    }
                break; // Detail Bump is not stored in texture
            case TEX_DET_NORMAL:
                if (em.detail_normal.is() || em.detail_bump.is())
                    if (detail) {
                        VI.shader(ShaderFiles("Main")->get("DrawTexDetNrm"));
                        detail->drawFit(rect);
                        tex = true;
                    }
                break;
            case TEX_DET_SMOOTH:
                if (em.detail_smooth.is())
                    if (detail) {
                        VI.shader(ShaderFiles("Main")->get("DrawTexZIG"));
                        detail->drawFit(rect);
                        tex = true;
                    }
                break; // inverse because it's roughness
            ...
            case TEX_BUMP:
                if (em.hasBumpMap())
                    if (base_2) {
                        VI.shader(ShaderFiles("Main")->get("DrawTexXSG"));
                        base_2->drawFit(rect);
                        tex = true;
                    }
                break;
...
void MaterialRegion::Texture::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        ...
        TextStyleParams ts;
        if (draw(r))
            ts.reset(false).size = 0.040f;
        else
            ts.reset(true).size = 0.036f;
        D.text(ts, r, text);

        // if mouse cursor is over reload button, and source file is not found, then draw exclamation as a warning
        if ((Gui.ms() == &mr->reload_base_textures || Gui.ms() == &mr->texture_options) && Proj.invalidTexSrc(file))
...
void MaterialRegion::render() {
    ...
    case RM_PREPARE: {
        if (InRange(preview_mode(), preview_mesh)) {
            Matrix m;
            m.setScale((game && game->technique == MTECH_FUR) ? 0.75f : 1.0f);
...
void MaterialRegion::PreChanged(C Property &prop) {
    cptr change_type = &prop;
    if (change_type == MtrlEdit.green || change_type == MtrlEdit.blue)
        change_type = MtrlEdit.red;
    MtrlEdit.undos.set(change_type);
}
void MaterialRegion::Changed(C Property &prop) { MtrlEdit.setChanged(); }
Str MaterialRegion::Tech(C MaterialRegion &mr) {
    REPA(mtrl_techs)
    if (mtrl_techs[i].tech == mr.edit.tech)
        return i;
    return S;
}
void MaterialRegion::Tech(MaterialRegion &mr, C Str &t) {
    int i = TextInt(t);
    if (InRange(i, mtrl_techs)) {
        mr.edit.tech = mtrl_techs[i].tech;
        mr.edit.tech_time.now();
        mr.setChanged();
        D.setShader(mr.game());
    }
}
Str MaterialRegion::TexDownsizeMobile(C MaterialRegion &mr) { return mr.edit.tex_downsize[TSP_MOBILE]; }
void MaterialRegion::TexDownsizeMobile(MaterialRegion &mr, C Str &t) {
    mr.edit.tex_downsize[TSP_MOBILE] = TextInt(t);
    mr.edit.tex_downsize_time.getUTC();
}
Str MaterialRegion::TexDownsizeSwitch(C MaterialRegion &mr) { return mr.edit.tex_downsize[TSP_SWITCH]; }
void MaterialRegion::TexDownsizeSwitch(MaterialRegion &mr, C Str &t) {
    mr.edit.tex_downsize[TSP_SWITCH] = TextInt(t);
    mr.edit.tex_downsize_time.getUTC();
}
Str MaterialRegion::TexQuality(C MaterialRegion &mr) {
    REPA(TexQualities)
    if (TexQualities[i].quality == mr.edit.tex_quality)
        return i;
    return S;
}
void MaterialRegion::TexQuality(MaterialRegion &mr, C Str &t) {
    int i = TextInt(t);
    if (InRange(i, TexQualities))
        mr.texQuality(TexQualities[i].quality, false);
}
void MaterialRegion::RGB1(MaterialRegion &mr) {
    mr.undos.set("rgb1");
    mr.edit.color_s.xyz = 1;
    mr.edit.color_time.getUTC();
    mr.setChanged();
    mr.toGui();
}
void MaterialRegion::RGB(MaterialRegion &mr) {
    ...
    int on = 0, pd = 0;
    REPA(MT)
    if (MT.b(i) && MT.guiObj(i) == &mr.brightness) {
        d += MT.ad(i);
        MT.freeze(i);
        if (MT.bp(i))
            pd++;
    ...
    Vec &rgb = mr.edit.color_s.xyz;
    if (pd && !on) {
        mr.mouse_edit_value = rgb;
        mr.mouse_edit_delta = 0;
    }
    flt d_sum = d.sum();
    if (mr.red)
        d_sum *= mr.red->mouse_edit_speed;
    mr.mouse_edit_delta += d_sum;
    flt max = mr.mouse_edit_value.max(), lum = max + mr.mouse_edit_delta;
    if (mr.red) {
        if (mr.red->min_use) {
            flt min = mr.red->min_value;
            if (lum < min) {
                mr.mouse_edit_delta -= lum - min;
                lum = min;
            }
        }
        if (mr.red->max_use) {
            flt max = mr.red->max_value;
            if (lum > max) {
                mr.mouse_edit_delta -= lum - max;
                lum = max;
            }
        }
    }
    Vec v = mr.mouse_edit_value;
    if (max)
        v /= max;
    else
        v = 1;
    v *= lum;
    if (mr.red) {
        mr.red->set(v.x, QUIET);
        rgb.x = mr.red->asFlt();
    }
    if (mr.green) {
        mr.green->set(v.y, QUIET);
        rgb.y = mr.green->asFlt();
    }
    if (mr.blue) {
        mr.blue->set(v.z, QUIET);
        rgb.z = mr.blue->asFlt();
    }
    mr.edit.color_time.getUTC();
    mr.setChanged();
...
void MaterialRegion::Emissive(MaterialRegion &mr) {
    ...
    int on = 0, pd = 0;
    REPA(MT)
    if (MT.b(i) && MT.guiObj(i) == &mr.emissive) {
        d += MT.ad(i);
        MT.freeze(i);
        if (MT.bp(i))
            pd++;
    ...
    flt d_sum = d.sum();
    if (mr.emit_red)
        d_sum *= mr.emit_red->mouse_edit_speed;
    mr.mouse_edit_delta += d_sum;
    flt max = mr.mouse_edit_value.max(), lum = max + mr.mouse_edit_delta;
    if (mr.emit_red) {
        if (mr.emit_red->min_use) {
            flt min = mr.emit_red->min_value;
            if (lum < min) {
                mr.mouse_edit_delta -= lum - min;
                lum = min;
            }
        }
        if (mr.emit_red->max_use) {
            flt max = mr.emit_red->max_value;
            if (lum > max) {
                mr.mouse_edit_delta -= lum - max;
                lum = max;
            }
    ...
    v *= lum;
    if (mr.emit_red) {
        mr.emit_red->set(v.x, QUIET);
        rgb.x = mr.emit_red->asFlt();
    }
    if (mr.emit_green) {
        mr.emit_green->set(v.y, QUIET);
        rgb.y = mr.emit_green->asFlt();
    }
    if (mr.emit_blue) {
        mr.emit_blue->set(v.z, QUIET);
        rgb.z = mr.emit_blue->asFlt();
    }
    mr.edit.emissive_time.getUTC();
    mr.setChanged();
    D.setShader(mr.game());
}
Str MaterialRegion::Red(C MaterialRegion &mr) { return mr.edit.color_s.x; }
void MaterialRegion::Red(MaterialRegion &mr, C Str &t) {
    mr.edit.color_s.x = TextFlt(t);
    mr.edit.color_time.getUTC();
}
Str MaterialRegion::Green(C MaterialRegion &mr) { return mr.edit.color_s.y; }
void MaterialRegion::Green(MaterialRegion &mr, C Str &t) {
    mr.edit.color_s.y = TextFlt(t);
    mr.edit.color_time.getUTC();
}
Str MaterialRegion::Blue(C MaterialRegion &mr) { return mr.edit.color_s.z; }
void MaterialRegion::Blue(MaterialRegion &mr, C Str &t) {
    mr.edit.color_s.z = TextFlt(t);
    mr.edit.color_time.getUTC();
}
Str MaterialRegion::Alpha(C MaterialRegion &mr) { return mr.edit.color_s.w; }
void MaterialRegion::Alpha(MaterialRegion &mr, C Str &t) {
    mr.edit.color_s.w = TextFlt(t);
    mr.edit.color_time.getUTC();
}
Str MaterialRegion::Bump(C MaterialRegion &mr) { return mr.edit.bump / BumpScale; }
void MaterialRegion::Bump(MaterialRegion &mr, C Str &t) {
    mr.edit.bump = TextFlt(t) * BumpScale;
    mr.edit.bump_time.getUTC();
    mr.setChanged();
    D.setShader(mr.game());
}
Str MaterialRegion::NrmScale(C MaterialRegion &mr) { return mr.edit.normal; }
void MaterialRegion::NrmScale(MaterialRegion &mr, C Str &t) {
    mr.edit.normal = TextFlt(t);
    mr.edit.normal_time.getUTC();
    mr.setChanged();
    D.setShader(mr.game());
}
Str MaterialRegion::FNY(C MaterialRegion &mr) { return mr.edit.flip_normal_y; }
void MaterialRegion::FNY(MaterialRegion &mr, C Str &t) {
    TEX_FLAG textures = mr.edit.textures();
    mr.edit.flip_normal_y = TextBool(t);
    mr.edit.flip_normal_y_time.getUTC();
    mr.rebuildBase(textures, EditMaterial::CHANGED_FLIP_NRM_Y, false);
}
Str MaterialRegion::RoughImage(C MaterialRegion &mr) { return mr.edit.smooth_is_rough; }
void MaterialRegion::RoughImage(MaterialRegion &mr, C Str &t) {
    TEX_FLAG textures = mr.edit.textures();
    mr.edit.smooth_is_rough = TextBool(t);
    mr.edit.smooth_is_rough_time.getUTC();
    mr.rebuildBase(textures, EditMaterial::CHANGED_SMOOTH_IS_ROUGH, false);
}
Str MaterialRegion::Smooth(C MaterialRegion &mr) { return mr.edit.smooth; }
void MaterialRegion::Smooth(MaterialRegion &mr, C Str &t) {
    mr.edit.smooth = TextFlt(t);
    mr.edit.smooth_time.getUTC();
}
Str MaterialRegion::ReflectMin(C MaterialRegion &mr) { return mr.edit.reflect_min; }
void MaterialRegion::ReflectMin(MaterialRegion &mr, C Str &t) {
    mr.edit.reflect_min = TextFlt(t);
    mr.edit.reflect_time.getUTC();
}
Str MaterialRegion::ReflectMax(C MaterialRegion &mr) { return mr.edit.reflect_max; }
void MaterialRegion::ReflectMax(MaterialRegion &mr, C Str &t) {
    mr.edit.reflect_max = TextFlt(t);
    mr.edit.reflect_time.getUTC();
}
Str MaterialRegion::Glow(C MaterialRegion &mr) { return mr.edit.glow; }
void MaterialRegion::Glow(MaterialRegion &mr, C Str &t) {
    mr.edit.glow = TextFlt(t);
    mr.edit.glow_time.getUTC();
}
Str MaterialRegion::DetUVScale(C MaterialRegion &mr) { return mr.edit.det_uv_scale; }
void MaterialRegion::DetUVScale(MaterialRegion &mr, C Str &t) {
    mr.edit.det_uv_scale = TextFlt(t);
    mr.edit.detail_time.getUTC();
}
Str MaterialRegion::DetPower(C MaterialRegion &mr) { return mr.edit.det_power; }
void MaterialRegion::DetPower(MaterialRegion &mr, C Str &t) {
    mr.edit.det_power = TextFlt(t);
    mr.edit.detail_time.getUTC();
    mr.setChanged();
    D.setShader(mr.game());
}
Str MaterialRegion::DetLOD(C MaterialRegion &mr) { return mr.edit.detail_all_lod; }
void MaterialRegion::DetLOD(MaterialRegion &mr, C Str &t) {
    mr.edit.detail_all_lod = TextBool(t);
    mr.edit.detail_all_lod_time.now();
    mr.setChanged();
    D.setShader(mr.game());
}
Str MaterialRegion::Cull(C MaterialRegion &mr) { return mr.edit.cull; }
void MaterialRegion::Cull(MaterialRegion &mr, C Str &t) {
    mr.edit.cull = TextBool(t);
    mr.edit.cull_time.now();
}
Str MaterialRegion::EmissiveR(C MaterialRegion &mr) { return mr.edit.emissive_s.x; }
void MaterialRegion::EmissiveR(MaterialRegion &mr, C Str &t) {
    mr.edit.emissive_s.x = TextFlt(t);
    mr.edit.emissive_time.getUTC();
    mr.setChanged();
    D.setShader(mr.game());
}
Str MaterialRegion::EmissiveG(C MaterialRegion &mr) { return mr.edit.emissive_s.y; }
void MaterialRegion::EmissiveG(MaterialRegion &mr, C Str &t) {
    mr.edit.emissive_s.y = TextFlt(t);
    mr.edit.emissive_time.getUTC();
    mr.setChanged();
    D.setShader(mr.game());
}
Str MaterialRegion::EmissiveB(C MaterialRegion &mr) { return mr.edit.emissive_s.z; }
void MaterialRegion::EmissiveB(MaterialRegion &mr, C Str &t) {
    mr.edit.emissive_s.z = TextFlt(t);
    mr.edit.emissive_time.getUTC();
    mr.setChanged();
    D.setShader(mr.game());
}
Str MaterialRegion::EmissiveGlow(C MaterialRegion &mr) { return mr.edit.emissive_glow; }
void MaterialRegion::EmissiveGlow(MaterialRegion &mr, C Str &t) {
    mr.edit.emissive_glow = TextFlt(t);
    mr.edit.emissive_time.getUTC();
    mr.setChanged();
    D.setShader(mr.game());
}
Str MaterialRegion::UVScale(C MaterialRegion &mr) { return mr.edit.uv_scale; }
void MaterialRegion::UVScale(MaterialRegion &mr, C Str &t) {
    mr.edit.uv_scale = TextFlt(t);
    mr.edit.uv_scale_time.getUTC();
    mr.setChanged();
    D.setShader(mr.game());
}
...
void MaterialRegion::ReloadBaseTextures(MaterialRegion &editor) {
    editor.undos.set("rebuildBase");
    editor.rebuildBase(editor.getEditMtrl().textures(), 0, false, true);
}
...
void MaterialRegion::setRGB(C Vec &srgb) {
    if (edit.color_s.xyz != srgb) {
        undos.set("rgb");
        edit.color_s.xyz = srgb;
        edit.color_time.getUTC();
        setChanged();
        toGui();
    }
}
void MaterialRegion::setNormal(flt normal) {
    if (edit.normal != normal) {
        undos.set("normal");
        edit.normal = normal;
        edit.normal_time.getUTC();
        setChanged();
        toGui();
    }
}
void MaterialRegion::setSmooth(flt smooth) {
    if (edit.smooth != smooth) {
        undos.set("smooth");
        edit.smooth = smooth;
        edit.smooth_time.getUTC();
        setChanged();
        toGui();
    }
}
void MaterialRegion::setReflect(flt reflect_min, flt reflect_max) {
    if (edit.reflect_min != reflect_min || edit.reflect_max != reflect_max) {
        undos.set("reflect");
        edit.reflect_min = reflect_min;
        edit.reflect_max = reflect_max;
        edit.reflect_time.getUTC();
        setChanged();
        toGui();
    }
}
void MaterialRegion::resetAlpha() {
    undos.set("alpha");
    edit.resetAlpha();
    setChanged();
    toGui();
}
void MaterialRegion::cull(bool on) {
    if (edit.cull != on) {
        undos.set("cull");
        edit.cull = on;
        edit.cull_time.getUTC();
        setChanged();
        toGui();
    }
}
void MaterialRegion::flipNrmY(bool on) {
    if (edit.flip_normal_y != on) {
        undos.set("fny");
        edit.flip_normal_y = on;
        edit.flip_normal_y_time.getUTC();
        rebuildBase(edit.textures(), EditMaterial::CHANGED_FLIP_NRM_Y, false);
    }
}
void MaterialRegion::smoothIsRough(bool on) {
    if (edit.smooth_is_rough != on) {
        undos.set("sir");
        edit.smooth_is_rough = on;
        edit.smooth_is_rough_time.getUTC();
        rebuildBase(edit.textures(), EditMaterial::CHANGED_SMOOTH_IS_ROUGH, false);
    }
}
void MaterialRegion::texDownsize(TEX_SIZE_PLATFORM tsp, byte downsize) {
    if (edit.tex_downsize[tsp] != downsize) {
        undos.set("texSize");
        edit.tex_downsize[tsp] = downsize;
        edit.tex_downsize_time.getUTC();
        setChanged();
        toGui();
    }
}
void MaterialRegion::texQuality(Edit::Material::TEX_QUALITY q, bool undo) {
    if (edit.tex_quality != q) {
        if (undo)
            undos.set("texQuality");
        edit.tex_quality = q;
        edit.tex_quality_time.getUTC();
        rebuildBase(edit.textures(), 0, false);
    }
}
void MaterialRegion::resizeBase(C VecI2 &size, bool relative) {
    ...
    if (relative && size.any())                                                         // if we want to have relative size and not original, then first revert to original size
        if (Proj.forceImageSize(edit.color_map, 0, relative, edit.color_map_time, time) // !! use '|' because all need to be processed !!
            | Proj.forceImageSize(edit.alpha_map, 0, relative, edit.alpha_map_time, time) | Proj.forceImageSize(edit.bump_map, 0, relative, edit.bump_map_time, time) | Proj.forceImageSize(edit.normal_map, 0, relative, edit.normal_map_time, time) | Proj.forceImageSize(edit.smooth_map, 0, relative, edit.smooth_map_time, time) | Proj.forceImageSize(edit.metal_map, 0, relative, edit.metal_map_time, time) | Proj.forceImageSize(edit.glow_map, 0, relative, edit.glow_map_time, time)) {
            MtrlImages mi;
            mi.fromMaterial(edit, Proj);
            mi.baseTextureSizes(&sizes[0], &sizes[1], &sizes[2]); // calculate actual sizes
            REPA(sizes) {
            ...
            if (sizes[1] != sizes[2])
                edit.separateNormalMap(time); // normal can be from bump
    ...
    if (Proj.forceImageSize(edit.color_map, sizes[0], relative, edit.color_map_time, time) // !! use '|' because all need to be processed !!
        | Proj.forceImageSize(edit.alpha_map, sizes[0], relative, edit.alpha_map_time, time) | Proj.forceImageSize(edit.bump_map, sizes[2], relative, edit.bump_map_time, time) | Proj.forceImageSize(edit.normal_map, sizes[1], relative, edit.normal_map_time, time) | Proj.forceImageSize(edit.smooth_map, sizes[2], relative, edit.smooth_map_time, time) | Proj.forceImageSize(edit.metal_map, sizes[2], relative, edit.metal_map_time, time) | Proj.forceImageSize(edit.glow_map, sizes[2], relative, edit.glow_map_time, time)) {
        edit.cleanupMaps();
...
void MaterialRegion::resizeBase0(C VecI2 &size, bool relative) {
    ...
    if (relative && size.any())                                                         // if we want to have relative size and not original, then first revert to original size
        if (Proj.forceImageSize(edit.color_map, 0, relative, edit.color_map_time, time) // !! use '|' because all need to be processed !!
            ...
            MtrlImages mi;
            mi.fromMaterial(edit, Proj);
            mi.baseTextureSizes(&size0, null, null); // calculate actual sizes
...
void MaterialRegion::resizeBase1(C VecI2 &size, bool relative) {
    ...
    if (relative || game && game->base_2 && game->base_2->size() != size1)
        edit.separateNormalMap(time); // separate if needed (normal can be from bump), and before reverting

    if (relative && size.any()) // if we want to have relative size and not original, then first revert to original size
        if (Proj.forceImageSize(edit.normal_map, 0, relative, edit.normal_map_time, time)) {
            MtrlImages mi;
            mi.fromMaterial(edit, Proj);
            mi.baseTextureSizes(null, &size1, null); // calculate actual sizes
...
void MaterialRegion::resizeBase2(C VecI2 &size, bool relative) {
    ...
    if (relative && size.any())                                                           // if we want to have relative size and not original, then first revert to original size
        if (Proj.forceImageSize(edit.smooth_map, 0, relative, edit.smooth_map_time, time) // !! use '|' because all need to be processed !!
            | Proj.forceImageSize(edit.metal_map, 0, relative, edit.metal_map_time, time) | Proj.forceImageSize(edit.bump_map, 0, relative, edit.bump_map_time, time) | Proj.forceImageSize(edit.glow_map, 0, relative, edit.glow_map_time, time)) {
            MtrlImages mi;
            mi.fromMaterial(edit, Proj);
            mi.baseTextureSizes(null, null, &size2); // calculate actual sizes
    ...
    if (Proj.forceImageSize(edit.smooth_map, size2, relative, edit.smooth_map_time, time) // !! use '|' because all need to be processed !!
        | Proj.forceImageSize(edit.metal_map, size2, relative, edit.metal_map_time, time) | Proj.forceImageSize(edit.bump_map, size2, relative, edit.bump_map_time, time) | Proj.forceImageSize(edit.glow_map, size2, relative, edit.glow_map_time, time)) {
        edit.cleanupMaps();
        rebuildBase(edit.textures());
    }
}
void MaterialRegion::bumpFromCol(int blur) {
    ...
    edit.bump_map = BumpFromColTransform(edit.color_map, blur);
...
void MaterialRegion::create() {
    ...
    T += big.create(Rect_LU(e, 0, h * 1.6f, h), "<<").focusable(false);
    big.mode = BUTTON_TOGGLE;
    T += set_mtrl.create(Rect_LU(big.rect().max.x + p, big.rect().max.y, h, h)).func(SetMtrl, T).focusable(false).desc("Enable this and click on the screen to set material at that location.\nOptionally hold Shift key to completely replace target material on all parts that use it.");
    set_mtrl.mode = BUTTON_TOGGLE;
    set_mtrl.image = "Gui/Misc/set.img";
    T += undo.create(Rect_LU(set_mtrl.rect().ru() + Vec2(p, 0), 0.05f, 0.05f)).func(Undo, T).focusable(false).desc("Undo");
    ...
    T += close.create(Rect_RU(w - e, 0, h, h)).func(Hide, T);
    ...
    T += preview.create(Rect_LU(big.rect().ld() - Vec2(0, 0.007f), we, we), DrawPreview);
    preview.fov = PreviewFOV;
    preview.user = this;
    Gui += preview_big.create(DrawPreview).hide();
    preview_big.fov = preview.fov;
    preview_big.user = preview.user;
    T += sub.create(Rect_LU(0, 0, rect().w(), 1)).skin(&EmptyGuiSkin, false);
    sub.kb_lit = false;
    ...
    tech.combobox.setColumns(tech_lc, Elms(tech_lc)).setData(mtrl_techs, Elms(mtrl_techs));
    tech.combobox.menu.list.setElmDesc(MEMBER(MaterialTech, desc));
    ...
    Property &tq = props.New().create("Tex Quality", MemberDesc(DATA_INT).setFunc(TexQuality, TexQuality)).setEnum().desc("Select Texture Quality");
    tq.combobox.setColumns(NameDescListColumn, Elms(NameDescListColumn)).setData(TexQualities, Elms(TexQualities));
    tq.combobox.menu.list.setElmDesc(MEMBER(NameDesc, desc));
    ...
    ts.reset().size = 0.038f;
    ts.align.set(1, 0);
    Rect prop_rect = AddProperties(props, sub, 0, prop_height, 0.16f, &ts);
    REPAO(props).autoData(this).changed(Changed, PreChanged);
    sub += emissive.create(Rect_RU(emit_red->textline.rect().left(), emit_red->button.rect().w(), prop_height * 2)).func(Emissive, T).focusable(false).subType(BUTTON_TYPE_PROPERTY_VALUE);
    emissive.mode = BUTTON_CONTINUOUS;
    sub += brightness.create(Rect_RU(red->textline.rect().left(), red->button.rect().w(), prop_height * 2)).func(RGB, T).focusable(false).subType(BUTTON_TYPE_PROPERTY_VALUE);
    brightness.mode = BUTTON_CONTINUOUS;
    sub += rgb_1.create(Rect_R(brightness.rect().left() - Vec2(0.01f, 0), prop_height, prop_height * 2), "1").func(RGB1, T).focusable(false).desc("Set RGB to 1");
    rgb_1.text_size /= 2;
    ...
    sub += texs.New().create(TEX_ALPHA, MEMBER(EditMaterial, alpha_map), MEMBER(EditMaterial, alpha_map_time), Rect_LU(prop_rect.ru() + Vec2(s + tex_size * 1, i * prop_height), tex_size, tex_size), "Alpha", T);
    i -= 3;
    ...
    sub += texs.New().create(TEX_NORMAL, MEMBER(EditMaterial, normal_map), MEMBER(EditMaterial, normal_map_time), Rect_LU(prop_rect.ru() + Vec2(s + tex_size * 1, i * prop_height), tex_size, tex_size), "Normal", T);
    i -= 3;
    ...
    sub += texs.New().create(TEX_METAL, MEMBER(EditMaterial, metal_map), MEMBER(EditMaterial, metal_map_time), Rect_LU(prop_rect.ru() + Vec2(s + tex_size * 1, i * prop_height), tex_size, tex_size), "Metal", T);
    i -= 3;
    ...
    {
        Node<MenuElm> &resize = (n += "Resize Base Textures");
        resize.desc("This allows to resize the base textures, such as Color, Alpha, Bump, Normal, Smooth, Metal and Glow to a custom size."); // #MaterialTextureLayout
    ...
    {
        Node<MenuElm> &resize = (n += (water() ? "Resize Color Texture" : "Resize Color+Alpha Textures"));
        if (!water())
            resize.desc("This allows to resize the Base 0 textures, such as Color and Alpha to a custom size."); // #MaterialTextureLayout #MaterialTextureLayoutWater
    ...
    {
        Node<MenuElm> &resize = (n += "Resize Normal Texture");
        resize.desc("This allows to resize the Base 1 textures, such as Normal to a custom size."); // #MaterialTextureLayout #MaterialTextureLayoutWater
    ...
    {
        Node<MenuElm> &resize = (n += (water() ? "Resize Bump Texture" : "Resize Smooth+Metal+Bump+Glow Textures"));
        if (!water())
            resize.desc("This allows to resize the Base 2 textures, such as Smooth, Metal, Bump and Glow to a custom size."); // #MaterialTextureLayout #MaterialTextureLayoutWater
    ...
    sub += texture_options.create().setData(n);
    texture_options.flag |= COMBOBOX_CONST_TEXT;
...
Image *MaterialRegion::getDetailBump(C Str &file) {
    if (!EqualPath(file, detail_bump_file)) {
        ...
        Image temp;
        Proj.loadImages(temp, null, detail_bump_file, true, false);                                                                              // use sRGB because this is for preview
...
void MaterialRegion::resize() {
    ...
    if (!sub.slidebar[1].visible())
        move(Vec2(slidebarSize(), 0));
...
void MaterialRegion::flush(C UID &elm_id) {
    if (T.elm_id == elm_id)
        flush();
}
void MaterialRegion::flush() {
    if (elm && game && changed) {
        ...
        Save(edit, Proj.editPath(elm_id));
        edit.copyTo(*game, Proj);
        Save(*game, Proj.gamePath(elm_id));
        Proj.savedGame(*elm);
        Proj.mtrlSetAutoTanBin(elm->id);
        Server.setElmLong(elm->id);
        REPA(edit.tex_downsize)
        if (saved.tex_downsize[i] != edit.tex_downsize[i])
            Proj.mtrlTexDownsize(elm_id, (TEX_SIZE_PLATFORM)i, edit.tex_downsize[i], saved.base_0_tex, saved.base_1_tex, saved.base_2_tex, true); // upon flushing set all materials with same textures to the same 'tex_downsize'
        if (saved.tex_quality != edit.tex_quality)
            Proj.mtrlTexQuality(elm_id, edit.tex_quality, saved.base_0_tex, saved.base_1_tex, saved.base_2_tex, true); // upon flushing set all materials with same textures to the same 'tex_quality'
        saved = edit;
    }
    changed = false;
}
...
void MaterialRegion::toggle(Elm *elm) {
    if (elm == T.elm && visible())
        elm = null;
    activate(elm);
}
void MaterialRegion::hideBig() {
    if (bigVisible())
        big.push();
}
MaterialRegion &MaterialRegion::show() {
    if (hidden()) {
        super::show();
        resize();
        CodeEdit.resize();
...
MaterialRegion &MaterialRegion::hide() {
    if (visible()) {
        super::hide();
        preview_big.hide();
        REPAO(props).close();
...
void MaterialRegion::set(Memt<MaterialPtr> mtrls) {
    REPD(i, mtrls.elms()) {
        if (!mtrls[i])
            mtrls.remove(i, true); // remove if null
        else
            REPD(j, i)
        if (mtrls[i] == mtrls[j]) {
            mtrls.remove(i, true);
    ...
    FREPA(mtrls)
    if (mtrls[i] == game) {
        set(mtrls[(i + 1) % mtrls.elms()]);
        return;
    } // activate next
    if (mtrls.elms())
        set(mtrls[0]); // activate first
}
void MaterialRegion::drag(Memc<UID> &elms, GuiObj *focus_obj, C Vec2 &screen_pos) {
    if (contains(focus_obj)) {
        ...
        if (Elm *elm = Proj.findElm(elms[i]))
            if (elm->type == elm_type) {
                activate(elm);
                break;
            } else if (elm->type == ELM_IMAGE) {
                REPA(texs)
                if (texs[i].contains(focus_obj)) {
...
::MaterialRegion::ImageSource::TexChannel &MaterialRegion::ImageSource::TexChannel::set(TEX_CHANNEL_TYPE type) {
    T.type = type;
    T.pos = -1;
    return T;
}
::MaterialRegion::ImageSource::TexChannel &MaterialRegion::ImageSource::TexChannel::find(C Str &name, C Str &text, bool case_sensitive, WHOLE_WORD whole_word) {
    if (pos < 0)
        pos = TextPosI(name, text, case_sensitive, whole_word);
    return T;
}
void MaterialRegion::ImageSource::TexChannel::fix() {
    if (pos < 0)
        pos = INT_MAX;
}
int MaterialRegion::ImageSource::TexChannel::Compare(C TexChannel &a, C TexChannel &b) { return ::Compare(a.pos, b.pos); }
void MaterialRegion::ImageSource::set(C Str &name, int index) {
    T.name = name;
    T.index = index;
    REPAO(channel) = -1;
}
void MaterialRegion::ImageSource::process() {
    ...
    if (Contains(base, "color") || Contains(base, "albedo") || Contains(base, "diffuse") || Contains(base, "col", false, WHOLE_WORD_ALPHA) || Contains(base, "BC", true, WHOLE_WORD_ALPHA) || Contains(base, "alb", false, WHOLE_WORD_ALPHA) || Contains(base, "diff", false, WHOLE_WORD_ALPHA))
        channel[TC_COLOR] = 0;
    else if (Contains(base, "alpha"))
        channel[TC_ALPHA] = 0;
    else if (Contains(base, "normal") || Contains(base, "nrm", false, WHOLE_WORD_ALPHA) || Contains(base, "N", true, WHOLE_WORD_ALPHA))
        channel[TC_NORMAL] = 0;
    else if (Contains(base, "emissive") || Contains(base, "emission") || Contains(base, "illum") || Contains(base, "glow") || Contains(base, "EMM", true, WHOLE_WORD_ALPHA) || Contains(base, "E", true, WHOLE_WORD_ALPHA))
        channel[TC_GLOW] = 0;
    else if (Contains(base, "RMA", true, WHOLE_WORD_ALPHA)) {
        channel[TC_ROUGH] = 0;
        channel[TC_METAL] = 1;
        channel[TC_AO] = 2;
    } else if (Contains(base, "ORM", true, WHOLE_WORD_ALPHA) || Contains(base, "ARM", true, WHOLE_WORD_ALPHA)) {
        channel[TC_AO] = 0;
        channel[TC_ROUGH] = 1;
        channel[TC_METAL] = 2;
    } else if (Contains(base, "MRG", true, WHOLE_WORD_ALPHA)) {
        channel[TC_METAL] = 0;
        channel[TC_ROUGH] = 1;
        channel[TC_GLOW] = 2;
    } else if (Contains(base, "mask", false, WHOLE_WORD_ALPHA) && Contains(base, "unity")) // Unity HDRP Mask R=metal, G=AO, B=Detail mask, A=smoothness
    {
        channel[TC_METAL] = 0;
        channel[TC_AO] = 1;
        channel[TC_SMOOTH] = 3;
    } else if (Contains(base, "metal") && ContainsAny(base, "smooth gloss") && !Contains(base, "AO", true)) // Unity RGB=metal, A=smoothness
    {
        channel[TC_METAL] = 0;
        channel[TC_SMOOTH] = 3;
        need_metal_channel = false;                                                                                                  // no need to specify channel for metal because it's in RGB slots
    } else if ((Contains(base, "specular") || Contains(base, "spec", false, WHOLE_WORD_ALPHA)) && ContainsAny(base, "smooth gloss")) // Unity RGB=specular, A=smoothness
    {
        channel[TC_SPEC] = 0;
        channel[TC_SMOOTH] = 3;
    } else // auto-detect
    ...
    if (channel[i] >= 0)
        detected_channels++;
    multi_channel = (detected_channels > 1);
}
void MaterialRegion::ImageSource::process(TEX_TYPE tex_type, EditMaterial &mtrl, bool append, bool multi_images) {
    if (tex_type == TEX_COLOR) {
        if (InRange(channel[TC_AO], 4)) {
            if (multi_channel)
                params.New().set("channel", IndexChannel(channel[TC_AO]));
            params.New().set("mode", "mulRGB");
            params.New().set("alpha", "0.5");
            params.New().set("resize", "full");
        } else if (InRange(channel[TC_GLOW], 4)) {
            if (multi_channel)
                params.New().set("channel", IndexChannel(channel[TC_GLOW]));
            params.New().set("mode", "addRGB");
            params.New().set("resize", "full");
        } else if (InRange(channel[TC_METAL], 4)) {
            if (multi_channel && need_metal_channel)
                params.New().set("channel", IndexChannel(channel[TC_METAL]));
            params.New().set("mode", "metal");
            params.New().set("resize", "full");
        } else if (Kb.shift())
            params.New().set("channel", "rgb"); // make shift to ignore alpha channel
    } else if (tex_type == TEX_SMOOTH) {
        ...
        {
            if (multi_channel) {
                if (InRange(channel[TC_SMOOTH], 4))
                    params.New().set("channel", IndexChannel(channel[TC_SMOOTH]));
                else if (InRange(channel[TC_ROUGH], 4))
                    params.New().set("channel", IndexChannel(channel[TC_ROUGH]));
            }
            if (multi_images) {
                if (InRange(channel[TC_SMOOTH], 4) && mtrl.smooth_is_rough || InRange(channel[TC_ROUGH], 4) && !mtrl.smooth_is_rough)
                    params.New().set("inverseRGB");
            } else // single image - adjust material 'smooth_is_rough'
            {
                if (InRange(channel[TC_SMOOTH], 4)) {
                    mtrl.smooth_is_rough = false;
                    mtrl.smooth_is_rough_time.getUTC();
                } else if (InRange(channel[TC_ROUGH], 4)) {
                    mtrl.smooth_is_rough = true;
                    mtrl.smooth_is_rough_time.getUTC();
                } else if (Kb.shift()
...
int MaterialRegion::Compare(C ImageSource &a, C ImageSource &b) {
    if (int c = ::Compare(a.order, b.order))
        return c;
...
void MaterialRegion::drop(Memc<Str> &names, GuiObj *focus_obj, C Vec2 &screen_pos) {
    if (contains(focus_obj)) {
        ...
        Memc<ImageSource> images;
        FREPA(names)
        if (ExtType(GetExt(names[i])) == EXT_IMAGE)
            images.New().set(CodeEdit.importPaths(names[i]), i);
        bool append = (Kb.ctrlCmd() && tex && tex->file.is()),
             multi_images = ((images.elms() > 1 || append) && tex); // multiple images

        if (multi_images || Kb.shift() || !tex) // auto detect
        {
            ...
            if (tex) {
                ...
                FREPA(images) // process in order
                {
                    ...
                    if (image.channel[TC_COLOR] >= 0) {
                        tex_images[TEX_COLOR].add(image);
                        has_color = true;
                    }
                    if (image.channel[TC_ALPHA] >= 0)
                        tex_images[TEX_ALPHA].add(image);
                    if (image.channel[TC_AO] >= 0)
                        tex_images[TEX_COLOR].add(image);
                    if (image.channel[TC_NORMAL] >= 0)
                        tex_images[TEX_NORMAL].add(image);
                    if (image.channel[TC_ROUGH] >= 0 || image.channel[TC_SMOOTH] >= 0)
                        tex_images[TEX_SMOOTH].add(image);
                    if (image.channel[TC_METAL] >= 0)
                        tex_images[TEX_METAL].add(image);
                    if (image.channel[TC_BUMP] >= 0)
                        tex_images[TEX_BUMP].add(image);
                    if (image.channel[TC_GLOW] >= 0)
                        tex_images[TEX_GLOW].add(image);
                }
                bool auto_reload = T.auto_reload;
                T.auto_reload = false; // disable auto-reload, so we can reload textures all at one time at the end
                TEX_FLAG changed = TEXF_NONE;
                REPA(tex_images) {
                    TEX_TYPE tex_type = (TEX_TYPE)i;
                    Memc<ImageSource> &tex_image = tex_images[tex_type];
                    if (tex_image.elms()) {
                        ...
                        REPA(texs) {
                            ...
                            if (tex.type == tex_type) {
                                bool append = false;
                                if (tex_type == TEX_COLOR && !has_color)
                                    append = true; // if we're applying to Color Slot (AO or Emissive) without Color Texture, then force append to apply on existing
                ...
                if (changed) {
                    if (changed & TEXF_BASE)
                        rebuildBase(old_textures, changed);
                    if (changed & TEXF_EMISSIVE)
                        rebuildEmissive(old_textures);
                }
            }
        }
        if (tex) {
...
void MaterialRegion::rebuildBase(TEX_FLAG old_textures, uint changed_in_mtrl, bool adjust_params, bool always) {
    if (elm && game) {
        ...
        if (auto_reload || always) {
            ...
            has_textures |= Proj.mtrlCreateBaseTextures(edit, changed_in_mtrl);
            Time.skipUpdate(); // compressing textures can be slow
        }
        if (adjust_params)
            edit.adjustParams(changed_in_mtrl, old_textures, has_textures, known_textures);
...
void MaterialRegion::rebuildEmissive(TEX_FLAG old_textures, bool adjust_params) {
    if (elm && game) {
        ...
        has_textures |= Proj.mtrlCreateEmissiveTexture(edit);
        if (adjust_params)
            edit.adjustParams(0, old_textures, has_textures, known_textures);
        setChanged();
        Proj.mtrlTexChanged();
        Time.skipUpdate(); // compressing textures can be slow
...
void MaterialRegion::erasing(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        set(null);
}
bool MaterialRegion::winIOContains(GuiObj *go) C {
    REPA(texs)
    if (texs[i].win_io.contains(go))
        return true;
    return false;
}
void MaterialRegion::update(C GuiPC &gpc) {
    ...
    if (Gui.ms() == &preview || Gui.ms() == &preview_big) {
        ...
        if (rot_light)
            light_angle += Ms.d() * Vec2(-1, 1);
        if (rot_cam || rot_light)
            Ms.freeze();
        if (Ms.bd(0))
            big.push();
    }
    if (Ms.bp(2)) {
        if (contains(Gui.ms()))
            set(null);
        else if (Gui.ms() == &preview_big)
            big.push();
    }
    REPA(Touches)
    if (Touches[i].guiObj() == &preview || Touches[i].guiObj() == &preview_big)
        if (Touches[i].on()) {
            preview_cam.yaw -= Touches[i].ad().x * 2.0f;
            preview_cam.pitch += Touches[i].ad().y * 2.0f;
            preview_cam.setSpherical();
        }
