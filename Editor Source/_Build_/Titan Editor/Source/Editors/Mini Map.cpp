...
int MiniMapEditor::areasPerImage() {
    ElmMiniMap *data = T.data();
    return data ? data->areas_per_image : 4;
}
UID MiniMapEditor::worldID() {
    ElmMiniMap *data = T.data();
...
void MiniMapEditor::World(MiniMapEditor &mme, C Str &text) {
    if (ElmMiniMap *data = mme.data()) {
        data->world_id = Proj.findElmID(text, ELM_WORLD);
        data->world_time.getUTC();
        mme.reloadWorld();
    }
}
Str MiniMapEditor::World(C MiniMapEditor &mme) {
    if (ElmMiniMap *data = mme.data())
        return Proj.elmFullName(data->world_id);
    return S;
}
void MiniMapEditor::Env(MiniMapEditor &mme, C Str &text) {
    if (ElmMiniMap *data = mme.data()) {
        data->env_id = Proj.findElmID(text, ELM_ENV);
        data->env_time.getUTC();
    }
}
Str MiniMapEditor::Env(C MiniMapEditor &mme) {
    if (ElmMiniMap *data = mme.data())
        return Proj.elmFullName(data->env_id);
    return S;
}
void MiniMapEditor::ArPerImg(MiniMapEditor &mme, C Str &text) {
    if (ElmMiniMap *data = mme.data()) {
        int i = TextInt(text);
...
Str MiniMapEditor::ArPerImg(C MiniMapEditor &mme) {
    int nearest = -1, dist;
    if (ElmMiniMap *data = mme.data())
        REPA(AreasPerImage) {
...
void MiniMapEditor::Size(MiniMapEditor &mme, C Str &text) {
    if (ElmMiniMap *data = mme.data()) {
        int i = TextInt(text);
...
Str MiniMapEditor::Size(C MiniMapEditor &mme) {
    int nearest = -1, dist;
    if (ElmMiniMap *data = mme.data())
        REPA(ImageSizes) {
...
void MiniMapEditor::makeDo() {
    if (elm)
        if (ElmMiniMap *data = elm->miniMapData())
            StateMiniMap.set(StateFadeTime);
}
...
Rect MiniMapEditor::sizeLimit() C {
...
void MiniMapEditor::activate(Elm *elm) {
    set(elm);
    if (T.elm)
        super::activate();
}
void MiniMapEditor::toggle(Elm *elm) {
    if (elm == T.elm)
        elm = null;
    set(elm);
}
MiniMapEditor &MiniMapEditor::hide() {
    if (visible()) {
        super::hide();
        set(null);
    }
...
void MiniMapEditor::erasing(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        set(null);
}
