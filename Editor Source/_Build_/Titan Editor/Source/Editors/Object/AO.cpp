...
/******************************************************************************/
void MeshAOClass::PreviewToggle(MeshAOClass &editor) {
    if (editor.preview_prop)
...
void MeshAOClass::startThread() {
    if (!thread.active())
        thread.create(Process, this);
}
void MeshAOClass::createSrc() {
    {
        MeshLod temp;
        temp.create(ObjEdit.getLod()).setBase().delRender();
        SyncLocker locker(lock);
        Swap(temp, src);
        src_ext = ObjEdit.mesh.ext;
        src_id++;
        scale = ObjEdit.posScale();
        changedParams();
        baked.del();
        baked_valid = finished = false;
    }
    startThread();
}
MeshAOClass &MeshAOClass::activate() {
    if (hidden()) {
        createSrc();
        thread.cancelStop();
...
MeshAOClass &MeshAOClass::hide() {
    thread.stop();
    super::hide();
    return T;
}
MeshAOClass::~MeshAOClass() { thread.del(); }
MeshAOClass &MeshAOClass::del() {
    thread.del();
    super::del();
    return T;
}
MeshAOClass &MeshAOClass::create() {
    super::create("Set Per-Vertex AO").hide();
    button[2].show();
    ...
    ts.align.set(1, 0);
    Rect r = AddProperties(props, T, Vec2(0.02f, -0.02f), 0.05f, 0.22f, &ts);
    REPAO(props).autoData(&MeshAO).changed(ChangedParams);
