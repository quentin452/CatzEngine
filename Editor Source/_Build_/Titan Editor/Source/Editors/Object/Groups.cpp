...
/******************************************************************************/
int GroupRegion::VisibleParts(C MeshLod &lod) {
    int n = 0;
    REPA(lod)
...
int GroupRegion::VisiblePartI(C MeshLod &lod, int i) {
    FREPAD(p, lod)
    if (!(lod.parts[p].part_flag & MSHP_HIDDEN))
        if (!i--)
            return p;
    return -1;
}
int GroupRegion::AbsPartI(C MeshLod &lod, int i) {
    int v = 0;
    FREPAD(p, lod)
    if (!(lod.parts[p].part_flag & MSHP_HIDDEN)) {
        if (p == i)
...
C MeshPart *GroupRegion::VisiblePart(C MeshLod &lod, int i) {
    i = VisiblePartI(lod, i);
    return lod.parts.addr(i);
}
Str GroupRegion::IndexToGroupName(C Index &index) {
    if (Enum *e = ObjEdit.mesh.drawGroupEnum())
        return e->elmName(index.i);
    return S;
}
...
void GroupRegion::SetGroup(Button &button) {
    ...
    REPA(ObjEdit.group.set_groups)
    if (&ObjEdit.group.set_groups[i] != &button)
        ObjEdit.group.set_groups[i].set(false, QUIET);
}
...
GroupRegion &GroupRegion::create() {
    super::create(Rect_LU(0, 0, 1.2f, 0.8f)).skin(&TransparentSkin, false);
    kb_lit = false;
    ...
    groups_l.cur_mode = LCM_MOUSE;
    FlagDisable(groups_l.flag, LIST_SORTABLE);
    groups_l.flag |= LIST_SCALABLE;
    groups_l.desc("Drag and drop groups onto mesh parts to assign them");
    ...
    parts_r += parts_l.create(parts_lc, Elms(parts_lc)).elmHeight(0.037f).textSize(0, 1).columnHeight(0.05f);
    parts_l.cur_mode = LCM_MOUSE;
    FlagDisable(parts_l.flag, LIST_SORTABLE);
    parts_l.flag |= LIST_RESIZABLE_COLUMNS | LIST_SCALABLE;
...
void GroupRegion::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        ...
        if (parts != parts_l.elms() || reset)
            parts_l.setData(indexes.data(), parts); // reset because of potential mem address change
        if (groups != groups_l.elms() || reset)
            groups_l.setData(indexes.data(), groups); // reset because of potential mem address change
