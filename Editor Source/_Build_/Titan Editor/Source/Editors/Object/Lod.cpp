...
void LodView::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        ...
        r.extend(-0.005f);
        r.draw((ObjEdit.selLod() == lod_index) ? RED : (ObjEdit.lit_lod == lod_index) ? AZURE
                                                                                      : BLACK,
               false);
        TextStyleParams ts;
        ts.size = 0.039f;
        if (InRange(lod_index, ObjEdit.mesh.lods())) {
            MeshLod &base = ObjEdit.mesh, &lod = ObjEdit.mesh.lod(lod_index);
            flt dist = ObjEdit.lodDist(lod);
            D.text(ts, r, S + "Lod " + lod_index + "\nVtxs " + VisibleVtxs(lod) + "\nTris " + VisibleTrisTotal(lod) + "\nSize " + SizeBytes(VisibleSize(lod)) + "\nQuality " + Round(VisibleLodQuality(ObjEdit.mesh, lod_index) * 100) + "%\n" + (NegativeSB(dist) ? S + "DISABLED" : lod_index > 0 ? S + "Distance " + TextSimple(dist)
                                                                                                                                                                                                                                                                                                    : S));
        } else {
            D.text(ts, r, "New");
        }
...
void LodRegion::Dist(LodRegion &lr, C Str &t) {
    ObjEdit.mesh_undos.set("lodDist");
    MeshLod &lod = ObjEdit.getLod();
    flt dist = TextFlt(t);
    if (NegativeSB(lod.dist2))
        CHSSB(dist);
    ObjEdit.setLodDist(lod, dist);
}
void LodRegion::EditDistChanged(LodRegion &lr) {
    if (lr.edit_dist() >= 0)
        NewLod.hide();
}
void LodRegion::EditDistToggle(LodRegion &lr) {
    lr.edit_dist.toggle(0);
    if (lr.edit_dist() >= 0 && lr.dist)
        lr.dist->textline.selectAll().kbSet();
}
LodRegion &LodRegion::create() {
    Gui += NewLod.create();
    super::create().skin(&EmptyGuiSkin, false);
    kb_lit = false;
    cchar8 *t_edit_dist[] =
        {
            "Edit Distance",
    ...
    edit_dist.tab(0) += edit_dist_text.create("Set distance at which LODs look similar", &ts);
    ts.reset().size = 0.05f;
    ...
    AddProperties(props, edit_dist.tab(0), edit_dist.rect().ru() + Vec2(0.01f, 0), 0.05f, 0.2f, &ts);
...
void LodRegion::resize() {
    ...
    Vec2 delta = edit_dist.posRU() - old;
    REPAO(props).move(delta);
    props_region.rect(Rect_D(ObjEdit.rect().w() / 2, -ObjEdit.rect().h(), 0.8f, 0.13f));
}
void LodRegion::toGui() {
    REPAO(props).toGui();
    if (dist)
        dist->textline.selectAll();
}
void LodRegion::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        ...
        FREPA(lods) {
            LodView &lod = lods[i];
            if (Gui.ms() == &lod || Gui.ms() == &lod.erase || Gui.ms() == &lod.disable)
                ObjEdit.lit_lod = i;
            if (!lod.is()) {
                T += lod.create();
                T += lod.erase.create().func(Erase, lod).desc("Completely erase this LOD\nYou will not be able to restore it later.");
                lod.erase.image = "Gui/close.img";
                T += lod.disable.create().func(Disable, lod);
                lod.disable.mode = BUTTON_TOGGLE;
            }
            ...
            REPAD(mt, MT)
            if (MT.bp(mt, 0) && MT.guiObj(mt) == &lod) {
                if (!in_range)
                    NewLod.activate();
                else
                    ObjEdit.selLod(lod.lod_index);
            }
        }
...
void LodRegion::drop(Memc<Str> &names, GuiObj *focus_obj, C Vec2 &screen_pos) {
    if (ObjEdit.obj_elm && contains(focus_obj))
        REPAD(l, lods)
    if (lods[l].contains(focus_obj)) {
        REPA(names)
        if (ExtType(GetExt(names[i])) == EXT_MESH) {
            ...
            if (ElmObj *obj_data = obj_elm->objData())
                if (Elm *mesh_elm = Proj.findElm(obj_data->mesh_id, ELM_MESH))
                    if (ElmMesh *mesh_data = mesh_elm->meshData()) {
                        Mesh src;
                        if (Load(src, Proj.editPath(mesh_elm->id), Proj.game_path))
                            if (src.is()) {
                                ...
                                flt dist_add;
                                if (InRange(l, ObjEdit.mesh.lods()))
                                    dist_add = ObjEdit.absLodDist(ObjEdit.mesh.lod(l));
                                else
                                    dist_add = NewLodClass::NextLodDist(ObjEdit.absLodDist(ObjEdit.mesh.lod(ObjEdit.mesh.lods() - 1))); // get distance that we would normally use for the next LOD
...
flt NewLodClass::NextLodDist(flt dist) {
    flt next = Max(2, Abs(dist) * 2);
    if (NegativeSB(dist))
        CHSSB(next);
    return next;
}
void NewLodClass::FromFile(NewLodClass &nl) {
    Str path = Proj.elmSrcFileFirst(ObjEdit.mesh_elm);
    if (FileInfoSystem(path).type == FSTD_FILE)
        path = GetPath(path);
    if (path.is())
        nl.win_io.path(S, path);
    nl.win_io.activate();
    nl.hide();
}
void NewLodClass::PreviewToggle(NewLodClass &nl) {
    if (nl.preview_prop)
        nl.preview_prop->set(!nl.preview_prop->asBool());
}
void NewLodClass::DrawAtDistToggle(NewLodClass &nl) {
    if (nl.draw_at_distance_prop)
        nl.draw_at_distance_prop->set(!nl.draw_at_distance_prop->asBool());
}
void NewLodClass::OK(NewLodClass &nl) {
    if (!nl.finished)
        Gui.msgBox(S, "Processing not finished yet");
    else {
        nl.hide();
        if (ObjEdit.mesh_elm) {
            ObjEdit.mesh_undos.set("lod");
...
void NewLodClass::changedParams() {
    ...
    finished = false;
    if (quality)
        quality->display(S + "Quality: Calculating..");
}
        ...
        if (EE::Import(name, &temp, null, null, SCAST(Memc<XMaterial>, mtrls), pmi))
            if (temp.is()) {
                ...
                FREPA(mtrls) {
                    ...
                    Elm &mtrl = Proj.newMtrl(mtrls[i], ObjEdit.obj_id, fp.encode());
                    Server.setElmFull(mtrl.id);
                ...
                FREPA(lod) {
                    ...
                    if (InRange(i, pmi))     // set materials
                    {
                        ...
                        if (InRange(mtrl_index, mtrl_ptrs))
                            part.material(mtrl_ptrs[mtrl_index]);
                    }
                }
...
bool NewLodClass::simplify() {
    ...
    if (needRebuild()) {
        ...
        ThreadMayUseGPUData();
        MeshLod processed;
        temp.simplify(intensity, dist, max_uv, max_color, max_material, max_skin, DegToRad(max_normal), keep_border, SIMPLIFY_QUADRIC, EPS * scale, &processed, &stop);
        if (!stop)
            processed.setRender(false);
        locker.on();
...
void NewLodClass::startThread() {
    if (!thread.active())
        thread.create(Simplify, this);
}
void NewLodClass::createSrc() {
    {
        MeshLod temp;
        temp.create(ObjEdit.mesh.lod(ObjEdit.mesh.lods() - 1)).setBase().delRender();
        SyncLocker locker(lock);
        Swap(temp, src);
        src_id++;
        scale = ObjEdit.posScale();
        changedParams();
        simplified.del();
        simplified_valid = finished = false;
    }
    startThread();
}
NewLodClass &NewLodClass::activate() {
    if (hidden()) {
        createSrc();
        thread.cancelStop();
        ObjEdit.lod.edit_dist.set(-1);
    }
    super::activate();
    return T;
}
NewLodClass &NewLodClass::hide() {
    thread.stop();
    super::hide();
    return T;
}
NewLodClass::~NewLodClass() { thread.del(); }
NewLodClass &NewLodClass::del() {
    thread.del();
    super::del();
    return T;
}
NewLodClass &NewLodClass::create() {
    ...
    ts.align.set(1, 0);
    Rect r = AddProperties(props, T, Vec2(0.02f, -0.02f), 0.05f, 0.2f, &ts);
    REPAO(props).autoData(&NewLod).changed(ChangedParams);
    preview_prop->changed(null, null);
    draw_at_distance_prop->changed(null, null);
    draw_distance_prop->changed(null, null);
    ...
    r |= from_file.rect();
    Vec2 size = Vec2(r.max.x, -r.min.y) + 0.02f + defaultInnerPaddingSize();
