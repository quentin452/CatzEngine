...
void ObjView::meshDelete() {
    if (mesh_parts.edit_selected()) {
        ...
        Memc<Memc<int>> vtxs;
        REPA(sel_vtx)
        vtxs(sel_vtx[i].x).add(sel_vtx[i].y);
        Memc<Memc<int>> faces;
        REPA(sel_face)
        faces(sel_face[i].x).add(sel_face[i].y);
        bool changed = false;
        REPA(lod) {
            ...
            if (v && v->elms())
                part.base.removeVtxs(*v);
            else if (f && f->elms())
                part.base.removeFaces(*f);
            else
                continue;
            changed = true;
            if (part.base.is())
...
void ObjView::meshSplit() {
    if (mesh_parts.edit_selected()) {
        ...
        REPA(lod) {
            ...
            Memc<int> *f = faces.addr(i);
            if (v && v->elms())
                part = lod.splitVtxs(i, *v);
            else if (f && f->elms())
                part = lod.splitFaces(i, *f);
            if (part) {
                changed = true;
                part->setRender();
                if (InRange(i, lod))
                    lod.parts[i].setRender();
...
void ObjView::meshAlign(bool xz) {
    if (lit_vtx < 0)
        Gui.msgBox(S, "No highlighted vertex");
    else if (C MeshPart *part = getPart(lit_vf_part))
...
void ObjView::meshWeldPos() {
    if (mesh_parts.edit_selected()) {
        if (!sel_vtx.elms())
            Gui.msgBox(S, "No vertexes selected");
        else {
            ...
            REPA(sel_vtx) {
                C VecI2 &v = sel_vtx[i];
                if (C MeshPart *part = lod.parts.addr(v.x))
                    if (InRange(v.y, part->base.vtx)) {
                        C Vec &pos = part->base.vtx.pos(v.y);
                        REPA(poss)
                        if (Equal(pos, poss[i], pos_eps))
                            goto has;
                        poss.add(pos);
                        center += pos;
                    has:;
...
void ObjView::meshWeldPos(flt pos_eps) {
    ...
    if (partOp(i)) {
        ...
        if (1)
            part.base.weldVtx(VTX_ALL, pos_eps, EPS_COL_COS, -1); // weld vtxs as one, don't remove degen faces because we've already done it above
        part.setRender();
        changed = true;
    }
    if (changed)
...
void ObjView::meshSetPos(bool keep_uv) {
    if (!sel_vtx.elms() || !mesh_parts.edit_selected())
        Gui.msgBox(S, "No vertexes selected");
    else if (lit_vtx < 0)
        Gui.msgBox(S, "No highlighted vertex");
    else {
        MeshLod &lod = getLod();
        TransformRegion::KeepUV kuv;
        if (keep_uv)
            kuv.create(lod);
        if (C MeshPart *part = lod.parts.addr(lit_vf_part))
            if (InRange(lit_vtx, part->base.vtx)) {
                ...
                REPA(sel_vtx) {
                    ...
                    if (MeshPart *part = lod.parts.addr(v.x))
                        if (InRange(v.y, part->base.vtx)) {
                            part->base.vtx.pos(v.y) = center;
                            if (keep_uv)
                                kuv.parts[v.x].vtxs[v.y].adjust = true;
                        }
                }
                if (keep_uv) {
                    kuv.keepUV(lod);
                    REPA(sel_vtx) {
                        ...
                        if (MeshPart *part = lod.parts.addr(v.x)) {
                            ...
                            if (InRange(v.y, base.vtx))
                                if (Vec2 *uv = base.vtx.tex0()) {
                                    TransformRegion::KeepUV::Vtx &vtx = kuv.parts[v.x].vtxs[v.y];
                                    if (vtx.weight)
                                        uv[v.y] = vtx.uv / vtx.weight;
                                }
                        }
                    }
                }
...
void ObjView::meshReverse() {
    ...
    if (mesh_parts.edit_selected()) {
        if (!sel_face.elms())
            Gui.msgBox(S, "No faces selected");
        else {
            ...
            if (Memc<int> *f = faces.addr(p))
                if (f->elms()) {
                    ...
                    REPA(*f) {
                        ...
                        if (face & SIGN_BIT) {
                            face ^= SIGN_BIT;
                            if (InRange(face, base.quad))
                                quads++;
                        } else {
                            if (InRange(face, base.tri))
                                tris++;
                        }
                    }
                    base.reverse(*f);
                    ...
                    REP(Min(tris, base.tris()))
                    sel_face.binaryInclude(VecI2(p, base.tris() - 1 - i));
                    REP(Min(quads, base.quads()))
                    sel_face.binaryInclude(VecI2(p, (base.quads() - 1 - i) ^ SIGN_BIT));
                    changed = true;
                }
        }
    } else {
...
void ObjView::meshSetNrm(MESH_FLAG vtx_test) {
    ...
    if (mesh_parts.edit_selected()) {
        ...
        if (vtxs.elms()) {
            ...
            MeshLod temp;
            temp.create(lod, ~VTX_DUP); // copy everything except VTX_DUP
            if (vtx_test)
                temp.setVtxDup(vtx_test, pos_eps);
            temp.setNormals();

            // copy normals on selected vertexes
            REPA(vtxs) {
                C VecI2 &v = vtxs[i];
                if (C MeshPart *src = temp.parts.addr(v.x))
                    if (InRange(v.y, src->base.vtx) && src->base.vtx.nrm())
                        if (MeshPart *dest = lod.parts.addr(v.x))
                            if (InRange(v.y, dest->base.vtx)) {
                                if (dest->base.vtx.nrm())
                                    dest->base.vtx.nrm(v.y) = src->base.vtx.nrm(v.y);
                                else if (dest->base.vtxs() == src->base.vtxs()) {
                                    dest->base.include(VTX_NRM);
                                    CopyN(dest->base.vtx.nrm(), src->base.vtx.nrm(), dest->base.vtxs());
                                } // copy all
                                changed = true;
                            }
            }
        ...
        if (partOp(i)) {
            MeshPart &part = lod.parts[i];
            MeshBase &base = part.base, temp;
            if (avg)
                temp.create(base);
            if (vtx_test)
                base.setVtxDup(vtx_test, pos_eps);
            else
                base.exclude(VTX_DUP);
            base.setNormals();
            if (avg)
                REPA(base.vtx)
            base.vtx.nrm(i) = !Avg(base.vtx.nrm(i), temp.vtx.nrm(i));
            base.setTanBinDbl();
            part.setRender(); // reset tan/bin as they may depend on normals and duplicates
            changed = true;
        }
        lod.exclude(VTX_DUP);
    }
...
void ObjView::meshCopyNrm() {
    ...
    if (mesh_parts.edit_selected()) {
        ...
        if (C MeshPart *part_lit = lod.parts.addr(lit_part)) {
            C MeshBase &lit = part_lit->base;
            if (lit.vtx.pos() && lit.vtx.nrm())
                REPA(lod)
            if (partOp(i)) {
                MeshPart &part = lod.parts[i];
                MeshBase &base = part.base;
                if (base.vtx.pos() && base.vtx.nrm()) {
                    ...
                    if (changed_part) {
                        ...
                        part.setRender();
                        changed = true;
                    } // reset tan/bin as they may depend on normals and duplicates
                }
            }
        }
    }
...
void ObjView::meshNrmY() {
    ...
    MeshLod &lod = getLod();
    Vec dest(0, 1, 0);
    dest.div(mesh_matrix.orn()).normalize();
    if (mesh_parts.edit_selected()) {
        Memt<VecI2> vtxs;
        getSelectedVtxs(vtxs);
        if (vtxs.elms()) {
        ...
        if (partOp(i)) {
            ...
            if (base.vtx.nrm()) {
                REPA(base.vtx)
                AlignVtxNormal(base.vtx.nrm(i), dest);
                base.setTanBinDbl();
                part.setRender(); // reset tan/bin as they may depend on normals and duplicates
                changed = true;
            }
...
void ObjView::meshCreateFace() {
    REPA(sel_vtx)
    if (sel_vtx[i].x != sel_vtx[0].x) {
        Gui.msgBox(S, "Selected Vertexes must be in the same mesh part");
        return;
    }
    if (!sel_vtx.elms())
        Gui.msgBox(S, "No Vertexes Selected");
    else if (MeshPart *part = getPart(sel_vtx[0].x)) {
        MeshBase &base = part->base;
        flt pos_eps = posEps();
        MemtN<Vec, 4> unique;
        FREPA(sel_vtx) // add in order
        {
            ...
            REPA(unique)
            if (Equal(unique[i], pos, pos_eps))
                goto has;
            if (unique.elms() >= 4)
                goto invalid_vtx_number;
            unique.add(pos);
        has:;
        }
        if (unique.elms() >= 3 && unique.elms() <= 4) {
            ...
            if (unique.elms() == 3) {
                ...
                if (Edit::Viewport4::View *last = v4.last())
                    if (!frontFace(tri.center(), &tri_n, last->camera.matrix)) {
                        tri_ind.reverse();
                        tri_n.chs();
                    }
                base.addTri(tri_ind);
                ...
                if (Edit::Viewport4::View *last = v4.last())
                    if (!frontFace(quad.center(), &quad_n, last->camera.matrix)) {
                        quad_ind.reverse();
                        quad_n.chs();
                    }
            ...
            if (added_vtxs) {
                ...
                Matrix3 tex_matrix;
                tex_matrix.setDir(face_nrm);
                REPA(unique) {
                    int v = ind.c[i];
                    if (base.vtx.nrm())
                        base.vtx.nrm(v) = face_nrm;
                    if (base.vtx.color())
                        base.vtx.color(v) = WHITE;
                    if (base.vtx.material())
                        base.vtx.material(v).set(255, 0, 0, 0);
                    if (base.vtx.blend())
                        base.vtx.blend(v).set(255, 0, 0, 0);
                    if (base.vtx.tex0()) {
                        C Vec &pos = base.vtx.pos(v);
                        base.vtx.tex0(v).set(Dot(pos, tex_matrix.x), Dot(pos, tex_matrix.y));
                    }
                }
...
void ObjView::meshMergeFaces() {
    if (!sel_face.elms())
        Gui.msgBox(S, "No Faces Selected");
    else if (sel_face.elms() > 2)
        Gui.msgBox(S, "Too many Faces Selected");
    else {
        Memt<VecI2> faces;
        faces = sel_face;
        if (faces.elms() == 1 && lit_vf_part >= 0 && lit_face != -1)
            faces.New().set(lit_vf_part, lit_face);
        if (faces.elms() != 2)
            Gui.msgBox(S, "2 Faces must be Selected");
        else if (faces[0].x != faces[1].x)
            Gui.msgBox(S, "Selected faces are of different mesh parts");
        else if (MeshPart *part = getPart(faces[0].x)) {
            ...
            if (merged_face != -1) // !=-1 and not >=0 because of SIGN_BIT for quads
            {
                ...
                setChangedMesh(true, false);
                litSelVFClear();
                selVFSet(faces[0].x, -1, merged_face);
            }
        } else
            Gui.msgBox(S, "Invalid Mesh Part");
    }
}
void ObjView::meshMergeCoplanarFaces(bool all) {
    ...
    if (all) {
        ...
        if (partOp(i)) {
            MeshPart &part = lod.parts[i];
            if (part.base.weldCoplanarFaces()) {
                part.setRender();
                changed = true;
            }
        }
...
void ObjView::meshRotQuads() {
    ...
    MeshLod &lod = getLod();
    REPA(sel_face) {
        C VecI2 &sf = sel_face[i];
        if (sf.y & SIGN_BIT) // only quads
        {
            int q = sf.y ^ SIGN_BIT;
            if (MeshPart *part = lod.parts.addr(sf.x))
                if (InRange(q, part->base.quad)) {
...
void ObjView::meshQuadToTri() {
    ...
    MeshLod &lod = getLod();
    Mems<int> part_tris;
    part_tris.setNum(lod.parts.elms());
    REPAO(part_tris) = lod.parts[i].tris(); // number of tris in each part before conversion
    if (mesh_parts.edit_selected()) {
        ...
        REPA(sel_face) {
            ...
            if (sf.y & SIGN_BIT) // only quads
            {
                ...
                if (MeshPart *part = lod.parts.addr(sf.x))
                    if (InRange(q, part->base.quad))
                        part_quads(sf.x).add(q);
            }
        }
        if (part_quads.elms()) {
            REPA(part_quads)
            if (MeshPart *part = lod.parts.addr(i)) {
                part->base.quadToTri(part_quads[i]);
                part->setRender();
            }
            changed = true;
        }
    ...
    if (changed) {
        REPA(sel_face)
        if (sel_face[i].y & SIGN_BIT)
            sel_face.remove(i, true); // remove all quads from selection
        FREPA(part_tris)
        for (int j = part_tris[i], end = lod.parts[i].tris(); j < end; j++)
            sel_face.binaryInclude(VecI2(i, j)); // add all tris to selection that were created in this operation
        setChangedMesh(true, false);
    }
}
    ...
    if (mesh_parts.edit_selected()) {
        ...
        if (!vtxs.elms())
            ...
            REPAD(p, lod) {
                ...
                REPAD(v, vtxs)
                if (vtxs[v].x == p)
                    part_vtxs.add(vtxs[v].y); // get vertexes for this part
...
void ObjView::meshColorBrghtn() {
    ...
    if (mesh_parts.edit_selected()) {
        ...
        if (!vtxs.elms())
            ...
            REPA(vtxs) {
                C VecI2 &v = vtxs[i];
                if (MeshPart *part = lod.parts.addr(v.x))
                    if (InRange(v.y, part->base.vtx) && part->base.vtx.color()) {
    ...
    if (partOp(i)) {
        ...
        if (base.vtx.color()) {
            REPA(base.vtx)
            base.vtx.color(i) = Lerp(base.vtx.color(i), WHITE, 0.1f);
            part.setRender();
            changed = true;
        }
    }
...
void ObjView::meshColorDarken() {
    ...
    if (mesh_parts.edit_selected()) {
        ...
        if (!vtxs.elms())
            ...
            REPA(vtxs) {
                ...
                if (MeshPart *part = lod.parts.addr(v.x))
                    if (InRange(v.y, part->base.vtx)) {
                        IncludeColors(part->base);
                        if (part->base.vtx.color()) {
                            Color &c = part->base.vtx.color(v.y);
                            c = Lerp(c, BLACK, 0.1f);
                            changed = true;
                        }
    ...
    if (partOp(i)) {
        ...
        if (base.vtx.color()) {
            REPA(base.vtx)
            base.vtx.color(i) = Lerp(base.vtx.color(i), BLACK, 0.1f);
            part.setRender();
            changed = true;
        }
    }
...
void ObjView::meshDelDblSide() {
    ...
    if (partOp(i)) {
        MeshPart &part = lod.parts[i];
        MeshBase &base = part.base, temp;
        temp.create(base).triToQuad();
        int faces = temp.faces();
        temp.setVtxDup(MESH_NONE, pos_eps).removeDoubleSideFaces().exclude(VTX_DUP);
        if (faces != temp.faces()) {
            Swap(base, temp);
            part.setRender();
            changed = true;
...
void ObjView::meshReplaceLods() {
    ...
    if (getMeshElm()) {
        ...
        if (mesh_skel && mesh_skel->is())
            mesh.skeleton(mesh_skel).skeleton(null);
        else
            mesh.clearSkeleton().exclude(VTX_SKIN); // call 'skeleton' to reassign bone mapping or remove it if not present
        setChangedMesh(true);
    }
}
void ObjView::meshSeparate1() {
    ...
    if (partOp(i)) {
        ...
        if (!name.is())
            name = part.name;
        all_lods &= SamePartInAllLods(mesh, i);
    } // add in order
    if (parts.elms() && parts.elms() != lod.parts.elms()) {
        ...
        Elm &obj_elm = Proj.Project::newElm(name, obj_id, ELM_OBJ);
        Proj.list_sel.add(obj_elm.id);
        EditObject obj_edit;
        obj_edit.newData();
        Save(obj_edit, Proj.editPath(obj_elm.id));
        if (ElmObj *obj_data = obj_elm.objData()) {
            obj_data->newData();
            obj_data->from(obj_edit);
            if (T.obj_elm)
                obj_data->setSrcFile(T.obj_elm->srcFile());
        }
        if (Elm *mesh_elm = Proj.getObjMeshElm(obj_elm.id, false, false)) // don't send to server yet, it will be sent below
            if (ElmMesh *mesh_data = mesh_elm->meshData()) {
                if (T.skel_elm && mesh_skel)
                    if (Elm *skel_elm = Proj.getObjSkelElm(obj_elm.id, false, false)) // don't send to server yet, it will be sent below
                        if (ElmSkel *skel_data = skel_elm->skelData()) {
                            if (ElmSkel *src_skel_data = T.skel_elm->skelData())
                                skel_data->transform = src_skel_data->transform;
                ...
                Mesh mesh_edit;
                mesh_edit.copyParams(mesh);
                mesh_edit.setLods(all_lods ? T.mesh.lods() : 1);
                REPD(l, mesh_edit.lods()) {
                    MeshLod &src = (all_lods ? T.mesh.lod(l) : lod), &dest = mesh_edit.lod(l);
                    dest.copyParams(src);
                    dest.parts.setNum(parts.elms());
                    REPAD(p, dest) {
                        dest.parts[p].create(src.parts[parts[p]]);
                        src.parts.remove(parts[p], true);
                        mesh_parts.erasedPart(parts[p]);
                    }
                }
                ...
                Server.setElmFull(mesh_elm->id);
            }
        Proj.makeGameVer(obj_elm);
        Server.setElmFull(obj_elm.id);
...
void ObjView::meshSeparateN() {
    ...
    if (obj_elm) {
        ...
        bool remove = (parts.elms() != lod.parts.elms()); // don't remove if there will be no parts left
        REPAD(p, parts)                                   // process in remove order
        {
            C MeshPart &part = lod.parts[parts[p]];
            Elm &obj_elm = Proj.Project::newElm(Is(part.name) ? Str(part.name) : T.obj_elm->name, obj_id, ELM_OBJ);
            Proj.list_sel.add(obj_elm.id);
            EditObject obj_edit;
            obj_edit.newData();
            Save(obj_edit, Proj.editPath(obj_elm.id));
...
void ObjView::meshCopyParts() {
    ...
    if (Elm *dest_mesh_elm = getMeshElm())
        if (ElmMesh *dest_mesh_data = dest_mesh_elm->meshData()) {
            bool changed = false;
            FREPA(menu_ids)
            if (Elm *src_obj_elm = Proj.findElm(menu_ids[i]))
                if (ElmObj *src_obj_data = src_obj_elm->objData())
                    if (Elm *src_mesh_elm = Proj.findElm(src_obj_data->mesh_id))
                        if (ElmMesh *src_mesh_data = src_mesh_elm->meshData()) {
                            Mesh src_mesh;
                            if (Load(src_mesh, Proj.editPath(src_mesh_elm->id), Proj.game_path)) {
                                ...
                                if (mesh_skel && mesh_skel->bones.elms())
                                    src_mesh.skeleton(mesh_skel); // adjust to the new skeleton
                                else
                                    src_mesh.exclude(VTX_SKIN).clearSkeleton();              // remove completely
                                if (mode() == LOD && (mesh.lods() > 1 || mesh.parts.elms())) // if in LOD mode, and mesh has anything (lods or parts), then add to current LOD only
                                {
                                    ...
                                    mesh.add(src_mesh);                   // otherwise copy to entire mesh
                                mesh.setTanBinDbl().setRender().setBox(); // set tan/bin because src mesh from disk doesn't have them
                                Memc<int> sel_parts;
                                REP(getLod().parts.elms() - old_parts)
                                sel_parts.add(old_parts + i);
    ...
    if (mesh_parts.partOp(i)) {
        ...
        if (base.vtx.matrix() && base.vtx.blend())
            REPA(base.vtx) {
                ...
                REPA(matrix)
                if (matrix.c[i] == bone && blend.c[i] > 0) {
                    IndexWeight skin[] =
                        {
