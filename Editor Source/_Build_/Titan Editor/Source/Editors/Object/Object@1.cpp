...
/******************************************************************************/
void SlotMesh::set(C MeshPtr &mesh, C Str &name) {
    T.mesh = mesh;
    T.name = name;
}
void SlotMesh::draw(C Skeleton &skel) {
    if (mesh)
        if (C SkelSlot *slot = skel.findSlot(name)) {
            Matrix m = *slot;
            m.scaleOrn(scale);
            mesh->draw(m);
        }
...
void SlotMesh::draw(C AnimSkel &skel) {
    if (mesh)
        if (C OrientM *slot = skel.findSlot(name)) {
            Matrix m = *slot;
            m.scaleOrn(scale);
            mesh->draw(m);
        }
...
void SlotMesh::Set(MemPtr<SlotMesh> slot_meshes, cchar8 *name, C MeshPtr &mesh) {
    SlotMesh *sm = null;
    REPA(slot_meshes)
    if (slot_meshes[i].name == name) {
        if (!mesh) {
            slot_meshes.remove(i);
            return;
        }
        sm = &slot_meshes[i];
        break;
...
void ObjView::BackMesh::drawBlend() {
    if (mesh)
        mesh->drawBlend(matrix, &NoTemp(Vec4(1, 1, 1, ObjEdit.background_alpha())));
}
...
void ObjView::MeshChange::removeBone(C Str &name) {
    ...
    if (C EditSkeleton::Bone *bone_remove = ObjEdit.cur_skel_to_saved_skel.findBone(name)) // add to restore list
    {
        ...
        REPA(bone_restore) {
            ...
            if (InRange(src.index, ObjEdit.cur_skel_to_saved_skel.nodes))
                dest.name = ObjEdit.cur_skel_to_saved_skel.nodes[src.index].name;
...
void ObjView::MeshChange::apply(bool undo) {
    ...
    if (undo)
        REPA(bone_renames)
    ObjEdit.cur_skel_to_saved_skel.renameBone(bone_renames[i].dest, bone_renames[i].src); // rename back in reversed order
    else FREPA(bone_renames)
        ObjEdit.cur_skel_to_saved_skel.renameBone(bone_renames[i].src, bone_renames[i].dest); // rename      in normal   order

    // removals
    if (undo)
        REPA(bone_restores) // restore in reversed order
        {
            C BoneWeight &bone_restore = bone_restores[i];
            if (EditSkeleton::Bone *bone = ObjEdit.cur_skel_to_saved_skel.getBone(bone_restore.name))
                FREPA(bone_restore) {
                    C NameWeight &src = bone_restore[i];
                    bone->addWeight(ObjEdit.cur_skel_to_saved_skel.findNodeI(src.name), src.weight);
                }
        }
    else
        FREPA(bone_removals)
    ObjEdit.cur_skel_to_saved_skel.removeBone(bone_removals[i]); // remove in normal order
...
void ObjView::PhysChange::create(ptr user) {
    if (ObjEdit.phys) {
        data = *ObjEdit.phys;
        if (ObjEdit.phys_elm)
            if (ElmPhys *phys_data = ObjEdit.phys_elm->physData())
                T.phys_data = *phys_data;
    }
    ObjEdit.undoVis();
}
void ObjView::PhysChange::apply(ptr user) {
    if (ObjEdit.phys && ObjEdit.phys_elm)
        if (ElmPhys *phys_data = ObjEdit.phys_elm->physData()) {
...
void ObjView::undoVis() {
    SetUndo(mesh_undos, mesh_undo, mesh_redo);
    SetUndo(phys_undos, phys_undo, phys_redo);
}
...
bool ObjView::partVisible(int p, C MeshPart &part, bool allow_lit) C {
    ...
    if (mode() == PHYS && phys_tabs() == PHYS_TOGGLE) {
        if (Kb.shift() && part.part_flag & MSHP_NO_PHYS_BODY)
            return false;
        return true;
    }
    if (mode() == MESH || mode() == SKIN) {
        if (allow_lit && p == lit_part)
            return true;
        if (mesh_parts.edit_selected() && !mesh_parts.partOp(p))
            return false;
    }
...
Matrix ObjView::transformMatrix(bool sel) C {
...
Shape ObjView::changeSkinShape() C {
    ...
    case BS_SQUARE:
        return Rect_C(Ms.pos(), r);
    case BS_CIRCLE:
        return Circle(r, Ms.pos());
    default:
        return Shape();
    }
}
int ObjView::boneTabs() C {
    int v = bone_tabs();
    if (v != BONE_SCALE && v != BONE_DYNAMIC && Kb.alt())
        return BONE_ROT;
    if (v < 0)
        v = BONE_MOVE;
    return v;
...
int ObjView::boneAxis() C {
    if (Kb.alt())
        switch (mode()) {
        case BONES:
            return (bone_tabs() == BONE_SCALE) ? 1 : 2;
        case RAGDOLL:
            return (ragdoll_tabs() == RAGDOLL_SCALE) ? 1 : 2;
        }
    return bone_axis;
}
flt ObjView::posScale() C {
    if (mesh_elm)
        if (ElmMesh *mesh_data = mesh_elm->meshData())
            return mesh_data->posScale(); // because we're operating on meshes in original import matrix, we need to adjust by the transform scale
    return 1;
}
...
Vec ObjView::selMeshCenter() C {
    ...
    if (mode() == MESH) {
        ...
        REPA(mesh_parts.list.sel)
        if (C MeshPart *part = lod.parts.addr(mesh_parts.list.sel[i]))
            Include(box, is, part->base);
    }
...
void ObjView::setMatrixAtDist(Matrix &matrix, flt dist) {
    ...
    matrix = mesh_matrix;
    matrix.move(ActiveCam.matrix.pos + ActiveCam.matrix.z * dist - mesh.lod_center * mesh_matrix);
}
...
void ObjView::render() {
    ...
    case RM_PREPARE: {
        ...
        if (lodEditDist()) // LOD compare models to edit distance
        {
            ...
            if (InRange(lod - 1, mesh.lods()) && InRange(lod, mesh.lods())) {
                ...
                Matrix matrix;
                setMatrixAtDist(matrix, absLodDist(b));
                FREPA(draw_lod)
                if (!(draw_lod.parts[i].part_flag & MSHP_HIDDEN))
                    draw_lod.parts[i].draw(matrix, mesh_matrix_prev_ptr ? *mesh_matrix_prev_ptr : matrix);
                if (mesh_matrix_prev_ptr)
        ...
        if (mode() == SLOTS && mesh_skel)
            REPAO(slot_meshes).draw(*mesh_skel);

        if (mode() == BODY && body)
            body->draw(MatrixIdentity); // body is game mesh which is already in correct transform
    ...
    case RM_BLEND: {
        ...
        if (showChangeSkin()) {
            ...
            FREPA(lod) {
                MeshPart &part = lod.parts[i];
                if (partVisible(i, lod.parts[i]))
                    part.drawBoneHighlight(bone);
            }
        }

        // draw preview
        if (ListElm *list_elm = Proj.list.visToData(Proj.list.lit))
            if (Elm *obj = list_elm->elm)
        ...
        if (background_alpha() > 0 && background_alpha() < 1)
            REPAO(back_meshes).drawBlend();
    } break;
    }
    particles.draw();
}
void ObjView::Draw(Viewport &viewport) {
    if (Edit::Viewport4::View *view = ObjEdit.v4.getView(&viewport))
        ObjEdit.draw(*view);
}
void ObjView::draw(Edit::Viewport4::View &view) {
    ...
    if (mode() == TRANSFORM) {
        SetMatrix();
        D.depthLock(false);
        DrawMatrix(trans.drawMatrix(), trans_axis);
        D.depthUnlock();
    }
    if (transMesh()) {
        SetMatrix();
        D.depthLock(false);
        DrawMatrix(trans_mesh.drawMatrix(), trans_axis);
        D.depthUnlock();
    }

    // draw boxes
    if (showBoxes()) {
        if (showMainBox() && mesh.is()) {
            SetMatrix();
            mesh_box.draw();
            if (mode() == TRANSFORM) {
                SetMatrix(trans.matrix);
                mesh_box.draw(YELLOW);
            }
        }
        MeshLod &lod = getLod();
        REPA(lod) {
            MeshPart &part = lod.parts[i];
            if (partVisible(i, part)) {
                bool lit = (i == lit_part), sel = partSel(i);
                if (lit || sel) {
                    Box box;
                    if (part.base.getBox(box, transformMatrix(partOp(i)))) {
    ...
    if (vtxs() && !customDrawMatrix()) {
        ...
        REPAD(p, lod) {
            ...
            if (partVisible(p, part)) {
                bool lit = (p == lit_part), sel = partSel(p);
                Matrix m = transformMatrix(partOp(p)), cam = ActiveCam.matrix / m;
                SetMatrix(m);
                    ...
                    if (C Vec *pos = base.vtx.pos())
                        if (C Vec *nrm = base.vtx.nrm()) {
                            ...
                            REPA(base.vtx) {
                                ...
                                if (!vtxs_front() || frontFace(p, &n, cam))
                                    VI.line(p, p + n * l);
                            }
                            VI.flush();
                        }
    ...
    if (editMeshParts())
        if (sel_vtx.elms() || sel_face.elms()) {
            ...
            REPA(sel_face) {
                C VecI2 &v = sel_face[i];
                if (InRange(v.x, lod)) {
                    C MeshPart &part = lod.parts[v.x];
                    if (partVisible(v.x, part))
                        if (C Vec *pos = part.base.vtx.pos()) {
                            if (v.y & SIGN_BIT) {
                                int f = (v.y ^ SIGN_BIT);
                                if (InRange(f, part.base.quad)) {
                                    ...
                                    if (trans_mesh.move_along_normal)
                                        if (C Vec *nrm = part.base.vtx.nrm()) {
                                            a.p[0] += nrm[v.x] * trans_mesh.move_along_normal;
                                            a.p[1] += nrm[v.y] * trans_mesh.move_along_normal;
                                            a.p[2] += nrm[v.w] * trans_mesh.move_along_normal;
                                            b.p[1] += nrm[v.z] * trans_mesh.move_along_normal;
                                            b.p[0] = a.p[1];
                                            b.p[2] = a.p[2];
                                        }
                                    VI.tri(a);
                                ...
                                if (InRange(f, part.base.tri)) {
                                    ...
                                    if (trans_mesh.move_along_normal)
                                        if (C Vec *nrm = part.base.vtx.nrm()) {
                                            a.p[0] += nrm[v.x] * trans_mesh.move_along_normal;
                                            a.p[1] += nrm[v.y] * trans_mesh.move_along_normal;
                                            a.p[2] += nrm[v.z] * trans_mesh.move_along_normal;
                                        }
                                    VI.tri(a);
                                }
                            }
    ...
    if (lit_vtx >= 0 || lit_face != -1)
        if (C MeshPart *part = getPart(lit_vf_part))
            if (C Vec *pos = part->base.vtx.pos()) {
                D.depthLock(false);
                Matrix matrix = transformMatrix(partOp(lit_vf_part)), cam = ActiveCam.matrix / matrix;
                SetMatrix(matrix);
                Memt<int> lit;
                ...
                if (lit.elms()) {
                    REPA(lit) {
                        int vtx = lit[i];
                        VI.dot(sel_vtx.binaryHas(VecI2(lit_vf_part, vtx)) ? SelColor : LitColor, pos[vtx], 0.005f);
                    }
                    VI.end();
                }
                if (InRange(lit_vtx, part->base.vtx))
                    part->base.vtx.pos(lit_vtx).draw(sel_vtx.binaryHas(VecI2(lit_vf_part, lit_vtx)) ? SelColor : LitColor, 0.01f);
    ...
    if (mode() == SLOTS || mode() == SKIN || mode() == BONES)
        if (Skeleton *skel = getVisSkel()) {
            ...
            bool bone = (mode() == BONES && (bone_tabs == BONE_ADD || bone_tabs == BONE_MOVE || bone_tabs == BONE_ROT || bone_tabs == BONE_SCALE) || mode() == SKIN),
            ...
            byte alpha = (bone_shape ? 50 : (mode() == SKIN && skin_tabs() == SKIN_SEL_BONE || mode() == BONES) ? 153
                                                                                                                : 100);
            ...
            FREPA(skel->bones) {
                ...
                Color col;
                if (bone.flag & mask)
                    col.set(255, 0, 0, (lit_bone == i) ? 255 : alpha);
                else if (sel_bone == i && lit_bone == i)
                    col = LitSelColor;
                else if (sel_bone == i)
                    col = SelColor;
                else if (lit_bone == i)
            ...
            if (mode() == SLOTS)
                FREPAO(skel->slots).draw((sel_slot == i && lit_slot == i) ? LitSelColor : (sel_slot == i || lit_slot == i) ? SelColor
                                                                                                                           : Color(255, 128, 0, 255),
                                         SkelSlotSize);
            if (InRange(sel_bone, skel->bones) && mode() == BONES && (bone_tabs == BONE_MOVE || bone_tabs == BONE_ROT || bone_tabs == BONE_SCALE)) {
                Matrix m = skel->bones[sel_bone];
                m.scaleOrn(SkelSlotSize);
                DrawMatrix(m, boneAxis());
            }
            if (InRange(sel_slot, skel->slots)) {
                Matrix m = skel->slots[sel_slot];
                m.scaleOrn(SkelSlotSize);
                DrawMatrix(m, slot_axis);
            }
            // draw parent<->children line
            if (C SkelBone *bone = skel->bones.addr(lit_bone))
                if (C SkelBone *parent = skel->bones.addr(bone->parent))
                    D.line(PURPLE, bone->pos, parent->center());
            if (C SkelSlot *slot = skel->slots.addr(lit_slot)) {
                if (C SkelBone *parent = skel->bones.addr(slot->bone))
                    D.line(PURPLE, slot->pos, parent->center());
                if (slot->bone != slot->bone1)
                    if (C SkelBone *parent = skel->bones.addr(slot->bone1))
                        D.line(PURPLE, slot->pos, parent->center());
            }
            REPA(skel->bones)
            if (skel->bones[i].parent == lit_bone)
                D.line(WHITE, skel->bones[i].pos, skel->bones[lit_bone].to());
            D.depthUnlock();
        }
    ...
    if (mode() == RAGDOLL && mesh_skel) {
        D.depthLock(false);
        SetMatrix(trans.matrix);
        FREPA(mesh_skel->bones)
        if (mesh_skel->bones[i].flag & BONE_RAGDOLL)
            mesh_skel->bones[i].shape.draw(ColorAlpha(ORANGE, (lit_bone == i) ? 1 : 0.55f), false, 16);
        FREPAO(mesh_skel->bones).draw(ColorAlpha(FlagOn(mesh_skel->bones[i].flag, BONE_RAGDOLL) ? RED : CYAN, (lit_bone == i) ? 1 : 0.60f));
        if (ragdoll_tabs() != RAGDOLL_TOGGLE)
            if (InRange(sel_bone, mesh_skel->bones)) {
                Matrix m = mesh_skel->bones[sel_bone];
                m.scaleOrn(SkelSlotSize);
                DrawMatrix(m, boneAxis());
            }
        D.depthUnlock();
    ...
    if (mode() == PHYS) {
        ...
        if (Gui.ms() == &phys_capsule && Kb.alt() && mesh_skel) {
            Capsule capsule;
            if (DefaultController(capsule, *mesh_skel))
                capsule.draw(ORANGE);
        }
        if (phys) {
            FREPAO(phys->parts).draw((sel_phys == i) ? LitSelColor : (lit_phys == i) ? SelColor
                                                                                     : LitColor);
            if (InRange(sel_phys, phys->parts))
                DrawMatrix(phys_part_matrix, phys_axis);
        }
        D.depthUnlock();
    }

    // cursor position
    if (show_cur_pos() && has_cur_pos && lit_vtx < 0) {
        SetMatrix();
        cur_pos.draw(Color(255, 112), 0.005f);
    } // don't draw when we're highlighting a vertex because it's the same position

    D.lineSmooth(line_smooth);
}
void ObjView::setLodTabsPos() {
    if (mode() == GROUP) {
        lod_tabs.move(goto_group.rect().down() - Vec2(0, 0.01f) - lod_tabs.rect().up());
    }
    if (mode() == VARIATIONS || mode() == REMOVE || mode() == MESH || mode() == SKIN) {
        lod_tabs.move(mode.rect().down() - Vec2(0, 0.01f) - lod_tabs.rect().up());
    }

    if (mode() == MESH && lod_tabs.rect().min.x < mesh_ops.rect().max.x)
        lod_tabs.pos(mesh_ops.rect().ru());
    if (mode() == SKIN && lod_tabs.rect().min.x < skin_ops.rect().max.x)
        lod_tabs.pos(skin_ops.rect().ru());
}
        ...
        if (ElmMesh *mesh_data = editor.mesh_elm->meshData())
            if (mesh_data->body_id.valid()) {
                if (editor.mode() == TRANSFORM)
                    Gui.msgBox(S, "This object is interpreted as cloth/armor to a different object (body).\nTherefore you cannot transform it.\nPlease transform the body object for this cloth.");
                if (editor.mode() == SLOTS || editor.mode() == BONES || editor.mode() == RAGDOLL)
                    Gui.msgBox(S, "This object is interpreted as cloth/armor to a different object (body).\nTherefore it doesn't have its own skeleton.\nPlease edit skeleton on the body object for this cloth.");
                editor.mode.set(-1);
            }
    if (editor.mode() == TRANSFORM || editor.mode() == MESH)
        editor.mode.tab(editor.mode()) += editor.trans_tabs;

    if (editor.mode() == MESH || editor.mode() == SKIN)
        editor.mode.tab(editor.mode()) += editor.mesh_parts;

    if (editor.mode() == GROUP && OpObj == OP_OBJ_NONE && editor.group.getSetGroup() >= 0)
        SetObjOp(OP_OBJ_SET_GROUP);

    if (editor.mode() == REMOVE || editor.mode() == MESH || editor.mode() == SKIN)
        SetObjOp(OP_OBJ_NONE);

    // lod_tabs
    if (editor.mode() == GROUP || editor.mode() == REMOVE || editor.mode() == MESH || editor.mode() == VARIATIONS || editor.mode() == SKIN) {
        editor.mode.tab(editor.mode()) += editor.lod_tabs;
        editor.setLodTabsPos();
    }

    if (editor.mode() == MESH || editor.mode() == VARIATIONS) {
...
void ObjView::Undo(ObjView &editor) {
    if (editor.mode() == PHYS)
        editor.phys_undos.undo();
    else if (editor.mode() == PARAM)
        editor.param_edit.undos.undo();
    else
        editor.mesh_undos.undo();
}
void ObjView::Redo(ObjView &editor) {
    if (editor.mode() == PHYS)
        editor.phys_undos.redo();
    else if (editor.mode() == PARAM)
        editor.param_edit.undos.redo();
    else
        editor.mesh_undos.redo();
}
...
void ObjView::VtxsChanged(ObjView &editor) {
    editor.vtxs_front.visible(editor.vtxs());
    editor.vtxs_normals.visible(editor.vtxs());
}
...
void ObjView::remVtx(MESH_FLAG flag, bool only_selected, C MaterialPtr &mtrl) {
    ...
    bool changed = false;
    REPD(i, mesh.lods())
    if (!only_selected || ObjEdit.selLod() == i || !ObjEdit.visibleLodSelection()) {
        MeshLod &lod = mesh.lod(i);
        REPA(lod.parts)
        if (!only_selected || ObjEdit.partOp(i) || !ObjEdit.mesh_parts.visibleOnActiveDesktop()) {
            MeshPart &part = lod.parts[i];
            if (!mtrl || HasMaterial(part, mtrl))
                if (FlagOn(part.base.flag() | part.render.flag(), flag)) {
                    part.exclude(flag);
                    changed = true;
                }
        }
    ...
    if (changed) {
        if ((flag & VTX_SKIN) && !(mesh.flag() & VTX_SKIN))
            mesh.clearSkeleton();
        setChangedMesh(true, false);
    }
}
void ObjView::MeshUndo(ObjView &editor) {
    editor.mesh_undos_undo = true;
    editor.mesh_undos.undo();
}
void ObjView::MeshRedo(ObjView &editor) {
    editor.mesh_undos_undo = false;
    editor.mesh_undos.redo();
}
...
void ObjView::setLodDist(MeshLod &lod, flt dist) {
    lodDist(lod, dist);
    setChangedMesh(true, false);
}
C MeshLod &ObjView::getLod() C { return ConstCast(T).getLod(); }
MeshLod &ObjView::getLod() {
    if (mode() == GROUP || mode() == REMOVE || mode() == MESH || mode() == SKIN || mode() == LOD || mode() == VARIATIONS)
        return mesh.lod(selLod());
    return mesh;
}
C MeshLod &ObjView::getDrawLod() C { return ConstCast(T).getDrawLod(); }
MeshLod &ObjView::getDrawLod() {
    if (NewLod.visible() && NewLod.preview && NewLod.simplified_valid)
        return NewLod.simplified;
    if (MeshAO.visible() && MeshAO.preview && MeshAO.baked_valid)
        return MeshAO.baked;
    if (InRange(lit_lod, mesh.lods()))
        return mesh.lod(lit_lod);
    return getLod();
}
...
int ObjView::getPart(GuiObj *go, C Vec2 &screen_pos, Vec *hit_pos) {
    ...
    if (Edit::Viewport4::View *view = v4.getView(go)) {
        ...
        REPA(lod)
        if (partVisible(i, lod.parts[i]))
            if (Sweep(pos, dir, lod.parts[i], &NoTemp(transformMatrix(partOp(i))), &f, &hp, null, true, -1, false))
                if (hit_part < 0 || f < frac) {
                    hit_part = i;
                    frac = f;
                    if (hit_pos)
                        *hit_pos = hp;
    ...
    if (Edit::Viewport4::View *view = v4.getView(go)) {
        ...
        if (hit_vtx) {
            REPAD(p, lod) {
                C MeshPart &part = lod.parts[p];
                if (partVisible(p, part))
                    if (part.base.vtx.pos())
                        REPA(part.base.vtx) {
                            C Vec &pos = part.base.vtx.pos(i);
                            if (!vtxs_front() || frontFace(pos, part.base.vtx.nrm() ? &part.base.vtx.nrm(i) : null, cam)) {
                                Vec world_pos = pos * matrix;
                                Vec2 s;
                                if (PosToScreen(world_pos, s)) {
                                    d = Dist2(s, screen_pos);
                                    if (hit_part < 0 || d < dist) {
                                        *hit_vtx = i;
                                        hit_part = p;
                                        if (hit_pos)
                                            *hit_pos = world_pos;
        ...
        if (hit_face && hit_part < 0) {
            ...
            REPAD(p, lod) {
                C MeshPart &part = lod.parts[p];
                if (partVisible(p, part))
                    if (Sweep(pos, dir, part, &matrix, &d, &hp, &f, true, -1, false))
                        if (hit_part < 0 || d < dist) {
                            *hit_face = f;
                            hit_part = p;
                            if (hit_pos)
        ...
        if (Edit::Viewport4::View *view = v4.getView(go)) {
            ...
            if (bone_i) {
                ...
                REPA(skel->bones) {
                    ...
                    if (bone_shape) {
                        ...
                        flt d;
                        if (Distance2D(screen_pos, (bone_shape ? bone.shape.fullEdge() : Edge(bone.pos, bone.to())) * trans.matrix, d, 0))
                            if (*bone_i < 0 || d < dist) {
                                *bone_i = i;
                                dist = d;
                            }
            ...
            if (slot_i) {
                REPA(skel->slots) {
                    ...
                    flt d;
                    if (Distance2D(screen_pos, Edge(slot.pos, slot.pos + slot.perp * SkelSlotSize) * trans.matrix, d, 0))
                        if (*slot_i < 0 && (!bone_i || *bone_i < 0) || d < dist) {
                            *slot_i = i;
                            dist = d;
                            if (bone_i)
                                *bone_i = -1;
            ...
            if (bone_i && *bone_i >= 0 && !bone_shape || slot_i && *slot_i >= 0)
                if (dist > 0.04f) {
                    if (bone_i)
                        *bone_i = -1;
                    if (slot_i)
...
MeshPart *ObjView::validateDup(int part) {
    ...
    if (p) {
        if (!(p->flag() & VTX_DUP) || vtx_dup_mode != vtx_face_sel_mode()) {
            ...
            case VFS_NRM:
                flag = VTX_NRM;
                break; // must share the same normal
            case VFS_TEX0:
                flag = VTX_TEX0;
                break; // must share the same UV
            case VFS_TEX1:
                flag = VTX_TEX1;
                break; // must share the same UV1
            case VFS_TEX2:
                flag = VTX_TEX2;
                break; // must share the same UV2
            case VFS_TEX3:
                flag = VTX_TEX3;
                break; // must share the same UV3
            case VFS_ALL:
                flag = MESH_NONE;
                break; // ALL = no requirements
            }
...
void ObjView::getVtxNeighbors(MemPtr<int> vtxs, int vtx, int part) {
    ...
    if (Kb.b(KB_Z) && vtx_face_sel_mode() >= VFS_TEX0) // select from entire mesh (not just neighbors)
    {
        if (C MeshPart *p = getPart(part)) {
            C MeshBase &base = p->base;
            if (InRange(vtx, base.vtx))
                switch (vtx_face_sel_mode()) {
                case VFS_TEX0:
                case VFS_TEX1:
                case VFS_TEX2:
                case VFS_TEX3:
                    if (C Vec2 *tex = (vtx_face_sel_mode() == VFS_TEX0) ? base.vtx.tex0() : (vtx_face_sel_mode() == VFS_TEX1) ? base.vtx.tex1()
                                                                                        : (vtx_face_sel_mode() == VFS_TEX2)   ? base.vtx.tex2()
                                                                                                                              : base.vtx.tex3()) {
                        C Vec2 &t = tex[vtx];
                        FREPA(base.vtx)
                        if (Equal(t, tex[i]))
                    ...
                    break;
                case VFS_ALL:
                    vtxs.setNum(base.vtxs());
                    REPAO(vtxs) = i;
                    break;
                }
        }
    } else if (vtx_face_sel_mode() == VFS_SINGLE) {
        if (vtx >= 0)
            vtxs.add(vtx);
    } else if (C MeshPart *p = validateDup(part))
        p->base.getVtxNeighbors(vtx, vtxs);
}
void ObjView::getFaceNeighbors(MemPtr<int> faces, int face, int part) {
    ...
    if (Kb.b(KB_Z) && vtx_face_sel_mode() >= VFS_TEX0) // select from entire mesh (not just neighbors)
    {
        if (C MeshPart *p = getPart(part)) {
            C MeshBase &base = p->base;
            if ((face & SIGN_BIT) ? InRange(face ^ SIGN_BIT, base.quad) : InRange(face, base.tri))
                switch (vtx_face_sel_mode()) {
                case VFS_TEX0:
                case VFS_TEX1:
                case VFS_TEX2:
                case VFS_TEX3:
                    if (C Vec2 *tex = (vtx_face_sel_mode() == VFS_TEX0) ? base.vtx.tex0() : (vtx_face_sel_mode() == VFS_TEX1) ? base.vtx.tex1()
                                                                                        : (vtx_face_sel_mode() == VFS_TEX2)   ? base.vtx.tex2()
                                                                                                                              : base.vtx.tex3()) {
                        C int *ind;
                        int inds; // vtx indexes for selected face
                        if (face & SIGN_BIT) {
                            ind = base.quad.ind(face ^ SIGN_BIT).c;
                            inds = 4;
                        } else {
                            ind = base.tri.ind(face).c;
                            inds = 3;
                        }
                        if (ind) {
                            Vec2 sel_tex[4];
                            int sel_texs = 0;
                            FREP(inds) // list of all unique texture coordinates in selected face
                            {
                                C Vec2 &face_vtx_tex = tex[ind[i]]; // face i-th vertex value
                                REP(sel_texs)
                                if (sel_tex[i] == face_vtx_tex)
                                    goto has;                       // check if has already
                                sel_tex[sel_texs++] = face_vtx_tex; // not found, so add it
                            has:;
                            }
                            FREPAD(t, base.tri) // iterate all triangles in mesh
                            {
                                C VecI &ind = base.tri.ind(t);
                                REPAD(v, ind) {
                                    C Vec2 &vtx_tex = tex[ind.c[v]];
                                    REPD(s, sel_texs)
                                    if (Equal(vtx_tex, sel_tex[s])) {
                                        faces.add(t);
                                        goto added_tri;
                                    } // if vertex value is listed in desired selection then add this face
                                }
                            added_tri:;
                            }
                            FREPAD(q, base.quad) // iterate all quads in mesh
                            {
                                C VecI4 &ind = base.quad.ind(q);
                                REPAD(v, ind) {
                                    C Vec2 &vtx_tex = tex[ind.c[v]];
                                    REPD(s, sel_texs)
                                    if (Equal(vtx_tex, sel_tex[s])) {
                                        faces.add(q ^ SIGN_BIT);
                                        goto added_quad;
                                    } // if vertex value is listed in desired selection then add this face
                                }
                            added_quad:;
                    ...
                    break;
                case VFS_ALL:
                    faces.setNum(base.faces());
                    FREPA(base.tri)
                    faces[i] = i;
                    FREPA(base.quad)
                    faces[base.tris() + i] = i ^ SIGN_BIT;
                    break;
                }
        }
    } else if (vtx_face_sel_mode() == VFS_SINGLE) {
        if (face != -1)
            faces.add(face);
    } else if (MeshPart *p = validateDup(part))
        p->base.getFaceNeighbors(face, faces);
}
    ...
    if (from_faces) {
        ...
        REPA(sel_face) {
            ...
            if (MeshPart *part = lod.parts.addr(v.x)) {
                ...
                if (v.y & SIGN_BIT) {
                    int f = v.y ^ SIGN_BIT;
                    if (InRange(f, base.quad)) {
                        C VecI4 &q = base.quad.ind(f);
                        REPA(q)
                        vtxs.binaryInclude(VecI2(v.x, q.c[i]));
                    }
                } else {
                    int f = v.y;
                    if (InRange(f, base.tri)) {
                        C VecI &t = base.tri.ind(f);
                        REPA(t)
                        vtxs.binaryInclude(VecI2(v.x, t.c[i]));
                    }
                }
            }
        }
...
void ObjView::getSamePos(int part, int vtx, MemPtr<VecI2> vtxs) {
    ...
    flt pos_eps = posEps();
    C MeshLod &lod = getLod();
    if (C MeshPart *p = lod.parts.addr(part))
        if (InRange(vtx, p->base.vtx)) {
            C Vec &pos = p->base.vtx.pos(vtx);
            REPAD(p, lod) {
                C MeshPart &part = lod.parts[p];
                if (partVisible(p, part))
                    REPA(part.base.vtx)
                if (Equal(part.base.vtx.pos(i), pos, pos_eps))
                    vtxs.add(VecI2(p, i));
            }
        }
}
void ObjView::includeSamePos(int part, MemPtr<VecI2> vtxs) // !! assumes that 'vtxs' is sorted and valid (point to valid indexes) !!
{
    ...
    if (C MeshPart *p = lod.parts.addr(part))
        REPA(p->base.vtx) {
            C Vec &pos = p->base.vtx.pos(i);
            REPAD(v, vtxs) {
                C VecI2 &vtx = vtxs[v];
                if (Equal(pos, lod.parts[vtx.x].base.vtx.pos(vtx.y), pos_eps)) {
                    vtxs.binaryInclude(VecI2(part, i));
                    break;
                } // if share position then process this one too
            }
        }
}
...
void ObjView::selVFSet(int part, int vtx, int face, bool same_pos) {
    ...
    if (part >= 0) {
        if (vtx >= 0)
            if (!same_pos)
                sel_vtx.add(VecI2(part, vtx));
            else
                getSamePos(part, vtx, sel_vtx);
        if (face != -1)
            sel_face.add(VecI2(part, face));
    }
}
void ObjView::selVFToggle(int part, int vtx, int face, bool same_pos) {
    if (part >= 0) {
        if (vtx >= 0)
            if (!same_pos)
                sel_vtx.binaryToggle(VecI2(part, vtx));
            else {
                Memt<VecI2> vtxs;
                getSamePos(part, vtx, vtxs);
                REPA(vtxs)
                sel_vtx.binaryToggle(vtxs[i]);
            }
        if (face != -1)
            sel_face.binaryToggle(VecI2(part, face));
    }
}
void ObjView::selVFInclude(int part, int vtx, int face, bool same_pos) {
    if (part >= 0) {
        if (vtx >= 0)
            if (!same_pos)
                sel_vtx.binaryInclude(VecI2(part, vtx));
            else {
                Memt<VecI2> vtxs;
                getSamePos(part, vtx, vtxs);
                REPA(vtxs)
                sel_vtx.binaryInclude(vtxs[i]);
            }
        if (face != -1)
            sel_face.binaryInclude(VecI2(part, face));
    }
}
void ObjView::selVFExclude(int part, int vtx, int face, bool same_pos) {
    if (part >= 0) {
        if (vtx >= 0)
            if (!same_pos)
                sel_vtx.binaryExclude(VecI2(part, vtx));
            else {
                Memt<VecI2> vtxs;
                getSamePos(part, vtx, vtxs);
                REPA(vtxs)
                sel_vtx.binaryExclude(vtxs[i]);
            }
        if (face != -1)
            sel_face.binaryExclude(VecI2(part, face));
    }
}
void ObjView::selVFDo() {
    Memt<int> vtxs;
    getVtxNeighbors(vtxs, lit_vtx, lit_vf_part);
    Memt<int> faces;
    getFaceNeighbors(faces, lit_face, lit_vf_part);
    bool same_pos = (vtx_face_sel_mode() == VFS_SINGLE);
    if (mesh_parts.list.selMode() == LSM_SET)
        selVFClear();
    switch (mesh_parts.list.selMode()) {
    case LSM_SET:
    case LSM_INCLUDE:
        REPA(vtxs)
        selVFInclude(lit_vf_part, vtxs[i], -1, same_pos);
        REPA(faces)
        selVFInclude(lit_vf_part, -1, faces[i], same_pos);
        break;
    case LSM_EXCLUDE:
        REPA(vtxs)
        selVFExclude(lit_vf_part, vtxs[i], -1, same_pos);
        REPA(faces)
        selVFExclude(lit_vf_part, -1, faces[i], same_pos);
        break;
    case LSM_TOGGLE:
        REPA(vtxs)
        selVFToggle(lit_vf_part, vtxs[i], -1, same_pos);
        REPA(faces)
        selVFToggle(lit_vf_part, -1, faces[i], same_pos);
        break;
    }
}
...
void ObjView::selUndo() {
    ...
    if (MeshChange *change = mesh_undos.set(change_type, false, 1))
        if (same)
            change->selOnly();
}
...
void ObjView::setMenu(Node<MenuElm> &menu, C Str &prefix) {
    ...
    FREPA(menu.children)
    if (menu.children[i].name == prefix + "View") {
...
ObjView &ObjView::create() {
    MemStats mem;
    mem.get();
    long mem_limit = ((mem.total_phys > 0) ? mem.total_phys * 25 / 100 : 1024 * 1024 * 1024); // allocate 25% of RAM for Undo or 1 GB if RAM is unknown
    if (!X64)
        MIN(mem_limit, 400 * 1024 * 1024); // for 32-bit platforms limit to 400 MB
    mesh_undos.maxMemUsage(mem_limit);

    ts.reset().size = 0.045f;
    ts.align.set(1, 0);
    super::create(Draw, false, 0, PI, 1, 0.01f, 1000);
    v4.toggleHorizontal();
    flt h = 0.05f;
    ...
    axis.mode = BUTTON_TOGGLE;
    axis.set(true);
    axis.image = "Gui/Misc/axis.img";
    T += box.create(Rect_LU(axis.rect().ru(), h)).focusable(false).desc("Display object bounding box\nKeyboard Shortcut: Alt+B");
    box.mode = BUTTON_TOGGLE;
    box.image = "Gui/Misc/box.img";
    ...
    Property &body = body_props.New().create("Body Object", MemberDesc(DATA_STR));
    body.button.create("C").func(ClearBody, T).desc("Remove any current body this object is assigned to");
    AddProperties(body_props, mode.tab(BODY), mode.rect().ld() - Vec2(0, 0.01f), 0.05f, 0.90f, &ts);
    mode.tab(BODY) += goto_body.create(body.textline.rect()).func(GotoBody, T).focusable(false).desc("Click to open body in the editor.\nDrag and drop an object here to set it as body.");
    goto_body.text_align = 1;
    body.textline.del();
    ...
    Property &groups = group_props.New().create("Draw Groups Enum", MemberDesc(DATA_STR));
    groups.button.create("C").func(ClearGroups, T).desc("Remove draw group enum assigned to this object");
    ...
    goto_group.text_align = 1;
    groups.textline.del();

    mode.tab(BACKGROUND) += background_alpha.create(Rect_U(mode.rect().down() - Vec2(0, 0.01f), 0.2f, 0.055f), 0.5f);
    mode.tab(BACKGROUND) += background_alpha_t.create(background_alpha.rect().right() + Vec2(0.01f, 0), "Alpha", &ts);
...
void ObjView::createMesh() {
    ...
    mode.tab(MESH) += trans_mesh.create(true);
    trans_mesh.move(trans_tabs.rect().down() - Vec2(0, 0.01f) - trans_mesh.rect().up());
    ...
    REP(Min(Elms(vfs_desc), vtx_face_sel_mode.tabs()))
    vtx_face_sel_mode.tab(i).desc(vfs_desc[i]);

    {
        ...
        mode.tab(MESH) += mesh_ops.create(Rect_LU(vtx_face_sel_mode.rect().max.x + h, mode.rect().min.y - 0.01f, 0.25f, 0.055f), n).focusable(false);
        mesh_ops.text = "Operations";
        mesh_ops.flag |= COMBOBOX_CONST_TEXT;
    }

    {
        ...
        mode.tab(LOD) += lod_ops.create(Rect_RU(mode.rect().ld() - 0.01f, 0.25f * 0.9f, 0.055f * 0.9f), n).focusable(false);
        lod_ops.text = "Operations";
        lod_ops.flag |= COMBOBOX_CONST_TEXT;
    }
    ...
    REPA(skin_mode_desc)
    skin_tabs.tab(i).desc(skin_mode_desc[i]);
    {
        Node<MenuElm> n;
        ...
        n.New().create("Edit Selected", MeshEditSel, T).kbsc(KbSc(KB_E, KBSC_CTRL_CMD)).flag(MENU_HIDDEN);
        mode.tab(SKIN) += skin_ops.create(Rect_LU(skin_tabs.rect().max.x + h, mode.rect().min.y - 0.01f, 0.25f, 0.055f), n);
        skin_ops.text = "Operations";
        skin_ops.flag |= COMBOBOX_CONST_TEXT;
    }
...
void ObjView::flushObj() {
    if (obj_elm && (changed_obj || param_edit.changed)) {
        ...
        if (old_terrain != TerrainObj2(obj))
            Proj.rebuildEmbedForObj(obj_id); // if saving changed 'terrainObj'
        if (old_phys != PhysPath(obj))
            Proj.rebuildPathsForObj(obj_id, true); // if saving changed 'physPath', rebuild only for objects that don't override paths (if they override then it means that changing the base doesn't affect their path mode), we must rebuild this also for objects with final path mode set to ignore, in case we've just disabled paths
...
void ObjView::flushMeshSkel(SAVE_MODE save_mode) {
    if (save_mode != SAVE_AUTO) // can't flush the skeleton for auto-save because if we're in the middle of editing it, and accidentally removed a bone, then auto-save would trigger animation adjustment and remove that bone animations
    {
        ...
        if (skel_elm && changed_skel && mesh_skel) {
            ...
            Save(*mesh_skel, Proj.gamePath(skel_elm->id));
            Proj.savedGame(*skel_elm);
            Server.setElmLong(skel_elm->id);

            Mems<Mems<IndexWeight>> bone_weights;
            cur_skel_to_saved_skel.set(bone_weights, saved_skel, *mesh_skel, EditSkeleton::BONE_NAME_IS_NODE_NAME); // 'cur_skel_to_saved_skel' is always created from 'saved_skel' so node name matches bone name
            int old_bone_as_root = -1;
            if (InRange(edit_skel.root, edit_skel.nodes)) // if we want root
            {
                ...
                if (!InRange(saved_edit_skel.root, saved_edit_skel.nodes) || saved_edit_skel.nodes[saved_edit_skel.root].name != new_root.name) // saved didn't have root or had different one
                {
                    ...
                    if (old_root_i >= 0)
                        FREPA(saved_edit_skel.bones) // iterate all old bones, it's important to start from 0 to process parents first, so we will stop on the first parent linked to 'old_root_i'
                        {
                            C EditSkeleton::Bone &saved_bone = saved_edit_skel.bones[i];
                            REPA(saved_bone)
                            if (saved_bone[i].index == old_root_i) // if found any bone linked to 'old_root_i'
                            {
                                if (saved_bone.elms() == 1)
                                    old_bone_as_root = saved_skel.findBoneI(saved_bone.name); // we can accept it as root only if it has only 1 link
        ...
        old_bone_as_root_set:
            ...
            cur_skel_to_saved_skel.create(saved_skel); // we've saved, so 'saved_skel' should now be set to 'mesh_skel'
        ...
        if (mesh_elm && changed_mesh) {
            ...
            // save mesh
            Mesh edit;
            edit.create(mesh).keepOnly(EditMeshFlagAnd).delRender().drawGroupEnum(draw_group);
            Save(edit, Proj.editPath(mesh_elm->id), Proj.game_path); // edit
            Mesh game;
            EditToGameMesh(edit, game, body_skel, draw_group, &mesh_matrix);
            Save(game, Proj.gamePath(mesh_elm->id));
            Proj.savedGame(*mesh_elm); // game
            bool changed_mesh_file = false;
            if (mesh_data) {
                if (Sync(mesh_data->file_time, mesh_file_time))
                    changed_mesh_file = true; // we're operating on temporary memory 'mesh' (not on file) so adjust modification time only at saving (so background send will not use different time+file)
                mesh_data->newVer();
                mesh_data->from(game); // modify just before saving/sending in case we've received data from server after edit
            }
            Proj.meshChanged(*mesh_elm); // call 'Proj.meshChanged' instead of 'Proj.elmChanged' so 'ObjEdit.elmChanged' won't get called
            if (changed_mesh_file)
...
void ObjView::flushPhys() {
    if (phys_elm && changed_phys && phys) {
        Save(*phys, Proj.gamePath(phys_elm->id));
        Proj.savedGame(*phys_elm);
        if (ElmPhys *ep = phys_elm->physData()) {
            ...
            ep->newVer();
            ep->from(*phys); // modify just before saving/sending in case we've received data from server after edit
        }
...
void ObjView::flush(SAVE_MODE save_mode) {
    flushPhys();
    flushMeshSkel(save_mode);
    flushObj(); // flush starting from base-most element because parents rely on them existing
}
void ObjView::setChangedObj() {
    if (obj_elm) {
        changed_obj = true;
...
void ObjView::setChangedMesh(bool file, bool box) {
    if (mesh_elm) {
        changed_mesh = true;
        if (ElmMesh *mesh_data = mesh_elm->meshData())
            mesh_data->fromMtrl(mesh);
        if (file) {
            mesh_file_time.getUTC();
            mesh_parts.refresh();
...
void ObjView::setChangedSkel(bool bone) {
    if (skel_elm) {
        changed_skel = true;
        skel_file_time.getUTC();
        if (bone && mesh_skel)
            mesh_skel->setBoneShapes();
        if (bone) {
            toGuiSkel();
...
void ObjView::setBox() {
    MeshBase temp;
    REPA(mesh)
    if (!(mesh.parts[i].part_flag & MSHP_HIDDEN))
        temp += mesh.parts[i];
    if (!temp.transform(mesh_matrix).getBox(mesh_box))
        mesh_box.zero();
}
void ObjView::gotoPhysMtrl() {
    if (phys_elm)
        if (ElmPhys *phys_data = phys_elm->physData())
            if (Elm *phys_mtrl = Proj.findElm(phys_data->mtrl_id))
                PhysMtrlEdit.set(phys_mtrl);
}
void ObjView::gotoBody() {
    if (mesh_elm)
        if (ElmMesh *mesh_data = mesh_elm->meshData())
            if (Elm *body_obj = Proj.meshToObjElm(mesh_data->body_id))
                set(body_obj);
}
void ObjView::gotoDrawGroupEnum() {
    if (mesh_elm)
        if (ElmMesh *mesh_data = mesh_elm->meshData())
            if (Elm *draw_group = Proj.findElm(mesh_data->draw_group_id))
                EnumEdit.set(draw_group);
}
    ...
    if (!mtrl_id.valid() || mtrl_elm)    // no material or found material
        if (mtrl_id.valid() || phys_elm) // some material or we have phys already (this will skip creating phys elm if we want to clear material)
            if (getPhysElm())            // create if needed
                if (ElmPhys *phys_data = phys_elm->physData())
                    if (phys_data->mtrl_id != mtrl_id) // if different
                    {
                        phys_undos.set("mtrl");
        ...
        if (body_id.valid() || mesh_elm) // some body or we have mesh already (this will skip creating mesh elm if we want to clear body)
            if (getMeshElm())            // create if needed
            {
                ...
                if (!actual_body_id.valid() || actual_body) // no body or found body
                    if (ElmMesh *mesh_data = mesh_elm->meshData())
                        if (mesh_data->body_id != actual_body_id) // if different
                        {
                            mesh_undos.set("body");
                            ...
                            mesh_matrix = mesh_data->transform();
                            setBox();
                            setPhysPartMatrix();
                        }
            }
...
void ObjView::setDrawGroupEnum(C UID &enum_id) {
    ...
    if (!enum_id.valid() || enum_elm)    // no enum or found enum
        if (enum_id.valid() || mesh_elm) // some enum or we have mesh already (this will skip creating mesh elm if we want to clear enum)
            if (getMeshElm())            // create if needed
                if (ElmMesh *mesh_data = mesh_elm->meshData())
                    if (mesh_data->draw_group_id != enum_id) // if different
                    {
...
void ObjView::animTargetObj() {
    if (obj_elm) {
        ...
        if (skel_id.valid())
            Proj.animSetTargetSkel(menu_ids, skel_id);
        else
            Gui.msgBox(S, "Can't set target object because it doesn't have a mesh skeleton.");
    }
}
void ObjView::resetTransform() {
    ...
    if (mesh_elm)
        if (ElmMesh *mesh_data = mesh_elm->meshData()) {
            ...
            mesh_data->transform.reset();
            mesh_data->transform_time.now();
            setChangedMesh(false, false);
            Mesh game;
            EditToGameMesh(mesh, game, body_skel, Proj.getEnum(mesh_data->draw_group_id), &mesh_matrix);
            Saved(game, Proj.gamePath(mesh_elm->id)); // here 'Saved' is used on purpose instead of 'Save', to copy mesh to cache, actual save will happen later
...
void ObjView::camCenter(bool zoom) {
    Vec hit_pos;
    bool hit = (getPart(Gui.ms(), Ms.pos(), &hit_pos) >= 0);
    flt dist;
    v4.moveTo(hit ? hit_pos : selMeshCenter());
    if (zoom && selectionZoom(dist))
        v4.dist(dist);
}
...
void ObjView::syncObj() {
    EditObject temp;
    if (temp.load(Proj.editPath(obj_id)))
        if (param_edit.p->sync(temp, Proj.edit_path)) {
            if (!param_edit.param_window.contains(Gui.kb()))
                param_edit.toGui(); // if we're editing param, then don't refresh gui
        }
}
void ObjView::reloadMeshSkel() {
    ...
    ElmMesh *mesh_data = (mesh_elm ? mesh_elm->meshData() : null);
    skel_elm = (mesh_data ? Proj.findElm(mesh_data->skel_id) : null);
    ElmSkel *skel_data = (skel_elm ? skel_elm->skelData() : null);
    if (mesh_data)
        mesh_file_time = mesh_data->file_time;
    else
        mesh_file_time.zero();
    if (skel_data)
        skel_file_time = skel_data->file_time;
    else
        skel_file_time.zero();

    Proj.getMeshSkels(mesh_data, &mesh_skel, &body_skel);
    if (mesh_skel) {
        mesh_skel_temp = *mesh_skel;
        mesh_skel = &mesh_skel_temp;
        saved_skel = *mesh_skel;
    } else {
        mesh_skel_temp.del();
        saved_skel.del();
    ...
    if (skel_elm)
        edit_skel.load(Proj.editPath(skel_elm->id));
    else
        edit_skel.del();
    saved_edit_skel = edit_skel;
    ...
    if (mesh_elm) {
        ...
        mesh.skeleton(mesh_skel).skeleton(null) // adjust mapping to skeleton in case that one got changed in the meantime
            .setTanBinDbl()
            .setRender(); // tan/bin needed for rendering (set always so we don't have to recalc when changing materials depending if they're needed or not)
    ...
    if (mesh_data)
        ...
        mesh_matrix.identity();
    if (skel_data)
        saved_skel_matrix = skel_data->transform();
    else
        saved_skel_matrix = mesh_matrix;
    if (mesh_data && mesh_data->body_id.valid() && (mode() == TRANSFORM /*|| mode()==SLOTS || mode()==BONES*/ || mode() == RAGDOLL))
        mode.set(-1); // disable unavailable modes if it's a cloth/armor object
    setBox();
    lod.toGui();
    toGuiSkel();
    mesh_parts.refresh();
...
Str ObjView::nodeDisplayName(int node_i) C {
    Str name;
    if (C EditSkeleton::Node *node = edit_skel.nodes.addr(node_i))
        name += node->name;
    FREPA(edit_skel.bones) // go from the start to find the first parent linked with this node
    {
        ...
        FREPA(bone)
        if (bone[i].index == node_i) // if bone is linked to this node
        {
            if (bone.elms() == 1 && name != bone.name)
                name += S + " (" + bone.name + ")"; // add bone name only if it's a direct link and is different than node name
...
void ObjView::addBoneRootData(int parent, int max_depth) {
    ...
    {
        C EditSkeleton::Node &node = edit_skel.nodes[i];
    ...
    if (--max_depth > 0)
        FREPA(edit_skel.nodes)
    if (edit_skel.nodes[i].parent == parent)
        addBoneRootData(i, max_depth); // add in order
}
void ObjView::setBoneRootTextSize() {
    bone_root.text_size = 1;
    flt text_size, text_padd;
    if (bone_root.textParams(text_size, text_padd)) {
        flt text_width = bone_root.textWidth(), rect_width = bone_root.rect().w() - text_padd * 2;
        if (text_width > rect_width)
            bone_root.text_size = rect_width / text_width;
    }
}
void ObjView::toGuiSkel() {
    ...
    if (InRange(edit_skel.root, edit_skel.nodes))
        bone_root.setText(nodeDisplayName(edit_skel.root), true, QUIET);
    else
        bone_root.set(0, QUIET);
    setBoneRootTextSize();
}
...
void ObjView::set(Elm *elm) {
    ...
    if (obj_elm != elm) {
        ...
        // setup elements
        obj_elm = elm;
        obj_id = (obj_elm ? obj_elm->id : UIDZero);
        reload();

        adjust_bone_orns.hide();

        sel_bone = (mesh_skel ? mesh_skel->findBoneI(sel_bone_name) : -1);
        sel_bone_vis = boneToVis(sel_bone);
...
void ObjView::resetDrawGroupEnum() {
    if (mesh_elm)
        if (ElmMesh *mesh_data = mesh_elm->meshData())
            mesh.drawGroupEnum(Proj.getEnum(mesh_data->draw_group_id));
}
void ObjView::enumChanged(C UID &enum_id) {
    param_edit.enumChanged();
    if (enum_id.valid() && mesh_elm)
        if (ElmMesh *mesh_data = mesh_elm->meshData())
            if (enum_id == mesh_data->draw_group_id)
                resetDrawGroupEnum();
}
...
void ObjView::dragPhysMtrl(Memc<UID> &elms) {
    FREPA(elms)
    if (Elm *phys_mtrl = Proj.findElm(elms[i], ELM_PHYS_MTRL)) {
...
void ObjView::dragDrawGroup(Memc<UID> &elms) {
    FREPA(elms)
    if (Elm *draw_group_enum = Proj.findElm(elms[i], ELM_ENUM)) {
...
void ObjView::dragBody(Memc<UID> &elms) {
    FREPA(elms)
    if (Elm *obj = Proj.findElm(elms[i]))
        if (ElmObj *obj_data = obj->objData()) {
...
void ObjView::setMaterial(int part_i, C MaterialPtr &material) {
    ...
    if (InRange(part_i, lod)) {
        ...
        if (set_all_parts) {
            ...
            REPD(l, mesh.lods()) {
                MeshLod &lod1 = mesh.lod(l);
                if (set_all_lods || &lod1 == &lod)
                    REPA(lod1) {
            ...
            if (set_all_lods)
                REPD(l, mesh.lods()) {
                    MeshLod &lod1 = mesh.lod(l);
                    if (&lod1 != &lod && lod.parts.elms() == lod1.parts.elms()) {
                        REPA(lod1) {
                            C MeshPart &part = lod.parts[i], &part1 = lod1.parts[i];
                            if (part.variation(variation) != part1.variation(variation) || !Equal(part.name, part1.name))
                                goto different;
                        }
...
void ObjView::drag(Memc<UID> &elms, GuiObj *&focus_obj, C Vec2 &screen_pos) {
    REPA(v4.view)
    if (focus_obj == &v4.view[i].viewport) // apply on model
    {
        MeshLod &lod = getLod();
        ...
        if (mode() == BACKGROUND) {
            ...
            if (anim)
                n.New().create("Link Animations with this Object", AnimTargetObj, T);
            if (n.children.elms()) {
                menu_ids = elms;
                Gui += menu.create(n);
    ...
    if (focus_obj == &goto_phys_mtrl)
        dragPhysMtrl(elms);
    else if (focus_obj == &goto_body)
        dragBody(elms);
    else if (focus_obj == &goto_group || group.contains(focus_obj))
        dragDrawGroup(elms);
...
void ObjView::drop(Memc<Str> &names, GuiObj *focus_obj, C Vec2 &screen_pos) {
    ...
    if (obj_elm)
        REPA(v4.view)
    if (focus_obj == &v4.view[i].viewport)
        Importer.import(*obj_elm, names, screen_pos);
}
void ObjView::animate(C AnimSkel &anim_skel, bool transform_anims, C UID &ignore_anim_id) {
    if (mesh_skel && skel_elm) {
        ...
        mesh_undos.del();
        undoVis(); // we're modifying 'saved_skel' so we can't go back

        MemtN<MatrixM, 256> matrixes;
        anim_skel.getMatrixes(matrixes);
        MatrixD mesh_matrix_d = mesh_matrix;
        REPAO(matrixes) = mesh_matrix_d * matrixes[i] / mesh_matrix_d;
        ...
        if (transform_anims)
            Proj.offsetAnimations(saved_skel, *mesh_skel, skel_elm->id, ignore_anim_id); // we've transformed mesh skeleton, and since animations can transform bones on top of skeleton, then we need to offset them back
...
void ObjView::animate(C Animation &anim, flt anim_time, bool transform_anims, C UID &ignore_anim_id) {
    if (mesh_skel) {
        SkelAnim skel_anim;
        skel_anim.create(*mesh_skel, anim);
        AnimSkel anim_skel;
        anim_skel.create(mesh_skel);
...
void ObjView::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        ...
        if (mode() == SLOTS) {
            REPA(MT)
            if (MT.b(i)) {
                getSkelSlot(MT.pos(i));
                if (lit_bone >= 0 || lit_slot >= 0)
                    break;
            }
            if (lit_bone < 0 && lit_slot < 0)
                getSkelSlot(Ms.pos());
        } else if ((mode() == SKIN && skin_tabs() == SKIN_SEL_BONE) || mode() == BONES) {
            if (!Ms.b(1)) // don't highlight when operating (like transforming)
            {
                getSkel(Gui.msLit(), Ms.pos(), &lit_bone_vis, null);
                lit_bone = visToBone(lit_bone_vis);
                if (mode() == SKIN && mesh_parts.list.lit >= 0)
                    lit_part = mesh_parts.visToPart(mesh_parts.list.lit);
            }
            ...
            if (lit_bone_vis < 0)
                getSkel(Gui.msLit(), Ms.pos(), &lit_bone_vis, null);
            lit_bone = visToBone(lit_bone_vis);
        } else if (mode() == PHYS && phys_tabs() != PHYS_TOGGLE) {
            REPA(MT)
            if (MT.b(i)) {
            ...
            if (lit_phys < 0)
                lit_phys = getPhys(Gui.msLit(), Ms.pos());
        } else if (NewLod.visible()) {
        } else if (MeshAO.visible()) {
            ...
            if (editMeshParts()) {
                if (!transMesh()) {
                    ...
                    lit_vf_part = getVtxFace(Gui.msLit(), Ms.pos(), ignore_vtxs ? null : &lit_vtx, sel_vtx.elms() ? null : &lit_face, &cur_pos);
                    if (lit_vf_part >= 0)
                        has_cur_pos = true;
                }
            } else if (mode() != SKIN || (mode() == SKIN && skin_tabs() == SKIN_SEL_MESH_PART)) {
                REPA(MT)
                if (MT.b(i)) {
                    lit_part = getPart(Gui.objAtPos(MT.pos(i)), MT.pos(i), &cur_pos);
                    if (lit_part >= 0) {
                        has_cur_pos = true;
                        break;
                    }
                ...
                {
                    lit_part = getPart(Gui.msLit(), Ms.pos(), &cur_pos);
                    if (lit_part >= 0)
                        has_cur_pos = true;
                }
            }
            if (mode() == GROUP) {
                if (group.parts_l.lit >= 0)
                    lit_part = GroupRegion::VisiblePartI(getLod(), group.parts_l.lit);
                else
                    group.highlight(lit_part);
            } else if (mode() == MESH || mode() == SKIN) {
                if (mesh_parts.list.lit >= 0)
                    lit_part = mesh_parts.visToPart(mesh_parts.list.lit);
                else
                    mesh_parts.highlight(lit_part);
            }
        }
        if (show_cur_pos() && !has_cur_pos)
            has_cur_pos = (getPart(Gui.msLit(), Ms.pos(), &cur_pos) >= 0);
        ...
        if (phys_elm)
            if (ElmPhys *phys_data = phys_elm->physData())
                phys_mtrl_id = phys_data->mtrl_id;
        ...
        {
            if (Gui.ms() == &param_edit.param_window)
                param_edit.param_window.maximize();
        }
        if (Ms.bp(2)) // close on middle click
        {
            if (trans_mesh.contains(Gui.ms()))
                trans_mesh.close.push();
            else if (param_edit.contains(Gui.ms()) || param_edit.param_window.contains(Gui.ms()) || trans.contains(Gui.ms()) || group.contains(Gui.ms()) || mesh_parts.contains(Gui.ms()) || mesh_variations.contains(Gui.ms()))
                mode.set(-1);
        }
    }
}
void ObjView::Add(Str &s, cchar8 *t) {
    if (s.last() != ' ')
        s += ',';
    s.space() += t;
}
void ObjView::drawInfo(C GuiPC &gpc) {
    if (gpc.visible) {
        ...
        C MeshLod &lod = getDrawLod();
        if (VisibleQuads(lod))
            s.line() += S + VisibleVtxs(lod) + " vtx" + CountS(VisibleVtxs(lod)) + ", " + VisibleTris(lod) + " tri" + CountS(VisibleTris(lod)) + ", " + VisibleQuads(lod) + " quad" + CountS(VisibleQuads(lod));
        else
            s.line() += S + VisibleVtxs(lod) + " vtx" + CountS(VisibleVtxs(lod)) + ", " + VisibleTrisTotal(lod) + " tri" + CountS(VisibleTrisTotal(lod));
        int parts = lod.partsAfterJoinAll(true, true, false, MeshJoinAllTestVtxFlag, true);
        s.line() += S + parts + " draw call" + CountS(parts);
        int size = 0;
        REP(mesh.lods())
        size += VisibleSize(mesh.lod(i));
        s.line() += S + "Mesh size " + SizeBytes(size);
        if (mode() == LOD)
            s.line() += S + mesh.lods() + " LOD" + CountS(mesh.lods());
        if (mesh.is()) {
            bottom.color(WHITE);
            Str s = "Vertex Data: ";
        ...
        if (mode() == PHYS && phys)
            s.line() += S + phys->parts.elms() + " physical body part" + CountS(phys->parts.elms());
        if (Skeleton *skel = getVisSkel()) {
            if (mode() == SLOTS)
                s.line() += S + skel->slots.elms() + " slot" + CountS(skel->slots.elms());
            if (mode() == BONES) {
            ...
            if (InRange(lit_bone_vis, skel->bones)) {
                ...
                if (bone.type) {
                    ...
                    REPA(skel->bones) {
                        C SkelBone &test = skel->bones[i];
                        if (test.type == bone.type) {
                            if (test.type_index)
                                index = true;
                            if (test.type_sub)
                                sub = true;
            ...
            if (InRange(lit_slot, skel->slots)) {
                ...
                if (skel_slot.bone != skel_slot.bone1)
                    s += S + ", Bone Parent1: " + (InRange(skel_slot.bone1, skel->bones) ? S + '"' + skel->bones[skel_slot.bone1].name + '"' : S + "none");
                REPA(slot_meshes)
                if (slot_meshes[i].name == skel_slot.name) {
                    s += S + ", Scale: " + slot_meshes[i].scale;
                    break;
                }
            }
        }
        ...
        if (s.is() || bottom.is()) {
            Rect r = D.rectUI();
            r.min.x = gpc.offset.x + 0.01f;
            r.max.y = gpc.offset.y - 0.25f;
            r.min.y = -rect().h() + gpc.offset.y;
            TextStyleParams ts(false);
            ts.size = 0.045f;
            ts.align.set(1, -1);
            D.clip();
        ...
        if (showVtxSelCircle())
            vtxSelCircle().draw(LitColor, false);
        if (showChangeSkinShape())
            changeSkinShape().draw(LitColor, false);
    }
}
int ObjView::visibleVariation() C {
    if (mesh_variations.list.lit >= 0)
        return mesh_variations.list.lit;
    if (variation_tabs.visibleOnActiveDesktop())
        return selVariation();
...
void ObjView::selVariation(int variation, SET_MODE mode) {
    if (variation != sel_variation) {
        sel_variation = variation;
        variation_tabs.set(selVariation(), QUIET);
        if (mode != QUIET)
            mesh_variations.list.setCur(selVariation());
    }
}
...
void ObjView::selLod(int lod) {
    ...
    if (sel_lod != lod) {
        ...
        T.lod.toGui();
        mesh_parts.refresh();
        lod_tabs.set(selLod(), QUIET);
        if (mode() == LOD && T.lod.dist)
            T.lod.dist->textline.kbSet();
    }
}
...
void ObjView::updateMesh() {
    // get trans_axis
    if (mode() == TRANSFORM) {
        if (trans_tabs() != TRANS_SCALE)
            MatrixAxis(v4, trans.drawMatrix(), trans_axis);
        else
            trans_axis = -1;
    ...
    if (mode() == TRANSFORM)
        if (Ms.b(1))
            if (Edit::Viewport4::View *view = v4.getView(Gui.ms())) {
                ...
                case TRANS_MOVE: {
                    if (trans.move_p[0])
                        multiplier = trans.move_p[0]->mouse_edit_speed;
                    Vec2 mul = multiplier * MoveScale(*view);
                    switch (trans_axis) {
                    case 0:
                        trans.trans.pos.x += AlignDirToCamEx(trans.matrix.x, Ms.d() * mul);
                        break;
                    case 1:
                        trans.trans.pos.y += AlignDirToCamEx(trans.matrix.y, Ms.d() * mul);
                        break;
                    case 2:
                        trans.trans.pos.z += AlignDirToCamEx(trans.matrix.z, Ms.d() * mul);
                        break;
                    default:
                        trans.trans.pos += (ActiveCam.matrix.x * Ms.d().x * mul.x + ActiveCam.matrix.y * Ms.d().y * mul.y) / trans.matrix.orn();
                        break;
                    }
                } break;
                ...
                case TRANS_ROT: {
                    switch (trans_axis) {
                    case 0:
                        trans.matrix.rotateXL(Ms.d().sum() * multiplier);
                        break;
                    case 1:
                        trans.matrix.rotateYL(Ms.d().sum() * multiplier);
                        break;
                    case 2:
                        trans.matrix.rotateZL(Ms.d().sum() * multiplier);
                        break;
                    default:
                        trans.matrix.rotate(ActiveCam.matrix.z, -Ms.d().sum() * multiplier);
                        break;
                    }
                    ...
                    /*switch(trans_axis)
               {
                  ...
                  default: */
                    trans.trans.scale *= ScaleFactor(Ms.d().sum() * multiplier); /*break;
               }*/
                } break;
                }
                trans.toGui();
        ...
        if (Ms.b(1))
            if (Edit::Viewport4::View *view = v4.getView(Gui.ms())) {
                ...
                if (!Kb.alt()) // don't transform when alt pressed, so we can hold alt at the start, to just show the GUI
                    if (Kb.ctrlCmd()) {
                        if (trans_mesh.move_p[0])
                            multiplier = trans_mesh.move_p[0]->mouse_edit_speed;
                        ...
                        case TRANS_MOVE: {
                            ...
                            switch (trans_axis) {
                            case 0:
                                trans_mesh.trans.pos += AlignDirToCam(trans_mesh.matrix.x, Ms.d() * mul);
                                break;
                            case 1:
                                trans_mesh.trans.pos += AlignDirToCam(trans_mesh.matrix.y, Ms.d() * mul);
                                break;
                            case 2:
                                trans_mesh.trans.pos += AlignDirToCam(trans_mesh.matrix.z, Ms.d() * mul);
                                break;
                            default:
                                trans_mesh.trans.pos += ActiveCam.matrix.x * Ms.d().x * mul.x + ActiveCam.matrix.y * Ms.d().y * mul.y;
                                break;
                            }
                        } break;
                        ...
                        case TRANS_ROT: {
                            switch (trans_axis) {
                            case 0:
                                trans_mesh.matrix.rotateXL(Ms.d().sum() * multiplier);
                                break;
                            case 1:
                                trans_mesh.matrix.rotateYL(Ms.d().sum() * multiplier);
                                break;
                            case 2:
                                trans_mesh.matrix.rotateZL(Ms.d().sum() * multiplier);
                                break;
                            default:
                                trans_mesh.matrix.rotate(ActiveCam.matrix.z, -Ms.d().sum() * multiplier);
                                break;
                            }
                        ...
                        case TRANS_SCALE: {
                            switch (trans_axis) {
                            case 0:
                                trans_mesh.trans_scale.x *= ScaleFactor(AlignDirToCamEx(trans_mesh.matrix.x, Ms.d() * multiplier));
                                break;
                            case 1:
                                trans_mesh.trans_scale.y *= ScaleFactor(AlignDirToCamEx(trans_mesh.matrix.y, Ms.d() * multiplier));
                                break;
                            case 2:
                                trans_mesh.trans_scale.z *= ScaleFactor(AlignDirToCamEx(trans_mesh.matrix.z, Ms.d() * multiplier));
                                break;
                            default:
                                trans_mesh.trans.scale *= ScaleFactor(Ms.d().sum() * multiplier);
                                break;
                            }
                        } break;
                        }
    ...
    if (showVtxSelCircle() && Ms.wheel())
        Clamp(vtx_sel_r *= ScaleFactor(Ms.wheel() * 0.3f), 0.01f, D.h() * 0.5f);
    ...
    if (mode() != SLOTS && mode() != RAGDOLL && mode() != PHYS && mode() != BONES)
        REPA(MT)
    if (MT.bp(i) && v4.getView(MT.guiObj(i))) {
        MeshLod &lod = getLod();
        ...
        if (OpObj == OP_OBJ_SET_MTRL && MtrlEdit.visible()) // set material
        {
            ...
            if (skin_tabs() == SKIN_SEL_MESH_PART) {
                mesh_parts.clicked(part_i);
            } else if (skin_tabs() == SKIN_SEL_BONE) {
                getSkel(MT.guiObj(i), MT.pos(i), &sel_bone_vis, null);
                sel_bone = visToBone(sel_bone_vis);
            }
        } else if (mode() == BACKGROUND) {
            ...
            if (InRange(part_i, lod)) {
                ...
                if (mode() == REMOVE) // toggle visibility
                {
                    ...
                    mesh.setBox();
                    setChangedMesh(true);
                } else if (mode() == GROUP && OpObj == OP_OBJ_SET_GROUP && group.getSetGroup() >= 0) {
                    mesh_undos.set("drawGroup");
                ...
                } else // get material
                {
                    ...
                    if (int v = visibleVariation())
                        if (C MaterialPtr &var = part.variation(v))
                            mtrls.add(var); // first try selecting variation if it exists
                    if (!mtrls.elms())
                        REP(4)
                    mtrls.include(part.multiMaterial(i)); // if doesn't exist, then add multi materials
                    MtrlEdit.set(mtrls);
                }
            }
        }
    ...
    // change skin
    if (mode() == SKIN && skin_tabs() == SKIN_CHANGE_SKIN)
        if (Ms.b(0) || Ms.b(1))
            if (Edit::Viewport4::View *view = v4.getView(Gui.ms())) {
                ...
                if (time >= min_time) // enough time passed
                {
                    ...
                    if (Ms.bp(0) || Ms.bp(1))
                        time = min_time; // on first click don't change more than minimum
                    ...
                    if (adelta) {
                        ...
                        Memt<VecI2> vtxs;
                        getVtxs(Gui.ms(), shape, vtxs);
                        Matrix matrix = transformMatrix();
                        MeshLod &lod = getLod();
                        REPA(lod)
                        if (mesh_parts.partOp(i))
                            includeSamePos(i, vtxs);
                        MemtN<int, 128> changed_parts;
                        REPA(vtxs) {
                            C VecI2 &vtx = vtxs[i];
                            if (mesh_parts.partOp(vtx.x))
                                if (MeshPart *part = lod.parts.addr(vtx.x)) {
                                    changed_parts.binaryInclude(vtx.x);
                                    MeshBase &base = part->base;
                                    if (add) // if adding then create and setup vertex data if needed
                                    {
                                        if (!base.vtx.matrix()) {
                                            base.include(VTX_MATRIX);
                                            REPA(base.vtx)
                                            base.vtx.matrix(i).zero();
                                        }
                                        if (!base.vtx.blend()) {
                                            base.include(VTX_BLEND);
                                            REPA(base.vtx)
                                            base.vtx.blend(i).set(255, 0, 0, 0);
                                        }
                                    }
                                    if (base.vtx.matrix() && base.vtx.blend()) {
                                        ...
                                        else // hard
                                        {
                                            ...
                                            switch (shape.type) {
                                            case SHAPE_CIRCLE:
                                                dist = Dist(pos, shape.circle.pos) / shape.circle.r;
                                                break;
                                            case SHAPE_RECT:
                                                dist = (pos - shape.rect.center()).abs().max() / shape.rect.w() * 2;
                                                break;
                                            }
                                        ...
                                        Int skins = 0;
                                        if (add)
                                            skin[skins++].set(bone, 0); // when adding, reserve room for 'bone' as it's required to be present
                                        int sum = 0;                    // actual weight of bones other than 'bone'
                                        FREPA(blend)
                                        if (blend.c[i]) // start with most important bones
                                        {
                                            if (matrix.c[i] == bone)
                                                Clamp(d += blend.c[i], 0, 255);
                                            else {
                                                sum += blend.c[i];
                                                if (InRange(skins, skin))
                                                    skin[skins++].set(matrix.c[i], blend.c[i]);
                                            }
                                        }
                                        int rest = 255 - d; // total weight of bones other than 'bone'
                                        if (sum && rest != sum) {
                                            flt mul = flt(rest) / sum;
                                            REP(skins)
                                            skin[i].weight *= mul;
                                        } // rescale from 'sum' to 'rest'
                                        if (add)
                                            skin[0].weight = d;
                                        else // if increasing, readjust the weight that may have changed during the loop
                                        {    // decreasing
                                            if (!skins)
                                                skin[skins++].set(parent, rest); // if there are no other bones, then we need to set parent bone
                                            if (d && InRange(skins, skin))
                                                skin[skins++].set(bone, d); // if still have room for 'bone'
                                        }
                        ...
                        REPA(changed_parts)
                        lod.parts[changed_parts[i]].setRender();
                        if (changed_parts.elms())
                            setChangedMesh(true, false);
                    }
                }
            }
    ...
    if (mesh.lods() != lod_tabs.tabs()) {
        for (; lod_tabs.tabs() > mesh.lods();)
            lod_tabs.remove(lod_tabs.tabs() - 1);
        for (; lod_tabs.tabs() < mesh.lods();)
            lod_tabs.New(S + "Lod " + lod_tabs.tabs());
    ...
    if (mesh.variations() != variation_tabs.tabs()) {
        for (; variation_tabs.tabs() > mesh.variations();)
            variation_tabs.remove(variation_tabs.tabs() - 1);
        for (; variation_tabs.tabs() < mesh.variations();)
            variation_tabs.New(S);
...
void ObjView::SlotCopy(ObjView &editor) {
    Skeleton &dest = Proj.slot_skel_mem;
    dest.del();
    if (C Skeleton *src = editor.mesh_skel) {
        dest.bones = src->bones;
        if (InRange(editor.sel_slot, src->slots))
            dest.slots.New() = src->slots[editor.sel_slot];
        else
            dest.slots = src->slots; // copy selected or all
    }
}
...
void ObjView::slotSetSelMirror(bool set_other) {
    if (mesh_skel) {
        ...
        if (SkelSlot *slot = mesh_skel->slots.addr(slot_i)) {
            ...
            REPA(mesh_skel->slots)
            if (i != slot_i && slot_name == BoneNeutralName(mesh_skel->slots[i].name)) {
...
void ObjView::createSlots() {
    ...
    slot_tabs.tab(SLOT_PARENT).desc("Set slot parent:\nClick on a slot and drag it to a bone to set it as the parent.\nAlternatively drag the bone to the slot.\nHold Control to set a secondary bone parent.\n\nKeyboard Shortcut: Shift+F7");
    {
        ...
        mode.tab(SLOTS) += slot_ops.create(Rect_LU(slot_tabs.rect().ru(), slot_tabs.rect().h()), n);
        slot_ops.flag |= COMBOBOX_CONST_TEXT;
    }
}
...
void ObjView::updateSlots() {
    if (mode() == SLOTS) {
        // get 'slot_axis'
        if (!mesh_skel || !InRange(sel_slot, mesh_skel->slots) || slot_tabs() != SLOT_MOVE && slot_tabs() != SLOT_ROT)
            slot_axis = -1;
        else {
            Matrix m = mesh_skel->slots[sel_slot];
            m.scaleOrn(SkelSlotSize);
            m *= trans.matrix;
        ...
        // operate on slots
        REPA(MT)
        if (Edit::Viewport4::View *view = v4.getView(MT.guiObj(i)))
            switch (slot_tabs()) {
            case -1:
                if (MT.bp(i))
                    sel_slot = lit_slot;
            ...
            case SLOT_ADD:
                if (MT.bp(i)) {
                    ...
                    if (mesh_skel) {
                        ...
                        FREP(1000) {
                            name = S + "Slot " + i;
                            if (!mesh_skel->findSlot(name))
                                break;
                        }
                        Set(slot.name, bone ? bone->name : name);
                        if (!bone) {
                            view->setViewportCamera();
            ...
            case SLOT_DEL:
                if (MT.bp(i) && mesh_skel && InRange(lit_slot, mesh_skel->slots)) {
                    mesh_undos.set("slotDel");
                    mesh_skel->slots.remove(lit_slot);
                    sel_slot = lit_slot = -1;
                    setChangedSkel(false);
                }
                break;
            ...
            case SLOT_PARENT:
                if (MT.bp(i) && mesh_skel) {
                    if (InRange(lit_slot, mesh_skel->slots))
                        Gui.drag(DragSlotsSlot, ptr(lit_slot), MT.touch(i));
                    else if (InRange(lit_bone, mesh_skel->bones))
                        Gui.drag(DragSlotsBone, ptr(lit_bone), MT.touch(i));
                }
                break;
            ...
            case SLOT_ROT:
            case SLOT_SCALE: {
                if (MT.bp(i) && MT.mouse(i))
                    sel_slot = lit_slot;
                if (mesh_skel && InRange(sel_slot, mesh_skel->slots))
                    if (MT.b(i, MT.mouse(i))) {
                        ...
                        case SLOT_MOVE: {
                            ...
                            switch (slot_axis) {
                            case 0:
                                slot.pos += AlignDirToCam(slot.cross(), MT.ad(i) * mul);
                                break;
                            case 1:
                                slot.pos += AlignDirToCam(slot.perp, MT.ad(i) * mul);
                                break;
                            case 2:
                                slot.pos += AlignDirToCam(slot.dir, MT.ad(i) * mul);
                                break;
                            default:
                                slot.pos += ActiveCam.matrix.x * MT.ad(i).x * mul.x + ActiveCam.matrix.y * MT.ad(i).y * mul.y;
                                break;
                            }
                            setChangedSkel(false);
                        } break;
                        ...
                        case SLOT_ROT: {
                            ...
                            switch (slot_axis) {
                            case 0:
                                slot.rotateCross(angle);
                                break;
                            case 1:
                                slot.rotatePerp(angle);
                                break;
                            case 2:
                                slot.rotateDir(angle);
                                break;
                            default:
                                slot.Orient::mul(Matrix3().setRotate(ActiveCam.matrix.z, angle));
                                break;
                            }
                            setChangedSkel(false);
                        } break;
...
void ObjView::renameSlot(int index, C Str &old_name, C Str &new_name) {
    if (mesh_skel)
        FREPA(mesh_skel->slots)
    if (old_name == mesh_skel->slots[i].name)
        if (!index--) {
...
void ObjView::putMeshToSlot(C MeshPtr &mesh, int slot_index) {
    if (mesh_skel && InRange(slot_index, mesh_skel->slots))
        SlotMesh::Set(slot_meshes, mesh_skel->slots[slot_index].name, mesh);
}
...
void ObjView::delRootBone() {
    if (mesh_skel) {
        ...
        if (C EditSkeleton::Bone *bone = edit_skel.bones.addr(bone_index))
            delBone(mesh_skel->findBoneI(bone->name));
    }
    Gui.closeMsgBox(del_root_bone_dialog_id);
}
void ObjView::delBone(int bone_i) {
    if (mesh_skel)
        if (C SkelBone *bone = mesh_skel->bones.addr(bone_i)) {
            ...
            mesh_undos.set("boneDel");
            if (MeshChange *change = mesh_undos.getNextUndo())
                change->removeBone(bone->name); // use 'getNextUndo' instead of 'set' because that may return null
...
void ObjView::BoneRootChanged(ObjView &editor) {
    ...
    if (BoneRoot *root = editor.bone_root_data.addr(editor.bone_root()))
        editor.edit_skel.root = (root->node.is ? editor.edit_skel.findNodeI(root->node.name) : -1);
    editor.setBoneRootTextSize();
    editor.setChangedSkel(false);

    int bone_index = editor.edit_skel.nodeToBoneDirect(editor.edit_skel.root);
    if (editor.mesh_skel && InRange(bone_index, editor.mesh_skel->bones) && editor.mesh_skel->bones[bone_index].parent != BONE_NULL)
        bone_index = -1; // don't ask to delete if has a parent
    if (bone_index >= 0) {
        MemtN<bool, 256> used_bones;
        editor.mesh.setUsedBones(used_bones);
        if (InRange(bone_index, used_bones) && used_bones[bone_index])
            bone_index = -1; // don't ask to delete if used by mesh
    }
    if (bone_index >= 0) {
        Dialog &dialog = Gui.getMsgBox(del_root_bone_dialog_id);
        dialog.set("Delete Root Bone", "Would you like to delete this bone from the Skeleton?", Memt<Str>().add("Delete").add("No"));
        dialog.button[2].show();
        dialog.activate();
...
void ObjView::SkelCopy(ObjView &editor) {
    Skeleton &dest = Proj.skel_mem;
    dest.del();
    if (C Skeleton *src = editor.mesh_skel)
        dest.bones = src->bones;
    Proj.edit_skel_mem = editor.edit_skel;
}
void ObjView::SkelPaste(ObjView &editor) {
    if (editor.getSkelElm()) // create skel if needed
        if (Skeleton *dest = editor.mesh_skel) {
            ...
            // check if can paste
            if (src.bones.elms() + dest->bones.elms() > BONE_NULL) {
                Gui.msgBox(S, S + "Can't paste skeleton bones because the total number will exceed the " + BONE_NULL + " bone limit.");
                return;
            }
            REPA(src.bones)
            if (dest->findBoneI(src.bones[i].name) >= 0) {
                Gui.msgBox(S, S + "Can't paste skeleton bones because both source and target contain a bone with the same name \"" + src.bones[i].name + "\". Skeleton Bone names must be unique.");
                return;
            }
...
void ObjView::skelSetSelMirror(bool set_other) {
    if (mesh_skel) {
        ...
        if (SkelBone *bone = mesh_skel->bones.addr(bone_i)) {
            ...
            REPA(mesh_skel->bones)
            if (i != bone_i && bone_name == BoneNeutralName(mesh_skel->bones[i].name)) {
...
void ObjView::boneTarget() {
    if (mesh_skel) {
        ...
        if (!bone) {
            Gui.msgBox(S, "No Bone Selected");
            return;
        }
        SkelBone *target = mesh_skel->bones.addr(lit_bone);
        if (!target) {
            Gui.msgBox(S, "No Target Highlighted");
            return;
        }
...
void ObjView::boneSetShape() {
    if (mesh_skel) {
        ...
        int bone_i = (sel_bone >= 0 ? sel_bone : lit_bone);
        SkelBone *bone = mesh_skel->bones.addr(bone_i);
        if (!bone) { /*Gui.msgBox(S, "No Bone Selected"); */
            return;
        }
        {
...
void ObjView::skelDelBones() {
    if (mesh_skel) {
        ...
        if (MeshChange *change = mesh_undos.getNextUndo())
            REPA(mesh_skel->bones)
        change->removeBone(mesh_skel->bones[i].name);
...
void ObjView::selectLit() {
    sel_bone = lit_bone;
    sel_bone_vis = lit_bone_vis;
}
int ObjView::boneToVis(int i) {
    if (Skeleton *vis_skel = getVisSkel())
        if (mesh_skel && InRange(i, mesh_skel->bones))
            return vis_skel->findBoneI(mesh_skel->bones[i].name);
    return -1;
}
int ObjView::visToBone(int i) {
    if (Skeleton *vis_skel = getVisSkel())
        if (mesh_skel && InRange(i, vis_skel->bones))
            return mesh_skel->findBoneI(vis_skel->bones[i].name);
    return -1;
}
void ObjView::selBone(int i) {
    sel_bone = i;
    sel_bone_vis = boneToVis(i);
}
void ObjView::createBones() {
    ...
    bone_shape.mode = BUTTON_TOGGLE;
    bone_shape.image = "Gui/Misc/capsule.img";

    {
        ...
        mode.tab(BONES) += bone_ops.create(Rect_LU(bone_tabs.rect().ru(), bone_tabs.rect().h()), n);
        bone_ops.flag |= COMBOBOX_CONST_TEXT;
    }
    {
        mode.tab(BONES) += bone_root_t.create(bone_ops.rect().right() + Vec2(0.05f, 0), "Root", &ts);
        mode.tab(BONES) += bone_root.create(Rect_L(bone_root_t.rect().right() + Vec2(0.09f, 0), 0.60f, 0.055f)).func(BoneRootChanged, T);
        FlagDisable(bone_root.flag, COMBOBOX_MOUSE_WHEEL);
        ListColumn lc[] =
            {
...
void ObjView::updateBones() {
    if (mode() == BONES) {
        ...
        // get 'bone_axis'
        if (!mesh_skel || !InRange(sel_bone, mesh_skel->bones) || bone_tabs != BONE_MOVE && bone_tabs != BONE_ROT && bone_tabs != BONE_SCALE)
            bone_axis = -1;
        else {
            Matrix m = mesh_skel->bones[sel_bone];
            m.scaleOrn(SkelSlotSize);
        ...
        if (Edit::Viewport4::View *view = v4.getView(MT.guiObj(i)))
                ...
                if (MT.bp(i, 1)) {
                    ...
                    if (mesh_skel) {
                        ...
                        Str8 bone_name;
                        {
                            ...
                            FREP(1000) {
                                bone_name = S + "Bone " + i;
                                if (!mesh_skel->findBone(bone_name))
                                    break;
                            }
                            Set(bone.name, bone_name); // generate a unique name
                            view->setViewportCamera();
            ...
            case BONE_RENAME:
                if (MT.bp(i))
                    RenameBone.activate(lit_bone);
                break;

            case BONE_PARENT:
                if (MT.bp(i) && mesh_skel) {
                    if (InRange(lit_bone, mesh_skel->bones))
                        Gui.drag(DragBonesBone, ptr(lit_bone), MT.touch(i));
                }
                break;

            case BONE_DYNAMIC:
                if (MT.bp(i) && mesh_skel && InRange(lit_bone, mesh_skel->bones)) {
                ...
                if (mesh_skel)
                    if (MT.b(i, MT.mouse(i))) {
                        ...
                        if (bone && Kb.ctrlCmd()) {
                            Str bone_name = BoneNeutralName(bone->name);
                            REPA(mesh_skel->bones)
                            if (i != sel_bone && bone_name == BoneNeutralName(mesh_skel->bones[i].name)) {
                        ...
                        case BONE_MOVE: {
                            ...
                            case 1:
                            case 2:
                                d = AlignDirToCam(axis_vec, MT.ad(i) * mul);
                                break;
                            default:
                                d = ActiveCam.matrix.x * MT.ad(i).x * mul.x + ActiveCam.matrix.y * MT.ad(i).y * mul.y;
                                break;
                            }
                            if (bone) {
                                Vec dm(-d.x, d.y, d.z);
                                if (bone_shape()) {
                                    bone->offset += d;
                                    if (bone_mirror)
                                        bone_mirror->offset += dm;
                                } else {
                                    switch (bone_move_tabs()) {
                                    case BONE_MOVE_START:
                                        bone->setFromTo(bone->pos + d, bone->to());
                                        if (bone_mirror)
                                            bone_mirror->setFromTo(bone_mirror->pos + dm, bone_mirror->to());
                                        break;
                                    case BONE_MOVE_END:
                                        bone->setFromTo(bone->pos, bone->to() + d);
                                        if (bone_mirror)
                                            bone_mirror->setFromTo(bone_mirror->pos, bone_mirror->to() + dm);
                                        break;
                                    default:
                                        bone->pos += d;
                                        if (bone_mirror)
                                    ...
                                    if (bone_children() != BONE_CHILDREN_NONE && bone_move_tabs() != BONE_MOVE_START)
                                        REPA(mesh_skel->bones)
                                    if (i != sel_bone && i != mirror_bone) {
                                        SkelBone &bone = mesh_skel->bones[i];
                                        if (bone_children() == BONE_CHILDREN_NEAREST) {
                                            if (bone.parent == sel_bone)
                                                bone.setFromTo(bone.pos + d, bone.to());
                                            else if (mirror_bone >= 0 && bone.parent == mirror_bone)
                                                bone.setFromTo(bone.pos + dm, bone.to());
                                        } else {
                                            if (mesh_skel->contains(sel_bone, i))
                                                bone += d;
                                            else if (mirror_bone >= 0 && mesh_skel->contains(mirror_bone, i))
                                                bone += dm;
                                        }
                                    }
                                }
                        ...
                        case BONE_ROT: {
                            ...
                            if (bone) {
                                ...
                                case 2:
                                    ...
                                    break;
                                default:
                                    axis = ActiveCam.matrix.z;
                                    break;
                                }
                                Matrix m;
                                m.setTransformAtPos(bone->pos, Matrix3().setRotate(axis, angle));
                                bone->transform(m);
                                if (bone_children_rot())
                                    REPA(mesh_skel->bones)
                                if (sel_bone != i && mesh_skel->contains(sel_bone, i)) {
                                    SkelBone &bone = mesh_skel->bones[i];
                                    if (freeze)
                                ...
                                if (bone_mirror) {
                                    axis.chsY().chsZ();
                                    m.setTransformAtPos(bone_mirror->pos, Matrix3().setRotate(axis, angle));
                                    bone_mirror->transform(m);
                                    if (bone_children_rot())
                                        REPA(mesh_skel->bones)
                                    if (mirror_bone != i && mesh_skel->contains(mirror_bone, i)) {
                                        SkelBone &bone = mesh_skel->bones[i];
                                        if (freeze)
                                            bone.rotateDir(-angle);
                                        else
                        ...
                        case BONE_SCALE: {
                            if (bone)
                                switch (bone_axis) {
                                case 0: {
                                    flt scale = ScaleFactor(AlignDirToCamEx(bone->cross(), MT.ad(i) * mul));
                                    bone->width *= scale;
                                    if (bone_mirror) {
                                        bone_mirror->width *= scale;
                                    }
                                } break; // width
                                case 1: {
                                    flt scale = ScaleFactor(Kb.alt() ? (MT.ad(i) * mul).sum() : AlignDirToCamEx(bone->perp, MT.ad(i) * mul));
                                    bone->width *= scale;
                                    if (bone_mirror) {
                                        bone_mirror->width *= scale;
                                    }
                                ...
                                case 2: {
                                    flt scale = ScaleFactor(AlignDirToCamEx(bone->dir, MT.ad(i) * mul));
                                    bone->length *= scale;
                                    bone->width /= scale;
                                    if (bone_mirror) {
                                        bone_mirror->length *= scale;
                                        bone_mirror->width /= scale;
                                    }
                                } break; // length
                                default: {
                                    flt scale = ScaleFactor((MT.ad(i) * mul).sum());
                                    bone->length *= scale;
                                    if (bone_mirror) {
                                        bone_mirror->length *= scale;
                                    }
...
bool ObjView::renameBone(C Str &old_name, C Str &new_name) {
    if (!mesh_skel || Equal(old_name, new_name, true))
        return true; // if it's exactly the same then close with success
    if (!new_name.is()) {
        Gui.msgBox(S, "Name can't be empty.");
        return false;
    }
    if (new_name.length() >= MEMBER_ELMS(SkelBone, name) - 1) {
        Gui.msgBox(S, "Name too long.");
        return false;
    }
    int bone_i = mesh_skel->findBoneI((Str8)old_name);
    if (bone_i >= 0) {
        REPA(mesh_skel->bones)
        if (i != bone_i && new_name == mesh_skel->bones[i].name) {
            Gui.msgBox(S, S + "Can't rename bone to \"" + new_name + "\" because there already exists a bone with that name. Bone names must be unique.");
            return false;
        } // if there already exists a bone with new name the fail with error (but accept renaming the same bone in case just upper/lower case is being changed)

        mesh_undos.set("boneRename");
        if (MeshChange *change = mesh_undos.getNextUndo())
            change->bone_renames.New().set(old_name, new_name); // use 'getNextUndo' instead of 'set' because that may return null
...
void ObjView::PhysChanged(ObjView &editor) {
    if (editor.phys_tabs() == PHYS_DEL)
        editor.setPhys(-1);
}
void ObjView::NewBox(ObjView &editor) {
    if (editor.getPhysElm() && editor.phys) {
        editor.phys_undos.set("box");
        if (editor.hasPhysMeshOrConvex())
            editor.physDel();
        MeshBase m;
        editor.physMesh(m);
        Box box = m;
        ...
        {
            CacheLock cl(PhysBodies);
            editor.phys->parts.New().create(obox);
            editor.phys->setBox();
        }
...
void ObjView::NewBall(ObjView &editor) {
    if (editor.getPhysElm() && editor.phys) {
        ...
        MAX(ball.r, 0.005f);
        if (Kb.ctrlCmd() || Kb.shift()) {
            flt min = ball.pos.y - ball.r;
            if (min < 0)
                ball.pos.y -= min;
        }
        if (Kb.ctrlCmd())
            ball.pos.x = ball.pos.z = 0;
        {
            CacheLock cl(PhysBodies);
            editor.phys->parts.New().create(ball);
            editor.phys->setBox();
        }
        editor.setChangedPhys();
    }
...
void ObjView::NewTube(ObjView &editor) {
    if (editor.getPhysElm() && editor.phys) {
        ...
        {
            CacheLock cl(PhysBodies);
            editor.phys->parts.New().create(tube);
            editor.phys->setBox();
        }
        editor.setChangedPhys();
    }
    ...
    if (editor.getPhysElm() && editor.phys) {
        ...
        if (Kb.alt() && skel && DefaultController(capsule, *skel)) {
            ...
            if (Kb.ctrlCmd())
                box -= box.center().x0z();
            if (Kb.shift())
                capsule.set(Max(box.size().xy.max() / 2, 0.005f), Max(box.d(), 0.01f), box.center(), Vec(0, 0, 1));
            else
                capsule.set(Max(box.size().xz().max() / 2, 0.005f), Max(box.h(), 0.01f), box.center());
        }
        {
            CacheLock cl(PhysBodies);
            editor.phys->parts.New().create(capsule);
            editor.phys->setBox();
        }
        editor.setChangedPhys();
    }
...
void ObjView::PhysDensity(ObjView &editor, C Str &text) {
    if (editor.phys_elm && editor.phys)
        if (ElmPhys *ep = editor.phys_elm->physData()) {
            editor.phys_undos.set("density");
            {
                CacheLock cl(PhysBodies);
                ep->density = editor.phys->density = TextFlt(text);
            }
...
bool ObjView::hasPhysMeshOrConvex() {
    if (phys)
        REPA(*phys)
    if (phys->parts[i].type() == PHYS_CONVEX || phys->parts[i].type() == PHYS_MESH)
        return true;
    return false;
}
...
void ObjView::physSetConvex(int max_vtxs) {
    if (getPhysElm() && phys) {
        ...
        MeshBase m;
        physMesh(m);
        m.createConvex(m.vtx.pos(), m.vtx.elms(), max_vtxs);
        {
            CacheLock cl(PhysBodies);
            if (phys->del().parts.New().createConvexTry(m, 1, true))
                phys->setBox();
            else
                phys->del();
        }
...
void ObjView::physSetMesh() {
    if (getPhysElm() && phys) {
        ...
        MeshBase m;
        physMesh(m);
        m.weldVtx(MESH_NONE, 0.005f).simplify(1, 0.01f, 1, 1, 1, 1, PI, true, SIMPLIFY_PLANES);
        {
            CacheLock cl(PhysBodies);
            if (phys->del().parts.New().createMeshTry(m))
                phys->setBox();
            else
                phys->del();
        }
...
void ObjView::physCopy() {
    if (phys && InRange(sel_phys, phys->parts) && phys->parts[sel_phys].type() == PHYS_SHAPE) {
        ...
        {
            CacheLock cl(PhysBodies);
            PhysPart &part = phys->parts.New();
            part = phys->parts[sel_phys];
        } // !! watch out for memory address change on create new part !!
        setChangedPhys();
    }
}
...
void ObjView::createPhys() {
...

    {
        ...
        parent += phys_ops.create(Rect_LU(phys_tabs.rect().ru(), d), n).focusable(false);
        phys_ops.flag |= COMBOBOX_CONST_TEXT;
    }

    parent += phys_redo.create(Rect_RU(phys_tabs.rect().lu() - Vec2(h, 0), d, d)).func(PhysRedo, T).focusable(false).desc("Physics Redo");
    phys_redo.image = "Gui/Misc/redo.img";
    parent += phys_undo.create(Rect_RU(phys_redo.rect().lu(), d, d)).func(PhysUndo, T).focusable(false).desc("Physics Undo");
    phys_undo.image = "Gui/Misc/undo.img";

    pos = phys_ops.rect().ru() + Vec2(h, 0);
    parent += phys_box.create(Rect_LU(pos, d, d)).func(NewBox, T).focusable(false).desc("Add box to the physical body");
    phys_box.image = "Gui/Misc/box.img";
    parent += phys_ball.create(Rect_LU(phys_box.rect().ru(), d, d)).func(NewBall, T).focusable(false).desc("Add ball to the physical body");
    phys_ball.image = "Gui/Misc/circle.img";
    parent += phys_capsule.create(Rect_LU(phys_ball.rect().ru(), d, d)).func(NewCapsule, T).focusable(false).desc("Add capsule to the physical body");
    phys_capsule.image = "Gui/Misc/capsule.img";
    parent += phys_tube.create(Rect_LU(phys_capsule.rect().ru(), d, d)).func(NewTube, T).focusable(false).desc("Add tube to the physical body");
    phys_tube.image = "Gui/Misc/tube.img";
    ...
    Property &mtrl = phys_props.New().create("Material", MemberDesc(DATA_STR));
    mtrl.button.create("C").func(ClearPhMtrl, T).desc("Remove any current physics material this object has");
    AddProperties(phys_props, parent, phys_tabs.rect().ld() + Vec2(-0.38f, -0.01f), 0.05f, 0.81f, &ts);
    REPAO(phys_props).autoData(this);
    parent += goto_phys_mtrl.create(mtrl.textline.rect()).func(GotoPhMtrl, T).focusable(false).desc("Click to open physics material in the editor.\nDrag and drop physics material here to set it for the physical body.");
    goto_phys_mtrl.text_align = 1;
    mtrl.textline.del();
}
...
void ObjView::updatePhys() {
    ...
    if (mode() == PHYS) {
        // get phys_axis
        if (!phys || !InRange(sel_phys, phys->parts) || phys_tabs() != PHYS_MOVE && phys_tabs() != PHYS_ROT && phys_tabs() != PHYS_SCALE)
            phys_axis = -1;
        else {
        ...
        if (Edit::Viewport4::View *view = v4.getView(MT.guiObj(i))) {
            ...
            switch (phys_tabs()) {
            case PHYS_DEL: {
                if (MT.bp(i) && phys && InRange(lit_phys, phys->parts)) {
                    phys_undos.set("del");
                    CacheLock cl(PhysBodies);
                    phys->parts.remove(lit_phys);
                    phys->setBox();
                    setPhys(lit_phys = -1);
                    setChangedPhys();
                }
                ...
                if (phys && InRange(sel_phys, phys->parts) && phys->parts[sel_phys].type() == PHYS_SHAPE)
                    if (MT.b(i, MT.mouse(i))) {
                        ...
                        case PHYS_MOVE: {
                            ...
                            switch (phys_axis) {
                            case 0:
                                move = AlignDirToCam(phys_part_matrix.x, MT.ad(i) * mul);
                                break;
                            case 1:
                                move = AlignDirToCam(phys_part_matrix.y, MT.ad(i) * mul);
                                break;
                            case 2:
                                move = AlignDirToCam(phys_part_matrix.z, MT.ad(i) * mul);
                                break;
                            default:
                                move = ActiveCam.matrix.x * MT.ad(i).x * mul.x + ActiveCam.matrix.y * MT.ad(i).y * mul.y;
                                break;
                            }
                        ...
                        case PHYS_ROT: {
                            ...
                            switch (phys_axis) {
                            case 0:
                                m.setTransformAtPos(phys_part_matrix.pos, Matrix3().setRotate(!phys_part_matrix.x, angle));
                                break;
                            case 1:
                                m.setTransformAtPos(phys_part_matrix.pos, Matrix3().setRotate(!phys_part_matrix.y, angle));
                                break;
                            case 2:
                                m.setTransformAtPos(phys_part_matrix.pos, Matrix3().setRotate(!phys_part_matrix.z, angle));
                                break;
                            default:
                                m.setTransformAtPos(phys_part_matrix.pos, Matrix3().setRotate(ActiveCam.matrix.z, angle));
                                break;
                            }
                            phys_part_matrix *= m;
                            shape *= m;
                        ...
                        case PHYS_SCALE: {
                            ...
                            case 0: {
                                ...
                                case SHAPE_BALL: {
                                    ...
                                    if (one) {
                                        flt old = ball.r;
                                        MAX(ball.r += delta / 2, 0.005f);
                                        ball.pos += ((ball.r - old) * 2 / 2) * !phys_part_matrix.x;
                                    } else
                                        ball.r *= scale;
                                } break;
                                case SHAPE_CAPSULE: {
                                    ...
                                    if (one) {
                                        flt old = capsule.r;
                                        MAX(capsule.r += delta / 2, 0.005f);
                                        capsule.pos += ((capsule.r - old) * 2 / 2) * !phys_part_matrix.x;
                                    } else
                                        capsule.r *= scale;
                                } break;
                                case SHAPE_TUBE: {
                                    ...
                                    if (one) {
                                        flt old = tube.r;
                                        MAX(tube.r += delta / 2, 0.005f);
                                        tube.pos += ((tube.r - old) * 2 / 2) * !phys_part_matrix.x;
                                    } else
                                        tube.r *= scale;
                                } break;
                                case SHAPE_OBOX: {
                                    ...
                                    if (one) {
                                        ...
                                        flt c = box.centerX();
                                        box.setX((box.min.x - c) * scale + c, (box.max.x - c) * scale + c);
                                    }
                                } break;
                                }
                            } break;
                            ...
                            case 1: {
                                ...
                                case SHAPE_BALL: {
                                    ...
                                    if (one) {
                                        flt old = ball.r;
                                        MAX(ball.r += delta / 2, 0.005f);
                                        ball.pos += ((ball.r - old) * 2 / 2) * !phys_part_matrix.y;
                                    } else
                                        ball.r *= scale;
                                } break;
                                case SHAPE_CAPSULE: {
                                    ...
                                    if (one) {
                                        flt old = capsule.h;
                                        MAX(capsule.h += delta, 0.01f);
                                        capsule.pos += ((capsule.h - old) / 2) * capsule.up;
                                    } else {
                                        capsule.h *= scale;
                                    }
                                } break;
                                case SHAPE_TUBE: {
                                    ...
                                    if (one) {
                                        flt old = tube.h;
                                        MAX(tube.h += delta, 0.01f);
                                        tube.pos += ((tube.h - old) / 2) * tube.up;
                                    } else {
                                        tube.h *= scale;
                                    }
                                } break;
                                case SHAPE_OBOX: {
                                    ...
                                    if (one) {
                                        ...
                                        flt c = box.centerY();
                                        box.setY((box.min.y - c) * scale + c, (box.max.y - c) * scale + c);
                                    }
                                } break;
                                }
                            } break;
                            ...
                            case 2: {
                                ...
                                case SHAPE_BALL: {
                                    ...
                                    if (one) {
                                        flt old = ball.r;
                                        MAX(ball.r += delta / 2, 0.005f);
                                        ball.pos += ((ball.r - old) * 2 / 2) * !phys_part_matrix.z;
                                    } else
                                        ball.r *= scale;
                                } break;
                                case SHAPE_CAPSULE: {
                                    ...
                                    if (one) {
                                        flt old = capsule.r;
                                        MAX(capsule.r += delta / 2, 0.005f);
                                        capsule.pos += ((capsule.r - old) * 2 / 2) * !phys_part_matrix.z;
                                    } else
                                        capsule.r *= scale;
                                } break;
                                case SHAPE_TUBE: {
                                    ...
                                    if (one) {
                                        flt old = tube.r;
                                        MAX(tube.r += delta / 2, 0.005f);
                                        tube.pos += ((tube.r - old) * 2 / 2) * !phys_part_matrix.z;
                                    } else
                                        tube.r *= scale;
                                } break;
                                case SHAPE_OBOX: {
                                    ...
                                    if (one) {
                                        ...
                                        flt c = box.centerZ();
                                        box.setZ((box.min.z - c) * scale + c, (box.max.z - c) * scale + c);
                                    }
                                } break;
                                }
                            } break;
                            ...
                            default: {
                                Matrix m;
                                m.setTransformAtPos(phys_part_matrix.pos, Matrix3(ScaleFactor((MT.ad(i) * mul).sum())));
                                shape *= m;
                            } break;
                            }
                            phys->setBox();
            ...
            case PHYS_TOGGLE: {
                if (MT.bp(i) && InRange(lit_part, mesh)) {
                    FlagToggle(mesh.parts[lit_part].part_flag, MSHP_NO_PHYS_BODY);
                    setChangedMesh(true, false);
                }
            } break;
            }
...
void ObjView::updateRagdoll() {
    if (mode() == RAGDOLL) {
        // get 'bone_axis'
        if (!mesh_skel || !InRange(sel_bone, mesh_skel->bones) || ragdoll_tabs() == RAGDOLL_TOGGLE)
            bone_axis = -1;
        else {
            Matrix m = mesh_skel->bones[sel_bone];
            m.scaleOrn(SkelSlotSize);
                ...
                if (InRange(sel_bone, mesh_skel->bones))
                    if (MT.b(i, MT.mouse(i))) {
                        ...
                        case RAGDOLL_MOVE: {
                            ...
                            switch (bone_axis) {
                            case 0:
                                bone.offset += AlignDirToCam(bone.cross(), MT.ad(i) * mul);
                                break;
                            case 1:
                                bone.offset += AlignDirToCam(bone.perp, MT.ad(i) * mul);
                                break;
                            case 2:
                                bone.offset += AlignDirToCam(bone.dir, MT.ad(i) * mul);
                                break;
                            default:
                                bone.offset += ActiveCam.matrix.x * MT.ad(i).x * mul.x + ActiveCam.matrix.y * MT.ad(i).y * mul.y;
                                break;
                            }
                        } break;
