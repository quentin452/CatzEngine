...
/******************************************************************************/
void MeshParts::RenamePart::Hide(RenamePart &rp) {
    rp.hide();
    ObjEdit.mesh_parts.activate();
}
void MeshParts::RenamePart::create() {
    Gui += super ::create(Rect_C(0, 0, 1, 0.14f), "Rename Part").hide();
...
void MeshParts::RenamePart::update(C GuiPC &gpc) {
    ...
    if (Gui.window() == this) {
        if (Kb.k(KB_ENTER)) {
            Kb.eatKey();
            ObjEdit.mesh_parts.renamePart(index, name, textline());
            button[2].push();
        }
    }
}
void MeshParts::Part::setColor() {
    color = GuiListTextColor();
    if (removed)
        color.a /= 2;
}
void MeshParts::WeldVtxPos::create() {
    Gui += super ::create(Rect_C(0, 0, 1, 0.24f), "Weld Vertex Positions").hide();
    button[2].show();
...
Str MeshParts::Removed(C Part &p) {
    if (MeshPart *part = ObjEdit.getPart(p.index))
        return FlagOn(part->part_flag, MSHP_HIDDEN);
    return S;
}
Str MeshParts::Name(C Part &p) {
    if (MeshPart *part = ObjEdit.getPart(p.index))
        return part->name;
    return S;
}
Str MeshParts::Vtxs(C Part &p) {
    if (MeshPart *part = ObjEdit.getPart(p.index))
        return part->vtxs();
    return S;
}
Str MeshParts::Tris(C Part &p) {
    if (MeshPart *part = ObjEdit.getPart(p.index))
        return part->trisTotal();
    return S;
}
Str MeshParts::Mtrl(C Part &p) {
    if (MeshPart *part = ObjEdit.getPart(p.index))
        return Proj.elmFullName(part->variation(ObjEdit.selVariation()).id());
    return S;
}
Str MeshParts::VtxsN(C Part &p) {
    if (MeshPart *part = ObjEdit.getPart(p.index))
        return TextInt(part->vtxs(), 9);
    return S;
}
Str MeshParts::TrisN(C Part &p) {
    if (MeshPart *part = ObjEdit.getPart(p.index))
        return TextInt(part->trisTotal(), 9);
    return S;
}
void MeshParts::Refresh(MeshParts &parts) { parts.refresh(); }
void MeshParts::Locate(MeshParts &parts) {
    if (parts.list.sel.elms())
        parts.list.scrollTo(parts.list.absToVis(parts.list.sel[0]), false, 1);
}
...
void MeshParts::Rename() {
    ...
    REPA(ObjEdit.mesh_parts.list.sel) {
        int p = ObjEdit.mesh_parts.list.sel[i];
        if (InRange(p, lod)) {
            ObjEdit.mesh_parts.rename.activate(p);
            break;
        }
    }
}
...
void MeshParts::Erase() {
    ...
    MeshLod &lod = ObjEdit.getLod();
    REPA(lod)
    if (ObjEdit.mesh_parts.partSel(i)) {
        lod.parts.remove(i, true);
        ObjEdit.mesh_parts.erasedPart(i);
    }
    ObjEdit.mesh.setBox();
    ObjEdit.setChangedMesh(true);
}
void MeshParts::Remove() {
    ...
    MeshLod &lod = ObjEdit.getLod();
    REPA(ObjEdit.mesh_parts.list.sel)
    if (MeshPart *part = lod.parts.addr(ObjEdit.mesh_parts.list.sel[i]))
        FlagEnable(part->part_flag, MSHP_HIDDEN);
    if (!ObjEdit.mesh_parts.show_removed())
        ObjEdit.mesh_parts.list.sel.clear(); // if we won't see those parts (showing removed is not enabled) then deselect them
    ObjEdit.mesh.setBox();
    ObjEdit.setChangedMesh(true);
}
void MeshParts::Restore() {
    ...
    REPA(ObjEdit.mesh_parts.list.sel)
    if (MeshPart *part = lod.parts.addr(ObjEdit.mesh_parts.list.sel[i]))
        FlagDisable(part->part_flag, MSHP_HIDDEN);
    ObjEdit.mesh.setBox();
    ObjEdit.setChangedMesh(true);
}
void MeshParts::Hide() {
    ...
    REPA(ObjEdit.mesh_parts.list.sel)
    if (MeshPart *part = lod.parts.addr(ObjEdit.mesh_parts.list.sel[i]))
        part->variations(ObjEdit.mesh.variations()) // first make sure we have room for all variations
...
void MeshParts::Duplicate() {
    ...
    REPA(lod)
    if (ObjEdit.mesh_parts.partSel(i)) {
        MeshPart &copy = lod.parts.NewAt(i + 1);
        copy.create(lod.parts[i]); // watch out because creating new elements invalidates old references
...
void MeshParts::CopyMem() {
    ...
    Mesh &dest = Proj.mesh_mem;
    C MeshLod &src = ObjEdit.getLod();
    dest.create(ObjEdit.mesh).setLods(1).parts.del(); // use 'Mesh.create' to copy 'BoneMap'
    FREPA(src)
    if (ObjEdit.mesh_parts.partSel(i))
        dest.parts.New().create(src.parts[i]);
}
void MeshParts::NewLod() {
    ...
    FREPA(lod)
    if (ObjEdit.mesh_parts.partSel(i))
        Swap(lod.parts[i], last.parts.New());
    REPA(lod)
    if (ObjEdit.mesh_parts.partSel(i)) {
        lod.parts.remove(i, true);
        ObjEdit.mesh_parts.erasedPart(i);
...
void MeshParts::SetNameFromMtrl() {
    ...
    if (MeshPart *part = lod.parts.addr(ObjEdit.mesh_parts.list.sel[i])) {
        Str name;
        if (C MaterialPtr &mtrl = part->material())
            if (C Elm *elm = Proj.findElm(mtrl.id()))
                name = elm->name;
        Set(part->name, name);
    }
    ObjEdit.setChangedMesh(true, false);
...
void MeshParts::clicked(int part) {
    list.cur = partToVis(part);
    list.processSel(part);
}
void MeshParts::erasedPart(int part) {
    if (ObjEdit.lit_vf_part == part) {
        ObjEdit.lit_vf_part = ObjEdit.lit_vtx = ObjEdit.lit_face = -1;
    } else if (ObjEdit.lit_vf_part >= part)
        ObjEdit.lit_vf_part--;
    REPA(ObjEdit.sel_vtx) {
        int &p = ObjEdit.sel_vtx[i].x;
        if (p == part)
            ObjEdit.sel_vtx.remove(i, true);
        else if (p >= part)
            p--;
    }
    REPA(ObjEdit.sel_face) {
        int &p = ObjEdit.sel_face[i].x;
        if (p == part)
            ObjEdit.sel_face.remove(i, true);
        else if (p >= part)
            p--;
    }
    REPA(list.sel) {
...
void MeshParts::showMenu(C Vec2 &pos) {
    if (list.sel.elms()) {
        ...
        REPA(list.sel) {
            ...
            if (InRange(p, lod)) {
                ...
                if (part.part_flag & MSHP_HIDDEN)
                    ...
                    remove = true;
                if (part.variation(ObjEdit.selVariation()))
                    hide = true;
            }
        }
        ...
        Node<MenuElm> node;
        if (list.sel.elms() == 1)
            node.New().create("Rename", Rename).desc("Rename part");
        if (remove)
            node.New().create("Remove", Remove).desc("Mark selected parts as removed, these parts won't be available in the game");
        if (restore)
            node.New().create("Restore", Restore).desc("Unmark selected parts as removed");
        if (hide)
            node.New().create("Hide in Variation", Hide).desc("Mesh Part will be hidden in current variation.\nThis works by clearing the material to null.");
        ...
        if (list.sel.elms() > 1)
            node.New().create("Merge", Merge).desc("Merge selected parts together");
        ...
        Gui += menu.create(node).posRU(pos);
        menu.activate();
    }
}
void MeshParts::AddBox() {
    ...
    if (ObjEdit.getMeshElm()) {
        MeshPart &part = ObjEdit.getLod().parts.New();
        Set(part.name, "Box");
        part.base.create(Box(0.5f), MESH_ALL);
        part.setRender();
        ObjEdit.mesh.setBox();
        ObjEdit.setChangedMesh(true);
    }
...
void MeshParts::AddBall() {
    ...
    if (ObjEdit.getMeshElm()) {
        MeshPart &part = ObjEdit.getLod().parts.New();
        Set(part.name, "Ball");
        part.base.create(Ball(0.5f), MESH_ALL);
        part.setRender();
        ObjEdit.mesh.setBox();
        ObjEdit.setChangedMesh(true);
    }
...
void MeshParts::AddBall2() {
    ...
    if (ObjEdit.getMeshElm()) {
        MeshPart &part = ObjEdit.getLod().parts.New();
        Set(part.name, "Ball");
        part.base.create2(Ball(0.5f), MESH_ALL);
        part.setRender();
        ObjEdit.mesh.setBox();
        ObjEdit.setChangedMesh(true);
    }
...
void MeshParts::AddBallIco() {
    ...
    if (ObjEdit.getMeshElm()) {
        MeshPart &part = ObjEdit.getLod().parts.New();
        Set(part.name, "Ball");
        part.base.createIco(Ball(0.5f), MESH_ALL);
        part.setRender();
        ObjEdit.mesh.setBox();
        ObjEdit.setChangedMesh(true);
    }
...
void MeshParts::AddCapsule() {
    ...
    if (ObjEdit.getMeshElm()) {
        MeshPart &part = ObjEdit.getLod().parts.New();
        Set(part.name, "Capsule");
        part.base.create(Capsule(0.3f, 1), MESH_ALL);
        part.setRender();
        ObjEdit.mesh.setBox();
        ObjEdit.setChangedMesh(true);
    }
...
void MeshParts::AddTube() {
    ...
    if (ObjEdit.getMeshElm()) {
        MeshPart &part = ObjEdit.getLod().parts.New();
        Set(part.name, "Tube");
        part.base.create(Tube(0.3f, 1), MESH_ALL);
        part.setRender();
        ObjEdit.mesh.setBox();
        ObjEdit.setChangedMesh(true);
    }
...
void MeshParts::AddCone() {
    ...
    if (ObjEdit.getMeshElm()) {
        MeshPart &part = ObjEdit.getLod().parts.New();
        Set(part.name, "Cone");
        part.base.create(Cone(0.5f, 0, 1), MESH_ALL);
        part.setRender();
        ObjEdit.mesh.setBox();
        ObjEdit.setChangedMesh(true);
    }
...
void MeshParts::AddTorus() {
    ...
    if (ObjEdit.getMeshElm()) {
        MeshPart &part = ObjEdit.getLod().parts.New();
        Set(part.name, "Torus");
        part.base.create(Torus(0.5f, 0.2f), MESH_ALL);
        part.setRender();
        ObjEdit.mesh.setBox();
        ObjEdit.setChangedMesh(true);
    }
...
void MeshParts::AddMemory() {
    ...
    if (ObjEdit.getMeshElm()) {
        Mesh temp;
        temp.create(Proj.mesh_mem).transform(Proj.mesh_matrix / ObjEdit.mesh_matrix);
        if (ObjEdit.mesh_skel && ObjEdit.mesh_skel->is())
            temp.skeleton(ObjEdit.mesh_skel).skeleton(null);
        else
            temp.clearSkeleton().exclude(VTX_SKIN); // call 'skeleton' to reassign bone mapping or remove it if not present
        if (!ObjEdit.mesh.is())
            Swap(ObjEdit.mesh, temp);
        else // if this mesh is empty, then just create from 'temp', this will copy all variations
        {
            temp.variations(ObjEdit.mesh.variations()); // set variations to match dest
            MeshLod &dest = ObjEdit.getLod();
            FREPA(temp)
            dest.parts.New().create(temp.parts[i]); // add parts in order as they're listed
        }
        ObjEdit.mesh.setBox();
        ObjEdit.setChangedMesh(true);
    }
...
MeshParts &MeshParts::create() {
    ...
    lc[3].text_align = lc[4].text_align = -1;
    lc[3].sort = &md_vtxs;
    lc[4].sort = &md_tris;
    md_vtxs.setDataToTextFunc(VtxsN);
    md_tris.setDataToTextFunc(TrisN);
    super::create("Parts");
    flag |= WIN_RESIZABLE;
    T += region.create().slidebarSize(0.04f);
    T += edit_selected.create("Edit selected").func(EditChanged, T).focusable(false);
    edit_selected.mode = BUTTON_TOGGLE;
    T += add.create(add_elms).focusable(false).desc("Add new shape to the mesh");
    add.text = "New";
    add.flag |= COMBOBOX_CONST_TEXT;
    T += locate.create("Locate").func(Locate, T).focusable(false).desc("Find selected parts in the list");
    T += show_removed.create().func(Refresh, T).focusable(false).desc("Show removed parts");
    show_removed.image = "Gui/Misc/trash.img";
    show_removed.mode = BUTTON_TOGGLE;
    region += list.create(lc, Elms(lc)).elmHeight(height).textSize(0, 1).columnHeight(0.05f).selChanging(ListChanging, T).selChanged(ListChanged, T);
    list.flag |= LIST_MULTI_SEL;
    list.cur_mode = LCM_ALWAYS;
    list.setElmTextColor(MEMBER(Part, color));
...
void MeshParts::refresh() {
    if (ObjEdit.obj_elm) // don't refresh when there's no object at all, so we don't lose list selection when unloading
    {
        ...
        FREPA(data) {
            ...
            if (p.removed = FlagOn(part.part_flag, MSHP_HIDDEN))
                p.remove.setText("R").desc("Restore this part");
            else
                p.remove.setImage("Gui/close.img").desc("Remove this part");
            p.setColor();
...
void MeshParts::drag(Memc<UID> &elms, GuiObj *focus_obj, C Vec2 &screen_pos) {
    ...
    {
        REPA(elms)
        if (Elm *mtrl = Proj.findElm(elms[i], ELM_MTRL)) {
