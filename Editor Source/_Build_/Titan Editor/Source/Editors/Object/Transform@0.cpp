...
void TransformRegion::moveBottom() {
    trans.pos.y = -ObjEdit.mesh_box.min.y;
    toGui();
}
void TransformRegion::moveBack() {
    trans.pos.z = -ObjEdit.mesh_box.min.z;
    toGui();
}
void TransformRegion::moveCenter() {
    trans.pos = -ObjEdit.mesh_box.center();
    toGui();
}
void TransformRegion::moveCenterXZ() {
    trans.pos.x = -ObjEdit.mesh_box.centerX();
    trans.pos.z = -ObjEdit.mesh_box.centerZ();
    toGui();
}
    ...
    REPA(lod) {
        ...
        if (!(part.part_flag & MSHP_HIDDEN))
            if (C Vec *src = part.base.vtx.pos()) {
                ...
                REP(vs)
                dest[i] = (src[i] * ObjEdit.mesh_matrix.orn()).xz();
            }
    }
    trans.rot.zero();
    Vec2 axis;
...
Vec TransformRegion::anchorPos() C {
    ...
    default: {
        Box box;
        bool is = false;
        Matrix matrix = ObjEdit.mesh_matrix;
        MeshLod &lod = ObjEdit.getLod();
        if (ObjEdit.mesh_parts.edit_selected()) {
            REPA(ObjEdit.sel_vtx) {
                C VecI2 &v = ObjEdit.sel_vtx[i];
                if (MeshPart *part = lod.parts.addr(v.x)) {
                    MeshBase &base = part->base;
                    if (base.vtx.pos() && InRange(v.y, base.vtx))
                        Include(box, is, base.vtx.pos(v.y) * matrix);
                }
            }
            REPA(ObjEdit.sel_face) {
                C VecI2 &v = ObjEdit.sel_face[i];
                if (MeshPart *part = lod.parts.addr(v.x)) {
                    MeshBase &base = part->base;
                    if (C Vec *pos = base.vtx.pos()) {
                        if (v.y & SIGN_BIT) {
                            ...
                            if (InRange(f, base.quad)) {
                                ...
                                REPA(q)
                                Include(box, is, pos[q.c[i]] * matrix);
                            }
                        } else {
                            int f = v.y;
                            if (InRange(f, base.tri)) {
                                ...
                                REPA(t)
                                Include(box, is, pos[t.c[i]] * matrix);
                            }
                        }
                    }
                }
            ...
            if (ObjEdit.partOp(i)) {
                ...
                if (ObjEdit.partVisible(i, part, false)) {
                    MeshBase &base = part.base;
                    if (C Vec *pos = base.vtx.pos())
                        REPA(base.vtx)
                    Include(box, is, pos[i] * matrix);
                }
            }
        }
        if (is)
...
void TransformRegion::KeepUV::Vtx::create(C Vec &pos) {
    T.adjust = false;
    T.pos = pos;
    T.uv.zero();
    T.weight = 0;
...
void TransformRegion::KeepUV::Part::create(C MeshPart &src) {
    if (C Vec *pos = src.base.vtx.pos()) {
        vtxs.setNum(src.base.vtxs());
        FREPAO(vtxs).create(pos[i]);
    }
}
...
void TransformRegion::KeepUV::create(C MeshLod &src) {
    parts.setNum(src.parts.elms());
    FREPAO(parts).create(src.parts[i]);
}
...
void TransformRegion::apply() {
    if (trans != PoseIdentity || trans_scale != Vec(1) || move_along_normal || move_uv.any() || scale_uv != 1) {
        if (full) {
            ...
            MeshLod &lod = ObjEdit.getLod();
            Matrix3 matrix_nrm = T.matrix;
            if (scale_normal)
                matrix_nrm.inverseScale();
            else
                matrix_nrm.normalize();
            ...
            if (ObjEdit.mesh_parts.edit_selected()) {
                ...
                if (ObjEdit.sel_face.elms()) {
                    ...
                    if (1) {
                        ...
                        REPA(lod)
                        if (ObjEdit.partVisible(i, lod.parts[i], false))
                            ObjEdit.includeSamePos(i, vtxs);
                    }
                } else {
                ...
                REPA(vtxs) {
                    ...
                    if (MeshPart *part = lod.parts.addr(v.x)) {
                        ...
                        if (InRange(v.y, base.vtx)) {
                            ...
                            if (move_along_normal && base.vtx.pos() && base.vtx.nrm())
                                base.vtx.pos(v.y) += base.vtx.nrm(v.y) * move_along_normal; // transform before transforming by matrix
                            if (base.vtx.pos())
                                base.vtx.pos(v.y) *= matrix;
                            if (base.vtx.hlp())
                                base.vtx.hlp(v.y) *= matrix;
                            if (base.vtx.nrm())
                                base.vtx.nrm(v.y) *= matrix_nrm;
...
#endif
                            if (base.vtx.tex0()) {
                                Vec2 &t = base.vtx.tex0(v.y);
                                t = t * scale_uv + move_uv;
                            }
                            if (keep_uv)
...
void TransformRegion::update(C GuiPC &gpc) {
    ...
    {
        if (anchor == CAM_TARGET)
            setMatrix(); // this will already call 'setAnchorPos'
        flt speed = CamMoveScale(ObjEdit.v4.perspective()) * (Kb.shift() ? 0.1f : 1) * 0.5f;
        if (!full && trans.scale)
            speed /= trans.scale;
        REPA(move_p)
        if (move_p[i])
            move_p[i]->mouseEditSpeed(speed);
    }
}
TransformRegion &TransformRegion::create(bool full) {
    super::create().skin(&TransparentSkin, false);
    kb_lit = false;
    T.full = full;
    if (full)
        hide();
    ...
    if (full)
        anchor_p = &props.New().create("Anchor", MemberDesc(MEMBER(TransformRegion, anchor))).setEnum(AnchorName, Elms(AnchorName));
    if (full) {
    ...
    Rect prop_rect = AddProperties(props, T, Vec2(0.01f, -0.01f), prop_height, 0.18f, &ts);
    REPAO(props).autoData(this).changed(Changed);
    prop_rect.min.y -= 0.01f;
    if (anchor_p)
        anchor_p->combobox.resize(Vec2(0.11f, 0));
    REPD(a, 3)
    REPD(i, 2)
    T += rot[a][i].create(Rect_LU(rot_p[a]->button.rect().ru() + Vec2(0.01f + prop_height * i, 0), prop_height)).setImage(i ? "Gui/arrow_right_big.img" : "Gui/arrow_left_big.img").func(i ? Inc90 : Dec90, *rot_p[a]);
    if (scale_normal) {
        scale_normal->display("Scale Normal");
        scale_normal->moveValue(Vec2(0.09f, 0));
    }
    if (keep_uv) {
        keep_uv->display("Keep UV");
        keep_uv->moveValue(Vec2(0.09f, 0));
    }
    if (!full) {
        T += rescale_width.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.29f, elm_height), "Rescale Width to:").func(RescaleWidth, T);
        T += rescale_width_value.create(Rect_LU(rescale_width.rect().max.x + e, prop_rect.min.y, 0.09f, elm_height), "1");
        prop_rect.min.y -= prop_height;
        T += rescale_height.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.29f, elm_height), "Rescale Height to:").func(RescaleHeight, T);
        T += rescale_height_value.create(Rect_LU(rescale_height.rect().max.x + e, prop_rect.min.y, 0.09f, elm_height), "1");
        prop_rect.min.y -= prop_height;
        T += rescale_depth.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.29f, elm_height), "Rescale Depth to:").func(RescaleDepth, T);
        T += rescale_depth_value.create(Rect_LU(rescale_depth.rect().max.x + e, prop_rect.min.y, 0.09f, elm_height), "1");
        prop_rect.min.y -= prop_height;
        T += move_bottom.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.33f, elm_height), "Move Bottom to Y=0").func(MoveBottom, T);
        prop_rect.min.y -= prop_height;
        T += move_back.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.29f, elm_height), "Move Back to Z=0").func(MoveBack, T);
        prop_rect.min.y -= prop_height;
        T += move_center.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.37f, elm_height), "Move Center to (0, 0, 0)").func(MoveCenter, T);
        prop_rect.min.y -= prop_height;
        T += move_center_xz.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.39f, elm_height), "Move Center XZ to (0, 0)").func(MoveCenterXZ, T);
        prop_rect.min.y -= prop_height;
        T += rest_bottom.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.26f, elm_height), "Rest on Bottom").func(RestBottom, T).desc("This will transform the object as if it fell on the ground");
        prop_rect.min.y -= prop_height;
        T += rot_y_min_box.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.38f, elm_height), "Rotate Y to Minimize Box").func(RotYMinBox, T).desc("This will rotate the object along Y axis to minimize its bounding box");
        prop_rect.min.y -= prop_height * 1.5f;
    }
    if (!full)
        T += ok.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.20f, elm_height * 1.2f), "OK").func(OK, T).desc("Apply transformation");
    else
        T += reset.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.20f, elm_height * 1.2f), "Reset").func(Reset, T).desc("Reset transformation values to default");
    T += cancel.create(Rect_LU(ok.is() ? ok.rect().max.x + elm_height : reset.is() ? reset.rect().max.x + elm_height
                                                                                   : prop_rect.min.x,
                               prop_rect.min.y, 0.20f, elm_height * 1.2f),
                       "Cancel")
             .func(Cancel, T)
             .desc("Cancel applying transformation");
    prop_rect.min.y -= prop_height * 2.0f;
    if (!full)
        T += original.create(Rect_LU(prop_rect.min.x, prop_rect.min.y, 0.45f, elm_height * 1.2f), "Reset Object Transform").func(Original, T).desc("Restore object original transformation as it was at the moment of object import");
    T.size(Vec2(slidebar[0].lengthTotal(), slidebar[1].lengthTotal()) + 0.01f);
    if (full)
        T += close.create(Rect_RU(clientWidth(), 0, elm_height, elm_height)).func(OK, T);
