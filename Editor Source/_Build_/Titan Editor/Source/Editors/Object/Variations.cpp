...
/******************************************************************************/
void MeshVariations::RenameVariation::Hide(RenameVariation &rv) {
    rv.hide();
    ObjEdit.mesh_variations.activate();
}
void MeshVariations::RenameVariation::create() {
    Gui += super ::create(Rect_C(0, 0, 1, 0.14f), "Rename Variation").hide();
...
void MeshVariations::RenameVariation::activate(int variation) {
    ...
    if (!variation)
        Gui.msgBox(S, "Variation #0 can't be renamed, it's name is always NULL.");
    else if (InRange(variation, mesh.variations())) {
        name = mesh.variationName(variation);
        index = 0;
        FREP(variation)
        if (name == mesh.variationName(i))
            index++;
...
void MeshVariations::RenameVariation::update(C GuiPC &gpc) {
    ...
    if (Gui.window() == this) {
        if (Kb.k(KB_ENTER)) {
            Kb.eatKey();
            ObjEdit.mesh_variations.renameVariation(index, name, textline());
            button[2].push();
        }
    }
}
void MeshVariations::Variation::setColor() {
...
void MeshVariations::Rename() {
    if (ObjEdit.mesh_variations.list.sel.elms())
        ObjEdit.mesh_variations.rename.activate(ObjEdit.mesh_variations.list.sel[0]);
}
void MeshVariations::renameVariation(int index, C Str &old_name, C Str &new_name) {
    if (!Equal(old_name, new_name, true)) {
        ...
        Mesh &mesh = ObjEdit.mesh;
        FREP(mesh.variations())
        if (old_name == mesh.variationName(i))
            if (!index--) {
...
int MeshVariations::ListVariation::getPos(C Vec2 &pos) {
...
void MeshVariations::showMenu(C Vec2 &pos) {
    if (list.sel.elms()) {
        Node<MenuElm> node;
        if (list.sel.elms() == 1)
            node.New().create("Rename", Rename).desc("Rename variation");
...
MeshVariations &MeshVariations::create() {
    ...
    super::create("Variations");
    flag |= WIN_RESIZABLE;
    T += region.create().slidebarSize(0.04f);
    T += add.create("New Variation").func(Add, T).focusable(false).desc("Create a new variation");
    region += list.create(lc, Elms(lc)).elmHeight(height).textSize(0, 1).columnHeight(0.05f).selChanged(ListChanged, T);
    list.flag |= LIST_MULTI_SEL;
    FlagDisable(list.flag, LIST_SORTABLE); // disable sorting so we can re-arrange variation order by drag and drop
    list.cur_mode = LCM_ALWAYS;
    list.setElmTextColor(MEMBER(Variation, color)).desc("Double click to rename variation\nDrag and drop to change order");
...
void MeshVariations::dragVariations(GuiObj *obj, C Vec2 &screen_pos) {
    if (obj == &list) {
        ...
        Memt<uint> var_id;
        FREPA(list.sel)
        var_id.add(ObjEdit.mesh.variationID(list.sel[i]));

        // move variations
        if (var_id.elms()) {
            ObjEdit.mesh_undos.set("varMove", true);
            FREPA(var_id) {
                int src = ObjEdit.mesh.variationFind(var_id[i]);
                ObjEdit.mesh.variationMove(src, target);
                if (src >= target)
                    target++;
            }
            ObjEdit.setChangedMesh(true, false);
            ...
            list.sel.clear();
            FREPA(var_id) {
                int v = ObjEdit.mesh.variationFind(var_id[i]);
                if (v >= 0)
                    list.sel.include(v);
            }
            Proj.meshVariationChanged();
    ...
    if (contains(focus_obj)) // create variations from materials
    {
        ...
        if (mtrl_ids.elms()) {
            ...
            if (Elm *elm = Proj.findElm(mtrl_ids[i])) {
                ...
                mesh.variationMove(mesh.variations() - 1, start + added);
                added++; // move last added to start
            }
            if (added) {
                ObjEdit.setChangedMesh(true, false);
                Proj.meshVariationChanged();
                ...
                list.sel.clear();
                FREP(added)
                list.sel.add(start + i); // select all added
            }
        }
    }
}
void MeshVariations::refresh() {
    if (ObjEdit.obj_elm) // don't refresh when there's no object at all, so we don't lose list selection when unloading
    {
        ...
        FREPA(data) {
            ...
            if (v.removed = false)
                v.remove.setText("R").desc("Restore this variation");
            else
                v.remove.setImage("Gui/close.img").desc("Remove this variation");
            v.setColor();
...
void MeshVariations::update(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        super::update(gpc);
        if (ObjEdit.mesh.variations() != data.elms())
            refresh();
        ...
        if (Gui.ms() == &list && Ms.bp(0)) {
            ...
            if (Variation *v = list())
                if (list.selMode() == LSM_SET)
                    Gui.drag(DragVariations, T, null);
        }
    }
}
