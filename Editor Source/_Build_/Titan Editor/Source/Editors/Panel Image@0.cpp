...
void PanelImageEditor::preview() {
    ...
    case 0: {
        have_image = true;
        image_rect = panel_image.image.fit(rect);
        panel_image.image.draw(image_rect);
        image_rect.draw(BLACK, false);
        if (draw_depth())
            drawDepth(image_rect);
        if (draw_lines())
            panel_image.drawBaseLines(PURPLE, image_rect);
    } break;

    case 1: {
        have_image = true;
        image_rect = rect;
        image_rect.min.y = rect.centerY();
        image_rect = panel_image.image.fit(image_rect);
        panel_image.image.draw(image_rect);
        image_rect.draw(AZURE, false);
        if (draw_depth())
            drawDepth(image_rect);
        if (draw_lines())
            panel_image.drawBaseLines(PURPLE, image_rect);
        Vec2 center = rect.lerp(0.5f, 0.25f);
        flt h = rect.h() * 0.25f, w = h * params.width / params.height, max_w = rect.w() * 0.9f;
        if (w > max_w) {
            h *= max_w / w;
            w = max_w;
        }
        ...
        panel_image.draw(rc);
        if (draw_lines()) {
            panel_image.drawScaledLines(RED, rc);
            panel_image.drawInnerPadding(GREEN, rc);
            rc.draw(AZURE, false);
        }
    } break;

    case 2:
        DrawPanelImage(panel_image, rect, draw_lines());
        break;
    }
    if (have_image && Gui.ms() == &viewport && Cuts(Ms.pos(), image_rect)) {
        ...
        TextStyleParams tsp;
        tsp.align.set(0, 1);
        tsp.color = ColorInverse(ca);
        tsp.color.a = 255;
        tsp.shadow = tsp.color.lum();
        tsp.size = 0.045f;
...
bool PanelImageEditor::make() {
    if (event.wait() && !thread.wantStop()) {
        ...
        ThreadMayUseGPUData();
        PanelImage temp;
        Image temp_soft, temp_depth;
        params.make(temp, Proj, &temp_soft, &temp_depth, true);
        {
            locker.on();
            Swap(panel_image, temp);
            Swap(soft, temp_soft);
            Swap(depth, temp_depth);
            locker.off();
        }
    }
    return true;
...
void PanelImageEditor::Copy(PanelImageEditor &editor) {
    TextData data;
    FileText f;
    SaveProperties(editor.props, data.nodes, ' ', true);
    data.save(f.writeMem());
    ClipSet(f.rewind().getAll());
}
void PanelImageEditor::Paste(PanelImageEditor &editor) {
    TextData data;
    FileText f;
    f.writeMem().putText(ClipGet());
    data.load(f.rewind());
    editor.force_undo_change_type = "paste";
    LoadProperties(editor.props, data.nodes, ' ', true);
    editor.force_undo_change_type = null;
    editor.setChanged();
}
...
void PanelImageEditor::Changed(C Property &prop) {
    if (!PanelImageEdit.force_undo_change_type)
        PanelImageEdit.setChanged();
}
void PanelImageEditor::SetCutLeft(Params &p, C Str &t) {
    p.cut_left = TextBool(t);
    p.base.cut_left.getUTC();
}
void PanelImageEditor::SetCutRight(Params &p, C Str &t) {
    p.cut_right = TextBool(t);
    p.base.cut_right.getUTC();
}
void PanelImageEditor::SetCutTop(Params &p, C Str &t) {
    p.cut_top = TextBool(t);
    p.base.cut_top.getUTC();
}
void PanelImageEditor::SetCutBottom(Params &p, C Str &t) {
    p.cut_bottom = TextBool(t);
    p.base.cut_bottom.getUTC();
}
void PanelImageEditor::SetCutCorner00(Params &p, C Str &t) {
    p.cut_corners[0][0] = TextBool(t);
    p.base.cut_corners[0][0].getUTC();
}
void PanelImageEditor::SetCutCorner01(Params &p, C Str &t) {
    p.cut_corners[0][1] = TextBool(t);
    p.base.cut_corners[0][1].getUTC();
}
void PanelImageEditor::SetCutCorner10(Params &p, C Str &t) {
    p.cut_corners[1][0] = TextBool(t);
    p.base.cut_corners[1][0].getUTC();
}
void PanelImageEditor::SetCutCorner11(Params &p, C Str &t) {
    p.cut_corners[1][1] = TextBool(t);
    p.base.cut_corners[1][1].getUTC();
}
void PanelImageEditor::SetCutCornerSlope(Params &p, C Str &t) {
    p.cut_corner_slope = TextFlt(t);
    p.base.cut_corner_slope.getUTC();
}
void PanelImageEditor::SetCutCornerAmount(Params &p, C Str &t) {
    p.cut_corner_amount = TextFlt(t);
    p.base.cut_corner_amount.getUTC();
}
void PanelImageEditor::SetForceUniformStretch0(Params &p, C Str &t) {
    p.force_uniform_stretch[0] = TextBool(t);
    p.base.force_uniform_stretch[0].getUTC();
}
void PanelImageEditor::SetForceUniformStretch1(Params &p, C Str &t) {
    p.force_uniform_stretch[1] = TextBool(t);
    p.base.force_uniform_stretch[1].getUTC();
}
void PanelImageEditor::SetCompressed(Params &p, C Str &t) {
    p.base.compressed = TextBool(t);
    p.base.compressed_time.getUTC();
}
void PanelImageEditor::SetMipMaps(Params &p, C Str &t) {
    p.base.mip_maps = TextBool(t);
    p.base.mip_maps_time.getUTC();
}
void PanelImageEditor::SetResolution(Params &p, C Str &t) {
    p.resolution = TextInt(t);
    p.base.resolution.getUTC();
}
void PanelImageEditor::SetWidth(Params &p, C Str &t) {
    p.width = TextInt(t);
    p.base.width.getUTC();
}
void PanelImageEditor::SetHeight(Params &p, C Str &t) {
    p.height = TextInt(t);
    p.base.height.getUTC();
}
void PanelImageEditor::SetRoundCorners(Params &p, C Str &t) {
    p.round_corners = TextFlt(t);
    p.base.round_corners.getUTC();
}
void PanelImageEditor::SetLeftSlope(Params &p, C Str &t) {
    p.left_slope = TextFlt(t);
    p.base.left_slope.getUTC();
}
void PanelImageEditor::SetRightSlope(Params &p, C Str &t) {
    p.right_slope = TextFlt(t);
    p.base.right_slope.getUTC();
}
void PanelImageEditor::SetBorderSize(Params &p, C Str &t) {
    p.border_size = TextFlt(t);
    p.base.border_size.getUTC();
}
void PanelImageEditor::SetMaxSideStretch(Params &p, C Str &t) {
    p.max_side_stretch = TextFlt(t);
    p.base.max_side_stretch.getUTC();
}
void PanelImageEditor::SetMinSize(Params &p, C Str &t) {
    p.min_size = TextVec2(t);
    p.base.min_size.getUTC();
}
void PanelImageEditor::SetExtend(Params &p, C Str &t) {
    p.extend = TextVec2(t);
    p.base.extend.getUTC();
}
void PanelImageEditor::ExtendInnerPaddLR(Params &p, C Str &t) {
    p.extend_inner_padd.min.x = p.extend_inner_padd.max.x = TextFlt(t);
    p.base.extend_inner_padd.getUTC();
}
Str PanelImageEditor::ExtendInnerPaddLR(C Params &p) { return p.extend_inner_padd.min.x; }
void PanelImageEditor::ExtendInnerPaddT(Params &p, C Str &t) {
    p.extend_inner_padd.max.y = -TextFlt(t);
    p.base.extend_inner_padd.getUTC();
}
Str PanelImageEditor::ExtendInnerPaddT(C Params &p) { return -p.extend_inner_padd.max.y; }
void PanelImageEditor::ExtendInnerPaddB(Params &p, C Str &t) {
    p.extend_inner_padd.min.y = TextFlt(t);
    p.base.extend_inner_padd.getUTC();
}
Str PanelImageEditor::ExtendInnerPaddB(C Params &p) { return p.extend_inner_padd.min.y; }
void PanelImageEditor::SetColor(Params &p, C Str &t) {
    p.color = TextVec4(t);
    p.base.color.getUTC();
}
void PanelImageEditor::SetColorTop(Params &p, C Str &t) {
    p.color_top = TextVec4(t);
    p.base.color_top.getUTC();
}
void PanelImageEditor::SetColorBottom(Params &p, C Str &t) {
    p.color_bottom = TextVec4(t);
    p.base.color_bottom.getUTC();
}
void PanelImageEditor::SetColorLeft(Params &p, C Str &t) {
    p.color_left = TextVec4(t);
    p.base.color_left.getUTC();
}
void PanelImageEditor::SetColorRight(Params &p, C Str &t) {
    p.color_right = TextVec4(t);
    p.base.color_right.getUTC();
}
void PanelImageEditor::SetOuterGlowColor(Params &p, C Str &t) {
    p.outer_glow_color = TextVec4(t);
    p.base.outer_glow_color.getUTC();
}
void PanelImageEditor::SetOuterGlowRadius(Params &p, C Str &t) {
    p.outer_glow_radius = TextFlt(t);
    p.base.outer_glow_radius.getUTC();
}
void PanelImageEditor::SetOuterGlowSpread(Params &p, C Str &t) {
    p.outer_glow_spread = TextFlt(t);
    p.base.outer_glow_spread.getUTC();
}
void PanelImageEditor::SetInnerGlowColor(Params &p, C Str &t) {
    p.inner_glow_color = TextVec4(t);
    p.base.inner_glow_color.getUTC();
}
void PanelImageEditor::SetInnerGlowRadius(Params &p, C Str &t) {
    p.inner_glow_radius = TextFlt(t);
    p.base.inner_glow_radius.getUTC();
}
void PanelImageEditor::SetDepth(Params &p, C Str &t) {
    p.depth = TextFlt(t);
    p.base.depth.getUTC();
}
void PanelImageEditor::SetRoundDepth(Params &p, C Str &t) {
    p.round_depth = TextFlt(t);
    p.base.round_depth.getUTC();
}
void PanelImageEditor::SetInnerDistance(Params &p, C Str &t) {
    p.inner_distance = TextFlt(t);
    p.base.inner_distance.getUTC();
}
void PanelImageEditor::SetSmoothDepth(Params &p, C Str &t) {
    p.smooth_depth = TextVec2(t);
    p.base.smooth_depth.getUTC();
}
void PanelImageEditor::SetShadowRadius(Params &p, C Str &t) {
    p.shadow_radius = TextFlt(t);
    p.base.shadow_radius.getUTC();
}
void PanelImageEditor::SetShadowOpacity(Params &p, C Str &t) {
    p.shadow_opacity = TextFlt(t);
    p.base.shadow_opacity.getUTC();
}
void PanelImageEditor::SetShadowSpread(Params &p, C Str &t) {
    p.shadow_spread = TextFlt(t);
    p.base.shadow_spread.getUTC();
}
Str PanelImageEditor::SetTopImage(C Params &p) { return Proj.elmFullName(p.base.top_id); }
void PanelImageEditor::SetTopImage(Params &p, C Str &t) {
    p.base.top_id = Proj.findElmImageID(t);
    p.base.top_image.getUTC();
}
Str PanelImageEditor::SetBottomImage(C Params &p) { return Proj.elmFullName(p.base.bottom_id); }
void PanelImageEditor::SetBottomImage(Params &p, C Str &t) {
    p.base.bottom_id = Proj.findElmImageID(t);
    p.base.bottom_image.getUTC();
}
Str PanelImageEditor::SetCenterImage(C Params &p) { return Proj.elmFullName(p.base.center_id); }
void PanelImageEditor::SetCenterImage(Params &p, C Str &t) {
    p.base.center_id = Proj.findElmImageID(t);
    p.base.center_image.getUTC();
}
Str PanelImageEditor::SetLeftImage(C Params &p) { return Proj.elmFullName(p.base.left_id); }
void PanelImageEditor::SetLeftImage(Params &p, C Str &t) {
    p.base.left_id = Proj.findElmImageID(t);
    p.base.left_image.getUTC();
}
Str PanelImageEditor::SetRightImage(C Params &p) { return Proj.elmFullName(p.base.right_id); }
void PanelImageEditor::SetRightImage(Params &p, C Str &t) {
    p.base.right_id = Proj.findElmImageID(t);
    p.base.right_image.getUTC();
}
Str PanelImageEditor::SetTopLeftImage(C Params &p) { return Proj.elmFullName(p.base.top_left_id); }
void PanelImageEditor::SetTopLeftImage(Params &p, C Str &t) {
    p.base.top_left_id = Proj.findElmImageID(t);
    p.base.top_left_image.getUTC();
}
Str PanelImageEditor::SetTopRightImage(C Params &p) { return Proj.elmFullName(p.base.top_right_id); }
void PanelImageEditor::SetTopRightImage(Params &p, C Str &t) {
    p.base.top_right_id = Proj.findElmImageID(t);
    p.base.top_right_image.getUTC();
}
Str PanelImageEditor::SetBottomLeftImage(C Params &p) { return Proj.elmFullName(p.base.bottom_left_id); }
void PanelImageEditor::SetBottomLeftImage(Params &p, C Str &t) {
    p.base.bottom_left_id = Proj.findElmImageID(t);
    p.base.bottom_left_image.getUTC();
}
Str PanelImageEditor::SetBottomRightImage(C Params &p) { return Proj.elmFullName(p.base.bottom_right_id); }
void PanelImageEditor::SetBottomRightImage(Params &p, C Str &t) {
    p.base.bottom_right_id = Proj.findElmImageID(t);
    p.base.bottom_right_image.getUTC();
}
void PanelImageEditor::SetImagesSize(Params &p, C Str &t) {
    p.images_size = TextFlt(t);
    p.base.images_size.getUTC();
}
void PanelImageEditor::SetTopHeight(Params &p, C Str &t) {
    p.top_height = TextFlt(t);
    p.base.top_height.getUTC();
}
void PanelImageEditor::SetBottomHeight(Params &p, C Str &t) {
    p.bottom_height = TextFlt(t);
    p.base.bottom_height.getUTC();
}
void PanelImageEditor::SetLeftRightWidth(Params &p, C Str &t) {
    p.left_right_width = TextFlt(t);
    p.base.left_right_width.getUTC();
}
void PanelImageEditor::SetTopCornerWidth(Params &p, C Str &t) {
    p.top_corner_width = TextFlt(t);
    p.base.top_corner_width.getUTC();
}
void PanelImageEditor::SetBottomCornerWidth(Params &p, C Str &t) {
    p.bottom_corner_width = TextFlt(t);
    p.base.bottom_corner_width.getUTC();
}
void PanelImageEditor::SetAmbientLight(Params &p, C Str &t) {
    p.light_ambient = TextFlt(t);
    p.base.light_ambient.getUTC();
}
void PanelImageEditor::Filter(PanelImageEditor &editor) {
    flt y = editor.prop_h * -0.5f;
    FREPA(editor.props) {
        Property &prop = editor.props[i];
        bool visible = (!editor.filter().is() || ContainsAll(prop.display(), editor.filter()));
        prop.visible(visible);
        if (visible) {
            prop.pos(Vec2(editor.prop_x, y));
            y -= editor.prop_h;
        }
    }
}
void PanelImageEditor::create() {
    ...
    props_w = super::create("Panel Image Editor", Vec2(prop_x, 0), 0.036f, prop_h, 0.3f).max.x + 0.02f;
    button[1].show();
    ...
    T += filter.create(Rect_LU(paste.rect().ru() + Vec2(0.01f, 0), 0.28f, 0.05f)).func(Filter, T).desc("Find property by name");
    filter.reset.show();
    filter.show_find = true;
    filter.hint = "Find Property";
    T += region.create();
    FREPA(props) {
        Property &prop = props[i];
        prop.parent(region).changed(Changed, PreChanged);
    }
    cchar8 *modes[] = {"Image", "Image + Scaled", "4 x Scaled"};
    T += preview_mode.create(modes, Elms(modes)).valid(true).set(1);
    T += draw_depth.create("Show Depth");
    draw_depth.mode = BUTTON_TOGGLE;
    T += draw_lines.create("Show Lines");
    draw_lines.mode = BUTTON_TOGGLE;
    T += viewport.create(Preview, this);
    rect(Rect_C(0, 0, 2.2f, 1.5f));
}
void PanelImageEditor::toGui() {
    super::toGui();
    refresh();
}
void PanelImageEditor::stopThread() {
    thread.stop();
    event.on();
}
void PanelImageEditor::refresh() {
    if (elm) {
        event.on();
        if (!thread.active() || thread.wantStop())
...
PanelImageEditor &PanelImageEditor::hide() {
    stopThread();
    set(null);
    super::hide();
    return T;
}
Rect PanelImageEditor::sizeLimit() C {
...
void PanelImageEditor::flush() {
    if (elm && changed) {
        ...
        params.make(panel_image, Proj);
        Save(panel_image, Proj.gamePath(*elm));
        Proj.savedGame(*elm);
...
void PanelImageEditor::set(Elm *elm) {
    if (elm && elm->type != ELM_PANEL_IMAGE)
        elm = null;
    if (T.elm != elm) {
        flush();
        undos.del();
        ...
        T.elm_id = (elm ? elm->id : UIDZero);
        {
            SyncLocker locker(lock);
            if (elm) {
                params.load(Proj.basePath(*elm));
                panel_image.load(Proj.gamePath(*elm));
            } else {
                params.reset();
                panel_image.del();
            }
...
void PanelImageEditor::activate(Elm *elm) {
    set(elm);
    if (T.elm)
        super::activate();
}
void PanelImageEditor::toggle(Elm *elm) {
    if (elm == T.elm)
        elm = null;
    set(elm);
}
void PanelImageEditor::elmChanged(C UID &panel_image_id) {
    if (elm && elm->id == panel_image_id) {
        undos.set(null, true);
        EditPanelImage temp;
        if (temp.load(Proj.basePath(*elm)))
            if (params.sync(temp))
                toGui();
    }
}
void PanelImageEditor::erasing(C UID &elm_id) {
