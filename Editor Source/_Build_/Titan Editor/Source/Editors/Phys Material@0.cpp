...
void PhysMtrlEditor::FricStat(PhysMtrlEditor &me, C Str &t) {
    me.edit.friction_static = TextFlt(t);
    me.edit.friction_static_time.getUTC();
}
Str PhysMtrlEditor::FricStat(C PhysMtrlEditor &me) { return me.edit.friction_static; }
void PhysMtrlEditor::FricDyn(PhysMtrlEditor &me, C Str &t) {
    me.edit.friction_dynamic = TextFlt(t);
    me.edit.friction_dynamic_time.getUTC();
}
Str PhysMtrlEditor::FricDyn(C PhysMtrlEditor &me) { return me.edit.friction_dynamic; }
void PhysMtrlEditor::FricMode(PhysMtrlEditor &me, C Str &t) {
    me.edit.friction_mode = PhysMtrl::MODE(TextInt(t));
    me.edit.friction_mode_time.getUTC();
}
Str PhysMtrlEditor::FricMode(C PhysMtrlEditor &me) { return me.edit.friction_mode; }
void PhysMtrlEditor::Boun(PhysMtrlEditor &me, C Str &t) {
    me.edit.bounciness = TextFlt(t);
    me.edit.bounciness_time.getUTC();
}
Str PhysMtrlEditor::Boun(C PhysMtrlEditor &me) { return me.edit.bounciness; }
void PhysMtrlEditor::BounMode(PhysMtrlEditor &me, C Str &t) {
    me.edit.bounciness_mode = PhysMtrl::MODE(TextInt(t));
    me.edit.bounciness_mode_time.getUTC();
}
Str PhysMtrlEditor::BounMode(C PhysMtrlEditor &me) { return me.edit.bounciness_mode; }
void PhysMtrlEditor::Density(PhysMtrlEditor &me, C Str &t) {
    me.edit.density = TextFlt(t);
    me.edit.density_time.getUTC();
}
Str PhysMtrlEditor::Density(C PhysMtrlEditor &me) { return me.edit.density; }
void PhysMtrlEditor::Damping(PhysMtrlEditor &me, C Str &t) {
    me.edit.damping = TextFlt(t);
    me.edit.damping_time.getUTC();
}
Str PhysMtrlEditor::Damping(C PhysMtrlEditor &me) { return me.edit.damping; }
void PhysMtrlEditor::ADamping(PhysMtrlEditor &me, C Str &t) {
    me.edit.adamping = TextFlt(t);
    me.edit.adamping_time.getUTC();
}
...
void PhysMtrlEditor::simType(SIM_TYPE sim_type) {
    ...
    case SIM_FRIC: {
        REP(7) {
            Matrix m;
            m.setRotateX(i * -0.19f);
            m.move(Vec((i - 3) * 2, 0, 0));
...
::PhysMtrlEditor::SIM_TYPE PhysMtrlEditor::desiredSimType() C {
    if (fric_stat && fric_stat->contains(Gui.ms()) || fric_dyn && fric_dyn->contains(Gui.ms()) || fric_mode && fric_mode->contains(Gui.ms()))
        return SIM_FRIC;
    if (boun && boun->contains(Gui.ms()) || boun_mode && boun_mode->contains(Gui.ms()))
        return SIM_BOUNCE;
    if (damp && damp->contains(Gui.ms()))
        return SIM_DAMPING;
    if (adamp && adamp->contains(Gui.ms()))
        return SIM_ADAMPING;
    return SIM_NONE;
}
void PhysMtrlEditor::create() {
    ...
    dfb = &add();
    autoData(this);
    prop_max_x = super::create("Physics Material Editor", Vec2(0.02f, -0.07f), 0.036f, 0.043f, 0.2f).max.x;
...
void PhysMtrlEditor::setMtrl() {
    REPAO(actor_dyn).material(null);
    edit.copyTo(temp);
    temp.density(1); // set to null in case changing material props would affect all actors
    REPAO(actor_dyn).material(&temp);
}
PhysMtrlEditor &PhysMtrlEditor::hide() {
    set(null);
    super::hide();
    return T;
}
Rect PhysMtrlEditor::sizeLimit() C {
    Rect r = super::sizeLimit();
    r.min.set(1.0f, 0.70f);
    return r;
}
...
void PhysMtrlEditor::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        ...
        if (Gui.wheel() == &viewport)
            Clamp(cam.dist *= ScaleFactor(Ms.wheel() * -0.2f), 10, 36);
        REPA(Touches)
        if (Touches[i].guiObj() == &viewport && Touches[i].on()) {
            cam.yaw -= Touches[i].ad().x * 2.0f;
            cam.pitch += Touches[i].ad().y * 2.0f;
        }

        sim_time += Time.d();
        if (SIM_TYPE sim = desiredSimType())
            if (sim != sim_type)
                simType(sim);
...
void PhysMtrlEditor::set(Elm *elm) {
    ...
    if (T.elm != elm) {
        ...
        if (elm) {
            ...
            game = PhysMtrls(Proj.gamePath(*elm));
        } else {
            edit.reset();
            game = null;
        }
...
void PhysMtrlEditor::toggle(Elm *elm) {
    if (elm == T.elm)
        elm = null;
    set(elm);
}
void PhysMtrlEditor::activate(Elm *elm) {
    set(elm);
    if (T.elm)
        super::activate();
}
...
void PhysMtrlEditor::erasing(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        set(null);
}
