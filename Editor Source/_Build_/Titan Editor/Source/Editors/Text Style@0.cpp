...
void TextStyleEditor::ParamsShadow(EditTextStyle &e, C Str &t) {
    e.shadow = TextInt(t);
    e.shadow_time.getUTC();
}
void TextStyleEditor::ParamsShade(EditTextStyle &e, C Str &t) {
    e.shade = TextInt(t);
    e.shade_time.getUTC();
}
void TextStyleEditor::ParamsColor(EditTextStyle &e, C Str &t) {
    e.color = TextVec4(t);
    e.color_time.getUTC();
}
void TextStyleEditor::ParamsSelect(EditTextStyle &e, C Str &t) {
    e.selection = TextVec4(t);
    e.selection_time.getUTC();
}
void TextStyleEditor::ParamsAlign(EditTextStyle &e, C Str &t) {
    e.align = TextVec2(t);
    e.align_time.getUTC();
}
void TextStyleEditor::ParamsSize(EditTextStyle &e, C Str &t) {
    e.size = TextVec2(t);
    e.size_time.getUTC();
}
void TextStyleEditor::ParamsSpace(EditTextStyle &e, C Str &t) {
    e.space = TextVec2(t);
    e.space_time.getUTC();
}
void TextStyleEditor::ParamsSpacing(EditTextStyle &e, C Str &t) {
    e.spacing = (TextBool(t) ? SPACING_CONST : SPACING_NICE);
    e.spacing_time.getUTC();
}
void TextStyleEditor::ParamsFont(EditTextStyle &e, C Str &t) {
    e.font.zero();
    int i = TextInt(t);
    if (InRange(i, Proj.font_node.children)) {
        C Str &name = Proj.font_node.children[i].name;
        REPA(Proj.publish_fonts)
        if (Elm *elm = Proj.findElm(Proj.publish_fonts[i]))
...
Str TextStyleEditor::ParamsFont(C EditTextStyle &e) {
    if (Elm *elm = Proj.findElm(e.font))
        REPA(Proj.font_node.children)
    if (Equal(elm->name, Proj.font_node.children[i].name, true))
        return i;
    return S;
}
...
void TextStyleEditor::create() {
    ...
    font_prop = &add("Font", MemberDesc().setFunc(ParamsFont, ParamsFont)).setEnum();
    font_prop->combobox.setData(Proj.font_node);
    autoData(&edit);

    super::create("Text Style Editor", Vec2(0.02f, -0.07f), 0.036f, 0.043f, 0.3f);
...
void TextStyleEditor::toGui() {
    ...
    {
        Elm *font = Proj.findElm(edit.font);
        font_prop->combobox.setText(font ? font->name : UnknownName, true, QUIET);
    }
}
TextStyleEditor &TextStyleEditor::hide() {
    set(null);
    super::hide();
    return T;
}
Rect TextStyleEditor::sizeLimit() C {
    Rect r = super::sizeLimit();
    r.min.set(1.0f, 0.49f);
    return r;
}
...
void TextStyleEditor::set(Elm *elm) {
    if (elm && elm->type != ELM_TEXT_STYLE)
        elm = null;
    if (T.elm != elm) {
        flush();
        undos.del();
...
void TextStyleEditor::activate(Elm *elm) {
    set(elm);
    if (T.elm)
        super::activate();
}
void TextStyleEditor::toggle(Elm *elm) {
    if (elm == T.elm)
        elm = null;
    set(elm);
}
...
void TextStyleEditor::erasing(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        set(null);
}
...
void TextStyleEditor::drag(Memc<UID> &elms, GuiObj *obj, C Vec2 &screen_pos) {
    if (contains(obj))
        REPA(elms)
    if (Elm *elm = Proj.findElm(elms[i], ELM_FONT)) {
        undos.set("font");
        edit.font = elm->id;
