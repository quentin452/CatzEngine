...
void VideoEditor::Custom::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        ...
        ve.video.update(ve.video_time);
        ve.video_time += Time.ad(); // Editor works in background so app time gets updated even when minimized, update time only when actually drawing
        ve.video.drawFit(r);
...
void VideoEditor::create() {
    ...
    Rect r = super::create("Video Player", Vec2(0.02f, -0.02f));
    button[1].show();
...
void VideoEditor::setInfo() {
    if (width)
        width->display(S + "Width: " + video.width());
    if (height)
        height->display(S + "Height: " + video.height());
    if (kbps)
        kbps->display(S + "Kbps: " + DivRound(video.bitRate(), 1000));
    if (fps)
        fps->display(S + "FPS: " + TextReal(video.fps(), -1));
    if (codec)
        codec->display(S + "Codec: " + video.codecName());
}
...
VideoEditor &VideoEditor::hide() {
    set(null);
    super::hide();
    return T;
}
Rect VideoEditor::sizeLimit() C {
    Rect r = super::sizeLimit();
    r.min.set(1.0f, 0.45f);
    return r;
}
...
void VideoEditor::set(Elm *elm) {
    ...
    if (T.elm != elm) {
        ...
        if (elm) {
            ...
            video.del(); // start from the beginning to avoid freezes
        setInfo();
        Proj.refresh(false, false);
        visible(T.elm != null).moveToTop();
    }
}
void VideoEditor::activate(Elm *elm) {
    set(elm);
    if (T.elm)
        super::activate();
}
void VideoEditor::toggle(Elm *elm) {
    if (elm == T.elm)
        elm = null;
    set(elm);
}
void VideoEditor::closeElm(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        video.del();
}
void VideoEditor::erasing(C UID &elm_id) {
    if (elm && elm->id == elm_id)
        set(null);
}
void VideoEditor::elmChanged(C UID &elm_id) {
    if (elm && elm->id == elm_id) {
        video.create(Proj.gamePath(*elm), true);
        video_time = 0;
        setInfo(); // start from the beginning to avoid freezes
    }
}
