...
void ColorBrushClass::Col::update(C GuiPC &gpc) {
    ...
    if (MT.guiObj(i) == this) {
        if (MT.bd(i)) {
            ColorBrush.set(sel);
            if (ColorBrush.sel >= 0)
                ColorBrush.cp.activate();
        } else if (MT.bp(i)) {
            ColorBrush.set((ColorBrush.sel == sel) ? -1 : sel);
        }
    }
}
...
void ColorBrushClass::set(int i, SET_MODE mode) {
    if (sel != i) {
        ...
        if (InRange(sel, col)) {
            ...
            if (cp.visible())
                cp.hide().show(); // force set old color
        }
        if (mode != QUIET)
            SetHmOp((sel >= 0) ? OP_HM_COLOR : OP_HM_NONE);
    }
}
void ColorBrushClass::Changed(ColorBrushClass &cb) {
    if (InRange(cb.sel, cb.col))
        cb.col[cb.sel].col = cb.cp().xyz;
}
ColorBrushClass &ColorBrushClass::create() {
    ...
    FREPA(col) {
        int x = i % X,
            y = i / X;
        T += col[i].create(Rect_LU(b + (s + e) * x, -b - (s + e) * y, s, s), ptr(i)).desc(MLT("Press LMB to select\nPress LMBx2 to edit", PL, u"Naciśnij LMB aby wybrać\nNaciśnij LMBx2 aby edytować", DE, u"LMB zum Auswählen drücken\nLMBx2 zum edit", RU, u"Нажмите LMB чтобы выбрать\nНажмите LMBx2 чтобы править"));
    }

    // set default colors
    {
        int i = 0;
        col[i++].col.set(1.0f, 1.0f, 1.0f);
        col[i++].col.set(1.0f, 0.8f, 0.8f);
        col[i++].col.set(0.8f, 1.0f, 0.8f);
        col[i++].col.set(0.8f, 0.8f, 1.0f);
        col[i++].col.set(0.8f, 0.8f, 0.8f);
        col[i++].col.set(0.8f, 0.6f, 0.6f);
        col[i++].col.set(0.6f, 0.8f, 0.6f);
        col[i++].col.set(0.6f, 0.6f, 0.8f);
        col[i++].col.set(0.6f, 0.6f, 0.6f);
        col[i++].col.set(0.6f, 0.4f, 0.4f);
        col[i++].col.set(0.4f, 0.6f, 0.4f);
        col[i++].col.set(0.4f, 0.4f, 0.6f);
    }

    cp.create(S).func(Changed, T).hide();
    return T;
}
Window &ColorBrushClass::hide() {
    cp.hide();
    return super::hide();
}
...
void ColorBrushClass::update() {
    if (visibleOnActiveDesktop()) {
        // edit
        if (sel >= 0)
            WorldEdit.grid.mtFunc(WorkerThreads, Brush.affectedAreas(1), EditColor, col[sel].col);

        // get the color
        if (WorldEdit.cur.valid() && InRange(sel, T.col) && Ms.bp(0)) {
            Vec color;
            if (WorldEdit.hmColor(WorldEdit.cur.pos(), color)) {
                T.col[sel].col = color;
                cp.setRGB(color);
            }
        }
