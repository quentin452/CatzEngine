...
/******************************************************************************/
void HeightBrushClass::AlignWaypoint::set(EditWaypoint &wp, EditWaypointPoint &p) {
    T.wp = &wp;
    T.p = &p;
}
...
HeightBrushClass &HeightBrushClass::create() {
    ...
    REPA(noise) {
        ...
        REPD(y, noise[i].h())
        REPD(x, noise[i].w())
        noise[i].pixel(x, y, Random(256));
        switch (i) {
        case 0:
            noise[i].blur(2, false).normalize();
            break;
        case 1:
            noise[i].blur(5, false).normalize();
            break;
        }
    }
    return T;
...
void HeightBrushClass::getHeight(Area &area, Cursor &cur) {
    if (area.hm && Brush.affects(cur.edge(), area.xy)) {
        ...
        REPD(x, res) {
            Vec2 pos(x, y);
            pos *= WorldEdit.areaSize() / (res - 1);
            pos += area_pos; // (pos/res+cell)*UNIT == pos*UNIT/res + cell*UNIT
        ...
        case HB_FLAT:
            if (dist >= 0) {
                ...
                if (cur._brush_height_help < 0 || dist < cur._brush_height_help) {
...
void HeightBrushClass::updateHeight(Area &area) {
    if (area.hm && Brush.affected(area.xy)) {
        ...
        {
            SyncLocker locker(lock);
            changed_areas.include(&area);
        }

        // detect objects
        Memt<Obj *> ground_objs;
        if (WorldEdit.hm_align)
            REPA(area.objs)
        if (area.objs[i]->onGround())
            ground_objs.add(area.objs[i]);
        ...
        REPD(x, res) {
            Vec2 pos(x, y);
            pos *= WorldEdit.areaSize() / flt(res - 1);
            pos += area_pos; // (pos/res+cell)*UNIT == pos*UNIT/res + cell*UNIT
            if (flt d = Brush.powerTotal(pos)) {
...
#endif
                case HB_AVG:
                    REP(WorldEdit.curTotal()) {
                        Cursor &cur = WorldEdit.curAll(i);
                        if (cur.valid() && cur.on())
                            AdjustValTime(pos_y, cur._brush_height, Lerp(1.0f, 0.5f, Brush.power(pos, true, cur)), speed);
                    }
                    break;
                case HB_LEVEL:
                    REP(WorldEdit.curTotal()) {
                ...
                case HB_FLAT:
                    REP(WorldEdit.curTotal()) {
                        Cursor &cur = WorldEdit.curAll(i);
                        if (cur.valid() && cur.on())
                            pos_y += Mid(cur._brush_height - pos_y, -speed, speed) * Brush.power(pos, true, cur);
                    }
                    break;
                case HB_NOISE:
                    pos_y += d * speed * (noise->pixelFCubicPlus(x * noise_mul + nx, y * noise_mul + ny, false) * 2 - 1);
                    break;
                case HB_SOFT: {
                    flt sum = 0;
                    if (x > 0)
                        ...
                        sum += area.hm_l()->height(res - 2, y);
                    else if (y == 0 && area.hm_lb())
                        sum += area.hm_lb()->height(res - 2, res - 1);
                    else if (y == res - 1 && area.hm_lf())
                        sum += area.hm_lf()->height(res - 2, 0);
                    else
                        sum += pos_y;
                    if (x < res - 1)
                        sum += hm.height(x + 1, y);
                    else if (area.hm_r())
                        sum += area.hm_r()->height(1, y);
                    else if (y == 0 && area.hm_rb())
                        sum += area.hm_rb()->height(1, res - 1);
                    else if (y == res - 1 && area.hm_rf())
                        sum += area.hm_rf()->height(1, 0);
                    else
                        sum += pos_y;
                    if (y > 0)
                        sum += hm.height(x, y - 1);
                    else if (area.hm_b())
                        sum += area.hm_b()->height(x, res - 2);
                    else if (x == 0 && area.hm_lb())
                        sum += area.hm_lb()->height(res - 1, res - 2);
                    else if (x == res - 1 && area.hm_rb())
                        sum += area.hm_rb()->height(0, res - 2);
                    else
                        sum += pos_y;
                    if (y < res - 1)
                        sum += hm.height(x, y + 1);
                    else if (area.hm_f())
                        sum += area.hm_f()->height(x, 1);
                    else if (x == 0 && area.hm_lf())
                        sum += area.hm_lf()->height(res - 1, 1);
                    else if (x == res - 1 && area.hm_rf())
                        sum += area.hm_rf()->height(0, 1);
                    else
                        sum += pos_y;
    ...
    REPA(WorldEdit.visible_waypoints) {
        ...
        if (!wp.removed) // if(Cuts(wp.area, rect))
        {
            REPA(wp) {
                ...
                if (!p.removed)
                    if (Cuts(WorldEdit.worldToArea(p.pos), rect)) {
                        if (Abs(p.pos.y - WorldEdit.hmHeight(p.pos)) <= 0.1f)
                            align_waypoints.New().set(wp, p);
                    }
            }
        }
    }
...
void HeightBrushClass::alignWaypoints() {
    REPA(align_waypoints) {
        ...
        if (!Equal(p.pos.y, y)) {
            ...
            if (wp)
                WorldEdit.undos.set(*wp, WorldUndo::TerrainType); // this change was caused by editing the Terrain so we need to set proper type
...
void HeightBrushClass::update() {
    ...
    if (visibleOnActiveDesktop() && OpHm == OP_HM_HEIGHT) {
        REP(WorldEdit.curTotal())
        if (WorldEdit.curAll(i).onViewport() && WorldEdit.curAll(i).on()) {
            view_on = true;
            break;
        }
        if (view_on) {
            ...
            bool valid_on = false;
            REP(WorldEdit.curTotal())
            if (WorldEdit.curAll(i).valid() && WorldEdit.curAll(i).on()) {
                valid_on = true;
                break;
            }
            if (valid_on && !Kb.alt()) // rotate image
            {
                // get height
                switch (mode()) {
                case HB_LEVEL:
                ...
                case HB_AVG:
                    REP(WorldEdit.curTotal()) {
                        ...
                        if (cur.valid()) {
                            ...
                            WorldEdit.grid.mtFunc(WorkerThreads, Brush.affectedAreas(cur.edge()), GetHeight, cur);
                            if (cur._brush_height_help)
                                cur._brush_height /= cur._brush_height_help;
                        }
                    }
                    break;
                }
                ...
                if (WorldEdit.hm_align)
                    detectWaypoints(affected_areas);
                ...
                REPA(changed_areas) {
                    ...
                    if (Heightmap2 *hm = area.hm) {
                        ...
                        // first vertical
                        if (Heightmap2 *b = area.hm_b())
                            REP(res)
                        hm->height(i, 0, b->height(i, res - 1));
                        if (Heightmap2 *f = area.hm_f())
                            REP(res)
                        hm->height(i, res - 1, f->height(i, 0));

                        // then horizontal
                        if (Heightmap2 *l = area.hm_l())
                            REP(res)
                        hm->height(0, i, l->height(res - 1, i));
                        if (Heightmap2 *r = area.hm_r())
                            REP(res)
                        hm->height(res - 1, i, r->height(0, i));

                        // then the corner
                        if (Heightmap2 *lf = area.hm_lf())
                            hm->height(0, res - 1, lf->height(res - 1, 0));
                    }
                }
