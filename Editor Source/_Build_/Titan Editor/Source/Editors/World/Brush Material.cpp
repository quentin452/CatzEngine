...
MtrlBrushClass &MtrlBrushClass::create() {
    ...
    FREP(Elms(image) - skip) {
        T += image[i].create(Rect_LU(x, -b, s, s)).desc(MLTC(u"Drag and drop a material here\nPress LMB to select\nPress LMBx2 to edit",                      //\nPress RMB to change",
    ...
    T += scale.create(Rect_L(x, -clientHeight() * 1 / 4, 0.11f, 0.04f), MLTC(u"Scale", PL, u"Skaluj", DE, u"Skaliere", RU, u"Масштаб", PO, u"Escala", CN, u"缩放")).func(PushedScale, T).focusable(false).desc(MLTC(u"Stretch Material's texture.\nTo change material's scale please push the \"Scale\" button and then press RMB on the terrain.\n\nWARNING:\nThis setting is per material.\nChanging the scale results in auto re-saving the Material file!", PL, u"Rozciągnij tekstury Materiału.\nAby rozciągnąć teksturę proszę nacisnąć przycisk \"Skaluj\" a następnie RMB na terenie.\n\nUWAGA:\nTe ustawienia są dokonywane dla każdego materiały oddzielnie.\nZmiana skali owocuje automatycznym zapisaniu pliku Materiału!", DE, u"Streckt die Material Textur.\nUm die Skalierung des Materials zu ändern drücken Sie bitte den \"Skaliere\" Button und anschließend RMB auf das Terrain.\n\nACHTUNG:\nDiese Einstellung ist pro Material.\nBeim ändern der Skalierung wird die Material Datei automatisch erneut gespeichert!", RU, u"Растягивание текстуры материала.\nЧтобы изменить масштаб материала, нажмите кнопку \"Масштаб\", а затем RMB на ландшафте.\n\nПРЕДУПРЕЖДЕНИЕ:\nЭта настройка на весь материал.\nПри изменение масштаба автоматически перезаписывается файл материала!", PO, u"Esticar a textura do Material.\nPara mudar a escala do material por favor carregue no botão \"Escala\" e depois pressione o Botão Direito do Rato no terreno.\n\nCUIDADO:\nEsta configuração é por material.\nMudar os resultados de escala ao auto regravar o ficheiro do Material!", CN, u"材质缩放.\n修改材质的缩放率请按 \"缩放\" 按钮,然后用鼠标右键换到地形上.\n\n警告:\n这项设置是针对每个材质的.\n修改缩放率会自动保存到材质文件中!"));
    scale.mode = BUTTON_TOGGLE;
    T += hole.create(Rect_L(x, -clientHeight() * 3 / 4, 0.11f, 0.04f), MLTC(u"Hole", PL, u"Dziura")).func(PushedHole, T).focusable(false).desc("Make hole in the terrain");
    hole.mode = BUTTON_TOGGLE;
    x += 0.12f;
    ...
    max2.mode = BUTTON_TOGGLE;

    setColors();
    return T;
}
void MtrlBrushClass::validateIcons() {
    REPA(mtrl_id)
    setMaterial(i, mtrl_id[i]);
}
...
void MtrlBrushClass::setMaterial(int i, C UID &mtrl_id) {
    if (InRange(i, Elms(T.mtrl))) {
        Elm *elm = Proj.findElm(mtrl_id);
        if (elm && elm->type != ELM_MTRL)
            elm = null;

        if (InRange(i, Proj.mtrl_brush_id))
            Proj.mtrl_brush_id[i] = (elm ? elm->id : UIDZero);
...
void MtrlBrushClass::UpdateMaterial(Cell<Area> &cell, MaterialPtr &material, int thread_index) {
    ...
    if (area.hm && Brush.affected(cell.xy()))
        if (!Kb.alt()) // rotate image
        {
            ...
            REPD(x, res) {
                Vec2 p(x, y);
                p *= WorldEdit.areaSize() / flt(res - 1);
                p += area_pos; // (p/res+cell)*UNIT == p*UNIT/res + cell*UNIT
        ...
        if (!Kb.alt()) // rotate image
        {
            ...
            REPD(x, res) {
                ...
                if (flt d = Brush.powerTotal(p)) {
                    ...
                    MtrlBlend mb[4];
                    Vec4 blend;
                    hm.getMaterial(x, y, mb[0].mtrl, mb[1].mtrl, mb[2].mtrl, mb[3].mtrl, blend);
                    REPAO(mb).blend = blend.c[i];
                    Sort(mb, Elms(mb), CompareBlend);

                    REPA(mb)
                    if (i >= (MtrlBrush.max1() ? 1 : 2)) // go from the end, to process least significant first
                    {
                    ...
                    REPAO(blend.c) = mb[i].blend;
                    hm.setMaterial(x, y, mb[0].mtrl, mb[1].mtrl, mb[2].mtrl, mb[3].mtrl, blend);
                }
            }
...
void MtrlBrushClass::DragMtrl(ptr slot_index, GuiObj *go, C Vec2 &screen_pos) {
    int i = intptr(slot_index);
    if (InRange(i, MtrlBrush.mtrl_id))
        if (MtrlBrush.mtrl_id[i].valid()) {
            Memc<UID> elms;
            elms.add(MtrlBrush.mtrl_id[i]);
            Drag(elms, go, screen_pos);
        }
}
void MtrlBrushClass::update() {
    if (visibleOnActiveDesktop()) {
        // edit heightmap material
        {
            ...
            if (InRange(sel, mtrl) && mtrl[sel])
                WorldEdit.grid.mtFunc(WorkerThreads, Brush.affectedAreas(1), UpdateMaterial, mtrl[sel]);
            else if (hole())
                WorldEdit.grid.mtFunc(WorkerThreads, Brush.affectedAreas(1), UpdateMaterial, mtrl_null);
            else if (max1() || max2())
                WorldEdit.grid.mtFunc(WorkerThreads, Brush.affectedAreas(1), ReduceMaterial);
        }
        ...
        if (WorldEdit.cur.valid() && sel >= 0 && Ms.bp(0)) {
            ...
            if (mtrl_id.valid())
                setMaterial(sel, mtrl_id);
        }

        // edit material scale
        if (scale() && !scale_material) {
            Cursor *cur = null;
            if (WorldEdit.cur.valid() && Ms.bp(1))
                cur = &WorldEdit.cur;
            REPA(WorldEdit.cur_touch)
            if (WorldEdit.cur_touch[i].valid() && WorldEdit.cur_touch[i].pd())
                cur = &WorldEdit.cur_touch[i];
            if (cur) {
                scale_material = WorldEdit.hmMtrl(cur->pos());
                scale_material_id = scale_material.id();
            }
        }
        if (scale_material) {
            C Vec2 *d = ((WorldEdit.cur.onViewport() && Ms.b(1)) ? &Ms.d() : null);
            if (!d)
                REPA(Touches)
            if (Touches[i].on() && WorldEdit.v4.getView(Touches[i].guiObj())) {
                d = &Touches[i].ad();
                break;
            }
            if (d) {
                ...
                if (Ms.b(0) || Touches.elms() >= 2)
                    uv_scale = 1;
                else
                    Clamp(uv_scale *= ScaleFactor(d->sum()), 0.01f, 16.0f);
        ...
        if (Gui.window() == this) {
            ...
            REPA(Touches) {
                ...
                REPA(image)
                if (touch.guiObj() == &image[i]) {
                    if (touch.db()) {
                        set(i);
                        MtrlEdit.set(mtrl[i]);
                    } else if (touch.pd()) {
                        set((i == sel) ? -1 : i);
                        Gui.drag(DragMtrl, ptr(i), &touch);
                    } /*else
