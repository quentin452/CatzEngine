...
RectI WorldBrushClass::affectedAreas(int mouse_button) C // always calculate on demand, and for example not in 'update' because that method may get called later in frame than when other codes need it (resulting in 1 frame delay which could result in wrong behavior)
{
    ...
    REPA(WorldEdit.cur_touch) {
        Cursor &cur = WorldEdit.cur_touch[i];
        if (cur.valid())
            Include(rect, Rect(WorldEdit.cur_touch[i].edge()));
    }

    // mouse
    if (WorldEdit.cur.valid() && WorldEdit.cur.on() && (mouse_button < 0 || Ms.b(mouse_button)))
        Include(rect, Rect(WorldEdit.cur.edge())); // if mouse cursor valid and pressed

    if (rect.valid())
        return WorldEdit.worldToArea(rect.extend(size * rotate_scale)) & WorldEdit.visible_area_1;
...
bool WorldBrushClass::affected(C VecI2 &xz) C // if area of 'xz' coordinates is affected by brush, this is more precise than the 'affectedAreas'
{
    ...
    // first the touches
    REPA(WorldEdit.cur_touch)
    if (WorldEdit.cur_touch[i].onViewport()) {
        touch_on_viewport = true;
        if (WorldEdit.cur_touch[i].valid())
            if (affects(WorldEdit.cur_touch[i].edge(), xz))
                return true;
    }
    ...
    if (WorldEdit.cur.valid())                             // if mouse cursor valid
        if (touch_on_viewport ? WorldEdit.cur.on() : true) // if there is already some touch on the viewport then add from mouse only if it's pressed
            if (affects(WorldEdit.cur.edge(), xz))
                return true;
...
flt WorldBrushClass::power(C Vec2 &pos, bool texture, Cursor &cur) C // get brush power (0..1) at world XZ position, 'texture'=if allow applying image coverage, 'cur'=get power from specified cursor
{
    ...
    case BS_CIRCLE:
        ...
        break;
    case BS_SQUARE:
        f = Abs((NearestPointOnEdge(pos, cur.edge()) - pos).rotateCosSin(rotate_cos, rotate_sin)).max();
        break;
    }
    ...
    if (f <= size) {
        ...
        // fade
        flt d = 1, v;
        v = pos.x / WorldEdit.areaSize();
        MIN(d, v - WorldEdit.visible_area_1.min.x);
        MIN(d, 1 + WorldEdit.visible_area_1.max.x - v);
        v = pos.y / WorldEdit.areaSize();
        MIN(d, v - WorldEdit.visible_area_1.min.y);
...
void WorldBrushClass::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        ...
        case WorldView::HEIGHTMAP: {
            if (HeightBrush.mode() >= 0)
                switch (HeightBrush.mode()) {
#if MOBILE
                case HB_ADD:
                case HB_SUB:
                    speed *= cds * Time.d() * (bsize() ? 0.5f : 1) * ass;
                    break;
#else
                case HB_ADD_SUB:
                    speed *= cds * Time.d() * (bsize() ? 0.5f : 1) * ass;
                ...
                case HB_SOFT:
                    ...
                    break;
                case HB_NOISE:
                    speed *= ass * Time.d() * 0.3f;
                    break;
                default:
                    speed *= 0;
                    break;
                }
            else if (MtrlBrush.sel != -1 || MtrlBrush.max1() || MtrlBrush.max2()) {
                speed *= Time.d() * 15;
            }
        } break;
