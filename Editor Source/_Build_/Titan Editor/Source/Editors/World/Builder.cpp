...
void Build(Heightmap &hm, Mesh &mesh, flt area_size, C VecI2 &area_xy, Heightmap *l, Heightmap *r, Heightmap *b, Heightmap *f, Heightmap *lb, Heightmap *lf, Heightmap *rb, Heightmap *rf) {
    ...
    if (l && res == l->resolution())
        REP(res)
    if (!Equal(l->height(res1, i), hm.height(0, i))) {
        l = null;
        break;
    }
    if (r && res == r->resolution())
        REP(res)
    if (!Equal(r->height(0, i), hm.height(res1, i))) {
        r = null;
        break;
    }
    if (b && res == b->resolution())
        REP(res)
    if (!Equal(b->height(i, res1), hm.height(i, 0))) {
        b = null;
        break;
    }
    if (f && res == f->resolution())
        REP(res)
    if (!Equal(f->height(i, 0), hm.height(i, res1))) {
        f = null;
        break;
    }
    if (lb && !Equal(lb->height(res1, res1), hm.height(0, 0)))
        lb = null;
    if (lf && !Equal(lf->height(res1, 0), hm.height(0, res1)))
        lf = null;
    if (rb && !Equal(rb->height(0, res1), hm.height(res1, 0)))
        rb = null;
    if (rf && !Equal(rf->height(0, 0), hm.height(res1, res1)))
        rf = null;
...
void Simplify(Mesh &mesh) {
    ...
    REPD(l, mesh.lods()) // go from the end
    {
        ...
        if (!lod.is() || (l > 0 && lod.faces() >= mesh.lod(l - 1).faces())) // if the LOD actually uses more faces than the base version
        {
            if (l == 1) // create basing on the first LOD, in order to use the second LOD for lower shaders quality
            {
                flt dist2 = lod.dist2;
                lod.create(mesh.lod(0));
                lod.dist2 = dist2;
            } else {
...
void EmbedObjBase::operator=(ObjVer &obj) {
    elm_obj_id = obj.elm_obj_id;
    obj.matrix.set(matrix);
    if (ovr_path = obj.ovrPath())
        path = obj.pathSelf(); // set only if is custom value, otherwise we'll take path from 'Object' later !! can't access Proj.getObjPath because it's not thread safe !!
    if (ovr_mesh_variation_id = obj.meshVarOvr())
        mesh_variation_id = obj.mesh_variation_id; // set only if is custom value, otherwise we'll take path from 'Object' later
}
    ...
    if (phys_ptr) {
        ...
        FREPA(*phys_ptr) {
            ...
            if (part.type() == PHYS_SHAPE)
                phys_shapes.add(part.shape);
            else
                phys_actor.add(temp);
            if (path != OBJ_PATH_IGNORE)
                phys_path.add(temp); // don't copy if we're going to ignore it
        }
    }

    Box box;
    mesh.setBase().delRender().exclude(VTX_SKIN).transform(matrix);
    mesh.getBox(box);
    mesh_rect = box.xz();
    phys_path.transform(matrix);
    phys_path.getBox(box);
    phys_path_rect = box.xz();
    phys_actor.transform(matrix);
    phys_actor.getBox(box);
    phys_actor_rect = box.xz();
...
void AreaBuild::PhysObj::set(C Object &src) // this is called on secondary thread
{
    ...
    if (path != OBJ_PATH_IGNORE) {
        phys = src.phys();
        {
            CacheLock clm(Meshes);
            CacheLock clp(PhysBodies);
            matrix = src.matrixFinal();
        } // lock because 'matrixFinal' accesses mesh and phys (lock first mesh in case in the future mesh would load physics like skeleton, in that case it would use phys lock inside mesh lock)
    }
}
void AreaBuild::PhysObj::set(C ObjData &src, Memt<ObjectPtr> &obj_cache) // this is called on secondary thread
{
    ...
    if (path != OBJ_PATH_IGNORE) {
        ...
        ObjectPtr obj = EditToGamePath(src.params.base.name());
        phys = obj->phys();
        Swap(obj_cache.New(), obj); // use lock so 'Object' members will not be changed on a secondary thread
    }
}
...
void AreaBuild::SetPathType(MeshBase &mesh, PATH_MESH_TYPE path_mode) {
    mesh.include(TRI_FLAG);
    REPA(mesh.tri)
    mesh.tri.flag(i) = path_mode;
    mesh.include(QUAD_FLAG);
    REPA(mesh.quad)
    mesh.quad.flag(i) = path_mode;
}
void AreaBuild::SetDefaultPathType(Memt<MeshBase> &meshes) {
    REPA(meshes)
    if (meshes[i].tri.flag() || meshes[i].quad.flag()) // if at least one mesh has flag specified, then set all that don't have
    {
        REPA(meshes) // set PM_GROUND to all face flags
        {
            MeshBase &base = meshes[i];
            if (!base.tri.flag()) {
                base.include(TRI_FLAG);
                REPA(base.tri)
                base.tri.flag(i) = PM_GROUND;
            }
            if (!base.quad.flag()) {
                base.include(QUAD_FLAG);
                REPA(base.quad)
                base.quad.flag(i) = PM_GROUND;
            }
        }
        break;
    }
...
void AreaBuild::process(BuilderClass &builder) {
    ...
    if (build & (AREA_REBUILD_HM | AREA_REBUILD_HM_MESH | AREA_REBUILD_HM_MESH_SIMPLIFY | AREA_REBUILD_HM_PHYS | AREA_REBUILD_EMBED_OBJ | AREA_REBUILD_WATER))
        ThreadMayUseGPUData();

    if (build & AREA_REBUILD_HM) {
        if (builder.io_thread.wantStop())
            return;
        ...
        if (hm.is()) {
            ...
            REPD(x, hm_height_map.w())
            hm_height_map.pixelF(x, y, builder.path_settings.areaSize() * hm.height(x, y));
            ...
            hm_mtrls.setNum(hm.materials());
            REPAO(hm_mtrls) = hm.material(i).id();
        } else {
            hm_height_map.del();
            hm_mtrl_map.del();
            hm_mtrls.del();
    ...
    if (build & AREA_REBUILD_HM_PHYS)
        if (!FastTerrainPhys) // creating terrain phys from original mesh is much slower
        {
            if (builder.io_thread.wantStop())
                return;
            MeshBase temp;
            temp.createPhys(hm_mesh).weldVtx(MESH_NONE, 0.01f, EPS_COL_COS, -1).simplify(1, 0.10f, 1, 1, 1, 1, PI, true, SIMPLIFY_PLANES);
    ...
    if (build & AREA_REBUILD_HM_PHYS)
        if (FastTerrainPhys) // creating terrain phys from a simplified mesh is much faster, but we need to use different 'max_distance'
        {
            if (builder.io_thread.wantStop())
                return;
            MeshBase temp;
            temp.createPhys(hm_mesh).weldVtx(MESH_NONE, 0.01f, EPS_COL_COS, -1).simplify(1, 0.05f, 1, 1, 1, 1, PI, true, SIMPLIFY_PLANES);
            hm_phys.createMeshTry(temp);
        }
    if (build & AREA_REBUILD_EMBED_OBJ) {
        ...
        FREPA(builder.embed_objs) {
            ...
            if (obj.mesh.is() && Cuts(area_rect, obj.mesh_rect)) {
                Mesh clipped;
                ClipMesh(obj.mesh, null, clipped, area_clip_planes, Elms(area_clip_planes));
                if (clipped.is()) {
            ...
            if (obj.phys_actor.is() && Cuts(area_rect, obj.phys_actor_rect)) {
                MeshBase clipped;
                ClipMesh(obj.phys_actor, null, clipped, area_clip_planes, Elms(area_clip_planes));
                if (clipped.is())
                    Swap(phys_meshes.New(), clipped);
            }

            // add physical shapes
            if (Cuts(area_rect, obj.phys_path_rect))
                REPA(obj.phys_shapes) {
                    C Shape &shape = obj.phys_shapes[i];
                    REPA(area_clip_planes)
                    if (Dist(shape, Shape(area_clip_planes[i])) > 0)
                        goto skip_shape; // if the shape is completely outside of at least 1 plane, then skip the shape
        ...
        REPA(obj_mesh)                     // go from end so split meshes aren't processed again
        {
            ...
            {
                Vec size = mesh.ext.size();
                int max_size = size.maxI();
                if (size.c[max_size] >= MeshSplitMinSize) // is big enough
                {
                    Plane plane;
                    plane.pos = mesh.ext.center; // split in half
                    switch (max_size) {
                    default:
                        plane.normal.set(1, 0, 0);
                        break; // the biggest is X dimension so split in half of the X
                    case 1:
                        plane.normal.set(0, 1, 0);
                        break; // the biggest is Y dimension so split in half of the Y
                    case 2:
                        plane.normal.set(0, 0, 1);
                        break; // the biggest is Z dimension so split in half of the Z
                    }
                    Mesh temp;
                    SplitMesh(mesh, null, mesh, temp, plane);
                    Swap(obj_mesh.meshes.New(), temp); // !! creating new element makes 'mesh' reference invalid !!
                }
            }
        }
        ...
        if (phys_meshes.elms()) {
            MeshBase temp;
            temp.create(phys_meshes.data(), phys_meshes.elms());
    ...
    if (build & AREA_REBUILD_WATER) {
        ...
        FREPA(builder.waters) {
            ...
            if (water.is() && Cuts(water.box().xz(), area_rect)) {
                MeshBase clipped;
                ClipMesh(water.mesh(), null, clipped, area_clip_planes, Elms(area_clip_planes));
                if (clipped.is())
                    waters.New().create(clipped, water.lake(), water.depth, water.material());
            }
        }
    }
    if (load & AREA_MAKE_PATH_SRC) // create 'path_src' MeshBase from heightmap+local_obj+embed_obj+water phys
    {
        ...
        // heightmap
        if (hm_phys.is())
            meshes.New().create(hm_phys);

        // local objects
        FREPA(objs) {
            PhysObj &obj = objs[i];
            if (obj.path != OBJ_PATH_IGNORE)
                if (obj.phys) {
                    ...
                    for (; meshes_elms < meshes.elms(); meshes_elms++) // after unlock, process added meshes
                    {
                        ...
                        if (obj.path == OBJ_PATH_BLOCK)
                            SetPathType(mesh, PM_OBSTACLE);
                    }
                }
        }
        ...
        FREPA(builder.embed_objs) {
            ...
            if (obj.phys_path.is() && Cuts(area_rect, obj.phys_path_rect)) {
                MeshBase clipped;
                ClipMesh(obj.phys_path, null, clipped, area_clip_planes, Elms(area_clip_planes));
                if (obj.path == OBJ_PATH_BLOCK)
                    SetPathType(clipped, PM_OBSTACLE);
                if (clipped.is())
                    Swap(meshes.New(), clipped);
            }
        }
...
void AreaBuild::processPath(BuilderClass &builder) {
    if (build & AREA_REBUILD_PATH) {
        ...
        Memt<MeshBase> meshes;
        if (path_src.is())
            meshes.New().create(path_src);
        if (l && l->path_src.is())
            meshes.New().create(l->path_src);
        if (r && r->path_src.is())
            meshes.New().create(r->path_src);
        if (b && b->path_src.is())
            meshes.New().create(b->path_src);
        if (f && f->path_src.is())
            meshes.New().create(f->path_src);
        if (lb && lb->path_src.is())
            meshes.New().create(lb->path_src);
        if (lf && lf->path_src.is())
            meshes.New().create(lf->path_src);
        if (rb && rb->path_src.is())
            meshes.New().create(rb->path_src);
        if (rf && rf->path_src.is())
            meshes.New().create(rf->path_src);
        if (meshes.elms()) {
            SetDefaultPathType(meshes);
            MeshBase mesh;
            mesh.create(meshes.data(), meshes.elms());
    ...
    if (!io_thread.wantStop()) {
        ...
        REPA(areas) {
            ...
            if (area.load) // if want to load something
            {
                ...
                if (area.load & (AREA_LOAD_HEIGHTMAP | AREA_LOAD_HM_MESH | AREA_LOAD_LOCAL_WATER))
                    ThreadMayUseGPUData(); // heightmaps use Materials and Textures, water uses textures and shaders
                if (area.load & (AREA_LOAD_HEIGHTMAP | AREA_LOAD_LOCAL_OBJ_PHYS)) {
                    ...
                    if (area.world_ver)
                        FREPA(objs)
                    if (objs[i].params.path != OBJ_PATH_IGNORE && !objs[i].removed && !area.world_ver->embedded(objs[i]))
                        area.objs.New().set(objs[i], obj_cache);
        ...
        if (embed_objs_rect.valid() && world_ver && (load_obj_mesh || load_obj_phys)) {
            ...
            Memt<UID> world_objs;
            {
                MapLock ml(world_ver->obj_embed);
                REPA(world_ver->obj_embed)
                if (Cuts(world_ver->obj_embed.lockedData(i), embed_objs_rect))
                    world_objs.add(world_ver->obj_embed.lockedKey(i));
            } // use lock because 'obj_embed' can be modified on MainThread, use lock inside brackets so it will be released after we've gathered ID's
            Memt<EmbedObjBase> embed_objs_base;
            if (world_objs.elms()) {
                MapLock ml(world_ver->obj);
                REPA(world_objs)
                if (ObjVer *obj_ver = world_ver->obj.find(world_objs[i]))
                    embed_objs_base.New() = *obj_ver;
            } // use lock because 'obj' can be modified on MainThread, use lock inside brackets so it will be released after we've gathered ID's
            FREPA(embed_objs_base) {
                EmbedObjBase &src = embed_objs_base[i];
                if (src.elm_obj_id.valid())
                    if (ObjectPtr obj = game_path + EncodeFileName(src.elm_obj_id))
                        embed_objs.New().set(src.matrix, *obj, load_obj_mesh, load_obj_phys, src.ovr_path ? &src.path : null, src.ovr_mesh_variation_id ? &src.mesh_variation_id : null);
            }
        }
        ...
        if (water_rect.valid()) {
            ...
            Memt<UID> lakes;
            {
                ...
                if (Cuts(world_ver->lakes.lockedData(i).areas, water_rect))
                    lakes.add(world_ver->lakes.lockedKey(i));
            } // use lock because 'lakes ' can be modified on MainThread
            Memt<UID> rivers;
            {
                MapLock ml(world_ver->rivers);
                REPA(world_ver->rivers)
                if (Cuts(world_ver->rivers.lockedData(i).areas, water_rect))
                    rivers.add(world_ver->rivers.lockedKey(i));
            } // use lock because 'rivers' can be modified on MainThread
            Lake lake;
            REPA(lakes) {
                bool ok;
                {
                    ReadLock rl(WorldAreaSync);
                    ok = lake.load(Project::EditLakePath(edit_path, world_ver->world_id, lakes[i]));
                }
                if (ok && !lake.removed)
                    Swap(waters.New(), lake.checkMesh(Proj).water_mesh);
            }
            River river;
            REPA(rivers) {
                bool ok;
                {
                    ReadLock rl(WorldAreaSync);
                    ok = river.load(Project::EditRiverPath(edit_path, world_ver->world_id, rivers[i]));
                }
                if (ok && !river.removed)
                    Swap(waters.New(), river.checkMesh(Proj).water_mesh);
            }
        }
...
void BuilderClass::stop() {
    io_thread.stop();
    io_wait.on();    // request the thread to stop
    io_thread.del(); // wait for it to finish
    SyncLocker locker(lock);
    finished_processing = false;
    REPA(areas) {
        AreaBuild &area = areas.lockedData(i);
        if (area.build && area.world_ver)
            area.world_ver->rebuildAreaNeighbor(area.xy, area.build);
    } // put back to world rebuild
    areas.del();
    embed_objs.del();
    embed_objs_rect.set(0, -1);
    waters.del();
    water_rect.set(0, -1);
    world_ver = null;
}
        ...
        if (!background || area_ver->old(now))             // process it only if it's old
        {
            ...
            for (int y = -1; y < RebuildBlockSize + 1; y++)     // process with 1 extra border (so we have mesh and phys built on that borders)
                for (int x = -1; x < RebuildBlockSize + 1; x++) // process with 1 extra border (so we have mesh and phys built on that borders)
                {
                    ...
                    if (byte *area_build = world_ver.rebuild.find(xy)) {
                        byte want_area_build = *area_build;
                        if (!build_path)
                            FlagDisable(want_area_build, AREA_REBUILD_PATH); // skip AREA_REBUILD_PATH if we don't want to build it
                        if (want_area_build) {
                            if (background)
                                if (AreaVer *area_ver = world_ver.areas.find(xy))
                                    if (!area_ver->old(now))
                                        continue; // don't process this area if it's not old
                            if (AreaBuild *area = areas.get(xy)) {
                                area->build = want_area_build;
                                uint neighbor_load = 0;

                                if (area->build & AREA_REBUILD_HM)
                                    area->load |= AREA_LOAD_HEIGHTMAP;
                                if (area->build & AREA_REBUILD_HM_MESH) {
                                    area->load |= AREA_LOAD_HEIGHTMAP;
                                    neighbor_load |= AREA_LOAD_HEIGHTMAP;
                                }
                                if (area->build & (AREA_REBUILD_HM_MESH_SIMPLIFY | AREA_REBUILD_HM_PHYS))
                                    area->load |= AREA_LOAD_HM_MESH;
                                ...
                                {
                                    if (!embed_objs_rect.valid())
                                        embed_objs_rect = xy;
                                    else
                                        embed_objs_rect |= xy;
                                    if (area->build & AREA_REBUILD_PATH)
                                        embed_objs_rect |= RectI(xy).extend(1); // get from neighbors as well
                                }

                                // check for water
                                if (area->build & AREA_REBUILD_WATER)
                                    if (!water_rect.valid())
                                        water_rect = xy;
                                    else
                                        water_rect |= xy;
                                ...
                                if (neighbor_load)
                                    for (int y = -1; y <= 1; y++)
                                        for (int x = -1; x <= 1; x++)
                                            if (x || y)
                                                if (AreaBuild *area = areas.get(VecI2(xy.x + x, xy.y + y)))
                                                    area->load |= neighbor_load;
                            }
                            // try to remove if now empty
                            {
                                world_ver.rebuild.lock();
                                FlagDisable(*area_build, want_area_build);
                                if (!*area_build) {
                                    world_ver.rebuild.removeKey(xy);
            ...
            if (areas.elms()) // if gathered areas to process
            {
                ...
                REPA(areas) {
                    ...
                    if (area_build.build & AREA_REBUILD_HM_MESH)
                        FlagDisable(area_build.load, AREA_LOAD_HM_MESH); // don't load mesh  if it needs to be rebuilt
                    if (area_build.build & AREA_REBUILD_HM_PHYS)
                        FlagDisable(area_build.load, AREA_LOAD_HM_PHYS); // don't load phys  if it needs to be rebuilt
                    if (area_build.build & AREA_REBUILD_WATER)
                        FlagDisable(area_build.load, AREA_LOAD_LOCAL_WATER); // don't load water if it needs to be rebuilt
                }
                ...
                if (&world_ver == WorldEdit.ver) // if we're processing currently edited world, then try to load data from areas that are already loaded
                {
                    ...
                    REPA(areas) {
                        ...
                        if (area_build.load) // want to load something
                            if (Cell<Area> *cell = WorldEdit.grid.find(area_build.xy)) {
                                ...
                                {
                                    if (area_build.load & AREA_LOAD_HEIGHTMAP) {
                                        FlagDisable(area_build.load, AREA_LOAD_HEIGHTMAP);
                                        if (area.hm)
                                            area_build.hm.create(*area.hm);
                                    }
                                    if (area_build.load & AREA_LOAD_HM_MESH) {
                                        FlagDisable(area_build.load, AREA_LOAD_HM_MESH);
                                        if (area.hm)
                                            area_build.hm_mesh.create(area.hm->mesh);
                                    }
                                    if (area_build.load & AREA_LOAD_HM_PHYS) {
                                        FlagDisable(area_build.load, AREA_LOAD_HM_PHYS);
                                        if (area.hm)
                                            area_build.hm_phys = area.hm->phys;
                                    }
                                    if (area_build.load & AREA_LOAD_LOCAL_OBJ_PHYS) {
                                        FlagDisable(area_build.load, AREA_LOAD_LOCAL_OBJ_PHYS);
                                        REPA(area.objs)
                                        if (Obj *obj = area.objs[i])
                                            if (obj->params.path != OBJ_PATH_IGNORE && obj->phys && !obj->removed && !obj->embedded())
                                                area_build.objs.New().set(*obj);
                                    }
                                    if (area_build.load & AREA_LOAD_LOCAL_WATER) {
                                        FlagDisable(area_build.load, AREA_LOAD_LOCAL_WATER);
                                        area_build.waters = area.waters;
                                    }
                                }
                            }
                    }
                    ...
                    // waters
                    if (water_rect.valid()) {
                        REPA(WorldEdit.ver->lakes)
                        if (Cuts(water_rect, WorldEdit.ver->lakes.lockedData(i).areas))
                            if (Lake *lake = WorldEdit.lakes.find(WorldEdit.ver->lakes.lockedKey(i)))
                                if (!lake->removed)
                                    waters.add(lake->checkMesh(Proj).water_mesh);
                        REPA(WorldEdit.ver->rivers)
                        if (Cuts(water_rect, WorldEdit.ver->rivers.lockedData(i).areas))
                            if (River *river = WorldEdit.rivers.find(WorldEdit.ver->rivers.lockedKey(i)))
                                if (!river->removed)
                                    waters.add(river->checkMesh(Proj).water_mesh);
                ...
                // setup thread
                {
                    SyncLocker locker(lock);
                    finished_processing = false;
                    world_data.copyTo(path_settings);
                    game_path = Proj.game_path;
                    edit_path = Proj.edit_path;
                    Swap(T.areas, areas);
                    T.embed_objs.del();
                    T.embed_objs_rect = embed_objs_rect;
                    Swap(T.waters, waters);
                    T.water_rect = water_rect;
                    T.world_ver = &world_ver;
                } // these elements should be changed only under critical section
                io_wait.on();
                if (!io_thread.active())
                    io_thread.create(IOFunc, this);
                return true;
            }
        }
    return false;
...
void BuilderClass::update(bool background, C UID *priority_world_id) {
    ...
    if (!processing()) // process only 1 world at a time
    {
        ...
        if (process(*world_ver, *world_data, world_ver->rebuild.lockedKey(i)))
            goto queued; // don't check other world/areas
                    ...
                    for (int y = WorldEdit.visible_area.min.y; y <= WorldEdit.visible_area.max.y; y++)
                        for (int x = WorldEdit.visible_area.min.x; x <= WorldEdit.visible_area.max.x; x++)
                            if (WorldEdit.ver->rebuild.find(VecI2(x, y)))
                                if (process(*WorldEdit.ver, *world_data, VecI2(x, y)))
                                    goto queued; // don't check other world/areas
    ...
    if (finished_processing) {
                ...
                if (area_build.world_ver)                                                    // if has valid world
                    if (AreaVer *area_ver = area_build.world_ver->areas.find(area_build.xy)) // if exists in world
                    {
                        ...
                        if (area_build.build) // if still has something
                        {
                                    ...
                                    if (area.loaded) // if area is loaded
                                    {
                                        if (area_build.build & AREA_REBUILD_HM) // has heightmap changed
                                        {
                                            if (area.hm) {
                                                ...
                                                Swap(area.hm->mtrls, area_build.hm_mtrls);
                                                area.setChanged();
                                            }
                                            area.delayedValidateRefs();
                                        }
                                        if (area_build.build & (AREA_REBUILD_HM_MESH | AREA_REBUILD_HM_MESH_SIMPLIFY))
                                            if (area.hm) // has mesh changed
                                            {
                                                Swap(area.hm->mesh, area_build.hm_mesh);
                                                area.setShader();
                                                area.setChanged();
                                            }
                                        if (area_build.build & AREA_REBUILD_HM_PHYS)
                                            if (area.hm) // has phys changed
                                            {
                                                Swap(area.hm->phys, area_build.hm_phys);
                                                area.setChanged();
                                            }
                                        if (area_build.build & AREA_REBUILD_EMBED_OBJ) // has embedded object changed
                                        {
                                            Swap(area.obj_mesh, area_build.obj_mesh);
                                            Swap(area.obj_phys, area_build.obj_phys);
                                            area.setChanged();
                                        }
                                        if (area_build.build & AREA_REBUILD_WATER) // has water changed
                                        {
                                            Swap(area.waters, area_build.waters);
                                            area.setChanged();
                                        }
                                        if (area_build.build & AREA_REBUILD_PATH) // has path changed
                                        {
                                            // unlink first, set new, and link to the PathWorld
                                            WorldEdit.path_world.set(null, area.xy);
                                            Swap(area.path, area_build.path);
                                            area.setChanged();
                            ...
                            // put into file
                            Chunks chunks;
                            chunks.load(area_build.game_path, WorldAreaSync); // load previous chunks
                            if (area_build.build & AREA_REBUILD_HM) {
                                if (!area_build.hm_height_map.is())
                                    chunks.delChunk("Heightmap");
                                else {
                                    File f;
                                    f.writeMem();
                                    area_build.hm_height_map.save(f);
                                    area_build.hm_mtrl_map.save(f);
                                    f.putInt(area_build.hm_mtrls.elms());
                                    REPA(area_build.hm_mtrls)
                                    PutStr(f, area_build.hm_mtrls[i].valid() ? EncodeFileName(area_build.hm_mtrls[i]) : S); // in future version replace this with 'putAsset' and save in order
                                    f.pos(0);
                                    chunks.setChunk("Heightmap", 0, f);
                                }
                            }
                            if (area_build.build & (AREA_REBUILD_HM_MESH | AREA_REBUILD_HM_MESH_SIMPLIFY)) {
                                if (!area_build.hm_mesh.is())
                                    chunks.delChunk("HeightmapMesh");
                                else {
                                    File f;
                                    area_build.hm_mesh.save(f.writeMem(), Proj.game_path);
                                    f.pos(0);
                                    chunks.setChunk("HeightmapMesh", 0, f);
                                }
                            }
                            if (area_build.build & AREA_REBUILD_HM_PHYS) {
                                if (!area_build.hm_phys.is())
                                    chunks.delChunk("HeightmapPhys");
                                else {
                                    File f;
                                    area_build.hm_phys.save(f.writeMem());
                                    f.pos(0);
                                    chunks.setChunk("HeightmapPhys", 0, f);
                                }
                            }
                            if (area_build.build & AREA_REBUILD_EMBED_OBJ) {
                                // mesh
                                {
                                    if (!area_build.obj_mesh.is())
                                        chunks.delChunk("ObjectMesh");
                                    else {
                                        File f;
                                        area_build.obj_mesh.save(f.writeMem(), Proj.game_path);
                                        f.pos(0);
                                        chunks.setChunk("ObjectMesh", 0, f);
                                    }
                                }
                                // phys
                                {
                                    if (!area_build.obj_phys.is())
                                        chunks.delChunk("ObjectPhys");
                                    else {
                                        File f;
                                        area_build.obj_phys.save(f.writeMem());
                                        f.pos(0);
                                        chunks.setChunk("ObjectPhys", 0, f);
                                    }
                                }
                            ...
                            if (area_build.build & AREA_REBUILD_PATH) {
                                if (!area_build.path.is())
                                    chunks.delChunk("PathMesh");
                                else {
                                    File f;
                                    area_build.path.save(f.writeMem());
                                    f.pos(0);
                                    chunks.setChunk("PathMesh", 0, f);
                                }
                            }
                            if (area_build.build & AREA_REBUILD_WATER) {
                                if (!area_build.waters.elms())
                                    chunks.delChunk("Water");
                                else {
                                    File f;
                                    Save(f.writeMem(), area_build.waters, Proj.game_path);
                                    f.pos(0);
                                    chunks.setChunk("Water", 0, f);
                                }
                            }
        ...
        processed:
            ...
            if (Time.curTimeMs() - time >= delay)
                break; // stop if it's taking too long, process remaining areas in the next frame
        }
        if (!areas.elms()) {
            embed_objs.del();
            embed_objs_rect.set(0, -1);
