...
/******************************************************************************/
RayTest::Area::Area(::Area &area, int dist) : area(null), dist(0) {
    T.area = &area;
    T.dist = dist;
}
...
void RayTest::test(::Area &area) // !! this function should be multi-threaded safe !!
{
    ...
    if (heightmaps && WorldEdit.visible_area.includes(area.xy))
                ...
                if (possibleHit(frac)) // proceed with the mesh testing only if it's possible that the collision will be closer than current collisions
                {
                    ...
                    if (MtrlBrush.hole()) // in hole mode we can't use mesh testing, because the mesh keeps on being removed as we operate it which generates incorrect results
                    {
                        if (hm->heightImage().raycast(T.from, T.move, &Matrix((Vec)area.xy.xy0(), WorldEdit.areaSize()).swapYZ(), &frac))
                            setHit(frac, true, null);
                    } else {
                        MeshLod &lod = hm->mesh.lod(Mid(1, 0, hm->mesh.lods() - 1));
                        if (Sweep(from, move, lod, null, &frac))
                            setHit(frac, true, null);
                    }
                }
        }
                ...
                if (mesh->is()) {
                    ...
                    if (SweepPointBox(from, move, mesh->ext, &frac)) // make additional test of point->box sweep (this is done already in point->mesh sweep, however making it here we can perform next line of testing the mesh only if the closest collision point on the box is closer than already detected collisions)
                        if (possibleHit(frac))                       // proceed with the mesh testing only if it's possible that the collision will be closer than current collisions
                        {
                            ...
                            if (Sweep(from, move, *mesh, null, &frac))
                                setHit(frac, false, obj);
                        }
...
bool RayTest::possibleHit(flt frac) C {
    SyncLocker locker(lock);
    return !hit || frac < T.frac;
}
...
void Cursor::update(C Vec2 &screen_pos, GuiObj *go, bool on, bool db, WorldView &we) {
    byte byte_on = on;
    UpdateButtonState(&_state, &byte_on, 1);
    FlagSet(_state, BS_DOUBLE, db);
    _valid = false;
    _on_heightmap = false;
    _obj = null;
    ...
    // if(!(we.mode()==WorldView.OBJECT && we.obj_drag_view)) // don't detect cursor when dragging
    {
        ...
        Vec from, dir;
        ScreenToPosDir(screen_pos, from, dir);
        from += dir * D.viewFrom();
        ...
        if (_valid) {
            ...
            if (we.gridAlignSize() > 0) {
                if (we.gridAlignRound())
                    _xz.set(Round(_pos.x / we.gridAlignSize()), Round(_pos.z / we.gridAlignSize()));
                else
                    _xz.set(Floor(_pos.x / we.gridAlignSize()), Floor(_pos.z / we.gridAlignSize()));
                if (we.gridAlign()) {
        ...
        if (center || matrix || icon || obj2d) {
            ...
            if (center || matrix || icon)
                REPA(WorldEdit.obj_visible) {
                    ...
                    if ((matrix || obj.icon) && PosToScreen(obj.pos(), p)) {
                        d = Dist(p, screenPos());
                        if (!lit || d < dist) {
                            dist = d;
                            lit = &obj;
            ...
            if (obj2d)
                REPA(_ray_test.objs2D) {
                    Obj &obj = *_ray_test.objs2D[i];
                    if (PosToScreen(obj.pos(), p)) {
                        d = Dist(p, screenPos());
                        if (!lit || d < dist) {
                            dist = d;
                            lit = &obj;
            ...
            if (lit && dist <= (_obj ? 0.05f : 0.1f))
                _obj = lit; // if an object is already highlighted through per-face, then use smaller max-distance for per-point selection
        }
    }
}
void Cursor::removed(Obj &obj) {
    if (&obj == T._obj)
        T._obj = null;
}
