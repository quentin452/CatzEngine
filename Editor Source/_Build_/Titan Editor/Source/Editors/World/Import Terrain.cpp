    ...
    if (gpc.visible && visible()) {
        ...
        if (image_hw.is()) {
            if (channel >= 0)
                switch (channel) {
                case 0:
                    VI.shader(ShaderFiles("Main")->get("DrawTexX"));
                    break; // 'image_hw' is sRGB
                case 1:
                    VI.shader(ShaderFiles("Main")->get("DrawTexY"));
                    break; // 'image_hw' is sRGB
                case 2:
                    VI.shader(ShaderFiles("Main")->get("DrawTexZ"));
                    break; // 'image_hw' is sRGB
                case 3:
            ...
            image_hw.drawFit(rect);
            D.alpha(alpha);
        }
        TextStyleParams ts(true);
        if (image_hw.is())
            ts.resetColors(false);
        D.text(ts, rect.center(), "Image");
        if (rect_color.a)
...
::ImportTerrainClass::MaterialChannel &ImportTerrainClass::MaterialChannel::create(C Rect &rect, Color rect_color, C Str &desc) {
    ...
    remove.create(Rect_RU(rect.ru(), 0.035f, 0.035f)).func(Clear, T);
...
void ImportTerrainClass::HeightModeChanged(ImportTerrainClass &it) {
    if (it.height_prop.elms())
        it.height_prop[0].visible(it.height_mode() == 0);
}
...
void ImportTerrainClass::import() {
    if (height() < 0 && mtrl() < 0 && color() < 0) {
        Gui.msgBox(S, "No elements selected to import");
        return;
    }
    if (!areaRectValid()) {
        Gui.msgBox(S, "Import Area Size is empty");
        return;
    }
    if (!WorldEdit.elm_id.valid()) {
        Gui.msgBox(S, "No world opened");
        return;
    }
    // check if any material is selected
    if (mtrl() >= 0) {
        REPA(mtrl_channels)
        if (mtrl_channels[i].mtrl_id.valid())
            goto have_mtrl;
        Gui.msgBox(S, "Please drag and drop at least one material\nto the \"Materials for image channels\" slots");
        return;
    } else if (!Proj.hm_mtrl_id.valid()) {
        WorldEdit.highlightHmMtrl();
        Gui.msgBox(S, "Please drag and drop a material to the terrain material slot");
        return;
    }
have_mtrl:
...
RectI ImportTerrainClass::areaRect() C {
    switch (pos_mode()) {
    default:
        return RectI(area).extend(radius);
    case 1: {
        RectI r(area);
        r.max.x += size.x;
        r.max.y += size.y;
...
void ImportTerrainClass::create() {
    ...
    Gui += super::create(Rect_C(0, 0, height_w + mtrl_w + color_w, 1.06f), "Import Heightmap").hide();
    button[2].show();
    ...
    area_p.New().create("AreaY", MEMBER(ImportTerrainClass, area.y)).autoData(this).mouseEditSpeed(5);
    AddProperties(area_p, T, Vec2(0.50f, y), h, 0.15f, &ts_l);
    radius_p.New().create("RadiusX", MEMBER(ImportTerrainClass, radius.x)).autoData(this).mouseEditSpeed(5).min(0);
    radius_p.New().create("RadiusY", MEMBER(ImportTerrainClass, radius.y)).autoData(this).mouseEditSpeed(5).min(0);
    AddProperties(radius_p, pos_mode.tab(0), Vec2(0.85f, y), h, 0.15f, &ts_l);
    size_p.New().create("SizeX", MEMBER(ImportTerrainClass, size.x)).autoData(this).mouseEditSpeed(5);
    size_p.New().create("SizeY", MEMBER(ImportTerrainClass, size.y)).autoData(this).mouseEditSpeed(5);
    AddProperties(size_p, pos_mode.tab(1), Vec2(0.85f, y), h, 0.15f, &ts_l);
    target_p.New().create("TargetX", MEMBER(ImportTerrainClass, target.x)).autoData(this).mouseEditSpeed(5);
    target_p.New().create("TargetY", MEMBER(ImportTerrainClass, target.y)).autoData(this).mouseEditSpeed(5);
    AddProperties(target_p, pos_mode.tab(2), Vec2(0.85f, y), h, 0.15f, &ts_l);
    y -= h * 2.5f;
    ...
    T += mtrl.create(Rect_C(x + mtrl_w / 2, y, 0.2f, 0.055f), 0, mtrl_t, Elms(mtrl_t));
    {
        ...
        REPA(mtrl_channels)
        mtrl.tab(0) += mtrl_channels[i].remove;
    }
    x += mtrl_w;
...
void ImportTerrainClass::erasing(C UID &elm_id) {
    REPA(ImportTerrainTasks)
    if (ImportTerrainTasks[i].world_id == elm_id)
        ImportTerrainTasks.removeValid(i, true);
}
void ImportTerrainClass::drag(Memc<UID> &elms, GuiObj *obj, C Vec2 &screen_pos) {
    REPAD(c, mtrl_channels)
    if (obj == &mtrl_channels[c]) {
        REPA(elms)
...
void ImportTerrainClass::drop(Memc<Str> &names, GuiObj *obj, C Vec2 &screen_pos) {
    ...
    if (obj == img[i]) {
        REPAD(n, names)
        if (ExtType(GetExt(names[n])) == EXT_IMAGE) {
            img[i]->load(names[n]);
            break;
        }
        break;
...
void ImportTerrainTask::create(ImportTerrainClass &it) {
    Gui += super::create(Rect_C(0, 0, 1, 0.10f), "Import Heightmap Task");
    button[2].show().func(Stop, T);
    T += progress.create(Rect_LU(0, 0, clientWidth(), clientHeight()).extend(-0.01f));

    area_rect = it.areaRect();
    if (!area_rect.validX())
        Swap(area_rect.min.x, area_rect.max.x);
    if (!area_rect.validY())
        Swap(area_rect.min.y, area_rect.max.y);
    area_xy = area_rect.min;

    if (height_do = (it.height() >= 0))
        it.height_image.image_sw.copy(height_image);
    if (mtrl_do = (it.mtrl() >= 0))
        it.mtrl_image.image_sw.copy(mtrl_image);
    if (color_do = (it.color() >= 0))
        it.color_image.image_sw.copy(color_image);
...
void ImportTerrainTask::MaterialIntensities::addMtrl(byte index, flt intensity) {
    if (intensity > 0) {
        REP(mtrls)
        if (mtrl[i].index == index) {
            mtrl[i].intensity += intensity;
            return;
        } // find existing
        if (InRange(mtrls, mtrl)) {
            Mtrl &m = mtrl[mtrls++];
            m.index = index;
    ...
    if (AreaVer *area_ver = world_ver->areas.get(area_xy)) {
        ...
        if (height_do) {
            ...
            REPD(x, res) {
                ...
                switch (height_mode) {
                default:
                    hm.height(x, y, Lerp(height.x, height.y, h));
                    break; // set
                case 1:
                    hm.height(x, y, hm.height(x, y) + height.y * h);
                    break; // add
                case 2:
                    hm.height(x, y, hm.height(x, y) - height.y * h);
                    break; // sub
        ...
        if (mtrl_do) {
            VecB4 mtrl_channel_index;
            REPAO(mtrl_channel_index.c) = hm.getMaterialIndex0(mtrl_channels[i]);
            REPD(y, res)
            REPD(x, res) {
                flt fx = (area_xy.x - area_rect.min.x + x / flt(res - 1)) / (area_rect.w() + 1),
                    fy = 1 - (area_xy.y - area_rect.min.y + y / flt(res - 1)) / (area_rect.h() + 1);
                Vec4 c = mtrl_image.colorFCubicPlus(fx * mtrl_image.w(), fy * mtrl_image.h(), true);
                REPA(mtrl_channel_index)
                if (!mtrl_channel_index.c[i])
                    c.c[i] = 0; // if material wasn't set, then clear its intensity

                if (mtrl_mode == 0 || empty) // set
                {
                    if (c.any())
                        hm.setMaterialSafe(x, y, mtrl_channel_index, c); // if any material then set it, if not then skip
                } else                                                   // blend
                {
                    c *= mtrl_blend;
                    VecB4 src_m;
                    Vec4 src_i;
                    hm.getMaterial(x, y, src_m, src_i);
                    ...
                    REPA(c)
                    mi.addMtrl(mtrl_channel_index.c[i], c.c[i]);
                    mi.sort();
                    if (mi.mtrls) {
                        VecB4 m;
                        Vec4 i;
...
void ImportTerrainTask::update() {
    ...
    if (!chunk)
        ...
        hm.load(File().readMem(chunk->data(), chunk->elms()), Proj.game_path);

    import(hm);
    File temp;
    hm.save(temp.writeMem(), Proj.game_path);
    temp.pos(0);
...
bool ImportTerrainTask::step() {
    if (!area_rect.includesX(area_xy.x)) {
        ...
        if (!area_rect.includesY(++area_xy.y))
            return false;
    }
    update();
