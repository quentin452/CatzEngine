...
/******************************************************************************/
void ObjListClass::GroupChanged(ObjListClass &ol) {
    REP(2)
    ol.list.columnVisible(i, i != ol.group());
    ol.set();
}
...
ObjListClass &ObjListClass::create() {
    super::create(Rect_RD(D.w(), -D.h(), 1.09f, 1.2f), "Object List").hide();
    button[1].show();
    button[2].show().func(Hide, T);
    flag |= WIN_RESIZABLE;
    ts.reset().size = 0.038f;
    ts.align.set(1, 0);
    ts2.reset().size = 0.033f;
    cchar8 *group_t[] =
        {
            "Class",
            "Object",
    ...
    T += filter.create(Rect_L(group.rect().right() + Vec2(0.03f, 0), 0.24f, 0.05f)).func(FilterChanged, T);
    filter.kbSet();
    filter.reset.show();
    filter.show_find = true;
    filter.hint = "Find Object";
    ...
    list.flag |= LIST_TYPE_SORT | LIST_RESIZABLE_COLUMNS | LIST_MULTI_SEL;
...
::ObjListClass::Elm &ObjListClass::dataNew(C Str &group) {
    if (T.group() >= 0) {
        int i;
        if (data.binarySearch(group, i, CompareOL)) {
            Elm &child = data[i].children.New();
            child.offset = 0.03f;
            return child;
        }
        Elm &elm = data.NewAt(i);
        elm.group = group;
        Elm &child = elm.children.New();
        child.offset = 0.03f;
        return child;
    }
    return data.New();
    ...
    FREPA(data) {
        ...
        if (elm.obj) {
            if (filter_is_id) {
                ...
                if (!has_filter)
                    elm_visible = false;
            } else if (hide_grass && elm.obj->params.access == OBJ_ACCESS_GRASS) {
                elm_visible = false;
                has_filter = false;
            } else if (filter.is()) {
                has_filter = (ContainsAll(elm.obj_type, filter) || ContainsAll(elm.base, filter) || (filter_params && elm.obj->params.paramValuesContain(filter, Proj)));
                if (!has_filter)
                    elm_visible = false;
            }
        }
        ...
        if (!elm.obj && !any_child_has_filter_or_visible)
            visible[vi] = false; // if this is a group && no children (contain filter or are visible), then hide this group
...
void ObjListClass::setVisible() {
    Memt<bool> visible;
    SetVisible(visible, ReplaceSlash(filter()), params(), grass(), data);
    list.setDataNode(data, visible, true);
    list.sel.clear();
    FREP(list.totalElms())
    if (Elm *elm = list.absToData(i))
        if (elm->obj && elm->obj->selected)
            list.sel.add(i);
}
...
void ObjListClass::set() {
    ...
    FREPA(WorldEdit.obj_visible) {
        ...
        Str type;
        if (obj.params.access != OBJ_ACCESS_CUSTOM && InRange(obj.params.access, ObjAccessNamesElms))
            type = ObjAccessNames[obj.params.access];
        else if (::Elm *obj_class = Proj.findElm(obj.params.type))
            type = obj_class->name;
        Elm &elm = dataNew((group() == 0) ? type : (group() == 1) ? base
                                                                  : S);
        elm.obj = &obj;
        elm.pos = obj.pos();
    ...
    if (group() >= 0)
        REPA(data)
    data[i].group = S + '<' + data[i].group + '>';
    if (group() >= 0)
        REPA(opened) {
            int di;
            if (data.binarySearch(opened[i], di, CompareOL))
                data[di].opened = true;
        }
    if (group() >= 0) {
        uint type = 0;
...
void ObjListClass::selChanged() {
    ...
    FREPA(list.sel)
    if (Elm *elm = list.absToData(list.sel[i]))
        if (elm->obj)
            Selection.select(*elm->obj);
}
...
void ObjListClass::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible() && _changed)
        set();          // first set data
    super::update(gpc); // now update list
    if (gpc.visible && visible()) {
        REPA(MT)
        if (MT.guiObj(i) == &list && MT.bp(i))
            if (Elm *elm = list()) {
                if (elm->obj && MT.bd(i) && list.selMode() == LSM_SET)
                    WorldEdit.v4.moveTo(elm->obj->center()); // camera center
                if (elm->children.elms()) {
                    opened.toggle(elm->group);
                    elm->opened ^= 1;
                    setVisible();
                } // open
            }
