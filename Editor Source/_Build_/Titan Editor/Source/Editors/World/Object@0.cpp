...
void NewObjs(C Vec &pos) {
    ...
    if (Selection.elms()) {
        ...
        Matrix delta(-center);
        if (WorldEdit.obj_random_angle)
            delta.rotateY(Random.f(PI2));
        delta.move(pos);
...
void ScaleOrgObj() {
    if (Selection.elms()) {
        ...
        REPA(Selection) {
            ...
            {
                flt scale = obj.matrix.avgScale();
                if (!Equal(scale, 1)) {
                    obj.matrix.orn() /= scale;
                    obj.setChanged(true);
                    Proj.transformApply(base_id, Matrix(scale));
                    *scaled(base_id) = scale;
                }
            }
        }
        if (scaled.elms())
...
void ObjAlignGrid() {
    ...
    REPA(Selection) {
        Obj &obj = Selection[i];
        bool ground = obj.onGround();
        obj.alignGrid();
        if (ground && WorldEdit.obj_hm_align())
            obj.alignTerrain(false);
        if (ground && WorldEdit.hm_align_nrm)
            obj.alignNormal();
    }
    WorldEdit.objTransChanged();
}
...
void OpenObjMaterial() {
    ...
    FREPA(Selection) {
        ...
        if (C MeshPtr &mesh = obj.mesh) {
            ...
            if (C MaterialPtr &mtrl = mesh->parts[p].variation(v))
                mtrls.include(mtrl); // use 'include' and not 'binaryInclude' to preserve order
        }
    }
    MtrlEdit.set(mtrls);
}
...
cchar8 *Obj::variationName() C {
    if (mesh_proper)
        return mesh_proper->variationName(mesh_variation);
    return null;
}
Matrix Obj::drawMatrix() {
    Matrix m = matrix;
    if (selected)
        m.pos += WorldEdit.obj_pos.delta;
    return m;
}
Vec Obj::pos() {
    Vec pos = matrix.pos;
    if (selected)
        pos += WorldEdit.obj_pos.delta;
    return pos;
}
Vec Obj::center() {
...
void Obj::setVisibility() {
    ...
    if (visible) {
        if (InRange(params.access, WorldEdit.show_obj_access))
            visible = WorldEdit.show_obj_access[params.access];
        if (visible)
            if (params.access == OBJ_ACCESS_CUSTOM && WorldEdit.hide_obj_classes.binaryHas(params.type))
                visible = false;
    }
    if (!visible) {
        Selection.removed(T);
...
void Obj::remove() {
    if (!removed) {
        setUndo();
        if (physPath())
            setChangedPaths(); // call before 'setChanged' because it may set path areas to non-existing
        setRemoved(true);
...
void Obj::create(C Obj &src) {
    super::create(src);
    mesh = src.mesh;
    mesh_proper = src.mesh_proper;
    mesh_variation = src.mesh_variation;
    phys = src.phys;
}
bool Obj::sync(C ObjData &src, cchar *edit_path) {
    ...
    if (super::sync(src, edit_path)) {
        if (base != params.base())
            setMeshPhys();
        else if (mesh_variation_id != params.mesh_variation_id)
            setMeshVariation(); // 'setMeshPhys' already calls 'setMeshVariation'
...
{
    if (area)
        if (WorldData *world = area->world)
            if (WorldVer *world_ver = world->ver)
                Proj.rebuildEmbedObj(id, area_xy ? *area_xy : area->xy, *world_ver, false); // 'rebuild_game_area_objs=false' this is Obj which belongs to Area, and Area will be saved to game ver on unload
    return T;
}
Obj &Obj::setChangedPaths() // must be called after 'setChangedEmbed'
{
    if (area && area->world && area->world->ver)
        area->world->ver->rebuildPaths(id, area->xy);
    return T;
}
Obj &Obj::setChanged(bool matrix) // this needs to be called after making all changes
{
    if (area)
        area->setChangedObj(T); // this will set ObjVer, call this before all other things
    if (matrix) {
        matrix_time.getUTC();
        setChangedEmbed();
        if (physPath())
            setChangedPaths();
    } // call 'setChangedEmbed' before 'setChangedPaths'
    setDraw();
    return T;
}
void Obj::setUndo(cptr type, bool as_new) {
    if (area && area->world)
        WorldEdit.undos.set(T, type, as_new);
}
...
void Obj::scaleBy(flt f) {
    setUndo();
    matrix.orn() *= f;
    setChanged(true);
}
void Obj::scaleXBy(flt f) {
    setUndo();
    matrix.x *= f;
    setChanged(true);
}
void Obj::scaleYBy(flt f) {
    setUndo();
    matrix.y *= f;
    setChanged(true);
}
void Obj::scaleZBy(flt f) {
    setUndo();
    matrix.z *= f;
    setChanged(true);
}
void Obj::randomRot() {
    setUndo();
    matrix.rotateYL(Random.f(PI2));
    setChanged(true);
}
void Obj::randomRot90() {
    setUndo();
    matrix.rotateYL(Random(4) * PI_2);
    setChanged(true);
}
void Obj::resetRot() {
    setUndo();
    matrix.orn().setScale(matrix.scale());
    setChanged(true);
}
void Obj::alignGrid() {
    Vec pos = matrix.pos;
    if (WorldEdit.gridAlignSize() > 0) {
        pos.x = AlignRound(pos.x, WorldEdit.gridAlignSize());
        pos.z = AlignRound(pos.z, WorldEdit.gridAlignSize());
    }
    if (WorldEdit.gridAlignSizeY() > 0)
        pos.y = AlignRound(pos.y, WorldEdit.gridAlignSizeY());
    moveTo(pos);
}
void Obj::alignTerrain(bool force, cptr undo_type) {
    if (area && area->hm)
        moveTo(Vec(matrix.pos.x, area->hmHeight(pos()), matrix.pos.z), undo_type);
    else if (force)
        moveTo(Vec(matrix.pos.x, WorldEdit.grid_plane_level, matrix.pos.z), undo_type);
}
...
void Obj::draw() {
    ...
    if (mesh && Frustum(*mesh, m)) {
        if (WorldEdit.mode() == WorldView::OBJECT) {
            Color col_lit, col_shape;
            getCol(col_lit, col_shape);
    ...
    case EDIT_OBJ_LIGHT_POINT: {
        LightLinear l(m.x.length(), m.pos, light_col);
        l.add(light_cast_shadows, this);
    } break;
    case EDIT_OBJ_LIGHT_CONE: {
        LightCone l(m.x.length(), m.pos, !m.z, light_col);
        l.pyramid.scale = light_angle;
    ...
    case EDIT_OBJ_PARTICLES: {
        ...
        {
        case RM_BLEND:
            WorldEdit.obj_blend.binaryInclude(this, ComparePtr);
            break;
        case RM_PALETTE:
            WorldEdit.obj_palette[0].binaryInclude(this, ComparePtr);
            break;
        case RM_PALETTE1:
            WorldEdit.obj_palette[1].binaryInclude(this, ComparePtr);
            break;
        }
    } break;
    }
...
void Obj::setDraw() {
    switch (edit_type) {
    case EDIT_OBJ_LIGHT_POINT: {
        if (C EditParam *p = params.findParam("color"))
            light_col = p->asVec();
        else
            light_col = 1;
        if (C EditParam *p = params.findParam("cast shadows"))
            light_cast_shadows = p->asBool();
        else
    ...
    case EDIT_OBJ_LIGHT_CONE: {
        ...
        if (C EditParam *p = params.findParam("cast shadows"))
            ...
            light_cast_shadows = true;
        if (C EditParam *p = params.findParam("angle"))
            light_angle = Max(p->asFlt(), 0);
        else
            light_angle = 1;
        if (C EditParam *p = params.findParam("falloff"))
            light_falloff = Sat(p->asFlt());
        else
            light_falloff = 0.5f;
    } break;
    }
...
void Obj::setMeshPhys() {
    if (removed) {
        ...
        default:
            ...
            break;
        case EDIT_OBJ_PARTICLES:
            icon = Proj.icon_particles;
            break;
        case EDIT_OBJ_LIGHT_POINT:
        case EDIT_OBJ_LIGHT_CONE:
            icon = Proj.icon_light;
            break;
        }
        setDraw();
...
#endif
        T.mesh_proper = mesh;
        if (edit_type == EDIT_OBJ_MESH)
            if (!mesh || !mesh->is())
                mesh = &WorldEdit.dummy_mesh;
        T.mesh = mesh;
        T.phys = phys;
        setMeshVariation();
    }
