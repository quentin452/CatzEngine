...
void WorldView::editObj() {
    if (Selection.elms()) {
        ...
        if (Kb.ctrlCmd() && Kb.shift() && !Kb.alt())
            ...
            op = OP_MOVE_Y;
        else if (Kb.bp(KB_INS))
            op = OP_INS;
        if (op == OP_ROT_GROUP && Selection.elms() <= 1)
            op = OP_ROT_Y;
        if (op == OP_INS) {
            obj_pos.apply();
            if (cur.valid() && (Ms.bp(1) || Kb.kf(KB_INS)))
                NewObjs(cur.pos());
            REPA(cur_touch)
            if (cur_touch[i].valid() && cur_touch[i].pd())
                NewObjs(cur_touch[i].pos());
        }
        ...
        bool view_on = (cur.onViewport() && Ms.b(1));
        if (view_on) {
            delta = Ms.d();
            Ms.freeze();
        }
        ...
        if (cur_touch[i].onViewport() && cur_touch[i].on()) {
            view_on = true;
            if (Touch *touch = cur_touch[i].touch())
                delta += touch->ad();
        }
        if (view_on && op != OP_INS) // edit objects matrix
        {
            ...
            if (!v4.perspective() || !v4.fpp())
                obj_edit_speed = CamMoveScale(v4.perspective());
            else if (first_push)
                obj_edit_speed = Dist(center, ActiveCam.matrix.pos) * 0.75f;
            ...
            if (first_push || Kb.bp(KB_LALT) || Kb.bp(KB_RALT))
                obj_edit_angle = 0;
            obj_edit_angle += delta.sum() * multiplier;
            if (first_push)
                obj_edit_vec.zero();
                ...
                REP(curTotal()) {
                    ...
                    if (cur.onViewport() && cur.on() && (touch ? touch_edit : Ms.b(1))) {
                        ...
                        if (!z.normalize())
                            ...
                            z.chs();
                        obj_edit_vec += x * (cur_delta.x * mul) + z * (cur_delta.y * mul);
                    }
                }
            ...
            Vec d = 0;
            {
                ...
                if (op == OP_MOVE && obj_axis >= 0 && Selection.elms()) {
                    ...
                    switch (obj_axis) {
                    case 0:
                        d = AlignDirToCam(m.x, delta) * mul / 3;
                        break;
                    case 1:
                        d = AlignDirToCam(m.y, delta) * mul / 3;
                        break;
                    case 2:
                        d = AlignDirToCam(m.z, delta) * mul / 3;
                        break;
                    }
                } else {
                    if (!align_xz) {
                        ...
                        if (Abs(obj_edit_vec.x) >= gridAlignSize() || Abs(obj_edit_vec.z) >= gridAlignSize()) {
                    ...
                    if (!align_y)
                        ...
                        if (Abs(obj_edit_vec.y) >= gridAlignSizeY())
                            d.y = AlignTrunc(obj_edit_vec.y, gridAlignSizeY());
                    }
                }
            }
            if (Abs(d.y) > EPS && d.xz().length() <= EPS)
                align_hm = false; // if moving only vertically, then disable aligning to heightmap (for example, if we're moving along Y axis, then we want to ignore aligning)
            ...
            // edit objects
            {
                ...
                REPA(Selection)  // for each object
                {
                    ...
                    if (op == OP_SCALE)
                        switch (obj_axis) {
                        case 0:
                            obj.scaleXBy(ScaleFactor(AlignDirToCamEx(obj.matrix.x, delta) * multiplier));
                            break;
                        case 1:
                            obj.scaleYBy(ScaleFactor(AlignDirToCamEx(obj.matrix.y, delta) * multiplier));
                            break;
                        case 2:
                            obj.scaleZBy(ScaleFactor(AlignDirToCamEx(obj.matrix.z, delta) * multiplier));
                            break;
                        default:
                            obj.scaleBy(ScaleFactor(delta.sum() * multiplier));
                            break;
                        }
                    else {
                        ...
                        case OP_SEPARATE: {
                            Vec2 d = matrix.pos.xz() - center.xz();
                            d *= ScaleFactor(delta.sum() * multiplier);
                        ...
                        case OP_ROT: {
                            switch (obj_axis) {
                            case 0:
                                matrix.orn().rotateXL(a);
                                break;
                            case 1:
                                matrix.orn().rotateYL(a);
                                break;
                            case 2:
                                matrix.orn().rotateZL(a);
                                break;
                            default:
                                matrix.orn().rotate(ActiveCam.matrix.z, -a);
                                break;
                            }
                        } break;
                        ...
                        if (matrix.pos != obj.matrix.pos) {
                            ...
                            if (align_hm && obj.onGround() && new_area->hm)
                                matrix.pos.y = new_area->hmHeight(matrix.pos);
                            if (align_nrm && obj.area) {
...
/******************************************************************************/
void ObjPos::Cancel(ObjPos &op) {
    op.reset();
    WorldEdit.obj_op.set(WorldView::OP_SEL);
}
void ObjPos::Changed(ObjPos &op) {
    if (op.props.elms() >= 3) {
        Vec center = 0;
        if (Selection.elms())
            center = Selection[0].matrix.pos;
...
ObjPos &ObjPos::create(C Vec2 &up) {
    ts.reset().size = 0.04f;
    ts.align.set(1, 0);
    super::create(Rect_U(up, 0.255f, 0.22f)).skin(&TransparentSkin, false).hide();
    kb_lit = false;
...
void ObjPos::apply() {
    if (delta.any()) {
        applying = true;
        REPAO(Selection).moveTo(Selection[i].matrix.pos + delta);
        delta = 0;
        applying = false;
    }
}
...
void ObjPos::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        if (Ms.bp(2) && contains(Gui.ms()))
            WorldEdit.obj_op.set(-1);
        REPA(props)
        if (props[i].button()) // if at least one property is being edited then adjust scale of editing
        {
            flt speed;
            if (!WorldEdit.v4.perspective() || !WorldEdit.v4.fpp())
                speed = CamMoveScale(WorldEdit.v4.perspective());
            else {
                Vec center = 0;
                if (Selection.elms()) {
                    REPA(Selection)
                    center += Selection[i].matrix.pos;
                    center /= Selection.elms();
                }
...
ObjScale &ObjScale::create(C Vec2 &up) {
    ts.reset().size = 0.04f;
    ts.align.set(1, 0);
    super::create(Rect_U(up, 0.30f, 0.19f)).skin(&TransparentSkin, false).hide();
    kb_lit = false;
    ...
    REPAO(props).mouseEditMode(PROP_MOUSE_EDIT_SCALAR).autoData(this);
    Zero(multi_val);
    return T;
}
void ObjScale::reset(int skip) {
    ...
    scale = 1;
    FREPA(Selection) {
        C Matrix3 &m = Selection[i].matrix;
        Vec s = m.scale();
        if (!i)
            scale = s;
        else {
    ...
    if (props.elms() >= 4) {
        if (multi_val[0])
            props[0].textline.set(MultipleName, QUIET);
        else if (skip != 0)
            props[0].set(scale.x, QUIET);
        if (multi_val[1])
            props[1].textline.set(MultipleName, QUIET);
        else if (skip != 1)
            props[1].set(scale.y, QUIET);
        if (multi_val[2])
            props[2].textline.set(MultipleName, QUIET);
        else if (skip != 2)
            props[2].set(scale.z, QUIET);
        if (multi_val[3])
            props[3].textline.set(MultipleName, QUIET);
        else if (skip != 3)
            props[3].set(scale.x, QUIET);
    }
}
...
ObjRot::Rot::operator Matrix3() {
    Matrix3 m;
    m.setRotateZ(angles.z).rotateXY(angles.x, angles.y);
    m.x *= scale.x;
    m.y *= scale.y;
    m.z *= scale.z;
    return m;
}
ObjRot::Rot::Rot(C Matrix3 &m) {
    angles = Angles(m);
    scale = m.scale();
}
void ObjRot::ChangedX(ObjRot &os) {
    if (os.props.elms() >= 3) {
        ...
        {
            flt d = DegToRad(angle - os.angles.x);
            REPA(Selection) {
                Obj &obj = Selection[i];
                Rot r = obj.matrix;
                r.angles.x += d;
            ...
            os.angles.x = angle;
            if (os.multi_val)
                os.props[0].textline.set(MultipleName, QUIET);
        } else // set rotation
        {
            os.angles.x = angle;
...
void ObjRot::ChangedY(ObjRot &os) {
    if (os.props.elms() >= 3) {
        ...
        {
            flt d = DegToRad(angle - os.angles.y);
            REPA(Selection) {
                Obj &obj = Selection[i];
                Rot r = obj.matrix;
                r.angles.y += d;
            ...
            os.angles.y = angle;
            if (os.multi_val)
                os.props[1].textline.set(MultipleName, QUIET);
        } else // set rotation
        {
            os.angles.y = angle;
            Vec a = DegToRad(os.angles);
...
void ObjRot::ChangedZ(ObjRot &os) {
    if (os.props.elms() >= 3) {
        ...
        {
            flt d = DegToRad(angle - os.angles.z);
            REPA(Selection) {
                Obj &obj = Selection[i];
                Rot r = obj.matrix;
                r.angles.z += d;
            ...
            os.angles.z = angle;
            if (os.multi_val)
                os.props[2].textline.set(MultipleName, QUIET);
        } else // set rotation
        {
            os.angles.z = angle;
            Vec a = DegToRad(os.angles);
...
ObjRot &ObjRot::create(C Vec2 &up) {
    ts.reset().size = 0.04f;
    ts.align.set(1, 0);
    super::create(Rect_U(up, 0.255f, 0.147f)).skin(&TransparentSkin, false).hide();
    kb_lit = false;
    ...
    AddProperties(props, T, Vec2(0.01f, -0.01f), 0.043f, 0.20f, &ts);
    REPAO(props).autoData(this).mouseEditSpeed(20);
    return T;
}
void ObjRot::reset(int skip) {
    ...
    FREPA(Selection) {
        ...
        if (!i)
            angles = a;
        else if (!Equal(angles, a, EPS_ANIM_ANGLE))
            multi_val = true;
    }
    ...
    if (props.elms() >= 3) {
        if (multi_val)
            ...
            props[0].set(angles.x, QUIET);
        if (multi_val)
            props[1].textline.set(MultipleName, QUIET);
        else if (skip != 1)
            props[1].set(angles.y, QUIET);
        if (multi_val)
            props[2].textline.set(MultipleName, QUIET);
        else if (skip != 2)
            props[2].set(angles.z, QUIET);
    }
}
...
ObjGrid &ObjGrid::create() {
    super ::create(Rect_LU(0, 0, 0.315f, 0.125f)).skin(&TransparentSkin, false);
    kb_lit = false;
    T += bxz.create(Rect_L(0.01f, -0.035f, 0.17f, 0.045f), "Horizontal").focusable(false).desc("Enable Horizontal Grid Aligning");
    bxz.mode = BUTTON_TOGGLE;
    bxz.set(true);
    T += by.create(Rect_L(0.01f, -0.090f, 0.17f, 0.045f), "Vertical").focusable(false).desc("Enable Vertical Grid Aligning");
    by.mode = BUTTON_TOGGLE;
...
void ObjGrid::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        if (Ms.bp(2) && contains(Gui.ms()))
            WorldEdit.show_obj_grid.toggle(0);
    }
}
...
WaypointPos &WaypointPos::create(C Vec2 &up) {
    ts.reset().size = 0.04f;
    ts.align.set(1, 0);
    super::create(Rect_U(up, 0.255f, 0.15f)).skin(&TransparentSkin, false).hide();
    kb_lit = false;
    props.New().create("X", MemberDesc(DATA_REAL));
    props.New().create("Y", MemberDesc(DATA_REAL));
    props.New().create("Z", MemberDesc(DATA_REAL));
...
void WaypointPos::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        if (Ms.bp(2) && contains(Gui.ms()))
            WorldEdit.waypoint_op.set(-1); // close
        REPA(props)
        if (props[i].button()) // if at least one property is being edited then adjust scale of editing
        {
            flt speed = -1;
            if (!WorldEdit.v4.perspective() || !WorldEdit.v4.fpp())
                speed = CamMoveScale(WorldEdit.v4.perspective());
            else if (WorldEdit.sel_waypoint)
                if (EditWaypointPoint *point = WorldEdit.sel_waypoint->find(WorldEdit.sel_waypoint_point))
