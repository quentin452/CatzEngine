...
uint WorldChange::memUsage() C {
    uint size = 0;
    REPA(terrain)
    size += terrain.lockedData(i).memUsage();
    REPA(objs)
    size += objs.lockedData(i).memUsage();
    REPA(waypoints)
    size += waypoints.lockedData(i).memUsage();
    REPA(lakes)
    size += lakes.lockedData(i).memUsage();
    REPA(rivers)
    size += rivers.lockedData(i).memUsage();
    return size;
}
void WorldChange::swap(ptr user) {
    ...
    Memt<Area *> rebuild;
    REPA(terrain)
    if (Area *area = WorldEdit.getArea(terrain.lockedKey(i))) {
        bool loaded = area->loaded;
        area->load();
        Terrain &undo = terrain.lockedData(i), temp;
        temp.set(*area);
        area->undo(undo, loaded);
        Swap(undo, temp); // if the area is loaded then skip rebuilding mesh through the Builder because we will do it here
        if (loaded)
            rebuild.add(area); // want to keep in memory so rebuild immediatelly
        else
            area->unload();
    }
    WorkerThreads.process1(rebuild, WorldView::HmBuild);
    rebuild.clear();
        ...
        REPA(objs) {
            ...
            if (C ObjVer *obj_ver = WorldEdit.ver->obj.find(undo.id))
                if (Area *world_area = WorldEdit.getArea(obj_ver->area_xy)) {
                    if (!world_area->loaded)
                        rebuild.include(world_area);
                    world_area->load();
                    if (Obj *world = world_area->findobj(undo.id))
                        ...
                        {
                            if (!undo_area->loaded)
                                rebuild.include(undo_area); // if the 'undo_area' isn't loaded before 'undo' then remember it for possible unload
                            temp = *world;
                            ...
                            if (world->undo(undo, Proj.edit_path)) {
                                ...
                                if (changed_embed)
                                    world->setChangedEmbed(&undo_area->xy); // call after 'changedObj' and before 'setChangedPaths' (use new area coordinates)
                                if (changed_phys_path)
                                    world->setChangedPaths();        // call 'setChangedPaths' after 'setChangedEmbed' for old area
                                world->attach(WorldEdit, undo_area); // attach to new area
                                if (changed_phys_path)
                                    world->setChangedPaths();     // call 'setChangedPaths' after 'setChangedEmbed' for new area
    ...
    if (waypoints.elms()) {
        ...
        if (EditWaypoint *world = WorldEdit.waypoints.find(waypoints.lockedKey(i))) {
            EditWaypoint &undo = waypoints.lockedData(i), temp = *world;
            world->undo(undo);
            Swap(undo, temp);
            WorldEdit.setChangedWaypoint(world);
        }
    ...
    if (lakes.elms() || rivers.elms()) {
        ...
        if (Lake *world = WorldEdit.lakes.find(lakes.lockedKey(i))) {
            LakeBase &undo = lakes.lockedData(i), temp = *world;
            world->undo(undo);
            Swap(undo, temp);
            WorldEdit.setChangedLake(world);
        }
        REPA(rivers)
        if (River *world = WorldEdit.rivers.find(rivers.lockedKey(i))) {
            RiverBase &undo = rivers.lockedData(i), temp = *world;
            world->undo(undo);
            Swap(undo, temp);
            WorldEdit.setChangedRiver(world);
        }
...
{
    if (WorldChange *change = set(TerrainType))
        if (!change->terrain.find(area.xy))
            change->terrain(area.xy)->set(area);
}
void WorldUndo::set(C ObjData &obj, cptr type, bool as_new) {
    if (WorldChange *change = set(type))
        if (!change->objs.find(obj.id)) {
            ObjData &undo = *change->objs(obj.id);
            undo = obj;
            if (as_new)
                undo.removed = true; // if we've just created this object, then set that 'undo' was originally removed (object didn't exist yet), so when this undo is applied, then object will be removed
        }
}
void WorldUndo::set(C EditWaypoint &waypoint, cptr type) {
    if (C UID *waypoint_id = WorldEdit.ID(&waypoint))
        if (WorldChange *change = set(type))
            if (!change->waypoints.find(*waypoint_id))
                *change->waypoints(*waypoint_id) = waypoint;
}
void WorldUndo::set(C Lake &lake) {
    if (C UID *lake_id = WorldEdit.ID(&lake))
        if (WorldChange *change = set("lake"))
            if (!change->lakes.find(*lake_id))
                *change->lakes(*lake_id) = lake;
}
void WorldUndo::set(C River &river) {
    if (C UID *river_id = WorldEdit.ID(&river))
        if (WorldChange *change = set("river"))
            if (!change->rivers.find(*river_id))
                *change->rivers(*river_id) = river;
}
