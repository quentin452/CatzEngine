...
WaypointListClass &WaypointListClass::create() {
    super::create(Rect_RD(D.w(), -D.h(), 0.88f, 1.2f), "Waypoint List").hide();
    button[1].show();
    button[2].show().func(Hide, T);
    flag |= WIN_RESIZABLE;

    flt y = -0.05f;
    T += filter.create(Rect_LU(0.03f, -0.02f, 0.6f, 0.05f)).func(FilterChanged, T).desc("Find waypoint\nType waypoint name or its ID to find it");
    filter.kbSet();
    filter.reset.show();
    filter.show_find = true;
    filter.hint = "Find Waypoint";
    T += show_removed.create(Rect_LU(filter.rect().ru(), filter.rect().h())).func(FilterChanged, T).focusable(false).desc("Show removed waypoints");
    ...
    list.flag |= LIST_TYPE_SORT | LIST_RESIZABLE_COLUMNS;
...
void WaypointListClass::setCur() {
    list.cur = -1;
    FREPA(list)
    if (Elm *data = list.visToData(i))
        if (data->waypoint == WorldEdit.sel_waypoint) {
            list.cur = i;
            break;
        }
}
    ...
    FREPA(data) {
        ...
        if (elm.waypoint)
            if (show_removed() || (!elm.waypoint->removed && elm.waypoint->hasExisting())) {
                if (filter_is_id) {
                    if (C UID *id = WorldEdit.ID(elm.waypoint))
                        vis = (filter_id == *id);
                } else if (filter().is())
                    vis = ContainsAll(elm.waypoint->name, filter());
                else
                    vis = true;
            }
        visible.add(vis);
...
void WaypointListClass::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        ...
        if (MT.guiObj(i) == &list && MT.bp(i))
            if (Elm *elm = list()) {
                if (elm->waypoint && MT.bd(i) && list.selMode() == LSM_SET)
                    WorldEdit.v4.moveTo(elm->waypoint->pos()); // camera center
            }
        if (cur != list.cur)
            if (Elm *elm = list())
                if (elm->waypoint)
                    WorldEdit.selWaypoint(elm->waypoint, elm->waypoint->firstExisting());
    }
}
void WaypointListClass::removed(EditWaypoint &waypoint) {
    REPA(data)
    if (data[i].waypoint == &waypoint) {
        data[i].waypoint = null;
        setChanged();
        break;
    }
}
