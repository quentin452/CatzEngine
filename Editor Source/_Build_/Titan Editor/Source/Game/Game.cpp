...
void InitGameObjContainers() {
    ...
    FREPA(ObjType) {
        ...
        if (obj_class->editor_type == EditObjType.elmID(EDIT_OBJ_LIGHT_POINT))
            Game::World.setObjType(GetObjContainer(PointLights, point_lights), i);
        else if (obj_class->editor_type == EditObjType.elmID(EDIT_OBJ_LIGHT_CONE))
            Game::World.setObjType(GetObjContainer(ConeLights, cone_lights), i);
        else if (obj_class->editor_type == EditObjType.elmID(EDIT_OBJ_PARTICLES))
            Game::World.setObjType(GetObjContainer(ObjParticles, particles), i);
        else
...
bool InitGame() {
    ...
    if (!Game::World.NewTry(GameWorld)) {
        Gui.msgBox(S, "Can't load world");
        ExitGame();
    }
...
void UpdateGameCam() {
    ...
    if (DPad.touched) {
        Vec2 d;
        CosSin(d.x, d.y, DPad.angle + Cam.yaw);
...
bool UpdateGame() {
    ...
    if (Kb.bp(KB_LBR)) {
        flt range = Mid(D.viewRange() * 0.8f, 32.0f, MaxGameViewRange);
        D.viewRange(range);
