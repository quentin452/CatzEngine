...
void BrushClass::Slope::PushedSide(Slope &slope) {
    slope.side ^= 1;
    slope.side_b.text = (slope.side ? "<" : ">");
}
...
void BrushClass::Slope::update(C GuiPC &gpc) {
    ...
    REPA(Touches)
    if (Touches[i].guiObj() == this && Touches[i].on()) {
...
BrushClass &BrushClass::create(GuiObj &parent, C Vec2 &rd) {
    ...
    T += shape.create(Rect_L(0.02f, -0.04f, 0.125f, 0.045f), 0, (cchar8 **)null, 2).layout(TABS_HORIZONTAL).valid(true).set(0).desc(MLTC(u"Brush Shape", PL, u"Kształt pędzla", RU, u"Фигура кисти", CN, u"刷子形状"));
    shape.tab(0).image = "Gui/Misc/circle.img";
    shape.tab(1).image = "Gui/Misc/square.img";
    T += tsize.create(Vec2(0.08f, -0.10f), MLTC(u"Size", PL, u"Rozmiar", DE, u"Größe", RU, u"Размер", PO, u"Tamanho"), &ts);
    T += ssize.create(Rect_R(clientWidth() - 0.02f, -0.10f, 0.20f, 0.05f), 0.5f).desc(MLT(S + "Change size with: " + Kb.ctrlCmdName() + " + MouseWheel", PL, S + u"Zmień Rozmiar przy pomocy: " + Kb.ctrlCmdName() + " + MouseWheel", DE, u"Größe ändern mit: Strg + Mausrad", RU, S + u"Чтобы изменить размер нажмите: " + Kb.ctrlCmdName() + u" + Колесо прокрутки мыши", PO, S + "Mudar o tamanho com: " + Kb.ctrlCmdName() + " + Scroll do Rato", CN, u"改变大小:Ctrl键+鼠标滚轮"));
    T += tspeed.create(Vec2(0.08f, -0.15f), MLTC(u"Speed", PL, u"Prędkość", DE, u"Speed", RU, u"Скорость", PO, u"Velocidade"), &ts);
    T += sspeed.create(Rect_R(clientWidth() - 0.02f, -0.15f, 0.20f, 0.05f), 0.5f).desc(MLT(S + "Change speed with: Shift + MouseWheel", PL, u"Zmień Prędkość przy pomocy: Shift + MouseWheel", DE, u"Geschwindigkeit ändern mit: Shift + Mausrad", RU, u"Чтобы изменить скорость нажмите: Shift + Колесо прокрутки мыши", PO, "Mudar a velocidade com: Shift + Scroll do Rato", CN, u"改变速度:Shift键+鼠标滚轮"));
    T += tsoft.create(Vec2(0.08f, -0.20f), MLTC(u"Soft", PL, u"Miękkość", DE, u"Soft", RU, u"Мягкость", PO, u"Suavidade"), &ts);
    T += ssoft.create(Rect_R(clientWidth() - 0.02f, -0.20f, 0.20f, 0.05f), 1.0f).desc(MLT(S + "Change soft with: " + Kb.ctrlCmdName() + " + Shift + MouseWheel", PL, S + u"Zmień Miękkość przy pomocy: " + Kb.ctrlCmdName() + " + Shift + MouseWheel", DE, u"Soft ändern mit: Strg + Shift + Mausrad", RU, S + u"Чтобы изменить мягкость нажмите: " + Kb.ctrlCmdName() + u" + Shift + Колесо прокрутки мыши", PO, S + "Mudar a dureza com: " + Kb.ctrlCmdName() + " + Shift + Scroll do Rato", CN, u"改变柔软度:Ctrl键+Shift键+鼠标滚轮"));
    T += bsize.create(Rect_C(0.08f, -0.10f, (App.lang() == EN ? 0.13f : 0.14f), 0.045f), tsize.text).focusable(false);
    bsize.desc(MLTC(u"Scale size proportionally to camera distance", PL, u"Skaluj rozmiar proporcjonalnie do odległości kamery", DE, u"Größe propertional zum Kamera Abstand skalieren", RU, u"Размер масштаба кисти пропорционально растоянию до камеры", PO, u"Escalar proporcionalmente ŕ distancia da camera", CN, u"按比例缩放大小到摄像机的距离"));
    bsize.mode = BUTTON_TOGGLE;
    bsize.set(true);

    T += slope_b.create(Rect_L(shape.rect().right() + Vec2(0.01f, 0), 0.11f, 0.05f), "Slope").func(SlopeWindow, T).focusable(false).desc("Make brush to be affected by terrain slope");
    slope_b.mode = BUTTON_TOGGLE;
    T += image.create(Rect_LU(slope_b.rect().ru(), 0.12f, 0.05f), MLTC(u"Image", CN, u"图片")).func(ImageWindow, T).focusable(false).desc(MLTC(u"Use source Image as brush modifier", PL, u"Użyj źródłowego obrazka (Image) jako modyfikator pędzla", DE, u"Bild als Pinsel Modifier verwenden", RU, u"Использовать изображение в качестве модификатора кисти", PO, u"Usar uma imagem como modificador do Pincel", CN, u"将源图片用于刷子"));
    image.mode = BUTTON_TOGGLE;

    skin(&NoShadowSkin);

    parent += slope_window.create(Rect_RD(rect().ld() - Vec2(0.03f, 0), 0.22f, 0.13f), "Slope").barVisible(false).hide();
    slope_window.skin(skin());
    FlagDisable(slope_window.flag, WIN_MOVABLE);
    ...
    if (SupportedTouches())
        slope_window += slope.side_b.create(Rect_LU(slope.rect().lu(), 0.04f, 0.04f), ">").func(Slope::PushedSide, slope).focusable(false).desc("Change side of the slop limit");

    parent += image_window.create(Rect_RD(rect().ru() + Vec2(0, 0.03f), 0.3f, 0.415f), MLTC(u"Image", PL, u"Obraz", DE, u"Bild", RU, u"Изобращение", PO, u"Imagem", CN, u"图片")).barVisible(false).hide();
    image_window.skin(skin());
    FlagDisable(image_window.flag, WIN_MOVABLE);
    ...
    image_window += tangle.create(Vec2(0.07f, -0.32f), MLTC(u"Angle", PL, u"Kąt", DE, u"Winkel", RU, u"Угол", PO, u"Ângulo", CN, u"角度"));
    image_window += sangle.create(Rect_L(0.14f, tangle.rect().centerY(), 0.14f, 0.05f), 0).desc(MLTC(u"Image rotation angle\nChange with: Alt+RMB", PL, u"Kąt obrotu obrazka\nZmień przy pomocy: Alt+RMB", DE, u"Bild Rotationswinkel\nÄndern mit: Alt + RMB", RU, u"Угол вращения изображения\nЧтобы изменить угол вращения нажмите: Alt+RMB", PO, u"Ângulo de rotação da Imagem\nMuda com: Alt+Botão Direito do Rato", CN, u"图片旋转:Alt键+鼠标右键"));
    image_window += pattern.create(Rect_C(0.07f, -0.38f, 0.11f, 0.038f), MLTC(u"Pattern", PL, u"Wzór", DE, u"Muster", RU, u"Шаблон", PO, u"Padrão", CN, u"样式"));
    image_window += cpattern.create(Rect_L(0.14f, pattern.rect().centerY(), 0.14f, 0.05f), PatternText, Elms(PatternText)).set(0).desc(MLTC(u"Pattern Scale", PL, u"Skala wzoru", DE, u"Muster Skalierung", RU, u"Масштаб шаблона", PO, u"Escala Padrão", CN, u"模板样式缩放"));
    pattern.mode = BUTTON_TOGGLE;
    pattern.desc(MLTC(u"Apply image using a repeating pattern", PL, u"Nałóż obrazek jako powtarzający się wzorzec"));
...
Window &BrushClass::hide() {
    super::hide();
    image_window.hide();
    slope_window.hide();
    return T;
}
Window &BrushClass::show() {
    super::show();
    if (image())
        image_window.show();
    if (slope_b())
    ...
    if (gpc.visible && visible()) {
        ...
        if (Ms.wheel() && (Gui.wheel() == Gui.desktop() || !Gui.wheel() || hasMsWheelFocus())) {
            if (Kb.ctrlCmd() && Kb.shift())
                ssoft.set(ssoft() + Ms.wheel() / 8.0f);
            else if (Kb.ctrlCmd())
                ssize.set(ssize() + Ms.wheel() / 12.0f);
            else if (Kb.shift())
                sspeed.set(sspeed() + Ms.wheel() / 8.0f);
        }
        if (alt_rotate && Kb.alt() && Ms.b(1)) {
            sangle.set(Frac(sangle() + Ms.d().sum() * 0.5f));
            Ms.freeze();
        }
        if (Gui.window() == &image_window)
            REPA(MT)
        if (MT.bp(i) && MT.guiObj(i) == &image_image)
            image_io.activate();
