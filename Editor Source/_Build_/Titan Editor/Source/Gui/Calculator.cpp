...
CalculatorClass &CalculatorClass::create(C Rect &rect) {
    ...
    T += expression.create(Rect(0.01f, -clientHeight() + 0.01f, clientWidth() - 0.01f, -clientHeight() + 0.01f + 0.05f));
...
void CalculatorClass::update(C GuiPC &gpc) {
    ...
    if (Gui.kb() == &expression) {
        CalcValue value;
        if (Kb.kf(KB_ENTER) && Calculate(value, expression())) {
            expression.set(value.asText(-9));
            expression.cursor(expression().length());
        }
    }
}
void CalculatorClass::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        ...
        CalcValue value;
        Str error;
        bool ok = Calculate(value, expression(), &error);
        ...
        rect.min.y += expression.rect().max.y + clientHeight();
        rect.extendX(-0.01f);
        rect += gpc.offset;

        if (!ok) {
            tsb.align.y = 0;
            D.text(tsb, rect.center(), error.is() ? error : "Error");
        } else {
            ...
            if (value.type == CVAL_INT || value.type == CVAL_REAL) {
                ...
                flt f;
                dbl d;
                if (value.type == CVAL_INT) {
                    d = value.i;
                    f = value.i;
                    i = value.i;
                } else if (value.type == CVAL_REAL) {
                    d = value.r;
                    f = value.r;
                    i = Round(value.r);
                }
                ...
                uint *x = (uint *)&d;
                D.text(ts, rect.min.x + 0.07f, rect.min.y + 0.05f, TextHex(x[1], 8, 0, true) + TextHex(x[0], 8));
            }
        }
    }
}
