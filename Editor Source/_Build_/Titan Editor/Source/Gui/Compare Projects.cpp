...
void CompareProjects::SelectSel(CompareProjects &cp) {
    Memt<UID> ids;
    FREPA(cp.list.sel)
    if (ElmListElm *elml = cp.list.absToData(cp.list.sel[i]))
        ids.add(elml->id);
    Proj.elmSelect(ids);
}
...
CompareProjects &CompareProjects::del() {
    release();
    super::del();
    return T;
}
CompareProjects &CompareProjects::hide() {
    release();
    super::hide();
    return T;
}
...
void CompareProjects::create() {
    ...
    Rect r = super::create("Project Comparison");
    prop_min_y = r.min.y;
    autoData(this);
    ...
    T += region.create();
    region += list.create(lc, Elms(lc)).elmHeight(0.038f).textSize(0, 1);
    list.flag |= LIST_RESIZABLE_COLUMNS | LIST_MULTI_SEL;
...
bool CompareProjects::DiffElm(C Project &proj_a, C Elm &elm_a, C Project &proj_b, C Elm &elm_b) {
    if (elm_a.newerData(elm_b) || elm_a.newerFile(elm_b) || elm_b.newerData(elm_a) || elm_b.newerFile(elm_a))
        return true;
    if (!FEqual(proj_a.editPath(elm_a), proj_b.editPath(elm_b)) || !FEqual(proj_a.gamePath(elm_a), proj_b.gamePath(elm_b)))
        return true;
    return false;
}
            ...
            FREPAD(ea, proj_a.elms) // start from the beginning to minimize 'binaryInclude' time
            {
                ...
                if (pb != pa)           // don't compare the same project
                {
                    ...
                    if (elm_b && elm_b->finalExists()) {
                        if (test_data && (elm_a.data ? elm_a.data->ver.ver : 0) != (elm_b->data ? elm_b->data->ver.ver : 0) && DiffElm(proj_a, elm_a, proj_b, *elm_b) || test_name && elm_a.name_time != elm_b->name_time && !Equal(elm_a.name, elm_b->name, true) || test_parent && elm_a.parent_time != elm_b->parent_time && elm_a.parent_id != elm_b->parent_id || test_publish && (elm_a.publish() != elm_b->publish() || elm_a.publishMobile() != elm_b->publishMobile()))
...
void CompareProjects::changed(C UID &proj_id) // project with 'proj_id' has changed
{
    if (visible()) {
        ...
        REPA(projects) {
            ...
            if (proj.id == proj_id || proj.id == Proj.id) // refresh 'proj_id' and also current 'Proj', since this is called when copying elements from 'Proj' to 'proj_id', 'Proj' also could have changed in the meantime
            {
                refresh = true;
                if (proj_id == Proj.id)
                    proj = Proj;
...
void CompareProjects::compare(C MemPtr<UID> &proj_ids) {
    ...
    {
        C Projects::Elm *proj = Projs.findProj(proj_ids[i]);
        if (!proj) {
            Gui.msgBox(S, S + "Project \"" + proj_ids[i].asFileName() + "\" was not found");
            return;
        }
        if (!proj->path.is()) {
            Gui.msgBox(S, S + "Project \"" + proj->name + "\" has no path");
            return;
        }
        Project temp;
        int ver;
        Str error;
        LOAD_RESULT result = temp.load3(proj->path, ver, error);
        if (LoadOK(result)) {
            ...
            switch (result) {
            case LOAD_NEWER:
                Gui.msgBox(S, S + "Project \"" + proj->name + "\" requires newer engine version.");
                return;
            case LOAD_ERROR:
                Gui.msgBox(S, S + "Project \"" + proj->name + "\" failed to load." + (error.is() ? "\n" : null) + error);
                return;
            case LOAD_LOCKED:
                Gui.msgBox(S, S + "Project \"" + proj->name + "\" is locked.");
                return;
            }
    }
    refresh();
...
void CompareProjects::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        ...
        if (Ms.bd(0) && Gui.ms() == &list || Kb.k(KB_ENTER) && Gui.kb() == &list)
            if (ElmListElm *list_elm = list())
                if (Elm *elm = Proj.findElm(list_elm->id)) {
                    if (!ElmVisible(elm->type))
                        elm = Proj.firstVisibleParent(elm);
                    if (elm) {
                        if (elm->type == ELM_FOLDER)
                            Proj.elmOpenParents(elm->id);
                        else
                            Proj.elmToggle(elm->id);        // call this first because it may open parents
