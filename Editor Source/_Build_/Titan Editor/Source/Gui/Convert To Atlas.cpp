...
void ConvertToAtlasClass::Mtrl::setScale() {
    scaled_size = Round(original_size * scale * ConvertToAtlas.scale);
    setScaleText(scaled_size);
}
void ConvertToAtlasClass::Mtrl::setScale(C Vec2 &scale) {
    T.scale = scale;
    setScale();
}
    ...
    if (gpc.visible && visible()) {
        ...
        if (ConvertToAtlas.tex_size.all()) {
            ...
            REPA(ConvertToAtlas.mtrls) {
                ...
                Rect mr = mtrl.packed_rect;
                mr /= Vec2(ConvertToAtlas.tex_size);
            ...
            D.alpha(alpha);
        } else if (ConvertToAtlas.mtrls.elms()) {
...
void ConvertToAtlasClass::Warning::create() {
    Gui += super::create(Rect_C(0, 0, 1.7f, 1), "Convert To Atlas").hide();
    button[2].show();
...
void ConvertToAtlasClass::Scale(Mtrl &mtrl, C Str &text) {
    mtrl.setScale(TextVec2Ex(text));
    ConvertToAtlas.refresh();
}
Str ConvertToAtlasClass::Scale(C Mtrl &mtrl) { return TextVec2Ex(mtrl.scale); }
void ConvertToAtlasClass::Del(Mtrl &mtrl) {
    ConvertToAtlas.mtrls.removeData(&mtrl, true);
    ConvertToAtlas.refresh();
}
void ConvertToAtlasClass::Refresh(C Property &prop) { ConvertToAtlas.refresh(); }
void ConvertToAtlasClass::ChangedScale(C Property &prop) {
    REPAO(ConvertToAtlas.mtrls).setScale();
    ConvertToAtlas.refresh();
}
...
void ConvertToAtlasClass::setErrors(Mesh &mesh, C Memc<UID> &mtrl_ids, C UID &mesh_id) {
    ...
    REPD(l, mesh.lods()) {
        ...
        REPA(lod) {
            ...
            if (base.vtx.tex0())
                REP(4) {
                    UID mtrl_id = part.multiMaterial(i).id();
                    if (mtrl_id.valid() && mtrl_ids.binaryHas(mtrl_id)) {
                        base.explodeVtxs().fixTexOffset();
                        flt error = 0;
                        REPA(base.vtx)
                        MAX(error, Dist(base.vtx.tex0(i), tex_rect)); // verify that all tex uv's are within allowed tex rect
                        if (error > 0.01f)                            // allow some tolerance
                        {
                            Error *e = null;
                            REPA(errors) {
                                Error &err = errors[i];
                                if (err.mesh_id == mesh_id && err.mtrl_id == err.mtrl_id) {
                                    e = &err;
                                    break;
                                }
                            }
...
bool ConvertToAtlasClass::Create(int &occurence, C UID &id, ptr user) {
    occurence = 0;
    return true;
}
void ConvertToAtlasClass::convertMeshes(C UID &atlas_id) {
    MaterialPtr atlas_mtrl = Proj.gamePath(atlas_id);
    Memc<UID> mtrl_ids, obj_ids;
    REPA(mtrls)
    mtrl_ids.binaryInclude(mtrls[i].id);
    REPA(Proj.elms) // iterate all elements
    {
        C Elm &obj = Proj.elms[i];
        if (C ElmObj *obj_data = obj.objData()) // if that's an object
            if (C Elm *mesh = Proj.findElm(obj_data->mesh_id))
                if (C ElmMesh *mesh_data = mesh->meshData()) {
                    REPA(mesh_data->mtrl_ids)
                    if (mtrl_ids.binaryHas(mesh_data->mtrl_ids[i])) // if that mesh has one of the materials
                    {
    ...
    Memc<UID> duplicated;
    Proj.duplicate(obj_ids, duplicated, (mode == NEW) ? " (Atlas)" : " (Atlas Backup)");
    //       obj_ids duplicated
    // NEW              atlas
    // REPLACE         backup
    if (mode == NEW)
        Swap(duplicated, obj_ids);
    Memc<UID> &atlased = obj_ids, &non_atlased = duplicated;

    REPA(atlased)
    if (Elm *obj = Proj.findElm(atlased[i]))
        if (ElmObj *obj_data = obj->objData())
            if (Elm *mesh_elm = Proj.findElm(obj_data->mesh_id, ELM_MESH))
                if (ElmMesh *mesh_data = mesh_elm->meshData()) {
                    if (ObjEdit.mesh_elm == mesh_elm)
                        ObjEdit.flushMeshSkel();
                    Mesh mesh;
                    if (Load(mesh, Proj.editPath(mesh_elm->id), Proj.game_path)) {
                        REPD(l, mesh.lods()) {
                            MeshLod &lod = mesh.lod(l);
                            REPA(lod) {
                                MeshPart &part = lod.parts[i];
                                ...
                                MaterialPtr mtrls[4];
                                REPA(mtrls) {
                                    MaterialPtr &mtrl_ptr = mtrls[i];
                                    if (mtrl_ptr = part.multiMaterial(i)) {
                                        UID mtrl_id = mtrl_ptr.id();
                                        REPA(T.mtrls)
                                        if (T.mtrls[i].id == mtrl_id) {
                                            mtrl = &T.mtrls[i];
                                            mtrl_ptr = atlas_mtrl;
                                            break;
                                        }
    ...
    int existing_atlased = 0;
    C UID *obj_id = null;
    REPA(atlased)
    if (C Elm *elm = Proj.findElm(atlased[i]))
        if (elm->finalExists()) {
            obj_id = &elm->id;
            existing_atlased++;
        }
    if (existing_atlased == 1) // if there's only one object created, then put the atlas inside of it
        if (Elm *atlas = Proj.findElm(atlas_id)) {
            atlas->setParent(*obj_id, time);
            Server.setElmParent(*atlas);
        }

    // process objects
    if (non_atlased.elms() == atlased.elms()) {
        REPA(non_atlased)
        if (Elm *elm = Proj.findElm(non_atlased[i])) {
            if (mode == REPLACE_NO_PUBLISH || mode == REPLACE_REMOVE) {
                elm->setParent(atlased[i], time);
                Server.setElmParent(*elm);
            } // move 'non_atlased' to 'atlased'
            if (mode == REPLACE_NO_PUBLISH)
                elm->setPublish(false, time);
            else if (mode == REPLACE_REMOVE)
                elm->setRemoved(true, time);
        }
        if (mode == REPLACE_NO_PUBLISH)
            Server.publishElms(non_atlased, false, -1, time);
        else if (mode == REPLACE_REMOVE)
            Server.removeElms(non_atlased, true, time);
        ...
        {
            REPA(mtrl_ids)
            if (Elm *mtrl = Proj.findElm(mtrl_ids[i])) {
                int index;
                if (atlased.binarySearch(mtrl->parent_id, index)) // if is located in one of 'atlased'
                {
                    mtrl->setParent(non_atlased[index], time);
                    Server.setElmParent(*mtrl);
                }
            }
        }
    }
    ...
    // process materials
    REPA(mtrl_ids)
    if (Elm *elm = Proj.findElm(mtrl_ids[i])) {
        if (mode == REPLACE_NO_PUBLISH)
            elm->setPublish(false, time);
        else if (mode == REPLACE_REMOVE)
            elm->setRemoved(true, time);
    ...
    if (mode == REPLACE_NO_PUBLISH)
        Server.publishElms(mtrl_ids, false, -1, time);
    else if (mode == REPLACE_REMOVE)
        Server.removeElms(mtrl_ids, true, time);

    if (mode == REPLACE_NO_PUBLISH || mode == REPLACE_REMOVE)
        REPA(Proj.elms) // move material sub elements to atlas
        {
            Elm &sub = Proj.elms[i];
            if (mtrl_ids.binaryHas(sub.parent_id)) {
                sub.setParent(atlas_id, time);
                Server.setElmParent(sub);
            }
        }
}
                                 ...
                                 flt dest_mul, flt dest_add)     // transformation of the dest   map
{
    ...
    if (src.is() || force) {
        ...
        if (Equal(dest_mul, 0)) {
            ...
            mul = src_mul / dest_mul;
            add = (src_add - dest_add) / dest_mul;
        }
        ...
        if (fps_src.elms()) // have source map
        {
            ...
            if (mtrl.edit.smooth_is_rough && tex_flag == TEXF_SMOOTH)
                fp.params.New().set("inverseRGB");
            if (mtrl.edit.flip_normal_y && tex_flag == TEXF_NORMAL)
                fp.params.New().set("inverseG"); // !! this needs to be done before 'swapRG' !!
            if (mtrl.rotated) {
                fp.params.New().set("swapXY");
                if (tex_flag == TEXF_NORMAL)
                    fp.params.New().set("swapRG");
            } // !! this needs to be done before 'resizeClamp' !!
            ...
            if (need_mul || need_add) // have to transform
            {
                ...
                {
                    if (need_mul)
                        fp.params.New().set("scale", TextVecEx(mul));
                } else {
                    if (need_mul && need_add)
                        fp.params.New().set("mulAddRGB", TextVecVecEx(mul, add));
                    else if (need_mul)
                        fp.params.New().set("mulRGB", TextVecEx(mul));
                    else if (need_add)
                        fp.params.New().set("addRGB", TextVecEx(add));
                }
            }
...
void ConvertToAtlasClass::convertPerform() {
    ...
    if (mtrls.elms()) {
        ...
        REPA(mtrls) {
            ...
            if (Elm *elm = Proj.findElm(mtrl.id))
                (*parent_occurence(elm->parent_id))++; // calculate which parent is most frequently used for stored materials
            tex_used |= mtrl.edit.texturesUsed();
            MIN(min_reflect, mtrl.edit.reflect_min);
        }
        int occurences = 0;
        UID parent_id = UIDZero;
        REPA(parent_occurence)
        if (parent_occurence[i] > occurences) {
            occurences = parent_occurence[i];
            parent_id = parent_occurence.key(i);
        ...
        flt alpha = 0;
        int alpha_num = 0;
        MATERIAL_TECHNIQUE tech = MTECH_OPAQUE; // parameters for alpha materials
        ...
        FREPA(mtrls) {
            ...
            atlas.rough_mul += mtrl.edit.roughMul();
            atlas.rough_add += mtrl.edit.roughAdd();
            reflect_min += mtrl.edit.reflect_min;
            reflect_max += mtrl.edit.reflect_max;
            atlas.glow += mtrl.edit.glow;
            atlas.emissive += mtrl.edit.emissive_s;
            atlas.emissive_glow += mtrl.edit.emissive_glow;

            REPA(atlas.tex_downsize)
            MIN(atlas.tex_downsize[i], mtrl.edit.tex_downsize[i]); // minimize downsize (maximize size)

            if (mtrl.edit.tech) {
                alpha += mtrl.edit.color_s.w;
                alpha_num++;
                tech = mtrl.edit.tech;
            }
            ...
            if (mtrl.edit.normal_map.is()) {
                Image temp;
                TextParam resize;
                if (Proj.loadImages(temp, &resize, mtrl.edit.normal_map))
                    if (temp.typeChannels() <= 1 || temp.monochromaticRG()) {
            ...
            if (AddMap(forced, atlas.color_map, mtrl.edit.color_map, FlagOn(tex_used, TEXF_COLOR), mtrl, TEXF_COLOR, mtrl.edit.color_s.xyz)) {
                tex_mtrl |= TEXF_COLOR;
                if (forced)
                    tex_force_size |= TEXF_COLOR;
            }
            if (AddMap(forced, atlas.alpha_map, mtrl.edit.alpha_map, FlagOn(tex_used, TEXF_ALPHA), mtrl, TEXF_ALPHA)) {
                tex_mtrl |= TEXF_ALPHA;
                if (forced)
                    tex_force_size |= TEXF_ALPHA;
            }
            if (AddMap(forced, atlas.bump_map, mtrl.edit.bump_map, FlagOn(tex_used, TEXF_BUMP), mtrl, TEXF_BUMP)) {
                tex_mtrl |= TEXF_BUMP;
                if (forced)
                    tex_force_size |= TEXF_BUMP;
            }
            if (AddMap(forced, atlas.normal_map, mtrl.edit.normal_map, FlagOn(tex_used, TEXF_NORMAL), mtrl, TEXF_NORMAL, mtrl.edit.normal)) {
                tex_mtrl |= TEXF_NORMAL;
                if (forced)
                    tex_force_size |= TEXF_NORMAL;
            }
            if (AddMap(forced, atlas.smooth_map, mtrl.edit.smooth_map, FlagOn(tex_used, TEXF_SMOOTH), mtrl, TEXF_SMOOTH, mtrl.edit.smoothMul(), mtrl.edit.smoothAdd())) {
                tex_mtrl |= TEXF_SMOOTH;
                if (forced)
                    tex_force_size |= TEXF_SMOOTH;
            }
            if (AddMap(forced, atlas.metal_map, mtrl.edit.metal_map, FlagOn(tex_used, TEXF_METAL), mtrl, TEXF_METAL, params.reflect_mul, params.reflect_add, 1 - min_reflect, min_reflect)) {
                tex_mtrl |= TEXF_METAL;
                if (forced)
                    tex_force_size |= TEXF_METAL;
            } // 1-min_reflect because we will operate on min_reflect..1 range, so mul=1-min_reflect, add=min_reflect
            if (AddMap(forced, atlas.glow_map, mtrl.edit.glow_map, FlagOn(tex_used, TEXF_GLOW), mtrl, TEXF_GLOW, mtrl.edit.glow)) {
                tex_mtrl |= TEXF_GLOW;
                if (forced)
                    tex_force_size |= TEXF_GLOW;
            }
            if (AddMap(forced, atlas.emissive_map, mtrl.edit.emissive_map, FlagOn(tex_used, TEXF_EMISSIVE), mtrl, TEXF_EMISSIVE, mtrl.edit.emissive_s)) {
                tex_mtrl |= TEXF_EMISSIVE;
                if (forced)
                    tex_force_size |= TEXF_EMISSIVE;
            }
        ...
        if (tex_wrote & TEXF_COLOR) {
            atlas.color.xyz = 1;
        } else
            atlas.color.xyz /= mtrls.elms(); // if we ended up having color  map, then it means we've used the baked textures, for which we need to set the full color    multiplier
        if (tex_wrote & (TEXF_COLOR | TEXF_ALPHA)) {
            atlas.color.w = (alpha_num ? alpha / alpha_num : 1);
            atlas.technique = tech;
        } else
            atlas.color.w /= mtrls.elms(); // if we ended up having alpha  map, then set parameters from alpha materials only (check color map too because alpha can come from it)
        if (tex_wrote & TEXF_BUMP) {
        }
        atlas.bump /= mtrls.elms();
        if (tex_wrote & TEXF_NORMAL) {
            atlas.normal = 1;
        } else
            atlas.normal /= mtrls.elms(); // if we ended up having normal map, then it means we've used the baked textures, for which we need to set the full normal   multiplier
        if (tex_wrote & TEXF_SMOOTH) {
            atlas.rough_mul = 1;
            atlas.rough_add = 0;
        } else {
            atlas.rough_mul /= mtrls.elms();
            atlas.rough_add /= mtrls.elms();
        } // if we ended up having smooth map, then it means we've used the baked textures, for which we need to set the full smooth   multiplier
        if (tex_wrote & TEXF_METAL) {
            atlas.reflect(min_reflect);
        } else
            atlas.reflect(reflect_min / mtrls.elms(), reflect_max / mtrls.elms()); // if we ended up having metal  map, then it means we've used the baked textures, for which we need to set the full reflect  multiplier
        if (tex_wrote & TEXF_GLOW) {
            atlas.glow = 1;
        } else
            atlas.glow /= mtrls.elms(); // if we ended up having glow   map, then it means we've used the baked textures, for which we need to set the full glow     multiplier
        if (tex_wrote & TEXF_EMISSIVE) {
            atlas.emissive = 1;
        } else
            atlas.emissive /= mtrls.elms(); // if we ended up having light  map, then it means we've used the baked textures, for which we need to set the full emissive multiplier
        ...
        IMAGE_TYPE ct;
        ImageProps(atlas.base_0, &atlas.base_0_id, &ct, MTRL_BASE_0, edit.tex_quality);
        if (Importer.includeTex(atlas.base_0_id))
            atlas.base_0.copy(atlas.base_0, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
        ImageProps(atlas.base_1, &atlas.base_1_id, &ct, MTRL_BASE_1);
        if (Importer.includeTex(atlas.base_1_id))
            atlas.base_1.copy(atlas.base_1, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
        ImageProps(atlas.base_2, &atlas.base_2_id, &ct, MTRL_BASE_2);
        if (Importer.includeTex(atlas.base_2_id))
            atlas.base_2.copy(atlas.base_2, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
        ImageProps(atlas.emissive_img, &atlas.emissive_id, &ct, MTRL_EMISSIVE);
        if (Importer.includeTex(atlas.emissive_id)) {
            SetFullAlpha(atlas.emissive_img, ct);
            atlas.emissive_img.copy(atlas.emissive_img, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
        }
...
void ConvertToAtlasClass::convertDo() {
    ...
    {
        Elm &elm = Proj.elms[i];
        if (ElmMesh *mesh_data = elm.meshData()) // if that's a mesh
        {
            REPA(mesh_data->mtrl_ids)
            if (mtrl_ids.binaryHas(mesh_data->mtrl_ids[i])) // if that mesh has one of the materials
            {
                ...
                if (ObjEdit.mesh_elm == &elm)
                    mesh.create(ObjEdit.mesh);
                else
                    Load(mesh, Proj.editPath(elm.id), Proj.game_path); // load edit so we can have access to MeshBase
...
void ConvertToAtlasClass::create() {
    ...
    Rect r = super::create("Convert To Atlas");
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
    mode.combobox.resize(Vec2(0.63f, 0));
    autoData(this);
...
void ConvertToAtlasClass::autoStretch() {
    if (mtrls.elms()) {
        RectI r = mtrls.last().packed_rect;
        REPA(mtrls)
        r |= mtrls[i].packed_rect;
        if (r.size().all()) {
    ...
    REPA(mtrls) {
        ...
        REPAD(j, mtrls)
        if (j != i) {
            C RectI &t = mtrls[j].packed_rect;
            if (Cuts(r, t))
                r.min.x = t.max.x + 1;
        }
        r.max.x = tex_size.x - 1;
        REPAD(j, mtrls)
        if (j != i) {
            C RectI &t = mtrls[j].packed_rect;
            if (Cuts(r, t))
...
void ConvertToAtlasClass::refresh() {
    ...
    Memc<RectSizeAnchor> mtrl_sizes;
    mtrl_sizes.setNum(mtrls.elms());
    REPAO(mtrl_sizes).size = mtrls[i].scaled_size;
    Memc<RectI> mtrl_rects;
    if (PackRectsUnknownLimit(mtrl_sizes, mtrl_rects, tex_size, allow_rotate, 0, false, true, ConvertToAtlas.force_square)) {
        REPA(mtrls) {
            Mtrl &mtrl = mtrls[i];
    ...
    REPA(mtrls) {
        Mtrl &mtrl = mtrls[i];
        mtrl.setScaleText(mtrl.packed_rect.size());
    }
    warning.hide();
}
void ConvertToAtlasClass::addElms(C MemPtr<UID> &elm_ids) {
    REPA(elm_ids)
    if (Elm *elm = Proj.findElm(elm_ids[i], ELM_MTRL)) {
        REPA(mtrls)
        if (mtrls[i].id == elm->id)
            goto skip;
        {
            Mtrl &mtrl = mtrls.New();
            mtrl.id = elm->id;
...
ConvertToAtlasClass &ConvertToAtlasClass::hide() {
    super::hide();
    mtrls.clear();
    warning.hide(); // release memory, since we're releasing then we also need to hide the warning to prevent from converting on empty materials
    return T;
}
