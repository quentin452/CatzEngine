...
void ConvertToDeAtlasClass::Preview::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        ...
        if (ImagePtr img = Proj.texPath(ConvertToDeAtlas.base_0_tex)) {
            ...
            img->draw(r);
            Rect sr = ConvertToDeAtlas.source_rect;
            sr /= Vec2(ConvertToDeAtlas.tex_size);
            sr.min.y = 1 - sr.min.y;
            sr.max.y = 1 - sr.max.y;
...
void ConvertToDeAtlasClass::convertMeshes(Memc<IDReplace> &mtrl_replace, C Rect *frac) {
    ...
    Memc<UID> duplicated;
    Proj.duplicate(objs, duplicated, (mode == NEW) ? " (De-Atlas)" : " (De-Atlas Backup)");
    //       objs  duplicated
    // NEW              atlas
    // REPLACE         backup
    if (mode == NEW)
        Swap(duplicated, objs);
    Memc<UID> &deatlased = objs, &non_deatlased = duplicated;

    REPA(deatlased)
    if (Elm *obj = Proj.findElm(deatlased[i]))
                    ...
                    REPD(l, mesh.lods()) {
                        ...
                        REPA(lod) {
                            ...
                            REPA(mtrls) {
                                ...
                                if (C IDReplace *id = ReplaceID(mtrl.id(), mtrl_replace)) {
                                    changed_multi_mtrl = true;
                                    mtrl = Proj.gamePath(id->to);
                                }
                            }
                            ...
                            REP(part.variations())
                            if (i)
                                if (C IDReplace *id = ReplaceID(part.variation(i).id(), mtrl_replace)) {
                                    changed_part = true;
                                    part.variation(i, Proj.gamePath(id->to));
                                }
                            if (changed_part) {
                                MeshBase &base = part.base;
                                if (frac && base.vtx.tex0()) {
                                    base.fixTexOffset();
                                    REPA(base.vtx) {
    ...
    if (non_deatlased.elms() == deatlased.elms()) {
        REPA(non_deatlased)
        if (Elm *elm = Proj.findElm(non_deatlased[i])) {
            if (mode == REPLACE_NO_PUBLISH || mode == REPLACE_REMOVE) {
                elm->setParent(deatlased[i], time);
                Server.setElmParent(*elm);
            } // move 'non_deatlased' to 'deatlased'
        ...
        if (mode == REPLACE_NO_PUBLISH)
            Server.publishElms(non_deatlased, false, -1, time);
        else if (mode == REPLACE_REMOVE)
            Server.removeElms(non_deatlased, true, time);
    ...
    if (deatlased.elms() == 1) // if created only one object, then move all materials to it
        REPA(mtrl_replace)
    if (Elm *mtrl = Proj.findElm(mtrl_replace[i].to)) {
        mtrl->setParent(deatlased[0], time);
    ...
    if (mode == REPLACE_NO_PUBLISH || mode == REPLACE_REMOVE) // check if we can remove/unpublish old materials
    {
        ...
        if (mode == REPLACE_REMOVE)
            Proj.getUsedMaterials(used_mtrls_exist, false); // get list of materials used by existing    elements
        REPA(mtrls) {
            C UID &mtrl_id = mtrls[i];
            if (mode == REPLACE_REMOVE && !used_mtrls_exist.binaryHas(mtrl_id))
                remove.add(mtrl_id);
...
Str ConvertToDeAtlasClass::Process(C Str &name, C Rect *crop, C VecI2 *resize) {
    ...
    {
        Mems<FileParams> files = FileParams::Decode(name);
        if (files.elms()) {
            TextParam src_resize;
            ExtractResize(files, src_resize); // first remove source resize
            if (files.elms() > 1)
                files.New(); // if there's more than 1 file, then add crop/resize parameters globally
            if (crop)        // crop first
            {
                Image image;
                if (Proj.loadImages(image, null, FileParams::Encode(files))) // load image at current state to extract its size
                {
            ...
            if (resize)
                files.last().params.New().set("resizeClamp", TextVecI2Ex(*resize)); // then resize
...
void ConvertToDeAtlasClass::convertDo() {
    ...
    if (mtrls.elms()) {
        Proj.clearListSel();
        REPA(mtrls)
        MtrlEdit.flush(mtrls[i]); // flush first

        // load first material and adjust its textures
        EditMaterial first;
        first.load(Proj.editPath(mtrls[0]));
        ...
        Proj.createBaseTextures(base_0, base_1, base_2, first);
        IMAGE_TYPE ct;
        ImageProps(base_0, &first.base_0_tex, &ct, MTRL_BASE_0, first.tex_quality);
        if (Proj.includeTex(first.base_0_tex)) {
            base_0.copy(base_0, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
            Proj.saveTex(base_0, first.base_0_tex);
        }
        Server.setTex(first.base_0_tex);
        ImageProps(base_1, &first.base_1_tex, &ct, MTRL_BASE_1);
        if (Proj.includeTex(first.base_1_tex)) {
            base_1.copy(base_1, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
            Proj.saveTex(base_1, first.base_1_tex);
        }
        Server.setTex(first.base_1_tex);
        ImageProps(base_2, &first.base_2_tex, &ct, MTRL_BASE_2);
        if (Proj.includeTex(first.base_2_tex)) {
            base_2.copy(base_2, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
            Proj.saveTex(base_2, first.base_2_tex);
        }
        Server.setTex(first.base_2_tex);
        ...
        time.getUTC();
        REPA(mtrls)
        if (Elm *elm_mtrl = Proj.findElm(mtrls[i])) {
            Elm &de_atlas = Proj.Project::newElm();
            de_atlas.type = ELM_MTRL;
            de_atlas.copyParams(*elm_mtrl).setName(elm_mtrl->name + " (De-Atlas)").setRemoved(false); // call 'setName' after 'copyParams'

            EditMaterial edit;
            edit.load(Proj.editPath(elm_mtrl->id));
            edit.color_map = first.color_map;
            edit.color_map_time = time;
            edit.alpha_map = first.alpha_map;
            edit.alpha_map_time = time;
            edit.bump_map = first.bump_map;
            edit.bump_map_time = time;
            edit.normal_map = first.normal_map;
            edit.normal_map_time = time;
            edit.smooth_map = first.smooth_map;
            edit.smooth_map_time = time;
            edit.metal_map = first.metal_map;
            edit.metal_map_time = time;
            edit.glow_map = first.glow_map;
            edit.glow_map_time = time;
            edit.base_0_tex = first.base_0_tex;
            edit.base_1_tex = first.base_1_tex;
            edit.base_2_tex = first.base_2_tex;
            if (ElmMaterial *mtrl_data = de_atlas.mtrlData())
                mtrl_data->from(edit);
...
void ConvertToDeAtlasClass::create() {
    ...
    w = &add();
    h = &add();
    Rect r = super::create("Extract from Atlas", Vec2(0.02f, -0.02f), 0.036f, 0.043f, 0.2f);
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
    mode.combobox.resize(Vec2(0.73f, 0));
    autoData(this);
...
void ConvertToDeAtlasClass::setElms(C MemPtr<UID> &elm_ids) {
        ...
        if (ElmObj *obj_data = elm->objData())
            if (Elm *mesh_elm = Proj.findElm(obj_data->mesh_id))
                if (ElmMesh *mesh_data = mesh_elm->meshData()) {
                    ...
                    if (Elm *elm_mtrl = Proj.findElm(mesh_data->mtrl_ids[i]))
                        if (ElmMaterial *mtrl_data = elm_mtrl->mtrlData())
                            if (mtrl_data->base_0_tex.valid() || mtrl_data->base_1_tex.valid() || mtrl_data->base_2_tex.valid()) {
                                if (!base_0_tex.valid() && !base_1_tex.valid() && !base_2_tex.valid()) {
                                    EditMaterial mtrl;
                                    Proj.mtrlGet(elm_mtrl->id, mtrl);
                                    mtrl_color_map = mtrl.color_map;
                                    base_0_tex = mtrl_data->base_0_tex;
                                    base_1_tex = mtrl_data->base_1_tex;
                                    base_2_tex = mtrl_data->base_2_tex;
                                }
                                if (mtrl_data->base_0_tex == base_0_tex && mtrl_data->base_1_tex == base_1_tex && mtrl_data->base_2_tex == base_2_tex) {
    ...
    if (!objs.elms())
        Gui.msgBox(S, "No Objects/Materials to process");
    else {
        // get 'tex_size'
        tex_size = 0;
        Image temp;
        TextParam temp_resize;
        if (Proj.loadImages(temp, &temp_resize, mtrl_color_map))
            tex_size = temp.size(); // first try loading from source image (because it may not be pow2) without any resize, we want exact size, can ignore sRGB
        if (!tex_size.all())
            if (ImagePtr base_0 = Proj.texPath(base_0_tex))
                tex_size = base_0->size(); // if failed to load then use base tex size
        t_tex_size.set(TexSize(tex_size));

        bool used_tex = false;
        Rect used_tex_rect(0, 0);
        REPA(objs)
        if (Elm *elm = Proj.findElm(objs[i]))
            if (ElmObj *obj_data = elm->objData())
                if (Elm *mesh_elm = Proj.findElm(obj_data->mesh_id))
                    if (ElmMesh *mesh_data = mesh_elm->meshData()) {
                        Mesh mesh;
                        if (ObjEdit.mesh_elm == mesh_elm)
                            ...
                            Load(mesh, Proj.editPath(mesh_elm->id), Proj.game_path); // load edit so we can have access to MeshBase
                        REPD(l, mesh.lods()) {
                            ...
                            REPA(lod) {
                                ...
                                if (base.vtx.tex0())
                                    REP(4) {
                                        ...
                                        if (mtrl_id.valid() && mtrls.binaryHas(mtrl_id)) {
                                            REPA(base.tri) {
        ...
        if (used_tex)
            source_rect.set(Floor(used_tex_rect.min * (Vec2)tex_size), Ceil(used_tex_rect.max * (Vec2)tex_size));
        else
            source_rect.zero();
        toGui();
...
void ConvertToDeAtlasClass::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        ...
        if (sw)
            sw->display(S + "Source Width: " + source_rect.w());
        if (sh)
            sh->display(S + "Source Height: " + source_rect.h());
        if (w)
            w->display(S + "Width: " + size.x);
        if (h)
            h->display(S + "Height: " + size.y);
    }
}
