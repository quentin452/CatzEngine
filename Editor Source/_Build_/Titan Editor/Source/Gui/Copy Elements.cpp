...
bool CopyElmsFunc(Thread &thread) {
    ...
    FREPA(CopyElms.texs_to_copy) {
        ...
        if (src.texs.binaryHas(id) && !dest.texs.binaryHas(id)) // if source has and dest doesn't
        {
            ...
            bool ok = false;
            if (pak.totalFiles()) {
                if (C PakFile *pf = pak.find(sp))
                    ok = FCopy(pak, *pf, dp, FILE_OVERWRITE_ALWAYS, null, suffix);
            } else {
                ok = FCopy(sp, dp, FILE_OVERWRITE_ALWAYS, null, null, suffix);
            }
            if (ok) {
                dest.texs.binaryInclude(id); // include in dest on success
                if (src.texs_update_base1.binaryHas(id))
                    dest.texs_update_base1.binaryInclude(id);
                if (src.texs_remove_srgb.binaryHas(id))
                    dest.texs_remove_srgb.binaryInclude(id);
            } else
                goto error;
        }
            ...
            if (!d || d->type == s->type) // don't copy if are of different types
            {
                ...
                if (s->type == ELM_WORLD) // World
                {
                    ...
                    if (pak.totalFiles()) {
                        ...
                        File f;
                        if (f.read(src.worldVerPath(id), pak))
                            if (world_ver_temp.load(f))
                                src_world_ver = &world_ver_temp;
                    } else {
                        if (FExist(sep) && !FCopyDir(sep, dep + suffix)) {
                            FDelDirs(dep + suffix);
                            goto error;
                        } // copy to temp folder
                    ...
                    FDelDirs(dep);
                    FRename(dep + suffix, dep); // replace target folder with temp
                    FDelDirs(dgp);
                    FRename(dgp + suffix, dgp); // replace target folder with temp
                    if (src_world_ver)
                        if (WorldVer *dest_world_ver = dest.worldVerRequire(id))
                            *dest_world_ver = *src_world_ver;
                } else if (s->type == ELM_MINI_MAP) // Mini Map
                {
                ...
                } else if (s->type == ELM_CODE && CopyElms.src_ver >= 49) // Code (stored separately only since version 49)
                {
                    ...
                    if (pak.totalFiles()) {
                        if (C PakFile *pf = pak.find(scp)) {
                            if (FCopy(pak, *pf, dcp, FILE_OVERWRITE_ALWAYS, null, suffix))
                                SavedCode(dcp);
                            else
                                goto error;
                        } else
                        ...
                        if (!FExist(scp))
                            FDelFile(dcp);
                        else if (FCopy(scp, dcp, FILE_OVERWRITE_ALWAYS, null, null, suffix))
                            SavedCode(dcp);
                        else
                            goto error;
                    }
                ...
                } else // regular file
                {
                    ...
                    if (pak.totalFiles()) {
                        ...
                        if (C PakFile *pf = pak.find(sep)) {
                            ...
                            FDelFile(dep);
                        if (C PakFile *pf = pak.find(sgp)) {
                            if (FCopy(pak, *pf, dgp, FILE_OVERWRITE_ALWAYS, null, suffix))
                                SavedGame(s->type, dgp);
                            else
                                goto error;
                        } else
                            FDelFile(dgp);
                        ...
                        if (!FExist(sgp))
                            FDelFile(dgp);
                        else if (FCopy(sgp, dgp, FILE_OVERWRITE_ALWAYS, null, null, suffix))
                            SavedGame(s->type, dgp);
                        else
                            goto error;
                    }
                ...
                if (d) {
                    ...
                    if (s->type == ELM_MESH)        // check if we're changing body
                    {
                        ...
                        if (s_body != d_body)
                            CopyElms.changed_body.add(id);
                    }
                } else
                    d = &dest.getElm(id); // create new element
                *d = *s;
                if (!d->parent_id.valid() && d->parent_id != CopyElms.root)
                    d->setParent(CopyElms.root);
...
void ShutCopyElms() {
    ...
    if (CopyElms.dest) {
        if (Elm *elm = CopyElms.dest->findElm(CopyElms.root))
            elm->opened(true);
        ...
        if (Elm *elm = CopyElms.dest->findElm(CopyElms.elms_replaced[i]))
            switch (elm->type) {
            case ELM_MESH:
                CopyElms.dest->meshTransformChanged(*elm, CopyElms.changed_body.binaryHas(elm->id));
                break; // need to call 'meshTransformChanged' in case mesh transform was changed and need to setup transform for skeletons/anims/phys/bodies, or if we need to rebuild world embedded objects
            case ELM_SKEL:
                CopyElms.dest->skelTransformChanged(elm->id);
                break; // need to call 'skelTransformChanged' because 'meshTransformChanged' will not trigger this, since 'setSkelTransform' calls this only when it detects a transform change, however transform was already adjusted during copying
            case ELM_MTRL:
                CopyElms.dest->mtrlSetAutoTanBin(elm->id);
                break;
            }
    }
...
bool UpdateCopyElms() {
    if (Kb.bp(KB_ESC)) {
        SetProjectState();
        Gui.msgBox(S, "Copying elements breaked on user request");
    }
...
/******************************************************************************/
CopyElements::ProjListElm::ProjListElm(C Projects::Elm &src) : id(UIDZero) {
    name = src.local_name;
    path = src.path;
    id = src.id;
}
CopyElements::ProjListElm::ProjListElm(C Project &src) : id(UIDZero) {
    name = src.name;
    path = src.path;
    id = src.id;
}
CopyElements::ReplaceElms::ElmListElm::ElmListElm(C UID &elm_id) : id(UIDZero) {
    T.id = elm_id;
    T.name = CopyElms.src->elmFullName(elm_id);
}
...
void CopyElements::ReplaceElms::copyDo() {
    ...
    if (CopyElms.canCopy()) {
        LOAD_RESULT result;
        Str error;
        if (CopyElms.target.id == Proj.id) {
            CopyElms.dest = &Proj;
            result = LOAD_OK;
        } else {
            CopyElms.dest = &CopyElms.temp_dest;
            result = CopyElms.dest->open(CopyElms.target.id, CopyElms.target.name, CopyElms.target.path, error);
            CopyElms.root.zero();
        } // can't use root because we're not copying to 'Proj'

        if (result == LOAD_NEWER)
            Gui.msgBox(S, "Target project was created with a newer version of " ENGINE_NAME " Engine.\nPlease upgrade your " ENGINE_NAME " software and try again.");
        else if (result == LOAD_LOCKED)
            Gui.msgBox(S, "Target project appears to be already opened in another instance of the Editor.\nIf it isn't, then please try re-opening it manually first.");
        else if (result == LOAD_ERROR)
            Gui.msgBox(S, S + "Target project failed to load." + (error.is() ? "\n" : null) + error);
        else if (CopyElms.dest->needUpdate())
            Gui.msgBox(S, "Target project needs to be updated first.\nPlease first open the target project normally in order to update it.");
        else {
            if (result == LOAD_EMPTY)
                CopyElms.dest->initSettings(*CopyElms.src); // if dest project didn't exist yet, then copy all settings from source
            ...
            if (!list.sel.binaryHas(i))                   // if i-th element is not selected
                if (ElmListElm *data = list.absToData(i)) // get data of that element
                {
                    ...
                    REPA(data->children)
                    CopyElms.elms_to_copy.binaryExclude(data->children[i]); // don't copy its  children
                }
...
void CopyElements::ReplaceElms::check() {
    ...
    if (CopyElms.target.id == Proj.id) {
        ...
        int ver;
        CopyElms.dest = &CopyElms.temp_dest;
        result = CopyElms.dest->load3(CopyElms.target.path, ver, error);
    } // just load data without opening it for editing (so we don't have to close it after this stage yet)
    ...
    if (result == LOAD_NEWER)
        ...
        REPA(CopyElms.elms_to_copy) {
            ...
            if (!src || (src && dest && src->type != dest->type))
        ...
        // setup list
        data.sort(ElmListElm::Compare);
        list.setData(data);
        list.sel.clear();
        FREPA(data)
        list.sel.add(i); // select all elements by default

        if (data.elms())
            activate(); // if there are elements to overwrite then display the window
        else
            copyDo(); // if all elements are new then proceed with copy
...
bool CopyElements::canCopy() C {
    if (StateActive == &StateProject || StateActive == &StateProjectList)
        return true;
    Gui.msgBox(S, "Copying Elements can't be performed in this Editor State");
    return false;
}
void CopyElements::create() {
    Gui += super::create(Rect_C(0, 0, 1.8f, 1.1f), "Copy elements").hide();
    ...
    elms_region += elms_list.create(elms_lc, Elms(elms_lc), true);
    elms_list.cur_mode = LCM_ALWAYS;
    proj_region += proj_list.create(proj_lc, Elms(proj_lc), true);
    proj_list.cur_mode = LCM_ALWAYS;
    proj_list.sort_column[0] = 0;
    T += include_children.create(Rect_LD(0.04f, -clientHeight() + 0.04f, 0.38f, 0.06f), "Include children").func(Refresh, T).focusable(false).desc("If include elements which are assigned as children to selected elements.");
    include_children.mode = BUTTON_TOGGLE;
    include_children.set(true, QUIET);
    T += include_dependencies.create(Rect_LU(include_children.rect().ru() + Vec2(0.03f, 0), 0.48f, 0.06f), "Include dependencies").func(Refresh, T).focusable(false).desc("If include elements on which selected elements depend on.\nFor example if a mesh is selected, then enabling this option will select all materials that the mesh uses.");
    include_dependencies.mode = BUTTON_TOGGLE;
    include_dependencies.set(true, QUIET);
    T += include_parents.create(Rect_LU(include_dependencies.rect().ru() + Vec2(0.03f, 0), 0.48f, 0.06f), "Include parents").func(Refresh, T).focusable(false).desc("If include elements which are assigned as parents of selected elements.");
    include_parents.mode = BUTTON_TOGGLE;
    T += copy.create(Rect_RD(clientWidth() - 0.04f, -clientHeight() + 0.04f, 0.25f, 0.06f), "Copy").func(Copy, T);
    replace_elms.create();
}
...
void CopyElements::includeTex(C UID &tex_id) {
    if (tex_id.valid())
        texs_to_copy.binaryInclude(tex_id);
}
void CopyElements::includeDep(C UID &elm_id) {
    if (elm_id.valid())
        if (processed_dep.binaryInclude(elm_id))
            if (Elm *elm = src->findElm(elm_id))
                switch (elm->type) {
                case ELM_APP:
                    if (ElmApp *data = elm->appData()) {
                        if (include_dependencies()) {
                            includeDep(data->icon);
                            includeDep(data->image_portrait);
                            includeDep(data->image_landscape);
                        }
                    }
                    break;
                ...
                case ELM_MESH:
                    if (ElmMesh *data = elm->meshData()) {
                        ...
                        if (include_dependencies()) {
                            includeDep(Proj.meshToObj(data->body_id));
                            includeDep(data->draw_group_id);
                            FREPA(data->mtrl_ids)
                            includeDep(data->mtrl_ids[i]);
                        }
                    }
                    break;
                ...
                case ELM_ANIM:
                    if (ElmAnim *data = elm->animData()) {
                        if (include_dependencies())
                            includeDep(data->skel_id);
                    }
                    break;

                case ELM_MTRL:
                    if (ElmMaterial *data = elm->mtrlData()) {
                        includeTex(data->base_0_tex);
                        includeTex(data->base_1_tex);
                        includeTex(data->base_2_tex);
                        includeTex(data->detail_tex);
                        includeTex(data->macro_tex);
                        includeTex(data->emissive_tex); // always include
                    }
                    break;
                ...
                case ELM_WATER_MTRL:
                    if (ElmWaterMtrl *data = elm->waterMtrlData()) {
                        includeTex(data->base_0_tex);
                        includeTex(data->base_1_tex);
                        includeTex(data->base_2_tex); // always include
                    }
                    break;
                ...
                case ELM_IMAGE_ATLAS:
                    if (ElmImageAtlas *data = elm->imageAtlasData()) {
                        if (include_dependencies())
                            REPA(data->images)
                        if (!data->images[i].removed)
                            includeDep(data->images[i].id);
                    }
                    break;
                ...
                case ELM_ICON:
                    if (ElmIcon *data = elm->iconData()) {
                        if (include_dependencies()) {
                            includeDep(data->icon_settings_id);
                            includeDep(data->obj_id);
                        }
                    }
                    break;
                ...
                case ELM_ENV:
                    if (ElmEnv *data = elm->envData()) {
                        if (include_dependencies()) {
                            REPA(data->cloud_id)
                            includeDep(data->cloud_id[i]);
                            includeDep(data->sun_id);
                            includeDep(data->skybox_id);
                            includeDep(data->star_id);
                        }
                    }
                    break;
                        ...
                        if (include_dependencies()) {
                            ...
                            if (WorldVer *world_ver = src->worldVerGet(elm->id)) {
                                ...
                                REPA(world_ver->obj) {
                                    ...
                                    if (!obj_ver.removed())
                                        includeDep(obj_ver.elm_obj_id);
                                }

                                // copy all materials from all existing waters
                                REPA(world_ver->lakes) {
                                    WaterVer &water_ver = world_ver->lakes.lockedData(i);
                                    if (!water_ver.removed()) {
                                        Lake lake;
                                        if (lake.load(src->editLakePath(elm->id, world_ver->lakes.lockedKey(i))))
                                            includeDep(lake.material);
                                    }
                                }
                                REPA(world_ver->rivers) {
                                    WaterVer &water_ver = world_ver->rivers.lockedData(i);
                                    if (!water_ver.removed()) {
                                        River river;
                                        if (river.load(src->editRiverPath(elm->id, world_ver->rivers.lockedKey(i))))
                                            includeDep(river.material);
                                    }
                                }
                            }
                        }
...
{                                            // don't include children because this function is already called for all elements
    ...
    case GO_REGION: {
        ...
        includeDep(obj.asRegion().slidebar[i]);
    } break;

    case GO_SLIDEBAR: {
        includeDep(obj.asSlideBar().skin().id());
        REPA(obj.asSlideBar().button)
        includeDep(obj.asSlideBar().button[i]);
    } break;
    ...
    case GO_WINDOW: {
        ...
        includeDep(obj.asWindow().button[i]);
    } break;
    }
}
void CopyElements::refresh() {
    ...
    if (include_parents()) {
        ...
        for (; elms.elms();) {
            ...
            if (Elm *elm = Proj.findElm(elm_id))
                if (Proj.findElm(elm->parent_id))
                    if (elms_to_copy.binaryInclude(elm->parent_id))
                        elms.add(elm->parent_id);
        }
    }
    ...
    processed_dep.clear();
    FREPA(elms_to_copy)
    includeDep(elms_to_copy[i]);
    Swap(processed_dep, elms_to_copy);

    elms_data.clear();
    FREPA(elms_to_copy)
    if (Elm *elm = src->findElm(elms_to_copy[i]))
        if (ElmVisible(elm->type))
            elms_data.add(src->elmFullName(elm));
    elms_data.sort(ComparePathNumberCI);
...
void CopyElements::display(C Str &project_package_file) {
    ...
    if (canCopy()) {
        src_ver = -1;
        root.zero();
        if (ListElm *elm = Proj.list.litToListElm())
            if (elm->elm)
                root = elm->elm->id;
        if (project_package_file.is()) {
            ...
            case PAK_LOAD_OK:
                break;
            case PAK_LOAD_UNSUPPORTED_VERSION:
                Gui.msgBox(S, "Selected project was created with a newer version of the engine.");
                return;
            case PAK_LOAD_INCOMPLETE_DATA:
                Gui.msgBox(S, S + "Selected project file is incomplete:\nFile size is: " + SizeBytes(actual_size) + "\nIt should be: " + SizeBytes(expected_size) + "\nIf downloaded from Esenthel Store, please make sure that the download was not interrupted.\nConsider downloading using " ENGINE_NAME " Editor which supports resuming interrupted downloads.");
                return;
            default:
                Gui.msgBox(S, "Can't load selected project");
                return;
            }

            src = &temp_src;
            File f;
            f.read("Data", project_package_pak);
            switch (src->load(f, src_ver)) // call 'load' always, and not only when 'read' succeeded, since we can't update the "ProjectPackage" file, we will update the destination project once it finished copying
            {
            case LOAD_EMPTY:
            case LOAD_ERROR:
            case LOAD_LOCKED:
                Gui.msgBox(S, "Invalid project file");
                return;
            case LOAD_NEWER:
                Gui.msgBox(S, "This project was created with a newer version of " ENGINE_NAME " Engine.\nPlease upgrade your " ENGINE_NAME " software and try again.");
                return;
            }
            if (src_ver <= 34 && f.read("Settings", project_package_pak))
                src->loadOldSettings(f); // ver 34 and below had settings in a separate file
            ...
            elms_to_copy.clear();
            FREPA(src->elms)
            elms_to_copy.binaryInclude(src->elms[i].id);
            texs_to_copy.clear();
            FREPA(src->texs)
            texs_to_copy.binaryInclude(src->texs[i]);
            ...
            int cur = -1;
            proj_data.clear();
            FREPA(Projs.proj_data) {
                Projects::Elm &src = Projs.proj_data[i];
                if (src.id != Proj.id) {
                    if (src.id == target.id)
                        cur = proj_data.elms();
                    proj_data.add(src);
                }
            } // add all except currently opened project, if there's a project to which we've copied last time, then select it
            proj_list.setData(proj_data);
            proj_list.setCur(proj_list.absToVis(cur)); // !! do not merge into one instruction !!
...
void CopyElements::copyDo() {
    if (!elms_to_copy.elms())
        Gui.msgBox(S, "No elements to copy");
    else if (ProjListElm *dest = proj_list()) {
        if (src == &Proj)
            REPA(elms_to_copy)
        Proj.flushElm(elms_to_copy[i]);
        hide();
        target = *dest;
        replace_elms.check();
    } else
...
void CopyElements::drop(Memc<Str> &names, GuiObj *focus_obj, C Vec2 &screen_pos) {
    REPA(names)
    if (GetExt(names[i]) == ProjectPackageExt) {
        display(names[i]);
        names.remove(i, true);
        break;
    }
...
void CopyElements::close() {
    ...
    if (src && src != &Proj)
        src->del();
    src = null; // we shouldn't 'close' 'src' because it's either 'Proj' or an "ProjectPackage" file
}
