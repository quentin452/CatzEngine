...
void DetectSimilarTextures::Pair::set(C UID &a, C UID &b) {
    T.a = a;
    T.b = b;
}
void DetectSimilarTextures::ImageEx::create(byte mtrl_base, C Image &src) {
    ...
    REPA(mips)
    src.extractMipMap(mips[i], Decompress ? ImageTypeUncompressed(src.type()) : -1, i);
}
void DetectSimilarTextures::CompareImage(UID &a_id, UID &b_id, int thread_index) {
    if (ImageEx *a_image = DST.loaded_texs.find(a_id))
        if (ImageEx *b_image = DST.loaded_texs.find(b_id)) {
            if (DST.mtrl_base) {
                if (DST.mtrl_base - 1 != a_image->mtrl_base || DST.mtrl_base - 1 != b_image->mtrl_base)
                    goto compared;
            }
            ImageCompare ic;
            int mips = Min(a_image->mips.elms(), b_image->mips.elms());
            FREP(mips) {
                if (!ic.compare(a_image->mips[a_image->mips.elms() - 1 - i], b_image->mips[b_image->mips.elms() - 1 - i], DST.similar_dif, false))
                    goto compared; // start comparing from smallest mip-map, if failed to compare
                if (ic.skipped || DST.pause)
                    goto compared;                   // if comparison was skipped
...
int DetectSimilarTextures::ImageLoad(ImageHeader &header, C Str &name) {
    if (GetThreadID() == DST.io_thread_id) // process only on the 'IOThread'
    {
        ...
        int shrink = 0;
        for (uint max_size = header.size.max(); max_size > DST.MaxTexSize; max_size /= 2)
            shrink++;
        return shrink;
    }
    return 0;
...
bool DetectSimilarTextures::ioThread() {
    ...
    REPA(proj_texs) {
        ...
        if (!loaded_texs.find(tex_id)) // if not yet loaded
        {
            ...
            if (ok) {
                ImageEx tex;
                tex.create(tex_id.mtrl_base, img);
                ImageEx &loaded = *loaded_texs(tex_id);
                Swap(loaded, tex);
                C UID &loaded_id = *loaded_texs.dataToKey(&loaded);
                REPA(loaded_texs) {
                    C UID &tex_id = loaded_texs.lockedKey(i);
                    if (loaded_id != tex_id)
                        calls.New().set(ConstCast(loaded_id), CompareImage, ConstCast(tex_id)); // since we can pass only pointers, then use Tex ID pointer to 'loaded_texs.key' and not 'proj_texs'
                }
                threads.queue(calls);
                calls.clear();
            } else
                proj_texs.remove(i); // if failed to load, then remove so we can set progress correctly
        }
    }
    return false;
}
...
void DetectSimilarTextures::reset() {
    ...
    REPA(Proj.elms) {
        ...
        if (ElmMaterial *mtrl_data = elm.mtrlData()) {
            int texs_i;
            if (mtrl_data->base_1_tex.valid() && texs.binarySearch(mtrl_data->base_1_tex, texs_i))
                proj_texs[texs_i].mtrl_base = 1;
            if (mtrl_data->base_2_tex.valid() && texs.binarySearch(mtrl_data->base_2_tex, texs_i))
                proj_texs[texs_i].mtrl_base = 2;
        }
    }
    REPA(loaded_texs)
    if (!texs.binaryHas(loaded_texs.lockedKey(i)))
        loaded_texs.remove(i); // remove loaded textures if they're no longer present in the project
    ...
    REPA(loaded_texs) {
        C UID &tex_id = loaded_texs.lockedKey(i);
        REPD(j, i)
        calls.New().set(ConstCast(tex_id), CompareImage, ConstCast(loaded_texs.lockedKey(j)));
    }
    threads.queue(calls);
...
void DetectSimilarTextures::addSimilarAll(C UID &a, C UID &b) {
    ...
    FREPA(data) {
        ...
        if (!d.id.valid())
            ...
            if (d.id == a) {
                for (i++; InRange(i, data) && data[i].id.valid();)
                    i++;              // skip all children
...
Rect DetectSimilarTextures::sizeLimit() C {
...
void DetectSimilarTextures::CurChanged(DetectSimilarTextures &dst) {
    if (Data *data = dst.list())
        Proj.elmActivate(Proj.findElmByTexture(data->id));
}
void DetectSimilarTextures::create() {
...
#endif
    flt h = 0.043f;
    Rect params = super::create("Detect Similar Textures", Vec2(0.02f, -0.02f), 0.036f, h, 0.15f);
    ...
    region += list.create(lc, Elms(lc), true).elmHeight(0.038f).textSize(0, 1).curChanged(CurChanged, T);
...
void DetectSimilarTextures::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        progress.set(compared, UniquePairs(proj_texs.elms()));
        progress.visible(progress() < 1 && proj_texs.elms()); // !! do not merge into a single instruction !!
        if (similar_pair.elms()) {
            Memt<Pair> similar;
            {
                SyncLocker locker(similar_pair_lock);
                similar = T.similar_pair;
                T.similar_pair.clear();
            }
            FREPA(similar) {
#if 1
