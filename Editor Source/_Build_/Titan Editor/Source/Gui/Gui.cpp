...
void HideBig() {
    MtrlEdit.hideBig();
    WaterMtrlEdit.hideBig();
    Theater.hideDo();
}
void HideProjsAct(GuiObj &go) {
    go.hide();
    Projs.proj_list.kbSet();
} // hide and set keyboard focus to the Projects list
void HideProjAct(GuiObj &go) {
    go.hide();
    Proj.list.kbSet();
} // hide and set keyboard focus to the Project  view
...
void SetGuiSkin(UID id) {
    ...
    if (color.b > color.r * 1.2f)
        Paks.addTry("Gui/neon.pak");
    else if (color.lum() < 64)
        Paks.addTry("Gui/white.pak");
    Pak pak;
    if (pak.load("Gui/white.pak"))
        REPA(pak)
    if (ImagePtr image = ImagePtr().find(pak.fullName(i)))
        image->load(pak.fullName(i)); // reload images listed in the pak
    ...
    {
        if (Gui.skin)
            NoComboBoxImage = *Gui.skin;
        NoComboBoxImage.combobox.image = null;
    }
    {
        if (Gui.skin)
            NoShadowSkin = *Gui.skin;
        if (NoShadowSkin.window.normal_no_bar) {
            WindowNoShadow = *NoShadowSkin.window.normal_no_bar;
            WindowNoShadow.shadow_opacity = 0;
            WindowNoShadow.border_color = Gui.borderColor();
            NoShadowSkin.window.normal_no_bar = &WindowNoShadow;
        }
        if (NoShadowSkin.window.active_no_bar) {
            ActiveWindowNoShadow = *NoShadowSkin.window.active_no_bar;
            ActiveWindowNoShadow.shadow_opacity = 0;
            ActiveWindowNoShadow.border_color = Gui.borderColor();
            NoShadowSkin.window.active_no_bar = &ActiveWindowNoShadow;
        }
    }
    {
...
void InitGui() {
    ...
    if (C TextNode *video = Settings.findNode("Video"))
        if (C TextNode *p = video->findNode("GuiSkin"))
            skin_id = VidOpt.skinID(p->asText());
    SetGuiSkin(skin_id);
}
void ShutGui() {
}
...
void SetHmOp(OP_HM_MODE op) {
    if (OpHm != op) {
        ...
        if (op != OP_HM_HEIGHT)
            HeightBrush.mode.set(-1, QUIET);
        if (op != OP_HM_COLOR)
            ColorBrush.set(-1, QUIET);
...
void SetObjOp(OP_OBJ_MODE op) {
    if (OpObj != op) {
        ...
        if (op != OP_OBJ_NONE && // if want some obj mode, then disable MODE's that have specific action when clicking
            (ObjEdit.mode() == ObjView::SLOTS || ObjEdit.mode() == ObjView::RAGDOLL || ObjEdit.mode() == ObjView::PHYS || ObjEdit.mode() == ObjView::BONES || ObjEdit.mode() == ObjView::SKIN || ObjEdit.mode() == ObjView::REMOVE))
            ObjEdit.mode.set(-1);
    }
}
...
void Drag(Memc<UID> &elms, GuiObj *obj, C Vec2 &screen_pos) {
    ...
    if (obj && obj->type() == GO_TEXTLINE && elms.elms())
        REPA(PropEx::props) // first check if this is a PropEx
        {
            PropEx &prop = PropEx::props[i];
            if (&prop.textline == obj) {
                // find compatible element
                if (prop.elm_type)
                    REPA(elms)
                if (Elm *elm = Proj.findElm(elms[i]))
                    if (prop.compatible(elm->type)) {
...
TextWhite &TextWhite::reset() {
    super::reset(false);
    return T;
}
TextWhite &TextWhite::operator=(C TextStyle &ts) {
    SCAST(TextStyle, T) = ts;
    return T;
}
TextWhite::TextWhite() { texts.include(T); }
TextWhite::~TextWhite() { texts.exclude(T); }
void TextBlack::skinChanged() { resetColors(true); }
TextBlack &TextBlack::reset() {
    super::reset(true);
    return T;
}
TextBlack &TextBlack::operator=(C TextStyle &ts) {
    SCAST(TextStyle, T) = ts;
    return T;
}
...
void PropEx::setDesc() {
    Str d = _desc;
    if (elm_type)
        d.line() += S + "Drag and drop " + ElmTypeName[elm_type] + " element here\nCtrl+LeftClick to open it";
    super::desc(d);
}
PropEx &PropEx::desc(C Str &desc) {
    _desc = desc;
    setDesc();
    return T;
}
PropEx &PropEx::setEnum() {
    super::setEnum();
    return T;
}
PropEx &PropEx::setEnum(cchar8 *data[], int elms) {
    super::setEnum(data, elms);
    return T;
}
PropEx &PropEx::setEnum(cchar *data[], int elms) {
    super::setEnum(data, elms);
    return T;
}
PropEx &PropEx::elmType(ELM_TYPE elm_type) {
    T.elm_type = elm_type;
    setDesc();
    if (elm_type)
        button.create("C").func(Clear, T).desc(S + "Clear this " + ((elm_type == ELM_ANY) ? "element" : ElmTypeName[elm_type]));
    return T;
}
void PropEx::clear() { super::set(S); }
PropEx &PropEx::create(C Str &name, C MemberDesc &md) {
    super::create(name, md);
    return T;
}
...
PropExs &PropExs::toGui() {
    REPAO(props).toGui();
    return T;
}
PropExs &PropExs::toGui(cptr data) {
    REPAO(props).toGui(data);
    return T;
}
PropExs &PropExs::autoData(ptr data) {
    REPAO(props).autoData(data);
    return T;
}
ptr PropExs::autoData() C {
    FREPA(props)
    if (ptr data = props[i].autoData())
        return data;
    return null;
}
PropExs &PropExs::changed(void (*changed)(C Property &prop), void (*pre_changed)(C Property &prop)) {
    REPAO(props).changed(changed, pre_changed);
    return T;
}
Rect PropWin::create(C Str &name, C Vec2 &lu, C Vec2 &text_scale, flt property_height, flt value_width) {
    ts.reset().size = text_scale;
    ts.align.set(1, 0);
    ...
    T.rect(Rect_RD(D.w(), -D.h(), right + padd.x, -rect.min.y + padd.y));
    return rect;
}
PropWin &PropWin::autoData(ptr data) {
    super::autoData(data);
    return T;
}
ptr PropWin::autoData() C { return super::autoData(); }
PropWin &PropWin::hide() {
    super::hide();
    REPAO(props).close();
    return T;
}
void RenameElmClass::create() {
    Gui += super ::create(Rect_C(0, 0, 1, 0.14f), "Rename Element").level(1).hide();
...
void RenameElmClass::activate(Elm *elm) {
    if (elm)
        activate(elm->id, elm->name);
}
void RenameElmClass::update(C GuiPC &gpc) {
    ...
    if (Gui.window() == this) {
        if (Kb.k(KB_ENTER)) {
            Proj.renameElm(elm_id, textline());
            hide();
            Kb.eatKey();
            Proj.list.activate();
        } // set keyboard focus to the Project view
    }
}
void ReplaceNameClass::create() {
    flt y = -0.05f;
    Gui += super::create(Rect_C(0, 0, 1, 0.23f), "Replace Name Part").level(1).hide();
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
    T += t_from.create(Vec2(0.1f, y), "From");
    T += from.create(Rect(0.2f, y - 0.03f, clientWidth() - 0.01f, y + 0.03f));
    y -= 0.07f;
    T += t_to.create(Vec2(0.1f, y), "To");
    T += to.create(Rect(0.2f, y - 0.03f, clientWidth() - 0.01f, y + 0.03f));
    y -= 0.07f;
}
void ReplaceNameClass::activate(C MemPtr<UID> &elm_ids) {
    T.elm_ids = elm_ids;
    if (elm_ids.elms()) {
        from.selectAll().activate();
        super::activate();
    } else
        button[2].push();
}
void ReplaceNameClass::update(C GuiPC &gpc) {
    ...
    if (Gui.window() == this) {
        if (Kb.k(KB_ENTER)) {
            Proj.replaceName(elm_ids, from(), to());
            hide();
            Kb.eatKey();
            Proj.list.activate();
        } // set keyboard focus to the Project view
...
void RenameSlotClass::create() {
    Gui += super ::create(Rect_C(0, 0, 1, 0.14f), "Rename Slot").hide();
    button[2].show();
    T += textline.create(Rect(0, -clientHeight(), clientWidth(), 0).extend(-0.01f));
}
void RenameSlotClass::activate(int slot) {
    if (ObjEdit.mesh_skel && InRange(slot, ObjEdit.mesh_skel->slots)) {
        ...
        for (int i = 0; i < slot; i++)
            if (slot_name == ObjEdit.mesh_skel->slots[i].name)
                slot_index++;
...
void RenameSlotClass::update(C GuiPC &gpc) {
    ...
    if (Gui.window() == this) {
        if (Kb.k(KB_ENTER)) {
            ObjEdit.renameSlot(slot_index, slot_name, textline());
            hide();
            Kb.eatKey();
        }
    }
...
void RenameBoneClass::create() {
    Gui += super ::create(Rect_C(0, 0, 1, 0.14f), "Rename Bone").hide();
    button[2].show();
    T += textline.create(Rect(0, -clientHeight(), clientWidth(), 0).extend(-0.01f));
}
...
void RenameBoneClass::update(C GuiPC &gpc) {
    ...
    if (Gui.window() == this) {
        if (Kb.k(KB_ENTER)) {
            Kb.eatKey();
            if (ObjEdit.renameBone(bone_name, textline()))
                hide();
        }
    }
}
void RenameEventClass::Hide(RenameEventClass &re) {
    re.hide();
    AnimEdit.focus();
}
void RenameEventClass::create() {
    Gui += super ::create(Rect_C(0, 0, 1, 0.14f), "Rename Event").hide();
...
void RenameEventClass::activate(int event) {
    if (AnimEdit.anim && InRange(event, AnimEdit.anim->events)) {
        ...
        for (int i = 0; i < event; i++)
            if (event_name == AnimEdit.anim->events[i].name)
                event_index++;
...
void RenameEventClass::update(C GuiPC &gpc) {
    ...
    if (Gui.window() == this) {
        if (Kb.k(KB_ENTER)) {
            AnimEdit.renameEvent(event_index, event_name, textline());
            Kb.eatKey();
            Hide(T);
        }
    }
}
void NewWorldClass::OK(NewWorldClass &nw) {
    Proj.newWorld(nw.name(), nw.areaSize(), nw.heightmapRes(), nw.parent_id, true);
    HideProjAct(nw);
}
...
void NewWorldClass::create() {
    ...
    Gui += super::create(Rect_C(0, 0, 1.0f, 0.7f), MLTC(u"New World", PL, u"Nowy Świat", DE, u"Neue Welt", RU, u"Новый мир", PO, u"Novo Mundo")).hide();
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
...
void ModeTabs::TabEx::update(C GuiPC &gpc) {
    ...
    if (Mode.Gui_skin) {
        Swap(temp, Gui.skin);
        Gui.skin = Mode.Gui_skin;
    } // restore Editor's Gui.skin before updating this Tab
    super::update(gpc);
    if (temp)
        Swap(Gui.skin, temp);
}
void ModeTabs::TabEx::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        ...
        if (Mode.Gui_skin) {
            Swap(temp, Gui.skin);
            Gui.skin = Mode.Gui_skin;
        } // restore Editor's Gui.skin before drawing this Tab
        super::draw(gpc);
        if (temp)
            Swap(Gui.skin, temp);
    }
...
void ModeTabs::kbToggle() {
    if (Proj.list.contains(Gui.kb()) || !Proj.list.visibleOnActiveDesktop())
        kbSet();
    else
        Proj.list.kbSet();
}
...
void ModeTabs::setTabs() {
    if (this == nullptr)
        return;
    flt x = rect().min.x;
    if (Misc.visible() && Misc.pos == VecI2(0, 1)) {
        x += rect().h() * 0.1f;
        if (Misc.move_misc.visible())
            x += Misc.move_misc.rect().w();
    }
    bool visible = visibleTabs();
    int v = 0;
    FREPA(T)
    if (tab(i).visible(visible && available[i]).visible()) {
        tab(i).pos(Vec2(x, rect().max.y));
        v++;
        tab(i).desc(S + "Keyboard Shortcut: " + Kb.ctrlCmdName() + "+F" + v);
        x += tab(i).rect().w();
    }
    setCloseVisibility();
}
Tabs &ModeTabs::rect(C Rect &rect) {
    super::rect(rect);
    setTabs();
    return T;
}
...
void ModeTabs::resize() {
    flt l = -D.w(), r = D.w();
    if (Misc.visible() && Misc.pos == VecI2(0, 1))
        MAX(l, Misc.rect().max.x);
    else if (Proj.visible())
        MAX(l, Proj.rect().max.x);

    if (Misc.move_misc.visible() && Misc.pos == VecI2(1, 1))
        MIN(r, Misc.move_misc.rect().min.x);
    else if (Misc.visible() && Misc.pos == VecI2(1, 1))
        MIN(r, Misc.rect().min.x);
    if (MtrlEdit.visible())
        MIN(r, MtrlEdit.rect().min.x);
    if (WaterMtrlEdit.visible())
        MIN(r, WaterMtrlEdit.rect().min.x);
    ...
    if (InRange(MODE_TEX_DOWN, tabs()))
        tab(MODE_TEX_DOWN).resize(Vec2(0.2f, 0)); // make this tab wider
}
...
bool ModeTabs::visibleTabs() {
    int v = 0;
    REPA(T)
    v += available[i];
    return v > (ModeTabsAlwaysVisible ? 0 : 1);
}
Tab *ModeTabs::visibleTab(int t) {
    if (InRange(t, T))
        FREPA(T)
    if (tab(i).visible())
        if (!t--)
...
GuiObj &ModeTabs::show() {
    setCloseVisibility();
    return super::show();
}
GuiObj &ModeTabs::hide() {
    close.hide();
    return super::hide();
}
void ModeTabs::update(C GuiPC &gpc) {
    super::update(gpc);
    if (gpc.visible && visible() && Ms.bp(2))
        REPA(T)
    if (Gui.ms() == &tab(i)) // close Tab on middle mouse
    {
        closeTab(i, true);
        break;
    }
...
void ModeTabs::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        if (visibleTabs()) {
            D.clip(gpc.clip);
            (rect() + gpc.offset).draw(BackgroundColorLight());
        }
        bool gui = (T() == MODE_GUI);
        if (gui) {
            Gui_skin = Gui.skin;
            if (GuiSkinPtr app_skin = Proj.appGuiSkin())
                Swap(app_skin, Gui.skin);
        }
        super::draw(gpc);
        if (gui) {
            Swap(Gui.skin, Gui_skin);
            Gui_skin.clear();
        }
    }
}
        ...
        if (Elm *elm = Proj.findElm(elms[0])) {
            ...
            if (files.elms()) {
                if (elm->type == ELM_ANIM) {
                    if (set_start_frame())
                        files[0].getParam("startFrame").value = start_frame();
                    if (set_end_frame())
                        files[0].getParam("endFrame").value = end_frame();
                    if (set_speed())
                        files[0].getParam("speed").value = speed();
                    if (set_optimize())
                        files[0].getParam("optimize").value = optimize();
                    if (mirror())
                        files[0].getParam("mirror"); // uses 'TextBool1' so no need to specify =1
                    if (del_end_keys())
                        files[0].getParam("delEndKeys"); // uses 'TextBool1' so no need to specify =1
                }
    ...
    if (Elm *elm = Proj.findElm(elm_id))
        if (elm->type != ELM_FOLDER && elm->type != ELM_LIB && elm->type != ELM_APP) {
            ...
            case ELM_ANIM: {
                ...
                T += optimize;
                if (files.elms()) {
                    C TextParam *start_frame = files[0].findParam("startFrame");
                    set_start_frame.set(start_frame != null);
                    T.start_frame.set(start_frame ? start_frame->value : S);
                    files[0].params.removeData(start_frame);
                    C TextParam *end_frame = files[0].findParam("endFrame");
                    set_end_frame.set(end_frame != null);
                    T.end_frame.set(end_frame ? end_frame->value : S);
                    files[0].params.removeData(end_frame);
                    C TextParam *speed = files[0].findParam("speed");
                    set_speed.set(speed != null);
                    T.speed.set(speed ? speed->value : S + "1");
                    files[0].params.removeData(speed);
                    C TextParam *optimize = files[0].findParam("optimize");
                    set_optimize.set(optimize != null);
                    T.optimize.set(optimize ? optimize->value : S + "1");
                    files[0].params.removeData(optimize);
                    C TextParam *name = files[0].findParam("name");
                    T.name.set(name ? name->value : S);
                    files[0].params.removeData(name);
                    C TextParam *mirror = files[0].findParam("mirror");
                    T.mirror.set(mirror ? mirror->asBool1() : false);
                    files[0].params.removeData(mirror);
                    C TextParam *del_end_keys = files[0].findParam("delEndKeys");
                    T.del_end_keys.set(del_end_keys ? del_end_keys->asBool1() : false);
                    files[0].params.removeData(del_end_keys);
                }
            } break;

            case ELM_MTRL: {
                T += t_name;
                T += name.desc("This is the Material name inside the source file.\nIf source file has multiple Materials, then one will be selected that has this name.");
                if (files.elms()) {
                    C TextParam *name = files[0].findParam("name");
                    T.name.set(name ? name->value : S);
                    files[0].params.removeData(name);
...
void ReloadElmClass::create() {
    ...
    Gui += super::create("Reload Elements").hide();
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
    ...
    set_start_frame.create(Rect_L(0.50f, -0.264f, 0.27f, 0.0475f), "Set Start Frame").func(ToggleStart, T);
    set_start_frame.mode = BUTTON_TOGGLE;
    start_frame.create(Rect_LU(set_start_frame.rect().ru(), 0.15f, set_start_frame.rect().h())).hide();
    set_end_frame.create(Rect_L(0.98f, -0.264f, 0.25f, 0.0475f), "Set End Frame").func(ToggleEnd, T);
    set_end_frame.mode = BUTTON_TOGGLE;
    end_frame.create(Rect_LU(set_end_frame.rect().ru(), 0.15f, set_end_frame.rect().h())).hide();
    set_speed.create(Rect_L(0.50f, -0.328f, 0.27f, 0.0475f), "Set Speed").func(ToggleSpeed, T).desc(speed_t);
    set_speed.mode = BUTTON_TOGGLE;
    speed.create(Rect_LU(set_speed.rect().ru(), 0.15f, set_speed.rect().h())).hide().desc(speed_t);
    set_optimize.create(Rect_L(0.98f, -0.328f, 0.25f, 0.0475f), "Set Optimize").func(ToggleOptimize, T).desc(optimize_t);
    set_optimize.mode = BUTTON_TOGGLE;
    optimize.create(Rect_LU(set_optimize.rect().ru(), 0.15f, set_optimize.rect().h())).hide().desc(optimize_t);
    mirror.create(Rect_L(path.rect().min.x, -0.328f, 0.14f, 0.0475f), "Mirror");
    mirror.mode = BUTTON_TOGGLE;
    del_end_keys.create(Rect_LU(mirror.rect().ru() + Vec2(0.02f, 0), 0.25f, 0.0475f), "Del End Keys");
    del_end_keys.mode = BUTTON_TOGGLE;
...
::ImportCodeClass::Update &ImportCodeClass::Update::set(C UID &id) {
    T.id = id;
    return T;
}
...
void ImportCodeClass::Yes(ImportCodeClass &is) {
    is.apply();
    is.del();
}
Rect ImportCodeClass::sizeLimit() C {
    Rect size = super::sizeLimit();
    size.min.set(0.9f, 0.5f);
    return size;
}
...
void ImportCodeClass::import() {
    if (updates.elms()) {
        Gui += super ::create("Code Import");
        button[2].func(No, T).show();
        flag |= WIN_RESIZABLE;
...
void SetMtrlColorClass::create() {
    ...
    Rect r = super::create(S, Vec2(0.02f, -0.02f), 0.04f, 0.045f, 0.2f);
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
    hide();
...
void MulSoundVolumeClass::create() {
    ...
    Rect r = super::create("Multiply Sound Volume", Vec2(0.02f, -0.02f), 0.04f, 0.045f, 0.2f);
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
    hide();
    T += ok.create(Rect_U(clientWidth() / 2, r.min.y - 0.03f, 0.25f, 0.053f), "OK").func(OK, T);
    rect(Rect_C(0, size() + Vec2(0, 0.09f)));
...
void MulSoundVolumeClass::display(C MemPtr<UID> &elms) {
    ...
    activate();
    if (props.elms())
        props[0].textline.selectAll().activate();
}
...
void EraseRemovedElms::OK(EraseRemovedElms &ere) {
    ere.hide();
    Proj.eraseRemoved(false);
}
void EraseRemovedElms::Full(EraseRemovedElms &ere) {
    ere.hide();
    Proj.eraseRemoved(true);
}
...
void EraseRemovedElms::create() {
    Gui += super ::create(Rect_C(0, 0, 1.44f, 1.5f), "Erase Removed Elements").hide();
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
    T += text.create(Rect_C(clientWidth() / 2, -0.15f, clientWidth() - 0.05f, 0.1f), "Are you sure you wish to erase all removed elements from the project?\nWarning: This operation cannot be undone!\n\nThis will remove files only from the local computer - when connected to server it will redownload the elements.");
    text.auto_line = true;
    ...
    region += list.create(lc, Elms(lc), true).elmHeight(0.038f).textSize(0, 1);
...
EraseRemovedElms &EraseRemovedElms::show() {
    if (hidden()) {
        ...
        data.clear();
        FREPA(remove)
        if (::Elm *elm = Proj.findElm(remove[i]))
...
void EraseRemovedElms::update(C GuiPC &gpc) {
    ...
    if (Ms.bd(0) && Gui.ms() == &list)
        if (Elm *elm = list())
            Proj.elmToggle(elm->id);
}
void ProjectLocked::OK(ProjectLocked &pl) {
    pl.del();
    if (Projects::Elm *proj = Projs.findProj(pl.proj_id))
        Projs.open(*proj, true);
}
void ProjectLocked::create(C UID &proj_id) {
    T.proj_id = proj_id;
    Gui += super ::create(Rect_C(0, 0, 1.59f, 0.38f), "Project Already Opened");
    button[2].show();
...
void ElmProperties::Explore(ElmProperties &ep) {
    if (Elm *elm = Proj.findElm(ep.elm_id))
        Proj.explore(*elm);
}
void ElmProperties::create() {
    Gui += super::create(Rect_C(0, 0, 1.3f, 0.47f)).hide();
    button[2].func(HideProjAct, SCAST(GuiObj, T)).show();
    ts.reset().size = 0.046f;
    ts.align.set(1, 0);
    ts_small = ts;
    ts_small.size = 0.04f;
    ts_small.align.set(1, -1);
    T += t_id.create(Vec2(0.02f, -0.05f), "ID:", &ts);
    T += id.create(Vec2(0.23f, -0.03f), S, &ts_small);
    T += b_id.create(Rect_R(clientWidth() - 0.02f, -0.05f, 0.15f, 0.05f), "Copy").func(CopyID, T).desc(S + "Element ID can be accessed in additional ways:\n- Drag and drop the element to the codes to insert its ID at that location\n- " + Kb.ctrlCmdName() + "+RightClick the element to copy its ID to current location in codes\n- " + Kb.ctrlCmdName() + "+MiddleClick the element to copy its ID to system clipboard");
    T += t_file.create(Vec2(0.02f, -0.12f), "File ID:", &ts);
    T += file.create(Vec2(0.23f, -0.10f), S, &ts_small);
    T += b_file.create(Rect_R(clientWidth() - 0.02f, -0.12f, 0.15f, 0.05f), "Copy").func(CopyFile, T);
    T += t_size.create(Vec2(0.02f, -0.19f), "Size:", &ts);
    T += size.create(Vec2(0.23f, -0.17f), S, &ts_small);
    T += t_class.create(Vec2(0.02f, -0.26f), "C++ Class:", &ts);
    T += Class.create(Vec2(0.23f, -0.24f), S, &ts_small);
    T += t_src.create(Vec2(0.02f, -0.33f), "Source:", &ts);
    T += b_src.create(Rect_R(clientWidth() - 0.02f, -0.33f, 0.15f, 0.05f), "Explore").func(Explore, T).desc("Open folder containing the source file");
    T += src.create(Rect(0.23f, -0.38f, b_src.rect().min.x - 0.02f, -0.31f), S, &ts_small);
    src.auto_line = true;
}
void ElmProperties::activate(Elm &elm) {
    if (elm.type == ELM_APP)
        ...
        size.clear();
        if (FExist(Proj.gamePath(elm.id)))
            size.set(SizeBytes(FSize(Proj.gamePath(elm.id))));
        Class.set(ElmTypeClass[elm.type]);
        Mems<FileParams> files = FileParams::Decode(elm.srcFile());
        Str src_text;
        FREPA(files)
        src_text.line() += files[i].name; // ignore parameters
        src.set(src_text);
