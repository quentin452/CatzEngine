...
Elm *ImportFind(C Str &name, ELM_TYPE type) {
    if (C UID *id = ImportElms.find(SkipStartPath(name, ImportSrc)))
        if (Elm *elm = Proj.findElm(*id, type))
            return elm;
    return null;
}
UID ImportFindID(C Str &name, ELM_TYPE type) {
    if (Elm *elm = ImportFind(name, type))
        return elm->id;
    return UIDZero;
}
Str ImportFindGamePath(C Str &name, ELM_TYPE type) {
    if (Elm *elm = ImportFind(name, type))
        return Proj.gamePath(*elm);
    return S;
}
/******************************************************************************/
void Clean(GuiObj &obj) { // don't clean children because this function is already called for all elements
    switch (obj.type()) {
    case GO_BUTTON: {
        if (!FExist(obj.asButton().image.name()))
            obj.asButton().image = null;
        if (!FExist(obj.asButton().skin.name()))
            obj.asButton().skin = null;
    } break;

    case GO_CHECKBOX: {
        if (!FExist(obj.asCheckBox().skin.name()))
            obj.asCheckBox().skin = null;
    } break;

    case GO_COMBOBOX: {
        if (!FExist(obj.asComboBox().image.name()))
            obj.asComboBox().image = null;
        if (!FExist(obj.asComboBox().skin().name()))
            obj.asComboBox().skin(null, false);
    } break;

    case GO_IMAGE: {
        if (!FExist(obj.asImage().image.name()))
            obj.asImage().image = null;
    } break;

    case GO_MENU: {
        if (!FExist(obj.asMenu().skin().name()))
            obj.asMenu().skin(null, false);
    } break;

    case GO_PROGRESS: {
        if (!FExist(obj.asProgress().skin.name()))
            obj.asProgress().skin = null;
    } break;

    case GO_REGION: {
        Clean(obj.asRegion().view);
        REPA(obj.asRegion().slidebar)
        Clean(obj.asRegion().slidebar[i]);
        if (!FExist(obj.asRegion().skin().name()))
            obj.asRegion().skin(null, false);
    } break;

    case GO_SLIDEBAR: {
        REPA(obj.asSlideBar().button)
        Clean(obj.asSlideBar().button[i]);
        if (!FExist(obj.asSlideBar().skin().name()))
            obj.asSlideBar().skin(null);
    } break;

    case GO_SLIDER: {
        if (!FExist(obj.asSlider().skin.name()))
            obj.asSlider().skin = null;
    } break;

    case GO_TAB: {
        if (!FExist(obj.asTab().image.name()))
            obj.asTab().image = null;
        if (!FExist(obj.asTab().skin.name()))
            obj.asTab().skin = null;
    } break;

    case GO_TABS: {
        REPA(obj.asTabs())
        Clean(obj.asTabs().tab(i));
        if (!FExist(obj.asTabs().skin().name()))
            obj.asTabs().skin(null);
    } break;

    case GO_TEXT: {
        if (!FExist(obj.asText().skin.name()))
            obj.asText().skin = null;
        if (!FExist(obj.asText().text_style.name()))
            obj.asText().text_style = null;
    } break;

    case GO_TEXTLINE: {
        Clean(obj.asTextLine().reset);
        if (!FExist(obj.asTextLine().skin().name()))
            obj.asTextLine().skin(null, false);
    } break;

    case GO_WINDOW: {
        REPA(obj.asWindow().button)
        Clean(obj.asWindow().button[i]);
        if (!FExist(obj.asWindow().skin().name()))
            obj.asWindow().skin(null);
    } break;
    }
}
/******************************************************************************/
...
void Adjust(Mesh &mesh) {
    REP(mesh.lods()) {
        ...
        REPA(lod.parts) {
            ...
            REP(part.variations())
            if (i) {
                MaterialPtr m = part.variation(i);
                Adjust(m);
                part.variation(i, m);
            }
        }
...
void Adjust(Heightmap &heightmap) {
    REP(heightmap.materials())
    Adjust(ConstCast(heightmap.material(i)));
}
    ...
    REPA(obj) {
        ...
        if (p.type == PARAM_ENUM)
            if (p.enum_type = Enums(ImportFindGamePath(Enums.name(p.enum_type), ELM_ENUM))) // adjust Enum*
            {
                ...
                if (InRange(e, *p.enum_type))
                    p.value.id = (*p.enum_type)[e].id; // adjust enum ID
            }
    }
}
/******************************************************************************/
bool ImportFunc(Thread &thread) // 'ObjType' must be initialized because loading old format 'Object' assumes that it is set correctly
{
    ...
    FREPA(ImportFiles) {
        ...
        if (ImportFiles[i].type == FSTD_FILE) {
            C Str &file = ImportFiles[i].name;
            Str name = GetBaseNoExt(file), ext = GetExt(file), path = GetPath(file);
            if (ext == "enum") // enum
            {
                Enum game;
                if (game.load(file)) {
                    if (EqualPath(file, "enum/obj_type.enum")) {
                        ...
                        FREPA(game) {
                            ...
                            EditObject edit;
                            edit.setType(true, elm.id, Proj.edit_path).setAccess(true, OBJ_ACCESS_CUSTOM);
                            if (elm.name == "OBJ_LIGHT_POINT")
                                edit.setEditorType(EditObjType.elmID(EDIT_OBJ_LIGHT_POINT));
                            else if (elm.name == "OBJ_LIGHT_CONE")
                                edit.setEditorType(EditObjType.elmID(EDIT_OBJ_LIGHT_CONE));
                            else if (elm.name == "OBJ_PARTICLES")
                                edit.setEditorType(EditObjType.elmID(EDIT_OBJ_PARTICLES));
                            Save(edit, Proj.editPath(elm));
                            Object game;
                            edit.copyTo(game, Proj, false, null, null);
            ...
            } else if (ext == "font") // font
            {
                ...
                if (game.load(file)) {
                    ...
                    FCopy(file, Proj.gamePath(elm));
                    EditFont edit;
                    edit.german = game.hasChar(u'ä');
                    edit.french = game.hasChar(u'à');
                    edit.polish = game.hasChar(u'ą');
                    edit.russian = game.hasChar(u'л');
                    edit.chinese = game.hasChar(u'嗨');
                    edit.german_time.getUTC();
                    edit.french_time.getUTC();
                    edit.polish_time.getUTC();
                    edit.russian_time.getUTC();
                    edit.chinese_time.getUTC();
                    edit.size = game.height();
                    edit.font = name;
                    edit.size_time.getUTC();
                    edit.font_time.getUTC();
                    Save(edit, Proj.editPath(elm));
                    *ImportElms(file) = elm.id;
                }
            } else if (ext == "ogg" || ext == "wav") // sound
            {
                Elm &elm = Proj.Project::newElm(name, Proj.getPathID(path), ELM_SOUND);
                elm.soundData()->newData();
                FCopy(file, Proj.gamePath(elm));
                *ImportElms(file) = elm.id;
            } else if (ext == "ogm" || ext == "ogv" || ext == "theora" || ext == "webm") // video
            {
                Elm &elm = Proj.Project::newElm(name, Proj.getPathID(path), ELM_VIDEO);
                elm.videoData()->newData();
                FCopy(file, Proj.gamePath(elm));
                *ImportElms(file) = elm.id;
            } else if (ext == "mtrl") // material
            {
                Material game;
                if (game.load(file)) {
                    Elm &elm = Proj.Project::newElm(name, Proj.getPathID(path), ELM_MTRL);
                    XMaterialEx mtrl_ex(&Proj);
                    mtrl_ex.create(game);
                    EditMaterial edit;
                    edit.create(mtrl_ex); // create from material

                    // set textures
                    if (game.base_0)
                        ImageProps(*game.base_0, &edit.base_0_tex, null, MTRL_BASE_0);
                    else
                        edit.base_0_tex.zero();
                    if (game.base_1)
                        ImageProps(*game.base_1, &edit.base_1_tex, null, MTRL_BASE_1);
                    else
                        edit.base_1_tex.zero();
                    if (game.base_2)
                        ImageProps(*game.base_2, &edit.base_2_tex, null, MTRL_BASE_2);
                    else
                        edit.base_2_tex.zero();
                    if (game.detail_map)
                        ImageProps(*game.detail_map, &edit.detail_tex, null, MTRL_DETAIL);
                    else
                        edit.detail_tex.zero();
                    if (game.macro_map)
                        ImageProps(*game.macro_map, &edit.macro_tex, null, MTRL_MACRO);
                    else
                        edit.macro_tex.zero();
                    if (game.emissive_map)
                        ImageProps(*game.emissive_map, &edit.emissive_tex, null, MTRL_EMISSIVE);
                    else
                        edit.emissive_tex.zero();
                    if (edit.base_0_tex.valid())
                        if (Proj.includeTex(edit.base_0_tex))
                            game.base_0->save(Proj.texPath(edit.base_0_tex));
                    if (edit.base_1_tex.valid())
                        if (Proj.includeTex(edit.base_1_tex))
                            game.base_1->save(Proj.texPath(edit.base_1_tex));
                    if (edit.base_2_tex.valid())
                        if (Proj.includeTex(edit.base_2_tex))
                            game.base_2->save(Proj.texPath(edit.base_2_tex));
                    if (edit.detail_tex.valid())
                        if (Proj.includeTex(edit.detail_tex))
                            game.detail_map->save(Proj.texPath(edit.detail_tex));
                    if (edit.macro_tex.valid())
                        if (Proj.includeTex(edit.macro_tex))
                            game.macro_map->save(Proj.texPath(edit.macro_tex));
                    if (edit.emissive_tex.valid())
                        if (Proj.includeTex(edit.emissive_tex))
                            game.emissive_map->save(Proj.texPath(edit.emissive_tex));
                    ...
                    edit.normal_map = base1_path;
                    AddTransform(edit.normal_map, "channel", "xy");
                    edit.bump_map_time.getUTC();
                    edit.bump_map = base2_path;
                    AddTransform(edit.bump_map, "channel", "z");
                    edit.glow_map_time.getUTC();
                    edit.glow_map = base2_path;
                    AddTransform(edit.glow_map, "channel", "w");
                    edit.smooth_map_time.getUTC();
                    edit.smooth_map = base2_path;
                    AddTransform(edit.smooth_map, "channel", "y");
                    AddTransform(edit.smooth_map, "inverseRGB");
                    edit.metal_map_time.getUTC();
                    edit.metal_map = base2_path;
                    AddTransform(edit.metal_map, "channel", "x");
                    // #MaterialTextureLayoutDetail
                    edit.detail_map_time.getUTC();
                    edit.detail_color = detail_path;
                    AddTransform(edit.detail_color, "channel", "w");
                    edit.detail_smooth = detail_path;
                    AddTransform(edit.detail_smooth, "channel", "z");
                    AddTransform(edit.detail_smooth, "inverseRGB");
                    edit.detail_normal = detail_path;
                    AddTransform(edit.detail_normal, "channel", "xy");
                    edit.detail_bump.clear();
                    edit.macro_map_time.getUTC();
                    edit.macro_map = macro_path;
                    edit.emissive_map_time.getUTC();
                    edit.emissive_map = emissive_path;
            ...
            } else if (ext == "obj") // object (only create elements but don't set data)
            {
                Object game;
                if (game.load(file)) {
    ...
    FREPA(ImportFiles) {
        ...
        if (ImportFiles[i].type == FSTD_FILE) {
                ...
                if (!ImportMtrlImages.binaryHas(file, ImportComparePath)) // if not material texture
                    if (game.load(file))
                        if (game.is()) {
                            ...
                            if (game.typeInfo().compressed)
                                elm.imageData()->type = ElmImage::COMPRESSED;
                            else if (game.type() == IMAGE_A8)
                                elm.imageData()->type = ElmImage::ALPHA;
                            else
            ...
            } else if (ext == "txds") // text styles
            {
                TextStyle game;
                if (game.load(file)) {
                    Elm &elm = Proj.Project::newElm(name, Proj.getPathID(path), ELM_TEXT_STYLE);
                    EditTextStyle edit;
                    edit.create(game, ImportFindID(game.font().name(), ELM_FONT));
                    Save(edit, Proj.editPath(elm));
                    edit.copyTo(game, Proj);
                    Save(game, Proj.gamePath(elm));
            ...
            } else if (ext == "obj") // object
            {
                if (Elm *elm = ImportFind(file, ELM_OBJ)) {
                    ...
                    EditObjectPtr base;
                    if (Elm *base_elm = ImportFind(game.base().name(), ELM_OBJ))
                        base = Proj.editPath(*base_elm);
                    else if (type.valid())
                        base = Proj.editPath(type);
                    game.type(false);
                    type.zero(); // clear type as it is always taken from base (OBJ_CLASS)
                    if (game.access() == OBJ_ACCESS_CUSTOM)
                        game.access(false); // use access from base
                    else
                        base = null; // we're not using custom class, so disable base
                    EditObject edit;
                    edit.create(game, type, base);
                    Adjust(edit);
                    Save(edit, Proj.editPath(*elm));
                    UID *mesh_id = null, *phys_id = null;
                    Matrix matrix = game.matrixFinal();
                    Pose transform = matrix;

                    ImportObjMeshes.binaryInclude(Str(game.mesh().name()), ImportComparePath);
                    *ImportObjMatrix(elm->id) = ~matrix;

                    if (game.mesh() && game.mesh()->is() || game.phys() && game.phys()->is()) {
                        Elm &mesh_elm = Proj.Project::newElm(ElmNameMesh, elm->id, ELM_MESH);
                        mesh_id = &mesh_elm.id;
                        ...
                        Mesh edit_mesh, game_mesh;
                        if (game.mesh())
                            edit_mesh.create(*game.mesh());
                        ...
                        if (edit_mesh.skeleton()) {
                            Skeleton skel = *edit_mesh.skeleton();
                            EditSkeleton edit_skel;
                            edit_skel.create(skel);
                            skel.transform(matrix);
                            Elm &skel_elm = Proj.Project::newElm(ElmNameSkel, mesh_elm.id, ELM_SKEL);
                            mesh_elm.meshData()->skel_id = skel_elm.id;
                        ...
                        Skeleton *body_skel;
                        Proj.getMeshSkels(mesh_elm.meshData(), null, &body_skel);
                        ...
                        if (game.phys() && game.phys()->is()) {
                            PhysBody phys = *game.phys();
                            phys.transform(matrix);
                            Elm &phys_elm = Proj.Project::newElm(ElmNamePhys, mesh_elm.id, ELM_PHYS);
                            phys_id = &phys_elm.id;
                            mesh_elm.meshData()->phys_id = phys_elm.id;
                    ...
                    edit.copyTo(game, Proj, true, mesh_id, phys_id);
                    Save(game, Proj.gamePath(*elm));
                    elm->objData()->from(edit);
                    if (mesh_id)
                        elm->objData()->mesh_id = *mesh_id;
                }
            }
        }
    }
    ...
    FREPA(ImportFiles) {
        ...
        if (ImportFiles[i].type == FSTD_FILE) {
            ...
            if (ext == "gstl") // Panel (old GuiStyle)
            {
                ...
                if (game.load(file)) {
                    ...
                    edit.create(game, ImportFindID(game.center_image.name(), ELM_IMAGE),
                                ...
                                ImportFindID(game.panel_image.name(), ELM_PANEL_IMAGE));
                    Save(edit, Proj.editPath(elm));
                    edit.copyTo(game, Proj);
                    Save(game, Proj.gamePath(elm));
                    ...
                    if (edit_mesh.load(file))
                        if (edit_mesh.is()) {
                            ...
                            *ImportElms(file) = elm.id; // src mesh is now ELM_OBJ
                            ...
                            // skeleton
                            if (edit_mesh.skeleton()) {
                                Elm &skel_elm = Proj.Project::newElm(ElmNameSkel, elm.id, ELM_SKEL);
                                mesh_elm.meshData()->skel_id = skel_elm.id;
                                ElmSkel *skel_data = skel_elm.skelData();
                                skel_data->newData();
                                skel_data->src_file = MakeFullPath(Skeletons.name(edit_mesh.skeleton()), FILE_DATA);
                                skel_data->mesh_id = mesh_elm.id;
                                EditSkeleton edit_skel;
                                edit_skel.create(*edit_mesh.skeleton());
                                Save(edit_skel, Proj.editPath(skel_elm));
                            ...
                            Proj.getMeshSkels(mesh_elm.meshData(), null, &body_skel);
                            Mesh game_mesh;
                            EditToGameMesh(edit_mesh, game_mesh, body_skel, null, null);
                            Save(edit_mesh, Proj.editPath(mesh_elm), Proj.game_path); // must use 'game_path'
                            Save(game_mesh, Proj.gamePath(mesh_elm));
                            mesh_elm.meshData()->from(game_mesh);
            ...
            } else if (ext == "particle") // particles
            {
                Particles p;
                if (p.load(file)) {
                    Elm &elm = Proj.Project::newElm(name, Proj.getPathID(path), ELM_OBJ);
                    ...
                    if (Elm *elm = ImportFind(p.image.name(), ELM_IMAGE))
                        edit.New().setName("Image").setAsIDArray(elm->id).id.set(5, 0, 0, 0);
                    if (Elm *elm = ImportFind(p.palette_image.name(), ELM_IMAGE))
                        edit.New().setName("Palette Image").setAsIDArray(elm->id).id.set(34, 0, 0, 0);
                    if (p.image_x_frames > 1)
                        edit.New().setName("Image Frames X").forceInt(p.image_x_frames).id.set(6, 0, 0, 0);
                    if (p.image_y_frames > 1)
                        edit.New().setName("Image Frames Y").forceInt(p.image_y_frames).id.set(7, 0, 0, 0);
                    if (!Equal(p.image_speed, 1))
                        edit.New().setName("Image Speed").forceFlt(p.image_speed).id.set(8, 0, 0, 0);
                    if (!p.reborn)
                        edit.New().setName("Reborn").forceBool(p.reborn).id.set(9, 0, 0, 0);
                    edit.New().setName("Palette Mode").forceBool(p.palette()).id.set(10, 0, 0, 0);
                    if (p.paletteIndex())
                        edit.New().setName("Palette Index").forceBool(p.paletteIndex() != 0).id.set(11, 0, 0, 0);
                    if (p.smooth_fade)
                        edit.New().setName("Smooth Fade").forceBool(p.smooth_fade).id.set(12, 0, 0, 0);
                    edit.New().setName("Elements").forceInt(p.p.elms()).id.set(13, 0, 0, 0);
                    edit.New().setName("Color").forceColor(p.color).id.set(1, 0, 0, 0);
                    if (p.glow)
                        edit.New().setName("Glow").forceInt(p.glow).id.set(14, 0, 0, 0);
                    edit.New().setName("Radius").forceFlt(p.radius).id.set(15, 0, 0, 0);
                    if (!Equal(p.radius_random, 0))
                        edit.New().setName("Radius Random").forceFlt(p.radius_random).id.set(32, 0, 0, 0);
                    if (!Equal(p.radius_growth, 1))
                        edit.New().setName("Radius Growth").forceFlt(p.radius_growth).id.set(16, 0, 0, 0);
                    if (!Equal(p.offset_range, 0))
                        edit.New().setName("Offset Range").forceFlt(p.offset_range).id.set(17, 0, 0, 0);
                    if (!Equal(p.offset_speed, 1))
                        edit.New().setName("Offset Speed").forceFlt(p.offset_speed).id.set(18, 0, 0, 0);
                    edit.New().setName("Life").forceFlt(p.life).id.set(19, 0, 0, 0);
                    if (!Equal(p.life_random, 0))
                        edit.New().setName("Life Random").forceFlt(p.life_random).id.set(33, 0, 0, 0);
                    if (!Equal(p.glue, 0))
                        edit.New().setName("Glue").forceFlt(p.glue).id.set(20, 0, 0, 0);
                    if (!Equal(p.damping, 0))
                        edit.New().setName("Velocity Damping").forceFlt(p.damping).id.set(21, 0, 0, 0);
                    if (!Equal(p.ang_vel, 0))
                        edit.New().setName("Angular Velocity").forceFlt(p.ang_vel).id.set(22, 0, 0, 0);
                    edit.New().setName("Random Velocity").forceFlt(p.vel_random).id.set(23, 0, 0, 0);
                    if (!Equal(p.vel_constant.x, 0))
                        edit.New().setName("Initial Velocity X").forceFlt(p.vel_constant.x).id.set(24, 0, 0, 0);
                    if (!Equal(p.vel_constant.y, 0))
                        edit.New().setName("Initial Velocity Y").forceFlt(p.vel_constant.y).id.set(25, 0, 0, 0);
                    if (!Equal(p.vel_constant.z, 0))
                        edit.New().setName("Initial Velocity Z").forceFlt(p.vel_constant.z).id.set(26, 0, 0, 0);
                    if (!Equal(p.accel.x, 0))
                        edit.New().setName("Acceleration X").forceFlt(p.accel.x).id.set(27, 0, 0, 0);
                    if (!Equal(p.accel.y, 0))
                        edit.New().setName("Acceleration Y").forceFlt(p.accel.y).id.set(28, 0, 0, 0);
                    if (!Equal(p.accel.z, 0))
                        edit.New().setName("Acceleration Z").forceFlt(p.accel.z).id.set(29, 0, 0, 0);
                    if (p.sourceType() == PARTICLE_STATIC_SHAPE)
                        edit.New().setName("Source Shape").forceInt(p.shape.type).id.set(30, 0, 0, 0);
                    if (!p.inside_shape)
                        edit.New().setName("Inside Source").forceBool(p.inside_shape).id.set(31, 0, 0, 0);

                    Save(edit, Proj.editPath(elm));
                    edit.copyTo(game, Proj, true, null, null);
                    Save(game, Proj.gamePath(elm));
                    elm.objData()->newData();
                    elm.objData()->from(edit);
                    *ImportElms(file) = elm.id;
    ...
    FREPA(ImportFiles) {
        ...
        } else if (ImportFiles[i].type == FSTD_DIR) {
            if (ext == "world") // world
            {
                EE1WorldSettings settings;
                Str edit = file;
                edit.tailSlash(true) += "Edit/";
                if (settings.load(edit + "Settings")) {
                    ...
                    *ImportElms(file) = elm.id;
                    ...
                    WorldVer *world_ver = Proj.worldVerRequire(elm.id);
                    world_ver->changed = true;
                    for (FileFind ff(MakeFullPath(edit + "Area", FILE_DATA)); ff();)
                        if (ff.type == FSTD_FILE && !GetExt(ff.name).is()) {
                            if (thread.wantStop())
                                return false;
                            VecI2 xy = TextVecI2(ff.name);
                            File f;
                            if (f.readStd(ff.pathName())) {
                                Heightmap h;
                                ...
                                if (h.is() || objs.elms()) {
                                    Area area(xy, null);
                                    area.ver = world_ver->areas(xy); // create AreaVer (needed for rebuild)
                                    Memt<Obj> area_objs;
                                    if (h.is()) {
                                        Adjust(h);
                                    ...
                                    REPA(objs) {
                                        ...
                                        EditObjectPtr base;
                                        if (src.access() != OBJ_ACCESS_CUSTOM)
                                            type.zero(); // clear class if we're not using custom
                                        Elm *base_elm = ImportFind(src.base().name(), ELM_OBJ);
                                        if (!base_elm)
                                            base_elm = ImportFind(src.mesh().name(), ELM_OBJ); // if object base was not found, then try checking its mesh (because earlier we've imported non-object meshes as objects)
                                        if (base_elm)                                          // create only if we've found ELM_OBJ base
                                        {
                                            ...
                                            Adjust(obj.params);
                                            if (C Matrix *matrix = ImportObjMatrix.find(base_elm->id))
                                                obj.matrix = (*matrix) * obj.matrix; // object meshes are already transformed
                                            world_ver->changedObj(obj, xy);
                                        }
                                    }
                                    if (area_objs.elms()) {
                                        FREPA(area_objs)
                                        area.objs.add(&area_objs[i]);
                                        if (area.ver)
                                            area.ver->obj_ver.randomize();
                    ...
                    Pak global;
                    if (global.load(MakeFullPath(edit + "Global/Waypoints.pak", FILE_DATA)))
                        FREPA(global)
                    if (global.file(i).data_size) // waypoints
                    {
                        EE1EditWaypoint src;
                        File f;
                        if (f.read(global.file(i), global))
                            if (src.load(f, global.file(i).name)) {
                                EditWaypoint edit;
                                edit.create(src, src.name);
                                Save(edit, Proj.editWaypointPath(elm.id, src.id));
                                Game::Waypoint game;
                                edit.copyTo(game);
                                Save(game, Proj.gameWaypointPath(elm.id, src.id));
                                world_ver->changedWaypoint(src.id);
                            }
                    }
                    if (global.load(MakeFullPath(edit + "Global/Lakes.pak", FILE_DATA)))
                        FREPA(global)
                    if (global.file(i).data_size) // lakes
                    {
                        EE1EditLake src;
                        File f;
                        if (f.read(global.file(i), global))
                            if (src.load(f, global.file(i).name)) {
                                Lake lake;
                                lake.setDepth(src.depth);
                                Swap(lake.polys, src.polys);
                                lake.polys_time.getUTC();
                                Save(lake, Proj.editLakePath(elm.id, src.id));
                                world_ver->changedLake(src.id);
                            }
                    }
                    if (global.load(MakeFullPath(edit + "Global/Rivers.pak", FILE_DATA)))
                        FREPA(global)
                    if (global.file(i).data_size) // rivers
                    {
                        EE1EditRiver src;
                        File f;
                        if (f.read(global.file(i), global))
                            if (src.load(f, global.file(i).name)) {
                                River river;
                                river.setDepth(src.depth);
                                FREPA(src.vtxs)
                                river.vtxs.New().set(src.vtxs[i].radius, src.vtxs[i].pos);
                                Swap(river.edges, src.edges);
                                river.vtx_edge_time.getUTC();
    ...
    REPA(Proj.elms) {
        ...
        if (ElmMesh *mesh_data = elm.meshData()) {
            REPA(mesh_data->mtrl_ids) {
                ...
                {
                    Elm &elm_other = Proj.elms[i];
                    if (ElmMesh *mesh_data = elm_other.meshData())
                        if (&elm_other != &elm)
                            if (mesh_data->mtrl_ids.binaryHas(mtrl_id))
                                goto used;
                }
                if (Elm *mtrl_elm = Proj.findElm(mtrl_id, ELM_MTRL))
                    if (Elm *obj = Proj.meshToObjElm(&elm))
                        mtrl_elm->setParent(obj); // set to object
            used:;
            }
        }
    }
...
bool ImportFilter(C Str &name) {
    ...
    if (ext == "world" || ext == "atlas")          // world and atlas
        ...
        {
            BackgroundFileFind::File &file = ImportManual.New();
            file.get(name);
            file.name = name; // add this element manually
...
bool InitImport() {
    ...
    if (FileInfoSystem(ImportSrc).type != FSTD_DIR)
        StateProjectList.set(StateFadeTime);
    else {
        UID id;
        id.randomizeValid();
        Str name = GetBase(ImportSrc);
        if (name == "Data")
            name = GetBase(GetPath(ImportSrc));
        Str path = ProjectsPath + EncodeFileName(id);
        Str error;
        LOAD_RESULT result = Proj.Project::open(id, name, path, error); // open using 'Project' method
        if (!FExistSystem(path) || !LoadOK(result)) {
            StateProjectList.set(StateFadeTime);
            Gui.msgBox(S, S + "Can't write to:\"\n" + ProjectsPath + '"');
        } else {
            DataPath(ImportSrc);
...
void ShutImport() {
    ...
    ImportBFF.del();
    ImportFiles.del();
    ImportManual.del(); // delete after BFF
    ImportElms.del();
...
bool UpdateImport() {
    ...
    if (ImportPhase == IMPORT_FINISHED) {
        ...
        Proj.save();
        Str error;
        Proj.open(Proj.id, Proj.name, Proj.path, error); // reopen using 'ProjectEx' method
        SetProjectState();
    }
...
bool EE1WorldSettings::load(C Str &name) {
    File f;
    if (f.readStd(name))
        return load(f);
    reset();
...
bool EE1EditWaypoint::loadData(File &f) {
    File temp;
    temp.writeMem().putUInt(CC4('W', 'P', 'N', 'T'));
    f.copy(temp);
    temp.pos(0);
    return Game::Waypoint::load(temp);
}
    ...
    if (id.fromHex(name))
        if (super::load(f))
            ...
            case 0: {
                ...
                polys.clear();
                REP(f.getInt()) {
                    Memc<Vec> &poly = polys.New();
                    poly.setNum(f.getInt());
                    FREPA(poly)
                    f >> poly[i];
                }
                GetStr(f, material);
...
bool EE1EditRiver::load(File &f, C Str &name) {
    if (id.fromHex(name))
        if (super::load(f))
            ...
            case 0: {
                f >> depth;
                vtxs.setNum(f.getInt());
                FREPA(vtxs)
                f >> vtxs[i];
                edges.setNum(f.getInt());
                FREPA(edges)
                f >> edges[i];
                GetStr(f, material);
            }
