...
void MiscRegion::ProjList(bool all_saved, ptr) {
    if (all_saved)
...
void MiscRegion::FindProj(MiscRegion &mr) {
    if (mr.hide_proj())
        mr.hide_proj.push();
    Proj.filter.selectAll().activate();
}
void MiscRegion::ModeClose(MiscRegion &mr) { Mode.closeActive(false); }
void MiscRegion::Mode0(MiscRegion &mr) {
    if (Tab *tab = Mode.visibleTab(0))
        tab->push();
}
void MiscRegion::Mode1(MiscRegion &mr) {
    if (Tab *tab = Mode.visibleTab(1))
        tab->push();
}
void MiscRegion::Mode2(MiscRegion &mr) {
    if (Tab *tab = Mode.visibleTab(2))
        tab->push();
}
void MiscRegion::Mode3(MiscRegion &mr) {
    if (Tab *tab = Mode.visibleTab(3))
        tab->push();
}
void MiscRegion::Mode4(MiscRegion &mr) {
    if (Tab *tab = Mode.visibleTab(4))
        tab->push();
}
void MiscRegion::Mode5(MiscRegion &mr) {
    if (Tab *tab = Mode.visibleTab(5))
        tab->push();
}
...
void MiscRegion::PublishProjectPackage(MiscRegion &mr) {
    if (!PublishProjectPackageIO.is())
        PublishProjectPackageIO.create(ProjectPackageExt, S, SystemPath(SP_DESKTOP), null, PublishProjectPackageAs);
    PublishProjectPackageIO.save().name(CleanFileName(Proj.name));
}
...
void MiscRegion::About(MiscRegion &mr) {
    ...
    if (s.is())
        s += ", ";
    s += S + Cpu.threads() + " HW Threads";
    if (s.is())
        about.line() += S + "CPU: " + s;
    s = D.deviceName();
    if (s.is()) {
        if (D.deviceMemory() > 0)
            s += S + ", " + SizeBytes(D.deviceMemory(), '.') + " RAM";
        about.line() += S + "GPU: " + s;
    }
    MemStats m;
    m.get();
    if (m.total_phys > 0)
        about.line() += S + "RAM: " + SizeBytes(m.total_phys, '.');
    s = OSName();
    if (s.is())
        about.line() += S + "Operating System: " + s;
    s = OSUserName();
    if (s.is())
        about.line() += S + "System User Name: " + s;
#if DESKTOP
    s = S + App.desktopW() + 'x' + App.desktopH();
    if (s.is())
        about.line() += S + "Desktop Resolution: " + s;
#endif
    s = S + D.resW() + 'x' + D.resH();
    if (s.is())
        about.line() += S + "Application Resolution: " + s;
...
void MiscRegion::create() {
    ...
    Node<MenuElm> build_menu;
    {
        ...
        build_menu.New().create(S + "Save as *." + ProjectPackageExt, PublishProjectPackage, T).kbsc(KbSc(KB_F8, KBSC_CTRL_CMD | KBSC_SHIFT)).desc(S + "Export your project into a single *." + ProjectPackageExt + " file which can be easily imported by other users.");
        {
            ...
            {
                Node<MenuElm> &code = (Export += "Code Synchronization");
                code.desc("Import/Export Project Source Code which can be synchronized using 3rd party tools.");
    ...
    hide_proj.mode = BUTTON_TOGGLE;
    T += menu.create(Rect_LU(hide_proj.rect().ru(), 0.060f, 0.06f), menu_menu).skin(&NoComboBoxImage).focusable(false).desc("Menu");
    menu.text = "M";
    menu.text_align = 0;
    ...
    play.image = "Gui/arrow_right_big.img";
    T += build.create(Rect_LU(play.rect().ru(), 0.125f, 0.06f)).setData(build_menu).focusable(false);
    build.text = "Build";
    build.text_size *= 0.64f;
    FlagDisable(build.flag, COMBOBOX_MOUSE_WHEEL);
    build.flag |= COMBOBOX_CONST_TEXT;
    Gui += move_misc.create(Rect_LU(0, 0, h, h)).func(Move, T).hide().focusable(false).desc("Move bar around the screen");
    move_misc.image = "Gui/Misc/move.img";
    move_misc.mode = BUTTON_CONTINUOUS;
    // FINISH
...
void MiscRegion::updateMove() {
    if (Mode() != MODE_GUI)
        setPos(VecI2(0, MiscOnTop));
    move_misc.visible(visible() && Mode() == MODE_GUI);
}
void MiscRegion::update(C GuiPC &gpc) {
    super::update(gpc);
    if (screenshot && !--screenshot)
        Renderer.screenShots(SystemPath(SP_DESKTOP).tailSlash(true) + ENGINE_NAME " Editor ScreenShots/", "bmp");
}
...
MiscRegion &MiscRegion::show() {
    super::show();
    updateMove();
    return T;
}
MiscRegion &MiscRegion::hide() {
    super::hide();
    updateMove();
    return T;
}
