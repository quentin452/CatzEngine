...
void SizeStatistics::ElmListElm::setElm(C UID &id, uint size, uint size_compressed) {
    T.id = id;
    T.size = size;
    T.size_compressed = size_compressed;
    if (Elm *elm = Proj.findElm(id))
        type_name = ElmTypeName[T.type = elm->type];
}
    ...
    if (ElmListElm *list_elm = list()) {
        if (list_elm->type == ELM_IMAGE) // for images and textures read them directly from the data
        {
            ...
            if (!list_elm->proj_elm)
                D.alpha(ALPHA_NONE); // disable alpha blending for textures
            Rect rect = image.fit(D.viewRect());
            if (image.mode() == IMAGE_CUBE)
                image.drawCubeFace(WHITE, TRANSPARENT, rect, DIR_FORWARD);
            else {
                if (image.isSByte()) {
                    int channels = image.typeChannels();
                    VI.shader(ShaderFiles("Main")->get((channels == 1) ? "DrawTexXSG" : (channels == 2) ? "DrawTexXYSG"
                                                                                                        : "DrawTexSG"));
                }
                image.draw(rect);
            }
...
SizeStatistics &SizeStatistics::del() {
    release();
    super::del();
    return T;
}
SizeStatistics &SizeStatistics::hide() {
    release();
    super::hide();
    return T;
}
...
void SizeStatistics::create() {
    ...
    lc[2].sort = &sort_size[0];
    lc[3].sort = &sort_size[1];
    packed_col = 3;
    sort_size[0] = MEMBER(ElmListElm, size);
    sort_size[1] = MEMBER(ElmListElm, size_compressed);

    Gui += super::create("Project Data Size Statistics").hide();
    button[1].show();
    ...
    list.sort_column[0] = 3;
    list.sort_column[1] = 2;
    list.sort_swap[0] = list.sort_swap[1] = true; // sort by compressed size (from biggest to smallest)
...
void SizeStatistics::refresh(C Str &path, Cipher *cipher) {
    ...
    if (path.is()) {
        ...
        if (fi.type == FSTD_FILE) // PAK file
        {
            if (pak.load(path, cipher)) {
                REP(pak.rootFiles()) {
                    ...
                    if (elm_id.valid())
                        data.New().setElm(elm_id, pak.totalSize(pf, false), pak.totalSize(pf, true));
                }
                if (C PakFile *pf = pak.find("Tex", false))
                    REP(pf->children_num) {
        ...
        } else if (fi.type == FSTD_DIR) // DIR of separate files
        {
            T.path = path;
    ...
    {
        T.path = Proj.game_path;
        T.path.tailSlash(true);
        Memb<PakFileData> files;
        GetPublishFiles(files);
        FREPA(files) {
            ...
            if (start == "Tex") {
                ...
                Str path = pfd.data.name;
                if (!FExist(path) && elm_id.valid())
                    path = Proj.texPath(elm_id);
                long size = FSize(path);
                data.New().setTex(GetBase(pfd.name), path, size, size);
            } else if (start.length() == pfd.name.length() && elm_id.valid()) {
    ...
    bool packed = false;
    REPA(data)
    if (data[i].size != data[i].size_compressed) {
        packed = true;
        break;
    }
...
void SizeStatistics::displayUnusedMaterials() {
    release();
    T.path = Proj.game_path;
    T.path.tailSlash(true); // set from Project
    Memc<UID> used_mtrls;
    Proj.getUsedMaterials(used_mtrls, true);
    REPA(Proj.elms) {
        Elm &elm = Proj.elms[i];
        if (elm.finalPublish())
            switch (elm.type) {
    ...
    if (gpc.visible && visible()) {
        if (Ms.bd(0) && Gui.ms() == &list || Kb.k(KB_ENTER) && Gui.kb() == &list)
            if (ElmListElm *list_elm = list()) {
                ...
                } else if (list_elm->type == ELM_IMAGE) // texture
                {
        ...
        if (Ms.bp(1) && Gui.ms() == &list) {
            ...
            if (list.sel.elms()) {
                menu_elm.clear();
                FREPA(list.sel)
                if (ElmListElm *elm = list.absToData(list.sel[i]))
                    if (elm->proj_elm)
                        menu_elm.add(elm->id); // add in order
                Node<MenuElm> n;
