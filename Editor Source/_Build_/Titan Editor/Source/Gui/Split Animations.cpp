...
/******************************************************************************/
void SplitAnimation::Anim::Remove(Anim &anim) {
    SplitAnim.anims.removeData(&anim, true);
    SplitAnim.setList();
}
SplitAnimation::Anim::Anim() {
    SplitAnim.region += remove.create().func(Remove, T).focusable(false).desc("Remove this split");
...
void SplitAnimation::Hide(SplitAnimation &sa) {
    sa.hide();
    if (AnimEdit.preview.visible())
        AnimEdit.preview.activate();
    else
        Proj.list.kbSet();
}
void SplitAnimation::splitDo() {
    if (anims.elms()) {
        ...
        // split
        if (Elm *base = Proj.findElm(anim_id, ELM_ANIM))
            if (ElmAnim *base_data = base->animData()) {
                Mems<FileParams> files = FileParams::Decode(base->srcFile());
                if (files.elms() != 1)
                    Gui.msgBox(S, S + "Unknown source file for animation.");
                else {
                    Str src = files[0].name, anim_name;
                    if (C TextParam *name = files[0].findParam("name"))
                        anim_name = name->value;
                    if (FileInfoSystem(src).type != FSTD_FILE)
                        Gui.msgBox(S, S + "Can't find file \"" + src + '"');
                    else {
                        TimeStamp time;
                        time.getUTC();
                        ...
                        if (anim_name.is())
                            files[0].params[3].set("name", anim_name);
                        FREPA(anims) {
                            C Anim &anim = anims[i];
                            ...
                            {
                                base->setName(anim.name(), time).setSrcFile(FileParams::Encode(files), time);
                                Proj.elmReload(base->id, false, false);
                                Server.renameElm(*base);
...
void SplitAnimation::clearDo() {
    anims.clear();
    setList();
}
void SplitAnimation::addNew() {
    anims.New();
    setList();
}
...
bool SplitAnimation::Create(int &data, C Str &key, ptr user) {
    data = 0;
    return true;
}
...
void SplitAnimation::add(C Str &text) {
    ...
    FREPA(lines) {
        ...
        if (tokens.elms()) {
            ...
            if (IsNumber(tokens[i])) {
                ...
                {
                    Str separator;
                    for (int j = number_pos + 1; j < i; j++)
                        separator.space() += tokens[j];
                    if (int *s = separators(separator))
                        (*s)++; // increase occurence of this separator
                }
                number_pos = i;
            }
            if (!IsNumber(tokens.first()))
                if (int *s = left(tokens.first()))
                    (*s)++; // increase occurence of first token
            if (!IsNumber(tokens.last()))
                if (int *s = right(tokens.last()))
                    (*s)++; // increase occurence of last  token
        }
    }

    // find the separator with most occurences
    C Str *separator = null;
    int occurences = 0;
    REPA(separators)
    if (separators[i] > occurences) {
    ...
    REPA(left)
    MAX(left_repeat, left[i]);
    REPA(right)
    MAX(right_repeat, right[i]);

    // tokenize the separator
    Memt<Str> sep_tokens;
    if (separator)
        Tokenize(sep_tokens, *separator);
    ...
    FREPA(lines) {
        ...
        Tokenize(tokens, lines[i]);
        FREPA(tokens)
        if (IsNumber(tokens[i]) && InRange(i + 1 + sep_tokens.elms(), tokens) && IsNumber(tokens[i + 1 + sep_tokens.elms()])) {
            for (int j = 0; j < sep_tokens.elms(); j++)
                if (!InRange(i + 1 + j, tokens) || tokens[i + 1 + j] != sep_tokens[j])
                    goto different;
            {
                ...
                if (InRange(name_range_left.y, tokens) && CharType(tokens[name_range_left.y][0]) == CHART_SIGN)
                    name_range_left.y--;
                if (InRange(name_range_right.x, tokens) && CharType(tokens[name_range_right.x][0]) == CHART_SIGN)
                    name_range_right.x++;

                bool left = (left_repeat < right_repeat);
                if (name_range_left.y < name_range_left.x)
                    left = false;
                ...
                Str name;
                for (int i = name_range.x; i <= name_range.y; i++)
                    if (InRange(i, tokens))
                        name.space() += tokens[i];
...
void SplitAnimation::drop(Memc<Str> &names, GuiObj *obj, C Vec2 &screen_pos) {
    if (contains(obj)) {
        FREPA(names) {
            ...
            } else if (ext == "meta") // unity metafile
            {
                bool added = false;
                TextData td;
                if (td.loadYAML(name))
                        ...
                        if (C TextNode *animations = ModelImporter->findNode("animations"))
                            if (C TextNode *clipAnimations = animations->findNode("clipAnimations"))
                                FREPA(clipAnimations->nodes) {
                                    ...
                                    Str name, first, last;
                                    if (C TextNode *n = clip.findNode("name"))
                                        name = n->asText();
                                    if (C TextNode *n = clip.findNode("firstFrame"))
                                        first = n->asText();
                                    if (C TextNode *n = clip.findNode("lastFrame"))
                                        last = n->asText();
                                    if (name.is() || first.is() || last.is()) {
                                        ...
                                        anim.to.set(last);
                                        anim.name.set(name);
                                        if (C TextNode *n = clip.findNode("loop"))
                                            anim.loop.set(n->asBool());
                                        added = true;
                                    }
                                }
                if (added)
