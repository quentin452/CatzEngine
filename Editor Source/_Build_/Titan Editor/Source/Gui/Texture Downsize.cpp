...
bool TextureDownsize::DownsizedMtrl::Create(DownsizedMtrl &mtrl, C UID &original_id, ptr user) {
    mtrl.create(original_id);
    return true;
}
void TextureDownsize::DownsizedMtrl::create(C UID &original_id) {
    if (original = Proj.gamePath(original_id))
        SCAST(Material, T) = *original; // copy everything
    setTextures();
}
...
int TextureDownsize::MaterialUses(C MeshLod &lod, C MaterialPtr &mtrl, int variation) // how many 'lod' parts use 'mtrl' in 'variation'
{
    int uses = 0;
    REPA(lod)
    if (lod.parts[i].variation(variation) == mtrl)
        uses++;
    return uses;
}
VecI2 TextureDownsize::LodAndVariationWithMostMaterialUses(C Mesh &mesh, C MaterialPtr &mtrl) // X=LOD, Y=Variation
{
    ...
    FREPD(l, mesh.lods()) // iterate all lods, go from the start to prefer most detailed
    {
        ...
        if (most_uses)
            return VecI2(l, variation); // stop on first LOD with any uses
    }
    ...
    case RM_PREPARE: {
        ...
        if (draw_as_obj) // draw as object
        {
            if (normal_mesh) {
                ...
                C Mesh &mesh = (interval ? downsized_mesh : *normal_mesh);
                if (InRange(lv.x, mesh.lods())) {
                    C MeshLod &lod = mesh.lod(lv.x);
                    lod.waitForStream();
                    SetVariation(lv.y);
                    lod.draw(MatrixIdentity);
                    SetVariation();
                }
            }
            LightDir(light_dir() ? !Vec(-1, -1, -1) : ActiveCam.matrix.z, 1 - D.ambientColorL()).add(false);
        } else if (normal_mtrl) // draw as material
        {
...
void TextureDownsize::Draw(Viewport &viewport) {
    if (Edit::Viewport4::View *view = TexDownsize.v4.getView(&viewport))
        TexDownsize.draw(*view);
}
void TextureDownsize::draw(Edit::Viewport4::View &view) {
    ...
    // environment
    EnvironmentPtr env = EnvEdit.cur();
    if (!env)
    ...
    if (!elm)
        D.text(D.viewRect(), "No Material/Object Selected\nLeft-click on a Project Element");
}
...
Str TextureDownsize::Global(C TextureDownsize &td) {
    if (C Settings *s = td.curSettings())
        return s->global;
    return S;
}
void TextureDownsize::Global(TextureDownsize &td, C Str &text) {
    if (Settings *s = td.curSettings())
        s->global = TextInt(text);
}
Str TextureDownsize::Base0(C TextureDownsize &td) {
    if (C Settings *s = td.curSettings())
        return s->base_0;
    return S;
}
void TextureDownsize::Base0(TextureDownsize &td, C Str &text) {
    if (Settings *s = td.curSettings())
        s->base_0 = TextInt(text);
}
Str TextureDownsize::Base1(C TextureDownsize &td) {
    if (C Settings *s = td.curSettings())
        return s->base_1;
    return S;
}
void TextureDownsize::Base1(TextureDownsize &td, C Str &text) {
    if (Settings *s = td.curSettings())
        s->base_1 = TextInt(text);
}
Str TextureDownsize::Base2(C TextureDownsize &td) {
    if (C Settings *s = td.curSettings())
        return s->base_2;
    return S;
}
void TextureDownsize::Base2(TextureDownsize &td, C Str &text) {
    if (Settings *s = td.curSettings())
        s->base_2 = TextInt(text);
}
Str TextureDownsize::Emissive(C TextureDownsize &td) {
    if (C Settings *s = td.curSettings())
        return s->emiss;
    return S;
}
void TextureDownsize::Emissive(TextureDownsize &td, C Str &text) {
    if (Settings *s = td.curSettings())
        s->emiss = TextInt(text);
}
...
bool TextureDownsize::Next(int dir) // find next compatible element
{
    ...
    if (nextSecondary(dir, old_base, old_emis))
        return true; // iterate through OBJ materials first
    int c = Proj.list.cur;
    REP(Proj.list.elms()) {
        c = Mod(c + dir, Proj.list.elms());
        if (Elm *elm = Proj.list.visToElm(c))
            if (elm->type == ELM_MTRL || elm->type == ELM_OBJ) {
                setElm(elm, dir < 0); // set secondary as first/last based on 'dir'
                if (normal_base[0] != old_base[0] || normal_base[1] != old_base[1] || normal_base[2] != old_base[2] || normal_emis != old_emis || nextSecondary(dir, old_base, old_emis)) {
                    Proj.list.setCur(c).scrollTo(c, false, 0.5f);
                    return true;
                }
            }
    }
...
VecI2 TextureDownsize::ImageSize(C ImagePtr &image, int down) {
    if (image && image->is())
        return VecI2(Max(1, Shr(image->w(), down)), Max(1, Shr(image->h(), down)));
    return 0;
}
...
void TextureDownsize::applyConfirm(bool revert) {
    ...
    if (Settings *settings = curSettings()) {
        ...
        if (revert)
            base0 = base1 = base2 = emis = 0;
        if (base0 || base1 || base2 || emis || relative) // if we want any change
        {
            EditMaterial original;
            if (!normal_mtrl || !Proj.mtrlGet(mtrl_id, original)) {
                Gui.msgBox(S, "Can't access Material");
                return;
            }
            if (!original.base_0_tex.valid())
                base0 = 0; // if texture is not present, then we can't modify it
            if (!original.base_1_tex.valid())
                base1 = 0; // if texture is not present, then we can't modify it
            if (!original.base_2_tex.valid())
                base2 = 0; // if texture is not present, then we can't modify it
            if (!original.emissive_tex.valid())
                emis = 0;
            ...
            if (base0 || base1 || base2 || emis || relative)                                                                                    // if we want any change
                if (original.base_0_tex.valid() || original.base_1_tex.valid() || original.base_2_tex.valid() || original.emissive_tex.valid()) // and we have any texture
                {
                    ...
                    Str invalid;
                    if (Proj.invalidTexSrc(original.color_map, &invalid) || Proj.invalidTexSrc(original.alpha_map, &invalid) || Proj.invalidTexSrc(original.bump_map, &invalid) || Proj.invalidTexSrc(original.normal_map, &invalid) || Proj.invalidTexSrc(original.smooth_map, &invalid) || Proj.invalidTexSrc(original.metal_map, &invalid) || Proj.invalidTexSrc(original.glow_map, &invalid) || Proj.invalidTexSrc(original.emissive_map, &invalid)) {
                        Gui.msgBox(S, S + "Can't find:\n" + invalid.replace(' ', Nbsp));
                        return;
                    }
                    ...
                    if (base1_size != base2_size)
                        downsized.separateNormalMap(time); // normal can be from bump
                    ...
                    if (Proj.mtrlSync(mtrl_id, downsized, true, false))
                        if (Proj.mtrlGet(mtrl_id, downsized)) // get after syncing, because of reload base textures, we should now have new Tex ID's
                        {
                            ...
                            upscaled = ((changed_base0 && base0 < 0) || (changed_base1 && base1 < 0) || (changed_base2 && base2 < 0) || (changed_emis && emis < 0));
                                        ...
                                        if (changed_base0 && mtrl_data->base_0_tex == original.base_0_tex        // first check if we're changing this texture, then compare if it's the same
                                            ...
                                            if (Proj.mtrlGet(elm.id, test))
                                                if (AdjustTexSrc(test.color_map, test.color_map_time, original.color_map, downsized.color_map, time) // !! use '|' and not '||' because we need to process all !!
                                                    | AdjustTexSrc(test.alpha_map, test.alpha_map_time, original.alpha_map, downsized.alpha_map, time) | AdjustTexSrc(test.bump_map, test.bump_map_time, original.bump_map, downsized.bump_map, time) | AdjustTexSrc(test.normal_map, test.normal_map_time, original.normal_map, downsized.normal_map, time) | AdjustTexSrc(test.smooth_map, test.smooth_map_time, original.smooth_map, downsized.smooth_map, time) | AdjustTexSrc(test.metal_map, test.metal_map_time, original.metal_map, downsized.metal_map, time) | AdjustTexSrc(test.glow_map, test.glow_map_time, original.glow_map, downsized.glow_map, time) | AdjustTexSrc(test.emissive_map, test.emissive_map_time, original.emissive_map, downsized.emissive_map, time)) {
                                                    if (changed_base0 && test.base_0_tex == original.base_0_tex)
                                                        test.base_0_tex = downsized.base_0_tex;
                                                    if (changed_base1 && test.base_1_tex == original.base_1_tex)
                                                        test.base_1_tex = downsized.base_1_tex;
                                                    if (changed_base2 && test.base_2_tex == original.base_2_tex)
                                                        test.base_2_tex = downsized.base_2_tex;
                                                    if (changed_emis && test.emissive_tex == original.emissive_tex)
                                                        test.emissive_tex = downsized.emissive_tex;
    ...
    if (revert || upscaled || !Next())
        refresh(); // proceed to next one
}
...
void TextureDownsize::applyDo() {
    if (different) // if there was any change applied
    {
        ...
        dialog.set("Downsize Material Textures", "Are you sure you want to downsize material textures?", Memt<Str>().add("Yes").add("No"));
...
bool TextureDownsize::sweep(GuiObj *go, C Vec2 &screen_pos, int *variation_index, int *lod_index, int *part_index, Vec *hit_pos) {
    ...
    if (normal_mesh)
        if (Edit::Viewport4::View *view = v4.getView(go)) {
            VecI2 lv = LodAndVariationWithMostMaterialUses(*normal_mesh, normal_mtrl);
            if (InRange(lv.x, normal_mesh->lods())) {
                ...
                Vec hp;
                REPA(lod)
                if (Sweep(pos, dir, lod.parts[i], null, &f, &hp, null, true, -1, false))
                    if (!hit || f < frac) {
                        hit = true;
                        frac = f;
                        if (variation_index)
                            *variation_index = lv.y;
                        if (lod_index)
                            *lod_index = lv.x;
                        if (part_index)
                            *part_index = i;
...
void TextureDownsize::camCenter(bool zoom) {
    Vec hit_pos;
    bool hit = sweep(Gui.ms(), Ms.pos(), null, null, null, &hit_pos);
    flt dist;
    v4.moveTo(hit ? hit_pos : normal_mesh ? normal_mesh->ext.center
                                          : VecZero);
    if (zoom && selectionZoom(dist))
        v4.dist(hit ? dist / 5 : dist); // use smaller dist because here we focus on texture quality, so zoom up close
}
...
void TextureDownsize::toggle() {
    if (Mode() == MODE_TEX_DOWN)
        close();
    else {
        Mode.tabAvailable(MODE_TEX_DOWN, true);
        Mode.set(MODE_TEX_DOWN);
        HideBig();
    }
}
...
TextureDownsize &TextureDownsize::create() {
    ...
    T += mtrl_image.create().hide();
    mtrl_image.fit = FIT_FULL;
    mtrl_image.alpha_mode = ALPHA_NONE;
    T += region.create().removeSlideBars().skin(&TransparentSkin).hide();
    region.kb_lit = false;

    prop_ts.reset();
    prop_ts.align.set(1, 0);
    prop_ts.size = 0.053f;
    ...
    Rect r = AddProperties(props, region, Vec2(padd, -0.005f), prop_h, 0.18f, &prop_ts);
    REPAO(props).autoData(this).range(-1, 10).mouseEditSpeed(1).changed(SetTextures);

    Node<MenuElm> menu;
    Viewport4Region &v4 = T;
    ...
    REP(2) {
        ...
        Str desc = S + (i ? "Increase" : "Decrease") + "\nKeyboard Shortcut: " + elm.kbsc().asText();
        if (elm.kbsc1().is())
            desc.space() += elm.kbsc1().asText();
    ...
    left_ts.reset().size = 0.0475f;
    left_ts.align.set(-1, 0);
    right_ts = left_ts;
    right_ts.align.set(1, 0);
    Vec2 xs(r.min.x - 0.05f, downsize[0][1].rect().max.x + 0.05f);
    REPD(p, 5)
    REP(2)
    region += info[p][i].create(Vec2(xs.c[i], downsize[p][0].rect().centerY()), S, i ? &right_ts : &left_ts);
...
Str TextureDownsize::ImageInfo(C Image *image, int &size, bool side) {
    if (image && image->is()) {
        int mem_size = image->typeMemUsage();
        size += mem_size; // use 'typeMemUsage' because we need this only for stats
...
void TextureDownsize::setMtrl() {
    setTexIDs();
    mtrl_image.set(normal_mtrl ? normal_mtrl->base_0 : ImagePtr());
    mtrl_image.visible(mtrl_image.image != null); // !! DO NOT MERGE INTO SINGLE COMMAND !!
    setTextures();
}
void TextureDownsize::setObj() {
    ...
    normal_mesh.clear();
    if (C Elm *obj = Proj.findElm(obj_id))
        if (C ElmObj *obj_data = obj->objData())
            normal_mesh = Proj.gamePath(obj_data->mesh_id);
    if (normal_mesh) {
        downsized_mesh.create(*normal_mesh);
        REPD(l, downsized_mesh.lods()) {
            MeshLod &downsized_lod = downsized_mesh.lod(l);
            REPA(downsized_lod) {
                MeshPart &part = downsized_lod.parts[i];
                REP(part.variations())
                if (C MaterialPtr &mtrl = part.variation(i)) {
                    if (normal_base[0].valid() && normal_base[0] == mtrl->base_0.id() || normal_base[1].valid() && normal_base[1] == mtrl->base_1.id() || normal_base[2].valid() && normal_base[2] == mtrl->base_2.id() || normal_emis.valid() && normal_emis == mtrl->emissive_map.id())
        ...
        case ELM_MTRL: // get all objects using this material's textures
        {
            if (ElmMaterial *mtrl_data = elm->mtrlData()) {
                ...
                FREPA(Proj.elms) // 'elms' are sorted by ID, so process in order so we don't need to sort 'mtrls' later
                {
                    Elm &elm = Proj.elms[i];
                    if (ElmMaterial *mtrl_data = elm.mtrlData())
                        if (base0_valid && base0_id == mtrl_data->base_0_tex || base1_valid && base1_id == mtrl_data->base_1_tex || base2_valid && base2_id == mtrl_data->base_2_tex || emis_valid && emis_id == mtrl_data->emissive_tex)
                            mtrls.add(elm.id);
                }
                // get objects using these materials
                FREPA(Proj.elms) {
                    Elm &elm = Proj.elms[i];
                    if (allow_removed || elm.finalExists())
                    ...
                    if (mtrls.binaryHas(mesh_data->mtrl_ids[i])) {
                        secondary.add(elm.id);
                        break;
                    }
                }
            }
               ...
               REPA(mesh_data.mtrl_ids)secondary.binaryInclude(mesh_data.mtrl_ids[i]);
                ...
                if (MeshPtr mesh = Proj.gamePath(obj_data->mesh_id))
                    FREPD(l, mesh->lods()) // iterate all lods
                    {
                        ...
                        if (C MaterialPtr &mtrl = lod.parts[p].variation(v))
                            secondary.include(mtrl.id());
                    }
#endif
            }
        } break;
...
void TextureDownsize::setSecondary(int i) {
    if (elm) {
        ...
        if (elm->type == ELM_MTRL)
            setMtrlObj(elm->id, secondary_id);
        else if (elm->type == ELM_OBJ)
            setMtrlObj(secondary_id, elm->id);
    } else
        setMtrlObj(UIDZero, UIDZero);
}
void TextureDownsize::setElm(Elm *elm, bool prev) {
    if (elm)
        if (elm->type != ELM_OBJ && elm->type != ELM_MTRL)
            elm = null;
    if (T.elm != elm) {
        T.elm = elm;
        getSecondaryList();
    ...
    if (gpc.visible && visible()) {
        ...
        if (draw_as_obj) {
            if (normal_mesh) {
                if (Ms.b(0)) {
                    ...
                    if (sweep(Gui.ms(), Ms.pos(), &variation, &lod_i, &part))
                        if (InRange(lod_i, normal_mesh->lods())) {
                            C MeshLod &lod = normal_mesh->lod(lod_i);
                            if (InRange(part, lod)) {
                ...
                if (Ms.bd(0) && v4.getView(Gui.ms()))
                    ObjEdit.activate(Proj.findElm(obj_id));
            }
        } else {
            if (Edit::Viewport4::View *view = v4.getView(Gui.ms())) {
                ...
                if (Ms.b(0) && !Ms.b(2) && !Ms.b(MS_BACK)) {
                    view->camera.yaw -= Ms.d().x;
                    view->camera.pitch += Ms.d().y;
                    Ms.freeze();
                }
                if (Ms.bd(0))
                    MtrlEdit.activate(Proj.findElm(mtrl_id));
        ...
        if (Ms.bp(0) && Gui.ms() == &mtrl_image)
            MtrlEdit.activate(Proj.findElm(mtrl_id));
        if (Ms.bp(2))
            if (Dialog *dialog = Gui.findMsgBox(DialogID))
                if (dialog->contains(Gui.ms()))
                    dialog->hide();
