...
void TheaterClass::Options::Horizontal(Options &options, C Str &text) {
    options.horizontal = TextBool(text);
    Theater.refreshSize();
}
void TheaterClass::Options::Rows(Options &options, C Str &text) {
    Theater.old_rows = options.rows;
    options.rows = TextInt(text);
    Theater.refreshSize();
}
void TheaterClass::Options::Size(Options &options, C Str &text) {
    options.item_size = TextFlt(text);
    Theater.refreshSize();
}
void TheaterClass::Options::Show(Options &options, C Str &text) {
    options.show = SHOW(TextInt(text));
    Theater.refreshData();
}
...
::TheaterClass::Options &TheaterClass::Options::create() {
    ...
    Rect r = super::create("Options", Vec2(0.02f, -0.02f), 0.04f, 0.045f, 0.26f);
    barVisible(false).show();
    FlagDisable(flag, WIN_MOVABLE);
    button[2].func(null);
    size(Vec2(r.max.x, -r.min.y) + 0.02f + defaultInnerPaddingSize());
    return T;
}
void TheaterClass::Options::rowsDelta(int delta) {
    if (p_rows)
        p_rows->textline.set(S + p_rows->clamp(rows + delta));
}
void TheaterClass::Options::sizeFactor(flt factor) {
    if (p_size)
        p_size->textline.set(S + p_size->clamp(item_size * ScaleFactor(factor)));
}
void TheaterClass::Options::scaleFactor(flt factor) {
    if (p_scale)
        p_scale->textline.set(S + p_scale->clamp(item_3d_scale * ScaleFactor(factor)));
}
int TheaterClass::List2::screenToVis(C Vec2 &screen, C GuiPC *gpc) C {
    Theater.viewport.setDisplayView();
    Camera temp = ActiveCam, cam;
    cam.set(MatrixIdentity).set();
    Vec pos, dir;
    ScreenToPosDir(screen, pos, dir);
    dir *= D.viewRange();
    int vis = -1;
    flt frac;
    ...
    for (int i = vis_range.x; i <= vis_range.y; i++) {
        if (Elm *elm = Proj.findElm(visToData(i)->id))
            switch (elm->type) {
            case ELM_OBJ:
                if (ObjectPtr obj = Proj.gamePath(elm->id))
...
void TheaterClass::ModeChanged(TheaterClass &theater) {
    theater.refreshData();
    theater.refreshNow();
}
...
void TheaterClass::ToggleAxis(TheaterClass &theater) {
    theater.options.draw_axis ^= 1;
    theater.options.toGui();
}
void TheaterClass::ToggleAxisTop(TheaterClass &theater) {
    theater.options.axis_on_top ^= 1;
    theater.options.toGui();
}
void TheaterClass::ToggleBox(TheaterClass &theater) {
    theater.options.draw_box ^= 1;
    theater.options.toGui();
}
void TheaterClass::ToggleCenter(TheaterClass &theater) {
    theater.options.center ^= 1;
    theater.options.toGui();
}
void TheaterClass::ToggleScale(TheaterClass &theater) {
    theater.options.scale_fit ^= 1;
    theater.options.toGui();
}
void TheaterClass::ToggleLight(TheaterClass &theater) {
    theater.options.light_dir ^= 1;
    theater.options.toGui();
}
...
void TheaterClass::DragElmsStart(ProjectEx &proj) {
    ...
    if (ListElm *list_elm = Theater.list())
        proj.drag_list_sel.add(list_elm->id);
...
Matrix TheaterClass::getMatrix(C UID &elm_id, Box box, C Rect &rect) {
    ...
    Matrix m(1);
    if (transform_ids.binaryHas(elm_id))
        m = transform_matrix;
    if (options.center) {
        m.move(-box.center());
        box -= box.center(); // move so it's at the center
    }
    flt scale = 1;
    if (options.scale_fit) {
    ...
    if (small)
        m.rotateX(-PI_4);        // if it's short, then rotate to look from above
...
void TheaterClass::draw(C UID &elm_id, C Mesh &mesh, C Rect &rect) {
    ...
    int elms = ElmMatrixes.elms();
    ElmMatrixPtr elm_matrix = Proj.gamePath(elm_id);
    if (elms != ElmMatrixes.elms())
        elm_matrix->matrix = matrix;                // if just added then copy matrix
...
void TheaterClass::MeshMtrls::clean() {
    REP(variations)
    parts[0].variation(i, null, -1);
    variations = 0;
}
::TheaterClass::MeshMtrls &TheaterClass::MeshMtrls::create() {
    super::create(1).parts[0].variations(32).base.create(Ball(0.42f), VTX_NRM | VTX_TAN | VTX_TEX0, 16);
    setRender().setBox();
    delBase();
    return T;
}
bool TheaterClass::MeshMtrls::available() C { return InRange(variations, parts[0].variations()); }
Mesh &TheaterClass::MeshMtrls::set(C MaterialPtr &mtrl, int &variation) {
    parts[0].variation(variations, mtrl);
...
Mesh &TheaterClass::getMeshMaterial(C MaterialPtr &mtrl, int &variation) {
    MeshMtrls *mm = null;
    FREPA(mesh_mtrls) {
        MeshMtrls &mesh_mtrl = mesh_mtrls[i];
        if (mesh_mtrl.available()) {
            mm = &mesh_mtrl;
            goto found;
        }
    }
    ...
    case RM_PREPARE: {
        ...
        for (int i = vis_range.x; i <= vis_range.y; i++)
            if (Elm *elm = Proj.findElm(data[i].id)) {
                ...
                case ELM_OBJ:
                    if (ObjectPtr obj = Proj.gamePath(elm->id))
                        if (C MeshPtr &mesh = obj->mesh()) // use "obj->mesh" instead of "Proj.gamePath(obj_data.mesh_id)" in case the object inherits mesh from base
                        {
                ...
                case ELM_MTRL:
                    if (MaterialPtr mtrl = Proj.gamePath(elm->id)) {
...
#else
                        int variation;
                        Mesh &mesh = getMeshMaterial(mtrl, variation);
                        SetVariation(variation);
                        draw(elm->id, mesh, rect);
                        SetVariation();
#endif
...
void TheaterClass::draw() {
    if (MtrlEdit.bigVisible() || WaterMtrlEdit.bigVisible())
        return; // don't draw if covered by something (without this we would need to set Renderer.temporal_id before Rendering overlapping fullscreen viewports)

    // environment
    EnvironmentPtr env = EnvEdit.cur();
    if (!env)
        env = Proj.gamePath(options.env_id);
    if (!env)
    ...
    Water.draw = false;
    Camera temp = ActiveCam, cam;
    cam.updateBegin().set(MatrixIdentity).updateEnd().updateBegin().updateEnd().set();

    Renderer(Render);
    cleanMeshMaterial(); // clean after rendering finished

    if (options.draw_axis || options.draw_box) {
        if (options.axis_on_top)
            D.depthLock(false);
        else
                ...
                if (options.draw_axis || options.draw_box)
                    if (ObjectPtr obj = Proj.gamePath(elm->id))
                        if (C MeshPtr &mesh = obj->mesh()) {
                            ...
                            if (options.draw_box)
                                OBox(mesh->ext, m).draw();
                            if (options.draw_axis)
                                m.scaleOrn(options.axis_size).draw();
                        }
                break;

            case ELM_ICON:
            case ELM_IMAGE:
                if (ImagePtr image = Proj.gamePath(elm->id)) {
                    Rect image_rect = image->fit(rect);
                    if (image->mode() == IMAGE_CUBE)
                        image->drawCubeFace(WHITE, TRANSPARENT, image_rect, DIR_FORWARD);
                    else
                        image->drawFilter(image_rect);
                }
                break;
            }
        if (litSel()) {
            Color lit = GetLitSelCol(highlighted(i), selected(i), TRANSPARENT);
            if (lit.a) {
                lit.a /= 2;
                rect.draw(lit, false);
                lit.a /= 12;
...
void TheaterClass::create() {
    ...
    Gui += viewport.create(Draw);
    viewport.fov = DegToRad(30);
    Gui += super ::create().slidebarSize(0.06f).skin(&EmptyGuiSkin, false);
    kb_lit = false;
    REPAO(slidebar).skin(&HalfTransparentSkin);
    Gui += mode.create((cchar8 **)null, ELMS(modes)).valid(true).set(0).func(ModeChanged, T);
    REPA(mode)
    mode.tab(i).setText(Plural(ElmTypeName[modes[i]])).desc(S + "Keyboard Shortcut: F" + (i + 1));
    FREPA(modes)
    if (modes[i] == ELM_OBJ) {
        mode.tab(i) += obj_mode.create(t_obj_mode, Elms(t_obj_mode)).hide();
        REPA(obj_mode)
        obj_mode.tab(i).desc(S + "Select with LMB\nTransform with RMB\nKeyboard Shortcut: Shift+F" + (i + 1));
        break;
    }
    Gui += show_options.create(options_t, Elms(options_t));
    show_options.tab(0) += options.create();
    show_options.tab(0).desc("Keyboard Shortcut: F10");
    T += list.create().drawMode(LDM_RECTS).horizontal(true).skin(&EmptyGuiSkin);
        ...
        if (elm.finalExists())
            if (ignore_publish || elm.finalPublish()) // we're going recursively down, from parents to their children, so before checking children, we can only check for modes that will affect all children in the same way, and this is only if element is going to be published
            {
                ...
                if (ElmCompatible(elm.type, elm_type) && this_contains_name) {
                    switch (options.show) {
                    case NON_PUBLISHABLE:
                        if (elm.finalPublish())
                            goto skip;
                        break;
                    case INVALID:
                        if (!Proj.invalidRefs(elm))
                            goto skip;
                        break;
                    }
                    data.New().id = elm.id;
...
VecD2 TheaterClass::offset() C {
    VecD2 offset = 0;
    if (dbl size = list.imageSizeBase().c[!options.horizontal]) {
        dbl scale = (old_rows ? old_rows : options.rows) / size;
        REPA(offset)
        offset.c[i] = slidebar[i].offset() * scale;
    }
    return offset;
}
void TheaterClass::offset(C VecD2 &offset) {
    dbl scale = list.imageSizeBase().c[!options.horizontal];
    scale /= options.rows;
    REPA(offset)
    slidebar[i].offset(offset.c[i] * scale);
}
...
void TheaterClass::refreshNow() {
    ...
    if (refresh & REFRESH_DATA) {
        // save selection
        Memt<UID> sel;
        sel.reserve(list.sel.elms());
        FREPA(list.sel)
        if (ListElm *elm = list.absToData(list.sel[i]))
            sel.binaryInclude(elm->id);

        data.clear();
        if (!Proj.filter_is_id)
            listElms(Proj.root);
        else if (Proj.findElm(Proj.filter_id))
            data.New().id = Proj.filter_id;
        list.setData(data);
    ...
    T.offset(offset);
    old_rows = 0;
    Rect sr = slidebar[0].rect();
    MAX(sr.min.x, Misc.rect().max.x);
    slidebar[0].rect(sr);
...
void TheaterClass::update(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible() && Gui.ms() == &list && Ms.wheel())
        if (Kb.ctrlCmd() || Kb.shift() || Kb.alt()) {
            if (Kb.ctrlCmd())
                options.rowsDelta(-Ms.wheel());
            else {
                if (Kb.shift())
                    options.scaleFactor(Ms.wheel() * 0.1f); // allow processing both alt+shift at the same time
                if (Kb.alt())
                    options.sizeFactor(Ms.wheel() * 0.1f); // allow processing both alt+shift at the same time
            }
            Ms.eatWheel();
        }
    ...
    if (gpc.visible && visible()) {
        ...
        if (Gui.ms() == &list) {
            if (litSel() && modes[mode()] == ELM_OBJ) {
                ...
                if (Ms.bp(1)) // start transforming
                {
                    ...
                    if (!list.sel.has(list.visToAbs(list.lit)))
                        list.sel.clear(); // if highlited is not selected, then clear selection
                    FREPA(list.sel)
                    if (ListElm *elm = list.absToData(list.sel[i]))
                        transform_ids.binaryInclude(elm->id);
                    if (!transform_ids.elms())
                        if (ListElm *elm = list.visToData(list.lit))
                            transform_ids.add(elm->id);
                    transform_rotate_y = 0;
                    transform_rotate_y_frac = 0;
                    transform_scale = 1;
                } else if (Ms.br(1) && transform_ids.elms()) // apply transform
                {
            ...
            if (Ms.bp(0))
                if (ListElm *elm = list()) {
                    if (Ms.bd(0))
                        Proj.elmToggle(elm->id); // call this first, because it may open folders (parents)
            ...
            if (Ms.wheel()) {
                if (Kb.ctrlCmd()) {
                    ...
                    options.scaleFactor(Ms.wheel() * 0.1f);
                    Ms.eatWheel();
                } else if (Kb.alt()) {
                    options.sizeFactor(Ms.wheel() * 0.1f);
                    Ms.eatWheel();
                }
            }
            if (Ms.b(2) || Ms.b(MS_BACK) || Kb.winCtrl()) {
                ...
                if (!(!Kb.ctrlCmd() && Kb.alt()))
                    options.angle.x += Ms.d().x; // disable X rotation when only alt  pressed
                if (!(Kb.ctrlCmd() && !Kb.alt()))
                    options.angle.y += Ms.d().y; // disable Y rotation when only ctrl pressed
                options.toGui();
                rotate = false;
            }
        }
        if (Ms.bp(2) && Gui.window() == &options)
            show_options.toggle(0);

        if (refresh)
            refreshNow();

        if (options.p_angle && options.p_angle->button())
            rotate = false;
        if (rotate)
            options.time_angle += options.rot_speed * Time.ad();

        if (!can_transform || Kb.kf(KB_ESC))
            transform_ids.clear(); // cancel transform
    }
}
void TheaterClass::resize() {
    if (visible()) {
        Rect r = EditRect(false);
        if (rect() != r) {
            rect(r);
            viewport.rect(r);
