...
void VideoOptions::Advanced::TexFilter(Advanced &adv, C Str &text) {
    int i = TextInt(text);
    if (InRange(i, TexFilter_t))
        D.texFilter(TextInt(TexFilter_t[i]));
}
...
void VideoOptions::Advanced::Samples(Advanced &adv, C Str &text) {
    D.samples(TextBool(text) ? 4 : 1);
    VidOpt.setVis();
}
Str VideoOptions::Advanced::Density(C Advanced &adv) {
    int nearest = -1;
    flt dist;
    REPA(Density_t) {
        flt d = Abs(D.density() - TextFlt(Density_t[i]));
        if (nearest < 0 || d < dist) {
            nearest = i;
            dist = d;
        }
...
void VideoOptions::Advanced::Density(Advanced &adv, C Str &text) {
    int i = TextInt(text);
    if (InRange(i, Density_t))
        D.density(TextFlt(Density_t[i]));
}
...
void VideoOptions::Advanced::DensityFilter(Advanced &adv, C Str &text) {
    int i = TextInt(text);
    if (InRange(i, DensityFilter_v))
        D.densityFilter(DensityFilter_v[i]);
}
Str VideoOptions::Advanced::GrassRange(C Advanced &adv) { return D.grassRange(); }
void VideoOptions::Advanced::GrassRange(Advanced &adv, C Str &text) {
    D.grassRange(TextFlt(text));
...
void VideoOptions::Advanced::ToneMap(Advanced &adv, C Str &text) { D.toneMap(TextBool(text)); }
Str VideoOptions::Advanced::Contrast(C Advanced &adv) { return D.contrast(); }
void VideoOptions::Advanced::Contrast(Advanced &adv, C Str &text) { D.contrast(TextFlt(text)); }
...
void VideoOptions::Advanced::skyNightLight(bool on) {
    if (sky_night_light_on != on) {
        ...
        if (sky_night_light)
            sky_night_light->toGui();
    }
}
flt VideoOptions::Advanced::skyNightLightIntensity() C { return sky_night_light_intensity; }
void VideoOptions::Advanced::skyNightLightIntensity(flt intensity) {
    sky_night_light_intensity = intensity;
    if (sky_night_light_on)
        Sky.nightLight(intensity);
}
...
void VideoOptions::Advanced::skyNightLightSchedule(Str text) {
    ...
    if (text.is())
        Split(t, text, ':');
    if (!t.elms())
        sky_night_light_start = -1;
    else // disable
    {
        sky_night_light_start = TextInt(t[0]) * 60;
        if (t.elms() >= 2)
            sky_night_light_start += TextInt(t[1]);
    }
}
...
void VideoOptions::Advanced::skyNightLightUpdate() {
    ...
    bool on = skyNightLightOn(time);
    if ((int)on != sky_night_light_last_check) {
...
void VideoOptions::Advanced::create() {
    int tex_filter = Elms(TexFilter_t);
    FREPA(TexFilter_t)
    if (TextInt(TexFilter_t[i]) > D.maxTexFilter()) {
        tex_filter = i;
        break;
    }
    ...
    super::create("Advanced Video Options");
    autoData(this);
    button[2].show();
}
...
void VideoOptions::Mode(VideoOptions &vo, C Str &t) {
    int m = TextInt(t);
    if (InRange(m, D.modes()))
        D.mode(D.modes()[m].x, D.modes()[m].y);
}
...
void VideoOptions::Shadow(VideoOptions &vo, C Str &t) {
    D.shadowMode(TextBool(t) ? SHADOW_MAP : SHADOW_NONE);
    vo.setVis();
}
...
void VideoOptions::ShadowSize(VideoOptions &vo, C Str &t) {
    int s = TextInt(t);
    if (InRange(s, ShadowSize_t))
        D.shadowMapSize(TextInt(ShadowSize_t[s]));
}
...
void VideoOptions::MotionMode(VideoOptions &vo, C Str &t) {
    D.motionMode(MOTION_MODE(TextInt(t)));
    vo.setVis();
}
...
void VideoOptions::SkinChanged(VideoOptions &vo) {
    if (vo.skin && InRange(vo.skin->combobox(), skins))
        SetGuiSkin(skins[vo.skin->combobox()].id);
}
...
void VideoOptions::setScale(flt scale) {
    T.scale = scale;
    setScale();
}
void VideoOptions::setScaleWin(bool scale) {
    T.scale_win = scale;
    setScale();
}
...
void VideoOptions::resize() {
    if (mode)
        mode->combobox.setText(VecI2(D.resW(), D.resH()), true, QUIET);
    if (full)
        full->set(D.full(), QUIET);
}
...
#if DESKTOP
    mode = &props.New().create("Resolution", MemberDesc().setTextToDataFunc(Mode)).setEnum();
    mode->combobox.setColumns(mode_list_column, Elms(mode_list_column)).setData(ConstCast(D.modes()));
...
#endif
    skin = &props.New().create("Gui Skin", MemberDesc()).setEnum();
    skin->combobox.func(SkinChanged, T).menu.setColumns(theme_list_column, Elms(theme_list_column), true).setData(skins, Elms(skins));
    skin->combobox.text_size *= 0.95f;

    Rect r = super::create("Video Options", Vec2(0.02f, -0.02f), 0.041f, 0.050f, 0.27f);
    autoData(this);
    button[2].show();
    skin->set(skinIndex(Gui.skin.id()), QUIET); // call after 'autoData'
    T += advanced_show.create(Rect_U(clientWidth() / 2, r.min.y - 0.015f, 0.3f, 0.055f), "Advanced").func(ShowAdvanced, T).desc("Keyboard Shortcut: Ctrl+F12");
    advanced_show.mode = BUTTON_TOGGLE;
...
void VideoOptions::setVis() {
    if (shd_siz)
        shd_siz->visible(D.shadowMode() == SHADOW_MAP);
    if (shd_num)
        shd_num->visible(D.shadowMode() == SHADOW_MAP);
    if (shd_sft)
        shd_sft->visible(Renderer.type() == RT_DEFERRED && D.shadowMode() != SHADOW_NONE);
    if (shd_jit)
        shd_jit->visible(D.shadowMode() == SHADOW_MAP);
    if (advanced.diffuse)
        advanced.diffuse->visible(Renderer.type() == RT_DEFERRED);
}
...
void VideoOptions::update(C GuiPC &gpc) {
    super::update(gpc);
    if (visible())
        setTitle(S + "Video Options - " + TextReal(Time.fps(), 1) + " Fps");
