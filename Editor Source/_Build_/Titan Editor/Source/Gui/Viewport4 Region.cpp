...
void Viewport4Region::CamSpherical(Viewport4Region &vr) {
    vr.v4.fpp(!vr.cam_spherical());
    vr.fpp_speed.visible(vr.v4.fpp());
}
void Viewport4Region::CamMode(Viewport4Region &vr) {
    if (vr.cam_spherical.visible())
        vr.cam_spherical.push();
}
...
void Viewport4Region::moveMode(Edit::Viewport4::MOVE_MODE mode) {
    v4.moveMode(mode);
    setMoveMenu();
}
void Viewport4Region::toggleMoveMode(Edit::Viewport4::MOVE_MODE mode) {
    v4.toggleMoveMode(mode);
    setMoveMenu();
}
void Viewport4Region::orientMode(Edit::Viewport4::ORIENT_MODE mode) {
    v4.orientMode(mode);
    setOrientMenu();
}
void Viewport4Region::toggleOrientMode(Edit::Viewport4::ORIENT_MODE mode) {
    v4.toggleOrientMode(mode);
    setOrientMenu();
}
void Viewport4Region::zoom(bool on) {
    v4.drawZoom(on);
    setZoomMenu();
}
void Viewport4Region::toggleZoom() {
    v4.toggleZoom();
    setZoomMenu();
}
...
void Viewport4Region::setMenu(Node<MenuElm> &menu, C Str &prefix) {
    T.prefix = prefix;
    {
        Node<MenuElm> &v = (menu += prefix + "View");
        v.display("View").disabled = true;
...
Viewport4Region &Viewport4Region::create(void (*draw)(Viewport &), bool world_editor, flt default_pitch, flt default_yaw, flt default_dist, flt from, flt range) {
    ...
    super::create(Rect_LU(0, 0, 3, 2)).removeSlideBars().skin(&EmptyGuiSkin, false);
    kb_lit = false;
    flt h = 0.05f; // set initial size so sub-windows can be aligned to sides
    ...
    T += cam_tabs.create(Rect_LU(0, 0, 2 * 0.03f, 2 * 0.03f), 0, (cchar8 **)null, 4).func(CamChanged, T).set(v4.sel(), QUIET);
    cam_tabs.tab(0).rect(Rect(cam_tabs.rect().left(), cam_tabs.rect().up()));
    cam_tabs.tab(1).rect(Rect(cam_tabs.rect().center(), cam_tabs.rect().ru()));
    cam_tabs.tab(2).rect(Rect(cam_tabs.rect().ld(), cam_tabs.rect().center()));
    cam_tabs.tab(3).rect(Rect(cam_tabs.rect().down(), cam_tabs.rect().right()));
    FREPA(cam_tabs)
    cam_tabs.tab(i).setCornerTab(i & 1, !(i & 2)).desc(cam_desc[i]);
    ...
    cam_spherical.mode = BUTTON_TOGGLE;
    cam_spherical.set(true);
    cam_spherical.image = "Gui/Misc/spherical.img";
    T += cam_lock.create(Rect_LU(cam_spherical.rect().ld(), h)).func(CamLocked, T).focusable(false).desc(S + "Lock camera's position and orientation\nKeyboard Shortcut: " + Kb.ctrlCmdName() + "+Tab");
    cam_lock.mode = BUTTON_TOGGLE;
    cam_lock.set(true).hide();
    cam_lock.image = "Gui/Misc/lock.img";
...
void Viewport4Region::update(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        ...
        if (fpp_speed.visible() && Ms.wheel() && v4.focus() && !Kb.ctrlCmd() && !Kb.shift())
            fpp_speed.set(fpp_speed() + Ms.wheel() * 0.1f); // ctrl and shift are reserved for terrain brush params
