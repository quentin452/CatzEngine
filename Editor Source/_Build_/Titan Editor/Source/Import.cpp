...
void ImporterClass::Import::MaterialEx::check(C Str &path, Str &tex) {
    ...
    FREPA(texs)
    if (!FExistSystem(texs[i].name)) {
        Str test = path;
        test.tailSlash(true) += GetBaseNoExt(texs[i].name);
        test += '.';
        Str test_ext = test + "webp";
        if (FExistSystem(test_ext))
            texs[i].name = test_ext;
        test_ext = test + "avif";
        if (FExistSystem(test_ext))
            texs[i].name = test_ext;
        test_ext = test + "jxl";
...
void ImporterClass::Import::MaterialEx::process(C Str &path) {
    ...
    if (GetExt(color_map) == "img" || GetExt(normal_map) == "img" || GetExt(smooth_map) == "img" || GetExt(detail_color_map) == "img") // this is 'EE.Material' ("mtrl" format) #MaterialTextureLayout
    {
        ...
        base_0.load(base0_path);
        ImageProps(base_0, &base_0_id, null, MTRL_BASE_0);
        base_1.load(base1_path);
        ImageProps(base_1, &base_1_id, null, MTRL_BASE_1);
        base_2.load(base2_path);
        ImageProps(base_2, &base_2_id, null, MTRL_BASE_2);
        detail.load(detail_path);
        ImageProps(detail, &detail_id, null, MTRL_DETAIL);
        macro.load(macro_path);
        ImageProps(macro, &macro_id, null, MTRL_MACRO);
        emissive_img.load(emissive_path);
        ImageProps(emissive_img, &emissive_id, null, MTRL_EMISSIVE);
        ...
        if (base1_path.is()) {
            fp = base1_path;
            fp.params.New().set("channel", "xy");
            normal_map = fp.encode();
        } else {
            normal_map.clear();
        }
        ...
        if (base2_path.is()) {
            fp = base2_path;
            fp.params.New().set("channel", "y");
            fp.params.New().setName("inverseRGB");
            smooth_map = fp.encode();
            fp = base2_path;
            fp.params.New().set("channel", "x");
            metal_map = fp.encode();
            fp = base2_path;
            fp.params.New().set("channel", "z");
            bump_map = fp.encode();
            fp = base2_path;
            fp.params.New().set("channel", "w");
            glow_map = fp.encode();
        } else {
            smooth_map.clear();
            metal_map.clear();
        ...
        if (detail_path.is()) // #MaterialTextureLayoutDetail
        {
            fp = detail_path;
            fp.params.New().set("channel", "w");
            detail_color_map = fp.encode();
            fp = detail_path;
            fp.params.New().set("channel", "z");
            fp.params.New().setName("inverseRGB");
            detail_smooth_map = fp.encode();
            fp = detail_path;
            fp.params.New().set("channel", "xy");
            detail_normal_map = fp.encode();
        } else {
        ...
        IMAGE_TYPE ct;
        ImageProps(base_0, &base_0_id, &ct, MTRL_BASE_0);
        if (Importer.includeTex(base_0_id))
            base_0.copy(base_0, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
        else
            base_0.del();
        ImageProps(base_1, &base_1_id, &ct, MTRL_BASE_1);
        if (Importer.includeTex(base_1_id))
            base_1.copy(base_1, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
        else
            base_1.del();
        ImageProps(base_2, &base_2_id, &ct, MTRL_BASE_2);
        if (Importer.includeTex(base_2_id))
            base_2.copy(base_2, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
        else
            base_2.del();
        ImageProps(emissive_img, &emissive_id, &ct, MTRL_EMISSIVE);
        if (Importer.includeTex(emissive_id)) {
            SetFullAlpha(emissive_img, ct);
            emissive_img.copy(emissive_img, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
        } else
            emissive_img.del();
    }
}
Str ImporterClass::Import::nodeName(int bone_i) C {
    ...
    if (InRange(bone_i, skel.bones))
        return skel.bones[bone_i].name; // use 'skel'
    return S;
}
Str ImporterClass::Import::nodeUID(int bone_i) C // unique string identifying bone's node !! needs to be the same as 'EditSkeleton.nodeUID' !!
{
    ...
    if (InRange(bone_i, xskel.bones)) // use 'xskel' if available
    {
        int node_i = xskel.bones[bone_i].node;
        Memt<int> parents;
        for (; InRange(node_i, xskel.nodes) && parents.include(node_i);) {
            C XSkeleton::Node &node = xskel.nodes[node_i];
            uid += node.name; // node name
            int parent = node.parent, child_index = 0;
            REP(node_i)
            if (xskel.nodes[i].parent == parent && xskel.nodes[i].name == node.name)
                child_index++;
            if (child_index) {
                uid += CharAlpha;
                uid += child_index;
    ...
    } else // use 'skel'
    {
        for (; InRange(bone_i, skel.bones);) {
            ...
            int parent = skel.boneParent(bone_i), child_index = 0;
            REP(bone_i)
            if (skel.boneParent(i) == parent && nodeName(i) == node_name)
                child_index++;
            if (child_index) {
                uid += CharAlpha;
                uid += child_index;
...
::ImporterClass::Import &ImporterClass::Import::set(C UID &elm_id, C UID &parent_id, C Str &file, MODE mode, ELM_TYPE type, C Str &force_name, bool remember_result) {
    ...
    T.mode = mode;
    T.type = type;
    T.force_name = force_name;
    T.remember_result = remember_result;
    switch (type) {
    case ELM_FONT:
        edit_font.load(Proj.editPath(elm_id));
        break;
    ...
    case ELM_IMAGE: // check for skybox
    {
        ...
        if (files.elms() == 6) {
            ...
            UID image_id;
            REPA(files)
            if (DecodeFileName(files[i].name, image_id))
                if (src.read(Proj.editPath(image_id))) {
                    ImageEx &image = images(i);
                    src.copy(image.raw.writeMem());
                    if (Elm *elm = Proj.findElm(image_id))
                        if (ElmImage *data = elm->imageData())
                            image.cube = IsCube(data->mode);
                }

            // check if any of the images were not imported
            if (src.read(Proj.editPath(elm_id)))
                src.copy(raw.writeMem());
        }
    } break;
    ...
    case ELM_OBJ: // check if we want to ignore animations
    {
        if (mode == UPDATE // check only for UPDATE because others treat 'elm_id' as target
            && Proj.objToSkel(elm_id).valid())
            ignore_anims = true; // if object that's being reloaded already had a skeleton set, then ignore loading animations because they were already imported before
    } break;
    }
    return T;
}
bool ImporterClass::Import::ApplyVolume(ptr data, int size, flt vol) {
    if (data) {
        if (size & 1)
            return false; // need 2 byte alignment for a full sample
...
bool ImporterClass::Import::DownMix(ptr data, int &size, DOWNMIX downmix) {
    if (!downmix || !data)
        return true;
    if (size & 1)
        return false; // need 2 byte alignment for a full sample
...
bool ImporterClass::Import::import() // !! this is called on a secondary thread !!
{
    ...
    case ELM_SOUND: {
        ...
        dbl start = 0;
        if (C TextParam *param = files[0].findParam("start"))
            start = param->asDbl();
        dbl end = -1;
        if (C TextParam *param = files[0].findParam("end"))
            end = param->asDbl();
        dbl length = -1;
        if (C TextParam *param = files[0].findParam("length"))
            length = param->asDbl();
        flt volume = 1;
        if (C TextParam *param = files[0].findParam("volume"))
            volume = param->asFlt();
        bool apply_vol = !Equal(volume, 1);
        int hz = -1;
        if (C TextParam *param = files[0].findParam("hz"))
            hz = param->asInt();
        DOWNMIX downmix = DOWNMIX_NO;
        if (C TextParam *param = files[0].findParam("mono")) {
            if (param->value == "l" || param->value == "left")
                downmix = DOWNMIX_LEFT;
            else if (param->value == "r" || param->value == "right")
                downmix = DOWNMIX_RIGHT;
            else
                downmix = DOWNMIX_MIX;
        }
        int rel_bit_rate = -1;
        ...
        if (codec || rel_bit_rate >= 0 || start > 0 || end >= 0 || length >= 0 || hz > 0 || apply_vol || downmix) {
            SoundStream s;
            if (s.create(file) && s.block()) {
                long start_sample = ((start > 0) ? RoundL(start * s.frequency()) : 0);
                MIN(start_sample, s.samples());
                long end_sample = ((end >= 0) ? RoundL(end * s.frequency()) : s.samples());
                MIN(end_sample, s.samples());
                long length_sample = Max(0, end_sample - start_sample);
                if (length >= 0)
                    MIN(length_sample, RoundL(length * s.frequency()));
                if (!codec)
                    codec = s.codec(); // if not specified then use original
                if (hz <= 0)
                    hz = s.frequency();
                if (s.channels() != 2)
                    downmix = DOWNMIX_NO;
                ...
                int bit_rate = -1;
                if (rel_bit_rate > 0) // calculate from relative bit rate
                {
                    bit_rate = long(rel_bit_rate * 1000) * s.frequency() / 44100 * channels / 2;
                } else if (rel_bit_rate < 0 && (Equal(s.codecName(), "opus") || Equal(s.codecName(), "vorbis"))) // get from source (accept only from good lossy compressed codecs, because Raw/Flac will have very big values)
                {
                    bit_rate = s.bitRate();
                }
                if (codec && s.codecName() != CodecName(codec)                // compare names instead of value, to ignore audio container and just check codec, because names always return just the codec
                    ...
                    || downmix) {
                    if (codec != SOUND_WAV && !Equal(CodecName(codec), "opus") && !Equal(CodecName(codec), "vorbis"))
                    ...
                    if (bit_rate <= 0) // use default bit rate
                    {
                        if (Equal(CodecName(codec), "opus"))
                            bit_rate = long(DefaultOpusBitRate) * s.frequency() / 44100 * channels / 2;
                        else if (Equal(CodecName(codec), "vorbis"))
                            bit_rate = long(DefaultVorbisBitRate) * s.frequency() / 44100 * channels / 2;
                    }
                    int bit_rate_hq = Max(long(DefaultHQBitRate) * s.frequency() / 44100 * channels / 2, bit_rate);

                    raw.writeMem();
                    byte temp[65536];
                    if (!s.pos(start_sample * s.block()))
                        goto error;
                    long size = length_sample * s.block();
                    if (codec == SOUND_WAV) {
                        SndRawEncoder encoder;
                        if (encoder.create(raw, hz, channels, length_sample)) {
                            for (; size > 0;) {
                                int r = s.set(temp, Min(size, (int)SIZE(temp)));
                                if (r <= 0)
                                    goto error;
                                size -= r;
                                if (downmix && !DownMix(temp, r, downmix))
                                    goto error;
                                if (apply_vol && !ApplyVolume(temp, r, volume))
                                    goto error;
                                if (!encoder.encode(temp, r))
                                    goto error;
                            }
                            if (!encoder.finish())
                                goto error;
                    ...
                    } else if (Equal(CodecName(codec), "opus")) {
                        SndOpusEncoder encoder;
                        if (encoder.create(raw, length_sample, hz, channels, bit_rate_hq)) // prioritize first frame (to avoid pop/clicks for looped audio)
                        {
                            ...
                            for (; size > 0;) {
                                ...
                                if (first)
                                    MIN(r, frame_size); // make sure we encode only the first frame with higher bit rate
                                r = s.set(temp, r);
                                if (r <= 0)
                                    goto error;
                                size -= r;
                    ...
                    } else if (Equal(CodecName(codec), "vorbis") && channels && s.frequency()) {
                        ...
                        if (encoder.create(raw, hz, channels, quality)) {
                            for (; size > 0;) {
                                int r = s.set(temp, Min(size, (int)SIZE(temp)));
                                if (r <= 0)
                                    goto error;
    ...
    case ELM_ANIM: {
        if (GetExt(file) == "anim") // EE animation
        {
            ...
            if (xanim.anim.load(file)) {
                xanim.name = GetBaseNoExt(file);
                return true;
            }
    ...
    case ELM_IMAGE: {
        if (files.elms() == 6) // special case of cube map
        {
            ...
            FREPA(files) {
                ImageEx &image = images(i);
                if (!ImportImage(image, files[i].name)) {
                    image.raw.pos(0);
                    if (image.Import(image.raw)) {
                        if (image.cube)
                            image.crop(image, i * image.w() / 6, 0, image.w() / 6, image.h()); // crop to i-th face for cubes
                    }
                }
            ...
            if (!images[i].is()) {
                Image src;
                raw.pos(0);
                if (src.Import(raw)) // try extracting from existing data
                {
                    ...
                    if (!images[i].is())
                        if (one)
                            ...
                            src.crop(images[i], i * src.w() / 6, 0, src.w() / 6, src.h());
                }
                break;
            }
            ...
            int size = 0;
            REPA(images)
            MAX(size, Max(images[i].w(), images[i].h()));

            // if actually has some images
            if (size > 0) {
                Image dest;
                if (dest.createSoft(size * 6, size, 1, IMAGE_R8G8B8A8_SRGB)) // create soft RGBA so we can use simple mem copy
                {
            ...
            {
                Image image;
                if (LoadImages(null, image, null, T.file)) // proj null because this is called on secondary thread
                {
                    image.ExportWEBP(raw.writeMem(), 1, 1);
                    has_color = HasColor(image);
                    has_alpha = HasAlpha(image);
                    return true;
                }
...
void ImporterClass::ImportElm::set(C UID &elm_id, bool remember_result) {
    T.elm_id = elm_id;
    T.remember_result = remember_result;
}
...
void ImporterClass::ImportFull(ImporterClass &ic) {
    Proj.drop(ic.import_files, ic.import_target);
    ic.import_files.clear();
}
void ImporterClass::ImportReplace(ImporterClass &ic) {
    if (ic.import_files.elms())
        if (Elm *elm = Proj.findElm(ic.import_target)) {
            elm->setSrcFile(ic.import_files[0]);
            Proj.elmReload(elm->id);
...
void ImporterClass::importSpecialFile(C Str &file) {
    Str ext = GetExt(file);
    if (ExtType(ext) == EXT_MESH)
        importNew(import_target, import_target, file, import_mode, ELM_OBJ);
    else if (ext == "anim")
        importNew(import_target, import_target, file, import_mode, ELM_ANIM);
}
void ImporterClass::importSpecialDir(C Str &path) {
    for (FileFind ff(path); ff();)
        switch (ff.type) {
        case FSTD_FILE:
            importSpecialFile(ff.pathName());
            break;
        case FSTD_DIR:
            importSpecialDir(ff.pathName());
            break;
        }
}
void ImporterClass::importSpecial(MODE mode) {
    ...
    FREPA(import_files) {
        FileInfo fi;
        if (fi.getSystem(import_files[i]))
            switch (fi.type) {
            case FSTD_FILE:
                importSpecialFile(import_files[i]);
                break;
            case FSTD_DIR:
                importSpecialDir(import_files[i]);
                break;
            }
    }
}
...
::ImporterClass::Import *ImporterClass::findImport(C UID &elm_id) {
    REPA(imports) {
        ...
        if (import.mode == UPDATE && import.elm_id == elm_id)
            return &import;
    }
    return null;
}
::ImporterClass::ImportElm *ImporterClass::findImportQueue(C UID &elm_id) {
    FREPA(import_queue)
    if (import_queue[i].elm_id == elm_id)
        return &import_queue[i];
    return null;
}
...
void ImporterClass::cancelImports(C MemPtr<UID> &sorted_elm_ids) {
    REPA(imports) // check all 'imports' in case there are multiple imports for the same element
    {
        ...
        if (import.mode == UPDATE    // only this mode operates on 'elm_id' while others treat it as target
            ...
            if (threads.cancel(import, ImportDo, T))
                import.status = 0; // if succesfully canceled, then mark as finished, otherwise we need to wait until it will finish on its own
            import.cancel = true;
            if (import.remember_result)
                *import_results(import.elm_id) = Edit::RELOAD_CANCELED; // if wanted to remember result, then set as canceled
        }
    }
}
...
void ImporterClass::getResult(C MemPtr<UID> &elms, MemPtr<Edit::IDParam<Edit::RELOAD_RESULT>> results) {
    ...
    FREPA(results) {
        ...
        result.id = elms[i];
        if (Edit::RELOAD_RESULT *r = import_results.find(result.id))
            result.value = *r;
        else {
            C Elm *elm = Proj.findElm(result.id);
            if (!elm)
                result.value = Edit::RELOAD_ELM_NOT_FOUND;
            else if (elm->importing())
                result.value = Edit::RELOAD_IN_PROGRESS;
            else
...
void ImporterClass::clearImportResults(C MemPtr<UID> &elm_ids) {
    REPA(elm_ids)
    import_results.removeKey(elm_ids[i]);
}
    ...
    if (elm_ids.elms()) {
        ...
        REPA(imports) {
            ...
            if (import.mode == UPDATE     // only this mode operates on 'elm_id' while others treat it as target
                ...
                && sorted.binaryHas(import.elm_id))
                import.remember_result = false;
        }
        REPA(import_queue) {
            ImportElm &import = import_queue[i];
            if (import.remember_result && sorted.binaryHas(import.elm_id))
                import.remember_result = false;
        }
...
void ImporterClass::reload(C MemPtr<UID> &elm_ids, bool remember_result) {
    ...
    cancelImports(sorted);
    if (remember_result)
        clearImportResults(sorted); // since we'll reload elements, we need to clear any previous results, do this after calling 'cancelImports' which may set RELOAD_CANCELED
    REPA(elm_ids)                   // add in reversed order because elements last in the queue are imported first
    {
        C UID &elm_id = elm_ids[i];
        if (ImportElm *elm = findImportQueue(elm_id))
            elm->remember_result |= remember_result;
        else
            import_queue.New().set(elm_id, remember_result);
    }
}
void ImporterClass::investigate(Elm &elm) {
    if (elm.importing() && !inQueue(elm.id))
        import_queue.New().set(elm.id);
}
...
void ImporterClass::investigate(Memc<UID> &elm_ids) {
    if (!Proj.needUpdate()) {
        SyncLocker locker(lock);
        FREPA(elm_ids)
        if (Elm *elm = Proj.findElm(elm_ids[i]))
            if (ImportRemovedElms || elm->finalExists()) {
                int index = Proj.elms.validIndex(elm);
                if (InRange(index, Proj.hierarchy)) {
...
void ImporterClass::processUpdate(Import &import) {
    if (Elm *elm = Proj.findElm(import.elm_id)) {
        ...
        switch (elm->type) {
        case ELM_MTRL:
            if (import.mtrls.elms()) {
                FileParams fp = import.file;
                fp.getParam("name").value = import.mtrls[0].name;
        ...
        case ELM_IMAGE: {
            ...
            import.raw.pos(0);
            Proj.imageSet(elm->id, import.raw, import.has_color, import.has_alpha);
        } break;
        ...
        case ELM_FILE: {
            elm->setSrcFile(import.file, time);
            import.raw.pos(0);
            Proj.fileSet(elm->id, import.raw);
        } break;
        ...
        case ELM_FONT: {
            ...
            elm->fontData()->newVer();
            Save(import.font, Proj.gamePath(*elm));
            Proj.savedGame(*elm);
            Proj.elmChanged(*elm);
            Server.setElmLong(elm->id);
        } break;
        ...
        case ELM_ANIM: {
            if (import.anims.elms())
                if (ElmAnim *anim_data = elm->animData()) {
                    ...
                    {
                        anim_data->loop(anim.loop());
                        anim_data->loop_time = time; // set from imported animation
                    } else
                        anim.loop(anim_data->loop()); // otherwise keep the old setting
                    ...
                    if (!anim.events.elms()) // if new animation doesn't have events, then check old anim for them
                    {
                        ...
                        {
                            Animation temp;
                            if (Animation *anim = Proj.getAnim(elm->id, temp))
                                old.loop(anim->loop()).linear(anim->linear()).length(anim->length(), false).events = anim->events; // we just need events
                        }
                        if (old.events.elms()) // if old had events
                        {
                            ...
                            if (old_fps) {
                                ...
                                if (C TextParam *p = old_fps->findParam("offsetTime"))
                                    old.offsetTime(p->asFlt());
                                if (anim_data->fps > 0)
                                    if (C TextParam *p = old_fps->findParam("offsetFrame"))
                                        old.offsetTime(p->asFlt() / anim_data->fps);
                                ...
                                REPA(old_fps->params) // process in reversed order
                                {
                                    ...
                                    if (p.name == "speedTime") {
                                        Vec v = p.asVec();
                                        flt speed = v.x, start = v.y, end = v.z;
                                        if (speed)
                                            old.scaleTime(start, end ? start + (end - start) / speed : old.length(), speed);
                                    } else if (p.name == "speedFrame") {
                                        if (anim_data->fps > 0) {
                                            Vec v = p.asVec();
                                            flt speed = v.x, start = v.y / anim_data->fps, end = v.z / anim_data->fps;
                                            if (speed)
                                                old.scaleTime(start, end ? start + (end - start) / speed : old.length(), speed);
                                        }
                                    }
                            ...
                            if (anim_data->fps > 0) {
                                flt offset = 0;
                                if (old_fps)
                                    if (C TextParam *start_frame = old_fps->findParam("startFrame"))
                                        offset += start_frame->asFlt() / anim_data->fps - xanim.start; // currently importer will already offset keyframes by 'anim.start', so if we want custom ranges, we need to revert it back
                                if (fps)
                                    if (C TextParam *start_frame = fps->findParam("startFrame"))
                                        offset -= start_frame->asFlt() / anim_data->fps - xanim.start; // currently importer will already offset keyframes by 'anim.start', so if we want custom ranges, we need to revert it back
                                if (offset)
                            ...
                            REPA(anim.events)
                            Clamp(anim.events[i].time, 0, anim.length());
                        }
                    }
                    ...
                    if (fps) // 'speedTime' !! BEFORE ROOT !!
                    {
                        FREPA(fps->params) // process in order
                        {
                            ...
                            if (p.name == "speedTime") // adjust speed for specified time range !! do this after clipping so the clip isn't affected by speed !!
                            {
                                ...
                                flt speed = v.x, start = v.y, end = v.z;
                                if (speed)
                                    anim.scaleTime(start, end ? end : anim.length(), 1 / speed); // if no end specified then use until end of anim
                            } else if (p.name == "speedFrame") {
                                if (anim_data->fps > 0) {
                                    Vec v = p.asVec();
                                    flt speed = v.x, start = v.y / anim_data->fps, end = v.z / anim_data->fps;
                    ...
                    if (Elm *skel_elm = Proj.findElm(anim_data->skel_id))
                        if (ElmSkel *skel_data = skel_elm->skelData()) // load target skeleton
                        {
                        ...
                        This is only for few special cases where Animation FBX files have more bones detected than the base Mesh+Skel FBX */
                                ...
                                REPA(import.skel.bones) {
                                    ...
                                    if (node >= 0)
                                        *skel_to_node(import.skel.bones[i].name) = node;
                                    else
                                        remove = true; // set only those that were found, others will be removed below
                                }
                                if (remove) // if we need to remove some bones
                                {
                                    Skeleton temp = import.skel;
                                    REPA(temp.bones)
                                    if (!skel_to_node.find(temp.bones[i].name))
                                        temp.removeBone(i);                                              // if this bone is not present in 'EditSkeleton' then remove it
                                ...
                                // convert to EditSkeleton from nodes (with bone names set as node index)
                                {
                                    ...
                                    REPA(import.skel.bones) {
                                        SkelBone &bone = import.skel.bones[i];
                                        Set(bone.name, TextInt(*skel_to_node(bone.name)));
                                    }
                                    REPA(anim.bones) {
                                ...
                                if (fps) {
                                    ...
                                    FREPA(fps->params) {
                                        C TextParam &p = fps->params[i];
                                        if (p.name == "rotX")
                                            anim.transform(Matrix().setRotateX(DegToRad(p.asFlt())), sk, true);
                                        else if (p.name == "rotY")
                                            anim.transform(Matrix().setRotateY(DegToRad(p.asFlt())), sk, true);
                                        else if (p.name == "rotZ")
                                            anim.transform(Matrix().setRotateZ(DegToRad(p.asFlt())), sk, true);
                                    }
                                }
                                {
                                    Mems<Mems<IndexWeight>> weights;
                                    edit_skel.set(weights, import.skel, *skel, EditSkeleton::BONE_NAME_IS_NODE_INDEX);
                                    ...
                                    if (anim_data->flag & ElmAnim::ROOT_FROM_BODY) // if want from body, then override 'root' bone index, and set custom
                                    {
                                        ...
                                        if (InRange(bone, skel->bones))
                                            root = edit_skel.boneToNode(skel->bones[bone].name);
                                    }
                            ...
                            if (fps) { // 'offsetTime' !! AFTER ROOT !!
                                if (C TextParam *p = fps->findParam("offsetTime"))
                                    anim.offsetTime(-p->asFlt());
                                if (anim_data->fps > 0)
                                    if (C TextParam *p = fps->findParam("offsetFrame"))
                                        anim.offsetTime(-p->asFlt() / anim_data->fps);
                            }
                            // optimize (after transform because scale affects position key removal, after 'adjustForSameTransformWithDifferentSkeleton' so it can operate on highest precision and to cleanup keyframes generated by it, after 'offsetTime' to cleanup keyframes generated by it)
                            {
                                ...
                                if (fps)
                                    if (C TextParam *optimize = fps->findParam("optimize")) {
                                        flt o = optimize->asFlt();
                                        angle_eps *= o;
                                        pos_eps *= o;
                                        scale_eps *= o;
                                    }
                            ...
                            if (fps) {
                                ...
                                // mirror
                                if (C TextParam *mirror = fps->findParam("mirror"))
                                    if (mirror->asBool1())
                                        anim.mirror(*skel);

                                // delete keys at the end !! after clipping and root !!
                                if (C TextParam *p = fps->findParam("delEndKeys"))
                                    DelEndKeys(anim);
                            }
                        }
        ...
        case ELM_OBJ:
            if (ElmObj *obj_data = elm->objData()) {
                ...
                if (new_skel.is())
                    Proj.getObjSkelElm(elm->id, false, false); // need to have skeleton element to insert skeleton
                ...
                if (Elm *mesh_elm = Proj.findElm(obj_data->mesh_id))
                    if (ElmMesh *mesh_data = mesh_elm->meshData()) {
                        ...
                        if (new_skel.is()) // only if there's a new skeleton, otherwise keep the current
                            if (skel_elm = Proj.findElm(mesh_data->skel_id))
                                if (ElmSkel *skel_data = skel_elm->skelData()) {
                                    ...
                                    EditSkeleton old_edit_skel, new_edit_skel;
                                    old_edit_skel.load(skel_edit_path);
                                    new_edit_skel.create(new_skel, &import.xskel); // !! create before transform is applied !!
                                    ...
                                    bone_weights.setNum(new_skel.bones.elms());
                                    {
                                        // slots
                                        new_skel.slots = old_skel->slots;
                                        REPA(new_skel.slots) {
                                        ...
                                        REPAD(new_skel_bone_i, new_skel.bones) {
                                            int new_node_i = new_edit_skel.boneToNode(new_skel_bone_i);
                                            if (new_node_i >= 0) {
                                                int old_node_i = old_edit_skel.findNodeI(new_edit_skel.nodes[new_node_i].name, new_edit_skel.nodeUID(new_node_i));
                                                if (old_node_i >= 0) {
                                                    int best_old_skel_bone_i = -1;
                                                    flt weight;
                                                    REPAD(b, old_edit_skel.bones) {
                                                        C EditSkeleton::Bone &old_edit_skel_bone = old_edit_skel.bones[b];
                                                        REPAD(bw, old_edit_skel_bone) {
                                                            C IndexWeight &iw = old_edit_skel_bone[bw];
                                                            if (iw.index == old_node_i) {
                                                                int old_skel_bone_i = old_skel->findBoneI(old_edit_skel_bone.name);
                                                                if (old_skel_bone_i >= 0) {
                                                                    bone_weights[new_skel_bone_i].New().set(old_skel_bone_i, iw.weight);
                                                                    if (best_old_skel_bone_i < 0 || iw.weight > weight) {
                                                                        best_old_skel_bone_i = old_skel_bone_i;
                                                                        weight = iw.weight;
                                                                    }
                                                                }
                                                            }
                                    ...
                                    Save(new_skel, skel_game_path);
                                    Proj.savedGame(*skel_elm, skel_game_path);
                                    ...
                                    FREPA(import.anims) {
                                        ...
                                        Save(xanim.anim, Proj.gamePath(anim));
                                        Proj.savedGame(anim);
                        ...
                        FREPA(import.mtrls) {
                            ...
                            fp.getParam("name").value = src.name;
                            Str mtrl_src_file = fp.encode();
                        ...
                        int pmi = 0;
                        FREPD(l, import.mesh.lods()) // set materials
                        {
                            MeshLod &lod = import.mesh.lod(l);
                            FREPA(lod) {
                                if (InRange(pmi, import.part_mtrl_index)) {
                                    int mtrl_index = import.part_mtrl_index[pmi];
                                    if (InRange(mtrl_index, mtrls))
                                        lod.parts[i].material(Proj.gamePath(mtrls[mtrl_index]));
                                }
                                pmi++;
                            }
                        }
                        ...
                        Mesh old_mesh;
                        if (Load(old_mesh, edit_path, Proj.game_path))
                            KeepParams(old_mesh, import.mesh);
                        ...
                        Mesh game_mesh;
                        EditToGameMesh(import.mesh, game_mesh, body_skel, Proj.getEnum(mesh_data->draw_group_id), &NoTemp(mesh_data->transform()));
                        Save(game_mesh, Proj.gamePath(mesh_elm->id));
                        Proj.savedGame(*mesh_elm); // save
            ...
            if (ElmObj *obj_data = obj_elm->objData())
                if (Elm *body_elm = Proj.findElm(obj_data->mesh_id))
                    if (ElmMesh *body_mesh_data = body_elm->meshData()) {
                        if (Elm *parent = Proj.findElm(import.parent_id))
                            parent->opened(true);
                        ...
                        if (Elm *skel_elm = Proj.findElm(mesh_data->skel_id))
                            if (ElmSkel *skel_data = skel_elm->skelData())
                                if (C Skeleton *skel = Skeletons(Proj.gamePath(skel_elm->id))) {
                                    FileParams fp = import.file;
                                    EditSkeleton edit_skel;
                                    edit_skel.load(Proj.editPath(skel_elm->id)); // 'edit_skel' is always in identity matrix
                                    Matrix m = skel_data->transform();
                                    ...
                                    FREPA(import.anims) {
                                        ...
                                        if (!import.force_name.is() && import.anims.elms() == 1 && (xanim.name == "Take 01" || xanim.name == "Take 001" || xanim.name == "Default Take" || xanim.name == "C4D Animation Take" || xanim.name == "mixamo.com" || xanim.name == "Unreal Take" || xanim.name == "Scene" || xanim.name == "Base Stack")) {
                                            ...
                                            REPA(import.force_name)
                                            if (import.force_name[i] == '@') // check for unity style file names (one anim per file, name "take 01" or "take 001"), file name "Forest Wolf@Bite Attack.FBX"
                                            {
                                        ...
                                        // create element
                                        Elm &anim = Proj.Project::newElm(import.force_name.is() ? import.force_name : xanim.name.is() ? xanim.name
                                                                                                                                      : GetBaseNoExt(FileParams(import.file).name),
                                                                         import.parent_id, ELM_ANIM);
                                        if (ElmAnim *anim_data = anim.animData()) {
                                            fp.getParam("name").value = xanim.name;
                                            anim_data->newData();
                                            anim_data->skel_id = skel_elm->id;
                                        ...
                                        // save and send
                                        Save(xanim.anim, Proj.gamePath(anim.id));
                                        Proj.savedGame(anim);
                                        Server.setElmFull(anim.id);
                                    }
                                    Proj.setList();
            ...
            if (ElmObj *obj_data = elm->objData()) {
                ...
                if (Elm *mesh_elm = Proj.findElm(obj_data->mesh_id))
                    if (ElmMesh *mesh_data = mesh_elm->meshData()) {
                        ...
                        Skeleton *mesh_skel, *body_skel;
                        Proj.getMeshSkels(mesh_data, &mesh_skel, &body_skel);
                        if (mesh_skel && import.skel.is()) {
                            EditSkeleton edit_skel;
                            if (edit_skel.load(Proj.editPath(mesh_data->skel_id))) {
                                MemtN<BoneType, 256> old_to_new;
                                old_to_new.setNum(import.skel.bones.elms());
                    ...
                    bone_map_set:;
                        ...
                        Str edit_path = Proj.editPath(mesh_elm->id);
                        Mesh mesh;
                        Load(mesh, edit_path, Proj.game_path);   // load current mesh
                        ...
                        // game mesh
                        Mesh game_mesh;
                        EditToGameMesh(mesh, game_mesh, body_skel, Proj.getEnum(mesh_data->draw_group_id), &NoTemp(mesh_data->transform()));
                        Save(game_mesh, Proj.gamePath(mesh_elm->id));
                        Proj.savedGame(*mesh_elm); // save
                        mesh_data->from(game_mesh);
...
void ImporterClass::update() {
    ...
    REPA(imports) {
        Import &import = imports[i];
        if (import.status >= 0) // finished importing
        {
            if (!import.cancel) {
                if (import.remember_result)
                    *import_results(import.elm_id) = (import.status ? Edit::RELOAD_SUCCESS : Edit::RELOAD_FAILED);
                if (import.status)
                    switch (import.mode) // ok
                    {
                    case UPDATE:
                        processUpdate(import);
                        break;
                    case ANIM:
                        processAnim(import);
                        break;
