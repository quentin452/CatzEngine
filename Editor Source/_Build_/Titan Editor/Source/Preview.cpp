...
void PreviewClass::render() {
    ...
    case RM_PREPARE: {
        ...
        case ELM_MESH:
            if (mesh) {
                ...
                mesh->MeshLod::draw(MatrixIdentity);
                SetVariation();
            }
            break;
        ...
        case ELM_ANIM:
            if (mesh)
                mesh->MeshLod::draw(anim_skel);
            break;
        }

        if (elm_type == ELM_WATER_MTRL)
            LightDir(Matrix3().setRotateXY(PI_4, PI_4).z, 1 - D.ambientColorL()).add(false);
        else
    ...
    if (Renderer() == RM_BLEND) {
        if (elm_type == ELM_OBJ && body)
            body->drawBlend(MatrixMIdentity, &NoTemp(Vec4(1, 1, 1, 0.333f)));
        if (elm_type == ELM_IMAGE && image && image->mode() == IMAGE_3D)
...
void PreviewClass::draw(Elm &elm, C Rect &rect) {
    ...
    Camera c = ActiveCam;
    Rect r = D.viewRect();
    flt fov = D.viewFov(), from = D.viewFrom(), range = D.viewRange(); // remember
    ...
    case ELM_OBJ: {
        if (ElmObj *obj_data = elm.objData())
            if (obj = Proj.gamePath(elm.id)) {
                ...
                D.viewFrom(dist * 0.01f).viewRange(dist * 5);
                if (mesh)
                    SetCam(cam, mesh->ext, Time.appTime(), (mesh->ext.size().y < mesh->ext.size().max() * 0.3f) ? -PI_4 : 0); // set
                Renderer(PreviewClass::Render);
                D.viewRect(r).viewFov(fov).viewFrom(from).viewRange(range);
                c.set(); // restore
    ...
    case ELM_MTRL: {
        ...
        D.viewFrom(dist * 0.01f).viewRange(dist * 3);
        SetCam(cam, ball.ext, Time.appTime() / 2); // set
        Renderer(PreviewClass::Render);
        D.viewRect(r).viewFov(fov).viewFrom(from).viewRange(range);
        c.set(); // restore
        rect.draw(rect_color, false);
    ...
    case ELM_WATER_MTRL: {
        ...
        if (water_mtrl = Proj.gamePath(elm.id))
            SCAST(WaterMtrl, Water) = *water_mtrl;

        D.viewRect(rect).viewFov(PreviewFOV).viewFrom(0.1f).viewRange(100);
        SetCam(cam, Box(5), 0, -PI_6); // set
        Renderer(PreviewClass::Render);
        D.viewRect(r).viewFov(fov).viewFrom(from).viewRange(range);
        c.set(); // restore
        Swap(temp, SCAST(WaterMtrl, Water));
    ...
    case ELM_ANIM: {
        ...
        if (!mesh_id.valid())
            if (Elm *obj = Proj.firstParent(&elm, ELM_OBJ))
                if (ElmObj *obj_data = obj->objData())
                    mesh_id = obj_data->mesh_id; // get mesh from firstObjParent->mesh
        if (mesh_id.valid()) {
            anim = Animations(Proj.gamePath(elm.id));
            mesh = Proj.gamePath(mesh_id);
            skel = mesh->skeleton();
            if (skel) {
                if (anim_skel.skeleton() != skel || anim_skel.bones.elms() != skel->bones.elms() || anim_skel.slots.elms() != skel->slots.elms())
            ...
            D.viewFrom(dist * 0.01f).viewRange(dist * 5);
            if (mesh)
                SetCam(cam, mesh->ext, Time.appTime()); // set
            Renderer(PreviewClass::Render);
            D.viewRect(r).viewFov(fov).viewFrom(from).viewRange(range);
            c.set(); // restore
            rect.draw(rect_color, false);
    ...
    case ELM_FONT: {
        TextStyle ts;
        ts.size = rect.h() / 16;
        ts.font(Proj.gamePath(elm.id));
        if (Font *font = ts.font()()) {
    ...
    case ELM_TEXT_STYLE:
        if (TextStylePtr ts = Proj.gamePath(elm.id)) {
            cchar8 *t = "ABCDEFGHIJKLMN\nOPQRSTUVWXYZ\nabcdefghijklmn\nopqrstuvwxyz\n0123456789\n!@#$%^&*+-=_()[]{}<>,.:;'\"\\|/?";
            Color c = ColorInverse(ts->color);
            c.a = 255;
            rect.draw(c);
            D.clip(rect);
            D.text(*ts, rect, t);
    ...
    case ELM_PANEL_IMAGE: {
        drawBack(rect);
        if (PanelImagePtr panel = Proj.gamePath(elm.id))
            DrawPanelImage(*panel, rect);
    } break;

    case ELM_PANEL: {
        drawBack(rect);
        if (PanelPtr panel = Proj.gamePath(elm.id))
            panel->draw(Rect_C(rect.center(), rect.size() * 0.5f));
    } break;
    ...
    case ELM_GUI_SKIN: {
        if (GuiSkinPtr skin = Proj.gamePath(elm.id)) {
            ...
            Window window;
            window.create(Rect_C(0, 0, rect.w() * 0.75f, rect.h() * 0.5f), "Window");
            Text text;
            text.create(window.clientRect().lerp(0.5f, 0.65f), "Hello!");
            Button button;
            button.create(Rect_C(window.clientRect().lerp(0.5f, 0.35f), 0.3f, 0.06f), "OK");
            GuiPC gpc;
    ...
    case ELM_GUI: {
        ...
        GuiPC gpc;
        gpc.visible = gpc.enabled = true;
        gpc.offset = (Vec2(0, 0) & rect);
        gpc.clip = gpc.client_rect = rect; // try to set the 'offset' as close to screen center as possible (so the preview will give similar results to what's actually available)
        FREP(objs_container.childNum())
        objs_container.child(i)->draw(gpc); // draw children manually and in order as they're listed in the parent
        Gui.skin = temp;
    } break;
    ...
    case ELM_IMAGE: {
        ...
        if (image = Proj.gamePath(elm.id)) {
            if (image->mode() == IMAGE_2D)
                image->drawFit(rect);
            else if (image->mode() == IMAGE_CUBE) {
                D.viewRect(rect).viewFov(DefaultFOV);
                SetCam(cam, Box(0), Time.appTime() / 2);
                Sky.skybox(image);
                Renderer(PreviewClass::Render);
                Sky.atmospheric();
                D.viewRect(r).viewFov(fov);
                c.set();
            } else if (image->mode() == IMAGE_3D) {
                D.viewRect(rect).viewFov(PreviewFOV);
                SetCam(cam, Box(1), Time.appTime() / 2);
                Renderer(PreviewClass::Render);
                D.viewRect(r).viewFov(fov);
                c.set();
            } else
                D.text(TextStyleParams(true), rect, "Image is software based\nand can't be previewed");
        }
    } break;

    case ELM_IMAGE_ATLAS: {
        drawBack(rect);
        if (atlas = Proj.gamePath(elm.id))
            if (int parts = atlas->parts.elms()) {
                ...
                if (InRange(part.image_index, atlas->images)) {
                    ...
                    if (tex_size.y > rect_size.y)
                        tex_size *= rect_size.y / tex_size.y; // scale to fit Y
                    ...
                    // draw image
                    if (part.rotated)
                        image.drawPartVertical(draw_rect, part.tex_rect);
                    else
                        image.drawPart(draw_rect, part.tex_rect);
    ...
    case ELM_SOUND: {
        ...
        drawBack(rect);
        D.clip(rect);
        D.text(TextStyleParams(true), rect, sound_info);
        D.clip();
    } break;
    ...
    case ELM_VIDEO: {
        if (video_id != elm.id) {
            video_id = elm.id;
            video.create(Proj.gamePath(elm), true);
            video_time = 0;
        } // start from the beginning to avoid freezes
        video.update(video_time);
        video_time += Time.ad(); // Editor works in background so app time gets updated even when minimized, update time only when actually drawing
        rect.draw(BLACK);
        video.drawFit(rect);
        rect.draw(rect_color, false);
    } break;
    ...
    case ELM_ENV: {
        env = Proj.gamePath(elm.id);
        env->set();
        D.viewRect(rect).viewFov(PreviewFOV).viewFrom(0.01f).viewRange(300);
        SetCam(cam, Box(0), Time.appTime() / 2); // set
        Renderer(PreviewClass::Render);
        D.viewRect(r).viewFov(fov).viewFrom(from).viewRange(range);
        c.set(); // restore
        rect.draw(rect_color, false);
    ...
    if (elm.importing()) {
        ...
        ts.size *= 0.6f;
        ts.align.set(0, 1);
        D.text(ts, rect.down(), (ImportRemovedElms || elm.finalExists()) ? "Processing" : "Not Processed");
    }
}
void PreviewClass::clipRect(Rect &rect) {
    flt t = D.h();
    if (Mode.visibleTabs())
        MIN(t, Mode.rect().min.y);
    if (rect.max.y > t)
        rect += Vec2(0, t - rect.max.y);
    if (rect.min.y < -D.h())
        rect += Vec2(0, -D.h() - rect.min.y);
}
void PreviewClass::drawAround(Elm &elm, C Rect &rect, flt y) {
    ...
    size = size.avg();
    Rect r;
    if (rect.min.x - (-D.w()) > D.w() - rect.max.x)
        r = Rect_R(rect.min.x - 0.02f, y, size.x, size.y);
    else
        r = Rect_L(rect.max.x + 0.02f, y, size.x, size.y);
    clipRect(r);
    draw(elm, r);
}
...
void PreviewClass::draw(C GuiPC &gpc) {
    if (ListElm *list_elm = Proj.list.visToData(Proj.list.lit)) {
        ...
        if (Elm *elm = list_elm->elm) {
            ...
            size *= 0.75f;
            size = size.avg();
            if (elm->type == ELM_SOUND)
                size.set(0.38f, 0.31f);
            Rect_L rect(Proj.rect().max.x, Ms.pos().y, size.x, size.y);
            clipRect(rect);
            if (elm->type == ELM_GUI)
                rect = EditRect();
            draw(*elm, rect);
    ...
    if (MergeSimilarMaterials::Data *list_elm = MSM.list.visToData(MSM.list.lit))
        if (Elm *elm = Proj.findElm(list_elm->id)) {
            ...
            if (rect.min.x - (-D.w()) > D.w() - rect.max.x)
                rect = Rect_R(rect.min.x - 0.02f, Ms.pos().y, size.x, size.y);
            else
                rect = Rect_L(rect.max.x + 0.02f, Ms.pos().y, size.x, size.y);
            clipRect(rect);
            draw(*elm, rect);
        }
    ...
    if (MaterialRegion::Texture *tex = CAST(MaterialRegion::Texture, Gui.ms())) {
        ...
        if (WaterMtrlEdit.visible())
            MIN(x, WaterMtrlEdit.rect().min.x);
        Rect_R rect(x, Ms.pos().y, size.x, size.y);
        clipRect(rect);
        tex->draw(rect);
    }
    if (ObjPaintClass::Object *obj = ObjPaint.objects_list.visToData(ObjPaint.objects_list.lit))
        if (Elm *elm = Proj.findElm(obj->id)) {
            ...
            size *= 0.5f;
            size = size.avg();
            Rect_R rect(ObjPaint.rect().min.x, Ms.pos().y, size.x, size.y);
            clipRect(rect);
            draw(*elm, rect);
        }
    // Object Parameter
    {
        ...
        if (!param)
            param = ObjClassEdit.findParam(Gui.ms());
        if (!param)
            param = ObjEdit.param_edit.findParam(Gui.ms());
        if (!param)
            param = WorldEdit.param_edit.findParam(Gui.ms());
        if (param) {
            if (param->src.type == PARAM_ID && (Gui.ms() == &param->val_textline || Gui.ms() == &param->name))
                if (Elm *elm = Proj.findElm(param->src.asID()))
                    drawAround(*elm, param->name.screenRect() | param->val_textline.screenRect());
            return;
        }
    }

    // textline
    if (Gui.ms() && Gui.ms()->type() == GO_TEXTLINE)
        if (Elm *elm = Proj.findElm(Gui.ms()->asTextLine()())) {
