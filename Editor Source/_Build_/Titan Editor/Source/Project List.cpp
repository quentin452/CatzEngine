...
void CreateDirectoryAndReadme(Project &proj) {
    std::string readmeContent = "ALL THESE FILES IN THIS FOLDER WILL BE COPY AND PASTED TO THE PUBLISH AND BUILD FOLDER OF YOUR GAME";
...
// TODO COPY AND PASTE TO BUILD FOLDER
// TODO COPY AND PASTE TO BUILD FOLDER
// TODO COPY AND PASTE TO BUILD FOLDER
// TODO COPY AND PASTE TO BUILD FOLDER
// TODO COPY AND PASTE TO BUILD FOLDER
...
bool InitProjectList() {
    ...
    if (App.cmd_line.elms()) {
        if (GetExt(App.cmd_line[0]) == ProjectPackageExt)
            CopyElms.display(App.cmd_line[0]);
...
/******************************************************************************/
::Projects::Elm *Projects::findProj(C UID &id) {
    if (id.valid())
        REPA(proj_data)
    if (proj_data[i].id == id)
        return &proj_data[i];
    return null;
}
...
void Projects::PathChanged(Projects &projs) {
    ProjectsPath = Str(projs.proj_path()).tailSlash(true);
...
void Projects::ImportProj(Projects &projs) {
    if (!projs.import_proj_io.is())
        projs.import_proj_io.create(ProjectPackageExt, S, S, ImportProj);
    projs.import_proj_io.activate();
}
void Projects::ImportProj(C Str &name, ptr user) { CopyElms.display(name); }
void Projects::ImportProj_1_0(Projects &projs) {
    if (!projs.import_proj_1_0_io.is())
        projs.import_proj_1_0_io.create(S, S, S, ImportProj_1_0).modeDirSelect();
    projs.import_proj_1_0_io.activate();
    Gui.msgBox(S, "Please specify path to project data created with Esenthel Engine 1.0");
}
void Projects::ImportProj_1_0(C Str &name, ptr user) {
    if (!ProjectsPath.is())
        Gui.msgBox(S, "Path for Projects is invalid");
    else if (FileInfoSystem(name).type == FSTD_DIR) {
        ImportSrc = name;
        StateImport.set(StateFadeTime);
    }
}
...
void Projects::Open() {
    if (Elm *proj = Projs.findProj(Projs.proj_menu_id))
        Projs.open(*proj);
}
void Projects::Rename() {
    if (Elm *proj = Projs.findProj(Projs.proj_menu_id))
        RenameProj.activate(*proj);
}
void Projects::Remove() {
    if (Elm *proj = Projs.findProj(Projs.proj_menu_id))
        RemoveProj.activate(*proj);
}
void Projects::ExploreProj() {
    if (Elm *proj = Projs.findProj(Projs.proj_menu_id))
        Explore(proj->path, true);
}
void Projects::CompareProj() {
    Memc<UID> ids;
    FREPA(Projs.proj_list.sel)
    if (Elm *proj = Projs.proj_list.absToData(Projs.proj_list.sel[i]))
...
void Projects::create() {
    ...
    ts_left_down = ts_left;
    ts_left_down.align.set(1, 1);
    ts_small_left_up = ts_left;
    ts_small_left_up.size *= 0.8f;
    ts_small_left_up.align.set(1, -1);
    ...
    proj_region.create() += proj_list.create(lc, Elms(lc)).elmHeight(0.06f).setElmDesc(MEMBER(Elm, desc));
    proj_list.sort_column[0] = 0;
    proj_list.cur_mode = LCM_ALWAYS;
    proj_list.flag |= LIST_MULTI_SEL;
    ...
    menu.create(menu_menu).skin(&NoComboBoxImage).focusable(false).desc("Menu");
    menu.text = "M";
    menu.text_align = 0;
    menu.flag |= COMBOBOX_CONST_TEXT;
    ...
    vid_opt.image = "Gui/Misc/display.img";
    vid_opt.mode = BUTTON_TOGGLE;
...
void Projects::resize() {
    ...
    flt s = h * 1.2f;
    flt w = D.w() * 0.42f;
    server.rect(Rect_LU(x, y, w, h));
    t_server.pos(server.rect().left() - Vec2(0.3f, 0));
    y -= s;
    email.rect(Rect_LU(x, y, w, h));
    t_email.pos(email.rect().left() - Vec2(0.3f, 0));
    y -= s;
    pass.rect(Rect_LU(x, y, w, h));
    t_pass.pos(pass.rect().left() - Vec2(0.3f, 0));
    y -= s;
    ...
    forgot_pass.rect(Rect_U(Lerp(t_pass.rect().min.x, pass.rect().max.x, 0.75f), y, 0.4f, h));
    y -= s;
...
void Projects::forgotPass() {
    SockAddr sa;
    if (!sa.fromText(server()))
        Gui.msgBox(S, "Invalid server address.");
    else if (!ValidEmail(email()))
        Gui.msgBox(S, "Invalid e-mail address.");
    else {
...
bool Projects::newProj(C Str &name) {
    ...
    else if (!FCreateDirs(proj.path))
        Gui.msgBox(S, "Can't write in \"Path for Projects\".\nTry choosing a different path."
#if WINDOWS
...
bool Projects::toggleSynchronizeProj(C UID &proj_id, C Str &name) {
    Project proj;
    Str error;
    switch (proj.open(proj_id, name, ProjectsPath + EncodeFileName(proj_id), error)) {
    case LOAD_NEWER:
        ...
        break;
    case LOAD_LOCKED:
        Gui.msgBox(S, "This project appears to be already opened in another instance of the Editor.\nIf it isn't, then please try re-opening it manually first.");
        break;
    case LOAD_ERROR:
        Gui.msgBox(S, S + "This project failed to load." + (error.is() ? "\n" : null) + error);
        break;
...
bool Projects::renameProj(C UID &proj_id, Str name) {
    ...
    if (!ValidFileName(name))
        ...
        if (elm == nullptr)
            return false;     // If the project is not found, return false
...
bool Projects::removeProj(C UID &proj_id) {
    ...
    Str path = elm->path; // Use Complete Project Path
    if (!FRecycle(path))
        Gui.msgBox(S, "Error when removing the project");
    else {
        refresh();
        return true;
    }
...
bool Projects::open(Elm &proj, bool ignore_lock) {
    ...
    if (!FCreateDirs(path))
        ...
        Str error;
        LOAD_RESULT result = Proj.open(proj.id, proj.name, path, error, ignore_lock);
...
void Projects::projMenu(C Vec2 &pos, bool touch) {
    Node<MenuElm> node;
    if (proj_list.sel.elms() == 1)
        if (Elm *proj = proj_list.absToData(proj_list.sel[0])) {
    ...
    int locals = 0;
    REPA(proj_list.sel)
    if (Elm *proj = proj_list.absToData(proj_list.sel[i]))
        locals += proj->local_name.is();
    if (locals > 1)
        node.New().create("Compare", CompareProj);
...
void Projects::update() {
    ...
    if (Kb.k(KB_ENTER)) {
        if (Gui.kb() == &server || Gui.kb() == &email || Gui.kb() == &pass)
            connect.push();
        else if (Gui.kb() == &proj_list) {
            if (Elm *proj = proj_list())
                open(*proj);
        }
    }

    REPA(MT)
    if (MT.guiObj(i) == &proj_list) {
        if (MT.bd(i))
            if (Elm *proj = proj_list()) {
                open(*proj);
                break;
            }
        if (MT.bp(i, 1) || MT.b(i) && MT.touch(i) && MT.life(i) >= LongPressTime && !MT.dragging(i) && !Gui.menu()) {
            proj_list.cur = proj_list.screenToVisY(MT.pos(i).y);
            if (!proj_list.sel.has(proj_list.visToAbs(proj_list.cur)))
                proj_list.setCur(proj_list.cur);
            projMenu(MT.pos(i), MT.touch(i) != null);
            break;
        }
    }
...
void NewProjWin::OK(NewProjWin &npw) {
    if (Projs.newProj(npw.name()))
        npw.button[2].push();
}
void NewProjWin::create() {
    Gui += super::create(Rect_C(0, 0, 0.75f, 0.37f), "New Project").hide();
    button[2].func(HideProjsAct, SCAST(GuiObj, T)).show();
...
void RenameProjWin::OK(RenameProjWin &rpw) {
    if (Projs.renameProj(rpw.proj_id, rpw.name()))
        rpw.button[2].push();
}
...
void RenameProjWin::create() {
    Gui += super::create(Rect_C(0, 0, 0.75f, 0.37f), "Rename Project").hide();
    button[2].func(HideProjsAct, SCAST(GuiObj, T)).show();
...
void RemoveProjWin::OK(RemoveProjWin &rpw) {
    Projs.removeProj(rpw.proj_id);
    rpw.button[2].push();
}
...
void RemoveProjWin::create() {
    Gui += super::create(Rect_C(0, 0, 0.75f, 0.37f), "Delete Project").hide();
    button[2].func(HideProjsAct, SCAST(GuiObj, T)).show();
    T += t_name.create(Rect_C(clientWidth() / 2, -0.11f, 0.7f, 0.0f), "Are you sure you wish to delete selected project from your disk?");
    t_name.auto_line = true;
    T += ok.create(Rect_C(clientWidth() / 2, -0.23f, 0.3f, 0.06f), "OK").func(OK, T);
}
void RegisterWin::OK(RegisterWin &rw) {
    Server.Register(rw.name());
    rw.hide();
}
void RegisterWin::create() {
    Gui += super::create(Rect_C(0, 0, 0.8f, 0.5f), "Register").hide();
    button[2].show();
...
void ChangePassWin::changePass() {
    if (!ValidPass(new_pass()))
        Gui.msgBox(S, "Invalid New Password");
    else if (use_key) {
        if (!cur().length())
        ...
        else if (cur() == new_pass())
            Gui.msgBox(S, "Passwords are the same");
        else {
...
void ChangePassWin::activate(bool use_key) {
    ...
    new_pass.clear();
    if (use_key)
        Gui.msgBox(S, "Unique key has been sent to your email address.\nPlease look into your inbox, and use that key to change your password.");
}
void ChangePassWin::create() {
    Gui += super::create(Rect_C(0, 0, 0.8f, 0.5f), "Change Password").hide();
    button[2].show();
