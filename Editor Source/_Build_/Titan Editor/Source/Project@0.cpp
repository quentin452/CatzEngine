...
void HideProject() {
    ...
    ReloadElm.hide();
    NewWorld.hide();
    WorldEdit.grid_plane_level_win.hide();
    WorldEdit.goto_area.hide();
    CopyElms.hide();
    CopyElms.replace_elms.hide();
    SizeStats.hide();
    PublishRes.hide();
    EraseRemoved.hide();
    ElmProps.hide();
...
Elm *GuiObjToElm(GuiObj *go) {
    if (go) {
        if (go == &AnimEdit || go == &AnimEdit.preview)
            return AnimEdit.elm;
        if (go == &PhysMtrlEdit)
            return PhysMtrlEdit.elm;
        if (go == &MiniMapEdit)
            return MiniMapEdit.elm;
        if (go == &EnvEdit)
            return EnvEdit.elm;
        if (go == &EnumEdit)
            return EnumEdit.elm;
        if (go == &ImageEdit)
            return ImageEdit.elm;
        if (go == &ImageAtlasEdit)
            return ImageAtlasEdit.elm;
        if (go == &IconEdit)
            return IconEdit.elm;
        if (go == &IconSettsEdit)
            return IconSettsEdit.elm;
        if (go == &FontEdit)
            return FontEdit.elm;
        if (go == &TextStyleEdit)
            return TextStyleEdit.elm;
        if (go == &PanelImageEdit)
            return PanelImageEdit.elm;
        if (go == &PanelEdit)
            return PanelEdit.elm;
        if (go == &ObjClassEdit)
            return ObjClassEdit.elm;
        if (go == &SoundEdit)
            return SoundEdit.elm;
        if (go == &VideoEdit)
            return VideoEdit.elm;
        if (go == &AppPropsEdit)
            return AppPropsEdit.elm;
        if (go == &MtrlEdit)
            return MtrlEdit.elm;
        if (go == &WaterMtrlEdit)
            return WaterMtrlEdit.elm;
        if (go == &ObjEdit)
            return ObjEdit.obj_elm;
        if (go == &WorldEdit)
            return WorldEdit.elm;
        if (go == &GuiSkinEdit)
            return GuiSkinEdit.elm;
        if (go == &GuiEdit)
            return GuiEdit.elm;
    }
    return null;
...
bool UpdateProject() {
   ...
   SPSet("MY", Ms.pos().y);
    ...
    if (Ms.bp(MS_MAXIMIZE))
        if (!ObjEdit.contains(Gui.ms()) && !AnimEdit.contains(Gui.ms()) && !WorldEdit.contains(Gui.ms()) && !TexDownsize.contains(Gui.ms()))
            App.window().toggle();

    // open element
    if (Kb.ctrlCmd() && Ms.bp(0) && Gui.ms() && Gui.ms()->type() == GO_TEXTLINE) {
        Memc<Str> splits;
        Split(splits, Gui.ms()->asTextLine()(), '\n');
        Memc<Elm *> elms;
        FREPA(splits)
        ...
        default: // cycle between elements
        {
            ...
            if (GuiObj *desktop = Gui.desktop())
                REP(desktop->childNum())
            if (GuiObj *go = desktop->child(i))
...
void DrawProject() {
    ...
    if (Mode() < 0)
        ...
        GuiEdit.drawPre();
    else if (Mode() == MODE_GUI_SKIN)
        GuiSkinEdit.draw();
    else if (Mode() == MODE_CODE)
        CodeEdit.CodeEditorInterface::draw();
...
void ProjectEx::OuterRegion::update(C GuiPC &gpc) {
    ...
    if (Gui.ms() == this) {
        if (!Ms.b(0)) // if not pressed then detect new resize, if pressed then keep previous
        {
            ...
            if (pos.x >= rect().max.x - D.pixelToScreenSize().x)
                resize_on = true;
        } else             // button pressed
            if (resize_on) // resize
            {
...
void ProjectEx::OuterRegion::draw() {
    if (resize_on && Gui.ms() == this)
        if (Image *image = Gui.image_resize_x()) {
            Vec2 pos = Ms.pos(), size(MOUSE_IMAGE_SIZE * image->aspect(), MOUSE_IMAGE_SIZE);
            pos += Vec2(-size.x, size.y) * 0.5f;
            image->draw(Rect_LU(D.screenAlignedToPixel(pos), size));
        }
}
GuiObj &ProjectEx::OuterRegion::show() {
    if (hidden()) {
        super::show();
        MtrlEdit.resize();
        WaterMtrlEdit.resize();
        CodeEdit.resize();
        if (!Gui.window() && Mode() < 0)
            Proj.list.kbSet();
    }
    return T;
}
GuiObj &ProjectEx::OuterRegion::hide() {
    if (visible()) {
        super::hide();
        MtrlEdit.resize();
        WaterMtrlEdit.resize();
...
Elm *ProjectEx::ElmList::operator()() C {
    if (ListElm *e = visToData(cur))
        return e->elm;
    return null;
}
EEItem *ProjectEx::ElmList::visToItem(int visible) C {
    if (ListElm *e = visToData(visible))
        return e->item;
    return null;
}
EEItem *ProjectEx::ElmList::absToItem(int absolute) C {
    if (ListElm *e = absToData(absolute))
        return e->item;
    return null;
}
Elm *ProjectEx::ElmList::visToElm(int visible) C {
    if (ListElm *e = visToData(visible))
        return e->elm;
    return null;
}
Elm *ProjectEx::ElmList::absToElm(int absolute) C {
    if (ListElm *e = absToData(absolute))
        return e->elm;
    return null;
}
int ProjectEx::ElmList::itemToVis(C EEItem *data) C {
    if (data)
        REP(visibleElms())
    if (visToItem(i) == data)
        return i;
...
int ProjectEx::ElmList::itemToAbs(C EEItem *data) C {
    if (data)
        REP(totalElms())
    if (absToItem(i) == data)
        return i;
...
int ProjectEx::ElmList::elmToVis(C Elm *data) C {
    if (data)
        REP(visibleElms())
    if (visToElm(i) == data)
        return i;
...
int ProjectEx::ElmList::elmToAbs(C Elm *data) C {
    if (data)
        REP(totalElms())
    if (absToElm(i) == data)
        return i;
...
bool ProjectEx::ElmList::identitySort() C // if list is sorted in identity mode
{
    FREPA(sort_column) // check in order
    {
        ...
        if (col < 0)
            continue; // unspecified = ignore
        if (col <= icon_col)
            return sort_swap[i] == false; // these columns sort in identity, so check if it's not swapped
...
void ProjectEx::ElmList::update(C GuiPC &gpc) {
    ...
    if (tapped_vis >= 0) {
        ...
        {
            if (tapped_open)
                if (Elm *elm = visToElm(tapped_vis)) {
                    elm->opened(!elm->opened());
                    Proj.refresh(false, false);
                }
            tapped_vis = -1;
        } else if (!tapped_open)
            Ms.eat(0); // don't process any new presses
    }
    ...
    if (Gui.kb() == this || menu.contains(Gui.kb())) {
        ...
        if (rename.pd()) {
            ...
            mulVol.eat();
            Proj.setMenuListSel();
            if (Proj.menu_list_sel.elms())
                MulSoundVolume.display(Proj.menu_list_sel);
        } else if (!Kb.k.ctrlCmd()) {
            if (Kb.k(KB_RIGHT)) {
                if (ListElm *elm = curToListElm()) {
                    if (elm->hasVisibleChildren()) {
                        if (elm->item && !elm->item->opened) {
                            elm->item->opened = true;
                            goto right_done;
                        }
                        if (elm->elm && !elm->elm->opened()) {
                            elm->elm->opened(true);
                            goto right_done;
                        }
                        if (cur < elms() - 1)
            ...
            } else if (Kb.k(KB_LEFT)) {
                if (ListElm *elm = curToListElm()) {
                    if (!elm->close()) // try closing current element
                        for (int i = cur - 1; i >= 0; i--)
                            if (ListElm *e = visToData(i))
                                if (e->offset < elm->offset) {
                                    setCur(i);
                                    break;
                                } // find first parent
                    Proj.refresh(false, false);
    ...
    if (Gui.ms() == &sound_play)
        lit = elmToVis(Proj.findElm(sound_play.lit_id));
    UID lit_elm_id = UIDZero;
    if (ListElm *elm = litToListElm())
        if (elm->elm)
            lit_elm_id = elm->elm->id;
...
void ProjectEx::ElmChange::swap(ptr user) {
    ...
    case PUBLISH:
        set ^= test;
        Proj._setElmPublish(elms, test, set, false, false);
        break; // "set^=test" means toggle "disable/enable"
    case REMOVE:
        type = RESTORE;
        Proj.restore(elms, false);
        break;
    case RESTORE:
        type = REMOVE;
        Proj.remove(elms, false, false);
        break;
    case SET_PARENT:
        Proj.setElmParent(elm_parents, true);
        break;
    case SET_NAME:
        Proj.setElmNames(elm_names, true);
        break;
    }
}
...
void ProjectEx::ElmRename(ProjectEx &proj) {
    if (proj.menu_list_sel.elms() == 1)
        if (Elm *elm = proj.findElm(proj.menu_list_sel[0]))
            RenameElm.activate(elm->id, elm->name);
}
void ProjectEx::ReplaceName(ProjectEx &proj) { ::ReplaceName.activate(proj.menu_list_sel); }
void ProjectEx::Remove(ProjectEx &proj) {
    proj.list.kbSet();
    proj.remove(proj.menu_list_sel, true);
}
void ProjectEx::Restore(ProjectEx &proj) {
    proj.list.kbSet();
    proj.restore(proj.menu_list_sel);
}
...
void ProjectEx::SplitAnim(ProjectEx &proj) {
    if (proj.menu_list_sel.elms())
        ::SplitAnim.activate(proj.menu_list_sel[0]);
}
...
void ProjectEx::Duplicate(ProjectEx &proj) {
    proj.list.kbSet();
    proj.duplicate(proj.menu_list_sel);
}
void ProjectEx::CopyTo(ProjectEx &proj) { CopyElms.display(); }
void ProjectEx::Expand(ProjectEx &proj) {
    proj.list.kbSet();
    proj.expand(proj.menu_list_sel, proj.menu_list_sel_item);
}
void ProjectEx::ExpandAll(ProjectEx &proj) {
    proj.list.kbSet();
    proj.expandAll(proj.menu_list_sel, proj.menu_list_sel_item);
}
void ProjectEx::Collapse(ProjectEx &proj) {
    proj.list.kbSet();
    proj.collapse(proj.menu_list_sel, proj.menu_list_sel_item);
}
void ProjectEx::ExploreElm(ProjectEx &proj) {
    proj.list.kbSet();
    if (proj.menu_list_sel.elms() == 1)
        if (Elm *elm = proj.findElm(proj.menu_list_sel[0]))
            proj.explore(*elm);
}
void ProjectEx::Export(ProjectEx &proj) {
    if (proj.menu_list_sel.elms())
        ::Export.activate(proj.menu_list_sel[0]);
}
void ProjectEx::Properties(ProjectEx &proj) {
    if (proj.menu_list_sel.elms() == 1)
        if (Elm *elm = proj.findElm(proj.menu_list_sel[0]))
            ElmProps.activate(*elm);
}
...
void ProjectEx::SoundPlay(ProjectEx &proj) {
    if (proj.sound.playing() && proj.list.sound_play.lit_id == proj.list.sound_play.play_id)
        proj.sound.close();
    else {
        proj.sound.close();
        if (Elm *elm = proj.findElm(proj.list.sound_play.lit_id, ELM_SOUND)) {
            proj.list.sound_play.play_id = elm->id;
            proj.sound.play(proj.gamePath(*elm));
        }
    }
}
void ProjectEx::EnablePublish(ProjectEx &proj) {
    proj.list.kbSet();
    proj.setElmPublish(proj.menu_list_sel, true, -1, false);
}
void ProjectEx::DisablePublish(ProjectEx &proj) {
    proj.list.kbSet();
    proj.setElmPublish(proj.menu_list_sel, false, -1, true);
}
void ProjectEx::EnablePublishMobile(ProjectEx &proj) {
    proj.list.kbSet();
    proj.setElmPublish(proj.menu_list_sel, -1, true, false);
}
void ProjectEx::DisablePublishMobile(ProjectEx &proj) {
    proj.list.kbSet();
    proj.setElmPublish(proj.menu_list_sel, -1, false, true);
}
...
void ProjectEx::MtrlSetRGBCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetRGB(proj.menu_list_sel, MtrlEdit.edit.color_s.xyz);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetTechCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetTech(proj.menu_list_sel, MtrlEdit.edit.tech);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetBumpCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetBump(proj.menu_list_sel, MtrlEdit.edit.bump);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetNormalCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetNormal(proj.menu_list_sel, MtrlEdit.edit.normal);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetSmoothCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetSmooth(proj.menu_list_sel, MtrlEdit.edit.smooth);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetReflectCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetReflect(proj.menu_list_sel, MtrlEdit.edit.reflect_min, MtrlEdit.edit.reflect_max);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetGlowCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetGlow(proj.menu_list_sel, MtrlEdit.edit.glow);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetUVScale(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetUVScale(proj.menu_list_sel, MtrlEdit.edit.uv_scale);
    else
        Gui.msgBox(S, "There's no Material opened");
}
...
void ProjectEx::MtrlSetColorTexCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetTexColor(proj.menu_list_sel, MtrlEdit.edit.color_map);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetBumpTexCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetTexBump(proj.menu_list_sel, MtrlEdit.edit.bump_map);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetNormalTexCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetTexNormal(proj.menu_list_sel, MtrlEdit.edit.normal_map, MtrlEdit.edit.flip_normal_y);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetSmoothTexCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetTexSmooth(proj.menu_list_sel, MtrlEdit.edit.smooth_map, MtrlEdit.edit.smooth_is_rough);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetMetalTexCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetTexMetal(proj.menu_list_sel, MtrlEdit.edit.metal_map);
    else
        Gui.msgBox(S, "There's no Material opened");
}
void ProjectEx::MtrlSetGlowTexCur(ProjectEx &proj) {
    if (MtrlEdit.elm)
        proj.mtrlSetTexGlow(proj.menu_list_sel, MtrlEdit.edit.glow_map);
    else
        Gui.msgBox(S, "There's no Material opened");
}
...
void ProjectEx::columnVisible(int column, bool visible) {
    ...
    if (visible)
        outer_region.setRegionWidth(Max(region_width, list.columnOffset(column + 1) + region.slidebarSize())); // make room to display entire column
    else
        outer_region.setRegionWidth(region_width - col_width); // decrease size by column width
    if (list.columnsHidden())
        list.sort(0, false); // if there are no columns visible then restore original sorting
}
...
void ProjectEx::ListOnlyFolder(ProjectEx &proj) {
    proj.list.show_only_folder ^= 1;
    proj.refresh(false, false);
}
void ProjectEx::ListOnlyObj(ProjectEx &proj) {
    proj.list.show_only_obj ^= 1;
    proj.refresh(false, false);
}
void ProjectEx::ListOnlyMtrl(ProjectEx &proj) {
    proj.list.show_only_mtrl ^= 1;
    proj.refresh(false, false);
}
void ProjectEx::ListOnlyAnim(ProjectEx &proj) {
    proj.list.show_only_anim ^= 1;
    proj.refresh(false, false);
}
void ProjectEx::ListOnlySound(ProjectEx &proj) {
    proj.list.show_only_sound ^= 1;
    proj.refresh(false, false);
}
void ProjectEx::ListFlat(ProjectEx &proj) {
    proj.list.flat_want ^= 1;
    proj.refresh(false, false);
}
...
C ImagePtr &ProjectEx::elmIcon(ELM_TYPE type) {
    ...
    case ELM_MTRL:
        return icon_mtrl;
    case ELM_WATER_MTRL:
        return icon_water_mtrl;
    case ELM_PHYS_MTRL:
        return icon_phys_mtrl;
    case ELM_ANIM:
        return icon_anim;
    case ELM_ENV:
        return icon_env;
    ...
    case ELM_IMAGE_ATLAS:
        return icon_image_atlas;
    case ELM_ICON_SETTS:
        return icon_icon_setts;
    case ELM_ICON:
        return icon_icon;
    case ELM_SOUND:
        return icon_sound;
...
Enum *ProjectEx::getEnumFromName(C Str &enum_name) {
    REPA(existing_enums)
    if (Elm *elm = findElm(existing_enums[i]))
        if (elm->name == enum_name)
            return Enums(gamePath(elm->id));
    return null;
}
C UID &ProjectEx::curApp() // warning: this relies on 'existing_apps' being setup properly
{
    if (existing_apps.binaryHas(app_id))
        return app_id; // if desired app exists
    if (list.app_checks.elms())
        return list.app_checks[0].app_id; // return first visible checkbox/app
    if (existing_apps.elms())
        return existing_apps[0]; // return any existing app
    ...
    if (Elm *obj_elm = findElm(obj_id))
        if (ElmObj *obj_data = obj_elm->objData()) {
            if (!obj_data->mesh_id.valid()) // it is not specified at the moment
            {
                ...
                mesh_elm = &getElm(mesh_id);
                if (mesh_elm->type && mesh_elm->type != ELM_MESH)
                    return null;     // can't create because this ID is used by something else
                if (!mesh_elm->type) // new element
                {
                    ...
                    if (ElmMesh *mesh_data = mesh_elm->meshData()) {
                        mesh_data->newData();
                        mesh_data->obj_id = obj_elm->id;
                    }
                    Mesh mesh;
                    mesh.save(editPath(mesh_elm->id)); // save a dummy mesh
                    makeGameVer(*mesh_elm);            // save mesh first before the object
                    if (send_to_server)
                        Server.setElmFull(mesh_elm->id); // send to server
                }
                obj_data->newVer();
                ...
                if (send_to_server)
                    Server.setElmShort(obj_elm->id);
                if (set_gui)
                    setList();

                // call at the end
                if (ObjEdit.obj_elm == obj_elm)
                    ObjEdit.getMeshElm(); // make sure obj editor has assigned mesh element
                REPAO(WorldEdit.objs).setMeshPhys();
            } else
                mesh_elm = findElm(obj_data->mesh_id);
        }
    return mesh_elm;
}
        ...
        if (ElmMesh *mesh_data = mesh_elm->meshData())
            if (Elm *obj_elm = findElm(obj_id, ELM_OBJ)) {
                if (!mesh_data->skel_id.valid()) // it is not specified at the moment
                {
                    ...
                    skel_elm = &getElm(skel_id);
                    if (skel_elm->type && skel_elm->type != ELM_SKEL)
                        return null;     // can't create because this ID is used by something else
                    if (!skel_elm->type) // new element
                    {
                        ...
                        Skeleton skel;
                        skel.save(gamePath(skel_elm->id)); // save a dummy skel
                        if (send_to_server)
                    ...
                    if (send_to_server)
                        Server.setElmShort(mesh_elm->id);
                    if (set_gui)
                        setList();

                    // call at the end
                    if (ObjEdit.obj_elm == obj_elm)
                        ObjEdit.getSkelElm(); // make sure obj editor has assigned skel element
                } else
                    skel_elm = findElm(mesh_data->skel_id);
            }
    return skel_elm;
}
        ...
        if (ElmMesh *mesh_data = mesh_elm->meshData())
            if (Elm *obj_elm = findElm(obj_id, ELM_OBJ)) {
                if (!mesh_data->phys_id.valid()) // it is not specified at the moment
                {
                    ...
                    phys_elm = &getElm(phys_id);
                    if (phys_elm->type && phys_elm->type != ELM_PHYS)
                        return null;     // can't create because this ID is used by something else
                    if (!phys_elm->type) // new element
                    {
                        ...
                        PhysBody phys;
                        phys.save(gamePath(phys_elm->id)); // save a dummy phys body
                        if (send_to_server)
                            Server.setElmFull(phys_elm->id);
                    }
                    mesh_data->newVer();
                    ...
                    // call at the end
                    if (ObjEdit.obj_elm == obj_elm)
                        ObjEdit.getPhysElm(); // make sure obj editor has assigned phys element
                    REPAO(WorldEdit.objs).setMeshPhys();
                } else
                    phys_elm = findElm(mesh_data->phys_id);
            }
    return phys_elm;
}
...
void ProjectEx::disableMenu(Menu &menu) {
    if (menu.hidden())
        Gui -= menu; // if the menu got hidden then unlink it from Gui, so its keyboard shortcuts won't be used
}
...
void ProjectEx::create() {
...

    {
        ...
        n++;
        Node<MenuElm> &New = (n += "New");
        setNewElm(New);
        outer_region += list_options.create().setData(n).focusable(false);
        list_options.flag |= COMBOBOX_CONST_TEXT;
        checkIncludeChildrenSize();
        checkIncludeTextureSize();
    }

    outer_region += show_removed.create().func(ShowRemoved, T).focusable(false).desc("Show removed elements\nKeyboard Shortcut: Alt+R");
    show_removed.image = "Gui/Misc/trash.img";
    show_removed.mode = BUTTON_TOGGLE;
    ...
    theater.mode = BUTTON_TOGGLE;
    outer_region += filter.create().func(FilterChanged, T).desc(S + "Find element\nType element name or its ID to find it.\n\nKeyboard Shortcut: " + Kb.ctrlCmdName() + "+Shift+F");
    filter.reset.show();
    filter.show_find = true;
    filter.hint = S + "Find (" + KbSc(KB_F, KBSC_CTRL_CMD | KBSC_SHIFT).asText() + ')';
    ...
    for (int i = list.name_col + 1; i < list.columns(); i++)
        list.columnVisible(i, false);
    list.flag |= LIST_MULTI_SEL;
    list.draw_column = -1;
...
Str ProjectEx::newElmName(ELM_TYPE type, C UID &parent_id) {
    ...
    FREP(1000) {
        Str name = ElmTypeName[type];
        if (i)
            name.space() += i;
        bool exists = false;
        FREPA(parent_n.children)
        if (name == elms[parent_n.children[i]].name) {
            exists = true;
            break;
        }
        if (!exists)
...
Elm *ProjectEx::newElm(ELM_TYPE type, C UID &parent_id, C Str *name, bool refresh_elm_list) {
    if (InRange(type, ELM_NUM) && type != ELM_NONE && type != ELM_MESH && type != ELM_SKEL && type != ELM_PHYS && type != ELM_SHADER && type != ELM_WORLD) {
        ...
        case ELM_CODE: {
            Str code;
            SaveCode(code, codePath(elm));
            if (ElmCode *data = elm.codeData()) {
                data->newData();
                data->from(code);
            }
        ...
        case ELM_MTRL: {
            EditMaterial edit;
            edit.newData(Proj);
            Save(edit, editPath(elm));
            if (ElmMaterial *data = elm.mtrlData()) {
                data->newData();
                data->from(edit);
            }
            makeGameVer(elm);
        ...
        case ELM_WATER_MTRL: {
            ...
            edit.newData(Proj);
            Save(edit, editPath(elm));
            if (ElmWaterMtrl *data = elm.waterMtrlData()) {
                data->newData();
                data->from(edit);
            }
            makeGameVer(elm);
        ...
        case ELM_PHYS_MTRL: {
            ...
            edit.newData();
            Save(edit, editPath(elm));
            if (ElmPhysMtrl *data = elm.physMtrlData()) {
                data->newData();
                data->from(edit);
            }
            makeGameVer(elm);
        ...
        case ELM_PANEL_IMAGE: {
            ...
            PanelImage game;
            edit.make(game, T);
            Save(game, gamePath(elm));
            savedGame(elm); // makeGameVer(elm);
        ...
        case ELM_OBJ_CLASS: {
            EditObject edit;
            edit.newData();
            edit.setType(true, elm.id, edit_path).setAccess(true, OBJ_ACCESS_CUSTOM);
            Save(edit, editPath(elm.id)); // type must be set at the moment of creation, also since this is an object class, then make all objects based on this have OBJ_ACCESS_CUSTOM access
        ...
        case ELM_ANIM: {
            ...
            if (ElmAnim *data = elm.animData()) {
                data->newData();
                data->from(anim);
                if (parent && parent->type == ELM_OBJ)
                    data->skel_id = objToSkel(parent);
                if (Elm *skel = findElm(data->skel_id))
                    if (ElmSkel *skel_data = skel->skelData())
                        data->transform = skel_data->transform;
                anim.linear(data->linear()).loop(data->loop());
            }
            anim.save(gamePath(elm));
            savedGame(elm);
            Server.setElmFull(elm.id);
        } break;
        ...
        case ELM_ICON: {
            if (ElmIcon *data = elm.iconData()) {
                data->newData();
                if (parent && parent->type == ELM_OBJ)
                    data->obj_id = parent->id;
            }
            Image image;
            image.save(gamePath(elm));
            savedGame(elm);
            Server.setElmFull(elm.id); // save empty game image (edit can be left empty because it will be set to default)
        } break;

        case ELM_GUI: {
            if (ElmGui *data = elm.guiData())
                data->newData();
            GuiObjs go;
            go.save(gamePath(elm));
            savedGame(elm);
            Server.setElmFull(elm.id);
        } break; // currently gui doesn't have edit version
        case ELM_IMAGE_ATLAS: {
            if (ElmImageAtlas *data = elm.imageAtlasData())
                data->newData();
            ImageAtlas atlas;
            atlas.save(gamePath(elm));
            savedGame(elm);
            Server.setElmFull(elm.id);
        } break; // save empty game atlas

        case ELM_SOUND: {
            if (ElmData *data = elm.Data())
                data->newData();
            File f;
            f.write(gamePath(elm));
            f.del();
            savedGame(elm);
            Server.setElmFull(elm.id);
        } break;
        ...
        if (refresh_elm_list)
            setList();
        if (type == ELM_CODE)
            activateSources(-1); // call after 'setList'
...
Elm *ProjectEx::newWorld(C Str &name, int area_size, int hm_res, C UID &parent_id, bool set_cur) {
    ...
    if (area_size >= 32 && area_size <= 128 && hm_res >= 32 && hm_res <= 128) {
        setListCurSel();
        Elm *parent = findElm(parent_id);
        if (parent)
            parent->opened(true);
...
void ProjectEx::setMtrl(Elm &mtrl, C XMaterialEx &src, C Str &src_file) {
    if (ElmMaterial *data = mtrl.mtrlData()) {
        ...
        if (src.base_0.is() && includeTex(src.base_0_id))
            saveTex(src.base_0, src.base_0_id);
        Server.setTex(src.base_0_id);
        if (src.base_1.is() && includeTex(src.base_1_id))
            saveTex(src.base_1, src.base_1_id);
        Server.setTex(src.base_1_id);
        if (src.base_2.is() && includeTex(src.base_2_id))
            saveTex(src.base_2, src.base_2_id);
        Server.setTex(src.base_2_id);
        if (src.detail.is() && includeTex(src.detail_id))
            saveTex(src.detail, src.detail_id);
        Server.setTex(src.detail_id);
        if (src.macro.is() && includeTex(src.macro_id))
            saveTex(src.macro, src.macro_id);
        Server.setTex(src.macro_id);
        if (src.emissive_img.is() && includeTex(src.emissive_id))
            saveTex(src.emissive_img, src.emissive_id);
        Server.setTex(src.emissive_id);

        TimeStamp time;
        time.getUTC();
        EditMaterial edit_mtrl;
        edit_mtrl.create(src, time);
...
{
    Elm &mtrl = super::newElm(src.name, parent_id, ELM_MTRL);
    mtrl.mtrlData()->newData();
...
void ProjectEx::setElmName(Elm &elm, C Str &name, C TimeStamp &time) {
    ...
    case ELM_ENUM:
        makeGameVer(elm);
        break; // this relies on element name
    }
    switch (elm.type) {
    case ELM_ENUM:
    case ELM_OBJ_CLASS:
...
void ProjectEx::setElmNames(Memc<Edit::IDParam<Str>> &elms, bool adjust_elms) // 'adjust_elms'=if this is performed because of undo, and in that case we need to remember current names, so we can undo this change later
{
    if (elms.elms()) {
        ...
        if (Elm *elm = findElm(elms[i].id)) // get element
        {
            ...
            if (adjust_elms)
                elms[i].value = elm->name; // !! if we're adjusting, then set current name !!
...
void ProjectEx::replaceName(C MemPtr<UID> &elm_ids, C Str &from, C Str &to) {
    if (elm_ids.elms()) {
        ...
        Str name;
        FREPA(elm_ids)
        if (Elm *elm = findElm(elm_ids[i])) {
            name = Replace(elm->name, from, to);
            if (!Equal(elm->name, name, true)) {
...
{
    if (elm_id == list.sound_play.play_id)
        sound.close();
...
void ProjectEx::remove(Memc<UID> &ids, bool parents_only, bool set_undo) // !! WARNING: MIGHT SORT 'ids' !!
{
    if (ids.elms()) {
        ...
        if (parents_only) {
            ids.sort();
            remove(root, ids, removed, time);
        } else
            REPA(ids)
        if (Elm *elm = findElm(ids[i]))
            if (elm->exists() && time > elm->removed_time) {
                elm->setRemoved(true, time);
                removed.add(elm->id);
            }
        ...
        if (set_undo)
            if (ElmChange *change = elm_undos.set(null, true)) // !! 'force_create' ALSO NEEDED BECAUSE OF "Swap(change.elms" BELOW !!
            {
...
void ProjectEx::restore(Memc<UID> &ids, bool set_undo) {
    if (ids.elms()) {
        ...
        REPA(ids)
        if (Elm *elm = findElm(ids[i]))
            if (elm->removed() && time > elm->removed_time) {
                elm->setRemoved(false, time);
                restored.add(elm->id);
            }
...
void ProjectEx::_setElmPublish(Memc<UID> &ids, uint test, uint set, bool parents_only, bool set_undo) // !! WARNING: MIGHT SORT 'ids' !!
{
    if (ids.elms()) {
        ...
        {
            ids.sort();
            _setElmPublish(root, ids, test, set, processed, time);
        } else
            REPA(ids)
        if (Elm *elm = findElm(ids[i]))
            if (time > elm->publish_time && (elm->flag & test) != set) {
                FlagCopy(elm->flag, set, test);
                elm->publish_time = time;
                processed.add(elm->id);
            }
...
void ProjectEx::setElmPublish(Memc<UID> &ids, sbyte all, sbyte mobile, bool parents_only, bool set_undo) // !! WARNING: MIGHT SORT 'ids' !!
{
    if (ids.elms())                  // any elements
        if (all >= 0 || mobile >= 0) // anything to change
        {
            ...
            if (mobile >= 0) {
                test |= Elm::NO_PUBLISH_MOBILE;
                if (!mobile)
                    set |= Elm::NO_PUBLISH_MOBILE;
            }
...
void ProjectEx::reload(Memc<UID> &elm_ids) {
    if (elm_ids.elms() > 1)
        ReloadElm.activate(elm_ids);
    else if (elm_ids.elms() == 1)
        ReloadElm.activate(elm_ids[0]);
}
...
void ProjectEx::removeMeshVtx(C MemPtr<UID> &elm_ids, MESH_FLAG flag) {
    if (flag)
        REPA(elm_ids)
    if (Elm *obj_elm = findElm(elm_ids[i], ELM_OBJ))
        if (ElmObj *obj_data = obj_elm->objData())
            if (Elm *mesh_elm = findElm(obj_data->mesh_id, ELM_MESH)) {
                if (elm_ids[i] == ObjEdit.obj_id) // if this is currently edited object
                {
                    ObjEdit.remVtx(flag);
                } else {
                    Mesh mesh;
                    if (Load(mesh, editPath(mesh_elm->id), game_path)) {
                        bool changed = false;
                        REPD(l, mesh.lods()) {
        ...
        if (ElmObj *obj_data = obj_elm->objData())
            if (Elm *mesh_elm = findElm(obj_data->mesh_id, ELM_MESH)) {
                if (elm_ids[i] == ObjEdit.obj_id) // if this is currently edited object
                {
                    ...
                    Mesh mesh;
                    if (Load(mesh, editPath(mesh_elm->id), game_path) && DisableLQLODs(mesh)) {
                        Save(mesh, editPath(mesh_elm->id), game_path);
                        makeGameVer(*mesh_elm);
                        if (ElmMesh *mesh_data = mesh_elm->meshData()) {
                            mesh_data->newVer();
        ...
        if (ElmObj *obj_data = obj_elm->objData())
            if (Elm *mesh_elm = findElm(obj_data->mesh_id, ELM_MESH)) {
                if (elm_ids[i] == ObjEdit.obj_id) // if this is currently edited object
                {
                    ...
                    Mesh mesh;
                    if (Load(mesh, editPath(mesh_elm->id), game_path) && mesh.weldCoplanarFaces()) {
                        Save(mesh, editPath(mesh_elm->id), game_path);
                        makeGameVer(*mesh_elm);
                        if (ElmMesh *mesh_data = mesh_elm->meshData()) {
                            mesh_data->newVer();
...
bool ProjectEx::forceImageSize(Str &file, C VecI2 &size, bool relative) {
    ...
    if (files.elms()) {
        ...
        REPA(files) // go from end
        {
            ...
            if (i && (file.name.is() || file.nodes.elms()))
                break;        // stop on first file that has name (but allow the first which means there's only one file) so we don't process transforms for only 1 of multiple images
            REPA(file.params) // go from end
            {
                TextParam &p = file.params[i];
                if (ResizeTransform(p.name)) {
                    if (!resize_name.is())
                        resize_name = p.name;    // get the name of resize at the end (check if not yet set because we're processing from the end), this is needed to preserve filtering choice
                    file.params.remove(i, true); // remove resize transforms
                } else if (SizeDependentTransform(p))
                    goto skip; // if encountered a size dependent transform then it means we can't remove any other resize transforms
            }
            if (!file.is())
                files.remove(i, true); // if nothing left then remove it
        }
    skip:
        if (!resize_name.is())
            resize_name = "resize";                                                // set default resize name
        ...
        if (files.elms() && !(relative && !size.x && !size.y && !specific_resize)) // "relative && !size" means original size, for which we don't need to do anything, because "resize" was already removed
        {
            ...
            {
                Image temp;
                if (!loadImages(temp, null, FileParams::Encode(files)))
                    return false; // if failed to load, then do nothing, can ignore sRGB
            ...
            if (specific_resize || s.any())
                SetResizeTransform(files, resize_name, s.any() ? TextVecI2Ex(s) : S); // only if any specified
        }
    ...
    if (Elm *image = findElm(elm_ids[i], ELM_IMAGE)) {
        if (ElmImage *image_data = image->imageData()) {
            if (image_data->mipMaps() != on)
                if (ImageEdit.elm == image)
                    ImageEdit.setMipMap(on);
                else {
                    image_data->newVer();
                    image_data->mipMaps(on);
                    image_data->mip_maps_time.now();
                    makeGameVer(*image);
    ...
    if (Elm *image = findElm(elm_ids[i], ELM_IMAGE))
        if (ElmImage *image_data = image->imageData())
            if (image_data->size != size)
                if (ImageEdit.elm == image)
                    ImageEdit.setSize(size);
                else {
                    image_data->newVer();
                    image_data->size = size;
                    image_data->size_time.now();
                    makeGameVer(*image);
                    Server.setElmShort(image->id);
                }
}
void ProjectEx::mtrlResetAlpha(C MemPtr<UID> &elm_ids) {
    ...
    if (Elm *mtrl = findElm(elm_ids[i], ELM_MTRL))
        if (ElmMaterial *mtrl_data = mtrl->mtrlData())
            if (MtrlEdit.elm == mtrl)
                MtrlEdit.resetAlpha();
            else {
                EditMaterial edit;
                if (edit.load(editPath(mtrl->id))) {
                    mtrl_data->newVer();
                    edit.resetAlpha();
...
void ProjectEx::mtrlSetRGB(C MemPtr<UID> &elm_ids, C Vec &srgb, bool mul) {
    ...
    if (Elm *mtrl = findElm(elm_ids[i], ELM_MTRL))
        if (ElmMaterial *mtrl_data = mtrl->mtrlData())
            if (MtrlEdit.elm == mtrl)
                MtrlEdit.setRGB(mul ? MtrlEdit.edit.color_s.xyz * srgb : srgb);
            else {
                EditMaterial edit;
                if (edit.load(editPath(mtrl->id)))
                    if (mul || edit.color_s.xyz != srgb) {
                        mtrl_data->newVer();
                        if (mul)
                            edit.color_s.xyz *= srgb;
                        else
                            edit.color_s.xyz = srgb;
                        edit.color_time.now();
                        Save(edit, editPath(mtrl->id));
...
bool ProjectEx::mtrlSetTech(C MemPtr<UID> &elm_ids, MATERIAL_TECHNIQUE tech) {
    ...
    REPA(elm_ids) {
        EditMaterial edit;
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (edit.tech != tech) {
            edit.tech = tech;
            edit.tech_time.now();
...
bool ProjectEx::mtrlSetNormal(C MemPtr<UID> &elm_ids, flt normal, bool mul) {
    ...
    if (!mul || normal != 1)
        REPA(elm_ids) {
            ...
            if (!mtrlGet(elm_ids[i], edit))
                ok = false;
            else if (mul || edit.normal != normal) {
                if (mul)
                    edit.normal *= normal;
                else
                    edit.normal = normal;
                edit.normal_time.now();
...
bool ProjectEx::mtrlSetSmooth(C MemPtr<UID> &elm_ids, flt smooth, bool mul) {
    ...
    if (!mul || smooth != 1)
        REPA(elm_ids) {
            ...
            if (!mtrlGet(elm_ids[i], edit))
                ok = false;
            else if (mul || edit.smooth != smooth) {
                if (mul)
                    edit.smooth *= smooth;
                else
                    edit.smooth = smooth;
                edit.smooth_time.now();
...
bool ProjectEx::mtrlSetReflect(C MemPtr<UID> &elm_ids, flt reflect_min, flt reflect_max) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (edit.reflect_min != reflect_min || edit.reflect_max != reflect_max) {
            edit.reflect_min = reflect_min;
            edit.reflect_max = reflect_max;
            edit.reflect_time.now();
...
bool ProjectEx::mtrlSetUVScale(C MemPtr<UID> &elm_ids, flt uv_scale) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (edit.uv_scale != uv_scale) {
            edit.uv_scale = uv_scale;
            edit.uv_scale_time.now();
...
bool ProjectEx::mtrlSetTexColor(C MemPtr<UID> &elm_ids, C Str &color_map) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (!Equal(edit.color_map, color_map, true)) {
            edit.color_map = color_map;
            edit.color_map_time.now();
...
bool ProjectEx::mtrlSetTexBump(C MemPtr<UID> &elm_ids, C Str &bump_map) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (!Equal(edit.bump_map, bump_map, true)) {
            edit.bump_map = bump_map;
            edit.bump_map_time.now();
...
bool ProjectEx::mtrlSetTexNormal(C MemPtr<UID> &elm_ids, C Str &normal_map, bool flip_normal_y) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (!Equal(edit.normal_map, normal_map, true) || edit.flip_normal_y != flip_normal_y) {
            edit.normal_map = normal_map;
            edit.normal_map_time.now();
            edit.flip_normal_y = flip_normal_y;
            edit.flip_normal_y_time = edit.normal_map_time;
...
bool ProjectEx::mtrlSetTexSmooth(C MemPtr<UID> &elm_ids, C Str &smooth_map, bool smooth_is_rough) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (!Equal(edit.smooth_map, smooth_map, true) || edit.smooth_is_rough != smooth_is_rough) {
            edit.smooth_map = smooth_map;
            edit.smooth_map_time.now();
            edit.smooth_is_rough = smooth_is_rough;
            edit.smooth_is_rough_time = edit.smooth_map_time;
...
bool ProjectEx::mtrlSetTexMetal(C MemPtr<UID> &elm_ids, C Str &metal_map) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (!Equal(edit.metal_map, metal_map, true)) {
            edit.metal_map = metal_map;
            edit.metal_map_time.now();
...
bool ProjectEx::mtrlSetTexGlow(C MemPtr<UID> &elm_ids, C Str &glow_map) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (!Equal(edit.glow_map, glow_map, true)) {
            edit.glow_map = glow_map;
            edit.glow_map_time.now();
...
void ProjectEx::mtrlFlipNrmY(C MemPtr<UID> &elm_ids, bool on) {
    ...
    if (Elm *mtrl = findElm(elm_ids[i], ELM_MTRL))
        if (ElmMaterial *mtrl_data = mtrl->mtrlData())
            if (MtrlEdit.elm == mtrl)
                MtrlEdit.flipNrmY(on);
            else {
                EditMaterial edit;
                if (edit.load(editPath(mtrl->id)))
                    if (edit.flip_normal_y != on) {
                        mtrl_data->newVer();
...
void ProjectEx::mtrlSmoothIsRough(C MemPtr<UID> &elm_ids, bool on) {
    ...
    if (Elm *mtrl = findElm(elm_ids[i], ELM_MTRL))
        if (ElmMaterial *mtrl_data = mtrl->mtrlData())
            if (MtrlEdit.elm == mtrl)
                MtrlEdit.smoothIsRough(on);
            else {
                EditMaterial edit;
                if (edit.load(editPath(mtrl->id)))
                    if (edit.smooth_is_rough != on) {
                        mtrl_data->newVer();
...
void ProjectEx::mtrlTexDownsize(C MemPtr<UID> &elm_ids, TEX_SIZE_PLATFORM tsp, byte downsize, C UID &base_0, C UID &base_1, C UID &base_2, bool precise) // 'precise'=if do a more precise but slower processing (if true then all materials from 'elm_ids' will have to be loaded from disk for checking, which might be slow)
{
    Memt<UID> mtrls;
    REPA(elm_ids)
    if (C Elm *mtrl = findElm(elm_ids[i]))
        if (C ElmMaterial *mtrl_data = mtrl->mtrlData())
            if (precise                                      // if always check (will correctly set other materials)
                ...
                if (mtrls.binaryInclude(mtrl->id))           // if just added
                {
                    ...
                    Memt<UID> tex_ids;
                    if (mtrl_data->base_0_tex.valid())
                        tex_ids.binaryInclude(mtrl_data->base_0_tex); // include searching for 'mtrl_data.base_0_tex'
                    if (mtrl_data->base_1_tex.valid())
                        tex_ids.binaryInclude(mtrl_data->base_1_tex); // include searching for 'mtrl_data.base_1_tex'
                    if (mtrl_data->base_2_tex.valid())
                        tex_ids.binaryInclude(mtrl_data->base_2_tex); // include searching for 'mtrl_data.base_2_tex'
                    if (base_0.valid())
                        tex_ids.binaryInclude(base_0); // include searching for 'base_0' (Tex ID before reload)
                    if (base_1.valid())
                        tex_ids.binaryInclude(base_1); // include searching for 'base_1' (Tex ID before reload)
                    if (base_2.valid())
                        tex_ids.binaryInclude(base_2); // include searching for 'base_2' (Tex ID before reload)
                    if (tex_ids.elms())                // if has any Tex ID's to compare
                    {
                        ...
                        {
                            C Elm &elm = elms[i];
                            if (C ElmMaterial *test_data = elm.mtrlData())
                                if (test_data->tex_downsize[tsp] != downsize)
                                    if (mtrl != &elm)
                                        if (tex_ids.binaryHas(test_data->base_0_tex) // if their textures match any in 'tex_ids'
                                            || tex_ids.binaryHas(test_data->base_1_tex) || tex_ids.binaryHas(test_data->base_2_tex)) {
                                            if (!edit_loaded) {
                                                if (mtrlGet(mtrl->id, edit))
                                                    edit_loaded = true;
                                                else
                                                    break;
                                            } // make sure 'edit' is now loaded

                                            EditMaterial test;
                                            if (mtrlGet(elm.id, test)) // do extra checks if maps are the same
                                                if (edit.color_map.is() && EqualPath(edit.color_map, test.color_map) || edit.smooth_map.is() && EqualPath(edit.smooth_map, test.smooth_map) || edit.metal_map.is() && EqualPath(edit.metal_map, test.metal_map) || edit.bump_map.is() && EqualPath(edit.bump_map, test.bump_map) || edit.normal_map.is() && EqualPath(edit.normal_map, test.normal_map) || edit.glow_map.is() && EqualPath(edit.glow_map, test.glow_map))
    ...
    REPA(mtrls)
    if (Elm *mtrl = findElm(mtrls[i]))
        if (ElmMaterial *mtrl_data = mtrl->mtrlData())
            if (mtrl_data->tex_downsize[tsp] != downsize) {
                if (MtrlEdit.elm == mtrl)
                    MtrlEdit.texDownsize(tsp, downsize);
                else {
                    EditMaterial edit;
                    if (edit.load(editPath(mtrl->id))) {
                        mtrl_data->newVer();
...
bool ProjectEx::mtrlTexQuality(C MemPtr<UID> &elm_ids, Edit::Material::TEX_QUALITY quality, C UID &base_0, C UID &base_1, C UID &base_2, bool precise) // 'precise'=if do a more precise but slower processing (if true then all materials from 'elm_ids' will have to be loaded from disk for checking, which might be slow) // !! this is not perfect !!
{
                ...
                if (mtrls.binaryInclude(mtrl->id))    // if just added
                {
                    ...
                    if (tex_ids.elms())                // if has any Tex ID's to compare
                    {
                        ...
                        FREPA(elms)        // check all other materials, process in order to make 'binaryInclude' faster
                        {
                            C Elm &elm = elms[i];
                            if (C ElmMaterial *test_data = elm.mtrlData())
                                if (test_data->tex_quality != quality)
                                    if (mtrl != &elm)
                                        if (tex_ids.binaryHas(test_data->base_0_tex) // if their textures match any in 'tex_ids'
                                            || tex_ids.binaryHas(test_data->base_1_tex) || tex_ids.binaryHas(test_data->base_2_tex)) {
                                            if (!edit_loaded) {
    ...
    if (Elm *mtrl = findElm(mtrls[i]))
        if (ElmMaterial *mtrl_data = mtrl->mtrlData())
            if (mtrl_data->tex_quality != quality) {
                if (MtrlEdit.elm == mtrl)
                    MtrlEdit.texQuality(quality);
                else {
                    EditMaterial edit;
                    if (!mtrlGet(mtrl->id, edit))
                        ok = false;
                    else {
                        edit.tex_quality = quality;
                        edit.tex_quality_time.now();
...
bool ProjectEx::mtrlMulTexCol(C MemPtr<UID> &elm_ids) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (!Equal(edit.color_s.xyz, Vec(1)) && edit.color_map.is()) {
            Mems<FileParams> fps = FileParams::Decode(edit.color_map);
            Vec mul = edit.color_s.xyz;
            if (C TextParam *p = FindTransform(fps, "mulRGB"))
                mul *= TextVecEx(p->value);
            if (Equal(mul, Vec(1)))
                DelTransform(fps, "mulRGB");
            else
                SetTransform(fps, "mulRGB", TextVecEx(mul));
            edit.color_map = FileParams::Encode(fps);
            edit.color_map_time.now();
            edit.color_s.xyz = 1;
            edit.color_time.now();
...
bool ProjectEx::mtrlMulTexNormal(C MemPtr<UID> &elm_ids) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (!Equal(edit.normal, 1) && edit.hasNormalMap()) {
            edit.separateNormalMap();
            Mems<FileParams> fps = FileParams::Decode(edit.normal_map);
            flt mul = edit.normal;
            if (C TextParam *p = FindTransform(fps, "scale"))
                mul *= p->asFlt();
            if (Equal(mul, 1))
                DelTransform(fps, "scale");
            else
                SetTransform(fps, "scale", TextReal(mul, -3));
            edit.normal_map = FileParams::Encode(fps);
            edit.normal_map_time.now();
            edit.normal = 1;
            edit.normal_time.now();
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ...
            if (!Equal(edit.smoothMul(), 1) || !Equal(edit.smoothAdd(), 0)) {
                ...
                if (need_mul && need_add)
                    AddTransform(fps, "mulAddRGB", TextVecVecEx(mul, add));
                else if (need_mul)
                    AddTransform(fps, "mulRGB", TextVecEx(mul));
                else if (need_add)
                    AddTransform(fps, "addRGB", TextVecEx(add));

                edit.smooth_map = FileParams::Encode(fps);
                edit.smooth_map_time.now();
                edit.smooth = 1;
                edit.smooth_time.now();
...
bool ProjectEx::mtrlMulTexGlow(C MemPtr<UID> &elm_ids) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (!Equal(edit.glow, 1) && edit.glow_map.is()) {
            ...
            if (Equal(mul, 1))
                ...
                SetTransform(fps, "mulRGB", TextReal(mul, -3));
            edit.glow_map = FileParams::Encode(fps);
            edit.glow_map_time.now();
            edit.glow = 1;
            edit.glow_time.now();
...
bool ProjectEx::mtrlMulTexEmissive(C MemPtr<UID> &elm_ids) {
    ...
    REPA(elm_ids) {
        ...
        if (!mtrlGet(elm_ids[i], edit))
            ok = false;
        else if (!Equal(edit.emissive_s, Vec(1)) && edit.emissive_map.is()) {
            Mems<FileParams> fps = FileParams::Decode(edit.emissive_map);
            Vec mul = edit.emissive_s;
            if (flt mul_f = mul.avg())
                edit.emissive_glow /= mul_f;
            if (C TextParam *p = FindTransform(fps, "mulRGB"))
                mul *= TextVecEx(p->value);
            if (Equal(mul, Vec(1)))
                DelTransform(fps, "mulRGB");
                ...
                SetTransform(fps, "mulRGB", TextVecEx(mul));
            edit.emissive_map = FileParams::Encode(fps);
            edit.emissive_map_time.now();
            edit.emissive_s = 1;
            edit.emissive_time.now();
...
void ProjectEx::mtrlMoveToObj(C MemPtr<UID> &elm_ids) {
    ...
    FREPA(elm_ids) {
        C UID &mtrl_id = elm_ids[i], obj_id = mtrlToObj(mtrl_id);
        if (obj_id.valid())
            moves.New() = Edit::IDParam<UID>(mtrl_id, obj_id);
    }
...
bool ProjectEx::mtrlGet(C UID &elm_id, EditMaterial &mtrl) {
    if (C Elm *elm = findElm(elm_id)) {
        ...
        case ELM_WATER_MTRL: {
            EditWaterMtrl water_mtrl;
            if (!water_mtrl.load(editPath(elm_id)))
                break;
            mtrl = water_mtrl;
        }
            return true;
    ...
    if (Elm *elm = findElm(elm_id)) {
        if (MtrlEdit.elm_id == elm_id) {
            ...
            if (uint changed = MtrlEdit.edit.sync(mtrl)) {
                if (reload_textures) {
                    if (changed & EditMaterial::CHANGED_BASE)
                        MtrlEdit.rebuildBase(textures, changed, adjust_params, true);
                    if (changed & TEXF_DET)
                        MtrlEdit.rebuildDetail();
                    if (changed & TEXF_MACRO)
                        MtrlEdit.rebuildMacro();
                    if (changed & TEXF_EMISSIVE)
                        MtrlEdit.rebuildEmissive(textures, adjust_params);
                } else {
        ...
        if (WaterMtrlEdit.elm_id == elm_id) {
            ...
            if (uint changed = WaterMtrlEdit.edit.sync(mtrl)) {
                if (reload_textures) {
                    if (changed & EditMaterial::CHANGED_BASE)
                        WaterMtrlEdit.rebuildBase(textures, changed, adjust_params, true);
                    if (changed & TEXF_DET)
                        WaterMtrlEdit.rebuildDetail();
                    if (changed & TEXF_MACRO)
                        WaterMtrlEdit.rebuildMacro();
                    if (changed & TEXF_EMISSIVE)
                        WaterMtrlEdit.rebuildEmissive(textures, adjust_params);
                } else {
                    if (changed & (EditMaterial::CHANGED_BASE | TEXF_DET | TEXF_MACRO | TEXF_EMISSIVE))
                        mtrlTexChanged();
                }
        ...
        case ELM_MTRL: {
            ...
            if (uint changed = edit.sync(mtrl)) // if changed anything
            {
                ...
                if (reload_textures) {
                    if (changed & EditMaterial::CHANGED_BASE) {
                        known_textures |= TEXF_BASE;
                        has_textures |= mtrlCreateBaseTextures(edit, changed);
                    }
                    if (changed & TEXF_DET) {
                        known_textures |= TEXF_DET;
                        has_textures |= mtrlCreateDetailTexture(edit);
                    }
                    if (changed & TEXF_MACRO) {
                        known_textures |= TEXF_MACRO;
                        has_textures |= mtrlCreateMacroTexture(edit);
                    }
                    if (changed & TEXF_EMISSIVE) {
                        known_textures |= TEXF_EMISSIVE;
                        has_textures |= mtrlCreateEmissiveTexture(edit);
                    }
                }

                if (adjust_params)
                    edit.adjustParams(changed, old_textures, has_textures, known_textures);
                edit.copyTo(*game, T);
                ...
                Save(*game, gamePath(elm_id));
                savedGame(*elm);
                Server.setElmLong(elm_id);

                // process dependencies
                if (want_tan_bin != game->needTanBin())
                    mtrlSetAutoTanBin(elm_id);
                if (changed & (EditMaterial::CHANGED_BASE | TEXF_DET | TEXF_MACRO | TEXF_EMISSIVE))
                    mtrlTexChanged();
                D.setShader(game());
            }
...
TEX_FLAG ProjectEx::mtrlCreateBaseTextures(EditMaterial &material, uint changed) // 'changed'=only CHANGED_FLIP_NRM_Y or CHANGED_SMOOTH_IS_ROUGH are valid
{
    ...
    // base 0
    old_tex_id = material.base_0_tex;
    ImageProps(base_0, &material.base_0_tex, &ct, MTRL_BASE_0, material.tex_quality);
    if (old_tex_id != material.base_0_tex)
        material.color_map_time.getUTC(); // in order for 'base_0_tex' to sync, a base 0 texture time must be changed, but set it only if the new texture is different #MaterialTextureLayout
    if (base_0.is()) {
    ...
    // base 1
    old_tex_id = material.base_1_tex;
    ImageProps(base_1, &material.base_1_tex, &ct, MTRL_BASE_1);
    if (old_tex_id != material.base_1_tex)
        material.normal_map_time.getUTC(); // in order for 'base_1_tex' to sync, a base 1 texture time must be changed, but set it only if the new texture is different #MaterialTextureLayout
    if (base_1.is()) {
    ...
    // base 2
    old_tex_id = material.base_2_tex;
    ImageProps(base_2, &material.base_2_tex, &ct, MTRL_BASE_2);
    if (old_tex_id != material.base_2_tex)
        material.smooth_map_time.getUTC(); // in order for 'base_2_tex' to sync, a base 2 texture time must be changed, but set it only if the new texture is different #MaterialTextureLayout
    if (base_2.is()) {
...
TEX_FLAG ProjectEx::mtrlCreateBaseTextures(EditWaterMtrl &material, uint changed) // 'changed'=only CHANGED_FLIP_NRM_Y or CHANGED_SMOOTH_IS_ROUGH are valid
{
    ...
    // base 0
    old_tex_id = material.base_0_tex;
    ImageProps(base_0, &material.base_0_tex, &ct, MTRL_BASE_0 | WATER_MTRL, material.tex_quality);
    if (old_tex_id != material.base_0_tex)
        material.color_map_time.getUTC(); // in order for 'base_0_tex' to sync, a base 0 texture time must be changed, but set it only if the new texture is different #MaterialTextureLayoutWater
    if (base_0.is()) {
        if (includeTex(material.base_0_tex)) {
            base_0.copy(base_0, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
            saveTex(base_0, material.base_0_tex);
    ...
    // base 1
    old_tex_id = material.base_1_tex;
    ImageProps(base_1, &material.base_1_tex, &ct, MTRL_BASE_1 | WATER_MTRL);
    if (old_tex_id != material.base_1_tex)
        material.normal_map_time.getUTC(); // in order for 'base_1_tex' to sync, a base 1 texture time must be changed, but set it only if the new texture is different #MaterialTextureLayoutWater
    if (base_1.is()) {
        if (includeTex(material.base_1_tex)) {
            base_1.copy(base_1, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
            saveTex(base_1, material.base_1_tex);
    ...
    // base 2
    old_tex_id = material.base_2_tex;
    ImageProps(base_2, &material.base_2_tex, &ct, MTRL_BASE_2 | WATER_MTRL);
    if (old_tex_id != material.base_2_tex)
        material.smooth_map_time.getUTC(); // in order for 'base_2_tex' to sync, a base 2 texture time must be changed, but set it only if the new texture is different #MaterialTextureLayoutWater
    if (base_2.is()) {
        if (includeTex(material.base_2_tex)) {
            base_2.copy(base_2, -1, -1, -1, ct, IMAGE_2D, 0, FILTER_BEST, IC_WRAP);
            saveTex(base_2, material.base_2_tex);
...
TEX_FLAG ProjectEx::mtrlCreateDetailTexture(EditMaterial &material) {
    ...
    // process resize
    if (color_ok)
        color.setFrom(color_resize);
    if (smooth_ok)
        smooth.setFrom(smooth_resize);
    if (bump_ok)
        bump.setFrom(bump_resize);
    if (normal_ok)
        normal.setFrom(normal_resize);

    if (!bump_ok && !material.detail_normal.is())
        normal_ok = false; // if bump map failed to load, and there is no dedicated normal map, and since it's possible that normal was created from the bump, which is not available, so normal needs to be marked as failed
    ...
    if (color_ok || smooth_ok || bump_ok || normal_ok) // proceed only if succeeded with setting anything, this is to avoid clearing existing texture when all failed to load, continue if at least one succeeded, in case the image is different while others will be extracted from old version
    {
        ...
        Image detail;
        textures = CreateDetailTexture(detail, color, bump, normal, smooth);
        IMAGE_TYPE ct;
        ImageProps(detail, &material.detail_tex, &ct, MTRL_DETAIL);
        material.detail_map_time.getUTC(); // in order for 'detail_tex' to sync, 'detail_map_time' time must be changed
        if (detail.is()) {
...
TEX_FLAG ProjectEx::mtrlCreateMacroTexture(EditMaterial &material) {
    Image macro;
    if (loadImages(macro, null, material.macro_map, true, false)) // proceed only if loaded ok
    {
        macro.resize(NearestPow2(macro.w()), NearestPow2(macro.h()), FILTER_BEST, IC_WRAP);
        IMAGE_TYPE ct;
        ImageProps(macro, &material.macro_tex, &ct, MTRL_MACRO);
        material.macro_map_time.getUTC(); // in order for 'macro_tex' to sync, 'macro_map_time' time must be changed
        if (macro.is()) {
...
TEX_FLAG ProjectEx::mtrlCreateEmissiveTexture(EditMaterial &material) {
    Image emissive;
    if (loadImages(emissive, null, material.emissive_map, true, false)) // proceed only if loaded ok
    {
        emissive.resize(NearestPow2(emissive.w()), NearestPow2(emissive.h()));
        IMAGE_TYPE ct;
        ImageProps(emissive, &material.emissive_tex, &ct, MTRL_EMISSIVE);
        material.emissive_map_time.getUTC(); // in order for 'emissive_tex' to sync, 'emissive_map_time' time must be changed
        if (emissive.is()) {
...
bool ProjectEx::mtrlReloadTextures(C UID &elm_id, bool base, bool detail, bool macro, bool emissive) {
    if (!base && !detail && !macro && !emissive)
        return true; // nothing to reload
    if (Elm *elm = findElm(elm_id)) {
        if (MtrlEdit.elm_id == elm_id) {
            MtrlEdit.undos.set("EI");
            if (base)
                MtrlEdit.rebuildBase(TEXF_NONE, 0, false, true);
            if (detail)
                MtrlEdit.rebuildDetail();
            if (macro)
                MtrlEdit.rebuildMacro();
            if (emissive)
                MtrlEdit.rebuildEmissive(TEXF_NONE, false);
            return true;
        }
        if (WaterMtrlEdit.elm_id == elm_id) {
            WaterMtrlEdit.undos.set("EI");
            if (base)
                WaterMtrlEdit.rebuildBase(TEXF_NONE, 0, false, true);
            if (detail)
                WaterMtrlEdit.rebuildDetail();
            if (macro)
                WaterMtrlEdit.rebuildMacro();
            if (emissive)
                WaterMtrlEdit.rebuildEmissive(TEXF_NONE, false);
            return true;
        }
        switch (elm->type) {
        case ELM_MTRL: {
            ...
            // reload
            if (base)
                mtrlCreateBaseTextures(edit);
            if (detail)
                mtrlCreateDetailTexture(edit);
            if (macro)
                mtrlCreateMacroTexture(edit);
            if (emissive)
                mtrlCreateEmissiveTexture(edit);
...
void ProjectEx::mtrlReloadTextures(C MemPtr<UID> &elm_ids, bool base, bool detail, bool macro, bool emissive) {
    FREPA(elm_ids)
    mtrlReloadTextures(elm_ids[i], base, detail, macro, emissive);
}
bool ProjectEx::skelGet(C UID &elm_id, Skeleton &skel) C {
    if (C Elm *elm = findElm(elm_id, ELM_SKEL)) {
        ...
        if (C Skeleton *cached = Skeletons.find(file)) {
            skel = *cached;
            return true;
        }
...
bool ProjectEx::animGet(C UID &elm_id, Animation &anim) C {
    if (C Elm *elm = findElm(elm_id, ELM_ANIM))
        return anim.load(gamePath(*elm));
    return false;
}
...
void ProjectEx::setAnimParams(Elm &elm_anim) {
    if (ElmAnim *anim_data = elm_anim.animData())
        if (Animation *anim = Animations.get(gamePath(elm_anim.id)))
            if (anim->loop() != anim_data->loop() || anim->linear() != anim_data->linear()) {
                anim->loop(anim_data->loop()).linear(anim_data->linear());
                Save(*anim, gamePath(elm_anim.id));
                savedGame(elm_anim);
            }
}
void ProjectEx::animClip(C MemPtr<UID> &elm_ids) {
    ...
    if (Elm *anim = findElm(elm_ids[i], ELM_ANIM))
        if (ElmAnim *anim_data = anim->animData()) {
            ...
            anim_data->file_time.getUTC();
            animation->clipAuto();
            Save(*animation, gamePath(anim->id));
            savedGame(*anim);
...
void ProjectEx::animSetTargetSkel(C MemPtr<UID> &anim_ids, C UID &skel_id) {
    bool changed = false;
    Elm *skel_elm = findElm(skel_id, ELM_SKEL);
    UID sid = (skel_elm ? skel_elm->id : UIDZero);
    Skeleton *skel;
    if (RenameAnimBonesOnSkelChange) {
        skel = Skeletons(gamePath(sid));
        if (ObjEdit.skel_elm == skel_elm)
            ObjEdit.flushMeshSkel();
    } // !! if this skeleton is currently edited, then we have to flush it, for example, it could have been empty at the start and we've just added bones, so 'new_skel' is empty and bones are only in 'ObjEdit.mesh_skel_temp' memory, and before changing 'anim_data.skel_id' !!
    REPA(anim_ids)
    if (Elm *anim = findElm(anim_ids[i]))
        if (ElmAnim *anim_data = anim->animData())
            if (anim_data->skel_id != sid) {
                ...
                if (skel_elm)
                    anim_data->transform = skel_elm->skelData()->transform;
...
void ProjectEx::transformSet(C MemPtr<UID> &elm_ids) {
    if (!ObjEdit.obj_elm)
        Gui.msgBox(S, "No object is currently opened");
    else {
        Pose pose = PoseIdentity;
        if (ObjEdit.mesh_elm)
            if (ElmMesh *mesh_data = ObjEdit.mesh_elm->meshData())
                pose = mesh_data->transform;
        REPA(elm_ids)
        if (Elm *obj = findElm(elm_ids[i]))
            if (ElmObj *obj_data = obj->objData())
                if (Elm *mesh = findElm(obj_data->mesh_id))
                    if (ElmMesh *mesh_data = mesh->meshData())
                        if (mesh_data->canHaveCustomTransform())
                            if (ObjEdit.mesh_elm != mesh)
                                if (mesh_data->transform != pose) {
                                    mesh_data->transform = pose;
                                    mesh_data->transform_time.getUTC();
                                    mesh_data->newVer();
                                    makeGameVer(*mesh);
...
void ProjectEx::transformApply(C MemPtr<UID> &elm_ids) {
    if (!ObjEdit.obj_elm)
        Gui.msgBox(S, "No object is currently opened");
    else if (ObjEdit.mode() != ObjView::TRANSFORM)
        Gui.msgBox(S, "Transform mode is not enabled");
    else if (ObjEdit.trans.trans == PoseIdentity)
        Gui.msgBox(S, "Transformation is empty");
    else {
...
void ProjectEx::transformRotYMinBox(C MemPtr<UID> &elm_ids) {
    ...
    if (Elm *obj = findElm(elm_ids[i]))
        if (ElmObj *obj_data = obj->objData())
            if (Elm *mesh = findElm(obj_data->mesh_id))
                if (ElmMesh *mesh_data = mesh->meshData())
                    if (mesh_data->canHaveCustomTransform() && mesh_data->box.valid() && meshGet(mesh->id, edit)) {
                        Matrix matrix = mesh_data->transform();
                        C MeshLod &lod = edit;
                        vtxs.reserve(lod.vtxs());
                        REPA(lod) {
                            C MeshPart &part = lod.parts[i];
                            if (!(part.part_flag & MSHP_HIDDEN))
                                if (C Vec *src = part.base.vtx.pos()) {
                                    int vs = part.base.vtxs();
                                    Vec2 *dest = &vtxs[vtxs.addNum(vs)];
                                    REP(vs)
                                    dest[i] = (src[i] * matrix.orn()).xz();
                                }
                        }
                        Vec2 axis;
                        if (BestFit(vtxs.data(), vtxs.elms(), axis)) {
                            flt a = Angle(axis);
                            a = Frac(a, PI_2);
                            if (a > EPS && a < PI_2 - EPS) {
                                if (a > PI_4)
                    ...
                    if (Elm *mesh = findElm(obj_data->mesh_id))
                        if (ElmMesh *mesh_data = mesh->meshData()) {
                            ...
                            if (mesh_data->body_id != actual_body_id) {
                                if (obj == ObjEdit.obj_elm)
                                    ObjEdit.setBody(actual_body_id);
                                else {
                                    mesh_data->newVer();
...
void ProjectEx::soundImportAs(C MemPtr<UID> &elm_ids, SOUND_CODEC codec, int rel_bit_rate) {
    Memt<UID> reloading;
    FREPA(elm_ids)
    if (Elm *elm = findElm(elm_ids[i], ELM_SOUND)) {
        Mems<FileParams> files = FileParams::Decode(elm->srcFile());
        if (files.elms()) {
            FileParams &file = files[0];
            if (codec)
                file.getParam("codec").setValue((codec == SOUND_WAV) ? "raw" : CodecName(codec));
            else
                file.params.removeData(file.findParam("codec"), true);
            if (rel_bit_rate >= 0)
                file.getParam("relBitRate").setValue(rel_bit_rate);
            else
                file.params.removeData(file.findParam("relBitRate"), true);
            file.params.removeData(file.findParam("relativeBitRate"), true);
            elm->setSrcFile(FileParams::Encode(files));
            Server.setElmShort(elm->id);
            reloading.add(elm->id);
        }
    }
    elmReload(reloading);
}
void ProjectEx::mulSoundVolume(C MemPtr<UID> &elm_ids, flt volume) {
    if (!Equal(volume, 1) && volume >= 0) {
        ...
        if (Elm *elm = findElm(elm_ids[i], ELM_SOUND)) {
            ...
            flt v = volume;
            if (C TextParam *p = FindTransform(fps, "volume"))
                v *= p->asFlt();
            if (Equal(v, 1))
                DelTransform(fps, "volume");
            else
                SetTransform(fps, "volume", TextReal(v, -3));

            elm->setSrcFile(FileParams::Encode(fps));
            Server.setElmShort(elm->id);
            changed.add(elm->id);
        }
        elmReload(changed);
...
void ProjectEx::adjustAnimations(C UID &skel_id, C EditSkeleton &old_edit_skel, C Skeleton &old_skel, C Skeleton &new_skel, C MemPtr<Mems<IndexWeight>> &bone_weights, int old_bone_as_root) {
    ...
    full_difference:
        ...
        REPAD(ob, old_skel.bones)                 // check only old bones, because we have animations only for old bones
        {
            ...
            int new_bone_i = -1;
            REPD(nb, Min(bone_weights.elms(), new_skel.bones.elms())) {
                C Mems<IndexWeight> &weights = bone_weights[nb];
                if (weights.elms() > 1)
                    goto full_difference; // if there's any new bone with more than 1 weight, then we have a full difference
                if (weights.elms() && weights[0].index == ob) {
                    if (new_bone_i >= 0)
                        goto full_difference; // if there was already another new bone referencing this old bone, then we have a full difference
                    new_bone_i = nb;
                }
            }
            if (new_bone_i < 0)
                goto full_difference; // bone not found = full difference
            C SkelBone &new_bone = new_skel.bones[new_bone_i];

            if (Dot(old_bone.dir, new_bone.dir) < EPS_COS                                            // orientation was changed
                || Dot(old_bone.perp, new_bone.perp) < EPS_COS || !Equal(old_bone.pos, new_bone.pos) // position    was changed
            ...
            {
                if (!InRange(new_bone.parent, bone_weights))
                    goto full_difference; // can't access new parent
                C Mems<IndexWeight> &new_parent = bone_weights[new_bone.parent];
                if (new_parent.elms() != 1 || new_parent[0].index != old_bone.parent)
                    goto full_difference; // if new parent doesn't point to old parent
            }
    ...
    if (difference) // if there's any difference
        REPA(elms)  // iterate all elements
        {
            ...
            if (ElmAnim *anim_data = anim_elm.animData())
                if (anim_data->skel_id == skel_id) // process animations using this skeleton
                {
                    ...
                    if (anim->is()) // process only if has any data (this also skips animations that haven't finished downloading from the server)
                    {
                        if (difference == 1) // if difference is only in name/types
                            REPA(anim->bones) {
                                ...
                                if (InRange(old_bone_i, old_to_new)) {
                                    int new_bone_i = old_to_new[old_bone_i];
                                    if (InRange(new_bone_i, new_skel.bones)) {
                                        bone.id() = new_skel.bones[new_bone_i];
                                        continue;
                                    } // continue so we don't remove this bone
                                }
                        ...
                        anim_data->newVer();
                        anim_data->file_time.getUTC();
                        Save(*anim, gamePath(anim_elm.id));
                        savedGame(anim_elm);
...
void ProjectEx::offsetAnimations(C Skeleton &old_skel, C Skeleton &new_skel, C UID &skel_id, C UID &ignore_anim_id) {
    REPA(elms) // iterate all project elements
    {
        Elm &anim_elm = elms[i];
        if (ElmAnim *anim_data = anim_elm.animData())
            if (anim_data->skel_id == skel_id && anim_elm.id != ignore_anim_id) // process animations using this skeleton
            {
                Animation temp, *anim = getAnim(anim_elm.id, temp);
    ...
    if (elm.type)
        if (elm.type == ELM_FOLDER || elm.type == ELM_LIB || elm.initialized()) {
            if (elm.type == ELM_WORLD)
                return elm.worldData()->valid();
            if (elm.type == ELM_OBJ) {
                ElmObj *data = elm.objData();
                if (Elm *mesh = findElm(data->mesh_id))
                    if (!validElm(*mesh))
                        return false;
            }
            if (elm.type == ELM_MESH) {
                ...
                if (Elm *skel = findElm(data->skel_id))
                    if (!validElm(*skel))
                        return false;
                if (Elm *phys = findElm(data->phys_id))
                    if (!validElm(*phys))
                        return false;
            }
            return true;
...
void ProjectEx::duplicate(Memc<UID> &ids, MemPtr<UID> duplicated, C Str &suffix) {
    ...
    Memc<UID> temp;
    if (!duplicated) {
        duplicated.point(temp);
        manually_by_user = true;
    }
    duplicated.clear();
    FREPA(ids) // !! process in order, this is important so 'duplicated' matches the 'ids' !!
    if (Elm *src = findElm(ids[i]))
        if (validElm(*src)) {
            if (src->type != ELM_CODE)
                flushElm(src->id);
            Elm &dup = super::newElm();
            dup.type = src->type;                             // set 'type' before calling 'copyParams'
            dup.copyParams(*src).setName(src->name + suffix); // call 'setName' after 'copyParams'
            duplicated.add(dup.id);
            switch (src->type) {
            case ELM_OBJ_CLASS: {
                EditObject params;
                params.load(editPath(*src));
                params.setType(true, dup.id, edit_path);
                Save(params, editPath(dup));
                makeGameVer(dup);
            } break;
            ...
            case ELM_OBJ:
                if (ElmObj *src_obj_data = src->objData())
                    if (ElmObj *dup_obj_data = dup.objData()) {
                        File f;
                        if (f.read(editPath(*src)))
                            SafeOverwrite(f, editPath(dup));                // copy obj edit file
                        if (Elm *src_mesh = findElm(src_obj_data->mesh_id)) // get source mesh
                        {
                            if (Elm *dup_mesh = getObjMeshElm(dup.id, false, false)) // grab the duplicated mesh, but don't send to the server yet
                            {
                                ...
                                if (f.read(editPath(*src_mesh)) && SafeOverwrite(f, editPath(*dup_mesh)))
                                    SavedMesh(editPath(*dup_mesh)); // copy mesh edit file
                                    ...
                                    if (ElmMesh *dup_mesh_data = dup_mesh->meshData()) {
                                        ...
                                        if (Elm *src_skel = findElm(src_mesh_data->skel_id)) // get source skel
                                        {
                                            if (Elm *dup_skel = getObjSkelElm(dup.id, false, false)) // grab the duplicated skeleton, but don't send to the server yet
                                            {
                                                ...
                                                if (f.read(editPath(*src_skel)) && SafeOverwrite(f, editPath(*dup_skel)))
                                                    SavedEditSkel(editPath(*dup_skel)); // copy skel edit file
                                        ...
                                        if (Elm *src_phys = findElm(src_mesh_data->phys_id)) // get source phys
                                        {
                                            if (Elm *dup_phys = getObjPhysElm(dup.id, false, false)) // grab the duplicated phys body, but don't send to the server yet
                                            {
                                                ...
                                                if (f.read(editPath(*src_phys)) && SafeOverwrite(f, editPath(*dup_phys)))
                                                    SavedEditPhys(editPath(*dup_phys)); // copy phys edit file
                                ...
                                dup_obj_data->mesh_id.zero(); // couldn't create a mesh
                        }
            ...
            case ELM_FILE: {
                File f;
                if (f.read(editPath(*src)))
                    SafeOverwrite(f, editPath(dup));
                if (f.read(gamePath(*src)))
                    if (SafeOverwrite(f, gamePath(dup)))
                        savedGame(dup);
            } break;
            }
    ...
    if (duplicated.elms())
        list_cur = duplicated.last();
    list_sel = duplicated;
    setList();
    activateSources(); // rebuild sources if needed
    if (manually_by_user) {
        if (duplicated.elms() == 1)
            RenameElm.activate(duplicated[0]);
        if (ElmChange *change = elm_undos.set(null, true)) {
            change->type = ElmChange::RESTORE;
            change->name = "Copy";
            REPA(duplicated)
            change->elms.binaryInclude(duplicated[i]);
        }
    }
}
...
void ProjectEx::eraseWorldAreaObjs(C UID &world_id, C VecI2 &area_xy) {
    if (world_id == WorldEdit.elm_id)
        if (Area *area = WorldEdit.findAreaLoaded(area_xy)) {
            bool erased = false;
            REPA(area->objs)
            if (Obj *obj = area->objs[i])
                if (obj->removed) {
                    erased = true;
                    WorldEdit.objs.removeData(obj, true);
                }
            if (erased)
                area->setChangedObj();
            return;
...
void ProjectEx::elmChangedParentRemovePublish(bool network) {
    ...
    if (!network)
        paramEditObjChanged(); // removed elements should display in red in param list, however disable from network because this might cause keyboard to lose focus
}
...
{
    if (elms.elms()) {
        ElmChange *change = null;
        if (as_undo)
            if (change = elm_undos.set(null, true)) {
        ...
        FREPA(elms) {
            ...
            {
                if (adjust_elms)
                    elms[i].value = elm->parent_id; // !! IF WE'RE ADJUSTING, THEN REMEMBER CURRENT PARENT, BEFORE SETTING 'dest' !!
                if (!dest || ElmCanHaveChildren(dest->type))
                    if (ElmMovable(elm->type))
                        if (!contains(*elm, dest)) {
                            if (change) {
...
void ProjectEx::drag(Memc<UID> &elms, GuiObj *focus_obj, C Vec2 &screen_pos) {
    ...
    {
        Elm *dest = list.visToElm(list.screenToVis(screen_pos));
        if (dest)
            dest->opened(true);
        if (!dest || ElmCanHaveChildren(dest->type)) {
            ElmChange *change = elm_undos.set(null, true);
...
void ProjectEx::collapse(Memc<UID> &ids, Memc<EEItem *> &items) {
    setListCurSel();
    REPA(ids)
    if (Elm *elm = findElm(ids[i]))
        elm->opened(false);
    REPAO(items)->opened = false;
    setList(false, false);
}
void ProjectEx::expand(Memc<UID> &ids, Memc<EEItem *> &items) {
    setListCurSel();
    REPA(ids)
    if (Elm *elm = findElm(ids[i]))
        elm->opened(true);
    REPAO(items)->opened = true;
    setList(false, false);
}
void ProjectEx::expandAll(Memc<UID> &ids, Memc<EEItem *> &items) {
    setListCurSel();
    REPA(ids) {
        int e = findElmI(ids[i]);
        if (InRange(e, hierarchy)) {
...
void ProjectEx::floodExisting(ElmNode &node, uint publish_flag, bool parent_publish) {
    REPA(node.children) {
        ...
        if (elm.exists()) {
            ...
            if (elm_publish)
                elm.finalPublish(true);
            switch (elm.type) {
            case ELM_ENUM:
                existing_enums.binaryInclude(elm.id);
                break;
            case ELM_OBJ_CLASS:
                existing_obj_classes.binaryInclude(elm.id);
                break;
            case ELM_FONT:
                if (elm_publish)
                    publish_fonts.binaryInclude(elm.id);
                break;
            case ELM_APP:
                existing_apps.binaryInclude(elm.id);
                break;
            }
...
void ProjectEx::setExisting() {
    ...
    existing_apps.clear();
    Memc<UID> old_enums;
    Swap(existing_enums, old_enums); // this also clears 'existing_enums'
    Memc<UID> old_obj_classes;
    Swap(existing_obj_classes, old_obj_classes); // this also clears 'existing_obj_classes'
    Memc<UID> old_fonts;
    Swap(publish_fonts, old_fonts); // this also clears 'publish_fonts'
    floodExisting(root, PublishableFlag());
    if (!Same(existing_enums, old_enums) || !Same(existing_obj_classes, old_obj_classes))
        enumChanged();
    if (!Same(publish_fonts, old_fonts))
        fontChanged();
}
...
void ProjectEx::enumChanged(C UID &enum_id) // 'enum_id' can also point to OBJ_CLASS
{
    ...
    Memt<Str> obj_classes;
    FREPA(existing_obj_classes)
    if (Elm *obj_class = findElm(existing_obj_classes[i]))
        obj_classes.binaryInclude(obj_class->name, CompareEnum);
    if (InRange(OBJ_ACCESS_TERRAIN, ObjAccessNamesElms))
        obj_class_node += ObjAccessNames[OBJ_ACCESS_TERRAIN];
    if (InRange(OBJ_ACCESS_GRASS, ObjAccessNamesElms))
        obj_class_node += ObjAccessNames[OBJ_ACCESS_GRASS];
    if (InRange(OBJ_ACCESS_OVERLAY, ObjAccessNamesElms))
        obj_class_node += ObjAccessNames[OBJ_ACCESS_OVERLAY];
    if (obj_classes.elms())
        obj_class_node++;
    FREPA(obj_classes)
    obj_class_node += obj_classes[i];

    param_type_node.children.clear();
    ASSERT(PARAM_BOOL == 0);
    (param_type_node += "Bool").desc("Boolean value\nFalse/True");
    ASSERT(PARAM_INT == 1);
    (param_type_node += "Int").desc("Integer value");
    ASSERT(PARAM_FLT == 2);
    (param_type_node += "Real").desc("Floating point value");
    ASSERT(PARAM_STR == 3);
    (param_type_node += "String").desc("Text string");
    {
        ASSERT(PARAM_ENUM == 4);
        Node<MenuElm> &e = param_type_node += "Enum";
        e.flag(e.flag() | MENU_NOT_SELECTABLE).desc("Custom enum");
        Memt<Str> enums;
        FREPA(existing_enums)
        if (Elm *e = findElm(existing_enums[i]))
            enums.binaryInclude(e->name, CompareEnum);
        FREPA(enums)
        e += enums[i];
    }
    ASSERT(PARAM_VEC2 == 5);
    (param_type_node += "Vec2").desc("2D vector");
    ASSERT(PARAM_VEC == 6);
    (param_type_node += "Vec").desc("3D vector");
    ASSERT(PARAM_VEC4 == 7);
    (param_type_node += "Vec4").desc("4D vector");
    ASSERT(PARAM_COLOR == 8);
    (param_type_node += "Color").desc("Color value");
    ASSERT(PARAM_ID == 9);
    (param_type_node += "Element ID").desc("Project element or custom ID\nIn order to set its value please drag and drop a project element to the textline, or manually enter an ID \"UID(..)\"\nYou can drag and drop multiple elements at the same time."); // can't use slash in the name because ComboBox doesn't support it
...
void ProjectEx::fontChanged() {
    ...
    Memt<Str> names;
    FREPA(publish_fonts)
    if (Elm *font = findElm(publish_fonts[i]))
        names.binaryInclude(font->name, CompareEnum);
...
void ProjectEx::DragElmsStart(ProjectEx &proj) {
    ...
    FREPA(proj.list.sel)
    if (ListElm *list_elm = proj.list.absToData(proj.list.sel[i]))
        if (list_elm->elm)
            proj.drag_list_sel.add(list_elm->elm->id); // list in order
...
void ProjectEx::dragElmsStart() {
    // get element types
    bool elm_types[ELM_NUM_ANY];
    Zero(elm_types);
    REPA(drag_list_sel)
    if (Elm *elm = findElm(drag_list_sel[i]))
        elm_types[elm->type] = true;
...
void ProjectEx::update() {
            ...
            if (file_size_getter.get()) {
                if (file_size_getter_step == 0) // game files
                {
                    REPA(file_size_getter.elms) {
                        ...
                        if (Elm *elm = findElm(file_size.id))
                            if (elm->file_size < 0)
                                elm->file_size = file_size.file_size; // set file size only if unknown, because if it's known, then most likely it has more recent info than from 'FileSizeGetter'
                    }
                } else // textures
                {
                    REPA(file_size_getter.elms) {
    ...
    if (region.contains(Gui.ms())) {
        if (Ms.bp(1)) {
            ...
            {
                list.cur = list.lit;
                if (!list.sel.has(list.visToAbs(list.lit)))
                    list.setCur(list.lit);
                elmMenu(Ms.pos());
            }
        } else if (Ms.bp(2)) {
            if (Kb.ctrlCmd()) // copy elm ID to clipboard
            {
                ...
                Misc.hide_proj.push(); // hide project view
        } else if (Ms.bp(MS_BACK)) {
            if (ListElm *elm = list.litToListElm()) {
                if (!elm->close()) // try closing highlighted element
                    for (; --list.lit >= 0;)
                        if (ListElm *e = list.litToListElm())
                            if (e->offset < elm->offset && e->close())
                                break; // find first parent
                refresh(false, false);
    ...
    REPA(Touches)
    if (Touches[i].on() && Touches[i].guiObj() == &list && Touches[i].life() >= LongPressTime && !Touches[i].dragging() && !Gui.dragging() && !Gui.menu()) {
...
int ProjectEx::CompareChildren(C int &a, C int &b) {
    ...
    if (int c = Compare(ElmOrder(ea.type), ElmOrder(eb.type)))
        return c;
    if (int c = CompareNumber(ea.name, eb.name))
        return c;
    return Compare(ea.id, eb.id);
}
int ProjectEx::CompareChildren(C EEItem &a, C Elm &b) {
    if (int c = Compare(ElmOrder(a.type), ElmOrder(b.type)))
        return c;
    if (int c = Compare(a.base_name, b.name))
        return c;
    return 0;
}
bool ProjectEx::hasInvalid(ElmNode &node) {
    REPA(node.children) {
        ...
        if (elm.finalPublish() && ElmVisible(elm.type)) // could use "elm.exists() && elm.publish()" here, however we need platform
            if (invalidRefs(elm) || hasInvalid(hierarchy[child_i]))
                return true;
    }
    return false;
}
...
void ProjectEx::getActiveAppElms(Memt<Elm *> &app_elms, ElmNode &node, uint publish_flag, C UID &app_id, bool inside_valid) {
    REPA(node.children) {
        ...
        if (!(elm.flag & publish_flag)) // exists and publishable, "!" because we have NO_PUBLISH* flags
        {
            if (inside_valid)
                app_elms.add(&elm);
            getActiveAppElms(app_elms, hierarchy[child_i], publish_flag, app_id, (elm.type == ELM_LIB) ? true : (elm.type == ELM_APP) ? (elm.id == app_id)
                                                                                                                                      : inside_valid); // include elements from all libraries and from active app only, in other case inherit valid from the parent
        }
    }
}
...
void ProjectEx::activateSources(int rebuild) // -1=never, 0=auto, 1=always
{
    ...
    if (app_id != cur_app) {
        CodeEdit.makeAuto();
        app_id = cur_app;
    } // set last app id to currently available, 'makeAuto' because 'APP_NAME' relies on active app
    CodeEdit.clearActiveSources();
    Memt<Elm *> app_elms;
    getActiveAppElms(app_elms, CodeEdit.configEXE());
    FREPA(app_elms)
    if (app_elms[i]->type == ELM_CODE)
        CodeEdit.activateSource(app_elms[i]->id);
    if (rebuild >= 0)
        CodeEdit.activateApp(rebuild >= 1);
    else
        CodeEdit.makeAuto(); // if not activating app then call 'makeAuto' which will activate the auto header
}
...
void ProjectEx::setList(EEItem &item, int depth, bool parent_contains_name) {
    ...
    if (filter().is()) {
        ...
        if (!(child_contains_name || this_contains_name || parent_contains_name))
            return;
    }
    ...
    if (opened) // list children
        FREPA(item.children)
    setList(item.children[i], depth + 1, parent_contains_name || this_contains_name);
}
    ...
    REPA(node.children) {
        ...
        if (ElmVisible(elm.type)) {
            ...
            if (elm.exists() || show_removed() || FlagOn(child.flag, ELM_EDITED | ELM_EDITED_CHILD))
                if (list.show_elm_type[elm.type] || hasVisibleChildren(child))
                    return true;
        }
    }
    return false;
...
void ProjectEx::setList(ElmNode &node, int depth, int vis_parent, bool parent_contains_name, bool parent_no_publish_all) {
    ...
    EEItem *ee = null;
    if (&node == &root && CodeEdit.items.elms() && list.show_all_elm_types)
        ee = &CodeEdit.items.first();
    FREPA(node.children) {
        int child_i = node.children[i];
        ElmNode &child = hierarchy[child_i];
        Elm &elm = elms[child_i];
        if (ee && CompareChildren(*ee, elm) < 0) {
            setList(*ee, depth, parent_contains_name);
            ee = null;
        } // if "Engine" item should be included before this element
        if (elm.exists() || show_removed() || FlagOn(child.flag, ELM_EDITED | ELM_EDITED_CHILD))
            if (ElmVisible(elm.type)) {
                bool this_contains_name = false, child_contains_name = false;
                if (filter().is()) {
                ...
                if (list.show_elm_type[elm.type]) {
                    ...
                    if (exists) {
                                        ...
                                        if (opened && list.show_all_elms)                               // if element is opened (then it means we've listed its children), and if all elements are listed, then we can use an optimization by summing just the first children sizes from the list, instead of all children recursively, because those first children will already include their children sizes
                                            for (int i = elm_list_index + 1; i < list.data.elms(); i++) // iterate all elements added after this one (this will be its children and their children)
                                            {
                                                ...
                                                if (child.depth == depth + 1)
                                                    e.includeSize(child); // add only for direct children (and not children of children)
                                            }
                                        else {
                                            includeElmFileSize(e, child);
                                            included_all_children = true;
                                        } // need to process all children recursively
                                    }
                        ...
                        } else if (no_publish_all) // disabled for all platforms
                        {
                            list.warnings.New().create(elm_list_index, false, e.offset);
                            e.desc = "Element will not be included in publishing";
                        } else if (!publish) // disabled for this platform
                        {
                            ElmList::Warning &warning = list.warnings.New();
                            warning.create(elm_list_index, false, e.offset + list.columnWidth(list.icon_col));
                            warning.color.a = 128; // half-transparent on right side
                        ...
                        if (elm.type == ELM_APP && existing_apps.elms() > 1) // create checkboxes only if there are more than 1 apps
                        {
                            ...
                            app_check.create().func(ActivateApp, app_check).desc("Activate this application");
                            app_check.app_id = elm.id;
                            app_check.setRect();
...
ProjectEx &ProjectEx::editing(C UID &elm_id, bool force_open_parents) {
    int i = findElmI(elm_id);
    if (InRange(i, hierarchy)) {
        hierarchy[i].flag |= ELM_EDITED;
        if (force_open_parents)
            for (i = hierarchy[i].parent; InRange(i, hierarchy); i = hierarchy[i].parent)
                hierarchy[i].flag |= ELM_EDITED_CHILD;
    }
    return T;
}
...
ProjectEx &ProjectEx::editing(C Str &name) {
    editing(CodeEdit.items, name);
    return T;
}
void ProjectEx::setList(bool set_hierarchy, bool set_existing) // !! CALL 'setListCurSel' or 'clearListSel' before this !!
{
    if (set_hierarchy)
        ...
        REPAO(hierarchy).flag = 0; // if no need to set whole 'hierarchy' then reset flag manually
    if (set_existing)
        setExisting();                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     // after hierarchy
    ...
    if (filter().is()) {
        ...
        setFilter(root);
        filter_path.clear();
        setFilter(CodeEdit.items);
    } else
        filter_is_id = false; // if there's no filter then clear 'filter_is_id' because some codes may check it without checking for "filter().is()"
    list.show_all_elm_types = !(list.show_only_folder || list.show_only_obj || list.show_only_mtrl || list.show_only_anim || list.show_only_sound);
    REPAO(list.show_elm_type) = list.show_all_elm_types;
    list.flat_is = (list.flat_want && !list.show_all_elm_types);
    list.list_all_children = list.flat_is;
    if (list.show_only_folder)
        list.show_elm_type[ELM_FOLDER] = true;
    if (list.show_only_obj)
        list.show_elm_type[ELM_OBJ] = true;
    if (list.show_only_mtrl)
        list.show_elm_type[ELM_MTRL] = true;
    if (list.show_only_anim)
        list.show_elm_type[ELM_ANIM] = true;
    if (list.show_only_sound)
        list.show_elm_type[ELM_SOUND] = true;
    ...
    if (list.app_checks.elms()) {
        UID cur_app = curApp();
        REPAO(list.app_checks).set(list.app_checks[i].app_id == cur_app, QUIET);
    } // set checkboxes state
    if (lit_elm_id.valid() || lit_elm_name.is())
        elmHighlight(lit_elm_id, lit_elm_name, true);

    // set cur/sel
    list.cur = (list_cur.valid() ? list.elmToVis(findElm(list_cur)) : list.itemToVis(list_cur_item));
    list_cur.zero();
    list_cur_item = null;
    list.sel.clear();
    FREPA(list_sel) {
        int abs = list.elmToAbs(findElm(list_sel[i]));
        if (abs >= 0)
            list.sel.add(abs);
    }
    FREPA(list_sel_item) {
        int abs = list.itemToAbs(list_sel_item[i]);
        if (abs >= 0)
            list.sel.add(abs);
    }
    clearListSel();
...
void ProjectEx::setListSel(Memc<UID> &list_sel, MemPtr<EEItem *> list_sel_item) {
    ...
    list_sel_item.clear();
    FREPA(list.sel)
    if (ListElm *list_elm = list.absToData(list.sel[i])) {
        if (list_elm->elm)
            list_sel.add(list_elm->elm->id);
        if (list_elm->item && list_sel_item)
            list_sel_item.add(list_elm->item);
    }
}
...
ProjectEx &ProjectEx::refresh(bool set_hierarchy, bool set_existing) {
    setListCurSel();
    setList(set_hierarchy, set_existing);
    return T;
}
void ProjectEx::elmOpenParents(C UID &id, bool set_list) {
    if (Elm *elm = findElm(id))
        for (; elm = findElm(elm->parent_id);)
            elm->opened(true);
    if (set_list)
        setList(false, false);
}
...
bool ProjectEx::elmReload(C MemPtr<UID> &elm_ids, bool remember_result, bool refresh_elm_list) {
    ...
    REPA(elm_ids)
    if (Elm *elm = findElm(elm_ids[i])) {
        if (elm->type != ELM_FOLDER && elm->type != ELM_LIB && elm->type != ELM_APP)
            elm->importing(true);
    } else
        ok = false;
...
void ProjectEx::elmLocate(C UID &id, bool set_cur) {
    for (Elm *elm = findElm(id); elm; elm = findElm(elm->parent_id)) {
        int vis = list.elmToVis(elm);
        if (vis >= 0) {
            if (set_cur)
                list.setCur(vis);
            list.scrollTo(vis, false, 1);
            return;
        }
    }
}
void ProjectEx::elmHighlight(C UID &id, C Str &name, bool force) {
    if (lit_elm_id != id || !EqualPath(lit_elm_name, name) || force) {
        ...
        for (Elm *elm = findElm(id); elm; elm = findElm(elm->parent_id)) {
            int vis = list.elmToVis(elm);
            if (InRange(vis, list.data)) {
                list.data[vis].highlight();
                return;
            }
        }
        if (name.is())
            if (EEItem **item_ptr = CodeEdit.items_sorted.binaryFind(name, CodeView::CompareItem))
                for (EEItem *item = *item_ptr; item; item = item->parent) {
                    int vis = list.itemToVis(item);
                    if (InRange(vis, list.data)) {
                        list.data[vis].highlight();
                        return;
                    }
...
void ProjectEx::elmToggle(Elm *elm) {
    if (elm)
        ...
        case ELM_MTRL:
            ...
            break;
        case ELM_WATER_MTRL:
            WaterMtrlEdit.toggle(elm);
            break;
        case ELM_PHYS_MTRL:
            PhysMtrlEdit.toggle(elm);
            break;
        case ELM_OBJ:
            ObjEdit.toggle(elm);
            break;
        case ELM_WORLD:
            WorldEdit.toggle(elm);
            break;
        case ELM_MINI_MAP:
            MiniMapEdit.toggle(elm);
            break;
        case ELM_ENV:
            EnvEdit.toggle(elm);
            break;
        case ELM_ENUM:
            ...
            break;
        case ELM_IMAGE:
            ImageEdit.toggle(elm);
            break;
        case ELM_IMAGE_ATLAS:
            ImageAtlasEdit.toggle(elm);
            break;
        case ELM_ICON:
            IconEdit.toggle(elm);
            break;
        case ELM_ICON_SETTS:
            IconSettsEdit.toggle(elm);
            break;
        case ELM_FONT:
            FontEdit.toggle(elm);
            break;
        case ELM_TEXT_STYLE:
            TextStyleEdit.toggle(elm);
            break;
        case ELM_PANEL_IMAGE:
            PanelImageEdit.toggle(elm);
            break;
        case ELM_PANEL:
            PanelEdit.toggle(elm);
            break;
        case ELM_GUI_SKIN:
        ...
        case ELM_GUI:
            ...
            break;
        case ELM_OBJ_CLASS:
            ObjClassEdit.toggle(elm);
            break;
        case ELM_SOUND:
            SoundEdit.toggle(elm);
            break;
        case ELM_VIDEO:
            VideoEdit.toggle(elm);
            break;
        case ELM_ANIM:
            AnimEdit.toggle(elm);
            break;
        case ELM_CODE:
            ...
            break;
        case ELM_APP:
            AppPropsEdit.toggle(elm);
            break;

        case ELM_FILE:
            elmLocate(elm->id, true);
            break; // TODO: in the future replace with some Hex Viewer
        }
}
void ProjectEx::elmToggle(ListElm *elm) {
    if (elm) {
        if (elm->elm)
            ...
            elmToggle(elm->item); // use else because after first toggle the pointer may be invalid
    }
}
...
void ProjectEx::elmActivate(Elm *elm) {
    if (elm)
        switch (elm->type) {
        case ELM_MTRL:
            MtrlEdit.activate(elm);
            break;
        case ELM_WATER_MTRL:
            WaterMtrlEdit.activate(elm);
            break;
        case ELM_PHYS_MTRL:
            PhysMtrlEdit.activate(elm);
            break;
        case ELM_OBJ:
            ObjEdit.activate(elm);
            break;
        case ELM_WORLD:
            WorldEdit.activate(elm);
            break;
        case ELM_MINI_MAP:
            MiniMapEdit.activate(elm);
            break;
        case ELM_ENV:
            EnvEdit.activate(elm);
            break;
        case ELM_ENUM:
            EnumEdit.activate(elm);
            break;
        case ELM_IMAGE:
            ImageEdit.activate(elm);
            break;
        case ELM_IMAGE_ATLAS:
            ImageAtlasEdit.activate(elm);
            break;
        case ELM_ICON:
            IconEdit.activate(elm);
            break;
        case ELM_ICON_SETTS:
            IconSettsEdit.activate(elm);
            break;
        case ELM_FONT:
            FontEdit.activate(elm);
            break;
        case ELM_TEXT_STYLE:
            TextStyleEdit.activate(elm);
            break;
        case ELM_PANEL_IMAGE:
            PanelImageEdit.activate(elm);
            break;
        case ELM_PANEL:
        ...
        case ELM_GUI_SKIN:
            ...
            break;
        case ELM_GUI:
            GuiEdit.activate(elm);
            break;
        case ELM_OBJ_CLASS:
            ObjClassEdit.activate(elm);
            break;
        case ELM_SOUND:
            SoundEdit.activate(elm);
            break;
        case ELM_VIDEO:
            VideoEdit.activate(elm);
            break;
        case ELM_ANIM:
            AnimEdit.activate(elm);
            break;
        case ELM_CODE:
            CodeEdit.activate(elm);
            break;
        case ELM_APP:
            AppPropsEdit.activate(elm);
            break;
        }
}
void ProjectEx::elmNext(C UID &elm_id, int dir) {
    if (Elm *elm = findElm(elm_id)) {
        ...
        int vis = list.elmToVis(elm);
        if (vis >= 0)
            REP(list.visibleElms() - 1) {
                vis = Mod(vis + dir, list.visibleElms());
                if (Elm *elm = list.visToElm(vis))
                    if (elm->type == type) {
                        list.scrollTo(vis, false, 0.5f);
                        elmActivate(elm);
                        break;
                    }
            }
...
void ProjectEx::resize() {
    flt t = D.h(), b = -D.h(), w = Max(Misc.size().x, size().x);
    if (Misc.pos.x == 0) {
        if (Misc.pos.y == 0)
            MAX(b, Misc.rect().max.y);
        else
            MIN(t, Misc.rect().min.y);
    }
...
void ProjectEx::elmMenu(C Vec2 &pos, bool touch) {
    int items = 0, elms = 0;
    Elm *elm = null;
    bool remove = false, restore = false, expand = false, expand_all = false, collapse = false, image = false, object = false, material = false, animation = false, sound = false, cancel_reload = false;
    REPA(list.sel)
    if (ListElm *list_elm = list.absToData(list.sel[i])) {
        if (list_elm->item)
            items++;
    ...
    if (!elms && items)
        return; // don't do anything when just Items are selected
    ...
    if (!elms) {
        ...
        if (expand || expand_all || collapse) {
            if (expand)
                n.New().create("Expand", Expand, T);
            if (expand_all)
                n.New().create("Expand All", ExpandAll, T).desc("Expand all selected elements including their children");
            if (collapse)
                n.New().create("Collapse", Collapse, T);
            n++;
        }
        if (elms == 1) {
        ...
        if (cancel_reload)
            n.New().create("Cancel Reload", CancelReload, T);
        n++;
        if (elms == 1) {
        ...
        if (image || object || material || animation || sound) {
            ...
            if (sound) {
                ...
                {
                    FREP(SoundEditor::ImportAsElms)
                    s.New().create(SoundEditor::Import_as[i].name, SoundImportAs, ptr(i));
                    s++;
    ...
    if (elm_undos.changes()) {
        if (n.children.elms())
            n++;
        if (ElmChange *undo = elm_undos.getNextUndo())
            n.New().create(S + "Undo " + undo->name, UndoElmChange, T);
        if (ElmChange *redo = elm_undos.getNextRedo())
            n.New().create(S + "Redo " + redo->name, RedoElmChange, T);
    }
    setMenuListSel();
    Gui += list.menu.create(n);
    list.menu.setSize(touch).posRU(pos).moveToBottom().activate(); // move to bottom so it gets keyboard priority
}
bool ProjectEx::drop(C Str &name, C UID &parent_id, Memt<UID> &new_folders) {
    ...
    case FSTD_FILE: {
        ...
        if (!type)
            if (ext == "wav" || ext == "ogg" || ext == "flac" || ext == "opus" || ext == "mp3" || ext == "m4a" || ext == "weba")
                type = ELM_SOUND;
            else if (ext == "ogm" || ext == "ogv" || ext == "theora" || ext == "webm")
                type = ELM_VIDEO;
            else if (ext == "mtrl")
                type = ELM_MTRL;
            else if (ext == "anim")
                type = ELM_ANIM;
            else if (ext == "c" || ext == "cpp" || ext == "h" || ext == "cc" || ext == "cxx" || ext == "m" || ext == "mm")
                type = ELM_CODE;
            else if (ext == "skel" || Ends(name, ".mesh.ascii") || Ends(name, ".xps.ascii"))
                type = ELM_OBJ;
            else
                type = ELM_FILE;
        if (Elm *elm = newElm(type, parent_id, &NoTemp(GetBaseNoExt(name)), false)) {
            elm->setSrcFile(CodeEdit.importPaths(name));
            elmReload(elm->id, false, false);
            list_sel.add(elm->id); // no need to call 'Server.setElmShort' because 'Server.setElmFull' was already requested in 'newElm'
            return true;
        }
    } break;
    ...
    case FSTD_DIR: {
        bool added_folder;
        Elm &folder = getFolder(GetBase(name), parent_id, added_folder);
        folder.opened(true);
        bool added_children = false;
        for (FileFind ff(name); ff();)
            added_children |= drop(ff.pathName(), folder.id, new_folders);
        if (added_folder && !added_children)
            elms.removeData(&folder, true);
        else {
            list_sel.add(folder.id);
            if (added_folder)
                new_folders.add(folder.id);
            return true;
        }
    } break;
    }
...
void ProjectEx::drop(Memc<Str> &names, C UID &parent_id) {
    ...
    clearListSel();
    FREPA(names)
    drop(names[i], parent_id, new_folders);
    REPA(new_folders)
    if (Elm *elm = findElm(new_folders[i]))
        Server.newElm(*elm); // go from end because first are leafs
    setList();
}
...
void ProjectEx::meshSetAutoTanBin(Elm &elm, C MaterialPtr &material) {
    ...
    if (ObjEdit.mesh_elm && ObjEdit.mesh_elm->id == elm.id)
        ObjEdit.setAutoTanBin(material);
}
void ProjectEx::animTransformChanged(Elm &elm_anim) {
    super::animTransformChanged(elm_anim);
    if (&elm_anim == AnimEdit.elm)
        AnimEdit.toGui();
}
void ProjectEx::skelTransformChanged(C UID &skel_id) {
    super::skelTransformChanged(skel_id);

    if (ObjEdit.skel_elm && ObjEdit.skel_elm->id == skel_id)
        ObjEdit.skelTransformChanged();
    if (AnimEdit.skel_id == skel_id)
        AnimEdit.setMeshSkel(true);
}
...
void ProjectEx::meshChanged(Elm &mesh) {
    ...
    if (ObjEdit.mesh_elm && ObjEdit.mesh_elm->id == mesh.id)
        ObjEdit.meshChanged();
    if (AnimEdit.mesh_id == mesh.id)
        AnimEdit.setMeshSkel(true);
    if (Selection.elms() && Selection[0].mesh_proper.id() == mesh.id)
        WorldEdit.param_edit.meshVariationChanged();
}
void ProjectEx::elmChanging(Elm &elm) {
    switch (elm.type) {
    case ELM_ANIM:
        AnimEdit.elmChanging(elm.id);
        break;
    }
}
void ProjectEx::elmChanged(Elm &elm) {
    switch (elm.type) {
    case ELM_ANIM:
        setAnimTransform(elm);
        setAnimParams(elm);
        AnimEdit.elmChanged(elm.id);
        break;
    case ELM_SKEL:
        setSkelTransform(elm);
        skelChanged(elm);
        break;
    case ELM_PHYS:
        if (!setPhysTransform(elm))
            physChanged(elm);
        setPhysParams(elm);
        break;
    case ELM_MESH:
        meshTransformChanged(elm);
        break;
    case ELM_MTRL:
        mtrlSetAutoTanBin(elm.id);
        MtrlEdit.elmChanged(elm.id);
        if (MaterialPtr mtrl = MaterialPtr().find(gamePath(elm.id)))
            D.setShader(mtrl());
        break;
    case ELM_WATER_MTRL:
        WaterMtrlEdit.elmChanged(elm.id);
        break;
    case ELM_PHYS_MTRL:
        PhysMtrlEdit.elmChanged(elm.id);
        break;
    case ELM_ENUM:
        enumChanged(elm.id);
        EnumEdit.elmChanged(elm.id);
        break;
    case ELM_IMAGE:
        ImageEdit.elmChanged(elm.id);
        break;
    case ELM_IMAGE_ATLAS:
        ImageAtlasEdit.elmChanged(elm.id);
        break;
    case ELM_ICON_SETTS:
        IconSettsEdit.elmChanged(elm.id);
        break;
    case ELM_FONT:
        fontChanged();
        FontEdit.elmChanged(elm.id);
        break;
    case ELM_TEXT_STYLE:
        TextStyleEdit.elmChanged(elm.id);
        break;
    case ELM_PANEL_IMAGE:
        panelImageChanged(elm.id);
        PanelImageEdit.elmChanged(elm.id);
        break;
    case ELM_PANEL:
        panelChanged(elm.id);
        PanelEdit.elmChanged(elm.id);
        break;
    case ELM_GUI_SKIN:
    ...
    case ELM_OBJ_CLASS:
        enumChanged(elm.id);
        objChanged(elm);
        ObjClassEdit.elmChanged(elm.id);
        break;
    case ELM_SOUND:
        SoundEdit.elmChanged(elm.id);
        break;
    case ELM_VIDEO:
        VideoEdit.elmChanged(elm.id);
        break;
    case ELM_ENV:
        EnvEdit.elmChanged(elm.id);
        break;
    case ELM_WORLD:
        WorldEdit.elmChanged(elm.id);
        break;
    case ELM_MINI_MAP:
        MiniMapEdit.elmChanged(elm.id);
        break;
    case ELM_APP:
        AppPropsEdit.elmChanged(elm.id);
        break;
    }
...
void ProjectEx::receivedData(Elm &elm, File &data, File &extra) {
    ...
    if (ElmHasFile(elm.type)) {
        ...
        if (ElmSendBoth(elm.type)) {
            path = editPath(elm);
            if (SafeOverwrite(data, path))
                SavedEdit(elm.type, path);
            path = gamePath(elm);
            if (SafeOverwrite(extra, path)) {
                SavedGame(elm.type, path);
                savedGame(elm, path);
            }
        } else {
            path = basePath(elm);
            if (SafeOverwrite(data, path))
                SavedBase(elm.type, path);
        }
    }
    data.pos(0);
    makeGameVer(elm, &data);
    elmChanged(elm);
}
void ProjectEx::syncElm(Elm &elm, Elm &src, File &src_data, File &src_extra, bool sync_long) {
    ...
    switch (elm.type) {
    case ELM_MESH:
        body_id = elm.meshData()->body_id;
        break;
    case ELM_OBJ_CLASS:
        path_mode = elm.objClassData()->pathSelf();
        break;
    case ELM_OBJ:
        obj = gamePath(elm);
        terrain = *obj;
        ...
        break;
    case ELM_WORLD:
        world_valid = elm.worldData()->valid();
        elm.worldData()->copyTo(path_settings);
        break;
    }
    if (super::syncElm(elm, src, src_data, src_extra, sync_long, elm_newer_src, src_newer_elm)) {
        ...
        switch (elm.type) {
        case ELM_MESH: {
            if (elm.meshData()->body_id != body_id)
                meshTransformChanged(elm, true); // TODO: this is already called in 'elmChanged' above, however without checks for body, the right solution would be to call it only one time with body checks
        } break;

        case ELM_OBJ_CLASS: {
            if (elm.objClassData()->pathSelf() != path_mode)
                verifyPathsForObjClass(elm.id); // if syncing changed the 'path_mode' then verify world paths
        } break;

        case ELM_OBJ: {
            if (TerrainObj2(*obj) != terrain)
                rebuildEmbedForObj(elm.id); // if syncing changed the 'terrainObj' then rebuild world embedded objects
            if (PhysPath(*obj) != phys)
                rebuildPathsForObj(elm.id, true); // if syncing changed the 'physPath'   then rebuild world paths, rebuild only for objects that don't override paths (if they override then it means that changing the base doesn't affect their path mode), we must rebuild this also for objects with final path mode set to ignore, in case we've just disabled paths
        } break;

        case ELM_WORLD: {
            PathSettings temp;
            elm.worldData()->copyTo(temp);
            if (path_settings != temp)
                rebuildWorldAreas(elm);                   // if syncing changed 'path_settings' then paths need to be rebuilt
            if (!world_valid && elm.worldData()->valid()) // if world settings became valid after syncing, then rebuild some elements
                if (WorldVer *world_ver = worldVerGet(elm.id)) {
                    REPA(world_ver->lakes) {
                        ...
                        Lake lake;
                        if (lake.load(editLakePath(elm.id, water_id)))
                            if (!lake.removed)
                                rebuildWater(&lake, null, water_id, *world_ver);
                    }
                    REPA(world_ver->rivers) {
                        ...
                        River river;
                        if (river.load(editRiverPath(elm.id, water_id)))
                            if (!river.removed)
                                rebuildWater(null, &river, water_id, *world_ver);
                    }
                }
        } break;
        }
    ...
    // request from server
    if (!sync_long && (src.newerFile(elm) || src_newer_elm))
        Server.getElmLong(elm.id); // if server has newer file version than local then request it

    // send to server
    if (elm.newerFile(src) || elm_newer_src)
...
bool ProjectEx::syncWaypoint(C UID &world_id, C UID &waypoint_id, Version &src_ver, EditWaypoint &src) {
        ...
        if (Version *waypoint_ver = WorldEdit.ver->waypoints.get(waypoint_id))
            if (src_ver != *waypoint_ver)
                if (EditWaypoint *waypoint = WorldEdit.waypoints.get(waypoint_id)) {
                    ...
                    if (changed) {
                        WorldEdit.setChangedWaypoint(waypoint);
                        WorldEdit.setVisibleWaypoints();
                        WaypointList.setChanged();
                    }
                    if (waypoint->equal(src)) {
                        *waypoint_ver = src_ver;
                        WorldEdit.ver->changed = true;
                    } else if (changed) {
                        waypoint_ver->randomize();
                        WorldEdit.ver->changed = true;
                    }
                    return changed;
                }
    ...
    if (src.newer(server))
        Server.setWaypoint(world_id, waypoint_id, src_ver, src); // if after syncing local waypoint is newer than server, then send its data to the server
    return changed;
}
bool ProjectEx::syncLake(C UID &world_id, C UID &lake_id, Version &src_ver, Lake &src) {
        ...
        if (WaterVer *lake_ver = WorldEdit.ver->lakes.get(lake_id))
            if (src_ver != lake_ver->ver)
                if (Lake *lake = WorldEdit.lakes.get(lake_id)) {
                    ...
                    if (changed) {
                        WorldEdit.setChangedLake(lake);
                        WorldEdit.setVisibleWaters();
                    }
                    if (lake->equal(src)) {
                        lake_ver->ver = src_ver;
                        WorldEdit.ver->changed = true;
                    } else if (changed) {
                        lake_ver->ver.randomize();
                        WorldEdit.ver->changed = true;
                    }
                    return changed;
                }
    ...
    if (src.newer(server))
        Server.setLake(world_id, lake_id, src_ver, src); // if after syncing local lake is newer than server, then send its data to the server
    return changed;
}
bool ProjectEx::syncRiver(C UID &world_id, C UID &river_id, Version &src_ver, River &src) {
        ...
        if (WaterVer *river_ver = WorldEdit.ver->rivers.get(river_id))
            if (src_ver != river_ver->ver)
                if (River *river = WorldEdit.rivers.get(river_id)) {
                    ...
                    if (changed) {
                        WorldEdit.setChangedRiver(river);
                        WorldEdit.setVisibleWaters();
                    }
                    if (river->equal(src)) {
                        river_ver->ver = src_ver;
                        WorldEdit.ver->changed = true;
                    } else if (changed) {
                        river_ver->ver.randomize();
                        WorldEdit.ver->changed = true;
                    }
                    return changed;
                }
    ...
    if (src.newer(server))
        Server.setRiver(world_id, river_id, src_ver, src); // if after syncing local lake is newer than server, then send its data to the server
    return changed;
}
void ProjectEx::hmDel(C UID &world_id, C VecI2 &area_xy, C TimeStamp *time) {
    if (WorldEdit.elm_id == world_id)
        WorldEdit.hmDel(area_xy, time);
    else
        super ::hmDel(world_id, area_xy, time);
}
...
void ProjectEx::objGet(C UID &world_id, C VecI2 &area_xy, C Memc<UID> &obj_ids, Memc<ObjData> &objs) {
    if (world_id.valid() && WorldEdit.elm_id == world_id)
        if (Area *area = WorldEdit.findAreaLoaded(area_xy)) {
            REPA(area->objs)
            if (Obj *obj = area->objs[i])       // iterate all objects in the area
...
bool ProjectEx::syncObj(C UID &world_id, C VecI2 &area_xy, Memc<ObjData> &objs, Map<VecI2, Memc<ObjData>> *obj_modified, Memc<UID> *local_newer) {
    if (world_id.valid() && WorldEdit.elm_id == world_id && WorldEdit.ver) {
        ...
        Area &target_area = *WorldEdit.getArea(area_xy);
        if (!target_area.loaded)
            unload_areas.binaryInclude(target_area.xy);
        REPA(objs) {
            ObjData &obj = objs[i];
            ...
            if (!obj_ver)                                      // not present in this world, then add to target area
            {
                ...
                Obj &cur_obj = WorldEdit.objs.New();
                cur_obj.id = obj.id;
                ...
                if (cur_obj.physPath())
                    cur_obj.setChangedPaths(); // call after 'setChangedEmbed'
                target_area.setChanged();
                if (AreaVer *area_ver = target_area.getVer())
                    area_ver->obj_ver.randomize();

                if (area_loaded) {
                    loaded_areas_changed = true;
                    target_area.delayedValidateRefs();
                }
            } else {
                Area &cur_area = *WorldEdit.getArea(obj_ver->area_xy);
                if (!cur_area.loaded)
                    unload_areas.binaryInclude(cur_area.xy);
                ...
                if (cur_area.objs[i]->id == obj.id) // found that object
                {
                    ...
                    if (cur_obj.sync(obj, edit_path)) {
                        ...
                        if (changed_embed)
                            cur_obj.setChangedEmbed(&new_area.xy); // call after 'changedObj' and before 'setChangedPaths' (use new_area coordinates)
                        cur_area.setChanged();
                        if (changed_phys_path)
                            cur_obj.setChangedPaths();
                        if (AreaVer *area_ver = cur_area.getVer())
                            area_ver->obj_ver.randomize();    // call 'setChangedPaths' after 'setChangedEmbed' for old area
                        cur_obj.attach(WorldEdit, &new_area); // attach to new area
                        new_area.setChanged();
                        if (changed_phys_path)
                            cur_obj.setChangedPaths();
                        if (AreaVer *area_ver = new_area.getVer())
                            area_ver->obj_ver.randomize(); // call 'setChangedPaths' after 'setChangedEmbed' for new area

                        if (area_loaded) {
                            loaded_areas_changed = true;
                            cur_area.delayedValidateRefs();
                            new_area.delayedValidateRefs();
                        }
                    }
                    break;
                }
        ...
        Memc<ObjData> target_objs;
        REPA(target_area.objs)
        target_objs.New() = *target_area.objs[i]; // copy before unloading
        if (local_newer)
            GetNewer(target_objs, objs, *local_newer);
        REPA(unload_areas)
        if (Area *area = WorldEdit.findArea(unload_areas[i]))
            if (!area->hasSelectedObj())
                area->unload(); // unload areas
        if (loaded_areas_changed)
            ObjList.setChanged();
...
void ProjectEx::syncCodes() {
    ...
    Memc<ElmTypeVer> elm_type_vers;
    FREPA(elms) // get list of project apps and codes
    {
        Elm &elm = elms[i];
        if (elm.type == ELM_APP || elm.type == ELM_CODE)
            elm_type_vers.New().set(elm);
    }
...
void ProjectEx::syncCodes(C Memc<ElmCodeData> &elm_code_datas) {
    ...
    FREPA(elm_code_datas) {
        C ElmCodeData &ecd = elm_code_datas[i];
        if (Elm *elm = findElm(ecd.id, ecd.type))
            switch (ecd.type) {
            case ELM_APP: {
                if (ElmApp *app_data = elm->appData()) {
                    if (app_data->sync(ecd.app, true))
                        AppPropsEdit.elmChanged(elm->id);
                    if (app_data->newer(ecd.app))
                        send.New().set(*elm); // if after syncing, local copy has newer data than server, then include it in sending
                }
            } break;
            ...
            case ELM_CODE: {
                if (ElmCode *code_data = elm->codeData()) {
                    Code code;
                    LoadCode(code.current, codePath(*elm));
                    LoadCode(code.base, codeBasePath(*elm)); // load code from this computer
                    Str latest;
                    if (CodeEdit.sourceDataGet(elm->id, latest))
                        Swap(code.current, latest); // update to latest data taken from Code Editor, in case user made a change between network data exchange
                    if (ecd.ver)                    // if server data is valid then merge
                    {
                        if (!code_data->ver)
                            new_source = true;                                                                                      // if local version didn't exist then enable 'new_source'
                        ...
                        if (Equal(merged, ecd.code.current, true))
                            code_data->ver = ecd.ver;
                        else if (!Equal(merged, code.current, true))
                            code_data->newVer(); // if same as server then set ver from server, else if changed then set new ver
                        code.current = merged;
                        code.base = ecd.code.current; // new base is latest server current
                        SaveCode(code.current, codePath(*elm));
                        SaveCode(code.base, codeBasePath(*elm));       // save code to this computer
                        CodeEdit.sourceDataSet(elm->id, code.current); // update Code Editor with latest source
                    }
                    if (Server.canWriteCode() && !Equal(code.current, ecd.code.current, true))
                        send.New().set(*elm, &code); // if after merging, local is different than server, then include it in sending
                }
            } break;
            }
    }
    if (send.elms())
        Server.syncCodes(send); // if we have different elements, then send them
    if (new_source)
        activateSources(); // if we've got a brand new source then reactivate sources in case it's used by the application
}
...
bool ProjectEx::codeGet(C UID &elm_id, Str &data) {
    if (CodeEdit.sourceDataGet(elm_id, data))
        return true;
    if (Elm *elm = findElm(elm_id, ELM_CODE))
        return LoadCode(data, codePath(elm_id)) == Edit::EE_ERR_NONE;
    data.clear();
    return false;
}
    ...
    if (Elm *elm = findElm(elm_id))
        ...
        case ELM_VIDEO: {
            ...
            if (SafeOverwrite(f, gamePath(elm->id))) {
                elm->Data()->newVer();
                if (ElmFile *data = elm->fileData())
                    data->file_time.getUTC();
                if (ElmSound *data = elm->soundData())
                    data->file_time.getUTC();
                if (ElmVideo *data = elm->videoData())
                    data->file_time.getUTC();
                elmChanged(*elm);
                savedGame(*elm);
                Server.setElmLong(elm->id);
...
bool ProjectEx::imageSet(C UID &elm_id, C Image &image) {
    if (Elm *elm = findElm(elm_id, ELM_IMAGE)) {
        File f;
        f.writeMem();
        if ((image.mode() == IMAGE_3D) ? image.save(f) : image.ExportWEBP(f, 1, 1)) {
            f.pos(0);
            return imageSet(elm_id, f, HasColor(image), HasAlpha(image));
        }
    }
    return false;
}
bool ProjectEx::meshGet(C UID &elm_id, Mesh &mesh) {
    ...
    if (elm)
        if (ElmObj *obj_data = elm->objData())
            elm = getObjMeshElm(elm->id); // if this is an object then get its mesh
    if (elm)
        if (ElmMesh *mesh_data = elm->meshData()) // mesh
        {
            if (elm == ObjEdit.mesh_elm) {
                mesh = ObjEdit.mesh;
                return true;
            } else {
                if (Load(mesh, editPath(elm->id), game_path))
                    return true;
            }
        }
    mesh.del();
            ...
            if (mesh.load(data, game_path)) {
                ...
                REP(mesh.lods()) {
                    ...
                    REPA(lod) {
                        ...
                        REP(part.variations())
                        if (i)
                            part.variation(i, gamePath(findElmID(part.variation(i).id(), ELM_MTRL)));
                        Str mtrls[4];
                        REPAO(mtrls) = gamePath(findElmID(part.multiMaterial(i).id(), ELM_MTRL));
...
bool ProjectEx::physSet(C UID &elm_id, File &data) {
    ...
    if (elm)
        if (ElmObj *obj_data = elm->objData())
            elm = getObjPhysElm(elm->id); // if this is an object then get its phys
    if (elm)
        if (ElmPhys *phys_data = elm->physData()) // phys
        {
            PhysBody phys;
            if (phys.load(data, game_path)) {
                ...
                phys_data->density_time.getUTC();
                phys_data->density = phys.density;
                phys_data->from(phys);
                phys_data->newVer();
                if (ObjEdit.phys_elm == elm)
                    ObjEdit.reloadPhys();
                physChanged(*elm);
                Server.setElmLong(elm->id);
                return true;
...
bool ProjectEx::skelSetSlots(C UID &elm_id, C CMemPtr<Edit::SkeletonSlot> &skel_slots) {
    ...
    if (elm)
        if (ElmObj *obj_data = elm->objData())
            elm = getObjSkelElm(elm->id); // if this is an object then get its skeleton
    if (elm)
        if (ElmSkel *skel_data = elm->skelData()) // skel
        {
            if (ObjEdit.skel_elm == elm && ObjEdit.mesh_skel) {
                ObjEdit.mesh_undos.set();
                ObjEdit.mesh_skel->slots.setNum(skel_slots.elms());
                REPAO(skel_slots).setTo(ObjEdit.mesh_skel->slots[i], *ObjEdit.mesh_skel);
                ObjEdit.setChangedSkel(false);
                return true;
            } else {
                Skeleton skel;
                if (skelGet(elm->id, skel)) {
                    skel.slots.setNum(skel_slots.elms());
                    REPAO(skel_slots).setTo(skel.slots[i], skel);
        ...
        if (elm->type == ELM_OBJ || elm->type == ELM_OBJ_CLASS)
            if (EditObjectPtr params = editPath(obj_id)) {
                ...
                if (params->modify(changes, T)) {
                    if (ElmObj *obj_data = elm->objData()) {
                        obj_data->newVer();
                        obj_data->from(*params);
                    }
                    if (ElmObjClass *obj_class_data = elm->objClassData()) {
                        obj_class_data->newVer();
                        obj_class_data->from(*params);
                    }
...
bool ProjectEx::codeExport(bool gui) {
    if (codeCheck()) {
        ...
        REPA(elms) {
            Elm &elm = elms[i];
            if (elm.type == ELM_CODE && elm.finalExists()) {
                // access code data
                Str data;
                codeGet(elm.id, data);
                // save to file
                ...
                FileText f;
                if (f.write(name, HasUnicode(data) ? UTF_8 : ANSI)) {
                    f.putText(data);
                    ok &= f.flushOK();
                } else {
                    if (gui)
                        Gui.msgBox(S, S + "Can't write to \"" + name + "\"");
                    ok = false;
                    break;
                }
            }
            ...
            if (elm.type == ELM_CODE && elm.finalExists()) {
                ...
                if (f.read(name)) {
                    ...
                    if (!Equal(data, code, true)) {
                        if (gui)
                            Swap(ImportCode.updates.New().set(elm.id).data, code); // remember for updating with gui
                        else
                            ok &= codeSet(elm.id, code); // update instantly
                    }
                }
            }
        }
...
void ProjectEx::flushElm(C UID &elm_id) {
    if (ImageEdit.elm_id == elm_id)
        ImageEdit.flush();
    if (ImageAtlasEdit.elm_id == elm_id)
        ImageAtlasEdit.flush();
    if (IconSettsEdit.elm_id == elm_id)
        IconSettsEdit.flush();
    if (IconEdit.elm_id == elm_id)
        IconEdit.flush();
    if (EnumEdit.elm_id == elm_id)
        EnumEdit.flush();
    if (FontEdit.elm_id == elm_id)
        FontEdit.flush();
    if (TextStyleEdit.elm_id == elm_id)
        TextStyleEdit.flush();
    if (PanelImageEdit.elm_id == elm_id)
        PanelImageEdit.flush();
    if (PanelEdit.elm_id == elm_id)
        PanelEdit.flush();
    if (GuiSkinEdit.elm_id == elm_id)
        GuiSkinEdit.flush();
    if (GuiEdit.elm_id == elm_id)
        GuiEdit.flush();
    if (ObjClassEdit.elm_id == elm_id)
        ObjClassEdit.flush();
    if (SoundEdit.elm_id == elm_id)
        SoundEdit.flush();
    if (VideoEdit.elm_id == elm_id)
        VideoEdit.flush();
    if (AnimEdit.elm_id == elm_id)
        AnimEdit.flush();
    MtrlEdit.flush(elm_id);
    if (WaterMtrlEdit.elm_id == elm_id)
        WaterMtrlEdit.flush();
    if (PhysMtrlEdit.elm_id == elm_id)
        PhysMtrlEdit.flush();
    if (AppPropsEdit.elm_id == elm_id)
        AppPropsEdit.flush();
    if (EnvEdit.elm_id == elm_id)
        EnvEdit.flush();
    if (MiniMapEdit.elm_id == elm_id)
        MiniMapEdit.flush();
    if (ObjEdit.obj_elm && ObjEdit.obj_elm->id == elm_id)
        ObjEdit.flushObj();
    if (ObjEdit.mesh_elm && ObjEdit.mesh_elm->id == elm_id || ObjEdit.skel_elm && ObjEdit.skel_elm->id == elm_id)
        ObjEdit.flushMeshSkel();
    if (ObjEdit.phys_elm && ObjEdit.phys_elm->id == elm_id)
        ObjEdit.flushPhys();
    CodeEdit.sourceOverwrite(elm_id);
}
void ProjectEx::getFileSizes() {
    if (list.file_size && !file_size_getter.created())
        file_size_getter.get(game_path);
}
...
bool ProjectEx::saveTex(C Image &img, C UID &tex_id) {
    ...
    img.save(f.writeMem());
    f.pos(0);
    Str name = texPath(tex_id);
    if (SafeOverwrite(f, name)) {
        savedTex(tex_id, f.size());
        Saved(img, name);
        return true;
    }
    return false;
...
LOAD_RESULT ProjectEx::open(C UID &id, C Str &name, C Str &path, Str &error, bool ignore_lock) {
    ...
    if (LoadOK(result)) {
        ...
        FREPA(mtrl_brush_id)
        MtrlBrush.setMaterial(i, mtrl_brush_id[i]);
