...
{
    if (!pak_name.is())
        return DATA_READY;                                                                                        // if we don't want to create project data pak (no file)
    if (CompareFile(FileInfoSystem(pak_name).modify_time_utc, CodeEdit.appEmbedSettingsTime(PublishExeType)) > 0) // if existing Pak time is newer than settings (compression/encryption)
    {
        ...
        Pak pak;
        if (pak.loadEx(pak_name, Publish.cipher(), 0, null, null, need_optimized ? &used_file_ranges : null) == PAK_LOAD_OK) {
            if (need_optimized && used_file_ranges.elms() != 1)
                return DATA_CREATE; // needs to be optimized, so fully recreate
...
FILE_LIST_MODE CleanPublish(C FileFind &ff, bool &OK) {
    ...
    if (PublishKeep.binaryHas(SkipStartPath(full, PublishPath), ComparePathCI))
...
bool CleanPublish(Memt<SrcDest> &files) {
    ...
    FREPA(files)
    KeepPublish(files[i].dest);
    PublishKeep.sort(ComparePathCI);
    bool ok = true;
    FList(PublishPath, CleanPublish, ok);
    if (!ok) {
        Gui.msgBox(S, S + "Can't delete \"" + PublishPath + '"');
        return false;
    }
    return true;
}
...
bool StartPublish(C Str &exe_name, Edit::EXE_TYPE exe_type, Edit::BUILD_MODE build_mode, bool no_compile, C Str &custom_project_data_path, bool open_ide, bool project_package) {
    ...
    if (exe_type == Edit::EXE_AAB && (build_mode == Edit::BUILD_PLAY || build_mode == Edit::BUILD_DEBUG))
        exe_type = Edit::EXE_APK; // cannot play AAB, only APK
    ...
    if (PublishProjectPackage)
        Publish.cipher.clear(); // ProjectPackage is never encrypted
    else
        Publish.cipher.set(Proj);

    if (PublishProjectPackage)
        CodeEdit.saveChanges();

    if (PublishDataOnly || PublishProjectPackage) // data only
    {
        PublishPath = custom_project_data_path;
        FCreateDirs(GetPath(PublishPath));
    ...
    } else // executable
    {
        ...
        if (exe_type == Edit::EXE_EXE || exe_type == Edit::EXE_DLL) {
            ...
            if (!FCreateDirs(PublishPath)) {
                Gui.msgBox(S, S + "Can't create \"" + PublishPath + '"');
                return false;
            }
            if (!CodeEdit.appEmbedEngineData() || CodeEdit.appPublishProjData() || physx_dll) // if want to store something in "Bin" folder
            {
                PublishBinPath = PublishPath + "Bin\\";
                if (!FCreateDirs(PublishBinPath)) {
                    Gui.msgBox(S, S + "Can't create \"" + PublishBinPath + '"');
                    return false;
                }
                KeepPublish(PublishBinPath);
                if (CodeEdit.appPublishProjData() && PublishDataAsPak)
                    PublishProjectDataPath = PublishBinPath + "Project.pak";

                if (!CodeEdit.appEmbedEngineData() && PublishDataAsPak)
                    files.New().set(bin_path, PublishBinPath, "Engine.pak");
                if (physx_dll) {
                    ...
                    if (CodeEdit.config32Bit())
                        FREPA(physx_files32)
                    files.New().set(bin_path, PublishBinPath, physx_files32[i]);
                    else FREPA(physx_files64)
                        files.New()
                            .set(bin_path, PublishBinPath, physx_files64[i]);
                }
            }
            if (CodeEdit.appPublishSteamDll())
                files.New().set("Code/Windows/", PublishPath, CodeEdit.config32Bit() ? "steam_api.dll" : "steam_api64.dll");
            if (CodeEdit.appPublishOpenVRDll())
                files.New().set(S + "Code/Windows/" + (CodeEdit.config32Bit() ? "openvr_api.32.dll" : "openvr_api.64.dll"), PublishPath + "openvr_api.dll");
            if (PublishExePath.is())
                files.New().set(PublishExePath, PublishPath + GetBase(PublishExePath));
            if (!CleanPublish(files))
                return false;
        } else if (exe_type == Edit::EXE_WEB) {
            ...
            if (1 /*!CodeEdit.appEmbedEngineData()*/ || CodeEdit.appPublishProjData()) // if want to store something in "Bin" folder, embed engine data should be ignored (we always need "Engine.pak" for Web)
            {
                ...
                if (1 /*!CodeEdit.appEmbedEngineData()*/ && PublishDataAsPak)
                    files.New().set(bin_path + "Web/Engine.pak", PublishBinPath + "Engine.pak"); // embed engine data should be ignored (we always need "Engine.pak" for Web)
            }
            if (PublishExePath.is()) {
            ...
            if (PublishExePath.is())
                switch (FileInfoSystem(PublishExePath).type) {
                case FSTD_FILE:
                    files.New().set(PublishExePath, PublishPath + GetBase(PublishExePath));
                    break;
                case FSTD_DIR:
                    dirs.New().set(PublishExePath, PublishPath + GetBase(PublishExePath));
                    break;
                default:
                    Gui.msgBox(S, S + "Unknown file type \"" + PublishExePath + '"');
                    return false;
                }
            if (!CleanPublish(files))
                return false;
        } else if (exe_type == Edit::EXE_LINUX) {
            PublishPath = ProjectsPath + ProjectsPublishPath + GetBase(CodeEdit.appPath(CodeEdit.appName())).tailSlash(true);
            PublishDataAsPak = CodeEdit.appPublishDataAsPak();
            if (!FCreateDirs(PublishPath)) {
                Gui.msgBox(S, S + "Can't create \"" + PublishPath + '"');
            ...
            if (!CodeEdit.appEmbedEngineData() || CodeEdit.appPublishProjData() || physx_dll || CodeEdit.appPublishSteamDll() || CodeEdit.appPublishOpenVRDll()) // if want to store something in "Bin" folder
            {
                ...
                if (physx_dll) {
                    ...
                    FREPA(physx_files)
                    files.New().set(bin_path, PublishBinPath, physx_files[i]);
                }
                if (CodeEdit.appPublishSteamDll())
                    files.New().set(bin_path, PublishBinPath, "libsteam_api.so");
                if (CodeEdit.appPublishOpenVRDll())
                    files.New().set(bin_path, PublishBinPath, "libopenvr_api.so");
            }
            if (PublishExePath.is())
                files.New().set(PublishExePath, PublishPath + GetBase(PublishExePath));
            if (!CleanPublish(files))
            ...
            // if(CodeEdit.appPublishProjData()) always setup 'PublishProjectDataPath' because even if we don't include Project data, we still include App data
            {
                ...
                if (!PublishProjectDataPath.is()) {
                    Gui.msgBox(S, "Invalid path for project data file");
                    return false;
                }
        ...
        if (!FCopy(files[i].src, files[i].dest, overwrite)) {
            Gui.msgBox(S, S + "Can't copy \"" + GetBase(files[i].src) + '"');
            return false;
        }
        FREPA(dirs)
        if (!FReplaceDir(dirs[i].src, dirs[i].dest, overwrite)) {
            Gui.msgBox(S, S + "Can't copy \"" + GetBase(dirs[i].src) + '"');
            return false;
        }
    }
...
void ImageGenerateProcess(ImageGenerate &generate, ptr user, int thread_index) {
    if (!Publish.progress.stop) {
        ...
        generate.process();
        {
            SyncLocker locker(PublishLock);
            Publish.progress.progress += 1.0f / PublishGenerate.elms();
        }
    }
}
bool SetPak(MemPtr<PakFileData> files, C Str &pak_name) {
    DATA_STATE state = PublishDataState(files, pak_name);
    if (state == DATA_READY)
        return true;
    ...
    if (state == DATA_UPDATE && PakReplaceInPlace(files, pak_name, PAK_SET_HASH, Publish.cipher(), compress, compress_level, &PublishErrorMessage, &Publish.progress))
        return true;
...
bool PublishFunc(Thread &thread) {
    // generate
    PublishStage = PUBLISH_MTRL_SIMPLIFY;
    Publish.progress.progress = 0;
    WorkerThreads.process1(PublishGenerate, ImageGenerateProcess);
    ...
    {
        PublishStage = PUBLISH_TEX_OPTIMIZE;
        Publish.progress.progress = 0;
        ThreadMayUseGPUData();
    ...
    if (PublishDataAsPak) {
        if (PublishExeType == Edit::EXE_AAB && CodeEdit.appGooglePlayAssetDelivery()) // asset packs are only for AAB (not APK)
        {
            ...
            Str android_path = CodeEdit.androidPath();
            if (!android_path.is()) {
                PublishErrorMessage = "Invalid 'androidPath'";
                return false;
            }
            android_path.tailSlash(true);
            ...
            for (int i = 0;; i++) {
                ...
                if (!pfd || split_size + pfd_size > max_size) // if reached the end, or adding this file would break the limit
                {
                    if (split.elms()) // have any data
                    {
                        ...
                        if (!SetPak(split, pack_path + pack_name + ".pak"))
                            return false; // !! FILE NAMES MUST BE UNIQUE BECAUSE ANDROID GRADLE WILL COMPLAIN IF THERE ARE MULTIPLE FILES WITH SAME NAMES ACROSS DIFFERENT ASSET PACKS !!
                        split.clear();
                        split_size = 0;
                        CodeEdit.android_asset_packs++;
                    }
                    if (!pfd) {
                        PublishOk = true;
                        break;
            ...
            PublishOk = SetPak(PublishFiles, PublishProjectDataPath);
    } else {
        Memc<Str> dest_paths;
        FREPA(PublishFiles) {
            if (Publish.progress.stop)
            ...
            if (pfd.type != FSTD_DIR) {
                ...
                if (dest_paths.binaryInclude(dest_path, ComparePathCI))
                    FCreateDirs(dest_path); // create path only once
                if (!FCopy(pfd.data.name, dest)) {
                    PublishErrorMessage = "Error copying file.";
                    if (!FExist(pfd.data.name))
                        PublishErrorMessage.line() += "File not found.";
                    PublishErrorMessage.line() += S + "File: \"" + pfd.name + '"';
                    if (Elm *elm = Proj.findElm(GetFileNameID(pfd.name)))
                        PublishErrorMessage.line() += S + "Element: \"" + Proj.elmFullName(elm) + '"';
                    return false;
                }
            }
...
Texture *GetTexture(MemPtr<Texture> textures, C UID &tex_id) {
    if (tex_id.valid()) {
        int index;
        if (textures.binarySearch(tex_id, index, Texture::CompareTex))
            return &textures[index];
        Texture &tex = textures.NewAt(index);
        tex.id = tex_id;
        return &tex;
    }
    return null;
...
void AddPublishFiles(Memt<Elm *> &elms, MemPtr<PakFileData> files, Memc<ImageGenerate> &generate, Memc<ImageConvert> &convert, Edit::EXE_TYPE exe_type) {
    ...
    if (Elm *elm = elms[i])
        if (ElmPublish(elm->type)) {
            if (elm->type == ELM_WORLD) // world
            {
                ...
                pfd.name = world_path;
                pfd.type = FSTD_DIR; // world folder
                ...
                if (Proj.getWorldPaths(elm->id, world_edit_path_src, world_game_path_src))
                    if (WorldVer *ver = Proj.worldVerGet(elm->id)) {
                        ...
                        {
                            Str src = src_area_path + ver->areas.lockedKey(i);
                            if (FExist(src)) // if an area was created, but later its data was removed, then the AreaVer will remain, however the area data file may be removed, because of that, we need to check if it exists
                            {
            ...
            } else if (elm->type == ELM_MINI_MAP) // mini map
            {
            ...
            } else // regular files
            {
                ...
                if (elm->type == ELM_MTRL)
                    if (ElmMaterial *data = elm->mtrlData()) // material
                    {
                        ...
                        if (mtrl_simplify && (base_1_tex.valid() || base_2_tex.valid())) {
                            ...
                            if (CompareFile(src_fi.modify_time_utc, FileInfoSystem(dest_name).modify_time_utc)) // if different (compare just modify time, because sizes will always be different due to different formats)
                            {
                                ...
                                {
                                    Vec4 avg;
                                    if (mtrl->base_2 && mtrl->base_2->stats(null, null, &avg)) {
                                        flt avg_rough = avg.y, avg_metal = avg.x; // #MaterialTextureLayout
                                        temp.rough_add = avg_rough * temp.rough_mul + temp.rough_add;
                                        temp.rough_mul = 0; // !! clear 'rough_mul' after calculating 'rough_add' !!
                                        temp.reflect(avg_metal * temp.reflect_mul + temp.reflect_add);
                                    }
                                }
                                if (data->usesTexGlow())
                                    temp.glow = 0; // disable glow if there was a glow map, because now it's removed
                                File f;
                                temp.save(f.writeMem(), Proj.game_path);
                                f.pos(0);
                                SafeOverwrite(f, dest_name, &src_fi.modify_time_utc);
                                flags |= Texture::REGENERATE; // material parameters could have changed, so regenerate texture too as it may depend on them
                            }

                            // merge textures
                            FileInfo src_base(Proj.texPath(data->base_0_tex.valid() ? data->base_0_tex : data->base_1_tex.valid() ? data->base_1_tex
                                                                                                                                  : data->base_2_tex)); // get modify time of first available original texture
                        ...
                        Texture *t0;
                        if (t0 = GetTexture(publish_texs, base_0_tex)) {
                            t0->sRGB(true);
                            t0->downSize(downsize);
                            MAX(t0->quality, MinMtrlTexQualityBase0);
                            MAX(t0->quality, data->tex_quality);
                            t0->flags |= flags;
                        }
                        Texture *t1;
                        if (t1 = GetTexture(publish_texs, base_1_tex)) {
                            t1->downSize(downsize);
                            MAX(t1->quality, MinMtrlTexQualityBase1);
                            t1->normal();
                        }
                        Texture *t2;
                        if (t2 = GetTexture(publish_texs, base_2_tex)) {
                            t2->downSize(downsize);
                            MAX(t2->quality, MinMtrlTexQualityBase2);
                        }
                        if (Texture *t = GetTexture(publish_texs, data->detail_tex)) {
                            t->downSize(downsize);
                            MAX(t->quality, MinMtrlTexQualityDetail);
                            t->usesAlpha();
                        } // Detail uses Alpha for Color #MaterialTextureLayoutDetail
                        if (Texture *t = GetTexture(publish_texs, data->macro_tex)) {
                            t->sRGB(true);
                            t->downSize(downsize);
                            MAX(t->quality, MinMtrlTexQualityMacro);
                            MAX(t->quality, data->tex_quality);
                        } // doesn't use Alpha, 'GetTexture' needs to be called
                        if (Texture *t = GetTexture(publish_texs, data->emissive_tex)) {
                            t->sRGB(true);
                            t->downSize(downsize);
                            MAX(t->quality, MinMtrlTexQualityLight);
                        } // doesn't use Alpha, 'GetTexture' needs to be called

                        // check which base textures use Alpha Channel, #MaterialTextureLayout
                        if (t0) {
                            if (data->usesTexAlpha())
                                t0->usesAlpha(); // t0 Alpha used for opacity
                        }
                        if (t2) {
                            if (uses_tex_glow)
                                t2->usesAlpha(); // t2 Alpha used for glow
                        }
                    }

                if (elm->type == ELM_WATER_MTRL)
                    if (ElmWaterMtrl *data = elm->waterMtrlData()) // water material
                    {
                        // !! 'GetTexture' needs to be called always because it adds texture to publish list !!
                        // #MaterialTextureLayoutWater
                        Texture *t0;
                        if (t0 = GetTexture(publish_texs, data->base_0_tex)) {
                            t0->sRGB(true);
                            t0->downSize(0);
                            MAX(t0->quality, MinMtrlTexQualityBase0);
                            MAX(t0->quality, data->tex_quality);
                        ...
                        Texture *t1;
                        if (t1 = GetTexture(publish_texs, data->base_1_tex)) {
                            t1->downSize(0);
                            MAX(t1->quality, MinMtrlTexQualityBase1);
                            t1->normal();
                        }
                        Texture *t2;
                        if (t2 = GetTexture(publish_texs, data->base_2_tex)) {
                            t2->downSize(0);
                            MAX(t2->quality, MinMtrlTexQualityBase2);
                            t2->channels = 1;
                            t2->sign(true);
                ...
                if (elm->type == ELM_IMAGE)           // image
                    if (android || iOS || uwp || web) // desktop/switch platforms already have the best format chosen through 'EditToGameImage' and 'ImageProps'
                        if (ElmImage *data = elm->imageData())
                            if (IMAGE_TYPE dest_type = (android ? data->androidType() : iOS ? data->iOSType()
                                                                                    : uwp   ? data->uwpType()
                                                                                            : data->webType())) // want to use custom type
                            {
                                Str src_name = pfd.data.name,
                ...
                if (elm->type == ELM_ICON)            // icon
                    if (android || iOS || uwp || web) // desktop/switch platforms already have the best format chosen through 'EditToGameImage' and 'ImageProps'
                        if (ElmIcon *data = elm->iconData())
                            if (IMAGE_TYPE dest_type = (android ? data->androidType(&Proj) : iOS ? data->iOSType(&Proj)
                                                                                         : uwp   ? data->uwpType(&Proj)
                                                                                                 : data->webType(&Proj))) // want to use custom type
                            {
                                Str src_name = pfd.data.name,
                                    dest_name = Proj.formatPath(elm->id, FormatSuffix(dest_type));
                                pfd.data.set(dest_name); // adjust pak file path
                ...
                if (elm->type == ELM_IMAGE_ATLAS)                               // image atlas
                    if (android || iOS || (uwp && !UWPBC7) || (web && !WebBC7)) // desktop/switch platforms already have the best format chosen during image atlas creation
                        if (ElmImageAtlas *data = elm->imageAtlasData())
                            if (data->compress())
                                if (IMAGE_TYPE dest_type = ((android || iOS) ? IMAGE_ETC2_RGBA_SRGB : iOS ? IMAGE_PVRTC1_4_SRGB
                                                                                                  : uwp   ? IMAGE_BC3_SRGB
                                                                                                          : IMAGE_BC3_SRGB)) // we assume that atlas images contain transparency
                                {
                                    Str src_name = pfd.data.name,
                                        dest_name = Proj.formatPath(elm->id, FormatSuffix(dest_type));
                                    pfd.data.set(dest_name); // adjust pak file path
                ...
                if (elm->type == ELM_PANEL_IMAGE)                               // panel image
                    if (android || iOS || (uwp && !UWPBC7) || (web && !WebBC7)) // desktop/switch platforms already have the best format chosen during image atlas creation
                        if (ElmPanelImage *data = elm->panelImageData())
                            if (IMAGE_TYPE dest_type = ((android || iOS) ? IMAGE_ETC2_RGBA_SRGB : iOS ? IMAGE_PVRTC1_4_SRGB
                                                                                              : uwp   ? IMAGE_BC3_SRGB
                                                                                                      : IMAGE_BC3_SRGB)) // we assume that panel images contain transparency
                            {
                                Str src_name = pfd.data.name,
                                    dest_name = Proj.formatPath(elm->id, FormatSuffix(dest_type));
                                pfd.data.set(dest_name); // adjust pak file path
    ...
    FREPA(publish_texs) {
        ...
        if (Proj.texs.binaryHas(tex.id) || tex.dynamic()) {
            ...
            {
                if (android || iOS)
                    change_type = ((tex.channels == 1) ? (tex.sign() ? IMAGE_ETC2_R_SIGN : IMAGE_ETC2_R) : (tex.channels == 2) ? (tex.sign() ? IMAGE_ETC2_RG_SIGN : IMAGE_ETC2_RG)
                                                                                                       : (tex.channels == 3)   ? IMAGE_ETC2_RGB
                                                                                                                               : IMAGE_ETC2_RGBA);
                else if (iOS)
                    change_type = ((tex.channels == 1) ? (tex.sign() ? IMAGE_ETC2_R_SIGN : IMAGE_ETC2_R) : (tex.channels == 2)                   ? (tex.sign() ? IMAGE_ETC2_RG_SIGN : IMAGE_ETC2_RG)
                                                                                                       : (tex.quality >= Edit::Material::MEDIUM) ? IMAGE_PVRTC1_4
                                                                                                                                                 : IMAGE_PVRTC1_2);
                else if (uwp)
            ...
            if (change_type >= 0 || max_size != INT_MAX || tex.downsize) // if any change is desired
            {
                ...
                {
                    ImageHeader header;
                    if (ImageLoadHeader(pfd.data.name, header)) // this may fail if this is a dynamically generated texture which doesn't exist yet, in that case just proceed with conversion
                    {
                        if (header.type == change_type)
                            change_type = -1; // if image already is of that type , then disable retyping
                        if (header.size.max() <= max_size)
                            max_size = INT_MAX; // if image size fits into the limit, then disable resizing
                    }
                }
    ...
    if (tex_not_found)
        Gui.msgBox(S, S + tex_not_found + " Texture" + CountS(tex_not_found) + " were not found");
}
/******************************************************************************/
void SetPublishFiles(Memb<PakFileData> &files, Memc<ImageGenerate> &generate, Memc<ImageConvert> &convert, Memc<Mems<byte>> &file_data) {
    ...
    if (PublishProjectPackage) // publish as *.ProjectPackage
    {
        ...
        temp = Proj;
        Memc<UID> remove;
        Proj.floodRemoved(remove, Proj.root);
        remove.sort();
        REPA(temp.elms)
        if (remove.binaryHas(temp.elms[i].id))
            temp.elms.removeValid(i, true);
        temp.getTextures(temp.texs); // keep only used textures

        // project "Data" file
        {
            File file;
            temp.save(file.writeMem());
            file.pos(0);
            Mems<byte> &data = file_data.New();
            data.setNum(file.size()).loadRawData(file);
            PakFileData &pfd = files.New();
            pfd.name = "Data";
        ...
        FREPA(temp.elms) {
            ...
            if (elm.type == ELM_WORLD) {
                ...
                {
                    PakFileData &pfd = files.New();
                    pfd.name = world_edit_path;
                    pfd.type = FSTD_DIR;
                } // world edit folder
                {
                    PakFileData &pfd = files.New();
                    pfd.name = world_game_path;
                    pfd.type = FSTD_DIR;
                } // world game folder
                ...
                if (Proj.getWorldPaths(elm.id, world_edit_path_src, world_game_path_src))
                    if (WorldVer *ver = Proj.worldVerGet(elm.id)) {
                        ...
                        FREPA(ver->areas) // areas, process in order to avoid re-sorting
                        {
                            ...
                            // edit
                            src = area_edit_path_src + ver->areas.lockedKey(i);
                            if (FExist(src)) // if an area was created, but later its data was removed, then the AreaVer will remain, however the area data file may be removed, because of that, we need to check if it exists
                            {
                                PakFileData &pfd = files.New();
                            ...
                            // game
                            src = area_game_path_src + ver->areas.lockedKey(i);
                            if (FExist(src)) // if an area was created, but later its data was removed, then the AreaVer will remain, however the area data file may be removed, because of that, we need to check if it exists
                            {
                                PakFileData &pfd = files.New();
        ...
        if (!CodeEdit.appEmbedEngineData() && !PublishDataAsPak) {
            Pak engine;
            if (engine.load(BinPath() + "Engine.pak"))
                FREPA(engine) {
        ...
        } else if (PublishExeType == Edit::EXE_UWP || PublishExeType == Edit::EXE_APK || PublishExeType == Edit::EXE_AAB || PublishExeType == Edit::EXE_IOS || PublishExeType == Edit::EXE_NS) // for Windows New, Android, iOS and Switch, if Project data is not included, then include only App data
        {
...
bool InitPublish() {
    ...
    Gui += PublishSkipOptimize.create(Rect_C(0, -0.20f, 0.45f, 0.08f), "Skip for now").focusable(false);
    PublishSkipOptimize.mode = BUTTON_TOGGLE;
    return true;
}
void ShutPublish() {
    PublishCancel();
    ...
    if (!PublishOk)
        Gui.msgBox("Publishing Failed", PublishErrorMessage);
...
void PublishSuccess() {
    ...
    } else if (PublishDataNeeded(PublishExeType) && !PublishNoCompile) // we've published data, now we need to compile the code
    {
        CodeEdit.codeDo(PublishBuildMode);
    } else if (PublishExeType == Edit::EXE_WEB && PublishBuildMode == Edit::BUILD_PLAY) {
        Run("emrun", S + '"' + PublishPath + GetBase(PublishExePath) + '"');
    } else {
        Str text;
        if (PublishProjectDataPath.is())
            text = S + (PublishProjectPackage ? "Project size: " : "Project data size: ") + SizeBytes(FSize(PublishProjectDataPath));
        else
            text = "Publishing succeeded";
        PublishSuccess(PublishPath, text);
    }
}
void PublishCancel() {
    UpdateThread.stop();
    Publish.progress.stop = true;
    App.stateWorking();
}
/******************************************************************************/
bool UpdatePublish() {
    ...
    if (Publish.progress.stop) // when wanting to stop then wait until thread finishes
    {
        if (!UpdateThread.active()) {
            SetProjectState();
            PublishOk = false;
            if (!PublishErrorMessage.is())
                PublishErrorMessage = "Publishing breaked on user request";
        }
    } else {
        ...
        } else if (!UpdateThread.active()) // finished
        {
            SetProjectState();
            if (PublishOk)
                PublishSuccess();
    ...
    if (!App.maximized())
        REPA(MT)
    if (MT.b(i)) {
        GuiObj *go = MT.guiObj(i);
        if (!go || go == Gui.desktop() || go->type() == GO_WINDOW && !(go->asWindow().flag & WIN_MOVABLE)) {
            App.window().move(MT.pixelDelta(i).x, MT.pixelDelta(i).y);
            sleep = 1;
        }
    }
    Time.wait(sleep);
...
void DrawPublish() {
    ...
    if (Publish.progress.stop) {
        ...
        if (!UpdateThread.created()) {
            ...
            Str text;
            switch (PublishStage) {
            case PUBLISH_MTRL_SIMPLIFY:
                text = "Simplifying Materials";
                break;
            case PUBLISH_TEX_OPTIMIZE:
                text = (PublishSkipOptimize() ? "Copying Textures" : "Optimizing Textures");
                break;
            default:
                text = (PublishProjectPackage ? "Compressing Project" : "Publishing Project");
                break;
            }
            D.text(0, 0.05f, text);
        }
...
/******************************************************************************/
void ImageGenerate::set(C Str &src_mtrl, C Str &dest_base_0, C DateTime &time) {
    T.src_mtrl = src_mtrl;
    T.dest_base_0 = dest_base_0;
    T.time = time;
}
void ImageGenerate::process() {
    Image base_0;
    MaterialPtr mtrl = src_mtrl;
    if (mtrl && MergeBaseTextures(base_0, *mtrl)) {
        File f;
        if (base_0.save(f.writeMem())) {
            f.pos(0);
            SafeOverwrite(f, dest_base_0, &time);
        }
    }
}
...
void ImageConvert::set(C Str &src, C Str &dest, C DateTime &time, C ElmImage &data, IMAGE_TYPE type) {
    ...
    T.time = time;
    T.pow2 = data.pow2();
    T.mip_maps = (data.mipMapsActual() ? 0 : 1);
    T.alpha_lum = data.alphaLum();
    T.has_color = data.hasColor();
    T.has_alpha = data.hasAlpha3();
    T.ignore_alpha = data.ignoreAlpha();
    T.env = data.envActual();
    T.type = type;
    T.mode = data.mode;
    T.size = data.size;
}
    ...
    if (PublishSkipOptimize())
        if (InRange(type, IMAGE_TYPES) && ImageTI[type].compressed) // if compressed
            if (type < IMAGE_BC1 || type > IMAGE_BC5_SIGN)          // BC1..BC5 compression is fast, so no need to skip
            {
                type = -1;
                time.decDay();
                return true; // use original type and set previous date, so the file will be regenerated next time
            }
    return false;
}
void ImageConvert::process() C {
    ...
    if (!time.valid())
        time = FileInfo(src).modify_time_utc; // 'time' could've been empty if the file didn't exist yet at the moment of setting up this object (this can happen for dynamically generated textures)
    int type = T.type;
    bool skip = SkipOptimize(type, time), pvrtc = (type == IMAGE_PVRTC1_2 || type == IMAGE_PVRTC1_4 || type == IMAGE_PVRTC1_2_SRGB || type == IMAGE_PVRTC1_4_SRGB);
    switch (family) {
    case ELM_IMAGE: {
        Image image;
        if (image.Import(src))
            if (EditToGameImage(image, image, pow2, true, alpha_lum, ElmImage::COMPRESSED, mode, mip_maps, has_color, has_alpha, ignore_alpha, env, size, &type)) // sRGB is ignored here because we force custom type
            {
                File f;
                if (image.save(f.writeMem())) {
                    f.pos(0);
                    SafeOverwrite(f, dest, &time);
                } // save using specified time
            }
    } break;
    ...
    case ELM_IMAGE_ATLAS: {
        ...
        if (atlas.load(src)) {
            FREPA(atlas.images) {
                ...
                VecI2 size = image.size(), old = size;
                if (pvrtc)
                    size = CeilPow2(size.max()); // image must be square for PVRTC. Unlike for regular textures, for atlases it's better to use 'CeilPow2', to make sure we don't make textures much smaller than original.
    ...
    case ELM_PANEL_IMAGE: {
        ...
        if (panel_image.load(src)) {
            VecI2 size = panel_image.image.size();
            if (pvrtc)
                size = CeilPow2(size.max()); // image must be square for PVRTC. Unlike for regular textures, for panel images it's better to use 'CeilPow2', to make sure we don't make textures much smaller than original.
            panel_image.image.copy(panel_image.image, size.x, size.y, -1, type, -1, -1, FILTER_BEST, IC_CLAMP | IC_ALPHA_WEIGHT);
            File f;
            if (panel_image.save(f.writeMem())) {
                f.pos(0);
                SafeOverwrite(f, dest, &time);
            } // save using specified time
        }
    ...
    case IMAGE: {
        if (skip) // just copy
        {
            ...
            if (f.read(src))
                SafeOverwrite(f, dest, &time);
        } else if (C ImagePtr &image = src) {
            Image temp, *s = image();
            VecI size = s->size3();
            if (pvrtc)
                size.xy = NearestPow2(size.xy.avgI()); // image must be square for PVRTC
            ...
            {
                if (mip_maps < 0)
                    mip_maps = ((s->mipMaps() == 1) ? 1 : 0); // source will have now only one mip-map so we can't use "-1", auto-detect instead
                if (mode < 0)
                    mode = s->mode(); // source will now be as IMAGE_SOFT      so we can't use "-1", auto-detect instead
                if (s->copy(temp, -1, -1, -1, ImageTypeExcludeAlpha(ImageTypeUncompressed(s->type())), IMAGE_SOFT, 1))
                    s = &temp;
            }
            if (s->copy(temp, size.x, size.y, size.z, type, mode, mip_maps, FILTER_BEST, (clamp ? IC_CLAMP : IC_WRAP))) {
                File f;
                if (temp.save(f.writeMem())) {
                    f.pos(0);
                    SafeOverwrite(f, dest, &time);
                } // save using specified time
            }
...
void PublishResult::SizeStats(PublishResult &pr) {
    pr.del();
    ::SizeStats.display(pr.path, Publish.cipher());
}
void PublishResult::display(C Str &text) {
    path = (PublishProjectDataPath.is() ? PublishProjectDataPath : PublishBinPath);

    Gui += super::create(Rect_C(0, 0, 1, 0.34f), "Publishing succeeded");
    button[2].show();
...
void PublishClass::exportData(Edit::EXE_TYPE exe) {
    export_data_exe = exe;
    export_data.save();
}
Texture &Texture::downSize(int size) {
    MIN(downsize, size);
    return T;
}
Texture &Texture::usesAlpha() {
    channels = 4;
    return T;
}
...
Texture &Texture::sRGB(bool on) {
    FlagSet(flags, SRGB, on);
    return T;
}
bool Texture::sign() C { return FlagOn(flags, SIGN); }
Texture &Texture::sign(bool on) {
    FlagSet(flags, SIGN, on);
    return T;
}
bool Texture::dynamic() C { return FlagOn(flags, DYNAMIC); }
Texture &Texture::dynamic(bool on) {
    FlagSet(flags, DYNAMIC, on);
    return T;
}
bool Texture::regenerate() C { return FlagOn(flags, REGENERATE); }
Texture &Texture::regenerate(bool on) {
    FlagSet(flags, REGENERATE, on);
    return T;
}
