...
void EditorServer::ConvertHeight(C Heightmap &src, Image &dest, flt area_size) {
    ...
    REPD(y, dest.h())
    REPD(x, dest.w())
    dest.pixF(x, y) = src.height(x, y) * area_size;
}
void EditorServer::ConvertHeight(Image &src, Heightmap &dest, flt area_size) // 'src' will get modified !!
{
    ...
    dest.height(x, y, src.pixelF(x, y) / area_size);
}
...
void EditorServer::ConvertMaterial(C Heightmap &src, Edit::MaterialMap &dest) {
    Memt<UID> materials;
    materials.setNum(src.materials());
    REPAO(materials) = src.material(i).id(); // set material palette
...
check_again:
    if (EditServer.busy)
        return true; // don't do anything if the editor is busy, wait until it finishes processing to update clients
    ...
    if (queued >= 0) // if there was a queued command to be processed later, at this stage the 'EditServer' is no longer busy (if any publishing was requested, then it already finished)
    {
        ...
        case Edit::EI_EXPORT_APP:
            f.putBool(PublishOk);
            break; // save last publishing result
        }
        f.pos(0);
        connection.send(f);
        queued = -1;
    ...
    if (connection_version_ok)
        if (connection.receive(timeout)) {
            ...
            case Edit::EI_GET_PROJECTS_PATH: {
                File &f = connection.data.reset().putByte(Edit::EI_GET_PROJECTS_PATH).putStr(ProjectsPath);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_PROJECTS_PATH: {
                ...
                Projs.proj_path.set(f.getStr());
                f.reset().putByte(Edit::EI_SET_PROJECTS_PATH).putBool(true);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_PROJECTS: {
                ...
                Mems<Edit::Project> projects;
                projects.setNum(Projs.proj_data.elms());
            ...
            case Edit::EI_GET_PROJECT: {
                File &f = connection.data.reset().putByte(Edit::EI_GET_PROJECT).putUID(Proj.id);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_DATA_PATH: {
                File &f = connection.data.reset().putByte(Edit::EI_GET_DATA_PATH) << Proj.game_path;
                f.pos(0);
                connection.send(f);
            } break;

            case Edit::EI_GET_EDITOR_PATH: {
                File &f = connection.data.reset().putByte(Edit::EI_GET_EDITOR_PATH) << SDKPath();
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_ELMS: {
                ...
                if (Proj.valid()) {
                    ...
                    if (include_removed) {
                        ...
                        FREPA(mems) {
                            ...
                            elm.flags = 0;
                            if (src.removed())
                                elm.flags |= Edit::Elm::REMOVED;
                            if (src.noPublish())
                                elm.flags |= Edit::Elm::NO_PUBLISH;
                            if (src.noPublishMobile())
                                elm.flags |= Edit::Elm::NO_PUBLISH_MOBILE;
                            elm.id = src.id;
                ...
                File &f = connection.data.reset().putByte(Edit::EI_GET_ELMS).putBool(elms);
                if (elms)
                    elms.save(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_GET_ELM_LIT: {
                File &f = connection.data.reset().putByte(Edit::EI_GET_ELM_LIT).putUID(Proj.valid() ? Proj.list.lit_elm_id : UIDZero);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_ELMS_SELECTED: {
                ...
                File &f = connection.data.reset().putByte(Edit::EI_GET_ELMS_SELECTED).putBool(ok);
                if (ok)
                    elms.saveRaw(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_SET_ELMS_SELECTED: {
                ...
                if (Proj.valid()) {
                    Mems<UID> elms;
                    if (ok = elms.loadRaw(f))
                        Proj.elmSelect(elms);
                }
                f.reset().putByte(Edit::EI_SET_ELMS_SELECTED).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_RLD_ELMS: {
                File &f = connection.data;
                bool remember_result = f.getBool();
                bool ok = false;
                if (Proj.valid()) {
                    Mems<UID> elms;
                    if (elms.loadRaw(f))
                        ok = Proj.elmReload(elms, remember_result);
                }
                f.reset().putByte(Edit::EI_RLD_ELMS).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_RLD_ELMS_CANCEL: {
                ...
                if (Proj.valid()) {
                    Mems<UID> elms;
                    if (ok = elms.loadRaw(f))
                        Proj.cancelReload(elms);
                }
                f.reset().putByte(Edit::EI_RLD_ELMS_CANCEL).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_RLD_ELMS_GET_RESULT: {
                ...
                Mems<UID> elms;
                if (Proj.valid() && elms.loadRaw(f)) {
                    ok = true;
                    Importer.getResult(elms, results);
                }
                f.reset().putByte(Edit::EI_RLD_ELMS_GET_RESULT).putBool(ok);
                if (ok)
                    results.save(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_RLD_ELMS_FORGET_RESULT: {
                ...
                Mems<UID> elms;
                if (ok = elms.loadRaw(f))
                    Importer.forgetResult(elms);
                f.reset().putByte(Edit::EI_RLD_ELMS_FORGET_RESULT).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_NEW_ELM: {
                File &f = connection.data;
                ELM_TYPE type = ELM_TYPE(f.getByte());
                Str name = f.getStr();
                UID parent = f.getUID();
                Elm *elm = (Proj.valid() ? Proj.newElm(type, parent, &name) : null);
                f.reset().putByte(Edit::EI_NEW_ELM).putUID(AsID(elm)).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_NEW_WORLD: {
                File &f = connection.data;
                Str name = f.getStr();
                int area_size = f.getInt();
                int terrain_res = f.getInt();
                UID parent = f.getUID();
                Elm *elm = (Proj.valid() ? Proj.newWorld(name, area_size, terrain_res, parent, false) : null);
                f.reset().putByte(Edit::EI_NEW_WORLD).putUID(AsID(elm)).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_ELM_NAME: {
                bool ok = false;
                if (Proj.valid()) {
                    Memc<Edit::IDParam<Str>> elms;
                    if (elms.load(connection.data)) {
                        ok = true;
                        Proj.setElmNames(elms);
                    }
                }
                File &f = connection.data.reset().putByte(Edit::EI_SET_ELM_NAME).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_ELM_REMOVED: {
                bool ok = false;
                if (Proj.valid()) {
                    Memc<Edit::IDParam<bool>> elms;
                    if (elms.load(connection.data)) {
                        ok = true;
                        Memc<UID> remove, restore;
                        FREPA(elms)
                        if (elms[i].value)
                            remove.add(elms[i].id);
                        else
                            restore.add(elms[i].id);
                        Proj.remove(remove, false);
                        Proj.restore(restore);
                    }
                }
                File &f = connection.data.reset().putByte(Edit::EI_SET_ELM_REMOVED).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_ELM_PUBLISH_MOBILE: {
                ...
                if (Proj.valid()) {
                    ...
                    if (elms.load(connection.data)) {
                        ok = true;
                        Memc<UID> publish, no_publish;
                        FREPA(elms)
                        if (elms[i].value)
                            publish.add(elms[i].id);
                        else
                            no_publish.add(elms[i].id);
                ...
                File &f = connection.data.reset().putByte(cmd).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_ELM_PARENT: {
                bool ok = false;
                if (Proj.valid()) {
                    Memc<Edit::IDParam<UID>> elms;
                    if (elms.load(connection.data)) {
                        ok = true;
                        Proj.setElmParent(elms);
                    }
                }
                File &f = connection.data.reset().putByte(Edit::EI_SET_ELM_PARENT).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_ELM_SRC_FILE: {
                ...
                if (Proj.valid()) {
                    ...
                    if (elms.load(connection.data)) {
                        ...
                        time.getUTC();
                        FREPA(elms)
                        if (Elm *elm = Proj.findElm(elms[i].id)) {
                            elm->setSrcFile(elms[i].value, time);
                            Server.setElmShort(elm->id);
                        }
                    }
                }
                File &f = connection.data.reset().putByte(Edit::EI_SET_ELM_SRC_FILE).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_WORLD: {
                File &f = connection.data.reset().putByte(Edit::EI_GET_WORLD).putUID(WorldEdit.elm_id);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_WORLD: {
                File &f = connection.data;
                UID world_id = f.getUID();
                WorldEdit.activate(Proj.findElm(world_id, ELM_WORLD));
                f.reset().putByte(Edit::EI_SET_WORLD).putBool(WorldEdit.elm_id == world_id).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_WORLD_AREA_SIZE: {
                ...
                if (Elm *world = Proj.findElm(world_id, ELM_WORLD))
                    if (ElmWorld *data = world->worldData())
                        area_size = data->area_size;
                f.reset().putByte(Edit::EI_GET_WORLD_AREA_SIZE).putInt(area_size);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_WORLD_TERRAIN_RES: {
                ...
                if (Elm *world = Proj.findElm(world_id, ELM_WORLD))
                    if (ElmWorld *data = world->worldData())
                        terrain_res = data->hmRes();
                f.reset().putByte(Edit::EI_GET_WORLD_TERRAIN_RES).putInt(terrain_res);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_WORLD_TERRAIN_AREAS: {
                ...
                if (Elm *world = Proj.findElm(world_id, ELM_WORLD))
                    if (WorldVer *world_ver = Proj.worldVerGet(world_id)) {
                        ok = true;
                        areas = world_ver->getTerrainAreas();
                    }
                f.reset().putByte(Edit::EI_GET_WORLD_TERRAIN_AREAS).putBool(ok);
                if (ok)
                    f << areas;
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_DEL_WORLD_TERRAIN: {
                ...
                f.reset().putByte(Edit::EI_DEL_WORLD_TERRAIN).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_WORLD_TERRAIN_IS: {
                ...
                if (Elm *world = Proj.findElm(world_id, ELM_WORLD))
                    if (WorldVer *world_ver = Proj.worldVerGet(world_id))
                        is = world_ver->hasHm(area_xy);
                f.reset().putByte(Edit::EI_GET_WORLD_TERRAIN_IS).putBool(is).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_WORLD_TERRAIN_HEIGHT: {
                ...
                f.reset().putByte(Edit::EI_GET_WORLD_TERRAIN_HEIGHT).putBool(ok);
                if (ok)
                    height.save(f);
                f.pos(0);
                connection.send(f);
                    ...
                    if (Elm *world = Proj.findElm(world_id, ELM_WORLD))
                        if (world->worldData()->valid())
                            if (WorldVer *world_ver = Proj.worldVerGet(world_id)) {
                                ...
                                if (!Proj.findElm(material_id, ELM_MTRL))
                                    material_id = Proj.hm_mtrl_id;                                    // use default material if specified doesn't exist
                                if (material_id.valid() || (dest_area_ver && dest_area_ver->hasHm())) // we have a valid material or there already exists terrain (which has material set)
                                {
                                    ...
                                    if (Proj.hmUpdate(world_id, area_xy, AREA_SYNC_HM, area_ver, hm))
                                        Synchronizer.delayedSetArea(world_id, area_xy);
                                }
                            }
                f.reset().putByte(Edit::EI_SET_WORLD_TERRAIN_HEIGHT).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_WORLD_TERRAIN_COLOR: {
                ...
                if (Elm *world = Proj.findElm(world_id, ELM_WORLD))
                    if (WorldVer *world_ver = Proj.worldVerGet(world_id)) {
                        ...
                        Heightmap temp, *hm = null;
                        if (hm = Proj.hmGet(world_id, area_xy, temp))
                            ConvertColor(*hm, color);
                    }
                f.reset().putByte(Edit::EI_GET_WORLD_TERRAIN_COLOR).putBool(ok);
                if (ok)
                    color.save(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_SET_WORLD_TERRAIN_COLOR: {
                ...
                f.reset().putByte(Edit::EI_SET_WORLD_TERRAIN_COLOR).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_WORLD_TERRAIN_MATERIAL: {
                ...
                f.reset().putByte(Edit::EI_GET_WORLD_TERRAIN_MATERIAL).putBool(ok);
                if (ok)
                    material.save(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_SET_WORLD_TERRAIN_MATERIAL: {
                ...
                f.reset().putByte(Edit::EI_SET_WORLD_TERRAIN_MATERIAL).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_WORLD_TERRAIN: {
                ...
                f.reset().putByte(Edit::EI_GET_WORLD_TERRAIN).putBool(ok);
                if (ok) {
                    height.save(f);
                    material.save(f);
                    if (expects_color)
            ...
            case Edit::EI_SET_WORLD_TERRAIN: {
                ...
                f.reset().putByte(Edit::EI_SET_WORLD_TERRAIN).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_NEW_WORLD_OBJ: {
                                                    ...
                                                    if (hm) {
                                                        ...
                                                        if (src.align_to_terrain_normal > EPS) // align to terrain normal
                                                        {
                                                            ...
                                                            if (ElmObj *obj_data = obj_elm->objData())
                                                                mesh = Proj.gamePath(obj_data->mesh_id);
                ...
                f.reset().putByte(Edit::EI_NEW_WORLD_OBJ).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_WORLD_OBJ_BASIC: {
                File &f = connection.data;
                UID world_id = f.getUID();
                bool limit_areas = f.getBool();
                RectI areas;
                if (limit_areas)
                    f >> areas;
                bool only_selected = f.getBool();
                bool include_removed = f.getBool();
                Memc<UID> world_obj_instance_ids;
                world_obj_instance_ids.loadRaw(f);
                world_obj_instance_ids.sort(Compare);
                bool limit_obj_ids = (world_obj_instance_ids.elms() > 0);
                bool ok = false;
                ...
                f.reset().putByte(Edit::EI_GET_WORLD_OBJ_BASIC).putBool(ok);
                if (ok)
                    objs.save(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_GET_WORLD_OBJ_FULL: {
                ...
                bool only_selected = f.getBool();
                bool include_removed = f.getBool();
                bool include_removed_params = f.getBool();
                Memc<UID> world_obj_instance_ids;
                world_obj_instance_ids.loadRaw(f);
                bool limit_obj_ids = (world_obj_instance_ids.elms() > 0);
                bool ok = false;
                            ...
                            if (limit_obj_ids) // filter out selected objects
                            {
                                REPA(world_obj_instance_ids) {
                                    ...
                                    if (C ObjVer *src = world_ver->obj.find(src_id)) // if object actually exists
                                        if (include_removed ? true : !src->removed())
                                            if (limit_areas ? Cuts(src->area_xy, areas) : true)
                                                if (only_selected ? Selection.has(src_id) : true)
                                                    continue;
                                    ...
                                    if (world_id == WorldEdit.elm_id)
                                        REPA(Selection) {
                                            ...
                                            if (src.area)
                                                if (include_removed ? true : !src.removed)
                                                    if (limit_areas ? Cuts(src.area->xy, areas) : true)
                                                        world_obj_instance_ids.add(src.id);
                                        }
                                } else {
                                    MapLock ml(world_ver->obj);
                                    REPA(world_ver->obj) {
                                        ...
                                        if (include_removed ? true : !src.removed())
                                            if (limit_areas ? Cuts(src.area_xy, areas) : true)
                                                world_obj_instance_ids.add(src_id);
                                    }
                                }
                            }
                ...
                f.reset().putByte(Edit::EI_GET_WORLD_OBJ_FULL).putBool(ok);
                if (ok)
                    objs.save(f);
                f.pos(0);
                connection.send(f);
                ...
                if (C Elm *world = Proj.findElm(world_id, ELM_WORLD))
                    if (world->worldData()->valid())
                        if (C WorldVer *world_ver = Proj.worldVerGet(world_id)) {
                            ok = true;
                            FREPA(world_ver->waypoints)
                            if (Proj.waypointExists(world_id, world_ver->waypoints.lockedKey(i)))
                                waypoints.add(world_ver->waypoints.lockedKey(i));
                        }
                f.putBool(ok);
                if (ok)
                    waypoints.saveRaw(f);
            ...
            case Edit::EI_GET_WORLD_CAM: {
                File &f = connection.data.reset().putByte(Edit::EI_GET_WORLD_CAM);
                WorldEdit.v4.view[Edit::Viewport4::VIEW_FRONT].camera.save(f);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_WORLD_CAM: {
                ...
                bool ok = cam.load(f);
                if (ok)
                    WorldEdit.v4.view[Edit::Viewport4::VIEW_FRONT].camera = cam;
                f.reset().putByte(Edit::EI_SET_WORLD_CAM).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_DRAW_WORLD_LINES: {
                File &f = connection.data;
                bool ok = WorldEdit.lines.load(f);
                f.reset().putByte(Edit::EI_DRAW_WORLD_LINES).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_IMAGE: {
                ...
                UID elm_id = f.getUID();
                Image image;
                bool ok = Proj.imageGet(elm_id, image);
                f.reset().putByte(Edit::EI_GET_IMAGE).putBool(ok);
                if (ok)
                    image.save(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_SET_IMAGE: {
                ...
                bool ok = false;
                if (image.load(f))
                    if (Proj.imageSet(elm_id, image))
                        ok = true;
                f.reset().putByte(Edit::EI_SET_IMAGE).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_IMAGE_MIP_MAPS: {
                ...
                f.reset().putByte(Edit::EI_SET_IMAGE_MIP_MAPS).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_CODE: {
                ...
                UID elm_id = f.getUID();
                Str code;
                bool ok = Proj.codeGet(elm_id, code);
                f.reset().putByte(Edit::EI_GET_CODE).putBool(ok);
                if (ok)
                    code.save(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_SET_CODE: {
                ...
                f.reset().putByte(Edit::EI_SET_CODE).putBool(ok).pos(0);
                connection.send(f);
            } break;

            case Edit::EI_CODE_SYNC_IMPORT: {
                File &f = connection.data;
                f.reset().putByte(Edit::EI_CODE_SYNC_IMPORT).putBool(Proj.valid() && Proj.codeImport(false)).pos(0);
                connection.send(f);
            } break;

            case Edit::EI_CODE_SYNC_EXPORT: {
                File &f = connection.data;
                f.reset().putByte(Edit::EI_CODE_SYNC_EXPORT).putBool(Proj.valid() && Proj.codeExport(false)).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_FILE: {
                ...
                UID elm_id = f.getUID();
                File data;
                bool ok = Proj.fileRead(elm_id, data);
                f.reset().putByte(Edit::EI_GET_FILE).putBool(ok);
                if (ok)
                    data.copy(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_SET_FILE: {
                ...
                f.reset().putByte(Edit::EI_SET_FILE).putBool(ok).pos(0);
                connection.send(f);
            } break;

            // MTRL
            case Edit::EI_GET_MTRL_CUR: {
                File &f = connection.data.reset().putByte(Edit::EI_GET_MTRL_CUR).putUID(MtrlEdit.elm_id);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_MTRL_CUR: {
                ...
                f.reset().putByte(Edit::EI_SET_MTRL_CUR).putBool(MtrlEdit.elm_id == elm_id);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_MTRL: {
                ...
                UID elm_id = f.getUID();
                EditMaterial mtrl;
                bool ok = Proj.mtrlGet(elm_id, mtrl);
                f.reset().putByte(Edit::EI_GET_MTRL).putBool(ok);
                if (ok) {
                    Edit::Material edit_mtrl;
                    mtrl.copyTo(edit_mtrl);
                    edit_mtrl.save(f);
                }
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_SET_MTRL: {
                ...
                f.reset().putByte(Edit::EI_SET_MTRL).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_RLD_MTRL_TEX: {
                ...
                f.reset().putByte(Edit::EI_RLD_MTRL_TEX).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_MUL_MTRL_TEX_COL: {
                ...
                f.reset().putByte(Edit::EI_MUL_MTRL_TEX_COL).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_MESH: {
                ...
                if (Elm *elm = Proj.findElm(elm_id)) {
                    if (elm->type == ELM_OBJ) {
                        ok = true;
                        matrix.identity();
                        elm = Proj.objToMeshElm(elm);
                    } // if this is an object, then set ok to true, in case it has no mesh, we will just send empty data
                    if (elm && elm->type == ELM_MESH)
                        if (ok = data.read(Proj.editPath(*elm)))
                            matrix = elm->meshData()->transform();
                }
                f.reset().putByte(Edit::EI_GET_MESH).putBool(ok);
                if (ok) {
                    f << matrix;
                    data.copy(f);
                }
            ...
            case Edit::EI_SET_MESH: {
                ...
                f.reset().putByte(Edit::EI_SET_MESH).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_SKEL: {
                ...
                if (Elm *elm = Proj.findElm(elm_id)) {
                    if (elm && elm->type == ELM_OBJ) {
                        ok = true;
                        elm = Proj.objToSkelElm(elm);
                    } // if this is an object, then set ok to true, in case it has no skel, we will just send empty data
                    if (elm && elm->type == ELM_MESH) {
                        ok = true;
                        elm = Proj.meshToSkelElm(elm);
                    } // if this is a  mesh  , then set ok to true, in case it has no skel, we will just send empty data
                    if (elm && elm->type == ELM_SKEL) {
#if 0
                        ok=data.read(Proj.gamePath(*elm));
#else
                        Skeleton skel;
                        if (ok = Proj.skelGet(elm->id, skel)) {
                            skel.save(data.writeMem());
                            data.pos(0);
                        }
#endif
                    }
                }
                f.reset().putByte(Edit::EI_GET_SKEL).putBool(ok);
                if (ok)
                    data.copy(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_GET_SKEL_SLOTS: {
                ...
                UID elm_id = f.getUID();
                bool ok = false;
                Mems<Edit::SkeletonSlot> skel_slots;
                if (Elm *elm = Proj.findElm(elm_id)) {
                    if (elm && elm->type == ELM_OBJ) {
                        ok = true;
                        elm = Proj.objToSkelElm(elm);
                    ...
                    if (elm && elm->type == ELM_SKEL) {
                        Skeleton skel;
                        if (ok = Proj.skelGet(elm->id, skel)) {
                            skel_slots.setNum(skel.slots.elms());
                            REPAO(skel_slots).setFrom(skel.slots[i], skel);
                        }
                    }
                }
                f.reset().putByte(Edit::EI_GET_SKEL_SLOTS).putBool(ok);
                if (ok)
                    skel_slots.save(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_SET_SKEL_SLOTS: {
                ...
                f.reset().putByte(Edit::EI_SET_SKEL_SLOTS).putBool(ok).pos(0);
                connection.send(f);
            } break;

            // ANIM
            case Edit::EI_GET_ANIM_CUR: {
                File &f = connection.data.reset().putByte(Edit::EI_GET_ANIM_CUR).putUID(AnimEdit.elm_id);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_ANIM_CUR: {
                ...
                f.reset().putByte(Edit::EI_SET_ANIM_CUR).putBool(AnimEdit.elm_id == elm_id);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_ANIM: {
                ...
                UID elm_id = f.getUID();
                Animation anim;
                bool ok = Proj.animGet(elm_id, anim);
                f.reset().putByte(Edit::EI_GET_ANIM).putBool(ok);
                if (ok)
                    anim.save(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_SET_ANIM: {
                ...
                bool ok = false;
                if (anim.load(f))
                    ok = Proj.animSet(elm_id, anim);
                f.reset().putByte(Edit::EI_SET_ANIM).putBool(ok).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_ANIM_SKEL: {
                File &f = connection.data;
                UID elm_id = f.getUID();
                f.reset().putByte(Edit::EI_GET_ANIM_SKEL).putUID(Proj.animToSkel(elm_id)).pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_PHYS: {
                ...
                if (Elm *elm = Proj.findElm(elm_id)) {
                    if (elm->type == ELM_OBJ) {
                        ok = true;
                        elm = Proj.objToPhysElm(elm);
                    } // if this is an object, then set ok to true, in case it has no phys, we will just send empty data
                    if (elm && elm->type == ELM_PHYS)
                        ok = data.read(Proj.gamePath(*elm));
                }
                f.reset().putByte(Edit::EI_GET_PHYS).putBool(ok);
                if (ok)
                    data.copy(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_SET_PHYS: {
                ...
                f.reset().putByte(Edit::EI_SET_PHYS).putBool(ok).pos(0);
                connection.send(f);
            } break;

            // OBJ
            case Edit::EI_GET_OBJ_CUR: {
                File &f = connection.data.reset().putByte(Edit::EI_GET_OBJ_CUR).putUID(ObjEdit.obj_id);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_OBJ_CUR: {
                ...
                if (elm)
                    switch (elm->type) {
                    case ELM_MESH:
                        elm = Proj.meshToObjElm(elm);
                        break;
                    case ELM_SKEL:
                        elm = Proj.skelToObjElm(elm);
                        break;
                    }
                ObjEdit.activate(elm);
                f.reset().putByte(Edit::EI_SET_OBJ_CUR).putBool(ObjEdit.obj_id == elm_id);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_GET_OBJ: {
                ...
                f.reset().putByte(Edit::EI_GET_OBJ).putBool(ok);
                if (ok)
                    data.save(f);
                f.pos(0);
                connection.send(f);
            ...
            case Edit::EI_MODIFY_OBJ: {
                File &f = connection.data;
                UID elm_id = f.getUID();
                Memc<Edit::ObjChange> changes;
                changes.load(f, Proj.game_path);
                bool ok = Proj.modifyObj(elm_id, changes);
                f.reset().putByte(Edit::EI_MODIFY_OBJ).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_SET_ACTIVE_APP: {
                File &f = connection.data;
                bool ok = Proj.activateApp(f.getUID());
                f.reset().putByte(Edit::EI_SET_ACTIVE_APP).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_EXPORT_APP: {
                File &f = connection.data;
                Edit::EXPORT_MODE mode = Edit::EXPORT_MODE(f.getByte());
                bool data = f.getBool();
                f.reset();
                bool ok = CodeEdit.Export(mode, data);
                ...
                {
                    queued = cmd;
                    return true; // queue this command to be processed ASAP, return and don't process any other commands, because we still haven't replied to this command yet
                }
                f.putByte(Edit::EI_EXPORT_APP).putBool(ok);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_BUILD_DEBUG: {
                File &f = connection.data;
                CodeEdit.configDebug(f.getBool());
                f.reset().putByte(Edit::EI_BUILD_DEBUG).putBool(true);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_BUILD_32BIT: {
                File &f = connection.data;
                CodeEdit.config32Bit(f.getBool());
                f.reset().putByte(Edit::EI_BUILD_32BIT).putBool(true);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_BUILD_API: {
                File &f = connection.data;
                CodeEdit.configAPI(f.getByte());
                f.reset().putByte(Edit::EI_BUILD_API).putBool(true);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_BUILD_EXE: {
                File &f = connection.data;
                CodeEdit.configEXE(Edit::EXE_TYPE(f.getByte()));
                f.reset().putByte(Edit::EI_BUILD_EXE).putBool(true);
                f.pos(0);
                connection.send(f);
            } break;
            ...
            case Edit::EI_BUILD_PATHS: {
                File &f = connection.data;
                CodeEdit.exportPaths(f.getBool());
                f.reset().putByte(Edit::EI_BUILD_PATHS).putBool(true);
                f.pos(0);
                connection.send(f);
            } break;
            }
