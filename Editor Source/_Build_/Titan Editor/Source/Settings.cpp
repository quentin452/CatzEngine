...
bool SaveSettings(C Str &name) {
    ...
    TextNode &main = data.getNode("Main");
    {
        ...
        {
            TextNode &favorite = fp.getNode("Favorite");
            FREPA(WindowIOFavorites)
            favorite.nodes.New().set("Path", WindowIOFavorites[i]);
            TextNode &recent = fp.getNode("Recent");
            FREPA(WindowIORecents)
            recent.nodes.New().set("Path", WindowIORecents[i]);
        }
    }
    ...
    TextNode &video = data.getNode("Video");
    {
        ...
        video.nodes.New().set("ClangFormatOnSave", D.clangformat());
    ...
    if (TextNode *env = theater.findNode("Environment"))
        theater.nodes.removeData(env, true); // don't store project element
...
void ApplyVideoSettings(C TextData &data) {
    if (C TextNode *video = data.findNode("Video")) {
        int x = -1, y = -1, full = -1;
        if (C TextParam *p = video->findNode("ScreenX"))
            x = p->asInt();
        if (C TextParam *p = video->findNode("ScreenY"))
            y = p->asInt();
        if (C TextParam *p = video->findNode("Fullscreen"))
            full = p->asBool();
        if (C TextParam *p = video->findNode("Exclusive"))
            D.exclusive(p->asBool());
        if (C TextParam *p = video->findNode("ColorSpace"))
            D.colorSpace((COLOR_SPACE)p->asInt());
        if (C TextParam *p = video->findNode("Synchronization"))
            D.sync(p->asBool());
        if (C TextParam *p = video->findNode("AutoSaveScript"))
            D.autosavescript(p->asBool());
        if (C TextParam *p = video->findNode("ClangFormatOnSave"))
        ...
        if (C TextParam *p = video->findNode("TemporalAntiAliasing"))
            D.temporalAntiAlias(p->asBool());
        if (C TextParam *p = video->findNode("TemporalSuperRes"))
            D.temporalSuperRes(p->asBool());
        if (C TextParam *p = video->findNode("EdgeSoftening"))
            D.edgeSoften(EDGE_SOFTEN_MODE(p->asInt()));
        if (C TextParam *p = video->findNode("Shadows"))
            D.shadowMode(p->asBool() ? SHADOW_MAP : SHADOW_NONE);
        if (C TextParam *p = video->findNode("ShadowMapSize"))
            D.shadowMapSize(p->asInt());
        if (C TextParam *p = video->findNode("ShadowMapNum"))
            D.shadowMapNum(p->asInt());
        if (C TextParam *p = video->findNode("ShadowSoft"))
            D.shadowSoft(p->asInt());
        if (C TextParam *p = video->findNode("ShadowJitter"))
            D.shadowJitter(p->asBool());
        if (C TextParam *p = video->findNode("ShadowReduceFlicker"))
            D.shadowReduceFlicker(p->asBool());
        if (C TextParam *p = video->findNode("ShadowRangeFraction"))
            D.shadowFrac(p->asFlt());
        if (C TextParam *p = video->findNode("ShadowFadeFraction"))
            D.shadowFade(p->asFlt());
        if (C TextParam *p = video->findNode("DiffuseMode"))
            D.diffuseMode(DIFFUSE_MODE(p->asInt()));
        if (C TextParam *p = video->findNode("BumpMapping"))
            D.bumpMode(BUMP_MODE(p->asInt()));
        if (C TextParam *p = video->findNode("MotionBlur"))
            D.motionMode(MOTION_MODE(p->asInt()));
        if (C TextParam *p = video->findNode("DepthOfField"))
            D.dofMode(DOF_MODE(p->asInt()));
        if (C TextParam *p = video->findNode("AmbientOcclusion"))
            D.ambientMode(AMBIENT_MODE(p->asInt()));
        if (C TextParam *p = video->findNode("AmbientOcclusionRange"))
            D.ambientRange(p->asFlt());
        if (C TextParam *p = video->findNode("AmbientOcclusionContrast"))
            D.ambientContrast(p->asFlt());
        if (C TextParam *p = video->findNode("EyeAdaptation"))
            D.eyeAdaptation(p->asBool());
        if (C TextParam *p = video->findNode("EyeAdaptationBrightness"))
            D.eyeAdaptationBrightness(p->asFlt());
        if (C TextParam *p = video->findNode("HDR"))
            D.outputPrecision(p->asBool() ? IMAGE_PRECISION_16 : IMAGE_PRECISION_8);
        if (C TextParam *p = video->findNode("ToneMapping"))
            D.toneMap(p->asBool());
        if (C TextParam *p = video->findNode("Contrast"))
            D.contrast(p->asFlt());
        if (C TextParam *p = video->findNode("Dither"))
            D.dither(p->asBool());
        if (C TextParam *p = video->findNode("ColRTPrecision"))
            D.highPrecColRT(p->asBool());
        if (C TextParam *p = video->findNode("NrmRTPrecision"))
            D.highPrecNrmRT(p->asBool());
        if (C TextParam *p = video->findNode("LumRTPrecision"))
            D.highPrecLumRT(p->asBool());
        if (C TextParam *p = video->findNode("LitColRTPrecision"))
            D.litColRTPrecision(IMAGE_PRECISION(p->asInt()));
        if (C TextParam *p = video->findNode("FieldOfView"))
            VidOpt.advanced.setFov(DegToRad(p->asFlt()));
        if (C TextParam *p = video->findNode("TextureFiltering"))
            D.texFilter(p->asInt());
        if (C TextParam *p = video->findNode("TextureMipFiltering"))
            D.texMipFilter(p->asBool());
        if (C TextParam *p = video->findNode("DetailTexture"))
            D.texDetail(TEXTURE_USAGE(p->asInt()));
        if (C TextParam *p = video->findNode("Samples"))
            D.samples(p->asInt());
        if (C TextParam *p = video->findNode("Density"))
            D.density(p->asFlt());
        if (C TextParam *p = video->findNode("DensityFilter"))
            VidOpt.advanced.DensityFilter(VidOpt.advanced, p->asText());
        if (C TextParam *p = video->findNode("Sharpen"))
            D.sharpen(p->asBool());
        if (C TextParam *p = video->findNode("SoftParticles"))
            D.particlesSoft(p->asBool());
        if (C TextParam *p = video->findNode("GrassRange"))
            D.grassRange(p->asFlt());
        if (C TextParam *p = video->findNode("MaxLights"))
            D.maxLights(p->asInt());
        if (C TextParam *p = video->findNode("EdgeDetect"))
            D.edgeDetect(EDGE_DETECT_MODE(p->asInt()));
        if (C TextParam *p = video->findNode("AllowGlow"))
            D.glowAllow(p->asBool());
        if (C TextParam *p = video->findNode("ForwardPerPixel"))
            Renderer.forwardPrecision(p->asBool());
        if (C TextParam *p = video->findNode("MaterialBlendPerPixel"))
            D.materialBlend(p->asBool());
        if (C TextParam *p = video->findNode("SkyNightLightIntensity"))
            VidOpt.advanced.skyNightLightIntensity(p->asFlt());
        if (C TextParam *p = video->findNode("SkyNightLightSchedule"))
            VidOpt.advanced.skyNightLightSchedule(p->asText());
        D.mode(x, y, full);

        // following needs to be called after setting resolution
        if (C TextParam *p = video->findNode("GuiScale"))
            VidOpt.setScale(p->asFlt());
        if (C TextParam *p = video->findNode("GuiScaleWin"))
            VidOpt.setScaleWin(p->asBool());
    }
}
void ApplySettings(C TextData &data) {
    if (C TextNode *main = data.findNode("Main")) {
        ...
        if (C TextParam *p = main->findNode("ServerAddress"))
            Projs.server.set(p->value);
        if (C TextParam *p = main->findNode("Email"))
            Projs.email.set(p->value);
        if (C TextParam *p = main->findNode("NetworkInterface"))
            Projs.editor_network_interface.set(p->asBool());

        if (C TextNode *pl = main->findNode("ProjectList")) {
            if (C TextParam *p = pl->findNode("ChildrenFileSize"))
                Proj.includeChildrenSize(ProjectEx::ElmList::INCLUDE_CHILDREN_SIZE(p->asInt()));
            if (C TextParam *p = pl->findNode("TextureFileSize"))
                Proj.includeTextureSize(ProjectEx::ElmList::INCLUDE_TEXTURE_SIZE(p->asInt()));
            if (C TextParam *p = pl->findNode("TextureFileSizeInOjects"))
                Proj.includeTextureSizeInObject(p->asBool());
            if (C TextParam *p = pl->findNode("UnpublishedFileSize"))
                Proj.includeUnpublishedElmSize(p->asBool());
        }
        if (C TextNode *fp = main->findNode("FilePaths")) {
            if (C TextNode *favorite = fp->findNode("Favorite"))
                for (int i = 0; C TextParam *path = favorite->findNode("Path", i); i++)
                    WindowIOFavorites.include(path->value);
            if (C TextNode *recent = fp->findNode("Recent"))
                for (int i = 0; C TextParam *path = recent->findNode("Path", i); i++)
                    WindowIORecents.include(path->value);
        }
    }
    if (C TextNode *video = data.findNode("Video")) {
        if (C TextParam *p = video->findNode("BloomScale"))
            DefaultEnvironment.bloom.scale = p->asFlt();
        if (C TextParam *p = video->findNode("AmbientLight"))
            DefaultEnvironment.ambient.color_s = p->asFlt();
    }
    if (C TextNode *store = data.findNode("Store")) {
        if (C TextParam *p = store->findNode("Email"))
            AppStore.login.email.set(p->value);
    }
    if (C TextNode *object = data.findNode("Object")) {
        if (C TextNode *general = object->findNode("General")) {
            if (C TextParam *p = general->findNode("ActiveViewport"))
                ObjEdit.setView(p->asInt());
            if (C TextParam *p = general->findNode("CameraMoveMode"))
                ObjEdit.moveMode(Edit::Viewport4::MOVE_MODE(p->asInt()));
            if (C TextParam *p = general->findNode("CameraOrientation"))
                ObjEdit.orientMode(Edit::Viewport4::ORIENT_MODE(p->asInt()));
            if (C TextParam *p = general->findNode("CameraZoom"))
                ObjEdit.zoom(p->asBool());
        }
    }
    if (C TextNode *anim = data.findNode("Animation")) {
        if (C TextNode *general = anim->findNode("General")) {
            if (C TextParam *p = general->findNode("ActiveViewport"))
                AnimEdit.setView(p->asInt());
            if (C TextParam *p = general->findNode("CameraMoveMode"))
                AnimEdit.moveMode(Edit::Viewport4::MOVE_MODE(p->asInt()));
            if (C TextParam *p = general->findNode("CameraOrientation"))
                AnimEdit.orientMode(Edit::Viewport4::ORIENT_MODE(p->asInt()));
            if (C TextParam *p = general->findNode("CameraZoom"))
                AnimEdit.zoom(p->asBool());
        }
    }
    if (C TextNode *world = data.findNode("World")) {
        if (C TextNode *brush = world->findNode("Brush")) {
            if (C TextParam *p = brush->findNode("SizeProportional"))
                Brush.bsize.set(p->asBool());
            if (C TextParam *p = brush->findNode("Image"))
                Brush.setImage(p->value);
        }
        if (C TextNode *colors = world->findNode("Colors")) {
            FREPA(ColorBrush.col)
            if (C TextParam *p = colors->findNode("Color", i))
        ...
        if (C TextNode *general = world->findNode("General")) {
            ...
            if (C TextParam *p = general->findNode("ActiveViewport"))
                WorldEdit.setView(p->asInt());
            if (C TextParam *p = general->findNode("CameraMoveMode"))
                WorldEdit.moveMode(Edit::Viewport4::MOVE_MODE(p->asInt()));
            if (C TextParam *p = general->findNode("CameraOrientation"))
                WorldEdit.orientMode(Edit::Viewport4::ORIENT_MODE(p->asInt()));
            if (C TextParam *p = general->findNode("CameraZoom"))
                WorldEdit.zoom(p->asBool());
            if (C TextParam *p = general->findNode("CameraSpherical"))
                WorldEdit.cam_spherical.set(p->asBool());
            if (C TextParam *p = general->findNode("CameraFppSpeed"))
                WorldEdit.fpp_speed.set(p->asFlt());
        }
    }
    if (C TextNode *theater = data.findNode("Theater")) {
        LoadProperties(Theater.options.props, ConstCast(theater->nodes));
        if (C TextNode *mode = theater->findNode("Mode"))
            Theater.mode.set(mode->asInt());
    }
}
/******************************************************************************/
