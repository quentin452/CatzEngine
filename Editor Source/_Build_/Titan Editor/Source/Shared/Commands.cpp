...
void ClientSendVersion(Connection &conn) {
    File f;
    f.writeMem().putByte(CS_VERSION_CHECK).cmpUIntV(ClientServerVersion).putStr(ClientServerString).pos(0);
    conn.send(f);
}
bool ServerRecvVersion(File &f) {
    if (f.decUIntV() == ClientServerVersion)
        if (f.getStr() == ClientServerString)
            return true;
    return false;
}
void ServerSendVersion(Connection &conn, bool ok) {
    File f;
    f.writeMem().putByte(CS_VERSION_CHECK).putBool(ok).pos(0);
    conn.send(f, -1, false);
}
...
void ClientSendLogin(Connection &conn, C Str &email, C Str &pass, C Str &license_key, OS_VER os_ver) {
    File f;
    f.writeMem().putByte(CS_LOGIN) << email << license_key << PassToMD5(pass);
    f.putByte(os_ver);
    f.pos(0);
    conn.send(f);
}
...
void ServerSendLogin(Connection &conn, LOGIN_RESULT result, USER_ACCESS access) {
    File f;
    f.writeMem().putByte(CS_LOGIN).putByte(result).putByte(access);
    f.pos(0);
    conn.send(f, -1, false);
}
...
void ClientSendRegister(Connection &conn, C Str &email, C Str &pass, C Str &name) {
    File f;
    f.writeMem().putByte(CS_REGISTER) << email << name << PassToMD5(pass);
    f.pos(0);
    conn.send(f);
}
...
void ServerSendRegister(Connection &conn, REGISTER_RESULT result) {
    File f;
    f.writeMem().putByte(CS_REGISTER).putByte(result);
    f.pos(0);
    conn.send(f, -1, false);
}
...
void ClientSendForgotPass(Connection &conn, C Str &email) {
    File f;
    f.writeMem().putByte(CS_FORGOT_PASS) << email;
    f.pos(0);
    conn.send(f);
}
...
void ServerSendForgotPass(Connection &conn, FORGOT_PASS_RESULT result) {
    File f;
    f.writeMem().putByte(CS_FORGOT_PASS).putByte(result);
    f.pos(0);
    conn.send(f, -1, false);
}
...
void ClientSendChangePass(Connection &conn, C Str &email, C Str &new_pass, C Str &old_pass, uint change_pass_key) {
    File f;
    f.writeMem().putByte(CS_CHANGE_PASS) << email << PassToMD5(new_pass) << PassToMD5(old_pass) << change_pass_key;
    f.pos(0);
    conn.send(f);
}
...
void ServerSendChangePass(Connection &conn, CHANGE_PASS_RESULT result) {
    File f;
    f.writeMem().putByte(CS_CHANGE_PASS).putByte(result);
    f.pos(0);
    conn.send(f, -1, false);
}
...
void ClientSendLicenseKey(Connection &conn, C Str &license_key) {
    File f;
    f.writeMem().putByte(CS_LICENSE_KEY) << license_key;
    f.pos(0);
    conn.send(f);
}
...
void ClientSendProjectsListRequest(Connection &conn) {
    File f;
    f.writeMem().putByte(CS_PROJECTS_LIST);
    f.pos(0);
    conn.send(f, -1, false);
}
void ServerSendProjectsList(Connection &conn, Memx<Project> &projects) {
    File f;
    f.writeMem().putByte(CS_PROJECTS_LIST).cmpUIntV(projects.elms());
    FREPAO(projects).save(f, true, Project::SAVE_ID_NAME);
    f.pos(0);
    conn.send(f, -1, false);
}
bool ClientRecvProjectsList(File &f, Memx<Project> &projects) {
    projects.setNum(f.decUIntV());
    int ver;
    FREPA(projects)
    if (projects[i].load(f, ver, true, Project::SAVE_ID_NAME) != LOAD_OK) {
        projects.clear();
        return false;
    }
    return true;
...
void ClientSendProjectOpen(Connection &conn, C UID &proj_id, C Str &proj_name) {
    File f;
    f.writeMem().putByte(CS_PROJECT_OPEN) << proj_name << proj_id;
    f.pos(0);
    conn.send(f, -1, false);
}
...
{
    File f;
    f.writeMem().putByte(CS_PROJECT_DATA);
    f.pos(0);
    conn.send(f, -1, false);
}
bool ServerSendProjectData(Connection &conn, Project &project) {
    ...
    f.writeMem().putByte(CS_PROJECT_DATA);
    File data;
    project.save(data.writeMem(), true);
    data.pos(0);
    ...
    if (Compress(data, f, ServerNetworkCompression, ServerNetworkCompressionLevel)) {
        f.pos(0);
        conn.send(f, -1, false);
        return true;
    }
...
bool ClientRecvProjectData(File &f, Project &project) {
    File decompressed;
    if (Decompress(f, decompressed, true)) {
        decompressed.pos(0);
        int ver;
        return project.load(decompressed, ver, true) == LOAD_OK;
    }
    return false;
}
/******************************************************************************/
...
void ClientSendProjectSettings(Connection &conn, Project &project) {
    File f;
    f.writeMem().putByte(CS_PROJECT_SETTINGS);
    project.save(f, true, Project::SAVE_SETTINGS);
    f.pos(0);
    conn.send(f, -1, false);
}
bool ServerRecvProjectSettings(File &f, Project &project) {
    int ver;
    return project.load(f, ver, true, Project::SAVE_SETTINGS) == LOAD_OK;
}
void ServerWriteProjectSettings(File &f, Project &project) {
    f.putByte(CS_PROJECT_SETTINGS);
    project.save(f, true, Project::SAVE_SETTINGS);
}
...
void ClientSendRenameElm(Connection &conn, C UID &elm_id, C Str &name, C TimeStamp &name_time) {
    File f;
    f.writeMem().putByte(CS_RENAME_ELM) << elm_id << name_time << name;
    f.pos(0);
    conn.send(f, -1, false);
}
...
void ClientSendSetElmParent(Connection &conn, C UID &elm_id, C UID &parent_id, C TimeStamp &parent_time) {
    File f;
    f.writeMem().putByte(CS_SET_ELM_PARENT) << elm_id << parent_id << parent_time;
    f.pos(0);
    conn.send(f, -1, false);
}
...
void ClientSendRemoveElms(Connection &conn, Memc<UID> &elm_ids, bool removed, C TimeStamp &removed_time) {
    if (elm_ids.elms()) {
        File f;
        f.writeMem().putByte(CS_REMOVE_ELMS).cmpUIntV(elm_ids.elms()) << removed << removed_time;
        FREPA(elm_ids)
        f << elm_ids[i];
        f.pos(0);
        conn.send(f, -1, false);
...
void ClientSendPublishElms(Connection &conn, Memc<UID> &elm_ids, sbyte publish, sbyte publish_mobile, C TimeStamp &publish_time) {
    if (elm_ids.elms()) {
        File f;
        f.writeMem().putByte(CS_PUBLISH_ELMS) << publish << publish_mobile << publish_time;
        elm_ids.saveRaw(f);
        f.pos(0);
        conn.send(f, -1, false);
    }
...
void ServerRecvPublishElms(File &f, Memc<UID> &elm_ids, sbyte &publish, sbyte &publish_mobile, TimeStamp &publish_time) {
    f >> publish >> publish_mobile >> publish_time;
    elm_ids.loadRaw(f);
}
...
void ClientSendGetElmNames(Connection &conn, Memc<UID> &elm_ids) {
    if (elm_ids.elms()) {
        File f;
        f.writeMem().putByte(CS_GET_ELM_NAMES).cmpUIntV(elm_ids.elms());
        FREPA(elm_ids)
        f << elm_ids[i];
        f.pos(0);
        conn.send(f, -1, false);
...
void ServerSendGetElmNames(Connection &conn, Memc<ElmName> &elm_names, C UID &proj_id) {
    if (elm_names.elms()) {
        File f;
        f.writeMem().putByte(CS_GET_ELM_NAMES);
        File data;
        data.writeMem().cmpUIntV(elm_names.elms()) << proj_id;
        FREPA(elm_names)
        elm_names[i].save(data);
        data.pos(0);
        if (Compress(data, f, ServerNetworkCompression, ServerNetworkCompressionLevel)) {
            f.pos(0);
            conn.send(f, -1, false);
...
void ClientRecvGetElmNames(File &f, Memc<ElmName> &elm_names, UID &proj_id) {
    File data;
    if (Decompress(f, data, true)) {
        data.pos(0);
        elm_names.setNum(data.decUIntV());
        data >> proj_id;
        FREPA(elm_names)
        elm_names[i].load(data);
    }
}
/******************************************************************************/
...
void ClientSendGetTextures(Connection &conn, Memc<UID> &tex_ids) {
    if (tex_ids.elms()) {
        File f;
        f.writeMem().putByte(CS_GET_TEXTURES).cmpUIntV(tex_ids.elms());
        FREPA(tex_ids)
        f << tex_ids[i];
        f.pos(0);
        conn.send(f, -1, false);
...
void ClientSendGetElmShort(Connection &conn, Memc<UID> &elm_ids) {
    if (elm_ids.elms()) {
        File f;
        f.writeMem().putByte(CS_GET_ELM_SHORT).cmpUIntV(elm_ids.elms());
        FREPA(elm_ids)
        f << elm_ids[i];
        f.pos(0);
        conn.send(f, -1, false);
...
void ClientSendSetElmShort(Connection &conn, Elm &elm, C Project &proj) {
    File f;
    f.writeMem().putByte(CS_SET_ELM_SHORT);
    elm.compressData(f);
    if (ElmFileInShort(elm.type)) {
        File data;
        if (data.read(proj.basePath(elm)))
            data.copy(f);
    }
    f.pos(0);
    conn.send(f, -1, false);
...
bool ServerRecvSetElmShort(File &f, Elm &elm, File &data) {
    if (!elm.decompressData(f))
        return false;
    return f.copy(data);
}

void ServerWriteSetElmShort(File &f, Elm &elm, C Project &proj) {
    f.putByte(CS_SET_ELM_SHORT) << proj.id;
    elm.compressData(f);
    if (ElmFileInShort(elm.type)) {
        File data;
        if (data.read(proj.basePath(elm)))
...
void ClientSendGetElmLong(Connection &conn, Memc<UID> &elm_ids) {
    if (elm_ids.elms()) {
        File f;
        f.writeMem().putByte(CS_GET_ELM_LONG).cmpUIntV(elm_ids.elms());
        FREPA(elm_ids)
        f << elm_ids[i];
        f.pos(0);
        conn.send(f, -1, false);
...
void ClientWriteSetElmLong(File &elm_file, File &data_file, Elm &elm, Project &proj, bool &compress) {
    elm_file.writeMem().putByte(CS_SET_ELM_LONG);
    elm.compressData(elm_file);
    if (ElmHasFile(elm.type)) {
        if (ElmSendBoth(elm.type)) {
            File data;
            data.read(proj.editPath(elm));
            elm_file.cmpUIntV(data.size());
            data.copy(elm_file); // if sending both, then store edit in the 'elm_file', because we will keep 'elm_file' uncompressed, and only 'data_file' compressed for 'ElmCompressable'
            if (data.read(proj.gamePath(elm)))
                data.copy(data_file);
        } else {
            File data;
            if (data.read(proj.basePath(elm)))
...
void ServerWriteSetElmLong(File &f, Elm &elm, C Project &proj) {
    f.putByte(CS_SET_ELM_LONG) << proj.id;
    elm.compressData(f);
    if (ElmHasFile(elm.type)) {
        if (ElmSendBoth(elm.type)) {
            File data;
            data.read(proj.editPath(elm));
            f.cmpUIntV(data.size());
            data.copy(f);
            if (data.read(proj.gamePath(elm)))
                data.copy(f);
        } else {
...
bool ClientRecvSetElmLong(File &f, Elm &elm, File &data, File &extra, UID &proj_id) {
    ...
    if (ElmHasFile(elm.type)) {
        if (ElmSendBoth(elm.type)) {
            f.copy(data, f.decUIntV());
            if (ElmCompressable(elm.type) ? !Decompress(f, extra) : !f.copy(extra))
                return false;
        } else {
            if (ElmCompressable(elm.type) ? !Decompress(f, data) : !f.copy(data))
                return false;
        }
    }
    return true;
...
void ClientWriteSetElmFull(File &elm_file, File &data_file, Elm &elm, Project &proj, bool &compress) {
    elm_file.writeMem().putByte(CS_SET_ELM_FULL);
    elm.save(elm_file, true, false);
    if (ElmHasFile(elm.type)) {
        if (ElmSendBoth(elm.type)) {
            File data;
            data.read(proj.editPath(elm));
...
void ServerWriteSetElmFull(File &f, Elm &elm, C Project &proj) {
    f.putByte(CS_SET_ELM_FULL) << proj.id;
    elm.save(f, true, false);
    if (ElmHasFile(elm.type)) {
        if (ElmSendBoth(elm.type)) {
            File data;
            data.read(proj.editPath(elm));
...
void ClientSendGetWorldVer(Connection &conn, C UID &world_id) {
    File f;
    f.writeMem().putByte(CS_GET_WORLD_VER) << world_id;
    f.pos(0);
    conn.send(f, -1, false);
}
...
void ServerSendGetWorldVer(Connection &conn, WorldVer *world_ver, C UID &world_id, C UID &proj_id) {
    File f;
    f.writeMem().putByte(CS_GET_WORLD_VER) << world_id << proj_id;
    f.putBool(world_ver != null);
    if (world_ver) {
        File temp;
        world_ver->save(temp.writeMem(), true);
        temp.pos(0);
...
void ClientSendGetWorldAreas(Connection &conn, C UID &world_id, Memc<AreaSync> &areas) {
    if (areas.elms()) {
        File f;
        f.writeMem().putByte(CS_GET_WORLD_AREAS) << world_id;
        f.cmpUIntV(areas.elms());
        FREPA(areas)
        f << areas[i].xy << areas[i].flag;
        f.pos(0);
        conn.send(f, -1, false);
    }
}
...
void WriteSetWorldArea(File &f, C UID &world_id, C VecI2 &area_xy, byte area_sync_flag, C AreaVer &ver, C Heightmap *hm, C Memc<ObjData> &objs, C Str &edit_path) {
    ...
    if (area_sync_flag & AREA_SYNC_REMOVED)
        f << ver.hm_removed_time;
...
bool RecvSetWorldArea(File &f, UID &world_id, VecI2 &area_xy, byte &area_sync_flag, AreaVer &ver, Heightmap &hm, Memc<ObjData> &objs, C Str &game_path, C Str &edit_path) {
    ...
    if (area_sync_flag & (AREA_SYNC_HEIGHT | AREA_SYNC_MTRL | AREA_SYNC_COLOR)) {
        ...
        if (area_sync_flag & AREA_SYNC_HEIGHT) {
            ...
            REPD(y, res)
            REPD(x, res)
            hm.height(x, y, f.getFlt());
        }
        if (area_sync_flag & AREA_SYNC_MTRL) {
            f >> ver.hm_mtrl_time;
            Mems<MaterialPtr> mtrls;
            mtrls.setNum(f.decUIntV());
            FREPA(mtrls) {
                UID mtrl_id;
                f >> mtrl_id;
                if (mtrl_id.valid())
                    mtrls[i] = game_path + EncodeFileName(mtrl_id);
            }
            REPD(y, res)
            REPD(x, res) {
                VecB4 m, i;
...
bool ServerRecvSetWorldArea(File &f, UID &world_id, VecI2 &area_xy, byte &area_sync_flag, AreaVer &ver, Heightmap &hm, C Str &game_path, C Str &edit_path) {
    ...
    if (Decompress(f, data, true)) {
        Memc<ObjData> objs;
        data.pos(0);
        return RecvSetWorldArea(data, world_id, area_xy, area_sync_flag, ver, hm, objs, game_path, edit_path);
    }
    return false;
}
...
void ServerSendSetWorldArea(Connection &conn, C UID &world_id, C VecI2 &area_xy, byte area_sync_flag, C AreaVer &ver, C Heightmap *hm, Memc<ObjData> &objs, C UID &proj_id, C Str &edit_path) {
    File f;
    f.writeMem().putByte(CS_SET_WORLD_AREA);
    File data;
    data.writeMem() << proj_id;
    WriteSetWorldArea(data, world_id, area_xy, area_sync_flag, ver, hm, objs, edit_path);
    data.pos(0);
    if (Compress(data, f, ServerNetworkCompression, ServerNetworkCompressionLevel)) {
        f.pos(0);
        conn.send(f, -1, false);
...
bool ServerRecvSetWorldObjs(File &f, UID &world_id, VecI2 &area_xy, Memc<ObjData> &objs, C Str &edit_path) {
    ...
    if (Decompress(f, data, true)) {
        data.pos(0);
        data.decIntV(area_xy.x).decIntV(area_xy.y) >> world_id;
...
void ClientSendGetWorldWaypoints(Connection &conn, C UID &world_id, Memc<UID> &waypoints) {
    if (waypoints.elms()) {
        File f;
        f.writeMem().putByte(CS_GET_WORLD_WAYPOINTS) << world_id;
        f.cmpUIntV(waypoints.elms());
        FREPA(waypoints)
        f << waypoints[i];
        f.pos(0);
        conn.send(f, -1, false);
...
void ClientSendSetWorldWaypoint(Connection &conn, C UID &world_id, C UID &waypoint_id, C Version &waypoint_ver, EditWaypoint &waypoint) {
    File f;
    f.writeMem().putByte(CS_SET_WORLD_WAYPOINT) << world_id << waypoint_id << waypoint_ver;
    waypoint.save(f);
    f.pos(0);
    conn.send(f, -1, false);
}
bool ServerRecvSetWorldWaypoint(File &f, UID &world_id, UID &waypoint_id, Version &waypoint_ver, EditWaypoint &waypoint) {
    f >> world_id >> waypoint_id >> waypoint_ver;
    return waypoint.load(f);
}
...
void ServerWriteSetWorldWaypoint(File &f, C UID &world_id, C UID &waypoint_id, C Version &waypoint_ver, EditWaypoint &waypoint, C UID &proj_id) {
    f.putByte(CS_SET_WORLD_WAYPOINT) << world_id << waypoint_id << waypoint_ver << proj_id;
    waypoint.save(f);
}
bool ClientRecvSetWorldWaypoint(File &f, UID &world_id, UID &waypoint_id, Version &waypoint_ver, EditWaypoint &waypoint, UID &proj_id) {
    f >> world_id >> waypoint_id >> waypoint_ver >> proj_id;
    return waypoint.load(f);
}
/******************************************************************************/
// CS_GET_WORLD_LAKES
...
void ClientSendGetWorldLakes(Connection &conn, C UID &world_id, Memc<UID> &lakes) {
    if (lakes.elms()) {
        File f;
        f.writeMem().putByte(CS_GET_WORLD_LAKES) << world_id;
        f.cmpUIntV(lakes.elms());
        FREPA(lakes)
        f << lakes[i];
        f.pos(0);
        conn.send(f, -1, false);
...
void ClientSendGetWorldRivers(Connection &conn, C UID &world_id, Memc<UID> &rivers) {
    if (rivers.elms()) {
        File f;
        f.writeMem().putByte(CS_GET_WORLD_RIVERS) << world_id;
        f.cmpUIntV(rivers.elms());
        FREPA(rivers)
        f << rivers[i];
        f.pos(0);
        conn.send(f, -1, false);
...
void ClientSendGetMiniMapVer(Connection &conn, C UID &mini_map_id) {
    File f;
    f.writeMem().putByte(CS_GET_MINI_MAP_VER) << mini_map_id;
    f.pos(0);
    conn.send(f, -1, false);
}
...
void ServerSendGetMiniMapVer(Connection &conn, MiniMapVer *mini_map_ver, C UID &mini_map_id, C UID &proj_id) {
    File f;
    f.writeMem().putByte(CS_GET_MINI_MAP_VER) << mini_map_id << proj_id;
...
void ClientSendGetMiniMapImages(Connection &conn, C UID &mini_map_id, Memc<VecI2> &images) {
    File f;
    f.writeMem().putByte(CS_GET_MINI_MAP_IMAGES) << mini_map_id;
...
void ServerWriteSetMiniMapSettings(File &f, C UID &mini_map_id, C Game::MiniMap::Settings &settings, C TimeStamp &settings_time, C UID &proj_id) {
    f.putByte(CS_SET_MINI_MAP_SETTINGS) << mini_map_id << proj_id << settings_time;
    settings.save(f);
}
bool ClientRecvSetMiniMapSettings(File &f, UID &mini_map_id, Game::MiniMap::Settings &settings, TimeStamp &settings_time, UID &proj_id) {
    f >> mini_map_id >> proj_id >> settings_time;
    return settings.load(f);
}
/******************************************************************************/
...
void ServerSendSetMiniMapImage(Connection &conn, C UID &mini_map_id, C VecI2 &image_xy, C TimeStamp &image_time, File &cmpr_image_data, C UID &proj_id) {
    File f;
    f.writeMem().putByte(CS_SET_MINI_MAP_IMAGE) << mini_map_id << proj_id << image_xy << image_time;
...
bool ClientRecvSetMiniMapImage(File &f, UID &mini_map_id, VecI2 &image_xy, TimeStamp &image_time, File &image_data, UID &proj_id) {
    f >> mini_map_id >> proj_id >> image_xy >> image_time;
...
void ClientWriteSetMiniMapImage(File &elm, File &data, C UID &mini_map_id, C VecI2 &image_xy, C TimeStamp &image_time, File &image_data) {
    elm.putByte(CS_SET_MINI_MAP_IMAGE) << mini_map_id << image_xy << image_time;
...
void ClientSendGetCodeVer(Connection &conn, Memc<ElmTypeVer> &elms) {
    ...
    File f;
    f.writeMem().putByte(CS_GET_CODE_VER);
    elms.save(f);
    f.pos(0);
    conn.send(f, -1, false);
}
...
void ServerSendSetCodeData(Connection &conn, C Memc<ElmCodeData> &elms, C UID &proj_id) // use compression because source code can be big
{
    ...
    data.pos(0);
    File f;
    f.writeMem().putByte(CS_SET_CODE_DATA);
    Compress(data, f, ServerNetworkCompression, ServerNetworkCompressionLevel);
    f.pos(0);
    conn.send(f, -1, false);
}
...
void ClientSendSetCodeData(Connection &conn, C Memc<ElmCodeData> &elms) // use compression because source code can be big
{
    ...
    File f;
    f.writeMem().putByte(CS_SET_CODE_DATA);
    Compress(data, f, ServerNetworkCompression, 9);
    f.pos(0);
    conn.send(f, -1, false);
}
...
{
    File data;
    data.writeMem() << resync << proj_id;
    elms.save(data);
    data.pos(0);
    File f;
    f.writeMem().putByte(CS_CODE_SYNC_STATUS);
    Compress(data, f, ServerNetworkCompression, ServerNetworkCompressionLevel);
    f.pos(0);
    conn.send(f, -1, false);
}
...
/******************************************************************************/
void ElmName::set(Elm &elm) {
    id = elm.id;
    time = elm.name_time;
    name = elm.name;
}
bool ElmName::save(File &f) {
    f << id << time << name;
    return f.ok();
}
bool ElmName::load(File &f) {
    f >> id >> time >> name;
    return f.ok();
}
void AreaSync::set(uint flag, C VecI2 &xy) {
    T.flag = flag;
    T.xy = xy;
}
void ElmTypeVer::set(C UID &id, ELM_TYPE type, C Version &ver) {
    T.id = id;
    T.type = type;
    T.ver = ver;
}
void ElmTypeVer::set(C Elm &elm) { set(elm.id, elm.type, elm.data ? elm.data->ver : Version()); }
bool ElmTypeVer::save(File &f) C {
    f << id << type << ver;
    return f.ok();
}
bool ElmTypeVer::load(File &f) {
    f >> id >> type >> ver;
    return f.ok();
}
...
void ElmCodeBase::set(C UID &id, C Str &data) {
    T.id = id;
    T.data = data;
}
bool ElmCodeBase::save(File &f) C {
    f << id << data;
    return f.ok();
}
bool ElmCodeBase::load(File &f) {
    f >> id >> data;
    return f.ok();
}
/******************************************************************************/
