...
bool EditEnv::newer(C EditEnv &src) C {
    return ambient_on_time > src.ambient_on_time || ambient_color_time > src.ambient_color_time || night_shade_color_time > src.night_shade_color_time || bloom_on_time > src.bloom_on_time || bloom_original_time > src.bloom_original_time || bloom_scale_time > src.bloom_scale_time || bloom_cut_time > src.bloom_cut_time || bloom_glow_time > src.bloom_glow_time || clouds_on_time > src.clouds_on_time || clouds_vertical_scale_time > src.clouds_vertical_scale_time || clouds_scale_time[0] > src.clouds_scale_time[0] || clouds_velocity_time[0] > src.clouds_velocity_time[0] || clouds_color_time[0] > src.clouds_color_time[0] || clouds_image_time[0] > src.clouds_image_time[0] || clouds_scale_time[1] > src.clouds_scale_time[1] || clouds_velocity_time[1] > src.clouds_velocity_time[1] || clouds_color_time[1] > src.clouds_color_time[1] || clouds_image_time[1] > src.clouds_image_time[1] || clouds_scale_time[2] > src.clouds_scale_time[2] || clouds_velocity_time[2] > src.clouds_velocity_time[2] || clouds_color_time[2] > src.clouds_color_time[2] || clouds_image_time[2] > src.clouds_image_time[2] || clouds_scale_time[3] > src.clouds_scale_time[3] || clouds_velocity_time[3] > src.clouds_velocity_time[3] || clouds_color_time[3] > src.clouds_color_time[3] || clouds_image_time[3] > src.clouds_image_time[3] || fog_on_time > src.fog_on_time || fog_affect_sky_time > src.fog_affect_sky_time || fog_density_time > src.fog_density_time || fog_color_time > src.fog_color_time || sky_on_time > src.sky_on_time || sky_frac_time > src.sky_frac_time || sky_atmospheric_density_exponent_time > src.sky_atmospheric_density_exponent_time || sky_atmospheric_horizon_exponent_time > src.sky_atmospheric_horizon_exponent_time || sky_atmospheric_horizon_color_time > src.sky_atmospheric_horizon_color_time || sky_atmospheric_sky_color_time > src.sky_atmospheric_sky_color_time || sky_atmospheric_stars_time > src.sky_atmospheric_stars_time || sky_atmospheric_stars_orientation_time > src.sky_atmospheric_stars_orientation_time || sky_skybox_time > src.sky_skybox_time || sun_on_time > src.sun_on_time || sun_glow_time > src.sun_glow_time || sun_size_time > src.sun_size_time || sun_highlight_front_time > src.sun_highlight_front_time || sun_highlight_back_time > src.sun_highlight_back_time || sun_pos_time > src.sun_pos_time || sun_light_color_time > src.sun_light_color_time || sun_rays_color_time > src.sun_rays_color_time || sun_image_color_time > src.sun_image_color_time || sun_image_time > src.sun_image_time;
}
...
void EditEnv::newData() {
    ...
    bloom_glow_time++;
    clouds_on_time++;
    clouds_vertical_scale_time++;
    REPAO(clouds_scale_time)
    ++;
    REPAO(clouds_velocity_time)
    ++;
    REPAO(clouds_color_time)
    ++;
    REPAO(clouds_image_time)
    ++;
    fog_on_time++;
    fog_affect_sky_time++;
    fog_density_time++;
    ...
    sky_on_time++;
    sky_frac_time++;
    sky_atmospheric_density_exponent_time++;
    sky_atmospheric_horizon_exponent_time++;
    sky_atmospheric_horizon_color_time++;
    sky_atmospheric_sky_color_time++;
    sky_atmospheric_stars_time++;
    sky_atmospheric_stars_orientation_time++;
    sky_skybox_time++;
    sun_on_time++;
    sun_glow_time++;
    sun_size_time++;
    sun_highlight_front_time++;
    sun_highlight_back_time++;
    sun_pos_time++;
    sun_light_color_time++;
    sun_rays_color_time++;
    sun_image_color_time++;
    sun_image_time++;
}
...
bool EditEnv::load(File &f) {
    ...
    case 6: {
        if (!super::load(f))
            break;
        f >> sun_id >> star_id >> skybox_id >> cloud_id >> ambient_on_time >> ambient_color_time >> night_shade_color_time >> bloom_on_time >> bloom_original_time >> bloom_scale_time >> bloom_cut_time >> bloom_glow_time >> clouds_on_time >> clouds_vertical_scale_time >> clouds_scale_time >> clouds_velocity_time >> clouds_color_time >> clouds_image_time >> fog_on_time >> fog_affect_sky_time >> fog_density_time >> fog_color_time >> sky_on_time >> sky_frac_time >> sky_atmospheric_density_exponent_time >> sky_atmospheric_horizon_exponent_time >> sky_atmospheric_horizon_color_time >> sky_atmospheric_sky_color_time >> sky_atmospheric_stars_time >> sky_atmospheric_stars_orientation_time >> sky_skybox_time >> sun_on_time >> sun_glow_time >> sun_size_time >> sun_highlight_front_time >> sun_highlight_back_time >> sun_pos_time >> sun_light_color_time >> sun_rays_color_time >> sun_image_color_time >> sun_image_time;
        if (f.ok())
            return true;
    } break;
    ...
    case 5: {
        if (!super::load(f))
            break;
        f >> sun_id >> star_id >> skybox_id >> cloud_id >> ambient_on_time >> ambient_color_time >> night_shade_color_time >> bloom_on_time >> bloom_half_time >> bloom_maximum_time >> bloom_blurs_time >> bloom_original_time >> bloom_scale_time >> bloom_cut_time >> clouds_on_time >> clouds_vertical_scale_time >> clouds_scale_time >> clouds_velocity_time >> clouds_color_time >> clouds_image_time >> fog_on_time >> fog_affect_sky_time >> fog_density_time >> fog_color_time >> sky_on_time >> sky_frac_time >> sky_atmospheric_density_exponent_time >> sky_atmospheric_horizon_exponent_time >> sky_atmospheric_horizon_color_time >> sky_atmospheric_sky_color_time >> sky_atmospheric_stars_time >> sky_atmospheric_stars_orientation_time >> sky_skybox_time >> sun_on_time >> sun_glow_time >> sun_size_time >> sun_highlight_front_time >> sun_highlight_back_time >> sun_pos_time >> sun_light_color_time >> sun_rays_color_time >> sun_image_color_time >> sun_image_time;
        if (f.ok())
            return true;
    } break;
    ...
    case 4: {
        if (!super::load(f))
            break;
        f >> sun_id >> star_id >> skybox_id >> cloud_id >> ambient_on_time >> ambient_color_time >> night_shade_color_time >> bloom_on_time >> bloom_half_time >> bloom_saturate_time >> bloom_maximum_time >> bloom_blurs_time >> bloom_original_time >> bloom_scale_time >> bloom_cut_time >> clouds_on_time >> clouds_vertical_scale_time >> clouds_scale_time >> clouds_velocity_time >> clouds_color_time >> clouds_image_time >> fog_on_time >> fog_affect_sky_time >> fog_density_time >> fog_color_time >> sky_on_time >> sky_frac_time >> sky_atmospheric_density_exponent_time >> sky_atmospheric_horizon_exponent_time >> sky_atmospheric_horizon_color_time >> sky_atmospheric_sky_color_time >> sky_atmospheric_stars_time >> sky_atmospheric_stars_orientation_time >> sky_skybox_time >> sun_on_time >> sun_glow_time >> sun_size_time >> sun_highlight_front_time >> sun_highlight_back_time >> sun_pos_time >> sun_light_color_time >> sun_rays_color_time >> sun_image_color_time >> sun_image_time;
        if (f.ok())
            return true;
    } break;
    ...
    case 3: {
        if (!super::load(f))
            break;
        f >> sun_id >> star_id >> skybox_id >> cloud_id >> ambient_on_time >> ambient_color_time >> night_shade_color_time >> bloom_on_time >> bloom_half_time >> bloom_saturate_time >> bloom_maximum_time >> bloom_blurs_time >> bloom_original_time >> bloom_scale_time >> bloom_cut_time >> clouds_on_time >> clouds_vertical_scale_time >> clouds_scale_time >> clouds_velocity_time >> clouds_color_time >> clouds_image_time >> fog_on_time >> fog_affect_sky_time >> fog_density_time >> fog_color_time >> sky_on_time >> sky_frac_time >> sky_atmospheric_density_exponent_time >> sky_atmospheric_horizon_exponent_time >> sky_atmospheric_horizon_color_time >> sky_atmospheric_sky_color_time >> sky_atmospheric_stars_time >> sky_atmospheric_stars_orientation_time >> sky_skybox_time >> sun_on_time >> sun_blend_time >> sun_glow_time >> sun_size_time >> sun_highlight_front_time >> sun_highlight_back_time >> sun_pos_time >> sun_light_color_time >> sun_rays_color_time >> sun_image_color_time >> sun_image_time;
        if (f.ok())
            return true;
    } break;
    ...
    case 2: {
        if (!super::load(f))
            break;
        f >> sun_id >> star_id >> skybox_id >> cloud_id >> ambient_on_time >> ambient_color_time >> night_shade_color_time >> bloom_on_time >> bloom_half_time >> bloom_saturate_time >> bloom_maximum_time >> bloom_blurs_time >> bloom_original_time >> bloom_scale_time >> bloom_cut_time >> clouds_on_time >> clouds_vertical_scale_time >> clouds_ray_mask_contrast_time >> clouds_scale_time >> clouds_velocity_time >> clouds_color_time >> clouds_image_time >> fog_on_time >> fog_affect_sky_time >> fog_density_time >> fog_color_time >> sky_on_time >> sky_frac_time >> sky_atmospheric_density_exponent_time >> sky_atmospheric_horizon_exponent_time >> sky_atmospheric_horizon_color_time >> sky_atmospheric_sky_color_time >> sky_atmospheric_stars_time >> sky_atmospheric_stars_orientation_time >> sky_skybox_time >> sun_on_time >> sun_blend_time >> sun_glow_time >> sun_size_time >> sun_highlight_front_time >> sun_highlight_back_time >> sun_pos_time >> sun_light_color_time >> sun_rays_color_time >> sun_image_color_time >> sun_image_time;
        if (f.ok())
            return true;
    } break;
    ...
    case 1: {
        if (!super::load(f))
            break;
        f >> sun_id >> star_id >> skybox_id >> cloud_id >> ambient_on_time >> ambient_color_time >> night_shade_color_time >> bloom_on_time >> bloom_half_time >> bloom_maximum_time >> bloom_blurs_time >> bloom_original_time >> bloom_scale_time >> bloom_cut_time >> bloom_contrast_time >> clouds_on_time >> clouds_vertical_scale_time >> clouds_ray_mask_contrast_time >> clouds_scale_time >> clouds_velocity_time >> clouds_color_time >> clouds_image_time >> fog_on_time >> fog_affect_sky_time >> fog_density_time >> fog_color_time >> sky_on_time >> sky_frac_time >> sky_atmospheric_density_exponent_time >> sky_atmospheric_horizon_exponent_time >> sky_atmospheric_horizon_color_time >> sky_atmospheric_sky_color_time >> sky_atmospheric_stars_time >> sky_atmospheric_stars_orientation_time >> sky_skybox_time >> sun_on_time >> sun_blend_time >> sun_glow_time >> sun_size_time >> sun_highlight_front_time >> sun_highlight_back_time >> sun_pos_time >> sun_light_color_time >> sun_rays_color_time >> sun_image_color_time >> sun_image_time;
        if (f.ok())
            return true;
    } break;
    ...
    case 0: {
        if (!super::load(f))
            break;
        f >> sun_id >> star_id >> skybox_id >> cloud_id >> ambient_on_time >> ambient_color_time >> bloom_on_time >> bloom_half_time >> bloom_maximum_time >> bloom_blurs_time >> bloom_original_time >> bloom_scale_time >> bloom_cut_time >> bloom_contrast_time >> clouds_on_time >> clouds_vertical_scale_time >> clouds_ray_mask_contrast_time >> clouds_scale_time >> clouds_velocity_time >> clouds_color_time >> clouds_image_time >> fog_on_time >> fog_affect_sky_time >> fog_density_time >> fog_color_time >> sky_on_time >> sky_frac_time >> sky_atmospheric_density_exponent_time >> sky_atmospheric_horizon_exponent_time >> sky_atmospheric_horizon_color_time >> sky_atmospheric_sky_color_time >> sky_atmospheric_stars_time >> sky_atmospheric_stars_orientation_time >> sky_skybox_time >> sun_on_time >> sun_blend_time >> sun_glow_time >> sun_size_time >> sun_highlight_front_time >> sun_highlight_back_time >> sun_pos_time >> sun_light_color_time >> sun_rays_color_time >> sun_image_color_time >> sun_image_time;
        if (f.ok())
            return true;
    } break;
    }
