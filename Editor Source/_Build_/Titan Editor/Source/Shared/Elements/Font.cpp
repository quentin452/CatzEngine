...
Str EditFont::chars() C {
    ...
    if (ascii)
        c += Ascii;
    if (chinese)
        c += LanguageSpecific(CN);
    if (japanese)
        c += LanguageSpecific(JP);
    if (korean)
        c += LanguageSpecific(KO);
    if (german)
...
bool EditFont::make(Font &font, C Str *chars) C {
    ...
    if (chars || fp.software)
        fp.image_type = ImageTypeUncompressed(fp.image_type); // if 'chars' is provided then it means we're creating only a preview, so use un-compressed for fast processing (also for software modes)
    return font.create(fp);
}
bool EditFont::newer(C EditFont &src) C {
    return diagonal_shadow_time > src.diagonal_shadow_time || clear_type_time > src.clear_type_time || software_time > src.software_time || ascii_time > src.ascii_time || german_time > src.german_time || french_time > src.french_time || polish_time > src.polish_time || chinese_time > src.chinese_time || japanese_time > src.japanese_time || korean_time > src.korean_time || russian_time > src.russian_time || mip_maps_time > src.mip_maps_time || size_time > src.size_time || scale_time > src.scale_time || weight_time > src.weight_time || shadow_blur_time > src.shadow_blur_time || shadow_opacity_time > src.shadow_opacity_time || shadow_spread_time > src.shadow_spread_time || font_time > src.font_time || custom_chars_time > src.custom_chars_time;
}
...
bool EditFont::load(File &f) {
    flt min_filter;
    TimeStamp min_filter_time;
    reset();
    switch (f.decUIntV()) {
    case 6: {
        f >> diagonal_shadow >> clear_type >> software >> ascii >> german >> french >> polish >> chinese >> japanese >> korean >> russian >> mip_maps >> size >> scale >> weight >> shadow_blur >> shadow_opacity >> shadow_spread >> diagonal_shadow_time >> clear_type_time >> software_time >> ascii_time >> german_time >> french_time >> polish_time >> chinese_time >> japanese_time >> korean_time >> russian_time >> mip_maps_time >> size_time >> scale_time >> weight_time >> shadow_blur_time >> shadow_opacity_time >> shadow_spread_time >> font_time >> custom_chars_time >> font >> custom_chars;
        if (f.ok())
            return true;
    } break;

    case 5: {
        f >> diagonal_shadow >> clear_type >> software >> ascii >> german >> french >> polish >> chinese >> japanese >> korean >> russian >> mip_maps >> size >> scale >> weight >> min_filter >> shadow_blur >> shadow_opacity >> shadow_spread >> diagonal_shadow_time >> clear_type_time >> software_time >> ascii_time >> german_time >> french_time >> polish_time >> chinese_time >> japanese_time >> korean_time >> russian_time >> mip_maps_time >> size_time >> scale_time >> weight_time >> min_filter_time >> shadow_blur_time >> shadow_opacity_time >> shadow_spread_time >> font_time >> custom_chars_time >> font >> custom_chars;
        if (f.ok())
            return true;
    } break;

    case 4: {
        f >> diagonal_shadow >> clear_type >> ascii >> german >> french >> polish >> chinese >> japanese >> korean >> russian >> mip_maps >> size >> scale >> weight >> min_filter >> shadow_blur >> shadow_opacity >> shadow_spread >> diagonal_shadow_time >> clear_type_time >> ascii_time >> german_time >> french_time >> polish_time >> chinese_time >> japanese_time >> korean_time >> russian_time >> mip_maps_time >> size_time >> scale_time >> weight_time >> min_filter_time >> shadow_blur_time >> shadow_opacity_time >> shadow_spread_time >> font_time >> custom_chars_time >> font >> custom_chars;
        if (f.ok())
            return true;
    } break;

    case 3: {
        f >> diagonal_shadow >> clear_type >> ascii >> german >> french >> polish >> chinese >> japanese >> korean >> russian >> mip_maps >> size >> scale >> weight >> min_filter >> shadow_blur >> shadow_opacity >> shadow_spread >> diagonal_shadow_time >> clear_type_time >> ascii_time >> german_time >> french_time >> polish_time >> chinese_time >> japanese_time >> korean_time >> russian_time >> mip_maps_time >> size_time >> scale_time >> weight_time >> min_filter_time >> shadow_blur_time >> shadow_opacity_time >> shadow_spread_time >> font_time >> custom_chars_time;
        GetStr2(f, font);
        GetStr2(f, custom_chars);
        if (f.ok())
            return true;
    } break;

    case 2: {
        f >> diagonal_shadow >> clear_type >> ascii >> german >> french >> polish >> chinese >> japanese >> korean >> russian >> mip_maps >> size >> weight >> min_filter >> shadow_blur >> shadow_opacity >> shadow_spread >> diagonal_shadow_time >> clear_type_time >> ascii_time >> german_time >> french_time >> polish_time >> chinese_time >> japanese_time >> korean_time >> russian_time >> mip_maps_time >> size_time >> weight_time >> min_filter_time >> shadow_blur_time >> shadow_opacity_time >> shadow_spread_time >> font_time >> custom_chars_time;
        GetStr2(f, font);
        GetStr2(f, custom_chars);
        if (f.ok())
            return true;
    ...
    case 1: {
        f >> diagonal_shadow >> clear_type >> ascii >> german >> french >> polish >> chinese >> japanese >> korean >> russian >> mip_maps >> size >> weight >> min_filter >> shadow_blur >> shadow_opacity >> diagonal_shadow_time >> clear_type_time >> ascii_time >> german_time >> french_time >> polish_time >> chinese_time >> japanese_time >> korean_time >> russian_time >> mip_maps_time >> size_time >> weight_time >> min_filter_time >> shadow_blur_time >> shadow_opacity_time >> font_time >> custom_chars_time;
        GetStr(f, font);
        GetStr(f, custom_chars);
        if (f.ok())
            return true;
    } break;

    case 0: {
        f >> diagonal_shadow >> clear_type >> ascii >> german >> french >> polish >> chinese >> russian >> mip_maps >> size >> weight >> min_filter >> shadow_blur >> shadow_opacity >> diagonal_shadow_time >> clear_type_time >> ascii_time >> german_time >> french_time >> polish_time >> chinese_time >> russian_time >> mip_maps_time >> size_time >> weight_time >> min_filter_time >> shadow_blur_time >> shadow_opacity_time >> font_time >> custom_chars_time;
        GetStr(f, font);
        GetStr(f, custom_chars);
        if (f.ok())
            return true;
