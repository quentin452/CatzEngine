...
void EditGuiSkin::Button::copyTo(GuiSkin::Button &skin, C Project &proj) C {
    ...
    skin.pushed_disabled = proj.gamePath(pushed_disabled_id.valid() ? pushed_disabled_id : pushed_id.valid() ? pushed_id
                                                                                       : disabled_id.valid() ? disabled_id
                                                                                                             : normal_id);
...
bool EditGuiSkin::Button::newer(C Button &src) C {
    return normal > src.normal || pushed > src.pushed || disabled > src.disabled || pushed_disabled > src.pushed_disabled || normal_color > src.normal_color || pushed_color > src.pushed_color || disabled_color > src.disabled_color || pushed_disabled_color > src.pushed_disabled_color || text_size > src.text_size || text_padd > src.text_padd || text_style > src.text_style;
}
...
bool EditGuiSkin::Button::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 0: {
        f >> normal_id >> pushed_id >> disabled_id >> pushed_disabled_id >> text_style_id >> normal >> pushed >> disabled >> pushed_disabled >> normal_color >> pushed_color >> disabled_color >> pushed_disabled_color >> text_size >> text_padd >> text_style;
        if (f.ok())
            return true;
    } break;
    }
...
bool EditGuiSkin::CheckBox::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 0: {
        f >> off_id >> on_id >> multi_id >> off >> on >> multi >> normal_color >> disabled_color;
        if (f.ok())
            return true;
    } break;
    }
...
bool EditGuiSkin::Desc::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 0: {
        f >> normal_id >> text_style_id >> normal >> normal_color >> padding >> text_style;
        if (f.ok())
            return true;
    } break;
    }
...
bool EditGuiSkin::List::newer(C List &src) C {
    return cursor > src.cursor || highlight > src.highlight || selection > src.selection || cursor_color > src.cursor_color || highlight_color > src.highlight_color || selection_color > src.selection_color || text_style > src.text_style || resize_column > src.resize_column || column.newer(src.column);
}
bool EditGuiSkin::List::equal(C List &src) C {
    return cursor == src.cursor && highlight == src.highlight && selection == src.selection && cursor_color == src.cursor_color && highlight_color == src.highlight_color && selection_color == src.selection_color && text_style == src.text_style && resize_column == src.resize_column && column.equal(src.column);
}
...
bool EditGuiSkin::List::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 1: {
        f >> cursor_id >> highlight_id >> selection_id >> text_style_id >> resize_column_id >> cursor >> highlight >> selection >> cursor_color >> highlight_color >> selection_color >> text_style >> resize_column;
        if (column.load(f))
            if (f.ok())
                return true;
    } break;
    ...
    case 0: {
        f >> cursor_id >> highlight_id >> selection_id >> text_style_id >> cursor >> highlight >> selection >> cursor_color >> highlight_color >> selection_color >> text_style;
        if (column.load(f))
            if (f.ok())
                return true;
    } break;
...
bool EditGuiSkin::Menu::newer(C Menu &src) C {
    return normal > src.normal || normal_color > src.normal_color || padding > src.padding || list_elm_height > src.list_elm_height || check > src.check || sub_menu > src.sub_menu || check_color > src.check_color || sub_menu_color > src.sub_menu_color;
}
...
bool EditGuiSkin::Menu::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 1: {
        f >> normal_id >> check_id >> sub_menu_id >> normal >> normal_color >> padding >> list_elm_height >> check >> sub_menu >> check_color >> sub_menu_color;
        if (f.ok())
            return true;
    } break;

    case 0: {
        f >> normal_id >> check_id >> sub_menu_id >> normal >> normal_color >> padding >> check >> sub_menu >> check_color >> sub_menu_color;
        if (f.ok())
            return true;
    } break;
    }
...
bool EditGuiSkin::MenuBar::newer(C MenuBar &src) C {
    return background > src.background || highlight > src.highlight || background_color > src.background_color || highlight_color > src.highlight_color || bar_height > src.bar_height || text_size > src.text_size || text_padd > src.text_padd || text_style > src.text_style;
}
...
bool EditGuiSkin::MenuBar::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 0: {
        f >> background_id >> highlight_id >> text_style_id >> background >> highlight >> background_color >> highlight_color >> bar_height >> text_size >> text_padd >> text_style;
        if (f.ok())
            return true;
    } break;
    }
...
bool EditGuiSkin::Progress::newer(C Progress &src) C {
    return background > src.background || progress > src.progress || background_color > src.background_color || progress_color > src.progress_color || text_style > src.text_style || text_size > src.text_size || draw_progress_partial > src.draw_progress_partial;
}
...
bool EditGuiSkin::Progress::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 0: {
        f >> background_id >> progress_id >> text_style_id >> background >> progress >> background_color >> progress_color >> text_style >> text_size >> draw_progress_partial;
        if (f.ok())
            return true;
    } break;
    }
...
bool EditGuiSkin::Slider::newer(C Slider &src) C {
    return background > src.background || progress > src.progress || slider > src.slider || background_color > src.background_color || progress_color > src.progress_color || slider_color > src.slider_color || background_shrink > src.background_shrink || draw_progress_partial > src.draw_progress_partial;
}
...
bool EditGuiSkin::Slider::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 0: {
        f >> background_id >> progress_id >> slider_id >> background >> progress >> slider >> background_color >> progress_color >> slider_color >> background_shrink >> draw_progress_partial;
        if (f.ok())
            return true;
    } break;
    }
...
bool EditGuiSkin::Tab::Kind::apply(Kind &kind) C {
    if (valid()) {
        kind = T;
        return true;
    }
...
bool EditGuiSkin::Tab::Kind::apply(Button &button) C {
    if (valid()) {
        copyTo(button);
        return true;
    }
...
void EditGuiSkin::Tab::Kind::newData() {
    normal++;
    pushed++;
}
...
void EditGuiSkin::Tab::includeIDs(MemPtr<UID> ids) C {
    super::includeIDs(ids);
    left.includeIDs(ids);
    horizontal.includeIDs(ids);
    right.includeIDs(ids);
    top.includeIDs(ids);
    vertical.includeIDs(ids);
    bottom.includeIDs(ids);
    top_left.includeIDs(ids);
    top_right.includeIDs(ids);
    bottom_left.includeIDs(ids);
    bottom_right.includeIDs(ids);
}
bool EditGuiSkin::Tab::newer(C Tab &src) C {
    return super::newer(src) || left.newer(src.left) || horizontal.newer(src.horizontal) || right.newer(src.right) || top.newer(src.top) || vertical.newer(src.vertical) || bottom.newer(src.bottom) || top_left.newer(src.top_left) || top_right.newer(src.top_right) || bottom_left.newer(src.bottom_left) || bottom_right.newer(src.bottom_right);
}
bool EditGuiSkin::Tab::equal(C Tab &src) C {
    return super::equal(src) && left.equal(src.left) && horizontal.equal(src.horizontal) && right.equal(src.right) && top.equal(src.top) && vertical.equal(src.vertical) && bottom.equal(src.bottom) && top_left.equal(src.top_left) && top_right.equal(src.top_right) && bottom_left.equal(src.bottom_left) && bottom_right.equal(src.bottom_right);
}
...
bool EditGuiSkin::TextLine::newer(C TextLine &src) C {
    return normal > src.normal || disabled > src.disabled || normal_panel_color > src.normal_panel_color || normal_text_color > src.normal_text_color || disabled_panel_color > src.disabled_panel_color || disabled_text_color > src.disabled_text_color || rect_color > src.rect_color || text_size > src.text_size || text_style > src.text_style || find_image > src.find_image || clear.newer(src.clear);
}
...
bool EditGuiSkin::TextLine::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 3: {
        f >> normal_id >> disabled_id >> text_style_id >> find_image_id >> normal >> disabled >> normal_panel_color >> normal_text_color >> disabled_panel_color >> disabled_text_color >> rect_color >> text_size >> text_style >> find_image;
        if (clear.load(f))
            if (f.ok())
                return true;
    } break;
    ...
    case 2: {
        f >> normal_id >> disabled_id >> text_style_id >> find_image_id >> normal >> disabled >> normal_panel_color >> normal_text_color >> disabled_panel_color >> disabled_text_color >> rect_color >> text_style >> find_image;
        if (clear.load(f))
            if (f.ok())
                return true;
    } break;
    ...
    case 0: {
        f >> normal_id >> disabled_id >> text_style_id >> find_image_id >> normal >> disabled >> normal_panel_color >> disabled_panel_color >> rect_color >> text_style >> find_image;
        normal_text_color = normal_panel_color;
        disabled_text_color = disabled_panel_color;
        if (f.ok())
            return true;
    } break;
    }
...
void EditGuiSkin::Window::copyTo(GuiSkin::Window &skin, C Project &proj) C {
    skin.normal = proj.gamePath(normal_id.valid() ? normal_id : active_id.valid()      ? active_id
                                                            : normal_no_bar_id.valid() ? normal_no_bar_id
                                                                                       : active_no_bar_id);
    skin.active = proj.gamePath(active_id.valid() ? active_id : normal_id.valid()      ? normal_id
                                                            : active_no_bar_id.valid() ? active_no_bar_id
                                                                                       : normal_no_bar_id);
    skin.normal_no_bar = proj.gamePath(normal_no_bar_id.valid() ? normal_no_bar_id : active_no_bar_id.valid() ? active_no_bar_id
                                                                                 : normal_id.valid()          ? normal_id
                                                                                                              : active_id);
    skin.active_no_bar = proj.gamePath(active_no_bar_id.valid() ? active_no_bar_id : normal_no_bar_id.valid() ? normal_no_bar_id
                                                                                 : active_id.valid()          ? active_id
                                                                                                              : normal_id);
...
bool EditGuiSkin::Window::newer(C Window &src) C {
    return normal > src.normal || active > src.active || normal_no_bar > src.normal_no_bar || active_no_bar > src.active_no_bar || normal_color > src.normal_color || active_color > src.active_color || normal_text_style > src.normal_text_style || active_text_style > src.active_text_style || text_size > src.text_size || text_padd > src.text_padd || button_offset > src.button_offset || minimize.newer(src.minimize) || maximize.newer(src.maximize) || close.newer(src.close);
}
bool EditGuiSkin::Window::equal(C Window &src) C {
    return normal == src.normal && active == src.active && normal_no_bar == src.normal_no_bar && active_no_bar == src.active_no_bar && normal_color == src.normal_color && active_color == src.active_color && normal_text_style == src.normal_text_style && active_text_style == src.active_text_style && text_size == src.text_size && text_padd == src.text_padd && button_offset == src.button_offset && minimize.equal(src.minimize) && maximize.equal(src.maximize) && close.equal(src.close);
}
...
bool EditGuiSkin::Window::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 1: {
        f >> normal_id >> active_id >> normal_no_bar_id >> active_no_bar_id >> normal_text_style_id >> active_text_style_id >> normal >> active >> normal_no_bar >> active_no_bar >> normal_color >> active_color >> normal_text_style >> active_text_style >> text_size >> text_padd >> button_offset;
        if (minimize.load(f))
            if (maximize.load(f))
                if (close.load(f))
                    if (f.ok())
    ...
    case 0: {
        f >> normal_id >> active_id >> normal_no_bar_id >> active_no_bar_id >> normal_text_style_id >> active_text_style_id >> normal >> active >> normal_no_bar >> active_no_bar >> normal_color >> active_color >> normal_text_style >> active_text_style >> text_size >> text_padd;
        if (minimize.load(f))
            if (maximize.load(f))
                if (close.load(f))
                    if (f.ok())
...
void EditGuiSkin::Base::copyTo(GuiSkin &skin, C Project &proj) C {
    ...
    if (!skin.button.text_style)
        skin.button.text_style = skin.text_style;
    if (!skin.combobox.text_style)
        skin.combobox.text_style = skin.button.text_style;
    if (!skin.list.column.text_style)
        skin.list.column.text_style = skin.button.text_style;
    if (!skin.property.value.text_style)
        skin.property.value.text_style = skin.button.text_style;
    if (!skin.textline.clear.text_style)
        skin.textline.clear.text_style = skin.button.text_style;

    if (!skin.desc.text_style)
        skin.desc.text_style = skin.text_style;
    if (!skin.imm.text_style)
        skin.imm.text_style = skin.desc.text_style;

    if (!skin.text.text_style)
        skin.text.text_style = skin.text_style;
    if (!skin.list.text_style)
        skin.list.text_style = skin.text_style;
    if (!skin.menubar.text_style)
        skin.menubar.text_style = skin.text_style;
    if (!skin.progress.text_style)
        skin.progress.text_style = skin.text_style;
    if (!skin.textline.text_style)
        skin.textline.text_style = skin.text_style;
    if (!skin.window.normal_text_style)
        skin.window.normal_text_style = skin.text_style;
    if (!skin.window.active_text_style)
        skin.window.active_text_style = skin.text_style;
    ...
    // tabs
    {
        ...
        if (!temp.text_style_id.valid())
            temp.text_style_id = skin.button.text_style.id(); // adjust text
        ...
        // copy ID based
        if (!tab.left.apply(temp) && !hor.apply(temp))
            any.copyTo(temp);
        temp.copyTo(skin.tab.left, proj);
        if (!tab.horizontal.apply(temp) && !hor.apply(temp))
            any.copyTo(temp);
        temp.copyTo(skin.tab.horizontal, proj);
        if (!tab.right.apply(temp) && !hor.apply(temp))
            any.copyTo(temp);
        temp.copyTo(skin.tab.right, proj);
        if (!tab.top.apply(temp) && !ver.apply(temp))
            any.copyTo(temp);
        temp.copyTo(skin.tab.top, proj);
        if (!tab.vertical.apply(temp) && !ver.apply(temp))
            any.copyTo(temp);
        temp.copyTo(skin.tab.vertical, proj);
        if (!tab.bottom.apply(temp) && !ver.apply(temp))
            any.copyTo(temp);
        temp.copyTo(skin.tab.bottom, proj);
        if (!tab.top_left.apply(temp) && !hor.apply(temp))
            any.copyTo(temp);
        temp.copyTo(skin.tab.top_left, proj);
        if (!tab.top_right.apply(temp) && !hor.apply(temp))
            any.copyTo(temp);
        temp.copyTo(skin.tab.top_right, proj);
        if (!tab.bottom_left.apply(temp) && !hor.apply(temp))
            any.copyTo(temp);
        temp.copyTo(skin.tab.bottom_left, proj);
        if (!tab.bottom_right.apply(temp) && !hor.apply(temp))
            any.copyTo(temp);
        temp.copyTo(skin.tab.bottom_right, proj);
    }
}
bool EditGuiSkin::Base::newer(C Base &src) C {
    return background_color > src.background_color || border_color > src.border_color || mouse_highlight_color > src.mouse_highlight_color || keyboard_highlight_color > src.keyboard_highlight_color || font > src.font || text_style > src.text_style || button.newer(src.button) || checkbox.newer(src.checkbox) || combobox.newer(src.combobox) || desc.newer(src.desc) || imm.newer(src.imm) || list.newer(src.list) || menu.newer(src.menu) || menubar.newer(src.menubar) || progress.newer(src.progress) || property.newer(src.property) || region.newer(src.region) || slidebar.newer(src.slidebar) || slider.newer(src.slider) || tab.newer(src.tab) || text.newer(src.text) || textline.newer(src.textline) || window.newer(src.window);
}
bool EditGuiSkin::Base::equal(C Base &src) C {
    return background_color == src.background_color && border_color == src.border_color && mouse_highlight_color == src.mouse_highlight_color && keyboard_highlight_color == src.keyboard_highlight_color && font == src.font && text_style == src.text_style && button.equal(src.button) && checkbox.equal(src.checkbox) && combobox.equal(src.combobox) && desc.equal(src.desc) && imm.equal(src.imm) && list.equal(src.list) && menu.equal(src.menu) && menubar.equal(src.menubar) && progress.equal(src.progress) && property.equal(src.property) && region.equal(src.region) && slidebar.equal(src.slidebar) && slider.equal(src.slider) && tab.equal(src.tab) && text.equal(src.text) && textline.equal(src.textline) && window.equal(src.window);
}
...
bool EditGuiSkin::Base::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 1: {
        f >> background_color >> border_color >> mouse_highlight_color >> keyboard_highlight_color >> font_id >> text_style_id >> font >> text_style;
        if (button.load(f))
            if (checkbox.load(f))
                if (combobox.load(f))
                    if (desc.load(f))
...
bool EditGuiSkin::sync(C EditGuiSkin &src) {
    bool changed = base.sync(T, src.base, src);
    return changed;
}
bool EditGuiSkin::undo(C EditGuiSkin &src) {
    bool changed = base.undo(T, src.base, src);
    return changed;
}
void EditGuiSkin::copyTo(GuiSkin &skin, C Project &proj) C {
    skin = T;
    base.copyTo(skin, proj);
}
