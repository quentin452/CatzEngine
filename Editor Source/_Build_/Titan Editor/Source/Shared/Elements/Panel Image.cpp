...
bool EditPanelImage::Light::newer(C Light &src) C {
    return enabled > src.enabled || intensity > src.intensity || back > src.back || highlight > src.highlight || highlight_cut > src.highlight_cut || specular > src.specular || specular_back > src.specular_back || specular_exp > src.specular_exp || specular_highlight > src.specular_highlight || specular_highlight_cut > src.specular_highlight_cut || angle > src.angle;
}
...
bool EditPanelImage::ImageParams::newer(C ImageParams &src) C {
    return blur_clamp > src.blur_clamp || blur > src.blur || uv_scale > src.uv_scale || uv_offset > src.uv_offset || uv_warp > src.uv_warp || intensity > src.intensity || mode > src.mode;
}
...
bool EditPanelImage::Section::newer(C Section &src) C {
    return size > src.size || top_offset > src.top_offset || round_depth > src.round_depth || outer_depth > src.outer_depth || inner_depth > src.inner_depth || inner_distance > src.inner_distance || specular > src.specular || reflection_intensity > src.reflection_intensity || smooth_depth > src.smooth_depth || color > src.color || outer_color > src.outer_color || inner_color > src.inner_color || color_top > src.color_top || color_bottom > src.color_bottom || color_left > src.color_left || color_right > src.color_right || outer_border_color > src.outer_border_color || inner_border_color > src.inner_border_color || prev_border_color > src.prev_border_color || depth_overlay > src.depth_overlay || color_overlay > src.color_overlay || reflection > src.reflection || depth_overlay_params.newer(src.depth_overlay_params) || color_overlay_params.newer(src.color_overlay_params) || depth_noise.newer(src.depth_noise) || color_noise.newer(src.color_noise);
}
bool EditPanelImage::Section::equal(C Section &src) C {
    return size == src.size && top_offset == src.top_offset && round_depth == src.round_depth && outer_depth == src.outer_depth && inner_depth == src.inner_depth && inner_distance == src.inner_distance && specular == src.specular && reflection_intensity == src.reflection_intensity && smooth_depth == src.smooth_depth && color == src.color && outer_color == src.outer_color && inner_color == src.inner_color && color_top == src.color_top && color_bottom == src.color_bottom && color_left == src.color_left && color_right == src.color_right && outer_border_color == src.outer_border_color && inner_border_color == src.inner_border_color && prev_border_color == src.prev_border_color && depth_overlay == src.depth_overlay && color_overlay == src.color_overlay && reflection == src.reflection && depth_overlay_params.equal(src.depth_overlay_params) && color_overlay_params.equal(src.color_overlay_params) && depth_noise.equal(src.depth_noise) && color_noise.equal(src.color_noise);
}
void EditPanelImage::Section::newData() {
    ...
    color_overlay++;
    reflection++;
    depth_overlay_params.newData();
    color_overlay_params.newData();
    depth_noise.newData();
    color_noise.newData();
}
...
bool EditPanelImage::Section::load(File &f) {
    switch (f.decUIntV()) {
    case 0: {
        f >> size >> top_offset >> round_depth >> outer_depth >> inner_depth >> inner_distance >> specular >> reflection_intensity >> smooth_depth >> color >> outer_color >> inner_color >> color_top >> color_bottom >> color_left >> color_right >> outer_border_color >> inner_border_color >> prev_border_color >> depth_overlay >> color_overlay >> reflection >> depth_overlay_id >> color_overlay_id >> reflection_id;
...
void EditPanelImage::Base::includeIDs(MemPtr<UID> ids) C {
    Include(ids, top_id);
    Include(ids, bottom_id);
    Include(ids, center_id);
    Include(ids, left_id);
    Include(ids, right_id);
    Include(ids, top_left_id);
    Include(ids, top_right_id);
    Include(ids, bottom_left_id);
    Include(ids, bottom_right_id);
    REPAO(sections).includeIDs(ids);
}
bool EditPanelImage::Base::newer(C Base &src) C {
    return cut_left > src.cut_left || cut_right > src.cut_right || cut_bottom > src.cut_bottom || cut_top > src.cut_top || cut_corners[0][0] > src.cut_corners[0][0] || cut_corners[0][1] > src.cut_corners[0][1] || cut_corners[1][0] > src.cut_corners[1][0] || cut_corners[1][1] > src.cut_corners[1][1] || cut_corner_slope > src.cut_corner_slope || cut_corner_amount > src.cut_corner_amount || force_uniform_stretch[0] > src.force_uniform_stretch[0] || force_uniform_stretch[1] > src.force_uniform_stretch[1] || resolution > src.resolution || width > src.width || height > src.height || round_corners > src.round_corners || left_slope > src.left_slope || right_slope > src.right_slope || max_side_stretch > src.max_side_stretch || min_size > src.min_size || extend > src.extend || extend_inner_padd > src.extend_inner_padd || border_size > src.border_size || outer_glow_spread > src.outer_glow_spread || outer_glow_radius > src.outer_glow_radius || inner_glow_radius > src.inner_glow_radius || outer_glow_color > src.outer_glow_color || inner_glow_color > src.inner_glow_color || light_ambient > src.light_ambient || depth > src.depth || round_depth > src.round_depth || inner_distance > src.inner_distance || smooth_depth > src.smooth_depth || shadow_radius > src.shadow_radius || shadow_opacity > src.shadow_opacity || shadow_spread > src.shadow_spread || color > src.color || color_top > src.color_top || color_bottom > src.color_bottom || color_left > src.color_left || color_right > src.color_right || images_size > src.images_size || top_height > src.top_height || bottom_height > src.bottom_height || left_right_width > src.left_right_width || top_corner_width > src.top_corner_width || bottom_corner_width > src.bottom_corner_width || top_image > src.top_image || bottom_image > src.bottom_image || center_image > src.center_image || left_image > src.left_image || right_image > src.right_image || top_left_image > src.top_left_image || top_right_image > src.top_right_image || bottom_left_image > src.bottom_left_image || bottom_right_image > src.bottom_right_image || compressed_time > src.compressed_time || mip_maps_time > src.mip_maps_time || lights[0].newer(src.lights[0]) || lights[1].newer(src.lights[1]) || sections[0].newer(src.sections[0]) || sections[1].newer(src.sections[1]);
}
bool EditPanelImage::Base::equal(C Base &src) C {
    return cut_left == src.cut_left && cut_right == src.cut_right && cut_bottom == src.cut_bottom && cut_top == src.cut_top && cut_corners[0][0] == src.cut_corners[0][0] && cut_corners[0][1] == src.cut_corners[0][1] && cut_corners[1][0] == src.cut_corners[1][0] && cut_corners[1][1] == src.cut_corners[1][1] && cut_corner_slope == src.cut_corner_slope && cut_corner_amount == src.cut_corner_amount && force_uniform_stretch[0] == src.force_uniform_stretch[0] && force_uniform_stretch[1] == src.force_uniform_stretch[1] && resolution == src.resolution && width == src.width && height == src.height && round_corners == src.round_corners && left_slope == src.left_slope && right_slope == src.right_slope && max_side_stretch == src.max_side_stretch && min_size == src.min_size && extend == src.extend && extend_inner_padd == src.extend_inner_padd && border_size == src.border_size && outer_glow_spread == src.outer_glow_spread && outer_glow_radius == src.outer_glow_radius && inner_glow_radius == src.inner_glow_radius && outer_glow_color == src.outer_glow_color && inner_glow_color == src.inner_glow_color && light_ambient == src.light_ambient && depth == src.depth && round_depth == src.round_depth && inner_distance == src.inner_distance && smooth_depth == src.smooth_depth && shadow_radius == src.shadow_radius && shadow_opacity == src.shadow_opacity && shadow_spread == src.shadow_spread && color == src.color && color_top == src.color_top && color_bottom == src.color_bottom && color_left == src.color_left && color_right == src.color_right && images_size == src.images_size && top_height == src.top_height && bottom_height == src.bottom_height && left_right_width == src.left_right_width && top_corner_width == src.top_corner_width && bottom_corner_width == src.bottom_corner_width && top_image == src.top_image && bottom_image == src.bottom_image && center_image == src.center_image && left_image == src.left_image && right_image == src.right_image && top_left_image == src.top_left_image && top_right_image == src.top_right_image && bottom_left_image == src.bottom_left_image && bottom_right_image == src.bottom_right_image && compressed_time == src.compressed_time && mip_maps_time == src.mip_maps_time && lights[0].equal(src.lights[0]) && lights[1].equal(src.lights[1]) && sections[0].equal(src.sections[0]) && sections[1].equal(src.sections[1]);
}
...
bool EditPanelImage::Base::sync(PanelImageParams &val, C Base &src, C PanelImageParams &src_val) {
    ...
    REPAD(y, cut_corners)
    REPAD(x, cut_corners[y])
    changed |= Sync(cut_corners[y][x], src.cut_corners[y][x], val.cut_corners[y][x], src_val.cut_corners[y][x]);
    ...
    REPA(force_uniform_stretch)
    changed |= Sync(force_uniform_stretch[i], src.force_uniform_stretch[i], val.force_uniform_stretch[i], src_val.force_uniform_stretch[i]);
    ...
    REPA(lights)
    changed |= lights[i].sync(val.lights[i], src.lights[i], src_val.lights[i]);
    REPA(sections)
    changed |= sections[i].sync(val.sections[i], src.sections[i], src_val.sections[i]);
    return changed;
}
bool EditPanelImage::Base::undo(PanelImageParams &val, C Base &src, C PanelImageParams &src_val) {
    ...
    REPAD(y, cut_corners)
    REPAD(x, cut_corners[y])
    changed |= Undo(cut_corners[y][x], src.cut_corners[y][x], val.cut_corners[y][x], src_val.cut_corners[y][x]);
    ...
    REPA(force_uniform_stretch)
    changed |= Undo(force_uniform_stretch[i], src.force_uniform_stretch[i], val.force_uniform_stretch[i], src_val.force_uniform_stretch[i]);
    ...
    REPA(lights)
    changed |= lights[i].undo(val.lights[i], src.lights[i], src_val.lights[i]);
    REPA(sections)
    changed |= sections[i].undo(val.sections[i], src.sections[i], src_val.sections[i]);
    return changed;
}
...
bool EditPanelImage::Base::load(File &f) {
    ...
    switch (f.decUIntV()) {
    case 2: {
        f >> cut_left >> cut_right >> cut_bottom >> cut_top >> cut_corners >> cut_corner_slope >> cut_corner_amount >> force_uniform_stretch >> resolution >> width >> height >> round_corners >> left_slope >> right_slope >> max_side_stretch >> min_size >> extend >> extend_inner_padd >> border_size >> outer_glow_spread >> outer_glow_radius >> inner_glow_radius >> outer_glow_color >> inner_glow_color >> light_ambient >> depth >> round_depth >> inner_distance >> smooth_depth >> shadow_radius >> shadow_opacity >> shadow_spread >> color >> color_top >> color_bottom >> color_left >> color_right >> images_size >> top_height >> bottom_height >> left_right_width >> top_corner_width >> bottom_corner_width >> top_image >> bottom_image >> center_image >> left_image >> right_image >> top_left_image >> top_right_image >> bottom_left_image >> bottom_right_image >> compressed_time >> mip_maps_time >> compressed >> mip_maps >> top_id >> bottom_id >> center_id >> left_id >> right_id >> top_left_id >> top_right_id >> bottom_left_id >> bottom_right_id;
        FREPA(lights)
        if (!lights[i].load(f))
            goto error;
        FREPA(sections)
    ...
    case 1: {
        f >> cut_left >> cut_right >> cut_bottom >> cut_top >> cut_corners >> cut_corner_slope >> cut_corner_amount >> force_uniform_stretch >> resolution >> width >> height >> round_corners >> left_slope >> right_slope >> max_side_stretch >> extend >> extend_inner_padd >> border_size >> outer_glow_spread >> outer_glow_radius >> inner_glow_radius >> outer_glow_color >> inner_glow_color >> light_ambient >> depth >> round_depth >> inner_distance >> smooth_depth >> shadow_radius >> shadow_opacity >> shadow_spread >> color >> color_top >> color_bottom >> color_left >> color_right >> images_size >> top_height >> bottom_height >> left_right_width >> top_corner_width >> bottom_corner_width >> top_image >> bottom_image >> center_image >> left_image >> right_image >> top_left_image >> top_right_image >> bottom_left_image >> bottom_right_image >> compressed_time >> mip_maps_time >> compressed >> mip_maps >> top_id >> bottom_id >> center_id >> left_id >> right_id >> top_left_id >> top_right_id >> bottom_left_id >> bottom_right_id;
        FREPA(lights)
        if (!lights[i].load(f))
            goto error;
        FREPA(sections)
    ...
    case 0: {
        f >> cut_left >> cut_right >> cut_bottom >> cut_top >> cut_corners >> cut_corner_slope >> cut_corner_amount >> force_uniform_stretch >> resolution >> width >> height >> round_corners >> left_slope >> right_slope >> max_side_stretch >> extend >> border_size >> outer_glow_spread >> outer_glow_radius >> inner_glow_radius >> outer_glow_color >> inner_glow_color >> light_ambient >> depth >> round_depth >> inner_distance >> smooth_depth >> shadow_radius >> shadow_opacity >> shadow_spread >> color >> color_top >> color_bottom >> color_left >> color_right >> images_size >> top_height >> bottom_height >> left_right_width >> top_corner_width >> bottom_corner_width >> top_image >> bottom_image >> center_image >> left_image >> right_image >> top_left_image >> top_right_image >> bottom_left_image >> bottom_right_image >> compressed_time >> mip_maps_time >> compressed >> mip_maps >> top_id >> bottom_id >> center_id >> left_id >> right_id >> top_left_id >> top_right_id >> bottom_left_id >> bottom_right_id;
        FREPA(lights)
        if (!lights[i].load(f))
            goto error;
        FREPA(sections)
...
Image *EditPanelImage::Images::get(C UID &image_id) {
    ...
    if (images[i].id == image_id)
        return images[i].image_ptr();
    Src &src = images.New();
    src.id = image_id;
    if (!fast && src.image.Import(proj.editPath(image_id)))
        src.image_ptr = &src.image;
    if (!src.image_ptr)
        src.image_ptr = proj.gamePath(image_id);
...
void EditPanelImage::make(PanelImage &panel_image, Project &proj, Image *soft, Image *depth_map, bool fast) {
    ...
    IMAGE_TYPE type = panel_image.image.type();
    if (base.compressed)
        ImageProps(panel_image.image, null, &type);
