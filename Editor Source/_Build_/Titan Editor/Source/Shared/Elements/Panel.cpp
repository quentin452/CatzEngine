...
bool EditPanel::newer(C EditPanel &src) C {
    return center_stretch_time > src.center_stretch_time || center_shadow_time > src.center_shadow_time || side_mode_time > src.side_mode_time || center_color_time > src.center_color_time || bar_color_time > src.bar_color_time || border_color_time > src.border_color_time || side_color_time > src.side_color_time || blur_color_time > src.blur_color_time || shadow_opacity_time > src.shadow_opacity_time || shadow_radius_time > src.shadow_radius_time || shadow_offset_time > src.shadow_offset_time || center_scale_time > src.center_scale_time || bar_size_time > src.bar_size_time || border_size_time > src.border_size_time || side_min_scale_time > src.side_min_scale_time || top_size_time > src.top_size_time || bottom_size_time > src.bottom_size_time || left_right_size_time > src.left_right_size_time || top_corner_size_time > src.top_corner_size_time || bottom_corner_size_time > src.bottom_corner_size_time || top_offset_time > src.top_offset_time || bottom_offset_time > src.bottom_offset_time || left_right_offset_time > src.left_right_offset_time || top_corner_offset_time > src.top_corner_offset_time || bottom_corner_offset_time > src.bottom_corner_offset_time || center_image_time > src.center_image_time || bar_image_time > src.bar_image_time || border_image_time > src.border_image_time || top_image_time > src.top_image_time || bottom_image_time > src.bottom_image_time || left_right_image_time > src.left_right_image_time || top_corner_image_time > src.top_corner_image_time || bottom_corner_image_time > src.bottom_corner_image_time || panel_image_time > src.panel_image_time;
}
...
void EditPanel::newData() {
    ...
    top_size_time++;
    bottom_size_time++;
    left_right_size_time++;
    top_corner_size_time++;
    bottom_corner_size_time++;
    top_offset_time++;
    bottom_offset_time++;
    left_right_offset_time++;
    top_corner_offset_time++;
    bottom_corner_offset_time++;
    center_image_time++;
    bar_image_time++;
    border_image_time++;
    top_image_time++;
    bottom_image_time++;
    left_right_image_time++;
    top_corner_image_time++;
    bottom_corner_image_time++;
    panel_image_time++;
}
void EditPanel::includeIDs(MemPtr<UID> ids) C {
    Include(ids, center_image);
    Include(ids, bar_image);
    Include(ids, border_image);
    Include(ids, top_image);
    Include(ids, bottom_image);
    Include(ids, left_right_image);
    Include(ids, top_corner_image);
    Include(ids, bottom_corner_image);
    Include(ids, panel_image);
}
...
void EditPanel::create(C Panel &src, C UID &center_image, C UID &bar_image, C UID &border_image, C UID &top_image, C UID &bottom_image, C UID &left_right_image, C UID &top_corner_image, C UID &bottom_corner_image, C UID &panel_image, C TimeStamp &time) {
    SCAST(Panel, T) = src;
    super::center_image = super::bar_image = super::border_image = super::top_image = super::bottom_image = super::left_right_image = super::top_corner_image = super::bottom_corner_image = null;
    super::panel_image = null;
    T.center_image = center_image;
    T.bar_image = bar_image;
    T.border_image = border_image;
    T.top_image = top_image;
    T.bottom_image = bottom_image;
    T.left_right_image = left_right_image;
    T.top_corner_image = top_corner_image;
    T.bottom_corner_image = bottom_corner_image;
    T.panel_image = panel_image;

    center_stretch_time = center_shadow_time = side_mode_time = center_color_time = bar_color_time = border_color_time = side_color_time = blur_color_time = shadow_opacity_time = shadow_radius_time = shadow_offset_time = center_scale_time = bar_size_time = border_size_time = side_min_scale_time = top_size_time = bottom_size_time = left_right_size_time = top_corner_size_time = bottom_corner_size_time = top_offset_time = bottom_offset_time = left_right_offset_time = top_corner_offset_time = bottom_corner_offset_time = center_image_time = bar_image_time = border_image_time = top_image_time = bottom_image_time = left_right_image_time = top_corner_image_time = bottom_corner_image_time = panel_image_time = time;
}
void EditPanel::copyTo(Panel &panel, C Project &proj) C {
    ...
    if (!Equal(panel.shadow_offset, 0))
        panel.center_shadow = true; // if the user requested shadow offset then 'center_shadow' must be enabled
}
...
bool EditPanel::load(File &f) {
    ...
    case 4: {
        if (!super::load(f))
            break;
        f >> center_image >> bar_image >> border_image >> top_image >> bottom_image >> left_right_image >> top_corner_image >> bottom_corner_image >> panel_image >> center_stretch_time >> center_shadow_time >> side_mode_time >> center_color_time >> bar_color_time >> border_color_time >> side_color_time >> blur_color_time >> shadow_opacity_time >> shadow_radius_time >> shadow_offset_time >> center_scale_time >> bar_size_time >> border_size_time >> side_min_scale_time >> top_size_time >> bottom_size_time >> left_right_size_time >> top_corner_size_time >> bottom_corner_size_time >> top_offset_time >> bottom_offset_time >> left_right_offset_time >> top_corner_offset_time >> bottom_corner_offset_time >> center_image_time >> bar_image_time >> border_image_time >> top_image_time >> bottom_image_time >> left_right_image_time >> top_corner_image_time >> bottom_corner_image_time >> panel_image_time;
        if (f.ok())
            return true;
    } break;
    ...
    case 3: {
        if (!super::load(f))
            break;
        f >> center_image >> bar_image >> border_image >> top_image >> bottom_image >> left_right_image >> top_corner_image >> bottom_corner_image >> panel_image >> center_stretch_time >> center_shadow_time >> center_color_time >> bar_color_time >> border_color_time >> side_color_time >> blur_color_time >> shadow_opacity_time >> shadow_radius_time >> shadow_offset_time >> center_scale_time >> bar_size_time >> border_size_time >> side_min_scale_time >> top_size_time >> bottom_size_time >> left_right_size_time >> top_corner_size_time >> bottom_corner_size_time >> top_offset_time >> bottom_offset_time >> left_right_offset_time >> top_corner_offset_time >> bottom_corner_offset_time >> center_image_time >> bar_image_time >> border_image_time >> top_image_time >> bottom_image_time >> left_right_image_time >> top_corner_image_time >> bottom_corner_image_time >> panel_image_time;
        if (f.ok())
            return true;
    } break;
    ...
    case 2: {
        long pos = f.pos();
        bool old_side_stretch = false;
        if (f.getUInt() == CC4('G', 'S', 'T', 'L')) {
            uint ver = f.decUIntV();
            if (ver >= 5 && ver <= 6 && f.skip(1))
                old_side_stretch = f.getBool();
        }
        f.pos(pos);
        if (!super::load(f))
            break;
        f >> center_image >> border_image >> top_image >> bottom_image >> left_right_image >> top_corner_image >> bottom_corner_image >> panel_image >> center_stretch_time >> old_side_stretch_time >> center_shadow_time >> center_color_time >> border_color_time >> side_color_time >> blur_color_time >> shadow_opacity_time >> shadow_radius_time >> shadow_offset_time >> border_size_time >> center_scale_time >> top_size_time >> bottom_size_time >> left_right_size_time >> top_corner_size_time >> bottom_corner_size_time >> top_offset_time >> bottom_offset_time >> left_right_offset_time >> top_corner_offset_time >> bottom_corner_offset_time >> center_image_time >> border_image_time >> top_image_time >> bottom_image_time >> left_right_image_time >> top_corner_image_time >> bottom_corner_image_time >> panel_image_time;
        if (old_side_stretch && top_image.valid()) {
            bar_color = side_color;
            bar_size = top_size;
            Swap(bar_image, top_image);
        }
        if (f.ok())
            return true;
    } break;
    ...
    case 1: {
        if (!super::load(f))
            break;
        f >> center_image >> border_image >> top_corner_image >> top_image >> bottom_image >> left_right_image >> center_stretch_time >> shadow_opacity_time >> center_color_time >> border_color_time >> blur_color_time >> shadow_offset_time >> shadow_radius_time >> border_size_time >> center_scale_time >> top_corner_size_time >> top_size_time >> bottom_size_time >> left_right_size_time >> top_corner_offset_time >> top_offset_time >> bottom_offset_time >> left_right_offset_time >> center_image_time >> border_image_time >> top_corner_image_time >> top_image_time >> bottom_image_time >> left_right_image_time;
        bottom_corner_image = top_corner_image;
        bottom_corner_image_time = top_corner_image_time;
        bottom_corner_size_time = top_corner_size_time;
        bottom_corner_offset_time = top_corner_offset_time;
        if (f.ok())
            return true;
    } break;
    ...
    case 0: {
        if (!super::load(f))
            break;
        f >> center_image >> border_image >> top_corner_image >> top_image >> center_stretch_time >> shadow_opacity_time >> center_color_time >> border_color_time >> blur_color_time >> shadow_offset_time >> shadow_radius_time >> border_size_time >> center_scale_time >> top_corner_size_time >> top_size_time >> top_corner_offset_time >> top_offset_time >> center_image_time >> border_image_time >> top_corner_image_time >> top_image_time;
        bottom_corner_image = top_corner_image;
        bottom_corner_image_time = top_corner_image_time;
        bottom_corner_size_time = top_corner_size_time;
        bottom_corner_offset_time = top_corner_offset_time;
