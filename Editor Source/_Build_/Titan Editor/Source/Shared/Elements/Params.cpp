...
bool EditParam::contains(C Str &text, Project &proj) C {
    if (Contains(name, text))
        return true;
    if (type == PARAM_STR && Contains(value.s, text))
        return true;
    if (ParamTypeID(type))
        REP(IDs())
    if (Contains(proj.elmFullName(asID(i)), text))
        return true;
    return false;
}
...
void EditParam::setRemoved(bool removed) {
    T.removed = removed;
    removed_time.getUTC();
}
EditParam &EditParam::setName(C Str &name) {
    T.name = name;
    name_time.getUTC();
    return T;
}
void EditParam::clearValue() {
    if (type == PARAM_ID)
        setAsIDArray(null);
    else {
        super::clearValue();
...
void EditParam::setBool(bool v) {
    value.b = v;
    value_time.getUTC();
}
void EditParam::setColor(C Color &v) {
    value.c = v;
    value_time.getUTC();
}
void EditParam::setValue(int v) {
    super::setValue(v);
    value_time.getUTC();
}
void EditParam::setValue(C Str &v) {
    super::setValue(v);
    value_time.getUTC();
}
EditParam &EditParam::forceBool(bool v) {
    setType(PARAM_BOOL);
    value.b = v;
    value_time.getUTC();
    return T;
}
EditParam &EditParam::forceInt(int v) {
    setType(PARAM_INT);
    value.i = v;
    value_time.getUTC();
    return T;
}
EditParam &EditParam::forceFlt(flt v) {
    setType(PARAM_FLT);
    value.f = v;
    value_time.getUTC();
    return T;
}
EditParam &EditParam::forceColor(C Color &v) {
    setType(PARAM_COLOR);
    value.c = v;
    value_time.getUTC();
    return T;
}
EditParam &EditParam::setAsIDArray(C MemPtr<UID> &ids) {
    super::setAsIDArray(ids);
    type_time.getUTC();
    value_time.getUTC();
    return T;
}
EditParam &EditParam::includeAsIDArray(C MemPtr<UID> &ids) {
    super::includeAsIDArray(ids);
    type_time.getUTC();
    value_time.getUTC();
    return T;
}
EditParam &EditParam::excludeAsIDArray(C MemPtr<UID> &ids) {
    super::excludeAsIDArray(ids);
    type_time.getUTC();
    value_time.getUTC();
    return T;
}
EditParam &EditParam::setType(PARAM_TYPE type, Enum *enum_type) {
    super::setType((type == PARAM_ID) ? PARAM_ID_ARRAY : type, enum_type);
    type_time.getUTC();
    return T;
}
EditParam &EditParam::setTypeValue(C Param &src) {
    super::setTypeValue(src);
    type_time.getUTC();
    value_time.getUTC();
    return T;
}
void EditParam::nameTypeValueUTC() {
    name_time.getUTC();
    type_time.getUTC();
    value_time.getUTC();
}
...
EditParam *EditParams::findParam(C UID &id) {
    REPA(T) {
        EditParam &p = T[i];
        if (p.id == id)
            return &p;
...
EditParam *EditParams::findParam(C Str &name) {
    REPA(T) {
        EditParam &p = T[i];
        if (!p.removed && p.name == name)
            return &p;
    }
    return null;
}
EditParam *EditParams::findParam(C Str &name, PARAM_TYPE type) {
    REPA(T) {
        EditParam &p = T[i];
        if (!p.removed && p.name == name && ParamTypeCompatible(p.type, type))
            return &p;
    }
    return null;
}
...
EditParam *EditParams::findParamInclRemoved(C Str &name) {
    EditParam *ret = null;
    REPA(T) {
        EditParam &p = T[i];
        if (p.name == name) {
...
EditParam *EditParams::findParamInclRemoved(C Str &name, PARAM_TYPE type) {
    EditParam *ret = null;
    REPA(T) {
        EditParam &p = T[i];
        if (p.name == name && ParamTypeCompatible(p.type, type)) {
            ret = &p;
            if (!p.removed)
                break;
        }
...
bool EditParams::equal(C EditParams &src) C {
    if (elms() == src.elms()) {
        REPA(T) {
            ...
            if (!src_param || !param.equal(*src_param))
                return false;
        }
        return true;
    }
...
bool EditParams::newer(C EditParams &src) C {
    REPA(T) {
        C EditParam &param = T[i], *src_param = src.findParam(param.id);
        if (!src_param || param.newer(*src_param))
            return true;
    }
    return false;
}
...
bool EditParams::sync(C EditParams &src) {
    ...
    {
        EditParam &param = T[i];
        if (C EditParam *src_param = src.findParam(param.id))
            changed |= param.sync(*src_param);
    }
    FREPA(src) // add new params from src
    {
        C EditParam &src_param = src[i];
        if (!findParam(src_param.id)) {
            changed = true;
            add(src_param);
        }
    }
...
bool EditParams::undo(C EditParams &src) {
    ...
    REPA(T) {
        EditParam &param = T[i];
        if (!param.removed && !src.findParam(param.id)) {
            param.removed = true;
            param.removed_time++;
            changed = true;
        }
...
void EditObject::SubObj::setRemoved(bool removed) {
    T.removed = removed;
    removed_time.getUTC();
}
...
::EditObject::SubObj *EditObject::findSubObj(C UID &id) {
    REPA(sub_objs)
    if (sub_objs[i].id == id)
        return &sub_objs[i];
    return null;
...
C EditParam *EditObject::findParam(C Str &name) C {
    ...
    for (C EditObject *cur = this; cur && bases.include(cur); cur = cur->base())
        if (C EditParam *param = cur->EditParams::findParam(name))
            return param; // check self and bases
    if (C Memc<GuiEditParam> *params = EditObjType.findParams(editor_type))
        REPA(*params)
    if ((*params)[i].name == name)
        return &(*params)[i]; // check editor type
    return null;
}
C EditParam *EditObject::baseFindParam(C Str &name, PARAM_TYPE type) C {
    Memt<C EditObject *> bases;
    for (C EditObject *cur = base(); cur && bases.include(cur); cur = cur->base())
        if (C EditParam *param = cur->EditParams::findParam(name, type))
            return param; // check bases (skip self)
    if (C Memc<GuiEditParam> *params = EditObjType.findParams(editor_type))
        REPA(*params)
    if ((*params)[i].name == name && ParamTypeCompatible((*params)[i].type, type))
        return &(*params)[i]; // check editor type
    return null;
}
...
EditObject &EditObject::setAccess(bool custom, OBJ_ACCESS access) {
    ...
    T.access = (custom ? access : base ? base->access
                                       : OBJ_ACCESS_TERRAIN);
    return T;
}
...
EditObject &EditObject::setConst(bool custom, bool on) {
    ...
    FlagSet(flag, CONSTANT, custom ? on : base ? base->constant()
                                               : false);
    return T;
}
EditObject &EditObject::setPath(bool custom, OBJ_PATH path) {
    ...
    T.path = (custom ? path : base ? base->path
                                   : OBJ_PATH_CREATE);
    return T;
}
EditObject &EditObject::setMeshVariation(bool custom, uint mesh_variation_id) {
    ...
    T.mesh_variation_id = (custom ? mesh_variation_id : base ? base->mesh_variation_id
                                                             : 0);
    return T;
}
bool EditObject::subObjsOld(C TimeStamp &now) C {
    REPA(sub_objs)
    if (!sub_objs[i].old(now))
        return false;
    return true;
}
...
void EditObject::create(C EditObject &src) {
    super::create(src);
    flag = src.flag;
    const_time = src.const_time;
    access = src.access;
    access_time = src.access_time;
    path = src.path;
    path_time = src.path_time;
    type = src.type;
    type_time = src.type_time;
    base = src.base;
    base_time = src.base_time;
    mesh_variation_id = src.mesh_variation_id;
    mesh_variation_time = src.mesh_variation_time;
...
void EditObject::copyTo(Object &obj, C Project &proj, bool zero_align, C UID *mesh_id, C UID *phys_id) C // 'zero_align' should be set only to ELM_OBJ
{
    ...
    UID base_id = base.id();
    obj.base(proj.gamePath(base_id));
    if (!mesh_id)
        obj.mesh(false);
    else
        obj.mesh(true, proj.gamePath(*mesh_id));
    if (!phys_id)
        obj.phys(false);
    else
        obj.phys(true, proj.gamePath(*phys_id));
    FREPA(sub_objs) {
        C SubObj &src = sub_objs[i];
        if (!src.removed)
            obj.sub_objs.New().base(proj.gamePath(src.elm_obj_id));
    }
}
...
void EditObject::updateEditorType(cchar *edit_path) {
    if (!editor_type_time.is()) // if this is not a manually set value (this can occur only for OBJ_CLASS), this check is important because we can't adjust the 'editor_type' of OBJ_CLASS in this method
    {
        if (base) // if we have a base
        {
            ...
            {
                EditObjectPtr type_base;
                if (type.valid())
                    type_base = Str(edit_path).tailSlash(true) + EncodeFileName(type);
                if (type_base)
                    editor_type = type_base->editor_type;
                else
                    editor_type.zero();
            } else
...
void EditObject::updateBase(cchar *edit_path) {
    if (FlagOff(flag, OVR_ACCESS))
        access = (base ? base->access : OBJ_ACCESS_TERRAIN);
    if (FlagOff(flag, OVR_TYPE))
        if (base)
            type = base->type;
        else
            type.zero();
    if (FlagOff(flag, OVR_CONST))
        FlagSet(flag, CONSTANT, base ? base->constant() : false);
    if (FlagOff(flag, OVR_PATH))
        path = (base ? base->path : OBJ_PATH_CREATE);
    if (FlagOff(flag, OVR_MESH_VARIATION))
        mesh_variation_id = (base ? base->mesh_variation_id : 0);
    updateEditorType(edit_path);
}
bool EditObject::newParam(C Str &name, PARAM_TYPE type, C Project &proj) {
    ...
    if (base_param) {
        ...
        if (!dest) {
            ...
            return false; // if doesn't exist then create new and set id, if removed then un-remove, if exists then continue and don't override its value (this can happen if we're overriding value for multiple objects and some of which have this value already and some don't)
...
bool EditObject::sync(C EditObject &src, cchar *edit_path) {
    ...
    REPA(src.sub_objs) {
        ...
        if (SubObj *dest = findSubObj(s.id))
            changed |= dest->sync(s);
        else {
            changed = true;
            sub_objs.add(s);
...
bool EditObject::undo(C EditObject &src, cchar *edit_path) {
    ...
    REPA(sub_objs) {
        SubObj &s = sub_objs[i];
        if (!s.removed && !src.findSubObj(s.id)) {
            s.removed = true;
            s.removed_time++;
            changed = true;
        }
...
bool EditObject::modify(C MemPtr<Edit::ObjChange> &changes, Project &proj) {
    ...
    FREPA(changes) // process in order
    {
        ...
        case Edit::EI_OBJ_PARAM_RENAME_ID: {
            if (EditParam *param = EditParams::findParam(change.id))
                if (!Equal(param->name, change.param.name, true)) {
                    changed = true;
                    param->setName(change.param.name);
                }
        } break;
        ...
        case Edit::EI_OBJ_PARAM_SET_TYPE_VALUE_ID: {
            if (EditParam *param = EditParams::findParam(change.id)) {
                if (CompareValue(*param, change.param)) {
                    changed = true;
                    param->setTypeValue(change.param);
                } // set type value
                if (param->removed) {
                    changed = true;
                    param->setRemoved(false);
        ...
        case Edit::EI_OBJ_PARAM_SET_TYPE_VALUE_NAME: {
            ...
            } else // create new one
            {
                ...
                if (EditParam *param = (InRange(change.type, PARAM_NUM) ? findParamInclRemoved(change.name, change.type) : findParamInclRemoved(change.name))) {
                    ...
                    if (param->removed)
                        param->setRemoved(false); // restore
                } else                            // create new one
                {
...
bool EditObject::load(File &f, cchar *edit_path) {
    switch (f.decUIntV()) {
    case 2: {
        if (!super::load(f, EditToGamePath(edit_path)))
            goto error; // 'EditParams' requires game path
        UID base_id;
        f >> flag >> base_id >> editor_type;
        if (base_id.valid())
            base = Str(edit_path).tailSlash(true) + EncodeFileName(base_id);
        else
            base = null;
        if (flag & OVR_ACCESS)
            f >> access;
...
GuiEditParam &GuiEditParam::desc(C Str &desc) {
    _desc = desc;
    return T;
}
GuiEditParam &GuiEditParam::min(dbl min) {
    min_use = true;
    min_value = min;
    return T;
}
GuiEditParam &GuiEditParam::max(dbl max) {
    max_use = true;
    max_value = max;
    return T;
}
GuiEditParam &GuiEditParam::range(dbl min, dbl max) { return T.min(min).max(max); }
GuiEditParam &GuiEditParam::mouseEditSpeed(flt speed) {
    mouse_edit_speed = speed;
    return T;
}
GuiEditParam &GuiEditParam::mouseEditMode(PROP_MOUSE_EDIT_MODE mode) {
    mouse_edit_mode = mode;
    return T;
}
