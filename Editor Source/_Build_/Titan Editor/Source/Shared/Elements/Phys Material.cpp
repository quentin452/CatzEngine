...
bool EditPhysMtrl::newer(C EditPhysMtrl &src) C {
    return friction_static_time > src.friction_static_time || friction_dynamic_time > src.friction_dynamic_time || bounciness_time > src.bounciness_time || density_time > src.density_time || damping_time > src.density_time || adamping_time > src.adamping_time || friction_mode_time > src.friction_mode_time || bounciness_mode_time > src.bounciness_mode_time;
}
...
void EditPhysMtrl::newData() {
    friction_static_time++;
    friction_dynamic_time++;
    bounciness_time++;
    density_time++;
    damping_time++;
    adamping_time++;
    friction_mode_time++;
    bounciness_mode_time++;
}
...
void EditPhysMtrl::copyTo(PhysMtrl &dest) C {
    dest.create()
        ...
        .frictionDynamic(friction_dynamic)
        .bounciness(bounciness)
        .density(density)
        .damping(damping)
        .adamping(adamping)
        .frictionMode(friction_mode)
        .bouncinessMode(bounciness_mode);
}
...
bool EditPhysMtrl::load(File &f) {
    switch (f.decUIntV()) {
    case 0: {
        f >> friction_static >> friction_dynamic >> bounciness >> density >> damping >> adamping >> friction_mode >> bounciness_mode >> friction_static_time >> friction_dynamic_time >> bounciness_time >> density_time >> damping_time >> adamping_time >> friction_mode_time >> bounciness_mode_time;
        if (f.ok())
            return true;
    } break;
    }
