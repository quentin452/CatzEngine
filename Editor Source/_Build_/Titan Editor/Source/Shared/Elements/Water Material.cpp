...
/******************************************************************************/
EditWaterMtrl::EditWaterMtrl() : density(0.3f), density_add(0.45f), scale_color(1.0f / 200), scale_normal(1.0f / 10), scale_bump(1.0f / 100), refract(0.10f), refract_reflection(0.06f), wave_scale(0.25f), color_underwater0(0.26f, 0.35f, 0.42f), color_underwater1(0.10f, 0.20f, 0.30f) {
    smooth = 1;
    reflect_min = 0.02f;
}
bool EditWaterMtrl::usesTexBump() C { return wave_scale > EPSL && hasBumpMap(); }
bool EditWaterMtrl::equal(C EditWaterMtrl &src) C {
    return super::equal(src) && density_time == src.density_time && scale_color_time == src.scale_color_time && scale_normal_time == src.scale_normal_time && scale_bump_time == src.scale_bump_time && refract_time == src.refract_time && refract_reflection_time == src.refract_reflection_time && wave_scale_time == src.wave_scale_time && color_underwater_time == src.color_underwater_time;
}
bool EditWaterMtrl::newer(C EditWaterMtrl &src) C {
    return super::newer(src) || density_time > src.density_time || scale_color_time > src.scale_color_time || scale_normal_time > src.scale_normal_time || scale_bump_time > src.scale_bump_time || refract_time > src.refract_time || refract_reflection_time > src.refract_reflection_time || wave_scale_time > src.wave_scale_time || color_underwater_time > src.color_underwater_time;
}
...
void EditWaterMtrl::adjustParams(uint changed, TEX_FLAG old_textures, TEX_FLAG has_textures, TEX_FLAG known_textures) // 'old_textures'=textures() before making any change, 'has_textures'=used textures based on per-pixel data (if known), 'known_textures'=what textures in 'has_textures' are known
{
    ...
    if (changed_presence & TEXF_BUMP)
        if (!(new_textures & TEXF_BUMP)) {
            wave_scale = 0;
            wave_scale_time = time;
        } else if (wave_scale <= EPS_MATERIAL_BUMP) {
            wave_scale = 0.1f;
            wave_scale_time = time;
        }

    if (changed_presence & (TEXF_BUMP | TEXF_NORMAL))
        if (!(new_textures & TEXF_BUMP) && !(new_textures & TEXF_NORMAL)) {
            normal = 0;
            normal_time = time;
        } else if (normal <= EPS_COL8) {
            normal = 1;
            normal_time = time;
        }
}
bool EditWaterMtrl::save(File &f) C {
    ...
    TimeStamp refract_underwater_time;
...
bool EditWaterMtrl::load(File &f) {
    ...
    case 2:
        if (super::load(f)) {
            f >> density >> density_add >> scale_color >> scale_normal >> scale_bump >> refract >> refract_reflection >> refract_underwater >> wave_scale >> color_underwater0 >> color_underwater1 >> density_time >> scale_color_time >> scale_normal_time >> scale_bump_time >> refract_time >> refract_reflection_time >> refract_underwater_time >> wave_scale_time >> color_underwater_time;
            if (f.ok())
                return true;
        }
        break;
    ...
    case 1:
        if (super::load(f)) {
            ...
            Vec temp_vec;
            TimeStamp temp_time;
            f >> density >> density_add >> temp_flt >> temp_flt >> scale_color >> scale_normal >> scale_bump >> temp_flt >> refract >> refract_reflection >> refract_underwater >> wave_scale >> temp_flt >> temp_flt >> temp_vec >> color_underwater0 >> color_underwater1 >> density_time >> temp_time >> scale_color_time >> scale_normal_time >> scale_bump_time >> temp_time >> refract_time >> refract_reflection_time >> refract_underwater_time >> wave_scale_time >> temp_time >> temp_time >> temp_time >> color_underwater_time;
