...
uint LakeBase::memUsage() C {
    uint size = 0;
    REPA(polys)
    size += polys[i].memUsage();
    return size;
}
void LakeBase::setDepth(flt depth) {
    T.depth = depth;
    T.depth_time.getUTC();
}
...
void RiverBase::Vtx::set(flt radius, C Vec &pos) {
    T.radius = radius;
    T.pos = pos;
}
...
bool RiverBase::getRect(Rect &rect) C {
    ...
    REPA(vtxs) {
        C Vtx &v = vtxs[i];
        Include(rect, found, Rect_C(v.pos.xz(), v.radius));
    }
    return found;
}
...
void RiverBase::setSmooth(byte smooth) {
    T.smooth = smooth;
    T.smooth_time.getUTC();
}
void RiverBase::setDepth(flt depth) {
    T.depth = depth;
    T.depth_time.getUTC();
}
void RiverBase::setUVScale(C Vec2 &uv_scale) {
    T.uv_scale = uv_scale;
    T.uv_scale_time.getUTC();
}
...
void RiverBase::draw(C Color &color, C Color &depth_color) C {
    ...
    if (color.a) {
        VI.color(color);
        REPA(edges)
        VI.line(vtxs[edges[i].x].pos, vtxs[edges[i].y].pos);
        VI.end();
...
void Lake::setMesh(C Project &proj) {
    ...
    if (polys[i].elms() >= 3)
        vtxs += polys[i].elms();
    MeshBase mesh;
    mesh.create(vtxs, vtxs, 0, 0, EDGE_FLAG);
    vtxs = 0;
    FREPA(polys) {
        Memc<Vec> &poly = polys[i];
        if (poly.elms() >= 3) {
    ...
    REPA(mesh.vtx)
    Swap(mesh.vtx.pos(i).y, mesh.vtx.pos(i).z);
...
void River::setMesh(C Project &proj) {
    // set mesh 2D
    MeshBase mesh;
    mesh.create(vtxs.elms(), edges.elms(), 0, 0, VTX_SIZE);
    REPA(vtxs) {
        mesh.vtx.pos(i) = vtxs[i].pos.xzy();
        mesh.vtx.size(i) = vtxs[i].radius;
    }
    REPA(edges)
    mesh.edge.ind(i) = edges[i];
