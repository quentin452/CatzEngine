...
void ElmData::newData() {
    newVer();
    src_file_time++;
}
void ElmData::setSrcFile(C Str &src_file, C TimeStamp &time) {
    T.src_file = src_file;
    T.src_file_time = time;
}
...
void ElmData::save(MemPtr<TextNode> nodes) C {
    ...
    if (src_file.is())
        nodes.New().set("SrcFile", src_file);
    if (src_file_time.is())
        nodes.New().set("SrcFileTime", src_file_time.text());
}
void ElmData::load(C MemPtr<TextNode> &nodes) {
    REPA(nodes) {
        ...
        if (n.name == "Version")
            n.getValue(ver.ver);
        else if (n.name == "SrcFile")
            n.getValue(src_file);
        else if (n.name == "SrcFileTime")
            src_file_time.text(n);
    }
}
ElmData::~ElmData() {}
...
void ElmObjClass::pathSelf(OBJ_PATH path) {
    FlagDisable(flag, PATH_MASK << PATH_SHIFT);
    flag |= ((path & PATH_MASK) << PATH_SHIFT);
}
void ElmObjClass::from(C EditObject &params) {
    flag = 0;
    ovrAccess(FlagOn(params.flag, EditObject::OVR_ACCESS));
    terrain(params.access == OBJ_ACCESS_TERRAIN);
    ovrPath(FlagOn(params.flag, EditObject::OVR_PATH));
    pathSelf(params.path);
}
...
void ElmObjClass::save(MemPtr<TextNode> nodes) C {
    super::save(nodes);
    if (ovrAccess())
        nodes.New().set("OverrideAccess");
    if (ovrPath())
        nodes.New().set("OverridePath");
...
void ElmObjClass::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        C TextNode &n = nodes[i];
        if (n.name == "OverrideAccess")
            ovrAccess(n.asBool1());
        else if (n.name == "OverridePath")
            ovrPath(n.asBool1());
        else if (n.name == "Terrain")
            terrain(n.asBool1());
        else if (n.name == "Path")
            pathSelf((OBJ_PATH)n.asUInt());
    }
}
...
void ElmObj::save(MemPtr<TextNode> nodes) C {
    super::save(nodes);
    if (mesh_id.valid())
        nodes.New().setFN("Mesh", mesh_id);
    if (base_id.valid())
        nodes.New().setFN("Base", base_id);
}
...
void ElmMesh::fromMtrl(C Mesh &mesh) {
    ...
    REPD(l, mesh.lods()) {
        ...
        if (!NegativeSB(lod.dist2))
            REPA(lod) // LOD not disabled
            {
                C MeshPart &part = lod.parts[i];
                if (!(part.part_flag & MSHP_HIDDEN)) // skip hidden parts
                {
                    REP(4)
                    if (C MaterialPtr &mtrl = part.multiMaterial(i)) {
                        UID mtrl_id = mtrl.id();
                        if (mtrl_id.valid())
                            mtrl_ids.binaryInclude(mtrl_id);
                    }
                    REP(part.variations())
                    if (i)
                        if (C MaterialPtr &mtrl = part.variation(i)) {
                            UID mtrl_id = mtrl.id();
                            if (mtrl_id.valid())
                                mtrl_ids.binaryInclude(mtrl_id);
                        }
...
bool ElmMesh::syncFile(C ElmMesh &src) {
    ...
    if (Sync(file_time, src.file_time)) {
        changed |= true;
        mtrl_ids = src.mtrl_ids;
    }
...
bool ElmMesh::save(File &f) C {
    super::save(f);
    f.cmpUIntV(2);
    f << transform << obj_id << skel_id << body_id << phys_id << draw_group_id;
    f.cmpUIntV(mtrl_ids.elms());
    FREPA(mtrl_ids)
    f << mtrl_ids[i];
    f << box;
...
bool ElmMesh::load(File &f) {
    if (super::load(f))
        ...
        case 1: {
            obj_id.zero();
            f >> transform >> skel_id >> body_id >> phys_id >> draw_group_id;
            mtrl_ids.setNum(f.decUIntV());
            FREPA(mtrl_ids)
            f >> mtrl_ids[i];
            f >> box;
        ...
        case 0: {
            obj_id.zero();
            f >> transform >> skel_id >> body_id >> phys_id;
            draw_group_id.zero();
            mtrl_ids.setNum(f.decUIntV());
            FREPA(mtrl_ids)
            f >> mtrl_ids[i];
            f >> box;
            f >> file_time >> body_time >> transform_time;
            draw_group_time.zero();
            if (f.ok())
                return true;
        } break;
        }
...
void ElmMesh::save(MemPtr<TextNode> nodes) C {
    ...
    if (obj_id.valid())
        nodes.New().setFN("Object", obj_id);
    if (skel_id.valid())
        nodes.New().setFN("Skeleton", skel_id);
    if (phys_id.valid())
        nodes.New().setFN("PhysBody", phys_id);
    if (body_id.valid())
        nodes.New().setFN("Body", body_id);
    if (draw_group_id.valid())
        nodes.New().setFN("DrawGroup", draw_group_id);
    ...
    if (mtrl_ids.elms()) {
        ...
        FREPA(mtrl_ids)
        mtrls.nodes.New().setValueFN(mtrl_ids[i]); // list in order
    }
}
void ElmMesh::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        C TextNode &n = nodes[i];
        if (n.name == "Pose")
            n.getValueRaw(transform);
        else if (n.name == "Box")
            n.getValueRaw(box);
        else if (n.name == "Object")
            n.getValue(obj_id);
        else if (n.name == "Skeleton")
            n.getValue(skel_id);
        else if (n.name == "PhysBody")
            n.getValue(phys_id);
        else if (n.name == "Body")
            n.getValue(body_id);
        else if (n.name == "DrawGroup")
            n.getValue(draw_group_id);
        else if (n.name == "FileTime")
            file_time.text(n);
        else if (n.name == "BodyTime")
            body_time.text(n);
        else if (n.name == "DrawGroupTime")
            draw_group_time.text(n);
        else if (n.name == "PoseTime")
            transform_time.text(n);
        else if (n.name == "Materials") {
            FREPA(n.nodes) // get in order
            {
                UID id;
                if (n.nodes[i].getValue(id) && id.valid())
                    mtrl_ids.binaryInclude(id);
            }
        }
    }
}
...
bool ElmMaterial::containsTex(C UID &id, bool test_merged) C {
    if (id.valid()) {
        if (base_0_tex == id || base_1_tex == id || base_2_tex == id || detail_tex == id || macro_tex == id || emissive_tex == id)
            return true;
        if (test_merged && MergedBaseTexturesID(base_0_tex, base_1_tex, base_2_tex) == id)
            return true;
    }
    return false;
}
void ElmMaterial::listTexs(MemPtr<UID> texs) C {
    if (base_0_tex.valid())
        texs.binaryInclude(base_0_tex);
    if (base_1_tex.valid())
        texs.binaryInclude(base_1_tex);
    if (base_2_tex.valid())
        texs.binaryInclude(base_2_tex);
    if (detail_tex.valid())
        texs.binaryInclude(detail_tex);
    if (macro_tex.valid())
        texs.binaryInclude(macro_tex);
    if (emissive_tex.valid())
        texs.binaryInclude(emissive_tex);
}
...
bool ElmMaterial::load(File &f) {
    ...
    byte tex_downsize_mobile;
    if (super::load(f))
        switch (f.decUIntV()) {
        case 7: {
            f >> base_0_tex >> base_1_tex >> base_2_tex >> detail_tex >> macro_tex >> emissive_tex >> tex_downsize >> tex_quality >> flag;
            ASSERT(ELMS(tex_downsize) == 2);
            if (f.ok())
                return true;
        } break;

        case 6: {
            f >> base_0_tex >> base_1_tex >> base_2_tex >> detail_tex >> macro_tex >> emissive_tex >> tex_downsize_mobile >> tex_quality >> flag;
            REPAO(tex_downsize) = tex_downsize_mobile;
            if (f.ok())
                return true;
        } break;

        case 5: {
            f >> base_0_tex >> base_1_tex >> base_2_tex >> detail_tex >> macro_tex >> emissive_tex >> tex_downsize_mobile >> flag;
            if (flag & (1 << 3)) {
                tex_quality = Edit::Material::HIGH;
                FlagDisable(flag, 1 << 3);
            }
            REPAO(tex_downsize) = tex_downsize_mobile;
            if (f.ok())
                return true;
        ...
        case 4: {
            f >> base_0_tex >> base_1_tex >> detail_tex >> macro_tex >> old_reflection_tex >> emissive_tex >> tex_downsize_mobile >> flag;
            base_2_tex.zero();
            if (flag & (1 << 3)) {
                tex_quality = Edit::Material::HIGH;
                FlagDisable(flag, 1 << 3);
        ...
        case 3: {
            byte max_tex_size;
            f >> max_tex_size >> base_0_tex >> base_1_tex >> detail_tex >> macro_tex >> old_reflection_tex >> emissive_tex;
            base_2_tex.zero();
            tex_downsize_mobile = (max_tex_size >= 1 && max_tex_size <= 10);
            flag = 0;
            REPAO(tex_downsize) = tex_downsize_mobile;
            if (f.ok())
                return true;
        ...
        case 2: {
            byte max_tex_size;
            UID mesh_id;
            f >> max_tex_size >> base_0_tex >> base_1_tex >> detail_tex >> macro_tex >> old_reflection_tex >> emissive_tex >> mesh_id;
            base_2_tex.zero();
            tex_downsize_mobile = (max_tex_size >= 1 && max_tex_size <= 10);
            flag = 0;
            REPAO(tex_downsize) = tex_downsize_mobile;
        ...
        case 1: {
            UID mesh_id;
            f >> base_0_tex >> base_1_tex >> detail_tex >> macro_tex >> old_reflection_tex >> emissive_tex >> mesh_id;
            base_2_tex.zero();
            tex_downsize_mobile = 0;
            flag = 0;
            REPAO(tex_downsize) = tex_downsize_mobile;
            if (f.ok())
                return true;
        ...
        case 0: {
            UID mesh_id;
            f >> base_0_tex >> base_1_tex >> detail_tex >> macro_tex >> old_reflection_tex >> mesh_id;
            base_2_tex.zero();
            tex_downsize_mobile = 0;
            flag = 0;
            emissive_tex.zero();
...
void ElmMaterial::save(MemPtr<TextNode> nodes) C {
    super::save(nodes);
    if (base_0_tex.valid())
        nodes.New().setFN("Base0", base_0_tex);
    if (base_1_tex.valid())
        nodes.New().setFN("Base1", base_1_tex);
    if (base_2_tex.valid())
        nodes.New().setFN("Base2", base_2_tex);
    if (detail_tex.valid())
        nodes.New().setFN("Detail", detail_tex);
    if (macro_tex.valid())
        nodes.New().setFN("Macro", macro_tex);
    if (emissive_tex.valid())
        nodes.New().setFN("Emissive", emissive_tex);
    if (tex_downsize[TSP_MOBILE])
        nodes.New().set("TexDownsizeMobile", tex_downsize[TSP_MOBILE]);
    if (tex_downsize[TSP_SWITCH])
        nodes.New().set("TexDownsizeSwitch", tex_downsize[TSP_SWITCH]);
    nodes.New().set("TexQuality", tex_quality);
    if (usesTexAlpha())
        nodes.New().set("UsesTexAlpha");
    if (usesTexBump())
        nodes.New().set("UsesTexBump");
    if (usesTexGlow())
        nodes.New().set("UsesTexGlow");
}
void ElmMaterial::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        C TextNode &n = nodes[i];
        if (n.name == "Base0")
            n.getValue(base_0_tex);
        else if (n.name == "Base1")
            n.getValue(base_1_tex);
        else if (n.name == "Base2")
            n.getValue(base_2_tex);
        else if (n.name == "Detail")
            n.getValue(detail_tex);
        else if (n.name == "Macro")
            n.getValue(macro_tex);
        else if (n.name == "Emissive")
            n.getValue(emissive_tex);
        else if (n.name == "TexDownsizeMobile")
            tex_downsize[TSP_MOBILE] = n.asInt();
        else if (n.name == "TexDownsizeSwitch")
            tex_downsize[TSP_SWITCH] = n.asInt();
        else if (n.name == "TexQuality")
            tex_quality = (Edit::Material::TEX_QUALITY)n.asInt();
        else if (n.name == "UsesTexAlpha")
            FlagSet(flag, USES_TEX_ALPHA, n.asBool1());
        else if (n.name == "UsesTexBump")
            FlagSet(flag, USES_TEX_BUMP, n.asBool1());
        else if (n.name == "UsesTexGlow")
            FlagSet(flag, USES_TEX_GLOW, n.asBool1());
    }
}
...
bool ElmWaterMtrl::containsTex(C UID &id, bool test_merged) C {
    if (id.valid()) {
        if (base_0_tex == id || base_1_tex == id || base_2_tex == id)
            return true;
        if (test_merged && MergedBaseTexturesID(base_0_tex, base_1_tex, base_2_tex) == id)
            return true;
    }
...
bool ElmWaterMtrl::load(File &f) {
    ...
    if (super::load(f))
        ...
        case 1: {
            f >> base_0_tex >> base_1_tex >> old_reflection_tex >> flag;
            base_2_tex.zero();
            if (flag & (1 << 3)) {
                tex_quality = Edit::Material::HIGH;
                FlagDisable(flag, 1 << 3);
        ...
        case 0: {
            f >> base_0_tex >> base_1_tex >> old_reflection_tex;
            flag = 0;
            base_2_tex.zero();
            if (f.ok())
                return true;
...
uint ElmPhys::undo(C ElmPhys &src) {
    ...
    if (Undo(file_time, src.file_time)) {
        changed |= CHANGE_AFFECT_FILE;
        transform = src.transform;
        box = src.box;
    }
...
void ElmPhys::save(MemPtr<TextNode> nodes) C {
    ...
    if (mesh_id.valid())
        nodes.New().setFN("Mesh", mesh_id);
    if (mtrl_id.valid())
        nodes.New().setFN("Material", mtrl_id);
    nodes.New().set("FileTime", file_time.text());
...
void ElmPhys::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        ...
        if (n.name == "Pose")
            ...
            n.getValueRaw(box);
        else if (n.name == "Density")
            n.getValueRaw(density);
        else if (n.name == "Mesh")
            n.getValue(mesh_id);
            ...
            file_time.text(n);
        else if (n.name == "MaterialTime")
            mtrl_time.text(n);
        else if (n.name == "DensityTime")
            density_time.text(n);
    }
}
bool ElmAnim::loop() C { return FlagOn(flag, LOOP); }
ElmAnim &ElmAnim::loop(bool on) {
    FlagSet(flag, LOOP, on);
    return T;
}
bool ElmAnim::linear() C { return FlagOn(flag, LINEAR); }
ElmAnim &ElmAnim::linear(bool on) {
    FlagSet(flag, LINEAR, on);
    return T;
}
...
void ElmAnim::rootMove(C Vec &move) {
    root_move = move;
    if (!rootMove())
        CHSSB(root_move.x);
}
void ElmAnim::rootRot(C Vec &rot) {
    root_rot = rot;
    if (!rootRot())
        CHSSB(root_rot.x);
}
uint ElmAnim::rootFlags() C {
    uint f = 0;
    if (flag & ROOT_DEL_POS_X)
        f |= ::ROOT_DEL_POSITION_X;
    if (flag & ROOT_DEL_POS_Y)
        f |= ::ROOT_DEL_POSITION_Y;
    if (flag & ROOT_DEL_POS_Z)
        f |= ::ROOT_DEL_POSITION_Z;
    if (flag & ROOT_DEL_ROT_X)
        f |= ::ROOT_DEL_ROTATION_X;
    if (flag & ROOT_DEL_ROT_Y)
        f |= ::ROOT_DEL_ROTATION_Y;
    if (flag & ROOT_DEL_ROT_Z)
        f |= ::ROOT_DEL_ROTATION_Z;
    if (flag & ROOT_FROM_BODY)
        f |= ::ROOT_BONE_POSITION | ::ROOT_START_IDENTITY;
    if (flag & ROOT_SMOOTH_ROT)
        f |= ::ROOT_SMOOTH_ROT;
    if (flag & ROOT_SMOOTH_POS)
        f |= ::ROOT_SMOOTH_POS;
    return f;
}
...
uint ElmAnim::undo(C ElmAnim &src) {
    ...
    if (Undo(loop_time, src.loop_time)) {
        changed |= CHANGE_AFFECT_FILE;
        loop(src.loop());
    }
    if (Undo(linear_time, src.linear_time)) {
        changed |= CHANGE_AFFECT_FILE;
        linear(src.linear());
    }
    if (Undo(file_time, src.file_time)) {
        changed |= CHANGE_AFFECT_FILE;
        transform = src.transform;
        root_move = src.root_move;
        root_rot = src.root_rot;
        fps = src.fps;
        FlagCopy(flag, src.flag, ROOT_ALL);
        imported_file_params = src.imported_file_params;
    }
...
uint ElmAnim::sync(C ElmAnim &src) {
    ...
    if (Sync(loop_time, src.loop_time)) {
        changed |= CHANGE_AFFECT_FILE;
        loop(src.loop());
    }
    if (Sync(linear_time, src.linear_time)) {
        changed |= CHANGE_AFFECT_FILE;
        linear(src.linear());
    }
...
void ElmAnim::fix5() {
    ...
    REPA(fps.params) {
        ...
        if (param.name == "start_frame")
            param.name = "startFrame";
        else if (param.name == "end_frame")
            param.name = "endFrame";
    }
    src_file = fps.encode();
...
bool ElmAnim::load(File &f) {
    if (super::load(f))
        ...
        case 4: {
            f >> skel_id >> transform >> root_move >> root_rot >> fps;
            flag = OldFlag1(f.getUShort());
            f >> loop_time >> linear_time >> skel_time >> file_time;
            fix5();
            if (f.ok())
                return true;
        } break;
        ...
        case 3: {
            f >> skel_id >> transform >> root_move >> root_rot;
            flag = OldFlag1(f.getUShort());
            f >> loop_time >> linear_time >> skel_time >> file_time;
            fps = 0;
            fix5();
...
void ElmAnim::save(MemPtr<TextNode> nodes) C {
    ...
    if (rootMove())
        nodes.New().setRaw("RootMove", root_move);
    if (rootRot())
        nodes.New().setRaw("RootRot", root_rot);
    if (flag & ROOT_DEL_POS)
        nodes.New().set("RootDelPos", FlagAll(flag, ROOT_DEL_POS) ? S : S + (FlagOn(flag, ROOT_DEL_POS_X) ? "X" : null) + (FlagOn(flag, ROOT_DEL_POS_Y) ? "Y" : null) + (FlagOn(flag, ROOT_DEL_POS_Z) ? "Z" : null));
    if (flag & ROOT_DEL_ROT)
        nodes.New().set("RootDelRot", FlagAll(flag, ROOT_DEL_ROT) ? S : S + (FlagOn(flag, ROOT_DEL_ROT_X) ? "X" : null) + (FlagOn(flag, ROOT_DEL_ROT_Y) ? "Y" : null) + (FlagOn(flag, ROOT_DEL_ROT_Z) ? "Z" : null));
    if (flag & ROOT_SMOOTH_ROT)
        nodes.New().set("RootSmoothRot");
    if (flag & ROOT_SMOOTH_POS)
        nodes.New().set("RootSmoothPos");
    if (fps > 0)
        nodes.New().set("FPS", fps);
    if (imported_file_params.is())
        nodes.New().set("ImportedFileParams", imported_file_params);
...
void ElmAnim::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        ...
        if (n.name == "Skeleton")
            ...
            n.getValueRaw(transform);
        else if (n.name == "RootMove")
            n.getValueRaw(root_move);
        else if (n.name == "RootRot")
            n.getValueRaw(root_rot);
        else if (n.name == "Loop")
            loop(n.asBool1());
        else if (n.name == "Linear")
            linear(n.asBool1());
        else if (n.name == "RootSmoothRot")
            FlagSet(flag, ROOT_SMOOTH_ROT, n.asBool1());
        else if (n.name == "RootSmoothPos")
            FlagSet(flag, ROOT_SMOOTH_POS, n.asBool1());
        else if (n.name == "FPS")
            fps = n.asFlt();
        else if (n.name == "ImportedFileParams")
            imported_file_params = n.asText();
        else if (n.name == "LoopTime")
            loop_time.text(n);
        else if (n.name == "LinearTime")
            linear_time.text(n);
        else if (n.name == "SkeletonTime")
            skel_time.text(n);
        else if (n.name == "FileTime")
            file_time.text(n);
        else if (n.name == "RootDelPos") {
            if (!n.value.is())
                FlagEnable(flag, ROOT_DEL_POS);
            else if (CharFlag(n.value[0]) & CHARF_DIG)
                FlagSet(flag, ROOT_DEL_POS, n.asBool());
            else {
        ...
        } else if (n.name == "RootDelRot") {
            if (!n.value.is())
                FlagEnable(flag, ROOT_DEL_ROT);
            else if (CharFlag(n.value[0]) & CHARF_DIG)
                FlagSet(flag, ROOT_DEL_ROT, n.asBool());
            else {
...
void ElmEnv::from(C EditEnv &src) {
    sun_id = src.sun_id;
    star_id = src.star_id;
    skybox_id = src.skybox_id;
    REPAO(cloud_id) = src.cloud_id[i];
}
...
void ElmEnv::save(MemPtr<TextNode> nodes) C {
    super::save(nodes);
    if (sun_id.valid())
        nodes.New().setFN("Sun", sun_id);
    if (star_id.valid())
        nodes.New().setFN("Stars", star_id);
    if (skybox_id.valid())
        nodes.New().setFN("SkyBox", skybox_id);
    {
...
void ElmEnv::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        ...
        if (n.name == "Sun")
            n.getValue(sun_id);
        else if (n.name == "Stars")
            n.getValue(star_id);
        else if (n.name == "SkyBox")
            n.getValue(skybox_id);
        else if (n.name == "Clouds") {
            REP(Min(n.nodes.elms(), Elms(cloud_id)))
            n.nodes[i].getValue(cloud_id[i]);
        }
    }
}
bool ElmWorld::valid() C { return area_size > 0; }
bool ElmWorld::equal(C ElmWorld &src) C {
    return super::equal(src) && hm_res_time == src.hm_res_time && ctrl_r_time == src.ctrl_r_time && ctrl_h_time == src.ctrl_h_time && max_climb_time == src.max_climb_time && max_slope_time == src.max_slope_time && cell_size_time == src.cell_size_time && cell_h_time == src.cell_h_time && env_time == src.env_time;
}
bool ElmWorld::newer(C ElmWorld &src) C {
    return super::newer(src) || hm_res_time > src.hm_res_time || ctrl_r_time > src.ctrl_r_time || ctrl_h_time > src.ctrl_h_time || max_climb_time > src.max_climb_time || max_slope_time > src.max_slope_time || cell_size_time > src.cell_size_time || cell_h_time > src.cell_h_time || env_time > src.env_time;
}
int ElmWorld::hmRes() C { return hm_res ? NearestPow2(hm_res) + 1 : 0; }
ElmWorld &ElmWorld::ctrlRadius(flt r) {
    ctrl_r = Max(r, 0);
    ctrl_r_time.getUTC();
    newVer();
    return T;
}
ElmWorld &ElmWorld::ctrlHeight(flt h) {
    ctrl_h = Max(h, 0);
    ctrl_h_time.getUTC();
    newVer();
    return T;
}
ElmWorld &ElmWorld::maxClimb(flt climb) {
    max_climb = Max(climb, 0);
    max_climb_time.getUTC();
    newVer();
    return T;
}
ElmWorld &ElmWorld::maxSlope(flt slope) {
    max_slope = Mid(slope, 0.0f, PI_2);
    max_slope_time.getUTC();
    newVer();
    return T;
}
ElmWorld &ElmWorld::cellSize(flt size) {
    cell_size = Max(size, 0.001f);
    cell_size_time.getUTC();
    newVer();
    return T;
}
ElmWorld &ElmWorld::cellHeight(flt h) {
    cell_h = Max(h, 0.001f);
    cell_h_time.getUTC();
    newVer();
    return T;
}
ElmWorld &ElmWorld::env(C UID &id) {
    env_id = id;
    env_time.getUTC();
    newVer();
    return T;
...
uint ElmWorld::undo(C ElmWorld &src) {
    ...
    if (!area_size && src.area_size) {
        area_size = src.area_size;
        changed |= CHANGE_AFFECT_FILE;
    }
...
void ElmWorld::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        C TextNode &n = nodes[i];
        if (n.name == "AreaSize")
            n.getValue(area_size);
        else if (n.name == "HeightmapResolution")
            n.getValue(hm_res);
        else if (n.name == "ControllerRadius")
            n.getValueRaw(ctrl_r);
        else if (n.name == "ControllerHeight")
            n.getValueRaw(ctrl_h);
        else if (n.name == "MaxClimb")
            n.getValueRaw(max_climb);
        else if (n.name == "MaxSlope")
            n.getValueRaw(max_slope);
        else if (n.name == "CellSize")
            n.getValueRaw(cell_size);
        else if (n.name == "CellHeight")
            n.getValueRaw(cell_h);
        else if (n.name == "Environment")
            n.getValue(env_id);
        else if (n.name == "HeightmapResolutionTime")
            hm_res_time.text(n);
        else if (n.name == "ControllerRadiusTime")
            ctrl_r_time.text(n);
        else if (n.name == "ControllerHeightTime")
            ctrl_h_time.text(n);
        else if (n.name == "MaxClimbTime")
            max_climb_time.text(n);
        else if (n.name == "MaxSlopeTime")
            max_slope_time.text(n);
        else if (n.name == "CellSizeTime")
            cell_size_time.text(n);
        else if (n.name == "CellHeightTime")
            cell_h_time.text(n);
...
uint ElmImage::undo(C ElmImage &src) {
    ...
    if (Undo(mip_maps_time, src.mip_maps_time)) {
        changed |= CHANGE_AFFECT_FILE;
        mipMaps(src.mipMaps());
    }
    if (Undo(pow2_time, src.pow2_time)) {
        changed |= CHANGE_AFFECT_FILE;
        pow2(src.pow2());
    }
    if (Undo(srgb_time, src.srgb_time)) {
        changed |= CHANGE_AFFECT_FILE;
        sRGB(src.sRGB());
    }
    if (Undo(env_time, src.env_time)) {
        changed |= CHANGE_AFFECT_FILE;
        env(src.env());
    }
    if (Undo(alpha_lum_time, src.alpha_lum_time)) {
        changed |= CHANGE_AFFECT_FILE;
        alphaLum(src.alphaLum());
    }
...
uint ElmImage::sync(C ElmImage &src) {
    ...
    if (Sync(mip_maps_time, src.mip_maps_time)) {
        changed |= CHANGE_AFFECT_FILE;
        mipMaps(src.mipMaps());
    }
    if (Sync(pow2_time, src.pow2_time)) {
        changed |= CHANGE_AFFECT_FILE;
        pow2(src.pow2());
    }
    if (Sync(srgb_time, src.srgb_time)) {
        changed |= CHANGE_AFFECT_FILE;
        sRGB(src.sRGB());
    }
    if (Sync(env_time, src.env_time)) {
        changed |= CHANGE_AFFECT_FILE;
        env(src.env());
    }
    if (Sync(alpha_lum_time, src.alpha_lum_time)) {
        changed |= CHANGE_AFFECT_FILE;
        alphaLum(src.alphaLum());
    }
...
bool ElmImage::load(File &f) {
    if (super::load(f))
        ...
        case 0: {
            f >> flag >> type >> mode >> mip_maps_time >> pow2_time >> alpha_lum_time >> type_time >> mode_time >> file_time;
            size = 0;
            size_time.zero();
            if (type >= 1)
                type = ElmImage::TYPE(type + 1);
...
void ElmImage::save(MemPtr<TextNode> nodes) C {
    super::save(nodes);
    if (InRange(type, ImageTypes))
        nodes.New().set("Type", ImageTypes[type].name);
    REPA(ImageModes)
    if (mode == ImageModes[i].mode) {
        nodes.New().set("Mode", ImageModes[i].name);
        break;
    }
    if (size.any())
        nodes.New().set("Size", size);
    ...
    if (sRGB())
        nodes.New().set("sRGB");
    if (env())
        nodes.New().set("Environment");
    if (alphaLum())
        nodes.New().set("AlphaFromLum");
    if (hasColor())
        nodes.New().set("HasColor");
    if (hasAlpha())
        nodes.New().set("HasAlpha");
...
void ElmImage::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        ...
        if (n.name == "Type") {
            REPA(ImageTypes)
            if (n.value == ImageTypes[i].name) {
                type = TYPE(i);
                break;
            }
        } else if (n.name == "Mode") {
            REPA(ImageModes)
            if (n.value == ImageModes[i].name) {
                mode = ImageModes[i].mode;
                break;
            }
        } else if (n.name == "Size")
            n.getValue(size);
        else if (n.name == "MipMaps")
            mipMaps(n.asBool1());
        else if (n.name == "Pow2")
            pow2(n.asBool1());
        else if (n.name == "sRGB")
            ...
            env(n.asBool1());
        else if (n.name == "AlphaFromLum")
            alphaLum(n.asBool1());
        else if (n.name == "HasColor")
            hasColor(n.asBool1());
        else if (n.name == "HasAlpha")
            hasAlpha(n.asBool1());
        else if (n.name == "MipMapsTime")
            mip_maps_time.text(n);
        else if (n.name == "Pow2Time")
            pow2_time.text(n);
        else if (n.name == "sRGBTime")
            srgb_time.text(n);
        else if (n.name == "EnvironmentTime")
            env_time.text(n);
        else if (n.name == "AlphaFromLumTime")
            alpha_lum_time.text(n);
        else if (n.name == "TypeTime")
            type_time.text(n);
        else if (n.name == "ModeTime")
            mode_time.text(n);
        else if (n.name == "SizeTime")
            size_time.text(n);
        else if (n.name == "FileTime")
            file_time.text(n);
    }
...
::ElmImageAtlas::Img &ElmImageAtlas::get(C UID &id) {
    int i;
    return images.binarySearch(id, i, CompareID) ? images[i] : images.NewAt(i);
}
void ElmImageAtlas::newData() {
    super::newData();
    file_time++;
    mip_maps_time++;
    compress_time++;
...
bool ElmImageAtlas::equal(C ElmImageAtlas &src) C {
    if (file_time != src.file_time || mip_maps_time != src.mip_maps_time || compress_time != src.compress_time)
        return false;
    if (images.elms() != src.images.elms())
        return false;
    REPA(images) {
        C Img &img = images[i];
        C Img *s = src.find(img.id);
        if (!s || !img.equal(*s))
            return false;
    }
...
bool ElmImageAtlas::mayContain(C UID &id) C {
    REPA(images)
    if (id == images[i].id)
        return true;
    return false;
...
uint ElmImageAtlas::undo(C ElmImageAtlas &src) {
    ...
    if (Undo(compress_time, src.compress_time)) {
        changed |= CHANGE_AFFECT_FILE;
    ...
    REPA(images) {
        Img &img = images[i];
        if (!img.removed && !src.find(img.id)) {
            img.removed = true;
            img.removed_time++;
            changed |= CHANGE_AFFECT_FILE;
        }
    ...
    REPA(src.images) {
        ...
        Img *t = find(s.id);
        if (t)
            changed |= t->undo(s) * CHANGE_AFFECT_FILE;
        else {
            get(s.id) = s;
            changed |= CHANGE_AFFECT_FILE;
        }
...
uint ElmImageAtlas::sync(C ElmImageAtlas &src) {
    ...
    if (Sync(compress_time, src.compress_time)) {
        changed |= CHANGE_AFFECT_FILE;
        compress(src.compress());
    }
    REPA(src.images) {
...
bool ElmImageAtlas::save(File &f) C {
    ...
    FREPA(images)
    f << images[i].removed << images[i].id << images[i].removed_time;
    return f.ok();
}
...
void ElmImageAtlas::save(MemPtr<TextNode> nodes) C {
    ...
    FREPA(T.images) // list in order
    {
        C Img &src = T.images[i];
        TextNode &dest = images.nodes.New().setName(src.id.asFileName());
        if (src.removed)
            dest.nodes.New().set("Removed");
        if (src.removed_time.is())
            dest.nodes.New().set("RemovedTime", src.removed_time.text());
    }
}
void ElmImageAtlas::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        ...
        if (n.name == "MipMaps")
            mipMaps(n.asBool1());
        else if (n.name == "Compress")
            compress(n.asBool1());
        else if (n.name == "MipMapsTime")
            mip_maps_time.text(n);
        else if (n.name == "CompressTime")
            compress_time.text(n);
        else if (n.name == "FileTime")
            file_time.text(n);
        else if (n.name == "Images")
            FREPA(n.nodes) // get in order
            {
                C TextNode &src = n.nodes[i];
                UID id;
                if (id.fromText(src.name) && id.valid()) {
                    Img &dest = images.New();
                    dest.id = id;
                    REPA(src.nodes) {
                        C TextNode &n = src.nodes[i];
                        if (n.name == "Removed")
                            dest.removed = n.asBool1();
                        else if (n.name == "RemovedTime")
                            dest.removed_time.text(n);
                    }
                }
            }
    }
...
void ElmIconSetts::save(MemPtr<TextNode> nodes) C {
    super::save(nodes);
    if (InRange(type, ElmImage::ImageTypes))
        nodes.New().set("Type", ElmImage::ImageTypes[type].name);
}
void ElmIconSetts::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        C TextNode &n = nodes[i];
        if (n.name == "Type") {
            REPA(ElmImage::ImageTypes)
            if (n.value == ElmImage::ImageTypes[i].name) {
                type = ElmImage::TYPE(i);
                break;
            }
        }
    }
}
ElmImage::TYPE ElmIcon::type(Project *proj) C {
    if (proj)
        if (Elm *elm = proj->findElm(icon_settings_id))
            if (ElmIconSetts *data = elm->iconSettsData())
                return data->type;
    return ElmImage::COMPRESSED;
}
bool ElmIcon::hasColor() C { return FlagOn(flag, HAS_COLOR); }
ElmIcon &ElmIcon::hasColor(bool on) {
    FlagSet(flag, HAS_COLOR, on);
    return T;
}
bool ElmIcon::hasAlpha() C { return FlagOn(flag, HAS_ALPHA); }
ElmIcon &ElmIcon::hasAlpha(bool on) {
    FlagSet(flag, HAS_ALPHA, on);
    return T;
}
IMAGE_TYPE ElmIcon::androidType(Project *proj) C {
    ElmImage::TYPE type = T.type(proj);
    return (type == ElmImage::COMPRESSED || type == ElmImage::COMPRESSED2) ? hasAlpha() ? IMAGE_ETC2_RGBA_SRGB : IMAGE_ETC2_RGB_SRGB : IMAGE_NONE;
}
IMAGE_TYPE ElmIcon::iOSType(Project *proj) C { return androidType(proj); }
IMAGE_TYPE ElmIcon::uwpType(Project *proj) C {
    ElmImage::TYPE type = T.type(proj);
    return (!UWPBC7 && (type == ElmImage::COMPRESSED && hasAlpha() || type == ElmImage::COMPRESSED2)) ? hasAlpha() ? IMAGE_BC3_SRGB : IMAGE_BC1_SRGB : IMAGE_NONE;
}
IMAGE_TYPE ElmIcon::webType(Project *proj) C {
    ElmImage::TYPE type = T.type(proj);
    return (!WebBC7 && (type == ElmImage::COMPRESSED && hasAlpha() || type == ElmImage::COMPRESSED2)) ? hasAlpha() ? IMAGE_BC3_SRGB : IMAGE_BC1_SRGB : IMAGE_NONE;
}
...
bool ElmIcon::load(File &f) {
    if (super::load(f))
        ...
        case 0: {
            f >> flag >> icon_settings_id >> obj_id >> icon_settings_time >> obj_time >> file_time;
            anim_id.zero();
            anim_pos = 0;
            if (f.ok())
                return true;
...
void ElmIcon::save(MemPtr<TextNode> nodes) C {
    super::save(nodes);
    if (icon_settings_id.valid())
        nodes.New().setFN("IconSettings", icon_settings_id);
    if (obj_id.valid())
        nodes.New().setFN("Object", obj_id);
    if (anim_id.valid())
        nodes.New().setFN("Animation", anim_id);
    nodes.New().set("Variation", variation_id);
    if (anim_pos)
        nodes.New().setRaw("AnimationPosition", anim_pos);
    if (hasColor())
        nodes.New().set("HasColor");
    if (hasAlpha())
        nodes.New().set("HasAlpha");
...
void ElmIcon::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        C TextNode &n = nodes[i];
        if (n.name == "IconSettings")
            n.getValue(icon_settings_id);
        else if (n.name == "Object")
            n.getValue(obj_id);
        else if (n.name == "Animation")
            n.getValue(anim_id);
        else if (n.name == "Variation")
            n.getValue(variation_id);
        else if (n.name == "AnimationPosition")
            n.getValueRaw(anim_pos);
        else if (n.name == "HasColor")
            hasColor(n.asBool1());
        else if (n.name == "HasAlpha")
            hasAlpha(n.asBool1());
        else if (n.name == "IconSettingsTime")
            icon_settings_time.text(n);
        else if (n.name == "ObjectTime")
            obj_time.text(n);
        else if (n.name == "FileTime")
            file_time.text(n);
        else if (n.name == "AnimationTime")
            anim_id_time.text(n);
        else if (n.name == "AnimationPositionTime")
            anim_pos_time.text(n);
        else if (n.name == "VariationTime")
            variation_time.text(n);
    }
}
...
void ElmPanelImage::save(MemPtr<TextNode> nodes) C {
    ...
    nodes.New().set("Compressed", compressed);
    {
        TextNode &images = nodes.New().setName("Images");
        FREPA(image_ids)
        images.nodes.New().setValueFN(image_ids[i]); // list in order
    }
}
void ElmPanelImage::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        C TextNode &n = nodes[i];
        if (n.name == "Compressed")
            compressed = n.asBool1();
        else if (n.name == "Images") {
            FREPA(n.nodes) // add in order
            {
                UID id;
                if (n.nodes[i].getValue(id) && id.valid())
                    image_ids.binaryInclude(id);
            }
        }
    }
}
...
void ElmGuiSkin::save(MemPtr<TextNode> nodes) C {
    super::save(nodes);
    {
        ...
        FREPA(elm_ids)
        elms.nodes.New().setValueFN(elm_ids[i]); // list in order
    }
}
...
bool ElmApp::equal(C ElmApp &src) C {
    return super::equal(src) && dirs_windows_time == src.dirs_windows_time && dirs_mac_time == src.dirs_mac_time && dirs_linux_time == src.dirs_linux_time && dirs_android_time == src.dirs_android_time && dirs_ios_time == src.dirs_ios_time && dirs_nintendo_time == src.dirs_nintendo_time && headers_windows_time == src.headers_windows_time && headers_mac_time == src.headers_mac_time && headers_linux_time == src.headers_linux_time && headers_android_time == src.headers_android_time && headers_ios_time == src.headers_ios_time && headers_nintendo_time == src.headers_nintendo_time && libs_windows_time == src.libs_windows_time && libs_mac_time == src.libs_mac_time && libs_linux_time == src.libs_linux_time && libs_android_time == src.libs_android_time && libs_ios_time == src.libs_ios_time && libs_nintendo_time == src.libs_nintendo_time && package_time == src.package_time && android_license_key_time == src.android_license_key_time && location_usage_reason_time == src.location_usage_reason_time && build_time == src.build_time && save_size_time == src.save_size_time && ms_publisher_id_time == src.ms_publisher_id_time && ms_publisher_name_time == src.ms_publisher_name_time && xbl_program_time == src.xbl_program_time && xbl_title_id_time == src.xbl_title_id_time && xbl_scid_time == src.xbl_scid_time && nintendo_initial_code_time == src.nintendo_initial_code_time && nintendo_app_id_time == src.nintendo_app_id_time && nintendo_publisher_name_time == src.nintendo_publisher_name_time && nintendo_legal_info_time == src.nintendo_legal_info_time && fb_app_id_time == src.fb_app_id_time && am_app_id_ios_time == src.am_app_id_ios_time && am_app_id_google_time == src.am_app_id_google_time && cb_app_id_ios_time == src.cb_app_id_ios_time && cb_app_signature_ios_time == src.cb_app_signature_ios_time && cb_app_id_google_time == src.cb_app_id_google_time && cb_app_signature_google_time == src.cb_app_signature_google_time && storage_time == src.storage_time && supported_orientations_time == src.supported_orientations_time && supported_languages_time == src.supported_languages_time && embed_engine_data_time == src.embed_engine_data_time && publish_proj_data_time == src.publish_proj_data_time && publish_physx_dll_time == src.publish_physx_dll_time && publish_steam_dll_time == src.publish_steam_dll_time && publish_open_vr_dll_time == src.publish_open_vr_dll_time && publish_data_as_pak_time == src.publish_data_as_pak_time && play_asset_delivery_time == src.play_asset_delivery_time && icon_time == src.icon_time && notification_icon_time == src.notification_icon_time && image_portrait_time == src.image_portrait_time && image_landscape_time == src.image_landscape_time && gui_skin_time == src.gui_skin_time;
}
bool ElmApp::newer(C ElmApp &src) C {
    return super::newer(src) || dirs_windows_time > src.dirs_windows_time || dirs_mac_time > src.dirs_mac_time || dirs_linux_time > src.dirs_linux_time || dirs_android_time > src.dirs_android_time || dirs_ios_time > src.dirs_ios_time || dirs_nintendo_time > src.dirs_nintendo_time || headers_windows_time > src.headers_windows_time || headers_mac_time > src.headers_mac_time || headers_linux_time > src.headers_linux_time || headers_android_time > src.headers_android_time || headers_ios_time > src.headers_ios_time || headers_nintendo_time > src.headers_nintendo_time || libs_windows_time > src.libs_windows_time || libs_mac_time > src.libs_mac_time || libs_linux_time > src.libs_linux_time || libs_android_time > src.libs_android_time || libs_ios_time > src.libs_ios_time || libs_nintendo_time > src.libs_nintendo_time || package_time > src.package_time || android_license_key_time > src.android_license_key_time || location_usage_reason_time > src.location_usage_reason_time || build_time > src.build_time || save_size_time > src.save_size_time || ms_publisher_id_time > src.ms_publisher_id_time || ms_publisher_name_time > src.ms_publisher_name_time || xbl_program_time > src.xbl_program_time || xbl_title_id_time > src.xbl_title_id_time || xbl_scid_time > src.xbl_scid_time || nintendo_initial_code_time > src.nintendo_initial_code_time || nintendo_app_id_time > src.nintendo_app_id_time || nintendo_publisher_name_time > src.nintendo_publisher_name_time || nintendo_legal_info_time > src.nintendo_legal_info_time || fb_app_id_time > src.fb_app_id_time || am_app_id_ios_time > src.am_app_id_ios_time || am_app_id_google_time > src.am_app_id_google_time || cb_app_id_ios_time > src.cb_app_id_ios_time || cb_app_signature_ios_time > src.cb_app_signature_ios_time || cb_app_id_google_time > src.cb_app_id_google_time || cb_app_signature_google_time > src.cb_app_signature_google_time || storage_time > src.storage_time || supported_orientations_time > src.supported_orientations_time || supported_languages_time > src.supported_languages_time || embed_engine_data_time > src.embed_engine_data_time || publish_proj_data_time > src.publish_proj_data_time || publish_physx_dll_time > src.publish_physx_dll_time || publish_steam_dll_time > src.publish_steam_dll_time || publish_open_vr_dll_time > src.publish_open_vr_dll_time || publish_data_as_pak_time > src.publish_data_as_pak_time || play_asset_delivery_time > src.play_asset_delivery_time || icon_time > src.icon_time || notification_icon_time > src.notification_icon_time || image_portrait_time > src.image_portrait_time || image_landscape_time > src.image_landscape_time || gui_skin_time > src.gui_skin_time;
}
...
ElmApp &ElmApp::embedEngineData(int e) {
    FlagSet(flag, EMBED_ENGINE_DATA, e != 0);
    FlagSet(flag, EMBED_ENGINE_DATA_FULL, e > 1);
    return T;
}
bool ElmApp::publishProjData() C { return FlagOn(flag, PUBLISH_PROJ_DATA); }
ElmApp &ElmApp::publishProjData(bool on) {
    FlagSet(flag, PUBLISH_PROJ_DATA, on);
    return T;
}
bool ElmApp::publishPhysxDll() C { return FlagOn(flag, PUBLISH_PHYSX_DLL); }
ElmApp &ElmApp::publishPhysxDll(bool on) {
    FlagSet(flag, PUBLISH_PHYSX_DLL, on);
    return T;
}
bool ElmApp::publishSteamDll() C { return FlagOn(flag, PUBLISH_STEAM_DLL); }
ElmApp &ElmApp::publishSteamDll(bool on) {
    FlagSet(flag, PUBLISH_STEAM_DLL, on);
    return T;
}
bool ElmApp::publishOpenVRDll() C { return FlagOn(flag, PUBLISH_OPEN_VR_DLL); }
ElmApp &ElmApp::publishOpenVRDll(bool on) {
    FlagSet(flag, PUBLISH_OPEN_VR_DLL, on);
    return T;
}
bool ElmApp::publishDataAsPak() C { return FlagOn(flag, PUBLISH_DATA_AS_PAK); }
ElmApp &ElmApp::publishDataAsPak(bool on) {
    FlagSet(flag, PUBLISH_DATA_AS_PAK, on);
    return T;
}
bool ElmApp::playAssetDelivery() C { return FlagOn(flag, PLAY_ASSET_DELIVERY); }
ElmApp &ElmApp::playAssetDelivery(bool on) {
    FlagSet(flag, PLAY_ASSET_DELIVERY, on);
    return T;
}
void ElmApp::newData() {
    super::newData();
    ...
    dirs_ios_time++;
    dirs_nintendo_time++;
    headers_windows_time++;
    headers_mac_time++;
    headers_linux_time++;
    headers_android_time++;
    headers_ios_time++;
    headers_nintendo_time++;
    libs_windows_time++;
    libs_mac_time++;
    libs_linux_time++;
    libs_android_time++;
    libs_ios_time++;
    libs_nintendo_time++;
    ms_publisher_id_time++;
    ms_publisher_name_time++;
    xbl_program_time++;
    xbl_title_id_time++;
    xbl_scid_time++;
    nintendo_initial_code_time++;
    nintendo_app_id_time++;
    nintendo_publisher_name_time++;
    nintendo_legal_info_time++;
    fb_app_id_time++;
    am_app_id_ios_time++;
    am_app_id_google_time++;
    cb_app_id_ios_time++;
    cb_app_signature_ios_time++;
    cb_app_id_google_time++;
    cb_app_signature_google_time++;
    package_time++;
    android_license_key_time++;
    location_usage_reason_time++;
    build_time++;
    save_size_time++;
    storage_time++;
    supported_orientations_time++;
    supported_languages_time++;
    embed_engine_data_time++;
    publish_proj_data_time++;
    publish_physx_dll_time++;
    publish_steam_dll_time++;
    publish_open_vr_dll_time++;
    publish_data_as_pak_time++;
    play_asset_delivery_time++;
    icon_time++;
    notification_icon_time++;
    image_portrait_time++;
    image_landscape_time++;
    gui_skin_time++;
}
uint ElmApp::undo(C ElmApp &src) {
...

    {
        ...
        if (Undo(embed_engine_data_time, src.embed_engine_data_time)) {
            ch = true;
            embedEngineData(src.embedEngineData());
        }
        if (Undo(publish_proj_data_time, src.publish_proj_data_time)) {
            ch = true;
            publishProjData(src.publishProjData());
        }
        if (Undo(publish_physx_dll_time, src.publish_physx_dll_time)) {
            ch = true;
            publishPhysxDll(src.publishPhysxDll());
        }
        if (Undo(publish_steam_dll_time, src.publish_steam_dll_time)) {
            ch = true;
            publishSteamDll(src.publishSteamDll());
        }
        if (Undo(publish_open_vr_dll_time, src.publish_open_vr_dll_time)) {
            ch = true;
            publishOpenVRDll(src.publishOpenVRDll());
        }
        if (Undo(publish_data_as_pak_time, src.publish_data_as_pak_time)) {
            ch = true;
            publishDataAsPak(src.publishDataAsPak());
        }
        if (Undo(play_asset_delivery_time, src.play_asset_delivery_time)) {
            ch = true;
...
uint ElmApp::sync(C ElmApp &src, bool manual) {
    ...
    if (manual) // these are advanced settings and can be synced only manually
    {
        ...
        if (Sync(embed_engine_data_time, src.embed_engine_data_time)) {
            ch = true;
            embedEngineData(src.embedEngineData());
        }
        if (Sync(publish_proj_data_time, src.publish_proj_data_time)) {
            ch = true;
            publishProjData(src.publishProjData());
        }
        if (Sync(publish_physx_dll_time, src.publish_physx_dll_time)) {
            ch = true;
            publishPhysxDll(src.publishPhysxDll());
        }
        if (Sync(publish_steam_dll_time, src.publish_steam_dll_time)) {
            ch = true;
            publishSteamDll(src.publishSteamDll());
        }
        if (Sync(publish_open_vr_dll_time, src.publish_open_vr_dll_time)) {
            ch = true;
            publishOpenVRDll(src.publishOpenVRDll());
        }
        if (Sync(publish_data_as_pak_time, src.publish_data_as_pak_time)) {
            ch = true;
            publishDataAsPak(src.publishDataAsPak());
        }
        if (Sync(play_asset_delivery_time, src.play_asset_delivery_time)) {
            ch = true;
            playAssetDelivery(src.playAssetDelivery());
        }
...
bool ElmApp::load(File &f) {
    ...
    if (super::load(f))
        ...
        case 23: {
            ...
            f >> embed_engine_data_time >> publish_proj_data_time >> publish_physx_dll_time >> publish_steam_dll_time >> publish_open_vr_dll_time >> publish_data_as_pak_time >> play_asset_delivery_time;
            f >> icon_time >> notification_icon_time >> image_portrait_time >> image_landscape_time >> gui_skin_time;
            if (supported_languages.loadRaw(f) && f.ok())
                return true;
        } break;
        ...
        case 13: {
            GetStr2(f, dirs_windows);
            GetStr2(f, dirs_nonwindows);
            GetStr2(f, headers_windows);
            GetStr2(f, headers_nonwindows);
            GetStr2(f, libs_windows);
            GetStr2(f, libs_mac);
            GetStr2(f, libs_linux);
            GetStr2(f, libs_android);
            GetStr2(f, libs_ios);
            GetStr2(f, package);
            GetStr2(f, android_license_key);
            GetStr2(f, location_usage_reason);
            f >> build >> storage >> supported_orientations >> flag;
            f >> fb_app_id;
            GetStr2(f, cb_app_id_ios);
            GetStr2(f, cb_app_signature_ios);
            GetStr2(f, cb_app_id_google);
            GetStr2(f, cb_app_signature_google);
            f >> icon >> image_portrait >> image_landscape >> gui_skin;
            f >> dirs_windows_time >> dirs_nonwindows_time >> headers_windows_time >> headers_nonwindows_time;
            f >> libs_windows_time >> libs_mac_time >> libs_linux_time >> libs_android_time >> libs_ios_time;
            f >> package_time >> android_license_key_time >> location_usage_reason_time >> build_time >> storage_time >> supported_orientations_time;
        ...
        case 1: {
            ...
            GetStr(f, libs_windows);
            GetStr(f, package);
            f >> build >> storage >> supported_orientations >> flag >> icon >> image_portrait >> headers_windows_time >> libs_windows_time >> package_time >> build_time >> storage_time >> supported_orientations_time >> embed_engine_data_time >> publish_proj_data_time >> publish_physx_dll_time >> publish_data_as_pak_time >> icon_time >> image_portrait_time;
            if (f.ok())
                return true;
        } break;
        ...
        case 0: {
            ...
            GetStr(f, libs_windows);
            GetStr(f, package);
            f >> build >> storage >> supported_orientations >> embed_engine_data >> icon >> image_portrait >> headers_windows_time >> libs_windows_time >> package_time >> build_time >> storage_time >> supported_orientations_time >> embed_engine_data_time >> icon_time >> image_portrait_time;
...
void ElmApp::save(MemPtr<TextNode> nodes) C {
    ...
    if (package.is())
        nodes.New().set("Package", package);
    if (save_size >= 0)
        nodes.New().set("SaveSize", save_size);

    if (icon.valid())
        nodes.New().setFN("Icon", icon);
    if (notification_icon.valid())
        nodes.New().setFN("NotificationIcon", notification_icon);
    if (image_portrait.valid())
        nodes.New().setFN("SplashPortrait", image_portrait);
    if (image_landscape.valid())
        nodes.New().setFN("SplashLandscape", image_landscape);
    if (gui_skin.valid())
        nodes.New().setFN("GuiSkin", gui_skin);

    if (embedEngineData())
        nodes.New().set("EmbedEngineData", embedEngineData());
    if (!publishProjData())
        nodes.New().set("PublishProjectData", publishProjData());
    if (!publishDataAsPak())
        nodes.New().set("PublishDataAsPAK", publishDataAsPak());
    nodes.New().set("PublishPhysXDLL", publishPhysxDll());
    if (publishSteamDll())
        nodes.New().set("PublishSteamDLL");
    if (publishOpenVRDll())
        nodes.New().set("PublishOpenVRDLL");

    if (dirs_windows.is())
        nodes.New().set("IncludePathsWindows", dirs_windows);
    if (dirs_mac.is())
        nodes.New().set("IncludePathsMac", dirs_mac);
    if (dirs_linux.is())
        nodes.New().set("IncludePathsLinux", dirs_linux);
    if (dirs_android.is())
        nodes.New().set("IncludePathsAndroid", dirs_android);
    if (dirs_ios.is())
        nodes.New().set("IncludePathsiOS", dirs_ios);
    if (dirs_nintendo.is())
        nodes.New().set("IncludePathsNintendo", dirs_nintendo);
    if (headers_windows.is())
        nodes.New().set("IncludeHeadersWindows", headers_windows);
    if (headers_mac.is())
        nodes.New().set("IncludeHeadersMac", headers_mac);
    if (headers_linux.is())
        nodes.New().set("IncludeHeadersLinux", headers_linux);
    if (headers_android.is())
        nodes.New().set("IncludeHeadersAndroid", headers_android);
    if (headers_ios.is())
        nodes.New().set("IncludeHeadersiOS", headers_ios);
    if (headers_nintendo.is())
        nodes.New().set("IncludeHeadersNintendo", headers_nintendo);
    if (libs_windows.is())
        nodes.New().set("IncludeLibrariesWindows", libs_windows);
    if (libs_mac.is())
        nodes.New().set("IncludeLibrariesMac", libs_mac);
    if (libs_linux.is())
        nodes.New().set("IncludeLibrariesLinux", libs_linux);
    if (libs_android.is())
        nodes.New().set("IncludeLibrariesAndroid", libs_android);
    if (libs_ios.is())
        nodes.New().set("IncludeLibrariesiOS", libs_ios);
    if (libs_nintendo.is())
        nodes.New().set("IncludeLibrariesNintendo", libs_nintendo);

    REPA(StorageModes)
    if (storage == StorageModes[i].mode) {
        nodes.New().set("Storage", StorageModes[i].name);
        break;
    }
    nodes.New().set("SupportedOrientations", supported_orientations);
    if (location_usage_reason.is())
        nodes.New().set("LocationUsageReason", location_usage_reason);
    if (playAssetDelivery())
        nodes.New().set("PlayAssetDelivery");
    if (android_license_key.is())
        nodes.New().set("AndroidLicenseKey", android_license_key);
    if (supported_languages.elms()) {
        TextNode &l = nodes.New().setName("SupportedLanguages");
        FREPA(supported_languages)
        l.nodes.New().setValue(LanguageCode(supported_languages[i]));
    }

    {
        TextNode &ms = nodes.New().setName("Microsoft");
        if (ms_publisher_id.valid())
            ms.nodes.New().set("PublisherID", ms_publisher_id.asCanonical());
        ms.nodes.New().set("PublisherIDTime", ms_publisher_id_time.text());
        if (ms_publisher_name.is())
            ms.nodes.New().set("PublisherName", ms_publisher_name);
...

    {
        ...
        if (xbl_title_id)
            xbl.nodes.New().set("TitleID", xbl_title_id);
        xbl.nodes.New().set("TitleIDTime", xbl_title_id_time.text());
        if (xbl_scid.valid())
            xbl.nodes.New().set("SCID", xbl_scid.asCanonical());
        xbl.nodes.New().set("SCIDTime", xbl_scid_time.text());
    }

    {
        TextNode &nintendo = nodes.New().setName("Nintendo");
        if (nintendo_initial_code.is())
            nintendo.nodes.New().set("InitialCode", nintendo_initial_code);
        nintendo.nodes.New().set("InitialCodeTime", nintendo_initial_code_time.text());
        if (nintendo_app_id)
            nintendo.nodes.New().set("AppID", nintendo_app_id);
        nintendo.nodes.New().set("AppIDTime", nintendo_app_id_time.text());
        if (nintendo_publisher_name.is())
            nintendo.nodes.New().set("PublisherName", nintendo_publisher_name);
        nintendo.nodes.New().set("PublisherNameTime", nintendo_publisher_name_time.text());
        if (nintendo_legal_info.is())
            nintendo.nodes.New().set("LegalInfo", nintendo_legal_info);
        nintendo.nodes.New().set("LegalInfoTime", nintendo_legal_info_time.text());
    }

    if (fb_app_id)
        nodes.New().set("FacebookAppID", fb_app_id);

    {
        TextNode &am = nodes.New().setName("AdMob");
        if (am_app_id_ios.is())
            am.nodes.New().set("AppIDiOS", am_app_id_ios);
        if (am_app_id_google.is())
            am.nodes.New().set("AppIDGoogle", am_app_id_google);
...

    {
        ...
        if (cb_app_id_ios.is())
            cb.nodes.New().set("AppIDiOS", cb_app_id_ios);
        if (cb_app_signature_ios.is())
            cb.nodes.New().set("AppSignatureiOS", cb_app_signature_ios);
        if (cb_app_id_google.is())
            cb.nodes.New().set("AppIDGoogle", cb_app_id_google);
        if (cb_app_signature_google.is())
            cb.nodes.New().set("AppSignatureGoogle", cb_app_signature_google);
...
void ElmApp::load(C MemPtr<TextNode> &nodes) {
    ...
    REPA(nodes) {
        ...
        if (n.name == "Build")
            ...
            n.getValue(package);
        else if (n.name == "SaveSize")
            n.getValue(save_size);
        else

            if (n.name == "Icon")
            n.getValue(icon);
        else if (n.name == "NotificationIcon")
            n.getValue(notification_icon);
        else if (n.name == "SplashPortrait")
            n.getValue(image_portrait);
        else if (n.name == "SplashLandscape")
            n.getValue(image_landscape);
        else if (n.name == "GuiSkin")
            n.getValue(gui_skin);
        else

            if (n.name == "EmbedEngineData")
            embedEngineData(n.value.is() ? n.asInt() : true);
        else if (n.name == "PublishProjectData")
            publishProjData(n.asBool1());
        else if (n.name == "PublishDataAsPAK")
            publishDataAsPak(n.asBool1());
        else if (n.name == "PublishPhysXDLL")
            publishPhysxDll(n.asBool1());
        else if (n.name == "PublishSteamDLL")
            publishSteamDll(n.asBool1());
        else if (n.name == "PublishOpenVRDLL")
            publishOpenVRDll(n.asBool1());
        else

            if (n.name == "IncludePathsWindows")
            n.getValue(dirs_windows);
        else if (n.name == "IncludePathsMac")
            n.getValue(dirs_mac);
        else if (n.name == "IncludePathsLinux")
            n.getValue(dirs_linux);
        else if (n.name == "IncludePathsAndroid")
            n.getValue(dirs_android);
        else if (n.name == "IncludePathsiOS")
            n.getValue(dirs_ios);
        else if (n.name == "IncludePathsNintendo")
            n.getValue(dirs_nintendo);
        else if (n.name == "IncludeHeadersWindows")
            n.getValue(headers_windows);
        else if (n.name == "IncludeHeadersMac")
            n.getValue(headers_mac);
        else if (n.name == "IncludeHeadersLinux")
            n.getValue(headers_linux);
        else if (n.name == "IncludeHeadersAndroid")
            n.getValue(headers_android);
        else if (n.name == "IncludeHeadersiOS")
            n.getValue(headers_ios);
        else if (n.name == "IncludeHeadersNintendo")
            n.getValue(headers_nintendo);
        else if (n.name == "IncludeLibrariesWindows")
            n.getValue(libs_windows);
        else if (n.name == "IncludeLibrariesMac")
            n.getValue(libs_mac);
        else if (n.name == "IncludeLibrariesLinux")
            n.getValue(libs_linux);
        else if (n.name == "IncludeLibrariesAndroid")
            n.getValue(libs_android);
        else if (n.name == "IncludeLibrariesiOS")
            n.getValue(libs_ios);
        else if (n.name == "IncludeLibrariesNintendo")
            n.getValue(libs_nintendo);
        else

            if (n.name == "Storage") {
            REPA(StorageModes)
            if (n.value == StorageModes[i].name) {
                storage = StorageModes[i].mode;
                break;
            }
        } else if (n.name == "SupportedOrientations")
            supported_orientations = n.asInt();
        else if (n.name == "LocationUsageReason")
            n.getValue(location_usage_reason);
        else if (n.name == "PlayAssetDelivery")
            playAssetDelivery(n.asBool1());
        else if (n.name == "AndroidLicenseKey")
            n.getValue(android_license_key);
        else

            if (n.name == "SupportedLanguages") {
            Memc<LANG_TYPE> langs;
            FREPA(n.nodes) {
                C TextNode &l = n.nodes[i];
                if (LANG_TYPE lang = LanguageCode(l.value))
                    langs.include(lang);
            }
            supported_languages = langs;
        } else if (n.name == "Microsoft") {
            REPA(n.nodes) {
                C TextNode &ms = n.nodes[i];
                if (ms.name == "PublisherID")
                    ms_publisher_id.fromCanonical(ms.asText());
                else if (ms.name == "PublisherIDTime")
                    ms_publisher_id_time.text(ms);
                else if (ms.name == "PublisherName")
                    ms.getValue(ms_publisher_name);
                else if (ms.name == "PublisherNameTime")
                    ms_publisher_name_time.text(ms);
            }
        } else if (n.name == "XboxLive") {
            REPA(n.nodes) {
                C TextNode &xbl = n.nodes[i];
                if (xbl.name == "Program")
                    xbl_program = (Edit::XBOX_LIVE)xbl.asInt();
                else if (xbl.name == "ProgramTime")
                    xbl_program_time.text(xbl);
                else if (xbl.name == "TitleID")
                    xbl.getValue(xbl_title_id);
                else if (xbl.name == "TitleIDTime")
                    xbl_title_id_time.text(xbl);
                else if (xbl.name == "SCID")
                    xbl_scid.fromCanonical(xbl.asText());
                else if (xbl.name == "SCIDTime")
                    xbl_scid_time.text(xbl);
            }
        } else if (n.name == "Nintendo") {
            REPA(n.nodes) {
                C TextNode &nintendo = n.nodes[i];
                if (nintendo.name == "InitialCode")
                    nintendo.getValue(nintendo_initial_code);
                else if (nintendo.name == "InitialCodeTime")
                    nintendo_initial_code_time.text(nintendo);
                else if (nintendo.name == "AppID")
                    nintendo.getValue(nintendo_app_id);
                else if (nintendo.name == "AppIDTime")
                    nintendo_app_id_time.text(nintendo);
                else if (nintendo.name == "PublisherName")
                    nintendo.getValue(nintendo_publisher_name);
                else if (nintendo.name == "PublisherNameTime")
                    nintendo_publisher_name_time.text(nintendo);
                else if (nintendo.name == "LegalInfo")
                    nintendo.getValue(nintendo_legal_info);
                else if (nintendo.name == "LegalInfoTime")
                    nintendo_legal_info_time.text(nintendo);
            }
        } else if (n.name == "FacebookAppID")
            n.getValue(fb_app_id);
        else if (n.name == "AdMob") {
            REPA(n.nodes) {
                C TextNode &am = n.nodes[i];
                if (am.name == "AppIDiOS")
                    am.getValue(am_app_id_ios);
                else if (am.name == "AppIDGoogle")
                    am.getValue(am_app_id_google);
                else if (am.name == "AppIDiOSTime")
                    am_app_id_ios_time.text(am);
                else if (am.name == "AppIDGoogleTime")
                    am_app_id_google_time.text(am);
            }
        } else if (n.name == "Chartboost") {
            REPA(n.nodes) {
                C TextNode &cb = n.nodes[i];
                if (cb.name == "AppIDiOS")
                    cb.getValue(cb_app_id_ios);
                else if (cb.name == "AppSignatureiOS")
                    cb.getValue(cb_app_signature_ios);
                else if (cb.name == "AppIDGoogle")
                    cb.getValue(cb_app_id_google);
                else if (cb.name == "AppSignatureGoogle")
                    cb.getValue(cb_app_signature_google);
                else if (cb.name == "AppIDiOSTime")
                    cb_app_id_ios_time.text(cb);
                else if (cb.name == "AppSignatureiOSTime")
                    cb_app_signature_ios_time.text(cb);
                else if (cb.name == "AppIDGoogleTime")
                    cb_app_id_google_time.text(cb);
                else if (cb.name == "AppSignatureGoogleTime")
                    cb_app_signature_google_time.text(cb);
            }
        } else if (n.name == "BuildTime")
            build_time.text(n);
        else if (n.name == "PackageTime")
            package_time.text(n);
        else if (n.name == "SaveSizeTime")
            save_size_time.text(n);
        else

            if (n.name == "IconTime")
            icon_time.text(n);
        else if (n.name == "NotificationIconTime")
            notification_icon_time.text(n);
        else if (n.name == "SplashPortraitTime")
            image_portrait_time.text(n);
        else if (n.name == "SplashLandscapeTime")
            image_landscape_time.text(n);
        else if (n.name == "GuiSkinTime")
            gui_skin_time.text(n);
        else

            if (n.name == "EmbedEngineDataTime")
            embed_engine_data_time.text(n);
        else if (n.name == "PublishProjectDataTime")
            publish_proj_data_time.text(n);
        else if (n.name == "PublishDataAsPAKTime")
            publish_data_as_pak_time.text(n);
        else if (n.name == "PublishPhysXDLLTime")
            publish_physx_dll_time.text(n);
        else if (n.name == "PublishSteamDLLTime")
            publish_steam_dll_time.text(n);
        else if (n.name == "PublishOpenVRDLLTime")
            publish_open_vr_dll_time.text(n);
        else

            if (n.name == "IncludePathsWindowsTime")
            dirs_windows_time.text(n);
        else if (n.name == "IncludePathsMacTime")
            dirs_mac_time.text(n);
        else if (n.name == "IncludePathsLinuxTime")
            dirs_linux_time.text(n);
        else if (n.name == "IncludePathsAndroidTime")
            dirs_android_time.text(n);
        else if (n.name == "IncludePathsiOSTime")
            dirs_ios_time.text(n);
        else if (n.name == "IncludePathsNintendoTime")
            dirs_nintendo_time.text(n);
        else if (n.name == "IncludeHeadersWindowsTime")
            headers_windows_time.text(n);
        else if (n.name == "IncludeHeadersMacTime")
            headers_mac_time.text(n);
        else if (n.name == "IncludeHeadersLinuxTime")
            headers_linux_time.text(n);
        else if (n.name == "IncludeHeadersAndroidTime")
            headers_android_time.text(n);
        else if (n.name == "IncludeHeadersiOSTime")
            headers_ios_time.text(n);
        else if (n.name == "IncludeHeadersNintendoTime")
            headers_nintendo_time.text(n);
        else if (n.name == "IncludeLibrariesWindowsTime")
            libs_windows_time.text(n);
        else if (n.name == "IncludeLibrariesMacTime")
            libs_mac_time.text(n);
        else if (n.name == "IncludeLibrariesLinuxTime")
            libs_linux_time.text(n);
        else if (n.name == "IncludeLibrariesAndroidTime")
            libs_android_time.text(n);
        else if (n.name == "IncludeLibrariesiOSTime")
            libs_ios_time.text(n);
        else if (n.name == "IncludeLibrariesNintendoTime")
            libs_nintendo_time.text(n);
        else

            if (n.name == "StorageTime")
            storage_time.text(n);
        else if (n.name == "SupportedOrientationsTime")
            supported_orientations_time.text(n);
        else if (n.name == "SupportedLanguagesTime")
            supported_languages_time.text(n);
        else if (n.name == "LocationUsageReasonTime")
            location_usage_reason_time.text(n);
        else if (n.name == "PlayAssetDeliveryTime")
            play_asset_delivery_time.text(n);
        else if (n.name == "AndroidLicenseKeyTime")
            android_license_key_time.text(n);
        else

            if (n.name == "FacebookAppIDTime")
            fb_app_id_time.text(n);
    }
}
...
Elm &Elm::setRemoved(bool removed, C TimeStamp &time) {
    T.removed(removed);
    T.removed_time = time;
    return T;
}
Elm &Elm::setPublish(bool all, C TimeStamp &time) {
    T.publish(all);
    T.publish_time = time;
    return T;
}
Elm &Elm::setPublish(bool all, bool mobile, C TimeStamp &time) {
    T.publish(all);
    T.publishMobile(mobile);
    T.publish_time = time;
    return T;
}
Elm &Elm::setName(C Str &name, C TimeStamp &time) {
    T.name = name;
    T.name_time = time;
    return T;
}
Elm &Elm::setParent(C UID &parent_id, C TimeStamp &time) {
    T.parent_id = parent_id;
    T.parent_time = time;
    return T;
}
Elm &Elm::setParent(C Elm *parent, C TimeStamp &time) { return setParent(parent ? parent->id : UIDZero, time); }
Elm &Elm::setSrcFile(C Str &src_file, C TimeStamp &time) {
    if (ElmData *data = Data()) {
        data->setSrcFile(src_file, time);
        data->newVer();
    }
    return T;
}
ElmObjClass *Elm::objClassData() {
    if (type == ELM_OBJ_CLASS) {
        if (!data)
            data = new ElmObjClass;
        return CAST(ElmObjClass, data);
    }
    return null;
}
...
ElmEnv *Elm::envData() {
    if (type == ELM_ENV) {
        if (!data)
            data = new ElmEnv;
        return CAST(ElmEnv, data);
    }
    return null;
}
C ElmEnv *Elm::envData() C { return (type == ELM_ENV) ? CAST(ElmEnv, data) : null; }
ElmWorld *Elm::worldData() {
    if (type == ELM_WORLD) {
        if (!data)
            data = new ElmWorld;
        return CAST(ElmWorld, data);
    }
    return null;
}
...
bool Elm::newerFile(C Elm &src) C // if 'this' has newer file than 'src'
{
    if (type == src.type)
        switch (type) {
        case ELM_MESH:
            return meshData()->file_time > src.meshData()->file_time;
        case ELM_SKEL:
            return skelData()->file_time > src.skelData()->file_time;
        case ELM_PHYS:
            return physData()->file_time > src.physData()->file_time;
        case ELM_ANIM:
            return animData()->file_time > src.animData()->file_time;
        case ELM_GUI:
            return guiData()->file_time > src.guiData()->file_time;
        case ELM_IMAGE:
            return imageData()->file_time > src.imageData()->file_time;
        case ELM_IMAGE_ATLAS:
            return imageAtlasData()->file_time > src.imageAtlasData()->file_time;
        case ELM_SOUND:
            return soundData()->file_time > src.soundData()->file_time;
        case ELM_VIDEO:
            return videoData()->file_time > src.videoData()->file_time;
        case ELM_FILE:
            return fileData()->file_time > src.fileData()->file_time;
        case ELM_ICON:
            return iconData()->file_time > src.iconData()->file_time;
        }
    return false;
}
bool Elm::newerData(C Elm &src) C // if 'this' has any data newer than 'src'
{
    if (type == src.type)
        switch (type) {
        case ELM_ENUM:
            return enumData()->newer(*src.enumData());
        case ELM_IMAGE:
            return imageData()->newer(*src.imageData());
        case ELM_IMAGE_ATLAS:
            return imageAtlasData()->newer(*src.imageAtlasData());
        case ELM_FONT:
            return fontData()->newer(*src.fontData());
        case ELM_TEXT_STYLE:
            return textStyleData()->newer(*src.textStyleData());
        case ELM_PANEL:
            return panelData()->newer(*src.panelData());
        case ELM_GUI:
            return guiData()->newer(*src.guiData());
        case ELM_MTRL:
            return mtrlData()->newer(*src.mtrlData());
        case ELM_WATER_MTRL:
            return waterMtrlData()->newer(*src.waterMtrlData());
        case ELM_PHYS_MTRL:
            return physMtrlData()->newer(*src.physMtrlData());
        case ELM_OBJ_CLASS:
            return objClassData()->newer(*src.objClassData());
        case ELM_OBJ:
            return objData()->newer(*src.objData());
        case ELM_MESH:
            return meshData()->newer(*src.meshData());
        case ELM_SKEL:
            return skelData()->newer(*src.skelData());
        case ELM_PHYS:
            return physData()->newer(*src.physData());
        case ELM_ANIM:
            return animData()->newer(*src.animData());
        case ELM_PANEL_IMAGE:
            return panelImageData()->newer(*src.panelImageData());
        case ELM_ICON:
            return iconData()->newer(*src.iconData());
        case ELM_ENV:
            return envData()->newer(*src.envData());
        case ELM_WORLD:
            return worldData()->newer(*src.worldData());
        case ELM_SOUND:
            return soundData()->newer(*src.soundData());
        case ELM_VIDEO:
            return videoData()->newer(*src.videoData());
        case ELM_FILE:
            return fileData()->newer(*src.fileData());
        case ELM_CODE:
            return codeData()->newer(*src.codeData());
        case ELM_APP:
            return appData()->newer(*src.appData());
        case ELM_ICON_SETTS:
...
uint Elm::syncData(C Elm &src) {
    if (type == src.type)
        switch (type) {
        case ELM_ENUM:
            return enumData()->sync(*src.enumData());
        case ELM_IMAGE:
            return imageData()->sync(*src.imageData());
        case ELM_IMAGE_ATLAS:
            return imageAtlasData()->sync(*src.imageAtlasData());
        case ELM_FONT:
            return fontData()->sync(*src.fontData());
        case ELM_TEXT_STYLE:
            return textStyleData()->sync(*src.textStyleData());
        case ELM_PANEL:
            return panelData()->sync(*src.panelData());
        case ELM_GUI:
            return guiData()->sync(*src.guiData());
        case ELM_MTRL:
            return mtrlData()->sync(*src.mtrlData());
        case ELM_WATER_MTRL:
            return waterMtrlData()->sync(*src.waterMtrlData());
        case ELM_PHYS_MTRL:
            return physMtrlData()->sync(*src.physMtrlData());
        case ELM_OBJ_CLASS:
            return objClassData()->sync(*src.objClassData());
        case ELM_OBJ:
            return objData()->sync(*src.objData());
        case ELM_MESH:
            return meshData()->sync(*src.meshData());
        case ELM_SKEL:
            return skelData()->sync(*src.skelData());
        case ELM_PHYS:
            return physData()->sync(*src.physData());
        case ELM_ANIM:
            return animData()->sync(*src.animData());
        case ELM_PANEL_IMAGE:
            return panelImageData()->sync(*src.panelImageData());
        case ELM_ICON:
            return iconData()->sync(*src.iconData());
        case ELM_ENV:
            return envData()->sync(*src.envData());
        case ELM_WORLD:
            return worldData()->sync(*src.worldData());
        case ELM_SOUND:
            return soundData()->sync(*src.soundData());
        case ELM_VIDEO:
            return videoData()->sync(*src.videoData());
        case ELM_FILE:
            return fileData()->sync(*src.fileData());
        case ELM_CODE:
            return codeData()->sync(*src.codeData());
        case ELM_APP:
            return appData()->sync(*src.appData(), false);
        case ELM_ICON_SETTS:
...
bool Elm::save(File &f, bool network, bool skip_name_data) C {
    ...
    if (network)
        FlagDisable(flag, IMPORTING); // don't transmit IMPORTING status over network, to make reload requests local only, so one reload doesn't trigger reload on all connected computers
    if (data)
        flag |= DATA; // !! ENABLE AFTER DISABLING OTHERS ABOVE BECAUSE BITS MAY OVERLAP !!
    f.cmpUIntV(4);
...
bool Elm::load(File &f, bool network, bool skip_name_data) {
    ...
    case 0: {
        ...
        if (type >= ELM_NUM)
            return false;
        f >> name_time >> parent_time >> removed_time;
        publish_time.zero();
        if (!skip_name_data)
            GetStr(f, name);
...
void Elm::save(TextNode &node) C {
    ...
    if (parent_id.valid())
        node.nodes.New().setFN("Parent", parent_id);
    if (removed())
        node.nodes.New().set("Removed");
    if (noPublish())
        node.nodes.New().set("Publish", publish());
    if (noPublishMobile())
        node.nodes.New().set("PublishMobile", publishMobile());

    node.nodes.New().set("NameTime", name_time.text());
    node.nodes.New().set("ParentTime", parent_time.text());
    if (removed_time.is())
        node.nodes.New().set("RemovedTime", removed_time.text());
    if (publish_time.is())
        node.nodes.New().set("PublishTime", publish_time.text());
    ...
    if (data)
        data->save(node.nodes.New().setName("Data").nodes);
}
bool Elm::load(C TextNode &node, Str &error) // assumes that 'error' doesn't need to be cleared at start, and 'T.id' was already set
{
    ...
    REPA(node.nodes) {
        ...
        if (n.name == "Type") {
            REP(ELM_NUM)
            if (n.value == ElmTypeNameNoSpaceDummy.names[i]) {
                type = ELM_TYPE(i);
                break;
            }
        } else if (n.name == "Name")
            n.getValue(name);
        else if (n.name == "Parent")
            n.getValue(parent_id);
        else if (n.name == "Removed")
            removed(n.asBool1());
        else if (n.name == "Publish")
            publish(n.asBool1());
        else if (n.name == "PublishMobile")
            publishMobile(n.asBool1());
        else if (n.name == "NameTime")
            name_time.text(n);
        else if (n.name == "ParentTime")
            parent_time.text(n);
        else if (n.name == "RemovedTime")
            removed_time.text(n);
        else if (n.name == "PublishTime")
            publish_time.text(n);
        else if (n.name == "Data")
            data_node = &n; // remember for later, because to load data, first we must know the type
    }
    if (!type) {
        error = S + "Element \"" + node.name + "\" has no type";
        return false;
    }
    if (data_node)
        if (ElmData *data = Data())
            data->load(ConstCast(data_node->nodes)); /*else return false; we can silently ignore this*/
    return true;
}
