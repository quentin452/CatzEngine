...
bool SafeCopy(C Str &src, C Str &dest) {
    File f;
    return f.readStd(src) && EE::SafeOverwrite(f, dest, &NoTemp(FileInfoSystem(src).modify_time_utc), null, S + "@new" + Random());
}
/******************************************************************************/
void RemoveChunk(C Str &file, C Str &chunk, ReadWriteSync &rws) {
    ...
    if (src.read(file)) {
        ...
        cw.endChunk();
        temp.pos(0);
        SafeOverwriteChunk(temp, file, rws);
    }
}
/******************************************************************************/
void SavedImage(C Str &name) {
    if (ImagePtr e = ImagePtr().find(name))
        if (!IsServer)
            e->load(name);
} // on server the file may be compressed
void SavedImageAtlas(C Str &name) {
    if (ImageAtlasPtr e = ImageAtlasPtr().find(name))
        if (!IsServer)
            e->load(name);
}
void SavedEditSkel(C Str &name) {}
void SavedSkel(C Str &name) {
    if (Skeleton *e = Skeletons.find(name))
        if (!IsServer)
            e->load(name);
}
void SavedAnim(C Str &name) {
    if (Animation *e = Animations.find(name))
        if (!IsServer)
            e->load(name);
}
void SavedMesh(C Str &name) {
    if (MeshPtr e = MeshPtr().find(name))
        if (!IsServer) {
            CacheLock cl(Meshes);
            e->load(name);
        }
...
void SavedMtrl(C Str &name) {
    if (MaterialPtr e = MaterialPtr().find(name))
        if (!IsServer)
            e->load(name);
}
void SavedWaterMtrl(C Str &name) {
    if (WaterMtrlPtr e = WaterMtrlPtr().find(name))
        if (!IsServer)
            e->load(name);
}
void SavedPhysMtrl(C Str &name) {
    if (PhysMtrl *e = PhysMtrls.find(name))
        if (!IsServer)
            e->load(name);
}
void SavedEditPhys(C Str &name) {}
void SavedPhys(C Str &name) {
    if (PhysBodyPtr e = PhysBodyPtr().find(name))
        if (!IsServer) {
...
void SavedEnum(C Str &name) {
    if (Enum *e = Enums.find(name))
        if (!IsServer)
            e->load(name);
}
void SavedEditEnum(C Str &name) {}
void SavedEditObjPar(C Str &name) {
    if (EditObjectPtr e = EditObjectPtr().find(name))
        if (!IsServer)
            e->load(name);
}
void SavedGameObjPar(C Str &name) {
    if (ObjectPtr e = ObjectPtr().find(name))
        if (!IsServer) {
            CacheLock cl(Objects);
            e->load(name);
        }
}
void SavedGameWayp(C Str &name) {
    if (Game::Waypoint *e = Game::Waypoints.find(name))
        if (!IsServer)
            e->load(name);
}
void SavedFont(C Str &name) {
    if (FontPtr e = FontPtr().find(name))
        if (!IsServer)
            e->load(name);
}
void SavedTextStyle(C Str &name) {
    if (TextStylePtr e = TextStylePtr().find(name))
        if (!IsServer)
            e->load(name);
}
void SavedPanelImage(C Str &name) {
    if (PanelImagePtr e = PanelImagePtr().find(name))
        if (!IsServer)
            e->load(name);
}
void SavedPanel(C Str &name) {
    if (PanelPtr e = PanelPtr().find(name))
        if (!IsServer)
            e->load(name);
}
void SavedGuiSkin(C Str &name) {
    if (GuiSkinPtr e = GuiSkinPtr().find(name))
        if (!IsServer)
            e->load(name);
}
void SavedGui(C Str &name) {}
void SavedEnv(C Str &name) {
    if (EnvironmentPtr e = EnvironmentPtr().find(name))
        if (!IsServer)
            e->load(name);
}
...
void Save(C Animation &anim, C Str &name) {
    File f;
    anim.save(f.writeMem());
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(anim, name);
}
void Save(C Mesh &mesh, C Str &name, C Str &resource_path) {
    File f;
    mesh.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    CacheLock cl(Meshes);
    if (SafeOverwrite(f, name))
        Saved(mesh, name);
...
void Save(C Material &mtrl, C Str &name, C Str &resource_path) {
    File f;
    mtrl.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(mtrl, name);
}
void Save(C WaterMtrl &mtrl, C Str &name, C Str &resource_path) {
    File f;
    mtrl.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    if (SafeOverwrite(f, name))
...
void Save(C PhysBody &phys, C Str &name, C Str &resource_path) {
    File f;
    phys.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    CacheLock cl(PhysBodies);
    if (SafeOverwrite(f, name))
        Saved(phys, name);
...
void Save(C EditEnums &enms, C Str &name) {
    File f;
    enms.save(f.writeMem());
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(enms, name);
}
void Save(C EditObject &obj, C Str &name, C Str &resource_path) {
    File f;
    obj.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(obj, name);
}
void Save(C Object &obj, C Str &name, C Str &resource_path) {
    File f;
    obj.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    CacheLock cl(Objects);
...
void Save(C TextStyle &ts, C Str &name, C Str &resource_path) {
    File f;
    ts.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(ts, name);
}
...
void Save(C EditPanel &panl, C Str &name) {
    File f;
    panl.save(f.writeMem());
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(panl, name);
}
void Save(C Panel &panl, C Str &name, C Str &resource_path) {
    File f;
    panl.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(panl, name);
}
void Save(C EditGuiSkin &skin, C Str &name) {
    File f;
    skin.save(f.writeMem());
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(skin, name);
}
void Save(C GuiSkin &skin, C Str &name, C Str &resource_path) {
    File f;
    skin.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(skin, name);
}
void Save(C GuiObjs &gui, C Str &name, C Str &resource_path) {
    File f;
    gui.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(gui, name);
}
void Save(C Lake &lake, C Str &name) {
    File f;
    lake.save(f.writeMem());
    f.pos(0);
    if (SafeOverwrite(f, name, WorldAreaSync))
        Saved(lake, name);
}
void Save(C River &rivr, C Str &name) {
    File f;
    rivr.save(f.writeMem());
    f.pos(0);
    if (SafeOverwrite(f, name, WorldAreaSync))
        Saved(rivr, name);
}
...
void Save(C Environment &env, C Str &name, C Str &resource_path) {
    File f;
    env.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(env, name);
}
void Save(C Game::WorldSettings &s, C Str &name, C Str &resource_path) {
    File f;
    s.save(f.writeMem(), resource_path.is() ? resource_path : GetPath(name));
    f.pos(0);
    if (SafeOverwrite(f, name))
        Saved(s, name);
}

bool Load(Mesh &mesh, C Str &name, C Str &resource_path) {
    File f;
    if (f.read(name))
        return mesh.load(f, resource_path.is() ? resource_path : GetPath(name));
    mesh.del();
    return false;
}
bool Load(Material &mtrl, C Str &name, C Str &resource_path) {
    File f;
    if (f.read(name))
        return mtrl.load(f, resource_path.is() ? resource_path : GetPath(name));
    mtrl.reset();
    return false;
}
bool Load(WaterMtrl &mtrl, C Str &name, C Str &resource_path) {
    File f;
    if (f.read(name))
        return mtrl.load(f, resource_path.is() ? resource_path : GetPath(name));
    mtrl.reset();
...
bool Load(EditObject &obj, C Str &name, C Str &resource_path) {
    File f;
    if (f.read(name))
        return obj.load(f, resource_path.is() ? resource_path : GetPath(name));
    obj.del();
    return false;
}
...
Edit::ERROR_TYPE LoadCode(Str &code, C Str &name) {
    ...
    if (f.read(name)) {
        f.getAll(code);
        return f.ok() ? Edit::EE_ERR_NONE : Edit::EE_ERR_FILE_READ_ERROR;
    }
    code.clear();
    return Edit::EE_ERR_FILE_NOT_FOUND;
}
...
void SavedEdit(ELM_TYPE type, C Str &path) // called when saved the edit version
{
    ...
    case ELM_MTRL:
        ...
        break;
    case ELM_WATER_MTRL:
        SavedEditWaterMtrl(path);
        break;
    case ELM_PHYS_MTRL:
        SavedEditPhysMtrl(path);
        break;
    }
}
...
void MAX1(TimeStamp &time, C TimeStamp &src_time) {
    if (src_time > time)
        time = src_time;
    time++;
} // set as max from both and increase by one, "time=Max(time, src_time)+1"
...
Str GetCubeFile(C Str &files, int face) {
    ...
    return (faces.elms() == 1) ? files : InRange(face, faces) ? faces[face].encode()
                                                              : S;
}
Str SetCubeFile(Str files, int face, C Str &file) // put 'file' into specified 'face' and return all files
{
    if (InRange(face, 6)) {
        ...
        if (faces.elms() != 6)
            faces.clear();                 // if invalid number then clear
...
bool SetFullAlpha(Image &image, IMAGE_TYPE dest_type) // returns if any change was made
{
    if (NeedFullAlpha(image, dest_type)) {
        if (image.compressed())
            return image.copy(image, -1, -1, -1, ImageTypeExcludeAlpha(ImageTypeUncompressed(image.type())), IMAGE_SOFT, 1);
        if (image.lock()) {
            REPD(y, image.h())
            REPD(x, image.w()) {
...
void ImageProps(C Image &image, UID *hash, IMAGE_TYPE *best_type, uint flags, Edit::Material::TEX_QUALITY quality) // calculate image hash and best type for image compression
{
    if (hash || best_type) {
        if (image.is()) {
            ...
            if (flags & WATER_MTRL) {
                if (flags & MTRL_BASE_0) {
                    MAX(quality, MinMtrlTexQualityBase0);
                    flags |= SRGB;
                } // #MaterialTextureLayoutWater
                if (flags & MTRL_BASE_1) {
                    MAX(quality, MinMtrlTexQualityBase1);
                    sign = true;
                    type = ((quality >= Edit::Material::FULL) ? IMAGE_R8G8_SIGN : IMAGE_BC5_SIGN);
                } // normal tex always uses BC5_SIGN (RG HQ) #MaterialTextureLayoutWater
                if (flags & MTRL_BASE_2) {
                    MAX(quality, MinMtrlTexQualityBase2);
                    sign = true;
                    type = ((quality >= Edit::Material::FULL) ? IMAGE_R8_SIGN : IMAGE_BC4_SIGN);
                } // bump   tex always uses BC4_SIGN (R  HQ) #MaterialTextureLayoutWater
            } else {
                if (flags & MTRL_BASE_0) {
                    MAX(quality, MinMtrlTexQualityBase0);
                    flags |= SRGB;
                ...
                if (flags & MTRL_BASE_1) {
                    ...
                    sign = true;
                    type = ((quality >= Edit::Material::FULL) ? IMAGE_R8G8_SIGN : IMAGE_BC5_SIGN);
                } // normal tex always uses BC5_SIGN (RG HQ) #MaterialTextureLayout
                if (flags & MTRL_BASE_2) {
                    MAX(quality, MinMtrlTexQualityBase2);
                } // #MaterialTextureLayout
            }
            if (flags & MTRL_DETAIL) {
                MAX(quality, MinMtrlTexQualityDetail);
            } // #MaterialTextureLayoutDetail
            if (flags & MTRL_MACRO) {
                MAX(quality, MinMtrlTexQualityMacro);
                flags |= SRGB | IGNORE_ALPHA;
            }
            if (flags & MTRL_EMISSIVE) {
                MAX(quality, MinMtrlTexQualityLight);
                flags |= SRGB | IGNORE_ALPHA;
            }
            ...
            Image temp;
            C Image *src = (extract ? &temp : &image);
            Color min(255, 255), max(0, 0);
            FREPD(face, image.faces()) {
                int src_face = face;
                if (extract)
                    if (image.extractMipMap(temp, sign ? IMAGE_R8G8B8A8_SIGN : image.sRGB() ? IMAGE_R8G8B8A8_SRGB
                                                                                            : IMAGE_R8G8B8A8,
                                            0, DIR_ENUM(face)))
                        src_face = 0; // !! RGBA is needed because below we use 'pixC' !!
                    else
                        goto error;
                if (src->lockRead(0, DIR_ENUM(src_face))) {
                    ...
                    if (hash) {
                        ...
                        {
                            if (src->pitch() == pitch)
                                h.update(src->data() + z * src->pitch2(), pitch2); // if don't have any extra padding then we can update hash for all Y's in one go
                            else
                                FREPD(y, src->h())
                            h.update(src->data() + y * src->pitch() + z * src->pitch2(), pitch); // process in order
                        }
                    }
                    if (calc_type)
                        REPD(z, src->d())
            ...
            if (calc_type) {
                if (min.a < 254)
                    bc1 = false; // BC1 supports only           A=255 #BC1RGB
                if (max.g > 1 || max.b > 1 || min.a < 254)
                    bc4 = false; // BC4 supports only G=0, B=0, A=255
                if (max.b > 1 || min.a < 254)
                    bc5 = false; // BC5 supports only      B=0, A=255
                if ((flags & MTRL_BASE_2) && !(flags & WATER_MTRL) && max.b - min.b > 1)
                    MAX(quality, Edit::Material::HIGH); // if this is Base2 Ext and have bump map #MaterialTextureLayout then disable MEDIUM quality BC1 and use HIGH quality BC7

                if (quality >= Edit::Material::FULL) {
                    if (srgb || min.a < 254 || max.b > 1)
...
void LoadTexture(C Project &proj, C UID &tex_id, Image &image) {
    ...
    if (src)
        src->copy(image, -1, -1, -1, ImageTypeUncompressed(src->type()), IMAGE_SOFT, 1);
    else
        image.del(); // always copy, because: src texture will always be compressed, also soft doesn't require locking
}
void ExtractBaseTextures(C Project &proj, C UID &base_0, C UID &base_1, C UID &base_2, Image *color, Image *alpha, Image *bump, Image *normal, Image *smooth, Image *metal, Image *glow) { // #MaterialTextureLayout
    TEX_FLAG tex = TEXF_NONE;
    if (base_0.valid() && (color || alpha)) {
        Image b0;
        LoadTexture(proj, base_0, b0);
        tex |= ExtractBase0Texture(b0, color, alpha);
    }
    if (base_1.valid() && (normal)) {
        Image b1;
        LoadTexture(proj, base_1, b1);
        tex |= ExtractBase1Texture(b1, normal);
    }
    if (base_2.valid() && (bump || smooth || metal || glow)) {
        Image b2;
        LoadTexture(proj, base_2, b2);
        tex |= ExtractBase2Texture(b2, bump, smooth, metal, glow);
    }
    if (color && !(tex & TEXF_COLOR))
        color->del();
    if (alpha && !(tex & TEXF_ALPHA))
        alpha->del();
    if (bump && !(tex & TEXF_BUMP))
        bump->del();
    if (normal && !(tex & TEXF_NORMAL))
        normal->del();
    if (smooth && !(tex & TEXF_SMOOTH))
        smooth->del();
    if (metal && !(tex & TEXF_METAL))
        metal->del();
    if (glow && !(tex & TEXF_GLOW))
        glow->del();
}
void ExtractWaterBaseTextures(C Project &proj, C UID &base_0, C UID &base_1, C UID &base_2, Image *color, Image *alpha, Image *bump, Image *normal, Image *smooth, Image *reflect, Image *glow) { // #MaterialTextureLayoutWater
    TEX_FLAG tex = TEXF_NONE;
    if (base_0.valid() && color) {
        Image b0;
        LoadTexture(proj, base_0, b0);
        tex |= ExtractWaterBase0Texture(b0, color);
    }
    if (base_1.valid() && normal) {
        Image b1;
        LoadTexture(proj, base_1, b1);
        tex |= ExtractWaterBase1Texture(b1, normal);
    }
    if (base_2.valid() && bump) {
        Image b2;
        LoadTexture(proj, base_2, b2);
        tex |= ExtractWaterBase2Texture(b2, bump);
    }
    if (color && !(tex & TEXF_COLOR))
        color->del();
    if (alpha && !(tex & TEXF_ALPHA))
    ...
    if (smooth && !(tex & TEXF_SMOOTH))
        smooth->del();
    if (reflect && !(tex & TEXF_METAL))
        reflect->del();
    if (glow && !(tex & TEXF_GLOW))
        glow->del();
}
void ExtractDetailTextures(C Project &proj, C UID &detail_tex, Image *color, Image *bump, Image *normal, Image *smooth) { // #MaterialTextureLayoutDetail
    TEX_FLAG tex = TEXF_NONE;
    if (detail_tex.valid())
        if (color || bump || normal || smooth) {
            Image detail;
            LoadTexture(proj, detail_tex, detail);
            tex = ExtractDetailTexture(detail, color, bump, normal, smooth);
        }
    if (color && !(tex & TEXF_DET_COLOR))
        color->del();
    if (bump && !(tex & TEXF_DET_BUMP))
        bump->del();
    if (normal && !(tex & TEXF_DET_NORMAL))
        normal->del();
    if (smooth && !(tex & TEXF_DET_SMOOTH))
        smooth->del();
}
...
VecI ImageSize(C VecI &src, C VecI2 &custom, bool pow2) {
    ...
    if (custom.y > 0)
        size.y = custom.y;
    if (!custom.x && custom.y > 0 && src.y)
        size.x = Max(1, (src.x * custom.y + src.y / 2) / src.y); // keep aspect ratio
    if (!custom.y && custom.x > 0 && src.x)
        size.y = Max(1, (src.y * custom.x + src.x / 2) / src.x); // keep aspect ratio
    if (pow2)
        size.set(NearestPow2(size.x), NearestPow2(size.y), NearestPow2(size.z));
    return size;
}
VecI2 GetSize(C Str &name, C Str &value, C VecI &src, C VecI &background_size) {
    VecI2 size;
    if (value == "quarter")
        size.set(Max(1, src.x / 4), Max(1, src.y / 4));
    else if (value == "half")
        size.set(Max(1, src.x / 2), Max(1, src.y / 2));
    else if (value == "double")
        size = src.xy * 2;
    else if (value == "full")
        size = background_size.xy;
    else {
        Vec2 sf;
        if (Contains(value, ','))
            sf = TextVec2(value);
...
int GetFilterI(C Str &name) {
    if (Contains(name, "nearest") || Contains(name, "point") || Contains(name, "FilterNone"))
        return FILTER_NONE;
    if (Contains(name, "linear"))
        return FILTER_LINEAR;
    if (Contains(name, "cubic+") || Contains(name, "cubicPlus"))
        return FILTER_CUBIC_PLUS; // !! check this before "cubic" !!
    if (Contains(name, "cubic"))
        return FILTER_CUBIC_FAST;
    if (Contains(name, "cubicMed"))
        return FILTER_CUBIC_FAST_MED;
    if (Contains(name, "cubicSharp"))
        return FILTER_CUBIC_FAST_SHARP;
    if (Contains(name, "Waifu"))
        return FILTER_WAIFU;
    if (Contains(name, "EASU"))
        return FILTER_EASU;
    if (Contains(name, "NoStretch"))
        return FILTER_NO_STRETCH;
    return -1;
}
FILTER_TYPE GetFilter(C Str &name) {
    int i = GetFilterI(name);
    return InRange(i, FILTER_NUM) ? FILTER_TYPE(i) : FILTER_BEST;
}
bool GetClampWrap(C Str &name, bool default_clamp) {
    if (Contains(name, "clamp"))
        return true; // clamp=1
    if (Contains(name, "wrap"))
        return false;     // clamp=0
...
bool EditToGameImage(Image &edit, Image &game, bool pow2, bool srgb, bool alpha_lum, ElmImage::TYPE type, int mode, int mip_maps, bool has_color, bool has_alpha, bool ignore_alpha, bool env, C VecI2 &custom_size, C int *force_type) {
    ...
    if (!edit.cube() && IsCube((IMAGE_MODE)mode))
        switch (edit.cubeLayout()) {
        case CUBE_LAYOUT_CROSS:
            size.x /= 4;
            size.y /= 3;
            break;
        case CUBE_LAYOUT_6x1:
            size.x /= 6;
            break;
        }
    ...
    if (force_type) {
        if (*force_type == IMAGE_NONE || *force_type < 0)
            force_type = null;
    }
    if (alpha_lum) {
        if (&edit != &game) {
    ...
    {
        if (mip_maps < 0)
            mip_maps = ((src->mipMaps() == 1) ? 1 : 0); // source will have now only one mip-map so we can't use "-1", auto-detect instead
        if (mode < 0)
            mode = src->mode(); // source will now be as IMAGE_SOFT      so we can't use "-1", auto-detect instead
        if (src->copy(temp, -1, -1, -1, IMAGE_R8G8B8_SRGB, IMAGE_SOFT, 1))
            src = &temp;
    }

    IMAGE_TYPE dest_type;
    if (force_type)
        dest_type = IMAGE_TYPE(*force_type);
    else if (type == ElmImage::ALPHA)
        dest_type = IMAGE_A8;
    else if (type == ElmImage::FULL)
        dest_type = (has_color ? (srgb ? IMAGE_R8G8B8A8_SRGB : IMAGE_R8G8B8A8) : has_alpha ? (srgb ? IMAGE_L8A8_SRGB : IMAGE_L8A8)
                                                                                           : (srgb ? IMAGE_L8_SRGB : IMAGE_L8));
    else
        ImageProps(*src, null, &dest_type, (srgb ? SRGB : 0) | (ignore_alpha ? IGNORE_ALPHA : 0), (type == ElmImage::FULL) ? Edit::Material::FULL : (type == ElmImage::COMPRESSED2) ? Edit::Material::HIGH
                                                                                                                                                                                    : Edit::Material::MEDIUM);

    if ((src->type() == IMAGE_L8 || src->type() == IMAGE_L8_SRGB) && dest_type == IMAGE_A8 || src->type() == IMAGE_A8 && (dest_type == IMAGE_L8 || dest_type == IMAGE_L8_SRGB)) {
        Image temp2;
        if (temp2.createSoft(src->w(), src->h(), src->d(), dest_type) && src->lockRead()) {
            REPD(z, temp2.d())
            REPD(y, temp2.h())
            REPD(x, temp2.w())
            temp2.pixel3D(x, y, z, src->pixel3D(x, y, z));
            src->unlock();
            Swap(temp, temp2);
            src = &temp;
        }
...
void DrawPanelImage(C PanelImage &pi, C Rect &rect, bool draw_lines) {
    ...
    Rect r = rect;
    r.extend(-size / 3);
    Rect_LU lu(r.lu(), size);
    Rect_RU ru(r.ru(), size.x * 3, size.y);
    Rect_LD ld(r.ld(), size.x, size.y * 3);
    Rect_RD rd(r.rd(), size.x * 3, size.y * 3);
    pi.draw(lu);
    pi.draw(ru);
    pi.draw(ld);
    pi.draw(rd);
    if (draw_lines) {
        pi.drawScaledLines(RED, lu);
        pi.drawScaledLines(RED, ru);
        pi.drawScaledLines(RED, ld);
        pi.drawScaledLines(RED, rd);
        lu.draw(AZURE, false);
        ru.draw(AZURE, false);
        ld.draw(AZURE, false);
        rd.draw(AZURE, false);
...
bool UpdateMtrlBase1Tex(C Image &src, Image &dest) {
    Image temp;
    if (src.copy(temp, -1, -1, -1, IMAGE_R8G8B8A8, IMAGE_SOFT, 1)) {
...
bool ImportImage(Image &image, C Str &name, int type, int mode, int mip_maps, bool decompress) {
    if (image.Import(name, type, mode, mip_maps)) {
        if (image.compressed() && decompress && !image.copy(image, -1, -1, -1, ImageTypeUncompressed(image.type())))
            return false;
        return true;
    }
    /*if(name.is())
...
bool NonMonoTransform(C TextParam &p)                                                                                                                                                                                                       // if can change a mono image to non-mono, this is NOT the same as "!MonoTransform"
{
    ...
    return p.name == "inverseR" || p.name == "inverseG" || p.name == "inverseRG" || p.name == "lerpRGB" && values > 2 || p.name == "iLerpRGB" && values > 2 || p.name == "mulRGB" && TextVecEx(p.value).anyDifferent() || p.name == "mulRGBLin" && TextVecEx(p.value).anyDifferent() || p.name == "addRGB" && TextVecEx(p.value).anyDifferent() || p.name == "setRGB" && TextVecEx(p.value).anyDifferent() || p.name == "setRGBA" && p.asVec4().xyz.anyDifferent() || p.name == "mulAddRGB" && values > 2 || p.name == "addMulRGB" && values > 2 || p.name == "mulRGBIS" && TextVecEx(p.value).anyDifferent() || p.name == "mulRGBS" && TextVecEx(p.value).anyDifferent() || p.name == "mulRGBH" && values > 1 || p.name == "mulRGBHS" && values > 1 || p.name == "gamma" && TextVecEx(p.value).anyDifferent() || p.name == "brightness" && TextVecEx(p.value).anyDifferent() || p.name == "sigmoid" && TextVecEx(p.value).anyDifferent() || p.name == "contrast" && TextVecEx(p.value).anyDifferent() || p.name == "contrastAlphaWeight" && TextVecEx(p.value).anyDifferent() || p.name == "addSat" || p.name == "addSatPhoto" || p.name == "setSat" || p.name == "setSatPhoto" || p.name == "mulAddSat" || p.name == "mulAddSatPhoto" || p.name == "addHueSat" || p.name == "setHueSat" || p.name == "setHueSatPhoto" || p.name == "lerpHueSat" || p.name == "rollHueSat" || p.name == "rollHueSatPhoto" || p.name == "min" && TextVecEx(p.value).anyDifferent() || p.name == "max" && TextVecEx(p.value).anyDifferent() || p.name == "channel" && !ChannelMonoTransform(p.value) || p.name == "scaleXY" && TextVec2Ex(p.value).anyDifferent() || p.name == "bumpToNormal";
}
bool HighPrecTransform(C Str &name) {
    return ResizeTransform(name) || name == "mulRGB" || name == "mulRGBLin" || name == "addRGB" || name == "setRGB" || name == "setRGBA" || name == "mulAddRGB" || name == "addMulRGB" || name == "mulA" || name == "mulRGBbyA" || name == "mulRGBS" || name == "mulRGBIS" || name == "mulRGBH" || name == "mulRGBHS" || name == "normalize" || name == "scale" || name == "scaleXY" || name == "lerpRGB" || name == "iLerpRGB" || name == "lerpLum" || name == "iLerpLum" || name == "blur" || name == "sharpen" || name == "bump" || name == "bumpClamp" || name == "contrast" || name == "contrastLum" || name == "contrastAlphaWeight" || name == "contrastLumAlphaWeight" || name == "brightness" || name == "brightnessLum" || name == "brightnessLumS" || name == "sigmoid" || name == "sigmoidLum" || name == "gamma" || name == "gammaA" || name == "gammaLum" || name == "gammaLumPhoto" || name == "gammaSat" || name == "SRGBToLinear" || name == "LinearToSRGB" || name == "greyPhoto" || name == "avgLum" || name == "medLum" || name == "avgContrastLum" || name == "medContrastLum" || name == "avgHue" || name == "avgHuePhoto" || name == "avgHueAlphaWeight" || name == "avgHuePhotoAlphaWeight" || name == "medHue" || name == "medHueAlphaWeight" || name == "medHuePhoto" || name == "medHuePhotoAlphaWeight" || name == "addHue" || name == "addHuePhoto" || name == "setHue" || name == "setHuePhoto" || name == "contrastHue" || name == "contrastHuePhoto" || name == "medContrastHue" || name == "medContrastHuePhoto" || name == "contrastHueAlphaWeight" || name == "contrastHuePhotoAlphaWeight" || name == "contrastHuePow" || name == "lerpHue" || name == "lerpHueSat" || name == "rollHue" || name == "rollHueSat" || name == "lerpHuePhoto" || name == "lerpHueSatPhoto" || name == "rollHuePhoto" || name == "rollHueSatPhoto" || name == "addSat" || name == "addSatPhoto" || name == "mulSat" || name == "mulSatPhoto" || name == "mulAddSat" || name == "mulAddSatPhoto" || name == "avgSat" || name == "avgSatPhoto" || name == "medSat" || name == "medSatPhoto" || name == "contrastSat" || name == "contrastSatPhoto" || name == "medContrastSat" || name == "contrastSatAlphaWeight" || name == "contrastSatPhotoAlphaWeight" || name == "minSat" || name == "minSatPhoto" || name == "setSat" || name == "setSatPhoto" || name == "addHueSat" || name == "setHueSat" || name == "setHueSatPhoto" || name == "mulSatH" || name == "mulSatHS" || name == "mulSatHPhoto" || name == "mulSatHSPhoto"
        //|| name=="metalToReflect"
        ;
}
bool SizeDependentTransform(C TextParam &p) {
    return p.name == "blur"                                                         // range depends on size
           ...
           || p.name == "resizeNoStretch" || p.name == "rotate" || p.name == "tile" // tile range depends on size
...
bool ForcesMono(C Str &file) {
    ...
    REPA(files) // go from end
    {
        ...
        REPA(file.params) // go from end
        {
            ...
            if (MonoTransform(param) && !PartialTransform(param))
                return true; // if current transform generates mono (fully) and no non-mono were encountered up to now, then result is mono
            if (NonMonoTransform(param))
                return false; // if there's at least one transform that can generate color then result is not mono
        }
    }
    return false;
}
Str BumpFromColTransform(C Str &color_map, int blur) // 'blur'<0 = empty (default)
{
    ...
    REPA(files) // go from end
    {
        ...
        REPA(file.params) // go from end
        {
            ...
            if (p.name != "size" && p.name != "crop" && p.name != "trim" && p.name != "extend" && p.name != "resizeNoStretch" && p.name != "swapXY" && p.name != "mirrorXY" && p.name != "mirrorX" && p.name != "mirrorY" && p.name != "flipXY" && p.name != "flipX" && p.name != "flipY")
                file.params.remove(i, true); // allow only these transforms
        }
        if (!file.is())
    ...
    REPA(files) // go from end
    {
        ...
        REPAD(pi, file.params) // go from end
        {
            ...
            if (ResizeTransform(p.name)) {
                ...
                return false; // if encountered a size dependent transform, it means we can't keep looking
        }
    }
    return false;
}
bool ExtractLinearTransform(MemPtr<FileParams> files, Vec &mul, Vec &add) {
    REPA(files) // go from end
    {
        ...
        {
            C TextParam &p = file.params[pi];
            if (PartialTransform(p))
                goto none;
            if (LinearTransform(p.name)) {
                if (p.name == "mulRGB") {
                    mul = TextVecEx(p.value);
                    add = 0;
                } else if (p.name == "addRGB") {
                    add = TextVecEx(p.value);
                    mul = 1;
                } else if (p.name == "mulAddRGB") {
                    if (!TextVecVecEx(p.value, mul, add))
                        goto none;
                } else
                    goto none;
                file.params.remove(pi, true); // remove it
            ...
            if (!ResizeTransform(p.name))
                goto none; // allow continue only on resize
        }
    }
none:
    mul = 1;
...
void AdjustImage(Image &image, bool rgb, bool alpha, bool high_prec) {
    ...
    if (rgb)
        type = ImageTypeIncludeRGB(type);
    if (alpha)
        type = ImageTypeIncludeAlpha(type);
    if (high_prec)
        type = ImageTypeHighPrec(type);
    if (type != image.type()) {
...
void AvgContrastLum(Image &image, flt contrast, dbl avg_lum, C BoxI &box) {
    if (avg_lum && image.lock()) // lock for writing because we will use this lock for applying contrast too
    {
        ...
        if (weight_total)
            if (contrast_total /= weight_total)
                ContrastLum(image, contrast / contrast_total, avg_lum, box);
        image.unlock();
    }
}
    ...
    for (int z = box.min.z; z < box.max.z; z++)
        for (int y = box.min.y; y < box.max.y; y++)
            for (int x = box.min.x; x < box.max.x; x++) {
                ...
                if (sat)
                    sat_mul = Lerp(1.0f, sat_mul, hsb.y);
                hsb.y *= sat_mul;
                c.xyz = HsbToRgb(hsb);
                if (photo) {
                    //c.xyz=SRGBToLinear(c.xyz); if(flt cur_lin_lum=LinearLumOfLinearColor(c.xyz))c.xyz*=lin_lum/cur_lin_lum; c.xyz=LinearToSRGB(c.xyz);
...
flt _ApplyBrightness(flt x, flt brightness) // !! ASSUMES THAT "brightness!=0" !!
{
    ...
    if (brightness < 0)
        x = SigmoidSqrInv(x * SigmoidSqr(brightness)) / brightness;
    else
...
flt _ApplyContrast(flt x, flt contrast) // !! ASSUMES THAT "contrast!=0" !!
{
    ...
    x = x * 2 - 1;
    if (contrast < 0)
        x = SigmoidSqrInv(x * SigmoidSqr(contrast)) / contrast;
    else
        x = SigmoidSqr(x * contrast) / SigmoidSqr(contrast);
    x = x * 0.5f + 0.5f;
    return ContrastToSRGB(x);
}
void ApplyBrightness(flt &x, flt brightness) {
    if (x > 0 && x < 1 && brightness)
        x = _ApplyBrightness(x, brightness);
}
void ApplyContrast(flt &x, flt contrast) {
    if (x > 0 && x < 1 && contrast)
        x = _ApplyContrast(x, contrast);
}

void Crop(Image &image, int x, int y, int w, int h, C Color &background, bool hp) {
    Vec4 clear_color = background;
    if (image.is())
        image.crop(image, x, y, w, h, &clear_color);
    else if (image.createSoft(w, h, 1, hp ? IMAGE_F32_4_SRGB : IMAGE_R8G8B8A8_SRGB))
        REPD(y, h)
    REPD(x, w)
    image.colorF(x, y, clear_color);
}
...
void TransformImage(Image &image, TextParam param, bool clamp, C Color &background, C VecI &background_size) {
    ...
    if (HighPrecTransform(param.name))
        AdjustImage(image, false, false, true); // if transform might generate high precision values then make sure we can store them

    if (param.name == "size") {
        VecI2 v = TextVecI2Ex(param.value);
        Crop(image, 0, 0, v.x, v.y, background);
    } else if (param.name == "crop") {
        VecI4 v = TextVecI4(param.value);
        Crop(image, v.x, v.y, v.z, v.w, background);
    } else if (param.name == "trim") {
        VecI4 v;
        Memc<Str> c;
        Split(c, param.value, ',');
        switch (c.elms()) {
        case 1:
            v = TextInt(param.value);
            break;
        case 2:
            v.xy = v.zw = TextVecI2(param.value);
            break;
        case 4:
            v = TextVecI4(param.value);
            break;
        default:
            v = 0;
            break;
        ...
        Crop(image, -v.x, -v.y, image.w() + v.x + v.z, image.h() + v.y + v.w, background);
    } else if (ResizeTransformAny(param.name)) {
    ...
    } else if (Starts(param.name, "rotateScale")) {
        Vec2 rs = param.asVec2();
        image.rotateScale(image, DegToRad(rs.x), rs.y, GetFilter(param.name));
    } else if (Starts(param.name, "rotate"))
        image.rotate(image, DegToRad(param.asFlt()), GetFilter(param.name));
    else if (param.name == "tile")
        image.tile(TextVecI2Ex(param.value));
    else if (param.name == "inverseRGB") {
            ...
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
                    for (int x = box.min.x; x < box.max.x; x++) {
                        ...
                        c.g = 255 - c.g;
                        c.b = 255 - c.b;
                        image.color3D(x, y, z, c);
                    }
        }
    } else if (param.name == "inverseR") {
        if (image.highPrecision()) {
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
                    for (int x = box.min.x; x < box.max.x; x++) {
    ...
    } else if (param.name == "swapXY") {
        Image temp;
        temp.createSoft(image.h(), image.w(), image.d(), image.type());
        if (temp.highPrecision()) {
            REPD(y, image.h())
            REPD(x, image.w())
            temp.colorF(y, x, image.colorF(x, y));
        } else {
            REPD(y, image.h())
            REPD(x, image.w())
            temp.color(y, x, image.color(x, y));
        }
        Swap(temp, image);
    } else if (param.name == "mirrorX" || param.name == "flipX")
        image.mirrorX();
    else if (param.name == "mirrorY" || param.name == "flipY")
        image.mirrorY();
    else if (param.name == "mirrorXY" || param.name == "flipXY")
        image.mirrorXY();
    else if (param.name == "normalize")
        image.normalize(true, true, true, true, &box);
    else if (param.name == "absRGB") {
        for (int z = box.min.z; z < box.max.z; z++)
            for (int y = box.min.y; y < box.max.y; y++)
                for (int x = box.min.x; x < box.max.x; x++) {
                    Vec4 c = image.color3DF(x, y, z);
        ...
        if (box.min.allZero() && box.max == image.size3())
            image.blur(r, clamp);
        else {
            Image temp;
            image.blur(temp, r, clamp);
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
                    for (int x = box.min.x; x < box.max.x; x++)
                        image.color3DF(x, y, z, temp.color3DF(x, y, z));
        }
    } else if (param.name == "sharpen") {
        Memc<Str> c;
        Split(c, param.value, ',');
        if (c.elms() >= 1 && c.elms() <= 2) {
            ...
            if (box.min.allZero() && box.max == image.size3())
                image.sharpen(power, range, clamp);
            else {
                Image temp;
                image.copy(temp);
                temp.sharpen(power, range, clamp);
                for (int z = box.min.z; z < box.max.z; z++)
                    for (int y = box.min.y; y < box.max.y; y++)
                        for (int x = box.min.x; x < box.max.x; x++)
                            image.color3DF(x, y, z, temp.color3DF(x, y, z));
    ...
    } else if (param.name == "lerpRGB") {
        Vec from, to;
        if (TextVecVecEx(param.value, from, to)) {
            Vec2 ma[3] = {LerpToMad(from.x, to.x), LerpToMad(from.y, to.y), LerpToMad(from.z, to.z)};
            AdjustImage(image, true, false, false);
            image.mulAdd(Vec4(ma[0].x, ma[1].x, ma[2].x, 1), Vec4(ma[0].y, ma[1].y, ma[2].y, 0), &box);
        }
    } else if (param.name == "iLerpRGB") {
        Vec from, to;
        if (TextVecVecEx(param.value, from, to)) {
    ...
    } else if (param.name == "mulA") {
        flt alpha = param.asFlt();
        if (alpha != 1) {
            AdjustImage(image, false, true, false);
            image.mulAdd(Vec4(1, 1, 1, alpha), 0, &box);
        }
    } else if (param.name == "mulRGB") {
        Vec4 mul(TextVecEx(param.value), 1);
        if (mul.xyz.anyDifferent())
            AdjustImage(image, true, false, false);
        image.mulAdd(mul, Vec4Zero, &box);
    } else if (param.name == "addRGB") {
        Vec4 add(TextVecEx(param.value), 0);
        if (add.xyz.anyDifferent())
            AdjustImage(image, true, false, false);
        image.mulAdd(1, add, &box);
    } else if (param.name == "setRGB") {
        Vec4 add(TextVecEx(param.value), 0);
        if (add.xyz.anyDifferent())
            AdjustImage(image, true, false, false);
        image.mulAdd(Vec4(0, 0, 0, 1), add, &box);
    } else if (param.name == "setRGBA") {
        Vec4 add = param.asVec4();
        bool rgb = add.xyz.anyDifferent(), alpha = (add.w != 1);
        if (rgb || alpha)
            AdjustImage(image, rgb, alpha, false);
        image.mulAdd(Vec4(0, 0, 0, 1), add, &box);
    } else if (param.name == "mulAddRGB") {
        Vec mul, add;
        if (TextVecVecEx(param.value, mul, add)) {
            AdjustImage(image, true, false, false);
            image.mulAdd(Vec4(mul, 1), Vec4(add, 0), &box);
        }
    } else if (param.name == "addMulRGB") { // x=x*m+a, x=(x+A)*M
        Vec add, mul;
        if (TextVecVecEx(param.value, add, mul)) {
            AdjustImage(image, true, false, false);
            image.mulAdd(Vec4(mul, 1), Vec4(add * mul, 0), &box);
        }
    } else if (param.name == "mulRGBLin") {
    ...
    } else if (param.name == "mulRGBH") {
        Mems<Str> vals;
        Split(vals, param.value, ',');
        switch (vals.elms()) {
        case 1: {
            flt v = TextFlt(vals[0]);
            MulRGBH(image, v, v, v, v, v, v, box);
        } break;
        case 3: {
            Vec v(TextFlt(vals[0]), TextFlt(vals[1]), TextFlt(vals[2]));
            MulRGBH(image, v.x, Avg(v.x, v.y), v.y, Avg(v.y, v.z), v.z, Avg(v.z, v.x), box);
        } break;
        case 6:
            MulRGBH(image, TextFlt(vals[0]), TextFlt(vals[1]), TextFlt(vals[2]), TextFlt(vals[3]), TextFlt(vals[4]), TextFlt(vals[5]), box);
            break;
        }
    } else if (param.name == "mulRGBHS") {
        Mems<Str> vals;
        Split(vals, param.value, ',');
        switch (vals.elms()) {
        case 1: {
            flt v = TextFlt(vals[0]);
            MulRGBHS(image, v, v, v, v, v, v, box);
        } break;
        case 3: {
            Vec v(TextFlt(vals[0]), TextFlt(vals[1]), TextFlt(vals[2]));
            MulRGBHS(image, v.x, Avg(v.x, v.y), v.y, Avg(v.y, v.z), v.z, Avg(v.z, v.x), box);
        } break;
        case 6:
            MulRGBHS(image, TextFlt(vals[0]), TextFlt(vals[1]), TextFlt(vals[2]), TextFlt(vals[3]), TextFlt(vals[4]), TextFlt(vals[5]), box);
            break;
        }
    } else if (param.name == "getSat") {
        for (int z = box.min.z; z < box.max.z; z++)
            for (int y = box.min.y; y < box.max.y; y++)
                for (int x = box.min.x; x < box.max.x; x++) {
                    Vec4 c = image.color3DF(x, y, z);
        ...
        if (g != VecOne) {
            ...
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
                    for (int x = box.min.x; x < box.max.x; x++) {
                        Vec4 c = image.color3DF(x, y, z);
                        c.xyz.set(PowMax(c.x, g.x), PowMax(c.y, g.y), PowMax(c.z, g.z));
                        image.color3DF(x, y, z, c);
                    }
        }
    } else if (param.name == "gammaA") {
        flt g = param.asFlt();
        if (g != 1)
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
        ...
        if (g != 1)
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
                    for (int x = box.min.x; x < box.max.x; x++) {
                        Vec4 c = image.color3DF(x, y, z);
                        if (flt lum = c.xyz.max()) {
                            c.xyz *= PowMax(lum, g) / lum;
                            image.color3DF(x, y, z, c);
                        }
                    }
    } else if (param.name == "gammaLumPhoto") {
        flt g = param.asFlt();
        if (g != 1)
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
    ...
    } else if (param.name == "SRGBToLinear") {
        for (int z = box.min.z; z < box.max.z; z++)
            for (int y = box.min.y; y < box.max.y; y++)
                for (int x = box.min.x; x < box.max.x; x++)
                    image.color3DF(x, y, z, SRGBToLinear(image.color3DF(x, y, z)));
    } else if (param.name == "LinearToSRGB") {
        for (int z = box.min.z; z < box.max.z; z++)
            for (int y = box.min.y; y < box.max.y; y++)
                for (int x = box.min.x; x < box.max.x; x++)
                    image.color3DF(x, y, z, LinearToSRGB(image.color3DF(x, y, z)));
    } else if (param.name == "brightness") {
        Vec bright = TextVecEx(param.value), mul;
        if (bright.any()) {
            if (bright.anyDifferent())
                AdjustImage(image, true, false, false);
            flt (*R)(flt);
            flt (*G)(flt);
            flt (*B)(flt);
            if (!bright.x) {
                ...
                mul.x = 1 / bright.x;
                bright.x = SigmoidSqr(bright.x);
                R = SigmoidSqrInv;
            } else {
                mul.x = 1 / SigmoidSqr(bright.x);
                R = SigmoidSqr;
            }
            if (!bright.y) {
                bright.y = 1;
                ...
                mul.y = 1 / bright.y;
                bright.y = SigmoidSqr(bright.y);
                G = SigmoidSqrInv;
            } else {
                mul.y = 1 / SigmoidSqr(bright.y);
                G = SigmoidSqr;
            }
            if (!bright.z) {
                bright.z = 1;
                ...
                mul.z = 1 / bright.z;
                bright.z = SigmoidSqr(bright.z);
                B = SigmoidSqrInv;
            } else {
                mul.z = 1 / SigmoidSqr(bright.z);
                B = SigmoidSqr;
            }
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
                    for (int x = box.min.x; x < box.max.x; x++) {
                        Vec4 c = image.color3DF(x, y, z);
                        if (c.x > 0 && c.x < 1)
                            c.x = BrightnessToSRGB(R(SRGBToBrightness(c.x) * bright.x) * mul.x);
                        if (c.y > 0 && c.y < 1)
                            c.y = BrightnessToSRGB(G(SRGBToBrightness(c.y) * bright.y) * mul.y);
                        if (c.z > 0 && c.z < 1)
                            c.z = BrightnessToSRGB(B(SRGBToBrightness(c.z) * bright.z) * mul.z);
                        image.color3DF(x, y, z, c);
                    }
        }
    } else if (param.name == "brightnessLum") {
        flt bright = param.asFlt(), mul;
        flt (*f)(flt);
        if (bright) {
            if (bright < 0) {
                mul = 1 / bright;
                bright = SigmoidSqr(bright);
                f = SigmoidSqrInv;
            } else {
                mul = 1 / SigmoidSqr(bright);
                f = SigmoidSqr;
            }
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
    ...
    } else if (param.name == "brightnessLumS") {
    ...
    } else if (param.name == "sigmoid") {
        Vec contrast = TextVecEx(param.value), mul;
        if (contrast.any()) {
            if (contrast.anyDifferent())
                AdjustImage(image, true, false, false);
            flt (*R)(flt);
            flt (*G)(flt);
            flt (*B)(flt);
            if (!contrast.x) {
                ...
                mul.x = 1;
                R = FloatSelf;
            } else if (contrast.x < 0) {
                mul.x = 1 / contrast.x;
                contrast.x = SigmoidSqr(contrast.x);
                R = SigmoidSqrInv;
            } else {
                mul.x = 1 / SigmoidSqr(contrast.x);
                R = SigmoidSqr;
            }
            if (!contrast.y) {
                contrast.y = 1;
                mul.y = 1;
                G = FloatSelf;
            } else if (contrast.y < 0) {
                mul.y = 1 / contrast.y;
                contrast.y = SigmoidSqr(contrast.y);
                G = SigmoidSqrInv;
            } else {
                mul.y = 1 / SigmoidSqr(contrast.y);
                G = SigmoidSqr;
            }
            if (!contrast.z) {
                contrast.z = 1;
                mul.z = 1;
                B = FloatSelf;
            } else if (contrast.z < 0) {
                mul.z = 1 / contrast.z;
                contrast.z = SigmoidSqr(contrast.z);
                B = SigmoidSqrInv;
            } else {
                mul.z = 1 / SigmoidSqr(contrast.z);
                B = SigmoidSqr;
            }
            Vec ofs = contrast;
            contrast *= 2;
            ...
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
                    for (int x = box.min.x; x < box.max.x; x++) {
                        Vec4 c = image.color3DF(x, y, z);
                        if (c.x > 0 && c.x < 1)
                            c.x = ContrastToSRGB(R(SRGBToContrast(c.x) * contrast.x - ofs.x) * mul.x + 0.5f);
                        if (c.y > 0 && c.y < 1)
                            c.y = ContrastToSRGB(G(SRGBToContrast(c.y) * contrast.y - ofs.y) * mul.y + 0.5f);
                        if (c.z > 0 && c.z < 1)
                            c.z = ContrastToSRGB(B(SRGBToContrast(c.z) * contrast.z - ofs.z) * mul.z + 0.5f);
                        image.color3DF(x, y, z, c);
                    }
        }
    } else if (param.name == "sigmoidLum") {
        flt contrast = param.asFlt(), mul;
        flt (*f)(flt);
        if (contrast) {
            if (contrast < 0) {
                mul = 1 / contrast;
                contrast = SigmoidSqr(contrast);
                f = SigmoidSqrInv;
            } else {
                mul = 1 / SigmoidSqr(contrast);
                f = SigmoidSqr;
            }
            flt ofs = contrast;
            contrast *= 2;
        ...
        Vec contrast = TextVecEx(param.value);
        if (contrast != VecOne) {
            Vec4 avg;
            if (image.stats(null, null, &avg, null, null, null, null, &box)) {
                if (contrast.anyDifferent())
                    AdjustImage(image, true, false, false);
    ...
    } else if (param.name == "contrastAlphaWeight") {
        Vec contrast = TextVecEx(param.value);
        if (contrast != VecOne) {
            Vec avg;
            if (image.stats(null, null, null, null, null, &avg, null, &box)) {
                if (contrast.anyDifferent())
                    AdjustImage(image, true, false, false);
                image.mulAdd(Vec4(contrast, 1), Vec4(avg * (Vec(1) - contrast), 0), &box);
            }
        }
    } else if (param.name == "contrastLum") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            Vec4 avg;
            if (image.stats(null, null, &avg, null, null, null, null, &box))
                ContrastLum(image, contrast, avg.xyz.max(), box);
        }
    } else if (param.name == "contrastLumAlphaWeight") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            Vec avg;
            if (image.stats(null, null, null, null, null, &avg, null, &box))
                ContrastLum(image, contrast, avg.max(), box);
        }
    } else if (param.name == "contrastHue") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            Vec4 avg;
            if (image.stats(null, null, &avg, null, null, null, null, &box))
                ContrastHue(image, contrast, avg.xyz, box);
        }
    } else if (param.name == "contrastHuePhoto") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            Vec4 avg;
            if (image.stats(null, null, &avg, null, null, null, null, &box))
                ContrastHue(image, contrast, avg.xyz, box, true);
        }
    } else if (param.name == "medContrastHue") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            Vec4 med;
            if (image.stats(null, null, null, &med, null, null, null, &box))
                ContrastHue(image, contrast, med.xyz, box);
        }
    } else if (param.name == "medContrastHuePhoto") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            Vec4 med;
            if (image.stats(null, null, null, &med, null, null, null, &box))
                ContrastHue(image, contrast, med.xyz, box, true);
        }
    } else if (param.name == "contrastHueAlphaWeight") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            Vec avg;
            if (image.stats(null, null, null, null, null, &avg, null, &box))
                ContrastHue(image, contrast, avg, box);
        }
    } else if (param.name == "contrastHuePhotoAlphaWeight") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            Vec avg;
            if (image.stats(null, null, null, null, null, &avg, null, &box))
                ContrastHue(image, contrast, avg, box, true);
        }
    } else if (param.name == "contrastHuePow") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            Vec4 avg;
            if (image.stats(null, null, &avg, null, null, null, null, &box) && image.lock()) {
    ...
    } else if (param.name == "contrastSat") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            flt avg;
            if (image.statsSat(null, null, &avg, null, null, null, null, &box))
                ContrastSat(image, contrast, avg, box);
        }
    } else if (param.name == "contrastSatPhoto") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            flt avg;
            if (image.statsSat(null, null, &avg, null, null, null, null, &box))
                ContrastSat(image, contrast, avg, box, true);
        }
    } else if (param.name == "medContrastSat") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            flt med;
            if (image.statsSat(null, null, null, &med, null, null, null, &box))
                ContrastSat(image, contrast, med, box);
        }
    } else if (param.name == "contrastSatAlphaWeight") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            flt avg;
            if (image.statsSat(null, null, null, null, null, &avg, null, &box))
                ContrastSat(image, contrast, avg, box);
        }
    } else if (param.name == "contrastSatPhotoAlphaWeight") {
        flt contrast = param.asFlt();
        if (contrast != 1) {
            flt avg;
            if (image.statsSat(null, null, null, null, null, &avg, null, &box))
    ...
    } else if (param.name == "avgLum") {
        Vec4 avg;
        if (image.stats(null, null, &avg, null, null, null, null, &box))
            if (flt avg_l = avg.xyz.max())
                image.mulAdd(Vec4(Vec(param.asFlt() / avg_l), 1), 0, &box);
    } else if (param.name == "medLum") {
        Vec4 med;
        if (image.stats(null, null, null, &med, null, null, null, &box))
            if (flt med_l = med.xyz.max())
                image.mulAdd(Vec4(Vec(param.asFlt() / med_l), 1), 0, &box);
    } else if (param.name == "avgContrastLum") {
        Vec4 avg;
        if (image.stats(null, null, &avg, null, null, null, null, &box))
            AvgContrastLum(image, param.asFlt(), avg.xyz.max(), box);
    } else if (param.name == "medContrastLum") {
        Vec4 med;
        if (image.stats(null, null, null, &med, null, null, null, &box))
            AvgContrastLum(image, param.asFlt(), med.xyz.max(), box);
    } else if (param.name == "addHue")
        AddHue(image, param.asFlt(), box);
    else if (param.name == "addHuePhoto")
        AddHue(image, param.asFlt(), box, true);
    else if (param.name == "avgHue" || param.name == "avgHuePhoto") {
        if (image.lock()) // lock for writing because we will use this lock for applying hue too
        {
            Vec4 col;
            if (image.stats(null, null, &col, null, null, null, null, &box))
                AddHue(image, HueDelta(RgbToHsb(col.xyz).x, param.asFlt()), box, param.name == "avgHuePhoto");
            image.unlock();
        }
    } else if (param.name == "avgHueAlphaWeight" || param.name == "avgHuePhotoAlphaWeight") {
        if (image.lock()) // lock for writing because we will use this lock for applying hue too
        {
            Vec col;
            if (image.stats(null, null, null, null, null, &col, null, &box))
                AddHue(image, HueDelta(RgbToHsb(col).x, param.asFlt()), box, param.name == "avgHuePhotoAlphaWeight");
            image.unlock();
        }
    } else if (param.name == "medHue" || param.name == "medHuePhoto") {
        if (image.lock()) // lock for writing because we will use this lock for applying hue too
        {
            Vec4 col;
            if (image.stats(null, null, null, &col, null, null, null, &box))
                AddHue(image, HueDelta(RgbToHsb(col.xyz).x, param.asFlt()), box, param.name == "medHuePhoto");
            image.unlock();
        }
    } else if (param.name == "medHueAlphaWeight" || param.name == "medHuePhotoAlphaWeight") {
        if (image.lock()) // lock for writing because we will use this lock for applying hue too
        {
            Vec col;
            if (image.stats(null, null, null, null, null, null, &col, &box))
                AddHue(image, HueDelta(RgbToHsb(col).x, param.asFlt()), box, param.name == "medHuePhotoAlphaWeight");
            image.unlock();
        }
    } else if (param.name == "gammaSat")
        GammaSat(image, param.asFlt(), box);
    else if (param.name == "gammaSatPhoto")
        GammaSat(image, param.asFlt(), box, true);
    else if (param.name == "mulSat")
        MulAddSat(image, param.asFlt(), 0, box);
    else if (param.name == "mulSatPhoto")
        MulAddSat(image, param.asFlt(), 0, box, true);
    else if (param.name == "mulAddSat") {
        Vec2 ma = param.asVec2();
        MulAddSat(image, ma.x, ma.y, box);
    } else if (param.name == "mulAddSatPhoto") {
        Vec2 ma = param.asVec2();
        MulAddSat(image, ma.x, ma.y, box, true);
    } else if (param.name == "addSat")
        MulAddSat(image, 1, param.asFlt(), box);
    else if (param.name == "addSatPhoto")
        MulAddSat(image, 1, param.asFlt(), box, true);
    else if (param.name == "setSat")
        MulAddSat(image, 0, param.asFlt(), box);
    else if (param.name == "setSatPhoto")
        MulAddSat(image, 0, param.asFlt(), box, true);
    else if (param.name == "minSat")
        MinSat(image, param.asFlt(), box);
    else if (param.name == "minSatPhoto")
        MinSat(image, param.asFlt(), box, true);
    else if (param.name == "avgSat" || param.name == "avgSatPhoto") {
        flt avg;
        if (image.statsSat(null, null, &avg, null, null, null, null, &box))
            if (avg)
                MulAddSat(image, param.asFlt() / avg, 0, box, param.name == "avgSatPhoto");
    } else if (param.name == "medSat" || param.name == "medSatPhoto") {
        flt med;
        if (image.statsSat(null, null, null, &med, null, null, null, &box))
            if (med)
                MulAddSat(image, param.asFlt() / med, 0, box, param.name == "medSatPhoto");
    } else if (param.name == "mulSatH" || param.name == "mulSatHS") {
        ...
        case 1: {
            flt v = TextFlt(vals[0]);
            MulSatH(image, v, v, v, v, v, v, sat, false, box);
        } break;
        case 3: {
            Vec v(TextFlt(vals[0]), TextFlt(vals[1]), TextFlt(vals[2]));
            MulSatH(image, v.x, Avg(v.x, v.y), v.y, Avg(v.y, v.z), v.z, Avg(v.z, v.x), sat, false, box);
        } break;
        case 6:
            MulSatH(image, TextFlt(vals[0]), TextFlt(vals[1]), TextFlt(vals[2]), TextFlt(vals[3]), TextFlt(vals[4]), TextFlt(vals[5]), sat, false, box);
            break;
        }
    } else if (param.name == "mulSatHPhoto" || param.name == "mulSatHSPhoto") {
        ...
        case 1: {
            flt v = TextFlt(vals[0]);
            MulSatH(image, v, v, v, v, v, v, sat, true, box);
        } break;
        case 3: {
            Vec v(TextFlt(vals[0]), TextFlt(vals[1]), TextFlt(vals[2]));
            MulSatH(image, v.x, Avg(v.x, v.y), v.y, Avg(v.y, v.z), v.z, Avg(v.z, v.x), sat, true, box);
        } break;
        case 6:
            MulSatH(image, TextFlt(vals[0]), TextFlt(vals[1]), TextFlt(vals[2]), TextFlt(vals[3]), TextFlt(vals[4]), TextFlt(vals[5]), sat, true, box);
            break;
        }
    } else if (param.name == "addHueSat") {
        Vec2 hue_sat = param.asVec2();
        if (hue_sat.any() && image.lock()) {
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
                    for (int x = box.min.x; x < box.max.x; x++) {
                        Vec4 c = image.color3DF(x, y, z);
    ...
    } else if (param.name == "setHuePhoto") {
        if (image.lock()) {
            flt hue = param.asFlt();
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
    ...
    } else if (param.name == "setHueSatPhoto") // photometric
    {
        if (image.lock()) {
            Vec2 hue_sat = param.asVec2();
            Vec rgb = HsbToRgb(Vec(hue_sat, 1));
            rgb /= SRGBLumOfSRGBColor(rgb);
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
                    for (int x = box.min.x; x < box.max.x; x++) {
                        Vec4 c = image.color3DF(x, y, z);
    ...
    } else if (param.name == "lerpHue") {
        Vec2 hue_alpha = param.asVec2();
        if (hue_alpha.y && image.lock()) {
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
                    for (int x = box.min.x; x < box.max.x; x++) {
                        Vec4 c = image.color3DF(x, y, z);
    ...
    } else if (param.name == "lerpHueSat") {
        Vec hue_sat_alpha = param.asVec();
        if (hue_sat_alpha.z && image.lock()) {
    ...
    } else if (param.name == "rollHuePhoto") {
        Vec2 hue_alpha = param.asVec2();
        if (hue_alpha.y && image.lock()) {
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
    ...
    } else if (param.name == "rollHueSatPhoto") {
        Vec hue_sat_alpha = param.asVec();
        if (hue_sat_alpha.z && image.lock()) {
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
        ...
        /*if(param.name=="metalToReflect")
   {
    ...
    if (param.name == "scale") // the formula is ok (for normal too), it works as if the bump was scaled vertically by 'scale' factor
    {
        ...
        if (scale != 1) {
            if (image.typeChannels() <= 1 || image.monochromatic())
        ...
        if (image.typeChannels() <= 1 || image.monochromatic()) {
            flt a = r.avg();
            image.mulAdd(Vec4(Vec(a), 1), Vec4(Vec(-0.5f * a + 0.5f), 0), &box);
        } // if image is 1-channel or monochromatic then we need to transform all RGB together
        else
            image.mulAdd(Vec4(r, 1, 1), Vec4(-0.5f * r + 0.5f, 0, 0), &box);
    } else if (param.name == "scaleAngle") // the formula is ok (for normal too), it works as if the bump was scaled vertically by 'scale' factor
    {
        flt scale = param.asFlt();
        if (scale != 1) {
            if (image.typeChannels() <= 1 || image.monochromatic())
    ...
    } else if (param.name == "uniformTransparent") {
        Vec4 avg;
        avg.w = 0;
        if (image.stats(null, null, null, null, null, &avg.xyz, null, &box))
            for (int z = box.min.z; z < box.max.z; z++)
                for (int y = box.min.y; y < box.max.y; y++)
                    for (int x = box.min.x; x < box.max.x; x++) {
                        Vec4 c = image.color3DF(x, y, z);
...
void TransformImage(Image &image, C MemPtr<TextParam> &params, bool clamp, C Color &background, C VecI &background_size) {
    FREPA(params)
    TransformImage(image, params[i], clamp, background, background_size); // process in order
}
bool LoadImage(C Project *proj, Image &image, TextParam *image_resize, C FileParams &fp, bool srgb, bool clamp, C Color &background, C VecI &background_size, C Image *color, C TextParam *color_resize, C Image *smooth, C TextParam *smooth_resize, C Image *bump, C TextParam *bump_resize) // !! this ignores 'fp.nodes' !!
{
    ...
    if (name == "|color|") {
        if (color) {
            ...
            if (color_resize) {
                if (image_resize)
                    *image_resize = *color_resize;
                else
                    TransformImage(image, *color_resize, clamp, background, background_size);
            }
            goto imported;
        }
...
bool LoadImages(C Project *proj, Image &image, TextParam *image_resize, C Str &src, bool srgb, bool clamp, C Color &background, C Image *color, C TextParam *color_resize, C Image *smooth, C TextParam *smooth_resize, C Image *bump, C TextParam *bump_resize) {
    ...
    Image layer;
    TextParam layer_resize, image_resize_final;
    ExtractResize(files, image_resize_final); // get what this image wants to be resized to at the end
    TextParam *layer_resize_ptr = null;       // if null then apply resize to source images (such as 'color_resize' for 'color' etc.)
    REPA(files)                               // go from end
    {
        FileParams &file = files[i];
        if (i && (file.name.is() || file.nodes.elms()))
            goto force_src_resize; // force resize if there's more than one file name specified
        REPA(file.params)
        if (SizeDependentTransform(file.params[i]))
            goto force_src_resize; // if there's at least one size dependent transform anywhere then always apply
    }
...
force_src_resize:
    ...
    REPA(files)
    if (C TextParam *p = files[i].findParam("mode"))
    ...
    FREPA(files) // process in order
    {
        ...
        flt alpha = 1;
        {
            C TextParam *p = file.findParam("opacity");
            if (!p)
                p = file.findParam("alpha");
            if (p)
                alpha = p->asFlt();
        }
        if (alpha != 0) {
            APPLY_MODE mode = APPLY_SET;
            if (C TextParam *p = file.findParam("mode")) {
                if (p->value == "blend")
                    mode = APPLY_BLEND;
                else if (p->value == "blendSimple")
                    mode = APPLY_BLEND_SIMPLE;
                else if (p->value == "merge" || p->value == "blendPremultiplied" || p->value == "premultipliedBlend")
                    mode = APPLY_MERGE;
                else if (p->value == "mergeSimple")
                    mode = APPLY_MERGE_SIMPLE;
                else if (p->value == "mul")
                    mode = APPLY_MUL;
                else if (p->value == "mulRGB")
                    mode = APPLY_MUL_RGB;
                else if (p->value == "mulRGBblend")
                    mode = APPLY_MUL_RGB_BLEND;
                else if (p->value == "mulRGBS")
                    mode = APPLY_MUL_RGB_SAT;
                else if (p->value == "mulRGBIS")
                    mode = APPLY_MUL_RGB_INV_SAT;
                else if (p->value == "mulRGBLin")
                    mode = APPLY_MUL_RGB_LIN;
                else if (p->value == "mulA")
                    mode = APPLY_MUL_A;
                else if (p->value == "mulLum")
                    mode = APPLY_MUL_LUM;
                else if (p->value == "mulSat")
                    mode = APPLY_MUL_SAT;
                else if (p->value == "mulSatPhoto")
                    mode = APPLY_MUL_SAT_PHOTO;
                else if (p->value == "div")
                    mode = APPLY_DIV;
                else if (p->value == "divRGB")
                    mode = APPLY_DIV_RGB;
                else if (p->value == "add")
                    mode = APPLY_ADD;
                else if (p->value == "addRGB")
                    mode = APPLY_ADD_RGB;
                else if (p->value == "addLum")
                    mode = APPLY_ADD_LUM;
                else if (p->value == "mulInvLum")
                    mode = APPLY_MUL_INV_LUM;
                else if (p->value == "mulInvLumAddRGB")
                    mode = APPLY_MUL_INV_LUM_ADD_RGB;
                else if (p->value == "addSat")
                    mode = APPLY_ADD_SAT;
                else if (p->value == "addSatPhoto")
                    mode = APPLY_ADD_SAT_PHOTO;
                else if (p->value == "addHue")
                    mode = APPLY_ADD_HUE;
                else if (p->value == "addHuePhoto")
                    mode = APPLY_ADD_HUE_PHOTO;
                else if (p->value == "setAfromRGB")
                    mode = APPLY_SET_A_FROM_RGB;
                else if (p->value == "setHue")
                    mode = APPLY_SET_HUE;
                else if (p->value == "setHuePhoto")
                    mode = APPLY_SET_HUE_PHOTO;
                else if (p->value == "setLum")
                    mode = APPLY_SET_LUM;
                else if (p->value == "setLumPhoto")
                    mode = APPLY_SET_LUM_PHOTO;
                else if (p->value == "sub")
                    mode = APPLY_SUB;
                else if (p->value == "subRGB")
                    mode = APPLY_SUB_RGB;
                else if (p->value == "gamma")
                    mode = APPLY_GAMMA;
                else if (p->value == "gammaLum")
                    mode = APPLY_GAMMA_LUM;
                else if (p->value == "brightness")
                    mode = APPLY_BRIGHTNESS;
                else if (p->value == "brightnessLum")
                    mode = APPLY_BRIGHTNESS_LUM;
                else if (p->value == "avg" || p->value == "average")
                    mode = APPLY_AVG;
                else if (p->value == "min")
                    mode = APPLY_MIN;
                else if (p->value == "max")
                    mode = APPLY_MAX;
                else if (p->value == "maxRGB")
                    mode = APPLY_MAX_RGB;
                else if (p->value == "maxA" || p->value == "maxAlpha")
                    mode = APPLY_MAX_A;
                else if (p->value == "maxLum")
                    mode = APPLY_MAX_LUM;
                else if (p->value == "maskMul")
                    mode = APPLY_MASK_MUL;
                else if (p->value == "maskAdd")
                    mode = APPLY_MASK_ADD;
                else if (p->value == "metal")
                    mode = APPLY_METAL;
                else if (p->value == "scale")
                    mode = APPLY_SCALE;
                else if (p->value == "scaleXY")
                    mode = APPLY_SCALE_XY;
                else if (p->value == "scaleAngle")
                    mode = APPLY_SCALE_ANGLE;
                else if (p->value == "fire")
                    mode = APPLY_FIRE;
                else if (p->value == "skip" || p->value == "ignore")
                    mode = APPLY_SKIP;
            }
            if (mode != APPLY_SKIP) {
                bool layer_ok;
                if (file.nodes.elms()) {
                    ...
                    if (layer_ok)
                        TransformImage(layer, file.params, clamp, background, image.size3());
                } else {
                ...
                if (layer_ok) {
                    if (layer_resize.name.is() && !image_resize_final.name.is())
                        Swap(layer_resize, image_resize_final); // if have info about source resize and final resize was not specified, then use source resize as final
                    VecI2 pos = 0;
                    {
                        C TextParam *p = file.findParam("position");
                        if (!p)
                            p = file.findParam("pos");
                        if (p)
                            pos = p->asVecI2();
                    }
                    bool alpha_1 = Equal(alpha, 1),
                         simple_set = (mode == APPLY_SET && alpha_1);
                    if (!has_data && pos.allZero() && simple_set && layer.is())
                        Swap(layer, image);
                    else // if this is the first image, then just swap it as main
                    {
                        ...
                        if (layer.is()) {
                            ...
                            case APPLY_MASK_MUL:
                            case APPLY_MASK_ADD: {
                                if (C TextParam *p = file.findParam("maskRed"))
                                    mask[0] = TextVecEx(p->asText());
                                else
                                    mask[0] = 1;
                                if (C TextParam *p = file.findParam("maskGreen"))
                                    mask[1] = TextVecEx(p->asText());
                                else
                                    mask[1] = 1;
                                if (C TextParam *p = file.findParam("maskBlue"))
                                    mask[2] = TextVecEx(p->asText());
                                else
                                    mask[2] = 1;
                                if (C TextParam *p = file.findParam("maskAlpha"))
                                    mask[3] = TextVecEx(p->asText());
                                else
                                    mask[3] = 1;
                            } break;
                            ...
                            case APPLY_SCALE_ANGLE:
                                mono = (image.typeChannels() <= 1 || image.monochromatic());
                                break;
                            }
                            ...
                            REPD(x, layer.w()) {
                                ...
                                if (simple_set) {
                                    ...
                                    case APPLY_BLEND:
                                        c = Blend(base, l);
                                        break;
                                    case APPLY_BLEND_SIMPLE:
                                        c = FastBlend(base, l);
                                        break;
                                    case APPLY_MERGE:
                                        c = MergeBlend(base, l);
                                        break;
                                    case APPLY_MERGE_SIMPLE:
                                        c = FastMergeBlend(base, l);
                                        break;
                                    case APPLY_MUL:
                                        c = base * l;
                                        break;
                                    case APPLY_MUL_RGB:
                                        c.set(base.xyz * l.xyz, base.w);
                                        break;
                                    case APPLY_MUL_RGB_BLEND:
                                        c.set(base.xyz * Lerp(VecOne, l.xyz, l.w), base.w);
                                        break;
                                    case APPLY_MUL_RGB_LIN:
                                        c.set(LinearToSRGB(SRGBToLinear(base.xyz) * l.xyz), base.w);
                                        break; // this treats 'l' as already linear
                                    case APPLY_MUL_A:
                                        c.set(base.xyz, base.w * l.w);
                                        break;
                                    case APPLY_SET_A_FROM_RGB:
                                        c.set(base.xyz, l.xyz.max());
                                        break;
                                    case APPLY_MUL_LUM:
                                        c.set(base.xyz * l.xyz.max(), base.w);
                                        break;
                                    case APPLY_MUL_INV_LUM:
                                        c.set(base.xyz * Sat(1 - l.xyz.max()), base.w);
                                        break;
                                    case APPLY_MUL_INV_LUM_ADD_RGB:
                                        c.set(base.xyz * Sat(1 - l.xyz.max()) + l.xyz, base.w);
                                        break;
                                    case APPLY_MUL_SAT:
                                        c.xyz = RgbToHsb(base.xyz);
                                        c.y *= l.xyz.max();
                                        c.set(HsbToRgb(c.xyz), base.w);
                                        break;
                                    case APPLY_ADD_SAT:
                                        c.xyz = RgbToHsb(base.xyz);
                                        c.y += l.xyz.max();
                                    ...
                                    case APPLY_ADD_RGB:
                                        ...
                                        break;
                                    case APPLY_ADD_LUM: {
                                        flt old_lum = base.xyz.max(), new_lum = old_lum + l.xyz.max();
                                        if (old_lum > 0)
                                            c.xyz = base.xyz * (new_lum / old_lum);
                                        else
                                            c.xyz = new_lum;
                                        c.w = base.w;
                                    } break;
                                    ...
                                    case APPLY_GAMMA: {
                                        flt gamma = l.xyz.max();
                                        c.set(PowMax(base.x, gamma), PowMax(base.y, gamma), PowMax(base.z, gamma), base.w);
                                    } break;
                                    case APPLY_GAMMA_LUM: {
                                        flt gamma = l.xyz.max();
                                        if (flt lum = base.xyz.max())
                                            c.xyz = base.xyz * (PowMax(lum, gamma) / lum);
                                        else
                                            c.xyz = base.xyz;
                                        c.w = base.w;
                                    } break;
                                    ...
                                    case APPLY_MAX:
                                        ...
                                        break;
                                    case APPLY_MAX_RGB:
                                        c.set(Max(base.xyz, l.xyz), base.w);
                                        break;
                                    case APPLY_MAX_A:
                                        c.set(base.xyz, Max(base.w, l.w));
                                        break;
                                    case APPLY_METAL: {
                                        flt metal = l.xyz.max();
                                        c.set(Lerp(base.xyz, l.xyz, metal), base.w);
                                    } break; // this applies metal map onto diffuse map (by lerping from diffuse to metal based on metal intensity)
                                    ...
                                    case APPLY_SET_LUM: {
                                        ...
                                        if (flt cur_lum = base.xyz.max())
                                            ...
                                            c.xyz = lum; // prefer multiplications in sRGB space, as linear mul may change perceptual contrast and saturation
                                        c.w = base.w;
                                    } break;
                                    ...
                                    case APPLY_MAX_LUM: {
                                        ...
                                        if (layer_lum > base_lum) {
                                            if (base_lum)
                                                c.xyz = base.xyz * (layer_lum / base_lum);
                                            else
                                                c.xyz = layer_lum;
                                        } else
                                            c.xyz = base.xyz;
                                    ...
                                    case APPLY_SCALE: {
                                        ...
                                        c.w = base.w;
                                        if (mono)
                                            c.xyz = (base.xyz - 0.5f) * scale + 0.5f;
                                        else if (!scale)
                                            c.xyz.set(0.5f, 0.5f, 1);
                                        else {
                                            Vec &n = c.xyz;
                                            n = base.xyz * 2 - 1;
                                            n.normalize();
                        ...
                        } else if (!(file.name.is() || file.nodes.elms()))
                            TransformImage(image, file.params, clamp, background, image.size3()); // if this 'layer' is empty and no file name was specified, then apply params to the entire 'image', so we can process entire atlas
                    }
                    has_data = true;
                } else
                    ok = false;
    ...
    if (image_resize)
        *image_resize = image_resize_final;
    else
        TransformImage(image, image_resize_final, clamp, background, image.size3());
...
bool ValidFileNameForUpload(C Str &name, int max) {
    if (ValidFileName(name) && ValidText(name, 1, max)) {
        REPA(name)
        if (name[i] == '@' || name[i] == '~')
            return false; // because of "@new" and "~" is special char on unix ?
        return true;
    }
    return false;
}
...
Str TimeAgo(C DateTime &date) {
    DateTime now;
    now.getUTC();
    return TimeText(now - date, TIME_NAME_MED) + " ago";
}
...
bool TextVecVecEx(cchar *t, Vec &a, Vec &b) {
    ...
    case 2:
        ...
        return true;
    case 6:
        a.set(TextFlt(c[0]), TextFlt(c[1]), TextFlt(c[2]));
        b.set(TextFlt(c[3]), TextFlt(c[4]), TextFlt(c[5]));
        return true;
    }
    return false;
}
Str TextVecVecEx(C Vec &a, C Vec &b, int precision) {
    return (Equal(a.x, a.y) && Equal(a.x, a.z) && Equal(b.x, b.y) && Equal(b.x, b.z)) ? TextReal(a.x, precision) + ", " + TextReal(b.x, precision)
...
Str EditToGamePath(Str path) {
    ...
    for (path.tailSlash(false);;) {
        Str base = GetBase(path);
        path = GetPath(path);
        if (!base.is())
            return path.tailSlash(true) + out; // needed for "/xxx" unix style paths
        if (base == "Edit")
            return path.tailSlash(true) + "Game\\" + out; // replace "edit" with "game"
...
TextParam *FindTransform(MemPtr<FileParams> files, C Str &name) // this ignores partial(non full size) transforms
{
    REPA(files) // go from end
    {
        ...
        {
            TextParam &p = file.params[i];
            if (p.name == name && !PartialTransform(p))
                return &p;
        }
    }
    return null;
...
void SetTransform(MemPtr<FileParams> files, C Str &name, C Str &value) // this ignores partial(non full size) transforms
{
    ...
    {
        if (files.elms() > 1 && (files.last().name.is() || files.last().nodes.elms()))
            files.New(); // if we have more than one image, then we need to make sure that we add the parameter not to one of the images, but as last file that has no name specified to set transform for everything
        TextParam *p;
        REPA(files) // go from end
        {
            FileParams &file = files[i];
    ...
    if (files.elms()) // set only if we have something (to ignore setting for completely empty)
    {
        ...
        REPA(files) // go from end
        {
            ...
            REPA(file.params) // go from end
            {
                ...
                if (p->name == name && !PartialTransform(*p))
                    goto found;
                if (SizeDependentTransform(*p))
                    goto need_new; // if encountered a size-dependent transform then it means we can't change any resize transforms before that, but need to create a new one
            }
        }
    need_new:
        p = &files.last().params.New().setName(name); // if didn't found then create a new one
...
SOUND_CODEC TextSoundCodec(C Str &t) {
    if (t == "raw" || t == "wav" || t == "uncompressed")
        return SOUND_WAV;
    if (t == "vorbis")
        return SOUND_SND_VORBIS;
    if (t == "opus")
        return SOUND_SND_OPUS;
    return SOUND_NONE;
}
/******************************************************************************/
// MESH
...
int VisibleSize(C MeshLod &mesh) {
    ...
    REPA(mesh)
    if (!(mesh.parts[i].part_flag & MSHP_HIDDEN))
        size += mesh.parts[i].render.memUsage();
    return size;
}
MESH_FLAG VisibleFlag(C MeshLod &mesh) {
    ...
    REPA(mesh)
    if (!(mesh.parts[i].part_flag & MSHP_HIDDEN))
        flag |= mesh.parts[i].flag();
    return flag;
}
MESH_FLAG VisibleFlag(C Mesh &mesh) {
    MESH_FLAG flag = MESH_NONE;
    REP(mesh.lods())
    flag |= VisibleFlag(mesh.lod(i));
    return flag;
}
    ...
    REPAD(d, dest) {
        ...
        REPAD(s, src) {
            ...
            if (Equal(src_part.name, dest_part.name)) // if same name
                if (src_part_i < 0 || Abs(s - d) < Abs(src_part_i - d))
                    src_part_i = s; // if new index is closer to original index
        }
        if (InRange(src_part_i, src)) {
            ...
            REP(dest_part.variations())
            if (i)
                dest_part.variation(i, src_part.variation(i));
        }
    }
...
void EditToGameMesh(C Mesh &edit, Mesh &game, Skeleton *skel, Enum *draw_group, C Matrix *matrix) {
    ...
    if (matrix)
        game.transform(*matrix); // transform before welding
    game.setAutoTanBin()         // calculate tangents before welding
        .weldVtx(VTX_ALL, EPS, EPS_COL8_COS, -1)
        .skeleton(skel)
        .drawGroupEnum(draw_group)
        .setBox(); // use 8-bit for vtx normals because they can't handle more anyway, always recalculate box because of transform and welding
    game.setRender().delBase();
}
/******************************************************************************/
bool HasMaterial(C MeshPart &part, C MaterialPtr &material) {
    REP(part.variations())
    if (part.variation(i) == material)
        return true;
    return false;
}
/******************************************************************************/
...
bool FixVtxNrm(MeshBase &base) {
    bool ok = false;
    if (base.vtx.nrm()) {
        MeshBase temp(base, VTX_POS | FACE_IND);
        temp.setNormals();                                // copy to a temp mesh and set its vtx normals
        if (base.vtxs() == temp.vtxs() && temp.vtx.nrm()) // safety checks
        {
            ok = true;
            REPA(base.vtx) {
                Vec &nrm = base.vtx.nrm(i);
                if (nrm.allZero()) // if any 'base' vtx normal is zero
                {
                    nrm = temp.vtx.nrm(i);
                    if (nrm.allZero())
                        ok = false; // copy from 'temp'
                }
            }
        }
    }
...
void FixMesh(Mesh &mesh) {
    mesh.setBase(true).delRender() // create base if empty
        .material(null)
        .variations(0) // clear any existing materials, they will be set later according to 'mtrls'
        .skeleton(null)
        .drawGroupEnum(null) // clear 'skeleton', clear 'drawGroupEnum'
    ...
    REP(mesh.lods()) {
        ...
        REPA(lod) {
            ...
            if (base.vtx.nrm())
                REPA(base.vtx)
            if (!base.vtx.nrm(i).any()) // all zero
            {
...
bool SamePartInAllLods(C Mesh &mesh, int part) {
   ...
   return false;
    ...
    for (Int i = mesh.lods(); --i >= 1;) {
        C MeshLod &lod = mesh.lod(i);
        if (lod.parts.elms() != mesh.parts.elms())
            return false;
        REPA(lod.parts)
        if (!Equal(lod.parts[i].name, mesh.parts[i].name))
            return false;
    }
    return true;
#endif
...
void SetDrawGroup(Mesh &mesh, MeshLod &lod, int part, int group, Enum *draw_group_enum) {
    if (SamePartInAllLods(mesh, part)) {
        REP(mesh.lods()) {
            MeshLod &lod = mesh.lod(i);
            if (InRange(part, lod))
                lod.parts[part].drawGroup(group, draw_group_enum);
        }
    } else {
        if (InRange(part, lod))
            lod.parts[part].drawGroup(group, draw_group_enum);
    ...
    FREP(mesh.lods()) {
        ...
        if (!NegativeSB(lod.dist2)) // not disabled
        {
            ...
            if (disable || tris + 128 >= last_tris) // reached low tris, or there's only 128 tri difference between this and previous LOD
            {
                ...
                last_tris = tris;
                if (tris <= 256)
                    disable = true; // disable next LODs
            }
        }
    }
    return changed;
...
{
    if (int c = Compare(a.mesh().id(), b.mesh().id()))
        return c; // first compare by mesh
    if (int c = Compare(a.meshVariationID(), b.meshVariationID()))
        return c; // then  compare by mesh variation
    if (int c = Compare(a.matrix.pos, b.matrix.pos))
        return c; // last  compare by position
    return 0;
}
/******************************************************************************/
// ANIMATION
...
void SetRootMoveRot(Animation &anim, C Vec *root_move, C Vec *root_rot) {
    if (root_rot) {
        ...
        if (num) {
            ...
            if (num >= 2) {
                num--;
                VecD axis = *root_rot;
                dbl angle = axis.normalize();
                MatrixD3 rot;
                rot.setRotate(axis, angle / num);
                for (int i = 1; i <= num; i++) {
                    orn.mul(rot, true);
    ...
    if (root_move) {
        ...
        int num = (Equal(*root_move, VecZero) ? Equal(anim.rootStart().pos, VecZero) ? 0 : 1 : no_rot ? 2
                                                                                                      : 1 + Max(1, Round(root_move->length() * precision)));
...
bool DelEndKeys(Animation &anim) // return if any change was made
{
    ...
    REPA(anim.bones) {
        ...
        if (bone.poss.elms() >= 2 && bone.poss.last().time >= time && bone.poss.first().time <= EPS) {
            bone.poss.removeLast();
            bone_changed = true;
        }
        if (bone.orns.elms() >= 2 && bone.orns.last().time >= time && bone.orns.first().time <= EPS) {
            bone.orns.removeLast();
            bone_changed = true;
        }
        if (bone.scales.elms() >= 2 && bone.scales.last().time >= time && bone.scales.first().time <= EPS) {
            bone.scales.removeLast();
            bone_changed = true;
        }
        if (bone_changed) {
            bone.setTangents(anim.loop(), anim.length());
            changed = true;
        }
    }
...
bool Distance2D(C Vec2 &point, C Edge &edge, flt &dist, flt min_length) // calculate 2D distance between 'point' and 'edge' projected to screen, true is returned if 'edge' is at least partially visible (not behind camera), 'dist' will contain distance between point and edge
{
    ...
    if (Clip(e, Plane(ActiveCam.matrix.pos + D.viewFrom() * ActiveCam.matrix.z, -ActiveCam.matrix.z))) {
        ...
        if (e2.length() < min_length)
            return false; // if edge is too short then skip it
        dist = Dist(point, e2);
        return true;
    }
    return false;
...
int MatrixAxis(C Vec2 &screen_pos, C Matrix &matrix) {
    int axis = -1;
    flt d, dist = 0;
    if (Distance2D(screen_pos, Edge(matrix.pos, matrix.pos + matrix.x), d))
        if (axis < 0 || d < dist) {
            dist = d;
            axis = 0;
        }
    if (Distance2D(screen_pos, Edge(matrix.pos, matrix.pos + matrix.y), d))
        if (axis < 0 || d < dist) {
            dist = d;
            axis = 1;
        }
    if (Distance2D(screen_pos, Edge(matrix.pos, matrix.pos + matrix.z), d))
        if (axis < 0 || d < dist) {
            dist = d;
            axis = 2;
        }
...
void MatrixAxis(Edit::Viewport4 &v4, C Matrix &matrix, int &axis, Vec *axis_vec) {
    int editing = -1;
    REPA(MT)
    if (MT.b(i, MT.mouse(i)) && v4.getView(MT.guiObj(i))) {
        editing = i;
        break;
    }
    if (editing < 0) {
        ...
        if (editing < 0)
            REPA(MT)
        if (MT.mouse(i) && v4.getView(MT.guiObj(i))) {
            editing = i;
            break;
        } // get mouse
        if (editing >= 0)
            if (Edit::Viewport4::View *view = v4.getView(MT.guiObj(editing))) {
...
bool UniformScale(C Vec &s) {
    return Equal(s.x / s.y, 1) && Equal(s.x / s.z, 1);
}
/******************************************************************************/
...
void DrawMatrix(C Matrix &matrix, int bold_axis) {
    ...
    switch (bold_axis) {
    case 0:
        DrawArrow2(RED, matrix.pos, matrix.pos + matrix.x, 0.005f);
        break;
    case 1:
        DrawArrow2(GREEN, matrix.pos, matrix.pos + matrix.y, 0.005f);
        break;
    case 2:
        DrawArrow2(BLUE, matrix.pos, matrix.pos + matrix.z, 0.005f);
        break;
    }
}
/******************************************************************************/
...
Rect GetRect(C Rect &rect, Memt<Rect> &rects) // !! this will modify 'rects' !!
{
    for (flt x = D.w();;) // start with right side
    {
        ...
        {
            Rect temp = rect;
            temp += Vec2(x, y) - temp.ru(); // move to test position

            if (!rects.elms())
                return temp;

            if (temp.min.y < -D.h())
                break; // we're outside the screen

            bool cuts = false;
            REPA(rects)
            if (Cuts(temp, rects[i])) {
        ...
        int found = -1;
        REPA(rects)
        if (found == -1 || rects[i].min.x > x) {
            found = i;
            x = rects[i].min.x;
        }
        REPA(rects)
        if (rects[i].min.x >= x - EPS)
            rects.remove(i);
    }
}
/******************************************************************************/
...
bool EmbedObject(C Box &obj_box, C VecI2 &area_xy, flt area_size) {
    return obj_box.min.x / area_size < area_xy.x - 0.5f || obj_box.max.x / area_size > area_xy.x + 1.5f || obj_box.min.z / area_size < area_xy.y - 0.5f || obj_box.max.z / area_size > area_xy.y + 1.5f;
}
/******************************************************************************/
...
UID GetFileNameID(Str name) {
    for (; name.is(); name = GetPath(name)) {
        UID id = FileNameID(name);
        if (id.valid())
            return id;
    }
    return UIDZero;
...
void SetPath(WindowIO &win_io, C Str &path, bool clear) {
    Mems<FileParams> fps = FileParams::Decode(path);
    if (fps.elms() == 1 && !fps[0].nodes.elms()) {
        Str first = FFirstUp(fps[0].name);
        if (FileInfoSystem(first).type == FSTD_FILE)
            first = GetPath(first);
        fps[0].name = SkipStartPath(fps[0].name, first);
        win_io.path(S, first).textline.set(fps[0].encode());
        return;
    } else if (fps.elms()) {
        win_io.path(S).textline.set(path);
        return;
    }
    if (clear)
        win_io.path(S).textline.clear();
}
/******************************************************************************/
...
UNIT_TYPE GetUnitType(C Str &s) {
    if (s.is()) {
        ...
        int pos = s.length() - 1;
        for (; pos > 0 && (CharFlag(s[pos - 1]) & flag_and) == flag; pos--)
            ;
...
Color GuiListTextColor() {
    if (Gui.skin && Gui.skin->list.text_style)
        return Gui.skin->list.text_style->color;
    return BLACK;
}
...
{
    for (int i = 1; i < Elms(BitRateQualities); i++)
        if (rel_bit_rate <= BitRateQualities[i].bit_rate) {
...
bool FileSizeGetter::get() {
    ...
    if (elms_thread.elms()) {
        ...
        if (!elms.elms())
            Swap(elms_thread, elms);
        else {
            FREPA(elms_thread)
            elms.add(elms_thread[i]);
            elms_thread.clear();
        }
        return true;
    }
...
void FileSizeGetter::cleanup() {
    if (!thread.active())
        thread.del(); // delete to free resources
}
...
SrcDest &SrcDest::set(C Str &src, C Str &dest) {
    T.src = src;
    T.dest = dest;
    return T;
}
SrcDest &SrcDest::set(C Str &src, C Str &dest, C Str &name) {
    T.src = src + name;
    T.dest = dest + name;
    return T;
}
