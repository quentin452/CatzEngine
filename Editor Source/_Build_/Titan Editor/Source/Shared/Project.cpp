...
Project &Project::del() {
    ...
    elms.del();
    texs.del();
    texs_update_base1.del();
    texs_remove_srgb.del();
...
int Project::findElmI(C UID &id) C {
    int index;
    return (id.valid() && elms.binarySearch(id, index, CompareID)) ? index : -1;
}
Elm *Project::findElm(C UID &id) { return elms.addr(findElmI(id)); }
C Elm *Project::findElm(C UID &id) C { return elms.addr(findElmI(id)); }
Elm &Project::getElm(C UID &id) {
    int index;
    if (elms.binarySearch(id, index, CompareID))
        return elms[index];
    Elm &elm = elms.NewAt(index);
    elm.id = id;
    return elm;
}
...
again:
    id.randomizeValid();
    test = id - ELM_OFFSET_NUM;
    REP(ELM_OFFSET_NUM * 2 + 1) {
        if (!test.valid() || hasElm(test))
            goto again;
        test++;
    }
    return getElm(id);
...
Elm &Project::newElm(C Str &name, C UID &parent_id, ELM_TYPE type) {
    Elm &elm = newElm().setName(name, NewElmTime).setParent(parent_id, NewElmTime);
    elm.type = type;
    return elm;
}
Elm *Project::findElmByPath(C Str &path) {
    if (path.is()) {
        ...
        for (;;) {
            ...
            REPA(elms) {
                Elm &elm = elms[i];
                if (elm.parent_id == parent && elm.name == name && ElmVisible(elm.type)) // don't list hidden types
                {
                    found_elm = &elm;
                    if (elm.exists())
                        break; // stop looking if this element exists
                }
            }
            if (!p.is())
                return found_elm;
...
UID Project::findElmID(C Str &text) {
    if (Elm *elm = findElm(text))
        return elm->id;
    return UIDZero;
}
Elm *Project::findElm(C UID &id, ELM_TYPE type) {
    if (Elm *elm = findElm(id))
        if (elm->type == type)
            return elm;
    return null;
}
C Elm *Project::findElm(C UID &id, ELM_TYPE type) C {
    if (C Elm *elm = findElm(id))
        if (elm->type == type)
            return elm;
    return null;
}
Elm *Project::findElm(C Str &text, ELM_TYPE type) {
    if (Elm *elm = findElm(text))
        if (elm->type == type)
            return elm;
    return null;
}
UID Project::findElmID(C UID &id, ELM_TYPE type) {
    if (Elm *elm = findElm(id, type))
        return elm->id;
    return UIDZero;
}
UID Project::findElmID(C Str &text, ELM_TYPE type) {
    if (Elm *elm = findElm(text, type))
        return elm->id;
    return UIDZero;
}
Elm *Project::findElmImage(C Str &text) {
    if (Elm *elm = findElm(text))
        if (ElmImageLike(elm->type))
            return elm;
    return null;
}
UID Project::findElmImageID(C Str &text) {
    if (Elm *elm = findElmImage(text))
        return elm->id;
    return UIDZero;
}
Elm *Project::findElmByTexture(C UID &tex_id) {
    ...
    {
        Elm &elm = elms[i];
        if (elm.data && elm.data->containsTex(tex_id, true)) {
            if (elm.finalPublish())
                return &elm; // if found a publishable then return immediately
            if (elm.finalExists())
                ret = &elm; // save existing for later
        }
    }
    return ret;
}
Str Project::elmFullName(C UID &elm_id, int max_elms) C {
    if (C Elm *elm = findElm(elm_id))
        return elmFullName(elm, max_elms);
    return elm_id.valid() ? UnknownName : S;
}
Str Project::elmFullName(C Elm *elm, int max_elms) C {
    int length = 0;
    Memt<C Elm *> processed;
    Str name;
...
UID Project::physToMesh(C Elm *phys) {
    if (phys)
        if (C ElmPhys *phys_data = phys->physData())
            return phys_data->mesh_id;
    return UIDZero;
}
UID Project::animToSkel(C Elm *anim) {
    if (anim)
        if (C ElmAnim *anim_data = anim->animData())
            return anim_data->skel_id;
    return UIDZero;
}
UID Project::skelToMesh(C Elm *skel) {
    if (skel)
        if (C ElmSkel *skel_data = skel->skelData())
            return skel_data->mesh_id;
    return UIDZero;
}
UID Project::meshToSkel(C Elm *mesh) {
    if (mesh)
        if (C ElmMesh *mesh_data = mesh->meshData())
            return mesh_data->skel_id;
    return UIDZero;
}
UID Project::meshToPhys(C Elm *mesh) {
    if (mesh)
        if (C ElmMesh *mesh_data = mesh->meshData())
            return mesh_data->phys_id;
    return UIDZero;
}
UID Project::meshToObj(C Elm *mesh) {
    if (mesh)
        if (C ElmMesh *mesh_data = mesh->meshData())
            return mesh_data->obj_id;
    return UIDZero;
}
UID Project::objToMesh(C Elm *obj) {
    if (obj)
        if (C ElmObj *obj_data = obj->objData())
            return obj_data->mesh_id;
    return UIDZero;
}
...
Elm *Project::mtrlToMeshElm(C UID &mtrl_id) {
    if (mtrl_id.valid())
        REPA(elms) {
            Elm &elm = elms[i];
            if (C ElmMesh *mesh_data = elm.meshData())
                if (mesh_data->mtrl_ids.binaryHas(mtrl_id))
                    return &elm;
        }
    return null;
}
UID Project::mtrlToObj(C UID &mtrl_id) {
    if (Elm *mesh = mtrlToMeshElm(mtrl_id))
...
void Project::getTextures(MemPtr<UID> texs, bool existing_elms) C // 'existing_elms'=if get only from elements that exist (are not removed)
{
    ...
    REPA(elms) {
        C Elm &elm = elms[i];
        if (existing_elms ? elm.finalExists() : true)
            if (elm.data)
                elm.data->listTexs(texs);
    } // get textures from elements
}
void Project::getUsedMaterials(MemPtr<UID> used, bool publish) C // !! this needs to have hierarchy set !! 'publish'=if true then get materials used by publishable elements, if false then get materials used by existing elements
{
    ...
    REPA(elms) {
        C Elm &elm = elms[i];
        if (publish ? elm.finalPublish() : elm.finalExists())
            switch (elm.type) {
            case ELM_MESH: {
                if (C ElmMesh *mesh_data = elm.meshData())
                    FREPA(mesh_data->mtrl_ids)
                used.binaryInclude(mesh_data->mtrl_ids[i]);
            } break;
            }
    }
}
...
{
    if (mesh_skel)
        *mesh_skel = (mesh_data ? mesh_data->skel_id : UIDZero);
    // get the last body
    if (body_skel) {
        for (Memt<UID> bodies; mesh_data && bodies.include(mesh_data->body_id);)
            if (Elm *body = findElm(mesh_data->body_id))
                mesh_data = body->meshData();
...
void Project::getMeshSkels(ElmMesh *mesh_data, Skeleton **mesh_skel, Skeleton **body_skel) // get skeletons for specified mesh
{
    ...
    if (mesh_skel)
        *mesh_skel = Skeletons(gamePath(mesh_skel_id));
    if (body_skel)
        *body_skel = Skeletons(gamePath(body_skel_id));
}
bool Project::getObjBox(C UID &elm_id, Box &box) // get box of ELM_OBJ (mesh.box|phys.box)
{
    Memt<UID> processed;
    for (UID id = elm_id; processed.binaryInclude(id);) // to avoid potential infinite loops (A is based on A)
        if (Elm *obj_elm = findElm(id))
            if (ElmObj *obj = obj_elm->objData()) {
                if (Elm *mesh_elm = findElm(obj->mesh_id))
                    if (ElmMesh *mesh = mesh_elm->meshData()) {
                        bool have = false;
                        if (mesh->box.valid())
                            if (have)
                                box |= mesh->box;
                        ...
                        if (Elm *phys_elm = findElm(mesh->phys_id))
                            if (ElmPhys *phys = phys_elm->physData())
                                if (phys->box.valid())
                                    if (have)
                                        box |= phys->box;
                                    else {
...
bool Project::getObjTerrain(C UID &elm_id) // get if ELM_OBJ has OBJ_ACCESS_TERRAIN
{
    ...
    for (UID id = elm_id; processed.binaryInclude(id);) // to avoid potential infinite loops (A is based on A)
        if (Elm *obj_elm = findElm(id)) {
            if (ElmObjClass *obj = obj_elm->objClassData())
                return obj->terrain();
            if (ElmObj *obj = obj_elm->objData()) {
                if (obj->ovrAccess())
                    return obj->terrain();
...
RectI Project::getWorldAreas(C UID &world_id, bool water) {
    ...
    if (Elm *world = findElm(world_id, ELM_WORLD))
        if (WorldVer *world_ver = worldVerGet(world_id)) {
            ...
            if (water)
                Include(rect, world_ver->getLakeAreas());
            if (water)
                Include(rect, world_ver->getRiverAreas());
        }
    return rect;
}
int Project::worldAreasToRebuild(C UID *world_id) {
    ...
    if (!world_id)
        REPA(world_vers)
    n += world_vers.lockedData(i).rebuild.elms();
    else // check all worlds
        if (WorldVer *world_ver = worldVerFind(*world_id))
            n += world_ver->rebuild.elms(); // check only specified world
    return n;
}
bool Project::isBasedOnObjs(C Elm &elm, C Memt<UID> &objs) C // check if 'elm' is based on 'objs' (assumes that 'objs' is sorted)
{
    ...
    if (elm.type == ELM_OBJ || elm.type == ELM_OBJ_CLASS)
        for (UID id = elm.id; processed.binaryInclude(id);) // to avoid potential infinite loops (A is based on A)
            if (C Elm *obj_elm = findElm(id)) {
                if (objs.binaryHas(id))
                    return true; // if this object is in the 'objs'
                if (C ElmObj *obj = obj_elm->objData())
                    id = obj->base_id; // proceed to base
            }
    return false;
}
void Project::getExtendedObjs(C Memt<UID> &objs, Memt<UID> &exts) C // get list of all objects that are based on 'objs' (assumes that 'objs' is sorted)
{
    REPA(elms) {
        C Elm &elm = elms[i];
        if (isBasedOnObjs(elm, objs))
            exts.binaryInclude(elm.id);
    }
}
...
Str Project::idToText(C UID &id, bool *valid, ID_MODE id_mode) // 'valid'=if target is valid (not removed)
{
    ...
    if (id.valid()) {
        ...
        REPA(world_vers) // check all worlds for objects/waypoints
        {
            WorldVer &world_ver = world_vers.lockedData(i);
            if (ObjVer *obj_ver = world_ver.obj.find(id)) {
                if (valid && obj_ver->removed())
                    *valid = false;
                return (S + "Object " + ((id_mode == ID_SKIP_UNKNOWN) ? S : id.asCString())).space() + "in World \"" + elmFullName(world_ver.world_id) + '"';
            }
            if (world_ver.waypoints.find(id)) {
                return (S + "Waypoint " + ((id_mode == ID_SKIP_UNKNOWN) ? S : id.asCString())).space() + "in World \"" + elmFullName(world_ver.world_id) + '"';
            }
        }
    }
    if (id_mode == ID_ALWAYS || id_mode == ID_SKIP_ZERO && id.valid())
        return id.asCString();
    return S;
}
bool Project::invalidSrc(C Mems<FileParams> &files, Str *invalid) C // if specified and is not present
{
    REPA(files) {
        ...
        if (invalidSrc(fp.nodes, invalid))
            return true;
    }
    if (invalid)
        invalid->clear();
...
{
    if (src.is() && invalidSrc(FileParams::Decode(src), invalid))
        return true;
    if (invalid)
        invalid->clear();
    return false; // OK
...
bool Project::invalidTexSrc(C Mems<FileParams> &files, Str *invalid) C // if specified and is not present
{
    REPA(files) {
        ...
        if (name.is()) {
            ...
            } else if (DecodeFileName(name, id)) // project element
            {
                C Elm *elm = findElm(id);
                if (!elm || !ElmImageLike(elm->type)) {
                    if (invalid)
                        *invalid = name;
                    return true;
                } // INVALID
        ...
        if (invalidTexSrc(fp.nodes, invalid))
            return true;
    }
    if (invalid)
        invalid->clear();
...
{
    if (src.is() && invalidTexSrc(FileParams::Decode(src), invalid))
        return true;
    if (invalid)
        invalid->clear();
    return false; // OK
...
{
    if (!elm_id.valid())
        return false; // empty/null reference is valid
    if (C Elm *elm = findElm(elm_id))
        return optional ? elm->finalRemoved() : elm->finalNoPublish(); // if it's optional, then we need to check if it exists, if required then we need to check if exists and is publishable
...
bool Project::invalidRefs(Elm &elm) // check if this element has invalid references
{
    ...
    case ELM_OBJ:
        if (ElmObj *data = elm.objData()) {
            if (Elm *mesh_elm = findElm(data->mesh_id))
                if (invalidRefs(*mesh_elm))
                    return true; // process mesh because it's hidden
    ...
    case ELM_MESH:
        if (ElmMesh *data = elm.meshData()) {
            ...
            if (Elm *phys_elm = findElm(data->phys_id))
                if (invalidRefs(*phys_elm))
                    return true; // process phys because it's hidden
            REPA(data->mtrl_ids)
            if (invalidRef(data->mtrl_ids[i]))
                return true;
    ...
    case ELM_SKEL:
        if (ElmSkel *data = elm.skelData()) {
            return invalidRef(data->mesh_id);
        }
        break;

    case ELM_PHYS:
        if (ElmPhys *data = elm.physData()) {
    ...
    case ELM_IMAGE_ATLAS:
        if (ElmImageAtlas *data = elm.imageAtlasData()) {
            REPA(data->images)
            if (!data->images[i].removed && invalidRef(data->images[i].id, true))
                return true;
        }
        break;
    ...
    case ELM_PANEL_IMAGE:
        if (ElmPanelImage *data = elm.panelImageData()) {
            REPA(data->image_ids)
            if (invalidRef(data->image_ids[i], true))
                return true;
        }
        break;
    ...
    case ELM_PANEL:
        if (ElmPanel *data = elm.panelData()) {
            REPA(data->image_ids)
            if (invalidRef(data->image_ids[i]))
                return true;
        }
        break;
    ...
    case ELM_WORLD:
        if (ElmWorld *data = elm.worldData()) {
...
bool Project::materialSimplify(Edit::EXE_TYPE type) C {
    ...
    default:
        return false; // MS_NEVER
    case MS_MOBILE:
        return type == Edit::EXE_APK || type == Edit::EXE_AAB || type == Edit::EXE_IOS || type == Edit::EXE_NS;
    case MS_ALWAYS:
        return true;
    }
}
Elm &Project::getFolder(C Str &name, C UID &parent_id, bool &added, bool ignore_removed) {
    ...
    added = false;
    REPA(elms)
    if (elms[i].parent_id == parent_id && elms[i].name == name && (ignore_removed ? !elms[i].removed() : true))
        return elms[i];
    added = true;
    return newElm(name, parent_id, ELM_FOLDER);
}
UID Project::getPathID(C Str &path, C UID &parent_id, bool ignore_removed) // create all project element folders from 'path' and return last element's id
{
    ...
    for (Str p = path; p.is();) {
        ...
        p = GetStartNot(p);
        bool added;
        parent = getFolder(start, parent, added, ignore_removed).id;
    }
    return parent;
}
...
void Project::createWorldPaths(C UID &world_id) {
    ...
    {
        Str edit, game;
        if (getWorldPaths(world_id, edit, game)) {
...
void Project::makeGameVer(Elm &elm, File *file) {
    if (IsServer)
        return; // this doesn't need to be performed on the server
    Str file_edit_path = editPath(elm), file_game_path = gamePath(elm);
    switch (elm.type) {
    case ELM_MTRL: {
        EditMaterial edit;
        edit.load(file_edit_path);
        Material game;
        edit.copyTo(game, T);
        Save(game, file_game_path);
        savedGame(elm, file_game_path);
    } break;
    ...
    case ELM_MESH: {
        ...
        if (data)
            ...
            matrix.identity();
        Skeleton *body_skel;
        getMeshSkels(data, null, &body_skel);
        Enum *draw_group = (data ? getEnum(data->draw_group_id) : null);
        Mesh mesh;
        if (file)
            mesh.load(*file, game_path);
        else
            Load(mesh, file_edit_path, game_path);
        EditToGameMesh(mesh, mesh, body_skel, draw_group, &matrix);
        Save(mesh, file_game_path);
        if (data)
            data->from(mesh);
        savedGame(elm, file_game_path);
    } break;
    ...
    case ELM_IMAGE: {
        Image image;
        image.Import(file_edit_path);
        EditToGameImage(image, image, *elm.imageData());
        Save(image, file_game_path);
        savedGame(elm, file_game_path);
    } break;
    ...
    case ELM_OBJ: {
        ElmObj *obj_data = (elm.objData());
        Elm *mesh_elm = (obj_data ? findElm(obj_data->mesh_id) : null);
        ElmMesh *mesh_data = (mesh_elm ? mesh_elm->meshData() : null);

        // set mesh/phys only if they're not empty
        bool override_mesh = (obj_data && obj_data->mesh_id.valid());
        if (override_mesh)
            if (MeshPtr mesh = gamePath(obj_data->mesh_id))
                override_mesh &= OverrideMeshSkel(mesh(), mesh->skeleton());
        bool override_phys = (mesh_data && mesh_data->phys_id.valid());
        if (override_phys)
            if (PhysBodyPtr phys = gamePath(mesh_data->phys_id))
                override_phys &= OverridePhys(phys());

        EditObjectPtr edit = file_edit_path;
        Object game;
        edit->copyTo(game, T, true, override_mesh ? &obj_data->mesh_id : null, override_phys ? &mesh_data->phys_id : null);
        Save(game, file_game_path);
        savedGame(elm, file_game_path);
    } break;
    ...
    case ELM_OBJ_CLASS: {
        EditObjectPtr edit = file_edit_path;
        Object game;
        edit->copyTo(game, T, false, null, null);
        Save(game, file_game_path);
        savedGame(elm, file_game_path);
    } break;
        ...
        if (ElmWorld *data = elm.worldData())
            if (data->valid()) {
                ...
                if (getWorldPaths(elm.id, world_edit_path, world_game_path)) {
                    ...
                    data->copyTo(settings, T);
                    Save(settings, world_game_path + "Settings", game_path);
                }
            }
        break;
...
void Project::removeOrphanedElms() {
    ...
    FREPA(elms) // go forward because most likely 'used' ID's will be increasing
    {
        ...
        if (C ElmObj *obj_data = elm.objData())
            used.binaryInclude(obj_data->mesh_id);
    }
    // remove unused meshes
    REPA(elms) {
        Elm &elm = elms[i];
        if (elm.type == ELM_MESH && elm.exists() && !used.binaryHas(elm.id))
    ...
    FREPA(elms) // go forward because most likely 'used' ID's will be increasing
    {
        C Elm &elm = elms[i];
        if (C ElmMesh *mesh_data = elm.meshData())
            if (mesh_data->skel_id.valid())
                used.binaryInclude(mesh_data->skel_id);
    }
    // remove unused skel
    REPA(elms) {
        Elm &elm = elms[i];
        if (elm.type == ELM_SKEL && elm.exists() && !used.binaryHas(elm.id))
            elm.setRemoved(true, time);
    }
    ...
    FREPA(elms) // go forward because most likely 'used' ID's will be increasing
    {
        ...
        if (C ElmMesh *mesh_data = elm.meshData())
            if (mesh_data->phys_id.valid())
                used.binaryInclude(mesh_data->phys_id);
    }
    // remove unused phys
    REPA(elms) {
        Elm &elm = elms[i];
        if (elm.type == ELM_PHYS && elm.exists() && !used.binaryHas(elm.id))
            elm.setRemoved(true, time);
    }
}
void Project::eraseElm(C UID &elm_id) {
    if (Elm *elm = findElm(elm_id)) {
        if (elm->type == ELM_WORLD) // worlds need to have their 'WorldVer' removed from the cache
        {
            CacheLock cl(world_vers);
            REPA(world_vers)
            if (world_vers.lockedData(i).world_id == elm_id)
                world_vers.removeData(&world_vers.lockedData(i));
        }
        ...
        if (elm->type == ELM_CODE) {
            ...
            REP(FormatSuffixElms)
            FDel(formatPath(elm_id, FormatSuffixes[i])); // delete all possible formats (these can be folder, for example Mini Maps)
        }
    }
}
bool Project::eraseElms(C MemPtr<UID> &elm_ids, bool full) {
    FREPA(elm_ids)
    eraseElm(elm_ids[i]); // process removal of all elements
    FREPA(elm_ids)
    elms.removeData(findElm(elm_ids[i]), true); // remove from container
    if (full)                                   // this doesn't need to return true because those files are not present in project data
    {
        for (FileFind ff(edit_path); ff();) {
            UID id;
            if (!id.fromFileName(ff.name) || !findElm(id))
                FRecycle(ff.pathName());
        }
        for (FileFind ff(game_path); ff();) {
            UID id;
            if (!id.fromFileName(ff.name) || !findElm(id))
                if (ff.name != "Tex")
                    FRecycle(ff.pathName());
        }
        for (FileFind ff(code_path); ff();) {
            UID id;
            if (!id.fromFileName(SkipEnd(ff.name, CodeExt)) || !findElm(id, ELM_CODE))
                if (ff.name != "Base")
                    FRecycle(ff.pathName());
        }
    }
...
bool Project::eraseTexs(bool full) {
    ...
    getTextures(used);
    REPA(texs)
    if (!used.binaryHas(texs[i]))
        erased |= eraseTex(texs[i]); // go from the end because of removal

    if (full) // this doesn't need to return true because those files are not present in project data
    {
        for (FileFind ff(tex_path); ff();) {
            UID id;
            if (!id.fromFileName(ff.name) || !texs.binaryHas(id))
                FRecycle(ff.pathName());
        }
    }
    ...
    used.clear();
    REPA(elms)
    if (ElmMaterial *mtrl_data = elms[i].mtrlData())
    ...
    for (FileFind ff(temp_tex_dynamic_path); ff();) {
        bool tex_used = false;
        UID tex_id;
        if (DecodeFileName(ff.name, tex_id)) {
            tex_used = used.binaryHas(tex_id);
            if (!tex_used)
                eraseTexFormats(tex_id);
        }
        if (!tex_used)
            FDelFile(ff.pathName());
    }
...
void Project::eraseWorldAreaObjs(C UID &world_id, C VecI2 &area_xy) {
    ...
    Chunks chunks;
    chunks.load(name, WorldAreaSync);
...
VecI2 Project::TextVecI2Check(cchar *name) {
    VecI2 v = TextVecI2(name);
    return (Equal(name, S8 + v.x + ',' + v.y) || Equal(name, S8 + v.x + ", " + v.y)) ? v : VecI2(INT_MAX, INT_MAX); // if name doesn't match, then return invalid VecI2
}
bool Project::eraseWorlds(bool full) {
    ...
    REPA(world_vers) // iterate all worlds
    {
        ...
        if (update_areas.elms()) {
            erased = true;
            REPA(update_areas)
            eraseWorldAreaObjs(world_ver.world_id, update_areas[i]);
            update_areas.clear();
        }
        if (full) // this doesn't need to return true because those files are not present in project data
        {
            { // areas
                ...
                for (FileFind ff(editAreaPath(world_ver.world_id)); ff();) {
                    VecI2 area_xy = TextVecI2Check(ff.name);
                    if (!world_ver.areas.find(area_xy))
                        FRecycle(ff.pathName());
                }
                for (FileFind ff(gameAreaPath(world_ver.world_id)); ff();) {
                    VecI2 area_xy = TextVecI2Check(ff.name);
                    if (!world_ver.areas.find(area_xy))
                        FRecycle(ff.pathName());
                }
...
void Project::quickUpdateVersion(int ver) // this is called inside 'load', it occurs when opening projects and loading from ProjectPackage file, we can't modify files here !!
{
    ...
    if (ver <= 21) // there was a bug in 'duplicate' which did not set 'mesh_id' correctly, so fix it
        REPA(elms) {
            Elm &elm = elms[i];
            if (ElmMesh *mesh_data = elm.meshData()) {
                if (Elm *skel_elm = findElm(mesh_data->skel_id))
                    if (ElmSkel *skel_data = skel_elm->skelData())
                        skel_data->mesh_id = elm.id;
                if (Elm *phys_elm = findElm(mesh_data->phys_id))
                    if (ElmPhys *phys_data = phys_elm->physData())
                        phys_data->mesh_id = elm.id;
            }
        }
    if (ver <= 45) // 'ElmMesh' didn't have 'obj_id'
        REPA(elms) {
            ...
            if (ElmMesh *mesh_data = mesh_elm.meshData())
                REPA(elms)
            if (ElmObj *obj_data = elms[i].objData())
                if (obj_data->mesh_id == mesh_elm.id) {
                    mesh_data->obj_id = elms[i].id;
                    break;
                }
        }
        ...
        REPA(elms) {
            ...
            case ELM_MTRL:
                if (C ElmMaterial *mtrl_data = elm.mtrlData()) {
                    if (mtrl_data->base_1_tex.valid() && texs.binaryHas(mtrl_data->base_1_tex))
                        tex_update.binaryInclude(mtrl_data->base_1_tex);
                    if (mtrl_data->detail_tex.valid() && texs.binaryHas(mtrl_data->detail_tex))
                        tex_update.binaryInclude(mtrl_data->detail_tex);
                }
                break;

            case ELM_WATER_MTRL:
                if (C ElmWaterMtrl *water_mtrl_data = elm.waterMtrlData()) {
                    if (water_mtrl_data->base_1_tex.valid() && texs.binaryHas(water_mtrl_data->base_1_tex))
                        tex_update.binaryInclude(water_mtrl_data->base_1_tex);
                }
                break;
            }
        }
...
void Project::updateVersion(int ver, bool this_project, C MemPtr<UID> &elm_ids) // if 'elm_ids' is null, then all elements are processed, this is called inside 'open' or after copying elements to project
{
    ...
    {
        REPA(elms)
        if (elms[i].type == ELM_SKEL) {
            Str path = gamePath(elms[i]);
            if (f.read(path) && Compress(f, temp.writeMem(), ClientNetworkCompression, ClientNetworkCompressionLevel)) {
                f.del();
                temp.pos(0);
                SafeOverwrite(temp, path);
            }
        }
    }
    ...
    if (ver <= 48) {
        ...
        if (Elm *elm = (elm_ids ? findElm(elm_ids[i]) : &elms[i]))
            switch (elm->type) {
            case ELM_CODE:
                if (ver <= 48) // in ver 48 and below, codes were stored in "Edit" folder without extension
                {
                    Code code;
                    if (code.load(editPath(*elm))) {
                        if (SaveCode(code.current, codePath(*elm)))
                            FDelFile(editPath(*elm));
                        if (!IsServer) {
                            if (code.base.is())
                                SaveCode(code.base, codeBasePath(*elm));
                            else
                                FDelFile(codeBasePath(*elm));
                        }
                    }
                }
                break;
                        ...
                        if (src->read(gamePath(*elm))) {
                            ...
                            if (src) {
                                ...
                                if (anim.load(*src)) {
                                    ...
                                    src->pos(0);
                                    if (compress_anim)
                                        if (Compress(*src, temp.writeMem(), ClientNetworkCompression, ClientNetworkCompressionLevel)) {
                                            src = &temp;
                                            src->pos(0);
                                        } else
                                            src = null;
                                    if (src)
                                        SafeOverwrite(*src, gamePath(*elm));
                                }
                            }
                        }
                    }
        ...
        if (Elm *elm = (elm_ids ? findElm(elm_ids[i]) : &elms[i]))
            ...
            case ELM_SKEL:
                if (ver <= 47) {
                    ...
                    if (src->read(gamePath(*elm))) {
                        ...
                        if (skel.load(*src)) {
                            if (ver <= 47)
                                skel.setBoneTypes(); // !! we can call it only if we've adjusted animations to new bone types above with the same 'ver' check !!
                            ...
                            if (ver <= 21 && !FExistSystem(editPath(*elm))) // if there's no skeleton edit version
                            {
                                ...
                                if (ElmSkel *skel_data = elm->skelData())
                                    skel.transform(~skel_data->transform()); // !! transform after saving game version, and before creating edit version !!
                                EditSkeleton edit;
                                edit.create(skel);
                                edit.save(f.writeMem());
                                src = &f;
                                src->pos(0);
                                if (compress_skel)
                                    if (Compress(*src, temp.writeMem(), ClientNetworkCompression, ClientNetworkCompressionLevel)) {
                                        ...
                                        src = null;
                                if (src)
                                    SafeOverwrite(*src, editPath(*elm));
                            }
                        }
                    }
                }
            ...
            case ELM_MTRL:
                if (ver <= 46)
                    if (ElmMaterial *mtrl_data = elm->mtrlData()) // ver 46 and below used '|' as separator, and didn't have some 'ElmMaterial' members set
                    {
                        EditMaterial edit;
                        if (edit.load(editPath(elm->id))) {
                            if (ver <= 46) {
...
void Project::textData(bool on) {
    if (text_data != on) {
        text_data = on;
        if (save()) {
            if (!text_data && path.is())
                FDelFile(path + "Data.txt"); // if disabled text_data then delete the text file
        }
    }
}
void Project::meshSetAutoTanBin(Elm &elm, C MaterialPtr &material) {
    if (elm.type == ELM_MESH) {
        Mesh mesh;
        if (mesh.load(gamePath(elm.id))) {
            bool changed = false;
            REP(mesh.lods()) {
                MeshLod &lod = mesh.lod(i);
                REPA(lod) {
                    MeshPart &part = lod.parts[i];
                    if (!material || HasMaterial(part, material)) {
                        MESH_FLAG flag = ((part.base.flag() | part.render.flag()) & VTX_TAN_BIN);
                        part.setAutoTanBin();
                        if (flag != ((part.base.flag() | part.render.flag()) & VTX_TAN_BIN))
                            changed = true;
                    }
                }
            }
            if (changed) {
                Save(mesh, gamePath(elm.id));
                savedGame(elm);
            }
        }
    }
        ...
        REPA(elms) {
            ...
            if (C ElmMesh *data = elm.meshData())
                if (data->mtrl_ids.binaryHas(mtrl_id)) {
                    if (!material)
                        material = gamePath(mtrl_id); // load only when needed
    ...
    if (ElmPhys *phys_data = phys_elm.physData())
        if (PhysBodyPtr phys = PhysBodyPtr().get(gamePath(phys_elm.id))) {
            PhysMtrl *mtrl = PhysMtrls(gamePath(phys_data->mtrl_id));
            {
                ...
                phys->density = phys_data->density;
                phys->material = mtrl;
            }
            Save(*phys, gamePath(phys_elm.id));
            savedGame(phys_elm);
        }
}
void Project::skelTransformChanged(C UID &skel_id) {
    if (skel_id.valid())
        REPA(elms) {
            Elm &elm = elms[i];
            if (ElmAnim *data = elm.animData())
                if (data->skel_id == skel_id)
                    setAnimTransform(elm); // transform all animations linked with this skeleton
        }
}
void Project::skelChanged(Elm &skel_elm) {
    if (skel_elm.type == ELM_SKEL) {
        REPA(elms) {
            ...
            case ELM_MESH: {
                UID body_skel;
                getMeshSkels(elm.meshData(), null, &body_skel);
                if (body_skel == skel_elm.id)
                    uses_skel = true; // if mesh uses the skeleton
            } break;
            ...
            if (ElmMesh *mesh_data = mesh_elm->meshData())
                if (skel_data->transform != mesh_data->transform)
                    if (Skeleton *skel = Skeletons.get(gamePath(skel_elm.id))) {
                        ...
                        Save(*skel, gamePath(skel_elm.id));
                        savedGame(skel_elm);
...
void Project::meshTransformChanged(Elm &mesh_elm, bool body_changed) {
    Memt<UID> processed;
    meshTransformChanged(mesh_elm, body_changed, processed);
}
        ...
        if (ElmMesh *mesh_data = mesh_elm.meshData()) {
            ...
            if (Elm *body_elm = findElm(mesh_data->body_id))
                if (ElmMesh *body_mesh_data = body_elm->meshData())
                    if (mesh_data->transform != body_mesh_data->transform) {
                        mesh_data->transform = body_mesh_data->transform;
                        transform_changed = true;
                    }
            if (body_changed || transform_changed)
                makeGameVer(mesh_elm); // have to make game mesh if body changes (because we will have a new target skeleton) or transform changes

            // set skel+phys transforms according to this transform
            if (Elm *skel_elm = findElm(mesh_data->skel_id))
                setSkelTransform(*skel_elm);
            if (Elm *phys_elm = findElm(mesh_data->phys_id))
                setPhysTransform(*phys_elm);

            // set cloths that use this mesh as a body
            REPA(elms) {
                Elm &elm = elms[i];
                if (ElmMesh *cloth_mesh_data = elm.meshData())
                    if (cloth_mesh_data->body_id == mesh_elm.id)
                        meshTransformChanged(elm, body_changed, processed);
            }
...
void Project::rebuildEmbedForObj(C UID &obj) {
    Memt<UID> objs, exts;
    objs.add(obj);
...
void Project::rebuildPathsForObj(C UID &obj, bool only_not_ovr) {
    Memt<UID> objs, exts;
    objs.add(obj);
    getExtendedObjs(objs, exts);
    rebuildPathsForObjs(exts, only_not_ovr);
}
...
void Project::verifyPathsForObjClass(C UID &obj_class) {
    if (obj_class.valid()) {
        ...
        REPA(elms) // iterate all objects
        {
            Elm &elm = elms[i];
            if (ElmObj *data = elm.objData())
                if (data->base_id == obj_class && !data->ovrPath()) // if they have this base and don't override path
                    if (Elm *mesh_elm = findElm(data->mesh_id))
                        if (ElmMesh *mesh_data = mesh_elm->meshData()) // if has mesh
                            if (Elm *phys_elm = findElm(mesh_data->phys_id))
                                if (ElmPhys *phys_data = phys_elm->physData())
                                    if (phys_data->hasBody()) // if has phys
                                    {
...
void Project::physChanged(Elm &phys) {
    ...
    if (ElmPhys *phys_data = phys.physData())
        REPA(elms) // iterate all objects
        {
            Elm &obj = elms[i];
            if (ElmObj *obj_data = obj.objData())
                if (obj_data->mesh_id == phys_data->mesh_id) // check if object mesh matches phys mesh
                {
                    makeGameVer(obj);
...
void Project::objChanged(Elm &obj) {
    {
        CacheLock cl(EditObjects);
        REPA(EditObjects)
        EditObjects.lockedData(i).updateBase(edit_path);
    }
    {
        CacheLock cl(Objects);
        REPA(Objects)
        Objects.lockedData(i).updateBase();
    }
}
...
void Project::rebuildWorldAreas(Elm &elm, uint flag) {
    if (elm.type == ELM_WORLD && flag)
        if (WorldVer *world_ver = worldVerGet(elm.id))
            REPA(world_ver->areas)
    world_ver->rebuildAreaNeighbor(world_ver->areas.lockedKey(i), flag);
}
...
Heightmap *Project::hmGet(C UID &world_id, C VecI2 &area_xy, Heightmap &temp) {
    if (world_id.valid()) {
        if (LoadEditHeightmap(editAreaPath(world_id, area_xy), temp, game_path))
            return &temp;
    }
    return null;
}
uint Project::hmUpdate(C UID &world_id, C VecI2 &area_xy, uint area_sync_flag, C AreaVer &src_area, Heightmap &src_hm) {
    if (world_id.valid() && area_sync_flag)
        if (WorldVer *world_ver = worldVerRequire(world_id)) {
            ...
            Chunks chunks;
            chunks.load(name, WorldAreaSync);
            Chunk *chunk = chunks.findChunk("Heightmap");
            if (!chunk)
                chunk = &chunks.chunks.New();
            else if (chunk->ver == 0)
                dest_hm.load(File().readMem(chunk->data(), chunk->elms()), game_path);

            if (uint synced = dest_area->sync(src_area, dest_hm, src_hm, area_sync_flag)) {
                world_ver->changed = true;
                world_ver->rebuildArea(area_xy, synced);
                File temp;
                dest_hm.save(temp.writeMem(), game_path);
                temp.pos(0);
                chunk->create("Heightmap", 0, temp);
                createWorldPaths(world_id);
                chunks.save(name, WorldAreaSync);
                return synced;
            }
...
Heightmap *Project::hmObjGet(C UID &world_id, C VecI2 &area_xy, Heightmap &temp, Memc<ObjData> &objs, bool get_hm, bool get_objs) {
    if (world_id.valid()) {
        if (LoadEdit(editAreaPath(world_id, area_xy), get_hm ? &temp : null, get_objs ? &objs : null, game_path, edit_path))
            return get_hm ? &temp : null;
    }
    return null;
}
bool Project::syncElm(Elm &elm, Elm &src, File &src_data, File &src_extra, bool sync_long, bool &elm_newer_src, bool &src_newer_elm) {
    ...
    if (elm.type == src.type)
        switch (elm.type) {
        case ELM_MTRL: {
            ElmMaterial &mtrl_data = *elm.mtrlData(), &src_mtrl_data = *src.mtrlData();
            if (has_file) // has 'EditMaterial'
            {
                EditMaterial mtrl, src_mtrl;
                mtrl.load(path);
                src_mtrl.load(src_data);
                if (file_changed = mtrl.sync(src_mtrl)) {
                    mtrl_data.from(mtrl);
                    Save(mtrl, path);
                    makeGameVer(elm);
                }
                if (mtrl_data.equal(src_mtrl_data) && mtrl.equal(src_mtrl))
                    mtrl_data.ver = src_mtrl_data.ver;
        ...
        case ELM_IMAGE: {
            ...
            if (has_file && image_data.syncFile(src_image_data)) {
                file_changed = true;
                SafeOverwrite(src_data, path);
            } // we're saving to Edit so there's no need to call 'SavedImage'
            if ((data_changed & CHANGE_AFFECT_FILE) || file_changed)
                makeGameVer(elm);
        } break;
        ...
        case ELM_IMAGE_ATLAS: {
            ...
            if (has_file && image_data.syncFile(src_image_data)) {
                file_changed = true;
                if (SafeOverwrite(src_data, path)) {
                    SavedImageAtlas(path);
                    savedGame(elm, game_path);
                }
            }
        } break;
        ...
        case ELM_ICON_SETTS: {
            if (has_file) // has 'IconSettings'
            {
                ...
                IconSettings icon, src_icon;
                icon.load(path);
                src_icon.load(src_data);
                if (file_changed = icon.sync(src_icon)) {
                    icon_data.from(icon);
                    Save(icon, path);
                    makeGameVer(elm);
                }
                if (icon_data.equal(src_icon_data) && icon.equal(src_icon))
                    icon_data.ver = src_icon_data.ver;
                else if (data_changed || file_changed)
                    icon_data.newVer();
        ...
        case ELM_ICON: {
            if (has_file && elm.iconData()->syncFile(*src.iconData())) {
                file_changed = true;
                if (SafeOverwrite(src_data, path)) {
                    SavedImage(path);
                    savedGame(elm, game_path);
        ...
        case ELM_FONT: // for fonts we first send short: data+edit (ElmFont+EditFont), if EditFont is newer, then we need to request long: data+edit+game
        {
            if (has_file) // has 'EditFont'
            {
                ...
                EditFont font, src_font;
                font.load(path);
                src_font.load(src_data);
                ...
                {              // we can synchronize 'edit' only if we have 'game', because both of them are tied together and saved at the same time
                    if (file_changed = font.sync(src_font)) {
                        font_data.from(font);
                        Save(font, path);
                ...
                if (font_data.equal(src_font_data) && font.equal(src_font))
                    font_data.ver = src_font_data.ver;
                else if (data_changed || file_changed)
                    font_data.newVer();
        ...
        case ELM_PANEL_IMAGE: {
            if (has_file) // has 'EditPanelImage'
            {
                ...
                EditPanelImage panel_image, src_panel_image;
                panel_image.load(path);
                src_panel_image.load(src_data);
                if (sync_long) // has 'PanelImage'
                {              // we can synchronize 'edit' only if we have 'game', because both of them are tied together and saved at the same time
                    if (file_changed = panel_image.sync(src_panel_image)) {
                        panel_image_data.from(panel_image);
                        Save(panel_image, path);
                        if (SafeOverwrite(src_extra, game_path)) {
                            SavedPanelImage(game_path);
                            savedGame(elm, game_path);
                        }
                    }
                } else {
                ...
                if (panel_image_data.equal(src_panel_image_data) && panel_image.equal(src_panel_image))
                    panel_image_data.ver = src_panel_image_data.ver;
                else if (data_changed || file_changed)
                    panel_image_data.newVer();
        ...
        case ELM_TEXT_STYLE: {
            if (has_file) // has 'EditTextStyle'
            {
                ...
                EditTextStyle ts, src_ts;
                ts.load(path);
                src_ts.load(src_data);
                if (file_changed = ts.sync(src_ts)) {
                    ts_data.from(ts);
                    Save(ts, path);
                    makeGameVer(elm);
                }
                if (ts_data.equal(src_ts_data) && ts.equal(src_ts))
                    ts_data.ver = src_ts_data.ver;
                else if (data_changed || file_changed)
                    ts_data.newVer();
        ...
        case ELM_PANEL: {
            if (has_file) // has 'EditPanel'
            {
                ...
                EditPanel panel, src_panel;
                panel.load(path);
                src_panel.load(src_data);
                if (file_changed = panel.sync(src_panel)) {
                    panel_data.from(panel);
                    Save(panel, path);
                    makeGameVer(elm);
                }
                if (panel_data.equal(src_panel_data) && panel.equal(src_panel))
                    panel_data.ver = src_panel_data.ver;
                else if (data_changed || file_changed)
                    panel_data.newVer();
        ...
        case ELM_GUI_SKIN: {
            if (has_file) // has 'EditGuiSkin'
            {
                ...
                EditGuiSkin gui_skin, src_gui_skin;
                gui_skin.load(path);
                src_gui_skin.load(src_data);
                if (file_changed = gui_skin.sync(src_gui_skin)) {
                    gui_skin_data.from(gui_skin);
                    Save(gui_skin, path);
                    makeGameVer(elm);
                }
                if (gui_skin_data.equal(src_gui_skin_data) && gui_skin.equal(src_gui_skin))
                    gui_skin_data.ver = src_gui_skin_data.ver;
                else if (data_changed || file_changed)
        ...
        case ELM_ENV: {
            if (has_file) // has 'EditEnv'
            {
                ...
                EditEnv env, src_env;
                env.load(path);
                src_env.load(src_data);
                if (file_changed = env.sync(src_env)) {
                    env_data.from(env);
                    Save(env, path);
                    makeGameVer(elm);
                }
                if (env_data.equal(src_env_data) && env.equal(src_env))
                    env_data.ver = src_env_data.ver;
                else if (data_changed || file_changed)
                    env_data.newVer();
        ...
        case ELM_MESH: {
            ...
            if (has_file && mesh_data.syncFile(src_mesh_data)) {
                if (!IsServer) // check if mesh existence has changed for possible 'makeGameVer' of object which depends on mesh existence, this doesn't need to be done on the server
                {
                    ...
                    Load(original, path, T.game_path);
                    updated.load(src_data, T.game_path);
                    src_data.pos(0);
                    if (OverrideMeshSkel(&original, null) != OverrideMeshSkel(&updated, null) || (data_changed & CHANGE_AFFECT_OBJ) // if change affects object
                    )
            ...
            if ((data_changed & CHANGE_AFFECT_FILE) || file_changed) {
                src_data.pos(0);
                makeGameVer(elm, has_file ? &src_data : null);
            }
            if (rebuild)
                makeGameVer(*rebuild); // rebuild after 'makeGameVer' of the mesh
        } break;
        ...
        case ELM_SKEL: {
            ...
            if (has_file && skel_data.syncFile(src_skel_data)) {
                Elm *rebuild = null;
                if (!IsServer) // check if skeleton existence has changed for possible 'makeGameVer' of object which depends on skeleton existence, this doesn't need to be done on the server
                {
                    Skeleton original, updated;
                    original.load(game_path);
                    updated.load(src_extra);
                    src_extra.pos(0);
                    if (OverrideMeshSkel(null, &original) != OverrideMeshSkel(null, &updated))
                        rebuild = skelToObjElm(&elm); // set object of this skeleton to be rebuilt
                }
                file_changed = true;
                if (SafeOverwrite(src_data, path))
                    SavedEditSkel(path);
                if (SafeOverwrite(src_extra, game_path)) {
                    ...
                    savedGame(elm, game_path);
                    if (rebuild)
                        makeGameVer(*rebuild); // rebuild after saving the skeleton
                }
            }
        } break;
        ...
        case ELM_PHYS: {
            ...
            if (has_file && phys_data.syncFile(src_phys_data)) {
                Elm *rebuild = null;
                if (!IsServer) // check if phys body existence has changed for possible 'makeGameVer' of object which depends on phys body existence, this doesn't need to be done on the server
                {
                    PhysBody original, updated;
                    original.load(path);
                    updated.load(src_data, T.game_path);
                    src_data.pos(0);
                    if (OverridePhys(&original) != OverridePhys(&updated))
                        rebuild = physToObjElm(&elm); // set object of this phys body to be rebuilt
                }
                file_changed = true;
                if (SafeOverwrite(src_data, path)) {
                    SavedPhys(path);
                    savedGame(elm, game_path);
                    if (rebuild)
                        makeGameVer(*rebuild); // rebuild after saving the phys body
                }
            }
        } break;
        ...
        case ELM_ANIM: {
            ...
            if (has_file && anim_data.syncFile(src_anim_data)) {
                file_changed = true; /*anim_data.from(anim);*/
                if (SafeOverwrite(src_data, path)) {
                    SavedAnim(path);
                    savedGame(elm, game_path);
                }
        ...
        case ELM_WORLD: {
            if (data_changed & CHANGE_AFFECT_FILE)
                makeGameVer(elm);
        } break;
        ...
        case ELM_ENUM: {
            ...
            EditEnums enums, src_enums;
            enums.load(path);
            src_enums.load(src_data);
            if (file_changed = enums.sync(src_enums)) {
                enum_data.from(enums);
                Save(enums, path);
                makeGameVer(elm);
            }
            if (enum_data.equal(src_enum_data) && enums.equal(src_enums))
                enum_data.ver = src_enum_data.ver;
            else if (data_changed || file_changed)
                enum_data.newVer();
        ...
        case ELM_OBJ: {
            ...
            EditObject params, src_params;
            if (IsServer)
                Load(params, path, edit_path);
            else
                params = *EditObjectPtr(path);
            src_params.load(src_data, edit_path); // 'EditObject' must use 'edit_path', on client load from cache in case we're editing the file
            if (file_changed = params.sync(src_params, edit_path)) {
                obj_data.from(params);
                Save(params, path, edit_path);
            } // 'EditObject' must use 'edit_path'
            if ((data_changed & CHANGE_AFFECT_FILE) || file_changed)
                makeGameVer(elm);
            if (obj_data.equal(src_obj_data) && params.equal(src_params))
                obj_data.ver = src_obj_data.ver;
            else if (data_changed || file_changed)
                obj_data.newVer();
        ...
        case ELM_GUI: {
            if (has_file && elm.guiData()->syncFile(*src.guiData())) {
                file_changed = true;
                SafeOverwrite(src_data, path);
            }
        } break;
        ...
        case ELM_SOUND: {
            if (has_file && elm.soundData()->syncFile(*src.soundData())) {
                file_changed = true;
                SafeOverwrite(src_data, path);
            }
        } break;
        ...
        case ELM_VIDEO: {
            if (has_file && elm.videoData()->syncFile(*src.videoData())) {
                file_changed = true;
                SafeOverwrite(src_data, path);
            }
        } break;
        ...
        case ELM_FILE: {
            if (has_file && elm.fileData()->syncFile(*src.fileData())) {
                file_changed = true;
                SafeOverwrite(src_data, path);
            }
        } break;
    ...
    if (world_id.valid())
        if (WorldVer *world_ver = worldVerRequire(world_id))
            if (AreaVer *dest_area = world_ver->areas.get(area_xy)) {
                ...
                {
                    if (syncObj(world_id, area_xy, src_objs, null, local_objs_newer)) {
                        dest_area->obj_ver = src_area.obj_ver;
                        world_ver->changed = true;
                    } // if after syncing objects they are in equal state, then set same 'obj_ver' as the source
                }
                ...
                {
                    hmDel(world_id, area_xy, &src_area.hm_removed_time);
                    return AREA_SYNC_REMOVED;
                }
                ...
                {
                    uint do_sync_flag = (((area_sync_flag & AREA_SYNC_HEIGHT) && src_area.hm_height_time > dest_area->hm_height_time) ? AREA_SYNC_HEIGHT : 0) | (((area_sync_flag & AREA_SYNC_MTRL) && src_area.hm_mtrl_time > dest_area->hm_mtrl_time) ? AREA_SYNC_MTRL : 0) | (((area_sync_flag & AREA_SYNC_COLOR) && src_area.hm_color_time > dest_area->hm_color_time) ? AREA_SYNC_COLOR : 0);
                    if (do_sync_flag)
                        return hmUpdate(world_id, area_xy, area_sync_flag, src_area, src_hm); // if any element is newer
                }
            }
    return 0;
}
...
Project::AreaSyncObj::~AreaSyncObj() {
    if (changed) {
        ...
        SaveEditObject(chunks, objs, edit_path);
        chunks.save(chunk_edit_path, WorldAreaSync); // save edit
        if (!IsServer) {
            chunks.load(chunk_game_path, WorldAreaSync);
            SaveGameObject(chunks, objs, *project, *world_ver);
            chunks.save(chunk_game_path, WorldAreaSync);
        } // save game
    }
}
bool Project::AreaSyncObj::Create(AreaSyncObj &area, C VecI2 &area_xy, ptr asod_ptr) {
    ...
    if (Chunk *chunk = area.chunks.findChunk("Object"))
        LoadEditObject(chunk->ver, File().readMem(chunk->data(), chunk->elms()), area.objs, area.edit_path); // load edit objects
    return true;
}
                ...
                if (world_data->valid())
                    if (ObjVer *obj_ver = world_ver.obj.find(world_obj_instance_id)) // get obj ver
                    {
                        ...
                        if (!obj_ver->removed() && obj_ver->terrain(T) && getObjBox(obj_ver->elm_obj_id, box)) // if can be embedded
                        {
                            ...
                            {
                                want_embed = true;
                                want_embed_rect.set(Floor(box.min.xz() / area_size), Floor(box.max.xz() / area_size));
                            }
                        }

                        // rebuild if needed
                        if (is_embed)
                            world_ver.rebuildEmbedObj(*is_embed); // rebuild old areas
                        if (want_embed)
                            world_ver.rebuildEmbedObj(want_embed_rect); // rebuild new areas
                        if (rebuild_game_area_objs && (is_embed != null) != want_embed)
                            world_ver.rebuildGameAreaObjs(area_xy); // if changing embedded state then we need to rebuild game area objects (add or remove the object from area obj list)

                        // setup in world ver (do this after using 'is_embed')
                        {
                            MapLock ml(world_ver.obj_embed); // lock before changing 'obj_embed'
                            if (want_embed)
                                *world_ver.obj_embed(world_obj_instance_id) = want_embed_rect;
                            else
                                world_ver.obj_embed.removeKey(world_obj_instance_id);
                            world_ver.changed = true;
                        }
                    }
}
                    ...
                    if (world_data->valid())
                        if (WaterVer *water_ver = (lake ? world_ver.lakes.find(water_id) : world_ver.rivers.find(water_id))) {
                            ...
                            RectI old_area = water_ver->areas,
                                  new_area(0, -1);
                            Rect rect;
                            if (lake ? (!lake->removed && lake->getRect(rect)) : (!river->removed && river->getRect(rect)))
                                new_area.set(Floor(rect.min / area_size), Floor(rect.max / area_size));

                            // rebuild if needed
                            if (old_area.valid())
                                world_ver.rebuildWater(old_area);
                            if (new_area.valid())
                                world_ver.rebuildWater(new_area);

                            // setup in world ver
                            if (lake) {
                                MapLock ml(world_ver.lakes);
                                water_ver->areas = new_area;
                                world_ver.changed = true;
                            } // lock before changing 'world_ver'
                            if (river) {
                                MapLock ml(world_ver.rivers);
                                water_ver->areas = new_area;
                                world_ver.changed = true;
                            } // lock before changing 'world_ver'
                        }
    ...
    if (world_id.valid())
        if (WorldVer *world_ver = worldVerRequire(world_id)) {
            ...
            REPA(objs) {
                ...
                {
                    if (obj_modified)
                        (*obj_modified)(target_area->xy)->New() = obj;
                    ...
                    if (!IsServer)
                        rebuildEmbedObj(obj.id, target_area->xy, *world_ver, false); // call after 'changedObj' (doesn't need to be performed on the server), 'rebuild_game_area_objs=false' because we're saving game area here anyway
                    if (!IsServer && obj.physPath())
                ...
                if (cur_area->objs[i].id == obj.id) // found that object
                {
                    ...
                    PhysPath old_phys;
                    if (!IsServer)
                        old_phys = cur_obj.physPath(); // don't check for 'physPath' on server because loading 'Object' may fail (doesn't need to be performed on the server)
                    if (cur_obj.sync(obj, edit_path))  // if performed any change
                    {
                        bool changed_matrix = (cur_obj.matrix_time > old_matrix_time),
                             changed_embed = false;
                        if (!IsServer)
                            changed_embed = (cur_obj.terrainObj() != old_terrain || changed_matrix); // if changed terrain or changed matrix (doesn't need to be performed on the server)
                        bool changed_phys_path = false;
                        if (!IsServer)
                            changed_phys_path = (cur_obj.physPath() != old_phys || (changed_matrix && (old_phys || cur_obj.physPath()))); // if changed phys, or if changed matrix and have phys (doesn't need to be performed on the server)
                        AreaSyncObj &new_area = (changed_matrix ? *target_area : *cur_area);                                              // check if received newer position
                        if (obj_modified)
                            (*obj_modified)(new_area.xy)->New() = cur_obj;
                        cur_area->changed = true;
                        world_ver->changed = true;
                        createWorldPaths(world_id);
                        world_ver->changedObj(cur_obj, new_area.xy); // call before 'rebuildEmbedObj'

                        if (changed_embed)
                            rebuildEmbedObj(cur_obj.id, new_area.xy, *world_ver, false); // call after 'changedObj' and before 'rebuildPaths', 'rebuild_game_area_objs=false' because we're saving game area here anyway
                        if (changed_phys_path)
                            world_ver->rebuildPaths(obj.id, cur_area->xy); // call after 'rebuildEmbedObj' for old area
                        if (cur_area != target_area && changed_matrix)     // we're moving to different area
                        {
                            target_area->changed = true;
                            if (changed_phys_path)
                                world_ver->rebuildPaths(obj.id, target_area->xy); // call after 'rebuildEmbedObj' for new area
                            Swap(target_area->objs.New(), cur_obj);
                            cur_area->objs.remove(i); // move to new area and remove from old area
                        }
                    }
                    break;
                }
            ...
            if (local_newer)
                GetNewer(target_area->objs, objs, *local_newer);
            ...
            if (Version *waypoint_ver = world_ver->waypoints.get(waypoint_id))
                if (src_ver != *waypoint_ver) {
                    ...
                    if (changed) {
                        ...
                        if (!IsServer) {
                            Game::Waypoint w;
                            Str game = gameWaypointPath(world_id, waypoint_id);
                            if (waypoint.copyTo(w))
                                Save(w, game);
                            else
                                FDelFile(game);
                        } // make game
    ...
    if (mini_map_id.valid())
        if (MiniMapVer *mini_map_ver = miniMapVerRequire(mini_map_id))
            if (settings_time > mini_map_ver->time) {
                ...
                mini_map_ver->settings = settings;
                settings.save(gamePath(mini_map_id).tailSlash(true) + "Settings");
            ...
            if (image_time == mini_map_ver->time) {
                ...
                if (image_data.is()) // if image data exists then save it
                {
                    ...
                    if (SafeOverwrite(image_data, image_name))
                        if (mini_map_ver->images.binaryInclude(image_xy))
                            mini_map_ver->changed = true;
...
bool Project::newerSettings(C Project &src) C {
    REPA(compress_time)
    if (compress_time[i] > src.compress_time[i])
        return true;
...
bool Project::syncSettings(C Project &src) {
    ...
    REPA(compress_time)
    if (Sync(compress_time[i], src.compress_time[i])) {
        changed = true;
...
bool Project::save(File &f, bool network, SAVE_DATA mode) C {
    ...
    if (network)
        f << id; // ID needed only for network, on local it is obtained from folder name
    f << name;
    if (!network)
        f << synchronize;
    ...
    if (mode >= SAVE_SETTINGS) {
        ...
        if (!network) {
            f << text_data << app_id << hm_mtrl_id << water_mtrl_id;
            FREPA(mtrl_brush_id)
            f << mtrl_brush_id[i];
        }
        ...
        if (mode >= SAVE_ALL) {
            ...
            FREPAO(elms).save(f, network, network);
            f.cmpUIntV(texs.elms());
            FREPA(texs)
            f << texs[i];
            ...
            {
                f.cmpUIntV(texs_update_base1.elms());
                FREPA(texs_update_base1)
                f << texs_update_base1[i];
                f.cmpUIntV(texs_remove_srgb.elms());
                FREPA(texs_remove_srgb)
                f << texs_remove_srgb[i];
            }
        }
    }
    return f.ok();
...
LOAD_RESULT Project::load(File &f, int &ver, bool network, SAVE_DATA mode) {
    ...
    case 4: {
        ...
        if (mode >= SAVE_SETTINGS) {
            // 'ProjectVersion'
            ver = f.decUIntV();
            if (ver > ProjectVersion)
                goto newer; // if project requires a newer version then don't load it
            ...
            case 2: {
                f >> cipher >> cipher_key >> compress_type >> compress_level >> material_simplify >> tex_downsize;
                ASSERT(ELMS(tex_downsize) == 2 && ELMS(compress_type) == 2 && ELMS(compress_level) == 2 && ELMS(compress_time) == 2);
                f >> cipher_time >> cipher_key_time >> compress_time >> material_simplify_time >> tex_downsize_time;
                if (!network) {
                    f >> text_data >> app_id >> hm_mtrl_id >> water_mtrl_id;
                    FREPA(mtrl_brush_id)
            ...
            case 1: {
                f >> cipher >> cipher_key >> compress_type[PCP_DEFAULT] >> compress_level[PCP_DEFAULT] >> material_simplify >> tex_downsize;
                ...
                REP(PCP_NUM) {
                    compress_type[i] = compress_type[PCP_DEFAULT];
                    compress_level[i] = compress_level[PCP_DEFAULT];
                    compress_time[i] = compress_time[PCP_DEFAULT];
                }
            } break;

            case 0: {
                f >> cipher >> cipher_key >> compress_type[PCP_DEFAULT] >> compress_level[PCP_DEFAULT] >> material_simplify >> cipher_time >> cipher_key_time >> compress_time[PCP_DEFAULT] >> compress_time[PCP_DEFAULT] >> material_simplify_time;
                if (!network) {
                    f >> text_data >> app_id >> hm_mtrl_id >> water_mtrl_id;
                    FREPA(mtrl_brush_id)
                    f >> mtrl_brush_id[i];
            ...
            if (mode >= SAVE_ALL)
                ...
                case 1: {
                    elms.setNum(f.decUIntV());
                    FREPA(elms)
                    if (!elms[i].load(f, network, network)) {
                        if (f.ok())
                            goto newer;
                        goto error;
                    }
                    texs.setNum(f.decUIntV());
                    FREPA(texs)
                    f >> texs[i];

                    // update
                    if (!network) {
                        texs_update_base1.setNum(f.decUIntV());
                        FREPA(texs_update_base1)
                        f >> texs_update_base1[i];
                        texs_remove_srgb.setNum(f.decUIntV());
                        FREPA(texs_remove_srgb)
                        f >> texs_remove_srgb[i];
                    }
                } break;
...
bool Project::loadOldSettings(File &f) {
    if (f.getUInt() == CC4('P', 'R', 'S', 'T')) {
        ...
        case 3: {
            ...
            GetStr2(f, name);
            f >> cipher >> encrypt_key >> compress_type[PCP_DEFAULT] >> compress_level[PCP_DEFAULT] >> material_simplify >> cipher_time >> cipher_key_time >> compress_time[PCP_DEFAULT] >> compress_time[PCP_DEFAULT] >> material_simplify_time;
            f >> app_id >> hm_mtrl_id >> water_mtrl_id;
            FREPA(mtrl_brush_id)
            f >> mtrl_brush_id[i];
            REP(PCP_NUM) {
                compress_type[i] = compress_type[PCP_DEFAULT];
        ...
        case 2: {
            byte max_tex_size;
            TimeStamp max_tex_size_time;
            f >> proj_id;
            GetStr2(f, name);
            f >> cipher >> encrypt_key >> compress_type[PCP_DEFAULT] >> compress_level[PCP_DEFAULT] >> material_simplify >> max_tex_size >> cipher_time >> cipher_key_time >> compress_time[PCP_DEFAULT] >> compress_time[PCP_DEFAULT] >> material_simplify_time >> max_tex_size_time;
            f >> app_id >> hm_mtrl_id >> water_mtrl_id;
            FREPA(mtrl_brush_id)
            f >> mtrl_brush_id[i];
            REP(PCP_NUM) {
...
bool Project::loadOldSettings(C Str &name) {
    File f;
    return f.read(name) && loadOldSettings(f);
}
...
void Project::save(MemPtr<TextNode> nodes) C {
    ...
    if (save_all || synchronize)
        nodes.New().set("Synchronize", synchronize);
    if (save_all || cipher)
        nodes.New().set("Encrypt", cipher);
    REPA(cipher_key)
    if (save_all || cipher_key[i]) {
        nodes.New().setRaw("EncryptionKey", cipher_key);
        break;
    }
    if (save_all || compress_type[PCP_DEFAULT])
        nodes.New().set("Compress", CompressionName(compress_type[PCP_DEFAULT]));
    if (save_all || compress_type[PCP_MOBILE])
        nodes.New().set("CompressMobile", CompressionName(compress_type[PCP_MOBILE]));
    ...
    if (save_all || tex_downsize[TSP_MOBILE])
        nodes.New().set("TexDownsizeMobile", tex_downsize[TSP_MOBILE]);
    if (save_all || tex_downsize[TSP_SWITCH])
    ...
    if (save_all || texs.elms()) {
        ...
        FREPA(texs)
        node.nodes.New().setValueFN(texs[i]); // save in order
    }
}
LOAD_RESULT Project::load(C MemPtr<TextNode> &nodes, int &ver, Str &error) // !! this assumes that binary was already loaded and 'ver' already set !!
{
    ...
    FREPA(nodes) {
        ...
        if (node.name == "Version") {
            ...
            node.getValue(name);
        else if (node.name == "Synchronize")
            synchronize = node.asBool1();
        else if (node.name == "Encrypt")
            cipher = (CIPHER_TYPE)node.asInt();
        else if (node.name == "EncryptionKey")
            node.getValueRaw(cipher_key);
        else if (node.name == "Compress") {
            REP(COMPRESS_NUM)
            if (node.value == CompressionName(COMPRESS_TYPE(i))) {
                compress_type[PCP_DEFAULT] = COMPRESS_TYPE(i);
                break;
            }
        } else if (node.name == "CompressMobile") {
            REP(COMPRESS_NUM)
            if (node.value == CompressionName(COMPRESS_TYPE(i))) {
                compress_type[PCP_MOBILE] = COMPRESS_TYPE(i);
                break;
            }
        } else if (node.name == "CompressLevel")
            compress_level[PCP_DEFAULT] = node.asInt();
        else if (node.name == "CompressLevelMobile")
            compress_level[PCP_MOBILE] = node.asInt();
        else if (node.name == "SimplifyMaterials")
            material_simplify = (MATERIAL_SIMPLIFY)node.asInt();
        else if (node.name == "TexDownsizeMobile")
            tex_downsize[TSP_MOBILE] = node.asInt();
        else if (node.name == "TexDownsizeSwitch")
            tex_downsize[TSP_SWITCH] = node.asInt();
        else if (node.name == "EncryptTime")
            cipher_time.text(node);
        else if (node.name == "EncryptionKeyTime")
            cipher_key_time.text(node);
        else if (node.name == "CompressTime")
            compress_time[PCP_DEFAULT].text(node);
        else if (node.name == "CompressTimeMobile")
            compress_time[PCP_MOBILE].text(node);
        else if (node.name == "SimplifyMaterialsTime")
            material_simplify_time.text(node);
        else if (node.name == "TexDownsizeTime")
            tex_downsize_time.text(node);
            ...
            FREPA(node.nodes) // process in order because they are sorted to avoid moving elements in 'getElm'
            {
                ...
                UID id;
                if (id.fromText(elm_node.name) && id.valid()) {
                    Elm &elm = getElm(id);
                    if (elm.type) {
                        error = S + "Element \"" + elm_node.name + "\" listed more than 1 time";
                        goto error;
                    }
                    if (!elm.load(elm_node, error))
                        goto error;
                } else {
                    error = S + "Invalid Element ID \"" + elm_node.name + '"';
                    goto error;
                }
            }

            FREPA(importing)
            if (Elm *elm = findElm(importing[i]))
                elm->importing(true);
            FREPA(opened)
            if (Elm *elm = findElm(opened[i]))
                elm->opened(true);
        } else if (node.name == "Textures") {
            texs.clear();
            FREPA(node.nodes) // process in order because they are sorted to avoid moving elements in 'texs.binaryInclude'
            {
                UID id;
                if (node.nodes[i].getValue(id) && id.valid())
                    texs.binaryInclude(id); // use 'binaryInclude' in case text file is messed up
                else {
                    error = S + "Invalid Texture ID \"" + node.nodes[i].value + '"';
                    goto error;
                }
            }
        }
...
LOAD_RESULT Project::load(C Str &name, int &ver, SAVE_DATA mode) {
    File f;
    if (f.read(name))
        return load(f, ver, false, mode);
    ver = -1;
    del();
    return LOAD_EMPTY;
}
bool Project::save(C Str &name) C {
    File f;
    save(f.writeMem());
    f.pos(0);
    return SafeOverwrite(f, name);
}
LOAD_RESULT Project::load2(C Str &name, int &ver, SAVE_DATA mode) {
    LOAD_RESULT result = load(name, ver, mode);
    if (result == LOAD_EMPTY || result == LOAD_ERROR)
        result = load(name + ".old", ver, mode);
    return result;
}
bool Project::save2(C Str &name) C {
    if (FExistSystem(name))
        if (!FRename(name, name + ".old"))
            return false;
    return save(name);
}
bool Project::saveTxt(C Str &name) C {
    if (name.is()) {
        ...
        save(data.nodes);
        Str temp = name + "@new";
        if (data.save(temp) && FRename(temp, name))
            return true;
        FDelFile(temp);
    }
    return false;
...
LOAD_RESULT Project::loadTxt(C Str &name, int &ver, Str &error) // !! this assumes that binary was already loaded and 'ver' already set !!
{
    ...
    if (file.read(name)) {
        TextData data;
        if (data.load(file))
            return load(data.nodes, ver, error);
        VecI2 col_line;
        char c = file.posInfo(file.pos() - 1, col_line); // if TextData failed to load, due to unexpected character, then it stopped reading after that character, so get what was last read
        error = S + "Unexpected character '" + c + "', Column: " + (col_line.x + 1) + ", Line: " + (col_line.y + 1) + ", in File: \"" + Str(name).replace(' ', Nbsp) + '"';
        ver = -1;
        del();
        return LOAD_ERROR;
    } else if (FExistSystem(name)) {
        error = S + "Can't access file \"" + Str(name).replace(' ', Nbsp) + "\"";
        ver = -1;
        del();
        return LOAD_ERROR;
    }
    error.clear();
    return LOAD_EMPTY; // here unlike 'load', 'ver' is not cleared and 'del' is not called for LOAD_EMPTY because we assume that binary was already loaded
}
...
bool Project::isProject(C FileFind &ff) // this will only set 'id' and 'name', but not 'path'
{
    if (ff.type == FSTD_DIR) {
        ...
        if (DecodeFileName(ff.name, id)) {
            int ver;
            Str error;
            LOAD_RESULT result = load3(ff.pathName(), ver, error, SAVE_ID_NAME);
            switch (result) {
            case LOAD_OK:
            case LOAD_NEWER:
            case LOAD_LOCKED:
                T.id = id;
                return true; // don't set 'path' because if 'close' is called later then it would save data
            }
        }
    }
    return false;
...
void Project::setIDPath(C UID &id, C Str &path) {
    T.id = id;
    T.path = path;
    T.path.tailSlash(true);
    code_path = T.path + "Code\\";
    code_base_path = code_path + "Base\\";
...
LOAD_RESULT Project::open(C UID &id, C Str &name, C Str &path, Str &error, bool ignore_lock) {
    error.clear();
    UID _id = id;
    Str _name = name, _path = path;
    _path.tailSlash(true); // copy to temp vars in case params are set to this, and would get cleared in 'close'
    ...
    if (!ignore_lock && FExistSystem(lock_name))
        return LOAD_LOCKED; // check if it's already opened, do this at start so the project won't be opened
    File f;
    f.write(lock_name);
    f.del(); // lock it

    int ver;
    LOAD_RESULT result = load3(_path, ver, error);
    if (LoadOK(result)) {
        if (result == LOAD_EMPTY)
            T.name = _name;
        ...
        FDelFile(lock_name); // unlock on error
...
void ProjectHierarchy::addHierarchy(ElmNode &node, int node_i, ElmNode &target, int target_i) {
    if (InRange(node.parent, hierarchy))
        hierarchy[node.parent].children.exclude(node_i); // remove from previous parent
    node.parent = target_i;
    target.children.add(node_i); // add to new parent
    node.added = true;           // mark as added
    floodHierarchy(node);        // add children of node
}
void ProjectHierarchy::setHierarchy() {
    root.clear();
    hierarchy.clear().setNum(elms.elms());
    FREPA(elms) {
        Elm &elm = elms[i];
        int parent = findElmI(elm.parent_id);
        hierarchy[i].parent = parent;
        if (InRange(parent, hierarchy))
            hierarchy[parent].children.add(i);
        else
            root.children.add(i);
    }
    floodHierarchy(root);
    ...
    FREPA(elms) {
        ...
        if (!node.added) // if not yet added
        {
            ...
            for (ElmNode *cur = &node;;) {
                ...
                if (!InRange(parent, elms))
                    goto no_loop;             // if it has no parent
...
bool ProjectHierarchy::contains(C Elm &a, C Elm *b) C // if 'a' contains 'b'
{
    ...
    for (; InRange(bi, hierarchy);) {
        if (bi == ai)
            return true;
        bi = hierarchy[bi].parent;
    }
    return false;
}
...
Str ProjectHierarchy::ElmSrcFileFirst(C Mems<FileParams> &files) {
    FREPA(files) {
        C FileParams &fp = files[i];
        Str path = FFirstUp(fp.name);
        if (path.is())
            return path;
        path = ElmSrcFileFirst(fp.nodes);
        if (path.is())
            return path;
    }
    return S;
    ...
    if (path.is()) {
        ...
        for (;;) {
            ...
            REPA(parent->children) {
                ...
                Elm &elm = elms[child_i];
                if (elm.name == name && ElmVisible(elm.type)) // don't list hidden types
                {
                    found_elm = &elm;
