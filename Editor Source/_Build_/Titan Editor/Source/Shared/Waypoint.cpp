...
/******************************************************************************/
void EditWaypointPoint::setPos(C Vec &pos) {
    T.pos = pos;
    pos_time.getUTC();
}
void EditWaypointPoint::setRemoved(bool removed) {
    T.removed = removed;
    removed_time.getUTC();
}
...
int EditWaypoint::FindI(C Memc<EditWaypointPoint> &waypoints, C UID &waypoint_id) {
    REPA(waypoints)
    if (waypoints[i].id == waypoint_id)
        return i;
    return -1;
}
EditWaypointPoint *EditWaypoint::Find(Memc<EditWaypointPoint> &waypoints, C UID &waypoint_id) {
    int i = FindI(waypoints, waypoint_id);
    return (i >= 0) ? &waypoints[i] : null;
}
...
int EditWaypoint::liveIndex(C UID &waypoint_id) {
    int n = 0;
    FREPA(T)
    if (!T[i].removed) {
        if (T[i].id == waypoint_id)
            return n;
        n++;
    }
    return -1;
...
bool EditWaypoint::newer(C EditWaypoint &src) C {
    if (loop_mode_time > src.loop_mode_time)
        return true;
    if (removed_time > src.removed_time)
        return true;
    if (name_time > src.name_time)
        return true;
    REPA(T) {
        C EditWaypointPoint &e = T[i], *s = src.find(e.id);
        if (!s || e.newer(*s))
            return true;
    }
    return false;
...
bool EditWaypoint::getRect(Rect &rect) C {
    ...
    REPA(T)
    if (!T[i].removed)
        Include(rect, found, T[i].pos.xz());
    return found;
}
bool EditWaypoint::hasExisting() C {
    REPA(T)
    if (!T[i].removed)
        return true;
    return false;
}
UID EditWaypoint::firstExisting() C {
    FREPA(T)
    if (!T[i].removed)
        return T[i].id;
    return UIDZero;
...
UID EditWaypoint::existingNear(C UID &point_id) C {
    ...
    if (T[i].id == point_id) {
        REPD(j, i)
        if (!T[j].removed)
            return T[j].id; // try before
        for (int j = i + 1; j < elms(); j++)
            if (!T[j].removed)
                return T[j].id; // try after
        break;
    }
    return UIDZero;
}
...
EditWaypoint &EditWaypoint::setRemoved(bool removed) {
    T.removed = removed;
    removed_time.getUTC();
    return T;
}
EditWaypoint &EditWaypoint::setLoopMode(Game::Waypoint::LOOP_MODE loop_mode) {
    T.loop_mode = loop_mode;
    loop_mode_time.getUTC();
    return T;
}
EditWaypoint &EditWaypoint::setName(C Str &name) {
    T.name = name;
    name_time.getUTC();
    return T;
}
bool EditWaypoint::sync(C EditWaypoint &src) {
    ...
    for (; this_waypoints.elms() || src_waypoints.elms();) {
        ...
        if (t && t_in_s && t_in_s->order_time > t->order_time)          // this elm is present in src and there it has newer order
        {
            changed = true;
            t_in_s->sync(*t);
            this_waypoints.removeData(t, true);
        ...
        } else if (t && (!s || t->order_time >= s->order_time)) // there is this elm which is same or newer than src elm
        {
            if (t_in_s) {
                ...
                src_waypoints.removeData(t_in_s, true);
            }
            Swap(*t, New());
            this_waypoints.removeData(t, true);
        } else if (s) {
...
EditWaypoint &EditWaypoint::reverse() {
    reverseOrder();
    REPAO(T).order_time.getUTC();
    return T;
}
EditWaypoint &EditWaypoint::rotateLeft() {
    if (elms()) {
        T[0].order_time.getUTC();
        rotateOrder(-1);
    }
    return T; // old 1st is now new last
}
...
bool EditWaypoint::copyTo(Game::Waypoint &w) {
    ...
    if (!removed) {
        ...
        FREPA(T)
        if (!T[i].removed)
            w.points.New().pos = T[i].pos;
        w.updateTotalLengths();
    }
    return w.points.elms() > 0;
}
void EditWaypoint::draw(C Color &color, int edge_smooth_steps) {
    Game::Waypoint w;
    copyTo(w);
    w.draw(color, color, WaypointRadius, edge_smooth_steps);
}
void EditWaypoint::drawText(C UID &sel, C UID &lit) {
    ...
    FREPA(T)
    if (!T[i].removed) {
        if (PosToScreen(T[i].pos, screen)) {
            ts.color = ((T[i].id == sel) ? LitSelColor : (T[i].id == lit) ? LitColor
                                                                          : DefColor);
            D.text(ts, screen, S + n);
        }
        n++;
    }
}
...
bool EditWaypoint::load(File &f) {
    ...
    case 1: {
        f >> removed >> loop_mode;
        GetStr2(f, name);
        f >> removed_time >> loop_mode_time >> name_time;
        if (super::load(f))
            if (f.ok())
                return true;
    } break;
    ...
    case 0: {
        f >> removed >> loop_mode >> removed_time >> loop_mode_time >> name_time;
        GetStr(f, name);
        if (super::load(f))
            if (f.ok())
                return true;
