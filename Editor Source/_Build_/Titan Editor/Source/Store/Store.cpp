...
Str StoreClass::ItemRes(int item_id, C Str &file, C Str &name) {
    ...
    if (path.is()) {
        if (!GetPath(path).is())
            path = ItemPath(item_id) + name + file;
        else if (!StartsPath(path, "http://") && !StartsPath(path, "https://"))
            path = S + "http://esenthel.com/" + path;
    }
    return path;
}
...
StoreClass::Item::Temp::Temp(C TextNode &node) : ready(false), cat(-1), sub_cat(-1), price(-1), computer_id(-1), subscription(-1) {
    if (C TextNode *n = node.findNode("Ready"))
        ready = n->asBool();
    if (C TextNode *n = node.findNode("Cat"))
        cat = n->asInt();
    if (C TextNode *n = node.findNode("Sub"))
        sub_cat = n->asInt();
    if (C TextNode *n = node.findNode("CID"))
        computer_id = n->asInt();
    if (C TextNode *n = node.findNode("Price"))
        price = n->asInt();
    if (C TextNode *n = node.findNode("Subscr"))
        subscription = n->asInt();
    if (C TextNode *n = node.findNode("Name"))
        name = n->asText();
    if (C TextNode *n = node.findNode("Video"))
        video = n->asText();
    if (C TextNode *n = node.findNode("Icon"))
        icon = n->asText();
    if (C TextNode *n = node.findNode("Files"))
        SetFiles(files, *n);
    if (C TextNode *n = node.findNode("Desc"))
        desc = DecodeText(n->asText());
    if (C TextNode *n = node.findNode("DescShort"))
        desc_short = n->asText();

    if (C TextNode *n = node.findNode("Image_0"))
        image[0] = n->asText();
    if (C TextNode *n = node.findNode("Image_1"))
        image[1] = n->asText();
    if (C TextNode *n = node.findNode("Image_2"))
        image[2] = n->asText();
}
void StoreClass::Item::SetFiles(MemPtr<ItemFile> files, C TextNode &node) {
    FREPA(node.nodes) {
        ...
        if (C TextNode *n = src.findNode("name"))
            file.name = n->asText();
        if (C TextNode *n = src.findNode("file"))
            file.file = n->asText();
        if (C TextNode *n = src.findNode("free"))
            file.free = n->asBool();
    }
}
...
StoreClass::Item::Item() : requested_details(false), has_details(false), like(false), uses_computer_id(false), uses_github_account(false), has_temp(false), id(-1), price(0), category(0), sub_category(0), purchased(0), popularity(0), author(-1), likes(0), sold(0), developer_discount(0), subscription(0) {
    date_added.zero();
    developer_date_joined.zero();
}
Str StoreClass::Item::buyText(bool include_price) C {
    Str t = ((price > 0 || price == -1) ? "Buy" : (price < 0) ? "Contribute"
                                                              : "");
    if (include_price && price > 0)
        t += S + " (" + price_text + ")";
    return t;
}
...
::StoreClass::Item::Temp &StoreClass::Item::setTemp() {
    if (!has_temp) {
        ...
        temp.video = video;
        if (id == AppStore.cur_item)
            AppStore.item_publish.show();
    }
    return temp;
}
void StoreClass::Item::setIcon(bool force) {
    if (!icon || force) {
        if (temp.icon == "keep" || !temp.icon.is()) {
            if (icon_path.is())
                icon = IC.getImage(icon_path);
        } else {
            if (temp.icon != "null")
                icon = IC.getImage(ItemRes(id, temp.icon, "icon_temp."));
        }
        if (AppStore.cur_item == id)
            AppStore.item_icon.set(icon);
    }
}
void StoreClass::Item::setPrice(int price) {
    T.price = price;
    price_text = ((price > 0) ? TextReal(price / 100.0f, 2) + " USD" : (price == 0) ? S + "Free"
                                                                   : (price == -1)  ? S + "Negotiable"
                                                                                    : S);
    if (subscription > 0) {
        price_text += " / ";
        if (subscription == 30)
            price_text += "Month";
        else if (subscription == 365)
            price_text += "Year";
        else
...
StoreClass::Item::Item(C TextNode &node) : requested_details(false), has_details(false), like(false), uses_computer_id(false), uses_github_account(false), has_temp(false), id(-1), price(0), category(0), sub_category(0), purchased(0), popularity(0), author(-1), likes(0), sold(0), developer_discount(0), subscription(0) {
    int p = 0;
    if (C TextNode *n = node.findNode("id"))
        id = n->asInt();
    if (C TextNode *n = node.findNode("author"))
        author = n->asInt();
    if (C TextNode *n = node.findNode("pop"))
        popularity = Sqrt(n->asInt() / 100);
    if (C TextNode *n = node.findNode("likes"))
        likes = n->asInt();
    if (C TextNode *n = node.findNode("subscr"))
        subscription = n->asInt(); // set before price
    if (C TextNode *n = node.findNode("price"))
        p = n->asInt();
    if (C TextNode *n = node.findNode("cost"))
        p = n->asInt();
    if (C TextNode *n = node.findNode("cat"))
        category = n->asInt();
    if (C TextNode *n = node.findNode("category"))
        category = n->asInt();
    if (C TextNode *n = node.findNode("sub"))
        sub_category = n->asInt();
    if (C TextNode *n = node.findNode("subcategory"))
        sub_category = n->asInt();
    if (C TextNode *n = node.findNode("name"))
        name = n->asText();
    if (C TextNode *n = node.findNode("desc"))
        desc_short = n->asText();
    if (C TextNode *n = node.findNode("date"))
        date_added.fromText(n->asText());
    if (C TextNode *n = node.findNode("icon"))
        icon_path = ItemRes(id, n->asText(), "icon.");
...
void StoreClass::Item::setDetails(C TextData &data) {
    has_details = true;
    files.clear();
    if (C TextNode *n = data.findNode("name"))
        name = n->asText();
    if (C TextNode *n = data.findNode("subscr"))
        subscription = n->asInt(); // set before price
    if (C TextNode *n = data.findNode("price"))
        setPrice(n->asInt());
    if (C TextNode *n = data.findNode("cat"))
        category = n->asInt();
    if (C TextNode *n = data.findNode("sub"))
        sub_category = n->asInt();
    if (C TextNode *n = data.findNode("cid"))
        uses_computer_id = n->asBool();
    if (C TextNode *n = data.findNode("gha"))
        uses_github_account = n->asBool();
    if (C TextNode *n = data.findNode("sold"))
        sold = n->asInt();
    if (C TextNode *n = data.findNode("likes"))
        likes = n->asInt();
    if (C TextNode *n = data.findNode("files"))
        SetFiles(files, *n);
    if (C TextNode *n = data.findNode("desc"))
        desc = DecodeText(n->asText());
    if (C TextNode *n = data.findNode("DescShort"))
        desc_short = n->asText();
    if (C TextNode *n = data.findNode("video"))
        video = n->asText();
    if (C TextNode *n = data.findNode("icon"))
        icon_path = ItemRes(id, n->asText(), "icon.");
    if (C TextNode *n = data.findNode("image_0"))
        image_path[0] = ItemRes(id, n->asText(), "image_0.");
    if (C TextNode *n = data.findNode("image_1"))
        image_path[1] = ItemRes(id, n->asText(), "image_1.");
    if (C TextNode *n = data.findNode("image_2"))
        image_path[2] = ItemRes(id, n->asText(), "image_2.");
    if (C TextNode *n = data.findNode("DevName"))
        developer_name = n->asText();
    if (C TextNode *n = data.findNode("DevPayPal"))
        developer_paypal = n->asText();
    if (C TextNode *n = data.findNode("DevSupport"))
        developer_support = n->asText();
    if (C TextNode *n = data.findNode("DevDiscount"))
        developer_discount = n->asInt();
    if (C TextNode *n = data.findNode("DevDate"))
        developer_date_joined.fromText(n->asText());
    ...
    if (C TextNode *n = data.findNode("Purchases")) {
        ...
        FREPA(n->nodes) {
            ...
            if (C TextNode *lk = purchase.findNode("LicenseKey"))
                license_keys.line() += lk->asText();
            if (C TextNode *like = purchase.findNode("Like"))
                if (like->asBool())
                    T.like = true;
        }
    }
    ...
    has_temp = false;
    temp.reset();
    if (C TextNode *n = data.findNode("Temp")) {
        has_temp = true;
        temp = *n;
    }
    setIcon(true);
...
void StoreClass::NewItem::Create(NewItem &new_item) {
    if (ValidText(new_item.name(), 1, 128)) {
        ...
        Gui.msgBox(S, "Invalid item name.");
}
ClosableWindow &StoreClass::NewItem::create() {
    ts.reset().size *= 0.75f;
    ts.align.set(1, 0);
    super::create("New Item").hide();
    button[2].show();
    ...
    T += terms.create(
        ...
        &ts);
    terms.auto_line = true;
    T += t_name.create("Item Name");
    T += name.create().maxLength(128);
    T += b_create.create("Create").func(Create, T);
    return T;
}
ClosableWindow &StoreClass::NewItem::rect(C Rect &rect) {
    super::rect(rect);
    t_name.rect(Vec2(clientWidth() / 2 - 0.4f, -clientHeight() + 0.06f));
    name.rect(Rect_L(t_name.pos() + Vec2(0.13f, 0), 0.45f, 0.05f));
...
void StoreClass::NewItem::update(C GuiPC &gpc) {
    super::update(gpc);
    if (Kb.k(KB_ENTER) && contains(Gui.kb()))
        b_create.push();
}
...
ClosableWindow &StoreClass::Login::create() {
    super ::create("Log In").hide().barVisible(false);
    FlagDisable(flag, WIN_MOVABLE);
    button[2].func(Cancel, T);
    T += t_email.create("E-mail");
    T += email.create();
    T += t_pass.create("Password");
    T += pass.create().password(true);
...
ClosableWindow &StoreClass::Login::rect(C Rect &rect) {
    super::rect(rect);
    t_email.rect(Vec2(0.07f, -0.05f));
    email.rect(Rect_L(0.22f, -0.05f, 0.45f, 0.05f));
    t_pass.rect(Vec2(0.11f, -0.11f));
    pass.rect(Rect_L(0.22f, -0.11f, 0.45f, 0.05f));
...
::StoreClass::Register &StoreClass::Register::create(C Vec2 &pos) {
    super ::create(Rect_C(pos, 0.9f, 0.4f), "Log In").hide();
    button[2].show();
    ...
    T += text.create(Rect(0, yes.rect().max.y, clientWidth(), 0).extend(-0.05f));
    text.auto_line = true;
    return T;
}
...
void StoreClass::ConfirmPayPal::Confirm(ConfirmPayPal &cp) {
    if (cp.paypal().is()) {
        if (cp.paypal() == AppStore.e_dev_paypal()) {
            if (AppStore.cur_dev == AppStore.user_id)
                AppStore.user_paypal = cp.paypal();
            AppStore.cur_dev_paypal = cp.paypal();
            AppStore.refreshDev();
            AppStore.sendCommand("set_user_paypal", "user_id", S + AppStore.cur_dev, "user_paypal", cp.paypal());
            cp.hide();
        } else {
...
::StoreClass::ConfirmPayPal &StoreClass::ConfirmPayPal::create() {
    ...
    super ::create(Rect_LU(0, 0, 0.9f, 0.22f), "Edit PayPal").barVisible(false).hide();
    button[2].show();
    FlagDisable(flag, WIN_MOVABLE);
...
void StoreClass::Contribute::Buy(Contribute &contr) {
    ...
    if (c >= 10 * 100) {
        ...
        Gui.msgBox(S, "Minimum amount to contribute is 10 $");
}
ClosableWindow &StoreClass::Contribute::create() {
    super::create(Rect_LU(0, 0, 0.7f, 0.23f), "Contribute").barVisible(false).hide();
    button[2].show();
    FlagDisable(flag, WIN_MOVABLE);
    ...
    T += text.create(Rect(0, amount.pos().y, clientWidth(), 0).extend(-0.05f), "Please enter the amount of USD which you'd like to contribute", &ts);
    text.auto_line = true;
    return T;
}
...
void StoreClass::ItemList::draw(C GuiPC &gpc) {
    if (gpc.visible && visible()) {
        Rect clip = gpc.clip;
        if (!columnsHidden())
            clip.max.y -= columnHeight();
        VecI2 visible_elms = visibleElmsOnScreen(&gpc);

        for (int i = visible_elms.x; i <= visible_elms.y; i++)
            if (Item *item = T(i))
                item->setIcon(); // first set those seen
        for (int i = visible_elms.x - 5; i <= visible_elms.y + 5; i++)
            if (Item *item = T(i))
                item->setIcon(); // then  set those nearby
        ...
        if (drawMode() == LDM_RECTS) {
            ...
            ts.size = 0.038f * Min(1.2f, zoom());
            ts.align.set(0, -1);
            ts.color.a = 220;
            TextStyleParams ts_small = ts;
            ts_small.align.set(-1, 1);
            ts_small.color.a = 128;
            ts_small.size *= 0.91f;
            for (int i = visible_elms.x; i <= visible_elms.y; i++) {
                if (Item *item = T(i)) {
                    Rect rect = visToScreenRect(i);
                    rect.min += padd.max;
                    rect.max += padd.min;
                    rect.max.y = rect.min.y - padd.min.y - padd.max.y;
                    D.clip(clip & rect);
                    rect.extendX(zoom() * -0.006f);
                    D.text(ts, rect, item->name, null, 0, true);
                    if (!item->purchased)
                        D.text(ts_small, rect, item->price_text, null, 0, true);
                    else
                        AppStore.ok_image->drawFit(Rect_RD(rect.rd(), 0.08f * zoom()));
                }
            }
        }
    }
...
void StoreClass::Purchase::create(GuiObj &parent, C Str &license_key, bool computer_id, bool github_account) {
    ...
    if (computer_id)
        parent += T.set_computer_id.create("Set Computer ID for this Key").func(ItemWebsite, AppStore);
    else
        T.set_computer_id.del();
    if (github_account)
        parent += T.set_github_account.create("Set GitHub account for this Key").func(ItemWebsite, AppStore);
    else
        T.set_github_account.del();
}
...
void StoreClass::DownloadFile::Open(DownloadFile &df) {
    Explore(df.dest, true);
    Cancel(df);
}
void StoreClass::DownloadFile::Import(DownloadFile &df) {
    CopyElms.display(df.dest);
    Cancel(df);
}
...
void StoreClass::DownloadFile::download(bool resume) {
    ...
    if (ValidURL(file))
        down.create(file, null, null, file_done.size());
    else {
...
void StoreClass::DownloadFile::create(int item_id, C Str &file, bool temp) {
    ...
    T.temp = temp;
#if 1
    win_io.create(GetExt(file), S, SystemPath(SP_DESKTOP)).io(Select, Select, T).save();
    win_io.textline.set(GetBase(file));
#else
...
void StoreClass::DownloadFile::update(C GuiPC &gpc) {
    ...
    {
        if (file_size < 0)
            file_size = down.totalSize();
        if (file_size != down.totalSize())
            download(false); // restart
    }
    if (down.modifyTimeUTC().valid()) // if modify time is known
    {
        if (!file_modify_time.valid())
            file_modify_time = down.modifyTimeUTC();
        if (file_modify_time != down.modifyTimeUTC())
            download(false); // restart
    }
    ...
    case DWNL_DONE: {
        if (!saved) {
            saved = true;
            if (file_done.size()) {
                file_done.put(down.data(), down.done());
                file_done.pos(0);
            }
            ...
            if (SafeOverwrite(src, dest)) {
                open.show();
                cancel.text = "Close";
                if (GetExt(file) == ProjectPackageExt)
                    import.show();
    ...
    case DWNL_DOWNLOAD: {
        long done = file_done.size() + down.done();
        text.set(SizeBytes(done) + " / " + (down.totalSize() >= 0 ? SizeBytes(down.totalSize()) : S + "?"));
        progress.set(done, down.totalSize());
    } break;
    case DWNL_ERROR:
        if (down.done())
            retry.show();
        text.set("Error occured");
        break; // if downloaded anything
    default:
        text.set("Connecting");
        break;
    }
...
void StoreClass::ItemFile::Download(ItemFile &df) {
    if (ItemDownloadable() || df.is_free) {
        if (StartsPath(df.file, "http://") || StartsPath(df.file, "https://")) // check if this is a link to item in App Stores
        {
            ...
            if (Contains(s, "steampowered.com", false, WHOLE_WORD_STRICT) || Contains(s, "microsoft.com", false, WHOLE_WORD_STRICT) || Contains(s, "google.com", false, WHOLE_WORD_STRICT) || Contains(s, "apple.com", false, WHOLE_WORD_STRICT) || Contains(s, "amazon.com", false, WHOLE_WORD_STRICT) || Contains(s, "samsung.com", false, WHOLE_WORD_STRICT) || Contains(s, "github.com", false, WHOLE_WORD_STRICT)) {
                Explore(df.file);
                return;
            }
        }
...
#endif
        if (ValidURL(df.file) || CleanFileName(df.file) == df.file)
            AppStore.download_files.New().create(AppStore.cur_item, df.file, df.temp);
    } else
        Gui.msgBox(S, "You need to buy the item to download this file");
}
...
void StoreClass::ItemFile::create(GuiObj &parent, C Item::ItemFile &file, bool temp) {
    ...
    Str desc = S + "File: \"" + file.file + "\"";
    if (!ItemDownloadable() && is_free)
        desc.line() += "This file is accessible for free";
    if (downloadable)
        desc.line() += "Click to download";
    else
        desc.line() += "You need to buy the item to download this file";
    parent += download.create(name).func(Download, T).setImage(downloadable ? AppStore.ok_image : AppStore.fail_image).desc(desc);
    download.skin = (downloadable ? null : &HalfTransparentSkin);
    if (temp)
        parent += rename.create("Rename").func(Rename, T).desc("Rename this file");
    else
        rename.del();
    if (temp)
        parent += remove.create("Remove").func(Remove, T).desc("Delete this file");
    else
        remove.del();
    if (temp)
        parent += free.create(is_free ? "Set paid access" : "Set access for free").func(SetFree, T).desc("Make this file to be accessible for free.\nNormally files are accessible only after the item is purchased.\nHowever you may want to include some files to preview for free, before purchasing.\nAfter clicking this button, the file will become accessible for free.");
    else
        free.del();
}
...
void StoreClass::RenameFile::ok() {
    if (ValidText(new_name(), 1, 256)) {
        if (!Equal(new_name(), name, true))
            AppStore.sendCommand("rename_item_file", "i", S + AppStore.cur_item, "item_file_file", file, "item_file_name", new_name());
        hide();
    } else
        Gui.msgBox(S, "Invalid name");
}
::StoreClass::RenameFile &StoreClass::RenameFile::create(C Vec2 &pos) {
    Vec2 size(0.78f, 0.05f), padd = defaultInnerPaddingSize() + 0.02f;
    super::create(Rect_LU(pos, size + padd));
    button[2].show();
...
void StoreClass::RemoveFile::OK(RemoveFile &fr) {
    fr.hide();
    AppStore.sendCommand("remove_item_file", "i", S + AppStore.cur_item, "item_file_file", fr.file);
}
...
::StoreClass::RemoveFile &StoreClass::RemoveFile::create(C Vec2 &pos) {
    super::create(Rect_C(pos, 0.72f, 0.32f), "Remove File").hide();
    button[2].show();
    T += t_name.create(Rect_C(clientWidth() / 2, -0.09f, 0.7f, 0.0f), "Are you sure you wish to remove selected file?");
    t_name.auto_line = true;
...
void StoreClass::PublishConfirm::OK(PublishConfirm &pc) {
    pc.hide();
    if (Item *item = AppStore.findItem(AppStore.cur_item)) {
        item->setTemp().ready = true;
        AppStore.item_publish.set(item->temp.ready, QUIET);
        AppStore.filter();
    }
...
void StoreClass::Upload::pos(C Vec2 &pos) {
    rect(Rect_LU(pos, 1.0f, 0.045f));
    cancel.rect(Rect_LU(progress.rect().ru() + Vec2(0.01f, 0), 0.17f, 0.045f));
}
...
void StoreClass::ItemWebsite(StoreClass &store) {
    if (store.cur_item >= 0)
        Explore(S + "https://esenthel.com/?id=store&item=" + store.cur_item);
}
...
void StoreClass::Buy(StoreClass &store) {
    if (Item *item = store.findItem(store.cur_item)) {
        ...
        } else if (store.cur_item <= 0) {
            Gui.msgBox(S, "Invalid item");
        } else if (item && item->price == -2) {
...
void StoreClass::DeveloperPage(StoreClass &store) {
    if (Item *item = store.findItem(store.cur_item))
        store.openDeveloper(item->author, item->developer_name, item->developer_support, item->developer_paypal, item->developer_date_joined, item->developer_discount);
}
void StoreClass::Back(StoreClass &store) { store.openItem(-1); }
void StoreClass::Filter(StoreClass &store) { store.filter(); }
void StoreClass::Readies(StoreClass &store) {
    if (store.readies())
        store.categories.set(-1);
    store.filter();
}
...
Str StoreClass::MD5Text(C Str8 &text) {
    UID id = MD5Mem(text(), text.length());
    return TextHexMem(&id, SIZE(id));
}
void StoreClass::SetUserParams(MemPtr<HTTPParam> params, C Str &user, C Str &password, C Str &cmd) {
    ...
    if (cmd.is())
        params.New().set("cmd", cmd);
}
void StoreClass::OpenSupport(C Str &support) {
    if (ValidEmail(support)) {
        ClipSet(support);
        Gui.msgBox(S, S + "Support email\n\"" + support + "\"\nhas been copied to clipboard.");
    } else if (ValidURL(support))
        Explore(support);
}
void StoreClass::ItemSupport(StoreClass &store) {
    if (Item *item = store.findItem(store.cur_item))
        OpenSupport(item->developer_support);
}
...
void StoreClass::Account(StoreClass &store) {
    if (store.cur_dev == store.user_id && store.mode() == 0)
        store.openDeveloper(-1);
    else
        store.openDeveloper(store.user_id, store.user_name, store.user_support, store.user_paypal, store.user_date, store.user_discount);
}
void StoreClass::NewItemDo(StoreClass &store) {
    if (!store.loggedIn()) {
        store.b_login.set(true);
        Gui.msgBox(S, "Please log in to create new items in the store.");
    } else if (!store.user_name.is() || !store.user_paypal.is() || !store.user_support.is()) {
        if (store.cur_dev != store.user_id)
            Account(store);
        Str msg;
        if (!store.user_name.is())
            msg.line() += "Please set your Developer Name";
        if (!store.user_paypal.is())
            msg.line() += "Please set your Developer PayPal email address";
        if (!store.user_support.is())
            msg.line() += "Please set your Developer Support Contact (website or email)";
        Gui.msgBox(S, msg);
...
::StoreClass::Item *StoreClass::findItem(int id) {
    if (id > 0)
        REPA(items)
    if (items[i].id == id)
        return &items[i];
...
bool StoreClass::isPurchased(int id) {
    if (Item *item = findItem(id))
        return item->purchased > 0;
    return false;
}
bool StoreClass::isSeller() C { return loggedIn() && user_seller; }
void StoreClass::clearDownloadFiles() {
    REPA(download_files)
    if (!download_files[i].valid())
        download_files.removeValid(i);
}
...
void StoreClass::sendCommand(C Str &cmd, C Str &name, C Str &value, C Str &name2, C Str &value2, C Str &name3, C Str &value3) {
    ...
    if (name.is())
        params.New().set(name, value);
    if (name2.is())
        params.New().set(name2, value2);
    if (name3.is())
        params.New().set(name3, value3);
...
void StoreClass::sendCommandPost(C Str &cmd, C Str &name, C Str &value, C Str &post_name, C Str &post_value) {
    ...
    if (name.is())
        params.New().set(name, value);
    if (post_name.is())
        params.New().set(post_name, post_value, HTTP_POST);
    store_commands.New().create(EsenthelStoreURL, params);
}
void StoreClass::setLogin() {
    ...
    if (readies.hidden())
        readies.set(false, QUIET); // quietly disable if hidden
}
void StoreClass::loginDo(int mode, C Str &email, C Str &pass) {
    _register.hide();
    if (mode == 2 || ValidPass(pass)) {
        Memt<HTTPParam> params;
        SetUserParams(params, email, pass, mode == 0 ? "register" : mode == 1 ? "log_in"
                                                                              : "forgot_pass");
        if (mode == 0)
            params.New().set("p", pass, HTTP_POST);
        store_commands.New().create(EsenthelStoreURL, params);
        login_email = email;
        login_pass = pass;
    } else
        Gui.msgBox(S, "Password length must be in range of 4-16 characters and may not contain unicode characters.");
}
...
void StoreClass::setCategory(int cat, int sub) {
    ...
    if (InRange(cat, categories_node.children)) {
        ...
        if (InRange(sub, subs))
            item_category.text = subs[sub].name;
    }
}
void StoreClass::getCategory(int &cat, int &sub) {
    ...
    if (InRange(cat, categories_node.children)) {
        Memx<Node<MenuElm>> &subs = categories_node.children[cat].children;
        REPA(subs)
        if (subs[i].name == item_category.text) {
            sub = i;
            break;
        }
    }
...
void StoreClass::create() {
    // categories
    FREPA(categories_t)
    categories_node += categories_t[i];
    FREPA(source_code_cats)
    categories_node.children[2] += source_code_cats[i]; // #Category
    FREPA(game_cats)
    categories_node.children[3] += game_cats[i];
    FREPA(asset_cats)
    categories_node.children[4] += asset_cats[i];
    ...
    Gui += super::create("Esenthel Store").hide();
    button[1].show();
    button[2].show();
    flag |= WIN_RESIZABLE;
    T += mode.create((cchar **)null, 2).set(0);
    REPA(mode)
    mode.tab(i).hide();
    T += hello_user.create();
    T += line.create();
    ...
    // item list page
    mode.tab(0) += filter_name.create().func(Filter, T);
    filter_name.kbClear();
    filter_name.reset.show();
    filter_name.show_find = true;
    filter_name.hint = "Find";
    ...
    mode.tab(0) += purchased.create("Bought").func(Filter, T).desc("Display only the items which you have purchased");
    purchased.mode = BUTTON_TOGGLE;
    mode.tab(0) += readies.create("Readies").func(Readies, T).desc("Display items ready for publishing").hide();
    readies.mode = BUTTON_TOGGLE;
    mode.tab(0) += items_region.create();
    items_region.kb_lit = false;
    flt padd = 0.01f;
    items_region += items_list.create(lc, Elms(lc)).columnHeight(0.048f).elmHeight(0.1f).textSize(0.04f, 0).drawMode(LDM_RECTS);
    items_list.cur_mode = LCM_MOUSE;
    items_list.flag |= LIST_SCALABLE;
    items_list.imageSize(0.286f, 0, Rect(-padd, -padd - 0.15f, padd, padd)).columnVisible(0, false);
    items_list.sort_column[0] = 1;
    items_list.sort_swap[0] = true;
    items_list.sort_column[1] = 2;
    items_list.sort_swap[1] = true;
    items_list.sort_column[2] = 3;
    items_list.sort_swap[2] = true;
    mode.tab(0) += close_dev_page.create().func(CloseDevPage, T).hide().desc("Close Developer Page");
    close_dev_page.image = "Gui/close.img";
    ...
    ts_desc.reset().align.set(1, -1);
    ts_desc.size = 0.041f;
    ts_stats.reset().align.set(1, 0);
    ts_stats.size = 0.037f;
    ts_dev_name.reset().align.set(1, 0);
    ts_dev_name.size = 0.05f;
    ...
    T += b_login.create("Log In").func(LoginToggle, T);
    b_login.mode = BUTTON_TOGGLE;
...
void StoreClass::filter() {
    Memt<bool> visible;
    visible.setNumZero(items.elms());
    FREPA(items) {
        Item &item = items[i];
        Tabs *sub_cat = (InRange(categories(), sub_categories) ? &sub_categories[categories()] : null);
        if (sub_cat && !sub_cat->is())
            sub_cat = null;
                ...
                if (!sub_cat || (*sub_cat)() < 0 || item.sub_category == (*sub_cat)())
                    if (!purchased() || item.purchased)
                        if (!readies() || item.temp.ready)
                            if (!filter_name().is() || ContainsAll(item.name, filter_name()))
                                visible[i] = true;
    }
...
void StoreClass::setFilesRects() {
    ...
    FREPA(item_purchases) {
        ...
        if (p.set_computer_id.is()) {
            p.set_computer_id.rect(Rect_LU(pos + Vec2(0.01f, 0), 0.55f, h));
            pos = p.set_computer_id.rect().ru();
        }
        if (p.set_github_account.is()) {
            p.set_github_account.rect(Rect_LU(pos + Vec2(0.01f, 0), 0.55f, h));
            pos = p.set_github_account.rect().ru();
        }
        y -= h;
    }
    y -= 0.01f;
    ...
    if (!has_temp) {
        t_new_files.rect(Rect_LU(0.01f, y, 0, h));
        y -= h;
    }
...
Window &StoreClass::rect(C Rect &rect) {
    ...
    {
        flt y = hello_user.pos().y - 0.05f, b = 0.3f;
        line.rect(Rect(b, y, clientWidth() - b, y));
    }
    filter_name.rect(Rect_LU(0.05f, -0.025f, 0.35f, 0.05f));
    {
        ...
        t_dev_support.rect(Rect_L(0.05f, y, 0.8f, 0.05f));
        y += 0.05f;
    ...
    Rect r(0, -clientHeight(), clientWidth(), back.rect().min.y - 0.01f);
    r.extend(-0.04f);
    ...
    item_publish.rect(Rect_LU(cx, y, 0.2f, 0.05f));
    t_item_name.rect(Rect_U(xx, y, 1.0f, 0.05f));
    item_www.rect(Rect_RU(r.max.x, y, 0.12f, 0.045f));
    y -= 0.09f;
    t_item_developer.rect(Vec2(cx, y));
    item_developer.rect(Rect_L(t_item_developer.rect().right() + Vec2(0.16f, 0), 0.5f, 0.045f));
    t_item_video.rect(Rect_L(xx, y, 0.7f, 0.05f));
    item_video.rect(Rect_L(t_item_video.rect().left() + Vec2(0.13f, 0), 0.3f, 0.045f));
    t_item_id.rect(item_video.rect().right() + Vec2(0.06f, 0));
    y -= 0.045f;
    t_item_date.rect(Vec2(cx, y));
    t_item_support.rect(Vec2(xx, y));
    item_support.rect(Rect_L(t_item_support.pos() + Vec2(0.13f, 0), 0.3f, 0.045f));
    item_category.rect(Rect_LU(item_support.rect().ru() + Vec2(0.05f, 0), 0.25f, 0.045f));
    y -= 0.045f;
    t_item_sold.rect(Vec2(cx, y));
    t_item_likes.rect(Vec2(xx, y));
    item_like.rect(Rect_R(t_item_likes.rect().right() + Vec2(0.43f, 0), 0.15f, 0.045f));
    t_item_cid.rect(Vec2(item_like.rect().right() + Vec2(0.05f, 0)));
    item_cid.rect(Rect_L(t_item_cid.pos() + Vec2(0.181f, 0), 0.045f, 0.045f));
    y -= 0.03f;
...
void StoreClass::setItemImages(Item &item) {
    REPA(item_images) {
        if (item.temp.image[i] == "keep" || !item.temp.image[i].is()) {
            ...
            if (item.temp.image[i] != "null")
                item_images[i].set(IC.getImage(ItemRes(item.id, item.temp.image[i], S + "image_" + i + "_temp.")));
        }
    }
}
void StoreClass::setItemDetails(Item &item) {
    ...
    item_support.text = (ValidEmail(item.developer_support) ? "Email" : ValidURL(item.developer_support) ? "Open Website"
                                                                                                         : "");
    item_support.visible(item_support.text.is());
    ...
    buy.text = item.buyText(true);
    buy.visible(item.price != 0 && item.has_details);
    item_refresh.visible((buy.visible() && loggedIn()) || item_editable);
    Memt<Str> license_keys;
    Split(license_keys, item.license_keys, '\n');
    if (license_keys.elms() && !license_keys.last().is())
        license_keys.removeLast();
    item_purchases.setNum(license_keys.elms());
    FREPAO(item_purchases).create(item_files_region, license_keys[i], item.uses_computer_id, item.uses_github_account);
    if (!license_keys.elms() && item.has_details && item.price > 0)
        item_files_region += item_purchases.New().t_license_key.create("License Key: You need to buy this item in order to get a license key", &ts_desc);
    item_files.setNum(item.files.elms());
    FREPAO(item_files).create(item_files_region, item.files[i]);
    FREPA(item.temp.files)
    item_files.New().create(item_files_region, item.temp.files[i], true);
    t_new_files.visible(item_editable);

    e_item_name.set(item.temp.name.is() ? item.temp.name : item.name);
    e_item_price.set(TextReal(((item.temp.price >= 0) ? item.temp.price : item.price) / 100.0f, 2));
    e_item_price.pos(buy.pos() + Vec2(0, buy.visible() * e_item_price.size().y));
    e_item_desc.set(item.has_temp ? item.temp.desc : item.desc);
    e_item_video.set((item.has_temp && item.temp.video != "keep") ? item.temp.video : item.video);
    Rect rect = e_item_video.rect();
    rect.min.x = item_video.rect().lerpX(item_video.visible());
    e_item_video.rect(rect);
...
void StoreClass::refreshDev() {
    ...
    dev_support.text = (ValidEmail(cur_dev_support) ? "Email" : ValidURL(cur_dev_support) ? "Open Website"
                                                                                          : "");
    dev_support.visible(dev_support.text.is());
    rect(super::rect());
}
...
bool StoreClass::itemCanUploadFile(Item &item, C Str &file) {
    ...
    if (ext == ProjectPackageExt || ext == "7z" || ext == "rar" || ext == "zip" || ext == "exe") {
        if (ValidFileNameForUpload(file)) {
            REPA(item.temp.files)
            if (EqualPath(item.temp.files[i].file, file)) {
                Gui.msgBox(S, S + "Item already has a file \"" + file + "\"");
                return false;
            }
            REPA(AppStore.store_uploads)
            if (EqualPath(AppStore.store_uploads[i].item_file, file) && AppStore.store_uploads[i].item_id == item.id) {
                Gui.msgBox(S, S + "Item already has a file \"" + file + "\" being uploaded");
                return false;
            }

            int item_files = item.temp.files.elms();
            REPA(AppStore.store_uploads)
            if (AppStore.store_uploads[i].item_id == item.id)
                item_files++;
            if (item_files < EsenthelStoreMaxFileNum) {
                return true;
            } else
                Gui.msgBox(S, S + "Can't upload more than " + EsenthelStoreMaxFileNum + " files per item");
        } else
            Gui.msgBox(S, S + "Invalid file name \"" + file + "\"");
    } else
        Gui.msgBox(S, S + "Only " + ProjectPackageExt + "/7Z/RAR/ZIP/EXE extensions are supported");
    return false;
}
void StoreClass::drop(Memc<Str> &names, GuiObj *obj, C Vec2 &screen_pos) {
    if (item_editable) {
        if (item_files_region.contains(obj)) {
            if (Item *item = findItem(cur_item))
                FREPA(names) {
                    ...
                    if (itemCanUploadFile(*item, file_file)) {
                        ...
                        if (src.readStd(name)) {
                            if (src.size() <= EsenthelStoreMaxFileSize) {
                                ...
                                Gui.msgBox(S, S + "File size " + SizeBytes(src.size()) + " exceeds " + SizeBytes(EsenthelStoreMaxFileSize) + " limit");
                        } else
                            Gui.msgBox(S, S + "Can't read file \"" + name + "\"");
                    }
                }
        } else if (obj == &item_icon) {
            FREPA(names) {
                ...
                if (ExtType(ext) == EXT_IMAGE) {
                    ...
                    if (temp.Import(name)) {
                        ...
                        if (src.size() <= EsenthelStoreMaxIconSize) {
                            ...
                            Swap(src, SCAST(File, upload.src));
                            Memt<HTTPParam> params;
                            setUserParams(params, "set_item_icon");
                            params.New().set("i", S + cur_item);
                            params.New().set("item_icon", ext);
                            upload.down.create(EsenthelStoreURL, params, upload.src);
                        } else
                            Gui.msgBox(S, S + "File size " + SizeBytes(src.size()) + " exceeds " + SizeBytes(EsenthelStoreMaxIconSize) + " limit");
                    } else
                        Gui.msgBox(S, S + "Couldn't import image \"" + name + "\"");
                    break;
                }
            }
        } else
            REPA(item_images)
        if (obj == &item_images[i]) {
            int image_index = i;
            FREPA(names) {
                Str name = names[i], ext = GetExt(name);
                if (ExtType(ext) == EXT_IMAGE) {
                    Image temp;
                    if (temp.Import(name, IMAGE_R8G8B8_SRGB)) // ignore alpha channel
                    {
                        File src;
                        if (ext == "jpeg")
                            ext = "jpg";
                        ...
                        if (src.size() <= EsenthelStoreMaxImageSize) {
                            ...
                            Swap(src, SCAST(File, upload.src));
                            Memt<HTTPParam> params;
                            setUserParams(params, "set_item_image");
                            params.New().set("i", S + cur_item);
                            params.New().set("item_image_index", S + image_index);
                            params.New().set("item_image_status", ext);
                            upload.down.create(EsenthelStoreURL, params, upload.src);
                        } else
                            Gui.msgBox(S, S + "File size " + SizeBytes(src.size()) + " exceeds " + SizeBytes(EsenthelStoreMaxImageSize) + " limit");
                    } else
                        Gui.msgBox(S, S + "Couldn't import image \"" + name + "\"");
                    break;
                }
...
void StoreClass::update(C GuiPC &gpc) {
    super::update(gpc);
    if (gpc.visible && visible()) {
        if ((Ms.tapped(1) && Gui.window() == this) || (Ms.bp(MS_BACK) && Gui.window() == this && !(Gui.kb() && Gui.kb()->type() == GO_TEXTLINE)))
            back.push();
        REPA(MT)
        if (MT.tapped(i) && MT.guiObj(i) == &items_list)
            if (Item *item = items_list())
                openItem(item->id);

        bool preview = false;
        REPAD(mt, MT)
        REPA(item_images)
        if (MT.guiObj(mt) == &item_images[i] && item_images[i].image && item_images[i].image->is()) {
    ...
    found:
        ...
        if ((Kb.k(KB_ENTER) || Kb.k(KB_NPENTER)) && Kb.k.first() && Gui.kb() && Gui.kb()->type() == GO_TEXTLINE) {
            if (Gui.kb() == &e_dev_name) {
                if (e_dev_name().is()) {
                    if (cur_dev == user_id)
                        user_name = e_dev_name();
                    cur_dev_name = e_dev_name();
                    refreshDev();
                    sendCommand("set_user_name", "user_id", S + cur_dev, "user_name", e_dev_name());
                } else
                    Gui.msgBox(S, "Invalid User Name");
            } else if (Gui.kb() == &e_dev_paypal) {
                if (ValidEmail(e_dev_paypal())) {
                    confirm_paypal.activate();
                } else
                    Gui.msgBox(S, "Invalid User PayPal email");
            } else if (Gui.kb() == &e_dev_support) {
                if (ValidSupport(e_dev_support())) {
                    if (cur_dev == user_id)
                        user_support = e_dev_support();
                    cur_dev_support = e_dev_support();
                    refreshDev();
                    sendCommand("set_user_support", "user_id", S + cur_dev, "user_support", Replace(e_dev_support(), '/', '\\')); // server will fail for some reason if parameter contains "http://" (instead use '\')
                } else
                    Gui.msgBox(S, "Invalid User Support Contact.\nIt must be either a website link or an email address.");
            } else if (Gui.kb() == &e_dev_discount) {
                ...
                if (discount >= 0 && discount < 100) {
                    ...
                    cur_dev_discount = discount;
                    refreshDev();
                    sendCommand("set_user_discount", "user_id", S + cur_dev, "user_discount", S + discount);
                } else
                    Gui.msgBox(S, "Invalid discount.\nIt must be in range 0 .. 99 %");
            } else if (Gui.kb() == &e_item_name) {
                if (e_item_name().is()) {
                    if (Item *item = findItem(cur_item))
                        item->setTemp().name = e_item_name();
                    sendCommand("set_item_name", "i", S + cur_item, "item_name", e_item_name());
                } else
                    Gui.msgBox(S, "Item name must not be empty");
            } else if (Gui.kb() == &e_item_price) {
                dbl priced = TextDbl(e_item_price());
                int price = Round(priced * 100);
                if (price >= 0) {
                    if (Item *item = findItem(cur_item))
                        item->setTemp().price = price;
                    sendCommand("set_item_price", "i", S + cur_item, "item_price", S + price);
                } else
                    Gui.msgBox(S, "Invalid item price");
            } else if (Gui.kb() == &e_item_desc) {
                if (Item *item = findItem(cur_item))
                    item->setTemp().desc = e_item_desc();
                sendCommandPost("set_item_desc", "i", S + cur_item, "item_desc", e_item_desc());
            } else if (Gui.kb() == &e_item_video) {
                Str video = YouTubeFullToEmbed(e_item_video());
                if (ValidVideo(video)) {
                    if (Item *item = findItem(cur_item))
                        item->setTemp().video = video;
        ...
        REPA(store_commands) // process from end because they will get removed
        {
            ...
            if (Check(down)) {
                TextData data;
                if (data.load(FileText().readMem(down.data(), down.size(), UTF_8_NAKED)))
                    if (C TextNode *cmd = data.findNode("cmd")) {
                        C TextNode *ok = data.findNode("OK");
                        Str msg;
                        for (int i = 0; C TextNode *n = data.findNode("msg", i); i++)
                            msg.line() += n->asText();
                        if (cmd->asText() == "log_in" || cmd->asText() == "register" || cmd->asText() == "forgot_pass") {
                            user_id = -1;
                            C TextNode *result = data.findNode("result");
                            if (ok && ok->asBool()) {
                                if (C TextNode *user_id = data.findNode("u"))
                                    T.user_id = user_id->asInt();
                                if (C TextNode *user_name = data.findNode("n"))
                                    T.user_name = user_name->asText();
                                if (C TextNode *user_support = data.findNode("s"))
                                    T.user_support = user_support->asText();
                                if (C TextNode *user_paypal = data.findNode("p"))
                                    T.user_paypal = user_paypal->asText();
                                if (C TextNode *user_date = data.findNode("d"))
                                    T.user_date.fromText(user_date->asText());
                                if (C TextNode *user_seller = data.findNode("seller"))
                                    T.user_seller = user_seller->asBool();
                                if (C TextNode *user_discount = data.findNode("discount"))
                                    T.user_discount = user_discount->asInt();
                                T.user_email = login_email;
                                T.user_pass = login_pass;
                                downloadItems(); // redownload items after logging in
                            }
                            if (user_id == -1 && msg == "User not found") {
                                _register.activate();
                                msg.clear();
                            }
                            setLogin();
                            refreshDev();
                        } else if (cmd->asText() == "get_items") {
                            if (ok && ok->asBool()) {
                                items.clear();
                                if (C TextNode *node = data.findNode("Items"))
                                    FREPA(node->nodes)
                                items.New() = node->nodes[i];
                                if (C TextNode *node = data.findNode("Purchased"))
                                    FREPA(node->nodes)
                                if (Item *item = findItem(node->nodes[i].asInt()))
                                    item->purchased++;
                                if (C TextNode *node = data.findNode("Ready"))
                                    FREPA(node->nodes)
                                if (Item *item = findItem(node->nodes[i].asInt()))
                                    item->temp.ready = true;
                                if (Item *item = findItem(cur_item))
                                    item->getDetails();
                                filter();
                            }
                        } else if (cmd->asText() == "new_item") {
                            if (ok && ok->asBool()) {
                                new_item.name.clear();
                                int item_id = -1;
                                if (C TextNode *n = data.findNode("ItemID"))
                                    item_id = n->asInt();
                                if (item_id >= 0) {
                                    Item &item = items.New();
                                    item.id = item_id;
                                    item.author = user_id;
                                    if (C TextNode *n = data.findNode("ItemName"))
                                        item.name = n->asText();
                                    filter();
                                }
                                openItem(item_id);
                            }
                        } else if (cmd->asText() == "like_item") {
                            if (ok && ok->asBool()) {
                                if (C TextNode *id = data.findNode("id"))
                                    if (Item *item = findItem(id->asInt())) {
                                        if (C TextNode *n = data.findNode("like"))
                                            item->like = n->asBool();
                                        if (C TextNode *n = data.findNode("likes"))
                                            item->likes = n->asInt();
                                        if (item->id == cur_item)
                        ...
                        } else if (cmd->asText() == "get_item_details") {
                            if (C TextNode *id = data.findNode("id"))
                                if (Item *item = findItem(id->asInt()))
                                    item->setDetails(data);
                        } else if (cmd->asText() == "set_item_ready") {
                            if (isAdmin()) // admin has published an item
                            {
                                if (Item *item = findItem(cur_item))
                                    item->getDetails(true);
                            }
                        } else if (cmd->asText() == "remove_item_file") {
                            if (ok && ok->asBool())
                                if (Item *item = findItem(cur_item))
                                    item->getDetails(true);
                        } else if (cmd->asText() == "rename_item_file") {
                            if (ok && ok->asBool())
                                if (Item *item = findItem(cur_item))
                                    item->getDetails(true);
                        } else if (cmd->asText() == "set_item_file_access") {
                            if (ok && ok->asBool())
                                if (Item *item = findItem(cur_item))
                                    item->getDetails(true);
                        } else if (cmd->asText() == "set_user_discount") {
                            if (ok && ok->asBool())
                                downloadItems(); // redownload items to get the new prices
                        }
                        if (msg.is())
                            Gui.msgBox("Esenthel Store", msg);
                    }
                down.del();
            }
            if (!down.state())
                store_commands.removeValid(i);
        }
        ...
        REPA(store_uploads) // process from end because they will get removed
        {
            ...
            upload.update();
            if (upload.dest) {
                Rect r = upload.dest->rect();
                upload.rect(Rect_C(r.center(), r.w(), 0.04f));
            } else {
                if (upload.visible()) {
                    upload.pos(Vec2(0.01f, y));
                    y -= 0.045f;
            ...
            if (down.state() == DWNL_ERROR && down.sent() && down.sent() == down.toSend()) // finished uploading, but didn't receive confirmation
            {
                ...
                down.del();
            } else if (Check(down) && upload.checkChunked()) {
                TextData data;
                if (data.load(FileText().readMem(down.data(), down.size())))
                    if (C TextNode *cmd = data.findNode("cmd")) {
                        C TextNode *ok = data.findNode("OK");
                        Str msg;
                        for (int i = 0; C TextNode *n = data.findNode("msg", i); i++)
                        ...
                        if (cmd->asText() == "set_item_icon") {
                            if (ok && ok->asBool()) {
                                if (Item *item = findItem(upload.item_id)) {
                                    item->setTemp().icon = upload.set;
                                    item->setIcon(true);
                                }
                                IC.changed(ItemRes(upload.item_id, upload.set, "icon_temp."));
                            }
                        } else if (cmd->asText() == "set_item_image") {
                            if (ok && ok->asBool()) {
                                if (Item *item = findItem(upload.item_id)) {
                                    item->setTemp().image[upload.index] = upload.set;
                                    if (upload.item_id == cur_item)
                                        setItemImages(*item);
                                }
                                IC.changed(ItemRes(upload.item_id, upload.set, S + "image_" + upload.index + "_temp."));
                            }
                        } else if (cmd->asText() == "add_item_file") {
                            if (ok && ok->asBool()) {
                                if (Item *item = findItem(upload.item_id)) {
                                    item->setTemp();
                                    item->getDetails(true);
            ...
            if (!down.state())
                store_uploads.removeValid(i, true);
        }
    }
}
void StoreClass::draw(C GuiPC &gpc) {
    ...
    if (gpc.visible && visible()) {
        if (image_preview && image_preview_step > EPS_COL) {
            ...
            D.rect().draw(ColorAlpha(BLACK, 0.7f));
            D.alpha(alpha);
        ...
        } else if (image_preview_step <= EPS)
            image_preview = null;
    }
}
