...
struct Message {
    ...
    Message &set(C Str &text, TYPE type = NONE) {
        T.type = type;
        T.text = text;
        T.token = null;
        T.source = null;
    ...
    Message &set(C Str &text, Token *token, TYPE type = NONE) {
        T.type = type;
        T.text = text;
        T.token = token;
        T.source = ((token && token->line) ? token->line->source : null);
        return T;
    }
    Message &set(C Str &text, Source *source, Int token, TYPE type = NONE) {
        T.type = type;
        T.text = text;
        T.token = (source ? source->getToken(token) : null);
        T.source = source;
        return T;
    }
...
struct Compiler // Function Compiler
{
    ...
    Compiler(Memc<Message> &msgs, Memc<Token *> &tokens, Source *source, CompilerContext *ctx) : msgs(msgs), tokens(tokens), source(source), ctx(ctx) {
        strict = true;
        quiet = false;
        allow_cast = true;
        allow_func_lists = false;
        final = -1;
        labels = 0;
        cmd_index = 0;
        scope_label = -1;
    ...
    Compiler &setFinal(Int token_index, Bool allow_func_lists) {
        T.final = token_index;
        T.allow_func_lists = allow_func_lists;
        return T;
    }
