...
struct CodeEnvironment {
    ...
    struct Thread {
        ...
        void zero() {
            func_code_pos = func_code_end = func_stack = func_param_stack = func_this = func_result = heap = null;
            func = null;
            debug = false;
        }
        void del();
...
struct Call {
    ...
    struct Param {
      ...
      T1(TYPE) INLINE TYPE& value  (CodeEnvironment::Thread &thread) {return *(TYPE*)address(thread);}
        ...
        T1(TYPE)
        TYPE &value(CodeEnvironment::Thread &thread);
        Ptr address(CodeEnvironment::Thread &thread);
#endif
    ...
    Func func;
    Int params;
    INLINE Param &param(Int i) {
        DEBUG_RANGE_ASSERT(i, params);
        return ((Param *)(this + 1))[i];
    }
};
/******************************************************************************/
} // namespace Edit
