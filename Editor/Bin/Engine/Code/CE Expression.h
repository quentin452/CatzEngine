...
struct Expr : BStr {
    ...
    struct Memory // use 64-bit values to support 64-bit compilation
    {
        ...
        void setGlobal(Symbol &symbol) {
            T.type = GLOBAL;
            T.symbol = &symbol;
            T.index = symbol.raw_offset;
        }
        void setParam(Symbol &symbol) {
            T.type = PARAM;
            T.symbol = &symbol;
            T.index = symbol.raw_offset;
        }
        void setLocal(Symbol &symbol) {
            T.type = LOCAL;
            T.symbol = &symbol;
            T.index = symbol.raw_offset;
        } // 'symbol' is already assumed to have its 'raw_offset' set to locals
        void setLocal(Int local_index) {
            T.type = LOCAL;
            T.symbol = null;
            T.index = local_index;
        } // set layout for 'local_index' local variable
        void setThis() { T.type = THIS; }
        void setResult() { T.type = RESULT; }
        void setKnown() { T.type = KNOWN; }

        void addOffset(Long offset, Bool ref) {
            if (ref)
                T.offset2 += offset;
            else
                T.offset += offset;
    ...
    Expr() {
        instance = final = _operator = separator = l_to_r = temporary = conditional = false;
        func_call_mask = priority = 0;
        symbol_origin = null;
        origin = null;
        func_call = null;
    }
...
inline Int CompareLowest(C Edit::FuncMatch &a, C Edit::FuncMatch &b) {
    if (Int c = Compare(b.lowest_match, a.lowest_match))
        return c;
    return CompareAverage(a, b);
} // compare in reversed order so functions with greatest match are listed first (this is the exact       compare used for compilers), use average comparison as second alternative in case there are multiple choices for lowest match (useful for auto-complete suggestions)
/******************************************************************************/
#endif
/******************************************************************************/
