...
struct Find : Region {
    ...
    struct ResultRegion : Region {
        struct Result {
            ...
            Result &set(Bool skip_ee, C Str &t, C SymbolPtr &symbol, Int priority = 0) {
                T.text = PathToSymbol(skip_ee ? SkipStartPath(t, "EE") : t);
                T.symbol = symbol;
                T.priority = priority;
                return T;
            }
            Result &setText(C Str &t, C SourceLoc &source_loc, Int line = -1) {
                T.text = t;
                T.source_loc = source_loc;
                T.line = line;
                return T;
            }
        };
