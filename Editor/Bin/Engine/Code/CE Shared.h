...
inline Str RandomName() {
    UID id;
    id.randomize();
    return TextHex(id.i[0], 8);
}

inline void ReplacePath(Str &path, Str src, Str dest) {
    if (StartsPath(path, src))
        path = dest.tailSlash(true) + SkipStartPath(path, src);
}
