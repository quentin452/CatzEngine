...
struct Line : Str, Text, LineMode {
    ...
    Line() {
        changed = true;
        text_valid = false;
        tokens_preproc_use = tokens_preproc_condition_unavailable = false;
        line = -1;
        id.randomize();
        source = null;
    }
    ...
    void resetTokens() {
        LineMode::resetTokens();
        comment_mode.resetTokens();
        REPAO(tokens_preproc).reset();
    }
    void resetPreproc(Bool inside_macro_param) {
        text_valid = false;
        starts_with_macro_param = inside_macro_param;
        tokens_preproc_use = inside_macro_param;
        tokens_preproc.clear();
        REPAO(tokens).macro = false;
    }
    ...
    Char chrNext(Int x, Int *pos = null) {
...
struct PrepCond // Preprocess Conditional (#if, #ifdef, #ifndef, #elif, #else, #endif)
{
    ...
    void set(Bool can_enter, Bool currently_valid) {
        T.can_enter = can_enter;
        T.currently_valid = currently_valid;
    }
};
/******************************************************************************/
const_mem_addr struct Source : Region {
    ...
    void detectDataTypes(SymbolPtr parent = null) {
        if (active)
            DetectDataTypes(symbols, decls, tokens, parent);
    }
    void linkDataTypes() {
        if (active)
            LinkDataTypes(symbols, tokens);
    }
    void detectVarFuncs() {
        if (active)
            DetectVarFuncs(symbols, tokens);
    }
