...
struct Symbol : Str // C++ Symbol
{
    ...
    static ACCESS_LEVEL ModifToAccessLevel(UInt modifiers) {
        if (modifiers & MODIF_PRIVATE)
            return ACCESS_PRIVATE;
        if (modifiers & MODIF_PROTECTED)
    ...
    static UInt AccessLevelToModif(ACCESS_LEVEL level) {
        switch (level) {
        default:
            return 0; // ACCESS_PUBLIC
        case ACCESS_PRIVATE:
            return MODIF_PRIVATE;
        case ACCESS_PROTECTED:
            return MODIF_PROTECTED;
        }
    }

    static void SetPrivate(UInt &modifiers) {
        modifiers &= ~MODIF_ACCESS;
        modifiers |= MODIF_PRIVATE;
    }
    static void SetProtected(UInt &modifiers) {
        modifiers &= ~MODIF_ACCESS;
        modifiers |= MODIF_PROTECTED;
    }
    ...
    struct Modif : SymbolPtr // pointer to symbol with modifiers
    {
        ...
        Bool save(File &f, StrLibrary &sl) {
            sl.putStr(f, name());
            f.putMulti(const_level, ptr_level, modifiers);
            f.cmpUIntV(array_dims.elms());
            FREPA(array_dims)
            f.cmpUIntV(((array_dims[i] >= DIM_MIN) ? array_dims[i] : DIM_INVALID) + DIM_MIN);
            f.cmpUIntV(templates.elms());
            FREPAO(templates).save(f, sl);
            sl.putStr(f, src_template.name());
            return true;
        }
        Bool load(File &f, StrLibrary &sl) {
            require(sl.getStr(f));
            f.getMulti(const_level, ptr_level, modifiers);
            array_dims.setNum(f.decUIntV());
            FREPA(array_dims)
            array_dims[i] = f.decUIntV() - DIM_MIN;
            templates.setNum(f.decUIntV());
            FREPAO(templates).load(f, sl);
            src_template = sl.getStr(f);
            return true;
        }
    ...
    Bool save(File &f, StrLibrary &sl) {
        ...
        f.cmpIntV(token_index).cmpIntV(type_range.x).cmpIntV(type_range.y).cmpIntV(def_range.x).cmpIntV(def_range.y);
        if (modifiers & MODIF_DEF_VALUE)
            f.cmpIntV(def_val_range.x).cmpIntV(def_val_range.y);

        value.save(f, sl);
        f.cmpUIntV(funcs.elms());
        FREPA(funcs)
        sl.putStr(f, funcs[i].name());
        f.cmpUIntV(templates.elms());
        FREPA(templates)
        sl.putStr(f, templates[i].name());
        f.cmpUIntV(params.elms());
        FREPA(params)
        sl.putStr(f, params[i].name());
        f.cmpUIntV(ctor_inits.elms());
        FREPA(ctor_inits)
        sl.putStr(f, ctor_inits[i].name());
        f.cmpUIntV(base.elms());
        FREPAO(base).save(f, sl);
    ...
    Bool load(File &f, StrLibrary &sl, Source &source) {
        ...
        f.decIntV(token_index).decIntV(type_range.x).decIntV(type_range.y).decIntV(def_range.x).decIntV(def_range.y);
        if (modifiers & MODIF_DEF_VALUE)
            f.decIntV(def_val_range.x).decIntV(def_val_range.y);

        value.load(f, sl);
        funcs.setNum(f.decUIntV());
        FREPA(funcs)
        funcs[i] = sl.getStr(f);
        templates.setNum(f.decUIntV());
        FREPA(templates)
        templates[i] = sl.getStr(f);
        params.setNum(f.decUIntV());
        FREPA(params)
        params[i] = sl.getStr(f);
        ctor_inits.setNum(f.decUIntV());
        FREPA(ctor_inits)
        ctor_inits[i] = sl.getStr(f);
        base.setNum(f.decUIntV());
        FREPAO(base).load(f, sl);
        parent = sl.getStr(f);
        T.source = &source;
        return true;
    }
