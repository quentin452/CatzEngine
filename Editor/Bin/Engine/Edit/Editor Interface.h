...
struct Elm // Project Element
{
    ...
    Bool save(File &f) C {
        f << type << flags << id << parent_id << name << src_file;
        return f.ok();
    }
    Bool load(File &f) {
        f >> type >> flags >> id >> parent_id >> name >> src_file;
...
struct Project // Project
{
    ...
    Project &set(C UID &id, C Str &name) {
        T.id = id;
        T.name = name;
        return T;
    }
...
/******************************************************************************/
T1(TYPE)
struct IDParam // ID with a custom parameter
{
    UID id;
    TYPE value;
...
struct WorldObjParams // World Object parameters
{
    ...
    WorldObjParams &set(C UID &id, C Matrix &matrix, Bool align_to_terrain = false, Flt align_to_terrain_normal = 0) {
        T.id = id;
        T.matrix = matrix;
        T.align_to_terrain = align_to_terrain;
        T.align_to_terrain_normal = align_to_terrain_normal;
        return T;
    }

    Bool save(File &f) C {
        f << id << matrix << align_to_terrain << align_to_terrain_normal;
        return f.ok();
    }
    Bool load(File &f) {
        f >> id >> matrix >> align_to_terrain >> align_to_terrain_normal;
...
struct WorldObjDesc // Basic Description of a World Object
{
    ...
    Bool save(File &f) C {
        f << instance_id << elm_obj_id << matrix << selected << removed;
        return f.ok();
    }
    Bool load(File &f) {
        f >> instance_id >> elm_obj_id >> matrix >> selected >> removed;
...
struct EditorInterface {
    ...
    struct Line {
        ...
        Line &set(C Color &color, C Edge &edge) {
            T.color = color;
            T.edge = edge;
            return T;
        }
