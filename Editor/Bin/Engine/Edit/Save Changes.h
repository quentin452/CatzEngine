...
struct SaveChanges : ClosableWindow {
    struct Elm {
        ...
        // set
        Elm &set(C Str &name, C Str &display, C UID &id = UIDZero, Ptr user = null) {
            T.name = name;
            T.display = display;
            T.id = id;
            T.user = user;
        ...
        Elm &save(Bool (*func)(Elm &elm) = null) {
            T._save = func;
            return T;
        }
        Elm &close(void (*func)(Elm &elm) = null) {
            T._close = func;
            return T;
        }
    ...
    T1(TYPE)
    void set(C CMemPtr<Elm> &elms, void (*after_save_close)(Bool all_saved, TYPE *user), TYPE *user) { set(elms, (void (*)(Bool, Ptr))after_save_close, user); }
    T1(TYPE)
    void set(C CMemPtr<Elm> &elms, void (*after_save_close)(Bool all_saved, TYPE &user), TYPE &user) { set(elms, (void (*)(Bool, Ptr))after_save_close, &user); }
