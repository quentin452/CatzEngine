...
const_mem_addr struct Viewport4 {
    ...
    Bool visible() C;
    Viewport4 &visible(Bool on); // get/set viewport visibility
    C Rect &rect() C { return _rect; }
    Viewport4 &rect(C Rect &rect); // get/set viewport rectangle
    Bool fpp() C { return _fpp; }
    Viewport4 &fpp(Bool on); // get/set if camera in   fpp      mode
    Bool lock() C { return _lock; }
    Viewport4 &lock(Bool on); // get/set if camera in   lock     mode
    Bool perspective() C { return _perspective; }
    Viewport4 &perspective(Bool on); // get/set if perspective view     mode
    Bool horizontal() C { return _horizontal; }
    Viewport4 &horizontal(Bool on); // get/set if horizontal  movement mode
    Bool keyboard() C { return _keyboard; }
    Viewport4 &keyboard(Bool on); // get/set if keyboard    movement enabled
    Int sel() C { return _sel; }
    Viewport4 &sel(Int view); // get/set selected view (VIEW_TYPE), -1=all
    Flt fppSpeed() C { return _fpp_speed; }
    Viewport4 &fppSpeed(Flt speed); // get/set FPP camera speed, default=5
    Flt perspFov() C { return _persp_fov; }
    Viewport4 &perspFov(Flt fov); // get/set perspective FOV , default=DegToRad(70)
    Bool drawZoom() C { return _draw_zoom; }
    Viewport4 &drawZoom(Bool on); // get/set camera zoom        visibility
    ORIENT_MODE orientMode() C { return _orn_mode; }
    Viewport4 &orientMode(ORIENT_MODE mode); // get/set camera orientation mode
    MOVE_MODE moveMode() C { return _move_mode; }
    Viewport4 &moveMode(MOVE_MODE mode); // get/set camera movement    mode
