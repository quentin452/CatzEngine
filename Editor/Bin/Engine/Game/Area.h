...
struct AreaPath2D {
  ...
  private:
    ...
    struct Neighbor {
        Byte a, b, cost;
#if EE_PRIVATE
        void set(Byte a, Byte b, Byte cost) {
            T.a = a;
            T.b = b;
            T.cost = cost;
        }
...
struct Area // World Area
{
    ...
    T1(TYPE)
    TYPE *data() { return CAST(TYPE, _data); } // get 'Area.Data' (don't create data if it's null)
    Data *getData();
    T1(TYPE)
    TYPE *getData() { return CAST(TYPE, getData()); } // get 'Area.Data' (      create data if it's null, null can still be returned if this area has AREA_UNLOADED state)
