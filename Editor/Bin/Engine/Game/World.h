...
/******************************************************************************/
T1(TYPE)
struct ObjMap // Container for Game Objects, this is the same thing as 'Map<UID, TYPE>', except without the control of manual adding new elements, which should be handled by 'WorldManager' (through 'WorldManager.objCreate')
{
    ...
    // misc
    T1(BASE)
    operator ObjMap<BASE> &(); // casting to container of 'BASE' objects, 'TYPE' must be extended from BASE
    T1(BASE)
    operator C ObjMap<BASE> &() C; // casting to container of 'BASE' objects, 'TYPE' must be extended from BASE
...
struct WorldSettings // World Settings
{
    ...
    Flt areaSize() C { return _area_size; }
    WorldSettings &areaSize(Flt size); // get/set area size           , 0..Inf, default=32
    Int hmRes() C { return _hm_res; }
    WorldSettings &hmRes(Int res); // get/set heightmap resolution, 2..129, default=65, setting resolution will automatically align the value to the "(nearest power of 2)+1"
    Int path2DRes() C { return _path2d_res; }
    WorldSettings &path2DRes(Int res); // get/set 2d paths  resolution, 1..64 , default=32
...
struct WorldManager // World Manager
{
    ...
    // init
    T1(TYPE)
    WorldManager &setAreaClass() {
        _grid.replaceClass<TYPE>();
        return T;
    } // set class responsible for areas    , TYPE must be extended from 'Game.Area'
    T1(TYPE)
    WorldManager &setAreaData() { return _setAreaData<TYPE>(); } // set class responsible for area data, TYPE must be extended from 'Game.Area.Data'
    T1(TYPE)
    WorldManager &setObjType(ObjMap<TYPE> &obj_map, Int obj_type) { return _setObjType(&SCAST(ObjMap<Obj>, obj_map), obj_type, CType<TYPE>()); } // set   memory container responsible for selected OBJ_TYPE
    ...
    struct AreaState {
        ...
        void set(C VecI2 &xz, AREA_STATE state) {
            T.xz = xz;
            T.state = state;
        }
    ...
    WorldManager &objGet(MemPtr<Obj *> objects, C Ball &ball, Int obj_type = -1) {
        objects.clear();
        return objGetAdd(objects, ball, obj_type);
    } // get pointers to objects which position (Game.Obj.pos) is inside 'ball'   , process only objects with 'obj_type' OBJ_TYPE (-1=process all types)
    WorldManager &objGet(MemPtr<Obj *> objects, C Capsule &capsule, Int obj_type = -1) {
        objects.clear();
        return objGetAdd(objects, capsule, obj_type);
    } // get pointers to objects which position (Game.Obj.pos) is inside 'capsule', process only objects with 'obj_type' OBJ_TYPE (-1=process all types)
    WorldManager &objGet(MemPtr<Obj *> objects, C Box &box, Int obj_type = -1) {
        objects.clear();
        return objGetAdd(objects, box, obj_type);
    } // get pointers to objects which position (Game.Obj.pos) is inside 'box'    , process only objects with 'obj_type' OBJ_TYPE (-1=process all types)
    WorldManager &objGet(MemPtr<Obj *> objects, C OBox &obox, Int obj_type = -1) {
        objects.clear();
        return objGetAdd(objects, obox, obj_type);
    } // get pointers to objects which position (Game.Obj.pos) is inside 'obox'   , process only objects with 'obj_type' OBJ_TYPE (-1=process all types)
  ...
  private:
    ...
    Memc<Decal2> _decals;
    struct MeshOverlay2 : MeshOverlay {
        Flt time;
    };
    Memc<MeshOverlay2> _mesh_overlays;
...
#endif
    T1(TYPE)
    static Area::Data &NewAreaData(Area::Data *&data, Area &area) {
        data = new TYPE(area);
        return *data;
    }
    T1(TYPE)
    WorldManager &_setAreaData() {
        ASSERT_BASE_EXTENDED<Area::Data, TYPE>();
        _area_data = NewAreaData<TYPE>;
        return T;
    }
...
#endif
    T1(TYPE)
    friend struct EE::Reference;
    NO_COPY_CONSTRUCTOR(WorldManager);
} extern World; // Main World
/******************************************************************************/
} // namespace Game
/******************************************************************************/
