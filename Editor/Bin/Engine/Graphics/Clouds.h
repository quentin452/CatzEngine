...
struct LayeredClouds {
    struct Layer // Cloud Layer
    {
        ...
        C Vec4 &colorL() C { return color_l; }
        void colorL(C Vec4 &color_l) { T.color_l = color_l; } // get/set Linear Gamma color
        Vec4 colorS() C;
        void colorS(C Vec4 &color_s); // get/set sRGB   Gamma color
#if EE_PRIVATE
#if LINEAR_GAMMA
    ...
    // set / get
    LayeredClouds &set(Byte active_layers, C ImagePtr &image = null);
    Int layers() C { return _layers; } // set/get number of active layers 0..4, if 'image'!=null layers will have 'image' set as their texture
    LayeredClouds &frac(Flt frac);
    Flt frac() C { return _frac; } // set/get cloud viewport fraction  , 0..1  , default=0.9 , fraction of the Viewport range where clouds start (1 is the fastest)
    LayeredClouds &scaleY(Flt scale);
    Flt scaleY() C { return _scale_y; } // set/get Y scaling                , 1..2  , default=1.05, setting this value higher than 1 helps covering the empty gap between flat ground and the clouds
...
struct AllClouds {
    ...
    void shadowMap() {
        layered.shadowMap();
        volumetric.shadowMap();
    } // !! Warning: 'Frustum' is invalid here !!
