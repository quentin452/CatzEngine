...
struct MaterialParams // Material Parameters
{
    ...
    C Vec &emissiveL() C { return emissive_l; }
    void emissiveL(C Vec &emissive_l) { T.emissive_l = emissive_l; } // get/set Linear Gamma emissive
    Vec emissiveS() C;
    void emissiveS(C Vec &emissive_s); // get/set sRGB   Gamma emissive

    Flt reflect() C { return reflect_add; }
    void reflect(Flt reflect); // get/set reflectivity, 0..1, default=MATERIAL_REFLECT
    Flt reflectMax() C;
    void reflect(Flt min, Flt max); // advanced
#if EE_PRIVATE
#if LINEAR_GAMMA
...
struct Material : MaterialParams // Mesh Rendering Material - contains render parameters and textures
{
  ...
  private:
    ...
    struct MaterialShader // Material->Shader link
    {
        ...
        void clear() {
            shader = null;
            next_material_shader = shader_material = -1;
        }
...
struct UniqueMultiMaterialKey {
    ...
    UniqueMultiMaterialKey(C Material *a, C Material *b, C Material *c, C Material *d) {
        m[0] = a;
        m[1] = b;
        m[2] = c;
        m[3] = d;
