...
struct Particles // Set of Particles
{
    ...
    Particles &palette(Bool palette);
    Bool palette() C { return _palette; } // if draw particles in palette mode (RM_PALETTE or RM_PALETTE1) instead of blend mode (RM_BLEND)        , false/true, default=false (particles in palette mode use only Alpha component from the Texture, while blend mode uses all RGBA channels from the Texture)
    Particles &paletteIndex(Byte index);
    Byte paletteIndex() C { return _palette_index; } // specifies which      palette mode (RM_PALETTE or RM_PALETTE1) should be used when 'palette' is enabled,    0..1   , defailt=0     (if 'palette' is enabled then this will specify whether RM_PALETTE index=0 or RM_PALETTE1 index=1 should be used)
...
struct RawParticles // buffered particles
{
    ...
    RawParticles &palette(Bool palette);
    Bool palette() C { return _palette; } // if draw particles in palette mode (RM_PALETTE or RM_PALETTE1) instead of blend mode (RM_BLEND)        , false/true, default=false (particles in palette mode use only Alpha component from the Texture, while blend mode uses all RGBA channels from the Texture)
    RawParticles &paletteIndex(Byte index);
    Byte paletteIndex() C { return _palette_index; } // specifies which      palette mode (RM_PALETTE or RM_PALETTE1) should be used when 'palette' is enabled,    0..1   , defailt=0     (if 'palette' is enabled then this will specify whether RM_PALETTE index=0 or RM_PALETTE1 index=1 should be used)
