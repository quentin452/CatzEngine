...
struct MatrixPair {
    ...
    void set(C Matrix &cur, C Matrix &prev) {
        T.cur = cur;
        T.prev = prev;
    }
...
struct BlendInstance {
    ...
    void setViewMatrix(C Matrix &view_matrix) {
        stat.view_matrix.cur = view_matrix;
        viewMatrixChanged();
    } // here 'stat.view_matrix.prev' is ignored because it's assumed to be unused
    void setViewMatrix(C Matrix &view_matrix, C Matrix &view_matrix_prev) {
        stat.view_matrix.cur = view_matrix;
        stat.view_matrix.prev = view_matrix_prev;
        viewMatrixChanged();
    }
