...
struct ShaderParam // Shader Parameter
{
    ...
    T1(TYPE)
    void set(C TYPE &data) { set((CPtr)&data, SIZE(data)); }
#if EE_PRIVATE
...
#define MIN_SHADER_PARAM_DATA_SIZE SIZEI(Vec4)
    ...
    struct Translation {
        ...
        void set(Int cpu_offset, Int gpu_offset, Int elm_size) {
            T.cpu_offset = cpu_offset;
            T.gpu_offset = gpu_offset;
            T.elm_size = elm_size;
        }
    };
...
struct ShaderParamChange // Shader Parameter Change
{
    ...
    ShaderParamChange &set(Bool b) {
        value.x = b;
        return T;
    }
    ShaderParamChange &set(Int i) {
        value.x = i;
        return T;
    }
    ShaderParamChange &set(Flt f) {
        value.x = f;
        return T;
    }
    ShaderParamChange &set(C Vec2 &v) {
        value.xy = v;
        return T;
    }
    ShaderParamChange &set(C Vec &v) {
        value.xyz = v;
        return T;
    }
    ShaderParamChange &set(C Vec4 &v) {
        value = v;
        return T;
    }
    ShaderParamChange &set(C Color &v);

    ShaderParamChange &set(ShaderParam *param) {
        T.param = param;
        return T;
    }
...
struct ShaderBuffer // Constant Buffer
{
    struct Buffer {
        ...
        buffer; // keep this as first member because it's used most often
        Int size;
...
struct BufferLink {
    Int index;
    GPU_API(ShaderBuffer *, UInt)
    buffer;

    void set(Int index, GPU_API(ShaderBuffer *, UInt) buffer) {
        T.index = index;
        T.buffer = buffer;
    }
...
struct ImageLink {
    ...
    void set(Int index, ShaderImage &image) {
        T.index = index;
        T.image = &image;
    }
...
struct RWImageLink {
    ...
    void set(Int index, ShaderRWImage &image) {
        T.index = index;
        T.image = &image;
    }
...
struct BufferLinkPtr {
    ...
    C BufferLink &operator[](Int i) C {
        DEBUG_RANGE_ASSERT(i, elms);
        return data[i];
    }
    void operator=(C Mems<BufferLink> &links) {
        data = links.data();
        elms = links.elms();
    }
};
struct ImageLinkPtr {
    ...
    C ImageLink &operator[](Int i) C {
        DEBUG_RANGE_ASSERT(i, elms);
        return data[i];
    }
    void operator=(C Mems<ImageLink> &links) {
        data = links.data();
        elms = links.elms();
    }
};
struct RWImageLinkPtr {
    ...
    C RWImageLink &operator[](Int i) C {
        DEBUG_RANGE_ASSERT(i, elms);
        return data[i];
    }
    void operator=(C Mems<RWImageLink> &links) {
        data = links.data();
        elms = links.elms();
    }
};
...
struct Shader11 {
    ...
    Int data_index[ST_BASE] = {-1, -1, -1, -1};
    ASSERT(ST_BASE == 4);
    Str8 name;
...
struct SamplerImageLink {
    ...
    void set(Int index, Int sampler, ShaderImage &image) {
        T.index = index;
        T.sampler = sampler;
        T.image = &image;
    }
...
struct ShaderBase {
    ...
    INLINE Shader &getShader(Bool forward) {
        if (forward)
            return asForwardShader();
        else
            return asShader();
    }
    INLINE Shader &getBlendShader(Bool blst) {
        if (blst)
            return asBlendShader();
        else
            return asShader();
    }
...
struct ShaderMacro // macro used for shader compilation
{
    ...
    void set(C Str8 &name, C Str8 &definition) {
        T.name = name;
...
struct ShaderGLSL {
    ...
    ShaderGLSL &set(C Str8 &group_name, C Str8 &tech_name) {
        T.group_name = group_name;
        T.tech_name = tech_name;
        return T;
    }
    ShaderGLSL &par(C Str8 &name, C Str8 &definition) {
        params.New().set(name, definition);
        return T;
    }
};
#endif
...
inline void SPSet(CChar8 *name, Bool b) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(b);
} // set boolean  value
inline void SPSet(CChar8 *name, Int i) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(i);
} // set integer  value
inline void SPSet(CChar8 *name, Flt f) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(f);
} // set float    value
inline void SPSet(CChar8 *name, Dbl d) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(d);
} // set double   value
inline void SPSet(CChar8 *name, C Vec2 &v) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(v);
} // set vector2D value
inline void SPSet(CChar8 *name, C VecD2 &v) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(v);
} // set vector2D value
inline void SPSet(CChar8 *name, C VecI2 &v) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(v);
} // set vector2D value
inline void SPSet(CChar8 *name, C Vec &v) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(v);
} // set vector3D value
inline void SPSet(CChar8 *name, C VecD &v) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(v);
} // set vector3D value
inline void SPSet(CChar8 *name, C VecI &v) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(v);
} // set vector3D value
inline void SPSet(CChar8 *name, C Vec4 &v) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(v);
} // set vector4D value
inline void SPSet(CChar8 *name, C VecD4 &v) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(v);
} // set vector4D value
inline void SPSet(CChar8 *name, C VecI4 &v) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(v);
} // set vector4D value
inline void SPSet(CChar8 *name, C Color &color) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(color);
} // set vector4D value
inline void SPSet(CChar8 *name, C Rect &rect) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(rect);
} // set vector4D value
inline void SPSet(CChar8 *name, C Matrix3 &matrix) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(matrix);
} // set matrix   value
inline void SPSet(CChar8 *name, C Matrix &matrix) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(matrix);
} // set matrix   value
inline void SPSet(CChar8 *name, C Matrix4 &matrix) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(matrix);
} // set matrix4  value
inline void SPSet(CChar8 *name, C Vec *v, Int elms) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(v, elms);
} // set vector3D array
inline void SPSet(CChar8 *name, C Vec4 *v, Int elms) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(v, elms);
} // set vector4D array
inline void SPSet(CChar8 *name, C Matrix *matrix, Int elms) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(matrix, elms);
} // set matrix   array
inline void SPSet(CChar8 *name, CPtr data, Int size) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(data, size);
} // set memory
T1(TYPE)
inline void SPSet(CChar8 *name, C TYPE &data) {
    if (ShaderParam *sp = FindShaderParam(name))
        sp->set(data);
} // set memory

inline void SPISet(CChar8 *name, Int i) {
    if (ShaderParamInt *sp = FindShaderParamInt(name))
        sp->set(i);
} // set integer  value
inline void SPISet(CChar8 *name, C VecI2 &v) {
    if (ShaderParamInt *sp = FindShaderParamInt(name))
        sp->set(v);
} // set vector2D value
inline void SPISet(CChar8 *name, C VecI &v) {
    if (ShaderParamInt *sp = FindShaderParamInt(name))
        sp->set(v);
} // set vector3D value
inline void SPISet(CChar8 *name, C VecI4 &v) {
    if (ShaderParamInt *sp = FindShaderParamInt(name))
        sp->set(v);
} // set vector4D value
