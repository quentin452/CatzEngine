...
struct VtxFormatKey {
    ...
    VtxFormatKey() {}
    VtxFormatKey(MESH_FLAG flag, UInt compress) {
        T.flag = flag;
        T.compress = compress;
    }
};
...
struct VtxFormatGL {
    struct Elm {
        ...
        void set(GL_VTX_SEMANTIC semantic, Byte component_num, GLenum component_type, Bool component_normalize, UInt offset) {
            T.semantic = semantic;
            T.component_num = component_num;
            T.component_type = component_type;
            T.component_normalize = component_normalize;
            T.offset = offset;
        }
    };
    Mems<Elm> elms;
...
// draw
#if DX11
    void set(Int stream = 0) C {
        UInt stride = _vtx_size, offset = 0;
        D3DC->IASetVertexBuffers(stream, 1, &_buf, &stride, &offset);
    }
#elif GL
...
#if EE_PRIVATE
    GPU_API(
        ID3D11Buffer *_buf, union {UInt _buf; Ptr _buf_ptr; });
#else
    Ptr _buf;
#endif
