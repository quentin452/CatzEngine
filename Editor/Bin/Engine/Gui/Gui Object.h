...
const_mem_addr struct GuiObj // Gui Object interface inherited by all Gui Object classes (Button, CheckBox, Window, ..) !! must be stored in constant memory address !!
{
    ...
    GuiObj &visibleActivate(Bool on) {
        if (on) {
            if (hidden())
                activate();
        } else
            hide();
        return T;
    } // set    visibility and activate if was hidden
    GuiObj &visibleToggle() {
        if (hidden())
            show();
        else
            hide();
    ...
    GuiObj &visibleToggleActivate() {
        if (hidden())
            activate();
        else
            hide();
        return T;
    }                          // toggle visibility and activate if was hidden
    ...
    GuiObj &hidden(Bool hidden);
    Bool hidden() C { return !_visible; } // set/get if hidden
    virtual GuiObj &enabled(Bool enabled);
    Bool enabled() C { return !_disabled; } // set/get if  enabled
    virtual GuiObj &disabled(Bool disabled);
    Bool disabled() C { return _disabled; } // set/get if disabled
    virtual GuiObj &desc(C Str &desc);
    C Str &desc() C { return _desc; } // set/get description
    virtual GuiObj &rect(C Rect &rect);
    C Rect &rect() C { return _rect; } // set/get rectangle
    virtual GuiObj &size(C Vec2 &size);
    Vec2 size() C { return _rect.size(); } // set/get size
    virtual GuiObj &pos(C Vec2 &pos);
    Vec2 pos() C { return _rect.lu(); } // set/get top    left  position
    virtual GuiObj &posRU(C Vec2 &pos);
    Vec2 posRU() C { return _rect.ru(); } // set/get top    right position
    virtual GuiObj &posLD(C Vec2 &pos);
    Vec2 posLD() C { return _rect.ld(); } // set/get bottom left  position
    virtual GuiObj &posL(C Vec2 &pos);
    Vec2 posL() C { return _rect.left(); } // set/get left         position
    virtual GuiObj &posR(C Vec2 &pos);
    Vec2 posR() C { return _rect.right(); } // set/get right        position
    virtual GuiObj &posD(C Vec2 &pos);
    Vec2 posD() C { return _rect.down(); } // set/get bottom       position
    virtual GuiObj &posU(C Vec2 &pos);
    Vec2 posU() C { return _rect.up(); } // set/get top          position
    virtual GuiObj &posC(C Vec2 &pos);
    Vec2 posC() C { return _rect.center(); } // set/get center       position
    ...
    GuiObj &baseLevel(Int level);
    Int baseLevel() C { return _base_level; } // set/get base level
    ...
    // convert
    Button &asButton() { return (Button &)T; }
    C Button &asButton() C { return (Button &)T; } // return as Button   (you may use this only if type()==GO_BUTTON  )
    CheckBox &asCheckBox() { return (CheckBox &)T; }
    C CheckBox &asCheckBox() C { return (CheckBox &)T; } // return as CheckBox (you may use this only if type()==GO_CHECKBOX)
    ComboBox &asComboBox() { return (ComboBox &)T; }
    C ComboBox &asComboBox() C { return (ComboBox &)T; } // return as ComboBox (you may use this only if type()==GO_COMBOBOX)
    GuiCustom &asCustom() { return (GuiCustom &)T; }
    C GuiCustom &asCustom() C { return (GuiCustom &)T; } // return as Custom   (you may use this only if type()==GO_CUSTOM  )
    Desktop &asDesktop() { return (Desktop &)T; }
    C Desktop &asDesktop() C { return (Desktop &)T; } // return as Desktop  (you may use this only if type()==GO_DESKTOP )
    GuiImage &asImage() { return (GuiImage &)T; }
    C GuiImage &asImage() C { return (GuiImage &)T; } // return as Image    (you may use this only if type()==GO_IMAGE   )
    _List &asList() { return (_List &)T; }
    C _List &asList() C { return (_List &)T; } // return as List     (you may use this only if type()==GO_LIST    )
    Menu &asMenu() { return (Menu &)T; }
    C Menu &asMenu() C { return (Menu &)T; } // return as Menu     (you may use this only if type()==GO_MENU    )
    MenuBar &asMenuBar() { return (MenuBar &)T; }
    C MenuBar &asMenuBar() C { return (MenuBar &)T; } // return as MenuBar  (you may use this only if type()==GO_MENU_BAR)
    Progress &asProgress() { return (Progress &)T; }
    C Progress &asProgress() C { return (Progress &)T; } // return as Progress (you may use this only if type()==GO_PROGRESS)
    Region &asRegion() { return (Region &)T; }
    C Region &asRegion() C { return (Region &)T; } // return as Region   (you may use this only if type()==GO_REGION  )
    SlideBar &asSlideBar() { return (SlideBar &)T; }
    C SlideBar &asSlideBar() C { return (SlideBar &)T; } // return as SlideBar (you may use this only if type()==GO_SLIDEBAR)
    Slider &asSlider() { return (Slider &)T; }
    C Slider &asSlider() C { return (Slider &)T; } // return as Slider   (you may use this only if type()==GO_SLIDER  )
    Tab &asTab() { return (Tab &)T; }
    C Tab &asTab() C { return (Tab &)T; } // return as Tab      (you may use this only if type()==GO_TAB     )
    Tabs &asTabs() { return (Tabs &)T; }
    C Tabs &asTabs() C { return (Tabs &)T; } // return as Tabs     (you may use this only if type()==GO_TABS    )
    Text &asText() { return (Text &)T; }
    C Text &asText() C { return (Text &)T; } // return as Text     (you may use this only if type()==GO_TEXT    )
    TextBox &asTextBox() { return (TextBox &)T; }
    C TextBox &asTextBox() C { return (TextBox &)T; } // return as TextBox  (you may use this only if type()==GO_TEXTBOX )
    TextLine &asTextLine() { return (TextLine &)T; }
    C TextLine &asTextLine() C { return (TextLine &)T; } // return as TextLine (you may use this only if type()==GO_TEXTLINE)
    Viewport &asViewport() { return (Viewport &)T; }
    C Viewport &asViewport() C { return (Viewport &)T; } // return as Viewport (you may use this only if type()==GO_VIEWPORT)
    Window &asWindow() { return (Window &)T; }
    C Window &asWindow() C { return (Window &)T; } // return as Window   (you may use this only if type()==GO_WINDOW  )
