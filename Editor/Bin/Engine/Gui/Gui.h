...
struct GUI // Graphical User Interface
{
    ...
    // operations
    T1(TYPE)
    void operator+=(TYPE &child) {
        if (_desktop)
            *_desktop += child;
    } // add    gui object to   active desktop
    T1(TYPE)
    void operator-=(TYPE &child) {
        if (_desktop)
            *_desktop -= child;
    } // remove gui object from active desktop
    ...
    GUI &windowButtonsRight(Bool right);
    Bool windowButtonsRight() C { return _window_buttons_right; } // set/get if Window Buttons (minimise, maximise, close) should appear on the right side of the Window, default=false for Mac and true otherwise
    ...
    T1(TYPE)
    void addFuncCall(void func(TYPE *user), TYPE *user) { addFuncCall((void (*)(Ptr))func, user); } // add custom function to the gui function callback list to be automatically called at the end of the 'Gui.update'
    T1(TYPE)
    void addFuncCall(void func(TYPE &user), TYPE &user) { addFuncCall((void (*)(Ptr))func, &user); } // add custom function to the gui function callback list to be automatically called at the end of the 'Gui.update'
  ...
  private:
    ...
    Bool Switch();
    void kbLit(GuiObj *obj, C Rect &rect, C GuiSkin *skin) {
        if (draw_keyboard_highlight)
            draw_keyboard_highlight(obj, rect, skin);
    }
    void setText();
