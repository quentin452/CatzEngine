...
const_mem_addr struct Button : GuiObj // Gui Button !! must be stored in constant memory address !!
{
    ...
    Button &subType(BUTTON_TYPE type);
    BUTTON_TYPE subType() C { return _sub_type; } // set/get button type, default=BUTTON_TYPE_DEFAULT
    virtual Button &enabled(Bool enabled) override;
    Bool enabled() C { return super::enabled(); } // set/get if  enabled
    virtual Button &disabled(Bool disabled) override;
    Bool disabled() C { return super::disabled(); } // set/get if disabled
    Button &focusable(Bool on);
    Bool focusable() C { return _focusable; } // set/get if can catch keyboard focus, default=true
    Button &setSkin(C GuiSkinPtr &skin);
    ...
    Button &funcImmediate(Bool immediate) {
        _func_immediate = immediate;
        return T;
    } // set immediate parameter for function called when button state has changed
