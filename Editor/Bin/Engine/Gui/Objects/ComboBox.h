...
const_mem_addr struct ComboBox : Button // Gui ComboBox !! must be stored in constant memory address !!
{
    ...
    ComboBox &setData(CChar8 *data[], Int elms, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        menu.setData(data, elms, visible, keep_cur);
        return T;
    } // set columns and data from text array
    ComboBox &setData(CChar *data[], Int elms, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        menu.setData(data, elms, visible, keep_cur);
        return T;
    } // set columns and data from text array
    ComboBox &setData(C Node<MenuElm> &node) {
        menu.setData(node);
        return T;
    } // set columns and data from node of menu elements

    ComboBox &setColumns(ListColumn *column, Int columns, Bool columns_hidden = true) {
        menu.setColumns(column, columns, columns_hidden);
        return T;
    } // set list columns
    T1(TYPE)
    ComboBox &setData(TYPE *data, Int elms, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        menu.setData(data, elms, visible, keep_cur);
        return T;
    } // set data from continuous memory
    T1(TYPE)
    ComboBox &setData(Mems<TYPE> &mems, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        menu.setData(mems, visible, keep_cur);
        return T;
    } // set data from Mems
    ComboBox &setData(_Memc &memc, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        menu.setData(memc, visible, keep_cur);
        return T;
    } // set data from Memc
    ComboBox &setData(_Memb &memb, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        menu.setData(memb, visible, keep_cur);
        return T;
    } // set data from Memb
    ComboBox &setData(_Memx &memx, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        menu.setData(memx, visible, keep_cur);
        return T;
    } // set data from Memx
    ComboBox &setData(_Meml &meml, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        menu.setData(meml, visible, keep_cur);
        return T;
    } // set data from Meml
    ComboBox &setData(_Map &map, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        menu.setData(map, visible, keep_cur);
        return T;
    } // set data from Map
    ...
    ComboBox &set(Int abs_sel, SET_MODE mode = SET_DEFAULT);
    Int operator()() C { return _abs_sel; }                                  // set/get active absolute selection, -1=none
...
#endif
    virtual ComboBox &rect(C Rect &rect) override;
    C Rect &rect() C { return super::rect(); } // set/get rectangle
    ComboBox &skin(C GuiSkinPtr &skin, Bool sub_objects = true);
    C GuiSkinPtr &skin() C { return super::skin; } // set/get skin override, default=null (if set to null then current value of 'Gui.skin' is used), 'sub_objects'=if additionally change the skin of sub-menus
    ...
    ComboBox &funcImmediate(Bool immediate) {
        _func_immediate = immediate;
        return T;
    } // set immediate parameter for function called when combobox state has changed
