...
struct ListColumn : Button // List Column
{
    ...
    T1(TYPE)
    explicit ListColumn(TYPE &member, Flt width, C Str &name); // create using member auto-detection     , 'member'      =custom class member - 'MEMBER' macro usage is required  , 'width'=column width (can also be set to LCW_DATA, LCW_PARENT or LCW_MAX_DATA_PARENT), 'name'=column name
    T1(TYPE)
    explicit ListColumn(Str (*data_to_text)(C TYPE &data), Flt width, C Str &name);    // create using manual conversion function, 'data_to_text'=custom function converting data to text                 , 'width'=column width (can also be set to LCW_DATA, LCW_PARENT or LCW_MAX_DATA_PARENT), 'name'=column name
...
const_mem_addr struct _List : GuiObj // Gui List !! must be stored in constant memory address !!
{
    ...
    T1(TYPE)
    _List &setData(TYPE *data, Int elms, C CMemPtr<Bool> &visible = null, Bool keep_cur = false); // set list data from continuous memory  !! after any change in source data, 'setData' must be called again !!
    T1(TYPE)
    _List &setData(Mems<TYPE> &mems, C CMemPtr<Bool> &visible = null, Bool keep_cur = false); // set list data from Mems               !! after any change in source data, 'setData' must be called again !!
    ...
    _List &columnsHidden(Bool hidden);
    Bool columnsHidden() C { return _columns_hidden; } // set/get if all columns are hidden
    Bool columnsVisible() C;                           //     get if all columns are visible and there are any columns
    _List &columnHeight(Flt base, Flt relative = 0);
    Flt columnHeight() C;                             // set/get columns           height  ,                     default=(0.055, 0.0   ), final height = "zoom * columnHeightRel() + columnHeightBase()"
    ...
    _List &elmHeight(Flt height);
    Flt elmHeight() C { return _elm_height; } // set/get elements          height  ,        0..Inf     , default= 0.050
    _List &textSize(Flt base, Flt relative = 0);
    Flt textSize() C;                           // set/get text              size    ,                   , default=(0.050, 0.0   ), final text  size = "zoom * elmHeight() *  textSizeRel() +  textSizeBase() "
    ...
    _List &imageSize(C Vec2 &base, Flt relative, C Rect &padding = RectZero);
    C Vec2 &imageSizeBase() C { return _image_base; }    // set/get image    base     size    ,        0..Inf     , default=(0.0  , 0.1/64), final text  size = "zoom * (image.size * imageSizeRel() + imageSizeBase())", 'padding'=padding applied to the whole element rectangle
    ...
    _List &zoom(Flt zoom);
    Flt zoom() C { return _zoom; } // set/get list     zoom             , zoom_min..zoom_max, default= 1.0
    _List &zoomStep(Flt step);
    Flt zoomStep() C { return _zoom_step; }                     // set/get list     zoom step        ,     -Inf..Inf     , default= 0.2, this affects the change of zoom when using Ctrl+MouseWheel on a zoomable List
    Flt zoomScaleFactor() C { return ScaleFactor(_zoom_step); } //     get list     zoom scale factor,        0..Inf     , default= 1.2
    _List &skin(C GuiSkinPtr &skin);
    C GuiSkinPtr &skin() C { return _skin; }                      // set/get skin override, default=null (if set to null then current value of 'Gui.skin' is used), changing this value will automatically change the skin of the list columns
    GuiSkin *getSkin() C { return _skin ? _skin() : Gui.skin(); } //     get actual skin
    TextStyle *getTextStyle() C;                                  //     get actual text style
    _List &horizontal(Bool horizontal);
    Bool horizontal() C { return _horizontal; } // set/get if LDM_RECTS mode should be displayed horizontally, default=false
    _List &vertical(Bool vertical);
    Bool vertical() C { return !_horizontal; } // set/get if LDM_RECTS mode should be displayed   vertically, default=true
    _List &focusable(Bool on);
    Bool focusable() C { return _focusable; } // set/get if can catch keyboard focus, default=true
    _List &drawMode(LIST_DRAW_MODE mode);
    LIST_DRAW_MODE drawMode() C { return _draw_mode; } // set/get draw      mode, LIST_DRAW_MODE, default=LDM_LIST
    ...
    _List &columnWidth(Int i, Flt width);
    Flt columnWidth(Int i) C; // set/get i-th column width, ('width' can also be set to LCW_DATA, LCW_PARENT or LCW_MAX_DATA_PARENT)
    _List &columnVisible(Int i, Bool visible);
    Bool columnVisible(Int i) C; // set/get i-th column visibility
    _List &columnText(Int i, C Str &text);
    C Str &columnText(Int i) C; // set/get i-th column text
    ...
    T1(TYPE)
    _List &curChanged(void (*func)(TYPE *user), TYPE *user) { return curChanged((void (*)(Ptr))func, user); } // set function called when list 'cur' cursor has changed, with 'user' as its parameter
    T1(TYPE)
    _List &curChanged(void (*func)(TYPE &user), TYPE &user) { return curChanged((void (*)(Ptr))func, &user); } // set function called when list 'cur' cursor has changed, with 'user' as its parameter
    ...
    T1(TYPE)
    _List &selChanged(void (*func)(TYPE *user), TYPE *user) { return selChanged((void (*)(Ptr))func, user); } // set function called when list 'sel' selection has changed, with 'user' as its parameter
    T1(TYPE)
    _List &selChanged(void (*func)(TYPE &user), TYPE &user) { return selChanged((void (*)(Ptr))func, &user); } // set function called when list 'sel' selection has changed, with 'user' as its parameter
    ...
    T1(TYPE)
    _List &selChanging(void (*func)(TYPE *user), TYPE *user) { return selChanging((void (*)(Ptr))func, user); } // set function called when list 'sel' selection is about to change, with 'user' as its parameter
    T1(TYPE)
    _List &selChanging(void (*func)(TYPE &user), TYPE &user) { return selChanging((void (*)(Ptr))func, &user); } // set function called when list 'sel' selection is about to change, with 'user' as its parameter
...
const_mem_addr T1(TYPE) struct List : _List // Gui List Template !! must be stored in constant memory address !!
{
    ...
    List &setData(TYPE *data, Int elms, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        super::setData(data, elms, visible, keep_cur);
        return T;
    } // set list data from continuous memory  !! after any change in source data, 'setData' must be called again !!
    List &setData(Mems<TYPE> &mems, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        super::setData(mems, visible, keep_cur);
        return T;
    } // set list data from Mems               !! after any change in source data, 'setData' must be called again !!
    List &setData(Memc<TYPE> &memc, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        super::setData(memc, visible, keep_cur);
        return T;
    } // set list data from Memc               !! after any change in source data, 'setData' must be called again !!
    List &setData(Memb<TYPE> &memb, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        super::setData(memb, visible, keep_cur);
        return T;
    } // set list data from Memb               !! after any change in source data, 'setData' must be called again !!
    List &setData(Memx<TYPE> &memx, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        super::setData(memx, visible, keep_cur);
        return T;
    } // set list data from Memx               !! after any change in source data, 'setData' must be called again !!
    List &setData(Meml<TYPE> &meml, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        super::setData(meml, visible, keep_cur);
        return T;
    } // set list data from Meml               !! after any change in source data, 'setData' must be called again !!
    T1(KEY)
    List &setData(Map<KEY, TYPE> &map, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        super::setData(map, visible, keep_cur);
        return T;
    } // set list data from Map                !! after any change in source data, 'setData' must be called again !!
    List &setDataNode(Memx<TYPE> &memx, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        super::setDataNode(memx, visible, keep_cur);
        return T;
    } // set list data from Memx with children !! after any change in source data, 'setData' must be called again !! TYPE must have a "Memx<TYPE> children" member, for example: "struct TYPE { Memx<TYPE> children; }", in that case 'setDataNode' will add all elements including children recursively
};
/******************************************************************************/
