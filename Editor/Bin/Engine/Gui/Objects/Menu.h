...
struct MenuElm // Menu Element
{
    ...
    T1(TYPE)
    MenuElm &create(C Str &name, void (*func)(TYPE *user), TYPE *user, Bool immediate = false) { return create(name, (void (*)(Ptr))func, user, immediate); } // create
    T1(TYPE)
    MenuElm &create(C Str &name, void (*func)(TYPE &user), TYPE &user, Bool immediate = false) { return create(name, (void (*)(Ptr))func, &user, immediate); } // create
#if EE_PRIVATE
    ...
    MenuElm &flag(Byte flag);
    Byte flag() C { return _flag; } // set/get MENU_FLAG
    MenuElm &kbsc(C KbSc &kbsc);
    KbSc kbsc() C { return _kbsc; } // set/get keyboard shortcut
    MenuElm &kbsc1(C KbSc &kbsc);
    KbSc kbsc1() C { return _kbsc1; } // set/get keyboard shortcut (alternative)
    MenuElm &desc(C Str &desc);
...
const_mem_addr struct Menu : GuiObj // Gui Menu !! must be stored in constant memory address !!
{
    ...
    T1(TYPE)
    Menu &func(void (*func)(C CMemPtr<MenuPush> &path, TYPE *user), TYPE *user) { return T.func((void (*)(C CMemPtr<MenuPush> &, Ptr))func, user); } // set function called when any sub-element is pushed, with 'user' as its parameter
    T1(TYPE)
    Menu &func(void (*func)(C CMemPtr<MenuPush> &path, TYPE &user), TYPE &user) { return T.func((void (*)(C CMemPtr<MenuPush> &, Ptr))func, &user); } // set function called when any sub-element is pushed, with 'user' as its parameter
    ...
    T1(TYPE)
    Menu &setData(TYPE *data, Int elms, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        list.setData(data, elms, visible, keep_cur);
        return T;
    } // set data from continuous memory
    T1(TYPE)
    Menu &setData(Mems<TYPE> &mems, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        list.setData(mems, visible, keep_cur);
        return T;
    } // set data from Mems
    Menu &setData(_Memc &memc, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        list.setData(memc, visible, keep_cur);
        return T;
    } // set data from Memc
    Menu &setData(_Memb &memb, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        list.setData(memb, visible, keep_cur);
        return T;
    } // set data from Memb
    Menu &setData(_Memx &memx, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        list.setData(memx, visible, keep_cur);
        return T;
    } // set data from Memx
    Menu &setData(_Meml &meml, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        list.setData(meml, visible, keep_cur);
        return T;
    } // set data from Meml
    Menu &setData(_Map &map, C CMemPtr<Bool> &visible = null, Bool keep_cur = false) {
        list.setData(map, visible, keep_cur);
        return T;
    } // set data from Map
    ...
    virtual Menu &rect(C Rect &rect) override;
    C Rect &rect() C { return super::rect(); } // set/get rectangle
    virtual Menu &pos(C Vec2 &pos) override;
    Vec2 pos() C { return super::pos(); } // set/get top    left  position
    virtual Menu &posRU(C Vec2 &pos) override;
    Vec2 posRU() C { return super::posRU(); } // set/get top    right position
    virtual Menu &posLD(C Vec2 &pos) override;
    Vec2 posLD() C { return super::posLD(); } // set/get bottom left  position
    virtual Menu &posL(C Vec2 &pos) override;
    Vec2 posL() C { return super::posL(); } // set/get left         position
    virtual Menu &posR(C Vec2 &pos) override;
    Vec2 posR() C { return super::posR(); } // set/get right        position
    virtual Menu &posD(C Vec2 &pos) override;
    Vec2 posD() C { return super::posD(); } // set/get bottom       position
    virtual Menu &posU(C Vec2 &pos) override;
    Vec2 posU() C { return super::posU(); } // set/get top          position
    virtual Menu &posC(C Vec2 &pos) override;
    Vec2 posC() C { return super::posC(); }     // set/get center       position
    ...
    Menu &skin(C GuiSkinPtr &skin, Bool sub_objects = true);
    C GuiSkinPtr &skin() C { return _skin; }                      // set/get skin override, default=null (if set to null then current value of 'Gui.skin' is used), 'sub_objects'=if additionally change the skin of the 'list' and sub-menus
...
/******************************************************************************/
inline Node<MenuElm> &operator+=(Node<MenuElm> &node, C Str &name) {
    Node<MenuElm> &child = node.New();
    child.create(name);
    return child;
}
