...
const_mem_addr struct MenuBar : GuiObj // Gui Menu Bar !! must be stored in constant memory address !!
{
    ...
    virtual MenuBar &rect(C Rect &rect) override;
    C Rect &rect() C { return super::rect(); } // set/get rectangle
    Rect elmsRect() C;                         //     get rectangle covering all Menu Elements

    MenuBar &skin(C GuiSkinPtr &skin, Bool sub_objects = true);
    C GuiSkinPtr &skin() C { return _skin; }                      // set/get skin override, default=null (if set to null then current value of 'Gui.skin' is used), 'sub_objects'=if additionally change the skin of sub-menus
...
#if EE_PRIVATE
    friend struct GUI;
    friend struct GuiObjChildren;
    friend struct GuiObj;
#endif
};
/******************************************************************************/
