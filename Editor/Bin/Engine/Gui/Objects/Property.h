...
const_mem_addr struct Property // Class Member Gui Control !! must be stored in constant memory address !!
{
    ...
    // get / set
    Property &display(C Str &name);
    C Str &display() C { return text.text; } // set/get property display name

    Property &desc(C Str &desc);
    C Str &desc() C; // set/get property description

    Property &autoData(Ptr object);
    Ptr autoData() C { return _auto_data; } // set/get automatic data setting

    Property &set(Bool value, SET_MODE mode = SET_DEFAULT);
    Bool asBool() C;
    Bool asBool(CPtr object) C { return md.asBool(object); } // set  /  get from gui  /  get from data, false       on fail
    Property &set(Int value, SET_MODE mode = SET_DEFAULT);
    Int asInt() C;
    Int asInt(CPtr object) C { return md.asInt(object); } // set  /  get from gui  /  get from data, 0           on fail
    Property &set(Long value, SET_MODE mode = SET_DEFAULT);
    Long asLong() C;
    Long asLong(CPtr object) C { return md.asLong(object); } // set  /  get from gui  /  get from data, 0           on fail
    Property &set(Flt value, SET_MODE mode = SET_DEFAULT);
    Flt asFlt() C;
    Flt asFlt(CPtr object) C { return md.asFlt(object); } // set  /  get from gui  /  get from data, 0           on fail
    Property &set(Dbl value, SET_MODE mode = SET_DEFAULT);
    Dbl asDbl() C;
    Dbl asDbl(CPtr object) C { return md.asDbl(object); } // set  /  get from gui  /  get from data, 0           on fail
    Property &set(C Vec2 &value, SET_MODE mode = SET_DEFAULT);
    Vec2 asVec2() C;
    Vec2 asVec2(CPtr object) C { return md.asVec2(object); } // set  /  get from gui  /  get from data, Vec2 (0, 0) on fail
    Property &set(C VecD2 &value, SET_MODE mode = SET_DEFAULT);
    VecD2 asVecD2() C;
    VecD2 asVecD2(CPtr object) C { return md.asVecD2(object); } // set  /  get from gui  /  get from data, VecD2(0, 0) on fail
    Property &set(C Str &value, SET_MODE mode = SET_DEFAULT);
    Str asText() C;
    Str asText(CPtr object) C { return md.asText(object, real_precision); } // set  /  get from gui  /  get from data, ""          on fail
    Int asEnum(CPtr object) C { return md.asEnum(object); }                 // set  /  get from gui  /  get from data, -1          on fail

    Property &min(Dbl value) {
        min_use = true;
        min_value = value;
        return T;
    } // set minimum allowed value
    Property &max(Dbl value) {
        max_use = true;
        max_value = value;
        return T;
    } // set maximum allowed value
    Property &range(Dbl min, Dbl max) {
        min_use = max_use = true;
        min_value = min;
        max_value = max;
        return T;
    ...
    Property &mouseEditLinked(Bool linked) {
        mouse_edit_linked = linked;
        return T;
    } //         default=false
    Property &mouseEditMode(PROP_MOUSE_EDIT_MODE mode) {
        mouse_edit_mode = mode;
        return T;
    } //         default=PROP_MOUSE_EDIT_LINEAR
    Property &mouseEditSpeed(Flt speed) {
        mouse_edit_speed = speed;
        return T;
    } // 0..Inf, default=1.0 (40 for Int's)

    Property &precision(Int precision) {
        real_precision = precision;
        return T;
    } // set real value precision
    ...
    Property &visible(Bool on);
    Bool visible() C;          // set/get visibility
