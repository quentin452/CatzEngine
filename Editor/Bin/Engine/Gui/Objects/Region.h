...
const_mem_addr struct Region : GuiObj // Gui Region !! must be stored in constant memory address !!
{
    ...
    Region &scrollX(Flt delta, Bool immediate = false) {
        slidebar[0].scroll(delta, immediate);
        return T;
    } // horizontal scroll by delta
    Region &scrollToX(Flt pos, Bool immediate = false) {
        slidebar[0].scrollTo(pos, immediate);
        return T;
    } // horizontal scroll to pos
    Region &scrollFitX(Flt min, Flt max, Bool immediate = false) {
        slidebar[0].scrollFit(min, max, immediate);
        return T;
    } // horizontal scroll to fit min..max range

    Region &scrollY(Flt delta, Bool immediate = false) {
        slidebar[1].scroll(delta, immediate);
        return T;
    } // vertical scroll by delta
    Region &scrollToY(Flt pos, Bool immediate = false) {
        slidebar[1].scrollTo(pos, immediate);
        return T;
    } // vertical scroll to pos
    Region &scrollFitY(Flt min, Flt max, Bool immediate = false) {
        slidebar[1].scrollFit(min, max, immediate);
        return T;
    } // vertical scroll to fit min..max range
    Region &scrollEndY(Bool immediate = false) {
        slidebar[1].scrollEnd(immediate);
        return T;
    } // vertical scroll to end

    Region &scrollLeft() {
        slidebar[0].scrollLeftUp();
        return T;
    }
    Region &scrollRight() {
        slidebar[0].scrollRightDown();
        return T;
    }
    Region &scrollUp() {
        slidebar[1].scrollLeftUp();
        return T;
    }
    Region &scrollDown() {
        slidebar[1].scrollRightDown();
        return T;
    }
    ...
    Region &alwaysHideHorizontalSlideBar(Bool hide);
    Bool alwaysHideHorizontalSlideBar() C; // set/get if horizontal SlideBar should be always hidden
    ...
    // set / get
    virtual Region &rect(C Rect &rect) override;
    C Rect &rect() C { return super::rect(); }    // set/get rectangle
    virtual Region &move(C Vec2 &delta) override; // move by delta
    Region &slidebarSize(Flt size);
    Flt slidebarSize() C { return _slidebar_size; }             // set/get slidebar size, default=0.05
    ...
    Region &virtualSize(C Vec2 *size);
    Vec2 virtualSize() C { return Vec2(virtualWidth(), virtualHeight()); } // set/get virtual  size, pass null to use 'childrenSize'
    Vec2 childrenSize() C;                                                 //     get virtual  size needed to cover all children
    Region &skin(C GuiSkinPtr &skin, Bool sub_objects = true);
    C GuiSkinPtr &skin() C { return _skin; }                      // set/get skin override, default=null (if set to null then current value of 'Gui.skin' is used), 'sub_objects'=if additionally change the skin of slidebars and view button
