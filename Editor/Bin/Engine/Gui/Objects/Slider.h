...
const_mem_addr struct Slider : GuiObj // Gui Slider !! must be stored in constant memory address !!
{
    ...
    Slider &create(C Rect &rect) {
        create().rect(rect);
        return T;
    } // create and set initial values
    Slider &create(C Rect &rect, Flt value) {
        create().set(value).rect(rect);
        return T;
    }                              // create and set initial values
    ...
    T1(TYPE)
    Slider &func(void (*func)(TYPE *user), TYPE *user, Bool immediate = true) { return T.func((void (*)(Ptr))func, user, immediate); } // set function called when value has changed, with 'user' as its parameter
    T1(TYPE)
    Slider &func(void (*func)(TYPE &user), TYPE &user, Bool immediate = true) { return T.func((void (*)(Ptr))func, &user, immediate); } // set function called when value has changed, with 'user' as its parameter
    ...
    Slider &funcImmediate(Bool immediate) {
        _func_immediate = immediate;
        return T;
    } // set immediate parameter for function called when value has changed

    Slider &focusable(Bool on);
    Bool focusable() C { return _focusable; } // set/get if can catch keyboard focus, default=false

    // operations
    virtual Slider &rect(C Rect &rect) override;
