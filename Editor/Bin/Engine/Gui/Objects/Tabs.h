...
struct Tab : Button // single Tab
{
    ...
    Tab &text(C Str &text);
    C Str &text() C { return super::text; } // set/get Tab text
...
const_mem_addr struct Tabs : GuiObj // Gui Tabs !! must be stored in constant memory address !!
{
    ...
    Tabs &set(Int i, SET_MODE mode = SET_DEFAULT);
    Int operator()() C { return _sel; }               // set/get active    tab
    Tabs &toggle(Int i, SET_MODE mode = SET_DEFAULT); // toggle  i-th      tab
    Tabs &valid(Bool on);
    Bool valid() C { return _valid; } // set/get if one    tab must be selected, if false then no tab can be selected as well, default=false
    Tabs &layout(TABS_LAYOUT layout);
    TABS_LAYOUT layout() C { return _layout; } // set/get tabs layout mode, default=TABS_AUTO
    virtual Tabs &desc(C Str &desc) override;
    C Str &desc() C { return super::desc(); } // set/get tabs description
    Flt space() C { return _space; }          //     get tabs spacing
    Bool autoSize() C { return _auto_size; }  //     get tabs auto size
    Tabs &skin(C GuiSkinPtr &skin);
    C GuiSkinPtr &skin() C { return _skin; }                      // set/get skin override, default=null (if set to null then current value of 'Gui.skin' is used), changing this value will automatically change the skin of each tab
    ...
    virtual Tabs &rect(C Rect &rect) override;
