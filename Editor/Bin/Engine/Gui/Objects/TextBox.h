...
const_mem_addr struct TextBox : GuiObj // Gui TextBox !! must be stored in constant memory address !!
{
    ...
    TextBox &create(C Rect &rect, C Str &text = S) {
    ...
    // get / set
    Int maxLength() C { return _max_length; }
    TextBox &maxLength(Int max_length); // get/set maximum allowed text length (-1=no limit), default=-1
    Int cursor() C { return _edit.cur; }
    TextBox &cursor(Int position, Bool margin = true); // get/set cursor position
    Bool wordWrap() C { return _word_wrap; }
    TextBox &wordWrap(Bool wrap); // get/set word wrapping, default=true
    Bool autoHeight() C { return _auto_height; }
    TextBox &autoHeight(Bool auto_height); // get/set auto height adjustment to fit full text, default=false
    C Str &operator()() C { return _text; }
    TextBox &set(C Str &text, SET_MODE mode = SET_DEFAULT); // get/set text
    TextBox &clear(SET_MODE mode = SET_DEFAULT);            // clear   text
    Flt slidebarSize() C { return _slidebar_size; }
    TextBox &slidebarSize(Flt size); // set/get slidebar size, default=0.05
    C GuiSkinPtr &skin() C { return _skin; }
    TextBox &skin(C GuiSkinPtr &skin, Bool sub_objects = true);   // get/set skin override, default=null (if set to null then current value of 'Gui.skin' is used), 'sub_objects'=if additionally change the skin of slidebars and view button
    ...
    virtual TextBox &rect(C Rect &rect) override;
    ...
    TextBox &scrollX(Flt delta, Bool immediate = false) {
        slidebar[0].scroll(delta, immediate);
        return T;
    } // horizontal scroll by delta
    TextBox &scrollToX(Flt pos, Bool immediate = false) {
        slidebar[0].scrollTo(pos, immediate);
        return T;
    } // horizontal scroll to pos
    TextBox &scrollFitX(Flt min, Flt max, Bool immediate = false) {
        slidebar[0].scrollFit(min, max, immediate);
        return T;
    } // horizontal scroll to fit min..max range

    TextBox &scrollY(Flt delta, Bool immediate = false) {
        slidebar[1].scroll(delta, immediate);
        return T;
    } // vertical scroll by delta
    TextBox &scrollToY(Flt pos, Bool immediate = false) {
        slidebar[1].scrollTo(pos, immediate);
        return T;
    } // vertical scroll to pos
    TextBox &scrollFitY(Flt min, Flt max, Bool immediate = false) {
        slidebar[1].scrollFit(min, max, immediate);
        return T;
    } // vertical scroll to fit min..max range
