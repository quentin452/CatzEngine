...
const_mem_addr struct TextLine : GuiObj // Gui TextLine !! must be stored in constant memory address !!
{
    ...
    // get / set
    Bool password() C;
    TextLine &password(Bool on); // get/set password mode, in password mode text characters will be displayed as '*' (the character can be changed using 'Gui.passwordChar' method), password mode additionally prevents copying text to the clipboard
    Bool number() C;
    TextLine &number(Bool on); // get/set number   mode
    Bool email() C;
    TextLine &email(Bool on); // get/set email    mode
    Bool url() C;
    TextLine &url(Bool on); // get/set url      mode
    Int maxLength() C { return _max_length; }
    TextLine &maxLength(Int max_length); // get/set maximum allowed text length (-1=no limit), default=-1
    ...
    TextLine &skin(C GuiSkinPtr &skin, Bool sub_objects = true);  // get/set skin override, default=null (if set to null then current value of 'Gui.skin' is used), 'sub_objects'=if additionally change the skin of 'reset' button
    ...
    TextLine &funcImmediate(Bool immediate) {
        _func_immediate = immediate;
        return T;
    } // set immediate parameter for function called when text has changed

    virtual TextLine &rect(C Rect &rect) override;
