...
const_mem_addr struct WindowIO : ClosableWindow // Gui Window Input Output !! must be stored in constant memory address !!
{
    ...
    T1(TYPE)
    WindowIO &create(C Str &ext, C Str &path, C Str &sub_path, void (*load)(C Str &name, TYPE *user), void (*save)(C Str &name, TYPE *user), TYPE *user) { return create(ext, path, sub_path).io(load, save, user); }
    T1(TYPE)
    WindowIO &create(C Str &ext, C Str &path, C Str &sub_path, void (*load)(C Str &name, TYPE &user), void (*save)(C Str &name, TYPE &user), TYPE &user) { return create(ext, path, sub_path).io(load, save, user); }

    // set / get
    WindowIO &ext(C Str &ext, C Str &desc = S);
    C Str &ext() C { return _ext; } // set/get supported extensions, 'ext'=list of supported extensions separated with '|' symbol (null for all kinds of extensions), 'desc'=description of supported files types (if specified then it will be used in the window title bar instead of the 1st extension)
    WindowIO &path(C Str &path, C Str &sub_path = S);
    C Str &path() C { return _path; }        // set/get io path's, 'path'=main path, 'sub_path'=sub-path
    ...
    T1(TYPE)
    WindowIO &io(void (*load)(C Str &name, TYPE *user), void (*save)(C Str &name, TYPE *user), TYPE *user) { return io((void (*)(C Str &, Ptr))load, (void (*)(C Str &, Ptr))save, user); }
    T1(TYPE)
    WindowIO &io(void (*load)(C Str &name, TYPE &user), void (*save)(C Str &name, TYPE &user), TYPE &user) { return io((void (*)(C Str &, Ptr))load, (void (*)(C Str &, Ptr))save, &user); }
    ...
    virtual WindowIO &rect(C Rect &rect) override;
...
#if EE_PRIVATE
        void set(FSTD_TYPE type, Long size, C Str &name) {
            T.type = type;
            T.size = size;
            T.name = name;
        }
