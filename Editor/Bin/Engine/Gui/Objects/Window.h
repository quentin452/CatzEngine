...
const_mem_addr struct Window : GuiObj // Gui Window !! must be stored in constant memory address !!
{
    ...
    Window &create(C Rect &rect, C Str &title = S) {
        create(title).rect(rect);
        return T;
    ...
    Window &maximize();
    Bool maximized() C; // set/get maximized state, this method will check if the Window rectangle covers the entire screen
    Window &level(Int level);
    Int level() C { return _level; } // set/get window level (windows with level 1 will always be drawn on top of level 0 windows), default=0
    Window &skin(C GuiSkinPtr &skin);
    C GuiSkinPtr &skin() C { return _skin; }                      // set/get skin override, default=null (if set to null then current value of 'Gui.skin' is used), changing this value will automatically change the skin of the window buttons
    GuiSkin *getSkin() C { return _skin ? _skin() : Gui.skin(); } //     get actual skin
    Window &barVisible(Bool visible);
    Bool barVisible() C { return _bar_visible; }  // set/get title bar visibility, default=true
    ...
    Window &alpha(Flt alpha);
    Flt alpha() C { return _alpha; }              // set/get custom  window opacity controlled by the user         (0..1)
    ...
    Window &clientRect(C Rect &rect);
    C Rect &clientRect() C { return _crect; } // set/get client rect
