...
struct StrEx : Mems<StrData> // Extended String, which can hold: Text, Images, Color/Shadow/Font information, Panel background
{
    ...
    StrEx &shadow(Int shadow) {
        T.shadow(Byte(shadow));
        return T;
    }
...
struct TextStyleParams // Text Style Params
{
    ...
    Font *font() C { return _font; }
    void font(Font *font) { T._font = font; } // get/set font, default=null (if set to null then current value of 'Gui.skin.font' is used)
    ...
    TextStyleParams(TextStyleParams *ts, Bool gui) {
        if (ts)
            T = *ts;
        else
            reset(gui);
...
{
    C FontPtr &font() C { return _font; }
    TextStyle &font(C FontPtr &font); // get/set Font
