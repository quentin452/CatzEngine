...
struct VirtualReality // Virtual Reality - Head Mounted Display (HMD)
{
    ...
    Flt pixelDensity() C { return _density; }
    VirtualReality &pixelDensity(Flt density); // get/set HMD rendering pixel density, 0..2, default=1
    C VecI2 &guiRes() C { return _gui_res; }
    VirtualReality &guiRes(Int w, Int h); // get/set HMD gui resolution, 1x1 .. Inf x Inf, default=1024x1024
    Flt guiDepth() C { return _gui_depth; }
    VirtualReality &guiDepth(Flt depth); // get/set HMD gui depth (distance from camera in meters), 0..Inf, default=1, changing 'guiDepth' does not affect visible gui size
    Flt guiSize() C { return _gui_size; }
    VirtualReality &guiSize(Flt size); // get/set HMD gui size  (size when seen at 1 m distance), 0..Inf, default=1
...
struct Input {
    ...
    void set(Bool pushed, INPUT_TYPE type, Byte button, Byte device = 0) {
        T.pushed = pushed;
        T.type = type;
        T.button = button;
        T.device = device;
