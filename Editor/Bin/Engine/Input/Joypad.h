...
#define JOYPAD_VENDOR_PRODUCT_ID (JP_DIRECT_INPUT && JP_GAMEPAD_INPUT) // needed only if using both JP_DIRECT_INPUT and JP_GAMEPAD_INPUT
    ...
    struct Elm {
...
#if EE_PRIVATE
        void setPad(C IOHIDElementCookie &cookie, Int max) {
            T.type = PAD;
            T.cookie = cookie;
            T.max = max + 1;
            T.mul = -PI2 / T.max;
        ...
        void setButton(C IOHIDElementCookie &cookie, Byte index, Int min, Int max) {
            T.type = BUTTON;
            T.cookie = cookie;
            T.index = index;
            T.avg = (min + max) / 2;
        }
        void setAxis(C IOHIDElementCookie &cookie, Byte index, Int min, Int max) {
            T.type = AXIS;
            T.cookie = cookie;
            T.index = index;
            T.mul = 2.0f / (max - min);
...
#endif
#if EE_PRIVATE
        void set(TYPE type, Byte index, GCControllerElement *element) {
            T.type = type;
            T.index = index;
            T.element = element;
        }
    ...
    struct State {
        ...
        union {
            ...
            ABI::Windows::Gaming::Input::GamepadReading gamepad;
            ASSERT(SIZE(gamepad) <= SIZE(data));
            ...
            ASSERT(SIZE(raw_game_controller) <= SIZE(data));
#endif
#if JP_X_INPUT
            XINPUT_STATE xinput;
            ASSERT(SIZE(xinput) <= SIZE(data));
#endif
#if JP_DIRECT_INPUT
            DIJOYSTATE dinput;
            ASSERT(SIZE(dinput) <= SIZE(data));
#endif
#endif
        };
#endif
...
#pragma pack() // default packing required for storing WinRT classes not as ^ pointers, without this 'gamepad' members were getting corrupt values
        Windows::Gaming::Input::GamepadReading gamepad;
        ASSERT(SIZE(gamepad) == SIZE(UInt) * 16); // use UInt for alignment
#pragma pack(pop)
#else
