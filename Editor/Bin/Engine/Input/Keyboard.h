...
struct KeyboardClass // Keyboard Input
{
    ...
    Bool exclusive() C { return _exclusive; }
    void exclusive(Bool on); // get/set keyboard exclusive mode (which disables Windows key on Windows platform), default=false
...
#if EE_PRIVATE
    Bool imm() C;
    void imm(Bool enable); // get/set if disable/enable the ability to use Windows Input Method Manager, this method is automatically called be the engine when keyboard focus is switched to text editing capable gui object
#endif
    Bool immNative() C;
    void immNative(Bool native);                             // get/set if native typing mode is currently enabled
    ...
    void swapCtrlCmd(Bool swapped);
    Bool swapCtrlCmd() C { return _swap_ctrl_cmd; } // set/get swap Ctrl with Cmd key on Mac, enable this method if you want to swap Ctrl with Cmd key input, this method is used only on Mac, default=false
#endif
...
struct KbSc // Keyboard Shortcut
{
    ...
    KbSc(Char8 c, Byte flag = 0) {
        T.index = Char8To16(c);
        T.flag = flag;
        T.mode = KBSC_CHAR;
    }
    KbSc(Char c, Byte flag = 0) {
        T.index = c;
        T.flag = flag;
        T.mode = KBSC_CHAR;
    }
    KbSc(KB_KEY key, Byte flag = 0) {
        T.index = key;
        T.flag = flag;
        T.mode = KBSC_KEY;
    }
...
#elif MAC
extern const KB_KEY ScanCodeToQwertyKey[0x80];
extern KB_KEY ScanCodeToKey[Elms(ScanCodeToQwertyKey)];
#elif LINUX
extern const KB_KEY ScanCodeToQwertyKey[0x90];
extern KB_KEY ScanCodeToKey[Elms(ScanCodeToQwertyKey)];
#endif
#endif
/******************************************************************************/
