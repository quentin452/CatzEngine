...
inline void MinMax(Byte a, Byte b, Byte &min, Byte &max) {
    if (a < b) {
        min = a;
        max = b;
    } else {
...
} // get min max value: min=Min(a, b); max=Max(a, b);
...
inline void MinMax(Flt a, Flt b, Flt &min, Flt &max) {
    if (a < b) {
        min = a;
        max = b;
    } else {
...
inline void MinMax(Dbl a, Dbl b, Dbl &min, Dbl &max) {
    if (a < b) {
        min = a;
        max = b;
    } else {
...
// get middle value
constexpr Int Mid(Int x, Int min, Int max) { return (x >= max) ? max : (x <= min) ? min
                                                                                  : x; }
constexpr Long Mid(Long x, Long min, Long max) { return (x >= max) ? max : (x <= min) ? min
                                                                                      : x; }
constexpr Flt Mid(Flt x, Int min, Int max) { return (x >= max) ? max : (x <= min) ? min
                                                                                  : x; }
constexpr Flt Mid(Flt x, Int min, Flt max) { return (x >= max) ? max : (x <= min) ? min
                                                                                  : x; }
constexpr Flt Mid(Flt x, Flt min, Int max) { return (x >= max) ? max : (x <= min) ? min
                                                                                  : x; }
constexpr Flt Mid(Flt x, Flt min, Flt max) { return (x >= max) ? max : (x <= min) ? min
                                                                                  : x; }
constexpr Dbl Mid(Dbl x, Dbl min, Dbl max) { return (x >= max) ? max : (x <= min) ? min
                                                                                  : x; }
...
constexpr Int Sign(Int x) { return (x > 0) ? 1 : (x < 0) ? -1
                                                         : 0; } // sign
constexpr Int Sign(Long x) { return (x > 0) ? 1 : (x < 0) ? -1
                                                          : 0; } // sign
constexpr Int Sign(Flt x) { return (x > 0) ? 1 : (x < 0) ? -1
                                                         : 0; } // sign
constexpr Int Sign(Dbl x) { return (x > 0) ? 1 : (x < 0) ? -1
                                                         : 0; } // sign
...
inline Int SignEps(Flt x, Flt eps = EPS) { return (x > eps) ? 1 : (x < -eps) ? -1
                                                                             : 0; } // sign using epsilon
inline Int SignEps(Dbl x, Dbl eps = EPSD) { return (x > eps) ? 1 : (x < -eps) ? -1
                                                                              : 0; } // sign using epsilon
...
Vec2 ScaleFactorR(C Vec2 &vec);
VecD2 ScaleFactor(C VecD2 &vec);
VecD2 ScaleFactorR(C VecD2 &vec);
Vec ScaleFactor(C Vec &vec);
Vec ScaleFactorR(C Vec &vec);
VecD ScaleFactor(C VecD &vec);
VecD ScaleFactorR(C VecD &vec);
Vec4 ScaleFactor(C Vec4 &vec);
Vec4 ScaleFactorR(C Vec4 &vec);
VecD4 ScaleFactor(C VecD4 &vec);
VecD4 ScaleFactorR(C VecD4 &vec);
