...
struct Box // Box Shape
{
    ...
    Box &set(C Vec &min, C Vec &max) {
        T.min = min;
        T.max = max;
        return T;
    }
    Box &set(Flt min_x, Flt min_y, Flt min_z, Flt max_x, Flt max_y, Flt max_z) {
        min.set(min_x, min_y, min_z);
        max.set(max_x, max_y, max_z);
        return T;
    }
    Box &set(Flt r, C Vec &pos = VecZero) {
        min.set(pos.x - r, pos.y - r, pos.z - r);
        max.set(pos.x + r, pos.y + r, pos.z + r);
        return T;
    }
    Box &set(Flt w, Flt h, Flt d, C Vec &pos = VecZero) {
        min.set(pos.x - w * 0.5f, pos.y - h * 0.5f, pos.z - d * 0.5f);
        max.set(pos.x + w * 0.5f, pos.y + h * 0.5f, pos.z + d * 0.5f);
        return T;
    }
    Box &setR(Flt w, Flt h, Flt d, C Vec &pos = VecZero) {
        min.set(pos.x - w, pos.y - h * 0.5f, pos.z - d * 0.5f);
        max.set(pos.x, pos.y + h * 0.5f, pos.z + d * 0.5f);
        return T;
    } // set right
    Box &setL(Flt w, Flt h, Flt d, C Vec &pos = VecZero) {
        min.set(pos.x, pos.y - h * 0.5f, pos.z - d * 0.5f);
        max.set(pos.x + w, pos.y + h * 0.5f, pos.z + d * 0.5f);
        return T;
    } // set left
    Box &setU(Flt w, Flt h, Flt d, C Vec &pos = VecZero) {
        min.set(pos.x - w * 0.5f, pos.y - h, pos.z - d * 0.5f);
        max.set(pos.x + w * 0.5f, pos.y, pos.z + d * 0.5f);
        return T;
    } // set up
    Box &setD(Flt w, Flt h, Flt d, C Vec &pos = VecZero) {
        min.set(pos.x - w * 0.5f, pos.y, pos.z - d * 0.5f);
        max.set(pos.x + w * 0.5f, pos.y + h, pos.z + d * 0.5f);
        return T;
    } // set down
    Box &setF(Flt w, Flt h, Flt d, C Vec &pos = VecZero) {
        min.set(pos.x - w * 0.5f, pos.y - h * 0.5f, pos.z - d);
        max.set(pos.x + w * 0.5f, pos.y + h * 0.5f, pos.z);
        return T;
    } // set forward
    Box &setB(Flt w, Flt h, Flt d, C Vec &pos = VecZero) {
        min.set(pos.x - w * 0.5f, pos.y - h * 0.5f, pos.z);
        max.set(pos.x + w * 0.5f, pos.y + h * 0.5f, pos.z + d);
        return T;
    } // set back

    Box &operator+=(C Vec &v) {
        min += v;
        max += v;
        return T;
    }
    ...
    Box &setX(Flt min, Flt max) {
        T.min.x = min;
        T.max.x = max;
        return T;
    }
    Box &setY(Flt min, Flt max) {
        T.min.y = min;
        T.max.y = max;
        return T;
    }
    Box &setZ(Flt min, Flt max) {
        T.min.z = min;
        T.max.z = max;
        return T;
    }
...
struct Box_R : Box {
    Box_R(Flt w, Flt h, Flt d, C Vec &pos = VecZero) { setR(w, h, d, pos); }
}; // right
struct Box_L : Box {
    Box_L(Flt w, Flt h, Flt d, C Vec &pos = VecZero) { setL(w, h, d, pos); }
}; // left
struct Box_U : Box {
    Box_U(Flt w, Flt h, Flt d, C Vec &pos = VecZero) { setU(w, h, d, pos); }
}; // up
struct Box_D : Box {
    Box_D(Flt w, Flt h, Flt d, C Vec &pos = VecZero) { setD(w, h, d, pos); }
}; // down
struct Box_F : Box {
    Box_F(Flt w, Flt h, Flt d, C Vec &pos = VecZero) { setF(w, h, d, pos); }
}; // forward
struct Box_B : Box {
    Box_B(Flt w, Flt h, Flt d, C Vec &pos = VecZero) { setB(w, h, d, pos); }
}; // back
/******************************************************************************/
struct BoxD // Box Shape (double precision)
{
    ...
    BoxD &set(C VecD &min, C VecD &max) {
        T.min = min;
        T.max = max;
        return T;
    }
    BoxD &set(Dbl min_x, Dbl min_y, Dbl min_z, Dbl max_x, Dbl max_y, Dbl max_z) {
        min.set(min_x, min_y, min_z);
        max.set(max_x, max_y, max_z);
        return T;
    }
    BoxD &set(Dbl r, C VecD &pos = VecDZero) {
        min.set(pos.x - r, pos.y - r, pos.z - r);
        max.set(pos.x + r, pos.y + r, pos.z + r);
        return T;
    }
    BoxD &set(Dbl w, Dbl h, Dbl d, C VecD &pos = VecDZero) {
        min.set(pos.x - w * 0.5, pos.y - h * 0.5, pos.z - d * 0.5);
        max.set(pos.x + w * 0.5, pos.y + h * 0.5, pos.z + d * 0.5);
        return T;
    }
...
struct BoxI // Box Shape (integer)
{
    ...
    BoxI &set(C VecI &min, C VecI &max) {
        T.min = min;
        T.max = max;
        return T;
    }
    BoxI &set(Int min_x, Int min_y, Int min_z, Int max_x, Int max_y, Int max_z) {
        min.set(min_x, min_y, min_z);
        max.set(max_x, max_y, max_z);
        return T;
    }
    ...
    BoxI &setX(Int min, Int max) {
        T.min.x = min;
        T.max.x = max;
        return T;
    }
    BoxI &setY(Int min, Int max) {
        T.min.y = min;
        T.max.y = max;
        return T;
    }
    BoxI &setZ(Int min, Int max) {
        T.min.z = min;
        T.max.z = max;
        return T;
    }
...
struct OBox // Oriented Box (Box with matrix transformation, can be rotated)
{
    ...
    OBox() {}
    OBox(C Box &box, C Matrix &matrix = MatrixIdentity) {
        T.box = box;
        T.matrix = matrix;
    } // 'matrix' must be normalized
...
struct Extent // similar to 'Box' however it operates on center position and extents in each axis
{
    ...
    Extent &set(C Vec &ext, C Vec &pos = VecZero) {
        T.ext = ext;
        T.pos = pos;
        return T;
    }

    Extent &operator+=(C Vec &v) {
        pos += v;
        return T;
    }
    Extent &operator-=(C Vec &v) {
    ...
    // operations
    Extent &extendX(Flt e) {
        ext.x += e;
        return T;
    } // extend X by 'e'
    Extent &extendY(Flt e) {
        ext.y += e;
        return T;
    } // extend Y by 'e'
    Extent &extendZ(Flt e) {
        ext.z += e;
        return T;
    } // extend Z by 'e'
    Extent &extend(Flt e) {
