...
struct Rect // Rectangle Shape
{
    ...
    Rect &zero() {
        ...
        max.zero();
        return T;
    } // zero rectangle coordinates
    Rect &set(C Vec2 &min, C Vec2 &max) {
        T.min = min;
        T.max = max;
        return T;
    } // set  rectangle from minimum maximum coordinates
    Rect &set(Flt min_x, Flt min_y, Flt max_x, Flt max_y) {
        min.set(min_x, min_y);
        max.set(max_x, max_y);
        return T;
    } // set  rectangle from minimum maximum coordinates
    ...
    Rect &setX(Flt min, Flt max) {
        T.min.x = min;
        T.max.x = max;
        return T;
    }
    Rect &setY(Flt min, Flt max) {
        T.min.y = min;
        T.max.y = max;
        return T;
    }

    Rect &setRU(C Vec2 &ru, C Vec2 &size) {
        min.set(ru.x - size.x, ru.y - size.y);
        max.set(ru.x, ru.y);
        return T;
    } // set rectangle by specifying its "Right Up"   position
    Rect &setLU(C Vec2 &lu, C Vec2 &size) {
        min.set(lu.x, lu.y - size.y);
        max.set(lu.x + size.x, lu.y);
        return T;
    } // set rectangle by specifying its "Left  Up"   position
    Rect &setRD(C Vec2 &rd, C Vec2 &size) {
        min.set(rd.x - size.x, rd.y);
        max.set(rd.x, rd.y + size.y);
        return T;
    } // set rectangle by specifying its "Right Down" position
    Rect &setLD(C Vec2 &ld, C Vec2 &size) {
        min.set(ld.x, ld.y);
        max.set(ld.x + size.x, ld.y + size.y);
        return T;
    } // set rectangle by specifying its "Left  Down" position
    Rect &setR(C Vec2 &r, C Vec2 &size) {
        min.set(r.x - size.x, r.y - size.y * 0.5f);
        max.set(r.x, r.y + size.y * 0.5f);
        return T;
    } // set rectangle by specifying its "Right"      position
    Rect &setL(C Vec2 &l, C Vec2 &size) {
        min.set(l.x, l.y - size.y * 0.5f);
        max.set(l.x + size.x, l.y + size.y * 0.5f);
        return T;
    } // set rectangle by specifying its "Left"       position
    Rect &setU(C Vec2 &u, C Vec2 &size) {
        min.set(u.x - size.x * 0.5f, u.y - size.y);
        max.set(u.x + size.x * 0.5f, u.y);
        return T;
    } // set rectangle by specifying its "Up"         position
    Rect &setD(C Vec2 &d, C Vec2 &size) {
        min.set(d.x - size.x * 0.5f, d.y);
        max.set(d.x + size.x * 0.5f, d.y + size.y);
        return T;
    } // set rectangle by specifying its "Down"       position
    Rect &setC(C Vec2 &c, C Vec2 &size) {
        min.set(c.x - size.x * 0.5f, c.y - size.y * 0.5f);
        max.set(c.x + size.x * 0.5f, c.y + size.y * 0.5f);
        return T;
    } // set rectangle by specifying its "Center"     position

    Rect &setRU(C Vec2 &ru, Flt w, Flt h) {
        min.set(ru.x - w, ru.y - h);
        max.set(ru.x, ru.y);
        return T;
    } // set rectangle by specifying its "Right Up"   position
    Rect &setLU(C Vec2 &lu, Flt w, Flt h) {
        min.set(lu.x, lu.y - h);
        max.set(lu.x + w, lu.y);
        return T;
    } // set rectangle by specifying its "Left  Up"   position
    Rect &setRD(C Vec2 &rd, Flt w, Flt h) {
        min.set(rd.x - w, rd.y);
        max.set(rd.x, rd.y + h);
        return T;
    } // set rectangle by specifying its "Right Down" position
    Rect &setLD(C Vec2 &ld, Flt w, Flt h) {
        min.set(ld.x, ld.y);
        max.set(ld.x + w, ld.y + h);
        return T;
    } // set rectangle by specifying its "Left  Down" position
    Rect &setR(C Vec2 &r, Flt w, Flt h) {
        min.set(r.x - w, r.y - h * 0.5f);
        max.set(r.x, r.y + h * 0.5f);
        return T;
    } // set rectangle by specifying its "Right"      position
    Rect &setL(C Vec2 &l, Flt w, Flt h) {
        min.set(l.x, l.y - h * 0.5f);
        max.set(l.x + w, l.y + h * 0.5f);
        return T;
    } // set rectangle by specifying its "Left"       position
    Rect &setU(C Vec2 &u, Flt w, Flt h) {
        min.set(u.x - w * 0.5f, u.y - h);
        max.set(u.x + w * 0.5f, u.y);
        return T;
    } // set rectangle by specifying its "Up"         position
    Rect &setD(C Vec2 &d, Flt w, Flt h) {
        min.set(d.x - w * 0.5f, d.y);
        max.set(d.x + w * 0.5f, d.y + h);
        return T;
    } // set rectangle by specifying its "Down"       position
    Rect &setC(C Vec2 &c, Flt w, Flt h) {
        min.set(c.x - w * 0.5f, c.y - h * 0.5f);
        max.set(c.x + w * 0.5f, c.y + h * 0.5f);
        return T;
    } // set rectangle by specifying its "Center"     position
    Rect &setC(C Vec2 &c, Flt r) {
        min.set(c.x - r, c.y - r);
        max.set(c.x + r, c.y + r);
        return T;
    } // set rectangle by specifying its "Center"     position

    Rect &setRU(Flt x, Flt y, Flt w, Flt h) {
        min.set(x - w, y - h);
        max.set(x, y);
        return T;
    } // set rectangle by specifying its "Right Up"   position
    Rect &setLU(Flt x, Flt y, Flt w, Flt h) {
        min.set(x, y - h);
        max.set(x + w, y);
        return T;
    } // set rectangle by specifying its "Left  Up"   position
    Rect &setRD(Flt x, Flt y, Flt w, Flt h) {
        min.set(x - w, y);
        max.set(x, y + h);
        return T;
    } // set rectangle by specifying its "Right Down" position
    Rect &setLD(Flt x, Flt y, Flt w, Flt h) {
        min.set(x, y);
        max.set(x + w, y + h);
        return T;
    } // set rectangle by specifying its "Left  Down" position
    Rect &setR(Flt x, Flt y, Flt w, Flt h) {
        min.set(x - w, y - h * 0.5f);
        max.set(x, y + h * 0.5f);
        return T;
    } // set rectangle by specifying its "Right"      position
    Rect &setL(Flt x, Flt y, Flt w, Flt h) {
        min.set(x, y - h * 0.5f);
        max.set(x + w, y + h * 0.5f);
        return T;
    } // set rectangle by specifying its "Left"       position
    Rect &setU(Flt x, Flt y, Flt w, Flt h) {
        min.set(x - w * 0.5f, y - h);
        max.set(x + w * 0.5f, y);
        return T;
    } // set rectangle by specifying its "Up"         position
    Rect &setD(Flt x, Flt y, Flt w, Flt h) {
        min.set(x - w * 0.5f, y);
        max.set(x + w * 0.5f, y + h);
        return T;
    } // set rectangle by specifying its "Down"       position
    Rect &setC(Flt x, Flt y, Flt w, Flt h) {
        min.set(x - w * 0.5f, y - h * 0.5f);
        max.set(x + w * 0.5f, y + h * 0.5f);
        return T;
    } // set rectangle by specifying its "Center"     position
    Rect &setC(Flt x, Flt y, Flt r) {
        min.set(x - r, y - r);
        max.set(x + r, y + r);
        return T;
    } // set rectangle by specifying its "Center"     position

    Rect &operator+=(C Vec2 &v) {
        min += v;
        max += v;
        return T;
    }
...
struct Rect_RU : Rect {
    Rect_RU(C Vec2 &ru, C Vec2 &size) { setRU(ru, size); }
    Rect_RU(C Vec2 &ru, Flt w, Flt h) { setRU(ru, w, h); }
    Rect_RU(Flt x, Flt y, Flt w, Flt h) { setRU(x, y, w, h); }
    Rect_RU(C Rect &rect) { SCAST(Rect, T) = rect; }
}; // create rectangle by specifying its "Right Up  " position
struct Rect_LU : Rect {
    Rect_LU(C Vec2 &lu, C Vec2 &size) { setLU(lu, size); }
    Rect_LU(C Vec2 &lu, Flt w, Flt h) { setLU(lu, w, h); }
    Rect_LU(Flt x, Flt y, Flt w, Flt h) { setLU(x, y, w, h); }
    Rect_LU(C Rect &rect) { SCAST(Rect, T) = rect; }
}; // create rectangle by specifying its "Left  Up  " position
struct Rect_RD : Rect {
    Rect_RD(C Vec2 &rd, C Vec2 &size) { setRD(rd, size); }
    Rect_RD(C Vec2 &rd, Flt w, Flt h) { setRD(rd, w, h); }
    Rect_RD(Flt x, Flt y, Flt w, Flt h) { setRD(x, y, w, h); }
    Rect_RD(C Rect &rect) { SCAST(Rect, T) = rect; }
}; // create rectangle by specifying its "Right Down" position
struct Rect_LD : Rect {
    Rect_LD(C Vec2 &ld, C Vec2 &size) { setLD(ld, size); }
    Rect_LD(C Vec2 &ld, Flt w, Flt h) { setLD(ld, w, h); }
    Rect_LD(Flt x, Flt y, Flt w, Flt h) { setLD(x, y, w, h); }
    Rect_LD(C Rect &rect) { SCAST(Rect, T) = rect; }
}; // create rectangle by specifying its "Left  Down" position
struct Rect_R : Rect {
    Rect_R(C Vec2 &r, C Vec2 &size) { setR(r, size); }
    Rect_R(C Vec2 &r, Flt w, Flt h) { setR(r, w, h); }
    Rect_R(Flt x, Flt y, Flt w, Flt h) { setR(x, y, w, h); }
    Rect_R(C Rect &rect) { SCAST(Rect, T) = rect; }
}; // create rectangle by specifying its "Right     " position
struct Rect_L : Rect {
    Rect_L(C Vec2 &l, C Vec2 &size) { setL(l, size); }
    Rect_L(C Vec2 &l, Flt w, Flt h) { setL(l, w, h); }
    Rect_L(Flt x, Flt y, Flt w, Flt h) { setL(x, y, w, h); }
    Rect_L(C Rect &rect) { SCAST(Rect, T) = rect; }
}; // create rectangle by specifying its "Left      " position
struct Rect_U : Rect {
    Rect_U(C Vec2 &u, C Vec2 &size) { setU(u, size); }
    Rect_U(C Vec2 &u, Flt w, Flt h) { setU(u, w, h); }
    Rect_U(Flt x, Flt y, Flt w, Flt h) { setU(x, y, w, h); }
    Rect_U(C Rect &rect) { SCAST(Rect, T) = rect; }
}; // create rectangle by specifying its "Up        " position
struct Rect_D : Rect {
    Rect_D(C Vec2 &d, C Vec2 &size) { setD(d, size); }
    Rect_D(C Vec2 &d, Flt w, Flt h) { setD(d, w, h); }
    Rect_D(Flt x, Flt y, Flt w, Flt h) { setD(x, y, w, h); }
    Rect_D(C Rect &rect) { SCAST(Rect, T) = rect; }
}; // create rectangle by specifying its "Down      " position
struct Rect_C : Rect {
    Rect_C(C Vec2 &c, C Vec2 &size) { setC(c, size); }
    Rect_C(C Vec2 &c, Flt w, Flt h) { setC(c, w, h); }
    Rect_C(Flt x, Flt y, Flt w, Flt h) { setC(x, y, w, h); }
    Rect_C(C Rect &rect) { SCAST(Rect, T) = rect; }
    Rect_C(C Vec2 &c, Flt r) { setC(c, r); }
    Rect_C(Flt x, Flt y, Flt r) { setC(x, y, r); }
}; // create rectangle by specifying its "Center" position
/******************************************************************************/
struct RectD // Rectangle Shape (double precision)
{
    ...
    RectD &set(C VecD2 &min, C VecD2 &max) {
        T.min = min;
        T.max = max;
        return T;
    } // set  rectangle from minimum maximum coordinates
    RectD &set(Dbl min_x, Dbl min_y, Dbl max_x, Dbl max_y) {
        min.set(min_x, min_y);
        max.set(max_x, max_y);
        return T;
    } // set  rectangle from minimum maximum coordinates
    ...
    RectD &setX(Dbl min, Dbl max) {
        T.min.x = min;
        T.max.x = max;
        return T;
    }
    RectD &setY(Dbl min, Dbl max) {
        T.min.y = min;
        T.max.y = max;
        return T;
    }

    RectD &operator+=(C VecD2 &v) {
        min += v;
        max += v;
        return T;
    }
...
struct RectI // Rectangle Shape (integer)
{
    ...
    RectI &set(C VecI2 &min, C VecI2 &max) {
        T.min = min;
        T.max = max;
        return T;
    } // set  rectangle from minimum maximum coordinates
    RectI &set(Int min_x, Int min_y, Int max_x, Int max_y) {
        min.set(min_x, min_y);
        max.set(max_x, max_y);
        return T;
    } // set  rectangle from minimum maximum coordinates

    RectI &setRU(C VecI2 &ru, C VecI2 &size) {
        min.set(ru.x - size.x, ru.y - size.y);
        max.set(ru.x, ru.y);
        return T;
    } // set rectangle by specifying its "Right Up"   position
    RectI &setLU(C VecI2 &lu, C VecI2 &size) {
        min.set(lu.x, lu.y - size.y);
        max.set(lu.x + size.x, lu.y);
        return T;
    } // set rectangle by specifying its "Left  Up"   position
    RectI &setRD(C VecI2 &rd, C VecI2 &size) {
        min.set(rd.x - size.x, rd.y);
        max.set(rd.x, rd.y + size.y);
        return T;
    } // set rectangle by specifying its "Right Down" position
    RectI &setLD(C VecI2 &ld, C VecI2 &size) {
        min.set(ld.x, ld.y);
        max.set(ld.x + size.x, ld.y + size.y);
        return T;
    } // set rectangle by specifying its "Left  Down" position

    RectI &setRU(C VecI2 &ru, Int w, Int h) {
        min.set(ru.x - w, ru.y - h);
        max.set(ru.x, ru.y);
        return T;
    } // set rectangle by specifying its "Right Up"   position
    RectI &setLU(C VecI2 &lu, Int w, Int h) {
        min.set(lu.x, lu.y - h);
        max.set(lu.x + w, lu.y);
        return T;
    } // set rectangle by specifying its "Left  Up"   position
    RectI &setRD(C VecI2 &rd, Int w, Int h) {
        min.set(rd.x - w, rd.y);
        max.set(rd.x, rd.y + h);
        return T;
    } // set rectangle by specifying its "Right Down" position
    RectI &setLD(C VecI2 &ld, Int w, Int h) {
        min.set(ld.x, ld.y);
        max.set(ld.x + w, ld.y + h);
        return T;
    } // set rectangle by specifying its "Left  Down" position

    RectI &setRU(Int x, Int y, Int w, Int h) {
        min.set(x - w, y - h);
        max.set(x, y);
        return T;
    } // set rectangle by specifying its "Right Up"   position
    RectI &setLU(Int x, Int y, Int w, Int h) {
        min.set(x, y - h);
        max.set(x + w, y);
        return T;
    } // set rectangle by specifying its "Left  Up"   position
    RectI &setRD(Int x, Int y, Int w, Int h) {
        min.set(x - w, y);
        max.set(x, y + h);
        return T;
    } // set rectangle by specifying its "Right Down" position
    RectI &setLD(Int x, Int y, Int w, Int h) {
        min.set(x, y);
        max.set(x + w, y + h);
        return T;
    } // set rectangle by specifying its "Left  Down" position

    RectI &operator+=(C VecI2 &v) {
        min += v;
        max += v;
        return T;
    }
    ...
    RectI &setX(Int min, Int max) {
        T.min.x = min;
        T.max.x = max;
        return T;
    }
    RectI &setY(Int min, Int max) {
        T.min.y = min;
        T.max.y = max;
        return T;
    }
