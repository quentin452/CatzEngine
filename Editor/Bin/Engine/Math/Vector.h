...
T2(TA, TB)
inline TA &MIN(TA &x, TB y) {
    if (y < x)
        x = y;
    return x;
} // equal to: x=Min(x, y)
T2(TA, TB)
inline TA &MAX(TA &x, TB y) {
    if (y > x)
        x = y;
    return x;
} // equal to: x=Max(x, y)
...
inline Flt Xor(Flt a, UInt b) {
    ((U32 &)a) ^= b;
    return a;
} // used for fast changing of the 'a' sign, 'b' should be either 0 or SIGN_BIT
inline Dbl Xor(Dbl a, UInt b) {
    ((U32 *)&a)[1] ^= b;
    return a;
} // used for fast changing of the 'a' sign, 'b' should be either 0 or SIGN_BIT
...
#endif
inline Int Compare(C Half &a, C Half &b) {
    if (a < b)
        return -1;
    if (a > b)
        return +1;
...
struct VecD2 // Vector 2D (double precision)
{
    ...
    VecD2 &set(Dbl x, Dbl y) {
        T.x = x;
        T.y = y;
        return T;
    }

    VecD2 &operator+=(Dbl r) {
        x += r;
        y += r;
        return T;
    }
    ...
    friend VecD2 operator!(C VecD2 &v) {
        VecD2 ret = v;
        ret.normalize();
        return ret;
    } // return normalized vector
...
struct Vec // Vector 3D
{
    ...
    Vec &set(Flt x, Flt y, Flt z) {
        T.x = x;
        T.y = y;
        T.z = z;
        return T;
    }
    Vec &set(C Vec2 &xy, Flt z) {
        T.xy = xy;
        T.z = z;
        return T;
    }
...
struct VecD // Vector 3D (double precision)
{
    ...
    VecD &set(Dbl x, Dbl y, Dbl z) {
        T.x = x;
        T.y = y;
        T.z = z;
        return T;
    }
    VecD &set(C VecD2 &xy, Dbl z) {
        T.xy = xy;
        T.z = z;
        return T;
    }
...
struct Vec4 // Vector 4D
{
    ...
    Vec4 &set(Flt x, Flt y, Flt z, Flt w) {
        T.x = x;
        T.y = y;
        T.z = z;
        T.w = w;
    ...
    Vec4 &set(C Vec2 &xy, Flt z, Flt w) {
        T.xy = xy;
        T.z = z;
        T.w = w;
        return T;
    }
    Vec4 &set(C Vec &xyz, Flt w) {
        T.xyz = xyz;
        T.w = w;
        return T;
    }
    Vec4 &set(C Vec2 &xy, C Vec2 &zw) {
        T.xy = xy;
        T.zw = zw;
        return T;
    }
...
extern Vec4 const Vec4Zero; // Vec4(0, 0, 0, 0)
/******************************************************************************/
struct VecD4 // Vector 4D (double precision)
{
    ...
    VecD4 &set(Dbl x, Dbl y, Dbl z, Dbl w) {
        T.x = x;
        T.y = y;
        T.z = z;
        T.w = w;
    ...
    VecD4 &set(C VecD2 &xy, Dbl z, Dbl w) {
        T.xy = xy;
        T.z = z;
        T.w = w;
        return T;
    }
    VecD4 &set(C VecD &xyz, Dbl w) {
        T.xyz = xyz;
        T.w = w;
        return T;
    }
    VecD4 &set(C VecD2 &xy, C VecD2 &zw) {
        T.xy = xy;
        T.zw = zw;
        return T;
    }

    VecD4 &operator+=(Dbl r) {
        x += r;
        y += r;
        z += r;
        w += r;
    ...
    friend VecD4 operator!(C VecD4 &v) {
        VecD4 ret = v;
        ret.normalize();
        return ret;
    } // return normalized vector
...
extern VecD4 const VecD4Zero; // VecD4(0, 0, 0, 0)
/******************************************************************************/
...
struct VecSB2 // Vector 2D (SByte)
{
    ...
    VecSB2 &set(SByte x, SByte y) {
        T.x = x;
        T.y = y;
        return T;
    }

    VecSB2 &operator+=(C VecSB2 &v) {
        x += v.x;
        y += v.y;
        return T;
    }
...
struct VecSB // Vector 3D (SByte)
{
    ...
    VecSB &set(SByte x, SByte y, SByte z) {
        T.x = x;
        T.y = y;
        T.z = z;
        return T;
...
struct VecB4 // Vector 4D (Byte)
{
    ...
    VecB4 &set(Byte x, Byte y, Byte z, Byte w) {
        T.x = x;
        T.y = y;
        T.z = z;
        T.w = w;
...
struct VecSB4 // Vector 4D (SByte)
{
    ...
    VecSB4 &set(SByte x, SByte y, SByte z, SByte w) {
        T.x = x;
        T.y = y;
        T.z = z;
        T.w = w;
...
#if EE_PRIVATE
    VecUS2 &remap(C Int *map) {
        if (map) {
            x = map[x];
            y = map[y];
        }
        return T;
    } // remap if map is provided
    VecUS2 &remapAll(C CMemPtr<Int> &map) {
        x = (InRange(x, map) ? map[x] : -1);
        y = (InRange(y, map) ? map[y] : -1);
        return T;
    } // remap all components
    VecUS2 &remapFit(C CMemPtr<Int> &map) {
        if (InRange(x, map))
            x = map[x];
        if (InRange(y, map))
            y = map[y];
        return T;
    } // remap     components which are in range of the remap array
#endif

    VecUS2 &operator+=(Int i) {
        x += i;
        y += i;
        return T;
    }
...
struct VecUS // Vector 3D (Unsigned Short)
{
    ...
    VecUS &set(UShort x, UShort y, UShort z) {
        T.x = x;
        T.y = y;
        T.z = z;
        return T;
    }
    VecUS &set(C VecUS2 &xy, UShort z) {
        T.xy = xy;
        T.z = z;
        return T;
    }
    ...
    VecUS &remapAll(C CMemPtr<Int> &map) {
        x = (InRange(x, map) ? map[x] : -1);
        y = (InRange(y, map) ? map[y] : -1);
        z = (InRange(z, map) ? map[z] : -1);
        return T;
    } // remap all components
    VecUS &remapFit(C CMemPtr<Int> &map) {
        if (InRange(x, map))
            x = map[x];
        if (InRange(y, map))
            y = map[y];
...
struct VecUS4 // Vector 4D (Unsigned Short)
{
    ...
    VecUS4 &set(UShort x, UShort y, UShort z, UShort w) {
        T.x = x;
        T.y = y;
        T.z = z;
        T.w = w;
    ...
    VecUS4 &remapAll(C CMemPtr<Int> &map) {
        x = (InRange(x, map) ? map[x] : -1);
        y = (InRange(y, map) ? map[y] : -1);
        z = (InRange(z, map) ? map[z] : -1);
        w = (InRange(w, map) ? map[w] : -1);
        return T;
    } // remap all components
    VecUS4 &remapFit(C CMemPtr<Int> &map) {
        if (InRange(x, map))
            x = map[x];
        if (InRange(y, map))
            y = map[y];
...
struct VecH // Vector 3D (Half)
{
    ...
    VecH &set(C Half &x, C Half &y, C Half &z) {
        T.x = x;
        T.y = y;
        T.z = z;
        return T;
    }
    VecH &set(C VecH2 &xy, C Half &z) {
        T.xy = xy;
        T.z = z;
        return T;
    }
...
struct VecH4 // Vector 4D (Half)
{
    ...
    VecH4 &set(C Half &x, C Half &y, C Half &z, C Half &w) {
        T.x = x;
        T.y = y;
        T.z = z;
        T.w = w;
    ...
    VecH4 &set(C VecH2 &xy, C Half &z, C Half &w) {
        T.xy = xy;
        T.z = z;
        T.w = w;
        return T;
    }
    VecH4 &set(C VecH &xyz, C Half &w) {
        T.xyz = xyz;
        T.w = w;
        return T;
    }
    VecH4 &set(C VecH2 &xy, C VecH2 &zw) {
        T.xy = xy;
        T.zw = zw;
        return T;
    }
...
struct VecI2 // Vector 2D (integer)
{
    ...
    VecI2 &operator+=(C VecI2 &v) {
        x += v.x;
        y += v.y;
        return T;
    }
    VecI2 &operator+=(C VecSB2 &v) {
        x += v.x;
        y += v.y;
        return T;
    }
    VecI2 &operator+=(C VecB2 &v) {
        x += v.x;
        y += v.y;
        return T;
    }
    ...
    VecI2 &remapAll(C CMemPtr<Int> &map) {
        x = (InRange(x, map) ? map[x] : -1);
        y = (InRange(y, map) ? map[y] : -1);
        return T;
    } // remap all components
    VecI2 &remapFit(C CMemPtr<Int> &map) {
        if (InRange(x, map))
            x = map[x];
        if (InRange(y, map))
            y = map[y];
...
struct VecI // Vector 3D (integer)
{
    ...
    VecI &set(Int x, Int y, Int z) {
        T.x = x;
        T.y = y;
        T.z = z;
        return T;
    }
    VecI &set(C VecI2 &xy, Int z) {
        T.xy = xy;
        T.z = z;
        return T;
    }
    ...
    VecI &operator+=(C VecI &v) {
        x += v.x;
        y += v.y;
        z += v.z;
        return T;
    }
    VecI &operator+=(C VecSB &v) {
        x += v.x;
        y += v.y;
        z += v.z;
        return T;
    }
    VecI &operator+=(C VecB &v) {
        x += v.x;
        y += v.y;
        z += v.z;
        return T;
    ...
    VecI &remapAll(C CMemPtr<Int> &map) {
        x = (InRange(x, map) ? map[x] : -1);
        y = (InRange(y, map) ? map[y] : -1);
        z = (InRange(z, map) ? map[z] : -1);
        return T;
    } // remap all components
    VecI &remapFit(C CMemPtr<Int> &map) {
        if (InRange(x, map))
            x = map[x];
        if (InRange(y, map))
            y = map[y];
...
struct VecI4 // Vector 4D (integer)
{
    ...
    VecI4 &set(Int x, Int y, Int z, Int w) {
        T.x = x;
        T.y = y;
        T.z = z;
        T.w = w;
    ...
    VecI4 &set(C VecI2 &xy, Int z, Int w) {
        T.xy = xy;
        T.z = z;
        T.w = w;
        return T;
    }
    VecI4 &set(C VecI &xyz, Int w) {
        T.xyz = xyz;
        T.w = w;
        return T;
    }
    VecI4 &set(C VecI2 &xy, C VecI2 &zw) {
        T.xy = xy;
        T.zw = zw;
        return T;
    }
    ...
    VecI4 &remapAll(C CMemPtr<Int> &map) {
        x = (InRange(x, map) ? map[x] : -1);
        y = (InRange(y, map) ? map[y] : -1);
        z = (InRange(z, map) ? map[z] : -1);
        w = (InRange(w, map) ? map[w] : -1);
    ...
    VecI4 &remapFit(C CMemPtr<Int> &map) {
        if (InRange(x, map))
            x = map[x];
        if (InRange(y, map))
            y = map[y];
...
extern VecI4 const VecI4Zero; // VecI4(0, 0, 0, 0)
/******************************************************************************/
...
inline Vec2 &Vec2::operator+=(C VecD2 &v) {
    x += v.x;
    y += v.y;
    return T;
}
...
inline Vec2 &Vec2::operator+=(C VecI2 &v) {
    x += v.x;
    y += v.y;
    return T;
}
...
inline Vec &Vec::operator+=(C VecD &v) {
    x += v.x;
    y += v.y;
    z += v.z;
    return T;
...
inline Vec &Vec::operator+=(C VecI &v) {
    x += v.x;
    y += v.y;
    z += v.z;
    return T;
...
inline Vec4 &Vec4::operator+=(C VecD4 &v) {
    x += v.x;
    y += v.y;
    z += v.z;
    w += v.w;
...
inline Vec4 &Vec4::operator+=(C VecI4 &v) {
    x += v.x;
    y += v.y;
    z += v.z;
    w += v.w;
...
struct SmoothValue {
    void mode(SMOOTH_VALUE_MODE mode);
    SMOOTH_VALUE_MODE mode() C { return _mode; } // set/get SMOOTH_VALUE_MODE
