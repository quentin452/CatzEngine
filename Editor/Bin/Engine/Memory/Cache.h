...
/******************************************************************************/
T1(TYPE)
struct Cache : _Cache // Cache - container for dynamically loaded data, used for fast accessing data through file name, 'Cache' is multi-threaded safe
{
    ...
    template <Cache<TYPE> &CACHE>
    C Str &name(C CacheElmPtr<TYPE, CACHE> &data) C { return name(data()); } // get object file name, ""   on fail
    CChar *name(C TYPE *data, CChar *path) C;                                // get object file name, null on fail
    template <Cache<TYPE> &CACHE>
    CChar *name(C CacheElmPtr<TYPE, CACHE> &data, CChar *path) C { return name(data(), path); } // get object file name, null on fail
    ...
    template <Cache<TYPE> &CACHE>
    UID id(C CacheElmPtr<TYPE, CACHE> &data) C { return id(data()); } // get object file name ID, 'UIDZero' on fail
    ...
    template <Cache<TYPE> &CACHE>
    Bool contains(C CacheElmPtr<TYPE, CACHE> &data) C { return contains(data()); } // check if cache contains this object
    ...
    template <Cache<TYPE> &CACHE>
    Int ptrCount(C CacheElmPtr<TYPE, CACHE> &data) C { return ptrCount(data()); } // check if cache contains this object and return current number of active pointer references to it, -1 is returned if object is not stored in this cache
    Bool dummy(C TYPE *data) C;                                                   // check if cache contains this object and it's a dummy (it was not loaded but created as empty)
    template <Cache<TYPE> &CACHE>
    Bool dummy(C CacheElmPtr<TYPE, CACHE> &data) C { return dummy(data()); } // check if cache contains this object and it's a dummy (it was not loaded but created as empty)
    ...
    template <Cache<TYPE> &CACHE>
    void dummy(C CacheElmPtr<TYPE, CACHE> &data, Bool dummy) { T.dummy(data(), dummy); } //       if cache contains this object then set its dummy state (this can be used for example if object was first loaded as a dummy, but then you've downloaded/generated/saved its data, and now need to update the dummy state)
...
#if EE_PRIVATE
    void canBeRemoved(Bool CanBeRemoved(C TYPE &data)) {
        lock();
        _can_be_removed = (Bool(*)(CPtr))CanBeRemoved;
        unlock();
    }
#endif
...
struct CacheElmPtr // Cache Element Pointer - can hold a reference to a TYPE based object in the CACHE cache, number of active references for a given object is stored in the cache
{
    // operators
...
/******************************************************************************/
#define DECLARE_CACHE(TYPE, cache_name, ptr_name) \
    extern Cache<TYPE> cache_name;                \
    typedef CacheElmPtr<TYPE, cache_name> ptr_name; // this declares a Cache and a CacheElmPtr in a header, the cache stores 'TYPE' data, the cache name is 'cache_name' and the name for the pointer to elements is 'ptr_name'
#define DEFINE_CACHE(TYPE, cache_name, ptr_name, debug_name) \
    Cache<TYPE> cache_name(debug_name);                      \
    typedef CacheElmPtr<TYPE, cache_name> ptr_name; // this defines  a Cache and a CacheElmPtr in C++ file
#if EE_PRIVATE
#define DEFINE_CACHE_EX(TYPE, cache_name, ptr_name, debug_name, block_elms) \
    Cache<TYPE> cache_name(debug_name, block_elms);                         \
    typedef CacheElmPtr<TYPE, cache_name> ptr_name; // this defines a Cache and a CacheElmPtr in C++ file
#endif
/******************************************************************************/
