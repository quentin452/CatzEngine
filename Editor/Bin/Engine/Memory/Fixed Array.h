...
struct FixedArray // array of fixed size, with the option to dynamically replace the type of elements
{
    TYPE &operator[](Int i) {
        DEBUG_RANGE_ASSERT(i, NUM);
        return _data ? *(TYPE *)((Byte *)_data + UIntPtr(i) * _elm_size) : _data_org[i];
    } // get i-th element in the container
    C TYPE &operator[](Int i) C {
        DEBUG_RANGE_ASSERT(i, NUM);
        return _data ? *(TYPE *)((Byte *)_data + UIntPtr(i) * _elm_size) : _data_org[i];
    } // get i-th element in the container
...
struct FixedElm // single element, with the option to dynamically replace its type (works like 'FixedArray' but stores only one element)
{
    TYPE &operator()() { return _data ? *(TYPE *)_data : _data_org; }
    TYPE *operator->() { return &T(); }
    TYPE *operator&() { return &T(); }
    operator TYPE &() { return T(); } // get the element
    C TYPE &operator()() C { return _data ? *(TYPE *)_data : _data_org; }
    C TYPE *operator->() C { return &T(); }
    C TYPE *operator&() C { return &T(); }
    operator C TYPE &() C { return T(); } // get the element

    T1(EXTENDED)
    FixedElm &replaceClass(); // replace the type of class of the element, previous element is automatically removed before changing the type of the class, the new type must be extended from the base 'TYPE' (if you're receiving a compilation error pointing to this method this means that the new class isn't extended from the base class)
...
/******************************************************************************/
template <typename TYPE, Int NUM>
inline Int Elms(C FixedArray<TYPE, NUM> &array) { return array.elms(); }
/******************************************************************************/
