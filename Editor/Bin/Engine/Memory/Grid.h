...
/******************************************************************************/
T1(TYPE)
struct Cell // Cell (template)
{
    TYPE *data() { return _data; }        // get cell data
    TYPE &operator()() { return *_data; } // get cell data
    T1(TYPE2)
    TYPE2 *cast() { return CAST(TYPE2, _data); } // get cell data casted to custom class
  ...
  private:
    ...
    union {
        ...
        struct {
            Cell *_lb, *_b, *_rb, *_l, *_c, *_r, *_lf, *_f, *_rf;
        };
    };
...
/******************************************************************************/
T1(TYPE)
struct Grid : private _Grid // Grid
{
    // manage
    ...
    Ptr user() C { return _Grid::user; }
    void user(Ptr user) { _Grid::user = user; } // get/set custom user data
    ...
    T1(EXTENDED)
    Grid &replaceClass(); // replace the type of class stored in the grid, all grid cells are automatically removed before changing the type of the class, the new type must be extended from the base 'TYPE' (if you're receiving a compilation error pointing to this method this means that the new class isn't extended from the base class)

    // call custom function on grid cells
    void func(void func(Cell<TYPE> &cell, Ptr user)); // call 'func' on all existing grid cells
    T1(USER_DATA)
    void func(void func(Cell<TYPE> &cell, USER_DATA *user), USER_DATA *user = null); // call 'func' on all existing grid cells
    T1(USER_DATA)
    void func(void func(Cell<TYPE> &cell, USER_DATA &user), USER_DATA &user); // call 'func' on all existing grid cells
    void func(C RectI &rect, void func(Cell<TYPE> &cell, Ptr user));          // call 'func' on all existing grid cells in specified rectangle
    T1(USER_DATA)
    void func(C RectI &rect, void func(Cell<TYPE> &cell, USER_DATA *user), USER_DATA *user = null); // call 'func' on all existing grid cells in specified rectangle
    T1(USER_DATA)
    void func(C RectI &rect, void func(Cell<TYPE> &cell, USER_DATA &user), USER_DATA &user); // call 'func' on all existing grid cells in specified rectangle
    ...
    T1(USER_DATA)
    void funcCreate(C RectI &rect, void func(Cell<TYPE> &cell, USER_DATA *user), USER_DATA *user = null); // call 'func' on all          grid cells in specified rectangle (this method creates grid cells if they don't exist yet)
    T1(USER_DATA)
    void funcCreate(C RectI &rect, void func(Cell<TYPE> &cell, USER_DATA &user), USER_DATA &user); // call 'func' on all          grid cells in specified rectangle (this method creates grid cells if they don't exist yet)
    ...
    T1(USER_DATA)
    void mtFunc(Threads &threads, void func(Cell<TYPE> &cell, USER_DATA *user, Int thread_index), USER_DATA *user = null); // call 'func' on all existing grid cells
    T1(USER_DATA)
    void mtFunc(Threads &threads, void func(Cell<TYPE> &cell, USER_DATA &user, Int thread_index), USER_DATA &user); // call 'func' on all existing grid cells
    ...
    T1(USER_DATA)
    void mtFunc(Threads &threads, C RectI &rect, void func(Cell<TYPE> &cell, USER_DATA *user, Int thread_index), USER_DATA *user = null); // call 'func' on all existing grid cells in specified rectangle
    T1(USER_DATA)
    void mtFunc(Threads &threads, C RectI &rect, void func(Cell<TYPE> &cell, USER_DATA &user, Int thread_index), USER_DATA &user); // call 'func' on all existing grid cells in specified rectangle
