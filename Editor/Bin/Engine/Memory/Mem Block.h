...
struct Memb : _Memb // Block Based Container
{
    // manage
    ...
    T1(VALUE)
    Memb &add(C VALUE &value) {
        New() = value;
        return T;
    } // add      'value' to container                                                                                       , this method does not change the memory address of any of the elements
    T1(VALUE)
    Bool include(C VALUE &value) {
        ...
        return false;
    } // include  'value' if it's not already present in container, returns true if value wasn't present and has been added  , this method does not change the memory address of any of the elements
    T1(VALUE)
    Bool exclude(C VALUE &value, Bool keep_order = false) {
        ...
        return false;
    } // exclude  'value' if present  in container                , returns true if value was    present and has been removed, this method may      change the memory address of some       elements
    T1(VALUE)
    Bool toggle(C VALUE &value, Bool keep_order = false) {
        ...
        add(value);
        return true;
    } // toggle   'value'    presence in container                , returns true if value is now present in container        , this method may      change the memory address of some       elements
    ...
    T1(VALUE)
    Memb &binaryAdd(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        Int i;
        binarySearch(value, i, compare);
        NewAt(i) = value;
        return T;
    } // add      'value' (using binary search)                                                                                                    , this method may change the memory address of some elements
    T1(VALUE)
    Bool binaryInclude(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        ...
        return false;
    } // include  'value' (using binary search) if it's not already present in container, returns true if value wasn't present and has been added  , this method may change the memory address of some elements
    T1(VALUE)
    Bool binaryExclude(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        ...
        return false;
    } // exclude  'value' (using binary search) if present  in container                , returns true if value was    present and has been removed, this method may change the memory address of some elements
    T1(VALUE)
    Bool binaryToggle(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        ...
        remove(i, true);
        return false;
    } // toggle   'value' (using binary search)    presence in container                , returns true if value is now present in container        , this method may change the memory address of some elements
    ...
    template <Int size>
    Memb &operator=(C Memt<TYPE, size> &src); // copy elements using assignment operator
    ...
    template <Int size>
    Memb &operator=(C CMemPtr<TYPE, size> &src); // copy elements using assignment operator
    Memb &operator=(Memb<TYPE> &&src);           // copy elements using assignment operator

    T1(EXTENDED)
    Memb &replaceClass(); // replace the type of class stored in the container, all elements are automatically removed before changing the type of the class, the new type must be extended from the base 'TYPE' (if you're receiving a compilation error pointing to this method this means that the new class isn't extended from the base class)

    T1(BASE)
    operator Memb<BASE> &(); // casting to container of 'BASE' elements, 'TYPE' must be extended from BASE
    T1(BASE)
    operator C Memb<BASE> &() C; // casting to container of 'BASE' elements, 'TYPE' must be extended from BASE
#if EE_PRIVATE
    void copyTo(TYPE *dest) C { _Memb::copyTo(dest); } // copy raw memory of all elements to   'dest'
    Memb &copyFrom(C TYPE *src) {
        _Memb::copyFrom(src);
        return T;
...
/******************************************************************************/
T1(TYPE)
struct MembAbstract : _Memb // Block Based Container which allows storage of abstract classes, 'replaceClass' should be called before creating new elements in it
{
    // manage
    ...
    T1(EXTENDED)
    MembAbstract &replaceClass(); // replace the type of class stored in the container, all elements are automatically removed before changing the type of the class, the new type must be extended from the base 'TYPE' (if you're receiving a compilation error pointing to this method this means that the new class isn't extended from the base class)
...
/******************************************************************************/
T1(TYPE)
struct MembConst : Memb<TYPE> // Block Based Container which allows modifying elements even when being 'const'
{
    ...
    T1(BASE)
    operator MembConst<BASE> &(); // casting to container of 'BASE' elements, 'TYPE' must be extended from BASE
    T1(BASE)
    operator C MembConst<BASE> &() C; // casting to container of 'BASE' elements, 'TYPE' must be extended from BASE
