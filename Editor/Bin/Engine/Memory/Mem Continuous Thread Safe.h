...
struct MemcThreadSafe : _MemcThreadSafe // Thread-Safe Continuous Memory Based Container
{
    // manage
    ...
    T1(VALUE)
    Bool lockedHas(C VALUE &value) C { return lockedFind(value) >= 0; } // check if 'value' is present in container, container must be locked first using the 'lock' method before using this method
    T1(VALUE)
    Bool has(C VALUE &value) C {
        SyncLocker locker(_lock);
        return lockedHas(value);
    } // check if 'value' is present in container
    T1(VALUE)
    MemcThreadSafe &lockedAdd(C VALUE &value) {
        lockedNew() = value;
        return T;
    } // add      'value' to container                                                                                       , this method may change the memory address of all elements, container must be locked first using the 'lock' method before using this method
    T1(VALUE)
    MemcThreadSafe &add(C VALUE &value) {
        SyncLocker locker(_lock);
        return lockedAdd(value);
    } // add      'value' to container                                                                                       , this method may change the memory address of all elements
    T1(VALUE)
    MemcThreadSafe &lockedSwapAdd(VALUE &value) {
        Swap(lockedNew(), value);
        return T;
    } // add      'value' to container using 'Swap'                                                                          , this method may change the memory address of all elements, container must be locked first using the 'lock' method before using this method
    T1(VALUE)
    MemcThreadSafe &swapAdd(VALUE &value) {
        SyncLocker locker(_lock);
        return lockedSwapAdd(value);
    } // add      'value' to container using 'Swap'                                                                          , this method may change the memory address of all elements
    T1(VALUE)
    ...
    T1(VALUE)
    Bool lockedBinarySearch(C VALUE &value, Int &index, Int compare(C TYPE &a, C VALUE &b) = Compare) C; // search sorted container for presence of 'value' and return if it was found in the container, 'index'=if the function returned true then this index points to the location where the 'value' is located in the container, if the function returned false then it means that 'value' was not found in the container however the 'index' points to the place where it should be added in the container while preserving sorted data, 'index' will always be in range (0..elms) inclusive, container must be locked first using the 'lock' method before using this method
    T1(VALUE)
    Bool binaryHas(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) C {
        SyncLocker locker(_lock);
        Int i;
        return lockedBinarySearch(value, i, compare);
    } // check if 'value' (using binary search) is present in container
    T1(VALUE)
    TYPE *lockedBinaryFind(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        Int i;
        return lockedBinarySearch(value, i, compare) ? &lockedElm(i) : null;
    } // check if 'value' (using binary search) is present in container and return it, null on fail, container must be locked first using the 'lock' method before using this method
    T1(VALUE)
    C TYPE *lockedBinaryFind(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) C { return ConstCast(T).lockedBinaryFind(value, compare); } // check if 'value' (using binary search) is present in container and return it, null on fail, container must be locked first using the 'lock' method before using this method
    T1(VALUE)
    MemcThreadSafe &binaryAdd(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        SyncLocker locker(_lock);
        Int i;
        lockedBinarySearch(value, i, compare);
        lockedNewAt(i) = value;
        return T;
    ...
    Bool binaryInclude(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        SyncLocker locker(_lock);
        Int i;
        if (!lockedBinarySearch(value, i, compare)) {
            lockedNewAt(i) = value;
            return true;
        }
        return false;
