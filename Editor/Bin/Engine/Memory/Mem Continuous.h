...
/******************************************************************************/
T1(const_mem_addr TYPE)
struct Memc : _Memc // Continuous Memory Based Container
{
    // manage
    ...
    T1(VALUE)
    Memc &add(C VALUE &value) {
        New() = value;
        return T;
    } // add      'value' to container                                                                                       , this method may change the memory address of all elements
    T1(VALUE)
    Bool include(C VALUE &value) {
        ...
        return false;
    } // include  'value' if it's not already present in container, returns true if value wasn't present and has been added  , this method may change the memory address of all elements
    T1(VALUE)
    Bool exclude(C VALUE &value, Bool keep_order = false) {
        ...
        return false;
    } // exclude  'value' if present  in container                , returns true if value was    present and has been removed, this method may change the memory address of all elements
    T1(VALUE)
    Bool toggle(C VALUE &value, Bool keep_order = false) {
        ...
        add(value);
        return true;
    } // toggle   'value'    presence in container                , returns true if value is now present in container        , this method may change the memory address of all elements
    ...
    T1(VALUE)
    TYPE *binaryFind(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        Int i;
    ...
    T1(VALUE)
    C TYPE *binaryFind(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) C { return ConstCast(T).binaryFind(value, compare); } // check if 'value' (using binary search) is present in container and return it, null on fail
    T1(VALUE)
    Memc &binaryAdd(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        Int i;
        binarySearch(value, i, compare);
        NewAt(i) = value;
        return T;
    } // add      'value' (using binary search)                                                                                                    , this method may change the memory address of all elements
    T1(VALUE)
    Bool binaryInclude(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        Int i;
        if (!binarySearch(value, i, compare)) {
            NewAt(i) = value;
            return true;
        }
        return false;
    } // include  'value' (using binary search) if it's not already present in container, returns true if value wasn't present and has been added  , this method may change the memory address of all elements
    T1(VALUE)
    Bool binaryExclude(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        ...
        return false;
    } // exclude  'value' (using binary search) if present  in container                , returns true if value was    present and has been removed, this method may change the memory address of all elements
    T1(VALUE)
    Bool binaryToggle(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        ...
        remove(i, true);
        return false;
    } // toggle   'value' (using binary search)    presence in container                , returns true if value is now present in container        , this method may change the memory address of all elements
    ...
    template <Int size>
    Memc &operator=(C Memt<TYPE, size> &src); // copy elements using assignment operator
    ...
    template <Int size>
    Memc &operator=(C CMemPtr<TYPE, size> &src); // copy elements using assignment operator
    ...
    T1(EXTENDED)
    Memc &replaceClass(); // replace the type of class stored in the container, all elements are automatically removed before changing the type of the class, the new type must be extended from the base 'TYPE' (if you're receiving a compilation error pointing to this method this means that the new class isn't extended from the base class)

    T1(BASE)
    operator Memc<BASE> &(); // casting to container of 'BASE' elements, 'TYPE' must be extended from BASE
    T1(BASE)
    operator C Memc<BASE> &() C; // casting to container of 'BASE' elements, 'TYPE' must be extended from BASE
#if EE_PRIVATE
    void copyTo(TYPE *dest) C { _Memc::copyTo(dest); } // copy raw memory of all elements to   'dest'
    Memc &copyFrom(C TYPE *src) {
        _Memc::copyFrom(src);
        return T;
    } // copy raw memory of all elements from 'src '
#endif
    ...
    T1(USER)
    Bool save(File &f, C USER &user) C; // save elements with their own 'save' method and 'user' parameter, this method first saves number of current elements, and then for each element calls its 'save' method, false on fail
    T1(USER)
    Bool load(File &f, C USER &user); // load elements with their own 'load' method and 'user' parameter, this method first loads number of saved   elements, and then for each element calls its 'load' method, false on fail

    T2(USER, USER1)
    Bool save(File &f, C USER &user, C USER1 &user1) C; // save elements with their own 'save' method and 'user, user1' parameter, this method first saves number of current elements, and then for each element calls its 'save' method, false on fail
    T2(USER, USER1)
    Bool load(File &f, C USER &user, C USER1 &user1); // load elements with their own 'load' method and 'user, user1' parameter, this method first loads number of saved   elements, and then for each element calls its 'load' method, false on fail
...
/******************************************************************************/
T1(TYPE)
struct MemcAbstract : _Memc // Continuous Memory Based Container which allows storage of abstract classes, 'replaceClass' should be called before creating new elements in it
{
    // manage
    ...
    T1(EXTENDED)
    MemcAbstract &replaceClass(); // replace the type of class stored in the container, all elements are automatically removed before changing the type of the class, the new type must be extended from the base 'TYPE' (if you're receiving a compilation error pointing to this method this means that the new class isn't extended from the base class)
...
T1(TYPE)
struct std__unique_ptr : private Mems<TYPE> {
...
T1(TYPE)
struct std__vector : private Memc<TYPE> {
    TYPE &operator[](Int i) { return super::operator[](i); }
    C TYPE &operator[](Int i) C { return super::operator[](i); }
    ...
    std__vector(Int elms, C TYPE &def_val) {
        resize(elms);
        REPAO(T) = def_val;
    }
};
T1(TYPE)
inline Int Elms(C std__vector<TYPE> &vector) { return vector.size(); }
#endif
/******************************************************************************/
