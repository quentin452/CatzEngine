...
/******************************************************************************/
T1(TYPE)
struct Memx : _Memx // Block Based Extended Container
{
    // manage
    ...
    T1(VALUE)
    Memx &binaryAdd(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        Int i;
        binarySearch(value, i, compare);
        NewAt(i) = value;
        return T;
    } // add      'value' (using binary search)                                                                                                    , this method does not change the memory address of any of the elements
    T1(VALUE)
    Bool binaryInclude(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        ...
        return false;
    } // include  'value' (using binary search) if it's not already present in container, returns true if value wasn't present and has been added  , this method does not change the memory address of any of the elements
    T1(VALUE)
    Bool binaryExclude(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        ...
        return false;
    } // exclude  'value' (using binary search) if present  in container                , returns true if value was    present and has been removed, this method does not change the memory address of any of the elements
    T1(VALUE)
    Bool binaryToggle(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        ...
        removeValid(i, true);
        return false;
    } // toggle   'value' (using binary search)    presence in container                , returns true if value is now present in container        , this method does not change the memory address of any of the elements
    ...
    template <Int size>
    Memx &operator=(C Memt<TYPE, size> &src); // copy elements using assignment operator
    ...
    template <Int size>
    Memx &operator=(C CMemPtr<TYPE, size> &src); // copy elements using assignment operator
    Memx &operator=(Memx<TYPE> &&src);           // copy elements using assignment operator

    T1(EXTENDED)
    Memx &replaceClass(); // replace the type of class stored in the container, all elements are automatically removed before changing the type of the class, the new type must be extended from the base 'TYPE' (if you're receiving a compilation error pointing to this method this means that the new class isn't extended from the base class)

    T1(BASE)
    operator Memx<BASE> &(); // casting to container of 'BASE' elements, 'TYPE' must be extended from BASE
    T1(BASE)
    operator C Memx<BASE> &() C; // casting to container of 'BASE' elements, 'TYPE' must be extended from BASE
...
/******************************************************************************/
T1(TYPE)
struct MemxAbstract : _Memx // Block Based Extended Container which allows storage of abstract classes, 'replaceClass' should be called before creating new elements in it
{
    // manage
    ...
    T1(EXTENDED)
    MemxAbstract &replaceClass(); // replace the type of class stored in the container, all elements are automatically removed before changing the type of the class, the new type must be extended from the base 'TYPE' (if you're receiving a compilation error pointing to this method this means that the new class isn't extended from the base class)
