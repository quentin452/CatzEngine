...
struct CMemPtr // Const Memory Container Pointer, 'Memt_size'=size of the 'Memt' helper stack memory
{
    // get / set
    ...
    T1(VALUE)
    Bool has(C VALUE &value) C { return find(value) >= 0; } // check if 'value' is present in container
    ...
    T1(VALUE)
    Bool binaryHas(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) C {
        Int i;
        return binarySearch(value, i, compare);
    } // check if 'value' (using binary search) is present in container
    T1(VALUE)
    C TYPE *binaryFind(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) C {
        Int i;
        return binarySearch(value, i, compare) ? &T[i] : null;
    } // check if 'value' (using binary search) is present in container and return it, null on fail
    ...
    template <Int src_elms>
    CMemPtr(C TYPE (&src)[src_elms]) { point(src); }
    ...
    CMemPtr(C Memc<TYPE> *src) { point(src); }
    T1(EXTENDED)
    CMemPtr(C Memc<EXTENDED> *src) { point(src); }
    CMemPtr(C Memt<TYPE, Memt_size> &src) { point(src); }
    CMemPtr(C Memt<TYPE, Memt_size> *src) { point(src); }
    CMemPtr(C Memb<TYPE> &src) { point(src); }
    CMemPtr(C Memb<TYPE> *src) { point(src); }
    T1(EXTENDED)
    CMemPtr(C Memb<EXTENDED> *src) { point(src); }
    CMemPtr(C Memx<TYPE> &src) { point(src); }
    CMemPtr(C Memx<TYPE> *src) { point(src); }
    T1(EXTENDED)
    CMemPtr(C Memx<EXTENDED> *src) { point(src); }
...
/******************************************************************************/
template <typename TYPE, Int Memt_size>
struct MemPtr : CMemPtr<TYPE, Memt_size> // Memory Container Pointer, 'Memt_size'=size of the 'Memt' helper stack memory
{
    // manage
    ...
    Bool has(C VALUE &value) C { return find(value) >= 0; } // check if 'value' is present in container
    T1(VALUE)
    MemPtr &add(C VALUE &value) {
        New() = value;
        return T;
    } // add      'value' to container                                                                                       , this method changes the memory address of all elements
    T1(VALUE)
    Bool include(C VALUE &value) {
        if (!has(value)) {
            add(value);
            return true;
        }
        return false;
    } // include  'value' if it's not already present in container, returns true if value wasn't present and has been added  , this method changes the memory address of all elements
    T1(VALUE)
    Bool exclude(C VALUE &value, Bool keep_order = false) {
        Int i = find(value);
        if (i >= 0) {
            remove(i, keep_order);
            return true;
        ...
        return false;
    } // exclude  'value' if present  in container                , returns true if value was    present and has been removed, this method changes the memory address of all elements
    T1(VALUE)
    Bool toggle(C VALUE &value, Bool keep_order = false) {
        Int i = find(value);
        if (i >= 0) {
            remove(i, keep_order);
            return false;
        ...
        add(value);
        return true;
    } // toggle   'value'    presence in container                , returns true if value is now present in container        , this method changes the memory address of all elements

    T1(VALUE)
    TYPE *binaryFind(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) { return ConstCast(super::binaryFind(value, compare)); } // check if 'value' (using binary search) is present in container and return it, null on fail
    T1(VALUE)
    C TYPE *binaryFind(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) C { return super::binaryFind(value, compare); } // check if 'value' (using binary search) is present in container and return it, null on fail
    T1(VALUE)
    MemPtr &binaryAdd(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        Int i;
        super::binarySearch(value, i, compare);
        NewAt(i) = value;
        return T;
    } // add      'value' (using binary search)                                                                                                    , this method changes the memory address of all elements
    T1(VALUE)
    Bool binaryInclude(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        Int i;
        if (!super::binarySearch(value, i, compare)) {
            NewAt(i) = value;
            return true;
        }
        return false;
    } // include  'value' (using binary search) if it's not already present in container, returns true if value wasn't present and has been added  , this method changes the memory address of all elements
    T1(VALUE)
    Bool binaryExclude(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        Int i;
        if (super::binarySearch(value, i, compare)) {
            remove(i, true);
            return true;
        ...
        return false;
    } // exclude  'value' (using binary search) if present  in container                , returns true if value was    present and has been removed, this method changes the memory address of all elements
    T1(VALUE)
    Bool binaryToggle(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        Int i;
        if (!super::binarySearch(value, i, compare)) {
            NewAt(i) = value;
            return true;
        ...
        remove(i, true);
        return false;
    } // toggle   'value' (using binary search)    presence in container                , returns true if value is now present in container        , this method changes the memory address of all elements
    ...
    template <Int src_elms>
    MemPtr &operator=(C TYPE (&src)[src_elms]); // copy elements using assignment operator
    ...
    template <Int src_size>
    MemPtr &operator=(C Memt<TYPE, src_size> &src); // copy elements using assignment operator
    ...
    template <Int src_size>
    MemPtr &operator=(C CMemPtr<TYPE, src_size> &src); // copy elements using assignment operator (this will allow copying from 'CMemPtr' and 'MemPtr' with other sizes)
    ...
    template <Int src_elms>
    MemPtr(TYPE (&src)[src_elms]) : CMemPtr<TYPE, Memt_size>(src) {}
    ...
    MemPtr(Memc<TYPE> *src) : CMemPtr<TYPE, Memt_size>(src) {}
    T1(EXTENDED)
    MemPtr(Memc<EXTENDED> *src) : CMemPtr<TYPE, Memt_size>(src) {}
    MemPtr(Memt<TYPE, Memt_size> &src) : CMemPtr<TYPE, Memt_size>(src) {}
    MemPtr(Memt<TYPE, Memt_size> *src) : CMemPtr<TYPE, Memt_size>(src) {}
    MemPtr(Memb<TYPE> &src) : CMemPtr<TYPE, Memt_size>(src) {}
    MemPtr(Memb<TYPE> *src) : CMemPtr<TYPE, Memt_size>(src) {}
    T1(EXTENDED)
    MemPtr(Memb<EXTENDED> *src) : CMemPtr<TYPE, Memt_size>(src) {}
    MemPtr(Memx<TYPE> &src) : CMemPtr<TYPE, Memt_size>(src) {}
    MemPtr(Memx<TYPE> *src) : CMemPtr<TYPE, Memt_size>(src) {}
    T1(EXTENDED)
    MemPtr(Memx<EXTENDED> *src) : CMemPtr<TYPE, Memt_size>(src) {}
    ...
    T1(EXTENDED)
    MemPtr &operator=(C Memc<EXTENDED> *src) = delete;
    template <Int src_size>
    MemPtr &operator=(C Memt<TYPE, src_size> *src) = delete;
    MemPtr &operator=(C Memb<TYPE> *src) = delete;
    T1(EXTENDED)
    MemPtr &operator=(C Memb<EXTENDED> *src) = delete;
    MemPtr &operator=(C Memx<TYPE> *src) = delete;
    T1(EXTENDED)
    MemPtr &operator=(C Memx<EXTENDED> *src) = delete;
    MemPtr &operator=(C Meml<TYPE> *src) = delete;
};
/******************************************************************************/
template <const_mem_addr typename TYPE, Int Memt_elms>
struct MemPtrN : MemPtr<TYPE, SIZE(TYPE) * Memt_elms> // Memory Container Pointer, 'Memt_elms'=number of elements of the 'Memt'
{
    // copy elements using assignment operator
    MemPtrN &operator=(C TYPE &src); // copy elements using assignment operator
    template <Int src_elms>
    MemPtrN &operator=(C TYPE (&src)[src_elms]); // copy elements using assignment operator
    ...
    template <Int src_size>
    MemPtrN &operator=(C Memt<TYPE, src_size> &src); // copy elements using assignment operator
    ...
    template <Int src_size>
    MemPtrN &operator=(C CMemPtr<TYPE, src_size> &src);  // copy elements using assignment operator (this will allow copying from 'MemPtr' with other sizes)
    ...
    template <Int src_elms>
    MemPtrN(TYPE (&src)[src_elms]) : MemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    ...
    MemPtrN(Memc<TYPE> *src) : MemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    T1(EXTENDED)
    MemPtrN(Memc<EXTENDED> *src) : MemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    ...
    MemPtrN(Memb<TYPE> *src) : MemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    T1(EXTENDED)
    MemPtrN(Memb<EXTENDED> *src) : MemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    MemPtrN(Memx<TYPE> &src) : MemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    MemPtrN(Memx<TYPE> *src) : MemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    T1(EXTENDED)
    MemPtrN(Memx<EXTENDED> *src) : MemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    ...
    T1(EXTENDED)
    MemPtrN &operator=(C Memc<EXTENDED> *src) = delete;
    template <Int src_elms>
    MemPtrN &operator=(C Memt<TYPE, SIZE(TYPE) * Memt_elms> *src) = delete;
    MemPtrN &operator=(C Memb<TYPE> *src) = delete;
    T1(EXTENDED)
    MemPtrN &operator=(C Memb<EXTENDED> *src) = delete;
    MemPtrN &operator=(C Memx<TYPE> *src) = delete;
    T1(EXTENDED)
    MemPtrN &operator=(C Memx<EXTENDED> *src) = delete;
    MemPtrN &operator=(C Meml<TYPE> *src) = delete;
};
/******************************************************************************/
template <const_mem_addr typename TYPE, Int Memt_elms>
struct CMemPtrN : CMemPtr<TYPE, SIZE(TYPE) * Memt_elms> // Const Memory Container Pointer, 'Memt_elms'=number of elements of the 'Memt'
{
    ...
    template <Int src_elms>
    CMemPtrN(C TYPE (&src)[src_elms]) : CMemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    ...
    CMemPtrN(C Memc<TYPE> *src) : CMemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    T1(EXTENDED)
    CMemPtrN(C Memc<EXTENDED> *src) : CMemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    ...
    CMemPtrN(C Memb<TYPE> *src) : CMemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    T1(EXTENDED)
    CMemPtrN(C Memb<EXTENDED> *src) : CMemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    CMemPtrN(C Memx<TYPE> &src) : CMemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    CMemPtrN(C Memx<TYPE> *src) : CMemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
    T1(EXTENDED)
    CMemPtrN(C Memx<EXTENDED> *src) : CMemPtr<TYPE, SIZE(TYPE) * Memt_elms>(src) {}
...
inline Int Elms(C CMemPtr<TYPE, size> &memp) { return memp.elms(); }
/******************************************************************************/
