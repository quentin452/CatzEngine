...
struct Mems // Simple Continuous Memory Based Container
{
    ...
    T1(VALUE)
    Mems &binaryAdd(C VALUE &value, Int compare(C TYPE &a, C VALUE &b) = Compare) {
        Int i;
    ...
    template <Int size>
    Mems &operator=(C Memt<TYPE, size> &src); // copy elements using assignment operator
    ...
    template <Int size>
    Mems &operator=(C CMemPtr<TYPE, size> &src); // copy elements using assignment operator
...
#if EE_PRIVATE
T1(const_mem_addr TYPE)
struct FixedMems : Mems<TYPE> // Unresizable Mems container
#else
T1(const_mem_addr TYPE)
struct FixedMems : private Mems<TYPE> // Unresizable Mems container
#endif
{
    // get / set
...
/******************************************************************************/
T1(TYPE)
Int Elms(C Mems<TYPE> &mems) { return mems.elms(); }
T1(TYPE)
Int Elms(C FixedMems<TYPE> &mems) { return mems.elms(); }
/******************************************************************************/
