...
struct TreeNode {
    ...
    void parent(TreeNode *p) {
        parent_tag &= 3;
        parent_tag |= UIntPtr(p);
    }
  ...
  private:
    ...
    template <Int t>
    void setTag() { setTag(std::integral_constant<Int, t>()); }
};
/******************************************************************************/
...
struct BSTree {
    ...
    Node *find(C Key &value) C {
        Node *node = root();
        while (node) {
            if (compare(value, key(*node)))
                node = (Node *)node->left;
            else if (compare(key(*node), value))
  ...
  protected:
    ...
    void insert(Node &node) {
        ...
        while (*link) {
            parent = (Node *)(*link);
            if (compare(node_key, key(*parent)))
                link = &parent->left;
            else
                link = &parent->right;
        }
        ...
        *link = &node;
        ...
        if (!_last || !compare(node_key, key(*(Node *)_last)))
            _last = &node;
    }
    Bool insertUnique(Node &node) // no duplicate keys
    {
        ...
        while (*link) {
            ...
            if (compare(node_key, key(*parent)))
                link = &parent->left;
            else if (compare(key(*parent), node_key))
                link = &parent->right;
            else
                return false;
        }
...
/******************************************************************************/
T2(Node, Key)
struct AVLTree : BSTree<Node, Key> {
...
/******************************************************************************/
T2(Node, Key)
struct WAVLTree : BSTree<Node, Key> {
