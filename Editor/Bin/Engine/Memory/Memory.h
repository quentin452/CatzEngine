...
T1(TYPE)
TYPE *Alloc(Long elms) { return (TYPE *)Alloc(elms * SIZEL(TYPE)); } // allocate          'elms' elements of 'type', Exit on fail, !! combine only with 'Free' !!, sample usage: "           Alloc    <Int> (100);" will allocate raw memory of 100        Int elements
T1(TYPE)
TYPE *Alloc(UInt elms = 1) { return (TYPE *)Alloc(elms * SIZEUL(TYPE)); } // allocate          'elms' elements of 'type', Exit on fail, !! combine only with 'Free' !!, sample usage: "           Alloc    <Int> (100);" will allocate raw memory of 100        Int elements
T1(TYPE)
TYPE *Alloc(Int elms) { return (TYPE *)Alloc(elms * SIZEL(TYPE)); } // allocate          'elms' elements of 'type', Exit on fail, !! combine only with 'Free' !!, sample usage: "           Alloc    <Int> (100);" will allocate raw memory of 100        Int elements
T1(TYPE)
TYPE *AllocZero(Long elms) { return (TYPE *)AllocZero(elms * SIZEL(TYPE)); } // allocate and zero 'elms' elements of 'type', Exit on fail, !! combine only with 'Free' !!, sample usage: "           AllocZero<Int> (100);" will allocate raw memory of 100 zeroed Int elements
T1(TYPE)
TYPE *AllocZero(UInt elms = 1) { return (TYPE *)AllocZero(elms * SIZEUL(TYPE)); } // allocate and zero 'elms' elements of 'type', Exit on fail, !! combine only with 'Free' !!, sample usage: "           AllocZero<Int> (100);" will allocate raw memory of 100 zeroed Int elements
T1(TYPE)
TYPE *AllocZero(Int elms) { return (TYPE *)AllocZero(elms * SIZEL(TYPE)); } // allocate and zero 'elms' elements of 'type', Exit on fail, !! combine only with 'Free' !!, sample usage: "           AllocZero<Int> (100);" will allocate raw memory of 100 zeroed Int elements
T1(TYPE)
TYPE *&Alloc(TYPE *&data, Long elms) {
    data = Alloc<TYPE>(elms);
    return data;
} // allocate          memory for 'elms' TYPE's , Exit on fail, !! combine only with 'Free' !!, sample usage: "Int *data; Alloc    (data, 100);" will allocate raw memory of 100        Int elements
T1(TYPE)
TYPE *&Alloc(TYPE *&data, UInt elms = 1) {
    data = Alloc<TYPE>(elms);
    return data;
} // allocate          memory for 'elms' TYPE's , Exit on fail, !! combine only with 'Free' !!, sample usage: "Int *data; Alloc    (data, 100);" will allocate raw memory of 100        Int elements
T1(TYPE)
TYPE *&Alloc(TYPE *&data, Int elms) {
    data = Alloc<TYPE>(elms);
    return data;
} // allocate          memory for 'elms' TYPE's , Exit on fail, !! combine only with 'Free' !!, sample usage: "Int *data; Alloc    (data, 100);" will allocate raw memory of 100        Int elements
T1(TYPE)
TYPE *&AllocZero(TYPE *&data, Long elms) {
    data = AllocZero<TYPE>(elms);
    return data;
} // allocate and zero memory for 'elms' TYPE's , Exit on fail, !! combine only with 'Free' !!, sample usage: "Int *data; AllocZero(data, 100);" will allocate raw memory of 100 zeroed Int elements
T1(TYPE)
TYPE *&AllocZero(TYPE *&data, UInt elms = 1) {
    data = AllocZero<TYPE>(elms);
    return data;
} // allocate and zero memory for 'elms' TYPE's , Exit on fail, !! combine only with 'Free' !!, sample usage: "Int *data; AllocZero(data, 100);" will allocate raw memory of 100 zeroed Int elements
T1(TYPE)
TYPE *&AllocZero(TYPE *&data, Int elms) {
    data = AllocZero<TYPE>(elms);
    return data;
} // allocate and zero memory for 'elms' TYPE's , Exit on fail, !! combine only with 'Free' !!, sample usage: "Int *data; AllocZero(data, 100);" will allocate raw memory of 100 zeroed Int elements
  /******************************************************************************/
...
TYPE *&Free(TYPE *&data) {
    Free(*(Ptr *)&data);
    return data;
} // free memory and set pointer to null !! combine only with 'Alloc' !!
/******************************************************************************/
// REALLOCATE MEMORY
/******************************************************************************/
T1(TYPE)
void Realloc(TYPE *&data, Int elms_new, Int elms_old); // reallocate memory without losing data with 'elms_new' and 'elms_old'                       , Exit on fail !! combine only with 'Alloc' !!
T1(TYPE)
void ReallocZero(TYPE *&data, Int elms_new, Int elms_old); // reallocate memory without losing data with 'elms_new' and 'elms_old', and zero new elements, Exit on fail !! combine only with 'Alloc' !!
                                                           /******************************************************************************/
                                                           // ZERO MEMORY
                                                           /******************************************************************************/
...
T1(TYPE)
void Zero(TYPE &elm) { memset(&elm, 0, SIZE(TYPE)); } // zero        element  memory
T1(TYPE)
void ZeroN(TYPE *data, Int elms) { Zero(data, elms * SIZEL(TYPE)); } // zero 'elms' elements memory
                                                                     /******************************************************************************/
                                                                     // SET MEMORY
                                                                     /******************************************************************************/
...
T1(TYPE)
void SetMem(TYPE &elm, Byte value) { memset(&elm, value, SIZE(TYPE)); } // set 'elm'           memory to Byte value
T1(TYPE)
void SetMemN(TYPE *data, Byte value, Int elms) { SetMem(data, value, elms * SIZEL(TYPE)); } // set 'elms' elements memory to Byte value
/******************************************************************************/
// COPY MEMORY
/******************************************************************************/
#if EE_PRIVATE
...
T1(TYPE)
void Copy(TYPE &dest, C TYPE &src) { memcpy(&dest, &src, SIZE(TYPE)); } // copy 'src'  element to 'dest'
T1(TYPE)
void CopyN(TYPE *dest, C TYPE *src, Int elms) { Copy(dest, src, elms * SIZEL(TYPE)); } // copy 'elms' elements
                                                                                       /******************************************************************************/
                                                                                       // MEMORY ADVANCED
                                                                                       /******************************************************************************/
inline void ZeroFast(Ptr data, UIntPtr size) { memset(data, 0, size); }                // !! this does not check for pointers!=null, size>=0
T1(TYPE)
void ZeroFast(TYPE &elm) { memset(&elm, 0, SIZE(TYPE)); }
T1(TYPE)
void ZeroFastN(TYPE *data, UInt elms) { memset(data, 0, elms * SIZE(TYPE)); } // !! this does not check for pointers!=null, elms>=0
...
T1(TYPE)
void MoveFastN(TYPE *dest, C TYPE *src, UInt elms) { memmove(dest, src, elms * SIZE(TYPE)); } // !! this does not check for pointers!=null, elms>=0                       , this is more safe than 'CopyFast'
...
T1(TYPE)
void CopyFast(TYPE &dest, C TYPE &src) { memcpy(&dest, &src, SIZE(TYPE)); } // !! this does not check                                 if memory overlaps, this is less safe than 'MoveFast'
T1(TYPE)
void CopyFastN(TYPE *dest, C TYPE *src, UInt elms) { memcpy((Ptr)dest, (CPtr)src, elms * SIZE(TYPE)); } // !! this does not check for pointers!=null, elms>=0 and if memory overlaps, this is less safe than 'MoveFast', cast to ptr to silence "-Wdynamic-class-memaccess" warning
...
inline void Swap(Char8 &a, Char8 &b) {
    Char8 temp = a;
    a = b;
    b = temp;
} // swap elements
inline void Swap(SByte &a, SByte &b) {
    SByte temp = a;
    a = b;
    b = temp;
} // swap elements
...
inline void Swap(UShort &a, UShort &b) {
    UShort temp = a;
    a = b;
    b = temp;
} // swap elements
...
inline void Swap(Flt &a, Flt &b) {
    Flt temp = a;
    a = b;
    b = temp;
} // swap elements
inline void Swap(Dbl &a, Dbl &b) {
    Dbl temp = a;
    a = b;
    b = temp;
} // swap elements
inline void Swap(Ptr &a, Ptr &b) {
    Ptr temp = a;
    a = b;
    b = temp;
} // swap elements
...
T1(TYPE)
void Swap(TYPE &a, TYPE &b) { SwapFast(&a, &b, SIZE(TYPE)); } // swap elements
#if EE_PRIVATE
inline void SwapEndian(Short &i) {
    Byte *b = (Byte *)&i;
    Swap(b[0], b[1]);
}
inline void SwapEndian(UShort &i) {
    Byte *b = (Byte *)&i;
    Swap(b[0], b[1]);
}
inline void SwapEndian(Int &i) {
    Byte *b = (Byte *)&i;
    Swap(b[0], b[3]);
    Swap(b[1], b[2]);
}
inline void SwapEndian(UInt &i) {
    Byte *b = (Byte *)&i;
    Swap(b[0], b[3]);
    Swap(b[1], b[2]);
}
inline void SwapEndian(Long &i) {
    Byte *b = (Byte *)&i;
    Swap(b[0], b[7]);
    Swap(b[1], b[6]);
    Swap(b[2], b[5]);
...
inline void SwapEndian(ULong &i) {
    Byte *b = (Byte *)&i;
    Swap(b[0], b[7]);
    Swap(b[1], b[6]);
    Swap(b[2], b[5]);
...
/******************************************************************************/
T1(TYPE)
void ReverseOrder(TYPE *data, Int elms); // reverse   order of elements (first<->last)
T1(TYPE)
void RotateOrder(TYPE *data, Int elms, Int offset); // rotate    order of elements, changes the order of elements so "new_index=old_index+offset", 'offset'=offset of moving the original indexes into target indexes (-Inf..Inf)
T1(TYPE)
void RandomizeOrder(TYPE *data, Int elms, Randomizer &random = Random); // randomize order of elements
                                                                        /******************************************************************************/
                                                                        // COMPARE MEMORY
                                                                        /******************************************************************************/
Bool EqualMem(CPtr a, CPtr b, IntPtr size);                             // if equal memories
T1(TYPE)
Bool EqualMem(C TYPE &a, C TYPE &b) { return EqualMem((CPtr)&a, (CPtr)&b, SIZE(TYPE)); } // if equal elements (in memory)
/******************************************************************************/
...
TYPE *&New(TYPE *&data) {
    data = new TYPE;
    return data;
} // create        object  !! combine only with 'Delete'  or 'delete'   !!
T1(TYPE)
TYPE *&New(TYPE *&data, Int elms) {
    data = ((elms > 0) ? new TYPE[elms] : null);
    return data;
} // create 'elms' objects !! combine only with 'DeleteN' or 'delete[]' !!
/******************************************************************************/
...
TYPE *&Delete(TYPE *&data) {
    ...
    data = null;
    return data;
} // delete object  and set pointer to null !! combine only with "New(data      )" or 'new'   !!
T1(TYPE)
TYPE *&DeleteN(TYPE *&data) {
    delete[] data;
    data = null;
    return data;
} // delete objects and set pointer to null !! combine only with "New(data, elms)" or 'new[]' !!
/******************************************************************************/
...
T1(TYPE)
void CopyIs(TYPE *dest, C TYPE *src, C CMemPtr<Bool> &is) { _CopyIs(dest, (CPtr)src, is, SIZE(TYPE)); } // copy only elements with "is[]=true"
T1(TYPE)
void CopyList(TYPE *dest, C TYPE *src, C CMemPtr<Int> &list) { _CopyList(dest, (CPtr)src, list, SIZE(TYPE)); } // copy only elements mentioned on the list
