...
struct _Cell // Cell - Do not use this class, use 'Cell' instead
{
  ...
  private:
    ...
    union {
        ...
        struct {
            _Cell *_lb, *_b, *_rb, *_l, *_c, *_r, *_lf, *_f, *_rf;
        };
    };
...
struct _Grid // Grid - Do not use this class, use 'Grid' instead
{
    ...
    void replaceClass() {
        struct Helper {
            static void New(EXTENDED *&elm, C VecI2 &xy, Ptr grid_user) {
                Alloc(elm);
                new (elm) EXTENDED(xy, grid_user);
            } // first allocate memory to setup pointer, then call the constructor in case the constructor checks the grid for self
            static void Del(EXTENDED *&elm) {
                if (elm) {
                    elm->~EXTENDED();
                    Free(elm);
                }
