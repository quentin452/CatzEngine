...
struct Blocks {
    struct Light {
        ...
        Light &set(C Ball &ball, C Vec &color = 1) {
            T.ball = ball;
            T.color = color;
            return T;
        }
    ...
    Int subDivision() C { return _sub_division; }
    Blocks &subDivision(Int steps); // get/set mesh sub division, changing the 'subDivision' does not automatically rebuild the mesh, you should call 'setMesh' afterwards
    ...
    // draw
    void draw(C MatrixM &matrix) C {
        if (Frustum(_mesh, matrix))
            _mesh.draw(matrix);
    } // 'matrix'=matrix of this 'Blocks' object
    void drawShadow(C MatrixM &matrix) C {
        if (Frustum(_mesh, matrix))
            _mesh.drawShadow(matrix);
    } // 'matrix'=matrix of this 'Blocks' object
  ...
  private:
    ...
    struct Level {
        ...
        void init(Int resolution, Int y) {
            T.y = y;
            init(resolution);
        }
...
#if EE_PRIVATE
        MtrlCombo &set(Byte top, Byte side, Byte bottom) {
            T.top = top;
            T.side = side;
            T.bottom = bottom;
            return T;
    ...
    struct Part {
        struct Vtx {
            ...
            void set(C Vec &pos, C Vec2 &tex, C Color &col) {
                T.pos = pos;
                T.tex = tex;
                T.col = col;
            }
