...
struct MeshVtxs // Mesh Vertexes
{
    ...
    Vec &nrm(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _nrm[i];
    } // get i-th vertex normal
    C Vec *nrm() C { return _nrm; }
    C Vec &nrm(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _nrm[i];
    } // get i-th vertex normal
    ...
    Vec &tan(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _tan[i];
    } // get i-th vertex tangent
    C Vec *tan() C { return _tan; }
    C Vec &tan(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _tan[i];
    } // get i-th vertex tangent
    ...
    Vec &bin(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _bin[i];
    } // get i-th vertex binormal
    C Vec *bin() C { return _bin; }
    C Vec &bin(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _bin[i];
    } // get i-th vertex binormal
    ...
    Vec &hlp(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _hlp[i];
    } // get i-th vertex helper
    C Vec *hlp() C { return _hlp; }
    C Vec &hlp(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _hlp[i];
    } // get i-th vertex helper
    Vec2 *tex0() { return _tex0; }
    Vec2 &tex0(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _tex0[i];
    } // get i-th vertex texture UV 0
    C Vec2 *tex0() C { return _tex0; }
    C Vec2 &tex0(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _tex0[i];
    } // get i-th vertex texture UV 0
    Vec2 *tex1() { return _tex1; }
    Vec2 &tex1(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _tex1[i];
    } // get i-th vertex texture UV 1
    C Vec2 *tex1() C { return _tex1; }
    C Vec2 &tex1(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _tex1[i];
    } // get i-th vertex texture UV 1
    Vec2 *tex2() { return _tex2; }
    Vec2 &tex2(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _tex2[i];
    } // get i-th vertex texture UV 2
    C Vec2 *tex2() C { return _tex2; }
    C Vec2 &tex2(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _tex2[i];
    } // get i-th vertex texture UV 2
    Vec2 *tex3() { return _tex3; }
    Vec2 &tex3(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _tex3[i];
    } // get i-th vertex texture UV 3
    C Vec2 *tex3() C { return _tex3; }
    C Vec2 &tex3(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _tex3[i];
    } // get i-th vertex texture UV 3
    ...
    Color &color(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _color[i];
    } // get i-th vertex color
    C Color *color() C { return _color; }
    C Color &color(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _color[i];
    } // get i-th vertex color
    VecB4 *material() { return _material; }
    VecB4 &material(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _material[i];
    } // get i-th vertex material blend
    C VecB4 *material() C { return _material; }
    C VecB4 &material(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _material[i];
    } // get i-th vertex material blend
    VtxBone *matrix() { return _matrix; }
    VtxBone &matrix(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _matrix[i];
    } // get i-th vertex bone/matrix index , index maps to a specific bone of a skeleton, where 0=skeleton root bone, 1=skeleton bone #0, 2=skeleton bone #1, 3=skeleton bone #2, and so on, this can be summarized in the following : bone = (index ? skeleton.bone[index-1] : skeleton.root)
    C VtxBone *matrix() C { return _matrix; }
    C VtxBone &matrix(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _matrix[i];
    } // get i-th vertex bone/matrix index , index maps to a specific bone of a skeleton, where 0=skeleton root bone, 1=skeleton bone #0, 2=skeleton bone #1, 3=skeleton bone #2, and so on, this can be summarized in the following : bone = (index ? skeleton.bone[index-1] : skeleton.root)
    VecB4 *blend() { return _blend; }
    VecB4 &blend(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _blend[i];
    } // get i-th vertex bone/matrix weight, weight factor between corresponding matrix.xyzw bones, their sum must be equal to 255 !!
    C VecB4 *blend() C { return _blend; }
    C VecB4 &blend(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _blend[i];
    } // get i-th vertex bone/matrix weight, weight factor between corresponding matrix.xyzw bones, their sum must be equal to 255 !!
    Flt *size() { return _size; }
    Flt &size(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _size[i];
    } // get i-th vertex size
    C Flt *size() C { return _size; }
    C Flt &size(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _size[i];
    } // get i-th vertex size
    Byte *flag() { return _flag; }
    Byte &flag(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _flag[i];
    } // get i-th vertex flag
    C Byte *flag() C { return _flag; }
    C Byte &flag(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _flag[i];
    } // get i-th vertex flag
    ...
    Int &dup(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _dup[i];
    } // get i-th vertex duplicate, index of identical vertex
    C Int *dup() C { return _dup; }
    C Int &dup(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _dup[i];
    } // get i-th vertex duplicate, index of identical vertex
...
#endif
    Int _elms;
    Vec *_pos, *_nrm, *_tan, *_bin, *_hlp;
    Vec2 *_tex0, *_tex1, *_tex2, *_tex3;
    Color *_color;
    VecB4 *_material;
    VtxBone *_matrix;
    VecB4 *_blend;
...
struct MeshEdges // Mesh Edges
{
    ...
    VecI2 *ind() { return _ind; }
    VecI2 &ind(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _ind[i];
    } // get i-th edge vertex indexes
    C VecI2 *ind() C { return _ind; }
    C VecI2 &ind(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _ind[i];
    } // get i-th edge vertex indexes
    VecI2 *adjFace() { return _adj_face; }
    VecI2 &adjFace(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _adj_face[i];
    } // get i-th edge adjacent faces, index to faces adjacent to the edge encoded in following way : if(adj_face==-1) -> no face, else if(adj_face&SIGN_BIT)adj_quad_index=adj_face^SIGN_BIT, else adj_tri_index=adj_face
    C VecI2 *adjFace() C { return _adj_face; }
    C VecI2 &adjFace(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _adj_face[i];
    } // get i-th edge adjacent faces, index to faces adjacent to the edge encoded in following way : if(adj_face==-1) -> no face, else if(adj_face&SIGN_BIT)adj_quad_index=adj_face^SIGN_BIT, else adj_tri_index=adj_face
    Vec *nrm() { return _nrm; }
...
struct MeshTris // Mesh Triangles
{
    ...
    VecI *ind() { return _ind; }
    VecI &ind(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _ind[i];
    } // get i-th triangle vertex indexes
    C VecI *ind() C { return _ind; }
    C VecI &ind(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _ind[i];
    } // get i-th triangle vertex indexes
    VecI *adjFace() { return _adj_face; }
    VecI &adjFace(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _adj_face[i];
    } // get i-th triangle adjacent faces, index to faces adjacent to the triangle encoded in following way : if(adj_face==-1) -> no face, else if(adj_face&SIGN_BIT)adj_quad_index=adj_face^SIGN_BIT, else adj_tri_index=adj_face
    C VecI *adjFace() C { return _adj_face; }
    C VecI &adjFace(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _adj_face[i];
    } // get i-th triangle adjacent faces, index to faces adjacent to the triangle encoded in following way : if(adj_face==-1) -> no face, else if(adj_face&SIGN_BIT)adj_quad_index=adj_face^SIGN_BIT, else adj_tri_index=adj_face
    VecI *adjEdge() { return _adj_edge; }
    VecI &adjEdge(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _adj_edge[i];
    } // get i-th triangle adjacent edges, index to edges adjacent to the triangle encoded in following way : if(adj_edge==-1) -> no edge, else adj_edge_index=adj_edge
    C VecI *adjEdge() C { return _adj_edge; }
    C VecI &adjEdge(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _adj_edge[i];
    } // get i-th triangle adjacent edges, index to edges adjacent to the triangle encoded in following way : if(adj_edge==-1) -> no edge, else adj_edge_index=adj_edge
    Vec *nrm() { return _nrm; }
...
struct MeshQuads // Mesh Quads
{
    ...
    VecI4 *ind() { return _ind; }
    VecI4 &ind(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _ind[i];
    } // get i-th quad vertex indexes
    C VecI4 *ind() C { return _ind; }
    C VecI4 &ind(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _ind[i];
    } // get i-th quad vertex indexes
    VecI4 *adjFace() { return _adj_face; }
    VecI4 &adjFace(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _adj_face[i];
    } // get i-th quad adjacent faces, index to faces adjacent to the quad encoded in following way : if(adj_face==-1) -> no face, else if(adj_face&SIGN_BIT)adj_quad_index=adj_face^SIGN_BIT, else adj_tri_index=adj_face
    C VecI4 *adjFace() C { return _adj_face; }
    C VecI4 &adjFace(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _adj_face[i];
    } // get i-th quad adjacent faces, index to faces adjacent to the quad encoded in following way : if(adj_face==-1) -> no face, else if(adj_face&SIGN_BIT)adj_quad_index=adj_face^SIGN_BIT, else adj_tri_index=adj_face
    VecI4 *adjEdge() { return _adj_edge; }
    VecI4 &adjEdge(Int i) {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _adj_edge[i];
    } // get i-th quad adjacent edges, index to edges adjacent to the quad encoded in following way : if(adj_edge==-1) -> no edge, else adj_edge_index=adj_edge
    C VecI4 *adjEdge() C { return _adj_edge; }
    C VecI4 &adjEdge(Int i) C {
        DEBUG_RANGE_ASSERT(i, _elms);
        return _adj_edge[i];
    } // get i-th quad adjacent edges, index to edges adjacent to the quad encoded in following way : if(adj_edge==-1) -> no edge, else adj_edge_index=adj_edge
    Vec *nrm() { return _nrm; }
...
#endif
    Int _elms;
    VecI4 *_ind, *_adj_face, *_adj_edge;
    Vec *_nrm;
    Byte *_flag;
    Int *_id;
...
struct MeshBase // Mesh Base (the most low level software mesh, contains : Vertexes + Edges + Triangles + Quads)
{
    ...
    void copyVtxs(C MeshBase &src);
    void copyVtxs(C MeshBase &src, C CMemPtr<Bool> &is);
    void copyEdges(C MeshBase &src);
    void copyEdges(C MeshBase &src, C CMemPtr<Bool> &is);
    void copyTris(C MeshBase &src);
    void copyTris(C MeshBase &src, C CMemPtr<Bool> &is);
    void copyQuads(C MeshBase &src);
    void copyQuads(C MeshBase &src, C CMemPtr<Bool> &is);
#if PHYSX
