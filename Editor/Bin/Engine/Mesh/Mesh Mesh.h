...
struct Mesh : MeshLod // Mesh (array of Mesh LODs)
{
    ...
    // variations
    Mesh &variations(Int variations);
    Int variations() C; // set/get number of material variations (meshes always have at least 1 material variation)
    Mesh &variationName(Int variation, C Str8 &name);
    CChar8 *variationName(Int i) C;                    // set/get           material variation name (first material variation always has name = null and it can't be changed)
