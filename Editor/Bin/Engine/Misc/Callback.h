...
struct CallbackUser {
    ...
    Bool is() C { return func != null; }
    void set(void func(Ptr user), Ptr user = null) {
        T.func = func;
        T.user = user;
    }
    void clear() { set(null); }
...
struct Callbacks {
    ...
    T1(TYPE)
    void add(void func(TYPE *user), TYPE *user = null) { add((void (*)(Ptr))func, user); } // add custom function to the queue, it will be called in the 'update' method
    T1(TYPE)
    void add(void func(TYPE &user), TYPE &user) { add((void (*)(Ptr))func, &user); } // add custom function to the queue, it will be called in the 'update' method
    ...
    T1(TYPE)
    void include(void func(TYPE *user), TYPE *user = null) { include((void (*)(Ptr))func, user); } // add custom function to the queue if it's not listed yet, it will be called in the 'update' method
    T1(TYPE)
    void include(void func(TYPE &user), TYPE &user) { include((void (*)(Ptr))func, &user); } // add custom function to the queue if it's not listed yet, it will be called in the 'update' method
    ...
    T1(TYPE)
    void exclude(void func(TYPE *user), TYPE *user = null) { exclude((void (*)(Ptr))func, user); } // remove function from the queue
    T1(TYPE)
    void exclude(void func(TYPE &user), TYPE &user) { exclude((void (*)(Ptr))func, &user); } // remove function from the queue
