...
struct MemberDesc // Member Description - Description of a Member in a C++ class
{
    ...
    T1(TYPE)
    MemberDesc &setDataToTextFunc(Str (*data_to_text)(C TYPE &object)) {
        T._data_to_text = (Str(*)(CPtr object))data_to_text;
        return T;
    ...
    T1(TYPE)
    MemberDesc &setFunc(Str (*data_to_text)(C TYPE &object), void (*text_to_data)(TYPE &object, C Str &text)) { return setDataToTextFunc(data_to_text).setTextToDataFunc(text_to_data); } // set              custom functions

    T1(TYPE)
    MemberDesc &setCompareFunc(Int (*compare)(C TYPE &object0, C TYPE &object1)) {
        T._compare = (Int(*)(CPtr object0, CPtr object1))compare;
        return T;
    ...
    T1(TYPE)
    TYPE &as(Ptr object) C { return *(TYPE *)((Byte *)object + offset); } // get reference to member, 'object'=pointer to C++ object of class that this member belongs to !! specified TYPE should be the exact same type of the class member !!
    ...
    T1(TYPE)
    MemberDesc(TYPE &member) { set(member); }     // create using auto-detection, 'member'=custom structure/class member - 'MEMBER' macro usage is required
    MemberDesc(MemberDesc &md) { Copy(T, md); }   // create as copy of 'md'
    MemberDesc(C MemberDesc &md) { Copy(T, md); } // create as copy of 'md'

    // manual setting
    MemberDesc &setType(DATA_TYPE type) {
        T.type = type;
        return T;
    }
    ...
    T1(TYPE)
    MemberDesc &set(TYPE &member) { return setDefault(DATA_NONE, member); } // 'member'=custom structure/class member - 'MEMBER' macro usage is required
    template <Int elms>
    MemberDesc &set(Char (&member)[elms]) { return setDefault(DATA_CHAR, member); }
    ...
    template <Int elms>
    MemberDesc &set(Char8 (&member)[elms]) { return setDefault(DATA_CHAR8, member); }
  ...
  private:
    ...
    T1(TYPE)
    MemberDesc &setDefault(DATA_TYPE type, TYPE &member) { return set(type, UInt(UIntPtr(&member)), SIZE(member)); }
};
/******************************************************************************/
#if EE_PRIVATE
