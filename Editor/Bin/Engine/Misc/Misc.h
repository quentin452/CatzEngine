...
T2(TYPE, RET_TYPE)
constexpr ENABLE_IF_ENUM(TYPE, RET_TYPE) Unsigned(TYPE x) { return Unsigned(ENUM_TYPE(TYPE)(x)); }
/******************************************************************************/
template <typename TYPE, Int elms>
constexpr Int Elms(C TYPE (&Array)[elms]) { return elms; } // get number of elements in array
...
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(SByte i, TYPE elms) { return InRange(i, ENUM_TYPE(TYPE)(elms)); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(Byte i, TYPE elms) { return InRange(i, ENUM_TYPE(TYPE)(elms)); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(Short i, TYPE elms) { return InRange(i, ENUM_TYPE(TYPE)(elms)); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(UShort i, TYPE elms) { return InRange(i, ENUM_TYPE(TYPE)(elms)); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(Int i, TYPE elms) { return InRange(i, ENUM_TYPE(TYPE)(elms)); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(UInt i, TYPE elms) { return InRange(i, ENUM_TYPE(TYPE)(elms)); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(Long i, TYPE elms) { return InRange(i, ENUM_TYPE(TYPE)(elms)); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(ULong i, TYPE elms) { return InRange(i, ENUM_TYPE(TYPE)(elms)); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(TYPE i, SByte elms) { return InRange(ENUM_TYPE(TYPE)(i), elms); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(TYPE i, Byte elms) { return InRange(ENUM_TYPE(TYPE)(i), elms); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(TYPE i, Short elms) { return InRange(ENUM_TYPE(TYPE)(i), elms); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(TYPE i, UShort elms) { return InRange(ENUM_TYPE(TYPE)(i), elms); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(TYPE i, Int elms) { return InRange(ENUM_TYPE(TYPE)(i), elms); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(TYPE i, UInt elms) { return InRange(ENUM_TYPE(TYPE)(i), elms); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(TYPE i, Long elms) { return InRange(ENUM_TYPE(TYPE)(i), elms); } // template specialization for enums
T1(TYPE)
constexpr ENABLE_IF_ENUM(TYPE, Bool) InRange(TYPE i, ULong elms) { return InRange(ENUM_TYPE(TYPE)(i), elms); } // template specialization for enums
T1(TYPE)
...
T2(TA, TB)
inline Bool FlagOn(TA flags, TB f) { return (flags & f) != 0; } // check if 'f' flag is  enabled               in 'flags', in case 'f' contains multiple options then this will succeed if any of them are  enabled
T2(TA, TB)
inline Bool FlagAll(TA flags, TB f) { return (flags & f) == f; } // check if 'f' flag is  enabled               in 'flags', in case 'f' contains multiple options then this will succeed if all of them are  enabled
T2(TA, TB)
inline Bool FlagOff(TA flags, TB f) { return (flags & f) == 0; } // check if 'f' flag is disabled               in 'flags', in case 'f' contains multiple options then this will succeed if all of them are disabled
T2(TA, TB)
inline void FlagEnable(TA &flags, TB f) { flags |= f; } // enable   'f' flag                           in 'flags'
T2(TA, TB)
inline void FlagDisable(TA &flags, TB f) { flags &= ~f; } // disable  'f' flag                           in 'flags'
T2(TA, TB)
inline void FlagToggle(TA &flags, TB f) { flags ^= f; } // toggle   'f' flag                           in 'flags'
T2(TA, TB)
inline void FlagSet(TA &flags, TB f, Bool on) {
    if (on)
        FlagEnable(flags, f);
    else
        FlagDisable(flags, f);
} // set      'f' flag to be enabled or disabled in 'flags'
T3(TA, TB, TC)
inline void FlagCopy(TA &flags, TB f, TC mask) { flags = (flags & ~mask) | (f & mask); } // copy     'f' flags                          to 'flags' using specified mask
...
Bool IsDrive(CChar8 *path); // if path is a     drive  path, sample usage:  IsDrive ("x:/"     ) -> true,  IsDrive("x:/data/0.bmp") -> false,  IsDrive("data/0.bmp") -> false
Bool HasDrive(CChar *path);
Bool HasDrive(CChar8 *path); // if path contains drive  path, sample usage: HasDrive ("x:/"     ) -> true, HasDrive("x:/data/0.bmp") -> true , HasDrive("data/0.bmp") -> false
Bool HasRemote(CChar *path);
Bool HasRemote(CChar8 *path); // if path contains remote path, sample usage: HasRemote("\\\\comp") -> true
Bool FullPath(CChar *path);
Bool FullPath(CChar8 *path); // if path is a full path (has a drive or is remote)

T3(TA, TB, TC)
inline TA &Clamp(TA &x, TB min, TC max) {
    if (x < min)
        x = min;
    else if (x > max)
        x = max;
...
constexpr Int Mod(Int x, Int y) {
    ...
    Int z = x % y;
    return (z >= 0) ? z : z + y;
} // safe modulo "x%y" , returns always a positive number between   "0..y-1"
...
void LogName(C Str &name);
Str LogName();                                                     // set/get name for the log file, default="log.txt" (null on Mobile platforms), specifying an empty file name prevents any writing to the file
...
T1(TYPE)
Str EncodeFileName(C TYPE &elm) { return EncodeFileName(&elm, SIZE(elm)); }
T1(TYPE)
void EncodeFileName(Str &dest, C TYPE &elm) { return EncodeFileName(dest, &elm, SIZE(elm)); }
T1(TYPE)
Bool DecodeFileName(C Str &src, TYPE &elm) { return DecodeFileName(src, &elm, SIZE(elm)); }
...
T1(TYPE)
Str EncodeRaw(C TYPE &elm) { return EncodeRaw(&elm, SIZE(elm)); }
T1(TYPE)
void EncodeRaw(Str &dest, C TYPE &elm) { return EncodeRaw(dest, &elm, SIZE(elm)); }
T1(TYPE)
void EncodeRaw(Str8 &dest, C TYPE &elm) { return EncodeRaw(dest, &elm, SIZE(elm)); }
T1(TYPE)
Bool DecodeRaw(C Str &src, TYPE &elm) { return DecodeRaw(src, &elm, SIZE(elm)); }
T1(TYPE)
Bool DecodeRaw(C Str8 &src, TYPE &elm) { return DecodeRaw(src, &elm, SIZE(elm)); }
...
inline Int Compare(C IndexWeight &a, C IndexWeight &b) {
    if (Int c = Compare(b.weight, a.weight))
        return c;                     // first compare by weight, reverse order to list most important first
...
/******************************************************************************/
T2(BASE, EXTENDED)
void ASSERT_BASE_EXTENDED_EX(BASE &, EXTENDED &) {
    BASE *t = (EXTENDED *)null;
    (void)t;
    DYNAMIC_ASSERT(static_cast<BASE *>((EXTENDED *)256) == (BASE *)256, "Selected class is not the base class of extended one");
} // "BASE *t=(EXTENDED*)null;" needed for "ASSERT_BASE_EXTENDED<EXTENDED, BASE>();"
T2(BASE, EXTENDED)
void ASSERT_BASE_EXTENDED_EX(BASE *, EXTENDED *) {
    BASE *t = (EXTENDED *)null;
    (void)t;
...
void ASSERT_BASE_EXTENDED() {
    int i = 0;
    ASSERT_BASE_EXTENDED_EX((BASE &)i, (EXTENDED &)i);
} // asserts that 'BASE' class is the main base class of 'EXTENDED', this also works for pointers

T1(TYPE)
CPtr CType() { return (CPtr) & typeid(TYPE); } // convert C++ type into pointer
T1(TYPE)
CPtr CType(TYPE &x) { return (CPtr) & typeid(x); } // convert C++ type into pointer

#if EE_PRIVATE
#if WINDOWS
T1(TYPE)
Bool IsVirtual() { return std::is_polymorphic<TYPE>(); }
T1(TYPE)
Bool IsVirtual(C TYPE &x) { return std::is_polymorphic<TYPE>(); }
#endif
#endif
/******************************************************************************/
struct Notification {
...
struct DataRangeRel {
    ...
    void set(Long start, Long size) {
        T.start = start;
        T.size = size;
    }
    void setAbs(Long start, Long end) {
        T.start = start;
        T.size = end - start;
    }

    void moveStart(Long delta) {
        start += delta;
        size -= delta;
    }
    void moveStartTo(Long pos) {
        size += start - pos;
        start = pos;
    }
};
