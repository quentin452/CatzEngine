...
struct Object // Object Parameters - they're created in the Editor and are used for creating typical Game Objects 'Game.Obj'
{
    ...
    void base(C ObjectPtr &base);
    C ObjectPtr &base() C { return _base; } // set/get base object
    void type(Bool custom, C UID &type = UIDZero);
    C UID &type() C { return _type; } // set/get object type        (OBJ_TYPE 'ObjType' enum element ID), 'custom'=if override base defaults and set custom value (when set to true then custom value will be used, if set to false then base value will be used)
    void access(Bool custom, OBJ_ACCESS access = OBJ_ACCESS_TERRAIN);
    OBJ_ACCESS access() C { return _access; } // set/get object access mode (OBJ_ACCESS                        ), 'custom'=if override base defaults and set custom value (when set to true then custom value will be used, if set to false then base value will be used)
    void path(Bool custom, OBJ_PATH path = OBJ_PATH_CREATE);
    OBJ_PATH path() C { return _path; } // set/get object path   mode (OBJ_PATH                          ), 'custom'=if override base defaults and set custom value (when set to true then custom value will be used, if set to false then base value will be used)
    void mesh(Bool custom, C MeshPtr &mesh = null);
    C MeshPtr &mesh() C { return _mesh; } // set/get mesh                                                   , 'custom'=if override base defaults and set custom value (when set to true then custom value will be used, if set to false then base value will be used)
    void meshVariationID(Bool custom, UInt id = 0);
    UInt meshVariationID() C { return _mesh_variation_id; } // set/get mesh                          variation ID             , 'custom'=if override base defaults and set custom value (when set to true then custom value will be used, if set to false then base value will be used), based on this value, 'meshVariationIndex' can be obtained. This ID is the unique ID of the mesh variation - 'Mesh.variationID'. It should not be passed to the 'SetVariation' function, instead please use 'meshVariationIndex' for that.
    ...
    void phys(Bool custom, C PhysBodyPtr &phys = null);
    C PhysBodyPtr &phys() C { return _phys; } // set/get physical body                                          , 'custom'=if override base defaults and set custom value (when set to true then custom value will be used, if set to then false base value will be used)
    void constant(Bool custom, Bool on = false);
    Bool constant() C; // set/get object constant mode                                   , 'custom'=if override base defaults and set custom value (when set to true then custom value will be used, if set to then false base value will be used), this is special modifier for OBJ_ACCESS_CUSTOM access specifying that the object is always constant (never changes) and should not be saved in SaveGame, but should always created as new object from its default parameters (this reduces SaveGame size)
    void scale(Bool custom, C Vec &scale = 1);
    Flt scale() C;  // set/get object scale                                           , 'custom'=if override base defaults and set custom value (when set to true then custom value will be used, if set to then false base value will be used)
