...
struct PathFind // 2D Map Path Finder
{
    ...
    // set
    UInt pixelFlag(Int x, Int y) C;
    PathFind &pixelFlag(Int x, Int y, Byte flag); // get/set    map pixel flag PATH_FIND_PIXEL_FLAG
    Bool pixelWalkable(Int x, Int y) C;
    PathFind &pixelWalkable(Int x, Int y, Bool on); // get/set if map pixel is   walkable
    Bool pixelTarget(Int x, Int y) C;
    PathFind &pixelTarget(Int x, Int y, Bool on); // get/set if map pixel is a destination target by marking it with PFP_TARGET flag
    Bool pixelStart(Int x, Int y) C;
    PathFind &pixelStart(Int x, Int y, Bool on); // get/set if map pixel is a beginning          by marking it with PFP_START  flag
