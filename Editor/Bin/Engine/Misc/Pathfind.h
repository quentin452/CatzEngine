...
struct PathSettings {
    ...
    PathSettings &areaSize(Flt size); // get/set area size         (in meters ), 0..Inf , default=32
    Flt ctrlRadius() C { return _ctrl_r; }
    PathSettings &ctrlRadius(Flt r); // get/set controller radius (in meters ), 0..Inf , default=0.33f
    Flt ctrlHeight() C { return _ctrl_h; }
    PathSettings &ctrlHeight(Flt h); // get/set controller height (in meters ), 0..Inf , default=2.0f
    Flt maxClimb() C { return _max_climb; }
    PathSettings &maxClimb(Flt climb); // get/set max climb  height (in meters ), 0..Inf , default=0.7f
    Flt maxSlope() C { return _max_slope; }
    PathSettings &maxSlope(Flt slope); // get/set max slope  angle  (in radians), 0..PI_2, default=PI_4
    Flt cellSize() C { return _cell_size; }
    PathSettings &cellSize(Flt size); // get/set path cell  size   (in meters ), 0..Inf , default=1.0f/3
    Flt cellHeight() C { return _cell_h; }
    PathSettings &cellHeight(Flt h); // get/set path cell  height (in meters ), 0..Inf , default=0.1f
...
struct RecastCompactHeightfield : rcCompactHeightfield {
    ...
    RecastCompactHeightfield(C RecastCompactHeightfield &src) {
        zero();
        T = src;
    }
};
