...
struct SyncLockerEx // Synchronization Locker Extended
{
    ...
    } // set if lock should be enabled

    Bool tryOn() {
        if (!_on && _lock.tryOn())
            _on = true;
        return _on;
    } // try entering locking, false on fail

    explicit SyncLockerEx(C SyncLock &lock, Bool on = true) : _lock(lock) {
        if (_on = on)
            _lock.on();
    }
    ~SyncLockerEx() {
...
struct SafeSyncLocker {
    explicit SafeSyncLocker(C SyncLock &lock) {
        if (lock.created()) {
            _lock = &lock;
            lock.on();
        } else
...
struct SafeSyncLockerEx {
    ...
    Bool tryOn() {
        if (!_on && _lock && _lock->tryOn())
            _on = true;
        return _on;
    } // try entering locking, false on fail

    explicit SafeSyncLockerEx(C SyncLock &lock, Bool on = true) {
        if (lock.created()) {
            _lock = &lock;
            if (_on = on)
                lock.on();
...
struct SyncUnlocker {
    explicit SyncUnlocker(C SyncLock &lock) : _lock(lock) {
        if (_owned = lock.owned())
            _lock.off();
    }
    ~SyncUnlocker() {
        if (_owned)
...
struct SyncEvent // Synchronization Event (multi-threaded safe)
{
  ...
  private:
#if EE_PRIVATE
    PLATFORM(HANDLE, pthread_cond_t *)
    _handle;
#else
    Ptr _handle;
#endif
...
struct WriteLockEx // 'ReadWriteSync' Write Lock Extended
{
    ...
    explicit WriteLockEx(ReadWriteSync &lock, Bool on = true) : _lock(lock) {
        if (_on = on)
            lock.enterWrite();
    }
    ~WriteLockEx() { off(); }
...
struct SafeWriteLock {
    explicit SafeWriteLock(ReadWriteSync &lock) {
        if (lock.created()) {
            _lock = &lock;
            lock.enterWrite();
        } else
...
struct SimpleReadLockEx // SimpleReadWriteSync Read Lock Extended
{
    ...
    explicit SimpleReadLockEx(C SimpleReadWriteSync &lock, Bool on = true) : _lock(lock) {
        if (_on = on)
            lock.enterRead();
    }
...
struct SimpleWriteLockEx // SimpleReadWriteSync Write Lock Extended
{
    ...
    explicit SimpleWriteLockEx(C SimpleReadWriteSync &lock, Bool on = true) : _lock(lock) {
        if (_on = on)
            lock.enterWrite();
    }
...
const_mem_addr struct Threads // Worker Threads, allow to process data on multiple threads !! must be stored in constant memory address !!
{
    struct Call {
        ...
        T1(DATA)
        void set(DATA &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null) { _set((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
        T2(DATA, USER_DATA)
        void set(DATA &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null) { _set((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
        T2(DATA, USER_DATA)
        void set(DATA &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user) { _set((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }

        void _set(Ptr data, void func(Ptr data, Ptr user, Int thread_index), Ptr user = null) {
            T.data = data;
            T.user = user;
            T.func = func;
        }
    ...
    T1(USER_DATA)
    void process(Int elms, void func(IntPtr elm_index, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(elms, (void (*)(IntPtr elm_index, Ptr user, Int thread_index))func, user, max_threads, false); }
    T1(USER_DATA)
    void process(Int elms, void func(IntPtr elm_index, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(elms, (void (*)(IntPtr elm_index, Ptr user, Int thread_index))func, &user, max_threads, false); }
    ...
    T1(DATA)
    void process(Mems<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, false); }
    T1(DATA)
    void process(Mems<DATA *> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, true); }
    T1(DATA)
    void process(Memc<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, false); }
    T1(DATA)
    void process(Memc<DATA *> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, true); }
    T1(DATA)
    void process(Memt<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, false); }
    T1(DATA)
    void process(Memt<DATA *> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, true); }
    T1(DATA)
    void process(Memb<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false); }
    T1(DATA)
    void process(Memx<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false); }

    T2(DATA, USER_DATA)
    void process(Mems<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, false); }
    T2(DATA, USER_DATA)
    void process(Mems<DATA *> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, true); }
    T2(DATA, USER_DATA)
    void process(Memc<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, false); }
    T2(DATA, USER_DATA)
    void process(Memc<DATA *> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, true); }
    T2(DATA, USER_DATA)
    void process(Memt<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, false); }
    T2(DATA, USER_DATA)
    void process(Memt<DATA *> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false, true); }
    T2(DATA, USER_DATA)
    void process(Memb<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false); }
    T2(DATA, USER_DATA)
    void process(Memx<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, false); }

    T2(DATA, USER_DATA)
    void process(Mems<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, false, false); }
    T2(DATA, USER_DATA)
    void process(Mems<DATA *> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, false, true); }
    T2(DATA, USER_DATA)
    void process(Memc<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, false, false); }
    T2(DATA, USER_DATA)
    void process(Memc<DATA *> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, false, true); }
    T2(DATA, USER_DATA)
    void process(Memt<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, false, false); }
    T2(DATA, USER_DATA)
    void process(Memt<DATA *> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, false, true); }
    T2(DATA, USER_DATA)
    void process(Memb<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, false); }
    T2(DATA, USER_DATA)
    void process(Memx<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, false); }
    ...
    T1(USER_DATA)
    void process1(Int elms, void func(IntPtr elm_index, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(elms, (void (*)(IntPtr elm_index, Ptr user, Int thread_index))func, user, max_threads, true); }
    T1(USER_DATA)
    void process1(Int elms, void func(IntPtr elm_index, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(elms, (void (*)(IntPtr elm_index, Ptr user, Int thread_index))func, &user, max_threads, true); }

    // perform multi-threaded call on 'func' function, by giving a unique 'data' element from the memory container as the function parameter, this function will return only after all calls have been processed, 'max_threads'=max threads to wake up and perform processing (threads that are already awake may also do processing) -1=default value which is 'threads', 'thread_index' will always be 0..'threads' (inclusive), which means if you're using per-thread data, then make sure to allocate 'threads'+1 per-thread data (use 'threads1' method to get the number of elements), this method is preferred instead of 'process' offering better performance, however caution must be taken to allocate 1 extra per-thread data
    T1(DATA)
    void process1(Mems<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, false); }
    T1(DATA)
    void process1(Mems<DATA *> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, true); }
    T1(DATA)
    void process1(Memc<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, false); }
    T1(DATA)
    void process1(Memc<DATA *> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, true); }
    T1(DATA)
    void process1(Memt<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, false); }
    T1(DATA)
    void process1(Memt<DATA *> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, true); }
    T1(DATA)
    void process1(Memb<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true); }
    T1(DATA)
    void process1(Memx<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true); }

    T2(DATA, USER_DATA)
    void process1(Mems<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, false); }
    T2(DATA, USER_DATA)
    void process1(Mems<DATA *> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, true); }
    T2(DATA, USER_DATA)
    void process1(Memc<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, false); }
    T2(DATA, USER_DATA)
    void process1(Memc<DATA *> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, true); }
    T2(DATA, USER_DATA)
    void process1(Memt<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, false); }
    T2(DATA, USER_DATA)
    void process1(Memt<DATA *> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true, true); }
    T2(DATA, USER_DATA)
    void process1(Memb<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true); }
    T2(DATA, USER_DATA)
    void process1(Memx<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, max_threads, true); }

    T2(DATA, USER_DATA)
    void process1(Mems<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, true, false); }
    T2(DATA, USER_DATA)
    void process1(Mems<DATA *> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, true, true); }
    T2(DATA, USER_DATA)
    void process1(Memc<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, true, false); }
    T2(DATA, USER_DATA)
    void process1(Memc<DATA *> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, true, true); }
    T2(DATA, USER_DATA)
    void process1(Memt<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, true, false); }
    T2(DATA, USER_DATA)
    void process1(Memt<DATA *> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, true, true); }
    T2(DATA, USER_DATA)
    void process1(Memb<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, true); }
    T2(DATA, USER_DATA)
    void process1(Memx<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int max_threads = -1) { _process(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, max_threads, true); }
    ...
    T1(USER_DATA)
    void queue(Int elms, void func(IntPtr elm_index, USER_DATA *user, Int thread_index), USER_DATA *user = null) { _queue(elms, (void (*)(IntPtr elm_index, Ptr user, Int thread_index))func, user); }
    T1(USER_DATA)
    void queue(Int elms, void func(IntPtr elm_index, USER_DATA &user, Int thread_index), USER_DATA &user) { _queue(elms, (void (*)(IntPtr elm_index, Ptr user, Int thread_index))func, &user); }
    ...
    T1(DATA)
    void queue(DATA &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null) { _queue((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    void queue(DATA &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null) { _queue((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    void queue(DATA &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user) { _queue((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }
    ...
    T1(USER_DATA)
    Int cancelFuncUser(void func(IntPtr elm_index, USER_DATA *user, Int thread_index), USER_DATA *user = null) { return _cancel((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T1(USER_DATA)
    Int cancelFuncUser(void func(IntPtr elm_index, USER_DATA &user, Int thread_index), USER_DATA &user) { return _cancel((void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }
    ...
    T1(USER_DATA)
    Int cancelFunc(void func(IntPtr elm_index, USER_DATA *user, Int thread_index)) { return _cancel((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T1(USER_DATA)
    Int cancelFunc(void func(IntPtr elm_index, USER_DATA &user, Int thread_index)) { return _cancel((void (*)(Ptr data, Ptr user, Int thread_index))func); }

    T1(DATA)
    Int cancel(DATA &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null) { return _cancel((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Int cancel(DATA &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null) { return _cancel((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Int cancel(DATA &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user) { return _cancel((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }

    T1(DATA)
    Int cancelFuncUser(void func(DATA &data, Ptr user, Int thread_index), Ptr user = null) { return _cancel((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Int cancelFuncUser(void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null) { return _cancel((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Int cancelFuncUser(void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user) { return _cancel((void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }

    T1(DATA)
    Int cancelFunc(void func(DATA &data, Ptr user, Int thread_index)) { return _cancel((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T2(DATA, USER_DATA)
    Int cancelFunc(void func(DATA &data, USER_DATA *user, Int thread_index)) { return _cancel((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T2(DATA, USER_DATA)
    Int cancelFunc(void func(DATA &data, USER_DATA &user, Int thread_index)) { return _cancel((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    ...
    T1(USER_DATA)
    void waitFuncUser(void func(IntPtr elm_index, USER_DATA *user, Int thread_index), USER_DATA *user = null) { _wait((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T1(USER_DATA)
    void waitFuncUser(void func(IntPtr elm_index, USER_DATA &user, Int thread_index), USER_DATA &user) { _wait((void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }

    void waitFunc(void func(IntPtr elm_index, Ptr user, Int thread_index)) { _wait((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T1(USER_DATA)
    void waitFunc(void func(IntPtr elm_index, USER_DATA *user, Int thread_index)) { _wait((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T1(USER_DATA)
    void waitFunc(void func(IntPtr elm_index, USER_DATA &user, Int thread_index)) { _wait((void (*)(Ptr data, Ptr user, Int thread_index))func); }

    T1(DATA)
    void wait(DATA &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null) { _wait((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    void wait(DATA &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null) { _wait((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    void wait(DATA &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user) { _wait((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }

    T1(DATA)
    void waitFuncUser(void func(DATA &data, Ptr user, Int thread_index), Ptr user = null) { _wait((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    void waitFuncUser(void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null) { _wait((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    void waitFuncUser(void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user) { _wait((void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }

    T1(DATA)
    void waitFunc(void func(DATA &data, Ptr user, Int thread_index)) { _wait((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T2(DATA, USER_DATA)
    void waitFunc(void func(DATA &data, USER_DATA *user, Int thread_index)) { _wait((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T2(DATA, USER_DATA)
    void waitFunc(void func(DATA &data, USER_DATA &user, Int thread_index)) { _wait((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    ...
    T1(USER_DATA)
    Int queuedFuncUser(void func(IntPtr elm_index, USER_DATA *user, Int thread_index), USER_DATA *user = null) { return _queued((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T1(USER_DATA)
    Int queuedFuncUser(void func(IntPtr elm_index, USER_DATA &user, Int thread_index), USER_DATA &user) { return _queued((void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }
    ...
    T1(USER_DATA)
    Int queuedFunc(void func(IntPtr elm_index, USER_DATA *user, Int thread_index)) { return _queued((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T1(USER_DATA)
    Int queuedFunc(void func(IntPtr elm_index, USER_DATA &user, Int thread_index)) { return _queued((void (*)(Ptr data, Ptr user, Int thread_index))func); }

    T1(DATA)
    Int queued(DATA &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null) C { return _queued((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Int queued(DATA &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null) C { return _queued((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Int queued(DATA &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user) C { return _queued((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }

    T1(DATA)
    Int queuedFuncUser(void func(DATA &data, Ptr user, Int thread_index), Ptr user = null) C { return _queued((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Int queuedFuncUser(void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null) C { return _queued((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Int queuedFuncUser(void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user) C { return _queued((void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }

    T1(DATA)
    Int queuedFunc(void func(DATA &data, Ptr user, Int thread_index)) C { return _queued((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T2(DATA, USER_DATA)
    Int queuedFunc(void func(DATA &data, USER_DATA *user, Int thread_index)) C { return _queued((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T2(DATA, USER_DATA)
    Int queuedFunc(void func(DATA &data, USER_DATA &user, Int thread_index)) C { return _queued((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    ...
    T1(USER_DATA)
    Bool busyFuncUser(void func(IntPtr elm_index, USER_DATA *user, Int thread_index), USER_DATA *user = null) { return _busy((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T1(USER_DATA)
    Bool busyFuncUser(void func(IntPtr elm_index, USER_DATA &user, Int thread_index), USER_DATA &user) { return _busy((void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }
    ...
    T1(USER_DATA)
    Bool busyFunc(void func(IntPtr elm_index, USER_DATA *user, Int thread_index)) { return _busy((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T1(USER_DATA)
    Bool busyFunc(void func(IntPtr elm_index, USER_DATA &user, Int thread_index)) { return _busy((void (*)(Ptr data, Ptr user, Int thread_index))func); }

    T1(DATA)
    Bool busy(DATA &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null) C { return _busy((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Bool busy(DATA &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null) C { return _busy((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Bool busy(DATA &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user) C { return _busy((Ptr)&data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }

    T1(DATA)
    Bool busyFuncUser(void func(DATA &data, Ptr user, Int thread_index), Ptr user = null) C { return _busy((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Bool busyFuncUser(void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null) C { return _busy((void (*)(Ptr data, Ptr user, Int thread_index))func, user); }
    T2(DATA, USER_DATA)
    Bool busyFuncUser(void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user) C { return _busy((void (*)(Ptr data, Ptr user, Int thread_index))func, &user); }

    T1(DATA)
    Bool busyFunc(void func(DATA &data, Ptr user, Int thread_index)) C { return _busy((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T2(DATA, USER_DATA)
    Bool busyFunc(void func(DATA &data, USER_DATA *user, Int thread_index)) C { return _busy((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    T2(DATA, USER_DATA)
    Bool busyFunc(void func(DATA &data, USER_DATA &user, Int thread_index)) C { return _busy((void (*)(Ptr data, Ptr user, Int thread_index))func); }
    ...
    Int activeThreads() C;
    Threads &activeThreads(Int active); // get/set how many threads should be active (remaining threads will be paused)
    Int priority() C;
    Threads &priority(Int priority); // get/set threads priority, 'priority'=-3..3
...
struct ConsoleProcess // allows running console processes and reading their output
{
    ...
    ~ConsoleProcess() {
        del();
        _thread.del();
    } // make sure 'thread' is deleted before other members
    ConsoleProcess() {
        _binary = _thread_exiting = _proc_exited = false;
        _exit_code = -1;
        _proc_id = 0;
        _proc = _out_read = _in_write = PLATFORM(null, 0);
    }
...
T1(TYPE)
TYPE *AtomicGetSet(TYPE *&x, TYPE *y) { return (TYPE *)AtomicGetSet((UIntPtr &)x, UIntPtr(y)); }
...
T1(TYPE)
Bool AtomicCAS(TYPE *&x, TYPE *compare, TYPE *new_value) { return AtomicCAS((UIntPtr &)x, UIntPtr(compare), UIntPtr(new_value)); }
...
T1(USER_DATA)
void MultiThreadedCall(Int elms, void func(Int elm_index, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int threads = Cpu.threads()) { MultiThreadedCall(elms, (void (*)(Int elm_index, Ptr user, Int thread_index))func, user, threads); }
T1(USER_DATA)
void MultiThreadedCall(Int elms, void func(Int elm_index, USER_DATA &user, Int thread_index), USER_DATA &user, Int threads = Cpu.threads()) { MultiThreadedCall(elms, (void (*)(Int elm_index, Ptr user, Int thread_index))func, &user, threads); }
...
T1(DATA)
void MultiThreadedCall(Mems<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, false); }
T1(DATA)
void MultiThreadedCall(Mems<DATA *> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, true); }
T1(DATA)
void MultiThreadedCall(Memc<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, false); }
T1(DATA)
void MultiThreadedCall(Memc<DATA *> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, true); }
T1(DATA)
void MultiThreadedCall(Memt<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, false); }
T1(DATA)
void MultiThreadedCall(Memt<DATA *> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, true); }
T1(DATA)
void MultiThreadedCall(Memb<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, false); }
T1(DATA)
void MultiThreadedCall(Memb<DATA *> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, true); }
T1(DATA)
void MultiThreadedCall(Memx<DATA> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, false); }
T1(DATA)
void MultiThreadedCall(Memx<DATA *> &data, void func(DATA &data, Ptr user, Int thread_index), Ptr user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, true); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Mems<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, false); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Mems<DATA *> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, true); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memc<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, false); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memc<DATA *> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, true); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memt<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, false); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memt<DATA *> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, true); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memb<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, false); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memb<DATA *> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, true); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memx<DATA> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, false); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memx<DATA *> &data, void func(DATA &data, USER_DATA *user, Int thread_index), USER_DATA *user = null, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, user, threads, true); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Mems<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, threads, false); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Mems<DATA *> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, threads, true); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memc<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, threads, false); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memc<DATA *> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, threads, true); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memt<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, threads, false); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memt<DATA *> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int threads = Cpu.threads()) { _MultiThreadedCall(data.data(), data.elms(), data.elmSize(), (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, threads, true); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memb<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, threads, false); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memb<DATA *> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, threads, true); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memx<DATA> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, threads, false); }
T2(DATA, USER_DATA)
void MultiThreadedCall(Memx<DATA *> &data, void func(DATA &data, USER_DATA &user, Int thread_index), USER_DATA &user, Int threads = Cpu.threads()) { _MultiThreadedCall(data, (void (*)(Ptr data, Ptr user, Int thread_index))func, &user, threads, true); }
