...
const_mem_addr struct Vehicle // Physical actor of vehicle type !! must be stored in constant memory address !!
{
    ...
    Flt accel() C;
    Vehicle &accel(Flt accel); // get/set           acceleration      , -1..1
    Flt brake() C;
    Vehicle &brake(Flt brake); // get/set current         brake       ,  0..1
    Flt angle() C;
    Vehicle &angle(Flt angle); // get/set current   steer angle       , -1..1
    Flt maxAccel() C;
    Vehicle &maxAccel(Flt accel); // get/set max       acceleration force,  0..Inf , default=10
    Flt maxBackAccel() C;
    Vehicle &maxBackAccel(Flt accel); // get/set max back  acceleration force,  0..Inf , default=4.5
    Flt maxBrake() C;
    Vehicle &maxBrake(Flt brake); // get/set max             brake  force,  0..Inf , default=12
    Flt brakeRatio() C;
    Vehicle &brakeRatio(Flt ratio); // get/set      front/rear brake  ratio,  0..1   , default=0.43, this parameter specifies how much of the braking goes into front wheels, rear wheels will have a value "1-ratio" (for example if front has 0.6, then rear will have 0.4)
    Flt maxAngle() C;
    Vehicle &maxAngle(Flt angle); // get/set max       steer angle       ,  0..PI_2, default=PI_3
    Flt frictionLinear() C;
    Vehicle &frictionLinear(Flt friction); // get/set         wheel linear fricion,  0..Inf , default=13  , this parameter specifies the linear portion of the friction when moving sideways, side velocity will be decreased by this value by    subtraction
    Flt frictionScalar() C;
    Vehicle &frictionScalar(Flt friction); // get/set         wheel scalar fricion,  0..1   , default=0.3 , this parameter specifies the scalar portion of the friction when moving sideways, side velocity will be decreased by this value by multiplication
    Flt suspSpring() C;
    Vehicle &suspSpring(Flt spring); // get/set   suspension spring strength,  0..Inf , default=20  , this parameter specifies bounciness strength of the suspension
    Flt suspDamping() C;
    Vehicle &suspDamping(Flt damping); // get/set   suspension spring damping ,  0..Inf , default=0.7 , this parameter specifies how quickly does the suspension stabilize
    Flt suspCompress() C;
    Vehicle &suspCompress(Flt compress); // get/set   suspension max compression,  0..1   , default=0.75 (1.0 means full wheel radius)
    WHEEL_DRIVE wheelDrive() C;
    Vehicle &wheelDrive(WHEEL_DRIVE wd); // get/set wheel drive                           , default=WHEEL_AWD

    Flt energy() C; // get     kinetic energy , 0..Inf
    Flt damping() C;
    Vehicle &damping(Flt damping); // get/set linear  damping, 0..Inf, default=0.05
    Flt adamping() C;
    Vehicle &adamping(Flt damping); // get/set angular damping, 0..Inf, default=0.05
    Flt mass() C;
    Vehicle &mass(Flt mass); // get/set mass           , 0..Inf
    Vec massCenterL() C;
    Vehicle &massCenterL(C Vec &center); // get/set mass center in actor local space
    Vec massCenterW() C;
    Vehicle &massCenterW(C Vec &center); // get/set mass center in world       space
    Vec inertia() C;
    Vehicle &inertia(C Vec &inertia); // get/set inertia tensor
    Vec pos() C;
    Vehicle &pos(C Vec &pos); // get/set position
    Matrix3 orn() C;
    Vehicle &orn(C Matrix3 &orn); // get/set orientation, 'orn'    must be normalized
    Matrix matrix() C;
    Vehicle &matrix(C Matrix &matrix); // get/set matrix     , 'matrix' must be normalized
    Vec vel() C;
    Vehicle &vel(C Vec &vel); // get/set         velocity
    Vec angVel() C;
    Vehicle &angVel(C Vec &vel);       // get/set angular velocity
    ...
    Bool gravity() C;
    Vehicle &gravity(Bool on); // get/set if gravity is enabled for this vehicle
    Bool ray() C;
    Vehicle &ray(Bool on); // get/set if this actor should be included when performing ray tests
    Bool collision() C;
    Vehicle &collision(Bool on); // get/set if this actor should collide with other actors in the world
    Bool sleep() C;
    Vehicle &sleep(Bool sleep); // get/set sleeping
    Flt sleepEnergy() C;
    Vehicle &sleepEnergy(Flt energy); // get/set the amount of energy below the actor is put to sleep, default=0.005
    Bool ccd() C;
    Vehicle &ccd(Bool on); // get/set continuous collision detection
    Ptr user() C;
    Vehicle &user(Ptr user); // get/set custom user data
    Ptr obj() C;
    Vehicle &obj(Ptr obj); // get/set pointer to object containing the actor
    Byte group() C;
    Vehicle &group(Byte group); // get/set collision group (0..31, default value is taken according to ACTOR_GROUP)
    Byte dominance() C;
    Vehicle &dominance(Byte dominance); // get/set dominance index (0..31, default=0), for more information about dominance please check comments on 'Physics.dominance' method
    PhysMtrl *material() C;
    Vehicle &material(PhysMtrl *material); // get/set physics material for vehicle (use 'null' for default material)
