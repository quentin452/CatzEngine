...
struct AppVolumeClass // Application Volume (in the system) Control
{
    ...
    Flt volume(); // set/get volume (0..1)
    void mute(Bool mute);
    Bool mute(); // set/get mute   (true/false)
...
#elif APPLE
#if EE_PRIVATE
    AudioComponentInstance _handle;
    ASSERT(SIZE(AudioComponentInstance) == SIZE(Ptr));
#else
    Ptr _handle;
#endif
    UInt _flags;
...
struct OpusEncoder {
    ...
    Int frameLength() C;
    OpusEncoder &frameLength(Int length); // get/set frame length in milliseconds, smaller values result in worse quality but smaller latency(delay), higher values result in better quality but higher latency(delay) (2 .. 60 range supported), however Opus provides best quality for OPUS_FRAME_SIZE_BEST_QUALITY
    Int bitRate() C;
    OpusEncoder &bitRate(Int bit_rate); // get/set encoding bit-rate in bits per second (500 .. 512,000 range supported)
    Bool vbr() C;
    OpusEncoder &vbr(Bool on); // get/set variable bit-rate (VBR), default=true
    Flt complexity() C;
    OpusEncoder &complexity(Flt complex); // get/set computational complexity (0..1, 0=fastest/low quality, 1=slowest/high quality)
#if EE_PRIVATE
    void reset();
    Int delay() C;
...
struct SndOpusEncoder // Opus Encoder into Esenthel SND file format
{
    ...
    Int bitRate() C;
    SndOpusEncoder &bitRate(Int bit_rate); // get/set encoding bit-rate in bits per second (500 .. 512,000 range supported)
...
struct SoundResampler {
    ...
    SoundResampler(Flt speed, Flt volume[2], Int dest_channels, Int dest_samples, Ptr dest_data, Int src_channels, C ResampleBuffer *buffer = null) : end(false), speed(speed), volume{volume[0], volume[1]},
        ...
        if (src_channels == 2 && dest_channels == 1)
            REPAO(T.volume) /= 2;
        if (buffer)
            T.buffer = *buffer;
        else
    ...
    void setSrc(Int src_samples, Ptr src_data) {
        T.src_samples = src_samples;
        T.src_data = src_data;
    }
