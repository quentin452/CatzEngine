...
struct SoundVolumeClass {
    void fx(Flt v);
    Flt fx() C { return _v[VOLUME_FX]; } // set/get fx      volume (0..1)
    void music(Flt v);
    Flt music() C { return _v[VOLUME_MUSIC]; } // set/get music   volume (0..1)
    void ambient(Flt v);
    Flt ambient() C { return _v[VOLUME_AMBIENT]; } // set/get ambient volume (0..1)
    void voice(Flt v);
    Flt voice() C { return _v[VOLUME_VOICE]; } // set/get voice   volume (0..1)
    void ui(Flt v);
    Flt ui() C { return _v[VOLUME_UI]; } // set/get UI      volume (0..1)
    void global(Flt v);
    Flt global() C { return _v[VOLUME_NUM]; } // set/get global  volume (0..1)
  ...
  private:
    ...
    SoundVolumeClass();
} extern SoundVolume; // Main Sound Volume Control
/******************************************************************************/
struct ListenerClass {
  ...
  private:
    ...
    ListenerClass();
} extern Listener; // Main 3D Sound Listener Control
/******************************************************************************/
...
const_mem_addr struct _Sound // can be moved however 'memAddressChanged' needs to be called afterwards
{
    ...
    void raw(Long raw);
    Long raw() C; // set/get sound raw      position, 0..size   ()
    void sample(Long sample);
    Long sample() C; // set/get sound sample   position, 0..samples()
    void time(Flt t);
    Flt time() C { return _time; } // set/get sound time     position, 0..length ()
    void frac(Flt f);
    Flt frac() C; // set/get sound fraction position, 0..1
    void loop(Bool loop);
    Bool loop() C { return _loop; } // set/get sound looping          , true/false
    void volume(Flt volume);
    Flt volume() C { return _volume; } // set/get sound volume           , 0..1
    void speed(Flt speed);
    Flt speed() C { return _speed; } // set/get sound speed            , 0..3, default=1
    void range(Flt range);
    Flt range() C { return _range; } // set/get sound 3D range         , 0..Inf
    void pos(C Vec &pos);
    C Vec &pos() C { return _pos; } // set/get sound 3D position
    void vel(C Vec &vel);
    C Vec &vel() C { return _vel; } // set/get sound 3D velocity

    void preciseRaw(Long raw);
    Long preciseRaw() C; // set/get sound raw      position, 0..size  ()
    void preciseTime(Flt t);
    Flt preciseTime() C; // set/get sound time     position, 0..length()
    void preciseFrac(Flt f);
    Flt preciseFrac() C; // set/get sound fraction position, 0..1
    ...
    void fadeCurve(FADE_CURVE curve);
    FADE_CURVE fadeCurve() C { return _fade_curve; } // set/get fade curve
...
struct Sound {
    ...
    Sound &raw(Long raw);
    Long raw() C; // set/get sound raw      position  , 0..size   ()
    Sound &sample(Long sample);
    Long sample() C; // set/get sound sample   position  , 0..samples()
    Sound &time(Flt t);
    Flt time() C; // set/get sound time     position  , 0..length ()
    Sound &frac(Flt f);
    Flt frac() C; // set/get sound fraction position  , 0..1
    Sound &loop(Bool loop);
    Bool loop() C; // set/get sound looping            , true/false
    Sound &volume(Flt volume);
    Flt volume() C; // set/get sound volume             , 0..1
    Sound &speed(Flt speed);
    Flt speed() C; // set/get sound speed              , 0..3, default=1
    Sound &range(Flt range);
    Flt range() C; // set/get sound 3D range multiplier, 0..Inf
    Sound &pos(C Vec &pos);
    C Vec &pos() C; // set/get sound 3D position
    Sound &vel(C Vec &vel);
    C Vec &vel() C; // set/get sound 3D velocity

    // volume fade
    Flt fade() C; //     get current fade value
    Sound &fadeCurve(FADE_CURVE curve);
    FADE_CURVE fadeCurve() C;                    // set/get fade curve
    ...
    // data callback
    Sound &callback(SoundDataCallback *callback);
    SoundDataCallback *callback() C; // set/get data callback, it will be called every time a new portion of data is processed by the sound
...
void SoundMaxConcurrent(Int max); // get/set maximum number of concurrent sounds                , default=16  , if number of sounds exceeds the specified limit then sounds with lower priority will be skipped for faster processing, specifying negative value (-1) is the same as allowing unlimited number of concurrent sounds
Int SoundMaxThreads();
void SoundMaxThreads(Int max); // get/set maximum number of threads used for sound processing, default=1
/******************************************************************************/
#if EE_PRIVATE
