...
#define T3(a, b, c)                                                            \
  template <typename a, typename b, typename c> // 3 types template
#if EE_PRIVATE
#define T4(a, b, c, d)                                                         \
  template <typename a, typename b, typename c, typename d> //  4 types template
#define T5(a, b, c, d, e)                                                      \
  template <typename a, typename b, typename c, typename d,                    \
            typename e> //  5 types template
#define T6(a, b, c, d, e, f)                                                   \
  template <typename a, typename b, typename c, typename d, typename e,        \
            typename f> //  6 types template
#define T7(a, b, c, d, e, f, g)                                                \
  template <typename a, typename b, typename c, typename d, typename e,        \
            typename f, typename g> //  7 types template
#define T8(a, b, c, d, e, f, g, h)                                             \
  template <typename a, typename b, typename c, typename d, typename e,        \
            typename f, typename g, typename h> //  8 types template
#define T9(a, b, c, d, e, f, g, h, i)                                          \
  template <typename a, typename b, typename c, typename d, typename e,        \
            typename f, typename g, typename h,                                \
            typename i> //  9 types template
#define T10(a, b, c, d, e, f, g, h, i, j)                                      \
  template <typename a, typename b, typename c, typename d, typename e,        \
            typename f, typename g, typename h, typename i,                    \
            typename j> // 10 types template
#define T11(a, b, c, d, e, f, g, h, i, j, k)                                   \
  template <typename a, typename b, typename c, typename d, typename e,        \
            typename f, typename g, typename h, typename i, typename j,        \
            typename k> // 11 types template
#endif
/******************************************************************************/
// HELPER MACROS
/******************************************************************************/
...
#define SIZEI(x)                                                               \
  Int(sizeof(x)) // get raw size of C++ element in bytes as   'Int' type
#define SIZEU(x)                                                               \
  UInt(sizeof(x)) // get raw size of C++ element in bytes as  'UInt' type
#define SIZEL(x)                                                               \
  Long(sizeof(x)) // get raw size of C++ element in bytes as  'Long' type
#define SIZEUL(x)                                                              \
...
#define MEMBER_SIZE(Class, member)                                             \
  SIZE(MEMBER(Class, member)) // get size     of member in class
#define MEMBER_ELMS(Class, member)                                             \
  ELMS(MEMBER(Class, member)) // get elements of member in class
#define CAST(Class, object)                                                    \
  dynamic_cast<Class *>(                                                       \
      object) // perform a dynamic cast of 'object' to 'Class' class
#define SCAST(Class, object)                                                   \
  static_cast<Class &>(                                                        \
...
T1(TYPE) TYPE &ConstCast(C TYPE &x) {
...
/******************************************************************************/
#define REP(n)                                                                 \
  for (Int i = (n); --i >= 0;) //         repeat                : n-1 .. 0
#define REPD(i, n)                                                             \
  for (Int i = (n); --i >= 0;) //         repeat with definition: n-1 .. 0
#define FREP(n)                                                                \
  for (Int i = 0; i < (n); i++) // forward repeat                :   0 .. n-1
#define FREPD(i, n)                                                            \
  for (Int i = 0; i < (n); i++) // forward repeat with definition:   0 .. n-1

#define REPA(a)                                                                \
  for (Int i = Elms(a);                                                        \
       --i >= 0;) //         repeat all                : Elms(a)-1 .. 0
#define REPAD(i, a)                                                            \
  for (Int i = Elms(a);                                                        \
       --i >= 0;) //         repeat all with definition: Elms(a)-1 .. 0
#define FREPA(a)                                                               \
  for (Int i = 0; i < Elms(a);                                                 \
       i++) // forward repeat all                :         0 .. Elms(a)-1
#define FREPAD(i, a)                                                           \
  for (Int i = 0; i < Elms(a);                                                 \
       i++) // forward repeat all with definition:         0 .. Elms(a)-1

#define REPAO(a)                                                               \
  for (Int i = Elms(a); --i >= 0;)                                             \
  (a)[i] //         repeat all                 and operate: Elms(a)-1 .. 0
#define REPAOD(i, a)                                                           \
  for (Int i = Elms(a); --i >= 0;)                                             \
...
#define REPS(i, n)                                                             \
  for ((i) = (n);                                                              \
       --(i) >= 0;) //         repeat     with i specified:      n -1 .. 0
#define FREPS(i, n)                                                            \
  for ((i) = 0; (i) < (n);                                                     \
       (i)++) // forward repeat     with i specified:         0 .. n-1
#define REPAS(i, a)                                                            \
  for ((i) = Elms(a);                                                          \
       --(i) >= 0;) //         repeat all with i specified: Elms(a)-1 .. 0
...
// ENUM MACROS
/******************************************************************************/
#define ENABLE_IF_ENUM(ENUM, RESULT)                                           \
  typename std::enable_if<std::is_enum<ENUM>::value, RESULT>::type
#define DISABLE_IF_ENUM(ENUM, RESULT)                                          \
  typename std::enable_if<!std::is_enum<ENUM>::value, RESULT>::type

T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Int) operator-(TYPE a, TYPE b) {
  return Int(a) - Int(b);
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Int) operator+(Bool a, TYPE b) {
  return a + Int(b);
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Int) operator*(Bool a, TYPE b) {
  return a * Int(b);
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Int) operator+(Int a, TYPE b) {
  return a + Int(b);
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Int) operator-(Int a, TYPE b) {
  return a - Int(b);
...
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, UInt) operator+(UInt a, TYPE b) {
  return a + UInt(b);
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, UInt) operator-(UInt a, TYPE b) {
  return a - UInt(b);
...
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, ULong) operator+(ULong a, TYPE b) {
  return a + ULong(b);
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, ULong) operator-(ULong a, TYPE b) {
  return a - ULong(b);
...
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Flt) operator+(Flt a, TYPE b) {
  return a + ENUM_TYPE(TYPE)(b);
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Flt) operator-(Flt a, TYPE b) {
  return a - ENUM_TYPE(TYPE)(b);
}
...
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Dbl) operator+(Dbl a, TYPE b) {
  return a + ENUM_TYPE(TYPE)(b);
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Dbl) operator-(Dbl a, TYPE b) {
  return a - ENUM_TYPE(TYPE)(b);
...
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Int) operator+(TYPE a, Bool b) {
  return Int(a) + b;
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Int) operator-(TYPE a, Bool b) {
  return Int(a) - b;
...
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Int) operator+(TYPE a, Int b) {
  return Int(a) + b;
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Int) operator-(TYPE a, Int b) {
  return Int(a) - b;
...
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, UInt) operator+(TYPE a, UInt b) {
  return UInt(a) + b;
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, UInt) operator-(TYPE a, UInt b) {
  return UInt(a) - b;
...
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, ULong) operator+(TYPE a, ULong b) {
  return ULong(a) + b;
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, ULong) operator-(TYPE a, ULong b) {
  return ULong(a) - b;
...
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Flt) operator+(TYPE a, Flt b) {
  return ENUM_TYPE(TYPE)(a) + b;
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Flt) operator-(TYPE a, Flt b) {
  return ENUM_TYPE(TYPE)(a) - b;
}
...
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Dbl) operator+(TYPE a, Dbl b) {
  return ENUM_TYPE(TYPE)(a) + b;
}
T1(TYPE) constexpr ENABLE_IF_ENUM(TYPE, Dbl) operator-(TYPE a, Dbl b) {
  return ENUM_TYPE(TYPE)(a) - b;
...
constexpr typename std::enable_if<
    std::is_enum<ENUM0>::value && std::is_enum<ENUM1>::value, Int>::type
operator+(ENUM0 a, ENUM1 b) {
  return Int(a) + Int(b);
} // ENUM0+ENUM1
/******************************************************************************/
// ASSERTIONS
...
#else
#define DEBUG_ASSERT(value, error)                                             \
  {} // debug        assertion unavailable      in release mode
#define DEBUG_EXIT(error)                                                      \
  {} // Exit in debug only
#endif
...
#define SUPPORT_VP                                                             \
  (WINDOWS || MAC || LINUX || ANDROID || (IOS && !IOS_SIMULATOR) || SWITCH)
...
T1(TYPE) TYPE &DTOR(TYPE &elm) {
  elm.~TYPE();
  return elm;
} //  destructor
T1(TYPE) TYPE &CTOR(TYPE &elm) {
  new (&elm) TYPE;
  return elm;
} // constructor
T2(TA, TB) TA &CTOR(TA &elm, TB &param) {
  new (&elm) TA(param);
  return elm;
} // constructor with a parameter
T2(TA, TB) TA &CTOR(TA &elm, C TB &param) {
  new (&elm) TA(param);
  return elm;
} // constructor with a parameter

T1(TYPE) Bool OK(TYPE x) { return x >= 0; }
T1(TYPE) void RELEASE(TYPE *&x) {
  if (x) {
    x->Release();
    x = null;
  }
