...
Bool CodeLine::isKeyword(Int i, CChar8 *keyword) C {
    if (type(i - 1) != TOKEN_KEYWORD && keyword)
        for (;; i++) {
            Char8 k = *keyword++;
            if (!k)
                return type(i) != TOKEN_KEYWORD;
            if (type(i) != TOKEN_KEYWORD || T[i] != k)
                break;
        }
    return false;
}
Bool CodeLine::starts(Int i, CChar8 *text) C {
    if (InRange(i, cols) && text)
        for (;;) {
            Char c = *text++;
            if (!c)
                return true;
...
Int SourceLoc::Compare(C SourceLoc &a, C SourceLoc &b) {
    if (Int c = ::Compare(a.file, b.file))
        return c;
...
void LineMap::operator++(int) {
    ...
    if (sections.elms()) {
        ...
        offset = last.offset + last.target_to_original.elms();
        if (!last.src.is())
            section = &last;
    }
    if (!section) {
...
void LineMap::add(C SourceLoc &src, C Memc<CodeLine> &lines) {
    ...
    FREPA(lines) // order important
    {
        ...
        map.line_id.zero();
        if (source && InRange(map.line_index, source->lines))
            map.line_id = source->lines[map.line_index].id;
    }
}
/******************************************************************************/
// FUNCTIONS
...
CodeLine *FindLine(Memc<CodeLine> &code_lines, Int line) {
    Int i = FindLineI(code_lines, line);
    return (i >= 0) ? &code_lines[i] : null;
}

CodeLine *FindLineCol(Memc<CodeLine> &code_lines, C VecI2 &pos, Int &cl_col) {
    VecI2 cl_pos;
    if (FindLineCol(code_lines, pos, cl_pos)) {
        cl_col = cl_pos.x;
        return &code_lines[cl_pos.y];
    }
    cl_col = -1;
    return null;
}
Bool FindLineCol(Memc<CodeLine> &code_lines, C VecI2 &pos, VecI2 &cl_pos) {
    if (pos.x >= 0 && pos.y >= 0)
        ...
        {
            CodeLine &cl = code_lines[i];
            if (cl.hasLine(pos.y)) {
                Int cl_col = cl.findPos(pos);
                if (cl_col >= 0) {
                    cl_pos.set(cl_col, i);
                    return true;
                } // operate on temporary 'cl_col' in case 'cl_pos' is actually 'pos'
            }
        }
    return false;
}
Bool FindLineCol(Memc<CodeLine> &code_lines, Int token_index, Int token_line, Bool token_start, VecI2 &cl_pos) {
    if (token_index >= 0 && token_line >= 0)
        ...
        {
            CodeLine &cl = code_lines[i];
            if (cl.hasLine(token_line)) {
                cl_pos.x = cl.findToken(token_index, token_start);
                if (cl_pos.x >= 0) {
                    cl_pos.y = i;
                    return true;
                }
...
void Clean(Memc<CodeLine> &lines) {
    ...
    REPA(lines) {
        ...
        if (line.toRemove()) {
            ...
            if (InRange(i - 1, lines) && InRange(i, lines)) {
                CodeLine &prev = lines[i - 1];
                Bool prev_can_be_removed = true;
    ...
    FREPAD(y, lines) {
        ...
        if (l.type(i) == TOKEN_REMOVE) {
            ...
            Bool obstacle = false;
            if (!obstacle)
                for (y0 = y; InRange(y0 - 1, lines);) {
                    if (!InRange(i, lines[y0 - 1]) || lines[y0 - 1].type(i) == TOKEN_COMMENT || lines[y0 - 1][i] == ')')
                        break;
                    y0--;
                    if (ValidType(lines[y0].type(i)) || lines[y0][i - 1] == ',' || lines[y0][i + 1] == '{' || MustSeparate(lines[y0].type(i - 1), lines[y0].type(i + 1))) {
                        obstacle = true;
                        break;
                    }
                }
            if (!obstacle)
                for (y1 = y; InRange(y1 + 1, lines);) {
                    if (!InRange(i, lines[y1 + 1]) || lines[y1 + 1].type(i) == TOKEN_COMMENT)
                        break;
                    y1++;
                    if (ValidType(lines[y1].type(i)) || lines[y1][i - 1] == ',' || lines[y1][i + 1] == '{' || MustSeparate(lines[y1].type(i - 1), lines[y1].type(i + 1))) {
                        obstacle = true;
                        break;
                    }
                }
            if (!obstacle) {
                ...
                for (; y0 <= y1; y0++) {
                    CodeLine &l = lines[y0];
                    if (!(ValidType(l.type(i - 1)) && l.type(i + 1) == TOKEN_COMMENT)) {
                        l.remove(i);
                        removed = true;
                    }
                }
                if (removed)
                    i--; // try again only if we've actually removed something
            }
        }
...
void Write(FileText &f, C Memc<CodeLine> &lines) {
    FREPA(lines) {
        C CodeLine &cl = lines[i];
        FREPA(cl.cols)
        f.putChar(cl.cols[i].c);
        f.endLine();
    }
}
/******************************************************************************/
void AdjustNameSymbol(Memc<CodeLine> &lines, Symbol *&src, Symbol *dest) {
    if (src != dest) {
        ...
        if (dest)
            for (Symbol *cur = dest->Parent(), *child = dest;;) {
                if (cur == src) {
                    src = child;
                    {
                        CodeLine &cl = lines.New();
                        REP(TabLength * src->level)
                        cl.append(' ', TOKEN_NONE);
                        cl.append("namespace", TOKEN_KEYWORD).append(' ', TOKEN_NONE).append(*src, TOKEN_CODE);
                    }
                    {
                        CodeLine &cl = lines.New();
                        REP(TabLength * src->level)
                        cl.append(' ', TOKEN_NONE);
                        cl.append('{', TOKEN_OPERATOR);
                    }
        ...
        // we must go back, because 'dest' is not a child of 'src'
        {
            ...
            REP(TabLength * src->level)
            cl.append(' ', TOKEN_NONE);
            cl.append('}', TOKEN_OPERATOR).append(' ', TOKEN_NONE).append(S + "// namespace " + *src, TOKEN_COMMENT);
        }
        src = src->Parent();
        AdjustNameSymbol(lines, src, dest);
        return;
...
void Parse(Memc<CodeLine> &lines) {
    FREPA(lines) {
        ...
        FREPA(cl) {
            ...
            if (type == TOKEN_CHAR16 && cl.type(i - 1) != TOKEN_CHAR16 && cl[i] == '\'')
...
Bool OverwriteOnChange(File &src, C Str &dest, Bool *changed) {
    ...
    if (changed)
        *changed = true;
    if (!f.write(dest)) {
        FCreateDirs(_GetPath(dest));
        if (!f.write(dest))
            return false;
    }
    src.pos(0);
    return src.copy(f) && f.flush();
}
...
Bool OverwriteOnChangeLoud(XmlData &src, C Str &dest, Bool params_in_separate_lines, ENCODING encoding) {
    if (!OverwriteOnChange(src, dest, params_in_separate_lines, encoding))
        return ErrorWrite(dest);
    return true;
}
