...
void ColorTheme::load(C TextNode &node) {
    if (C TextNode *p = node.findNode("Background"))
        colors[TOKEN_NONE] = p->asColor();
    if (C TextNode *p = node.findNode("Operator"))
        colors[TOKEN_OPERATOR] = p->asColor();
    if (C TextNode *p = node.findNode("Keyword"))
        colors[TOKEN_KEYWORD] = p->asColor();
    if (C TextNode *p = node.findNode("Code"))
        colors[TOKEN_CODE] = p->asColor();
    if (C TextNode *p = node.findNode("Comment"))
        colors[TOKEN_COMMENT] = p->asColor();
    if (C TextNode *p = node.findNode("Text"))
        colors[TOKEN_TEXT8] = colors[TOKEN_TEXT16] = p->asColor();
    if (C TextNode *p = node.findNode("Character"))
        colors[TOKEN_CHAR8] = colors[TOKEN_CHAR16] = p->asColor();
    if (C TextNode *p = node.findNode("Number"))
        colors[TOKEN_NUMBER] = p->asColor();
    if (C TextNode *p = node.findNode("Preprocessor"))
        colors[TOKEN_PREPROC] = p->asColor();
    if (C TextNode *p = node.findNode("PreprocessorDisabled"))
        colors[TOKEN_PREPROC_DISABLED] = p->asColor();
    if (C TextNode *p = node.findNode("Macro"))
        colors[TOKEN_MACRO] = p->asColor();
    if (C TextNode *p = node.findNode("EnumType"))
        colors[TOKEN_ENUM_TYPE] = p->asColor();
    if (C TextNode *p = node.findNode("EnumElement"))
        colors[TOKEN_ENUM_ELM] = p->asColor();
    if (C TextNode *p = node.findNode("Function"))
        colors[TOKEN_FUNC] = p->asColor();
    if (C TextNode *p = node.findNode("Selection"))
        colors[TOKEN_SELECT] = p->asColor();
    if (C TextNode *p = node.findNode("LineHighlight"))
        colors[TOKEN_LINE_HIGHLIGHT] = p->asColor();
    if (C TextNode *p = node.findNode("SymbolHighlight"))
        colors[TOKEN_SYMBOL_HIGHLIGHT] = p->asColor();
    if (C TextNode *p = node.findNode("BraceHighlight"))
        colors[TOKEN_BRACE_HIGHLIGHT] = p->asColor();
    if (C TextNode *p = node.findNode("ElementBackground"))
        colors[TOKEN_ELM_BACKGROUND] = p->asColor();
    if (C TextNode *p = node.findNode("ElementName"))
        colors[TOKEN_ELM_NAME] = p->asColor();
    if (C TextNode *p = node.findNode("LineNumberBackground"))
        colors[TOKEN_LINE_NUM_BACKGROUND] = p->asColor();
    if (C TextNode *p = node.findNode("LineNumber"))
        colors[TOKEN_LINE_NUM] = p->asColor();
}
/******************************************************************************/
void CodeEditor::HideAndFocusCE(GuiObj &go) {
    go.hide();
    CE.cei().kbSet();
}
/******************************************************************************/
...
void CodeEditor::MenuBarEx::parentClientRectChanged(C Rect *old_client, C Rect *new_client) {
    if (new_client) {
        ...
        if (CE.menu_on_top)
            rect(Rect_LU(r.min.x, r.max.y, r.w(), 0.06f));
        else
            rect(Rect_LD(r.min.x, r.min.y, r.w(), 0.06f));
    }
}
/******************************************************************************/
void CodeEditor::GotoLineWindow::create(CodeEditor &ce) {
    if (ce.parent) {
        T.ce = &ce;
        *ce.parent += super ::create(Rect_C(0, 0, 1, 0.14f), "Go To Line").hide();
        button[2].func(HideAndFocusCE, SCAST(GuiObj, T)).show();
...
void CodeEditor::GotoLineWindow::update(C GuiPC &gpc) {
    ...
    if (Gui.window() == this) {
        if (Kb.k(KB_ENTER)) {
            if (ce && ce->cur()) {
                ce->markCurPos();
                ce->cur()->highlight(TextInt(textline()) - 1, false);
            }
            button[2].push();
            Kb.eatKey();
        }
...
Window &CodeEditor::Options::VSVersions::show() {
    ...
    REPA(installs) // list newest first
    {
...
void CodeEditor::Options::VSVersions::create(CodeEditor &ce) {
    T.ce = &ce;
    Gui += super::create(Rect_C(0, 0, 1.0f, 0.44f), "Visual Studio Versions Detected").hide();
    button[2].show();
}
/******************************************************************************/
...
CodeEditor::Options::ColorThemeEditor &CodeEditor::Options::ColorThemeEditor::create() {
    ...
    r.max += defaultInnerPaddingSize() + 0.04f;
    rect(Rect_RD(D.w(), -D.h(), r.w(), r.h()));
    REPAO(props).autoData(&ThemeCustom).changed(ThemeColorChanged);
    button[2].show();
    return T;
}
...
Str CodeEditor::Options::FontData::chars() C {
    ...
    if (1)
        c += Ellipsis;
    if (1)
        c += LanguageSpecific(DE);
    if (1)
        c += LanguageSpecific(FR);
    if (1)
        c += LanguageSpecific(PL);
    if (1)
        c += LanguageSpecific(RU);
    REPA(langs)
    c += LanguageSpecific(langs[i]);
...
void CodeEditor::Options::FontData::save(TextNode &node) C {
    ...
    Str &l = node.getNode("languages").value;
    l.clear();
    FREPA(langs)
    l.space() += langs[i];
}
void CodeEditor::Options::FontData::load(C TextNode &node) {
    langs.clear();
    if (C TextNode *p = node.findNode("size"))
        size = Mid(p->asInt(), 1, 256);
    if (C TextNode *p = node.findNode("spacing.x"))
        spacing.x = Mid(p->asFlt(), 0.0f, 1.0f);
    if (C TextNode *p = node.findNode("spacing.y"))
        spacing.y = Mid(p->asFlt(), 0.0f, 2.0f);
    if (C TextNode *p = node.findNode("font"))
        font = p->value;
    if (C TextNode *p = node.findNode("custom_chars"))
        custom_chars = p->value;
    if (C TextNode *p = node.findNode("languages")) {
        Memt<Str> l;
        Split(l, p->value, ' ');
        FREPA(l)
        if (l[i].is())
...
static void ParamsChinese(CodeEditor::Options::FontParams &p, C Str &t) {
    if (p.chinese = TextBool(t))
        p.langs.include(CN);
    else
        p.langs.exclude(CN);
}
static void ParamsJapanese(CodeEditor::Options::FontParams &p, C Str &t) {
    if (p.japanese = TextBool(t))
        p.langs.include(JP);
    else
        p.langs.exclude(JP);
}
static void ParamsKorean(CodeEditor::Options::FontParams &p, C Str &t) {
    if (p.korean = TextBool(t))
        p.langs.include(KO);
    else
        p.langs.exclude(KO);
}
...
void CodeEditor::Options::FontEditor::preview() {
    ...
    ts.space.set(params.spacing.x, AlignRound(params.spacing.y, 1.0f / font.height()));
...
Bool CodeEditor::Options::FontEditor::make() {
    if (visible()) {
        ...
        CodeEditor::Options::FontParams params = T.params;
        locker.off();
        ThreadMayUseGPUData();
        Font temp;
        if (params.make(temp, &params.sample_text)) {
            locker.on();
            Swap(font, temp);
            locker.off();
        }
...
static void Apply(CodeEditor::Options::FontEditor &font_edit) {
    Font temp;
    if (font_edit.params.make(temp)) {
        File f;
        temp.save(f.writeMem());
        f.pos(0);
        if (SafeOverwrite(f, "Bin/Code Editor.font")) {
            FontPtr font;
            if (font.find("Bin/Code Editor.font"))
                Swap(*font, temp);
            CE.fontChanged();
        }
    }
...
CodeEditor::Options::FontEditor &CodeEditor::Options::FontEditor::hide() {
    if (visible())
        thread.stop();
    super::hide();
    return T;
}
CodeEditor::Options::FontEditor &CodeEditor::Options::FontEditor::show() {
    if (hidden())
        thread.create(Make, this);
    super::show();
    return T;
}
...
CodeEditor::Options::FontEditor &CodeEditor::Options::FontEditor::create() {
    ...
    Vec2 size(viewport.rect().max.x, -apply.rect().min.y);
    size += defaultInnerPaddingSize() + 0.02f;
...
static void NetBeansChanged(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setNetBeansPath(op.netbeans_path());
}
static void AndroidSDKChanged(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setAndroidSDKPath(op.android_sdk());
}
static void AndroidNDKChanged(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setAndroidNDKPath(op.android_ndk());
}
static void JDKChanged(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setJDKPath(op.jdk_path());
}
static void AndroidCertChangedF(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setAndroidCertPath(op.android_cert_file());
}
static void AndroidCertChangedP(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setAndroidCertPass(op.android_cert_pass());
}
static void AppleTeamIDChanged(CodeEditor::Options &op) {
    if (op.ce)
...
static void NetBeansLoad(C Str &path, CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setNetBeansPath(path);
}
static void AndroidSDKLoad(C Str &path, CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setAndroidSDKPath(path);
}
static void AndroidNDKLoad(C Str &path, CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setAndroidNDKPath(path);
}
static void JDKLoad(C Str &path, CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setJDKPath(path);
}
static void AndroidCertLoad(C Str &path, CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setAndroidCertPath(path);
}
static void VSAutodetect(CodeEditor::Options &op) { op.vs_versions.activate(); }
static void AndroidSDKAutodetect(CodeEditor::Options &op) {
    Str p = GetRegStr(RKG_CURRENT_USER, "Environment/ANDROID_SDK_ROOT");
    if (p.is())
        AndroidSDKLoad(p, op);
}
static void JDKAutodetect(CodeEditor::Options &op) {
    ...
    if (!p.is()) {
        ...
        if (p.is())
            p = GetRegStr(RKG_LOCAL_MACHINE, S + "Software/JavaSoft/JDK/" + p + "/JavaHome");
    }
    if (p.is())
        JDKLoad(p, op);
}
...
static void AndroidCertCreate(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->android_certificate.activate();
}
static void FontSizeChanged(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->fontChanged();
}
static void ThemeChanged(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->themeChanged();
}
...
static void ScrollChanged(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->scrollChanged();
}
static void LineNumbersChanged(CodeEditor::Options &op) {
    if (op.ce && op.ce->cur())
        op.ce->cur()->setRegionSize();
}
static void HideHorizontalSlidebar(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->hideSlideBarChanged();
}
static void AutoHideMenuBar(CodeEditor::Options &op) {
    if (op.ce)
        op.ce->setMenuBarVisibility();
}
static void ShowFileTabs(CodeEditor::Options &op) {
    CE.code_tabs.visible(op.show_file_tabs());
    CE.resize();
}

static void FacebookAndroidKeyHash(CodeEditor::Options &op) {
#if WINDOWS
    if (!CE.jdk_path.is() || !FExistSystem(CE.jdk_path.tailSlash(true) + "bin/keytool.exe")) {
        CE.options.activatePaths();
        Gui.msgBox(S, "Path to Java Development Kit was not specified or is invalid.");
        return;
    }
#endif
    if (!CE.android_cert_file.is() || !FExistSystem(CE.android_cert_file)) {
        CE.options.activateCert();
        Gui.msgBox(S, "Path to Android Certificate File was not specified or is invalid.");
        return;
    }
    if (!CE.android_cert_pass.is()) {
        CE.options.activateCert();
        Gui.msgBox(S, "Android Certificate Password was not specified.");
        return;
    }
    ConsoleProcess cp;
    if (!cp.create(PLATFORM(CE.jdk_path.tailSlash(true) + "bin/keytool.exe", "keytool"), S + "-exportcert -keystore \"" + CE.android_cert_file + "\" -storepass \"" + CE.android_cert_pass + "\" -keypass \"" + CE.android_cert_pass + "\" -alias \"key\"", true, true)) {
        Gui.msgBox(S, "Failed to start the Java Development Kit Key Tool.");
        return;
    }
    if (!cp.wait(-1)) {
        Gui.msgBox(S, "Java Development Kit Key Tool timed out.");
        return;
    }
    Str out = cp.get();
    Memt<Byte> data;
    data.setNum(out.length());
    REPAO(data) = out[i];
...
void CodeEditor::Options::activatePaths() {
    tabs.set(1);
    activate();
}
void CodeEditor::Options::activateCert() {
    tabs.set(2);
    activate();
}
...
void CodeEditor::Options::create(CodeEditor &ce) {
    ...
    Flt Y = -0.05f;
    T += tabs.create(Rect_C(clientWidth() / 2, Y, 0.8f, 0.05f), 0, tabs_t, Elms(tabs_t), true).valid(true).set(0);
    Y -= 0.10f;
    ...
    // code editor
    {
        ...
        tab += font_edit.create(Rect_L(font_size.rect().right() + Vec2(0.02f, 0), 0.10f, h), "Edit").func(EditCustomFont, T);
        y -= s;
        ...
        tab += color_theme_edit.create(Rect_L(color_theme.rect().right() + Vec2(0.02f, 0), 0.10f, h), "Edit").func(EditCustomTheme, T);
        y -= s;
        ...
        y -= s;
        tab += imm_scroll.create(Rect_C(clientWidth() / 2, y, w, h), "Immediate Scroll").func(ScrollChanged, T).desc("Enable immediate mouse-wheel scrolling instead of default smooth.");
        imm_scroll.mode = BUTTON_TOGGLE;
        y -= s;
        tab += eol_clip.create(Rect_C(clientWidth() / 2, y, w, h), "End of Line Cursor Clip").desc("Clip cursor position to end of line when mouse clicking.");
        eol_clip.mode = BUTTON_TOGGLE;
        eol_clip.set(true);
        y -= s;
        tab += line_numbers.create(Rect_C(clientWidth() / 2, y, w, h), "Show Line Numbers").func(LineNumbersChanged, T);
        line_numbers.mode = BUTTON_TOGGLE;
        y -= s;
        tab += hide_horizontal_slidebar.create(Rect_C(clientWidth() / 2, y, w, h), "Hide Horizontal SlideBar").func(HideHorizontalSlidebar, T);
        hide_horizontal_slidebar.mode = BUTTON_TOGGLE;
        y -= s;
        tab += auto_hide_menu.create(Rect_C(clientWidth() / 2, y, w, h), "Auto-Hide MenuBar").func(AutoHideMenuBar, T);
        auto_hide_menu.mode = BUTTON_TOGGLE;
        y -= s;
        tab += show_file_tabs.create(Rect_C(clientWidth() / 2, y, w, h), "Show File Tabs").func(ShowFileTabs, T);
        show_file_tabs.mode = BUTTON_TOGGLE;
        show_file_tabs.set(true, QUIET);
        y -= s;
        tab += t_export_path_mode.create(Vec2(0.475f, y), "Project Export Paths:");
        tab += export_path_mode.create(Rect_R((clientWidth() + w) / 2, y, 0.24f, h), path_mode_t, Elms(path_mode_t)).set(0);
        y -= s;
    }
...
#elif LINUX
        tab += netbeans_path.create(Rect_C(clientWidth() / 2, y, w, h), ce.netbeans_path).func(NetBeansChanged, T);
        y -= s;
...
#if WINDOWS
        tab += jdk_path.create(Rect_C(clientWidth() / 2, y, w, h), ce.jdk_path).func(JDKChanged, T);
        y -= s;
    ...
    // certificates
    {
        ...
        tab += android_cert_file.create(Rect_C(clientWidth() * 0.5f, y, w, h)).func(AndroidCertChangedF, T);
        y -= s;
        tab += t_android_cert_file.create(android_cert_file.rect().lu(), "Android Certificate File", &ts);
        y -= s;
        ...
        tab += android_cert_pass.create(Rect_C(clientWidth() * 0.5f, y, w, h)).func(AndroidCertChangedP, T);
        y -= s;
        tab += t_android_cert_pass.create(android_cert_pass.rect().lu(), "Android Certificate Password", &ts);
        y -= s;

        tab += apple_team_id.create(Rect_C(clientWidth() * 0.5f, y, w, h)).func(AppleTeamIDChanged, T);
        y -= s;
        tab += t_apple_team_id.create(apple_team_id.rect().lu(), "Apple Team ID", &ts);
        y -= s;
        tab += b_apple_team_id.create(Rect_LU(apple_team_id.rect().ru(), h * 1.5f, h), "Get").func(AppleTeamIDGet, T);

        tab += facebook_android_key_hash.create(Rect_C(clientWidth() * 0.5f, y, 0.80f, 0.06f), "Get Android Key Hash for Facebook").func(FacebookAndroidKeyHash, T);
        y -= s;
        y -= s;
        ...
        authenticode.mode = BUTTON_TOGGLE;
        authenticode.set(false);
        y -= s;
    }
    ...
    // importing
    {
        ...
        tab += import_path_mode.create(Rect_R((clientWidth() + w) / 2, y, 0.24f, h), path_mode_t, Elms(path_mode_t)).set(0);
        y -= s;
        tab += import_image_mip_maps.create(Rect_C(clientWidth() / 2, y, w, h), "Create Image Mip Maps").desc("This option defines if newly imported images should have mip maps created.");
        import_image_mip_maps.mode = BUTTON_TOGGLE;
        import_image_mip_maps.set(true);
        y -= s;
    }
...
#endif
    if (Contains(ac.org_name(), '"')) {
        Gui.msgBox(S, "Developer Name cannot contain quotation marks");
        return;
    }
    if (Contains(ac.pass(), '"')) {
        Gui.msgBox(S, "Password cannot contain quotation marks");
        return;
    }
    if (ac.pass().length() < 6) {
        Gui.msgBox(S, "Password must be at least 6 characters long");
        return;
    }
    ac.win_io.save();
}
static void AndroidCreateCert(C Str &name, CodeEditor::AndroidCertificate &ac) {
    ...
    if (!cp.create(PLATFORM(CE.jdk_path.tailSlash(true) + "bin/keytool.exe", "keytool"), S + "-genkey -keystore \"" + name + "\" -storepass \"" + ac.pass() + "\" -keypass \"" + ac.pass() + "\" -storetype PKCS12 -keyalg RSA -keysize 2048 -validity 10000 -alias \"key\" -dname \"o=" + ac.org_name() + "\"")) {
        Gui.msgBox(S, "Failed to start the Java Development Kit Certificate Generator.");
        return;
    }
    if (cp.wait(-1)) {
        if (!FExistSystem(name)) {
            Gui.msgBox(S, S + "Failed to create certificate:\n" + cp.get());
            return;
        }
...
void CodeEditor::AndroidCertificate::create(CodeEditor &ce) {
    Gui += super::create(Rect_C(0, 0, 0.7f, 0.42f), "Android Certificate Creator").hide();
    button[2].show();
    Flt y = -0.05f, h = 0.05f, s = 0.06f;
    T += torg_name.create(Rect_C(clientWidth() * 0.5f, y, 0, 0), "Developer Name");
    y -= s * 0.8f;
    T += org_name.create(Rect_C(clientWidth() * 0.5f, y, clientWidth() * 0.9f, h));
    y -= s;
    T += tpass.create(Rect_C(clientWidth() * 0.5f, y, 0, 0), "Password");
    y -= s * 0.8f;
    T += pass.create(Rect_C(clientWidth() * 0.5f, y, clientWidth() * 0.9f, h));
    y -= s * 1.4f;
    T += save.create(Rect_C(clientWidth() * 0.5f, y, 0.3f, h), "Create").func(AndroidCreateCert, T);
    y -= s;
    ...
    if (devlog_region.visible() != on) {
        ...
        if (on && !devlog_process.active()) {
            ...
            if (!adb_path.is()) {
                ...
                Gui.msgBox(S, "The path to Android SDK has not been specified or is invalid.");
            } else
...
static void RegionRect(Region &region, C Rect &rect) {
    if (region.rect() != rect) {
        Bool at_end = region.slidebar[1].wantedAtEnd();
        region.rect(rect);
        if (at_end)
            region.slidebar[1].scrollEnd(true);
    }
}
...
void CodeEditor::skinChanged() {
    ...
    if (Gui.skin)
        suggestions_skin = find_result_skin = source_skin = cjk_skin = *Gui.skin;
...
void CodeEditor::zoom(Int zoom) {
    if (InRange(options.font_size(), options.font_sizes) && options.font_sizes[options.font_size()].size > 0) {
        Int new_index = options.font_size() + zoom;
        if (InRange(new_index, options.font_sizes) && options.font_sizes[new_index].size > 0)
...
void CodeEditor::fontChanged() {
    if (InRange(options.font_size(), options.font_sizes)) {
        ...
        if (size <= 0) // try to load custom font
        {
            ...
            if (font.get("Bin/Code Editor.font"))
                if (font->is()) {
                    ts.font(font);
                    size = font->height();
                    spacing = options.font_editor.params.spacing;
                }
        }
        ...
        if (size <= 0)
            size = 18;
        if (!ts.font())
            ts.font(S + "Font/Lucida Console " + Mid(size, 11, 18) + ".font");
...
void CodeEditor::themeChanged() {
    Theme = (options.color_theme() == 0 ? ThemeLight : options.color_theme() == 1 ? ThemeBlue
                                                   : options.color_theme() == 2   ? ThemeDark
                                                                                  : ThemeCustom);
    options.color_theme_edit.visible(options.color_theme() == 3);
    REPAO(sources).themeChanged();
}
...
static Bool JumpTo(C Str &file, Int line) {
    ...
    if (LineMap *lm = CE.build_line_maps.find(file)) {
        ...
        Int original_index;
        UID original_id;
        lm->get(line, src, original_index, original_id);
        if (CE.load(src)) {
            Int l = CE.cur()->findLine(original_id);
            if (l >= 0)
                original_index = l;
...
Bool CodeEditor::BuildResult::jumpTo() {
    ...
    } else // try to parse the text and detect file paths and lines
    {
        ...
        FREPA(text)
        if (!(CharFlagFast(text[i]) & CHARF_DIG)) {
            if (text[i] == '>')
                text.remove(0, i + 1);
            break;
        }
        ...
        Int pos = posa;
        if (posb >= 0 && (pos < 0 || posb < pos))
            pos = posb; // take whichever is first and valid
        if (pos >= 0) {
            text.clip(pos);
            REPA(text) {
                if (text[i] == '(') {
                    ...
                    text.clip(i);
                    if (Starts(text, ".\\"))
                        text.remove(0, 2); // remove ".\\" in ".\\Source\\file.cpp"
                    if (!FullPath(text))
                        text = CE.build_path + text; // "Source\\file.cpp"
                    if (JumpTo(text, line))
                        return true;
                    break;
                } else if (!(CharFlagFast(text[i]) & CHARF_DIG) && text[i] != ',')
                    break;
            }
        } else {
            Int pos = TextPosI(text, ':');
            if (pos == 1)
                pos = TextPosIN(text, ':', 1); // if it's ':' from "C:" then get the next ':'
            if (pos >= 0) {
                ...
                if (line >= 0) {
                    ...
                    if (FullPath(text) || StartsPath(text, "Source") || StartsPath(text, "..")) {
                        if (!FullPath(text))
                            text = NormalizePath(CE.build_path + text);
                        if (JumpTo(text, line))
                            return true;
                    }
                }
...
void CodeEditor::buildNew(Memc<Message> &msgs) {
    ...
    FREPA(msgs) {
        ...
        if (last_source != cur_source) {
            last_source = cur_source;
            buildNew().set(S + "\"" + (cur_source ? cur_source->loc.asText() : S) + "\":", cur_source);
        }
        if (msg.type == Message::ERROR) {
            if (msg.token)
                buildNew().set(S + "  Error: " + msg.text, msg.token);
            else
                buildNew().set(S + "  Error: " + msg.text, msg.source);
        } else if (msg.type == Message::WARNING) {
            if (msg.token)
                buildNew().set(S + "  Warning: " + msg.text, msg.token);
            else
                buildNew().set(S + "  Warning: " + msg.text, msg.source);
        } else {
            if (msg.token)
                buildNew().set(msg.text, msg.token);
            else
                buildNew().set(msg.text, msg.source);
        }
        FREPA(msg.children) {
            Message &c = msg.children[i];
            if (msg.token)
                buildNew().set(S + "     " + c.text, c.token);
            else
                buildNew().set(S + "     " + c.text, c.source);
        }
    }
}
