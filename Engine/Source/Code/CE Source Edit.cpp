...
void Source::makeCurVisible(Bool center, Bool immediate) {
    ...
    if (center) {
        ...
        if (cur.y < Ceil(slidebar[1].offset() / CE.ts.lineHeight()))
            scrollToY(cur.y * CE.ts.lineHeight(), immediate);
        else if (cur.y + 1 > Trunc((slidebar[1].offset() + clientHeight()) / CE.ts.lineHeight()))
            scrollToY((cur.y + 1) * CE.ts.lineHeight(), immediate);
    }
    if (cur.x < Ceil((slidebar[0].offset() + CE.lineNumberSize()) / CE.ts.colWidth()))
        scrollToX(cur.x * CE.ts.colWidth(), true);
    else if (cur.x + 1 > Trunc((slidebar[0].offset() + clientWidth()) / CE.ts.colWidth()))
        scrollToX((cur.x + 8) * CE.ts.colWidth(), true);
...
void Source::curLeft() {
    forceCreateNextUndo();
    clearSuggestions();
    if (sel.x >= 0 && !Kb.k.shift()) {
        if (sel.y < cur.y || (sel.y == cur.y && sel.x < cur.x))
            cur = sel;
        sel = -1;
    } else {
        startSel();
        MAX(--cur.x, 0);
        if (0 && CE.view_elm_names && InRange(cur.y, lines)) {
            UID id;
            VecI2 range;
            if (TextToIDInside(lines[cur.y], cur.x, id, range))
                cur.x = range.x;
        }
    }
    makeCurVisible();
}
void Source::curRight() {
    forceCreateNextUndo();
    clearSuggestions();
    if (sel.x >= 0 && !Kb.k.shift()) {
        if (sel.y > cur.y || (sel.y == cur.y && sel.x > cur.x))
        ...
        if (0 && CE.view_elm_names && InRange(cur.y, lines)) {
            UID id;
            VecI2 range;
            if (TextToIDInside(lines[cur.y], cur.x - 1, id, range))
                cur.x = range.y + 1;
        }
    }
    makeCurVisible();
...
void Source::curLineBegin() {
    forceCreateNextUndo();
    clearSuggestions();
    startSel();
    if (InRange(cur.y, lines)) {
        Int start = lines[cur.y].start();
        cur.x = ((cur.x == start) ? 0 : start);
    } else
        cur.x = 0;
    makeCurVisible();
}
void Source::curLineEnd() {
    forceCreateNextUndo();
    clearSuggestions();
    startSel();
    if (InRange(cur.y, lines))
...
void Source::curPrevWord() {
    ...
    if (cur.x) {
        cur.x--;
        if (InRange(cur.y, lines))
            for (CHAR_TYPE ct = CodeCharType(lines[cur.y][cur.x]); cur.x > 0; cur.x--) {
                ...
                if (ct == CHART_SPACE || ct == CHART_NONE)
                    ct = nt;
                if (ct != nt || (ct == CHART_SIGN && (/*lines[cur.y].Type(cur.x-1)==TOKEN_OPERATOR || */ lines[cur.y].Type(cur.x) == TOKEN_OPERATOR)))
                    break;
            }
    } else {
        if (cur.y) {
            cur.y--;
            cur.x = lines[cur.y].end();
        }
    }
    makeCurVisible();
}
    ...
    if (InRange(cur.y, lines)) {
        if (cur.x >= lines[cur.y].end()) // at the end of line
        {
            if (InRange(cur.y + 1, lines)) {
                cur.y++;
                for (cur.x = 0; cur.x < lines[cur.y].length(); cur.x++)
            ...
            for (CHAR_TYPE ct = CodeCharType(lines[cur.y][cur.x - 1]); cur.x < lines[cur.y].length(); cur.x++) {
                ...
                if (ct != nt || (ct == CHART_SIGN && lines[cur.y].Type(cur.x) == TOKEN_OPERATOR)) {
                    for (; cur.x < lines[cur.y].length() && lines[cur.y][cur.x] == ' '; cur.x++)
                        ;
                    break;
                }
            }
...
/******************************************************************************/
void Source::curPrevBracket() {
    for (VecI2 p = cur; lineValid(dec(p));)
        if ((lines[p.y][p.x] == '{' || lines[p.y][p.x] == '}') && lines[p.y].Type(p.x) == TOKEN_OPERATOR) {
            cur = p;
            makeCurVisible();
            break;
        }
}
void Source::curNextBracket() {
    for (VecI2 p = cur; lineValid(inc(p));)
        if ((lines[p.y][p.x] == '{' || lines[p.y][p.x] == '}') && lines[p.y].Type(p.x) == TOKEN_OPERATOR) {
            cur = p;
            makeCurVisible();
            break;
...
void Source::curPrevLevelBracket() {
    ...
    for (VecI2 p = cur; lineValid(dec(p));)
        if (lines[p.y].Type(p.x) == TOKEN_OPERATOR) {
            if (lines[p.y][p.x] == '{')
                level++;
            else if (lines[p.y][p.x] == '}')
                level--;
            else
                continue;
            if (level > 0) {
...
void Source::curPageUp() {
    ...
    MAX(cur.y -= ys, 0);
    if (visible)
        scrollY(-ys * CE.ts.lineHeight(), true);
    else
        makeCurVisible();
}
...
void Source::delSel(Bool set_undo, Bool clear_suggestions) {
    ...
    if (sel.x >= 0 && sel.y >= 0) {
        ...
        curSel(min, max);
        if (min.y == max.y) {
            if (InRange(min.y, lines))
                lines[min.y].remove(min.x, max.x - min.x);
        } else {
            if (InRange(min.y, lines)) {
            ...
            if (InRange(max.y, lines))
                lines[max.y].remove(0, max.x);
        }
        REP(max.y - min.y - 1)
        removeLine(min.y + 1);                           // remove lines between
...
void Source::delForward() {
    ...
    if (sel.x >= 0)
        ...
        if (CE.view_elm_names && InRange(cur.y, lines)) {
            UID id;
            VecI2 range;
            if (TextToIDInside(lines[cur.y], cur.x, id, range)) {
                setUndo();
                clearSuggestions();
                lines[cur.y].remove(range.x, range.y - range.x + 1);
                cur.x = range.x;
                changed(cur.y);
                makeCurVisible();
                return;
        ...
        if (InRange(cur.y, lines)) {
            if (cur.x < lines[cur.y].length())
                lines[cur.y].remove(cur.x);
            else if (InRange(cur.y + 1, lines)) {
                exist(cur.x, cur.y);
...
void Source::delBack() {
    ...
    if (sel.x >= 0)
        ...
        clearSuggestions();
        if (undos.lastChangeTypeI() == INS_CHRS)
            undoAsChange();
        else {
            setUndo(DEL_CHR);
            if (cur.x == 0) {
                if (InRange(cur.y - 1, lines) && InRange(cur.y, lines)) {
        ...
        for (Int y = min.y; y <= max.y; y++)
            if (InRange(y, lines)) {
                ...
                for (Int max_x = ((y == max.y) ? Min(max.x, line.length()) : line.length()),
                         x = ((y == min.y) ? Max(min.x, 0) : 0);
                     x < max_x; x++)
                    line.setChar(x, MakeCase(line[x], upper));
            }
        changed(min.y, max.y - min.y + 1);
    } else if (InRange(cur.y, lines)) {
        Line &line = lines[cur.y];
        if (InRange(cur.x, line)) {
            setUndo();
...
void Source::UndoChange::create(Ptr source) {
    if (Source *src = (Source *)source) {
        File temp;
        temp.writeMem();
        temp.putInt(src->lines.elms());
        FREPA(src->lines)
        src->lines[i].saveData(temp);
        temp.pos(0);
        Compress(temp, data.writeMem(), COMPRESS_LZ4, 0, false);
        cur = src->cur;
        sel = src->sel;
...
void Source::UndoChange::apply(Ptr source) {
    if (Source *src = (Source *)source) {
        File temp;
        data.pos(0);
        Decompress(data, temp, true);
        temp.pos(0);
        src->lines.clear();
        REP(temp.getInt())
        src->lines.New().loadData(temp);
        src->cur = cur;
        src->sel = sel;
        src->changed(0, -1);
...
void Source::setUndo(UNDO_TYPE undo_type) {
    ...
    if (UndoChange *change = undos.set(undo_type, undo_type == DEFAULT || undo_type == INS_CHRS)) {
        if (!InRange(undo_original_state, undos.changes()))
            undo_original_state = -1;
        if (CE.cur() == this && modified != T.modified())
