...
void Line::setGui(Int i, GuiObj &parent) {
    text_valid = false;
    if (T.parent() != &parent)
        parent += create(S, &CE.ts).visible(!CE.view_mode());
    setRect(i);
}
/******************************************************************************/
...
void Source::validateView() {
    ...
    Int const_line = Trunc(const_full_y);
    Flt frac = const_full_y - const_line;
    if (view_lines.elms() && view_lines[0].visible())
        const_line = view_lines[Mid(const_line, 0, view_lines.elms() - 1)].line(); // top visible line

    if (CE.view_mode())
        if ((lines.elms() && !view_lines.elms()) || view_comments != CE.view_comments || view_funcs != CE.view_funcs || view_func_bodies != CE.view_func_bodies || view_private_members != CE.view_private_members)
            setView(CE.view_comments, CE.view_funcs, CE.view_func_bodies, CE.view_private_members);

    REPAO(lines).visible(!CE.view_mode());
    REPAO(view_lines).visible(CE.view_mode());

    if (CE.view_mode())
        const_line = realToView(const_line);
...
void Source::setView(Bool comments, Bool funcs, Bool func_bodies, Bool private_members) {
    ...
    FREPAD(y, lines) {
        ...
        {
            ViewLine &dest = view_lines.New();
            dest.includeLine(y);
            dest.source = this;
            FREPA(src) {
                if (!func_bodies && !func_level && src.Type(i) == TOKEN_OPERATOR && src[i] == ')') {
                ...
                if ((func_level || last_was_bracket) && src.Type(i) == TOKEN_OPERATOR && src[i] == '{') {
                    if (!func_level)
                        if (CodeLine *cl = FindLine(view_lines, bracket_line)) {
                            if (lines[bracket_line][bracket_col + 1] == ' ' && bracket_line == y)
                                cl->remove(bracket_col + 1);
                            cl->insert(bracket_col + 1, ";", TOKEN_OPERATOR);
                        }
                    func_level++;
                }
                if (!func_level)
                    dest.cols.New().set(src[i], i, y, -1, src.Type(i));
                else
                    dest.cols.New().remove();
                if (func_level && src.Type(i) == TOKEN_OPERATOR && src[i] == '}')
                    func_level--;
    ...
    if (!private_members) {
        ...
        FREPA(view_lines) {
            ...
            if (private_level && !line.cols.elms()) // if it's an empty line and we're inside 'private_level' then remove fully
            {
                ...
                FREPA(line) {
                    if (!private_level && !lines[line.line()].preproc && line.isKeyword(i, "private") && line[i + 7] == ':')
                        private_level = 1; // check for ':' to overcome "struct A : private B" issues
                    if (private_level) {
                        if (!lines[line.line()].preproc) {
                            if (line.CodeLine::type(i) == TOKEN_OPERATOR)
                                if (line[i] == '{')
                                    private_level++;
                                else if (line[i] == '}')
                                    private_level--;
                            if (private_level == 1 && (line.isKeyword(i, "public") || line.isKeyword(i, "protected")))
                                private_level = 0;
                        }
                        if (private_level)
                            line.cols[i].remove();
    ...
    // clean
    if (!comments || !funcs || !func_bodies || !private_members)
        Clean(view_lines);

    // set gui text
    FREPA(view_lines) {
        ViewLine &line = view_lines[i];
        T += line.create(S, &CE.ts);
        line.setRect(i);
    }
}
/******************************************************************************/
...
GuiObj *Source::test(C GuiPC &gpc, C Vec2 &pos, GuiObj *&mouse_wheel) {
    ...
    if (go && Kb.ctrlCmd() && prev == mouse_wheel)
        mouse_wheel = this; // if didn't change mouse wheel focus and has general focus then set wheel focus as well (needed for zooming)
    return go;
}
void Source::update(C GuiPC &gpc) {
    if (Kb.ctrlCmd() && contains(Gui.wheel())) {
        CE.zoom(Ms.wheelI());
        Ms.eatWheel();
    }
    ...
    if (slidebar[1].button[SB_LEFT_UP]() || slidebar[1].button[SB_RIGHT_DOWN]())
        if (!isCurVisible()) {
            forceCreateNextUndo();
            clearSuggestions();
            sel = -1;
    ...
    if (hasKbFocus()) {
        if (Kb.ctrlCmd() && !Kb.alt() && Kb.b(KB_UP))
            scrollUp();
        if (Kb.ctrlCmd() && !Kb.alt() && Kb.b(KB_DOWN))
            scrollDown();

        for (; Kb.k.any() && !Kb.k.winCtrl(); Kb.nextKey()) {
            if (Kb.k(KB_BACK) && !Kb.k.alt()) {
                if (Kb.k.ctrlCmd())
                    delWordBack();
                else
                    delBack();
            } else if (Kb.k(KB_DEL) && !Kb.k.shift() && !Kb.k.alt()) {
                if (Kb.k.ctrlCmd())
                    delWordForward();
                else
                    delForward();
            } else if (Kb.k(KB_LEFT) && !Kb.k.alt()) {
                if (Kb.k.ctrlCmd())
                    curPrevWord();
                else
                    curLeft();
            } else if (Kb.k(KB_RIGHT) && !Kb.k.alt()) {
                if (Kb.k.ctrlCmd())
                    curNextWord();
                else
                    curRight();
            } else if (Kb.k(KB_HOME) && !Kb.k.alt()) {
                if (Kb.k.ctrlCmd())
                    curViewBegin();
                else /*if(suggestions_region.visible())setSuggestion(0                      );else */
                    curLineBegin();
            } else if (Kb.k(KB_END) && !Kb.k.alt()) {
                if (Kb.k.ctrlCmd())
                    curViewEnd();
                else /*if(suggestions_region.visible())setSuggestion(suggestions_list.elms());else */
                    curLineEnd();
            } else if (Kb.k(KB_PGUP) && !Kb.k.alt()) {
                if (Kb.k.ctrlCmd())
                    curDocBegin();
                else if (suggestions_region.visible())
                    setSuggestion(suggestions_list.cur - Trunc(suggestions_region.slidebar[1].length() / CE.ts.lineHeight()));
                else
                    curPageUp();
            } else if (Kb.k(KB_PGDN) && !Kb.k.alt()) {
                if (Kb.k.ctrlCmd())
                    curDocEnd();
                else if (suggestions_region.visible())
                    setSuggestion(suggestions_list.cur + Trunc(suggestions_region.slidebar[1].length() / CE.ts.lineHeight()));
                ...
                if (Kb.k.shift() && Kb.k(KB_DEL))
                cut();
            else if (Kb.k.shift() && Kb.k(KB_INS))
                paste(null, !Kb.k.ctrlCmd());
            else if (Kb.k.ctrlCmd() && Kb.k(KB_INS))
                copy();
            else if (Kb.k.ctrlCmd() && Kb.k.alt() && Kb.k(KB_UP))
                curPrevLevelBracket();
            else if (Kb.k.ctrlCmd() && Kb.k.alt() && Kb.k(KB_DOWN))
                curNextLevelBracket();
            else if (!Kb.k.ctrlCmd() && Kb.k.alt() && Kb.k(KB_UP))
                curPrevBracket();
            else if (!Kb.k.ctrlCmd() && Kb.k.alt() && Kb.k(KB_DOWN))
    ...
    if (hasFocus(MT.guiObj(i))) {
        ...
        if (CE.view_mode()) {
            if (MT.bp(i, 0) || MT.bp(i, 2)) {
                ...
                if (viewToReal(view, real)) {
                    ...
                    if (getSymbolMacroID(real, symbol, macro, id)) {
                        ...
                        if (Kb.ctrlCmd()) {
                            if (symbol)
                                CE.findAllReferences(symbol());
                            else if (macro)
                                CE.findAllReferences(macro->name);
                            else if (id.valid())
                                CE.findAllReferences(id);
                        } else {
                            if (symbol)
                                CE.jumpTo(symbol());
                            else if (macro)
            ...
            if (MT.bp(i, 2) || (Kb.ctrlCmd() && MT.bp(i, 0))) {
                ...
                makeCurVisible();
            } else if (MT.b(i, 0) && sel_temp.x >= 0 && (!Overwrite || Ms.selecting())) // for Overwrite, require some mouse movement, because there initial position is calculated based on 'Trunc', but target using 'Round'
            {
                if (MT.pos(i).y >= _crect.max.y)
                    scrollUp();
                else if (MT.pos(i).y <= _crect.min.y)
                    scrollDown();
                ...
                if (sel == cur)
                    sel = -1;
            } else if (MT.bp(i, MS_BACK))
                delBack();
        }
    }
    if (Gui.ms() == &suggestions_list) {
        if (Ms.bp(0)) {
            if (suggestions_list.lit >= 0)
                suggestions_list.cur = suggestions_list.lit;
            autoComplete();
        } else if (Ms.b(1)) {
            if (suggestions_list.lit >= 0)
                suggestions_list.cur = suggestions_list.lit;
    ...
    REPA(Touches) {
        ...
        if (touch.guiObj() == &suggestions_list && touch.rs()) {
            autoComplete();
            break;
        }
    }
...
static void HighlightFind(C Str &str, C Rect &rect, C GuiPC &gpc) {
    ...
    for (Int offset = 0;;) {
        Int match_length, i = TextPosSkipSpaceI(str() + offset, CE.find.text(), match_length, CE.find.case_sensitive(), whole_word);
        if (i < 0)
            break;
        offset += i;
        Rect_LU(rect.lu() + gpc.offset + Vec2(offset * CE.ts.colWidth(), 0), match_length * CE.ts.colWidth(), CE.ts.lineHeight()).draw(ColorAlpha(YELLOW, LINEAR_GAMMA ? Sqr(0.5f) : 0.5f));
        offset += match_length;
    }
}
static void ShowElmName(C UID &id, C Rect &rect, C GuiPC &gpc, C VecI2 &range, Bool comment) {
    ...
    if (name.is()) {
        TextStyleParams ts = CE.ts_small;
        ts.align = 0;
        ts.color = Theme.colors[TOKEN_ELM_NAME];
        Rect r = rect + gpc.offset;
        Flt x = range.x * CE.ts.colWidth(), w = Max((name.length() + 0.5f) * ts.colWidth(), (range.y - range.x + 1) * CE.ts.colWidth());
        r.min.x += x;
        r.max.x = r.min.x + w; // operate on absolute values, instead of relative, to get the same exact values for previous/next lines
        Color c = (valid ? Theme.colors[TOKEN_ELM_BACKGROUND] : Color(148, 0, 0));
        if (comment)
            c = Lerp(c, Theme.colors[TOKEN_NONE], 0.7f);
...
static void ShowElmNames(C Str &str, C Rect &rect, C GuiPC &gpc, C Line *line, C Source::ViewLine *view_line) {
    ...
    VecI2 range;
    FREPA(str)
    if (TextToIDAt(str, i, id, range)) {
        Bool comment = (line ? line->Type(i) == TOKEN_COMMENT : view_line ? view_line->CodeLine::type(i) == TOKEN_COMMENT
                                                                          : false);
...
void Line::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible())
        if (rect().max.y + gpc.offset.y >= gpc.clip.min.y && rect().min.y + gpc.offset.y <= gpc.clip.max.y) {
            if (!text_valid) {
                text_valid = true;
                setTextData();
            }
            GuiPC gpc2 = gpc;
            gpc2.offset.x += CE.lineNumberSize();
            ...
            GuiPC gpc3 = gpc2;
            gpc3.offset.y -= CE.fontSpaceOffset();

            // highlight symbol
            if (source)
                if (Symbol *lit = source->lit_symbol) {
                    ...
                    REPA(tokens) {
                        ...
                        if (token == source->lit_symbol_cpp_name) {
                            if (!token.symbol) {
                                ...
                                if (!token.symbol)
                                    token.symbol = source->finalSymbol(token.source_index);
                            }
                            if (token.symbol && token.symbol->type == Symbol::FUNC_LIST)
                                if (Symbol *token_symbol = source->finalSymbol(token.source_index))
                                    token.symbol = token_symbol;
                            if (token.symbol)
                                if (lit->sameSymbol(*token.symbol)) {
                                highlight:;
                                    D.clip(gpc3.clip);
            ...
            super::draw(gpc2);
            if (CE.find.visible() && CE.find.text().is())
                HighlightFind(T, rect(), gpc3);
            if (CE.view_elm_names)
                ShowElmNames(T, rect(), gpc3, this, null);
        }
}
void Source::ViewLine::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible())
        if (rect().max.y + gpc.offset.y >= gpc.clip.min.y && rect().min.y + gpc.offset.y <= gpc.clip.max.y) {
            ...
            // gpc2.offset+=D.alignScreenToPixelOffset(Vec2(gpc2.offset.x, CE.fontSpaceOffset()+CE.ts.posY(gpc2.offset.y)));
            super::draw(gpc2);
            gpc2.offset.y -= CE.fontSpaceOffset();
            if (CE.find.visible() && CE.find.text().is())
                HighlightFind(asStr(), rect(), gpc2);
            if (CE.view_elm_names)
                ShowElmNames(asStr(), rect(), gpc2, null, this);
        }
}
/******************************************************************************/
void Source::drawSelection(C Color &color, Int y, Int min_x, Int max_x) { // #SourceOffset
    Vec2 offset = rect().lu() + T.offset();
...
void Source::drawSelection(C Color &color, C VecI2 &a, C VecI2 &b, Bool including) {
    VecI2 min = a, max = b;
    if (max.y < min.y || (max.y == min.y && max.x < min.x))
        Swap(min, max);

    if (min.y == max.y)
        drawSelection(color, min.y, min.x, max.x + including);
    else {
        drawSelection(color, min.y, min.x, -1);
        for (Int y = min.y + 1; y < max.y; y++)
            drawSelection(color, y, 0, -1);
...
void Source::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible()) {
        ...
        if (!CE.view_mode()) {
            // draw selection
            if (sel.x >= 0 && sel.y >= 0)
                drawSelection(Theme.colors[TOKEN_SELECT], sel, cur);

            // draw braces highlight
            if (InRange(cur.y, lines)) {
                VecI2 test = cur;
                Char c = lines[test.y][test.x];
                if (c != '{' && c != '}' && c != '(' && c != ')' && c != '[' && c != ']')
                    c = lines[test.y][--test.x]; // try the previous char
                if (c == '{' || c == '}' || c == '(' || c == ')' || c == '[' || c == ']') {
                    TOKEN_TYPE type = MainType(lines[test.y].Type(test.x));
                    VecI2 pos = test;
                    Int level = 0;
                    if (c == '{') {
                        for (inc(pos); posValid(pos); inc(pos)) {
                            Char c = lines[pos.y][pos.x];
                            if (type == MainType(lines[pos.y].Type(pos.x)))
                                if (c == '{')
                                    level++;
                                else if (c == '}')
                                    level--;
                        ...
                        for (dec(pos); posValid(pos); dec(pos)) {
                            Char c = lines[pos.y][pos.x];
                            if (type == MainType(lines[pos.y].Type(pos.x)))
                                if (c == '{')
                                    level++;
                    ...
                    if (pos != test && posValid(pos)) {
                        Rect_LU(offset + posVisual(pos), CE.ts.colWidth(), CE.ts.lineHeight()).draw(Theme.colors[TOKEN_BRACE_HIGHLIGHT]);
                        drawSelection(Theme.colors[TOKEN_BRACE_HIGHLIGHT], pos, test, true);
                    }
                }
            }
            ...
            if (Theme.colors[TOKEN_LINE_HIGHLIGHT].a) {
                Int line = cur.y;
                if (CE.view_mode())
                    line = realToView(line);
                Vec2 pos = offset + posVisual(VecI2(0, line));
                Rect rect(_crect.min.x, pos.y - CE.ts.lineHeight(), _crect.max.x, pos.y);
                Flt h = CE.ts.lineHeight() * 0.06f;
                if (h <= D.pixelToScreenSize().y)
                    rect.draw(Theme.colors[TOKEN_LINE_HIGHLIGHT], false);
                else
                    rect.drawBorder(Theme.colors[TOKEN_LINE_HIGHLIGHT], h);
            }
        }

        if (InRange(highlight_line, lines) && highlight_time > 0) {
            Int line = highlight_line;
            if (CE.view_mode())
                line = realToView(line);
            Vec2 pos = offset + posVisual(VecI2(0, line));
            ...
            if (viewToReal(view, real)) {
                ...
                if (getSymbolMacroID(real, symbol, macro, lit_elm_id, &x_range)) {
                    ...
                    if (macro && macro->source) {
                        lit_token_source = macro->source;
                        lit_token_line = macro->line;
                    }
                    if (symbol && symbol->source) {
                        Int token = symbol->token_index;
                        if (InRange(token, symbol->source->tokens)) {
                            lit_token_source = symbol->source;
                            lit_token_line = symbol->source->tokens[token]->lineIndex();
                        }
                    }
                    if (lit_token_source == this && lit_token_line >= 0) {
                        ...
                        if (Cuts(lit_rect, screen_crect))
                            lit_token_source = null; // if definition is on the screen then don't draw its preview
                    }

                    // highlight token
                    x_range.x = view_lines[view.y].findCol(x_range.x);
                    x_range.y = view_lines[view.y].findCol(x_range.y);
                    if (x_range.x >= 0 && x_range.y >= 0)
                        drawSelection(Theme.colors[TOKEN_SELECT], view.y, x_range.x, x_range.y + 1);
                }
            }
        }
        ...
        if (hasKbFocus() && !Kb._cur_hidden && App.active()) {
            Vec2 pos = offset + posVisual(cur);
            pos += D.pixelToScreenSize(VecI2(1, 0));
            if (Overwrite && sel.x < 0)
                DrawKeyboardCursorOverwrite(pos, CE.ts.lineHeight(), CE.ts.colWidth()); // don't draw overwrite if we have selection
            else
                DrawKeyboardCursor(pos, CE.ts.lineHeight());
        }

        // draw line numbers
        if (CE.options.line_numbers()) {
            Rect r = _crect; /*r.min.x+=D.pixelToScreenSize().x*0.5f;*/
            r.max.x = r.min.x + CE.lineNumberSize();
            ...
            TextStyleParams ts = CE.ts_small;
            ts.color = Theme.colors[TOKEN_LINE_NUM];
            ts.align.set(1, -1);
            ts.space.y = CE.ts.lineHeight() / ts.size.y;
            Char8 text[1024], temp[256];
            text[0] = 0;
            Int lines = Ceil(r.h() / CE.ts.lineHeight()) + 1;
            Clamp(lines, 0, 1024);
            FREP(lines) {
                Int li = line_add + i;
                Append(text, TextInt(CE.view_mode() ? viewToReal(li) : li, temp));
                Append(text, "\n");
            }
        ...
        {
            Int c;
            if (findPrevToken(cur, c))
                for (Int i = c, level = 0; i >= 0; i--) {
                    Int func_i = i;
                    ...
                    if (func.type == TOKEN_CODE && level > 0) {
                        if (InRange(func_i + 1, tokens)) {
                            if (CE.view_elm_names && func == "UID" && func.line) // don't display info for this function if we're inside project element UID(..), instead continue to previous function "func(|UID(..))"
                            {
                                UID id;
                                VecI2 range;
                                if (TextToIDAt(*func.line, func.col, id, range))
                                    if (CE.cei().idToText(id).is())
                                        continue; // only if it's a text ID
                            }
                                    ...
                                    if (symbol->type == Symbol::FUNC || symbol->type == Symbol::FUNC_LIST || symbol->isVar() || symbol->type == Symbol::CLASS)            // function, operator, constructor (through class "Vec2(0, 0)" or default value "Vec2 x(0, 0)")
                                        if (!(symbol->type == Symbol::FUNC && func.def_decl && (symbol->params.elms() || (symbol->modifiers & Symbol::MODIF_FUNC_BODY)))) // don't use when defining function (and typing params)
                                        {
                                            ...
                                            if (symbol->type == Symbol::CLASS) {
                                                if (func == *symbol && func.parent == symbol)
                                                    symbol = null;                         // defining constructor/destructor, don't list parameters
                                                if (symbol && InRange(func_i - 2, tokens)) // X.X  X.~X // defining constructor/destructor, don't list parameters
                                                {
                                                    ...
                                                    if (token == '.' && prev == func || token == '~' && prev == '.' && (InRange(func_i - 3, tokens) && *tokens[func_i] == func))
                                                        symbol = null;
                                                }
                                                if (symbol)
                                                    symbol = SymbolPtr().find(symbol->full_name + SEP + *symbol)(); // set symbol to constructor function list
                                            }
                                            if (symbol)
                                                for (; i < tokens.elms();) {
                                                    Token &op = *tokens[i++];
                                                    if (op == '(') {
                                                        ...
                                                        Memc<Expr> params;
                                                        if (InRange(i, tokens))
                                                            for (Int level = 0, param_start = i;; i++) {
                                                                Token &token = *tokens[i];
                                                                if (token == '(' || token == '[' || token == TMPL_B)
                                                                    level++;
                                                                else if (token == ')' || token == ']' || token == TMPL_E)
                                                                    level--;
                                                        ...
                                                        if (!force_ctor && symbol->type == Symbol::FUNC && symbol->parent && symbol->parent->type == Symbol::FUNC_LIST)
                                                            symbol = symbol->parent(); // display all functions
                                                        if (!force_ctor && symbol->type == Symbol::FUNC)
                                                            AddMatch(matches, symbol, true, params.data(), params.elms(), templates, compiler);
                                                        else if (!force_ctor && symbol->type == Symbol::FUNC_LIST)
                                                            AddMatches(matches, symbol->funcs, true, params.data(), params.elms(), templates, compiler);
                                                        else {
                                                            // variable
                                                            if (symbol = GetFinalSymbol(symbol))
                                                                if (symbol->type == Symbol::CLASS) {
                                                        ...
                                                        if (matches.elms()) {
                                                            ...
                                                            Int match = matches.last().lowest_match;
                                                            REPA(matches)
                                                            MAX(match, matches[i].lowest_match);
                                                            FREPA(matches) {
                                                            ...
                                                            Flt w;
                                                            CE.ts_small.textLines((CChar *)null, sx.data(), sx.elms(), 0, false, &w);
                                                            ...
                                                            Flt ext = 0.01f;
                                                            Rect_LU r(fp.x, suggestions_region.visible() ? pos.y - size.y - ext : fp.y - CE.ts.lineHeight() - ext, w, 0);
                                                            if (r.max.x >= _crect.max.x)
                                                                r -= Vec2(r.max.x - _crect.max.x, 0);
        ...
        if (Suggestion *sugg = suggestions_list()) {
            ...
            if (symbol || sugg->is_macro) {
                ...
                if (symbol) {
                    ...
                    if (symbol->type == Symbol::FUNC_LIST && funcs.elms() == 1)
                        symbol = funcs[0];
                ...
                Rect_LU r(pos + Vec2(size.x + space, 0), w, 0);
                if (r.max.x >= D.w())
                    r -= Vec2(r.max.x - pos.x + space, 0);
                if (r.min.x < rect().min.x)
                    r += Vec2(pos.x + size.x + space - r.min.x, 0);
                r.min.y = r.max.y - CE.ts_small.textHeight((CChar *)null, sx.data(), sx.elms(), r.w(), true);

                if (Gui.skin) {
                    Rect rect = r;
                    rect.extend(0.01f);
                    if (Gui.skin->region.normal)
                        Gui.skin->region.normal->draw(Gui.skin->region.normal_color, rect);
                    else if (Gui.skin->region.normal_color.a)
                        rect.draw(Gui.skin->region.normal_color);
                }
                D.text(CE.ts_small, r, (CChar *)null, sx.data(), sx.elms(), true);
            }
            if (sugg->elm_id.valid()) // project element
            {
                ...
                Flt space = D.pixelToScreenSize().x * 24;
                Rect_LU r(rect.lu() + Vec2(rect.w() + space, 0), rect.size());
                if (r.max.x >= D.w())
                    r -= Vec2(r.max.x - rect.min.x + space, 0);
                if (r.min.x < rect.min.x)
                    r += Vec2(rect.max.x + space - r.min.x, 0);
        ...
        if (CE.view_mode() && hasMsFocus()) {
            ...
            if (InRange(l, view_lines)) {
                ...
                if (view_line.rect().w() >= clientWidth() * 0.95f) {
                    ...
                    if (rect.min.y - 0.05f <= pos.y - clientHeight())
                        rect += Vec2(0, rect.h() + 0.15f * 2);
                    Rect rect_e = rect;
                    rect_e.extend(0.02f);
        ...
        if (lit_token_source && lit_token_line >= 0) {
            if (lit_token_source != this)
                lit_token_source->prepareForDraw();

            GuiPC gpc;
            gpc.visible = true;
            gpc.enabled = true;
            gpc.clip = (_crect - _crect.lu() + screenPos() + _crect.lu() - rect().lu()).extend(-0.05f);
            gpc.clip.min.x = gpc.clip.lerpX(0.6f);
