...
Token *Source::TokenSource::previewNext() {
    if (tokens && InRange(token_index, *tokens))
        return &(*tokens)[token_index]; // first try custom token list
    if (source)
        return source->previewNextToken(*line_index, *line_token_index); // then  try tokens from the source
    return null;
}
Token *Source::TokenSource::next(Bool only_cur_line) {
    if (tokens && InRange(token_index, *tokens))
        return &(*tokens)[token_index++]; // first try custom token list
    if (source)
        return source->getNextToken(*line_index, *line_token_index, only_cur_line); // then  try tokens from the source
    return null;
}
void Source::TokenSource::removeLastRead() {
    if (tokens && token_index > 0)
        tokens->remove(--token_index, true);
}
/******************************************************************************/
Token *Source::previewNextToken(Int line_index, Int line_token_index) {
    if (InRange(line_index, lines)) {
        Line &line = lines[line_index];
        if (InRange(line_token_index, line.tokens))
            return &line.tokens[line_token_index];
...
Token *Source::getNextToken(Int &line_index, Int &line_token_index, Bool only_cur_line) {
    for (;;) {
        ...
        Line &line = lines[line_index];
        if (InRange(line_token_index, line.tokens))
            return &line.tokens[line_token_index++]; // if we still have tokens in current line
        if (only_cur_line)
            return null;
        line_token_index = 0;
        line_index++;
        if (InRange(line_index, lines))
            lines[line_index].resetPreproc(true); // need to proceed to next line, when doing so we need to initialize its macro state
    }
}
/******************************************************************************/
Int Source::findMacro(Memc<Macro> &macros, BStr &macro, Mems<Bool> *macro_used) {
    Int index;
    Bool found = macros.binarySearch(macro, index, CompareCS);
    if (found && macro_used && InRange(index, *macro_used) && (*macro_used)[index])
        found = false; // if macro was already used then skip
...
Int Source::findMacroToReplace(Memc<Macro> &macros, BStr &macro, TokenSource &ts, Mems<Bool> *macro_used) {
    ...
    if (InRange(index, macros) && macros[index].params >= 0) // if macro was defined with (..) then require source tokens to have (..) as well, otherwise don't replace (this is how C++ works)
    {
        ...
        if (!(next && *next == '('))
            return ~index; // if next token is not '(' then don't detect the macro replacement
    }
    return index;
}
...
void Source::replaceMacro(Int macro_index, Memc<Macro> &macros, TokenSource &ts, Mems<Bool> &macro_used, Int depth, Int col, Line &line) {
            ...
            if (*token == '(')
                for (Int level = 0; Token *token = ts.next();) {
                    ...
                    if (!level && *token == ',')
                        ...
                        params.last().tokens.add(*token);
                }

        // remove read elements from token buffer
        REP(ts.token_index - prev_pos)
        ts.removeLastRead();
    ...
    FREPA(macro.parts) // iterate through all parts of the macro
    {
        ...
        for (; start < token_index; start++)
            ...
            {
                if (*token == "__LINE__") {
                    token->BStr::setCustom(S + line.line);
                    token->type = TOKEN_NUMBER;
                } // replace '__LINE__' with code line
            }

        Int last = token_index - 1,
            first = token_index - 3;

        if (InRange(first, *ts.tokens) && InRange(last, *ts.tokens) && (*ts.tokens)[first + 1] == "##") // if previous token was merger then merge the neighbours
        {
            Token &a = (*ts.tokens)[first],
                  &b = (*ts.tokens)[last];
            a.BStr::setCustom(a.asStr() + b.asStr());
            ts.tokens->remove(first + 1, true).remove(first + 1, true);
            token_index -= 2;
        }
    }
    ...
    for (; InRange(ts.token_index, *ts.tokens);) {
        Token &token = (*ts.tokens)[ts.token_index];
        if (token.macro_depth < depth)
            break; // don't process parent tokens (process only from this or lower level)
        ...
        {
            if (!macro_used.elms())
                macro_used.setNumZero(macros.elms());
            macro_used[macro_index] = true; // we must specify which macros were used
            ts.removeLastRead();
            replaceMacro(macro_index2, macros, ts, macro_used, depth + 1, col, line);
        }
    }
    if (InRange(macro_index, macro_used))
        macro_used[macro_index] = false; // macro.used=false;
}
/******************************************************************************/
Bool Source::getConditionalValue(Memc<Token> &tokens, Memc<Macro> &macros, Line &line) {
    ...
    if (tokens[i] == "defined") {
        ...
        if (InRange(i, tokens) && tokens[i] == '(')
            tokens.remove(i, true).remove(i + 1, true); // remove '(' ')' in "defined(x)"
        if (InRange(i, tokens)) {
    ...
    FREPA(tokens) {
        ...
        if (token.type == TOKEN_CODE || token.type == TOKEN_KEYWORD) {
            ...
            if (macro_index >= 0) {
                ...
                // replace macro
                REPAO(macro_used) = false;
                replaceMacro(macro_index, macros, ts, macro_used, 1, 0, line);
...
void Source::preprocess(Memc<Macro> &macros, Int &line_index, Memc<Token *> &temp, Bool allow_defines, Memc<PrepCond> &prep_if) {
    ...
    Line &line = lines[line_index];
    line.resetPreproc(false);
    if (vis)
        line.tokens_preproc_condition_unavailable = false;
                    ...
                    if (!prep_if.elms() || prep_if.last().currently_valid) {
                        if (Token *name = tokens.addr(2))
                            if (name->type == TOKEN_CODE || name->type == TOKEN_KEYWORD) {
                                ...
                                Macro &macro = ((index >= 0) ? macros[index].reset() : macros.NewAt(~index).set(*name));
                                macro.def = line;
                                Int i = 3;
                                temp.clear();
                                ...
                                for (; Token *token = getNextToken(line_index, i, true);) {
                                    ...
                                    {
                                        if (!InRange(line_index + 1, lines))
                                            break; // if there is no next line then break
                                        i = 0;     // start with first token in next line
                                        Line &line = lines[++line_index];
                                        line.resetPreproc(false);
                                        if (vis)
                                            line.tokens_preproc_condition_unavailable = false; // increase line counter and initialize the next line
                                    } else {
                                        if (token->type == TOKEN_CODE)
                                            REPA(temp)
                                            if ((*temp[i]) == *token) {
                        ...
                        line.tokens_preproc_condition_unavailable = true;
                } else if (tokens[1] == "undef" && allow_defines) // remove macro
                {
                    if (!prep_if.elms() || prep_if.last().currently_valid) {
                    ...
                    // TODO: line.ends_with_preproc
                    if (!prep_if.elms() || prep_if.last().currently_valid) {
                        for (Int i = 2; i < tokens.elms(); i++)
                            line.tokens_preproc.New() = tokens[i];
                    ...
                    if (vis && prep_if.elms() >= 2 && !prep_if[prep_if.elms() - 2].currently_valid)
                        line.tokens_preproc_condition_unavailable = true;
                    ...
                    prep_if.removeLast();
                    if (vis && prep_if.elms() && !prep_if.last().currently_valid)
                        line.tokens_preproc_condition_unavailable = true;
                } else
    ...
    } else if (prep_if.elms() && !prep_if.last().currently_valid) {
        // if there was a preprocess condition which wasn't met, then no actual tokens should be used for the code compilation
        if (vis)
            line.tokens_preproc_condition_unavailable = true;
        line.tokens_preproc_use = true;
        line.tokens_preproc.clear();
    } else if (macros.elms()) // normal line, try to replace tokens with macros
    {
        Mems<Bool> macro_used;
        FREPA(tokens) {
            Token &token = tokens[i];
            if (token.type == TOKEN_CODE || token.type == TOKEN_KEYWORD) {
                ...
                if (macro_index >= 0) // we've encountered token which should be overriden by macro
                {
                    ...
                    token.macro = true;
                    for (Int j = 0; j < i; j++)
                        line.tokens_preproc.add(tokens[j]); // copy all tokens up to now into preprocessed token buffer
                    ...
                    // replace macro
                    REPAO(macro_used) = false;
                    replaceMacro(macro_index, macros, ts, macro_used, 1, token.col, *token.line);
...
void Source::preprocess(Int from, Int num) {
    ...
    if (active) {
        ...
        {
            MIN(from, preproc_line_changed);
            preproc_line_changed = -1;
        ...
        if (from <= to) {
            // preprocess tokens
            if (from > 0 && (lines[from - 1].starts_with_macro_param || lines[from - 1].starts_with_preproc || lines[from - 1].tokens_preproc_condition_unavailable))
                from--;
            for (; from > 0 && (lines[from].starts_with_macro_param || lines[from].starts_with_preproc || lines[from].tokens_preproc_condition_unavailable); from--)
                ; // go to first line                which doesn't start with macro param or preprocess
            for (; InRange(to + 1, lines) && (lines[to + 1].starts_with_macro_param || lines[to + 1].starts_with_preproc); to++)
                ; // expand range to cover all lines which         start with macro param or preprocess
            ...
            Int cur = from;
            for (; cur <= to; cur++)
                preprocess(ProjectMacros, cur, temp, false, prep_if); // preprocess lines
            FREPA(view_lines) {
                ViewLine &line = view_lines[i];
                if (line.line() >= from && line.line() < cur)
                    line.text_valid = false;
            } // force text reset on all processed lines (use 'cur' because it may be bigger than 'to')
        }
    }
    setTokenPtrs();
}
