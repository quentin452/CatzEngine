...
void SuggestionsUsed(C Str &t) {
    ...
    Suggestions.add(t);
    if (Suggestions.elms() > 16)
        Suggestions.remove(0, true);
}
/******************************************************************************/
// Factors:
...
static Int SameChars(CChar *t, CChar *s, Int &score) // assumes that 't' and 's' != null
{
    ...
    for (;;) {
        Char ct = *t++,
             cs = *s++;
        if (!Equal(ct, cs) || !ct || ct == ' ')
            break; // skip space to avoid situations where "123 456 789" has higher score for "ABC\456 789" than "123\456 789"
        if (CharFlag(ct) & CHARF_UP)
            sc += ((cs == ct) ? 1 : -1); // check case only if was specified
        same++;
    }
    sc += same * 2;
...
Int SuggestionsPriority(C Str &s, C Str &t, Bool all_up_case) // 's'=suggestion, 't'=text
{
    ...
    for (Int ti = 0; ti < t.length();) // check every part of 't' text
    {
        ...
        if (*t1 == ' ')
            ti++;
        else // don't add score for separators
        {
            ...
            ti += best_same_chars;
            if (best_same_chars == s.length() && s.length() == t.length() && Equal(s, t, true))
                total_score++; // if this is an exact match of entire strings, then boost priority
        }
    }
    return total_score;
}
...
static void BestMatch(CChar *s, Int s_length, CChar *t, Int t_length, Match &match) {
    ...
    FREPD(ti, t_length) {
        CChar *t1 = t + ti;
        if (*t1 != ' ') // skip checking spaces because they're typically used to seperate multiple words
        {
            ...
            FREPD(si, s_length) {
                ...
                for (;;) {
                    ...
                    if (!Equal(s, t))
                        break;
                    if (CharFlag(t) & CHARF_UP)
                        score += ((s == t) ? 1 : -1); // check case only if was specified
                    length++;
...
Int SuggestionsPriority(C Str &s, C Str &t, Bool all_up_case) // 's'=suggestion, 't'=text !! Warning: this is not thread-safe because of one global 'srcs', 'texts' !!
{
    ...
    for (; srcs.elms() && texts.elms();) {
        ...
        best_match.score = 0;
        FREPAD(si, srcs) {
            C PosLength &src = srcs[si];
            CChar *s1 = s() + src.pos;
            FREPAD(ti, texts) {
                C PosLength &text = texts[ti];
                Match m;
                BestMatch(s1, src.length, t() + text.pos, text.length, m);
        ...
        if (best_match.score > 0) {
         ...
         same for text
            Int s_ofs=m.s_pos+m.length;
         CChar *s1 = s + s_ofs;
         Int s1_length = s_length - s_ofs; // s after match
         Int t_ofs = m.t_pos + m.length;
         CChar *t1 = t + t_ofs;
         Int t1_length = t_length - t_ofs; // t after match
         t, m.t_pos t1, t1_length s, m.s_pos s1, s1_length
        }
    }
    return score;
}
...
static Int ComparePriority(C Source::Suggestion &a, C Source::Suggestion &b) {
    if (Int c = Compare(a.priority, b.priority))
        return -c;
    if (Int c = Compare(a.order(), b.order()))
        return -c;
    if (Int c = Compare(a.text.length(), b.text.length()))
        return c;                         // prefer shorter first
...
// SOURCE SUGGESTION
/******************************************************************************/
Int Source::Suggestion::order() C {
    if (_order == OrderUnknown)
        _order = SuggestionsOrder(text);
    return _order;
}

void Source::Suggestion::set(Int priority, C Str &text, Symbol &symbol) {
    T.priority = priority;
    T._order = OrderUnknown;
    T.text = text;
    T.display = text;
    T.symbol = &symbol;
    T.elm_id.zero();
    T.is_macro = false;
    T.macro_params = 0;
    if (symbol.type == Symbol::FUNC || symbol.type == Symbol::FUNC_LIST)
        T.display += ELLIPSIS;
    T.icon = null;
}
void Source::Suggestion::set(Int priority, C Macro &macro) {
    T.priority = priority;
    T._order = OrderUnknown;
    T.text = macro.name;
    T.display = text;
    ...
    T.elm_id.zero();
    T.is_macro = true;
    T.macro_params = Mid(macro.params + 1, 0, 2);
    macro_def = macro.def;
    T.icon = null;
}
void Source::Suggestion::set(Int priority, C Str &text, C UID &id, C ImagePtr &icon) {
    T.priority = priority;
    T._order = OrderUnknown;
    T.text = text;
    T.display = text;
...
void Source::refreshSuggestions() {
    Memc<CodeEditorInterface::ElmLink> elms;
    CE.cei().getProjPublishElms(elms);
    ...
    if (t.is()) // if we have some text typed
    {
        FREPA(elms) {
            ...
            if (Int p = SuggestionsPriority(elm.full_name, t, false))
                suggestions.New().set(p, elm.full_name, elm.id, elm.icon);
        }
        suggestions.sort(ComparePriority); // sort by priority
        if (suggestions.elms() > 64)
            suggestions.setNum(64); // limit to max 64 suggestions
    } else {
        FREPA(elms)
        suggestions.New().set(0, elms[i].full_name, elms[i].id, elms[i].icon);
...
void Source::listSuggestions(Int force) {
    ...
    if (InRange(cur.y, lines)) {
        Line &l = lines[cur.y];
        if (l.tokens_preproc_condition_unavailable)
            return; // if inside inactive block then disable
        ...
        if (l.Type(cur.x - 1) == TOKEN_CODE || l.Type(cur.x - 1) == TOKEN_KEYWORD || l.Type(cur.x - 1) == TOKEN_PREPROC) {
            ...
            if (l[suggestions_pos.x - 1] == '#')
                suggestions_pos.x--;
            for (Int i = suggestions_pos.x; i < cur.x; i++)
                cur_text += l[i];
        } else if (l.Type(cur.x - 1) == TOKEN_COMMENT)
            return;
        else // don't list suggestions if in comment
        {
            Int i;
            if (Token *sep = findPrevToken(cur, i)) {
                if (*sep == '.' || *sep == "->" || *sep == "::") {
                    suggestions_pos = cur;
                    if ((force >= 0) ? true : (sep->lineIndex() == cur.y && sep->col + sep->length() == cur.x))
                        force = 1;
                }
            }
        }
    ...
    if (cur_text.is() || force > 0) {
        ...
        Int t;
        Token *token = findToken(suggestions_pos, t);
        Symbol *caller = (token ? token->parent : null);
        if (token && token->def_decl && force <= 0)
            return; // if token is definition/declaration then skip suggestions
        if (!token)
        ...
        if (InRange(suggestions_pos.y, lines)) {
            ...
            if (l.preproc && l.tokens.elms() && suggestions_pos.x >= l.tokens[0].col + l.tokens[0].length() || l.starts_with_preproc)
                return;
        }

        Bool precise_parent = false, ctor_init = false, allow_self = true;
        Expr parent;
        parent.symbol = caller; // set initial parent as the 'caller'
        if (token) {
            if (token->ctor_initializer) {
                ...
                if (!(*token == '.' || *token == "->" || *token == "::"))
                    token = (InRange(t - 1, tokens) ? tokens[--t] : null);
                if (token)
                    if (*token == '.' || *token == "->" || *token == "::") // 't' now points to the separator
                    {
                        precise_parent = true;
                        parent.symbol.clear(true);
                        evaluateSymbol(t, parent); // calculate correct parent, we DO need to calculate from 'i' (and include the '.' separator) because expressions like "(TYPE*)obj." without the '.' separator would look like "(TYPE*)obj" and always return obj already casted to some type and be a pointer, which we don't want, we want the original "obj." first
        ...
        if (cur_text[0] != '#' && !precise_parent) {
            Memc<Macro> &macros = ProjectMacros /*macrosForLine(suggestions_pos.y)*/;
            REPA(macros) {
                Macro &m = macros[i];
                if (m.use_for_suggestions)
                    if (Int p = (cur_text.is() ? SuggestionsPriority(m.name, cur_text, m.all_up_case) : 1))
                        suggestions.New().set(p, m);
            }
        } // list macros
        if (cur_text[0] == '#' && !precise_parent) {
            REPA(Symbols) {
...
void Source::autoComplete(Bool auto_space, Bool set_undo, Bool auto_brace, Bool call_changed, Bool ignore_params) {
    ...
    if (Suggestion *sugg = suggestions_list()) {
        ...
        if (!Const) {
            Str text = sugg->text;
            if (sugg->elm_id.valid())
                text = sugg->elm_id.asCString();
            exist(suggestions_pos.x, suggestions_pos.y);
            Line &line = lines[suggestions_pos.y];
            if (Equal(text, "else", true) && line[suggestions_pos.x - 1] == ' ' && line[suggestions_pos.x - 2] == ';')
                suggestions_pos.x--; // if we've typed "; else" then remove the space before 'else'
            ...
            if (auto_space) {
                auto_space = false;
                if (sugg->symbol && (sugg->symbol->modifiers & Symbol::MODIF_FOLLOW_BY_SPACE))
                    auto_space = true;
                else {
                    changed(cur.y);
                    changed_called = true; // needed for 'isDeclaration'
                    Int i;
                    if (Token *token = findPrevToken(cur, i))
                        if (token->symbol && (token->symbol->modifiers & Symbol::MODIF_DATA_TYPE) && !token->symbol->templates.elms() && isDeclaration(token->symbol(), i))
                            auto_space = true; // don't insert space if we're using class with templates (like "Memc<>")
                }
                if (auto_space) {
                    if (Equal(text, "else", true) && line.Type(cur.x) == TOKEN_NONE)
                        cur.x++;
            ...
            if (auto_brace && (Equal(text, "if", true) || Equal(text, "for", true) || Equal(text, "while", true) || Equal(text, "switch", true) || sugg->symbol && (sugg->symbol->type == Symbol::FUNC || sugg->symbol->type == Symbol::FUNC_LIST) || sugg->is_macro && sugg->macro_params)) {
                if (line[cur.x] != '(') {
                    line.insert(cur.x++, "()", TOKEN_OPERATOR);
                    changed_called = false; // set cursor inside the brackets
                    // if the function does not have parameters, then set the cursor after brackets
                    if (sugg->is_macro) {
                        if (sugg->macro_params == 1)
                            cur.x++;
                    } else if (sugg->symbol) {
                        if (sugg->symbol->type == Symbol::FUNC && (ignore_params || sugg->symbol->params.elms() == 0))
                            cur.x++;
                        else if (sugg->symbol->type == Symbol::FUNC_LIST) {
            ...
            if (call_changed && !changed_called)
                changed(cur.y);
        }
        makeCurVisible();
        clearSuggestions();
    ...
    if (InRange(start, tokens)) {
        ...
        for (Int level = 0, i = start; i >= 0; i--) {
            ...
            default:
                if (token == TMPL_B)
                    level++;
                else if (token == TMPL_E)
                    level--;
                break;
            }
            if (!(!start_parent || start_parent->contains(token.parent)))
                break; // break if the token.parent is not a child of start_parent (basically break if we've reached some parent of the start_parent)
            if (level > 0)
                break;
            if (level < 0)
                from = i;
            else {
                if (token.type == TOKEN_KEYWORD && (token == "return" || token == "do" || token == "else"))
                    break; // return (a+b).x; - break on encountering command keyword

                if (token.type == TOKEN_KEYWORD || token.type == TOKEN_CODE || (token.type == TOKEN_OPERATOR && (token == '(' || token == ')' || token == '[' || token == ']' || token == TMPL_B || token == TMPL_E || token == '.' || token == "->" || token == "::")) || final >= 0) // for 'final' we need full formula
                    from = i;
                else
                    break;
            }
...
Symbol *Source::finalSymbol(Int final, Bool allow_func_lists) {
    if (InRange(final, tokens)) {
        ...
        if (!allow_func_lists) // move cursor from "func|()" to "func()|" to include parameters for 'func' in order to detect correct polymorphic function
        {
            ...
            for (Int level = (start_token == '(' || start_token == '[' || start_token == TMPL_B); start + 1 < tokens.elms();) {
                ...
                if ((token == ',' || token == ':') && !level)
                    break; // skip ':' for  ctor initializers, and for "x ? y : z", but not for "func(x ? y : z)" in case we're calculating 'func'
        ...
        Expr expr;
        if (evaluateSymbol(start, expr, final, allow_func_lists))
            return expr.symbol();
    }
    return null;
}
