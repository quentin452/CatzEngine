...
void LineMode::setTokens(Line &line) {
    ...
    FREPA(s) {
        ...
        if (TokenType(t)) {
            ...
            token.setBorrowed(s() + i, 1, i, line, t);
            if (t == TOKEN_OPERATOR) {
                Char c = s[i + 1];
                switch (s[i]) {
                case '<':
                    ...
                    break; // <= <<= (<< is not processed here due to possible template problems, processed later)
                case '>':
                    if (c == '=') {
                        token.extend();
                        i++;
                ...
                if (t == TOKEN_CODE) {
                    ...
                    Keyword *keyword;
                    if (preproc && !starts_with_preproc && tokens.elms() == 2)
                        goto preproc_only; // if we're processing the "#TOKEN"
                    ...
                    preproc_only:;
                        if (BinaryHas(PreprocKeywords, PreprocKeywordsElms, bs, CompareCS))
                            t = TOKEN_PREPROC;
                    }
                    if (t != token.type) {
                        token.type = t;
    ...
    for (;;) {
        ...
        if (c == '\\') // check for escape characters
        {
            ...
            if (!c)
                break;
            if (c == 'x' || c == 'u' || c == 'U')
                return true; // Warning: for simplicity we assume that the next char is a wide char
            continue;
        }
        if (!c || c == stop)
            break;
...
void LineMode::resetType(Line &line) // this just resets the 'type' without changing the tokens
{
    ...
    FREPA(s) {
        ...
        if (ignore_char) {
            ...
            if (!ignore_rest) {
                ...
                if (TextType(t) && c == '\\')
                    ignore_char = true;
                else if (t != TOKEN_COMMENT && !TextType(t)) {
                    if (c == '/' && s[i + 1] == '*') {
                        t = TOKEN_COMMENT;
                        type.add(t);
                        i++;
    ...
    if (t != TOKEN_COMMENT || ignore_rest)
        t = TOKEN_NONE;
...
Bool LineMode::load(File &f, StrLibrary &sl, C Str &text, Line &line, Str &temp) {
    Byte flag;
    f >> flag;
    starts_with_comment = FlagOn(flag, SWC);
    ends_with_comment = FlagOn(flag, EWC);
    preproc = FlagOn(flag, PRP);
    starts_with_preproc = FlagOn(flag, SWP);
    ends_with_preproc = FlagOn(flag, EWP);
    starts_with_macro_param = FlagOn(flag, SWM);
    if (type.loadRaw(f)) {
...
Int Line::wordStart(Int x) {
    Clamp(x, 0, length() - 1);
    CHAR_TYPE type = CodeCharType(T[x]);
    for (; x > 0 && CodeCharType(T[x - 1]) == type && !Individual(T[x - 1]); x--)
        ;
    VecI2 range;
    if (CE.view_elm_names) {
        UID id;
        if (TextToIDInside(T, x, id, range))
            return range.x;
    }
    if (LineFileName(T, x, range))
        return range.x;
    return x;
}
...
void Line::setTextData() // must be in sync with "Source::ViewLine::setTextData"
{
    ...
    if (tokens_preproc_condition_unavailable) {
        ...
        if (tokens.elms()) {
            token_org_cur = tokens.data();
            token_org_end = token_org_cur + tokens.elms();
        } else
            token_org_cur = null; // set null when there are no elements
        if (Tokens().elms()) {
            token_prc_cur = Tokens().data();
            token_prc_end = token_prc_cur + Tokens().elms();
        } else
            token_prc_cur = null; // set null when there are no elements
            ...
            if (last_type != type) {
                ...
                if (token_prc_cur) {
                    for (; token_prc_cur->col < i;) {
                        token_prc_cur++;
                        if (token_prc_cur == token_prc_end) {
                            token_prc_cur = null;
                            break;
                        }
                    }
                    if (token_prc_cur && token_prc_cur->col == i)
                        if (token_prc_cur->macro)
                            type = TOKEN_KEYWORD;
                        else if (Symbol *symbol = token_prc_cur->symbol()) {
                            if (symbol->type == Symbol::ENUM)
                                type = TOKEN_ENUM_TYPE;
                            else if (symbol->type == Symbol::ENUM_ELM)
                                type = TOKEN_ENUM_ELM;
                            else if ((symbol->type == Symbol::FUNC_LIST || symbol->type == Symbol::FUNC) && symbol->isGlobalOrStatic())
                                type = TOKEN_FUNC;
                            else if ((symbol->modifiers & Symbol::MODIF_DATA_TYPE) || symbol->type == Symbol::NAMESPACE)
                                type = TOKEN_KEYWORD;
                        }
                }
                ...
                if (token_org_cur) {
                    for (; token_org_cur->col < i;) {
                        token_org_cur++;
                        if (token_org_cur == token_org_end) {
                            token_org_cur = null;
                            break;
                        }
                    }
                    if (token_org_cur && token_org_cur->col == i && token_org_cur->macro)
                        type = TOKEN_MACRO;
                }
...
void Source::ViewLine::setTextData() // must be in sync with "Line::setTextData"
{
    ...
    if (Line *line = ((source && InRange(T.line(), source->lines)) ? &source->lines[T.line()] : null))
        if (line->tokens_preproc_condition_unavailable) {
            ...
            Token *token_org_cur, *token_org_end, *token_prc_cur, *token_prc_end;
            if (line->tokens.elms()) {
                token_org_cur = line->tokens.data();
                token_org_end = token_org_cur + line->tokens.elms();
            } else
                token_org_cur = null; // set null when there are no elements
            if (line->Tokens().elms()) {
                token_prc_cur = line->Tokens().data();
                token_prc_end = token_prc_cur + line->Tokens().elms();
            } else
                token_prc_cur = null; // set null when there are no elements
            ...
            FREPA(T) {
                ...
                if (last_type != type) {
                    ...
                    if (token_org_cur) {
                        ...
                        if (token_org_cur && token_org_cur->col == col && token_org_cur->macro)
                            type = TOKEN_MACRO;
                    }
...
Bool Source::getSymbolMacroID(C VecI2 &cur, SymbolPtr &symbol_ptr, Macro *&macro_ptr, UID &id, VecI2 *x_range, Bool prefer_definition) {
    ...
    if (InRange(cur.y, lines)) {
        ...
        VecI2 range;
        if (CE.view_elm_names)
            if (TextToIDInside(line, cur.x, id, x_range ? *x_range : range))
                return true;
            ...
            if (token.macro && token.col == x) {
                REPA(ProjectMacros) {
                    ...
                    if (token == macro.name) {
                        macro_ptr = &macro;
                        if (x_range)
                            x_range->set(x, x + macro.name.length() - 1);
                        return true;
                    }
                }
                break;
    ...
    // check for symbol
    Int token;
    if (Token *t = findToken(cur, token)) {
        ...
        if (Symbol *symbol = finalSymbol(token)) {
            if (symbol->isFunc()) {
                ...
                if (preference && preference->source == this && InRange(preference->token_index, tokens) && tokens[preference->token_index]->lineIndex() == cur.y)
                    preference = null; // if we're already at the preference and trying to jump to it again, then cancel jumping to preference
                if (preference)
                    symbol = preference;
                else
                    REPA(funcs)
            ...
            symbol_ptr = symbol;
            if (x_range)
                x_range->set(t->col, t->col + t->length() - 1);
            return true;
        }
    }
    return false;
            ...
            for (Int i = body_start; i < body_end && i < tokens.elms();) {
                ...
                if (token == '>') {
                    ...
                    if (token == '.' || token == "->" || token == "::")
                        if (InRange(i - 1, tokens)) {
                            Token &c = *tokens[i - 1];
                            if (c == ')' || c == ']' || c.type == TOKEN_KEYWORD || c.type == TOKEN_CODE || c == TMPL_E)
                                detect = false; // don't detect if we're preceeded by some possible symbol
                        }
...
void Source::parseFunc(Token &token) {
    if (token.parent)
        if (Symbol *func = token.parent->func())
            parseFunc(*func);
}
...
Bool Source::hasKbFocus() C {
    if (CE.view_mode() || Gui.kb() == &suggestions_textline)
        return false;
    return contains(Gui.kb()) || ((!Gui.kb() || Gui.kb() == Gui.desktop()) && visibleFull() && parent());
}
...
Bool Source::isCurVisible() C {
    return cur.y >= Ceil(slidebar[1].offset() / CE.ts.lineHeight()) && cur.y + 1 <= Trunc((slidebar[1].offset() + clientHeight()) / CE.ts.lineHeight()) && cur.x >= Ceil((slidebar[0].offset() + CE.lineNumberSize()) / CE.ts.colWidth()) && cur.x + 1 <= Trunc((slidebar[0].offset() + clientWidth()) / CE.ts.colWidth());
}

Bool Source::insideRSTBrackets(C VecI2 &pos) // if inside Round Square Template brackets
{
    Int level = 0, i;
    if (findPrevToken(pos, i))
        for (; i >= 0;) {
            Token &token = *tokens[i--];
            if (token == '(' || token == '[' || token == '<' || token == TMPL_B)
                level++;
            else if (token == ')' || token == ']' || token == '>' || token == TMPL_E)
                level--;
            if (token == '{' || token == '}')
                break;
        }
...
Vec2 Source::posCur(C Vec2 &pos) C {
    Vec2 c = pos - T.pos();
    c -= offset();
    c /= Vec2(CE.ts.colWidth(), -CE.ts.lineHeight());
    return c;
}
Vec2 Source::offset() C { return Vec2(-slidebar[0].offset() + CE.lineNumberSize(), slidebar[1].offset()); }

Int Source::viewToReal(Int view) C {
    if (InRange(view, view_lines))
        return view_lines[view].line();
    if (view_lines.elms())
        return view_lines.last().line() + view - view_lines.elms() + 1;
    return view;
}
    ...
    if (InRange(view.y, view_lines)) {
        ...
        if (InRange(view_line.line(), lines)) {
            ...
            if (InRange(view.x, view_line)) {
                real.set(view_line.cols[view.x].pos.x, view_line.line());
                return InRange(real.x, line);
            }
            real.set(line.length(), view_line.line());
            return false; // set 'real' as close as possible
        }
    }
    real.set(0, lines.elms());
    return false; // set 'real' as close as possible
}
...
VecI2 Source::realToView(C VecI2 &real) C {
    ...
    if (InRange(y, view_lines)) {
        ...
        if (view_line.line() == real.y)
            return VecI2(Max(0, view_line.findCol(real.x)), y);
        return VecI2(0, y);
    }
    return VecI2(0, 0);
}
...
VecI2 &Source::inc(VecI2 &p) {
    if (InRange(p.y, lines)) {
        if (p.x >= lines[p.y].length() - 1) {
            ...
            do
                p.y++;
            while (InRange(p.y, lines) && !lines[p.y].length());
        } else
            p.x++;
    }
    return p;
...
void Source::setTokenPtrs() {
    ...
    tokens.clear();
    FREPA(lines) {
        Memc<Token> &tokens = lines[i].Tokens();
        FREPA(tokens) {
            Token &token = tokens[i];
            T.tokens.add(&token);
...
void Source::exist(Int x, Int y) {
    if (y >= lines.elms())
        lines.setNum(y + 1);
    REP(x - lines[y].length())
    lines[y].append(' ');
}
/******************************************************************************/
static Int CompareToken(Token *C &token, C VecI2 &pos) {
    if (Int c = Compare(token->lineIndex(), pos.y))
        return c;
...
static Int CompareTokenOverlap(Token *C &token, C VecI2 &pos) {
    if (Int c = Compare(token->lineIndex(), pos.y))
        return c;
    if (token->col + token->length() - 1 < pos.x)
        return -1;
    if (token->col > pos.x)
        return +1;
    return 0;
...
Token *Source::findPrevToken(C VecI2 &pos, Int &i) {
    ...
    if (tokens.binarySearch(p, i, CompareToken))
        return tokens[i];
    if (InRange(--i, tokens)) {
        Token &token = *tokens[i];
        if (token.macro_col < 0)
            return &token; // this is not a token made by macro
        ...
        for (; i < tokens.elms(); i++) {
            Token *cur = tokens[i];
            switch (CompareTokenOverlap(cur, p)) {
            case 0:
                return cur;
            case 1:
                goto too_far;
...
Token *Source::findToken(C VecI2 &pos, Int &i) {
    ...
    if (InRange(--i, tokens)) {
        Token &token = *tokens[i];
        if (token.lineIndex() == pos.y && token.col + token.length() > pos.x)
            return &token;
        if (token.macro_col >= 0) {
            // go left to first token that is created by that macro
            for (;; i--) {
            ...
            for (; i < tokens.elms(); i++) {
                Token *cur = tokens[i];
                if (!cur->sameMacro(token))
                    break;
                if (cur->lineIndex() == pos.y && cur->col + cur->length() > pos.x)
                    return cur;
            }
        }
    }
...
Bool Source::save(File &f, StrLibrary &sl) C {
    ...
    if (!modify_time.save(f))
        return false;
    f.cmpUIntV(symbols.elms());
    FREPA(symbols) {
        C SymbolDef &symbol = symbols[i];
        sl.putStr(f, symbol.name());
        if (!symbol->save(f, sl))
            return false;
    } // write before 'lines'
    f.cmpUIntV(decls.elms());
    FREPA(decls) {
        C SymbolDecl &decl = decls[i];
        sl.putStr(f, decl.name());
        if (!decl->save(f, sl))
            return false;
    } // write before 'lines'
    f.cmpUIntV(lines.elms());
    FREPA(lines) {
        C Line &line = lines[i];
        if (!line.save(f, sl))
            return false;
...
Bool Source::load(File &f, StrLibrary &sl, Str &temp) {
    ...
    sl.getStr(f, loc.file_name);
    loc.setFile(loc.file_name);
    if (!modify_time.load(f))
        goto error;
#if SOURCE_DEBUG_SPEED_SIZE
    Dbl t = Time.curTime();
    Int s = f.pos();
#endif
    symbols.setNum(f.decUIntV());
    FREPA(symbols) {
        sl.getStr(f, temp);
        if (!symbols[i].require(temp)->load(f, sl, T))
            goto error;
    }
#if SOURCE_DEBUG_SPEED_SIZE
    F0 += Time.curTime() - t;
    ...
    s = f.pos();
#endif
    decls.setNum(f.decUIntV());
    FREPA(decls) {
        sl.getStr(f, temp);
        if (!decls[i].require(temp)->load(f, sl, T))
            goto error;
    }
#if SOURCE_DEBUG_SPEED_SIZE
    F1 += Time.curTime() - t;
...
ERROR_TYPE Source::load() {
    ...
    Str data;
    if (!loc.is())
        error = EE_ERR_ELM_NOT_FOUND;
    else {
        if (loc.file) {
            cpp = true;
            FileText f;
            if (!f.read(loc.file_name))
                error = EE_ERR_FILE_NOT_FOUND;
            else {
                error = EE_ERR_NONE;
                f.getAll(data);
            }
    ...
    if (error == EE_ERR_NONE && loc.file) {
        ...
        if (fi.getSystem(loc.file_name)) {
            modify_time = fi.modify_time_utc;
            Const = FlagOn(fi.attrib, FATTRIB_READ_ONLY);
        }
...
Bool Source::saveTxt(C Str &name) {
    FileText f;
    if (f.write(name, hasUnicode() ? UTF_8 : ANSI)) {
        FREPA(lines) {
            if (i)
                f.endLine();
            f.putText(lines[i]);
...
Bool Source::save(C SourceLoc &loc) {
    ...
    if (!loc.is()) {
        ...
        if (!ok)
            Gui.msgBox(S, S + "Error saving to \"" + loc.file_name + '"');
    } else {
        ok = CE.cei().sourceDataSave(loc.id, asText());
        if (!ok)
            Gui.msgBox(S, S + "Error saving source code");
    }
    if (ok) {
        T.loc = loc;
        undo_original_state = undos.undos();
        ...
        forceCreateNextUndo();
        if (loc.file)
            FTimeUTC(loc.file_name, modify_time);
        if (header)
            CE.rebuildSymbols(true);
        if (CE.cur() == this)
            CE.cei().sourceChanged();
    }
    return ok;
}
...
void Source::save() {
    if (!win_io_save)
        New(win_io_save)->create(CODE_EXT, S, GetPath(GetPath(DataPath())), SaveEs, SaveEs, T);
...
Source::Source() : undos(true, this, 15) {
    lit_symbol = null;
    win_io_save = null;
    active = was_active = header = engine_header = false;
    opened = false;
    Const = false;
    view_comments = view_funcs = view_func_bodies = true;
    view_private_members = false;
    recursive = parse_count = 0;
    preproc_line_changed = highlight_line = -1;
    highlight_time = 0;
    cur = 0;
    sel = sel_temp = -1;
    lc_offset.zero();
    delUndo();
    create().skin(&CE.source_skin, false);
    kb_lit = false;
    setScroll();
    setHideSlideBar();
