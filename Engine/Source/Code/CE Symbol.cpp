...
SymbolPtr FindChild(C Str &name, Symbol *parent, Memc<Symbol::Modif> *parent_templates, Bool allow_base, Bool allow_self, RecTest &rt) {
    ...
    if (rt)
        return null;
    SymbolPtr s;
    if (allow_self && s.find(parent ? parent->full_name + SEP + name : name))
        if ((s->valid || s->valid_decl) && (CppMode ? !(s->modifiers & Symbol::MODIF_ES_ONLY) : true))
            return s; // when in CppMode then skip Code Editor-only keywords
    if (allow_base && parent && parent->type == Symbol::CLASS)
        FREPA(parent->base) {
            Symbol::Modif base = parent->base[i];
            base.proceedToFinal(parent_templates);
            if (*base == name())
                return base; // check base itself
            if (s = FindChild(name, base(), &base.templates, allow_base, true, rt))
                return s; // check base members
        }
    return null;
}
SymbolPtr FindSymbol(C Str &name, Symbol *parent) {
    if (SymbolPtr s = FindChild(name, parent))
        if (!(s->modifiers & Symbol::MODIF_CTOR_DTOR))
            return s;
    if (parent)
        return FindSymbol(name, parent->Parent());
    return null;
}
...
void AddBaseTemplates(Memc<Symbol::Modif> &templates, Symbol &Class, RecTest &rt) {
    ...
    if (Symbol::Modif &base = Class.base[i]) {
        ...
        REPA(base.templates)
        templates.add(base.templates[i]); // add all templates from that base (process in reversed order, so first is most important)
    }
}
/******************************************************************************/
...
void Symbol::Modif::clear(Bool clear_templates) {
    ...
    if (clear_templates)
        ...
        REPAO(templates).src_template_i = -1;
    super::clear();
}
/******************************************************************************/
...
Int Symbol::Modif::rawSize(Bool ref_as_ptr_size, RecTest &rt) {
    ...
    if (ref_as_ptr_size && (modifiers & MODIF_REF))
        return PtrSize; // ignore arrays because "arrays of references are illegal in C++"

    // calculate total array dimensions
    Int dims = 1;
    REPA(array_dims) // must go from the end to start
    {
        Int d = array_dims[i];
        if (d == DIM_PTR)
            return dims * PtrSize; // if encountered a pointer {1,2,PTR,4,5} then the size is 4*5*PtrSize
        if (d >= 1)
            dims *= d;
        else {
    ...
    if (ptr_level)
        return dims * PtrSize;
    if (T)
        return dims * T->rawSize(ref_as_ptr_size, &templates, rt);

    // TODO: compile error
    return 0;
}
Int Symbol::Modif::firstMemberSize(RecTest &rt) {
    if (modifiers & MODIF_REF)
        return PtrSize; // ignore arrays because "arrays of references are illegal in C++"
    if (anyPtr())
        return PtrSize;
    if (T)
        return T->firstMemberSize(&templates, rt);
    return 0;
}
            ...
            if (templat && (templat->modifiers & MODIF_CONST_MEM_ADDR))                       // if this element requires constant memory address "const_mem_addr class Thread"
                if (!(templat.modifiers & MODIF_CONST_MEM_ADDR))                              // and not marked to ignore "Memc<const_mem_addr Thread>"
                    if (templat.dependable())                                                 // "Memc<Thread*>" is ok, so ignore pointers
                        return false;
        if (!templat.constMemAddrOK())
            return false; // if any sub-templates have issue then fail too, "Memx< Memc<const_mem_addr Thread> >" the Memx is OK but the sub Memc should fail
    }
    return true;
}
Str Symbol::Modif::modifName(Bool spaces, Bool include_const, Bool include_ref, Bool invalid_array_as_1, Bool template_space) {
    Str s;
    if ((const_level & 1) && include_const)
        s = "const ";
    ...
    if (T && T->templates.elms()) // Memc<>
    {
        ...
        FREPA(T->templates) {
            ...
            Modif *target = null;
            REPA(templates)
            if (templates[i].src_template == src) {
                target = &templates[i];
                break;
            }
            if (target)
                s += target->modifName(spaces);
            else
                s += *src; // here include const and ref
        }
        if (template_space && s.last() == '>')
            s += ' ';
        s += '>';
    }
    ...
    for (RecTest lrt; T;) {
        ...
        if (T->isFunc()) {
            if (func_call)
                ...
                break;
        } // we can't proceed to the value of the function without calling it (we can use only one call at a time)
        ...
        if (T->type == Symbol::TYPENAME) {
            ...
            proceedTo(T->value, rt);
        else
            break;
    }
    if (templates)
        REPAO(T.templates).proceedToFinal(templates, false, rt);
}
void Symbol::Modif::proceedTo(Modif &src, RecTest &rt) // new order of const,ptr should be : src, T (since 'src' is the more low-level)
{
    ...
    if (src.array_dims.elms()) {
        ...
        FREPA(src.array_dims)
        array_dims[i] = src.array_dims[i]; // set src.array
        FREP(T.ptr_level)
        array_dims[src.array_dims.elms() + i] = DIM_PTR; // set T  .ptr
        FREPA(T.array_dims)
        array_dims[src.array_dims.elms() + T.ptr_level + i] = T.array_dims[i]; // set T  .array
...
Str Symbol::typeName() {
    ...
    case NAMESPACE:
        return "Namespace";
    case CLASS:
        return (modifiers & MODIF_UNION) ? "Union" : "Class";
    case TYPENAME:
        return "Typename";
    case FUNC_LIST:
...
Str Symbol::fileName() {
    if (modifiers & MODIF_NAMELESS)
        return S + "nameless-" + nameless_index;
    return Replace(full_name, SEP, SEP_FILE);
}
Str Symbol::cppName() // returns base name only (return NAMELESS names because 'fullCppName' requires it)
{
    if (isFunc())
        return parent ? PathToSymbol(*parent) : S; // sample FUNC name "Namespace/Func/B123FE", so return parent FUNC_LIST name
    if (type == SPACE)
        return S;
    return T;
}
...
Str Symbol::shortName() {
    Str name = cppName();
    if (Starts(name, "operator", true, WHOLE_WORD_STRICT))
        name = _SkipStart(name, "operator");
    return name;
}
/******************************************************************************/
...
Str Symbol::comments() {
    ...
    {
        if (T == "continue")
            return "proceed to the next step of current loop";
        if (T == "break")
            return "break current loop or case statement";
        if (T == "return")
            return "return from function";
        if (T == "if")
            return "Perform conditional code.\n\nSample usage:\n\n   int x;\n   if(x>0)..; // do codes if x is greater than 0";
        if (T == "goto")
            return "Jump to specified label (code position).\n\nSample usage:\n\n   for(int x=0; x<10; x++)\n   {\n      for(int y=0; y<10; y++)\n      {\n         if(..)goto break_both_loops;\n      }\n   }\nbreak_both_loops: // specify label";
        if (T == "sizeof")
            return "Obtain size of element in bytes.\n\nSample usage:\n\n   byte b;\n   sizeof(b   ); // 1\n   sizeof(byte); // 1\n\n   int i;\n   sizeof(i  ); // 4\n   sizeof(int); // 4";
        if (T == "this")
            return "Get reference to self.\n\nSample usage:\n\n   class Class\n   {\n      int value;\n\n      void set(int value) {this.value=value;}\n   }";
        if (T == "super" || T == "__super")
            return "Access member/method of a base class.\n\nSample usage:\n\n   class BaseClass\n   {\n      void method() // base method\n      {\n      }\n   }\n   class ExtendedClass : BaseClass\n   {\n      void method() // extended method\n      {\n         super.method(); // call base method\n      }\n   }";
        if (T == "switch" || T == "case" || T == "default")
            return "keyword used to perform different codes basing on a value of parameter.\n\nSample usage:\n\nint x;\nswitch(x)\n{\n   case 0: .. break; // do codes for 0 value\n\n   case 1: .. break; // do codes for 1 value\n\n   case 2:\n   case 3: .. break; // do codes for 2,3 values\n\n   default: .. break; // do codes for all other values\n}";
        if (T == "const_mem_addr")
            return "Element must be stored in constant memory address.\nIf you see this keyword next to a class declaration,\nyou must ensure that when defining objects of that class\nyou will store them in constant memory address,\nthis can be either global namespace or inside 'Memx' 'Meml' containers.";

        if (T == "bool" || T == "Bool")
            return "Boolean value 'true' or 'false' (8-bit size).";
        if (T == "char" || T == "Char")
            return "Unicode Character (16-bit size).";
        if (T == "cchar" || T == "CChar")
            return "Const Unicode Character (16-bit size).";
        if (T == "char8" || T == "Char8")
            return "Ansi Character (8-bit size).";
        if (T == "cchar8" || T == "CChar8")
            return "Const Ansi Character (8-bit size).";
        if (T == "sbyte" || T == "SByte" || T == "I8")
            return "Signed Integer -128 .. 127 (8-bit size).";
        if (T == "byte" || T == "Byte" || T == "U8")
            return "Unsigned Integer 0 .. 255 (8-bit size).";
        if (T == "short" || T == "Short" || T == "I16")
            return "Signed Integer -32 768 .. 32 767 (16-bit size).";
        if (T == "ushort" || T == "UShort" || T == "U16")
            return "Unsigned Integer 0 .. 65 535 (16-bit size).";
        if (T == "int" || T == "Int" || T == "I32")
            return "Signed Integer -2 147 483 648 .. 2 147 483 647 (32-bit size).";
        if (T == "uint" || T == "UInt" || T == "U32")
            return "Unsigned Integer 0 .. 4 294 967 295 (32-bit size).";
        if (T == "long" || T == "Long" || T == "I64")
            return "Signed Integer -9 223 372 036 854 775 808 ..  9 223 372 036 854 775 807 (64-bit size).";
        if (T == "ulong" || T == "ULong" || T == "U64")
            return "Unsigned Integer 0 .. 18 446 744 073 709 551 615 (64-bit size).";
        if (T == "flt" || T == "Flt" || T == "float")
            return "Floating Point 1-bit sign + 8-bit exponent + 23-bit fraction (32-bit size).";
        if (T == "dbl" || T == "Dbl" || T == "double")
            return "Floating Point 1-bit sign + 11-bit exponent + 52-bit fraction (64-bit size).";
        if (T == "ptr" || T == "Ptr")
            return "General Pointer (32-bit or 64-bit).";
        if (T == "cptr" || T == "CPtr")
            return "General Pointer to const data (32-bit or 64-bit).";
        if (T == "intptr" || T == "IntPtr")
            return "Signed Integer capable of storing full memory address (32-bit or 64-bit).";
        if (T == "uintptr" || T == "UIntPtr")
            return "Unsigned Integer capable of storing full memory address (32-bit or 64-bit).";
    }
    if (source && InRange(token_index, source->tokens) && type != NAMESPACE && type != FUNC_LIST) {
        ...
        if (Line *line = token.line) {
            Str com;
            for (Int i = token.col; i < line->length(); i++)
                if (line->Type(i) == TOKEN_COMMENT) {
                    Char c = (*line)[i];
                    if (!(com.last() == ' ' && c == ' '))
                        com += c;
                }
...
Symbol *Symbol::Parent() {
    Symbol *p = T.parent();
    for (; p && (p->modifiers & MODIF_TRANSPARENT);)
        p = p->parent();
    return p;
}
...
Symbol *Symbol::func() {
    for (Symbol *symbol = this; symbol; symbol = symbol->parent()) {
        if (symbol->type == FUNC)
            return symbol;
        if (symbol->type == CLASS || symbol->type == NAMESPACE)
            return null; // "void func() { class A { int a; void met() {} } }" // let "int a" tokens being child of "class A" don't return 'func'
    }
    return null;
}
...
Symbol *Symbol::firstNonTransparent() {
    for (Symbol *symbol = this; symbol; symbol = symbol->parent())
        if (!(symbol->modifiers & MODIF_TRANSPARENT))
            return symbol;
    return null;
}
...
Bool Symbol::insideFunc() {
    for (Symbol *cur = Parent(); cur; cur = cur->Parent())
        if (cur->type == FUNC || cur->type == FUNC_LIST)
            return true; // use Parent because when testing FUNC its parent is FUNC_LIST which would result in FUNC being inside func
    return false;
}
Bool Symbol::insideClass() {
    if (source) {
        ...
        if (InRange(start, source->tokens))
            return source->tokens[start]->parent == Parent();
    }
    return false;
}
/******************************************************************************/
...
Bool Symbol::isTemplateClass() {
    for (Symbol *cur = this; cur && cur->type == CLASS; cur = cur->parent())
        if (cur->templates.elms())
            return true; // if any of class parents has templates
    return false;
}
Bool Symbol::insideTemplateClass() {
    if (Symbol *parent = Parent())
        return parent->isTemplateClass();
    return false;
}
Bool Symbol::isTemplateFunc() {
    if (isFunc()) {
        if (templates.elms() // func is "template func" if it has templates
            || insideTemplateClass())
            return true; // or any of its class parents   has templates
    }
    return false;
}
...
Bool Symbol::hasVirtualDestructor(Memc<Modif> *templates, RecTest &rt) // assumes 'T.children' is set
{
    ...
    REPA(base) {
        ...
        if (base->hasVirtualDestructor(&base.templates, rt))
            return true;
    }
    return false;
}
Bool Symbol::hasVirtualFunc(Symbol &func, Memc<Modif> *templates, RecTest &rt) // assumes 'T.children' is set
{
    ...
    REPA(base) {
        Modif base = T.base[i];
        base.proceedToFinal(templates);
        if (base->hasVirtualFunc(func, &base.templates, rt))
            return true;
    }
    return false;
}
Bool Symbol::isVirtualClass(Memc<Modif> *templates, RecTest &rt) // assumes 'T.children' is set
{
    ...
    REPA(base) {
        Modif base = T.base[i];
        base.proceedToFinal(templates);
        if (base->isVirtualClass(&base.templates, rt))
            return true;
    }
    return false;
}
...
{
    if (modifiers & MODIF_VIRTUAL)
        return true;
    if (modifiers & MODIF_DTOR) // destructor needs to be checked separately (there are no virtual constructors, so skip them)
    {
        if (Symbol *Class = T.Class())
            return Class->hasVirtualDestructor(templates);
    } else if (isClassNonStaticFunc()) // if at least one identical function in base classes is virtual then this is virtual too
    {
        if (Symbol *Class = T.Class())
            return Class->hasVirtualFunc(T, templates);
    }
    return false;
}
Bool Symbol::hasConstructor(Memc<Modif> *templates, RecTest &rt) // assumes 'T.children' is set
{
    ...
    if (Symbol *child = children[i])
        ...
        {
            if (child->modifiers & MODIF_CTOR)
                return true; // we have found constructor
            ...
            {
                Modif value = child->value;
                value.proceedToFinal(templates);
                if (value.hasConstructor(rt))
                    return true; // if any type of variable has constructor, then this class needs constructor
            }
        }
    REPA(base) {
        Modif base = T.base[i];
        base.proceedToFinal(templates);
        if (base.hasConstructor(rt))
            return true;
    }
    return false;
}
    ...
    if (Symbol *child = children[i])
        if (!(child->modifiers & MODIF_STATIC)) // non-static child
        {
                ...
                || (child->type == CLASS && (child->modifiers & MODIF_TRANSPARENT))) // transparent class
            {
                Modif value = child->value;
                value.proceedToFinal(templates);
                if (value.hasDestructor(rt))
                    return true; // if any type of variable has destructor, then this class needs destructor
            }
        }
    REPA(base) {
        Modif base = T.base[i];
        base.proceedToFinal(templates);
        if (base.hasDestructor(rt))
            return true;
    }
    return false;
}
...
Bool Symbol::fullyPublic() {
    for (Symbol *cur = this; cur; cur = cur->Parent())
        if (ModifToAccessLevel(cur->modifiers) != ACCESS_PUBLIC)
            return false;
    return true;
}
Int Symbol::templateClasses() {
    Int tc = 0;
    for (Symbol *cur = Parent(); cur && cur->type == CLASS; cur = cur->Parent())
        if (cur->templates.elms())
            tc++; // if class has templates, can use Parent to skip transparent classes because those can't have templates, use Parent also to skip FUNC_LIST
    return tc;
}
Int Symbol::baseOffset(Int base_index, Memc<Modif> *templates, RecTest &rt) {
    ...
    Bool base0_virtual = false;
    if (base.elms()) {
        Modif base0 = base[0];
        base0.proceedToFinal(templates);
        if (base0 && base0->isVirtualClass(&base0.templates))
            base0_virtual = true;
    }
    if (!base0_virtual && isVirtualClass(templates))
        offset = PtrSize; // if this is virtual and 1st base is not, then we need to add virtual

    MIN(base_index, base.elms());
    FREP(base_index) // iterate the first bases
    {
        Symbol::Modif b = base[i];
        b.proceedToFinal(templates);
...
Int Symbol::memberOffset(Symbol *member, Bool *found, Memc<Modif> *templates, RecTest &rt) {
    ...
    if (Symbol *child = children[i])
            ...
            || (child->type == CLASS && (child->modifiers & MODIF_TRANSPARENT))) // transparent classes
        {
            ...
            if (modifiers & MODIF_UNION)
                MAX(total_size, child_pos + child_size); // if this 'class' is an 'union' then use max size of all members
            else
                pos = total_size = child_pos + child_size;
        }
...
Int Symbol::rawSize(Bool ref_as_ptr_size, Memc<Modif> *templates, RecTest &rt) {
    ...
    case FUNC:
        if (isVar())
            return PtrSize;
        break; // pointer to function

    case VAR: {
        Modif modif = value;
        modif.proceedToFinal(templates);
        return modif.rawSize(ref_as_ptr_size, rt);
    }
    ...
    case CLASS:
        return memberOffset(null, null, templates, rt);
    }
    return 0;
}
Int Symbol::firstMemberSize(Memc<Modif> *templates, RecTest &rt) {
    ...
    case VAR: {
        Modif modif = value;
        modif.proceedToFinal(templates);
        return modif.firstMemberSize(rt);
    }
    ...
    case CLASS: {
        if (isVirtualClass(templates, rt))
            return Min(class_pack, PtrSize);
        FREPA(base) {
            Symbol::Modif b = base[i];
            b.proceedToFinal(templates);
            if (Int size = b.firstMemberSize(rt))
                return Min(class_pack, size);
        }
        ...
        if (Symbol *child = children[i])
            if ((child->isVar() && !(child->modifiers & MODIF_STATIC))               // non-static variables
                ...
                if (Int size = child->firstMemberSize(templates, rt))
                    if (modifiers & MODIF_UNION)
                        MAX(max_size, size); // from union we must pick the biggest element
                    else
                        return Min(class_pack, size);
...
Bool Symbol::sameFunc(Symbol &f) {
    if (value.sameFuncParamValue(f.value)) {
        if ((modifiers & MODIF_SAME_FUNC) != (f.modifiers & MODIF_SAME_FUNC))
            return false;
...
Bool Symbol::constDefineInHeader() // if define const in header
{
    ...
    if (value)
        return value.const_level == 1 && !value.anyPtr() && (value->var_type == VAR_BOOL || IntType(value->var_type)); // !anyPtr: accept OBJ (int x=5) and ARRAY (int x[]={1, 2})
    return false;
}
...
Bool Symbol::hasNonPrivateBase(Symbol *Class, Memc<Modif> *templates, Bool allow_self, Bool test_private, RecTest &rt) {
    ...
    REPA(base) {
        ...
        if (test_private ? FlagOff(b.modifiers, MODIF_PRIVATE) : true) {
            Modif base = b;
            base.proceedToFinal(templates);
            if (base->hasNonPrivateBase(Class, &base.templates, true, true, rt))
                return true;
        }
    }
    return false;
...
Bool Symbol::hasPrivateBase(Symbol *Class, Memc<Modif> *templates, Bool test_private, RecTest &rt) {
    ...
    REPA(base) {
        ...
        if (test_private ? FlagOn(base.modifiers, MODIF_PRIVATE) : false) {
            ...
            if (base->hasPrivateBase(Class, &base.templates, true, rt))
                return true;
        }
    }
    return false;
...
Bool Symbol::hasNonPublicBase(Symbol *Class, Memc<Modif> *templates, Bool test_access, RecTest &rt) {
    ...
    REPA(base) {
        ...
        if (test_access ? FlagOn(base.modifiers, MODIF_PRIVATE | MODIF_PROTECTED) : false) {
            ...
            if (base->hasNonPublicBase(Class, &base.templates, true, rt))
                return true;
        }
    }
    return false;
...
{
    if (type == KEYWORD || type == PREPROC || type == TYPENAME)
        return false;
    ...
    if ((modifiers & MODIF_CTOR_DTOR) && !caller_is_namespace)
        return false; // constructors can be listed only in namespaces
    ...
    if (!ignore_instance) {
        if (instance) {
            if ((type == CLASS && !allow_bases) || type == NAMESPACE || type == ENUM || type == ENUM_ELM || type == TYPEDEF /* || fullyStatic()*/)
                return false; // don't check for 'fullyStatic' because for example "EE.D.clear()" where 'clear' is static method but can be accessed
            if (access == ACCESS_PROTECTED && !caller_is_namespace)
                if (!symbol || !symbol->hasNonPrivateBase(caller_class, &symbol_templates))
                    return false;                                    // formula obtained using hand-made tests (works!)
        } else if (type == VAR || type == FUNC || type == FUNC_LIST) // if it's a member
            if (Parent && Parent->type == CLASS) {
                ...
                if (!(type == FUNC_LIST && allow_func) && !((type == VAR || type == FUNC) && ctor_init))
                    if (!partiallyStatic())
                        return false; // can't access non-static members from non-instance
            }
    }

    if (!Parent && !symbol)
        return true; // if both are global namespace
    if (ctor_init)
        return symbol == Parent; // constructor initializers can be used only from the same class
    if (access == ACCESS_PRIVATE && !caller_is_namespace)
        if (Parent != caller_class)
            return false; // private   members can be used only from the same class
    if (access == ACCESS_PROTECTED && !caller_is_namespace)
        if (!caller_class || !caller_class->hasNonPrivateBase(Parent))
            return false; // protected members can be used only from extended classes
    if (caller_class && caller_class->hasPrivateBase(Parent))
        return false; // we're trying to access from extended class -> base     class                "class Base{int x;}   class Middle : private Base{}   class Ext : Middle{|}"
    if (symbol && symbol->hasNonPublicBase(Parent, &symbol_templates, true))
        if (!caller_class || !caller_class->hasBase(symbol))
            return false; // we're trying to access from base     class -> extended class -> base class) "class Base{void method(){Middle m; m.|}}   class Middle : private Base{}
    Symbol *base = Parent;
    if (allow_bases && type == CLASS)
        base = this;
...
{
    if (type == KEYWORD && precise_parent)
        return false;
    if (modifiers & MODIF_CTOR_DTOR)
        return false;
    ...
    if (type == VAR || type == FUNC || type == FUNC_LIST) {
        Symbol *caller_class = (caller ? caller->Class() : null);
        if (highestAccess() == ACCESS_PRIVATE)
            if (Parent != caller_class)
                return false; // private members can be used only from the same class
...
Bool Symbol::addDependency(Modif &symbol) {
    ...
    for (Int i = 0; final && i < 64; i++) // find first value which has 'rootClass'
    {
        if (Symbol *root = final->rootClass()) // we need to be actually dependent on the root class of symbol (because only root classes are stored as headers)
        {
            ...
            {
                if (final->type == TYPENAME)
                    added |= dependencies.include(final()); // if this is a TYPENAME, then add dependency to it and only to class using it "<TYPE> class A { TYPE t; }" A depends on TYPE, "<TYPE> class A { class Sub {TYPE t;} }" Sub depends on TYPE
                if (root != this && !(root->helper & HELPER_PROCESSED))
                    added |= dependencies.include(root); // ignore self-dependencies
            }
            break;
        }
    ...
    if (symbol)
        if (Symbol *root = symbol->nested()) // if 'symbol' is nested
            if (root != this)                // if 'root' is not this (so we don't make 'root' to be dependent on itself)
                if (!(root->helper & HELPER_PROCESSED))
                    dependencies.include(root);
}
/******************************************************************************/
void Symbol::clearBody() {
    if (isFunc() && (modifiers & MODIF_FUNC_BODY)) {
        ...
        ctor_inits.clear();
        if (source)
            for (Int i = source->getBodyStart(token_index), level = 0; InRange(i, source->tokens); i++) {
