...
VAR_TYPE MulResult(VAR_TYPE a, VAR_TYPE b) {
    if (a == VAR_NONE || a == VAR_VOID || a == VAR_NULL || b == VAR_NONE || b == VAR_VOID || b == VAR_NULL)
        return VAR_NONE;
    if (a == VAR_DBL || b == VAR_DBL)
        return VAR_DBL;
    if (a == VAR_FLT || b == VAR_FLT)
        return VAR_FLT;
    if (a == VAR_BOOL && b == VAR_BOOL)
        return VAR_BOOL;
    if (a == VAR_LONG || a == VAR_ULONG || b == VAR_LONG || b == VAR_ULONG)
        return (SignedType(a) && SignedType(b)) ? VAR_LONG : VAR_ULONG;
...
VAR_TYPE DivResult(VAR_TYPE a, VAR_TYPE b) {
    ...
    if (a == VAR_FLT || b == VAR_FLT)
        return VAR_FLT;
    if (a == VAR_LONG || a == VAR_ULONG)
        return (SignedType(a) && SignedType(b)) ? VAR_LONG : VAR_ULONG;
    return (SignedType(a) && SignedType(b)) ? VAR_INT : VAR_UINT;
}
VAR_TYPE ModResult(VAR_TYPE a, VAR_TYPE b) {
    if (a == VAR_NONE || a == VAR_VOID || a == VAR_NULL || b == VAR_NONE || b == VAR_VOID || b == VAR_NULL || RealType(a) || RealType(b))
        return VAR_NONE;
    if (a == VAR_LONG || a == VAR_ULONG)
        return (SignedType(a) && SignedType(b)) ? VAR_LONG : VAR_ULONG;
    return (SignedType(a) && SignedType(b)) ? VAR_INT : VAR_UINT;
...
VAR_TYPE ShiftLeftResult(VAR_TYPE a, VAR_TYPE b) {
    if (a == VAR_NONE || a == VAR_VOID || a == VAR_NULL || b == VAR_NONE || b == VAR_VOID || b == VAR_NULL || RealType(a) || RealType(b))
        return VAR_NONE;
    if (a == VAR_LONG || a == VAR_ULONG)
        return SignedType(a) ? VAR_LONG : VAR_ULONG; // in C++ "Int<<Int64" is still an Int
...
VAR_TYPE ShiftRightResult(VAR_TYPE a, VAR_TYPE b) {
    ...
    if (a == VAR_BOOL && b == VAR_BOOL)
        return VAR_BOOL;
    if (a == VAR_LONG || a == VAR_ULONG)
        return SignedType(a) ? VAR_LONG : VAR_ULONG;
    return SignedType(a) ? VAR_INT : VAR_UINT;
}
...
Int AlignAddress(Int address, Int size) {
    if (size >= 2)
        ...
        case 2:
        case 3:
            return Ceil2(address);

        case 4:
        case 5:
        case 6:
        case 7:
            return Ceil4(address);

        default:
            return Ceil8(address);
        }
    return address;
}
/******************************************************************************/
Bool SourceExt(C Str &ext) {
    return ext == "h" || ext == "hpp" || ext == "cpp" || ext == "c" || ext == "cc" || ext == "cxx" || ext == "inc" // include c file
           || ext == "m"                                                                                           // Apple Objective-C
           || ext == "mm";                                                                                         // Apple Objective-C++
}
Bool TextExt(C Str &ext) {
    return SourceExt(ext) || ext == "cs" // C#
           || ext == "java" || ext == "txt" || ext == "xml" || ext == "htm" || ext == "html" || ext == "php" || ext == "mk";
}
Bool AllowedExt(C Str &ext) {
    return TextExt(ext) || ext == "lib" || ext == "ico" || ext == "pak";
}
/******************************************************************************/
...
static void EditPaste() {
    if (!CE.view_mode())
        CE.paste();
}
static void EditPaste2() {
    if (!CE.view_mode())
        CE.paste(null, false);
}
static void EditSeparator() {
    if (!CE.view_mode())
        CE.separator();
}
static void EditSelectAll() {
    if (!CE.view_mode())
        CE.selectAll();
}
static void EditSelectWord() {
    if (!CE.view_mode())
        CE.selectWord();
}
static void EditMakeLower() {
    if (!CE.view_mode())
        CE.makeCase(false);
}
static void EditMakeUpper() {
    if (!CE.view_mode())
        CE.makeCase(true);
}
...
static void EditGotoDef() {
    if (CE.cur())
        CE.cur()->jumpToCur();
}
static void EditFindRef() {
    if (CE.cur())
        CE.cur()->findAllReferences();
}
...
static void EditAutoComplete() {
    if (CE.cur())
        CE.cur()->listSuggestions(1);
}
static void EditAutoCompleteElm() {
    if (CE.cur())
        CE.cur()->listSuggestions(-2);
}
...
static void DevlogClear(CodeEditor &ce) {
    ce.devlog_data.clear();
    ce.devlog_list.clear();
}
...
static void ViewComments() {
    CE.view_comments = CE.view_what.menu("Comments");
    RefreshSourceLines(CE);
}
static void ViewFuncs() {
    CE.view_funcs = CE.view_what.menu("Functions");
    RefreshSourceLines(CE);
}
static void ViewFuncBodies() {
    CE.view_func_bodies = CE.view_what.menu("Function Bodies");
    RefreshSourceLines(CE);
}
static void ViewPrivateMembers() {
    CE.view_private_members = CE.view_what.menu("Private Members");
    RefreshSourceLines(CE);
}
...
static void DevlogFilter2(CodeEditor &ce) {
    DevlogFilter(ce);
    ce.devlog_region.scrollEndY(true);
}
...
static void BuildError(C Str &msg, Bool at_end) {
    ...
    if (at_end)
        CE.build_region.scrollEndY(); // auto-scroll to the end
}

static Int CompareItem(C Item &a, C Item &b) {
    if (Int c = Compare(a.type, b.type))
        return c;
    if (Int c = Compare(a.base_name, b.base_name))
        return c;
    return 0;
}
static void SortItems(Memx<Item> &items) {
    ...
    REPA(items)
    SortItems(items[i].children);
}
/******************************************************************************/
CodeEditor::CodeEditor() {
    _cur = null;
    view_elm_names = view_comments = view_funcs = view_func_bodies = true;
    view_private_members = false;
    devenv_version = -1;
    devenv_express = devenv_com = build_msbuild = false;
    build_mode = BUILD_BUILD;
    build_exe_type = EXE_EXE;
    build_debug = true;
    build_windows_code_sign = false;
    build_phase = build_phases = build_step = build_steps = 0;
    build_refresh = 0;
...
static void CodeTabChanged(Ptr user) {
    if (InRange(CE.code_tabs(), CE.code_tabs))
        CE.cur(CE.code_tabs.tab(CE.code_tabs()).source); // set source
}
void CodeEditor::create(GuiObj *parent, Bool menu_on_top) {
    ...
    {
        Symbol &s = *Symbols("auto");
        s.type = typedef_type;
        s.value = "void";
    }
    {
        Symbol &s = *Symbols("ptr");
        s.type = typedef_type;
        s.value = "void";
        s.value.const_level = 0;
        s.value.ptr_level = 1;
    }
    {
        Symbol &s = *Symbols("cptr");
        s.type = typedef_type;
        s.value = "void";
        s.value.const_level = 1;
        s.value.ptr_level = 1;
    }
    {
        Symbol &s = *Symbols("cchar8");
        s.type = typedef_type;
        s.value = "char8";
        s.value.const_level = 1;
        s.value.ptr_level = 0;
    ...
    {
        Symbol &s = *Symbols("float");
        s.type = typedef_type;
        s.value = "flt";
    }
    {
        Symbol &s = *Symbols("double");
        s.type = typedef_type;
        s.value = "dbl";
    }
#if X64
    {
        Symbol &s = *Symbols("intptr");
        s.type = typedef_type;
        s.value = "long";
    }
    {
        Symbol &s = *Symbols("uintptr");
        s.type = typedef_type;
        s.value = "ulong";
    }
#else
...
#endif
    {
        Symbol &s = *Symbols("size_t");
        s.type = typedef_type;
        s.value = "intptr";
    }
    {
        Symbol &s = *Symbols("ssize_t");
        s.type = typedef_type;
        s.value = "uintptr";
    }
    {
        Symbol &s = *Symbols("intptr_t");
        s.type = typedef_type;
        s.value = "intptr";
    }
    {
        Symbol &s = *Symbols("uintptr_t");
        s.type = typedef_type;
        s.value = "uintptr";
    }
    {
        Symbol &s = *Symbols("wchar_t");
        s.type = typedef_type;
        s.value = "char";
    }
    {
        Symbol &s = *Symbols("char16_t");
        s.type = typedef_type;
        s.value = "char";
    }
    {
        Symbol &s = *Symbols("char32_t");
        s.type = typedef_type;
        s.value = "uint";
    }
    {
        Symbol &s = *Symbols("__int8");
        s.type = typedef_type;
        s.value = "sbyte";
    }
    {
        Symbol &s = *Symbols("__int16");
        s.type = typedef_type;
        s.value = "short";
    }
    {
        Symbol &s = *Symbols("__int32");
        s.type = typedef_type;
        s.value = "int";
    }
    {
        Symbol &s = *Symbols("__int64");
        s.type = typedef_type;
        s.value = "long";
    }
    {
        Symbol &s = *Symbols("int8_t");
        s.type = typedef_type;
        s.value = "sbyte";
    }
    {
        Symbol &s = *Symbols("uint8_t");
        s.type = typedef_type;
        s.value = "byte";
    }
    {
        Symbol &s = *Symbols("int16_t");
        s.type = typedef_type;
        s.value = "short";
    }
    {
        Symbol &s = *Symbols("uint16_t");
        s.type = typedef_type;
        s.value = "ushort";
    }
    {
        Symbol &s = *Symbols("int32_t");
        s.type = typedef_type;
        s.value = "int";
    }
    {
        Symbol &s = *Symbols("uint32_t");
        s.type = typedef_type;
        s.value = "uint";
    }
    {
        Symbol &s = *Symbols("int64_t");
        s.type = typedef_type;
        s.value = "long";
    }
    {
        Symbol &s = *Symbols("uint64_t");
        s.type = typedef_type;
        s.value = "ulong";
    }
    {
        Symbol &s = *Symbols("signed");
        s.type = typedef_type;
        s.value = "int";
    }
    {
        Symbol &s = *Symbols("unsigned");
        s.type = typedef_type;
        s.value = "uint";
    }
    ...
    if (T.parent) // only if we're adding it to parent, so we can create Code Editor in Engine Builder without using GUI at all
    {
        ...
        *parent += b_close.create().func(MenuClose).hide().desc("Close current file");
        ...
        *parent += view_what.create().setData(view_what_elms);
        ...
        view_what.flag |= COMBOBOX_CONST_TEXT;
        ...
        android_certificate.create(T);
        {
            ...
            Gui += build_region.create();
            build_region.kb_lit = false;
            ListColumn lc[] =
                {
                    ListColumn(MEMBER(BuildResult, text), LCW_MAX_DATA_PARENT, "Output"),
                };
            build_region += build_list.create(lc, Elms(lc)).skin(&cjk_skin);
            build_list.column(0).disabled(true);
            build_list.cur_mode = LCM_MOUSE;
            build_list.setElmTextColor(MEMBER(BuildResult, color));
            Gui += build_close.create().func(BuildClose, T);
            build_close.image = "Gui/close.img";
            build_close.skin = &EmptyGuiSkin;
            Gui += build_export.create("Export").func(OutputExport, T);
            Gui += build_copy.create("Copy").func(OutputCopy, T);
            Gui += build_io.create("txt", S, SystemPath(SP_DESKTOP), OutputExport, OutputExport, T);
            build_io.textline.set("Output.txt");
        }
        {
            Gui += devlog_region.create();
            devlog_region.kb_lit = false;
            ListColumn lc[] =
                {
            ...
            lc[0].sort = &devlog_time_sort;
            devlog_time_sort.setDataToTextFunc(DeviceLogOrder); // when sorting by time, then actually sort by order in the container
            devlog_region += devlog_list.create(lc, Elms(lc));
            devlog_list.cur_mode = LCM_MOUSE;
            devlog_list.setElmTextColor(MEMBER(DeviceLog, color));
            Gui += devlog_close.create().func(DevlogClose, T);
            devlog_close.image = "Gui/close.img";
            devlog_close.skin = &EmptyGuiSkin;
            Gui += devlog_filter.create("App Only").func(DevlogFilter2, T).desc("Display messages only from your application");
            devlog_filter.mode = BUTTON_TOGGLE;
            Gui += devlog_export.create("Export").func(DevlogExport, T);
            Gui += devlog_clear.create("Clear").func(DevlogClear, T);
            Gui += devlog_io.create("txt", S, SystemPath(SP_DESKTOP), DevlogExport, DevlogExport, T);
            devlog_io.textline.set("Android Device Log.txt");
        }
...
void CodeEditor::genSymbols(C Str &editor_bin) {
    ...
    SymbolPtr s;
    if (s.find("Bool"))
        FlagEnable(s->modifiers, Symbol::MODIF_SKIP_SUGGESTIONS);
    if (s.find("Char8"))
        FlagEnable(s->modifiers, Symbol::MODIF_SKIP_SUGGESTIONS);
    if (s.find("CChar8"))
        FlagEnable(s->modifiers, Symbol::MODIF_SKIP_SUGGESTIONS);
    ...
    if (s.find("IntPtr")) {
        FlagEnable(s->modifiers, Symbol::MODIF_SKIP_SUGGESTIONS);
        s->value = "intptr";
    } // replace typedef with  "intptr" because that one is changed for 32/64 compilation
    if (s.find("UIntPtr")) {
        FlagEnable(s->modifiers, Symbol::MODIF_SKIP_SUGGESTIONS);
        s->value = "uintptr";
    } // replace typedef with "uintptr" because that one is changed for 32/64 compilation
    if (s.find("Flt"))
        FlagEnable(s->modifiers, Symbol::MODIF_SKIP_SUGGESTIONS);
    if (s.find("Dbl"))
        FlagEnable(s->modifiers, Symbol::MODIF_SKIP_SUGGESTIONS);
    ...
    REPA(Symbols) {
        ...
        if (symbol[0] == '_')
            FlagEnable(symbol.modifiers, Symbol::MODIF_SKIP_SUGGESTIONS);
    }
    Symbols.unlock();
...
void CodeEditor::init() {
    if (!symbols_loaded) {
        ...
        // load
#if LOAD_SYMBOLS
        if (!loadSymbols(CodeEditorDat))
            Exit("Error loading \"Code Editor.dat\".");
#endif
...
Rect CodeEditor::sourceRect() {
    ...
    if (menu.visible())
        if (menu_on_top)
            MIN(r.max.y, menu.rect().min.y);
        else
            MAX(r.min.y, menu.rect().max.y);
    if (code_tabs.visible())
        MIN(r.max.y, code_tabs.rect().min.y);
...
Source *CodeEditor::getSource(C SourceLoc &loc, ERROR_TYPE *error) {
    ...
    if (!s && loc.is()) {
        s = &sources.New();
        ERROR_TYPE e = s->load(loc);
        if (error)
            *error = e;
        if (e != EE_ERR_NONE) {
            sources.removeData(s, true);
            s = null;
        }
    }
    return s;
...
void CodeEditor::nextFile() {
    if (code_tabs.tabs())
        cur(code_tabs.tab(Mod(TabI() + 1, code_tabs.tabs())).source);
} // activate next opened source
void CodeEditor::prevFile() {
    if (code_tabs.tabs())
        cur(code_tabs.tab(Mod(TabI() - 1, code_tabs.tabs())).source);
} // activate prev opened source

Str CodeEditor::title() {
    if (cur()) {
        Str path = cur()->loc.asText();
        if (path.is())
            return S + '"' + path + (cur()->modified() ? "*" : null) + '"' + (cur()->Const ? " (Read Only)" : null);
    }
    return S;
}
...
Str CodeEditor::zipalignPath() C {
    if (android_sdk.is()) {
        ...
        Str sdk = Str(android_sdk).tailSlash(true);
        Str p = sdk + "tools/" + name;
        if (FExistSystem(p))
            return p;
        for (FileFind ff(sdk + "build-tools"); ff();)
            p = ff.pathName() + '/' + name;
        if (FExistSystem(p))
            return p;
    }
...
static Bool EmbedAppResource(C Str &dest, C Str &src, UInt type) {
    File f;
    if (f.readStd(src))
        return EmbedAppResource(dest, f, type);
    return false;
}
void CodeEditor::BuildResult::setWarning() {
    mode = 1;
    setColor();
}
void CodeEditor::BuildResult::setError() {
    mode = 2;
    setColor();
}
void CodeEditor::BuildResult::setColor() {
    ...
    default:
        if (GuiSkin *skin = Gui.skin())
            if (TextStyle *ts = skin->text.text_style()) {
                color = ts->color;
                break;
            }
        color = BLACK;
...
void CodeEditor::DeviceLog::setWarning() {
    mode = WARNING;
    setColor();
}
void CodeEditor::DeviceLog::setError() {
    mode = ERROR;
    setColor();
}
void CodeEditor::DeviceLog::setApp() {
    mode = APP;
    setColor();
}
...
static Str FindPath(C Str &registry, C Str &sub_path) {
    Str path = GetRegStr(RKG_LOCAL_MACHINE, registry);
    if (path.is()) {
...
void CodeEditor::update(Bool active) {
    if (active) {
        ...
        if (Ms.bp(2)) {
            if (build_region.contains(Gui.ms()))
                visibleOutput(false);
            else if (devlog_region.contains(Gui.ms()))
                visibleAndroidDevLog(false);
            else if (code_tabs.contains(Gui.ms())) {
                REPA(code_tabs)
                if (code_tabs.tab(i).contains(Gui.ms())) {
                    close(code_tabs.tab(i).source);
                    break;
                }
            }
        }
            ...
            if (cur()->viewToReal(view, real)) {
                ...
                if (cur()->getSymbolMacroID(real, symbol, macro, id)) {
                    if (symbol && symbol->source)
                        elm_highlight = symbol->source->loc;
                    else if (macro && macro->source)
                        elm_highlight = macro->source->loc;
                    else if (id.valid())
                        elm_highlight = id;
                }
            }
    ...
    if (build_process.created()) {
        ...
        if (refresh) {
            ...
            if (build_log.is()) // read from file
            {
                ...
                if (f.read(build_log))
                    for (; !f.end();)
                        f.fullLine(lines.New());
            } else // read from process
            {
                lines = Split(build_output, '\n');
            }
            buildClear();
            if (build_exe_type == EXE_EXE || build_exe_type == EXE_DLL || build_exe_type == EXE_LIB || build_exe_type == EXE_UWP || build_exe_type == EXE_MAC || build_exe_type == EXE_IOS || build_exe_type == EXE_LINUX || build_exe_type == EXE_APK || build_exe_type == EXE_AAB || build_exe_type == EXE_NS || build_exe_type == EXE_WEB)
                build_step = 0; // we're going to count compilation progress for these platforms
            FREPA(lines) {
                C Str &line = lines[i];
                if (SkipWhiteChars(line).is()) {
                    BuildResult &br = buildNew().set(line);
                    if (Contains(line, "warning:", false, WHOLE_WORD_STRICT) || Contains(line, ": warning", false, WHOLE_WORD_STRICT))
                        br.setWarning();
                    else if (Contains(line, "error:", false, WHOLE_WORD_STRICT) || Contains(line, ": error ", false, WHOLE_WORD_STRICT))
                        br.setError();
                        ...
                        if (build_msbuild) {
                            if (line[0] == ' ' && line[1] == ' ') {
                                if (Ends(line, ".cpp")) {
                                    ...
                                    if (CleanFileName(l) == l)
                                        build_step++;
                                } else if (line == "  Generating Code...")
                                    build_step++;
                            }
                        } else
                            FREPA(line) {
                                if (CharFlagFast(line[i]) & CHARF_DIG) {
                                } else if (line[i] == '>') {
                                    Str l = SkipWhiteChars(line() + i + 1);
                                    if (Ends(l, ".cpp") && CleanFileName(l) == l || Equal(l, "linking..."))
                                        build_step++;
                                } else
                                    break;
                            }
                    } else if (build_exe_type == EXE_APK || build_exe_type == EXE_AAB) {
                        if (Ends(line, ".o") && Contains(line, ".cpp -> ", true, WHOLE_WORD_STRICT))
                            build_step++;
                        if (Contains(line, ": Failure [", true, WHOLE_WORD_STRICT))
                            br.setError(); // happens with certificate mismatch - when wanting to install Debug/Release when other option already installed, full error: ": Failure [INSTALL_FAILED_UPDATE_INCOMPATIBLE: Package *.*.* signatures do not match previously installed version; ignoring!]"
                    } else if (build_exe_type == EXE_MAC || build_exe_type == EXE_IOS) {
                        if (Starts(line, "CompileC", true, WHOLE_WORD_STRICT) || Starts(line, "Ld", true, WHOLE_WORD_STRICT))
                            build_step++;
                        if (Starts(line, "Undefined symbols", true, WHOLE_WORD_STRICT) || Starts(line, "ld: framework not found"))
                            br.setError();
                    } else if (build_exe_type == EXE_LINUX) {
                        if (Starts(line, "g++", true, WHOLE_WORD_STRICT) || Starts(line, "clang++", true, WHOLE_WORD_STRICT))
                            build_step++;
                        if (Starts(line, "/usr/bin/ld: cannot find", true, WHOLE_WORD_STRICT))
                            br.setError();
                    }
                }
            }
            buildUpdate(false);
            if (at_end)
                build_region.scrollEndY(); // auto-scroll to the end
        }
        if (!active) // process finished building
        {
            ...
            if (build_exe_type == EXE_EXE || build_exe_type == EXE_DLL || build_exe_type == EXE_LIB || build_exe_type == EXE_UWP || build_exe_type == EXE_WEB || build_exe_type == EXE_APK || build_exe_type == EXE_AAB) {
                    ...
                    if (1 && ok)          // store hash, !! have to do this before Code Signing, because we modify the EXE file, otherwise it would make the signature invalid !!
                        if (!build_debug) // skip for DEBUG because it interferes with incremental linking
                            if (build_exe_type == EXE_EXE || build_exe_type == EXE_DLL) {
                                Str error;
                                File f;
                                if (!f.readStd(build_exe))
                                    error = "Can't open file";
                                else {
                                    Memc<ExeSection> sections;
                                    if (!ParseExe(f, sections))
                                        error = "Can't parse EXE";
                                    else {
                                        xxHash64 file_hash;
                                        C ExeSection *section_hash_ptr = null;
                                        FREPA(sections) // process in order
                                        {
                                            C ExeSection &section = sections[i];
                                            switch (section.type) {
                                            case ExeSection::VARIABLE:
                                                break; // don't calc hash because this section data can be outside of file range
                                            case ExeSection::HASH:
                                                section_hash_ptr = &section;
                                                break; // remember HASH section for use later
                                            default: {
                                                if (!f.pos(section.offset)) {
                                                    error = "Seek failed";
                                                    goto hash_error;
                                                }
                                                ULong section_hash = f.xxHash64(section.size);
                                                if (!f.ok()) {
                                                    error = "Hash failed";
                                                    goto hash_error;
                                        ...
                                        else if (build_debug ? section_hash_ptr->size < SIZE(ULong) : section_hash_ptr->size != SIZE(ULong))
                                            error = "Invalid hash section size";
                                        else // when building in DEBUG mode, some extra data may be allocated
                                        {
                                            ULong hash = file_hash();
                                            if (Cipher *cipher = CE.cei().appEmbedCipher())
                                                cipher->encrypt(&hash, &hash, SIZE(hash), 0); // encrypt with project cipher
                                            if (!f.pos(section_hash_ptr->offset))
                                                error = "Seek failed";
                                            else {
                                                if (f.getULong() == hash)
                                                    goto hash_ok; // file already has correct hash, check this in case we are building for 2nd time and file was already adjusted, perhaps that EXE is already running so we can't modify it
                                                if (!f.append(build_exe))
                            ...
                            hash_error:
                                BuildError(S + "Error: Can't set EXE hash" + (error.is() ? S + " (" + error + ")" : S), at_end); // ok=false; ignore clearing 'ok' to allow proceeding in case hash is not needed by the user
                            hash_ok:;
                            }
                    if (ok) {
                        build_phase++;
                        if (build_windows_code_sign) // !! have to do this after storing Hash !! CodeSigning will not change any ExeSection offset, size or data
                        {
                            ...
                            if (!signtool.is())
                                signtool = FindPath("Software/Microsoft/Microsoft SDKs/Windows/v10.0/InstallationFolder", "App Certification Kit\\signtool.exe");
                            if (!signtool.is())
                                signtool = FindPath("Software/Microsoft/Windows Kits/Installed Roots/KitsRoot10", "App Certification Kit\\signtool.exe");
                        ...
                        } else if (build_exe_type == EXE_APK && build_mode == BUILD_PLAY) // install APK
                        {
                ...
                } else if (build_phase == (build_windows_code_sign ? 1 : -2)) // verify code sign
                {
                    ok = (exit_code == 0);
                    if (ok)
                        build_phase++;
                } else if (build_phase == 1 && build_exe_type == EXE_AAB) // verify gradlew bundle*
                {
                    ok = (exit_code == 0);
                    if (ok)
                        build_phase++;
                } else if (build_phase == 1 && build_exe_type == EXE_APK && build_mode == BUILD_PLAY) // verify install APK
                {
                ...
                if (ok && build_phase == (build_windows_code_sign                                   ? 2
                    ...
                    case BUILD_PLAY: {
                        ...
                        case EXE_EXE: {
                            Str params;
                            Int editor_port = CE.cei().editorAddrPort();
                            if (editor_port > 0)
                                params.space() += S + "EditorPort " + editor_port;
                            Run(build_exe, params);
                        } break;

                        case EXE_WEB:
                            goto publish; // when playing for WEB we will create PAKs after compilation, so call publish success to do so
                    ...
                    case BUILD_PUBLISH:
                    publish:
                        cei().publishSuccess(build_exe, build_exe_type, build_mode, build_project_id);
                        break;
                    }
                else if (build_phase == ((build_exe_type == EXE_APK && build_mode == BUILD_PLAY) ? 3 : -2)) {
                ...
                if (ok) {
                    ...
                    if (!FRename(build_exe, new_exe))
                        Gui.msgBox(S, S + "Can't rename file from: " + build_exe + "\nto: " + new_exe);
                    else {
                        build_exe = new_exe;
                        ...
                        FREPA(build_embed) {
                            ...
                            if (!EmbedAppResource(build_exe, be.path, be.type)) {
                                Gui.msgBox(S, "Can't embed app resource");
                                ok = false;
                                break;
                            }
                        }
            ...
            } else if (build_exe_type == EXE_MAC) {
                Bool ok = false;
                // if(exit_code>=0)ok=(exit_code==0);else                                     // exit code known
                if (build_data.elms())
                    ok = (build_data.last().text == "** BUILD SUCCEEDED **"); // detect success
                if (ok)
                    switch (build_mode) {
                    case BUILD_PLAY:
                    case BUILD_DEBUG:
        ...
        if (log.is()) {
            ...
            if (lines.elms() >= 2) {
                ...
                FREP(lines.elms() - 1) {
                    ...
                    if (l[0] != '-')
                        if (CChar *time = TextPos(l, ' ')) // skip date "mm-dd"
                        {
                            time++;
                            if (CChar *debug = TextPos(time, ' ')) {
                                ...
                                if (Equal(dl.app, ENGINE_NAME))
                                    dl.setApp();
                                else if (*debug == 'D')
                                    dl.setWarning();
                                else if (*debug == 'E' || *debug == 'F')
                                    dl.setError();
                                // filter out unwanted messages
                                if (Equal(dl.app, "AlarmManager") || Equal(dl.app, "Lights") // on HTC control screen brightness
                                    || Equal(dl.app, "Sensors")                              // gyroscope, accelerometer, etc
                                                                                             //|| Equal(dl.app, "Ethernet")
                                                                                             //|| Equal(dl.app, "WifiService")
                                    || Equal(dl.app, "BatteryService") || Equal(dl.app, "BATT_ALG") || Equal(dl.app, "TabletStatusBar") || Equal(dl.app, "CameraService") || Equal(dl.app, "NvOmxCamera") || Equal(dl.app, "NvOmxCameraBuffers") || Equal(dl.app, "NvOmxCameraCallbacks") || Equal(dl.app, "NvOmxCameraSettings") || Equal(dl.app, "NvOmxCameraSettingsParser") || Equal(dl.app, "PowerManagerService") || Equal(dl.app, "DownloadManager") || Equal(dl.app, "[WeatherService]") || Equal(dl.app, "[WeatherService1]") || Equal(dl.app, "[WeatherReceiver]") || Equal(dl.app, "[WeatherSettingsAdapter]") || Equal(dl.app, "Unity") || (Equal(dl.app, "DalvikVM") && (Starts(dl.message, "GC_EXPLICIT") || Starts(dl.message, "GC_CONCURRENT"))) || (Equal(dl.app, "LocationManagerService") && Starts(dl.message, "getLastLocation: Request")))
                ...
                if (devlog_filter()) {
                    remove_vis = 0;
                    FREP(remove)
                    remove_vis += (devlog_data[i].mode == DeviceLog::APP);
                }
                devlog_data.removeNum(0, remove, true);
                if (devlog_list.lit >= 0)
                    devlog_list.lit -= remove_vis;
                if (devlog_list.cur >= 0)
                    devlog_list.cur -= remove_vis;
                if (!at_end)
                    devlog_region.scrollY(-remove_vis * devlog_list.elmHeight(), true); // keep current position, call before 'DevlogFilter' so it can use current position to detect 'lit/cur'
                DevlogFilter(T);
                if (at_end)
                    devlog_region.scrollEndY(); // auto-scroll to the end
            }
        }
    }
}
...
void CodeEditor::rebuild3rdPartyHeaders() {
...
#endif
    FREPA(headers)
    parseHeader(headers[i], LibMacros, LibUsings, false);
    ...
    // add stuff from EE
    REPA(EEUsings)
    LibUsings.include(EEUsings[i]); // include all usings defined by EE
    REPA(EEMacros)
    LibMacros.exclude(EEMacros[i]); // remove  all macros defined by EE (to avoid duplicates, and replace old ones with new ones)
    REPA(EEMacros)
    LibMacros.add(EEMacros[i]); // add     all macros defined by EE
...
void CodeEditor::rebuildSymbols(Bool rebuild_3rd_party_headers) {
    ...
    if (rebuild_3rd_party_headers)
        rebuild3rdPartyHeaders();
    // setup custom macros
    Macro *macro;
    Int index;
    CChar8 *name = "STEAM";
    if (ProjectMacros.binarySearch(name, index, CompareCS))
        macro = &ProjectMacros[index];
    else
        macro = &ProjectMacros.NewAt(index).set(name);
    macro->def = cei().appPublishSteamDll();
    macro->parts.setNum(1).first().set(TOKEN_NUMBER, -1, &BStr().setBorrowed(macro->def));
    name = "OPEN_VR";
    if (ProjectMacros.binarySearch(name, index, CompareCS))
        macro = &ProjectMacros[index];
    else
        macro = &ProjectMacros.NewAt(index).set(name);
    macro->def = cei().appPublishOpenVRDll();
    macro->parts.setNum(1).first().set(TOKEN_NUMBER, -1, &BStr().setBorrowed(macro->def));
...
Str CodeEditorInterface::appPath(C Str &app_name) {
    Str build_path, build_project_name;
    return CE.getBuildPath(build_path, build_project_name, &app_name) ? build_path : S;
}
Str CodeEditorInterface::androidPath() {
    Str build_path, build_project_name;
    return CE.getBuildPath(build_path, build_project_name) ? build_path + "Android" : S;
}
Str CodeEditorInterface::androidProjectPakPath() {
    Str build_path, build_project_name;
    return CE.getBuildPath(build_path, build_project_name) ? build_path + "Android/app/src/main/assets/Project.pak" : S;
}
Str CodeEditorInterface::iOSProjectPakPath() {
    Str build_path, build_project_name;
    return CE.getBuildPath(build_path, build_project_name) ? build_path + "Assets/Project.pak" : S;
}
Str CodeEditorInterface::UWPProjectPakPath() {
    Str build_path, build_project_name;
    return CE.getBuildPath(build_path, build_project_name) ? build_path + "Project.pak" : S;
} // if we would set "Assets/Project.pak" then the file would be included inside the UWP EXE including the "Assets/" too
Str CodeEditorInterface::nintendoProjectPakPath() {
    Str build_path, build_project_name;
    return CE.getBuildPath(build_path, build_project_name) ? build_path + "Assets/Nintendo Switch/Data/Project.pak" : S;
}
...
Bool CodeEditorInterface::sourceCur(C Str &name) {
    CE.init();
    return CE.load(name);
} // 'init' in case we're loading source from data
...
void CodeEditorInterface::sourceRename(C UID &id) {
    SourceLoc loc = id;
    if (Source *source = CE.findSource(loc)) {
        source->loc = loc;
        REPA(CE.code_tabs)
        if (CE.code_tabs.tab(i).source == source) {
            CE.code_tabs.tab(i).text(loc.base_name);
            break;
        }
    }
}
void CodeEditorInterface::paste(C CMemPtr<UID> &elms, GuiObj *obj, C Vec2 &screen_pos) {
    if (Source *src = CE.cur())
        if (!src->Const && src->contains(obj) && elms.elms()) {
            Str text;
            Vec2 c = src->posCur(screen_pos);
            MAX(c.x, 0);
            MAX(c.y, 0);
            src->sel = src->sel_temp = -1;
            src->cur.set(Round(c.x), Trunc(c.y));
            if (CE.view_mode())
                src->viewToReal(src->cur, src->cur);
            src->curClip();
            FREPA(elms) {
                if (i) {
                    if (Kb.ctrlCmd())
...
Bool CodeEditorInterface::Export(EXPORT_MODE mode) {
    if (CE.Export(mode, BUILD_EXPORT)) {
        Explore(CE.build_path);
        return true;
    }
    return false;
}
...
void CodeEditorInterface::sourceDrawPreview(C Str &name) {
    CE.init();
    DrawPreview(name);
} // 'init' in case we're viewing source from data

void CodeEditorInterface::sourceAuto(C Str &data) {
    Source *source = CE.findSource(Str(AutoSource));
    if (!source) {
        source = &CE.sources.New();
        source->loc.setFile(AutoSource);
    }
    source->cpp = false;
    source->Const = true;
    source->active = true;
...
void CodeEditorInterface::sourceOverwrite(C UID &id) {
    if (Source *source = CE.findSource(id))
        if (source->modified())
            source->overwrite();
}

Bool CodeEditorInterface::sourceDataGet(C UID &id, Str &data) {
    if (Source *source = CE.findSource(id)) {
        data = source->asText();
        return true;
    }
...
Bool CodeEditorInterface::sourceDataSet(C UID &id, C Str &data) {
    if (Source *source = CE.findSource(id)) {
        source->setUndo();
        source->fromText(data);
        source->undo_original_state = source->undos.undos();
        source->forceCreateNextUndo();
        return true;
    }
    return false;
}

void CodeEditorInterface::kbSet() {
    if (CE.cur())
        CE.cur()->kbSet();
}
...
void CodeEditorInterface::clearActiveSources() {
    REPA(CE.sources) {
        Source &src = CE.sources[i];
        src.was_active = src.active;
        src.active = false;
    }
}
void CodeEditorInterface::activateSource(C UID &id) {
    if (Source *source = CE.getSource(id))
        source->active = true;
}
void CodeEditorInterface::activateSource(C Str &name) {
    if (Source *source = CE.getSource(name))
        source->active = true;
}
void CodeEditorInterface::activateApp(Bool rebuild_3rd_party_headers) {
    // always make sure auto generated header is active
    if (Source *source = CE.findSource(Str(AutoSource)))
        source->active = true;
    ...
    if (!CE.symbols_loaded && CE.cur() && CE.cur()->used()) {
        ...
        // rebuild
        if (rebuild || rebuild_3rd_party_headers)
            CE.rebuildSymbols(rebuild_3rd_party_headers);
    }
...
Bool CheckVisualStudio(C VecI4 &vs_ver, Str *message, Bool check_minor) {
    if (vs_ver.x <= 0) {
        if (message)
            *message = S + "Visual Studio was not found.\n" + SupportedVS;
        return false;
    }
    if (vs_ver.x < 15) {
        if (message)
            *message = S + "Visual Studio version " + TextVer(vs_ver) + " is no longer supported.\n" + SupportedVS;
        return false;
    }
    if (vs_ver.x > 17) {
        if (message)
            *message = S + "Visual Studio version " + TextVer(vs_ver) + " is not yet supported.\n" + SupportedVS;
        return false;
    } // #VisualStudio
    if (check_minor) {
        if (vs_ver.x == 15 && vs_ver.y >= 0 && vs_ver.y < 5) {
            if (message)
                *message = S + "Visual Studio version " + TextVer(vs_ver) + " is not supported due to a bug in compiler.\n" + SupportedVS;
            return false;
        } // disable VS 2017 ver 15.0 .. 15.4
    }
    if (message)
        message->clear();
    return true;
...
static Int Compare(C VisualStudioInstallation &a, C VisualStudioInstallation &b) {
    ...
    if (Int c = CompareNumber(a.name, b.name))
        return c;
    if (Int c = ComparePathNumber(a.path, b.path))
        return c;
    return 0;
}
...
#endif
                                                Char8 temp[256];
                                                Set(temp, WChar(install_version.operator const wchar_t *()));
                                                ReplaceSelf(temp, '.', ',');
                                                VisualStudioInstallation &install = installs.New();
                                                install.path = path.operator const wchar_t *();
                                                install.ver = TextVecI4(temp);
                                                if (OK(instance->GetDisplayName(0, display_name.GetAddress())))
                                                    install.name = display_name.operator const wchar_t *();
                                            }
            }
        }
    }
...
static Str _MSBuildPath(C Str &vs_path, C VecI4 &vs_ver) {
    if (vs_path.is()) {
        Str ms_build_path = vs_path;
        ms_build_path.tailSlash(true) += "MSBuild\\";
        if (vs_ver.x > 0) {
        ...
        Memt<Str> paths;
        for (FileFind ff(ms_build_path); ff();)
            if (ff.type == FSTD_DIR && FExistSystem(ff.pathName() + "\\Bin\\MSBuild.exe"))
                paths.binaryInclude(ff.name, CompareNumberCI); // use CompareNumber to make sure we have correct sorting by version number
        if (paths.elms())
            return ms_build_path + paths.last() + "\\Bin\\MSBuild.exe"; // use the latest version available
    }
    return S;
}
Str MSBuildPath(C Str &vs_path, C VecI4 &vs_ver) {
    ...
    if (GetVisualStudioInstallations(installs))
        REPA(installs) // go from the end to try the latest version first
        {
            VisualStudioInstallation &install = installs[i];
            if (!EqualPath(vs_path, install.path) && CheckVisualStudio(install.ver)) {
...
Str MSBuildParams(C Str &project, C Str &config, C Str &platform) {
    ...
    int maxThreads = 1; // Spécifiez le nombre maximal de threads ici
    return S + '"' + NormalizePath(MakeFullPath(project)) + "\" /Verbosity:minimal /nologo /m:" + maxThreads + (config.is() ? S + " /p:Configuration=\"" + config + '"' : S) + (platform.is() ? S + " /p:Platform=\"" + platform + '"' : S);
}
