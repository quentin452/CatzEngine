...
Int Compare(C Edit::Message &a, C Edit::Message &b) {
    if (a.source && b.source) {
        if (a.source != b.source)
            if (Int c = ComparePath(a.source->loc.asText(), b.source->loc.asText()))
                return c;
...
Int CompilerContext::heapConstant(CPtr data, Int size, Int align) {
    if (data && size > 0) {
        Int l = 0, r = constants.elms();
        for (; l < r;) {
            Int mid = UInt(l + r) / 2;
            Constant &c = constants[mid];
...
Int Compiler::heapConstant(CPtr data, Int size, Int align) {
    if (!ctx)
        msgs.New().error("CompilerContext not set", source, -1);
    return ctx ? ctx->heapConstant(data, size, align) : 0;
}
void Compiler::varHeapConstant(Int heap_offset, CPtr data, Int size) {
    if (!ctx)
        msgs.New().error("CompilerContext not set", source, -1);
    if (ctx)
        ctx->varHeapConstant(heap_offset, data, size);
}
...
void Compiler::mapLocalInStack(Expr::Memory &mem, Bool error_on_fail) {
    if (mem.type != Expr::Memory::LOCAL)
        msgs.New().error("Can't map variable in the stack because it's not a local variable");
    else if (!InRange(mem.index, locals))
        msgs.New().error("Specified local variable index is invalid");
    else if (!ctx)
        msgs.New().error("null compiler context");
    else {
        Local &local = locals[mem.index];
        if (local.stack_offset >= 0) {
            if (error_on_fail)
                msgs.New().error("Can't map local variable to stack because it's already mapped");
        } else {
            ...
            if (local.type.hasDestructor())
                live_locals.add(mem.index);  // add to just mapped container
...
void Compiler::mapVar(Expr::Memory &mem) {
    if (mem.type == Expr::Memory::LOCAL)
        mapLocalInStack(mem, false);
}
Bool Compiler::unmappedLocal(Expr::Memory &mem) {
    if (mem.type == Expr::Memory::LOCAL && InRange(mem.index, locals))
        return locals[mem.index].stack_offset < 0;
    return false;
}
/******************************************************************************/
void Compiler::expand(Memc<Command> &src, Memc<Command> &dest, Int label_break, Int label_continue, Int label_return, Int label_scope) {
    FREPA(src) {
        ...
        case CMD_LABEL: {
            Command &d = dest.New();
            d = cmd;
            if (InRange(cmd.raw_range.x, tokens)) {
                Token &token = *tokens[cmd.raw_range.x];
                if (token.symbol && token.symbol->type == Symbol::LABEL)
                    d.label_index = token.symbol->raw_offset;
            }
        } break;
        ...
        case CMD_IF: {
           ...
           false_scope:            #6
            ...
            expand(cmd.cmds, dest, label_break, label_continue, label_return, label_true_scope); // 1
            {
                Command &d = dest.New();
                d.type = CMD_LABEL;
                d.label_index = label_true_scope;
            } // 2
            if (has_else) {
                Command &d = dest.New();
                d.type = CMD_GOTO;
                d.label_index = label_end;
            } // 3
            {
                Command &d = dest.New();
                d.type = CMD_LABEL;
            ...
            if (has_else)
                expand(cmd.cmds_false, dest, label_break, label_continue, label_return, label_false_scope); // 5
            if (has_else) {
                Command &d = dest.New();
                d.type = CMD_LABEL;
                d.label_index = label_false_scope;
            } // 6
            {
                Command &d = dest.New();
                d.type = CMD_LABEL;
        ...
        case CMD_FOR: {
           ...
           break: // any "break;" command will jump here       #9*/
            ...
            Int label_loop = labels++,
                ...
                label_break = labels++;
            if (cmd._for.init_range.x <= cmd._for.init_range.y) {
                Command &d = dest.New();
                d.type = CMD_INSTRUCT;
                d.raw_range = cmd._for.init_range;
                d.scope_label = label_break;
            } // 0 (add step only if 'init_range' is specified)
            {
                Command &d = dest.New();
                d.type = CMD_LABEL;
                d.label_index = label_loop;
            } // 1
            if (cmd._for.cond_range.x <= cmd._for.cond_range.y) {
                Command &d = dest.New();
                d.type = CMD_GOTO_COND_N;
                d._for.cond_range = cmd._for.cond_range;
                d.label_index = label_break;
            ...
            expand(cmd.cmds, dest, label_break, label_continue, label_return, label_cmds_scope); // 3
            {
                Command &d = dest.New();
                d.type = CMD_LABEL;
                d.label_index = label_cmds_scope;
            } // 4
            {
                Command &d = dest.New();
                d.type = CMD_LABEL;
            ...
            if (cmd._for.step_range.x <= cmd._for.step_range.y) {
                Command &d = dest.New();
                d.type = CMD_INSTRUCT;
                d.raw_range = cmd._for.step_range;
                d.scope_label = label_step_scope;
            } // 6 (add step only if 'step_range' is specified)
            if (cmd._for.step_range.x <= cmd._for.step_range.y) {
                Command &d = dest.New();
                d.type = CMD_LABEL;
                d.label_index = label_step_scope;
            } // 7 (add step only if 'step_range' is specified)
            {
                Command &d = dest.New();
                d.type = CMD_GOTO;
                d.label_index = label_loop;
...
static void CompileSetConst(Expr &expr, Compiler &compiler, Memc<Byte> &code) {
    ...
    if (size > 0) // size can be 0 for empty classes
    {
        Int params = 2, // stack_offset + raw value
            ...
            offset = AlignAddress(compiler.stack_size, align);
        compiler.stack_size = offset + size; // ALIGN
        ...
        call.params = params;
        call.func = GetSetConstCall(size, heap);
        if (!call.func) {
            compiler.msgs.New().error("CompileSetConst invalid expression size");
            return;
        }
        call.param(0).setInstructI(offset); // set as offset from stack to the value
        if (heap)
            call.param(1).setHeap(compiler.heapConstant(&raw, size, align), 0, false); // create constant on the heap
        else
            call.param(1).setInstruct(expr.asRaw(compiler));
    }
}
static void CompileSetAddrStackHeap(Expr &expr, Compiler &compiler, Bool stack, Int offset, Memc<Byte> &code, Bool auto_map = true) // this function puts address of 'expr' variable into specified position offset at the stack/heap
{
    ...
    Call &call = *(Call *)(&code[pos]);
    call.params = params;
    call.func = (stack ? GetSetAddrStackCall() : GetSetAddrHeapCall());
    if (!call.func) {
        compiler.msgs.New().error("CompileSetAddrStackHeap unknown func call");
        return;
    }
...
static void CompileSetAddrResult(Expr &expr, Compiler &compiler, Memc<Byte> &code) // this function puts address of 'expr' variable into the result stack
{
    ...
    if (!call.func) {
        compiler.msgs.New().error("CompileSetAddrResult unknown func call");
        return;
    }
...
static void CompileExpr(Expr &expr, Compiler &compiler, Memc<Byte> &code) {
    ...
    FREPA(expr.parent)
    CompileExpr(expr.parent[i], compiler, code);
    ...
    if (!expr.known()) {
        if (expr.func.elms() == 1) {
            ...
            {
                FREPA(func.params)
                CompileExpr(func.params[i], compiler, code);
            } else if (func.func_call) {
                // compile func parent and params first
                FREPA(func.parent)
                CompileExpr(func.parent[i], compiler, code);
                FREPA(func.params)
                CompileExpr(func.params[i], compiler, code);
                ...
                if (func.func_call != GetIgnoreCall()) {
                    ...
                    params = 0;
                    if (func.func_call_mask & Expr::FC_RESULT) {
                        call.param(params++).set(expr, compiler);
                    }
                    if (func.func_call_mask & Expr::FC_PARENT) {
                        if (func.parent.elms() != 1)
                            goto error;
                        call.param(params++).set(func.parent[0], compiler);
                    }
                    FREPA(func.params) { call.param(params++).set(func.params[i], compiler); }
                }
                return;                                                                                                        // success
            } else if (func.symbol && func.symbol->type == Symbol::FUNC && func.symbol->source && func.symbol->source->active) // Code Editor function, RESULT PARENT PARAMS
            {
                if (func.params.elms() != func.symbol->realParams())
                    compiler.msgs.New().error("Invalid number of function parameters", expr.origin);
                else {
                    ...
                    // TODO: array_dims
                    Mems<PASS_TYPE> param_pass;
                    param_pass.setNumZero(func.params.elms());
                    FREPA(func.params) // order is important as parameters must be in continuous memory
                    {
                        Expr &src = func.params[i];
                        Symbol::Modif dest = func.symbol->params[i]->value;
                        dest.proceedToFinal(&expr.symbol.templates, true);
                        ...
                        UInt src_const_level = src.symbol.const_level;
                        FlagDisable(src_const_level, 1 << (src.symbol.ptr_level + src.symbol.array_dims.elms())); // disable top const
                        ...
                        } else if ((is_null || LostConst(src.symbol.const_level, dest.const_level)) && FlagOn(dest.modifiers, Symbol::MODIF_REF)) // const int/int& -> int&  BAD
                        {
                        ...
                        {
                            if (src.mem.type == Expr::Memory::LOCAL)
                                param_pass[i] = PASS_MAP;
                            else if (src.mem.type == Expr::Memory::KNOWN)
                                param_pass[i] = PASS_CONST;
                            else // since we're copying the constant to the stack, then this allows above optimization when losing const but operating on basic type
                                compiler.msgs.New().error("Invalid expression memory type for src_temporary + dest_parameter merging optimization", src.origin);
                        } else if (dest.modifiers & Symbol::MODIF_REF) // use REF CTOR, compile code which sets Ptr address at specified location of a custom variable
                        {
                    ...
                    if (param_pass.elms())
                        compiler.stack_size = Ceil8(compiler.stack_size); // must be aligned to max possible var size, because when compiling functions, each of their parameter's offset is calculated from zero
                    Int param_stack_offset = compiler.stack_size;
                    FREPA(param_pass) {
                        Expr &src = func.params[i];
                        switch (param_pass[i]) {
                        case PASS_MAP:
                            compiler.mapLocalInStack(src.mem);
                            break;
                        case PASS_CONST:
                            CompileSetConst(src, compiler, code);
                            break;
                        case PASS_ADDR: {
                            Int offset = AlignAddress(compiler.stack_size, PtrSize);
                            compiler.stack_size = offset + PtrSize;
                            CompileSetAddrStackHeap(src, compiler, true, offset, code, false);
                        } break;
                        }
                    }
                    ...
                    if (has_this) {
                        ...
                        if (func.parent.elms() == 1) {
                            Expr &expr_parent = func.parent[0];
                            if (!expr_parent.instance || !expr_parent.symbol.isObj())
                                compiler.msgs.New().error("Invalid object for method call", expr.origin);
                    ...
                    if (has_result)
                        call.param(params++).setRef(expr, compiler); // if result is a reference then we want to get the address of the reference and not the value that it points to
                }
...
static void DestroyVar(Symbol::Modif &type, Token *token, Compiler &compiler, Memc<Byte> &code, Bool local, Int local_index, Symbol *global_symbol) {
    if (type.isArray()) {
        ...
        if (!func.symbol && !func.func_call)
            compiler.msgs.New().error(S + "Destructor not found for " + type.modifName(false, false, false), token);
        else {
            parent.instance = true;
            parent.symbol = type;
            if (local)
                parent.mem.setLocal(local_index);
            else
                parent.mem.setGlobal(*global_symbol);
            if (func.func_call || result.calculate(compiler) > CAST_NONE)
                CompileExpr(result, compiler, code); // there is no need to calculate if we already know the 'func_call'
        }
    }
}
static void DestroyLiveLocal(Int i, Bool remove, Compiler &compiler, Memc<Byte> &code) {
    if (InRange(i, compiler.live_locals)) {
        Int local_index = compiler.live_locals[i];
        if (remove)
            compiler.live_locals.remove(i, true); // keep order
...
static void DestroyInstructScopes(Compiler &compiler, Memc<Byte> &code) {
    REPA(compiler.live_locals) // process in reversed order (ORDER IS IMPORTANT)
    {
        ...
        if (!local.block_scope)
            DestroyLiveLocal(i, true, compiler, code); // only instruction scope
    }
}
static void DestroyBlockScopes(Int label_index, Compiler &compiler, Memc<Byte> &code) {
    ...
    {
        Local &local = compiler.locals[compiler.live_locals[i]];
        if (local.block_scope && local.scope_label == label_index)
            DestroyLiveLocal(i, true, compiler, code);
    }
}
...
static void CreateVar(Expr &var, Token &token, Symbol &symbol, Compiler &compiler, Memc<Byte> &code) {
    ...
    {
        if (var.symbol.isArray())
            compiler.msgs.New().error("Arrays of references are not supported", &token);
        else if (!(symbol.modifiers & Symbol::MODIF_DEF_VALUE))
            compiler.msgs.New().error("References must be initialized", &token);
        else {
            Expr expr;
            if (!symbol.source || compiler.compileTokens(symbol.def_val_range.x, symbol.def_val_range.y, expr, &symbol.source->tokens) != COMPILE_FULL)
                compiler.msgs.New().error("Can't compile tokens", symbol.source, symbol.def_val_range.x);
            else if (!expr.castTo(var.symbol, compiler))
                compiler.msgs.New().error(S + "Can't initialize '" + var.symbol.modifName(true, true, true) + "' from '" + expr.symbol.modifName(true, true, false), &token);
            else {
                ...
                if (var.mem.type == Expr::Memory::LOCAL) {
                    if (!InRange(var.mem.index, compiler.locals))
                        compiler.msgs.New().error("Local variable index is invalid", &token);
                    else {
                        Local &local = compiler.locals[var.mem.index];
                        if (local.stack_offset < 0)
                            compiler.msgs.New().error("Local variable stack offset is invalid", &token);
                        else
                            CompileSetAddrStackHeap(expr, compiler, !local.force_heap, local.stack_offset, code); // add instruction that sets the 'expr' address into the 'var' reference
                    }
                } else if (var.mem.type == Expr::Memory::GLOBAL) {
                    if (var.mem.index < 0)
                        compiler.msgs.New().error("Global variable heap offset is invalid", &token);
                    else
                        CompileSetAddrStackHeap(expr, compiler, false, var.mem.index, code); // add instruction that sets the 'expr' address into the 'var' reference
                } else {
                    compiler.msgs.New().error("Invalid variable memory type for CreateVar", &token);
                }
            }
        }
    } else if (var.symbol.anyPtr() || (var.symbol && var.symbol->var_type != VAR_NONE)) // basic type obj or pointer - "int x", "Obj *x"
    {
        if (symbol.modifiers & Symbol::MODIF_DEF_VALUE) {
            if (var.symbol.isArray())
                compiler.msgs.New().error("Variable initialization as array is not yet supported", &token);
            else {
                var.symbol.setConst(false); // disable const to allow setting the value
                Memc<Expr> expr;
                Swap(expr.New(), var);
                expr.New().setOperator(u"=").origin = &token;
                expr.New().setOperator(u"(");
                Bool ok = (symbol.source && compiler.appendTokens(expr, symbol.def_val_range.x, symbol.def_val_range.y, &symbol.source->tokens));
                expr.New().setOperator(u")"); // create the "var=(..)" expressions
                Expr result;
                if (!ok || compiler.compileExpr(expr, token.parent, result) != COMPILE_FULL)
                    compiler.msgs.New().error("Can't compile tokens", symbol.source, symbol.def_val_range.x);
                else {
                    if (compiler.ctx && compiler.ctx->store_known_global_var_on_heap && result.mem.type == Expr::Memory::GLOBAL && result.mem.index >= 0 // if we're initializing global var
                        && result.func.elms() == 1 && result.func[0] == '=' && result.func[0].params.elms() == 2 && result.func[0].params[1].known())    // and the result is known then store that value on the heap instead of as codes (check known for the 'b' in "a=b" because 'a' known may be disabled)
                    {
                        ...
                        U64 raw = temp.asRaw(compiler);
                        compiler.varHeapConstant(result.mem.index + result.mem.offset, &raw, result.symbol.rawSize(true));
                    } else {
...
{
    if (InRange(i + 1, cmds)) {
        Command &cmd = cmds[i + 1];
        return cmd.type == CMD_LABEL && cmd.label_index == label_index;
    }
    return false;
}
#else
static Bool FollowedByLabel(Memc<Command> &cmds, Int i, Int label_index) // check all following labels if one of them is 'label_index' (break on any other command)
{
    for (; ++i < cmds.elms();) // start from 1 after 'i'
    {
        ...
        if (cmd.type != CMD_LABEL)
            break;
        if (cmd.label_index == label_index)
            return true;
    }
    return false;
}
...
void Compiler::compile(Memc<Command> &cmds, Mems<Byte> &code, Symbol &func, Symbol::Modif *result_value) {
    Memc<Int> label_code_pos;
    label_code_pos.setNum(labels); // code position for labels
    ...
    FREPA(cmds) {
        ...
        switch (cmd.type) {
        default:
            msgs.New().error("Unsupported CMD_TYPE", source, cmd.startTokenIndex());
            break;

        case CMD_LABEL: {
            if (!InRange(cmd.label_index, labels))
                msgs.New().error("Unknown label", source, cmd.startTokenIndex());
            else {
        ...
        case CMD_INSTRUCT: {
            ...
            for (Int i = cmd.raw_range.x; i <= cmd.raw_range.y && InRange(i, tokens); i++) // check if in the token range there is a variable definition "int x;", "int a=0, b=a;", "Vec v(0,0,0);" (order is important)
            {
                ...
                if (token.def_decl)
                    if (Symbol *symbol = token.symbol()) // TYPE x; TYPE x=..; TYPE x(..);
                    {
                        has_vars = true;
                        if (symbol->isVar()) // ignore functions, classes or some other declarations
                        {
                            Expr var;
                            CreateVar(var, token, *symbol, T, temp);
                        }
                    }
            }
            ...
            {
                Expr expr;
                if (compileTokens(cmd.raw_range.x, cmd.raw_range.y, expr) != COMPILE_FULL)
                    msgs.New().error("Can't compile tokens", source, cmd.startTokenIndex());
                else {
        ...
        case CMD_RETURN: // RESULT PARENT PARAMS
        {
            ...
            {
                if (!result_value)
                    msgs.New().error("Function can't return a value", source, cmd.startTokenIndex());
                else {
                    Expr result, value;
                    ...
                    if (result.symbol.modifiers & Symbol::MODIF_REF) {
                        if (compileTokens(cmd.raw_range.x, cmd.raw_range.y, value) != COMPILE_FULL)
                            msgs.New().error("Can't compile tokens", source, cmd.raw_range.x);
                        else if (!value.castTo(result.symbol, T))
                            msgs.New().error(S + "Can't initialize '" + result.symbol.modifName(true, true, true) + "' from '" + value.symbol.modifName(true, true, false), source, cmd.raw_range.x);
                        else {
                    ...
                    } else if (result.basicType() && !result.symbol.isArray()) // basic type obj or pointer - "int x", "Obj *x"
                    {
                        result.symbol.setConst(false); // disable const to allow setting the value
                        Memc<Expr> expr;
                        Swap(expr.New(), result);
                        expr.New().setOperator(u"=");
                        expr.New().setOperator(u"(");
                        Bool ok = appendTokens(expr, cmd.raw_range.x, cmd.raw_range.y);
                        expr.New().setOperator(u")"); // create the "result=(..)" expressions
                        if (!ok || compileExpr(expr, InRange(cmd.raw_range.x, tokens) ? tokens[cmd.raw_range.x]->parent : null, value) != COMPILE_FULL)
                            msgs.New().error("Can't compile tokens", source, cmd.raw_range.x);
                        else
                            CompileExpr(value, T, temp);
                    } else if (!result.basicType() && result.symbol.isObj()) // class object - "Vec x", "Vec x(..)", "Str x(..)"
                    {
                        if (compileTokens(cmd.raw_range.x, cmd.raw_range.y, value) != COMPILE_FULL)
                            msgs.New().error("Can't compile tokens", source, cmd.raw_range.x);
                        else {
                            ...
                            } else // convert result to class constructor, and convert "return x" -> "result(x)" (as in ctor call)
                            {
                                ...
                                Expr ctor, &func = ctor.func.New();
                                Swap(func, result);
                                Swap(func.params.New(), value);
                                if (ctor.calculate(T) > CAST_NONE)
                                    CompileExpr(ctor, T, temp);
                            }
                        }
                    } else {
                        // TODO:
            ...
            {
                if (result_value)
                    msgs.New().error("Function must return a value", source, cmd.startTokenIndex());
            }
            DestroyInstructScopes(T, temp);
        ...
        case CMD_BREAK:
            if (InRange(cmd.label_index, labels))
                goto write_goto;
            msgs.New().error("Invalid 'break'");
            break;
        case CMD_CONTINUE:
            if (InRange(cmd.label_index, labels))
                goto write_goto;
            msgs.New().error("Invalid 'continue'");
            break;
        ...
        write_goto : {
            ...
            else if (!FollowedByLabel(cmds, i, cmd.label_index)) // check if we're not using goto to jump to command after us
            {
                ...
                if (!call.func)
                    msgs.New().error("Unknown 'goto' func", source, cmd.startTokenIndex());
        ...
        case CMD_GOTO_COND_N: // if(!cond)goto label; -> if(cond==0)goto label;
        {
            Expr cond;
            if (!InRange(cmd.label_index, labels))
                msgs.New().error("Unknown label for 'goto conditional' command", source, cmd.startTokenIndex());
            else {
                Bool has_var = false;
                for (Int i = cmd._for.cond_range.x; i <= cmd._for.cond_range.y && InRange(i, tokens); i++) // check if in the token range there is a variable definition "int x;", "int a=0, b=a;", "Vec v(0,0,0);" (order is important)
                {
                    ...
                    if (token.def_decl)
                        ...
                        {
                            if (has_var) {
                                msgs.New().error("Multiple variable definition inside condition is invalid", source, cmd.startTokenIndex());
                                break;
                            }
                            has_var = true;
                            if (symbol->isVar())
                                CreateVar(cond, token, *symbol, T, temp);
                            else
                                msgs.New().error("Invalid symbol declaration inside condition", source, cmd.startTokenIndex());
                        }
                }
                if (!has_var) {
                    if (compileTokens(cmd._for.cond_range.x, cmd._for.cond_range.y, cond) != COMPILE_FULL) {
                        msgs.New().error("Can't compile tokens", source, cmd.startTokenIndex());
                        break;
                    }
                }
                ...
                if (cond.known()) {
                    if (cond.asBool(T) == (cmd.type == CMD_GOTO_COND))
                        goto write_goto; // for GOTO_COND type add goto only if "cond!=0", for GOTO_COND_N type add goto only if "cond==0" (in other case don't do anything)
                } else {
                    ...
                    if (!cond.castToConditional())
                        msgs.New().error("Can't convert to bool", source, cmd.startTokenIndex());
                    else {
                        ...
                        if (!has_var) // don't need to test if it's a named variable
                        {
                            ...
                            {
                                REPA(live_locals)
                                if (!locals[live_locals[i]].block_scope) // if there is at least one variable that needs to be destroyed
                                {
                                    cond.copyCtor(T);
                                    break;
                                }
                        ...
                        if (!FollowedByLabel(cmds, i, cmd.label_index)) // check if we're not using goto to jump to command after us
                        {
                            ...
                            Call &call = *(Call *)(&temp[pos]);
                            call.params = 2;
                            call.func = ((cmd.type == CMD_GOTO_COND) ? GetGotoCondCall(cond.symbol) : GetGotoCondNCall(cond.symbol));
                            if (!call.func)
                                msgs.New().error("Unknown 'goto conditional' func", source, cmd.startTokenIndex());
                            call.param(0).setInstructI(cmd.label_index); // REMAP needed
    ...
    if (result_value && !has_return)
        msgs.New().error("Function does not return a value", &func);
}
...
void CodeEnvironment::FuncBody::create(Symbol &func, Memc<Message> &msgs, CompilerContext &ctx) {
    ...
    {
        Symbol::Modif value = func.params[i]->value;
        value.proceedToFinal(null);
        if (!value || value->type == Symbol::TYPENAME)
            msgs.New().error("Uknown parameter type", func.params[i]());
    ...
    if (Source *source = func.source) {
        Memc<Command> cmds, exp;
        source->parseFunc(func, cmds, msgs);
        ...
        compiler.expand(cmds, exp, -1, -1, label_return, label_body_scope);
        {
            Command &d = exp.New();
            d.type = CMD_LABEL;
            d.label_index = label_body_scope;
        } // place body scope label at the end of the codes (this deletes all variables)
        {
            Command &d = exp.New();
            d.type = CMD_LABEL;
            d.label_index = label_return;
        } // place return     label at the end of the codes
...
void CodeEnvironment::createGlobalVarSetup(Int func_index, Memc<Message> &msgs, CompilerContext &ctx) {
    if (InRange(func_index, func_bodies) && InRange(func_index + 1, func_bodies)) {
        Str temp;
            ...
            if (symbol.modifiers & Symbol::MODIF_DEF_VALUE) {
                for (Int i = symbol.def_val_range.x; i <= symbol.def_val_range.y;) {
                    ...
                    if (Symbol *s = GetFullSymbol(symbol.source->tokens, i, temp, symbol.parent(), templates)) {
                        if (!(s->helper & Symbol::HELPER_PROCESSED))
                            symbol.dependencies.include(s);
                    }
                    MAX(i, start + 1);
                }
            }
        ...
        REPA(vars) {
            ...
            REPA(symbol.dependencies) // check if any dependency got processed
            {
                ...
                if (dep.helper & Symbol::HELPER_PROCESSED)
                    symbol.dependencies.remove(i); // if dependency was processed then remove it
            }
        ...
        FREPA(sorted) // in forward order
        {
            ...
            if (Token *token = symbol.getToken()) {
                Expr var;
                CreateVar(var, *token, symbol, compiler, temp_init);
                if (var.symbol.hasDestructor())
                    dtors.add(&symbol); // upon creation add in same order to destructor list
            } else {
        ...
        init.del();
        init.name = "global var init";
        init.code = temp_init;
        init.stack_size = Ceil8(compiler.stack_size); // ALIGN, use Ceil8 to make sure that newer variables will have 8 byte alignment
        shut.del();
        shut.name = "global var shut";
        shut.code = temp_shut;
        shut.stack_size = Ceil8(compiler.stack_size);
    }
}
void CodeEnvironment::FuncBody::link(Memc<Message> &msgs, CodeEnvironment &ce) {
    ...
    for (Int pos = 0; pos < code.elms();) {
        Call &call = *(Call *)(&code[pos]);
        REPA(c)
        if (call.func == c[i]) // if any of the 'call' functions
        {
            if (Symbol *func = (Symbol *)call.param(0).getInstructP()) {
                if (InRange(func->raw_offset, ce.func_bodies))
                    call.param(0).addr = &ce.func_bodies[func->raw_offset]; // REMAP from Symbol* to FuncBody*
                else
                    msgs.New().error("Invalid func call 'func body'");
            } else
                msgs.New().error("Invalid func call 'func' pointer");
            break;
        }
...
void CodeEnvironment::FuncBody::optimize(Memc<Message> &msgs, CodeEnvironment &ce) {
    for (Int pos = 0; pos < code.elms();) {
        ...
        REP(call.params) {
            ...
            if (param.type == Call::ADDR_HEAP) {
                param.type = Call::ADDR_GLOBAL;
                param.addr = ce.heap.data() + param.offset;
            } else if (param.type == Call::ADDR_HEAP_REF) {
                param.type = Call::ADDR_GLOBAL_REF;
                param.addr = ce.heap.data() + param.offset;
            }
        }
        pos += Call::Size(call.params);
