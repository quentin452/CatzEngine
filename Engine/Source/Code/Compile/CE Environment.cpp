...
static void SetMemUInt(Ptr data, UInt value, UInt size) {
    ...
    v.u = value;
    UInt *u = (UInt *)data;
    REP(size / SIZE(UInt)) *u++ = v.u;
    Byte *b = (Byte *)u;
    REP(size % SIZE(UInt))
    b[i] = v.c[i & 3];
}
/******************************************************************************/
void CodeEnvironment::Thread::del() {
    stack.del();
    levels.del();
    zero();
}
void CodeEnvironment::Thread::create(CodeEnvironment &env) {
    ...
    debug = env.debug;
    if (debug)
        SetMemUInt(stack.data(), 0xBAADF00D, stack.elms()); // in debug mode fill stack with 0xBAADF00D
}
#if WINDOWS
...
Bool CodeEnvironment::Thread::start(FuncBody &func) {
    if (func.stack_size > stack.elms())
        Exit("Function requires too big stack size");
...
Bool CodeEnvironment::VerifySymbols(Memc<Message> &msgs, Memc<Symbol *> &sorted_classes) {
    ...
    FREPA(Symbols) {
        ...
        if (symbol.source && symbol.source->active && symbol.valid)
    ...
    FREPA(active_symbols) {
        ...
        if (symbol.isGlobalAndStatic())
            msgs.New().error("Invalid \"static\" keyword for global variable/function.", &symbol);
        ...
        if (symbol.type == Symbol::CLASS && (symbol.modifiers & Symbol::MODIF_TRANSPARENT)) {
            ...
            if (!p || p->type != Symbol::CLASS)
                msgs.New().error("Transparent class must belong to non-transparent class.", &symbol);
        }
    ...
    REPA(Symbols) {
        Symbol &symbol = Symbols.lockedData(i);
        symbol.dependencies.del(); // clear dependencies
    ...
    REPA(Symbols) {
        ...
        */
        {
            REPA(symbol.base)
            symbol.addDependency(symbol.base[i]); // add base classes
            if (symbol.type == Symbol::VAR && symbol.Parent() && symbol.Parent()->type == Symbol::CLASS)
                symbol.Parent()->addDependency(symbol.value); // add member types to their classes
            ...
            if (symbol.type == Symbol::VAR || symbol.type == Symbol::FUNC || symbol.type == Symbol::TYPEDEF) {
                if (symbol.Parent() && symbol.Parent()->type == Symbol::CLASS) {
                    ...
                    REPA(symbol.params)
                    if (Symbol *param = symbol.params[i]()) // iterate all params of a function
                    {
                        ...
                        if (param->modifiers & Symbol::MODIF_DEF_VALUE)         // include default value for function parameters to handle "class A{}; A global; void func(A p=global);"
                            for (Int i = param->def_val_range.x; i <= param->def_val_range.y;) {
                                ...
                                if (Symbol *s = GetFullSymbol(param->source->tokens, i, temp, &symbol, templates)) {
                                    Symbol::Modif modif;
                                    modif = s;
                                    symbol.Parent()->addDependency(modif);
                                }
                                MAX(i, start + 1);
                            }
                    }
    ...
    */
    ...
    REPA(Symbols) // get a list of all classes that have a base with some dependable templates
    {
        ...
        if (symbol.type == Symbol::CLASS)
            REPA(symbol.base) // check all bases
            {
                Symbol::Modif &base = symbol.base[i];
                REPA(base.templates) // check all templates
                {
                    Symbol::Modif &templat = base.templates[i];
                    if (templat.dependable()) // if at least one dependable
                    {
                        classes.add(&symbol);
                        goto added;
                    }
    ...
    for (;;) {
        ...
        REPA(classes) {
            ...
            REPA(symbol.base) // check all bases of Class : Base0, Base1
            {
                Symbol::Modif &base = symbol.base[i];
                REPA(base.templates) // check all templates for each base: Base<T0, T1>
                {
                    Symbol::Modif &templat = base.templates[i];
                    if (templat.dependable())                              // get 'T0'
        ...
        if (!processed)
            break; // if no class was processed in this step, then break the loop
    }
    classes.clear();
    ...
    REPA(active_symbols) {
        ...
        if (Symbol *symbol_parent = symbol.Parent())
            if (symbol_parent && symbol_parent->type == Symbol::CLASS) // element that belongs to class
            {
                ...
                if (Symbol::Modif &templat = value.templates[i]) // "Memc<int>" -> "int"
                {
                    if (templat->nested()) // nested symbols can't be forward declared
                    {
                        symbol_parent->addDependency(templat);                // add dependency to 'templat'
                    } else if (Symbol *src_template = templat.src_template()) // iterate all its templates "Memc.TYPE"
            ...
            if (Symbol *root_class = symbol.rootClass())
                if (root_class != &symbol)
                    REPA(symbol.dependencies) {
                        ...
                        if (dep != root_class)
                            root_class->dependencies.include(dep);
                    }
    }
    ...
    REPA(active_symbols) {
        Symbol &symbol = *active_symbols[i];
        if (symbol.type == Symbol::CLASS && symbol.isGlobal())
            classes.add(&symbol); // global classes only
    }
    ...
    FREPA(classes) // report all un-processed classes as errors
    {
        ...
        FREPA(symbol.dependencies) {
            ...
            if (!(dependency.helper & Symbol::HELPER_PROCESSED) && dependency.type != Symbol::TYPENAME)
                msg.children.New().set(dependency.fullCppName(), &dependency); // here ignore typename dependencies
        }
    }
    return !msgs.elms();
}
...
void CodeEnvironment::SetupChildren() {
    ...
    FREPA(Symbols)
    Symbols.lockedData(i).children.sort(CompareTokenIndex);
}
/******************************************************************************
...
void CodeEnvironment::del() {
    ...
    FREPA(CE.sources) // iterate all sources
    {
        ...
        if (source.active)
            REPA(source.symbols) // iterate all symbols from active source
            {
...
Bool CodeEnvironment::create(Memc<Message> &msgs) {
    ...
    if (VerifySymbols(msgs, sorted_classes)) {
        ...
        // copy constants
        REPA(ctx.constants) {
            CompilerContext::Constant &c = ctx.constants[i];
            Copy(heap.data() + c.heap_offset, ctx.const_data.data() + c.const_offset, c.size);
        }
        REPA(ctx.var_constants) {
            CompilerContext::Constant &c = ctx.var_constants[i];
            Copy(heap.data() + c.heap_offset, ctx.const_data.data() + c.const_offset, c.size);
        }
...
void CodeEditor::playEngineCompiler() {
    ...
    Memc<Message> msgs;
    CodeEnvironment CEnv;
    CEnv.debug = debug;
    Dbl compile_time = Time.curTime();
    Bool ok = (CE.verifyBuildFiles(msgs) && CEnv.create(msgs));
