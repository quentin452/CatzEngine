...
{
    if (src == dest && src != VAR_ENUM && src != VAR_NONE)
        return CAST_MAX;
    switch (dest) {
    case VAR_BOOL:
        if (src == VAR_BYTE || src == VAR_SBYTE || src == VAR_SHORT || src == VAR_USHORT || src == VAR_INT || src == VAR_UINT || src == VAR_LONG || src == VAR_ULONG || src == VAR_CHAR8 || src == VAR_CHAR16 || src == VAR_FLT || src == VAR_DBL || src == VAR_ENUM)
            return CAST_CONV;
        break;

    case VAR_BYTE:
        if (src == VAR_BOOL || src == VAR_CHAR8)
            return CAST_CONV;
        if (src == VAR_SBYTE || src == VAR_SHORT || src == VAR_USHORT || src == VAR_INT || src == VAR_UINT || src == VAR_LONG || src == VAR_ULONG || src == VAR_CHAR16 || src == VAR_FLT || src == VAR_DBL || src == VAR_ENUM)
            return CAST_CONV;
        break;
    case VAR_SBYTE:
        if (src == VAR_BOOL || src == VAR_CHAR8)
            return CAST_CONV;
        if (src == VAR_BYTE || src == VAR_SHORT || src == VAR_USHORT || src == VAR_INT || src == VAR_UINT || src == VAR_LONG || src == VAR_ULONG || src == VAR_CHAR16 || src == VAR_FLT || src == VAR_DBL || src == VAR_ENUM)
            return CAST_CONV;
        break;

    case VAR_SHORT:
        if (src == VAR_BOOL || src == VAR_BYTE || src == VAR_SBYTE || src == VAR_CHAR8 || src == VAR_CHAR16)
            return CAST_CONV;
        if (src == VAR_USHORT || src == VAR_INT || src == VAR_UINT || src == VAR_LONG || src == VAR_ULONG || src == VAR_FLT || src == VAR_DBL || src == VAR_ENUM)
            return CAST_CONV;
        break;
    case VAR_USHORT:
        if (src == VAR_BOOL || src == VAR_BYTE || src == VAR_CHAR8 || src == VAR_CHAR16)
            return CAST_CONV;
        if (src == VAR_SBYTE || src == VAR_SHORT || src == VAR_INT || src == VAR_UINT || src == VAR_LONG || src == VAR_ULONG || src == VAR_FLT || src == VAR_DBL || src == VAR_ENUM)
            return CAST_CONV;
        break;

    case VAR_INT:
        if (src == VAR_BOOL || src == VAR_BYTE || src == VAR_SBYTE || src == VAR_SHORT || src == VAR_USHORT || src == VAR_CHAR8 || src == VAR_CHAR16 || src == VAR_ENUM)
            return CAST_CONV;
        if (src == VAR_UINT || src == VAR_LONG || src == VAR_ULONG || src == VAR_FLT || src == VAR_DBL)
            return CAST_CONV;
        break;

    case VAR_UINT:
        if (src == VAR_BOOL || src == VAR_BYTE || src == VAR_USHORT || src == VAR_CHAR8 || src == VAR_CHAR16 || src == VAR_ENUM)
            return CAST_CONV;
        if (src == VAR_SBYTE || src == VAR_SHORT || src == VAR_INT || src == VAR_LONG || src == VAR_ULONG || src == VAR_FLT || src == VAR_DBL)
            return CAST_CONV;
        break;

    case VAR_LONG:
        if (src == VAR_BOOL || src == VAR_BYTE || src == VAR_SBYTE || src == VAR_SHORT || src == VAR_USHORT || src == VAR_INT || src == VAR_UINT || src == VAR_CHAR8 || src == VAR_CHAR16 || src == VAR_ENUM)
            return CAST_CONV;
        if (src == VAR_ULONG || src == VAR_FLT || src == VAR_DBL)
            return CAST_CONV;
        break;
    case VAR_ULONG:
        if (src == VAR_BOOL || src == VAR_BYTE || src == VAR_USHORT || src == VAR_UINT || src == VAR_CHAR8 || src == VAR_CHAR16 || src == VAR_ENUM)
            return CAST_CONV;
        if (src == VAR_SBYTE || src == VAR_SHORT || src == VAR_INT || src == VAR_LONG || src == VAR_FLT || src == VAR_DBL)
            return CAST_CONV;
        break;

    case VAR_CHAR8:
        if (src == VAR_BYTE || src == VAR_SBYTE)
            return CAST_CONV;
        if (src == VAR_SHORT || src == VAR_USHORT || src == VAR_INT || src == VAR_UINT || src == VAR_LONG || src == VAR_ULONG || src == VAR_CHAR16)
            return CAST_CONV;
        break;
    case VAR_CHAR16:
        if (src == VAR_CHAR8 || src == VAR_BYTE || src == VAR_SBYTE || src == VAR_SHORT || src == VAR_USHORT)
            return CAST_CONV;
        if (src == VAR_INT || src == VAR_UINT || src == VAR_LONG || src == VAR_ULONG)
            return CAST_CONV;
        break;

    case VAR_FLT:
        if (src == VAR_BOOL || src == VAR_BYTE || src == VAR_SBYTE || src == VAR_SHORT || src == VAR_USHORT)
            return CAST_CONV;
        if (src == VAR_INT || src == VAR_UINT || src == VAR_LONG || src == VAR_ULONG || src == VAR_DBL)
            return CAST_CONV;
        break;
    case VAR_DBL:
        if (src == VAR_BOOL || src == VAR_BYTE || src == VAR_SBYTE || src == VAR_SHORT || src == VAR_USHORT || src == VAR_INT || src == VAR_UINT || src == VAR_FLT)
            return CAST_CONV;
        if (src == VAR_LONG || src == VAR_ULONG)
            return CAST_CONV;
        break;
    }
    return CAST_NONE;
...
static CAST_MATCH CastMatchConst(CAST_MATCH cast) {
    ...
    default:
        return cast;
    case CAST_CTOR:
        return CAST_CTOR_CONST;
    case CAST_CONV:
        return CAST_CONV_CONST;
    case CAST_MAX:
        return CAST_MAX_CONST;
    }
}
/******************************************************************************/
// EXPRESSION
/******************************************************************************/
static void ConvertPtrToUIntPtr(Symbol::Modif &symbol) {
    symbol.clear(true);
    symbol = TypeSymbol((PtrSize == 4) ? VAR_UINT : VAR_ULONG);
} // treat pointers as unsigned's - this is needed for comparing pointers

void Expr::calculateKnown(Expr &op, Expr &a, Compiler &compiler) {
    if (known() && symbol) {
        if (op == '!')
            setBorrowed(a.asBool(compiler) ? u"false" : u"true");
        else if (op == '~')
            switch (symbol->var_type) {
            case VAR_INT:
                setCustom(S + ~a.asInt(compiler));
                break;
            case VAR_UINT:
                setCustom(S + ~a.asUInt(compiler));
                break;
            case VAR_LONG:
                setCustom(S + ~a.asLong(compiler));
                break;
            case VAR_ULONG:
                setCustom(S + ~a.asULong(compiler));
                break;
            }
        else if (op == '-')
            switch (symbol->var_type) {
            ...
            case VAR_LONG:
                ...
                break;
            case VAR_FLT:
                setCustom(S + -a.asDbl(compiler));
                break; // use Dbl to increase precision
            case VAR_DBL:
                setCustom(S + -a.asDbl(compiler));
                break;
            }
...
CAST_MATCH Expr::calculate(Compiler &compiler) {
    if (func.elms() == 1) {
        ...
        if (func == "sizeof") {
            setBasic(VAR_INT);
            if (compiler.strict && func.params.elms() != 1) {
                compiler.msgs.New().error(S + "Invalid number of parameters to '" + func + '\'', origin);
                return CAST_NONE;
            }
            if (func.params.elms() >= 1) {
                Expr &a = func.params[0];
                mem.setKnown();
                setCustom(S + a.symbol.rawSize(false));
            }
            return CAST_MAX;
        } else if (func == "typeid") {
            setPtr();
            return CAST_MAX;
        } else if (func == "dynamic_cast" || func == "static_cast" || func == "const_cast" || func == "reinterpret_cast") {
            if (compiler.strict) {
                if (func.symbol.templates.elms() != 1) {
                    compiler.msgs.New().error(S + "Invalid number of template parameters to '" + func + '\'', origin);
                    return CAST_NONE;
                }
                if (func.params.elms() != 1) {
                    compiler.msgs.New().error(S + "Invalid number of parameters to '" + func + '\'', origin);
                ...
                if (!dest) {
                    compiler.msgs.New().error(S + "Unknown template parameter", origin);
                    return CAST_NONE;
                }
                if (func == "dynamic_cast") {
                    if (dest.ptr_level == 1 && !(dest.modifiers & Symbol::MODIF_REF) && !dest.array_dims.elms() // C++ allows 'dynamic_cast' only when target is TYPE* or TYPE&
                        ...
                        if (src.symbol.ptr_level == dest.ptr_level && !src.symbol.array_dims.elms()) {
                            temporary = (dest.ptr_level ? true : src.temporary);
                            symbol = dest;
                            if (dest.ptr_level)
                                symbol.setConst();
                            return CAST_MAX; // yes it is a temporary - C++ compiling failed on "struct A{virtual ~A(){}}; struct B:A{virtual ~B(){}}; A *a=null; &CAST(B,a);"
                        }
                        compiler.msgs.New().error(S + "Can't perform 'dynamic_cast' from: " + src.symbol.modifName(true, true, false) + ", to: " + dest.modifName(true, true, false), origin);
                        return CAST_NONE;
                    }
                    compiler.msgs.New().error(S + "Target for 'dynamic_cast' must be a pointer or a reference", origin);
                    return CAST_NONE;
                } else {
                    temporary = true;
                    symbol = dest;
        ...
        } else if (func._operator && func != "()") // there is no built-in "operator()"
        {
            if (func.params.elms() == 1) // unary [] *x, &x, !x, ~x, +x, -x, ++x, --x, x++, x--
            {
                if (func == "[]") // parent[a], C++ supports only non-static [] operators
                {
                    ...
                    if (func.parent.elms() != 1) {
                        compiler.msgs.New().error("Invalid object for the [] operator", origin);
                        return CAST_NONE;
                    }
                    if (compiler.strict)
                        if (!a.symbol.isObj() || !a.symbol || !OffsetType(a.symbol->var_type)) {
                            compiler.msgs.New().error("Invalid index for the [] operator", a.origin);
                            return CAST_NONE;
                        }
                    Expr &parent = func.parent[0];

                    if (!parent.symbol.isObj()) {
                        if (!parent.indirection(compiler))
                            return CAST_NONE; // do "TYPE a[]; *a;"
                        ...
                        if (a.known()) {
                            ...
                            if (!func.func_call) {
                                if (!compiler.quiet)
                                    compiler.msgs.New().error("No function/operator found able to process command", origin);
                                return CAST_NONE;
                            } // if didn't found the function then it means this call is impossible
                            ...
                            Expr &raw_size = func.params.New();
                            raw_size.setBasic(VAR_INT);
                            raw_size.mem.setKnown();
                            raw_size.setCustom(S + parent.symbol.rawSize(false));
                        }
                        return CAST_MAX;
                    } else if (parent.basicType()) {
                        compiler.msgs.New().error("Invalid object for the [] operator", origin);
                        return CAST_NONE;
                    }
                } else if (func.params[0].basicType()) {
                indirection_address_of:
                    Expr &a = func.params[0];
                    if (func == '*') // verified Obj, Ptr, Array
                    {
                        if (a.symbol.isObj() && compiler.strict) {
                            compiler.msgs.New().error("Invalid * operator", origin);
                            return CAST_NONE;
                        } // in C++ "int a[2]; *a;" is correct!
                        temporary = false;
                        symbol = a.symbol;
                        symbol.setConst(false); // remove const from pointer, 'temporary' IS false because this "int a[1]; &(*a);" compiles on C++
                        if (a.symbol.isArray()) {
                            if (symbol.array_dims.elms())
                                symbol.array_dims.removeLast();
                            else if (symbol.ptr_level > 0)
                                symbol.ptr_level--;
                        ...
                        } else // use REF for arrays too ? (consider REF array and non-REF array)
                        {
                            ...
                            } else // we have direct access to the param Ptr, so reuse it
                            {
                                memFrom(a.mem, compiler);
                                if (parent.elms() != 0) {
                                    compiler.msgs.New().error("Parent should be empty in indirection", origin);
                                    return CAST_NONE;
                                }
                        ...
                        return CAST_MAX;
                    } else if (func == '&') // verified Obj, Ptr, Array
                    {
                        if (a.temporary && a.basicType() && compiler.strict) {
                            compiler.msgs.New().error("Can't take address of a temporary", origin);
                            return CAST_NONE;
                        }
                        if (a.func.elms()) // "&(*x)" -> "x"
                        {
                        ...
                        temporary = true;
                        symbol = a.symbol;                                // make the address a 'temporary'
                        FlagDisable(symbol.modifiers, Symbol::MODIF_REF); // clear any reference modifier
                        if (symbol.array_dims.elms())
                            symbol.array_dims.add(Symbol::DIM_PTR);
                        else
                            symbol.ptr_level++;
                    ...
                    {
                        setBasic(VAR_BOOL);
                        memNew(a.known(), compiler);
                        calculateKnown(func, a, compiler);
                        Symbol::Modif src = a.symbol;
                        if (src.isPtr())
                            ConvertPtrToUIntPtr(src); // treat ptr as int
                        if (src.isObj() && src)
                            src = TypeSymbol(SameSizeType(src->var_type));
                        func.func_call_mask = FC_RESULT;
                        func.func_call = FindFuncCall(NativeOperatorName(func, symbol, src));
                        return CAST_MAX;
                    } else if (func == '~') // verified Obj, Ptr, Array
                    {
                        setBasic(ComplementResult(a.symbol ? a.symbol->varType() : VAR_NONE));
                        memNew(a.known(), compiler);
                        calculateKnown(func, a, compiler);
                        Symbol::Modif src = a.symbol;
                        if (src.isObj() && src)
                            src = TypeSymbol(SameSizeSignType(src->var_type));
                        func.func_call_mask = FC_RESULT;
                        func.func_call = FindFuncCall(NativeOperatorName(func, symbol, src));
                        return CAST_MAX;
                    } else if (func == '+') // verified Obj, Ptr, Array
                    ...
                    {
                        setBasic(NegativeResult(a.symbol ? a.symbol->varType() : VAR_NONE));
                        memNew(a.known(), compiler);
                        calculateKnown(func, a, compiler);
                        Symbol::Modif src = a.symbol;
                        if (src.isObj() && src)
                        ...
                        func.func_call = FindFuncCall(NativeOperatorName(func, symbol, src));
                        return CAST_MAX;
                    } else if (func == "++" || func == "--") // verified Obj, Ptr, Array
                    {
                        if (a.symbol.isConst() && compiler.strict) {
                            compiler.msgs.New().error("Can't modify a const value", origin);
                            return CAST_NONE;
                        }
                        if (func.l_to_r) // x++, x--
                        {
                            ...
                            symbol.setConst();
                            if (compiler.strict) {
                                compiler.msgs.New().error("This operator is not yet supported, try using ++x or --x", origin);
                                return CAST_NONE;
                            }
                        } else // ++x, --x
                        {
                            ...
                            symbol = a.symbol;
                            memFrom(a.mem, compiler); // use the same memory layout as 'a' because these operators do not return any other address
                            Symbol::Modif src = a.symbol;
                            func.func_call_mask = 0;
                            if (src.isPtr()) {
                                if (src.array_dims.elms())
                                    src.array_dims.removeLast();
                                else
                                    src.ptr_level--;           // remove the ptr
                                Int size = src.rawSize(false); // get the size of under the pointer "byte *x" would be 1, while "byte **x" would be PtrSize
                                if (size <= 0 && compiler.strict) {
                                    compiler.msgs.New().error("Unknown pointer size", origin);
                                    return CAST_NONE;
                                }
                                if (size > 0)
                                    switch (size) {
                                    case 1:
                                        ConvertPtrToUIntPtr(src);
                                        goto inc_dec_1;

                                    case 2:
                                    case 4:
                                    case 8:
                                        func.func_call = FindFuncCall(S + ((func == "++") ? "inc ptr " : "dec ptr ") + size);
                                        break;

                                    default: {
                                        Expr &by = func.params.New();
                                        by.origin = origin;
                                        by.setBasic(VAR_INT);
                                        by.mem.setKnown();
                                        by.setCustom(S + size);
                        ...
                        compiler.msgs.New().error("Invalid unary operator", origin);
                        return CAST_NONE;
                    }
                }
            } else if (func.params.elms() == 2) // binary * / % + - << >> < > <= >= == != === !!= & ^ | && ^^ || = *= /= %= += -= <<= >>= &= |= ^= ,
            {
                Expr &a = func.params[0], &b = func.params[1];

                // cast to conditionals
                if (func == "&&" || func == "||" || func == "^^") {
                    if (!a.castToConditional() && compiler.strict) {
                        compiler.msgs.New().error("Can't cast to bool", a.origin);
                        return CAST_NONE;
                    }
                    if (!b.castToConditional() && compiler.strict) {
                        compiler.msgs.New().error("Can't cast to bool", b.origin);
                        return CAST_NONE;
                    }
                }
                ...
                if (func == ',') // a, b
                {
                    ...
                    symbol = b.symbol;
                    memFrom(b.mem, compiler);
                    set(b);
                    return CAST_MAX;
                } else if (a.basicType() && b.basicType()) {
                    if (func == "==" || func == "===" || func == "!=" || func == "!!=" || func == '<' || func == '>' || func == "<=" || func == ">=" || func == "&&" || func == "^^" || func == "||") // verified Obj, Ptr, Array
                    {
                        ...
                        if (func == "&&" || func == "^^" || func == "||") {
                            if (pa.isObj() && pa)
                                pa = TypeSymbol(SameSizeType(pa->var_type));
                            if (pb.isObj() && pb)
                                pb = TypeSymbol(SameSizeType(pb->var_type));
                        } else {
                            if (pa.isObj() && pa)
                                pa = TypeSymbol(SameSizeSignType(pa->var_type));
                            if (pb.isObj() && pb)
                                pb = TypeSymbol(SameSizeSignType(pb->var_type));
                        }

                        func.func_call_mask = FC_RESULT;
                        func.func_call = FindFuncCall(NativeOperatorName(func, symbol, pa, pb));
                        return CAST_MAX;
                    } else if (func == '=' || func == "*=" || func == "/=" || func == "%=" || func == "+=" || func == "-=" || func == "<<=" || func == ">>=" || func == "&=" || func == "|=" || func == "^=") // verified Obj, Ptr, Array
                    {
                        if (a.symbol.isConst() && compiler.strict) {
                            if (!compiler.quiet)
                                compiler.msgs.New().error("Can't modify a const value", origin);
                        ...
                        {
                            if (!b.addressOf(compiler))
                                return CAST_NONE;
                            if (b.symbol.array_dims.elms() < 2)
                                return CAST_NONE;
                            ...
                            if (b.symbol.array_dims.elms() != 2)
                                b.symbol.array_dims.remove(b.symbol.array_dims.elms() - 2, true); // remove element before last
                            ...
                            b.symbol.const_level = start;
                            b.symbol.setConst(end); // set new const levels
                            array_to_ptr = true;
                        }
                        ...
                        temporary = a.temporary;
                        symbol = a.symbol;
                        if (a.known() && b.known() && a.basicType() && func == '=')
                            mem.setKnown();
                        else
                            memFrom(a.mem, compiler); // allow setting known only if both 'a' and 'b' are known (this is used in casting 'CastTo' when converting constants, however do not set known if 'a' is not a known constant, because it would cause skipping of setting the value in the compiler) otherwise use the same memory layout as 'a' because these operators do not return any other address
                        calculateKnown(func, a, b, compiler);
                        Symbol::Modif result, pa = a.symbol, pb = b.symbol;
                        result = TypeSymbol(VAR_VOID);
                        if (pa.isPtr()) {
                            if (func == "+=" || func == "-=") {
                                if (compiler.strict)
                                    if (!pb.isObj() || !pb || !OffsetType(pb->var_type)) {
                                        compiler.msgs.New().error(S + "Invalid offset for the " + func + " operator", b.origin);
                                        return CAST_NONE;
                                    }
                                if (pa.array_dims.elms())
                                    pa.array_dims.removeLast();
                                else
                                    pa.ptr_level--;           // remove the ptr
                                ...
                                {
                                    ConvertPtrToUIntPtr(pa);
                                } else if (b.known()) // "TYPE *p; p+=const;" (can use integer += -= by "const*SIZE(TYPE)")
                                {
                                    ConvertPtrToUIntPtr(pa);
                                    Long total = b.asLong(compiler) * size;
                                    b.setBasic((Int(total) != total) ? VAR_LONG : VAR_INT);
                                    b.setCustom(S + total);
                                } else {
                                    func.func_call_mask = 0;
                                    ...
                                    raw_size.setBasic(VAR_INT);
                                    raw_size.mem.setKnown();
                                    raw_size.setCustom(S + ((func == "+=") ? size : -size));
                                    return CAST_MAX;
                                }
                            } else if (func == '=' && pb.isPtr()) {
                                ...
                                if (a_is_void_ptr || same || pb == "null") {
                                    if (pb != "null" && compiler.strict) {
                                        ...
                                        if (a_is_void_ptr)
                                            pb.const_level = (pb.const_level != 0); // 'a' can be Ptr or CPtr, in that case set const_level to 'b' only to 1st bit
                                        if (LostConst(pb.const_level, pa.const_level) && compiler.strict) {
                                            if (compiler.quiet)
                                                return CAST_NONE;
                                            compiler.msgs.New().error("Lost const", origin);
                                        }
                                    }
                                    ConvertPtrToUIntPtr(pa);
                                    ConvertPtrToUIntPtr(pb);
                                    if (!same)
                                        cast = CAST_CONV;                              // if the pointers are not the same, but we still can convert them, then set conversion priority (otherwise this could be CAST_NONE through 'CastMatch')
                                    ...
                                    if (compiler.strict) {
                                        if (!compiler.quiet)
                                            compiler.msgs.New().error("Can't set pointer to a different type", origin);
                                        return CAST_NONE;
                                    }
                                }
                            }
                        ...
                        if (pa.isObj() && pa)
                            pa = TypeSymbol(SameSizeSignBoolType(pa->var_type));
                        if (pb.isObj() && pb)
                            pb = TypeSymbol((pa && pa->var_type == VAR_BOOL && func == '=') ? SameSizeSignBoolType(pb->var_type) : SameSizeSignType(pb->var_type));
                        func.func_call_mask = 0;
                        ...
                        return cast;
                    } else if (func == '*' || func == '/' || func == '%' || func == '+' || func == '-' || func == "<<" || func == ">>" || func == '&' || func == '^' || func == '|') // verified Obj, Ptr, Array
                    {
                        if (a.symbol.isPtr()) {
                            if (func == '-' && b.symbol.isPtr()) // ptr-ptr (pointer difference)
                            {
                                if (!a.symbol.same(b.symbol, false, false) && compiler.strict) {
                                    compiler.msgs.New().error("Can't perform pointer difference on different type of pointers", origin);
                                    return CAST_NONE;
                                }
                                Symbol::Modif pa = a.symbol;
                                if (pa.array_dims.elms())
                                    pa.array_dims.removeLast();
                                else if (pa.ptr_level > 0)
                                    pa.ptr_level--;
                                Int size = pa.rawSize(false);
                                if (size <= 0 && compiler.strict) {
                                    compiler.msgs.New().error("Can't perform pointer difference on zero sized pointers", origin);
                                    return CAST_NONE;
                                }
                                ...
                                raw_size.setBasic(VAR_INT);
                                raw_size.mem.setKnown();
                                raw_size.setCustom(S + size);
                                return CAST_MAX;
                            } else if (func == '+' || func == '-') // ptr+int, ptr-int (pointer offset)
                            {
                                if (compiler.strict)
                                    if (!b.symbol.isObj() || !b.symbol || !OffsetType(b.symbol->var_type)) {
                                        compiler.msgs.New().error(S + "Invalid offset for the " + func + " operator", b.origin);
                                        return CAST_NONE;
                                    }
                                Symbol::Modif pa = a.symbol;
                        ...
                        } else if (a.symbol && a.symbol.isObj() && b.symbol && b.symbol.isObj()) {
                            Symbol::Modif pa, pb;
                            pa = TypeSymbol(SameSizeSignType(a.symbol->var_type));
                            pb = TypeSymbol(SameSizeSignType(b.symbol->var_type));
                            VAR_TYPE type = VAR_NONE;
                            if (func == '*')
                                type = MulResult(pa->var_type, pb->var_type);
                            else if (func == '/')
                                type = DivResult(pa->var_type, pb->var_type);
                            else if (func == '%')
                                type = ModResult(pa->var_type, pb->var_type);
                            else if (func == '+')
                                type = AddResult(pa->var_type, pb->var_type);
                            else if (func == '-')
                                type = SubResult(pa->var_type, pb->var_type);
                            else if (func == "<<")
                                type = ShiftLeftResult(pa->var_type, pb->var_type);
                            else if (func == ">>")
                                type = ShiftRightResult(pa->var_type, pb->var_type);
                            else if (func == '&' || func == '^' || func == '|')
                                type = BitResult(pa->var_type, pb->var_type);
                            setBasic(type);
                            memNew(a.known() && b.known(), compiler);
                            calculateKnown(func, a, b, compiler);
                            func.func_call_mask = FC_RESULT;
                        ...
                        compiler.msgs.New().error("Invalid binary operator", origin);
                        return CAST_NONE;
                    }
                }
            } else if (func.params.elms() == 3) // ternary
            {
                ...
                if (func == '?') // a ? b : c
                {
                    ...
                    compiler.msgs.New().error("Invalid ternary operator", origin);
                    return CAST_NONE;
                }
            } else {
                compiler.msgs.New().error("Invalid parameter number for operator", origin);
                return CAST_NONE;
            }
        }
        ...
        if (func.symbol && (func.symbol->modifiers & Symbol::MODIF_DATA_TYPE)) // constructor/cast
        {
            if (!func.instance) {
                ...
                {
                    symbol = func.symbol;
                    if (func.params.elms() != 1 && compiler.strict) {
                        compiler.msgs.New().error(S + "Invalid number of parameters for cast to " + func, origin);
                        return CAST_NONE;
                    }
                    if (func.params.elms() >= 1) {
                        Expr &a = func.params[0];
                        if (a.known() && compiler.strict) {
                            compiler.msgs.New().error(S + "Can't perform cast on a known constant", origin);
                            return CAST_NONE;
                        }
                        temporary = a.temporary;
                        memFrom(a.mem, compiler);
                        FlagSet(symbol.modifiers, Symbol::MODIF_REF, FlagOn(a.symbol.modifiers, Symbol::MODIF_REF)); // discard reference from the func, and keep it from the original 'a' parameter
                    }
                    return CAST_MAX;
                } else if (func.basicType()) // cast
                {
                    if (func.params.elms() != 1 && compiler.strict) {
                        compiler.msgs.New().error(S + "Invalid number of parameters for cast to " + func, origin);
                        return CAST_NONE;
                    ...
                    symbol.setConst();
                    return CAST_MAX;
                } else if (func.symbol->type == Symbol::CLASS) // class constructor
                {
        ...
        } else if (func.symbol && func.symbol->type == Symbol::FUNC_LIST) // function (global func, class static method, class non-static method)
        {
            AddMatches(matches, func.symbol->funcs, func.l_to_r, func.params.data(), func.params.elms(), func.symbol.templates, compiler);
        } else if (func.symbol && func.symbol->type == Symbol::FUNC) // if a ready function was already provided (global func, class static method, class non-static method)
        {
            AddMatch(matches, func.symbol(), func.l_to_r, func.params.data(), func.params.elms(), func.symbol.templates, compiler);
        } else if (func._operator) // custom operator ("parent[param]", "parent(param)" - both can be class non-static method only) (global "operator(obj, param)", or class non-static "Class::operator(param)")
        {
            if (func == "()" || func == "[]") // "parent[param]", "parent(param)"
            {
                if (func.parent.elms() != 1) {
                    compiler.msgs.New().error(S + "Invalid object for the " + func + " operator", origin);
                    return CAST_NONE;
                }
                Expr &parent = func.parent[0];
                if (!parent.instance || !parent.symbol || parent.symbol->type != Symbol::CLASS) {
                    compiler.msgs.New().error(S + "Invalid object for the " + func + " operator", origin);
                    return CAST_NONE;
                }

                Str func_name = SymbolToPath(S + "operator" + func);
                if (Symbol *op = FindChild(func_name, parent.symbol())())
                    if (op->type == Symbol::FUNC_LIST)
                        AddMatches(matches, op->funcs, func.l_to_r, func.params.data(), func.params.elms(), func.symbol.templates, compiler); // add class non-static operators "parent[params]", "parent(params)"
            } else if (func.params.elms() >= 1)                                                                                               // global "operator(obj, param)" or class non-static "Class::operator(param)"
            {
                Str func_name = SymbolToPath(S + "operator" + func);
                ignore_first = true; // allow for potential ignoring of the first parameter
                if (Symbol *global_op = FindChild(func_name, null)())
                    if (global_op->type == Symbol::FUNC_LIST)
                        AddMatches(matches, global_op->funcs, func.l_to_r, func.params.data(), func.params.elms(), func.symbol.templates, compiler); // add global           operators "       op(obj, params)"
                if (func.params.elms() >= 1 && func.params[0].symbol && func.params[0].symbol.isObj())
                    if (Symbol *class_op = FindChild(func_name, func.params[0].symbol())())
                        if (class_op->type == Symbol::FUNC_LIST)
                            AddMatches(matches, class_op->funcs, func.l_to_r, func.params.data() + 1, func.params.elms() - 1, func.symbol.templates, compiler, true); // add class non-static operators "Class::op(     params)", add extra match, because 'this' already fits perfectly, and unlike matching line above it is not included in param list so it doesn't benefit from full match and we must add this manually

                if (!matches.elms() && func.params.elms() == 1) // if there are no matches
                {
                    // continue with built-in operators
                    if (func == '*' || func == '&')
                        goto indirection_address_of;
                }
            }
        } else {
            compiler.msgs.New().error("Invalid token", origin);
            return CAST_NONE;
        }
        ...
        if (matches.elms()) {
            ...
            REPA(matches) {
                Symbol *a = matches[i].func;
                REPD(j, i)
                if (a->sameFunc(*matches[j].func)) {
                    matches.remove((a->modifiers & Symbol::MODIF_FUNC_BODY) ? i : j, true);
                    break;
                }
            ...
            {
                if (!compiler.quiet)
                    compiler.msgs.New().error(S + "Invalid parameters for function call " + f->definition(), origin);
                return CAST_NONE;
            }
            ...
            if (ignore_first && f->isClassNonStaticFunc()) {
                if (func.parent.elms() != 0 || func.params.elms() <= 0) {
                    compiler.msgs.New().error(S + "Can't adjust class non-static operator", origin);
                    return CAST_NONE;
                }
                Swap(func.parent.New(), func.params[0]);
                func.params.remove(0, true); // move the 1st param as the parent and remove from params list
            }
            ...
            if (f->realParams() != func.params.elms() && compiler.strict) {
                compiler.msgs.New().error(S + "Invalid number of parameters for function call", origin);
                return CAST_NONE;
            }
            ...
            FREP(f->realParams())
            if (f->params[i]->value && f->params[i]->value->type == Symbol::TYPENAME && InRange(i, func.params)) {
            ...
            if (compiler.strict && !func.castParentToSymbolClass(compiler))
                return CAST_NONE; // do this after setting 'func.symbol_origin'
            ...
            if (!(symbol.modifiers & Symbol::MODIF_REF)) // for functions that don't return references
            {
                temporary = true;
                if (basicType())
                    symbol.setConst(); // this is C++ behavior (can't modify returned basic types)
            }
            if (allow_mem_reuse && func.mem.type)
                memFrom(func.mem, compiler);
            else
                memNew(false, compiler); // set memory layout for the result
            ...
            if (compiler.strict && (f->modifiers & Symbol::MODIF_CTOR)) {
                if (func.parent.elms() != 0) {
                    compiler.msgs.New().error("Constructor parent should be empty", origin);
                    return CAST_NONE;
                }
...
{
    if (!InRange(func_i, expr) || params < 0 || params >= expr.elms() - func_i)
        Exit("Func out of range");
    ...
    temp_func.params.setNum(params);
    FREP(params)
    Swap(temp_func.params[i], expr[func_i + 1 + i]); // move 'params' from 'expr' into 'temp.func.params'
...
Expr &Template(Memc<Expr> &expr, Int templ_i, Int params) {
    if (!InRange(templ_i, expr) || params < 0 || params >= expr.elms() - templ_i)
        Exit("Template out of range");
    ...
    FREP(params) {
        ...
        dest = param.symbol;
        if (templ.symbol) {
            if (templ.symbol->type == Symbol::FUNC_LIST)
    ...
    REP(params)
    expr.remove(templ_i + 1, true); // remove 'params' from 'expr' container !! after this call 'templ' can no longer be accessed !! (because memory addresses may changed)
...
Bool Expr::instanceChild() {
    if (parent.elms())
        return parent[0].firstInstance() != null;
    return false;
} // if any of parents is an instance then this expr is a child and not full object
Bool Expr::fullTemporary() { return temporary && !instanceChild(); }
Expr *Expr::firstInstance() {
    for (Expr *expr = this; expr; expr = expr->parent.data())
        if (expr->instance)
            return expr;
    return null;
}
...
Bool Expr::asBool(Compiler &compiler) {
    if (known()) {
        ...
        if (symbol.isObj() && symbol)
            ...
            case VAR_CHAR8:
            case VAR_CHAR16:
                return asChar(compiler) != 0;

            case VAR_FLT:
            case VAR_DBL:
                return TextDbl(asStr()) != 0;
...
Char CppChar(C Str &s, Int &i, Token *token, Compiler &compiler) {
    if (s[i] == '\\')
        switch (s[(++i)++]) {
        default:
            compiler.msgs.New().error(S + "Unrecognized '" + s[i - 1] + "' character escape sequence", token);
            return '\0';
        case 'n':
            return '\n';
        case 't':
...
Char Expr::asChar(Compiler &compiler) {
    if (known()) {
        ...
        if (symbol.isObj() && symbol)
            ...
            case VAR_CHAR16: {
                Str s = T;
                if ((s[0] == '8' || s[0] == 'u' || s[0] == 'U' || s[0] == 'L') && s[1] == '\'' && s.last() == '\'')
                    s.remove(0); // eat 8,u,U,L from 8"..", u"..", U"..", L".."
                if (s[0] == '\'' && s.last() == '\'')
                    s.removeLast().remove(0); // eat '' from '..'
                Int i = 0;
                return CppChar(s, i, origin, compiler);
            } break;
...
Str Expr::asText(Compiler &compiler) {
    ...
    for (Int i = 0; i < s.length();)
        out += CppChar(s, i, origin, compiler);
    out += '\0'; // append 0 because strings are always NUL terminated
    return out;
}
U64 Expr::asRaw(Compiler &compiler) {
    if (known() && is()) {
        // pointers
        if (symbol.isPtr())
            return asLong(compiler);

        // values
        if (symbol.isObj() && symbol)
            switch (symbol->var_type) {
            case VAR_BOOL:
                return asBool(compiler);
            ...
            case VAR_CHAR8:
                return (Byte)asChar8(compiler);
            case VAR_CHAR16:
                return (UShort)asChar(compiler);

            case VAR_FLT: {
                Flt f = asFlt(compiler);
                return (U32 &)f;
            }
            case VAR_DBL: {
                Dbl d = asDbl(compiler);
                return (U64 &)d;
            }
            }
    }
    compiler.msgs.New().error("Expression value is not known and can't be converted to raw memory");
    return 0;
}
/******************************************************************************/
...
Expr &Expr::toSymbol(Symbol *symbol, Bool func_call) // this function moves from current symbol (usually a parent) to 'symbol' (usually a child), keeping templates (templates are kept because in "<TYPE> class X {TYPE x;}" if we're moving from parent "X<int> obj; obj.x" then 'obj' (parent) has the template information and proceeding to 'x' symbol (child) we will still know its template and thus its type)
{
    ...
    if (!final)
        T.symbol.proceedToFinal(&T.symbol.templates, func_call); // for final we are interested only in the root 'symbol' (not its value) for example when we want to use jump-to-definition on function we want to calculate which FUNC is that

    if (T.symbol)
        AddBaseTemplates(T.symbol.templates, *T.symbol); // after getting the final value we need to add base templates for its class (new templates may override previous values)
    ...
    if (symbol_origin) // check 'T.symbol_origin' and not 'T.symbol', because for example 'T.symbol_origin' operates on root (like variable) while 'T.symbol' is already set to target type (such as type/class of variable)
    {
        if (symbol_origin->type == Symbol::VAR                                                 // variable, for example 'x' in "int x"
            ...
            || symbol_origin->type == Symbol::ENUM_ELM)
            T.instance = true; // enum element is also an instance (just like constant integer '1')

        if (symbol_origin->type == Symbol::ENUM_ELM) {
            T.temporary = true;
            T.symbol.const_level |= 1;
            mem.setKnown();
        }
    }
    return T;
...
#define UNARY_PRIORITY 17
        if (T == '.' || T == "->" || T == "::")
            priority = 19;
        else if (T == '(' || T == ')' || T == '[' || T == ']' || T == TMPL_B || T == TMPL_E || T == "++" || T == "--")
            priority = 18;
        else if (T == '~' || T == '!' /* || T=='*' || T=='&' || T=="+" || T=="-"*/) {
...
void Expr::memAuto(Compiler &compiler) {
    if (symbol_origin && symbol_origin->isVar()) {
        if (mem.type)
            compiler.msgs.New().error("Expression already had memory layout set", origin);

        if (symbol_origin->modifiers & Symbol::MODIF_FUNC_PARAM) // function parameter
        {
            mem.setParam(*symbol_origin);
        } else if (symbol_origin->func()) // symbol belongs to a function - it's a local variable in a function
        {
        ...
        } else if (symbol_origin->isGlobalOrStatic()) // global or class static variable
        {
...
void Expr::memNew(Bool known, Compiler &compiler) {
    ...
    if (known)
        ...
        mem.setLocal(compiler.newLocal(symbol, false, origin));
}
...
static void CastTo(Expr &expr, Symbol::Modif &dest, CAST_MATCH &max_cast, Expr &max_result, Compiler &compiler, RecTest &rt = NoTemp(RecTest())) // function which recursively updates the cast
{
    ...
    if (src && dest) {
        ...
        UInt src_const_level = src.const_level;
        FlagDisable(src_const_level, 1 << (src.ptr_level + src.array_dims.elms())); // disable top const
        UInt dest_const_level = dest.const_level;
        FlagDisable(dest_const_level, 1 << (dest.ptr_level + dest.array_dims.elms())); // disable top const
        ...
        if (dest.basicType()) // try operator = (this will automatically calculate known value)
        {
            ...
            func.setOperator(u"=");
            func.params.setNum(2); // allocate parameters up-front so New will not change address of references
            ...
            expr_dest.symbol.setConst(false);
            FlagDisable(expr_dest.symbol.modifiers, Symbol::MODIF_REF); // disable main const to enable setting the value, and disable reference because we're still setting to a temporary local variable (or known const) which then later will be passed to the reference
            ...
            if (src.const_level != dest.const_level)
                cast = CastMatchConst(cast); // adjust const if needed (do this here and not inside 'calculate' because there the 'expr_dest' has const disabled in order to allow setting the value)
            if (cast > max_cast) {
                max_cast = cast;
                Swap(max_result, result);
                if (max_cast >= CAST_MAX_CONST)
                    return;
    ...
    try_base:
        if (max_cast < CAST_MAX_CONST)
            REPA(src->base) {
                Expr base = expr;
                base.proceedToBase(i); // operate on temporary value so that 'expr' remains unmodified
                CastTo(base, dest, max_cast, max_result, compiler, rt);
                if (max_cast >= CAST_MAX_CONST)
                    return;
            }
    }
...
static Bool CastToBase(Expr &expr, Symbol::Modif &dest, Expr &out, RecTest &rt = NoTemp(RecTest())) {
    ...
    if (src)
        REPA(src->base) {
            ...
            Expr base = expr;
            base.proceedToBase(i); // operate on temporary value so that 'expr' remains unmodified
            if (CastToBase(base, dest, out, rt))
                return true;
        }
    return false;
}
...
{
    if (!instance)
        return false; // can't convert from non-instance
    if (!(dest.modifiers & Symbol::MODIF_REF) && dest.isPtr() && T == "null")
        return true; // no conversion needed (test this in case "operator=" may not set "null" value in the result)
...
CAST_MATCH Expr::canBeCastedTo(Symbol *dest, Memc<Symbol::Modif> &templates, Compiler &compiler) {
    ...
    final.templates = templates;
    final.proceedToFinal(&final.templates);
    if (!(final.modifiers & Symbol::MODIF_REF) && final.isPtr() && T == "null")
        return CAST_MAX;
    if (!compiler.strict && final && final->type == Symbol::TYPENAME)
        return CAST_CONV; // if dest is template then it can always be used
    CAST_MATCH cast = CAST_NONE;
    Expr result;
    CastTo(T, final, cast, result, compiler);
    return cast;
}
...
Bool Expr::castParentToSymbolClass(Compiler &compiler) {
    ...
    {
        if (symbol_origin->modifiers & Symbol::MODIF_CTOR) {
            parent.del();
            return true;
        } // constructors can be called without specifying the parent
        ...
        if (parent.elms() && !parent[0].instance) {
            Swap(parent[0], parent_path);
            parent.clear();                                   // move all parents to the temp
                ...
                if (Symbol *caller_func = origin->parent->func())
                    if (caller_func->isClassNonStaticFunc())
                        if (Symbol *caller_class = caller_func->Class()) {
                            ...
                            if (!parent_instance->setThis(caller_func, origin, compiler))
                                return false;
                        }

        // verify the obtained instance
        if (!parent_instance || !parent_instance->instance) {
            compiler.msgs.New().error(S + "Can't access class non-static member without specifying the class object", origin);
            return false;
        }
        if (parent_instance->symbol.array_dims.elms()) {
            compiler.msgs.New().error(S + "Can't access class non-static member from an array", origin);
            return false;
        }
        if (parent_instance->symbol.ptr_level > 1) {
            compiler.msgs.New().error(S + "Can't access class non-static member from deep pointer", origin);
            return false;
        }
        if (parent_instance->symbol.ptr_level > 0)
            if (!parent_instance->indirection(compiler))
                return false;

        // follow the parent path (this is needed in case of "Class::Base1::Str" and "Class::Base2::Str"
        Memc<Symbol::Modif *> parents;
        for (Expr *cur = &parent_path; cur; cur = cur->parent.data())
            if (cur->symbol && cur->symbol->type == Symbol::CLASS)
                parents.add(&cur->symbol); // parents are now listed in C++ reversed order "empty_instance::base::class::namespace"
        REPA(parents)
        parent_instance->castToBase(*parents[i]); // process in reversed order to process in normal C++ order (first namespace, then class, then bases)
        ...
        if (!symbol_origin->isMemberOf(parent_symbol, symbol.templates, origin ? origin->parent : null, true, false, allow_self, true)) {
            compiler.msgs.New().error(S + "Can't access class non-static member from specified object", origin);
            return false;
        }

        // cast parent to class made from "symbol_origin->Class" with templates extracted from "this.templates"
        Symbol::Modif cast;
        cast = symbol_origin->Class();
        cast.templates = symbol.templates;
        if (!parent_instance->castToBase(cast)) {
            compiler.msgs.New().error("Can't cast object from its class to class of the member", parent_instance->origin ? parent_instance->origin : origin);
            return false;
        }

        // setup memory layout for the member
        if (symbol_origin->isVar()) {
            if (!parent_instance || !parent_instance->symbol) {
                compiler.msgs.New().error(S + "Something went wrong during member object calculation", origin);
                return COMPILE_FAILED;
            }

            Bool found;
            Int member_offset = parent_instance->symbol->memberOffset(symbol_origin, &found, &symbol.templates);
            if (!found) {
                compiler.msgs.New().error(S + "Member not found in class", origin);
                return COMPILE_FAILED;
            }
...
void Expr::proceedToBase(Int base_index) // this function moves self to T.parent, and sets self as the child
{
    if (symbol && InRange(base_index, symbol->base)) {
        Symbol::Modif base = symbol->base[base_index];
        base.proceedToFinal(&symbol.templates);
        ...
        Expr child;
        copyThis(child);             // copy all params to child (except parent, func, params)
    ...
    if (caller)
        if (Symbol *func = caller->func())
            if (func->isClassNonStaticFunc()) {
                toSymbol(func->Class());
                instance = true;
                symbol.const_level = FlagOn(func->modifiers, Symbol::MODIF_FUNC_CONST); // if the function is const "void func()const" then mark the data as const
                mem.setThis();
            }
    if (!symbol)
        compiler.msgs.New().error(S + "'this' can be used only in non-static class methods", origin);
    return symbol != null;
}
void Expr::create(Token &src, Compiler &compiler, Bool final) {
    ...
    case TOKEN_TEXT8:
        ...
        temporary = false;
        symbol.const_level = 1 | 2;
        symbol.array_dims.add(Symbol::DIM_UNKNOWN);
        break; // convert "" text8 to 'char8[]' (include null-terminating character, make the characters and the array pointer itself both constants "1|2")
    case TOKEN_TEXT16:
        setBasic(VAR_CHAR16);
        temporary = false;
        symbol.const_level = 1 | 2;
        symbol.array_dims.add(Symbol::DIM_UNKNOWN);
        break; // convert "" text  to 'char []' (include null-terminating character, make the characters and the array pointer itself both constants "1|2")
    case TOKEN_NUMBER: {
        VAR_TYPE type = VAR_NONE;
        REPA(bs)
        if (bs[i] == '.') {
            type = ((bs.last() == 'd') ? VAR_DBL : VAR_FLT);
            break;
        }          // detect if real type
    ...
    default: {
        ...
        if (src == "this") {
            if (setThis(src.parent, &src, compiler))
                addressOf(compiler);
        } else if (src == "true") {
            setBasic(VAR_BOOL);
            mem.setKnown();
            ...
            setBasic(VAR_NULL);
            mem.setKnown();
            symbol.const_level = 2;
            symbol.ptr_level = 1;
        } else // make the pointer as constant (not the data to which it points to), always fit as KNOWN because 'null' is always zero
        {
...
void FuncMatch::create(Symbol *func, Expr *param, Int params, Memc<Symbol::Modif> &templates, Compiler &compiler, Bool extra_match) {
    ...
    FREP(Max(func->realParams(), params)) {
        if (InRange(i, func->realParams())) {
            ...
            } else if (func->params[i]->modifiers & Symbol::MODIF_DEF_VALUE) // custom param is not specified but there is default value in the function param
            {
                // don't modify
    ...
    if (func && func->valid) {
        if (Symbol *parent = func->parent()) // func FUNC has "29386481" name, so check the FUNC_LIST parent
        {
            if (*parent == "operator*" || *parent == "operator&" || *parent == "operator+" || *parent == "operator-") {
                if (func->params.elms() != params)
                    return; // above funcs/operators can be both unary/binary, so make additional check for the number of parameters (this is to avoid situations of for example "Rect rect; &rect" when wanting to get the address of the object using unary '&', but there is custom binary operator '&' of Rect&Rect format)
                ...
                // ++x |   0    |   0
                // --x |   0    |   0
                if (func->params.elms() != Int(l_to_r))
                    return;
            }
        }
...
void AddMatches(Memc<FuncMatch> &matches, Memc<SymbolPtr> &funcs, Bool l_to_r, Expr *param, Int params, Memc<Symbol::Modif> &templates, Compiler &compiler, Bool extra_match) {
    FREPA(funcs)
    AddMatch(matches, funcs[i](), l_to_r, param, params, templates, compiler, extra_match);
}
/******************************************************************************/
// COMPILER
/******************************************************************************
...
COMPILE_RESULT Compiler::compileExpr(Memc<Expr> &expr, Symbol *space, Expr &out) {
    IntLock lock(recursive);
    if (recursive > 16) {
        msgs.New().error("Compiler recursive loop", expr.elms() ? expr.first().origin : null);
        return COMPILE_FAILED;
    } // this can happen if calculating "int func(int par=func())" or "int f(int par=g()); int g(int par=f());"
    ...
    if (Class)
        AddBaseTemplates(class_base_templates, *Class);
    ...
    FREPA(expr) {
        ...
        if (!(prev && prev->separator)) // if not preceeded by separator (check only root symbols, sub-symbols will be checked in the '.' merging)
        {
            ...
            e.symbol.templates.NewAt(i) = class_base_templates[i]; // set only for expressions not preceeded by separator (later below needs to be called for ".ROOT" too), add in this order because 'class_base_templates' need to be placed at start, to be least important
            if (strict && e.symbol_origin && !e.symbol_origin->canBeAccessedFrom(space, space, false, ProjectUsings)) {
                msgs.New().error("Symbol can't be accessed from current space", e.origin);
                return COMPILE_FAILED;
            } // this needs to be checked here because later access is checked only on detecting of unknown yet symbols
            if (strict && !e.castParentToSymbolClass(T))
                return COMPILE_FAILED; // this will make sure that "member_var" have "this." parent set
        }
    }
    ...
    for (; expr.elms();) {
        ...
        FREPA(expr) {
            ...
            if (!e.symbol) // not yet detected
            {
                if (!e._operator) // starts with name
                {
                    ...
                    if (!(prev && prev->separator)) // if not preceeded by separator
                    {
                        if (Symbol *s = FindSymbol(temp = e, space, ProjectUsings)()) {
                            changed = true;
                            e.toSymbol(s);
                            if (e.knownFinal(allow_func_lists)) {
                                Swap(out, e);
                                return COMPILE_FULL;
                            }
                            if (strict && !e.symbol_origin->canBeAccessedFrom(space, space, false, ProjectUsings)) {
                                msgs.New().error("Symbol can't be accessed from current space", e.origin);
                                return COMPILE_FAILED;
                            }
                ...
                } else if (e.separator) // encountered separator (this can be symbol with parent "obj.member" or global namespace "::EE::SomeFunc", this can also be object with base class specification "obj.Base<Str>::func")
                {
                    Expr *prev = (InRange(i - 1, expr) ? &expr[i - 1] : null),
                    ...
                    if (!(prev && (*prev == ']' || *prev == TMPL_E || (!prev->_operator && !prev->symbol)))) // don't process if it's ). ]. >. ?. (? = unknown symbol), however allow ")." if it's a cast "(TYPE).EE.SomeFunc" - perform additional checks below
                        if (!(prev2 && prev2->separator))                                                    // if not preceeded by operator
                        {
                            if (prev && *prev == ')') // test if we're preceeded by cast "(TYPE).x"
                            {
                                ...
                                if (!(prev3 && *prev3 == '(' && prev2 && prev2->symbol && (prev2->symbol->modifiers & Symbol::MODIF_DATA_TYPE) && !prev2->instance))
                                    continue; // if it's not a cast then continue the loop
                            }

                            if (prev && prev->instance && prev->symbol && prev->symbol.isObj() && e == "->") // check for overloaded -> operator
                            {
                                // TODO:
                                if (Symbol *s = FindChild("operator->", prev->symbol(), &prev->symbol.templates)())
                                    if (s->type == Symbol::FUNC_LIST && s->funcs.elms()) {
                                        prev->final |= e.final;
                                        prev->toSymbol(s->funcs[0](), true);
                                        MAX(--prev->symbol.ptr_level, 0);
                                        if (prev->knownFinal(allow_func_lists)) {
                                            Swap(out, *prev);
                                            return COMPILE_FULL;
                                        }
                            ...
                            if (InRange(i + 1, expr)) {
                                ...
                                if (!s) {
                                    ...
                                    if (!s) // if not found
                                        if (prev_is_parent && prev->symbol->type == Symbol::CLASS && *prev->symbol == temp())
                                            s = prev->symbol();
                                        else // check "X::X"
                                ...
                                if (s) {
                                    if (prev_is_parent) {
                                        {
                                            FREPA(prev->symbol.templates)
                                            next.symbol.templates.NewAt(i) = prev->symbol.templates[i];
                                        }                                     // inherit all template informations from the parent (in correct order)
                                        Swap(next.parent.New(), expr[i - 1]); // store 'parent' into 'child' "parent.child"
                                    }
                                    next.final |= e.final;
                                    next.toSymbol(s);
                                    if (next.knownFinal(allow_func_lists)) {
                                        Swap(out, next);
                                        return COMPILE_FULL;
                                    }
                                    if (!prev_is_parent)
                                    ...
                                    if (strict) {
                                        if (next == "this") {
                                            msgs.New().error("Keyword can't be used as a child", next.origin);
                                            return COMPILE_FAILED;
                                        }
                                        if (!prev_is_parent && !next.symbol_origin->canBeAccessedFrom(null, space, true, ProjectUsings)) {
                                            msgs.New().error("Symbol can't be accessed from current space", next.origin);
                                            return COMPILE_FAILED;
                                        }
                                        if (!next.castParentToSymbolClass(T))
                                            return COMPILE_FAILED;
                                        if (!next.symbol_origin->isClassNonStaticMember() && !next.mem.type)
                                            next.memAuto(T); // global, class static, memAuto can be used
                                    }

                                    // remove expressions
                                    expr.remove(i, true); // remove "."
                                    if (prev_is_parent)
                                        expr.remove(i - 1, true); // remove "parent" / 'prev'
                                    changed = true;
                                }
                            }
                        }
                    ...
                    if (InRange(i - 1, expr)) {
                        ...
                        if (value.instance) {
                            ...
                            if (!(prev && prev->separator)) // if not preceeded by separator
                            {
                                ...
                                Expr &result = Func(expr, i - 1, 1, T);
                                if (result.knownFinal(allow_func_lists)) {
                                    Swap(out, result);
                                    return COMPILE_FULL;
                                }
                                changed = true;
                            ...
                            if (!prev_op || ((l_to_r ? prev_op->priority < priority : prev_op->priority <= priority) || *prev_op == '{' || *prev_op == '(' || *prev_op == '[' || *prev_op == TMPL_B || *prev_op == ')')) // allow special case of ')' which is checked later
                                if (!next_op || ((l_to_r ? next_op->priority <= priority : next_op->priority < priority) || *next_op == '}' || *next_op == ')' || *next_op == ']' || *next_op == TMPL_E)) {
                                    if (prev_op && *prev_op == ')') // test if we're preceeded by cast "(TYPE)&x"
                                    {
                                        ...
                                        if (!(prev_op2 && *prev_op2 == '(' && type && type->symbol && (type->symbol->modifiers & Symbol::MODIF_DATA_TYPE) && !type->instance))
                                            continue; // if it's not a cast then continue the loop
                                    }
                                    // adjust operator parameters
                                    op.l_to_r = l_to_r;
                                    op.priority = priority;
                                    Expr &result = Func(expr, i, 1, T);
                                    if (result.knownFinal(allow_func_lists)) {
                                        Swap(out, result);
                                        return COMPILE_FULL;
                                    }
        ...
        FREPA(expr) {
            Expr &op = expr[i];
            if (op._operator)
                if (op == '*' || op == '/' || op == '%' || op == '+' || op == '-' || op == "<<" || op == ">>" || op == '<' || op == '>' || op == "<=" || op == ">=" || op == "==" || op == "!=" || op == "===" || op == "!!=" || op == '&' || op == '^' || op == '|' || op == "&&" || op == "^^" || op == "||" || op == '=' || op == "*=" || op == "/=" || op == "%=" || op == "+=" || op == "-=" || op == "<<=" || op == ">>=" || op == "&=" || op == "|=" || op == "^=")
                    if (InRange(i - 1, expr) && InRange(i + 1, expr)) {
                        Expr &a = expr[i - 1],
                             &b = expr[i + 1],
                             *next_op = (InRange(i + 2, expr) ? &expr[i + 2] : null),
        ...
        FREPA(expr) {
            if (expr[i] == '(' && InRange(i + 2, expr) && expr[i + 2] == ')') {
                Expr *prev = (InRange(i - 1, expr) ? &expr[i - 1] : null);
        ...
        // [] operators, like x[i]
        FREPA(expr) {
            if (expr[i] == '[' && InRange(i + 2, expr) && expr[i + 2] == ']' && InRange(i - 1, expr)) {
                Expr &value = expr[i - 1];
                if (value.instance) {
                    Expr *prev = (InRange(i - 2, expr) ? &expr[i - 2] : null);
                    if (!(prev && prev->separator)) // if not preceeded by separator
            ...
            if (type.symbol && (type.symbol->modifiers & Symbol::MODIF_DATA_TYPE) && !type.instance) {
                ...
                if (!(prev && prev->separator)) // if not preceeded by separator
                {
                    if (InRange(i + 1, expr)) // check for "TYPE*", "TYPE&", "TYPE const"
                    {
                        Expr &modif = expr[i + 1];
                        if (modif == '&') {
                            type.symbol.modifiers |= Symbol::MODIF_REF;
                            expr.remove(i + 1, true);
                            changed = true;
                        } else if (modif == '*') {
                            type.symbol.ptr_level++;
                            expr.remove(i + 1, true);
                            changed = true;
                        } else if (modif == "const") {
                            type.symbol.const_level |= (1 << type.symbol.ptr_level);
                            expr.remove(i + 1, true);
                            changed = true;
                        }
                    }
                    if (prev && *prev == "const") // check for "const TYPE"
                    {
                        ...
                        {
                            type.symbol.const_level |= 1;
                            expr.remove(i - 1, true);
                            changed = true; // add const to TYPE, remove 'const' expr
                        }
                    }
                }
            }
                    ...
                    if (!(prev && prev->separator)) // if not preceeded by separator
                    {
                        for (Int j = i + 1, last_elm = -1; j < expr.elms(); j++) // last_elm: -1=start, 0=parameter, 1=comma(,)
                        {
                            ...
                            if (param == ',') // comma
                            {
                                ...
                                {
                                    if (strict) {
                                        msgs.New().error("Unexpected ','", param.origin);
                                        return COMPILE_FAILED;
                                    }
                                    ...
                                    last_elm = 1;       // set last element to comma
                            } else if (param == TMPL_E) // end
                            {
                                if (last_elm == 1) // if last element was comma
                                {
                                    if (strict) {
                                ...
                                templ.final |= expr[i].final;
                                expr.remove(i, true);
                                j--; // remove '<'
                                templ.final |= expr[j].final;
                                expr.remove(j, true);
                                j--; // remove '>'
                                for (Int k = j; k >= i; k--)
                                    if (expr[k] == ',') {
                                        templ.final |= expr[k].final;
                                        expr.remove(k, true);
                                        j--;
                                    } // remove ','

                                Int params = j - i + 1;
                                Expr &result = Template(expr, i - 1, params);
                                if (result.knownFinal(allow_func_lists)) {
                                    Swap(out, result);
                                    return COMPILE_FULL;
                                }
                                changed = true;
                                break;
                            } else if (!param._operator) // parameter
                            {
                                if (last_elm == 0)
                                    break;    // if last element was parameter then it means both parameters haven't been merged yet
                    ...
                    if (!(prev && prev->separator)) // if not preceeded by separator, needed because "D.mode(320,240)" could result into "D.D" if "mode(320,240)" would be computed before "D.mode"
                    {
                        for (Int j = i + 1, last_elm = -1; j < expr.elms(); j++) // last_elm: -1=start, 0=parameter, 1=comma(,)
                        {
                            ...
                            } else if (param == ')') // end
                            {
                                ...
                                func.final |= expr[i].final;
                                expr.remove(i, true);
                                j--; // remove '('
                                func.final |= expr[j].final;
                                expr.remove(j, true);
                                j--; // remove ')'
                                for (Int k = j; k >= i; k--)
                                    if (expr[k] == ',') {
                                        func.final |= expr[k].final;
                                        expr.remove(k, true);
                                        j--;
                                    } // remove ','
                                ...
                                changed = true;
                                break;
                            } else if (param.instance || (size_of && param.symbol && (param.symbol->modifiers & Symbol::MODIF_DATA_TYPE))) // parameter (must be instance, however sizeof can accept data types as well)
                            {
                                if (last_elm == 0)
                                    break;    // if last element was parameter then it means both parameters haven't been merged yet
                                last_elm = 0; // set last element to parameter
                                ...
                                if (!prev_op || ((op.l_to_r ? prev_op->priority < op.priority : prev_op->priority <= op.priority) || *prev_op == '{' || *prev_op == '(' || *prev_op == '[' /*|| *prev_op==TMPL_B*/)) // don't allow inside templates "<a,b>"
                                    if (!next_op || ((op.l_to_r ? next_op->priority <= op.priority : next_op->priority < op.priority) || *next_op == '}' || *next_op == ')' || *next_op == ']' /*|| *next_op==TMPL_E*/)) {
                                        if (prev_op && *prev_op == '(' && InRange(i - 3, expr)) {
                                            Expr &func = expr[i - 3];
                                            if (func.possibleInstanceOrDataTypeOrNativeFunc() || func == ')' || func == ']' || func == TMPL_E)
                                                continue; // disallow "func(a,b", "x()(a,b", "x[i](a,b", "func<t>(a,b"
                                        }
    ...
    {
        if (expr.elms() >= 2)
            expr[expr.elms() - 2].final |= expr.last().final; // merge final from last separator to expr before
...
Bool Compiler::appendTokens(Memc<Expr> &expr, Int from, Int to, Memc<Token *> *tokens) {
    Memc<Token *> &t = (tokens ? *tokens : T.tokens);
    if (InRange(from, t) && InRange(to, t)) {
        for (Int i = from; i <= to; i++)
            expr.New().create(*t[i], T, i == final);
        return true;
    }
    return false;
}
