...
namespace Edit {
/******************************************************************************/
Bool SaveChanges::Elm::doSaveClose() {
    if (_save)
        if (!_save(T))
            return false;
    doClose();
    return true;
} // if failed to save then don't close
void SaveChanges::Elm::doClose() {
    if (_close)
        _close(T);
}
/******************************************************************************/
void SaveChanges::create() {
    Gui += super::create("Save changes to following elements?").hide();
    button[2].show();
    flag |= WIN_RESIZABLE;
    T += region.create();
    T += save.create("Yes").func(Save, T);
    save.kbSet();
...
void SaveChanges::set(C CMemPtr<Elm> &elms, void (*after_save_close)(Bool all_saved, Ptr user), Ptr user) {
    ...
    if (elms.elms())
        ...
        if (_after_save_close)
            _after_save_close(true, _user);
        hide();
    }
}
/******************************************************************************/
void SaveChanges::clear() {
    _after_save_close = null;
    list.clear();
    elms.del();
    hide();
}
void SaveChanges::doSave() {
    Bool all_saved = true;
    FREPA(elms)
    all_saved &= elms[i].doSaveClose();
    if (_after_save_close)
        _after_save_close(all_saved, _user);
    clear();
} // save  in specified order
void SaveChanges::doClose() {
    FREPAO(elms).doClose();
    if (_after_save_close)
        _after_save_close(true, _user);
    clear();
