...
void _Undo::clean(Bool test_mem_usage) {
    ...
    Long mem_usage;
    Memt<UInt> mem_usages;
    if (test_mem_usage) {
        mem_usage = 0;
        mem_usages.setNum(_changes.elms());
        REPA(mem_usages) {
            UInt u = _changes[i].memUsage();
            mem_usages[i] = u;
            mem_usage += u;
        }
    }
    for (; test_mem_usage && mem_usage > _max_mem_usage || test_change_limit && _changes.elms() > _max_changes;) {
        if (undos() > redos()) // remove first undo
        {
            _undos--;
            _changes.removeValid(0, true);
            if (test_mem_usage) {
                mem_usage -= mem_usages[0];
                mem_usages.remove(0, true);
            }
        } else // remove last redo
        {
            _changes.removeLast();
            if (test_mem_usage) {
                mem_usage -= mem_usages.last();
                mem_usages.removeLast();
            }
        }
    }
}
...
_Undo &_Undo::maxMemUsage(Long limit) {
    ...
    if (_max_mem_usage != limit) {
        ...
        if (_max_mem_usage >= 0 && (_max_mem_usage < old_limit || old_limit < 0))
            clean(true); // if new limit is smaller then potentially we need to remove some changes
    }
    return T;
}
_Undo &_Undo::maxChanges(Int limit) {
    ...
    if (_max_changes != limit) {
        Int old_limit = limit;
        _max_changes = limit;
        if (_max_changes >= 0 && (_max_changes < old_limit || old_limit < 0))
            clean(false); // if new limit is smaller then potentially we need to remove some changes
    }
    return T;
}
/******************************************************************************/
...
_Undo::Change *_Undo::set(CPtr change_type, Bool force_create, Flt extra_time) {
    ...
    if (!undos()                                                                                                                          // no undos yet
        ...
        change = &_changes.New();
        if (_full)
            change->create(user); // create only in full mode
    } else if (!_full)            // in partial mode we need to re-use the last change
    {
        // return last one
...
Bool _Undo::undo() {
    ...
    {
        if (_full && undos() == changes())
            if (set(null, true, 0))
                _undos--; // in full mode if we're at the end then we need to save current state
...
Bool _Undo::redo() {
    ...
    if (InRange(undos(), changes())) // if there are still any Redo Changes
    {
        ...
        if (_full) {
            change.apply(user);
            if (undos() == changes())
                _changes.removeLast(); // if we're at the end, then there's no need to keep this change anymore
