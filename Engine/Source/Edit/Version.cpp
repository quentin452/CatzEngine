...
static Bool LoadVersions(C Str &name, MemPtr<FileVersion> versions, Bool latest_only = false) {
    File f;
    if (f.readStd(name))
        return LoadVersions(f, versions, latest_only);
    if (versions.resizable())
        versions.clear();
    else
        REPAO(versions).zero();
...
static Bool SaveVersions(C Str &name, C CMemPtr<FileVersion> &versions) {
    File f;
    if (f.write(name)) {
        SaveVersions(f, versions);
        return true;
    }
    return false;
}
static Bool SafeOverwriteVersions(C Str &name, C CMemPtr<FileVersion> &versions) {
    File f;
    f.writeMem();
    SaveVersions(f, versions);
    f.pos(0);
...
{
    File dest;
    if (WriteVersion(dest, name, date_time))
        return f.copy(dest) && dest.flush();
    return false;
}
    ...
    if (versions.binaryInclude(version, Compare))                    // if version doesn't exist yet
        if (version.removed() || SaveVersion(f, name, version.time)) // if written file data (skip saving if file is removed)
        {
            if (treat_base_as_latest)                 // check if we're saving latest version
                ...
                {
                    if (versions.elms() >= 2 && !versions[1].removed())
                        FRename(name, VersionName(name, versions[1].time)); // move previous latest from base into the version folder
                    if (!versions[0].removed())
                        FRename(VersionName(name, versions[0].time), name); // move new      latest from version folder to base
                }
