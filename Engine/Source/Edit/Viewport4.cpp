    ...
    if (/*gpc.visible &&*/ visible()) {
        ...
        if (Viewport4 *v4 = (Viewport4 *)user)
            if (v4->sel() < 0 && rect_color.a) {
                ...
                D.clip(gpc.clip);
                // down
                if (this == &v4->view[VIEW_FRONT].viewport || this == &v4->view[VIEW_LEFT].viewport)
                    D.lineX(rect_color, r.max.y - D._pixel_size_2.y, r.min.x, r.max.x); // align down = -pixel_size
                // right
                if (this == &v4->view[VIEW_DIAGONAL].viewport || this == &v4->view[VIEW_LEFT].viewport)
                    D.lineY(rect_color, r.min.x + D._pixel_size_2.x, r.min.y, r.max.y); // align right = +pixel_size
            }
    }
}
/******************************************************************************/
// CUBE
/******************************************************************************/
Viewport4::Cube &Viewport4::Cube::create(Viewport4 &v4, View &view) {
    super::create();
    T.v4 = &v4;
    T.view = &view;
    mesh = "Gui/Viewcube/0.mesh";
    return T;
}
...
void Viewport4::Cube::setView(C GuiPC &gpc, DisplayClass::ViewportSettings &view, Camera &camera, MatrixM &matrix) {
    if (T.view) {
        ...
        matrix.identity();
        matrix.pos.z += 3.3f;
        matrix.pos *= T.view->camera.matrix;
    }
}
...
GuiObj *Viewport4::Cube::test(C GuiPC &gpc, C Vec2 &pos, GuiObj *&mouse_wheel) {
    ...
    if (mesh && super::test(gpc, pos, mouse_wheel)) {
        DisplayClass::ViewportSettings view;
        Camera camera;
        MatrixM matrix;
        setView(gpc, view, camera, matrix);
        VecD pos3D;
        Vec dir;
        ScreenToPosDir(pos, pos3D, dir);
        if (Sweep(pos3D, dir * D.viewRange(), *mesh, &matrix, null, null, null, &part, false))
            ret = this;
    ...
    if (view && v4) {
        ...
        if (touch || Gui.ms() == this) {
            ...
            if (rot && !move) {
                ...
                if (!(!Kb.ctrlCmd() && Kb.alt()))
                    view.camera.yaw -= (touch ? touch->ad().x * 3.0f : Ms.d().x); // disable X rotation when only Alt  pressed
                if (!(Kb.ctrlCmd() && !Kb.alt()))
                    view.camera.pitch += (touch ? touch->ad().y * 3.0f : Ms.d().y); // disable Y rotation when only Ctrl pressed
                view.camera.setSpherical();
                if (v4->fpp())
                    view.camera += pos - view.camera.matrix.pos;
                if (possible_drag)
                    part = -1;
            }
            if (move) {
                ...
                VecD at = cam.at;
                cam.transformByMouse(0, 0, ((v4->horizontal() ^ rot) ? CAMH_MOVE_XZ : CAMH_MOVE) | CAMH_NO_BEGIN_END | CAMH_NO_SET);
                VecD d = cam.at - at;
                if (v4->lock())
                    REPA(v4->view)
                if (&v4->view[i] != view)
            ...
            if ((touch ? touch->rs() : Ms.br(0)) && !possible_drag && mesh && InRange(part, *mesh)) {
                ...
                if (Equal(name, "l"))
                    y = -PI_2;
                else if (Equal(name, "r"))
                    y = PI_2;
                else if (Equal(name, "f"))
                    y = PI;
                else if (Equal(name, "b"))
                    y = 0;
                else if (Equal(name, "u")) {
...
void Viewport4::Cube::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible() && mesh) {
        ...
        setView(gpc, view, camera, matrix);
        D.depthLock(false);
        D.depthOnWriteFunc(false, false, FUNC_ALWAYS);
        mesh->drawBlend(matrix);
        if (Gui.ms() == this && InRange(part, *mesh))
            mesh->parts[part].drawBlend(&NoTemp(Vec4(2))); // set func in case 'depthLock' is ignored for performance reasons
        D.depthUnlock();
        D.depthOnWriteFunc(false, true, FUNC_DEFAULT);
        D.alpha(alpha); // reset 'depthWrite' and 'alpha' because they can be modified by 'drawBlend'
        resetView(view, camera);
    }
}
/******************************************************************************/
// ZOOM
/******************************************************************************/
Viewport4::Zoom &Viewport4::Zoom::create(Viewport4 *v4, View *view) {
    super::create();
    T.v4 = v4;
    T.view = view;
    return T;
...
void Viewport4::Zoom::update(C GuiPC &gpc) {
    ...
    if (gpc.enabled) {
        ...
        REPA(Touches) {
            ...
            if (touch.guiObj() == this && touch.on())
                zoom -= touch.ad().y * 2.0f;
        }
        if (zoom && v4 && view) {
            ...
            if (v4->lock())
                REPA(v4->view) {
                    if (v4->perspective()) {
                        Clamp(v4->view[i].camera.dist *= scale, v4->minDist(), v4->maxDist());
                        v4->view[i].camera.setSpherical();
                    } else
                        v4->view[i].viewport.fov *= scale;
                }
            else {
                if (v4->perspective()) {
                    Clamp(view->camera.dist *= scale, v4->minDist(), v4->maxDist());
                    view->camera.setSpherical();
                } else
                    view->viewport.fov *= scale;
            }
        }
    }
}
    ...
    if (/*gpc.visible &&*/ visible()) {
        ...
        rect.draw(Color(0, 0, 0, 128), false);
        {
            ...
            Flt offset = -0.2f * Log2((v4 && view) ? v4->perspective() ? view->camera.dist : view->viewport.fov : ActiveCam.dist),
                ...
                x1 = rect.lerpX(0.8f);
            REP(3) {
                Flt y = rect.min.y + Frac(offset + (i * rect.h()) / 3, rect.h());
                VI.line(Vec2(x0, y), Vec2(x1, y));
            }
            VI.end();
        }
    }
...
Viewport4::DPad &Viewport4::DPad ::create(Viewport4 *v4, View *view) {
    super::create();
    T.v4 = v4;
    T.view = view;
    return T;
}
Viewport4::DPadY &Viewport4::DPadY::create(Viewport4 *v4, View *view) {
    super::create();
    T.v4 = v4;
    T.view = view;
    return T;
...
void Viewport4::DPad::update(C GuiPC &gpc) {
    ...
    if (gpc.enabled) {
        ...
        if (pos) {
            ...
            if (!Cuts(*pos, inner)) {
                touched = true;
                Vec2 delta = *pos - inner.pos;
                angle = AlignRound(AngleFast(delta), PI_4);
                CosSin(delta.x, delta.y, angle);
                axis.set(SignEps(delta.x), SignEps(delta.y));
                if (v4 && view) {
                    delta *= v4->moveScale(*view);
                    Vec move;
                    if (AdjustToGround(*v4)) {
                        if (view->camera.matrix.y.y < 0)
                            CHS(delta.y);
                        move.y = 0;
                        CosSin(move.x, move.z, view->camera.yaw + Angle(delta));
                        move *= delta.length();
                    } else if (AdjustToCamera(*v4))
                        move = view->camera.matrix.x * delta.x + view->camera.matrix.y * delta.y;
                    else
                        move = view->camera.matrix.x * delta.x + view->camera.matrix.z * delta.y;
                    if (v4->lock())
                        REPAO(v4->view).camera += move;
                    else
                        view->camera += move;
                }
            }
...
void Viewport4::DPadY::update(C GuiPC &gpc) {
    ...
    if (gpc.enabled) {
        ...
        if (pos) {
            ...
            if (v4 && view) {
                Vec move = (AdjustToGround(*v4) ? Vec(0, 1, 0) : AdjustToCamera(*v4) ? view->camera.matrix.z
                                                                                     : view->camera.matrix.y) *
                           (v4->moveScale(*view) * dir);
                if (v4->lock())
                    REPAO(v4->view).camera += move;
                else
                    view->camera += move;
...
void Viewport4::DPad::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible()) {
        ...
        if (touched)
            circle.drawPie(Color(0, 192, 255, 192), r2, angle - PI_4 / 2, PI_4);
        // draw lines
        {
            VI.color(Color(0, 0, 0, LINEAR_GAMMA ? 192 : 64));
            REP(8) {
                Flt c, s;
                CosSin(c, s, i * PI_4 + PI_4 / 2);
                VI.line(Vec2(circle.pos.x + c * circle.r, circle.pos.y + s * circle.r), Vec2(circle.pos.x + c * r2, circle.pos.y + s * r2));
            }
            VI.end();
        }
        circle.r = r2;
        circle.draw(Color(0, 0, 0, 64));
    }
}
void Viewport4::DPadY::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible()) {
        D.clip(gpc.clip);
        Rect rect = T.rect() + gpc.offset;
        Vec2 right = rect.lerp(0.66f, 0.5f);
        ...
        VI.end();
        if (dir > 0)
            Quad2(rect.left(), rect.lu(), rect.ru(), right).draw(Color(0, 192, 255, 192), true);
        else if (dir < 0)
            Quad2(rect.rd(), rect.ld(), rect.left(), right).draw(Color(0, 192, 255, 192), true);
...
// ANALOG PAD
/******************************************************************************/
Viewport4::APad &Viewport4::APad ::create(Viewport4 &v4, View &view) {
    super::create();
    T.v4 = &v4;
    T.view = &view;
    return T;
}
Viewport4::APadY &Viewport4::APadY::create(Viewport4 &v4, View &view) {
    super::create();
    T.v4 = &v4;
    T.view = &view;
    return T;
...
void Viewport4::APad::update(C GuiPC &gpc) {
    ...
    if (gpc.enabled) {
        ...
        if (pos) {
            touched = true;
            cur = *pos - (rect().center() + gpc.offset);
            if (pushed)
                start = cur;
...
void Viewport4::APadY::update(C GuiPC &gpc) {
    ...
    if (gpc.enabled) {
        ...
        if (pos) {
            touched = true;
            cur = pos->y - (rect().centerY() + gpc.offset.y);
            if (pushed)
                start = cur;
            Vec move = (AdjustToGround(*v4) ? Vec(0, 1, 0) : AdjustToCamera(*v4) ? view->camera.matrix.z
                                                                                 : view->camera.matrix.y) *
                       (v4->moveScale(*view) * (cur - start) / rect().h());
            if (v4->lock())
                REPAO(v4->view).camera += move;
            else
                view->camera += move;
...
// DRAG PAD
/******************************************************************************/
Viewport4::Drag &Viewport4::Drag ::create(Viewport4 &v4, View &view) {
    super::create();
    T.v4 = &v4;
    T.view = &view;
    return T;
}
Viewport4::DragY &Viewport4::DragY::create(Viewport4 &v4, View &view) {
    super::create();
    T.v4 = &v4;
    T.view = &view;
    return T;
...
void Viewport4::DragY::update(C GuiPC &gpc) {
    if (gpc.enabled) {
        ...
        REPA(Touches) {
            Touch &touch = Touches[i];
            if (touch.guiObj() == this && touch.on())
                delta -= touch.ad().y * 2.0f;
        }
        if (delta) {
            Vec move = (AdjustToGround(*v4) ? Vec(0, 1, 0) : AdjustToCamera(*v4) ? view->camera.matrix.z
                                                                                 : view->camera.matrix.y) *
                       (v4->moveScale(*view, false) * delta);
            if (v4->lock())
                REPAO(v4->view).camera += move;
            else
                view->camera += move;
...
// ARROWS
/******************************************************************************/
Viewport4::Arrows &Viewport4::Arrows::create(Viewport4 &v4, View &view) {
    super::create();
    T.v4 = &v4;
    T.view = &view;
    return T;
    ...
    if (gpc.visible && visible() && v4 && view) {
        ...
        if (AdjustToGround(*v4)) {
            ...
            if (di.any()) {
                if (view->camera.matrix.y.y < 0)
                    CHS(di.y);
                CosSin(move.x, move.z, view->camera.yaw + Angle(Vec2(di)));
            }
        } else if (AdjustToCamera(*v4)) {
            if (arrow_pushed[DIR_RIGHT])
                move += view->camera.matrix.x;
            if (arrow_pushed[DIR_LEFT])
                move -= view->camera.matrix.x;
            if (arrow_pushed[DIR_FORWARD])
                move += view->camera.matrix.y;
            if (arrow_pushed[DIR_BACK])
                move -= view->camera.matrix.y;
        } else {
            if (arrow_pushed[DIR_RIGHT])
                move += view->camera.matrix.x;
            if (arrow_pushed[DIR_LEFT])
                move -= view->camera.matrix.x;
            if (arrow_pushed[DIR_FORWARD])
                move += view->camera.matrix.z;
            if (arrow_pushed[DIR_BACK])
                move -= view->camera.matrix.z;
        }
        if (arrow_pushed[DIR_UP])
            move += (AdjustToGround(*v4) ? Vec(0, 1, 0) : AdjustToCamera(*v4) ? view->camera.matrix.z
                                                                              : view->camera.matrix.y);
        if (arrow_pushed[DIR_DOWN])
            move -= (AdjustToGround(*v4) ? Vec(0, 1, 0) : AdjustToCamera(*v4) ? view->camera.matrix.z
                                                                              : view->camera.matrix.y);
        move *= v4->moveScale(*view);
        if (v4->lock())
            REPAO(v4->view).camera += move;
        else
            view->camera += move;
...
void Viewport4::Arrows::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible()) {
        ...
        b.text_size = 0.75f; // set disabled to be half transparent
        REPA(arrow_rect) {
            if (i == DIR_UP)
                b.text = (AdjustToCamera(*v4) ? "Fw" : "Up");
            else if (i == DIR_DOWN)
                b.text = (AdjustToCamera(*v4) ? "Bk" : "Dn");
            else
                b.text.clear();
...
Viewport4 &Viewport4::create(void (*draw)(Viewport &), Flt default_pitch, Flt default_yaw, Flt default_dist, Flt from, Flt range) {
    ...
    REPA(_cube)
    _cube[i].create(T, view[i]);
    REPA(_zoom)
    _zoom[i].create(&T, &view[i]);
    REPA(_dpad)
    _dpad[i].create(&T, &view[i]);
    REPA(_dpady)
    _dpady[i].create(&T, &view[i]);
    REPA(_apad)
    _apad[i].create(T, view[i]);
    REPA(_apady)
    _apady[i].create(T, view[i]);
    REPA(_drag)
    _drag[i].create(T, view[i]);
    REPA(_dragy)
    _dragy[i].create(T, view[i]);
    REPA(_arrows)
    _arrows[i].create(T, view[i]);
    resetPosOrn();
    sel(-1);
    return T;
}
...
Viewport4::View *Viewport4::getViewCtrl(GuiObj *go) {
    if (go)
        REPA(view)
    if (go == &view[i].viewport || go == &_cube[i] || go == &_zoom[i] || go == &_dpad[i] || go == &_dpady[i] || go == &_apad[i] || go == &_apady[i] || go == &_drag[i] || go == &_dragy[i] || go == &_arrows[i])
        return &view[i];
    return null;
}
...
Viewport4 &Viewport4::sel(Int view) {
    ...
    if (T._sel != view) {
        T._sel = view;
        if (InRange(T._sel, T.view))
            _last = &T.view[T._sel]; // if we're selecting one view then set it as last to help with setting default view/cam for it without needing the mouse to hover over it
        if (visible())
            visible(true);
        setRect();
    }
...
Viewport4 &Viewport4::fppSpeed(Flt speed) {
    T._fpp_speed = Max(0, speed);
    return T;
}
Viewport4 &Viewport4::perspFov(Flt fov) {
    _persp_fov = fov;
    if (perspective())
        REPAO(view).viewport.fov = fov;
    return T;
}
...
Viewport4 &Viewport4::visible(Bool visible) {
    ...
    REPAO(_cube).visible(visible && _orn_mode && (T._sel < 0 || T._sel == i));
    if (visible && _orn_mode)
        setCubeRect();
...
Viewport4 &Viewport4::resetPosOrn() {
    if (lock()) {
        ...
        if (_last == &view[VIEW_FRONT])
            _last->camera.setSpherical(VecDZero, _default_yaw, _default_pitch, 0, perspective() ? _default_dist : _last->viewport.range * 0.5f);
        else if (_last == &view[VIEW_TOP])
            _last->camera.setSpherical(VecDZero, _default_yaw, _default_pitch - PI_2, 0, perspective() ? _default_dist : _last->viewport.range * 0.5f);
        else if (_last == &view[VIEW_DIAGONAL])
            _last->camera.setSpherical(VecDZero, _default_yaw + PI_4, _default_pitch - PI_4, 0, perspective() ? _default_dist : _last->viewport.range * 0.5f);
        else if (_last == &view[VIEW_LEFT])
            _last->camera.setSpherical(VecDZero, _default_yaw + PI_2, _default_pitch, 0, perspective() ? _default_dist : _last->viewport.range * 0.5f);
    }
    return T;
}
Viewport4 &Viewport4::axisAlign() {
    if (lock()) {
        ...
        if (_last == &view[VIEW_FRONT])
            _last->camera.setSpherical(_last->camera.at, _default_yaw, 0, 0, _last->camera.dist);
        else if (_last == &view[VIEW_TOP])
            _last->camera.setSpherical(_last->camera.at, _default_yaw, -PI_2, 0, _last->camera.dist);
        else if (_last == &view[VIEW_DIAGONAL])
            _last->camera.setSpherical(_last->camera.at, _default_yaw + PI_4, -PI_4, 0, _last->camera.dist);
        else if (_last == &view[VIEW_LEFT])
            _last->camera.setSpherical(_last->camera.at, _default_yaw + PI_2, 0, 0, _last->camera.dist);
    }
    return T;
}
...
Viewport4 &Viewport4::toggleLock() {
    if (_lock ^= 1) {
        if (!fpp())
            view[VIEW_FRONT].camera.at = _last->camera.at;
        else
            view[VIEW_FRONT].camera += _last->camera.matrix.pos - view[VIEW_FRONT].camera.matrix.pos;
    }
    return T;
}
Viewport4 &Viewport4::setViewportCamera() {
    if (_last)
        _last->setViewportCamera();
    return T;
}
/******************************************************************************/
// UPDATE
...
void Viewport4::update() {
    ...
    Bool zoom = (Ms.wheel() && !Kb.ctrlCmd() && !Kb.shift() && !Kb.alt() && !fpp()),
         ...
         rotate_move = (rotate && move);
    if (rotate_move)
        rotate = move = false;

    // set main camera
    View &v = (lock() ? view[VIEW_FRONT] : *_last);
    v.viewport.setDisplayView();
    Camera &cam = v.camera;

    // zoom / rotate
    if (_focus) {
        if (rotate || move || rotate_move || zoom_manual)
            Ms.freeze();
        ...
        if (zoom_manual) {
            ...
            if (perspective())
                Clamp(cam.dist *= f, minDist(), maxDist());
            else
                REPAO(view).viewport.fov *= f;
        }
        UInt flag = CAMH_NO_BEGIN_END | CAMH_NO_SET;
        if (rotate) {
            if (rotate_ex) {
                if (!(!Kb.ctrlCmd() && Kb.alt()))
                    flag |= CAMH_ROT_X; // disable X rotation when only Alt  pressed
                if (!(Kb.ctrlCmd() && !Kb.alt()))
                    flag |= CAMH_ROT_Y; // disable Y rotation when only Ctrl pressed
            } else
                flag |= CAMH_ROT;
        }
    ...
    if (zoom)
        if (View *view = getViewCtrl(Gui.ms())) {
            if (perspective())
                Clamp(cam.dist *= ScaleFactor(Ms.wheel() * -0.3f), minDist(), maxDist());
            else
                REPAO(T.view).viewport.fov *= ScaleFactor(Ms.wheel() * -0.3f);
        }
    ...
    if (lock()) {
        ...
        view[VIEW_DIAGONAL].camera.pitch -= PI_4;
        view[VIEW_DIAGONAL].camera.yaw += PI_4;
    ...
    if (_focus && !Gui.window()) {
        ...
        if (_keyboard && !Kb.ctrlCmd() && !Kb.alt())
            if (!Gui.kb() || Gui.kb()->type() != GO_TEXTLINE && Gui.kb()->type() != GO_TEXTBOX && Gui.kb()->type() != GO_LIST) {
                ...
                if (Kb.b(Kb.qwerty(KB_A)) || Kb.b(KB_LEFT))
                    cam.at -= cam.matrix.x * scale;
                if (Kb.b(Kb.qwerty(KB_D)) || Kb.b(KB_RIGHT))
                    cam.at += cam.matrix.x * scale;
                if (Kb.b(Kb.qwerty(KB_S)) || Kb.b(KB_DOWN))
                    cam.at -= cam.matrix.z * scale;
                if (Kb.b(Kb.qwerty(KB_W)) || Kb.b(KB_UP))
                    cam.at += cam.matrix.z * scale;
                if (Kb.b(Kb.qwerty(KB_Q)))
                    cam.at.y -= scale;
                if (Kb.b(Kb.qwerty(KB_E)))
                    cam.at.y += scale;
            }
        ...
        if (move || rotate_move) {
            if (!horizontal())
                Swap(move, rotate_move);
            Flt temp;
            if (fpp()) {
                temp = cam.dist;
                cam.dist = fppSpeed() * 0.5f;
            }
            cam.transformByMouse(minDist(), maxDist(), (move ? CAMH_MOVE_XZ : 0) | (rotate_move ? CAMH_MOVE : 0) | CAMH_NO_BEGIN_END | CAMH_NO_SET);
            if (fpp())
                cam.dist = temp;
        }
...
void operator+=(GuiObj &gui_obj, Edit::Viewport4 &child) {
    REPA(child.view)
    gui_obj += child.view[i].viewport;
    REPA(child._cube)
    gui_obj += child._cube[i];
    REPA(child._zoom)
    gui_obj += child._zoom[i];
    REPA(child._dpad)
    gui_obj += child._dpad[i];
    REPA(child._dpady)
    gui_obj += child._dpady[i];
    REPA(child._apad)
    gui_obj += child._apad[i];
    REPA(child._apady)
    gui_obj += child._apady[i];
    REPA(child._drag)
    gui_obj += child._drag[i];
    REPA(child._dragy)
    gui_obj += child._dragy[i];
    REPA(child._arrows)
    gui_obj += child._arrows[i];
}
void operator-=(GuiObj &gui_obj, Edit::Viewport4 &child) {
    REPA(child.view)
    gui_obj -= child.view[i].viewport;
