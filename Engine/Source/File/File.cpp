...
Bool File::setBuf(Int size) {
    if (size > _buf_size) {
        delBuf();
        if (_buf = Alloc(size))
            _buf_size = size;
    }
    return !size || _buf != null;
}

File &File::del() {
    close();
    delBuf(); // !! delete buffer after closing, because it may be used by 'flush' !!
#if !FILE_MEMB_UNION
    _memb.del();
#endif
    return T;
...
void File::close() {
    ...
    case FILE_STD_WRITE: {
        ...
        if (_writable)
            FlushIO(); // check for '_writable' instead of 'FILE_STD_WRITE' because File could have been created as writable but later switched to FILE_STD_READ
    } break;
...
File &File::mustEdit(C Str &name, Cipher *cipher) {
    if (!edit(name, cipher))
        Exit(MLT(S + "Can't edit \"" + name + '"', PL, S + u"Nie można edytować \"" + name + '"'));
    return T;
}
File &File::mustAppend(C Str &name, Cipher *cipher) {
    if (!append(name, cipher))
        Exit(MLT(S + "Can't append \"" + name + '"', PL, S + u"Nie można nadpisać \"" + name + '"'));
    return T;
}
File &File::mustWrite(C Str &name, Cipher *cipher) {
    if (!write(name, cipher))
        Exit(MLT(S + "Can't create \"" + name + '"', PL, S + u"Nie można utworzyć \"" + name + '"'));
    return T;
}
File &File::mustReadStd(C Str &name, Cipher *cipher) {
    if (!readStd(name, cipher))
        Exit(MLT(S + "Can't open \"" + name + '"', PL, S + u"Nie można otworzyć \"" + name + '"'));
    return T;
}
File &File::mustRead(C PakFile &file, C Pak &pak) {
    if (!read(file, pak))
        Exit(MLT(S + "Can't open \"" + file.name + '"', PL, S + u"Nie można otworzyć \"" + file.name + '"'));
    return T;
}
File &File::mustRead(C Str &name, C Pak &pak) {
    if (!read(name, pak))
        Exit(MLT(S + "Can't open \"" + name + '"', PL, S + u"Nie można otworzyć \"" + name + '"'));
    return T;
}
File &File::mustRead(C UID &id, C Pak &pak) {
    if (!read(id, pak))
        Exit(MLT(S + "Can't open \"" + id.asCString() + '"', PL, S + u"Nie można otworzyć \"" + id.asCString() + '"'));
    return T;
}
File &File::mustRead(C Str &name, C PakSet &paks) {
    if (!read(name, paks))
        Exit(MLT(S + "Can't open \"" + name + '"', PL, S + u"Nie można otworzyć \"" + name + '"'));
    return T;
}
File &File::mustRead(C UID &id, C PakSet &paks) {
    if (!read(id, paks))
        Exit(MLT(S + "Can't open \"" + id.asCString() + '"', PL, S + u"Nie można otworzyć \"" + id.asCString() + '"'));
    return T;
}
File &File::mustRead(C Str &name, Cipher *cipher) {
    if (!read(name, cipher))
        Exit(MLT(S + "Can't open \"" + name + '"', PL, S + u"Nie można otworzyć \"" + name + '"'));
    return T;
}
File &File::mustRead(C UID &id, Cipher *cipher) {
    if (!read(id, cipher))
        Exit(MLT(S + "Can't open \"" + id.asCString() + '"', PL, S + u"Nie można otworzyć \"" + id.asCString() + '"'));
    return T;
}
...
Bool File::copyToAndDiscard(Mems<Byte> &dest) {
    ...
    {                                                                     // swap memories, perhaps 'File' may reuse 'Mems' data for future operations
        Ptr data = dest.data();
        Int elms = dest.elms();
        dest.setTemp((Byte *)T._mem, T._size);
        T._ok = true;
        T._cipher_offset = 0;
        T._offset = 0;
        T._pos = 0;
...
Bool File::decompress(COMPRESS_TYPE compression, ULong decompressed_size, Bool stream) {
    if (!compression || end())
        return true; // no compression or nothing left to decompress
    if (stream) {
        FileStream *stream = null;
        switch (compression) {
#if SUPPORT_LZ4
        case COMPRESS_LZ4:
            if (decompressed_size <= LZ4_BUF_SIZE)
                goto mem;
            stream = Alloc<FileStreamLZ4>();
            CTOR(*(FileStreamLZ4 *)stream);
            break; // #LZ4Mem
#endif
#if SUPPORT_ZSTD
        case COMPRESS_ZSTD:
            if (decompressed_size <= ZSTD_BLOCKSIZE_MAX)
                goto mem;
            UInt buf_size = ZSTDDecompressBufSize(decompressed_size);
            FileStreamZSTD *zstd = (FileStreamZSTD *)Alloc(SIZE(FileStreamZSTD) + buf_size);
            CTOR(*zstd);
            zstd->buf_size = buf_size;
            stream = zstd;
            break; // #ZSTDMem
#endif
        }
...
/******************************************************************************/
#if WINDOWS
Bool File::edit(C Str &name, Cipher *cipher) {
    close();
    if (name.is() && !_wsopen_s(&_handle, name, _O_BINARY | _O_RDWR | _O_CREAT, _SH_DENYWR, _S_IREAD | _S_IWRITE)) {
        SetLastError(0);
        _type = FILE_STD_WRITE;
        if (setBuf(BUF_SIZE)) {
            _writable = true;
            _path = FILE_CUR;
            _size = _full_size = Max(0, _filelengthi64(_handle));
            _cipher = cipher;
            return true;
        }
        _type = FILE_NONE;
    ...
    return false;
} // clear error 183 file already exists
Bool File::append(C Str &name, Cipher *cipher) {
    close();
    if (name.is() && !_wsopen_s(&_handle, name, _O_BINARY | _O_RDWR | _O_CREAT, _SH_DENYWR, _S_IREAD | _S_IWRITE)) {
        SetLastError(0);
        _type = FILE_STD_WRITE;
        if (setBuf(BUF_SIZE)) {
            ...
            _path = FILE_CUR;
            _size = _full_size = Max(0, _filelengthi64(_handle));
            _pos = Max(0, Seek(_handle, 0, SEEK_END));
            _cipher = cipher;
            return true;
        }
        _type = FILE_NONE;
...
Bool File::write(C Str &name, Cipher *cipher) {
    close();
    if (name.is() && !_wsopen_s(&_handle, name, _O_BINARY | _O_RDWR | _O_CREAT | _O_TRUNC, _SH_DENYWR, _S_IREAD | _S_IWRITE)) {
        SetLastError(0);
        _type = FILE_STD_WRITE;
        if (setBuf(BUF_SIZE)) {
            _writable = true;
...
Bool File::readStdEx(C Str &name, Cipher *cipher, UInt max_buf_size) {
    ...
    if (name.is()) {
        if (!_wsopen_s(&_handle, name, _O_BINARY | _O_RDONLY, _SH_DENYNO, _S_IREAD | _S_IWRITE)) {
            _type = FILE_STD_READ;
            _size = Max(0, _filelengthi64(_handle));
            if (setBuf(Min(max_buf_size, Min(BUF_SIZE, _size)))) { /*_writable=false; already cleared in 'close'*/
                _path = FILE_CUR;
                _cipher = cipher;
                _full_size = _size;
                return true;
        ...
        if (DataPath().is() && !FullPath(name)) {
            Char path_name[MAX_LONG_PATH];
            MergePath(path_name, DataPath(), name);
...
static void DetectSymLink(File &f, C Str8 &unix_name) {
    ...
    if (!fstat(f._handle, &stats))
        if (S_ISLNK(stats.st_mode)) {
            ...
            ssize_t read = readlink(unix_name, (char *)f._buf, Min(f._buf_size, f.size()));
            if (read > 0)
                f._buf_len = Int(read);
        }
}
Bool File::edit(C Str &name, Cipher *cipher) {
    close();
    if (name.is()) {
        _handle = open(UnixPathUTF8(name), O_RDWR | O_CREAT | O_EXLOCK | O_NONBLOCK | O_SYMLINK, S_IRWXU | S_IRWXG | S_IRWXO);
        if (_handle >= 0) {
            _type = FILE_STD_WRITE;
            if (setBuf(BUF_SIZE)) {
                _writable = true;
                _path = FILE_CUR;
                _pos = _size = _full_size = Max(0, Seek(_handle, 0, SEEK_END));
                Long pos = Seek(_handle, 0, SEEK_SET);
                if (pos >= 0)
                    _pos = pos;
                _cipher = cipher;
                return true;
...
Bool File::write(C Str &name, Cipher *cipher) {
    close();
    if (name.is()) {
        _handle = open(UnixPathUTF8(name), O_RDWR | O_CREAT | O_TRUNC | O_EXLOCK | O_NONBLOCK | O_SYMLINK, S_IRWXU | S_IRWXG | S_IRWXO);
        if (_handle >= 0) {
            _type = FILE_STD_WRITE;
            if (setBuf(BUF_SIZE)) {
                _writable = true;
...
Bool File::readStdEx(C Str &name, Cipher *cipher, UInt max_buf_size) {
    ...
    if (name.is()) {
        ...
        _handle = open(unix_name, O_RDONLY | O_NONBLOCK | O_SYMLINK);
        if (_handle >= 0) {
            _type = FILE_STD_READ;
            _size = Max(0, Seek(_handle, 0, SEEK_END));
            if (setBuf(Min(max_buf_size, Min(BUF_SIZE, _size)))) { /*_writable=false; already cleared in 'close'*/
                _path = FILE_CUR;
                Seek(_handle, 0, SEEK_SET);
                _cipher = cipher;
                _full_size = _size;
                DetectSymLink(T, unix_name);
                return true;
            }
            _type = FILE_NONE;
            ::close(_handle);
...
Bool File::edit(C Str &name, Cipher *cipher) {
    close();
    if (name.is()) {
        _handle = open64(UnixPathUTF8(name), O_RDWR | O_CREAT | O_NONBLOCK, S_IRWXU | S_IRWXG | S_IRWXO);
        if (_handle >= 0) {
            _type = FILE_STD_WRITE;
            if (setBuf(BUF_SIZE)) {
                _writable = true;
...
Bool File::write(C Str &name, Cipher *cipher) {
    close();
    if (name.is()) {
        _handle = open64(UnixPathUTF8(name), O_RDWR | O_CREAT | O_TRUNC | O_NONBLOCK, S_IRWXU | S_IRWXG | S_IRWXO);
        if (_handle >= 0) {
            _type = FILE_STD_WRITE;
            if (setBuf(BUF_SIZE)) {
                _writable = true;
...
Bool File::readStdEx(C Str &name, Cipher *cipher, UInt max_buf_size) {
    ...
    if (name.is()) {
        Str8 unix_name = UnixPathUTF8(name);
        _handle = open64(unix_name, O_RDONLY | O_NONBLOCK);
        if (_handle >= 0) {
            _type = FILE_STD_READ;
            _size = Max(0, Seek(_handle, 0, SEEK_END));
            if (setBuf(Min(max_buf_size, Min(BUF_SIZE, _size)))) { /*_writable=false; already cleared in 'close'*/
...
Bool File::edit(C Str &name, Cipher *cipher) {
    close();
    if (name.is()) {
        _handle = open(UnixPathUTF8(name), O_RDWR | O_CREAT | O_NONBLOCK | O_LARGEFILE, S_IRWXU | S_IRWXG | S_IRWXO);
        if (_handle >= 0) {
            _type = FILE_STD_WRITE;
            if (setBuf(BUF_SIZE)) {
                _writable = true;
...
Bool File::write(C Str &name, Cipher *cipher) {
    close();
    if (name.is()) {
        _handle = open(UnixPathUTF8(name), O_RDWR | O_CREAT | O_TRUNC | O_NONBLOCK | O_LARGEFILE, S_IRWXU | S_IRWXG | S_IRWXO);
        if (_handle >= 0) {
            _type = FILE_STD_WRITE;
            if (setBuf(BUF_SIZE)) {
                _writable = true;
    ...
    if (name.is()) {
        ...
        if (!FullPath(name) && AssetManager)
            if (AAsset *asset = AAssetManager_open(AssetManager, UnixPathUTF8(name), AASSET_MODE_RANDOM)) {
                ...
                if (size < 0)
                    ok = false;
                else if (!size) // no need for opening the file using Android API when it has no size
                {
                    T._type = FILE_MEM;
                    // T._writable =false; already cleared in 'close'
                    // T._allocated=false; already cleared in 'close'
        ...
        // original path
        _handle = open(UnixPathUTF8(name), O_RDONLY | O_NONBLOCK | O_LARGEFILE);
        if (_handle >= 0) {
            _type = FILE_STD_READ;
            _size = Max(0, Seek(_handle, 0, SEEK_END));
            if (setBuf(Min(max_buf_size, Min(BUF_SIZE, _size)))) { /*_writable=false; already cleared in 'close'*/
        ...
        if (DataPath().is() && !FullPath(name)) {
            Char path_name[MAX_LONG_PATH];
            MergePath(path_name, DataPath(), name);
            _handle = open(UnixPathUTF8(path_name), O_RDONLY | O_NONBLOCK | O_LARGEFILE);
            if (_handle >= 0) {
                _type = FILE_STD_READ;
                _size = Max(0, Seek(_handle, 0, SEEK_END));
                if (setBuf(Min(max_buf_size, Min(BUF_SIZE, _size)))) { /*_writable=false; already cleared in 'close'*/
...
Bool File::readFD(Int fd, const_mem_addr Cipher *cipher) // !! THIS TAKES OWNERSHIP OF 'fd' AND WILL 'close' IT !!
{
    ...
    if (fd >= 0) {
        ...
        _type = FILE_STD_READ;
        _size = Max(0, Seek(_handle, 0, SEEK_END));
        if (setBuf(Min(BUF_SIZE, _size))) { /*_writable=false; already cleared in 'close'*/
            _path = FILE_CUR;
            Seek(_handle, 0, SEEK_SET);
            _cipher = cipher;
            _full_size = _size;
...
Bool File::readRaw(C PakFile &file, C Pak &pak) {
    if (!(file.flag & PF_REMOVED))
        switch (pak._file_type) {
        case FILE_MEM: {
        ...
        case FILE_STD_READ: {
            ...
            } else if (readStdEx(pak.pakFileName(), pak._file_cipher, file.data_size_compressed)) {
                _offset = pak._data_offset + file.data_offset;
                if (Seek(_handle, _offset, SEEK_SET) == _offset) {
                    T._pak = &pak;
...
Bool File::readEx(C PakFile &file, C Pak &pak, Cipher *cipher, Bool *processed, Bool stream) {
    if (readRaw(file, pak)) {
        ...
        {
            if (!decompress(file.compression, file.data_size, stream))
                goto error;
        ...
        if (cipher) // if we want to use custom cipher on top of what's already available (don't check for 'cipher!=T._cipher' because we want to add new cipher on top of existing)
        {
            if (T._cipher) // get rid of existing cipher by decrypting it to a memory file
            {
                p = true; // we had to perform processing
                File temp;
                if (!copy(temp.writeMemFixed(size(), null)))
                    goto error; // we can't apply 'cipher' here and later clear it, because that would encrypt it with 'cipher', however what we want is to decrypt it with 'cipher'
                Swap(T, temp);
                pos(0);
            } else
                _cipher_offset = 0; // if there's no cipher yet, but there was '_cipher_offset', then we need to clear it, as it would affect the new cipher
...
Bool File::readEx(C Str &name, Cipher *cipher, Bool *processed, Bool stream) {
    if (readEx(name, Paks, cipher, processed, stream))
        return true;
#if !ANDROID
    if (processed)
        *processed = false;
    return readStdEx(name, cipher);
#else
    return readStdEx(name, cipher, UINT_MAX, processed);
#endif
}
Bool File::readEx(C UID &id, Cipher *cipher, Bool *processed, Bool stream) {
    if (readEx(id, Paks, cipher, processed, stream))
        return true;
#if !ANDROID
    if (processed)
        *processed = false;
    return readStdEx(_EncodeFileName(id), cipher);
#else
...
Bool File::read(C Str &name, C Pak &pak) {
    if (C PakFile *file = pak.find(name, false))
        return read(*file, pak);
    close();
    return false;
}
Bool File::read(C UID &id, C Pak &pak) {
    if (C PakFile *file = pak.find(id, false))
        return read(*file, pak);
    close();
    return false;
}
...
Long File::posFile() C {
    ...
    default:
        return pos();
    case FILE_STD_READ:
        return pos() + _buf_len; // in read  mode, the actual position is ahead
    case FILE_STD_WRITE:
        return pos() - _buf_len; // in write mode, the actual position is behind
    }
}
    ...
    if (pos >= 0)
        ...
        case FILE_STD_READ: {
            {
                    ...
                    && -delta <= _buf_pos) // we skip backward and we have previous data in the buffer
                {
                    ...
                    _buf_pos += delta;
                    T._pos = pos;
                    return true; // don't seek (because file position is already in order with the buffer)
                }
            }
            clearBuf(); // if we're going to seek then we need to clear the buffer if any

        seek:
            Long abs_pos = Seek(_handle, _offset + pos, SEEK_SET);
            if (abs_pos < 0)
                return false;
        ...
        case FILE_STREAM: {
            ...
            } else // go forward
            {
                if (!_buf_len)
                stream_get:
                    _stream->get(T); // read from stream
                if (_buf_len) {
...
File &File::cmpIntV(Int i) {
    Byte data[MaxCmpUIntVSize];
    Int pos = 0;
    Bool negative = (i < 0);
    UInt u = (negative ? -(i + 1) : i);
    data[pos++] = ((u & 63) | (negative << 6) | ((u >= 64) << 7)); // 0
    if (u >= 64) {
        u >>= 6;
        data[pos++] = ((u & 127) | ((u >= 128) << 7)); // 1
        if (u >= 128) {
            u >>= 7;
            data[pos++] = ((u & 127) | ((u >= 128) << 7)); // 2
            if (u >= 128) {
                u >>= 7;
                data[pos++] = ((u & 127) | ((u >= 128) << 7)); // 3
                if (u >= 128) {
                    u >>= 7;
                    data[pos++] = (u); // 4
                }
    ...
    if (u >= 64) {
        ...
        if (u >= 128) {
            ...
            if (u >= 128) {
                ...
                if (u >= 128) {
                    u >>= 7;
                    data[pos++] = ((u & 127) | ((u >= 128) << 7)); // 4
                    if (u >= 128) {
                        u >>= 7;
                        data[pos++] = ((u & 127) | ((u >= 128) << 7)); // 5
                        if (u >= 128) {
                            u >>= 7;
                            data[pos++] = ((u & 127) | ((u >= 128) << 7)); // 6
                            if (u >= 128) {
                                u >>= 7;
                                data[pos++] = ((u & 127) | ((u >= 128) << 7)); // 7
                                if (u >= 128) {
                                    u >>= 7;
                                    data[pos++] = (u); // 8
                                }
    ...
    if (v & 128) {
        ...
        if (v & 128) {
            ...
            if (v & 128) {
                ...
                if (v & 128) {
                    T >> v;
                    u |= (U64(v & 127) << (6 + 7 + 7 + 7)); // << 27 requires U64
                    if (v & 128) {
                        T >> v;
                        u |= (U64(v & 127) << (6 + 7 + 7 + 7 + 7));
                        if (v & 128) {
                            T >> v;
                            u |= (U64(v & 127) << (6 + 7 + 7 + 7 + 7 + 7));
                            if (v & 128) {
                                T >> v;
                                u |= (U64(v & 127) << (6 + 7 + 7 + 7 + 7 + 7 + 7));
                                if (v & 128) {
                                    T >> v;
                                    u |= (U64(v) << (6 + 7 + 7 + 7 + 7 + 7 + 7 + 7));
                                }
                            }
                        }
                    }
    ...
    if (v & 128) {
        ...
        if (v & 128) {
            ...
            if (v & 128) {
                ...
                if (v & 128) {
                    T >> v;
                    u |= (U64(v & 127) << (7 + 7 + 7 + 7)); // << 28 requires U64
                    if (v & 128) {
                        T >> v;
                        u |= (U64(v & 127) << (7 + 7 + 7 + 7 + 7));
                        if (v & 128) {
                            T >> v;
                            u |= (U64(v & 127) << (7 + 7 + 7 + 7 + 7 + 7));
                            if (v & 128) {
                                T >> v;
                                u |= (U64(v & 127) << (7 + 7 + 7 + 7 + 7 + 7 + 7));
                                if (v & 128) {
                                    T >> v;
                                    u |= (U64(v) << (7 + 7 + 7 + 7 + 7 + 7 + 7 + 7));
                                }
                            }
                        }
                    }
...
File &File::cmpFlt3cm(C Flt &r) {
    UInt u = Mid(Round(r * 100) + 8388608, 0, 16777216 - 1);
    put(&u, 3);
    ASSERT(SIZE(u) >= 3);
    return T;
}
File &File::decFlt3cm(Flt &r) {
    Int i = 0;
    get(&i, 3);
    r = (i - 8388608) / 100.0f;
    ASSERT(SIZE(i) >= 3);
...
File &File::cmpSatFlt1(C Flt &r) {
    putByte(FltToByte(r));
    return T;
}
File &File::cmpSatFlt2(C Flt &r) {
    putUShort(RoundU(Sat(r) * 65535));
    return T;
} // it's better to clamp flt for bigger values
File &File::decSatFlt1(Flt &r) {
    r = getByte() / 255.0f;
    return T;
}
File &File::decSatFlt2(Flt &r) {
    r = getUShort() / 65535.0f;
    return T;
}

File &File::cmpAngle1(C Flt &r) {
    putByte(RoundU(AngleFull(r) * (256 / PI2)));
    return T;
} // use 'AngleFull' to make angle smaller and thus 'RoundPos' work with better precision, there's no need for 'Mid' or "&" because 1) AngleFull already sets 0..PI2 range, 2) putByte   takes only  Byte  as param
File &File::cmpAngle2(C Flt &r) {
    putUShort(RoundU(AngleFull(r) * (65536 / PI2)));
    return T;
} // use 'AngleFull' to make angle smaller and thus 'RoundPos' work with better precision, there's no need for 'Mid' or "&" because 1) AngleFull already sets 0..PI2 range, 2) putUShort takes only UShort as param
File &File::decAngle1(Flt &r) {
    r = getByte() * (PI2 / 256);
    return T;
}
File &File::decAngle2(Flt &r) {
    r = getUShort() * (PI2 / 65536);
    return T;
}
...
Bool File::flushDo() {
    ...
    if (written == _buf_len) {
        _buf_len = 0;
        return true;
    } // check this first because most likely this will happen, so return early
#if ALLOW_REFLUSH
...
#else
    if (written > 0)
        _pos += written; // this avoids errors (<0) and when no data was written (==0)
...
Bool File::flush() {
    if (_buf_len && _type == FILE_STD_WRITE)
        return flushDo();
    return true;
}
Bool File::sync() {
    ...
    if (T._cipher != cipher) {
        if (_type == FILE_STD_READ)                 // if we're in reading mode (we can skip FILE_STD_WRITE because any data saved in the buffer is saved with correct cipher already, and we don't want to change it)
            if (Int buf_size = _buf_pos + _buf_len) // and we have any data in buffer that we've read ahead
            {
                ...
                if (T._cipher)
                    T._cipher->encrypt(_buf, _buf, buf_size, offset); // we have to encrypt entire buffer back using old cipher
                if (cipher)
                    cipher->decrypt(_buf, _buf, buf_size, offset); // decrypt using new cipher
            }
        T._cipher = cipher;
    }
}
    ...
    if (T._cipher_offset != offset) {
        if (_type == FILE_STD_READ)                     // if we're in reading mode (we can skip FILE_STD_WRITE because any data saved in the buffer is saved with correct cipher offset already, and we don't want to change it)
            if (_cipher)                                // we have any cipher
                if (Int buf_size = _buf_pos + _buf_len) // and we have any data in buffer that we've read ahead
                {
                    _cipher->encrypt(_buf, _buf, buf_size, posCipher() - _buf_pos);
                    T._cipher_offset = offset;                                      // we have to encrypt entire buffer back using old cipher offset
...
Bool File::size(Long size) {
    ...
    if (size < 0 || _offset)
        return false;
    if (!flush() && ALLOW_REFLUSH)
        error(); // if flush failed, then set as ok=false, because the data will be discarded
    switch (_type) {
    case FILE_STD_READ:
    ...
    case FILE_MEM: {
        if (_allocated)
            _Realloc(_mem, Unsigned(size), _size); // memory allocated manually must be reallocated
        _size = _full_size = size;
    } break;

    case FILE_MEMB:
        _size = _full_size = _memb.setNum(size).elms();
        break; // set '_size' from what 'Memb' will actually have
    }
    return _size == size;
}
/******************************************************************************/
    ...
    if (!data)
        ...
        if (size > 0)
            ...
            case FILE_STD_READ: {
                                           ...
                                           // "size>=_buf_size" was tested as well, but it was slower
                {
                    ...
                    if (l <= 0)
                        break;
                    if (_cipher)
                        _cipher->decrypt(data, data, l, posCipher());
                    _pos += l;
                    size -= l;
                    if (size <= 0)
                        break;
                ...
                if (l <= 0)
                    break;
                if (_cipher)
                    _cipher->decrypt(_buf, _buf, l, posCipher()); // decrypt entire buffer at the same time (faster)
                _buf_pos = 0;
                _buf_len = l;
            ...
            case FILE_MEM: {
                CPtr src = memFast();
                if (_cipher)
                    _cipher->decrypt(data, src, size, posCipher());
                else
                    CopyFast(data, src, size);
                _pos += size;
            ...
            case FILE_MEMB: {
                for (;;) {
                    ...
                    if (_cipher)
                        _cipher->decrypt(data, &_memb[pos_abs], l, posCipher());
                    else
                        CopyFast(data, &_memb[pos_abs], l);
                    _pos += l;
                    size -= l;
                    if (size <= 0)
                        break;
            ...
            case FILE_STREAM: {
                ...
                get_from_stream_buffer:
                    ...
                    if (_cipher)
                        _cipher->decrypt(data, src, l, posCipher());
                    else
                        CopyFast(data, src, l);
                    _buf_pos += l;
                    _buf_len -= l;
    ...
    if (size > 0) {
        ...
        case FILE_STD_WRITE: {
            if (!_cipher && data) // buffering optional (check for 'data' because it's allowed to be null and write zeros)
            {
                do {
                    if (_buf_len || size < _buf_size) // copy to buffer
                    {
                        ...
                        if (_buf_len >= _buf_size && !flushDo())
                            break; // if buffer is full then flush and check for errors
                    } else         // direct write
                    {
                        Int written = Write(_handle, data, size);
                        if (written <= 0)
                            break;
            ...
            } else // buffering necessary
            {
                do {
                    ...
                    Int l = Min(size, _buf_size - _buf_len);
                    if (_cipher)
                        _cipher->encrypt((Byte *)_buf + _buf_len, data, l, posCipher());
                    else
                        Copy((Byte *)_buf + _buf_len, data, l);
                    _buf_len += l;
                    if (data)
                        data = (Byte *)data + l;
                    _pos += l;
        ...
        case FILE_MEM:
            if (_writable) {
                ...
                Ptr dest = memFast();
                if (_cipher)
                    _cipher->encrypt(dest, data, write, posCipher());
                else
                    Copy(dest, data, write);
                _pos += write;
                size -= write;
            }
            break;
        ...
        case FILE_MEMB: {
            Int dest_elms = posAbs() + size;
            if (dest_elms > _memb.elms())
                _memb.setNum(dest_elms); // Long not needed because is used for Memb based on 32-bits
            do {
                Int pos_abs = posAbs(),                     // Long not needed because later is used only as Int memb element index
                    elm = pos_abs & _memb.mask(),           // index of element in a block
                    l = Min(size, _memb.blockElms() - elm); // how much elements left in this block
                if (_cipher)
                    _cipher->encrypt(&_memb[pos_abs], data, l, posCipher());
                else
                    Copy(&_memb[pos_abs], data, l);
                if (data)
                    data = (Byte *)data + l;
                _pos += l;
                size -= l;
...
Bool File::getFast(Ptr data, Int size) {
    if (getReturnSize(data, size) == size)
        return true;
    error();
...
Bool File::get(Ptr data, Int size) {
    ...
    if (read == size)
        return true; // check this first because this is what's most likely going to happen
    if (data && size > read)
        ZeroFast((Byte *)data + read, size - read); // zero unread data, this is important because methods such as f.getInt, f.decUIntV, .. don't check for status, however they're expected to return zeros
    error();
    return false; // set error
}
...
File &File::putStr(CChar *t) {
    ...
    if (length) {
        if (wide) {
            ...
            Memt<Char8> temp;
            temp.setNum(length);
            Copy16To8(temp.data(), t, temp.elms());
...
File &File::putStr(C Str &s) // keep this function to allow having '\0' chars in the middle
{
    ...
    if (length) {
        if (wide) {
            ...
            Memt<Char8> temp;
            temp.setNum(length);
            Copy16To8(temp.data(), s(), temp.elms());
            putN(temp.data(), length);
        }
    }
    return T;
...
File &File::skipStr() {
    ...
    {
        error();
        pos(size()); // if the length was too long then go at the end of file, in case the user will try to read more data after this call, this is important so that the partially available string data is not treated as something else
    } else
        skip(length);
    return T;
}
...
File &File::getStr(Str8 &s) // warning: this must handle having '\0' chars in the middle
{
    ...
    {
        CHS(length);
        if (left() < length * SIZEI(Char))
            goto length_too_long;
        s.reserve(length);
        Memt<Char> temp;
        temp.setNum(length);
        getN(temp.data(), length);
...
File &File::getStr(Str &s) // warning: this must handle having '\0' chars in the middle
{
    ...
    if (length < 0) // wide
    {
        ...
        if (left() < length * SIZEI(Char8))
            goto length_too_long;
        s.reserve(length);
        Char8 *temp = (Char8 *)s._d.data();
        getN(temp, length);           // we can re-use the string memory because it uses Char which has 2x Char8 capacity
...
File &File::getStr(Char *t, Int t_elms) {
    ...
    if (length < 0) // wide
    {
        ...
        if (t && t_elms > 0) {
            Int read = Min(length, t_elms - 1);
            Char8 *temp = (Char8 *)t;
            getN(temp, read); // we can re-use the char array memory because it uses Char which has 2x Char8 capacity
...
Str File::getStr() {
    ...
    getStr(s);
    return s;
} // warning: this must handle having '\0' chars in the middle
/******************************************************************************/
...
File &File::putAsset(CChar *t) {
    if (!Is(t))
        ...
        UID id;
        if (DecodeFileName(t, id))
            putAsset(id);
        else
            putByte(0xFF).putStr(t);
    }
    return T;
}
UID File::getAssetID() {
...
Bool File::equal(File &f, Long max_size) {
    if (max_size < 0 || left() < max_size || f.left() < max_size) {
        if (left() != f.left())
            return false;
        max_size = left();
    } // if 'max_size' is not specified or any of the files has less than expected bytes, then continue only if both have same amount of bytes less, and test only those bytes
    if (max_size > 0) {
        Memt<Byte, TEMP_BUF_SIZE> temp;
        temp.setNum(TEMP_BUF_SIZE);
        Int buf_size = temp.elms() / 2;
        Ptr buf = temp.data(), buf2 = temp.data() + buf_size;
        for (; max_size > 0;) {
            Int l = Min(buf_size, max_size);
            max_size -= l;
            if (!getFast(buf, l) || !f.getFast(buf2, l))
                return false;
            if (!EqualMemFast(buf, buf2, l))
                return false;
        }
    }
    return true;
...
Bool File::copyEncrypt(File &dest, Cipher &cipher, Long max_size) {
    ...
    if (max_size > 0) {
        ...
        temp.setNum(TEMP_BUF_SIZE);
        Ptr buf = temp.data();
        Int buf_size = temp.elms(), offset = 0;
        REP(max_size / buf_size) {
            if (!getFast(buf, buf_size))
                return false;
            cipher.encrypt(buf, buf, buf_size, offset);
            if (!dest.put(buf, buf_size))
                return false;
            offset += buf_size;
        }
        buf_size = max_size % buf_size;
...
Bool File::discardBuf(Bool flush) {
    if (_buf_pos || _buf_len) // if there's any data in the buffer
    {
        ...
        if (flush && !T.flush() && ALLOW_REFLUSH)
            error();        // !! do this after remembering position because this method may change it !! if we had some data to save which failed, then only disable '_ok', but still proceed because here the priority is to set correct file position
...
/******************************************************************************/
void File::limit(ULong &total_size, ULong &applied_offset, Long new_size) {
    total_size = size();
    applied_offset = pos();
    T._size = new_size;
    T._offset += applied_offset;
    T._cipher_offset += applied_offset;
    T._pos -= applied_offset;
    T._full_size -= applied_offset;
}
void File::unlimit(ULong &total_size, ULong &applied_offset) {
    T._size = total_size;
    T._offset -= applied_offset;
    T._cipher_offset -= applied_offset;
    T._pos += applied_offset;
...
UInt File::crc32(Long max_size) {
    ...
    if (max_size < 0)
        max_size = left();
    for (;;) {
        Int left = Min(SIZEI(buf), max_size);
        if (left <= 0)
            break;
        if (!getFast(buf, left))
            return 0;
        hash.update(buf, left);
        max_size -= left;
    }
    return hash();
}
...
File &File::_getStr1(Str &s) // warning: this must handle having '\0' chars in the middle
{
    ...
    if (length < 0) // unicode
    {
        ...
        REP(length)
        s._d[i] = Char8To16(temp[i]); // need to process from the end to not overwrite the source
    }
...
File &File::_getStr1(Char *t, Int t_elms) {
    ...
    if (length < 0) // unicode
    {
        ...
        if (t && t_elms > 0) {
            ...
            REP(read)
            t[i] = Char8To16(temp[i]); // need to process from the end to not overwrite the source
            length -= read;
        }
    }
    skip(length);
