...
Str CurDir() {
    ...
    path[0] = 0;
    GetCurrentDirectory(Elms(path), path);
    return path;
#else
    Char8 path[MAX_UTF_PATH];
    path[0] = 0;
    getcwd(path, Elms(path));
...
Str MakeFullPath(C Str &path, FILE_PATH type, Bool keep_empty) {
    if (type == FILE_CUR || type == FILE_DATA) // do not adjust Android Assets because they're always accessed without any paths (just asset name)
        if (!keep_empty || path.is())
            if (!FullPath(path))
                return ((type == FILE_CUR) ? CurDir().tailSlash(true) : DataPath()) + path; // 'DataPath' already has 'tailSlash'
    return path;
}
/******************************************************************************/
#if !SWITCH
...
void FileInfo::from(WIN32_FIND_DATA &fd) {
    ...
    SYSTEMTIME sys_time;
    FileTimeToSystemTime(&fd.ftLastWriteTime, &sys_time);
    ...
    if (fd.dwFileAttributes & FILE_ATTRIBUTE_HIDDEN)
        attrib |= FATTRIB_HIDDEN;
    if (fd.dwFileAttributes & FILE_ATTRIBUTE_READONLY)
        attrib |= FATTRIB_READ_ONLY;
}
#endif
Bool FileInfo::getSystem(C Str &name) {
    if (name.is()) {
        if (IsDrive(name)) {
            ...
            GetDrives(drives);
            REPA(drives)
            if (EqualPath(drives[i].path, name)) {
                zero();
                type = FSTD_DRIVE;
                return true;
            }
        ...
        if (GetFileAttributesEx(name, GetFileExInfoStandard, &fad)) {
            ...
            SYSTEMTIME sys_time;
            FileTimeToSystemTime(&fad.ftLastWriteTime, &sys_time);
            modify_time_utc.year = sys_time.wYear;
            modify_time_utc.month = sys_time.wMonth;
            modify_time_utc.day = sys_time.wDay;
            modify_time_utc.hour = sys_time.wHour;
...
#else
        struct stat stats;
        if (!lstat(UnixPathUTF8(name), &stats)) {
            attrib = 0;

#ifdef UF_HIDDEN
            if (stats.st_flags & UF_HIDDEN)
                attrib |= FATTRIB_HIDDEN;
            else if (CChar *base = _GetBase(name))
                if (*base == '.')
                    attrib |= FATTRIB_HIDDEN;
#else
            if (CChar *base = _GetBase(name))
                if (*base == '.')
                    attrib |= FATTRIB_HIDDEN;
#endif
            if (!(stats.st_mode & S_IWUSR) && !(stats.st_mode & S_IWGRP) && !(stats.st_mode & S_IWOTH))
                attrib |= FATTRIB_READ_ONLY;
...
#if APPLE
            tm gmt;
            gmtime_r(&stats.st_mtimespec.tv_sec, &gmt);
#else
            tm gmt;
            gmtime_r(&stats.st_mtime, &gmt);
#endif
...
Bool FileInfo::get(C Str &name) {
    if (name.is()) {
        if (C PaksFile *psf = Paks.find(name)) {
            T = *psf->file;
            return true;
        }
        if (getSystem(name))
        ...
        if (DataPath().is() && !FullPath(name)) {
            Char full[MAX_LONG_PATH];
            MergePath(full, DataPath(), name);
            if (getSystem(full))
                return true;
        }
    }
...
Bool operator==(C FileInfo &f0, C FileInfo &f1) {
    if (f0.type == f1.type)
        ...
        case FSTD_LINK:
        case FSTD_FILE:
            return f0.size == f1.size && !CompareFile(f0.modify_time_utc, f1.modify_time_utc);
        }
    return false;
}
/******************************************************************************/
...
Bool FileFind::findValid(WIN32_FIND_DATA &fd) {
    for (; fd.cFileName[0];) {
        if (fd.cFileName[0] == '.')
            if (!fd.cFileName[1] || (fd.cFileName[1] == '.' && !fd.cFileName[2]))
                goto dot; // "." or ".."
...
void FileFind::find(C Str &path, C Str &ext) {
    ...
    if (_handle != INVALID_HANDLE_VALUE)
        if (findValid(fd))
            _state = FILE_WAITING;
#else
    if (_handle = opendir(_path.is() ? UnixPathUTF8(_path)() : "."))
        _state = NEED_CHECK;
#endif
}
...
void BackgroundFileFind::find(C Str &path, Bool (*filter)(C Str &name)) {
    ...
    _find_id++;
    if (!_thread.active())
        _thread.create(BackgroundFileFindThreadFunc, this);
}
/******************************************************************************/
...
Bool BackgroundFileFind::update() {
    for (; !_thread.wantStop();) {
        ...
        if (!_paths.elms())
            return false;
        Str path = _paths.pop();
        UInt find_id = T._find_id; // get unique id of the 'find' operation
        locker.off();
        ...
        for (FileFind ff(path); !_thread.wantStop() && ff();) {
            ...
            file.name = ff.pathName();
            SyncLockerEx locker(_lock);
            if (find_id == T._find_id) // check id's in case during this loop new 'find' was called
            {
                if (!_filter || _filter(file.name)) {
                    if (file.type == FSTD_DRIVE || file.type == FSTD_DIR)
                        new_paths.add(file.name); // add folder to paths list
...
static Bool FListEx(C Str &path, FILE_LIST_MODE func(C FileFind &ff, Ptr user), Ptr user) {
    for (FileFind ff(path); ff();)
        ...
        case FSTD_LINK:
        case FSTD_FILE:
            if (func(ff, user) == FILE_LIST_BREAK)
                return false;
            break;
        ...
        case FSTD_DIR: {
            ...
            case FILE_LIST_BREAK:
                return false;
            case FILE_LIST_CONTINUE:
                if (!FListEx(ff.pathName(), func, user))
                    return false;
                break;
            }
        } break;
...
Bool FEqual(C Str &a, C Str &b, Cipher *a_cipher, Cipher *b_cipher) {
    File fa, fb;
    if (fa.read(a, a_cipher) && fb.read(b, b_cipher))
        return fa.equal(fb);
    return false;
}
Bool FCopy(C Str &src, C Str &dest, FILE_OVERWRITE_MODE overwrite, Cipher *src_cipher, Cipher *dest_cipher, CChar *safe_overwrite_suffix) {
    if (EqualPath(src, dest))
        return src_cipher == dest_cipher;
    Str full_dest = MakeFullPath(dest); // we can't make 'src' to be full path because it can be path from 'DataPath' or 'Pak'
    if (EqualPath(src, full_dest))
        return src_cipher == dest_cipher;
    if (overwrite != FILE_OVERWRITE_NEVER || !FExistSystem(full_dest)) {
        File d, s;
        if (s.read(src, src_cipher)) {
            // get source info
            C PakFile *pf = null;
            FileInfo fi;
            if (s._pak)
                pf = s._pak->find(src, false);
            else
                fi.get(src);
            ...
            if (overwrite == FILE_OVERWRITE_DIFFERENT) {
                ...
                if (fi == FileInfoSystem(full_dest))
                    return true;
            }
            ...
            if (s._pak ? (pf && (pf->flag & PF_STD_LINK)) : (fi.type == FSTD_LINK)) // if this is a symbolic link
            {
                ...
                if (!CreateSymLink(full_dest, DecodeSymLink(s)))
                    return false;
            } else
#endif
                if (Is(safe_overwrite_suffix)) {
                if (!SafeOverwrite(s, full_dest, pf ? &pf->modify_time_utc : fi.type ? &fi.modify_time_utc
                                                                                     : null,
                                   dest_cipher, safe_overwrite_suffix))
                    return false;
            } else {
                if (!d.write(full_dest, dest_cipher) || !s.copy(d) || !d.flush())
                    return false;
                d.del(); // release handle so we can apply file params

                if (pf)
                    FTimeUTC(full_dest, pf->modify_time_utc);
                else if (fi.type)
                    FTimeUTC(full_dest, fi.modify_time_utc);
            }

            // apply params
            if (fi.type)
                FAttrib(full_dest, fi.attrib);
            return true;
        }
    }
    return false;
...
Bool FCopy(Pak &pak, C Str &src, C Str &dest, FILE_OVERWRITE_MODE overwrite, Cipher *dest_cipher, CChar *safe_overwrite_suffix) {
    if (C PakFile *pak_file = pak.find(src, false))
        return FCopy(pak, *pak_file, dest, overwrite, dest_cipher, safe_overwrite_suffix);
    return false;
}
...
Bool FCopyDir(C Str &src, C Str &dest, FILE_OVERWRITE_MODE overwrite, Cipher *src_cipher, Cipher *dest_cipher) {
    ...
    if (FCreateDirs(dest))
        for (FileFind ff(src); ff();) {
            Str name = dest;
            name.tailSlash(true) += ff.name;
            switch (ff.type) {
            case FSTD_LINK:
            case FSTD_FILE:
                if (overwrite != FILE_OVERWRITE_DIFFERENT || ff != FileInfoSystem(name))
                    ok &= FCopy(ff.pathName(), name, overwrite, src_cipher, dest_cipher);
                break;
            case FSTD_DIR:
                ok &= FCopyDir(ff.pathName(), name, overwrite, src_cipher, dest_cipher);
                break;
            }
        }
    else
...
#if WINDOWS
        UInt out = 0;
        if (attrib & FATTRIB_READ_ONLY)
            out |= FILE_ATTRIBUTE_READONLY;
        if (attrib & FATTRIB_HIDDEN)
...
#ifdef UF_HIDDEN
            UInt flags = stats.st_flags;
            FlagSet(flags, UF_HIDDEN, FlagOn(attrib, FATTRIB_HIDDEN));
...
#elif WINDOWS_NEW
        WIN32_FILE_ATTRIBUTE_DATA fad;
        return GetFileAttributesEx(name, GetFileExInfoStandard, &fad) != 0;
#else
        struct stat stats;
        return !lstat(UnixPathUTF8(name), &stats);
#endif
    }
    return false;
}
Bool FExist(C Str &name) {
    if (name.is()) {
        ...
        if (!full_path && Paks.find(name))
            return true;
        if (FExistSystem(name))
            return true;
        if (DataPath().is() && !full_path) {
            Char full[MAX_LONG_PATH];
            MergePath(full, DataPath(), name);
            if (FExistSystem(full))
                return true;
        }
    }
    return false;
...
Str FFirst(C Str &name, C Str &ext) {
    ...
    FREP(10000) {
        ...
        if (!FExistSystem(file))
            return file;
    }
    return S;
}
Str FFirstUp(C Str &name) {
    for (Str path = name; path.is(); path = GetPath(path))
...
Bool FRename(C Str &src, C Str &dest) {
    ...
    if (EqualPath(s, d) && Equal(GetBase(s), GetBase(d), true))
        return true; // allow changing case
#if WINDOWS
    ...
    {
        if (FSPathMoveObjectSync(UnixPathUTF8(s), UnixPathUTF8(GetPath(d)), CFStringAuto(UnixPath(GetBase(d))), null, kFSFileOperationOverwrite | kFSFileOperationSkipPreflight) == noErr)
            return true; // return only on success, if failed, then try methods below (this can fail for current app's exe file)
    }
#endif
...
#else
Bool FDelFile(C Str &name) {
    if (CHECK_NULL && !name.is())
        return false;
    if (!unlink(UnixPathUTF8(name))) {
        FlushIO();
        return true;
    }
    return false;
} // 'remove' will delete both files/dirs
Bool FCreateDir(C Str &name) {
    if (CHECK_NULL && !name.is())
        return false;
    if (!mkdir(UnixPathUTF8(name), S_IRWXU | S_IRWXG | S_IRWXO)) {
        FlushIO();
        return true;
    }
    return false;
}
Bool FDelDir(C Str &name) {
    if (CHECK_NULL && !name.is())
        return false;
    if (!rmdir(UnixPathUTF8(name))) {
        FlushIO();
        return true;
    }
    return false;
...
Bool FCreateDirs(C Str &name) {
    ...
    if (last_create)
        return true;                   // if last create succeeded
...
Bool FDelInside(C Str &name) {
    if (name.is()) {
        Bool ok = true;
        for (FileFind ff(name); ff();)
            ok &= FDel(ff); // keep removing other files even if one fails
        return ok;
    }
    return false;
}
...
Bool FRecycle(C Str &name, Bool hidden) {
    if (name.is()) {
        ...
        if (HasDrive(full)) {
            Char path[MAX_LONG_PATH + 1];
            path[SetReturnLength(path, full, Elms(path) - 1) + 1] = 0; // must be "double null terminated" - "\0\0"
            SHFILEOPSTRUCT sh;
            Zero(sh);
            sh.wFunc = FO_DELETE;
...
#elif MAC
        FSRef fs_ref;
        if (FSPathMakeRef((UInt8 *)(UnixPathUTF8(name)()), &fs_ref, null) == noErr)
            return FSMoveObjectToTrashSync(&fs_ref, null, 0) == noErr;
#elif LINUX
        Str full = MakeFullPath(name); // must be full path
        if (HasDrive(full)) {
            Str trash = SystemPath(SP_TRASH);
            if (trash.is()) {
                Str base = GetBase(full), first = FFirst(trash + "/files/" + base);
                if (first.is()) {
                    FileText f;
                    if (f.write(trash + "/info/" + GetBase(first) + ".trashinfo", UTF_8_NAKED)) // spec requires writing to "info" first, UTF is probably not supported
                    {
                        DateTime dt;
                        dt.getLocal();
...
static Bool FMoveDirEx(C Str &src, C Str &dest) {
    ...
    FCreateDir(dest);
    Str src_t = src;
    src_t.tailSlash(true);
    Str dest_t = dest;
    dest_t.tailSlash(true);
    for (FileFind ff(src); ff();)
        switch (ff.type) {
        case FSTD_LINK:
        case FSTD_FILE:
            ok &= FRename(src_t + ff.name, dest_t + ff.name);
            break;
        case FSTD_DIR:
            ok &= FMoveDirEx(src_t + ff.name, dest_t + ff.name);
            break;
        }
    ok &= FDelDir(src);
    return ok;
...
static Bool FReplaceDirEx(C Str &src, C Str &dest, FILE_OVERWRITE_MODE overwrite, Cipher *src_cipher, Cipher *dest_cipher) // !! assumes that 'src' and 'dest' have 'tailSlash(true)' !!
{
    ...
    for (FileFind ff(dest); ff();)
        if (FileInfoSystem(src + ff.name).type != ff.type)
            ok &= FDel(ff);
    ...
    for (FileFind ff(src); ff();)
        if (ff.type == FSTD_DIR)
            ok &= FReplaceDirEx(ff.pathName().tailSlash(true), dest + ff.name + '\\', overwrite, src_cipher, dest_cipher);
        else
            ok &= FCopy(ff.pathName(), dest + ff.name, overwrite, src_cipher, dest_cipher);
...
void GetDrives(MemPtr<Drive> drives) {
    ...
    if (drv & (1 << i)) {
        ...
        case DRIVE_FIXED:
            ...
            break;
        case DRIVE_CDROM:
            d.type = DRIVE_OPTICAL;
            break;
        case DRIVE_REMOVABLE:
            d.type = DRIVE_USB;
            break;
        default:
            d.type = DRIVE_UNDEFINED;
            break;
        }
        wchar_t name[MAX_LONG_PATH + 1];
        name[0] = 0;
        GetVolumeInformation(d.path, name, Elms(name), null, null, null, null, 0);
        d.name = name;
    }
#elif WINDOWS_NEW
    ...
    FREP(32) {
        ...
        HFSUniStr255 name;
        FSVolumeInfo info;
        if (FSGetVolumeInfo(kFSInvalidVolumeRefNum, 1 + i, &ref, kFSVolInfoFlags, &info, &name, null) != noErr)
            break;

        Bool read_only = ((info.flags & kFSVolFlagSoftwareLockedMask) != 0);
        Char8 url_path[MAX_UTF_PATH];
        CFURLRef url;
        FSCopyURLForVolume(ref, &url);
        ...
        d.path = Replace(FromUTF8(url_path), '/', '\\');
        d.path.tailSlash(true); // make sure slash is added to preserve consistency with Windows version
        FREP(name.length)
        d.name += (Char)name.unicode[i];
    }
#elif ANDROID
    {
        Drive &d = drives.New();
    ...
    d.type = DRIVE_DISK;
#else
    Memc<struct statfs> st;
    Int fs = getfsstat(null, 0, MNT_NOWAIT);
    st.setNum(Max(0, fs) + 16);
    fs = getfsstat(st.data(), st.elmsMem(), MNT_NOWAIT);
    FREP(fs)
    if (st[i].f_blocks && st[i].f_bsize) {
        Drive &d = drives.New();
        d.path = st[i].f_mntonname;
        d.path.tailSlash(true);
...
Bool GetDriveSize(C Str &path, Long *free, Long *total) {
#if WINDOWS
    ULARGE_INTEGER _free, _total;
    if (path.is() && GetDiskFreeSpaceEx(path, &_free, &_total, null)) {
        if (free)
            *free = _free.QuadPart;
        if (total)
    ...
    if (NSStringAuto _path = UnixPath(path))
        if (NSDictionary *dict = [[NSFileManager defaultManager] attributesOfFileSystemForPath:_path error:nil]) {
            if (free)
                *free = [[dict objectForKey:NSFileSystemFreeSize] longLongValue];
            if (total)
                *total = [[dict objectForKey:NSFileSystemSize] longLongValue];
    ...
    if (!statvfs(UnixPathUTF8(path), &stats)) { // !! need to use 'f_frsize' instead of 'f_bsize' because on Apple it won't return correct results !!
        if (free)
            *free = ULong(stats.f_frsize) * stats.f_bavail; // use 'f_bavail' instead of 'f_bfree' because that matches what File Managers report about being free on Mac/Linux (on iOS it makes no difference)
        if (total)
            *total = ULong(stats.f_frsize) * stats.f_blocks;
        return true;
    }
#endif
    if (free)
...
Bool SafeOverwrite(File &src, C Str &dest, C DateTime *modify_time_utc, Cipher *dest_cipher, C Str &suffix, ReadWriteSync *rws) {
    ...
    if (dest.is()) {
        ...
        if (rws && !suffix.is()) {
            rws->enterWrite();
            locked = true;
        } // if there's no suffix then it means we're writing directly to the target, so write lock needs to be enabled at start
        File f;
        if (f.write(temp, dest_cipher)) {
            ok = (src.copy(f) && f.flush());
            f.del(); // release file handle so we can set file time/params and rename/remove if needed
            if (ok) {
                if (modify_time_utc)
                    FTimeUTC(temp, *modify_time_utc);
                if (suffix.is()) {
                    if (rws) {
                        rws->enterWrite();
                        locked = true;
                    ...
                    if (!FRename(temp, dest)) {
                        ok = false;
                        UInt attr = FAttrib(dest);
                        if (attr & FATTRIB_READ_ONLY)
                            if (FAttrib(dest, attr & (~FATTRIB_READ_ONLY))) // if failed, then try clearing READ_ONLY attribute
                            {
            ...
            if (!ok)
                FDelFile(temp); // don't leave temp files
        }
        if (locked)
            rws->leaveWrite();
    }
...
Bool SafeOverwrite(FileText &src, C Str &dest, C DateTime *modify_time_utc, Cipher *dest_cipher, C Str &suffix, ReadWriteSync *rws) {
    ...
    if (src._f.pos(0))
        ok = SafeOverwrite(src._f, dest, modify_time_utc, dest_cipher, suffix, rws);
...
Str DecodeSymLink(File &f) {
    ...
    {
        Char8 s[MAX_UTF_PATH + 1];
        s[f.getReturnSize(s, MAX_UTF_PATH)] = '\0';
        return FromUTF8(s);
    }
    return S;
}
