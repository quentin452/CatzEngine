...
void AreaPath2D::create(Int size) {
    PROFILE_START("CatzEngine::AreaPath2D::create(Int size)")
    _map.createSoft(size, size, 1, IMAGE_I8);
    _map.clear();
    group();
    PROFILE_STOP("CatzEngine::AreaPath2D::create(Int size)")
}
void AreaPath2D::createFromQuarter(AreaPath2D &src, Bool right, Bool forward, WorldSettings &settings) {
    PROFILE_START("CatzEngine::AreaPath2D::createFromQuarter(AreaPath2D &src, Bool right, Bool forward, WorldSettings &settings)")
    ...
    group();
    PROFILE_STOP("CatzEngine::AreaPath2D::createFromQuarter(AreaPath2D &src, Bool right, Bool forward, WorldSettings &settings)")
}
void AreaPath2D::createFromQuad(AreaPath2D *lb, AreaPath2D *rb, AreaPath2D *lf, AreaPath2D *rf, WorldSettings &settings) {
    PROFILE_START("CatzEngine::AreaPath2D::createFromQuad(AreaPath2D *lb, AreaPath2D *rb, AreaPath2D *lf, AreaPath2D *rf, WorldSettings &settings)")
    ...
    group();
    PROFILE_STOP("CatzEngine::AreaPath2D::createFromQuad(AreaPath2D *lb, AreaPath2D *rb, AreaPath2D *lf, AreaPath2D *rf, WorldSettings &settings)")
}
/******************************************************************************/
...
AreaPath2D &AreaPath2D::walkable(Int x, Int y, Bool walkable) {
    PROFILE_START("CatzEngine::AreaPath2D::walkable(Int x, Int y, Bool walkable)")
    if (InRange(x, _map.w()) && InRange(y, _map.h())) {
        _map.pixB(x, y) = (walkable ? 0xFE : 0xFF);
        _changed = true;
    }
    PROFILE_STOP("CatzEngine::AreaPath2D::walkable(Int x, Int y, Bool walkable)")
    return T;
}
/******************************************************************************/
Bool AreaPath2D::fullyWalkable() {
    PROFILE_START("CatzEngine::AreaPath2D::fullyWalkable()")
    REPD(y, _map.h())
    REPD(x, _map.w())
    if (!walkable(x, y)) {
        PROFILE_STOP("CatzEngine::AreaPath2D::fullyWalkable()")
        return false;
    }
    PROFILE_STOP("CatzEngine::AreaPath2D::fullyWalkable()")
    return true;
...
void AreaPath2D::group() {
    PROFILE_START("CatzEngine::AreaPath2D::group()")
    // set groups
    {
#define BLOCKED 0xFFFF
        ...
        REPD(y, _map.h())
        REPD(x, _map.w())
        path[y][x] = (walkable(x, y) ? groups++ : BLOCKED);
...
#if 1
                    if (right || up)
                        if (InRange(r, _map.w()) && InRange(u, _map.h())) {
                            UInt n = path[u][r]; // neighbor
                            if (n != BLOCKED && n != g && new_group[n] == n && !parent[n]) {
                                new_group[n] = g;
                                parent[g] = true;
                            } // link
                        }
        ...
        T._groups = 0;
        REP(groups)
        new_group[i] = BLOCKED; // set not yet added to target 'path' map
        REPD(y, _map.h())
        REPD(x, _map.w()) {
            UInt g = path[y][x];
            if (g != BLOCKED) {
    ...
    REPD(x, _map.w()) {
        ...
        if (a != 0xFF) {
            REPA(offset) {
                Int sx = x + offset[i].x,
                    sy = y + offset[i].y;
                if (InRange(sx, _map.w()) && InRange(sy, _map.h())) {
                    ...
                    if (b != 0xFF && a != b) {
                        ...
                        {
                            // if blocked diagonal
                            if (!walkable(sx, y) && !walkable(x, sy))
                                continue; // don't add
    ...
    _changed = false;
    PROFILE_STOP("CatzEngine::AreaPath2D::group()")
}
/******************************************************************************/
void AreaPath2D::resize(Int size) {
    PROFILE_START("CatzEngine::AreaPath2D::resize(Int size)")
    _map.resize(size, size, FILTER_NONE);
    group();
    PROFILE_STOP("CatzEngine::AreaPath2D::resize(Int size)")
}
/******************************************************************************/
...
Bool Area::Data::save(File &f) {
    ChunkWriter cw(f);
    if (height.is() || material_map.is() || materials.elms())
        if (File *f = cw.beginChunk("Heightmap", 0)) {
            if (!height.save(*f))
                return false;
            if (!material_map.save(*f))
                return false;
            f->putInt(materials.elms());
            REPA(materials)
            f->_putStr(materials[i].name());
        }
    if (mesh.is())
        if (File *f = cw.beginChunk("Mesh", 0))
            if (!mesh.save(*f))
                return false;
    if (phys.is())
        if (File *f = cw.beginChunk("Phys", 0))
            if (!phys.save(*f))
                return false;
    if (objs.elms())
    ...
    if (mesh_overlays.elms())
        if (File *f = cw.beginChunk("Mesh Overlay", 0))
            if (!mesh_overlays._save(*f))
                return false;
    if (_path_mesh)
        if (File *f = cw.beginChunk("PathMesh", 0))
            if (!_path_mesh->save(*f))
                return false;
    if (_path2D)
        if (File *f = cw.beginChunk("AreaPath", 0))
            if (!_path2D->save(*f))
                return false;
    if (waters.elms())
...
Bool Area::Data::load(File &f) {
    for (ChunkReader cr(f); File *f = cr();) {
        if (cr.name() == "HeightmapMesh")
        ...
        else if (cr.name() == "HeightmapPhys")
            switch (cr.ver()) {
            case 0:
                phys.loadAdd(*f);
                break;
            }
        else if (cr.name() == "ObjectMesh")
            switch (cr.ver()) {
            case 0:
                mesh.loadAdd(*f);
                break;
            }
        else if (cr.name() == "ObjectPhys")
            switch (cr.ver()) {
            case 0:
                phys.loadAdd(*f);
                break;
            }
        else if (cr.name() == "Heightmap")
            switch (cr.ver()) {
            case 0:
                if (!height.load(*f))
                    break;
                if (!material_map.load(*f))
                    break;
                materials.setNum(f->getInt());
                REPAO(materials) = f->_getStr();
                break;
            }
        else if (cr.name() == "Mesh")
            switch (cr.ver()) {
            case 0:
                mesh.loadAdd(*f);
                break;
            }
        else if (cr.name() == "Phys")
            switch (cr.ver()) {
            case 0:
                phys.loadAdd(*f);
                break;
            ...
            case 0:
                for (; !f->end();)
                    if (!objs.New().loadOld(*f)) {
                        objs.removeLast();
                        break;
                    }
                break;
            }
        else if (cr.name() == "Decal")
            switch (cr.ver()) {
            case 0:
                decals._load(*f);
                break;
            }
        else if (cr.name() == "Mesh Overlay")
            switch (cr.ver()) {
            case 0:
                mesh_overlays._load(*f);
                break;
            }
        else if (cr.name() == "PathMesh")
            switch (cr.ver()) {
            case 0:
                New(Delete(_path_mesh))->load(*f);
                break;
            }
        else if (cr.name() == "AreaPath")
            switch (cr.ver()) {
            case 0:
                New(Delete(_path2D))->load(*f);
                break;
            }
        else if (cr.name() == "Water")
            switch (cr.ver()) {
            case 0:
                waters._load(*f);
                break;
    ...
    Box terrain_objs_box;
    terrain_objs_box.zero();
    Bool have_terrain_box = false;
    Box foliage_objs_box;
    foliage_objs_box.zero();
    Bool have_foliage_box = false;
    terrain_objs.clear();
    foliage_objs.clear();
    FREPA(objs) // add in order
    {
        ...
        case OBJ_ACCESS_TERRAIN:
            if (obj.base()) {
                ...
                if (ter_obj.obj->mesh()) {
                    ...
                    if (have_terrain_box)
                        terrain_objs_box |= box;
                    else {
                        terrain_objs_box = box;
                        have_terrain_box = true;
                    }
                }
            }
            break;
        ...
        case OBJ_ACCESS_GRASS: {
            ...
            GrassObj *foliage = null;
            REPA(foliage_objs) {
                GrassObj &fo = foliage_objs[i];
                if (fo.mesh == obj.mesh() && fo.phys == obj.phys() && fo.mesh_variation == mesh_variation) {
                    foliage = &fo;
                    break;
                }
            }
            ...
            if (obj.mesh()) {
                ...
                if (have_foliage_box)
                    foliage_objs_box |= box;
                else {
                    foliage_objs_box = box;
                    have_foliage_box = true;
                }
            }
        } break;
        }
...
Flt Area::hmHeight(C Vec2 &xz, Bool smooth) C {
    PROFILE_START("CatzEngine::Area::hmHeight(C Vec2 &xz, Bool smooth)")
    if (_data) {
        ...
        if (height.is()) {
            ...
            PROFILE_STOP("CatzEngine::Area::hmHeight(C Vec2 &xz, Bool smooth)")
...
C MaterialPtr &Area::hmMaterial(C Vec2 &xz) C {
    PROFILE_START("CatzEngine::Area::hmMaterial(C Vec2 &xz)")
    if (_data) {
        ...
        if (map.is()) {
            ...
            if (InRange(index, _data->materials)) {
                PROFILE_STOP("CatzEngine::Area::hmMaterial(C Vec2 &xz)")
...
Area::Data *Area::getData() {
    if (state() != AREA_UNLOADED)
        if (!data())
            world()->_area_data(_data, T); // first check state, then data (in case the Area is being loaded in background thread)
    return data();
}
/******************************************************************************/
void Area::setShader() {
...
Bool Area::saveObjs() {
    ...
    FREPA(_objs) {
        Obj &obj = *_objs[i];
        if (!obj.isConst() && obj.canBeSaved())
            if (!saveObj(obj))
                return false; // don't save constant objects, and save only those which want to be saved
    }
    return true;
}
