...
{
    PROFILE_START("CatzEngine::Chr::actionBreak()")
    action = ACTION_NONE;
    path.clear();
    ...
    input.move.zero();
    input.turn.zero();
    PROFILE_STOP("CatzEngine::Chr::actionBreak()")
}
/******************************************************************************/
Bool Chr::actionMoveTo(C Vec &pos) // set 'move to' action
{
    PROFILE_START("CatzEngine::Chr::actionMoveTo(C Vec &pos)")
    ...
    if (World.path().find(T.pos(), path_target, path)) // if found a path to the target destination
    {
        ...
        PROFILE_STOP("CatzEngine::Chr::actionMoveTo(C Vec &pos)")
        return true;
    }
    PROFILE_STOP("CatzEngine::Chr::actionMoveTo(C Vec &pos)")
    return false;
...
{
    PROFILE_START("CatzEngine::Chr::actionMoveDir(C Vec &dir)")
    // break any current action
    actionBreak();
    ...
    move_dir = dir.xz();
    PROFILE_STOP("CatzEngine::Chr::actionMoveDir(C Vec &dir)")
}
/******************************************************************************/
...
static Bool InputToAngle(Chr &chr, Flt from, Flt to, Flt turn_speed, Flt desired_speed) {
    ...
    Flt d = AngleDelta(from, to),
        ad = Abs(d);
    if (ad < 0.001f)
        chr.input_turn.x = chr.input.turn.x = 0;
    else {
    ...
    if (desired_speed > EPS)
        limit /= Max(1, desired_speed / 4.1f);
    if (turn_speed > EPS)
    ...
    for (;;) {
        if (!chr.path.elms()) // if don't have a waypoint
        {
            if (!path_searched) // if didn't yet searched
            {
                ...
                if (World.path().find(from, chr.path_target, chr.path))
                    chr.path.reverseOrder(); // search the path
            }
...
void Chr::updateAction() {
    PROFILE_START("CatzEngine::Chr::updateAction()")
    ...
    // update character according to input
    {
        ...
        dodge_availability -= Time.d();
        if (input.dodge && !ctrl.crouched() && ctrl.onGround() && dodge_availability <= 0) {
            dodging = input.dodge;
            dodge_step = 1;
            dodge_availability = 0.5f;
        }
    }
    PROFILE_STOP("CatzEngine::Chr::updateAction()")
}
/******************************************************************************/
void Chr::updateController() {
    PROFILE_START("CatzEngine::Chr::updateController()")
    // set movement velocity
    Vec velocity;
    if (dodging) // set according to dodging
    {
        velocity.y = 0;
        CosSin(velocity.x, velocity.z, angle.x);
        if (dodging < 0)
            velocity.chs();
        velocity.setLength(speed * 2.5f * Pow(dodge_step, 0.25f));
        dodge_step -= Time.d() * 3.33f;
        if (dodge_step <= 0)
            dodging = 0;
    } else // set according to the input
    {
        ...
        Flt length = 1;
        if (dir.z < 0)
            length = Lerp(Lerp(1.0f, 0.75f, anim.walk_run), 1.0f, dir.z + 1); // if dir.z is in range -1..0
    ...
    PROFILE_STOP("CatzEngine::Chr::updateController()")
}
/******************************************************************************/
} // namespace Game
} // namespace EE
