...
static SkelAnim *GetAnim(AnimatedSkeleton &anim_skel, C ObjectPtr &obj, CChar8 *anim) {
    if (obj)
        if (Param *param = obj->findParam(anim))
...
void Chr::animate() {
    ...
    if (ctrl.onGround() && anim.stop_move > 0.25f) {
        ...
        if (b_crc_l > blend) {
            blend = b_crc_l;
            skel_anim = sac.crouch_l;
        }
        if (b_crc_r > blend) {
            blend = b_crc_r;
            skel_anim = sac.crouch_r;
        }
        if (b_walk_l > blend) {
            blend = b_walk_l;
            skel_anim = sac.strafe_wl;
        }
        if (b_walk_r > blend) {
            blend = b_walk_r;
            skel_anim = sac.strafe_wr;
        }
        if (b_run_l > blend) {
            blend = b_run_l;
            skel_anim = sac.strafe_l;
        }
        if (b_run_r > blend) {
            blend = b_run_r;
            skel_anim = sac.strafe_r;
        }

        // check if "step" event occurred
        if (skel_anim && blend > 0.25f) {
            Flt dt = Time.d() * anim.speed;
            if (skel_anim->eventOccurred("step", anim.time - dt, dt))
                animateStepOccurred(); // anim.time was already updated before this method was called so we need to set it back (-dt)
        }
    }
...
void Chr::updateAnimation() {
        ...
        && anim.stand_crouch > EPS_ANIM_BLEND) // crouching
    {
        ...
        skel.clear();
        skel.animate(sac.stand, time, 1 - anim.stand_crouch).updateMatrixParents(scale, sac.toe_l);
        foot_offset = pos_foot * skel.bones[sac.toe_l].matrix();
        skel.animate(sac.crouch, time, anim.stand_crouch).updateMatrixParents(scale, sac.toe_l);
        foot_offset = foot_offset - pos_foot * skel.bones[sac.toe_l].matrix();
        foot_offset.y = 0;
    }
        ...
        head_neck_pitch = body_pitch_part + anim.fly * anim.fly_z * 0.5f * body_sign; // add flying adjustment split into 2 bones
    {
        ...
        // body
        v = skel.root.orn.cross();
        if (v.allZero())
            v.x = 1;
        CHS(v.z); // body x-axis
        if (sac.body >= 0) {
            m.setRotate(v, body_pitch_part * -body_sign).rotateY(body_roll);
            skel.bones[sac.body].orn *= m;
        }
        ...
        if (sac.head >= 0) {
            ...
            if (sac.body_u >= 0) {
                m.setRotate(v, body_pitch_part).rotateZ(body_roll);
                skel.bones[sac.body_u].orn *= m;
            }

            // neck + head
            m.setRotate(v, head_neck_pitch).rotateZ(head_neck_roll);
            if (sac.neck >= 0)
                skel.bones[sac.neck].orn *= m;
    ...
    Matrix m;
    {
        ...
        // orientation
        m.orn().setRotateY(anim.fly * anim.fly_turn + anim.strafe_yaw * PI_4).rotateZ(anim.lean * (1 - anim.straight_strafe) + anim.fly * (anim.fly_x - anim.fly_strafe)).rotateX(anim.lean * (anim.straight_strafe) * (anim.left_right * 2 - 1) + anim.fly * (anim.fly_z - anim.fly_full_body * angle.y) + body_pitch_rest * -body_sign).rotateY(-angle.x);
        ...
        if (anim.fly > EPS_ANIM_BLEND) {
            ...
            if (angle > EPS) {
                ...
                if (sac.leg_lu >= 0)
                    skel.bones[sac.leg_lu].rot.axis += axis;
                if (sac.leg_ru >= 0)
                    skel.bones[sac.leg_ru].rot.axis += axis;
                if (axis.x > EPS) {
                    if (sac.leg_ld >= 0)
                        skel.bones[sac.leg_ld].rot.axis.x += axis.x;
                    if (sac.leg_rd >= 0)
                        skel.bones[sac.leg_rd].rot.axis.x += axis.x;
                }
            }
        }
#endif
