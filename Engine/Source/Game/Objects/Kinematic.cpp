...
void Kinematic::setUnsavedParams() {
    ...
    if (base && base->phys())
        actor.create(*base->phys(), 1, scale, true)
            .matrix(_matrix)
            .obj(this)
            .group(AG_TERRAIN);
}
void Kinematic::create(Object &obj) {
    ...
    _matrix_scaled.scaleOrnL(scale);
...
void Kinematic::pos(C Vec &pos) {
    actor.pos(pos);
    T._matrix_scaled.pos = T._matrix.pos = pos;
}
void Kinematic::kinematicMoveTo(C Vec &pos) {
    actor.kinematicMoveTo(pos);
    T._matrix_scaled.pos = T._matrix.pos = pos;
}
void Kinematic::matrix(C Matrix &matrix) {
    actor.matrix(matrix);
    T._matrix_scaled = T._matrix = matrix;
    T._matrix_scaled.scaleOrnL(scale);
}
void Kinematic::kinematicMoveTo(C Matrix &matrix) {
