...
void ObjLightCone::create(Object &obj) {
    ...
    if (Param *param = obj.findParam("angle"))
        angle = Max(param->asFlt(), 0.0f);
    if (Param *param = obj.findParam("falloff"))
        falloff = Sat(param->asFlt());
...
void ObjLightCone::matrix(C Matrix &matrix) {
    position = matrix.pos;
    direction = matrix.z;
}
/******************************************************************************/
