        ...
        if (Cell<Area> *start_g = World._grid.find(start_pn.area.xy))
            if (Cell<Area> *target_g = ((start_pn.area.xy == target_pn.area.xy) ? start_g : World._grid.find(target_pn.area.xy))) {
                ...
                // set path map
                {
                    ...
                    if (start_g != target_g) {
                        SetWalkable(target_path, target_ofs_x, target_ofs_y);
                        if (start_g->x() != target_g->x() && start_g->y() != target_g->y()) // diagonal
                        {
                            Cell<Area> *cell;
                            AreaPath2D *path;

                            path = null;
                            if (cell = World._grid.find(target_g->xy()))
                                if (cell->data()->loaded() && cell->data()->_data)
                                    path = cell->data()->_data->path2D();
                            if (path)
                                SetWalkable(*path, target_ofs_x, start_ofs_y);
                            else
                                SetNonwalkable(target_ofs_x, start_ofs_y);
                            path = null;
                            if (cell = World._grid.find(start_g->xy()))
                                if (cell->data()->loaded() && cell->data()->_data)
                                    path = cell->data()->_data->path2D();
                            if (path)
                                SetWalkable(*path, start_ofs_x, target_ofs_y);
                            else
                                SetNonwalkable(start_ofs_x, target_ofs_y);
                        }
                    }

                    // include path find target flag
                    if (InRange(target_xy.x, World.settings().path2DRes()) && InRange(target_xy.y, World.settings().path2DRes()))
                        World._path_find.pixel(target_ofs_x + target_xy.x, target_ofs_y + target_xy.y).flag |= PFP_TARGET;
                    else // we have precise target position
                    {
...
Bool Path2DWalker::find(C Vec &start, C Vec &target, C Vec *alternate_start) {
    ...
    if (start_node < 0 && alternate_start)
        start_node = World.pathGetNode(*alternate_start, start_xy); // if current start position is invalid try using last known position (if given)
    if (start_node >= 0) {
