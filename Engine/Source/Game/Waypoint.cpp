...
Flt Waypoint::length() C {
    ...
    switch (loop_mode) {
    case LOOP:
        return points.last().total_length + Dist(points.first().pos, points.last().pos);
    case PING_PONG:
        return points.last().total_length * 2;
    default:
        return points.last().total_length; // SINGLE
    }
}
/******************************************************************************/
Vec Waypoint::pos(Flt x, Bool smooth) C {
    ...
    default: {
        ...
        case LOOP: {
            ...
            // binary search
            Int l = 0, r = points.elms();
            for (; l < r;) {
                Int mid = UInt(l + r) / 2;
                if (x < points[mid].total_length)
                    r = mid;
                else
                    l = mid + 1;
            }
            if (l != points.elms()) {
                C Point &from = points[l - 1],
                        &to = points[l];
                Flt dl = to.total_length - from.total_length;
                if (!dl)
                    return from.pos;
                Flt s = (x - from.total_length) / dl;
                if (!smooth)
                    return Lerp(from.pos, to.pos, s);
...
/******************************************************************************/
Waypoint &Waypoint::rol() {
    points.rotateOrder(-1);
    return updateTotalLengths();
}
Waypoint &Waypoint::ror() {
    points.rotateOrder(1);
    return updateTotalLengths();
}
...
void Waypoint::draw(C Color &point_color, C Color &edge_color, Flt point_radius, Int edge_steps) C {
    if (edge_color.a) {
        ...
        REP(points.elms() - 1) {
            ...
            if (edge_steps > 1) {
                Vec cur = b.pos;
                const Flt s = (b.total_length - a.total_length) * step;
                REP(edge_steps) {
        ...
        if (loop_mode == LOOP && points.elms() >= 3) {
            ...
            if (edge_steps > 1) {
                Vec cur = b.pos;
                const Flt s = (length() - a.total_length) * step;
                REP(edge_steps) {
                    Vec next = pos(a.total_length + i * s, true);
                    VI.line(cur - up, next - up);
                    VI.line(cur + up, next + up);
    ...
    if (point_color.a && point_radius > EPS) {
        VI.color(point_color);
        REPA(points)
        DrawPoint(points[i].pos, point_radius);
        VI.end();
    }
}
