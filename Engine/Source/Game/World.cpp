...
Bool WorldSettings::compatible(C WorldSettings &settings) C {
    return Equal(areaSize(), settings.areaSize()) && hmRes() == settings.hmRes() && path2DRes() == settings.path2DRes();
}
...
Bool WorldSettings::load(File &f, CChar *path) {
    ...
    case 1: {
        f >> _area_size >> _hm_res >> _path2d_res;
        environment.require(f._getStr(), path);
        if (f.ok())
            return true;
    } break;
...
void WorldManager::ObjContainer::set(ObjMap<Obj> *map, CPtr type) {
    ...
    {
        if (T.map)
            T.map->clear(); // delete objects, use 'clear' in case there are any 'Reference' objects left pointing to memory adresses inside containers
        T.map = map;
    }
    T.type = type;
}
...
WorldManager &WorldManager::del() {
    if (is()) {
        ...
        _area_active.del();
        _area_draw.del();
        _area_draw_shadow.del();
        _area_draw_secondary.del();
        _area_inactive.del();
        _area_cache.del();
...
Bool WorldManager::NewTry(C Str &world_name) {
    ...
    if (FExist(world_name)) {
        if (!DecodeFileName(world_name, _id))
            _id.zero();
        T._name = world_name;
        T._data_path = world_name;
        _data_path.tailSlash(true);

        // load settings
        {
            _settings.reset();
            File f;
            if (!f.read(dataPath() + "Settings") || !_settings.load(f))
                goto error;
            setRanges(); // after loading settings we need to update the ranges, because they're dependent on world area size
        }

        // path finder
        if (!_path.create(_settings.areaSize()))
            Exit("Can't initialize PathWorld");
...
AREA_STATE WorldManager::rangeState(C VecI2 &xzi) {
    ...
    case WORLD_STREAM: {
        ...
        if (dist <= _rangei2)
            return AREA_ACTIVE;
        if (dist <= _rangei2_inactive)
            return AREA_INACTIVE;
        if (dist <= _rangei2_cache)
            return AREA_CACHE;
...
Flt WorldManager::hmHeight(C Vec2 &xz, Bool smooth) {
    if (Area *area = areaLoaded(worldToArea(xz)))
        return area->hmHeight(xz, smooth);
    return 0;
}
C MaterialPtr &WorldManager::hmMaterial(C Vec2 &xz) {
    if (Area *area = areaLoaded(worldToArea(xz)))
        return area->hmMaterial(xz);
    return MaterialNull;
}
    ...
    if (area.state()) {
        ...
        if (area.data()) {
            ...
            if (area.data()->customSaveWant()) {
                File f;
                if (area.data()->save(f.writeMem())) {
                    Str path = dataPath() + "Area/";
                    FCreateDirs(path);
                    f.pos(0);
                    SafeOverwrite(f, path + area.xz(), null, area_cipher);
                }
            }
            Delete(area._data);
        }
        ...
        switch (area.state()) {
        case AREA_ACTIVE:
            _area_active.exclude(&area);
            break;
        case AREA_INACTIVE:
            _area_inactive.exclude(&area);
            break;
        case AREA_CACHE:
            _area_cache.exclude(&area);
            break;
        }
    ...
    if (area.loaded()) {
        ...
        if (area.loaded()) {
            if (Area::Data *data = area._data) {
                ...
                REPA(data->foliage_objs) {
                    Area::Data::GrassObj &go = data->foliage_objs[i];
                    REPAO(go.instances).actor.del();
                }
            }
            area.saveObjs();
        }
...
void WorldManager::areaCache(Area &area, Bool inc_progress, File &file_area) {
    if (area.state() == AREA_UNLOADED) {
        if (file_area.read(dataPath() + "Area/" + area.xz(), area_cipher)) {
            Area::Data *data;
            _area_data(data, area).load(file_area); // first create
...
void WorldManager::areaLoad(Area &area, Bool active, File &file_area) {
    if (area.state() == AREA_UNLOADED || area.state() == AREA_CACHE) {
        ...
        if (area.state() == AREA_UNLOADED) {
            if (file_area.read(dataPath() + "Area/" + area.xz(), area_cipher))
                _area_data(area._data, area).load(file_area);
        }
        ...
        // objects
        {
                ...
                REPA(objs) {
                    ...
                    case OBJ_ACCESS_CUSTOM: {
                        if (!area._visited || obj.constant()) // non-constant objects should be created only the first time we're entering an area, since then they are stored in savegame
                            if (!loadObj(area, active, obj, obj.constant())) {
                            } // don't break since objects are stored in container and can proceed
                    } break;
                    }
                }
            }
            ...
            // temporary/save data
            {
                File &f = area._saved_obj;
                f.pos(0);
                ChunkReader cr;
                if (cr.read(f))
                    for (; File *obj_file = cr();) {
                        switch (cr.ver()) {
                        case 0:
                            loadObj(area, active, ObjType.find(cr.name()), *obj_file);
                            break;
                        case 1: {
                            UID id;
                            if (DecodeRaw(cr.name(), id))
                                loadObj(area, active, ObjType.find(id), *obj_file);
                        } break;
                        }
                    }
                f.reset(); // zero data
        ...
        // finalize
        if (area.state() == AREA_CACHE)
            _area_cache.exclude(&area);
        if (active)
            _area_active.add(&area);
        else
            _area_inactive.add(&area);
...
void WorldManager::linkReferences() {
    ...
    REPA(_area_active) {
        ...
        REPAO(objs)->linkReferences();
    }
    REPA(_area_inactive) {
        Memc<Obj *> &objs = _area_inactive[i]->_objs;
        REPAO(objs)->linkReferences();
    }
...
struct RectSetter {
    ...
    void includeActive(C VecI2 &xy) {
        if (active_set)
            active_rect->validInclude(xy);
        else {
            active_set = true;
            *active_rect = xy;
        }
    }
    void includeLoaded(C VecI2 &xy) {
        if (loaded_set)
            loaded_rect->validInclude(xy);
        else {
            loaded_set = true;
            *loaded_rect = xy;
        }
    ...
    RectSetter(RectI &active_rect, RectI &loaded_rect) {
        active_set = loaded_set = false;
        T.active_rect = &active_rect.zero();
        T.loaded_rect = &loaded_rect.zero();
    }

    static void Func(Cell<Area> &cell, RectSetter &rs) {
        if (cell().state() == AREA_ACTIVE)
            rs.includeActive(cell.xy());
        if (cell().state() != AREA_UNLOADED)
            rs.includeLoaded(cell.xy());
    }
};
void WorldManager::areaSetLoadedRect() {
    ...
    case WORLD_MANUAL: {
        RectSetter rs(_area_active_rect, _area_loaded_rect);
        _grid.func(RectSetter::Func, rs);
    } break;
    }
...
void WorldManager::areaUpdateState() {
    if (_mode != WORLD_MANUAL) {
        ...
        if (_mode == WORLD_FULL)
            ...
            _grid.funcCreate(rect, TestState, T);
        ...
        else // process all areas for 'WORLD_FULL'
        {
            Bool delay_remove = (_areas_to_load && !low_memory_usage); // if want to load something and we don't want low memory usage
            if (delay_remove)
                DelayRemoveInc();
            REPA(_area_active)
            areaUpdateState(*_area_active[i], file_area); // order is important
            REPA(_area_inactive)
            areaUpdateState(*_area_inactive[i], file_area); // order is important
            REPA(_area_cache)
            areaUpdateState(*_area_cache[i], file_area); // order is important
...
void WorldManager::areaSetState(C CMemPtr<AreaState> &area_states, Bool unload_remaining) {
    if (_mode == WORLD_MANUAL) {
        ...
        {
            C AreaState &area_state = area_states[i];
            Area &area = *_grid.get(area_state.xz).data();
            if (unload_remaining)
                area._temp = true; // if want to unload remaining, then mark this area as "manually processed"
            areaUpdateState(area, area_state.state, file_area);
            if (use_background_loading && _thread.created() && area_state.state == AREA_CACHE && area.state() == AREA_UNLOADED)
                _area_background.add(&area); // if after update is still unloaded, but wants to be cached then add to background list
        }

        // unload remaining
        if (unload_remaining)
            _grid.func(AreaUnloadRemaining, T);
...
void WorldManager::update(C Vec2 &xz) {
    if (is()) {
                    ...
                    if (DataPath().is())
                        for (FileFind ff(DataPath() + dataPath() + "Area"); ff();)
                            if (ff.type == FSTD_FILE)
                                _grid.get(TextVecI2(ff.name)); // DataPath+Std
                }
                for (FileFind ff(dataPath() + "Area"); ff();)
                    if (ff.type == FSTD_FILE)
                        _grid.get(TextVecI2(ff.name)); //          Std
                areaUpdateState();
            }
        } break;
        ...
        // update objects
        if (update_objects_after_physics == false)
            updateObjects();
        ...
        // update objects
        if (update_objects_after_physics == true)
            updateObjects();
...
Bool WorldManager::load(File &f) {
    ...
    if (NewTry(world_name)) {
        ...
        for (;;) {
            VecI2 xz;
            f >> xz;
            if (xz.x == SIGN_BIT && xz.y == SIGN_BIT)
                break; // end marker
...
WorldManager::~WorldManager() {
    ...
    _lock.on();
    _area_background.clear(); // clear areas for load, so the thread won't process them
    _lock.off();
