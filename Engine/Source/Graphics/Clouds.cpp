...
LayeredClouds::LayeredClouds() {
    ...
    layer[0].color_l = 1;
    layer[0].scale = 1.0f / 2.8f;
    layer[0].position.set(0.00f, 0.0f);
    layer[0].velocity = 0.010f;
    layer[1].color_l = 1;
    layer[1].scale = 1.0f / 2.4f;
    layer[1].position.set(0.25f, 0.2f);
    layer[1].velocity = 0.008f;
    layer[2].color_l = 1;
    layer[2].scale = 1.0f / 2.0f;
    layer[2].position.set(0.50f, 0.1f);
    layer[2].velocity = 0.006f;
    layer[3].color_l = 1;
    layer[3].scale = 1.0f / 1.6f;
    layer[3].position.set(0.75f, 0.8f);
    layer[3].velocity = 0.004f;
    frac(0.9f);
...
LayeredClouds &LayeredClouds::scaleY(Flt scale) {
    Clamp(scale, 1, 2);
    if (T._scale_y != scale)
        SPSet("LCScaleY", T._scale_y = scale);
    return T;
}
/******************************************************************************/
void LayeredClouds::update() {
    REP(_layers)
    layer[i].position += layer[i].velocity * Time.d();
}
#pragma pack(push, 4)
...
inline Shader *LayeredCloudsFx::get(Int layers, Bool blend) {
    Shader *&s = Clouds[layers][blend];
    if (!s)
        s = shader->get(S + "Clouds" + (layers + 1) + blend);
    return s;
}
...
{
    Flt d = Abs(a.density - b.density) + Abs(a.noise_min - b.noise_min) + Abs(a.noise_max - b.noise_max)
    ...
    return (d >= 0.05f) ? 2 : (d >= 1.0f / 256 || a.detail != b.detail) ? 1
                                                                        : 0; // return difference (0=ignore, 1=update, 2=update now)
}
/******************************************************************************/
static void SetDensityRow(IntPtr elm_index, Ptr user, Int thread_index) {
    VolumetricCloud &vc = *(VolumetricCloud *)user;
    vc.setDensityRow(elm_index);
}
...
void VolumetricCloud::setImageRow(Int z) {
    ...
    Flt *light_cur = _light, *light_prev = light_cur + width * depth;
    if (_level & 1)
        Swap(light_cur, light_prev); // swap every level
    Byte *image_data = _image_data + _image.bytePP() * (_level + z * _pitch);
    Int image_add = _image.bytePP() * height;
    C Voxel *voxel = _voxels + (_level + z * _pitch);
    Int voxel_add = SIZE(Voxel) * _image.w();
    C Voxel *detail[3];
    Flt x_mul, x_add, dy2_dz2;
    if (_object) {
        x_mul = 2.0f / (width + (2 - 1));
        x_add = -1 + x_mul;                                       // +2 because we're treating as if we have 2 extra pixels at the start and end, so first/last actual pixels will have some density and not zero (because that's a waste of memory to have all zero pixels)
    ...
    if (_build.detail) {
        ...
        REP(_build.detail) {
            yd *= 2;
            zd *= 2;
            detail[i] = _voxels + (yd % height + (zd % depth) * _pitch);
        }
    }
    FREPD(x, width) // go from the start because of 'image_data' and 'voxel'
    {
        ...
        if (_object) {
            ...
            else // start
            {
                ...
                REP(_srcs) {
                    ...
                    if (InRange(xp, width) && InRange(zp, depth))
                        ...
                        light += src.mul * _build.light_power; // if coordinates are out of range, then use full light
                }
            }
        } else {
        ...
        if (density > 0) // there are a lot of cases which don't have density, so perform some calculations only if we have it
        {
            if (_build.detail) // add details, this must be done here and not in create, because it depends on 'noise_min,noise_max' which are configurable
            {
                ...
                REP(_build.detail) {
                    xd *= 2;
                    density += mul * detail[i][(xd % width) * height].get();
                    mul *= 0.5f; // 'height' is the pitch for X coordinate
                }
                SAT(density);
            }
...
void VolumetricCloud::build() {
    if (_creating) // if hasn't finished creating
    {
        ...
        if (_threads->cancel(T, SetDensity))
            setDensity(); // if removed from queue (hasn't started yet) then create here
        else
            _threads->wait(T, SetDensity); // if didn't remove, then it means it's processing now, so wait for it
    }
    ...
    if (_build.light_power > EPS_COL8) {
        ...
        if (bottom > 0) {
            ...
            VecI2 diagonal(Sign(light_pos_xy.x), Sign(light_pos_xy.y));
            if (!diagonal.all()) {
                Src &s = _src[_srcs++];
                s.dir = diagonal;
                s.mul = bottom;
                ...
                if (1) // produces smoother results
                {
                    ...
                    Flt axis_dot;
                    if (Abs(light_pos_xy.x) > Abs(light_pos_xy.y)) {
                        axis.set(diagonal.x, 0);
                        axis_dot = Abs(light_pos_xy.x);
                    } else {
                        axis.set(0, diagonal.y);
                        axis_dot = Abs(light_pos_xy.y);
                    }
        ...
        if (!_object)
            REP(_srcs) // coords must be >=0, so we can use only % in the loop without checking for sign
            {
                Src &src = _src[i];
                if (src.dir.x < 0)
                    src.dir.x += width();
...
void VolumetricCloud::createEx(Int width, Int height, Int depth, Flt frequency, Threads *threads, UInt seed, Flt noise_gain, Bool object) {
    ...
    if (width > 0 && height > 0 && depth > 0)
        if (_image.create3D(height, width, depth, IMAGE_R8G8, 1)) {
            ...
            if (_threads)
                _threads->queue(T, SetDensity);
            else
                setDensity(); // call this last, once all members are set
        }
}
/******************************************************************************/
void VolumetricCloud::update(C Settings &settings) {
    ...
    temp.light_pos = settings.light_pos;
    temp.light_pos.normalize(); // build funtion and 'LightPos' assume it's normalized
    ...
    {                  // want build
        if (_building) // already building
        {
            ...
            {
                _building = difference;
                return;
            } // adjust building before returning
            cancelBuild();
        }
        _build = temp;
        _building = difference; // remember what kind of build we want, before build starts
        if (_threads)
            _threads->queue(T, Build);
        else
            build();
    }
}
...
void VolumetricCloud::checkBuild() {
    ...
    again:
        if (_build_finished) // check if finished first
        {
            ...
            _cur = _build;
        } else if (_building == 2) // have to wait
        {
...
void VolumetricClouds::draw() {
    if (drawable()) {
        ...
        Flt s = tex_scale / size;
        c.pos.set(ActiveCam.matrix.pos.x * s - pos.x, ActiveCam.matrix.pos.z * s - pos.z);
        ...
        if (!D._view_main.full) {
            ext_rect = D.viewRect();
            rect = &ext_rect.extend(Renderer.pixelToScreenSize(1));
        ...
        Bool gamma = LINEAR_GAMMA, swap = (gamma && Renderer._col->canSwapRTV());
        if (swap) {
            gamma = false;
