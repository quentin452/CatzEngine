...
StrO Color::asText() C {
    ...
    s += a;
    return s;
} // 18 because of 4x"255" + 3x", " = 4*3 + 3*2 = 12 + 6 = 18
StrO Color::asHex() C {
    ASSERT(SIZE(T) == 4);
    return TextHexMem(this, SIZE(T), false);
}
StrO Color::asHex3() C {
    ASSERT(SIZE(T) == 4);
    return TextHexMem(this, 3, false);
}

Bool Color::fromHex(C Str &t) {
    ASSERT(SIZE(T) == 4);
    return TextHexMem(t, this, SIZE(T));
}
/******************************************************************************/
...
Color ColorHue(C Color &color, Flt hue) {
    ...
    Color c = ColorHSB(hsb.x + hue, hsb.y, hsb.z);
    c.a = color.a;
    return c;
}
...
Int ColorDiffSum(C Color &x, C Color &y) {
    return Abs(x.r - y.r) + Abs(x.g - y.g) + Abs(x.b - y.b) + Abs(x.a - y.a);
}
...
Color Blend(C Color &base, C Color &color) {
    ...
    if (Flt sum = base_w + color_w) {
        base_w /= sum;
        color_w = 1 - base_w; // faster than "color_w/=sum;"
...
again:
   ...
   FREP(256)if(SRGBToLinearByte(LinearByteToSRGB(i))!=i){Elms_SRGBToLinearByteArray_++; goto again;}
    ...
    FREP(Elms(ByteSRGBToLinearArray) - 1)
    DYNAMIC_ASSERT(ByteSRGBToLinearArray[i] <= ByteSRGBToLinearArray[i + 1], "ByteSRGBToLinearArray[i] > ByteSRGBToLinearArray[i+1]");
    FREP(Elms(LinearToByteSRGBArray) - 1)
    DYNAMIC_ASSERT(LinearToByteSRGBArray[i] <= LinearToByteSRGBArray[i + 1], "LinearToByteSRGBArray[i] > LinearToByteSRGBArray[i+1]");
    FREP(256)
    DYNAMIC_ASSERT(LinearToByteSRGB(ByteSRGBToLinear(i)) == i, "LinearToByteSRGB(ByteSRGBToLinear(s))!=s");
    FREP(256)
    DYNAMIC_ASSERT(SRGBToLinearByte(LinearByteToSRGB(i)) == i, "SRGBToLinearByte(LinearByteToSRGB(s))!=s");
#endif
}
...
static CChar8 *ColorSpaceFileName(COLOR_SPACE color_space) {
    ...
    case COLOR_SPACE_SRGB:
        return "Color/sRGB.icc";
    case COLOR_SPACE_DISPLAY_P3:
        return "Color/DisplayP3.icc";
...
struct WinColorTransform {
    ...
    Bool create(COLOR_SPACE src_color_space, C Str &dest_color_space) {
        ...
        if (dest_profile = OpenColorProfileW(&profile, PROFILE_READ, FILE_SHARE_READ, OPEN_EXISTING)) {
            File f;
            if (f.read(ColorSpaceFileName(src_color_space))) {
                Memt<Byte> data;
                data.setNum(f.size());
...
#if WINDOWS_OLD
        WinColorTransform ct;
        if (ct.create(src_color_space, dest_color_space)) {
            ...
            if (lut.lock(LOCK_WRITE)) {
                ...
                REPD(z, lut.d()) {
                    ...
                    REPD(y, lut.h()) {
                        ...
                        REPD(x, lut.w()) {
                            ...
                            Vec4 dest;
                            Bool diff;
                            ct.convert(src, dest, diff);
                            lut.color3DF(x, y, z, dest);
                            different |= diff;
                        }
                    }
                }
                lut.unlock();
                if (different)
                    return true; // no point in using LUT if it's not different
            }
        }
#endif
    }
