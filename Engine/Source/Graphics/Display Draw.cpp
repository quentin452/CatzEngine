...
void DisplayDraw::lines(C Color &color, C Vec2 *point, Int points) {
    if (point && points > 1) {
        ...
        if (points * SIZE(*point) > VI._mem_max) {
            ...
            if (Vtx2DFlat *v = (Vtx2DFlat *)VI.addVtx(points))
                CopyFast(v, point, points * SIZE(*v));
        }
        VI.end();
    }
}
...
void Image::drawFilter(C Rect &rect, FILTER_TYPE filter) C {
    ...
    if (pixel.x > w() || pixel.y > h())
        switch (filter) // if that size is bigger than the image resolution
        {
            // case FILTER_LINEAR: VI.shader(null); break;

        case FILTER_NONE:
            VI.shader(Sh.DrawTexPoint);
            break; // or SamplerPoint.setPS(SSI_DEFAULT_2D);
        ...
        case FILTER_CUBIC_FAST_SHARP:
            Sh.imgSize(T);
            VI.shader(Sh.DrawTexCubicFast[false]);
            break;
        ...
        case FILTER_CUBIC_PLUS_SHARP:
            Sh.imgSize(T);
            Sh.loadCubicShaders();
            VI.shader(Sh.DrawTexCubicPlus[false]);
            break;
...
void Image::drawFilter(C Color &color, C Color &color_add, C Rect &rect, FILTER_TYPE filter) C {
    ...
    if (pixel.x > w() || pixel.y > h())
        ...
        case FILTER_CUBIC_FAST_SHARP:
            Sh.imgSize(T);
            VI.shader(Sh.DrawTexCubicFast[true]);
            break;
        ...
        case FILTER_CUBIC_PLUS_SHARP:
            Sh.imgSize(T);
            Sh.loadCubicShaders();
            VI.shader(Sh.DrawTexCubicPlus[true]);
            break;
...
void Image::drawRotate(C Vec2 &center, C Vec2 &size, Flt angle, C Vec2 *rotation_center_uv) C {
    ...
    if (Vtx2DTex *v = (Vtx2DTex *)VI.addVtx(4)) {
        ...
        c.y *= -size.y;
        Matrix2P m;
        m.orn().setRotate(angle);
        m.pos = center - c * m.orn();
        v[0].pos.set(0, 0) *= m;
...
static void DrawMask(C Image &image, C Color &color, C Color &color_add, C Rect &rect, C Image &mask, C Rect &mask_rect, C Shader *shader) {
    Rect r;
    r.from(rect.min, rect.max); // needed in case      'rect' is flipped
    Rect m;
    m.from(mask_rect.min, mask_rect.max); // needed in case 'mask_rect' is flipped
    Rect rm = r & m;
    if (rm.valid()) {
        VI.color(color);
        VI.color1(color_add);
...
void Image::drawBorder(C Rect &rect, Flt border, Flt tex_scale, Flt tex_offset, Bool wrap_mode) C {
    ...
    if (Vtx2DTex *v = (Vtx2DTex *)VI.addVtx(10)) {
        ...
        if (wrap_mode)
            ...
            VI.flushIndexed(IndBufBorder, 4 * 2 * 3);
    }
    VI.clear();
}
...
void Image::drawFadeLR(C Color &color, C Rect &rect, Flt trans_l, Flt opaque_l, Flt opaque_r, Flt trans_r) C {
    ...
    if (Vtx2DTexCol *v = (Vtx2DTexCol *)VI.addVtx(16)) {
        Flt min, max;
        MinMax(rect.min.x, rect.max.x, min, max); // this is needed in case 'rect' is flipped
...
void Image::drawCubeFace(C Color &color, C Color &color_add, C Rect &rect, DIR_ENUM face) C {
    if (InRange(face, 6)) {
        if (!Sh.DrawCubeFace)
            Sh.DrawCubeFace = Sh.get("DrawCubeFace");
        ...
        if (Vtx2DFont *v = (Vtx2DFont *)VI.addVtx(4)) {
            ...
            v[2].pos.set(rect.min.x, rect.min.y);
            v[3].pos.set(rect.max.x, rect.min.y);
            switch (face) { // here 'shade' is used as the 3rd tex Z coordinate
            case DIR_RIGHT:
                v[0].tex.set(1, 1);
                v[0].shade = 1;
                v[1].tex.set(1, 1);
...
void Image::drawVolume(C Color &color, C Color &color_add, C OBox &obox, Flt voxel_density_factor, Flt precision, Int min_steps, Int max_steps) C {
    if (Frustum(obox) && Renderer.canReadDepth()) {
        if (!Sh.Volume) {
            ...
            REPD(inside, 3)
            REPD(la, 2)
            Sh.DrawVolume[inside][la] = sf.get(S8 + "Volume" + inside + la);
        ...
        if (v.inside.x >= -v.size.x - e && v.inside.x <= v.size.x + e && v.inside.y >= -v.size.y - e && v.inside.y <= v.size.y + e && v.inside.z >= -v.size.z - e && v.inside.z <= v.size.z + e) {
            D.depth(false);

            if (v.inside.x >= -v.size.x + e && v.inside.x <= v.size.x - e && v.inside.y >= -v.size.y + e && v.inside.y <= v.size.y - e && v.inside.z >= -v.size.z + e && v.inside.z <= v.size.z - e)
                Sh.DrawVolume[2][LA]->begin();
            else
                Sh.DrawVolume[1][LA]->begin();
            MshrBoxR.set().draw();
        } else {
            D.depthOnWrite(true, false);
            Sh.DrawVolume[0][LA]->begin();
            MshrBox.set().draw();
        }
    }
}
