...
#endif
        del();
        if (D3D && OK(D3D->CreateBlendState(&desc, &state)))
            return;
...
#endif
        del();
        if (D3D && OK(D3D->CreateDepthStencilState(&desc, &state)))
            return;
...
#endif
        del();
        if (D3D && OK(D3D->CreateRasterizerState(&desc, &state)))
            return;
...
void DisplayState::depthLock(Bool on) {
    depth(on);
    D._depth_lock = true;
}
/******************************************************************************/
...
Bool DisplayState::depthWrite(Bool write) {
    Bool last = D._depth_write;
    if (D._depth_write != write) {
        D._depth_write = write;
        SetDS();
    }
    return last;
}
void DisplayState::depthFunc(UInt func) {
    if (D._depth_func != func) {
        D._depth_func = func;
        SetDS();
    }
}
void DisplayState::depthOnWrite(Bool on, Bool write) {
    if (D._depth != on || D._depth_write != write) {
        D._depth = on;
        D._depth_write = write;
        SetDS();
    }
} // this ignores '_depth_lock'
void DisplayState::depthOnWriteFunc(Bool on, Bool write, UInt func) {
    if (D._depth != on || D._depth_write != write || D._depth_func != func) {
        D._depth = on;
        D._depth_write = write;
        D._depth_func = func;
        SetDS();
...
void DisplayState::depthWriteFunc(Bool write, UInt func) {
    if (D._depth_write != write || D._depth_func != func) {
        D._depth_write = write;
        D._depth_func = func;
        SetDS();
    }
} // this ignores '_depth_lock'
void DisplayState::stencilRef(Byte ref) {
    if (D._stencil_ref != ref) {
        D._stencil_ref = ref;
        SetDS();
    }
}
void DisplayState::stencil(STENCIL_MODE mode, Byte ref) {
    if (D._stencil_ref != ref || D._stencil != mode) {
        D._stencil_ref = ref;
        D._stencil = mode;
        SetDS();
    }
...
#elif GL
Bool DisplayState::depthWrite(Bool on) {
    Bool last = D._depth_write;
    if (D._depth_write != on)
        glDepthMask(D._depth_write = on);
    return last;
}
void DisplayState::depthFunc(UInt func) {
    if (D._depth_func != func)
        glDepthFunc(D._depth_func = func);
}
void DisplayState::stencilRef(Byte ref) {
    if (D._stencil_ref != ref)
        glStencilFunc(StencilFunc, D._stencil_ref = ref, StencilMask);
}
void DisplayState::stencil(STENCIL_MODE mode, Byte ref) {
#if 1
    STENCIL_MODE old_mode = LastStencilMode; // remember old values
    Byte old_ref = D._stencil_ref;
    D._stencil_ref = ref; // already change 'D._stencil_ref' because calling 'stencil' might use it
    stencil(mode);
    if (old_mode == LastStencilMode && old_ref != ref)
        glStencilFunc(StencilFunc, ref, StencilMask); // if 'stencil' didn't change 'LastStencilMode' and we wanted to change 'D._stencil_ref' then manually change the 'ref'
#else
    stencil(mode);
    stencilRef(ref);
#endif
...
void DisplayState::stencil(STENCIL_MODE stencil) {
    ...
    if (D._stencil != stencil) {
        ...
        if (!stencil) {
            ...
            if (LastStencilMode != stencil)
                switch (LastStencilMode = stencil) {
...
void DisplayState::cull(Bool on) {
    if (D._cull != on) {
        D._cull = on;
        SetRS();
    }
...
#if !GL_ES
    if (D._wire != on)
        glPolygonMode(GL_FRONT_AND_BACK, (D._wire = on) ? GL_LINE : GL_FILL);
#endif
}
void DisplayState::cull(Bool on) {
    if (D._cull != on)
...
void DisplayState::depthClip(Bool on) {
    if (D._depth_clip != on) {
        ...
        if (on)
            ...
            glEnable(GL_DEPTH_CLAMP); // !! on GL enabling GL_DEPTH_CLAMP actually disables clipping !!
#endif
    }
}
void DisplayState::frontFace(Bool ccw) {
...
#define rect r
    ...
    if (D._clip_recti != rect) {
        ...
        rectangle.left = rect.min.x;
        rectangle.right = rect.max.x;
        rectangle.top = rect.min.y;
        rectangle.bottom = rect.max.y;
...
static void SetClipRect() {
    ...
    if (recti.max.x <= recti.min.x || recti.max.y <= recti.min.y)
        recti.zero(); // if the rectangle is invalid then zero it, to prevent negative sizes in case drivers can't handle them properly
    SetClipRect(recti);
}
...
void DisplayState::clipAllow(C RectI &rect) // this gets called during shadow rendering
{
    ...
    D._clip = D._clip_allow = true;
    SetClip(); // call after setting '_clip' and '_clip_allow'
}
void DisplayState::clipAllow(Bool on) // this gets called when a render target has changed
{
    // if(D._clip_allow!=on) we must always do below, because: we may have a different sized RT now, and need to recalculate clip rect. for GL we may have changed between D.mainFBO for which we need different orientation of the clip rect
    {
        ...
        SetClip();
        if (D._clip_real)
            SetClipRect(); // call after 'SetClip' because we need '_clip_real'
    }
}
/******************************************************************************/
...
#if DX11
    if (D._alpha != alpha)
        BlendStates[D._alpha = alpha].set();
#elif GL
    if (D._alpha != alpha)
        switch (D._alpha = alpha) {
        case ALPHA_NONE:
            glDisable(GL_BLEND);
            break;
        ...
        case ALPHA_RENDER_BLEND:
            ...
            if (glBlendFunci)
                glBlendFunci(1, GL_ONE_MINUS_DST_COLOR, GL_ONE); // #RTOutput.Blend set RT1 as Increase
            break;
...
void DisplayState::colWrite(Byte color_mask, Byte index) {
    ...
    if (D._col_write[index] != color_mask) {
        ...
        if (!index) {
            color_mask &= Col0WriteAllow;
            if (color_mask == Col0WriteReal)
                return;
...
#if GL_ES
        if (!index)
            glColorMask(FlagOn(color_mask, COL_WRITE_R), FlagOn(color_mask, COL_WRITE_G), FlagOn(color_mask, COL_WRITE_B), FlagOn(color_mask, COL_WRITE_A)); // TODO: 'glColorMaski' requires GLES 3.2 - https://www.khronos.org/registry/OpenGL-Refpages/es3/html/glColorMask.xhtml
#else
...
#if DX11
        D3D11_VIEWPORT vp;
        vp.TopLeftX = D._viewport.min.x;
        vp.Width = D._viewport.w();
        vp.MinDepth = 0;
        vp.TopLeftY = D._viewport.min.y;
        vp.Height = D._viewport.h();
        vp.MaxDepth = 1;
...
#if DX11
    if (D._vf != vf)
        D3DC->IASetInputLayout(D._vf = vf);
#elif GL
...
/******************************************************************************/
void DisplayState::set2D() {
    D.clipPlane(false);
    D.wire(false);
}
void DisplayState::set3D() {
    if (Renderer.mirror())
        D.clipPlane(true);
...
void DisplayState::linearGamma(Bool on) {
    if (D._linear_gamma != on) {
        ...
        MaterialClear(); // TRANSPARENT gives the same Vec4 for sRGB and non-sRGB, 'MaterialClear' must be called when changing 'D.alphaFactor'
#endif
    }
}
/******************************************************************************/
...
void DisplayState::fbo(UInt fbo) {
    if (D._fbo != fbo)
        glBindFramebuffer(GL_FRAMEBUFFER, D._fbo = fbo);
}
#endif
#if IOS
...
void DisplayState::setDeviceSettings() {
    ...
    depth(old._depth);
    _depth_write ^= 1;
    depthWrite(old._depth_write);
    _depth_clip ^= 1;
    depthClip(old._depth_clip);
    _depth_func ^= 1;
    depthFunc(old._depth_func);
    ...
    _stencil_ref ^= 1;
    stencilRef(old._stencil_ref);

    _line_smooth ^= 1;
    lineSmooth(!_line_smooth);
    _wire ^= 1;
    wire(!_wire);
    _cull ^= 1;
    cull(!_cull);
    ...
    _depth_bias ^= 1;
    depthBias((BIAS_MODE)old._depth_bias);

    _alpha = ALPHA_MODE(_alpha ^ 1);
    alpha(old._alpha);
    _alpha_factor.r ^= 1;
    alphaFactor(old._alpha_factor);

    _col_write[0] ^= 1;
    colWrite(old._col_write[0], 0);
    _col_write[1] ^= 1;
    colWrite(old._col_write[1], 1);
    _col_write[2] ^= 1;
    colWrite(old._col_write[2], 2);
    _col_write[3] ^= 1;
    colWrite(old._col_write[3], 3);
#endif

    _clip_recti.set(0, -1);
    _clip ^= 1;
    clip(old._clip ? &old._clip_rect : null);
    _clip_allow ^= 1;
    clipAllow(old._clip_allow);

    _viewport.set(0, -1);
    viewport(old._viewport);

    _prim_type ^= 1;
    primType(old._prim_type);
...
void DisplayState::del() {
    ...
    REPAD(i, DepthStates)
    REPAD(j, DepthStates[i]) REPAD(k, DepthStates[i][j]) REPAOD(l, DepthStates[i][j][k]).del();
    REPAD(i, RasterStates)
    REPAD(j, RasterStates[i]) REPAD(k, RasterStates[i][j]) REPAD(l, RasterStates[i][j][k]) REPAD(m, RasterStates[i][j][k][l]) REPAD(n, RasterStates[i][j][k][l][m]) REPAOD(o, RasterStates[i][j][k][l][m][n]).del();
#endif
}
void DisplayState::create() {
    if (LogInit)
        LogN("DisplayState.create");
    ...
    }
    {
        ...
        if (D.independentBlendAvailable()) // #RTOutput.Blend set RT1 as Increase
        {
            ...
            for (Int i = 1; i < Elms(desc.RenderTarget); i++)
    ...
    REPD(front_face, 2) {
        ...
        desc.CullMode = ((cull == 0) ? D3D11_CULL_NONE : (cull == 1) ? D3D11_CULL_BACK
                                                                     : D3D11_CULL_FRONT); // D3D11_CULL_FRONT is not used (instead 'FrontCounterClockwise' is used, so we don't have to make "BackFlip" in shader conditional on a constant)
        ...
        case BIAS_ZERO:
            ...
            break;
        case BIAS_SHADOW:
            desc.DepthBias = DEPTH_BIAS_SHADOW;
            desc.SlopeScaledDepthBias = SLOPE_SCALED_DEPTH_BIAS_SHADOW;
            break;
        case BIAS_OVERLAY:
            desc.DepthBias = DEPTH_BIAS_OVERLAY;
            desc.SlopeScaledDepthBias = SLOPE_SCALED_DEPTH_BIAS_OVERLAY;
            break;
        case BIAS_EARLY_Z:
            desc.DepthBias = DEPTH_BIAS_EARLY_Z;
            desc.SlopeScaledDepthBias = SLOPE_SCALED_DEPTH_BIAS_EARLY_Z;
            break;
        }
