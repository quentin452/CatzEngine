...
void DrawLaser(C Color &color, C Color &middle_color, Flt middle_exponent, Flt radius, Bool sharp_ending, C CMemPtr<Vec> &points) {
    ...
    if (Renderer.firstPass() && points.elms() > 1) {
        ...
        if (Renderer() == RM_EMISSIVE) {
            ...
            Shader *&shader = Sh.Laser[normals];
            if (!shader)
                shader = ShaderFiles("Effects 3D")->get(S8 + "Laser" + normals);
            VI.shader(shader);
            VI.color(color);
            VI.color1(middle_color);
            Sh.Step->set(middle_exponent);
            if (color.a || middle_color.a)
        ...
        for (Int i = 1; i < points.elms(); i++) {
            ...
            if (i + 1 < points.elms()) {
                ...
                n->m.y = PointOnPlane(c->m.y, n->m.z);
                if (!n->m.y.normalize())
                    n->m.y = PerpN(n->m.z);
        ...
        if (original.elms() > 1) {
            ...
            for (Int i = 1; i < original.elms(); i++) {
                ...
                for (;;) {
                    ...
                    if (dist > random_step) {
                        if (i == original.elms() - 1 && dist < random_step * 1.5)
                            break;
