...
static void SetupFur(Int res = 256) {
    ...
    static Image img;
    img.create2D(res, res, IMAGE_R8, 0, false);
    if (img.lock(LOCK_WRITE)) {
        FREPD(y, img.h())
        FREPD(x, img.w())
        img.pixB(x, y) = random(256);
        img.unlock().updateMipMaps();
        if (1)
            img.save("C:/Esenthel/Data/Img/Fur.img");
    }
...
/******************************************************************************/
void InitFur() {
    if (FurCol.get("Img/Fur.img"))
        GetShaderImage("FurCol")->set(FurCol());
