...
static UInt GetUInt(File &f) {
    Byte b[4];
    f >> b;
    return Calculate(b, Elms(b));
}
static UInt GetU16(File &f) {
    Byte b[2];
    f >> b;
    return Calculate(b, Elms(b));
}
static Int GetI16(File &f) {
    Byte b[2];
    f >> b;
    return (I16)Calculate(b, Elms(b));
}
...
Bool PSD::ReadImageResource(File &f, IMAGE_RESOURCE &image_resource) {
    ...
    for (; !f.end() && nBytesRead < nTotalBytes;) {
        ...
        if (image_resource.OSType[0] == '8' && image_resource.OSType[1] == 'B' && image_resource.OSType[2] == 'I' && image_resource.OSType[3] == 'M') {
            ...
            if (0 != (image_resource.nSize % 2))
                image_resource.nSize++; // resource data must be even
...
Bool PSD::ProccessBuffer(Byte *pData, Image &dest) {
    if (pData) {
        ...
        case 8: // Duotone
        {
            if (dest.createSoft(nWidth, nHeight, 1, (header_info.nBitsPerPixel == 32) ? IMAGE_I32 : (header_info.nBitsPerPixel == 24) ? IMAGE_I24
                                                                                                : (header_info.nBitsPerPixel == 16)   ? IMAGE_I16
                                                                                                                                      : IMAGE_L8_SRGB)) {
                for (; nCounter < nTotalBytes; nCounter += header_info.nChannels * bytesPerPixelPerChannel) {
                    UInt u = Calculate(pData + nCounter, bytesPerPixelPerChannel);
                    if (header_info.nBitsPerPixel == 32)
                        u = RoundUClamp(Sat(Pow((Flt &)u, 1.0f / 1.75f)) * 0xFFFFFFFFu);
                    dest.pixel(x, y, u);
                    x++;
                    if (x >= nWidth) {
                        x = 0;
        ...
        case 4: // CMYK
        {
            if (dest.createSoft(nWidth, nHeight, 1, IMAGE_R8G8B8_SRGB)) {
                for (; nCounter < nTotalBytes; nCounter += header_info.nChannels * bytesPerPixelPerChannel) {
                    ...
                    Int red, green, blue;
                    CMYKToRGB(c, m, Y, k, red, green, blue);
                    dest.color(x, y, Color(red, green, blue));
                    x++;
                    if (x >= nWidth) {
                        x = 0;
...
Bool PSD::ReadImageData(File &f, Image &dest) {
    ...
    case 2:
        break; // ZIP without prediction, no specification
    case 3:
        break; // ZIP with    prediction, no specification
    default:
                ...
                if (psd.ReadLayerAndMaskInfoSection(f))
                    if (psd.ReadImageData(f, T)) {
                        if (psd.merged_alpha && typeInfo().a) // PSD's are blended on white background
                        {
                            REPD(y, h())
                            REPD(x, w())
                            color(x, y, UnblendWhite(color(x, y)));
