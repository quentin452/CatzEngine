    ...
    for (unsigned int y = 0; y < ui32WordHeight; y++) {
        for (unsigned int x = 0; x < ui32WordWidth; x++) {
            ...
            if (punchthroughAlpha)
                ...
                result.alpha = (upscaledColourA[y * ui32WordWidth + x].alpha * (8 - mod) + upscaledColourB[y * ui32WordWidth + x].alpha * mod) / 8;
...
Bool DecompressPVRTC(C Image &src, Image &dest, Int max_mip_maps) {
    if (src.hwType() == IMAGE_PVRTC1_2 || src.hwType() == IMAGE_PVRTC1_2_SRGB || src.hwType() == IMAGE_PVRTC1_4 || src.hwType() == IMAGE_PVRTC1_4_SRGB)
        if (dest.is() || dest.create(src.w(), src.h(), src.d(), src.sRGB() ? IMAGE_R8G8B8A8_SRGB : IMAGE_R8G8B8A8, AsSoft(src.mode()), src.mipMaps())) // use 'IMAGE_R8G8B8A8' because 'pvrtcDecompress' operates on that format
            if (dest.size3() == src.size3()) {
                ...
                REPD(mip, Min(src.mipMaps(), dest.mipMaps(), max_mip_maps)) {
                    ...
                    if (!write_to_dest && !temp.create(src_mip_hwW, src_mip_hwH, Max(1, dest.d() >> mip), IMAGE_R8G8B8A8, IMAGE_SOFT, 1))
                        return false;
                    REPD(face, dest.faces()) // use 'dest.faces' (not 'target.faces' because it may be 1 for 'temp', and not 'src.faces' because we need to write all to dest)
                    {
                        if (!src.lockRead(mip, (DIR_ENUM)Min(face, src_faces1)))
                            return false;
                        if (write_to_dest && !dest.lock(LOCK_WRITE, mip, (DIR_ENUM)face)) {
                            src.unlock();
                            return false;
                        } // we have to lock only for 'dest' because 'temp' is 1mip-1face-SOFT and doesn't need locking

                        using namespace PVRTC;
                        REPD(z, target.ld())
                        pvrtcDecompress((PVRTuint8 *)(src.data() + z * src.pitch2()), (Pixel32 *)(target.data() + z * target.pitch2()), src_mip_hwW, src_mip_hwH, bits);

                        if (write_to_dest)
                            dest.unlock();
                        src.unlock();

                        if (!write_to_dest && !dest.injectMipMap(temp, mip, (DIR_ENUM)face, FILTER_NO_STRETCH, IC_CLAMP))
                            return false;
                    }
                }
                return true;
