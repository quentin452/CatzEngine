...
static tmsize_t TIFRead(thandle_t file, void *data, tmsize_t size) {
    if (TIFFile *f = (TIFFile *)file) {
        Int read = f->f.getReturnSize(data, size);
        MAX(f->end, f->f.pos());
        return read;
    }
    return 0;
...
static tmsize_t TIFWrite(thandle_t file, void *data, tmsize_t size) {
    if (TIFFile *f = (TIFFile *)file) {
        Int written = f->f.putReturnSize(data, size);
        MAX(f->end, f->f.pos());
        return written;
    }
    return 0;
        ...
        if (scanline_size * 8 == samples * w * bits) {
            ...
            if (type && createSoft(w, h, 1, type)) {
                if (compression == COMPRESSION_NONE) {
                    ok = true;
                    FREPD(y, h)
                    if (!TIFFReadScanline(tif, data() + y * pitch(), y)) {
                        ok = false;
                        break;
                    } // read in forward order to avoid file seeking going back
                ...
                if (TIFFReadRGBAImageOriented(tif, w, h, (uint32 *)data(), ORIENTATION_TOPLEFT, 0)) {
                    ...
                    if (photometric != PHOTOMETRIC_PALETTE) // keep palette as RGBA
                        if (samples == 1 || samples == 3)
                            ok = copy(T, -1, -1, -1, (samples == 1) ? IMAGE_L8_SRGB : IMAGE_R8G8B8_SRGB);
                }
        TIFFClose(tif);
...
Bool Image::ExportTIF(File &f, Flt compression_level) C {
    ...
    if (src->lockRead()) {
        ...
        if (TIFF *tif = TIFFClientOpen("", "w", &tf, TIFRead, TIFWrite, TIFSeek, TIFClose, TIFSize, null, null)) // can't use null because crash will occur
        {
            ...
            Byte channels = type_info.channels;
            if (channels == 2)
                channels = 3;
            ...
            TIFFSetField(tif, TIFFTAG_COMPRESSION, (compression_level < 0) ? COMPRESSION_NONE : (compression_level > 0.5f) ? COMPRESSION_LZW
                                                                                                                           : COMPRESSION_NONE);
            TIFFSetField(tif, TIFFTAG_PREDICTOR, PREDICTOR_HORIZONTAL);
            Memt<Byte> buf;
            buf.setNum(src->w() * channels * (bit16 ? 2 : 1));
            FREPD(y, src->h()) {
                Byte *b = buf.data();
                if (bit16)
                    CopyFast(b, src->data() + src->pitch() * y, buf.elms());
                else
                    FREPD(x, src->w()) {
                ...
                if (TIFFWriteScanline(tif, buf.data(), y) < 0) {
                    ok = false;
                    break;
                }
            }
...
Bool Image::ExportTIF(C Str &name, Flt compression_level) C {
    File f;
    if (f.write(name)) {
        if (ExportTIF(f, compression_level) && f.flush())
            return true;
        f.del();
        FDelFile(name);
    }
