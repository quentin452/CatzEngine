...
Bool Image::ImportWEBP(File &f) {
    ...
    if (f.getUInt() == CC4('R', 'I', 'F', 'F')) {
        UInt size;
        if (f.getFast(size) && f.left() >= size && f.getUInt() == CC4('W', 'E', 'B', 'P') && f.skip(-12)) {
            ...
            if (WebPInitDecoderConfig(&config)) {
                ...
                if (WebPGetFeatures((uint8_t *)data, size, &config.input) == VP8_STATUS_OK) {
                    ...
                    if (createSoft(config.input.width, config.input.height, 1, type)) {
                        ...
                        if (WebPDecode((uint8_t *)data, size, &config) == VP8_STATUS_OK)
                            ok = true;
                    }
                }
...
Bool Image::ExportWEBP(File &f, Flt rgb_quality, Flt alpha_quality) C {
    ...
    if (src->cube())
        if (temp.fromCube(*src, IMAGE_B8G8R8A8_SRGB))
            ...
            return false;
    if (src->hwType() != IMAGE_B8G8R8A8 && src->hwType() != IMAGE_B8G8R8A8_SRGB)
        if (src->copy(temp, -1, -1, -1, IMAGE_B8G8R8A8_SRGB, IMAGE_SOFT, 1))
            src = &temp;
        else
            return false; // WEBP uses BGRA
...
Bool Image::ExportWEBP(C Str &name, Flt rgb_quality, Flt alpha_quality) C {
    ...
    File f;
    if (f.write(name)) {
        if (ExportWEBP(f, rgb_quality, alpha_quality) && f.flush())
            return true;
        f.del();
        FDelFile(name);
    }
