...
void MaterialParams::reflect(Flt reflect) {
    reflect_add = reflect;
    reflect_mul = 1 - reflect;
}
void XMaterial ::reflect(Flt reflect) {
    reflect_add = reflect;
    reflect_mul = 1 - reflect;
}
void MaterialParams::reflect(Flt min, Flt max) {
    reflect_add = min;
    reflect_mul = (1 - min) * max;
}
void XMaterial ::reflect(Flt min, Flt max) {
    reflect_add = min;
    reflect_mul = (1 - min) * max;
}
Flt MaterialParams::reflectMax() C {
    Flt div = (1 - reflect_add);
    return Equal(div, 0) ? 1 : reflect_mul / div;
}
Flt XMaterial ::reflectMax() C {
    Flt div = (1 - reflect_add);
    return Equal(div, 0) ? 1 : reflect_mul / div;
}
/******************************************************************************/
...
#endif
    {
        UniqueMultiMaterialMap.lock();
        REPA(UniqueMultiMaterialMap) {
            C UniqueMultiMaterialKey &key = UniqueMultiMaterialMap.lockedKey(i);
            if (key.m[0] == this || key.m[1] == this || key.m[2] == this || key.m[3] == this)
                UniqueMultiMaterialMap.lockedRemove(i);
        }
...
Material &Material::validate() // #MaterialTextureLayout
{
    ...
    _has = 0;
    if (_alpha_factor.a || emissive_glow > EPS_COL8)
        _has |= HAS_GLOW;
    if (technique == MTECH_CLEAR_COAT)
        _has |= HAS_CLEAR_COAT;
    if (technique == MTECH_FUR)
        _has |= HAS_FUR;
    ...
    // set multi
    {
        ...
        } else { // same as above with det_power=0
            _multi.det_mul = 0;
            _multi.det_add = 0;
            _multi.det_inv = 1;
        }
...
void Material::setBlend() C {
    if (MaterialLast != this) {
        ...
        if (_alpha_factor.a)
            Renderer._has |= HAS_GLOW; // here operate only on '_alpha_factor.a' instead of '_has' because blend shaders use glow only from alpha factor but not 'emissive_glow'
...
void Material::setMulti(Int i) C {
    ...
    if (MaterialLast4[i] != this) {
        ...
        if (!i)
            MaterialLast = null; // because they use the same shader images
...
Bool Material::loadData(File &f, CChar *path) {
    ...
    Flt sss, smooth, reflect;
    switch (f.decUIntV()) {
    case 14: {
        f.getMulti(cull, detail_all_lod, technique) >> mp;
        f.getStr(temp);
        base_0.require(temp, path); // base_0 is RGBA
        f.getStr(temp);
        base_1.require(temp, path); // base_1 is NormalXY
        f.getStr(temp);
        base_2.require(temp, path); // base_2 is Metal, Rough, Bump, Glow
        f.getStr(temp);
        detail_map.require(temp, path);
        f.getStr(temp);
        macro_map.require(temp, path);
        f.getStr(temp);
        emissive_map.require(temp, path);
    } break;
    ...
    case 12: {
        f.getMulti(cull, technique) >> color_l >> emissive_l >> rough_mul >> rough_add >> reflect_mul >> reflect_add >> glow >> normal >> bump >> det_power >> det_uv_scale >> uv_scale;
        emissive_glow = 0;
        detail_all_lod = false;
        f.getStr(temp);
        base_0.require(temp, path); // base_0 is RGBA
    ...
    case 11: {
        ...
        base_1.require(temp, path); // base_1 is NormalXY
        f.getStr(temp);
        base_2.require(temp, path);
        if (Is(temp)) {
            T.rough_add = 1;
            T.rough_mul = -smooth;
        } else {
            T.rough_add = 1 - smooth;
            T.rough_mul = 0;
        } // base_2 is Metal, Smooth, Bump, Glow
        f.getStr(temp);
        detail_map.require(temp, path);
        f.getStr(temp);
        macro_map.require(temp, path);
    ...
    case 10: {
        ...
        if (Is(temp)) {
            T.rough_add = 1;
            T.rough_mul = 0;
            T.reflect_add = MATERIAL_REFLECT;
            T.reflect_mul = 0;
        } else {
            T.rough_add = 1 - smooth;
            T.rough_mul = 0;
            T.reflect_add = reflect;
            T.reflect_mul = 0;
        } // base_2 is Smooth, Reflectivity, Bump, Glow
        f.getStr(temp);
        detail_map.require(temp, path);
        f.getStr(temp);
        macro_map.require(temp, path);
    ...
    case 9: {
        f.getMulti(cull, technique) >> color_l >> emissive_l >> smooth >> sss >> glow >> normal >> bump >> uv_scale >> det_uv_scale >> det_power >> reflect;
        colorS(color_l);
        emissive_glow = 0;
        detail_all_lod = false;
        f.getStr(temp);
        base_0.require(temp, path);
        f.getStr(temp);
        base_1.require(temp, path);
        if (Is(temp)) {
            T.rough_add = 1;
            T.rough_mul = 0;
        } else {
            T.rough_add = 1 - smooth;
            T.rough_mul = 0;
        } // base_1 had normals and specular
        base_2 = null;
        f.getStr(temp);
        detail_map.require(temp, path);
        f.getStr(temp);
    ...
    case 7: {
        f >> color_l >> emissive_l >> smooth >> sss >> glow >> normal >> bump >> uv_scale >> det_uv_scale >> det_power >> reflect >> cull >> technique;
        colorS(color_l);
        emissive_glow = 0;
        detail_all_lod = false;
        f._getStr(temp);
    ...
    case 3: {
        f >> color_l >> emissive_l >> smooth >> sss >> glow >> normal >> bump >> det_uv_scale >> det_power >> reflect >> cull >> technique;
        uv_scale = 1;
        colorS(color_l);
        emissive_glow = 0;
        detail_all_lod = false;
    ...
    case 2: {
        f.skip(1);
        f >> color_l >> smooth >> sss >> glow >> normal >> bump >> det_uv_scale >> det_power >> reflect >> cull >> technique;
        emissive_l = 0;
        uv_scale = 1;
        colorS(color_l);
        emissive_glow = 0;
    ...
    case 1: {
        f.skip(1);
        f >> color_l >> smooth >> glow >> normal >> bump >> det_uv_scale >> det_power >> reflect >> cull >> technique;
        sss = 0;
        emissive_l = 0;
        uv_scale = 1;
        colorS(color_l);
    ...
    case 0: {
        f.skip(1);
        f >> color_l >> smooth >> glow >> normal >> bump >> det_uv_scale >> det_power >> reflect >> cull;
        sss = 0;
        emissive_l = 0;
        uv_scale = 1;
        colorS(color_l);
        ...
        default:
            ...
            break;
        case 1:
            technique = MTECH_ALPHA_TEST;
            break;
        case 4:
            technique = MTECH_FUR;
            break;
        case 5:
            technique = MTECH_GRASS;
            break;
        }
        if (technique == MTECH_FUR) {
            det_power = color_l.w;
...
Bool Material::load(File &f, CChar *path) {
    if (f.getUInt() == CC4_MTRL)
        return loadData(f, path);
    reset();
    return false;
}
...
/******************************************************************************/
static Int ImgW(C ImageSource &src, C Image *img) { return (!img->is()) ? 0 : (src.size.x > 0) ? src.size.x
                                                                                               : img->w(); }
static Int ImgH(C ImageSource &src, C Image *img) { return (!img->is()) ? 0 : (src.size.y > 0) ? src.size.y
                                                                                               : img->h(); }

static FILTER_TYPE Filter(Int filter) { return InRange(filter, FILTER_NUM) ? FILTER_TYPE(filter) : FILTER_BEST; }

TEX_FLAG CreateBaseTextures(Image &base_0, Image &base_1, Image &base_2, C ImageSource &color, C ImageSource &alpha, C ImageSource &bump, C ImageSource &normal, C ImageSource &smooth, C ImageSource &metal, C ImageSource &glow, Bool resize_to_pow2, Bool flip_normal_y, Bool smooth_is_rough) { // #MaterialTextureLayout
    ...
    Image dest_0, dest_1, dest_2;
    {
        ...
        if (color_src->compressed())
            if (color_src->copy(color_temp, -1, -1, -1, color_src->typeInfo().a ? IMAGE_R8G8B8A8_SRGB : IMAGE_R8G8B8_SRGB, IMAGE_SOFT, 1, FILTER_BEST, IC_IGNORE_GAMMA))
                color_src = &color_temp;
            else
                goto error; // keep alpha because we might use it for alpha and IC_ALPHA_WEIGHT
        Image alpha_temp;
        C Image *alpha_src = &alpha.image;
        if (alpha_src->compressed())
            if (alpha_src->copy(alpha_temp, -1, -1, -1, alpha_src->typeInfo().a ? IMAGE_L8A8 : IMAGE_L8, IMAGE_SOFT, 1, FILTER_BEST, IC_IGNORE_GAMMA))
                alpha_src = &alpha_temp;
            else
                goto error;
        Image bump_temp;
        C Image *bump_src = &bump.image;
        if (bump_src->compressed())
            if (bump_src->copy(bump_temp, -1, -1, -1, bump_src->highPrecision() ? IMAGE_F32 : IMAGE_L8, IMAGE_SOFT, 1, FILTER_BEST, IC_IGNORE_GAMMA))
                bump_src = &bump_temp;
            else
                goto error; // use high precision because we might use it to create normal map or stretch
        Image normal_temp;
        C Image *normal_src = &normal.image;
        if (normal_src->compressed())
            if (normal_src->copy(normal_temp, -1, -1, -1, normal_src->highPrecision() ? IMAGE_F32_2 : IMAGE_R8G8, IMAGE_SOFT, 1, FILTER_BEST, IC_IGNORE_GAMMA))
                normal_src = &normal_temp;
            else
                goto error; // use high precision because we still do math operations so higher precision could be useful
        Image smooth_temp;
        C Image *smooth_src = &smooth.image;
        if (smooth_src->compressed())
            if (smooth_src->copy(smooth_temp, -1, -1, -1, IMAGE_L8, IMAGE_SOFT, 1, FILTER_BEST, IC_IGNORE_GAMMA))
                smooth_src = &smooth_temp;
            else
                goto error;
        Image metal_temp;
        C Image *metal_src = &metal.image;
        if (metal_src->compressed())
            if (metal_src->copy(metal_temp, -1, -1, -1, IMAGE_L8, IMAGE_SOFT, 1, FILTER_BEST, IC_IGNORE_GAMMA))
                metal_src = &metal_temp;
            else
                goto error;
        Image glow_temp;
        C Image *glow_src = &glow.image;
        if (glow_src->compressed())
            if (glow_src->copy(glow_temp, -1, -1, -1, IMAGE_L8, IMAGE_SOFT, 1, FILTER_BEST, IC_IGNORE_GAMMA))
                glow_src = &glow_temp;
            else
                goto error;
        ...
        if (alpha_src->is()) // there's alpha map specified
        {
            if (alpha_src->typeChannels() > 1 && alpha_src->typeInfo().a // if alpha has both RGB and Alpha channels, then check which one to use
                ...
                alpha_src->unlock();
                if (min_alpha >= 254 && min_lum < 254)
                    if (alpha_src->copy(alpha_temp, -1, -1, -1, IMAGE_L8, IMAGE_SOFT, 1, FILTER_BEST, IC_IGNORE_GAMMA))
                        alpha_src = &alpha_temp;
                    else
                        goto error; // alpha channel is fully white -> use luminance as alpha
            }
        ...
        if (!alpha_src->is())
            ...
            alpha_size.set((alpha.size.x > 0) ? alpha.size.x : (alpha_from_col && color.size.x > 0) ? color.size.x
                                                                                                    : alpha_src->w(),
                           (alpha.size.y > 0) ? alpha.size.y : (alpha_from_col && color.size.y > 0) ? color.size.y
                                                                                                    : alpha_src->h());
        ...
        if (texf & (TEXF_COLOR | TEXF_ALPHA | TEXF_SMOOTH | TEXF_METAL | TEXF_BUMP | TEXF_GLOW)) // if want any for Base0 or Base2, because shaders support base_2 only if base_0 is also present, so if we want base_2 then also need base_0
        {
            Int w = Max(1, ImgW(color, color_src), alpha_size.x), // Max 1 in case all images are empty, but we still need it because of Base2
                h = Max(1, ImgH(color, color_src), alpha_size.y);
            if (resize_to_pow2) {
                w = NearestPow2(w);
                h = NearestPow2(h);
            }
            if (color_src->is() && (color_src->w() != w || color_src->h() != h))
                if (color_src->copy(color_temp, w, h, -1, IMAGE_R8G8B8_SRGB, IMAGE_SOFT, 1, Filter(color.filter), (color.clamp ? IC_CLAMP : IC_WRAP) | IC_IGNORE_GAMMA | IC_ALPHA_WEIGHT))
                    color_src = &color_temp;
                else
                    goto error;
            if (alpha_src->is() && (alpha_src->w() != w || alpha_src->h() != h))
                if (alpha_src->copy(alpha_temp, w, h, -1, alpha_src->typeInfo().a ? IMAGE_A8 : IMAGE_L8, IMAGE_SOFT, 1, alpha_filter, (alpha.clamp ? IC_CLAMP : IC_WRAP) | IC_IGNORE_GAMMA))
                    alpha_src = &alpha_temp;
                else
                    goto error;
            if (!color_src->is() || color_src->lockRead()) {
                if (!alpha_src->is() || alpha_src->lockRead()) {
                    ...
                    REPD(x, dest_0.w()) {
                        if (color_src->is())
                            c.rgb = color_src->color(x, y).rgb;
                        if (alpha_src->is())
                            c.a = alpha_src->color(x, y).c[alpha_component];
                        dest_0.color(x, y, c);
                    }
                    alpha_src->unlock();
                }
        ...
        if (bump_to_normal)                  // create normal from bump
        {
            ...
            Int w = ((normal.size.x > 0) ? normal.size.x : (bump_to_normal == bump_src && bump.size.x > 0) ? bump.size.x
                                                                                                           : bump_to_normal->w()),
                h = ((normal.size.y > 0) ? normal.size.y : (bump_to_normal == bump_src && bump.size.y > 0) ? bump.size.y
                                                                                                           : bump_to_normal->h());
            if (resize_to_pow2) {
                w = NearestPow2(w);
                h = NearestPow2(h);
            ...
            if (bump_to_normal->w() != w || bump_to_normal->h() != h)
                if (bump_to_normal->copy(normal_temp, w, h, -1, IMAGE_F32, IMAGE_SOFT, 1, Filter(src.filter), (src.clamp ? IC_CLAMP : IC_WRAP) | IC_IGNORE_GAMMA))
                    bump_to_normal = &normal_temp;
                else
                    goto error; // !! convert to 'normal_temp' instead of 'bump_temp' because we still need original bump later !!
            bump_to_normal->bumpToNormal(normal_temp, AvgF(w, h) * BUMP_TO_NORMAL_SCALE);
            normal_src = &normal_temp;
            flip_normal_y = false; // no need to flip since normal map generated from bump is always correct
        }
        if (normal_src->is()) {
            Int w = ImgW(normal, normal_src),
                h = ImgH(normal, normal_src);
            if (resize_to_pow2) {
                w = NearestPow2(w);
                h = NearestPow2(h);
            }
            if (normal_src->is() && (normal_src->w() != w || normal_src->h() != h))
                if (normal_src->copy(normal_temp, w, h, -1, IMAGE_F32_2, IMAGE_SOFT, 1, Filter(normal.filter), (normal.clamp ? IC_CLAMP : IC_WRAP) | IC_IGNORE_GAMMA))
                    normal_src = &normal_temp;
                else
                    goto error; // use high precision because we still do math operations so higher precision could be useful
        ...
        if (texf & (TEXF_SMOOTH | TEXF_METAL | TEXF_BUMP | TEXF_GLOW)) {
            Int w = Max(ImgW(smooth, smooth_src), ImgW(metal, metal_src), ImgW(bump, bump_src), ImgW(glow, glow_src)),
                h = Max(ImgH(smooth, smooth_src), ImgH(metal, metal_src), ImgH(bump, bump_src), ImgH(glow, glow_src));
            if (resize_to_pow2) {
                w = NearestPow2(w);
                h = NearestPow2(h);
            }

            if (smooth_src->is() && (smooth_src->w() != w || smooth_src->h() != h))
                if (smooth_src->copy(smooth_temp, w, h, -1, IMAGE_L8, IMAGE_SOFT, 1, Filter(smooth.filter), (smooth.clamp ? IC_CLAMP : IC_WRAP) | IC_IGNORE_GAMMA))
                    smooth_src = &smooth_temp;
                else
                    goto error;
            if (metal_src->is() && (metal_src->w() != w || metal_src->h() != h))
                if (metal_src->copy(metal_temp, w, h, -1, IMAGE_L8, IMAGE_SOFT, 1, Filter(metal.filter), (metal.clamp ? IC_CLAMP : IC_WRAP) | IC_IGNORE_GAMMA))
                    metal_src = &metal_temp;
                else
                    goto error;
            if (bump_src->is() && (bump_src->w() != w || bump_src->h() != h))
                if (bump_src->copy(bump_temp, w, h, -1, IMAGE_L8, IMAGE_SOFT, 1, Filter(bump.filter), (bump.clamp ? IC_CLAMP : IC_WRAP) | IC_IGNORE_GAMMA))
                    bump_src = &bump_temp;
                else
                    goto error;
            if (glow_src->is() && (glow_src->w() != w || glow_src->h() != h))
                if (glow_src->copy(glow_temp, w, h, -1, IMAGE_L8, IMAGE_SOFT, 1, Filter(glow.filter), (glow.clamp ? IC_CLAMP : IC_WRAP) | IC_IGNORE_GAMMA))
                    glow_src = &glow_temp;
                else
                    goto error;
                ...
                if (!metal_src->is() || metal_src->lockRead()) {
                    if (!bump_src->is() || bump_src->lockRead()) {
                        if (!glow_src->is() || glow_src->lockRead()) {
                            ...
                            REPD(x, dest_2.w()) {
                                if (smooth_src->is()) {
                                    c.BASE_CHANNEL_ROUGH = smooth_src->color(x, y).lum();
                                    if (smooth_is_rough != TEX_IS_ROUGH)
                                        c.BASE_CHANNEL_ROUGH = 255 - c.BASE_CHANNEL_ROUGH;
                                }
                                if (metal_src->is())
                                    c.BASE_CHANNEL_METAL = metal_src->color(x, y).lum();
                                if (bump_src->is())
                                    c.BASE_CHANNEL_BUMP = bump_src->color(x, y).lum();
                                if (glow_src->is())
                                    c.BASE_CHANNEL_GLOW = glow_src->color(x, y).lum();
                                dest_2.color(x, y, c);
                            }
                            glow_src->unlock();
                        }
...
TEX_FLAG ExtractBase0Texture(Image &base_0, Image *color, Image *alpha) {
    ...
    if (alpha)
        alpha->createSoft(base_0.w(), base_0.h(), 1, IMAGE_L8);
    REPD(y, base_0.h())
    REPD(x, base_0.w()) {
...
TEX_FLAG ExtractBase1Texture(Image &base_1, Image *normal) {
    ...
    if (normal) {
        ...
        REPD(x, base_1.w()) {
            Vec4 n;
            n.xy = base_1.colorF(x, y).xy; // #MaterialTextureLayout
            if (Abs(n.x) > 1.5 / 127 || Abs(n.y) > 1.5 / 127)
                tex |= TEXF_NORMAL;
            n.z = CalcZ(n.xy);
            n.xyz = n.xyz * 0.5 + 0.5;
            n.w = 1;
...
TEX_FLAG ExtractBase2Texture(Image &base_2, Image *bump, Image *smooth, Image *metal, Image *glow) {
    ...
    if (metal)
        metal->createSoft(base_2.w(), base_2.h(), 1, IMAGE_L8);
    if (bump)
        bump->createSoft(base_2.w(), base_2.h(), 1, IMAGE_L8);
    if (glow)
        glow->createSoft(base_2.w(), base_2.h(), 1, IMAGE_L8);
    REPD(y, base_2.h())
    REPD(x, base_2.w()) {
        Color c = base_2.color(x, y); // #MaterialTextureLayout
        if (smooth) {
            smooth->pixel(x, y, TexSmooth(c.BASE_CHANNEL_ROUGH));
            if (TexSmooth(c.BASE_CHANNEL_ROUGH) > 1)
                tex |= TEXF_SMOOTH;
        }
        ASSERT(TEX_DEFAULT_SMOOTH == 0);
        if (metal) {
            metal->pixel(x, y, c.BASE_CHANNEL_METAL);
            if (c.BASE_CHANNEL_METAL > 1)
                tex |= TEXF_METAL;
        }
        ASSERT(TEX_DEFAULT_METAL == 0);
        if (bump) {
            bump->pixel(x, y, c.BASE_CHANNEL_BUMP);
            if (Abs(c.BASE_CHANNEL_BUMP - TEX_DEFAULT_BUMP) > 1)
                tex |= TEXF_BUMP;
        }
        if (glow) {
            glow->pixel(x, y, c.BASE_CHANNEL_GLOW);
            if (c.BASE_CHANNEL_GLOW < 254)
                tex |= TEXF_GLOW;
        }
    }
    return tex;
...
TEX_FLAG CreateDetailTexture(Image &detail, C ImageSource &color, C ImageSource &bump, C ImageSource &normal, C ImageSource &smooth, Bool resize_to_pow2, Bool flip_normal_y, Bool smooth_is_rough) {
    ...
    Image dest;
    {
        ...
        C Image *color_src = &color.image;
        if (color_src->compressed())
            if (color_src->copy(color_temp, -1, -1, -1, IMAGE_L8_SRGB, IMAGE_SOFT, 1, FILTER_BEST, IC_IGNORE_GAMMA))
                color_src = &color_temp;
            else
                goto error;
        Image bump_temp;
        ...
        C Image *normal_src = &normal.image;
        if (normal_src->compressed())
            if (normal_src->copy(normal_temp, -1, -1, -1, IMAGE_R8G8, IMAGE_SOFT, 1, FILTER_BEST, IC_IGNORE_GAMMA))
                normal_src = &normal_temp;
            else
                goto error;
        Image smooth_temp;
        ...
        if (normal_src->is())
            texf |= TEXF_DET_NORMAL;
        if (smooth_src->is())
            texf |= TEXF_DET_SMOOTH;

        Int w = Max(ImgW(color, color_src), ImgW(smooth, smooth_src)),
            h = Max(ImgH(color, color_src), ImgH(smooth, smooth_src));
        if (resize_to_pow2) {
            w = NearestPow2(w);
            h = NearestPow2(h);
        }
        ...
        if (bump_to_normal)                  // create normal from bump
        {
            ...
            MAX(w, (normal.size.x > 0) ? normal.size.x : (bump_to_normal == bump_src && bump.size.x > 0) ? bump.size.x
                                                                                                         : bump_to_normal->w());
            MAX(h, (normal.size.y > 0) ? normal.size.y : (bump_to_normal == bump_src && bump.size.y > 0) ? bump.size.y
                                                                                                         : bump_to_normal->h());
            if (resize_to_pow2) {
                w = NearestPow2(w);
                h = NearestPow2(h);
            ...
            normal_src = &normal_temp;
            flip_normal_y = false; // no need to flip since normal map generated from bump is always correct
        } else if (normal_src->is()) {
            MAX(w, ImgW(normal, normal_src));
            MAX(h, ImgH(normal, normal_src));
            if (resize_to_pow2) {
                w = NearestPow2(w);
                h = NearestPow2(h);
            }
        ...
        if (color_src->is() && (color_src->w() != w || color_src->h() != h))
            if (color_src->copy(color_temp, w, h, -1, IMAGE_L8_SRGB, IMAGE_SOFT, 1, Filter(color.filter), (color.clamp ? IC_CLAMP : IC_WRAP) | IC_IGNORE_GAMMA | IC_ALPHA_WEIGHT))
                color_src = &color_temp;
            else
                goto error;
        ...
        if (normal_src->is() && (normal_src->w() != w || normal_src->h() != h))
            if (normal_src->copy(normal_temp, w, h, -1, IMAGE_R8G8, IMAGE_SOFT, 1, Filter(normal.filter), (normal.clamp ? IC_CLAMP : IC_WRAP) | IC_IGNORE_GAMMA))
                normal_src = &normal_temp;
            else
                goto error;
        if (smooth_src->is() && (smooth_src->w() != w || smooth_src->h() != h))
            ...
            if (!bump_src->is() || bump_src->lockRead()) {
                if (!normal_src->is() || normal_src->lockRead()) {
                    if (!smooth_src->is() || smooth_src->lockRead()) {
                        ...
                        REPD(x, dest.w()) {
                            if (color_src->is())
                                detail.DETAIL_CHANNEL_COLOR = color_src->color(x, y).lum();
                            // if(  bump_src->is()) bump                        =  bump_src->color(x, y).lum();
                            if (normal_src->is()) {
                                detail.DETAIL_CHANNEL_NORMAL = normal_src->color(x, y).rg;
                                if (flip_normal_y)
                                    detail.DETAIL_CHANNEL_NORMAL_Y = 255 - detail.DETAIL_CHANNEL_NORMAL_Y;
                            }
                            if (smooth_src->is()) {
                                detail.DETAIL_CHANNEL_ROUGH = smooth_src->color(x, y).lum();
                                if (smooth_is_rough != TEX_IS_ROUGH)
                                    detail.DETAIL_CHANNEL_ROUGH = 255 - detail.DETAIL_CHANNEL_ROUGH;
                            }
...
TEX_FLAG ExtractDetailTexture(C Image &detail, Image *color, Image *bump, Image *normal, Image *smooth) {
    ...
    if (bump)
        bump->del();
    if (normal)
        normal->createSoft(detail.w(), detail.h(), 1, IMAGE_R8G8B8);
    if (smooth)
        smooth->createSoft(detail.w(), detail.h(), 1, IMAGE_L8);
    REPD(y, detail.h())
    REPD(x, detail.w()) {
        Color c = detail.color(x, y); // #MaterialTextureLayoutDetail
        if (color) {
            color->pixel(x, y, c.DETAIL_CHANNEL_COLOR);
            if (Abs(c.DETAIL_CHANNEL_COLOR - 128) > 1)
                tex |= TEXF_DET_COLOR;
        }
        if (smooth) {
            smooth->pixel(x, y, TexSmooth(c.DETAIL_CHANNEL_ROUGH));
            if (Abs(c.DETAIL_CHANNEL_ROUGH - 128) > 1)
                tex |= TEXF_DET_SMOOTH;
        }
        if (normal) {
            Vec n;
            n.xy.set((c.DETAIL_CHANNEL_NORMAL_X - 128) / 127.0, (c.DETAIL_CHANNEL_NORMAL_Y - 128) / 127.0);
            n.z = CalcZ(n.xy);
            normal->color(x, y, Color(c.DETAIL_CHANNEL_NORMAL_X, c.DETAIL_CHANNEL_NORMAL_Y, Mid(Round(n.z * 127 + 128), 0, 255)));
            if (Abs(c.DETAIL_CHANNEL_NORMAL_X - 128) > 1 || Abs(c.DETAIL_CHANNEL_NORMAL_Y - 128) > 1)
                tex |= TEXF_DET_NORMAL;
        }
    }
    return tex;
    ...
    Image dest_0, dest_1, dest_2;
    {
        ...
        if (bump_src->is()) {
            Int w = ImgW(bump, bump_src),
                h = ImgH(bump, bump_src);
            if (resize_to_pow2) {
                w = NearestPow2(w);
                h = NearestPow2(h);
            }
            if (bump_src->is() && (bump_src->w() != w || bump_src->h() != h))
                if (bump_src->copy(bump_temp, w, h, -1, IMAGE_F32, IMAGE_SOFT, 1, Filter(bump.filter), (bump.clamp ? IC_CLAMP : IC_WRAP) | IC_IGNORE_GAMMA))
                    bump_src = &bump_temp;
                else
                    goto error;
            if (!bump_src->is() || bump_src->lockRead()) {
...
Bool CreateBumpFromColor(Image &bump, C Image &color, Flt min_blur_range, Flt max_blur_range, Bool clamp) {
    ...
    if (color_src->compressed())
        if (color_src->copy(color_temp, -1, -1, -1, ImageTypeUncompressed(color_src->type()), IMAGE_SOFT, 1))
            color_src = &color_temp;
        else
            goto error;
    {
        Image bump_temp;
        if (bump_temp.createSoft(color.w(), color.h(), 1, IMAGE_F32)) // operate on temporary in case "&bump==&color", create as high precision to get good quality for blur/normalize
        {
            if (color_src->lockRead()) {
                REPD(y, bump_temp.h())
                REPD(x, bump_temp.w())
                bump_temp.pixF(x, y) = color_src->colorF(x, y).xyz.max();
                color_src->unlock();

                if (min_blur_range < 0)
                    min_blur_range = 0; // auto
                if (max_blur_range < 0)
                    max_blur_range = 3; // auto
                Bool first = true;
                Flt power = 1;
                Image bump_step;
                for (Flt blur = max_blur_range;;) // start with max range, because it's the most important, so we want it to be precise, and then we will go with half steps down
                {
                    ...
                    if (!first) {
                        REPD(y, bump.h())
                        REPD(x, bump.w())
                        bump.pixF(x, y) += bump_step.pixF(x, y) * power;
                    }
                    if (blur <= min_blur_range)
                        break;
                    first = false;
                    blur *= 0.5f;
                    if (blur < 1)
                        blur = 0; // if we reach below 1 blur, then go straight to 0 to avoid doing 0.5, 0.25, 0.125, ..
                    power *= 0.5f;
                }
                bump.normalize();
                return true;
...
Bool MergeBaseTextures(Image &base_0, C Material &material, Int image_type, Int max_image_size, C Vec *light_dir, Flt light_power, Flt spec_mul, FILTER_TYPE filter) { // #MaterialTextureLayout
    ...
    VecI2 size = 0;
    if (material.base_0)
        size = Max(size, material.base_0->size());
    if (material.base_1)
        size = Max(size, material.base_1->size());
    if (material.base_2)
        size = Max(size, material.base_2->size());
    if (max_image_size > 0) {
    ...
    if (size.any()) // this implies we have at least one texture
    {
        ...
        if (has_normal || has_spec) {
            ...
            REPD(x, color.w()) {
                ...
                Vec n;
                n.xy = normal.colorF(x, y).xy * material.normal;
                CHS(n.y);
                n.z = -CalcZ(n.xy);
                n.normalize();
                ...
                if (has_spec)
                    if (Byte s = TexSmooth(b2.color(x, y).BASE_CHANNEL_ROUGH)) // #MaterialTextureLayout
                    {
                        Flt spec = LightSpecular(-n, *light_dir, Vec(0, 0, 1)) * spec_mul;
                        Color cs = ColorBrightness(s * spec);
                        cs.a = 0;
                        col = ColorAdd(col, cs);
                    }
                color.color(x, y, col);
        ...
        if (image_type <= 0) {
            if (material.base_0) {
                ...
                if (has_alpha)
                    image_type = ImageTypeIncludeAlpha(IMAGE_TYPE(image_type)); // convert image type to one with    alpha channel
                else
                    image_type = ImageTypeExcludeAlpha(IMAGE_TYPE(image_type)); // convert image type to one without alpha channel
            } else
                image_type = (has_alpha ? IMAGE_BC7_SRGB : IMAGE_BC1_SRGB);
        }
        if (image_type == IMAGE_PVRTC1_2 || image_type == IMAGE_PVRTC1_4 || image_type == IMAGE_PVRTC1_2_SRGB || image_type == IMAGE_PVRTC1_4_SRGB)
            size = NearestPow2(size.avgI()); // PVRTC1 must be square and pow2
