...
Bool RendererClass::rtCreateMain() // !! call only under lock !!
{
    ...
    if (secondary) {
        ...
        if (!(high_prec && _main_temp.create(_main.size(), IMAGE_F16_4, IMAGE_RT, _main.samples())))
            if (!(_main_temp.create(_main.size(), IMAGE_R8G8B8A8_SRGB, IMAGE_RT, _main.samples())))
                goto error;

        if (!(_main_temp_ds.create(_main.size(), IMAGE_D24S8, IMAGE_DS, _main.samples())))
            goto error;
...
Bool RendererClass::rtCreate() {
    ...
    if (!(_eye_adapt_scale[0].create(1, IMAGE_F32) || _eye_adapt_scale[0].create(1, IMAGE_F16)) || !(_eye_adapt_scale[1].create(1, IMAGE_F32) || _eye_adapt_scale[1].create(1, IMAGE_F16)))
...
void RendererClass::update() {
    if (_t_measure) {
        ...
        if (t > _t_last_measure + 1) {
            ...
            _t_reflection[0] = _t_reflection[1] * mul;
            _t_reflection[1] = 0;
            _t_prepare[0] = _t_prepare[1] * mul;
            _t_prepare[1] = 0;
            _t_opaque[0] = _t_opaque[1] * mul;
            _t_opaque[1] = 0;
            _t_overlay[0] = _t_overlay[1] * mul;
            _t_overlay[1] = 0;
            _t_water[0] = _t_water[1] * mul;
            _t_water[1] = 0;
            _t_light[0] = _t_light[1] * mul;
            _t_light[1] = 0;
            _t_emissive[0] = _t_emissive[1] * mul;
            _t_emissive[1] = 0;
            _t_sky[0] = _t_sky[1] * mul;
            _t_sky[1] = 0;
            _t_water_under[0] = _t_water_under[1] * mul;
            _t_water_under[1] = 0;
            _t_edge_detect[0] = _t_edge_detect[1] * mul;
            _t_edge_detect[1] = 0;
            _t_blend[0] = _t_blend[1] * mul;
            _t_blend[1] = 0;
            _t_palette[0] = _t_palette[1] * mul;
            _t_palette[1] = 0;
            _t_behind[0] = _t_behind[1] * mul;
            _t_behind[1] = 0;
            _t_rays[0] = _t_rays[1] * mul;
            _t_rays[1] = 0;
            _t_volumetric[0] = _t_volumetric[1] * mul;
            _t_volumetric[1] = 0;
            _t_post_process[0] = _t_post_process[1] * mul;
            _t_post_process[1] = 0;
            _t_gpu_wait[0] = _t_gpu_wait[1] / _t_measures[1];
            _t_gpu_wait[1] = 0; // '_t_gpu_wait' has it's own counter (_t_measures[1]) because it's called once per frame, while others can be called multiple times per frame
    ...
    if (main_fbo != was_main_fbo || change_0 || change_ds || !EqualTxtr(_cur[1], t1) || !EqualTxtr(_cur[2], t2) || !EqualTxtr(_cur[3], t3)) {
        ...
        if (main_fbo) {
...
#endif
            {
#if DEBUG_DISCARD
                if (_main._discard) {
                    _main._discard = false;
                    D.clearCol(0, PURPLE);
                } // use indexed version to ignore the viewport
                if (_main_ds._discard) {
                    _main_ds._discard = false;
                    D.clearDS();
                }
#else
                ...
                GLenum attachment[3];
                GLsizei attachments = 0; // RT0+Depth+Stencil
                if (_main._discard) {
                    _main._discard = false;
                    attachment[attachments++] = GL_COLOR;
                }
                if (_main_ds._discard) {
                    _main_ds._discard = false;
                    attachment[attachments++] = GL_DEPTH;
                    if (_main_ds.hwTypeInfo().s)
                        attachment[attachments++] = GL_STENCIL;
                }
                if (attachments)
                    glInvalidateFramebuffer(GL_FRAMEBUFFER, attachments, attachment);
#endif
            }
#endif
        } else
#endif
        {
...
#endif
            {
                if (!set_ds) {
                    ...
                    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, 0);
                    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, 0);
                } else if (_cur_ds_id = set_ds->_txtr) {
                    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, _cur_ds_id, 0);
...
#endif
            {
                if (!t0)
                    ...
                    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, t0->_txtr, 0);
                else
...
#endif
            {
                ...
                if (set_ds && set_ds->_discard) {
                    set_ds->_discard = false;
                    _cur_ds = ds;
                    D.clearDS();
                }
#else
                GLenum attachment[ELMS(_cur) + 1];
                GLsizei attachments = 0; // RT's+DS
                if (t0 && t0->_discard) {
                    t0->_discard = false;
                    attachment[attachments++] = GL_COLOR_ATTACHMENT0;
                }
                if (t1 && t1->_discard) {
                    t1->_discard = false;
                    attachment[attachments++] = GL_COLOR_ATTACHMENT1;
                }
                if (t2 && t2->_discard) {
                    t2->_discard = false;
                    attachment[attachments++] = GL_COLOR_ATTACHMENT2;
                }
                if (t3 && t3->_discard) {
                    t3->_discard = false;
                    attachment[attachments++] = GL_COLOR_ATTACHMENT3;
                }
                if (set_ds && set_ds->_discard) {
                    set_ds->_discard = false;
                    attachment[attachments++] = (set_ds->hwTypeInfo().s ? GL_DEPTH_STENCIL_ATTACHMENT : GL_DEPTH_ATTACHMENT);
                }
                if (attachments)
                    glInvalidateFramebuffer(GL_FRAMEBUFFER, attachments, attachment);
#endif
        ...
        _cur_ds = ds;
        changed = true;
    } else if (_cur_ds != ds) // check this in case 'set_ds' was cleared to null
    {
        _cur_ds = ds;
        changed = true;
#if !IOS // there is no default frame buffer on iOS
