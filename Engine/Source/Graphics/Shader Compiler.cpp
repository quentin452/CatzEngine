    ...
    if (elms <= 1)
        ...
        for (Int e = 1, co = 0, go = 0, t = single_translations; e < elms; e++) // add rest of the elements
        {
            ...
            FREP(single_translations) {
                ...
                DYNAMIC_ASSERT(first.cpu_offset + co == trans.cpu_offset && first.gpu_offset + go == trans.gpu_offset && first.elm_size == trans.elm_size, "ShaderParam has irregular translation");
            }
        }
#endif
    }
...
struct Include11 : ID3DInclude {
    ...
    HRESULT __stdcall Open(D3D_INCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData, LPCVOID *ppData, UINT *pBytes) {
        ...
        *pBytes = 0;
        ...
        File f;
        if (f.readStd(path.tailSlash(true) + GetBase(pFileName))) {
    ...
    virtual HRESULT STDMETHODCALLTYPE LoadSource(_In_ LPCWSTR pFilename, _COM_Outptr_result_maybenull_ IDxcBlob **ppIncludeSource) override {
        if (ppIncludeSource) {
            ...
            if (f.read(pFilename)) {
                ...
                Mems<Byte> file_data(f.left());
                if (f.getFast(file_data.data(), file_data.elms())) {
                    IDxcLibrary *lib = null;
                    CreateLibrary(&lib);
                    if (lib) {
...
void ShaderCompiler::Param::addTranslation(ID3D11ShaderReflectionType *type, C D3D11_SHADER_TYPE_DESC &type_desc, CChar8 *name, Int &offset, SByte &was_min16) // 'was_min16'=if last inserted parameter was of min16 type (-1=no last parameter)
{
    ...
    */
    if (type_desc.Elements)
        offset = Ceil16(offset);                                  // arrays are 16-byte aligned (even 1-element arrays "f[1]"), non-arrays have Elements=0, so check Elements!=0
    ...
    {
        if (type_desc.Rows <= 0 || type_desc.Rows > 4)
            Exit("Invalid Shader Param Rows");
        if (type_desc.Columns <= 0 || type_desc.Columns > 4)
            Exit("Invalid Shader Param Cols");
        ...
        case D3D_SVT_MIN16FLOAT: {
            ...
            if (type_desc.Class != D3D_SVC_MATRIX_COLUMNS)
                ...
                gpu_size = base_size * 4 * (type_desc.Columns - 1) // for matrixes, the lines use  4 X's
            ...
            FREP(elms) {
                if (offset / 16 != (offset + gpu_size - 1) / 16 || (was_min16 >= 0 && was_min16 != (Byte)min16))
                    offset = Ceil16(offset); // "Additionally, HLSL packs data so that it does not cross a 16-byte boundary."

                if (type_desc.Class != D3D_SVC_MATRIX_COLUMNS)
                    translation.New().set(cpu_data_size, offset, cpu_size);
                else { // !! process y's first to add translation sorted by 'cpu_offset' to make later sorting faster !!
                    FREPD(y, type_desc.Rows)
                    FREPD(x, type_desc.Columns)
                    translation.New().set(cpu_data_size + base_size * (x + y * type_desc.Columns), offset + base_size * (y + x * 4), base_size);
                }
        ...
        default:
            Exit(S + "Unhandled Shader Parameter Type for \"" + name + '"');
            break;
        }
    } break;
    ...
    case D3D_SVC_STRUCT:
        FREP(elms) {
            ...
            {
                ID3D11ShaderReflectionType *member = type->GetMemberTypeByIndex(i);
                if (!member)
                    Exit("'GetMemberTypeByIndex' failed");
                D3D11_SHADER_TYPE_DESC member_desc;
                if (!OK(member->GetDesc(&member_desc)))
                    Exit("'ID3D11ShaderReflectionType.GetDesc' failed");
...
void ShaderCompiler::Param::addTranslation(ID3D12ShaderReflectionType *type, C D3D12_SHADER_TYPE_DESC &type_desc, CChar8 *name, Int &offset, Bool true_half) {
    ...
    case D3D_SVC_MATRIX_COLUMNS: // for example: Matrix m,m[];
    {
        ...
        case D3D_SVT_MIN16FLOAT: {
            ...
            Int base_size = ((type_desc.Type == D3D_SVT_DOUBLE) ? SIZE(Dbl) : half ? SIZE(Half)
                                                                                   : SIZE(Flt)),
            ...
            if (type_desc.Class != D3D_SVC_MATRIX_COLUMNS)
                ...
                gpu_size = up_size * 4 * (type_desc.Columns - 1) // for matrixes, the lines use  4 X's
            ...
            FREP(elms) {
                if (!half) {
                    ...
                    if (type_desc.Type == D3D_SVT_DOUBLE)
                        offset = Ceil8(offset); // on GPU Dbl is 8-byte aligned
                    else
                        offset = Ceil4(offset); // on GPU Flt is 4-byte aligned
                }
                if (offset / 16 != (offset + gpu_size - 1) / 16)
                    offset = Ceil16(offset); // "Additionally, HLSL packs data so that it does not cross a 16-byte boundary."
                ...
                if (type_desc.Class != D3D_SVC_MATRIX_COLUMNS)
                    ...
                    FREPD(y, type_desc.Rows)
                    FREPD(x, type_desc.Columns)
                    translation.New().set(cpu_data_size + base_size * (x + y * type_desc.Columns), offset + base_size * y + x * up_size * 4, base_size);
                }
    ...
    case D3D_SVC_STRUCT:
        FREP(elms) {
            ...
            FREP(type_desc.Members)  // iterate all struct members
            {
                ...
                if (!OK(member->GetDesc(&member_desc)))
                    Exit("'ID3D12ShaderReflectionType.GetDesc' failed");
...
void ShaderCompiler::Param::addTranslation(spvc_compiler compiler, spvc_type_id parent_id, spvc_type parent, spvc_type_id var_id, spvc_type var, Int var_i, Int offset, C Str8 &names) {
    ...
    auto array_dimensions = spvc_type_get_num_array_dimensions(var);
    if (array_dimensions > 1)
        Exit("Multi-dimensional arrays are not supported");
    auto array_elms = (array_dimensions ? spvc_type_get_array_dimension(var, 0) : 0); // use 0 for non-arrays to match DX behavior
    unsigned array_stride = 0;
    if (array_elms > 1)
        spvc_compiler_type_struct_member_array_stride(compiler, parent, var_i, &array_stride);
    spvc_basetype type = spvc_type_get_basetype(var);
    Bool is_struct = (type == SPVC_BASETYPE_STRUCT);
    Str8 names_name;
    if (is_struct) {
        names_name.reserve(names.length() + 1 + Length(name));
        names_name = names;
        if (names_name.is())
            names_name += '.';
        names_name += name;
    ...
    if (!is_struct) {
        if (type != SPVC_BASETYPE_FP32 && type != SPVC_BASETYPE_INT32 && type != SPVC_BASETYPE_UINT32)
            Exit(S + "Unhandled Shader Parameter Type for \"" + names + '.' + name + '"');
        auto vec_size = spvc_type_get_vector_size(var),
             cols = spvc_type_get_columns(var);
        if (vec_size <= 0 || vec_size > 4)
            Exit("Invalid Shader Param Vector Size");
        if (cols <= 0 || cols > 4)
            Exit("Invalid Shader Param Columns");
        Int base_size = SIZE(Flt),
        ...
        FREP(elms) {
            if (matrix) // matrix
            {
                ...
                spvc_compiler_type_struct_member_matrix_stride(compiler, parent, var_i, &matrix_stride);
                FREPD(y, cols)
                FREPD(x, vec_size)
                translation.New().set(cpu_data_size + base_size * (x + y * vec_size), offset + base_size * y + x * matrix_stride, base_size);
            } else // scalar, vector
            {
                translation.New().set(cpu_data_size, offset, cpu_size);
            }
        ...
        FREP(elms) {
            FREP(members) {
                ...
                spvc_compiler_type_struct_member_offset(compiler, var, i, &member_offset);
...
void ShaderCompiler::Param::setDataFrom(C Param &src) {
    ...
    if (src.data.elms()) {
        ...
        Memt<Byte> cpu_data;
        cpu_data.setNumZero(src.cpu_data_size);
        Int start = src.translation[0].gpu_offset;
        FREPA(src.translation) {
            C ShaderParam::Translation &trans = src.translation[i];
            Int gpu_offset = trans.gpu_offset - start;
        ...
        FREPA(translation) {
            C ShaderParam::Translation &trans = translation[i];
            Int gpu_offset = trans.gpu_offset - start;
            AssertRange(trans.cpu_offset, trans.elm_size, cpu_data.elms());
...
Bool ShaderCompiler::Param::operator==(C Param &p) C {
    if (translation != p.translation)
        return false;
    if (data.elms() != p.data.elms() || !EqualMem(data.data(), p.data.data(), data.elms()))
        return false;
...
Bool ShaderCompiler::Buffer::operator==(C Buffer &b) C {
    if (params != b.params)
        return false;
    if (bind_explicit && bind_slot != b.bind_slot)
        return false; // check 'bind_slot' only if they are explicit
...
ShaderCompiler::Param &ShaderCompiler::Buffer::getParam(C Str8 &name) {
    ...
    if (!param)
        Exit(S + "Param \"" + name + "\" not found");
    return *param;
}
static Int Compare(C ShaderCompiler::Bind &a, C ShaderCompiler::Bind &b) {
    if (Int c = Compare(a.bind_slot, b.bind_slot))
        return c; // !! it's important to compare by bind slot first, so we can easily merge setting multiple buffers with API calls !!
...
void ShaderCompiler::Shader::finalizeName() {
    FREPA(params) {
        C TextParam8 &p = params[i];
        if (p.value.length() != 1 && i != params.elms() - 1)
            Exit("Shader Param Value Length != 1"); // allow last parameter to have length!=1 because it won't introduce conflicts
        name += p.value;
    }
}
/******************************************************************************/
...
void ShaderCompiler::SubShader::compile() {
    ...
    {
        SHADER_MODEL model = shader->model;
        if (type == ST_HS || type == ST_DS || type == ST_CS)
            MAX(model, SM_5); // HS DS CS are supported only in SM5+ (CS in SM4 too but has limitations)
        if (!get_default_val && compiler->api != API_DX)
            MAX(model, SM_6); // need to use new compiler for DX-> conversions
        switch (model) {
        case SM_4:
            target[3] = '4';
            target[5] = '0';
        ...
        case SM_6_2:
            target[3] = '6';
            target[5] = '2';
            break; // required for Half
        default:
            Exit("Invalid Shader Model");
            break;
        }
        ...
        if (model >= SM_6) {
            ...
            MemtN<DxcDefine, 64> defines;
            defines.setNum(params + API_NUM + get_default_val + 1 + (compiler->api == API_GL) + force_func_name_main);
            Int defs = 0;
            MemtN<Str, 64 * 2> temp;
            temp.setNum(params * 2 + API_NUM + force_func_name_main);
            Int temps = 0;
            FREPA(shader->params) {
                DxcDefine &define = defines[defs++];
                C TextParam8 &param = shader->params[i];
                define.Name = (temp[temps++] = param.name);
                define.Value = (temp[temps++] = param.value);
            }
            FREPA(shader->extra_params) {
                DxcDefine &define = defines[defs++];
                C TextParam8 &param = shader->extra_params[i];
                define.Name = (temp[temps++] = param.name);
                define.Value = (temp[temps++] = param.value);
            }
            FREP(API_NUM) {
            ...
            if (get_default_val) {
                ...
                if (SKIP_OPTIMIZATION)
                    ...
                    arguments.add(DXC_ARG_OPTIMIZATION_LEVEL3); // L"-O3" lvl3 optimizations
                if (compiler->api != API_DX)
                    arguments.add(L"-spirv");
            }

            IDxcBlob *buffer = null;
            IDxcBlobEncoding *error_blob = null;
            IDxcCompiler *dxc_compiler = null;
            CreateCompiler(&dxc_compiler);
            if (dxc_compiler) {
            ...
            if (error_blob) {
                BOOL known = false;
                UINT32 code_page = 0;
                error_blob->GetEncoding(&known, &code_page);
                ...
                default: {
                    Memt<wchar_t> w;
                    w.setNum(size + 1);
                    Int out = MultiByteToWideChar(code_page, 0, (LPCCH)data, size, w.data(), size);
                    w[out] = '\0';
                    error += w.data();
                } break;
                case CP_UTF8: {
            ...
            if (buffer) {
                if (!get_default_val && compiler->api != API_DX) {
                    ...
                    IDxcContainerReflection *container_reflection = null;
                    CreateContainerReflection(&container_reflection);
                    if (container_reflection) {
                        UINT32 part_index;
                        if (OK(container_reflection->Load(buffer)))
                            if (OK(container_reflection->FindFirstPartKind(DXC_PART_DXIL, &part_index))) {
                                ...
                                container_reflection->GetPartReflection(part_index, __uuidof(ID3D12ShaderReflection), (Ptr *)&reflection);
                                if (reflection) {
                                    D3D12_SHADER_DESC desc;
                                    if (!OK(reflection->GetDesc(&desc))) {
                                        error.line() += "'ID3D12ShaderReflection.GetDesc' failed.";
                                    } else {
                                        Int images_elms = 0, rw_images_elms = 0, buffers_elms = 0;
                                        FREP(desc.BoundResources) {
                                            D3D12_SHADER_INPUT_BIND_DESC desc;
                                            if (!OK(reflection->GetResourceBindingDesc(i, &desc))) {
                                                error.line() += "'GetResourceBindingDesc' failed.";
                                                goto error_new;
                                            }
                                            switch (desc.Type) {
                                        ...
                                        images.setNum(images_elms);
                                        images_elms = 0;
                                        rw_images.setNum(rw_images_elms);
                                        rw_images_elms = 0;
                                        buffers.setNum(buffers_elms);
                                        buffers_elms = 0;
                                        FREP(desc.BoundResources) {
                                            D3D12_SHADER_INPUT_BIND_DESC desc;
                                            if (!OK(reflection->GetResourceBindingDesc(i, &desc))) {
                                            ...
                                            switch (desc.Type) {
                                            case D3D_SIT_TEXTURE: {
                                                if (!InRange(desc.BindPoint, MAX_SHADER_IMAGES)) {
                                                    error.line() += S + "Image index: " + desc.BindPoint + ", is too big";
                                                    goto error_new;
                                                }
                                                Image &image = images[images_elms++];
                                                image.name = desc.Name;
                                                image.bind_slot = desc.BindPoint;
                                            } break;

                                            case D3D_SIT_UAV_RWTYPED: {
                                                if (!InRange(desc.BindPoint, MAX_SHADER_IMAGES)) {
                                                    error.line() += S + "RW Image index: " + desc.BindPoint + ", is too big";
                                                    goto error_new;
                                                }
                                                Image &image = rw_images[rw_images_elms++];
                                                image.name = desc.Name;
                                                image.bind_slot = desc.BindPoint;
                                            } break;

                                            case D3D_SIT_CBUFFER: {
                                                if (!InRange(desc.BindPoint, MAX_SHADER_BUFFERS)) {
                                                    error.line() += S + "Constant Buffer index: " + desc.BindPoint + ", is too big";
                                                    goto error_new;
                                                }
                                                ...
                                                if (!cb) {
                                                    error.line() += "'GetConstantBufferByIndex' failed.";
                                                    goto error_new;
                                                }
                                                {
                                                    D3D12_SHADER_BUFFER_DESC desc;
                                                    if (!OK(cb->GetDesc(&desc))) {
                                                        error.line() += "'ID3D12ShaderReflectionConstantBuffer.GetDesc' failed.";
                                                        goto error_new;
                                                    }
                                                    ...
                                                    FREP(desc.Variables) {
                                                        ...
                                                        if (!var) {
                                                            error.line() += "'GetVariableByIndex' failed.";
                                                            goto error_new;
                                                        }
                                                        ID3D12ShaderReflectionType *type = var->GetType();
                                                        if (!type) {
                                                            error.line() += "'GetType' failed.";
                                                            goto error_new;
                                                        }
                                                        D3D12_SHADER_VARIABLE_DESC var_desc;
                                                        if (!OK(var->GetDesc(&var_desc))) {
                                                            error.line() += "'ID3D12ShaderReflectionVariable.GetDesc' failed.";
                                                            goto error_new;
                                                        }
                                                        D3D12_SHADER_TYPE_DESC type_desc;
                                                        if (!OK(type->GetDesc(&type_desc))) {
                                                            error.line() += "'ID3D12ShaderReflectionType.GetDesc' failed.";
                                                            goto error_new;
                                                        }
                                                        ...
                                                        Int offset = var_desc.StartOffset;
                                                        param.addTranslation(type, type_desc, var_desc.Name, offset, model >= SM_6_2);
                                                        param.sortTranslation();
                                                        param.gpu_data_size = offset - var_desc.StartOffset;
                                                        if (!param.translation.elms())
                                                            Exit("Shader Param is empty.\nPlease contact Developer.");
                                                        if (param.gpu_data_size > var_desc.Size || Ceil16(param.gpu_data_size) != Ceil16(var_desc.Size))
                                                            Exit("Incorrect Shader Param size.\nPlease contact Developer."); // FIXME DX Compiler returns padded struct sizes, so it can be a little bigger than actually used 'gpu_data_size' - https://github.com/microsoft/DirectXShaderCompiler/issues/2376
                                                        if (param.translation[0].gpu_offset != var_desc.StartOffset)
                                                            Exit("Incorrect Shader Param offset.\nPlease contact Developer.");
                                                        if (param.gpu_data_size + var_desc.StartOffset > buffer.size)
                                                            Exit("Shader Param does not fit in Constant Buffer.\nPlease contact Developer.");
                                        ...
                                        if (get_default_val) {
                                            images.del();
                                            rw_images.del();
                                        } // we don't need images for default value, only buffers
                                        if (!shader->dummy) {
                                            ...
                                            inputs.setNum(desc.InputParameters);
                                            FREPA(inputs) {
                                                D3D12_SIGNATURE_PARAMETER_DESC desc;
                                                if (!OK(reflection->GetInputParameterDesc(i, &desc)))
                                                    Exit("'GetInputParameterDesc' failed");
                                                inputs[i] = desc;
                                            }
                                            outputs.setNum(desc.OutputParameters);
                                            FREPA(outputs) {
                                                D3D12_SIGNATURE_PARAMETER_DESC desc;
                                                if (!OK(reflection->GetOutputParameterDesc(i, &desc)))
                                                    Exit("'GetOutputParameterDesc' failed");
                                                outputs[i] = desc;
                                            }
                                            ...
                                            if (type == ST_VS) {
                                                Byte *data = (Byte *)buffer->GetBufferPointer();
                                            ...
                                            {
                                                IDxcContainerBuilder *container_builder = null;
                                                CreateContainerBuilder(&container_builder);
                                                if (container_builder) {
                                                    if (OK(container_builder->Load(buffer))) {
                                                        ...
                                                        container_builder->SerializeContainer(&result);
                                                        if (result) {
                                                            HRESULT hr;
                                                            if (OK(result->GetStatus(&hr)))
                                                                if (OK(hr)) {
                                                                    IDxcBlob *stripped = null;
                                                                    result->GetResult(&stripped);
                                                                    if (stripped) {
                                                                        buffer->Release();
                                                                        buffer = stripped;
                                                                    } // swap 'stripped' with 'buffer'
                                                                }
                                                            result->Release();
                                                        }
                                                    }
            ...
            D3DCompile(source->file_data.data(), source->file_data.elms(), (Str8)source->file_name, macros.data(), &NoTemp(Include11(source->file_name)), func_name, target, (get_default_val ? D3DCOMPILE_SKIP_VALIDATION | D3DCOMPILE_SKIP_OPTIMIZATION : SKIP_OPTIMIZATION ? D3DCOMPILE_SKIP_OPTIMIZATION
            ...
            if (buffer) {
                ...
                D3DReflect(buffer->GetBufferPointer(), buffer->GetBufferSize(), IID_ID3D11ShaderReflection, (Ptr *)&reflection);
                if (reflection) {
                    D3D11_SHADER_DESC desc;
                    if (!OK(reflection->GetDesc(&desc))) {
                        error.line() += "'ID3D11ShaderReflection.GetDesc' failed.";
                    } else {
                        Int images_elms = 0, rw_images_elms = 0, buffers_elms = 0;
                        FREP(desc.BoundResources) {
                        ...
                        FREP(desc.BoundResources) {
                            ...
                            case D3D_SIT_CBUFFER: {
                                ...
                                ID3D11ShaderReflectionConstantBuffer *cb = reflection->GetConstantBufferByName(desc.Name);
                                if (!cb) {
                                    error.line() += "'GetConstantBufferByIndex' failed.";
                                    goto error;
                                }
                                {
                                    ...
                                    if (!OK(cb->GetDesc(&desc))) {
                                        error.line() += "'ID3D11ShaderReflectionConstantBuffer.GetDesc' failed.";
                                        goto error;
                                    }
                                    buffer.size = desc.Size;
                                    buffer.params.setNum(desc.Variables);
                                    FREP(desc.Variables) {
                                        ...
                                        if (!OK(var->GetDesc(&var_desc))) {
                                            error.line() += "'ID3D11ShaderReflectionVariable.GetDesc' failed.";
                                            goto error;
                                        }
                                        D3D11_SHADER_TYPE_DESC type_desc;
                                        if (!OK(type->GetDesc(&type_desc))) {
                                            error.line() += "'ID3D11ShaderReflectionType.GetDesc' failed.";
                                            goto error;
                                        }
                                        ...
                                        Int offset = var_desc.StartOffset;
                                        SByte was_min16 = -1;
                                        param.addTranslation(type, type_desc, var_desc.Name, offset, was_min16);
                                        param.sortTranslation();
                                        param.gpu_data_size = offset - var_desc.StartOffset;
                                        if (!param.translation.elms())
                                            Exit("Shader Param is empty.\nPlease contact Developer.");
...
#pragma message("!! Warning: Use this only for debugging !!")
                                ...
                                D3DDisassemble(buffer->GetBufferPointer(), buffer->GetBufferSize(), 0, null, &disassembly);
                                if (disassembly) {
                            ...
                            if (compiler->api == API_DX) // strip
                            {
                                ...
                                D3DStripShader(buffer->GetBufferPointer(), buffer->GetBufferSize(), ~0, &stripped);
                                if (stripped) {
                                    buffer->Release();
                                    buffer = stripped;
                                }
...
#pragma message("!! Warning: Use this only for debugging !!")
                            ID3DBlob *code = null;
                            D3DDisassemble(buffer->GetBufferPointer(), buffer->GetBufferSize(), 0, null, &code);
                            if (code) {
                                ...
                                if (..) {
                                    ...
                                    FREPA(macros) {
                                        ...
                                        if (macro.Name)
                                            source_text += S + "#define " + macro.Name + ' ' + macro.Definition + '\n';
                                    }
                                    source_text += '\n';
                                    if (0) {
                                        FileText f;
                                        f.readMem(source->file_data.data(), source->file_data.elms());
    ...
    if (result == GOOD) {
        ...
        for (Int i = type; --i >= 0;) {
            C SubShader &prev = shader->sub[i];
            if (prev.is()) {
                if (prev.result == GOOD && !Match(prev, T, error))
                    result = FAIL;
                break;
            }
        } // can check only if other shader also completed successfully, stop on first valid sub shader
        for (Int i = type; InRange(++i, ST_NUM);) {
            C SubShader &next = shader->sub[i];
            if (next.is()) {
                if (next.result == GOOD && !Match(T, next, error))
                    result = FAIL;
                break;
            }
        } // can check only if other shader also completed successfully, stop on first valid sub shader
...
/******************************************************************************/
static UShort AsUShort(Int i) {
    RANGE_ASSERT_ERROR(i, USHORT_MAX + 1, "Value too big to be represented as UShort");
    return i;
}
...
Bool ShaderCompiler::Shader::save(File &f, C ShaderCompiler &compiler) C {
    ...
    if (compiler.api != API_GL) {
        ...
        if (compute()) // compute shader
        {
            ...
            FREPAO(indexes) = sub[i]; // !! WARNING: HERE 'sub' HAS MORE ELEMENTS THAN 'indexes' !!
            f << indexes;
        }
        ...
        FREPA(sub) {
            ...
            FREPA(sub.buffers) {
                ...
                Int src_index = compiler.buffers.findValidIndex(buffer.name);
                if (src_index < 0)
                    Exit("Buffer not found in Shader");
...
static Str8 RemoveSpaces(C Str8 &str) {
    ...
    if (str.is()) {
        ...
        CChar8 *text = str;
        for (Char8 last = 0;;) {
            if (text[0] == ' ' && (CharType(last) != CHART_CHAR || CharType(text[1]) != CHART_CHAR) && (preproc ? last != ')' && text[1] != '(' : true) // don't remove spaces around brackets when in preprocessor mode "#define X(a) a" would turn into "#define X(a)a", same thing for "#define X (X+1)" would turn into "#define X(X+1)"
                ...
                s += last;
                if (last == '\n') {
                    possible_preproc = true;
                    preproc = false;
                } else if (last != ' ' && last != '\t' && last != '#')
                    possible_preproc = false;
                else if (last == '#' && possible_preproc)
                    preproc = true;
            }
        }
    }
...
static void Convert(ShaderData &shader_data, ShaderCompiler::SubShader &sub, Int thread_index) {
    ...
    if (word_count * SIZE(SpvId) != shader_data.elms())
        Exit("Incorrect Spir-V size");

    spvc_context context = null;
    spvc_context_create(&context);
    if (!context)
        Exit("Can't create context");
    spvc_context_set_error_callback(context, ErrorCallback, null);
    spvc_parsed_ir ir = null;
    spvc_context_parse_spirv(context, spirv, word_count, &ir);
    if (!ir)
        Exit("'spvc_context_parse_spirv' failed");
    spvc_compiler spirv_compiler = null;
    spvc_context_create_compiler(context, (compiler.api == API_METAL) ? SPVC_BACKEND_MSL : SPVC_BACKEND_GLSL, ir, SPVC_CAPTURE_MODE_TAKE_OWNERSHIP, &spirv_compiler);
    if (!spirv_compiler)
        Exit("'spvc_context_create_compiler' failed");
    ...
    case SpvExecutionModelGLCompute:
        ...
        break;
    default:
        Exit("Invalid Execution model");
        break;
    }

    spvc_resources resources = null;
    spvc_compiler_create_shader_resources(spirv_compiler, &resources);
    const spvc_reflected_resource *list = null;
    size_t count = 0;
    spvc_resources_get_resource_list_for_type(resources, SPVC_RESOURCE_TYPE_UNIFORM_BUFFER, &list, &count);
    Memc<Str8> buffer_instances;
    Memc<ShaderCompiler::Buffer> buffers;
    buffers.setNum((Int)count);
    FREPA(buffers) {
        ShaderCompiler::Buffer &buffer = buffers[i];
        C spvc_reflected_resource &res = list[i];
        auto buffer_handle = spvc_compiler_get_type_handle(spirv_compiler, res.type_id);
        ...
        Str8 &instance_name = buffer_instances.New();
        instance_name = S + "_BufferInstance_" + i;
        ...
        size_t size = 0;
        spvc_compiler_get_declared_struct_size(spirv_compiler, buffer_handle, &size);
        ...
        FREPA(buffer.params) {
            ...
            auto array_dimensions = spvc_type_get_num_array_dimensions(member_handle);
            if (array_dimensions > 1)
                Exit("Multi-dimensional arrays are not supported");
            ...
            unsigned offset = 0;
            spvc_compiler_type_struct_member_offset(spirv_compiler, buffer_handle, i, &offset);

            param.addTranslation(spirv_compiler, res.base_type_id, buffer_handle, member, member_handle, i, offset);
            param.sortTranslation();

            Int member_size = 0;
            if (param.array_elms) {
                unsigned stride = 0;
                spvc_compiler_type_struct_member_array_stride(spirv_compiler, buffer_handle, i, &stride);
                member_size = stride * (param.array_elms - 1); // all elements except last
            }
            // add last element
            if (member_type == SPVC_BASETYPE_STRUCT) {
                size_t struct_size = 0;
                spvc_compiler_get_declared_struct_size(spirv_compiler, member_handle, &struct_size);
                member_size += (Int)struct_size;
            } else {
                ...
                {
                    unsigned stride = 0;
                    spvc_compiler_type_struct_member_matrix_stride(spirv_compiler, buffer_handle, i, &stride);
            ...
            size_t spirv_member_size = 0;
            spvc_compiler_get_declared_struct_member_size(spirv_compiler, buffer_handle, i, &spirv_member_size);
            if (Ceil16(member_size) != Ceil16((UInt)spirv_member_size))
                Exit("Incorrect Shader Param size.\nPlease contact Developer.");

            Int min = INT_MAX, max = 0;
            REPA(param.translation) {
                ShaderParam::Translation &translation = param.translation[i];
                MIN(min, translation.gpu_offset);
                MAX(max, translation.gpu_offset + translation.elm_size);
            } // iterate all translations just for safety (normally checking just the first and last one should be enough)
            param.gpu_data_size = max - min;

            if (!param.translation.elms())
                Exit("Shader Param is empty.\nPlease contact Developer.");
            if (param.gpu_data_size > member_size || Ceil16(param.gpu_data_size) != Ceil16(member_size))
            ...
            if (param.translation[0].gpu_offset != offset || offset != min)
                Exit("Incorrect Shader Param offset.\nPlease contact Developer.");
            if (param.gpu_data_size + offset > buffer.size)
                Exit("Shader Param does not fit in Constant Buffer.\nPlease contact Developer.");
    ...
    if (buffers.elms()) {
        ...
        FREPA(buffers) {
            ...
            if (!dest.name.is())
                ...
                Exit(S + "Buffer \"" + dest.name + "\" is not always the same in all shaders");
        }
    }
    ...
    FREP(count) {
        C spvc_reflected_resource &res = list[i];
        CChar8 *name = spvc_compiler_get_name(spirv_compiler, res.id);
        name = _SkipStart(name, "in_var_");
        Int loc = spvc_compiler_get_decoration(spirv_compiler, res.id, SpvDecorationLocation); // DYNAMIC_ASSERT(loc==i, S+"location!=i "+shader.name);
        // Bool no_persp=spvc_compiler_get_decoration(spirv_compiler, res.id, SpvDecorationNoPerspective);
        if (Starts(name, "ATTR"))
            DYNAMIC_ASSERT(TextInt(name + 4) == loc, "ATTR index!=loc"); // verify vtx input ATTR index
        Set(start, (type == ST_VS) ? "ATTR" : "IO");
        Append(start, TextInt(loc, temp)); // OUTPUT name must match INPUT name, this solves problem when using "UV" and "UV0"
        spvc_compiler_set_name(spirv_compiler, res.id, start);
    ...
    spvc_compiler_create_compiler_options(spirv_compiler, &options);
    if (!options)
        Exit("'spvc_compiler_create_compiler_options' failed");
    switch (compiler.api) {
    case API_GL: {
    ...
    spvc_compiler_build_dummy_sampler_for_combined_images(spirv_compiler, &dummy_sampler); // this is needed when using Texture.Load or Texture[pixel] 'texelFetch' (accessing Texture without a Sampler)
    ...
    FREP(num_samplers) {
        ...
        if (cis.sampler_id == dummy_sampler) // if this is a dummy sampler
        {
            ...
            sampler_index = GetSamplerIndex(sampler_name);
            if (sampler_index < 0)
                Exit(S + "Unknown sampler for '" + image_name + "'");
        }
        spvc_compiler_set_name(spirv_compiler, cis.combined_id, SamplerName(sampler_index, image_name));
        {
            SyncLocker lock(compiler.lock);
            compiler.images.binaryInclude(image_name, CompareCS);
        }
    next:;
    }
    ...
    if (unique_samplers > MAX_SHADER_IMAGES) {
        ...
        FREP(num_samplers) {
            C spvc_combined_image_sampler &cis = samplers[i];
            sampler_info += spvc_compiler_get_name(spirv_compiler, cis.image_id);
            sampler_info += ", ";
            sampler_info += spvc_compiler_get_name(spirv_compiler, cis.sampler_id);
            sampler_info += ", ";
            sampler_info += spvc_compiler_get_name(spirv_compiler, cis.combined_id);
            sampler_info += '\n';
        }
    ...
    FREP(count) {
        ...
        {
            SyncLocker lock(compiler.lock);
            compiler.rw_images.binaryInclude(image_name, CompareCS);
        }
    }

    Str8 code;
    if (compiler.flag & SC_SPIRV) {
        data = shader_data.data();
        size = shader_data.elms();
    } else {
        const char *glsl = null;
        spvc_compiler_compile(spirv_compiler, &glsl);
        code = glsl;
        ...
        for (;;) {
            Int pos = TextPosI(code, "binding=", true, WHOLE_WORD_YES);
            if (pos < 0)
                break;
            Int end = pos + 8;
            Bool removed_prev = (pos > 0 && code[pos - 1] == ',');
            if (removed_prev)
                pos--;
            for (; CharFlag(code[end]) & CHARF_DIG;)
                end++;
            if (!removed_prev && code[end] == ',')
                end++;
...
#pragma message("!! Warning: Use this only for debugging !!")
        ...
        if (!code.length())
            Exit("Can't convert HLSL to GLSL"); // this is also needed for null char below
        data = code();
    ...
    if (COMPRESS_GL_SHADER) // compressed
    {
        ...
        f.readMem(data, size);
        File cmp;
        if (!Compress(f, cmp.writeMem(), COMPRESS_GL_SHADER, COMPRESS_GL_SHADER_LEVEL, false))
            Exit("Can't compress shader data");
        f.del();
        cmp.pos(0);
        shader_data.setNum(cmp.size()).loadRawData(cmp);
    } else // uncompressed
    {
...
struct ImageBindMap : Mems<ShaderCompiler::Image> {
    void operator=(C Mems<ShaderCompiler::Image> &images) {
        setNum(images.elms());
        FREPAO(T) = images[i];
    }
    ...
    Bool save(File &f, C Memc<Str8> &images) C {
        ...
        FREPA(T) {
            ...
            Int src_index;
            if (!images.binarySearch(bind.name, src_index, CompareCS))
                Exit("Image not found in Shader");
...
struct BufferBindMap : Mems<ShaderCompiler::Bind> {
    ...
    void operator=(C Mems<ShaderCompiler::Buffer> &buffers) // !! we shouldn't store buffers with explicit bind slots, because they're always bound at their creation, this will avoid overhead when drawing shaders !!
    {
        Int elms = 0;
        FREPA(buffers)
        elms += !buffers[i].bind_explicit; // skip explicit
        setNum(elms);
        elms = 0;
        FREPA(buffers)
    ...
    Bool operator==(C Mems<ShaderCompiler::Buffer> &buffers) C // !! we shouldn't store buffers with explicit bind slots, because they're always bound at their creation, this will avoid overhead when drawing shaders !!
    {
        Int elms = 0;
        FREPA(buffers) {
            C ShaderCompiler::Buffer &buffer = buffers[i];
            if (!buffer.bind_explicit) // skip explicit
            {
                if (!InRange(elms, T) || T[elms] != buffer)
                    return false;
                elms++;
            }
        }
    ...
    Bool save(File &f, C ShaderCompiler &compiler) C {
        ...
        {
            C ShaderCompiler::Bind &bind = T[i];
            Int src_index = compiler.buffers.findValidIndex(bind.name);
            if (src_index < 0)
                Exit("Buffer not found in Shader");
...
Bool ShaderCompiler::compileTry(Threads &threads) {
    ...
    FREPA(sources) {
        ...
        if (!source.load())
            return error(S + "Can't open file:" + source.file_name);
        FREPA(source.shaders) {
            ...
            shader.finalizeName();
            if (!shader.dummy)
                (shader.compute() ? compute_shaders_num : shaders_num)++;
            FREPA(shader.sub) {
    ...
    Mems<Shader *> shaders(shaders_num), compute_shaders(compute_shaders_num);
    shaders_num = compute_shaders_num = 0;
    FREPA(sources) {
        Source &source = sources[i];
        FREPA(source.shaders) {
            Shader &shader = source.shaders[i];
            FREPA(shader.sub) {
                SubShader &sub = shader.sub[i];
                if (sub.result == FAIL)
                    return error(S + "Compiling \"" + shader.name + "\" in \"" + source.file_name + "\" failed:\n" + sub.error);
                FREPA(sub.buffers) {
                    Buffer &sub_buffer = sub.buffers[i];
                    Buffer &buffer = *buffers(sub_buffer.name);
                    if (!buffer.name.is())
                        buffer = sub_buffer;
                    else if (buffer != sub_buffer)
                        return error(S + "Buffer \"" + buffer.name + "\" is not always the same in all shaders");
                }
                FREPA(sub.images)
                images.binaryInclude(sub.images[i].name, CompareCS);
                FREPA(sub.rw_images)
                rw_images.binaryInclude(sub.rw_images[i].name, CompareCS);

                if (!shader.dummy) {
                    // buffer map
                    FREPA(buffer_maps)
                    if (buffer_maps[i] == sub.buffers) {
                        sub.buffer_bind_index = i;
                        goto got_buffer;
                    } // find same
                    sub.buffer_bind_index = buffer_maps.elms();
                    buffer_maps.add(sub.buffers);
                got_buffer:

                    // image map
                    FREPA(image_maps)
                    if (image_maps[i] == sub.images) {
                        sub.image_bind_index = i;
                        goto got_image;
                    } // find same
                    sub.image_bind_index = image_maps.elms();
                    image_maps.add(sub.images);
                got_image:

                    // rw image map
                    FREPA(rw_image_maps)
                    if (rw_image_maps[i] == sub.rw_images) {
                        sub.rw_image_bind_index = i;
                        goto got_rw_image;
                    } // find same
                    sub.rw_image_bind_index = rw_image_maps.elms();
                    rw_image_maps.add(sub.rw_images);
                got_rw_image:
                    ...
                    if (sub.shader_data.elms()) {
                        ...
                        FREPA(sds)
                        if (sds[i] == sub.shader_data) {
                            sub.shader_data_index = i;
                            goto got_shader_data;
                        } // find same. #RemoveLayoutLocation
                        {
                            sub.shader_data_index = sds.elms();
                            ShaderDataEx &sd = sds.New();                 // add new
            ...
            if (!shader.dummy)
                (shader.compute() ? compute_shaders[compute_shaders_num++]
    ...
    if (api != API_DX) {
        Map<Str8, Buffer> def_val_buffers(CompareCS);
        Swap(buffers, def_val_buffers);

        FREPA(shader_datas) {
            Memc<ShaderDataEx> &sds = shader_datas[i];
            FREPA(sds) {
                ShaderDataEx &sd = sds[i];
                threads.queue(SCAST(ShaderData, sd), Convert, *sd.sub);
            }
        }
        ...
        FREPA(sources) {
            ...
            FREPA(source.shaders) {
                ...
                if (shader.dummy)
                    FREPA(shader.sub) {
                        SubShader &sub = shader.sub[i];
                        ShaderData &sd = sub.shader_data;
                        if (sd.elms())
                            threads.queue(sd, Convert, sub);
                    }
            }
        }
        threads.wait1();
        ...
        REPA(buffers) {
            ...
            if (!src)
                Exit(S + "def_val_buffer \"" + dest.name + "\" not found");
            REPA(dest.params) {
    ...
    if (f.write(dest)) {
        ...
        FREPA(images)
        f.putStr(images[i]);
        f.cmpUIntV(rw_images.elms());
        FREPA(rw_images)
        f.putStr(rw_images[i]);

        // buffer+image map
        if (api != API_GL) {
            if (!buffer_maps.save(f, T))
                goto error;
            if (!image_maps.save(f, images))
                goto error;
            if (!rw_image_maps.save(f, rw_images))
                goto error;
        }

        // shader data
        FREPA(shader_datas) {
            Memc<ShaderData> &shader_data = shader_datas[i];
            if (!shader_data.save(f))
                goto error;
        }
    ...
    error:
        f.del();
        FDelFile(dest);
        error("Can't save data to file");
    } else
        error(S + "Can't write to file \"" + dest + "\"");
    return false;
}
void ShaderCompiler::compile(Threads &threads) {
    if (!compileTry(threads))
        Exit(S + "Failed to compile" + (messages.is() ? ":\n" : null) + messages);
}
/******************************************************************************/
} // namespace EE
/******************************************************************************/
