...
ShaderImage *GetShaderImage(CChar8 *name) {
    ShaderImage *image = FindShaderImage(name);
    if (!image)
        Exit(S + "Shader Image \"" + name + "\" not found.");
    return image;
}
ShaderRWImage *GetShaderRWImage(CChar8 *name) {
    ShaderRWImage *image = FindShaderRWImage(name);
    if (!image)
        Exit(S + "Shader RW Image \"" + name + "\" not found.");
    return image;
}

ShaderParam *FindShaderParam(CChar8 *name) {
    Str8Temp key(name); // use 'Str8Temp' to prevent memory allocation
    if (ShaderParam *sp = ShaderParams.find(key))
        return sp;
    ...
    FREPA(key) {
        ...
        {
            Char8 parent_name[MAX_LONG_PATH];
            if (InRange(i, parent_name)) {
                i++;
                Set(parent_name, name, i); // copy all before '['
                if (ShaderParam *parent = ShaderParams.find(Str8Temp(parent_name))) {
                    CalcValue val;
                    CChar8 *end = TextValue(name + i, val);
                    if (end && end[0] == ']' && end[1] == '\0') // support only 1-dim arrays without further members "val[x]" but not "val[x][y].z"
                    {
                        Int index = val.asInt();
                        if (InRange(index, parent->_elements)) {
                            ShaderParams.lock();
                            ShaderParam &elm = *ShaderParams(key);
                            if (!elm.is())
                                elm.initAsElement(*parent, index); // init only if not initialized yet, in case it just got initialized on another thread before the lock
                            ShaderParams.unlock();
                            return &elm;
                        }
                    }
...
ShaderParam *GetShaderParam(CChar8 *name) {
    ShaderParam *param = FindShaderParam(name);
    if (!param)
        Exit(S + "Shader Param \"" + name + "\" not found.");
    return param;
}
/******************************************************************************/
...
ShaderBuffer *GetShaderBuffer(CChar8 *name) {
    ShaderBuffer *sb = FindShaderBuffer(name);
    if (!sb)
        Exit(S + "Shader Buffer \"" + name + "\" not found.");
    return sb;
}
/******************************************************************************/
...
{                                       // #MaterialTextureLayout
    if (material.base_0) {
        if (material.base_2)
            return 2; // layout 2 assumes we have both 'base_0' and 'base_2'
        return 1;
    }
    return 0;
...
static MESH_FLAG FlagHeightmap(MESH_FLAG mesh_flag, Byte heightmap) {
    if (heightmap) {
        if (mesh_flag & VTX_POS)
            mesh_flag |= VTX_TEX0; // heightmap shaders generate tex from pos
        if (mesh_flag & VTX_NRM)
            mesh_flag |= VTX_TAN; // heightmap shaders generate tan from nrm
    }
    return mesh_flag;
}
/******************************************************************************/
...
void DefaultShaders::init(C Material *material[4], MESH_FLAG mesh_flag, Int lod_index, Byte heightmap) {
    ...
    C Material *m = (material ? material[0] : null);
    if (!m) {
        if (D.drawNullMaterials())
            m = &MaterialDefault;
        else
            goto set_empty;
    }
    ...
    layout = Layout(*m);
    bump = BumpMode(*m, mesh_flag);
    detail = Detail(*m);
    macro = Macro(*m);
    reflect = Reflect(*m);
    emissive = EmissiveMode(*m);
    ...
    {
        materials++;
        MAX(layout, Layout(*material[1]));
        MAX(bump, BumpMode(*material[1], mesh_flag));
        detail |= Detail(*material[1]);
        macro |= Macro(*material[1]);
        reflect |= Reflect(*material[1]);
        MAX(emissive, EmissiveMode(*material[1]));
        if (material[2]) {
            materials++;
            MAX(layout, Layout(*material[2]));
            MAX(bump, BumpMode(*material[2], mesh_flag));
            detail |= Detail(*material[2]);
            macro |= Macro(*material[2]);
            reflect |= Reflect(*material[2]);
            MAX(emissive, EmissiveMode(*material[2]));
#if MAX_MTRLS >= 4
            if (material[3]) {
                materials++;
                MAX(layout, Layout(*material[3]));
                MAX(bump, BumpMode(*material[3], mesh_flag));
                detail |= Detail(*material[3]);
                macro |= Macro(*material[3]);
                reflect |= Reflect(*material[3]);
                MAX(emissive, EmissiveMode(*material[3]));
            }
#endif
        }
    }
    switch (D.texDetail()) {
    case TEX_USE_DISABLE:
        detail = false;
        break;
    case TEX_USE_SINGLE:
        if (materials > 1)
    ...
    if (!D.texMacro() || lod_index <= 0 || skin || !heightmap)
        macro = false; // disable macro  for LODs=0, skin, !heightmaps
    if (!D.texDetailLOD() && !m->detail_all_lod && lod_index > 0)
        detail = false; // disable detail for LODs>0
    if (lod_index > 0 || layout < 2)
        MIN(bump, SBUMP_NORMAL); // limit to normal mapping for LODs>0 and layout<2 (no bump channel)
    if (!uv) {
        layout = 0;
        detail = macro = false;
        MIN(emissive, 1);
    } // disable all textures if we don't have UV
    if (!normal || !D.envMap())
        reflect = false; // reflection requires vtx normals
    if (materials > 1)
        MAX(layout, 1); // multi-materials currently don't support 0 textures
    if (materials > 1 || heightmap)
        emissive = 0; // multi-materials and heightmaps currently don't support emissive
    ...
    fx = (grass ? (m->hasGrass2D() ? FX_GRASS_2D : FX_GRASS_3D) : leaf ? (m->hasLeaf2D() ? (size ? FX_LEAFS_2D : FX_LEAF_2D) : (size ? FX_LEAFS_3D : FX_LEAF_3D))
                                                                       : FX_NONE); // don't set FX_CLEAR_COAT here, because only Deferred shader supports it, and it's processed manually there
    ...
    if (bump == SBUMP_ZERO) { /*materials=1; can't return same shader for multi/single*/
        layout = 0;
        alpha_test = detail = macro = mtrl_blend = uv_scale = false;
        heightmap = HEIGHTMAP_NO;
        fx = FX_NONE;
        MIN(emissive, 1);
    } // shaders with SBUMP_ZERO currently are very limited
    if (fx) {
        uv_scale = detail = macro = tesselate = false;
        MIN(bump, SBUMP_NORMAL);
    } // shaders with effects    currently don't support uv_scale/detail/macro/tesselate/fancy bump
    if (clear_coat) {
        uv_scale = detail = macro = false;
        MIN(bump, SBUMP_NORMAL);
    } // shaders with clear coat currently don't support uv_scale/detail/macro/fancy bump
}
Shader *DefaultShaders::EarlyZ() C {
#if SUPPORT_EARLY_Z
    if (valid && !alpha_blend && !alpha_test && !fx && !tesselate)
        return ShaderFiles("Early Z")->get(ShaderEarlyZ(skin));
#endif
    return null;
}
Shader *DefaultShaders::Opaque(Bool mirror) C {
    if (valid && !alpha_blend && Renderer.anyDeferred()) {
        ...
        Bool detail = T.detail, tesselate = T.tesselate;
        Byte bump = T.bump;
        if (mirror) {
            detail = tesselate = false;
            MIN(bump, SBUMP_NORMAL);
        } // disable detail, tesselation and fancy bump for mirror
...
Shader *DefaultShaders::Overlay() C {
    if (valid && !fx)
        return ShaderFiles("Overlay")->get(ShaderOverlay(skin, tesselate));
    return null;
}
Shader *DefaultShaders::Emissive() C {
#if SUPPORT_EMISSIVE
    if (valid && !alpha_blend && emissive)
        return ShaderFiles("Emissive")->get(ShaderEmissive(skin, color, alpha_test, emissive - 1, fx, tesselate)); // for alpha_blend emissive is in the blend shader, emissive_map=(emissive==2) ? 1 : 0
#endif
    return null;
}
Shader *DefaultShaders::Outline() C {
    if (valid && !alpha_blend && !fx)
        return ShaderFiles("Set Color")->get(ShaderSetColor(skin, color, alpha_test, tesselate));
    return null;
}
Shader *DefaultShaders::Behind() C {
    if (valid && !fx)
        return ShaderFiles("Behind")->get(ShaderBehind(skin, color, alpha_test));
    return null;
}
Shader *DefaultShaders::Fur() C {
    if (valid && fur)
        return ShaderFiles("Fur")->get(ShaderFurSoft(skin, size, layout > 0));
    return null;
}
Shader *DefaultShaders::Shadow() C {
    if (valid && (!alpha_blend || alpha_test))
        return ShaderFiles("Position")->get(ShaderPosition(skin, color, alpha_test, alpha_test && alpha_blend_light, fx, tesselate));
    return null;
}
