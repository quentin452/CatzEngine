...
static Int CompareLevel(C GuiObj &a, C GuiObj &b) {
    if (Int c = Compare(a.baseLevel(), b.baseLevel()))
        return c;
    if (a.isWindow() && b.isWindow())
        if (Int c = Compare(a.asWindow().level(), b.asWindow().level()))
            return c;
    if (a.isMenu() && b.isMenu())
        if (Int c = Compare(b.asMenu().parents(), a.asMenu().parents()))
            return c; // order swapped, sort Menu by parents, so Menus attached to Windows are processed before those attached to Desktop, so Window Menu's process keyboard shortcuts first, and unprocessed shortcuts are checked later by Desktop Menu's.
    return 0;
}
/******************************************************************************/
void GuiObj::zero() {
...
GuiObj &GuiObj::del() {
    ...
    deactivate();
    if (contains(Gui.msSrc()))
        Gui._ms_src = null;
    if (_parent)
        *_parent -= T; // detach from the parent first without clearing members as it may use them
...
GuiObj &GuiObj::create(C GuiObj &src) {
    ...
    if (CAST(ComboBox, this) && CAST(C ComboBox, &src))
        asComboBox().create(src.asComboBox());
    else if (CAST(Tab, this) && CAST(C Tab, &src))
        asTab().create(src.asTab());
    else

        if (CAST(Button, this) && CAST(C Button, &src))
        asButton().create(src.asButton());
    else if (CAST(CheckBox, this) && CAST(C CheckBox, &src))
        asCheckBox().create(src.asCheckBox());
    else if (CAST(GuiCustom, this) && CAST(C GuiCustom, &src))
        asCustom().create(src.asCustom());
    else if (CAST(Desktop, this) && CAST(C Desktop, &src))
        asDesktop().create(src.asDesktop());
    else if (CAST(GuiImage, this) && CAST(C GuiImage, &src))
        asImage().create(src.asImage());
    else if (CAST(_List, this) && CAST(C _List, &src))
        asList().create(src.asList());
    else if (CAST(Menu, this) && CAST(C Menu, &src))
        asMenu().create(src.asMenu());
    else if (CAST(MenuBar, this) && CAST(C MenuBar, &src))
        asMenuBar().create(src.asMenuBar());
    else if (CAST(Progress, this) && CAST(C Progress, &src))
        asProgress().create(src.asProgress());
    else if (CAST(Region, this) && CAST(C Region, &src))
        asRegion().create(src.asRegion());
    else if (CAST(SlideBar, this) && CAST(C SlideBar, &src))
        asSlideBar().create(src.asSlideBar());
    else if (CAST(Slider, this) && CAST(C Slider, &src))
        asSlider().create(src.asSlider());
    else if (CAST(Tabs, this) && CAST(C Tabs, &src))
        asTabs().create(src.asTabs());
    else if (CAST(Text, this) && CAST(C Text, &src))
        asText().create(src.asText());
    else if (CAST(TextBox, this) && CAST(C TextBox, &src))
        asTextBox().create(src.asTextBox());
    else if (CAST(TextLine, this) && CAST(C TextLine, &src))
        asTextLine().create(src.asTextLine());
    else if (CAST(Viewport, this) && CAST(C Viewport, &src))
        asViewport().create(src.asViewport());
    else if (CAST(Window, this) && CAST(C Window, &src))
        asWindow().create(src.asWindow());
    return T;
}
...
void GuiObj::operator-=(GuiObj &child) {
    ...
    switch (type()) {
    case GO_DESKTOP:
        asDesktop().removeChild(child);
        break;
    case GO_LIST:
        asList().removeChild(child);
        break;
    case GO_REGION:
        asRegion().removeChild(child);
        break;
    case GO_TAB:
        asTab().removeChild(child);
        break;
    case GO_TABS:
        asTabs().removeChild(child);
        break;
    case GO_WINDOW:
        asWindow().removeChild(child);
        break;
    }
}
void GuiObj::operator+=(GuiObjs &children) {
    FREPA(children._objs) // add from start to preserve order
    {
        ...
        if (!goi.parent_type)
            if (GuiObj *go = children.go(goi.type, goi.index))
                T += *go; // add all main (which don't have a parent)
    }
}
void GuiObj::operator-=(GuiObjs &children) {
    REPA(children._objs) // remove from end to minimize overhead (moving indexes)
    {
        ...
        if (!goi.parent_type)
            if (GuiObj *go = children.go(goi.type, goi.index))
                T -= *go; // remove all main (which don't have a parent)
    }
}
GuiObjChildren *GuiObj::children() {
    switch (_type) {
    case GO_DESKTOP:
        return &asDesktop()._children;
    case GO_LIST:
        return &asList()._children;
    case GO_REGION:
        return &asRegion()._children;
    case GO_TAB:
        return &asTab()._children;
    case GO_WINDOW:
        return &asWindow()._children;
    }
    return null;
}
...
void GuiObj::notifyParentOfRectChange(C Rect &old_rect, Bool old_visible) {
    DEBUG_BYTE_LOCK(_used);
    if (parent())
        parent()->childRectChanged(old_visible ? &old_rect : null, visible() ? &rect() : null, T);
}
void GuiObj::notifyParentOfRectChange() {
    DEBUG_BYTE_LOCK(_used);
    if (parent())
        parent()->childRectChanged(&Rect(FLT_MAX, -FLT_MAX, FLT_MAX, -FLT_MAX), &Rect(-FLT_MAX, FLT_MAX, -FLT_MAX, FLT_MAX), T); // this will trigger full reset
}
/******************************************************************************/
...
{
    for (; child; child = child->owner())
        if (child == this)
            return true;
    return false;
}
GuiObj *GuiObj::last(GUI_OBJ_TYPE type) {
    GuiObj *last = null;
    for (GuiObj *go = this; go; go = go->owner())
        if (go->type() == type)
            last = go;
    return last;
}
GuiObj *GuiObj::first(GUI_OBJ_TYPE type) {
    for (GuiObj *go = this; go; go = go->owner())
        if (go->type() == type)
            return go;
    return null;
} // !! this method is safe to work with "this==null" upon changing that, fix all calls to this !!
GuiObj *GuiObj::firstNon(GUI_OBJ_TYPE type) {
    for (GuiObj *go = this; go; go = go->owner())
        if (go->type() != type)
            return go;
    return null;
...
GuiObj *GuiObj::firstKbParent() {
    for (GuiObj *go = parent(); go; go = go->parent())
        switch (go->type()) {
        case GO_DESKTOP:
        case GO_LIST:
        case GO_MENU:
...
Region *GuiObj::firstScrollableRegion() {
    for (GuiObj *go = this; go; go = go->parent())
        if (go->isRegion()) {
            Region &region = go->asRegion();
            if (region.slidebar[0]._usable || region.slidebar[1]._usable)
                return &region;
        }
    return null;
}
...
Int GuiObj::parents() C {
    Int parents = 0;
    for (GuiObj *go = owner(); go; go = go->owner())
        parents++;
    return parents;
}
/******************************************************************************/
Bool GuiObj::kbCatch() C {
    ...
    case GO_BUTTON:
        return asButton().focusable() && visible() && enabled();
    case GO_CHECKBOX:
        return asCheckBox().focusable() && visible() && enabled();
    case GO_COMBOBOX:
        return asComboBox().focusable() && visible() && enabled();
    case GO_CUSTOM:
        return asCustom().focusable() && visible() && enabled();
    case GO_SLIDEBAR:
        return asSlideBar().focusable() && visible() && enabled();
    case GO_SLIDER:
        return asSlider().focusable() && visible() && enabled();
    case GO_TAB:
        return asTab().focusable() && visible() && enabled();
    case GO_LIST:
        return asList().focusable() && visible() && enabled();
    default:
        return false;
    }
}
...
GuiObj &GuiObj::kbSet() // this means setting keyboard focus to this element
{
    if (is()) // allow setting focus only if created, to keep consistency with clearing kb focus when deleting object
    {
        ...
        for (GuiObj *go = this; go;) {
            ...
            if (parent) {
                ...
                for (;;) {
                    ...
                    if (cur->isTab() && cur->asTab()._children.kb) {
                        kb = cur;
                        break;
                    } // force setting tab when it owns a child with keyboard focus, do not set 'kb' to child directly, because kb pointers must be only 1 level !!
                    cur = cur->parent();
                    if (cur == parent || !cur) {
                        kb = null;
                        break;
                    } // if reached the parent, or for some reason null, then stop
                }
                switch (parent->type()) {
                case GO_MENU:
                    parent->asMenu()._kb = kb;
                    break;
                case GO_DESKTOP:
                    parent->asDesktop()._children.kb = kb;
                    break;
                case GO_LIST:
                    parent->asList()._children.kb = kb;
                    break;
                case GO_REGION:
                    parent->asRegion()._children.kb = kb;
                    break;
                case GO_TAB:
                    parent->asTab()._children.kb = kb;
                    break;
                case GO_WINDOW:
                    parent->asWindow()._children.kb = kb;
                    break;
                }
            }
            go = parent;
        ...
        for (GuiObj *go = Gui.desktop(); go;) {
            ...
            switch (go->type()) {
            case GO_MENU_BAR: {
                MenuBar &menubar = go->asMenuBar();
                Menu *menu = (InRange(menubar._lit, menubar.elms()) ? &menubar.elm(menubar._lit).menu : null);
                go = menu;
            } break;
            case GO_TABS: {
                Tabs &tabs = go->asTabs();
                Tab *tab = (InRange(tabs(), tabs) ? &tabs.tab(tabs()) : null);
                go = ((tab && (tab->kbCatch() || tab->_children.kb)) ? tab : null);
            } break; // set this tab only if it can catch keyboard focus or has a child with kb focus
            case GO_TAB:
                go = go->asTab()._children.kb;
                break;
            case GO_DESKTOP:
                go = go->asDesktop()._children.kb;
                break;
            case GO_LIST:
                go = go->asList()._children.kb;
                break;
            case GO_REGION:
                go = go->asRegion()._children.kb;
                break;
            case GO_WINDOW:
                go = go->asWindow()._children.kb;
                break;
            case GO_MENU:
                go = go->asMenu()._kb;
                break;
...
GuiObj &GuiObj::kbClear() {
    ...
    if (GuiObj *kb = firstKbParent())
        switch (kb->type()) {
        case GO_DESKTOP: {
            Desktop &desktop = kb->asDesktop();
            if (contains(desktop._children.kb))
                desktop._children.kb = null;
        } break;
        case GO_LIST: {
            _List &list = kb->asList();
            if (contains(list._children.kb))
                list._children.kb = null;
        } break;
        case GO_MENU: {
            Menu &menu = kb->asMenu();
            if (contains(menu._kb))
                menu._kb = &menu.list;
        } break;
        case GO_REGION: {
            Region &region = kb->asRegion();
            if (contains(region._children.kb))
                region._children.kb = null;
        } break;
        case GO_TAB: {
            Tab &tab = kb->asTab();
            if (contains(tab._children.kb))
                tab._children.kb = null;
        } break;
        case GO_WINDOW: {
            Window &window = kb->asWindow();
            if (contains(window._children.kb))
                window._children.kb = null;
        } break;
        }
    ...
    if (contains(Gui.kb())) {
        ...
        for (; Gui.kb() && !Gui.kb()->kbCatch();)
            Gui._kb = Gui.kb()->parent();
        GuiKbChanged();
        Kb.setVisible();
    }
    return T;
...
Int GuiObj::compareLevel(C GuiObj &obj) C {
    if (parent() && obj.parent() == parent()) {
        if (GuiObjChildren *children = parent()->children())
            return children->compareLevel(T, obj);
...
GuiObj &GuiObj::validateLevel() {
    if (parent())
        if (GuiObjChildren *children = parent()->children())
            children->validateLevel(T);
    return T;
}
Bool GuiObj::partiallyOccludedInSameLevel() C {
    if (parent())
        if (GuiObjChildren *children = parent()->children())
            return children->partiallyOccludedInSameLevel(T);
    return false;
}
Bool GuiObj::moveUp() C {
    if (parent())
        if (GuiObjChildren *children = parent()->children())
            return children->moveUp(T);
    return false;
}
Bool GuiObj::moveDown() C {
    if (parent())
        if (GuiObjChildren *children = parent()->children())
            return children->moveDown(T);
    return false;
}
GuiObj &GuiObj::moveToTop() {
    if (parent())
        if (GuiObjChildren *children = parent()->children())
            children->moveToTop(T);
    return T;
}
GuiObj &GuiObj::moveToBottom() {
    if (parent())
        if (GuiObjChildren *children = parent()->children())
            children->moveToBottom(T);
    return T;
}
GuiObj &GuiObj::moveAbove(C GuiObj &obj) {
    if (parent() && obj.parent() == parent())
        if (GuiObjChildren *children = parent()->children())
            children->moveAbove(T, obj);
    return T;
}
GuiObj &GuiObj::moveBelow(C GuiObj &obj) {
    if (parent() && obj.parent() == parent())
        if (GuiObjChildren *children = parent()->children())
            children->moveBelow(T, obj);
    return T;
}
...
Bool GuiObj::visibleFull() C {
    if (hidden())
        return false; // if we're starting from 'Tab' then check this, because 'Tab' visibility is ignored below
    for (C GuiObj *go = this;;) {
        if (go->isTab()) {
            if (go->parent() && go->parent()->isTabs()) // check if parent is 'Tabs' and it's set to that tab
            {
                ...
                if (tabs_tab != go)
                    return false; // if active tab is not the processed one, then it means it's not selected and children should be hidden
            }
...
Bool GuiObj::visibleOnActiveDesktop() C {
    ...
    for (C GuiObj *go = this;;) {
        ...
        C GuiObj *parent = go->parent();
        if (!parent)
            return go == Gui.desktop(); // if no parent, then check if last object is active desktop
        go = parent;
    }
}
...
GuiObj &GuiObj::activate() {
    ...
    for (GuiObj *menu = Gui.menu(); menu && menu->isMenu(); menu = menu->parent())
    ...
    if (isDesktop())
        Gui._desktop = &asDesktop();

    // show
    if (isWindow()) // treat window as a special case because it supports fading
    {
        Window &window = asWindow();
        switch (window._fade_type) {
        case FADE_OUT:
            window.fadeIn();
            break; // if window was fading out then fade in
        case FADE_NONE:
            window.show();
            break; // show immediately
...
GuiObj &GuiObj::deactivate() {
    ...
    if (contains(Gui.menu()))
        Gui._menu = &_parent->first(GO_MENU)->asMenu();
    if (contains(Gui.windowLit()))
        Gui._window_lit = &_parent->first(GO_WINDOW)->asWindow();
    if (contains(Gui.ms()))
        Gui._ms = null;
    if (contains(Gui.msLit()))
        Gui._ms_lit = _parent;
    if (contains(Gui.wheel()))
        Gui._wheel = null;
    if (contains(Gui._overlay_textline))
        Gui._overlay_textline = null;
    if (contains(Gui._desc))
        Gui._desc = null;
    if (contains(Gui._touch_desc))
        Gui._touch_desc = null;
    REPA(Touches) {
        Touch &touch = Touches[i];
        if (contains(touch.guiObj()))
            touch._gui_obj = null;
    }
    return T;
}
...
GuiObj &GuiObj::desc(C Str &desc) {
    T._desc = desc;
    return T;
}
GuiObj &GuiObj::move(C Vec2 &delta) {
    if (delta.any()) {
        Rect old_rect = rect();
        T._rect += delta;
        notifyParentOfRectChange(old_rect, visible());
...
again:
    if (C GuiObj *parent = go->parent()) {
        switch (parent->type()) {
        case GO_WINDOW:
            pos += parent->asWindow().clientRect().lu();
            break;
        case GO_MENU:
            pos += parent->asMenu().clientRect().lu();
            goto finished; // menus are always on top
...
Vec2 GuiObj::clientOffset() C {
    switch (type()) {
    case GO_MENU:
        return asMenu().clientRect().lu() - pos();
    case GO_WINDOW:
        return asWindow().clientRect().lu() - pos();
    case GO_REGION:
        return asRegion().clientRect().lu() - pos();
    case GO_TEXTBOX:
        return asTextBox().clientRect().lu() - pos();
    }
    return 0;
}
Vec2 GuiObj::clientSize() C {
    switch (type()) {
    case GO_MENU:
        return asMenu().clientSize();
    case GO_WINDOW:
        return asWindow().clientSize();
    case GO_REGION:
        return asRegion().clientSize();
    case GO_TEXTBOX:
        return asTextBox().clientSize();
    case GO_TEXTLINE:
        return asTextLine().clientSize();
    default:
        return size();
    }
}
Rect GuiObj::localClientRect() C {
    ...
    if (go)
        ...
        case GO_DESKTOP:
            return go->rect();
        default:
            return Rect_LU(Vec2Zero, go->clientSize());
        }
    return RectZero;
}
...
GuiObj &GuiObj::baseLevel(Int level) {
    if (_base_level != level) {
        _base_level = level;
        validateLevel();
    }
    return T;
...
static Bool Exclude(Rect &rect, C Rect &cover) // adjust 'rect' to don't have any parts belonging to 'cover' !! assumes that rectangles intersect !!
{
    ...
    if (cover.min.x > rect.min.x)
        part[parts++].set(rect.min.x, rect.min.y, cover.min.x, rect.max.y); // set left  part
    if (cover.max.x < rect.max.x)
        part[parts++].set(cover.max.x, rect.min.y, rect.max.x, rect.max.y); // set right part
    if (cover.min.y > rect.min.y)
        part[parts++].set(rect.min.x, rect.min.y, rect.max.x, cover.min.y); // set lower part
    if (cover.max.y < rect.max.y)
        part[parts++].set(rect.min.x, cover.max.y, rect.max.x, rect.max.y); // set upper part
...
void GuiObjNearest::cover(C Rect &rect) {
    REPA(nearest) {
        ...
        if (Cover(obj_rect, rect)) {
            if (Exclude(obj_rect, rect) && obj_rect.area() >= obj.area_min)
                obj.recalc = true; // if still have some rectangle left and its area is big enough, then mark that it needs to be recalculated
            else
                nearest.remove(i); // just remove it
        }
    }
    if (state) // if already encountered starting object, which means that at this stage we can occlude it
        if (Cover(T.rect, rect)) {
            ...
            if (!Exclude(T.rect, rect))
                T.rect = T.plane.pos; // if completely covered then set as a single point
        }
}
...
static Vec2 Delta(C Vec2 &delta, C GuiObjNearest &gon, C Rect &test_rect) {
    ...
    if (move.x && CoverY(start_rect, test_rect))
        return Vec2(DeltaX(start_rect, test_rect), 0); // if moving horizontally and rectangles intersect in Y coordinates then return shortest delta between the rectangles
    if (move.y && CoverX(start_rect, test_rect))
        return Vec2(0, DeltaY(start_rect, test_rect)); // if moving   vertically and rectangles intersect in X coordinates then return shortest delta between the rectangles
    return delta;
}
...
GuiObjNearest::Obj *GuiObjNearest::findNearest() {
    if (nearest.elms()) {
        ...
        REP(T.nearest.elms() - 1) {
            ...
            if (obj_dist_rect < nearest_dist_rect_min || obj_dist_rect <= nearest_dist_rect_max && obj.dist < nearest->dist) {
                nearest = &obj;
...
void GuiObj::nearest(C GuiPC &gpc, GuiObjNearest &gon) {
    if (/*gpc.visible &&*/ visible()) {
        if (gon.obj == this)
            gon.state = 1;
        else // if encountered the starting object, then mark as encountered, and don't process it
        {
...
Bool GuiObjChildren::remove(GuiObj &child) {
    ...
    if (find(child, i)) {
        ...
        if (deactivate)
            child.deactivate();
        else if (child.contains(kb))
            kb = null; // deactivate before clearing '_parent'
        child._parent = null;
        changed = true;
        return true;
    }
...
GuiObjChildren::Child *GuiObjChildren::add(GuiObj &child, GuiObj &parent) {
    ...
    if (old_parent != &parent) {
        ...
        Int l = 0, r = children.elms();
        for (; l < r;) {
            Int m = UInt(l + r) / 2;
            if (CompareLevel(child, *children[m].go) < 0)
                r = m;
            else
                l = m + 1;
        }

        Child &c = children.NewAt(l);
        c.go = &child;
        child._parent = &parent;
        child.parentClientRectChanged(old_parent ? &NoTemp(old_parent->localClientRect()) : null, &NoTemp(parent.localClientRect()));
    ...
    Int index;
    if (find(child, index))
        return &children[index]; // if that's the same parent then return its existing reference
    return null;
}
void GuiObjChildren::validateLevel(C GuiObj &child) {
    Int i;
    if (find(child, i)) {
        for (; InRange(i - 1, children) && CompareLevel(*children[i].go, *children[i - 1].go) < 0; i--) {
            children.swapOrder(i, i - 1);
            changed = true;
        }
        for (; InRange(i + 1, children) && CompareLevel(*children[i].go, *children[i + 1].go) > 0; i++) {
            children.swapOrder(i, i + 1);
...
Int GuiObjChildren::compareLevel(C GuiObj &child_a, C GuiObj &child_b) C {
    Int ia, ib;
    if (find(child_a, child_b, ia, ib))
        return ia - ib; // if both were found
    return 0;
}
Bool GuiObjChildren::partiallyOccludedInSameLevel(C GuiObj &child) {
    ...
    {
        C GuiObj &test = *children[i].go;
        if (&test == &child)
            return false; // found self
        if (test.visible() && Cuts(test.rect(), rect) && CompareLevel(child, test) >= 0)
            return true; // if visible and partially occludes and level of 'test' is not higher (but same or lower), we test the level because we use this function for focusing windows (to know if activating it, and thus moving it to front, would make any difference)
    }
    return false;
}
...
Bool GuiObjChildren::moveUp(C GuiObj &child) {
    Int i;
    if (find(child, i))
        if (InRange(i + 1, children) && CompareLevel(child, *children[i + 1].go) >= 0) {
            children.swapOrder(i, i + 1);
            changed = true;
            return true;
        }
...
void GuiObjChildren::moveToTop(C GuiObj &child) {
    Int i;
    if (find(child, i))
        for (; InRange(i + 1, children) && CompareLevel(child, *children[i + 1].go) >= 0; i++) {
            children.swapOrder(i, i + 1);
            changed = true;
        }
}
...
void GuiObjChildren::moveAbove(C GuiObj &child_a, C GuiObj &child_b) {
    Int ia, ib;
    if (find(child_a, child_b, ia, ib))
        for (; ia < ib && InRange(ia + 1, children) && CompareLevel(child_a, *children[ia + 1].go) >= 0; ia++) {
            children.swapOrder(ia, ia + 1);
            changed = true;
        }
}
...
static Bool ValidForSwitch(GuiObj *go) {
    if (go && go->kbCatch())
        ...
        case GO_BUTTON:
            if (GuiSkin *skin = go->asButton().getSkin())
                return skin->keyboard_highlight_color.a != 0;
            break; // only if there's visual indication of keyboard highlight
...
Bool GuiObjChildren::Switch(C GuiObj &go, Bool next) {
    ...
    if (find(go, i)) {
        ...
        {
            GuiObj *go = T[j % children.elms()];
            if (ValidForSwitch(go)) {
                go->activate();
                if (go->isTextLine() && go->enabled())
                    go->asTextLine().selectAll();
                return true;
            }
        }
    }
