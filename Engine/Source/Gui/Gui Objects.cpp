    ...
    void set(Int index, C GuiObj &go, CChar *name, Memc<C Memx<GuiObj> *> &src, Memc<Str> &names) {
        ...
        if (GuiObj *parent = go.parent()) {
            ...
            if (parent->isTab()) {
                tab = &parent->asTab();
                parent = tab->parent();
            } // use parent of Tab (which is Tabs)
            if (parent)
                if (C Memx<GuiObj> *parent_container = src[parent->type()]) {
                    parent_index = parent_container->validIndex(parent);
                    if (parent_index >= 0) {
                        parent_type = parent->type();
                        if (parent_type == GO_TABS)
                            parent_sub = parent->asTabs()._tabs.validIndex(tab); // get index of Tab in Tabs
                    }
                }
        }
    ...
    if (C GuiObj *ga = a.go)
        if (C GuiObj *gb = b.go) {
            ...
            if (ga == gb)
                return 0;
            if (ga->contains(gb))
                return -1; // if 'a' is parent of 'b', the 'a' must be stored first
            if (gb->contains(ga))
                return +1; // if 'b' is parent of 'a', the 'b' must be stored first

            // set history (add all parents starting from self to root)
            Memc<C GuiObj *> ah;
            for (C GuiObj *g = ga; g; g = g->parent())
                ah.add(g);
            Memc<C GuiObj *> bh;
            for (C GuiObj *g = gb; g; g = g->parent())
                bh.add(g);
            ...
            for (; ah.elms() && bh.elms();) {
                ...
                if (a != b)
                    return a->compareLevel(*b); // return the order in which elements are stored
            }
        }
    return 0;
}
GuiObjs &GuiObjs::create( // create from objects
    ...
    Memc<C Memx<GuiObj> *> src;
    src.setNumZero(GO_NUM);
    ...
    FREPA(button)
    T._button.New().create(button[i]);
    FREPA(checkbox)
    T._checkbox.New().create(checkbox[i]);
    FREPA(combobox)
    T._combobox.New().create(combobox[i]);
    FREPA(menu)
    T._menu.New().create(menu[i]);
    FREPA(custom)
    T._custom.New().create(custom[i]);
    FREPA(desktop)
    T._desktop.New().create(desktop[i]);
    FREPA(image)
    T._image.New().create(image[i]);
    FREPA(list)
    T._list.New().create(list[i]);
    FREPA(menubar)
    T._menubar.New().create(menubar[i]);
    FREPA(progress)
    T._progress.New().create(progress[i]);
    FREPA(region)
    T._region.New().create(region[i]);
    FREPA(slidebar)
    T._slidebar.New().create(slidebar[i]);
    FREPA(slider)
    T._slider.New().create(slider[i]);
    FREPA(tabs)
    T._tabs.New().create(tabs[i]);
    FREPA(text)
    T._text.New().create(text[i]);
    FREPA(textbox)
    T._textbox.New().create(textbox[i]);
    FREPA(textline)
    T._textline.New().create(textline[i]);
    FREPA(viewport)
    T._viewport.New().create(viewport[i]);
    FREPA(window)
    T._window.New().create(window[i]);

    Memc<GuiObjsObj> goi; // create an array of all gui object types (no specific order)
    Memc<Str> names;
    FREPA(button)
    goi.New().set(i, button[i], button[i].name, src, names);
    FREPA(checkbox)
    goi.New().set(i, checkbox[i], checkbox[i].name, src, names);
    FREPA(combobox)
    goi.New().set(i, combobox[i], combobox[i].name, src, names);
    FREPA(menu)
    goi.New().set(i, menu[i], menu[i].name, src, names);
    FREPA(custom)
    goi.New().set(i, custom[i], custom[i].name, src, names);
    FREPA(desktop)
    goi.New().set(i, desktop[i], desktop[i].name, src, names);
    FREPA(image)
    goi.New().set(i, image[i], image[i].name, src, names);
    FREPA(list)
    goi.New().set(i, list[i], list[i].name, src, names);
    FREPA(menubar)
    goi.New().set(i, menubar[i], menubar[i].name, src, names);
    FREPA(progress)
    goi.New().set(i, progress[i], progress[i].name, src, names);
    FREPA(region)
    goi.New().set(i, region[i], region[i].name, src, names);
    FREPA(slidebar)
    goi.New().set(i, slidebar[i], slidebar[i].name, src, names);
    FREPA(slider)
    goi.New().set(i, slider[i], slider[i].name, src, names);
    FREPA(tabs)
    goi.New().set(i, tabs[i], tabs[i].name, src, names);
    FREPA(text)
    goi.New().set(i, text[i], text[i].name, src, names);
    FREPA(textbox)
    goi.New().set(i, textbox[i], textbox[i].name, src, names);
    FREPA(textline)
    goi.New().set(i, textline[i], textline[i].name, src, names);
    FREPA(viewport)
    goi.New().set(i, viewport[i], viewport[i].name, src, names);
    FREPA(window)
    goi.New().set(i, window[i], window[i].name, src, names);
    Int name_length = 0;
    REPA(names)
    name_length += names[i].length() + 1;
    T._names.setNum(name_length);
    name_length = 0;
    FREPA(names) {
        C Str &name = names[i];
        FREPA(name)
        T._names[name_length++] = name[i];
        T._names[name_length++] = '\0';
    }
...
static GuiObj *GetObj(Memc<Memx<GuiObj> *> &objs, Memc<Int> &offset, GUI_OBJ_TYPE type, Int index, Int sub = -1) {
    if (InRange(type, objs) && InRange(type, offset))
        if (Memx<GuiObj> *obj = objs[type]) {
            index += offset[type];
            if (InRange(index, *obj)) {
                GuiObj &go = (*obj)[index];
                if (go.isTabs() && InRange(sub, go.asTabs()))
                    return &go.asTabs().tab(sub);
                return &go;
            }
        }
    return null;
...
void GuiObjs::copyTo( // copy self to object containers
    ...
    GuiObj *parent) C {
    Memc<Memx<GuiObj> *> dest;
    dest.setNumZero(GO_NUM);
    Memc<Int> offset;
    offset.setNumZero(GO_NUM); // offset needed to be applied to object index in the target container, depending on elements present there already (before creating new elements)
    ...
    // create objects in dest containers (order is important, so it matches source indexes)
    FREPA(T._button)
    button.New().create(T._button[i]);
    FREPA(T._checkbox)
    checkbox.New().create(T._checkbox[i]);
    FREPA(T._combobox)
    combobox.New().create(T._combobox[i]);
    FREPA(T._custom)
    custom.New().create(T._custom[i]);
    FREPA(T._desktop)
    desktop.New().create(T._desktop[i]);
    FREPA(T._image)
    image.New().create(T._image[i]);
    FREPA(T._list)
    list.New().create(T._list[i]);
    FREPA(T._menu)
    menu.New().create(T._menu[i]);
    FREPA(T._menubar)
    menubar.New().create(T._menubar[i]);
    FREPA(T._progress)
    progress.New().create(T._progress[i]);
    FREPA(T._region)
    region.New().create(T._region[i]);
    FREPA(T._slidebar)
    slidebar.New().create(T._slidebar[i]);
    FREPA(T._slider)
    slider.New().create(T._slider[i]);
    FREPA(T._tabs)
    tabs.New().create(T._tabs[i]);
    FREPA(T._text)
    text.New().create(T._text[i]);
    FREPA(T._textbox)
    textbox.New().create(T._textbox[i]);
    FREPA(T._textline)
    textline.New().create(T._textline[i]);
    FREPA(T._viewport)
    viewport.New().create(T._viewport[i]);
    FREPA(T._window)
    window.New().create(T._window[i]);
    ...
    FREPA(_objs) // from start to preserve order in containers
    {
        ...
        if (GuiObj *go = GetObj(dest, offset, goi.type, goi.index)) {
            ...
            if (Is(name))
                switch (goi.type) {
                case GO_BUTTON:
                    button[dest_index].name = name;
                    break;
                case GO_CHECKBOX:
                    checkbox[dest_index].name = name;
                    break;
                case GO_COMBOBOX:
                    combobox[dest_index].name = name;
                    break;
                case GO_CUSTOM:
                    custom[dest_index].name = name;
                    break;
                case GO_DESKTOP:
                    desktop[dest_index].name = name;
                    break;
                case GO_IMAGE:
                    image[dest_index].name = name;
                    break;
                case GO_LIST:
                    list[dest_index].name = name;
                    break;
                case GO_MENU:
                    menu[dest_index].name = name;
                    break;
                case GO_MENU_BAR:
                    menubar[dest_index].name = name;
                    break;
                case GO_PROGRESS:
                    progress[dest_index].name = name;
                    break;
                case GO_REGION:
                    region[dest_index].name = name;
                    break;
                case GO_SLIDEBAR:
                    slidebar[dest_index].name = name;
                    break;
                case GO_SLIDER:
                    slider[dest_index].name = name;
                    break;
                case GO_TABS:
                    tabs[dest_index].name = name;
                    break;
                case GO_TEXT:
                    text[dest_index].name = name;
                    break;
                case GO_TEXTBOX:
                    textbox[dest_index].name = name;
                    break;
                case GO_TEXTLINE:
                    textline[dest_index].name = name;
                    break;
                case GO_VIEWPORT:
                    viewport[dest_index].name = name;
                    break;
                case GO_WINDOW:
                    window[dest_index].name = name;
                    break;
                }

            // set parent
            if (GuiObj *p = GetObj(dest, offset, goi.parent_type, goi.parent_index, goi.parent_sub))
                *p += *go;
            else if (parent)
                *parent += *go;
        }
...
MembConst<GuiObj> *GuiObjs::objs(GUI_OBJ_TYPE type) {
    ...
    default:
        return null;
    case GO_BUTTON:
        return &SCAST(MembConst<GuiObj>, _button);
    case GO_CHECKBOX:
        return &SCAST(MembConst<GuiObj>, _checkbox);
    case GO_COMBOBOX:
        return &SCAST(MembConst<GuiObj>, _combobox);
    case GO_CUSTOM:
        return &SCAST(MembConst<GuiObj>, _custom);
    case GO_DESKTOP:
        return &SCAST(MembConst<GuiObj>, _desktop);
    case GO_IMAGE:
        return &SCAST(MembConst<GuiObj>, _image);
    case GO_LIST:
        return &SCAST(MembConst<GuiObj>, _list);
    case GO_MENU:
        return &SCAST(MembConst<GuiObj>, _menu);
    case GO_MENU_BAR:
        return &SCAST(MembConst<GuiObj>, _menubar);
    case GO_PROGRESS:
        return &SCAST(MembConst<GuiObj>, _progress);
    case GO_REGION:
        return &SCAST(MembConst<GuiObj>, _region);
    case GO_SLIDEBAR:
        return &SCAST(MembConst<GuiObj>, _slidebar);
    case GO_SLIDER:
        return &SCAST(MembConst<GuiObj>, _slider);
    case GO_TABS:
        return &SCAST(MembConst<GuiObj>, _tabs);
    case GO_TEXT:
        return &SCAST(MembConst<GuiObj>, _text);
    case GO_TEXTBOX:
        return &SCAST(MembConst<GuiObj>, _textbox);
    case GO_TEXTLINE:
        return &SCAST(MembConst<GuiObj>, _textline);
    case GO_VIEWPORT:
        return &SCAST(MembConst<GuiObj>, _viewport);
    case GO_WINDOW:
        return &SCAST(MembConst<GuiObj>, _window);
    }
}
/******************************************************************************/
GuiObj *GuiObjs::go(GUI_OBJ_TYPE type, Int index, Int sub) {
    if (Memb<GuiObj> *objs = T.objs(type))
        if (InRange(index, *objs)) {
...
GuiObj *GuiObjs::find(C Str &name, GUI_OBJ_TYPE type) {
    ...
    REPA(_objs) {
        C Obj &goi = _objs[i];
        if (goi.type == type && EqualPath(T._names.data() + goi.name_offset, name))
            return go(goi.type, goi.index);
    }
#else // binary search
    Int l = 0, r = _objs.elms();
    for (; l < r;) {
        Int mid = UInt(l + r) / 2;
        Obj &goi = _objs[mid];
        Int compare = ComparePath(name, T._names.data() + goi.name_offset);
        if (!compare)
            compare = type - goi.type;
        if (!compare)
            return go(goi.type, goi.index);
        if (compare < 0)
            r = mid;
...
GuiObj &GuiObjs::get(C Str &name, GUI_OBJ_TYPE type) {
    GuiObj *go = find(name, type);
    if (!go)
        Exit(MLT(S + "Can't find Gui Object \"" + name + '"',
...
Str GuiObjs::name(C GuiObj &go) C {
    ...
    if (C Memb<GuiObj> *objs = T.objects(type))
        REPA(_objs) {
            C Obj &goi = _objs[i];
            if (goi.type == type && InRange(goi.index, *objs) && &(*objs)[goi.index] == &go)
                return T._names.data() + goi.name_offset;
        }
    return S;
}
/******************************************************************************/
...
Bool GuiObjs::load(File &f, CChar *path) {
    ...
    if (f.getUInt() == CC4_GOBJ)
                ...
                if (_objs._loadRaw(f)) {
                    ...
                    REPA(_objs) {
                        if (Memb<GuiObj> *objs = T.objs(GUI_OBJ_TYPE(f.getByte()))) {
                            if (!objs->New().load(f, path))
                                goto error; // load gui object data
                        } else
                            goto error; // invalid Gui Object Type
                    }
        ...
        case 2: {
            ...
            if (old._loadRaw(f)) {
                ...
                FREPA(_objs) {
                    ...
                    dest.name_offset = old_names.elms();
                    Int length = Length(src.name);
                    FREP(length)
                    old_names.add(src.name[i]);
                    old_names.add('\0');
                    dest.type = src.type;
        ...
        case 0: {
            // helper texts
            Mems<Char8> helper;
            if (helper._loadRaw(f)) {
                // Gui Object Info
                Mems<GuiObjsObj0> old;
                if (old._loadRaw(f)) {
                    Memc<Char> old_names;
                    _objs.setNum(old.elms());
                    FREPA(_objs) {
                    ...
                    FREPA(_objs) // from start to preserve order in containers
                    {
                        ...
                        if (GuiObj *go = T.go(goi.type, goi.index)) {
                            if (old[i].desc_pos >= 0)
                                go->desc(helper.data() + old[i].desc_pos);
                            if (GuiObj *parent = T.go(goi.parent_type, goi.parent_index))
                                *parent += *go;
                        }
                    }
...
void GuiObjs::operator=(C Str &name) {
    if (!load(name))
        Exit(MLT(S + "Can't load Gui Objects \"" + name + "\"",
