...
struct QueuedMsgBox // queue these commands for thread-safety, this is so that 'Gui.msgBox' does not require 'Gui.cs' lock, this is important in case for example the main thread is deleting a thread inside a 'Gui.update' callback "Gui.update -> thread.del()" while that thread is "if(thread.wantStop())Gui.msgBox(failed)" calling 'Gui.msgBox' would require 'Gui.cs' lock which is already locked on the main thread
{
    ...
    void set(C Str &title, C Str &text, C TextStylePtr &text_style) {
        T.title = title;
        T.text = text;
...
struct MsgBox : Dialog {
    ...
    static void Del(MsgBox &mb) {
        SyncLocker locker(Gui._lock);
        MsgBoxs.removeData(&mb);
    }
    static void Close(MsgBox &mb) {
        if (Gui.window_fade)
            mb.fadeOut();
        else
            Del(mb);
    ...
    if (Kb.alt()) {
        ...
        if (GuiObj *c = kb())
            if (GuiObj *p = c->parent()) {
                ...
                switch (p->type()) {
                case GO_DESKTOP:
                    return p->asDesktop()._children.Switch(*c, next);
                case GO_REGION:
                    return p->asRegion()._children.Switch(*c, next);
                case GO_TAB:
                    return p->asTab()._children.Switch(*c, next);
                case GO_WINDOW:
                    return p->asWindow()._children.Switch(*c, next);
                case GO_TABS: {
                    Tabs &tabs = p->asTabs();
                    if (InRange(tabs(), tabs))
                        return tabs.tab(tabs())._children.Switch(*c, next);
                } break;
                }
            }
    }
...
struct TextMenuTabs : Tabs {
    virtual void draw(C GuiPC &gpc) override {
      ...
      D.drawShadow(128, r, 0.01f);
        ...
        ShaderParam *sp;
        Vec col;
        D.textBackgroundReset(sp, col); // text menu may be drawn with different style/colors/theme, so reset auto text background
        super::draw(gpc);
...
static void TextSelectAll(Ptr user) {
    ...
    if (GuiObj *go = Gui.kb())
        switch (go->type()) {
        case GO_TEXTLINE:
            if (go->asTextLine().selectAll().cursor() <= 0)
                return;
            break; // if have no selection then return and don't show menu
        case GO_TEXTBOX:
            if (go->asTextBox().selectAll().cursor() <= 0)
                return;
            break; // if have no selection then return and don't show menu
        }
    Gui.showTextMenu();
    ...
    if (GuiObj *go = Gui.kb())
        if (go->isTextEdit()) {
            ...
            case GO_TEXTLINE: {
                ...
                any = tl().is();
                pass = tl.password();
                rect = tl.localSelRect();
                rect.clampFull(Rect_LU(0, 0, tl.clientWidth(), tl.clientHeight()));
                rect += tl.overlayScreenPos();
            } break;
            ...
            case GO_TEXTBOX: {
                ...
                any = tb().is();
                pass = false;
                rect = tb.localSelRect();
                rect.clampFull(Rect_LU(0, 0, tb.clientWidth(), tb.clientHeight()));
                rect += tb.screenPos();
            } break;
            }
            Node<MenuElm> n;
            if (sel) {
                if (pass)
                    n.New().create("Delete", TextCut);
                else {
                    n.New().create("Cut", TextCut);
                    n.New().create("Copy", TextCopy);
                }
            } else if (any)
                n.New().create("Select All", TextSelectAll);
            ...
            if (h_above > h_below + (prefer_up ? -EPS : EPS)) {
                ...
                if (rect.max.y > screen.max.y)
                    rect.moveY(screen.max.y - rect.max.y);
                if (TextMenu.tabs() == 1)
                    TextMenu.tab(0).subType(BUTTON_TYPE_TAB_TOP);
                else if (TextMenu.tabs() > 1) {
                ...
                if (rect.min.y < screen.min.y)
                    rect.moveY(screen.min.y - rect.min.y);
                if (TextMenu.tabs() == 1)
                    TextMenu.tab(0).subType(BUTTON_TYPE_TAB_BOTTOM);
                else if (TextMenu.tabs() > 1) {
...
#else
            TextMenu.create(n).skin(Gui.text_menu_skin);
            TextMenu.list.cur_mode = LCM_MOUSE;
            if (GuiSkin *skin = TextMenu.getSkin())
                if (C PanelPtr &panel = skin->menu.normal) {
                    Rect margin;
                    panel->outerMargin(TextMenu.rect(), margin);
...
void GUI::moveMouse(C Vec2 &dir) C {
    if (Gui.msLit()) {
        ...
        list:
            ...
            if (list->flag & LIST_NEAREST_COLUMN && list->drawMode() == LDM_LIST) {
                VecI2 col_vis = list->nearest2(pos, dir);
                if (col_vis.y >= 0) {
                    list->scrollToCol(col_vis.x).scrollTo(col_vis.y);
                    if (list->scrolling()) {
    ...
    Vec2 pos;
    if (GuiObj *nearest = Gui.objNearest(Ms.pos(), dir, pos))
        Ms.pos(pos);
}
/******************************************************************************/
Color GUI::backgroundColor() C {
    if (GuiSkin *skin = Gui.skin())
        return skin->background_color;
    return WHITE;
}
Color GUI::borderColor() C {
    if (GuiSkin *skin = Gui.skin())
        return skin->border_color;
    return Color(0, 112);
}
/******************************************************************************/
TextLine *GUI::overlayTextLine(Vec2 &offset) {
    Rect kb_rect;
    if (Kb.rect(kb_rect) && kb() && kb()->isTextLine()) {
...
void GUI::msgBox(C Str &title, C Str &text, C TextStylePtr &text_style) {
    ...
    REPA(QueuedMsgBoxs) {
        ...
        if (Equal(qmb.title, title, true) && Equal(qmb.text, text, true) && qmb.text_style == text_style)
            return; // if already exists then do nothing
    }
...
Dialog *GUI::findMsgBox(CPtr id) {
    if (id && (SYNC_LOCK_SAFE || MsgBox::MsgBoxs.elms())) {
        SyncLocker locker(_lock);
        return MsgBox::Find(id);
    }
    return null;
}
void GUI::delMsgBox(CPtr id) {
    if (id && (SYNC_LOCK_SAFE || MsgBox::MsgBoxs.elms())) {
        SyncLocker locker(_lock);
        if (MsgBox *mb = MsgBox::Find(id))
            MsgBox::MsgBoxs.removeData(mb);
    }
}
void GUI::fadeOutMsgBox(CPtr id) {
    if (id && (SYNC_LOCK_SAFE || MsgBox::MsgBoxs.elms())) {
        SyncLocker locker(_lock);
        if (MsgBox *mb = MsgBox::Find(id))
            mb->fadeOut();
...
{
    if (length < _pass_temp.length())
        _pass_temp.clip(length);
    else
        for (; length > _pass_temp.length();)
            _pass_temp += _pass_char;
    return _pass_temp;
}
/******************************************************************************/
...
void GUI::dragCancel() {
    if (_drag_cancel) {
        if (dragging())
            _drag_cancel(_drag_user);
        _drag_cancel = null;
    } // call cancel only if actually started dragging
    _drag_want = _dragging = false;
}
void GUI::drag(C Str &name, Touch *touch) {
    ...
    {
        if (touch)
            touch->disableScroll(); // when dragging is initiated, then disable scrolling
...
void GUI::dragDraw() {
    if (dragging())
        if (Image *image = image_drag())
            image->draw(Rect_D(dragPos(), Vec2(0.05f * image->aspect(), 0.05f)));
}
/******************************************************************************/
void GUI::update() {
    Dbl t = Time.curTime();
    ...
    // test
    auto ovr_tl = overlayTextLine(_overlay_textline_offset);
    if (ovr_tl != _overlay_textline) // TODO: we could also check offset "|| (ovr_tl && ovr_tl_ofs!=_overlay_textline_offset)"
    {
    ...
    BS_FLAG ms_button = BS_NONE;
    REPA(Ms._button)
    ms_button |= Ms._button[i];
    if (!(ms_button & (BS_ON | BS_RELEASED)))
        _ms = _ms_src = msLit();
    if (App.active()) {
#if 1
        if ((ms_button & BS_PUSHED) && msLit())
            msLit()->activate();
#else
        if (Ms.bp(0) && msLit())
            msLit()->activate();
#endif
        REPA(Touches) {
            C Touch &touch = Touches[i];
            BS_FLAG state = touch._state;
            if (touch.scrolling()) // we might want to scroll
            {
                if (state & (BS_RELEASED | BS_TAPPED | BS_LONG_PRESS)) // potential activation
                    if (GuiObj *go = touch.guiObj()) {
                        ...
                        case GO_TEXTLINE:
                        case GO_TEXTBOX:
                            mask = BS_TAPPED | BS_LONG_PRESS;
                            break; // activate TextEdits only on Tap/LongPress, in case we just want to Touch-Scroll, because their activation will show soft keyboard
                        default:
                            mask = BS_RELEASED;
    ...
    if (QueuedMsgBoxs.elms()) {
        ...
        FREPA(QueuedMsgBoxs) // process in order
        {
            ...
            REPA(MsgBox::MsgBoxs) {
                ...
                if (Equal(mb.title, qmb.title, true) && Equal(mb.text.text, qmb.text, true) && mb.text.extra == qmb.extra && mb.text.text_style == qmb.text_style)
                    goto skip; // if already exists then do nothing
            }
            // create new one
    ...
    {
        if (Ms.pixelDelta().allZero())
            _desc = null;
        else // set it only if we've moved the mouse (to eliminate showing description by elements activated with keyboard, touch or programatically)
        {
            _desc = ms();
            _desc_time = Time.appTime();
        }
    } else if (Ms.pixelDelta().any() || (ms_button & (BS_ON | BS_PUSHED | BS_RELEASED)))
        _desc_time = Time.appTime(); // if there was a mouse action then reset the timer

    // touch description
    Touch *stylus = null;
    REPA(Touches) {
        Touch &touch = Touches[i];
        if (touch.stylus() && !touch.on() && !touch.rs()) {
            stylus = &touch;
            break;
        }
    }
...
static void DrawDescriptionObj(GuiObj &obj, C Vec2 &pos, Flt start_time, Bool mouse) {
    ...
    case GO_LIST: {
        ...
        if (list._desc_offset >= 0)
            if (Ptr data = list.screenToData(pos))
                text = *(Char **)((Byte *)data + list._desc_offset);
    } break;
    }
    if (Is(text) && (immediate || Time.appTime() - start_time >= Gui.desc_delay) && Gui.draw_description)
        Gui.draw_description(&obj, pos, text, mouse);
}
/******************************************************************************/
    ...
    if ((Kb.immBuffer().is() || Kb.immCandidate().elms()) && obj && obj->isTextLine())
        if (Gui.skin)
            if (TextStyle *text_style = Gui.skin->imm.text_style()) {
                ...
                TextEdit edit;
                edit.cur = Kb.immCursor();
                if (Kb.immCandidate().elms())
                    edit.sel = Kb.immSelection().y;
                ...
                if (Kb.immCandidate().elms()) {
                    if (t.is())
                        t += '\n';
                    FREPA(Kb.immCandidate())
                    t.space() += S + (i + 1) % 10 + Kb.immCandidate()[i];
                } // use %10 so "10" will be displayed as "0" (because pressing 0 button activates that candidate)
...
void GUI::draw() {
    ...
    if (desktop()) {
        ...
        if (draw_description) {
            if (_desc)
                DrawDescriptionObj(*_desc, Ms.pos(), _desc_time, true);
            if (_touch_desc)
                REPA(Touches) {
                    Touch &touch = Touches[i];
                    if (touch.stylus() && touch.guiObj() == _touch_desc) {
                        DrawDescriptionObj(*_touch_desc, touch.pos(), _touch_desc_time, false);
                        break;
                    }
                }
        }
