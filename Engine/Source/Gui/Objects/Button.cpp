...
static void WindowButtonCheck(Button &button) {
    if (GuiObj *parent = button.parent())
        if (parent->isWindow()) {
            ...
            if (InRange(index, window.button))
                window.setButtons();
        }
}
/******************************************************************************/
...
void Button::call(Bool allow_sound) {
    if (allow_sound && sound && mode != BUTTON_CONTINUOUS)
        switch (_sub_type) // don't play sounds for 'Tab' because they're played by 'Tabs'
        {
...
TextStyle *Button::textParams(Flt &text_size, Flt &text_padd, C Flt *height) C {
    if (GuiSkin *skin = getSkin()) {
        ...
        switch (_sub_type) {
        default:
            text_style = skin->button.text_style();
            text_size = skin->button.text_size;
            text_padd = skin->button.text_padd;
            break; // BUTTON_TYPE_DEFAULT
        case BUTTON_TYPE_COMBOBOX:
            text_style = skin->combobox.text_style();
            text_size = skin->combobox.text_size;
            text_padd = skin->combobox.text_padd;
            break;
        case BUTTON_TYPE_LIST_COLUMN:
            text_style = skin->list.column.text_style();
            text_size = skin->list.column.text_size;
            text_padd = skin->list.column.text_padd;
            break;
        ...
        case BUTTON_TYPE_TAB_BOTTOM_RIGHT:
            text_style = skin->tab.horizontal.text_style();
            text_size = skin->tab.horizontal.text_size;
            text_padd = skin->tab.horizontal.text_padd; // for simplicity return the same tab as they should be all the same
            break;
...
Flt Button::textWidth(C Flt *height, Bool padd) C {
    if (hasData()) {
        ...
        if (TextStyle *text_style = textParams(text_size, text_padd, height)) {
...
Button &Button::fitText() {
    if (hasData()) {
        Flt text_size, text_padd;
        if (TextStyle *text_style = textParams(text_size, text_padd)) {
            TextStyleParams ts = *text_style;
            ts.size = text_size;
#if DEFAULT_FONT_FROM_CUSTOM_SKIN
            if (!ts.font())
            ...
            if (_sub_type == BUTTON_TYPE_COMBOBOX)
                avail_width -= rect().h();
            if (text_width > avail_width)
                T.text_size *= avail_width / text_width;
        }
    }
    return T;
}
Button &Button::fitText(Flt text_size) {
...
Button &Button::setText(C Str &text) {
    T.text = text;
    extra.del();
    return T;
}
Button &Button::setImage(C ImagePtr &image) {
    T.image = image;
    return T;
}
Button &Button::setSkin(C GuiSkinPtr &skin) {
    T.skin = skin;
    return T;
}
Button &Button::subType(BUTTON_TYPE type) {
    T._sub_type = type;
    return T;
}
/******************************************************************************/
...
Button &Button::disabled(Bool disabled) {
    if (T.disabled() != disabled) {
        if (disabled && mode != BUTTON_TOGGLE)
            _on = false;
        super::disabled(disabled);
    }
    return T;
}
Button &Button::focusable(Bool on) {
    if (T._focusable != on) {
        _focusable = on;
        if (!on)
            kbClear();
...
void Button::update(C GuiPC &gpc) {
    ...
    if (enabled) {
        ...
        if (Gui.ms() == this) {
            ...
            if (Ms.bp(0))
                state |= BS_PUSHED;
            if (Ms.b(0) && (mode != BUTTON_DEFAULT || Gui.msLit() == this))
                state |= BS_ON; // mode==BUTTON_DEFAULT requires Gui.msLit()==this while other modes don't
            if (Ms.br(0) && Gui.msLit() == this)
                state |= BS_RELEASED;
        }
        if (Gui.kb() == this) {
            if ((Kb.k(KB_ENTER) || Kb.k(KB_NPENTER)) && Kb.k.first()) {
                state |= BS_PUSHED | BS_RELEASED | BS_ON;
                Kb.eatKey();
            } // 'BS_RELEASED' to work for 'BUTTON_DEFAULT', this is because there's no way to cancel keyboard activation (mouse cursor for example can be moved away before releasing button, while keyboard can't)
            if (mode == BUTTON_CONTINUOUS && (Kb.b(KB_ENTER) || Kb.b(KB_NPENTER)))
                state |= BS_ON;
        }
        REPA(Touches) {
            Touch &t = Touches[i];
            if (t.guiObj() == this) {
                ...
                if (t.scrolling()) // if touch is used for scrolling, then we need to process the button differently
                {
                    ...
                    if (t.on() && (mode != BUTTON_DEFAULT || Gui.objAtPos(t.pos()) == this))
                        state |= BS_ON; // mode==BUTTON_DEFAULT requires Gui.objAtPos(t.pos())==this while other modes don't
                    if (t.rs() && Gui.objAtPos(t.pos()) == this)
                        state |= BS_RELEASED | BS_PUSHED; // process touch release as both BS_RELEASED|BS_PUSHED so it can be used instead of touch pushes
                } else {
                    if (t.pd())
                        state |= BS_PUSHED; // don't 't.eat' because 't.pd' information is needed for Combobox 'ChangedButton'
                    if (t.on() && (mode != BUTTON_DEFAULT || Gui.objAtPos(t.pos()) == this))
                        state |= BS_ON; // mode==BUTTON_DEFAULT requires Gui.objAtPos(t.pos())==this while other modes don't
                    if (t.rs() && Gui.objAtPos(t.pos()) == this)
                        state |= BS_RELEASED;
        ...
        if (lit)
            ...
            MAX(_lit -= Gui._time_d_fade_out, 0);
    }
}
/******************************************************************************/
void Button::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible())
        if (GuiSkin *skin = getSkin()) {
            ...
            switch (_sub_type) {
            default:
                button_skin = &skin->button;
                button_image = null;
                break; // BUTTON_TYPE_DEFAULT
            case BUTTON_TYPE_COMBOBOX:
                button_skin = button_image = &skin->combobox;
                image_x = 1;
                break;
            case BUTTON_TYPE_LIST_COLUMN:
                button_skin = &skin->list.column;
                button_image = null;
                break;
            case BUTTON_TYPE_PROPERTY_VALUE:
                button_skin = button_image = &skin->property.value;
                image_x = 0;
                break;
            case BUTTON_TYPE_REGION_VIEW:
                button_skin = button_image = &skin->region.view;
                image_x = 0;
                break;
            case BUTTON_TYPE_SLIDEBAR_LEFT:
                button_skin = button_image = &skin->slidebar.left;
                image_x = 0;
                break;
            case BUTTON_TYPE_SLIDEBAR_CENTER:
                button_skin = button_image = &skin->slidebar.center;
                image_x = 0;
                break;
            case BUTTON_TYPE_SLIDEBAR_RIGHT:
                button_skin = button_image = &skin->slidebar.right;
                image_x = 0;
                break;
            case BUTTON_TYPE_TAB_LEFT:
                button_skin = &skin->tab.left;
                button_image = null;
                break;
            case BUTTON_TYPE_TAB_HORIZONTAL:
                button_skin = &skin->tab.horizontal;
                button_image = null;
                break;
            case BUTTON_TYPE_TAB_RIGHT:
                button_skin = &skin->tab.right;
                button_image = null;
                break;
            case BUTTON_TYPE_TAB_TOP:
                button_skin = &skin->tab.top;
                button_image = null;
                break;
            case BUTTON_TYPE_TAB_VERTICAL:
                button_skin = &skin->tab.vertical;
                button_image = null;
                break;
            case BUTTON_TYPE_TAB_BOTTOM:
                button_skin = &skin->tab.bottom;
                button_image = null;
                break;
            case BUTTON_TYPE_TAB_TOP_LEFT:
                button_skin = &skin->tab.top_left;
                button_image = null;
                break;
            case BUTTON_TYPE_TAB_TOP_RIGHT:
                button_skin = &skin->tab.top_right;
                button_image = null;
                break;
            case BUTTON_TYPE_TAB_BOTTOM_LEFT:
                button_skin = &skin->tab.bottom_left;
                button_image = null;
                break;
            case BUTTON_TYPE_TAB_BOTTOM_RIGHT:
                button_skin = &skin->tab.bottom_right;
                button_image = null;
                break;
            case BUTTON_TYPE_TEXTLINE_CLEAR:
                button_skin = button_image = &skin->textline.clear;
                image_x = 0;
                break;
            case BUTTON_TYPE_WINDOW_MINIMIZE:
                button_skin = button_image = &skin->window.minimize;
                image_x = 0;
                break;
            case BUTTON_TYPE_WINDOW_MAXIMIZE:
                button_skin = button_image = &skin->window.maximize;
                image_x = 0;
                break;
            case BUTTON_TYPE_WINDOW_CLOSE:
                button_skin = button_image = &skin->window.close;
                image_x = 0;
                break;
            }
            ...
            if (enabled) {
                if (T()) {
                    panel_image = button_skin->pushed();
                    pushed_color = &button_skin->pushed_color;
                } else {
                    panel_image = button_skin->normal();
                    pushed_color = &button_skin->normal_color;
                }
            } else {
                if (T()) {
                    panel_image = button_skin->pushed_disabled();
                    pushed_color = &button_skin->pushed_disabled_color;
                } else {
                    panel_image = button_skin->disabled();
                    pushed_color = &button_skin->disabled_color;
                }
            }
            Rect ext_rect;
            if (panel_image)
                panel_image->extendedRect(r, ext_rect);
            else
                ext_rect = r;
            if (Cuts(ext_rect, gpc.clip)) {
                ...
                if (hasData()) {
                    text_rect = r;
                    if (text_style = textParams(text_size, text_padd)) {
                        text_rect.min.x += text_padd;
                        text_rect.max.x -= text_padd;
                    }
                }
                ...
                if (panel_image) {
                    if (_vertical)
                        panel_image->drawVertical(*pushed_color, highlight, r);
                    else
                        panel_image->draw(*pushed_color, highlight, r);
                ...
                if (hasData() && text_style) {
                    ...
                    C Color *normal_color = &button_skin->normal_color;
                    if (pushed_color != normal_color) {
                ...
                if (button_image)
                    if (C Image *special_image = button_image->image()) {
                        ...
                        if (_vertical) {
                            if (!image_x)
                                special_image->drawFitVertical(col, TRANSPARENT, r);
                            else
                                special_image->drawVertical(col, TRANSPARENT, Rect(r.min.x, r.max.y - special_image->aspect() * r.w(), r.max.x, r.max.y));
                        } else {
                            if (!image_x)
                                special_image->drawFit(col, TRANSPARENT, r);
                            else {
                                Rect rect;
                                rect.setY(r.min.y, r.max.y);
                                Flt w = special_image->aspect() * r.h();
                                if (r.w() >= w)
                                    rect.setX(r.max.x - w, r.max.x); // if image can fit in rectangle
                                else {
                                    Flt c = r.centerX();
                                    w *= 0.5f;
                                    rect.setX(c - w, c + w);
                                } // if image width is bigger than rect width, then draw at the center of the rectangle
...
Bool Button::load(File &f, CChar *path) {
    ...
    if (super::load(f, path))
        ...
        case 5: {
            setParams();
            f >> mode >> image_color >> text_align >> text_size >> _on >> _focusable;
            f._getStr1(text);
        ...
        case 4: {
            ...
            f >> _focusable;
            f.skip(1);
            f >> image_color >> text_align >> text_size;
            f.skip(4);
            f >> _on;
            f._getStr(text);
            f._getStr();
        ...
        case 1: {
            ...
            f.skip(4);
            f >> _on;
            Swap(image_color.r, image_color.b);
            text_size = 1;
            text = f._getStr16();
            f._getStr16();
            if (f.ok())
