...
/******************************************************************************/
CheckBox &CheckBox::focusable(Bool on) {
    if (_focusable != on) {
        _focusable = on;
        if (!on)
            kbClear();
...
void CheckBox::update(C GuiPC &gpc) {
    ...
    if (enabled) {
        if ((Gui.kb() == this && Kb.k(' ')) || (Gui.ms() == this && Ms.bp(0)))
            toggle();
        else // don't check touches if we already toggled it with keyboard/mouse (this is to avoid double presses when using 'SimulateTouches')
            REPA(Touches) {
                Touch &touch = Touches[i];
                if (touch.guiObj() == this)
                    if (touch.scrolling() ? touch.rs() : touch.pd())
                        toggle();
            }
...
void CheckBox::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible())
        if (GuiSkin *skin = getSkin())
            if (PanelImage *panel_image = (_multi ? skin->checkbox.multi() : T() ? skin->checkbox.on()
                                                                                 : skin->checkbox.off())) {
