...
void ColorPicker::SetTextStyle() {
    ColorPickerTS.reset(true).size = 0.04f;
    ColorPickerTS.align.set(1, 0);
}
/******************************************************************************/
...
ColorPicker &ColorPicker::set(C Vec4 &color, SET_MODE mode) {
    _set(color, mode);
    toGui();
    return T;
}
ColorPicker &ColorPicker::setRGB(C Vec &rgb, SET_MODE mode) {
    _setRGB(rgb, mode);
    toGui(true, false, true);
    return T;
}
ColorPicker &ColorPicker::setHSB(C Vec &hsb, SET_MODE mode) {
    _setHSB(hsb, mode);
    toGui(true, false, true);
    return T;
}
ColorPicker &ColorPicker::setAlpha(Flt alpha, SET_MODE mode) {
    _setAlpha(alpha, mode);
    toGui(false, true, false);
    return T;
}
/******************************************************************************/
...
void ColorPicker::SatLum::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible()) {
        ...
        Circle(0.01f, rect.lerp(cp._hsb.y, cp._hsb.z)).draw(WHITE, false);
        D.alpha(alpha);
    }
}
...
void ColorPicker::Hue::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible()) {
        ...
        ALPHA_MODE alpha = D.alpha(ALPHA_INVERT);
        Rect(rect.lerp(0.5f, cp._hsb.x)).extend(rect.w() / 2 + 0.01f, 0.008f).draw(WHITE, false);
        D.alpha(alpha);
    }
}
void ColorPicker::Colors::update(C GuiPC &gpc) {
    if (gpc.visible && gpc.enabled && visible() && enabled()) {
        ...
        set:
            ColorPicker &cp = *(ColorPicker *)user;
            Rect rect = T.rect() + gpc.offset;
            if (Cuts(*pos, Rect(rect.min.x, rect.min.y, rect.max.x, rect.centerY())))
                cp.set(cp._old);
        } else
            REPA(Touches) {
                Touch &touch = Touches[i];
...
/******************************************************************************/
static void ColorRed(ColorPicker &cp, C Str &text) {
    Flt val = TextFlt(text);
    if (!cp._real)
        val /= 255;
    cp._setRGB(Vec(val, cp._rgba.y, cp._rgba.z));
    cp.toGui(false, false, true);
} // update HSB, RGBA
static void ColorGreen(ColorPicker &cp, C Str &text) {
    Flt val = TextFlt(text);
    if (!cp._real)
        val /= 255;
    cp._setRGB(Vec(cp._rgba.x, val, cp._rgba.z));
    cp.toGui(false, false, true);
} // update HSB, RGBA
static void ColorBlue(ColorPicker &cp, C Str &text) {
    Flt val = TextFlt(text);
    if (!cp._real)
        val /= 255;
    cp._setRGB(Vec(cp._rgba.x, cp._rgba.y, val));
    cp.toGui(false, false, true);
} // update HSB, RGBA
static void ColorAlpha(ColorPicker &cp, C Str &text) {
    Flt val = TextFlt(text);
    if (!cp._real)
        val /= 255;
    cp._setAlpha(val);
    cp.toGui(false, false, false);
} // udpate      RGBA
static void ColorHue(ColorPicker &cp, C Str &text) {
    Flt val = TextFlt(text);
    if (!cp._real)
        val /= 255;
    cp._setHSB(Vec(val, cp._hsb.y, cp._hsb.z));
    cp.toGui(true, false, false);
} // update RGB, RGBA
static void ColorSat(ColorPicker &cp, C Str &text) {
    Flt val = TextFlt(text);
    if (!cp._real)
        val /= 255;
    cp._setHSB(Vec(cp._hsb.x, val, cp._hsb.z));
    cp.toGui(true, false, false);
} // update RGB, RGBA
static void ColorLum(ColorPicker &cp, C Str &text) {
    Flt val = TextFlt(text);
    if (!cp._real)
        val /= 255;
    cp._setHSB(Vec(cp._hsb.x, cp._hsb.y, val));
    cp.toGui(true, false, false);
} // update RGB, RGBA
static void ColorRGBA(ColorPicker &cp, C Str &text) {
    Color c;
    c.fromHex(text);
    cp._set(c);
    cp.toGui(true, true, true, false);
} // update RGB, HSB, ALPHA
...
ColorPicker &ColorPicker::create(C Str &name) {
    ...
    Gui += super::create(Rect_C(0, 0, 1.01f, 0.69f), S + "Color Picker" + (name.is() ? S + " - \"" + name + '"' : S));
    T.button[2].show();
    ...
    T += _mode.create(Rect_U(Avg(_color.rect().min.x, clientWidth() - 0.04f), _color.rect().min.y - 0.01f, 0.2f, 0.04f), "Byte / Real").func(Mode, T).focusable(false);
    _mode.mode = BUTTON_TOGGLE;
