...
static void MenuPushed(C CMemPtr<MenuPush> &path, ComboBox &cb) {
    if (path.elms())
        cb.setAbsText(path.first().abs, path.last().name);
}
static void ChangedButton(ComboBox &cb) {
    if (cb.Button::operator()()) {
        ...
        REPA(Touches) {
            Touch &t = Touches[i];
            if ((t.state() & (BS_PUSHED | BS_RELEASED)) && t.guiObj() == &cb) {
                touch = true;
                break;
            }
        }
        cb.menu.setSize(touch); // can be pushed with BS_PUSHED or BS_RELEASED depending on 't.scrolling'
        cb.menu.list.cur = -1;
    ...
    if (!(flag & COMBOBOX_CONST_TEXT)) {
        if (absSel() >= 0)
            if (ListColumn *lc = menu.listColumn()) {
                text = lc->md.asText(menu.list.absToData(absSel()));
                extra.clear();
                goto end;
            }
        text.clear();
...
ComboBox &ComboBox::set(Int abs_sel, SET_MODE mode) {
    if (flag & COMBOBOX_SET_CLAMP)
        MIN(abs_sel, menu.list.totalElms() - 1); // clamp to last value
    if (menu.list.absToVis(abs_sel) < 0)
        abs_sel = -1; // disable selecting hidden and out of range elements
    if (T._abs_sel != abs_sel) {
        ...
        resetText(); // if we're not changing selection, the element's text can be now different, so set it in case it got changed
    return T;
}
ComboBox &ComboBox::setText(C Str &text, Bool force, SET_MODE mode) {
    ...
    if (ListColumn *lc = menu.listColumn()) {
        REPA(menu.list)
        if (Equal(lc->md.asText(menu.list.visToData(i), lc->precision), text) && !extra.is()) {
...
ComboBox &ComboBox::setAbsText(Int abs, C Str &text) {
    StrEx extra;
    if (menu.list.absToVis(abs) < 0)
        abs = -1; // disable selecting hidden and out of range elements
...
static void Next(ComboBox &cb, Int delta) {
    if (Int sign = Sign(delta))
        for (Int i = cb.visSel() + sign; InRange(i, cb); i += sign) {
            if (InRange(i, cb.menu.elms())) {
                MenuElm &elm = cb.menu.elm(i);
                if (elm.disabled || (elm.flag() & MENU_NOT_SELECTABLE) || elm.menu())
                    continue; // if element is disabled, can't be selected or has children (normally we'd need to select one of the children, but for simplicity let's just skip it)
            }
            if (cb.menu._selectable_offset >= 0)
                if (Ptr data = cb.menu.list.visToData(i))
                    if (!*(Bool *)((Byte *)data + cb.menu._selectable_offset))
                        continue;
            if (delta -= sign)
                continue; // want to keep going
...
void ComboBox::update(C GuiPC &gpc) {
    ...
    if (gpc_this.enabled) {
        ...
        if (gpc_this.visible) {
            ...
            if (Gui.wheel() == this)
                Next(T, -Ms.wheelI());
            if (menu_active && !menu.contains(Gui.menu()))
                super::set(false, QUIET);
...
Bool ComboBox::load(File &f, CChar *path) {
...

    {
        ...
        if (ver < 3)
            goto old;
    } // in the past the base class of ComboBox was GuiObj, so detect that and jump to old load if needed, '3' is the GuiObj save file version from the past
