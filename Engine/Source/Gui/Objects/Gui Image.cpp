...
/******************************************************************************/
GuiImage &GuiImage::set(C ImagePtr &image) {
    T.image = image;
    return T;
}
/******************************************************************************/
    ...
    if (/*gpc.visible &&*/ visible()) {
        ...
        if (Cuts(rect, gpc.clip)) {
            ...
            if (image) {
                ...
                case FIT_NONE:
                    D.clip(gpc.clip);
                    image->draw(color, color_add, rect);
                    break;
                case FIT_FULL:
                    D.clip(gpc.clip);
                    image->drawFit(color, color_add, rect, fit);
                    break;
                default:
                    D.clip(gpc.clip & rect);
                    image->drawFit(color, color_add, rect, fit);
                    break;
                }
                D.alpha(alpha);
...
Bool GuiImage::load(File &f, CChar *path) {
    ...
    if (super::load(f, path))
        ...
        case 2: {
            ...
            f >> alpha_mode >> rect_color >> color >> color_add;
            Swap(rect_color.r, rect_color.b);
            Swap(color.r, color.b);
            Swap(color_add.r, color_add.b);
            image.require(f._getStr(), path);
            if (f.ok())
                return true;
        } break;
