...
static Int NodeElms(_Memx &node, Int children_offset) {
    Int elms = 0;
    FREPA(node)
    elms += 1 + NodeElms(NodeChildren(node[i], children_offset), children_offset); // increase self + children of self
    return elms;
}
static void NodeVisible(_Memx &node, Int children_offset, Ptr *&abs_to_data, Ptr *&vis_to_data, C CMemPtr<Bool> &visible, Int &index) {
    FREPA(node) {
        Ptr data = node[i];
        *abs_to_data++ = data;
        if (!visible || ElmIs(visible, index))
            *vis_to_data++ = data;
...
static Ptr NodeAbsToData(_Memx &node, Int children_offset, Int &abs) {
    FREPA(node) {
        ...
        if (!abs--)
            return e;
        if (e = NodeAbsToData(NodeChildren(e, children_offset), children_offset, abs))
            return e;
    }
    return null;
}
static Int NodeDataToAbs(_Memx &node, Int children_offset, Ptr data, Int &abs) {
    FREPA(node) {
        ...
        if (e == data)
            return abs;
        abs++;
        Int j = NodeDataToAbs(NodeChildren(e, children_offset), children_offset, data, abs);
        if (j >= 0)
            return j;
    }
    return -1;
        ...
        if (!(list.drawMode() == LDM_RECTS && abs_col.y != list.draw_column))                                           // for 'LDM_RECTS' we draw only the 'draw_column'
            if (!(list.drawMode() == LDM_LIST && InRange(abs_col.y, list._columns) && list.column(abs_col.y).hidden())) // for 'LDM_LIST' don't draw hidden columns (do this check only for existing columns, but not when we want after all columns)
            {
                ...
                if (vis >= 0 && vis >= visible_range.x && vis <= visible_range.y) {
                    ...
                    if (list.drawMode() == LDM_LIST)
                        gpc.offset.x += list.columnOffset(abs_col.y);
                    return true;
                }
            }
    return false;
...
void ListColumn::create(Str (*data_to_text)(CPtr data), Flt width, C Str &name) {
    ...
    if (data_to_text && !T.md.type)
        T.md.type = DATA_CHAR_PTR;
    T._resize_edge = 0;
...
void ListColumn::pushed() {
    if (_List *list = CAST(_List, parent())) {
        ...
        if (index >= 0) {
            ...
            {
                if (list->flag & LIST_SORTABLE)
                    list->sort(index); // check for sorting
            }
        }
    }
}
...
void ListColumn::create(C ListColumn &src, _List &list) {
    ...
    if (md.type == DATA_IMAGE || md.type == DATA_IMAGE_PTR || md.type == DATA_IMAGEPTR) {
        Int index = list._columns.index(this);
        if (index >= 0)
            list.draw_column = index;
    }
}
/******************************************************************************/
static Int ResizeColumn(_List *list, ListColumn &column, Int &resize_edge) {
    if (resize_edge) {
        if (!list)
            ...
            Int i = list->_columns.index(&column);
            if (i >= 0) {
                if (resize_edge == -1)
                    for (i--; i >= 0; i--)
                        if (list->_columns[i].visible())
                            break; // find first previous visible column
                if (!InRange(i, list->_columns))
                    resize_edge = 0;
                return i;
            }
        }
...
void ListColumn::update(C GuiPC &gpc) {
    if (Gui.ms() == this) {
        ...
        if (Ms.b(0) || Ms.br(0)) // resize
        {
            ...
            if (list && InRange(col, list->_columns)) {
                ...
                } else if (Flt d = Ms.dc().x)
                    list->columnWidth(col, Max(MIN_COLUMN_WIDTH, list->columnWidth(col) + d));
            }
        } else {
            _resize_edge = 0;
            if (list && (list->flag & LIST_RESIZABLE_COLUMNS)) {
                if (Ms.pos().x <= rect().min.x + gpc.offset.x + COLUMN_RESIZE_WIDTH)
                    _resize_edge = -1;
                else if (Ms.pos().x >= rect().max.x + gpc.offset.x - COLUMN_RESIZE_WIDTH)
                    _resize_edge = +1;
            }
    ...
    if (_resize_edge)
        super::update(GuiPC(gpc, true, false));
    else
        super::update(gpc);
}
/******************************************************************************/
Flt _List::parentWidth() C {
    if (GuiObj *parent = T.parent()) {
        if (parent->isMenu()) {
            if (GuiObj *owner = parent->asMenu().Owner())
                if (owner->isComboBox())
                    return owner->clientSize().x;
        }

        if (parent->isRegion())
            return parent->asRegion().minClientWidth();
        else
            return parent->clientSize().x;
    }
    return 0;
}
        ...
        if (GuiSkin *skin = getSkin())
            if (TextStyle *text_style = skin->list.text_style()) {
                ...
                REP(visible ? visibleElms() : totalElms()) {
                    ...
                    switch (lc.md.type) {
                    case DATA_STR_EX: {
                        StrEx &sx = *(StrEx *)((Byte *)data + lc.md.offset);
                        elm_width = ts.textWidth(null, sx.data(), sx.elms());
                    } break;
                    default:
                        elm_width = ts.textWidth(lc.md.asText(data, lc.precision));
                        break;
                    }
                    if (_offset_offset >= 0)
                        elm_width += *(Flt *)((Byte *)data + _offset_offset);
...
Flt _List::columnDataWidthEx(Int i) C {
    ...
    if (InRange(i, _columns)) {
        ...
        if (lc.visible()) {
            ...
            MAX(w, lc.textWidth(&col_height));
        } // specify 'columnHeight' because 'ListColumn' height may not be set yet
        w += columnPadding() * 2;
    }
    return w;
}
Flt _List::columnWidthActual(Int i) C {
    if (InRange(i, _columns)) {
        ...
        {
            if (Equal(lc.width, LCW_DATA, 0.5f))
                return columnDataWidthEx(i);
            if (Equal(lc.width, LCW_PARENT, 0.5f))
                return parentWidth();
            if (Equal(lc.width, LCW_MAX_DATA_PARENT, 0.5f))
                return Max(columnDataWidthEx(i), parentWidth());
            return 0;
        }
        return lc.width;
    }
...
Flt _List::columnOffset(Int i) C {
    if (i >= 1) {
        if (InRange(i, _columns))
            return _columns[i].rect().min.x;
        if (_columns.elms())
            return _columns.last().rect().max.x;
    }
    return 0;
}
Int _List::localToColumnX(Flt local_x) C {
    REPA(_columns) {
        C ListColumn &lc = _columns[i];
        if (lc.visible() && lc.rect().includesX(local_x))
            return i;
    }
    return -1;
}
Int _List::localToVirtualColumnX(Flt local_x) C {
    if (_columns.elms()) {
        if (local_x > _columns.last().rect().max.x)
            return _columns.elms();
        REPA(_columns) {
            C ListColumn &lc = _columns[i];
            if (lc.visible() && local_x >= lc.rect().min.x)
                return i;
        }
    }
    return -1;
...
void _List::zero() {
    ...
    _zoom = 1;
    _zoom_step = 0.2f;
    Flt zoom_scale_factor = ScaleFactor(_zoom_step);
...
_List &_List::create(C _List &src) {
    if (this != &src) {
        if (!src.is())
            ...
            if (src._abs_to_vis)
                CopyFastN(Alloc(_abs_to_vis, totalElms()), src._abs_to_vis, totalElms());
            if (src._abs_to_data)
                CopyFastN(Alloc(_abs_to_data, totalElms()), src._abs_to_data, totalElms());
            if (src._vis_to_abs)
                CopyFastN(Alloc(_vis_to_abs, visibleElms()), src._vis_to_abs, visibleElms());
            if (src._vis_to_data)
                CopyFastN(Alloc(_vis_to_data, visibleElms()), src._vis_to_data, visibleElms());
            if (src._rects)
                CopyFastN(Alloc(_rects, visibleElms()), src._rects, visibleElms());
...
void _List::init(Int elms, C CMemPtr<Bool> &visible, Bool keep_cur) {
    ...
    Free(_vis_to_data);
    Free(_abs_to_vis);
    Alloc(_abs_to_vis, totalElms());
    REP(totalElms())
    _abs_to_vis[i] = -1;
    Free(_vis_to_abs);
    Alloc(_vis_to_abs, visibleElms());
    Int v = 0;
    FREPD(e, elms)
    if (!visible || ElmIs(visible, e)) {
        _abs_to_vis[e] = v;
        _vis_to_abs[v] = e;
        v++;
    }
    ...
    if (_meml) {
        ...
        Int e = 0, v = 0;
        MFREP(*_meml) {
            _abs_to_data[e] = i->data();
            if (!visible || ElmIs(visible, e))
                _vis_to_data[v++] = i->data();
            e++;
        }
    }
    if (_node) {
        ...
        Int index = 0;
        NodeVisible(*_node, _children_offset, etd, otd, visible, index);
    }
    ...
    if (cur_mode == LCM_MOUSE && !_kb_action && Gui.ms() == this)
        lit = cur = screenToVis(Ms.pos());
    else if (!keep_cur)
        cur = -1;
    if (!keep_cur)
        sel.clear();
...
void _List::setRects() {
    ...
    if (drawMode() == LDM_LIST) {
    ...
    } else if (drawMode() == LDM_RECTS) {
        if (!_rects)
            Alloc(_rects, visibleElms());
        ...
        FREPA(T) {
            ...
            if (Ptr data = visToData(i)) {
                ...
                if (InRange(draw_column, _columns))
                    if (Image *image = _columns[draw_column].md.asImage(data)) {
                ...
                if (type_new_line)
                    type_cur = *(UInt *)((Byte *)data + _type_offset);
                if (_horizontal) {
                    if (y && ((type_new_line && type_cur != type_prev) || -y + h > max_size.y)) {
                        y = 0;
                        x += W;
                        W = 0;
                    }
                    ...
                    if (x && ((type_new_line && type_cur != type_prev) || x + w > max_size.x)) {
                        x = 0;
                        y -= H;
                        H = 0;
                    }
...
_List &_List::columnsHidden(Bool hidden) {
    if (T._columns_hidden != hidden) {
        T._columns_hidden = hidden;
        setRects();
    }
    return T;
}
_List &_List::columnHeight(Flt base, Flt relative) {
    MAX(base, 0);
    MAX(relative, 0);
    if (T._column_base != base || T._column_rel != relative) {
        T._column_base = base;
        T._column_rel = relative;
        setRects();
    }
    return T;
}
_List &_List::elmHeight(Flt height) {
    MAX(height, 0);
    if (T._elm_height != height) {
        T._elm_height = height;
        _height_ez = _elm_height * _zoom;
        setRects();
    }
    return T;
}
_List &_List::textSize(Flt base, Flt relative) {
    MAX(base, 0);
    MAX(relative, 0);
    if (T._text_base != base || T._text_rel != relative) {
        T._text_base = base;
        T._text_rel = relative;
        setRects();
    }
    return T;
}
_List &_List::imageSize(C Vec2 &base, Flt relative, C Rect &padding) {
    Vec2 b = Max(base, Vec2Zero);
    MAX(relative, 0);
    if (T._image_base != b || T._image_rel != relative || T._image_padd != padding) {
        T._image_base = b;
        T._image_rel = relative;
        T._image_padd = padding;
        setRects();
    }
    return T;
}
_List &_List::zoom(Flt zoom) {
...
_List &_List::zoomStep(Flt step) {
    _zoom_step = step;
    return T;
}
_List &_List::drawMode(LIST_DRAW_MODE mode) {
    if (T._draw_mode != mode) {
        T._draw_mode = mode;
        setRects();
    }
    return T;
}
_List &_List::horizontal(Bool horizontal) {
    if (T._horizontal != horizontal) {
...
Ptr _List::visToData(Int visible) C {
    if (InRange(visible, visibleElms())) {
        if (_vis_to_data)
            return _vis_to_data[visible];
        return absToData(visToAbs(visible));
    }
    return null;
}
Int _List::visToAbs(Int visible) C {
    if (InRange(visible, visibleElms()) && _vis_to_abs)
        return _vis_to_abs[visible];
    return -1;
}
Int _List::absToVis(Int absolute) C {
    if (InRange(absolute, totalElms()) && _abs_to_vis)
        return _abs_to_vis[absolute];
    return -1;
}
Ptr _List::absToData(Int absolute) C {
    if (InRange(absolute, totalElms())) {
        if (_abs_to_data)
            return _abs_to_data[absolute];
        if (_data)
            return (Byte *)_data + absolute * _elm_size;
        if (_memb)
            return (*_memb)[absolute];
        if (_memx)
            return (*_memx)[absolute];
        if (_meml)
            return (*_meml)[absolute];
        if (_map)
            return (*_map)[absolute];
        if (_node)
            return NodeAbsToData(*_node, _children_offset, absolute);
    }
    return null;
}
...
Int _List::localToVirtualX(Flt local_x) C {
    switch (drawMode()) {
    case LDM_RECTS:
        if (_rects && local_x >= 0) // if "local_x<0" then we want -1, so just skip to the end
        {
            if (local_x > rect().max.x)
                return elms(); // out of range
            Int i;
            if (!BinarySearch(_rects, elms(), local_x, i, CompareRectX))
                i--; // if didn't found an exact match, then we need to get the previous one, and not the next one, this will make 'i' always in range (-1..elms-1) inclusive
            ...
            for (; InRange(i - 1, T) && Equal(_rects[i - 1].min.x, _rects[i].min.x); i--)
                ; // if multiple elements have the same rect.min.x then get the first one
            return i;
        }
        break;
    }
...
Int _List::localToVirtualY(Flt local_y) C {
    if (columnsVisible())
        local_y += columnHeight();
    switch (drawMode()) {
    case LDM_LIST:
        return Floor(-local_y / _height_ez);
    case LDM_RECTS:
        if (_rects && local_y <= 0) // if "local_y>0" then we want -1, so just skip to the end
        {
            if (local_y < rect().min.y)
                return elms(); // out of range
            Int i;
            if (!BinarySearch(_rects, elms(), local_y, i, CompareRectY))
                i--; // if didn't found an exact match, then we need to get the previous one, and not the next one, this will make 'i' always in range (-1..elms-1) inclusive
                     // if(InRange(i, T)) no need to check this, because we're checking below "InRange(i-1, T)" and in this case if i-1 is in range, then 'i' is in range too
            for (; InRange(i - 1, T) && Equal(_rects[i - 1].max.y, _rects[i].max.y); i--)
                ; // if multiple elements have the same rect.max.y then get the first one
            return i;
        }
        break;
    }
...
Int _List::localToVis(C Vec2 &local_pos) C {
    if (drawMode() == LDM_RECTS) {
        if (_rects) {
            ...
            if (columnsVisible())
                pos.y += columnHeight();
            if (pos.y <= 0 && pos.y >= rect().min.y && pos.x >= 0 && pos.x <= rect().max.x) {
                if (_horizontal) {
                    for (Int i = Max(0, localToVirtualX(local_pos.x)); i < elms(); i++) {
                        C Rect &r = _rects[i];
                        if (r.min.x > pos.x)
                            break;
                        if (Cuts(pos, r))
                    ...
                    for (Int i = Max(0, localToVirtualY(local_pos.y)); i < elms(); i++) {
                        C Rect &r = _rects[i];
                        if (r.max.y < pos.y)
                            break;
                        if (Cuts(pos, r))
...
Int _List::localToVisX(Flt local_x) C {
    Int v = localToVirtualX(local_x);
    return InRange(v, visibleElms()) ? v : -1;
}
Int _List::localToVisY(Flt local_y) C {
    Int v = localToVirtualY(local_y);
    return InRange(v, visibleElms()) ? v : -1;
}
...
Flt _List::visToLocalY(Int visible) C {
    ...
    switch (drawMode()) {
    case LDM_LIST:
        y -= visible * _height_ez;
        break;
    case LDM_RECTS:
        if (_rects && InRange(visible, visibleElms()))
            y += _rects[visible].max.y;
        break;
    }
    return y;
}
Vec2 _List::visToLocalPos(Int visible) C {
    ...
    switch (drawMode()) {
    case LDM_LIST:
        pos.y -= visible * _height_ez;
        break;
    case LDM_RECTS:
        if (_rects && InRange(visible, visibleElms()))
            pos += _rects[visible].lu();
        break;
    }
    return pos;
}
Rect _List::visToLocalRect(Int visible) C {
    ...
    case LDM_LIST:
        pos.y -= visible * _height_ez;
        return Rect_LU(pos, rect().w(), _height_ez);
    case LDM_RECTS:
        if (_rects && InRange(visible, visibleElms()))
            return _rects[visible] + pos;
        break;
    }
    return pos;
}
...
Vec2 _List::visToScreenPos(Int visible, C GuiPC *gpc) C {
    Vec2 pos = (gpc ? gpc->offset : screenPos());
    if (columnsVisible())
        pos.y -= columnHeight();
    switch (drawMode()) {
    case LDM_LIST:
...
Rect _List::visToScreenRect(C VecI2 &col_vis, C GuiPC *gpc) C {
    ...
    switch (drawMode()) {
    case LDM_LIST: {
        pos.y -= col_vis.y * _height_ez;
        Rect rect;
        rect.setY(pos.y - _height_ez, pos.y);
        if (InRange(col_vis.x, columns())) {
            C Rect &col_rect = column(col_vis.x).rect();
            rect.setX(col_rect.min.x + pos.x, col_rect.max.x + pos.x);
        } else
            rect.setX(pos.x, pos.x + T.rect().w());
        return rect;
    }
    case LDM_RECTS:
        if (_rects && InRange(col_vis.y, visibleElms()))
            return _rects[col_vis.y] + pos;
        break;
    }
    return pos;
}
...
Rect _List::elmsScreenRect(C GuiPC *gpc) C {
    ...
    Rect elms_rect = (gpc ? gpc->client_rect : parent() ? parent()->screenClientRect()
                                                        : clip);
    if (columnsVisible())
    ...
    if (visibleElms())
        if (gpc ? visible() && gpc->visible : visibleFull()) {
            ...
            if (_horizontal && drawMode() == LDM_RECTS) {
                ...
                if (drawMode() == LDM_RECTS)
                    for (; InRange(range.y + 1, visibleElms()) && _rects[range.y + 1].min.x + offset.x < elms_rect.max.x; range.y++)
                        ;
            } else {
                ...
                if (drawMode() == LDM_RECTS)
                    for (; InRange(range.y + 1, visibleElms()) && _rects[range.y + 1].max.y + offset.y > elms_rect.min.y; range.y++)
                        ;
            }
            return range;
        }
    ...
    if (visibleElms())
        ...
        case LDM_LIST:
            if (dir.y) {
                ...
                if (dir.y > 0)
                    return Mid(v - 1, -1, visibleElms() - 1);
                v++;
                if (v >= visibleElms())
                    return -1;
                return Max(v, 0);
        ...
        case LDM_RECTS: {
            ...
            if (dir_n.normalize()) {
                ...
                if (_horizontal) {
                    ...
                    {
                        Flt x = _rects[--range.x].min.x;
                        for (; range.x - 1 >= 0 && Equal(_rects[range.x - 1].min.x, x); range.x--)
                            ; // keep going as long as rect.min.x is the same
                    }
                    if (range.y + 1 < visibleElms()) // go to the end of current line
                    {
                        Flt x = _rects[range.y].min.x;
                        for (; range.y + 1 < visibleElms() && Equal(_rects[range.y + 1].min.x, x); range.y++)
                            ;                            // keep going as long as rect.min.x is the same
                    ...
                    if (range.x > 0) // go to the start of previous line
                    {
                        Flt y = _rects[--range.x].max.y;
                        for (; range.x - 1 >= 0 && Equal(_rects[range.x - 1].max.y, y); range.x--)
                            ; // keep going as long as rect.max.y is the same
                    }
                    if (range.y + 1 < visibleElms()) // go to the end of current line
                    {
                        Flt y = _rects[range.y].max.y;
                        for (; range.y + 1 < visibleElms() && Equal(_rects[range.y + 1].max.y, y); range.y++)
                            ;                            // keep going as long as rect.max.y is the same
                        if (range.y + 1 < visibleElms()) // go to the end of next line
                        {
                            Flt y = _rects[++range.y].max.y;
                ...
                if (columnsVisible())
                    local_pos.y += columnHeight(); // make 'local_pos' and '_rects' in the same space
...
VecI2 _List::nearest2(C Vec2 &screen_pos, C Vec2 &dir) C {
    if (visibleElms())
            ...
            if (dir.any()) {
                ...
                if (InRange(v, visibleElms())) {
                    ...
                    next_col: // find next visible column
                        if (!InRange(c, columns()))
                            return -1;
                        if (!columnVisible(c)) {
                            c++;
                            goto next_col;
                        }
                    } else if (dir.x < 0) // move left
                    {
                    left:
                        MIN(--c, visibleElms() - 1);
        ...
        case LDM_RECTS:
            return VecI2(0, nearest(screen_pos, dir));
        }
    return -1;
}
...
Bool _List::scrolling() C {
    if (_parent && _parent->isRegion())
        return _parent->asRegion().scrolling();
    return false;
}
Bool _List::scrollingMain() C {
    if (_parent && _parent->isRegion()) {
        ...
        switch (drawMode()) {
        case LDM_LIST:
            return region.slidebar[1].scrolling();
        case LDM_RECTS:
            return region.slidebar[vertical()].scrolling();
        }
    }
    return false;
}
Vec2 _List::scrollDelta() C {
    if (_parent && _parent->isRegion())
        return _parent->asRegion().scrollDelta();
    return 0;
}
    ...
    if (InRange(i, T) && _parent && _parent->isRegion()) {
        ...
        case LDM_RECTS:
            if (_rects) {
                C Rect &rect = _rects[i];
                if (_horizontal) {
                    e = (region.clientWidth() - rect.w()) * center;
                    region.scrollFitX(rect.min.x - e, rect.max.x + e, immediate);
                } else {
                    e = (h - rect.h()) * center;
                    region.scrollFitY(-rect.max.y - e, -rect.min.y + add + e, immediate);
                }
            }
            break;
        }
...
_List &_List::scrollY(Flt delta, Bool immediate) {
    if (_parent && _parent->isRegion())
        _parent->asRegion().scrollY(delta, immediate);
    return T;
}
/******************************************************************************/
...
static inline Int ListCompareData(CPtr d0, CPtr d1, Int i0, Int i1) {
    ...
    if (LCmp.md[0])
        if (Int c = LCmp.md[0]->compare(d0, d1))
            return LCmp.swap[0] ? -c : c;
    if (LCmp.md[1])
        if (Int c = LCmp.md[1]->compare(d0, d1))
            return LCmp.swap[1] ? -c : c;
    if (LCmp.md[2])
        if (Int c = LCmp.md[2]->compare(d0, d1))
            return LCmp.swap[2] ? -c : c;
...
void _List::sort() {
    ...
    if (type_sort || sort_column[0] >= 0 || sort_column[1] >= 0 || sort_column[2] >= 0) {
        ...
        REPD(v, visibleElms()) {
            ...
            if (_vis_to_data)
                _vis_to_data[v] = absToData(absolute);
        }
...
_List &_List::sort(Int column, Int swap) {
    if (InRange(column, _columns)) {
        ...
        if (column == sort_column[0]) {
            ...
            if (sort_swap[0] == new_swap)
                return T; // if swap is the same, then do nothing
            sort_swap[0] = new_swap;
        } else if (column == sort_column[1]) {
            Swap(sort_column[0], sort_column[1]);
            Swap(sort_swap[0], sort_swap[1]);
            if (swap >= 0)
                sort_swap[0] = (swap != 0);
        } else if (column == sort_column[2]) {
            if (swap < 0)
                swap = sort_swap[2];
            sort_column[2] = sort_column[1];
            sort_swap[2] = sort_swap[1];
            sort_column[1] = sort_column[0];
            sort_swap[1] = sort_swap[0];
            ...
            sort_swap[1] = sort_swap[0];
            sort_column[0] = column;
            sort_swap[0] = ((swap < 0) ? false : (swap != 0));
        }
...
LIST_SEL_MODE _List::selMode() C {
    if (Kb.alt())
        return Kb.shift() ? LSM_EXCLUDE_MULTI : LSM_EXCLUDE;
    if (Kb.shift())
        return LSM_INCLUDE;
    if (Kb.ctrlCmd())
        return LSM_TOGGLE;
...
void _List::childRectChanged(C Rect *old_rect, C Rect *new_rect, GuiObj &child) {
    ...
    if (old_rect || new_rect) {
        Flt x = (new_rect ? GuiMaxX(*new_rect) : 0),
            ...
            ofs_x = 0;
        if (_columns.elms())
            ofs_x += _columns.last().rect().max.x;
        ofs_x += padding.x;
        x += ofs_x;
        old_x += ofs_x;
        Int index;
        if (_children.find(child, index)) // if child belongs to List
        {
            C Children::Child &c = _children[index];
            if (c.abs_col.y >= _columns.elms() && absToVis(c.abs_col.x) >= 0) // process only if it's attached to the last column
            {
                if (old_x >= rect().w() - EPS && x < rect().w() - EPS)
                    setRects();
        ...
        {
            if (old_x >= rect().w() - EPS && x < rect().w() - EPS)
                setRects(); // if decreasing then we need to recalculate fully
        }
...
_List &_List::addChild(GuiObj &child, Int abs, Int column) {
    ...
    {
        if (same_parent && c->abs_col.x == abs && c->abs_col.y == column)
            return T; // if child had the same parent and we're not changing indexes, then we're done
        ...
        if (column >= _columns.elms() && absToVis(abs) >= 0 && child.visible()) {
            Flt width = GuiMaxX(child.rect());
            MAX(width, 0);
            if (_columns.elms())
                width += _columns.last().rect().max.x;
            width += padding.x;
            if (width > rect().w())
                size(Vec2(width, rect().h()));
        }
    }
    return T;
}
...
void _List::callSelChanging() {
    if (_sel_changing) {
        ...
        _sel_changing(_sel_changing_user);
    }
}
/******************************************************************************/
_List &_List::clearElmType() {
    _type_offset = -1;
    return T;
}
_List &_List::clearElmDesc() {
    _desc_offset = -1;
    return T;
}
_List &_List::clearElmImageColor() {
    _image_color_offset = -1;
    return T;
}
_List &_List::clearElmTextColor() {
    _text_color_offset = -1;
    return T;
}
_List &_List::clearElmTextShadow() {
    _text_shadow_offset = -1;
    return T;
}
_List &_List::clearElmAlphaMode() {
    _alpha_mode_offset = -1;
    return T;
}
_List &_List::clearElmOffset() {
    _offset_offset = -1;
    return T;
}
_List &_List::clearElmGroup() {
    _group_offset = -1;
    return T;
}
_List &_List::setElmType(UInt &member) {
    _type_offset = UInt(UIntPtr(&member));
    return T;
}
_List &_List::setElmDesc(CChar *&member) {
    _desc_offset = UInt(UIntPtr(&member));
    return T;
}
_List &_List::setElmDesc(Str &member) {
    _desc_offset = UInt(UIntPtr(&member)) + OFFSET(Str, _d);
    return T;
}
_List &_List::setElmImageColor(Color &member) {
    _image_color_offset = UInt(UIntPtr(&member));
    return T;
}
_List &_List::setElmTextColor(Color &member) {
    _text_color_offset = UInt(UIntPtr(&member));
    return T;
}
_List &_List::setElmTextShadow(Byte &member) {
    _text_shadow_offset = UInt(UIntPtr(&member));
    return T;
}
_List &_List::setElmAlphaMode(ALPHA_MODE &member) {
    _alpha_mode_offset = UInt(UIntPtr(&member));
    return T;
}
_List &_List::setElmOffset(Flt &member) {
    _offset_offset = UInt(UIntPtr(&member));
    return T;
}
_List &_List::setElmGroup(Str &member) {
    _group_offset = UInt(UIntPtr(&member));
    return T;
}
/******************************************************************************/
GuiObj *_List::test(C GuiPC &gpc, C Vec2 &pos, GuiObj *&mouse_wheel) {
    if (/*gpc.visible &&*/ visible() && Cuts(pos, gpc.clip)) {
        if (Kb.ctrlCmd() && (flag & LIST_SCALABLE))
            mouse_wheel = this;
        if (columnsVisible()) {
            GuiPC gpc_col(gpc, T);
            REP(columns())
        ...
        if (_children.children.elms()) {
            ...
            if (visible_range.y >= visible_range.x) {
                GuiPC gpc_this(gpc, visible(), enabled());
                Vec2 offset = gpc_this.offset;
                if (columnsVisible()) {
                ...
                {
                    Children::Child &child = _children[i];
                    if (SetGPC(T, gpc_this, offset, child.abs_col, visible_range))
                        if (GuiObj *go = child.go->test(gpc_this, pos, mouse_wheel))
                            return go;
                }
            }
        }
    ...
    if (/*gpc.visible &&*/ visible()) {
      ...
      if(gon.test((T.rect()+gpc.offset)&gpc.clip)) // this already tests if rect is valid
#endif
        {
            ...
            case LDM_LIST: {
                Rect rect;
                rect.setX(gpc.clip.min.x, gpc.clip.max.x);
                ...
                for (Int i = visible_range.x; i <= visible_range.y; i++) {
                    ...
                    if (flag & LIST_NEAREST_COLUMN)
                        REP(columns()) {
                            C ListColumn &col = _columns[i];
                            if (col.visible()) {
                                rect.min.x = pos.x + col.rect().min.x;
                                rect.max.x = pos.x + col.rect().max.x;
                                if (CutsX(gpc.clip, rect)) {
                                    if (ignore_start && Cuts(gon.plane.pos, rect))
                                        continue;
            ...
            case LDM_RECTS:
                if (_rects) {
                    for (Int i = visible_range.x; i <= visible_range.y; i++) {
                        ...
                        if (ignore_start && Cuts(gon.plane.pos, screen_rect))
                            continue;
...
_List &_List::processSel(Int absolute, Int sel_mode) // this may call ONLY 'selChanging', 'selChanged', but NOT 'curChanged'
{
    if (flag & LIST_MULTI_SEL) {
        ...
        if (sel_mode == LSM_SET) {
            ...
            {
                if (sel.elms() != 1 || sel[0] != absolute) {
                    callSelChanging();
                    sel.setNum(1).first() = absolute;
                    sel_changed = true;
                }
            } else {
                if (sel.elms()) {
            ...
            case LSM_TOGGLE:
                callSelChanging();
                sel.toggle(absolute, true);
                sel_changed = true;
                break;
            case LSM_INCLUDE:
                if (_sel_changing && !sel.has(absolute))
                    callSelChanging();
                if (sel.include(absolute))
                    sel_changed = true;
                break;
    ...
    if (flag & LIST_MULTI_SEL) {
        if (_group_offset >= 0)
            if (Ptr data = visToData(cur))
                if (((Str *)((Byte *)data + _group_offset))->is())
                    goto skip_sel_change; // do not modify 'sel' because we want to click on a group, to open it, to show its elements
        ...
        case LSM_SET: {
            if (touch_id && !pushed) {
                lit = cur;
                return false;
            } // if it's a touch and it's not pushed then set 'lit' only and don't do anything on the cursor and selection
            if (pushed && sel.has(visToAbs(cur))) {
                _tap_vis = cur;
                _tap_touch_id = touch_id;
            } // if we're activating using button push, and the element is already selected, then there's possibility we want to drag and drop the elements, so instead of activating that element, we will store its index to temporary member, and activate it only quick release of the button (tap)
            else
                sel_changed |= setSel(cur);
        } break;

        case LSM_TOGGLE: {
            if (!pushed)
                return false; // don't do anything if it's TOGGLE mode and button was not pushed
            ...
            if (InRange(cur, visibleElms())) {
                if (InRange(T.cur, visibleElms())) {
                    ...
                    for (Bool called = false;; T.cur = Mod(T.cur + dir, visibleElms())) {
                        ...
                        if (_sel_changing && !called && !sel.has(abs)) {
                            called = true;
                            callSelChanging();
                        }
...
void _List::update(C GuiPC &gpc) {
    ...
    if (disabled() || gpc.disabled()) {
        if (cur_mode == LCM_MOUSE)
            setCur(-1);
    } else if (gpc.visible && visible()) {
        ...
        if (visibleElms()) {
            ...
            REPA(Touches) {
                ...
                if (touch.guiObj() == this && (touch.state() & (BS_PUSHED | BS_ON | BS_RELEASED))) // process cursor for (pushed to set) and (on+released to skip from mouse), release needed so 'Menu' detection can work
                {
                    GuiObj *container = this;
                    if (parent() && parent()->isMenu())
                        container = parent();
                    if (container->contains(Gui.objAtPos(touch.pos()))) {
            ...
            // mouse cursor
            if (Gui.ms() == this) {
                if (Ms.bp(0) || (cur_mode == LCM_MOUSE && !_kb_action))
                    sel_changed |= setCurEx(lit, 0, Ms.bp(0));
            } else {
                if (cur_mode == LCM_MOUSE && !_kb_action)
                    sel_changed |= setSel(cur = -1);
            }
        skip_mt:
            ...
            if (_tap_vis >= 0) {
                ...
                if (_tap_touch_id ? (!touch || touch->rs()) : !Ms.b(0)) {
                    if (InRange(_tap_vis, T))
                        if (_tap_touch_id ? (touch && touch->tapped()) : Ms.tapped(0))
                            sel_changed |= setCurEx(_tap_vis);
                    _tap_vis = -1;
                }
            }
            ...
            if (Gui.kb() == this) {
                if (Kb.k.ctrlCmd()) {
                    ...
                    if (Kb.k(KB_HOME)) {
                        _kb_action = true;
                        sel_changed |= setCurEx(0);
                        Kb.eatKey();
                    }
                    if (Kb.k(KB_END)) {
                        _kb_action = true;
                        sel_changed |= setCurEx(visibleElms() - 1);
                        Kb.eatKey();
                    }
                    ...
                    if (drawMode() == LDM_LIST) {
                        if (Kb.k(KB_UP)) {
                            _kb_action = true;
                            sel_changed |= setCurEx((flag & LIST_ROLLABLE) ? ((cur < 0) ? visibleElms() - 1 : Mod(cur - 1, visibleElms())) : Max(0, cur - 1), -1);
                        }
                        if (Kb.k(KB_DOWN)) {
                            _kb_action = true;
                            sel_changed |= setCurEx((flag & LIST_ROLLABLE) ? Mod(cur + 1, visibleElms()) : Min(visibleElms() - 1, cur + 1), +1);
                        }
                    } else if (drawMode() == LDM_RECTS) {
                        if (Kb.k(KB_LEFT)) {
                            _kb_action = true;
                            sel_changed |= setCurEx(Max(0, cur - 1));
                        }
                        if (Kb.k(KB_RIGHT)) {
                            _kb_action = true;
                            sel_changed |= setCurEx(Min(visibleElms() - 1, cur + 1));
                        }
                    }
                }
                ...
                if (flag & LIST_SEARCHABLE) {
                    if (Kb.k.c && !Kb.k.ctrl() && !Kb.k.lalt() && !Kb.k.win()) {
                        _kb_action = true;
                        for (; Kb.k.c; Kb.nextKey())
                            if (_search_i < Elms(_search) - 1) {
                        ...
                        _search_t = 0.9f;
                    } else if ((_search_t -= Time.ad()) <= 0 || (Kb.k.k && Kb.k.k != KB_SHIFT && Kb.k.k != KB_LSHIFT && Kb.k.k != KB_RSHIFT))
                        _search[_search_i = 0] = 0; // if time has passed, or function key was pressed (like arrow up/down or enter, but not shift)
                }
            }
            if (cur != cur_prev) {
                if ((cur_prev2 != cur || cur_mode != LCM_MOUSE) && cur >= 0)
                    scrollTo(cur, _search_i != 0, _search_i != 0); // scroll immediately if cursor was changed by "search"
                callCurChanged();
            }
            if (sel_changed)
                callSelChanged();
        ...
        if (Gui.wheel() == this) {
            if (Kb.ctrlCmd() && Ms.wheel() && (flag & LIST_SCALABLE)) {
                ...
                if (drawMode() == LDM_LIST) {
                    zoom = T.zoom();
                    virt = screenToVirtualYF(Ms.pos().y, &gpc);
                }
                T.zoom(T.zoom() * ScaleFactor(zoomStep() * Ms.wheel()));
                if (drawMode() == LDM_LIST && zoom != T.zoom())
                    scrollY((virt - screenToVirtualYF(Ms.pos().y, &gpc)) * _height_ez, true);
                Ms.eatWheel();
            }
        }
    ...
    if (/*gpc.visible &&*/ visible()) {
        ...
        if (visibleElms()) {
            ...
            if (drawMode() == LDM_RECTS) {
                if (_rects) {
                    ...
                    if (show_sel && sel.elms()) {
                        REPA(sel) {
                            ...
                            if (/*vis>=0 && */ vis >= pos /* && vis<end*/) // no need to test for >=0 because 'pos' is always >=0, we don't test for 'end' because we don't know it yet
                            {
                                Rect r = _rects[vis] + elms_offset;
                                if (_horizontal ? r.min.x <= elms_rect.max.x : r.max.y >= elms_rect.min.y) {
                                    if (list_skin->selection)
                                        list_skin->selection->draw(list_skin->selection_color, TRANSPARENT, r);
                                    else
                                        r.draw(list_skin->selection_color);
                                }
                            }
                        }
                    }
                    ...
                    for (; pos < elms(); pos++)
                        if (Ptr data = visToData(pos)) {
                            Rect r = _rects[pos] + elms_offset;
                            if (_horizontal ? r.min.x > elms_rect.max.x : r.max.y < elms_rect.min.y)
                                break;
                            if (show_lit && lit == pos) {
                                D.alpha(alpha);
                                if (list_skin->highlight)
                                    list_skin->highlight->draw(list_skin->highlight_color, TRANSPARENT, r);
                                else
                                    r.draw(list_skin->highlight_color);
                            }
                            if (show_cur && cur == pos) {
                                D.alpha(alpha);
                                if (flag & LIST_MULTI_SEL) {
                                    Color c = list_skin->cursor_color;
                                    c.a = Min(255, c.a * 2); // since we're drawing only borders then increase the alpha
                                    if (list_skin->cursor)
                                        list_skin->cursor->drawBorders(c, TRANSPARENT, r);
                                    else
                                        r.draw(c, false);
                                } else {
                                    if (list_skin->cursor)
                                        list_skin->cursor->draw(list_skin->cursor_color, TRANSPARENT, r);
                                    else
                                        r.draw(list_skin->cursor_color);
                                }
                            }
                            if (InRange(draw_column, _columns))
                                if (Image *image = _columns[draw_column].md.asImage(data)) {
                                    ALPHA_MODE alpha = image_alpha;
                                    if (_alpha_mode_offset >= 0)
                                        alpha = *(ALPHA_MODE *)((Byte *)data + _alpha_mode_offset);
                                    D.alpha(alpha);
                                    if (_image_color_offset >= 0)
                                        color = *(Color *)((Byte *)data + _image_color_offset);
                ...
                // highlight
                if (columnsVisible() && list_skin && list_skin->highlight_color.a)
                    REPA(_columns) {
                        ListColumn &lc = column(i);
                        if (lc.lit()) {
                            ...
                            if (list_skin->highlight)
                                list_skin->highlight->draw(color, TRANSPARENT, r);
                            else
                                r.draw(color);
                        }
                    }
                        ...
                        for (; pos <= end; pos++)
                            if (Ptr data = visToData(pos)) {
                                ...
                                Flt ofs = 0,
                                    ...
                                    yp = pos * _height_ez * D._pixel_size_inv.y;
                                if (Equal(Frac(yp), 0.5f, 0.001f))
                                    y += D._pixel_size_2.y;     // vertical offset in pixel coordinates (this can happen very often, text being aligned exactly between 2 pixels)
                                Flt yt = y - 0.5f * _height_ez; // element text position

                                if (_text_color_offset >= 0)
                                    ts.color = *(Color *)((Byte *)data + _text_color_offset);
                                if (_text_shadow_offset >= 0)
                                    ts.shadow = *(Byte *)((Byte *)data + _text_shadow_offset);
                                if (_alpha_mode_offset >= 0)
                                    image_alpha = *(ALPHA_MODE *)((Byte *)data + _alpha_mode_offset);
                                if (_image_color_offset >= 0)
                                    color = *(Color *)((Byte *)data + _image_color_offset);
                                if (_offset_offset >= 0)
                                    ofs = *(Flt *)((Byte *)data + _offset_offset);
                                    ...
                                    FREPAD(c, _columns) {
                                        ...
                                        if (lc.visible())
                                            ...
                                            case DATA_STR_EX: {
                                                ...
                                                if (_offset_first_column)
                                                    ofs = 0; // reset offset for >=1 columns, this should not be done for images
                                            } break;

                                            default: {
                                                C Str &text = lc.md.asText(data, lc.precision);
                                                if (text.is()) {
                                                    ts.align.x = lc.text_align;
                                                    D.clip(gpc.clip & Rect(lc.rect().min.x + wae_2 + x, elms_rect.min.y, lc.rect().max.x - wae_2 + x, elms_rect.max.y));
        ...
        if (columnsVisible()) {
            ...
            if (resize_col && list_skin)
                if (Image *image = list_skin->resize_column()) {
                    D.clip(gpc.clip);
