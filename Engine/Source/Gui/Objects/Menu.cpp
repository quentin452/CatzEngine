...
GuiPC::GuiPC(C GuiPC &old, Menu &menu) {
    ...
    enabled = old.enabled & menu.enabled();
    if (menu._owner)
        enabled &= menu._owner->enabledFull();
...
MenuElm &MenuElm::create(C MenuElm &src, Menu *parent) {
    if (this != &src) {
        ...
        if (src._menu)
            New(T._menu)->create(*src._menu)._parent = parent;
...
/******************************************************************************/
MenuElm &MenuElm::flag(Byte flag) {
    T._flag = flag;
    return T;
}
MenuElm &MenuElm::kbsc(C KbSc &kbsc) {
    T._kbsc = kbsc;
    return T;
}
MenuElm &MenuElm::kbsc1(C KbSc &kbsc) {
    T._kbsc1 = kbsc;
    return T;
}
MenuElm &MenuElm::desc(C Str &desc) {
    T._desc = desc;
    return T;
}
MenuElm &MenuElm::setOn(Bool on) {
    T.on = on;
    return T;
}
MenuElm &MenuElm::display(C Str &name) {
    if (name.is())
        display_name = name;
    return T;
}
MenuElm &MenuElm::func(void (*func)(), Bool immediate) {
    T._func = func;
    T._func1 = null;
    T._func_user = null;
    T._func_immediate = immediate;
    return T;
} // clear the other function as only one function type should be available at a time
MenuElm &MenuElm::func(void (*func)(Ptr user), Ptr user, Bool immediate) {
    T._func = null;
    T._func1 = func;
    T._func_user = user;
    T._func_immediate = immediate;
...
void MenuElm::call() {
    if (_func)
        if (_func_immediate)
            ...
            Gui.addFuncCall(_func);
    if (_func1)
        if (_func_immediate)
            _func1(_func_user);
        else
            Gui.addFuncCall(_func1, _func_user);
}
/******************************************************************************/
        ...
        for (GuiObj *go = this; go && go->isMenu(); go = go->parent()) // iterate all menu parents (including this)
            if (go->asMenu()._func)                                    // if any has a callback
            {
                ...
                for (; parent && parent->isMenu();) {
                    ...
                    FREP(parent_menu.elms())
                    if (parent_menu.elm(i).menu() == menu) {
                ...
                go->asMenu()._func(mps, go->asMenu()._func_user);
...
Menu &Menu::create(C Menu &src) {
    if (this != &src) {
        if (!src.is())
            ...
            FREPAO(_elms).create(src._elms[i], this);
        }
    }
    return T;
}
...
Menu &Menu::setData(C Node<MenuElm> &node) {
    ...
    TextStyle *text_style = (skin ? skin->list.text_style() : null);
    TextStyleParams ts(text_style, false);
    ts.size = list.textSize();
    ...
    FREPA(_elms) {
        ...
        {
            Menu &menu = New(elm._menu)->create();
            menu._parent = this;
        ...
        if (visible[i]) {
            ...
            if (elm.kbsc().is())
                MAX(width_kbsc, ts.textWidth(elm.kbsc().asText()));
            if (elm.kbsc1().is())
                MAX(width_kbsc1, ts.textWidth(elm.kbsc1().asText()));
        }
    }
...
Menu &Menu::rect(C Rect &rect) {
    if (T.rect() != rect) {
        ...
        if (GuiSkin *skin = getSkin()) {
            ...
            if (!_crect.validX())
                _crect.setX(_crect.centerX());
            if (!_crect.validY())
                _crect.setY(_crect.centerY());
        }
...
Menu &Menu::moveClamp(C Vec2 &delta) {
    Vec2 d = delta;
    if (rect().max.x + d.x > D.rectUI().max.x)
        d.x = D.rectUI().max.x - rect().max.x;
    if (rect().min.y + d.y < D.rectUI().min.y)
        d.y = D.rectUI().min.y - rect().min.y;
    if (rect().min.x + d.x < D.rectUI().min.x)
        d.x = D.rectUI().min.x - rect().min.x;
    if (rect().max.y + d.y > D.rectUI().max.y)
        d.y = D.rectUI().max.y - rect().max.y;
    return move(d);
}
...
/******************************************************************************/
Menu &Menu::clearElmSelectable() {
    _selectable_offset = -1;
    return T;
}
Menu &Menu::setElmSelectable(Bool &selectable) {
    _selectable_offset = UInt(UIntPtr(&selectable));
    return T;
}
/******************************************************************************/
...
ListColumn *Menu::listColumn() {
    if (elms() && InRange(1, list.columns()))
        return &list.column(1);
    if (InRange(0, list.columns()))
...
void Menu::nearest(C GuiPC &gpc, GuiObjNearest &gon) {
    if (/*gpc.visible &&*/ visible()) { // menus are always on top, so we can ignore offset and clip, also don't do 'test' because sub menus can be located outside of this Menu rectangle
        gon.cover(rect());
        GuiPC gpc_this(gpc, T);
        ...
        if (gpc_this.visible) {
            if (_elms.elms()) {
                ...
                if (Gui.menu() == this) {
                    ...
                    if (Ms._action && list.contains(Gui.ms())) {
                        activate();
            ...
            if (Gui.menu() == this) {
                ...
                REPA(Touches) {
                    ...
                    if (touch.on() && Gui.menu()->contains(touch.guiObj())) {
                        pos = &touch.pos();
                        break;
                    }
                }
                if (!pos && Gui.menu()->contains(Gui.ms()) && !list._kb_action) {
                    pos = &Ms.pos();
                }
                if (pos) // use detection basing on the 'pos' screen position
                {
                    ...
                    {
                        Flt d = D.rectUI().max.y - rect().max.y;
                        if (d < 0)
                            T.move(Vec2(0, Max(d, -ScrollSpeed(T))));
                    } else if (pos->y <= -margin) // bottom of the screen
                    {
                        Flt d = D.rectUI().min.y - rect().min.y;
                        if (d > 0)
                ...
                C Vec2 *rs_pos = null;
                if (Ms.br(0) && Gui.menu()->contains(Gui.ms()))
                    rs_pos = &Ms.pos();
                else
                    REPA(Touches) {
                        Touch &touch = Touches[i];
                        if (touch.rs() && Gui.menu()->contains(touch.guiObj())) {
                            rs_pos = &touch.pos();
                            by_touch = true;
                            break;
                        }
                ...
                if (rs_pos || enter_only || ((Kb.k(KB_ENTER) || Kb.k(KB_NPENTER)) && Kb.k.first())) {
                    ...
                    if (InRange(list.cur, list.visibleElms())) {
                        if (_elms.elms()) {
                            if (MenuElm *e = (MenuElm *)list.visToData(list.cur))
                                if (!e->disabled) {
                                    Menu *menu = e->menu();
                                    if ((e->flag() & MENU_NOT_SELECTABLE) // can't be selected (so enter only)
                                        ...
                                        if (menu) // has child menu
                                        {
                                            if (0 && D.smallSize())
                                                ...
                                                menu->pos(Vec2(rs_pos ? by_touch ? Avg(rs_pos->x - menu->rect().w() * 0.5f, menu->rect().min.x) // move half way if by touch, to avoid finger covering menu
                                            ...
                                            if (enter_only)
                                                menu->_no_child_draw = true; // when moving right using keyboard, don't display children of selected element
                                            _no_child_draw = false;
                                            entered = true;
                                        }
                                    } else {
                                        ...
                                        if (!(e->flag() & (MENU_TOGGLABLE | MENU_NO_CLOSE)) && Gui.menu() == this)
                                            hideAll();
                                    }
                                }
                        } else {
                            if (_selectable_offset >= 0)
                                if (Ptr data = list.visToData(list.cur))
                                    if (!*(Bool *)((Byte *)data + _selectable_offset))
                                        goto skip;
                            push(list.visToAbs(list.cur));
                            hideAll();
                        skip:;
                    ...
                    if (enter_only && !entered)
                        if (GuiObj *go = first(GO_MENU_BAR)) // activate next MenuBar element
                        {
                            MenuBar &g = go->asMenuBar();
                            FREPA(g)
                            if (!g.elm((g._lit + 1 + i) % g.elms()).menu.disabled()) {
                                g._push = (g._lit + 1 + i) % g.elms();
                                break;
                            }
                        }
                    ...
                    if (GuiObj *go = first(GO_MENU_BAR)) // activate previous 'MenuBar' element
                    {
                        MenuBar &g = go->asMenuBar();
                        REPA(g)
                        if (!g.elm((g._lit + i) % g.elms()).menu.disabled()) {
                            g._push = (g._lit + i) % g.elms();
                            Kb.eatKey();
                            break;
                        }
                    }
                } else if (contains(Gui.ms()) && Ms.bp(2) || Kb.kf(KB_ESC)) // close all menu's (check mouse focus too in case we've just activated the Menu in this frame with the use of Ms.bp(2) in such case the Menu would get immediately closed without showing up at all)
                {
                    hideAll();
                    Ms.eat(2);
                    Kb.eatKey();
                } else if (contains(Gui.ms()) && Ms.bp(1) || Kb.kf(KB_NAV_BACK)) // activate parent menu (check mouse focus too in case we've just activated the Menu in this frame with the use of Ms.bp(1) in such case the Menu would get immediately closed without showing up at all)
                {
                    if (parent()) {
                        parent()->activate();
                        if (parent()->isMenu()) {
            ...
            REPA(_elms) {
                ...
                if (Menu *menu = elm.menu())
                    if (!elm.disabled) {
                        if (Gui.menu() == this && Ms._action && menu->visible() && menu->contains(Gui.ms()))
                            menu->activate(); // activate child on mouse action
                        if (menu->contains(Gui.menu()))
                        ...
                        if (menu->visible() != visible) {
                            if (visible) {
                                ...
                                Vec2 pos((r >= w || r >= l)                                                                               // put on right side if there's enough space to fit all, or right side has more visible space
                                             ? _crect.max.x
                                             : _crect.min.x - w,
                                         rect().max.y + list.visToLocalY(list.absToVis(i)));
                                menu->pos(pos); // first use 'pos' to use clamping
                                pos.x -= menu->rect().min.x;
                                pos.y = 0;
...
void Menu::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible()) {
        if (GuiSkin *skin = getSkin()) {
            D.clip();
            if (skin->menu.normal)
                skin->menu.normal->draw(skin->menu.normal_color, rect());
            else if (skin->menu.normal_color.a)
                rect().draw(skin->menu.normal_color);
        }
        GuiPC gpc_this(gpc, T);
        list.draw(gpc_this);
        REPA(_elms)
