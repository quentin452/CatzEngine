...
void MenuBar::setElms() {
    if (GuiSkin *skin = getSkin())
        if (TextStyle *text_style = skin->menubar.text_style()) {
            ...
            ts.size = skin->menubar.text_size * h;
            FREP(elms()) {
...
void MenuBar::update(C GuiPC &gpc) {
    if (enabled() && gpc.enabled) {
        ...
        {
            if (Kb.k.c || (Kb.k.k && Kb.k.k != KB_ALT && Kb.k.k != KB_LALT && Kb.k.k != KB_RALT && Kb.k.k != KB_MENU) || Kb.ctrl() || Kb.shift() || Kb.win() || Kb.b(KB_FIND) || Ms._button[0] || Ms._button[1] || Ms._button[2] || !App.active() // if lost focus due to Alt-tab
            )
                _alt = false;
        ...
        if (_push >= 0)
            _lit = _push;
        else if (Gui.ms() != this || (!Ms._action && _menu_prev >= 0))
            _lit = _menu_prev;
        else {
            _lit = -1;
            ...
            REP(elms()) {
                Elm &e = elm(i);
                if (!e.hidden && !e.menu.disabled() && Ms.test(Rect(l + e.x, b, l + e.x1(), t))) {
                    _lit = i;
                    goto found;
                }
            }
            ...
            if (touch.guiObj() == this) {
                ...
                REP(elms()) {
                    ...
                    if (!e.hidden && !e.menu.disabled() && Cuts(touch.pos(), Rect(l + e.x, b, l + e.x1(), t))) {
                        ...
                        {
                            activate();
                        } else if (touch.pd() || _menu_prev >= 0) // if we just pushed or there was already selected
                        {
                            e.menu.list.cur = -1;
                            Activate(T, e, gpc);
                        }
...
void MenuBar::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible())
        if (GuiSkin *skin = getSkin()) {
            ...
            if (skin->menubar.background)
                skin->menubar.background->draw(skin->menubar.background_color, TRANSPARENT, rect);
            else if (skin->menubar.background_color.a)
                rect.draw(skin->menubar.background_color);

            if (TextStyle *text_style = skin->menubar.text_style()) {
                TextStyleParams ts = *text_style;
                ts.align.set(0, 0);
                ts.size = rect.h() * skin->menubar.text_size;
                Flt y = rect.centerY();
                FREP(elms())
                if (!elm(i).hidden) {
                    Elm &e = elm(i);
                    ...
                    if (_lit == i) {
                        ...
                        if (skin->menubar.highlight)
                            skin->menubar.highlight->draw(skin->menubar.highlight_color, TRANSPARENT, lit_rect);
                        else if (skin->menubar.highlight_color.a)
                            lit_rect.draw(skin->menubar.highlight_color);
                    }
