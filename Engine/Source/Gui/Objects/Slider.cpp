...
/******************************************************************************/
Slider &Slider::focusable(Bool on) {
    if (_focusable != on) {
        _focusable = on;
        if (!on)
            kbClear();
...
GuiObj *Slider::test(C GuiPC &gpc, C Vec2 &pos, GuiObj *&mouse_wheel) {
    if (/*gpc.visible &&*/ visible()) {
        ...
        if (Cuts(pos, (r + gpc.offset) & gpc.clip)) {
            if (!mouse_wheel || mouse_wheel->type() != GO_SLIDEBAR)
                mouse_wheel = this; // don't grab wheel focus if it already belongs to a SlideBar (for example Slider located in a Region with SlideBars)
            return this;
        }
    }
    return null;
...
void Slider::update(C GuiPC &gpc) {
    ...
    if (enabled) {
        if (Gui.wheel() == this && (Ms.wheel() || Ms.wheelX()))
            set(_value + (Ms.wheel() + Ms.wheelX()) * 0.1f);
        if (Gui.kb() == this) {
            if (_vertical) {
                if (Kb.b(KB_DOWN))
                    set(_value - Time.d() * 0.2f);
                if (Kb.b(KB_UP))
                    set(_value + Time.d() * 0.2f);
            } else {
                if (Kb.b(KB_LEFT))
        ...
        C Vec2 *pos;
        if (Gui.ms() == this && (Ms.state(0) & (BS_ON | BS_PUSHED))) {
            pos = &Ms.pos();
        set:
            ...
            REPA(Touches) {
                ...
                if (touch.guiObj() == this && touch.state() & (BS_ON | BS_PUSHED)) {
                    pos = &touch.pos();
                    touch.disableScroll();
                    goto set;
                }
    ...
    if (/*gpc.visible &&*/ visible())
        if (GuiSkin *skin = getSkin()) {
            ...
            if (skin->slider.background)
                skin->slider.background->extendedRect(rect, ext_rect);
            else
                ext_rect = rect;
            if (skin->slider.progress) {
                Rect r;
                if (skin->slider.progress->extendedRect(rect, r))
                    ext_rect |= r;
            }
            ...
            if (Cuts(ext_rect, gpc.clip)) {
                ...
                if (skin->slider.background) {
                    if (_vertical)
                        skin->slider.background->drawVertical(skin->slider.background_color, TRANSPARENT, background_rect);
                    else
                        skin->slider.background->draw(skin->slider.background_color, TRANSPARENT, background_rect);
                } else if (skin->slider.background_color.a)
                    background_rect.draw(skin->slider.background_color);

                // progress
                if (skin->slider.progress) {
                    if (skin->slider.draw_progress_partial) {
                        if (_vertical)
                            skin->slider.progress->drawVerticalFrac(skin->slider.progress_color, TRANSPARENT, background_rect, T(), false);
                        else
                            skin->slider.progress->drawFrac(skin->slider.progress_color, TRANSPARENT, background_rect, T(), false);
                    } else {
                        Rect prog_rect = background_rect;
                        if (_vertical)
                            prog_rect.max.y = background_rect.lerpY(T());
                        else
                            prog_rect.max.x = background_rect.lerpX(T());
                        if (_vertical)
                            skin->slider.progress->drawVertical(skin->slider.progress_color, TRANSPARENT, prog_rect);
                        else
                            skin->slider.progress->draw(skin->slider.progress_color, TRANSPARENT, prog_rect);
                    }
                } else if (skin->slider.progress_color.a) {
                    Rect prog_rect = background_rect;
                    if (_vertical)
                        prog_rect.max.y = background_rect.lerpY(T());
                    else
                ...
                if (skin->slider.slider) {
                    ...
                    if (_vertical) {
                        Rect_C r(rect.centerX(), rect.lerpY(T()), rect.w(), rect.w() * skin->slider.slider->image.aspect());
                        D.alignScreenToPixel(r);
                        skin->slider.slider->drawVertical(skin->slider.slider_color, highlight, r);
                    } else {
                        Rect_C r(rect.lerpX(T()), rect.centerY(), rect.h() * skin->slider.slider->image.aspect(), rect.h());
                        D.alignScreenToPixel(r);
