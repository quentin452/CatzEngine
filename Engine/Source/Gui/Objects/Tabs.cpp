...
Tab &Tab::setCornerTab(Bool right, Bool top) {
    _sub_type = (top     ? right ? BUTTON_TYPE_TAB_TOP_RIGHT : BUTTON_TYPE_TAB_TOP_LEFT
                 : right ? BUTTON_TYPE_TAB_BOTTOM_RIGHT
...
void Tabs::setButtonSubType() {
    ...
    if (_actual_layout == TABS_HORIZONTAL) {
        first = BUTTON_TYPE_TAB_LEFT;
        middle = BUTTON_TYPE_TAB_HORIZONTAL;
        last = BUTTON_TYPE_TAB_RIGHT;
    } else {
        first = BUTTON_TYPE_TAB_TOP;
        middle = BUTTON_TYPE_TAB_VERTICAL;
        last = BUTTON_TYPE_TAB_BOTTOM;
    }

    REPA(T) {
        Tab &tab = T.tab(i);
        if (tab._sub_type < BUTTON_TYPE_TAB_CORNER_START || tab._sub_type > BUTTON_TYPE_TAB_CORNER_END) // if wasn't set to be a corner
            tab._sub_type = ((i == first_i) ? first : (i == last_i) ? last
                                                                    : middle);
    }
}
void Tabs::setParams() {
...
Tabs &Tabs::create(C Tabs &src) {
    if (this != &src) {
        if (!src.is())
            ...
            _tabs.setNum(src.tabs());
            FREPAO(_tabs).create(src.tab(i))._parent = this;
        }
    }
    return T;
}
    ...
    if (_tabs.elms()) {
        ...
        if (_actual_layout == TABS_HORIZONTAL) {
            ...
            if (autoSize()) {
                ...
                FREPA(T)
                text_width += tab(i).textWidth(&size);
                Flt padd = (w - text_width) / _tabs.elms();
                FREPA(T) {
                    Tab &tab = T.tab(i);
                    Flt w = tab.textWidth(&size) + padd;
                ...
                w /= _tabs.elms();
                REPA(T)
                tab(i).rect(Rect(rect().min.x + (w + space()) * i, rect().min.y, rect().min.x + (w + space()) * i + w, rect().max.y));
            }
        } else {
            h -= (_tabs.elms() - 1) * space();
            h /= _tabs.elms();
            REPA(T)
            tab(i).rect(Rect(rect().min.x, rect().max.y - (h + space()) * i - h, rect().max.x, rect().max.y - (h + space()) * i));
        }
    }
}
...
Tabs &Tabs::set(Int i, SET_MODE mode) {
    ...
    if (T() != i) {
        ...
        REPAD(j, T)
        tab(j).set(i == j);
        if (valid() && InRange(T(), T))
            tab(T())._lit = 0; // disable highlight for newly selected tab
        if (kb)
            if (InRange(T(), T))
                tab(T()).kbSet();
            else
...
Tab &Tabs::New(C Str &text, Int i) {
    ...
    if (_sel >= i)
        _sel++;
    Tab &tab = _tabs.NewAt(i).create(text);
    tab._parent = this;
    tab.skin = skin();
    setRect();
    return tab;
...
GuiObj *Tabs::test(C GuiPC &gpc, C Vec2 &pos, GuiObj *&mouse_wheel) {
    if (/*gpc.visible &&*/ visible() && Cuts(pos, gpc.clip)) {
        if (InRange(T(), T))
            if (GuiObj *go = tab(T())._children.test(gpc, pos, mouse_wheel))
                return go;
...
void Tabs::update(C GuiPC &gpc) {
    ...
    FREPA(T) {
        ...
        tab.update(gpc_this);
        if ((T() == i) != tab())
            toggle(i); // if Tab update state changed
    }
...
void Tabs::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible()) {
        ...
        Vec2 ofs = D.alignScreenToPixelOffset(r.lu());
        r += ofs;
        GuiPC gpc_this(gpc, visible(), enabled());
        gpc_this.offset += ofs; // adjust offset so that all tabs have the same pixel alignment as the first one
        FREPA(T)
        tab(i).draw(gpc_this);
        if (InRange(T(), T))
            tab(T())._children.draw(gpc);
    }
}
/******************************************************************************/
...
Bool Tabs::load(File &f, CChar *path) {
    ...
    if (super::load(f, path))
        ...
        case 0: {
            ...
            _tabs.clear().setNum(f.getInt());
            FREPA(_tabs) {
                Tab &tab = _tabs[i];
                if (!tab.load(f, path))
                    goto error;
                tab.enabled(true);
            } // restore enabled because in the past tabs were disabled on click
            if (f.ok()) {
                _actual_layout = actualLayout();
                setParams();
                return true;
