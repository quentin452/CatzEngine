...
Flt TextLine::clientWidth() C {
    Flt w = rect().w();
    if (showClear() && reset.visible())
        w -= reset.rect().w();
    return w;
}
...
TextLine &TextLine::password(Bool on) {
    if (password() != on) {
        _edit.password = on;
        setTextInput();
    }
...
TextLine &TextLine::number(Bool on) {
    if (number() != on) {
        _flag ^= NUMBER;
        setTextInput();
    }
...
TextLine &TextLine::email(Bool on) {
    if (email() != on) {
        _flag ^= EMAIL;
        setTextInput();
    }
...
void TextLine::adjustOffset(Bool margin) {
    if (GuiSkin *skin = getSkin())
        if (TextStyle *text_style = skin->textline.text_style()) {
            ...
            ts.size = rect().h() * skin->textline.text_size;
            ...
            if (x < margin_w)
                _offset = Min(_offset - x + w * 0.5f, 0);
            else if (x > w - margin_w)
                _offset = Min(_offset - x + w * 0.5f, 0);
        }
}
...
Rect TextLine::localTextRect(Int index0, Int index1) C {
    Vec2 lx = localTextPosX(index0, index1);
    if (lx.x > lx.y)
        lx.swap();
...
GuiObj *TextLine::test(C GuiPC &gpc, C Vec2 &pos, GuiObj *&mouse_wheel) {
    if (GuiObj *go = super::test(gpc, pos, mouse_wheel)) {
        if (showClear())
            if (GuiObj *go = reset.test(gpc, pos, mouse_wheel))
                return go;
        return go;
    }
    return null;
                ...
                if (TextStyle *text_style = skin->textline.text_style()) {
                    ...
                    if (ButtonDb(mt_state) && _text.is()) {
                        ...
                        if (mt_state & (BS_PUSHED | BS_TAPPED | BS_LONG_PRESS)) // check BS_TAPPED|BS_LONG_PRESS too, because touches activate TextEdits only on BS_TAPPED|BS_LONG_PRESS (to allow for Touch-Scroll) and we want to set cursor in that case as well
                        {
                            ...
                            MAX(l, gpc.clip.min.x);
                            MIN(r, gpc.clip.max.x);          // clipped_text_rect
                            ...
                            if (mt_pos->x <= l)
                                _offset = Min(0, Time.d() * 2 + _offset);
                            else if (mt_pos->x >= r)
                                _offset = Min(0, Max(Time.d() * -2 + _offset, -ts.textWidth(text) + w / 2));
...
void TextLine::draw(C GuiPC &gpc) {
    if (/*gpc.visible &&*/ visible())
        if (GuiSkin *skin = getSkin()) {
            ...
            if (enabled) {
                panel_image = skin->textline.normal();
                panel_color = &skin->textline.normal_panel_color;
                text_color = &skin->textline.normal_text_color;
            } else {
                panel_image = skin->textline.disabled();
                panel_color = &skin->textline.disabled_panel_color;
            ...
            if (Cuts(ext_rect, gpc.clip)) {
                D.clip(gpc.clip);
                if (panel_image)
                    panel_image->draw(*panel_color, TRANSPARENT, rect);
                else if (panel_color->a)
                    rect.draw(*panel_color, true);
                if (skin->textline.rect_color.a)
                    rect.draw(skin->textline.rect_color, false);
                ...
                if (TextStyle *text_style = skin->textline.text_style()) {
                    ...
                    if (T().is() || active || show_find || hint.is()) {
                        Rect clip_rect = rect;
                        clip_rect.max.x = clip_rect.min.x + clientWidth();
                        D.clip(clip_rect & gpc.clip);

                        TextStyleParams ts = *text_style;
                        ts.size = rect.h() * skin->textline.text_size;
                        ts.align.set(1, 0);
                        ts.color = ColorMul(ts.color, *text_color);
                        ts.image_color.a = (enabled ? 255 : 128);
#if DEFAULT_FONT_FROM_CUSTOM_SKIN
                        if (!ts.font())
                            ts.font(skin->font()); // adjust font in case it's empty and the custom skin has a different font than the 'Gui.skin'
#endif
                        ...
                        if (T().is() || active) {
                            ...
                            if (show_find)
                                if (Image *image = skin->textline.find_image()) {
...
Bool TextLine::load(File &f, CChar *path) {
    ...
    if (super::load(f, path))
        ...
        case 6: {
            ...
            Bool kb_catch;
            f >> kb_catch >> kb_lit >> show_find >> _edit.password >> _max_length;
            f._getStr1(hint)._getStr1(_text);
            reset.visible(f.getBool());
            _skin.require(f._getAsset(), path);
            if (f.ok())
                return true;
