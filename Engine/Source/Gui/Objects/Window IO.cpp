...
static Str FileSize(C WindowIO::File &f) {
    switch (f.type) {
    case FSTD_FILE:
        return TextInt((f.size + 1023) >> 10, -1, 3) + " KB";
    case FSTD_LINK:
        return "Unknown";
    default:
        return S;
...
void WindowIORecent(C Str &path) {
    if (path.is()) {
        REPA(WindowIORecents)
        if (EqualPath(WindowIORecents[i], path)) {
            REPD(j, i)
            Swap(WindowIORecents[j], WindowIORecents[j + 1]); // move to 1st position
            return;
        }
...
static void PathChanged(C CMemPtr<MenuPush> &mps, QuickPathSelector &qps) {
    if (mps.elms()) {
        C Str &path = mps.last().name;
        if (path.is()) {
            if (path == QPS_DATA)
                qps.setPath(DataPath());
            else if (path == QPS_DESKTOP)
                qps.setPath(SystemPath(SP_DESKTOP));
            else if (path == QPS_ONEDRIVE)
                qps.setPath(SystemPath(SP_ONE_DRIVE));
            else if (path == QPS_ADD_FAVORITE) {
...
static void QPSChangedButton(QuickPathSelector &qps) {
    if (qps.Button::operator()())
        qps.setData(); // set most recent data
    if (qps._func)
        qps._func(&qps); // call stored callback
}
...
void QuickPathSelector::setData() {
    Node<MenuElm> n;
    {
        if (DataPath().is())
            n += QPS_DATA;
        if (SystemPath(SP_DESKTOP).is())
            n += QPS_DESKTOP;
        if (SystemPath(SP_ONE_DRIVE).is())
            n += QPS_ONEDRIVE;
        Bool header = false;
        FREPA(WindowIOFavorites) {
            ...
            if (p.is()) {
                if (!header) {
                    ...
                    if (n.children.elms())
                        n++;
                    Node<MenuElm> &c = (n += QPS_FAVORITES);
                    c.flag(MENU_NOT_SELECTABLE);
                }
                n += p;
            }
        }
...
void WindowIO::QPS::setPath(C Str &path) {
    win_io.path(S, path);
    win_io.getList();
}
/******************************************************************************/
...
WindowIO &WindowIO::create(C Str &ext, C Str &path, C Str &sub_path, void (*load)(C Str &name, Ptr user), void (*save)(C Str &name, Ptr user), Ptr user) {
    ...
    region += list.create(win_io_file_column, Elms(win_io_file_column)).setElmType(MEMBER(WindowIO::File, type));
    list.sort_column[0] = 0;
    list.flag |= LIST_TYPE_SORT | LIST_SCALABLE;
    list.cur_mode = LCM_ALWAYS;
    list.zoom_min = 1 / 1.2f;
    list.zoom_max = 1.2f;
    Node<MenuElm> node;
    T += menu.create(node).hide();

    Gui += rename_window.create().level(1).rect(Rect_C(0, 0, 1.2f, 0.14f)).hide();
    rename_window.button[2].show();
...
WindowIO &WindowIO::ext(C Str &ext, C Str &desc) {
    ...
    T._ext = ext;
    T._ext_desc = desc;
    _dot_exts.setNum(exts.elms());
    FREPAO(_dot_exts) = S + '.' + exts[i];
    return T;
}
WindowIO &WindowIO::path(C Str &path, C Str &sub_path) {
    T._path = Replace(NormalizePath(MakeFullPath(path)), '/', '\\').tailSlash(true);
    T._sub_path = Replace(NormalizePath(sub_path), '/', '\\').tailSlash(true);
    if (StartsPath(_sub_path, ".."))
        _sub_path.clear();
    ...
    } else if (StartsPath(full, root)) // T._path starts with root but only after combining with T._sub_path - "/../"+"documents/xxx" starts with "/../documents"
    {
...
static Bool ProcessPaths(Mems<FileParams> &fps, C Str &path, C Str &sub_path) {
    REPA(fps) {
        ...
        if (fp.name.is()) {
            ...
            if (path.is() && !StartsPath(fp.name, path))
                return false; // if 'name' doesn't start with 'path' then fail
        }
        if (!ProcessPaths(fp.nodes, path, sub_path))
            return false; // if nodes failed
    }
    return true;
}
...
Mems<FileParams> WindowIO::final(C Str &name) C // this will always include 'path', or be empty on error
{
    ...
    if (_mode == WIN_IO_DIR && !fps.elms())
        fps.New().name = path() + subPath();
    else if (!ProcessPaths(fps, path(), subPath()))
        fps.clear(); // if processing failed then clear all
    return fps;
}
...
static void ProcessExt(Mems<FileParams> &fps, C WindowIO &wio) {
    REPA(fps) {
        ...
        if (fp.name.is() && !wio.goodExt(fp.name) && (wio.ext_mode == WIN_IO_EXT_ALWAYS || !GetExt(fp.name).is()))
...
void WindowIO::Ok() {
    ...
    if (fps.elms()) {
        if (_mode == WIN_IO_DIR) {
            if (_load)
                _load(FileParams::Encode(fps), _func_user);
            ...
            {
                // add extension
                if (ext_mode && ((_mode == WIN_IO_SAVE) ? _dot_exts.elms() >= 1 : _dot_exts.elms() == 1))
                    ProcessExt(fps, T); // for saving add if we have at least 1, for other mode add if we have only 1
                ...
                if (_mode == WIN_IO_LOAD) {
                    ...
                    FREPA(fps)
                    if (fps[i].name.is()) {
                        WindowIORecent(GetPath(fps[i].name));
                        break;
                    }
                ...
                if (hide)
                    T.hide(); // hide at the end, in case it will delete this object
                              // !! don't perform any operations on this object afterwards !!
            }
        }
    }
...
void WindowIO::createDir() {
    auto fps = final();
    if (fps.elms() == 1 && fps[0].name.is()) {
        if (FCreateDirs(fps[0].name)) {
            path(_path, SkipStartPath(fps[0].name, _path));
            getList();
            textline.clear();
        } else
            Gui.msgBox("Error", "Failed to create folder.");
    } else
        Gui.msgBox(S, "Please first enter the folder name in the textline.");
}
void WindowIO::renameDo() {
    if (File *f = list())
        if (f->type != FSTD_DRIVE) {
    ...
    if (File *f = list())
        if (f->type != FSTD_DRIVE) {
            ...
            remove_window.setTitle(MLT(S + "Move to Recycle Bin \"" + f->name + '"',
                                       PL, S + u"Przenieś do kosza \"" + f->name + '"'))
                .activate();
            remove_yes.activate();
        }
}
...
void WindowIO::exploreDo() {
    if (_path.is() || _sub_path.is())
        Explore(_path + _sub_path);
}
...
void WindowIO::setBar() {
    ...
    case WIN_IO_PM_NONE:
        switch (_mode) {
        case WIN_IO_LOAD:
            setTitle(S + MLTC(u"Load ", PL, u"Wczytaj ") + ext);
            break;
        case WIN_IO_SAVE:
            setTitle(S + MLTC(u"Save ", PL, u"Zapisz ") + ext);
            break;
        case WIN_IO_DIR:
            setTitle(MLTC(u"Select Directory", PL, u"Wybierz Katalog"));
            break;
        }
        break;
    ...
    case WIN_IO_PM_PART:
        switch (_mode) {
        case WIN_IO_LOAD:
            setTitle(S + MLTC(u"Load ", PL, u"Wczytaj ") + ext + " - " + subPath());
            break;
        case WIN_IO_SAVE:
            setTitle(S + MLTC(u"Save ", PL, u"Zapisz ") + ext + " - " + subPath());
            break;
        case WIN_IO_DIR:
            if (FullPath(textline()))
                setTitle(textline());
            else
                setTitle(subPath() + textline());
            break;
        }
        break;
    ...
    case WIN_IO_PM_FULL:
        switch (_mode) {
        case WIN_IO_LOAD:
            setTitle(S + MLTC(u"Load ", PL, u"Wczytaj ") + ext + " - " + SkipStartPath(path() + subPath(), DataPath()));
            break;
        case WIN_IO_SAVE:
            setTitle(S + MLTC(u"Save ", PL, u"Zapisz ") + ext + " - " + SkipStartPath(path() + subPath(), DataPath()));
            break;
        case WIN_IO_DIR:
            if (FullPath(textline()))
                setTitle(SkipStartPath(textline(), DataPath()));
            else
                setTitle(SkipStartPath(path() + subPath() + textline(), DataPath()));
            break;
        }
        break;
    }
...
void WindowIO::getList() {
    ...
    FileFind ff;
    if (_path.is() || _sub_path.is())
        ff.find(_path + _sub_path);
    else
        ff.findDrives();
    for (; ff();)
        if (!(ff.attrib & FATTRIB_HIDDEN))
            switch (ff.type) {
            case FSTD_DRIVE:
                file.New().set(ff.type, 0, ff.name.tailSlash(false));
                break;
            case FSTD_DIR:
                file.New().set((_dir_operate && goodExt(ff.name)) ? FSTD_LINK : ff.type, 0, ff.name);
                break;
            case FSTD_FILE:
                if (!_dir_operate && _mode != WIN_IO_DIR && goodExt(ff.name))
                    file.New().set(ff.type, ff.size, ff.name);
                break;
            }
...
void WindowIO::setFile(C Str &name) {
    REPA(file)
    if (Equal(file[i].name, name)) {
        list.cur = list.absToVis(i);
        if (list.cur >= 0)
            list.scrollTo(list.cur, true, 1);
        return;
    }
}
/******************************************************************************/
void WindowIO::back() {
    if (_sub_path.last() == '\\' || _sub_path.last() == '/') {
        ...
        for (; _sub_path.is() && _sub_path.last() != '\\' && _sub_path.last() != '/';)
            _sub_path.removeLast();
        getList();
        setFile(base);
    }
...
WindowIO &WindowIO::show() {
    if (hidden()) {
        super::show();
        if (_dot_exts.elms() == 1)
            textline.set(SkipEnd(textline(), _dot_exts[0])); // skip only if we have 1 extension specified
        getList();
    }
    return T;
}
...
Rect WindowIO::sizeLimit() C {
    ...
    r.min.set(Max(0.70f, ((up.visible() && up.is()) ? up.rect().w() : 0) + ((rename.visible() && rename.is()) ? rename.rect().w() : 0) + ((remove.visible() && remove.is()) ? remove.rect().w() : 0) + ((explore.visible() && explore.is()) ? explore.rect().w() : 0) + ((quick.visible() && quick.is()) ? quick.rect().w() : 0) + 0.03f) + _border * 2, 0.48f + _border * 2);
    return r;
}
/******************************************************************************/
void WindowIO::setRect() {
    Rect r(0, -clientHeight(), clientWidth(), 0);
    r.extendX(-_border).min.y += _border;
    r.max.y -= _border * 0.5f; // extend top less because there's always the window bar
    ...
    if (rename.is() && rename.visible()) {
        bw += rename.rect().w();
        n++;
    }
    if (remove.is() && remove.visible()) {
        bw += remove.rect().w();
        n++;
    }
    if (explore.is() && explore.visible()) {
        bw += explore.rect().w();
        n++;
    }
    if (quick.is() && quick.visible()) {
        bw += quick.rect().w();
        n++;
    }
    n--;
    ...
    rename.pos(pos);
    if (rename.is() && rename.visible())
        pos.x += rename.rect().w() + s;
    remove.pos(pos);
    if (remove.is() && remove.visible())
        pos.x += remove.rect().w() + s;
    explore.pos(pos);
    if (explore.is() && explore.visible())
        pos.x += explore.rect().w() + s;
    quick.pos(pos);
    if (quick.is() && quick.visible())
        pos.x += quick.rect().w() + s;
...
void WindowIO::update(C GuiPC &gpc) {
    ...
    if (Gui.window() == &rename_window) // rename
    {
        if ((Kb.k(KB_ENTER) || Kb.k(KB_NPENTER)) && Kb.k.first()) {
            Kb.eatKey();
            auto fps = final(rename_textline());
            if (fps.elms() == 1 && fps[0].name.is() && FRename(_op_name, fps[0].name)) {
                getList();
        ...
        } else if ((Kb.k(KB_ESC) || Kb.k(KB_NAV_BACK)) && Kb.k.first() || Ms.bp(2)) {
            Kb.eatKey();
            Ms.eat(2);
            rename_window.hide();
            disabled(false);
    ...
    } else if (Gui.window() == &remove_window) // delete
    {
        if ((Kb.k(KB_ESC) || Kb.k(KB_NAV_BACK)) && Kb.k.first() || Ms.bp(2)) {
            Kb.eatKey();
            Ms.eat(2);
            removeCancel();
        }
    } else if (Gui.window() == &overwrite_window) // overwrite
    {
        if ((Kb.k(KB_ESC) || Kb.k(KB_NAV_BACK)) && Kb.k.first() || Ms.bp(2)) {
            Kb.eatKey();
            Ms.eat(2);
    ...
    if (Gui.window() == this && enabled()) {
        ...
        if (_mode == WIN_IO_DIR) {
            if (Gui.ms() == &list && Ms.bp(0) && !Ms.bd(0) && list.cur >= 0) {
            ...
            } else if (Gui.kb() == &list && (Kb.k(KB_ENTER) || Kb.k(KB_NPENTER)) && Kb.k.first() && (list.cur >= 0 || list.lit >= 0)) {
                ...
                REPA(Touches) {
                    ...
                    if (touch.guiObj() == &list && touch.tapped()) {
                        if (File *f = list()) {
                            textline.clear();
                            enter(f->name);
                        }
            ...
            if (list_cur != list.cur)
                if (File *f = list())
                    if (f->type == (_dir_operate ? FSTD_LINK : FSTD_FILE))
                        textline.set(f->name);
            ...
            if (list.cur >= 0) {
                REPA(Touches) {
                    Touch &touch = Touches[i];
                    if (touch.guiObj() == &list && touch.db()) {
                        touch_db = true;
                        break;
                    }
                }

                if ((Gui.kb() == &list && (Kb.k(KB_ENTER) || Kb.k(KB_NPENTER)) && Kb.k.first()) || (Gui.ms() == &list && Ms.bd(0)) || touch_db)
                    if (File *f = list())
                        if (f->type == FSTD_DRIVE || f->type == FSTD_DIR) {
                            Kb.eatKey();
                            Ms.eat(0);
