...
static void GetFracMulAddFromImage(Int x0, Int x1, Int y0, Int y1, C Image *image, Vec4 &frac_mul_add, Bool mirror_x = false, Bool mirror_y = false) {
    if (image) {
        ...
        frac_mul_add.x = Flt(image->w()) / w;
        frac_mul_add.y = -x0 * frac_mul_add.x + frac_mul_add.x * 0.5f - 0.5f;
        frac_mul_add.z = Flt(image->h()) / h;
        frac_mul_add.w = -y0 * frac_mul_add.z + frac_mul_add.z * 0.5f - 0.5f;
    }
}
static void ScaleTex(Flt &tex, Flt scale) { tex = (tex - 0.5f) * scale + 0.5f; }
static void ScaleTex(Vec2 &tex, C Vec2 &scale) {
    ScaleTex(tex.x, scale.x);
    ScaleTex(tex.y, scale.y);
}
/******************************************************************************/
struct SmoothDepth {
    Bool is[2];
...
struct SectionParams {
    ...
    void setColors(C PanelImageParams::Section &src) {
        ...
        Vec4 src_color = Convert(src.color);
        outer_color *= src_color;
        inner_color *= src_color;
    }
    ...
    void setDepthOverlay(C PanelImageParams::Section &src, Int resolution) {
        ...
        if (overlay = src.depth_overlay) {
            if (overlay->compressed() || src.depth_overlay_params.blur)
                if (overlay->copy(overlay_image, -1, -1, -1, overlay->typeInfo().a ? IMAGE_F32_4 : IMAGE_F32, IMAGE_SOFT, 1, FILTER_BEST, IC_IGNORE_GAMMA))
    ...
    void setColorOverlay(C PanelImageParams::Section &src, Int resolution) {
        ...
        if (overlay = src.color_overlay) {
            if (overlay->compressed() || src.color_overlay_params.blur)
                if (overlay->copy(overlay_image, -1, -1, -1, ImageTypeUncompressed(overlay->type()), IMAGE_SOFT, 1))
                    overlay = &overlay_image;
                else
                    overlay = null;
            if (overlay) {
    ...
    void setReflection(C PanelImageParams::Section &src) {
        if (reflection = src.reflection) {
            if (reflection->compressed() || reflection->cube())
                if (reflection->copy(reflection_image, -1, -1, -1, ImageTypeUncompressed(reflection->type()), IMAGE_SOFT, 1))
                    reflection = &reflection_image;
                else
                    reflection = null;
            if (reflection) {
                reflection->lockRead();
    ...
    void setOffset(Flt offset, Flt offset_top, Int resolution, Int image_w, Int image_h) {
        ...
        norm_add = frac_add;
        REP(2)
        AdjustFracMulAdd_1_1(norm_mul.c[i], norm_add.c[i]);
    }
...
Bool PanelImageParams::ImageParams::save(File &f) C {
    ...
    f << blur_clamp << blur << uv_scale << uv_offset << uv_warp << intensity;
    f.putUInt(mode);
    return f.ok();
}
...
PanelImageParams::Section::Section() {
    ...
    outer_border_color.a = 128;
    inner_border_color = WHITE;
    inner_border_color.a = 64;
...
Bool PanelImageParams::Section::load(File &f) {
    switch (f.decUIntV()) {
    case 0: {
        f >> size >> top_offset >> round_depth >> outer_depth >> inner_depth >> inner_distance >> specular >> reflection_intensity >> smooth_depth >> color >> outer_color >> inner_color >> color_top >> color_bottom >> color_left >> color_right >> outer_border_color >> inner_border_color >> prev_border_color;
...
struct PanelImageCreate {
    struct Line {
        ...
        Flt yAtX(Flt x) C {
            if (!dir.x)
                return pos.y;
            Flt dx = x - pos.x, d = dx / dir.x;
            return pos.y + d * dir.y;
        }
        Flt xAtY(Flt y) C {
            if (!dir.y)
                return pos.x;
            Flt dy = y - pos.y, d = dy / dir.y;
            return pos.x + d * dir.x;
        }
    };
    struct ImageSrc {
    ...
    ~PanelImageCreate() {
        ...
        if (right_image)
            right_image->unlock();
        if (top_left_image)
            top_left_image->unlock();
        if (top_right_image)
            top_right_image->unlock();
        if (bottom_left_image)
            bottom_left_image->unlock();
        if (bottom_right_image)
            bottom_right_image->unlock();
    }
    C Image *getImage(C Image *image) {
        if (image && image->compressed()) {
            REPA(temp_images)
            if (temp_images[i].src == image) {
                image = &temp_images[i].soft;
                goto found;
            } // goto found and lock it, because it's always unlocked in the codes below
            ImageSrc &temp = temp_images.New();
            temp.src = image;
            if (image->copy(temp.soft, -1, -1, -1, IMAGE_R8G8B8A8_SRGB, IMAGE_SOFT, 1))
                image = &temp.soft;
            else
                image = null;
        }
    ...
    Bool create() {
        if (image.createSoft(resolution * params.width, resolution * params.height, 1, IMAGE_R8G8B8A8_SRGB) && depth_map.createSoft(image.w(), image.h(), 1, IMAGE_F32) && dist_map.createSoft(image.w(), image.h(), 1, IMAGE_F32_4)) {
            REPA(params.lights) {
            ...
            image_size_2i = image_size / 2;
            image_size_2 = image_size * 0.5f;
            smooth_depth = params.smooth_depth;
            corner_size = resolution * 0.5f * params.round_corners;
            REPAO(sps).setParams(params.sections[i], resolution);
            sps[0].setSize(1 - params.sections[1].size, 0);
            ...
            corner_line[0][0].pos.set(0, 0 + cut_corner_amount);
            corner_line[0][0].normal.set(1, cut_corner_slope);
            corner_line[0][1].pos.set(image_size1.x, 0 + cut_corner_amount);
            corner_line[0][1].normal.set(-1, cut_corner_slope);
            corner_line[1][0].pos.set(0, image_size1.y - cut_corner_amount);
            corner_line[1][0].normal.set(1, -cut_corner_slope);
            corner_line[1][1].pos.set(image_size1.x, image_size1.y - cut_corner_amount);
            corner_line[1][1].normal.set(-1, -cut_corner_slope);
            Bool line_on = (cut_corner_amount > 0.5f);
            REPD(y, 2)
            REPD(x, 2)
            corner_line[y][x].finalize(line_on && params.cut_corners[y][x]);

            side_line[0].pos.set(0, (params.left_slope < 0) ? 0 : image_size1.y);
            side_line[0].normal.set(1, params.left_slope / params.height);
            side_line[0].finalize(!Equal(params.left_slope, 0));

            side_line[1].pos.set(image_size1.x, (params.right_slope < 0) ? 0 : image_size1.y);
            side_line[1].normal.set(-1, params.right_slope / params.height);
            ...
            if (params.cut_left) {
                ...
                side_line[0].on = false;
            }
            if (params.cut_right) {
                REPD(y, 2)
                corner_line[y][1].on = false;
                side_line[1].on = false;
            }
            ...
            Flt tex = 0, tex_top;
            // set section #0
            if (params.sections[0].outer_border_color.a)
                MAX(tex, params.border_size);
            if (params.sections[0].inner_border_color.a)
                MAX(tex, params.border_size * 2);
            if (params.depth && !Equal(params.sections[0].outer_depth, params.sections[0].inner_depth))
                MAX(tex, params.sections[0].inner_distance);
            // add section #1
            tex += sps[1].size;
            ...
            Flt global = 0;
            if (!Equal(params.depth, 0))
                MAX(global, params.inner_distance);
            if (params.inner_glow_color.a)
                MAX(global, params.inner_glow_radius);
            ...
            if (top_left_image || top_image)
                MAX(panel_image._tex_x[0][0], top_x1); // any of these images require    'top_x1'
            if (top_right_image || top_image)
                MIN(panel_image._tex_x[0][1], top_x2); // any of these images require    'top_x2'
            if (left_image || center_image)
                MAX(panel_image._tex_x[1][0], x1); // any of these images require        'x1'
            if (right_image || center_image)
                MIN(panel_image._tex_x[1][1], x2); // any of these images require        'x2'
            if (bottom_left_image || bottom_image)
                MAX(panel_image._tex_x[2][0], bottom_x1); // any of these images require 'bottom_x1'
            if (bottom_right_image || bottom_image)
                MIN(panel_image._tex_x[2][1], bottom_x2); // any of these images require 'bottom_x2'

            if (top_left_image || top_image || top_right_image || left_image || center_image || right_image)
                MAX(panel_image._tex_y[0], y1); // any of these images require 'y1'
            if (bottom_left_image || bottom_image || bottom_right_image || left_image || center_image || right_image)
                MIN(panel_image._tex_y[1], y2); // any of these images require 'y2'
            ...
            REPD(y, 2) {
                ...
                if (l0.on) {
                    ...
                    if (y)
                        ...
                        n.y++;
                    n /= Abs(n.y);
                    Flt lt = n.x * tex; // line.normal+Vec2(0, 1)
                    MAX(l, l0.xAtY(0) + lt);
                    MAX(l, l0.xAtY(image_size1.y) + lt);
                }
                C Line &l1 = corner_line[y][1];
                if (l1.on) {
                    Vec2 n = l1.normal;
                    if (y)
                        n.y--;
                        ...
                        n.y++;
                    n /= Abs(n.y);
                    Flt rt = n.x * tex; // line.normal+Vec2(0, 1)
                    MIN(r, l1.xAtY(0) + rt);
                    MIN(r, l1.xAtY(image_size1.y) + rt);
                }
            }
            if (side_line[0].on) {
                Vec2 n = side_line[0].normal;
                if (n.y > 0)
                    n.y++;
                else
                    n.y--;
                n /= Abs(n.y);
                Flt t = n.x * tex; // normal+Vec2(0, 1)
                MAX(l, side_line[0].xAtY(0) + t);
                MAX(l, side_line[0].xAtY(image_size1.y) + t);
            }
            if (side_line[1].on) {
                Vec2 n = side_line[1].normal;
                if (n.y > 0)
                    n.y++;
                else
                    n.y--;
            ...
            if (l + tex_ofs < r)
                ...
                include_size_mid[0] = true; // if lines overlap then we need to force middle part in the size calculation

            Flt t = panel_image._tex_y[0], b = panel_image._tex_y[1];
            REPD(x, 2) {
                C Line &l0 = corner_line[0][x];
                if (l0.on) {
                    Vec2 n = l0.normal;
                    if (x)
                        n.x--;
                        ...
                        n.x++;
                    n /= Abs(n.x);
                    Flt tt = n.y * tex; // line.normal+Vec2(1, 0)
                    MAX(t, l0.yAtX(0) + tt);
                    MAX(t, l0.yAtX(image_size1.x) + tt);
                }
                C Line &l1 = corner_line[1][x];
                if (l1.on) {
                    Vec2 n = l1.normal;
                    if (x)
                        n.x--;
                        ...
                        n.x++;
                    n /= Abs(n.x);
                    Flt bt = n.y * tex; // line.normal+Vec2(1, 0)
                    MIN(b, l1.yAtX(0) + bt);
            ...
            if (t + tex_ofs < b) {
                ...
                include_size_mid[1] = true; // if lines overlap then we need to force middle part in the size calculation

            if (params.force_uniform_stretch[0])
                REPAD(y, panel_image._tex_x) {
            ...
            alignTex();
            REPAD(y, panel_image._tex_x) {
                if (panel_image._tex_x[y][0] > EPS)
                    MIN(panel_image._tex_x[y][0] += tex_ofs, image_size.x); // if there's anything in the left  section, then apply offset to avoid blending with it
                if (panel_image._tex_x[y][1] < image_size.x - EPS)
                    MAX(panel_image._tex_x[y][1] -= tex_ofs, 0); // if there's anything in the right section, then apply offset to avoid blending with it
                if (panel_image._tex_x[y][0] > panel_image._tex_x[y][1])
                    panel_image._tex_x[y][0] = panel_image._tex_x[y][1] = Avg(panel_image._tex_x[y][0], panel_image._tex_x[y][1]); // this can happen because we're applying 'tex_ofs'
                REPAD(x, panel_image._tex_x[y])
                panel_image._tex_x[y][x] /= image_size.x;
            }
            Bool top_mid_bottom[3];
            if (top_mid_bottom[0] = (panel_image._tex_y[0] > EPS))
                MIN(panel_image._tex_y[0] += tex_ofs, image_size.y); // if there's anything in the top    section, then apply offset to avoid blending with it
            if (top_mid_bottom[2] = (panel_image._tex_y[1] < image_size.y - EPS))
                MAX(panel_image._tex_y[1] -= tex_ofs, 0);                        // if there's anything in the bottom section, then apply offset to avoid blending with it
            top_mid_bottom[1] = (panel_image._tex_y[0] < panel_image._tex_y[1]); // if there's anything in the middle section
            if (panel_image._tex_y[0] > panel_image._tex_y[1])
                panel_image._tex_y[0] = panel_image._tex_y[1] = Avg(panel_image._tex_y[0], panel_image._tex_y[1]); // this can happen because we're applying 'tex_ofs'
            ...
            if (include_size_mid[0])
                panel_image._min_size.x = mulx;
            else {
                panel_image._min_size.x = 0;
                REPAD(y, panel_image._size_x)
                MAX(panel_image._min_size.x, Max(panel_image._size_x[y][0], 0) + Max(panel_image._size_x[y][1], 0));
            }
            if (include_size_mid[1])
                panel_image._min_size.y = muly;
            else
                panel_image._min_size.y = Max(panel_image._size_y[0], 0) + Max(panel_image._size_y[1], 0);
    ...
    void setDepthY(Int y) {
        REPD(x, depth_map.w()) {
            ...
            d = (image_size1.x - pos.x) / resolution;
            MIN(dist_global, d);
            MIN(dist_sect, d);
            MIN(dist_x, d);
            if (!params.cut_right) {
            ...
            d = (image_size1.y - pos.y) / resolution;
            MIN(dist_global, d);
            MIN(dist_sect, d);
            MIN(dist_y, d);
            if (!params.cut_bottom) {
            ...
            if (corner_size.max() > 0.5f) {
                ...
                if (!params.cut_top && !params.cut_left) // top left
                {
                    ...
                    if (cx && cy) {
                        d = (-Dist(pos, corner) + corner_size.x) / resolution;
                        MIN(dist_global, d);
                        MIN(dist_depth, d);
                    }
                    if (cx) {
                        Flt y = (1 - CosSin(1 - pos.x / corner_size.x)) * corner_size.y;
                        d = (pos.y - y) / resolution;
                        MIN(dist_y, d);
                    }
                    if (cy) {
                        Flt x = (1 - CosSin(1 - pos.y / corner_size.y)) * corner_size.x;
                        d = (pos.x - x) / resolution;
                        MIN(dist_x, d);
                    }
                ...
                if (!params.cut_top && !params.cut_right) // top right
                {
                    ...
                    if (cx) {
                        Flt y = (1 - CosSin((corner.x - pos.x) / corner_size.x)) * corner_size.y;
                        d = (pos.y - y) / resolution;
                        MIN(dist_y, d);
                    }
                    if (cy) {
                        Flt x = corner.x + CosSin(1 - pos.y / corner_size.y) * corner_size.x;
                        d = (x - pos.x) / resolution;
                        MIN(dist_x, d);
                    }
                ...
                if (!params.cut_bottom && !params.cut_left) // bottom left
                {
                    ...
                    if (cx) {
                        Flt y = corner.y + CosSin(1 - pos.x / corner_size.x) * corner_size.y;
                        d = (y - pos.y) / resolution;
                        MIN(dist_y, d);
                    }
                    if (cy) {
                        Flt x = (1 - CosSin((corner.y - pos.y) / corner_size.y)) * corner_size.x;
                        d = (pos.x - x) / resolution;
                        MIN(dist_x, d);
                    }
                }
                if (!params.cut_bottom && !params.cut_right) // bottom right
                {
                    ...
                    if (cx) {
                        Flt y = corner.y + CosSin((corner.x - pos.x) / corner_size.x) * corner_size.y;
                        d = (y - pos.y) / resolution;
                        MIN(dist_y, d);
                    }
                    if (cy) {
                        Flt x = corner.x + CosSin((corner.y - pos.y) / corner_size.y) * corner_size.x;
                        d = (x - pos.x) / resolution;
                        MIN(dist_x, d);
                    }
                }
            ...
            if (dist_global >= 0) // 'dist_depth' is now 0 .. 0.5
            {
                        ...
                        if (i == 0) {
                            if (pos.y < corner_size.y && !params.cut_top) {
                                if ((x > image_size_2i.x) ? !params.cut_right : !params.cut_left) {
                                    Flt x = (1 - CosSin(1 - pos.y / corner_size.y)) * corner_size.x;
                                    x = image_size_2.x / (image_size_2.x - x);
                                    center *= Lerp(1.0f, x, sd.sqr.y); // make this adjustment smaller if the other axis is disabled
                                }
                            } else if (pos.y > image_size1.y - corner_size.y && !params.cut_bottom) {
                                if ((x > image_size_2i.x) ? !params.cut_right : !params.cut_left) {
                                    Flt x = (1 - CosSin(1 - (image_size1.y - pos.y) / corner_size.y)) * corner_size.x;
                                    x = image_size_2.x / (image_size_2.x - x);
                                    center *= Lerp(1.0f, x, sd.sqr.y); // make this adjustment smaller if the other axis is disabled
                                }
                            }
                            ...
                            if (pos.x < corner_size.x && !params.cut_left) {
                                if ((y > image_size_2i.y) ? !params.cut_bottom : !params.cut_top) {
                                    Flt y = (1 - CosSin(1 - pos.x / corner_size.x)) * corner_size.y;
                                    y = image_size_2.y / (image_size_2.y - y);
                                    center *= Lerp(1.0f, y, sd.sqr.x); // make this adjustment smaller if the other axis is disabled
                                }
                            } else if (pos.x > image_size1.x - corner_size.x && !params.cut_right) {
                                if ((y > image_size_2i.y) ? !params.cut_bottom : !params.cut_top) {
                                    Flt y = (1 - CosSin(1 - (image_size1.x - pos.x) / corner_size.x)) * corner_size.y;
                                    y = image_size_2.y / (image_size_2.y - y);
                                    center *= Lerp(1.0f, y, sd.sqr.x); // make this adjustment smaller if the other axis is disabled
                                }
                            }
                ...
                if (sp.overlay) {
                    ...
                    case PanelImageParams::ImageParams::MULTIPLY:
                        depth *= Lerp(1.0f, ovr.x, ovr.w);
                        break;
                    case PanelImageParams::ImageParams::SCALE:
                        depth *= ScaleFactor((ovr.x - 0.5f) * ovr.w);
                        break;
                    case PanelImageParams::ImageParams::ADD:
                        depth += ovr.x * ovr.w;
                        break;
                    case PanelImageParams::ImageParams::ADD_SIGNED:
                        depth += (ovr.x - 0.5f) * ovr.w;
                        break;
                    case PanelImageParams::ImageParams::BLEND:
                        depth = Blend(Vec4(depth, depth, depth, 1), ovr).x;
                        break;
                    }
                }
                ...
                if (sp.noise_map.is()) {
                    ...
                    case PanelImageParams::ImageParams::MULTIPLY:
                        depth *= Lerp(1.0f, noise, sp.noise_intensity);
                        break;
                    case PanelImageParams::ImageParams::SCALE:
                        depth *= ScaleFactor((noise - 0.5f) * sp.noise_intensity);
                        break;
                    case PanelImageParams::ImageParams::ADD:
                        depth += noise * sp.noise_intensity;
                        break;
                    case PanelImageParams::ImageParams::ADD_SIGNED:
                        depth += (noise - 0.5f) * sp.noise_intensity;
                        break;
                    case PanelImageParams::ImageParams::BLEND:
                        depth = Blend(Vec4(depth, depth, depth, 1), Vec4(noise, noise, noise, sp.noise_intensity)).x;
                        break;
                    }
                }
            } else
    ...
    void afterDepth() {
        if (depth_map_ptr)
            depth_map.copy(*depth_map_ptr);                   // store a copy for the user
    ...
    void setColorY(Int y) {
        ...
        REPD(sy, 3)
        REPD(sx, 3)
        image_frac_y[sx][sy] = y * image_frac_mul_add[sx][sy].z + image_frac_mul_add[sx][sy].w;
        REPA(sps) {
        ...
        REPD(x, image_size.x) {
            ...
            if (dist_global >= 0) {
                ...
                if (sp.overlay) {
                    Vec2 tex(x, y);
                    tex *= sp.overlay_uv_scale;
                    tex += sp.overlay_uv_offset;
                    if (sp.overlay_uv_warp) {
                        Vec2 n = nrm.xy;
                        n.setLength(1 - CosSin(n.length()));
                        tex += sp.overlay_uv_warp * n;
                    }
                    Vec4 ovr = sp.overlay->GET_COLOR(tex.x, tex.y, false);
                    switch (section.color_overlay_params.mode) {
                    case PanelImageParams::ImageParams::MULTIPLY:
                        col *= Lerp(Vec4(1), ovr, sp.overlay_intensity);
                        break;
                    case PanelImageParams::ImageParams::SCALE:
                        col.xyz *= ScaleFactor((ovr.xyz - 0.5f) * (ovr.w * sp.overlay_intensity));
                        break;
                    case PanelImageParams::ImageParams::ADD:
                        col.xyz += ovr.xyz * (ovr.w * sp.overlay_intensity);
                        break;
                    case PanelImageParams::ImageParams::ADD_SIGNED:
                        col.xyz += (ovr.xyz - 0.5f) * (ovr.w * sp.overlay_intensity);
                        break;
                    case PanelImageParams::ImageParams::BLEND:
                        ovr.w *= sp.overlay_intensity;
                        col = Blend(col, ovr);
                ...
                if (sp.noise_map.is()) {
                    Vec2 tex(x, y);
                    tex *= sp.noise_uv_scale;
                    if (sp.noise_uv_warp) {
                        Vec2 n = nrm.xy;
                        n.setLength(1 - CosSin(n.length()));
                        tex += sp.noise_uv_warp * n;
                    }
                    Flt noise = sp.noise_map.pixelFCubicFast(tex.x, tex.y, false);
                    switch (section.color_noise.mode) {
                    case PanelImageParams::ImageParams::MULTIPLY:
                        col.xyz *= Lerp(1.0f, noise, sp.noise_intensity);
                        break;
                    case PanelImageParams::ImageParams::SCALE:
                        col.xyz *= ScaleFactor((noise - 0.5f) * sp.noise_intensity);
                        break;
                    case PanelImageParams::ImageParams::ADD:
                        col.xyz += noise * sp.noise_intensity;
                        break;
                    case PanelImageParams::ImageParams::ADD_SIGNED:
                        col.xyz += (noise - 0.5f) * sp.noise_intensity;
                        break;
                    case PanelImageParams::ImageParams::BLEND:
                        col.xyz = Blend(col, Vec4(noise, noise, noise, sp.noise_intensity)).xyz;
                        break;
                    }
                }
                ...
                if (y < y1) {
                    if (x < top_x1) {
                        if (top_left_image)
                            col *= top_left_image->GET_COLOR(x * image_frac_mul_add[0][0].x + image_frac_mul_add[0][0].y, image_frac_y[0][0], true, true);
                    } else if (x < top_x2) {
                        if (top_image)
                            col *= top_image->GET_COLOR(x * image_frac_mul_add[1][0].x + image_frac_mul_add[1][0].y, image_frac_y[1][0], true, true);
                    } else {
                        if (top_right_image)
                            col *= top_right_image->GET_COLOR(x * image_frac_mul_add[2][0].x + image_frac_mul_add[2][0].y, image_frac_y[2][0], true, true);
                    }
                } else if (y < y2) {
                    if (x < x1) {
                        if (left_image)
                            col *= left_image->GET_COLOR(x * image_frac_mul_add[0][1].x + image_frac_mul_add[0][1].y, image_frac_y[0][1], true, true);
                    } else if (x < x2) {
                        if (center_image)
                            col *= center_image->GET_COLOR(x * image_frac_mul_add[1][1].x + image_frac_mul_add[1][1].y, image_frac_y[1][1], true, true);
                    } else {
                        if (right_image)
                            col *= right_image->GET_COLOR(x * image_frac_mul_add[2][1].x + image_frac_mul_add[2][1].y, image_frac_y[2][1], true, true);
                    }
                } else {
                    if (x < bottom_x1) {
                        if (bottom_left_image)
                            col *= bottom_left_image->GET_COLOR(x * image_frac_mul_add[0][2].x + image_frac_mul_add[0][2].y, image_frac_y[0][2], true, true);
                    } else if (x < bottom_x2) {
                        if (bottom_image)
                            col *= bottom_image->GET_COLOR(x * image_frac_mul_add[1][2].x + image_frac_mul_add[1][2].y, image_frac_y[1][2], true, true);
                    } else {
                        if (bottom_right_image)
                            col *= bottom_right_image->GET_COLOR(x * image_frac_mul_add[2][2].x + image_frac_mul_add[2][2].y, image_frac_y[2][2], true, true);
                    }
                }
                ...
                REPA(params.lights)
                if (params.lights[i].enabled) {
                    Flt dot = Dot(light_dir[i], nrm);
                    if (dot > 0)
                        light += params.lights[i].intensity * dot;
                    else
                        light -= params.lights[i].back * dot;
...
#define FACE_UP 5
                    ...
                    CHS(dir.x);
                    CHS(dir.z); // make FACE_FRONT to be visible by default
#if DEBUG
                    if (Kb.b(KB_R))
                        dir *= Matrix().setRotateY(Time.appTime());
#endif
                    dir /= Abs(dir).max();
                    Int face = 0;
                    switch (Abs(dir).maxI()) {
                    case 0:
                        face = ((dir.x < 0) ? FACE_LEFT : FACE_RIGHT);
                        break;
                    case 1:
                        face = ((dir.y < 0) ? FACE_DOWN : FACE_UP);
                        break;
                    case 2:
                        face = ((dir.z < 0) ? FACE_BACK : FACE_FRONT);
                        break;
                    }
                    Vec2 uv;
                    switch (face) {
                    ...
                    if (!sp.reflection_one)
                        uv.x += sp.reflection->h() * face;
                ...
                if (params.inner_glow_color.a && params.inner_glow_radius > 0 && dist_global >= 0 && dist_global < params.inner_glow_radius) // glow
                {
                    ...
                    if (x >= 0 && y >= 0)                                             // only in corners (this may process circular corners too)
                        if (Equal(dist_x, dist_global) || Equal(dist_y, dist_global)) // allow only square corners
                            MIN(d, Max(0, params.inner_glow_radius - Dist(x, y)));
                    Flt g = 1 - d / params.inner_glow_radius;
                    Vec4 glow = inner_glow_color;
                    glow.w *= g;
                    col = Blend(col, glow);
                }
                ...
                if (section.outer_border_color.a && dist >= 0 && dist < params.border_size)
                    col = Blend(col, sp.outer_border_color);
                else if (section.inner_border_color.a && dist >= params.border_size && dist < params.border_size * 2)
                    col = Blend(col, sp.inner_border_color);
                else if (section.prev_border_color.a && section_i && dist_sect >= sps[1].size - params.border_size)
                    col = Blend(col, sp.prev_border_color);
    ...
    void afterColor(FILTER_TYPE filter) {
...
#if DEBUG
        if (Kb.b(KB_N)) {
            normal_map.mulAdd(Vec4(0.5f, 0.5f, 0.5f, 1), Vec4(0.5f, 0.5f, 0.5f, 0));
            normal_map.mustCopy(image, -1, -1, -1, IMAGE_R8G8B8_SRGB);
        }
#endif
        ...
        if (glow_image.is())
            image.applyShadow(glow_image, params.outer_glow_color, (image.size() - glow_image.size()) / 2); // yes, glow needs to be applied first (before shadows), because 'applyShadow' works in a way that it doesn't apply pixels on opaque ones
        if (shadow_image.is())
            image.applyShadow(shadow_image, BLACK, (image.size() - shadow_image.size()) / 2);
        image_size = image.size();
        ...
        REPA(panel_image._force_uniform_stretch) {
            ...
            if (!params.force_uniform_stretch[i])
                panel_image._padd.c[i] *= max_scale;
        }
...
Bool PanelImage::getSideScale(C Rect &rect, Flt &scale) C {
    ...
    if (_min_size.x > w) {
        scale = w / _min_size.x;
        Flt side_size_y = _min_size.y * scale;
        if (side_size_y > h)
            scale *= h / side_size_y;
        return true;
    }
...
void PanelImage::innerPadding(C Rect &rect, Rect &padding) C {
    Flt scale;
    Bool scale_do = getSideScale(rect, scale);
...
void PanelImage::draw(C Color &color, C Color &color_add, C Rect &rect) C {
    if (image.mipMaps() > 1)
        VI.clampAniso(); // if have mip maps then use high quality sampler to workaround that when squeezing image in one dimension then smaller mip maps are used even the other dimension is stretched
    VI.color(color);
    VI.color1(color_add);
    VI.image(&image);
    ...
    if (Vtx2DTex *v = (Vtx2DTex *)VI.addVtx(vtxs)) {
        ...
        if (scale_do) {
            ...
            if (_padd_any) {
                Flt padd;
                padd = _padd.x * (_force_uniform_stretch[0] ? rect.w() : scale);
                l -= padd;
                r += padd;
                padd = _padd.y * (_force_uniform_stretch[1] ? rect.h() : scale);
                y0 += padd;
                y3 -= padd;
            }
        } else {
            ...
            if (_padd_any) {
                if (_force_uniform_stretch[0]) {
                    Flt padd = _padd.x * rect.w();
                    l -= padd;
                    r += padd;
                } else {
                    l -= _padd.x;
                ...
                if (_force_uniform_stretch[1]) {
                    Flt padd = _padd.y * rect.h();
                    y0 += padd;
                    y3 -= padd;
                } else {
                    y0 += _padd.y;
...
void PanelImage::drawVertical(C Color &color, C Color &color_add, C Rect &rect) C {
    ...
    if (Vtx2DTex *v = (Vtx2DTex *)VI.addVtx(vtxs)) {
        Flt scale;
        Bool scale_do = getSideScaleVertical(rect, scale);
        ...
        if (scale_do) {
            ...
            if (_padd_any) {
                Flt padd;
                padd = _padd.x * (_force_uniform_stretch[0] ? rect.h() : scale);
                b -= padd;
                t += padd;
                padd = _padd.y * (_force_uniform_stretch[1] ? rect.w() : scale);
                x0 -= padd;
                x3 += padd;
            }
        } else {
...
static void ClipR(Flt &min_pos, Flt &max_pos, Flt min_tex, Flt &max_tex, Flt clip_pos) {
    if (max_pos > clip_pos) // if need to clip
    {
        if (min_pos < clip_pos) // min is before clipping, size is >0, section is visible, adjust 'max_tex'. Here check for "<" instead of "<=" because calculations inside are more expensive than for "else" case.
        {
            ...
            min_pos = clip_pos; // clip fully by setting both positions to 'clip_pos' to set zero size, we can ignore adjusting 'max_tex'
        max_pos = clip_pos;
    }
}
static void ClipL(Flt &min_pos, Flt &max_pos, Flt &min_tex, Flt max_tex, Flt clip_pos) {
    if (min_pos < clip_pos) // if need to clip
    {
        if (max_pos > clip_pos) // max is after clipping, size is >0, section is visible, adjust 'min_tex'. Here check for ">" instead of ">=" because calculations inside are more expensive than for "else" case.
        {
            ...
            max_pos = clip_pos; // clip fully by setting both positions to 'clip_pos' to set zero size, we can ignore adjusting 'max_tex'
        min_pos = clip_pos;
    }
}
...
void PanelImage::drawFrac(C Color &color, C Color &color_add, C Rect &rect, Flt frac_x, Bool include_padding) C {
    ...
    if (Vtx2DTex *v = (Vtx2DTex *)VI.addVtx(vtxs)) {
        ...
        if (_same_x) {
            ...
            l = L;
            ClipR(l, x[1][0], 0, tex_x[1][0], pos_x);
            ...
            l = L;
            ClipR(l, x[2][0], 0, tex_x[2][0], pos_x);
...
void PanelImage::drawFrac2(C Color &color, C Color &color_add, C Rect &rect, Flt frac_min_x, Flt frac_max_x) C {
    if (frac_min_x >= 1 || frac_max_x <= 0 || frac_max_x <= frac_min_x)
        return;
    if (image.mipMaps() > 1)
        VI.clampAniso(); // if have mip maps then use high quality sampler to workaround that when squeezing image in one dimension then smaller mip maps are used even the other dimension is stretched
    VI.color(color);
    ...
    if (Vtx2DTex *v = (Vtx2DTex *)VI.addVtx(vtxs)) {
        ...
        if (_same_x) {
            ...
            l = L;
            r = R;
            ClipR(l, x[2][0], tex_l, tex_x[2][0], clip_max_x);
            ...
            l = L;
            r = R;
            ClipR(l, x[1][0], tex_l, tex_x[1][0], clip_max_x);
...
void PanelImage::drawVerticalFrac(C Color &color, C Color &color_add, C Rect &rect, Flt frac_y, Bool include_padding) C {
    ...
    if (Vtx2DTex *v = (Vtx2DTex *)VI.addVtx(vtxs)) {
        ...
        if (_same_x) {
            ...
            b = B;
            ClipR(b, y[1][0], 0, tex_x[1][0], pos_y);
            ...
            b = B;
            ClipR(b, y[2][0], 0, tex_x[2][0], pos_y);
...
void PanelImage::drawBaseLines(C Color &line_color, C Rect &rect) C {
    ...
    REP(2)
    D.lineY(line_color, r.lerpX(_tex_x[0][i]), lt.y, y1);
    REP(2)
    D.lineY(line_color, r.lerpX(_tex_x[1][i]), y1, y2);
    REP(2)
    D.lineY(line_color, r.lerpX(_tex_x[2][i]), y2, rb.y);
}
void PanelImage::drawScaledLines(C Color &line_color, C Rect &rect) C {
    ...
    REP(2)
    D.lineY(line_color, x[0][i], y0, y1);
    REP(2)
    D.lineY(line_color, x[1][i], y1, y2);
    REP(2)
    D.lineY(line_color, x[2][i], y2, y3);
}
/******************************************************************************/
void PanelImage::drawInnerPadding(C Color &line_color, C Rect &rect) C {
    Rect padding;
    innerPadding(rect, padding);
