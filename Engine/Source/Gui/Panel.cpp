...
void Panel::getRectBottom(C Rect &rect, Rect &bottom, Bool mirror) C {
    ...
    if (mirror && bottom_image == top_image)
        bottom.swapY(); // mirror vertically if it's the same as top
}
void Panel::getRectLeftRight(C Rect &rect, Rect &left, Rect &right, Bool mirror) C {
    ...
    if (mirror)
        left.set(l, yb, l - w, yt);
    else
        left.set(l - w, yb, l, yt);
}
...
void Panel::getRectBottomCorner(C Rect &rect, Rect &left, Rect &right, Bool mirror) C {
    ...
    if (mirror)
        ...
        left.set(l - w, yb, l, yt);
    if (mirror && bottom_corner_image == top_corner_image) {
        left.swapY();
        right.swapY();
    } // mirror vertically if it's the same as top
}
/******************************************************************************/
...
void Panel::extendedRect(C Rect &rect, Rect &extended) C {
    Rect r;
    if (panel_image)
        panel_image->extendedRect(rect, r);
    else
        r = rect;
    if (shadow_opacity)
        r |= (rect + Vec2(shadow_offset, -shadow_offset)).extend(shadow_radius);
    if (border_color.a && border_size > 0)
        r.extend(border_size);
    if (side_color.a) {
        if (top_image) {
            Rect top;
            getRectTop(rect, top, false);
            ClipX(T, top, rect);
            r |= top;
        }
        if (bottom_image) {
            Rect bottom;
            getRectBottom(rect, bottom, false);
            ClipX(T, bottom, rect);
            r |= bottom;
        }
        if (left_right_image) {
            Rect left, right;
            getRectLeftRight(rect, left, right, false);
            r |= left;
            r |= right;
        }
        if (top_corner_image) {
            Rect left, right;
            getRectTopCorner(rect, left, right, false);
            r |= left;
            r |= right;
        }
        if (bottom_corner_image) {
            Rect left, right;
            getRectBottomCorner(rect, left, right, false);
            r |= left;
            r |= right;
        }
    }
...
void Panel::defaultInnerPadding(Rect &padding) C {
    if (panel_image)
        padding = panel_image->defaultInnerPadding();
    else
        padding.zero();
...
void Panel::innerPadding(C Rect &rect, Rect &padding) C {
    if (panel_image)
        panel_image->innerPadding(rect, padding);
    else
        padding.zero();
    MAX(padding.max.y, bar_size);
    // border is not included
...
void Panel::outerMargin(C Rect &rect, Rect &margin) C {
    Rect ext;
    extendedRect(rect, ext);
    margin.set(rect.min.x - ext.min.x, rect.min.y - ext.min.y, ext.max.x - rect.max.x, ext.max.y - rect.max.y);
}
Vec2 Panel::defaultInnerPaddingSize() C {
    Rect padding;
    defaultInnerPadding(padding);
    return padding.min + padding.max;
}
/******************************************************************************/
void Panel::reset() {
...
Bool Panel::getSideScale(C Rect &rect, Flt &scale) C {
    if (panel_image)
        return panel_image->getSideScale(rect, scale);
    return false;
}
void Panel::scaleBorder(Flt scale) {
    ...
    bottom_size *= scale;
    bottom_offset *= scale;
    left_right_size *= scale;
    left_right_offset *= scale;
    top_corner_size *= scale;
    top_corner_offset *= scale;
    bottom_corner_size *= scale;
    bottom_corner_offset *= scale;
}
/******************************************************************************/
void Panel::drawShadow(Byte shadow, C Rect &rect) C {
    if (center_shadow)
        D.drawShadow(shadow, rect + Vec2(shadow_offset, -shadow_offset), shadow_radius);
    else
        D.drawShadowBorders(shadow, rect, shadow_radius);
}
void Panel::drawCenter(C Color &color, C Rect &rect) C {
    if (panel_image)
        panel_image->draw(color, TRANSPARENT, rect);
    else if (!center_image)
        rect.draw(color);
    else if (!center_stretch)
        center_image->drawTile(color, TRANSPARENT, rect, center_scale);
    else
...
void Panel::drawBar(C Color &color, C Rect &rect) C {
    if (bar_size > 0) {
        ...
        if (bar_image)
            bar_image->draw(color, TRANSPARENT, r);
        else
            r.draw(color, true);
    }
}
void Panel::drawBorder(C Color &color, C Rect &rect) C {
    Rect r;
    if (bar_size > 0 && border_size <= 0)
        r.set(rect.min.x, rect.min.y, rect.max.x, rect.max.y - bar_size); // if have bar and border is inner, move it under the bar
    else
        r = rect;
    if (border_image)
        border_image->drawBorder(color, TRANSPARENT, r, border_size);
    else if (border_size)
        r.drawBorder(color, border_size);
    else
        r.draw(color, false);
}
static void DrawTopBottom(C Panel &panel, C ImagePtr &image, C Color &color, C Rect &elm_rect, C Rect &panel_rect) {
    ...
    default: {
        ...
        {
            Flt opaque_r = panel_rect.max.x + side_fade_ext;
            if (elm_rect.max.x > opaque_r || elm_rect.min.x > opaque_r) // this is needed when using negative sizes which cause 'elm_rect' to be flipped
            {
    ...
    case Panel::SM_STRETCH:
        VI.clampAniso();
        image->draw(color, TRANSPARENT, elm_rect);
        break;
    case Panel::SM_WRAP:
        VI.wrapX();
        image->drawPart(color, TRANSPARENT, elm_rect, Rect(0, 0, image->invAspect() * elm_rect.aspect(), 1));
        break;
    }
}
static void DrawLeftRight(C Panel &panel, C ImagePtr &image, C Color &color, C Rect &elm_rect /*, C Rect &panel_rect*/) {
    ...
    case Panel::SM_WRAP:
        VI.wrapY();
        image->drawPart(color, TRANSPARENT, elm_rect, Rect(0, 0, 1, image->aspect() * elm_rect.invAspect()));
        break;
    }
}
void Panel::drawSide(C Color &color, C Rect &rect) C {
    if (top_image) {
        Rect top;
        getRectTop(rect, top);
        DrawTopBottom(T, top_image, color, top, rect);
    }
    if (bottom_image) {
        Rect bottom;
        getRectBottom(rect, bottom);
        DrawTopBottom(T, bottom_image, color, bottom, rect);
    }
    if (left_right_image) {
        Rect left, right;
        getRectLeftRight(rect, left, right);
        DrawLeftRight(T, left_right_image, color, left);
        DrawLeftRight(T, left_right_image, color, right);
    }
    if (top_corner_image) {
        Rect left, right;
        getRectTopCorner(rect, left, right);
        top_corner_image->draw(color, TRANSPARENT, left);
        top_corner_image->draw(color, TRANSPARENT, right);
    }
    if (bottom_corner_image) {
        Rect left, right;
        getRectBottomCorner(rect, left, right);
        bottom_corner_image->draw(color, TRANSPARENT, left);
...
void Panel::draw(C Rect &rect) C {
    ...
    if (blur_color.any() // fast check
        ...
        ImageRTPtrRef rt0(hi ? Renderer._h0 : Renderer._q0);
        rt0.get(rt_desc);
        ImageRTPtrRef rt1(hi ? Renderer._h1 : Renderer._q1);
        rt1.get(rt_desc);
        ImageRT *cur = Renderer._cur[0], *ds = Renderer._cur_ds;
        Rect r = rect;
        r.extend(D.pixelToScreenSize(SHADER_BLUR_RANGE << shift));
        ...
        if (secondary)
            Renderer._ui->copyHw(*cur, false, rect); // set background to be a copy

        // Sh.imgSize(*rt0); we can just use 'RTSize' instead of 'ImgSize' since there's no scale
        ALPHA_MODE alpha = D.alpha(ALPHA_NONE);
        Renderer.set(rt1, null, false);
        Sh.BlurX[true]->draw(rt0, &r);
        Renderer.set(rt0, null, false);
        Sh.BlurY[true]->draw(rt1, &r);
        Renderer.set(cur, ds, true);
        if (shadow_opacity) {
            D.alpha(ALPHA_BLEND);
            if (shadow_opacity) {
                if (center_shadow)
                    D.drawShadow(shadow_opacity, rect + Vec2(shadow_offset, -shadow_offset), shadow_radius);
                else
                    D.drawShadowBorders(shadow_opacity, rect, shadow_radius);
            }
        }
        if (secondary) // for secondary we need to force rt.alpha to 1.0
        {
            D.alphaFactor(Color(center_color.a, center_color.a, center_color.a, 255));
            MaterialClear(); // 'MaterialClear' must be called when changing 'D.alphaFactor'
            D.alpha(ALPHA_FACTOR);
            Sh.Color[0]->set(Color(blur_color.r, blur_color.g, blur_color.b, 0));
            Sh.Color[1]->set(Color(center_color.r, center_color.g, center_color.b, 255));
            Sh.DrawC->draw(rt0, &rect);
        } else {
            D.alpha(ALPHA_BLEND);
            Sh.Color[0]->set(Color(blur_color.r, blur_color.g, blur_color.b, 0));
            Sh.Color[1]->set(center_color);
            Sh.DrawC->draw(rt0, &rect);
        }
        D.alpha(alpha);
    } else
    ...
    {
        if (shadow_opacity)
            drawShadow(shadow_opacity, rect);
        if (center_color.a)
            drawCenter(center_color, rect);
    }

    if (bar_color.a)
        drawBar(bar_color, rect);
    if (border_color.a)
        drawBorder(border_color, rect);
    if (side_color.a)
        drawSide(side_color, rect);
}
...
void Panel::drawLines(C Color &line_color, C Rect &rect) C {
    ...
    if (top_image) {
        Rect top;
        getRectTop(rect, top, false);
        top.draw(line_color, false);
    }
    if (bottom_image) {
        Rect bottom;
        getRectBottom(rect, bottom, false);
        bottom.draw(line_color, false);
    }
    if (left_right_image) {
        Rect left, right;
        getRectLeftRight(rect, left, right, false);
        left.draw(line_color, false);
        right.draw(line_color, false);
    }
    if (top_corner_image) {
        Rect left, right;
        getRectTopCorner(rect, left, right, false);
    ...
    if (f.getUInt() == CC4_GSTL)
        ...
        case 6: {
            ...
            if (f.getFast(desc)) {
                ...
                Unaligned(bottom_offset, desc.bottom_offset);
                bottom_offset += bottom_size / 2;
                Unaligned(left_right_offset, desc.left_right_offset);
                left_right_offset.x -= left_right_size / 2;
                Unaligned(top_corner_offset, desc.top_corner_offset);
                top_corner_offset -= top_corner_size / 2;
                Unaligned(bottom_corner_offset, desc.bottom_corner_offset);
                bottom_corner_offset.x -= bottom_corner_size / 2;
                bottom_corner_offset.y += bottom_corner_size / 2;
                ...
                bar_image.clear();
                side_min_scale = 1;
                if (desc.side_stretch && top_image) {
...
#pragma pack(pop)
        ...
        case 3: {
            ...
            if (f.getFast(desc)) {
                ...
                panel_image.clear();
                bottom_image.clear();
                left_right_image.clear();
                bottom_size = left_right_size = DEFAULT_SIZE;
                left_right_offset = bottom_offset = 0;
                center_shadow = !Equal(shadow_offset, 0);
                side_mode = SM_DEFAULT;
                bar_color = TRANSPARENT;
                bar_size = 0;
        ...
        case 2: {
            ...
            if (f.getFast(desc)) {
                ...
                Unaligned(shadow_opacity, desc.shadow_opacity);
                Unaligned(center_color, desc.center_color);
                Swap(center_color.r, center_color.b);
                Unaligned(border_color, desc.border_color);
                Swap(border_color.r, border_color.b);
                Unaligned(blur_color, desc.blur_color);
                Swap(blur_color.r, blur_color.b);
                Unaligned(shadow_offset, desc.shadow_offset);
                Unaligned(shadow_radius, desc.shadow_radius);
                Unaligned(border_size, desc.border_size);
                CHS(border_size);
...
void Panel::operator=(C Str &name) {
    if (!load(name))
        Exit(MLT(S + "Can't load Panel \"" + name + "\"",
