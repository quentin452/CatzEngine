...
static struct ovrTexture {
...
#endif
        if (OculusRift._session)
            ovr_DestroySwapTextureSet(OculusRift._session, texture_set);
        texture_set = null;
    }
    Bool create(Int w, Int h) {
        ...
        if (ovr_CreateSwapTextureSetD3D11(OculusRift._session, D3D, &desc, ovrSwapTextureSetD3D11_Typeless, &texture_set) == ovrSuccess) {
            if (texture_set->TextureCount > Elms(rtv))
                Exit("Oculus Rift Swap Chain has too many textures");
            FREP(texture_set->TextureCount) {
                ...
                if (!OK(D3D->CreateRenderTargetView(tex.D3D11.pTexture, &rtvd, &rtv[i])))
                    goto error;
                ...
                if (!OK(D3D->CreateShaderResourceView(tex.D3D11.pTexture, &srvd, &srv[i])))
                    goto error;
            }
            return true;
        }
    ...
    ImageRTC *getImage() {
        if (texture_set) {
            if (++texture_set->CurrentIndex >= texture_set->TextureCount)
                texture_set->CurrentIndex = 0;
...
static void SetPose(Matrix &m, C ovrPosef &pose) {
    ASSERT(SIZE(pose.Orientation) == SIZE(Quaternion));
    Quaternion q = (Quaternion &)pose.Orientation;
    CHS(q.z);
    CHS(q.w);
    m.orn() = q;
    ASSERT(SIZE(pose.Position) == SIZE(Vec));
    m.pos.set(pose.Position.x, pose.Position.y, -pose.Position.z); // Oculus is right-handed
}
static void SetPose(ovrPosef &pose, C Matrix &m) {
    ASSERT(SIZE(pose.Orientation) == SIZE(Quaternion));
    (Quaternion &)pose.Orientation = Quaternion(m);
    CHS(pose.Orientation.z);
    CHS(pose.Orientation.w);
    ASSERT(SIZE(pose.Position) == SIZE(Vec));
    ((Vec &)pose.Position).set(m.pos.x, m.pos.y, -m.pos.z); // Oculus is right-handed
}
...
Bool OculusRiftApi::connect() {
    ...
    if (ovr_Create(&_session, (ovrGraphicsLuid *)&VR._adapter_id) == ovrSuccess) {
        ...
        REPAO(eye_render_desc) = ovr_GetRenderDesc(_session, ovrEyeType(i), desc.DefaultEyeFov[i]);
        ...
        REP(2)
        _layer_render.Fov[i] = desc.DefaultEyeFov[i]; // eyes
...
OculusRiftApi::OculusRiftApi() {
    ...
    Matrix m;
    m.identity();
    SetPose(_layer_gui.QuadPoseCenter, m);
    ...
    if (_initialized) {
        if (_session) {
            ...
            if (ts.StatusFlags & ovrStatus_HmdConnected) {
                C ovrVector3f &a = ts.RawSensorData.Accelerometer;
                AccelerometerValue.set(-a.x, -a.y, a.z);
                C ovrVector3f &g = ts.RawSensorData.Gyro;
                GyroscopeValue.set(g.x, g.y, -g.z);
                C ovrVector3f &m = ts.RawSensorData.Magnetometer;
                MagnetometerValue.set(m.x, m.y, -m.z);
                if (ts.HandStatusFlags[0] & (ovrStatus_OrientationTracked | ovrStatus_PositionTracked))
                    SetPose(VR._left, ts.HandPoses[0].ThePose);
                if (ts.HandStatusFlags[1] & (ovrStatus_OrientationTracked | ovrStatus_PositionTracked))
                    SetPose(VR._right, ts.HandPoses[1].ThePose);
                if (ts.StatusFlags & (ovrStatus_OrientationTracked | ovrStatus_PositionTracked)) {
...
#else
                    Vec eye_offset = VR._matrix.x * D.eyeDistance();
                    CHS(eye_offset.z); // need to change z because Oculus is right-handed
                    ASSERT(SIZE(ts.HeadPose.ThePose.Position) == SIZE(Vec));
                    (Vec &)ts.HeadPose.ThePose.Position -= eye_offset * 0.5f;
                    _layer_render.RenderPose[0] = ts.HeadPose.ThePose;
                    (Vec &)ts.HeadPose.ThePose.Position += eye_offset;
                    _layer_render.RenderPose[1] = ts.HeadPose.ThePose;
#endif
                }
            } else
                disconnect();               // lost connection
        } else if (VR.OculusRiftDetected()) // try to connect
        {
            connect();
        }
    }
...
void OculusRiftApi::draw() {
    ...
    if (active()) {
      ...
      FlagSet(_layer_gui   .Header.Flags, ovrLayerFlag_HighQuality, Ms.b(1));
        ...
        ovrLayerHeader *layer[2];
        Int layers = 0;
        if (VR._render)
            layer[layers++] = &_layer_render.Header;
        if (VR._ui && VR.draw_2d)
            layer[layers++] = &_layer_gui.Header;
