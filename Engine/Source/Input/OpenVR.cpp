...
Bool OpenVRApi::connect() {
    ...
    _vr->GetDXGIOutputInfo(&adapter_index);
    if (adapter_index >= 0) // if want a custom adapter
    {
#if DX11
    ...
    VR._name[0] = '\0';
    _vr->GetStringTrackedDeviceProperty(vr::k_unTrackedDeviceIndex_Hmd, vr::Prop_ModelNumber_String, VR._name, Elms(VR._name));
    ...
    Flt l, r, t, b;
    _vr->GetProjectionRaw(vr::Eye_Left, &l, &r, &t, &b);
...
Bool OpenVRApi::init() {
    ...
    if (!_vr) {
        ...
        if (_vr = vr::VR_Init(&error, vr::VRApplication_Scene)) {
            if (_compositor = vr::VRCompositor()) {
                ...
                if (_overlay = vr::VROverlay()) {
                    _overlay->CreateOverlay(ENGINE_NAME, "UI", &_overlay_id);
                    if (_overlay_id != vr::k_ulOverlayHandleInvalid) {
                        _overlay->SetHighQualityOverlay(_overlay_id);
                        _overlay->HideOverlay(_overlay_id);
                        _overlay_visible = false;
...
void OpenVRApi::shut() {
    ...
    if (_vr) {
        ...
        if (_overlay) {
            if (_overlay_id != vr::k_ulOverlayHandleInvalid)
                _overlay->DestroyOverlay(_overlay_id);
            _overlay = null;
        }
        _overlay_visible = false;
        _overlay_id = vr::k_ulOverlayHandleInvalid;
...
Matrix OpenVRApi::matrixCur() C {
    ...
    if (_vr) {
        ...
        _vr->GetDeviceToAbsoluteTrackingPose(vr::TrackingUniverseSeated, 0, &pose, 1);
        if (pose.bDeviceIsConnected && pose.bPoseIsValid) {
            Matrix m;
            SetPose(m, pose.mDeviceToAbsoluteTracking);
            return m;
        }
    }
#endif
...
void OpenVRApi::update() {
    ...
    if (_vr) {
        vr::VREvent_t event;
        while (_vr->PollNextEvent(&event, SIZE(event))) {
        }
        vr::TrackedDevicePose_t pose;
...
void OpenVRApi::draw() {
    ...
    if (active()) {
        if (!VR._render)
            _render.clearFull(Vec4Zero, true); // if render was not set, then clear
        ...
        if (_overlay && _overlay_id != vr::k_ulOverlayHandleInvalid) {
            ...
            if (_overlay_visible != vis) {
                if (_overlay_visible = vis)
                    _overlay->ShowOverlay(_overlay_id);
                else
                    _overlay->HideOverlay(_overlay_id);
            }
        ...
        rect.uMin = 0.0f;
        rect.uMax = 0.5f;
        _compositor->Submit(vr::Eye_Left, &t, &rect);
        rect.uMin = 0.5f;
        rect.uMax = 1.0f;
        _compositor->Submit(vr::Eye_Right, &t, &rect);
...
#if SUPPORT_OPEN_VR
    if (_overlay && _overlay_id != vr::k_ulOverlayHandleInvalid)
        _overlay->ClearOverlayTexture(_overlay_id);
    _render.del();
    _ui.del();
#endif
}
...
Bool OpenVRApi::createRenderImage() {
    ...
    if (_vr) {
        uint32_t w = 0, h = 0;
        _vr->GetRecommendedRenderTargetSize(&w, &h);
