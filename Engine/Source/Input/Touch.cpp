...
struct ReleasedTouch {
    ...
    void set(Dbl app_time, C Vec2 &pos) {
        T.app_time = app_time;
        T.pos = pos;
    }
};
...
Touch::Touch() {
    ...
    _start_pos = _prev_pos = _pos = _sm_pos = _delta = _abs_delta = _vel = 0;
    _pixeli = _delta_pixeli_clp = 0;
    _handle = null;
    _gui_obj = null;
}
Touch &Touch::init(C VecI2 &pixeli, C Vec2 &pos, CPtr handle, Bool stylus) {
    if (TouchesID == 0)
        TouchesID = 1;
    _id = TouchesID++; // don't select zero for the ID
    _start_pos = _prev_pos = _pos = _sm_pos = pos;
    _sv_pos.init(pos);
    _pixeli = pixeli;
    _handle = handle;
...
/******************************************************************************/
void MouseTouch::guiObj(Int i, GuiObj *obj) {
    if (InRange(i, Touches))
        Touches[i].guiObj(obj);
    else
        Gui.ms(obj);
}
/******************************************************************************/
...
void TouchesUpdate() {
    ...
    if (TouchesSimulated) {
        ...
        if (Ms.bp(0)) // simulate 'touchesBegan'
        {
            ...
            if (Ms.bd(0))
                t->_state |= BS_DOUBLE;
        } else if (Ms.b(0)) // simulate 'touchesMoved'
        {
            Touch *t = FindTouchByHandle(handle); // find existing one
            if (!t)                               // create new one
            {
        ...
        if (Ms.br(0) || !Ms.b(0))
            if (Touch *t = FindTouchByHandle(handle)) // simulate 'touchesReleased'
            {
                t->_pixeli = pixeli;
                t->_pos = pos;
                t->_delta_pixeli_clp = Ms.pixelDelta();
    ...
    REPA(Touches) {
        ...
        if (!(t.state() & (BS_PUSHED | BS_RELEASED)) // don't update on push/release because on those events position may not be updated
            || t.ad().any())
            if (Time.ad())
                t._vel = t._sv_vel.update(t._abs_delta / Time.ad(), Time.ad());
        ...
        if (t.on()) {
            ...
            {
                if (!t.selecting()) {
                    if (old_life < LongPressTime && t.life() >= LongPressTime)
                        t._state |= BS_LONG_PRESS;
                    ...
                    if (t.stylus() ? (dist2 >= TouchSelectDist2 && old_life >= StylusSelectTime) || dist2 >= TouchSelectBigDist2 // stylus can be slippery, because of that process it differently (for short distance require time, or allow big distance in case user made long swipe)
                                   : dist2 >= TouchSelectDist2)
                        t._selecting = true;
                }
                if (/*!t.dragging() && already checked above*/ t.selecting() && old_life >= DragTime)
                    t._dragging = true;
            }
            ...
            t._selecting = t._dragging = t._scrolling = false; // !on && !rs

        if (t._scrolling) // process for 'on' and 'rs'
            if (Region *region = t.guiObj()->firstScrollableRegion()) {
                if (t._axis_moved & 1)
                    region->scrollX(-t.d().x, true);
                if (t._axis_moved & 2)
                    region->scrollY(t.d().y, true);
                if (t.selecting())
                    switch (t.guiObj()->type()) // if enough movement occurred, then switch focus to the region itself, in case it belonged to a child of the region that is clickable, to avoid clicking that object on scroll end, however allow other objects in case we want to drag them
                    {
                    case GO_BUTTON:
                    case GO_CHECKBOX:
                    case GO_COMBOBOX:
        ...
        if (t.rs()) {
            if (!t.selecting() && old_life <= TapTime)
                t._state |= BS_TAPPED;
            ...
            if (t._axis_moved && t.id() != Gui._drag_touch_id) // only if not dragging something
                if (Region *region = t.guiObj()->firstScrollableRegion())
                    if (t.guiObj() != &region->view) {
                        ...
                        if (region->slidebar[0].contains(t.guiObj()))
                            vel.set(-vel.x, 0);
                        else if (region->slidebar[1].contains(t.guiObj()))
                            vel.set(0, -vel.y);
                        if (vel.length2() >= Sqr(0.15f))
                            region->scrollX(vel.x).scrollY(vel.y);
                    }
        }

#if DETECT_DOUBLE_TAP
        if (t.pd())
            REPA(ReleasedTouches) // if pushed then check for possibility of double click
            {
                ReleasedTouch &rt = ReleasedTouches[i];
                if (Time.appTime() - rt.app_time > TouchDoubleClickTime + Time.ad())
...
#if DETECT_DOUBLE_TAP
        if (t.rs() && t._first)
            ReleasedTouches.New().set(t.startTime(), t.startPos()); // detect based on releases (and not '_remove' because hovers may not be removed), only for first clicks
#endif
        if (t._remove) {
