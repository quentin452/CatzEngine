...
Str CalcValue::asText(Int precision) C {
    ...
    case CVAL_INT:
        ...
        break;
    case CVAL_REAL:
        return TextReal(r, (precision == INT_MAX) ? PRECISION_DBL : precision);
    case CVAL_VEC2:
        return v2.asText(precision);
    case CVAL_VEC:
        return v.asText(precision);
    case CVAL_VEC4:
        return v4.asText(precision);
    default:
        return S;
    }
}
...
VecI4 CalcValue::asVecI4() C {
    ...
    case CVAL_REAL:
        return Round(r);
    case CVAL_VEC2:
        return VecI4(Round(v2), 0, 0);
    case CVAL_VEC:
        return VecI4(Round(v), 0);
    case CVAL_VEC4:
        return Round(v4);
    default:
        return 0;
...
Bool CalcValue::sign() {
    ...
    case CVAL_REAL:
        ...
        return true;
    case CVAL_VEC2:
        v2.set(Sign(v2.x), Sign(v2.y));
        return true;
    case CVAL_VEC:
        v.set(Sign(v.x), Sign(v.y), Sign(v.z));
        return true;
    case CVAL_VEC4:
        v4.set(Sign(v4.x), Sign(v4.y), Sign(v4.z), Sign(v4.w));
        return true;
    default:
        return false;
    }
...
Bool CalcValue::sqrt() {
    ...
    case CVAL_REAL:
        ...
        return true;
    case CVAL_VEC2:
        v2.set(Sqrt(v2.x), Sqrt(v2.y));
        return true;
    case CVAL_VEC:
        v.set(Sqrt(v.x), Sqrt(v.y), Sqrt(v.z));
        return true;
    case CVAL_VEC4:
        v4.set(Sqrt(v4.x), Sqrt(v4.y), Sqrt(v4.z), Sqrt(v4.w));
        return true;
    default:
        return false;
    }
...
Bool CalcValue::cbrt() {
    ...
    case CVAL_REAL:
        ...
        return true;
    case CVAL_VEC2:
        v2.set(Cbrt(v2.x), Cbrt(v2.y));
        return true;
    case CVAL_VEC:
        v.set(Cbrt(v.x), Cbrt(v.y), Cbrt(v.z));
        return true;
    case CVAL_VEC4:
        v4.set(Cbrt(v4.x), Cbrt(v4.y), Cbrt(v4.z), Cbrt(v4.w));
        return true;
    default:
        return false;
    }
...
/******************************************************************************/
Bool CalcValue::And(C CalcValue &x) {
    if (type == CVAL_INT && x.type == CVAL_INT) {
        i &= x.i;
        return true;
    }
    return false;
}
Bool CalcValue::Or(C CalcValue &x) {
    if (type == CVAL_INT && x.type == CVAL_INT) {
        i |= x.i;
        return true;
    }
...
Bool CalcValue::Xor(C CalcValue &x) {
    if (type == CVAL_INT && x.type == CVAL_INT) {
        i ^= x.i;
        return true;
    }
...
Bool CalcValue::shl(C CalcValue &x) {
    if (type == CVAL_INT && x.type == CVAL_INT) {
        i <<= x.i;
        return true;
    }
...
Bool CalcValue::shr(C CalcValue &x) {
    if (type == CVAL_INT && x.type == CVAL_INT) {
        i >>= x.i;
        return true;
    }
...
Bool CalcValue::rol(C CalcValue &x) {
    if (type == CVAL_INT && x.type == CVAL_INT) {
        i = Rol(i, x.i);
        return true;
    }
...
Bool CalcValue::ror(C CalcValue &x) {
    if (type == CVAL_INT && x.type == CVAL_INT) {
        i = Ror(i, x.i);
        return true;
    }
...
Bool CalcValue::alignTrunc(C CalcValue &x) {
    ...
    case CVAL_INT:
        switch (x.type) {
        case CVAL_INT:
            r = AlignTrunc(Dbl(i), Dbl(x.i));
            type = CVAL_REAL;
            return true;
        case CVAL_REAL:
            r = AlignTrunc(Dbl(i), x.r);
            type = CVAL_REAL;
            return true;
        }
        break;
...
Bool CalcValue::alignRound(C CalcValue &x) {
    ...
    case CVAL_INT:
        switch (x.type) {
        case CVAL_INT:
            r = AlignRound(Dbl(i), Dbl(x.i));
            type = CVAL_REAL;
            return true;
        case CVAL_REAL:
            r = AlignRound(Dbl(i), x.r);
            type = CVAL_REAL;
            return true;
        }
        break;
...
Bool CalcValue::alignFloor(C CalcValue &x) {
    ...
    case CVAL_INT:
        switch (x.type) {
        case CVAL_INT:
            r = AlignFloor(Dbl(i), Dbl(x.i));
            type = CVAL_REAL;
            return true;
        case CVAL_REAL:
            r = AlignFloor(Dbl(i), x.r);
            type = CVAL_REAL;
            return true;
        }
        break;
...
Bool CalcValue::alignCeil(C CalcValue &x) {
    ...
    case CVAL_INT:
        switch (x.type) {
        case CVAL_INT:
            r = AlignCeil(Dbl(i), Dbl(x.i));
            type = CVAL_REAL;
            return true;
        case CVAL_REAL:
            r = AlignCeil(Dbl(i), x.r);
            type = CVAL_REAL;
            return true;
        }
        break;
...
Bool CalcValue::cross(C CalcValue &x) {
    if (type == CVAL_VEC && x.type == CVAL_VEC) {
        v = Cross(v, x.v);
        return true;
    }
    return false;
...
static Bool CAngle(CalcValue &x, CalcValue &y) {
    ...
    if (x.type == CVAL_INT)
        xr = x.i;
    else if (x.type == CVAL_REAL)
        xr = x.r;
    else
        return false;
    if (y.type == CVAL_INT)
        yr = y.i;
    else if (y.type == CVAL_REAL)
        yr = y.r;
    else
        return false;
...
static Bool CSmoothCubeInv(CalcValue &x) {
    if (x.type == CVAL_INT)
        x.r = SmoothCubeInv(Dbl(x.i));
    else if (x.type == CVAL_REAL)
        x.r = SmoothCubeInv(x.r);
    else
        return false;
    x.type = CVAL_REAL;
    return true;
...
static Bool CSmoothSextic(CalcValue &x) {
    if (x.type == CVAL_INT)
        x.r = SmoothSextic(Dbl(x.i));
    else if (x.type == CVAL_REAL)
        x.r = SmoothSextic(x.r);
    else
        return false;
    x.type = CVAL_REAL;
    return true;
...
static Bool CLerp(CalcValue &x, CalcValue &y, CalcValue &z) {
    ...
    if (x.type == CVAL_VEC2 && y.type == CVAL_VEC2) {
        x.v2 = Lerp(x.v2, y.v2, s);
        return true;
    }
    if (x.type == CVAL_VEC && y.type == CVAL_VEC) {
        x.v = Lerp(x.v, y.v, s);
        return true;
    }
    if (x.type == CVAL_VEC4 && y.type == CVAL_VEC4) {
        x.v4 = Lerp(x.v4, y.v4, s);
        return true;
    }
...
static Bool CScaleFactorR(CalcValue &x) {
    if (x.type == CVAL_INT)
        x.r = ScaleFactorR(Dbl(x.i));
    else if (x.type == CVAL_REAL)
        x.r = ScaleFactorR(x.r);
    else
        return false;
    x.type = CVAL_REAL;
    return true;
...
static Bool CLinearToSRGB(CalcValue &x) {
    if (x.type == CVAL_INT) {
        x.r = LinearToSRGB(Flt(x.i));
        x.type = CVAL_REAL;
    } else if (x.type == CVAL_REAL)
        x.r = LinearToSRGB(x.r);
    else if (x.type == CVAL_VEC)
        x.v = LinearToSRGB(Vec(x.v));
    else if (x.type == CVAL_VEC4)
        x.v4 = LinearToSRGB(Vec4(x.v4));
    else
        return false;
    return true;
}
static Bool CSRGBToLinear(CalcValue &x) {
    if (x.type == CVAL_INT) {
        x.r = SRGBToLinear(Flt(x.i));
        x.type = CVAL_REAL;
    } else if (x.type == CVAL_REAL)
        x.r = SRGBToLinear(x.r);
    else if (x.type == CVAL_VEC)
        x.v = SRGBToLinear(Vec(x.v));
    else if (x.type == CVAL_VEC4)
        x.v4 = SRGBToLinear(Vec4(x.v4));
    else
        return false;
    return true;
}
...
static Bool CSigmoidDivInv(CalcValue &x) {
    if (x.type == CVAL_INT) {
        x.r = SigmoidDivInv(Dbl(x.i));
        x.type = CVAL_REAL;
    } else if (x.type == CVAL_REAL)
        x.r = SigmoidDivInv(x.r);
    else
...
static Bool CSigmoidSqrInv(CalcValue &x) {
    if (x.type == CVAL_INT) {
        x.r = SigmoidSqrInv(Dbl(x.i));
        x.type = CVAL_REAL;
    } else if (x.type == CVAL_REAL)
        x.r = SigmoidSqrInv(x.r);
    else
...
static Bool CSigmoidExpInv(CalcValue &x) {
    if (x.type == CVAL_INT) {
        x.r = SigmoidExpInv(Dbl(x.i));
        x.type = CVAL_REAL;
    } else if (x.type == CVAL_REAL)
        x.r = SigmoidExpInv(x.r);
    else
...
static Bool CSigmoidExpAInv(CalcValue &x) {
    if (x.type == CVAL_INT) {
        x.r = SigmoidExpAInv(Dbl(x.i));
        x.type = CVAL_REAL;
    } else if (x.type == CVAL_REAL)
        x.r = SigmoidExpAInv(x.r);
    else
...
static Bool CSigmoidAtanInv(CalcValue &x) {
    if (x.type == CVAL_INT) {
        x.r = SigmoidAtanInv(Dbl(x.i));
        x.type = CVAL_REAL;
    } else if (x.type == CVAL_REAL)
        x.r = SigmoidAtanInv(x.r);
    else
...
static Bool CSigmoidTanhInv(CalcValue &x) {
    if (x.type == CVAL_INT) {
        x.r = SigmoidTanhInv(Dbl(x.i));
        x.type = CVAL_REAL;
    } else if (x.type == CVAL_REAL)
        x.r = SigmoidTanhInv(x.r);
    else
...
static Bool ReadChar(Meml<CalcElm> &elms, CChar *&text, Str *error, C CMemPtr<CalcVar> &vars) {
    ...
    for (Int i = 0;;) {
        Char c = *text;
        if (!(CharFlag(c) & (CHARF_ALPHA | CHARF_DIG | CHARF_UNDER)) && c != '.') {
            name[i] = '\0';
            break;
        }
        name[i++] = c;
    ...
    REPA(CalcFuncInfos)
    if (Equal(name, CalcFuncInfos[i].t)) {
        CalcElm &elm = elms.New();
        elm.type = CET_FUNC;
    ...
    if (error)
        *error = S + "Unrecognized token '" + name + '\'';
    return false;
}
/******************************************************************************/
...
static Bool ReadOp(Meml<CalcElm> &elms, CChar *&text, Str *error) {
    ...
    case '%':
        ...
        break;
    case '~':
        AddOp(elms, COT_BITWISE_NOT);
        break;
    case ',':
        AddOp(elms, COT_COMMA);
        break;
    case '(':
        AddOp(elms, COT_LBR);
        break;
    ...
    default:
        if (error)
            *error = S + "Unrecognized token '" + text[0] + '\'';
        return false;
    }
    text++;
    return true;
...
static Bool CFuncReady(Meml<CalcElm> &elms, MemlNode *node, Int args) {
    CalcElm *c;
    if (args < 1 || args > 4 || !node)
        return false;

    if (!(c = Celm(node = node->next())))
        return false;
    if (c->type != CET_OP || c->o.type != COT_LBR)
        return false; // (
    if (!(c = Celm(node = node->next())))
        return false;
    if (c->type != CET_VAL)
    ...
    for (; --args;) {
        if (!(c = Celm(node = node->next())))
            return false;
        if (c->type != CET_OP || c->o.type != COT_COMMA)
            return false; // ,
        if (!(c = Celm(node = node->next())))
            return false;
        if (c->type != CET_VAL)
    ...
    if (!(c = Celm(node = node->next())))
        return false;
    if (c->type != CET_OP || c->o.type != COT_RBR)
        return false; // )

    return true;
}
/******************************************************************************/
static Bool Error(Str *error, C Str &name) {
    if (error)
        *error = name;
    return false;
}
static Bool Error(Str *error, C Str &name, C CalcValue &arg) {
    if (error)
        *error = S + name + '(' + arg.typeName() + ')';
    return false;
}
static Bool Error(Str *error, C Str &name, C CalcValue &a0, C CalcValue &a1) {
    if (error)
        *error = S + name + '(' + a0.typeName() + ", " + a1.typeName() + ')';
    return false;
}
static Bool Error(Str *error, C Str &name, C CalcValue &a0, C CalcValue &a1, C CalcValue &a2) {
    if (error)
        *error = S + name + '(' + a0.typeName() + ", " + a1.typeName() + ", " + a2.typeName() + ')';
    return false;
}
static Bool Error(Str *error, C Str &name, C CalcValue &a0, C CalcValue &a1, C CalcValue &a2, C CalcValue &a3) {
    if (error)
        *error = S + name + '(' + a0.typeName() + ", " + a1.typeName() + ", " + a2.typeName() + ", " + a3.typeName() + ')';
    return false;
}
/******************************************************************************/
    ...
    if (CChar *text = expression)
        for (;;) {
            ...
            if (cf & CHARF_DIG || c == '.') {
                CalcElm &elm = elms.New();
                elm.type = CET_VAL;
                text = TextValue(text, elm.v);
                if (!elm.v.type)
                    return false;
            } else if (!(cf & (CHARF_ALPHA | CHARF_UNDER))) {
                if (!ReadOp(elms, text, error))
                    return false;
            } else {
                if (!ReadChar(elms, text, error, vars))
                    return false;
            }
        }
    ...
    {                \
        ...
        continue;    \
            ...
            if (cet == CET_OP && net == CET_VAL) {
                ...
                if (opi.args == 1) {
                    if (!((Bool(*)(CalcValue &))opi.func)(n->v))
                        return Error(error, opi.t, n->v);
                    elms.remove(i);
                    NEXT(next);
                }
                ...
                if (opi.args == 2 && pet == CET_VAL)
                    if (!ppet || (ppet == CET_OP && opi.priority < CalcOpInfos[pp->o.type].priority))
                        if (!nnet || (nnet == CET_OP && opi.priority <= CalcOpInfos[nn->o.type].priority)) {
                            if (!((Bool(*)(CalcValue &, CalcValue &))opi.func)(p->v, n->v))
                                return Error(error, opi.t, p->v, n->v);
                            elms.remove(i);
                            elms.remove(next);
                            NEXT(prev);
                        }
                ...
                if (cot == COT_ADD)
                    if (!pet || (pet == CET_OP && p->o.type != COT_RBR)) {
                        elms.remove(i);
                        NEXT(next);
                    }
                if (cot == COT_SUB)
                ...
                if (cot == COT_LBR)
                    if (nnet == CET_OP && nn->o.type == COT_RBR && pet != CET_FUNC) {
                        elms.remove(i);
                        elms.remove(nnext);
                        NEXT(next);
                    }
            } else if (cet == CET_FUNC) {
                C CalcFuncInfo &funi = *c->f.funi;
                if (CFuncReady(elms, i, funi.args)) {
                    CalcValue &v0 = nn->v;
                    if (funi.args == 1) {
                        if (!((Bool(*)(CalcValue &))funi.func)(v0))
                            return Error(error, funi.t, v0);
                    } else if (funi.args == 2) {
                        MemlNode *n1 = nnext->next()->next();
                        CalcValue &v1 = Celm(n1)->v;
                        if (!((Bool(*)(CalcValue &, CalcValue &))funi.func)(v0, v1))
                            return Error(error, funi.t, v0, v1);
                    } else if (funi.args == 3) {
                        MemlNode *n1 = nnext->next()->next();
                        CalcValue &v1 = Celm(n1)->v;
                        MemlNode *n2 = n1->next()->next();
                        CalcValue &v2 = Celm(n2)->v;
                        if (!((Bool(*)(CalcValue &, CalcValue &, CalcValue &))funi.func)(v0, v1, v2))
                            return Error(error, funi.t, v0, v1, v2);
                    } else if (funi.args == 4) {
                        MemlNode *n1 = nnext->next()->next();
                        CalcValue &v1 = Celm(n1)->v;
                        MemlNode *n2 = n1->next()->next();
                        CalcValue &v2 = Celm(n2)->v;
                        CalcValue &v3 = Celm(n2->next()->next())->v;
                        if (!((Bool(*)(CalcValue &, CalcValue &, CalcValue &, CalcValue &))funi.func)(v0, v1, v2, v3))
                            return Error(error, funi.t, v0, v1, v2, v3);
                    }
                    elms.remove(i);    // func
                    elms.remove(next); // (
                    i = nnext->next();
                    REPD(j, 2 * (funi.args - 1)) {
                        next = i->next();
                        elms.remove(i);
                        i = next;
                    } // ,y
...
/******************************************************************************/
Int CalculateI(C Str &expression, Str *error, C CMemPtr<CalcVar> &vars) {
    CalcValue value;
    Calculate(value, expression, error, vars);
    return value.asInt();
}
Long CalculateL(C Str &expression, Str *error, C CMemPtr<CalcVar> &vars) {
    CalcValue value;
    Calculate(value, expression, error, vars);
    return value.asLong();
}
Flt CalculateF(C Str &expression, Str *error, C CMemPtr<CalcVar> &vars) {
    CalcValue value;
    Calculate(value, expression, error, vars);
    return value.asFlt();
}
Dbl CalculateD(C Str &expression, Str *error, C CMemPtr<CalcVar> &vars) {
    CalcValue value;
    Calculate(value, expression, error, vars);
    return value.asDbl();
}
VecD2 CalculateV2(C Str &expression, Str *error, C CMemPtr<CalcVar> &vars) {
    CalcValue value;
    Calculate(value, expression, error, vars);
    return value.asVecD2();
}
VecD CalculateV(C Str &expression, Str *error, C CMemPtr<CalcVar> &vars) {
    CalcValue value;
    Calculate(value, expression, error, vars);
    return value.asVecD();
}
VecD4 CalculateV4(C Str &expression, Str *error, C CMemPtr<CalcVar> &vars) {
    CalcValue value;
    Calculate(value, expression, error, vars);
    return value.asVecD4();
}
/******************************************************************************/
} // namespace EE
/******************************************************************************/
