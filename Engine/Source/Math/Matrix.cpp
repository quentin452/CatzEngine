...
void Matrix2::mul(C Matrix2 &m, Matrix2 &dest) C {
    ...
    } else if (&dest != this) // here it's OK if "&dest==&m"
    {
        x = m.x.x;
        y = m.y.x;
    ...
    {
        Matrix2 temp = T;
        temp.mul(temp, dest); // have to operate on a temporary because all matrixes are the same
    }
}
...
void Matrix::mulTimes(Int n, C Matrix &matrix, Matrix &dest) C {
    ...
    default: {
        ...
        } else if (matrix.pos.allZero() && Equal(matrix.scale2(), VecOne)) // no position and no scale - we can just rotate by "n*angle"
        {
            Vec axis;
            Flt angle = matrix.axisAngle(axis);
            Matrix matrix_n;
            matrix_n.setRotate(axis, angle * n);
            mul(matrix_n, dest);
        } else {
            Matrix temp = matrix;
            if (n < 0) {
            ...
            mul(temp, dest);
            REP(n - 1)
            dest *= temp; // make the first mul from this->dest, and remaining mul's from dest->dest
        }
    } break;
    }
}
        ...
        } else if (matrix.pos.allZero() && Equal(matrix.scale2(), VecOne)) // no position and no scale - we can just rotate by "n*angle"
        {
            ...
            FREPA(PrimeNumbers) // optimize using prime numbers
            {
                ...
                for (; n % pm == 0; n /= pm) {
                    ...
                    REP(pm - 1)
                    temp *= single;
                } // in this case it doesn't matter if 'single' is 'Matrix' or 'RevMatrix'
            }
            mul(temp, dest);
            REP(n - 1)
            dest *= temp; // make the first mul from this->dest, and remaining mul's from dest->dest
...
void Matrix2::divNormalized(C Matrix2 &m, Matrix2 &dest) C {
    ...
    } else if (&dest != this) // here it's OK if "&dest==&m"
    {
        ...
        Matrix2 temp = T;
        temp.divNormalized(temp, dest); // have to operate on a temporary because all matrixes are the same
    }
}
void Matrix3::divNormalized(C Matrix3 &m, Matrix3 &dest) C {
    ...
    } else if (&dest != this) // here it's OK if "&dest==&m"
    {
        ...
        Flt y_x = m.y.x;
        dest.y.x = x_x * T.y.x + x_y * T.y.y + x_z * T.y.z;
        Flt z_x = m.z.x;
        dest.z.x = x_x * T.z.x + x_y * T.z.y + x_z * T.z.z;

        Flt y_y = m.y.y, y_z = m.y.z;
        dest.x.y = y_x * T.x.x + y_y * T.x.y + y_z * T.x.z;
        dest.y.y = y_x * T.y.x + y_y * T.y.y + y_z * T.y.z;
        Flt z_y = m.z.y;
        dest.z.y = y_x * T.z.x + y_y * T.z.y + y_z * T.z.z;
...
/******************************************************************************/
void Matrix2 ::div(C Matrix2 &m, Matrix2 &dest) C {
    Matrix2 temp;
    m.inverse(temp);
    mul(temp, dest);
}
void Matrix2 ::div(C Matrix2P &m, Matrix2P &dest) C {
    Matrix2P temp;
    m.inverse(temp);
    mul(temp, dest);
}
void Matrix3 ::div(C Matrix3 &m, Matrix3 &dest) C {
    Matrix3 temp;
    m.inverse(temp);
    mul(temp, dest);
}
void Matrix3 ::div(C Matrix &m, Matrix &dest) C {
    Matrix temp;
    m.inverse(temp);
    mul(temp, dest);
}
void Matrix3 ::div(C MatrixM &m, MatrixM &dest) C {
    MatrixM temp;
    m.inverse(temp);
    mul(temp, dest);
}
void MatrixD3::div(C Matrix3 &m, MatrixD3 &dest) C {
    Matrix3 temp;
    m.inverse(temp);
    mul(temp, dest);
}
void MatrixD3::div(C MatrixD3 &m, MatrixD3 &dest) C {
    MatrixD3 temp;
    m.inverse(temp);
    mul(temp, dest);
}
void MatrixD3::div(C MatrixD &m, MatrixD &dest) C {
    MatrixD temp;
    m.inverse(temp);
    mul(temp, dest);
}
void Matrix2P::div(C Matrix2 &m, Matrix2P &dest) C {
    Matrix2 temp;
    m.inverse(temp);
    mul(temp, dest);
}
void Matrix ::div(C Matrix3 &m, Matrix &dest) C {
    Matrix3 temp;
    m.inverse(temp);
    mul(temp, dest);
}
void MatrixM ::div(C Matrix3 &m, MatrixM &dest) C {
    Matrix3 temp;
    m.inverse(temp);
    mul(temp, dest);
}
void MatrixD ::div(C MatrixD3 &m, MatrixD &dest) C {
    MatrixD3 temp;
    m.inverse(temp);
    mul(temp, dest);
}
void Matrix2P::div(C Matrix2P &m, Matrix2P &dest) C {
    Matrix2P temp;
    m.inverse(temp);
    mul(temp, dest);
}
void Matrix ::div(C Matrix &m, Matrix &dest) C {
    Matrix temp;
    m.inverse(temp);
    mul(temp, dest);
}
void Matrix ::div(C MatrixM &m, Matrix &dest) C {
    MatrixM temp;
    m.inverse(temp);
    mul(temp, dest);
}
void Matrix ::div(C MatrixM &m, MatrixM &dest) C {
    MatrixM temp;
    m.inverse(temp);
    mul(temp, dest);
}
void MatrixM ::div(C MatrixM &m, Matrix &dest) C {
    MatrixM temp;
    m.inverse(temp);
    mul(temp, dest);
}
void MatrixM ::div(C Matrix &m, MatrixM &dest) C {
    Matrix temp;
    m.inverse(temp);
    mul(temp, dest);
}
void MatrixM ::div(C MatrixM &m, MatrixM &dest) C {
    MatrixM temp;
    m.inverse(temp);
    mul(temp, dest);
}
void MatrixD ::div(C MatrixD &m, MatrixD &dest) C {
    MatrixD temp;
    m.inverse(temp);
    mul(temp, dest);
}
...
Matrix3 &Matrix3::normalize() {
    if (!x.normalize() || !y.normalize() || !z.normalize())
        identity();
    return T;
}
...
Matrix2 &Matrix2::normalize(Flt scale) {
    if (!x.setLength(scale) || !y.setLength(scale))
        setScale(scale);
    return T;
}
Matrix3 &Matrix3::normalize(Flt scale) {
    if (!x.setLength(scale) || !y.setLength(scale) || !z.setLength(scale))
        setScale(scale);
    return T;
}
...
Matrix2 &Matrix2::normalize(C Vec2 &scale) {
    if (!x.setLength(scale.x) || !y.setLength(scale.y))
        setScale(scale);
    return T;
}
Matrix3 &Matrix3::normalize(C Vec &scale) {
    if (!x.setLength(scale.x) || !y.setLength(scale.y) || !z.setLength(scale.z))
        setScale(scale);
    return T;
}
...
Matrix2P &Matrix2P::scaleL(C Vec2 &scale) {
    ...
    if (d = scale.x - 1) {
        dir = x;
        dir.normalize();
        pos += dir * (DistPointPlane(pos, dir) * d);
    }
    if (d = scale.y - 1) {
        dir = y;
        dir.normalize();
...
Matrix3 &Matrix3::scalePlane(C Vec &nrm, Flt scale) {
    ...
    if (scale)
        T.scale(nrm, 1 / scale); // then perform axis-scale by reverse of 'scale'
    return T;
}
...
/******************************************************************************/
Matrix3 &Matrix3 ::rotateXY(Flt x, Flt y) {
    if (Any(x, y))
        T *= Matrix3().setRotateXY(x, y);
    return T;
}
MatrixD3 &MatrixD3::rotateXY(Dbl x, Dbl y) {
    if (Any(x, y))
        T *= MatrixD3().setRotateXY(x, y);
    return T;
}
Matrix &Matrix ::rotateXY(Flt x, Flt y) {
    if (Any(x, y))
        T *= Matrix3().setRotateXY(x, y);
    return T;
}
MatrixM &MatrixM ::rotateXY(Flt x, Flt y) {
    if (Any(x, y))
        T *= Matrix3().setRotateXY(x, y);
    return T;
}
MatrixD &MatrixD ::rotateXY(Dbl x, Dbl y) {
    if (Any(x, y))
        T *= MatrixD3().setRotateXY(x, y);
    return T;
}

Matrix3 &Matrix3 ::rotate(C Vec &axis, Flt angle) {
    if (Any(angle))
        T *= Matrix3().setRotate(axis, angle);
    return T;
}
MatrixD3 &MatrixD3::rotate(C VecD &axis, Dbl angle) {
    if (Any(angle))
        T *= MatrixD3().setRotate(axis, angle);
    return T;
}
Matrix &Matrix ::rotate(C Vec &axis, Flt angle) {
    if (Any(angle))
        T *= Matrix3().setRotate(axis, angle);
    return T;
}
MatrixM &MatrixM ::rotate(C Vec &axis, Flt angle) {
    if (Any(angle))
        T *= Matrix3().setRotate(axis, angle);
    return T;
}
MatrixD &MatrixD ::rotate(C VecD &axis, Dbl angle) {
    if (Any(angle))
        T *= MatrixD3().setRotate(axis, angle);
    return T;
}

Matrix3 &Matrix3::rotateXL(Flt angle) {
    if (Any(angle))
        rotate(!x, angle);
    return T;
}
Matrix3 &Matrix3::rotateYL(Flt angle) {
    if (Any(angle))
        rotate(!y, angle);
    return T;
}
Matrix3 &Matrix3::rotateZL(Flt angle) {
    if (Any(angle))
        rotate(!z, angle);
    return T;
}

MatrixD3 &MatrixD3::rotateXL(Dbl angle) {
    if (Any(angle))
        rotate(!x, angle);
    return T;
}
MatrixD3 &MatrixD3::rotateYL(Dbl angle) {
    if (Any(angle))
        rotate(!y, angle);
    return T;
}
MatrixD3 &MatrixD3::rotateZL(Dbl angle) {
    if (Any(angle))
        rotate(!z, angle);
    return T;
}
...
Matrix3 &Matrix3::keep01(Flt blend) {
    // orientation
    Vec axis;
    Flt angle = axisAngle(axis);

    // scale
    Vec scale = T.scale();
    REPA(scale)
    scale.c[i] = ScaleFactor(ScaleFactorR(scale.c[i]) * blend);
...
/******************************************************************************/
Matrix2P &Matrix2P::setRotate(Flt angle) {
    super::setRotate(angle);
    pos.zero();
    return T;
}

Matrix &Matrix::setRotateX(Flt angle) {
    super::setRotateX(angle);
    pos.zero();
    return T;
}
Matrix &Matrix::setRotateY(Flt angle) {
    super::setRotateY(angle);
    pos.zero();
    return T;
}
Matrix &Matrix::setRotateZ(Flt angle) {
    super::setRotateZ(angle);
    pos.zero();
    return T;
}
Matrix &Matrix::setRotateXY(Flt x, Flt y) {
    super::setRotateXY(x, y);
    pos.zero();
    return T;
}
Matrix &Matrix::setRotate(C Vec &axis, Flt angle) {
    super::setRotate(axis, angle);
    pos.zero();
    return T;
}

MatrixM &MatrixM::setRotateX(Flt angle) {
    super::setRotateX(angle);
    pos.zero();
    return T;
}
MatrixM &MatrixM::setRotateY(Flt angle) {
    super::setRotateY(angle);
    pos.zero();
    return T;
}
MatrixM &MatrixM::setRotateZ(Flt angle) {
    super::setRotateZ(angle);
    pos.zero();
    return T;
}
MatrixM &MatrixM::setRotateXY(Flt x, Flt y) {
    super::setRotateXY(x, y);
    pos.zero();
    return T;
}
MatrixM &MatrixM::setRotate(C Vec &axis, Flt angle) {
    super::setRotate(axis, angle);
    pos.zero();
    return T;
}

MatrixD &MatrixD::setRotateX(Dbl angle) {
    super::setRotateX(angle);
    pos.zero();
    return T;
}
MatrixD &MatrixD::setRotateY(Dbl angle) {
    super::setRotateY(angle);
    pos.zero();
    return T;
}
MatrixD &MatrixD::setRotateZ(Dbl angle) {
    super::setRotateZ(angle);
    pos.zero();
    return T;
}
MatrixD &MatrixD::setRotateXY(Dbl x, Dbl y) {
    super::setRotateXY(x, y);
    pos.zero();
    return T;
}
MatrixD &MatrixD::setRotate(C VecD &axis, Dbl angle) {
    super::setRotate(axis, angle);
    pos.zero();
    return T;
}
...
Matrix3 &Matrix3 ::setRight(C Vec &right) {
    x = right;
    y = PerpN(x);
    z = Cross(x, y);
    return T;
}
MatrixD3 &MatrixD3::setRight(C VecD &right) {
    x = right;
    y = PerpN(x);
    z = Cross(x, y);
    return T;
}
Matrix3 &Matrix3 ::setRight(C Vec &right, C Vec &up) {
    x = right;
    y = up;
    z = Cross(x, y);
    return T;
}
MatrixD3 &MatrixD3::setRight(C VecD &right, C VecD &up) {
    x = right;
    y = up;
    z = Cross(x, y);
    return T;
}
Matrix3 &Matrix3 ::setUp(C Vec &up) {
    y = up;
    z = PerpN(y);
    x = Cross(y, z);
    return T;
}
MatrixD3 &MatrixD3::setUp(C VecD &up) {
    y = up;
    z = PerpN(y);
    x = Cross(y, z);
    return T;
}
Matrix3 &Matrix3 ::setDir(C Vec &dir) {
    z = dir;
    y = PerpN(z);
    x = Cross(y, z);
    return T;
}
MatrixD3 &MatrixD3::setDir(C VecD &dir) {
    z = dir;
    y = PerpN(z);
    x = Cross(y, z);
    return T;
}
Matrix3 &Matrix3 ::setDir(C Vec &dir, C Vec &up) {
    z = dir;
    y = up;
    x = Cross(y, z);
    return T;
}
MatrixD3 &MatrixD3::setDir(C VecD &dir, C VecD &up) {
    z = dir;
    y = up;
    x = Cross(y, z);
    return T;
}
Matrix3 &Matrix3 ::setDir(C Vec &dir, C Vec &up, C Vec &right) {
    z = dir;
    y = up;
    x = right;
    return T;
}
MatrixD3 &MatrixD3::setDir(C VecD &dir, C VecD &up, C VecD &right) {
    z = dir;
    y = up;
    x = right;
    return T;
...
Matrix &Matrix ::setPosOrient(C Vec &pos, DIR_ENUM dir) {
    super::setOrient(dir);
    T.pos = pos;
    return T;
}
MatrixM &MatrixM::setPosOrient(C VecD &pos, DIR_ENUM dir) {
    super::setOrient(dir);
    T.pos = pos;
    return T;
}
MatrixD &MatrixD::setPosOrient(C VecD &pos, DIR_ENUM dir) {
    super::setOrient(dir);
    T.pos = pos;
    return T;
}
Matrix &Matrix ::setPosRight(C Vec &pos, C Vec &right) {
    super::setRight(right);
    T.pos = pos;
    return T;
}
MatrixM &MatrixM::setPosRight(C VecD &pos, C Vec &right) {
    super::setRight(right);
    T.pos = pos;
    return T;
}
MatrixD &MatrixD::setPosRight(C VecD &pos, C VecD &right) {
    super::setRight(right);
    T.pos = pos;
    return T;
}
Matrix &Matrix ::setPosUp(C Vec &pos, C Vec &up) {
    super::setUp(up);
    T.pos = pos;
    return T;
}
MatrixM &MatrixM::setPosUp(C VecD &pos, C Vec &up) {
    super::setUp(up);
    T.pos = pos;
    return T;
}
MatrixD &MatrixD::setPosUp(C VecD &pos, C VecD &up) {
    super::setUp(up);
    T.pos = pos;
    return T;
}
Matrix &Matrix ::setPosDir(C Vec &pos, C Vec &dir) {
    super::setDir(dir);
    T.pos = pos;
    return T;
}
MatrixM &MatrixM::setPosDir(C VecD &pos, C Vec &dir) {
    super::setDir(dir);
    T.pos = pos;
    return T;
}
MatrixD &MatrixD::setPosDir(C VecD &pos, C VecD &dir) {
    super::setDir(dir);
    T.pos = pos;
    return T;
}
Matrix &Matrix ::setPosDir(C Vec &pos, C Vec &dir, C Vec &up) {
    super::setDir(dir, up);
    T.pos = pos;
    return T;
}
MatrixM &MatrixM::setPosDir(C VecD &pos, C Vec &dir, C Vec &up) {
    super::setDir(dir, up);
    T.pos = pos;
    return T;
}
MatrixD &MatrixD::setPosDir(C VecD &pos, C VecD &dir, C VecD &up) {
    super::setDir(dir, up);
    T.pos = pos;
    return T;
}
Matrix &Matrix ::setPosDir(C Vec &pos, C Vec &dir, C Vec &up, C Vec &right) {
    z = dir;
    y = up;
    x = right;
    T.pos = pos;
...
MatrixM &MatrixM::setPosDir(C VecD &pos, C Vec &dir, C Vec &up, C Vec &right) {
    z = dir;
    y = up;
    x = right;
    T.pos = pos;
...
MatrixD &MatrixD::setPosDir(C VecD &pos, C VecD &dir, C VecD &up, C VecD &right) {
    z = dir;
    y = up;
    x = right;
    T.pos = pos;
...
Matrix &Matrix ::setPosTerrainOrient(C Vec &pos, DIR_ENUM dir) {
    super::setTerrainOrient(dir);
    T.pos = pos;
    return T;
}
MatrixM &MatrixM::setPosTerrainOrient(C VecD &pos, DIR_ENUM dir) {
    super::setTerrainOrient(dir);
    T.pos = pos;
    return T;
}
...
{
    Vec cross = Cross(dir_from, dir_to);
    if (Flt sin = cross.normalize())
        setRotateCosSin(cross, Dot(dir_from, dir_to), sin);
    else
        identity();
    return T;
}
...
{
    VecD cross = Cross(dir_from, dir_to);
    if (Dbl sin = cross.normalize())
        setRotateCosSin(cross, Dot(dir_from, dir_to), sin);
    else
        identity();
    return T;
...
Matrix3 &Matrix3::setRotationUp(C Vec &dir_to) {
    Vec cross = CrossUp(dir_to);
    if (Flt sin = cross.normalize())
        setRotateCosSin(cross, dir_to.y, sin);
    else
        identity();
    return T;
}
...
Matrix3 &Matrix3::setRotation(C Vec &dir_from, C Vec &dir_to, Flt blend) // !! Warning: this will give identity if directions are opposite "dir_from==-dir_to" !!
{
    Vec cross = Cross(dir_from, dir_to);
    if (Flt sin = cross.normalize())
        setRotate(cross, ACosSin(Dot(dir_from, dir_to), sin) * blend);
    else
        identity();
    return T;
}
...
{
    Vec cross = Cross(dir_from, dir_to);
    Flt sin = cross.normalize();
    if (roll) {
        setRotate(dir_from, roll);
        if (sin)
            rotate(cross, ACosSin(Dot(dir_from, dir_to), sin) * blend);
    } else {
        if (sin)
            setRotate(cross, ACosSin(Dot(dir_from, dir_to), sin) * blend);
        else
...
Matrix &Matrix::setRotation(C Vec &pos, C Vec &dir_from, C Vec &dir_to, Flt blend) // !! Warning: this will give identity if directions are opposite "dir_from==-dir_to" !!
{
    Vec cross = Cross(dir_from, dir_to);
    if (Flt sin = cross.normalize()) {
        orn().setRotate(cross, ACosSin(Dot(dir_from, dir_to), sin) * blend);
        anchor(pos);
    } else
        identity();
    return T;
...
Matrix &Matrix::setNormalizeX(C Box &box) {
    Flt d = box.w();
    setPosScale(-box.center(), (d > EPS) ? 1 / d : 1);
    return T;
}
Matrix &Matrix::setNormalizeY(C Box &box) {
    Flt d = box.h();
    setPosScale(-box.center(), (d > EPS) ? 1 / d : 1);
    return T;
}
Matrix &Matrix::setNormalizeZ(C Box &box) {
    Flt d = box.d();
    setPosScale(-box.center(), (d > EPS) ? 1 / d : 1);
    return T;
}
Matrix &Matrix::setNormalize(C Box &box) {
    ...
    if (size.x >= size.y && size.x >= size.z)
        return setNormalizeX(box); // x is the biggest
    if (size.y >= size.z)
        return setNormalizeY(box); // y is the biggest
...
MatrixD &MatrixD::mirrorX() {
    super::mirrorX();
    CHS(pos.x);
    return T;
}
MatrixD &MatrixD::mirrorY() {
    super::mirrorY();
    CHS(pos.y);
    return T;
}
MatrixD &MatrixD::mirrorZ() {
    super::mirrorZ();
    CHS(pos.z);
    return T;
}

Matrix &Matrix ::mirror(C Plane &plane) {
    super::mirror(plane.normal);
    pos = Mirror(pos, plane.pos, plane.normal);
    return T;
}
MatrixM &MatrixM::mirror(C PlaneM &plane) {
    super::mirror(plane.normal);
    pos = Mirror(pos, plane.pos, plane.normal);
    return T;
}
MatrixD &MatrixD::mirror(C PlaneD &plane) {
    super::mirror(plane.normal);
    pos = Mirror(pos, plane.pos, plane.normal);
    return T;
}
...
Flt Matrix3::angleY(Bool normalized) C {
    ...
    if (m->x.x >= EPS_COS)
        return 0; // axis=(1,0,0) so axis.y==0
    ...
    if (xx >= yy && xx >= zz)
        return (m->x.y + m->y.x) / SqrtFast(xx) * (PI_4 * SQRT2);
    if (yy >= zz)
        return SqrtFast(yy) * (PI / SQRT2);
...
Dbl MatrixD3::angleY(Bool normalized) C {
    ...
    if (xx >= yy && xx >= zz)
        return (m->x.y + m->y.x) / SqrtFast(xx) * (PID_4 * SQRT2);
    if (yy >= zz)
        return SqrtFast(yy) * (PID / SQRT2);
...
Vec Matrix3 ::axisAngle(Bool normalized) C {
    Vec axis;
    Flt angle = axisAngle(axis, normalized);
    return axis *= angle;
}
VecD MatrixD3::axisAngle(Bool normalized) C {
    VecD axis;
    Dbl angle = axisAngle(axis, normalized);
    return axis *= angle;
}
/******************************************************************************/
...
Matrix &Matrix::setTransformAtPos(C Vec &pos, C Matrix3 &matrix) {
    orn() = matrix;
    anchor(pos);
    return T;
}
Matrix &Matrix::setTransformAtPos(C Vec &pos, C Matrix &matrix) {
    orn() = matrix;
    anchor(pos);
    move(matrix.pos);
    return T;
...
MatrixM &MatrixM::setTransformAtPos(C VecD &pos, C Matrix3 &matrix) {
    orn() = matrix;
    anchor(pos);
    return T;
}
MatrixM &MatrixM::setTransformAtPos(C VecD &pos, C MatrixM &matrix) {
    orn() = matrix;
    anchor(pos);
    move(matrix.pos);
    return T;
...
MatrixD &MatrixD::setTransformAtPos(C VecD &pos, C MatrixD3 &matrix) {
    orn() = matrix;
    anchor(pos);
    return T;
}
MatrixD &MatrixD::setTransformAtPos(C VecD &pos, C MatrixD &matrix) {
    orn() = matrix;
    anchor(pos);
    move(matrix.pos);
    return T;
...
Flt Matrix4::determinant() C {
    return x.w * y.z * z.y * pos.x - x.z * y.w * z.y * pos.x - x.w * y.y * z.z * pos.x + x.y * y.w * z.z * pos.x + x.z * y.y * z.w * pos.x - x.y * y.z * z.w * pos.x - x.w * y.z * z.x * pos.y + x.z * y.w * z.x * pos.y + x.w * y.x * z.z * pos.y - x.x * y.w * z.z * pos.y - x.z * y.x * z.w * pos.y + x.x * y.z * z.w * pos.y + x.w * y.y * z.x * pos.z - x.y * y.w * z.x * pos.z - x.w * y.x * z.y * pos.z + x.x * y.w * z.y * pos.z + x.y * y.x * z.w * pos.z - x.x * y.y * z.w * pos.z - x.z * y.y * z.x * pos.w + x.y * y.z * z.x * pos.w + x.z * y.x * z.y * pos.w - x.x * y.z * z.y * pos.w - x.y * y.x * z.z * pos.w + x.x * y.y * z.z * pos.w;
}
/******************************************************************************/
...
void GetTransform(Matrix3 &transform, C Matrix3 &start, C Matrix3 &result) {
    Matrix3 temp;
    start.inverse(temp);
    temp.mul(result, transform);
} // use 'temp' in case "&transform==&result"
void GetTransform(MatrixD3 &transform, C MatrixD3 &start, C MatrixD3 &result) {
    MatrixD3 temp;
    start.inverse(temp);
    temp.mul(result, transform);
} // use 'temp' in case "&transform==&result"
void GetTransform(Matrix &transform, C Matrix &start, C Matrix &result) {
    Matrix temp;
    start.inverse(temp);
    temp.mul(result, transform);
} // use 'temp' in case "&transform==&result"
void GetTransform(MatrixM &transform, C MatrixM &start, C MatrixM &result) {
    MatrixM temp;
    start.inverse(temp);
    temp.mul(result, transform);
} // use 'temp' in case "&transform==&result"
void GetTransform(MatrixD &transform, C MatrixD &start, C MatrixD &result) {
    MatrixD temp;
    start.inverse(temp);
    temp.mul(result, transform);
} // use 'temp' in case "&transform==&result"
Matrix3 GetTransform(C Matrix3 &start, C Matrix3 &result) {
    Matrix3 transform;
    start.inverse(transform);
    transform *= result;
    return transform;
}
MatrixD3 GetTransform(C MatrixD3 &start, C MatrixD3 &result) {
    MatrixD3 transform;
    start.inverse(transform);
    transform *= result;
    return transform;
}
Matrix GetTransform(C Matrix &start, C Matrix &result) {
    Matrix transform;
    start.inverse(transform);
    transform *= result;
    return transform;
}
MatrixM GetTransform(C MatrixM &start, C MatrixM &result) {
    MatrixM transform;
    start.inverse(transform);
    transform *= result;
    return transform;
}
MatrixD GetTransform(C MatrixD &start, C MatrixD &result) {
    MatrixD transform;
    start.inverse(transform);
    transform *= result;
    return transform;
}
Matrix3 GetTransform(C Orient &start, C Orient &result) {
    Matrix3 transform;
    GetTransform(transform, start, result);
    return transform;
}
/******************************************************************************/
...
INLINE void CalcPos(Vec &pos_delta, C Vec &pos_delta0, C Vec &pos_delta1) {
    ...
    Flt sin = cross.normalize();
    if (sin > EPS) // here 'sin' is proportional to vector lengths
    {
        ...
        if (l0 < l1)
            angle *= SqrtFast(l0 / l1); // use Sqrt just one time instead of 2 times above if using 'length'
        else
            angle *= SqrtFast(l1 / l0);
        // if(angle>EPS)
        {
...
#else // optimized
                CosSin(cos, sin, angle);
                pos_delta *= angle / Dist(cos - 1, sin);
#endif
            }
        }
    }
...
void AnimatedSkeleton::setMatrix() C {
    ...
    if (Renderer._mesh_shader_vel)                                    // we need to process velocities (this is disabled in 'Renderer.del' to prevent using shader handles after deletion)
    {
        ...
        REP(matrixes)
        ViewMatrix[VIRTUAL_ROOT_BONE + i].fromMul(bones[i].matrix(), CamMatrixInv); // Warning: this does not call 'setChanged'
                                                                                    /* Bone Splits old code
...
void SetVelFur(C Matrix3 &view_matrix, C Vec &vel) {
    Vec v = vel * D.furStaticVelScale();
    v.y += D.furStaticGravity();
    v.clipLength(0.92f);
#if DEBUG && 0
    Matrix3 obj_matrix = view_matrix * CamMatrix.orn();
    obj_matrix.normalize();
    Vec v2 = v / obj_matrix; // 'v2' should be the same as 'v' below, this can be used for testing
#endif
...
void AnimatedSkeleton::setFurVel() C {
    ...
    if (VIRTUAL_ROOT_BONE)
        Sh.FurVel->set(FurVelShader(VecZero /*    root._fur_vel*/, matrix()));
    REP(matrixes)
    Sh.FurVel->set(FurVelShader(VecZero /*bones[i]._fur_vel*/, bones[i]._matrix), VIRTUAL_ROOT_BONE + i);
    /* Bone Splits
...
#if DEBUG
    if (Any(m.x.y, m.x.z, m.x.w) || Any(m.y.x, m.y.z, m.y.w)
        //|| Any(m.z  .x, m.z  .y              ) // 'm.z.x' can be adjusted for VR Stereo or Temporal, 'm.z.y' can be adjusted for Temporal
        || Any(m.pos.x, m.pos.y))
        Exit("Shader 'Project' needs to be adjusted"); // 'm.pos.x' could be adjusted, however it's possible only for orthogonal mode, which is not used in VR (shadows are not stereo, and main view is always perspective based on VR FOV)
#endif
}
...
void SetProjMatrix() // this needs to be additionally called when switching between '_main' and some other RT on OpenGL
{
    if (Renderer._temporal_use && Renderer() != RM_SHADOW) {
        ...
        m.offsetX(Renderer._temporal_offset.x);
        m.offsetY(Renderer._temporal_offset.y);
        Matrix4 p = Renderer._ctx_sub->proj_matrix_prev;
        p.offsetX(Renderer._temporal_offset.x);
        p.offsetY(Renderer._temporal_offset.y);

        if (GL && !D.mainFBO()) {
            FlipY(m);
            FlipY(p);
        } // in OpenGL when drawing to a RenderTarget the Y must be flipped
        Sh.ProjMatrix->set(m);
        TestProjMatrix(m);
        Sh.ProjMatrixPrev->set(p);
        TestProjMatrix(p);
    } else if (GL && !D.mainFBO()) // in OpenGL when drawing to a RenderTarget the Y must be flipped
    {
        Matrix4 m = ProjMatrix;
        FlipY(m);
        Sh.ProjMatrix->set(m);
        TestProjMatrix(m);
        m = Renderer._ctx_sub->proj_matrix_prev;
        FlipY(m);
        Sh.ProjMatrixPrev->set(m);
        TestProjMatrix(m);
    } else {
        Sh.ProjMatrix->set(ProjMatrix);
        TestProjMatrix(ProjMatrix);
        Sh.ProjMatrixPrev->set(Renderer._ctx_sub->proj_matrix_prev);
        TestProjMatrix(Renderer._ctx_sub->proj_matrix_prev);
    }
}
...
Flt GetLodDist2(C Vec &lod_center) {
    ...
    if (FovPerspective(D.viewFovMode()))
        dist2 *= Dist2(lod_center, ActiveCam.matrix.pos); // has to be 'ActiveCam.matrix' and not 'CamMatrix' because we need the same LOD for shadows as for normal
    return dist2;
}
Flt GetLodDist2(C Vec &lod_center, C Matrix3 &matrix) {
    ...
    if (FovPerspective(D.viewFovMode()))
        dist2 *= Dist2(lod_center * matrix, ActiveCam.matrix.pos); // has to be 'ActiveCam.matrix' and not 'CamMatrix' because we need the same LOD for shadows as for normal
    return dist2;
}
