...
Int Number::asInt() C {
    UInt u = (U32 &)d[_real ? NUMBER_DIGS / 2 : 0];
    return sign ? -Int(u) : u;
}
UInt Number::asUInt() C { return (U32 &)d[_real ? NUMBER_DIGS / 2 : 0]; }
Long Number::asLong() C {
    ULong u = (U64 &)d[_real ? NUMBER_DIGS / 2 : 0];
...
Number &Number::operator=(UInt u) {
    zero();
    *(U32 *)d = u;
    return T;
}
Number &Number::operator=(Long i) {
    zero();
    sign = (i < 0);
    *(U64 *)d = Abs(i);
    return T;
}
Number &Number::operator=(ULong u) {
    zero();
    *(U64 *)d = u;
    return T;
}
...
Flt Number::asFlt() C {
    ...
    if (Int digits = T.digits()) {
        ...
        UInt significand = 0;
        pos--;
        if (pos < NUMBER_DIGS * 16)
            significand |= Shr(d[pos >> 4], (pos & 15) - 22);
        pos -= 16;
        if (pos < NUMBER_DIGS * 16)
            significand |= Shr(d[pos >> 4], (pos & 15) - 6);
        pos -= 16;
        if (pos < NUMBER_DIGS * 16)
            significand |= Shr(d[pos >> 4], (pos & 15) + 10);
...
Dbl Number::asDbl() C {
    ...
    if (Int digits = T.digits()) {
        ...
        pos--;
        if (pos < NUMBER_DIGS * 16)
            m0 |= Shr(d[pos >> 4], (pos & 15) - 19);
        pos -= 16;
        if (pos < NUMBER_DIGS * 16) {
            m0 |= Shr(d[pos >> 4], (pos & 15) - 3);
            m1 |= Shr(d[pos >> 4], (pos & 15) - 35);
        }
        pos -= 16;
        if (pos < NUMBER_DIGS * 16) {
            m0 |= Shr(d[pos >> 4], (pos & 15) + 13);
            m1 |= Shr(d[pos >> 4], (pos & 15) - 19);
        }
        pos -= 16;
        if (pos < NUMBER_DIGS * 16)
            m1 |= Shr(d[pos >> 4], (pos & 15) - 3);
        pos -= 16;
        if (pos < NUMBER_DIGS * 16)
            m1 |= Shr(d[pos >> 4], (pos & 15) + 13);
        U32 f[2] =
            {
                m1,
...
Number &Number::operator=(Flt f) {
    ...
    pos--;
    if (pos < NUMBER_DIGS * 16)
        T.d[pos >> 4] |= Shl(significand, (pos & 15) - 22);
    pos -= 16;
    if (pos < NUMBER_DIGS * 16)
        T.d[pos >> 4] |= Shl(significand, (pos & 15) - 6);
    pos -= 16;
    if (pos < NUMBER_DIGS * 16)
        T.d[pos >> 4] |= Shl(significand, (pos & 15) + 10);
...
Number &Number::operator=(Dbl f) {
    ...
    pos--;
    if (pos < NUMBER_DIGS * 16)
        T.d[pos >> 4] |= Shl(m0, (pos & 15) - 19);
    pos -= 16;
    if (pos < NUMBER_DIGS * 16)
        T.d[pos >> 4] |= Shl(m0, (pos & 15) - 3) | Shl(m1, (pos & 15) - 35);
    pos -= 16;
    if (pos < NUMBER_DIGS * 16)
        T.d[pos >> 4] |= Shl(m0, (pos & 15) + 13) | Shl(m1, (pos & 15) - 19);
    pos -= 16;
    if (pos < NUMBER_DIGS * 16)
        T.d[pos >> 4] |= Shl(m1, (pos & 15) - 3);
    pos -= 16;
    if (pos < NUMBER_DIGS * 16)
        T.d[pos >> 4] |= Shl(m1, (pos & 15) + 13);
...
Int Number::absCompare(UInt N) C {
    if (!_real) {
        ...
        for (Int i = NUMBER_DIGS - 1; i >= NUMBER_DIGS / 2 + 2; i--)
            if (d[i])
                return +1;
        Int d = T.d[NUMBER_DIGS / 2 + 1] - (N >> 16);
        if (d > 0)
            return +1;
        if (d < 0)
            return -1;
        d = T.d[NUMBER_DIGS / 2] - (N & 0xFFFF);
        if (d > 0)
            return +1;
        if (d < 0)
            return -1;
...
Int Compare(C Number &n0, C Number &n1) {
    if (n0.sign == n1.sign)
        return n0.sign ? -n0.absCompare(n1) : n0.absCompare(n1);
    return n0.sign ? -1 : +1;
}
Int Compare(C Number &n0, C Int &n1) {
    if (n0.sign == (n1 < 0))
        return n0.sign ? -n0.absCompare(Abs(n1)) : n0.absCompare(Abs(n1));
    return n0.sign ? -1 : +1;
}
/******************************************************************************/
// SHIFT ROLL
...
Number &Number::rol(Int bits) {
    if (bits > 0) {
        ...
        REP(NUMBER_DIGS) {
            UInt j = i - D;
            T.d[i] = (digMod(j) << bits) | (digMod(j - 1) >> r);
        }
    } else if (bits)
        ror(-bits);
    return T;
...
static void DivModSub(Number &x, C Number &y, Number &div) {
    if (!DivModFast(x, y, div)) {
        UInt div_sub = 0;
        for (; x.rawCompare(y) >= 0; x.rawSub(y), div_sub++)
            ;
        div = div_sub;
    }
}
static void DivMod(Number &x, C Number &y, Number &div) {
    ...
    REP(NUMBER_DIGS) {
        ...
        if (temp.rawCompare(*Y) >= 0) {
            Number bkp = temp, div_sub;
            DivModSub(temp, *Y, div_sub);
...
Number operator+(C Number &a, Int b) { return Number(a) += b; }
Number operator+(Int a, C Number &b) { return Number(a) += b; }
Number operator+(C Number &a, Dbl b) { return Number(a) += b; }
Number operator+(Dbl a, C Number &b) { return Number(a) += b; }
Number operator-(C Number &a, C Number &b) { return Number(a) -= b; }
Number operator-(C Number &a, Int b) { return Number(a) -= b; }
Number operator-(Int a, C Number &b) { return Number(a) -= b; }
Number operator-(C Number &a, Dbl b) { return Number(a) -= b; }
...
Number Number2::normalize() {
    Number l = length();
    T /= l;
    return l;
}
Number Number3::normalize() {
    Number l = length();
    T /= l;
    return l;
}
