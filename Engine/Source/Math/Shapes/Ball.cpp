...
Bool Ball::from(C Vec *point, Int points) {
    Box box;
    if (box.from(point, points)) {
        pos = box.center();
        Flt dist2 = 0;
        REP(points)
        MAX(dist2, Dist2(point[i], pos));
        r = SqrtFast(dist2);
        return true;
    }
    zero();
...
Bool Ball::from(C Vec *point, Int points, C Matrix &matrix) {
    ...
    if (box.from(point, points)) {
        ...
        Flt dist2 = 0;
        REP(points)
        MAX(dist2, Dist2(point[i] * matrix, pos));
        r = SqrtFast(dist2);
        return true;
    }
    zero();
...
void Ball::drawVI(Bool fill, C VecI2 &resolution) C {
    ...
    REPD(y, res_y - 1) {
        Flt y2_cr, y2_sr;
        CosSin(y2_cr, y2_sr, -PI_2 + PI * y / (res_y - 1));
        y2_sr *= r;
        y2_cr *= r;
        Flt x1_s = 0,
            x1_c = 1;
        REPD(x, res_x) {
            ...
            if (fill) {
                ...
                if (y)
                    VI.line(pos + Vec(x1_c * y2_cr, y2_sr, x1_s * y2_cr), pos + Vec(x2_c * y2_cr, y2_sr, x2_s * y2_cr));
            }
            x1_s = x2_s;
            x1_c = x2_c;
        }
...
void Ball::draw(C Color &color, Bool fill, C VecI2 &resolution) C {
    VI.color(color);
    drawVI(fill, resolution);
    VI.end();
}
void Ball::drawAngle(C Color &color, Flt from, Flt to, C Vec &up, Bool fill, C VecI2 &resolution) C {
    to -= from;
    Matrix m;
    m.setPosUp(VecZero, up).scale(r).move(pos);
    Int res_x = ((resolution.x < 0) ? 24 : ((resolution.x < 3) ? 3 : resolution.x)),
        res_y = ((resolution.y < 0) ? 24 : ((resolution.y < 3) ? 3 : resolution.y));
    Flt y1_cr, y1_sr;
    CosSin(y1_cr, y1_sr, from + to);

    VI.color(color);
    REPD(y, res_y - 1) {
        Flt y2_cr, y2_sr;
        CosSin(y2_cr, y2_sr, from + to * y / (res_y - 1));
        Flt x1_s = 0,
            x1_c = 1;
        REPD(x, res_x) {
            Flt x2_c, x2_s;
            ...
            if (fill) {
                ...
                if (y)
                    VI.line(Vec(x1_c * y2_cr, y2_sr, x1_s * y2_cr) * m, Vec(x2_c * y2_cr, y2_sr, x2_s * y2_cr) * m);
            }
            x1_s = x2_s;
            x1_c = x2_c;
        }
...
void Ball::draw2(C Color &color, Bool fill, Int resolution) C {
    Matrix m[6];
    REP(6)
    m[i].setPosOrient(pos, DIR_ENUM(i));
    m[DIR_BACK].y.chs();
    m[DIR_LEFT].y.chs();
...
void SphereConvert::init(Int res) {
    T.res = res;
    pos_to_cell_mul = res / PI_2;
    pos_to_cell_add = res * 0.5f;
    cell_to_pos_mul = PI_2 / res; // cell_to_pos_add=-PI_4;
    tans.setNumDiscard(res + 1);
    REPAO(tans) = cellToPos(i);
}
void SphereConvertEx::init(Int res) {
    super::init(res);
    tan_mids.setNumDiscard(res);
    REPAO(tan_mids) = cellToPos(i + 0.5f);
}
/******************************************************************************/
DIR_ENUM DirToCubeFace(C Vec &dir) {
    ...
    if (abs.x >= abs.z) {
        if (abs.x >= abs.y)
            return (dir.x >= 0) ? DIR_RIGHT : DIR_LEFT; // X
    Y:
        return (dir.y >= 0) ? DIR_UP : DIR_DOWN; // Y
    }
    if (abs.y >= abs.z)
        goto Y;
...
void CubeFacePosToPos(DIR_ENUM cube_face, Vec &dest, C Vec &src) {
    switch (cube_face) {
    case DIR_RIGHT:
        dest.set(src.z, src.y, -src.x);
        break;
    case DIR_LEFT:
        dest.set(-src.z, src.y, src.x);
        break;
    case DIR_UP:
        dest.set(src.x, src.z, -src.y);
        break;
    case DIR_DOWN:
        dest.set(src.x, -src.z, src.y);
        break;
...
void PosToTerrainPos(DIR_ENUM cube_face, Vec2 &dest, C Vec &src) {
    ...
    {
    case DIR_RIGHT:
        dest.set(src.z, src.y);
        break;
    case DIR_LEFT:
        dest.set(-src.z, src.y);
        break;
    case DIR_UP:
        dest.set(src.x, src.z);
        break;
    case DIR_DOWN:
        dest.set(src.x, -src.z);
        break;
...
void PosToTerrainPos(DIR_ENUM cube_face, Edge &dest, C Edge &src) {
    ...
    case DIR_RIGHT:
        FREP(2)
        dest.p[i].set(src.p[i].z, src.p[i].y, src.p[i].x);
        break;
    case DIR_LEFT:
        FREP(2)
        dest.p[i].set(-src.p[i].z, src.p[i].y, -src.p[i].x);
    ...
    case DIR_UP:
        FREP(2)
        dest.p[i].set(src.p[i].x, src.p[i].z, src.p[i].y);
        break;
    case DIR_DOWN:
        FREP(2)
        dest.p[i].set(src.p[i].x, -src.p[i].z, -src.p[i].y);
    ...
    case DIR_FORWARD:
        FREP(2)
        dest.p[i].set(-src.p[i].x, src.p[i].y, src.p[i].z);
        break;
    case DIR_BACK:
        FREP(2)
        dest.p[i].set(src.p[i].x, src.p[i].y, -src.p[i].z);
...
DIR_ENUM DirToCubeFacePixel(C Vec &dir, Int res, Vec2 &xy) // this matches exact same results as drawing Cube on GPU, 'xy' has to be passed to 'Image.colorFLinear' (for exact results Image also needs to have linear gamma)
{
    ...
    if (abs.x >= abs.z) {
        if (abs.x >= abs.y) {
            if (!abs.x) {
                xy.zero();
                return DIR_RIGHT;
            } // only this case can have zero, because we've checked x>=z && x>=y, any other case will have non-zero
            mul /= abs.x;
            if (dir.x >= 0) {
                xy.set(-dir.z * mul + add, -dir.y * mul + add);
                return DIR_RIGHT;
            }
            {
                xy.set(dir.z * mul + add, -dir.y * mul + add);
    ...
    Y:
        mul /= abs.y;
        if (dir.y >= 0) {
            xy.set(dir.x * mul + add, dir.z * mul + add);
            return DIR_UP;
        }
        {
            xy.set(dir.x * mul + add, -dir.z * mul + add);
    ...
    mul /= abs.z;
    if (dir.z >= 0) {
        xy.set(dir.x * mul + add, -dir.y * mul + add);
        return DIR_FORWARD;
    }
    {
        xy.set(-dir.x * mul + add, -dir.y * mul + add);
...
DIR_ENUM DirToSphereCubeFacePixel(C Vec &dir, Int res, Vec2 &xy) {
    ...
    xy.set(Atan(x) * mul + add, Atan(y) * mul + add);
    return ret;
}
...
DIR_ENUM SphereConvert::dirToSphereTerrainPixel(C Vec &dir, Vec2 &xy) C {
    ...
    xy.set(posToCell(x), posToCell(y));
    return ret;
}
DIR_ENUM SphereConvert::dirToSphereTerrainPixelIMid(C Vec &dir, VecI2 &xy) C {
    ...
    xy.set(posToCellIMid(x), posToCellIMid(y));
    return ret;
}
...
Bool SphereConvert::intersects(C SphereArea &area_pos, C Ball &ball, Flt min_radius) C {
    ...
    Ball oriented_ball;
    PosToTerrainPos(area_pos.side, oriented_ball.pos, ball.pos);
    oriented_ball.r = ball.r;
...
static Bool PosToCellX(C SphereConvert &sc, C Vec &pos, Int &cell) {
    if (pos.z > 0) {
        cell = sc.posToCellI(pos.x / pos.z);
        return true;
    }
    return false;
}
static Bool PosToCellY(C SphereConvert &sc, C Vec &pos, Int &cell) {
    if (pos.z > 0) {
        cell = sc.posToCellI(pos.y / pos.z);
        return true;
    }
    return false;
}
 ...
 min_height - \------------/
    ...
    */
    ...
    for (ap.side = DIR_ENUM(0);;) {
        Ball oriented_ball;
        PosToTerrainPos(ap.side, oriented_ball.pos, ball.pos);
        oriented_ball.r = ball.r;
        if (ClipZ(oriented_ball, min_radius)) {
            RectI rect;
         ...
         rect.clampY(0, res-1);
            ...
            zd.set(oriented_ball.pos.x, 0, oriented_ball.pos.z);
            len2 = zd.length2();
            if (r2 >= len2)
                rect.setX(0, res - 1);
            else {
                ...
                cos = CosSin2(sin2);
                d = CrossUp(zd);
                d.setLength(cos * oriented_ball.r);
                zd *= -sin2;
                zd += oriented_ball.pos;
                if (PosToCellX(T, zd - d, rect.min.x))
                    MAX(rect.min.x, 0);
                else
                    rect.min.x = 0;
                if (PosToCellX(T, zd + d, rect.max.x))
                    MIN(rect.max.x, res - 1);
                else
                    rect.max.x = res - 1;
                if (!rect.validX())
                    goto next;
            ...
            zd.set(0, oriented_ball.pos.y, oriented_ball.pos.z);
            len2 = zd.length2();
            if (r2 >= len2)
                rect.setY(0, res - 1);
            else {
                ...
                zd *= -sin2;
                zd += oriented_ball.pos;
                if (PosToCellY(T, zd + d, rect.min.y))
                    MAX(rect.min.y, 0);
                else
                    rect.min.y = 0;
                if (PosToCellY(T, zd - d, rect.max.y))
                    MIN(rect.max.y, res - 1);
                else
                    rect.max.y = res - 1;
                if (!rect.validY())
                    goto next;
                ...
                for (ap.x = rect.min.x; ap.x <= rect.max.x; ap.x++) {
                        ...
                        && ap.x != ball_cell.x) // if ball is on left or right side
                    {
                        ...
                        Vec dir(_cellToPos(ap.x + (ap.x < ball_cell.x)), // (ap.x<ball_cell.x) ? _cellToPos(ap.x+1) : _cellToPos(ap.x)
                                ...
                                1);
                        dir.normalize();
                        if (Dist2PointLine(oriented_ball.pos, dir) >= r2)
                            continue;
                    }
                    area_pos.add(ap);
                }
    ...
    next:
        if (ap.side == DIR_NUM - 1)
            break;
        ap.side = DIR_ENUM(ap.side + 1);
    }
}
void SphereConvertEx::getIntersectingAreas(MemPtr<SphereArea> area_pos, C Vec &dir, Flt angle) C {
    ...
    for (ap.side = DIR_ENUM(0);;) {
        ...
        {
            Vec dir_face;
            PosToTerrainPos(ap.side, dir_face, dir); // convert world space 'dir' to local 'ap.side' space 'dir_face'
            ...
            // X
            zd.set(dir_face.x, 0, dir_face.z);
            len2 = zd.length2();
            if (ball_r2 >= len2)
                rect.setX(0, res - 1);
            else {
                ...
                test = zd - d;
                if (test.z > 0) {
                    rect.min.x = posToCellI(test.x / test.z);
                    if (rect.min.x < 0) {
                    left:
                        rect.min.x = 0;
                    }
                ...
                test = zd + d;
                if (test.z > 0) {
                    rect.max.x = posToCellI(test.x / test.z);
                    if (rect.max.x >= res) {
                    right:
                        rect.max.x = res - 1;
                    }
            ...
            // Y
            zd.set(0, dir_face.y, dir_face.z);
            len2 = zd.length2();
            if (ball_r2 >= len2)
                rect.setY(0, res - 1);
            else {
                ...
                test = zd + d;
                if (test.z > 0) {
                    rect.min.y = posToCellI(test.y / test.z);
                    if (rect.min.y < 0) {
                    down:
                        rect.min.y = 0;
                    }
                ...
                test = zd - d;
                if (test.z > 0) {
                    rect.max.y = posToCellI(test.y / test.z);
                    if (rect.max.y >= res) {
                    up:
                        rect.max.y = res - 1;
                    }
            ...
            for (ap.y = rect.min.y; ap.y <= rect.max.y; ap.y++)
                for (ap.x = rect.min.x; ap.x <= rect.max.x; ap.x++) {
                    Vec area_dir = _sphereTerrainPixelCenterToDir(ap.side, ap.x, ap.y);
                    area_dir.normalize();
                    if (Dot(area_dir, dir) > cos_min)
                        area_pos.add(ap);
                }
        }
    next:
...
void SphereConvertEx::sort(MemPtr<SphereArea> areas, C Vec &pos, Bool reverse) C {
    Memt<SphereAreaDist> area_dist;
    area_dist.setNum(areas.elms());
    REPA(area_dist) {
    ...
    if (reverse)
        area_dist.sort(Compare); // 'Compare' are reversed because 'pos' is treated as direction instead of position
    else
        area_dist.sort(CompareR);
    REPAO(areas) = area_dist[i];
}
void SphereConvertEx::sort(MemPtr<SphereAreaUS> areas, C Vec &pos, Bool reverse) C {
    Memt<SphereAreaUSDist> area_dist;
    area_dist.setNum(areas.elms());
    REPA(area_dist) {
...
void WrapCubeFacePixel(SphereArea &dest, C SphereArea &src, Int res) {
    Int x = (InRange(src.x, res) ? X(0) : (src.x < 0) ? X(-1)
                                                      : X(1));
    Int y = (InRange(src.y, res) ? Y(0) : (src.y < 0) ? Y(-1)
                                                      : Y(1));
    switch (src.side + x + y) {
    default:
        dest = src;
    ...
    case V(DIR_RIGHT, -1, 0):
        dest.set(DIR_FORWARD, N0(src.x), Z0(src.y));
        break;
    case V(DIR_RIGHT, 1, 0):
        dest.set(DIR_BACK, P0(src.x), Z0(src.y));
        break;
    case V(DIR_RIGHT, 0, -1):
        dest.set(DIR_UP, N0(src.y), Z1(src.x));
        break;
    case V(DIR_RIGHT, 0, 1):
        dest.set(DIR_DOWN, P1(src.y), Z0(src.x));
        break;

    case V(DIR_LEFT, -1, 0):
        dest.set(DIR_BACK, N0(src.x), Z0(src.y));
        break;
    case V(DIR_LEFT, 1, 0):
        dest.set(DIR_FORWARD, P0(src.x), Z0(src.y));
        break;
    case V(DIR_LEFT, 0, -1):
        dest.set(DIR_UP, N1(src.y), Z0(src.x));
        break;
    case V(DIR_LEFT, 0, 1):
        dest.set(DIR_DOWN, P0(src.y), Z1(src.x));
        break;

    case V(DIR_UP, -1, 0):
        dest.set(DIR_LEFT, Z0(src.y), N1(src.x));
        break;
    case V(DIR_UP, 1, 0):
        dest.set(DIR_RIGHT, Z1(src.y), P0(src.x));
        break;
    case V(DIR_UP, 0, -1):
        dest.set(DIR_BACK, Z1(src.x), N1(src.y));
        break;
    case V(DIR_UP, 0, 1):
        dest.set(DIR_FORWARD, Z0(src.x), P0(src.y));
        break;

    case V(DIR_DOWN, -1, 0):
        dest.set(DIR_LEFT, Z1(src.y), N0(src.x));
        break;
    case V(DIR_DOWN, 1, 0):
        dest.set(DIR_RIGHT, Z0(src.y), P1(src.x));
        break;
    case V(DIR_DOWN, 0, -1):
        dest.set(DIR_FORWARD, Z0(src.x), N0(src.y));
        break;
    case V(DIR_DOWN, 0, 1):
        dest.set(DIR_BACK, Z1(src.x), P1(src.y));
        break;

    case V(DIR_FORWARD, -1, 0):
        dest.set(DIR_LEFT, N0(src.x), Z0(src.y));
        break;
    case V(DIR_FORWARD, 1, 0):
        dest.set(DIR_RIGHT, P0(src.x), Z0(src.y));
        break;
    case V(DIR_FORWARD, 0, -1):
        dest.set(DIR_UP, Z0(src.x), N0(src.y));
        break;
    case V(DIR_FORWARD, 0, 1):
        dest.set(DIR_DOWN, Z0(src.x), P0(src.y));
        break;

    case V(DIR_BACK, -1, 0):
        dest.set(DIR_RIGHT, N0(src.x), Z0(src.y));
        break;
    case V(DIR_BACK, 1, 0):
        dest.set(DIR_LEFT, P0(src.x), Z0(src.y));
        break;
    case V(DIR_BACK, 0, -1):
        dest.set(DIR_UP, Z1(src.x), N1(src.y));
        break;
    case V(DIR_BACK, 0, 1):
        dest.set(DIR_DOWN, Z1(src.x), P1(src.y));
        break;
    }
...
void WrapSphereTerrainPixel(SphereArea &dest, C SphereArea &src, Int res) {
    Int x = (InRange(src.x, res + 1) ? X(0) : (src.x < 0) ? X(-1)
                                                          : X(1));
    Int y = (InRange(src.y, res + 1) ? Y(0) : (src.y < 0) ? Y(-1)
                                                          : Y(1));
    switch (src.side + x + y) {
    default:
        dest = src;
    ...
    case V(DIR_RIGHT, 1, 0):
        ...
        break;
    case V(DIR_RIGHT, 0, -1):
        dest.set(DIR_DOWN, N0(src.y), Z1(src.x));
        break;
    case V(DIR_RIGHT, 0, 1):
        dest.set(DIR_UP, P1(src.y), Z0(src.x));
        break;
    ...
    case V(DIR_LEFT, 1, 0):
        ...
        break;
    case V(DIR_LEFT, 0, -1):
        dest.set(DIR_DOWN, N1(src.y), Z0(src.x));
        break;
    case V(DIR_LEFT, 0, 1):
        dest.set(DIR_UP, P0(src.y), Z1(src.x));
        break;
    ...
    case V(DIR_UP, 1, 0):
        ...
        break;
    case V(DIR_UP, 0, -1):
        dest.set(DIR_BACK, Z0(src.x), N0(src.y));
        break;
    case V(DIR_UP, 0, 1):
        dest.set(DIR_FORWARD, Z1(src.x), P1(src.y));
        break;
    ...
    case V(DIR_DOWN, 1, 0):
        ...
        break;
    case V(DIR_DOWN, 0, -1):
        dest.set(DIR_FORWARD, Z1(src.x), N1(src.y));
        break;
    case V(DIR_DOWN, 0, 1):
        dest.set(DIR_BACK, Z0(src.x), P0(src.y));
        break;
    ...
    case V(DIR_FORWARD, 1, 0):
        ...
        break;
    case V(DIR_FORWARD, 0, -1):
        dest.set(DIR_DOWN, Z1(src.x), N1(src.y));
        break;
    case V(DIR_FORWARD, 0, 1):
        dest.set(DIR_UP, Z1(src.x), P1(src.y));
        break;
    ...
    case V(DIR_BACK, 1, 0):
        ...
        break;
    case V(DIR_BACK, 0, -1):
        dest.set(DIR_DOWN, Z0(src.x), N0(src.y));
        break;
    case V(DIR_BACK, 0, 1):
        dest.set(DIR_UP, Z0(src.x), P0(src.y));
        break;
    }
    Clamp(dest.x, 0, res);
    Clamp(dest.y, 0, res);
...
Bool ClipToTerrainAndProject(DIR_ENUM cube_face, Edge2 &dest, C Edge &src, Flt min_z) {
    Edge face_edge;
    PosToTerrainPos(cube_face, face_edge, src);
...
Flt Dist(C Box &box, C Ball &ball) {
    ...
    if (ball.pos.x < box.min.x)
        dist2 += Sqr(ball.pos.x - box.min.x);
    else if (ball.pos.x > box.max.x)
        dist2 += Sqr(ball.pos.x - box.max.x);

    if (ball.pos.y < box.min.y)
        dist2 += Sqr(ball.pos.y - box.min.y);
    else if (ball.pos.y > box.max.y)
        dist2 += Sqr(ball.pos.y - box.max.y);

    if (ball.pos.z < box.min.z)
        dist2 += Sqr(ball.pos.z - box.min.z);
    else if (ball.pos.z > box.max.z)
        dist2 += Sqr(ball.pos.z - box.max.z);
...
Bool Cuts(C Box &box, C Ball &ball) {
#if 1 // fastest
    if (ball.pos.x < box.min.x - ball.r || ball.pos.x > box.max.x + ball.r)
        return false;
    if (ball.pos.y < box.min.y - ball.r || ball.pos.y > box.max.y + ball.r)
        return false;
    if (ball.pos.z < box.min.z - ball.r || ball.pos.z > box.max.z + ball.r)
        return false;
    ...
    if (ball.pos.x < box.min.x) {
        dist2 = Sqr(box.min.x - ball.pos.x);
        if (dist2 > r2)
            return false;
    } else if (ball.pos.x > box.max.x) {
        dist2 = Sqr(ball.pos.x - box.max.x);
    ...
    if (ball.pos.y < box.min.y) {
        dist2 += Sqr(box.min.y - ball.pos.y);
        if (dist2 > r2)
            return false;
    } else if (ball.pos.y > box.max.y) {
        dist2 += Sqr(ball.pos.y - box.max.y);
    ...
    if (ball.pos.z < box.min.z) {
        dist2 += Sqr(box.min.z - ball.pos.z);
        if (dist2 > r2)
            return false;
    } else if (ball.pos.z > box.max.z) {
        dist2 += Sqr(ball.pos.z - box.max.z);
    ...
    if (ball.pos.x < box.min.x) {
        d = box.min.x - ball.pos.x;
        if (d > ball.r)
            return false;
        dist2 = Sqr(d);
    } else if (ball.pos.x > box.max.x) {
        d = ball.pos.x - box.max.x;
        if (d > ball.r)
            return false;
        dist2 = Sqr(d);
    }
    ...
    if (ball.pos.y < box.min.y) {
        d = box.min.y - ball.pos.y;
        if (d > ball.r)
            return false;
        dist2 += Sqr(d);
    } else if (ball.pos.y > box.max.y) {
        d = ball.pos.y - box.max.y;
        if (d > ball.r)
            return false;
        dist2 += Sqr(d);
    }
    ...
    if (ball.pos.z < box.min.z) {
        d = box.min.z - ball.pos.z;
        if (d > ball.r)
            return false;
        dist2 += Sqr(d);
    } else if (ball.pos.z > box.max.z) {
        d = ball.pos.z - box.max.z;
        if (d > ball.r)
            return false;
        dist2 += Sqr(d);
    }
...
#else // slower
    Flt dist2 = 0, d;
    d = Abs(ext.pos.x - ball.pos.x) - ext.ext.x;
    if (d > 0)
        dist2 = Sqr(d);
    d = Abs(ext.pos.y - ball.pos.y) - ext.ext.y;
    if (d > 0)
        dist2 += Sqr(d);
    d = Abs(ext.pos.z - ball.pos.z) - ext.ext.z;
    if (d > 0)
        dist2 += Sqr(d);
    return dist2 <= Sqr(ball.r);
#endif
...
Int CutsLineBall(C Vec &line_pos, C Vec &line_dir, C Ball &ball, Vec *contact_a, Vec *contact_b) {
    ...
    if (contact_a || contact_b) {
        ...
        if (contact_a)
            *contact_a = p - d * line_dir;
        if (contact_b)
            *contact_b = p + d * line_dir;
    }
    return 2;
}
...
#else // this is faster
    return Max(Sqr(a.max.x - b.pos.x), Sqr(a.min.x - b.pos.x)) + Max(Sqr(a.max.y - b.pos.y), Sqr(a.min.y - b.pos.y)) + Max(Sqr(a.max.z - b.pos.z), Sqr(a.min.z - b.pos.z)) <= Sqr(b.r);
#endif
}
...
Bool SweepPointBall(C Vec &point, C Vec &move, C Ball &ball, Flt *hit_frac, Vec *hit_normal) {
    ...
    Flt dist = DistPointPlane(p, point, dir);
    if (dist < 0 || dist > length)
        return false;
    if (hit_frac)
        *hit_frac = dist / length;
    if (hit_normal)
        *hit_normal = (p - ball.pos) / ball.r;
    return true;
}
...
Bool SweepPointBall(C VecD &point, C VecD &move, C BallD &ball, Dbl *hit_frac, VecD *hit_normal) {
    ...
    Dbl sin2 = dist2 / r2;
    p -= dir * (CosSin2(sin2) * ball.r);
    Dbl dist = DistPointPlane(p, point, dir);
    if (dist < 0 || dist > length)
        return false;
    if (hit_frac)
        *hit_frac = dist / length;
...
Bool SweepBallPoint(C Ball &ball, C Vec &move, C Vec &point, Flt *hit_frac, Vec *hit_normal) {
    ...
    Flt sin2 = dist2 / r2;
    p -= dir * (CosSin2(sin2) * ball.r);
    Flt dist = DistPointPlane(p, ball.pos, dir);
    if (dist < 0 || dist > length)
        return false;
    if (hit_frac)
        *hit_frac = dist / length;
    if (hit_normal)
        *hit_normal = (p - point) / ball.r;
    return true;
}
...
Bool SweepBallPoint(C BallD &ball, C VecD &move, C VecD &point, Dbl *hit_frac, VecD *hit_normal) {
    ...
    Dbl sin2 = dist2 / r2;
    p -= dir * (CosSin2(sin2) * ball.r);
    Dbl dist = DistPointPlane(p, ball.pos, dir);
    if (dist < 0 || dist > length)
        return false;
    if (hit_frac)
        *hit_frac = dist / length;
...
Bool SweepEdgeBall(C Edge &edge, C Vec &move, C Ball &ball, Flt *hit_frac, Vec *hit_normal) // safe in case 'edge' is zero length
{
    Int point_test;
    Vec dir = edge.delta();
    if (dir.normalize()) // check if 'edge' has length
    {
        Matrix matrix;
        matrix.setPosDir(edge.p[0], dir);
        ...
        if (SweepPointCircle(Vec2Zero, move2D, circle, &frac, &normal)) {
            ...
            if (DistPointPlane(point, edge.p[0], matrix.z) < 0)
                point_test = 0;
            else if (DistPointPlane(point, edge.p[1], matrix.z) > 0)
                point_test = 1;
            else {
                if (hit_frac)
                    *hit_frac = frac;
                if (hit_normal)
                    *hit_normal = matrix.orn().convert(normal);
                return true;
            }
        } else
            point_test = Closer(ball.pos, edge.p[0], edge.p[1]);
    } else
        point_test = 0; // if 'edge' is zero length then check first point
