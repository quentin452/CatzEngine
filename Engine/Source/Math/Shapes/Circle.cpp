...
void Circle::draw(C Color &color, Bool fill, Int resolution) C {
    PROFILE_START("CatzEngine::Circle::draw(C Color &color, Bool fill, Int resolution)")
    if (resolution < 0)
        resolution = 96;
    ...
    REP(resolution) {
        ...
        Vec2 next(pos.x + c * r, pos.y + s * r);
        if (fill)
            VI.tri(pos, next, prev);
        else
            VI.line(prev, next);
        prev = next;
    }
    VI.end();
    PROFILE_STOP("CatzEngine::Circle::draw(C Color &color, Bool fill, Int resolution)")
}
void CircleM::draw(C Color &color, Bool fill, Int resolution) C {
    PROFILE_START("CatzEngine::CircleM::draw(C Color &color, Bool fill, Int resolution)")
    Vec2 pos = T.pos;
    ...
    VI.end();
    PROFILE_STOP("CatzEngine::CircleM::draw(C Color &color, Bool fill, Int resolution)")
}
void Circle::drawPie(C Color &color, Flt r_start, Flt angle_start, Flt angle_range, Bool fill, Int resolution) C {
    PROFILE_START("CatzEngine::Circle::drawPie(C Color &color, Flt r_start, Flt angle_start, Flt angle_range, Bool fill, Int resolution)")
    if (Abs(angle_range) >= PI2 && !fill) // draw inner and outer lines only
    {
        ...
        if (resolution < 0)
            ...
            resolution = 3;
        Flt c, s;
        CosSin(c, s, angle_start + angle_range);
        Vec2 prev(pos.x + c * r, pos.y + s * r), prev2(pos.x + c * r_start, pos.y + s * r_start);

        VI.color(color);
        if (!fill)
            VI.line(prev, prev2); // side line
        REP(resolution) {
            CosSin(c, s, angle_start + (angle_range * i) / resolution);
            Vec2 next(pos.x + c * r, pos.y + s * r), next2(pos.x + c * r_start, pos.y + s * r_start);
            if (fill)
                VI.quad(prev, next, next2, prev2);
            else {
                VI.line(prev, next);
                VI.line(prev2, next2);
            }
    ...
    PROFILE_STOP("CatzEngine::Circle::drawPie(C Color &color, Flt r_start, Flt angle_start, Flt angle_range, Bool fill, Int resolution)")
}
/******************************************************************************/
...
Flt Dist(C Rect &rect, C Circle &circle) {
    ...
    if (circle.pos.x < rect.min.x)
        dist2 += Sqr(circle.pos.x - rect.min.x);
    else if (circle.pos.x > rect.max.x)
        dist2 += Sqr(circle.pos.x - rect.max.x);

    if (circle.pos.y < rect.min.y)
        dist2 += Sqr(circle.pos.y - rect.min.y);
    else if (circle.pos.y > rect.max.y)
        dist2 += Sqr(circle.pos.y - rect.max.y);
...
Int CutsLineCircle(C Vec2 &line_pos, C Vec2 &line_nrm, C Circle &circle, Vec2 *contact_a, Vec2 *contact_b, Flt *width) {
    ...
    if (!y) {
        right *= circle.r;
        if (contact_a)
            *contact_a = circle.pos + right;
        if (contact_b)
            *contact_b = circle.pos - right;
        if (width)
            *width = circle.r;
        return 2;
    }
    if (y == circle.r) {
        if (contact_a)
            *contact_a = circle.pos + d * line_nrm;
        if (width)
            *width = 0;
        return 1;
    }
    ...
    right *= x;
    if (contact_a)
        *contact_a = center + right;
    if (contact_b)
        *contact_b = center - right;
    if (width)
        *width = x;
    return 2;
}
...
Int CutsCircleCircle(C Circle &c1, C Circle &c2, Vec2 *contact_a, Vec2 *contact_b, Flt *width) {
    ...
    if (d + Sr < Lr || d > c1.r + c2.r)
        return 0;

    // inside || outside together
    if (d + Sr == Lr || d == c1.r + c2.r) {
        if (contact_a)
            *contact_a = c1.pos + (c1.r / d) * edge;
        return 1;
    }

    // 2 points, straight covering those points : ax + by = c
    Flt c = c1.r * c1.r - c2.r * c2.r;
    Flt x1 = c1.pos.x, x2 = c2.pos.x, a = 2 * (x2 - x1);
    c += x2 * x2 - x1 * x1;
    Flt y1 = c1.pos.y, y2 = c2.pos.y, b = 2 * (y2 - y1);
    c += y2 * y2 - y1 * y1;
...
#else // this is faster
    return Max(Sqr(a.max.x - b.pos.x), Sqr(a.min.x - b.pos.x)) + Max(Sqr(a.max.y - b.pos.y), Sqr(a.min.y - b.pos.y)) <= Sqr(b.r);
#endif
}
/******************************************************************************/
Bool SweepPointCircle(C Vec2 &point, C Vec2 &move, C Circle &circle, Flt *hit_frac, Vec2 *hit_normal) {
    ...
    Vec2 dir_n = Perp(dir);
    Flt dist = DistPointPlane(circle.pos, point, dir_n);
    if (Abs(dist) > circle.r)
        return false;
    Flt sin = dist / circle.r;
    Vec2 normal = -sin * dir_n - CosSin(sin) * dir;
    Flt d = DistPointPlane(circle.pos + normal * circle.r, point, dir);
    if (d < 0 || d > length)
        return false;
    if (hit_frac)
        *hit_frac = d / length;
...
Bool SweepPointCircle(C VecD2 &point, C VecD2 &move, C CircleD &circle, Dbl *hit_frac, VecD2 *hit_normal) {
    ...
    Dbl dist = DistPointPlane(circle.pos, point, dir_n);
    if (Abs(dist) > circle.r)
        return false;
    Dbl sin = dist / circle.r;
    VecD2 normal = -sin * dir_n - CosSin(sin) * dir;
    Dbl d = DistPointPlane(circle.pos + normal * circle.r, point, dir);
    if (d < 0 || d > length)
        return false;
    if (hit_frac)
        *hit_frac = d / length;
...
Bool SweepCirclePoint(C Circle &circle, C Vec2 &move, C Vec2 &point, Flt *hit_frac, Vec2 *hit_normal) {
    ...
    Flt length = dir.normalize();
    Vec2 dir_n = Perp(dir);
    Flt dist = DistPointPlane(point, circle.pos, dir_n);
    if (Abs(dist) > circle.r)
        return false;
    Flt sin = dist / circle.r;
    Vec2 normal = -sin * dir_n - CosSin(sin) * dir;
    Flt d = DistPointPlane(point, circle.pos - normal * circle.r, dir);
    if (d < 0 || d > length)
        return false;
    if (hit_frac)
        *hit_frac = d / length;
...
Bool SweepCirclePoint(C CircleD &circle, C VecD2 &move, C VecD2 &point, Dbl *hit_frac, VecD2 *hit_normal) {
    ...
    Dbl length = dir.normalize();
    VecD2 dir_n = Perp(dir);
    Dbl dist = DistPointPlane(point, circle.pos, dir_n);
    if (Abs(dist) > circle.r)
        return false;
    Dbl sin = dist / circle.r;
    VecD2 normal = -sin * dir_n - CosSin(sin) * dir;
    Dbl d = DistPointPlane(point, circle.pos - normal * circle.r, dir);
    if (d < 0 || d > length)
        return false;
    if (hit_frac)
        *hit_frac = d / length;
...
Bool SweepEdgeCircle(C Edge2 &edge, C Vec2 &move, C Circle &circle, Flt *hit_frac, Vec2 *hit_normal) {
    ...
    if (!Dot(move, edge_normal))
        point_test = (Dot(edge_dir, move) > 0);
    else {
        Vec2 point = circle.pos - edge_normal * circle.r;
        Flt frac;
        if (SweepPlanePoint(Plane2(edge.p[0], edge_normal), move, point, &frac)) {
            point -= frac * move;
            if (DistPointPlane(point, edge.p[0], edge_dir) < 0)
                point_test = 0;
            else if (DistPointPlane(point, edge.p[1], edge_dir) > 0)
                point_test = 1;
            else {
                if (hit_frac)
                    *hit_frac = frac;
            ...
            point_test = Closer(circle.pos, edge.p[0], edge.p[1]);
    }
...
Bool SweepEdgeCircle(C EdgeD2 &edge, C VecD2 &move, C CircleD &circle, Dbl *hit_frac, VecD2 *hit_normal) {
    ...
    if (!Dot(move, edge_normal))
        ...
        Dbl frac;
        if (SweepPlanePoint(PlaneD2(edge.p[0], edge_normal), move, point, &frac)) {
            point -= frac * move;
            if (DistPointPlane(point, edge.p[0], edge_dir) < 0)
                point_test = 0;
            else if (DistPointPlane(point, edge.p[1], edge_dir) > 0)
...
Bool SweepCircleCircle(C Circle &circle, C Vec2 &move, C Circle &c2, Flt *hit_frac, Vec2 *hit_normal) {
    ...
    if (xs >= 1 && x1 >= 0 && x1 <= length) {
        ...
        if (hit_frac)
            *hit_frac = frac;
        if (hit_normal)
            *hit_normal = !((circle.pos + frac * move) - c2.pos);
        return true;
    }
    return false;
}
