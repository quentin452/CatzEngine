    ...
    if (ball) {
        ...
        REP(resolution + 1) {
            ...
            Vec dx = step * matrix.x,
                dz = step * matrix.z;
            VecD a, b;
            if (CutsLineBall(lb + dx, matrix.z, ball, &a, &b) == 2)
                VI.line(a, b);
            if (CutsLineBall(lb + dz, matrix.x, ball, &a, &b) == 2)
                VI.line(a, b);
        }
    } else {
        REP(resolution + 1) {
...
void Plane::drawInfiniteBySize(C Color &color, Flt cell_size) C {
    ...
    VecD cam_delta = CamMatrix.pos / ObjMatrix - pos,
         aligned = pos + matrix.x * AlignRound(Dot(cam_delta, matrix.x), _cell_size) + matrix.z * AlignRound(Dot(cam_delta, matrix.z), _cell_size);
...
void Plane::drawInfiniteByResolution(C Color &color, Int resolution) C {
    ...
    VecD cam_delta = CamMatrix.pos / ObjMatrix - pos,
         aligned = pos + matrix.x * AlignRound(Dot(cam_delta, matrix.x), cell_size) + matrix.z * AlignRound(Dot(cam_delta, matrix.z), cell_size);
...
Bool SweepPointPlane(C Vec2 &point, C Vec2 &move, C Plane2 &plane, Flt *hit_frac, Vec2 *hit_normal, Vec2 *hit_pos, Bool two_sided) {
    Flt way = Dot(move, plane.normal);
    if (two_sided ? (way == 0) : (way >= 0))
        return false; // we ignore cases when the point is already on the plane to allow movement along the plane surface, in 'two_sided' mode we skip if we move along the plane surface "way==0", in one sided mode we skip if we move along the plane surface "way==0" or in the same direction as plane normal "way>0"
...
Bool CutsLinePlane(C Vec &line_pos, C Vec &line_dir, C Plane &plane, Flt *hit_frac, Vec *hit_normal, Vec *hit_pos, Bool two_sided) {
    Flt way = Dot(line_dir, plane.normal);
    if (two_sided ? (way == 0) : (way >= 0))
        return false; // we ignore cases when the point is already on the plane to allow movement along the plane surface, in 'two_sided' mode we skip if we move along the plane surface "way==0", in one sided mode we skip if we move along the plane surface "way==0" or in the same direction as plane normal "way>0"
    if (hit_frac || hit_pos) {
        ...
        if (hit_frac)
            *hit_frac = frac;
        if (hit_pos)
            *hit_pos = line_pos + frac * line_dir;
    }
    if (hit_normal) {
        *hit_normal = plane.normal;
        if (way > 0)
...
Bool CutsLinePlane(C VecD &line_pos, C VecD &line_dir, C PlaneD &plane, Dbl *hit_frac, VecD *hit_normal, VecD *hit_pos, Bool two_sided) {
    Dbl way = Dot(line_dir, plane.normal);
    if (two_sided ? (way == 0) : (way >= 0))
        return false; // we ignore cases when the point is already on the plane to allow movement along the plane surface, in 'two_sided' mode we skip if we move along the plane surface "way==0", in one sided mode we skip if we move along the plane surface "way==0" or in the same direction as plane normal "way>0"
    if (hit_frac || hit_pos) {
...
Bool Intersect(C Plane &a, C Plane &b, Vec &line_pos, Vec &line_dir) {
    line_dir = Cross(a.normal, b.normal);
    if (!line_dir.normalize())
        return false;
...
void SlideMovement(Vec2 &move, C Vec2 *normal, Int normals) {
    Flt dist = 0;
    Int find = -1;
    REP(normals) // find biggest distance (furthest from zero) that's <0 (intersecting)
    {
        Flt d = Dot(move, normal[i]);
        if (d < dist) {
            dist = d;
            find = i;
        } // if intersecting (and distance bigger than previous)
    }
    if (find >= 0) {
        C Vec2 &furthest_nrm = normal[find];
        move -= dist * furthest_nrm;
        REP(normals)
        if (i != find) // check all normals but skip the one we processed already
        {
            C Vec2 &nrm = normal[i];
...
void SlideMovement(Vec &move, C Vec *normal, Int normals) {
    ...
    REPD(i0, normals) {
        ...
        Flt d = Dot(move, nrm0);
        if (d < 0) // if intersecting
        {
            hit0 = true; // have any intersection
            Bool hit1 = false;
            Vec slide0 = move - d * nrm0;
            REPD(i1, normals)
            if (i0 != i1) // check all normals but skip the one we processed already
            {
                        ...
                        && Dot(nrm0, nrm1) < EPS_COS)) // and not the same normal
                {
                    ...
                    REPD(i2, normals)
                    if (i0 != i2 && i1 != i2) // check all normals but skip the one we processed already
                    {
                    ...
                    if (!slide_valid || slide1.length2() > slide.length2()) {
                        slide_valid = true;
                        slide = slide1;
                    }
                hit2:;
            ...
            if (!hit1) {
                if (!slide_valid || slide0.length2() > slide.length2()) {
                    slide_valid = true;
                    slide = slide0;
                }
            }
...
#endif
            Real move_dot = Dot(move_n, edge_n);
            if (move_dot < 0) // if moving towards edge
            {
                Real dist_nrm = DistPointPlane(cur_pos, edge_p0, edge_n);
                if (dist_nrm >= 0) // if outside the edge
                {
                    if (step && Dot(edge_n, hit_nrm_prev) >= EPS_NRM_DOT)
                        goto ignore_edge; // if same normal as from previous step, then ignore this edge

                    dist_nrm -= object_r;
                    if (dist_nrm <= 0) // object intersecting edge
                    {
                        VecR2 edge_dir = Perp(edge_n); // p0->p1 (p1-p0)
                        if (DistPointPlane(cur_pos, edge_p0, edge_dir) >= 0 && DistPointPlane(cur_pos, edge_p1, edge_dir) <= 0) {
                        ...
                        Real dist_move = -dist_nrm / move_dot;
                        if (dist_move < hit_len) // if less than previous distance
                        {
                            ...
                            VecR2 edge_dir = Perp(edge_n); // p0->p1 (p1-p0)
                            if (DistPointPlane(closest, edge_p0, edge_dir) >= 0 && DistPointPlane(closest, edge_p1, edge_dir) <= 0) {
                                hit_len = dist_move;
                                hit_nrm = edge_n;
                            }
                        }
                    ...
                    if (pos_rel.y > object_rel.y) // test only > and not >= because if it's =0 then it means cur_pos and vtx_pos are the same and movement would always be stuck no matter what direction
                    {
                        ...
                        if (dist_move < hit_len + object_r)        // if can potentially have smaller distance
                        {
                            ...
                            if (dist_move < hit_len)                              // if less than previous distance
                            {
                                ...
                                {
                                    VecR2 delta = cur_pos - pos;
                                    Real delta_len2 = delta.length2();
                                    if (!delta_len2)
                                        goto ignore_vtx; // if length is zero then cur pos and vtx pos are the same so just ignore it, this check is also needed for div by 0
                                                         // if(delta_len2<Sqr(object_r)) shouldn't be needed at this stage
                                    {
                                        Real delta_len = SqrtFast(delta_len2);
                                        VecR2 nrm = delta / delta_len;
                                        if (step && Dot(nrm, hit_nrm_prev) >= EPS_NRM_DOT)
                                            goto ignore_vtx; // if same normal as from previous step, then ignore this vertex
                                ...
                                VecR2 nrm = (cur_pos + move_n * dist_move) - pos;
                                nrm.normalize();
                                if (step && Dot(nrm, hit_nrm_prev) >= EPS_NRM_DOT)
                                    goto ignore_vtx; // if same normal as from previous step, then ignore this vertex
                        ...
                        if (dist_move < hit_len + r)               // if can potentially have smaller distance
                        {
                            ...
                            if (dist_move < hit_len)                // if less than previous distance
                            {
                                if (dist_move <= 0) // object intersecting circle
                                {
                                    ...
                                    Real delta_len2 = delta.length2();
                                    if (!delta_len2)
                                        goto ignore_circle; // if length is zero then cur pos and circle pos are the same so just ignore it, this check is also needed for div by 0
                                                            // if(delta_len2<Sqr(r)) shouldn't be needed at this stage
                                    {
                                        Real delta_len = SqrtFast(delta_len2);
                                        VecR2 nrm = delta / delta_len;
                                        if (step && Dot(nrm, hit_nrm_prev) >= EPS_NRM_DOT)
                                            goto ignore_circle; // if same normal as from previous step, then ignore this circle
