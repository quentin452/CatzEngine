...
void Poly::link(Poly &poly, MemlNode *i, MemlNode *pi) {
    ...
    temp.create(pos, id);
    MemlNode *c;
    for (c = T.vtx.first(); c; c = c->next()) {
        temp.addVtx(T.vtx[c].index);
        if (c == i)
            break;
    }
    for (c = pi; c; c = c->next()) {
        temp.addVtx(poly.vtx[c].index);
    }
    for (c = poly.vtx.first(); c; c = c->next()) {
        temp.addVtx(poly.vtx[c].index);
        if (c == pi)
            break;
    }
...
static Int CompareAngle(C Vec4 &a, C Vec4 &b) {
    if (Int c = Compare(b.z, a.z))
        return c; // compare 'b' against 'a' so generated poly will be in clockwise order
...
void CreateConvex2D(MemPtr<Vec2> poly, C Vec2 *point, Int points) {
    ...
    if (points >= 1) {
        if (points == 1)
            ...
            REP(points - 1) {
                C Vec2 &p = point[i];
                if (p.x < min_value.x || p.x == min_value.x && p.y < min_value.y) {
                    min_value = p;
                    min_index = i;
                }
            } // if same X then compare Y to avoid having last point on the same line as #0 #1 that is useless
            ...
            for (Int i = 2; i < temp.elms(); i++) {
                C Vec2 &p = temp[i].xy;
                for (; poly.elms() >= 2 && Covers(poly[poly.elms() - 2], poly.last(), p);)
                    poly.removeLast();
                poly.add(p);
            }
        }
...
void CreateConvex2Dxz(MemPtr<Vec2> poly, C Vec *point, Int points) {
    ...
    if (points >= 1) {
        if (points == 1)
            ...
            REP(points - 1) {
                C Vec &p = point[i];
                if (p.x < min_value.x || p.x == min_value.x && p.z < min_value.y) {
                    min_value = p.xz();
                    min_index = i;
                }
            } // if same X then compare Y to avoid having last point on the same line as #0 #1 that is useless
                    ...
                    REP(temp.elms() - 2) {
                        ...
                        if (Dot(*normal, tri_nu) > 0) {
                                    ...
                                    goto cuts;
                            {
                                Int t = mesh.tri.elms();
                                mesh.tri._elms++;
                                mesh.tri.ind(t).set(temp[i], temp[i + 1], temp[i + 2]);
                                temp.remove(i + 1, true);
                                added = true;
                            }
                        cuts:;
...
void Triangulate(C CMemPtr<Memc<Vec>> &polys, MeshBase &mesh, Flt weld_pos_eps, Bool convex, C Byte *poly_flags) {
    ...
    // build mesh
    Memt<Int> temp;
    mesh.create(vtxs, 0, tris, 0, poly_flags ? FACE_FLAG : MESH_NONE);
    mesh.tri._elms = 0;
    Vec *pos = mesh.vtx.pos();
    vtxs = 0;
    FREPA(polys) {
        ...
        if (poly.elms() >= 3) {
            ...
            if (poly.elms() == 3) {
                ...
                Int t = mesh.tri.elms();
                mesh.tri._elms++;
                mesh.tri.ind(t).set(vtxs, vtxs + 1, vtxs + 2);
                if (mesh.tri.flag())
                    mesh.tri.flag(t) = flag;
                vtxs += 3;
            } else if (convex) {
                FREP(poly.elms())
                pos[temp.New() = vtxs + i] = poly[i]; // order is important
                REP(temp.elms() - 2) {
                    Int t = mesh.tri.elms();
                    mesh.tri._elms++;
                    mesh.tri.ind(t).set(temp[i], temp[i + 1], temp[i + 2]);
...
void Triangulate(C CMemPtr<Memc<VtxFull>> &polys, MeshBase &mesh, MESH_FLAG flag_and, Flt weld_pos_eps, Bool convex, C Byte *poly_flags) {
    ...
    // build mesh
    Memt<Int> temp;
    mesh.create(vtxs, 0, tris, 0, flag_and & ((VTX_ALL & ~(VTX_FLAG | VTX_DUP)) | (poly_flags ? FACE_FLAG : MESH_NONE)));
    mesh.tri._elms = 0;
    Vec *pos = mesh.vtx.pos();
    vtxs = 0;
    FREPA(polys) {
        ...
        if (poly.elms() >= 3) {
            ...
            if (poly.elms() == 3) {
                ...
                FREP(poly.elms())
                poly[i].to(mesh, temp.New() = vtxs + i); // order is important
                REP(temp.elms() - 2) {
                    Int t = mesh.tri.elms();
                    mesh.tri._elms++;
                    mesh.tri.ind(t).set(temp[i], temp[i + 1], temp[i + 2]);
