...
Rect &Rect::clampFull(C Rect &r) {
    Clamp(min.x, r.min.x, r.max.x);
    Clamp(min.y, r.min.y, r.max.y);
    Clamp(max.x, r.min.x, r.max.x);
    Clamp(max.y, r.min.y, r.max.y);
    return T;
}
...
/******************************************************************************/
Rect &Rect::extendX(Flt e) {
    min.x -= e;
    max.x += e;
    return T;
}
Rect &Rect::extendY(Flt e) {
    min.y -= e;
    max.y += e;
    return T;
}
Rect &Rect::extend(Flt e) {
    min -= e;
    max += e;
    return T;
}
Rect &Rect::extend(C Vec2 &v) {
    min -= v;
    max += v;
    return T;
}

RectD &RectD::extendX(Dbl e) {
    min.x -= e;
    max.x += e;
    return T;
}
RectD &RectD::extendY(Dbl e) {
    min.y -= e;
    max.y += e;
    return T;
}
RectD &RectD::extend(Dbl e) {
    min -= e;
    max += e;
    return T;
}
RectD &RectD::extend(C VecD2 &v) {
    min -= v;
    max += v;
    return T;
}

RectI &RectI::extendX(Int e) {
    min.x -= e;
    max.x += e;
    return T;
}
RectI &RectI::extendY(Int e) {
    min.y -= e;
    max.y += e;
    return T;
}
RectI &RectI::extend(Int e) {
    min -= e;
    max += e;
    return T;
}
RectI &RectI::extend(C VecI2 &v) {
    min -= v;
    max += v;
    return T;
}
...
RectD &RectD::validIncludeX(Dbl x) {
    if (x < min.x)
        min.x = x;
    else if (x > max.x)
        max.x = x;
...
RectI &RectI::validIncludeX(Int x) {
    if (x < min.x)
        min.x = x;
    else if (x > max.x)
        max.x = x;
...
RectD &RectD::validIncludeY(Dbl y) {
    if (y < min.y)
        min.y = y;
    else if (y > max.y)
        max.y = y;
...
RectI &RectI::validIncludeY(Int y) {
    if (y < min.y)
        min.y = y;
    else if (y > max.y)
        max.y = y;
...
Rect &Rect ::includeX(Flt min, Flt max) {
    if (min < T.min.x)
        T.min.x = min;
    if (max > T.max.x)
        T.max.x = max;
...
RectD &RectD::includeX(Dbl min, Dbl max) {
    if (min < T.min.x)
        T.min.x = min;
    if (max > T.max.x)
        T.max.x = max;
...
RectI &RectI::includeX(Int min, Int max) {
    if (min < T.min.x)
        T.min.x = min;
    if (max > T.max.x)
        T.max.x = max;
...
Rect &Rect ::includeY(Flt min, Flt max) {
    if (min < T.min.y)
        T.min.y = min;
    if (max > T.max.y)
        T.max.y = max;
...
RectD &RectD::includeY(Dbl min, Dbl max) {
    if (min < T.min.y)
        T.min.y = min;
    if (max > T.max.y)
        T.max.y = max;
...
RectI &RectI::includeY(Int min, Int max) {
    if (min < T.min.y)
        T.min.y = min;
    if (max > T.max.y)
        T.max.y = max;
...
Rect &Rect ::clampX(Flt min, Flt max) {
    MAX(T.min.x, min);
    MIN(T.max.x, max);
    return T;
}
Rect &Rect ::clampY(Flt min, Flt max) {
    MAX(T.min.y, min);
    MIN(T.max.y, max);
    return T;
}
RectD &RectD::clampX(Dbl min, Dbl max) {
    MAX(T.min.x, min);
    MIN(T.max.x, max);
    return T;
}
RectD &RectD::clampY(Dbl min, Dbl max) {
    MAX(T.min.y, min);
    MIN(T.max.y, max);
    return T;
}
RectI &RectI::clampX(Int min, Int max) {
    MAX(T.min.x, min);
    MIN(T.max.x, max);
    return T;
}
RectI &RectI::clampY(Int min, Int max) {
    MAX(T.min.y, min);
    MIN(T.max.y, max);
    return T;
}
...
/******************************************************************************/
Rect &Rect::moveX(Flt dx) {
    min.x += dx;
    max.x += dx;
    return T;
}
Rect &Rect::moveY(Flt dy) {
    min.y += dy;
    max.y += dy;
    return T;
}
Rect &Rect::mulX(Flt x) {
    min.x *= x;
    max.x *= x;
    return T;
}
Rect &Rect::mulY(Flt y) {
    min.y *= y;
    max.y *= y;
    return T;
}

RectD &RectD::moveX(Dbl dx) {
    min.x += dx;
    max.x += dx;
    return T;
}
RectD &RectD::moveY(Dbl dy) {
    min.y += dy;
    max.y += dy;
    return T;
}
RectD &RectD::mulX(Dbl x) {
    min.x *= x;
    max.x *= x;
    return T;
}
RectD &RectD::mulY(Dbl y) {
    min.y *= y;
    max.y *= y;
    return T;
}
...
Rect &Rect::rotatePI_2(Int rotations) {
    switch (rotations & 3) {
    case 1:
        set(-max.y, min.x, -min.y, max.x);
        break;
    case 2:
        set(-max.x, -max.y, -min.x, -min.y);
        break;
    case 3:
        set(min.y, -max.x, max.y, -min.x);
        break;
    }
    return T;
}
...
Flt Dist(C Vec2 &point, C Rect &rect) {
    if (point.x > rect.max.x) // right
    {
        if (point.y < rect.min.y)
            return Dist(point, rect.rd()); // rd corner
        if (point.y > rect.max.y)
            return Dist(point, rect.max); // ru corner
        return point.x - rect.max.x;      // r  side
    }
    if (point.x < rect.min.x) // left
    {
        if (point.y > rect.max.y)
            return Dist(point, rect.lu()); // lu corner
        if (point.y < rect.min.y)
            return Dist(point, rect.min); // ld corner
        return rect.min.x - point.x;      // l  side
    }
    if (point.y > rect.max.y)
        return point.y - rect.max.y; // u side
    if (point.y < rect.min.y)
        return rect.min.y - point.y; // d side
    return 0;
}
...
Flt Dist2(C Vec2 &point, C Rect &rect) {
    ...
    {
        if (point.y < rect.min.y)
            return Dist2(point, rect.rd()); // rd corner
        if (point.y > rect.max.y)
            return Dist2(point, rect.max); // ru corner
        return Sqr(point.x - rect.max.x);  // r  side
    }
    if (point.x < rect.min.x) // left
    {
        if (point.y > rect.max.y)
            return Dist2(point, rect.lu()); // lu corner
        if (point.y < rect.min.y)
            return Dist2(point, rect.min); // ld corner
        return Sqr(rect.min.x - point.x);  // l  side
    }
    if (point.y > rect.max.y)
        return Sqr(point.y - rect.max.y); // u side
    if (point.y < rect.min.y)
        return Sqr(rect.min.y - point.y); // d side
    return 0;
}
...
Flt Dist2PointSquare(C Vec2 &pos, C Vec2 &square_center, Flt square_radius) {
    return Sqr(Max(Abs(pos.x - square_center.x) - square_radius, 0)) + Sqr(Max(Abs(pos.y - square_center.y) - square_radius, 0));
}
...
Bool Cuts(C Edge2 &edge, C Rect &rect) {
    if (Cuts(edge.p[0], rect) || Cuts(edge.p[1], rect))
        return true;

    if (edge.p[0].x < rect.min.x && edge.p[1].x >= rect.min.x) {
        if (rect.includesY(PointOnPlane(edge.p[0].y, edge.p[1].y, edge.p[0].x - rect.min.x, edge.p[1].x - rect.min.x)))
            return true;
    } else if (edge.p[0].x > rect.max.x && edge.p[1].x <= rect.max.x) {
        if (rect.includesY(PointOnPlane(edge.p[0].y, edge.p[1].y, edge.p[0].x - rect.max.x, edge.p[1].x - rect.max.x)))
            return true;
    }
    if (edge.p[0].y < rect.min.y && edge.p[1].y >= rect.min.y) {
        if (rect.includesX(PointOnPlane(edge.p[0].x, edge.p[1].x, edge.p[0].y - rect.min.y, edge.p[1].y - rect.min.y)))
            return true;
    } else if (edge.p[0].y > rect.max.y && edge.p[1].y <= rect.max.y) {
        if (rect.includesX(PointOnPlane(edge.p[0].x, edge.p[1].x, edge.p[0].y - rect.max.y, edge.p[1].y - rect.max.y)))
            return true;
    }
...
Bool CutsEps(C Vec2 &point, C Rect &rect) {
    return point.x >= rect.min.x - EPS && point.x <= rect.max.x + EPS && point.y >= rect.min.y - EPS && point.y <= rect.max.y + EPS;
}
/******************************************************************************/
...
Bool InsideEps(C Rect &a, C Rect &b) {
    return a.min.x >= b.min.x - EPS && a.max.x <= b.max.x + EPS && a.min.y >= b.min.y - EPS && a.max.y <= b.max.y + EPS;
}
/******************************************************************************/
...
Rect Fit(Flt src_aspect, C Rect &dest_rect, FIT_MODE fit) {
    ...
    Rect r = dest_rect;
    Bool mx = (r.min.x > r.max.x);
    if (mx)
        r.swapX();
    Flt w = r.w();
    Bool my = (r.min.y > r.max.y);
    if (my)
        r.swapY();
    Flt h = r.h(), dest_aspect = w / h;
...
Bool SweepPointRect(C Vec2 &point, C Vec2 &move, C Rect &rect, Flt *hit_frac, Vec2 *hit_normal, Vec2 *hit_pos) {
    ...
    Flt frac;
    Vec2 test;
    if (point.x < rect.min.x && point.x + move.x >= rect.min.x) {
        frac = (rect.min.x - point.x) / move.x;
        test.set(rect.min.x, point.y + move.y * frac);
        if (rect.includesY(test.y)) {
            if (hit_frac)
                *hit_frac = frac;
            if (hit_normal)
                hit_normal->set(-1, 0);
    ...
    } else if (point.x > rect.max.x && point.x + move.x <= rect.max.x) {
        frac = (rect.max.x - point.x) / move.x;
        test.set(rect.max.x, point.y + move.y * frac);
        if (rect.includesY(test.y)) {
            if (hit_frac)
                *hit_frac = frac;
            if (hit_normal)
    ...
    if (point.y < rect.min.y && point.y + move.y >= rect.min.y) {
        frac = (rect.min.y - point.y) / move.y;
        test.set(point.x + move.x * frac, rect.min.y);
        if (rect.includesX(test.x)) {
            if (hit_frac)
                *hit_frac = frac;
            if (hit_normal)
                hit_normal->set(0, -1);
    ...
    } else if (point.y > rect.max.y && point.y + move.y <= rect.max.y) {
        frac = (rect.max.y - point.y) / move.y;
        test.set(point.x + move.x * frac, rect.max.y);
        if (rect.includesX(test.x)) {
            if (hit_frac)
                *hit_frac = frac;
            if (hit_normal)
...
Int Compare(C Rect &a, C Rect &b) {
    if (Int c = Compare(a.min.x, b.min.x))
        return c;
    if (Int c = Compare(a.min.y, b.min.y))
        return c;
    if (Int c = Compare(a.max.x, b.max.x))
        return c;
    if (Int c = Compare(a.max.y, b.max.y))
        return c;
    return 0;
}
...
Bool PackRectsKnownLimit(C CMemPtr<RectSizeAnchor> &sizes, MemPtr<RectI> rects, C VecI2 &limit, Bool allow_rotate, Int border, Bool align_for_compression, Bool compact_arrangement) {
    ...
    if (align_for_compression)
        border += 3; // when using 'align_for_compression' we need to have at least 3 extra pixels to align the target

    Memc<RP::RectSizeIndex> rp_src;
    Memc<RP::RectIndex> rp_dest;
    rects.setNum(sizes.elms());

    if (!compact_arrangement) { // try "shelf" first, because it's fastest
        RP::ShelfBinPack pack(limit.x, limit.y, false, allow_rotate);
        FREPA(sizes) {
            C RectSizeAnchor &rsa = sizes[i];
            if (rsa.size.x > 0 && rsa.size.y > 0) {
                RP::Rect rect = pack.Insert(rsa.size.x + border, rsa.size.y + border, RP::ShelfBinPack::ShelfNextFit);
                if (!rect.width)
                    goto max_rects;
                Bool flipped = ((rect.width > rect.height) != (rsa.size.x > rsa.size.y));
                if (align_for_compression) {
                    VecI2 anchor = rsa.anchor;
                    anchor.x += rect.x;
                    anchor.y += rect.y; // final anchor position
                    rect.x += Ceil4(anchor.x) - anchor.x;
                    rect.y += Ceil4(anchor.y) - anchor.y; // offset rectangle so the anchor is 4-pixel aligned
                }
                rects[i].setLD(rect.x, rect.y, rsa.size.c[flipped], rsa.size.c[!flipped]);
            } else if (!rsa.size.x && !rsa.size.y)
                rects[i].zero();
            else
                return false;
        }
...
Bool PackRectsUnknownLimit(C CMemPtr<RectSizeAnchor> &sizes, MemPtr<RectI> rects, VecI2 &limit, Bool allow_rotate, Int border, Bool align_for_compression, Bool compact_arrangement, Bool only_square, Int max_size) {
    ...
    for (Int dim = FloorPow2(SqrtI(area)); dim <= max_size; dim <<= 1)
        for (Int aspect = (only_square ? 4 : 0); aspect < 5; aspect++) {
            ...
            case 0:
                limit.x >>= 2;
                break; // at 1st attempt try making (limit/4, limit  ), as textures should preferrably be taller than wider
            case 1:
                limit.y >>= 2;
                break; // at 2nd attempt try making (limit  , limit/4)
            case 2:
                limit.x >>= 1;
                break; // at 3rd attempt try making (limit/2, limit  ), as textures should preferrably be taller than wider
            case 3:
                limit.y >>= 1;
                break; // at 4th attempt try making (limit  , limit/2)
                       // at 5th attempt try making (limit  , limit  )
            }
            if (PackRectsKnownLimit(sizes, rects, limit, allow_rotate, border, align_for_compression, compact_arrangement))
                return true;
        }
    return false;
}
...
Bool PackRectsMultiLimit(C CMemPtr<RectSizeAnchor> &sizes, MemPtr<RectIndex> rects, MemPtr<VecI2> limits, Bool allow_rotate, Int border, Bool align_for_compression, Bool compact_arrangement, Bool only_square, Int max_size) {
    ...
    for (; left_all.elms();) {
        left_best.clear();
        placed_best.clear();
        ULong area_best = 0, area = 0;
        REPA(left_all) {
            C RP::RectSizeIndex &l = left_all[i];
            area += ULong(l.width) * ULong(l.height);
        }
        VecI2 limit_best = 0;
        for (Int dim = Min(FloorPow2(SqrtI(area)), max_size); dim <= max_size; dim <<= 1)
            for (Int aspect = (only_square ? 2 : 0); aspect < 3; aspect++) {
                ...
                case 0:
                    limit.x >>= 1;
                    break; // at 1st attempt try making (limit/2, limit  ), as textures should preferrably be taller than wider
                case 1:
                    limit.y >>= 1;
                    break; // at 2nd attempt try making (limit  , limit/2), in case this fits the parts but above method doesn't
                ...
                if (!compact_arrangement) {
                    ...
                    area = 0;
                    REPA(placed) {
                        C RectIndex &r = placed[i];
                        area += ULong(r.w()) * ULong(r.h());
                    } // check if it's best (covers most area)
                    if (area > area_best) {
                        area_best = area;
                        limit_best = limit;
                        Swap(left, left_best);
        ...
        if (!placed_best.elms())
            return false; // if no elements were placed in this step then fail
...
Bool BestFit(Vec2 *point, Int points, Vec2 &axis) {
    Memt<Vec2> convex_points;
    CreateConvex2D(convex_points, point, points);
    if (convex_points.elms() > 1) {
        ...
        REPA(convex_points) {
            ...
            if (dir.normalize()) {
                Vec2 perp = Perp(dir);
                Rect r;
                r.setX(Dot(last, dir)).setY(Dot(last, perp));
