...
Flt Dist(C Vec &point, C Torus &torus) {
    ...
    Flt dist;
    if (nearest.setLength(torus.R))
        dist = Dist(delta, nearest);
    else
        dist = Dist(torus.R, dist_point_plane); // point is located exactly on the axis of rotation (pos=torus.pos, dir=torus.up)
