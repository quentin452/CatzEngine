...
/******************************************************************************/
Tri2 ::Tri2(C TriD2 &tri) {
    p[0] = tri.p[0];
    p[1] = tri.p[1];
    p[2] = tri.p[2];
}
TriD2::TriD2(C Tri2 &tri) {
    p[0] = tri.p[0];
    p[1] = tri.p[1];
    p[2] = tri.p[2];
}
...
TriND::TriND(C TriD &tri) {
    p[0] = tri.p[0];
    p[1] = tri.p[1];
    p[2] = tri.p[2];
    setNormal();
}
TriND::TriND(C Tri &tri) {
    p[0] = tri.p[0];
    p[1] = tri.p[1];
    p[2] = tri.p[2];
    setNormal();
} // recalculate for high precision instead of using 'tri.n'

Tri2 &Tri2 ::operator+=(C Vec2 &v) {
    p[0] += v;
    p[1] += v;
    p[2] += v;
    return T;
...
TriD2 &TriD2::operator+=(C VecD2 &v) {
    p[0] += v;
    p[1] += v;
    p[2] += v;
    return T;
...
Bool Tri2::valid(Flt eps) C {
    return DistPointLine(p[0], p[1], !(p[2] - p[1])) > eps && DistPointLine(p[1], p[2], !(p[0] - p[2])) > eps && DistPointLine(p[2], p[0], !(p[1] - p[0])) > eps;
}
...
Bool Tri::coplanar(C TriN &tri) C {
    return Abs(DistPointPlane(p[0], tri)) <= EPS && Abs(DistPointPlane(p[1], tri)) <= EPS && Abs(DistPointPlane(p[2], tri)) <= EPS;
}
Bool TriN::coplanar(C Tri &tri) C {
    return Abs(DistPointPlane(tri.p[0], T)) <= EPS && Abs(DistPointPlane(tri.p[1], T)) <= EPS && Abs(DistPointPlane(tri.p[2], T)) <= EPS;
}
Bool TriD::coplanar(C TriND &tri) C {
    return Abs(DistPointPlane(p[0], tri)) <= EPSD && Abs(DistPointPlane(p[1], tri)) <= EPSD && Abs(DistPointPlane(p[2], tri)) <= EPSD;
}
Bool TriND::coplanar(C TriD &tri) C {
    return Abs(DistPointPlane(tri.p[0], T)) <= EPSD && Abs(DistPointPlane(tri.p[1], T)) <= EPSD && Abs(DistPointPlane(tri.p[2], T)) <= EPSD;
}
/******************************************************************************/
...
Flt TriABAngle(Flt a_length, Flt b_length, Flt c_length) {
    if (Flt div = 2 * a_length * b_length)
        return Acos((Sqr(a_length) + Sqr(b_length) - Sqr(c_length)) / div);
    return 0;
}
/******************************************************************************/
Vec TriBlend(C Vec2 &p, C Tri2 &tri) {
    ...
    Vec blend;
    if (Solve(tri.p[0].x - tri.p[2].x, tri.p[0].y - tri.p[2].y, tri.p[1].x - tri.p[2].x, tri.p[1].y - tri.p[2].y, p.x - tri.p[2].x, p.y - tri.p[2].y, blend.x, blend.y) != 1)
        blend.x = blend.y = 0;
...
Vec TriBlend(C Vec &p, C TriN &tri, Bool pos_on_tri_plane) {
    if (!pos_on_tri_plane)
        return TriBlend(PointOnPlane(p, tri.p[0], tri.n), tri, true);
    ...
    if (size.z <= size.x && size.z <= size.y) // if Z dimension is smallest
    {
        ...
        if (Solve(tri.p[0].x - tri.p[2].x, tri.p[0].y - tri.p[2].y, tri.p[1].x - tri.p[2].x, tri.p[1].y - tri.p[2].y, p.x - tri.p[2].x, p.y - tri.p[2].y, blend.x, blend.y) != 1)
            blend.x = blend.y = 0;
    } else if (size.y <= size.x && size.y <= size.z) // if Y dimension is smallest
    {
        // use XZ plane
        if (Solve(tri.p[0].x - tri.p[2].x, tri.p[0].z - tri.p[2].z, tri.p[1].x - tri.p[2].x, tri.p[1].z - tri.p[2].z, p.x - tri.p[2].x, p.z - tri.p[2].z, blend.x, blend.y) != 1)
            blend.x = blend.y = 0;
    } else {
        // use YZ plane
        if (Solve(tri.p[0].y - tri.p[2].y, tri.p[0].z - tri.p[2].z, tri.p[1].y - tri.p[2].y, tri.p[1].z - tri.p[2].z, p.y - tri.p[2].y, p.z - tri.p[2].z, blend.x, blend.y) != 1)
            blend.x = blend.y = 0;
    }
    blend.z = 1 - blend.x - blend.y;
    return blend;
...
Vec4 TetraBlend(C Vec &p, C Vec &p0, C Vec &p1, C Vec &p2, C Vec &p3) {
    ...
    m.x = q;
    blend.x = m.determinant();
    m.x = q0;
    m.y = q;
    blend.y = m.determinant();
    m.y = q1;
    m.z = q;
    blend.z = m.determinant();
...
Flt Dist(C Vec2 &point, C Tri2 &tri, DIST_TYPE *_type) {
    ...
    Vec2 p1 = point - tri.p[1];
    if (Xor(DistPointPlane(p1, Perp(tri.p[0] - tri.p[1])), sign) > 0) {
        d = Dist2PointEdge(point, tri.p[0], tri.p[1], &t);
        if (!type || d < dist) {
            dist = d;
            type = UpdateTypeT(t, DIST_POINT0, DIST_POINT1, DIST_EDGE0);
        }
    }
    if (Xor(DistPointPlane(p1, Perp(tri.p[1] - tri.p[2])), sign) > 0) {
        d = Dist2PointEdge(point, tri.p[1], tri.p[2], &t);
        if (!type || d < dist) {
            dist = d;
            type = UpdateTypeT(t, DIST_POINT1, DIST_POINT2, DIST_EDGE1);
        }
    }
    if (Xor(DistPointPlane(point, tri.p[2], Perp(tri.p[2] - tri.p[0])), sign) > 0) {
        d = Dist2PointEdge(point, tri.p[2], tri.p[0], &t);
        if (!type || d < dist) {
            dist = d;
            type = UpdateTypeT(t, DIST_POINT2, DIST_POINT0, DIST_EDGE2);
        }
    }
    if (!type) {
        type = DIST_PLANE;
...
Flt Dist(C Vec &point, C Tri &tri, DIST_TYPE *_type) {
    ...
    Vec p1 = point - tri.p[1];
    if (DistPointPlane(p1, Cross(tri_n, tri.p[0] - tri.p[1])) > 0) {
        d = Dist2PointEdge(point, tri.p[0], tri.p[1], &t);
        if (!type || d < dist) {
            dist = d;
            type = UpdateTypeT(t, DIST_POINT0, DIST_POINT1, DIST_EDGE0);
    ...
    if (DistPointPlane(p1, Cross(tri_n, tri.p[1] - tri.p[2])) > 0) {
        d = Dist2PointEdge(point, tri.p[1], tri.p[2], &t);
        if (!type || d < dist) {
            dist = d;
            type = UpdateTypeT(t, DIST_POINT1, DIST_POINT2, DIST_EDGE1);
    ...
    if (DistPointPlane(point, tri.p[2], Cross(tri_n, tri.p[2] - tri.p[0])) > 0) {
        d = Dist2PointEdge(point, tri.p[2], tri.p[0], &t);
        if (!type || d < dist) {
            dist = d;
            type = UpdateTypeT(t, DIST_POINT2, DIST_POINT0, DIST_EDGE2);
...
Flt Dist(C Vec &point, C TriN &tri, DIST_TYPE *_type) {
    ...
    Vec p1 = point - tri.p[1];
    if (DistPointPlane(p1, Cross(tri.n, tri.p[0] - tri.p[1])) > 0) {
        d = Dist2PointEdge(point, tri.p[0], tri.p[1], &t);
        if (!type || d < dist) {
            dist = d;
            type = UpdateTypeT(t, DIST_POINT0, DIST_POINT1, DIST_EDGE0);
    ...
    if (DistPointPlane(p1, Cross(tri.n, tri.p[1] - tri.p[2])) > 0) {
        d = Dist2PointEdge(point, tri.p[1], tri.p[2], &t);
        if (!type || d < dist) {
            dist = d;
            type = UpdateTypeT(t, DIST_POINT1, DIST_POINT2, DIST_EDGE1);
    ...
    if (DistPointPlane(point, tri.p[2], Cross(tri.n, tri.p[2] - tri.p[0])) > 0) {
        d = Dist2PointEdge(point, tri.p[2], tri.p[0], &t);
        if (!type || d < dist) {
            dist = d;
            type = UpdateTypeT(t, DIST_POINT2, DIST_POINT0, DIST_EDGE2);
...
Bool Cuts(C Vec &point, C Tri &tri, C Vec (&tri_cross)[3]) {
    Vec p1 = point - tri.p[1];
    if (DistPointPlane(p1, tri_cross[0]) > 0)
        return false;
    if (DistPointPlane(p1, tri_cross[1]) > 0)
        return false;
    if (DistPointPlane(point, tri.p[2], tri_cross[2]) > 0)
        return false;
    return true;
}
/******************************************************************************/
...
Bool Cuts(C Vec &point, C Tri &tri) {
    ...
    Vec p1 = point - tri.p[1];
    if (DistPointPlane(p1, Cross(tri_nu, tri.p[0] - tri.p[1])) > 0)
        return false;
    if (DistPointPlane(p1, Cross(tri_nu, tri.p[1] - tri.p[2])) > 0)
        return false;
    if (DistPointPlane(point, tri.p[2], Cross(tri_nu, tri.p[2] - tri.p[0])) > 0)
        return false;
    return true;
}
...
Bool CutsEps(C Vec2 &point, C Tri2 &tri) {
    UInt sign = (tri.clockwise() ? 0 : SIGN_BIT); // counter-clockwise tris require changing sign
    Vec2 d10 = !(tri.p[0] - tri.p[1]), p0 = point - tri.p[0];
    if (Xor(DistPointPlane(p0, Perp(d10)), sign) > EPS)
        return false;
    Vec2 d21 = !(tri.p[1] - tri.p[2]), p1 = point - tri.p[1];
    if (Xor(DistPointPlane(p1, Perp(d21)), sign) > EPS)
        return false;
    Vec2 d02 = !(tri.p[2] - tri.p[0]), p2 = point - tri.p[2];
    if (Xor(DistPointPlane(p2, Perp(d02)), sign) > EPS)
        return false;
    if (DistPointPlane(p0, !(d10 - d02)) > EPS)
        return false;
    if (DistPointPlane(p1, !(d21 - d10)) > EPS)
        return false;
    if (DistPointPlane(p2, !(d02 - d21)) > EPS)
        return false;
    return true;
}
Bool CutsEps(C VecD2 &point, C TriD2 &tri) {
    UInt sign = (tri.clockwise() ? 0 : SIGN_BIT); // counter-clockwise tris require changing sign
    VecD2 d10 = !(tri.p[0] - tri.p[1]), p0 = point - tri.p[0];
    if (Xor(DistPointPlane(p0, Perp(d10)), sign) > EPSD)
        return false;
    VecD2 d21 = !(tri.p[1] - tri.p[2]), p1 = point - tri.p[1];
    if (Xor(DistPointPlane(p1, Perp(d21)), sign) > EPSD)
        return false;
    VecD2 d02 = !(tri.p[2] - tri.p[0]), p2 = point - tri.p[2];
    if (Xor(DistPointPlane(p2, Perp(d02)), sign) > EPSD)
        return false;
    if (DistPointPlane(p0, !(d10 - d02)) > EPSD)
        return false;
    if (DistPointPlane(p1, !(d21 - d10)) > EPSD)
        return false;
    if (DistPointPlane(p2, !(d02 - d21)) > EPSD)
        return false;
    return true;
}
Bool CutsEps(C Vec &point, C TriN &tri) {
    Vec d10 = !(tri.p[0] - tri.p[1]), p0 = point - tri.p[0];
    if (DistPointPlane(p0, Cross(tri.n, d10)) > EPS)
        return false;
    Vec d21 = !(tri.p[1] - tri.p[2]), p1 = point - tri.p[1];
    if (DistPointPlane(p1, Cross(tri.n, d21)) > EPS)
        return false;
    Vec d02 = !(tri.p[2] - tri.p[0]), p2 = point - tri.p[2];
    if (DistPointPlane(p2, Cross(tri.n, d02)) > EPS)
        return false;
    if (DistPointPlane(p0, !(d10 - d02)) > EPS)
        return false;
    if (DistPointPlane(p1, !(d21 - d10)) > EPS)
...
Bool CutsEps(C VecD &point, C TriND &tri) {
    VecD d10 = !(tri.p[0] - tri.p[1]), p0 = point - tri.p[0];
    if (DistPointPlane(p0, Cross(tri.n, d10)) > EPSD)
        return false;
    VecD d21 = !(tri.p[1] - tri.p[2]), p1 = point - tri.p[1];
    if (DistPointPlane(p1, Cross(tri.n, d21)) > EPSD)
        return false;
    VecD d02 = !(tri.p[2] - tri.p[0]), p2 = point - tri.p[2];
    if (DistPointPlane(p2, Cross(tri.n, d02)) > EPSD)
        return false;
    if (DistPointPlane(p0, !(d10 - d02)) > EPSD)
        return false;
    if (DistPointPlane(p1, !(d21 - d10)) > EPSD)
...
Int CutsTriPlane(C Tri &tri, C Plane &plane, Edge &edge) {
    ...
    if (!(s0 + s1))
        edge.p[i++] = PointOnPlane(tri.p[0], tri.p[1], d0, d1);
    if (!(s1 + s2))
        edge.p[i++] = PointOnPlane(tri.p[1], tri.p[2], d1, d2);
    if (!(s2 + s0))
        edge.p[i++] = PointOnPlane(tri.p[2], tri.p[0], d2, d0);
    return 2;
}
...
Int Clip(Edge2 &edge, C Tri2 &tri) {
    ...
    REPD(e, 3) {
        Edge2 cuts;
        if (Int c = CutsEdgeEdge(Edge2_I(tri.p[e], tri.p[(e + 1) % 3]), Edge2_I(edge.p[0], edge.p[1]), &cuts)) {
            if (c == 2) {
                if (++edge_n[0] > 2 || ++edge_n[1] > 2)
                    return 0;
                ...
                Int dup;
                if (Equal(cuts.p[0], edge.p[0]))
                    dup = 0;
                else if (Equal(cuts.p[0], edge.p[1]))
                    dup = 1;
                else
                    dup = -1;
                if (dup != 0 && Xor(DistPointPlane(edge.p[0], tri.p[e], tri_n[e]), sign) < 0)
                    dup = 1;
                else if (dup != 1 && Xor(DistPointPlane(edge.p[1], tri.p[e], tri_n[e]), sign) < 0)
                    dup = 0;
                else {
                    edge.p[0] = cuts.p[0];
                    return 1;
    ...
    if (edge_n[0] || edge_n[1])
        return 2;
    return (Xor(DistPointPlane(edge.p[0], tri.p[0], tri_n[0]), sign) < 0 && Xor(DistPointPlane(edge.p[0], tri.p[1], tri_n[1]), sign) < 0 && Xor(DistPointPlane(edge.p[0], tri.p[2], tri_n[2]), sign) < 0) ? 2 : 0;
}
Int Clip(EdgeD2 &edge, C TriD2 &tri) {
    ...
    REPD(e, 3) {
        EdgeD2 cuts;
        if (Int c = CutsEdgeEdge(EdgeD2_I(tri.p[e], tri.p[(e + 1) % 3]), EdgeD2_I(edge.p[0], edge.p[1]), &cuts)) {
            if (c == 2) {
                if (++edge_n[0] > 2 || ++edge_n[1] > 2)
                    return 0;
                Int i = Closer(edge.p[0], cuts.p[0], cuts.p[1]);
