...
void Tube::drawVI(Bool fill, Int resolution) C {
    ...
    REP(resolution) {
        ...
        Vec next = pos + c * matrix.x + s * matrix.z;
        if (fill)
            VI.quad(next + matrix.y, prev + matrix.y, prev - matrix.y, next - matrix.y);
        else {
...
Bool Cuts(C Vec &point, C Tube &tube) {
    ...
    if (dist_y >= -h_2 && dist_y <= h_2)
        return DistPointLine(point, tube.pos, tube.up) <= tube.r;
    return false;
}
Bool Cuts(C Edge &edge, C Tube &tube) {
    ...
    if (dist_y0 > dist_y1) {
        temp.reverse();
        Swap(dist_y0, dist_y1);
    }

    if (dist_y0 <= h_2 && dist_y1 >= -h_2) {
...
Bool SweepPointTube(C Vec &point, C Vec &move, C Tube &tube, Flt *hit_frac, Vec *hit_normal) {
    ...
    Plane plane;
    Flt h_2 = tube.h * 0.5f;
    plane.pos = tube.pos + h_2 * tube.up;
    plane.normal = tube.up;
    if (SweepPointPlane(point, move, plane, hit_frac, hit_normal, &pos) && Dist(pos, plane.pos) <= tube.r)
        return true;
    plane.pos = tube.pos - h_2 * tube.up;
    plane.normal = -tube.up;
    if (SweepPointPlane(point, move, plane, hit_frac, hit_normal, &pos) && Dist(pos, plane.pos) <= tube.r)
        return true;
    Matrix matrix;
    matrix.setPosDir(tube.pos, tube.up);
    ...
    if (SweepPointCircle(point2, move2, Circle(tube.r), &frac, &nrm2)) {
        ...
        if (Abs(y) <= h_2) {
            if (hit_frac)
                *hit_frac = frac;
            if (hit_normal)
                *hit_normal = matrix.orn().convert(nrm2);
            return true;
        }
    }
    return false;
