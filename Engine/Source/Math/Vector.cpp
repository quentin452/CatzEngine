...
VecD Perp(C VecD &v) {
    if (Abs(v.x) < Abs(v.z))
        return VecD(0, v.z, -v.y); // Cross(v, VecD(1, 0,  0));
    else
        return VecD(-v.y, v.x, 0); // Cross(v, VecD(0, 0, -1));
}
...
INLINE Bool _EqualWrap(Flt r0, Flt r1) {
    Flt d = Frac(r0 - r1);
...
Bool Equal(C Vec2 &v0, C Vec2 &v1) {
    return _Equal(v0.x, v1.x) && _Equal(v0.y, v1.y);
}
Bool Equal(C Vec &v0, C Vec &v1) {
    return _Equal(v0.x, v1.x) && _Equal(v0.y, v1.y) && _Equal(v0.z, v1.z);
}
Bool Equal(C Vec4 &v0, C Vec4 &v1) {
    return _Equal(v0.x, v1.x) && _Equal(v0.y, v1.y) && _Equal(v0.z, v1.z) && _Equal(v0.w, v1.w);
}
/******************************************************************************/
...
Bool Equal(C Vec2 &v0, C Vec2 &v1, Flt epsilon) {
    return _Equal(v0.x, v1.x, epsilon) && _Equal(v0.y, v1.y, epsilon);
}
Bool Equal(C VecD2 &v0, C VecD2 &v1, Dbl epsilon) {
    return _Equal(v0.x, v1.x, epsilon) && _Equal(v0.y, v1.y, epsilon);
}
Bool Equal(C Vec &v0, C Vec &v1, Flt epsilon) {
    return _Equal(v0.x, v1.x, epsilon) && _Equal(v0.y, v1.y, epsilon) && _Equal(v0.z, v1.z, epsilon);
}
Bool Equal(C VecD &v0, C VecD &v1, Dbl epsilon) {
    return _Equal(v0.x, v1.x, epsilon) && _Equal(v0.y, v1.y, epsilon) && _Equal(v0.z, v1.z, epsilon);
}
Bool Equal(C Vec4 &v0, C Vec4 &v1, Flt epsilon) {
    return _Equal(v0.x, v1.x, epsilon) && _Equal(v0.y, v1.y, epsilon) && _Equal(v0.z, v1.z, epsilon) && _Equal(v0.w, v1.w, epsilon);
}
...
Bool EqualWrap(C Vec2 &v0, C Vec2 &v1) {
    return _EqualWrap(v0.x, v1.x) && _EqualWrap(v0.y, v1.y);
}
Bool EqualWrap(C Vec &v0, C Vec &v1) {
    return _EqualWrap(v0.x, v1.x) && _EqualWrap(v0.y, v1.y) && _EqualWrap(v0.z, v1.z);
}
/******************************************************************************/
Int Compare(C VecH2 &v0, C VecH2 &v1) {
    if (Int c = Compare(v0.x, v1.x))
        return c;
    if (Int c = Compare(v0.y, v1.y))
        return c;
    return 0;
}
...
Int Compare(C Color &c0, C Color &c1) {
    if (Int c = Compare(c0.r, c1.r))
        return c;
    if (Int c = Compare(c0.g, c1.g))
        return c;
    if (Int c = Compare(c0.b, c1.b))
        return c;
    if (Int c = Compare(c0.a, c1.a))
        return c;
    return 0;
}
/******************************************************************************/
...
Dbl VecD2::setLength(Dbl length) {
    if (Dbl l = T.length2()) {
        l = SqrtFast(l);
        T *= length / l;
        return l;
...
/******************************************************************************/
Vec2 &Vec2 ::clipLength(Flt max_length) {
    Flt l = length2();
    if (l > Sqr(max_length))
        T *= max_length / SqrtFast(l);
    return T;
}
VecD2 &VecD2::clipLength(Dbl max_length) {
    Dbl l = length2();
    if (l > Sqr(max_length))
        T *= max_length / SqrtFast(l);
    return T;
}
Vec &Vec ::clipLength(Flt max_length) {
    Flt l = length2();
    if (l > Sqr(max_length))
        T *= max_length / SqrtFast(l);
    return T;
}
VecD &VecD ::clipLength(Dbl max_length) {
    Dbl l = length2();
    if (l > Sqr(max_length))
        T *= max_length / SqrtFast(l);
    return T;
...
Vec &Vec::operator&=(C OBox &b) {
    ...
    Flt d = Dot(r, b.matrix.x);
    if (d < b.box.min.x)
        T += b.matrix.x * (b.box.min.x - d);
    else if (d > b.box.max.x)
        T -= b.matrix.x * (d - b.box.max.x);
    d = Dot(r, b.matrix.y);
    if (d < b.box.min.y)
        T += b.matrix.y * (b.box.min.y - d);
    else if (d > b.box.max.y)
        T -= b.matrix.y * (d - b.box.max.y);
    d = Dot(r, b.matrix.z);
    if (d < b.box.min.z)
        T += b.matrix.z * (b.box.min.z - d);
    else if (d > b.box.max.z)
        T -= b.matrix.z * (d - b.box.max.z);
    return T;
}
...
Bool VecD4::operator!=(C VecD4 &v) C { return x != v.x || y != v.y || z != v.z || w != v.w; }
/******************************************************************************/
Str VecH2::asText(Int precision) C {
    if (precision == INT_MAX)
        precision = PRECISION_HALF;
    Char8 temp[256];
    Str s = TextReal(x, temp, precision);
    s += ", ";
    s += TextReal(y, temp, precision);
    return s;
}
Str Vec2 ::asText(Int precision) C {
    if (precision == INT_MAX)
        precision = PRECISION_FLT;
    Char8 temp[256];
    Str s = TextReal(x, temp, precision);
    s += ", ";
    s += TextReal(y, temp, precision);
...
Str VecD2::asText(Int precision) C {
    if (precision == INT_MAX)
        precision = PRECISION_DBL;
    Char8 temp[256];
    Str s = TextReal(x, temp, precision);
    s += ", ";
    s += TextReal(y, temp, precision);
...
Str VecH ::asText(Int precision) C {
    if (precision == INT_MAX)
        precision = PRECISION_HALF;
    Char8 temp[256];
    Str s = TextReal(x, temp, precision);
    ...
    s += TextReal(y, temp, precision);
    s += ", ";
    s += TextReal(z, temp, precision);
    return s;
}
Str Vec ::asText(Int precision) C {
    if (precision == INT_MAX)
        precision = PRECISION_FLT;
    Char8 temp[256];
    Str s = TextReal(x, temp, precision);
...
Str VecD ::asText(Int precision) C {
    if (precision == INT_MAX)
        precision = PRECISION_DBL;
    Char8 temp[256];
    Str s = TextReal(x, temp, precision);
...
Str VecH4::asText(Int precision) C {
    if (precision == INT_MAX)
        precision = PRECISION_HALF;
    Char8 temp[256];
    Str s = TextReal(x, temp, precision);
    ...
    s += TextReal(z, temp, precision);
    s += ", ";
    s += TextReal(w, temp, precision);
    return s;
}
Str Vec4 ::asText(Int precision) C {
    if (precision == INT_MAX)
        precision = PRECISION_FLT;
    Char8 temp[256];
    Str s = TextReal(x, temp, precision);
...
Str VecD4::asText(Int precision) C {
    if (precision == INT_MAX)
        precision = PRECISION_DBL;
    Char8 temp[256];
    Str s = TextReal(x, temp, precision);
...
void Vec::draw(C Color &color, Flt r) C {
    Vec2 screen;
    if (PosToScreenM(T, screen))
        screen.draw(color, r);
}
void Vec::drawP(C Color &color, Flt r) C {
    Vec2 screen;
    if (PosToScreenM(T, screen))
        screen.draw(color, r / DistPointActiveCamPlaneZ(T * ObjMatrix));
}
