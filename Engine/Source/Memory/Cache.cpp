...
_Cache::_Cache(CChar8 *name, Int block_elms) : _memx(block_elms) {
    ...
    _delay_remove_counter = 0;
    _delay_remove_time = 0;
    _delay_remove_check = 0;
}
void _Cache::clear() {
    ...
    _memx.clear();
    _delay_remove.clear();
} // here can't Free '_order' because it assumes that its size is '_memx.maxElms()'
void _Cache::del() {
    SyncUnlocker unlocker(D._lock);
    SyncLocker locker(_lock);
    _elms = 0;
...
Byte _Cache::mode(Byte mode) {
    auto old_mode = T._mode;
    T._mode = mode;
    return old_mode;
}
void _Cache::caseSensitive(Bool sensitive) { T._case_sensitive = sensitive; }
void _Cache::setLoadUser(Bool (*load_user)(Ptr data, C Str &file, Ptr user), Ptr user) {
    T._user = user;
    T._load = null;
    T._load_user = load_user;
}
void _Cache::delayRemove(Flt time) {
    ...
    if (time != _delay_remove_time) {
        ...
        if (adjust_existing)
            REPAO(_delay_remove).time += delta; // adjust element removal times
        update();
    }
}
void _Cache::reserve(Int num) {
    auto max_elms = _memx.maxElms();
    _memx.reserve(num);
    if (max_elms != _memx.maxElms())
        Realloc(_order, _memx.maxElms(), max_elms);
}
/******************************************************************************/
void _Cache::lock() C {
    Byte d_locked = 0;
    if (!_lock.owned()) // if we don't have the '_lock' yet
    {
        ...
        if (D._lock.owned()) {
            d_locked = true;
            D._lock.off();
        } // if we own the 'D._lock' then disable it and remember that we had it
#else
        for (; D._lock.owned(); d_locked++)
            D._lock.off(); // if we own the 'D._lock' then disable it and remember that we had it
#endif
    }
    _lock.on();
    if (d_locked)
        _d_lock = d_locked; // if we've disabled 'D._lock' then set how many times
}
...
_Cache::Elm *_Cache::findElm(CChar *file, CChar *path) C {
    ...
    if (Is(path) && !FullPath(file)) // try using "path+file" (precisely "SkipStartPath(path, DataPath()) + file"), this needs to be done first, in case there's a "file.dat" and "folder/file.dat" loaded, but we're looking for "file.dat" with "path=folder", this is also needed when loading sub-assets for a file at its location when no 'DataPath' was specified, for example, loading a material for a mesh: "file=material", path="c:/project/data"
    {
        ...
        {
            Char path_file[MAX_LONG_PATH];
            MergePath(path_file, rel_path, file);
            if (elm = findExact(path_file, stop))
                return elm;
        }
    }
    if (elm = findExact(file, stop))
        return elm; // try using 'file'
    if (DataPath().is())
        if (CChar *after = _AfterPath(file, DataPath()))
            return findExact(after, stop); // try using 'file' without the 'DataPath' if it was specified
    return null;
}
/******************************************************************************/
...
void _Cache::addToOrder(Elm &elm) {
    Int stop;
    findExact(elmDesc(elm).file, stop);
...
void _Cache::removeFromOrder(Elm &elm) {
    Int stop;
    if (findExact(elmDesc(elm).file, stop)) {
        _elms--;
        MoveFastN(_order + stop, _order + stop + 1, _elms - stop); // faster version of: for(Int i=stop; i<_elms; i++)_order[i]=_order[i+1];
    }
}
/******************************************************************************/
Ptr _Cache::validElmData(Elm &elm, Bool counted) {
    ...
    if (!(desc.flag & CACHE_ELM_LOADING)) {
        if (counted)
            IncPtrNum(desc.ptr_num);
        else
            FlagEnable(desc.flag, CACHE_ELM_STD_PTR);
        return elmData(elm);
    }
    return null;
}
...
Ptr _Cache::find(CChar *file, CChar *path, Bool counted) {
    if (Is(file)) // valid file name
    {
        ...
        if (Elm *elm = findElm(file, path))
            return validElmData(*elm, counted);
    }
    return null;
}
/******************************************************************************/
...
Ptr _Cache::get(CChar *file, CChar *path, Bool counted) {
    if (Is(file)) // valid file name
    {
        ...
        Elm &elm = _memx.New();
        Desc &desc = elmDesc(elm);
        Ptr data = elmData(elm);
        desc.ptr_num = (counted ? 1 : 0);
        desc.flag = (counted ? 0 : CACHE_ELM_STD_PTR);
        if (max_elms != _memx.maxElms())
            Realloc(_order, _memx.maxElms(), max_elms);
        ...
        dummy:
            ...
            if (Is(path)) {
                desc.file = HelperPath(file, path);
                if (!FExist(desc.file) && FExist(file))
                    desc.file = file;
            } // if a file does not exist at path, but does exist at absolute, then use absolute
            else
                desc.file = file; // use absolute when no path available
            addToOrder(elm);
            return data;
        } else if (_mode == CACHE_DUMMY_NULL) // dummy on exist, null on fail
        {
            Bool exist = false;
            if (Is(path)) {
                desc.file = HelperPath(file, path);
...
Bool _Cache::contains(CPtr data) C {
    if (C Elm *elm = dataElm(data)) {
        ...
        SyncLocker locker(_lock);
        if (lockedContains(elm))
            if (!(elmDesc(*elm).flag & CACHE_ELM_LOADING))
                return true;
    }
    return false;
}
Bool _Cache::dummy(CPtr data) C {
    if (C Elm *elm = dataElm(data)) {
        ...
        SyncLocker locker(_lock);
        if (lockedContains(elm))
            if (elmDesc(*elm).flag & CACHE_ELM_DUMMY)
                return true;
    }
    return false;
}
void _Cache::dummy(CPtr data, Bool dummy) {
    if (Elm *elm = dataElm(data)) {
        ...
        SyncLocker locker(_lock);
        if (lockedContains(elm))
            FlagSet(elmDesc(*elm).flag, CACHE_ELM_DUMMY, dummy);
    }
}
...
C Str &_Cache::name(CPtr data) C {
    if (C Elm *elm = dataElm(data)) {
        ...
        if (lockedContains(elm)) {
            ...
            if (!(desc.flag & CACHE_ELM_LOADING))
                return desc.file; // name may change while loading
        }
    }
    return S;
}
CChar *_Cache::name(CPtr data, CChar *path) C {
    if (C Elm *elm = dataElm(data)) {
        ...
        if (lockedContains(elm)) {
            ...
            {
                if (Is(path))
                    return _SkipStartPath(desc.file, _SkipStartPath(path, DataPath())); // must be '_SkipStartPath' because we're returning CChar*
                return desc.file;
            }
        }
    }
...
UID _Cache::id(CPtr data) C {
    if (C Elm *elm = dataElm(data)) {
        ...
        SyncLocker locker(_lock);
        if (lockedContains(elm))
            return FileNameID(elmDesc(*elm).file); // ID does not change while loading, so ignore CACHE_ELM_LOADING
    }
    return UIDZero;
}
/******************************************************************************/
...
void _Cache::lockedRemoveData(CPtr data, Bool counted) {
    ...
    if (lockedContains(elm)) {
        ...
        if (!desc.ptr_num && (!counted || !(desc.flag & CACHE_ELM_STD_PTR))) // if there are no more pointers accessing this element (in "!counted" we've already disabled CACHE_ELM_STD_PTR so don't have to check it anymore)
        {
            ...
            if (delay_remove_time > 0 && GetThreadID() == DelayRemoveThreadID)
                delay_remove_time -= DelayRemoveWaited;         // if want to use delayed remove by time and we're unloading because of parent getting delay unloaded, then decrease the time which the parent already waited
                ...
                || (_can_be_removed && !_can_be_removed(data))) // or it can't be removed right now
            {
                ...
                {
                    Int i = findDelayRemove(*elm);
                    DEBUG_ASSERT(i >= 0, "'_Cache.decRef' Element has CACHE_ELM_DELAY_REMOVE but isn't listed in '_delay_remove'");
            ...
            {
                if (desc.flag & CACHE_ELM_DELAY_REMOVE)
                    _delay_remove.remove(findDelayRemove(*elm)); // if was listed in the 'delay_remove' then remove it from it
                removeFromOrder(*elm);
                _memx.removeData(elm);
            }
        }
...
/******************************************************************************/
void _Cache::incRefContained(CPtr data) {
    Elm *elm = dataElm(data); /*if(elm)*/
    IncPtrNum(elmDesc(*elm).ptr_num);
}
...
void _Cache::processDelayRemove(Bool always) {
    if (_delay_remove.elms()) {
        SyncUnlocker unlocker(D._lock); // this must be used also since later 'D._lock' can be locked when deleting the resource
        SyncLocker delay_locker(DelayRemoveLock);
        DelayRemoveThreadID = GetThreadID(); // support only one 'processDelayRemove' at a time, because we use only one global 'DelayRemoveWaited' based on 'DelayRemoveThreadID' for all caches/threads
        SyncLocker locker(_lock);
        ...
        REPA(_delay_remove) {
            ...
            if (always || Time.appTime() >= remove_time) // if always remove or enough time has passed (use >= so when having zero delay time then it will be processed immediately)
            {
                ...
                {
                    Desc &desc = elmDesc(elm);
                    _delay_remove.remove(i); // access before removal and remove afterwards
...
void _Cache::delayRemoveDec() {
    if (AtomicDec(_delay_remove_counter) == 1)
        update();
}
void _Cache::delayRemoveNow() { processDelayRemove(true); }
void _Cache::update() {
    if (_delay_remove.elms() && _delay_remove_counter == 0 && Time.appTime() >= _delay_remove_check)
...
//_data_offset   =src._data_offset;
  ...
  _memx._reset(src._memx.elmSize(), src._memx.blockElms(), src._memx._new, src._memx._del);
    ...
    {
        C Elm &src_elm = *src._order[i];
        Int memx_index = src._memx.validIndex(&src_elm);
        RANGE_ASSERT_ERROR(memx_index, _memx, "Invalid element index in Cache.operator=(C Cache &src)"); // get valid index of i-th element in memx container
        Elm &dest_elm = _memx[memx_index];
        _order[i] = &dest_elm; // set valid index of i-th element
