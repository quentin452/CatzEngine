...
void Cipher1::randomizeKey() {
    UID uid[256 / SIZE(UID)];
    REPAO(uid).randomize(); // 'UID.randomize' uses a more secure version than 'Random'
    setKey(uid[0].b, SIZE(uid));
}
Bool Cipher1::saveKey(File &f) C { return f.put(_enc, 256); }
Bool Cipher1::loadKey(File &f) {
    Bool ok = f.get(_enc, 256);
    setDec();
    wrapKey();
    return ok;
}
Bool Cipher1::mixKey(File &f) {
    Byte temp[256];
    f >> temp;
    REPAO(temp) ^= _enc[i];
    setKey(temp, Elms(temp));
    return f.ok();
}
/******************************************************************************/
...
void Cipher1::encrypt(Ptr dest, CPtr src, IntPtr size, Int offset) {
    if (Byte *d = (Byte *)dest) {
        ...
        if (C Byte *s = (Byte *)src) {
            ...
            {
            case 3:
                *d++ = _enc[(*s++) ^ _enc[offset]];
                offset++; // !! no break on purpose !!
            case 2:
                *d++ = _enc[(*s++) ^ _enc[offset]];
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = _enc[(*s++) ^ _enc[offset]]; // offset++; // !! no break on purpose !!
            }
        } else {
            REPP(size / 4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
            {
            ...
            case 2:
                *d++ = _enc[_enc[offset]];
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = _enc[_enc[offset]]; // offset++; // !! no break on purpose !!
            }
        }
    }
}
void Cipher1::decrypt(Ptr dest, CPtr src, IntPtr size, Int offset) {
    if (Byte *d = (Byte *)dest) {
        ...
        if (C Byte *s = (Byte *)src) {
            ...
            {
            case 3:
                *d++ = _dec[(*s++)] ^ _enc[offset];
                offset++; // !! no break on purpose !!
            case 2:
                *d++ = _dec[(*s++)] ^ _enc[offset];
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = _dec[(*s++)] ^ _enc[offset]; // offset++; // !! no break on purpose !!
            }
        } else {
            Byte dec0 = _dec[0];
            REPP(size / 4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
            ...
            case 2:
                *d++ = dec0 ^ _enc[offset];
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = dec0 ^ _enc[offset]; // offset++; // !! no break on purpose !!
            }
        }
    }
}
...
Bool Cipher2::loadKey(File &f) {
    Bool ok = f.get(_enc, 256);
    setDec();
    return ok;
}
Bool Cipher2::mixKey(File &f) {
    Byte temp[256];
    f >> temp;
    REPAO(temp) ^= _enc[i];
    setKey(temp, Elms(temp));
...
void Cipher2::encrypt(Ptr dest, CPtr src, IntPtr size, Int offset) {
    if (Byte *d = (Byte *)dest) {
        if (C Byte *s = (Byte *)src) {
            REPP(size / 4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
            {
                *d++ = _enc[_enc[(*s++) ^ (offset & 0xFF)] ^ ((offset >> 8) & 0xFF)];
                offset++;
                *d++ = _enc[_enc[(*s++) ^ (offset & 0xFF)] ^ ((offset >> 8) & 0xFF)];
                offset++;
                *d++ = _enc[_enc[(*s++) ^ (offset & 0xFF)] ^ ((offset >> 8) & 0xFF)];
            ...
            case 2:
                *d++ = _enc[_enc[(*s++) ^ (offset & 0xFF)] ^ ((offset >> 8) & 0xFF)];
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = _enc[_enc[(*s++) ^ (offset & 0xFF)] ^ ((offset >> 8) & 0xFF)]; // offset++; // !! no break on purpose !!
            }
        } else {
            REPP(size / 4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
            {
                *d++ = _enc[_enc[offset & 0xFF] ^ ((offset >> 8) & 0xFF)];
                offset++;
                *d++ = _enc[_enc[offset & 0xFF] ^ ((offset >> 8) & 0xFF)];
                offset++;
                *d++ = _enc[_enc[offset & 0xFF] ^ ((offset >> 8) & 0xFF)];
            ...
            case 2:
                *d++ = _enc[_enc[offset & 0xFF] ^ ((offset >> 8) & 0xFF)];
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = _enc[_enc[offset & 0xFF] ^ ((offset >> 8) & 0xFF)]; // offset++; // !! no break on purpose !!
            }
        }
    }
}
void Cipher2::decrypt(Ptr dest, CPtr src, IntPtr size, Int offset) {
    if (Byte *d = (Byte *)dest) {
        if (C Byte *s = (Byte *)src) {
            REPP(size / 4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
            {
                *d++ = _dec[_dec[*s++] ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++;
                *d++ = _dec[_dec[*s++] ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++;
                *d++ = _dec[_dec[*s++] ^ ((offset >> 8) & 0xFF)] ^ offset;
            ...
            case 2:
                *d++ = _dec[_dec[*s++] ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = _dec[_dec[*s++] ^ ((offset >> 8) & 0xFF)] ^ offset; // offset++; // !! no break on purpose !!
            }
        } else {
            Byte dec0 = _dec[0];
            REPP(size / 4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
            {
                *d++ = _dec[dec0 ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++;
                *d++ = _dec[dec0 ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++;
                *d++ = _dec[dec0 ^ ((offset >> 8) & 0xFF)] ^ offset;
            ...
            case 2:
                *d++ = _dec[dec0 ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = _dec[dec0 ^ ((offset >> 8) & 0xFF)] ^ offset; // offset++; // !! no break on purpose !!
            }
        }
    }
}
...
Bool Cipher3::loadKey(File &f) {
    Bool ok = f.get(_enc, 256);
    setDec();
    return ok;
}
Bool Cipher3::mixKey(File &f) {
    Byte temp[256];
    f >> temp;
    REPAO(temp) ^= _enc[i];
    setKey(temp, Elms(temp));
...
void Cipher3::encrypt(Ptr dest, CPtr src, IntPtr size, Int offset) {
    if (Byte *d = (Byte *)dest) {
        if (C Byte *s = (Byte *)src) {
            REPP(size / 4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
            {
                *d++ = _enc[_enc[_enc[(*s++) ^ (offset & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ ((offset >> 16) & 0xFF)];
                offset++;
                *d++ = _enc[_enc[_enc[(*s++) ^ (offset & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ ((offset >> 16) & 0xFF)];
                offset++;
                *d++ = _enc[_enc[_enc[(*s++) ^ (offset & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ ((offset >> 16) & 0xFF)];
            ...
            case 2:
                *d++ = _enc[_enc[_enc[(*s++) ^ (offset & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ ((offset >> 16) & 0xFF)];
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = _enc[_enc[_enc[(*s++) ^ (offset & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ ((offset >> 16) & 0xFF)]; // offset++; // !! no break on purpose !!
            }
        } else {
            REPP(size / 4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
            {
                *d++ = _enc[_enc[_enc[offset & 0xFF] ^ ((offset >> 8) & 0xFF)] ^ ((offset >> 16) & 0xFF)];
                offset++;
                *d++ = _enc[_enc[_enc[offset & 0xFF] ^ ((offset >> 8) & 0xFF)] ^ ((offset >> 16) & 0xFF)];
                offset++;
                *d++ = _enc[_enc[_enc[offset & 0xFF] ^ ((offset >> 8) & 0xFF)] ^ ((offset >> 16) & 0xFF)];
            ...
            case 2:
                *d++ = _enc[_enc[_enc[offset & 0xFF] ^ ((offset >> 8) & 0xFF)] ^ ((offset >> 16) & 0xFF)];
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = _enc[_enc[_enc[offset & 0xFF] ^ ((offset >> 8) & 0xFF)] ^ ((offset >> 16) & 0xFF)]; // offset++; // !! no break on purpose !!
            }
        }
    }
}
void Cipher3::decrypt(Ptr dest, CPtr src, IntPtr size, Int offset) {
    if (Byte *d = (Byte *)dest) {
        if (C Byte *s = (Byte *)src) {
            REPP(size / 4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
            {
                *d++ = _dec[_dec[_dec[*s++] ^ ((offset >> 16) & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++;
                *d++ = _dec[_dec[_dec[*s++] ^ ((offset >> 16) & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++;
                *d++ = _dec[_dec[_dec[*s++] ^ ((offset >> 16) & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ offset;
            ...
            case 2:
                *d++ = _dec[_dec[_dec[*s++] ^ ((offset >> 16) & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = _dec[_dec[_dec[*s++] ^ ((offset >> 16) & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ offset; // offset++; // !! no break on purpose !!
            }
        } else {
            Byte dec0 = _dec[0];
            REPP(size / 4) // doing 'VecB4' reads/writes made things slower, so process each 'Byte' separately, "for(; size>=4; size-=4)" was slower for size>=1024
            {
                *d++ = _dec[_dec[dec0 ^ ((offset >> 16) & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++;
                *d++ = _dec[_dec[dec0 ^ ((offset >> 16) & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++;
                *d++ = _dec[_dec[dec0 ^ ((offset >> 16) & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ offset;
            ...
            case 2:
                *d++ = _dec[_dec[dec0 ^ ((offset >> 16) & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ offset;
                offset++; // !! no break on purpose !!
            case 1:
                *d++ = _dec[_dec[dec0 ^ ((offset >> 16) & 0xFF)] ^ ((offset >> 8) & 0xFF)] ^ offset; // offset++; // !! no break on purpose !!
            }
        }
    }
}
    ...
    if (C Byte *key = (Byte *)key_data) {
        ...
        if (Cpu.flag() & CPU_AES) {
            ...
            for (;;) {
                ...
                if (rk + key_size / 4 + 4 >= _encrypt_key + ELMS(_encrypt_key))
                    break;
            ...
            Int i, j;
            for (i = 4, j = 4 * _rounds - 4; i < j; i += 4, j -= 4) {
