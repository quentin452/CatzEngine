...
void IndexPtrGroup::remVal(Ptr value) {
    for (Int i = 0; i < num;)
        if (elm[i] == value)
            elm[i] = elm[--num];
        else
...
void IndexGroup ::subVal(Int value) {
    for (Int i = 0; i < num;)
        if (elm[i] == value) {
            elm[i] = elm[--num];
            break;
...
void IndexPtrGroup::subVal(Ptr value) {
    for (Int i = 0; i < num;)
        if (elm[i] == value) {
            elm[i] = elm[--num];
            break;
...
void IndexGroup ::subElm(Int i) {
    if (InRange(i, num))
        elm[i] = elm[--num];
}
void IndexPtrGroup::subElm(Int i) {
    if (InRange(i, num))
        elm[i] = elm[--num];
}
/******************************************************************************/
...
Index &Index::set(Int *elm_group) {
    ...
    elms = 0;
    REP(groups) {
        if (ig->num > group_elms_max)
            group_elms_max = ig->num;
...
void CreateFaceIs(MemPtr<Bool> tri_is, MemPtr<Bool> quad_is, C CMemPtr<Int> &faces, Int tris, Int quads) {
    tri_is.setNumDiscard(tris);
    REPAO(tri_is) = false;
    quad_is.setNumDiscard(quads);
    REPAO(quad_is) = false;
    REPA(faces) {
        UInt f = faces[i];
        if (f & SIGN_BIT) {
            f ^= SIGN_BIT;
            if (InRange(f, quad_is))
                quad_is[f] = true;
        } else {
            if (InRange(f, tri_is))
                tri_is[f] = true;
...
void SetFaceIndex(Ptr data, C VecI *tri, Int tris, C VecI4 *quad, Int quads, Bool bit16) {
    if (bit16) {
        ...
        VecUS *t = (VecUS *)data + tris;
        REP(quads) {
            (t++)->set(quad->x, quad->y, quad->w);
            (t++)->set(quad->w, quad->y, quad->z);
            quad++;
        }
    } else {
        Copy32To32(data, tri, tris * 3);
        VecI *t = (VecI *)data + tris;
        REP(quads) {
            (t++)->set(quad->x, quad->y, quad->w);
            (t++)->set(quad->w, quad->y, quad->z);
            quad++;
...
void SetRemap(MemPtr<Int> remap, C CMemPtr<Bool> &is, Int elms) {
    ...
    for (; i < full; i++)
        remap[i] = (is[i] ? index++ : -1);
    for (; i < remap.elms(); i++)
        remap[i] = -1;
}
void IndRemap(C CMemPtr<Int> &remap, Int *ind, Int elms) {
    for (; elms-- > 0; ind++)
        *ind = (InRange(*ind, remap) ? remap[*ind] : -1);
}
/******************************************************************************/
Bool IndSave(File &f, C Int *ind, Int inds, Int elms) {
    ...
    if (elms > 0) {
        if (elms <= 0x00000100)
            bytes = 1;
        else if (elms <= 0x00010000)
            bytes = 2;
        else if (elms <= 0x01000000)
            bytes = 3;
    }
    Int size = inds * bytes;
    CPtr data = ind;
    ...
    if (bytes != 4) {
        ...
        switch (bytes) {
        case 3:
            Copy32To24(temp.data(), ind, inds);
            break;
        case 2:
            Copy32To16(temp.data(), ind, inds);
            break;
        case 1:
            Copy32To8(temp.data(), ind, inds);
            break;
        }
    }
...
Bool IndLoad(File &f, Int *ind, Int inds) {
    ...
    case 8:
        f.getFast(ind, size);
        Copy8To32(ind, ind, inds);
        break;
    case 16:
        f.getFast(ind, size);
        Copy16To32(ind, ind, inds);
        break;
    case 24:
        f.getFast(ind, size);
        Copy24To32(ind, ind, inds);
        break;
    case 32:
        f.getFast(ind, size);
        break;
