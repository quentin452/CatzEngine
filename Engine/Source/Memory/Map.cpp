...
ASSERT(OFFSET(MapInt::Elm, data) == 0); // '_data_offset' is not used because it's assumed to be always 0
/******************************************************************************/
...
Ptr _Map::get(CPtr key) {
    ...
    // find existing
    if (Elm *elm = findElm(key, stop))
        return (elmDesc(*elm).flag & CACHE_ELM_LOADING) ? null : elmData(*elm);

    // always null
    if (_mode == MAP_ALL_NULL)
        return null;
    ...
    Ptr data = elmData(elm), elm_key = elmKey(elm);
    desc.flag = 0;
...
void _Map::getFailed() C {
    if (_mode == MAP_EXIT)
        Exit("Can't create object in 'Map' from key");
}
Ptr _Map ::require(CPtr key) {
    if (Ptr data = get(key))
        return data;
    getFailed();
    return null;
}
Ptr _MapTS::require(CPtr key) {
    if (Ptr data = get(key))
        return data;
    getFailed();
    return null;
...
Int _Map ::requireAbsIndex(CPtr key) {
    Int abs_index = getAbsIndex(key);
    if (abs_index >= 0)
        return abs_index;
    getFailed();
    return -1;
}
Int _MapTS::requireAbsIndex(CPtr key) {
    Int abs_index = getAbsIndex(key);
    if (abs_index >= 0)
        return abs_index;
    getFailed();
...
/******************************************************************************/
Ptr _Map::get(CPtr key, Bool &just_created) {
    Int elms = T.elms();
    Ptr data = get(key);
    just_created = (elms != T.elms());
    return data;
}
Ptr _Map::require(CPtr key, Bool &just_created) {
    Int elms = T.elms();
    Ptr data = require(key);
    just_created = (elms != T.elms());
    return data;
}
/******************************************************************************/
Bool _Map::dummy(CPtr data) C {
    C Elm *elm = dataElm(data);
    if (containsElm(elm))
...
CPtr _Map::dataToKey(CPtr data) C {
    C Elm *elm = dataElm(data);
    if (containsElm(elm))
        return elmKey(*elm); // key does not change while loading, so ignore CACHE_ELM_LOADING
    return null;
}
...
Int _Map::dataToIndex(CPtr data) C {
    C Elm *elm = dataElm(data);
    if (containsElm(elm)) // only after we know that this element belongs to this container then we can access its key
    {
        Int i;
        if (findElm(elmKey(*elm), i))
            return i;
    }
    return -1;
}
        ...
        if (src_elm != dest_elm) // it's not the same element (this would mean that both keys are the same and we don't need to do anything)
        {
            if (src_elm && dest_elm) {
                ...
                if (src_elm) {
                    ...
                    if (dest_i > src_i)
                        dest_i--;                          // we're moving element from the order to the right, which means that one slot is already occupied by this element
                    MoveElm(_order, _elms, src_i, dest_i); // adjust order position
                } else if (dest_elm) {
                    _copy_key(elmKey(*dest_elm), src_key); // adjust key "dest_elm.key = src_key"
                    if (src_i > dest_i)
                        src_i--;                           // we're moving element from the order to the right, which means that one slot is already occupied by this element
...
//_data_offset=src._data_offset;
  ...
  _memx._reset(src._memx.elmSize(), src._memx.blockElms(), src._memx._new, src._memx._del);
    ...
    REPA(_memx)                     //      setup order exactly the same way as source
    {
        ...
        RANGE_ASSERT_ERROR(memx_index, _memx, "Invalid element index in Map.operator=(C Map &src)"); // get valid index of i-th element in memx container
...
void _MapEx::decRef(CPtr data) {
      ...
      #endif*/
    {
        ...
        if (containsElm(elm)) {
            DescPtrNum &desc = elmDesc(*elm);
            DEBUG_ASSERT(desc.ptr_num > 0, "'_MapEx.decRef' Decreasing 'ptr_num' when it's already zero");
            if (!--desc.ptr_num && !(desc.flag & CACHE_ELM_STD_PTR)) // if there are no more pointers accessing this element
            {
                ...
                if (delay_remove_time > 0 // if want to use delayed remove by time
                    ...
                    if (desc.flag & CACHE_ELM_DELAY_REMOVE)        // if already listed
                    {
                        ...
                        DEBUG_ASSERT(i >= 0, "'_MapEx.decRef' Element has CACHE_ELM_DELAY_REMOVE but isn't listed in '_delay_remove'");
