...
static Int MaxBlocks(C Blocks &blocks, C BlocksOcclusion *occl) {
    if (occl)
        if (Int steps = occl->maxSteps()) {
            if (blocks.subDivision() > 1)
                steps = DivCeil(steps, blocks.subDivision());
            return steps;
        }
    return 0;
}
...
static void PrepareAndClean(Memc<BlocksOcclusion::Node> &nodes, C BlocksOcclusion &occl) {
    ...
    REPA(nodes) {
        ...
        default:
            frac = node.fraction[0] + node.fraction[1];
            break; // ADD
        case BlocksOcclusion::MAX:
            frac = Max(node.fraction[0], node.fraction[1]);
            break; // MAX
        }
        if (frac <= threshold)
            nodes.remove(i);
            ...
            if (occl.angles()) {
                Vec dir = node.pos + 0.5f;
                dir.normalize();
                dir.abs();
                dir *= frac * (65536 / 0.5f); // div by 0.5 because the average normal component based on codes at the top is 0.5, so we need to recompensate that
...
void BlocksOcclusion::create(Int steps, Flt strength, Int steps_1, Flt strength_1, COMBINE_MODE ao_combine_mode, Bool angles) {
    ...
    if (steps_1 > steps) {
        Swap(steps, steps_1);
        Swap(strength, strength_1);
    } // make #0 always the higher
            ...
            REPD(y, rays_res) {
                ...
                REPD(f, 6)     // 6 cube faces
                {
                    ...
                    for (VoxelWalker walker(ray * EPS, ray * (steps - EPS)); walker.active(); walker.step()) // don't start at zero, but from a small step along the ray, also end a bit earlier so we don't accidentally add blocks out of range
                    {
                        ...
                        if (!node)
                            node = &nodes->New().init(pos);  // get node with such position (create new if not found)
                        node->fraction[0] += ray_fraction_0; // add ray to ray counter (as fraction of total amount of rays)
                        if ((pos + 0.5f).length2() <= steps_1_sqr)
                            node->fraction[1] += ray_fraction_1; // add ray to ray counter (as fraction of total amount of rays)
                        nodes = &node->nodes;
                    }
                }
            }
...
BlocksOcclusion &BlocksOcclusion::steps(Int steps) {
    create(steps, T.strength(), T.steps1(), T.strength1(), T.aoCombineMode(), T.angles());
    return T;
}
BlocksOcclusion &BlocksOcclusion::steps1(Int steps) {
    create(T.steps(), T.strength(), steps, T.strength1(), T.aoCombineMode(), T.angles());
    return T;
}
BlocksOcclusion &BlocksOcclusion::strength(Flt strength) {
    create(T.steps(), strength, T.steps1(), T.strength1(), T.aoCombineMode(), T.angles());
    return T;
}
BlocksOcclusion &BlocksOcclusion::strength1(Flt strength) {
    create(T.steps(), T.strength(), T.steps1(), strength, T.aoCombineMode(), T.angles());
    return T;
}
BlocksOcclusion &BlocksOcclusion::aoCombineMode(COMBINE_MODE aocm) {
    create(T.steps(), T.strength(), T.steps1(), T.strength1(), aocm, T.angles());
    return T;
}
BlocksOcclusion &BlocksOcclusion::angles(Bool on) {
    create(T.steps(), T.strength(), T.steps1(), T.strength1(), T.aoCombineMode(), on);
    return T;
}

BlocksOcclusion::Node &BlocksOcclusion::Node::init(C VecI &pos) {
    fraction[0] = fraction[1] = 0;
    dir = (((pos.x >= 0) ? DIRF_RIGHT : DIRF_LEFT) | ((pos.y >= 0) ? DIRF_UP : DIRF_DOWN) | ((pos.z >= 0) ? DIRF_FORWARD : DIRF_BACK));
    T.pos = pos;
    return T;
}
...
Bool BlocksMap::raycast(C Vec &start, C Vec &move, Vec *hit_pos, Vec *hit_normal) C {
    ...
    block_start /= _base_matrix;
    block_start /= resolution();
    block_move /= _base_matrix.orn();
    block_move /= resolution();
    ...
    for (PixelWalker walker(block_start.xz(), (block_start + block_move).xz()); walker.active(); walker.step())
        if (Blocks *blocks = findBlocks(walker.pos().x, walker.pos().y))
            if (blocks->raycast(start, move, hit_pos, hit_normal, &NoTemp(matrix(walker.pos().x, walker.pos().y))))
                return true;
    return false;
}
Vec BlocksMap::light(C Vec &pos, C BlocksOcclusion *occl, C CMemPtr<Blocks::Light> &lights) C {
    Vec local_pos = pos / _base_matrix;
    Vec block_pos = local_pos / resolution();
    ...
    REPA(lights) {
        ...
        if (dist2 < radius2)
            if (!raycast(pos, delta))
                lum += light.color * (1 - dist2 / radius2);
    }
    return lum;
}
/******************************************************************************/
...
Blocks::Neighbors::Neighbors(C BlocksMap *map, Int x, Int y, Int resolution) {
    if (map)
        set(map->findBlocks(x - 1, y), map->findBlocks(x + 1, y), map->findBlocks(x, y - 1), map->findBlocks(x, y + 1),
...
void Blocks::Level::createPhysBody(C Matrix &matrix, Int res, C Level *down, C Level *up) {
   ...
   FREPD(z, res)
   {
    ...
    FREPD(z, res) {
        ...
        if (wallBack(x, z, res)) {
            Int to = x;
            for (Int i = x + 1; i < res; i++)
                if (wallBack(i, z, res))
                    to = i;
                else if (!continuous || !map(i, z, res))
                    break;
            Int v = vtxs.elms();
            quads.New().set(v, v + 1, v + 2, v + 3);
            vtxs.New().set(x, y, z);
            vtxs.New().set(x, y + 1, z);
            vtxs.New().set(to + 1, y + 1, z);
            vtxs.New().set(to + 1, y, z);
        ...
        // forward
        FREPD(x, res)
        if (wallForward(x, z, res)) {
            Int to = x;
            for (Int i = x + 1; i < res; i++)
                if (wallForward(i, z, res))
                    to = i;
                else if (!continuous || !map(i, z, res))
                    break;
            Int v = vtxs.elms();
    ...
    FREPD(x, res) {
        ...
        if (wallLeft(x, z, res)) {
            Int to = z;
            for (Int i = z + 1; i < res; i++)
                if (wallLeft(x, i, res))
                    to = i;
                else if (!continuous || !map(x, i, res))
                    break;
            Int v = vtxs.elms();
            quads.New().set(v, v + 1, v + 2, v + 3);
            vtxs.New().set(x, y, to + 1);
        ...
        // right
        FREPD(z, res)
        if (wallRight(x, z, res)) {
            Int to = z;
            for (Int i = z + 1; i < res; i++)
                if (wallRight(x, i, res))
                    to = i;
                else if (!continuous || !map(x, i, res))
                    break;
            Int v = vtxs.elms();
    ...
    FREPD(z, res) {
        // bottom
        FREPD(x, res)
        if (wallBottom(x, z, res, down)) {
            Int to = x;
            for (Int i = x + 1; i < res; i++)
                if (wallBottom(i, z, res, down))
                    to = i;
                else if (!continuous || !map(i, z, res))
                    break;
            Int v = vtxs.elms();
        ...
        // top
        FREPD(x, res)
        if (wallTop(x, z, res, up)) {
            Int to = x;
            for (Int i = x + 1; i < res; i++)
                if (wallTop(i, z, res, up))
                    to = i;
                else if (!continuous || !map(i, z, res))
                    break;
            Int v = vtxs.elms();
...
C Color &Blocks::LevelBrightness::brightness(Int x, Int y, Int z, DIR_ENUM dir) {
    ...
    if (point.computed_y == y && (point.computed_dir & DirToFlag(dir)))
        return 0;
#endif
            ...
            REPA(lights) {
                ...
                if (dist2 < radius2) {
                    ...
                    if (dist2) {
                        Flt dist = Sqrt(dist2);
                        delta /= dist;
                        dot = Dot(VecDir[dir], delta);
                        pos += delta * EPSL;
                    } // normalize and move slightly towards the light, so we don't start from the wall itself
                    if (dot > 0) {
                        if (map)
                            pos *= map->matrix(blocks_pos.x, blocks_pos.y); // convert to world space, needed for 'BlocksMap.raycast'
                        if (map ? !map->raycast(pos, light.world_pos - pos) : !blocks.raycast(pos, light.ball.pos - pos))
                            col += ((1 - dist2 / radius2) * dot) * light.color;
                    }
                }
            }
...
Bool Blocks::hasBlock(C VecI &pos) C {
    if (InRange(pos.x, resolution()) && InRange(pos.z, resolution()))
    ...
    if (InRange(pos.x, resolution()) && InRange(pos.z, resolution()))
        if (C Level *level = findLevel(pos.y)) {
            ...
            if (m && InRange(m, _mtrl_combos)) {
                ...
                if (InRange(mc.top, _materials))
                    return _materials[mc.top];
            }
        }
    return null;
}
    ...
    if (InRange(pos.x, resolution()) && InRange(pos.z, resolution()))
        if (C Level *level = findLevel(pos.y)) {
            ...
            if (m && InRange(m, _mtrl_combos)) {
                C MtrlCombo &mc = _mtrl_combos[m];
                if (InRange(mc.top, _materials))
                    top = _materials[mc.top];
                else
                    top.clear();
                if (InRange(mc.side, _materials))
                    side = _materials[mc.side];
                else
                    side.clear();
                if (InRange(mc.bottom, _materials))
                    bottom = _materials[mc.bottom];
                else
                    bottom.clear();
                return;
            }
...
Bool Blocks::set(Int x, Int y, Int z, Byte b) {
    if (InRange(x, resolution()) && InRange(z, resolution()))
...
Bool Blocks::set(Int x, Int y, Int z, C MaterialPtr &top, C MaterialPtr &side, C MaterialPtr &bottom) {
    ...
    if (top && side && bottom)  // if all are set (we can't set materials only if some of them are set)
    {
        ...
        {
            Int mc = -1;
            if (!_mtrl_combos.elms())
                _mtrl_combos.New().set(0, 0, 0); // 0-th material combo is reserved for null materials
            FREPA(_mtrl_combos) {
                C MtrlCombo &mtrl_combo = _mtrl_combos[i];
                if (mtrl_combo.top == t && mtrl_combo.side == s && mtrl_combo.bottom == b) {
                    mc = i;
                    break;
                }
            } // find existing
            if (mc < 0 && _mtrl_combos.elms() < 256) {
                mc = _mtrl_combos.elms();
                _mtrl_combos.New().set(t, s, b);
            } // support indexes only up to 255
            if (mc >= 0)
                return set(x, y, z, mc);
        }
    }
...
Blocks &Blocks::setMesh(Flt tex_scale, C BlocksOcclusion *occl, C BoxI *local_box, C VecI2 &blocks_pos, C BlocksMap *map, C CMemPtr<Light> &lights, Bool optimize, Flt max_face_length) {
    ...
    if (local_box) {
        box = *local_box;
        box.extend(Max(1, MaxBlocks(T, occl))); // we must extend the box by at least one, because block face creation depends on its neighbors, we also extend it by 'BlocksOcclusion' because the blocks can affect the lighting of the nearby blocks
        if (box.min.x >= resolution() || box.min.z >= resolution() || box.max.x < 0 || box.max.z < 0)
            return T; // update box is outside of range
        ...
        if (_levels.elms())
            box.setY(_levels.first().y, _levels.last().y);
        else
            box.setY(0, -1);
    }
    ...
    if (_levels.elms()) {
        ...
        if (lights.elms()) {
            ...
            REPA(lights) {
                LightEx light;
                SCAST(Light, light) = lights[i];
                light.world_pos = light.ball.pos;
                if (map)
                    light.ball /= m;
                if (Cuts(boxf, light.ball))
                    Swap(local_lights.New(), light);
            }
        }
        ...
        REPA(level_brightness) {
            LevelBrightness &lb = level_brightness[i];
            if (local_box)
                lb.clear(box.min.x, box.min.z, box.max.x, box.max.z);
            else
                lb.clear();
        }
        for (Int i = (local_box ? findLevelI(box.min.y) : 0); i < _levels.elms(); i++) {
            Level &level = _levels[i];
            if (local_box && level.y > box.max.y)
                break;
    ...
    REPA(parts)
    if (InRange(i + 1, _materials)) // 'parts[i]' are mapped to 'mesh.parts[i]' and 'materials[i+1]'
    {
        Part &part = parts[i];
        ...
        {
            if (!local_box || !base.is())
                part.create(base);
            else {
                MeshBase temp;
                part.create(temp);
                base += temp;
            } // if resetting or the base is empty then create it, otherwise add to it
            ...
            if (optimize) {
                MeshBase optimized(base);
                optimized.weldCoplanarFaces(EPS_COL_COS, -1, false, max_face_length);
                mpart.render.create(optimized, MESH_ALL, false);
                mpart.setShader(0);
            } else
                mpart.setRender(false);
        }
    }
    _mesh.setBox();
    return T;
...
Blocks &Blocks::setPhysBody(C Matrix &matrix, C BoxI *local_box, Bool create_actor) {
    ...
    if (local_box) {
        box = *local_box;
        box.extend(1); // we must extend the box by at least one, because block face creation depends on its neighbors
        if (box.min.x >= resolution() || box.min.z >= resolution() || box.max.x < 0 || box.max.z < 0)
            return T;
    }
    ...
    for (Int i = (local_box ? findLevelI(box.min.y) : 0); i < _levels.elms(); i++) {
        Level &level = _levels[i];
        Int y = level.y;
        if (local_box && y > box.max.y)
            break;
        C Level *down = _levels.addr(i - 1);
        if (down && down->y != y - 1)
            down = null;
        C Level *up = _levels.addr(i + 1);
        if (up && up->y != y + 1)
            up = null;
        level.createPhysBody(matrix, resolution(), down, up);
        if (create_actor)
            level.createActor();
    }
    return T;
}
Blocks &Blocks::delActor() {
    REPAO(_levels).actor.del();
    return T;
}
Blocks &Blocks::createActor() {
    REPAO(_levels).createActor();
    return T;
}
/******************************************************************************/
Bool Blocks::raycast(C Vec &start, C Vec &move, Vec *hit_pos, Vec *hit_normal, C Matrix *matrix) C {
    if (_levels.elms()) {
        ...
        Box box(Vec(0, min_y, 0), Vec(resolution(), max_y, resolution()));
        box.extend(0.1f); // extend the box so we won't start inside, but from 1 block outside
        Vec hp;
        if (SweepPointBox(local_start, local_move, box, null, null, &hp)) {
            ...
            for (VoxelWalker walker(hp, local_end); walker.active(); walker.step()) {
                ...
                if (InRange(pos.x, resolution()) && InRange(pos.z, resolution())) {
                    was_inside = true;
                    if (last_level_i == SIGN_BIT)
                        last_level_i = findLevelI(pos.y);
                        ...
                        if (level->map(pos.x, pos.z, resolution())) {
                            ...
                            if (hit_normal) {
                                if (last_pos.x == SIGN_BIT)
                                    ...
                                    *hit_normal = last_pos - pos;
                                    if (matrix)
                                        *hit_normal *= matrix->orn();
                    ...
                    return false; // moved outside of range
                last_pos = pos;
            }
        }
    }
...
Bool Blocks::cleanMtrlCombos(Bool is[256], Byte remap[256]) {
    for (Int i = 1; i < _mtrl_combos.elms(); i++)
        if (!is[i]) {
            remap[0] = 0;
            for (Int i = 1, next = 1; i < _mtrl_combos.elms(); i++)
                if (is[i])
                    remap[i] = next++;
                else
                    remap[i] = 0;
            for (Int i = _mtrl_combos.elms(); --i >= 1;)
                if (!is[i])
                    _mtrl_combos.remove(i, true); // need to go from the end
            return true;
        }
    return false;
}
Blocks &Blocks::cleanMaterials() {
    if (_materials.elms() > 1) {
        ...
        Bool mc_is[256];
        Zero(mc_is); // assume all are unused
        REPA(_levels) {
            Level &level = _levels[i];
            REPD(z, resolution())
            REPD(x, resolution())
            mc_is[level.map(x, z, resolution())] = true;
        }

        // get used materials
        Bool mtrl_is[256];
        ZeroFastN(mtrl_is, _materials.elms()); // assume all are unused
        REPA(_mtrl_combos)
        if (mc_is[i]) {
        ...
        if (cleanMtrlCombos(mc_is, remap))
            REPA(_levels) {
                Level &level = _levels[i];
                REPD(z, resolution())
                REPD(x, resolution()) {
        ...
        // remap materials
        if (_materials.clean(mtrl_is, remap))
            REPA(_mtrl_combos) {
                MtrlCombo &mc = _mtrl_combos[i];
                mc.top = remap[mc.top];
...
Bool Blocks::load(File &f, CChar *path) {
    ...
    {
    case 1: {
        _resolution = f.decUIntV();
        _sub_division = f.decUIntV();
    ...
    case 0: {
        ...
        _sub_division = 1;
        if (_materials.load(f, path)) {
            _mtrl_combos.setNum(_materials.elms());
            REPA(_mtrl_combos) {
                MtrlCombo &mc = _mtrl_combos[i];
                mc.top = mc.side = mc.bottom = i;
            }
            Bool include_mesh_and_phys_body = f.getBool();
...
/******************************************************************************/
Int Blocks::findLevelI(Int y) C {
    Int i;
    _levels.binarySearch(y, i, Blocks::CompareLevel);
    return i;
}
Blocks::Level *Blocks::findLevel(Int y, Int from) {
    for (; from < _levels.elms(); from++) {
        Level &l = _levels[from];
        if (l.y == y)
            return &l;
        if (l.y > y)
            break;
...
Blocks::Level &Blocks::getLevel(Int y) {
    Int i;
    if (!_levels.binarySearch(y, i, Blocks::CompareLevel))
        _levels.NewAt(i).init(resolution(), y);
    return _levels[i];
}
Blocks::Level *Blocks::toLevel(Int y, Int &last) {
    if (_levels.elms()) {
        ...
        Int dir = Sign(y - level.y);
        if (!dir)
            return &level;
        for (last += dir; InRange(last, _levels);) {
            Level &level = _levels[last];
            Int new_dir = Sign(y - level.y);
            if (!new_dir)
                return &level;
            if (dir != new_dir)
                break; // if went too far and didn't find along the way
        }
    }
    return null;
}
...
UInt Blocks::occlusion(C BlocksOcclusion::Node &node, C VecI &pos, AXIS_TYPE axis, Int min_level_i, C Neighbors &neighbors) C {
    ...
    if (InRange(p.x, resolution()) && InRange(p.z, resolution())) // center
    {
        if (C Level *level = findLevel(p.y, min_level_i))
            if (level->map(p.x, p.z, resolution()))
                return node.fraction_axis[axis]; // if(hasBlock(p, min_level_i))
    } else if (p.x < 0)                          // left
    {
        if (p.z < 0) // back
        {
            if (neighbors.lb && neighbors.lb->hasBlock(p + VecI(resolution(), 0, resolution())))
                return node.fraction_axis[axis];
        } else if (p.z < resolution()) // middle
        {
            if (neighbors.l && neighbors.l->hasBlock(p + VecI(resolution(), 0, 0)))
                return node.fraction_axis[axis];
        } else // forward
        {
            if (neighbors.lf && neighbors.lf->hasBlock(p + VecI(resolution(), 0, -resolution())))
                return node.fraction_axis[axis];
        }
    } else if (p.x < resolution()) // middle
    {
        if (p.z < 0) // back
        {
            if (neighbors.b && neighbors.b->hasBlock(p + VecI(0, 0, resolution())))
                return node.fraction_axis[axis];
        } else // forward
        {
            if (neighbors.f && neighbors.f->hasBlock(p + VecI(0, 0, -resolution())))
                return node.fraction_axis[axis];
        }
    } else // right
    {
        if (p.z < 0) // back
        {
            if (neighbors.rb && neighbors.rb->hasBlock(p + VecI(-resolution(), 0, resolution())))
                return node.fraction_axis[axis];
        } else if (p.z < resolution()) // middle
        {
            if (neighbors.r && neighbors.r->hasBlock(p + VecI(-resolution(), 0, 0)))
                return node.fraction_axis[axis];
        } else // forward
        {
            if (neighbors.rf && neighbors.rf->hasBlock(p + VecI(-resolution(), 0, -resolution())))
                return node.fraction_axis[axis];
        }
    }
    UInt occl = 0;
    REPA(node.nodes)
    occl += occlusion(node.nodes[i], pos, axis, min_level_i, neighbors);
    return occl;
}
...
static void AdjustOrder(Blocks::Part &part) // depending on color difference, rotate quad order, so 2 triangles will have more smooth lighting
{
    ...
    if (ColorDiffSum(a.col, c.col) < ColorDiffSum(b.col, d.col))
        part.quads.last().rotateOrder();
}
void Blocks::buildLevel(Flt tex_scale, MemPtrN<Part, 256> parts, Int min_x, Int min_z, Int max_x, Int max_z, Int level_i, C Neighbors &neighbors, LevelBrightness (&lb)[2]) {
    Level &level = _levels[level_i];
    Int y = level.y;
    C Level *d_lvl = _levels.addr(level_i - 1);
    if (d_lvl && d_lvl->y != y - 1)
        d_lvl = null;
    C Level *u_lvl = _levels.addr(level_i + 1);
    if (u_lvl && u_lvl->y != y + 1)
        u_lvl = null;
    ...
    FREPD(sy, subDivision()) {
        ...
        for (Int z = min_z; z <= max_z; z++)
            for (Int x = min_x; x <= max_x; x++)
                if (Byte b = level.map(x, z, resolution()))
                    if (InRange(b, _mtrl_combos)) {
                        ...
                        if ((x <= 0) ? (l_lvl ? !l_lvl->map(resolution() - 1, z, resolution()) : true) : !level.map(x - 1, z, resolution())) {
                            if (subDivision() == 1) {
                                Int v = side.vtxs.elms();
                                side.quads.New().set(v, v + 1, v + 2, v + 3);
                                ...
                                REPD(sz, subDivision()) {
                                    Int v = side.vtxs.elms();
                                    side.quads.New().set(v, v + 1, v + 2, v + 3);
                                    Int Z = z * subDivision() + sz, Z1 = Z + 1;
                                    Flt fz = Z * sd, fz1 = fz + sd;
                        ...
                        if ((z <= 0) ? (b_lvl ? !b_lvl->map(x, resolution() - 1, resolution()) : true) : !level.map(x, z - 1, resolution())) {
                            if (subDivision() == 1) {
                                ...
                                REPD(sx, subDivision()) {
                                    Int v = side.vtxs.elms();
                                    side.quads.New().set(v, v + 1, v + 2, v + 3);
                                    Int X = x * subDivision() + sx, X1 = X + 1;
                                    Flt fx = X * sd, fx1 = fx + sd;
                        ...
                        if (sy == 0)
                            if (!d_lvl || !d_lvl->map(x, z, resolution())) {
                                if (subDivision() == 1) {
                                    Int v = bottom.vtxs.elms();
                                    bottom.quads.New().set(v, v + 1, v + 2, v + 3);
                                    ...
                                    REPD(sx, subDivision()) {
                                        Int X = x * subDivision() + sx;
                                        Flt fx = X * sd, fx1 = fx + sd;
                                        REPD(sz, subDivision()) {
                                            Int v = bottom.vtxs.elms();
                                            bottom.quads.New().set(v, v + 1, v + 2, v + 3);
                                            Int Z = z * subDivision() + sz;
                                            Flt fz = Z * sd, fz1 = fz + sd;
                        ...
                        if (sy == subDivision() - 1)
                            if (!u_lvl || !u_lvl->map(x, z, resolution())) {
                                if (subDivision() == 1) {
                                    Int v = top.vtxs.elms();
                                    top.quads.New().set(v, v + 1, v + 2, v + 3);
