...
static Vec4 MtrlBlendF(C Image &image, Int x, Int y) // !! assumes that 'x, y' are in range !!
{
    switch (image.hwType()) {
    case IMAGE_F32_4:
        return image.pixF4(x, y);
    case IMAGE_R8G8B8A8: {
        C VecB4 &mb = image.pixB4(x, y);
        return Vec4(mb.x / 255.0f, mb.y / 255.0f, mb.z / 255.0f, mb.w / 255.0f);
    }
    }
    return 0;
}
static VecB4 MtrlBlendB(C Image &image, Int x, Int y) // !! assumes that 'x, y' are in range !!
{
    switch (image.hwType()) {
    case IMAGE_R8G8B8A8:
        return image.pixB4(x, y);
    case IMAGE_F32_4: {
        Vec4 mb = image.pixF4(x, y);
        if (Flt sum = mb.sum())
            mb *= 255 / sum;
...
Heightmap &Heightmap::create(Int res, Flt height, C MaterialPtr &material, Bool align_height_to_neighbors, C Heightmap *h_l, C Heightmap *h_r, C Heightmap *h_b, C Heightmap *h_f, C Heightmap *h_lb, C Heightmap *h_lf, C Heightmap *h_rb, C Heightmap *h_rf) {
    if (res > 0) {
        Clamp(res, 2, MAX_HM_RES);
        res = NearestPow2(res) | 1;
        Int res1 = res - 1;
        ...
        } else // align to neighbors
        {
            ...
            if (!align_height_to_neighbors) {
                ...
                if (h_lf)
                    _height.pixF(0, res1) = h_lf->_height.pixF(res1, 0);
                if (h_rb)
                    _height.pixF(res1, 0) = h_rb->_height.pixF(0, res1);
                if (h_rf)
                    _height.pixF(res1, res1) = h_rf->_height.pixF(0, 0);
                if (!h_l) {
                    Flt b = _height.pixF(0, 0), f = _height.pixF(0, res1);
                    REP(res)
                    _height.pixF(0, i) = Lerp(b, f, i / Flt(res1));
                }
                if (!h_r) {
                    Flt b = _height.pixF(res1, 0), f = _height.pixF(res1, res1);
                    REP(res)
                    _height.pixF(res1, i) = Lerp(b, f, i / Flt(res1));
                }
                if (!h_b) {
                    Flt l = _height.pixF(0, 0), r = _height.pixF(res1, 0);
                    REP(res)
                    _height.pixF(i, 0) = Lerp(l, r, i / Flt(res1));
                }
                if (!h_f) {
                    Flt l = _height.pixF(0, res1), r = _height.pixF(res1, res1);
                    REP(res)
                    _height.pixF(i, res1) = Lerp(l, r, i / Flt(res1));
                }
                ...
                for (Int y = 1; y < res1; y++)
                    for (Int x = 1; x < res1; x++) {
                        ...
                        sx = 0;
                        sy = y;
                        p = 1.0f / Dist2(x - sx, y - sy);
                        h += p * _height.pixF(sx, sy);
                        power += p;
                        sx = res1;
                        sy = y;
                        p = 1.0f / Dist2(x - sx, y - sy);
            ...
            // material
            {
                if (h_l)
                    REP(res) {
                        VecB4 &index = _mtrl_index.pixB4(0, i);
                        index = h_l->_mtrl_index.pixB4(res1, i);
                ...
                if (h_r)
                    REP(res) {
                        VecB4 &index = _mtrl_index.pixB4(res1, i);
                        index = h_r->_mtrl_index.pixB4(0, i);
                ...
                if (h_b)
                    REP(res) {
                        VecB4 &index = _mtrl_index.pixB4(i, 0);
                        index = h_b->_mtrl_index.pixB4(i, res1);
                ...
                if (h_f)
                    REP(res) {
                        VecB4 &index = _mtrl_index.pixB4(i, res1);
                        index = h_f->_mtrl_index.pixB4(i, 0);
                ...
                if (h_lb) {
                    VecB4 &index = _mtrl_index.pixB4(0, 0);
                    index = h_lb->_mtrl_index.pixB4(res1, res1);
                ...
                if (h_rb) {
                    VecB4 &index = _mtrl_index.pixB4(res1, 0);
                    index = h_rb->_mtrl_index.pixB4(0, res1);
                ...
                if (h_lf) {
                    VecB4 &index = _mtrl_index.pixB4(0, res1);
                    index = h_lf->_mtrl_index.pixB4(res1, 0);
                ...
                if (h_rf) {
                    VecB4 &index = _mtrl_index.pixB4(res1, res1);
                    index = h_rf->_mtrl_index.pixB4(0, 0);
...
void Heightmap::createFromQuarter(C Heightmap &src, Bool right, Bool forward, C Game::WorldSettings &settings) {
    ...
    REPD(y, _height.h())
    REPD(x, _height.w())
    _height.pixF(x, y) *= 2; // heightmap scaling is proportional to area size, so we need to adjust it
    ...
    REPA(hqs) {
        ...
        if (hq.h) {
            ...
            REPD(x, temp_size) {
                ...
                if (temp._color.is())
                    _color.pixB3(dest_x, dest_y) = ColorB(temp._color, x, y);
                _mtrl_blend.pixB4(dest_x, dest_y) = MtrlBlendB(temp._mtrl_blend, x, y);
                _mtrl_index.pixB4(dest_x, dest_y).set(getMaterialIndex0(temp.material(mi.x)), getMaterialIndex0(temp.material(mi.y)), getMaterialIndex0(temp.material(mi.z)), getMaterialIndex0(temp.material(mi.w)));
            }
...
UInt Heightmap::memUsage() C {
    return _height.memUsage() + _color.memUsage() + _mtrl_index.memUsage() + _mtrl_blend.memUsage() + _materials.memUsage();
}
...
void Heightmap::height(Int x, Int y, Flt height) {
    if (InRange(x, _height.w()) && InRange(y, _height.h()))
        _height.pixF(x, y) = height;
}
/******************************************************************************/
...
void Heightmap::colorF(Int x, Int y, C Vec &color) {
    if (InRange(x, _height.w()) && InRange(y, _height.h())) // check '_height' size because '_color' may not exist yet, as it is optional
    {
        ...
        if (_color.is()) {
            if (_color.type() != IMAGE_F32_3_SRGB)
                _color.mustCopy(_color, -1, -1, -1, IMAGE_F32_3_SRGB); // convert to high precision
...
void Heightmap::mtrlBlendHP() {
    if (_mtrl_blend.hwType() == IMAGE_R8G8B8A8) {
        Image temp;
        temp.mustCreate(_mtrl_blend.w(), _mtrl_blend.h(), 1, IMAGE_F32_4, IMAGE_SOFT, 1, false); // use 'create' to have 'Exit' on fail, because the methods that use 'mtrlBlendHP', assume that it succeeded
        REPD(y, temp.h())
        REPD(x, temp.w()) {
...
Bool Heightmap::getMaterial(Int x, Int y, VecB4 &mtrl_index, VecB4 &mtrl_blend) C {
    if (InRange(x, _mtrl_index.w()) && InRange(y, _mtrl_index.h())) {
...
Bool Heightmap::getMaterial(Int x, Int y, MaterialPtr &m0, MaterialPtr &m1, MaterialPtr &m2, MaterialPtr &m3, VecB4 &mtrl_blend) C {
    if (InRange(x, _mtrl_index.w()) && InRange(y, _mtrl_index.h())) {
        ...
        if (InRange(mtrl_index.x, _materials))
            m0 = _materials[mtrl_index.x];
        else
            m0 = null;
        if (InRange(mtrl_index.y, _materials))
            m1 = _materials[mtrl_index.y];
        else
            m1 = null;
        if (InRange(mtrl_index.z, _materials))
            m2 = _materials[mtrl_index.z];
        else
            m2 = null;
        if (InRange(mtrl_index.w, _materials))
            m3 = _materials[mtrl_index.w];
        else
            m3 = null;
        return true;
    }
...
void Heightmap::cleanMaterials() // this function should be multi-threaded safe
{
    if (_materials.elms() > 1) {
        Bool is[256];
        ZeroFastN(is, _materials.elms()); // assume all are unused
        Byte remap[256];
...
void Heightmap::cleanColor() {
    if (_color.is()) {
        REPD(y, _color.h())
        REPD(x, _color.w()) {
            Color c = _color.color(x, y);
            if (c.r != 255 || c.g != 255 || c.b != 255)
                return;
        }             // if color is not white, then we need the color map, so return without deleting it
...
#define else
    ...
    {
        if (mtrl_index.x == pix_ind.x)
            mtrl_blend.x += pix_blend.x;
        else if (mtrl_index.x == pix_ind.y)
            mtrl_blend.x += pix_blend.y;
        else if (mtrl_index.x == pix_ind.z)
            mtrl_blend.x += pix_blend.z;
        else if (mtrl_index.x == pix_ind.w)
            mtrl_blend.x += pix_blend.w;
    }
    ...
    {
        if (mtrl_index.y == pix_ind.x)
            mtrl_blend.y += pix_blend.x;
        else if (mtrl_index.y == pix_ind.y)
            mtrl_blend.y += pix_blend.y;
        else if (mtrl_index.y == pix_ind.z)
            mtrl_blend.y += pix_blend.z;
        else if (mtrl_index.y == pix_ind.w)
            mtrl_blend.y += pix_blend.w;
        ...
        {
            if (mtrl_index.z == pix_ind.x)
                mtrl_blend.z += pix_blend.x;
            else if (mtrl_index.z == pix_ind.y)
                mtrl_blend.z += pix_blend.y;
            else if (mtrl_index.z == pix_ind.z)
                mtrl_blend.z += pix_blend.z;
            else if (mtrl_index.z == pix_ind.w)
                mtrl_blend.z += pix_blend.w;
            ...
            {
                if (mtrl_index.w == pix_ind.x)
                    mtrl_blend.w += pix_blend.x;
                else if (mtrl_index.w == pix_ind.y)
                    mtrl_blend.w += pix_blend.y;
                else if (mtrl_index.w == pix_ind.z)
                    mtrl_blend.w += pix_blend.z;
                else if (mtrl_index.w == pix_ind.w)
                    mtrl_blend.w += pix_blend.w;
            }
        }
    }
#if ALLOW_SAME_MTRL
#undef else
    ...
    Flt mul = 0;
    if (Int sum = mtrl_blend.sum())
        mul = 255.0f / sum; // else mtrl_blend.x=; since below we're not using 'mtrl_blend.x' then we don't need to set it
...
struct VtxMtrlCombos {
    ...
    void setUsedByMtrlCombo(UInt mtrl_combo, Memt<MtrlCombo> &mtrl_combos) {
        FREPA(vmc) {
            ...
            if (mc == VMC_NULL) // create new one
            {
                ...
                if (InRange(i + 1, vmc))
                    vmc[i + 1].mtrl_combo = VMC_NULL; // set next as non-existing
                return;
            }
        }
    ...
    Int vtxIndexInMtrlCombo(UInt mtrl_combo) {
        FREPA(vmc) {
            ...
            UInt mc = v.mtrl_combo;
            if (mc == mtrl_combo)
                return v.mc_vtx_index;
            if (mc == VMC_NULL)
                break; // reached the end
        }
        Exit("VMC");
        return 0; // we shouldn't reach this place
    }
};
...
struct Builder {
    ...
    INLINE void processMaterials(VecB4 &mtrl_index, Byte *mi, MB1 *mb, Int mtrls) {
        // set materials blend
        REP(mtrls)
        material_blends[mi[i]] += mb[i]; // 'material_blends' is assumed to be set to zeros on function start
        ...
        REP(mtrls) {
            ...
            if (MB1L b = material_blends[j]) {
                ...
                if (j)                  // don't add null materials (this could happen in case null material had non-zero blend set)
                {
                    ...
                    FREPA(mtrl_blend)
                    if (b > mtrl_blend.c[i]) // check if this material has higher blend than any listed
                    {
        ...
        if (mtrl_index.c[1]) {
            if (mtrl_blend.c[1] <= mtrl_blend.x * MATERIAL2_EPS)
        ...
        if (mtrl_index.c[1]) {
            if (mtrl_index.c[2]) {
                if (mtrl_index.c[3]) {
                    if (mtrl_index.c[3] > mtrl_index.c[0])
                        Swap(mtrl_index.c[3], mtrl_index.c[0]);
                    if (mtrl_index.c[3] > mtrl_index.c[1])
                        Swap(mtrl_index.c[3], mtrl_index.c[1]);
                    if (mtrl_index.c[3] > mtrl_index.c[2])
                        Swap(mtrl_index.c[3], mtrl_index.c[2]);
                }
                if (mtrl_index.c[2] > mtrl_index.c[0])
                    Swap(mtrl_index.c[2], mtrl_index.c[0]);
                if (mtrl_index.c[2] > mtrl_index.c[1])
                    Swap(mtrl_index.c[2], mtrl_index.c[1]);
            }
            if (mtrl_index.c[1] > mtrl_index.c[0])
                Swap(mtrl_index.c[1], mtrl_index.c[0]);
        }
    }
#if VMC_CONTINUOUS
    void setUsedByMtrlCombo(Int x, Int y, Int mtrl_combo, Memt<MtrlCombo> &mtrl_combos) {
        ...
        if (i == ~0) // not yet added
        {
            ...
            for (;;) {
                ...
                if (c.mtrl_combo == mtrl_combo)
                    return; // already listed by this mtrl combo
                if (c.next_add)
                    i += c.next_add;
    ...
    Int getMtrlComboForTri(C VecI2 &p0, C VecI2 &p1, C VecI2 &p2) {
        ...
        if ((mi[0] = mtrlIndex(p0.x, p0.y)).any() // get material indexes
            && (mi[1] = mtrlIndex(p1.x, p1.y)).any() && (mi[2] = mtrlIndex(p2.x, p2.y)).any()) {
    ...
    Int getMtrlComboForQuad(Int x, Int y, Int x1, Int y1) {
        ...
        if ((mi[0] = mtrlIndex(x, y1)).any() // get material indexes
            && (mi[1] = mtrlIndex(x1, y1)).any() && (mi[2] = mtrlIndex(x1, y)).any() && (mi[3] = mtrlIndex(x, y)).any()) {
    ...
    NOINLINE void downSampleNormalsB(Int res, Int step, Int edge_step) {
        ...
        for (Int y = step, ty = 0; y < res_step; y += step, ty++)
            for (Int x = step, tx = 0; x < res_step; x += step, tx++)
                vtx_nrm_b[y][x] = temp[ty][tx];
...
NOINLINE Bool Heightmap::buildEx2(Mesh &mesh, Int quality, UInt flag, BuildMem &mem, C Heightmap *h_l, C Heightmap *h_r, C Heightmap *h_b, C Heightmap *h_f, C Heightmap *h_lb, C Heightmap *h_lf, C Heightmap *h_rb, C Heightmap *h_rf, C Sphere *sphere) // this function should be multi-threaded safe
{
    ...
    if (h_l && h_l->resolution() != res)
        h_l = null;
    if (h_r && h_r->resolution() != res)
        h_r = null;
    if (h_b && h_b->resolution() != res)
        h_b = null;
    if (h_f && h_f->resolution() != res)
        h_f = null;
    if (h_lb && h_lb->resolution() != res)
        h_lb = null;
    if (h_lf && h_lf->resolution() != res)
        h_lf = null;
    if (h_rb && h_rb->resolution() != res)
        h_rb = null;
    if (h_rf && h_rf->resolution() != res)
        h_rf = null;
    ...
    for (Int y = 0; y < res; y += step) {
        ...
        for (Int x = 0; x < res; x += step) {
...
#if MTRL_BLEND_HP
            if (_mtrl_blend.hwType() == IMAGE_F32_4)
                builder.mtrl_blend[y][x] = _mtrl_blend.pixF4(x, y);
            else
                builder.mtrl_blend[y][x] = _mtrl_blend.pixB4(x, y); // doesn't need to be scaled if all blends have the same scale
#else
            ...
            Flt l, r;
            if (x != 0)
                l = _height.pixF(x - 1, y);
            else if (hl)
                l = hl->pixF(res - 2, y);
            else {
                nrm.x = (_height.pixF(0, y) - _height.pixF(1, y)) * 2;
                goto processed_x;
            }
            if (x != res1)
                r = _height.pixF(x + 1, y);
            else if (hr)
                r = hr->pixF(1, y);
            else {
                nrm.x = (_height.pixF(res - 2, y) - _height.pixF(res1, y)) * 2;
                goto processed_x;
            }
            nrm.x = l - r;
            sharpness += Abs(h - Avg(l, r));
            sharpness_count++;
        processed_x:

            Flt b, f;
            if (y != 0)
                b = _height.pixF(x, y - 1);
            else if (hb)
                b = hb->pixF(x, res - 2);
            else {
                nrm.z = (_height.pixF(x, 0) - _height.pixF(x, 1)) * 2;
                goto processed_z;
            }
            if (y != res1)
                f = _height.pixF(x, y + 1);
            else if (hf)
                f = hf->pixF(x, 1);
            else {
                nrm.z = (_height.pixF(x, res - 2) - _height.pixF(x, res1)) * 2;
                goto processed_z;
            }
            nrm.z = b - f;
            sharpness += Abs(h - Avg(b, f));
            sharpness_count++;
        processed_z:
            ...
            nrm.normalize();
            if (nrm_f)
                builder.vtx_nrm_f[y][x] = nrm;
            else
                builder.vtx_nrm_b[y][x] = NrmToByte4(nrm);
                    ...
                    for (Int dx = -ao_range; dx <= ao_range; dx += ao_step) {
                        ...
                        // if(p>0)
                        {
                            ...
                            if (tx < 0) {
                                if (ty < 0) {
                                    if (hlb)
                                        th = hlb->pixF(tx + res1, ty + res1);
                                    else
                                        th = h + (_height.pixF(0, 0) - _height.pixF(1, 1)) * dist;
                                } else if (ty >= res) {
                                    if (hlf)
                                        th = hlf->pixF(tx + res1, ty - res1);
                                    else
                                        th = h + (_height.pixF(0, res1) - _height.pixF(1, res1 - 1)) * dist;
                                } else {
                                    if (hl)
                                        th = hl->pixF(tx + res1, ty);
                                    else
                                        th = _height.pixF(x, ty) - (_height.pixF(0, ty) - _height.pixF(1, ty)) * dx;
                                } // here 'dx' is negative
                            } else if (tx >= res) {
                                if (ty < 0) {
                                    if (hrb)
                                        th = hrb->pixF(tx - res1, ty + res1);
                                    else
                                        th = h + (_height.pixF(res1, 0) - _height.pixF(res1 - 1, 1)) * dist;
                                } else if (ty >= res) {
                                    if (hrf)
                                        th = hrf->pixF(tx - res1, ty - res1);
                                    else
                                        th = h + (_height.pixF(res1, res1) - _height.pixF(res1 - 1, res1 - 1)) * dist;
                                } else {
                                    if (hr)
                                        th = hr->pixF(tx - res1, ty);
                                    else
                                        th = _height.pixF(x, ty) + (_height.pixF(res1, ty) - _height.pixF(res1 - 1, ty)) * dx;
                                } // here 'dx' is positive
                            } else {
                                if (ty < 0) {
                                    if (hb)
                                        th = hb->pixF(tx, ty + res1);
                                    else
    ...
    if (sharpness_count)
        sharpness *= Flt(Sqr(res1)) / sharpness_count // this formula gives the same sharpness regardless of heightmap resolution
                     / div_sharpness;                 // also adjust by some factor
    Flt size;
    if (sphere) {
        size = sphere->planet_radius / sphere->areas * PI_2;
        sharpness /= size;
    }
    ...
    // downsample
    REP(quality)
    builder.downSampleMaterials(res, 1 << i);
    Image color;
    if (T._color.is()) {
        T._color.copy(color);
        REP(quality)
        color.resize(color.w() / 2 + 1, color.h() / 2 + 1, FILTER_LINEAR, IC_CLAMP | IC_KEEP_EDGES);
    }
    ...
    FREPD(l, mesh.lods()) {
        ...
        if (l) {
            ...
            // downsample
            if (nrm_f)
                builder.downSampleNormalsF(res, step << l, Step); // normals
            else
                builder.downSampleNormalsB(res, step << l, Step);                                        // normals
        ...
        for (Int y = from; y < to; y++)
            for (Int x = from; x < to; x++)
                builder.map_mtrl_combo[y][x] = builder.getMtrlComboForQuad(x << L, y << L, (x + 1) << L, (y + 1) << L);
        ...
        // create mesh lods
        if (lod.parts.elms() != builder.mtrl_combos.elms())
            lod.create(builder.mtrl_combos.elms());
        ...
        FREPA(builder.mtrl_combos) {
            ...
            if (!VTX_HEIGHTMAP)
                mesh_flag |= VTX_TEX0 | VTX_TAN;
            if (ambient_occlusion || color.is())
                mesh_flag |= VTX_COLOR;
            if (mtrl_combo.mtrl_index.y /* || mtrl_combo.mtrl_index.z || mtrl_combo.mtrl_index.w*/)
            ...
            if (soft) {
                ...
                if (!mshr.create(mtrl_combo.vtxs, mtrl_combo.tris, mesh_flag, VTX_COMPRESS))
                    return false;

                if (mshr.vtxs() && !mshr.vtxLock(LOCK_WRITE))
                    return false;
                if (mshr.tris() && !mshr.indLock(LOCK_WRITE))
                    return false;
        ...
        if (soft) {
            ...
            REPD(x, res) {
#if VMC_CONTINUOUS
                UInt i = builder.vmc_first[y][x];
                if (i != ~0)
                    for (;;) {
                        VtxMtrlCombo &vmc = builder.vmc[i];
                        Int mc = vmc.mtrl_combo;
#else
                VtxMtrlCombos &vmc_xy = mem.vmc[y][x];
                FREPA(vmc_xy.vmc) {
                    VtxMtrlCombo &vmc = vmc_xy.vmc[i];
                    Int mc = vmc.mtrl_combo;
                    if (mc == VMC_NULL)
                        break;
#endif
                        ...
                        if (mshb.vtx._elms) // this can be 0 for null materials
                        {
...
#else
                            if (_mtrl_blend.hwType() == IMAGE_F32_4)
                                vtx_mtrl = VtxMtrlBlend(mc_mtrl, hm_mtrl, _mtrl_blend.pixF4(x, y));
                            else
                                vtx_mtrl = VtxMtrlBlend(mc_mtrl, hm_mtrl, _mtrl_blend.pixB4(x, y));
#endif
                            }
                            if (mshb.vtx.color()) // color
                            {
                                ...
                                if (ambient_occlusion) {
                                    ...
                                    if (color.is())
                                        vtx_color = ColorBrightnessB(color.color(x >> L, y >> L), occl);
                                    else
                                        vtx_color.set(occl);
                                } else
                                    vtx_color = color.color(x >> L, y >> L); // in this case 'Image color' always exists, because "mtrl_combo.ofs_color>=0" implies that vertex contains color, and it was created only if 'ambient_occlusion' or 'Image color' exists
                            }
                        }
#if VMC_CONTINUOUS
                        if (vmc.next_add)
            ...
            // set faces
            {
                ...
                for (Int y = from; y < to; y++)
                    for (Int x = from; x < to; x++)
                        builder.setQuadFaceSoft(x << L, y << L, (x + 1) << L, (y + 1) << L, builder.map_mtrl_combo[y][x]);
            ...
            // set faces
            {
                ...
                for (Int y = from; y < to; y++)
                    for (Int x = from; x < to; x++)
                        builder.setQuadFace(x << L, y << L, (x + 1) << L, (y + 1) << L, builder.map_mtrl_combo[y][x]);
...
void Heightmap::build(Mesh &dest_mesh, Int quality, UInt flag, C Heightmap *l, C Heightmap *r, C Heightmap *b, C Heightmap *f, C Heightmap *lb, C Heightmap *lf, C Heightmap *rb, C Heightmap *rf, Mems<Byte> *temp_mem, C Sphere *sphere) {
    if (is()) {
        ...
        Ptr build_mem;
        if (temp_mem)
            build_mem = temp_mem->setNum(SIZE(BuildMem)).data();
        else
            build_mem = Alloc(SIZE(BuildMem));
...
static Bool SaveAs(C Image &image, File &f, IMAGE_TYPE type) {
    if (!image.is() || image.type() == type)
        return image.saveData(f);
    Image temp;
    if (image.copy(temp, -1, -1, -1, type))
        return temp.saveData(f);
    return false;
}
static Bool LoadOldAs(Image &image, File &f, IMAGE_TYPE type) // !! uses old '_loadData' !!
{
    if (!image._loadData(f))
        return false;
    if (image.is() && image.type() != type)
        if (!image.copy(image, -1, -1, -1, type))
            return false;
    return true;
}
static Bool AddEmptyAlpha(Image &image) {
    if (image.copy(image, -1, -1, -1, IMAGE_R8G8B8A8, IMAGE_SOFT, 1))
        if (image.lock()) {
            REPD(y, image.lh())
            REPD(x, image.lw())
            image.pixC(x, y).a = 0;
...
Bool Heightmap::load(File &f, CChar *path) {
    ...
    case 1: {
        if (_height._loadData(f))
            if (LoadOldAs(_color, f, IMAGE_R8G8B8_SRGB))
                if (LoadOldAs(_mtrl_index, f, IMAGE_R8G8B8))
        ...
        if (_height._loadData(f))
            if (LoadOldAs(_color, f, IMAGE_R8G8B8_SRGB))
                if (LoadOldAs(_mtrl_index, f, IMAGE_B8G8R8))
                    if (LoadOldAs(_mtrl_blend, f, IMAGE_R8G8B8)) {
                        if (_mtrl_index.hwType() == IMAGE_B8G8R8)
                            _mtrl_index._type = _mtrl_index._hw_type = IMAGE_R8G8B8; // old version used 'IMAGE_B8G8R8' and the index for the first material was stored in first byte which is blue
                        if (_materials.loadOld(f, path)) {
                            ...
                            REPD(x, _mtrl_index.w()) {
                                ...
                                {
                                    if (v.x >= _materials.elms())
                                        v.x = 0;
                                    if (v.y >= _materials.elms())
                                        v.y = 0;
                                    if (v.z >= _materials.elms())
                                        v.z = 0;
                                }
                            }
                            if (f.ok())
