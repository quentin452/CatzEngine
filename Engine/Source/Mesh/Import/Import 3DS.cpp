...
#pragma pack(pop)
    ...
    if (f.read(name)) {
        ...
        for (; !f.end();) {
            Chunk chunk;
            f.get(&chunk, SIZE(chunk));
            Long end = f.pos() + Unaligned(chunk.length) - SIZE(chunk);
            ...
            case CHUNK3DS_MTRL_BUMP_MAP:
                last_tex = Unaligned(chunk.id);
                break;

            case CHUNK3DS_EDIT_OBJECT: {
                last_name.clear();
                for (Char8 c; c = f.getByte();)
                    last_name += c;
            } break;
            ...
            case CHUNK3DS_PIVOT:
                if (Node *node = nodes.addrLast()) {
                    f >> node->pivot;
                }
                goto skip;

            case CHUNK3DS_TRI_VERTEXLIST:
                if (Mesh *mesh = meshes.addrLast())
                    if (!mesh->vtxs()) {
                        mesh->vtx._elms = f.getUShort();
                        mesh->include(VTX_POS);
                        f.getN(mesh->vtx.pos(), mesh->vtxs());
                    }
                goto skip;

            case CHUNK3DS_TRI_TEXCOORD:
                if (Mesh *mesh = meshes.addrLast())
                    if (!mesh->vtx.tex0()) {
                        Int vtxs = f.getUShort();
                        if (vtxs == mesh->vtxs()) {
                            mesh->include(VTX_TEX0);
                            f.getN(mesh->vtx.tex0(), mesh->vtxs());
                        }
                    }
                goto skip;
                ...
                if (Mesh *mesh = meshes.addrLast())
                    if (!mesh->tris()) {
                        ...
                        FREPA(mesh->tri) {
                            U16 ind[4];
                            f >> ind;
                            mesh->tri.ind(i).set(ind[0], ind[1], ind[2]);
                        }
                    }
            }
                goto skip;

            case CHUNK3DS_TRI_MTRLLIST:
                if (Mesh *mesh = meshes.addrLast()) {
                    Str material_name;
                    for (Char8 c; c = f.getByte();)
                        material_name += c;
                    ...
                    if (materials) {
                        REPA(materials)
                        if (Equal(materials[i].name, material_name)) {
                            material_index = i;
                            break;
                        }
                        if (material_index < 0) {
                            material_index = materials.elms();
                            materials.New().name = material_name;
                        }
                    }
                    Int tris = f.getUShort();
                    if (!mesh->tri.id()) {
                    ...
                    REP(tris) {
                        Int tri = f.getUShort();
                        if (InRange(tri, mesh->tri))
                            mesh->tri.id(tri) = material_index;
                    }
                }
                goto skip;

            case CHUNK3DS_TRI_SMOOTH_GROUPS:
                if (Mesh *mesh = meshes.addrLast()) // a list of UInt's for each triangle, smooth group is a 32-bit UInt value, each bit specifies a smoothing group, if 2 tris share a smoothing group, then the vertex normals should be smoothened
                {
            ...
            case CHUNK3DS_MTRL_TEX_NAME: {
                ...
                if (materials.elms())
                    switch (last_tex) {
                    case CHUNK3DS_MTRL_COLOR_MAP:
                        materials.last().color_map = tex_name;
                        break;
                    case CHUNK3DS_MTRL_SPEC_MAP:
                        materials.last().smooth_map = tex_name;
                        break;
                    case CHUNK3DS_MTRL_ALPHA_MAP:
                        materials.last().alpha_map = tex_name;
                        break;
                    case CHUNK3DS_MTRL_REFL_MAP:
                        materials.last().metal_map = tex_name;
                        break;
                    case CHUNK3DS_MTRL_BUMP_MAP:
                        materials.last().bump_map = tex_name;
                        break;
                    }
            } break;
        ...
        if (mesh) {
            ...
            REPA(meshes) {
                ...
                {
                    Matrix mirrored = mesh.matrix;
                    mirrored.x.chs();
                    Matrix m = ~mesh.matrix * mirrored;
                ...
                if (!mesh.vtx.nrm()) {
                    if (mesh.tri_smooth_groups.elms() && mesh.tri.ind()) {
                        ...
                        REPA(mesh.tri)                // iterate all faces
                        {
                            ...
                            C VecI &t = tri[i];
                            REPA(t)
                            mesh.vtx.material(t.c[i]).u = group; // apply smoothing groups onto triangle vertexes
                        }
                ...
                if (!mesh.vtx.tan() || !mesh.vtx.bin())
                    mesh.setTanBin();                         // if(!mesh.vtx.tan())mesh.setTangents(); if(!mesh.vtx.bin())mesh.setBinormals(); // need to call before 'weldVtx' to don't remove too many vertexes
            ...
            {
                Node &node = nodes[i];
                REPA(meshes) // find mesh for this node
                {
                    C Mesh &mesh = meshes[i];
                    if (Equal(mesh.name, node.name, true)) // if they have the same name
                    {
                        ...
                        {               // copy the complete mesh
                            ...
                            if (part_material_index)
                                part_material_index.add(-1); // set material as -1
                        } else                               // if it has ID's
                            for (Int i = -1; i <= max_id; i++)
                                if (mesh.hasID(i)) // iterate through all id's and copy only those which have faces
                                {
                                    MeshPart &part = parts.New();
                                    Set(part.name, mesh.name);
                                    ...
                                    part.base.transform(m);
                                    if (part_material_index)
                                        part_material_index.add(i); // set i-th material
                                }
                        break;
                    }
                }
