...
struct Mtrl : XMaterial {
    ...
    void import(FileTextEx &f, C Str &path) {
        for (; f.level();) {
            if (f.cur("*MATERIAL_TWOSIDED"))
                cull = false;
            else if (f.cur("*MATERIAL_NAME"))
                name = f.getName();
            else
    ...
    void copyTo(MemPtr<XMaterial> materials) {
        global_index = materials.elms();
        materials.New() = T;
        FREPAO(sub_mtrl).copyTo(materials);
    }
};
struct Node {
    Int material;
...
static void ImportASE(FileTextEx &f, Ase &ase, C Str &path) {
    for (; f.level();) {
        if (f.cur("*MATERIAL_LIST") && f.getIn()) {
            for (; f.level();) {
                if (f.cur("*MATERIAL") && f.getIn())
                    ase.mtrl.New().import(f, path);
            }
        } else if (f.cur("*GROUP") && f.getIn())
            ImportASE(f, ase, path);
        else if (f.cur("*GEOMOBJECT") && f.getIn()) {
            Node &node = ase.node.New();
            for (; f.level();) {
                if (f.cur("*NODE_NAME"))
                    node.name = f.getName();
                else if (f.cur("*NODE_PARENT"))
                    node.parent = f.getName();
                else if (f.cur("*NODE_TM") && f.getIn()) {
                    for (; f.level();) {
                        if (f.cur("*TM_ROW0"))
                            f.get(node.matrix.x);
                        else if (f.cur("*TM_ROW1"))
                            f.get(node.matrix.y);
                        else if (f.cur("*TM_ROW2"))
                            f.get(node.matrix.z);
                        else if (f.cur("*TM_ROW3"))
                            f.get(node.matrix.pos);
                    }
                } else if (f.cur("*MESH") && f.getIn()) {
                    for (; f.level();) {
                        ...
                        } else if (f.cur("*MESH_FACE_LIST") && f.getIn()) {
                            for (; f.level();) {
                                if (f.cur("*MESH_FACE")) {
                                    ...
                                    f.getWord();
                                    tri.z = f.getInt();
                                } else if (f.cur("*MESH_MTLID")) {
                                    if (node.tri.elms())
                                        node.tri.last().sub_material = f.getInt();
                                }
                            }
                        } else if (f.cur("*MESH_TVERTLIST") && f.getIn()) {
                            for (; f.level();) {
                                if (f.cur("*MESH_TVERT")) {
                                    f.getWord();
                                    Vec2 &tex = node.tex.New();
                                    f.get(tex);
                                    tex.y = 1 - tex.y;
                                }
                            }
                        } else if (f.cur("*MESH_TFACELIST") && f.getIn()) {
                            for (; f.level();) {
                                if (f.cur("*MESH_TFACE")) {
                                    f.getWord();
                                    f.get(node.ttex.New());
                        ...
                        } else if (f.cur("*MESH_NORMALS") && f.getIn()) {
                            node.nrm.setNum(node.tri.elms());
                            for (Int face = -1, vtx; f.level();) {
                                if (f.cur("*MESH_FACENORMAL")) {
                                    face = f.getInt();
                                    vtx = 0;
                                } else if (f.cur("*MESH_VERTEXNORMAL") && InRange(face, node.tri.elms()) && vtx < 3) {
                                    f.getWord();
                                    f.get(node.nrm[face].nrm[vtx++]);
                                }
                            }
                        }
                    }
                } else if (f.cur("*MATERIAL_REF"))
                    node.material = f.getInt();
            }
        }
    }
}
        ...
        if (mesh) {
            FREPA(ase.node) {
                ...
                if (InRange(src.material, ase.mtrl) && ase.mtrl[src.material].sub_mtrl.elms()) // if selected material has sub-materials
                {
                    base.include(TRI_ID);
                    FREPA(base.tri)
                    base.tri.id(i) = src.tri[i].sub_material;
                }
                if (!base.vtx.nrm())
                ...
                if (base.tri.id()) // we need to split into parts
                {
                    ...
                    if (part.is()) {
                        ...
                        if (part_material_index)
                            part_material_index.add(InRange(src.material, ase.mtrl) ? ase.mtrl[src.material].global_index : -1);
                    }
                }
            }
