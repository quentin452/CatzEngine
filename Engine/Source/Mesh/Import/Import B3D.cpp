...
static void ImportB3DNode(File &f, Chunk &c, Memx<NODE> &nodes, NODE *parent, TextData &log) {
    ...
    if (parent)
        node.global_matrix *= parent->global_matrix;
    ...
    for (; !f.end() && c.size > f.pos();) {
        ...
        if (c.equal("NODE")) {
        ...
        } else if (c.equal("MESH")) {
            c.size += f.pos();
            MESH &mesh = node.meshes.New();
            f >> mesh.brush;
            for (; !f.end() && c.size > f.pos();) {
                Chunk c;
                f >> c;
                if (c.equal("VRTS")) {
                    ...
                    f >> flags >> tex_coord_sets >> tex_coord_set_size;
                ...
                } else if (c.equal("TRIS")) {
                    c.size += f.pos();
                    TRIS &tris = mesh.tris.New();
                    f >> tris.brush;
                    for (; !f.end() && c.size > f.pos();)
                        f >> tris.ind.New(); // range checks are done later
                } else {
                    f.skip(c.size); // skip chunk
                }
            }
        } else if (c.equal("BONE")) {
            node.bone = true;
            c.size += f.pos();
            for (; !f.end() && c.size > f.pos();)
                f >> node.bones.New();
        } else if (c.equal("KEYS")) {
            c.size += f.pos();
            ...
            for (; !f.end() && c.size > f.pos();) {
                ...
                if (key.has_scale)
                    f >> frame.scale;
                if (key.has_rot) {
                    f >> frame.quat.w >> frame.quat.xyz;
                    CHS(frame.quat.w);
                }
            }
        } else if (c.equal("ANIM")) {
            f >> node.anims.New();
        } else {
            f.skip(c.size); // skip chunk
        }
...
static void ProcessNodes(Memx<NODE> &nodes, Memc<MeshPart> &parts, MemPtr<Int> part_material_index, Memc<TEXS> &texs, Memc<BRUS> &brus, Skeleton &skeleton, MemtN<BoneType, 256> &old_to_new) {
    FREPA(nodes) {
        NODE &node = nodes[i];
        Matrix3 matrix3 = node.global_matrix;
        matrix3.normalize();
        FREPA(node.meshes) {
            MESH &mesh = node.meshes[i];
            if (mesh.vrt.elms())
                FREPA(mesh.tris) {
                    TRIS &tris = mesh.tris[i];
                    if (tris.ind.elms()) {
                        MeshPart &part = parts.New();
                        Set(part.name, node.name);
                        if (part_material_index)
                            part_material_index.add(tris.brush);
                            ...
                            REPA(mesh.bones) {
                                ...
                                if (InRange(node.bone_index, old_to_new)) {
                                    ...
                                    FREPA(node.bones) {
                                        ...
                                        if (InRange(bone.vertex_id, mesh.vrt))
                                            mesh.vrt[bone.vertex_id].skin.New().set(matrix, bone.weight);
                                    }
                                }
                            }
                            REPA(mesh.vrt)
                            SetSkin(mesh.vrt[i].skin, mshb.vtx.matrix(i), mshb.vtx.blend(i), &skeleton);
                        }

                        // triangles
                        Bool invalid = false;
                        FREPA(tris.ind) {
                            VecI &ind = mshb.tri.ind(i);
                            ind = tris.ind[i];
                            REPA(ind)
                            if (!InRange(ind.c[i], mshb.vtx)) {
                                ind.zero();
                                invalid = true;
                                break;
                            }
                        ...
                        if (Memc<Int> *tex_id = (InRange(tris.brush, brus) ? &brus[tris.brush].tex_id : null)) {
                            ...
                            switch (tex_id->elms()) {
                            case 1:
                                if (tx[0]->second_tex && mshb.vtx.tex1())
                                    Swap(mshb.vtx._tex0, mshb.vtx._tex1);
                                break;
                            case 2:
                                if (tx[0]->second_tex && !tx[1]->second_tex)
                                    Swap(mshb.vtx._tex0, mshb.vtx._tex1);
                                break;
                            }
                        }
                        if (!mshb.vtx.nrm())
                            mshb.setNormalsAuto(EPS_NRM_AUTO, EPSD); // use small pos epsilon in case mesh is scaled down, call before 'setTanBin' as it depends on normals
                        if (!mshb.vtx.tan() || !mshb.vtx.bin())
                            mshb.setTanBin();                         // if(!mshb.vtx.tan())mshb.setTangents(); if(!mshb.vtx.bin())mshb.setBinormals(); // need to call before 'weldVtx' to don't remove too many vertexes
...
static void CreateAnimation(C Memc<MESH_BONE> &mesh_bone, Skeleton &skeleton, XAnimation &animation) {
    ...
    FREPA(mesh_bone) {
        ...
        if (node.keys[i].has_rot) {
            ...
            FREPA(frames) {
                ...
                {
                    if (node.parent)
                        orn.orn *= ~node.parent->global_matrix.orn();          // transform bone from global space to local space relative to parent
                    ...
                    if (node.parent)
                        orn.orn *= node.parent->global_matrix.orn(); // transform bone from local space to global space
                }
        ...
        if (node.keys[i].has_pos) {
            ...
            FREPA(frames) {
                ...
                if (node.parent)
                    pos.pos *= node.parent->global_matrix.orn();
                if (parent)
    ...
    // scale length
    if (mesh_bone.elms())
        for (NODE *node = mesh_bone[0].node; node; node = node->parent) {
            if (node->anims.elms()) {
    ...
    FREPA(nodes) {
        ...
        FREPA(node.meshes) {
            ...
            if (mesh.bones.elms()) {
                ...
                if (animation)
                    CreateAnimation(mesh.bones, skeleton, *animation);
                return true;
            }
        }
        if (CreateSkeleton(node.nodes, skeleton, animation))
            return true;
    }
    return false;
}
        ...
        if (c.equal("BB3D")) {
            ...
            for (; !f.end();) {
                ...
                if (c.equal("TEXS")) {
                    ...
                    for (; !f.end() && c.size > f.pos();) {
                        ...
                        b.name = GetStr(f);
                        Int blend, fx;
                        f >> b.color >> b.specular >> blend >> fx;
                        REP(n_texs) {
                            Int tex_id;
                            f >> tex_id;
                            if (InRange(tex_id, texs))
                                b.tex_id.add(tex_id);
                        }
            ...
            if (skel) {
                ...
                if (mesh) {
                    ...
                    mesh->create(parts.elms());
                    FREPA(*mesh)
                    Swap(mesh->parts[i], parts[i]);
            ...
            if (materials)
                FREPA(brus) {
                    ...
                    if (b.tex_id.elms() >= 1)
                        material.color_map = texs[b.tex_id[0]].file;
