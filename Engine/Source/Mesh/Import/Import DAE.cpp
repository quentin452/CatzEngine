...
struct DAE {
    ...
    struct Channel {
        ...
        Channel() {
            time = transform = move = move_x = move_y = move_z = rot_z = rot_y = rot_x = scale_axis_r = scale = scale_axis = alpha = null;
            node = null;
        }
    };
    struct Animation {
    ...
    struct Source {
        ...
        CChar8 *getName(Int i) {
            i *= stride;
            return InRange(i, name_array) ? name_array[i + 0]() : null;
        }
        Flt getFloat(Int i) {
            i *= stride;
            return InRange(i, float_array) ? float_array[i + 0] : 0;
        }
        Vec2 getVec2(Int i) {
            i *= stride;
            return (InRange(i, float_array) && InRange(i + 1, float_array)) ? Vec2(float_array[i + 0], float_array[i + 1]) : 0;
        }
        Vec getVec(Int i) {
            i *= stride;
            return (InRange(i, float_array) && InRange(i + 2, float_array)) ? Vec(float_array[i + 0], float_array[i + 1], float_array[i + 2]) : VecZero;
        }
        Vec4 getVec4(Int i) {
            i *= stride;
            return (InRange(i, float_array) && InRange(i + 3, float_array)) ? Vec4(float_array[i + 0], float_array[i + 1], float_array[i + 2], float_array[i + 3]) : 0;
        }
        Matrix getMatrix(Int i) {
            i *= stride;
            if (InRange(i, float_array) && InRange(i + 11, float_array)) {
                Matrix m;
    ...
    struct Node {
        ...
        void setWorldMatrix() {
            ...
            REPAO(nodes).setWorldMatrix();
        }
        void defaultPose() {
            anim_matrix = local_matrix;
            REPAO(nodes).defaultPose();
        }
        ...
        void adjustBoneIndex(Int bone_index) {
            if (bone_index >= BONE_NULL)
                bone_index = -1;
            T.bone_index = bone_index;
            if (bone_index < 0)
                bone = false; // if we're clearing bone index, then it means we don't want this as a bone
        }
...
static void Import(Memc<Str8> &texts, XmlNode &node) {
    texts.setNum(node.data.elms());
    REPAO(texts) = node.data[i];
}
static void Import(Memc<Int> &values, XmlNode &node) {
    values.setNum(node.data.elms());
    REPAO(values) = TextInt(node.data[i]);
}
static void Import(Memc<Flt> &values, XmlNode &node) {
    values.setNum(node.data.elms());
    REPAO(values) = TextFlt(node.data[i]);
}
...
Int DAE::findMaterial(CChar *symbol, Memb<InstanceMaterial> &im) {
    REPA(im)
    if (Equal(im[i].material_symbol, symbol)) {
...
void DAE::Sampler::import(XmlNode &node, Memb<Source> &source) {
    ...
    if (node.nodes[i].name == "input") {
        Input input;
        input.import(node.nodes[i], source);
        if (input.source)
...
void DAE::Channel::import(XmlNode &node, Memb<Sampler> &sampler, DAE &dae) {
    Source *output = null;
    if (XmlParam *p = node.findParam("source"))
        if (Sampler *s = FindSampler(sampler, SkipStart(p->value, '#'))) {
            T.time = s->input;
            output = s->output;
        }
    if (XmlParam *p = node.findParam("target")) {
        ...
        if (Equal(type, "transform") || Equal(type, "matrix"))
            transform = output;
        else if (Equal(type, "translation"))
            move = output;
        else if (Equal(type, "translation.X") || Equal(type, "translate.X") || Equal(type, "transform(3)(0)"))
            move_x = output;
        else if (Equal(type, "translation.Y") || Equal(type, "translate.Y") || Equal(type, "transform(3)(1)"))
            move_y = output;
        else if (Equal(type, "translation.Z") || Equal(type, "translate.Z") || Equal(type, "transform(3)(2)"))
            move_z = output;
        else if (Equal(type, "RotZ.ANGLE") || Equal(type, "RotateZ.ANGLE") || Equal(type, "rotationZ.ANGLE"))
            rot_z = output;
        else if (Equal(type, "RotY.ANGLE") || Equal(type, "RotateY.ANGLE") || Equal(type, "rotationY.ANGLE"))
            rot_y = output;
        else if (Equal(type, "RotX.ANGLE") || Equal(type, "RotateX.ANGLE") || Equal(type, "rotationX.ANGLE"))
            rot_x = output;
        else if (Equal(type, "ScaleAxisR"))
            scale_axis_r = output;
        else if (Equal(type, "scale"))
            scale = output;
        else if (Equal(type, "ScaleAxis"))
            scale_axis = output;
        else if (Equal(type, "transparency"))
            alpha = output;
    }

    if (time && time->float_array.elms())
        MAX(dae.duration, time->float_array.last());
#if 0 && DEBUG
...
void DAE::Animation::import(XmlNode &node, DAE &dae) {
    ...
    FREPA(node.nodes) {
        ...
        if (n.name == "source")
            source.New().import(n);
        else if (n.name == "sampler")
            sampler.New().import(n, source);
        else if (n.name == "channel")
            channel.New().import(n, sampler, dae);
        else if (n.name == "animation")
            animation.New().import(n, dae);
    }
}
void DAE::Image::import(XmlNode &node) {
    if (XmlParam *id = node.findParam("id"))
        T.id = id->asText();
    if (XmlParam *name = node.findParam("name"))
        T.name = name->asText();

    if (XmlNode *n = node.findNode("init_from"))
        if (n->data.elms())
            texture = SkipStartPath(SkipStartPath(SkipStartPath(Replace(n->data[0], "%20", " "), "file:///"), "file://"), "./");
}
void DAE::Material::import(XmlNode &node) {
    ...
    if (XmlNode *n = node.findNode("instance_effect"))
        if (XmlParam *p = n->findParam("url"))
            fx = SkipStart(p->value, '#');
}
void DAE::NewParam::import(XmlNode &node, Memb<NewParam> &param, DAE &dae) {
    if (XmlParam *sid = node.findParam("sid"))
        T.sid = sid->asText();

    if (XmlNode *n = node.findNode("surface"))
        if (XmlNode *i = n->findNode("init_from"))
            Import(image_id, *i);
    if (XmlNode *n = node.findNode("sampler2D"))
...
void DAE::Effect::import(XmlNode &node, DAE &dae) {
    ...
    if (XmlNode *n = node.findNode("profile_COMMON")) {
        ...
        if (XmlNode *technique = n->findNode("technique")) {
            ...
            if (!sub)
                sub = technique->findNode("phong");
            if (sub) {
                if (XmlNode *ambient = sub->findNode("ambient"))
                    if (XmlNode *color = ambient->findNode("color"))
                        Import(T.ambient, *color);
                if (XmlNode *specular = sub->findNode("specular"))
                    if (XmlNode *color = specular->findNode("color"))
                        Import(T.specular, *color);
                if (XmlNode *shininess = sub->findNode("shininess"))
                    if (XmlNode *n = shininess->findNode("float"))
                        Import(T.shininess, *n);
                if (XmlNode *diffuse = sub->findNode("diffuse")) {
                    if (XmlNode *color = diffuse->findNode("color"))
                        Import(T.color, *color);
                    if (XmlNode *texture = diffuse->findNode("texture"))
                        if (XmlParam *t = texture->findParam("texture")) {
                            color_map_image_id = t->value;
                            REPA(param)
                            if (param[i].sid == t->value) {
                                color_map_image_id = param[i].image_id;
                                break;
                            }
                        }
                }
                if (XmlNode *transparent = sub->findNode("transparent")) {
                    if (XmlNode *texture = transparent->findNode("texture"))
                        if (XmlParam *t = texture->findParam("texture")) {
                            alpha_map_image_id = t->value;
                            REPA(param)
                            if (param[i].sid == t->value) {
                                alpha_map_image_id = param[i].image_id;
                                break;
                            }
                        }
                }
            ...
            if (XmlNode *extra = technique->findNode("extra"))
                if (XmlNode *technique = extra->findNode("technique")) {
                    XmlNode *spec = technique->findNode("spec_level");
                    if (!spec)
                        spec = technique->findNode("specularLevel");
                    if (spec) {
                        if (XmlNode *n = spec->findNode("float"))
                            Import(spec_level, *n);
                        if (XmlNode *texture = spec->findNode("texture"))
                            if (XmlParam *t = texture->findParam("texture")) {
                                specular_map_image_id = t->value;
                                REPA(param)
                                if (param[i].sid == t->value) {
                                    specular_map_image_id = param[i].image_id;
                                    break;
                                }
                            }
                    }
                    if (XmlNode *bump = technique->findNode("bump")) {
                        if (XmlNode *texture = bump->findNode("texture"))
                            if (XmlParam *t = texture->findParam("texture")) {
                                bump_map_image_id = t->value;
                                REPA(param)
                                if (param[i].sid == t->value) {
                                    bump_map_image_id = param[i].image_id;
                                    break;
                                }
                            }
                    }
    ...
    if (XmlNode *extra = node.findNode("extra"))
        if (XmlNode *technique = extra->findNode("technique"))
            if (XmlNode *dbl = technique->findNode("double_sided"))
                Import(double_sided, *dbl);
}
void DAE::Source::import(XmlNode &node) {
    if (XmlParam *id = node.findParam("id"))
        T.id = id->asText();

    if (XmlNode *float_array = node.findNode("float_array"))
        Import(T.float_array, *float_array);
    if (XmlNode *name_array = node.findNode("name_array"))
        Import(T.name_array, *name_array);

    if (XmlNode *technique = node.findNode("technique_common"))
        if (XmlNode *accessor = technique->findNode("accessor"))
            if (XmlParam *stride = accessor->findParam("stride"))
                T.stride = Max(1, stride->asInt());
}
void DAE::Input::import(XmlNode &node, Memb<Source> &source, Mesh *mesh) {
    if (XmlParam *semantic = node.findParam("semantic")) {
        if (semantic->value == "INPUT")
            ...
            T.semantic = OUTPUT;
        else if (semantic->value == "INTERPOLATION")
            T.semantic = INTERPOLATION;
        else if (semantic->value == "VERTEX")
            T.semantic = VERTEX;
        else if (semantic->value == "POSITION")
            T.semantic = POSITION;
        else if (semantic->value == "NORMAL")
            T.semantic = NORMAL;
        else if (semantic->value == "TEXCOORD")
            T.semantic = TEXCOORD;
        else if (semantic->value == "JOINT")
            T.semantic = JOINT;
        else if (semantic->value == "WEIGHT")
            T.semantic = WEIGHT;
        else if (semantic->value == "INV_BIND_MATRIX")
            T.semantic = INV_BIND_MATRIX;
    }
    ...
    if (XmlParam *offset = node.findParam("offset"))
        T.offset = offset->asInt();
    if (XmlParam *set = node.findParam("set"))
        T.set = set->asInt();
}
...
void DAE::Triangles::import(XmlNode &node, Mesh &mesh, DAE &dae) {
    if (XmlParam *material = node.findParam("material"))
        material_symbol = material->asText();
...
void DAE::Polygons::import(XmlNode &node, Mesh &mesh, DAE &dae) {
    ...
    FREPA(node.nodes) {
        XmlNode &n = node.nodes[i];
        if (n.name == "input")
            inputs.New().import(n, mesh.source, &mesh);
        else if (n.name == "p")
            Import(p.New(), n);
    }
}
void DAE::Mesh::import(XmlNode &node, DAE &dae) {
    FREPA(node.nodes) {
        ...
        if (n.name == "source")
            source.New().import(n);
        else if (n.name == "vertices")
            vertices.New().import(n, T);
        else if (n.name == "triangles")
            triangles.New().import(n, T, dae);
        else if (n.name == "polylist")
            poly_list.New().import(n, T, dae);
        else if (n.name == "polygons")
            polygons.New().import(n, T, dae);
    }
}
...
void DAE::Skin::import(XmlNode &node, DAE &dae) {
    if (XmlParam *source = node.findParam("source"))
        geometry_name = SkipStart(source->asText(), '#');

    if (XmlNode *bind_shape_matrix = node.findNode("bind_shape_matrix"))
        Import(T.bind_shape_matrix, *bind_shape_matrix);

    FREPA(node.nodes) {
        XmlNode &n = node.nodes[i];
        if (n.name == "source")
            source.New().import(n);
        else if (n.name == "joints")
            joint.New().import(n, source);
        else if (n.name == "vertex_weights")
            vtx_wgt.New().import(n, source);
    }
...
void DAE::Controller::linkSkin(DAE &dae) {
    FREPA(skin) {
        Skin &skin = T.skin[i];
        if (skin.geometry = dae.findGeometry(skin.geometry_name))
            skin.geometry->skin = &skin;
    }
}
void DAE::InstanceGeometry::import(XmlNode &node) {
    if (XmlParam *url = node.findParam("url"))
        geometry_id = SkipStart(url->asText(), '#');

    FREPA(node.nodes) {
        XmlNode &bm = node.nodes[i];
        if (bm.name == "bind_material")
            FREPA(bm.nodes) {
                XmlNode &tc = bm.nodes[i];
                if (tc.name == "technique_common")
                    FREPA(tc.nodes) {
                        XmlNode &im = tc.nodes[i];
                        if (im.name == "instance_material") {
                            InstanceMaterial &mtrl = instance_material.New();
                            if (XmlParam *symbol = im.findParam("symbol"))
                                mtrl.material_symbol = symbol->asText();
                            if (XmlParam *target = im.findParam("target"))
                                mtrl.material_id = SkipStart(target->asText(), '#');
                        }
                    }
            }
    }
...
void DAE::InstanceController::import(XmlNode &node) {
    if (XmlParam *url = node.findParam("url"))
        controller_id = SkipStart(url->asText(), '#');
...
void DAE::Node::import(XmlNode &node, Node *parent) {
    ...
    if (XmlParam *sid = node.findParam("sid"))
        T.sid = sid->asText();
    if (XmlParam *type = node.findParam("type"))
        T.bone = (type->asText() == "JOINT");

    FREPA(node.nodes) {
        XmlNode &n = node.nodes[i];
        if (n.name == "node")
            T.nodes.New().import(n, this);
        else if (n.name == "translate") {
            Vec temp;
            Import(temp, n);
            transform.New().setPos(temp);
        } else if (n.name == "rotate") {
            Vec4 temp;
            Import(temp, n);
            transform.New().setRotate(temp.xyz, DegToRad(temp.w));
        } else if (n.name == "scale") {
            Vec temp;
            Import(temp, n);
            transform.New().setScale(temp);
        } else if (n.name == "matrix") {
            Matrix temp;
            Import(temp, n);
            transform.New() = temp;
        } else if (n.name == "instance_geometry")
            instance_geometry.New().import(n);
        else if (n.name == "instance_controller")
            instance_controller.New().import(n);
    }

    REPA(transform)
    local_matrix *= transform[i]; // transformations need to be performed from the end
}
...
void DAE::loadAsset(XmlNode &node) {
    if (XmlNode *unit = node.findNode("unit"))
        if (XmlParam *meter = unit->findParam("meter"))
            scale = meter->asFlt();

    if (XmlNode *up_axis = node.findNode("up_axis"))
        if (up_axis->data.elms()) {
            if (up_axis->data[0] == "X_UP")
                T.up_axis = 0;
            else if (up_axis->data[0] == "Y_UP")
                T.up_axis = 1;
            else if (up_axis->data[0] == "Z_UP")
                T.up_axis = 2;
        }
}
...
Bool DAE::Node::setBoneNodesFromAnim(DAE &dae) {
    Bool child_bone_mesh = false;
    FREPA(nodes)
    child_bone_mesh |= nodes[i].setBoneNodesFromAnim(dae);
    ...
    if (!bone && child_bone_mesh && hasAnim())
        bone = true; // is not a bone, children have bone or mesh
...
void DAE::Node::animate(Flt t) {
    ...
    REPA(channels)              // order is important
    {
        ...
        if (channel.time) {
            Memc<Flt> &time = channel.time->float_array;
            if (time.elms()) {
                Int prev = -1,
                    next = -1;
                Flt step = 0;
                ...
                if (channel.alpha)
                    color.w = Lerp(channel.alpha->getFloat(prev), channel.alpha->getFloat(next), step);
                if (channel.move)
                    anim_matrix.pos = Lerp(channel.move->getVec(prev), channel.move->getVec(next), step);
                if (channel.move_x)
                    anim_matrix.pos.x = Lerp(channel.move_x->getFloat(prev), channel.move_x->getFloat(next), step);
                if (channel.move_y)
                    anim_matrix.pos.y = Lerp(channel.move_y->getFloat(prev), channel.move_y->getFloat(next), step);
                if (channel.move_z)
                    anim_matrix.pos.z = Lerp(channel.move_z->getFloat(prev), channel.move_z->getFloat(next), step);
                if (channel.rot_x) {
                    orn.rotateX(DegToRad(Lerp(channel.rot_x->getFloat(prev), channel.rot_x->getFloat(next), step)));
                    custom_orn = true;
                }
                if (channel.rot_y) {
                    orn.rotateY(DegToRad(Lerp(channel.rot_y->getFloat(prev), channel.rot_y->getFloat(next), step)));
                    custom_orn = true;
                }
                if (channel.rot_z) {
                    orn.rotateZ(DegToRad(Lerp(channel.rot_z->getFloat(prev), channel.rot_z->getFloat(next), step)));
                    custom_orn = true;
                }
                // if(channel.scale_axis_r){}
                if (channel.scale) {
                    orn.scale(Lerp(channel.scale->getFloat(prev), channel.scale->getFloat(next), step));
                    custom_orn = true;
                }
    ...
    FREPA(geometry.mesh) {
        ...
        if (Skin *skin = geometry.skin)
            if (skin->name && skin->matrix) {
                ...
                Matrix bind_shape_matrix_inv;
                skin->bind_shape_matrix.inverseNonOrthogonal(bind_shape_matrix_inv);
                REPA(skin->name->name_array) {
                    Int bone_index = skin->bone_remap(i);
                    if (InRange(bone_index, bones)) {
                        Node &node = *bones[bone_index];
                        Matrix &m = matrixes[bone_index + VIRTUAL_ROOT_BONE];
                        skin->matrix->getMatrix(i).inverseNonOrthogonal(m);
                        m *= bind_shape_matrix_inv; // m=GetTransform(skin.matrix->getMatrix(i), bind_shape_matrix_inv);
                        m.inverseNonOrthogonal(m);
                        m *= node.world_matrix; // m=GetTransform(                        m, node.world_matrix    );
                        if (m.mirrored())
                            mirrored_matrixes++;
                        else
                            normal_matrixes++;
        ...
        if (geometry.skin && geometry.skin->vtx_wgt.elms()) {
            ...
            if (vtx_wgt.joint && vtx_wgt.weight) {
                ...
                FREPA(vtx_wgt.vcount) {
                    ...
                    {
                        Int o = v_offset + vtx_wgt.joint->offset;
                        if (!InRange(o, vtx_wgt.v))
                            break;
                        Int joint_index = vtx_wgt.v[o];
                        o = v_offset + vtx_wgt.weight->offset;
                        if (!InRange(o, vtx_wgt.v))
                            break;
                        Int weight_index = vtx_wgt.v[o];
                        if (InRange(joint_index, bone_remap))
                            skin.New().set(bone_remap[joint_index] + VIRTUAL_ROOT_BONE, vtx_wgt.weight->source->getFloat(weight_index));
               ...
               LogN();
#endif
                    SetSkin(skin, skinning[i].bone, skinning[i].blend, &skeleton);
                    skin.clear();
                }
            }
        } else // doesn't have skinning but belongs to a bone
        {
            for (Node *cur = &node; cur; cur = cur->parent)
                if (cur->bone_index >= 0) // find first node that's a bone
                {
                    node_anim = true;
        ...
        FREPA(geom_mesh.triangles) {
            ...
            FREPA(triangles.inputs) {
                ...
                case NORMAL:
                    ...
                    break;
                case TEXCOORD:
                    if (input.source && InRange(input.set, tex))
                        tex[input.set] = &input;
                    break;
                case VERTEX:
                    if (Vertices *vertices = input.vertices)
                        FREPA(vertices->inputs) {
            ...
            if (triangles.p.elms() && triangles.inputs.elms() && position) {
                ...
                if (part_material_index)
                    part_material_index.add(findMaterial(triangles.material_symbol, im));
                ...
                REPA(base.tri) {
                    ...
                    if (skinning.elms()) {
                        ...
                        if (InRange(p0, skinning)) {
                            base.vtx.matrix(a) = skinning[p0].bone;
                            base.vtx.blend(a) = skinning[p0].blend;
                        } else {
                            base.vtx.matrix(a) = node_skin.bone;
                            base.vtx.blend(a) = node_skin.blend;
                        }
                        if (InRange(p1, skinning)) {
                            base.vtx.matrix(b) = skinning[p1].bone;
                            base.vtx.blend(b) = skinning[p1].blend;
                        } else {
                            base.vtx.matrix(b) = node_skin.bone;
                            base.vtx.blend(b) = node_skin.blend;
                        }
                        if (InRange(p2, skinning)) {
                            base.vtx.matrix(c) = skinning[p2].bone;
                            base.vtx.blend(c) = skinning[p2].blend;
                        } else {
                            base.vtx.matrix(c) = node_skin.bone;
                            base.vtx.blend(c) = node_skin.blend;
                        }
                    } else if (node_anim) {
                        base.vtx.matrix(a) = node_skin.bone;
                        base.vtx.blend(a) = node_skin.blend;
                    ...
                    base.tri.ind(i).set(a, b, c);
                    if (reverse)
                        base.tri.ind(i).reverse();
                }
                ...
                if (matrixes.elms())
                    ...
                    base.transform(node.world_matrix);
                if (!base.vtx.nrm())
                    base.setNormalsAuto(EPS_NRM_AUTO, EPSD); // use small pos epsilon in case mesh is scaled down, call before 'setTanBin' as it depends on normals
                if (!base.vtx.tan() || !base.vtx.bin())
                    base.setTanBin();                         // if(!base.vtx.tan())base.setTangents(); if(!base.vtx.bin())base.setBinormals(); // need to call before 'weldVtx' to don't remove too many vertexes
        ...
        FREPA(geom_mesh.poly_list) {
            ...
            if (polys.vcount.elms() && polys.p.elms() && polys.inputs.elms() && position) {
                MeshPart &part = mesh.parts.New();
                if (part_material_index)
                    part_material_index.add(findMaterial(polys.material_symbol, im));
                MeshBase &base = part.base;
                ...
                FREPA(polys.vcount) {
                    ...
                    FREP(vrts) {
                        ...
                        if (normal)
                            vtx.nrm = normal->source->getVec(Safe(polys.p, v + normal->offset));
                        if (tex[0])
                            vtx.tex[0] = tex[0]->source->getVec2(Safe(polys.p, v + tex[0]->offset));
                        if (tex[1])
                            vtx.tex[1] = tex[1]->source->getVec2(Safe(polys.p, v + tex[1]->offset));
                        if (tex[2])
                            vtx.tex[2] = tex[2]->source->getVec2(Safe(polys.p, v + tex[2]->offset));
                        if (tex[3])
                            vtx.tex[3] = tex[3]->source->getVec2(Safe(polys.p, v + tex[3]->offset));
                        v += polys.inputs.elms();
                    }
                    for (; poly.elms() >= 3;) {
                        Int p0 = tris * 3 + 0,
                        ...
                        if (skinning.elms() || node_anim) {
                            base.vtx.matrix(p0) = v0.bone;
                            base.vtx.blend(p0) = v0.blend;
                            base.vtx.matrix(p1) = v1.bone;
                            base.vtx.blend(p1) = v1.blend;
                            base.vtx.matrix(p2) = v2.bone;
                            base.vtx.blend(p2) = v2.blend;
                        }
                        if (normal) {
                        ...
                        base.tri.ind(tris).set(p0, p1, p2);
                        if (reverse)
                            base.tri.ind(tris).reverse();
                        tris++;
                        poly.remove(o1, true);
                    }
                    poly.clear();
        ...
        FREPA(geom_mesh.polygons) {
            ...
            if (polys.p.elms() && polys.inputs.elms() && position) {
                ...
                Int tris = 0;
                REPA(polys.p)
                tris += Max(0, polys.p[i].elms() / polys.inputs.elms() - 2);
                base.create(tris * 3, 0, tris, 0, (normal ? VTX_NRM : MESH_NONE) | (tex[0] ? VTX_TEX0 : MESH_NONE) | (tex[1] ? VTX_TEX1 : MESH_NONE) | (tex[2] ? VTX_TEX2 : MESH_NONE) | (tex[3] ? VTX_TEX3 : MESH_NONE) | ((skinning.elms() || node_anim) ? VTX_SKIN : MESH_NONE));
                ...
                FREPA(polys.p) {
                    ...
                    FREP(vrts) {
                        ...
                        vtx.pos = position->source->getVec(p);
                        if (normal)
                            vtx.nrm = normal->source->getVec(Safe(polys_p, v + normal->offset));
                        if (tex[0])
                            vtx.tex[0] = tex[0]->source->getVec2(Safe(polys_p, v + tex[0]->offset));
                        if (tex[1])
                            vtx.tex[1] = tex[1]->source->getVec2(Safe(polys_p, v + tex[1]->offset));
                        if (tex[2])
                            vtx.tex[2] = tex[2]->source->getVec2(Safe(polys_p, v + tex[2]->offset));
                        if (tex[3])
                            vtx.tex[3] = tex[3]->source->getVec2(Safe(polys_p, v + tex[3]->offset));
                        v += polys.inputs.elms();
                    }
                    for (; poly.elms() >= 3;) {
                        Int p0 = tris * 3 + 0,
...
void DAE::Node::create(::Mesh &mesh, MemPtr<Int> part_material_index, ::Skeleton &skeleton, DAE &dae) {
    ...
    FREPA(instance_controller) {
        ...
        if (Controller *controller = dae.findController(ic.controller_id))
            if (controller->skin.elms())
                if (Geometry *geometry = controller->skin[0].geometry)
                    dae.create(mesh, part_material_index, skeleton, *geometry, ic.instance_material, T);
    }
    FREPA(nodes)
    nodes[i].create(mesh, part_material_index, skeleton, dae);
}
...
void DAE::create(::Skeleton &skeleton, XAnimation *animation) {
    ...
    FREPA(bones) {
        ...
        BoneType parent = BONE_NULL;
        for (Node *cur = node.parent; cur; cur = cur->parent)
            if (cur->bone_index >= 0) {
                parent = cur->bone_index;
                break;
            } // find first parent which is a bone
        sbon.parent = parent;
#if !CONVERT_BIND_POSE
        Int bone_index = i;
        FREPA(controllers) {
            ...
            FREPA(controller.skin) {
                ...
                if (skin.name && skin.matrix) {
                    FREPA(skin.bone_remap)
                    if (skin.bone_remap[i] == bone_index) {
                        // Matrix bind_shape_matrix_inv; skin.bind_shape_matrix.inverseNonOrthogonal(bind_shape_matrix_inv);
                        Matrix m = skin.matrix->getMatrix(i);
                        m.inverseNonOrthogonal();
                        sbon.pos = m.pos;
                        sbon.dir = !m.x;
                        sbon.perp = !m.y;
                    }
    ...
    skeleton.sortBones(old_to_new);
    Memc<Node *> sorted_bones;
    sorted_bones.setNumZero(skeleton.bones.elms());
    FREPA(bones) {
        Node &node = *bones[i];
        node.adjustBoneIndex(InRange(node.bone_index, old_to_new) ? old_to_new[node.bone_index] : BONE_NULL);
        if (InRange(node.bone_index, sorted_bones))
            sorted_bones[node.bone_index] = &node;
        else
            node.bone_index = -1;
    }
...
void DAE::create(XAnimation &animation, ::Skeleton &skeleton) {
    ...
    FREPA(skeleton.bones) {
        ...
        if (channels.elms()) {
            SkelBone &sbon = skeleton.bones[i];
            AnimBone &abon = animation.anim.bones.New();
            abon.set(sbon.name);
            Matrix3 parent_matrix_inv;
            if (sbon.parent != BONE_NULL)
                skeleton.bones[sbon.parent].inverse(parent_matrix_inv);
            Matrix3 local_to_world;
            node.local_matrix.orn().inverseNonOrthogonal(local_to_world);
            local_to_world *= node.world_matrix.orn(); // GetTransform(node.local_matrix.orn(), node.world_matrix.orn());
            ...
            FREPA(times) {
                ...
                pos.time = t;
                AnimKeys::Scale &scale = abon.scales[i];
                scale.time = t;
#if HAS_ANIM_COLOR
                AnimKeys::Color &color = abon.colors[i];
                color.time = t;
#endif
                ...
                // position
                {
                    ...
                    if (node.parent)
                        pos.pos *= node.parent->world_matrix.orn();
                    if (sbon.parent != BONE_NULL)
    ...
    if (dt_num)
        dt_avg /= dt_num;
    animation.fps = (fps ? fps : dt_avg ? 1 / dt_avg
                                        : 0);
...
void DAE::create(MemPtr<XMaterial> materials, Str path) {
    FREPA(T.materials) {
        ...
        if (src.effect) {
            ...
            if (Image *color_map = findImage(fx.color_map_image_id))
                dest.color_map = color_map->texture;
            if (Image *alpha_map = findImage(fx.alpha_map_image_id))
                dest.alpha_map = alpha_map->texture;
            if (Image *specular_map = findImage(fx.specular_map_image_id))
                dest.smooth_map = specular_map->texture;
            if (Image *bump_map = findImage(fx.bump_map_image_id))
                dest.normal_map = bump_map->texture;
...
Bool ImportDAE(C Str &name, Mesh *mesh, Skeleton *skeleton, XAnimation *animation, MemPtr<XMaterial> materials, MemPtr<Int> part_material_index) {
    ...
    XmlData xml;
    xml.load(name);
    if (XmlNode *COLLADA = xml.findNode("COLLADA")) // try even if 'load' fails (partial data may be available)
    {
        DAE dae;

        // import
        if (XmlParam *version = COLLADA->findParam("version"))
            dae.version = version->asInt();
        FREPA(COLLADA->nodes) {
            XmlNode &node = COLLADA->nodes[i];
            if (node.name == "asset")
                dae.loadAsset(node);
            else if (node.name == "library_animations")
                dae.loadLibraryAnimations(node);
            else if (node.name == "library_images")
                dae.loadLibraryImages(node);
            else if (node.name == "library_materials")
                dae.loadLibraryMaterials(node);
            else if (node.name == "library_effects")
                dae.loadLibraryEffects(node);
            else if (node.name == "library_geometries")
                dae.loadLibraryGeometries(node);
            else if (node.name == "library_controllers")
                dae.loadLibraryControllers(node);
            else if (node.name == "library_visual_scenes")
                dae.loadLibraryVisualScenes(node);
        }
        ...
        if (materials) {
            // link
            FREPA(dae.materials)
            dae.materials[i].linkEffect(dae);
            dae.create(materials, GetPath(name));
        }

        Skeleton temp, *skel = (skeleton ? skeleton : (mesh || animation) ? &temp
                                                                          : null); // if skel not specified, but we want mesh or animation, then we have to process it
        if (skel) {
            // link
            FREPA(dae.animations)
            dae.animations[i].linkNode(dae); // order is important, channels must be added in order (because there can be many channels for each node, for example rot,pos,..), call this before 'setBoneNodesFromAnim' because we're setting if node has animation or not
            FREPA(dae.controllers)
            dae.controllers[i].linkSkin(dae); // call before 'setBoneNodesFromAnim' because we're setting controller skin links
            ...
            if (dae.scale >= 0.001f && dae.scale != 1) {
                ...
                if (animation)
                    animation->anim.scale(dae.scale);
                if (mesh)
                    mesh->scale(dae.scale);
            }
            ...
            if (dae.up_axis < 0 || dae.up_axis == 2) // unknown or Z
            {
                skel->rightToLeft();
                if (animation)
                    animation->anim.rightToLeft(*skel);
                if (mesh)
                    mesh->rightToLeft();
            } else if (dae.up_axis == 1) // Y
            {
                skel->mirrorX();
                if (animation)
                    animation->anim.mirrorX();
                if (mesh)
                    mesh->mirrorX();
            }
            ...
            if (mesh) {
                mesh->skeleton(skel).skeleton(null).setBox();
                CleanMesh(*mesh);
            }
            if (animation)
