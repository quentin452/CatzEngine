...
static void SetSkin(VtxBone &matrix, VecB4 &blend, msModel &ms3d, ms3d_vertex_t &vertex, MemtN<BoneType, 256> &old_to_new, Skeleton *skeleton) {
    int indices[4], weights[4];
    ms3d.FillJointIndicesAndWeights(&vertex, indices, weights);

    MemtN<IndexWeight, 256> skin; // div by 100 not needed because weight is normalized in 'SetSkin'
    if (weights[0] > 0 && InRange(indices[0], old_to_new))
        skin.New().set(old_to_new[indices[0]], weights[0] /* /100.0f */);
    if (weights[1] > 0 && InRange(indices[1], old_to_new))
        skin.New().set(old_to_new[indices[1]], weights[1] /* /100.0f */);
    if (weights[2] > 0 && InRange(indices[2], old_to_new))
        skin.New().set(old_to_new[indices[2]], weights[2] /* /100.0f */);
    if (weights[3] > 0 && InRange(indices[3], old_to_new))
        skin.New().set(old_to_new[indices[3]], weights[3] /* /100.0f */);
...
static void CreateSkeleton(Skeleton &skeleton, msModel &ms3d, Int bones, XAnimation *animation) {
    ...
    FREP(bones) {
        ...
        Matrix m;
        ToMatrix(m, joint.matGlobalSkeleton);
    ...
    if (animation) {
        ...
        FREPA(anim.bones) {
            ...
            FREPA(abon.orns) {
                ...
                if (parent)
                    sbon_src *= parent_matrix_inv; // src in parent space
                ...
                Matrix3 m;
                ToMatrix(m, joint.matGlobal);
                orn.orn.perp = m.y;
                orn.orn.dir = m.x; // dest in global space

                if (parent) {
                    Orient parent_dest;
                    ToMatrix(m, joint_parent->matGlobal);
                ...
                {
                    vec4_t quat;
                    AngleQuaternion(frame.key, quat); // calculate the quaternion basing on the angles
                    float matAnimate[3][4];
                    QuaternionMatrix(quat, matAnimate); // calculate the transformation matrix basing on the quaternion
                ...
                Matrix3 m;
                ToMatrix(m, joint.matLocal); // convert the matrix to easier form
            ...
            FREPA(abon.poss) {
                ...
                Matrix M_l;
                ToMatrix(M_l, joint.matLocal);
                Matrix M_ls;
                ToMatrix(M_ls, joint.matLocalSkeleton);
                pos.pos = M_l.pos - M_ls.pos;
                if (parent) {
                    Matrix3 p_gs;
                    ToMatrix(p_gs, joint_parent->matGlobalSkeleton);
                    pos.pos *= p_gs;
        ...
        // rescale in time
        if (Flt fps = ms3d.GetAnimationFps())
            anim.length(anim.length() / fps, true);
...
Bool ImportMS3D(C Str &name, Mesh *mesh, Skeleton *skeleton, XAnimation *animation, MemPtr<XMaterial> materials, MemPtr<Int> part_material_index) {
    ...
    if (ms3d.Load(name)) {
        ...
        if (skel) {
            CreateSkeleton(*skel, ms3d, bones, animation);
        ...
        if (mesh) {
            ...
            FREPA(*mesh) {
                ...
                if (part_material_index)
                    part_material_index.add(InRange(group.materialIndex, material_num) ? group.materialIndex : -1);
        ...
        // finalize
        if (skel)
            skel->mirrorX().setBoneTypes();
        if (animation)
            animation->anim.mirrorX().setBoneTypeIndexesFromSkeleton(*skel);
        if (mesh) {
            mesh->mirrorX().skeleton(skel).skeleton(null);
            CleanMesh(*mesh);
        }
        if (skeleton) {
            skel->setBoneShapes();
