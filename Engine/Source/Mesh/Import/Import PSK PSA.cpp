...
static void CreateSkeleton(Skeleton &skeleton, Memc<VBone> &bones) {
    ...
    FREPA(bones) {
        ...
        if (i)
            CHS(ubon.BonePos.Orientation.w);
        Matrix3 orient = ubon.BonePos.Orientation;
        if (i)
            CHS(ubon.BonePos.Orientation.w);
        ...
        if (InRange(sbon.parent, skeleton.bones))
            sbon *= (Matrix)skeleton.bones[sbon.parent];
    ...
    FREPA(bones)
    skeleton.bones[i].dir = skeleton.bones[i].cross(); // do this at the end, because sbon transformation by matrix relies on original value
...
Bool ImportPSK(C Str &name, Mesh *mesh, Skeleton *skeleton, MemPtr<XMaterial> materials, MemPtr<Int> part_material_index) {
    ...
    if (f.read(name)) {
        ...
        f >> points_header;
        Memc<VPoint> points;
        if (points_header.DataCount < 0 || points_header.DataCount && SIZE(VPoint) != points_header.DataSize)
            return false;
        points.setNum(points_header.DataCount);
        f.getN(points.data(), points.elms());
        VChunkHeader vertexs_header;
        f >> vertexs_header;
        Memc<VVertex> vertexs;
        if (vertexs_header.DataCount < 0 || vertexs_header.DataCount && SIZE(VVertex) != vertexs_header.DataSize)
            return false;
        vertexs.setNum(vertexs_header.DataCount);
        f.getN(vertexs.data(), vertexs.elms());
        VChunkHeader faces_header;
        f >> faces_header;
        Memc<VTriangle> faces;
        if (faces_header.DataCount < 0 || faces_header.DataCount && SIZE(VTriangle) != faces_header.DataSize)
            return false;
        faces.setNum(faces_header.DataCount);
        f.getN(faces.data(), faces.elms());
        VChunkHeader materials_header;
        f >> materials_header;
        Memc<VMaterial> mtrls;
        if (materials_header.DataCount < 0 || materials_header.DataCount && SIZE(VMaterial) != materials_header.DataSize)
            return false;
        mtrls.setNum(materials_header.DataCount);
        f.getN(mtrls.data(), mtrls.elms());
        VChunkHeader bones_header;
        f >> bones_header;
        Memc<VBone> bones;
        if (bones_header.DataCount < 0 || bones_header.DataCount && SIZE(VBone) != bones_header.DataSize)
            return false;
        bones.setNum(bones_header.DataCount);
        f.getN(bones.data(), bones.elms());
        VChunkHeader skins_header;
        f >> skins_header;
        Memc<VRawBoneInfluence> skins;
        if (skins_header.DataCount < 0 || skins_header.DataCount && SIZE(VRawBoneInfluence) != skins_header.DataSize)
            return false;
        skins.setNum(skins_header.DataCount);
        f.getN(skins.data(), skins.elms());
        if (!f.ok())
            return false;

        // skeleton
        MemtN<BoneType, 256> old_to_new;
        Skeleton temp, *skel = (skeleton ? skeleton : mesh ? &temp
                                                           : null); // if skel not specified, but we want mesh, then we have to process it
        if (skel) {
        ...
        if (mesh) {
            ...
            if (skins.elms()) {
                ...
                REPA(skins) {
                    ...
                    if (InRange(skin.PointIndex, vtx_skin) && InRange(skin.BoneIndex, old_to_new))
                        vtx_skin[skin.PointIndex].New().set(old_to_new[skin.BoneIndex], skin.Weight);
                }
            }
            ...
            FREPA(faces) {
                ...
                if (!InRange(vvtx0.PointIndex, points) || !InRange(vvtx1.PointIndex, points) || !InRange(vvtx2.PointIndex, points))
                    return false;
        ...
        if (materials) {
            ...
            FileText f;
            if (f.read(GetPath(name) + "\\X_ModelInfo_" + GetBaseNoExt(name) + ".LOG"))
                for (; !f.end();) {
                    if (Starts(f.getLine(), "= materials =")) {
                        Int index = -1;
                        for (Str line; !f.end();) {
                            f.getLine(line);
                            Int pos = TextPosI(line, "* Index:");
                            if (pos >= 0)
                                index++;
                            pos = TextPosI(line, "Original Bitmap:");
                            if (pos >= 0 && InRange(index, materials)) {
                                Str tex = SkipStart(line() + pos, "Original Bitmap:");
                                pos = TextPosI(tex, "Path:");
                                if (pos >= 0)
                                    tex.clip(pos);
        ...
        // finalize
        if (skel)
            skel->rightToLeft().setBoneTypes(); // 'setBoneTypes' must be called after 'sortBones' and 'rightToLeft' and before 'Mesh.skeleton'
        if (mesh) {
            mesh->rightToLeft().skeleton(skel).skeleton(null).setBox();
            CleanMesh(*mesh);
        }
        if (skeleton) {
            skel->setBoneShapes();
...
Bool ImportPSA(C Str &name, Skeleton *skeleton, MemPtr<XAnimation> animations) {
    ...
    if (f.read(name)) {
        ...
        bones.setNum(bones_header.DataCount);
        f.getN(bones.data(), bones.elms());
        VChunkHeader anim_header;
        f >> anim_header;
        Memc<AnimInfoBinary> anims;
        if (anim_header.DataCount < 0 || anim_header.DataCount && SIZE(AnimInfoBinary) != anim_header.DataSize)
            return false;
        anims.setNum(anim_header.DataCount);
        f.getN(anims.data(), anims.elms());
        VChunkHeader raw_keys_header;
        f >> raw_keys_header;
        Memc<VQuatAnimKey> keys;
        if (raw_keys_header.DataCount < 0 || raw_keys_header.DataCount && SIZE(VQuatAnimKey) != raw_keys_header.DataSize)
            return false;
        keys.setNum(raw_keys_header.DataCount);
        f.getN(keys.data(), keys.elms());
        if (!f.ok())
            return false;

        // create skeleton
        Skeleton temp, *skel = (skeleton ? skeleton : animations ? &temp
                                                                 : null); // if skel not specified, but we want animations, then we have to process it
        if (skel) {
            CreateSkeleton(*skel, bones);
            ...
            if (animations)
                FREPA(anims) {
                    ...
                    if (anim.TotalBones == skel->bones.elms() && anim.FirstRawFrame >= 0              // can be zero
                        ...
                        && (anim.FirstRawFrame + anim.NumRawFrames) * anim.TotalBones <= keys.elms()) // frames are in range
                    {
                        ...
                        FREPA(skel->bones) {
                            ...
                            AnimBone &abon = animation.bones[i];
                            abon.set(sbon.name);
                            ...
                            FREPD(k, anim.NumRawFrames) {
                                ...
                                AnimKeys::Pos &pos = abon.poss[k];
                                pos.time = time;
                                AnimKeys::Orn &orn = abon.orns[k];
                                orn.time = time;
                                ...
                                // orientation
                                {
                                    ...
                                    // animate
                                    {
                                        if (i) {
                                            CHS(ubon.BonePos.Orientation.w);
                                            CHS(key.Orientation.w);
                                        }
                                        if (parent)
                                            orn.orn *= parent_matrix_temp_inv;                                                // transform bone from global space to local space relative to parent
                                        ...
                                        if (parent)
                                            orn.orn *= parent_matrix_temp; // transform bone from local space to global space
                                        if (i) {
                                            CHS(ubon.BonePos.Orientation.w);
                                            CHS(key.Orientation.w);
                                        }
                        ...
                        // rescale in time
                        if (xanimation.fps > 0)
                            animation.length(animation.length() / xanimation.fps, true);

                        // override length
                        if (anim.TrackTime > 0)
                            animation.length((xanimation.fps > 0) ? anim.TrackTime / xanimation.fps : anim.TrackTime, false);
