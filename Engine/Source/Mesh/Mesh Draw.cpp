...
void MeshBase::draw2D(C Color &vtx_color, C Color &edge_color, C Color &face_color, Flt vtx_r, Flt side_width) C {
    ...
    if (face_color.a && quad.ind()) {
        ...
        REPA(quad) {
            p = quad.ind(i).c;
            VI.quad(vtx.pos(p[0]).xy, vtx.pos(p[1]).xy, vtx.pos(p[2]).xy, vtx.pos(p[3]).xy);
        }
        VI.end();
    }
    if (face_color.a && tri.ind()) {
        ...
        REPA(tri) {
            p = tri.ind(i).c;
            VI.tri(vtx.pos(p[0]).xy, vtx.pos(p[1]).xy, vtx.pos(p[2]).xy);
        }
        VI.end();
    }
    if (edge_color.a && edge.flag() && side_width) {
        ...
        REPA(edge) {
            ...
            if (f & ETQ_LR) {
                ...
                Vec2 r = Perp(p1 - p0);
                r.setLength((f & ETQ_NO_PHYS) ? side_width * 2 : side_width);
                if (f & ETQ_R)
                    VI.quad(c0, c1, p0, p0 + r, p1 + r, p1);
                if (f & ETQ_L)
                    VI.quad(c0, c1, p0, p0 - r, p1 - r, p1);
            }
        }
...
void MeshBase::drawNormals2D(Flt length, C Color &edge_color, C Color &vtx_color) C {
    ...
    if (vtx_color.a && (nrm = vtx.nrm())) {
        VI.color(vtx_color);
        REPA(vtx)
        VI.line(pos[i].xy, pos[i].xy + length * nrm[i].xy);
        VI.end();
    }
...
void MeshBase::drawNormals(Flt length, C Color &face_color, C Color &vtx_color, C Color &tangent_color, C Color &binormal_color) C {
    ...
    if (vtx_color.a)
        if (C Vec *nrm = vtx.nrm()) {
            VI.color(vtx_color);
            REPA(vtx)
            VI.line(pos[i], pos[i] + length * nrm[i]);
            VI.end();
        }
    if (tangent_color.a)
        if (C Vec *tan = vtx.tan()) {
            VI.color(tangent_color);
            REPA(vtx)
            VI.line(pos[i], pos[i] + length * tan[i]);
            VI.end();
        }
    if (binormal_color.a)
        if (C Vec *bin = vtx.bin()) {
            VI.color(binormal_color);
            REPA(vtx)
            VI.line(pos[i], pos[i] + length * bin[i]);
            VI.end();
        }
...
again:
    if (cur->shader != &shader) // check if we already have a match
    {
        ...
        {
            if (cur->next_material_shader >= 0) {
                cur = &MaterialShaders[cur->next_material_shader];
                goto again;
            } // !! we can access 'next_material_shader' only when knowing that "shader!=null", because only 'shader' is cleared in 'MaterialShader.unlink' !! check if there's another one, we could check this before "if(cur->shader)", however we prioritize cases where Materials have only one Shader (the most common scenario)
    ...
    shader_draw_initialized:
        ...
        if (..)
            shader_material->material = &material; // have to pass as parameter and can't use variation.material because this can be variation.getMaterial or variation.getShadowMaterial, also can't check for mesh._umm because this can be called for shadows which ignores it
        else
            shader_material->umm = &umm;
#endif
...
SkeletonBlendInstance &BlendInstancesClass::getSkeletonInstance(C AnimatedSkeleton &anim_skel) {
    // already exists
    if (anim_skel._instance.blend >= 0)
        return T[anim_skel._instance.blend].skeleton;
...
again_shader:
    if (cur_shader->shader != &shader) // check if we already have a match
    {
        ...
        {
            if (cur_shader->next_skeleton_shader >= 0) {
                cur_shader = &SkeletonShaders[cur_shader->next_skeleton_shader];
                goto again_shader;
            }
    ...
    again_material:
        if (cur_material->material != &material) // check if we already have a match
        {
            ...
            {
                if (cur_material->next_skeleton_shader_material >= 0) {
                    cur_material = &SkeletonShaderMaterials[cur_material->next_skeleton_shader_material];
                    goto again_material;
                }
        ...
        set_material:
            cur_material->material = &material;
            material.incUsage();
...
again_shader:
    if (cur_shader->shader != &shader) // check if we already have a match
    {
        ...
        {
            if (cur_shader->next_skeleton_shader >= 0) {
                cur_shader = &SkeletonBlendShaders[cur_shader->next_skeleton_shader];
                goto again_shader;
            }
    ...
    again_material:
        if (cur_material->material != &material) // check if we already have a match
        {
            ...
            {
                if (cur_material->next_skeleton_shader_material >= 0) {
                    cur_material = &SkeletonBlendShaderMaterials[cur_material->next_skeleton_shader_material];
                    goto again_material;
                }
...
void BlendInstance::unlink() {
    if (type == SKELETON)
        skeleton.unlinkBlend();
}

INLINE void BlendInstance::setZ(C VecD &pos) { setViewZ(DistPointActiveCamPlaneZ(pos)); }
INLINE void BlendInstance::setMatrix(C MatrixM &matrix) {
    SetViewMatrix(stat.view_matrix.cur, matrix);
    viewMatrixChanged();
} // here 'matrix_prev' is ignored because it's assumed to be unused
INLINE void BlendInstance::setMatrix(C MatrixM &matrix, C MatrixM &matrix_prev) {
    SetViewMatrix(stat.view_matrix, matrix, matrix_prev);
    viewMatrixChanged();
}
...
INLINE BlendInstance &BlendInstancesClass::add(Shader &shader, C Material &material, C MeshPart &mesh, C MeshPart::Variation &variation) {
    BlendInstance &obj = New();
    obj.type = BlendInstance::STATIC;
#if 1
...
INLINE BlendInstance &BlendInstancesClass::add(BLST &blst, C Material &material, C MeshPart &mesh, C MeshPart::Variation &variation) {
    BlendInstance &obj = New();
    obj.type = BlendInstance::STATIC_BLST;
#if 1
    obj.stat.blst = &blst;
    obj.stat.material = &material;
#else
...
INLINE BlendInstance &BlendInstancesClass::addFur(Shader &shader, C Material &material, C MeshPart &mesh, C MeshPart::Variation &variation) {
    BlendInstance &obj = New();
    obj.type = BlendInstance::STATIC_FUR;
#if 1
    obj.stat.shader = &shader;
    obj.stat.material = &material;
#else
...
INLINE void BlendInstancesClass::add(BlendObject &blend_obj, C VecD &pos) {
    BlendInstance &obj = New();
    obj.type = BlendInstance::BLEND_OBJ;
    obj.blend_obj = &blend_obj;
    obj.setZ(pos);
}
void BlendInstancesClass::add(Game::Obj &game_obj) {
    BlendInstance &obj = New();
    obj.type = BlendInstance::GAME_OBJ;
    obj.game_obj = &game_obj;
    obj.setZ(game_obj.pos());
}
void BlendInstancesClass::add(Game::Area::Data &game_area) {
    BlendInstance &obj = New();
    obj.type = BlendInstance::GAME_AREA;
    obj.game_area = &game_area;
    Game::Area &area = game_area.area();
    Flt area_size = area.world()->areaSize();
    obj.setZ(VecD((area.xz().x + 0.5f) * area_size, ActiveCam.matrix.pos.y, (area.xz().y + 0.5f) * area_size));
}
void BlendObject::scheduleDrawBlend(C VecD &pos) {
    if (Renderer.firstPass())
        BlendInstances.add(T, pos);
}
/******************************************************************************/
...
#if SUPPORT_EARLY_Z
                if (Renderer._mesh_early_z)
                    if (Shader *shader = variation.shader[RM_EARLY_Z]) {
                        if (shader == Renderer._shader_early_z)
                            SetViewMatrix(EarlyZInstances[variation.getMaterial().cull].New().set(render).view_matrix, matrix);
                    }
#endif

                if (variation.last_opaque_instance < 0) {
                    if (_umm)
                        NewInstance(MultiMaterialShaderDraws, OpaqueShaderMaterialMeshInstances.elms(), *shader, &_umm->material_shader, T, variation);
                    else
                        NewInstance(ShaderDraws, OpaqueShaderMaterialMeshInstances.elms(), *shader, &variation.getMaterial()._opaque_material_shader, T, variation);
                } else
                    OpaqueShaderMaterialMeshInstances[variation.last_opaque_instance].next_instance = OpaqueShaderMaterialMeshInstances.elms();
...
#if SUPPORT_EMISSIVE
                if (Shader *shader = variation.shader[RM_EMISSIVE])
                    SetViewMatrix(EmissiveInstances.New().set(T, variation).view_matrix, matrix);
#endif
            } else if (Shader *shader = variation.shader[RM_BLEND])
                BlendInstances.add(*shader, variation.getMaterial(), T, variation).setMatrix(matrix, matrix_prev);
            else if (variation.blst)
                BlendInstances.add(*variation.blst, variation.getMaterial(), T, variation).setMatrix(matrix, matrix_prev);

            if (Shader *shader = variation.shader[RM_FUR])
                BlendInstances.addFur(*shader, variation.getMaterial(), T, variation).setMatrix(matrix, matrix_prev);
        } break;

        case RT_FORWARD: // velocities not needed for RT_FORWARD
        {
#if SUPPORT_EARLY_Z
            if (Renderer._mesh_early_z && Renderer.firstPass())
                if (Shader *shader = variation.shader[RM_EARLY_Z]) {
                    if (shader == Renderer._shader_early_z)
                        SetViewMatrix(EarlyZInstances[variation.getMaterial().cull].New().set(render).view_matrix, matrix);
                }
            ...
            if (variation.frst) {
                if (Renderer.firstPass() || variation.frst->all_passes) // if(Shader *shader=_frst->getShader())
                {
                    if (variation.last_opaque_instance < 0) {
                        if (_umm)
                            NewInstance(MultiMaterialShaderDraws, OpaqueShaderMaterialMeshInstances.elms(), *variation.frst, &_umm->material_shader, T, variation);
                        else
                            NewInstance(ShaderDraws, OpaqueShaderMaterialMeshInstances.elms(), *variation.frst, &variation.getMaterial()._opaque_material_shader, T, variation);
                    } else
                        OpaqueShaderMaterialMeshInstances[variation.last_opaque_instance].next_instance = OpaqueShaderMaterialMeshInstances.elms();
                    variation.last_opaque_instance = OpaqueShaderMaterialMeshInstances.elms();
                    SetViewMatrix(OpaqueShaderMaterialMeshInstances.New().set().view_matrix.cur, matrix);
                }
            } else if (Renderer.firstPass()) {
                if (Shader *shader = variation.shader[RM_BLEND])
                    BlendInstances.add(*shader, variation.getMaterial(), T, variation).setMatrix(matrix);
                else if (variation.blst)
                    BlendInstances.add(*variation.blst, variation.getMaterial(), T, variation).setMatrix(matrix);
            }
        } break;
        }
    }
...
#if SUPPORT_EMISSIVE
                if (Shader *shader = variation.shader[RM_EMISSIVE])
                    SkeletonEmissiveInstances.New().set(T, variation, anim_skel);
#endif
            } else if (Shader *shader = variation.shader[RM_BLEND])
                BlendInstances.getSkeletonInstance(anim_skel).addBlend(*shader, material, T);
            else if (variation.blst)
                BlendInstances.getSkeletonInstance(anim_skel).addBlend(*variation.blst, material, T);

            if (Shader *shader = variation.shader[RM_FUR])
                BlendInstances.getSkeletonInstance(anim_skel).addFur(*shader, material, T);
        } break;
...
void MeshPart::drawShadow(C MatrixM &matrix) C {
    ...
    if (_draw_mask & Renderer._mesh_draw_mask) {
        C Variation &variation = getVariation();
        if (Shader *shader = variation.shader[RM_SHADOW]) {
            if (variation.last_shadow_instance < 0) {
                ...
                ShadowShaderMaterialMeshInstances[variation.last_shadow_instance].next_instance = ShadowShaderMaterialMeshInstances.elms();
...
void MeshLod::draw() C {
    ...
    switch (Renderer._cur_type) {
    case RT_DEFERRED: {
        Matrix view_matrix_prev;
        SetViewMatrixPrev(view_matrix_prev);
        FREPA(T) // process in order
        {
            C MeshPart &part = parts[i];
            if (part._draw_mask & Renderer._mesh_draw_mask) {
                ...
                if (Shader *shader = variation.shader[Renderer._opaque_mode_index]) {
                    ...
                    if (Renderer._mesh_early_z)
                        if (Shader *shader = variation.shader[RM_EARLY_Z]) {
                            if (shader == Renderer._shader_early_z)
                                EarlyZInstances[variation.getMaterial().cull].New().set(part.render).view_matrix = view_matrix;
                        }
#endif

                    if (variation.last_opaque_instance < 0) {
                        if (part._umm)
                            NewInstance(MultiMaterialShaderDraws, OpaqueShaderMaterialMeshInstances.elms(), *shader, &part._umm->material_shader, part, variation);
                        else
                            NewInstance(ShaderDraws, OpaqueShaderMaterialMeshInstances.elms(), *shader, &variation.getMaterial()._opaque_material_shader, part, variation);
                    } else
                        OpaqueShaderMaterialMeshInstances[variation.last_opaque_instance].next_instance = OpaqueShaderMaterialMeshInstances.elms();
                    variation.last_opaque_instance = OpaqueShaderMaterialMeshInstances.elms();
...
#if SUPPORT_EMISSIVE
                    if (Shader *shader = variation.shader[RM_EMISSIVE])
                        EmissiveInstances.New().set(part, variation).view_matrix = view_matrix;
#endif
                } else if (Shader *shader = variation.shader[RM_BLEND])
                    BlendInstances.add(*shader, variation.getMaterial(), part, variation).setViewMatrix(view_matrix, view_matrix_prev);
                else if (variation.blst)
                    BlendInstances.add(*variation.blst, variation.getMaterial(), part, variation).setViewMatrix(view_matrix, view_matrix_prev);

                if (Shader *shader = variation.shader[RM_FUR])
                    BlendInstances.addFur(*shader, variation.getMaterial(), part, variation).setViewMatrix(view_matrix, view_matrix_prev);
            }
        }
    } break;
            ...
            if (part._draw_mask & Renderer._mesh_draw_mask) {
                ...
                if (variation.frst) {
                    if (Renderer.firstPass() || variation.frst->all_passes) // if(Shader *shader=variation.frst->getShader())
                    {
                        if (variation.last_opaque_instance < 0) {
                            if (part._umm)
                                NewInstance(MultiMaterialShaderDraws, OpaqueShaderMaterialMeshInstances.elms(), *variation.frst, &part._umm->material_shader, part, variation);
                            else
                                NewInstance(ShaderDraws, OpaqueShaderMaterialMeshInstances.elms(), *variation.frst, &variation.getMaterial()._opaque_material_shader, part, variation);
                        } else
                            OpaqueShaderMaterialMeshInstances[variation.last_opaque_instance].next_instance = OpaqueShaderMaterialMeshInstances.elms();
                        variation.last_opaque_instance = OpaqueShaderMaterialMeshInstances.elms();
                    ...
                    if (Shader *shader = variation.shader[RM_BLEND])
                        BlendInstances.add(*shader, variation.getMaterial(), part, variation).setViewMatrix(view_matrix);
                    else if (variation.blst)
                        BlendInstances.add(*variation.blst, variation.getMaterial(), part, variation).setViewMatrix(view_matrix);
                }
            }
        }
    } break;
...
void MeshLod::draw(C MatrixM &matrix, C MatrixM &matrix_prev) C {
    ...
    case RT_DEFERRED: {
        Matrix view_matrix_prev;
        SetViewMatrixPrev(view_matrix_prev, matrix_prev);
        FREPA(T) // process in order
        {
            C MeshPart &part = parts[i];
            if (part._draw_mask & Renderer._mesh_draw_mask) {
    ...
    case RT_DEFERRED: {
        FREPA(T) // process in order
        {
            ...
            if (part._draw_mask & Renderer._mesh_draw_mask) {
                ...
                if (Shader *shader = variation.shader[Renderer._opaque_mode_index]) {
                    if (!opaque_skeleton)
                        opaque_skeleton = &SkeletonOpaqueInstances.getSkeletonInstanceOpaque(anim_skel);
...
#if SUPPORT_EMISSIVE
                    if (Shader *shader = variation.shader[RM_EMISSIVE])
                        SkeletonEmissiveInstances.New().set(part, variation, anim_skel);
#endif
                } else if (Shader *shader = variation.shader[RM_BLEND]) {
                    if (!blend_skeleton)
                        blend_skeleton = &BlendInstances.getSkeletonInstance(anim_skel);
                    blend_skeleton->addBlend(*shader, material, part);
                } else if (variation.blst) {
                    if (!blend_skeleton)
                        blend_skeleton = &BlendInstances.getSkeletonInstance(anim_skel);
                    blend_skeleton->addBlend(*variation.blst, material, part);
                }

                if (Shader *shader = variation.shader[RM_FUR]) {
                    if (!blend_skeleton)
                        blend_skeleton = &BlendInstances.getSkeletonInstance(anim_skel);
                    blend_skeleton->addFur(*shader, material, part);
                }
            }
        }
    } break;
...
void MeshLod::drawShadow(C MatrixM &matrix) C {
    DEBUG_ASSERT(Renderer() == RM_SHADOW, "'MeshLod.drawShadow' called outside of RM_SHADOW");
    Matrix view_matrix;
    Bool matrix_set = false; // set matrix only when needed, because some meshes may not have shadows
    REPA(T) {
        C MeshPart &part = parts[i];
        if (part._draw_mask & Renderer._mesh_draw_mask) {
            C MeshPart::Variation &variation = part.getVariation();
    ...
    REPA(T) {
        ...
        if (part._draw_mask & Renderer._mesh_draw_mask) {
            ...
            if (Shader *shader = variation.shader[RM_SHADOW]) {
                ...
                if (!shadow_skeleton)
                    shadow_skeleton = &SkeletonShadowInstances.getSkeletonInstanceShadow(anim_skel);
...
void MeshPart::drawBlend(C Vec4 *color) C {
    ...
    if (_draw_mask & Renderer._mesh_draw_mask) {
        ...
        if (!s && variation.blst)
            s = *(Shader **)(((Byte *)variation.blst) + Renderer._blst_light_offset); // then  try using Blend Light
        if (s) {
            C Material &material = variation.getMaterial();
            SetSkinning();
            D.cull(material.cull);
            D.depthOnWrite(true, material.hasDepthWrite());
            ...
            if (color || no_blend) {
                ...
                if (no_blend)
                    col.w = 1; // force full alpha for non-blending techniques
                if (color)
                    col *= *color;
...
void Mesh ::drawBlend(C MatrixM &matrix, C Vec4 *color) C {
    SetOneMatrix(matrix, matrix);
    getDrawLod(matrix).drawBlend(color);
} // blend might use velocity so set both matrixes
void Mesh ::drawBlend(C MatrixM &matrix, C MatrixM &matrix_prev, C Vec4 *color) C {
    SetOneMatrix(matrix, matrix_prev);
    getDrawLod(matrix).drawBlend(color);
}
void Mesh ::drawBlend(C AnimatedSkeleton &anim_skel, C Vec4 *color) C {
    anim_skel.setMatrix();
    getDrawLod(anim_skel.matrix()).drawBlend(color);
}
/******************************************************************************/
void MeshPart::drawBoneHighlight(Int bone) C {
    if (_draw_mask & Renderer._mesh_draw_mask)
        if (Shader *s = ShaderFiles("Bone Highlight")->get(S + FlagAll(render.flag(), VTX_SKIN) + (FlagOn(render.flag(), VTX_NRM) ? SBUMP_FLAT : SBUMP_ZERO))) {
            ...
            SetSkinning();
            D.cull(material.cull);
            D.depthOnWrite(true, true);
            Renderer.needDepthTest(); // !! 'needDepthTest' after 'depthWrite' !!
            D.alpha(ALPHA_NONE);
            material.setBlendForce();
            s->begin();
            render.set().drawBoneHighlight(bone, s);
        }
}
/******************************************************************************/
// OVERLAY
...
void MeshPart::drawOverlay(C Image &image, C Color &color) C {
    ...
    if (_draw_mask & Renderer._mesh_draw_mask) {
        C Variation &variation = getVariation();
        if (Shader *shader = variation.shader[RM_OVERLAY]) {
            SetSkinning();
...
void MeshPart::drawOutline(C Color &color) C {
    ...
    if (_draw_mask & Renderer._mesh_draw_mask) {
        C Variation &variation = getVariation();
        if (Shader *shader = variation.shader[RM_OUTLINE]) {
            C Material &material = variation.getMaterial();
...
void Mesh ::drawOutline(C Color &color, C MatrixM &matrix) C {
    SetOneMatrix(matrix);
    getDrawLod(matrix).drawOutline(color);
}
void Mesh ::drawOutline(C Color &color, C AnimatedSkeleton &anim_skel) C {
    anim_skel.setMatrix();
    getDrawLod(anim_skel.matrix()).drawOutline(color);
}
/******************************************************************************/
// OUTLINE
/******************************************************************************/
void MeshPart::drawBehind(C Color &color_perp, C Color &color_parallel) C {
    ...
    if (_draw_mask & Renderer._mesh_draw_mask) {
        C Variation &variation = getVariation();
        if (Shader *shader = variation.shader[RM_BEHIND]) {
            C Material &material = variation.getMaterial();
            SetSkinning();
            D.cull(material.cull);
            material.setBehind();
...
void Mesh ::drawBehind(C Color &color_perp, C Color &color_parallel, C MatrixM &matrix) C {
    SetOneMatrix(matrix);
    getDrawLod(matrix).drawBehind(color_perp, color_parallel);
}
void Mesh ::drawBehind(C Color &color_perp, C Color &color_parallel, C AnimatedSkeleton &anim_skel) C {
    anim_skel.setMatrix();
    getDrawLod(anim_skel.matrix()).drawBehind(color_perp, color_parallel);
}
/******************************************************************************/
} // namespace EE
/******************************************************************************/
