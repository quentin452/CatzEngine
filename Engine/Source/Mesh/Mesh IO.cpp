...
void XMaterial::create(C Material &src) {
    ...
    rough_mul = src.rough_mul;
    rough_add = src.rough_add;
    reflect_mul = src.reflect_mul;
    reflect_add = src.reflect_add;
    glow = src.glow;
    normal = src.normal;
    bump = src.bump;
...
Bool XMaterial::save(File &f) C { // !! IF CHANGING THIS IN ANY WAY THEN RECOMPILE FBX DLL'S !!
...
Bool XMaterial::load(File &f) {
    ...
    {
    case 0: {
        f >> cull >> flip_normal_y >> technique >> color >> emissive >> emissive_glow >> rough_mul >> rough_add >> reflect_mul >> reflect_add >> glow >> normal >> bump >> det_power >> det_uv_scale >> uv_scale >> color_map >> alpha_map >> bump_map >> glow_map >> emissive_map >> normal_map >> smooth_map >> metal_map >> detail_color_map >> detail_bump_map >> detail_normal_map >> detail_smooth_map >> name;
        if (f.ok())
            return true;
    } break;
    }
...
Bool MeshBase::saveData(File &f) C {
    ...
    if (flag & VTX_HLP)
        f.putN(vtx.hlp(), vtxs);
    if (flag & VTX_TEX0)
        f.putN(vtx.tex0(), vtxs);
    if (flag & VTX_TEX1)
        f.putN(vtx.tex1(), vtxs);
    if (flag & VTX_TEX2)
        f.putN(vtx.tex2(), vtxs);
    if (flag & VTX_TEX3)
        f.putN(vtx.tex3(), vtxs);
    if (flag & VTX_COLOR)
        f.putN(vtx.color(), vtxs);
    if (flag & VTX_MATERIAL)
        f.putN(vtx.material(), vtxs);
    if (flag & VTX_MATRIX)
        f.putN(vtx.matrix(), vtxs);
    if (flag & VTX_BLEND)
        f.putN(vtx.blend(), vtxs);
    if (flag & VTX_SIZE)
        f.putN(vtx.size(), vtxs);
    if (flag & VTX_FLAG)
        f.putN(vtx.flag(), vtxs);
    if (flag & VTX_DUP)
        f.putN(vtx.dup(), vtxs);

    if (flag & EDGE_NRM)
        f.putN(edge.nrm(), edges);
    if (flag & TRI_NRM)
        f.putN(tri.nrm(), tris);
    if (flag & QUAD_NRM)
        f.putN(quad.nrm(), quads);

    if (flag & EDGE_FLAG)
        f.putN(edge.flag(), edges);
    if (flag & TRI_FLAG)
        f.putN(tri.flag(), tris);
    if (flag & QUAD_FLAG)
        f.putN(quad.flag(), quads);

    if (flag & EDGE_ID)
        f.putN(edge.id(), edges);
    if (flag & TRI_ID)
        f.putN(tri.id(), tris);
    if (flag & QUAD_ID)
        f.putN(quad.id(), quads);

    if (flag & EDGE_IND)
        IndSave(f, edge.ind(), edges, vtxs);
    if (flag & TRI_IND)
        IndSave(f, tri.ind(), tris, vtxs);
    if (flag & QUAD_IND)
        IndSave(f, quad.ind(), quads, vtxs);
    if (flag & EDGE_ADJ_FACE)
        IndSave(f, edge.adjFace(), edges); // can't use size reduction for adjacent faces because they can contain -1 and SIGN_BIT
    if (flag & TRI_ADJ_FACE)
        IndSave(f, tri.adjFace(), tris); // can't use size reduction for adjacent faces because they can contain -1 and SIGN_BIT
    if (flag & QUAD_ADJ_FACE)
        IndSave(f, quad.adjFace(), quads); // can't use size reduction for adjacent faces because they can contain -1 and SIGN_BIT
    if (flag & TRI_ADJ_EDGE)
        IndSave(f, tri.adjEdge(), tris); // can't use size reduction for adjacent edges because they can contain -1
    if (flag & QUAD_ADJ_EDGE)
        IndSave(f, quad.adjEdge(), quads); // can't use size reduction for adjacent edges because they can contain -1
...
Bool MeshBase::loadData(File &f) {
    ...
    case 5: {
        ...
        if (flag & VTX_HLP)
            f.getN(vtx.hlp(), vtxs);
        if (flag & VTX_TEX0)
            f.getN(vtx.tex0(), vtxs);
        if (flag & VTX_TEX1)
            f.getN(vtx.tex1(), vtxs);
        if (flag & VTX_TEX2)
            f.getN(vtx.tex2(), vtxs);
        if (flag & VTX_TEX3)
            f.getN(vtx.tex3(), vtxs);
        if (flag & VTX_COLOR)
            f.getN(vtx.color(), vtxs);
        if (flag & VTX_MATERIAL)
            f.getN(vtx.material(), vtxs);
        if (flag & VTX_MATRIX)
            f.getN(vtx.matrix(), vtxs);
        if (flag & VTX_BLEND)
            f.getN(vtx.blend(), vtxs);
        if (flag & VTX_SIZE)
            f.getN(vtx.size(), vtxs);
        if (flag & VTX_FLAG)
        ...
        if (flag & EDGE_NRM)
            f.getN(edge.nrm(), edges);
        if (flag & TRI_NRM)
            f.getN(tri.nrm(), tris);
        if (flag & QUAD_NRM)
            f.getN(quad.nrm(), quads);

        if (flag & EDGE_FLAG)
            f.getN(edge.flag(), edges);
        if (flag & TRI_FLAG)
            f.getN(tri.flag(), tris);
        if (flag & QUAD_FLAG)
            f.getN(quad.flag(), quads);

        if (flag & EDGE_ID)
            f.getN(edge.id(), edges);
        if (flag & TRI_ID)
            f.getN(tri.id(), tris);
        if (flag & QUAD_ID)
            f.getN(quad.id(), quads);

        if (flag & EDGE_IND)
            IndLoad(f, edge.ind(), edges);
        if (flag & TRI_IND)
            IndLoad(f, tri.ind(), tris);
        if (flag & QUAD_IND)
            IndLoad(f, quad.ind(), quads);
        if (flag & EDGE_ADJ_FACE)
            IndLoad(f, edge.adjFace(), edges);
        if (flag & TRI_ADJ_FACE)
            IndLoad(f, tri.adjFace(), tris);
        if (flag & QUAD_ADJ_FACE)
            IndLoad(f, quad.adjFace(), quads);
        if (flag & TRI_ADJ_EDGE)
            IndLoad(f, tri.adjEdge(), tris);
        if (flag & QUAD_ADJ_EDGE)
            IndLoad(f, quad.adjEdge(), quads);
    ...
    case 2: {
        ...
        if (flag & EDGE_ID)
            f.skip(SIZE(VecI2) * edges);
        if (flag & TRI_ID)
            f.skip(SIZE(VecI2) * tris);
        if (flag & QUAD_ID)
            f.skip(SIZE(VecI2) * quads);
    ...
    case 1: {
        ...
        if (vtx.blend())
            REP(vtxs) {
                Flt f = (Flt &)vtx.blend(i);
                Byte b = RoundPos(f * 255);
                vtx.blend(i).set(b, 255 - b, 0, 0);
            }
        if (flag & VTX_SIZE)
            f.getN(vtx.size(), vtxs);
        if (flag & VTX_MATERIAL)
...
static void Fix(MeshRender &m, Bool color, Bool bone_weight) {
    ...
    if (col_ofs >= 0 || blend_ofs >= 0)
        if (Byte *vtx = m.vtxLock()) {
            REP(m._vb.vtxs()) {
                if (col_ofs >= 0) {
                    Color &c = *(Color *)(vtx + col_ofs);
                    Swap(c.r, c.b);
                }
                if (blend_ofs >= 0) {
                    VecB4 &b = *(VecB4 *)(vtx + blend_ofs);
                    Flt f = (Flt &)b;
                    Byte i = RoundPos(f * 255);
                    b.set(i, 255 - i, 0, 0);
                }
                vtx += m._vb._vtx_size;
            }
...
Bool MeshRender::loadData(File &f) {
        ...
        if (_vb.load(f))
            if (_ib.load(f)) {
                if (_vb._vtx_num || _ib._ind_num) {
                    ...
                    if (!setVF())
                        goto error; // !! call at the end (when have VB IB and flag/storage) !!
                }
        ...
        if (_vb.load(f))
            if (_ib.load(f)) {
                if (_vb._vtx_num || _ib._ind_num) {
                    Byte storage;
                    f >> storage;
                    _storage = (FlagOn(storage, 1 << 0) ? MSHR_COMPRESS : 0);
                    _flag = MeshFlagOld(f);
                    Mems<BoneSplit> bone_split;
                    bone_split._loadRaw(f);
                    _tris = _ib._ind_num / 3;
    ...
    case 3: {
        if (_vb.load(f))
            if (_ib.load(f)) {
                if (_vb._vtx_num || _ib._ind_num) {
                    Bool compressed, dx10;
                    f >> compressed >> dx10;
                    _storage = (compressed ? MSHR_COMPRESS : 0);
                    _flag = MeshFlagOld(f);
                    Mems<BoneSplit> bone_split;
                    bone_split._loadRaw(f);
                    _tris = _ib._ind_num / 3;
        ...
        if (vtxs && tris) {
            ...
            if (_vb.createNum(vtx_size, vtxs))
                if (_ib.create(tris * 3, bit16)) {
                    ...
                    T._tris = tris;
                    T._flag = flag;
                    if (!bone_split.loadRawData(f))
                        goto error;
                    Ptr data = vtxLock(LOCK_WRITE);
                    if (!f.get(data, vtxs * vtxSize()))
                        goto error;
                    vtxUnlock();
                    data = indLock(LOCK_WRITE);
                    if (!f.get(data, tris * tri_ind_size))
                        goto error;
                    indUnlock();
...
static void SetOldFlag(MeshPart &part, Byte flag) {
    if (flag & 0x1)
        part.part_flag |= MSHP_NO_PHYS_BODY;
    // if(flag&0x2)floor
    if (flag & 0x4)
        part.part_flag |= MSHP_HIDDEN;
}
Bool MeshPart::loadData(File &f, CChar *path, Int lod_index) {
            ...
            if (_variations.load(f, path))
                if (base.loadData(f))
                    if (render.loadData(f))
                        if (f.ok()) {
                            setUMM().setShader(lod_index);
                            return true;
                        }
    } break;

    case 7: {
        f.getStr(name).getMulti(part_flag, uv_scale, _draw_mask, _draw_mask_enum_id);
        if (uv_scale)
    ...
    case 6: {
        f._getStr1(name).getMulti(part_flag, uv_scale, _draw_mask, _draw_mask_enum_id);
        if (uv_scale)
            part_flag |= MSHP_HEIGHTMAP_FLAT;
        _materials[0].require(f.getAssetID(), path);
        _materials[1].require(f.getAssetID(), path);
    ...
    case 5: {
        f >> name;
        f.getMulti(part_flag, uv_scale, _draw_mask, _draw_mask_enum_id);
        if (uv_scale)
            part_flag |= MSHP_HEIGHTMAP_FLAT;
        _materials[0].require(f.getAssetID(), path);
        _materials[1].require(f.getAssetID(), path);
...
Bool MeshPart::loadOld0(File &f, CChar *path) {
    ...
    f >> material;
    Set(T.name, name);
    _variation.material.require(material, path);
    if (base.loadData(f))
        if (render.loadData(f))
            if (f.ok()) {
                setUMM().setShader(0);
...
Bool MeshPart::save(File &f, CChar *path) C {
    f.putUInt(CC4_MSHP);
    return saveData(f, path);
}
Bool MeshPart::load(File &f, CChar *path) {
    if (f.getUInt() == CC4_MSHP)
...
Bool MeshLod::save(File &f, CChar *path) C {
    f.putUInt(CC4_MSHL);
    return saveData(f);
}
Bool MeshLod::load(File &f, CChar *path) {
    if (f.getUInt() == CC4_MSHL)
        return loadData(f);
    del();
    return false;
}
...
Bool Mesh::loadData(File &f, CChar *path) {
    ...
    if (f.ok()) {
        if (App.flag & APP_AUTO_FREE_MESH_OPEN_GL_ES_DATA)
            freeOpenGLESData();
        return true;
    }
error:;
...
Bool Mesh::load(File &f, CChar *path) {
    ...
    case CC4_MSHB:
        if (create(1).parts[0].base.loadData(f)) {
            setBox();
            return true;
        }
        break;
    ...
    case CC4_MSHR:
        if (create(1).parts[0].render.loadData(f)) {
            setBox();
            return true;
        }
        break;
    ...
    case CC4_MSHL:
        if (del().MeshLod::loadData(f, path)) {
            setBox();
            return true;
        }
        break;
...
Bool MeshGroup::load(File &f, CChar *path) {
    ...
    case CC4_MESH:
        if (del().meshes.New().loadData(f, path)) {
            ext = meshes[0].ext;
            return true;
        }
        break;
...
Bool MeshGroup::loadAdd(File &f, CChar *path) {
    ...
    if (mshg.load(f, path)) {
        if (mshg.meshes.elms()) {
            ...
            FREPA(mshg)
            Swap(meshes[start + i], mshg.meshes[i]);
        }
        return true;
    }
    return false;
...
/******************************************************************************/
void MeshBase ::operator=(C Str &name) {
    if (!load(name))
        Exit(MLT(S + "Can't load mesh \"" + name + "\"", PL, S + u"Nie można wczytać modelu \"" + name + "\""));
}
void MeshRender::operator=(C Str &name) {
    if (!load(name))
        Exit(MLT(S + "Can't load mesh \"" + name + "\"", PL, S + u"Nie można wczytać modelu \"" + name + "\""));
}
void Mesh ::operator=(C Str &name) {
    if (!load(name))
        Exit(MLT(S + "Can't load mesh \"" + name + "\"", PL, S + u"Nie można wczytać modelu \"" + name + "\""));
}
void MeshGroup ::operator=(C Str &name) {
    if (!load(name))
        Exit(MLT(S + "Can't load mesh \"" + name + "\"", PL, S + u"Nie można wczytać modelu \"" + name + "\""));
}
/******************************************************************************/
...
Bool Import(C Str &name, Mesh *mesh, Skeleton *skeleton, MemPtr<XAnimation> animations, MemPtr<XMaterial> materials, MemPtr<Int> part_material_index, XSkeleton *xskeleton, Bool all_nodes_as_bones) {
    ...
    Str ext = GetExt(name);
    if (ext == "fbx")
        return ImportFBX(name, mesh, skeleton, animations, materials, part_material_index, xskeleton, all_nodes_as_bones);
    if (ext == "obj")
        return ImportOBJ(name, mesh, materials, part_material_index);
    if (ext == "mesh" || ext == "mshb" || ext == "mshr" || ext == "mshg") {
        if (mesh && mesh->load(name)) {
            if (mesh->skeleton())
                if (CChar *skel_name = Skeletons.name(mesh->skeleton())) {
                    mesh->skeleton()->load(skel_name);
                    mesh->skeleton(mesh->skeleton());
                } // because Import is used to load/reload, and because Skeleton is cached, then force reload skeleton from file, because it's possible that skel file was changed, however due to caching it will not be reloaded
            ...
            if (materials || part_material_index) {
                ...
                if (part_material_index)
                    FREPD(l, mesh->lods()) // set 'part_material_index'
                    {
                        C MeshLod &lod = mesh->lod(l);
                        FREPA(lod)
                        part_material_index.add(mesh_mtrls.find(lod.parts[i].material()()));
                    }
            }
            return true;
        } else if (ext == "mesh")
        ...
        if (skeleton)
            return skeleton->load(name);
    } else if (ext == "anim") {
        if (animations && animations.New().anim.load(name)) {
            animations.last().name = GetBaseNoExt(name);
            return true;
        }
    } else if (ext == "mtrl") {
        Material mtrl;
        ...
        if (ImportDAE(name, mesh, skeleton, animations ? &animations.New() : null, materials, part_material_index)) {
            if (animations.elms() && !animations.first().anim.is())
                animations.clear();
            return true;
        }
    } else if (ext == "b3d") {
        ...
        return ImportASE(name, mesh, materials, part_material_index);
    } else if (ext == "3ds") {
        return Import3DS(name, mesh, materials, part_material_index);
    } else if (ext == "psk") {
        return ImportPSK(name, mesh, skeleton, materials, part_material_index);
    } else if (ext == "psa") {
        return ImportPSA(name, skeleton, animations);
    } else if (ext == "xps") {
        return ImportXPSBinary(name, mesh, skeleton, materials, part_material_index);
    } else if (ext == "ascii") {
        return ImportXPSText(name, mesh, skeleton, materials, part_material_index);
    }
    return false;
}
/******************************************************************************
...
Int LodIndex(Str &name) {
    ...
    for (; CharFlag(name[index - 1]) & CHARF_DIG;)
        index--;
    Int index4 = index - 4;
    if (index4 >= 0 && Starts(name() + index4, "_LOD", true)) {
        lod_i = TextInt(name() + index);
        if (!InRange(lod_i, 16))
            lod_i = -1;    // keep within sensible ranges
...
void UniqueBoneNames(MemPtrN<Str8, 256> names) {
    FREPA(names) {
        Str8 &name = names[i];
        if (!name.is())
            name = "Root"; // we don't allow empty names because 'findBone' methods may skip searching if the name parameter is empty
    again:
        REPD(j, i) {
            C Str8 &test = names[j];
            if (name == test) {
                ...
                Char separator = '#';
                REPA(name)
                if (name[i] == separator) // find if this name already has a separator
                {
                    index = TextInt(name() + i + 1) + 1;
                    name.clip(i);
                    break;
                } // get the index value, increase by one, and remove the separator
                name += separator;
                name += index;
                goto again;
            }
...
void ShortenBoneNames(MemPtrN<Str8, 256> names) {
    ...
    if (name_length > max_name_length) // if name length exceeds allowed limit, then shorten names by removing the shared start
    {
        ...
        REPA(names) {
            Str8 &name = names[i];
            if (name_length)
                name.remove(0, name_length + (name[name_length] == ' ')); // if this name has a space after shared start, then remove that space too
            // even after removing shared start, bone name can still be too long
            if (name.length() > max_name_length)
                name.clip(max_name_length - 1 - 2); // leave room for 1 char separator + 2 char index
        }
