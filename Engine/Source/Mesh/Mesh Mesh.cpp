...
Bool Mesh::Variations::save(File &f) C {
    f.cmpUIntV(_variations);
    if (_variations) {
        Int name_size = nameSize();
        f.cmpUIntV(name_size);
        FREP(_variations) {
            C Variation &variation = _variation[i];
            f << variation.id;
            f.cmpUIntV(variation.name_offset);
        }
...
Bool Mesh::Variations::load(File &f) {
    _variations = f.decUIntV();
    if (_variations) {
        Int name_size = f.decUIntV();
        alloc(_variations, name_size);
        FREP(_variations) {
            Variation &variation = _variation[i];
            f >> variation.id;
            variation.name_offset = f.decUIntV();
        }
...
static void SetVariations(Memt<VariationEx> &variations, Mesh &mesh) {
    ...
    Int name_size = 0;
    for (Int i = 1; i < variations.elms(); i++)
        name_size += variations[i].name.length() + 1;
...
Mesh &Mesh::variations(Int variations) {
    ...
    if (T.variations() != variations) {
        Memt<VariationEx> variation_exs;
        GetVariations(variation_exs, T);
        variation_exs.setNum(variations);
        for (Int i = T.variations(); i < variation_exs.elms(); i++)
            variation_exs[i].randomizeID(); // randomize ID
        ...
        REP(lods()) {
            MeshLod &lod = T.lod(i);
            REPAO(lod.parts).variations(variation_exs.elms());
        }
...
Mesh &Mesh::variationName(Int variation, C Str8 &name) {
    if (variation > 0 // can't set name to 0-th default variation
        ...
        Memt<VariationEx> variations;
        GetVariations(variations, T);
...
Mesh &Mesh::variationRemove(Int variation) {
    if (InRange(variation, variations()) && variations() > 1) {
        Int num = T.variations();
        REP(lods()) {
            MeshLod &lod = T.lod(i);
            REPA(lod)
            lod.parts[i].variations(num).variationRemove(variation);
        } // allocate all first to make sure others will be processed ok
        Memt<VariationEx> variations;
        GetVariations(variations, T);
...
Mesh &Mesh::variationKeep(Int variation) {
    if (variations() > 1) {
        REP(lods()) {
            MeshLod &lod = T.lod(i);
            REPA(lod)
            lod.parts[i].variationKeep(variation);
        }
        _variations.del();
    }
    return T;
...
Mesh &Mesh::variationMove(Int variation, Int new_index) {
    if (InRange(variation, variations())) {
        Clamp(new_index, 0, variations() - 1);
        if (new_index != variation) {
            Int num = T.variations();
            REP(lods()) {
                MeshLod &lod = T.lod(i);
                REPA(lod)
                lod.parts[i].variations(num).variationMove(variation, new_index);
            } // allocate all first to make sure others will be processed ok
            Memt<VariationEx> variations;
            GetVariations(variations, T);
...
Mesh &Mesh::create(C Mesh &src, MESH_FLAG flag_and) {
    if (this == &src)
        ...
        _lods.setNum(src._lods.elms());
        FREP(lods())
        lod(i).create(src.lod(i), flag_and);
        copyParams(src);
    }
    return T;
}
...
Mesh &Mesh::create(C Mesh *mesh, Int meshes, MESH_FLAG flag_and) {
    if (!mesh || meshes <= 0)
        ...
        Int lods = 0;
        REP(meshes)
        MAX(lods, mesh[i].lods());            // get max number of LODs
        dest.setLods(lods).copyParams(*mesh); // copy params from first Mesh
        REPD(l, dest.lods()) {
            MeshLod &lod = dest.lod(l);
            Int parts = 0;
            REP(meshes) {
                C Mesh &src_mesh = mesh[i];
                if (InRange(l, src_mesh.lods()))
                    parts += src_mesh.lod(l).parts.elms();
            } // how many parts for this LOD
            lod.parts.setNum(parts);
            parts = 0; // always set LOD parts in case we're operating on MeshGroup that already had some data, and this will clear it
            Bool params = true;
            FREP(meshes) // add in order
            {
                ...
                if (InRange(l, src_mesh.lods())) {
                    ...
                    if (params) {
                        lod.copyParams(src_lod);
                        params = false;
                    } // copy params from first encountered LOD
                    FREPA(src_lod)
                    lod.parts[parts++].create(src_lod.parts[i], flag_and);
                }
            }
        }
        ...
        REP(meshes) {
            ...
            if (src_mesh.vtxs()) {
                if (valid)
                    ...
                    box = src_mesh.ext;
                    dest.lod_center = src_mesh.lod_center;
                }
                valid++;
            } // copy 'lod_center' on first found valid mesh
        }
        if (valid) // if encountered any valid mesh vtxs (otherwise keep what was already set in 'copyParams')
        {
            dest.ext = box;
            if (valid > 1)
                dest.lod_center = dest.ext.pos; // if more than 1 valid mesh, then use box center, if only 1 valid mesh then use its 'lod_center' that we've already set in the loop
        }
        if (&dest == &temp)
            Swap(temp, T);
    }
...
/******************************************************************************/
Mesh &Mesh::include(MESH_FLAG flag) {
    REP(lods())
    lod(i).include(flag);
    return T;
}
Mesh &Mesh::exclude(MESH_FLAG flag) {
    REP(lods())
    lod(i).exclude(flag);
    return T;
}
Mesh &Mesh::keepOnly(MESH_FLAG flag) {
    REP(lods())
    lod(i).keepOnly(flag);
    return T;
}
...
// GET
/******************************************************************************/
MESH_FLAG Mesh::flag() C {
    MESH_FLAG flag = MESH_NONE;
    REP(lods())
    flag |= lod(i).flag();
    return flag;
}
UInt Mesh::memUsage() C {
    UInt size = 0;
    REP(lods())
    size += lod(i).memUsage();
    return size;
...
C MeshLod &Mesh::getDrawLod(Flt dist2) C {
    REPA(_lods) {
        C MeshLod &test = _lods[i];
        if (dist2 >= test.dist2)
            return test;
...
// SET
/******************************************************************************/
Mesh &Mesh::setEdgeNormals(Bool flag) {
    REP(lods())
    lod(i).setEdgeNormals(flag);
    return T;
}
Mesh &Mesh::setFaceNormals() {
    REP(lods())
    lod(i).setFaceNormals();
    return T;
}
Mesh &Mesh::setNormals2D(Bool flag) {
    REP(lods())
    lod(i).setNormals2D(flag);
    return T;
}
Mesh &Mesh::setNormals() {
    REP(lods())
    lod(i).setNormals();
    return T;
}
Mesh &Mesh::setNormalsAuto(Flt angle, Flt pos_eps) {
    REP(lods())
    lod(i).setNormalsAuto(angle, pos_eps);
    return T;
}
Mesh &Mesh::setTanBin() {
    REP(lods())
...
Mesh &Mesh::setTanBinDbl() {
    REP(lods())
    lod(i).setTanBinDbl();
    return T;
}
Mesh &Mesh::setAutoTanBin() {
    REP(lods())
    lod(i).setAutoTanBin();
    return T;
}
Bool Mesh::setBox(Bool skip_hidden_parts) {
    Box box;
    if (getBox(box, skip_hidden_parts)) {
        ext = box;
        lod_center = ext.pos;
        return true;
    }
...
Mesh &Mesh::setVtxDup2D(MESH_FLAG flag, Flt pos_eps, Flt nrm_cos) {
    REP(lods())
    lod(i).setVtxDup2D(flag, pos_eps, nrm_cos);
    return T;
}
Mesh &Mesh::setVtxDup(MESH_FLAG flag, Flt pos_eps, Flt nrm_cos) {
    REP(lods())
    lod(i).setVtxDup(flag, pos_eps, nrm_cos);
    return T;
}
Mesh &Mesh::setVtxColorAlphaAsTesselationIntensity(Bool tesselate_edges) {
    REP(lods())
    lod(i).setVtxColorAlphaAsTesselationIntensity(tesselate_edges);
    return T;
}
Mesh &Mesh::setAdjacencies(Bool faces, Bool edges) {
    REP(lods())
    lod(i).setAdjacencies(faces, edges);
    return T;
}

Mesh &Mesh::setBase(Bool only_if_empty) {
    REP(lods())
    lod(i).setBase(only_if_empty);
    return T;
}
Mesh &Mesh::setRender(Bool optimize) {
    REP(lods())
    lod(i).setRender(optimize, i);
    return T;
}
Mesh &Mesh::setShader() {
    REP(lods())
...
Mesh &Mesh::material(C MaterialPtr &material) {
    REP(lods())
    lod(i).material(material, i);
    return T;
}
/******************************************************************************/
// JOIN / SPLIT
/******************************************************************************/
Mesh &Mesh::join(Int i0, Int i1, Flt weld_pos_eps) {
    super::join(i0, i1, weld_pos_eps);
    return T;
}
Mesh &Mesh::joinAll(Bool test_material, Bool test_draw_group, Bool test_name, MESH_FLAG test_vtx_flag, Flt weld_pos_eps) {
    REP(lods())
    lod(i).joinAll(test_material, test_draw_group, test_name, test_vtx_flag, weld_pos_eps);
    return T;
}
/******************************************************************************/
// TEXTURIZE
/******************************************************************************/
Mesh &Mesh::texMap(Flt scale, Byte uv_index) {
    REP(lods())
    lod(i).texMap(scale, uv_index);
    return T;
}
Mesh &Mesh::texMap(C Matrix &matrix, Byte uv_index) {
    REP(lods())
    lod(i).texMap(matrix, uv_index);
    return T;
}
Mesh &Mesh::texMap(C Plane &plane, Byte uv_index) {
    REP(lods())
    lod(i).texMap(plane, uv_index);
    return T;
}
Mesh &Mesh::texMap(C Ball &ball, Byte uv_index) {
    REP(lods())
    lod(i).texMap(ball, uv_index);
    return T;
}
Mesh &Mesh::texMap(C Tube &tube, Byte uv_index) {
    REP(lods())
    lod(i).texMap(tube, uv_index);
    return T;
}
Mesh &Mesh::texMove(C Vec2 &move, Byte uv_index) {
    REP(lods())
    lod(i).texMove(move, uv_index);
    return T;
}
Mesh &Mesh::texScale(C Vec2 &scale, Byte uv_index) {
    REP(lods())
    lod(i).texScale(scale, uv_index);
    return T;
}
Mesh &Mesh::texRotate(Flt angle, Byte uv_index) {
    REP(lods())
    lod(i).texRotate(angle, uv_index);
    return T;
}
/******************************************************************************/
// TRANSFORM
/******************************************************************************/
Mesh &Mesh::move(C Vec &move) {
    REP(lods())
    lod(i).move(move);
    ext += move;
    lod_center += move;
...
Mesh &Mesh::scaleMove(C Vec &scale, C Vec &move) {
    REP(lods())
    lod(i).scaleMove(scale, move);
    ext *= scale;
    ext += move;
    lod_center *= scale;
    lod_center += move;
...
Mesh &Mesh::scaleMoveBase(C Vec &scale, C Vec &move) {
    REP(lods())
    lod(i).scaleMoveBase(scale, move);
    ext *= scale;
    ext += move;
    lod_center *= scale;
    lod_center += move;
...
Mesh &Mesh::transform(C Matrix3 &matrix) {
    REP(lods())
    lod(i).transform(matrix);
    ext *= matrix;
    lod_center *= matrix;
    return T;
}
Mesh &Mesh::transform(C Matrix &matrix) {
    REP(lods())
    lod(i).transform(matrix);
    ext *= matrix;
    lod_center *= matrix;
...
Mesh &Mesh::animate(C MemPtrN<Matrix, 256> &matrixes) {
    REP(lods())
    lod(i).animate(matrixes);
    setBox();
    return T;
}
Mesh &Mesh::animate(C MemPtrN<MatrixM, 256> &matrixes) {
    REP(lods())
    lod(i).animate(matrixes);
    setBox();
    return T;
}
Mesh &Mesh::animate(C AnimatedSkeleton &skel) {
    REP(lods())
    lod(i).animate(skel);
    setBox();
    return T;
...
Mesh &Mesh::rightToLeft() {
    REP(lods())
    lod(i).rightToLeft();
    ext.rightToLeft();
    lod_center.rightToLeft();
    return T;
}
/******************************************************************************/
// OPERATIONS
/******************************************************************************/
Mesh &Mesh::weldVtx2D(MESH_FLAG flag, Flt pos_eps, Flt nrm_cos, Flt remove_degenerate_faces_eps) {
    REP(lods())
    lod(i).weldVtx2D(flag, pos_eps, nrm_cos, remove_degenerate_faces_eps);
    return T;
}
Mesh &Mesh::weldVtx(MESH_FLAG flag, Flt pos_eps, Flt nrm_cos, Flt remove_degenerate_faces_eps) {
    REP(lods())
    lod(i).weldVtx(flag, pos_eps, nrm_cos, remove_degenerate_faces_eps);
    return T;
}
Mesh &Mesh::weldVtxValues(MESH_FLAG flag, Flt pos_eps, Flt nrm_cos, Flt remove_degenerate_faces_eps) {
    REP(lods())
    lod(i).weldVtxValues(flag, pos_eps, nrm_cos, remove_degenerate_faces_eps);
    return T;
}
...
Mesh &Mesh::explodeVtxs() {
    REP(lods())
    lod(i).explodeVtxs();
    return T;
}
Mesh &Mesh::tesselate(Flt weld_pos_eps) {
    REP(lods())
    lod(i).tesselate(weld_pos_eps);
    return T;
}
Mesh &Mesh::subdivide() {
    REP(lods())
...
Mesh &Mesh::setVtxAO(Flt strength, Flt bias, Flt max, Flt ray_length, Flt pos_eps, Int rays, MESH_AO_FUNC func, Threads *threads) {
    REP(lods())
    lod(i).setVtxAO(strength, bias, max, ray_length, pos_eps, rays, func, threads);
    return T;
}

Mesh &Mesh::freeOpenGLESData() {
    REP(lods())
    lod(i).freeOpenGLESData();
    return T;
}

Bool Mesh::waitForStream() C {
    Bool ok = true;
    REP(lods())
    ok &= lod(i).waitForStream();
    return ok;
}
/******************************************************************************/
// FIX
/******************************************************************************/
Mesh &Mesh::fixTexWrapping(Byte uv_index) {
    REP(lods())
    lod(i).fixTexWrapping(uv_index);
    return T;
}
Mesh &Mesh::fixTexOffset(Byte uv_index) {
    REP(lods())
    lod(i).fixTexOffset(uv_index);
    return T;
}
/******************************************************************************/
// CONVERT
/******************************************************************************/
Mesh &Mesh::edgeToDepth(Bool tex_align) {
    REP(lods())
    lod(i).edgeToDepth(tex_align);
    return T;
}
Mesh &Mesh::edgeToTri(Bool set_id) {
    REP(lods())
    lod(i).edgeToTri(set_id);
    return T;
}
Mesh &Mesh::triToQuad(Flt cos) {
    REP(lods())
    lod(i).triToQuad(cos);
    return T;
}
Mesh &Mesh::quadToTri(Flt cos) {
    REP(lods())
    lod(i).quadToTri(cos);
    return T;
}
...
Mesh &Mesh::add(C MeshBase &src) {
    Box box;
    Bool valid_box = src.getBox(box), this_box = (vtxs() > 0);
    super::add(src);
    if (valid_box) {
        if (this_box)
            T.ext |= box;
...
Mesh &Mesh::variationInclude(C Mesh &src) {
    FREP(src.variations())
    if (variationFind(src.variationName(i)) < 0) // add in same order
    {
...
Mesh &Mesh::add(C Mesh &src) {
    ...
    if (src.lods() > lods())
        setLods(src.lods()); // make room for LODs
    FREP(src.lods())
    lod(i).add(src.lod(i), &src, this);
    return T;
}
...
// OPTIMIZE
/******************************************************************************/
Mesh &Mesh::removeDoubleEdges() {
    REP(lods())
    lod(i).removeDoubleEdges();
    return T;
}
Mesh &Mesh::removeDegenerateFaces(Flt eps) {
    REP(lods())
    lod(i).removeDegenerateFaces(eps);
    return T;
}
Bool Mesh::removeUnusedVtxs(Bool include_edge_references, Bool set_box) {
    Bool changed = false;
    REP(lods())
    changed |= lod(i).removeUnusedVtxs(include_edge_references);
    if (changed && set_box)
        setBox();
    return changed;
}
Mesh &Mesh::removeSingleFaces(Flt fraction) {
    REP(lods())
    lod(i).removeSingleFaces(fraction);
    return T;
}
Mesh &Mesh::weldInlineEdges(Flt cos_edge, Flt cos_vtx, Bool z_test) {
    REP(lods())
    lod(i).weldInlineEdges(cos_edge, cos_vtx, z_test);
    return T;
}
Bool Mesh::weldCoplanarFaces(Flt cos_face, Flt cos_vtx, Bool safe, Flt max_face_length) {
    Bool changed = false;
    REP(lods())
    changed |= lod(i).weldCoplanarFaces(cos_face, cos_vtx, safe, max_face_length);
    return changed;
}
Mesh &Mesh::sortByMaterials() {
    REP(lods())
    lod(i).sortByMaterials();
    return T;
}
...
Mesh &Mesh::simplify(Flt intensity, Flt max_distance, Flt max_uv, Flt max_color, Flt max_material, Flt max_skin, Flt max_normal, Bool keep_border, MESH_SIMPLIFY mode, Flt pos_eps, Mesh *dest, Bool *stop) {
    ...
    REP(lods()) // go from the end so we can remove LODs if needed
    {
        ...
        lod(i).simplify(intensity, max_distance, max_uv, max_color, max_material, max_skin, max_normal, keep_border, mode, pos_eps, &dest_lod, stop);
        if (stop && *stop)
            break;
        if (!dest_lod.is())
            dest->removeLod(i); // if became empty, then just remove it
    }
    return *dest;
}
/******************************************************************************/
