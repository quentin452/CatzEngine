...
static void OptimizeFaces(const Index *indexList, UInt indexCount, UInt vertexCount, Index *newIndexList, UInt lruCacheSize) {
    ...
    Mems<OptimizeVertexData> vertexDataList;
    vertexDataList.setNum(vertexCount);
    ...
    Mems<Bool> processedFaceList;
    processedFaceList.setNumZero(indexCount);
...
ScoreType findVertexScore(int numActiveTris, int cachePosition) {
    if (!numActiveTris)
        return 0; // No triangles need this vertex
    ScoreType score = 0;
    if (InRange(cachePosition, cachePositionScore))
        score = cachePositionScore[cachePosition];
    if (InRange(numActiveTris, valenceScore))
...
void reorderForsyth(const VertexIndexType *indices, int nTriangles, int nVertices, VertexIndexType *outputIndices) {
    ...
    for (int i = 0; i < nVertices; i++) {
        ...
        REPD(j, tris)
        triangleScore[triangleIndices[offset + j]] += score;
    }
    ...
    for (int i = 0; i < nTriangles; i++) {
        ...
        for (int j = 0; j < 3; j++)
            outputIndices[outPos++] = indices[t + j];
    }
...
MeshBase &MeshBase::optimizeCache(Bool faces, Bool vertexes) {
    ...
    if (faces) {
        ...
        ASSERT(Int(UNUSED32) == -1); // 'setAdjacencies' sets -1 for invalid while these codes operate on UNUSED32
        ASSERT(SIZE(tri.ind(0)) == SIZE(uint32_t) * 3);
        ASSERT(SIZE(tri.adjFace(0)) == SIZE(uint32_t) * 3);
        remap.reserve(Max(tris(), vertexes ? vtxs() : 0));
        ASSERT(SIZE(Int) == SIZE(uint32_t));
        remap.setNum(tris());
        if (OK(DirectX::OptimizeFaces((uint32_t *)tri.ind(), tris(), (uint32_t *)tri.adjFace(), (uint32_t *)remap.data(), VTX_CACHE_SIZE, VTX_CACHE_SIZE))) {
            ...
            REPA(remap)
            temp.tri.ind(i) = tri.ind(remap[i]);
            Swap(temp.tri, tri);
        }
#else
    ...
    if (vertexes) {
        ...
        if (vertexes) {
            ...
            REPA(temp.vtx)
            copyVtx(i, temp, remap[i]);
            Swap(temp.vtx, vtx);
            REPA(tri)
            tri.ind(i).remap(remap.data());
            REPA(quad)
            quad.ind(i).remap(remap.data());
        }
    }
    return T;
}
...
MeshRender &MeshRender::optimize(Bool faces, Bool vertexes) {
...
#endif
    {
        ...
        if (temp.createInd(_ib)) {
            ...
            if (ok)
                if (ind = indLock(LOCK_WRITE)) {
                    if (indBit16()) {
                        VecUS *tri = (VecUS *)ind;
                        REP(tris())
                        tri[i] = temp.tri.ind(remap[i]);
                    } else {
                        VecI *tri = (VecI *)ind;
                        REP(tris())
                        tri[i] = temp.tri.ind(remap[i]);
        ...
        if (ind = indLock()) {
            ...
            if (indBit16())
                Copy16To32(org, ind, tris() * 3);
            else
                Copy32To32(org, ind, tris() * 3);
            ...
            if (indBit16())
                Copy32To16(ind, reordered, tris() * 3);
            else
                Copy32To32(ind, reordered, tris() * 3);
        }
#endif
    }
        ...
        if (ind) {
            ...
            if (vertexes) {
                ...
                if (vertexes)
                    if (Byte *vtx = vtxLock()) {
                        Memt<Byte> vtx_temp;
                        vtx_temp.setNum(vtxs() * vtxSize());
                        REP(vtxs())
                        CopyFast(vtx_temp.data() + remap[i] * vtxSize(), vtx + i * vtxSize(), vtxSize());
