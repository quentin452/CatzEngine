    ...
    }else*/
    {
        ...
        FREPD(l, src.lods()) // order is important
        {
            ...
            REPA(src_lod) {
                ...
                if (part.material())
                    if (part.material()->hasAlpha() || part.material()->technique == MTECH_FUR)
                ...
                if (!semi_transparent) // don't apply overlays on semi-transparent parts (because it will look like floating in air)
                {
                    if (part.base.is()) // MSHB
                    {
                        ...
                        REPA(part.base.tri) // TRI
                        {
                            ...
                            Box b;
                            b.from(poly[poly_cur].data(), poly[poly_cur].elms());
                            if (Cuts(b, box)) {
                                REPD(p, clip_planes) {
                                    ClipPoly(poly[poly_cur], clip_plane[p], poly[poly_cur ^ 1]);
                                    poly_cur ^= 1;
                                    if (poly[poly_cur].elms() <= 2)
                                        goto mshb_tri_done;
                                }
            ...
            if (polys.elms()) {
                MeshBase mshb;
                Triangulate(polys, mshb, EPS, true);
                polys.clear();
        ...
        temp.setBox();
        temp.lod_center = src.lod_center;
        if (matrix)
            temp.lod_center *= *matrix;
    }
...
Bool MeshOverlay::createStatic(C Mesh &mesh, C MaterialPtr &material, C Matrix &overlay_matrix, C Matrix *mesh_matrix) {
    ...
    if (mesh.is()) {
        ...
        Memc<Plane> planes;
        Box box;
        SetBoxAndPlanes(box, planes, _matrix);
        Mesh temp;
        ClipMeshToPlanes(mesh, mesh_matrix, temp, planes.data(), planes.elms(), box);
...
Bool MeshOverlay::createAnimated(C Mesh &mesh, C MaterialPtr &material, C Matrix &overlay_matrix, C Matrix *mesh_matrix) {
    ...
    if (mesh.is()) {
        ...
        SetBoxAndPlanes(box, planes, _matrix);
        Mesh temp;
        ClipMeshToPlanes(mesh, null, temp, planes.data(), planes.elms(), box);
...
void MeshOverlay::setShader() {
    _shader = null;
    if (_lods.elms() && _material && _material->base_0)
        _shader = ShaderFiles("Mesh Overlay")->get(ShaderMeshOverlay(false, D.bumpMode() > BUMP_FLAT && _material->normal > EPS_COL8 && _material->base_1 != null, (_material->base_2 != null) ? 2 : 1)); // #MaterialTextureLayout
}
...
void MeshOverlay::draw(Flt alpha) C {
    if (_shader) {
        setParams(alpha);
        _shader->begin();
        C MeshRender &mshr = getDrawLod(ObjMatrix);
        mshr.set().draw();
    }
}
void MeshOverlay::draw(C MatrixM &matrix, Flt alpha) C {
    if (_shader) {
        ...
        setParams(alpha);
        _shader->begin();
        C MeshRender &mshr = getDrawLod(matrix);
        mshr.set().draw();
    }
}
void MeshOverlay::draw(C AnimatedSkeleton &anim_skel, Flt alpha) C {
    if (_shader) {
        ...
        setParams(alpha);
        _shader->begin();
        C MeshRender &mshr = getDrawLod(anim_skel.matrix());
        mshr.set();
        mshr.draw();
    }
}
