...
static void SetLeafAttachment(MeshBase &mesh, C Vec2 &tex, Memc<Int> &faces) {
    ...
    if (found != -1) // compare to -1 and not <0 because 'found' can have SIGN_BIT
    {
        ...
        */
        ...
        if (Solve(u_dir.x, u_dir.y, v_dir.x, v_dir.y, tex_frac.x - base.x, tex_frac.y - base.y, u, v) == 1)
            pos = pos_tri.p[0] + u * (pos_tri.p[1] - pos_tri.p[0]) + v * (pos_tri.p[2] - pos_tri.p[0]);
        else
            pos = pos_tri.center();
...
static void SetLeafAttachment(MeshBase &mesh, C CMemPtr<LeafAttachment> &attachments, Memc<Int> &faces) {
    ...
    Int best_attachment = 0;
    if (attachments.elms() > 1) // if have more than 1 attachments, then check which one is best (closest)
    {
        VecD2 face_tex = 0;
        Dbl weight = 0;
        ...
        REPA(faces) {
            ...
            if (f & SIGN_BIT) {
                VecI4 ind = mesh.quad.ind(f ^ SIGN_BIT);
                Flt w = QuadArea2(pos[ind.x], pos[ind.y], pos[ind.z], pos[ind.w]);
                Vec2 t = Avg(tx0[ind.x], tx0[ind.y], tx0[ind.z], tx0[ind.w]);
                weight += w;
                face_tex += w * t;
            } else {
                VecI ind = mesh.tri.ind(f);
                Flt w = TriArea2(pos[ind.x], pos[ind.y], pos[ind.z]);
                Vec2 t = Avg(tx0[ind.x], tx0[ind.y], tx0[ind.z]);
                weight += w;
                face_tex += w * t;
            }
        }
        ...
        Flt dist = FLT_MAX;
        REPA(attachments) {
            Flt d = Dist2Wrap(ft, attachments[i].center);
            if (d < dist) {
                dist = d;
                best_attachment = i;
            }
...
static void SetLeafAttachment(MeshBase &mesh, C CMemPtr<LeafAttachment> &attachments) {
    if (mesh.vtx.tex0()) {
        ...
        Memc<Int> faces;
        Mems<Bool> tri_done;
        tri_done.setNumZero(mesh.tris());
        Mems<Bool> quad_done;
        quad_done.setNumZero(mesh.quads());
        ...
        if (!tri_done[i]) {
            ...
            for (Int last = 0; last != faces.elms();) {
                ...
                for (Int i = faces.elms(); --i >= last;) // search recently added, order is important because 'faces' will receive new elements continuously
                {
                    ...
                    Int f = faces[i];
                    if (f & SIGN_BIT)
                        a = mesh.quad.adjFace(f ^ SIGN_BIT); // get all adjacent faces
                    else
                        a.set(mesh.tri.adjFace(f), -1); // get all adjacent faces
                    ...
                    REPA(a) // iterate through all adjacent faces
                    {
                        ...
                        {
                            if (af & SIGN_BIT) {
                                if (!quad_done[af ^ SIGN_BIT]) {
                                    quad_done[af ^ SIGN_BIT] = true;
                                    faces.add(af);
                                }
                            } else {
                                if (!tri_done[af]) {
...
static void SetRandomColor(MeshBase &mesh, Memc<Int> &faces, C Color &color) {
    REPA(faces) {
        Int f = faces[i];
        if (f & SIGN_BIT) {
            VecI4 q = mesh.quad.ind(f ^ SIGN_BIT);
            REPA(q)
            mesh.vtx.color(q.c[i]) = color;
        } else {
            VecI t = mesh.tri.ind(f);
            REPA(t)
            mesh.vtx.color(t.c[i]) = color;
        }
    }
}
...
static void SetRandomBend(MeshBase &mesh, Memc<Int> &faces, Flt value) {
    REPA(faces) {
        ...
        if (f & SIGN_BIT) {
            VecI4 q = mesh.quad.ind(f ^ SIGN_BIT);
            REPA(q)
            mesh.vtx.size(q.c[i]) = value;
        } else {
            VecI t = mesh.tri.ind(f);
            REPA(t)
            mesh.vtx.size(t.c[i]) = value;
        }
    }
}
...
void MeshPart::Variation::zero() {
    last_opaque_instance = last_shadow_instance = -1;
    REPAO(shader) = null;
    frst = null;
    blst = null;
}
void MeshPart::Variation::del() {
    material.clear();
    zero();
}
Bool MeshPart::Variation::save(File &f, CChar *path) C {
    f.putAsset(material.id());
    return f.ok();
}
Bool MeshPart::Variation::load(File &f, CChar *path) {
    material.require(f.getAssetID(), path);
    return f.ok();
}
/******************************************************************************/
...
MeshPart &MeshPart::create(C MeshPart &src, MESH_FLAG flag_and) {
    if (this == &src)
        ...
        if (src_render_flag == new_render_flag) {
            if (!render.create(src.render))
                setRender();
        } else {
            C MeshRender *src_render[] = {&src.render};
            if (render.create(src_render, Elms(src_render), flag_and))
                setShader(0);
            else
                setRender();
        }
...
void MeshPart::copyParams(C MeshPart &src, Bool copy_shaders) {
    if (this != &src) {
        ...
        _variation = src._variation;
        _variation.unlink();
        _variations = src._variations;
        REPAO(_variations).unlink();
...
MeshPart &MeshPart::include(MESH_FLAG flag) {
    Bool base_is = base.is();
    if (!base_is && render.is())
        base.create(render);
    base.include(flag);
    return T;
} // don't create 'render' from uninitialized data
MeshPart &MeshPart::exclude(MESH_FLAG flag) {
    if ((base.flag() | render.flag()) & flag) {
        Bool base_is = base.is();
        if (!base_is && render.is())
            base.create(render);
        base.exclude(flag);
...
C MaterialPtr &MeshPart::multiMaterial(Int i) C {
    if (!i)
        return _variation.material;
    if (InRange(--i, _materials))
        return _materials[i];
    return MaterialNull;
}
Bool MeshPart::sameMaterials(C MeshPart &part) C {
    if (material() == part.material() && _materials[0] == part._materials[0] && _materials[1] == part._materials[1] && _materials[2] == part._materials[2]) {
        ...
        for (Int i = shared_vars; i < part._variations.elms(); i++)
            if (part._variations[i].material != material())
                return false; // check extra variations in other part, if it's not default, then fail
        return true;
    }
    return false;
}
...
MeshPart &MeshPart::delBase() {
    base.del();
    return T;
}
MeshPart &MeshPart::delRender() {
    render.del();
    return T;
}
MeshPart &MeshPart::setBase(Bool only_if_empty) {
    if (only_if_empty ? !base.is() : true)
        base.create(render);
    return T;
}
...
MeshPart &MeshPart::setShader(Int lod_index) {
    if (!D._set_shader_material || multiMaterial(0) == D._set_shader_material || multiMaterial(1) == D._set_shader_material || multiMaterial(2) == D._set_shader_material || multiMaterial(3) == D._set_shader_material) // update only for changed material
        setShaderMulti(lod_index);
    REPA(_variations) {
        Variation &variation = _variations[i];
        if (!D._set_shader_material || variation.material == D._set_shader_material)
            setShader(lod_index, variation);
    }
    return T;
}
MeshPart &MeshPart::setShader(RENDER_MODE mode, Shader *shader, Int variation) {
    if (InRange(mode, MEMBER_ELMS(Variation, shader))) {
        ...
        if (!variation)
            var = &_variation;
        else if (InRange(--variation, _variations))
            var = &_variations[variation];
        else
            goto error;
...
MeshPart &MeshPart::material(C MaterialPtr &material, Int lod_index) {
    if (_variation.material != material || _materials[0] != null || _materials[1] != null || _materials[2] != null) {
        ...
        setUMM();
        if (lod_index >= 0)
            setShaderMulti(lod_index); // set shader only if valid lod specified (negative can be used for special case when we don't want to set shaders yet, for example because they may require locking D._lock)
    }
    return T;
}
MeshPart &MeshPart::multiMaterial(C MaterialPtr &m0, C MaterialPtr &m1, C MaterialPtr &m2, C MaterialPtr &m3, Int lod_index) {
    if (_variation.material != m0 || _materials[0] != m1 || _materials[1] != m2 || _materials[2] != m3) {
#if SUPPORT_MATERIAL_CHANGE_IN_RENDERING
        _variation.unlink();
#else
        if (_variation.drawn())
...
MeshPart &MeshPart::variations(Int variations) {
    ...
    Int old = _variations.elms();
    _variations.setNum(variations);
    for (Int i = old; i < _variations.elms(); i++)
        variation(i + 1, material()); // set default material for newly added variations
    return T;
}
...
C MaterialPtr &MeshPart::variationNull(Int variation) C {
    if (!variation)
        return material();
    if (InRange(--variation, _variations))
        return _variations[variation].material;
    return MaterialNull;
}
...
void MeshPart::variationKeep(Int variation) {
    if (InRange(--variation, _variations))
        Swap(_variation, _variations[variation]);
    _variations.del();
}
void MeshPart::variationMove(Int variation, Int new_index) {
    MemtN<Variation, 16> variations;
    variations.setNum(T.variations());
    Swap(variations[0], _variation);
    REPA(_variations)
    Swap(variations[i + 1], _variations[i]);
...
Byte MeshPart::heightmapType() C {
    if (heightmapFlat())
        return HEIGHTMAP_FLAT;
    if (heightmapSphere())
        return HEIGHTMAP_SPHERE;
    return HEIGHTMAP_NO;
}
MeshPart &MeshPart::heightmapFlat(Bool heightmap, Int lod_index) {
    if (T.heightmapFlat() != heightmap) {
        ...
        if (heightmap)
            exclude(VTX_TEX_ALL | VTX_TAN_BIN); // if enabled heightmap, then delete tex/tan/bin as they're no longer needed
        setShader(lod_index);
    }
    return T;
}
...
// TEXTURIZE
/******************************************************************************/
MeshPart &MeshPart::texMove(C Vec2 &move, Byte uv_index) {
    base.texMove(move, uv_index);
    render.texMove(move, uv_index);
    return T;
}
MeshPart &MeshPart::texScale(C Vec2 &scale, Byte uv_index) {
    base.texScale(scale, uv_index);
    render.texScale(scale, uv_index);
    return T;
}
MeshPart &MeshPart::texRotate(Flt angle, Byte uv_index) {
    base.texRotate(angle, uv_index);
    render.texRotate(angle, uv_index);
    return T;
}
/******************************************************************************/
// TRANSFORM
/******************************************************************************/
MeshPart &MeshPart::move(C Vec &move) {
    base.move(move);
    render.scaleMove(VecOne, move);
    return T;
}
MeshPart &MeshPart::scale(C Vec &scale) {
    base.scale(scale);
    render.scaleMove(scale);
    scaleParams(Abs(scale).max());
    return T;
}
MeshPart &MeshPart::scaleMove(C Vec &scale, C Vec &move) {
    base.scaleMove(scale, move);
    render.scaleMove(scale, move);
    scaleParams(Abs(scale).max());
    return T;
}
MeshPart &MeshPart::scaleMoveBase(C Vec &scale, C Vec &move) {
    base.scaleMove(scale, move);
    scaleParams(Abs(scale).max());
    return T;
}
...
void MeshPart::includeUsedBones(MemPtrN<Bool, 256> bones) C {
    if (base.is())
        base.includeUsedBones(bones);
    else
        render.includeUsedBones(bones);
}
void MeshPart::setUsedBones(MemPtrN<Bool, 256> bones) C {
    bones.clear();
    includeUsedBones(bones);
}
MeshPart &MeshPart::freeOpenGLESData() {
    render.freeOpenGLESData();
    return T;
}
/******************************************************************************/
...
MeshPart &MeshPart::delRandomLeafBending() {
    exclude(VTX_SIZE);
    return T;
}
/******************************************************************************/
static inline void WaitForStream(Bool &ok, C MaterialPtr &material) {
    if (material)
        ok &= material->waitForStream();
}
Bool MeshPart::waitForStream() C {
    Bool ok = true;
    WaitForStream(ok, _variation.material);
    REPA(_variations)
    WaitForStream(ok, _variations[i].material);
    REPA(_materials)
    WaitForStream(ok, _materials[i]);
    return ok;
}
/******************************************************************************/
MeshPart &MeshPart::operator+=(C MeshPart &src) {
    base += src.base;
    render += src.render;
    if (!(src.part_flag & MSHP_HIDDEN))
        FlagDisable(part_flag, MSHP_HIDDEN); // unhide part if 'src' is visible
    for (Int i = _variations.elms(); i < src._variations.elms(); i++)
        _variations.add(src._variations[i]); // add variations available in 'src' but not in 'this'
    return T;
}
/******************************************************************************/
void MeshPart::unlinkOpaque() C {
    _variation.unlinkOpaque();
    REPAO(_variations).unlinkOpaque();
}
void MeshPart::unlinkShadow() C {
    _variation.unlinkShadow();
    REPAO(_variations).unlinkShadow();
}
void MeshPart::unlink() C {
    _variation.unlink();
    REPAO(_variations).unlink();
}
/******************************************************************************/
