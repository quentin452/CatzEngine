...
struct QuadricMatrix {
    ...
    union {
        ...
        struct {
            Real a, d, e, g, b, f, h, c, i, j;
        }; // list in this order to match the above formula and 'error2' method
    };
    ...
    {
        Real det = m[a11] * m[a22] * m[a33] + m[a13] * m[a21] * m[a32] + m[a12] * m[a23] * m[a31] - m[a13] * m[a22] * m[a31] - m[a11] * m[a23] * m[a32] - m[a12] * m[a21] * m[a33];
        return det;
    }
    ...
    struct VtxData {
        ...
        void from(C MeshBase &mshb, Int i) {
            if (InRange(i, mshb.vtx)) {
                ...
                if (mshb.vtx.tex0())
                    tex0 = mshb.vtx.tex0(i);
                else
                    tex0.zero();
                if (mshb.vtx.tex1())
                    tex1 = mshb.vtx.tex1(i);
                else
                    tex1.zero();
                if (mshb.vtx.tex2())
                    tex2 = mshb.vtx.tex2(i);
                else
                    tex2.zero();
                if (mshb.vtx.tex3())
                    tex3 = mshb.vtx.tex3(i);
                else
                    tex3.zero();
                if (mshb.vtx.color())
                    color = mshb.vtx.color(i);
                else
                    color = 1;
                if (mshb.vtx.material())
                    material = mshb.vtx.material(i) / 255.0f;
                else
                    material.zero();
                if (mshb.vtx.matrix())
                    matrix = mshb.vtx.matrix(i);
                else
                    matrix.zero();
                if (mshb.vtx.blend())
                    blend = mshb.vtx.blend(i);
                else
                    blend.set(255, 0, 0, 0); // !! 'blend.sum' must be exactly equal to 255 !!
                if (mshb.vtx.size())
                    size = mshb.vtx.size(i);
                else
                    size = 0;
            } else
                zero();
        ...
        void to(MeshBase &mshb, Int i) C {
            if (InRange(i, mshb.vtx)) {
                ...
                if (mshb.vtx.hlp())
                    mshb.vtx.hlp(i) = hlp;
                if (mshb.vtx.nrm()) {
                    Vec &nrm = mshb.vtx.nrm(i);
                    nrm = T.nrm;
                    if (nrm.allZero())
                        nrm.set(DEFAULT_VTX_NRM);
                ...
                if (mshb.vtx.tex0())
                    mshb.vtx.tex0(i) = tex0;
                if (mshb.vtx.tex1())
                    mshb.vtx.tex1(i) = tex1;
                if (mshb.vtx.tex2())
                    mshb.vtx.tex2(i) = tex2;
                if (mshb.vtx.tex3())
                    mshb.vtx.tex3(i) = tex3;
                if (mshb.vtx.color())
                    mshb.vtx.color(i) = color;
                if (mshb.vtx.matrix())
                    mshb.vtx.matrix(i) = matrix;
                if (mshb.vtx.blend())
                    mshb.vtx.blend(i) = blend;
                if (mshb.vtx.size())
                    mshb.vtx.size(i) = size;
                if (mshb.vtx.dup())
                    mshb.vtx.dup(i) = i;
                if (mshb.vtx.material()) {
                    VecB4 &m = mshb.vtx.material(i);
                    if (Flt sum = material.sum()) {
                        Color mc = material / sum;
                        m.set(mc.r, mc.g, mc.b, mc.a);
                    } else
                        m.set(255, 0, 0, 0); // !! 'material.sum' must be exactly equal to 255 !!
                }
            }
        }
        void lerp(C VtxData &a, C VtxData &b, Flt step, MESH_FLAG flag) {
            ...
            if (flag & VTX_COLOR)
                color = a.color * step1 + b.color * step;
            if (flag & VTX_MATERIAL)
                material = a.material * step1 + b.material * step;
            if (flag & VTX_NRM)
                nrm = !(a.nrm * step1 + b.nrm * step);
            if (flag & VTX_SKIN) {
                MemtN<IndexWeight, 256> skin;
                FREPA(a.matrix)
                skin.New().set(a.matrix.c[i], a.blend.c[i] * step1);
                FREPA(b.matrix)
                skin.New().set(b.matrix.c[i], b.blend.c[i] * step);
        ...
        void lerp(C VtxData &a, C VtxData &b, C VtxData &c, C Vec &blend, MESH_FLAG flag) {
            if (flag & VTX_TEX0)
                tex0 = a.tex0 * blend.x + b.tex0 * blend.y + c.tex0 * blend.z;
            if (flag & VTX_COLOR)
                color = a.color * blend.x + b.color * blend.y + c.color * blend.z;
            if (flag & VTX_MATERIAL)
                material = a.material * blend.x + b.material * blend.y + c.material * blend.z;
            if (flag & VTX_NRM)
                nrm = !(a.nrm * blend.x + b.nrm * blend.y + c.nrm * blend.z);
            if (flag & VTX_SKIN) {
                MemtN<IndexWeight, 256> skin;
                FREPA(a.matrix)
                skin.New().set(a.matrix.c[i], a.blend.c[i] * blend.x);
                FREPA(b.matrix)
                skin.New().set(b.matrix.c[i], b.blend.c[i] * blend.y);
                FREPA(c.matrix)
                skin.New().set(c.matrix.c[i], c.blend.c[i] * blend.z);
                SetSkin(skin, T.matrix, T.blend, null);
            }
            if (flag & (VTX_HLP | VTX_TEX1 | VTX_TEX2 | VTX_TEX3 | VTX_SIZE)) // uncommon
            {
                if (flag & VTX_HLP)
                    hlp = a.hlp * blend.x + b.hlp * blend.y + c.hlp * blend.z;
                if (flag & VTX_SIZE)
                    size = a.size * blend.x + b.size * blend.y + c.size * blend.z;
                if (flag & VTX_TEX1)
                    tex1 = a.tex1 * blend.x + b.tex1 * blend.y + c.tex1 * blend.z;
                if (flag & VTX_TEX2)
                    tex2 = a.tex2 * blend.x + b.tex2 * blend.y + c.tex2 * blend.z;
                if (flag & VTX_TEX3)
                    tex3 = a.tex3 * blend.x + b.tex3 * blend.y + c.tex3 * blend.z;
            }
        }
...
#define EPS_SIMPLIFY_PLANE 0.001f
...
#if ALLOW_PLANES
            if (vtx.planes.elms() > planes.elms())
                Swap(vtx.planes, planes); // if 'vtx' uses more planes, then reuse its memory container, to avoid memory allocations
            REPA(vtx.planes)
            includePlane(vtx.planes[i]);
    ...
    static Flt PlanesError(C Vertex &vtx0, C Vertex &vtx1, C Vec &pos) // this is linear
    {
        Flt error = 0;
        REPA(vtx0.planes)
        MAX(error, vtx0.planes[i].error(pos));
        REPA(vtx1.planes)
        MAX(error, vtx1.planes[i].error(pos));
        return error;
    }
#endif
    Real calculateError(Int v0, Int v1, Vec &mid_pos) // TODO: make error calculation optional?
    {
        ...
        {
            if (vtx0.border & BORDER_POS) {
                mid_pos = vtx0.pos;
                goto calc_error;
            }
            if (vtx1.border & BORDER_POS) {
                mid_pos = vtx1.pos;
                goto calc_error;
            }
        }
        ...
        calc_error:
            ...
            if (mode == SIMPLIFY_PLANES)
                error = PlanesError(vtx0, vtx1, mid_pos);
            else
#endif
    ...
    Bool flipped(C Vec &mid_pos, Int edge_vtx1i, C Vertex &edge_vtx0, Bool add) {
        ...
        REP(edge_vtx0.tri_num) {
...
#endif
            {
                ...
                {
                    if (add)
                        middle_tris.add(&all_tris.New().set(tri)); // we're adding middle tris only for the first vertex, to avoid adding middle triangles twice, from both vertexes
                } else {
                    ...
                    if (new_nrm.length2() <= Sqr(EPSD)         // if new triangle surface is tiny (perhaps all 3 points are now on 1 line) then we won't be able to calculate triangle normal properly and it can cause issues with collision detection (raycast). Use EPSD instead of EPS because for 1cm x 1cm tri, normal length=0.0001 which is very close to EPS but we still need to process smaller tris.
                        || Dot(new_nrm, tri.nrm) < 0)
                        return true; // if new triangle normal is in the opposite direction of the original normal
    ...
    static inline void AddSkin(MemtN<IndexWeight, 256> &skin, C VtxBone &matrix, C VecB4 &blend, Flt weight) {
        REPA(matrix) {
            Flt w = blend.c[i] * weight;
            if (w > 0)
                skin.New().set(matrix.c[i], w);
        }
    }
    ...
    static Int SkinDifference(C VtxBone &matrix_a, C VecB4 &blend_a, C VtxBone &matrix_b, C VecB4 &blend_b) {
        Int dif = 0;
        REP(4) {
            if (Byte b = blend_a.c[i])
                MAX(dif, Abs(b - GetBlend(matrix_a.c[i], matrix_b, blend_b))); // get difference between i-th A bone
            if (Byte b = blend_b.c[i])
                MAX(dif, Abs(b - GetBlend(matrix_b.c[i], matrix_a, blend_a))); // get difference between i-th B bone
        }
        return dif;
    }
    void addVtxData(TrianglePtr &trip, Int v0, Int v1) {
        ...
        // start with self
        Weights weight(tri.weight);
        MemtN<IndexWeight, 256> skin;
        tri.vtx_new = trip.vtx_mid; // 'vtx_mid' is already multiplied by 'tri.weight'
        if (tri.flag & VTX_SKIN)
            AddSkin(skin, trip.vtx_mid.matrix, trip.vtx_mid.blend, tri.weight);

        // check all tris
        REPA(all_tris) // iterate all tris
        {
            TrianglePtr &trip2 = all_tris[i];
            if (&trip != &trip2) // skip self, remember that 'tri2' can be middle too, just like 'tri'
            {
                ...
                if (tri.mtrl_group != tri2.mtrl_group)
                    FlagDisable(add_flag, VTX_MATERIAL); // we can merge material only if materials match
                    ...
                    if (tv == v0 || tv == v1) // test only edge vertexes, this is to ignore other shared vertexes between the tris which are not on the edge to be removed, this will allow to simplify the mesh more
                        REPAD(tvi2, tri2.ind) {
                            ...
                            if (tv == tv2) // if the same vtx
                            {
                                ...
                                if ((add_flag & VTX_NRM) && !Equal(v.nrm, v2.nrm))
                                    FlagDisable(add_flag, VTX_NRM);
                                if ((add_flag & VTX_TEX0) && !Equal(v.tex0, v2.tex0))
                                    FlagDisable(add_flag, VTX_TEX0);
                                if ((add_flag & VTX_COLOR) && !Equal(v.color, v2.color))
                                    FlagDisable(add_flag, VTX_COLOR);
                                if ((add_flag & VTX_MATERIAL) && !Equal(v.material, v2.material))
                                    FlagDisable(add_flag, VTX_MATERIAL);
                                if ((add_flag & VTX_SKIN) && !EqualSkin(v.matrix, v.blend, v2.matrix, v2.blend))
                                    FlagDisable(add_flag, VTX_SKIN);
                                if (add_flag & (VTX_HLP | VTX_TEX1 | VTX_TEX2 | VTX_TEX3 | VTX_SIZE)) // uncommon
                                {
                                    if ((add_flag & VTX_HLP) && !Equal(v.hlp, v2.hlp))
                                        FlagDisable(add_flag, VTX_HLP);
                                    if ((add_flag & VTX_TEX1) && !Equal(v.tex1, v2.tex1))
                                        FlagDisable(add_flag, VTX_TEX1);
                                    if ((add_flag & VTX_TEX2) && !Equal(v.tex2, v2.tex2))
                                        FlagDisable(add_flag, VTX_TEX2);
                                    if ((add_flag & VTX_TEX3) && !Equal(v.tex3, v2.tex3))
                                        FlagDisable(add_flag, VTX_TEX3);
                                    if ((add_flag & VTX_SIZE) && !Equal(v.size, v2.size))
                                        FlagDisable(add_flag, VTX_SIZE);
                                }
                            }
                        }
                }
                // merge
                if (add_flag & VTX_NRM) {
                    tri.vtx_new.nrm += trip2.vtx_mid.nrm;
                    trip2.vtx_nrm = &tri.vtx_new;
                }
                if (add_flag & VTX_TEX0) {
                    tri.vtx_new.tex0 += trip2.vtx_mid.tex0;
                    trip2.vtx_tex0 = &tri.vtx_new;
                    weight.tex0 += tri2.weight;
                }
                if (add_flag & VTX_COLOR) {
                    tri.vtx_new.color += trip2.vtx_mid.color;
                    trip2.vtx_color = &tri.vtx_new;
                    weight.color += tri2.weight;
                }
                if (add_flag & VTX_MATERIAL) {
                    tri.vtx_new.material += trip2.vtx_mid.material;
                    trip2.vtx_material = &tri.vtx_new;
                    weight.material += tri2.weight;
                }
                if (add_flag & VTX_SKIN) {
                    AddSkin(skin, trip2.vtx_mid.matrix, trip2.vtx_mid.blend, tri2.weight);
                    trip2.vtx_skin = &tri.vtx_new;
                }
                if (add_flag & (VTX_HLP | VTX_TEX1 | VTX_TEX2 | VTX_TEX3 | VTX_SIZE)) // uncommon
                {
                    if (add_flag & VTX_HLP) {
                        tri.vtx_new.hlp += trip2.vtx_mid.hlp;
                        trip2.vtx_hlp = &tri.vtx_new;
                        weight.hlp += tri2.weight;
                    }
                    if (add_flag & VTX_TEX1) {
                        tri.vtx_new.tex1 += trip2.vtx_mid.tex1;
                        trip2.vtx_tex1 = &tri.vtx_new;
                        weight.tex1 += tri2.weight;
                    }
                    if (add_flag & VTX_TEX2) {
                        tri.vtx_new.tex2 += trip2.vtx_mid.tex2;
                        trip2.vtx_tex2 = &tri.vtx_new;
                        weight.tex2 += tri2.weight;
                    }
                    if (add_flag & VTX_TEX3) {
                        tri.vtx_new.tex3 += trip2.vtx_mid.tex3;
                        trip2.vtx_tex3 = &tri.vtx_new;
                        weight.tex3 += tri2.weight;
                    }
                    if (add_flag & VTX_SIZE) {
                        tri.vtx_new.size += trip2.vtx_mid.size;
                        trip2.vtx_size = &tri.vtx_new;
                        weight.size += tri2.weight;
                    }
                }
            }
        }
    ...
    Bool setVtxData(TrianglePtr &trip, Int v0, Int v1) {
        ...
        Triangle &tri = *trip.tri;
        if (tri.middle)
            return true;               // middle tris have already been calculated
        MESH_FLAG set_flag = tri.flag; // get what we need to calculate
        if ((set_flag & VTX_TEX0) && !trip.vtx_tex0)
            return false; // if this triangle has TEX0 but we haven't found any middle triangle match, then we can't proceed
        ...
        if (trip.vtx_hlp) {
            tri.vtx_new.hlp = trip.vtx_hlp->hlp;
            FlagDisable(set_flag, VTX_HLP);
        }
        if (trip.vtx_nrm) {
            tri.vtx_new.nrm = trip.vtx_nrm->nrm;
            FlagDisable(set_flag, VTX_NRM);
        }
        if (trip.vtx_tex0) {
            tri.vtx_new.tex0 = trip.vtx_tex0->tex0;
            FlagDisable(set_flag, VTX_TEX0);
        }
        if (trip.vtx_tex1) {
            tri.vtx_new.tex1 = trip.vtx_tex1->tex1;
            FlagDisable(set_flag, VTX_TEX1);
        }
        if (trip.vtx_tex2) {
            tri.vtx_new.tex2 = trip.vtx_tex2->tex2;
            FlagDisable(set_flag, VTX_TEX2);
        }
        if (trip.vtx_tex3) {
            tri.vtx_new.tex3 = trip.vtx_tex3->tex3;
            FlagDisable(set_flag, VTX_TEX3);
        }
        if (trip.vtx_skin) {
            tri.vtx_new.matrix = trip.vtx_skin->matrix;
            tri.vtx_new.blend = trip.vtx_skin->blend;
            FlagDisable(set_flag, VTX_SKIN);
        }
        if (trip.vtx_color) {
            tri.vtx_new.color = trip.vtx_color->color;
            FlagDisable(set_flag, VTX_COLOR);
        }
        if (trip.vtx_material) {
            tri.vtx_new.material = trip.vtx_material->material;
            FlagDisable(set_flag, VTX_MATERIAL);
        }
        if (trip.vtx_size) {
            tri.vtx_new.size = trip.vtx_size->size;
            FlagDisable(set_flag, VTX_SIZE);
        }
        if (set_flag & (VTX_HLP | VTX_NRM | VTX_TEX_ALL | VTX_SKIN | VTX_COLOR | VTX_MATERIAL | VTX_SIZE)) // if we still have something to process
        {
            ...
            // start with self
            if (set_flag & VTX_HLP) {
                tri.vtx_new.hlp = trip.vtx_mid.hlp;
                weight.hlp = tri.weight;
            }
            if (set_flag & VTX_NRM) {
                tri.vtx_new.nrm = trip.vtx_mid.nrm;
            }
            if (set_flag & VTX_TEX0) {
                tri.vtx_new.tex0 = trip.vtx_mid.tex0;
                weight.tex0 = tri.weight;
            }
            if (set_flag & VTX_TEX1) {
                tri.vtx_new.tex1 = trip.vtx_mid.tex1;
                weight.tex1 = tri.weight;
            }
            if (set_flag & VTX_TEX2) {
                tri.vtx_new.tex2 = trip.vtx_mid.tex2;
                weight.tex2 = tri.weight;
            }
            if (set_flag & VTX_TEX3) {
                tri.vtx_new.tex3 = trip.vtx_mid.tex3;
                weight.tex3 = tri.weight;
            }
            if (set_flag & VTX_COLOR) {
                tri.vtx_new.color = trip.vtx_mid.color;
                weight.color = tri.weight;
            }
            if (set_flag & VTX_MATERIAL) {
                tri.vtx_new.material = trip.vtx_mid.material;
                weight.material = tri.weight;
            }
            if (set_flag & VTX_SIZE) {
                tri.vtx_new.size = trip.vtx_mid.size;
                weight.size = tri.weight;
            }
            if (set_flag & VTX_SKIN)
                AddSkin(skin, trip.vtx_mid.matrix, trip.vtx_mid.blend, tri.weight);
            REPAD(tvi, tri.ind) {
                const Int tv = tri.ind.c[tvi];
                if (tv == v0 || tv == v1) // test only edge vertexes, this is to ignore other shared vertexes between the tris which are not on the edge to be removed, this will allow to simplify the mesh more
                {
                    ...
                    REPA(all_tris) {
                        C TrianglePtr &trip2 = all_tris[i];
                        if (&trip != &trip2) {
                            C Triangle &tri2 = *trip2.tri;
                            REPAD(tvi2, tri2.ind) {
                                const Int tv2 = tri2.ind.c[tvi2];
                                if (tv == tv2) // if the same vtx
                                {
                                    ...
                                    if ((add_flag & VTX_MATERIAL) && tri.mtrl_group == tri2.mtrl_group && Equal(v.material, v2.material)) {
                                        tri.vtx_new.material += trip2.vtx_mid.material;
                                        weight.material += tri2.weight;
                                    }
                                    if ((add_flag & VTX_SKIN) && EqualSkin(v.matrix, v.blend, v2.matrix, v2.blend))
                ...
                if (!tri.middle) // we don't need to check middle triangles
                {
                    ...
                    if (TEST_ORIGINAL_POS) // calculate original position in the new triangle
                    {
                        ...
                        if (test_flag & VTX_TEX0) {
                            ...
                            if (Dist2(got_tex, vtx0.tex0) > max_uv2)
                                return false;
                        }
                        ...
                        if (test_flag & VTX_SKIN) {
                            ...
                            VtxBone bone_index;
                            VecB4 bone_weight;
                            SetSkin(got_skin, bone_index, bone_weight, null);
                            if (SkinDifference(bone_index, bone_weight, vtx0.matrix, vtx0.blend) > max_skin)
                                return false;
                        }
                    } else // calculate new position in the old triangle
                    {
                        ...
                        if (test_flag & VTX_SKIN) {
                            ...
                            VtxBone bone_index;
                            VecB4 bone_weight;
                            SetSkin(expected_skin, bone_index, bone_weight, null);
                            if (SkinDifference(bone_index, bone_weight, vtx_n.matrix, vtx_n.blend) > max_skin)
                                return false;
                        }
                    }
                }
    ...
    void adjustRefs(Triangle &tri, Int v0, Int v1) // this removes the 'tri' reference from its vertexes
    {
        ...
        {
            Int v = tri.ind.c[i];
            if (v != v0 && v != v1) // we only need to process the 3rd vtx, because 'v0' will be adjusted, and 'v1' will be removed
            {
                Vertex &vtx = vtxs[v];
                REP(vtx.tri_num) // iterate all triangles of this vtx
                {
                    Ref &ref = refs[vtx.ref_start + i];
                    if (ref.tri_index == tri_index) // if the tri is this one
                    {
    ...
    void resetError(Triangle &tri) {
        ...
        if (error_min != tri.error_min) {
            ...
            if (Triangle *next = tree.last())
                for (; Triangle *prev = (Triangle *)next->prev(); next = prev)
    ...
    struct VtxConnection {
        ...
        void init(Int other_vtx, C Triangle &tri, Byte side) {
            T.other_vtx = other_vtx;
            T.tri = &tri;
            T.mirror = false;
            T.side[tri.visible] = side;
            T.side[!tri.visible] = 0;
            // T.tris=1;
        }
    };
    void addEdge(Memt<VtxConnection> &edges, Int vtx_index, C Triangle &tri, Byte side) {
        ...
        {
            VtxConnection &edge = edges[i];
            if (edge.other_vtx == vtx_index) // found
            {
    ...
    struct TriLink {
        ...
        void set(C Triangle &tri, Int v1, Int v2) {
            T.tri = &tri;
            p.set(v1, v2);
        }
    };
    void processVtxs() // detect which vertexes are on the border
    {
        ...
        REPAD(vi, vtxs) {
            ...
            REPA(edges) {
                ...
                if (edge.border())
                    vtx.border |= BORDER_POS; // if edge is referenced by only 1 tri
                if (edge.mirror)
                    vtx.border |= BORDER_TEX;
            ...
            if (VTX_NORMAL_QM > 0) {
                ...
                again:
                    ...
                    if (chs)
                        ...
                        tris_n += tri_nw; // add to gathered normal
                    TriLink next;
                    next.tri = null;
                    // remove all the same and reversed tris, and find next one
                    REPA(tris_left) {
                        VecI2 p = tris_left[i].p;
                        if (tri_link.p == p || (tri_link.p.x == p.y && tri_link.p.y == p.x))
                            tris_left.remove(i);
                        else if (tri_link.p.y == p.x)
                            next = tris_left[i]; // if this wasn't removed, and it's the next triangle, then remember it
                    }
                    if (next.tri) {
                        tri_link = next;
                        tri = tri_link.tri;
    ...
    void add(C MeshBase &mesh, Int part, C MeshPart *mesh_part) {
        ...
        if (visible)
            visible_tris += mesh.trisTotal();

        // set material group
        MtrlGroup mtrl_group(mesh_flag, mesh_part);
        Int mtrl_group_i = -1;
        REPA(mtrl_groups)
        if (mtrl_groups[i].testAndMerge(mtrl_group)) {
            mtrl_group_i = i;
            break;
        } // find existing one
        if (mtrl_group_i < 0) {
            MtrlGroup &mg = mtrl_groups[mtrl_group_i = mtrl_groups.addNum(1)];
            Swap(mg, mtrl_group);
        } // create new one
        ...
        FREPA(mesh.vtx) // add in order
        {
            ...
            if (mesh.vtx.tex0())
                vtx_dup.tex0 = mesh.vtx.tex0(i);
            else
                vtx_dup.tex0.zero(); // if not available then zero so BORDER_TEX can be detected properly
            ...
            if (tri.exists)
                REPAD(v, tri.ind) {
                    ...
                    REP(vtx.tri_num) {
                        ...
                        if (!vtx_tri.exists)
                            Exit("vtx tri doesn't exist");
                        if (&tri == &vtx_tri)
                            has = true;
                    }
                    if (!has)
                        Exit("vtx doesn't have tri");
                }
        }
    }
    void init(Flt pos_eps) {
        ...
        MESH_FLAG mtrl_groups_flag = MESH_NONE;
        REPA(mtrl_groups)
        mtrl_groups_flag |= mtrl_groups[i].flag; // what vtx components are we processing
        REPA(tris)                               // set actual triangle flag after adding all meshes, because they may modify the mtrl group flag
        {
            Triangle &tri = tris[i];
            tri.flag = mtrl_groups[tri.mtrl_group].flag;
        }
        mtrl_groups.del(); // release to free memory as it's no longer needed

        // create unique vertexes
        vtxs.setNum(SetVtxDup(SCAST(Memc<VtxDup>, vtx_dups), box, pos_eps));
        Int vs = 0;
        REPA(vtx_dups) {
            VtxDupMap &vd = vtx_dups[i];
            if (vd.dup == i) {
        ...
        REPA(tris) {
            ...
            REPA(tri.ind) {
                Int &tv = tri.ind.c[i];
                tv = vtx_dups[vtx_dups[tv].dup].vtxs_index; // remap triangle vertex index to unique !! WARNING: because of this remap, 'tri.ind' sometimes won't be 'allDifferent' !!
                Vertex &vtx = vtxs[tv];
                vtx.tri_num++;
            }
            tri.tan.zero();
            tri.bin.zero();
            setNrmTanBin(tri); // zero in init, in case 'setNrmTanBin' doesn't modify the values
...
#if ALLOW_PLANES
            Vertex::Plane plane;
            plane.normal = tri.nrm;
            plane.plane = plane_dist;
#endif
            REP(3) {
        ...
        Int ref_start = 0;
        REPA(vtxs) // prepare for a single continuous array of unique triangle-vertex pairs
        {
            Vertex &vtx = vtxs[i];
            vtx.ref_start = ref_start;
            ref_start += vtx.tri_num;
            vtx.tri_num = 0;
        }
        ...
        REPAD(t, tris) {
            ...
            REPAD(v, tri.ind) {
                Vec mid_pos;
                tri.edge_error[v] = calculateError(tri.ind.c[v], tri.ind.c[(v + 1) % 3], mid_pos);
            }
    ...
    void simplify(Flt intensity, Flt max_distance, Flt max_uv, Flt max_color, Flt max_material, Flt max_skin, Flt max_normal, Bool keep_border, MESH_SIMPLIFY mode, Flt pos_eps) {
        ...
        T.test_flag = ((T.max_uv2 < 1 - EPS) ? VTX_TEX_ALL : MESH_NONE) | ((T.max_color < 1 - EPS) ? VTX_COLOR : MESH_NONE) | ((T.max_material < 1 - EPS) ? VTX_MATERIAL : MESH_NONE) | ((T.max_skin < 255) ? VTX_SKIN : MESH_NONE) | ((T.max_normal > -1 + EPS) ? VTX_NRM : MESH_NONE);
        ...
        init(pos_eps);
        if (visible_tris <= target_tris)
            return; // must be checked after 'init'

        // main iteration loop
        Vec mid_pos;
        for (; tree.last();) {
            Triangle &tri = *tree.last();
            if (tri.error_min > max_error || stop())
                break; // tris are sorted by their error, so if we've reached the one above the limit, then stop

            Int i = MinI(tri.edge_error[0], tri.edge_error[1], tri.edge_error[2]);
            Int edge_vtx0i = tri.ind.c[i];
            Vertex &edge_vtx0 = vtxs[edge_vtx0i];
            Int edge_vtx1i = tri.ind.c[(i + 1) % 3];
            Vertex &edge_vtx1 = vtxs[edge_vtx1i];

            // border check
            if (keep_border && (edge_vtx0.border & edge_vtx1.border & BORDER_POS))
                goto cant; // if both are on the border, then we can't move, if only one is on border, then we can move but only to the border
            ...
            middle_tris.clear();
            all_tris.clear();                                 // clear before 'flipped' because that modifies this !! WARNING: this might list the same triangle multiple times !!
            if (flipped(mid_pos, edge_vtx1i, edge_vtx0, true) // middle tris will be collected only from the first vertex
                || flipped(mid_pos, edge_vtx0i, edge_vtx1, false))
                goto cant;
            ...
            // remove edge
            {
#if ADJUST_REFS
                REPA(middle_tris)
                adjustRefs(*middle_tris[i]->tri, edge_vtx0i, edge_vtx1i);
#endif

                REPA(middle_tris) {
                    Triangle &tri = *middle_tris[i]->tri;
                    if (tri.exists) // check if exists because this tri could be listed multiple times, and thus could be removed multiple times too
                    {
                        tri.exists = false;
                ...
                if (tri_num <= edge_vtx0.tri_num)      // if the new number fits in the previous range, then just replace it, to reduce memory usage
                {
                    ...
                    edge_vtx0.ref_start = ref_start; // set start to what we've added
...
#pragma message("!! Warning: This will slow down mesh simplification, use only for testing !!")
    ...
    void store(MeshBase &mesh, MESH_FLAG flag_and = MESH_ALL) {
        ...
        if (flags & VTX_TAN_BIN)
            mesh.setTanBin(); // if(flags&VTX_TAN)mesh.setTangents(); if(flags&VTX_BIN)mesh.setBinormals();
    ...
    void store(MemPtr<MeshPart> parts) {
        ...
        REPA(parts) // go from the end so we can remove if needed
        {
            ...
            if (flag & VTX_TAN_BIN)
                mesh.setTanBin(); // if(flag&VTX_TAN)mesh.setTangents(); if(flag&VTX_BIN)mesh.setBinormals();

            mesh.weldVtx(VTX_ALL, EPSD, EPS_COL_COS, -1); // use small pos epsilon in case mesh is scaled down, ignore degenerate faces
            if (!mesh.is())
                parts.remove(i, true); // if became empty, then just remove it
        }
    }
};
/******************************************************************************/
