...
MeshBase &MeshBase::addVtx(C Vec &pos) {
    ...
    if (vtx._nrm)
        ReallocZero1(vtx._nrm, vtxs());
    if (vtx._tan)
        ReallocZero1(vtx._tan, vtxs());
    if (vtx._bin)
        ReallocZero1(vtx._bin, vtxs());
    if (vtx._hlp)
        ReallocZero1(vtx._hlp, vtxs());
    if (vtx._tex0)
        ReallocZero1(vtx._tex0, vtxs());
    if (vtx._tex1)
        ReallocZero1(vtx._tex1, vtxs());
    if (vtx._tex2)
        ReallocZero1(vtx._tex2, vtxs());
    if (vtx._tex3)
        ReallocZero1(vtx._tex3, vtxs());
    if (vtx._matrix)
        ReallocZero1(vtx._matrix, vtxs());
    if (vtx._blend)
        ReallocZero1(vtx._blend, vtxs());
    if (vtx._size)
        ReallocZero1(vtx._size, vtxs());
    if (vtx._material)
        ReallocZero1(vtx._material, vtxs());
    if (vtx._color)
        ReallocZero1(vtx._color, vtxs());
    if (vtx._flag)
        ReallocZero1(vtx._flag, vtxs());
...
MeshBase &MeshBase::addEdge(C VecI2 &ind) {
    ...
    if (edge._nrm)
        ReallocZero1(edge._nrm, edges());
    if (edge._flag)
        ReallocZero1(edge._flag, edges());
    if (edge._id)
        ReallocZero1(edge._id, edges());
...
MeshBase &MeshBase::addTri(C VecI &ind) {
    ...
    if (tri._nrm)
        ReallocZero1(tri._nrm, tris());
    if (tri._flag)
        ReallocZero1(tri._flag, tris());
    if (tri._id)
        ReallocZero1(tri._id, tris());
...
MeshBase &MeshBase::addQuad(C VecI4 &ind) {
    ...
    if (quad._nrm)
        ReallocZero1(quad._nrm, quads());
    if (quad._flag)
        ReallocZero1(quad._flag, quads());
    if (quad._id)
        ReallocZero1(quad._id, quads());
...
MeshBase &MeshBase::add(C MeshRender &src, MESH_FLAG flag_and) {
    MeshBase mb(src, flag_and);
    return add(mb);
}
MeshBase &MeshBase::add(C MeshPart &src, MESH_FLAG flag_and) {
    MeshBase mb(src, flag_and);
    return add(mb);
}
/******************************************************************************
...
/******************************************************************************/
MeshBase &MeshBase::removeVtx(Int v) {
    if (InRange(v, vtx)) {
        Memt<Bool> is;
        is.setNum(vtxs());
        FREPA(vtx)
...
MeshBase &MeshBase::removeEdge(Int e) {
    if (InRange(e, edge)) {
        Memt<Bool> is;
        is.setNum(edges());
        FREPA(edge)
...
MeshBase &MeshBase::removeTri(Int t) {
    if (InRange(t, tri)) {
        Memt<Bool> is;
        is.setNum(tris());
        FREPA(tri)
...
MeshBase &MeshBase::removeQuad(Int q) {
    if (InRange(q, quad)) {
        Memt<Bool> is;
        is.setNum(quads());
        FREPA(quad)
...
MeshBase &MeshBase::keepVtxs(C CMemPtr<Bool> &is) {
    ...
    // edge
    face_is.setNum(edges());
    FREPA(edge) {
        p = edge.ind(i).c;
        face_is[i] = (ElmIs(is, p[0]) && ElmIs(is, p[1]));
    }
    temp.edge._elms = CountIs(face_is);
    temp.include(flag() & EDGE_ALL);
    ...
    FREPA(tri) {
        p = tri.ind(i).c;
        face_is[i] = (ElmIs(is, p[0]) && ElmIs(is, p[1]) && ElmIs(is, p[2]));
    }
    temp.tri._elms = CountIs(face_is);
    temp.include(flag() & TRI_ALL);
    temp.copyTris(T, face_is);
    IndRemap(vtx_remap, temp.tri.ind(), temp.tris());

    // quad
    face_is.setNum(quads());
    FREPA(quad) {
        p = quad.ind(i).c;
        face_is[i] = (ElmIs(is, p[0]) && ElmIs(is, p[1]) && ElmIs(is, p[2]) && ElmIs(is, p[3]));
    }
    temp.quad._elms = CountIs(face_is);
    temp.include(flag() & QUAD_ALL);
...
MeshBase &MeshBase::keepEdges(C CMemPtr<Bool> &is) {
    ...
    if (edges != T.edges()) {
        exclude(ADJ_ALL);
        MeshBase temp(0, edges, 0, 0, flag());
        temp.copyEdges(T, is);
        Swap(edge, temp.edge);
    }
    return T;
...
MeshBase &MeshBase::keepQuads(C CMemPtr<Bool> &is) {
    ...
    if (quads != T.quads()) {
        exclude(ADJ_ALL);
        MeshBase temp(0, 0, 0, quads, flag());
        temp.copyQuads(T, is);
        Swap(quad, temp.quad);
    }
    return T;
...
MeshBase &MeshBase::removeEdges(C CMemPtr<Int> &edges, Bool remove_unused_vtxs) {
    if (edges.elms()) {
        Memt<Bool> nis;
        CreateIsNot(nis, edges, T.edges());
        keepEdges(nis);
        if (remove_unused_vtxs)
            removeUnusedVtxs();
    }
    return T;
}
...
MeshBase &MeshBase::removeFaces(C CMemPtr<Int> &faces, Bool remove_unused_vtxs) {
    if (faces.elms()) {
        ...
        CreateFaceIsNot(tri_is, quad_is, faces, tris(), quads());
        keepTris(tri_is);
        keepQuads(quad_is);
        if (remove_unused_vtxs)
            removeUnusedVtxs();
...
static Bool Continuous(C Int *a, Int a_elms, C Int *b, Int b_elms) {
    REP(a_elms) {
        ...
        REP(b_elms) {
            if (y == b[i] && x == b[(i + 1) % b_elms])
                return true; // we need to check reversed order, because they are continuous if one face has the same vtxs but in reversed order
        }
    }
    return false;
}
...
MeshBase &MeshBase::removeDoubleSideFaces(Bool remove_unused_vtxs) {
    if (tri.elms() && !tri.adjFace() || quad.elms() && !quad.adjFace())
        setAdjacencies(true, false);

    Memt<Byte> face_flag;
    face_flag.setNumZero(faces());
    {
        Index face_face;
        linkFaceFace(face_face);
        Memt<Int> face_list;
        ...
        check:
            ...
            if (!(ff & (RDSF_REMOVE | RDSF_CHECKED))) // if this face was not yet removed or checked
            {
                ...
                C Int *adjacent;
                if (InRange(f, tri)) {
                    VecI &tri_vtx = face_vtx.xyz;
                    tri_vtx = tri.ind(f);
                    tri_vtx.remap(vtx.dup());
                    adjacent = tri.adjFace(f).c;
                    face_vtxs = 3;
                } else {
                    Int quad_index = f - tris();
                    face_vtx = quad.ind(quad_index);
                    face_vtx.remap(vtx.dup());
                    adjacent = quad.adjFace(quad_index).c;
                    face_vtxs = 4;
                }

                // check overlapping faces
                C IndexGroup &ig = face_face.group[f];
                REPA(ig) // iterate all faces connected to this face
                {
                    VecI4 test_face_vtx;
                    Int test_face_vtxs;
                    ...
                    {
                        test_face ^= SIGN_BIT;
                        test_face2 = tris() + test_face;
                        test_face_vtx = quad.ind(test_face);
                        test_face_vtx.remap(vtx.dup());
                        test_face_vtxs = 4;
                    } else {
                        test_face2 = test_face;
                        VecI &tri_vtx = test_face_vtx.xyz;
                        tri_vtx = tri.ind(test_face);
                        tri_vtx.remap(vtx.dup());
                        test_face_vtxs = 3;
                    }
                ...
                // iterate neighbors
                REP(face_vtxs) {
                    Int adj_face = adjacent[i];
                    if (adj_face != -1) // !=-1 and not <0 because of SIGN_BIT for quads
                    {
                ...
                if (face_list.elms()) {
                    f = face_list.pop();
                    goto check;
                } // process listed elements first
            }
        }
    }
    {
        Memt<Bool> is;
        is.reserve(Max(tris(), quads()));
        Byte *quad_flag = face_flag.data() + tris();
        is.setNum(quads());
        REPAO(is) = !(quad_flag[i] & RDSF_REMOVE);
        keepQuads(is); // !! delete quads first, because removing tris below changes the number of tris, which is needed to get 'quad_flag' !!
        Byte *tri_flag = face_flag.data();
        is.setNum(tris());
        REPAO(is) = !(tri_flag[i] & RDSF_REMOVE);
        keepTris(is); // !! delete tris  next !!
    }
...
Int MeshBase::mergeFaces(Int a, Int b) {
    Int merged = -1;
    if (a != b) {
        Bool a_quad = FlagOn(a, SIGN_BIT), a_in_range;
        Int ai = a;
        if (a_quad) {
            ai ^= SIGN_BIT;
            a_in_range = InRange(ai, quad);
        } else
            a_in_range = InRange(ai, tri);
        if (a_in_range) {
            Bool b_quad = FlagOn(b, SIGN_BIT), b_in_range;
            Int bi = b;
            if (b_quad) {
                bi ^= SIGN_BIT;
                b_in_range = InRange(bi, quad);
            } else
                b_in_range = InRange(bi, tri);
            if (b_in_range) {
                VecI4 a_ind, b_ind;
                Int a_inds, b_inds;
                if (a_quad) {
                    ...
                    if (b_ind.c[b_ofs] == ai) {
                        ...
                        if (a_next == b_prev) {
                            ...
                            if (!a_quad && !b_quad) // 2 triangles
                            {
                                ...
                                if (vtx.pos())
                                    REP(4) // check if any of the points is inline
                                    {
...
Bool MeshBase::removeUnusedVtxs(Bool include_edge_references) {
    ...
    if (vtxs != T.vtxs()) {
        if (!include_edge_references)
            exclude(EDGE_ALL);
        MeshBase temp(vtxs, 0, 0, 0, flag() & (~VTX_DUP));
        temp.copyVtxs(T, is);
        Memt<Int> remap;
...
MeshBase &MeshBase::removeDoubleEdges() {
    if (edge.flag()) {
        Memt<Bool> is;
        is.setNum(edges());
        REPAO(is) = ((edge.flag(i) & ETQ_LR) != ETQ_LR);
        keepEdges(is);
...
MeshBase &MeshBase::removeDegenerateFaces(Flt eps) {
    ...
    // triangles
    REPA(tri)
    if (tri_is[i] = tri.ind(i).allDifferent()) {
        C Int *p = tri.ind(i).c;
        if (!Tri(vtx.pos(p[0]), vtx.pos(p[1]), vtx.pos(p[2])).valid(eps))
            tri_is[i] = false;
    }
    ...
    REPA(quad) {
        ...
        if (Quad(vtx.pos(p[0]), vtx.pos(p[1]), vtx.pos(p[2]), vtx.pos(p[3])).valid(eps)) {
            ...
            if (Tri(vp0, vp1, vp3).valid(eps)) {
                tri_from_quad.New().from(i, p0, p1, p3);
    ...
    if (src_tris != tris() || src_quads != quads()) {
        ...
        // set missing data
        if (temp.tri.flag() && !tri.flag())
            ZeroFastN(temp.tri.flag(), src_tris);
        if (temp.tri.id() && !tri.id())
            ZeroFastN(temp.tri.id(), src_tris);
        if (temp.tri.nrm() && !tri.nrm())
            REP(src_tris) {
        ...
        REPA(tri_from_quad) {
            ...
            if (temp.tri.flag())
                temp.tri.flag(src_tris + i) = (quad.flag() ? quad.flag(tfq.quad_index) : 0);
            if (temp.tri.id())
                temp.tri.id(src_tris + i) = (quad.id() ? quad.id(tfq.quad_index) : 0);
            if (temp.tri.nrm())
                temp.tri.nrm(src_tris + i) = (quad.nrm() ? quad.nrm(tfq.quad_index) : GetNormal(vtx.pos(tfq.vtx_index.x), vtx.pos(tfq.vtx_index.y), vtx.pos(tfq.vtx_index.z)));
        }
...
MeshBase &MeshBase::removeSingleFaces(Flt fraction) {
    ...
    } // compare to -1 and not >=0 because it can have SIGN_BIT
    Memt<Bool> quad_is;
    quad_is.setNum(quads());
    REPA(quad) {
        C VecI4 &a = quad.adjFace(i);
...
static Bool VertexValueTest(MeshBase &mshb, Int mid, Int pa, Int pb, Flt cos_vtx) // return true if shouldn't be welded
{
    ...
    if (length > EPS) {
        ...
        if (mshb.vtx.nrm() && CosBetween(Lerp(mshb.vtx.nrm(pa), mshb.vtx.nrm(pb), step), mshb.vtx.nrm(mid)) < cos_vtx)
            return true;

        const Flt tex_eps2 = Sqr(1.0f / 1024);
        if (mshb.vtx.tex0() && Dist2(Lerp(mshb.vtx.tex0(pa), mshb.vtx.tex0(pb), step), mshb.vtx.tex0(mid)) > tex_eps2)
            return true;
        if (mshb.vtx.tex1() && Dist2(Lerp(mshb.vtx.tex1(pa), mshb.vtx.tex1(pb), step), mshb.vtx.tex1(mid)) > tex_eps2)
            return true;
        if (mshb.vtx.tex2() && Dist2(Lerp(mshb.vtx.tex2(pa), mshb.vtx.tex2(pb), step), mshb.vtx.tex2(mid)) > tex_eps2)
            return true;
        if (mshb.vtx.tex3() && Dist2(Lerp(mshb.vtx.tex3(pa), mshb.vtx.tex3(pb), step), mshb.vtx.tex3(mid)) > tex_eps2)
            return true;
        ...
        if (mshb.vtx.material()) {
            ...
            if (Abs(m.x - mm.x) > mtrl_eps || Abs(m.y - mm.y) > mtrl_eps || Abs(m.z - mm.z) > mtrl_eps || Abs(m.w - mm.w) > mtrl_eps)
                return true;
        }
        ...
        if (mshb.vtx.color()) {
            ...
            const Int col_eps = 3;
            if (Abs(c.a - cm.a) > col_eps || Abs(c.r - cm.r) > col_eps || Abs(c.g - cm.g) > col_eps || Abs(c.b - cm.b) > col_eps)
                return true;
        }
    }
    return false;
...
Bool MeshBase::weldCoplanarFaces(Flt cos_face, Flt cos_vtx, Bool safe, Flt max_face_length) {
    ...
    Bool vtx_nrm_test = (vtx.nrm() && cos_vtx > -1),
         ...
         face_length_test = (max_face_length >= 0);
    max_face_length *= max_face_length; // it's now squared
    C Vec *pos = vtx.pos();
    Memt<Bool> tri_is;
    tri_is.setNum(tris());
    SetMemN(tri_is.data(), true, tri_is.elms());
    Memt<Bool> quad_is;
    quad_is.setNum(quads());
    SetMemN(quad_is.data(), true, quad_is.elms());
    ...
    for (Int i = 0; i < tris();)
        ...
        else // for each tri
        {
            ...
            REPD(vi, 3) // for each tris adjacent tri
            {
                ...
                {
                    // co-planar test
                    if (Dot(nrm, tri.nrm(face1)) < cos_face)
                        continue;
                    ...
                    /*        F0V0 F1V1
                       ...
                       F0V2 <-----|-----> F1V2
                              F0V1 F1V0
                    */
                    Int F0V0 = p[vi], F1V1 = p1[(vi1 + 1) % 3];
                    if (F0V0 != F1V1)
                        continue;
                    Int &F0V1 = p[(vi + 1) % 3], F1V0 = p1[vi1];
                    if (F0V1 != F1V0)
                        continue; // !! need & for 'F0V1' because we change it later !!

                    // safety test (faces without other neighbors)
                    if (safe)
                        if (af[(vi + 1) % 3] != -1 || af1[(vi1 + 2) % 3] != -1)
                            continue; // compare to -1 and not >=0 because it can have SIGN_BIT
                    ...
                    // face length test
                    if (face_length_test && Dist2(pos[F0V2], pos[F1V2]) > max_face_length)
                        continue;

                    // vertex values test
                    if (vtx_value_test && VertexValueTest(T, F0V1, F0V2, F1V2, cos_vtx))
                        continue;
                    ...
                    // fix adjacency
                    REPD(a, 3)
                    if (af1[a] != -1) // compare to -1 and not >=0 because it can have SIGN_BIT
                    {
                        Int face2 = af1[a];
                        if (face2 & SIGN_BIT) {
                            REPD(j, 4)
        ...
        else // for each quad
        {
            ...
            REPD(vi, 4) // for each quads adjacent face
            {
                ...
                if (face1 != -1) // compare to -1 and not >=0 because it can have SIGN_BIT
                {
                    ...
                    Int *p1, *af1, vtxs;
                    if (face1 & SIGN_BIT) {
                        if (Dot(nrm, quad.nrm(face1 ^ SIGN_BIT)) < cos_face)
                            continue;
                        p1 = quad.ind(face1 ^ SIGN_BIT).c;
                        af1 = quad.adjFace(face1 ^ SIGN_BIT).c;
                        vtxs = 4;
                    } else {
                        if (Dot(nrm, tri.nrm(face1)) < cos_face)
                            continue;
                    ...
                    */
                    Int &F0V0 = p[vi], F1V1 = p1[(vi1 + 1) % vtxs];
                    if (F0V0 != F1V1)
                        continue; // !! need & for 'F0V0' because we change it later !!
                    Int &F0V1 = p[(vi + 1) % 4], F1V0 = p1[vi1];
                    if (F0V1 != F1V0)
                        continue; // !! need & for 'F0V1' because we change it later !!

                    // safety test (faces without other neighbors)
                    if (safe) {
                        if (af[(vi + 1) % 4] != -1 || af[(vi + 3) % 4] != -1)
                            continue; // compare to -1 and not >=0 because it can have SIGN_BIT
                        if (face1 & SIGN_BIT)
                            if (af1[(vi1 + 1) % 4] != -1 || af1[(vi1 + 3) % 4] != -1)
                                continue; // compare to -1 and not >=0 because it can have SIGN_BIT
                    }
                    ...
                    // face length test
                    if (face_length_test) {
                        if (Dist2(pos[F0V2], pos[F1V2]) > max_face_length || Dist2(pos[F0V3], pos[F1V2]) > max_face_length)
                            continue;
                        if (face1 & SIGN_BIT)
                            if (Dist2(pos[F0V2], pos[F1V3]) > max_face_length || Dist2(pos[F0V3], pos[F1V3]) > max_face_length)
                                continue;
                    }
                    ...
                    if (face1 & SIGN_BIT) {
                        if (DistPointEdge(pos[F0V0], pos[F0V3], pos[F1V2]) > EPS)
                            continue;
                        if (DistPointEdge(pos[F0V1], pos[F0V2], pos[F1V3]) > EPS)
                            continue;

                        // vertex values test
                        if (vtx_value_test) {
                            if (VertexValueTest(T, F0V0, F0V3, F1V2, cos_vtx) || VertexValueTest(T, F0V1, F0V2, F1V3, cos_vtx))
                                continue;
                        }
                        ...
                        if (DistPointEdge(pos[F0V1], pos[F0V2], pos[F1V2]) <= EPS && DistPointPlane(pos[F1V2], pos[F0V0], CrossN(nrm, pos[F0V3] - pos[F0V0])) <= EPS) // make sure quad won't be concave
                        {
                            if (safe && af1[(vi1 + 2) % 3] != -1)
                                continue; // compare to -1 and not >=0 because it can have SIGN_BIT
                            ...
                            F0V1 = F1V2;
                            af[vi] = af1[(vi1 + 1) % 3];
                        } else if (DistPointEdge(pos[F0V0], pos[F0V3], pos[F1V2]) <= EPS && DistPointPlane(pos[F1V2], pos[F0V1], CrossN(nrm, pos[F0V1] - pos[F0V2])) <= EPS) // make sure quad won't be concave
                        {
                            if (safe && af1[(vi1 + 1) % 3] != -1)
                                continue; // compare to -1 and not >=0 because it can have SIGN_BIT

                            // vertex values test
                            if (vtx_value_test && VertexValueTest(T, F0V0, F0V3, F1V2, cos_vtx))
                                continue;
