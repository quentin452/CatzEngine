...
struct EdgeTex {
    ...
    void set(C Vec &p0, C Vec &p1, C Vec2 &t0, C Vec2 &t1, C Color &c0, C Color &c1) {
        edge.set(p0, p1);
        tex[0] = t0;
        tex[1] = t1;
        col[0] = c0;
...
struct TriTex {
    ...
    void set(Vec &p0, Vec &p1, Vec &p2, Vec2 &t0, Vec2 &t1, Vec2 &t2, C Color &c0, C Color &c1, C Color &c2) {
        tri.set(p0, p1, p2);
        tex[0] = t0;
        tex[1] = t1;
        tex[2] = t2;
...
struct QuadTex {
    ...
    void set(Vec &p0, Vec &p1, Vec &p2, Vec &p3, Vec2 &t0, Vec2 &t1, Vec2 &t2, Vec2 &t3, C Color &c0, C Color &c1, C Color &c2, C Color &c3) {
        quad.set(p0, p1, p2, p3);
        tex[0] = t0;
        tex[1] = t1;
        tex[2] = t2;
        tex[3] = t3;
...
MeshBase &MeshBase::inflateEdges(TEX_MODE x_tex_coords, TEX_MODE y_tex_coords, Bool to_edges) // assumes that vertexes are welded, weldVtx
{
    ...
    if (y_tex_coords != TEX_NONE && y_tex_coords != TEX_KEEP) {
        ...
        Memc<Int> vtx_left;
        FREPA(vtx)
        if (!tex_set[i]) // iterate all not yet set vertexes (in case there are multiple sets not joined together)
        {
            vtx_left.New() = i;
            tex_set[i] = true;
            tex_y[i] = 0;
        ...
        if (y_tex_coords == TEX_UNIFORM) {
            Flt min, max;
            MinMax(tex_y, vtxs(), min, max);
            Flt scale = max - min;
            if (scale)
                scale = 1.0f / scale;
            Flt add = -min * scale;
            REP(vtxs())
            tex_y[i] = tex_y[i] * scale + add;
        }
    }
    ...
    REPAD(e, edge) {
        ...
        // lb
        {
            ...
            default:
                ...
                break;
            case TEX_KEEP:
                tlb.x = (vtx.tex0() ? vtx.tex0(p0).x : 0);
                break;
            case TEX_UNIFORM:
                tlb.x = 0;
                break;
            ...
            default:
                ...
                break;
            case TEX_KEEP:
                tlb.y = (vtx.tex0() ? vtx.tex0(p0).y : 0);
                break;
            case TEX_UNIFORM:
            case TEX_SCALED:
                tlb.y = tex_y[p0];
        ...
        // rb
        {
            ...
            default:
                ...
                break;
            case TEX_KEEP:
                trb.x = (vtx.tex0() ? vtx.tex0(p0).x : 0);
                break;
            case TEX_UNIFORM:
                trb.x = 1;
                break;
            ...
            default:
                ...
                break;
            case TEX_KEEP:
                trb.y = (vtx.tex0() ? vtx.tex0(p0).y : 0);
                break;
            case TEX_UNIFORM:
            case TEX_SCALED:
                trb.y = tex_y[p0];
        ...
        // lf
        {
            ...
            default:
                ...
                break;
            case TEX_KEEP:
                tlf.x = (vtx.tex0() ? vtx.tex0(p1).x : 0);
                break;
            case TEX_UNIFORM:
                tlf.x = 0;
                break;
            ...
            default:
                ...
                break;
            case TEX_KEEP:
                tlf.y = (vtx.tex0() ? vtx.tex0(p1).y : 0);
                break;
            case TEX_UNIFORM:
            case TEX_SCALED:
                tlf.y = tex_y[p1];
        ...
        // rf
        {
            ...
            default:
                ...
                break;
            case TEX_KEEP:
                trf.x = (vtx.tex0() ? vtx.tex0(p1).x : 0);
                break;
            case TEX_UNIFORM:
                trf.x = 1;
                break;
            ...
            default:
                ...
                break;
            case TEX_KEEP:
                trf.y = (vtx.tex0() ? vtx.tex0(p1).y : 0);
                break;
            case TEX_UNIFORM:
            case TEX_SCALED:
                trf.y = tex_y[p1];
        ...
        if (to_edges) {
            ...
            if (p0_p < 0 && p0_n < 0)
                fat_edge.New().set(rb, lb, trb, tlb, cb, cb); // upper edge
            if (p1_p < 0 && p1_n < 0)
                fat_edge.New().set(lf, rf, tlf, trf, cf, cf); // lower edge
        } else {
    ...
    Bool set_tex_coords = (x_tex_coords == TEX_UNIFORM || x_tex_coords == TEX_SCALED || y_tex_coords == TEX_UNIFORM || y_tex_coords == TEX_SCALED || ((x_tex_coords == TEX_KEEP || y_tex_coords == TEX_KEEP) && vtx.tex0()));
...
MeshBase &MeshBase::edgeToPoly(MemPtr<Poly> polys) {
    ...
    if (vtxs() && edges() && edge.flag()) {
        Byte *flag = Alloc<Byte>(edges());
        CopyN(flag, edge.flag(), edges());

        // link vtx->edge
        Index vtx_edge;
        linkVtxEdge(vtx_edge);
    ...
    FREPA(polys) {
        ...
        }
        {
            ...
            MFREPD(vtx, poly.vtx) {
                ...
                h = (height ? height[i] : 1);
                Vec &next = src[i];
                t += Dist(prev, next) * tex_scale;
                dest[j] = dest[j + 1] = next;
                dest[j].z += h;
                tex[j].set(t, 0);
                tex[j + 1].set(t, h);
                prev = next;
...
struct ETT {
    ...
    void vtxRem(MemlNode *vtx) {
        ...
        IndexPtrGroup &ig = vtx_poly.group[index];
        ig.subVal(mpoly);
        if (!ig.num)
            rect_vtx.group[rects.index(pos[index].xy)].subVal(index);
        poly->removeVtx(vtx);
    }
    Int testTri(MemlNode *vtx, Bool test) // returns 0-not ok try next vertex, 1-added triangle and repeat the test, 2-ok finish the test
    {
        ...
        if (test ? tri.valid() : true) {
            ...
            REPAD(p, ig) {
                MemlNode *tpoly = (MemlNode *)ig[p];
                if (tpoly == mpoly) {
                    mpoly_cut = null;
                    break;
                } // cuts self & skip
                if (((Poly *)tpoly->data())->infinite())
                    mpoly_cut = tpoly;
            }
            if (mpoly_cut) // link two polys
            {
                ...
                MemlNode *vtx2 = null;
                MFREP(poly2.vtx) {
                    Int index = poly2.vtx[i].index;
                    if (index == vtx_cut)
                        vtx2 = i;
                    vtx_poly.group[index].changeVal(mpoly_cut, mpoly);
                }
                poly->link(poly2, vtx, vtx2);
                poly2.del();
                polys.remove(mpoly_cut);
                if (poly->vtx.elms() > fis)
                    Realloc(fi, fis = poly->vtx.elms(), 0);
                return 1;
            }
        }
        return 0;
...
MeshBase &MeshBase::edgeToTri(Bool set_id) // assumes: weldVtx, removeUnusedVtxs, this method is required not to modify vertexes (only faces should be changed)
{
    if (vtxs() && edges() && edge.flag()) {
        ...
        FREP(ett.vtxs)
        ett.rect_vtx.elmGroup(ett.rects.index(ett.pos[i].xy), i);
        ett.rect_vtx.set();
        ...
        REPD(test, 2)
        for (ett.mpoly = ett.polys.first(); ett.mpoly;) {
        ...
        // free & create
        exclude(ADJ_ALL | FACE_ALL);
        tri._elms = ett.mtri.elms();
...
struct TriLink {
    ...
    void set(Int t0, Int t1, Flt length2) {
        tri[0] = t0;
        tri[1] = t1;
        T.length2 = length2;
    }
    ...
    if (tris()) {
        ...
        REPD(j, 3) {
            ...
            if (af != -1 && !(af & SIGN_BIT) && i < af && Dot(tri.nrm(i), tri.nrm(af)) >= cos) // compare to -1 and not >=0 because it can have SIGN_BIT
            {
                ...
                Int i0 = j;
                REPD(i1, 3)
                if (p0[i0] == p1[(i1 + 1) % 3] && p0[(i0 + 1) % 3] == p1[i1]) {
                    Vec &a = vtx.pos(p0[i0]),
                        &b = vtx.pos(p1[(i1 + 2) % 3]),
                        &c = vtx.pos(p0[(i0 + 1) % 3]),
                        &d = vtx.pos(p0[(i0 + 2) % 3]),
                        nrm = Cross(tri.nrm(i), b - d);
                    if (DistPointPlane(c, d, nrm) > EPS && DistPointPlane(a, d, nrm) < -EPS)
                        link.New().set(i, af, Dist2(a, c));
                    break;
                }
            }
        }
        if (link.elms()) {
            ...
            Memt<Int> linked;
            linked.setNum(tris());
            SetMemN(linked.data(), 0xFF, linked.elms());
            FREPA(link) {
                TriLink &l = link[i];
...
MeshBase &MeshBase::quadToTri(Flt cos) {
    if (quads() && cos > -1)                          // if cos<=-1 then keep all quads
        ...
        {
            if (tris() && !tri.nrm() && quad.nrm())
                setFaceNormals(); // if there are tris, and quads have normals but tris don't, then set normals for tris
            ...
            if (cos > 1 || !quad.ind() || !pos) // convert all quads to tris
            {
                ...
                REPA(quad) {
                    ...
                    C Vec &p0 = pos[p.x],
                          ...
                          &p2 = pos[p.z],
                          &p3 = pos[p.w];
                    if (keep[i] = (Dot(GetNormal(p0, p1, p3), GetNormal(p1, p2, p3)) >= cos))
                        keep_num++;
                    else
                        split_num++;
                }
...
MeshBase &MeshBase::quadToTri(C CMemPtr<Int> &quads) {
    Memt<Int> actual_quads;
    FREPA(quads) {
        Int q = quads[i];
        if (InRange(q, T.quad))
            actual_quads.binaryInclude(q);
    }
    if (actual_quads.elms()) {
        ...
        if (quad.nrm()) {
            ...
            FREPA(actual_quads) {
                d[0] = d[1] = quad.nrm(actual_quads[i]);
                d += 2;
            }
        }
        if (quad.flag()) {
            ...
            FREPA(actual_quads) {
                d[0] = d[1] = quad.flag(actual_quads[i]);
                d += 2;
            }
        }
        if (quad.id()) {
            ...
            FREPA(actual_quads) {
                d[0] = d[1] = quad.id(actual_quads[i]);
                d += 2;
            }
        }
        Swap(tri, temp.tri);
