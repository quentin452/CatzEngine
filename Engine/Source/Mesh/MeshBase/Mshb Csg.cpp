...
struct CSG // Constructive Solid Geometry
{
    struct Vtx : VtxFull // vertex with enhanced position precision
    {
        ...
        void to(MeshBase &mshb, Int i) C {
            super::to(mshb, i);
            if (InRange(i, mshb.vtx) && mshb.vtx.pos())
                mshb.vtx.pos(i) = posd;
        }
    };
    struct Face {
        ...
        void set(C VecI &ind, Int id, UInt flag) {
            T.ind = ind;
            T.id = id;
            T.flag = flag;
        }
    ...
    struct FaceEdgeI {
        ...
        void set(Int index0, Int index1, UInt flag) {
            T.index.set(index0, index1);
            T.flag = flag;
        }
    };
    ...
    void addTri(C VecI &ind, Int id, UInt flag) {
        ...
        box |= vtx[ind.y].pos;
        box |= vtx[ind.z].pos;
        if (CutsEps(box, box_ab))
            face.New().set(ind, id, flag); // add for processing
        else
            face_done.New().set(ind, id, flag); // add as  processed
    }
    void addMesh(C MeshBase &mshb, UInt flag) {
        Int vtxs = vtx.elms();
        FREPA(mshb.vtx)
        T.vtx.New().from(mshb, i);
        FREPA(mshb.tri)
        addTri(mshb.tri.ind(i) + vtxs, mshb.tri.id() ? mshb.tri.id(i) : -1, flag);
        FREPA(mshb.quad) {
            ...
            for (Int z = boxi.min.z; z <= boxi.max.z; z++)
                for (Int y = boxi.min.y; y <= boxi.max.y; y++)
                    for (Int x = boxi.min.x; x <= boxi.max.x; x++)
                        box_face[boxes_ab.index(VecI(x, y, z))].New() = i;
        }
    }
    ...
    void triFromEdges(Face &face, TriND &face_tri, MatrixD &face_matrix, Memb<Face> &faces) {
        ...
        REPAD(i, face_edge) {
            ...
            if (i != j) {
                ...
                REPD(c, CutsEdgeEdge(face_edge_i.edge_2d, face_edge[j].edge_2d, &cuts)) {
                    ...
                    if (step > EPSD && step < face_edge_i.edge_2d.length - EPSD)
                        steps.New() = step / face_edge_i.edge_2d.length;
                }
            }
            ...
            p1 = getPos(face_edge_i.edge.p[1], face_matrix);
            if (p1 != p0)
                face_edgei.New().set(p0, p1, face_edge_i.flag);
        ...
        // join edges
        FREPAD(i, face_edgei) {
            FaceEdgeI &face_edgei_i = face_edgei[i];
            Int pi0 = face_edgei_i.index.x, pi1 = face_edgei_i.index.y;
            for (Int j = i + 1; j < face_edgei.elms();) {
                FaceEdgeI &face_edgei_j = face_edgei[j];
                Int pj0 = face_edgei_j.index.x, pj1 = face_edgei_j.index.y;
    ...
    void cutFaces(Bool detect_self_intersections) {
        REPA(face) // for each face
        {
            ...
            MatrixD face_matrix;
            face_matrix.setPosDir(face_tri.p[0], -face_tri.n);
            ...
            for (Int z = boxi.min.z; z <= boxi.max.z; z++) // for every box where face lies
                for (Int y = boxi.min.y; y <= boxi.max.y; y++)
                    for (Int x = boxi.min.x; x <= boxi.max.x; x++) {
                        ...
                        {
                            Int testi = box_face[t];
                            if (testi != i) // if the face isn't current
                            {
                                Face &test = T.face[testi];
                                UInt flag = (face.flag | test.flag);
                                if (detect_self_intersections ? true : (flag & SOLID_A && flag & SOLID_B))
            ...
            {
#if DEBUG
                if (DrawEdges && !face_edge.elms())
                    face_tri.draw(RED);
#endif
                Memb<Face> &faces = (overlapped ? face_over : face_done);
                if (face_edge.elms())
                    triFromEdges(face, face_tri, face_matrix, faces); //    intersection
                else
                    faces.New() = face; // no intersection
            }
            face_edge.clear();
        }
    }
...
*/
        ...
        if (tri1.coplanar(tri0)) {
            ...
            REPD(e1, 3) {
                ...
                REPD(e0, 3)
                if (Int c = CutsEdgeEdge(ei1, ei0[e0], &cuts)) {
                    if (c == 2)
                        return DistPointPlane(tri1.p[(e1 + 2) % 3], tri0.p[e0], tri_n[e0]) < 0;

                    Int dup0;
                    if (Equal(cuts.p[0], ei0[e0].p[0]))
                        dup0 = 0;
                    else if (Equal(cuts.p[0], ei0[e0].p[1]))
                        dup0 = 1;
                    else
                        dup0 = -1;

                    Int dup1;
                    if (Equal(cuts.p[0], ei1.p[0]))
                        dup1 = 0;
                    else if (Equal(cuts.p[0], ei1.p[1]))
                        dup1 = 1;
                    else
                        dup1 = -1;
                    ...
                    if (dup0 < 0) {
                        if (dup1 < 0)
                            return true;
                        if (DistPointPlane(ei1.p[!dup1], tri0.p[e0], tri_n[e0]) < 0)
                            return true;
                    } else {
                        if (dup1 != 0)
                            if (DistPointPlane(ei1.p[0], tri0.p[(e0 + dup0) % 3], tri_n[(e0 + dup0) % 3]) < -EPSD && DistPointPlane(ei1.p[0], tri0.p[(e0 + dup0 + 2) % 3], tri_n[(e0 + dup0 + 2) % 3]) < -EPSD)
                                return true;
                        if (dup1 != 1)
                            if (DistPointPlane(ei1.p[1], tri0.p[(e0 + dup0) % 3], tri_n[(e0 + dup0) % 3]) < -EPSD && DistPointPlane(ei1.p[1], tri0.p[(e0 + dup0 + 2) % 3], tri_n[(e0 + dup0 + 2) % 3]) < -EPSD)
                                return true;
                    }
                    touch = true;
                }
                if (!touch && DistPointPlane(ei1.p[0], tri0.p[0], tri_n[0]) < 0 && DistPointPlane(ei1.p[0], tri0.p[1], tri_n[1]) < 0 && DistPointPlane(ei1.p[0], tri0.p[2], tri_n[2]) < 0)
                    return true;
            }
        }
        return false;
    ...
    void removeOverlapped() {
        // set boxes
        REPA(face_over)
        setFaceBox(face_over[i]);

        // detect overlaps
        REPAD(i, face_over) {
            Face &face_i = face_over[i];
            TriND tri_i(vtx[face_i.ind.x].posd, vtx[face_i.ind.y].posd, vtx[face_i.ind.z].posd);
        ...
        REPA(face_done) {
            ...
            if (flag & SOLID_CUT) {
                ...
                if (!(flag & SOLID_A) || !(flag & SOLID_B)) {
                    ...
                    {
                        if (CutsPointMesh(p, a, null, &cuts_cache_a))
                            flag |= SOLID_AR | SOLID_AL;
                        else
                            flag |= SOLID_NAR | SOLID_NAL;
#if DEBUG
                        if (DrawInside)
                            TriD(vtx[ind.x].posd, vtx[ind.y].posd, vtx[ind.z].posd).draw((flag & SOLID_AR) ? ORANGE : PURPLE, true);
#endif
                    }
                    if (!(flag & SOLID_B)) // if there's no info about SOLID_B
                    {
                        if (CutsPointMesh(p, b, null, &cuts_cache_b))
                            flag |= SOLID_BR | SOLID_BL;
                        else
                            flag |= SOLID_NBR | SOLID_NBL;
#if DEBUG
                        if (DrawInside)
                            TriD(vtx[ind.x].posd, vtx[ind.y].posd, vtx[ind.z].posd).draw((flag & SOLID_BR) ? ORANGE : PURPLE, true);
#endif
                    }
                }
            }
    ...
    {
        Memb<Int> tris;
        tris.New() = tri; // list of triangles for checking
        for (; tris.elms();) {
            ...
            if (!(mshb.tri.flag(tri) & solid)) // if there's no info about solid
            {
                ...
                if (!Kb.b(KB_D))
                    ...
                    REPA(adj_face) // for each neighbor
                    {
                        ...
                        if (adj != -1)
                            tris.New() = adj; // if it exists then add it to the list, compare to -1 and not >=0 because it can have SIGN_BIT
                    }
            }
        }
    }
            ...
            REPD(s, 2) // distribute 2 solids
            {
                ...
                REPA(mshb.tri)                        // for each triangle
                {
                    if (UInt f = (mshb.tri.flag(i) & solid)) // if it has info about solid
                    {
                        ...
                        {
                            Int adj = af.c[i];
                            if (adj != -1 && !(mshb.tri.flag(adj) & solid))
                                distributeSolid(mshb, adj, f, solid); // if the neighbor exists and it doesn't have the info about solid which we can provide, compare to -1 and not >=0 because it can have SIGN_BIT
                        }
                    }
                }
            }
    ...
    Bool faceIs(UInt sel, UInt f, Bool &reverse) {
        ...
        case SEL_A: // a sub b
        {
            ...
            {
                if (f & SOLID_AR && f & SOLID_NAL)
                    return true;
                if (f & SOLID_AL && f & SOLID_NAR)
                    return reverse = true;
            } else if (f & SOLID_AR && f & SOLID_AL) // inside A
            {
                if (f & SOLID_BR && f & SOLID_NBL)
                    return reverse = true;
                if (f & SOLID_BL && f & SOLID_NBR)
                    return true;
            }
        } break;
        ...
        case SEL_B: // b sub a
        {
            if (f & SOLID_NAL && f & SOLID_NAR) // outside A
            {
                ...
                if (f & SOLID_BL && f & SOLID_NBR)
                    return reverse = true;
            } else if (f & SOLID_BR && f & SOLID_BL) // inside B
            {
                if (f & SOLID_AR && f & SOLID_NAL)
                    return reverse = true;
                if (f & SOLID_AL && f & SOLID_NAR)
        ...
        case SEL_A | SEL_B | SEL_AB: // add,or
        {
            if (f & SOLID_NBL && f & SOLID_NBR) // outside B
            {
                ...
                if (f & SOLID_AL && f & SOLID_NAR)
                    return reverse = true;
            } else if (f & SOLID_NAL && f & SOLID_NAR) // outside A
            {
                if (f & SOLID_BR && f & SOLID_NBL)
                    return true;
                if (f & SOLID_BL && f & SOLID_NBR)
            ...
            {
                if (f & SOLID_AR && f & SOLID_NAL && f & SOLID_BR && f & SOLID_NBL)
                    return true;
                if (f & SOLID_AL && f & SOLID_NAR && f & SOLID_BL && f & SOLID_NBR)
                    return reverse = true;
            }
        } break;
        ...
        case SEL_AB: // and,mul
        {
            if (f & SOLID_BL && f & SOLID_BR) // inside B
            {
                ...
                if (f & SOLID_AL && f & SOLID_NAR)
                    return reverse = true;
            } else if (f & SOLID_AL && f & SOLID_AR) // inside A
            {
                if (f & SOLID_BR && f & SOLID_NBL)
                    return true;
                if (f & SOLID_BL && f & SOLID_NBR)
    ...
    void removeAndFlipFaces(MeshBase &mshb, UInt sel) {
        ...
        REPA(mshb.tri) {
            Bool reverse;
            if (is[i] = faceIs(sel, mshb.tri.flag(i), reverse))
                if (reverse) {
    ...
    void optimize(MeshBase &mshb, Flt weld_pos_eps) {
        ...
        mshb.weldVtxValues(VTX_POS, weld_pos_eps, EPS_COL_COS, -1).weldVtx(VTX_ALL, 0);
    }
...
void Csg(MeshLod &a, C MeshLod &b, UInt sel, MeshLod *dest, Bool detect_self_intersections, Flt weld_pos_eps) {
    ...
    FREPA(a)
    materials.add(a.parts[i]);
    FREPA(b)
    materials.add(b.parts[i]);

    MeshBase a_base;
    a_base.create(a, MESH_ALL, true);
    MeshBase b_base;
    b_base.create(b, MESH_ALL, true);

    REPA(b_base.edge)
    b_base.edge.id(i) += a.parts.elms();
    REPA(b_base.tri)
    b_base.tri.id(i) += a.parts.elms();
    REPA(b_base.quad)
    b_base.quad.id(i) += a.parts.elms();
    ...
    FREPA(*dest)
    dest->parts[i].multiMaterial(materials[i].m[0], materials[i].m[1], materials[i].m[2], materials[i].m[3]);
}
/******************************************************************************/
// CLIP MESH
/******************************************************************************/
static void ClipMeshSimple(C MeshBase &src, C Matrix *matrix, MeshBase &dest, C Plane *clip_plane, Int clip_planes, Flt weld_pos_eps) // processes only vertex positions and face (triangle and quad) indexes
{
    ...
    // box test
    Vec corner[8];
    OBox(Box(src), matrix ? *matrix : MatrixIdentity).toCorners(corner);
    Bool all_inside = true;
    REPD(p, clip_planes) {
        Bool inside = false,
             outside = false;
        REPAD(c, corner) {
            if (Dist(corner[c], clip_plane[p]) <= 0)
                inside = true;
            else
                outside = true;
        }
        if (!inside)
            goto finished; // if no vertexes are inside the plane then cancel
        if (outside)
            all_inside = false;
    }
    ...
    if (all_inside) {
        ...
        REPA(src.tri) {
            ...
            if (matrix)
                REPAO(poly[poly_cur]) *= *matrix;
            REP(clip_planes) {
                ClipPoly(poly[poly_cur], clip_plane[i], poly[poly_cur ^ 1]);
                poly_cur ^= 1;
                if (!poly[poly_cur].elms())
                    goto no_tri;
            }
            Swap(polys.New(), poly[poly_cur]);
            if (set_flags)
                poly_flags.add(src.tri.flag() ? src.tri.flag(i) : 0);
        no_tri:;
        ...
        REPA(src.quad) {
            ...
            Swap(polys.New(), poly[poly_cur]);
            if (set_flags)
                poly_flags.add(src.quad.flag() ? src.quad.flag(i) : 0);
        no_quad:;
            poly[poly_cur ^ 1].clear();
        }
    ...
    if (!(flag_and & VTX_ALL & ~(VTX_POS | VTX_FLAG | VTX_DUP))) // if simple is enough
    {
        ...
        if (all_inside) {
            ...
            REPA(src.tri) {
                ...
                if (matrix)
                    REPAO(poly[poly_cur]).mul(*matrix, matrix3);
                REP(clip_planes) {
                    ClipPoly(poly[poly_cur], clip_plane[i], poly[poly_cur ^ 1]);
                    poly_cur ^= 1;
                    if (!poly[poly_cur].elms())
...
void ClipMesh(C MeshLod &src, C Matrix *matrix, MeshLod &dest, C Plane *clip_plane, Int clip_planes, MESH_FLAG flag_and, Flt weld_pos_eps) {
    ...
    Box box = src;
    Vec corner[8];
    box.toCorners(corner);
    if (matrix)
        Transform(corner, *matrix, Elms(corner));
    Bool all_inside = true;
    REPD(p, clip_planes) {
        Bool inside = false,
             outside = false;
    ...
    if (all_inside) {
        ...
        FREPA(src) {
            C MeshPart &src_part = src.parts[i];
            MeshBase temp_src;
            if (!src_part.base.is() && src_part.render.is())
                temp_src.create(src_part.render);
            ...
            // polys to dest
            if (polys.elms()) {
                MeshPart &dest_part = temp_dest.parts.New();
                dest_part.copyParams(src_part);
                dest_part.scaleParams(scale);
...
void ClipMesh(C Mesh &src, C Matrix *matrix, Mesh &dest, C Plane *clip_plane, Int clip_planes, MESH_FLAG flag_and, Flt weld_pos_eps) {
    ...
    if (all_inside) {
        ...
        REPD(l, src.lods()) // order is important
        {
            ...
            FREPA(src_lod) {
                C MeshPart &src_part = src_lod.parts[i];
                MeshBase temp_src;
                if (!src_part.base.is() && src_part.render.is())
                    temp_src.create(src_part.render);
                C MeshBase &src = (src_part.base.is() ? src_part.base : temp_src);
                ...
                // polys to dest
                if (polys.elms()) {
                    MeshPart &dest_part = dest_lod.parts.New();
                    dest_part.copyParams(src_part);
                    dest_part.scaleParams(scale);
                    Triangulate(polys, dest_part.base, src.flag() & flag_and, weld_pos_eps, true, poly_flags.data());
                    polys.clear();
            ...
            if (!dest_lod.parts.elms())
                temp_dest.removeLod(l);
            else {
        ...
        temp_dest.setBox();
        temp_dest.lod_center = src.lod_center;
        if (matrix)
            temp_dest.lod_center *= *matrix; // set manual lod center after setting box
    }
...
void SplitMesh(C Mesh &src, C Matrix *matrix, Mesh &dest_positive, Mesh &dest_negative, C Plane &clip_plane, MESH_FLAG flag_and, Flt weld_pos_eps) {
    ...
    } else if (!outside) // if no vertexes are outside the plane
    {
    ...
    } else // vertexes are from both sides so we need to cut
    {
        ...
        temp_positive.setLods(src.lods());
        temp_positive.copyParams(src);
        if (!same) {
            temp_negative.setLods(src.lods());
            temp_negative.copyParams(src);
        }
        REPD(l, src.lods()) // order is important
        {
            C MeshLod &src_lod = src.lod(l);
            ...
            FREPA(src_lod) {
                ...
                REPA(src.tri) {
                    ...
                    if (matrix)
                        REPAO(poly_src).mul(*matrix, matrix3);
                    SplitPoly(poly_src, clip_plane, poly_pos, poly_neg);
                    if (poly_pos.elms())
                        Swap(polys_pos.New(), poly_pos);
                    if (poly_neg.elms())
                        Swap(polys_neg.New(), poly_neg);
                    poly_src.clear();
                }
                ...
                // polys to dest
                if (polys_pos.elms()) {
                    MeshPart &dest_part = dest_lod_p.parts.New();
                    dest_part.copyParams(src_part);
                    dest_part.scaleParams(scale);
                ...
                if (polys_neg.elms()) {
                    MeshPart &dest_part = dest_lod_n.parts.New();
                    dest_part.copyParams(src_part);
                    dest_part.scaleParams(scale);
            ...
            if (!dest_lod_p.parts.elms())
                temp_positive.removeLod(l);
            else {
                dest_lod_p.copyParams(src_lod);
                dest_lod_p.scaleParams(scale);
            }
            if (!same && !dest_lod_n.parts.elms())
                temp_negative.removeLod(l);
            else {
...
struct MiddlePoint {
    ...
    void set(Flt prev_dist, Flt next_dist, C Vec &pos) {
        T.prev_dist = prev_dist;
        T.next_dist = next_dist;
        T.pos = pos;
    }
};
static void SplitPoly(C Memc<VtxFull> &poly, C Plane &plane, Memc<VtxFull> &output_positive, Memc<VtxFull> &output_negative, Memc<Edge> &edges) {
    ...
    case 1:
        if (Dist(poly[0].pos, plane) < 0)
            output_negative.add(poly[0]);
        else
            output_positive.add(poly[0]);
        break;
    ...
    default: // >=2
    {
        ...
        FREPA(poly) // preserve order
        {
            ...
            if (prev_dist < 0) {
                ...
                if (next_dist >= 0) // change of side
                {
                    if (!have_mid) {
                        have_mid = true;
                        mid.lerp(*prev, next, prev_dist / (prev_dist - next_dist));
                    } // -prev_dist/(next_dist-prev_dist)
            ...
            if (have_mid)
                middles.New().set(prev_dist, next_dist, mid.pos);
        ...
        for (; middles.elms() >= 2;) // && Dot(middles.last().pos, cross)>Dot(middles.first().pos, cross); )
        {
            Flt a = Dot(middles.first().pos, cross),
                b = Dot(middles.last().pos, cross);
            if (a >= b)
                break; // must be stored in Flt variables insted of comparing values directly due to imprecisions of FPU
            middles.rotateOrder(1);
        }

        // create clip edges
        FREPA(middles)
        if (InRange(i + 1, middles)) {
            MiddlePoint &m = middles[i],
                        &m1 = middles[i + 1];

            if (m.prev_dist <= 0 && m.next_dist >= 0 && m1.prev_dist >= 0 && m1.next_dist <= 0)
                edges.New().set(m.pos, m1.pos);
        }
    } break;
    }
}
    ...
    } else // vertexes are from both sides so we need to cut
    {
        ...
        REPD(l, src.lods()) // order is important
        {
            ...
            if (edges.elms()) {
                ...
                MeshBase temp2D(temp);
                temp2D.transform(~Matrix3().setDir(clip_plane.normal)).edgeToTri();
                Swap(temp2D.tri, temp.tri);
                if (temp.faces()) {
                    temp.texMap(clip_plane).texScale(Vec2(tex_scale)).include(VTX_NRM);
                    REPA(temp.vtx)
                    temp.vtx.nrm(i) = -clip_plane.normal;
                    MeshPart &solid_p = dest_lod_p.parts.New();
                    solid_p.base = temp;
                    solid_p.material(material, l);
                    MeshPart &solid_n = dest_lod_n.parts.New();
                    solid_n.base = temp.reverse();
                    solid_n.material(material, l);
                }
            }
