...
void MeshBase::split(MemPtr<MeshBaseIndex> meshes, C Boxes &boxes, MESH_FLAG flag_and) C {
    ...
    Index cell_quad(boxes.num());
    {
        ...
        FREPA(edge) {
            p = edge.ind(i).c;
            cell_edge.incGroup(edge_cell[i] = boxes.index(Avg(vtx.pos(p[0]), vtx.pos(p[1]))));
        }
        cell_edge.set(edge_cell);
        Int *tri_cell = _cell;
        FREPA(tri) {
            p = tri.ind(i).c;
            cell_tri.incGroup(tri_cell[i] = boxes.index(Avg(vtx.pos(p[0]), vtx.pos(p[1]), vtx.pos(p[2]))));
        }
        cell_tri.set(tri_cell);
        Int *quad_cell = _cell;
        FREPA(quad) {
            p = quad.ind(i).c;
            cell_quad.incGroup(quad_cell[i] = boxes.index(Avg(vtx.pos(p[0]), vtx.pos(p[1]), vtx.pos(p[2]), vtx.pos(p[3]))));
        }
        cell_quad.set(quad_cell);
        Free(_cell);
    }
...
void MeshBase::split(MemPtr<MeshBaseIndex> meshes, C Plane *plane, Int planes, MESH_FLAG flag_and) C {
    ...
    Index cell_quad(cells);
    {
        ...
        FREPA(edge) {
            p = edge.ind(i).c;
            cell_edge.incGroup(edge_cell[i] = CellIndex(Avg(vtx.pos(p[0]), vtx.pos(p[1])), plane, planes));
        }
        cell_edge.set(edge_cell);
        Int *tri_cell = _cell;
        FREPA(tri) {
            p = tri.ind(i).c;
            cell_tri.incGroup(tri_cell[i] = CellIndex(Avg(vtx.pos(p[0]), vtx.pos(p[1]), vtx.pos(p[2])), plane, planes));
        }
        cell_tri.set(tri_cell);
        Int *quad_cell = _cell;
        FREPA(quad) {
            p = quad.ind(i).c;
            cell_quad.incGroup(quad_cell[i] = CellIndex(Avg(vtx.pos(p[0]), vtx.pos(p[1]), vtx.pos(p[2]), vtx.pos(p[3])), plane, planes));
        }
        cell_quad.set(quad_cell);
        Free(_cell);
    }
...
void MeshBase::splitVtxs(MeshBase &dest, C CMemPtr<Bool> &vtx_is, MESH_FLAG flag_and) {
    if (this != &dest) {
        if (!vtx_is.elms())
            ...
            is2.setNum(edges());
            REPA(edge) {
                C VecI2 &e = edge.ind(i);
                if (is2[i] = (ElmIs(vtx_is, e.x) || ElmIs(vtx_is, e.y)))
                    is[e.x] = is[e.y] = true;
            }
            dest.edge._elms = CountIs(is2);
            dest.include(flag() & flag_and & EDGE_ALL);
            dest.copyEdges(T, is2);
            is2.setNum(tris());
            REPA(tri) {
                C VecI &t = tri.ind(i);
                if (is2[i] = (ElmIs(vtx_is, t.x) || ElmIs(vtx_is, t.y) || ElmIs(vtx_is, t.z)))
                    is[t.x] = is[t.y] = is[t.z] = true;
            }
            dest.tri._elms = CountIs(is2);
            dest.include(flag() & flag_and & TRI_ALL);
            dest.copyTris(T, is2);
            is2.setNum(quads());
            REPA(quad) {
                C VecI4 &q = quad.ind(i);
                if (is2[i] = (ElmIs(vtx_is, q.x) || ElmIs(vtx_is, q.y) || ElmIs(vtx_is, q.z) || ElmIs(vtx_is, q.w)))
                    is[q.x] = is[q.y] = is[q.z] = is[q.w] = true;
            }
            dest.quad._elms = CountIs(is2);
            dest.include(flag() & flag_and & QUAD_ALL);
            dest.copyQuads(T, is2);
            dest.vtx._elms = CountIs(is);
            dest.include(flag() & flag_and & (VTX_ALL & ~VTX_DUP));
            dest.copyVtxs(T, is);
            Memt<Int> vtx_remap;
            ...
            REPA(tri) {
                C VecI &t = tri.ind(i);
                if (is2[t.x] && is2[t.y] && is2[t.z])
                    is[t.x] = is[t.y] = is[t.z] = true;
            }
            REPA(quad) {
                C VecI4 &q = quad.ind(i);
                if (is2[q.x] && is2[q.y] && is2[q.z] && is2[q.w])
                    is[q.x] = is[q.y] = is[q.z] = is[q.w] = true;
            }
            keepVtxs(is);
        }
...
void MeshBase::splitFaces(MeshBase &dest, C CMemPtr<Bool> &edge_is, C CMemPtr<Bool> &tri_is, C CMemPtr<Bool> &quad_is, MESH_FLAG flag_and) {
    if (this != &dest) {
        if (!edge_is.elms() && !tri_is.elms() && !quad_is.elms())
            dest.del();
        else {
            // copy
            ...
            Memt<Bool> edge_nis;
            edge_nis.setNum(edges());
            if (!edge_is)
                SetMem(edge_nis.data(), 1, edge_nis.elms());
            else {
                Int i = 0, elms = Min(edge_is.elms(), edge_nis.elms());
                for (; i < elms; i++)
                    edge_nis[i] = edge_is[i] ^ 1;
                for (; i < edge_nis.elms(); i++)
                    edge_nis[i] = true;
            }
            Memt<Bool> tri_nis;
            tri_nis.setNum(tris());
            if (!tri_is)
                SetMem(tri_nis.data(), 1, tri_nis.elms());
            else {
                Int i = 0, elms = Min(tri_is.elms(), tri_nis.elms());
                for (; i < elms; i++)
                    tri_nis[i] = tri_is[i] ^ 1;
                for (; i < tri_nis.elms(); i++)
                    tri_nis[i] = true;
            }
            Memt<Bool> quad_nis;
            quad_nis.setNum(quads());
            if (!quad_is)
                SetMem(quad_nis.data(), 1, quad_nis.elms());
            else {
                Int i = 0, elms = Min(quad_is.elms(), quad_nis.elms());
                for (; i < elms; i++)
                    quad_nis[i] = quad_is[i] ^ 1;
                for (; i < quad_nis.elms(); i++)
                    quad_nis[i] = true;
            }
