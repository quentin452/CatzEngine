...
Int SetVtxDup(MemPtr<VtxDup> vtxs, C Box &box, Flt pos_eps) {
    ...
    REPA(vtxs)
    box_vtx.elmGroup(boxes.index(vtxs[i].pos), i);
    box_vtx.set();
    ...
    FREPD(x, boxes.cells.x) {
        ...
        if (x)
            box_test[box_tests++] = box_cur - 1;
        if (y) {
            if (x < xs - 1)
                box_test[box_tests++] = box_cur + 1 - xs;
        ...
        if (z) {
            if (y < ys - 1) {
                if (x < xs - 1)
                    box_test[box_tests++] = box_cur + 1 + xs - xys;
                box_test[box_tests++] = box_cur + xs - xys;
                if (x)
                    box_test[box_tests++] = box_cur - 1 + xs - xys;
            }
            if (x < xs - 1)
                box_test[box_tests++] = box_cur + 1 - xys;
        ...
        FREPA(*box_cur) // iterate all vertexes in this box
        {
            ...
            REPD(c, box_tests)            // iterate all boxes to test
            {
                ...
                REPD(m, (box_cur == bt) ? i : bt->num) // iterate all vtxs in the test box (if the test box is the current box, then check only vertexes before the current one)
                {
                    ...
                    {
                        // found a duplicate
                        vtx_cur.dup = vtx_test_i;
                        goto next; // set 'dup' index to point to unique 'vtx_test_i'
                    }
                }
            }
...
MeshBase &MeshBase::setVtxDup2D(MESH_FLAG flag, Flt pos_eps, Flt nrm_cos) {
    ...
    if (nrm_cos <= -1)
        FlagDisable(flag, VTX_NRM); // disable vtx normal tests if we have tolerant 'nrm_cos'

    // link rect->vtx
    Rects rects(Rect(T), vtxs());
    Index rect_vtx(rects.num(), vtxs());
    REPA(vtx)
    rect_vtx.elmGroup(rects.index(vtx.pos(i).xy), i);
    rect_vtx.set();
    ...
    FREPD(x, xs) {
        ...
        if (x)
            rect_test[rect_tests++] = rect_cur - 1;
        if (y) {
            if (x < xs - 1)
                rect_test[rect_tests++] = rect_cur + 1 - xs;
        ...
        FREPA(*rect_cur) {
            ...
            REPD(c, rect_tests) {
                ...
                REPD(j, (rect_cur == rt) ? i : rt->num) {
                    Int ind_test = (*rt)[j];
                    if (vtx.dup(ind_test) == ind_test && Equal(pos_cur, vtx.pos(ind_test).xy, pos_eps)) {
                        if (flag) {
                            if (flag & VTX_NRM && Dot(vtx.nrm(ind_cur), vtx.nrm(ind_test)) < nrm_cos)
                                continue;
                            if (flag & VTX_TAN && Dot(vtx.tan(ind_cur), vtx.tan(ind_test)) < EPS_TAN_COS)
                                continue;
                            if (flag & VTX_BIN && Dot(vtx.bin(ind_cur), vtx.bin(ind_test)) < EPS_BIN_COS)
                                continue;
                            if (flag & VTX_HLP && !Equal(vtx.hlp(ind_cur), vtx.hlp(ind_test)))
                                continue;
                            if (flag & VTX_TEX0 && !EqualTex(vtx.tex0(ind_cur), vtx.tex0(ind_test)))
                                continue;
                            if (flag & VTX_TEX1 && !EqualTex(vtx.tex1(ind_cur), vtx.tex1(ind_test)))
                                continue;
                            if (flag & VTX_TEX2 && !EqualTex(vtx.tex2(ind_cur), vtx.tex2(ind_test)))
                                continue;
                            if (flag & VTX_TEX3 && !EqualTex(vtx.tex3(ind_cur), vtx.tex3(ind_test)))
                                continue;
                            if (flag & VTX_SIZE && !Equal(vtx.size(ind_cur), vtx.size(ind_test)))
                                continue;
                            if (flag & VTX_BLEND && vtx.blend(ind_cur) != vtx.blend(ind_test))
                                continue;
                            if (flag & VTX_MATRIX && vtx.matrix(ind_cur) != vtx.matrix(ind_test))
                                continue;
                            if (flag & VTX_MATERIAL && vtx.material(ind_cur) != vtx.material(ind_test))
                                continue;
                            if (flag & VTX_COLOR && vtx.color(ind_cur) != vtx.color(ind_test))
                                continue;
                            if (flag & VTX_FLAG && vtx.flag(ind_cur) != vtx.flag(ind_test))
                                continue;
                        }
                        vtx.dup(ind_cur) = ind_test;
                        goto next;
                    }
                }
            }
...
MeshBase &MeshBase::setVtxDupEx(MESH_FLAG flag, Flt pos_eps, Flt nrm_cos, Flt tan_cos, Flt bin_cos, Bool tex_wrap, Bool smooth_groups_in_vtx_material) {
    ...
    if (nrm_cos <= -1)
        FlagDisable(flag, VTX_NRM); // disable vtx normal   tests if we have tolerant 'nrm_cos'
    if (tan_cos <= -1)
        FlagDisable(flag, VTX_TAN); // disable vtx tangent  tests if we have tolerant 'tan_cos'
    if (bin_cos <= -1)
        FlagDisable(flag, VTX_BIN); // disable vtx binormal tests if we have tolerant 'bin_cos'

    // link box->vtx
    Boxes boxes(Box(T), vtxs());
    Index box_vtx(boxes.num(), vtxs());
    REPA(vtx)
    box_vtx.elmGroup(boxes.index(vtx.pos(i)), i);
    box_vtx.set();
                ...
                REPD(j, (box_cur == bt) ? i : bt->num) // iterate all vertexes in box for testing (but if it's current box then check only vertexes up to current vertex to make sure they were already processed before)
                {
                        ...
                        && Equal(pos_cur, vtx.pos(ind_test), pos_eps)) // same position
                    {
                        if (flag) {
                            if (flag & VTX_NRM && Dot(vtx.nrm(ind_cur), vtx.nrm(ind_test)) < nrm_cos)
                                continue;
                            if (flag & VTX_TAN && Dot(vtx.tan(ind_cur), vtx.tan(ind_test)) < tan_cos)
                                continue;
                            if (flag & VTX_BIN && Dot(vtx.bin(ind_cur), vtx.bin(ind_test)) < bin_cos)
                                continue;
                            if (flag & VTX_HLP && !Equal(vtx.hlp(ind_cur), vtx.hlp(ind_test)))
                                continue;
                            if (flag & VTX_TEX0 && !EqualTex(vtx.tex0(ind_cur), vtx.tex0(ind_test), tex_wrap))
                                continue;
                            if (flag & VTX_TEX1 && !EqualTex(vtx.tex1(ind_cur), vtx.tex1(ind_test), tex_wrap))
                                continue;
                            if (flag & VTX_TEX2 && !EqualTex(vtx.tex2(ind_cur), vtx.tex2(ind_test), tex_wrap))
                                continue;
                            if (flag & VTX_TEX3 && !EqualTex(vtx.tex3(ind_cur), vtx.tex3(ind_test), tex_wrap))
                                continue;
                            if (flag & VTX_SIZE && !Equal(vtx.size(ind_cur), vtx.size(ind_test)))
                                continue;
                            if (flag & VTX_BLEND && vtx.blend(ind_cur) != vtx.blend(ind_test))
                        ...
                        if (smooth_groups_in_vtx_material)
                            if (!(vtx.material(ind_cur).u & vtx.material(ind_test).u))
                                continue; // if there are no shared smooth groups
                        vtx.dup(ind_cur) = ind_test;
                        goto next; // set duplicate and continue
                    }
                }
            }
...
void MeshBase::linkVtxVtxSamePos(Index &vtx_vtx, Flt pos_eps, Bool dual) C {
        ...
        FREPA(*box_cur) // iterate all vertexes in current box
        {
            ...
            REPD(c, box_tests)                 // iterate all nearby boxes for testing
            {
                ...
                REPD(j, (box_cur == bt) ? i : bt->num) // iterate all vertexes in box for testing (but if it's current box then check only vertexes up to current vertex to make sure they were already processed before)
                {
                    ...
                    {
                        temp_vtx_vtx.New().set(ind_cur, ind_test);
                        vtx_vtx.incGroup(ind_cur);
                        if (dual) {
                            temp_vtx_vtx.New().set(ind_test, ind_cur);
                            vtx_vtx.incGroup(ind_test);
                        }
                    }
                }
            }
    ...
    vtx_vtx.set();
    REPA(temp_vtx_vtx) {
        C VecI2 &vv = temp_vtx_vtx[i];
        vtx_vtx.addElm(vv.x, vv.y);
    }
}
/******************************************************************************/
void MeshBase::linkVtxVtxOnFace(Index &vtx_vtx) C {
    ...
    if (!vtx.dup()) {
        REPA(tri) {
            p = (_tri++)->c;
            REPD(j, 3)
            vtx_vtx.incGroup(*p++, 2);
        }
        REPA(quad) {
            p = (_quad++)->c;
            REPD(j, 4)
            vtx_vtx.incGroup(*p++, 3);
        }
    } else {
        REPA(tri) {
            p = (_tri++)->c;
            REPD(j, 3)
            vtx_vtx.incGroup(vtx.dup(*p++), 2);
        }
        REPA(quad) {
            p = (_quad++)->c;
            REPD(j, 4)
            vtx_vtx.incGroup(vtx.dup(*p++), 3);
        }
    }
    vtx_vtx.set();
    ...
    REPA(tri) {
        VecI f = *_tri++;
        f.remap(vtx.dup());
        vtx_vtx.addElm(f.c[0], f.c[1]);
        vtx_vtx.addElm(f.c[0], f.c[2]);
        vtx_vtx.addElm(f.c[1], f.c[2]);
        vtx_vtx.addElm(f.c[1], f.c[0]);
        vtx_vtx.addElm(f.c[2], f.c[0]);
        vtx_vtx.addElm(f.c[2], f.c[1]);
    }
    _quad = quad.ind();
    REPA(quad) {
        VecI4 f = *_quad++;
        ...
        vtx_vtx.addElm(f.c[0], f.c[1]);
        vtx_vtx.addElm(f.c[0], f.c[2]);
        vtx_vtx.addElm(f.c[0], f.c[3]);
        vtx_vtx.addElm(f.c[1], f.c[2]);
        vtx_vtx.addElm(f.c[1], f.c[3]);
        vtx_vtx.addElm(f.c[1], f.c[0]);
        vtx_vtx.addElm(f.c[2], f.c[3]);
        vtx_vtx.addElm(f.c[2], f.c[0]);
        vtx_vtx.addElm(f.c[2], f.c[1]);
        vtx_vtx.addElm(f.c[3], f.c[0]);
        vtx_vtx.addElm(f.c[3], f.c[1]);
        vtx_vtx.addElm(f.c[3], f.c[2]);
    }
...
void MeshBase::linkVtxVtxOnEdge(Index &vtx_vtx, Bool sort) C {
    ...
    FREPA(edge) {
        p = edge.ind(i).c;
        vtx_vtx.incGroup(p[0]);
        vtx_vtx.incGroup(p[1]);
    }
    vtx_vtx.set();
    FREPA(edge) {
        p = edge.ind(i).c;
        vtx_vtx.addElm(p[0], p[1]);
        vtx_vtx.addElm(p[1], p[0]);
    }
...
void MeshBase::linkVtxEdge(Index &vtx_edge, Bool sort) C {
    ...
    FREPA(edge) {
        p = edge.ind(i).c;
        vtx_edge.incGroup(p[0]);
        vtx_edge.incGroup(p[1]);
    }
    vtx_edge.set();
    FREPA(edge) {
        p = edge.ind(i).c;
        vtx_edge.addElm(p[0], i);
        vtx_edge.addElm(p[1], i);
    }
...
void MeshBase::linkVtxFace(Index &vtx_face) C {
    ...
    if (!vtx.dup()) {
        ...
        FREPA(tri) {
            p = (_tri++)->c;
            REPD(j, 3)
            vtx_face.addElm(vtx.dup(*p++), i);
        }
        _quad = quad.ind();
        FREPA(quad) {
            p = (_quad++)->c;
            REPD(j, 4)
            vtx_face.addElm(vtx.dup(*p++), i ^ SIGN_BIT);
        }
    }
}
void MeshBase::linkFaceFace(Index &face_face) C {
    ...
    REPAD(f, tri) // iterate all tris
    {
        VecI a = tri.ind(f);
        a.remap(vtx.dup());
        REPA(a) // iterate all tri vtxs
        {
            C IndexGroup &ig = vtx_face.group[a.c[i]];
            REPA(ig) // iterate all faces connected to the vtx
            {
                UInt test_face = ig[i];
                if (test_face != f)
                    face.include(test_face); // face different than triangle 'f'
            }
        }
        face_face.incGroup(f, face.elms());
        face.clear();
    }
    REPAD(f, quad) // iterate all quads
    {
        ...
        REPA(a) // iterate all quad vtxs
        {
            ...
            REPA(ig) // iterate all faces connected to the vtx
            {
                UInt test_face = ig[i];
                if (test_face != fs)
                    face.include(test_face); // face different than quad 'f'
            }
        }
        face_face.incGroup(tris() + f, face.elms());
        face.clear(); // add 'tris()' because we're processing quads and tris were listed first
    }
    ...
    REPAD(f, tri) // iterate all tris
    {
        ...
        IndexGroup &ig = face_face.group[f];
        REPA(face)
        ig.add(face[i]);
        face.clear();
    }
    REPAD(f, quad) // iterate all quads
    {
        ...
        IndexGroup &ig = face_face.group[tris() + f];
        REPA(face)
        ig.add(face[i]);
        face.clear(); // add 'tris()' because we're processing quads and tris were listed first
    }
    ...
    FREPD(step, 2) {
        FREPA(edge) {
            ...
            for (PixelWalkerMask walker(rects.coords(vtx.pos(p[0]).xy), rects.coords(vtx.pos(p[1]).xy), RectI(0, 0, rects.cells.x - 1, rects.cells.y - 1)); walker.active(); walker.step()) {
                ...
                if (!step)
                    rect_edge.incGroup(group);
                else
                    rect_edge.addElm(group, i);
            }
        }
        if (!step)
            rect_edge.set();
...
void MeshBase::getVtxNeighbors(Int vtx, MemPtr<Int> vtxs) C {
    vtxs.clear();
    if (InRange(vtx, T.vtx)) {
        Index vtx_vtx;
        linkVtxVtxOnFace(vtx_vtx);
        Memt<Bool> is;
        is.setNumZero(T.vtxs());
...
void MeshBase::getFaceNeighbors(Int face, MemPtr<Int> faces) C {
    ...
    if ((face & SIGN_BIT) ? InRange(face ^ SIGN_BIT, quad) : InRange(face, tri)) {
        ...
        is.setNumZero(T.vtxs());
        if (face & SIGN_BIT) {
            VecI4 f = quad.ind(face ^ SIGN_BIT);
            f.remap(vtx.dup());
            REPA(f)
            FillVtx(vtx_vtx, is, f.c[i]);
        } else {
            VecI f = tri.ind(face);
            f.remap(vtx.dup());
            REPA(f)
        ...
        REPA(tri) {
            VecI f = tri.ind(i);
            f.remap(vtx.dup());
            if (is[f.x] || is[f.y] || is[f.z])
                faces.add(i);
        }
        REPA(quad) {
            VecI4 f = quad.ind(i);
            f.remap(vtx.dup());
            if (is[f.x] || is[f.y] || is[f.z] || is[f.w])
                faces.add(i ^ SIGN_BIT);
        }
    }
}
/******************************************************************************/
struct EdgeAdj {
    ...
    EdgeAdj &set(Int p0, Int p1, Int f0, Int f1) {
        ind.set(p0, p1);
        adj_face.set(f0, f1);
        return T;
    }
...
struct Adj {
    ...
    void set(Int face, Int vtxi, Int face_extra_vtx) {
        T.face = face;
        T.vtxi = vtxi;
        T.face_extra_vtx = face_extra_vtx;
    }
};
      ...
      REPAD(f0, quad) // for each quad
      {
            ...
            if (f0a.c[f0vi] == -1) // if not yet connected to something
            {
                ...
                REPAD(vfi, ig) // for each face that edge first vertex belongs to
                {
                    ...
                    if (f1 & SIGN_BIT) // quad
                    {
                        ...
                        VecI4 f1i = quad.ind(f1 ^ SIGN_BIT);
                        f1i.remap(vtx.dup());
                        if (f1i.c[0] == f0v1 && f1i.c[1] == f0v0) {
                            if (f1a.c[0] == -1)
                ...
                if (adj.elms()) {
                    ...
                    {
                        Matrix m;
                        m.setPosDir(vtx.pos(f0v0), !(vtx.pos(f0v1) - vtx.pos(f0v0))); // construct matrix with pos on the edge first vertex and dir along the edge
                        ...
                        REPA(adj)                                                     // iterate adjacent faces
                        {
                            ...
                            if (a <= a0)
                                a += PI2; // use <= to include coplanar faces
                            if (a < angle) {
                                adj_i = i;
                                angle = a;
                            } // find face with smallest angle difference from 'a0'
                        }
                    }
                    Adj &a = adj[adj_i];
                    Int f1 = a.face;
                    Int f1vi = a.vtxi;
                    if (faces) {
                        tri.adjFace(f0).c[f0vi] = f1;
                        ((f1 & SIGN_BIT) ? quad.adjFace(f1 ^ SIGN_BIT).c[f1vi] : tri.adjFace(f1).c[f1vi]) = f0;
                    }
                    if (edges) {
                        tri.adjEdge(f0).c[f0vi] = _edge.elms();
                        ((f1 & SIGN_BIT) ? quad.adjEdge(f1 ^ SIGN_BIT).c[f1vi] : tri.adjEdge(f1).c[f1vi]) = _edge.elms();
                        _edge.New().set(f0v0, f0v1, f0, f1);
                    }
                    adj.clear();
                }
            }
        ...
        REPAD(f0, quad) // for each quad
        {
            ...
            if (f0a.c[f0vi] == -1) // if not yet connected to something
            {
                ...
                if (adj.elms()) {
                    ...
                    Int f1vi = a.vtxi;
                    if (faces) {
                        quad.adjFace(f0).c[f0vi] = f1;
                        ((f1 & SIGN_BIT) ? quad.adjFace(f1 ^ SIGN_BIT).c[f1vi] : tri.adjFace(f1).c[f1vi]) = f0 ^ SIGN_BIT;
                    }
                    if (edges) {
                        quad.adjEdge(f0).c[f0vi] = _edge.elms();
                        ((f1 & SIGN_BIT) ? quad.adjEdge(f1 ^ SIGN_BIT).c[f1vi] : tri.adjEdge(f1).c[f1vi]) = _edge.elms();
                        _edge.New().set(f0v0, f0v1, f0 ^ SIGN_BIT, f1);
                    }
                    adj.clear();
                }
            }
        ...
        if (edges) {
            ...
            FREPA(tri) {
                ...
                REPD(j, 3)
                if (p[j] < 0) // if any triangle edge didn't get linked to an edge, then it means it's empty (one-sided)
                {
                    Int *v = tri.ind(i).c;
                    p[j] = _edge.elms();
            ...
            FREPA(quad) {
                ...
                REPD(j, 4)
                if (p[j] < 0) // if any quad edge didn't get linked to an edge, then it means it's empty (one-sided)
                {
                    Int *v = quad.ind(i).c;
                    p[j] = _edge.elms();
