...
static void Weld(MeshBase &mshb, MESH_FLAG flag, Flt pos_eps, Flt nrm_cos, Flt remove_degenerate_faces_eps, Bool flat) {
    if (pos_eps >= 0) {
        if (flat)
            mshb.setVtxDup2D(flag, pos_eps, nrm_cos);
        else
            mshb.setVtxDup(flag, pos_eps, nrm_cos);
        ...
        if (vtxs == mshb.vtxs()) {
            ...
            Memb<VecI4> quad_tri;
            {
                ...
                FREPA(mshb.quad) {
                    VecI4 f = qud_ind[i], r = f;
                    r.remapAll(vtx_remap);
                    if (!(is[i] = r.allDifferent())) {
                        if (r.c[0] != r.c[1] && r.c[1] != r.c[3] && r.c[0] != r.c[3])
                ...
                temp.include(QUAD_ALL & mshb.flag());
            ...
            // tri
            {
                ...
                temp.tri._elms = tris + quad_tri.elms();
                temp.include(f);
                temp.copyTris(mshb, is);

                if (temp.tri.nrm() && !mshb.tri.nrm())
                    ZeroFastN(temp.tri.nrm(), tris);
                if (temp.tri.flag() && !mshb.tri.flag())
                    ZeroFastN(temp.tri.flag(), tris);
                if (temp.tri.id() && !mshb.tri.id())
                    ZeroFastN(temp.tri.id(), tris);

                REPA(quad_tri) {
                    VecI4 &v = quad_tri[i];
                    temp.tri.ind(tris + i) = v.xyz;
                    if (temp.tri.nrm())
                        temp.tri.nrm(tris + i) = (mshb.quad.nrm() ? mshb.quad.nrm(v.w) : 0);
                    if (temp.tri.flag())
                        temp.tri.flag(tris + i) = (mshb.quad.flag() ? mshb.quad.flag(v.w) : 0);
                    if (temp.tri.id())
                        temp.tri.id(tris + i) = (mshb.quad.id() ? mshb.quad.id(v.w) : -1);
                }
            ...
            // edge
            {
                is.setNum(mshb.edges());
                FREPA(mshb.edge) {
                    Int *p = edg_ind[i].c;
                    is[i] = (vtx_remap[p[0]] != vtx_remap[p[1]]);
                }
                temp.edge._elms = CountIs(is);
                temp.include(EDGE_ALL & mshb.flag());
                temp.copyEdges(mshb, is);
                IndRemap(vtx_remap, temp.edge.ind(), temp.edges());
            }

            // remove degenerate faces
            if (remove_degenerate_faces_eps >= 0)
                temp.removeDegenerateFaces(remove_degenerate_faces_eps);
...
MeshBase &MeshBase::weldVtx2D(MESH_FLAG flag, Flt pos_eps, Flt nrm_cos, Flt remove_degenerate_faces_eps) {
    Weld(T, flag, pos_eps, nrm_cos, remove_degenerate_faces_eps, true);
    return T;
}
MeshBase &MeshBase::weldVtx(MESH_FLAG flag, Flt pos_eps, Flt nrm_cos, Flt remove_degenerate_faces_eps) {
    Weld(T, flag, pos_eps, nrm_cos, remove_degenerate_faces_eps, false);
    return T;
}
/******************************************************************************/
    ...
    if (flag & (VTX_POS | VTX_NRM_TAN_BIN | VTX_HLP | VTX_TEX_ALL | VTX_COLOR | VTX_MATERIAL | VTX_SKIN | VTX_SIZE)) // if have anything to weld
    {
        ...
        REPA(vtx) {
            ...
            if (d != i) // duplicate
            {
                ...
                if (flag & VTX_NRM)
                    vtx.nrm(d) += weight * vtx.nrm(i);
                if (flag & VTX_TAN)
                    vtx.tan(d) += weight * vtx.tan(i);
                if (flag & VTX_BIN)
                    vtx.bin(d) += weight * vtx.bin(i);
                if (flag & VTX_HLP)
                    vtx.hlp(d) += weight * vtx.hlp(i);
                if (flag & VTX_TEX0)
                    vtx.tex0(d) += weight * vtx.tex0(i);
                if (flag & VTX_TEX1)
                    vtx.tex1(d) += weight * vtx.tex1(i);
                if (flag & VTX_TEX2)
                    vtx.tex2(d) += weight * vtx.tex2(i);
                if (flag & VTX_TEX3)
                    vtx.tex3(d) += weight * vtx.tex3(i);
                if (flag & VTX_SIZE)
                    vtx.size(d) += weight * vtx.size(i);
                if (flag & VTX_COLOR)
                    color[d] += weight * vtx.color(i);
            }
        }
        ...
        REPA(vtx) {
            ...
            if (d != i) // duplicate
            {
                ...
                if (flag & VTX_MATERIAL)
                    vtx.material(i) = vtx.material(d);
                if (flag & VTX_MATRIX)
                    vtx.matrix(i) = vtx.matrix(d);
                if (flag & VTX_BLEND)
                    vtx.blend(i) = vtx.blend(d);
                if (flag & VTX_NRM)
                    vtx.nrm(i) = vtx.nrm(d);
                if (flag & VTX_TAN)
                    vtx.tan(i) = vtx.tan(d);
                ...
                if (flag & VTX_HLP)
                    vtx.hlp(i) = vtx.hlp(d);
                if (flag & VTX_TEX0)
                    vtx.tex0(i) = vtx.tex0(d);
                if (flag & VTX_TEX1)
                    vtx.tex1(i) = vtx.tex1(d);
                if (flag & VTX_TEX2)
                    vtx.tex2(i) = vtx.tex2(d);
                if (flag & VTX_TEX3)
                    vtx.tex3(i) = vtx.tex3(d);
                if (flag & VTX_SIZE)
                    vtx.size(i) = vtx.size(d);
                if (flag & VTX_COLOR)
                    vtx.color(i) = vtx.color(d);
            }
        }
...
void MeshBase::copyVtx(Int i, MeshBase &dest, Int dest_i) C {
    if (InRange(i, vtx) && InRange(dest_i, dest.vtx)) {
        if (dest.vtx.pos())
            dest.vtx.pos(dest_i) = (vtx.pos() ? vtx.pos(i) : 0);
        if (dest.vtx.nrm())
            dest.vtx.nrm(dest_i) = (vtx.nrm() ? vtx.nrm(i) : 0);
        if (dest.vtx.tan())
            dest.vtx.tan(dest_i) = (vtx.tan() ? vtx.tan(i) : 0);
        if (dest.vtx.bin())
            dest.vtx.bin(dest_i) = (vtx.bin() ? vtx.bin(i) : 0);
        if (dest.vtx.hlp())
            dest.vtx.hlp(dest_i) = (vtx.hlp() ? vtx.hlp(i) : 0);
        if (dest.vtx.tex0())
            dest.vtx.tex0(dest_i) = (vtx.tex0() ? vtx.tex0(i) : 0);
        if (dest.vtx.tex1())
            dest.vtx.tex1(dest_i) = (vtx.tex1() ? vtx.tex1(i) : 0);
        if (dest.vtx.tex2())
            dest.vtx.tex2(dest_i) = (vtx.tex2() ? vtx.tex2(i) : 0);
        if (dest.vtx.tex3())
            dest.vtx.tex3(dest_i) = (vtx.tex3() ? vtx.tex3(i) : 0);
        if (dest.vtx.matrix())
            dest.vtx.matrix(dest_i) = (vtx.matrix() ? vtx.matrix(i) : 0);
        if (dest.vtx.blend())
            dest.vtx.blend(dest_i) = (vtx.blend() ? vtx.blend(i) : VecB4(255, 0, 0, 0)); // !! sum must be equal to 255 !!
        if (dest.vtx.size())
            dest.vtx.size(dest_i) = (vtx.size() ? vtx.size(i) : 0);
        if (dest.vtx.material())
            dest.vtx.material(dest_i) = (vtx.material() ? vtx.material(i) : 0);
        if (dest.vtx.color())
            dest.vtx.color(dest_i) = (vtx.color() ? vtx.color(i) : TRANSPARENT);
        if (dest.vtx.flag())
            dest.vtx.flag(dest_i) = (vtx.flag() ? vtx.flag(i) : 0);
        if (dest.vtx.dup())
            dest.vtx.dup(dest_i) = (vtx.dup() ? vtx.dup(i) : dest_i);
    }
}
/******************************************************************************/
struct WeldEdge {
...
MeshBase &MeshBase::weldEdge() {
    if (edge.ind()) {
        // copy edges
        Memt<Bool> edge_is;
        edge_is.setNum(edges());
        SetMem(edge_is.data(), 1, edge_is.elms());
        Memt<WeldEdge> w_edge;
        w_edge.setNum(edges());
        FREPA(edge) {
            w_edge[i].ind = edge.ind(i);
            if (edge.id())
                w_edge[i].id = edge.id(i);
            if (edge.nrm())
                w_edge[i].nrm = edge.nrm(i);
            if (edge.flag())
                w_edge[i].flag = edge.flag(i);
        ...
        if (edges != T.edges()) {
            Bool nrm = (edge.nrm() != null),
                 ...
                 id = (edge.id() != null);
            Int num = T.edges();
            exclude(EDGE_ALL | FACE_ADJ_EDGE);
            edge._elms = edges;
...
// TESSELATE
/******************************************************************************/
static INLINE Flt Crease(C Vec &n) {
    Flt l = n.length2();
    return l ? 1 / Sqrt(l) : 0;
} // this function will make the formula work as if 'n' is normalized and then final value scaled by original length, scale is performed to avoid cases where both normal vectors are similar and its cross product has very small length (direction could be incorrect due to precision issues)
struct TriHull {
    Vec P0, P1, P2,
        N0, N1, N2;
   ...
   void set(Vec &pos, Vec &nrm, Flt U, Flt V)
   {
    ...
    void set01(C Vec &NA, C Vec &NB) {
        Vec N = Cross(N0, NA);
        B210_3 = P0 * 3 + (Dot(P1 - P0, N) * Crease(N)) * N;
        N = Cross(N1, NB);
        B120_3 = P1 * 3 + (Dot(P0 - P1, N) * Crease(N)) * N;
    }
    void set12(C Vec &NA, C Vec &NB) {
        Vec N = Cross(N1, NA);
        B021_3 = P1 * 3 + (Dot(P2 - P1, N) * Crease(N)) * N;
        N = Cross(N2, NB);
        B012_3 = P2 * 3 + (Dot(P1 - P2, N) * Crease(N)) * N;
    }
    void set20(C Vec &NA, C Vec &NB) {
        Vec N = Cross(N2, NA);
        B102_3 = P2 * 3 + (Dot(P0 - P2, N) * Crease(N)) * N;
        N = Cross(N0, NB);
        B201_3 = P0 * 3 + (Dot(P2 - P0, N) * Crease(N)) * N;
    }
    void finalize() {
        ...
        Flt V01 = 2 * Dot(P1 - P0, N0 + N1) / Dot(P1 - P0, P1 - P0);
        N110_2 = N0 + N1 - V01 * (P1 - P0);
        N110_2.setLength(2);
        Flt V12 = 2 * Dot(P2 - P1, N1 + N2) / Dot(P2 - P1, P2 - P1);
        N011_2 = N1 + N2 - V12 * (P2 - P1);
        N011_2.setLength(2);
        Flt V20 = 2 * Dot(P0 - P2, N2 + N0) / Dot(P0 - P2, P0 - P2);
        N101_2 = N2 + N0 - V20 * (P0 - P2);
        N101_2.setLength(2);
    }
    void set(Vec &pos, Vec &nrm, Flt U, Flt V) {
        Flt W = 1 - U - V,
        ...
        pos = P0 * (WW * W) + P1 * (UU * U) + P2 * (VV * V) + B210_3 * (WW * U) + B120_3 * (W * UU) + B201_3 * (WW * V) + B021_3 * (UU * V) + B102_3 * (W * VV) + B012_3 * (U * VV) + B111_6 * (W * U * V);

        nrm = N0 * WW + N1 * UU + N2 * VV + N110_2 * (W * U) + N011_2 * (U * V) + N101_2 * (W * V);
        nrm.normalize();
    }
#endif
...
struct QuadHull {
    ...
    void set01(C Vec &NA, C Vec &NB) {
        Vec N = Cross(N0, NA);
        B01 = P0 + (Dot(P1 - P0, N) * Crease(N) / 3) * N;
        N = Cross(N1, NB);
        B10 = P1 + (Dot(P0 - P1, N) * Crease(N) / 3) * N;
    }
    void set12(C Vec &NA, C Vec &NB) {
        Vec N = Cross(N1, NA);
        B12 = P1 + (Dot(P2 - P1, N) * Crease(N) / 3) * N;
        N = Cross(N2, NB);
        B21 = P2 + (Dot(P1 - P2, N) * Crease(N) / 3) * N;
    }
    void set23(C Vec &NA, C Vec &NB) {
        Vec N = Cross(N2, NA);
        B23 = P2 + (Dot(P3 - P2, N) * Crease(N) / 3) * N;
        N = Cross(N3, NB);
        B32 = P3 + (Dot(P2 - P3, N) * Crease(N) / 3) * N;
    }
    void set30(C Vec &NA, C Vec &NB) {
        Vec N = Cross(N3, NA);
        B30 = P3 + (Dot(P0 - P3, N) * Crease(N) / 3) * N;
        N = Cross(N0, NB);
        B03 = P0 + (Dot(P3 - P0, N) * Crease(N) / 3) * N;
    }
    void finalize() {
        Vec Q = B01 + B10 + B12 + B21 + B23 + B32 + B30 + B03;

        Vec E0 = (2 * (B01 + B03 + Q) - (B21 + B23)) / 18, V0 = (4 * P0 + 2 * (P3 + P1) + P2) / 9;
        B02 = E0 + (E0 - V0) / 2;
        Vec E1 = (2 * (B12 + B10 + Q) - (B32 + B30)) / 18, V1 = (4 * P1 + 2 * (P0 + P2) + P3) / 9;
        B13 = E1 + (E1 - V1) / 2;
        Vec E2 = (2 * (B23 + B21 + Q) - (B03 + B01)) / 18, V2 = (4 * P2 + 2 * (P1 + P3) + P0) / 9;
        B20 = E2 + (E2 - V2) / 2;
        Vec E3 = (2 * (B30 + B32 + Q) - (B10 + B12)) / 18, V3 = (4 * P3 + 2 * (P2 + P0) + P1) / 9;
        B31 = E3 + (E3 - V3) / 2;
        ...
        N12 = N1 + N2 - V12 * (P2 - P1);
        N12.normalize();
        Flt V23 = 2 * (Dot(P3 - P2, N2 + N3) / Dot(P3 - P2, P3 - P2));
        N23 = N2 + N3 - V23 * (P3 - P2);
        N23.normalize();
        Flt V30 = 2 * (Dot(P0 - P3, N3 + N0) / Dot(P0 - P3, P0 - P3));
        N30 = N3 + N0 - V30 * (P0 - P3);
        N30.normalize();
        N0123 = 2 * (N01 + N12 + N23 + N30) + (N0 + N1 + N2 + N3);
        N0123.normalize();
    }
    void set(Vec &pos, Vec &nrm, Flt U, Flt V) {
        Flt BU0 = (1 - U) * (1 - U) * (1 - U),
        ...
        pos = BV0 * (BU0 * P0 + BU1 * B01 + BU2 * B10 + BU3 * P1) + BV1 * (BU0 * B03 + BU1 * B02 + BU2 * B13 + BU3 * B12) + BV2 * (BU0 * B30 + BU1 * B31 + BU2 * B20 + BU3 * B21) + BV3 * (BU0 * P3 + BU1 * B32 + BU2 * B23 + BU3 * P2);
        ...
        nrm = NV0 * (NU0 * N0 + NU1 * N01 + NU2 * N1) + NV1 * (NU0 * N30 + NU1 * N0123 + NU2 * N12) + NV2 * (NU0 * N3 + NU1 * N23 + NU2 * N2);
...
{
    if (face == -1)
        return true; // compare to -1 and not >=0 because it can have SIGN_BIT, if there's no adjacent face, then return true so we can use default version
    Int face_vtxs;
    VecI4 face_ind;
    ...
    {
        Int test_v1 = face_ind.c[i];
        if ((mesh.vtx.dup() ? mesh.vtx.dup(test_v1) : test_v1) == v1) // first check for v1, because we're checking the neighbor, so it should have vertexes in rotated order
        {
            Int test_v0 = face_ind.c[(i + 1) % face_vtxs];
            if ((mesh.vtx.dup() ? mesh.vtx.dup(test_v0) : test_v0) == v0) // now check for v0
            {
...
MeshBase &MeshBase::tesselate(Flt weld_pos_eps) {
    if (vtx.pos()) {
        ...
        Int vtxs = T.vtxs(),
            ...
            quads = T.quads();
        MeshBase temp;
        temp.create(vtxs + tris * 3 + quads * 5, 0, tris * 4, quads * 4, flag() & ~(VTX_DUP | EDGE_ALL | ADJ_ALL | FACE_NRM));
        ...
        // original vtxs
        if (pos_dst) {
            CopyN(pos_dst, vtx.pos(), vtxs);
            pos_dst += vtxs;
        }
        if (nrm_dst) {
            CopyN(nrm_dst, vtx.nrm(), vtxs);
            nrm_dst += vtxs;
        }
        if (tan_dst) {
            CopyN(tan_dst, vtx.tan(), vtxs);
            tan_dst += vtxs;
        }
        if (bin_dst) {
            CopyN(bin_dst, vtx.bin(), vtxs);
            bin_dst += vtxs;
        }
        if (hlp_dst) {
            CopyN(hlp_dst, vtx.hlp(), vtxs);
            hlp_dst += vtxs;
        }
        if (tex0_dst) {
            CopyN(tex0_dst, vtx.tex0(), vtxs);
            tex0_dst += vtxs;
        }
        if (tex1_dst) {
            CopyN(tex1_dst, vtx.tex1(), vtxs);
            tex1_dst += vtxs;
        }
        if (tex2_dst) {
            CopyN(tex2_dst, vtx.tex2(), vtxs);
            tex2_dst += vtxs;
        }
        if (tex3_dst) {
            CopyN(tex3_dst, vtx.tex3(), vtxs);
            tex3_dst += vtxs;
        }
        if (mtrx_dst) {
            CopyN(mtrx_dst, vtx.matrix(), vtxs);
            mtrx_dst += vtxs;
        }
        if (blnd_dst) {
            CopyN(blnd_dst, vtx.blend(), vtxs);
            blnd_dst += vtxs;
        }
        if (size_dst) {
            CopyN(size_dst, vtx.size(), vtxs);
            size_dst += vtxs;
        }
        if (mtrl_dst) {
            CopyN(mtrl_dst, vtx.material(), vtxs);
            mtrl_dst += vtxs;
        }
        if (col_dst) {
            CopyN(col_dst, vtx.color(), vtxs);
            col_dst += vtxs;
        }
        if (flag_dst) {
            CopyN(flag_dst, vtx.flag(), vtxs);
            flag_dst += vtxs;
        }
        ...
        FREP(tris) {
            ...
            if (nrm_src) {
                ...
                VecI af = tri_adjf[i], ind_dup = ind;
                ind_dup.remap(vtx_dup);
                Vec NA, NB;
                if (SameNrm(T, af.x, ind_dup.x, ind_dup.y, hull.N0, hull.N1, NA, NB))
                    hull.set01();
                else
                    hull.set01(NA, NB);
                if (SameNrm(T, af.y, ind_dup.y, ind_dup.z, hull.N1, hull.N2, NA, NB))
                    hull.set12();
                else
                    hull.set12(NA, NB);
                if (SameNrm(T, af.z, ind_dup.z, ind_dup.x, hull.N2, hull.N0, NA, NB))
                    hull.set20();
                else
                    hull.set20(NA, NB);
            ...
            if (mtrx_dst && blnd_dst) {
                ...
                REP(4)
                skin.New().set(m0.c[i], b0.c[i]);
                REP(4)
                skin.New().set(m1.c[i], b1.c[i]);
                SetSkin(skin, *mtrx_dst++, *blnd_dst++, null);
                skin.clear();

                REP(4)
                skin.New().set(m1.c[i], b1.c[i]);
                REP(4)
                skin.New().set(m2.c[i], b2.c[i]);
                SetSkin(skin, *mtrx_dst++, *blnd_dst++, null);
                skin.clear();
        ...
        FREP(quads) {
            ...
            if (nrm_src) {
                ...
                VecI4 af = qud_adjf[i], ind_dup = ind;
                ind_dup.remap(vtx_dup);
                Vec NA, NB;
                if (SameNrm(T, af.x, ind_dup.x, ind_dup.y, hull.N0, hull.N1, NA, NB))
                    hull.set01();
                ...
                if (SameNrm(T, af.y, ind_dup.y, ind_dup.z, hull.N1, hull.N2, NA, NB))
                    ...
                    hull.set12(NA, NB);
                if (SameNrm(T, af.z, ind_dup.z, ind_dup.w, hull.N2, hull.N3, NA, NB))
                    hull.set23();
                else
                    hull.set23(NA, NB);
                if (SameNrm(T, af.w, ind_dup.w, ind_dup.x, hull.N3, hull.N0, NA, NB))
                    hull.set30();
                else
                    hull.set30(NA, NB);
...
MeshBase &MeshBase::tesselate(C CMemPtr<Int> &vtx_sel, Flt weld_pos_eps) {
    Memt<Bool> vtx_is;
    CreateIs(vtx_is, vtx_sel, vtxs());
    return tesselate(vtx_is, weld_pos_eps);
}
MeshBase &MeshBase::tesselate(C CMemPtr<Bool> &vtx_sel, Flt weld_pos_eps) {
    if (C Vec *vtx_pos = vtx.pos()) {
        ...
        FREPA(tri) {
            ...
            if (tess_vtxs < 2)
                tris.New().set(ofs, ofs + 1, ofs + 2);
            else // set as original
            {
                TriHull hull;
                if (vtx_nrm) {
                ...
                if (vs[v0])        // if 'v0' wants
                {
                    ...
                    if (vs[v1]) // and 'v1' wants
                    {
                        ...
                        if (vtx_nrm)
                            hull.set(vtxs[ofs + 3], hull.uv(v0));
        ...
        FREPA(quad) {
            ...
            vtxs.New().from(T, ind.c[i]); // copy original vtxs
            if (tess_vtxs < 2)
                quads.New().set(ofs, ofs + 1, ofs + 2, ofs + 3);
            else // set as original
            {
                QuadHull hull;
                if (vtx_nrm) {
                ...
                if (vs[v0])        // if 'v0' wants
                {
                    ...
                    // if(tess_vtxs==2)
                    {
                        ...
                        if (vs[v2]) // and 'v2' wants
                        {
                            ...
                            if (vtx_nrm)
                                hull.set(vtxs[ofs + 4], hull.uvCenter());
...
MeshBase &MeshBase::subdivide() {
    ...
    if (vtx.dup()) {
        dup_vtx = vtx.dup();
        vtx._dup = null;
        dup_edge_adj = edge.adjFace();
        edge._adj_face = null;
        VecI *dup_tri_adj_edg = tri.adjEdge();
        tri._adj_edge = null;
        VecI4 *dup_qud_adj_edg = quad.adjEdge();
        quad._adj_edge = null;
        ...
        REPA(edge) {
            ...
            REPD(f, 2) {
                ...
                {
                    if (face & SIGN_BIT) {
                        Int *fe = quad.adjEdge(face ^ SIGN_BIT).c;
                        REPD(e, 4)
                        if (fe[e] == i) {
                            dup_edge_edg[i] = dup_qud_adj_edg[face ^ SIGN_BIT].c[e];
                            goto found;
                        }
                    } else {
                        Int *fe = tri.adjEdge(face).c;
                        REPD(e, 3)
                        if (fe[e] == i) {
                            dup_edge_edg[i] = dup_tri_adj_edg[face].c[e];
                            goto found;
                        }
                    }
                }
    ...
    if (tri_as_quad)
        FREP(tris) {
            ...
            p2 = p[2];
            if (pos_dst)
                *pos_dst++ = Avg(pos_src[p0], pos_src[p1], pos_src[p2]);
            if (nrm_dst)
                *nrm_dst++ = !(nrm_src[p0] + nrm_src[p1] + nrm_src[p2]);
            if (tan_dst)
                *tan_dst++ = !(tan_src[p0] + tan_src[p1] + tan_src[p2]);
            if (bin_dst)
                *bin_dst++ = !(bin_src[p0] + bin_src[p1] + bin_src[p2]);
            if (hlp_dst)
                *hlp_dst++ = Avg(hlp_src[p0], hlp_src[p1], hlp_src[p2]);
            if (tex0_dst)
                *tex0_dst++ = Avg(tex0_src[p0], tex0_src[p1], tex0_src[p2]);
            if (tex1_dst)
                *tex1_dst++ = Avg(tex1_src[p0], tex1_src[p1], tex1_src[p2]);
            if (tex2_dst)
                *tex2_dst++ = Avg(tex2_src[p0], tex2_src[p1], tex2_src[p2]);
            if (tex3_dst)
                *tex3_dst++ = Avg(tex3_src[p0], tex3_src[p1], tex3_src[p2]);
            if (size_dst)
                *size_dst++ = Avg(size_src[p0], size_src[p1], size_src[p2]);
            if (mtrl_dst)
                *mtrl_dst++ = AvgI(mtrl_src[p0], mtrl_src[p1], mtrl_src[p2]);
            if (col_dst)
                *col_dst++ = Avg(col_src[p0], col_src[p1], col_src[p2]);
            if (flag_dst)
                *flag_dst++ = (flag_src[p0] | flag_src[p1] | flag_src[p2]);
            if (mtrx_dst && blnd_dst) {
                REP(3) {
                    Int point = p[i];
                    REP(4)
                    skin.New().set(mtrx_src[point].c[i], blnd_src[point].c[i]);
                }
                SetSkin(skin, *mtrx_dst++, *blnd_dst++, null);
                skin.clear();
            } else {
                if (mtrx_dst)
                    *mtrx_dst++ = mtrx_src[p0]; // just copy from first point, because we need correct data and blending with sum of 255
                if (blnd_dst)
                    *blnd_dst++ = blnd_src[p0]; // just copy from first point, because we need correct data and blending with sum of 255
            }
        }
    else
        FREP(tris) {
    ...
    FREP(quads) {
        ...
        p3 = p[3];
        if (pos_dst)
            *pos_dst++ = Avg(pos_src[p0], pos_src[p1], pos_src[p2], pos_src[p3]);
        if (nrm_dst)
            *nrm_dst++ = !(nrm_src[p0] + nrm_src[p1] + nrm_src[p2] + nrm_src[p3]);
        if (tan_dst)
            *tan_dst++ = !(tan_src[p0] + tan_src[p1] + tan_src[p2] + tan_src[p3]);
        if (bin_dst)
            *bin_dst++ = !(bin_src[p0] + bin_src[p1] + bin_src[p2] + bin_src[p3]);
        if (hlp_dst)
            *hlp_dst++ = Avg(hlp_src[p0], hlp_src[p1], hlp_src[p2], hlp_src[p3]);
        if (tex0_dst)
            *tex0_dst++ = Avg(tex0_src[p0], tex0_src[p1], tex0_src[p2], tex0_src[p3]);
        if (tex1_dst)
            *tex1_dst++ = Avg(tex1_src[p0], tex1_src[p1], tex1_src[p2], tex1_src[p3]);
        if (tex2_dst)
            *tex2_dst++ = Avg(tex2_src[p0], tex2_src[p1], tex2_src[p2], tex2_src[p3]);
        if (tex3_dst)
            *tex3_dst++ = Avg(tex3_src[p0], tex3_src[p1], tex3_src[p2], tex3_src[p3]);
        if (size_dst)
            *size_dst++ = Avg(size_src[p0], size_src[p1], size_src[p2], size_src[p3]);
        if (mtrl_dst)
            *mtrl_dst++ = AvgI(mtrl_src[p0], mtrl_src[p1], mtrl_src[p2], mtrl_src[p3]);
        if (col_dst)
            *col_dst++ = Avg(col_src[p0], col_src[p1], col_src[p2], col_src[p3]);
        if (flag_dst)
            *flag_dst++ = (flag_src[p0] | flag_src[p1] | flag_src[p2] | flag_src[p3]);
        if (mtrx_dst && blnd_dst) {
            REP(4) {
                Int point = p[i];
                REP(4)
    ...
    FREP(edges) {
        ...
        p1 = p[1];
        if (pos_dst)
            pos_dst[i] = Avg(pos_src[p0], pos_src[p1]);
        if (nrm_dst)
            nrm_dst[i] = !(nrm_src[p0] + nrm_src[p1]);
        if (tan_dst)
            tan_dst[i] = !(tan_src[p0] + tan_src[p1]);
        if (bin_dst)
            bin_dst[i] = !(bin_src[p0] + bin_src[p1]);
        if (hlp_dst)
            hlp_dst[i] = Avg(hlp_src[p0], hlp_src[p1]);
        if (tex0_dst)
            tex0_dst[i] = Avg(tex0_src[p0], tex0_src[p1]);
        if (tex1_dst)
            tex1_dst[i] = Avg(tex1_src[p0], tex1_src[p1]);
        if (tex2_dst)
            tex2_dst[i] = Avg(tex2_src[p0], tex2_src[p1]);
        if (tex3_dst)
            tex3_dst[i] = Avg(tex3_src[p0], tex3_src[p1]);
        if (size_dst)
            size_dst[i] = Avg(size_src[p0], size_src[p1]);
        if (mtrl_dst)
            mtrl_dst[i] = AvgI(mtrl_src[p0], mtrl_src[p1]);
        if (col_dst)
            col_dst[i] = Avg(col_src[p0], col_src[p1]);
        if (flag_dst)
            flag_dst[i] = (flag_src[p0] | flag_src[p1]);
        if (mtrx_dst && blnd_dst) {
            REP(2) {
                Int point = p[i];
                REP(4)
    ...
    // reposition original points
    {
        ...
        FREP(vtxs) {
            Int n = vtx_edg_num[i];
            if (n && vtx_fac_num[i] == n)
                pos_src[i] = ((vtx_fac_avg[i] + 2 * vtx_edg_avg[i]) / n + (n - 3) * pos_src[i]) / n;
            else if (n = vtx_edg_num_b[i])
                pos_src[i] = Avg(vtx_edg_avg_b[i] / n, pos_src[i]);
        }
    }
    ...
    VecI4 *qud_adj_edg = quad.adjEdge();
    FREP(quads) {
        p = qud_src[i].c;
...
MeshBase &MeshBase::subdivideEdge(Bool freeze_z, C CMemPtr<Bool> &is) {
    ...
    if (!is || ElmIs(is, i)) {
        ...
        if (temp.vtx.nrm())
            temp.vtx.nrm(vtxs + j) = !(vtx.nrm(p0) + vtx.nrm(p1));
        if (temp.vtx.tan())
            temp.vtx.tan(vtxs + j) = !(vtx.tan(p0) + vtx.tan(p1));
        if (temp.vtx.bin())
            temp.vtx.bin(vtxs + j) = !(vtx.bin(p0) + vtx.bin(p1));
        if (temp.vtx.hlp())
            temp.vtx.hlp(vtxs + j) = Avg(vtx.hlp(p0), vtx.hlp(p1));
        if (temp.vtx.tex0())
            temp.vtx.tex0(vtxs + j) = Avg(vtx.tex0(p0), vtx.tex0(p1));
        if (temp.vtx.tex1())
            temp.vtx.tex1(vtxs + j) = Avg(vtx.tex1(p0), vtx.tex1(p1));
        if (temp.vtx.tex2())
            temp.vtx.tex2(vtxs + j) = Avg(vtx.tex2(p0), vtx.tex2(p1));
        if (temp.vtx.tex3())
            temp.vtx.tex3(vtxs + j) = Avg(vtx.tex3(p0), vtx.tex3(p1));
        if (temp.vtx.size())
            temp.vtx.size(vtxs + j) = Avg(vtx.size(p0), vtx.size(p1));
        if (temp.vtx.material())
            temp.vtx.material(vtxs + j) = AvgI(vtx.material(p0), vtx.material(p1));
        if (temp.vtx.color())
            temp.vtx.color(vtxs + j) = Avg(vtx.color(p0), vtx.color(p1));
        if (temp.vtx.flag())
            temp.vtx.flag(vtxs + j) = (vtx.flag(p0) | vtx.flag(p1));
        if (temp.vtx.matrix() && temp.vtx.blend()) {
            REP(2) {
                Int point = p[i];
                REP(4)
                skin.New().set(vtx.matrix(point).c[i], vtx.blend(point).c[i]);
            }
            SetSkin(skin, temp.vtx.matrix(vtxs + j), temp.vtx.blend(vtxs + j), null);
            skin.clear();
        } else {
            if (temp.vtx.matrix())
                temp.vtx.matrix(vtxs + j) = vtx.matrix(p0); // just copy from first point, because we need correct data and blending with sum of 255
            if (temp.vtx.blend())
                temp.vtx.blend(vtxs + j) = vtx.blend(p0); // just copy from first point, because we need correct data and blending with sum of 255
        }
        ...
        if (temp.edge.flag())
            temp.edge.flag(edges + j) = edge.flag(i);
        if (temp.edge.id())
            temp.edge.id(edges + j) = edge.id(i);
        j++;
    }

    // move original points
    Memt<Int> vtx_num_mem;
    Int *vtx_num = vtx_num_mem.setNumZero(vtxs).data();
    ...
    if (!is || ElmIs(is, i)) {
        ...
        if (edge.flag() && (edge.flag(i) & ETQ_LR) == ETQ_LR) {
            ...
            vtx_num[p0]++;
            temp.vtx.pos(p0) += temp.vtx.pos(vtxs + j);
            vtx_num[p1]++;
            temp.vtx.pos(p1) += temp.vtx.pos(vtxs + j);
        }
        j++;
    }
    FREP(vtxs) {
        if (vtx_num[i] >= 2) {
            ...
            if (freeze_z && vtx_freeze[i])
                temp.vtx.pos(i).z = vtx.pos(i).z;
        } else {
...
MeshBase &MeshBase::boneRemap(C CMemPtrN<BoneType, 256> &old_to_new) {
    if (VtxBone *matrix = vtx.matrix()) {
        VecB4 *blend = vtx.blend();
        REP(vtxs())
        BoneRemap(matrix[i], old_to_new, blend ? &blend[i] : null);
    }
    return T;
}
...
void MeshBase ::setUsedBones(MemPtrN<Bool, 256> bones) C {
    bones.clear();
    includeUsedBones(bones);
}
void MeshRender::setUsedBones(MemPtrN<Bool, 256> bones) C {
    bones.clear();
    includeUsedBones(bones);
}
/******************************************************************************/
