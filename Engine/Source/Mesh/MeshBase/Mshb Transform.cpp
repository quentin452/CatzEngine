...
MeshBase &MeshBase::transform(C Matrix3 &matrix, Bool scale_normals) {
    ...
    n = vtxs();
    Transform(vtx.pos(), matrix, n);
    Transform(vtx.hlp(), matrix, n);
    Transform(vtx.nrm(), matrix_nrm, n);
    Normalize(vtx.nrm(), n);
    Transform(vtx.tan(), matrix_nrm, n);
    Normalize(vtx.tan(), n);
    Transform(vtx.bin(), matrix_nrm, n);
    Normalize(vtx.bin(), n);

    n = edges();
    Transform(edge.nrm(), matrix_nrm, n);
    Normalize(edge.nrm(), n);
    n = tris();
    Transform(tri.nrm(), matrix_nrm, n);
    Normalize(tri.nrm(), n);
    n = quads();
    Transform(quad.nrm(), matrix_nrm, n);
    Normalize(quad.nrm(), n);
        ...
        if (blend && mtrx)
            REPA(vtx) {
                ...
                C Matrix &m0 = (InRange(mtrx->c[0], matrixes) ? matrixes[mtrx->c[0]] : matrixes[0]),
                         ...
                         &m3 = (InRange(mtrx->c[3], matrixes) ? matrixes[mtrx->c[3]] : matrixes[0]);
                mtrx++;
                if (pos) {
                    v = *pos;
                    *pos = b.x * (v * m0) + b.y * (v * m1) + b.z * (v * m2) + b.w * (v * m3);
                    pos++;
                }
                if (hlp) {
                    v = *hlp;
                    *hlp = b.x * (v * m0) + b.y * (v * m1) + b.z * (v * m2) + b.w * (v * m3);
                    hlp++;
                }
                if (nrm) {
                    v = *nrm;
                    *nrm = b.x * (v * m0.orn()) + b.y * (v * m1.orn()) + b.z * (v * m2.orn()) + b.w * (v * m3.orn());
                    nrm->normalize();
                    nrm++;
                }
                if (tan) {
                    v = *tan;
                    *tan = b.x * (v * m0.orn()) + b.y * (v * m1.orn()) + b.z * (v * m2.orn()) + b.w * (v * m3.orn());
                    tan->normalize();
                    tan++;
                }
                if (bin) {
                    v = *bin;
                    *bin = b.x * (v * m0.orn()) + b.y * (v * m1.orn()) + b.z * (v * m2.orn()) + b.w * (v * m3.orn());
                    bin->normalize();
                    bin++;
                }
            }
...
MeshBase &MeshBase::mirrorX() {
    ...
    if (vtx.tan() && !vtx.bin())
        setTanBin(); // setBinormals(); // binormals are needed if tangents are present
    return T;
}
...
MeshBase &MeshBase::reverse(C CMemPtr<Int> &faces) {
    if (faces.elms()) {
        ...
        CreateFaceIs(tri_is, quad_is, faces, tris(), quads());
        MeshBase temp;
        splitFaces(temp, null, tri_is, quad_is);
        if (temp.is()) {
            T += temp.reverse();
