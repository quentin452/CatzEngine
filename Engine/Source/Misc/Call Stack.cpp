...
Bool GetCallStack(Str &stack) {
    stack.clear();
#if WINDOWS_OLD
    StackWalker::StackWalkerEx sw;
    return sw.check(stack);
#elif WEB
    ...
    if (size > 0) {
        Memt<Char8> temp;
        temp.setNum(size + 64);                                                                                                   // +64 for safety because of different line numbers than before
...
Bool GetCallStackFast(Str &stack) // !! this function is not thread-safe !!
{
    stack.clear();
#if WINDOWS_OLD
    static StackWalker::StackWalkerEx *SW;
    if (!SW) {
        New(SW);
        SW->fast = true;
    }
