...
static ULong GetBits(CChar8 *text) // sample: "0,1-3"
{
    ...
    CalcValue val;
    for (; text;) {
        text = TextValue(text, val);
        if (!val.type)
            break;
        Int a = val.asInt();
        if (text && *text == '-') // range
        {
            text = TextValue(text + 1, val);
            if (!val.type)
                break; // skip '-'
            Int b = val.asInt();
            for (Int i = a; i <= b; i++)
...
CPU::CPU() {
    ...
    _threads = sys_info.dwNumberOfProcessors;
#endif
    ...
    _threads = sysconf(_SC_NPROCESSORS_CONF);
#else
    size_t len = SIZE(ncpus);
    sysctl((int[2]){CTL_HW, HW_NCPU}, 2, &_threads, &len, null, 0);
#endif
#endif
    ...
    UInt ids = Min(CPUInfo[0], 0x80000004);
    for (UInt i = 0x80000002; i <= ids; i++) {
        __cpuid(CPUInfo, i);
        if (i == 0x80000002)
            CopyFast(string, CPUInfo, SIZE(CPUInfo));
        else if (i == 0x80000003)
            CopyFast(string + 16, CPUInfo, SIZE(CPUInfo));
        else if (i == 0x80000004)
            CopyFast(string + 32, CPUInfo, SIZE(CPUInfo));
    }
    _name = _SkipWhiteChars(string);
    if (LINUX)
        _name.removeOuterWhiteChars(); // remove outer white chars which may appear on Linux
#elif ANDROID
    Char8 data[65536];
    ...
    */
    ...
    if (UnixReadFile("/proc/cpuinfo", data, SIZE(data))) {
        // LogN(data);
        CChar8 *t;
        if (t = TextPos(data, "Hardware", false, WHOLE_WORD_STRICT))
            t += 8;
    ...
    UnixReadFile("/sys/devices/system/cpu/present", data, SIZE(data));
    ULong cpu_present = GetBits(data); // sample output: "0-3", "0,1-2" (no spaces, values separated with commas, ranges or single values, ranges inclusive)
    UnixReadFile("/sys/devices/system/cpu/possible", data, SIZE(data));
    ULong cpu_possible = GetBits(data);
    ULong mask = (cpu_present & cpu_possible);
    _threads = BitOn(mask);
