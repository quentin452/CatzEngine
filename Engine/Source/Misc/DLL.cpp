...
#if MEMORY == 1
    if (_dll_mem) {
        MemoryFreeLibrary(_dll_mem);
        _dll_mem = null;
    }
#elif MEMORY == 2
    if (_dll_mem) {
        UnloadDLL((LOAD_DLL_INFO *)_dll_mem);
        _dll_mem = null;
    }
#endif
    if (_dll_file) {
        FreeLibrary((HMODULE)_dll_file);
        _dll_file = null;
    }
    return T;
}
...
#elif MEMORY == 2
        LOAD_DLL_INFO *dll = Alloc<LOAD_DLL_INFO>();
        if (LoadDLLFromMemory(data, size, 0, dll) == ELoadDLLResult_OK)
            _dll_mem = dll;
        else
            Free(dll);
#endif
...
#if MEMORY == 1
    if (_dll_mem)
        return MemoryGetProcAddress(_dll_mem, name);
#elif MEMORY == 2
    if (_dll_mem)
        return myGetProcAddress_LoadDLLInfo((LOAD_DLL_INFO *)_dll_mem, name);
#endif
    if (_dll_file)
        return GetProcAddress((HMODULE)_dll_file, name);
    return null;
}
#elif WINDOWS_NEW
void DLL::delForce() {}
