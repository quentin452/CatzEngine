...
UInt MP4::Track::frameRangeSize(Int start, Int end) C {
    if (frame_size)
        return (end - start) * frame_size;
    MAX(start, 0);
    MIN(end, frame_sizes.elms());
    UInt size = 0;
    for (; start < end; start++)
        size += frame_sizes[start];
    return size;
}
...
Long MP4::Track::frameOffset(Int i) C {
    if (stsc.elms()) {
        Int chunk, chunk_frame;
        frameChunk(i, chunk, chunk_frame);
    ...
    if (C Part *part = findPart(i)) {
        ...
        if (part->frame_sizes.elms())
            for (; --i >= 0;)
                pos += part->frame_sizes[i];
        else
            pos += part->frame_size * i;
        return pos;
    }
    return 0;
}
UInt MP4::Track::frameSize(Int i) C {
    if (InRange(i, frame_sizes))
        return frame_sizes[i];
    if (C Part *part = findPart(i))
        return part->frameSize(i);
    return frame_size;
}
...
static Bool ReadHeader(File &f, UInt &type, Long &size) {
    ...
    switch (header.size) {
    case 0x00000000:
        size = f.left() + SIZE(header);
        break; // BigEndian 0 = to the end of the file (including size of the header)
    case 0x01000000:
        size = f.getBELong();
        break; // BigEndian 1 = 64-bit size
    default:
...
Bool MP4::read(File &f, Long max_pos, Track *track, Long moof_pos) {
    ...
    for (;;) {
        Long pos = f.pos();
        if (pos >= max_pos)
            break; // reached the end for this parent
        UInt type;
        Long size;
        if (!ReadHeader(f, type, size))
            goto error;
...
#endif
        {
            if (!read(f, end, (type == TRAK) ? &tracks.New() : track, (type == MOOF) ? pos : moof_pos))
                goto error;
        } break;
        ...
        case TKHD:
            if (track) {
                ...
                case 0: {
                    ...
                    f.getBEUInt();
                    track->duration = f.getBEUInt();
                    if (track->duration == UINT_MAX)
                        track->duration = -1;
                } break;
                        ...
                        case MP4A: {
                            ...
                            if (type == ESDS) {
                                ...
                                if (ver == 0) {
                                    ...
                                    decoder_config.setNum(decoder_config_len);
                                    if (!decoder_config.loadRawData(f)) {
        ...
        case CO64:
            if (track) {
                ...
                if (ver == 0) {
                    UInt flags = f.getBEUInt24();
                    Mems<Long> &chunk_offset = track->chunk_offset.setNum(f.getBEUInt());
                    chunk_offset.loadRawData(f);
                    REPA(chunk_offset)
                    SwapEndian(chunk_offset[i]);
                }
            }
            break;
        ...
        case STSC:
            if (track) {
                ...
                if (ver == 0) {
                    ...
                    stsc.loadRawData(f);
                    ASSERT(SIZE(Track::STSC) == 12);
                    REPA(stsc) {
                        Track::STSC &s = stsc[i];
                        SwapEndian(s.first_chunk);
                        SwapEndian(s.frames_per_chunk);
                        SwapEndian(s.frame_desc_index);
                    }
                }
            }
            break;
        ...
        case TFHD: {
            ...
            case 0: {
                ...
                if (flags & 0x000002)
                    UInt frame_description_index = f.getBEUInt();
                tfhd_default_frame_duration = ((flags & 0x000008) ? f.getBEUInt() : 0);
                tfhd_default_frame_size = ((flags & 0x000010) ? f.getBEUInt() : 0);
                if (flags & 0x000020)
                    UInt default_frame_flags = f.getBEUInt();
            } break;
            }
        } break;

        case TRUN:
            if (track) // 'track' should be available because this chunk is processed in the same level and after 'TFHD' which finds the track for us
            {
                Byte ver = f.getByte();
                UInt flags = f.getBEUInt24();
                switch (ver) {
                case 0: {
                    if (UInt frame_count = f.getBEUInt()) {
                        if (flags & 0x000001)
                            trun_data_offset = tfhd_base_data_offset + f.getBEInt(); // if not present, then we have to reuse the last value
                        if (flags & 0x000004)
                            UInt first_frame_flags = f.getBEUInt();
                        ...
                        if (flags & 0x000100)
                            part.frame_durations.setNum(frame_count);
                        if (flags & 0x000200)
                            part.frame_sizes.setNum(frame_count);
                        FREP(frame_count) {
                            UInt frame_duration = tfhd_default_frame_duration;
                            if (flags & 0x000100)
                                part.frame_durations[i] = frame_duration = f.getBEUInt();
                            UInt frame_size = tfhd_default_frame_size;
                            if (flags & 0x000200)
                                part.frame_sizes[i] = frame_size = f.getBEUInt();
                            if (flags & 0x000400)
                                UInt frame_flags = f.getBEUInt();
                            if (flags & 0x000800)
                                UInt frame_composition_time_offset = f.getBEUInt();
