...
void Object::align(Bool custom, ALIGN_TYPE x, ALIGN_TYPE y, ALIGN_TYPE z) {
    FlagSet(_flag, OBJ_OVR_ALIGN, custom);
    if (!custom)
        updateBaseSelf();
    else
        _align = (x | (y << 2) | (z << 4));
...
Matrix Object::matrixFinal() C {
    ...
    if (alignX() != ALIGN_NONE || alignY() != ALIGN_NONE || alignZ() != ALIGN_NONE) {
        ...
        if (!found) {
            if (_mesh && _mesh->is())
                box = _mesh->ext * matrix;
            else
                box = Ball(0.5f) * matrix;
        }
    }
    ...
    switch (alignX()) {
    default:
        O.pos.x = matrix.pos.x;
        break;
    case ALIGN_MIN:
        O.pos.x = matrix.pos.x * 2 - box.max.x;
        break;
    case ALIGN_CENTER:
        O.pos.x = matrix.pos.x * 2 - box.centerX();
        break;
    case ALIGN_MAX:
        O.pos.x = matrix.pos.x * 2 - box.min.x;
        break;
    }
    switch (alignY()) {
    default:
        O.pos.y = matrix.pos.y;
        break;
    case ALIGN_MIN:
        O.pos.y = matrix.pos.y * 2 - box.max.y;
        break;
    case ALIGN_CENTER:
        O.pos.y = matrix.pos.y * 2 - box.centerY();
        break;
    case ALIGN_MAX:
        O.pos.y = matrix.pos.y * 2 - box.min.y;
        break;
    }
    switch (alignZ()) {
    default:
        O.pos.z = matrix.pos.z;
        break;
    case ALIGN_MIN:
        O.pos.z = matrix.pos.z * 2 - box.max.z;
        break;
    case ALIGN_CENTER:
        O.pos.z = matrix.pos.z * 2 - box.centerZ();
        break;
    case ALIGN_MAX:
        O.pos.z = matrix.pos.z * 2 - box.min.z;
        break;
    }
    O.orn() = matrix.orn();
    return O;
...
Object &Object::updateBaseSelf() {
    ...
    if (!(_flag & OBJ_OVR_MESH))
        _mesh = (_base ? _base->_mesh : MeshPtr());
    if (!(_flag & OBJ_OVR_MESH_VARIATION))
        _mesh_variation_id = (_base ? _base->_mesh_variation_id : 0);
    if (!(_flag & OBJ_OVR_PHYS))
        _phys = (_base ? _base->_phys : PhysBodyPtr());
    if (!(_flag & OBJ_OVR_ALIGN))
        _align = (_base ? _base->_align : (ALIGN_MAX << 2));
    if (!(_flag & OBJ_OVR_ACCESS))
        _access = (_base ? _base->_access : OBJ_ACCESS_TERRAIN);
    if (!(_flag & OBJ_OVR_PATH))
        _path = (_base ? _base->_path : OBJ_PATH_CREATE);
    if (!(_flag & OBJ_OVR_CONST))
        FlagSet(_flag, OBJ_CONST, _base ? _base->constant() : false);
...
void Object::type(Bool custom, C UID &type) {
    FlagSet(_flag, OBJ_OVR_TYPE, custom);
    if (!custom)
        updateBaseSelf();
    else
        T._type = type;
}
void Object::access(Bool custom, OBJ_ACCESS access) {
    if (!custom || InRange(access, OBJ_ACCESS_NUM)) {
        FlagSet(_flag, OBJ_OVR_ACCESS, custom);
        if (!custom)
            updateBaseSelf();
        else
            T._access = access;
...
void Object::path(Bool custom, OBJ_PATH path) {
    if (!custom || InRange(path, OBJ_PATH_NUM)) {
        FlagSet(_flag, OBJ_OVR_PATH, custom);
        if (!custom)
            updateBaseSelf();
        else
            T._path = path;
...
void Object::mesh(Bool custom, C MeshPtr &mesh) {
    FlagSet(_flag, OBJ_OVR_MESH, custom);
    if (!custom)
        updateBaseSelf();
    else
        T._mesh = mesh;
}
void Object::meshVariationID(Bool custom, UInt id) {
    FlagSet(_flag, OBJ_OVR_MESH_VARIATION, custom);
    if (!custom)
        updateBaseSelf();
    else
        T._mesh_variation_id = id;
}
void Object::phys(Bool custom, C PhysBodyPtr &phys) {
    FlagSet(_flag, OBJ_OVR_PHYS, custom);
    if (!custom)
        updateBaseSelf();
    else
        T._phys = phys;
}
void Object::scale(Bool custom, C Vec &scale) {
    FlagSet(_flag, OBJ_OVR_SCALE, custom);
    if (!custom)
        updateBaseSelf();
    else
        matrix.normalize(scale);
}
void Object::constant(Bool custom, Bool on) {
    FlagSet(_flag, OBJ_OVR_CONST, custom);
    if (!custom)
        updateBaseSelf();
    else
        FlagSet(_flag, OBJ_CONST, on);
}
...
static void SetOldAccess(Object &op, Byte b) {
    switch (b) {
    default:
        op.access(true, OBJ_ACCESS_CUSTOM);
        if (op.base() && op.base()->constant())
            op.constant(true, false);
        break; // old OBJ_ACCESS_DYNAMIC
    case 1:
        op.access(true, OBJ_ACCESS_CUSTOM);
        if (!op.base() || !op.base()->constant())
            op.constant(true, true);
        break; // old OBJ_ACCESS_CONST
    case 2:
        op.access(true, OBJ_ACCESS_TERRAIN);
        break; // old OBJ_ACCESS_EMBED
    case 3:
        op.access(true, OBJ_ACCESS_GRASS);
        break; // old OBJ_ACCESS_GRASS
    case 4:
        op.access(true, OBJ_ACCESS_OVERLAY);
        break; // old OBJ_ACCESS_OVERLAY
    }
}
static void AdjustOldAccess(Object &op, C Str &type_name) {
    if (!op.base() && (type_name.is() && type_name != "OBJ_STATIC" || op.params.elms() || op.sub_objs.elms()))
        op.access(true, OBJ_ACCESS_CUSTOM);
}
Bool Object::loadData(File &f, CChar *path) {
    ...
    case 7: {
        ...
        if (_flag & OBJ_OVR_TYPE)
            f >> _type;
        if (_flag & OBJ_OVR_MESH)
            _mesh.require(f.getAssetID(), path);
        if (_flag & OBJ_OVR_MESH_VARIATION)
            f >> _mesh_variation_id;
        if (_flag & OBJ_OVR_PHYS)
            _phys.require(f.getAssetID(), path);
        if (_flag & OBJ_OVR_ACCESS)
            f >> _access;
        if (_flag & OBJ_OVR_PATH)
            f >> _path;
    ...
    case 6: {
        ...
        if (_flag & OBJ_OVR_MESH)
            _mesh.require(f._getStr(), path);
        if (_flag & (1 << 2)) {
            f._getStr();
            FlagDisable(_flag, 1 << 2);
        }
        _mesh_variation_id = 0; // old OBJ_OVR_MATERIAL
        if (_flag & OBJ_OVR_PHYS)
            _phys.require(f._getStr(), path);
        if (_flag & OBJ_OVR_ACCESS)
            f >> _access;
        if (_flag & OBJ_OVR_PATH)
            f >> _path;
    ...
    case 4: {
        ...
        _mesh_variation_id = 0; // old OBJ_OVR_MATERIAL
        if (_flag & (1 << 7)) {
            skeleton = Skeletons.name(Skeletons(f._getStr(), path));
            FlagDisable(_flag, 1 << 7);
        } // old OBJ_OVR_SKELETON
        if (_flag & OBJ_OVR_PHYS)
            _phys.require(f._getStr(), path);
        if (_flag & OBJ_OVR_ACCESS)
            SetOldAccess(T, f.getByte());
    ...
    case 3: {
        ...
        if (_flag & OBJ_OVR_TYPE) {
            type_name = f._getStr16();
            _type = ObjType.findID(type_name);
        }
        if (_flag & OBJ_OVR_MESH)
            _mesh.require(f._getStr16(), path);
        if (_flag & (1 << 2)) {
            f._getStr16();
            FlagDisable(_flag, 1 << 2);
        }
        _mesh_variation_id = 0; // old OBJ_OVR_MATERIAL
        if (_flag & (1 << 7)) {
            skeleton = Skeletons.name(Skeletons(f._getStr16(), path));
            FlagDisable(_flag, 1 << 7);
        } // old OBJ_OVR_SKELETON
        if (_flag & OBJ_OVR_PHYS)
            _phys.require(f._getStr16(), path);
        if (_flag & OBJ_OVR_ACCESS)
            SetOldAccess(T, f.getByte());
        if (_flag & OBJ_OVR_ALIGN)
            f >> _align;
    ...
    case 2: {
        ...
        if (_flag & OBJ_OVR_TYPE) {
            type_name = f._getStr8();
            _type = ObjType.findID(type_name);
        }
        if (_flag & OBJ_OVR_MESH)
            _mesh.require(f._getStr8(), path);
        if (_flag & (1 << 2)) {
            f._getStr8();
            FlagDisable(_flag, 1 << 2);
        }
    ...
    case 1: {
        ...
        _flag = f.getByte();
        f >> matrix;
        if (!matrix.x.y && !matrix.x.z && !matrix.y.y && !matrix.y.z && !matrix.z.y && !matrix.z.z)
            matrix.orn().setScale(matrix.x.length()); // adjust matrix scale because of bug in the past, matrix was set to X(scale,0,0), Y(scale,0,0), Z(scale,0,0)
        _base.require(f._getStr8(), path);
        if (_flag & OBJ_OVR_TYPE) {
            type_name = f._getStr8();
            _type = ObjType.findID(type_name);
    ...
    case 0: {
        ...
        if (_flag & (1 << 7)) {
            ...
            SetOldAccess(T, 0); // old OBJ_CONST
        REP(f.getInt())
        params.New().loadOld(f);
        REP(f.getInt())
        sub_objs.New().loadData(f);
        if (f.ok()) {
            updateBaseSelf();
            return true;
        }
...
void Object::operator=(C Str &name) {
    if (!load(name))
        Exit(MLT(S + "Can't load Object \"" + name + "\"",
