...
Int Param::asInt() C {
    ...
    case PARAM_VEC2:
        return Round(value.v2.x);
    case PARAM_VEC:
        return Round(value.v.x);
    case PARAM_VEC4:
        return Round(value.v4.x);
    case PARAM_COLOR:
        return value.c.r;
    case PARAM_STR:
        return TextInt(value.s);
    case PARAM_ENUM:
        return asEnum();
...
Flt Param::asFlt() C {
    ...
    case PARAM_VEC4:
        return value.v4.x;
    case PARAM_COLOR:
        return value.c.r / 255.0f;
    case PARAM_STR:
        return TextFlt(value.s);
    case PARAM_ENUM:
        return asEnum();
        // case PARAM_ID      : return
        // case PARAM_ID_ARRAY: return
...
Vec2 Param::asVec2() C {
    ...
    case PARAM_VEC4:
        return value.v4.xy;
    case PARAM_COLOR:
        return Vec2(value.c.r, value.c.g) / 255.0f;
    case PARAM_STR:
        return TextVec2(value.s);
...
Vec4 Param::asVec4() C {
    ...
    case PARAM_FLT:
        return value.f;
    case PARAM_VEC2:
        return Vec4(value.v2, 0, 0);
    case PARAM_VEC:
        return Vec4(value.v, 0);
    case PARAM_VEC4:
        return value.v4;
    case PARAM_COLOR:
        return value.c;
    case PARAM_STR:
        return TextVec4(value.s);
        // case PARAM_ENUM    : return asEnum();
        // case PARAM_ID      : return
        // case PARAM_ID_ARRAY: return
    default:
...
Color Param::asColor() C {
    switch (type) {
    case PARAM_BOOL:
        return value.b ? WHITE : TRANSPARENT;
    case PARAM_INT:
        return Color(value.i, value.i, value.i, 255);
    case PARAM_FLT:
        return Color(FltToByte(value.f), FltToByte(value.f), FltToByte(value.f));
    case PARAM_VEC2:
        return Color(FltToByte(value.v2.x), FltToByte(value.v2.y), 0);
    case PARAM_VEC:
        return value.v;
    case PARAM_VEC4:
        return value.v4;
...
UID Param::asID(Int id_index) C {
    ...
    case PARAM_ID:
        ...
        break;
    case PARAM_ID_ARRAY:
        if (InRange(id_index, arrayIDs()))
            return ((UID *)value.s())[id_index];
        break;
    }
    return UIDZero;
}
UID Param::asIDMod(Int id_index) C {
    ...
    case PARAM_ID_ARRAY:
        if (Int ids = arrayIDs())
            return ((UID *)value.s())[Mod(id_index, ids)];
        break;
    }
    return UIDZero;
}
Str Param::asText(Int precision) C {
    ...
    default:
        return S;
    case PARAM_BOOL:
        return TextInt(value.b);
    case PARAM_INT:
        return TextInt(value.i);
    case PARAM_FLT:
        return TextReal(value.f, precision);
    case PARAM_VEC2:
        return value.v2.asText(precision);
    case PARAM_VEC:
        return value.v.asText(precision);
    case PARAM_VEC4:
...
Int Param::asEnum() C {
    ...
    case PARAM_ENUM:
        if (enum_type) {
            Int i = enum_type->find(value.id);
            if (i >= 0)
                return i;
            i = enum_type->find(value.s);
            if (i >= 0)
                return i;
        }
        break; // try ID first, then try Name
    }
    return -1;
}
/******************************************************************************/
...
Param &Param::setValue(C Str &s) {
    ...
    case PARAM_ID:
        value.id.fromText(s);
        break;
    case PARAM_ID_ARRAY: { // this should handle the case when 's' is 'this.s'
        Memt<Str> lines;
        Split(lines, s, '\n');
        Memt<UID> ids;
        FREPA(lines) {
            UID temp;
            if (temp.fromText(lines[i]))
                Swap(ids.New(), temp);
        }
    ...
    case PARAM_ENUM: {
        ...
        value.id.zero();
        if (enum_type) {
            Int e = enum_type->find(s);
            if (InRange(e, *enum_type)) {
...
Param &Param::setValue(C Param &src) {
    if (type == src.type)
        ...
        case PARAM_ID_ARRAY:
            switch (src.type) {
            case PARAM_ID:
                setAsIDArray(src.value.id, false);
                break; // disallow changing type because we're in 'setValue' which can change only the value
                       // case PARAM_ID_ARRAY: this case is already handled in "type==src.type"
            case PARAM_STR:
                setValue(src.value.s);
                break;
            default:
                value.s.clear();
                break;
        ...
        case PARAM_STR:
            ...
            case PARAM_ID_ARRAY:
                value.s = src.asText();
                break; // 'asText' will already return an empty string if there are no ID's
            case PARAM_ENUM:
                value.s = src.value.s;
                break;
        ...
        case PARAM_ENUM:
            ...
            case PARAM_ENUM:
                value.s = src.value.s;
                value.id = src.value.id;
                if (enum_type) {
                    Int i = enum_type->find(value.id);
                    if (InRange(i, *enum_type))
                        value.s = (*enum_type)[i].name;
                    else {
                        i = enum_type->find(value.s);
                        if (InRange(i, *enum_type))
    ...
    if (InRange(type, PARAM_NUM))
        if (T.type != type || (type == PARAM_ENUM && T.enum_type != enum_type)) {
            ...
            case PARAM_ID_ARRAY:
                switch (T.type) {
                case PARAM_ID:
                    setAsIDArray(value.id, false);
                    break; // disallow changing type because we're in 'setType' which assumes that 'type' will be set precisely
                case PARAM_STR:
                    T.type = type;
                    setValue(value.s);
                    break; // change the type before calling 'setValue'
                default:
                    value.s.clear();
                    break;
                }
...
Param &Param::setAsIDArray(C CMemPtr<UID> &ids, Bool allow_PARAM_ID_type) {
    ...
    if (allow_PARAM_ID_type && ids.elms() == 1) {
        ...
        if (ids.elms()) {
            ...
            value.s.reserve(chars);
            ids.copyTo((UID *)value.s());
...
Param &Param::includeAsIDArray(C CMemPtr<UID> &ids, Bool allow_PARAM_ID_type) {
    if (Int id_num = IDs()) // if have any existing ID's
    {
        ...
        if (type == PARAM_ID)
            ...
            temp.copyFrom((UID *)value.s());
        FREPA(ids)
        temp.include(ids[i]); // include in order
...
Param &Param::excludeAsIDArray(C CMemPtr<UID> &ids, Bool allow_PARAM_ID_type) {
    if (Int id_num = IDs()) // if have any existing ID's
    {
        ...
        if (type == PARAM_ID)
            ...
            temp.copyFrom((UID *)value.s());
        FREPA(ids)
        temp.exclude(ids[i], true); // exclude
        return setAsIDArray(temp, allow_PARAM_ID_type);
    }
...
Bool Param::load(File &f, CChar *path) {
    ...
    case 5: {
        f >> type;
        if (type == PARAM_ENUM)
            enum_type = Enums(f.getAssetID(), path);
        else
            enum_type = null;
        f.getStr(name);
        switch (type) {
        ...
        case PARAM_STR:
            ...
            break;
        case PARAM_ID_ARRAY:
            if (UInt ids = f.decUIntV()) {
                Int chars = ids * (SIZE(UID) / SIZE(Char));
                value.s.reserve(chars);
                f.getN((UID *)value.s(), ids);
                value.s._d[value.s._length = chars] = '\0'; // this is a string so it needs to have a NUL terminate char
            }
            break;
        }
    ...
    case 3: {
        f >> type;
        if (type == PARAM_ENUM)
            enum_type = Enums(f._getStr(), path);
        else
            enum_type = null;
        f._getStr(name);
        switch (type) {
        ...
        case PARAM_ID:
            ...
            if (!value.id.valid())
                type = PARAM_ID_ARRAY;
            break; // upgrade old file format's empty ID to PARAM_ID_ARRAY with no elements
        case PARAM_ENUM:
            f >> value.id;
            f._getStr(value.s);
            if (enum_type) {
    ...
    case 1: {
        f >> type;
        if (type == PARAM_ENUM)
            enum_type = Enums(f._getStr16(), path);
        else
            enum_type = null;
        name = f._getStr16();
        switch (type) {
    ...
    case 0: {
        f >> type;
        if (type == PARAM_ENUM)
            enum_type = Enums(f._getStr8(), path);
        else
            enum_type = null;
        name = f._getStr8();
...
Int CompareValue(C Param &p0, C Param &p1) {
    if (Int i = Compare(p0.type, p1.type))
        return i;    // type
    switch (p0.type) // value
    {
    case PARAM_BOOL:
        return Compare(p0.value.b, p1.value.b);
    case PARAM_INT:
        return Compare(p0.value.i, p1.value.i);
    case PARAM_FLT:
        return Compare(p0.value.f, p1.value.f);
    case PARAM_VEC2:
        return Compare(p0.value.v2, p1.value.v2);
    case PARAM_VEC:
        return Compare(p0.value.v, p1.value.v);
    case PARAM_VEC4:
        return Compare(p0.value.v4, p1.value.v4);
    case PARAM_COLOR:
        return Compare(p0.value.c, p1.value.c);
    case PARAM_STR:
        return Compare(p0.value.s, p1.value.s);
    case PARAM_ID:
        return Compare(p0.value.id, p1.value.id);
    case PARAM_ID_ARRAY: {
    ...
    case PARAM_ENUM: {
        if (p0.enum_type && p1.enum_type) {
            if (p0.enum_type != p1.enum_type)
                return Compare(p0.enum_type->name, p1.enum_type->name);
        } else if (p0.enum_type)
            return +1;
        else if (p1.enum_type)
            return -1;
    }
        return Compare(p0.value.s, p1.value.s);
    }
...
Int Compare(C Param &p0, C Param &p1) {
    if (Int i = Compare(p0.name, p1.name))
        return i; // name
