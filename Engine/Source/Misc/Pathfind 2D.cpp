...
namespace EE {
/******************************************************************************/
void PathFind::Pixel::create(Int x, Int y) {
    xy.set(x, y);
    flag = 0;
    iteration = 0;
}
...
PathFind &PathFind::pixelWalkable(Int x, Int y, Bool on) {
    if (validPos(x, y))
        FlagSet(pixel(x, y).flag, PFP_WALKABLE, on);
    return T;
}
PathFind &PathFind::pixelTarget(Int x, Int y, Bool on) {
    if (validPos(x, y))
        FlagSet(pixel(x, y).flag, PFP_TARGET, on);
    return T;
}
PathFind &PathFind::pixelStart(Int x, Int y, Bool on) {
    if (validPos(x, y))
        FlagSet(pixel(x, y).flag, PFP_START, on);
    return T;
}
PathFind &PathFind::border(Int min_x, Int min_y, Int max_x, Int max_y) {
    _border.min.x = Max(0, min_x);
    _border.max.x = Min(_size.x, max_x);
    _border.min.y = Max(0, min_y);
    _border.max.y = Min(_size.y, max_y);
    return T;
}
/******************************************************************************/
static struct MOVE {
...
Bool PathFind::find(C VecI2 *start, C VecI2 *target, MemPtr<VecI2> path, Int max_steps, Bool diagonal, Bool reversed) {
    PROFILE_START("CatzEngine::PathFind::find(C VecI2 *start, C VecI2 *target, MemPtr<VecI2> path, Int max_steps, Bool diagonal, Bool reversed)")
    ...
    if (!start || validPos(*start)) // if valid start
    {
        ...
        if (start && onTarget(*start, target)) {
            PROFILE_STOP("CatzEngine::PathFind::find(C VecI2 *start, C VecI2 *target, MemPtr<VecI2> path, Int max_steps, Bool diagonal, Bool reversed)")
        ...
        for (Int step = 0;; step++) {
            REPA(_active) // order is important
            {
                ...
                {
                    C MOVE &move = Move[m];
                    VecI2 v = p->xy + move.dir;
                    if (validPos(v)) // target position
                    {
                        ...
                        if (new_length < to.length) {
                            if (move.dir.x && move.dir.y)             // diagonal move
                                if (pixel(p->xy).flag & PFP_WALKABLE) // test only if we're starting from 'walkable' pixel
                                    if (!(pixel(p->xy.x, v.y).flag & PFP_WALKABLE) || !(pixel(v.x, p->xy.y).flag & PFP_WALKABLE))
                                        continue;
            ...
            if (found) // found path
            {
                if (path) // build path
                {
                    if (!(found->flag & PFP_WALKABLE))
                        found = found->src; // if the last is not walkable then remove it
                    if (found)
                        for (;;) {
                            if (!start)
                                path.add(found->xy); // this          includes the starting position in the 'path'
                            Pixel *src = found->src;
                            if (!src)
                                break;
                            if (start)
                                path.add(found->xy); // this does not include  the starting position in the 'path'
                            found = src;
                        }
                    if (!reversed)
                        path.reverseOrder(); // 'path' is reversed by default when it's created, but if the user doesn't want that, then we need to reverse it
                }
                return true;
            }
            if (!_active.elms()) {
...
void PathFind::getWalkableNeighbors(C VecI2 &pos, MemPtr<VecI2> pixels, Bool diagonal) {
    PROFILE_START("CatzEngine::PathFind::getWalkableNeighbors(C VecI2 &pos, MemPtr<VecI2> pixels, Bool diagonal)")
    pixels.clear();
    if (pixelWalkable(pos.x, pos.y)) {
        const Int moves = (diagonal ? 8 : 4);
        step();
        pixels.add(pos);
        pixel(pos).iteration = _iteration;
        FREPAD(processed, pixels) {
            VecI2 pos = pixels[processed]; // don't use reference because 'pixels' may get modified later
            REP(moves) {
                VecI2 next = pos + Move[i].dir;
                if (validPos(next)) {
                    Pixel &pix = pixel(next);
                    if (FlagOn(pix.flag, PFP_WALKABLE) && pix.iteration != _iteration) {
                        pixels.add(next);
                        pix.iteration = _iteration;
                    }
                }
    ...
    PROFILE_STOP("CatzEngine::PathFind::getWalkableNeighbors(C VecI2 &pos, MemPtr<VecI2> pixels, Bool diagonal)")
}
/******************************************************************************/
} // namespace EE
/******************************************************************************/
