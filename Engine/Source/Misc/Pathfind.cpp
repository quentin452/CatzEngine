...
/******************************************************************************/
ASSERT(PM_OBSTACLE == RC_NULL_AREA && PM_BLOCKED == RC_WALKABLE_AREA);
/******************************************************************************
...
struct RecastHeightfield {
    ...
    void drawVoxels() {
        ...
        REPD(x, w) {
            ...
            for (; s; s = s->next)
                drawVoxel(fx, orig[1] + s->smin * ch, fz, fx + cs, orig[1] + s->smax * ch, fz + cs);
        }
    }
...
struct RecastPolyMeshDetail {
    ...
    ~RecastPolyMeshDetail() {
        rcFreePolyMeshDetail(_);
        _ = null;
    }
    RecastPolyMeshDetail() {
        _ = rcAllocPolyMeshDetail();
        if (!_)
            Exit("Out of memory");
    }
...
void RecastCompactHeightfield::operator=(C RecastCompactHeightfield &src) {
    if (this != &src) {
        del();
        Copy(T, src);
        cells = (rcCompactCell *)rcAlloc(SIZE(rcCompactCell) * width * height, RC_ALLOC_PERM);
        CopyN(cells, src.cells, width * height);
        spans = (rcCompactSpan *)rcAlloc(SIZE(rcCompactSpan) * spanCount, RC_ALLOC_PERM);
        CopyN(spans, src.spans, spanCount);
        dist = (unsigned short *)rcAlloc(SIZE(unsigned short) * spanCount, RC_ALLOC_PERM);
        CopyN(dist, src.dist, spanCount);
        areas = (unsigned char *)rcAlloc(SIZE(unsigned char) * spanCount, RC_ALLOC_PERM);
        CopyN(areas, src.areas, spanCount);
    }
}
/******************************************************************************/
...
void RecastCompactHeightfield::clean() // this removes all spans of RC_NULL_AREA
{
    ...
    Memt<Int> old_to_new;
    old_to_new.setNum(spanCount);
    Int cleaned = 0;
    FREP(spanCount) {
        if (areas[i] == RC_NULL_AREA)
            old_to_new[i] = -1;
        else
            old_to_new[i] = cleaned++;
    }
    ...
    if (cleaned != spanCount) {
        // set Span to XY
        Memt<VecI2> span_xy;
        span_xy.setNum(spanCount);
        REPD(y, height)
        REPD(x, width) {
            rcCompactCell &cell = cells[x + y * width];
            REP(cell.count)
            span_xy[cell.index + i].set(x, y);
        }
            ...
            if (n >= 0) {
                ...
                REPD(dir, 4) {
                    ...
                    if (connection != RC_NOT_CONNECTED) {
                        ...
                        if (InRange(nx, width) && InRange(ny, height)) {
                            Int valid = 0;
                            ...
                            FREP(neighb_cell.count) {
                                ...
                                if (old_to_new[span_index] >= 0) {
                                    ...
                                    {
                                        if (InRange(valid, RC_NOT_CONNECTED))
                                            new_connection = valid;
                                        break;
                                    }
                                    valid++;
                                }
...
Bool RecastCompactHeightfield::load(File &f) {
    ...
    case 0: {
        width = f.decUIntV();
        height = f.decUIntV();
        spanCount = f.decUIntV();
        walkableHeight = f.decUIntV();
        walkableClimb = f.decUIntV();
        borderSize = f.decUIntV();
        maxDistance = f.decUIntV();
        maxRegions = f.decUIntV();
        f >> bmin >> bmax >> cs >> ch;
        cells = (rcCompactCell *)rcAlloc(SIZE(*cells) * width * height, RC_ALLOC_PERM);
        f.getN(cells, width * height);
        spans = (rcCompactSpan *)rcAlloc(SIZE(*spans) * spanCount, RC_ALLOC_PERM);
        f.getN(spans, spanCount);
        areas = (unsigned char *)rcAlloc(SIZE(*areas) * spanCount, RC_ALLOC_PERM);
        f.getN(areas, spanCount);
        if (f.getBool()) {
            dist = (unsigned short *)rcAlloc(SIZE(*dist) * spanCount, RC_ALLOC_PERM);
            f.getN(dist, spanCount);
        }
        if (f.ok())
            return true;
    } break;
...
static void MarkConvexArea(rcContext &ctx, Vec *point, Int points, Flt ctrl_r, Flt ctrl_h, PATH_MESH_TYPE type, rcCompactHeightfield &chf) {
    if (point && points >= 3) {
        Vec min, max;
        MinMax(point, points, min, max);
        Memt<Vec2> poly;
        CreateConvex2Dxz(poly, point, points);
        if (poly.elms() >= 3) {
            ...
            REP(elms)
            vec[i].set(vec2[i].x, 0, vec2[i].y); // it's important to go from the end to don't overwrite existing elements
...
static Bool Build(RecastCompactHeightfield &chf, Shape *shape, Int shapes, Flt ctrl_r, Flt ctrl_h, Flt max_climb, C VecI2 &xy, Ptr user, Mems<Byte> &data) {
    {
        ...
        REP(shapes) {
            ...
            case SHAPE_POINT: {
                Box b = s.point;
                b.extendX(ctrl_r).extendZ(ctrl_r).extendY(cfg.ch).min.y -= ctrl_h;
                rcMarkBoxArea(&ctx, b.min.c, b.max.c, PM_BLOCKED, chf);
            } break;
            case SHAPE_BOX: {
                Box b = s.box;
                b.extendX(ctrl_r).extendZ(ctrl_r).min.y -= ctrl_h;
                rcMarkBoxArea(&ctx, b.min.c, b.max.c, PM_BLOCKED, chf);
            } break;
            case SHAPE_BALL: {
                Flt h = s.ball.r * 2;
                rcMarkCylinderArea(&ctx, (s.ball.pos - Vec(0, h * 0.5f + ctrl_h, 0)).c, s.ball.r + ctrl_r + cfg.cs * 0.50f, h + ctrl_h, PM_BLOCKED, chf);
            } break;
            case SHAPE_CAPSULE: {
                Flt h = s.capsule.h;
                rcMarkCylinderArea(&ctx, (s.capsule.pos - Vec(0, h * 0.5f + ctrl_h, 0)).c, s.capsule.r + ctrl_r + cfg.cs * 0.50f, h + ctrl_h, PM_BLOCKED, chf);
            } break;
            case SHAPE_TUBE: {
                Flt h = s.tube.h;
                rcMarkCylinderArea(&ctx, (s.tube.pos - Vec(0, h * 0.5f + ctrl_h, 0)).c, s.tube.r + ctrl_r + cfg.cs * 0.50f, h + ctrl_h, PM_BLOCKED, chf);
            } break;
            case SHAPE_OBOX: {
                Vec points[8];
                s.obox.toCorners(points);
                MarkConvexArea(ctx, points, Elms(points), ctrl_r + cfg.cs * 0.25f, ctrl_h, PM_BLOCKED, chf);
            } break;
            case SHAPE_PYRAMID: {
                Vec points[5];
                s.pyramid.toCorners(points);
                MarkConvexArea(ctx, points, Elms(points), ctrl_r + cfg.cs * 0.25f, ctrl_h, PM_BLOCKED, chf);
            } break;
            case SHAPE_EDGE: {
                ...
                MinMax(s.edge.p[0].y, s.edge.p[1].y, points[0].y, points[1].y);
                points[0].y -= cfg.ch;
                points[1].y += cfg.ch; // extend Y by cell height
        ...
        if (pmesh->nverts > 0x10000)
            goto error; // Recast uses U16 indexes
        ...
        default:
            ...
            break; // PM_OBSTACLE
        case PM_GROUND:
            pmesh->flags[i] = PMF_WALK;
            break;
        case PM_WATER:
            pmesh->flags[i] = PMF_SWIM;
            break;
        }
        ...
        if (dtCreateNavMeshData(&params, &navData, &navDataSize)) {
            data.setNum(navDataSize).copyFrom(navData);
            dtFree(navData);
            return true;
        }
    }
...
Box _PathMesh::box() {
    ...
    } else if (getChf()) // try accessing 'chf'
    {
...
void _PathMesh::link(PathWorld *world) {
    if (T.world != world) // if different
    {
        ...
        if (world)
            world->_set(this, xy, false); // link to new world, it will handle setting 'T.world' on success
    }
}
void _PathMesh::del() {
    link(null);
...
Bool _PathMesh::getChf() {
    ...
    if (chf_compressed.elms()) // if we have 'chf_compressed'
    {
        ...
        if (Decompress(compressed, decompressed, true))                              // decompress 'compressed' into 'decompressed'
        {
            ...
            decompressed.pos(0);
            return chf.load(decompressed); // load 'chf' from 'decompressed'
        }
    }
    return false;
}
...
static void SetChfCompressed(C RecastCompactHeightfield &chf, Mems<Byte> &chf_compressed) {
    File decompressed;
    chf.save(decompressed.writeMem()); // save 'chf' into 'decompressed'
    File compressed;
    decompressed.pos(0);
    if (!Compress(decompressed, compressed.writeMem(), CompressType, CompressLevel, false))
        Exit("SetChfCompressed"); // compress 'decompressed' into 'compressed'
    chf_compressed.setNum(compressed.size());
    compressed.pos(0);
    compressed.get(chf_compressed.data(), chf_compressed.elms()); // store 'compressed' into 'chf_compressed'
}
void _PathMesh::preSave() {
    if (is() && !chf_compressed.elms())
        SetChfCompressed(chf, chf_compressed);
}
Bool _PathMesh::save(File &f) C {
    ...
    if (data.saveRaw(f)) {
        if (is()) {
            ...
            if (!chf_compressed->elms()) {
                chf_compressed = &chf_compressed_temp;
                SetChfCompressed(chf, *chf_compressed);
            } // compress into temp so we won't keep it in the memory afterwards
            if (!chf_compressed->saveRaw(f))
                return false;
        }
        return f.ok();
    }
...
Bool _PathMesh::load(File &f) {
    ...
    case 0:
        if (data._loadRaw(f)) {
            if (is()) {
                if (!chf_compressed._loadRaw(f))
                    goto error;
                if (chf_compressed.elms()) // if we have 'chf_compressed'
                {
                    File compressed(chf_compressed.data(), chf_compressed.elms()), decompressed; // read from 'chf_compressed'
                    if (!_OldDecompress(compressed, decompressed, true))
                        goto error;       // decompress 'compressed' into 'decompressed'
                    chf_compressed.del(); // no longer needed so release it
                    decompressed.pos(0);
                    if (!chf.load(decompressed))
                        goto error; // load 'chf' from 'decompressed'
                }
            }
            if (dtMeshHeader *header = (dtMeshHeader *)data.data()) {
                header->userId = (UIntPtr)this; // 'userId' points to memory address of '_PathMesh'
...
PathSettings &PathSettings::areaSize(Flt size) {
    _area_size = Max(size, EPS);
    return T;
}
PathSettings &PathSettings::ctrlRadius(Flt r) {
    _ctrl_r = Max(r, 0);
    return T;
}
PathSettings &PathSettings::ctrlHeight(Flt h) {
    _ctrl_h = Max(h, 0);
    return T;
}
PathSettings &PathSettings::maxClimb(Flt climb) {
    _max_climb = Max(climb, 0);
    return T;
}
PathSettings &PathSettings::maxSlope(Flt slope) {
    _max_slope = Mid(slope, 0.0f, PI_2);
    return T;
}
PathSettings &PathSettings::cellSize(Flt size) {
    _cell_size = Max(size, 0.001f);
    return T;
}
PathSettings &PathSettings::cellHeight(Flt h) {
    _cell_h = Max(h, 0.001f);
    return T;
}

Bool PathSettings::operator!=(C PathSettings &path) C { return !(T == path); }
Bool PathSettings::operator==(C PathSettings &path) C {
    return Equal(_area_size, path._area_size) && Equal(_ctrl_r, path._ctrl_r) && Equal(_ctrl_h, path._ctrl_h) && Equal(_max_climb, path._max_climb) && Equal(_max_slope, path._max_slope) && Equal(_cell_size, path._cell_size) && Equal(_cell_h, path._cell_h);
}
...
Shape PathObstacle::shape() {
    if (_world && _shape && _world->_obstacles.contains(_shape))
        return *_shape;
...
Bool PathMesh::create(MeshBase &mesh, C VecI2 &area_xy, C PathSettings &settings) {
    ...
    if (mesh.quad.elms()) {
        ...
        Int t = 0;
        FREPA(mesh.tri) {
            tri_ind[t] = mesh.tri.ind(i);
            tri_type[t] = (mesh.tri.flag() ? mesh.tri.flag(i) : PM_GROUND);
            t++;
        }
        FREPA(mesh.quad) {
            tri_ind[t] = mesh.quad.ind(i).tri0();
            tri_type[t] = (mesh.quad.flag() ? mesh.quad.flag(i) : PM_GROUND);
            t++;
            tri_ind[t] = mesh.quad.ind(i).tri1();
            tri_type[t] = (mesh.quad.flag() ? mesh.quad.flag(i) : PM_GROUND);
            t++;
        }
    } else {
        if (mesh.tri.flag())
            tri_type.copyFrom(mesh.tri.flag());
        else
            SetMemN(tri_type.data(), PM_GROUND, tri_type.elms());
    }
    ...
    Box box = mesh;
    box.min.x = area_xy.x * settings.areaSize();
    box.max.x = box.min.x + settings.areaSize();
    box.min.z = area_xy.y * settings.areaSize();
    box.max.z = box.min.z + settings.areaSize();
    ...
    cfg.bmin[0] = box.min.x;
    cfg.bmin[1] = box.min.y;
    cfg.bmin[2] = box.min.z;
    cfg.bmax[0] = box.max.x;
    cfg.bmax[1] = box.max.y;
    cfg.bmax[2] = box.max.z;
    ...
    if (!rcBuildCompactHeightfield(&ctx, cfg.walkableHeight, cfg.walkableClimb, solid, _pm->chf))
        return false;

    // Erode the walkable area by agent radius.
    if (cfg.walkableRadius) {
        if (!rcErodeWalkableArea(&ctx, cfg.walkableRadius, _pm->chf))
            return false;
...
// PATH WORLD
/******************************************************************************/
static Bool ThreadFunc(Thread &thread) {
    ((PathWorld *)thread.user)->threadFunc();
    return true;
}
void PathWorld::threadFunc() {
    if (_build.elms()) {
        Build build;
        {
            ...
            if (_build.elms()) {
                Swap(build, _build[0]);
                _build.remove(0, true);
            } else
                goto wait; // goto will also unlock the 'lock'
        }
        Built built;
...
void PathWorld::zero() {
    _area_size = _ctrl_r = _ctrl_h = _max_climb = 0;
    _mesh = null;
    _query = null;
    _filter = null;
}
...
void PathWorld::del() {
    ...
    if (C dtMeshTile *tile = mesh->getTile(i))
        if (C dtMeshHeader *header = tile->header)
            if (_PathMesh *path_mesh = (_PathMesh *)header->userId)
                path_mesh->world = null;
...
Bool PathWorld::create(Flt area_size) {
    ...
    New(_filter, PMF_ALL + 1);
    REP(PMF_ALL + 1)
    _filter[i].setIncludeFlags(i);
    ...
    if (dtStatusFailed(_mesh->init(&params)))
        return false;
    if (dtStatusFailed(_query->init(_mesh, MaxSearchNodes)))
        return false;
...
Box PathWorld::obstacleBox(C Shape &shape) C {
    Box box = shape;
    box.extendX(_ctrl_r).extendZ(_ctrl_r).min.y -= _ctrl_h;
    return box;
}
/******************************************************************************/
...
void PathWorld::update() {
    if (_changed.elms() || _built.elms()) {
        Bool wake_up = (_changed.elms() > 0);
        {
            ...
            {
                _PathMesh &path_mesh = *_changed[i];
                if (path_mesh.is() && path_mesh.getChf()) {
                    Box mesh_box = path_mesh.box();
                    REPA(_obstacles) {
                        C Shape &shape = _obstacles[i];
                        if (Cuts(obstacleBox(shape), mesh_box))
                            area_shapes.add(shape);
                    }
                    // we always need to queue on the build thread, even if 'area_shapes' is empty, to preserve the build queue order
                    Build *build = null;
                    REPA(_build)
                    if (_build[i].xy == path_mesh.xy) {
                        build = &_build[i];
                        break;
                    }
                    if (!build)
                        build = &_build.New(); // if there's already a build queued up for this area, then reuse it with latest data but with the same queue position, otherwise create a new at the end of the queue
...
#pragma message("!! Warning: Use this only for debugging !!")
            ...
            if (InRange(i, _built)) // !! check this, because inside unlinking, we may delete some '_built', and 'i' index may be out of range, so keep going down to zero !!
            {
                ...
                if (_PathMesh *pm = pathMesh(built.xy)) // find path mesh at that location
                    ...
                    {
                        Mems<Byte> data;
                        Swap(data, built.data); // move to temp, because unlinking will delete 'built'
                        // !! don't access 'built' anymore because it gets deleted !!
                        pm->link(null);
                        Swap(pm->obstacle_data, data); // unlink before changing data, in case unlinking requires the same data that was used for linking
                        pm->link(this);
                        // !! don't access 'built' anymore because it gets deleted !!
                    }
                // !! don't access 'built' anymore because it gets deleted !!
...
Bool PathWorld::_set(_PathMesh *path_mesh, C VecI2 &area_xy, Bool set_obstacles) {
    _PathMesh *old = T.pathMesh(area_xy);
    if (old == path_mesh)
        return true;
    ...
    if (path_mesh)
        path_mesh->link(null); // if we want to link 'path_mesh' to this world, then unlink it from any other world first
    ...
    if (_mesh) {
        ...
        if (path_mesh && path_mesh->is()) {
            ...
            if (path_mesh->xy != area_xy)
                return false;

            if (!_ctrl_r) // set world controller settings (before setting obstacles)
                if (dtMeshHeader *header = (dtMeshHeader *)path_mesh->data.data()) {
                    _ctrl_r = header->walkableRadius;
                    _ctrl_h = header->walkableHeight;
                    _max_climb = header->walkableClimb;
                }
            ...
            if (set_obstacles) {
                ...
                if (!area_shapes.elms())
                    path_mesh->obstacle_data.del();
                else // if no shapes are needed, then delete 'obstacle_data' and use the main 'data' which we already have
                {
                    if (!path_mesh->getChf())
                        return false;
                    RecastCompactHeightfield chf = path_mesh->chf; // operate on 'chf' copy because it gets modified in 'Build'
                    if (!::Build(chf, area_shapes.data(), area_shapes.elms(), _ctrl_r, _ctrl_h, _max_climb, area_xy, path_mesh, path_mesh->obstacle_data))
                        return false; // 'userId' points to memory address of '_PathMesh'
                }
            }
            ...
            if (data.elms()) {
                if (dtStatusFailed(_mesh->addTile(data.data(), data.elms(), 0, 0, null)))
                    return false;
...
Bool PathWorld::find(C Vec &start, C Vec &end, MemPtr<Vec> path, Int max_steps, UInt walkable_flags, Bool allow_partial_paths, C Vec &end_extents) C {
    ...
    if (_query) {
        ...
        _query->findNearestPoly(start.c, extents.c, filter, &start_ref, null);
        if (start_ref) {
            dtPolyRef end_ref = 0;
            ...
            if (end_ref) {
                ...
                if (npolys) {
                    Vec path_points[2048], new_end = end;
                    Int path_length = 0;
                    if (polys[npolys - 1] != end_ref) // if there's no direct path
                    {
                        if (allow_partial_paths)
                            _query->closestPointOnPoly(polys[npolys - 1], end.c, new_end.c, null); // In case of partial path, make sure the end point is clamped to the last polygon
                        else
                            return false;
                    }
...
Bool PathWorld::nearestSurface(C Vec &pos, C Vec &extents, Vec &surface_pos, UInt walkable_flags) C {
    if (_query) {
        ...
        dtQueryFilter *filter = &T._filter[walkable_flags & PMF_ALL];
        dtPolyRef pos_ref;
        if (dtStatusSucceed(_query->findNearestPoly(pos.c, extents.c, filter, &pos_ref, surface_pos.c)))
            return true;
    }
    return false;
}
...
Bool PathWorld::nearestWall(C Vec &pos, Flt max_distance, Flt *hit_distance, Vec *hit_pos, Vec *hit_normal, UInt walkable_flags) C {
    if (_query) {
        ...
        dtPolyRef pos_ref = 0;
        _query->findNearestPoly(pos.c, extents.c, filter, &pos_ref, null);
    ...
    if (tile && tile->header) {
        if (surface_color_alpha) {
            for (int i = 0; i < tile->header->polyCount; ++i) {
                ...
                switch (p->getArea()) {
                case PM_BLOCKED:
                    surface_color.set(0, 0, 0, 0);
                    break;
                case PM_GROUND:
                    surface_color.set(0, 255, 0, surface_color_alpha);
                    break;
                case PM_WATER:
                    surface_color.set(0, 192, 255, surface_color_alpha);
                    break;
                }
                ...
                if (surface_color.a)
                    for (int j = 0; j < pd->triCount; ++j) {
                        ...
                        for (int k = 0; k < 3; ++k) {
                            C float *vtx_pos;
                            if (t[k] < p->vertCount)
                                vtx_pos = &tile->verts[p->verts[t[k]] * 3];
                            else
                                vtx_pos = &tile->detailVerts[(pd->vertBase + t[k] - p->vertCount) * 3];
