...
static BOOL CALLBACK EnumWindowClose(HWND window, LPARAM process_id) {
    SysWindow sys_win(window);
    if (sys_win.processID() == process_id)
        sys_win.close();
    return true;
}
#endif
...
Bool ProcWait(UInt id, Int milliseconds) {
    ...
    if (id) {
        ...
        if (HANDLE hproc = OpenProcess(SYNCHRONIZE, false, id)) {
            if (WaitForSingleObject(hproc, (milliseconds < 0) ? INFINITE : milliseconds) == WAIT_TIMEOUT)
                ok = false;
            CloseHandle(hproc);
        }
#elif !SWITCH
...
static BOOL CALLBACK EnumWindowFind(HWND window, LPARAM proc_window_find) {
    ...
    DWORD id;
    if (GetWindowThreadProcessId(window, &id) && id == pwf.process_id)
        if (IsWindowVisible(window) || IsIconic(window)) {
            pwf.window = window;
            return false;
        } else if (!pwf.window) // if found a window but it is hidden, then set it only if previous was not set
        {
            pwf.window = window;
        }
    return true;
...
Str ProcName(UInt id) {
    if (id) {
        ...
        if (!hproc)
            hproc = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, false, id); // if failed, then try using WindowsXP way, 'PROCESS_VM_READ' is needed here
#endif
        if (hproc) {
            ...
            DWORD dword;
            HMODULE hmod;
            if (EnumProcessModules(hproc, &hmod, SIZE(hmod), &dword))
                GetModuleFileNameEx(hproc, hmod, name, Elms(name)); // this requires PROCESS_QUERY_INFORMATION|PROCESS_VM_READ
            if (!name[0])                                           // requesting name failed, it's possible that encountered process is 64-bit and we're 32-bit, or PROCESS_QUERY_LIMITED_INFORMATION was used, and 'QueryFullProcessImageNameW' needs to be used
            {
                if (!QueryFullProcessImageNameTried) {
                    if (HMODULE kernel = GetModuleHandle(L"Kernel32.dll"))
                        QueryFullProcessImageNameW = (decltype(QueryFullProcessImageNameW))GetProcAddress(kernel, "QueryFullProcessImageNameW"); // available on Vista+
                    QueryFullProcessImageNameTried = true;
                }
                if (QueryFullProcessImageNameW) {
#endif
                    DWORD size = Elms(name);
        ...
        if (!sysctl(mib, 2, &max_data, &size, null, 0)) {
            Memt<Char8> temp;
            temp.setNum(max_data);

            mib[0] = CTL_KERN;
            mib[1] = KERN_PROCARGS2;
            mib[2] = id;
            size = temp.elmsMem();
            if (!sysctl(mib, 3, temp.data(), &size, null, 0))
                return FromUTF8(temp.data() + 4);
        }
#else // returns process window name
        ProcessSerialNumber psn;
        if (GetProcessForPID(id, &psn) == noErr) {
            CFStringRef proc_name = null;
            CopyProcessName(&psn, &proc_name);
            if (proc_name) {
                Str name = proc_name;
                CFRelease(proc_name);
                return name;
        ...
        again:;
            ...
            if (EnumProcessModules(hproc, temp.data(), size, &num)) {
                ...
                REPA(modules) {
                    wchar_t name[MAX_LONG_PATH];
                    name[0] = 0;
                    GetModuleFileNameEx(hproc, temp[i], name, Elms(name));
                    modules[i] = name;
                }
                return;
            }
    ...
    if (name.is()) {
      ...
      if(    snap!=INVALID_HANDLE_VALUE)
      {
        ...
        REPA(id) {
            ...
            if (drive) {
                if (!EqualPath(name, pn))
                    continue;
            } else {
                if (!Equal(name, _GetBase(pn)))
                    continue;
            }
            ret = id[i];
            break;
...
again:;
    ...
    if (EnumProcesses(temp.data(), size, &num)) {
        if (num >= size) {
            temp.addNum(Max(1, temp.elms() / 4));
            goto again;
        } // according to docs and tests, 'num' will be equal to 'size' if there are more processes that can fit into the buffer
        ASSERT(SIZE(DWORD) == SIZE(UInt));
        id.setNum(num / temp.elmSize()).copyFrom((UInt *)temp.data());
        return;
    }
#elif APPLE
    size_t size = 0;
    ...
    if (!code && size > 0) {
        Memt<kinfo_proc> temp;
        temp.setNum(Max(temp.maxElms(), Int(size / temp.elmSize() * 5 / 4))); // allocate a bit more
    again:;
        size_t size2 = temp.elmsMem();
        code = sysctl(names, Elms(names), temp.data(), &size2, null, 0);
        if (!code && !size2 && size) {
            size = 0;
            temp.addNum(Max(1, temp.elms() / 2));
            goto again;
        } // if not enough memory then try again
        if (!code && size2 > 0) {
            id.setNum(size2 / temp.elmSize());
            REPAO(id) = temp[i].kp_proc.p_pid;
            return;
        }
    }
#else
...
static pid_t popen2(CChar8 *cur_dir, CChar8 *command, int *in_fd, int *out_fd) {
    ...
    } else       // success
    {
        ...
        close(p_stdout[FD_WRITE]);
        if (in_fd)
            *in_fd = p_stdin[FD_WRITE];
        else
            close(p_stdin[FD_WRITE]);
        if (out_fd)
            *out_fd = p_stdout[FD_READ];
        else
            close(p_stdout[FD_READ]);
    }
    return pid;
...
start:
    if (cp._proc_id && cp._out_read) {
        auto out_read = cp._out_read;
        cp._out_read = NULL;        // take ownership of handle
    ...
    same_process:
        ...
        if (!active) {
            DWORD exit_code = 0;
            GetExitCodeProcess(cp._proc, &exit_code);                       // warning: process may have used 'STILL_ACTIVE' for the 'ExitProcess'
            cp._exit_code = ((exit_code != STILL_ACTIVE) ? exit_code : -1); // set while in lock to make sure we're setting for this process
        }
#else
        int status;
        pid_t pid = waitpid(proc_id, &status, WNOHANG);
        Bool active = (pid == 0);
        if (!active) {
    ...
    peek:
        DWORD available = 0;
        PeekNamedPipe(out_read, null, 0, null, &available, null);
        if (available > 0) // check if there's any data available, this is to prevent freezes in 'ReadFile' when there's nothing available
        {
            DWORD read = 0;
            ReadFile(out_read, buf, Min((UInt)available, total), &read, null);
            if (read > 0)
#else
    wait:
        fd_set fd;
        FD_ZERO(&fd);
        FD_SET(out_read, &fd); // !! THIS HAS TO BE REINITIALIZED BEFORE EACH 'select' CALL BECAUSE IT MODIFIES THE VALUE !!
        timeval tv;
        tv.tv_sec = time_ms / 1000;
        tv.tv_usec = (time_ms % 1000) * 1000;               // !! THIS HAS TO BE REINITIALIZED BEFORE EACH 'select' CALL BECAUSE IT MODIFIES THE VALUE !!
        ...
        peek:
            struct stat s;
            if (!fstat(out_read, &s)) {
                auto available = s.st_size;
                if (available > 0) {
                    Int read = ::read(out_read, buf, Min(available, total));
                    if (read > 0)
#else
        read:
            Int read = ::read(out_read, buf, total);
            if (read > 0)
#endif
#endif
            {
...
#else
                                if (active)
                                    goto wait; // if active then check first if we have data to avoid freezes
...
if (same_process && active)
    goto same_process; // if it's still the same process and active
#if WINDOWS
...
#endif
if (!same_process)
    goto start; // if started a different process, then process it instead of exiting thread
}
...
static void ProcessZombies() {
    ...
    REPA(ZombiePIDs) {
        int status;
        pid_t pid = waitpid(ZombiePIDs.lockedElm(i), &status, WNOHANG);
        Bool active = (pid == 0);
        if (!active)
            ZombiePIDs.lockedRemove(i);
    }
    ZombiePIDs.unlock();
    if (ZombiePIDs.elms())
        App.includeFuncCall(ProcessZombies); // queue again if we still have some zombies
}
#endif
void ConsoleProcess::del() {
    ...
    _data.clear();
    _exit_code = -1;
    _binary = false; // !! do not clear '_thread_exiting' !!
#if WINDOWS
    if (_proc) {
        CloseHandle(_proc);
        _proc = null;
                ...
                if (CreatePipe(&in_read, &_in_write, &sa, 0))            // Create the child input pipe
                {
                    if (SetHandleInformation(_in_write, HANDLE_FLAG_INHERIT, 0)) // Ensure the write handle to the pipe for STDIN is not inherited
                    {
                        ...
                        if (ext == "bat" || ext == "cmd") {
                            ...
                            wchar_t comspec[MAX_LONG_PATH];
                            comspec[0] = '\0';
                            if (GetEnvironmentVariable(L"COMSPEC", comspec, Elms(comspec)))
                                app = comspec;
                            // cmd =S+"/c "+name+"";
                            // cmd =S+"\"/c "+name+"\"";//+' '+params+"\"";
                            // cmd =S+"/c \""+name+' '+params+"\"";
                        ...
                        if (params.is())
                            cmd.space() += params;
                        Memt<Char> cmd_temp;
                        cmd_temp.setNum(cmd.length() + 1);
                        Set(cmd_temp.data(), cmd, cmd_temp.elms()); // can be very long, copy to 'cmd_temp' because 'CreateProcessW' can modify it
                        if (CreateProcessW(app, WChar(cmd_temp.data()), null, null, TRUE, CREATE_NEW_CONSOLE, null, cur_dir, &si, &pi)) {
                            _proc = pi.hProcess;
                            _proc_id = pi.dwProcessId;
                            _binary = binary;
                            Bool exiting = _thread_exiting;
                            _thread_exiting = false;

                            locker.off(); // UNLOCK, so we can create thread (which deletes if needed)

                            if (!_thread.active() || exiting)
                                if (!_thread.create(ConsoleProcessFunc, this))
                                    del();
                        }
                    }
                    CloseHandle(in_read);
                    ...
                    if (CreatePipe(&in_read, &_in_write, &sa, 0))            // Create the child input pipe
                    {
                        if (SetHandleInformation(_in_write, HANDLE_FLAG_INHERIT, 0)) // Ensure the write handle to the pipe for STDIN is not inherited
                        {
                            ...
                            if (CreateProcessW(comspec, WChar(cmd_temp.data()), null, null, TRUE, CREATE_NEW_CONSOLE, null, cur_dir, &si, &pi)) {
                                ...
                                if (!_thread.active() || exiting)
                                    _thread.create(ConsoleProcessFunc, this);
                            }
                        }
                        CloseHandle(in_read);
                        in_read = null;
...
Str ConsoleProcess::get() {
    if (_data.is()) {
        ...
        if (_binary) {
            ...
            temp._d.setNum(_data.length() + 1);
            temp._d.last() = '\0';
            REPA(_data)
            temp._d[i] = _data._d[i]; // do raw copy of 'Char8' to 'Char' to avoid code-page changes, can't do 'CopyFastN' because we're copying from 'Str8' to 'Str'
            return temp;
        } else
#if WINDOWS
        ...
        {
            Str s;
            Char8 last = _data.last();
            if ((last & 0x80) && active())
                _data.removeLast();
            else
                last = '\0'; // possibly beginning of a multi-byte character, and the process is still active (which may generate more characters) then don't process this char now (remove it from '_data' and keep it in 'last' to restore later), otherwise clear it
            if (Int length = _data.length()) {
                Int size = MultiByteToWideChar(CP_ACP, 0, _data(), length, null, 0);
                if (size > 0) {
                    s.reserve(size);
                    if (size == MultiByteToWideChar(CP_ACP, 0, _data(), length, WChar(s._d.data()), s._d.elms()))
                        s._d[s._length = size] = '\0';
                    else
                        s.clear();
                }
            }
            _data.clear();
            if (last)
                _data += last; // restore what was removed
            return s;
        } else
#endif
        { // UTF8
...
Bool GetProcessesAccessingThisProcess(MemPtr<ProcessAccess> proc, Bool write_only, Mems<Byte> *temp) {
    ...
    if (!HANDLE_TYPE_PROCESS) {
        ...
        if (Compare(ver, VecI4(6, 1, 0, 0)) >= 0)
    ...
    if (NtQuerySystemInformation) {
        Mems<Byte> temp_local;
        if (!temp)
            temp = &temp_local;
    again:
        ULONG size = 0;
        NTSTATUS status = NtQuerySystemInformation(SystemHandleInformation, temp->data(), temp->elms(), &size);
        ...
        if (NT_SUCCESS(status)) {
            ...
            SYSTEM_HANDLE *handles = shi.Handles;
            Int handle_count = Min((UInt)shi.HandleCount, INT_MAX);
...
#if SUPPORT_WINDOWS_XP
                                if (!dup_handle)
                                    NtDuplicateObject(proc_handle, HANDLE(handle.Handle), cur_proc, &dup_handle, PROCESS_QUERY_INFORMATION, 0, 0); // don't use DUPLICATE_SAME_ATTRIBUTES because exception can occur when trying to release handles that were "protected from close via NtSetInformationObject", don't use DUPLICATE_SAME_ACCESS because if handle would have write permission then probably we would have to, but we don't want to get write permission, but only check the PID, if failed, then try using WindowsXP way, 'PROCESS_VM_READ' is not needed here
#endif
                                if (dup_handle) {
