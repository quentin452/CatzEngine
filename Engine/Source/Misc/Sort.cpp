...
T1(TYPE)
static INLINE void _Sort(TYPE *data, Int elms) {
    if (elms > 1) {
        Memt<VecI2> stack;
        stack.New().set(0, elms - 1);
        for (; stack.elms();) {
            VecI2 lr = stack.pop();
            Int l = lr.x,
                r = lr.y;
            if (r - l > 48) // 48 gave best results
            {
                ...
                for (; i <= j;) {
                    for (; ELM(i) < temp;)
                        i++; // find first element from left  that is larger  or equal to 'temp'
                    for (; ELM(j) > temp;)
                        j--; // find first element from right that is smaller or equal to 'temp'
                    if (i <= j) {
                        SWAP(i, j);
                        i++;
                        j--;
    ...
    if (elms > 1) {
        ...
        for (; stack.elms();) {
            ...
            if (r - l > 16) // 16 gave the best result
            {
                // quicksort
                Int m = UInt(r + l) / 2;
                if (compare(ELM(l), ELM(r)) > 0)
                    SWAP(l, r);
                if (compare(ELM(l), ELM(m)) > 0)
                    SWAP(l, m);
                if (compare(ELM(m), ELM(r)) > 0)
                    SWAP(m, r);

                Int i = l, j = r;
                COPY(temp, ELM(m));
                for (; i <= j;) {
                    for (; compare(ELM(i), temp) < 0;)
                        i++; // find first element from left  that is larger  or equal to 'temp'
                    for (; compare(ELM(j), temp) > 0;)
                        j--; // find first element from right that is smaller or equal to 'temp'
                    if (i <= j) {
                        SWAP(i, j);
                        i++;
               ...
               COPY(ELM(j), temp);
                    ...
                    CPtr test = ELM(i);
                    Int j;
                    for (j = i; j > l && compare(ELM(j - 1), test) > 0; j--)
                        ;
    ...
    if (elms > 1) {
        ...
        for (; stack.elms();) {
            ...
            if (r - l > 16) // 16 gave the best result
            {
                // quicksort
                Int m = UInt(r + l) / 2;
                if (compare(ELM(l), ELM(r), user) > 0)
                    SWAP(l, r);
                if (compare(ELM(l), ELM(m), user) > 0)
                    SWAP(l, m);
                if (compare(ELM(m), ELM(r), user) > 0)
                    SWAP(m, r);

                Int i = l, j = r;
                COPY(temp, ELM(m));
                for (; i <= j;) {
                    for (; compare(ELM(i), temp, user) < 0;)
                        i++; // find first element from left  that is larger  or equal to 'temp'
                    for (; compare(ELM(j), temp, user) > 0;)
                        j--; // find first element from right that is smaller or equal to 'temp'
                    if (i <= j) {
                        SWAP(i, j);
                        i++;
               ...
               COPY(ELM(j), temp);
                    ...
                    CPtr test = ELM(i);
                    Int j;
                    for (j = i; j > l && compare(ELM(j - 1), test, user) > 0; j--)
                        ;
                    _MoveElmLeftUnsafe(data, elm_size, i, j, temp);
#endif
                }
        ...
        for (; stack.elms();) {
            ...
            if (r - l > 16) // 16 gave the best result
            {
                ...
                for (Int i = l + 1; i <= r; i++) {
                    Ptr temp = ELM(i); // we can use this because address remains the same
                    Int j;
                    for (j = i; j > l && compare(ELM(j - 1), temp) > 0; j--)
                        SWAP(j, j - 1);
                }
            }
        }
        ...
        for (; stack.elms();) {
            ...
            if (r - l > 16) // 16 gave the best result
            {
                ...
                for (Int i = l + 1; i <= r; i++) {
                    Ptr temp = ELM(i); // we can use this because address remains the same
                    Int j;
                    for (j = i; j > l && compare(ELM(j - 1), temp, user) > 0; j--)
                        SWAP(j, j - 1);
                }
            }
        }
...
Bool _Meml::binarySearch(CPtr value, Int &index, Int compare(CPtr a, CPtr b)) C {
    Int pos = 0;
    MemlNode *node = first(); // get first node
    Int l = 0, r = elms();
    for (; l < r;) {
        Int mid = UInt(l + r) / 2;
        for (; pos < mid; pos++)
            node = node->next(); // go forward
        for (; pos > mid; pos--)
            node = node->prev(); // go back
