...
CHAR_TYPE CharTypeFast(Char c) // don't INLINE because it's not a simple function
{
    ...
    if (f & (CHARF_ALPHA | CHARF_DIG | CHARF_UNDER | CHARF_NBSP))
        return CHART_CHAR; // check this first in case '_' is both CHARF_UNDER and CHARF_SIGN, and Nbsp is both CHARF_NBSP and CHARF_SPACE
    if (f & CHARF_SPACE)
        return CHART_SPACE;
    if (f & CHARF_SIGN)
        return CHART_SIGN;
...
/******************************************************************************/
Int CharOrder(Char8 c) {
    I();
    return CharOrderFast(c);
}
...
UInt CharFlag(Char c) {
    I();
    return CharFlagFast(c);
}
UInt CharFlag(Char8 c) {
    I();
    return CharFlagFast(c);
}

Char CaseDown(Char c) {
    I();
    return CaseDownFast(c);
}
Char CaseUp(Char c) {
    I();
    return CaseUpFast(c);
}
Char8 CaseDown(Char8 c) {
    I();
    return CaseDownFast(c);
}
Char8 CaseUp(Char8 c) {
    I();
    return CaseUpFast(c);
}

CHAR_TYPE CharType(Char c) {
    I();
    return CharTypeFast(c);
}
CHAR_TYPE CharType(Char8 c) {
    I();
    return CharTypeFast(c);
}
...
Int CompareCI(Char8 a, Char8 b) {
    I();
    return CharOrderFast(a) - CharOrderFast(b);
}
Int CompareCI(Char8 a, Char b) {
    I();
    return CharOrderFast(a) - CharOrderFast(b);
}
Int CompareCI(Char a, Char8 b) {
    I();
    return CharOrderFast(a) - CharOrderFast(b);
}
Int CompareCI(Char a, Char b) {
    I();
    return CharOrderFast(a) - CharOrderFast(b);
}
...
Bool EqualCI(Char8 a, Char8 b) {
    I();
    return EqualCIFast(a, b);
}
Bool EqualCI(Char8 a, Char b) {
    I();
    return EqualCIFast(a, b);
}
Bool EqualCI(Char a, Char8 b) {
    I();
    return EqualCIFast(a, b);
}
Bool EqualCI(Char a, Char b) {
    I();
    return EqualCIFast(a, b);
}
...
Bool Skip(Char c) {
    return c == '\r' || (Unsigned(c) >= 0xFE00 && Unsigned(c) <= 0xFE0F); // 0xFE00..0xFE0F useless character occurring in emojis causing "?" https://en.wikipedia.org/wiki/Variation_Selectors_(Unicode_block)
}
        ...
        if (case_sensitive)
            for (;; a++, b++) {
                ...
                if (a_order != b_order) {
                    ...
                    if (as) {
                        if (bs)
                            continue; // if both have slashes
                        if (!a[1] && !b[0])
                            return 0; // treat "c:/" and "c:" as the same
            ...
            for (;; a++, b++) {
                ...
                if (ac != bc) {
                    if (IsDigit(ac) && IsDigit(bc)                           // if both are  digits
                        ...
                        if (Int c = CompareNumberLength(a, b))
                            return c;
    ...
    if (t) {
        ...
        if (case_sensitive)
            for (;; t++, start++) {
                ...
                {
                    if (whole_word && CharWholeWord(last_start, whole_word))
                        return !CharWholeWord(t[0], whole_word);
                    return true;
                }
                if (!EqualCS(*t, *start))
                    return false;
    ...
    if (t) {
        ...
        if (case_sensitive)
            for (;; t++, start++) {
                if (!start[0]) // reached the end
                {
                    if (whole_word && CharWholeWord(last_start, whole_word) && CharWholeWord(t[0], whole_word))
                        return false;
                ...
                if (!EqualCS(c, last_start)) {
                    if (c == ' ')
                        for (t++;; t++) {
                            ...
                            if (c == ' ')
                                continue;
                            if (!EqualCS(c, last_start) || CharWholeWord(last_t, whole_word_sub) && CharWholeWord(c, whole_word_sub))
                                break; // if both are chars, then we can't merge
                            goto next_cs;
                        }
                    return false;
                }
                ...
                if (!EqualCIFast(c, last_start)) {
                    if (c == ' ')
                        for (t++;; t++) {
                            ...
                            if (c == ' ')
                                continue;
                            if (!EqualCIFast(c, last_start) || CharWholeWord(last_t, whole_word_sub) && CharWholeWord(c, whole_word_sub))
                                break; // if both are chars, then we can't merge
                            goto next_ci;
                        }
                    return false;
    ...
    if (t)
        for (I();; t++, start++) {
            if (!EqualCIFast(*t, *start)) {
                ...
                if (ts) {
                    if (ss)
                        continue; // both have slashes
                    if (!start[0])
                        return true;
                }
                if (ss && !start[1] && !t[0])
                    return true;
                if (!start[0] && IsSlash(t[-1]))
                    return true;
                return false;
            }
            if (!t[0])
...
Bool Contains(CChar8 *src, Char8 c) {
    ...
    return false;
} // break before checking to prevent returning true for '\0' chars
Bool Contains(CChar *src, Char c) {
    if (src)
        for (;;) {
            Char s = *src++;
            if (!s)
...
Bool Contains(CChar *src, Char8 c) {
    Char a = Char8To16(c);
    if (src)
        for (;;) {
            Char s = *src++;
...
Bool Contains(CChar8 *src, Char c) {
    Char8 a = Char16To8(c);
    if (Char8To16(a) == c)
        if (src)
            for (;;) {
...
CChar8 *_SkipWhiteChars(CChar8 *t) {
    if (t)
        for (; WhiteChar(*t);)
            t++;
    return t;
}
CChar *_SkipWhiteCharsComma(CChar *t) {
    if (t) {
        for (; WhiteChar(*t);)
            t++;
        if (*t == ',')
...
CChar8 *_SkipWhiteCharsComma(CChar8 *t) {
    if (t) {
        for (; WhiteChar(*t);)
            t++;
        if (*t == ',')
...
CChar *_SkipWhiteCharsDot(CChar *t) {
    if (t) {
        for (; WhiteChar(*t);)
            t++;
        if (*t == '.')
...
CChar8 *_SkipWhiteCharsDot(CChar8 *t) {
    if (t) {
        for (; WhiteChar(*t);)
            t++;
        if (*t == '.')
        ...
        for (I();; t++, start++) {
            if (!EqualCIFast(*t, *start)) {
                ...
                if (ss && !start[1] && !t[0])
                    return null;
                if (!start[0] && IsSlash(t[-1]))
                    return t; // here t[0] will never be '\0' because "start[0]=='\0' && !EqualCIFast(*t, *start)"
                return ret;
            }
            if (!t[0])
                return null;
            ...
            if (!EqualCIFast(*t, *start)) {
                ...
                if (ts) {
                    if (ss)
                        continue; // both have slashes
                    if (!start[0])
                        return t[1] ? t + 1 : null; // !! return null instead of an empty string "", to have consistency so that '_AfterPath' will return null in all 4 cases: !_AfterPath(u"abc", u"abc") && !_AfterPath(u"abc/", u"abc") && !_AfterPath(u"abc", u"abc/") && !_AfterPath(u"abc/", u"abc/"), also in the engine where '_AfterPath' is used, there is a quick check of it being non null only !!
                }
                if (ss && !start[1] && !t[0])
                    break;
                if (!start[0] && IsSlash(t[-1]))
...
Str SkipEnd(C Str &t, C Str &end) {
    if (end.length() <= t.length() && Equal(t() + t.length() - end.length(), end))
        return Str(t).clip(t.length() - end.length());
    return t;
}
/******************************************************************************/
Char *ReplaceSelf(Char *text, Char from, Char to) {
    if (text)
        for (Char *src = text, *dest = text;;) {
            Char c = *src++;
            if (!c) {
                *dest = '\0';
                break;
...
Str Replace(C Str &text, C Str &from, C Str &to, Bool case_sensitive, WHOLE_WORD whole_word) {
    if (text.is() && from.is()) {
        ...
        Bool dont_check[2];
        if (whole_word) {
            dont_check[0] = !CharWholeWord(from.first(), whole_word);
            dont_check[1] = !CharWholeWord(from.last(), whole_word);
            if (dont_check[0] && dont_check[1])
                whole_word = WHOLE_WORD_NO;
        }
        Char last = '\0';
...
/******************************************************************************/
Str CaseDown(C Str &t) {
    Str s;
    if (t.is()) {
        s.reserve(t.length());
        I();
        REPA(t)
        s._d[i] = CaseDownFast(t()[i]);
        s._d[s._length = t.length()] = '\0';
    }
    return s;
} // () avoids range checks
Str CaseUp(C Str &t) {
...
Memc<Str> Split(C Str &string, Char separator) {
    Memc<Str> splits;
    Split(splits, string, separator);
    return splits;
}
Memc<Str8> Split(C Str8 &string, Char8 separator) {
    Memc<Str8> splits;
    Split(splits, string, separator);
    return splits;
}
...
Int TextPosSkipSpaceI(CChar *src, CChar *t, Int &match_length, Bool case_sensitive, WHOLE_WORD whole_word, WHOLE_WORD whole_word_sub) {
    if (src && Is(t)) {
        ...
        for (Int pos = 0;; pos++) {
            ...
            if (!c)
                break;
            if (CharOrderFast(c) == order && StartsSkipSpace(src, t, match_length, case_sensitive, WHOLE_WORD_NO, whole_word_sub) && (whole_word ? (dont_check[0] || !CharWholeWord(last, whole_word)) && (dont_check[1] || !CharWholeWord(src[match_length], whole_word)) : true))
                return pos;
            last = c;
            src++;
        }
...
/****************************************************************************/
CChar *TextPos(CChar *src, Char c) {
    Int p = TextPosI(src, c);
    return (p < 0) ? null : &src[p];
}
CChar8 *TextPos(CChar8 *src, Char8 c) {
    Int p = TextPosI(src, c);
    return (p < 0) ? null : &src[p];
}
CChar *TextPos(CChar *src, CChar *t, Bool case_sensitive, WHOLE_WORD whole_word) {
    Int p = TextPosI(src, t, case_sensitive, whole_word);
    return (p < 0) ? null : &src[p];
}
CChar *TextPos(CChar *src, CChar8 *t, Bool case_sensitive, WHOLE_WORD whole_word) {
    Int p = TextPosI(src, t, case_sensitive, whole_word);
    return (p < 0) ? null : &src[p];
}
CChar8 *TextPos(CChar8 *src, CChar8 *t, Bool case_sensitive, WHOLE_WORD whole_word) {
    Int p = TextPosI(src, t, case_sensitive, whole_word);
    return (p < 0) ? null : &src[p];
}
/******************************************************************************/
    ...
    if (text)
        for (;;) {
            ...
            if (!(b0 & (1 << 7))) {
                ...
                if (!(b0 & (1 << 6)))
                    break; // bit 6 should be always on

                Byte b1 = *text;
                if ((b1 & ((1 << 7) | (1 << 6))) != (1 << 7))
                    break;
                b1 &= 0x3F;
                text++; // bit 7 should be always on, bit 6 always off, this handles NUL. Advance after checking if char is valid, so we don't skip past NUL
                if (!(b0 & (1 << 5))) {
                    b0 &= 0x1F;
                    c = (b1 | (b0 << 6));
                } else {
        ...
        for (;;) {
            ...
            if (!(b0 & (1 << 7))) {
                ...
                if ((b1 & ((1 << 7) | (1 << 6))) != (1 << 7))
                    break;
                b1 &= 0x3F;
                text++; // bit 7 should be always on, bit 6 always off, this handles Safe/NUL. Advance after checking if char is valid, so we don't skip past NUL
                if (!(b0 & (1 << 5))) {
                    b0 &= 0x1F;
                    c = (b1 | (b0 << 6));
                } else {
    ...
    *text = '\0';
    if (digits < 0 && !i)
        *--text = '0';
        ...
        if (separate > 0) {
            s = separate + 1;
            max_digits = max_digits * separate / s;
        }
        MIN(digits, max_digits);
        for (UInt u = Abs(i); (digits < 0) ? u : digits--; u /= 10) {
            if (!--s) {
                *--text = ' ';
                s = separate;
            }
    ...
    *text = '\0';
    if (digits < 0 && !i)
        *--text = '0';
        ...
        MIN(digits, max_digits);
        for (ULong u = Abs(i); (digits < 0) ? u : digits--; u /= 10) {
            if (!--s) {
                *--text = ' ';
                s = separate;
            }
...
StrO TextHexMem(File &file, Bool prefix) {
    StrO s;
    s.reserve((prefix ? 2 : 0) + file.left() * 2);
    if (prefix)
        s = "0x";
    for (; !file.end();) {
        Byte b = file.getByte();
...
CChar8 *TextReal(Dbl r, Char8 (&temp)[256], Int precision, Int separate) {
    ...
    Bool non_zero = false,
         cut = (precision < 0);
    precision = Abs(Mid(precision, -9, 15));
    ...
    if (a <= UINT_MAX + 0.5) {
        ...
        if (!a)
            *--text = '0';
            ...
            for (non_zero = true, u = 0; a && u < 128 - 2; u++) // leave 2 characters for potential separate and '-' sign
            {
                if (!--s) {
                    *--text = ' ';
                    s = separate;
...
{                                          // !! Warning: if changing codes then watch out for 'temp' size !!
    UInt frac = ((UInt &)f) & ((1 << 23) - 1);
    Int exp = ((((UInt &)f) >> 23) & ((1 << 8) - 1)) - 127;
    if (exp == 128)
        return frac ? "NaN" : (f < 0) ? "-Inf"
                                      : "Inf";
    Char8 *t = temp;
    if (f < 0)
        *t++ = '-'; // sign
    ...
    if (exp == -127) {
        *t++ = '0';
        if (frac)
            ...
            exp = 0; // frac!=0 -> denormalized else real=0 so disable writing exponent
    } else
        *t++ = '1';
    if (frac) {
        *t++ = '.';
...
{                                          // !! Warning: if changing codes then watch out for 'temp' size !!
    ULong frac = ((ULong &)d) & ((1ull << 52) - 1);
    Int exp = ((((ULong &)d) >> 52) & ((1 << 11) - 1)) - 1023;
    if (exp == 1024)
        return frac ? "NaN" : (d < 0) ? "-Inf"
                                      : "Inf";
    Char8 *t = temp;
    if (d < 0)
        *t++ = '-'; // sign
...
/******************************************************************************/
StrO TextInt(Int i, Int digits, Int separate) {
    Char8 temp[256];
    return TextInt(i, temp, digits, separate);
}
StrO TextInt(Long i, Int digits, Int separate) {
    Char8 temp[256];
    return TextInt(i, temp, digits, separate);
}
StrO TextInt(UInt u, Int digits, Int separate) {
    Char8 temp[256];
    return TextInt(u, temp, digits, separate);
}
StrO TextInt(ULong u, Int digits, Int separate) {
    Char8 temp[256];
    return TextInt(u, temp, digits, separate);
}
StrO TextBin(UInt u, Int digits, Int separate, Bool prefix) {
    Char8 temp[256];
    return TextBin(u, temp, digits, separate, prefix);
}
StrO TextBin(ULong u, Int digits, Int separate, Bool prefix) {
    Char8 temp[256];
    return TextBin(u, temp, digits, separate, prefix);
}
StrO TextHex(UInt u, Int digits, Int separate, Bool prefix) {
    Char8 temp[256];
    return TextHex(u, temp, digits, separate, prefix);
}
StrO TextHex(ULong u, Int digits, Int separate, Bool prefix) {
    Char8 temp[256];
    return TextHex(u, temp, digits, separate, prefix);
}
StrO TextHex(Flt r) { return _TextHex(r); }
StrO TextHex(Dbl r) { return _TextHex(r); }
StrO TextReal(Dbl r, Int precision, Int separate) {
    Char8 temp[256];
    return TextReal(r, temp, precision, separate);
}
StrO TextVer(C VecI4 &ver) {
    Char8 temp[256];
    StrO s;
    s += TextInt(ver.x, temp);
    if (ver.y || ver.z || ver.w) {
        s += '.';
        s += TextInt(ver.y, temp);
        if (ver.z || ver.w) {
            s += '.';
            s += TextInt(ver.z, temp);
            if (ver.w) {
                s += '.';
                s += TextInt(ver.w, temp);
            }
        }
    }
    return s;
...
Bool TextHexMem(C Str &t, Ptr data, Int size) // this method sets all correct digits, incorrect sets to zero, returns false on fail however preserves those that were correct
{
    ...
    if (Byte *dest = (Byte *)data) {
        ...
        {
            Int hi = CharInt(t[src++]);
            if (!InRange(hi, 16)) {
                ok = false;
                break;
            }
            hi <<= 4;
            Int lo = CharInt(t[src++]);
            if (!InRange(lo, 16)) {
                dest[i++] = hi;
                ok = false;
                break;
            } // set 'dest' from 'hi' only and increase 'i' to mark it as processed
            dest[i++] = (lo | hi);
        }
        for (; i < size;)
            dest[i++] = 0; // clear unprocessed with zeros
    }
    return ok;
}
/******************************************************************************/
...
CChar8 *TextValue(CChar8 *t, CalcValue &value, Bool allow_real) {
    ...
    if (t) {
...
#if !X64
                case T_NONE:
                    type = T_UINT; // !! no break on purpose !!
                case T_UINT: {
                    UInt next = u * mode + i;
                    if (u > max && (next - i) / mode != u) {
                        type = T_ULONG;
                        ul = u;
                        maxl = ULONG_MAX / mode;
                        goto as_big;
                    } // if we're close to limit then check for overflow, don't test "if(next<ul)" because in some cases this doesn't work
                    u = next;
                } break;
#endif
...
#if X64
                case T_NONE:
                    type = T_ULONG; // !! no break on purpose !!
#else
                as_big:
#endif
                case T_ULONG: {
                    ULong next = ul * mode + i;
                    if (ul > maxl && (next - i) / mode != ul) {
                        type = T_REAL;
                        r = ul;
                        goto as_real;
                    } // if we're close to limit then check for overflow, don't test "if(next<ul)" because in some cases this doesn't work
                ...
                if (allow_real)
                    ...
                    case 'P': {
                        ...
                        for (;;) {
                            Int c = *t - '0';
                            if (!InRange(c, 10))
                                break; // both 'e' and 'p' operate on decimal exponents
                            t++;
                            exp *= 10;
                            exp += c;
                        }
...
#if !X64
        case T_UINT: {
            if (SIZE(value.i) < SIZE(ULong) && u >= 0x80000000 + sign) {
                ul = u;
                goto set_long;
            } // Int would be invalid, u>=(sign ? 0x80000001 : 0x80000000)
            value.type = CVAL_INT;
            value.i = u;
            if (sign)
                CHS(value.i);
        ...
        case T_ULONG: {
            if (allow_real && ul >= 0x8000000000000000 + sign) {
                r = ul;
                goto set_real;
            } // Long would be invalid, u>=(sign ? 0x8000000000000001 : 0x8000000000000000), however check this only if we 'allow_real' because the value could still be correct if casted to ULong, however as real it would lose precision, some functions rely on this behavior, for example 'UID.fromCString'
#if !X64
        set_long:
#endif
            value.type = CVAL_INT;
...
UInt TextUInt(CChar8 *t) {
    CalcValue x;
    TextValue(t, x);
    return x.asUInt();
}
UInt TextUInt(CChar *t) {
    CalcValue x;
    TextValue(t, x);
    return x.asUInt();
}
Flt TextFlt(CChar8 *t) {
    CalcValue x;
    TextValue(t, x);
    return x.asFlt();
}
...
Vec2 TextVec2(CChar8 *t) {
    if (!t)
        return 0;
    CalcValue x, y;
    t = _SkipWhiteChars(TextValue(t, x));
    if (*t != ',')
        return x.asFlt();
    TextValue(t + 1, y);
    return Vec2(x.asFlt(), y.asFlt());
}
Vec2 TextVec2(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecD2 TextVecD2(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asDbl();
    TextValue(t + 1, y);
    return VecD2(x.asDbl(), y.asDbl());
}
VecD2 TextVecD2(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecI2 TextVecI2(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(t + 1, y);
    return VecI2(x.asInt(), y.asInt());
}
VecI2 TextVecI2(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecB2 TextVecB2(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(t + 1, y);
    return VecB2(x.asInt(), y.asInt());
}
VecB2 TextVecB2(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecSB2 TextVecSB2(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(t + 1, y);
    return VecSB2(x.asInt(), y.asInt());
}
VecSB2 TextVecSB2(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecUS2 TextVecUS2(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(t + 1, y);
    return VecUS2(x.asInt(), y.asInt());
}
VecUS2 TextVecUS2(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y;
    t = _SkipWhiteChars(TextValue(t, x));
...
Vec TextVec(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asFlt();
    TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z);
    return Vec(x.asFlt(), y.asFlt(), z.asFlt());
}
Vec TextVec(CChar *t) {
    if (!t)
        return 0;
...
VecD TextVecD(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asDbl();
    TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z);
    return VecD(x.asDbl(), y.asDbl(), z.asDbl());
}
VecD TextVecD(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecI TextVecI(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z);
    return VecI(x.asInt(), y.asInt(), z.asInt());
}
VecI TextVecI(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecB TextVecB(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z);
    return VecB(x.asInt(), y.asInt(), z.asInt());
}
VecB TextVecB(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecSB TextVecSB(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z);
    return VecSB(x.asInt(), y.asInt(), z.asInt());
}
VecSB TextVecSB(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecUS TextVecUS(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z);
    return VecUS(x.asInt(), y.asInt(), z.asInt());
}
VecUS TextVecUS(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z;
    t = _SkipWhiteChars(TextValue(t, x));
...
Vec4 TextVec4(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asFlt();
    TextValue(_SkipWhiteCharsComma(TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z)), w);
    return Vec4(x.asFlt(), y.asFlt(), z.asFlt(), w.asFlt());
}
Vec4 TextVec4(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z, w;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecD4 TextVecD4(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asDbl();
    TextValue(_SkipWhiteCharsComma(TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z)), w);
    return VecD4(x.asDbl(), y.asDbl(), z.asDbl(), w.asDbl());
}
VecD4 TextVecD4(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z, w;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecI4 TextVecI4(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(_SkipWhiteCharsComma(TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z)), w);
    return VecI4(x.asInt(), y.asInt(), z.asInt(), w.asInt());
}
VecI4 TextVecI4(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z, w;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecB4 TextVecB4(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(_SkipWhiteCharsComma(TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z)), w);
    return VecB4(x.asInt(), y.asInt(), z.asInt(), w.asInt());
}
VecB4 TextVecB4(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z, w;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecSB4 TextVecSB4(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(_SkipWhiteCharsComma(TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z)), w);
    return VecSB4(x.asInt(), y.asInt(), z.asInt(), w.asInt());
}
VecSB4 TextVecSB4(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z, w;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecUS4 TextVecUS4(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(_SkipWhiteCharsComma(TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z)), w);
    return VecUS4(x.asInt(), y.asInt(), z.asInt(), w.asInt());
}
VecUS4 TextVecUS4(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z, w;
    t = _SkipWhiteChars(TextValue(t, x));
...
Color TextColor(CChar8 *t) {
    ...
    if (*t != ',')
        return x.asInt();
    TextValue(_SkipWhiteCharsComma(TextValue(_SkipWhiteCharsComma(TextValue(t + 1, y)), z)), w);
    return Color(x.asInt(), y.asInt(), z.asInt(), w.asInt());
}
Color TextColor(CChar *t) {
    if (!t)
        return 0;
    CalcValue x, y, z, w;
    t = _SkipWhiteChars(TextValue(t, x));
...
VecI4 TextVer(CChar *t) {
    ...
    Int i = 0;
    for (;;) {
        t = TextValue(t, v, false);
        if (!v.type)
            break;
        ver.c[i++] = v.asInt();
        if (!InRange(i, ver))
            goto end;
        t = _SkipWhiteCharsDot(t);
    }
    for (; i < Elms(ver); i++)
        ver.c[i] = 0;
end:
...
Str8::Str8(CChar *t) {
    if (_length = Length(t)) {
        _d.setNum(length() + 1);
        Set(_d.data(), t, _d.elms());
    }
}
Str8::Str8(C wchar_t *t) {
    if (_length = Length(t)) {
        _d.setNum(length() + 1);
        _Set(_d.data(), t, _d.elms());
    }
}
Str ::Str(C wchar_t *t) {
    if (_length = Length(t)) {
...
Str8::Str8(C Str &s) {
    if (_length = s.length()) {
        _d.setNum(length() + 1);
        Copy16To8(_d.data(), s(), _d.elms());
    }
}
Str ::Str(C Str8 &s) {
    if (_length = s.length()) {
        _d.setNum(length() + 1);
        Copy8To16(_d.data(), s(), _d.elms());
    }
}
Str8::Str8(C BStr &s) {
    if (_length = s.length()) {
        _d.setNum(length() + 1);
        Copy16To8(_d.data(), s(), length());
        _d[length()] = '\0';
    }
} // borrowed string may not be null-terminated
Str ::Str(C BStr &s) {
    if (_length = s.length()) {
        _d.setNum(length() + 1);
...
Str8::Str8(C Str8 &s, UInt extra_length) {
    _length = s.length();
    if (Int size = NewStrSize(length(), extra_length)) {
        _d.setNum(size);
        CopyFastN(_d.data(), s(), length());
        _d[length()] = '\0';
    }
} // always set NUL manually because 's' can be null
Str ::Str(C Str &s, UInt extra_length) {
    _length = s.length();
    if (Int size = NewStrSize(length(), extra_length)) {
        _d.setNum(size);
        CopyFastN(_d.data(), s(), length());
...
Str ::Str(C Str8 &s, UInt extra_length) {
    _length = s.length();
    if (Int size = NewStrSize(length(), extra_length)) {
        _d.setNum(size);
        Copy8To16(_d.data(), s(), length());
        _d[length()] = '\0';
    }
} // always set NUL manually because 's' can be null
...
Str8 &Str8::insert(Int i, C Str8 &text) {
    if (text.is()) {
        Clamp(i, 0, length());
        Int size = StrSizeAdd(length(), text.length());
        if (size > _d.elms()) {
            Mems<Char8> temp(StrSize(size, _d.elms()));
            CopyFastN(temp.data(), T(), i);                             // copy  text  before 'i'
...
Str8 &Str8::removeLast(Int num) {
    if (length() && num > 0)
        _d[MAX(_length -= num, 0)] = 0;
    return T;
}
Str &Str ::removeLast(Int num) {
    if (length() && num > 0)
        _d[MAX(_length -= num, 0)] = 0;
    return T;
}
...
Str8 &Str8::trim(Int pos, Int length) {
    clip(pos + length).remove(0, pos);
    return T;
}
Str &Str ::trim(Int pos, Int length) {
    clip(pos + length).remove(0, pos);
    return T;
}
/******************************************************************************/
Str8 &Str8::reserve(Int length) {
    if (length > 0) {
        if (length >= INT_MAX)
            Exit("'Str8.reserve' size too big"); // check size because below we do "size=length+1" which can overflow
...
Str &Str::reserve(Int length) {
    if (length > 0) {
        if (length >= INT_MAX)
            Exit("'Str.reserve' size too big"); // check size because below we do "size=length+1" which can overflow
        Int size = length + 1;                  // 1 extra for '\0'
        if (size > _d.elms())                   // increase only
        {
            _d.setNum(size, T.length()); // keep only characters that we have, instead of entire container
...
Str8 &Str8::reserveAdd(Int length) {
    Long new_length = Long(T.length()) + length;
    if (new_length > 0) {
        if (new_length > INT_MAX)
            Exit("'Str8.reserveAdd' size too big"); // check overflow because 'reserve' accepts 'length' as Int
        reserve(new_length);
    }
    return T;
}
Str &Str::reserveAdd(Int length) {
    ...
    if (new_length > 0) {
        if (new_length > INT_MAX)
            Exit("'Str.reserveAdd' size too big"); // check overflow because 'reserve' accepts 'length' as Int
        reserve(new_length);
    }
    return T;
}
...
static inline void Reserve(Str8 &s, Int length, Bool keep_nul = false) // !! does not set last char as NUL !!
{
    ...
    if (size > s._d.elms())
        s._d.setNum(StrSize(size, s._d.elms()), s.length() + keep_nul);
}
...
Str8 &Str8::operator+=(CChar8 *t) {
    if (Is(t)) {
        ...
        if (size > _d.elms()) {
            UIntPtr offset = t - T();
            if (offset < UIntPtr(length())) // if 't' is part of 'T'
            {
...
Str8 &Str8::operator+=(CChar *t) {
    if (Is(t)) {
        Int src_length = Length(t);
        Reserve(T, src_length);
...
Str &Str::operator+=(C BStr &s) {
    if (s.is()) {
        CChar *t = s();
        Int t_length = s.length(), size = StrSizeAdd(length(), t_length);
        if (size > _d.elms()) {
            UIntPtr offset = t - T();
            _d.setNum(StrSize(size, _d.elms()), length()); // +1 not needed because we append '\0' always
            if (offset < UIntPtr(length()))
                t = T() + offset; // if 't' is part of 'T'
        }
...
Str8 &Str8::operator=(SByte i) {
    Char8 temp[256];
    T = TextInt(Int(i), temp);
    return T;
}
Str8 &Str8::operator+=(SByte i) {
    Char8 temp[256];
...
Str8 &Str8::operator=(Byte u) {
    Char8 temp[256];
    T = TextInt(UInt(u), temp);
    return T;
}
Str8 &Str8::operator+=(Byte u) {
    Char8 temp[256];
...
Str8 &Str8::operator=(CPtr p) {
    Char8 temp[256];
    T = TextHex(UIntPtr(p), temp, SIZE(p) * 2, 0, true);
    return T;
}
Str8 &Str8::operator+=(CPtr p) {
    Char8 temp[256];
...
Str8 Str8::operator+(CChar8 *t) C {
    ...
    Int length = Length(t);
    Str8 s(T, length);
    CopyFastN(s._d.data() + s.length(), t, 1 + length);
    s._length += length;
    return s;
}
...
Str Str::operator+(CChar8 *t) C {
    ...
    Int length = Length(t);
    Str s(T, length);
    Set(s._d.data() + s.length(), t, 1 + length);
    s._length += length;
    return s;
}
...
Str Str8::operator+(C wchar_t *t) C {
    ...
    Int length = Length(t);
    Str s(T, length);
    _Set(s._d.data() + s.length(), t, 1 + length);
    s._length += length;
    return s;
}
...
void StrLibrary::create(C CMemPtr<Str> &strings, Bool case_sensitive, Bool paths) {
   ...
   cleaned.sort(case_sensitive ? CompareCS : CompareCI);
    ...
    REPA(cleaned) {
        if (HasWide(cleaned[i]))
            _size += SIZE(Char) * (cleaned[i].length() + 1);
        else
            _size += SIZE(Char8) * (cleaned[i].length() + 1);
    }
    _elms = cleaned.elms();
    alloc();
    UInt data_pos = 0;
    ...
    {
        if (HasWide(cleaned[i])) {
            Set((Char *)(_data + data_pos), cleaned[i], cleaned[i].length() + 1);
            _index[i] = data_pos ^ SIGN_BIT;
            data_pos += SIZE(Char) * (cleaned[i].length() + 1);
        } else {
            Set((Char8 *)(_data + data_pos), cleaned[i], cleaned[i].length() + 1);
            _index[i] = data_pos;
            data_pos += SIZE(Char8) * (cleaned[i].length() + 1);
        }
    }
}
...
Str StrLibrary::elm(Int i) C {
    if (InRange(i, elms())) {
        ...
        if (offset & SIGN_BIT)
            return (CChar *)(_data + (offset ^ SIGN_BIT));
...
void StrLibrary::putStr(File &f, C Str &str) C {
    ...
    if (_elms <= 0x0000FF)
        f.put(&found, 1);
    else if (_elms <= 0x00FFFF)
        f.put(&found, 2);
    else if (_elms <= 0xFFFFFF)
        f.put(&found, 3);
    else
        f.put(&found, 4);
    if (found < 0)
...
void StrLibrary::getStr(File &f, Str &str) C {
    ...
    if (InRange(index, _elms)) {
        ...
        if (offset & SIGN_BIT)
            str = (CChar *)(_data + (offset ^ SIGN_BIT));
        else
            str = (CChar8 *)(_data + offset);
    } else
        f.getStr(str);
}
/******************************************************************************/
...
Bool StrLibrary::load(File &f) {
    ...
    case 0: {
        Byte flag;
        f.getMulti(flag, _elms, _size);
        _case_sensitive = FlagOn(flag, 1);
        _paths = FlagOn(flag, 2);
        alloc();
...
// BORROWED STRING
/******************************************************************************/
BStr &BStr::set(C BStr &s) {
    T = s;
    return T;
}
BStr &BStr::setCustom(C Str &s) { return setCustom(s(), s.length()); }
BStr &BStr::setCustom(CChar *t, Int length) {
    if (length < 0)
        length = Length(t);
    del();
    if (length) {
        ...
        Char *d;
        Alloc(d, length);
        CopyFastN(d, t, length);
        T._d = d;
        T._length = length;
    }
    return T;
}
BStr &BStr::setBorrowed(CChar *t, Int length) {
    if (length < 0)
        length = Length(t);
    del();
    if (length) {
...
BStr &BStr::operator=(C BStr &src) {
    if (this != &src) {
        if (src._custom)
            setCustom(src(), src.length());
        else
            setBorrowed(src(), src.length());
    }
    return T;
}
...
static void GenMap() {
    ...
    FREP(65536) {
        ...
        if (l != i)
            dus.binaryInclude(VecI2(l, i));
        if (i != u)
            dus.binaryInclude(VecI2(i, u));
    }
...
static void InitStr() {
   ...
   REP(256)
   {
    ...
    // Case Up/Down
    {
        ...
        REPAO(_CaseUp) = i;
        REPA(DownUps) {
            U16 d = DownUps[i][0], u = DownUps[i][1];
            _CaseDown[u] = d;
            _CaseUp[d] = u;
        }
#endif
...
#define SET(l, h)                         \
    {                                     \
        _CaseDown[Unsigned(u##h)] = u##l; \
        _CaseUp[Unsigned(u##l)] = u##h;   \
    }
        SET('ß', 'ẞ');
#undef SET
    }
    ...
    {
#define SET(x)                       \
    CharOrder16[Unsigned(u##x)] = o; \
    o++
#define SET2(l, h)                   \
    CharOrder16[Unsigned(u##l)] = o; \
    CharOrder16[Unsigned(u##h)] = o; \
    o++
        U16 o = 0;
        SET('\0');
        SET('\r');
...
#undef SET
        ...
        for (Int i = 1; i < Elms(CharOrder16); i++)
            if (!CharOrder16[i])
                CharOrder16[i] = o++; // if the character doesn't have order set, then set (skip 0 '\0')
...
#define SET(c, f) _CharFlag[Unsigned(c)] |= f;
        ...
        for (Int i = '2'; i <= '9'; i++)
            SET(i, CHARF_DIG10 | CHARF_DIG16);
        for (Int i = 'a'; i <= 'f'; i++)
            SET(i, CHARF_DIG16);
        for (Int i = 'A'; i <= 'F'; i++)
            SET(i, CHARF_DIG16);
        REP(Elms(Stack) - 1)
        SET(Stack[i], CHARF_STACK); // -1 to skip null char
        REP(Elms(Combining) - 1)
        SET(Combining[i], CHARF_COMBINING); // -1 to skip null char
...
#if !USE_STD
        FREPA(Alphas)
        for (Int c = Alphas[i][0], to = Alphas[i][1]; c <= to; c++)
            _CharFlag[c] |= CHARF_ALPHA;
#endif
...
if(CaseUpFast  (c)!=c)f|=CHARF_DOWN; CHARF_DOWN is not defined*/
            ...
            if (!(f & CHARF_ALPHA) || c == '_')
                f |= CHARF_FONT_SPACE; // everything except characters, and always include '_'
            _CharFlag[i] = f;
        }
    }
...
#define SET(sl, su, dl, du)        \
    replaces.New().set(u##sl, dl); \
    replaces.New().set(u##su, du);
    SET('ą', 'Ą', 'a', 'A'); // polish
    SET('à', 'À', 'a', 'A');
    SET('á', 'Á', 'a', 'A');
    SET('â', 'Â', 'a', 'A');
...
#undef SET
    ...
    Str s;
    FREPA(replaces)
    s += S + "   {u'" + replaces[i].src + "', '" + replaces[i].dest + "'},\n";
    ClipSet(s);
#endif
}
Char RemoveAccent(Char c) {
    if (CharReplace *cr = BinaryFind(CharReplaces, Elms(CharReplaces), c, CharReplace::Compare))
        return cr->dest;
    return c;
}
Font &Font::removeAccent(Bool permanent) {
    REPA(CharReplaces)
    replace(CharReplaces[i].src, CharReplaces[i].dest, permanent);
    return T;
}
/******************************************************************************/
#if APPLE
    ...
    if (str)
        if (NSString *s = (HasUnicode(str) ? [str precomposedStringWithCanonicalMapping] : str)) {
            ...
            if (Int length = [s length]) {
                ASSERT(SIZE(unichar) == SIZE(Char));
                temp.reserve(length);
                [s getCharacters:(unichar *)temp()];
                temp._d[temp._length = length] = '\0';
            }
...
Str AppleString(CFStringRef str) {
    if (str)
        if (Int length = CFStringGetLength(str)) {
            ...
            Str temp;
            temp.reserve(length);
            CFStringGetCharacters(str, CFRangeMake(0, length), (UniChar *)temp());
            temp._d[temp._length = length] = '\0';
            return temp;
        }
    return S;
    ...
    if (str && T)
        ...
        if (Int length = T->GetStringLength(str))
            if (C jchar *text = T->GetStringChars(str, null)) {
                ...
                FREP(length)
                s += ((CChar *)text)[i];
                T->ReleaseStringChars(str, text);
                return s;
            }
#else
        if (CChar8 *utf = T->GetStringUTFChars(str, null)) {
            C Str &s = FromUTF8(utf);
            T->ReleaseStringUTFChars(str, utf);
            return s;
        }
#endif
    return S;
