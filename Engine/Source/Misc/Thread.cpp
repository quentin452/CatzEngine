...
#if !X64
Long AtomicGet(C Long &x) {
    Long old = x;
    return _InterlockedCompareExchange64((LONG64 *)&x, old, old);
...
ULong AtomicGet(C ULong &x) {
    ULong old = x;
    return _InterlockedCompareExchange64((LONG64 *)&x, old, old);
} // this version was the fastest

void AtomicSet(Long &x, Long y) {
    Long old;
    do
        old = x;
    while (_InterlockedCompareExchange64((LONG64 *)&x, y, old) != old);
}
void AtomicSet(ULong &x, ULong y) {
    ULong old;
    do
        old = x;
    while (_InterlockedCompareExchange64((LONG64 *)&x, y, old) != old);
...
Long AtomicGet(C Long &x) {
    Long old = x;
    return __sync_val_compare_and_swap(&ConstCast(x), old, old);
} // 'ConstCast' is used to mute a warning, it can be used because 'x' will be modified to 'old' only if it's equal to 'old' already
...
void AtomicSet(Long &x, Long y) {
    ...
    do
        old = x;
    while (!__sync_bool_compare_and_swap(&x, old, y));
}
void AtomicSet(ULong &x, ULong y) {
    ULong old;
    do
...
#pragma pack(pop)
    ...
    __try {
        RaiseException(0x406D1388, 0, SIZE(info) / SIZE(ULONG_PTR), (ULONG_PTR *)&info);
...
SyncLock::SyncLock() {
    ...
    ex:
        ...
        if (!InitializeCriticalSectionExTried) {
            if (HMODULE kernel = GetModuleHandle(L"Kernel32.dll"))
                InitializeCriticalSectionEx = (decltype(InitializeCriticalSectionEx))GetProcAddress(kernel, "InitializeCriticalSectionEx"); // available on Vista+
...
#if MUTEX_MANUAL_RECURSIVE
        if (!owned()) {
            auto result = pthread_mutex_lock(&_lock);
            DEBUG_ASSERT(!result, "pthread_mutex_lock");
            _owner = GetThreadID();
        }
        _lock_count++;
#else
...
void SyncLock::off() C {
...
#endif
    {
        ...
        if (_lock_count <= 0) {
            _owner = 0;
            auto result = pthread_mutex_unlock(&_lock);
            DEBUG_ASSERT(!result, "pthread_mutex_unlock");
        }
#else
        _lock_count--;
        if (_lock_count <= 0)
...
void SyncEvent::on() C { /*if(_handle)*/
    SetEvent(_handle);
} // checking for handle!=null is not needed, as   'SetEvent'          will do nothing on null
void SyncEvent::off() C { /*if(_handle)*/
    ResetEvent(_handle);
} // checking for handle!=null is not needed, as 'ResetEvent'          will do nothing on null
Bool SyncEvent::wait() C { /*if(_handle)*/
    return WaitForSingleObject(_handle, INFINITE) != WAIT_TIMEOUT;
    return true;
} // checking for handle!=null is not needed, as 'WaitForSingleObject' will return -1  on null, WAIT_TIMEOUT=258 so result will be -1!=258 -> true
Bool SyncEvent::wait(Int milliseconds) C { /*if(_handle)*/
    return WaitForSingleObject(_handle, (milliseconds >= 0) ? milliseconds : INFINITE) != WAIT_TIMEOUT;
    return true;
} // checking for handle!=null is not needed, as 'WaitForSingleObject' will return -1  on null, WAIT_TIMEOUT=258 so result will be -1!=258 -> true
#else
SyncEvent::SyncEvent(Bool auto_off) {
    ...
    Alloc(_handle);
    DYNAMIC_ASSERT(!pthread_cond_init(_handle, &attr), "pthread_cond_init");
#else
    Alloc(_handle);
    DYNAMIC_ASSERT(!pthread_cond_init(_handle, null), "pthread_cond_init");
#endif
    Alloc(_mutex);
    DYNAMIC_ASSERT(!pthread_mutex_init(_mutex, null), "pthread_mutex_init");
}
SyncEvent::~SyncEvent() {
    if (_handle) {
...
void SyncEvent::on() C {
    if (_handle) {
        ...
        if (_auto_off)
            ...
            pthread_cond_broadcast(_handle); // _auto_off ? wake up 1 : wake up all
        pthread_mutex_unlock(_mutex);
    }
}
...
Bool SyncEvent::wait() C {
    if (_handle) {
        pthread_mutex_lock(_mutex);
        for (; !_condition && !pthread_cond_wait(_handle, _mutex);)
            ; // check for '_condition' first in case it's already met to avoid 'pthread_cond_wait' overhead, have to check in a loop because 'pthread_cond_wait' may return if OS interrupted it, 'pthread_cond_wait' automatically unlocks and locks the mutex
        Bool ok = _condition;
        if (_auto_off)
            _condition = false;
        pthread_mutex_unlock(_mutex);
    ...
    if (_handle) {
        ...
        if (!_condition) // if condition not met yet
        {
            if (milliseconds < 0) // infinite wait
            {
                for (; !_condition && !pthread_cond_wait(_handle, _mutex);)
                    ;                    // check for '_condition' first in case it's already met to avoid 'pthread_cond_wait' overhead, have to check in a loop because 'pthread_cond_wait' may return if OS interrupted it, 'pthread_cond_wait' automatically unlocks and locks the mutex
            } else if (milliseconds > 0) // timed wait
            {
                timespec ts;
                clock_gettime(USE_MONOTONIC_CLOCK ? CLOCK_MONOTONIC : CLOCK_REALTIME, &ts);
                ...
                for (; !_condition && !pthread_cond_timedwait(_handle, _mutex, &ts);)
                    ; // 'pthread_cond_timedwait' automatically unlocks and locks the mutex, keep waiting in the loop in case it returns multiple times before the end of the time, but somehow with the condition still set to false, in that case keep waiting still as long as it returns success and not timeout or other error
            }
        }
        Bool ok = _condition;
        if (_auto_off)
...
void SyncCounter::operator+=(Int count) C { /*if(_handle)*/
    ReleaseSemaphore(_handle, count, null);
} // checking for handle!=null is not needed, as 'ReleaseSemaphore'    will do nothing on null
Bool SyncCounter::wait() C { /*if(_handle)*/
    return WaitForSingleObject(_handle, INFINITE) != WAIT_TIMEOUT;
    return true;
} // checking for handle!=null is not needed, as 'WaitForSingleObject' will return -1  on null, WAIT_TIMEOUT=258 so result will be -1!=258 -> true
Bool SyncCounter::wait(Int milliseconds) C { /*if(_handle)*/
    return WaitForSingleObject(_handle, (milliseconds >= 0) ? milliseconds : INFINITE) != WAIT_TIMEOUT;
    return true;
} // checking for handle!=null is not needed, as 'WaitForSingleObject' will return -1  on null, WAIT_TIMEOUT=258 so result will be -1!=258 -> true
#else
...
void SyncCounter::operator+=(Int count) C {
    if (_handle) {
        pthread_mutex_lock(_mutex);
        _counter = Mid(Long(_counter) + count, (Long)INT_MIN, (Long)INT_MAX);
        if (count > 1)
            pthread_cond_broadcast(_handle);
        else
            pthread_cond_signal(_handle); // clamp to prevent overflow, if adding more than 1 then call 'pthread_cond_broadcast' to unlock all waiting threads, 'pthread_cond_signal' will unlock only 1
        pthread_mutex_unlock(_mutex);
    }
}
Bool SyncCounter::wait() C {
    if (_handle) {
        pthread_mutex_lock(_mutex);
        for (; _counter <= 0 && !pthread_cond_wait(_handle, _mutex);)
            ; // check for '_condition' first in case it's already met to avoid 'pthread_cond_wait' overhead, have to check in a loop because 'pthread_cond_wait' may return if OS interrupted it, 'pthread_cond_wait' automatically unlocks and locks the mutex
        Bool ok = false;
        if (_counter > 0) {
            _counter--;
    ...
    if (_handle) {
        ...
        if (_counter <= 0) // if condition not met yet
        {
            ...
            } else if (milliseconds > 0) // timed wait
            {
                ...
                // 'ts' specifies the end time at which waiting always fails (this is the "time position" and not "time duration")
                for (; _counter <= 0 && !pthread_cond_timedwait(_handle, _mutex, &ts);)
                    ; // 'pthread_cond_timedwait' automatically unlocks and locks the mutex, keep waiting in the loop in case it returns multiple times before the end of the time, but somehow with the condition still set to false, in that case keep waiting still as long as it returns success and not timeout or other error
            }
        }
        Bool ok = false;
...
void ReadWriteSync::enterRead() {
    ...
    if (_locks.elms()) // this is safe because we're interested only in locks from this thread, so if this thread has made a lock before, then it will be available here without the need of enabling '_locks_lock'
    {
        ...
        REPA(_locks) // iterate all locks
        {
            ...
            if (lock.thread_id == thread_id) {
                ...
                _locks_lock.off();
                return;
            } // if found an existing for this thread, then increase the lock counter, unlock, and return immediately without unlocking again below
        }
    ...
    l.locks = 1; // even though we've temporarily unlocked '_locks_lock' above, we can always create a new lock here, without checking again if there's already listed, because even if other thread will add a new lock, it will always be a different thread's lock
    _locks_lock.off();
    _write_lock.off();
}
...
again:
    if (_locks.elms()) {
        _locks_lock.on();
        REPA(_locks)
        if (_locks[i].thread_id != thread_id) // if found at least one reader from another thread
        {
...
void SimpleReadWriteSync::enterWrite() C {
    _lock.on();
    for (; AtomicGet(_readers);)
        _finished.wait(); // wait for readers to finish
}
...
void Thread::func() {
    ...
    if (_name.is()) {
        SetThreadName(_name);
        _name.del();
    } // delete because it's not needed anymore
#endif
again:
...
static void _SetThreadPriority(PLATFORM(HANDLE, pthread_t) handle, Int priority) {
    ...
    SetThreadPriority(handle, (priority < -2) ? THREAD_PRIORITY_IDLE : (priority > 2) ? THREAD_PRIORITY_TIME_CRITICAL
                                                                                      : priority);
#elif 0
    sched_param param;
    param.sched_priority = PriorityBase + PriorityRange * (priority + 3) / 6; // div by 6 because "priority==3" should give max
...
#if !WINDOWS
            for (; active();)
                Yield(); // !! wait until '_active' was actually disabled, which means that 'Thread.func' has exited, without this wait we could potentially delete the thread and release its memory, while the thread func was still running and setting '_active' to false !!
#endif
        }
#if WINDOWS
        CloseHandle(_handle);
    ...
    if (active()) {
        ...
        else // single step
        {
            ...
            for (UInt start = Time.curTimeMs();;) {
                EmulatedThreads.update();
                if (!active() || (Time.curTimeMs() - start) >= time)
    ...
    if (!pthread_attr_init(&attr)) {
        ...
        if (!pthread_attr_setstacksize(&attr, STACK_SIZE))
            if (!pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED)) // create as detached to auto-release system resources without having to call 'pthread_join'
            {
                ...
                if (!pthread_create(&_handle, &attr, ThreadFunc, this))
                    ok = true;
            }
...
Thread::Thread(Bool func(Thread &thread), Ptr user, Int priority, Bool paused) {
    zero();
    create(func, user, priority, paused);
}
/******************************************************************************/
// THREADS
/******************************************************************************/
...
static Bool ThreadsFunc(Threads::ThreadEx &thread) {
    ...
    process:
        ...
        if (index > 0) {
            ...
            case TF_INDEX:
                ...
                break;
            case TF_DATA:
                data = (Byte *)threads._func_data + index * threads._elm_size;
                break;
            case TF_DATA_PTR:
                data = *(Ptr *)((Byte *)threads._func_data + index * threads._elm_size);
                break;
            case TF_MEMB:
                data = (*threads._func_memb)[index];
                break;
            case TF_MEMX:
                data = (*threads._func_memx)[index];
                break;
            }
        ...
        if (processed)
            if (AtomicAdd(threads._processed, processed) // set what was processed by this thread, do this in just one atomic operation (and not many times inside the loop)
                    + processed >=
                threads._elms)          // add the 'AtomicAdd' result (which is the old value of 'threads._processed') and 'processed' to get the total number of elements processed at this time
    ...
    if (threads.callsLeft()) {
        ...
        if (threads.callsLeft()) {
            if (threads._ordered) {
                thread.call = threads._calls[threads._calls_pos++];
                threads.checkEnd();
            } else {
                thread.call = threads._calls.last();
                threads._calls.removeLast();
            }
            ...
            if (Int waiting = AtomicGet(threads._waiting))
                threads._queued_finished += waiting; // notify of finished processing only if there are any waiting threads (to avoid overhead of +=), this is needed for 'Threads.wait', this is not perfect because there is a small possibility that when 2 threads are waiting, 1 of them would quickly consume both notifications
...
void Threads::createIfEmpty(Bool ordered, Int threads, Int priority, C Str8 &name) {
    if (!created()) {
        SyncLocker locker(_lock_process);
        if (!created())
            create(ordered, threads, priority, name);
    }
}
/******************************************************************************/
...
Int Threads::activeThreads() C {
    Int paused = 0;
    REPA(_threads)
    paused += _threads[i].wantPause();
    return threads() - paused;
}
Threads &Threads::activeThreads(Int active) {
    Clamp(active, 0, threads());
    for (Int i = active; i < threads(); i++)
        _threads[i].pause(); // pause unwanted threads first
    REP(active)
    _threads[i].resume(); // resume  wanted threads after
    return T;
}
Int Threads::priority() C { return _threads.elms() ? _threads[0].priority() : 0; }
Threads &Threads::priority(Int priority) {
    REPAO(_threads).priority(priority);
    return T;
}
Threads &Threads::mask(ULong mask) {
    REPAO(_threads).mask(mask);
    return T;
}
/******************************************************************************/
...
void Threads::_process(Int elms, void func(IntPtr elm_index, Ptr user, Int thread_index), Ptr user, Int max_threads, Bool allow_processing_on_this_thread) {
    ...
    if (max_threads < 0)
        ...
        MIN(max_threads, threads() + allow_processing_on_this_thread);
    MIN(max_threads, elms);
    if (max_threads > 1) {
        SyncLocker locker(_lock_process); // this allows multiple 'process' calls on multiple threads
        _func_mode = TF_INDEX;
        _func = (void (*)(Ptr data, Ptr user, Int thread_index))func;
        _func_user = user;
        _elms = elms;
        AtomicSet(_left, _elms); // set '_left' as last
        ...
        process:
            ...
            if (processed)
                AtomicAdd(_processed, processed); // set what was processed by this thread, do this in just one atomic operation (not inside the loop)
        }

        // wait until all finished
        for (; _processed < _elms;)
            _finished.wait();
        _elms = 0;
        _processed = 0; // other members don't need to be cleared
    } else {
#endif
        FREP(elms)
        func(i, user, 0);
...
void Threads::_process(Ptr data, Int elms, Int elm_size, void func(Ptr data, Ptr user, Int thread_index), Ptr user, Int max_threads, Bool allow_processing_on_this_thread, Bool data_ptr) {
    ...
    if (max_threads > 1) {
        SyncLocker locker(_lock_process); // this allows multiple 'process' calls on multiple threads
        _func_mode = (data_ptr ? TF_DATA_PTR : TF_DATA);
        _func = func;
        _func_data = data;
        _func_user = user;
        _elm_size = elm_size;
        ...
        process:
            ...
            if (index > 0) {
                index = _elms - index; // process elements starting from zero index
                Ptr d = (Byte *)data + index * elm_size;
                if (data_ptr)
                    d = *(Ptr *)d;
...
{
    if (_calls_pos >= _calls.elms()) {
        _calls.clear();
        _calls_pos = 0;
    } // if none are left, then clear, this is important because "callsLeft()" can be simplified thanks to this
}
/******************************************************************************/
// !! 'elm_index' MUST BE IntPtr and not Int because we're casting to '_func' of Ptr type !!
        ...
        if (_threads.elms()) {
            {
                ...
                free();
                Int start = _calls.addNum(elms);
                REP(elms)
                _calls[start + i]._set(Ptr(i), (void (*)(Ptr data, Ptr user, Int thread_index))func, user);
            }
...
Threads &Threads::cancel() {
    if (_calls.elms()) {
        ...
        _calls_pos = 0;
        if (_waiting)
            _queued_finished += _waiting; // access '_waiting' only during lock !! if there's any thread waiting then notify of potential finish
    }
    return T;
}
Int Threads::_cancel(void func(Ptr data, Ptr user, Int thread_index)) {
    ...
    if (_calls.elms()) {
        SyncLocker locker(_lock_calls);
        for (Int i = _calls.elms(); --i >= _calls_pos;)
            if (_calls[i].func == func) {
                _calls.remove(i, _ordered);
                canceled++;
            }
        if (canceled) {
            checkEnd();
...
Int Threads::cancel(C CMemPtr<Call> &calls) {
    Int canceled = 0;
    if (_calls.elms() && calls.elms()) {
        SyncLocker locker(_lock_calls);
        for (Int i = _calls.elms(); --i >= _calls_pos;) // iterate all queued calls
        {
            C Call &call = _calls[i];
            REPAD(j, calls)
            if (call == calls[j]) // if this call is present on the list to cancel
            {
                _calls.remove(i, _ordered);
                canceled++;
                break;
...
again:
    if (callsLeft()) {
        SyncLockerEx locker(_lock_calls);
        if (callsLeft()) {
            if (_ordered) {
                call = _calls[_calls_pos++];
                checkEnd();
            } else {
                call = _calls.last();
                _calls.removeLast();
            ...
            if (Int waiting = AtomicGet(_waiting))
                _queued_finished += waiting; // notify of finished processing only if there are any waiting threads (to avoid overhead of +=), this is needed for 'Threads.wait', this is not perfect because there is a small possibility that when 2 threads are waiting, 1 of them would quickly consume both notifications
            goto again;
        }
    }
...
Int Threads::queued() C {
    if (_threads.elms()) {
        SyncLocker locker(_lock_calls);
        Int queued = _calls.elms() - _calls_pos;
        REPA(_threads)
        queued += _threads[i].call.is();
        return queued;
    }
    return 0;
}
...
struct MTFC {
    ...
    static Bool Func(Thread &thread) {
        ...
        for (Int thread_index = mtfc.threads.index(&thread);;) {
            ...
            if (InRange(i, mtfc.elms))
                mtfc.func(i, mtfc.user, thread_index);
            else
                break;
        }
        return false;
...
struct MTFC_Ptr // MTFC for Ptr
{
    ...
    MTFC_Ptr(Ptr data, Int elm_size, void func(Ptr data, Ptr user, Int thread_index), Ptr user) {
        T.data = (Byte *)data;
        T.elm_size = elm_size;
        T.func = func;
        T.user = user;
...
struct MTFC_Memb // MTFC for Memb
{
    ...
    MTFC_Memb(_Memb &data, void func(Ptr data, Ptr user, Int thread_index), Ptr user) : data(data) {
        T.func = func;
        T.user = user;
    }
...
struct MTFC_Memx // MTFC for Memx
{
    ...
    MTFC_Memx(_Memx &data, void func(Ptr data, Ptr user, Int thread_index), Ptr user) : data(data) {
        T.func = func;
        T.user = user;
    }
...
void _MultiThreadedCall(Ptr data, Int elms, Int elm_size, void func(Ptr data, Ptr user, Int thread_index), Ptr user, Int threads, Bool data_ptr) {
    MTFC_Ptr mtfc(data, elm_size, func, user);
    MultiThreadedCall(elms, data_ptr ? *MTFC_Ptr ::FuncPtr : *MTFC_Ptr ::Func, mtfc, threads);
}
void _MultiThreadedCall(_Memb &data, void func(Ptr data, Ptr user, Int thread_index), Ptr user, Int threads, Bool data_ptr) {
    MTFC_Memb mtfc(data, func, user);
    MultiThreadedCall(data.elms(), data_ptr ? *MTFC_Memb::FuncPtr : *MTFC_Memb::Func, mtfc, threads);
}
void _MultiThreadedCall(_Memx &data, void func(Ptr data, Ptr user, Int thread_index), Ptr user, Int threads, Bool data_ptr) {
    MTFC_Memx mtfc(data, func, user);
    MultiThreadedCall(data.elms(), data_ptr ? *MTFC_Memx::FuncPtr : *MTFC_Memx::Func, mtfc, threads);
}
/******************************************************************************/
// THREAD EMULATION
/******************************************************************************/
ThreadEmulation::ThreadEmulation() {
    _process_left = 0;
    _process_type = _time = 0;
}
...
void ThreadEmulation::exclude(Thread &thread) {
    if (thread.sleep()) {
        REPA(_delayed_threads)
        if (_delayed_threads[i].thread == &thread) {
            if (_process_type == 1 && i < _process_left)
                _process_left--; // if this element was going to be processed soon
            _delayed_threads.remove(i, true);
            break;
        }
    } else {
        REPA(_rt_threads)
        if (_rt_threads[i] == &thread) {
            if (_process_type == 0 && i < _process_left)
                _process_left--; // if this element was going to be processed soon
