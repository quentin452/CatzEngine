...
#if WINDOWS_OLD
    wchar_t temp[16 * 1024];
    temp[0] = '\0';
    GetWindowText(window, temp, Elms(temp));
    return temp;
#elif MAC
    if (window)
        return [window title]; // do not release [window title] as it will crash
#elif LINUX
    if (XDisplay && window) {
        Str s;
        if (_NET_WM_NAME && UTF8_STRING) {
            ...
            if (!XGetWindowProperty(XDisplay, window, _NET_WM_NAME, 0, 4096, false, UTF8_STRING, &type, &format, &items, &bytes_after, &data))
        ...
        if (!s.is()) {
            char *name = null;
            XFetchName(XDisplay, window, &name);
            if (name) {
                s = name;
                XFree(name);
            }
...
void SysWindow::minimize(Bool force) C {
    ...
    if (force)
        ...
        PostMessage(window, WM_SYSCOMMAND, SC_MINIMIZE, NULL);
#elif WINDOWS_NEW
    if (T == App.window()) {
        /* TODO:
...
#elif LINUX
    if (XDisplay && window)
        XIconifyWindow(XDisplay, window, DefaultScreen(XDisplay));
#elif ANDROID
    if (T == App.window())
        App.hide();
#endif
...
void SysWindow::maximize(Bool force) C {
    ...
    if (force)
        ...
        PostMessage(window, WM_SYSCOMMAND, SC_MAXIMIZE, NULL);
#elif MAC
    if (window) {
        ...
        if (!XGetWindowProperty(XDisplay, window, _NET_WM_STATE, 0, 1024, false, XA_ATOM, &type, &format, &items, &bytes_after, &data)) {
            ...
            if (items < Elms(temp) - 2) // room for _NET_WM_STATE_MAXIMIZED_HORZ, _NET_WM_STATE_MAXIMIZED_VERT
            {
                ...
                for (unsigned long i = 0; i < items; i++) {
                    temp[i] = atoms[i];
                    if (temp[i] == _NET_WM_STATE_MAXIMIZED_HORZ)
                        flags |= 1;
                    else if (temp[i] == _NET_WM_STATE_MAXIMIZED_VERT)
                        flags |= 2;
                }
                if (flags != 3) {
                    if (!(flags & 1))
                        temp[items++] = _NET_WM_STATE_MAXIMIZED_HORZ;
                    if (!(flags & 2))
                        temp[items++] = _NET_WM_STATE_MAXIMIZED_VERT;
...
void SysWindow::reset(Bool force) C {
    ...
    if (force)
        ...
        PostMessage(window, WM_SYSCOMMAND, SC_RESTORE, NULL);
#elif MAC
    if (window) {
        if ([window isMiniaturized])
            activate();
        ...
        if (T == App.window()) {
            ...
            if (!XGetWindowProperty(XDisplay, window, _NET_WM_STATE, 0, 1024, false, XA_ATOM, &type, &format, &items, &bytes_after, &data)) {
                ...
                if (items < Elms(temp)) {
                    ...
                    for (unsigned long i = 0; i < items; i++)
                        if (atoms[i] != _NET_WM_STATE_MAXIMIZED_HORZ && atoms[i] != _NET_WM_STATE_MAXIMIZED_VERT)
                            temp[new_items++] = atoms[i];

                    if (new_items != items)
                        XChangeProperty(XDisplay, window, _NET_WM_STATE, XA_ATOM, 32, PropModeReplace, (unsigned char *)temp, new_items);
                }
            }
            if (data)
                XFree(data);
...
void SysWindow::activate() C {
    ...
    if (cur_thread != act_thread)
        AttachThreadInput(act_thread, cur_thread, true);
    BringWindowToTop(window);
    SetForegroundWindow(window);
    if (cur_thread != act_thread)
        AttachThreadInput(act_thread, cur_thread, false);
#elif MAC
    if (T == App.window()) {
        ProcessSerialNumber psn;
        if (GetProcessForPID(App.processID(), &psn) == noErr)
            SetFrontProcess(&psn);
    }
#elif LINUX
    if (XDisplay && window) {
...
#elif LINUX
    if (XDisplay && window)
        XMoveWindow(XDisplay, window, x, y);
#endif
}
/******************************************************************************/
...
VecI2 SysWindow::size(Bool client) C {
    ...
    if (XDisplay && window) {
        ...
        if (XGetWindowAttributes(XDisplay, window, &attr) == true) {
            if (!client) {
                attr.width += App._bound.w();
                attr.height += App._bound.h();
            }
...
error:
    ...
    if (window) {
        ...
        if (client)
            rect = [window contentRectForFrameRect:rect];
        RectI r;
        r.min.x = Round(rect.origin.x);
        r.max.x = r.min.x + Round(rect.size.width);
        r.max.y = D.screenH() - Round(rect.origin.y);
        r.min.y = r.max.y - Round(rect.size.height);
        return r;
    }
#elif LINUX
    if (XDisplay && window) {
...
#endif
            for (; window = window.parent();)
                if (XGetWindowAttributes(XDisplay, window, &attr) == true)
                    r += VecI2(attr.x, attr.y);
#if 1
            if (!client) {
                r.min += App._bound.min;
                r.max += App._bound.max;
            }
#endif
            return r;
        }
    ...
    if (XDisplay && window && _NET_WM_STATE) {
        ...
        if (!XGetWindowProperty(XDisplay, window, _NET_WM_STATE, 0, 1024, false, XA_ATOM, &type, &format, &items, &bytes_after, &data))
            if (Atom *atoms = (Atom *)data)
                for (unsigned long i = 0; i < items; i++) {
                    if (atoms[i] == _NET_WM_STATE_MAXIMIZED_HORZ)
                        flags |= 1;
...
SysWindow SysWindow::parentTop() C {
    SysWindow window = T;
    for (; SysWindow parent = window.parent();)
        window = parent;
    return window;
}
/******************************************************************************/
...
SysWindow WindowActive() {
    ...
    if (XDisplay) {
        XWindow window;
        int revert_to;
        if (XGetInputFocus(XDisplay, &window, &revert_to) == True)
            return XmuClientWindow(XDisplay, SysWindow(window).parentTop());
    }
#endif
...
void WindowList(MemPtr<SysWindow> windows) {
    if (App.window())
        windows.setNum(1).first() = App.window();
    else
        windows.clear();
}
#endif
            ...
            // if(HBITMAP src_bitmap=(HBITMAP)GetCurrentObject(src_dc, OBJ_BITMAP))
            {
                // BITMAP bmp; if(GetObject(src_bitmap, SIZE(bmp), &bmp))
                {
                    ...
                    if (width != size.x || height != size.y) {
                        ...
                        if (bitmap = CreateCompatibleBitmap(src_dc, width, height)) {
                    ...
                    if (bitmap && BitBlt(dc, 0, 0, size.x, size.y, src_dc, 0, 0, SRCCOPY)) {
                        if (image.hwType() != IMAGE_B8G8R8A8_SRGB || image.size() != size || image.d() != 1 || image.pitch() != image.w() * image.bytePP())
                            image.createSoft(size.x, size.y, 1, IMAGE_B8G8R8A8_SRGB);
...
{
    Memc<Str> &candidate = (Kb._imm_candidate_hidden ? Kb._imm_candidate_temp : Kb._imm_candidate);
    candidate.clear();
    Int size = ImmGetCandidateList(Kb._imc, 0, null, 0);
    if (size > 0) {
        Memt<Byte> temp;
        CANDIDATELIST *list = (CANDIDATELIST *)temp.setNumZero(size).data();
        ImmGetCandidateList(Kb._imc, 0, list, size);
        if (list->dwStyle == IME_CAND_CODE) {
            if (list->dwCount == 1)
                candidate.New() = (Char)list->dwOffset[0];
            else if (list->dwCount > 1) {
            } // text representations of individual DBCS character values in hexadecimal notation ?
        } else if (list->dwPageSize) {
...
static void Pause(Bool pause) {
    if (pause != (PauseMode != NOT_PAUSED))
        switch (PauseMode) {
        case NOT_PAUSED: // is not paused, pause now
        {
            if (App.flag & APP_NO_PAUSE_ON_WINDOW_MOVE_SIZE) {
                Time.skipUpdate();
                PauseTimer = SetTimer(App.window(), 1, USER_TIMER_MINIMUM, null);
                PauseMode = PAUSED_TIMER;
            } else {
                PauseMode = (App.active() ? PAUSED_WAS_ACTIVE : PAUSED_WAS_INACTIVE);
                App.setActive(false);
            }
        } break;

        case PAUSED_TIMER:
            KillTimer(App.window(), PauseTimer);
            PauseTimer = 0;
            PauseMode = NOT_PAUSED;
            break;

        case PAUSED_WAS_ACTIVE:
            App.setActive(true); // !! no break on purpose !!
        case PAUSED_WAS_INACTIVE:
            PauseMode = NOT_PAUSED;
            break;
        }
}
...
static void ResetCursor() {
    Ms.resetCursor();
    if (ResetCursorCounter) {
        ResetCursorCounter--;
        App.includeFuncCall(ResetCursor);
    }
} // calling immediately may not have any effect, we have to try a few times

static void ConditionalDraw() {
    if (!(App.active() || (App.flag & APP_WORK_IN_BACKGROUND)))
        DrawState(); // draw only if will not draw by itself
}
static LRESULT CALLBACK WindowMsg(HWND window, UInt msg, WPARAM wParam, LPARAM lParam) {
   ...
   switch(msg)
   {
    ...
    case WM_EXITSIZEMOVE:
        ...
        D.getScreenInfo();
        D.setColorLUT();
        break; // called when (finished dragging by title bar or resizing by edge/corner, snapped by User), NOT called when (maximized, snapped by OS), call 'setColorLUT' in case moved to another monitor with different color profile
    ...
    case WM_DISPLAYCHANGE:
        if (D.initialized()) // needed only if device already initialized (to skip setting mouse cursor and screen size when initializing)
        {
            // VecI2 res(LOWORD(lParam), HIWORD(lParam)); // this is resolution of the primary monitor and not the one that's changing
            ResetCursorCounter = 8;
            App.includeFuncCall(ResetCursor); // it was noticed that after changing resolution, Windows will rescale current cursor, to prevent that, we need to reset it, calling immediately may not have any effect, we have to try a few times
            D.getScreenInfo();
            if (auto screen_changed = D.screen_changed)
                screen_changed(D.w(), D.h()); // if 'D.scale' is set based on current screen resolution, then we may need to adjust it
        }
        break;
    ...
    case WM_SYSCOMMAND:
        ...
        case SC_SIZE:
            return 0;
        case SC_MINIMIZE:
            if (!(App.flag & APP_MINIMIZABLE))
                return 0;
            break;
        case SC_MAXIMIZE:
            if (!(App.flag & APP_MAXIMIZABLE))
                return 0;
            break;
    ...
    case WM_POWERBROADCAST:
        ...
        case PBT_APMQUERYSUSPEND:
            return App._stay_awake ? BROADCAST_QUERY_DENY : true; // system asks if it's OK to sleep

        case PBT_APMSUSPEND:         // suspending
        case PBT_APMRESUMEAUTOMATIC: // resuming
        {
            if (auto sleep = App.sleep)
                sleep(wParam == PBT_APMSUSPEND); // copy first to avoid multi-thread issues
        } break;
        }
        break;
    ...
    case WM_MBUTTONDOWN:
        ...
        return 0;
    case WM_XBUTTONDOWN:
        Ms.push((GET_XBUTTON_WPARAM(wParam) & XBUTTON1) ? 3 : 4);
        return 0; // events are reported only when clicking on window client
    ...
    {
        if (Ms._on_client = (LOWORD(lParam) == HTCLIENT)) {
            Ms.resetCursor();
            return true;
        } // call this here to make sure that we have correct cursor set, in 'Mouse.update' there's also '_on_client' modification, because this isn't called when mouse goes outside the window, return true only when on client, so for example at the window edge, system resize cursor can be assigned by default functions
    } break;

#define WM_MOUSEHWHEEL 0x020E
    case WM_MOUSEHWHEEL:
        Ms._wheel.x += Flt(GET_WHEEL_DELTA_WPARAM(wParam)) / WHEEL_DELTA;
        break;
    case WM_MOUSEWHEEL:
        Ms._wheel.y += Flt(GET_WHEEL_DELTA_WPARAM(wParam)) / WHEEL_DELTA;
        break;
    ...
    case WM_INPUT: {
        ...
        if (GetRawInputData((HRAWINPUT)lParam, RID_INPUT, temp.data(), &size, sizeof(RAWINPUTHEADER)) == size) {
            RAWINPUT &raw = *(RAWINPUT *)temp.data();
            ...
            case RIM_TYPEKEYBOARD: {
                ...
                case VK_CONTROL:
                    if (raw.data.keyboard.MakeCode != 29)
                        return 0;
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_RCTRL : KB_LCTRL);
                    break; // 29=KB_LCTRL and KB_RCTRL (this must be checked because VK_CONTROL also gets triggered by AltGr)
                case VK_MENU:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_RALT : KB_LALT);
                    break;
                case VK_SHIFT:
                    switch (raw.data.keyboard.MakeCode) // 42=KB_LSHIFT, 54=KB_RSHIFT, 75=triggered when pressing Shift+Numpad
                    {
                    case 42:
                        key = KB_LSHIFT;
                        break;
                    ...
                    break;
                case VK_INSERT:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_INS : KB_NP0);
                    break;
                case VK_END:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_END : KB_NP1);
                    break;
                case VK_DOWN:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_DOWN : KB_NP2);
                    break;
                case VK_NEXT:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_PGDN : KB_NP3);
                    break;
                case VK_LEFT:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_LEFT : KB_NP4);
                    break;
                case VK_CLEAR:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_NONE : KB_NP5);
                    break;
                case VK_RIGHT:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_RIGHT : KB_NP6);
                    break;
                case VK_HOME:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_HOME : KB_NP7);
                    break;
                case VK_UP:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_UP : KB_NP8);
                    break;
                case VK_PRIOR:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_PGUP : KB_NP9);
                    break;
                case VK_DELETE:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_DEL : KB_NPDEL);
                    break;
                case VK_RETURN:
                    key = ((raw.data.keyboard.Flags & RI_KEY_E0) ? KB_NPENTER : KB_ENTER);
                    break;
                default:
                    if (raw.data.keyboard.VKey >= 255)
                        return 0;
                    key = (KB_KEY)raw.data.keyboard.VKey;
                    break;
                ...
                if (raw.data.keyboard.Flags & RI_KEY_BREAK)
                    ...
                    {
                        if (key >= 'A' && key <= 'Z')
                            Kb.queue(Char(key + (Kb.anyShift() ? 0 : 'a' - 'A')), raw.data.keyboard.MakeCode);
                        else if (key >= '0' && key <= '9')
                            Kb.queue(Char(key), raw.data.keyboard.MakeCode);
                    }
                }
            }
                return 0;
            ...
            case RIM_TYPEMOUSE: {
                ...
                if (raw.data.mouse.usButtonFlags & (RI_MOUSE_BUTTON_1_UP | RI_MOUSE_BUTTON_2_UP | RI_MOUSE_BUTTON_3_UP | RI_MOUSE_BUTTON_4_UP | RI_MOUSE_BUTTON_5_UP)) {
                    if (raw.data.mouse.usButtonFlags & RI_MOUSE_BUTTON_1_UP)
                        Ms.release(Ms._swapped); // 0
                    if (raw.data.mouse.usButtonFlags & RI_MOUSE_BUTTON_2_UP)
                        Ms.release(!Ms._swapped); // 1
    ...
    case WM_HOTKEY: {
        if ((lParam >> 16) == VK_SNAPSHOT)
            Kb.push(KB_PRINT, -1); // this is needed only for non-exclusive mode (both DirectInput and RawInput)
    } break;

    // IME
    case WM_INPUTLANGCHANGE:
        Kb.setLayout();
        break; // LANG_TYPE((lParam>>16)&0xFF)
    case WM_IME_CHAR:
    ...
    case WM_IME_SETCONTEXT:
        if (!D.exclusive())
            break;
        lParam = 0;
        break; // disables drawing of some windows, "lParam=0" disables drawing candidate list by OS, and enables accessing it manually using GetCandidateList
    case WM_IME_REQUEST:
        if (!D.exclusive())
            break;
        return 0; // don't process by OS
    ...
    case WM_IME_COMPOSITION: {
        ...
        if (chars_needed > 0) {
            Kb._imm_buffer.reserve(chars_needed);
            ImmGetCompositionString(Kb._imc, GCS_COMPSTR, Kb._imm_buffer._d.data(), chars_needed * SIZE(Char));
        ...
        if (clause_size > 0) {
            Memc<UInt> clause;
            clause.setNum(clause_size / SIZE(UInt));
            ImmGetCompositionStringA(Kb._imc, GCS_COMPCLAUSE, clause.data(), clause.elmsMem()); // use A because W crashes for GCS_COMPCLAUSE under WinXP
            FREPA(clause)
            if (clause[i] >= Kb.immCursor()) {
                Kb._imm_selection = clause[i];
                if (InRange(i + 1, clause))
                    Kb._imm_selection.y = clause[i + 1];
                break;
            }
        }
        if (lParam & GCS_RESULTSTR) {
            ...
            if (chars_needed > 0) {
                Memt<Char, 8 * 1024> imm;
                imm.setNum(chars_needed);
                ImmGetCompositionString(Kb._imc, GCS_RESULTSTR, imm.data(), chars_needed * SIZE(Char));
                FREP(chars_needed)
                Kb.queue(imm[i], -1);
            }
        }
    ...
    case WM_IME_NOTIFY: {
        ...
        case IMN_OPENCANDIDATE:
            if (Kb._imm_candidate_hidden) {
                Kb._imm_candidate_hidden = false;
                Swap(Kb._imm_candidate_temp, Kb._imm_candidate);
            }
            break; // open  must "show" candidates
        case IMN_CLOSECANDIDATE:
            if (!Kb._imm_candidate_hidden) {
                Kb._imm_candidate_hidden = true;
                Swap(Kb._imm_candidate_temp, Kb._imm_candidate);
            }
            break; // close must "hide" candidates
    ...
    case WM_DROPFILES:
        if (App.drop) {
            Byte pushed = 0;
            if (!App.active()) // check for key-states if app is inactive, in case the 'drop' callback wants to know about them
            {
                if (!Kb.b(KB_LCTRL) && GetKeyState(VK_LCONTROL) < 0 && GetKeyState(VK_RMENU) >= 0) {
                    Kb.push(KB_LCTRL, 29);
                    pushed |= 1;
                } // we can enable LCTRL only if we know the right alt isn't pressed, because AltGr (Polish, Norwegian, .. keyboards) generates a false LCTRL
                if (!Kb.b(KB_RCTRL) && GetKeyState(VK_RCONTROL) < 0) {
                    Kb.push(KB_RCTRL, 29);
                    pushed |= 2;
                }
                if (!Kb.b(KB_LSHIFT) && GetKeyState(VK_LSHIFT) < 0) {
                    Kb.push(KB_LSHIFT, 42);
                    pushed |= 4;
                }
                if (!Kb.b(KB_RSHIFT) && GetKeyState(VK_RSHIFT) < 0) {
                    Kb.push(KB_RSHIFT, 54);
                    pushed |= 8;
                }
                if (!Kb.b(KB_LALT) && GetKeyState(VK_LMENU) < 0) {
                    Kb.push(KB_LALT, 56);
                    pushed |= 16;
                }
                if (!Kb.b(KB_RALT) && GetKeyState(VK_RMENU) < 0) {
                    Kb.push(KB_RALT, 56);
                    pushed |= 32;
                }
                if (!Kb.b(KB_LWIN) && GetKeyState(VK_LWIN) < 0) {
                    Kb.push(KB_LWIN, 91);
                    pushed |= 64;
                }
                if (!Kb.b(KB_RWIN) && GetKeyState(VK_RWIN) < 0) {
                    Kb.push(KB_RWIN, 92);
                    pushed |= 128;
                }
                if (pushed)
            ...
            Memc<Str> names;
            for (Int i = 0; DragQueryFile(handle, i++, name, Elms(name)) > 0;)
                names.add(name);
            POINT point;
            DragQueryPoint(handle, &point);
            Vec2 pos = D.windowPixelToScreen(VecI2(point.x, point.y)); // get precise drop position
            DragFinish(handle);                                        // release before the callback
            App.drop(names, Gui.objAtPos(pos), pos);
            if (!D.full() && !(App.flag & APP_WORK_IN_BACKGROUND))
                DrawState();
            ...
            if (pushed) // release what was pushed
            {
                ...
                if (pushed & 2)
                    Kb.release(KB_RCTRL);
                if (pushed & 4)
                    Kb.release(KB_LSHIFT);
                if (pushed & 8)
                    Kb.release(KB_RSHIFT);
                if (pushed & 16)
                    Kb.release(KB_LALT);
                if (pushed & 32)
                    Kb.release(KB_RALT);
    ...
    case WM_COPYDATA:
        if (App.receive_data) {
            if (COPYDATASTRUCT *ds = (COPYDATASTRUCT *)lParam)
                App.receive_data(ds->lpData, ds->cbData, (HWND)wParam);
        }
        break;
            ...
            if (XWindow window = XCreateWindow(XDisplay, root_win, 0, 0, 256, 256, 0, vis_info->depth, InputOutput, vis_info->visual, CWBackPixmap | CWBorderPixel | CWColormap | CWEventMask, &win_attr)) {
                ...
                REP(1024)               // 1024 attempts
                {
                    ...
                    if (!XGetWindowProperty(XDisplay, window, _NET_FRAME_EXTENTS, 0, SIZE(unsigned long) * 4, false, XA_CARDINAL, &type, &format, &items, &bytes_after, &data))
                        if (type == XA_CARDINAL && format == 32 && items >= 4)
                            if (long *l = (long *)data) {
                                long left = l[0],
                                     right = l[1],
                                     top = l[2],
...
static void SetXInputValues(C Dbl *input, unsigned char *mask, Int mask_len, Dbl *output, Int output_elms) {
    ...
    for (Int i = 0, o = 0; i < mask_elms && o < output_elms; i++, o++)
...
void Application::windowCreate() {
    ...
    C wchar_t *icon = null;
    if (!_icon)
        EnumResourceNames(_hinstance, RT_GROUP_ICON, EnumResources, (LONG_PTR)&icon); // _hinstance must be used, only this supports loading icon from DLL (GetModuleHandle(null) and null didn't work)
...

    {
        RECT rect;
        Zero(rect);
        AdjustWindowRect(&rect, _style_window, false);
        _bound.set(rect.left, rect.top, rect.right, rect.bottom);
        Zero(rect);
        AdjustWindowRect(&rect, _style_window_maximized, false);
        _bound_maximized.set(rect.left, rect.top, rect.right, rect.bottom);
    }
#elif MAC
    ...
    if (!(flag & APP_NO_TITLE_BAR))
        _style_window |= NSWindowStyleMaskTitled;
    if (flag & APP_MINIMIZABLE)
        _style_window |= NSWindowStyleMaskMiniaturizable;
    if (flag & (APP_MAXIMIZABLE | APP_RESIZABLE))
        _style_window |= NSWindowStyleMaskResizable; // on Mac this needs to be enabled if we want to have any of APP_MAXIMIZABLE|APP_RESIZABLE, because without it, the maximize button will be hidden and window will not be resizable around the edges
    if (!(flag & APP_NO_CLOSE))
        _style_window |= NSWindowStyleMaskClosable;
    ...
    if (!GLConfig)
        Exit("Couldn't find valid GL Config");

    // window
    XVisualInfo *vis_info = glXGetVisualFromFBConfig(XDisplay, GLConfig);
    if (!vis_info)
        Exit("Couldn't get XVisualInfo");
    ...
    if (!(WindowColormap = win_attr.colormap))
        Exit("Can't create ColorMap");
    if (!(flag & APP_NO_TITLE_BAR)) {
    ...
    if (D.resW() >= full.w() && D.resH() >= full.h())
        D._full = true;
    if (D.full()) {
        if (!D.resW())
            D._res.x = full.w();
        ...
        if (!D.resW())
            D._res.x = _window_size.x;
        if (!D.resH())
            D._res.y = _window_size.y;
        if (D.resW() >= Min(maximized_win_client_size.x, max_normal_win_client_size.x + 1) && D.resH() >= Min(maximized_win_client_size.y, max_normal_win_client_size.y + 1)) // if exceeds the limits of a normal window
        {
            maximize = true;
...
#endif
        if (T.x <= -1)
            x = work.min.x + _bound.min.x + 1;
        else if (!T.x)
            x = work.centerXI() - W / 2;
        else
            x = work.max.x - W + _bound.max.x - 1;
        if (T.y >= 1)
            y = work.min.y;
        else if (!T.y)
            y = work.centerYI() - H / 2;
        else
            y = work.max.y - H + _bound.max.y - 1;
...
#define WS_EX_NOREDIRECTIONBITMAP 0x00200000L
...
#endif
    UInt style = (D.full() ? _style_full : maximize ? _style_window_maximized
                                                    : _style_window);
    if (hide)
        FlagDisable(style, WS_VISIBLE);

    _window = CreateWindowEx(ex_style, (LPCWSTR)WindowClass, name(), style, x, y, w, h, null, null, _hinstance, null);
    if (!_window)
        Exit(MLTC(u"Can't create window", PL, u"Nie można utworzyć okienka"));
    ...
    if (maximize) // when creating window as maximized, then set restore rectangle
    {
        ...
        if (T.x <= -1)
            ...
            x = work.w() - w + _bound.max.x - 1;
        if (T.y >= 1)
            y = 0;
        else if (!T.y)
            y = (work.h() - h) / 2;
        else
            y = work.h() - h + _bound.max.y - 1;
...
#define WM_COPYGLOBALDATA 0x0049
    ...
    if (!_window)
        Exit("Can't create window");
    name(name());
    XSetWindowBackground(XDisplay, window(), 0);

    if (FIND_ATOM(WM_PROTOCOLS))
        if (FIND_ATOM(WM_DELETE_WINDOW))
            XSetWMProtocols(XDisplay, window(), &WM_DELETE_WINDOW, true); // register custom "close window" callback

    setWindowFlags();

    if (drop)
        if (Atom FIND_ATOM(XdndAware)) {
            int xdnd_version = 5;
            XChangeProperty(XDisplay, window(), XdndAware, XA_ATOM, 32, PropModeReplace, (unsigned char *)&xdnd_version, 1);
        }
    FIND_ATOM(XdndDrop);
    ...
    if (!(flag & APP_HIDDEN))
        XMapWindow(XDisplay, window()); // display window after everything is ready
#elif WEB
    ...
    if (w <= 0 || h <= 0) // if dimensions haven't been specified
    {
        if (flag & APP_WEB_DISABLE_AUTO_RESIZE) {
            Dbl css_w, css_h;
            emscripten_get_element_css_size(null, &css_w, &css_h); // first try getting from CSS
            w = RoundPos(css_w * zoom);
            h = RoundPos(css_h * zoom);
            if (w <= 0 || h <= 0) // if still unavailable
            {
                ...
                if (w <= 0 || h <= 0) {
                    w = 800;
                    h = 600;
                } // if still unavailable then use some constant
            }
        } else {
         ...
         h=Max(1, RoundPos(TextFlt(JavaScriptRunS("window.innerHeight"))*zoom));
            ...
            Dbl css_w, css_h;
            emscripten_get_element_css_size(null, &css_w, &css_h); // get actual size
            w = Max(1, RoundPos(css_w * zoom));
            h = Max(1, RoundPos(css_h * zoom)); // calculate pixels
...
static Bool EventPost(Thread &thread) {
    ...
    if (!EventPostEvent[1].wait(EventPostWait)) // wait 'EventPostWait' time before posting event, post only on timeout
    {
        ...
        PostEvent();
#elif LINUX
        XSendEvent(XDisplay, App.window(), 0, 0, (XEvent *)&WakeUpEvent);
        XFlush(XDisplay);
#endif
    }
    return true;
...
void Application::windowDel() {
...
#if WINDOWS_OLD
    if (Kb._imc) {
        ImmReleaseContext(window(), Kb._imc);
        Kb._imc = null;
    }
    if (PowerNotify) {
        if (HMODULE user = GetModuleHandle(L"User32.dll"))
    ...
    if (_window) {
        XDestroyWindow(XDisplay, _window);
        _window = null;
    }
    if (WindowColormap) {
        XFreeColormap(XDisplay, WindowColormap);
        WindowColormap = NULL;
    }
#else
    _window = null;
...
#endif
        {
            ...
            [NSApp nextEventMatchingMask:NSEventMaskAny
                               untilDate:[NSDate distantFuture]
                                 ...
                                 dequeue:NO]; // 'distantFuture' will wait for an event
#elif LINUX
...
#elif MAC
            [NSApp nextEventMatchingMask:NSEventMaskAny
                               untilDate:[NSDate dateWithTimeIntervalSinceNow:(active_wait / 1000.0)]
                                  inMode:NSDefaultRunLoopMode
                                 dequeue:NO];
#elif LINUX
            WaitForEvent(active_wait);
...
again:
    ...
    process:
        ...
        if (window() == event.xany.window)
            ...
            case FocusOut:
                Deactivate = 2;
                break; // when pressing Alt Key, FocusOut/FocusIn may get called multiple times within 0-1 frames, to avoid this, we record the event and deactivate after a delay, during which the app may get 'FocusIn' which will cancel this
            case FocusIn:
                Deactivate = 0;
                setActive(true);
                XSetICFocus(IC);
                break; // disable 'Deactivate'
            ...
            case KeyPress: {
                ...
                if (InRange(code, ScanCodeToKey)) {
                    ...
                    {
                        if (key >= 'A' && key <= 'Z')
                            Kb.queue(Char(key + (Kb.anyShift() ? 0 : 'a' - 'A')), code);
                        else if (key >= '0' && key <= '9')
                            Kb.queue(Char(key), code);
                    }
                }
                // !! queue characters after push !!
                if (IC) {
            ...
            case KeyRelease: {
                ...
                if (InRange(code, ScanCodeToKey)) {
                    char keymap[32];
                    XQueryKeymap(XDisplay, keymap); // on Linux 'KeyRelease' may get called multiple times in case the key is still held on, so actual button state (keymap) must be checked
                    if (!(keymap[code >> 3] & (1 << (code & 7))))
                        Kb.release(ScanCodeToKey[code]); // if actually released
                }
            } break;
            ...
            case ClientMessage: {
                if (event.xclient.message_type == WM_PROTOCOLS) {
                    ...
                    if (use_list) {
                        XProp p;
                        ReadProperty(p, xdnd_source, XdndTypeList);
        ...
        case MappingNotify: {
            ...
            if (map.request == MappingKeyboard)
                Kb.setLayout();
        } break;

        case GenericEvent:
            if (event.xcookie.extension == XInput2Extension) {
                if (XGetEventData(XDisplay, &event.xcookie)) {
                    switch (event.xcookie.evtype) {
                    case XI_RawMotion: {
                        XIRawEvent &raw = *(XIRawEvent *)event.xcookie.data;
                        Dbl delta[2];
                        SetXInputValues(raw.raw_values, raw.valuators.mask, raw.valuators.mask_len, delta, Elms(delta));
                        Ms._delta_rel.x += delta[0];
                        Ms._delta_rel.y -= delta[1];
                    } break;

                    case XI_RawButtonPress:
                        if (active()) // this will get called even if app is not active
                        {
                            XIRawEvent &raw = *(XIRawEvent *)event.xcookie.data;
                            ...
                            {
                                if (!ButtonPressCount[b]++)
                                    Ms.push(b); // push only at first
                            }
                        }
                        break;
                    ...
                    case XI_RawButtonRelease: {
                        ...
                        if (b >= 3 && InRange(b, ButtonPressCount) && ButtonPressCount[b]) {
                            if (!--ButtonPressCount[b])
                                Ms.release(b); // release only at last
                        }
                    } break;
    ...
    if (!active() && !_close)
        if (Int wait = ((flag & APP_WORK_IN_BACKGROUND) ? background_wait : -1)) {
            if (wait < 0) // unlimited wait
            {
                ...
                goto again;
#elif MAC
                if ([NSApp nextEventMatchingMask:NSEventMaskAny untilDate:[NSDate distantFuture] inMode:NSDefaultRunLoopMode dequeue:NO])
                    goto start; // 'distantFuture' will wait for an event
...
#if MAC
                if (!untilDate)
                    untilDate = [NSDate dateWithTimeIntervalSinceNow:(wait / 1000.0)];
                if ([NSApp nextEventMatchingMask:NSEventMaskAny untilDate:untilDate inMode:NSDefaultRunLoopMode dequeue:NO])
                    goto start;
#else
...
#if WINDOWS_OLD
                if (MsgWaitForMultipleObjects(0, null, false, wait, QS_ALLINPUT) != WAIT_TIMEOUT)
                    goto start;
#elif WINDOWS_NEW
                if (WaitForEvent(wait))
                    goto again; // goto 'again' because WINDOWS_NEW 'WaitForEvent' already processes events
#elif LINUX
                if (WaitForEvent(wait))
                    goto start;
#else
...
stop:
    ...
    {
        Bool full = false;
        if (D.full() && _window_resized == D.res())
            if (auto monitor = D.getMonitor())
                if (monitor->mode() == D.res())
                    full = true; // can set fullscreen only if we requested it, we got the resolution that we wanted, and monitor actually set that resolution
