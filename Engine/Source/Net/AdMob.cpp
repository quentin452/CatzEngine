...
- (void)adViewDidReceiveAd:(GADBannerView *)bannerView {
    AdMob._banner_state = AdMobClass::DONE;
    AdMob._banner_size = Round(Vec2(bannerView.frame.size.width, bannerView.frame.size.height) * ScreenScale);
    if (BannerVisible)
        AdMob.bannerShow();
}
- (void)adView:(GADBannerView *)view didFailToReceiveAdWithError:(GADRequestError *)error {
    AdMob._banner_state = AdMobClass::ERROR;
}
@end
...
- (void)interstitialDidReceiveAd:(GADInterstitial *)interstitial {
    AdMob._fs_state = AdMobClass::DONE;
...
- (void)interstitial:(GADInterstitial *)interstitial didFailToReceiveAdWithError:(GADRequestError *)error {
    AdMob._fs_state = AdMobClass::ERROR;
}
- (void)interstitialDidDismissScreen:(GADInterstitial *)interstitial {
    AdMob.fsCreate(InterstitialUnitID); // when ad is closed, it cannot be displayed again, so reload another one
    if (ViewController)
        [ViewController.view layoutSubviews]; // if the device is rotated when view is displayed, then the render buffers don't get resized automatically, so do it manually
}
- (void)interstitialWillDismissScreen:(GADInterstitial *)interstitial {
}
@end
...
static void ResizeAd() {
    VecI2 old = AdMob.bannerPos();
    AdMob._banner_pos.x ^= 1; // change to force set
...
AdMobClass::~AdMobClass() {
    ...
    [InterstitialDelegate release];
    InterstitialDelegate = null; // release after interstitial because it's using it
#endif
}
AdMobClass::AdMobClass() {
    ...
    if (on)
        if (NSClassFromString(@"ASIdentifierManager"))
            if (NSString *device_id = [[[ASIdentifierManager sharedManager] advertisingIdentifier] UUIDString]) {
                ...
                if (NSStringAuto str = dev_id)
                    Request.testDevices = [NSArray arrayWithObjects:str, nil];
            }
#endif
    return T;
}
...
AdMobClass &AdMobClass::bannerCreate(C Str8 &unit_id, BANNER_TYPE type) {
    if (unit_id.is()) {
        ...
        case LEADERBOARD:
            ...
            break;
        case SMART_BANNER:
            size = &((DirToAxis(App.orientation()) == AXIS_X) ? kGADAdSizeSmartBannerLandscape : kGADAdSizeSmartBannerPortrait);
            break;
        }
        if (size) {
            bannerDel();
    ...
    if (_banner_pos.x != x || _banner_pos.y != y) {
        ...
        if (Banner) {
            ...
            Vec2 res = Vec2(D.resW(), D.resH()) / ScreenScale,
                 ...
                 pos((x < 0) ? 0 : (x > 0) ? res.x - size.x
                                           : (res.x - size.x) * 0.5f,
                     (y > 0) ? 0 : (y < 0) ? res.y - size.y
                                           : (res.y - size.y) * 0.5f);
