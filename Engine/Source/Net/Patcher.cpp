...
static void AddFiles(MemPtr<Patcher::LocalFile> &local_files, FileFind &ff, Str parent) {
    ...
    if (ff.type == FSTD_DIR) {
        parent += ff.name;
        parent += '\\';
        for (FileFind child(ff.pathName()); child();)
            AddFiles(local_files, child, parent);
    }
}
void Patcher::SetList(MemPtr<LocalFile> local_files, C Str &dir) {
    local_files.clear();
    for (FileFind ff(dir); ff();)
        AddFiles(local_files, ff, S);
}
...
void Patcher::Downloaded::create(C Pak &pak, Int index, Download &download, Cipher *cipher) {
    ...
    if (download.size() == pf.data_size_compressed) {
        File temp;
        temp.readMem(download.data(), download.size(), cipher);
...
Patcher &Patcher::del() {
    ...
    _pak_download.del();
    REPAO(_file_download).del();
    _inst_info_download.del();
    _inst_download.del(); // delete all downloaders
    _pak.del();
    _inst_info.zero();
    _inst.del();
    _to_download.del();
...
DWNL_STATE Patcher::installerInfoState() {
    if (_inst_info_available)
        return DWNL_DONE;
    DWNL_STATE state = _inst_info_download.state();
    if (state == DWNL_DONE) {
        FileText f;
        f.readMem(_inst_info_download.data(), _inst_info_download.size());
        TextData data;
        if (data.load(f)) {
            if (TextNode *p = data.findNode("Size"))
                _inst_info.size = p->asInt();
            // if(TextNode *p=data.findNode("xxHash32"     ))_inst_info.xxHash32   =TextUInt (S+"0x"+p->value);
            if (TextNode *p = data.findNode("xxHash64"))
                _inst_info.xxHash64_32 = TextULong(S + "0x" + p->value) & UINT_MAX; // use 'TextULong' because 'TextUInt' may fail on large values
                                                                                    // if(TextNode *p=data.findNode("xxHash64"     ))_inst_info.xxHash64   =TextULong(S+"0x"+p->value);
            if (TextNode *p = data.findNode("ModifyTimeUTC"))
                _inst_info.modify_time_utc.fromText(p->value);
            if (TextNode *p = data.findNode("Version"))
                _inst_info.version = TextVer(p->value);
            _inst_info_available = true;
            _inst_info_download.del();
            return DWNL_DONE;
        }
        _inst_info_download.del().error();
        state = DWNL_ERROR;
    }
    return state;
}
...
DWNL_STATE Patcher::installerState() {
    if (_inst_available)
        return DWNL_DONE;
    DWNL_STATE state = _inst_download.state();
    if (state == DWNL_DONE) {
        xxHash64Calc *hasher = null;
#if AUTO_INSTALLER_INFO
        DWNL_STATE inst_info_state = installerInfoState();
        if (inst_info_state != DWNL_DONE)
            return inst_info_state; // we need "Installer Info"
        xxHash64Calc _hasher;
        if (_inst_info.xxHash64_32)
            hasher = &_hasher; // if hash known, then test it
#endif
        ...
        if (Decompress(compressed, _inst, true, hasher)) {
            ...
            if (_inst.size() == _inst_info.size)
...
#endif
                {
                    ...
                    Long done = _inst_download.done(), size = _inst_download.size();
                    DateTime modify = _inst_download.modifyTimeUTC();
                    _inst_download.del();
                    _inst_download._done = done;
                    _inst_download._size = size;
                    _inst_download._modif_time = modify; // restore these in case we still want to access them
                    return DWNL_DONE;
                }
#if AUTO_INSTALLER_INFO
...
Patcher &Patcher::downloadIndex() {
    if (is()) {
        ...
        _pak_available = false;
        _pak.del();
        _pak_download.create(_http + CaseDown(_name) + ".index.pak");
        REPAO(_file_download).clearIndexDel();
        _to_download.del();
        _downloaded.del();
        _files_left = 0;
    }
...
static Str ServerPath(Str path, bool file = true) // !! THIS NEEDS TO BE IN SYNC WITH "Uploader" TOOL !!
{
    ...
    if (file) {
        Str ext = GetExt(path);
        if (ext == "pl" || ext == "php" || ext == "old" || ext == "dat" || ext == "dll" || ext == "bat" || ext == "cmd")
            path += '_';
    } // these file formats can't be downloaded normally, because they're treated as scripts or some can be blocked by servers
    for (;;) {
...
static Bool PatcherUpdate(Thread &thread) {
...
void Patcher::update() {
    ...
    REPA(_file_download) {
        ...
        case DWNL_DONE: // finished downloading
        {
            ...
            downloaded.create(_pak, download.index, download, _cipher);
            {
                locker.on();
                Swap(downloaded, T._downloaded.New());
                _bytes_downloaded += download.done();
                download.clearIndexDel();
                goto next;
            } // set '_bytes_downloaded' and 'clearIndexDel' under lock to have correct knowledge of progress, don't delete 'download' outside of the lock, because 'Patcher.progress' operates on both '_bytes_downloaded' and 'Download.done', goto next without disabling lock because we will need it anyway
        } break;
        ...
        error : // error encountered
        {
            ...
            downloaded.createFail(_pak, download.index); // create 'downloaded' as failed
            {
                ...
                download.clearIndexDel();
                goto next;
            } // 'clearIndexDel' under lock to have correct knowledge of progress (changing '_bytes_downloaded' is optional), goto next without disabling lock because we will need it anyway
        } break;

        case DWNL_NONE:
            if (_to_download.elms()) // not downloading anything
            {
                locker.on();
            next:
                if (!_to_download.elms())
                    locker.off();
                else // check for elements to download
                {
    ...
    if (C Pak *pak = index())
        if (InRange(i, pak->files())) {
            ...
            if (pf.data_size) // has data size -> needs to be downloaded
            {
                ...
                _event.on();
                if (!_thread.created())
                    _thread.create(PatcherUpdate, this);
            } else {
                SyncLocker locker(_lock);
                _downloaded.New().createEmpty(*pak, i);
                _files_left++;
            }
        }
    return T;
...
Bool Patcher::getNextDownload(Patcher::Downloaded &downloaded) {
    ...
    if (T._downloaded.elms()) {
        Swap(downloaded, T._downloaded.last());
        _downloaded.removeLast();
        _files_left--;
        return true;
    }
    return false;
...
Long Patcher::progress() C {
    SyncLocker locker(_lock);
    Long size = _bytes_downloaded;
    REPA(_file_download)
    size += _file_download[i].done();
    return size;
}
Long Patcher::filesSize() C {
    ...
    Long size = _bytes_downloaded;
    REPA(_to_download)
    size += _pak.file(_to_download[i]).data_size_compressed;
    REPA(_file_download) {
        Int file_index = _file_download[i].index;
        if (InRange(file_index, _pak))
            size += _pak.file(file_index).data_size_compressed;
    }
    return size;
}
...
struct LocalFilePtr {
    ...
    void set(C CMemPtr<Patcher::LocalFile> &local_files, Int index) {
        T.lf = &local_files[index];
        T.org_index = index;
    }
...
static Bool Equal(C PakFile *pf, C Patcher::LocalFile *lf) {
    ...
    if (!pf && !lf)
        return true; // both don't exist = they're the same
    if (pf && lf)    // both exist
    {
...
Bool Patcher::compare(C CMemPtr<LocalFile> &local_files, MemPtr<Int> local_remove, MemPtr<Int> server_download) {
    ...
    if (C Pak *pak = index()) {
        ...
        Memt<LocalFilePtr> lfp;
        FREPA(local_files)
        lfp.New().set(local_files, i);
        lfp.sort(LocalFilePtr::Compare);
        ...
        FREPA(*pak) {
            C PakFile &pf = pak->file(i);
            C LocalFile *lf = null;
            if (LocalFilePtr *lfp_elm = lfp.binaryFind(pak->fullName(pf), LocalFilePtr::Compare))
                lf = lfp_elm->lf;
            if (!Equal(&pf, lf))
                server_download.add(i); // add to list of files for download
        }
        return true;
    }
    return false;
...
void Patcher::Compare(C CMemPtr<LocalFile> &src_files, C CMemPtr<LocalFile> &dest_files, MemPtr<Int> dest_remove, MemPtr<Int> src_copy) {
    ...
    Memt<LocalFilePtr> dest_ptrs;
    FREPA(dest_files)
    dest_ptrs.New().set(dest_files, i);
    dest_ptrs.sort(LocalFilePtr::Compare);
    Memt<LocalFilePtr> src_ptrs;
    FREPA(src_files)
    src_ptrs.New().set(src_files, i);
    src_ptrs.sort(LocalFilePtr::Compare);
    ...
    FREPA(dest_ptrs) // 'dest_ptrs' is sorted by path, go from start to remove folders first
    {
        ...
        C LocalFile *src = null;
        if (LocalFilePtr *src_ptr = src_ptrs.binaryFind(dest.full_name, LocalFilePtr::Compare))
            src = src_ptr->lf;
    ...
    FREPA(src_ptrs) {
        ...
        C LocalFile *dest = null;
        if (LocalFilePtr *dest_ptr = dest_ptrs.binaryFind(src.full_name, LocalFilePtr::Compare))
            dest = dest_ptr->lf;
        if (!Equal(dest, &src))
            src_copy.add(src_ptr.org_index); // add to list of files for copy
    }
}
/******************************************************************************/
} // namespace EE
