...
static Int inet_ptonXP(Int addr_family, CChar8 *src, Ptr addr) {
    Char8 src_copy[INET6_ADDRSTRLEN];
    Set(src_copy, src); // need to make a copy because the 'WSAStringToAddressA' param is not const
    sockaddr_storage sock_addr;
    Zero(sock_addr);
    int size = SIZE(sock_addr);
    if (WSAStringToAddressA(src_copy, addr_family, null, (sockaddr *)&sock_addr, &size) == 0)
        switch (addr_family) {
        case AF_INET:
            *(in_addr *)addr = ((sockaddr_in &)sock_addr).sin_addr;
            return 1;
        case AF_INET6:
            *(in6_addr *)addr = ((sockaddr_in6 &)sock_addr).sin6_addr;
            return 1;
        }
    return 0;
}
static CChar8 *inet_ntopXP(Int addr_family, CPtr addr, Char8 *dest, DWORD dest_length) {
    ...
    case AF_INET:
        ((sockaddr_in &)sock_addr).sin_addr = *(in_addr *)addr;
        break;
    case AF_INET6:
        ((sockaddr_in6 &)sock_addr).sin6_addr = *(in6_addr *)addr;
        break;
    default:
        return null;
    }
...
static Bool TextToIP4(CChar8 *text, UInt &ip) {
    ...
    CalcValue c;
    FREPA(v.c) {
        text = TextValue(text, c, false);
        if (c.type != CVAL_INT || c.i < 0 || c.i > 255)
            goto error;
        if ((i < 3) ? !text || *text++ != '.' : Is(text))
            goto error;
        v.c[i] = c.i;
    }
    ip = v.u;
...
Int CompareIgnorePort(C SockAddr &a, C SockAddr &b) {
    ...
    case AF_INET6: // check if can convert to IP4
        if (IN6_IS_ADDR_V4MAPPED(&a.v6().sin6_addr)) {
            a_family = AF_INET;
            a_ip4 = a.v6Ip4();
        } else if (IN6_IS_ADDR_LOOPBACK(&a.v6().sin6_addr)) {
            a_family = AF_INET;
            a_ip4 = IP4_LOCAL_HOST;
        } else if (IN6_IS_ADDR_UNSPECIFIED(&a.v6().sin6_addr)) {
            a_family = AF_INET;
            a_ip4 = IP4_ANY;
        }
        break;
    ...
    case AF_INET:
        ...
        break;
    case AF_INET6: // check if can convert to IP4
        if (IN6_IS_ADDR_V4MAPPED(&b.v6().sin6_addr)) {
            b_family = AF_INET;
            b_ip4 = b.v6Ip4();
        } else if (IN6_IS_ADDR_LOOPBACK(&b.v6().sin6_addr)) {
            b_family = AF_INET;
            b_ip4 = IP4_LOCAL_HOST;
        } else if (IN6_IS_ADDR_UNSPECIFIED(&b.v6().sin6_addr)) {
            b_family = AF_INET;
            b_ip4 = IP4_ANY;
        }
        break;
    }
    if (Int c = Compare(a_family, b_family))
        return c;
    ...
    default:
        return 0; // the same
    case AF_INET:
        return Compare(a_ip4, b_ip4);
    case AF_INET6:
        return CompareV6(&a.v6().sin6_addr, &b.v6().sin6_addr);
    }
}
Int Compare(C SockAddr &a, C SockAddr &b) {
    if (Int c = CompareIgnorePort(a, b))
        return c;
...
struct GlobalIPChecker {
    ...
    void del() {
        if (busy) {
            ...
            if (busy) {
                REPAO(d).stop();
                App._callbacks.exclude(Update, T);
                REPAO(d).del();
                busy = false;
            }
        }
    ...
    void update(Bool callback) {
        ...
        case DWNL_DONE: {
            FileText f;
            f.readMem(d[i].data(), d[i].done());
            Str8 t;
            f.getLine(t);
            CChar8 *text = t;
            if (i == GlobalIPSiteSkipText) {
                if (text = TextPos(text, ':'))
                    text++;
                if (text && text[0] == ' ')
                    text++;
            }
...
Bool SockAddr::thisDevice() C {
    ...
    default:
        return false;
    case AF_INET:
        if (v4Ip4() == IP4_LOCAL_HOST)
            return true;
        break;
    case AF_INET6:
        if (v6Ip4() == IP4_LOCAL_HOST && IN6_IS_ADDR_V4MAPPED(&v6().sin6_addr) // check IP4 first because it's faster
...
Bool SockAddr::needsV6() C {
    return family() == AF_INET6 && !IN6_IS_ADDR_V4MAPPED(&v6().sin6_addr) && !IN6_IS_ADDR_LOOPBACK(&v6().sin6_addr) && !IN6_IS_ADDR_UNSPECIFIED(&v6().sin6_addr);
}
/******************************************************************************
...
Str8 SockAddr::ip4Text() C {
    ...
    case AF_INET6: {
        ...
        if (IN6_IS_ADDR_V4MAPPED(&v6().sin6_addr))
            v.u = v6Ip4();
        else if (IN6_IS_ADDR_LOOPBACK(&v6().sin6_addr))
            v.u = IP4_LOCAL_HOST;
        else if (IN6_IS_ADDR_UNSPECIFIED(&v6().sin6_addr))
            v.u = IP4_ANY;
        else
            break;
        return v.asTextDots();
...
Str8 SockAddr::ip6Text() C {
    ...
    case AF_INET6: {
        ...
        sockaddr_in6 temp;
        Zero(temp); // we need to set only 'family' and 'addr', because otherwise, 'scope' and 'port' can be included in the name
        temp.sin6_family = AF_INET6;
        temp.sin6_addr = v6().sin6_addr;
        DWORD length = Elms(text);
        return (WSAAddressToStringA((sockaddr *)&temp, SIZE(temp), null, text, &length) == 0) ? text : null;
#else
...
SockAddr &SockAddr::setLocal(Int port) {
    ...
    Memt<SockAddr> addrs;
    GetHostAddresses(addrs, ComputerName, port);
    FREPA(addrs)
    ...
    if (addrs.elms())
        ...
        clear(); // grab the first result as it is recommended to process results in order
#else
...
Bool SockAddr::setHost(C Str &host, Int port) {
    Memt<SockAddr> addrs;
    GetHostAddresses(addrs, host, port);
    if (addrs.elms()) {
...
Bool SockAddr::fromText(C Str8 &ip_port) {
    if (CChar8 *addr = SkipHttp(ip_port)) {
        if (*addr == '[') // try IPv6
        {
            ...
            if (pos >= 0 && addr[pos + 1] == ':') {
                Int port = TextInt(addr + pos + 2);
                if (InRange(port, 0x10000))
                    return setIp6Port(Str8(addr + 1).clip(pos - 1), port);
            }
        } else // try IPv4 + Name
        {
            Int pos = TextPosI(addr, ':');
            if (pos >= 0) {
                Int port = TextInt(addr + pos + 1);
                if (InRange(port, 0x10000)) {
                    Str8 a = addr;
                    a.clip(pos);
...
Bool SockAddr::convert(C SockAddr &src) {
    ...
    case AF_INET: {
        ...
        case IP4_ANY:
            v6setAfterClearAny();
            break; // handle special cases in case dual-stack isn't supported
        case IP4_LOCAL_HOST:
            v6setAfterClearLocalHost();
            break; // handle special cases in case dual-stack isn't supported
        default:
    ...
    case AF_INET6: {
        ...
        Int port_n = src.portN(); // we're doing a direct copy so 'ntohs' is not needed
        UInt ip4;
        if (IN6_IS_ADDR_V4MAPPED(&src.v6().sin6_addr))
            ip4 = src.v6Ip4();
        else if (IN6_IS_ADDR_LOOPBACK(&src.v6().sin6_addr))
            ip4 = IP4_LOCAL_HOST;
        else if (IN6_IS_ADDR_UNSPECIFIED(&src.v6().sin6_addr))
            ip4 = IP4_ANY;
        else
            break;
        clear();
...
Bool SockAddr::save(File &f) C {
    ...
    default:
        ...
        break;
    case AF_INET:
        f.putMulti(Byte(1), UInt(v4Ip4()), U16(port()));
        break;
    case AF_INET6:
        if (!needsV6())
            f.putMulti(Byte(1), UInt(ip4()), U16(port()));
        else
            f.putMulti(Byte(2), U16(port())) << v6().sin6_addr;
        break;
    }
    return f.ok();
}
...
#if APPLE || SWITCH
    int val = 1;
    setsockopt(_s, SOL_SOCKET, SO_NOSIGPIPE, &val, SIZE(val)); // disable SIGPIPE process signal on Unix machines (it is received when writing to closed socket, and it causes process termination)
#endif
    if (ipv6 && DualStackSocket) // don't bother if it's not supported
    {
        int val = 0;
        setsockopt(_s, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&val, SIZE(val)); // this allows binding to IPv4-mapped IPv6
    }
}
void Socket::del() {
    if (is()) {
...
Bool Socket::createTcp(Bool ipv6) {
    del();
    _s = socket(ipv6 ? AF_INET6 : AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (is()) {
        init(ipv6);
        return true;
    }
...
Bool Socket::createUdp(Bool ipv6) {
    del();
    _s = socket(ipv6 ? AF_INET6 : AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (is()) {
        init(ipv6);
        return true;
    }
...
/******************************************************************************/
Int Socket::portLocal() C {
    SockAddr addr;
    addr.setFromLocal(T);
    return addr.port();
}
Int Socket::portDest() C {
    SockAddr addr;
    addr.setFromDest(T);
    return addr.port();
}
SockAddr Socket::addrLocal() C {
    SockAddr addr;
    addr.setFromLocal(T);
    return addr;
}
SockAddr Socket::addrDest() C {
    SockAddr addr;
    addr.setFromDest(T);
    return addr;
}
...
#elif WINDOWS
        DWORD non_block = (on ? false : true);
        return ioctlsocket(_s, FIONBIO, &non_block) != SOCKET_ERROR;
#elif !SWITCH // Nintendo Switch doesn't support FIONBIO
        UInt non_block = (on ? false : true);
        return ioctl(_s, FIONBIO, &non_block) != SOCKET_ERROR;
#else
        auto flags = fcntl(_s, F_GETFL, 0);
        FlagSet(flags, O_NONBLOCK, !on);
        return fcntl(_s, F_SETFL, flags) != SOCKET_ERROR;
#endif
    }
    return false;
}
Bool Socket::tcpNoDelay(Bool on) {
    if (is()) {
        int val = on;
        return !setsockopt(_s, IPPROTO_TCP, TCP_NODELAY, (char *)&val, SIZE(val));
    }
    return false;
}
Bool Socket::broadcast(Bool on) {
    if (is()) {
        int val = on;
        return !setsockopt(_s, SOL_SOCKET, SO_BROADCAST, (char *)&val, SIZE(val)); // this allows broadcast mode, sending to 255.255.255.255 addr (all devices in local network)
    }
    return false;
}
/******************************************************************************/
...
again:
#endif
    if (::connect(_s, (C sockaddr *)&addr._data, addr.size()) != SOCKET_ERROR)
        return CONNECTED;
...
#if !WINDOWS
    case EINTR:
        goto again; // call was interrupted by a signal
#endif
    ...
    {
        SockAddr temp;
        if (temp.convert(addr)) {
#if !WINDOWS
        again_2:
#endif
            if (::connect(_s, (C sockaddr *)&temp._data, temp.size()) != SOCKET_ERROR)
                return CONNECTED;
            switch (PLATFORM(WSAGetLastError(), errno)) // don't use 'Socket::WouldBlock' here because it's not meant for 'connect'
            {
            case PLATFORM(WSAEWOULDBLOCK, EINPROGRESS):
                goto in_progress;
#if !WINDOWS
            case EINTR:
                goto again_2; // call was interrupted by a signal
#endif
            }
        }
    } break;
...
#if LOG_ERROR
    default:
        LogN(S + "Socket.connect failed:" + PLATFORM(WSAGetLastError(), errno));
        break;
#endif
    }
    return FAILED;
...
Bool Socket::connectFailed() {
    ...
    if (is()) {
        Int error = 0;
        socklen_t size = SIZE(error);
        if (getsockopt(_s, SOL_SOCKET, SO_ERROR, (char *)&error, &size) != SOCKET_ERROR)
            return error != 0;
    }
#endif
    return false;
...
Bool Socket::bind(C SockAddr &addr) {
    ...
    case PLATFORM(WSAEAFNOSUPPORT, EAFNOSUPPORT): {
        SockAddr temp;
        if (temp.convert(addr))
            return ::bind(_s, (C sockaddr *)&temp._data, temp.size()) != SOCKET_ERROR;
    } break;

#if LOG_ERROR
    default:
        LogN(S + "Socket.bind:" + PLATFORM(WSAGetLastError(), errno));
        break;
#endif
    }
    return false;
...
Bool Socket::accept(Socket &connection, SockAddr &addr) {
    ...
    addr.clear();
    if (is()) {
        socklen_t size = SIZE(addr._data);
        connection._s = ::accept(_s, (sockaddr *)&addr._data, &size);
        if (connection.is()) {
...
Int Socket::receive(SockAddr &addr, Ptr data, Int size) {
    socklen_t addr_size = SIZE(addr._data);
    return ::recvfrom(_s, (Char8 *)data, size, 0, (sockaddr *)&addr._data, &addr_size);
}
Int Socket::send(C SockAddr &addr, CPtr data, Int size) {
    ...
    if (sent < 0)
        ...
        case PLATFORM(WSAEAFNOSUPPORT, EAFNOSUPPORT): {
            SockAddr temp;
            if (temp.convert(addr))
                sent = ::sendto(_s, (Char8 *)data, size, SEND_FLAGS, (C sockaddr *)&temp._data, temp.size());
        } break;

#if LOG_ERROR
        default:
            LogN(S + "Socket.send failed:" + PLATFORM(WSAGetLastError(), errno));
            break;
#endif
        }
    return sent;
...
#if LOG_ERROR
    default:
        LogN(S + "Socket failed:" + PLATFORM(WSAGetLastError(), errno));
        break;
#endif
    }
    return false;
...
#if WINDOWS
        DWORD size;
        if (ioctlsocket(_s, FIONREAD, &size) != SOCKET_ERROR)
            return size;
#else
        UInt size;
        if (ioctl(_s, FIONREAD, &size) != SOCKET_ERROR)
            return size;
#endif
    }
    return -1;
...
static Bool ConnectSMTP(Socket &socket) {
    SockAddr addr;
    addr.setLocalFast(25);                         // 25 is the default port for SMTP servers
    ...
    {
        if (socket.createTcp(true))
            switch (socket.connect(addr)) {
            case Socket::CONNECTED:
                return true;
            default:
                ConnectSMTPIPv6Failed = true;
                break;
            }
    }
...
Bool SendMail(C Str &from_name, C Str &from_email, C Str &to_name, C Str &to_email, C Str &subject, C Str &body) {
    ...
    if (ConnectSMTP(sock)) {
        ...
        if (log)
            Zero(buf);
        sock.receive(buf, SIZE(buf));
        if (log)
            LogN(S + (char *)buf);
        m = "HELO SendMail\r\n";
        sock.send(m(), m.length());
        if (log)
            Zero(buf);
        sock.receive(buf, SIZE(buf));
        if (log)
...
void GetHostAddresses(MemPtr<SockAddr> addresses, C Str &host, Int port) {
    ...
    if (!GetAddrInfoW(host, null, &hints, &result)) {
        for (ADDRINFOW *r = result; r; r = r->ai_next)
            switch (r->ai_family) {
            case AF_INET:
                if (r->ai_addr && r->ai_addrlen <= MEMBER_SIZE(SockAddr, _data)) {
                    C sockaddr_in &addr = *(sockaddr_in *)r->ai_addr;
                    if (addr.sin_addr.s_addr != IP4_LOCAL_HOST) {
                        SockAddr &sa = addresses.New();
                        sa.v4() = addr;
                        sa.port(port);
                    } else
                        local_host = true;
    ...
    if (!addresses.elms()) {
        ...
        if (!getaddrinfo(Str8(host), null, &hints, &result)) {
...
#endif
    if (!addresses.elms() && local_host)
        addresses.New().setLocalFast(port); // if no other addresses were found, but only local host, then return it, this is needed if for example we want to use 'Download' (which uses this function) to connect to "localhost"
}
/******************************************************************************/
void GetGlobalIP(Bool refresh) {
    if (!GIPC.valid() || refresh)
        GIPC.start();
}
...
static Bool InitSocketEx() {
    ...
    if (WSAStartup(MAKEWORD(2, 2), &WsaData) == NO_ERROR) // init WinSock
    {
        ...
        // get host name
        {
            ...
            DWORD size = Elms(host);
            if (GetComputerNameW(host, &size))
                ComputerName = host;
#else // doesn't work if "hosts" file is modified
            sockaddr_in addr;
            Zero(addr); // this also clears 'port'
            ...
            if (!GetNameInfoW((sockaddr *)&addr, SIZE(addr), host, Elms(host), service, Elms(service), NI_NUMERICSERV)) { // ok
                ComputerName = host;
            }
#endif
        }
...
#if LINUX
    Socket sock;
    if (sock.createUdp(true)) // we need this only for Mac Address, prefer IPv6 because it's better
    {
        ifreq ifr;
        // try ethernet first
        Set(ifr.ifr_name, "eth0");
...
{
    *olen = 0;
    Int r = MBEDTLS_ERR_ENTROPY_SOURCE_FAILED;
#if WINDOWS_OLD
    HCRYPTPROV provider;
...
#else
    if (OK(BCryptGenRandom(null, output, (ULONG)len, BCRYPT_USE_SYSTEM_PREFERRED_RNG))) {
        *olen = len;
        r = 0;
    }
#endif
...
static int EntropyFunc(Ptr ctx, Byte *output, size_t len) {
    SyncLocker sl(SecureSyncLock);
    return mbedtls_entropy_func(ctx, output, len);
}
static int RandomFunc(Ptr ctx, Byte *output, size_t len) {
    SyncLocker sl(SecureSyncLock);
...
#define TEST_LOAD_SPEED (DEBUG && 0)
    ...
    if (!SecureConfig.f_dbg) {
        ...
        if (!SecureConfig.f_dbg) {
            {
                File cmpr, src;
                cmpr.readMem(CertificatesData, SIZE(CertificatesData));
                ...
                for (src.pos(0); !src.end();) {
                    Int size = src.getUShort();
                    DYNAMIC_ASSERT(size > 0 && size <= src.left(), "Invalid Certificate size");
                    DYNAMIC_ASSERT(mbedtls_x509_crt_parse_der(cert, (Byte *)src.memFast(), size) == 0, "Can't load Certificate");
                    if (cert->next)
                        cert = cert->next; // advance to the last available to avoid iteration inside 'mbedtls_x509_crt_parse_der'
                    src.skip(size);
                }
            ...
            UID custom;
            custom.randomize(); // using random values improves entropy
            ...
            mbedtls_ssl_conf_read_timeout(&SecureConfig, DOWNLOAD_WAIT_TIME);
            ASSERT(DOWNLOAD_WAIT_TIME != 0);                                       // for 'mbedtls_ssl_conf_read_timeout' 0 is actually unlimited and not instant
...
static void UpdateCertificates() {
...
#else // from internet
        Download down;
        down.create("https://curl.haxx.se/ca/cacert.pem");
        down.wait(); // this is from CURL based on Mozilla, some alternative resources:
    ...
    for (mbedtls_x509_crt *cert = &certs; cert; cert = cert->next) {
        Int size = Int(cert->raw.len);
        DYNAMIC_ASSERT(size > 0 && size <= USHORT_MAX, S + "Invalid CERT size:" + size);
    ...
    f.pos(0);
    File cmpr;
    DYNAMIC_ASSERT(CompressRaw(f, cmpr.writeMem(), COMPRESS_LZ4, 9), "failed to compress");
    {
        ...
        ft.putLine("{");
        for (cmpr.pos(0); !cmpr.end();) {
            if (cmpr.pos() != 0) {
                ft.putText(",");
                if (!(cmpr.pos() % 64))
                    ft.endLine();
            }
            ft.putText(cmpr.getByte());
        }
        ft.endLine();
        ft.putLine("};");
...
Bool SecureSocket::secure(CChar8 *host) {
    ...
    return !mbedtls_ssl_setup(_secure, &SecureConfig) && !mbedtls_ssl_set_hostname(_secure, _GetStart(host));
#endif
    return false;
}
#if SUPPORT_MBED_TLS
static int SendFunc(Ptr ctx, C Byte *buf, size_t len) {
    SecureSocket &s = *(SecureSocket *)ctx;
    Int r = s.Socket::send(buf, (Int)len);
    if (r < 0)
...
static int ReceiveFunc(Ptr ctx, Byte *buf, size_t len) {
    SecureSocket &s = *(SecureSocket *)ctx;
    Int r = s.Socket::receive(buf, (Int)len);
    if (r < 0)
...
static int ReceiveFuncTimeout(Ptr ctx, Byte *buf, size_t len, uint32_t timeout) {
    SecureSocket &s = *(SecureSocket *)ctx;
    if (!s.wait(timeout ? Min(timeout, INT_MAX) : INT_MAX))
...
static inline Int MbedTlsToResult(Int r) {
    ...
    case MBEDTLS_ERR_SSL_TIMEOUT:
        return SecureSocket::NEED_WAIT;

    case MBEDTLS_ERR_SSL_WANT_WRITE:
        return SecureSocket::NEED_FLUSH;

    default:
        return (r >= -1) ? r : SecureSocket::ERROR;
    }
}
#endif
...
#if SUPPORT_MBED_TLS
    if (_secure)
        return MbedTlsToResult(mbedtls_ssl_write(_secure, (Byte *)data, size));
#endif
...
#if SUPPORT_MBED_TLS
    if (_secure)
        return MbedTlsToResult(mbedtls_ssl_read(_secure, (Byte *)data, size));
#endif
...
SecureSocket::RESULT SecureSocket::handshake() {
    ...
    case 0:
        return mbedtls_ssl_get_verify_result(_secure) ? BAD_CERT : OK;
...
Bool NetworkServiceAccountIDToken(CPtr &data, Int &size) {
    data = null;
    size = 0;
    return true;
}
