...
void Controller::capsuleHeight(Flt height) {
    PxShape *shape;
    if (actor._actor)
        if (actor._actor->getShapes(&shape, 1)) {
            ...
            if (shape->getCapsuleGeometry(capsule)) {
                ...
                pose.p = Physx.vec(_shape_offset + Vec(0, (T._height_cur - T._height) * 0.5f, 0));
...
void Controller::capsuleHeight(Flt height) {
    if (actor._actor)
        if (btCapsuleShape *shape = CAST(btCapsuleShape, actor._actor->getCollisionShape())) {
            T._height_cur = Max(2 * T._radius + Physics.minCapsuleEdge(), height);
...
struct ControllerGroundCallback : PhysHitCallback {
    ...
    virtual Bool hit(PhysHit &phys_hit) {
        if (phys_hit.collision) {
            ...
            // detect ground plane
            if (phys_hit.plane.pos.y >= min_y && phys_hit.plane.pos.y <= max_y && walkable(phys_hit, group_mask)) {
                ...
                if (test_slide && (!phys_hit.dynamic || (group_mask & Physics.ctrl_slide_group_horizontal)) // don't slide against dynamic objects (or slide if it's specified for horizontal)
...
struct ControllerCeilCallback : PhysHitCallback {
    ...
    virtual Bool hit(PhysHit &phys_hit) {
        if (phys_hit.collision && !ctrl.actor.ignored(phys_hit)) {
            cuts = true;
            return false;
        }
        return true;
...
void Controller::update(C Vec &velocity, Bool crouch, Flt jump) {
    PROFILE_START("CatzEngine::Controller::update(C Vec &velocity, Bool crouch, Flt jump)")
    if (actor.is()) {
        Vec vel = velocity;
        if (!actor.sleep()) {
            if (_jumping) // update jumping
            {
                if ((_time_jump += Time.d()) > 0.2f) {
                    _jumping = false;
                    _time_jump = 0;
                }
            } else if (Physics.updated()) // enter only when physics was updated
            {
                // detect if on ground
                ...
                v.normalize();
                v *= width * 16;
                v.y = width * -2 - 0.1f; // horizontal*16 because when moving almost perpendicular to wall (||) because of smaller radius collision with wall would not be detected, vertical*2 because we need to travel both the radius that we've decreased, and secondary radius for testing (also subtract extra in case the ground is going down, but we're moving forward, so we still can detect the decreasing ground)
                ...
                if (cgc.do_slide) {
                    Flt d = Dot(vel, cgc.slide_normal);
                    if (d < 0)
                        vel -= d * cgc.slide_normal;
                }
        ...
        if (flying()) {
            ...
            if (onGround() || fall_control) {
                if (Physics.updated()) // process only when physics were updated, important because we may do 'Actor.addVel' instead of 'Actor.vel' to set new velocity, which in case physics didn't get simulated it would add delta multiple times (example: actor_vel(0, 0, 0), target_vel(1, 0, 0), addVel(target_vel-actor_vel) addVel(target_vel-actor_vel) addVel(target_vel-actor_vel) would add delta 3x times in case physics was not simulated between the calls)
                {
                    ...
                    Int s = Sign(v.y);
                    if (s == Sign(_vel_prev.y))
                        if (Sign(v.y -= _vel_prev.y) != s)
                            v.y = 0;

                    {
                        Vec2 vc_xz = v.xz();
                        if (Flt vc_xz_len = vc_xz.normalize()) { // current velocity of the actor
                            Vec2 vp_xz = _vel_prev.xz();
                            Flt vp_xz_len = vp_xz.normalize(); // previously set velocity
        ...
        if (crouch != T.crouched()) {
            ...
            } else // if want to stand up
            {
                ...
                ball.pos.y += height() * 0.5f - radius() - h_diff;                                   // set ball as crouched controller upper ball
        ...
        if (Physics._css && Physics.updated()) {
            ...
            switch (Physics._css) {
            case CSS_MATERIALS:
                actor.material(freeze ? &Physics.mtrl_ctrl_stop : &Physics.mtrl_ctrl);
                break;
            case CSS_GRAVITY:
                actor.gravity((freeze || flying()) ? false : true);
                break;
            case CSS_FREEZE_XZ:
                actor.freezePosX(freeze).freezePosZ(freeze);
                break;
            }
        }
    }
    PROFILE_STOP("CatzEngine::Controller::update(C Vec &velocity, Bool crouch, Flt jump)")
}
/******************************************************************************/
#pragma pack(push, 1)
...
Bool Controller::save(File &f) C {
    ...
    _Unaligned(desc.flags, (_crouched ? ControllerDesc::CROUCHED : 0) | (_on_ground ? ControllerDesc::ON_GROUND : 0) | (_jumping ? ControllerDesc::JUMPING : 0) | (_flying ? ControllerDesc::FLYING : 0) | (fall_control ? ControllerDesc::FALL_CONTROL : 0));
...
Bool DefaultController(Capsule &capsule, C Skeleton &skel) {
    ...
    REPA(skel.bones) {
        ...
        case BONE_UPPER_ARM:
            if (!bone.type_sub)
                cc.process(bone.shape.ballD());
            break;
        }
    }
    const Flt min_size = 0.01f;
    if (cc.r <= min_size / 2 || cc.h <= min_size)
        return false;
