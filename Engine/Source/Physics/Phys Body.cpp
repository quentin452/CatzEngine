...
/******************************************************************************/
void IncRef(PhysMesh *&pm) {
    if (pm)
        AtomicInc(pm->_used_by);
}
void DecRef(PhysMesh *&pm) {
    if (pm) {
        if (PhysMeshes.elms()) // if 'PhysMeshes' destructor was not yet called, and there are elements ('pm' is not pointing to already released memory)
        {
            ...
            if (pm->_used_by < 0)
                Exit("PhysMesh has <0 references");
...
void DecRef(btCollisionShape *shape) {
    if (shape) {
        ...
        IntPtr refs = (IntPtr)shape->getUserPointer();
        if (refs < 0)
            Exit("btCollisionShape has <0 references");
        if (refs == 0)
            Delete(shape);
        else
            shape->setUserPointer(Ptr(refs - 1));
...
static void Set(PhysMesh &pm, PhysMesh *&pm_ptr) {
    pm._used_by = 1;
    PhysMeshesLock.on();
    PhysMesh &stored_pm = PhysMeshes.New();
    PhysMeshesLock.off();
    Swap(stored_pm, pm);
    pm_ptr = &stored_pm;
}
...
void PhysMesh::del() {
    if (_convex || _mesh || _tvia) // delete API specific data first (because they may be dependent on data listed below)
    {
        SafeWriteLock lock(Physics._rws);
#if PHYSX
        if (_convex && Physx.physics)
            _convex->release();
        if (_mesh && Physx.physics)
            _mesh->release();
#else
        DecRef(_convex);
        _convex = null;
...
Bool PhysMesh::adjustStorage(Bool universal, Bool physx, Bool bullet, Bool *changed) {
    ...
    if (_type == PHYS_CONVEX || _type == PHYS_MESH) {
        if (bullet)
            universal = true; // 'universal' is always needed when using 'bullet'
        if (!universal && !physx && !bullet)
            universal = true; // if no storage is specified then keep 'universal' only
        ...
        // create physx
        if (physx && !_physx_cooked_data.elms() && _base)
            switch (_type) {
            case PHYS_CONVEX:
                if (cookConvex(_base, true))
                    if (changed)
                        *changed = true;
                break;
            case PHYS_MESH:
...
#if !PHYSX
                if (_mesh)
                    temp_bvh = _mesh->getOptimizedBvh(); // check if bullet mesh has already pre-computed data
#endif
                if (!temp_bvh && _base) {
        ...
        if (!universal && _physx_cooked_data.elms() && _base) {
            Delete(_base);
            if (changed)
                *changed = true;
        } // delete only if there's physx     left (don't delete if there's no physx    )
        if (!physx && _base && _physx_cooked_data.elms()) {
            _physx_cooked_data.del();
            if (changed)
                *changed = true;
        } // delete only if there's universal left (don't delete if there's no universal)
        if (!bullet && _bullet_cooked_data) {
            FreeAlign16(_bullet_cooked_data);
            _bullet_cooked_data_size = 0;
            if (changed)
                *changed = true;
        ...
        if ((!universal) != (!_base))
            return false;
        if ((!physx) != (!_physx_cooked_data.elms()))
            return false;
        if (_type == PHYS_MESH)
            if ((!bullet) != (!_bullet_cooked_data))
                return false; // only PHYS_MESH uses 'bullet' data
    }
    return true;
}
Bool PhysPart::adjustStorage(Bool universal, Bool physx, Bool bullet, Bool *changed) {
    if (_pm) {
        PhysPart temp;
        temp = T;
        Bool ok = temp._pm->adjustStorage(universal, physx, bullet, changed);
        Swap(temp, T);
        return ok; // adjust storage on separate 'pm' so any current actors using the mesh, will not get invalid
    }
    if (changed)
        *changed = false;
    return true;
...
Bool PhysBody::adjustStorage(Bool universal, Bool physx, Bool bullet, Bool *changed) {
    ...
    Bool ok = true, c;
    REPA(T) {
        ok &= parts[i].adjustStorage(universal, physx, bullet, &c);
        if (changed)
            *changed |= c;
    }
    return ok;
    ...
    if (!_physx_cooked_data.elms() && src && src->vtxs()) {
        ...
        if (mesh_is_already_convex && src->faces()) {
            ...
            FREPA(polys) {
                ...
                if (bit16)
                    p = *(VecUS *)((U16 *)desc.indices.data + offset);
                else
                    p = *(VecI *)((U32 *)desc.indices.data + offset);
...
Bool PhysMesh::cookMesh(MeshBase *src, Bool keep_face_indexes) // cook data only if the body doesn't have yet any cooked data
{
    ...
    if (!_physx_cooked_data.elms() && src && src->vtxs() && src->faces()) {
        ...
        OutputStream buf;
        if (!Physx.cook[keep_face_indexes])
            Exit("PhysPart.createMesh\nPhysics hasn't been created");
...
void PhysMesh::setPhysMesh() {
    ...
    case PHYS_CONVEX:
        if (!_convex) {
            ...
            if (!_convex && Physx.physics && cookConvex(_base, true)) {
                InputStream is(_physx_cooked_data.data(), _physx_cooked_data.elms());
                _convex = Physx.physics->createConvexMesh(is);
            }
        }
        break;
    case PHYS_MESH:
        if (!_mesh) {
            WriteLock lock(Physics._rws);
            if (!_mesh && Physx.physics && cookMesh(_base)) {
                InputStream is(_physx_cooked_data.data(), _physx_cooked_data.elms());
                _mesh = Physx.physics->createTriangleMesh(is);
            }
        }
        break;
    }
    ...
    if (_base)
        ...
        case PHYS_CONVEX:
            if (!_convex) {
                WriteLock lock(Physics._rws);
                if (!_convex)
                    if (_convex = new btConvexHullShape((Flt *)_base->vtx.pos(), _base->vtxs(), SIZE(Vec)))
                        _convex->setMargin(Physics.skin());
            }
            break;
        case PHYS_MESH:
            if (!_mesh) {
                ...
                if (!_mesh) {
                    ...
                    if (_bullet_cooked_data)
                        _mesh->setOptimizedBvh(btOptimizedBvh::deSerializeInPlace(_bullet_cooked_data, _bullet_cooked_data_size, false));
                    if (!_mesh->getOptimizedBvh())
                        _mesh->buildOptimizedBvh();
...
#endif
    if (App.flag & APP_AUTO_FREE_PHYS_BODY_HELPER_DATA)
        freeHelperData(); // after creating phys mesh we can try releasing helper data
}
/******************************************************************************/
Bool PhysMesh::createConvexTry(MeshBase &mshb, Bool mesh_is_already_convex) {
    ...
    if (mshb.vtxs()) {
#if PHYSX
        if (!cookConvex(&mshb, mesh_is_already_convex))
            return false;
#else
        New(_base);
        if (mesh_is_already_convex)
            _base->create(mshb, MESH_NONE).quadToTri();
        else
            _base->createConvex(mshb.vtx.pos(), mshb.vtxs()).quadToTri();
#endif
...
#if PHYSX
        if (!cookMesh(&mshb, keep_face_indexes))
            return false;
#else
...
Flt PhysMesh::volume() C {
    if (_base)
        return _base->convexVolume();
    if (_convex) // approximate with a Ball
    {
        btVector3 center;
        btScalar radius;
        _convex->getBoundingSphere(center, radius);
...
Bool PhysMesh::setBox() {
    ...
    case PHYS_CONVEX: {
        if (_base)
            return _base->getBox(_box);
#if PHYSX
        if (_convex) {
            _box = Physx.box(_convex->getLocalBounds());
            return true;
        }
#else
        if (_convex) {
            btVector3 min, max;
            _convex->getAabb(btTransform::getIdentity(), min, max);
            _box.set(Bullet.vec(min), Bullet.vec(max));
            return true;
        }
#endif
    } break;
...
#if PHYSX
        if (_mesh) {
            _box = Physx.box(_mesh->getLocalBounds());
            return true;
        }
#else
        if (_mesh) {
            btVector3 min, max;
            _mesh->getAabb(btTransform::getIdentity(), min, max);
            _box.set(Bullet.vec(min), Bullet.vec(max));
            return true;
        }
#endif
...
void PhysMesh::draw(C Color &color) C {
    ...
    case PHYS_CONVEX:
        if (_base)
            DrawPhys(*_base, color);
        else if (_convex)
            DrawConvex(*_convex, color);
        break;
    case PHYS_MESH:
        if (_base)
            DrawPhys(*_base, color);
        else if (_mesh)
            DrawMesh(*_mesh, color);
        break;
    }
}
PhysMesh &PhysMesh::operator=(C PhysMesh &src) {
    if (this != &src) {
        ...
        {
            if (src._convex) {
                New(_base)->create(*src._convex);
                setPhysMesh();
                Delete(_base);
                if (_convex)
                    _physx_cooked_data.del();
            } // try creating from phys mesh
            if (src._mesh) {
                New(_base)->create(*src._mesh);
                setPhysMesh();
                Delete(_base);
                if (_mesh)
                    _physx_cooked_data.del();
...
PhysPart::PhysPart(C Box &box, Flt density) {
    zero();
    create(box, density);
}
PhysPart::PhysPart(C OBox &obox, Flt density) {
    zero();
    create(obox, density);
}
PhysPart::PhysPart(C Extent &ext, Flt density) {
    zero();
    create(ext, density);
}
PhysPart::PhysPart(C Ball &ball, Flt density) {
    zero();
    create(ball, density);
}
PhysPart::PhysPart(C Capsule &capsule, Flt density) {
    zero();
    create(capsule, density);
}
PhysPart::PhysPart(C Tube &tube, Flt density) {
    zero();
    create(tube, density);
}
PhysPart::PhysPart(C Shape &shape, Flt density) {
    zero();
    create(shape, density);
}
...
Bool PhysPart::createConvexTry(MeshBase &mshb, Flt density, Bool mesh_is_already_convex) {
    ...
    if (mshb.vtxs()) {
        PhysMesh pm;
        if (!pm.createConvexTry(mshb, mesh_is_already_convex))
            return false;
        Set(pm, T._pm);
        T._type = PHYS_CONVEX;
        T.density = density;
...
Bool PhysPart::createMeshTry(MeshBase &mshb, Bool keep_face_indexes) {
    ...
    if (mshb.vtxs() && mshb.faces()) {
        PhysMesh pm;
        if (!pm.createMeshTry(mshb, keep_face_indexes))
            return false;
        Set(pm, T._pm);
        T._type = PHYS_MESH;
        T.density = 1;
...
Bool PhysPart::createConvexTry(MeshLod &mshl, Flt density, Bool mesh_is_already_convex, Bool skip_hidden_parts) {
    MeshBase mshb;
    mshb.createPhys(mshl, VTX_POS | FACE_IND, false, skip_hidden_parts);
...
/******************************************************************************/
PhysPart &PhysPart::createConvex(MeshBase &mshb, Flt density, Bool mesh_is_already_convex) {
    if (!createConvexTry(mshb, density, mesh_is_already_convex))
        Exit("PhysPart::createConvex");
    return T;
}
PhysPart &PhysPart::createConvex(MeshLod &mshl, Flt density, Bool mesh_is_already_convex, Bool skip_hidden_parts) {
    if (!createConvexTry(mshl, density, mesh_is_already_convex, skip_hidden_parts))
        Exit("PhysPart::createConvex");
    return T;
}

PhysPart &PhysPart::createMesh(MeshBase &mshb, Bool keep_face_indexes) {
    if (!createMeshTry(mshb, keep_face_indexes))
        Exit("PhysPart.createMesh");
    return T;
}
PhysPart &PhysPart::createMesh(MeshLod &mshl, Bool keep_face_indexes, Bool skip_hidden_parts) {
    if (!createMeshTry(mshl, keep_face_indexes, skip_hidden_parts))
        Exit("PhysPart.createMesh");
    return T;
}
/******************************************************************************/
...
PhysPart &PhysPart::transform(C Matrix3 &matrix) {
    ...
    case PHYS_CONVEX: {
        ...
        createConvex(temp, density, true);
    } break;
    case PHYS_MESH: {
        MeshBase temp;
        temp.create(T);
...
PhysPart &PhysPart::mirrorX() {
    ...
    case PHYS_CONVEX: {
        ...
        temp.create(T);
        temp.mirrorX();
        createConvexTry(temp, density, true);
    } break;
    case PHYS_MESH: {
        MeshBase temp;
        temp.create(T);
...
/******************************************************************************/
void PhysPart::setPhysMesh() {
    if (_pm)
        _pm->setPhysMesh();
}
/******************************************************************************/
PhysPart &PhysPart::freeHelperData() {
    if (_pm)
        _pm->freeHelperData();
    return T;
}
...
{
    if (f.getUInt() == CC4('N', 'X', 'S', 1))
        switch (f.getUInt()) // type
        {
...
#if PHYSX
        File f;
        f.readMem(pm._physx_cooked_data.data(), pm._physx_cooked_data.elms());
        if (!VerifyPhysX(f)) // if cooked version doesn't match with current PhysX SDK version
        {
            if (pm._base)
                pm._physx_cooked_data.del();
            else // if we have universal then just delete the incompatible physx cooked, and universal will be used
            {
                MeshBase temp;
                f.pos(0);
                if (ImportPhysX(temp, f)) // if import success
                {
                    Swap(*New(pm._base), temp);  // swap universal with temp
                    pm._physx_cooked_data.del(); // delete cooked so universal will be used
                    pm.setPhysMesh();
                    if (pm._mesh || pm._convex)
                        Delete(pm._base); // if cooked and created PhysMesh successfully, then delete universal because we didn't want it originally
                }
            }
        }
#else // bullet
...
Bool PhysPart::saveData(File &f) C {
    ...
    case PHYS_MESH: {
        ...
        if (!_pm->_base)
            ...
            f.putBool(true);
            if (!_pm->_base->saveData(f))
                return false;
        }
        if (_pm->_physx_cooked_data._saveRaw(f)) {
            if (!_pm->_bullet_cooked_data_size)
                f.cmpUIntV(0);
            else // bullet cooked data version, used in case future bullet cooked data will not be compatible, 0=no data, 1=current version
            {
...
Bool PhysPart::loadData(File &f) {
    ...
    case 0: {
        Byte unused;
        f.getMulti(unused, _type, density);
        switch (_type) {
        case PHYS_NONE:
            if (f.ok())
                return true;
        ...
        case PHYS_SHAPE: {
            ...
            if (f.pos(pos + 72) && f.ok())
                return true; // 72 is the old size of raw memory for the shape data
        } break;
...
Flt PhysBody::volume() C {
    Flt volume = 0;
    REPA(T)
    volume += parts[i].volume();
    return volume;
}
Flt PhysBody::mass() C {
    Flt mass = 0;
...
/******************************************************************************/
PhysBody &PhysBody::transform(C Matrix3 &matrix) {
    REPAO(parts) *= matrix;
    setBox();
    return T;
}
PhysBody &PhysBody::transform(C Matrix &matrix) {
    REPAO(parts) *= matrix;
    setBox();
    return T;
}
PhysBody &PhysBody::mirrorX() {
    REPAO(parts).mirrorX();
    setBox();
    return T;
}
PhysBody &PhysBody::mirrorY() {
    REPAO(parts).mirrorY();
    setBox();
    return T;
}
PhysBody &PhysBody::mirrorZ() {
    REPAO(parts).mirrorZ();
    setBox();
    return T;
}
/******************************************************************************/
PhysBody &PhysBody::freeHelperData() {
    REPAO(parts).freeHelperData();
    return T;
}
/******************************************************************************/
...
Bool PhysBody::loadAdd(File &f, CChar *path) {
    ...
    if (phys.load(f, path)) {
        if (phys.parts.elms()) {
            ...
            FREPA(phys)
            Swap(parts[start + i], phys.parts[i]);
        }
        return true;
    }
    return false;
...
/******************************************************************************/
void PhysPart ::operator=(C Str &name) {
    if (!load(name))
        Exit(S + "Can't load Physical Body \"" + name + "\"");
}
void PhysBody ::operator=(C Str &name) {
    if (!load(name))
        Exit(S + "Can't load Physical Body \"" + name + "\"");
}
/******************************************************************************/
