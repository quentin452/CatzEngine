        ...
        if (_m) {
            ...
            if (_m) {
                ...
                if (Physx.physics) {
                    _m->userData = null;
                    _m->release();
                } // set user data to null in case material is ref counted and released later
#endif
                _m = null;
            }
        }
...
PhysMtrl &PhysMtrl::create() // don't delete so pointers don't need to be reallocated, instead just reset its values
{
    ...
    if (!_m) {
        ...
        if (!_m) {
            if (Physx.physics)
                if (_m = Physx.physics->createMaterial(0, 0, 0))
                    _m->userData = this;
        }
    }
#endif
...
Flt PhysMtrl::density() C { return _density; }
PhysMtrl &PhysMtrl::density(Flt x) {
    MAX(x, 0);
    _density = x;
    return T;
}
Flt PhysMtrl::damping() C { return _damping; }
PhysMtrl &PhysMtrl::damping(Flt x) {
    MAX(x, 0);
    _damping = x;
    return T;
}
Flt PhysMtrl::adamping() C { return _adamping; }
PhysMtrl &PhysMtrl::adamping(Flt x) {
    MAX(x, 0);
    _adamping = x;
    return T;
}
/******************************************************************************/
#if PHYSX
/*Flt PhysMtrl::bounciness      ()C {return _m ? _m->getRestitution    () : 0;}*/ PhysMtrl &PhysMtrl::bounciness(Flt x) {
    SAT(x);
    if (_bounciness != x) {
        _bounciness = x;
        if (_m)
...
/*Flt PhysMtrl::frictionStatic  ()C {return _m ? _m->getStaticFriction () : 0;}*/ PhysMtrl &PhysMtrl::frictionStatic(Flt x) {
    MAX(x, 0);
    if (_friction_static != x) {
        _friction_static = x;
        if (_m)
            _m->setStaticFriction(x);
    }
    return T;
}
/*Flt PhysMtrl::frictionDynamic ()C {return _m ? _m->getDynamicFriction() : 0;}*/ PhysMtrl &PhysMtrl::frictionDynamic(Flt x) {
    MAX(x, 0);
    if (_friction_dynamic != x) {
        _friction_dynamic = x;
        if (_m)
            _m->setDynamicFriction(x);
    }
    return T;
}
Flt PhysMtrl::frictionStaticA() C { return 0; }
PhysMtrl &PhysMtrl::frictionStaticA(Flt x) { return T; }
Flt PhysMtrl::frictionDynamicA() C { return 0; }
PhysMtrl &PhysMtrl::frictionDynamicA(Flt x) { return T; }
...
PhysMtrl &PhysMtrl::bouncinessMode(MODE mode) {
    if (_m)
        switch (mode) {
        case MODE_AVG:
            _m->setRestitutionCombineMode(PxCombineMode::eAVERAGE);
            break;
        case MODE_MUL:
            _m->setRestitutionCombineMode(PxCombineMode::eMULTIPLY);
            break;
        case MODE_MIN:
            _m->setRestitutionCombineMode(PxCombineMode::eMIN);
            break;
        case MODE_MAX:
            _m->setRestitutionCombineMode(PxCombineMode::eMAX);
            break;
        }
    return T;
}
PhysMtrl &PhysMtrl::frictionMode(MODE mode) {
    if (_m)
        switch (mode) {
        case MODE_AVG:
            _m->setFrictionCombineMode(PxCombineMode::eAVERAGE);
            break;
        case MODE_MUL:
            _m->setFrictionCombineMode(PxCombineMode::eMULTIPLY);
            break;
        case MODE_MIN:
            _m->setFrictionCombineMode(PxCombineMode::eMIN);
            break;
        case MODE_MAX:
            _m->setFrictionCombineMode(PxCombineMode::eMAX);
            break;
        }
    return T;
}
PhysMtrl::MODE PhysMtrl::bouncinessMode() C {
    if (_m)
        switch (_m->getRestitutionCombineMode()) {
...
PhysMtrl::MODE PhysMtrl::frictionMode() C {
    if (_m)
        switch (_m->getFrictionCombineMode()) {
        case PxCombineMode::eAVERAGE:
            return MODE_AVG;
        case PxCombineMode::eMULTIPLY:
            return MODE_MUL;
...
PhysMtrl &PhysMtrl::bounciness(Flt x) {
    SAT(x);
    if (_bounciness != x) {
        _bounciness = x;
        Changed(this);
...
PhysMtrl &PhysMtrl::frictionStatic(Flt x) {
    MAX(x, 0);
    if (_friction_static != x) {
        _friction_static = x;
        Changed(this);
...
PhysMtrl &PhysMtrl::frictionDynamic(Flt x) {
    MAX(x, 0);
    if (_friction_dynamic != x) {
        _friction_dynamic = x;
        Changed(this);
...
PhysMtrl::MODE PhysMtrl::bouncinessMode() C { return _bounciness_mode; }
PhysMtrl &PhysMtrl::bouncinessMode(MODE mode) {
    _bounciness_mode = mode;
    return T;
}
PhysMtrl::MODE PhysMtrl::frictionMode() C { return _friction_mode; }
PhysMtrl &PhysMtrl::frictionMode(MODE mode) {
    _friction_mode = mode;
    return T;
}
...
Bool PhysMtrl::save(File &f) C {
    ...
    if (!anisotropic())
        ...
        f.putBool(true).putFlt(frictionStaticA()).putFlt(frictionDynamicA()) << anisotropicDir();
    }
    return f.ok();
}
Bool PhysMtrl::load(File &f) {
    create();
    if (f.getUInt() == CC4_PHMT)
        switch (f.decUIntV()) {
        case 0: {
            bounciness(f.getFlt());
