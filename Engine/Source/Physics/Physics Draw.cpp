...
void DrawMesh(PxTriangleMesh &mesh, C Color &color, Bool fill) {
    if (mesh.getTriangleMeshFlags() & PxTriangleMeshFlag::e16_BIT_INDICES)
        DrawTris16((Vec *)mesh.getVertices(), (VecUS *)mesh.getTriangles(), mesh.getNbTriangles(), color, fill);
    else
        DrawTris((Vec *)mesh.getVertices(), (VecI *)mesh.getTriangles(), mesh.getNbTriangles(), color, fill);
}
    ...
    if (shape) {
        ...
        case PxGeometryType::ePLANE: {
            ...
            if (shape->getPlaneGeometry(plane)) {
                SetMatrix(matrix);
                Plane(VecZero, Vec(1, 0, 0)).drawInfiniteByResolution(color);
            }
        } break;
        case PxGeometryType::eSPHERE: {
            ...
            if (shape->getSphereGeometry(sphere)) {
                SetMatrix(matrix);
                Ball(sphere.radius).draw(color, fill);
            }
        } break;
        case PxGeometryType::eCAPSULE: {
            ...
            if (shape->getCapsuleGeometry(capsule)) {
                SetMatrix(matrix);
                Capsule(capsule.radius, (capsule.halfHeight + capsule.radius) * 2, VecZero, Vec(1, 0, 0)).draw(color, fill);
            }
        } break;
        case PxGeometryType::eBOX: {
            ...
            if (shape->getBoxGeometry(box)) {
                SetMatrix(matrix);
                Box(Vec(-box.halfExtents.x, -box.halfExtents.y, -box.halfExtents.z), Vec(box.halfExtents.x, box.halfExtents.y, box.halfExtents.z)).draw(color, fill);
            }
        } break;
        case PxGeometryType::eCONVEXMESH: {
            PxConvexMeshGeometry convex;
            if (shape->getConvexMeshGeometry(convex)) {
                SetMatrix(Matrix().setScale(Physx.vec(convex.scale.scale)) * matrix);
                DrawConvex(*convex.convexMesh, color, fill);
            }
        } break;
        case PxGeometryType::eTRIANGLEMESH: {
            PxTriangleMeshGeometry mesh;
            if (shape->getTriangleMeshGeometry(mesh)) {
                SetMatrix(Matrix().setScale(Physx.vec(mesh.scale.scale)) * matrix);
                DrawMesh(*mesh.triangleMesh, color, fill);
            }
        } break;
    ...
    if (actor)
        if (Frustum(Physx.box(actor->getWorldBounds()))) {
            if (PxRigidActor *rigid = actor->is<PxRigidActor>()) {
                ...
                REP(shapes) {
                    PxShape *shape = null;
                    if (rigid->getShapes(&shape, 1, i))
                        DrawShape(shape, color, fill, matrix);
                }
            }
        }
}
...
void PhysicsClass::draw() {
    ...
    if (Physx.world) {
        ...
        REP(actors) {
            PxActor *actor = null;
            if (Physx.world->getActors(types, &actor, 1, i))
                ActorDraw(actor, WHITE, false);
        }
    }
}
/******************************************************************************/
...
void DrawConvex(btConvexHullShape &convex, C Color &color, Bool fill) {
    VI.color(color);
    REP(convex.getNumEdges()) {
        btVector3 a, b;
        convex.getEdge(i, a, b);
        VI.line(Bullet.vec(a), Bullet.vec(b));
    }
    ...
    if (btStridingMeshInterface *smi = mesh.getMeshInterface()) {
        ...
        if (vtx_type == PHY_FLOAT && ind_type == PHY_INTEGER) {
            ...
            REP(tris) {
                VecI v = ind[i];
                if (fill)
                    VI.tri(pos[v.x], pos[v.y], pos[v.z]);
                else {
...
static void Draw(btCollisionShape *shape, C Color &color, Bool fill, C Matrix &matrix) {
    if (shape) {
        if (btCompoundShape *compound = CAST(btCompoundShape, shape)) {
            ...
            Draw(compound->getChildShape(i), color, fill, Bullet.matrix(compound->getChildTransform(i)) * matrix);
        } else if (btUniformScalingShape *uni = CAST(btUniformScalingShape, shape))
            Draw(uni->getChildShape(), color, fill, Matrix(matrix).scaleOrn(uni->getUniformScalingFactor()));
        else if (btScaledBvhTriangleMeshShape *mesh = CAST(btScaledBvhTriangleMeshShape, shape))
            Draw(mesh->getChildShape(), color, fill, Matrix(matrix).scaleOrnL(Bullet.vec(mesh->getLocalScaling())));
        else {
            SetMatrix(matrix);

            if (btBoxShape *box = CAST(btBoxShape, shape)) {
                Vec ext = Bullet.vec(box->getHalfExtentsWithMargin());
                Box(-ext, ext).draw(color, fill);
            } else if (btSphereShape *ball = CAST(btSphereShape, shape)) {
                Ball(ball->getRadius()).draw(color, fill);
            } else if (btCapsuleShape *capsule = CAST(btCapsuleShape, shape)) {
                Flt r = capsule->getRadius();
                Capsule(r, (capsule->getHalfHeight() + r) * 2).draw(color, fill);
            } else if (btCylinderShape *tube = CAST(btCylinderShape, shape)) {
                Tube(tube->getRadius(), tube->getHalfExtentsWithMargin().y() * 2).draw(color, fill);
            } else if (btStaticPlaneShape *plane = CAST(btStaticPlaneShape, shape)) {
                Vec n = Bullet.vec(plane->getPlaneNormal());
                Plane(plane->getPlaneConstant() * n, n).drawInfiniteByResolution(color);
            } else if (btConvexHullShape *convex = CAST(btConvexHullShape, shape))
                DrawConvex(*convex, color, fill);
            else if (btBvhTriangleMeshShape *mesh = CAST(btBvhTriangleMeshShape, shape))
                DrawMesh(*mesh, color, fill);
        }
    }
}
void Actor ::draw(C Color &color, Bool fill) C {
    if (_actor)
        Draw(_actor->getCollisionShape(), color, fill, massCenterMatrix());
}
void PhysicsClass::draw() {
    ReadLock lock(_rws);
    ...
    if (C Vec *pos = base.vtx.pos()) {
        ...
        if (fill) {
            REPA(base.tri) {
                ...
                VI.tri(pos[ind.x], pos[ind.y], pos[ind.z]);
            }
            REPA(base.quad) {
                C VecI4 &ind = base.quad.ind(i);
                VI.quad(pos[ind.x], pos[ind.y], pos[ind.z], pos[ind.w]);
            }
        } else {
            REPA(base.tri) {
                C VecI &ind = base.tri.ind(i);
                VI.line(pos[ind.x], pos[ind.y]);
                VI.line(pos[ind.y], pos[ind.z]);
                VI.line(pos[ind.z], pos[ind.x]);
            }
            REPA(base.quad) {
                C VecI4 &ind = base.quad.ind(i);
                VI.line(pos[ind.x], pos[ind.y]);
                VI.line(pos[ind.y], pos[ind.z]);
                VI.line(pos[ind.z], pos[ind.w]);
                VI.line(pos[ind.w], pos[ind.x]);
            }
        }
        VI.end();
    }
