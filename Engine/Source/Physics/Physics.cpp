...
PxU32 OutputStream::write(const void *src, PxU32 count) {
    if (pos + count > max)
        Realloc(data, max = pos + count + 8192, pos);
    CopyFast(data + pos, src, count);
    pos += count;
    return count;
}
PxU32 InputStream::read(void *dest, PxU32 count) {
    Int read = Min(count, size - pos);
    CopyFast(dest, data + pos, read);
    pos += read;
    return read;
}
/******************************************************************************/
...
static struct CpuDispatcherClass : PxCpuDispatcher // !!!!!!!!!!!!! if PhysX crashes here, then most likely invalid data was put into Actors (0, NaN, +-Inf, etc.) or other physics classes !!!!!!!!
{
    ...
    static void ProcessTask(PxBaseTask &task, Ptr user, Int thread_index) {
        task.run();
        task.release();
    }
...
static PxFilterFlags FilterFunc(PxFilterObjectAttributes attributes0, PxFilterData filterData0, PxFilterObjectAttributes attributes1, PxFilterData filterData1, PxPairFlags &pairFlags, const void *constantBlock, PxU32 constantBlockSize) {
    ...
    // per-group filtering
    if (!Physics.collides(filterData0.word0, filterData1.word0))
        return PxFilterFlag::eSUPPRESS;
    // per-actor filtering
    if (filterData0.word1 && filterData1.word1 && (Physx.ignoreMap(filterData0.word1, filterData1.word1) & (1 << (filterData1.word1 & 7))))
        return PxFilterFlag::eSUPPRESS;
    ...
    if (Physics.reports(filterData0.word0, filterData1.word0))
                    ...
                    if (PxRigidActor *actor1 = pairHeader.actors[1]->is<PxRigidActor>()) {
                        ...
                        if (actor0->getShapes(&shape0, 1))
                            if (actor1->getShapes(&shape1, 1)) {
                                ActorInfo actor_a, actor_b;
                                actor_a.set(shape0);
                                actor_b.set(shape1);
                                PhysContact contact[MAX_CONTACTS];
                                Int contacts = 0;
                                REP(nbPairs) {
                                    ...
                                    {
                                        PxContactPairPoint px_contact[MAX_CONTACTS];
                                        Int px_contacts = pair.extractContacts(px_contact, Elms(px_contact));
                                        REP(px_contacts) {
                                            if (InRange(contacts, contact)) {
                                                ...
                                                contact[contacts].force = Physx.vec(px_contact[i].impulse);
                                                if (Physics.stepTime())
                                                    contact[contacts].force /= Physics.stepTime(); // according to PhysX doc, impulse should be divided by simulation step to get force
                                                contacts++;
                                            }
                                        }
                                    }
        ...
        if (func)
            FREP(count) {
                ...
                if (!(pair.flags & (PxTriggerPairFlag::eREMOVED_SHAPE_TRIGGER | PxTriggerPairFlag::eREMOVED_SHAPE_OTHER))) // if both still exist
                {
                    ...
                    else if (pair.status & PxPairFlag::eNOTIFY_TOUCH_FOUND)
                        contact = CONTACT_NEW;
                    else
                        contact = CONTACT_KEEP;
                    ActorInfo trigger, actor;
                    trigger.set(pair.triggerShape);
                    actor.set(pair.otherShape);
                    func(trigger, actor, contact);
                }
            }
    }
} EventCallback;
...
struct CollisionDispatcher : btCollisionDispatcher {
    bool needsCollision(btCollisionObject *body0, btCollisionObject *body1) {
        if (super::needsCollision(body0, body1)) {
            // test pair<->pair
            if (RigidBody *rb = (RigidBody *)body0->getUserPointer())
                if (rb->ignore.has(body1))
                    return false;
            return true;
        }
        return false;
...
PhysicsClass &PhysicsClass::startSimulation(Flt dt) {
    if (!_simulated && created()) {
        ...
        _step_time = 0;
        _new_cpu_time = 0;
        _accumulated_time += Max(0, dt);
        if (_accumulated_time > EPS) {
...
void PhysicsClass::stepCompleted() {
    ...
    if (void (*step)() = simulation_step_completed)
        step(); // call custom callback if specified, copy to temp var to avoid multi-threaded issues
...
PhysicsClass &PhysicsClass::stopSimulation() {
    if (_simulated) {
        if (_new_updated) {
            ...
            stepCompleted();
#else
            REPA(Bullet.breakables)
            if (btTypedConstraint *joint = Bullet.breakables[i]) {
                ...
                Flt impulse = joint->getAppliedImpulse() * joint_impulse_scale;
                joint->internalSetAppliedImpulse(0);
                if (btRigidBody *rb0 = &joint->getRigidBodyA())
                    impulse /= rb0->getInvMass();
...
Bool PhysicsClass::ignored(PHYS_API(PxRigidActor, RigidBody) & a, PHYS_API(PxRigidActor, RigidBody) & b) {
    if (&a == &b)
        return true; // collisions are always ignored between the same actor (this is needed for Controller tests)
#if PHYSX
    PxShape *as, *bs;
    if (a.getShapes(&as, 1))
        if (UInt ai = as->getSimulationFilterData().word1)
            if (b.getShapes(&bs, 1))
                if (UInt bi = bs->getSimulationFilterData().word1)
                    return FlagOn(Physx.ignoreMap(ai, bi), 1 << (bi & 7));
    return false;
#else
...
PhysicsClass::PhysicsClass() {
    ...
    _hw = _updated = _new_updated = _last_updated = false;
    Zero(_contact_report);
    _simulated = 0;
    _precision = _actual_precision = _step_left = 0;
    _update_count = 0;
    Zero(_collision_array);
...
Bool PhysicsClass::createTry(CONTROLLER_SLOPE_SLIDING_MODE css, Bool hardware) {
    ...
    T._precision = T._actual_precision = 60;
            ...
            if (cook[0] = PxCreateCooking(PX_PHYSICS_VERSION, *foundation, cook_params)) {
                ...
                if (cook[1] = PxCreateCooking(PX_PHYSICS_VERSION, *foundation, cook_params)) {
                    ...
                    if (!scene_desc.cpuDispatcher)
                        scene_desc.cpuDispatcher = CpuDispatcher = PxDefaultCpuDispatcherCreate(Max(0, Cpu.threads() - 1)); // leave 1 thread for the main update
#else
                    if (!scene_desc.cpuDispatcher)
                        scene_desc.cpuDispatcher = &CpuDispatcher.create();
#endif
                    ...
                    if (scene_desc.isValid())
                        if (world = physics->createScene(scene_desc)) {
                            ...
                            MeshBase mesh;
                            mesh.create(Tube(1.0f, 1.0f, VecZero, Vec(1, 0, 0)), 0, 8);
                            if (wheel_mesh.createConvexTry(mesh)) {
    ...
    if (world) {
        ...
        REP(world->getNumCollisionObjects()) {
            if (btCollisionObject *obj = world->getCollisionObjectArray()[i]) {
                if (btRigidBody *body = CAST(btRigidBody, obj))
                    if (btMotionState *motion = body->getMotionState())
                        Delete(motion);
...
static bool MaterialCallback(btManifoldPoint &cp, const btCollisionObjectWrapper *colObj0Wrap, int partId0, int index0, const btCollisionObjectWrapper *colObj1Wrap, int partId1, int index1) {
    if (RigidBody *rb0 = (RigidBody *)colObj0Wrap->m_collisionObject->getUserPointer())
        if (RigidBody *rb1 = (RigidBody *)colObj1Wrap->m_collisionObject->getUserPointer()) {
            ...
            case PhysMtrl::MODE_AVG:
                cp.m_combinedRestitution = Avg(rb0->getRestitution(), rb1->getRestitution());
                break;
            case PhysMtrl::MODE_MUL:
                cp.m_combinedRestitution = (rb0->getRestitution() * rb1->getRestitution());
                break;
            case PhysMtrl::MODE_MIN:
                cp.m_combinedRestitution = Min(rb0->getRestitution(), rb1->getRestitution());
                break;
            case PhysMtrl::MODE_MAX:
                cp.m_combinedRestitution = Max(rb0->getRestitution(), rb1->getRestitution());
                break;
            }
            switch (rb0->material->frictionMode()) {
            case PhysMtrl::MODE_AVG:
                cp.m_combinedFriction = 2 * Avg(rb0->getFriction(), rb1->getFriction());
                break; // 2* to match PhysX scale
            case PhysMtrl::MODE_MUL:
                cp.m_combinedFriction = 2 * (rb0->getFriction() * rb1->getFriction());
                break; // 2* to match PhysX scale
            case PhysMtrl::MODE_MIN:
                cp.m_combinedFriction = 2 * Min(rb0->getFriction(), rb1->getFriction());
                break; // 2* to match PhysX scale
            case PhysMtrl::MODE_MAX:
                cp.m_combinedFriction = 2 * Max(rb0->getFriction(), rb1->getFriction());
                break; // 2* to match PhysX scale
            }
        }
    return true;
...
/******************************************************************************/
#if PHYSX
PhysicsClass &PhysicsClass::skin(Flt skin) {
    if (Physx.world) {
        T._skin = Max(0, skin);
    }
    return T;
}
PhysicsClass &PhysicsClass::gravity(C Vec &gravity) {
    if (T._gravity != gravity && Physx.world) {
        T._gravity = gravity;
        Physx.world->setGravity(Physx.vec(T._gravity));
    }
    return T;
}
#else
...
PhysicsClass &PhysicsClass::gravity(C Vec &gravity) {
    if (T._gravity != gravity && Bullet.world) {
        T._gravity = gravity;
        Bullet.world->setGravity(Bullet.vec(T._gravity));
    }
    return T;
}
#endif
...
PhysicsClass &PhysicsClass::timestep(PHYS_TIMESTEP_MODE mode) {
    if (InRange(mode, PHYS_TIMESTEP_NUM) && created())
        _timestep = mode;
    return T;
}
/******************************************************************************/
...
#if PHYSX
    if (Physx.world)
        return Physx.world->getNbActors(PxActorTypeFlag::eRIGID_STATIC | PxActorTypeFlag::eRIGID_DYNAMIC);
#else
    if (Bullet.world)
        return Bullet.world->getNumCollisionObjects();
#endif
    return 0;
}
/******************************************************************************/
