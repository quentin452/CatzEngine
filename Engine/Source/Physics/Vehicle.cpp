...
Vehicle &Vehicle::create(C PhysBody &body, C Params &params, Flt density, Flt scale) {
    if (!createTry(body, params, density, scale))
        Exit("Can't create Vehicle");
    return T;
}
Bool Vehicle::createTry(C PhysBody &body, C Params &params, Flt density, Flt scale) {
    ...
    if (_actor.createTry(body, density, scale)) {
        ...
        wheels_center /= WHEEL_NUM;
        _avg_wheel_dist = 0;
        REPA(_wheel)
        _avg_wheel_dist += Dist(_wheel[i].pos, wheels_center);
        _avg_wheel_dist /= WHEEL_NUM;
        _avg_wheel_dist *= SQRT2_2; // mul by SQRT2_2 because it's applied to both X and Z
...
Vec Vehicle::wheelAxis(WHEEL_TYPE wheel) C {
    ...
    if (IsFront(wheel)) {
        Flt c, s;
        CosSin(c, s, _angle * _max_angle);
        axis.set(c, 0, -s) *= _actor.orn(); // setRotateY(angle).x
    } else
        axis = _actor.orn().x;
...
Vec Vehicle::wheelDir(WHEEL_TYPE wheel) C {
    ...
    if (IsFront(wheel)) {
        Flt c, s;
        CosSin(c, s, _angle * _max_angle);
        dir.set(s, 0, c) *= _actor.orn(); // setRotateY(angle).z
    } else
        dir = _actor.orn().z;
    return dir;
}
Matrix Vehicle::wheelMatrix(WHEEL_TYPE wheel, Int flip_side) C {
    if (InRange(wheel, _wheel)) {
        ...
        Matrix m;
        if (IsFront(wheel))
            m.orn().setRotateXY(w.angle, _angle * _max_angle);
        else
            m.orn().setRotateX(w.angle);
        m.pos = w.pos;
...
Flt Vehicle::wheelLongSlip(WHEEL_TYPE wheel) C {
    if (onGround(wheel) && InRange(wheel, _wheel)) {
        ...
        if (long_speed == 0 && w.angle_vel == 0)
            return 0;
        if (Abs(long_speed) > Abs(w.angle_vel * w.radius))
            return (w.angle_vel * w.radius - long_speed) / (Abs(long_speed) + 0.1f);
        else
            return (w.angle_vel * w.radius - long_speed) / (Abs(w.angle_vel * w.radius) + 1.0f);
    }
    return 0;
}
...
Flt Vehicle::accel() C { return _accel; }
Vehicle &Vehicle::accel(Flt accel) {
    T._accel = Mid(accel, -1.0f, 1.0f);
    return T;
}
Flt Vehicle::brake() C { return _brake; }
Vehicle &Vehicle::brake(Flt brake) {
    T._brake = Sat(brake);
    return T;
}
Flt Vehicle::angle() C { return _angle; }
Vehicle &Vehicle::angle(Flt angle) {
    T._angle = Mid(angle, -1.0f, 1.0f);
    return T;
}
Flt Vehicle::maxAccel() C { return _max_accel; }
Vehicle &Vehicle::maxAccel(Flt accel) {
    T._max_accel = Max(accel, 0.0f);
    return T;
}
Flt Vehicle::maxBackAccel() C { return _max_back_accel; }
Vehicle &Vehicle::maxBackAccel(Flt accel) {
    T._max_back_accel = Max(accel, 0.0f);
    return T;
}
Flt Vehicle::maxBrake() C { return _max_brake; }
Vehicle &Vehicle::maxBrake(Flt brake) {
    T._max_brake = Max(brake, 0.0f);
    return T;
}
Flt Vehicle::brakeRatio() C { return _brake_ratio; }
Vehicle &Vehicle::brakeRatio(Flt ratio) {
    T._brake_ratio = Sat(ratio);
    return T;
}
Flt Vehicle::maxAngle() C { return _max_angle; }
Vehicle &Vehicle::maxAngle(Flt angle) {
    T._max_angle = Mid(angle, 0.0f, PI_2);
    return T;
}
Flt Vehicle::frictionLinear() C { return _fric_linear; }
Vehicle &Vehicle::frictionLinear(Flt friction) {
    T._fric_linear = Max(friction, 0.0f);
    return T;
}
Flt Vehicle::frictionScalar() C { return _fric_scalar; }
Vehicle &Vehicle::frictionScalar(Flt friction) {
    T._fric_scalar = Sat(friction);
    return T;
}
Flt Vehicle::suspSpring() C { return _susp_spring; }
Vehicle &Vehicle::suspSpring(Flt spring) {
    T._susp_spring = Max(spring, 0.0f);
    return T;
}
Flt Vehicle::suspDamping() C { return _susp_damping; }
Vehicle &Vehicle::suspDamping(Flt damping) {
    T._susp_damping = Max(damping, 0.0f);
    return T;
}
Flt Vehicle::suspCompress() C { return _susp_compress; }
Vehicle &Vehicle::suspCompress(Flt compress) {
    T._susp_compress = Sat(compress);
    return T;
}
Vehicle::WHEEL_DRIVE Vehicle::wheelDrive() C { return _wd; }
Vehicle &Vehicle::wheelDrive(WHEEL_DRIVE wd) {
    T._wd = wd;
    return T;
}

Flt Vehicle::energy() C { return _actor.energy(); }
Flt Vehicle::damping() C { return _actor.damping(); }
Vehicle &Vehicle::damping(Flt damping) {
    _actor.damping(damping);
    return T;
}
Flt Vehicle::adamping() C { return _actor.adamping(); }
Vehicle &Vehicle::adamping(Flt damping) {
    _actor.adamping(damping);
    return T;
}
Flt Vehicle::mass() C { return _actor.mass(); }
Vehicle &Vehicle::mass(Flt mass) {
    _actor.mass(mass);
    return T;
}
Vec Vehicle::massCenterL() C { return _actor.massCenterL(); }
Vehicle &Vehicle::massCenterL(C Vec &center) {
    _actor.massCenterL(center);
    return T;
}
Vec Vehicle::massCenterW() C { return _actor.massCenterW(); }
Vehicle &Vehicle::massCenterW(C Vec &center) {
    _actor.massCenterW(center);
    return T;
}
Vec Vehicle::inertia() C { return _actor.inertia(); }
Vehicle &Vehicle::inertia(C Vec &inertia) {
    _actor.inertia(inertia);
    return T;
}
Vec Vehicle::pos() C { return _actor.pos(); }
Vehicle &Vehicle::pos(C Vec &pos) {
    _actor.pos(pos);
    return T;
}
Matrix3 Vehicle::orn() C { return _actor.orn(); }
Vehicle &Vehicle::orn(C Matrix3 &orn) {
    _actor.orn(orn);
    return T;
}
Matrix Vehicle::matrix() C { return _actor.matrix(); }
Vehicle &Vehicle::matrix(C Matrix &matrix) {
    _actor.matrix(matrix);
    return T;
}
Vec Vehicle::vel() C { return _actor.vel(); }
Vehicle &Vehicle::vel(C Vec &vel) {
    _actor.vel(vel);
    return T;
}
Vec Vehicle::angVel() C { return _actor.angVel(); }
Vehicle &Vehicle::angVel(C Vec &vel) {
    _actor.angVel(vel);
    return T;
}
...
Vehicle &Vehicle::addTorque(C Vec &torque) {
    _actor.addTorque(torque);
    return T;
}
Vehicle &Vehicle::addAngVel(C Vec &ang_vel) {
    _actor.addAngVel(ang_vel);
    return T;
}
Vehicle &Vehicle::addForce(C Vec &force) {
    _actor.addForce(force);
    return T;
}
Vehicle &Vehicle::addForce(C Vec &force, C Vec &pos) {
    _actor.addForce(force, pos);
    return T;
}
Vehicle &Vehicle::addImpulse(C Vec &impulse) {
    _actor.addImpulse(impulse);
    return T;
}
Vehicle &Vehicle::addImpulse(C Vec &impulse, C Vec &pos) {
    _actor.addImpulse(impulse, pos);
    return T;
}
Vehicle &Vehicle::addVel(C Vec &vel) {
    _actor.addVel(vel);
    return T;
}
Vehicle &Vehicle::addAccel(C Vec &accel) {
    _actor.addAccel(accel);
    return T;
}

Bool Vehicle::collision() C { return _actor.collision(); }
Vehicle &Vehicle::collision(Bool on) {
    _actor.collision(on);
    return T;
}
Bool Vehicle::gravity() C { return _actor.gravity(); }
Vehicle &Vehicle::gravity(Bool on) {
    _actor.gravity(on);
    return T;
}
Bool Vehicle::ray() C { return _actor.ray(); }
Vehicle &Vehicle::ray(Bool on) {
    _actor.ray(on);
    return T;
}
Bool Vehicle::sleep() C { return _actor.sleep(); }
Vehicle &Vehicle::sleep(Bool sleep) {
    _actor.sleep(sleep);
    return T;
}
Flt Vehicle::sleepEnergy() C { return _actor.sleepEnergy(); }
Vehicle &Vehicle::sleepEnergy(Flt energy) {
    _actor.sleepEnergy(energy);
    return T;
}
Bool Vehicle::ccd() C { return _actor.ccd(); }
Vehicle &Vehicle::ccd(Bool on) {
    _actor.ccd(on);
    return T;
}
Ptr Vehicle::user() C { return _actor.user(); }
Vehicle &Vehicle::user(Ptr user) {
    _actor.user(user);
    return T;
}
Ptr Vehicle::obj() C { return _actor.obj(); }
Vehicle &Vehicle::obj(Ptr obj) {
    _actor.obj(obj);
    return T;
}
Byte Vehicle::group() C { return _actor.group(); }
Vehicle &Vehicle::group(Byte group) {
    _actor.group(group);
    return T;
}
Byte Vehicle::dominance() C { return _actor.dominance(); }
Vehicle &Vehicle::dominance(Byte dominance) {
    _actor.dominance(dominance);
    return T;
}
PhysMtrl *Vehicle::material() C { return _actor.material(); }
Vehicle &Vehicle::material(PhysMtrl *material) {
...
void Vehicle::update() {
    ...
    Flt brake = _brake;
    if (brake)
        brake *= _max_brake * Physics.time() * 0.5f; // by half because of 2 wheels per front/rear
    ...
    REPA(_wheel) {
        ...
        hit_callback.has = false;
        Physics.ray(wheel_pos[i] + actor_matrix.y * start, actor_matrix.y * -(start + wheel.radius), hit_callback, Physics.collisionGroups(_actor.group()));
        if (hit_callback.has) // hit something
        {
            ...
            if (dist < 0) // we're touching it
            {
                ...
                // wheel dir
                wheel_orn[i] = actor_matrix;
                if (IsFront(WHEEL_TYPE(i)))
                    wheel_orn[i].rotateYL(_angle * _max_angle);

                // accelerate
                if (accel && HasAccel(WHEEL_TYPE(i), _wd)) {
                    Vec dir = PointOnPlane(wheel_orn[i].z, wheel.contact.normal);
                    dir.setLength(accel);
    ...
    Flt scalar_mul = 1 - _fric_scalar;
    if (scalar_mul > 0)
        scalar_mul = Pow(scalar_mul, Physics.time()); // watch out for Pow(<=0, ..) which causes NaN
    ...
    REP(WHEEL_NUM) {
        ...
        if (onGround(WHEEL_TYPE(i))) {
            ...
            Vec wheel_uni(IsRight(WHEEL_TYPE(i)) ? _avg_wheel_dist : -_avg_wheel_dist, 0, IsFront(WHEEL_TYPE(i)) ? _avg_wheel_dist : -_avg_wheel_dist);
            (wheel_uni *= actor_matrix.orn()) += actor_com; // set as uniform point along center of mass (reduces jittering)
...
struct PhysXVehicle // Physical actor of vehicle type (this class is available only in PhysX)
{
    ...
    struct Wheel {
        ...
        void set(Flt radius, Flt width, C Vec &pos) {
            T.radius = radius;
            T.width = width;
            T.pos = pos;
        }
    ...
    struct Params // all of these parameters (except masses) will be scaled according to 'scale' during vehicle creation
    {
        ...
        Params() {
            mass = 1500;
            wheel_mass = 20;
            mass_center_offset.set(0, -0.1f, 0);
        }
    };
    ...
    Int gear() C;
    PhysXVehicle &gear(Int gear); // get/set current   gear (-1=reverse, 0=neutral, 1..=normal gear)
#if EE_PRIVATE
    Int targetGear() C;
    PhysXVehicle &targetGear(Int gear); // set     target    gear
#endif
    Bool autoGear() C;
    PhysXVehicle &autoGear(Bool on); // get/set automatic gear changing     , true/false, default=true
    Flt accel() C;
    PhysXVehicle &accel(Flt accel); // get/set           acceleration      ,  0..1
    Flt brake() C;
    PhysXVehicle &brake(Flt brake); // get/set current         brake       ,  0..1
    Flt handBrake() C;
    PhysXVehicle &handBrake(Flt brake); // get/set current   hand  brake       ,  0..1
    Flt angle() C;
    PhysXVehicle &angle(Flt angle); // get/set current   steer angle       , -1..1
    Flt maxBrake() C;
    PhysXVehicle &maxBrake(Flt brake); // get/set max             brake torque,  0..Inf   , default=1500
    Flt maxHandBrake() C;
    PhysXVehicle &maxHandBrake(Flt brake); // get/set max       hand  brake torque,  0..Inf   , default=4000
    Flt maxAngle() C;
    PhysXVehicle &maxAngle(Flt angle); // get/set max       steer angle       ,  0..PI_2  , default=PI_3
    Flt wheelDamping() C;
    PhysXVehicle &wheelDamping(Flt damping); // get/set           wheel damping     ,  0..Inf   , default=0.05
    Flt suspSpring() C;
    PhysXVehicle &suspSpring(Flt spring); // get/set   suspension spring strength,  0..Inf   , default=35000
    Flt suspDamping() C;
    PhysXVehicle &suspDamping(Flt damping); // get/set   suspension spring damping ,  0..Inf   , default= 4500
    Flt suspCompress() C;
    PhysXVehicle &suspCompress(Flt compress); // get/set   suspension max compression,  0..1     , default=0.75 (1.0 means full wheel radius)
    Flt suspElongate() C;
    PhysXVehicle &suspElongate(Flt elongate); // get/set   suspension max elongation ,  0..1     , default=0.25 (1.0 means full wheel radius)
    Flt enginePeakTorque() C;
    PhysXVehicle &enginePeakTorque(Flt peak); // get/set   engine peak torque        ,  0..Inf   , default= 500, maximum acceleration applied to engine rotation
    Flt engineMaxSpeed() C;
    PhysXVehicle &engineMaxSpeed(Flt omega); // get/set   engine max rotation spead ,  0..Inf   , default= 600 (in radians per second)
    DIFF_TYPE diffType() C;
    PhysXVehicle &diffType(DIFF_TYPE diff); // get/set   differential type         ,  DIFF_TYPE, default=DIFF_LS_4WD
#if EE_PRIVATE
    Int tireType() C;
    PhysXVehicle &tireType(Int type); // get/set   tire type                 ,  0..Inf   , default=0 (this is a custom value not used internally by the engine, you can set it to anything you want and use it for custom friction calculations when using 'Physics.wheelFriction')
#endif
    Flt tireLongStiff() C;
    PhysXVehicle &tireLongStiff(Flt f); // get/set longitudinal stiffness per unit longitudinal slip per unit gravity, specified in N per radian per unit gravitational acceleration. Longitudinal stiffness of the tire per unit longitudinal slip is calculated as "gravity*tireLongStiff", 0..Inf, default=1000
    Flt tireLatStiffX() C;
    PhysXVehicle &tireLatStiffX(Flt f); // get/set the minimum normalised load (load/restLoad) that gives a flat lateral stiffness response        (Tire lateral stiffness is typically a graph of tire load that has linear behaviour near zero load and flattens at large loads)          , 0..Inf, default=2
    Flt tireLatStiffY() C;
    PhysXVehicle &tireLatStiffY(Flt f); // get/set the maximum possible lateral stiffness divided by the rest tire load, specified in "per radian" (Tire lateral stiffness is typically a graph of tire load that has linear behaviour near zero load and flattens at large loads)          , 0..Inf, default=17.9
    Flt tireCamberStiff() C;
    PhysXVehicle &tireCamberStiff(Flt f); // get/set Camber stiffness, specified in kilograms per radian                                                                                                                                                                                      , 0..Inf, default=5.73
    ...
    Flt tireFuncPoint0Fric() C;
    PhysXVehicle &tireFuncPoint0Fric(Flt f); // get/set friction value at point 0, 0..Inf, default=1
    Flt tireFuncPoint1Slip() C;
    PhysXVehicle &tireFuncPoint1Slip(Flt f); // get/set slip     value at point 1, 0..Inf, default=0.1
    Flt tireFuncPoint1Fric() C;
    PhysXVehicle &tireFuncPoint1Fric(Flt f); // get/set friction value at point 1, 0..Inf, default=1
    Flt tireFuncPoint2Slip() C;
    PhysXVehicle &tireFuncPoint2Slip(Flt f); // get/set slip     value at point 2, 0..Inf, default=1
    Flt tireFuncPoint2Fric() C;
    PhysXVehicle &tireFuncPoint2Fric(Flt f); // get/set friction value at point 2, 0..Inf, default=1
    ...
    Flt damping() C;
    PhysXVehicle &damping(Flt damping); // get/set linear  damping, 0..Inf, default=0.05
    Flt adamping() C;
    PhysXVehicle &adamping(Flt damping); // get/set angular damping, 0..Inf, default=0.05
    ...
    Vec inertia() C;
    PhysXVehicle &inertia(C Vec &inertia); // get/set inertia tensor
    Vec pos() C;
    PhysXVehicle &pos(C Vec &pos); // get/set position
    Matrix3 orn() C;
    PhysXVehicle &orn(C Matrix3 &orn); // get/set orientation, 'orn'    must be normalized
    Matrix matrix() C;
    PhysXVehicle &matrix(C Matrix &matrix); // get/set matrix     , 'matrix' must be normalized
    Vec vel() C;
    PhysXVehicle &vel(C Vec &vel); // get/set         velocity
    Vec angVel() C;
    PhysXVehicle &angVel(C Vec &vel); // get/set angular velocity
    ...
    Bool ray() C;
    PhysXVehicle &ray(Bool on); // get/set if this actor should be included when performing ray tests
    Bool collision() C;
    PhysXVehicle &collision(Bool on); // get/set if this actor should collide with other actors in the world
    Bool sleep() C;
    PhysXVehicle &sleep(Bool sleep); // get/set sleeping
    Flt sleepEnergy() C;
    PhysXVehicle &sleepEnergy(Flt energy); // get/set the amount of energy below the actor is put to sleep, default=0.005
    Bool ccd() C;
    PhysXVehicle &ccd(Bool on); // get/set continuous collision detection
    Ptr user() C;
    PhysXVehicle &user(Ptr user); // get/set custom user data
    Ptr obj() C;
    PhysXVehicle &obj(Ptr obj); // get/set pointer to object containing the actor
    Byte group() C;
    PhysXVehicle &group(Byte group); // get/set collision group (0..31, default value is taken according to ACTOR_GROUP)
    Byte dominance() C;
    PhysXVehicle &dominance(Byte dominance); // get/set dominance index (0..31, default=0), for more information about dominance please check comments on 'Physics.dominance' method

    PhysMtrl *bodyMaterial() C;
    PhysXVehicle &bodyMaterial(PhysMtrl *material); // get/set physics material for vehicle body   (use 'null' for default material)
    PhysMtrl *wheelMaterial() C;
    PhysXVehicle &wheelMaterial(PhysMtrl *material); // get/set physics material for vehicle wheels (use 'null' for default material)
    ...
    PhysXVehicle &gearUp() {
        Int g = gear();
        return gear((g <= 0) ? 1 : g + 1);
    } // set gear up   (avoiding neutral gear)
    PhysXVehicle &gearDown() {
        Int g = gear();
        return gear((g <= 1) ? -1 : g - 1);
    } // set gear down (avoiding neutral gear)
...
PhysXVehicle &PhysXVehicle::create(C PhysBody &body, C Params &params, Flt scale) {
    if (!createTry(body, params, scale))
        Exit("Can't create PhysXVehicle"
#if !PHYSX
...
Flt PhysXVehicle::damping() C { return _actor.damping(); }
PhysXVehicle &PhysXVehicle::damping(Flt damping) {
    _actor.damping(damping);
    return T;
}
Flt PhysXVehicle::adamping() C { return _actor.adamping(); }
PhysXVehicle &PhysXVehicle::adamping(Flt damping) {
    _actor.adamping(damping);
    return T;
}
...
Vec PhysXVehicle::inertia() C { return _actor.inertia(); }
PhysXVehicle &PhysXVehicle::inertia(C Vec &inertia) {
    _actor.inertia(inertia);
    return T;
}
Vec PhysXVehicle::pos() C { return _actor.pos(); }
PhysXVehicle &PhysXVehicle::pos(C Vec &pos) {
    _actor.pos(pos);
    return T;
}
Matrix3 PhysXVehicle::orn() C { return _actor.orn(); }
PhysXVehicle &PhysXVehicle::orn(C Matrix3 &orn) {
    _actor.orn(orn);
    return T;
}
Matrix PhysXVehicle::matrix() C { return _actor.matrix(); }
PhysXVehicle &PhysXVehicle::matrix(C Matrix &matrix) {
    _actor.matrix(matrix);
    return T;
}
Vec PhysXVehicle::vel() C { return _actor.vel(); }
PhysXVehicle &PhysXVehicle::vel(C Vec &vel) {
    _actor.vel(vel);
    return T;
}
Vec PhysXVehicle::angVel() C { return _actor.angVel(); }
PhysXVehicle &PhysXVehicle::angVel(C Vec &vel) {
    _actor.angVel(vel);
    return T;
}
...
PhysXVehicle &PhysXVehicle::addTorque(C Vec &torque) {
    _actor.addTorque(torque);
    return T;
}
PhysXVehicle &PhysXVehicle::addAngVel(C Vec &ang_vel) {
    _actor.addAngVel(ang_vel);
    return T;
}
PhysXVehicle &PhysXVehicle::addForce(C Vec &force) {
    _actor.addForce(force);
    return T;
}
PhysXVehicle &PhysXVehicle::addForce(C Vec &force, C Vec &pos) {
    _actor.addForce(force, pos);
    return T;
}
PhysXVehicle &PhysXVehicle::addImpulse(C Vec &impulse) {
    _actor.addImpulse(impulse);
    return T;
}
PhysXVehicle &PhysXVehicle::addImpulse(C Vec &impulse, C Vec &pos) {
    _actor.addImpulse(impulse, pos);
    return T;
}
PhysXVehicle &PhysXVehicle::addVel(C Vec &vel) {
    _actor.addVel(vel);
    return T;
}
PhysXVehicle &PhysXVehicle::addAccel(C Vec &accel) {
    _actor.addAccel(accel);
    return T;
}

Bool PhysXVehicle::collision() C { return _actor.collision(); }
Bool PhysXVehicle::ray() C { return _actor.ray(); }
PhysXVehicle &PhysXVehicle::ray(Bool on) {
    _actor.ray(on);
    return T;
}
Bool PhysXVehicle::sleep() C { return _actor.sleep(); }
PhysXVehicle &PhysXVehicle::sleep(Bool sleep) {
    _actor.sleep(sleep);
    return T;
}
Flt PhysXVehicle::sleepEnergy() C { return _actor.sleepEnergy(); }
PhysXVehicle &PhysXVehicle::sleepEnergy(Flt energy) {
    _actor.sleepEnergy(energy);
    return T;
}
Bool PhysXVehicle::ccd() C { return _actor.ccd(); }
PhysXVehicle &PhysXVehicle::ccd(Bool on) {
    _actor.ccd(on);
    return group(group());
} // need to re-apply group because of 'eCCD_LINEAR' in 'SimulationFilterData' (also 'Actor.ccd' calls 'Actor.group' and that clears 'vehicle_id')
Ptr PhysXVehicle::user() C { return _actor.user(); }
PhysXVehicle &PhysXVehicle::user(Ptr user) {
    _actor.user(user);
    return T;
}
Ptr PhysXVehicle::obj() C { return _actor.obj(); }
PhysXVehicle &PhysXVehicle::obj(Ptr obj) {
    _actor.obj(obj);
    return T;
}
Byte PhysXVehicle::dominance() C { return _actor.dominance(); }
PhysXVehicle &PhysXVehicle::dominance(Byte dominance) {
    _actor.dominance(dominance);
    return T;
}
...
PhysXVehicle &PhysXVehicle::del() {
    if (_vehicle) {
        ...
        if (_vehicle) {
            ...
            _vehicle = null;
            if (_vehicle_id) {
                Physx.vehicle_id_gen.Return(_vehicle_id);
                _vehicle_id = 0;
            }
        }
    }
            ...
            if (_actor.isDynamic())
                if (_vehicle = PxVehicleDrive4W::allocate(WHEEL_NUM))
                    if (PxVehicleWheelsSimData *wheels_sim_data = PxVehicleWheelsSimData::allocate(WHEEL_NUM)) // this must be manually released
                    {
                        ...
                        // get dimensions
                        T._scale = scale;
                        if (!body.box.size().any())
                            ConstCast(body).setBox(); // if someone forgot to set phys box then recalculate it (can result in invalid vehicle behavior with zero box)
                        Box body_box = body.box * scale;
                        Vec body_dims = body_box.size();
                        ...
                        if (_actor._dynamic)
                            _actor._dynamic->setCMassLocalPose(PxTransform(Physx.vec(mass_center))); // _actor.massCenterL(mass_center); (reset mass orientation as well because otherwise there was a case when rear wheels were placed below ground)
...
Vec PhysXVehicle::wheelPosL(WHEEL_TYPE wheel) C {
    if (_vehicle && _actor._actor && InRange(wheel, WHEEL_NUM)) {
        ...
        if (InRange(shape_index, _actor._actor->getNbShapes())) {
            PxShape *shape;
            if (_actor._actor->getShapes(&shape, 1, shape_index)) {
...
PhysXVehicle &PhysXVehicle::gear(Int gear) {
    if (_vehicle)
        _vehicle->mDriveDynData.forceGearChange(Mid(gear + PxVehicleGearsData::eNEUTRAL, 0, gears() + PxVehicleGearsData::eNEUTRAL));
    return T;
}
PhysXVehicle &PhysXVehicle::targetGear(Int gear) {
    if (_vehicle)
        _vehicle->mDriveDynData.startGearChange(Mid(gear + PxVehicleGearsData::eNEUTRAL, 0, gears() + PxVehicleGearsData::eNEUTRAL));
    return T;
}
PhysXVehicle &PhysXVehicle::autoGear(Bool on) {
    if (_vehicle)
        _vehicle->mDriveDynData.setUseAutoGears(on);
    return T;
}
PhysXVehicle &PhysXVehicle::angle(Flt angle) {
    if (_vehicle)
        _vehicle->mDriveDynData.setAnalogInput(PxVehicleDrive4WControl::eANALOG_INPUT_STEER_RIGHT, Mid(angle, -1.0f, 1.0f));
    return T;
}
PhysXVehicle &PhysXVehicle::accel(Flt accel) {
    if (_vehicle)
        _vehicle->mDriveDynData.setAnalogInput(PxVehicleDrive4WControl::eANALOG_INPUT_ACCEL, Sat(accel));
    return T;
}
PhysXVehicle &PhysXVehicle::brake(Flt brake) {
    if (_vehicle)
        _vehicle->mDriveDynData.setAnalogInput(PxVehicleDrive4WControl::eANALOG_INPUT_BRAKE, Sat(brake));
    return T;
}
PhysXVehicle &PhysXVehicle::handBrake(Flt brake) {
    if (_vehicle)
        _vehicle->mDriveDynData.setAnalogInput(PxVehicleDrive4WControl::eANALOG_INPUT_HANDBRAKE, Sat(brake));
    return T;
}
...
PhysXVehicle &PhysXVehicle::maxAngle(Flt angle) {
    if (_vehicle) {
        Clamp(angle, 0, PI_2);
        PxVehicleWheelData data = _vehicle->mWheelsSimData.getWheelData(WHEEL_LEFT_FRONT);
        data.mMaxSteer = angle;
        _vehicle->mWheelsSimData.setWheelData(WHEEL_LEFT_FRONT, data);
        data = _vehicle->mWheelsSimData.getWheelData(WHEEL_RIGHT_FRONT);
        data.mMaxSteer = angle;
        _vehicle->mWheelsSimData.setWheelData(WHEEL_RIGHT_FRONT, data);
    }
    return T;
}
PhysXVehicle &PhysXVehicle::maxBrake(Flt brake) {
    if (_vehicle) {
        MAX(brake, 0);
        PxVehicleWheelData data = _vehicle->mWheelsSimData.getWheelData(WHEEL_LEFT_FRONT);
        data.mMaxBrakeTorque = brake;
        _vehicle->mWheelsSimData.setWheelData(WHEEL_LEFT_FRONT, data);
        data = _vehicle->mWheelsSimData.getWheelData(WHEEL_RIGHT_FRONT);
        data.mMaxBrakeTorque = brake;
        _vehicle->mWheelsSimData.setWheelData(WHEEL_RIGHT_FRONT, data);
        data = _vehicle->mWheelsSimData.getWheelData(WHEEL_LEFT_REAR);
        data.mMaxBrakeTorque = brake;
        _vehicle->mWheelsSimData.setWheelData(WHEEL_LEFT_REAR, data);
        data = _vehicle->mWheelsSimData.getWheelData(WHEEL_RIGHT_REAR);
        data.mMaxBrakeTorque = brake;
        _vehicle->mWheelsSimData.setWheelData(WHEEL_RIGHT_REAR, data);
    }
    return T;
}
PhysXVehicle &PhysXVehicle::maxHandBrake(Flt brake) {
    if (_vehicle) {
        MAX(brake, 0);
        PxVehicleWheelData data = _vehicle->mWheelsSimData.getWheelData(WHEEL_LEFT_REAR);
        data.mMaxHandBrakeTorque = brake;
        _vehicle->mWheelsSimData.setWheelData(WHEEL_LEFT_REAR, data);
        data = _vehicle->mWheelsSimData.getWheelData(WHEEL_RIGHT_REAR);
        data.mMaxHandBrakeTorque = brake;
        _vehicle->mWheelsSimData.setWheelData(WHEEL_RIGHT_REAR, data);
...
PhysXVehicle &PhysXVehicle::wheelDamping(Flt damping) {
    if (_vehicle) {
        MAX(damping, 0);
        REP(WHEEL_NUM) {
            PxVehicleWheelData data = _vehicle->mWheelsSimData.getWheelData(i);
            data.mDampingRate = damping;
            _vehicle->mWheelsSimData.setWheelData(i, data);
        }
    }
    return T;
}
PhysXVehicle &PhysXVehicle::suspSpring(Flt spring) {
    if (_vehicle) {
        MAX(spring, 0);
        REP(WHEEL_NUM) {
            PxVehicleSuspensionData data = _vehicle->mWheelsSimData.getSuspensionData(i);
            data.mSpringStrength = spring;
            _vehicle->mWheelsSimData.setSuspensionData(i, data);
        }
    }
    return T;
}
PhysXVehicle &PhysXVehicle::suspDamping(Flt damping) {
    if (_vehicle) {
        MAX(damping, 0);
        REP(WHEEL_NUM) {
            PxVehicleSuspensionData data = _vehicle->mWheelsSimData.getSuspensionData(i);
            data.mSpringDamperRate = damping;
            _vehicle->mWheelsSimData.setSuspensionData(i, data);
        }
    }
    return T;
}
PhysXVehicle &PhysXVehicle::suspCompress(Flt compress) {
    if (_vehicle) {
        SAT(compress);
        REP(WHEEL_NUM) {
            PxVehicleSuspensionData data = _vehicle->mWheelsSimData.getSuspensionData(i);
            data.mMaxCompression = compress * _vehicle->mWheelsSimData.getWheelData(i).mRadius;
            _vehicle->mWheelsSimData.setSuspensionData(i, data);
        }
    }
    return T;
}
PhysXVehicle &PhysXVehicle::suspElongate(Flt elongate) {
    if (_vehicle) {
        SAT(elongate);
        REP(WHEEL_NUM) {
            PxVehicleSuspensionData data = _vehicle->mWheelsSimData.getSuspensionData(i);
            data.mMaxDroop = elongate * _vehicle->mWheelsSimData.getWheelData(i).mRadius;
            _vehicle->mWheelsSimData.setSuspensionData(i, data);
        }
    }
    return T;
...
PhysXVehicle &PhysXVehicle::enginePeakTorque(Flt peak) {
    if (_vehicle) {
        MAX(peak, 0);
        PxVehicleEngineData data = _vehicle->mDriveSimData.getEngineData();
        data.mPeakTorque = peak;
        _vehicle->mDriveSimData.setEngineData(data);
    }
    return T;
}
PhysXVehicle &PhysXVehicle::engineMaxSpeed(Flt omega) {
    if (_vehicle) {
        MAX(omega, 0);
        PxVehicleEngineData data = _vehicle->mDriveSimData.getEngineData();
        data.mMaxOmega = omega;
        _vehicle->mDriveSimData.setEngineData(data);
    }
    return T;
}

PhysXVehicle &PhysXVehicle::tireType(Int type) {
    if (_vehicle)
        REP(WHEEL_NUM) {
            PxVehicleTireData data = _vehicle->mWheelsSimData.getTireData(i);
            data.mType = type;
            _vehicle->mWheelsSimData.setTireData(i, data);
        }
    return T;
}
PhysXVehicle &PhysXVehicle::tireLongStiff(Flt f) {
    if (_vehicle) {
        MAX(f, 0);
        REP(WHEEL_NUM) {
            PxVehicleTireData data = _vehicle->mWheelsSimData.getTireData(i);
            data.mLongitudinalStiffnessPerUnitGravity = f;
            _vehicle->mWheelsSimData.setTireData(i, data);
        }
    }
    return T;
}
PhysXVehicle &PhysXVehicle::tireLatStiffX(Flt f) {
    if (_vehicle) {
        MAX(f, 0);
        REP(WHEEL_NUM) {
            PxVehicleTireData data = _vehicle->mWheelsSimData.getTireData(i);
...
PhysXVehicle &PhysXVehicle::tireLatStiffY(Flt f) {
    if (_vehicle) {
        MAX(f, 0);
        REP(WHEEL_NUM) {
            PxVehicleTireData data = _vehicle->mWheelsSimData.getTireData(i);
...
PhysXVehicle &PhysXVehicle::tireCamberStiff(Flt f) {
    if (_vehicle) {
        MAX(f, 0);
        REP(WHEEL_NUM) {
            PxVehicleTireData data = _vehicle->mWheelsSimData.getTireData(i);
            data.mCamberStiffnessPerUnitGravity = f;
            _vehicle->mWheelsSimData.setTireData(i, data);
        }
    }
    return T;
}
PhysXVehicle &PhysXVehicle::tireFuncPoint0Fric(Flt f) {
    if (_vehicle) {
        MAX(f, 0);
        REP(WHEEL_NUM) {
            PxVehicleTireData data = _vehicle->mWheelsSimData.getTireData(i);
            data.mFrictionVsSlipGraph[0][1] = f;
            _vehicle->mWheelsSimData.setTireData(i, data);
        }
    }
    return T;
}
PhysXVehicle &PhysXVehicle::tireFuncPoint1Slip(Flt f) {
    if (_vehicle) {
        MAX(f, 0);
        REP(WHEEL_NUM) {
            PxVehicleTireData data = _vehicle->mWheelsSimData.getTireData(i);
            data.mFrictionVsSlipGraph[1][0] = f;
            _vehicle->mWheelsSimData.setTireData(i, data);
        }
    }
    return T;
}
PhysXVehicle &PhysXVehicle::tireFuncPoint1Fric(Flt f) {
    if (_vehicle) {
        MAX(f, EPS);
        REP(WHEEL_NUM) {
            PxVehicleTireData data = _vehicle->mWheelsSimData.getTireData(i);
            data.mFrictionVsSlipGraph[1][1] = f;
            _vehicle->mWheelsSimData.setTireData(i, data);
        }
    }
    return T;
} // without EPS crash may occur
PhysXVehicle &PhysXVehicle::tireFuncPoint2Slip(Flt f) {
    if (_vehicle) {
        MAX(f, 0);
        REP(WHEEL_NUM) {
            PxVehicleTireData data = _vehicle->mWheelsSimData.getTireData(i);
            data.mFrictionVsSlipGraph[2][0] = f;
            _vehicle->mWheelsSimData.setTireData(i, data);
        }
    }
    return T;
}
PhysXVehicle &PhysXVehicle::tireFuncPoint2Fric(Flt f) {
    if (_vehicle) {
        MAX(f, EPS);
        REP(WHEEL_NUM) {
            PxVehicleTireData data = _vehicle->mWheelsSimData.getTireData(i);
            data.mFrictionVsSlipGraph[2][1] = f;
            _vehicle->mWheelsSimData.setTireData(i, data);
        }
    }
    return T;
...
PhysXVehicle &PhysXVehicle::precision(Flt thresholdLongitudinalSpeed, UInt lowForwardSpeedSubStepCount, UInt highForwardSpeedSubStepCount) {
    if (_vehicle)
        _vehicle->mWheelsSimData.setSubStepCount(thresholdLongitudinalSpeed, lowForwardSpeedSubStepCount, highForwardSpeedSubStepCount);
    return T;
}
/******************************************************************************/
...
PhysXVehicle &PhysXVehicle::collision(Bool on) {
    if (_actor._actor) {
        VecI4 wheel;
        if (_vehicle)
            wheel.set(_vehicle->mWheelsSimData.getWheelShapeMapping(0), _vehicle->mWheelsSimData.getWheelShapeMapping(1), _vehicle->mWheelsSimData.getWheelShapeMapping(2), _vehicle->mWheelsSimData.getWheelShapeMapping(3));
        else
            wheel = -1;
        for (Int offset = 0, shapes = _actor._actor->getNbShapes(); shapes > 0;) {
            PxShape *shape[32];
            ...
            REP(_actor._actor->getShapes(shape, s, offset)) {
                Bool shape_on = on;
                Int io = i + offset;
                if (io == wheel.x || io == wheel.y || io == wheel.z || io == wheel.w)
                    shape_on = false; // always disable for wheels
                if (shape_on)
...
PhysMtrl *PhysXVehicle::bodyMaterial() C {
    if (_actor._actor) {
        ...
        FREP(shapes) {
            if (_vehicle)
                if (_vehicle->mWheelsSimData.getWheelShapeMapping(0) == i || _vehicle->mWheelsSimData.getWheelShapeMapping(1) == i || _vehicle->mWheelsSimData.getWheelShapeMapping(2) == i || _vehicle->mWheelsSimData.getWheelShapeMapping(3) == i)
                    continue; // skip wheels
            PxShape *shape;
            if (_actor._actor->getShapes(&shape, 1, i)) {
                PxMaterial *mtrl;
                if (shape->getMaterials(&mtrl, 1))
                    if (mtrl)
                        if (PhysMtrl *phys_mtrl = (PhysMtrl *)mtrl->userData)
                            return (phys_mtrl != &Physics.mtrl_default) ? phys_mtrl : null; // return default material always as null (we can use 'userData' because PhysMtrl's are stored in non-ref-counted cache and kept forever)
            }
        }
    }
    return null;
        ...
        if (InRange(shape_index, _actor._actor->getNbShapes())) {
            ...
            if (_actor._actor->getShapes(&shape, 1, shape_index)) {
                ...
                if (shape->getMaterials(&mtrl, 1))
                    if (mtrl)
                        if (PhysMtrl *phys_mtrl = (PhysMtrl *)mtrl->userData)
                            if (phys_mtrl != &Physics.mtrl_default)
                                return phys_mtrl; // return default material always as null (we can use 'userData' because PhysMtrl's are stored in non-ref-counted cache and kept forever)
            }
        }
    }
    return null;
        ...
        if (PxMaterial *m = material->_m) {
            ...
            for (Int offset = 0, shapes = _actor._actor->getNbShapes(); shapes > 0;) {
                ...
                REP(_actor._actor->getShapes(shape, s, offset)) {
                    Int io = i + offset;
                    if (io != wheel.x && io != wheel.y && io != wheel.z && io != wheel.w)
                        shape[i]->setMaterials(&m, 1); // skip wheels
                }
                shapes -= s;
                offset += s;
            }
...
PhysXVehicle &PhysXVehicle::wheelMaterial(PhysMtrl *material) {
    if (_vehicle && _actor._actor) {
        ...
        if (PxMaterial *m = material->_m) {
            ...
            REPA(wheel)
            if (InRange(wheel[i], shapes)) {
                PxShape *shape;
                if (_actor._actor->getShapes(&shape, 1, wheel[i]))
                    shape->setMaterials(&m, 1);
            }
        }
...
PhysXVehicle::DIFF_TYPE PhysXVehicle::diffType() C {
    if (_vehicle)
        switch (_vehicle->mDriveSimData.getDiffData().mType) {
        ...
        case PxVehicleDifferential4WData::eDIFF_TYPE_OPEN_4WD:
            return DIFF_OPEN_4WD;
        case PxVehicleDifferential4WData::eDIFF_TYPE_OPEN_FRONTWD:
            return DIFF_OPEN_FWD;
        case PxVehicleDifferential4WData::eDIFF_TYPE_OPEN_REARWD:
            return DIFF_OPEN_RWD;
        }
    return DIFF_LS_4WD;
}
PhysXVehicle &PhysXVehicle::diffType(DIFF_TYPE diff) {
    if (_vehicle) {
        PxVehicleDifferential4WData data = _vehicle->mDriveSimData.getDiffData();
        switch (diff) {
        case DIFF_LS_4WD:
            data.mType = PxVehicleDifferential4WData::eDIFF_TYPE_LS_4WD;
            break;
        case DIFF_LS_FWD:
            data.mType = PxVehicleDifferential4WData::eDIFF_TYPE_LS_FRONTWD;
            break;
        case DIFF_LS_RWD:
            data.mType = PxVehicleDifferential4WData::eDIFF_TYPE_LS_REARWD;
            break;
        case DIFF_OPEN_4WD:
            data.mType = PxVehicleDifferential4WData::eDIFF_TYPE_OPEN_4WD;
            break;
        case DIFF_OPEN_FWD:
            data.mType = PxVehicleDifferential4WData::eDIFF_TYPE_OPEN_FRONTWD;
            break;
        case DIFF_OPEN_RWD:
            data.mType = PxVehicleDifferential4WData::eDIFF_TYPE_OPEN_REARWD;
            break;
        }
...
void PhysxClass::updateVehicles() {
    REPA(vehicles) {
        ...
        PxVehicleWheelQueryResult wqr;
        wqr.wheelQueryResults = vehicle->wheel_query_result;
        wqr.nbWheelQueryResults = Elms(vehicle->wheel_query_result);
...
bool Init() {
    ...
    PhysBody body;
    body.parts.New().create(Box(2, 1, 4));
    body.setBox();                   // create vehicle body
...
bool Update() {
    ...
    if (EasyReverse) {
        if (back > forward + EPS && vehicle.speed() <= 0.5)
            vehicle.gear(-1); // set reverse gear when 'back   ' exceeds 'forward' and moving very slow
        if (forward > back + EPS && vehicle.gear() <= 0)
            vehicle.gear(1); // set     1st gear when 'forward' exceeds 'back   ' and gear is reverse or neutral
        if (vehicle.gear() < 0)
            Swap(forward, back);
    }
    ...
    if (Kb.bp(KB_F))
        vehicle.gearDown();
    if (Kb.bp(KB_Z))
        vehicle.matrix(MatrixIdentity).vel(0).angVel(0);
