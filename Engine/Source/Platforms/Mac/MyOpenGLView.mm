...
- (NSDragOperation)draggingEntered:(id<NSDraggingInfo>)sender {
    ...
    if (NSPasteboard *pasteboard = [sender draggingPasteboard])
        if ([[pasteboard types] containsObject:NSFilenamesPboardType]) {
            ...
            if (drag_mask & NSDragOperationCopy)
        ...
        if (NSPasteboard *pasteboard = [sender draggingPasteboard])
            if ([[pasteboard types] containsObject:NSFilenamesPboardType])
                if (NSArray *files = [pasteboard propertyListForType:NSFilenamesPboardType]) {
                    Memc<Str> names;
                    FREP([files count])
                    if (NSString *string = [files objectAtIndex:i])
                        names.add(string);
                    if (names.elms())
                        App.drop(names, Gui.objAtPos(Ms.pos()), Ms.pos()); // don't use 'Gui.msLit' but 'Gui.objAtPos' because this is Apple callback and 'Ms.pos' may be newer than of 'Gui.msLit'
                }
    return YES;
}
/******************************************************************************/
...
- (void)scrollWheel:(NSEvent *)theEvent {
    Ms._wheel.x -= [theEvent deltaX];
    Ms._wheel.y += [theEvent deltaY];
} // deltaY is the normal mouse wheel, deltaX is the horizontal wheel (or vertical with Shift pressed)
- (void)mouseDown:(NSEvent *)theEvent {
    Ms.push([theEvent buttonNumber]);
}
- (void)mouseUp:(NSEvent *)theEvent {
    Ms.release([theEvent buttonNumber]);
}
...
- (void)drawRect:(NSRect)rect {
    if (D.created()) {
        if (App.minimized() || (D.full() && !App.activeOrBackFull()) || !D.created())
        ...
        if (!D._resetting) {
            if ([self inLiveResize]) // resizing
            {
                if (LiveResize) {
                    ...
                    LiveResize->drawFs(FIT_FULL, FILTER_LINEAR);
                    D.alpha(alpha);
                    D.viewport(viewport);
                    goto ok;
                }
            } else if (DrawState())
                return; // 'DrawState' already calls 'D.flip'
        }
        D.clearCol();
    ok:
        D.flip(); // flip screen
...
- (void)setFrameSize:(NSSize)newSize {
    ...
    App._window_resized.set(RoundPos(rect.size.width), RoundPos(rect.size.height));
#endif
}
...
#else // can't use this because if app was started in fullscreen then it will return incorrect sizes (including the border)
    NSRect rect = [self bounds];
    VecI2 size(RoundPos(rect.size.width), RoundPos(rect.size.height));
#endif
