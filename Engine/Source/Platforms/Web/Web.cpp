...
static EM_BOOL KeyDown(int eventType, const EmscriptenKeyboardEvent *e, void *userData) {
    ...
    {
        if (key >= 'A' && key <= 'Z')
            Kb.queue(Char(key + (e->shiftKey ? 0 : 'a' - 'A')), key);
        else if (key >= '0' && key <= '9')
            Kb.queue(Char(key), key);
...
static EM_BOOL Resized(int eventType, const EmscriptenUiEvent *e, void *userData) // this gets called when window is resized or zoom is changed
{
    ...
    EmscriptenFullscreenChangeEvent full;
    emscripten_get_fullscreen_status(&full);
#if 0
   ...
   LogN(S+"body:"+e->documentBodyClientWidth+'x'+e->documentBodyClientHeight+' '+e->documentBodyClientWidth*D.browserZoom()+'x'+e->documentBodyClientHeight*D.browserZoom());
    ...
    if (!(App.flag & APP_WEB_DISABLE_AUTO_RESIZE) && !full.isFullscreen) {
        D._full = false;
        D._res.zero(); // clear values to make sure D.mode will get executed
      ...
      D.mode(Max(1, RoundPos(e->windowInnerWidth *zoom)),
        ...
        D.mode(w, h);
#endif
    } else if (width != D.resW() || height != D.resH() || full.isFullscreen != D.full()) {
            ...
            && width == D.resW() && height == D.resH()) // but the resolution is the same
        {
            ...
            D.modeSet(width, height, full.isFullscreen);
        ...
        if (!App.active())
            if (Int wait = ((App.flag & APP_WORK_IN_BACKGROUND) ? App.background_wait : -1)) {
                if (wait > 0) // finite wait
                {
                    ...
                    {
                        wait = BackgroundUpdateTime - Time.curTimeMs();
                        MAX(wait, 0); // calculate remaining time, convert to Int first before maximizing
                    } else {
                        BackgroundUpdateTime = Time.curTimeMs() + wait;
                        BackgroundUpdateTimeSet = true;
                    }
                }
                if (!wait) {
                    App.update();
                    BackgroundUpdateTime = Time.curTimeMs() + App.background_wait;
                    BackgroundUpdateTimeSet = true; // for WEB immediately recalc new time, because browsers already introduce long delays between updates when app is not active
                }
                return;
            }
