...
BUFFER(MultiMaterial0)
MultiMaterialClass MultiMaterial0;
BUFFER_END
BUFFER(MultiMaterial1)
MultiMaterialClass MultiMaterial1;
BUFFER_END
BUFFER(MultiMaterial2)
MultiMaterialClass MultiMaterial2;
BUFFER_END
BUFFER(MultiMaterial3)
MultiMaterialClass MultiMaterial3;
BUFFER_END
/******************************************************************************/
...
VecH Perp(VecH v) {
    if (Abs(v.x) < Abs(v.z))
        return VecH(0, v.z, -v.y); // Cross(v, VecH(1, 0,  0));
    else
        return VecH(-v.y, v.x, 0); // Cross(v, VecH(0, 0, -1));
}
Vec Perp(Vec v) {
    if (Abs(v.x) < Abs(v.z))
        return Vec(0, v.z, -v.y); // Cross(v, Vec(1, 0,  0));
    else
        return Vec(-v.y, v.x, 0); // Cross(v, Vec(0, 0, -1));
}
...
{
    Half r = Abs(v.x) + Abs(v.y);
    return r ? VecH2(Length2(v), (v.y >= 0) ? 1 - v.x / r : v.x / r - 1) : 0;
}
...
Half DistPointEdge(VecH2 pos, VecH2 edge_a, VecH2 edge_b) // safe in case 'edge' is zero length because 'd' would be zero, and 'DistPointPlane' would be zero
{
    ...
    if (DistPointPlane(pos, edge_a, d) <= 0)
        return Dist(pos, edge_a);
    if (DistPointPlane(pos, edge_b, d) >= 0)
        return Dist(pos, edge_b);
...
Half Dist2PointEdge(VecH2 pos, VecH2 edge_a, VecH2 edge_b) // safe in case 'edge' is zero length because 'd' would be zero, and 'DistPointPlane' would be zero
{
    VecH2 d = edge_b - edge_a;
    if (DistPointPlane(pos, edge_a, d) <= 0)
        return Dist2(pos, edge_a);
    if (DistPointPlane(pos, edge_b, d) >= 0)
        return Dist2(pos, edge_b);
...
Vec ViewMatrixY(UInt mtrx) {
    mtrx *= 3;
    return Vec(ViewMatrix[mtrx].y, ViewMatrix[mtrx + 1].y, ViewMatrix[mtrx + 2].y);
}
Vec ViewMatrixPos(UInt mtrx) {
    mtrx *= 3;
    return Vec(ViewMatrix[mtrx].w, ViewMatrix[mtrx + 1].w, ViewMatrix[mtrx + 2].w);
}

Vec ViewMatrixPrevY(UInt mtrx) {
    mtrx *= 3;
    return Vec(ViewMatrixPrev[mtrx].y, ViewMatrixPrev[mtrx + 1].y, ViewMatrixPrev[mtrx + 2].y);
}
Vec ViewMatrixPrevPos(UInt mtrx) {
    mtrx *= 3;
    return Vec(ViewMatrixPrev[mtrx].w, ViewMatrixPrev[mtrx + 1].w, ViewMatrixPrev[mtrx + 2].w);
}
...
#include "!Set Prec Default.h"
    ...
    Vec2 uv(Int heightmap = HEIGHTMAP_NO) { return (heightmap == HEIGHTMAP_FLAT) ? Vec2(_pos.x, -_pos.z) : (heightmap == HEIGHTMAP_SPHERE) ? VtxUVScale * AtanFast(Vec2(_pos.x, -_pos.z) / _pos.y)
                                                                                                                                           : _uv; } // tex coords 0
...
/******************************************************************************/
Half DistPointPlaneRay(VecH2 p, VecH2 plane_normal, VecH2 ray) {
    Half rd = Dot(ray, plane_normal);
    return rd ? Dot(p, plane_normal) / rd : 0;
}
Flt DistPointPlaneRay(Vec2 p, Vec2 plane_normal, Vec2 ray) {
    Flt rd = Dot(ray, plane_normal);
    return rd ? Dot(p, plane_normal) / rd : 0;
}
Half DistPointPlaneRay(VecH p, VecH plane_normal, VecH ray) {
    Half rd = Dot(ray, plane_normal);
    return rd ? Dot(p, plane_normal) / rd : 0;
}
Flt DistPointPlaneRay(Vec p, Vec plane_normal, Vec ray) {
    Flt rd = Dot(ray, plane_normal);
    return rd ? Dot(p, plane_normal) / rd : 0;
}
Half DistPointPlaneRay(VecH2 p, VecH2 plane_pos, VecH2 plane_normal, VecH2 ray) {
    Half rd = Dot(ray, plane_normal);
    return rd ? DistPointPlane(p, plane_pos, plane_normal) / rd : 0;
}
Flt DistPointPlaneRay(Vec2 p, Vec2 plane_pos, Vec2 plane_normal, Vec2 ray) {
    Flt rd = Dot(ray, plane_normal);
    return rd ? DistPointPlane(p, plane_pos, plane_normal) / rd : 0;
}
Half DistPointPlaneRay(VecH p, VecH plane_pos, VecH plane_normal, VecH ray) {
    Half rd = Dot(ray, plane_normal);
    return rd ? DistPointPlane(p, plane_pos, plane_normal) / rd : 0;
}
Flt DistPointPlaneRay(Vec p, Vec plane_pos, Vec plane_normal, Vec ray) {
    Flt rd = Dot(ray, plane_normal);
    return rd ? DistPointPlane(p, plane_pos, plane_normal) / rd : 0;
}
...
Half BlendSmoothCube(Half x) {
    x = Sat(Abs(x));
    return 1 - SmoothCube(x);
}
Flt BlendSmoothCube(Flt x) {
    x = Sat(Abs(x));
    return 1 - SmoothCube(x);
}

Half BlendSmoothSin(Half x) {
    x = Sat(Abs(x));
...
Vec RgbToHsb(Vec rgb) {
    ...
    if (d <= 0)
        ...
        if (rgb.y >= rgb.z)
            ...
            hsb.x = (rgb.y - rgb.z) / d + 6;
    } else if (rgb.y >= max)
        hsb.x = (rgb.z - rgb.x) / d + 2;
    else
        hsb.x = (rgb.x - rgb.y) / d + 4;
...
Vec BlendColor(Vec base, Vec base_a, Vec color, Vec color_a) {
    ...
    if (sum.r) {
        base_a.r /= sum.r;
        color_a.r = 1 - base_a.r;
        ret.r = base.r * base_a.r + color.r * color_a.r;
    } else
        ret.r = 0;
    if (sum.g) {
        base_a.g /= sum.g;
        color_a.g = 1 - base_a.g;
        ret.g = base.g * base_a.g + color.g * color_a.g;
    } else
        ret.g = 0;
    if (sum.b) {
        base_a.b /= sum.b;
        color_a.b = 1 - base_a.b;
        ret.b = base.b * base_a.b + color.b * color_a.b;
    } else
        ret.b = 0;
    return ret;
}
Vec MergeBlendColor(Vec base, Vec base_a, Vec color, Vec color_a) {
    ...
    Vec ret;
    if (sum.r)
        ret.r = (base.r * base_a.r + color.r) / sum.r;
    else
        ret.r = 0;
    if (sum.g)
        ret.g = (base.g * base_a.g + color.g) / sum.g;
    else
        ret.g = 0;
    if (sum.b)
        ret.b = (base.b * base_a.b + color.b) / sum.b;
    else
        ret.b = 0;
    return ret;
#endif
...
Flt GetLod(Vec2 uv, Flt tex_size) {
    Vec2 pix = uv * tex_size;
    return 0.5 * Log2(Max(Length2(ddx(pix)), Length2(ddy(pix))));
}
Flt GetLod(Vec2 uv, Vec2 tex_size) {
    Vec2 pix = uv * tex_size;
    return 0.5 * Log2(Max(Length2(ddx(pix)), Length2(ddy(pix))));
}
/******************************************************************************/
...
void BendGrass(Vec local_pos, inout Vec view_pos, UInt mtrx = 0, Bool prev = false) {
    ...
    if (local_pos.y > 0) {
        ...
        Flt l2 = Length2(delta);
        delta *= Sqrt(len2 / l2); // delta.setLength2(len2);
...
void BendLeaf(Vec center, inout Vec pos, Bool prev = false) {
    ...
    CosSin(cos_sin.x, cos_sin.y, bend.x);
    delta.xy = Rotate(delta.xy, cos_sin);
    CosSin(cos_sin.x, cos_sin.y, bend.y);
    delta.zy = Rotate(delta.zy, cos_sin);
    pos = center + delta;
}
void BendLeaf(Vec center, inout Vec pos, inout VecH nrm, Bool prev = false) {
    ...
    CosSin(cos_sin.x, cos_sin.y, bend.x);
    delta.xy = Rotate(delta.xy, cos_sin);
    nrm.xy = Rotate(nrm.xy, cos_sin);
    CosSin(cos_sin.x, cos_sin.y, bend.y);
    delta.zy = Rotate(delta.zy, cos_sin);
    nrm.zy = Rotate(nrm.zy, cos_sin);
    pos = center + delta;
}
void BendLeaf(Vec center, inout Vec pos, inout VecH nrm, inout VecH tan, Bool prev = false) {
    ...
    delta.xy = Rotate(delta.xy, cos_sin);
    nrm.xy = Rotate(nrm.xy, cos_sin);
    tan.xy = Rotate(tan.xy, cos_sin);
    CosSin(cos_sin.x, cos_sin.y, bend.y);
    delta.zy = Rotate(delta.zy, cos_sin);
    nrm.zy = Rotate(nrm.zy, cos_sin);
    tan.zy = Rotate(tan.zy, cos_sin);
    pos = center + delta;
}
/******************************************************************************/
// Here can't use Half's because precision was not good enough
...
VecH4 GetDetail0(Vec2 tex) {
    VecH4 det = RTex(Det, tex * MultiMaterial0.det_uv_scale);
    det.xyz = det.xyz * MultiMaterial0.det_mul + MultiMaterial0.det_add;
    det.DETAIL_CHANNEL_COLOR = det.DETAIL_CHANNEL_COLOR * MultiMaterial0.det_mul + MultiMaterial0.det_inv;
    return det;
}
VecH4 GetDetail1(Vec2 tex) {
    VecH4 det = RTex(Det1, tex * MultiMaterial1.det_uv_scale);
    det.xyz = det.xyz * MultiMaterial1.det_mul + MultiMaterial1.det_add;
    det.DETAIL_CHANNEL_COLOR = det.DETAIL_CHANNEL_COLOR * MultiMaterial1.det_mul + MultiMaterial1.det_inv;
    return det;
}
VecH4 GetDetail2(Vec2 tex) {
    VecH4 det = RTex(Det2, tex * MultiMaterial2.det_uv_scale);
    det.xyz = det.xyz * MultiMaterial2.det_mul + MultiMaterial2.det_add;
    det.DETAIL_CHANNEL_COLOR = det.DETAIL_CHANNEL_COLOR * MultiMaterial2.det_mul + MultiMaterial2.det_inv;
    return det;
}
VecH4 GetDetail3(Vec2 tex) {
    VecH4 det = RTex(Det3, tex * MultiMaterial3.det_uv_scale);
    det.xyz = det.xyz * MultiMaterial3.det_mul + MultiMaterial3.det_add;
    det.DETAIL_CHANNEL_COLOR = det.DETAIL_CHANNEL_COLOR * MultiMaterial3.det_mul + MultiMaterial3.det_inv;
    return det;
}
/******************************************************************************/
// FACE NORMAL HANDLING
/******************************************************************************/
void BackFlip(inout VecH dir, Bool front) {
    if (!front)
        dir = -dir;
}
/******************************************************************************/
Half MultiMaterialWeight(Half weight, Half alpha) // 'weight'=weight of this material, 'alpha'=color texture alpha (opacity or bump)
{
   ...
   return weight;
    ...
    if (alpha < -HALF_MIN)
        weight = SigmoidSqrInv(weight * SigmoidSqr(alpha)) / alpha;
    else if (alpha > HALF_MIN)
        weight = SigmoidSqr(weight * alpha) / SigmoidSqr(alpha);
    return weight;
...
BUFFER_END
BUFFER(LightPoint)
GpuLightPoint LightPoint;
BUFFER_END
BUFFER(LightLinear)
GpuLightLinear LightLinear;
BUFFER_END
BUFFER(LightCone)
GpuLightCone LightCone;
BUFFER_END
#include "!Set Prec Default.h"
/******************************************************************************/
...
Half LightConeAngle(Vec2 pos) {
    Half v = Sat(Length(pos) * LightCone.falloff.x + LightCone.falloff.y);
    return v;
} // alternative is Sqr(v)
/******************************************************************************/
...
#define DEFAULT_LIGHT_RADIUS_FRAC 0.0036
    VecH specular(Half rough, Half reflectivity, VecH reflect_col, Bool quality, Half light_radius_frac = DEFAULT_LIGHT_RADIUS_FRAC) { // currently specular can be generated even for smooth=0 and reflectivity=0 #SpecularReflectionFromZeroSmoothReflectivity
#if 0
...
void ProcessGlow(Half glow, VecH base_col, inout VecH col) {
    if (GLOW_OCCLUSION)
        col += base_col * (glow * 2); // boost glow by 2 because here we don't maximize 'base_col', so average 'base_col' 0..1 is 0.5, so *2 makes it 1.0
    else {
        Half max = Max(Max(base_col), 1.0 / 32);
        col += base_col * (glow / max);
    } // use a decent max value (1/32 was the smallest value that prevented artifacts) to have 2 effects: prevent division by zero, and another very important is for colors close to black (0,0,0) like (1/255, 2/255, 0/255) where color can't be extracted precisely due to low precision and texture compression artifacts, this will make those colors remain very dark and don't affect glow much, thus preventing from showing artifacts
}
...
Vec ShadowDirTransform(Vec pos, Int num) { // using "Int/UInt matrix_index" and "matrix_index=.."  and "ShdMatrix4[matrix_index]" was slower
    ...
    if (num <= 4) {
        BRANCH if (num <= 2 || pos.z <= ShdStep[1]) {
            BRANCH if (num <= 1 || pos.z <= ShdStep[0]) p = Transform(pos, ShdMatrix4[0]);
            else p = Transform(pos, ShdMatrix4[1]);
        }
        else {
            BRANCH if (num <= 3 || pos.z <= ShdStep[2]) p = Transform(pos, ShdMatrix4[2]);
            else p = Transform(pos, ShdMatrix4[3]);
        }
    } else {
        BRANCH if (num <= 3 || pos.z <= ShdStep[2]) {
            BRANCH if (num <= 1 || pos.z <= ShdStep[0]) p = Transform(pos, ShdMatrix4[0]);
            else BRANCH if (num <= 2 || pos.z <= ShdStep[1]) p = Transform(pos, ShdMatrix4[1]);
            else p = Transform(pos, ShdMatrix4[2]);
        }
        else {
            BRANCH if (num <= 4 || pos.z <= ShdStep[3]) p = Transform(pos, ShdMatrix4[3]);
            else BRANCH if (num <= 5 || pos.z <= ShdStep[4]) p = Transform(pos, ShdMatrix4[4]);
            else p = Transform(pos, ShdMatrix4[5]);
        }
    }
    return p.xyz / p.w;
}
Vec ShadowPointTransform(Vec pos) {
    ...
    BRANCH if (-local.y >= a) p = Transform(pos, ShdMatrix4[3]);
    else BRANCH if (local.x >= a) p = Transform(pos, ShdMatrix4[0]);
    else BRANCH if (-local.x >= a) p = Transform(pos, ShdMatrix4[1]);
    else BRANCH if (local.z >= a) p = Transform(pos, ShdMatrix4[4]);
    else BRANCH if (-local.z >= a) p = Transform(pos, ShdMatrix4[5]);
    else p = Transform(pos, ShdMatrix4[2]);
    return p.xyz / p.w;
}
/******************************************************************************/
...
Half ShadowDirValue(Vec pos, Vec2 jitter_value, Bool jitter, Int num, Bool cloud) {
    ...
    if (cloud)
        return fade + CompareDepth(p, jitter_value, jitter) * CompareDepth2(p);
    else
        return fade + CompareDepth(p, jitter_value, jitter);
}
...
Half ShadowConeValue(Vec pos, Vec2 jitter_value, Bool jitter) {
    Vec4 p = Transform(pos, ShdMatrix4[0]);
    p.xyz /= p.w;
...
#if NORMAL_CUBIC_INTERPOLATE
    Flt V12 = 2 * Dot(B030 - B003, N002 + N020) / Dot(B030 - B003, B030 - B003);
    O.N110 = Normalize(N002 + N020 - V12 * (B030 - B003));
    Flt V23 = 2 * Dot(B300 - B030, N020 + N200) / Dot(B300 - B030, B300 - B030);
    O.N011 = Normalize(N020 + N200 - V23 * (B300 - B030));
    Flt V31 = 2 * Dot(B003 - B300, N200 + N002) / Dot(B003 - B300, B003 - B300);
    O.N101 = Normalize(N200 + N002 - V31 * (B003 - B300));
#endif
...
#if NORMAL_CUBIC_INTERPOLATE
    nrm = Normalize(nrm0 * WW + nrm1 * UU + nrm2 * VV + hs_data.N110 * (W * U) + hs_data.N011 * (U * V) + hs_data.N101 * (W * V));
#else // linear interpolation
    nrm = Normalize(nrm0 * W + nrm1 * U + nrm2 * V);
#endif

    BRANCH if (hs_data.InsideTessFactor > 1) {
        pos = pos0 * (WW * W) + pos1 * (UU * U) + pos2 * (VV * V) + hs_data.B210 * (WW * U) + hs_data.B120 * (W * UU) + hs_data.B201 * (WW * V) + hs_data.B021 * (UU * V) + hs_data.B102 * (W * VV) + hs_data.B012 * (U * VV) + hs_data.B111 * (W * U * V);
        if (clamp_tess)
            pos = Lerp(pos0 * W + pos1 * U + pos2 * V, pos, clamp_tess_factor);
    }
    else {
