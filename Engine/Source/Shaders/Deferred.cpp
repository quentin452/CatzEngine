...
void VS(
    ...
    if (BUMP_MODE >= SBUMP_FLAT)
        nrm = vtx.nrm();
    if (BUMP_MODE > SBUMP_FLAT)
        tan = vtx.tan(nrm, HEIGHTMAP);

    // VEL
    Vec local_pos_prev, view_pos_prev;
    VecH nrm_prev;
    if (USE_VEL) {
        local_pos_prev = local_pos;
        if (TESSELATE_VEL)
            nrm_prev = nrm;
    }
    ...
    if (UV_SCALE && MATERIALS == 1)
        O.uv *= Material.uv_scale;
#endif
...
#if ALPHA_TEST == ALPHA_TEST_DITHER
    if (MATERIALS <= 1)
        O.col = vtx.colorFast() * Material.color;
    else
        O.col = vtx.colorFast();
#else
    if (MATERIALS <= 1)
        O.col = vtx.colorFast3() * Material.color.rgb;
    else
        O.col = vtx.colorFast3();
#endif
#endif

    if (FX == FX_LEAF_2D || FX == FX_LEAF_3D) {
        if (BUMP_MODE > SBUMP_FLAT)
            BendLeaf(vtx.hlp(), local_pos, nrm, tan);
        else if (BUMP_MODE == SBUMP_FLAT)
            BendLeaf(vtx.hlp(), local_pos, nrm);
        else
            BendLeaf(vtx.hlp(), local_pos);
        if (USE_VEL) {
            if (TESSELATE_VEL)
                BendLeaf(vtx.hlp(), local_pos_prev, nrm_prev, true);
            else
    ...
    if (FX == FX_LEAFS_2D || FX == FX_LEAFS_3D) {
        if (BUMP_MODE > SBUMP_FLAT)
            BendLeafs(vtx.hlp(), vtx.size(), local_pos, nrm, tan);
        else if (BUMP_MODE == SBUMP_FLAT)
            BendLeafs(vtx.hlp(), vtx.size(), local_pos, nrm);
        else
            BendLeafs(vtx.hlp(), vtx.size(), local_pos);
        if (USE_VEL) {
            if (TESSELATE_VEL)
                BendLeafs(vtx.hlp(), vtx.size(), local_pos_prev, nrm_prev, true);
            else
    ...
    if (!SKIN) {
        if (true) // instance
        {
            ...
            if (USE_VEL)
                view_pos_prev = TransformPosPrev(local_pos_prev, vtx.instance());
            ...
            if (FX == FX_GRASS_2D || FX == FX_GRASS_3D) {
                ...
                if (USE_VEL)
                    BendGrass(local_pos_prev, view_pos_prev, vtx.instance(), true);
            }
#if GRASS_FADE
            ...
            if (USE_VEL)
                view_pos_prev = TransformPosPrev(local_pos_prev);
            ...
            if (FX == FX_GRASS_2D || FX == FX_GRASS_3D) {
                ...
                if (USE_VEL)
                    BendGrass(local_pos_prev, view_pos_prev, 0, true);
            }
#if GRASS_FADE
        ...
        if (USE_VEL)
            view_pos_prev = TransformPosPrev(local_pos_prev, bone, vtx.weight());
...
// apply uv coord bump offset
...
#elif BUMP_MODE == SBUMP_RELIEF // Relief
    {
        ...
        Vec2 TexSize;
        BUMP_IMAGE.GetDimensions(TexSize.x, TexSize.y);
        ...
        if (RELIEF_STEPS_MUL != 1)
            if (lod > 0)
                length *= RELIEF_STEPS_MUL; // don't use this for first LOD
        ...
        LOOP for (Int i = 0;; i++) {
            ray -= stp;
            I.uv += uv_step;
            if (i >= steps || RTexLodI(BUMP_IMAGE, I.uv, lod).BASE_CHANNEL_BUMP >= ray)
                break;
        }
...
// #MaterialTextureLayout
#if LAYOUT == 0
    {
        ...
        glow = Material.glow;
        if (DETAIL) {
            col.rgb *= det.DETAIL_CHANNEL_COLOR;
            APPLY_DETAIL_ROUGH(rough, det.DETAIL_CHANNEL_ROUGH);
...
#elif LAYOUT == 1
    {
        ...
        if (ALPHA_TEST) {
            ...
            col.a *= tex_col.a;
            MaterialAlphaTestDither(col.a, pixel.xy, I.face_id);
#endif
        }
        col.rgb *= tex_col.rgb;
        rough = Material.rough_add;
    ...
    if (DETAIL)
        nrm.xy += det.DETAIL_CHANNEL_NORMAL; // #MaterialTextureLayoutDetail
#endif
    ...
    if (MATERIALS == 2)
        I.material.xy = Sat(I.material.xy);
    if (MATERIALS == 3)
        I.material.xyz = Sat(I.material.xyz);
    if (MATERIALS == 4)
        I.material.xyzw = Sat(I.material.xyzw);
    ...
    if (UV_SCALE) {
        ...
        if (MATERIALS >= 3)
            uv2 = I.uv * MultiMaterial2.uv_scale;
        if (MATERIALS >= 4)
            uv3 = I.uv * MultiMaterial3.uv_scale;
    } else {
        uv0 = I.uv;
        uv1 = I.uv;
        if (MATERIALS >= 3)
...
// apply uv coord bump offset
#if BUMP_MODE >= SBUMP_PARALLAX_MIN && BUMP_MODE <= SBUMP_PARALLAX_MAX // Parallax
    {
        ...
        VecH4 bump_mul;
        bump_mul.x = MultiMaterial0.bump;
        VecH4 bump_add;
        bump_mul.y = MultiMaterial1.bump;
        if (MATERIALS == 2) {
            bump_mul.xy *= I.material.xy;
            bump_add.xy = bump_mul.xy * -0.5;
        }
        if (MATERIALS >= 3)
            bump_mul.z = MultiMaterial2.bump;
        if (MATERIALS == 3) {
            bump_mul.xyz *= I.material.xyz;
            bump_add.xyz = bump_mul.xyz * -0.5;
        }
        if (MATERIALS >= 4)
            bump_mul.w = MultiMaterial3.bump;
        if (MATERIALS == 4) {
            bump_mul.xyzw *= I.material.xyzw;
            bump_add.xyzw = bump_mul.xyzw * -0.5;
        }
        ...
        UNROLL for (Int i = 0; i < steps; i++) // I.uv+=h*tpos.xy;
        {
            ...
            if (MATERIALS >= 3)
                h += RTex(CONCAT(BUMP_IMAGE, 2), uv2).BASE_CHANNEL_BUMP * bump_mul.z + bump_add.z;
            if (MATERIALS >= 4)
                h += RTex(CONCAT(BUMP_IMAGE, 3), uv3).BASE_CHANNEL_BUMP * bump_mul.w + bump_add.w;
...
#elif BUMP_MODE == SBUMP_RELIEF // Relief
    {
        ...
        if (MATERIALS == 2) {
            bump_mul.xy *= I.material.xy;
            avg_bump = Sum(bump_mul.xy);
        } // use 'Sum' because they're premultipled by 'I.material'
        if (MATERIALS >= 3)
            bump_mul.z = MultiMaterial2.bump;
        if (MATERIALS == 3) {
            bump_mul.xyz *= I.material.xyz;
            avg_bump = Sum(bump_mul.xyz);
        }
        if (MATERIALS >= 4)
            bump_mul.w = MultiMaterial3.bump;
        if (MATERIALS == 4) {
            bump_mul.xyzw *= I.material.xyzw;
            avg_bump = Sum(bump_mul.xyzw);
        }
        ...
        Flt lod0;
        {
            BUMP_IMAGE.GetDimensions(TexSize0.x, TexSize0.y);
            lod0 = Max(0, GetLod(uv0, TexSize0) + RELIEF_LOD_OFFSET);
        } // yes, can be negative, so use Max(0) to avoid increasing number of steps when surface is close to camera
        Vec2 TexSize1;
        Flt lod1;
        {
            CONCAT(BUMP_IMAGE, 1).GetDimensions(TexSize1.x, TexSize1.y);
            lod1 = Max(0, GetLod(uv1, TexSize1) + RELIEF_LOD_OFFSET);
        }
        Vec2 TexSize2;
        Flt lod2;
        if (MATERIALS >= 3) {
            CONCAT(BUMP_IMAGE, 2).GetDimensions(TexSize2.x, TexSize2.y);
            lod2 = Max(0, GetLod(uv2, TexSize2) + RELIEF_LOD_OFFSET);
        }
        Vec2 TexSize3;
        Flt lod3;
        if (MATERIALS >= 4) {
            CONCAT(BUMP_IMAGE, 3).GetDimensions(TexSize3.x, TexSize3.y);
            lod3 = Max(0, GetLod(uv3, TexSize3) + RELIEF_LOD_OFFSET);
        }

        Half length;
        {
            Half length0 = Pixels(tpos.xy, TexSize0, lod0);
            if (RELIEF_STEPS_MUL != 1)
                if (lod0 > 0)
                    length0 *= RELIEF_STEPS_MUL;
            length = length0 * I.material.x;
        } // don't use RELIEF_STEPS_MUL for first LOD
        {
            Half length1 = Pixels(tpos.xy, TexSize1, lod1);
            if (RELIEF_STEPS_MUL != 1)
                if (lod1 > 0)
                    length1 *= RELIEF_STEPS_MUL;
            length += length1 * I.material.y;
        } // don't use RELIEF_STEPS_MUL for first LOD
        if (MATERIALS >= 3) {
            Half length2 = Pixels(tpos.xy, TexSize2, lod2);
            if (RELIEF_STEPS_MUL != 1)
                if (lod2 > 0)
                    length2 *= RELIEF_STEPS_MUL;
            length += length2 * I.material.z;
        } // don't use RELIEF_STEPS_MUL for first LOD
        if (MATERIALS >= 4) {
            Half length3 = Pixels(tpos.xy, TexSize3, lod3);
            if (RELIEF_STEPS_MUL != 1)
                if (lod3 > 0)
                    length3 *= RELIEF_STEPS_MUL;
            length += length3 * I.material.w;
        } // don't use RELIEF_STEPS_MUL for first LOD
        ...
        LOOP for (;;) {
            ...
            if (MATERIALS >= 3)
                height_next += RTexLodI(CONCAT(BUMP_IMAGE, 2), uv2, lod2).BASE_CHANNEL_BUMP * I.material.z;
            if (MATERIALS >= 4)
                height_next += RTexLodI(CONCAT(BUMP_IMAGE, 3), uv3, lod3).BASE_CHANNEL_BUMP * I.material.w;
        ...
        if (1) {
            ...
            Bool extra = (lod0 <= 0 || lod1 <= 0);
            if (MATERIALS >= 3 && lod2 <= 0)
                extra = true;
            if (MATERIALS >= 4 && lod3 <= 0)
                extra = true; // extra step (needed only for closeup)
            BRANCH if (extra) {
                Half ray_cur = ray + stp * frac,
                     // height_cur =RTexLodI(       BUMP_IMAGE  , I.uv , lod ).BASE_CHANNEL_BUMP;
                ...
                if (MATERIALS >= 3)
                    height_cur += RTexLodI(CONCAT(BUMP_IMAGE, 2), uv2, lod2).BASE_CHANNEL_BUMP * I.material.z;
                if (MATERIALS >= 4)
                    height_cur += RTexLodI(CONCAT(BUMP_IMAGE, 3), uv3, lod3).BASE_CHANNEL_BUMP * I.material.w;
    ...
    if (LAYOUT == 2) {
        ...
        if (MTRL_BLEND) {
            VecH4 mtrl;
            mtrl.x = MultiMaterialWeight(I.material.x, ext0.BASE_CHANNEL_BUMP);
            mtrl.y = MultiMaterialWeight(I.material.y, ext1.BASE_CHANNEL_BUMP);
            if (MATERIALS == 2) {
                Half sum = Sum(mtrl.xy);
                if (CanDiv(sum))
                    I.material.xy = mtrl.xy / sum;
            }
            if (MATERIALS >= 3) {
                mtrl.z = MultiMaterialWeight(I.material.z, ext2.BASE_CHANNEL_BUMP);
                if (MATERIALS == 3) {
                    Half sum = Sum(mtrl.xyz);
                    if (CanDiv(sum))
                        I.material.xyz = mtrl.xyz / sum;
                }
            }
            if (MATERIALS >= 4) {
                mtrl.w = MultiMaterialWeight(I.material.w, ext3.BASE_CHANNEL_BUMP);
                if (MATERIALS == 4) {
                    Half sum = Sum(mtrl.xyzw);
                    if (CanDiv(sum))
                        I.material.xyzw = mtrl.xyzw / sum;
                }
            }
        }
        {
            VecH refl_rogh_glow0 = ext0.xyw * MultiMaterial0.refl_rogh_glow_mul + MultiMaterial0.refl_rogh_glow_add;
            if (DETAIL)
                APPLY_DETAIL_ROUGH(refl_rogh_glow0.y, det0.DETAIL_CHANNEL_ROUGH);
            refl_rogh_glow = refl_rogh_glow0 * I.material.x;
        } // #MaterialTextureLayoutDetail
        {
            VecH refl_rogh_glow1 = ext1.xyw * MultiMaterial1.refl_rogh_glow_mul + MultiMaterial1.refl_rogh_glow_add;
            if (DETAIL)
                APPLY_DETAIL_ROUGH(refl_rogh_glow1.y, det1.DETAIL_CHANNEL_ROUGH);
            refl_rogh_glow += refl_rogh_glow1 * I.material.y;
        }
        if (MATERIALS >= 3) {
            VecH refl_rogh_glow2 = ext2.xyw * MultiMaterial2.refl_rogh_glow_mul + MultiMaterial2.refl_rogh_glow_add;
            if (DETAIL)
                APPLY_DETAIL_ROUGH(refl_rogh_glow2.y, det2.DETAIL_CHANNEL_ROUGH);
            refl_rogh_glow += refl_rogh_glow2 * I.material.z;
        }
        if (MATERIALS >= 4) {
            VecH refl_rogh_glow3 = ext3.xyw * MultiMaterial3.refl_rogh_glow_mul + MultiMaterial3.refl_rogh_glow_add;
            if (DETAIL)
                APPLY_DETAIL_ROUGH(refl_rogh_glow3.y, det3.DETAIL_CHANNEL_ROUGH);
            refl_rogh_glow += refl_rogh_glow3 * I.material.w;
        }
    } else {
        {
            VecH refl_rogh_glow0 = MultiMaterial0.refl_rogh_glow_add;
            if (DETAIL)
                APPLY_DETAIL_ROUGH(refl_rogh_glow0.y, det0.DETAIL_CHANNEL_ROUGH);
            refl_rogh_glow = refl_rogh_glow0 * I.material.x;
        } // #MaterialTextureLayoutDetail
        {
            VecH refl_rogh_glow1 = MultiMaterial1.refl_rogh_glow_add;
            if (DETAIL)
                APPLY_DETAIL_ROUGH(refl_rogh_glow1.y, det1.DETAIL_CHANNEL_ROUGH);
            refl_rogh_glow += refl_rogh_glow1 * I.material.y;
        }
        if (MATERIALS >= 3) {
            VecH refl_rogh_glow2 = MultiMaterial2.refl_rogh_glow_add;
            if (DETAIL)
                APPLY_DETAIL_ROUGH(refl_rogh_glow2.y, det2.DETAIL_CHANNEL_ROUGH);
            refl_rogh_glow += refl_rogh_glow2 * I.material.z;
        }
        if (MATERIALS >= 4) {
            VecH refl_rogh_glow3 = MultiMaterial3.refl_rogh_glow_add;
            if (DETAIL)
                APPLY_DETAIL_ROUGH(refl_rogh_glow3.y, det3.DETAIL_CHANNEL_ROUGH);
            refl_rogh_glow += refl_rogh_glow3 * I.material.w;
        }
    ...
    VecH rgb;
    {
        VecH col0 = RTex(Col, uv0).rgb;
        col0.rgb *= MultiMaterial0.color.rgb;
        if (DETAIL)
            col0.rgb *= det0.DETAIL_CHANNEL_COLOR;
        if (MACRO)
            col0.rgb = Lerp(col0.rgb, RTex(Mac, uv0 * MacroScale).rgb, MultiMaterial0.macro * mac_blend);
        rgb = I.material.x * col0;
    } // #MaterialTextureLayoutDetail
    {
        VecH col1 = RTex(Col1, uv1).rgb;
        col1.rgb *= MultiMaterial1.color.rgb;
        if (DETAIL)
            col1.rgb *= det1.DETAIL_CHANNEL_COLOR;
        if (MACRO)
            col1.rgb = Lerp(col1.rgb, RTex(Mac1, uv1 * MacroScale).rgb, MultiMaterial1.macro * mac_blend);
        rgb += I.material.y * col1;
    }
    if (MATERIALS >= 3) {
        VecH col2 = RTex(Col2, uv2).rgb;
        col2.rgb *= MultiMaterial2.color.rgb;
        if (DETAIL)
            col2.rgb *= det2.DETAIL_CHANNEL_COLOR;
        if (MACRO)
            col2.rgb = Lerp(col2.rgb, RTex(Mac2, uv2 * MacroScale).rgb, MultiMaterial2.macro * mac_blend);
        rgb += I.material.z * col2;
    }
    if (MATERIALS >= 4) {
        VecH col3 = RTex(Col3, uv3).rgb;
        col3.rgb *= MultiMaterial3.color.rgb;
        if (DETAIL)
            col3.rgb *= det3.DETAIL_CHANNEL_COLOR;
        if (MACRO)
            col3.rgb = Lerp(col3.rgb, RTex(Mac3, uv3 * MacroScale).rgb, MultiMaterial3.macro * mac_blend);
        rgb += I.material.w * col3;
    }
#if COLORS
    col.rgb *= rgb.rgb;
#else
    ...
    nrm = I.Nrm(); // can't add DETAIL normal because it would need 'I.mtrx'
#else
    if (DETAIL) { // #MaterialTextureLayoutDetail
        ...
        if (MATERIALS >= 3)
            nrm.xy += (RTex(Nrm2, uv2).BASE_CHANNEL_NORMAL * MultiMaterial2.normal + det2.DETAIL_CHANNEL_NORMAL) * I.material.z;
        if (MATERIALS >= 4)
            nrm.xy += (RTex(Nrm3, uv3).BASE_CHANNEL_NORMAL * MultiMaterial3.normal + det3.DETAIL_CHANNEL_NORMAL) * I.material.w;
    } else {
        ...
        if (MATERIALS >= 3)
            nrm.xy += RTex(Nrm2, uv2).BASE_CHANNEL_NORMAL * (MultiMaterial2.normal * I.material.z);
        if (MATERIALS >= 4)
            nrm.xy += RTex(Nrm3, uv3).BASE_CHANNEL_NORMAL * (MultiMaterial3.normal * I.material.w);
    }
    nrm.z = CalcZ(nrm.xy);
    nrm = Transform(nrm, I.mtrx);
#endif
    ...
    output.glow(glow);
    output.normal(nrm);
    output.mode((FX == FX_GRASS_3D || FX == FX_LEAF_3D || FX == FX_LEAFS_3D) ? PSM_TRANSLUCENT : (FX == FX_CLEAR_COAT) ? PSM_CLEAR_COAT
                                                                                                                       : PSM_NONE);
    output.rough(rough);
...
[maxtessfactor(5.0)]
    ...
    O.projected_prev_pos_xyw = I[0].projected_prev_pos_xyw * B.z + I[1].projected_prev_pos_xyw * B.x + I[2].projected_prev_pos_xyw * B.y + pixel.xyw - ProjectXYW(interpolated_pos); // + delta (tesselated pos - interpolated pos)
#endif
#endif
}
#endif
