...
VecH4 Draw2DCol_PS(NOPERSP VecH4 col
                   : COLOR) : TARGET { return col; }
/******************************************************************************/
...
VecH4 Draw2DTex_PS(NOPERSP Vec2 uv
                   : UV) : TARGET { return Tex(Img, uv); }
VecH4 Draw2DTexC_PS(NOPERSP Vec2 uv
                    : UV) : TARGET { return Tex(Img, uv) * Color[0] + Color[1]; }
...
VecH4 Draw2DTexA_PS(NOPERSP Vec2 uv
                    : UV) : TARGET { return VecH4(Tex(Img, uv).rgb, Step); }

#if defined IN0_GAMMA && defined IN1_GAMMA && defined OUT_GAMMA
VecH4 DrawFade_PS(NOPERSP Vec2 uv
                  : UV) : TARGET {
...
VecH4 DrawTexX_PS(NOPERSP Vec2 uv
                  : UV) : TARGET { return VecH4(Tex(Img, uv).xxx, 1); }
VecH4 DrawTexY_PS(NOPERSP Vec2 uv
                  : UV) : TARGET { return VecH4(Tex(Img, uv).yyy, 1); }
VecH4 DrawTexZ_PS(NOPERSP Vec2 uv
                  : UV) : TARGET { return VecH4(Tex(Img, uv).zzz, 1); }
VecH4 DrawTexW_PS(NOPERSP Vec2 uv
                  : UV) : TARGET { return VecH4(Tex(Img, uv).www, 1); }
...
VecH4 DrawTexG_PS(NOPERSP Vec2 uv
                  : UV) : TARGET { return SRGBToLinear(Tex(Img, uv)); }
VecH4 DrawTexCG_PS(NOPERSP Vec2 uv
                   : UV) : TARGET { return SRGBToLinear(Tex(Img, uv) * Color[0] + Color[1]); }

VecH4 DrawTexXG_PS(NOPERSP Vec2 uv
                   : UV) : TARGET { return VecH4(SRGBToLinear(Tex(Img, uv).x).xxx, 1); }
VecH4 DrawTexYG_PS(NOPERSP Vec2 uv
                   : UV) : TARGET { return VecH4(SRGBToLinear(Tex(Img, uv).y).xxx, 1); }
VecH4 DrawTexZG_PS(NOPERSP Vec2 uv
                   : UV) : TARGET { return VecH4(SRGBToLinear(Tex(Img, uv).z).xxx, 1); }
VecH4 DrawTexWG_PS(NOPERSP Vec2 uv
                   : UV) : TARGET { return VecH4(SRGBToLinear(Tex(Img, uv).w).xxx, 1); }

VecH4 DrawTexXIG_PS(NOPERSP Vec2 uv
                    : UV) : TARGET { return VecH4(SRGBToLinear(1 - Tex(Img, uv).x).xxx, 1); }
VecH4 DrawTexYIG_PS(NOPERSP Vec2 uv
                    : UV) : TARGET { return VecH4(SRGBToLinear(1 - Tex(Img, uv).y).xxx, 1); }
VecH4 DrawTexZIG_PS(NOPERSP Vec2 uv
                    : UV) : TARGET { return VecH4(SRGBToLinear(1 - Tex(Img, uv).z).xxx, 1); }
VecH4 DrawTexWIG_PS(NOPERSP Vec2 uv
                    : UV) : TARGET { return VecH4(SRGBToLinear(1 - Tex(Img, uv).w).xxx, 1); }

VecH4 DrawTexXSG_PS(NOPERSP Vec2 uv
                    : UV) : TARGET { return VecH4(SRGBToLinear(Tex(Img, uv).x * 0.5 + 0.5).xxx, 1); }
VecH4 DrawTexXYSG_PS(NOPERSP Vec2 uv
                     : UV) : TARGET { return VecH4(SRGBToLinear(Tex(Img, uv).xy * 0.5 + 0.5), 0, 1); }
VecH4 DrawTexSG_PS(NOPERSP Vec2 uv
                   : UV) : TARGET { return SRGBToLinear(Tex(Img, uv) * 0.5 + 0.5); }

VecH4 DrawTexNrm_PS(NOPERSP Vec2 uv
                    : UV) : TARGET {
    VecH nrm;
    nrm.xy = Tex(Img, uv).BASE_CHANNEL_NORMAL; // #MaterialTextureLayout
    nrm.z = CalcZ(nrm.xy);
...
VecH4 DrawTexDetNrm_PS(NOPERSP Vec2 uv
                       : UV) : TARGET {
    VecH nrm;
    nrm.xy = Tex(Img, uv).DETAIL_CHANNEL_NORMAL * 2 - 1; // #MaterialTextureLayoutDetail
    nrm.z = CalcZ(nrm.xy);
    nrm = Normalize(nrm) * 0.5 + 0.5;
#if LINEAR_GAMMA
    nrm = SRGBToLinear(nrm);
...
VecH4 DrawX_PS(NOPERSP Vec2 uv
               : UV) : TARGET { return VecH4(Tex(ImgX, uv).xxx, 1); }
VecH4 DrawXG_PS(NOPERSP Vec2 uv
                : UV) : TARGET { return VecH4(SRGBToLinear(Tex(ImgX, uv).x).xxx, 1); }

#if defined DITHER && defined GAMMA
VecH4 DrawXC_PS(NOPERSP Vec2 uv
                : UV,
...
VecH4 DrawTexPoint_PS(NOPERSP Vec2 uv
                      : UV) : TARGET { return TexPoint(Img, uv); }
VecH4 DrawTexPointC_PS(NOPERSP Vec2 uv
                       : UV) : TARGET { return TexPoint(Img, uv) * Color[0] + Color[1]; }
/******************************************************************************/
...
VecH4 Draw2DTexCol_PS(NOPERSP Vec2 uv
                      : UV,
                        NOPERSP VecH4 col
                      : COLOR) : TARGET {
...
VecH4 Draw2DDepthTex_PS(NOPERSP Vec2 uv
                        : UV
#if COLORS
                          ,
                          NOPERSP VecH4 vtx_col
                        : COLOR
#endif
                        ) : TARGET {
...
VecH4 DrawMask_PS(NOPERSP Vec2 uv
                  : UV,
                    NOPERSP Vec2 uv_mask
                  : UV_MASK) : TARGET {
#if POINT
...
VecH4 DrawCubeFace_PS(NOPERSP Vec uv
                      : UV) : TARGET { return TexCubeClamp(Cub, uv) * Color[0] + Color[1]; }
/******************************************************************************/
...
// EDGE DETECT
/******************************************************************************/
VecH4 EdgeDetect_PS(NOPERSP Vec2 uv
                    : UV,
                      NOPERSP Vec2 posXY
                    : POS_XY) : TARGET // use VecH4 because we may want to apply this directly onto RGBA destination
{
...
VecH4 EdgeDetectApply_PS(NOPERSP Vec2 uv
                         : UV) : TARGET // use VecH4 because we apply this directly onto RGBA destination
{
    const Int samples = 4;
...
Half SetAlphaFromDepth_PS(NOPERSP Vec2 posXY
                          : POS_XY, NOPERSP PIXEL) : TARGET {
    return IsForeground(Depth[pixel.xy], posXY);
}
#if 1 // this is needed
Half SetAlphaFromDepthMS_PS(NOPERSP Vec2 posXY
                            : POS_XY, NOPERSP PIXEL, UInt index
                            : SV_SampleIndex) : TARGET {
...
Half SetAlphaFromDepthMS_PS(NOPERSP Vec2 posXY
    ...
    Half alpha = 0;
    VecI2 pix = pixel.xy;
    UNROLL for (Int i = 0; i < MS_SAMPLES; i++) alpha += IsForeground(TexDepthRawMS(pix, i), posXY);
    return alpha / MS_SAMPLES;
}
#endif

Half SetAlphaFromDepthAndCol_PS(NOPERSP Vec2 uv
                                : UV, NOPERSP Vec2 posXY
                                : POS_XY, NOPERSP PIXEL) : TARGET {
...
#if 1 // this is needed
Half SetAlphaFromDepthAndColMS_PS(NOPERSP Vec2 uv
                                  : UV, NOPERSP Vec2 posXY
                                  : POS_XY, NOPERSP PIXEL, UInt index
                                  : SV_SampleIndex) : TARGET {
...
#endif
/******************************************************************************/
VecH4 CombineAlpha_PS(NOPERSP Vec2 uv
                      : UV) : TARGET {
    VecH4 col;
...
VecH4 ReplaceAlpha_PS(NOPERSP Vec2 uv
                      : UV) : TARGET {
   ...
   if(all((cols[0]==cols[1]) && (cols[0]==cols[2]) && (cols[0]==cols[3])))discard;
    ...
    /* this Eps was calculated using formula below, it's meant to disable MS for neighbor byte colors (such as 0,1) but enable for others (0,2), so calculate at borderline of 1.5:
       ...
       Flt eps=Dist2(cols[0], cols[1])
              +Dist2(cols[0], cols[2])
              +Dist2(cols[0], cols[3]); */
    if (Dist2(cols[0], cols[1]) + Dist2(cols[0], cols[2]) + Dist2(cols[0], cols[3]) <= 0.000207612466)
        discard;
#endif
}
...
void SetDepth_PS(NOPERSP Vec2 uv
                 : UV,
                   out Flt depth
                 : DEPTH) {
...
VecH4 DrawDepth_PS(NOPERSP Vec2 uv
                   : UV) : TARGET {
...
#ifdef PERSPECTIVE
Flt LinearizeDepth0_PS(NOPERSP Vec2 uv
                       : UV) : TARGET {
...
// PALETTE
/******************************************************************************/
VecH4 PaletteDraw_PS(NOPERSP Vec2 uv
                     : UV,
                       out Half alpha
                     : TARGET1 // #RTOutput.Blend
                     ) : TARGET {
    ...
    return VecH4((c0.rgb * c0.a + c1.rgb * c1.a + c2.rgb * c2.a + c3.rgb * c3.a) / (alpha + HALF_MIN), alpha); // NaN
}
/******************************************************************************/
void ClearDeferred_VS(VtxInput vtx,
                      NOPERSP out Vec projected_prev_pos_xyw
                      : PREV_POS,
                        NOPERSP out Vec4 vpos
                      : POSITION) {
...
void ClearDeferred_PS(NOPERSP Vec projected_prev_pos_xyw
                      : PREV_POS,
                        NOPERSP PIXEL,
...
/******************************************************************************/
void ClearLight_PS(out VecH lum
                   : TARGET0,
                     out VecH spec
                   : TARGET1) {
    lum = Color[0].rgb;
...
/******************************************************************************/
VecH4 ColorLUT_PS(NOPERSP Vec2 uv
                  : UV,
                    NOPERSP PIXEL) : TARGET {
...
Flt Params2_PS() : TARGET { return NightShadeColor.x + ToneMapAtanMul + Contrast.x; }
/******************************************************************************/
#if GL // #WebSRGB
VecH4 WebLToS_PS(NOPERSP Vec2 uv
                 : UV) : TARGET { return LinearToSRGB(TexLod(Img, uv)); }
#endif
/******************************************************************************/
