...
{                                                                                 /* Unoptimized:
    ...
    //TestDepth(depth, ld.x, ofs, VecI2(-1,  0));
      ...
      TestDepth(depth, ru.w, ofs, VecI2( 1,  1)); if(all(VecI2(1, 1)==sub_offset))depth_center=ru.w;
        ...
        depth_center = ld.y; // first, skip 'if' to avoid overhead
                             // TestDepth(depth, ld.y, ofs, VecI2( 0,  0)); /*if(all(VecI2(0, 0)==sub_offset))*/depth_center=ld.y;
        TestDepth(depth, rd.x, ofs, VecI2(1, 0));
        if (all(VecI2(1, 0) == sub_offset))
            depth_center = rd.x;
        TestDepth(depth, lu.z, ofs, VecI2(0, 1));
        if (all(VecI2(0, 1) == sub_offset))
            depth_center = lu.z;
        TestDepth(depth, ru.w, ofs, VecI2(1, 1));
        if (all(VecI2(1, 1) == sub_offset))
            depth_center = ru.w;
...
Half OldWeight_PS(NOPERSP Vec2 uv
    ...
    if (NEAREST_DEPTH_VEL)
        NearestDepthRaw4x4(depth_raw, ofs, uv, GATHER); // now we use 4x4 samples (old: need to use 3x3 because 2x2 are not enough), best results are when depth is from center and motion from 'ofs' (all combinations were tested)
    else
        depth_raw = TexDepthRawPoint(uv);
...
[numthreads(16, 16, 1)] void Temporal_CS(VecU GlobalPos
                                         : SV_DispatchThreadID)
#else
void Temporal_PS(
    NOPERSP Vec2 uv
...
{
#if COMPUTE
    VecU2 pixel = GlobalPos.xy + ViewportRectI.min;
    if (any(pixel >= ViewportRectI.max))
        return;
    Vec2 uv = (pixel + 0.5) * ImgSize.xy;
#endif
    // GET DEPTH
    Flt nearest_depth_raw, depth_raw;
    VecI2 ofs;
    if (NEAREST_DEPTH_VEL)
        nearest_depth_raw = NearestDepthRaw4x4(depth_raw, ofs, uv, GATHER); // now we use 4x4 samples (old: need to use 3x3 because 2x2 are not enough), best results are when depth is from center and motion from 'ofs' (all combinations were tested)
    else
        nearest_depth_raw = depth_raw = TexDepthRawPoint(uv);
    Flt depth = LinearizeDepth(depth_raw);
...
#if CUBIC && SUPER_RES
    CubicWeight sampler_weight;
    sampler_weight.setSharp(old_weight); // when 'old_weight' is zero and old pixels are ignored, then use blurry weight to avoid flickering (because old is smooth, and cur has sharpening and jitter, so when old is ignored and we jump to cur fast, then it's seen as flickering)
#endif
...
#define old_weight sub_old_weight
    ...
    Half old_alpha = TexLod(ImgX2, old_uv);
        ...
        Sampler.set(old_uv, RTSize);
        if (!VIEW_FULL)
            Sampler.UVClamp(ImgClamp.xy, ImgClamp.zw); // here do clamping because for CUBIC we check many samples around UV
    ...
    Half cur_alpha = TexLod(ImgX, cur_uv);
...
#if CUBIC && MERGE_CUBIC_MIN_MAX // merged CUBIC with MIN MAX
        {
            ...
            UNROLL for (Int y = 0; y < 4; y++)
                UNROLL for (Int x = 0; x < 4; x++) if (!skip_corners || (x != 0 && x != 3) || (y != 0 && y != 3)) // skip corners
            {
#if VIEW_FULL
...
// MIN MAX
...
#if !VIEW_FULL
            Vec2 uv_clamp[3];
            uv_clamp[0] = Vec2(Max(uv.x - ImgSize.x, ImgClamp.x), Max(uv.y - ImgSize.y, ImgClamp.y));
            uv_clamp[1] = uv;
   ...
   // WIDEN MIN MAX
   {
    ...
    VecH4 new_col = old * old_weight + cur * cur_weight;
        ...
        if (SHOW_IGNORE_OLD)
            new_col.r = Lerp(1, new_col.r, use_old);
        if (SHOW_FLICKER)
            new_col.b = SRGBToLinearFast(new_flicker * 10 * FLICKER_EPS); // visualize flicker
