...
static Int LockedSongGet(CChar *name) {
    if (Is(name)) {
        Int i = LockedSongFind(name);
        if (i >= 0)
            return i; // find existing
        Songs.New().name = name;
        return Songs.elms() - 1; // add new one
    }
    return -1;
}
...
static C Str &SongName(Int global_song) // !! since 'Songs' is 'Memb' and never deleted, we can just return a reference !!
{
    if (InRange(global_song, Songs)) {
        ...
        return LockedSongName(global_song);
    }
    return S;
}
static Int SongGet(CChar *name) {
    if (Is(name)) {
        SyncLocker locker(SongsLock);
        return LockedSongGet(name);
    }
    return -1;
}
static Int SongGet(C UID &id) {
    if (id.valid()) {
        SyncLocker locker(SongsLock);
        return LockedSongGet(id);
    }
    return -1;
}
/******************************************************************************/
...
/******************************************************************************/
Playlist::~Playlist() {
    if (Music.playlist() == this)
        Music.set(null);
    if (Ambient.playlist() == this)
        Ambient.set(null);
}
Playlist::Playlist() {
    _cur = -1;
...
Str Playlist::song(Int i) C {
    if (InRange(i, _songs)) {
        ...
        if (InRange(i, _songs))
            return LockedSongName(_songs[i]);
    }
    return S;
}
UID Playlist::songID(Int i) C {
    if (InRange(i, _songs)) {
        SyncLocker locker(SongsLock); // needed for both '_songs' and 'LockedSongName'
        UID id;
        if (InRange(i, _songs) && LockedSongID(_songs[i], id))
            return id;
    }
    return UIDZero;
}
...
Int Playlist::globalSong(Int song, Bool shuffle, Randomizer &random) {
    ...
    if (!InRange(song, _songs)) // if not in range
    {
        ...
        {
            if (_songs.elms() == 0)
                song = -1; // no songs available
            else
                song = (shuffle ? random(_songs.elms()) : 0); // set random or first
        }
    }
    _cur = song;
...
Str MusicManager::name() C {
    SyncLocker locker(SongsLock);
    return _sound[CUR].name();
}
UID MusicManager::id() C {
    SyncLocker locker(SongsLock);
    return _sound[CUR].id();
}
Flt MusicManager::length() C {
    SyncLocker locker(SongsLock);
    return _sound[CUR].length();
}
Flt MusicManager::time() C {
    SyncLocker locker(SongsLock);
    return _sound[CUR].time();
}
Flt MusicManager::fade() C {
    SyncLocker locker(SongsLock);
    return _sound[CUR].fade();
}
Flt MusicManager::frac() C {
    SyncLocker locker(SongsLock);
    return _sound[CUR].frac();
}
void MusicManager::time(Flt time) {
    SyncLocker locker(SongsLock);
    _sound[CUR].time(time);
}
void MusicManager::frac(Flt frac) {
    SyncLocker locker(SongsLock);
    _sound[CUR].frac(frac);
}
/******************************************************************************/
void MusicManager::set(Playlist *playlist) {
    T._playlist = playlist;
    if (_song[CUR] < 0)
        play(playlist); // start playing if nothing playing right now
}
...
void MusicManager::fadeIn(Bool i) {
    Sound &sound = T._sound[i];
    if (sound.playing()) {
        sound.fadeIn(fade_in);
    }
} // !! assumes lock !!
void MusicManager::fadeOut(Bool i) {
    Sound &sound = T._sound[i];
    if (sound.playing()) {
        storePos(i);
    ...
    if (InRange(global_song, Songs)) {
        if (global_song == _song[CUR] && _sound[CUR].playing()) // if the song is already playing on CUR slot
        {
            ...
            if (sound.timeLeft() < time_reset) // if remaining song time is less than minimum desired
            {
                ...
                swap();
                fadeOut(OLD);
                _sound[OLD].callback(null);
                set(CUR, global_song);
                _sound[CUR].callback(_callback).play(); // !! don't use 'sound' reference in case 'swap' makes it invalid
            } else {
                fadeIn(CUR);
            }
        } else if (global_song == _song[OLD] && _sound[OLD].playing()) // if the song is already playing on OLD slot
        {
            swap();
            fadeOut(OLD);
            _sound[OLD].callback(null);
            ...
            if (sound.timeLeft() < time_reset)
                sound.time(0); // if remaining song time is less than minimum desired, then start from the beginning
        ...
        } else // song is not on OLD or CUR
        {
            ...
            if (sound.length() - time < time_reset)
                time = 0; // if remaining song time is less than minimum desired, then start from the beginning
        ...
        if (_history_max) {
            ...
            if (!InRange(_history_pos - 1, _history) || _history[_history_pos - 1] != global_song) // check that previous song is different (if the same then do nothing)
                if (InRange(_history_pos, _history) && _history[_history_pos] == global_song)
                    _history_pos++;
                else // if next song is the same, then increase position
                {
        ...
        fadeOut(OLD);
        fadeOut(CUR);
        _song[CUR] = -1; // clear to -1 to prevent from auto-playing the next song
    }
}
/******************************************************************************/
void MusicManager::next() {
    ...
    if (select)
        play(select(UIDZero));
    else if (Playlist *playlist = T.playlist())
        playSong(playlist->nextSong(shuffle, _random)); // copy first to temp var to avoid multi-threading issues
    else
        playSong(-1);
}
    ...
    if (_history_max) // want to use history
    {
        if (_history_pos) // have previous
        {
            ...
            if (_history_pos) // check again after having lock
            {
                ...
                if (song != _song[CUR] || !_sound[CUR].playing())
                    playSong(song);
        ...
        playSong(playlist->prevSong(shuffle, _random)); // copy first to temp var to avoid multi-threading issues
                                                        // else                               playSong(-1); do nothing
}
void MusicManager::lockedUpdate() {
    if (_song[CUR] >= 0 && lockedTimeLeft() <= fade_out)
        next();
}
/******************************************************************************/
void MusicManager::maxHistory(Int max_history) {
    ...
    if (max_history != _history_max) {
        ...
        Int add_prevs = Min(prevs - new_prevs, left);
        if (add_prevs > 0) {
            new_prevs += add_prevs;
            left -= add_prevs;
        } else {
            Int add_nexts = Min(nexts - new_nexts, left);
            if (add_nexts > 0) {
                new_nexts += add_nexts;
                left -= add_nexts;
            }
