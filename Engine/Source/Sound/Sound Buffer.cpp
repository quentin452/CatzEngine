...
{
    REP(buffers)
    AudioBuffers.removeData(buffer[i]);
}
...
void SoundBuffer::del() {
    ...
    if (ALContext) {
        ...
        if (_buffer[0])
            alDeleteBuffers(1, &_buffer[0]);
        if (_buffer[1])
            alDeleteBuffers(1, &_buffer[1]);
    }
#elif OPEN_SL
...
#elif CUSTOM_AUDIO
    if (_voice)
        _voice->remove = true; // mark for removal, it will be processed on the audio thread as long as this voice is included in the list
#endif
    zero();
}
/******************************************************************************/
...
Bool SoundBuffer::create(Int frequency, Int bits, Int channels, Int samples, Bool is3D) {
    ...
    if (SoundAPI && frequency >= 1 && (bits == 8 || bits == 16 || bits == 24) && (channels >= 1 && channels <= 2) && samples > 0) {
...
#if OPEN_AL
        if (is3D)
            channels = 1; // OpenAL supports only mono 3D
#endif
        ...
        Bool ok = false;
        if (!is3D)
            ok = OK(DS->CreateSoundBuffer(&dsbd, &_s, null));
        else {
            if (!ok && Virtualization >= VIRT_HIGH) {
                dsbd.guid3DAlgorithm = DS3DALG_HRTF_FULL;
                ok = OK(DS->CreateSoundBuffer(&dsbd, &_s, null));
            }
            if (!ok && Virtualization >= VIRT_LOW) {
                dsbd.guid3DAlgorithm = DS3DALG_HRTF_LIGHT;
                ok = OK(DS->CreateSoundBuffer(&dsbd, &_s, null));
            }
            if (!ok && Virtualization >= VIRT_NONE) {
                dsbd.guid3DAlgorithm = DS3DALG_NO_VIRTUALIZATION;
                ok = OK(DS->CreateSoundBuffer(&dsbd, &_s, null));
            }
            if (ok && !OK(_s->QueryInterface(IID_IDirectSound3DBuffer, (Ptr *)&_s3d)))
                ok = false;
        }
        if (ok)
            return true;
        ...
        if (frequency <= 192000) {
            ...
            _3d = is3D; // if sound will be 3 dimensional
            if (!_3d)
                alSourcei(_source, AL_SOURCE_RELATIVE, true); // 2d sounds are relative to listener
            return true;
        }
#elif OPEN_SL
        if (frequency <= 192000) {
                        ...
                        if ((*player_object)->GetInterface(player_object, SL_IID_PLAY, &player_play) == SL_RESULT_SUCCESS)
                            if ((*player_object)->GetInterface(player_object, SL_IID_BUFFERQUEUE, &player_buffer_queue) == SL_RESULT_SUCCESS)
                                if ((*player_buffer_queue)->RegisterCallback(player_buffer_queue, PlayerCallback, this) == SL_RESULT_SUCCESS) {
                                    ...
                                    if (t3)
                                        (*player_object)->GetInterface(player_object, SL_IID_3DLOCATION, &player_location); // this is optional
                                    if (t3)
                                        (*player_object)->GetInterface(player_object, SL_IID_3DSOURCE, &player_source); // this is optional
                                    if (t3)
                                        (*player_object)->GetInterface(player_object, SL_IID_3DDOPPLER, &player_doppler); // this is optional
        ...
        if (bits == 16) // audio renderer supports only 16-bit for now
        {
            ...
            if (buffers <= MEMBER_ELMS(AudioVoice, buffer))         // if enough
            {
                _3d = is3D;
                {
                    ...
                    FREP(buffers)
                    _voice->buffer[i] = &AudioBuffers.New(); // allocate in order
                }
                _voice->play = false;
                _voice->remove = false;
            ...
            again:
                ...
                _voice->next = first;
                if (!AtomicCAS(AudioVoiceFirst, first, _voice))
                    goto again;
                return true;
            }
        }
...
Bool SoundBuffer::lock(Int pos, Int size) {
    if (_s && !_lock_data) {
        ...
        DWORD dw;
        SOUND_API_LOCK_WEAK;
        if (OK(_s->Lock(pos, size, &_lock_data, &dw, null, null, 0))) {
            _lock_size = dw;
            return true;
        }
        _lock_data = null;
...
void SoundBuffer::unlock() {
    if (_s && _lock_data) {
        SOUND_API_LOCK_WEAK;
        _s->Unlock(_lock_data, _lock_size, null, 0);
        _lock_data = null;
        _lock_size = 0;
    }
}
...
void SoundBuffer::raw(Int raw) {
    ...
    if (_source) {
        SOUND_API_LOCK_WEAK;
        alSourcei(_source, AL_BYTE_OFFSET, raw);
    }
#elif OPEN_SL
...
Int SoundBuffer::raw() C {
    ...
    if (_s) {
        ...
        SOUND_API_LOCK_WEAK;
        if (OK(_s->GetCurrentPosition(&play, &write)))
            return play;
    }
#elif XAUDIO
    if (_sv) {
    ...
    if (_source) {
        Int raw = 0;
        SOUND_API_LOCK_WEAK;
        alGetSourcei(_source, AL_BYTE_OFFSET, &raw);
        return raw;
    }
#elif OPEN_SL
...
void SoundBuffer::pan(Flt pan) // 'pan' should be in range -1..1
{
    ...
    if (_s) {
        SOUND_API_LOCK_WEAK;
        _s->SetPan(Round(pan * DSBPAN_RIGHT));
    }
    ASSERT(-(DSBPAN_LEFT) == DSBPAN_RIGHT);
#elif XAUDIO
    if (_sv) {
        Flt vols[] = {Sat(1 - pan), Sat(1 + pan)};
        SOUND_API_LOCK_WEAK;
        _sv->SetChannelVolumes(2, vols, OPERATION_SET);
    }
#elif OPEN_AL
    ...
    if (player_volume) {
        SOUND_API_LOCK_WEAK;
        (*player_volume)->SetStereoPosition(player_volume, Round(pan * 1000));
    } // will work only if 'EnableStereoPosition' was called
#endif
}
#if OPEN_SL
Bool SoundBuffer::emulate3D() {
    ...
    {
        Flt volume, pan;
        Get3DParams(_pos, _range, _volume, volume, pan);
        SOUND_API_LOCK_WEAK;
        T.pan(pan);
...
void SoundBuffer::volume(Flt volume) // 'volume' should be in range 0..1
{
#if DIRECT_SOUND
    if (_s) {
        if (_s3d && _par.channels == 2)
            volume *= 0.5f; // 3D stereo sounds will play at different volumes than 3D mono, so adjust to match
    ...
    if (_sv) {
        SOUND_API_LOCK_WEAK;
        _sv->SetVolume(volume, OPERATION_SET);
    }
#elif OPEN_AL
    if (_source) {
        if (_par.channels == 2)
            volume *= SQRT2_2; // on OpenAL mono sounds are more quiet, we can't make them louder because AL_GAIN is limited to 0..1, so instead make stereo sounds quiet too, to match mono
        SOUND_API_LOCK_WEAK;
...
void SoundBuffer::set3DParams(C _Sound &sound, Bool pos_range, Bool speed) {
    ...
    if (_sv) {
        ...
        if (pos_range) // !! process this first because of "Zero(emitter)" !!
        {
            ...
            DEBUG_ASSERT(emitter.ChannelCount <= Elms(ChannelAzimuths), "ChannelAzimuths");
        ...
        if (speed) {
            ...
            emitter.Velocity.x = sound._vel.x;
            emitter.Velocity.y = sound._vel.y;
            emitter.Velocity.z = sound._vel.z;
            emitter.DopplerScaler = 1;
        }

        emitter.Position.x = sound._pos.x;
        emitter.Position.y = sound._pos.y;
        emitter.Position.z = sound._pos.z; // this is needed for both
        ...
        if (pos_range) {
            ...
            case 1: {
                Flt scale = 2 - Abs(pan);
                REPA(matrix)
                dsp.pMatrixCoefficients[i] *= scale;
            } break;
#endif
...
#else
                pan = pan * 0.5f + 0.5f;
                Flt vol[] = {1 - pan, pan}; // this matches mono
#endif
                if (dsp.DstChannelCount == 2) {
#if DOWNMIX_3D_STEREO
                    dsp.pMatrixCoefficients[0] *= vol[0];
                    dsp.pMatrixCoefficients[2] *= vol[0]; // dest left
                    dsp.pMatrixCoefficients[1] *= vol[1];
                    dsp.pMatrixCoefficients[3] *= vol[1]; // dest right
#else                                                     // for left dest use only left source, for right dest use only right source
                    dsp.pMatrixCoefficients[0] = (dsp.pMatrixCoefficients[0] + dsp.pMatrixCoefficients[2]) * vol[0];
                    dsp.pMatrixCoefficients[2] = 0; // dest left
                    dsp.pMatrixCoefficients[3] = (dsp.pMatrixCoefficients[3] + dsp.pMatrixCoefficients[1]) * vol[1];
                    dsp.pMatrixCoefficients[1] = 0; // dest right
#endif
                } else
                    _sv->SetChannelVolumes(2, vol, OPERATION_SET);
            } break;
            }
        ...
        if (speed)
            _sv->SetFrequencyRatio(SoundSpeed(sound._actual_speed * dsp.DopplerFactor), OPERATION_SET);
    }
#elif CUSTOM_AUDIO
    if (_voice) {
        if (pos_range) {
            ...
            Flt volume, pan;
            Get3DParams(sound.pos(), sound._actual_range, _volume, volume, pan);
#if FULL_VOL_AT_CENTER
        ...
        if (speed)
            T.speed(SoundSpeed(sound._actual_speed));
    }
#endif
}
#endif
void SoundBuffer::frequency(Int frequency) {
    ...
    if (_s) {
        SOUND_API_LOCK_WEAK;
        _s->SetFrequency(Mid(frequency, DSBFREQUENCY_MIN, DSBFREQUENCY_MAX));
    }
#else
...
void SoundBuffer::speed(Flt speed) {
    ...
    if (_sv) {
        SOUND_API_LOCK_WEAK;
        _sv->SetFrequencyRatio(speed, OPERATION_SET);
    }
#elif OPEN_AL
    if (_source) {
        SOUND_API_LOCK_WEAK;
        alSourcef(_source, AL_PITCH, speed);
    }
#elif OPEN_SL
    if (player_playback_rate) {
        SLpermille min_rate = 500, max_rate = 2000, step_size;
        SLuint32 capabilities; // default limit is 0.5 to 2.0 (tested on HTC Evo 3D Android 2.3)
        SOUND_API_LOCK_WEAK;
...
#elif CUSTOM_AUDIO
    if (_voice)
        _voice->speed = Flt(_par.frequency) / AudioOutputFreq * speed;
#endif
}
void SoundBuffer::pos(C Vec &pos) {
    ...
    if (_s3d) {
        SOUND_API_LOCK_WEAK;
        _s3d->SetPosition(pos.x, pos.y, pos.z, DS3D_DEFERRED);
    }
    ...
    if (!emulate3D() && player_location) {
        SLVec3D location;
        location.x = Round(pos.x / SL_POS_UNIT);
        location.y = Round(pos.y / SL_POS_UNIT);
        location.z = Round(pos.z / SL_POS_UNIT);
        SOUND_API_LOCK_WEAK;
        (*player_location)->SetLocationCartesian(player_location, &location);
    }
#endif
}
Vec SoundBuffer::pos() C {
    ...
    if (_source && _3d) {
        Vec pos;
        SOUND_API_LOCK_WEAK;
        alGetSource3f(_source, AL_POSITION, &pos.x, &pos.y, &pos.z);
        return pos;
    }
#elif OPEN_SL
#if 1 // faster
...
void SoundBuffer::vel(C Vec &vel) {
    ...
    if (_s3d) {
        SOUND_API_LOCK_WEAK;
        _s3d->SetVelocity(vel.x, vel.y, vel.z, DS3D_DEFERRED);
    }
#elif XAUDIO || CUSTOM_AUDIO
    // handled in 'set3DParams'
#elif OPEN_AL
    ...
    if (player_doppler) {
        SLVec3D velocity;
        velocity.x = Round(vel.x / SL_POS_UNIT);
        velocity.y = Round(vel.y / SL_POS_UNIT);
        velocity.z = Round(vel.z / SL_POS_UNIT);
        SOUND_API_LOCK_WEAK;
        (*player_doppler)->SetVelocityCartesian(player_doppler, &velocity);
    }
#endif
}
Vec SoundBuffer::vel() C {
    ...
    if (_source && _3d) {
        Vec vel;
        SOUND_API_LOCK_WEAK;
        alGetSource3f(_source, AL_VELOCITY, &vel.x, &vel.y, &vel.z);
        return vel;
    }
#elif OPEN_SL
    if (player_doppler) {
...
void SoundBuffer::range(Flt range) {
    ...
    if (_s3d) {
        SOUND_API_LOCK_WEAK;
        _s3d->SetMinDistance(range, DS3D_DEFERRED);
    }
#elif XAUDIO || CUSTOM_AUDIO
    // handled in 'set3DParams'
#elif OPEN_AL
    if (_source) {
        SOUND_API_LOCK_WEAK;
        alSourcef(_source, AL_REFERENCE_DISTANCE, range);
    }
#elif OPEN_SL
    T._range = range;
    if (!emulate3D() && player_source) {
        SOUND_API_LOCK_WEAK;
        (*player_source)->SetRolloffDistances(player_source, RoundPos(range / SL_POS_UNIT), SL_MILLIMETER_MAX);
    }
#endif
}
Flt SoundBuffer::range() C {
    ...
    if (_s3d) {
        Flt range = 0;
        SOUND_API_LOCK_WEAK;
        if (OK(_s3d->GetMinDistance(&range)))
            return range;
    }
#elif XAUDIO || CUSTOM_AUDIO
    // unavailable
    ...
    if (_source) {
        SOUND_API_LOCK_WEAK;
        alGetSourcef(_source, AL_REFERENCE_DISTANCE, &range);
    }
    return range;
#elif OPEN_SL
#if 1 // faster
...
void SoundBuffer::stop() {
    ...
    if (_sv) {
        SOUND_API_LOCK_WEAK;
        _sv->Stop(0, OPERATION_SET);
        _sv->FlushSourceBuffers();
    } // 'FlushSourceBuffers' - removes all pending audio buffers from this voice's queue
#elif OPEN_AL
    if (_source) {
        SOUND_API_LOCK_WEAK;
        alSourceStop(_source);
        alSourcei(_source, AL_BUFFER, 0);
    } // calling "alSourcei(_source, AL_BUFFER, 0)" means clearing the buffer queue, this is correct
#elif OPEN_SL
    SOUND_API_LOCK_WEAK;
    if (player_play)
...
void SoundBuffer::pause() {
    ...
    if (player_play) {
        SOUND_API_LOCK_WEAK;
        (*player_play)->SetPlayState(player_play, SL_PLAYSTATE_PAUSED);
    }
#elif CUSTOM_AUDIO
    if (_voice)
        _voice->play = false;
...
void SoundBuffer::toggle(Bool loop) {
    ...
    if (_s) {
        ...
        if (playing())
            ...
            _s->Play(0, 0, loop ? DSBPLAY_LOOPING : 0);
    }
#elif XAUDIO
    // unavailable
#elif OPEN_AL
    ...
    if (player_play) {
        SOUND_API_LOCK_WEAK;
        (*player_play)->SetPlayState(player_play, playing() ? SL_PLAYSTATE_PAUSED : SL_PLAYSTATE_PLAYING);
    }
#elif CUSTOM_AUDIO
    if (_voice)
        _voice->play ^= 1;
...
Bool SoundBuffer::playing() C {
    ...
    return FlagOn(status, DSBSTATUS_PLAYING);
#elif XAUDIO
    // unavailable
    return false;
#elif OPEN_AL
    ...
    if (player_play) {
        SOUND_API_LOCK_WEAK;
        (*player_play)->GetPlayState(player_play, &state);
    }
...
void SoundBuffer::play(Bool loop) {
    ...
    if (_sv) {
        SOUND_API_LOCK_WEAK;
        _sv->Start(0, OPERATION_SET);
    }
#elif OPEN_AL
    if (_source) {
        SOUND_API_LOCK_WEAK;
        alSourcePlay(_source);
    } // don't touch AL_LOOPING because OpenAL buffers won't swap
#elif OPEN_SL
    if (player_play) {
        SOUND_API_LOCK_WEAK;
...
ListenerClass::ListenerClass() {
    ...
    _vel.zero();
    _range = 1;
    _flag = SOUND_CHANGED_POS | SOUND_CHANGED_VEL | SOUND_CHANGED_ORN | SOUND_CHANGED_RANGE;
} // set flag to make sure that we set params in case the SoundAPI uses different initial values
ListenerClass &ListenerClass::pos(C Vec &pos) {
    if (T.pos() != pos) {
        T._orn.pos = pos;
        AtomicOr(_flag, SOUND_CHANGED_POS);
    }
    return T;
} // modify first and enable flag at the end
ListenerClass &ListenerClass::vel(C Vec &vel) {
    if (T.vel() != vel) {
        T._vel = vel;
        AtomicOr(_flag, SOUND_CHANGED_VEL);
    }
    return T;
} // modify first and enable flag at the end
    ...
    if (_flag) {
        ...
        if (DSL) {
            if (flag & SOUND_CHANGED_POS)
                DSL->SetPosition(pos().x, pos().y, pos().z, DS3D_DEFERRED);
            if (flag & SOUND_CHANGED_VEL)
                DSL->SetVelocity(vel().x, vel().y, vel().z, DS3D_DEFERRED);
            if (flag & SOUND_CHANGED_ORN)
                DSL->SetOrientation(dir().x, dir().y, dir().z, up().x, up().y, up().z, DS3D_DEFERRED);
...
#elif XAUDIO
        if (flag & SOUND_CHANGED_POS) {
            X3DListener.Position.x = pos().x;
            X3DListener.Position.y = pos().y;
            X3DListener.Position.z = pos().z;
        }
        if (flag & SOUND_CHANGED_VEL) {
            X3DListener.Velocity.x = vel().x;
            X3DListener.Velocity.y = vel().y;
            X3DListener.Velocity.z = vel().z;
        }
        if (flag & SOUND_CHANGED_ORN) {
            X3DListener.OrientFront.x = dir().x;
            X3DListener.OrientFront.y = dir().y;
            X3DListener.OrientFront.z = dir().z;
            X3DListener.OrientTop.x = up().x;
            X3DListener.OrientTop.y = up().y;
            X3DListener.OrientTop.z = up().z;
        }
#elif OPEN_AL
        if (ALContext) {
            if (flag & SOUND_CHANGED_POS)
            ...
            if (flag & SOUND_CHANGED_ORN) {
                Flt f[] = {-dir().x, -dir().y, -dir().z, up().x, up().y, up().z};
        ...
        if (SLListenerLocation) {
            ...
            if (flag & SOUND_CHANGED_ORN) {
                SLVec3D front;
                front.x = Round(dir().x * 1024);
                front.y = Round(dir().y * 1024);
                front.z = Round(dir().z * 1024);
                SLVec3D above;
                above.x = Round(up().x * 1024);
                above.y = Round(up().y * 1024);
                above.z = Round(up().z * 1024);
                (*SLListenerLocation)->SetOrientationVectors(SLListenerLocation, &front, &above); // length of vectors is free of choice
            }
        }
        if (flag & SOUND_CHANGED_VEL)
            if (SLListenerDoppler) {
                SLVec3D velocity;
                velocity.x = Round(vel().x / SL_POS_UNIT);
                velocity.y = Round(vel().y / SL_POS_UNIT);
                velocity.z = Round(vel().z / SL_POS_UNIT);
...
#if DIRECT_SOUND
    if (DSL)
        DSL->CommitDeferredSettings();
#elif XAUDIO
    if (XAudio)
        XAudio->CommitChanges(XAUDIO2_COMMIT_ALL);
#elif OPEN_AL
    // 'alcProcessContext' 'alcSuspendContext' are not used because they seem to be a no-op and there's no way to test them
#elif OPEN_SL
    if (SLListenerCommit)
        (*SLListenerCommit)->Commit(SLListenerCommit);
    else {                                            // if listener is not available then it means 3D Audio is simulated manually
...
Bool ListenerClass::create() {
    ...
    if (DS) {
        ...
        if (!OK(DS->CreateSoundBuffer(&dsbd, &DSB, null)))
            return false;
    ...
    if (XAudioMasteringVoice) {
        ...
        if (OK(XAudioMasteringVoice->GetChannelMask(&channel_mask)))
            if (OK(X3DAudioInitialize(channel_mask, X3DAUDIO_SPEED_OF_SOUND, X3DAudio)))
                return true;
    }
    ...
    again:
        ...
        if (buffer_raw >= buffer_size) // proceed to the next buffer
        {
            ...
            if (AtomicDec(queued) > 1 // decrease number of queued buffers, if still have some available
                && resampler.dest_samples > 0)
                goto again; // process next buffer
        }
...
static inline Bool AudioSave() {
    ...
    if (timeGetDevCaps(&tc, SIZE(tc)) == MMSYSERR_NOERROR)
        timeBeginPeriod(Mid(ms, tc.wPeriodMin, tc.wPeriodMax)); // need to enable high precision timers because default accuracy is around 16 ms
#endif
    return true;
}
#endif
void InitSound() {
    if (SoundFunc)
        return; // return if it was already created
    if (LogInit)
        LogN("InitSound");
    SOUND_API_LOCK_FORCE;
    SoundFunc = true;
...
#pragma message("!! Warning: Use only for testing !!")
    ...
    if (OK(XAudio2Create(&XAudio, XAUDIO_DEBUG ? XAUDIO2_DEBUG_ENGINE : 0)))
        if (OK(XAudio->CreateMasteringVoice(&XAudioMasteringVoice))) // even though it is not used for anything, it is still needed because sound buffer creation would fail without it
        {
            ...
            XAudioMasteringVoice->GetVoiceDetails(&details);
    ...
    if (NS::InitSound())
            ...
            if (AudioOutputFreq && AudioOutputFrameSamples && AudioOutputFrameSize && AudioOutputFrameData && AudioSave() && AudioThread.create(AudioUpdate, null, 3 /*priority*/, false, "EE.Audio"))
#endif
...
void ShutSound() {
    ...
    if (SLListenerObject) {
        (*SLListenerObject)->Destroy(SLListenerObject);
        SLListenerObject = null;
    }
