    ...
    AudioClientGetter() {
        ...
        if (App.window()) // can call methods only if app was initialized, otherwise it will crash
        {
            ...
            if (OK(ActivateAudioInterfaceAsync(device_name->Data(), __uuidof(IAudioClient3), null, this, &audio_activate))) {
                ...
                HRESULT hr;
                IUnknown *ac = null;
                audio_activate->GetActivateResult(&hr, &ac);
                audio_activate->Release();
                if (ac) {
...
struct _SoundRecord {
    ...
    void update(SoundRecord &sound_record) {
        for (;;) {
            ...
            DWORD flags;
            UINT64 pos, pc_pos;
            if (OK(audio_capture_client->GetNextPacketSize(&frames)) && frames > 0 && OK(audio_capture_client->GetBuffer(&data, &frames, &flags, &pos, &pc_pos))) {
...
void SoundRecord::del() {
    ...
    if (_handle) {
        ...
        if (jni && ActivityClass) {
            Str8 signature = S8 + "(L" + AndroidPackageName + "/EsenthelActivity$EsenthelAudioRecord;)V";
            signature.replace('.', '/');
...
static OSStatus AudioInputProc(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, AudioBufferList *ioData) {
    ...
    if (AudioUnitRender(sr._handle, ioActionFlags, inTimeStamp, inBusNumber, inNumberFrames, &buffer.list) == noErr) {
        if (same_format) {
            ...
            src_left = src;
            if (non_interleaved) {
                src_right = (Byte *)src + got_data_size / 2;
                src_inc = got_bytes;
            } else if (got_stereo) {
                src_right = (Byte *)src + got_bytes;
                src_inc = got_bytes * 2;
            } else {
                src_right = src;
                src_inc = got_bytes;
            ...
            REP(inNumberFrames) {
                ...
                case 1:
                    if (int_signed) {
                        left = (*(I8 *)src_left << 8);
                        right = (*(I8 *)src_right << 8);
                    } else {
                        left = (*(U8 *)src_left << 8) - 32768;
                        right = (*(U8 *)src_right << 8) - 32768;
                    }
                    break;
                case 2:
                    if (int_signed) {
                        ...
                        left = *(U16 *)src_left - 32768;
                        right = *(U16 *)src_right - 32768;
                    }
                    break;
                case 4:
                    left = Round(*(Flt *)src_left * 32767);
                    right = Round(*(Flt *)src_right * 32767);
                    break;
                }
...
Bool SoundRecord::create(Device *device, Int bits, Int channels, Int frequency) // this needs to be called after 'DirectSound' playback device
{
    ...
    del();
    _SoundRecord sr;
    if (sr.create(bits, channels, frequency)) {
        New(_handle);
        Swap(*_handle, sr);
        SafeSyncLocker locker(SoundAPILock);
    ...
    if (channels >= 1 && channels <= 2 && (bits == 8 || bits == 16)) {
        ...
        if (device_id == kAudioDeviceUnknown) {
            ...
            size = SIZE(device_id);
            status = AudioObjectGetPropertyData(kAudioObjectSystemObject, &propertyAddress, 0, null, &size, &device_id);
        }
        if (device_id != kAudioDeviceUnknown)
#elif IOS
...
#error unknown platform
                    ...
                    if (AudioUnitSetProperty(_handle, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Input, kInputBus, &one, SIZE(one)) == noErr)                        // enable  input
...
#endif
                        {
                            ...
                            if (status == noErr && got_channels >= 1 && got_channels <= 2 && (got_bits == 8 || got_bits == 16 || got_bits == 32)) {
                                ...
                                _flags = (((FlagOn(format.mFormatFlags, kAudioFormatFlagIsSignedInteger)) << 0) | ((got_channels == 2 && FlagOn(format.mFormatFlags, kAudioFormatFlagIsNonInterleaved)) << 1) | ((channels == 2) << 2) | ((got_channels == 2) << 3) | ((bits / 8) << 4) | ((got_bits / 8) << 7));
    ...
    if (jni && ActivityClass) {
        ...
        Str8 signature = S8 + "(JIII)L" + AndroidPackageName + "/EsenthelActivity$EsenthelAudioRecord;";
        signature.replace('.', '/');
        if (JMethodID newAudioRecord = jni.staticFunc(ActivityClass, "newAudioRecord", signature))
            if (JObject audio_record = JObject(jni, jni->CallStaticObjectMethod(ActivityClass, newAudioRecord, jlong(this), jint(bits), jint(channels), jint(frequency)))) {
                audio_record.makeGlobal();
                _handle = audio_record();
                audio_record.clear();
                return true;
            }
    }
    ...
    if (channels >= 1 && channels <= 2 && (bits == 8 || bits == 16)) {
        if (_handle = alcCaptureOpenDevice(device ? Str8(device->name)() : null, frequency,
                                           ...
                                           frequency * SOUND_TIME_RECORD / 1000)) {
...
void SoundRecord::GetDevices(MemPtr<Device> devices) {
    devices.clear();
#if WINDOWS_NEW
    if (auto id = MediaDevice::GetDefaultAudioCaptureId(AudioDeviceRole::Default))
        if (Is(id->Data()))
    ...
    UInt32 dataSize = 0;
    if (AudioObjectGetPropertyDataSize(kAudioObjectSystemObject, &propertyAddress, 0, null, &dataSize) == kAudioHardwareNoError) {
        MemtN<AudioDeviceID, 16> audioDevices;
        audioDevices.setNum(dataSize / SIZE(AudioDeviceID));
...
Int SoundRecord::curPosNoLock() C {
    ...
    if (_dscb)
        _dscb->GetCurrentPosition(&capture, &read);
    return read;
#else
    return 0;
#endif
...
void SoundRecord::updateNoLock() {
    ...
    if (_dscb) {
        ...
        if (cur_pos != _pos) {
            Int size = cur_pos - _pos;
            if (size < 0)
                size += _size;
            Ptr data = null, data2 = null;
            DWORD locked = 0, locked2 = 0;
            if (OK(_dscb->Lock(_pos, size, &data, &locked, &data2, &locked2, 0))) {
                if (data)
                    receivedData(data, locked);
                if (data2)
                    receivedData(data2, locked2);
    ...
    again:
        ALint samples = 0;
        alcGetIntegerv(_handle, ALC_CAPTURE_SAMPLES, 1, &samples);
        if (samples > 0) {
            Byte data[64 * 1024];
            Int size = samples * _block;
            Bool full = (size >= SIZE(data));
            if (full) {
