...
CChar8 *CodecName(SOUND_CODEC codec) {
    ...
    case SOUND_WEBM_VORBIS:
        return "Vorbis";

    case SOUND_SND_OPUS:
    case SOUND_OGG_OPUS:
    case SOUND_WEBM_OPUS:
        return "Opus";
    }
}
/******************************************************************************/
            ...
            for (; !f.end();) {
                ...
                if (data == CC4('f', 'm', 't', ' ')) {
                    ...
                    case SOUND_WAV:
                        ...
                        break;
                    case SOUND_MP3:
                        break; // allow all bits because actual data will be loaded in 'LoadMP3Header'
                    }
                    params.bytes = bits / 8;
                    size -= 16; // 16 bytes were read
                } else if (data == CC4('d', 'a', 't', 'a')) {
                    params.size = size;
...
#define USE_TIME_RAW 0
...
#if USE_TIME_RAW
    instead of time <->raw it would be better if this would operate directly on frame_size, frame_pos, decoded_frame_size..struct TimeRaw {
        Long time, raw;

        void set(Long time, Long raw) {
            T.time = time;
            T.raw = raw;
        }
                    ...
                    if (C mkvparser::Cluster *cluster = block_entry->GetCluster())
                        ...
                        again:
                            ...
                            FREP(block->GetFrameCount()) {
                                ...
                                Byte temp[65536];
                                Int frame_size;
                                if (frame.len > SIZE(temp) || !file->pos(frame.pos) || !file->getFast(temp, frame.len) || !decodeFrameSize(temp, frame.len, frame_size))
                                    goto error;
                                size += frame_size;
                            }
                            if (advance(cluster, block_entry, block))
                                goto again;
                        }
    ...
    advance:
        ...
        if (!block)
            return false;
        if (block->GetTrackNumber() != track_number)
            goto advance;
        return true;
    }
    Bool advance() {
    ...
    virtual Bool pos(Long pos) {
        ...
        if (segment) {
            ...
            {
                Int index;
                if (!time_raw.binarySearch(pos, index, TimeRaw::Compare))
                    index--; // if didn't found an exact match, then we need to use the previous one
                tr = &time_raw[index];
                time = tr->time;
            } else
#endif
...
#if USE_TIME_RAW
                                    if (time_raw.elms())
                                        buffer_pos = pos - tr->raw;
                                    else
                                        this is wrong because we may have used previous block
#endif
        ...
        again:
            ...
            if(frame_index==0)
            {
               ...
               LogN(S+time+' '+time2+' '+(time2-time));
                ...
                Byte temp[65536];
                if (frame.len > SIZE(temp) || !file->pos(frame.pos) || !file->getFast(temp, frame.len) || !decode(data, size, temp, frame.len, read))
                    goto error;
                if (!read && !buffer.elms())
                    goto again; // if 'decode' succeeded, but nothing was 'read' or placed to the 'buffer' then try again (this can happen for Vorbis)
            }
            ...
            if (buffer_left > 0) // if we have any left in the buffer
            {
                ...
                buffer_pos += buffer_left;
                if (buffer_pos >= buffer.elms())
                    clearBuffer(); // if we've read the entire buffer then clear it
                data = (Byte *)data + buffer_left;
                read += buffer_left;
                size -= buffer_left;
            } else if (buffer.elms() && buffer_pos >= buffer.elms()) // this can happen if we had some data in the buffer, but we had to skip everything
            {
    ...
    WebmFile(C SoundStream::Params &params, File &file, Int track_number, mkvparser::Segment *&segment, Int extra_frames) : MkvReader(file) {
        ...
        if (T.segment = segment) {
            ...
            if (T.cluster = T.segment->GetFirst()) {
                ...
                if (T.block_entry)
                    if (T.block = T.block_entry->GetBlock())
                        if (T.block->GetTrackNumber() != track_number)
                            advance();
            }
...
static Int BitRate(C vorbis_info &vi) {
    return (vi.bitrate_nominal > 0) ? vi.bitrate_nominal : (vi.bitrate_lower > 0 && vi.bitrate_upper > 0) ? AvgI(UInt(vi.bitrate_lower), UInt(vi.bitrate_upper))
                                                                                                          : 0;
}
...
static Int VorbisClose(void *file) { /*((File*)file)->del          (                );*/
    return 0;
} // don't do anything in case it wants to close on load fail, but we still need the file to try other codecs, also we delete the file manually, so this is not needed
...
struct WebmVorbisFile : WebmFile {
    ...
    WebmVorbisFile(SoundStream::Params &params, File &file, Int track_number, mkvparser::Segment *&segment, C Byte *priv_data, Int priv_data_size) : WebmFile(params, file, track_number, segment, 1) {
        ...
        if (priv_data && priv_data_size && priv_data[0] == 2) {
            ...
            if (!error && !vorbis_synthesis_init(&vd, &vi) && !vorbis_block_init(&vd, &vb)) {
                Int bit_rate = BitRate(vi);
                if (bit_rate > 0)
    ...
    virtual Bool decodeFrameSize(Ptr temp, Int temp_size, Int &size) override {
        ...
        packet.granulepos = -1;
        packet.packetno = -1;
        size = vorbis_packet_blocksize(&vi, &packet);
        if (size < 0)
            return false;
    ...
    virtual Bool decode(Ptr &data, Int &size, Ptr temp, Int temp_size, Int &read) override {
        ...
        if (!vorbis_synthesis(&vb, &packet) && !vorbis_synthesis_blockin(&vd, &vb)) {
            for (;;) {
                Flt **pcm = null;
                Int decoded_samples = vorbis_synthesis_pcmout(&vd, &pcm);
                ...
                if (vorbis_synthesis_read(&vd, decoded_samples))
                    return false;
            }
            return true;
        }
...
static int OpusClose(void *file) { /*((File*)file)->del          (          );*/
    return 0;
} // don't do anything in case it wants to close on load fail, but we still need the file to try other codecs, also we delete the file manually, so this is not needed
...
struct WebmOpusFile : WebmFile {
...
#if USE_TIME_RAW
    virtual Bool decodeFrameSize(Ptr temp, Int temp_size, Int &size) override {
        size = opus_packet_get_nb_samples((Byte *)temp, temp_size, od.frequency());
        if (size < 0)
            return false;
        size *= buf_block;
        return true;
    ...
    virtual Bool decode(Ptr &data, Int &size, Ptr temp, Int temp_size, Int &read) override {
        if (buffer_pos == 0 && size >= OpusMaxFrameSize(od.channels())) // decode directly into 'data', we can do this only if we're not skipping (buffer_pos==0) and if we know that 'data' can fit an entire frame
        {
            ...
            read += decoded;
            size -= decoded;
        } else if (!od.decode(temp, temp_size, buffer))
            return false;
        return true;
    }
...
struct SndOpusFile {
    ...
    Bool pos(Long raw) {
        ...
        buffer_pos = raw - packet_index * frame_size;
        buffer.clear();
        Long pos = audio_offset;
        for (Int i = 0, end = Min(packet_sizes.elms(), packet_index); i < end; i++)
            pos += packet_sizes[i];
        return file->pos(pos);
    }
    Int set(Ptr data, Int size) // !! Warning: 'size' should be clamped to skip the zero padding at the end !! this is done in 'SoundStream.set' because 'SndOpusFile' doesn't store current position and total size (to reduce memory usage)
    {
        ...
        for (; size > 0;) {
            ...
            {
                if (!InRange(packet_index, packet_sizes))
                    break;
                Byte packet_data[65536];
                Int packet_size = packet_sizes[packet_index++];
                if (packet_size > SIZE(packet_data) || !file->getFast(packet_data, packet_size))
                    break;
                ...
                if (buffer_pos == 0 && size >= frame_size) // decode directly into 'data', we can do this only if we're not skipping (buffer_pos==0) and if we know that 'data' can fit an entire frame
                {
                    ...
                    if (!od.decode(packet_data, packet_size, buffer))
                        break;
                            ...
                            if (track->GetType() == mkvparser::Track::kAudio) {
#if SUPPORT_VORBIS
                                if (Equal(track->GetCodecId(), "A_VORBIS"))
                                    codec = SOUND_WEBM_VORBIS;
#endif
#if SUPPORT_OPUS
                                if (Equal(track->GetCodecId(), "A_OPUS"))
                                    codec = SOUND_WEBM_OPUS;
#endif
                                if (codec) // stop on first found
                                {
                                        ...
                                        && frequency >= 1)                                               // to avoid div by zero
                                    {
                                        ...
                                        C Byte *priv_data = audio_track.GetCodecPrivate(priv_data_size);
#if SUPPORT_VORBIS
                                        if (codec == SOUND_WEBM_VORBIS)
                                            return new WebmVorbisFile(params, f, audio_track.GetNumber(), segment, priv_data, priv_data_size);
#endif
#if SUPPORT_OPUS
                                        if (codec == SOUND_WEBM_OPUS)
                                            return new WebmOpusFile(params, f, audio_track.GetNumber(), segment, priv_data, priv_data_size);
#endif
                                    }
                                }
                            }
...
struct MP3 {
    ...
    Bool pos(Long sample) {
        if (sample >= 0) {
            ...
            Int frame_index = sample / frame_samples;
            if (InRange(frame_index, frame_pos)) {
    ...
    Int decode(const uint8_t *mp3, int mp3_bytes, Byte *pcm) // return number of bytes decoded (not samples)
    {
        ...
        if (samples == frame_samples)
            return frame_samples * block; // all OK
        switch (samples) {
        default:
            return 0; // failed
    ...
    Int set(Ptr data, Int size) {
        ...
        for (; size > 0;) {
            ...
            } else // need to decode
            {
                ...
                } else if (InRange(frame_index, frame_pos))              // last frame
                {
                ...
                if (!file->getFast(frame_data, frame_size))
                    break;
                ...
                {
                    Int decoded = decode(frame_data, frame_size, (Byte *)data);
                    if (decoded <= 0)
                        break;
                    read += decoded;
                    size -= decoded;
                    ...
                    Int decoded = decode(frame_data, frame_size, buf);
                    if (decoded <= 0)
                        break;
...
static MP3 *LoadMP3Header(File &f, SoundStream::Params &params) {
    ...
    for (;;) {
        if (buf_size < MAX_FRAME_SYNC_MATCHES * MAX_L3_FRAME_PAYLOAD_BYTES && !f.end()) // if not enough data left for N frames, but still have some data in the file
        {
            ...
            if (!f.getFast(buf + buf_size, read))
                return null; // read extra data
            buf_size += read;
        }

        mp3dec_frame_info_t frame_info;
        Int samples = mp3dec_decode_frame(&dec, buf, buf_size, null, &frame_info);
        Int offset_size = frame_info.frame_offset + frame_info.frame_size;
        if (!offset_size)
            break;
        if (samples > 0) {
            if (!frame_pos.elms()) {
                if (frame_info.channels < 1 || frame_info.channels > 2)
                    return null; // support only mono and stereo
                if (samples != MP3_FRAME_SAMPLES && samples != MP3_FRAME_SAMPLES2)
                    return null; // support only these samples
                params.channels = frame_info.channels;
                params.frequency = frame_info.hz;
                frame_samples = samples;
            } else {
                if (params.channels != frame_info.channels || params.frequency != frame_info.hz || frame_samples != samples)
                    return null;
            }
    ...
    if (frame_pos.elms()) {
        ...
        params.bit_rate = (1 ? bit_rate * 1000 / frame_pos.elms() : total_samples ? f.size() * 8 * params.frequency / total_samples
                                                                                  : 0);
...
struct FLAC {
    ...
    static void Meta(const FLAC__StreamDecoder *decoder, const FLAC__StreamMetadata *metadata, void *client_data) {
        if (metadata && metadata->type == FLAC__METADATA_TYPE_STREAMINFO) {
            ...
            if (flac.to_read < 0) // only if we're expecting metadata
            {
    ...
    static FLAC__StreamDecoderWriteStatus Write(const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame, const FLAC__int32 *const buffer[], void *client_data) {
        ...
        if (flac.to_read > 0) // process this only if we want to read something, this is to avoid cases when this callback is called during some other flac call like seeking (yes this can happen)
        {
            ...
            case 8: {
                ...
                for (; read_samples < n; read_samples++)
                    FREPD(c, channels) *data++ = buffer[c][read_samples] + 128; // FLAC data will be signed, however we need unsigned
                ...
                if (Int left_samples = frame_samples - read_samples) {
                    ...
                    for (; read_samples < frame_samples; read_samples++)
                        FREPD(c, channels) *data++ = buffer[c][read_samples] + 128; // FLAC data will be signed, however we need unsigned
                }
            } break;
            ...
            case 16: {
                ...
                for (; read_samples < n; read_samples++)
                    FREPD(c, channels) *data++ = buffer[c][read_samples];
...
static Bool Decode(HANDLE_AACDECODER decoder, UCHAR *packet_data, UInt packet_size, Ptr data, Int data_size, UInt flag) {
    ...
    return aacDecoder_Fill(decoder, &packet_data, &packet_size, &valid) == AAC_DEC_OK && aacDecoder_DecodeFrame(decoder, (Short *)data, data_size / SIZE(Short), flag) == AAC_DEC_OK;
}
struct AAC {
    struct Part {
        Long pos;
    ...
    AAC() {
        decoder = null;
        part_index = packet_index = buffer_pos = 0;
        frame_size = flag = delay = 0;
        file = null;
    }
    ~AAC() {
        if (decoder) {
    ...
    inline Bool decode(UCHAR *packet_data, UInt packet_size, Ptr data, Int data_size) {
        Bool ok = Decode(decoder, packet_data, packet_size, data, data_size, flag);
        flag = 0;
        return ok;
    }
    ...
    Bool pos(Long raw) {
        ...
        buffer_pos = raw - frame_index * frame_size;
        buffer.clear();
        FREPA(parts) {
            C Part &part = parts[i];
        ...
        for (; size > 0;) {
            ...
            } else // need to decode
            {
                if (!InRange(part_index, parts))
                    break;
                C Part &part = parts[part_index];
                if (!InRange(packet_index, part.packet_sizes))
                    break;
                Byte packet_data[65536];
                ...
                if (buffer_pos == 0 && size >= frame_size) // decode directly into 'data', we can do this only if we're not skipping (buffer_pos==0) and if we know that 'data' can fit an entire frame
                {
                    if (!decode(packet_data, packet_size, data, size))
                        break;
                    read += frame_size;
                    size -= frame_size;
                    ...
                    if (!decode(packet_data, packet_size, buffer.data(), buffer.elms())) {
                        buffer.clear();
                        break;
                    }
                    if (buffer_pos >= buffer.elms()) // if we have to skip everything
...
static AAC *LoadAACHeader(File &f, SoundStream::Params &params, Int audio_track = 0, Bool limit_to_stereo = true) {
    ...
    if (mp4.read(f))
        FREPA(mp4.tracks) {
            MP4::Track &track = mp4.tracks[i];
            if (track.decoder_config.elms() && !audio_track--) {
                    ...
                    if (aacDecoder_ConfigRaw(decoder, &ASC_data, &ASC_size) == AAC_DEC_OK)
                        if (CStreamInfo *info = aacDecoder_GetStreamInfo(decoder))
                            if (track.frames()) {
                                ...
                                Memt<Byte> frame_data;
                                frame_data.setNum(frame_size);
                                if (f.pos(frame_offset) && frame_data.loadRawData(f)) {
                                    Int channels = Max(2, track.channels, info->channelConfig, info->aacNumChannels);
                                    if (limit_to_stereo)
                                        MIN(channels, 2);                                    // use Max(2 because in one case (track.channels=1, info->channelConfig=1, info->aacNumChannels=0, but info->numChannels=2)
                                    ...
                                    Memt<Byte> pcm_data;
                                    pcm_data.setNum(frame_samples * channels * SIZE(Short));
                                    if (Decode(decoder, frame_data.data(), frame_data.elms(), pcm_data.data(), pcm_data.elms(), 0)) {
                                        ...
                                        MAX(params.size -= info->outputDelay, 0);
                                        params.size *= params.block;
                                        params.bit_rate = track.avg_bit_rate;
                                        if (!params.bit_rate && params.size)
                                            params.bit_rate = track.dataSize() * 8 * params.block * params.frequency / params.size;
...
#else
                                        Int parts = 0;
                                        Mems<U16> *part_packet_sizes = null;
                                        if (track.parts.elms()) {
                                            aac->parts.setNum(track.parts.elms()); // reserve memory because most likely this size we're going to get
                                            FREPA(track.parts) {
                                                C MP4::Track::Part &part = track.parts[i];
                                                if (part.frames) {
                                                    if (data_pos != part.pos || !parts) {
                                                        AAC::Part &aac_part = aac->parts(parts++);
                                                        aac_part.pos = data_pos = part.pos;
                                                        part_packet_sizes = &aac_part.packet_sizes;
                                                    }
                                                    Int first = part_packet_sizes->addNum(part.frames);
                                                    if (part.frame_sizes.elms()) {
                                                        REPA(part.frame_sizes)
                                                        data_pos += ((*part_packet_sizes)[first + i] = part.frame_sizes[i]);
                                                    } else {
                                                        for (Int i = first; i < part_packet_sizes->elms(); i++)
                                                            (*part_packet_sizes)[i] = part.frame_size;
                                                        data_pos += part.frames * part.frame_size;
                                                    }
                                                }
                                            }
                                        } else if (track.stsc.elms()) {
                                                ...
                                                if (chunk_frames > 0)
                                                    for (; chunk_i < next.first_chunk; chunk_i++) {
                                                        if (Int add_frames = Min(total_frames - cur_frame, chunk_frames)) {
                                                            ...
                                                            if (data_pos != chunk_offset || !parts) {
                                                                AAC::Part &aac_part = aac->parts(parts++);
                                                                aac_part.pos = data_pos = chunk_offset;
                                                                part_packet_sizes = &aac_part.packet_sizes;
                                                            }
                                                            Int first = part_packet_sizes->addNum(add_frames);
                                                            if (track.frame_sizes.elms()) {
                                                                FREP(add_frames)
                                                                data_pos += ((*part_packet_sizes)[first + i] = track.frame_sizes[cur_frame++]);
                                                            } else {
                                                                REP(add_frames)
                                                                (*part_packet_sizes)[first + i] = track.frame_size;
                                            ...
                                            if (chunk_frames > 0)
                                                for (chunk_i--; chunk_i < track.chunk_offset.elms(); chunk_i++) // first convert to 0-based indexes
                                                    if (Int add_frames = Min(total_frames - cur_frame, chunk_frames)) {
...
// SOUND STREAM
/******************************************************************************/
Flt SoundStream::Params::length() C {
    Int div = block * frequency;
    return div ? Dbl(size) / div : 0;
} // use Dbl because 'size' can be huge (more than INT_MAX)
/******************************************************************************/
void SoundStream::del() {
    switch (_codec) {
#if SUPPORT_FLAC
    case SOUND_FLAC:
        if (FLAC *&flac = (FLAC *&)_extra)
            Delete(flac);
        break;
#endif
#if SUPPORT_VORBIS
    case SOUND_OGG_VORBIS:
        if (OggVorbis_File *&vorbis = (OggVorbis_File *&)_extra) {
            ov_clear(vorbis);
            Free(vorbis);
        }
        break;
    case SOUND_WEBM_VORBIS:
        if (WebmVorbisFile *&vorbis = (WebmVorbisFile *&)_extra)
            Delete(vorbis);
        break;
    case SOUND_SND_VORBIS:
        if (SndVorbisFile *&vorbis = (SndVorbisFile *&)_extra)
            Delete(vorbis);
        break;
#endif
#if SUPPORT_OPUS
    case SOUND_OGG_OPUS:
        if (OggOpusFile *&opus = (OggOpusFile *&)_extra) {
            op_free(opus);
            opus = null;
        }
        break;
    case SOUND_WEBM_OPUS:
        if (WebmOpusFile *&opus = (WebmOpusFile *&)_extra)
            Delete(opus);
        break;
    case SOUND_SND_OPUS:
        if (SndOpusFile *&opus = (SndOpusFile *&)_extra)
            Delete(opus);
        break;
#endif
#if SUPPORT_MP3
    case SOUND_MP3:
        if (MP3 *&mp3 = (MP3 *&)_extra)
            Delete(mp3);
        break;
#endif
#if SUPPORT_AAC
    case SOUND_AAC:
        if (AAC *&aac = (AAC *&)_extra)
            Delete(aac);
        break;
#endif
    }
...
Bool SoundStream::create(C Str &name) // !! warning: set T._codec as last thing because it indicates that stream is loaded !!
{
    ...
    if (open(name)) {
        ...
        case CC4_SND: // Esenthel Sound
        {
            _f.pos(0);
            SND_CODEC codec;
            if (LoadSndHeader(_f, _par, codec))
                switch (codec) {
                case SND_RAW_16:
                    _raw_ofs = _f.pos();
                    _codec = SOUND_WAV;
                    return true; // set '_codec' as the last thing

#if SUPPORT_VORBIS
                case SND_VORBIS: {
                    SndVorbisFile temp;
                    if (temp.init(T)) {
                        SndVorbisFile *extra = new SndVorbisFile;
                        Swap(*extra, temp);
                        _extra = extra;
                        _codec = SOUND_SND_VORBIS;
                        return true; // set '_codec' as the last thing
                    }
                } break;
#endif
                ...
                case SND_OPUS: {
                    SndOpusFile temp;
                    if (temp.init(T)) {
                        SndOpusFile *extra = new SndOpusFile;
                        Swap(*extra, temp);
                        _extra = extra;
                        _codec = SOUND_SND_OPUS;
                        return true; // set '_codec' as the last thing
                    }
                } break;
#endif
        ...
        {
            _f.pos(0);
            switch (LoadWavHeader(_f, _par)) // reset file position after previous attempt
            {
            case SOUND_MP3:
                goto load_mp3;
            case SOUND_WAV:
...
#if SUPPORT_VORBIS
            _f.pos(0);
            if (_extra = LoadOggVorbisHeader(_f, _par)) {
                _codec = SOUND_OGG_VORBIS;
                return true;
            } // reset file position after previous attempt and set '_codec' as the last thing
#endif

#if SUPPORT_OPUS
            _f.pos(0);
            if (_extra = LoadOggOpusHeader(_f, _par)) {
                _codec = SOUND_OGG_OPUS;
                return true;
            } // reset file position after previous attempt and set '_codec' as the last thing
#endif
        } break;
        ...
        case 0xA3DF451A: // WebM Vorbis and Opus
        {
            ...
            _f.pos(0);
            SOUND_CODEC codec;
            if (_extra = LoadWebmHeader(_f, _par, codec)) {
                _codec = codec;
                return true;
            } // reset file position after previous attempt and set '_codec' as the last thing
#endif
...
#if SUPPORT_FLAC
            _f.pos(0);
            if (_extra = LoadFLACHeader(_f, _par)) {
                _codec = SOUND_FLAC;
                return true;
            } // reset file position after previous attempt and set '_codec' as the last thing
#endif
        ...
        default: {
            ...
            {
                _f.resetOK().pos(4);
                if (_f.getUInt() == CC4('f', 't', 'y', 'p')) {
                    _f.pos(0);
                    if (_extra = LoadAACHeader(_f, _par)) {
                        _codec = SOUND_AAC;
                        return true;
                    } // reset file position after previous attempt and set '_codec' as the last thing
                }
            ...
            load_mp3:;
#if SUPPORT_MP3
                if (_extra = LoadMP3Header(_f, _par)) {
                    _codec = SOUND_MP3;
                    return true;
                } // set '_codec' as the last thing
#endif
            }
        } break;
        }
...
Bool SoundStream::create(SoundCallback &callback) {
    del();
    Byte bits = 0, channels = 0;
    Int frequency = 0, bit_rate = -1;
    Long size = -1;
...
Bool SoundStream::pos(Long pos) {
    ...
    switch (_codec) {
    case SOUND_WAV:
        if (pos >= 0 && _f.pos(pos + _raw_ofs))
            goto ok;
        break;
#if SUPPORT_FLAC
    case SOUND_FLAC:
        if (((FLAC *)_extra)->pos(pos / _par.block))
            goto ok;
        break;
#endif
#if SUPPORT_VORBIS
    case SOUND_OGG_VORBIS:
        if (!ov_pcm_seek((OggVorbis_File *)_extra, pos / _par.block))
            goto ok;
        break;
    case SOUND_WEBM_VORBIS:
        if (((WebmVorbisFile *)_extra)->pos(pos))
            goto ok;
        break;
    case SOUND_SND_VORBIS:
        if (((SndVorbisFile *)_extra)->pos(pos))
            goto ok;
        break;
#endif
#if SUPPORT_OPUS
    case SOUND_OGG_OPUS:
        if (!op_pcm_seek((OggOpusFile *)_extra, pos / _par.block))
            goto ok;
        break;
    case SOUND_WEBM_OPUS:
        if (((WebmOpusFile *)_extra)->pos(pos))
            goto ok;
        break;
    case SOUND_SND_OPUS:
        if (((SndOpusFile *)_extra)->pos(pos))
            goto ok;
        break;
#endif
#if SUPPORT_MP3
    case SOUND_MP3:
        if (((MP3 *)_extra)->pos(pos / _par.block))
            goto ok;
        break;
#endif
#if SUPPORT_AAC
    case SOUND_AAC:
        if (((AAC *)_extra)->pos(pos))
            goto ok;
        break;
#endif
    case SOUND_DYNAMIC:
        if (_callback->raw(pos))
            goto ok;
        break;
    }
    return false;
...
Int SoundStream::set(Ptr data, Int size) {
    switch (_codec) {
    case SOUND_WAV:
        size = _f.getReturnSize(data, size);
        break;
#if SUPPORT_FLAC
    case SOUND_FLAC:
        size = ((FLAC *)_extra)->set(data, size);
        break;
#endif
#if SUPPORT_VORBIS
    case SOUND_OGG_VORBIS:
        size = ov_read((OggVorbis_File *)_extra, (char *)data, size, 0, 2, 1, null);
        break;
    case SOUND_WEBM_VORBIS:
        size = ((WebmVorbisFile *)_extra)->set(data, Min(size, left()));
        break; // 'WebmVorbisFile' may generate some extra data at the end when seeking, so we need to limit what we can return to skip this padding
    case SOUND_SND_VORBIS:
        size = ((SndVorbisFile *)_extra)->set(data, size);
        break;
#endif
#if SUPPORT_OPUS
    case SOUND_OGG_OPUS: {
        ...
        default:
            ...
            break;
        case 2:
            size = op_read_stereo(opus, (opus_int16 *)data, size / SIZE(opus_int16)) * _par.block;
            break;
        case 1: {
    ...
    case SOUND_WEBM_OPUS:
        size = ((WebmOpusFile *)_extra)->set(data, size);
        break;
    case SOUND_SND_OPUS:
        size = ((SndOpusFile *)_extra)->set(data, Min(size, left()));
        break; // 'SndOpusEncoder' may have introduced some zero padding which would result in extra data at the end, so we need to limit what we can return to skip this padding
#endif
#if SUPPORT_MP3
    case SOUND_MP3:
        size = ((MP3 *)_extra)->set(data, size);
        break;
#endif
#if SUPPORT_AAC
    case SOUND_AAC:
        size = ((AAC *)_extra)->set(data, size);
        break;
#endif
    case SOUND_DYNAMIC:
        size = _callback->set(data, size);
        break;
    default:
        size = 0;
        break;
...
#if SUPPORT_OPUS
    case SOUND_WEBM_OPUS:
        ((WebmOpusFile *)_extra)->extra_frames = OPUS_FRAME_HISTORY_FAST;
        break;
    case SOUND_SND_OPUS:
        ((SndOpusFile *)_extra)->extra_frames = OPUS_FRAME_HISTORY_FAST;
        break;
#endif
    }
}
...
#if SUPPORT_VORBIS
    case SOUND_OGG_VORBIS:
        ((OggVorbis_File *)_extra)->datasource = &_f;
        break;
    case SOUND_WEBM_VORBIS:
        ((WebmVorbisFile *)_extra)->file = &_f;
        break;
    case SOUND_SND_VORBIS:
        ((SndVorbisFile *)_extra)->file = &_f;
        break;
#endif
#if SUPPORT_OPUS
    case SOUND_OGG_OPUS:
        ((OggOpusFile *)_extra)->stream = &_f;
        break;
    case SOUND_WEBM_OPUS:
        ((WebmOpusFile *)_extra)->file = &_f;
        break;
    case SOUND_SND_OPUS:
        ((SndOpusFile *)_extra)->file = &_f;
        break;
#endif
#if SUPPORT_MP3
    case SOUND_MP3:
...
/******************************************************************************/
void SoundHeader::zero() {
    bytes = bits = channels = 0;
    frequency = bit_rate = 0;
    size = 0;
    length = 0;
    codec = SOUND_NONE;
}
...
Int OpusFrequency(Int freq)                                        // !! do not change in the future, because old 'SndOpusFile' will fail !!
{
    for (Int i = Elms(OpusFreqs); --i > 0;) {
        ...
        if (freq >= avg)
            return OpusFreqs[i]; // yes use >= instead of >, because this allows precise mapping if 'OpusFreqs' had 48000-2, 48000-1, 48000, 48000+1, 48000+2
    }
    return OpusFreqs[0];
}
/******************************************************************************/
...
#if SUPPORT_OPUS_ENC
    if (_encoder) {
        opus_encoder_destroy((OE)_encoder);
        _encoder = null;
        _channels = _frame_samples = 0;
        _data.del();
    }
...
#if SUPPORT_OPUS_ENC
    if (_encoder)
        opus_encoder_ctl((OE)_encoder, OPUS_GET_SAMPLE_RATE(&sample_rate));
#endif
    return sample_rate;
}
...
#if SUPPORT_OPUS_ENC
    if (_encoder)
        opus_encoder_ctl((OE)_encoder, OPUS_GET_BITRATE(&bit_rate));
#endif
    return bit_rate;
}
...
#if SUPPORT_OPUS_ENC
    if (_encoder)
        opus_encoder_ctl((OE)_encoder, OPUS_SET_BITRATE(Mid(bit_rate, 500, 512000))); // limits are described in 'OPUS_SET_BITRATE' header comments
#endif
    return T;
}
...
#if SUPPORT_OPUS_ENC
    if (_encoder)
        opus_encoder_ctl((OE)_encoder, OPUS_GET_VBR(&vbr));
#endif
    return vbr == 1;
}
...
#if SUPPORT_OPUS_ENC
    if (_encoder)
        opus_encoder_ctl((OE)_encoder, OPUS_SET_VBR(on ? 1 : 0));
#endif
    return T;
}
...
#if SUPPORT_OPUS_ENC
    if (_encoder)
        opus_encoder_ctl((OE)_encoder, OPUS_GET_COMPLEXITY(&complexity));
#endif
...
#if SUPPORT_OPUS_ENC
    if (_encoder)
        opus_encoder_ctl((OE)_encoder, OPUS_SET_COMPLEXITY(Mid(RoundPos(complexity * 10), 0, 10))); // range of 0..10 described in 'OPUS_SET_COMPLEXITY' header comments
#endif
    return T;
}
...
#if SUPPORT_OPUS_ENC
    if (_encoder)
        opus_encoder_ctl((OE)_encoder, OPUS_GET_LOOKAHEAD(&delay));
#endif
    return delay;
}
#endif
...
Bool OpusEncoder::flush(Int &flushed_samples, MemPtr<Byte> compressed_data, MemPtr<Int> packet_sizes) // !! has to be called only once, because it always adds zero padding for delay !!
{
    ...
    if (_encoder) {
     ...
     _data.addNumZero(delay()*block); // as zero bytes
        ...
        FREP(delay) {
            CopyFast(dest, _last_sample, block);
            dest += block;
        }
#endif
        ...
        for (Int pos = 0, left = _data.elms(); left > 0;) {
         ...
         if( left_samples<frame_samples) // if we want to write less samples than encoder frame samples, this check is important, without it we could request a bigger frame size than encoder's, which could result in fewer frames than expected
         {
            ...
            if (left < frame_raw)
                _data.addNumZero(frame_raw - left); // if what we have is not enough to fit in frame then padd with zeros, here it's actually better to use zeroes than '_last_sample' because it will give more accurate results
...
#if SUPPORT_OPUS
    if (_decoder) {
        opus_decoder_destroy((OD)_decoder);
        _decoder = null;
        _channels = 0;
    }
#endif
...
#if SUPPORT_OPUS
    if (_decoder)
        opus_decoder_ctl((OD)_decoder, OPUS_RESET_STATE);
#endif
}
...
#if SUPPORT_OPUS
    if (_decoder)
        opus_decoder_ctl((OD)_decoder, OPUS_GET_SAMPLE_RATE(&sample_rate));
#endif
    return sample_rate;
}
    ...
    Bool encodeEx(CPtr data, Int size) {
        if (f) {
            ...
            if (data ? !oe.encode(data, size, compressed_data, packet_sizes)
                     : !oe.flush(flushed_samples, compressed_data, packet_sizes))
                return false;
            Byte *save_data = compressed_data.data();
            if (!data && !packet_sizes.elms())
                packet_sizes.add(0);                                  // add a dummy packet if flushing but there's no audio
...
Bool OggOpusEncoder::create(File &f, Int channels, Int bit_rate, Bool vbr) {
    ...
    if (((_OggOpusEncoder *)_encoder)->init(f, channels, bit_rate, vbr))
        return true;
    Delete((_OggOpusEncoder *&)_encoder);
#endif
    return false;
...
void SndOpusEncoder::zero() {
    _f = null;
    _packet_sizes_pos = 0;
    _packet_sizes_expected = 0;
    _frequency = 0;
}
SndOpusEncoder &SndOpusEncoder::del() {
    finish();
    return T;
}
Bool SndOpusEncoder::create(File &f, Long samples, Int frequency, Int channels, Int bit_rate, Bool vbr) {
    ...
    if (samples >= 0 && frequency >= 0 && _encoder.create(channels, false, frequency) && SaveSndHeader(f, SND_OPUS, channels, frequency, samples)) {
        ...
        bitRate(bit_rate); // set bit rate after frequency
        _encoder.vbr(vbr);
        Int delay = _encoder.delay();
        f.cmpUIntV(delay);
...
Bool SndOpusEncoder::encode(CPtr data, Int size) {
    if (_f) {
        ...
        for (Byte *d = (Byte *)data; size > 0;) // encode in chunks to avoid big memory allocation inside 'OpusEncoder' for encoded data
        {
            ...
            if (!_encoder.encode(d, s, compressed_data, packet_sizes)) {
                del();
                return false;
            }
...
Int SndOpusEncoder::bitRate() C {
    if (Int enc_freq = _encoder.frequency())
        return DivRound(ULong(_encoder.bitRate()) * frequency(), (ULong)enc_freq);
    return 0;
}
/******************************************************************************/
// SND RAW ENCODER
...
void SndRawEncoder::zero() {
    _f = null;
    _block = 0;
    _samples_pos = -1;
}
SndRawEncoder &SndRawEncoder::del() {
    finish();
    return T;
}
Bool SndRawEncoder::create(File &f, Int frequency, Int channels, Long samples) {
    ...
    {
        if (samples < 0)
            _samples_pos = f.pos(); // if unknown then remember position to write later
...
Bool SndRawEncoder::finish() {
    ...
    if (_f) {
        ...
        if (_samples_pos >= 0) // need to write sample count
        {
            ok = false;
            Long pos = _f->pos();
            if (_f->pos(_samples_pos - MaxCmpULongVSize)) {
...
void WavEncoder::zero() {
    _f = null;
    _size_pos = -1;
}
WavEncoder &WavEncoder::del() {
    finish();
    return T;
}
Bool WavEncoder::create(File &f, Int bits, Int frequency, Int channels, Long samples) {
    ...
    if (SaveWavHeader(f, bits, channels, frequency, samples * channels * bits / 8)) {
        if (samples < 0)
            _size_pos = f.pos(); // if unknown then remember position to write later
        _f = &f;
        return f.ok();
    }
    return false;
...
Bool WavEncoder::finish() {
    ...
    if (_f) {
        ...
        if (_size_pos >= 0) // need to write sample count
        {
            ok = false;
            Long pos = _f->pos();
            if (_f->pos(_size_pos - 44 + 4)) {
                UInt size = pos - _size_pos;
                _f->putUInt(size + 36);
                if (_f->pos(_size_pos - 4))
                    _f->putUInt(size);
                ok = (_f->pos(pos) && _f->ok());
            }
        }
