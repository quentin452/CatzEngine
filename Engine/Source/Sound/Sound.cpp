...
Flt SoundMinVolume() { return SoundMinVol; }
void SoundMinVolume(Flt volume) { SoundMinVol = Sat(volume); }
Int SoundMaxConcurrent() { return SoundMaxConc; }
void SoundMaxConcurrent(Int max) { SoundMaxConc = max; }
Int SoundMaxThreads() { return SoundMaxThread; }
void SoundMaxThreads(Int max) { SoundMaxThread = max; }
/******************************************************************************/
// SOUND VOLUME CONTROL
/******************************************************************************/
SoundVolumeClass::SoundVolumeClass() { REPAO(_v) = 1; }
void SoundVolumeClass::fx(Flt v) {
    SAT(v);
    if (_v[VOLUME_FX] != v) {
        _v[VOLUME_FX] = v;
        update();
...
void SoundVolumeClass::music(Flt v) {
    SAT(v);
    if (_v[VOLUME_MUSIC] != v) {
        _v[VOLUME_MUSIC] = v;
        update();
    }
}
void SoundVolumeClass::ambient(Flt v) {
    SAT(v);
    if (_v[VOLUME_AMBIENT] != v) {
        _v[VOLUME_AMBIENT] = v;
        update();
    }
}
void SoundVolumeClass::voice(Flt v) {
    SAT(v);
    if (_v[VOLUME_VOICE] != v) {
        _v[VOLUME_VOICE] = v;
        update();
    }
}
void SoundVolumeClass::ui(Flt v) {
    SAT(v);
    if (_v[VOLUME_UI] != v) {
        _v[VOLUME_UI] = v;
        update();
...
void SoundVolumeClass::global(Flt v) {
    SAT(v);
    if (_v[VOLUME_NUM] != v) {
        _v[VOLUME_NUM] = v;
        update();
...
void _Sound::del() // !! requires 'SoundAPILock' !!
{
    ...
    if (stream_loaded)
        _stream.del();
    else // if 'stream_loaded' then we can access it on this thread
    {    // otherwise we need to use 'SoundStreamLock' in case it's being loaded on secondary thread
        SyncLocker locker(SoundStreamLock);
        _stream.del(); // delete always to clear '_stream._callback'
    }

    // don't delete the '_name' because we may be accessing it on a secondary thread (for both get 'name' method and stream load)
    zero();
    if (noRef())
        flag |= SOUND_REMOVE; // if there are no references to this sound then we can remove it, no need for 'AtomicOr' since it won't be processed by other thread
}
/******************************************************************************/
void _Sound::init(C Str &name, SoundCallback *call, Bool is3D, VOLUME_GROUP volume_group) // !! warning: 'name' can be 'T._name' !!
{
    ...
    setActualSpeed();                                                    // !! call this after setting 'volume_group' because that affects the result !!
    setActualRange();
}
Bool _Sound::init(C _Sound &src) {
    ...
    if (src.deleted)
        return false; // !! it is important to check if the sound got deleted after copying all parameters, which may mean that some of them were not set fully !!

    if (playing)
        play(); // call this at the end
    return true;
}
/******************************************************************************/
// OPERATIONS
/******************************************************************************/
inline Flt _Sound::actualRange() C { return range() * Listener.range(); }
inline Flt _Sound::actualSpeed() C {
    Flt speed = T._speed;
    if (volume_group != VOLUME_MUSIC && volume_group != VOLUME_AMBIENT && volume_group != VOLUME_UI)
        speed *= Time.speed();
    return speed;
}
inline Flt _Sound::actualVolume() C {
    ...
    case FADE_LINEAR:
        ...
        break;
    case FADE_SQRT:
        volume *= SqrtFast(_fade);
        break; // can use 'SqrtFast' because '_fade' always guaranteed to be 0..1
    }
#if !WINDOWS_OLD
...
void _Sound::setActualRange() { _actual_range = actualRange(); }
void _Sound::setActualSpeed() { _actual_speed = actualSpeed(); }
void _Sound::setActualVolume() { _buffer.volume(actualVolume()); } // !! requires 'SoundAPILock' !!
/******************************************************************************/
void _Sound::stop() {
    _playing = false;
    time(0);
...
SoundStream &_Sound::stream() // !! this may be called on the main thread !!
{
    if (!stream_loaded && !deleted) // don't load if already deleted
    {
        if (_name.is()) {
            SoundStream temp;
            temp.create(_name); // load to temporary first to minimize time needed for Lock (in worst case we'll just load # times on # threads, but most likely this won't happen)
        ...
        } else // for callback we always need to use Lock because we're storing it in '_stream._callback' for simplicity
        {
            SyncLocker locker(SoundStreamLock);
            if (!stream_loaded) {
                if (_stream._callback)
                    _stream.create(*_stream._callback);
                stream_loaded = true;
            }
        }
    }
...
UID _Sound::id() C {
    UID id;
    if (!deleted && DecodeFileName(_name, id))
        return id;
    return UIDZero;
}
...
void _Sound::range(Flt range) {
    MAX(range, 0);
    if (T._range != range) {
        T._range = range;
        setActualRange();
        AtomicOr(flag, SOUND_CHANGED_RANGE);
    }
} // have to 'setActualRange' here already because we need it for 'update' and 'priority'
void _Sound::pos(C Vec &pos) {
    if (T._pos != pos) {
        T._pos = pos;
        AtomicOr(flag, SOUND_CHANGED_POS);
    }
}
void _Sound::vel(C Vec &vel) {
    if (T._vel != vel) {
        T._vel = vel;
        AtomicOr(flag, SOUND_CHANGED_VEL);
    }
}
void _Sound::speed(Flt speed) {
    MAX(speed, 0);
    if (T._speed != speed) {
        T._speed = speed;
        if (!SOUND_SPEED_ON_SOUND_THREAD)
            setActualSpeed();
        AtomicOr(flag, SOUND_CHANGED_SPEED);
    }
}

void _Sound::raw(Long raw) {
    C SoundStream &stream = T.stream();
    if (Long size = stream.size())
        time(raw * (Dbl(stream.length()) / size));
    else
        time(0);
} // use Dbl because index can be huge (more than INT_MAX)
void _Sound::sample(Long sample) {
    C SoundStream &stream = T.stream();
    if (Long samples = stream.samples())
        time(sample * (Dbl(stream.length()) / samples));
    else
        time(0);
} // use Dbl because index can be huge (more than INT_MAX)
Long _Sound::raw() C {
    C SoundStream &stream = T.stream();
    if (Dbl length = stream.length())
        return time() * (stream.size() / length);
    return 0;
} // use Dbl because index can be huge (more than INT_MAX)
Long _Sound::sample() C {
    C SoundStream &stream = T.stream();
    if (Dbl length = stream.length())
        return time() * (stream.samples() / length);
    return 0;
} // use Dbl because index can be huge (more than INT_MAX)
void _Sound::frac(Flt f) { time(f * length()); }
void _Sound::time(Flt t) {
    if (T._time != t) {
        AtomicOr(flag, SOUND_CHANGING_TIME);
        T._time = t;
        AtomicOr(flag, SOUND_CHANGED_TIME);
    }
} // changing time value needs to be surrounded by flags because it gets modified on the sound thread as well
Flt _Sound::frac() C {
    if (Flt l = length())
        return time() / l;
    return 0;
...
void _Sound::preciseRaw(Long raw) {
    if (Int block = _stream.block()) // copy to temp in case the value is changed in another thread
    {
        ...
        if (kill                         // if started killing
            ...
            && raw_pos < _stream.size())
            kill = 0; // and new position is before the end, then cancel killing
    }
}
Long _Sound::preciseRaw() C // !! requires 'SoundAPILock' !!
{
    ...
    if (last_buffer == 0) {
        ...
        if (buffer_pos < buffer_half)
            pos += _buffer._par.size; // if 'buffer_pos' got back to the start due to wrapping, then we need to add by entire buffer (what was wrapped), because 'raw_pos' wasn't changed during wrapping
    }
    ...
    if (loop()) {
        Long size = _stream.size();
        if (size >= 0)
            total_pos = Mod(total_pos, size); // size can be negative for unlimited audio
    }
    return total_pos;
}
...
Flt _Sound::preciseTime() C {
    Int div = _stream.block() * _stream.frequency();
    return div ? Dbl(preciseRaw()) / div : 0;
} // !! requires 'SoundAPILock' !! use Dbl because 'preciseRaw' can be huge (more than INT_MAX)
Flt _Sound::preciseFrac() C {
    Long div = _stream.size();
    return div ? Dbl(preciseRaw()) / div : 0;
} // !! requires 'SoundAPILock' !! use Dbl because 'preciseRaw' can be huge (more than INT_MAX)
/******************************************************************************/
// FADE
...
void _Sound::setBufferData(Byte *buffer, Int size) // no extra care needed
{
    ...
    for (; size > 0;) {
        Long set, clear = 0;
        if (raw_size >= 0) {
            if (_loop && (raw_pos < 0 || raw_pos >= raw_size))
                raw_pos = Mod(raw_pos, raw_size); // adjust 'raw_pos' if it got out of range
            if (raw_pos < 0)
                clear = -raw_pos;
        ...
        if (clear) {
            ...
            set = size;
            if (raw_size >= 0)
                MIN(set, raw_size - raw_pos); // at once process only up to the allowed data range if specified
    ...
    if (SoundDataCallback *callback = _callback)
        callback->data(buffer_start, size_start, stream, raw_pos_start); // first copy to temp var to avoid multi-threading issues
...
Bool _Sound::setBuffer(Byte buffer, Int thread_index) // this manages locking on its own
{
    ...
    SOUND_API_LOCK_COND;
    if (_buffer.lock(buffer ? size : 0, size)) // !! requires 'SoundAPILock' !!
    {
        SOUND_API_LOCK_OFF;
        setBufferData((Byte *)_buffer._lock_data, _buffer._lock_size);
        SOUND_API_LOCK_SET;
        _buffer.unlock(); // !! requires 'SoundAPILock' !!
        return true;
    }
    return false;
#elif XAUDIO
    ...
    Int size = _buffer._par.size;
    if (!downmix)
        size /= 2;                       // we're setting only half of the buffer (don't do this for downmixing, because for that we need initially 2x more)
    Byte *buffer_data, temp[512 * 1024]; // 512 KB
    if (size <= SIZE(temp))
        buffer_data = temp;
    else // if we can fit all data in stack then use it
    {
    ...
    if (_buffer._par.bytes == 3) // convert 24-bit to 16-bit
    {
        ...
        Int samples = size / SIZE(*d);
        FREP(samples) {
            d[i] = *s;
            s = (I16 *)(((Byte *)s) + 3);
        } // advance source by 3 bytes
    }
    if (downmix) {
        size /= 2; // changing from stereo to mono cuts size in half
        if (bit16) {
            I16 *d = (I16 *)buffer_data;
            Int samples = size / SIZE(*d);
            FREP(samples)
            d[i] = (d[i * 2] + d[i * 2 + 1]) / 2;
        } else {
...
/******************************************************************************/
inline Bool _Sound::setNextBuffer(Int thread_index) { // !! Remember that 'last_buffer' can be 0xFF !! however this is called at a stage where it shouldn't be
#if CUSTOM_AUDIO
...
Bool _Sound::testBuffer(Int thread_index) // this manages locking on its own
{
#if DIRECT_SOUND
    SOUND_API_LOCK_COND;
    if ((_buffer.raw() >= _buffer._par.size / 2) == last_buffer) // remember that 'last_buffer' can be 0xFF
    {
        SOUND_API_LOCK_OFF;
        return setNextBuffer(thread_index);
    }
#elif XAUDIO
    if (_buffer._sv) {
        SOUND_API_LOCK_COND;
    ...
    Int processed = 0;
    if (_buffer._source)
        alGetSourcei(_buffer._source, AL_BUFFERS_PROCESSED, &processed); // !! requires 'SoundAPILock' !!
    if (processed > 0) {
        REP(processed) {
            UInt buffer = 0;
            alSourceUnqueueBuffers(_buffer._source, 1, &buffer); // !! requires 'SoundAPILock' !!
            SOUND_API_LOCK_OFF;
            if (buffer == _buffer._buffer[0]) {
                if (!setBuffer(false, thread_index))
                    return false;
            } else if (buffer == _buffer._buffer[1]) {
                if (!setBuffer(true, thread_index))
                    return false;
            } else
        ...
        if (!_buffer.playing())
            _buffer.play(true); // !! requires 'SoundAPILock' !! continue playing if it stopped (for example ran out of buffers - processed==2)
    }
#elif OPEN_SL
...
Bool _Sound::update(Flt dt) // !! requires 'SoundAPILock' !!
{
    ...
    if (playing()) {
        ...
        if (is3D()) {
            ...
            if (dist > _actual_range)
                priority *= _actual_range / dist;
        }

        // add modifiers
        priority += SoundGroupPriority[volume_group];
        if (_buffer_playing)
            priority += SoundPlayPriority; // increase priority for sounds that are already playing
    } else
        priority = -1; // if it's not playing then set priority below zero (zero is lowest possible 'SoundMinVolume' value, so it will never be played)
...
void _Sound::updatePlaying(Int thread_index) {
    { // brackets to avoid compile errors on GCC
...
#if !DEL_BUFFER_WHEN_NOT_PLAYING
        if (!_buffer_playing)
            AtomicOr(T.flag, SOUND_CHANGED_VOLUME | SOUND_CHANGED_SPEED | SOUND_CHANGED_TIME); // if not playing yet then we need to apply volume and speed because they are affected by global parameters, also reset the time because it's modified when sound is paused
#endif
        ...
        if (flag & handled) {
            ...
            if (flag & SOUND_CHANGED_VOLUME)
                setActualVolume(); // !! call this first before 'set3DParams' because CUSTOM_AUDIO needs this !!
#if XAUDIO
            if (_is3D) {
                if (flag & (SOUND_CHANGED_POS | SOUND_CHANGED_VEL | SOUND_CHANGED_ORN | SOUND_CHANGED_RANGE | SOUND_CHANGED_SPEED)) // need to check for SOUND_CHANGED_ORN because of Listener changes
                {
                    if (SOUND_SPEED_ON_SOUND_THREAD && flag & SOUND_CHANGED_SPEED)
                        setActualSpeed(); // !! call this first before 'set3DParams' !!
                ...
                if (flag & SOUND_CHANGED_SPEED) {
                    if (SOUND_SPEED_ON_SOUND_THREAD)
                        setActualSpeed();
                    _buffer.speed(SoundSpeed(_actual_speed));
                }
            }
#elif CUSTOM_AUDIO
            if (_is3D) {
                if (flag & (SOUND_CHANGED_POS | SOUND_CHANGED_VEL | SOUND_CHANGED_ORN | SOUND_CHANGED_RANGE | SOUND_CHANGED_VOLUME | SOUND_CHANGED_SPEED)) // need to check for SOUND_CHANGED_ORN because of Listener changes
                {
                    if (SOUND_SPEED_ON_SOUND_THREAD && flag & SOUND_CHANGED_SPEED)
                        setActualSpeed(); // !! call this first before 'set3DParams' !!
                    _buffer.set3DParams(T, FlagOn(flag, SOUND_CHANGED_POS | SOUND_CHANGED_ORN | SOUND_CHANGED_RANGE | SOUND_CHANGED_VOLUME), FlagOn(flag, SOUND_CHANGED_POS | SOUND_CHANGED_VEL | SOUND_CHANGED_SPEED));
                }
            } else {
                if (flag & SOUND_CHANGED_SPEED) {
                    if (SOUND_SPEED_ON_SOUND_THREAD)
            ...
            if (flag & SOUND_CHANGED_POS)
                _buffer.pos(_pos);
            if (flag & SOUND_CHANGED_VEL)
                _buffer.vel(_vel);
            if (flag & SOUND_CHANGED_RANGE)
                _buffer.range(_actual_range);
            if (flag & SOUND_CHANGED_SPEED) {
                if (SOUND_SPEED_ON_SOUND_THREAD)
                    setActualSpeed();
                _buffer.speed(SoundSpeed(_actual_speed));
        ...
        {
            if (!testBuffer(thread_index))
                goto error;                                           // check if we need to update playback buffers
    ...
    case 0: {
        ...
        if (_name.is()) {
            ...
            if (f.getFast(desc) && f.ok()) {
                ...
                if (FlagOn(flag, SD_PLAYING))
                    play(); // call this last after all parameters have been set
                return SOUND_LOAD_OK;
            }
        } else if (f.ok())
            return SOUND_LOAD_EMPTY;
    } break;
    }
...
Sound &Sound::del() {
    if (sound) {
        if (SoundFunc)
            AtomicOr(sound->flag, SOUND_REMOVE);
        sound = null;
    }
    return T;
}
...
Sound &Sound::_create(C Str &name, SoundCallback *call, Bool is3D, VOLUME_GROUP volume_group) // !! call 'call.del' if not passed down !!
{
    ...
    if ((name.is() || call) && SoundFunc) {
        ...
        sound = &SoundMemx.New();
        sound->init(name, call, is3D, volume_group);
...
Sound &Sound::raw(Long raw) {
    if (sound && SoundFunc)
        sound->raw(raw);
    return T;
}
Sound &Sound::sample(Long sample) {
    if (sound && SoundFunc)
        sound->sample(sample);
    return T;
}
Sound &Sound::time(Flt t) {
    if (sound && SoundFunc)
        sound->time(t);
    return T;
}
Sound &Sound::frac(Flt f) {
    if (sound && SoundFunc)
        sound->frac(f);
    return T;
}
Sound &Sound::loop(Bool loop) {
    if (sound && SoundFunc)
        sound->loop(loop);
    return T;
}
Sound &Sound::volume(Flt volume) {
    if (sound && SoundFunc)
        sound->volume(volume);
    return T;
}
Sound &Sound::speed(Flt speed) {
    if (sound && SoundFunc)
        sound->speed(speed);
    return T;
}
Sound &Sound::range(Flt range) {
    if (sound && SoundFunc)
        sound->range(range);
    return T;
}
Sound &Sound::pos(C Vec &pos) {
    if (sound && SoundFunc)
        sound->pos(pos);
    return T;
}
Sound &Sound::vel(C Vec &vel) {
    if (sound && SoundFunc)
        sound->vel(vel);
    return T;
}

Sound &Sound::callback(SoundDataCallback *callback) {
    if (sound && SoundFunc)
        sound->_callback = callback;
    return T;
}
SoundDataCallback *Sound::callback() C { return (sound && SoundFunc) ? sound->_callback : null; }

Sound &Sound::fadeCurve(FADE_CURVE curve) {
    if (sound && SoundFunc)
        sound->fadeCurve(curve);
    return T;
}
Sound &Sound::fadeInFromSilence(Flt fade_duration) {
    if (sound && SoundFunc)
        sound->fadeInFromSilence(fade_duration);
    return T;
}
Sound &Sound::fadeIn(Flt fade_duration) {
    if (sound && SoundFunc)
        sound->fadeIn(fade_duration);
    return T;
}
Sound &Sound::fadeOut(Flt fade_duration) {
    if (sound && SoundFunc)
...
Bool Sound::load(File &f, CChar *path) {
    ...
    if (SoundFunc)     // can load sounds only with 'SoundMemx' available
    {
        ...
        case SOUND_LOAD_OK:
            return true;
        case SOUND_LOAD_EMPTY:
            del();
            return true;
        default:
            del();
            break; // SOUND_LOAD_ERROR
        }
    }
    return false;
}
...
again:
...
#if XAUDIO
                if (XAudio)
                    XAudio->StopEngine(); // !! requires 'SoundAPILock' !!
#elif CUSTOM_AUDIO
                AudioThread.pause();
#else
    ...
    {                  // use brace for locks
        ...
        Flt dt = time - SoundTime;
        SoundTime = time; // get time that passed since the last update

        SyncLockerEx SoundMemxLocker(SoundMemxLock);
#if !SOUND_API_THREAD_SAFE
        SyncLockerEx SoundAPILocker(SoundAPILock);
        ...
        if (!SoundAPI)
            ...
            max_concurrent = SoundMemx.elms();
        else
            MIN(max_concurrent, SoundMemx.elms()); // needs to be done after 'SoundMemxLock' and after removing sounds from 'SoundMemx'
        Int i = 0;
        for (; i < max_concurrent; i++) // go from the start with highest priority
        {
            _Sound &sound = SoundMemx[i];
            if (sound.priority <= min_vol)
                break;
        ...
        for (; i < SoundMemx.elms(); i++) // all remaining sounds should not be played
        {
            ...
            if (!sound.deleted) {
                if (sound._buffer_playing) {
                    ...
                    ..
#endif
                } else if (sound.playing() && !sound.loop())                                        // update time but only if the sound is not looped, if it's looped then for simplicity we skip this
                {                                                                                   // update time over here because above it is already set from precise value
                    if (SOUND_SPEED_ON_SOUND_THREAD && AtomicGet(sound.flag) & SOUND_CHANGED_SPEED) // check if speed was changed, which affects time updates #SOUND_CHANGED_SPEED
                    {
        ...
        if (SoundMemxPlaying.elms()) {
            ...
            if (threads != SoundThreads.threads())
                SoundThreads.create(false, threads, SoundThread.priority(), "EE.Sound #");
#endif
...
#if OPEN_AL
            if (max_threads > SoundThreadBuffer.elms())
                SoundThreadBuffer.setNum(max_threads);
#endif
            ...
            if (SoundAPILockDo)
                SoundAPILocker.off(); // release from this thread so other threads can capture it
#endif
...
#if !SOUND_API_THREAD_SAFE
            if (SoundAPILockDo)
                SoundAPILocker.on(); // re-enable for this thread because it's needed for calls done below (Listener)
#endif
        }
...
void ShutSound2() {
    SoundThread.stop();
    ResumeSound();
    SoundEvent.on(); // resume to avoid waiting forever, enable event so the 'SoundThread' can wake up immediately
    SoundThread.del();
#if OPEN_AL
...
{
    AtomicOr(SoundPause, SOUND_WANT_PAUSED);
    SoundEvent.on(); // wake up the thread immediately to perform the changes
}
void ResumeSound() {
    AtomicDisable(SoundPause, SOUND_WANT_PAUSED);
    SoundEvent.on(); // wake up the thread immediately to perform the changes
}
/******************************************************************************/
...
void ChangeSoundsSeparate(UInt change) // set for all sounds separately
{
    ...
    SyncLocker SoundMemxLocker(SoundMemxLock);
    REPA(SoundMemx)
    AtomicOr(SoundMemx[i].flag, change);
}
void ChangeSounds(UInt change) // try using global
{
    if (LISTENER_CHANGED)
        AtomicOr(Listener._flag, change); // if we're using one global listener changed flag, then just use it
    else
