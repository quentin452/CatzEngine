...
#endif
#if SUPPORT_KISS
    kiss_fftr_cfg &fft = (kiss_fftr_cfg &)_fft;
    if (fft) {
        ::kiss_fft_free(fft);
        fft = null;
    }
...
Spectrometer &Spectrometer::create(Int resolution) {
    ...
    _image.createSoft(resolution, rows, 1, IMAGE_F32);
    _image.clear();
    return T;
}
/******************************************************************************/
void Spectrometer::data(Ptr data, Int size, C SoundStream &stream, Long raw_pos) {
...
#if SUPPORT_KISS
        if (1)
            samples = NearestPow2(samples); // this is optional (may improve performance)
        else
            samples = Ceil2(samples); // kiss_fftr requires even number of samples
        _fft = kiss_fftr_alloc(samples, 0, null, null);
        _fft_out.setNum((samples / 2 + 1) * 2);
        ASSERT(SIZE(kiss_fft_cpx) == SIZE(Flt) * 2); // *2 because we need 'kiss_fft_cpx' instead of 'Flt' which is 2x bigger
#endif
    ...
    if (!stream.block() || !_samples.elms() || !_image.is())
        return;
   ...
   REPAO(_samples)=Cos(i/Flt(_samples.elms()-1)*Ms.pos().x*10000); sample_pos=_samples.elms();
    ...
    for (Int data_samples = size / stream.block(); data_samples > 0;) {
        ...
        Int copy = Min(_samples.elms() - sample_pos, data_samples);
        data_samples -= copy;
        switch (stream.bytes()) {
        ...
        case 3: // Int24
        {
            ...
            case 2: {
                REP(copy) {
                    *dest++ = (src[0].asInt() + src[1].asInt()) / Flt(-INT24_MIN * 2);
                    src += 2;
                }
                sample_pos += copy;
            } break;
        ...
        if (sample_pos >= _samples.elms()) {
...
#if SUPPORT_FFTW
            REPAO(fftw_in) = _samples[i];
            fftw_execute((fftw_plan)_fft);
            REPA(fftw_out) {
            ...
            Int end = fftw_out.elms(), y = Unsigned(sample / _samples.elms()) % _image.h();
            sample += _samples.elms();
            REP(_image.w()) {
                Int start = i * fftw_out.elms() / _image.w();
                Flt value = fftw_out[start].c[0];
                for (Int i = start + 1; i < end; i++)
                    MAX(value, fftw_out[i].c[0]); // always take at least one value, in case start==end
                value = Log2(value + 1);
            ...
            Int end = out_elms, y = Unsigned(sample / _samples.elms()) % _image.h();
            sample += _samples.elms();
            REP(_image.w()) {
                Int start = i * out_elms / _image.w();
                Flt value = out[start].r;
                for (Int i = start + 1; i < end; i++)
                    MAX(value, out[i].r); // always take at least one value, in case start==end
                value = Log2(value + 1);
                // value=BlendSmoothCube(i*0.2f - 26*Flt(y)/_image.h()); // can be used for testing
                _image.pixF(i, y) = value;
                end = start;
            ...
            REP(_image.w()) {
                Int start = i * out_elms / _image.w();
                Flt value = _fft_out[start];
                for (Int i = start + 1; i < end; i++)
                    MAX(value, _fft_out[i]); // always take at least one value, in case start==end
                value = Log2(value + 1);
                // value=BlendSmoothCube(i*0.2f - 26*Flt(y)/_image.h()); // can be used for testing
                _image.pixF(i, y) = value;
                end = start;
        ...
        if (Int s = _samples.elms())
            if (Int h = _image.h()) {
                ...
                if (filter != FILTER_NONE)
                    yf -= 0.5f; // need to offset by -0.5, because when we're at half row position (y=0.5) then we need to get exactly values of the first row, the same as y=0 with no interpolation, and in linear interpolation we will get it with y=0, this is because first row collects samples from start of the song, with window fading out at start and end, making most significant data coming from the middle of the window
                else
                    y = Unsigned(TruncL(yf)) % h;
                if (meter_elms == resolution())
                    switch (filter) {
                    case FILTER_NONE:
                        CopyN(meter, &_image.pixF(0, y), meter_elms);
                        break;
                    case FILTER_LINEAR:
                        REP(meter_elms)
                        meter[i] = _image.pixelFLinear(i, yf, false);
                        break;
                    default:
                        REP(meter_elms)
                        meter[i] = _image.pixelFCubicFast(i, yf, false);
                        break; // FILTER_CUBIC* and others (actually use CubicFast because it uses fewer samples)
                    }
                else {
                    if (filter == FILTER_NONE)
                        yf = y;
                    ...
                    case FILTER_LINEAR:
                        REP(meter_elms)
                        meter[i] = _image.pixelFLinear(i * mul, yf, false);
                        break;
                    default:
                        REP(meter_elms)
                        meter[i] = _image.pixelFCubicFast(i * mul, yf, false);
                        break; // FILTER_CUBIC* and others (actually use CubicFast because it uses fewer samples)
                    }
                }
                return;
