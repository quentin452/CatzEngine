...
void MakeLinuxLibs() {
    ...
    if (Options.physics() == PHYS_ENGINE_PHYSX)
        lib_paths.add(ThirdPartyLibsPath + "PhysX/physx/bin/linux.clang/release/obj"); // PhysX (because it's stored inside a separate folder and should be linked only if PhysX is selected as physics engine)
...
static Bool UniversalEnginePakFilter(C Str &full) {
    ...
    if (!SUPPORT_MLAA && base == "MLAA Area.img")
        return false;
    if (EqualPath(name, "Shader/GL") || EqualPath(name, "Shader/GL SPIR-V") || EqualPath(name, "Emoji/ETC2.pak") || base == "World Editor")
        return false;
    return true;
}
static Bool MobileEnginePakFilter(C Str &full) {
    ...
    if (!SUPPORT_MLAA && base == "MLAA Area.img")
        return false;
    if (EqualPath(name, "Shader/4") || EqualPath(name, "Emoji/BC7.pak") || base == "World Editor")
        return false;
    return true;
}
...
void CreateEnginePak() {
    ...
    Pak engine_pak;
    if (!engine_pak.create(EngineDataPath, engine_pak_name, PAK_SHORTEN, null, null, COMPRESS_ZSTD, 255, DesktopEnginePakFilter, ComressMode))
    ...
    if (!engine_pak.create(EngineDataPath, engine_pak_name, PAK_SHORTEN, null, null, COMPRESS_ZSTD, 255, UniversalEnginePakFilter, ComressMode))
    ...
    FCreateDir(GetPath(engine_pak_name));
    if (!engine_pak.create(EngineDataPath, engine_pak_name, PAK_SHORTEN, null, null, COMPRESS_ZSTD, 255, MobileEnginePakFilter, ComressMode))
    ...
    FCreateDir(GetPath(engine_pak_name));
    if (!engine_pak.create(EngineDataPath, engine_pak_name, PAK_SHORTEN, null, null, COMPRESS_LZ4, 255, MobileEnginePakFilter))
...
Bool FilterEditorPak(C Str &name) {
    ...
    if (GetBase(rel) == "local.properties" && EqualPath(GetPath(GetPath(rel)), "Code/Android"))
        return false;                             // skip all "Code/Android/*/local.properties" as they're created manually by the Editor
...
void CreateEditorPak() {
    ...
    Pak editor_pak;
    if (!editor_pak.create(EditorDataPath, editor_pak_name, PAK_SHORTEN, null, null, COMPRESS_NONE, 9, FilterEditorPak))
...
void CompileVS(C Str &project, C Str &config, C Str &platform, void func() = null, void pre() = null) {
    ...
    if (Contains(platform, "Web", false, WHOLE_WORD_STRICT)) // WEB requires VS 2010
    {
        ...
        Str msbuild = MSBuildPath(Options.vs_path().is() ? Options.vs_path() : VSPath);
...
void CompileEngineNintendoSwitch() {
    if (CheckNintendoSwitch())
        CompileVS(EnginePath + VSEngineProject, "Release DX11", "5) Nintendo Switch", CopyEngineNintendoSwitch);
}
...
void CompileEngineDebugNintendoSwitch() {
    if (CheckNintendoSwitch())
        CompileVS(EnginePath + VSEngineProject, "Debug DX11", "5) Nintendo Switch", CopyEngineDebugNintendoSwitch);
}
...
Bool ExtractTry(File &f, C ZipFile &zip_file, C Str &dest, DateTime *utc) {
    if (f.pos(zip_file.offset)) {
        ...
        File temp;
        return DecompressRaw(f, temp, COMPRESS_ZLIB, zip_file.compressed_size, zip_file.uncompressed_size, true) && temp.pos(0) && SafeOverwrite(temp, dest, utc);
    }
    return false;
}
...
void InitPre() {
    ...
    for (Str path = GetPath(App.exe());; path = GetPath(path)) {
        ...
        if (FExistSystem(path + "/Data") && FExistSystem(path + "/Editor Data") && FExistSystem(path + "/Editor") && FExistSystem(path + "/Engine")) {
...
#endif
    FREPA(TaskBases)
    Tasks.New().create(TaskBases[i]);
    FREPA(CustomTasks)
    Tasks.New().create(CustomTasks[i]);
...
void Draw() {
    ...
    ts.size = 0.055;
    ts.align.set(1, -1);
    D.text(ts, -D.w() + 0.01, D.h() - 0.01, S + "Busy:" + build_threads.queued() + ", Queued:" + Todo.elms());
    Gui.draw();
}
/******************************************************************************/
