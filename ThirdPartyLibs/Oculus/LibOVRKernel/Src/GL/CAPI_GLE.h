...
// This file provides functionality similar to a reduced version of GLEW, plus
// some additional functionality that's useful to us, such as function hooking.
...
//
// - You #include this header instead of gl.h, glext.h, wglext.h (Windows),
// gl3.h (Apple), gl3ext.h (Apple), glx.h (Unix), and glxext.h (Unix).
//   Currently you still would #include <Windows.h> for the base wgl functions
//   on Windows and OpenGL.h or NSOpenGL for the base Apple cgl functions.
//
// - You call OpenGL functions just like you would if you were directly using
// OpenGL
//   headers and declarations. The difference is that this module automatically
//   loads extensions on init and so you should never need to use
//   GetProcAddress, wglGetProcAddress, etc.
//
// - OpenGL 1.1 functions can be called unilaterally without checking if they
// are present,
//   as it's assumed they are always present.
//
// - In order to use an OpenGL 1.2 or later function you can check the
// GLEContext::WholeVersion
//   variable to tell what version of OpenGL is present and active. Example
//   usage:
//       if(GLEContext::GetCurrentContext()->WholeVersion >= 302) // If
//       OpenGL 3.2 or later...
//
// - In order to use an OpenGL extension, you can check the GLE_ helper macro
// that exists for each
//   extension. For example, in order to check of the KHR_debug is present you
//   could do this:
//        if(GLE_KHR_debug) ...
//   You cannot check for the presence of extensions by testing the function
//   pointer, because when hooking is enabled then we aren't using function
//   pointers and thus all functions will look like they are present.
//
// - You can test if the OpenGL implementation is OpenGL ES by checking the
// GLEContext IsGLES
//   member variable. For example: if(GLEContext::GetCurrentContext()->IsGLES)
//   ...
//
// - You can test if the OpenGL implementation is a core profile ES by checking
// the GLEContext IsCoreProfile
//   member variable. For example:
//   if(GLEContext::GetCurrentContext()->IsCoreProfile) ...
//
///////////////////////////////////////////////////////////////////////////////
...
//
// For an example of how to do this, search the source files for all cases of
// KHR_Debug and just copy the things that it does but for your new extension.
//
//     1) Add the appropriate extension declaration to CAPI_GLE_GL.h, preferably
//     by
//        copying it from the standard header file it normally comes from. If
//        it's platform-specific (e.g. a Windows wgl function) then make sure
//        it's declared within the given platform section. Note that there are
//        potentially #defines, typedefs, function typedefs, and function
//        #defines. There is always a GLE_ macro declared which lets the user
//        know at runtime whether the extension is present. Note that entries
//        are alphabetically sorted in these files. e.g.  #ifndef GL_KHR_debug
//                  #define GL_KHR_debug 1
//                  #define GL_CONTEXT_FLAG_DEBUG_BIT 0x00000002 etc.
//                  typedef void (GLAPIENTRY * PFNGLPOPDEBUGGROUPPROC) ();
//                  #define glPopDebugGroup
//                  GLEGetCurrentFunction(glPopDebugGroup) #define
//                  GLE_KHR_DebugGLEGetCurrentVariable(gl_KHR_debug)
//              #endif etc.
//
//     2) Add a hook function for in the hook section of the GLEContext class in
//     this header,
//        ideally in the same order it's declared in the CAPI_GLE_GL.h so it's
//        easily readable. e.g. void glDebugMessageControl_Hook(GLenum source,
//        GLenum type, GLenum severity, GLsizei count, const GLuint* ids,
//        GLboolean enabled); etc.
//
//     3) Add a declaration for each interface function to the GLEContext class
//     in this header.
//        e.g. PFNGLDEBUGMESSAGECALLBACKPROC glDebugMessageCallback_Impl; etc.
//
//     4) Add code to GLEContext::InitExtensionLoad to load the function
//     pointer.
//        e.g. GLELoadProc(glDebugMessageCallback_Impl, glDebugMessageCallback);
//        etc.
//
//     5) Add code to GLEContext::InitExtensionSupport to detect the extension
//     support.
//        On Mac, core profile functions aren't identified as extensions and so
//        in addition to detecting them you need to unilaterally set them as
//        available when using 3.2+ by adding them to the section at the bottom
//        of InitExtensionSupport. e.g. { gl_KHR_debug, "GL_KHR_debug" }, etc.
//
//     6) Implement the GLEContext hook function(s) you declared.
//        e.g.  void OVR::GLEContext::glDebugMessageControl_Hook(GLenum source,
//        GLenum type, GLenum severity, GLsizei count, const GLuint* ids,
//        GLboolean enabled)
//              {
//                 if(glDebugMessageControl_Impl)
//                    glDebugMessageControl_Impl(source, type, severity, count,
//                    ids, enabled);
//                 PostHook();
//              }
//
// In order to test this, build with GLE_HOOKING_ENABLED defined and not
// defined.
//
// Note that if the extension is a WGL-, GLX-, or CGL-specific extension, they
// are handled like above but are in their own section below the section for
// regular OpenGL extensions.
//
// In some cases the given interface may already be present by currently
// commented out, in which case you can simply un-comment it to enable it.
///////////////////////////////////////////////////////////////////////////////

namespace OVR {
// Generic OpenGL GetProcAddress function interface. Maps to platform-specific
// functionality internally. On Windows this is equivalent to wglGetProcAddress
// as opposed to global GetProcAddress.
void *GLEGetProcAddress(const char *name);

// GLEContext
//
// Manages a collection of OpenGL extension interfaces.
// If the application has multiple OpenGL unrelated contexts then you will want
// to create a different instance of this class for each one you intend to use
// it with.
//
// Example usage:
...
//     gGLEContext.PlatformInit(); // Initializes WGL/GLX/etc. platform-specific
//     OpenGL functionality
//
//     if(GLE_WGL_ARB_create_context) // If wglCreateContextAttribsARB is
//     available...
//     {
//         int attribList[] = { WGL_CONTEXT_MAJOR_VERSION_ARB, 3,
//         WGL_CONTEXT_MINOR_VERSION_ARB, 2, WGL_CONTEXT_FLAGS_ARB,
//         WGL_CONTEXT_DEBUG_BIT_ARB, None }; HGLRC h =
//         wglCreateContextAttribsARB(hDC, 0, attribList);
//         [...]
//     }
//
//     gGLEContext.Init(); // Must be called after an OpenGL context has been
//     created.
//
//     if(GLE_WHOLE_VERSION() >= 302) // If OpenGL 3.2 or later
//     {
//         glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
//         someTexture, 0); // This is an OpenGL 3.2 function.
//         [...]
//     }
//
//     if(GLE_GL_ARB_texture_multisample) // If the GL_ARB_texture_multisample
...
public:
  ...
  // Initializes platform-specific functionality (e.g. Windows WGL, Unix GLX,
  // Android EGL, Apple CGL). You would typically call this before creating an
  // OpenGL context and using platform-specific functions.
  void PlatformInit();
  bool IsPlatformInitialized() const;

  // Loads all the extensions from the current OpenGL context. This must be
  // called after an OpenGL context has been created and made current.
  void Init();
  ...
  void SetEnableHookGetError(bool enabled) { EnableHookGetError = enabled; }

  // Returns the default instance of this class.
  static GLEContext *GetCurrentContext();

  // Sets the default instance of this class. This should be called after
  // enabling a new OpenGL context. This sets the current GLEContext; it does
  // not set the underlying OpenGL context itself.
...
public:
  ...
  int WholeVersion;   // Equals ((MajorVersion * 100) + MinorVersion). Example
                      // usage: if(glv.WholeVersion >= 302) // If OpenGL v3.02+
                      // ...
  bool IsGLES;        // Open GL ES?
  bool IsCoreProfile; // Is the current OpenGL context a core profile context?
                      // Its trueness may be a false positive but will never be
                      // a false negative.
  bool EnableHookGetError; // If enabled then hook functions call glGetError
                           // after making the call.

  int PlatformMajorVersion; // GLX/WGL/EGL/CGL version. Not the same as OpenGL
                            // version.
  int PlatformMinorVersion;
  int PlatformWholeVersion;

  void InitVersion(); // Initializes the version information (e.g.
                      // MajorVersion). Called by the public Init function.
  void InitExtensionLoad();    // Loads the function addresses into the function
                               // pointers.
...
public:
// GL_VERSION_1_1
// Not normally included because all OpenGL 1.1 functionality is always present.
// But if we have hooking enabled then we implement our own version of each
// function.
#if defined(GLE_HOOKING_ENABLED)
  // void PreHook(const char* functionName);             // Called at the
  // beginning of a hook function.
  void
  PostHook(const char *functionName); // Called at the end of a hook function.

  void glAccum_Hook(GLenum op, GLfloat value);
  void glAlphaFunc_Hook(GLenum func, GLclampf ref);
  GLboolean glAreTexturesResident_Hook(GLsizei n, const GLuint *textures,
                                       GLboolean *residences);
  void glArrayElement_Hook(GLint i);
  void glBegin_Hook(GLenum mode);
  void glBindTexture_Hook(GLenum target, GLuint texture);
  void glBitmap_Hook(GLsizei width, GLsizei height, GLfloat xorig,
  ...
  void glClearAccum_Hook(GLfloat red, GLfloat green, GLfloat blue,
                         GLfloat alpha);
  void glClearColor_Hook(GLclampf red, GLclampf green, GLclampf blue,
                         GLclampf alpha);
  ...
  void glColor4d_Hook(GLdouble red, GLdouble green, GLdouble blue,
                      GLdouble alpha);
  ...
  void glColor4us_Hook(GLushort red, GLushort green, GLushort blue,
                       GLushort alpha);
  void glColor4usv_Hook(const GLushort *v);
  void glColorMask_Hook(GLboolean red, GLboolean green, GLboolean blue,
                        GLboolean alpha);
  void glColorMaterial_Hook(GLenum face, GLenum mode);
  void glColorPointer_Hook(GLint size, GLenum type, GLsizei stride,
                           const void *pointer);
  void glCopyPixels_Hook(GLint x, GLint y, GLsizei width, GLsizei height,
                         GLenum type);
  void glCopyTexImage1D_Hook(GLenum target, GLint level, GLenum internalFormat,
                             GLint x, GLint y, GLsizei width, GLint border);
  void glCopyTexImage2D_Hook(GLenum target, GLint level, GLenum internalFormat,
                             GLint x, GLint y, GLsizei width, GLsizei height,
                             GLint border);
  void glCopyTexSubImage1D_Hook(GLenum target, GLint level, GLint xoffset,
                                GLint x, GLint y, GLsizei width);
  void glCopyTexSubImage2D_Hook(GLenum target, GLint level, GLint xoffset,
  ...
  void glDrawElements_Hook(GLenum mode, GLsizei count, GLenum type,
                           const void *indices);
  void glDrawPixels_Hook(GLsizei width, GLsizei height, GLenum format,
  ...
  void glFrustum_Hook(GLdouble left, GLdouble right, GLdouble bottom,
  ...
  void glGetTexImage_Hook(GLenum target, GLint level, GLenum format,
                          GLenum type, void *pixels);
  void glGetTexLevelParameterfv_Hook(GLenum target, GLint level, GLenum pname,
                                     GLfloat *params);
  void glGetTexLevelParameteriv_Hook(GLenum target, GLint level, GLenum pname,
                                     GLint *params);
  ...
  void glInterleavedArrays_Hook(GLenum format, GLsizei stride,
                                const void *pointer);
  ...
  void glMap1d_Hook(GLenum target, GLdouble u1, GLdouble u2, GLint stride,
                    GLint order, const GLdouble *points);
  void glMap1f_Hook(GLenum target, GLfloat u1, GLfloat u2, GLint stride,
                    GLint order, const GLfloat *points);
  void glMap2d_Hook(GLenum target, GLdouble u1, GLdouble u2, GLint ustride,
                    GLint uorder, GLdouble v1, GLdouble v2, GLint vstride,
                    GLint vorder, const GLdouble *points);
  void glMap2f_Hook(GLenum target, GLfloat u1, GLfloat u2, GLint ustride,
  ...
  void glMapGrid2d_Hook(GLint un, GLdouble u1, GLdouble u2, GLint vn,
                        GLdouble v1, GLdouble v2);
  void glMapGrid2f_Hook(GLint un, GLfloat u1, GLfloat u2, GLint vn, GLfloat v1,
                        GLfloat v2);
  ...
  void glOrtho_Hook(GLdouble left, GLdouble right, GLdouble bottom,
                    GLdouble top, GLdouble zNear, GLdouble zFar);
  ...
  void glPrioritizeTextures_Hook(GLsizei n, const GLuint *textures,
  ...
  void glReadPixels_Hook(GLint x, GLint y, GLsizei width, GLsizei height,
  ...
  void glTexCoordPointer_Hook(GLint size, GLenum type, GLsizei stride,
                              const void *pointer);
  ...
  void glTexImage1D_Hook(GLenum target, GLint level, GLint internalformat,
  ...
  void glTexParameterfv_Hook(GLenum target, GLenum pname,
  ...
  void glTexSubImage1D_Hook(GLenum target, GLint level, GLint xoffset,
                            GLsizei width, GLenum format, GLenum type,
                            const void *pixels);
  void glTexSubImage2D_Hook(GLenum target, GLint level, GLint xoffset,
  ...
  void glVertexPointer_Hook(GLint size, GLenum type, GLsizei stride,
                            const void *pointer);
  void glViewport_Hook(GLint x, GLint y, GLsizei width, GLsizei height);

  // GL_VERSION_1_2
  void glBlendColor_Hook(GLclampf red, GLclampf green, GLclampf blue,
                         GLclampf alpha);
  void glBlendEquation_Hook(GLenum mode);
  void glDrawRangeElements_Hook(GLenum mode, GLuint start, GLuint end,
                                GLsizei count, GLenum type,
                                const GLvoid *indices);
  void glTexImage3D_Hook(GLenum target, GLint level, GLint internalformat,
                         GLsizei width, GLsizei height, GLsizei depth,
                         GLint border, GLenum format, GLenum type,
                         const GLvoid *pixels);
  void glTexSubImage3D_Hook(GLenum target, GLint level, GLint xoffset,
                            GLint yoffset, GLint zoffset, GLsizei width,
                            GLsizei height, GLsizei depth, GLenum format,
                            GLenum type, const GLvoid *pixels);
  void glCopyTexSubImage3D_Hook(GLenum target, GLint level, GLint xoffset,
  ...
  GLenum format, GLenum type, const GLvoid *table); void
  glColorTableParameterfv_Hook(GLenum target, GLenum pname, const GLfloat
  *params); void glColorTableParameteriv_Hook(GLenum target, GLenum pname, const
  GLint *params); void glCopyColorTable_Hook(GLenum target, GLenum
  internalformat, GLint x, GLint y, GLsizei width); void
  glGetColorTable_Hook(GLenum target, GLenum format, GLenum type, GLvoid
  *table); void glGetColorTableParameterfv_Hook(GLenum target, GLenum pname,
  GLfloat *params); void glGetColorTableParameteriv_Hook(GLenum target, GLenum
  pname, GLint *params); void glColorSubTable_Hook(GLenum target, GLsizei start,
  GLsizei count, GLenum format, GLenum type, const GLvoid *data); void
  glCopyColorSubTable_Hook(GLenum target, GLsizei start, GLint x, GLint y,
  GLsizei width); void glConvolutionFilter1D_Hook(GLenum target, GLenum
  internalformat, GLsizei width, GLenum format, GLenum type, const GLvoid
  *image); void glConvolutionFilter2D_Hook(GLenum target, GLenum internalformat,
  GLsizei width, GLsizei height, GLenum format, GLenum type, const GLvoid
  *image); void glConvolutionParameterf_Hook(GLenum target, GLenum pname,
  GLfloat params); void glConvolutionParameterfv_Hook(GLenum target, GLenum
  pname, const GLfloat *params); void glConvolutionParameteri_Hook(GLenum
  target, GLenum pname, GLint params); void glConvolutionParameteriv_Hook(GLenum
  target, GLenum pname, const GLint *params); void
  glCopyConvolutionFilter1D_Hook(GLenum target, GLenum internalformat, GLint x,
  GLint y, GLsizei width); void glCopyConvolutionFilter2D_Hook(GLenum target,
  GLenum internalformat, GLint x, GLint y, GLsizei width, GLsizei height); void
  glGetConvolutionFilter_Hook(GLenum target, GLenum format, GLenum type, GLvoid
  *image); void glGetConvolutionParameterfv_Hook(GLenum target, GLenum pname,
  GLfloat *params); void glGetConvolutionParameteriv_Hook(GLenum target, GLenum
  pname, GLint *params); void glGetSeparableFilter_Hook(GLenum target, GLenum
  format, GLenum type, GLvoid *row, GLvoid *column, GLvoid *span); void
  glSeparableFilter2D_Hook(GLenum target, GLenum internalformat, GLsizei width,
  GLsizei height, GLenum format, GLenum type, const GLvoid *row, const GLvoid
  *column); void glGetHistogram_Hook(GLenum target, GLboolean reset, GLenum
  format, GLenum type, GLvoid *values); void
  glGetHistogramParameterfv_Hook(GLenum target, GLenum pname, GLfloat *params);
  void glGetHistogramParameteriv_Hook(GLenum target, GLenum pname, GLint
  *params); void glGetMinmax_Hook(GLenum target, GLboolean reset, GLenum format,
  GLenum type, GLvoid *values); void glGetMinmaxParameterfv_Hook(GLenum target,
  GLenum pname, GLfloat *params); void glGetMinmaxParameteriv_Hook(GLenum
  target, GLenum pname, GLint *params); void glHistogram_Hook(GLenum target,
  GLsizei width, GLenum internalformat, GLboolean sink); void
  glMinmax_Hook(GLenum target, GLenum internalformat, GLboolean sink); void
  glResetHistogram_Hook(GLenum target); void glResetMinmax_Hook(GLenum target);
  */

  // GL_VERSION_1_3
  void glActiveTexture_Hook(GLenum texture);
  void glSampleCoverage_Hook(GLclampf value, GLboolean invert);
  void glCompressedTexImage3D_Hook(GLenum target, GLint level,
  ...
  void glCompressedTexImage2D_Hook(GLenum target, GLint level,
                                   ...
                                   GLsizei imageSize, const GLvoid *data);
  void glCompressedTexImage1D_Hook(GLenum target, GLint level,
                                   GLenum internalformat, GLsizei width,
                                   GLint border, GLsizei imageSize,
                                   const GLvoid *data);
  void glCompressedTexSubImage3D_Hook(GLenum target, GLint level, GLint xoffset,
                                      GLint yoffset, GLint zoffset,
                                      GLsizei width, GLsizei height,
                                      GLsizei depth, GLenum format,
                                      GLsizei imageSize, const GLvoid *data);
  void glCompressedTexSubImage2D_Hook(GLenum target, GLint level, GLint xoffset,
                                      GLint yoffset, GLsizei width,
                                      GLsizei height, GLenum format,
                                      GLsizei imageSize, const GLvoid *data);
  void glCompressedTexSubImage1D_Hook(GLenum target, GLint level, GLint xoffset,
  ...
  void glMultiTexCoord3d_Hook(GLenum target, GLdouble s, GLdouble t,
                              GLdouble r);
  ...
  void glMultiTexCoord4d_Hook(GLenum target, GLdouble s, GLdouble t, GLdouble r,
                              GLdouble q);
  void glMultiTexCoord4dv_Hook(GLenum target, const GLdouble *v);
  void glMultiTexCoord4f_Hook(GLenum target, GLfloat s, GLfloat t, GLfloat r,
                              GLfloat q);
  void glMultiTexCoord4fv_Hook(GLenum target, const GLfloat *v);
  void glMultiTexCoord4i_Hook(GLenum target, GLint s, GLint t, GLint r,
                              GLint q);
  void glMultiTexCoord4iv_Hook(GLenum target, const GLint *v);
  void glMultiTexCoord4s_Hook(GLenum target, GLshort s, GLshort t, GLshort r,
                              GLshort q);
  ...
  // GL_VERSION_1_4
  void glBlendFuncSeparate_Hook(GLenum sfactorRGB, GLenum dfactorRGB,
                                GLenum sfactorAlpha, GLenum dfactorAlpha);
  void glMultiDrawArrays_Hook(GLenum mode, const GLint *first,
                              const GLsizei *count, GLsizei primcount);
  void glMultiDrawElements_Hook(GLenum mode, const GLsizei *count, GLenum type,
  ...
  void glFogCoordPointer_Hook(GLenum type, GLsizei stride,
  ...
  void glSecondaryColorPointer_Hook(GLint size, GLenum type, GLsizei stride,
                                    const GLvoid *pointer);
  ...
  void glBufferData_Hook(GLenum target, GLsizeiptr size, const GLvoid *data,
                         GLenum usage);
  void glBufferSubData_Hook(GLenum target, GLintptr offset, GLsizeiptr size,
                            const GLvoid *data);
  void glGetBufferSubData_Hook(GLenum target, GLintptr offset, GLsizeiptr size,
                               GLvoid *data);
  ...
  void glStencilOpSeparate_Hook(GLenum face, GLenum sfail, GLenum dpfail,
                                GLenum dppass);
  void glStencilFuncSeparate_Hook(GLenum face, GLenum func, GLint ref,
                                  GLuint mask);
  void glStencilMaskSeparate_Hook(GLenum face, GLuint mask);
  void glAttachShader_Hook(GLuint program, GLuint shader);
  void glBindAttribLocation_Hook(GLuint program, GLuint index,
                                 const GLchar *name);
  ...
  void glGetActiveAttrib_Hook(GLuint program, GLuint index, GLsizei bufSize,
                              GLsizei *length, GLint *size, GLenum *type,
                              GLchar *name);
  void glGetActiveUniform_Hook(GLuint program, GLuint index, GLsizei bufSize,
                               GLsizei *length, GLint *size, GLenum *type,
                               GLchar *name);
  void glGetAttachedShaders_Hook(GLuint program, GLsizei maxCount,
  ...
  void glGetProgramInfoLog_Hook(GLuint program, GLsizei bufSize,
                                GLsizei *length, GLchar *infoLog);
  void glGetShaderiv_Hook(GLuint shader, GLenum pname, GLint *params);
  void glGetShaderInfoLog_Hook(GLuint shader, GLsizei bufSize, GLsizei *length,
                               GLchar *infoLog);
  void glGetShaderSource_Hook(GLuint shader, GLsizei bufSize, GLsizei *length,
                              GLchar *source);
  ...
  void glGetVertexAttribPointerv_Hook(GLuint index, GLenum pname,
                                      GLvoid **pointer);
  GLboolean glIsProgram_Hook(GLuint program);
  GLboolean glIsShader_Hook(GLuint shader);
  void glLinkProgram_Hook(GLuint program);
  void glShaderSource_Hook(GLuint shader, GLsizei count, const GLchar **string,
                           const GLint *length);
  ...
  void glUniformMatrix2fv_Hook(GLint location, GLsizei count,
                               GLboolean transpose, const GLfloat *value);
  void glUniformMatrix3fv_Hook(GLint location, GLsizei count,
                               GLboolean transpose, const GLfloat *value);
  void glUniformMatrix4fv_Hook(GLint location, GLsizei count,
                               GLboolean transpose, const GLfloat *value);
  ...
  void glVertexAttrib4Nub_Hook(GLuint index, GLubyte x, GLubyte y, GLubyte z,
                               GLubyte w);
  ...
  void glVertexAttrib4d_Hook(GLuint index, GLdouble x, GLdouble y, GLdouble z,
                             GLdouble w);
  void glVertexAttrib4dv_Hook(GLuint index, const GLdouble *v);
  void glVertexAttrib4f_Hook(GLuint index, GLfloat x, GLfloat y, GLfloat z,
                             GLfloat w);
  void glVertexAttrib4fv_Hook(GLuint index, const GLfloat *v);
  void glVertexAttrib4iv_Hook(GLuint index, const GLint *v);
  void glVertexAttrib4s_Hook(GLuint index, GLshort x, GLshort y, GLshort z,
                             GLshort w);
  ...
  void glVertexAttribPointer_Hook(GLuint index, GLint size, GLenum type,
                                  GLboolean normalized, GLsizei stride,
                                  const GLvoid *pointer);

  // GL_VERSION_2_1
  void glUniformMatrix2x3fv_Hook(GLint location, GLsizei count,
                                 GLboolean transpose, const GLfloat *value);
  void glUniformMatrix3x2fv_Hook(GLint location, GLsizei count,
                                 GLboolean transpose, const GLfloat *value);
  void glUniformMatrix2x4fv_Hook(GLint location, GLsizei count,
                                 GLboolean transpose, const GLfloat *value);
  void glUniformMatrix4x2fv_Hook(GLint location, GLsizei count,
                                 GLboolean transpose, const GLfloat *value);
  void glUniformMatrix3x4fv_Hook(GLint location, GLsizei count,
                                 GLboolean transpose, const GLfloat *value);
  void glUniformMatrix4x3fv_Hook(GLint location, GLsizei count,
                                 GLboolean transpose, const GLfloat *value);

  // GL_VERSION_3_0
  void glColorMaski_Hook(GLuint index, GLboolean r, GLboolean g, GLboolean b,
                         GLboolean a);
  ...
  void glBindBufferRange_Hook(GLenum target, GLuint index, GLuint buffer,
                              GLintptr offset, GLsizeiptr size);
  void glBindBufferBase_Hook(GLenum target, GLuint index, GLuint buffer);
  void glTransformFeedbackVaryings_Hook(GLuint program, GLsizei count,
                                        const GLchar **varyings,
                                        GLenum bufferMode);
  void glGetTransformFeedbackVarying_Hook(GLuint program, GLuint index,
  ...
  void glVertexAttribIPointer_Hook(GLuint index, GLint size, GLenum type,
  ...
  void glVertexAttribI4ui_Hook(GLuint index, GLuint x, GLuint y, GLuint z,
                               GLuint w);
  ...
  void glBindFragDataLocation_Hook(GLuint program, GLuint color,
                                   const GLchar *name);
  ...
  void glUniform4ui_Hook(GLint location, GLuint v0, GLuint v1, GLuint v2,
                         GLuint v3);
  ...
  void glTexParameterIuiv_Hook(GLenum target, GLenum pname,
                               const GLuint *params);
  ...
  void glClearBufferiv_Hook(GLenum buffer, GLint drawbuffer,
                            const GLint *value);
  void glClearBufferuiv_Hook(GLenum buffer, GLint drawbuffer,
                             const GLuint *value);
  void glClearBufferfv_Hook(GLenum buffer, GLint drawbuffer,
                            const GLfloat *value);
  void glClearBufferfi_Hook(GLenum buffer, GLint drawbuffer, GLfloat depth,
                            GLint stencil);
  const GLubyte *glGetStringi_Hook(GLenum name, GLuint index);

  // GL_VERSION_3_1
  void glDrawArraysInstanced_Hook(GLenum mode, GLint first, GLsizei count,
                                  GLsizei primcount);
  void glDrawElementsInstanced_Hook(GLenum mode, GLsizei count, GLenum type,
                                    const GLvoid *indices, GLsizei primcount);
  ...
  void glGetBufferParameteri64v_Hook(GLenum target, GLenum pname,
                                     GLint64 *params);
  void glFramebufferTexture_Hook(GLenum target, GLenum attachment,
  ...
  void glBlendEquationSeparatei_Hook(GLuint buf, GLenum modeRGB,
                                     GLenum modeAlpha);
  void glBlendFunci_Hook(GLuint buf, GLenum src, GLenum dst);
  void glBlendFuncSeparatei_Hook(GLuint buf, GLenum srcRGB, GLenum dstRGB,
                                 GLenum srcAlpha, GLenum dstAlpha);

  // GL_AMD_debug_output
  void glDebugMessageEnableAMD_Hook(GLenum category, GLenum severity,
                                    GLsizei count, const GLuint *ids,
                                    GLboolean enabled);
  void glDebugMessageInsertAMD_Hook(GLenum category, GLenum severity, GLuint id,
                                    GLsizei length, const GLchar *buf);
  void glDebugMessageCallbackAMD_Hook(GLDEBUGPROCAMD callback,
                                      GLvoid *userParam);
  GLuint glGetDebugMessageLogAMD_Hook(GLuint count, GLsizei bufsize,
  ...
  void glDrawRangeElementArrayAPPLE_Hook(GLenum mode, GLuint start, GLuint end,
                                         GLint first, GLsizei count);
  void glMultiDrawElementArrayAPPLE_Hook(GLenum mode, const GLint *first,
                                         const GLsizei *count,
                                         GLsizei primcount);
  void glMultiDrawRangeElementArrayAPPLE_Hook(GLenum mode, GLuint start,
  ...
  void glFlushMappedBufferRangeAPPLE_Hook(GLenum target, GLintptr offset,
                                          GLsizeiptr size);

  // GL_APPLE_object_purgeable
  GLenum glObjectPurgeableAPPLE_Hook(GLenum objectType, GLuint name,
                                     GLenum option);
  GLenum glObjectUnpurgeableAPPLE_Hook(GLenum objectType, GLuint name,
                                       GLenum option);
  void glGetObjectParameterivAPPLE_Hook(GLenum objectType, GLuint name,
                                        GLenum pname, GLint *params);

  // GL_APPLE_texture_range
  void glTextureRangeAPPLE_Hook(GLenum target, GLsizei length,
                                const GLvoid *pointer);
  void glGetTexParameterPointervAPPLE_Hook(GLenum target, GLenum pname,
                                           GLvoid **params);
  ...
  void glMapVertexAttrib1dAPPLE_Hook(GLuint index, GLuint size, GLdouble u1,
                                     GLdouble u2, GLint stride, GLint order,
                                     const GLdouble *points);
  void glMapVertexAttrib1fAPPLE_Hook(GLuint index, GLuint size, GLfloat u1,
                                     GLfloat u2, GLint stride, GLint order,
                                     const GLfloat *points);
  void glMapVertexAttrib2dAPPLE_Hook(GLuint index, GLuint size, GLdouble u1,
                                     GLdouble u2, GLint ustride, GLint uorder,
                                     GLdouble v1, GLdouble v2, GLint vstride,
                                     GLint vorder, const GLdouble *points);
  void glMapVertexAttrib2fAPPLE_Hook(GLuint index, GLuint size, GLfloat u1,
  ...
  // GL_ARB_copy_buffer
  void glCopyBufferSubData_Hook(GLenum readtarget, GLenum writetarget,
                                GLintptr readoffset, GLintptr writeoffset,
                                GLsizeiptr size);

  // GL_ARB_debug_output
  void glDebugMessageControlARB_Hook(GLenum source, GLenum type,
                                     GLenum severity, GLsizei count,
                                     const GLuint *ids, GLboolean enabled);
  void glDebugMessageInsertARB_Hook(GLenum source, GLenum type, GLuint id,
                                    GLenum severity, GLsizei length,
                                    const GLchar *buf);
  void glDebugMessageCallbackARB_Hook(GLDEBUGPROCARB callback,
                                      const GLvoid *userParam);
  GLuint glGetDebugMessageLogARB_Hook(GLuint count, GLsizei bufsize,
  ...
  void glShaderBinary_Hook(GLsizei count, const GLuint *shaders,
                           GLenum binaryformat, const GLvoid *binary,
                           GLsizei length);
  void glGetShaderPrecisionFormat_Hook(GLenum shadertype, GLenum precisiontype,
  ...
  void glRenderbufferStorage_Hook(GLenum target, GLenum internalformat,
                                  GLsizei width, GLsizei height);
  void glGetRenderbufferParameteriv_Hook(GLenum target, GLenum pname,
                                         GLint *params);
  ...
  void glFramebufferTexture1D_Hook(GLenum target, GLenum attachment,
                                   GLenum textarget, GLuint texture,
                                   GLint level);
  void glFramebufferTexture2D_Hook(GLenum target, GLenum attachment,
                                   GLenum textarget, GLuint texture,
                                   GLint level);
  void glFramebufferTexture3D_Hook(GLenum target, GLenum attachment,
                                   GLenum textarget, GLuint texture,
                                   GLint level, GLint zoffset);
  void glFramebufferRenderbuffer_Hook(GLenum target, GLenum attachment,
                                      GLenum renderbuffertarget,
                                      GLuint renderbuffer);
  void glGetFramebufferAttachmentParameteriv_Hook(GLenum target,
                                                  GLenum attachment,
                                                  GLenum pname, GLint *params);
  void glGenerateMipmap_Hook(GLenum target);
  void glBlitFramebuffer_Hook(GLint srcX0, GLint srcY0, GLint srcX1,
                              GLint srcY1, GLint dstX0, GLint dstY0,
                              GLint dstX1, GLint dstY1, GLbitfield mask,
                              GLenum filter);
  void glRenderbufferStorageMultisample_Hook(GLenum target, GLsizei samples,
                                             GLenum internalformat,
                                             GLsizei width, GLsizei height);
  void glFramebufferTextureLayer_Hook(GLenum target, GLenum attachment,
                                      GLuint texture, GLint level, GLint layer);

  // GL_ARB_texture_multisample
  void glTexImage2DMultisample_Hook(GLenum target, GLsizei samples,
                                    GLint internalformat, GLsizei width,
                                    GLsizei height,
                                    GLboolean fixedsamplelocations);
  void glTexImage3DMultisample_Hook(GLenum target, GLsizei samples,
                                    GLint internalformat, GLsizei width,
  ...
  void glTexStorage3D(GLenum target, GLsizei levels, GLenum internalformat,
                      GLsizei width, GLsizei height, GLsizei depth);
  void glTextureStorage1DEXT(GLuint texture, GLenum target, GLsizei levels,
                             GLenum internalformat, GLsizei width);
  void glTextureStorage2DEXT(GLuint texture, GLenum target, GLsizei levels,
                             GLenum internalformat, GLsizei width,
                             GLsizei height);
  void glTextureStorage3DEXT(GLuint texture, GLenum target, GLsizei levels,
                             GLenum internalformat, GLsizei width,
                             GLsizei height, GLsizei depth);
  ...
  void glTexStorage3DMultisample(GLenum target, GLsizei samples,
                                 ...
                                 GLsizei height, GLsizei depth,
                                 GLboolean fixedsamplelocations);
  void glTextureStorage2DMultisampleEXT(GLuint texture, GLenum target,
                                        GLsizei samples, GLenum internalformat,
                                        GLsizei width, GLsizei height,
                                        GLboolean fixedsamplelocations);
  void glTextureStorage3DMultisampleEXT(GLuint texture, GLenum target,
                                        GLsizei samples, GLenum internalformat,
                                        GLsizei width, GLsizei height,
                                        GLsizei depth,
                                        GLboolean fixedsamplelocations);
  ...
  void glColorMaskIndexedEXT_Hook(GLuint index, GLboolean r, GLboolean g,
                                  GLboolean b, GLboolean a);
  void glGetBooleanIndexedvEXT_Hook(GLenum target, GLuint index,
                                    GLboolean *data);
  ...
  // GL_KHR_debug
  void glDebugMessageControl_Hook(GLenum source, GLenum type, GLenum severity,
                                  GLsizei count, const GLuint *ids,
                                  GLboolean enabled);
  void glDebugMessageInsert_Hook(GLenum source, GLenum type, GLuint id,
                                 GLenum severity, GLsizei length,
                                 const char *buf);
  void glDebugMessageCallback_Hook(GLDEBUGPROC callback, const void *userParam);
  GLuint glGetDebugMessageLog_Hook(GLuint count, GLsizei bufSize,
                                   GLenum *sources, GLenum *types, GLuint *ids,
                                   GLenum *severities, GLsizei *lengths,
                                   char *messageLog);
  void glPushDebugGroup_Hook(GLenum source, GLuint id, GLsizei length,
                             const char *message);
  void glPopDebugGroup_Hook(void);
  void glObjectLabel_Hook(GLenum identifier, GLuint name, GLsizei length,
                          const char *label);
  void glGetObjectLabel_Hook(GLenum identifier, GLuint name, GLsizei bufSize,
                             GLsizei *length, char *label);
  void glObjectPtrLabel_Hook(void *ptr, GLsizei length, const char *label);
  void glGetObjectPtrLabel_Hook(void *ptr, GLsizei bufSize, GLsizei *length,
                                char *label);

  // GL_WIN_swap_hint
  void glAddSwapHintRectWIN_Hook(GLint x, GLint y, GLsizei width,
                                 GLsizei height);
  ...
  LPGLYPHMETRICSFLOAT); BOOL  wglUseFontOutlinesW_Hook(HDC, DWORD, DWORD, DWORD,
  FLOAT, FLOAT, int, LPGLYPHMETRICSFLOAT); BOOL  wglDescribeLayerPlane_Hook(HDC,
  int, int, UINT, LPLAYERPLANEDESCRIPTOR); int
  wglSetLayerPaletteEntries_Hook(HDC, int, int, int, CONST COLORREF *); int
  wglGetLayerPaletteEntries_Hook(HDC, int, int, int, COLORREF *); BOOL
  wglRealizeLayerPalette_Hook(HDC, int, BOOL); BOOL
  wglSwapLayerBuffers_Hook(HDC, UINT); DWORD wglSwapMultipleBuffers_Hook(UINT,
  CONST WGLSWAP *);
  */
  ...
  BOOL wglSaveBufferRegionARB_Hook(HANDLE hRegion, int x, int y, int width,
                                   int height);
  BOOL wglRestoreBufferRegionARB_Hook(HANDLE hRegion, int x, int y, int width,
  ...
  BOOL wglGetPixelFormatAttribivARB_Hook(HDC hdc, int iPixelFormat,
                                         int iLayerPlane, UINT nAttributes,
                                         const int *piAttributes,
                                         int *piValues);
  BOOL wglGetPixelFormatAttribfvARB_Hook(HDC hdc, int iPixelFormat,
                                         int iLayerPlane, UINT nAttributes,
                                         const int *piAttributes,
                                         FLOAT *pfValues);
  BOOL wglChoosePixelFormatARB_Hook(HDC hdc, const int *piAttribIList,
  ...
  // WGL_ARB_pbuffer
  HPBUFFERARB wglCreatePbufferARB_Hook(HDC hDC, int iPixelFormat, int iWidth,
  ...
  BOOL wglQueryPbufferARB_Hook(HPBUFFERARB hPbuffer, int iAttribute,
                               int *piValue);
  ...
  BOOL wglSetPbufferAttribARB_Hook(HPBUFFERARB hPbuffer,
                                   const int *piAttribList);

  // WGL_NV_present_video
  int wglEnumerateVideoDevicesNV_Hook(HDC hDC,
                                      HVIDEOOUTPUTDEVICENV *phDeviceList);
  BOOL wglBindVideoDeviceNV_Hook(HDC hDC, unsigned int uVideoSlot,
                                 HVIDEOOUTPUTDEVICENV hVideoDevice,
                                 const int *piAttribList);
  BOOL wglQueryCurrentContextNV_Hook(int iAttribute, int *piValue);

  // WGL_ARB_create_context
  HGLRC wglCreateContextAttribsARB_Hook(HDC hDC, HGLRC hShareContext,
  ...
  INT64 wglSwapBuffersMscOML_Hook(HDC hdc, INT64 target_msc, INT64 divisor,
                                  INT64 remainder);
  INT64 wglSwapLayerBuffersMscOML_Hook(HDC hdc, int fuPlanes, INT64 target_msc,
                                       INT64 divisor, INT64 remainder);
  BOOL wglWaitForMscOML_Hook(HDC hdc, INT64 target_msc, INT64 divisor,
                             INT64 remainder, INT64 *ust, INT64 *msc,
                             INT64 *sbc);
  BOOL wglWaitForSbcOML_Hook(HDC hdc, INT64 target_sbc, INT64 *ust, INT64 *msc,
                             INT64 *sbc);

  // WGL_NV_video_output
  BOOL wglGetVideoDeviceNV_Hook(HDC hDC, int numDevices,
                                HPVIDEODEV *hVideoDevice);
  BOOL wglReleaseVideoDeviceNV_Hook(HPVIDEODEV hVideoDevice);
  BOOL wglBindVideoImageNV_Hook(HPVIDEODEV hVideoDevice, HPBUFFERARB hPbuffer,
                                int iVideoBuffer);
  BOOL wglReleaseVideoImageNV_Hook(HPBUFFERARB hPbuffer, int iVideoBuffer);
  BOOL wglSendPbufferToVideoNV_Hook(HPBUFFERARB hPbuffer, int iBufferType,
                                    unsigned long *pulCounterPbuffer,
                                    BOOL bBlock);
  BOOL wglGetVideoInfoNV_Hook(HPVIDEODEV hpVideoDevice,
  ...
  BOOL wglQueryMaxSwapGroupsNV_Hook(HDC hDC, GLuint *maxGroups,
                                    GLuint *maxBarriers);
  BOOL wglQueryFrameCountNV_Hook(HDC hDC, GLuint *count);
  BOOL wglResetFrameCountNV_Hook(HDC hDC);

  // WGL_NV_video_capture
  BOOL wglBindVideoCaptureDeviceNV_Hook(UINT uVideoSlot,
                                        HVIDEOINPUTDEVICENV hDevice);
  UINT
  wglEnumerateVideoCaptureDevicesNV_Hook(HDC hDc,
                                         HVIDEOINPUTDEVICENV *phDeviceList);
  BOOL wglLockVideoCaptureDeviceNV_Hook(HDC hDc, HVIDEOINPUTDEVICENV hDevice);
  BOOL wglQueryVideoCaptureDeviceNV_Hook(HDC hDc, HVIDEOINPUTDEVICENV hDevice,
                                         int iAttribute, int *piValue);
  BOOL wglReleaseVideoCaptureDeviceNV_Hook(HDC hDc,
                                           HVIDEOINPUTDEVICENV hDevice);

  // WGL_NV_copy_image
  BOOL wglCopyImageSubDataNV_Hook(HGLRC hSrcRC, GLuint srcName,
  ...
  HANDLE wglDXRegisterObjectNV_Hook(HANDLE hDevice, void *dxObject, GLuint name,
  ...
  // GLX_VERSION_1_3
  GLXFBConfig *glXChooseFBConfig_Hook(::Display *dpy, int screen,
                                      const int *attrib_list, int *nelements);
  GLXContext glXCreateNewContext_Hook(::Display *dpy, GLXFBConfig config,
                                      int render_type, GLXContext share_list,
                                      Bool direct);
  GLXPbuffer glXCreatePbuffer_Hook(::Display *dpy, GLXFBConfig config,
                                   const int *attrib_list);
  GLXPixmap glXCreatePixmap_Hook(::Display *dpy, GLXFBConfig config,
                                 Pixmap pixmap, const int *attrib_list);
  GLXWindow glXCreateWindow_Hook(::Display *dpy, GLXFBConfig config, Window win,
                                 const int *attrib_list);
  ...
  int glXGetFBConfigAttrib_Hook(::Display *dpy, GLXFBConfig config,
                                int attribute, int *value);
  GLXFBConfig *glXGetFBConfigs_Hook(::Display *dpy, int screen, int *nelements);
  void glXGetSelectedEvent_Hook(::Display *dpy, GLXDrawable draw,
                                unsigned long *event_mask);
  XVisualInfo *glXGetVisualFromFBConfig_Hook(::Display *dpy,
                                             GLXFBConfig config);
  Bool glXMakeContextCurrent_Hook(::Display *display, GLXDrawable draw,
                                  GLXDrawable read, GLXContext ctx);
  int glXQueryContext_Hook(::Display *dpy, GLXContext ctx, int attribute,
                           int *value);
  void glXQueryDrawable_Hook(::Display *dpy, GLXDrawable draw, int attribute,
                             unsigned int *value);
  void glXSelectEvent_Hook(::Display *dpy, GLXDrawable draw,
                           unsigned long event_mask);
  ...
  GLXContext glXCreateContextAttribsARB_Hook(Display *dpy, GLXFBConfig config,
                                             GLXContext share_context,
                                             Bool direct,
                                             const int *attrib_list);

  // GLX_EXT_swap_control
  void glXSwapIntervalEXT_Hook(::Display *dpy, GLXDrawable drawable,
                               int interval);

  // GLX_OML_sync_control
  Bool glXGetMscRateOML_Hook(::Display *dpy, GLXDrawable drawable,
                             int32_t *numerator, int32_t *denominator);
  Bool glXGetSyncValuesOML_Hook(::Display *dpy, GLXDrawable drawable,
                                int64_t *ust, int64_t *msc, int64_t *sbc);
  int64_t glXSwapBuffersMscOML_Hook(::Display *dpy, GLXDrawable drawable,
                                    int64_t target_msc, int64_t divisor,
                                    int64_t remainder);
  Bool glXWaitForMscOML_Hook(::Display *dpy, GLXDrawable drawable,
                             int64_t target_msc, int64_t divisor,
                             int64_t remainder, int64_t *ust, int64_t *msc,
                             int64_t *sbc);
  Bool glXWaitForSbcOML_Hook(::Display *dpy, GLXDrawable drawable,
                             int64_t target_sbc, int64_t *ust, int64_t *msc,
                             int64_t *sbc);
  ...
  */
  ...
  PFNGLGETFRAMEBUFFERATTACHMENTPARAMETERIVPROC
      glGetFramebufferAttachmentParameteriv_Impl;
...
#if defined(GLE_WGL_ENABLED)
  // WGL
  // We don't declare pointers for these because we statically link to the
  // implementations, same as with the OpenGL 1.1 functions. BOOL
  // wglCopyContext_Hook(HGLRC, HGLRC, UINT); HGLRC wglCreateContext_Hook(HDC);
  // HGLRC wglCreateLayerContext_Hook(HDC, int);
  // BOOL  wglDeleteContext_Hook(HGLRC);
  // HGLRC wglGetCurrentContext_Hook(VOID);
  // HDC   wglGetCurrentDC_Hook(VOID);
  ...
  // BOOL  wglUseFontBitmapsW_Hook(HDC, DWORD, DWORD, DWORD);
  // BOOL  wglUseFontOutlinesA_Hook(HDC, DWORD, DWORD, DWORD, FLOAT, FLOAT, int,
  // LPGLYPHMETRICSFLOAT); BOOL  wglUseFontOutlinesW_Hook(HDC, DWORD, DWORD,
  // DWORD, FLOAT, FLOAT, int, LPGLYPHMETRICSFLOAT); BOOL
  // wglDescribeLayerPlane_Hook(HDC, int, int, UINT, LPLAYERPLANEDESCRIPTOR);
  ...
  PFNWGLENUMERATEVIDEOCAPTUREDEVICESNVPROC
      wglEnumerateVideoCaptureDevicesNV_Impl;
...
#if defined(GLE_GLX_ENABLED)
  // GLX_VERSION_1_1
  // We don't create any pointers, because we assume these functions are always
  // present.
