...
// Windows headers
//     <wingdi.h>            Windows-specific OpenGL 1.1 interfaces. Long ago
//     this was <GL/wgl.h>. <GL/gl.h>             OpenGL 1.1 interface.
//     <GL/glext.h>          OpenGL 1.2+ compatibility profile and extension
//     interfaces. Not provided by Microsoft. <GL/wglext.h> Windows-specific
//     extension interfaces. Not provided by Microsoft. <GL/glcorearb.h> OpenGL
//     core profile and ARB extension interfaces. Doesn't include interfaces
//     found only in the compatibility profile. Overlaps with gl.h and glext.h.
//
// Mac headers
//     <OpenGL/gl.h>         OpenGL 1.1 interface.
//     <OpenGL/glext.h>      OpenGL 1.2+ compatibility profile and extension
//     interfaces. <OpenGL/gl3.h>        Includes only interfaces supported in a
//     core OpenGL 3.1 implementations plus a few related extensions.
//     <OpenGL/gl3ext.h>     Includes extensions supported in a core OpenGL 3.1
//     implementation. <OpenGL/OpenGL.h>     Apple-specific OpenGL interfaces.
//     <OpenGL/NSOpenGL.h>   Apple-specific OpenGL interfaces.
//
// Linux headers
//     <GL/gl.h>             OpenGL 1.1 interface.
//     <GL/glext.h>          OpenGL 1.2+ compatibility profile and extension
//     interfaces. <GL/glx.h>            X Windows-specific OpenGL interfaces.
//     <GL/glxext.h>         X Windows 1.3+ API and GLX extension interfaces.
//     <GL/glcorearb.h>      OpenGL core profile and ARB extension interfaces.
//     Doesn't include interfaces found only in the compatibility profile.
//     Overlaps with gl.h and glext.h.
...
// We do not disable Microsoft's wingdi.h and redeclare its functions. That's a
// big header that includes many other things. We do not currently disable
// Apple's OpenGL/OpenGL.h, though we could if we replicated its declarations in
// this header file.
...
#define GLE_WINGDIAPI_DEFINED // We define this so that we can know to undefine
                              // WINGDIAPI at the end of this file.
#endif
...
// GLE_CLASS_EXPORT
// Possibly maps to Microsoft __declspec(dllexport) or nothing on other
// platforms.
#define GLE_CLASS_EXPORT // Currently defined to nothing. Could be defined to a
                         // dll export type.

// GLE_HOOKING_ENABLED
// When enabled, we intercept all OpenGL calls and do any useful internal
// processing before or after the call. An example use case for this is to
// intercept OpenGL errors on platforms that don't support the OpenGL debug
// functionality (e.g. KHR_Debug).
#if !defined(GLE_WGL_ENABLED) && !defined(GLE_GLX_ENABLED) &&                  \
    defined(                                                                   \
        OVR_BUILD_DEBUG) // Windows and Unix don't need it because they have
                         // OpenGL debug extension support (e.g. KHR_Debug).
#define GLE_HOOKING_ENABLED 1
#endif

// When using hooking, we map all OpenGL function usage to our member functions
// that end with _Hook. These member hook functions will internally call the
// actual OpenGL functions after doing some internal processing.
...
// OpenGL 1.1 declarations are present in all versions of gl.h, including
// Microsoft's. You don't need to dynamically link to these functions on any
// platform and can just assume they are present. A number of these functions
// have been deprecated by OpenGL v3+, and if an OpenGL v3+ core profile is
// enabled then usage of the deprecated functions is an error.
...
#if defined(GLE_HOOKING_ENABLED)
// In this case we map these functions to internal versions instead of the
// standard versions.
...
#define glAreTexturesResident(...)                                             \
  GLEGetCurrentFunction(glAreTexturesResident)(__VA_ARGS__)
...
#define glCopyTexImage1D(...)                                                  \
  GLEGetCurrentFunction(glCopyTexImage1D)(__VA_ARGS__)
#define glCopyTexImage2D(...)                                                  \
  GLEGetCurrentFunction(glCopyTexImage2D)(__VA_ARGS__)
#define glCopyTexSubImage1D(...)                                               \
  GLEGetCurrentFunction(glCopyTexSubImage1D)(__VA_ARGS__)
#define glCopyTexSubImage2D(...)                                               \
  GLEGetCurrentFunction(glCopyTexSubImage2D)(__VA_ARGS__)
...
#define glDeleteTextures(...)                                                  \
  GLEGetCurrentFunction(glDeleteTextures)(__VA_ARGS__)
...
#define glDisableClientState(...)                                              \
  GLEGetCurrentFunction(glDisableClientState)(__VA_ARGS__)
...
#define glEdgeFlagPointer(...)                                                 \
  GLEGetCurrentFunction(glEdgeFlagPointer)(__VA_ARGS__)
...
#define glEnableClientState(...)                                               \
  GLEGetCurrentFunction(glEnableClientState)(__VA_ARGS__)
...
#define glFeedbackBuffer(...)                                                  \
  GLEGetCurrentFunction(glFeedbackBuffer)(__VA_ARGS__)
...
#define glGetPixelMapuiv(...)                                                  \
  GLEGetCurrentFunction(glGetPixelMapuiv)(__VA_ARGS__)
#define glGetPixelMapusv(...)                                                  \
  GLEGetCurrentFunction(glGetPixelMapusv)(__VA_ARGS__)
#define glGetPointerv(...) GLEGetCurrentFunction(glGetPointerv)(__VA_ARGS__)
#define glGetPolygonStipple(...)                                               \
  GLEGetCurrentFunction(glGetPolygonStipple)(__VA_ARGS__)
...
#define glGetTexLevelParameterfv(...)                                          \
  GLEGetCurrentFunction(glGetTexLevelParameterfv)(__VA_ARGS__)
#define glGetTexLevelParameteriv(...)                                          \
  GLEGetCurrentFunction(glGetTexLevelParameteriv)(__VA_ARGS__)
#define glGetTexParameterfv(...)                                               \
  GLEGetCurrentFunction(glGetTexParameterfv)(__VA_ARGS__)
#define glGetTexParameteriv(...)                                               \
  GLEGetCurrentFunction(glGetTexParameteriv)(__VA_ARGS__)
...
#define glInterleavedArrays(...)                                               \
  GLEGetCurrentFunction(glInterleavedArrays)(__VA_ARGS__)
...
#define glPixelTransferf(...)                                                  \
  GLEGetCurrentFunction(glPixelTransferf)(__VA_ARGS__)
#define glPixelTransferi(...)                                                  \
  GLEGetCurrentFunction(glPixelTransferi)(__VA_ARGS__)
...
#define glPolygonStipple(...)                                                  \
  GLEGetCurrentFunction(glPolygonStipple)(__VA_ARGS__)
...
#define glPrioritizeTextures(...)                                              \
  GLEGetCurrentFunction(glPrioritizeTextures)(__VA_ARGS__)
#define glPushAttrib(...) GLEGetCurrentFunction(glPushAttrib)(__VA_ARGS__)
#define glPushClientAttrib(...)                                                \
  GLEGetCurrentFunction(glPushClientAttrib)(__VA_ARGS__)
...
#define glTexCoordPointer(...)                                                 \
  GLEGetCurrentFunction(glTexCoordPointer)(__VA_ARGS__)
...
#define glTexParameterfv(...)                                                  \
  GLEGetCurrentFunction(glTexParameterfv)(__VA_ARGS__)
#define glTexParameteri(...) GLEGetCurrentFunction(glTexParameteri)(__VA_ARGS__)
#define glTexParameteriv(...)                                                  \
  GLEGetCurrentFunction(glTexParameteriv)(__VA_ARGS__)
...
#else
// There is no need to typedef OpenGL 1.1 function types because they are
// present in all OpenGL implementations and don't need to be treated
// dynamically like extensions.
...
GLclampf blue, GLclampf alpha); typedef void (GLAPIENTRY PFNGLBLENDEQUATIONPROC)
(GLenum mode); typedef void (GLAPIENTRY PFNGLCOLORTABLEPROC) (GLenum target,
GLenum internalformat, GLsizei width, GLenum format, GLenum type, const GLvoid
*table); typedef void (GLAPIENTRY PFNGLCOLORTABLEPARAMETERFVPROC) (GLenum
target, GLenum pname, const GLfloat *params); typedef void (GLAPIENTRY
PFNGLCOLORTABLEPARAMETERIVPROC) (GLenum target, GLenum pname, const GLint
*params); typedef void (GLAPIENTRY PFNGLCOPYCOLORTABLEPROC) (GLenum target,
GLenum internalformat, GLint x, GLint y, GLsizei width); typedef void
(GLAPIENTRY PFNGLGETCOLORTABLEPROC) (GLenum target, GLenum format, GLenum type,
GLvoid *table); typedef void (GLAPIENTRY PFNGLGETCOLORTABLEPARAMETERFVPROC)
(GLenum target, GLenum pname, GLfloat *params); typedef void (GLAPIENTRY
PFNGLGETCOLORTABLEPARAMETERIVPROC) (GLenum target, GLenum pname, GLint *params);
typedef void (GLAPIENTRY PFNGLCOLORSUBTABLEPROC) (GLenum target, GLsizei start,
GLsizei count, GLenum format, GLenum type, const GLvoid *data); typedef void
(GLAPIENTRY PFNGLCOPYCOLORSUBTABLEPROC) (GLenum target, GLsizei start, GLint x,
GLint y, GLsizei width); typedef void (GLAPIENTRY PFNGLCONVOLUTIONFILTER1DPROC)
(GLenum target, GLenum internalformat, GLsizei width, GLenum format, GLenum
type, const GLvoid *image); typedef void (GLAPIENTRY
PFNGLCONVOLUTIONFILTER2DPROC) (GLenum target, GLenum internalformat, GLsizei
width, GLsizei height, GLenum format, GLenum type, const GLvoid *image); typedef
void (GLAPIENTRY PFNGLCONVOLUTIONPARAMETERFPROC) (GLenum target, GLenum pname,
GLfloat params); typedef void (GLAPIENTRY PFNGLCONVOLUTIONPARAMETERFVPROC)
(GLenum target, GLenum pname, const GLfloat *params); typedef void (GLAPIENTRY
PFNGLCONVOLUTIONPARAMETERIPROC) (GLenum target, GLenum pname, GLint params);
typedef void (GLAPIENTRY PFNGLCONVOLUTIONPARAMETERIVPROC) (GLenum target, GLenum
pname, const GLint *params); typedef void (GLAPIENTRY
PFNGLCOPYCONVOLUTIONFILTER1DPROC) (GLenum target, GLenum internalformat, GLint
x, GLint y, GLsizei width); typedef void (GLAPIENTRY
PFNGLCOPYCONVOLUTIONFILTER2DPROC) (GLenum target, GLenum internalformat, GLint
x, GLint y, GLsizei width, GLsizei height); typedef void (GLAPIENTRY
PFNGLGETCONVOLUTIONFILTERPROC) (GLenum target, GLenum format, GLenum type,
GLvoid *image); typedef void (GLAPIENTRY PFNGLGETCONVOLUTIONPARAMETERFVPROC)
(GLenum target, GLenum pname, GLfloat *params); typedef void (GLAPIENTRY
PFNGLGETCONVOLUTIONPARAMETERIVPROC) (GLenum target, GLenum pname, GLint
*params); typedef void (GLAPIENTRY PFNGLGETSEPARABLEFILTERPROC) (GLenum target,
GLenum format, GLenum type, GLvoid *row, GLvoid *column, GLvoid *span); typedef
void (GLAPIENTRY PFNGLSEPARABLEFILTER2DPROC) (GLenum target, GLenum
internalformat, GLsizei width, GLsizei height, GLenum format, GLenum type, const
GLvoid *row, const GLvoid *column); typedef void (GLAPIENTRY
PFNGLGETHISTOGRAMPROC) (GLenum target, GLboolean reset, GLenum format, GLenum
type, GLvoid *values); typedef void (GLAPIENTRY
PFNGLGETHISTOGRAMPARAMETERFVPROC) (GLenum target, GLenum pname, GLfloat
*params); typedef void (GLAPIENTRY PFNGLGETHISTOGRAMPARAMETERIVPROC) (GLenum
target, GLenum pname, GLint *params); typedef void (GLAPIENTRY
PFNGLGETMINMAXPROC) (GLenum target, GLboolean reset, GLenum format, GLenum type,
GLvoid *values); typedef void (GLAPIENTRY PFNGLGETMINMAXPARAMETERFVPROC) (GLenum
target, GLenum pname, GLfloat *params); typedef void (GLAPIENTRY
...
GLenum internalformat, GLboolean sink); typedef void (GLAPIENTRY
PFNGLMINMAXPROC) (GLenum target, GLenum internalformat, GLboolean sink); typedef
void (GLAPIENTRY PFNGLRESETHISTOGRAMPROC) (GLenum target); typedef void
(GLAPIENTRY PFNGLRESETMINMAXPROC) (GLenum target);
*/
...
#define glCompressedTexSubImage1D                                              \
  GLEGetCurrentFunction(glCompressedTexSubImage1D)
#define glCompressedTexSubImage2D                                              \
  GLEGetCurrentFunction(glCompressedTexSubImage2D)
#define glCompressedTexSubImage3D                                              \
  GLEGetCurrentFunction(glCompressedTexSubImage3D)
...
typedef void(GLAPIENTRY *PFNGLMULTIDRAWARRAYSPROC)(GLenum mode,
                                                   const GLint *first,
                                                   const GLsizei *count,
                                                   GLsizei drawcount);
...
typedef void(GLAPIENTRY *PFNGLBINDATTRIBLOCATIONPROC)(GLuint program,
                                                      GLuint index,
                                                      const GLchar *name);
typedef void(GLAPIENTRY *PFNGLBLENDEQUATIONSEPARATEPROC)(GLenum modeRGB,
                                                         GLenum modeAlpha);
...
typedef void(GLAPIENTRY *PFNGLGETVERTEXATTRIBPOINTERVPROC)(GLuint index,
                                                           GLenum pname,
                                                           void **pointer);
typedef void(GLAPIENTRY *PFNGLGETVERTEXATTRIBDVPROC)(GLuint index, GLenum pname,
                                                     GLdouble *params);
typedef void(GLAPIENTRY *PFNGLGETVERTEXATTRIBFVPROC)(GLuint index, GLenum pname,
                                                     GLfloat *params);
typedef void(GLAPIENTRY *PFNGLGETVERTEXATTRIBIVPROC)(GLuint index, GLenum pname,
                                                     GLint *params);
...
typedef void(GLAPIENTRY *PFNGLSTENCILMASKSEPARATEPROC)(GLenum face,
                                                       GLuint mask);
...
#define glDisableVertexAttribArray                                             \
  GLEGetCurrentFunction(glDisableVertexAttribArray)
#define glDrawBuffers GLEGetCurrentFunction(glDrawBuffers)
#define glEnableVertexAttribArray                                              \
  GLEGetCurrentFunction(glEnableVertexAttribArray)
...
#define glGetVertexAttribPointerv                                              \
  GLEGetCurrentFunction(glGetVertexAttribPointerv)
...
typedef void(GLAPIENTRY *PFNGLBEGINCONDITIONALRENDERPROC)(GLuint id,
                                                          GLenum mode);
typedef void(GLAPIENTRY *PFNGLBEGINTRANSFORMFEEDBACKPROC)(GLenum primitiveMode);
typedef void(GLAPIENTRY *PFNGLBINDFRAGDATALOCATIONPROC)(GLuint program,
                                                        GLuint colorNumber,
                                                        const GLchar *name);
typedef void(GLAPIENTRY *PFNGLCLAMPCOLORPROC)(GLenum target, GLenum clamp);
typedef void(GLAPIENTRY *PFNGLCLEARBUFFERFIPROC)(GLenum buffer,
                                                 GLint drawBuffer,
                                                 GLfloat depth, GLint stencil);
typedef void(GLAPIENTRY *PFNGLCLEARBUFFERFVPROC)(GLenum buffer,
                                                 GLint drawBuffer,
                                                 const GLfloat *value);
typedef void(GLAPIENTRY *PFNGLCLEARBUFFERIVPROC)(GLenum buffer,
                                                 GLint drawBuffer,
                                                 const GLint *value);
typedef void(GLAPIENTRY *PFNGLCLEARBUFFERUIVPROC)(GLenum buffer,
                                                  GLint drawBuffer,
                                                  const GLuint *value);
typedef void(GLAPIENTRY *PFNGLCOLORMASKIPROC)(GLuint buf, GLboolean red,
...
typedef void(GLAPIENTRY *PFNGLBINDBUFFERBASEPROC)(GLenum target, GLuint index,
                                                  GLuint buffer);
typedef void(GLAPIENTRY *PFNGLGETBOOLEANI_VPROC)(GLenum pname, GLuint index,
                                                 GLboolean *data);
typedef void(GLAPIENTRY *PFNGLGETINTEGERI_VPROC)(GLenum target, GLuint index,
                                                 GLint *data);
typedef GLint(GLAPIENTRY *PFNGLGETFRAGDATALOCATIONPROC)(GLuint program,
                                                        const GLchar *name);
typedef const GLubyte *(GLAPIENTRY *PFNGLGETSTRINGIPROC)(GLenum name,
                                                         GLuint index);
typedef void(GLAPIENTRY *PFNGLGETTEXPARAMETERIIVPROC)(GLenum target,
                                                      GLenum pname,
                                                      GLint *params);
typedef void(GLAPIENTRY *PFNGLGETTEXPARAMETERIUIVPROC)(GLenum target,
                                                       GLenum pname,
                                                       GLuint *params);
typedef void(GLAPIENTRY *PFNGLGETTRANSFORMFEEDBACKVARYINGPROC)(
    GLuint program, GLuint index, GLsizei bufSize, GLsizei *length,
    GLsizei *size, GLenum *type, GLchar *name);
typedef void(GLAPIENTRY *PFNGLGETUNIFORMUIVPROC)(GLuint program, GLint location,
                                                 GLuint *params);
typedef void(GLAPIENTRY *PFNGLGETVERTEXATTRIBIIVPROC)(GLuint index,
                                                      GLenum pname,
                                                      GLint *params);
typedef void(GLAPIENTRY *PFNGLGETVERTEXATTRIBIUIVPROC)(GLuint index,
                                                       GLenum pname,
                                                       GLuint *params);
typedef GLboolean(GLAPIENTRY *PFNGLISENABLEDIPROC)(GLenum cap, GLuint index);
typedef void(GLAPIENTRY *PFNGLTEXPARAMETERIIVPROC)(GLenum target, GLenum pname,
                                                   const GLint *params);
typedef void(GLAPIENTRY *PFNGLTEXPARAMETERIUIVPROC)(GLenum target, GLenum pname,
                                                    const GLuint *params);
...
typedef void(GLAPIENTRY *PFNGLUNIFORM1UIVPROC)(GLint location, GLsizei count,
                                               const GLuint *value);
typedef void(GLAPIENTRY *PFNGLUNIFORM2UIPROC)(GLint location, GLuint v0,
                                              GLuint v1);
typedef void(GLAPIENTRY *PFNGLUNIFORM2UIVPROC)(GLint location, GLsizei count,
                                               const GLuint *value);
typedef void(GLAPIENTRY *PFNGLUNIFORM3UIPROC)(GLint location, GLuint v0,
                                              GLuint v1, GLuint v2);
typedef void(GLAPIENTRY *PFNGLUNIFORM3UIVPROC)(GLint location, GLsizei count,
                                               const GLuint *value);
typedef void(GLAPIENTRY *PFNGLUNIFORM4UIPROC)(GLint location, GLuint v0,
                                              GLuint v1, GLuint v2, GLuint v3);
typedef void(GLAPIENTRY *PFNGLUNIFORM4UIVPROC)(GLint location, GLsizei count,
                                               const GLuint *value);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI1IPROC)(GLuint index, GLint v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI1IVPROC)(GLuint index,
                                                    const GLint *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI1UIPROC)(GLuint index, GLuint v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI1UIVPROC)(GLuint index,
                                                     const GLuint *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI2IPROC)(GLuint index, GLint v0,
                                                   GLint v1);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI2IVPROC)(GLuint index,
                                                    const GLint *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI2UIPROC)(GLuint index, GLuint v0,
                                                    GLuint v1);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI2UIVPROC)(GLuint index,
                                                     const GLuint *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI3IPROC)(GLuint index, GLint v0,
                                                   GLint v1, GLint v2);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI3IVPROC)(GLuint index,
                                                    const GLint *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI3UIPROC)(GLuint index, GLuint v0,
                                                    GLuint v1, GLuint v2);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI3UIVPROC)(GLuint index,
                                                     const GLuint *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI4BVPROC)(GLuint index,
                                                    const GLbyte *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI4IPROC)(GLuint index, GLint v0,
                                                   GLint v1, GLint v2,
                                                   GLint v3);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI4IVPROC)(GLuint index,
                                                    const GLint *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI4SVPROC)(GLuint index,
                                                    const GLshort *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI4UBVPROC)(GLuint index,
                                                     const GLubyte *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI4UIPROC)(GLuint index, GLuint v0,
                                                    GLuint v1, GLuint v2,
                                                    GLuint v3);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI4UIVPROC)(GLuint index,
                                                     const GLuint *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBI4USVPROC)(GLuint index,
                                                     const GLushort *v0);
typedef void(GLAPIENTRY *PFNGLVERTEXATTRIBIPOINTERPROC)(GLuint index,
...
#define glGetTransformFeedbackVarying                                          \
  GLEGetCurrentFunction(glGetTransformFeedbackVarying)
...
#define glTransformFeedbackVaryings                                            \
  GLEGetCurrentFunction(glTransformFeedbackVaryings)
...
typedef void(GLAPIENTRY *PFNGLDRAWARRAYSINSTANCEDPROC)(GLenum mode, GLint first,
                                                       GLsizei count,
                                                       GLsizei primcount);
typedef void(GLAPIENTRY *PFNGLDRAWELEMENTSINSTANCEDPROC)(GLenum mode,
                                                         GLsizei count,
                                                         GLenum type,
                                                         const void *indices,
                                                         GLsizei primcount);
typedef void(GLAPIENTRY *PFNGLPRIMITIVERESTARTINDEXPROC)(GLuint buffer);
typedef void(GLAPIENTRY *PFNGLTEXBUFFERPROC)(GLenum target,
...
typedef void(GLAPIENTRY *PFNGLFRAMEBUFFERTEXTUREPROC)(GLenum target,
                                                      GLenum attachment,
                                                      GLuint texture,
                                                      GLint level);
typedef void(GLAPIENTRY *PFNGLGETBUFFERPARAMETERI64VPROC)(GLenum target,
                                                          GLenum value,
                                                          GLint64 *data);
typedef void(GLAPIENTRY *PFNGLGETINTEGER64I_VPROC)(GLenum pname, GLuint index,
                                                   GLint64 *data);
...
typedef void(GLAPIENTRY *GLDEBUGPROCAMD)(GLuint id, GLenum category,
...
#define glDebugMessageCallbackAMD                                              \
  GLEGetCurrentFunction(glDebugMessageCallbackAMD)
...
GLuint* monitors); typedef void (GLAPIENTRY * PFNGLENDPERFMONITORAMDPROC)
(GLuint monitor); typedef void (GLAPIENTRY * PFNGLGENPERFMONITORSAMDPROC)
(GLsizei n, GLuint* monitors); typedef void (GLAPIENTRY *
PFNGLGETPERFMONITORCOUNTERDATAAMDPROC) (GLuint monitor, GLenum pname, GLsizei
dataSize, GLuint* data, GLint *bytesWritten); typedef void (GLAPIENTRY *
PFNGLGETPERFMONITORCOUNTERINFOAMDPROC) (GLuint group, GLuint counter, GLenum
pname, void *data); typedef void (GLAPIENTRY *
PFNGLGETPERFMONITORCOUNTERSTRINGAMDPROC) (GLuint group, GLuint counter, GLsizei
bufSize, GLsizei* length, GLchar *counterString); typedef void (GLAPIENTRY *
PFNGLGETPERFMONITORCOUNTERSAMDPROC) (GLuint group, GLint* numCounters, GLint
*maxActiveCounters, GLsizei countersSize, GLuint *counters); typedef void
(GLAPIENTRY * PFNGLGETPERFMONITORGROUPSTRINGAMDPROC) (GLuint group, GLsizei
bufSize, GLsizei* length, GLchar *groupString); typedef void (GLAPIENTRY *
PFNGLGETPERFMONITORGROUPSAMDPROC) (GLint* numGroups, GLsizei groupsSize, GLuint
*groups); typedef void (GLAPIENTRY * PFNGLSELECTPERFMONITORCOUNTERSAMDPROC)
(GLuint monitor, GLboolean enable, GLuint group, GLint numCounters, GLuint*
counterList);

    #define glBeginPerfMonitorAMD GLEGetCurrentFunction(glBeginPerfMonitorAMD)
    #define glDeletePerfMonitorsAMD
GLEGetCurrentFunction(glDeletePerfMonitorsAMD) #define glEndPerfMonitorAMD
GLEGetCurrentFunction(glEndPerfMonitorAMD) #define glGenPerfMonitorsAMD
GLEGetCurrentFunction(glGenPerfMonitorsAMD) #define
glGetPerfMonitorCounterDataAMD
GLEGetCurrentFunction(glGetPerfMonitorCounterDataAMD) #define
glGetPerfMonitorCounterInfoAMD
GLEGetCurrentFunction(glGetPerfMonitorCounterInfoAMD) #define
glGetPerfMonitorCounterStringAMD
GLEGetCurrentFunction(glGetPerfMonitorCounterStringAMD) #define
glGetPerfMonitorCountersAMD GLEGetCurrentFunction(glGetPerfMonitorCountersAMD)
    #define glGetPerfMonitorGroupStringAMD
GLEGetCurrentFunction(glGetPerfMonitorGroupStringAMD) #define
glGetPerfMonitorGroupsAMD GLEGetCurrentFunction(glGetPerfMonitorGroupsAMD)
    #define glSelectPerfMonitorCountersAMD
GLEGetCurrentFunction(glSelectPerfMonitorCountersAMD)

    #define GLE_AMD_performance_monitor
GLEGetCurrentVariable(gle_AMD_performance_monitor)
...
#define GLE_APPLE_aux_depth_stencil                                            \
  GLEGetCurrentVariable(gle_APPLE_aux_depth_stencil)
#endif
...
#define glDrawRangeElementArrayAPPLE                                           \
  GLEGetCurrentFunction(glDrawRangeElementArrayAPPLE)
#define glElementPointerAPPLE GLEGetCurrentFunction(glElementPointerAPPLE)
#define glMultiDrawElementArrayAPPLE                                           \
  GLEGetCurrentFunction(glMultiDrawElementArrayAPPLE)
#define glMultiDrawRangeElementArrayAPPLE                                      \
  GLEGetCurrentFunction(glMultiDrawRangeElementArrayAPPLE)
...
#define glFlushMappedBufferRangeAPPLE                                          \
  GLEGetCurrentFunction(glFlushMappedBufferRangeAPPLE)

#define GLE_APPLE_flush_buffer_range                                           \
  GLEGetCurrentVariable(gle_APPLE_flush_buffer_range)
#endif
...
#define glGetObjectParameterivAPPLE                                            \
  GLEGetCurrentFunction(glGetObjectParameterivAPPLE)
...
#define GLE_APPLE_object_purgeable                                             \
  GLEGetCurrentVariable(gle_APPLE_object_purgeable)
#endif
...
#define GLE_APPLE_specular_vector                                              \
  GLEGetCurrentVariable(gle_APPLE_specular_vector)
#endif
...
typedef void(GLAPIENTRY *PFNGLGETTEXPARAMETERPOINTERVAPPLEPROC)(GLenum target,
                                                                GLenum pname,
                                                                void **params);
...
#define glGetTexParameterPointervAPPLE                                         \
  GLEGetCurrentFunction(glGetTexParameterPointervAPPLE)
...
// This has been superceded by GL_ARB_vertex_array_object, though if you are
// using Apple OpenGL prior to 3.x then only this interface will be available.
// However, we have made it so that glBindVertexArray maps to
// glBindVertexArrayApple when only the latter is present, thus allowing you to
// write cleaner code. You can always just call glBindVertexArray instead of
// glBindVertexArrayAPPLE, etc.
...
#define GLE_APPLE_vertex_array_object                                          \
  GLEGetCurrentVariable(gle_APPLE_vertex_array_object)
#endif
...
#define glFlushVertexArrayRangeAPPLE                                           \
  GLEGetCurrentFunction(glFlushVertexArrayRangeAPPLE)
#define glVertexArrayParameteriAPPLE                                           \
  GLEGetCurrentFunction(glVertexArrayParameteriAPPLE)
#define glVertexArrayRangeAPPLE GLEGetCurrentFunction(glVertexArrayRangeAPPLE)

#define GLE_APPLE_vertex_array_range                                           \
  GLEGetCurrentVariable(gle_APPLE_vertex_array_range)
#endif
...
#define glDisableVertexAttribAPPLE                                             \
  GLEGetCurrentFunction(glDisableVertexAttribAPPLE)
#define glEnableVertexAttribAPPLE                                              \
  GLEGetCurrentFunction(glEnableVertexAttribAPPLE)
#define glIsVertexAttribEnabledAPPLE                                           \
  GLEGetCurrentFunction(glIsVertexAttribEnabledAPPLE)
...
typedef void(GLAPIENTRY *GLDEBUGPROCARB)(GLenum source, GLenum type, GLuint id,
                                         GLenum severity, GLsizei length,
                                         const GLchar *message,
...
#define glDebugMessageCallbackARB                                              \
  GLEGetCurrentFunction(glDebugMessageCallbackARB)
...
#define GLE_ARB_depth_buffer_float                                             \
  GLEGetCurrentVariable(gle_ARB_depth_buffer_float)
#endif
...
/* Disabled until needed
    ...
    typedef void (GLAPIENTRY * PFNGLBINDTEXTUREUNITPROC) (GLuint unit, GLuint
texture); typedef void (GLAPIENTRY * PFNGLBLITNAMEDFRAMEBUFFERPROC) (GLuint
readFramebuffer, GLuint drawFramebuffer, GLint srcX0, GLint srcY0, GLint srcX1,
GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, GLbitfield
mask, GLenum filter); typedef GLenum (GLAPIENTRY *
PFNGLCHECKNAMEDFRAMEBUFFERSTATUSPROC) (GLuint framebuffer, GLenum target);
    typedef void (GLAPIENTRY * PFNGLCLEARNAMEDBUFFERDATAPROC) (GLuint buffer,
GLenum internalformat, GLenum format, GLenum type, const void *data); typedef
void (GLAPIENTRY * PFNGLCLEARNAMEDBUFFERSUBDATAPROC) (GLuint buffer, GLenum
internalformat, GLintptr offset, GLsizeiptr size, GLenum format, GLenum type,
const void *data); typedef void (GLAPIENTRY * PFNGLCLEARNAMEDFRAMEBUFFERFIPROC)
(GLuint framebuffer, GLenum buffer, GLfloat depth, GLint stencil); typedef void
(GLAPIENTRY * PFNGLCLEARNAMEDFRAMEBUFFERFVPROC) (GLuint framebuffer, GLenum
buffer, GLint drawbuffer, const GLfloat* value); typedef void (GLAPIENTRY *
PFNGLCLEARNAMEDFRAMEBUFFERIVPROC) (GLuint framebuffer, GLenum buffer, GLint
drawbuffer, const GLint* value); typedef void (GLAPIENTRY *
PFNGLCLEARNAMEDFRAMEBUFFERUIVPROC) (GLuint framebuffer, GLenum buffer, GLint
drawbuffer, const GLuint* value); typedef void (GLAPIENTRY *
PFNGLCOMPRESSEDTEXTURESUBIMAGE1DPROC) (GLuint texture, GLint level, GLint
xoffset, GLsizei width, GLenum format, GLsizei imageSize, const void *data);
    typedef void (GLAPIENTRY * PFNGLCOMPRESSEDTEXTURESUBIMAGE2DPROC) (GLuint
texture, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei
height, GLenum format, GLsizei imageSize, const void *data); typedef void
(GLAPIENTRY * PFNGLCOMPRESSEDTEXTURESUBIMAGE3DPROC) (GLuint texture, GLint
...
height, GLsizei depth, GLenum format, GLsizei imageSize, const void *data);
    typedef void (GLAPIENTRY * PFNGLCOPYNAMEDBUFFERSUBDATAPROC) (GLuint
readBuffer, GLuint writeBuffer, GLintptr readOffset, GLintptr writeOffset,
GLsizeiptr size); typedef void (GLAPIENTRY * PFNGLCOPYTEXTURESUBIMAGE1DPROC)
(GLuint texture, GLint level, GLint xoffset, GLint x, GLint y, GLsizei width);
    typedef void (GLAPIENTRY * PFNGLCOPYTEXTURESUBIMAGE2DPROC) (GLuint texture,
GLint level, GLint xoffset, GLint yoffset, GLint x, GLint y, GLsizei width,
GLsizei height); typedef void (GLAPIENTRY * PFNGLCOPYTEXTURESUBIMAGE3DPROC)
(GLuint texture, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLint
x, GLint y, GLsizei width, GLsizei height); typedef void (GLAPIENTRY *
PFNGLCREATEBUFFERSPROC) (GLsizei n, GLuint* buffers); typedef void (GLAPIENTRY *
PFNGLCREATEFRAMEBUFFERSPROC) (GLsizei n, GLuint* framebuffers); typedef void
(GLAPIENTRY * PFNGLCREATEPROGRAMPIPELINESPROC) (GLsizei n, GLuint* pipelines);
    typedef void (GLAPIENTRY * PFNGLCREATEQUERIESPROC) (GLenum target, GLsizei
n, GLuint* ids); typedef void (GLAPIENTRY * PFNGLCREATERENDERBUFFERSPROC)
(GLsizei n, GLuint* renderbuffers); typedef void (GLAPIENTRY *
PFNGLCREATESAMPLERSPROC) (GLsizei n, GLuint* samplers); typedef void (GLAPIENTRY
* PFNGLCREATETEXTURESPROC) (GLenum target, GLsizei n, GLuint* textures); typedef
void (GLAPIENTRY * PFNGLCREATETRANSFORMFEEDBACKSPROC) (GLsizei n, GLuint* ids);
    typedef void (GLAPIENTRY * PFNGLCREATEVERTEXARRAYSPROC) (GLsizei n, GLuint*
arrays); typedef void (GLAPIENTRY * PFNGLDISABLEVERTEXARRAYATTRIBPROC) (GLuint
vaobj, GLuint index); typedef void (GLAPIENTRY *
PFNGLENABLEVERTEXARRAYATTRIBPROC) (GLuint vaobj, GLuint index); typedef void
(GLAPIENTRY * PFNGLFLUSHMAPPEDNAMEDBUFFERRANGEPROC) (GLuint buffer, GLintptr
offset, GLsizeiptr length); typedef void (GLAPIENTRY *
PFNGLGENERATETEXTUREMIPMAPPROC) (GLuint texture); typedef void (GLAPIENTRY *
PFNGLGETCOMPRESSEDTEXTUREIMAGEPROC) (GLuint texture, GLint level, GLsizei
bufSize, void *pixels); typedef void (GLAPIENTRY *
PFNGLGETNAMEDBUFFERPARAMETERI64VPROC) (GLuint buffer, GLenum pname, GLint64*
params); typedef void (GLAPIENTRY * PFNGLGETNAMEDBUFFERPARAMETERIVPROC) (GLuint
buffer, GLenum pname, GLint* params); typedef void (GLAPIENTRY *
PFNGLGETNAMEDBUFFERPOINTERVPROC) (GLuint buffer, GLenum pname, void** params);
    typedef void (GLAPIENTRY * PFNGLGETNAMEDBUFFERSUBDATAPROC) (GLuint buffer,
GLintptr offset, GLsizeiptr size, void *data); typedef void (GLAPIENTRY *
PFNGLGETNAMEDFRAMEBUFFERATTACHMENTPARAMETERIVPROC) (GLuint framebuffer, GLenum
attachment, GLenum pname, GLint* params); typedef void (GLAPIENTRY *
PFNGLGETNAMEDFRAMEBUFFERPARAMETERIVPROC) (GLuint framebuffer, GLenum pname,
GLint* param); typedef void (GLAPIENTRY *
PFNGLGETNAMEDRENDERBUFFERPARAMETERIVPROC) (GLuint renderbuffer, GLenum pname,
GLint* params); typedef void (GLAPIENTRY * PFNGLGETTEXTUREIMAGEPROC) (GLuint
texture, GLint level, GLenum format, GLenum type, GLsizei bufSize, void
*pixels); typedef void (GLAPIENTRY * PFNGLGETTEXTURELEVELPARAMETERFVPROC)
(GLuint texture, GLint level, GLenum pname, GLfloat* params); typedef void
(GLAPIENTRY * PFNGLGETTEXTURELEVELPARAMETERIVPROC) (GLuint texture, GLint level,
GLenum pname, GLint* params); typedef void (GLAPIENTRY *
PFNGLGETTEXTUREPARAMETERIIVPROC) (GLuint texture, GLenum pname, GLint* params);
    typedef void (GLAPIENTRY * PFNGLGETTEXTUREPARAMETERIUIVPROC) (GLuint
texture, GLenum pname, GLuint* params); typedef void (GLAPIENTRY *
PFNGLGETTEXTUREPARAMETERFVPROC) (GLuint texture, GLenum pname, GLfloat* params);
    typedef void (GLAPIENTRY * PFNGLGETTEXTUREPARAMETERIVPROC) (GLuint texture,
GLenum pname, GLint* params); typedef void (GLAPIENTRY *
PFNGLGETTRANSFORMFEEDBACKI64_VPROC) (GLuint xfb, GLenum pname, GLuint index,
GLint64* param); typedef void (GLAPIENTRY * PFNGLGETTRANSFORMFEEDBACKI_VPROC)
(GLuint xfb, GLenum pname, GLuint index, GLint* param); typedef void (GLAPIENTRY
* PFNGLGETTRANSFORMFEEDBACKIVPROC) (GLuint xfb, GLenum pname, GLint* param);
    typedef void (GLAPIENTRY * PFNGLGETVERTEXARRAYINDEXED64IVPROC) (GLuint
vaobj, GLuint index, GLenum pname, GLint64* param); typedef void (GLAPIENTRY *
PFNGLGETVERTEXARRAYINDEXEDIVPROC) (GLuint vaobj, GLuint index, GLenum pname,
GLint* param); typedef void (GLAPIENTRY * PFNGLGETVERTEXARRAYIVPROC) (GLuint
vaobj, GLenum pname, GLint* param); typedef void (GLAPIENTRY *
PFNGLINVALIDATENAMEDFRAMEBUFFERDATAPROC) (GLuint framebuffer, GLsizei
numAttachments, const GLenum* attachments); typedef void (GLAPIENTRY *
PFNGLINVALIDATENAMEDFRAMEBUFFERSUBDATAPROC) (GLuint framebuffer, GLsizei
numAttachments, const GLenum* attachments, GLint x, GLint y, GLsizei width,
GLsizei height); typedef void * (GLAPIENTRY * PFNGLMAPNAMEDBUFFERPROC) (GLuint
buffer, GLenum access); typedef void * (GLAPIENTRY *
PFNGLMAPNAMEDBUFFERRANGEPROC) (GLuint buffer, GLintptr offset, GLsizeiptr
length, GLbitfield access); typedef void (GLAPIENTRY * PFNGLNAMEDBUFFERDATAPROC)
(GLuint buffer, GLsizeiptr size, const void *data, GLenum usage); typedef void
(GLAPIENTRY * PFNGLNAMEDBUFFERSTORAGEPROC) (GLuint buffer, GLsizeiptr size,
const void *data, GLbitfield flags); typedef void (GLAPIENTRY *
PFNGLNAMEDBUFFERSUBDATAPROC) (GLuint buffer, GLintptr offset, GLsizeiptr size,
const void *data); typedef void (GLAPIENTRY *
PFNGLNAMEDFRAMEBUFFERDRAWBUFFERPROC) (GLuint framebuffer, GLenum mode); typedef
void (GLAPIENTRY * PFNGLNAMEDFRAMEBUFFERDRAWBUFFERSPROC) (GLuint framebuffer,
GLsizei n, const GLenum* bufs); typedef void (GLAPIENTRY *
PFNGLNAMEDFRAMEBUFFERPARAMETERIPROC) (GLuint framebuffer, GLenum pname, GLint
param); typedef void (GLAPIENTRY * PFNGLNAMEDFRAMEBUFFERREADBUFFERPROC) (GLuint
framebuffer, GLenum mode); typedef void (GLAPIENTRY *
PFNGLNAMEDFRAMEBUFFERRENDERBUFFERPROC) (GLuint framebuffer, GLenum attachment,
GLenum renderbuffertarget, GLuint renderbuffer); typedef void (GLAPIENTRY *
PFNGLNAMEDFRAMEBUFFERTEXTUREPROC) (GLuint framebuffer, GLenum attachment, GLuint
texture, GLint level); typedef void (GLAPIENTRY *
PFNGLNAMEDFRAMEBUFFERTEXTURELAYERPROC) (GLuint framebuffer, GLenum attachment,
GLuint texture, GLint level, GLint layer); typedef void (GLAPIENTRY *
PFNGLNAMEDRENDERBUFFERSTORAGEPROC) (GLuint renderbuffer, GLenum internalformat,
GLsizei width, GLsizei height); typedef void (GLAPIENTRY *
PFNGLNAMEDRENDERBUFFERSTORAGEMULTISAMPLEPROC) (GLuint renderbuffer, GLsizei
samples, GLenum internalformat, GLsizei width, GLsizei height); typedef void
(GLAPIENTRY * PFNGLTEXTUREBUFFERPROC) (GLuint texture, GLenum internalformat,
GLuint buffer); typedef void (GLAPIENTRY * PFNGLTEXTUREBUFFERRANGEPROC) (GLuint
texture, GLenum internalformat, GLuint buffer, GLintptr offset, GLsizeiptr
size); typedef void (GLAPIENTRY * PFNGLTEXTUREPARAMETERIIVPROC) (GLuint texture,
GLenum pname, const GLint* params); typedef void (GLAPIENTRY *
PFNGLTEXTUREPARAMETERIUIVPROC) (GLuint texture, GLenum pname, const GLuint*
params); typedef void (GLAPIENTRY * PFNGLTEXTUREPARAMETERFPROC) (GLuint texture,
GLenum pname, GLfloat param); typedef void (GLAPIENTRY *
PFNGLTEXTUREPARAMETERFVPROC) (GLuint texture, GLenum pname, const GLfloat*
param); typedef void (GLAPIENTRY * PFNGLTEXTUREPARAMETERIPROC) (GLuint texture,
GLenum pname, GLint param); typedef void (GLAPIENTRY *
PFNGLTEXTUREPARAMETERIVPROC) (GLuint texture, GLenum pname, const GLint* param);
    typedef void (GLAPIENTRY * PFNGLTEXTURESTORAGE1DPROC) (GLuint texture,
GLsizei levels, GLenum internalformat, GLsizei width); typedef void (GLAPIENTRY
* PFNGLTEXTURESTORAGE2DPROC) (GLuint texture, GLsizei levels, GLenum
internalformat, GLsizei width, GLsizei height); typedef void (GLAPIENTRY *
PFNGLTEXTURESTORAGE2DMULTISAMPLEPROC) (GLuint texture, GLsizei samples, GLenum
internalformat, GLsizei width, GLsizei height, GLboolean fixedsamplelocations);
    typedef void (GLAPIENTRY * PFNGLTEXTURESTORAGE3DPROC) (GLuint texture,
GLsizei levels, GLenum internalformat, GLsizei width, GLsizei height, GLsizei
depth); typedef void (GLAPIENTRY * PFNGLTEXTURESTORAGE3DMULTISAMPLEPROC) (GLuint
texture, GLsizei samples, GLenum internalformat, GLsizei width, GLsizei height,
GLsizei depth, GLboolean fixedsamplelocations); typedef void (GLAPIENTRY *
PFNGLTEXTURESUBIMAGE1DPROC) (GLuint texture, GLint level, GLint xoffset, GLsizei
width, GLenum format, GLenum type, const void *pixels); typedef void (GLAPIENTRY
* PFNGLTEXTURESUBIMAGE2DPROC) (GLuint texture, GLint level, GLint xoffset, GLint
yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const void
*pixels); typedef void (GLAPIENTRY * PFNGLTEXTURESUBIMAGE3DPROC) (GLuint
texture, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei
width, GLsizei height, GLsizei depth, GLenum format, GLenum type, const void
*pixels); typedef void (GLAPIENTRY * PFNGLTRANSFORMFEEDBACKBUFFERBASEPROC)
(GLuint xfb, GLuint index, GLuint buffer); typedef void (GLAPIENTRY *
PFNGLTRANSFORMFEEDBACKBUFFERRANGEPROC) (GLuint xfb, GLuint index, GLuint buffer,
GLintptr offset, GLsizeiptr size); typedef GLboolean (GLAPIENTRY *
PFNGLUNMAPNAMEDBUFFERPROC) (GLuint buffer); typedef void (GLAPIENTRY *
PFNGLVERTEXARRAYATTRIBBINDINGPROC) (GLuint vaobj, GLuint attribindex, GLuint
bindingindex); typedef void (GLAPIENTRY * PFNGLVERTEXARRAYATTRIBFORMATPROC)
(GLuint vaobj, GLuint attribindex, GLint size, GLenum type, GLboolean
normalized, GLuint relativeoffset); typedef void (GLAPIENTRY *
PFNGLVERTEXARRAYATTRIBIFORMATPROC) (GLuint vaobj, GLuint attribindex, GLint
size, GLenum type, GLuint relativeoffset); typedef void (GLAPIENTRY *
PFNGLVERTEXARRAYATTRIBLFORMATPROC) (GLuint vaobj, GLuint attribindex, GLint
size, GLenum type, GLuint relativeoffset); typedef void (GLAPIENTRY *
PFNGLVERTEXARRAYBINDINGDIVISORPROC) (GLuint vaobj, GLuint bindingindex, GLuint
divisor); typedef void (GLAPIENTRY * PFNGLVERTEXARRAYELEMENTBUFFERPROC) (GLuint
vaobj, GLuint buffer); typedef void (GLAPIENTRY *
PFNGLVERTEXARRAYVERTEXBUFFERPROC) (GLuint vaobj, GLuint bindingindex, GLuint
buffer, GLintptr offset, GLsizei stride); typedef void (GLAPIENTRY *
PFNGLVERTEXARRAYVERTEXBUFFERSPROC) (GLuint vaobj, GLuint first, GLsizei count,
...
GLEGetCurrentFunction(glCheckNamedFramebufferStatus) #define
glClearNamedBufferData GLEGetCurrentFunction(glClearNamedBufferData) #define
glClearNamedBufferSubData GLEGetCurrentFunction(glClearNamedBufferSubData)
    #define glClearNamedFramebufferfi
GLEGetCurrentFunction(glClearNamedFramebufferfi) #define
glClearNamedFramebufferfv GLEGetCurrentFunction(glClearNamedFramebufferfv)
    #define glClearNamedFramebufferiv
GLEGetCurrentFunction(glClearNamedFramebufferiv) #define
glClearNamedFramebufferuiv GLEGetCurrentFunction(glClearNamedFramebufferuiv)
    #define glCompressedTextureSubImage1D
GLEGetCurrentFunction(glCompressedTextureSubImage1D) #define
glCompressedTextureSubImage2D
GLEGetCurrentFunction(glCompressedTextureSubImage2D) #define
glCompressedTextureSubImage3D
GLEGetCurrentFunction(glCompressedTextureSubImage3D) #define
glCopyNamedBufferSubData GLEGetCurrentFunction(glCopyNamedBufferSubData) #define
glCopyTextureSubImage1D GLEGetCurrentFunction(glCopyTextureSubImage1D) #define
glCopyTextureSubImage2D GLEGetCurrentFunction(glCopyTextureSubImage2D) #define
glCopyTextureSubImage3D GLEGetCurrentFunction(glCopyTextureSubImage3D) #define
glCreateBuffers GLEGetCurrentFunction(glCreateBuffers) #define
glCreateFramebuffers GLEGetCurrentFunction(glCreateFramebuffers) #define
glCreateProgramPipelines GLEGetCurrentFunction(glCreateProgramPipelines) #define
glCreateQueries GLEGetCurrentFunction(glCreateQueries) #define
glCreateRenderbuffers GLEGetCurrentFunction(glCreateRenderbuffers) #define
glCreateSamplers GLEGetCurrentFunction(glCreateSamplers) #define
glCreateTextures GLEGetCurrentFunction(glCreateTextures) #define
glCreateTransformFeedbacks GLEGetCurrentFunction(glCreateTransformFeedbacks)
    #define glCreateVertexArrays GLEGetCurrentFunction(glCreateVertexArrays)
    #define glDisableVertexArrayAttrib
GLEGetCurrentFunction(glDisableVertexArrayAttrib) #define
glEnableVertexArrayAttrib GLEGetCurrentFunction(glEnableVertexArrayAttrib)
    #define glFlushMappedNamedBufferRange
GLEGetCurrentFunction(glFlushMappedNamedBufferRange) #define
glGenerateTextureMipmap GLEGetCurrentFunction(glGenerateTextureMipmap) #define
glGetCompressedTextureImage GLEGetCurrentFunction(glGetCompressedTextureImage)
    #define glGetNamedBufferParameteri64v
GLEGetCurrentFunction(glGetNamedBufferParameteri64v) #define
glGetNamedBufferParameteriv GLEGetCurrentFunction(glGetNamedBufferParameteriv)
    #define glGetNamedBufferPointerv
GLEGetCurrentFunction(glGetNamedBufferPointerv) #define glGetNamedBufferSubData
GLEGetCurrentFunction(glGetNamedBufferSubData) #define
glGetNamedFramebufferAttachmentParameteriv
GLEGetCurrentFunction(glGetNamedFramebufferAttachmentParameteriv) #define
glGetNamedFramebufferParameteriv
GLEGetCurrentFunction(glGetNamedFramebufferParameteriv) #define
glGetNamedRenderbufferParameteriv
GLEGetCurrentFunction(glGetNamedRenderbufferParameteriv) #define
glGetTextureImage GLEGetCurrentFunction(glGetTextureImage) #define
glGetTextureLevelParameterfv GLEGetCurrentFunction(glGetTextureLevelParameterfv)
    #define glGetTextureLevelParameteriv
GLEGetCurrentFunction(glGetTextureLevelParameteriv) #define
glGetTextureParameterIiv GLEGetCurrentFunction(glGetTextureParameterIiv) #define
glGetTextureParameterIuiv GLEGetCurrentFunction(glGetTextureParameterIuiv)
    #define glGetTextureParameterfv
GLEGetCurrentFunction(glGetTextureParameterfv) #define glGetTextureParameteriv
GLEGetCurrentFunction(glGetTextureParameteriv) #define
glGetTransformFeedbacki64_v GLEGetCurrentFunction(glGetTransformFeedbacki64_v)
    #define glGetTransformFeedbacki_v
GLEGetCurrentFunction(glGetTransformFeedbacki_v) #define
glGetTransformFeedbackiv GLEGetCurrentFunction(glGetTransformFeedbackiv) #define
glGetVertexArrayIndexed64iv GLEGetCurrentFunction(glGetVertexArrayIndexed64iv)
    #define glGetVertexArrayIndexediv
GLEGetCurrentFunction(glGetVertexArrayIndexediv) #define glGetVertexArrayiv
GLEGetCurrentFunction(glGetVertexArrayiv) #define
glInvalidateNamedFramebufferData
GLEGetCurrentFunction(glInvalidateNamedFramebufferData) #define
glInvalidateNamedFramebufferSubData
GLEGetCurrentFunction(glInvalidateNamedFramebufferSubData) #define
glMapNamedBuffer GLEGetCurrentFunction(glMapNamedBuffer) #define
glMapNamedBufferRange GLEGetCurrentFunction(glMapNamedBufferRange) #define
glNamedBufferData GLEGetCurrentFunction(glNamedBufferData) #define
glNamedBufferStorage GLEGetCurrentFunction(glNamedBufferStorage) #define
glNamedBufferSubData GLEGetCurrentFunction(glNamedBufferSubData) #define
glNamedFramebufferDrawBuffer GLEGetCurrentFunction(glNamedFramebufferDrawBuffer)
    #define glNamedFramebufferDrawBuffers
GLEGetCurrentFunction(glNamedFramebufferDrawBuffers) #define
glNamedFramebufferParameteri GLEGetCurrentFunction(glNamedFramebufferParameteri)
    #define glNamedFramebufferReadBuffer
GLEGetCurrentFunction(glNamedFramebufferReadBuffer) #define
glNamedFramebufferRenderbuffer
GLEGetCurrentFunction(glNamedFramebufferRenderbuffer) #define
glNamedFramebufferTexture GLEGetCurrentFunction(glNamedFramebufferTexture)
    #define glNamedFramebufferTextureLayer
GLEGetCurrentFunction(glNamedFramebufferTextureLayer) #define
glNamedRenderbufferStorage GLEGetCurrentFunction(glNamedRenderbufferStorage)
    #define glNamedRenderbufferStorageMultisample
GLEGetCurrentFunction(glNamedRenderbufferStorageMultisample) #define
glTextureBuffer GLEGetCurrentFunction(glTextureBuffer) #define
glTextureBufferRange GLEGetCurrentFunction(glTextureBufferRange) #define
glTextureParameterIiv GLEGetCurrentFunction(glTextureParameterIiv) #define
glTextureParameterIuiv GLEGetCurrentFunction(glTextureParameterIuiv) #define
glTextureParameterf GLEGetCurrentFunction(glTextureParameterf) #define
glTextureParameterfv GLEGetCurrentFunction(glTextureParameterfv) #define
glTextureParameteri GLEGetCurrentFunction(glTextureParameteri) #define
glTextureParameteriv GLEGetCurrentFunction(glTextureParameteriv) #define
glTextureStorage1D GLEGetCurrentFunction(glTextureStorage1D) #define
glTextureStorage2D GLEGetCurrentFunction(glTextureStorage2D) #define
glTextureStorage2DMultisample
GLEGetCurrentFunction(glTextureStorage2DMultisample) #define glTextureStorage3D
GLEGetCurrentFunction(glTextureStorage3D) #define glTextureStorage3DMultisample
GLEGetCurrentFunction(glTextureStorage3DMultisample) #define glTextureSubImage1D
GLEGetCurrentFunction(glTextureSubImage1D) #define glTextureSubImage2D
GLEGetCurrentFunction(glTextureSubImage2D) #define glTextureSubImage3D
GLEGetCurrentFunction(glTextureSubImage3D) #define glTransformFeedbackBufferBase
GLEGetCurrentFunction(glTransformFeedbackBufferBase) #define
glTransformFeedbackBufferRange
GLEGetCurrentFunction(glTransformFeedbackBufferRange) #define glUnmapNamedBuffer
GLEGetCurrentFunction(glUnmapNamedBuffer) #define glVertexArrayAttribBinding
GLEGetCurrentFunction(glVertexArrayAttribBinding) #define
glVertexArrayAttribFormat GLEGetCurrentFunction(glVertexArrayAttribFormat)
    #define glVertexArrayAttribIFormat
GLEGetCurrentFunction(glVertexArrayAttribIFormat) #define
glVertexArrayAttribLFormat GLEGetCurrentFunction(glVertexArrayAttribLFormat)
    #define glVertexArrayBindingDivisor
GLEGetCurrentFunction(glVertexArrayBindingDivisor) #define
glVertexArrayElementBuffer GLEGetCurrentFunction(glVertexArrayElementBuffer)
    #define glVertexArrayVertexBuffer
GLEGetCurrentFunction(glVertexArrayVertexBuffer) #define
glVertexArrayVertexBuffers GLEGetCurrentFunction(glVertexArrayVertexBuffers)

    #define GLE_ARB_direct_state_access
GLEGetCurrentVariable(gle_ARB_direct_state_access) #endif //
...
#define glGetShaderPrecisionFormat                                             \
  GLEGetCurrentFunction(glGetShaderPrecisionFormat)
...
#define GLE_ARB_ES2_compatibility                                              \
  GLEGetCurrentVariable(gle_ARB_ES2_compatibility)
#endif
...
#define glFramebufferRenderbuffer                                              \
  GLEGetCurrentFunction(glFramebufferRenderbuffer)
...
#define glFramebufferTextureLayer                                              \
  GLEGetCurrentFunction(glFramebufferTextureLayer)
...
#define glGetRenderbufferParameteriv                                           \
  GLEGetCurrentFunction(glGetRenderbufferParameteriv)
...
#define glRenderbufferStorageMultisample                                       \
  GLEGetCurrentFunction(glRenderbufferStorageMultisample)

#define GLE_ARB_framebuffer_object                                             \
  GLEGetCurrentVariable(gle_ARB_framebuffer_object)
...
#define GLE_ARB_texture_multisample                                            \
  GLEGetCurrentVariable(gle_ARB_texture_multisample)
...
#define GLE_ARB_texture_non_power_of_two                                       \
  GLEGetCurrentVariable(gle_ARB_texture_non_power_of_two)
#endif

#ifndef GL_ARB_texture_rectangle
#define GL_ARB_texture_rectangle 1

// texture_rectangle was added to the OpenGL 3.1 core profile and so this
// extension is not needed unless using an earlier version of OpenGL. There are
// also the GL_EXT_texture_rectangle and GL_NV_texture_rectangle extensions.
// Apple reports the preseence of GL_EXT_texture_rectangle but not
// GL_ARB_texture_rectangle or GL_NV_texture_rectangle. You should check for
// GL_ARB_texture_rectangle instead of these other two.
...
#define GLE_ARB_texture_rectangle                                              \
  GLEGetCurrentVariable(gle_ARB_texture_rectangle)
#endif
...
typedef void(GLAPIENTRY *PFNGLTEXSTORAGE1DPROC)(GLenum target, GLsizei levels,
                                                GLenum internalformat,
                                                GLsizei width);
typedef void(GLAPIENTRY *PFNGLTEXSTORAGE2DPROC)(GLenum target, GLsizei levels,
                                                GLenum internalformat,
                                                GLsizei width, GLsizei height);
typedef void(GLAPIENTRY *PFNGLTEXSTORAGE3DPROC)(GLenum target, GLsizei levels,
                                                GLenum internalformat,
                                                GLsizei width, GLsizei height,
                                                GLsizei depth);
typedef void(GLAPIENTRY *PFNGLTEXTURESTORAGE1DEXTPROC)(GLuint texture,
                                                       GLenum target,
                                                       GLsizei levels,
                                                       GLenum internalformat,
                                                       GLsizei width);
...
#define glTexStorage2DMultisample                                              \
  GLEGetCurrentFunction(glTexStorage2DMultisample)
#define glTexStorage3DMultisample                                              \
  GLEGetCurrentFunction(glTexStorage3DMultisample)
#define glTextureStorage2DMultisampleEXT                                       \
  GLEGetCurrentFunction(glTextureStorage2DMultisampleEXT)
#define glTextureStorage3DMultisampleEXT                                       \
  GLEGetCurrentFunction(glTextureStorage3DMultisampleEXT)
...
#define GLE_ARB_vertex_array_object                                            \
  GLEGetCurrentVariable(gle_ARB_vertex_array_object)
#endif
...
GLuint buffer, GLintptr offset, GLsizei stride); typedef void (GLAPIENTRY *
PFNGLVERTEXARRAYBINDVERTEXBUFFEREXTPROC) (GLuint vaobj, GLuint bindingindex,
GLuint buffer, GLintptr offset, GLsizei stride); typedef void (GLAPIENTRY *
PFNGLVERTEXARRAYVERTEXATTRIBBINDINGEXTPROC) (GLuint vaobj, GLuint attribindex,
GLuint bindingindex); typedef void (GLAPIENTRY *
PFNGLVERTEXARRAYVERTEXATTRIBFORMATEXTPROC) (GLuint vaobj, GLuint attribindex,
GLint size, GLenum type, GLboolean normalized, GLuint relativeoffset); typedef
void (GLAPIENTRY * PFNGLVERTEXARRAYVERTEXATTRIBIFORMATEXTPROC) (GLuint vaobj,
GLuint attribindex, GLint size, GLenum type, GLuint relativeoffset); typedef
void (GLAPIENTRY * PFNGLVERTEXARRAYVERTEXATTRIBLFORMATEXTPROC) (GLuint vaobj,
GLuint attribindex, GLint size, GLenum type, GLuint relativeoffset); typedef
void (GLAPIENTRY * PFNGLVERTEXARRAYVERTEXBINDINGDIVISOREXTPROC) (GLuint vaobj,
GLuint bindingindex, GLuint divisor); typedef void (GLAPIENTRY *
PFNGLVERTEXATTRIBBINDINGPROC) (GLuint attribindex, GLuint bindingindex); typedef
void (GLAPIENTRY * PFNGLVERTEXATTRIBFORMATPROC) (GLuint attribindex, GLint size,
GLenum type, GLboolean normalized, GLuint relativeoffset); typedef void
(GLAPIENTRY * PFNGLVERTEXATTRIBIFORMATPROC) (GLuint attribindex, GLint size,
GLenum type, GLuint relativeoffset); typedef void (GLAPIENTRY *
PFNGLVERTEXATTRIBLFORMATPROC) (GLuint attribindex, GLint size, GLenum type,
GLuint relativeoffset); typedef void (GLAPIENTRY *
...
GLEGetCurrentFunction(glVertexArrayBindVertexBufferEXT) #define
glVertexArrayVertexAttribBindingEXT
GLEGetCurrentFunction(glVertexArrayVertexAttribBindingEXT) #define
glVertexArrayVertexAttribFormatEXT
GLEGetCurrentFunction(glVertexArrayVertexAttribFormatEXT) #define
glVertexArrayVertexAttribIFormatEXT
GLEGetCurrentFunction(glVertexArrayVertexAttribIFormatEXT) #define
glVertexArrayVertexAttribLFormatEXT
GLEGetCurrentFunction(glVertexArrayVertexAttribLFormatEXT) #define
glVertexArrayVertexBindingDivisorEXT
GLEGetCurrentFunction(glVertexArrayVertexBindingDivisorEXT) #define
glVertexAttribBinding GLEGetCurrentFunction(glVertexAttribBinding) #define
glVertexAttribFormat GLEGetCurrentFunction(glVertexAttribFormat) #define
glVertexAttribIFormat GLEGetCurrentFunction(glVertexAttribIFormat) #define
glVertexAttribLFormat GLEGetCurrentFunction(glVertexAttribLFormat) #define
glVertexBindingDivisor GLEGetCurrentFunction(glVertexBindingDivisor)

    #define GLE_ARB_vertex_attrib_binding
GLEGetCurrentVariable(gle_ARB_vertex_attrib_binding) #endif
*/
...
#define GLE_EXT_texture_compression_s3tc                                       \
  GLEGetCurrentVariable(gle_EXT_texture_compression_s3tc)
#endif
...
#define GLE_EXT_texture_filter_anisotropic                                     \
  GLEGetCurrentVariable(gle_EXT_texture_filter_anisotropic)
#endif
...
/* Disabled until needed
    ...
    #define GLE_KHR_context_flush_control
GLEGetCurrentVariable(gle_KHR_context_flush_control) #endif
*/
...
typedef GLuint(GLAPIENTRY *PFNGLGETDEBUGMESSAGELOGPROC)(
    GLuint count, GLsizei bufSize, GLenum *sources, GLenum *types, GLuint *ids,
    GLenum *severities, GLsizei *lengths, GLchar *messageLog);
typedef void(GLAPIENTRY *PFNGLGETOBJECTLABELPROC)(GLenum identifier,
                                                  GLuint name, GLsizei bufSize,
                                                  GLsizei *length,
                                                  GLchar *label);
typedef void(GLAPIENTRY *PFNGLGETOBJECTPTRLABELPROC)(const void *ptr,
                                                     GLsizei bufSize,
                                                     GLsizei *length,
                                                     GLchar *label);
typedef void(GLAPIENTRY *PFNGLOBJECTLABELPROC)(GLenum identifier, GLuint name,
                                               GLsizei length,
                                               const GLchar *label);
typedef void(GLAPIENTRY *PFNGLOBJECTPTRLABELPROC)(const void *ptr,
                                                  GLsizei length,
                                                  const GLchar *label);
typedef void(GLAPIENTRY *PFNGLPOPDEBUGGROUPPROC)(void);
typedef void(GLAPIENTRY *PFNGLPUSHDEBUGGROUPPROC)(GLenum source, GLuint id,
                                                  GLsizei length,
                                                  const GLchar *message);
...
#define GLE_KHR_DebugGLEGetCurrentVariable(gle_KHR_debug)
#endif // GL_KHR_debug
...
/* Disabled until needed
    ...
    typedef void (GLAPIENTRY * PFNGLGETNUNIFORMFVPROC) (GLuint program, GLint
location, GLsizei bufSize, GLfloat* params); typedef void (GLAPIENTRY *
PFNGLGETNUNIFORMIVPROC) (GLuint program, GLint location, GLsizei bufSize, GLint*
params); typedef void (GLAPIENTRY * PFNGLGETNUNIFORMUIVPROC) (GLuint program,
GLint location, GLsizei bufSize, GLuint* params); typedef void (GLAPIENTRY *
PFNGLREADNPIXELSPROC) (GLint x, GLint y, GLsizei width, GLsizei height, GLenum
...
#endif
#define __wglext_h_ // Prevent wglext.h from having any future effect if it's
                    // #included.
...
#define wglCreateContext(...)                                                  \
  GLEGetCurrentFunction(wglCreateContext)(__VA_ARGS__)
#define wglCreateLayerContext(...)                                             \
  GLEGetCurrentFunction(wglCreateLayerContext)(__VA_ARGS__)
#define wglDeleteContext(...)                                                  \
  GLEGetCurrentFunction(wglDeleteContext)(__VA_ARGS__)
#define wglGetCurrentContext(...)                                              \
  GLEGetCurrentFunction(wglGetCurrentContext)(__VA_ARGS__)
#define wglGetCurrentDC(...) GLEGetCurrentFunction(wglGetCurrentDC)(__VA_ARGS__)
#define wglGetProcAddress(...)                                                 \
  GLEGetCurrentFunction(wglGetProcAddress)(__VA_ARGS__)
...
#define wglUseFontBitmapsA(...)                                                \
  GLEGetCurrentFunction(wglUseFontBitmapsA)(__VA_ARGS__)
#define wglUseFontBitmapsW(...)                                                \
  GLEGetCurrentFunction(wglUseFontBitmapsW)(__VA_ARGS__)
#define wglUseFontOutlinesA(...)                                               \
  GLEGetCurrentFunction(wglUseFontOutlinesA)(__VA_ARGS__)
#define wglUseFontOutlinesW(...)                                               \
  GLEGetCurrentFunction(wglUseFontOutlinesW)(__VA_ARGS__)
#define wglDescribeLayerPlane(...)                                             \
  GLEGetCurrentFunction(wglDescribeLayerPlane)(__VA_ARGS__)
#define wglSetLayerPaletteEntries(...)                                         \
  GLEGetCurrentFunction(wglSetLayerPaletteEntries)(__VA_ARGS__)
#define wglGetLayerPaletteEntries(...)                                         \
  GLEGetCurrentFunction(wglGetLayerPaletteEntries)(__VA_ARGS__)
#define wglRealizeLayerPalette(...)                                            \
  GLEGetCurrentFunction(wglRealizeLayerPalette)(__VA_ARGS__)
#define wglSwapLayerBuffers(...)                                               \
  GLEGetCurrentFunction(wglSwapLayerBuffers)(__VA_ARGS__)
#define wglSwapMultipleBuffers(...)                                            \
  GLEGetCurrentFunction(wglSwapMultipleBuffers)(__VA_ARGS__)
#else
                             // The following functions are directly declared in
                             // Microsoft's <wingdi.h> without associated
                             // typedefs, and are exported from Opengl32.dll.
// We can link to them directly through Opengl32.lib/dll (same as OpenGL 1.1
// functions) or we can dynamically link them from OpenGL32.dll at runtime.
...
typedef BOOL(WINAPI *PFNWGLUSEFONTOUTLINESAPROC)(HDC, DWORD, DWORD, DWORD,
                                                 FLOAT, FLOAT, int,
                                                 LPGLYPHMETRICSFLOAT);
typedef BOOL(WINAPI *PFNWGLUSEFONTOUTLINESWPROC)(HDC, DWORD, DWORD, DWORD,
                                                 FLOAT, FLOAT, int,
                                                 LPGLYPHMETRICSFLOAT);
typedef BOOL(WINAPI *PFNWGLDESCRIBELAYERPLANEPROC)(HDC, int, int, UINT,
                                                   LPLAYERPLANEDESCRIPTOR);
typedef int(WINAPI *PFNWGLSETLAYERPALETTEENTRIESPROC)(HDC, int, int, int,
                                                      CONST COLORREF *);
typedef int(WINAPI *PFNWGLGETLAYERPALETTEENTRIESPROC)(HDC, int, int, int,
                                                      COLORREF *);
...
#define wglCreateContext                                                       \
  OVR::GLEContext::GetCurrentContext()->wglCreateContext_Impl
#define wglCreateLayerContext                                                  \
  OVR::GLEContext::GetCurrentContext()->wglCreateLayerContext_Impl
#define wglDeleteContext                                                       \
  OVR::GLEContext::GetCurrentContext()->wglDeleteContext_Impl
#define wglGetCurrentContext                                                   \
  OVR::GLEContext::GetCurrentContext()->wglGetCurrentContext_Impl
#define wglGetCurrentDC                                                        \
  OVR::GLEContext::GetCurrentContext()->wglGetCurrentDC_Impl
#define wglGetProcAddress                                                      \
  OVR::GLEContext::GetCurrentContext()->wglGetProcAddress_Impl
...
#define wglUseFontBitmapsA                                                     \
  OVR::GLEContext::GetCurrentContext()->wglUseFontBitmapsA_Impl
#define wglUseFontBitmapsW                                                     \
  OVR::GLEContext::GetCurrentContext()->wglUseFontBitmapsW_Impl
#define wglUseFontOutlinesA                                                    \
  OVR::GLEContext::GetCurrentContext()->wglUseFontOutlinesA_Impl
#define wglUseFontOutlinesW                                                    \
  OVR::GLEContext::GetCurrentContext()->wglUseFontOutlinesW_Impl
#define wglDescribeLayerPlane                                                  \
  OVR::GLEContext::GetCurrentContext()->wglDescribeLayerPlane_Impl
#define wglSetLayerPaletteEntries                                              \
...
#define wglRealizeLayerPalette                                                 \
  OVR::GLEContext::GetCurrentContext()->wglRealizeLayerPalette_Impl
#define wglSwapLayerBuffers                                                    \
  OVR::GLEContext::GetCurrentContext()->wglSwapLayerBuffers_Impl
#define wglSwapMultipleBuffers                                                 \
  OVR::GLEContext::GetCurrentContext()->wglSwapMultipleBuffers_Impl
#endif
#endif

// Note: In order to detect the WGL extensions' availability, we need to call
// wglGetExtensionsStringARB or wglGetExtensionsStringEXT instead of
...
#define wglRestoreBufferRegionARB                                              \
  GLEGetCurrentFunction(wglRestoreBufferRegionARB)

#define GLE_WGL_ARB_buffer_region                                              \
  GLEGetCurrentVariable(gle_WGL_ARB_buffer_region)
#endif
...
#define wglGetExtensionsStringARB                                              \
  GLEGetCurrentFunction(wglGetExtensionsStringARB)

#define GLE_WGL_ARB_extensions_string                                          \
  GLEGetCurrentVariable(gle_WGL_ARB_extensions_string)
#endif
...
#define wglGetPixelFormatAttribivARB                                           \
  GLEGetCurrentFunction(wglGetPixelFormatAttribivARB)
#define wglGetPixelFormatAttribfvARB                                           \
  GLEGetCurrentFunction(wglGetPixelFormatAttribfvARB)
...
#define GLE_WGL_ARB_make_current_read                                          \
  GLEGetCurrentVariable(gle_WGL_ARB_make_current_read)
#endif
...
#define GLE_WGL_ARB_render_texture                                             \
  GLEGetCurrentVariable(gle_WGL_ARB_render_texture)
#endif
...
#define GLE_WGL_ARB_pixel_format_float                                         \
  GLEGetCurrentVariable(gle_WGL_ARB_pixel_format_float)
#endif
...
#define GLE_WGL_ARB_framebuffer_sRGB                                           \
  GLEGetCurrentVariable(gle_WGL_ARB_framebuffer_sRGB)
#endif
...
#define wglEnumerateVideoDevicesNV                                             \
  GLEGetCurrentFunction(wglEnumerateVideoDevicesNV)
...
#define wglCreateContextAttribsARB                                             \
  GLEGetCurrentFunction(wglCreateContextAttribsARB)

#define GLE_WGL_ARB_create_context                                             \
  GLEGetCurrentVariable(gle_WGL_ARB_create_context)
#endif
...
#define GLE_WGL_ARB_create_context_profile                                     \
  GLEGetCurrentVariable(gle_WGL_ARB_create_context_profile)
#endif
...
#define wglGetExtensionsStringEXT                                              \
  GLEGetCurrentFunction(wglGetExtensionsStringEXT)

#define GLE_WGL_EXT_extensions_string                                          \
  GLEGetCurrentVariable(gle_WGL_EXT_extensions_string)
#endif
...
#define wglSwapLayerBuffersMscOML                                              \
  GLEGetCurrentFunction(wglSwapLayerBuffersMscOML)
...
#define wglBindVideoCaptureDeviceNV                                            \
  GLEGetCurrentFunction(wglBindVideoCaptureDeviceNV)
#define wglEnumerateVideoCaptureDevicesNV                                      \
  GLEGetCurrentFunction(wglEnumerateVideoCaptureDevicesNV)
#define wglLockVideoCaptureDeviceNV                                            \
  GLEGetCurrentFunction(wglLockVideoCaptureDeviceNV)
#define wglQueryVideoCaptureDeviceNV                                           \
  GLEGetCurrentFunction(wglQueryVideoCaptureDeviceNV)
#define wglReleaseVideoCaptureDeviceNV                                         \
  GLEGetCurrentFunction(wglReleaseVideoCaptureDeviceNV)
...
#define wglDXSetResourceShareHandleNV                                          \
  GLEGetCurrentFunction(wglDXSetResourceShareHandleNV)
...
#if defined(GLE_CGL_ENABLED)
// We don't currently disable Apple's OpenGL/OpenGL.h and replicate its
// declarations here. We might want to do that if we intended to support hooking
// its functions here like we do for wgl functions.
...
// GLE_HOOKING_ENABLED
// We don't currently support hooking the following GLX 1.0 functions like we do
// with the analagous windows wgl functions. However, we can do this if needed.
// We would just have something like this:
//     #define glXQueryExtension(...)
//     GLEGetCurrentFunction(glXQueryExtension)(__VA_ARGS__)
...
#define glXGetCurrentReadDrawable                                              \
  GLEGetCurrentFunction(glXGetCurrentReadDrawable)
...
#define glXCreateContextAttribsARB                                             \
  GLEGetCurrentFunction(glXCreateContextAttribsARB)

#define GLE_GLX_ARB_create_context                                             \
  GLEGetCurrentVariable(gle_GLX_ARB_create_context)
#endif
...
#define GLE_GLX_ARB_create_context_profile                                     \
  GLEGetCurrentVariable(gle_GLX_ARB_create_context_profile)
#endif
...
// GLX_MESA_swap_control has the same functionality as GLX_EXT_swap_control but
// with a different interface, so we have an independent entry for it here.
...
// Undo some defines, because the user may include <Windows.h> after including
// this header.
